# File handoff plan (first 18 drops)

Each drop is independently shippable and reversible (feature flags). Keep each handoff small: < ~800 LOC net unless it is a migration.

Handoff artifacts should be named with the active workstream:

- `E6-F1-S1__<author>__YYYY-MM-DD__<slug>.bundle`
- `E6-F1-S1__<author>__YYYY-MM-DD__<slug>.patches/`

Use the same sequence and order as this file when planning daily.

## Drop 01: Add shared contracts package

- Add `packages/dispatch-contracts` (types + validators).
- CI: typecheck + unit tests.

## Drop 02: Trace propagation in tool bridge

- Propagate `traceparent`/`tracestate` and map legacy trace header.
- CI: unit tests for header propagation.

## Drop 03: Temporal dev deployment + worker skeleton
...
## Drop 07: Policy bundle loader v0
