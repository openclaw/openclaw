...
import { i as resolveNpmSpecMetadata } from "./install-source-utils-B6zBPP4C.js";
import { n as recordPluginInstall, t as buildNpmResolutionInstallFields } from "./installs-D6Y94Rvd.js";
import { i as parseComparableSemver, t as compareComparableSemver } from "./semver-compare-BcnCrvly.js";
import { o as resolvePluginInstallDir } from "./install-paths-imIYZny-.js";
import { i as resolveBundledPluginSources } from "./bundled-sources-CgwBRzTe.js";
import { i as installPluginFromNpmSpec, t as PLUGIN_INSTALL_ERROR_CODE } from "./install-Cp7oErY_.js";
import { r as installPluginFromClawHub, t as CLAWHUB_INSTALL_ERROR_CODE } from "./clawhub-COSqnTkw.js";
import { t as installPluginFromGitSpec } from "./git-install-DoRFY35s.js";
import { t as resolveChannelAwareNpmSpec } from "./channel-npm-spec-C1D7ijr4.js";
import { t as buildClawHubPluginInstallRecordFields } from "./clawhub-install-records-BsffZr9-.js";
import { t as installPluginFromMarketplace } from "./marketplace-CBZ5Cz2G.js";
...
function shouldFallbackBetaNpmUpdate(result) {
	if (result.code === PLUGIN_INSTALL_ERROR_CODE.NPM_PACKAGE_NOT_FOUND) return true;
	return /\b(ETARGET|notarget)\b|No matching version found|dist-tag|tag .*not found/i.test(result.error);
}
function isDefaultNpmSpecForBetaUpdate(spec) {
...
function resolveNpmUpdateSpecs(params) {
	...
	if (params.specOverride || params.updateChannel !== "beta") return {
...
function isDefaultClawHubSpecForBetaUpdate(spec) {
...
function resolveClawHubUpdateSpecs(params) {
	if (!params.record.clawhubPackage) return {};
...
function isBridgeAlreadyInstalledFromPreferredSource(params) {
	...
	if (npmSpec && params.record.source === "npm" && params.record.spec === npmSpec) return true;
	const clawhubSpec = getExternalizedBundledPluginClawHubSpec(params.bridge);
	return Boolean(clawhubSpec && params.record.source === "clawhub" && params.record.spec === clawhubSpec);
}
function resolveBridgeNpmSpec(bridge, channel) {
	return resolveChannelAwareNpmSpec({
...
async function updateNpmInstalledPlugins(params) {
	...
	for (const pluginId of targets) {
		...
		if (params.dryRun) {
			...
			if (!probe.ok && record.source === "npm" && npmSpecs?.fallbackSpec && shouldFallbackBetaNpmUpdate(probe)) {
				logger.warn?.(`Plugin "${pluginId}" has no beta npm release for ${npmSpecs.fallbackLabel ?? effectiveSpec}; falling back to ${npmSpecs.fallbackSpec}.`);
		...
		if (!result.ok && record.source === "npm" && npmSpecs?.fallbackSpec && shouldFallbackBetaNpmUpdate(result)) {
			logger.warn?.(`Plugin "${pluginId}" has no beta npm release for ${npmSpecs.fallbackLabel ?? effectiveSpec}; falling back to ${npmSpecs.fallbackSpec}.`);
