...
import { c as initSubagentRegistry } from "./subagent-registry-BTp9tQ5h.js";
...
import { a as mergeActivationSectionsIntoRuntimeConfig, i as setFallbackGatewayContextResolver, t as loadGatewayStartupPlugins } from "./server-plugin-bootstrap-CP43DDgl.js";
import { t as loadPluginLookUpTable } from "./plugin-lookup-table-CAeVZQ4U.js";
import { a as getRequiredSharedGatewaySessionGeneration, d as DEFAULT_CHANNEL_CONNECT_GRACE_MS, f as DEFAULT_CHANNEL_STALE_EVENT_THRESHOLD_MS, g as enqueueConfigRecoveryNotice, h as buildGatewayCronService, i as enforceSharedGatewaySessionGenerationForConfigWrite, m as applyGatewayLaneConcurrency, n as disconnectAllSharedGatewayAuthClients, o as setCurrentSharedGatewaySessionGeneration, p as evaluateChannelHealth, r as disconnectStaleSharedGatewayAuthClients, s as activateGatewayScheduledServices, t as resolveHookClientIpConfig, u as startGatewayRuntimeServices } from "./hook-client-ip-config-1u5rXkeQ.js";
...
function getManagedImageAttachmentsModule() {
	managedImageAttachmentsModulePromise ??= import("./managed-image-attachments-DC0_ByiC.js");
...
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-J3zBDbFV.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-CAfKr5aD.js");
	return openResponsesHttpModulePromise;
}
function getSessionHistoryHttpModule() {
	sessionHistoryHttpModulePromise ??= import("./sessions-history-http-h3L5lkwg.js");
	return sessionHistoryHttpModulePromise;
}
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-B7tboOxb.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-DSiKgppP.js");
	return toolsInvokeHttpModulePromise;
}
function getVoiceClawRealtimeUpgradeModule() {
	voiceClawRealtimeUpgradeModulePromise ??= import("./upgrade-CPvusMtw.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-DnhMAdfM.js");
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.restart-sentinel", async () => {
		...
		setTimeout(() => {
			import("./server-restart-sentinel-ijxYyOvr.js").then(({ scheduleRestartSentinelWake }) => scheduleRestartSentinelWake({ deps: params.deps })).catch((err) => {
	...
	await measureStartup(params.startupTrace, "sidecars.subagent-recovery", async () => {
		const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-BlzahzBr.js");
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	refreshLatestUpdateRestartSentinel: async () => (await import("./server-restart-sentinel-ijxYyOvr.js")).refreshLatestUpdateRestartSentinel(),
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-DLLwwCyJ.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
...
async function closeMcpLoopbackServerOnDemand() {
	const { closeMcpLoopbackServer } = await import("./mcp-http-BsHSu3Eu.js");
	...
	try {
		...
		const reloadAttachedGatewayPlugins = async (params) => {
			...
			const [{ loadPluginLookUpTable }, { prepareGatewayPluginLoad }, { startPluginServices }] = await Promise.all([
				...
				import("./server-plugin-bootstrap-KWf0hqDZ.js"),
		...
		if (!minimalTestGateway) {
			if (runtimePluginsLoaded && deferredConfiguredChannelPluginIds.length > 0) {
				const { reloadDeferredGatewayPlugins } = await import("./server-plugin-bootstrap-KWf0hqDZ.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-isCs3HuJ.js");
