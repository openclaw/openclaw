...
import { d as isLikelyExecutionAckPrompt } from "./selection-BdMNZw6y.js";
...
import { r as enqueueCommitmentExtraction } from "./runtime-BID5tkqK.js";
...
import { n as dispatchPostCompactionDelegates, r as persistPendingPostCompactionDelegates } from "./post-compaction-delegate-dispatch-B7bpOwDM.js";
						...
						const result = await runEmbeddedPiAgentDefault$1({
							...
							requestCompactionOpts: runtimeConfig?.agents?.defaults?.continuation?.enabled === true ? {
								...
								triggerCompaction: async (request) => {
									try {
										const { compactEmbeddedPiSession } = await import("./compact.queued-DSFDUoqL.js");
...
const piEmbeddedRuntimeLoader = createLazyImportLoader(() => import("./pi-embedded-CduyT_SY.js"));
		...
		try {
			...
			if (runtimeConfig?.agents?.defaults?.continuation?.enabled === true && sessionKey) {
				const [{ dispatchToolDelegates }, { resolveLiveContinuationRuntimeConfig }, { loadContinuationChainState, persistContinuationChainState }, { updateSessionStore, resolveSessionStoreEntry }] = await Promise.all([
					import("./delegate-dispatch-C9Ht-Ya7.js"),
