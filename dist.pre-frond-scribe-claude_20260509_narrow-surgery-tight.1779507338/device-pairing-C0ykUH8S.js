...
import { a as readJsonIfExists, m as writeJson } from "./json-files-C2hqjU--.js";
import { n as normalizeDeviceAuthScopes } from "./device-auth-CLuEWAWX.js";
import { a as reconcilePendingPairingRequests, c as resolveScopeOutsideRequestedRoles, i as pruneExpiredPending, l as roleScopesAllow, n as verifyPairingToken, o as resolvePairingPaths, r as coercePairingStateRecord, s as resolveMissingRequestedScope, t as generatePairingToken } from "./pairing-token-B8HDR-dd.js";
import { c as revokeDeviceBootstrapTokensForDevice, m as resolveBootstrapProfileScopesForRoles, p as resolveBootstrapProfileScopesForRole } from "./device-bootstrap-Uj_SJFqb.js";
...
function preserveRoleScopedApprovalScopes(role, scopes) {
	...
	for (const scope of scopes) {
		const trimmed = scope.trim();
		if (!trimmed) continue;
		if (role === OPERATOR_ROLE ? trimmed.startsWith(OPERATOR_SCOPE_PREFIX) : !trimmed.startsWith(OPERATOR_SCOPE_PREFIX)) out.add(trimmed);
	}
	return [...out];
}
