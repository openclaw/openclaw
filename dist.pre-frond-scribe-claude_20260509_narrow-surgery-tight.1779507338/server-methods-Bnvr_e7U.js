...
import { s as consumeSubagentTraceparentHandoff } from "./subagent-spawn-Hcc8DJDJ.js";
...
import { d as normalizeCronJobCreate, i as triggerSessionPatchHook, p as normalizeCronJobPatch } from "./openclaw-tools-HooJv9O2.js";
...
import { c as waitForTerminalGatewayDedupe, o as readTerminalSnapshotFromGatewayDedupe, r as chatHandlers, s as setGatewayDedupeEntry } from "./chat-liExAqGT.js";
import { i as parseMessageWithAttachments, n as MediaOffloadError, o as resolveChatAttachmentMaxBytes, t as normalizeRpcAttachmentsToChatAttachments } from "./attachment-normalize-DBhv5pGf.js";
...
import "./heartbeat-runner-CP4-BChX.js";
...
import { n as recordLatestUpdateRestartSentinel, t as getLatestUpdateRestartSentinel } from "./server-restart-sentinel-CrAYbIce.js";
			...
			if (requestedSessionKey) {
				...
				if (storePath) {
					...
					const persisted = await updateSessionStore(storePath, (store) => {
						...
						const merged = mergeSessionEntry(freshEntry, {
							...recoveredSessionStartedAt !== void 0 && freshEntry?.sessionStartedAt === void 0 && freshEntry?.sessionId === entry?.sessionId ? {
								...patchBuild.patch,
			...
			(async () => {
				...
				try {
					...
					const inheritedTraceparent = request.traceparent ?? consumeSubagentTraceparentHandoff({
						idempotencyKey: idem,
						sessionKey: resolvedSessionKey
					})?.traceparent ?? sessionEntry?.continuationTraceparent;
...
const nodeHandlers = {
	...
	"node.event": async ({ params, respond, context, client }) => {
		...
		await respondUnavailableOnThrow(respond, async () => {
			const { handleNodeEvent } = await import("./server-node-events-BmLY_os7.js");
