import { n as isVitestRuntimeEnv } from "./env-BmQxkjR-.js";
...
import { r as startHeartbeatRunner } from "./heartbeat-runner-CP4-BChX.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-CJLBc9pR.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-qNH8iCZ2.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-CDytPwiI.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-XmYXX7tk.js");
		if (stopped) return;
