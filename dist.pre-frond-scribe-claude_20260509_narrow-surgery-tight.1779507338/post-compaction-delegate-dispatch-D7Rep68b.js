import { i as formatErrorMessage$1 } from "./errors-B5idDZn1.js";
...
import { F as resolveSessionStoreEntry, t as loadSessionStore } from "./store-load-DM26fo1a.js";
import { u as updateSessionStore } from "./store-CuGD5gZu.js";
import { t as spawnSubagentDirect } from "./subagent-spawn-Hcc8DJDJ.js";
import { r as hasCrossSessionDelegateTargeting } from "./targeting-pure-DU9HQ1D9.js";
import { t as generateChainId } from "./secure-random-B-x2FMyg.js";
import { a as enqueueSystemEvent } from "./system-events-BqSflfol.js";
import { a as loadPendingSessionDeliveries, c as pruneFailedOlderThan, i as failSessionDelivery, n as enqueuePostCompactionDelegateDelivery, o as loadPendingSessionDelivery, s as moveSessionDeliveryToFailed, t as ackSessionDelivery } from "./session-delivery-queue-storage-DHvN3r6-.js";
import { t as consumeStagedPostCompactionDelegates } from "./continuation-delegate-store-COC6L6XF.js";
import { n as resolveContinuationRuntimeConfig } from "./config-CgkTT-_G.js";
import { n as readPostCompactionContext } from "./post-compaction-context-KMsMaCqE.js";
...
function applyReleaseTraceparent(delegate, releaseTraceparent) {
	if (!releaseTraceparent || delegate.traceparent) return delegate;
	return {
		...delegate,
		traceparent: releaseTraceparent
	};
}
async function deliverQueuedPostCompactionDelegate(params, deps = defaultPostCompactionDelegateDeliveryDeps) {
	...
	const { maxChainLength: maxCompactionChainLength, costCapTokens: compactionCostCapTokens, crossSessionTargeting } = deps.resolveContinuationRuntimeConfig(cfg);
	...
	if (crossSessionTargeting === "disabled" && hasCrossSessionDelegateTargeting(params.entry, params.entry.sessionKey)) {
		deps.log(`Post-compaction delegate rejected: crossSessionTargeting=disabled at delivery time for session ${params.entry.sessionKey}`);
		deps.enqueueSystemEvent(`[continuation] Post-compaction delegate rejected: cross-session targeting was disabled at delivery time. Task: ${params.entry.task}`, {
			sessionKey: params.entry.sessionKey,
			...params.entry.traceparent ? { traceparent: params.entry.traceparent } : {}
		});
		return;
...
async function dispatchPostCompactionDelegates(params, deps = defaultPostCompactionDelegateDispatchDeps) {
	...
	const allCompactionDelegates = [...persistedCompactionDelegates, ...stagedCompactionDelegates].map((delegate) => applyReleaseTraceparent(normalizePostCompactionDelegate(delegate), params.releaseTraceparent));
	const now = deps.now();
...
//#endregion
export { drainPostCompactionDelegateDeliveries as a, takePendingPostCompactionDelegates as c, dispatchPostCompactionDelegates as i, drainPendingSessionDeliveries as l, buildPostCompactionLifecycleEvent as n, normalizePostCompactionDelegate as o, deliverQueuedPostCompactionDelegate as r, persistPendingPostCompactionDelegates as s, POST_COMPACTION_DELEGATE_TTL_MS as t, recoverPendingSessionDeliveries as u };
