...
import { r as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-QLeDpUg6.js";
import "./auth-CCZTHYGp.js";
import { n as resolveGatewayAuth } from "./auth-resolve-DlSgCnyd.js";
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-DvRJ3B33.js";
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-C2X-XHzM.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-y1eDo1e_.js");
	const { MEMORY_SYSTEM_PROMPT, shouldSuggestMemorySystem } = await import("./doctor-workspace-CjI6XYge.js");
	const notes = [];
...
const hooksModelCheck = {
	...
	async detect(ctx) {
		...
		const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } = await import("./model-selection-BodaiM0e.js");
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-BkRFyvNI.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		...
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-DmBLo-i8.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./pi-embedded-helpers-BFwpr69A.js");
...
const claudeCliCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { noteClaudeCliHealth } = await import("./doctor-claude-cli-BYp5HgLc.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./doctor-cron-Dc5zY3yx.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-B3kL-t7U.js");
...
const browserCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { noteChromeMcpBrowserReadiness } = await import("./doctor-browser-DPMVgh-_.js");
...
const workspaceStatusCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyWorkspaceDirs } = await import("./doctor-workspace-CjI6XYge.js");
		const workspaceDir = resolveAgentWorkspaceDir(ctx.cfg, resolveDefaultAgentId(ctx.cfg));
