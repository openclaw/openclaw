...
function commandExists(command) {
  var result = require("child_process").spawnSync("sh", ["-c", "command -v " + command + " >/dev/null 2>&1"], {
    stdio: "ignore",
  });
...
function run(command, args, options) {
  return require("child_process").spawnSync(command, args, Object.assign({ stdio: "inherit" }, options || {}));
}
...
function installNodeWithSystemPackageManager() {
  ...
  if (!canElevate()) {
    console.error("[openclaw] Cannot auto-install Node.js because this user is not root and sudo is unavailable.");
    return false;
  }
  ...
  if (commandExists("apk")) {
    return runShell(
      [
        "set -e",
        asRoot("apk add --no-cache nodejs-current npm"),
      ].join("\n"),
    ).status === 0;
  }
  ...
  console.error("[openclaw] No supported package manager was found for automatic Node.js installation.");
  return false;
}
...
function repairShadowedNodePath() {
  ...
  var commands = [
    "set -e",
    "if [ -e " + shellQuote(pathNode) + " ] || [ -L " + shellQuote(pathNode) + " ]; then " +
  ...
  ["npm", "npx"].forEach(function (binary) {
    ...
    if (pathBinary && pathBinary !== systemBinary) {
      ...
      commands.push(
        "if [ -e " + shellQuote(systemBinary) + " ] || [ -L " + shellQuote(systemBinary) + " ]; then " +
          "if [ -e " + shellQuote(pathBinary) + " ] || [ -L " + shellQuote(pathBinary) + " ]; then " +
  ...
  if (runShell(commands.join("\n")).status === 0) {
    console.error("[openclaw] Repaired PATH so NodeSource Node.js is used before an older shadowing node binary.");
    return true;
  }
  return false;
}
...
function shouldAutoInstall() {
  if (process.env.OPENCLAW_AUTO_INSTALL_NODE === "0" || process.env.OPENCLAW_SKIP_NODE_AUTO_INSTALL === "1") {
    return false;
  }
