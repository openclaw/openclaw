...
import { n as spawnSubagentDirect } from "./subagent-spawn-CLispquW.js";
import { n as resolveContinuationRuntimeConfig } from "./config-D9MPOF2I.js";
import { i as failFlow } from "./task-flow-runtime-internal-J0eTvwb0.js";
import { a as enqueueSystemEvent } from "./system-events-CoCLzqF0.js";
import { i as consumePendingDelegates, l as highestDelayedContinuationReservationHop, u as peekSoonestUnmaturedDelegateDueAt } from "./delegate-store-CXooQL49.js";
import { l as retainContinuationTimerRef, o as registerContinuationTimerHandle, u as unregisterContinuationTimerHandle } from "./state-dv9ktEcj.js";
...
function armHedgeTimer(sessionKey, fireAt, params) {
	...
	const handle = setTimeout(async () => {
		...
		const refreshedChainState = params.loadFreshChainState ? params.loadFreshChainState() : params.chainState;
		try {
			...
			if (params.persistChainState && result.dispatched > 0) await params.persistChainState(result.chainState);
		} catch (err) {
	...
	for (const delegate of delegatesWithinLimit) {
		...
		try {
			...
			if (result.status === "accepted") {
				...
				const summary = result.error ? `DELEGATE spawn ${result.status}: ${result.error}` : `DELEGATE spawn ${result.status}: delegation was not accepted.`;
...
async function dispatchStagedPostCompactionDelegates(delegates, sessionKey, spawnCtx) {
	...
	for (const delegate of delegates) try {
		...
		const rejectionSummary = spawnResult.error ? `Post-compaction delegate spawn ${spawnResult.status}: ${spawnResult.error}` : `Post-compaction delegate spawn ${spawnResult.status}: delegation was not accepted.`;
		...
		enqueueSystemEvent(`[continuation] ${rejectionSummary}. Task: ${delegate.task}`, { sessionKey });
		failed++;
	} catch (err) {
