...
import { d as isLikelyExecutionAckPrompt } from "./selection-BFquHzf2.js";
...
import { r as enqueueCommitmentExtraction } from "./runtime-BB3_sIrH.js";
...
import { n as dispatchPostCompactionDelegates, r as persistPendingPostCompactionDelegates } from "./post-compaction-delegate-dispatch-BNI6KB0f.js";
						...
						const result = await runEmbeddedPiAgentDefault$1({
							...
							requestCompactionOpts: runtimeConfig?.agents?.defaults?.continuation?.enabled === true ? {
								...
								triggerCompaction: async (request) => {
									try {
										const { compactEmbeddedPiSession } = await import("./compact.queued-HCj1kAks.js");
...
const piEmbeddedRuntimeLoader = createLazyImportLoader(() => import("./pi-embedded-DY4wD8p4.js"));
		...
		try {
			...
			if (runtimeConfig?.agents?.defaults?.continuation?.enabled === true && sessionKey) {
				const [{ dispatchToolDelegates }, { resolveLiveContinuationRuntimeConfig }, { loadContinuationChainState, persistContinuationChainState }, { updateSessionStore, resolveSessionStoreEntry }] = await Promise.all([
					import("./delegate-dispatch-Dsm3r0Qj.js"),
