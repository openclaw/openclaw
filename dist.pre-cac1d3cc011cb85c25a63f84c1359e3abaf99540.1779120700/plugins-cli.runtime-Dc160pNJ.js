...
import { n as collectConfiguredRuntimePluginIds, r as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-BJVQTuxS.js";
...
async function runPluginsEnableCommand(id) {
	...
	const { enablePluginInConfig } = await import("./enable-C0rRk_Db.js");
	const { normalizePluginId } = await import("./config-state-CDowxzeJ.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-BYhQqsjy.js");
	const { applySlotSelectionForPlugin, logSlotWarnings } = await import("./plugins-command-helpers-BmPrD2Hg.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-BUtFJnAg.js");
...
async function runPluginsDisableCommand(id) {
	...
	const { normalizePluginId } = await import("./config-state-CDowxzeJ.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-BYhQqsjy.js");
	const { setPluginEnabledInConfig } = await import("./plugins-config-CV93qrAz.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-BUtFJnAg.js");
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-FM015xQF.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-DqGeFTGx.js");
...
async function runPluginsDoctorCommand() {
	const { buildPluginCompatibilityNotices, buildPluginDiagnosticsReport, formatPluginCompatibilityNotice } = await import("./status-BYhQqsjy.js");
	const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-I01t3mL9.js");
	const cfg = getRuntimeConfig();
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-k9r06_HQ.js");
	const { createPluginInstallLogger } = await import("./plugins-command-helpers-BmPrD2Hg.js");
