...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-BsqtHigT.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-DIHxBwz-.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-D8V6ipkd.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-k7DLiaQd.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-D7zsN9u8.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-CkIMTh8J.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-DWc8dACi.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-CC-ZK0LU.js"), import("./setup-registry-DL-eEVpO.js")]);
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-DpXvQqsM.js");
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-BQBHMGzl.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DPLPxiTc.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-CmlqjHIX.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-CmlqjHIX.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-CxGyVGHb.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-Drv1Bts5.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-D5nWoUnC.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-B3gMeLq6.js");
		const { setupChannels } = await import("./onboard-channels-CyIABlo3.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-h_pdGn79.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-D5yWd9-o.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-CfRm9pgs.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-BolsH1q-.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-C3BdiyJo.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BBo5Bkgw.js");
