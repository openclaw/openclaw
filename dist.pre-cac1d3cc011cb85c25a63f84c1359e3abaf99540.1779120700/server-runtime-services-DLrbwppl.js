import { n as isVitestRuntimeEnv } from "./env-BE3qwYMq.js";
...
import { i as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-BwnpwXJG.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-DsL7V5MR.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-BVntP3gQ.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-BcUJz57f.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-DHV6qnDv.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-BgQwg3mm.js");
		if (stopped) return;
