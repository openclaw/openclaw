...
export function normalizeExternalPluginCompatibility(
	...
	const minGatewayVersion =
		getTrimmedString(compat?.minGatewayVersion) ?? minHostVersion;
	if (minGatewayVersion) {
		compatibility.minGatewayVersion = minGatewayVersion;
	}

	const builtWithOpenClawVersion =
		getTrimmedString(build?.openclawVersion) ?? version;
...
export function listMissingExternalCodePluginFieldPaths(
	packageJson: unknown,
): string[] {
...
export function validateExternalCodePluginPackageJson(
	...
	const issues = listMissingExternalCodePluginFieldPaths(packageJson).map(
		(fieldPath) => ({
			fieldPath,
