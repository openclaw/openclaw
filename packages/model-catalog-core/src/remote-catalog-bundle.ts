...
const modelFieldsSchema = z.object({
  ...
  inference: ModelInferenceCapabilitiesSchema.optional(),
...
const modelV2Schema = modelFieldsSchema
  .omit({ cost: true, baseUrl: true, headers: true, inference: true })
  .extend({ provider: z.string().trim().min(1), pricing: pricingV2Schema })
  .strict()
  .superRefine(validateContextWindowDefault);

function validateBundleModelRefs(
  bundle: { providers: Record<string, unknown>; models: Array<{ provider: string; id: string }> },
  context: z.RefinementCtx,
) {
...
export const remoteModelCatalogBundleV2Schema = remoteModelCatalogBundleSchema
  ...
  .superRefine(validateBundleModelRefs);

export type RemoteModelCatalogModelV2 = Omit<
  ModelCatalogModel,
  "cost" | "baseUrl" | "headers" | "upstreamModel" | "inference"
> & {
  provider: string;
...
/** Typed task metadata is a new wire version: existing strict v2 readers reject these fields. */
export const remoteModelCatalogBundleV3Schema = z
  .object({
    ...remoteModelCatalogBundleV2Schema.shape,
    schemaVersion: z.literal(3),
    models: z
      .array(modelV2Schema.safeExtend({ inference: ModelInferenceCapabilitiesSchema.optional() }))
      .min(1),
  })
  .strict()
  .superRefine(validateBundleModelRefs);
export type RemoteModelCatalogModelV3 = RemoteModelCatalogModelV2 &
  Pick<ModelCatalogModel, "inference">;
export type RemoteModelCatalogBundleV3 = Omit<
  RemoteModelCatalogBundleV2,
  "schemaVersion" | "models"
> & {
  schemaVersion: 3;
  models: RemoteModelCatalogModelV3[];
};
export function validateAndSanitizeRemoteModelCatalogBundleV3(
  value: unknown,
): RemoteModelCatalogBundleV3 {
  const bundle = remoteModelCatalogBundleV3Schema.parse(value);
  return {
    ...bundle,
    models: bundle.models.map((model) => {
      // The same transport scrubber owns freeform compatibility metadata in all wire versions.
      const sanitized = stripRemoteTransportOverrides(model);
      return remoteModelCatalogBundleV3Schema.shape.models.element.parse(sanitized);
    }),
  };
}
