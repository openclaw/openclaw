...
describe("anthropic transport stream", () => {
  ...
  it.each([
    ["malformed JSON", '{"path":'],
    ["non-object JSON", "[]"],
  ])("surfaces %s tool input as a runtime validation sentinel", async (_label, partialJson) => {
    guardedFetchMock.mockResolvedValueOnce(
      createSseResponse([
        {
          type: "message_start",
          message: { id: "msg_invalid_args", usage: { input_tokens: 10, output_tokens: 0 } },
        },
        {
          type: "content_block_start",
          index: 0,
