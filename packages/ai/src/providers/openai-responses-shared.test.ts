...
describe("processResponsesStream", () => {
  ...
  it("finalizes failed terminal usage and service-tier pricing before preserving the error", async () => {
    const model = {
      ...nativeOpenAIModel,
    ...
    ).rejects.toThrow("server_error: provider failed after consuming tokens");

    expect(output).toMatchObject({
      responseId: "resp_failed_usage",
      stopReason: "stop",
      usage: {
        input: 13,
        output: 4,
