...
// Some openai-responses providers re-emit the same assistant message item as
// cumulative snapshots — each a strict prefix-superset of the previous one —
// instead of one final message item. A same-item, same-phase strict extension
// replaces the prior text block, or the visible reply repeats once per snapshot
// (#91959).
...
export function resolveResponsesMessageSnapshotCollapse(params: {
  prior: { text: string; phase: string | undefined; itemId: string | undefined } | null;
  nextText: string;
  ...
  if (
    ...
    !prior.itemId ||
    !params.nextItemId ||
    prior.itemId !== params.nextItemId ||
    prior.phase !== params.nextPhase
  ) {
    return { kind: "keep" };
  }
