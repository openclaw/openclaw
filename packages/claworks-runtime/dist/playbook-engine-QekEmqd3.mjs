...
import { i as executePlaybookStep, n as HitlSuspendedError, r as StepFailedError, t as CW_EVENTS } from "./event-names-Cxh8ioTw.mjs";
...
function buildEventContext(payload, eventType) {
	...
	const robotId = typeof payload.robot_id === "string" ? payload.robot_id : void 0;
	...
	const meta = {};
	if (pendingRuns !== void 0) meta.pending_runs = pendingRuns;
	if (playbookCount !== void 0) meta.playbook_count = playbookCount;
  ...
  `);
	...
	return {
		...
		async submitHitlDecision(runId, stepId, decision, comment) {
			...
			if (run.status === "completed") recordAssistantTurnOnCompletion(pending.input, ctx.variables, pending.playbookId, runId, deps.contextEngine);
			runs.set(runId, run);
			persistRun(upsertRun, run, suspended.get(runId));
			return run;
		}
...
function recordAssistantTurnOnCompletion(input, variables, playbookId, runId, contextEngine) {
	...
	if (replyText) contextEngine.append(sessionId, "assistant", replyText, {
		playbook_id: playbookId,
		run_id: runId
	});
}
