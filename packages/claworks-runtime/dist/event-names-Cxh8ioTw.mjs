...
async function executePlaybookStep(step, ctx, run, deps) {
	...
	try {
		if (step.kind === "notification") {
			...
			let skillOutput;
			if (deps.scriptRun) try {
				skillOutput = {
					...await deps.scriptRun({
						scriptId: skillId,
						input
					}) ?? {},
					source: "local",
				...
				if (!(msg.includes("Script not found") || msg.includes(`Script not found: ${skillId}`))) throw err;
			}
			if (!skillOutput) if (deps.skillRun) skillOutput = {
				...await deps.skillRun({
					skillId,
					input
				}) ?? {},
			...
			else if (deps.productionMode) throw new StepFailedError(`skill bridge 未配置（production_mode=true 时 stub 不可用），skillId=${skillId}`, step.id, "abort");
			else {
				skillOutput = {
					stub: true,
					skillId,
