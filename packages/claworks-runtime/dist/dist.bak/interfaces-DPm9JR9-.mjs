...
async function recordAssistantTurnIfCompleted(runtime, sessionId, runId, playbookId) {
		...
		try {
			...
			if (method === "POST" && parts[1] === "playbooks" && parts[3] === "runs" && parts[2]) {
				...
				if (sessionId && run.status === "completed" && run.output) {
					const replyText = extractReplyText(run.output);
					if (replyText) runtime.contextEngine?.append(sessionId, "assistant", replyText, {
						playbookId: parts[2],
						runId: run.id,
						channel: "rest"
					});
				}
				sendJson$2(res, 202, run);
				return true;
			}
			...
			if (method === "POST" && parts[1] === "events") {
				...
				if (publishResult.action === "intent_routed") {
					if (eventSessionId) await recordAssistantTurnIfCompleted(runtime, eventSessionId, publishResult.runId, publishResult.playbookId);
					sendJson$2(res, 202, {
