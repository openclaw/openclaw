...
import { m as resolveA2aTarget, t as CW_EVENTS, u as matchGlob } from "./event-names-Cxh8ioTw.mjs";
...
import { l as createActionRegistry, n as createIntentRegistry, r as createEventKernel, t as createPlaybookScheduler } from "./scheduler-BmrYAp_0.mjs";
...
import { t as createPlaybookEngine } from "./playbook-engine-QekEmqd3.mjs";
...
function makePerceiveMessageDescriptor(runtime) {
	return {
		...
		handler: async (_ctx, params) => {
			...
			try {
				const { tryParseJson } = await import("./function-executor-BxOXYxqv.mjs");
				const result = await completeFn({ prompt });
	...
	*/
	async importEvolutionPack(pack) {
		...
		for (const playbook of pack.improved_playbooks ?? []) try {
			...
			if (evolveEngine?.deploy) await evolveEngine.deploy({
				...
				} else if (typeof pbExt.loadFromYaml === "function") await pbExt.loadFromYaml(yamlContent, source);
				else throw new Error("playbookEngine.load / loadFromYaml 不可用");
			}
		...
		for (const table of pack.updated_rule_tables ?? []) try {
			...
			if (ruleEngine?.registerTable) {
				...
				applied.push(`规则表已更新: ${table.name}`);
			} else errors.push(`规则表 ${table.name} 跳过: ruleEngine 无 registerTable/loadTable`);
		} catch (err) {
	...
	loadFeedbackRecords() {
		return (this.runtime.cbrStore?.list({ limit: 200 }) ?? []).filter((c) => c.outcome !== void 0).map((c) => ({
			...
			timestamp: (c.createdAt ?? /* @__PURE__ */ new Date(0)).toISOString()
		}));
	}
	...
	collectRuleTableNames() {
		...
		if (!engine) return [];
		try {
			if (typeof engine.listTables === "function") return [...new Set(engine.listTables().map((table) => table.id.split(".")[0] ?? "unknown"))];
			const listRules = engine.listRules;
			if (typeof listRules === "function") return [...new Set(listRules.call(engine).map((rule) => rule.id.split(".")[0] ?? "unknown"))];
			return [];
		} catch {
			return [];
		}
...
async function createClaworksRuntime(config, opts) {
	...
	const hitl = opts?.hitl ?? (await import("./hitl-gate-DBPCsQ_8.mjs").then((n) => n.n)).createHitlGate();
	let kernel;
	let runtime;
