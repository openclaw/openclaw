...
const PLURAL_RULES = /* @__PURE__ */ new Map();
function createCatalogSnapshot(params) {
	const catalogs = Object.fromEntries(Object.entries(params.catalogs).map(([locale, catalog]) => [locale, freezeCatalog(catalog ?? {})]));
	return Object.freeze({
		registryRevision: params.registryRevision ?? "sha256:f1fc485ce67ea02b74c69e63e648da3fddc51e276d507a2eeb21d49a18898207",
		catalogRevision: params.catalogRevision,
		catalogs: Object.freeze(catalogs)
	});
}
function renderLocalizedMessage(snapshot, context, message) {
	...
	for (const locale of locales) {
		...
		if (entry !== void 0) return renderCatalogEntry(entry, locale, message.params, fallback);
	}
	return fallback;
}
function interpolateMessage(value, params) {
	...
	return value.replace(PLACEHOLDER_PATTERN, (match, key) => {
		...
		return param === void 0 ? match : String(param);
	});
}
function validateCatalog(params) {
	const issues = [];
	for (const [key, sourceEntry] of Object.entries(params.source)) {
		if (!MESSAGE_KEY_PATTERN.test(key) || !key.startsWith(`${params.namespace}.`)) issues.push({
			code: "invalid-key",
			key,
			detail: `Key must be namespaced under ${params.namespace}.`
		});
		const candidateEntry = params.candidate[key];
		if (candidateEntry === void 0) {
			issues.push({
				code: "missing-key",
				key,
				detail: "Candidate catalog is missing the key."
			});
			continue;
		}
		validateEntry(key, sourceEntry, candidateEntry, issues);
	}
	for (const key of Object.keys(params.candidate)) if (!(key in params.source)) issues.push({
		code: "unknown-key",
		key,
...
function renderCatalogEntry(entry, locale, params, fallback) {
	if (typeof entry === "string") return interpolateMessage(entry, params);
	const selector = params?.[entry.param];
	if (entry.kind === "plural") {
		if (typeof selector !== "number" || !Number.isFinite(selector)) return fallback;
		const category = getPluralRules(locale).select(selector);
	...
	function getPluralRules(localeId) {
	...
	if (typeof selector !== "string" && typeof selector !== "boolean") return fallback;
	const template = entry.cases[String(selector)] ?? entry.cases.other;
	return typeof template === "string" ? interpolateMessage(template, params) : fallback;
}
function freezeCatalog(catalog) {
	return Object.freeze(Object.fromEntries(Object.entries(catalog).map(([key, entry]) => [key, typeof entry === "string" ? entry : Object.freeze({
		...entry,
		cases: Object.freeze({ ...entry.cases })
	})])));
}
function validateEntry(key, source, candidate, issues) {
	...
	if (sourceCases.kind === "select" && Object.keys(sourceCases.values).toSorted().join(",") !== Object.keys(candidateCases.values).toSorted().join(",")) issues.push({
		code: "invalid-selector",
		key,
		detail: "Source and candidate select entries must declare the same cases."
	});
	for (const category of Object.keys(candidateCases.values)) if (candidateCases.kind === "plural" && !PLURAL_CATEGORIES.has(category)) issues.push({
		code: "invalid-selector",
		key,
	...
	for (const [caseName, value] of Object.entries(sourceCases.values)) if (placeholderSignature(value) !== expectedPlaceholders) issues.push({
	...
	for (const [caseName, value] of Object.entries(candidateCases.values)) {
		...
		if (candidatePlaceholders !== expectedPlaceholders) issues.push({
			code: "placeholder-mismatch",
			key,
			detail: `Case ${caseName} expected placeholders ${expectedPlaceholders || "(none)"}; received ${candidatePlaceholders || "(none)"}.`
		});
	}
	for (const value of Object.values(candidateCases.values)) if (FORBIDDEN_BIDI_CONTROL_PATTERN.test(value)) {
		issues.push({
...
function hasValidSelectorShape(entry) {
	if (typeof entry === "string") return true;
	return typeof entry.param === "string" && entry.param.length > 0 && typeof entry.cases === "object" && entry.cases !== null && typeof entry.cases.other === "string" && Object.values(entry.cases).every((value) => typeof value === "string");
}
...
//#endregion
export { createCatalogSnapshot, interpolateMessage, renderLocalizedMessage, validateCatalog };
