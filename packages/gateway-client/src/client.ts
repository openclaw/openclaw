...
export class GatewayClient {
  ...
  private beginStop(): Promise<void> | null {
    ...
    if (ws) {
      ...
      // in CLI contexts (e.g. `openclaw message send`) where the caller
      // exits after the send completes. The TCP/unix close-handshake may
      // outlive `process.exit()`, but the open socket must not block a
      // natural exit (no explicit process.exit). The forceTerminateTimer
      // below is already `.unref()`'d; the socket itself was not.
      // Fixes #88230.
      try {
        const internalSocket = (ws as unknown as {
          _socket?: { unref?(): void };
        })["_socket"];
