...
describe("generateSummary output budget", () => {
  ...
  it("does not authorize summary output beyond the shipped default share", async () => {
    // The reserve is an input-side budget. A raised one must not become a matching
    // output authorization: the retained summary is capped far below it anyway.
  ...
  it("keeps a split turn's combined authorization at its default-reserve share", async () => {
    // A split turn summarizes history and turn prefix into ONE retained entry, so
    // the pair - not just each call - must stay at the share the default reserve
    // already authorizes, otherwise the raised path survives across two calls.
    const raised = await captureSplitTurnMaxTokens(80_000);
    const byDefault = await captureSplitTurnMaxTokens(20_000);

    expect(raised).toEqual([16_000, 10_000]);
    expect(raised).toEqual(byDefault);
  });
});
