...
async function maybeCommit(task: TaskPacket) {
  const files = await collectCommitFiles(task);
  if (files.length === 0) {
    return false;
  }
  const message = `${task.externalTicketId}: finalize ${task.role} output`;
  await run("./scripts/committer", [message, ...files], task.repoCwd);
  return true;
}

async function collectCommitFiles(task: TaskPacket) {
  const modified = await runMaybe("git", ["ls-files", "-m"], task.repoCwd);
  const untracked = await runMaybe(
    "git",
    ["ls-files", "--others", "--exclude-standard"],
    task.repoCwd,
  );
  const candidates = `${modified.stdout}\n${untracked.stdout}`
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  const selected = new Set<string>();
  for (const file of candidates) {
    if (file.startsWith(".openclaw-operator/")) {
      continue;
    }
    ...
    if (
      file === task.repoRelativeArtifactDir ||
      file.startsWith(`${task.repoRelativeArtifactDir}/`)
    ) {
      selected.add(file);
    }
  }
  return Array.from(selected).toSorted();
}
...
async function main() {
  ...
  if (task.prRequired && task.prSyncCommand) {
    ...
    writeStdoutLine(updated.prUrl ?? "");
    return;
  }
  writeStdoutLine("finalized");
}
