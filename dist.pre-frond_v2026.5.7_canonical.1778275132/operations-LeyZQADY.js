...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-Cp8oawzr.js");
	if (operation === "start") {
...
async function loadOverviewForOperation(deps) {
	...
	const { loadCrestodianOverview } = await import("./overview-CCaK01co.js");
...
async function formatOverviewForOperation(overview, deps) {
	...
	const { formatCrestodianOverview } = await import("./overview-CCaK01co.js");
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-list") {
		...
		await (opts.deps?.runPluginsList ?? (async (pluginRuntime) => {
			const { runPluginsListCommand } = await import("./plugins-list-command-DmlqTvLo.js");
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-CoqHhGaq.js");
	...
	if (operation.kind === "setup") {
		...
		const applyDefaultModelPrimaryUpdate = setupModel.model ? (await import("./shared-D0CT4H-4.js")).applyDefaultModelPrimaryUpdate : void 0;
	...
	if (operation.kind === "config-set") {
		...
		await (opts.deps?.runConfigSet ?? (async (setOpts) => {
			const { runConfigSet: importedRunConfigSet } = await import("./config-cli-hR2K79cL.js");
	...
	if (operation.kind === "plugin-install") {
		...
		await (opts.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
			const { runPluginInstallCommand } = await import("./plugins-install-command-LPqopAZ1.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-DvOymCEW.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-LIJ-bVLm.js")).agentsAddCommand)({
	...
	if (operation.kind === "doctor") {
		...
		await (opts.deps?.runDoctor ?? (await import("./doctor-DtWsgyra.js")).doctorCommand)(runtime, { nonInteractive: true });
	...
	if (operation.kind === "doctor-fix") {
		...
		const { readConfigFileSnapshot } = await import("./config/config.js");
		const before = await readConfigFileSnapshot();
		await (opts.deps?.runDoctor ?? (await import("./doctor-DtWsgyra.js")).doctorCommand)(runtime, {
			nonInteractive: true,
			repair: true,
			yes: true
		});
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-DqyQn6wo.js");
	...
	if (operation.kind === "health") {
		...
		const { healthCommand } = await import("./health-DzD1KsY7.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-Bjw0zyGH.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
	...
	if (operation.kind === "set-default-model") {
		...
		const { mutateConfigFile, readConfigFileSnapshot } = await loadConfigFileMutationHelpers();
		const before = await readConfigFileSnapshot();
		const { applyDefaultModelPrimaryUpdate } = await import("./shared-D0CT4H-4.js");
		const result = await mutateConfigFile({
			base: "source",
			mutate: (cfg) => {
