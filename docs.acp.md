...
This document describes how the Activi ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`activi acp` exposes an ACP agent over stdio and forwards prompts to a running
Activi Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `activi acp` and speaks ACP messages over stdio.
...
`activi acp` resolves the Gateway URL and auth from CLI flags or config:
