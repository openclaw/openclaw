...
This document describes how the DNA ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`dna acp` exposes an ACP agent over stdio and forwards prompts to a running
DNA Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `dna acp` and speaks ACP messages over stdio.
...
`dna acp` resolves the Gateway URL and auth from CLI flags or config:
