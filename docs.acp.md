# Gemmaclaw ACP Bridge

This document describes how the Gemmaclaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`gemmaclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
Gemmaclaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
`gemmaclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It is designed to route IDE prompts into an existing Gemmaclaw Gateway
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Gemmaclaw gateway or agent instead.                                                                                                                                    |
...
drive a Gemmaclaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `gemmaclaw acp` over stdio.

Example config:

```bash
gemmaclaw config set gateway.remote.url wss://gateway-host:18789
gemmaclaw config set gateway.remote.token <token>
```

Example run:

```bash
gemmaclaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
gemmaclaw acp --session agent:main:main
gemmaclaw acp --session agent:design:main
gemmaclaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Gemmaclaw ACP” to start a thread.

## Execution Model

- ACP client spawns `gemmaclaw acp` and speaks ACP messages over stdio.
...
```bash
gemmaclaw acp --session agent:main:main
gemmaclaw acp --session-label "support inbox"
gemmaclaw acp --reset-session
```
...
`gemmaclaw acp` resolves the Gateway URL and auth from CLI flags or config:
