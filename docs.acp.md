...
This document describes how the Mullusi ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`mullusi acp` exposes an ACP agent over stdio and forwards prompts to a running
Mullusi Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Mullusi Gateway
...
- ACP client spawns `mullusi acp` and speaks ACP messages over stdio.
...
`mullusi acp` resolves the Gateway URL and auth from CLI flags or config:
