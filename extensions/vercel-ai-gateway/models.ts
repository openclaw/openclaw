...
function buildStaticModelDefinition(
	model: StaticVercelGatewayModel,
...
function getStaticFallbackModel(id: string): ModelDefinitionConfig | undefined {
	const fallback = STATIC_VERCEL_AI_GATEWAY_MODEL_CATALOG.find(
		(model) => model.id === id,
	);
...
export async function discoverVercelAiGatewayModels(): Promise<
	ModelDefinitionConfig[]
> {
	...
	try {
		...
		if (!response.ok) {
			log.warn(
				`Failed to discover Vercel AI Gateway models: HTTP ${response.status}`,
			);
			return getStaticVercelAiGatewayModelCatalog();
		}
