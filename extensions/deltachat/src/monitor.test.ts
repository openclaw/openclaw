...
describe("Delta.Chat Monitor - Reply Media", () => {
  const eventHandlers: Record<string, (...args: any[]) => any> = {};
  const onMock = vi.fn((event: string, handler: (...args: any[]) => any) => {
    eventHandlers[event] = handler;
  });
  ...
  const mockDc = {
    ...
    getContextEvents: vi.fn(() => ({ on: onMock, off: vi.fn() })),
    close: vi.fn(),
    transport: {
      input: { end: vi.fn() },
      output: vi.fn(),
  ...
  function makeRuntime() {
    return {
      config: {
        loadConfig: vi.fn(() => ({
          channels: { deltachat: { dm: { policy: "open", allowFrom: ["*"] } } },
        })),
      },
      logging: { getChildLogger: vi.fn(() => ({ info: vi.fn(), error: vi.fn() })) },
      channel: {
        ...
        text: { resolveTextChunkLimit: vi.fn(() => 4000), hasControlCommand: vi.fn(() => false) },
        debounce: {
          resolveInboundDebounceMs: vi.fn(() => 0),
          createInboundDebouncer: vi.fn((params: { onFlush: (items: any[]) => Promise<void> }) => ({
            enqueue: async (item: any) => params.onFlush([item]),
            flushKey: vi.fn(),
          })),
        },
        pairing: {
        ...
        reply: {
          ...
          formatInboundEnvelope: vi.fn(() => "formatted-body"),
          finalizeInboundContext: vi.fn((ctx) => ctx),
          dispatchReplyFromConfig: vi.fn(() => ({ queuedFinal: false })),
          createReplyDispatcherWithTyping: vi.fn(() => ({
            dispatcher: { markComplete: vi.fn(), waitForIdle: vi.fn(() => Promise.resolve()) },
            replyOptions: {},
            markDispatchIdle: vi.fn(),
        ...
        activity: { record: vi.fn() },
        media: { saveMediaBuffer: saveMediaBufferMock },
      },
      log: vi.fn(),
      error: vi.fn(),
      exit: vi.fn(),
  ...
  it("includes quoted message media in MediaPaths when quoted msg has a file", async () => {
    const fs = await import("node:fs/promises");
    vi.mocked(fs.readFile).mockResolvedValue(Buffer.from("fake-image-data") as any);
    saveMediaBufferMock.mockResolvedValue({
      path: "/tmp/oc-media/reply-photo.jpg",
      size: 100,
      contentType: "image/jpeg",
    });

    const rt = makeRuntime();
    setDeltaChatRuntime(rt);
    vi.mocked(getDeltaChatRuntime).mockReturnValue(rt);
    ...
    mockDc.rpc.getMessage.mockImplementation(async (_accountId: number, msgId: number) => {
      if (msgId === 10) {
        return {
      ...
      // Quoted message (msgId 99) has a file
      return {
        file: "/dc/blob/photo.jpg",
        fileMime: "image/jpeg",
        fileName: "photo.jpg",
    ...
    mockDc.rpc.getContact.mockResolvedValue({ address: "sender@example.com" });
    mockDc.rpc.getChatSecurejoinQrCode.mockResolvedValue("OPENPGP4FPR:test");

    let capturedCtx: any;
    rt.channel.reply.finalizeInboundContext.mockImplementation((ctx: any) => {
      capturedCtx = ctx;
      return ctx;
    });
    ...
    await eventHandlers["IncomingMsg"]({ chatId: 1, msgId: 10 });

    expect(saveMediaBufferMock).toHaveBeenCalledOnce();
    expect(capturedCtx.MediaPath).toBe("/tmp/oc-media/reply-photo.jpg");
    expect(capturedCtx.MediaUrl).toBe("/tmp/oc-media/reply-photo.jpg");
    expect(capturedCtx.MediaPaths).toEqual(["/tmp/oc-media/reply-photo.jpg"]);
    expect(capturedCtx.MediaUrls).toEqual(["/tmp/oc-media/reply-photo.jpg"]);

    abortController.abort();
    await promise;
  });

  it("does not fetch reply media for JustText quotes (no quotedMsgId)", async () => {
    const fs = await import("node:fs/promises");
    ...
    mockDc.rpc.getMessage.mockResolvedValue({
      ...
      isInfo: false,
      timestamp: 1000,
      // JustText: no messageId
      quote: { kind: "JustText", text: "some prior text" },
    });
    mockDc.rpc.getContact.mockResolvedValue({ address: "sender@example.com" });
    mockDc.rpc.getChatSecurejoinQrCode.mockResolvedValue("OPENPGP4FPR:test");
    ...
    // No media fetch triggered — JustText quotes have no messageId
    expect(saveMediaBufferMock).not.toHaveBeenCalled();
    expect(vi.mocked(fs.readFile)).not.toHaveBeenCalled();
    expect(capturedCtx.MediaPath).toBeUndefined();
  ...
  it("skips reply media gracefully when quoted message has no file", async () => {
    const rt = makeRuntime();
    setDeltaChatRuntime(rt);
    vi.mocked(getDeltaChatRuntime).mockReturnValue(rt);
  ...
  it("continues gracefully when reply media readFile throws", async () => {
    const fs = await import("node:fs/promises");
    ...
    await expect(eventHandlers["IncomingMsg"]({ chatId: 1, msgId: 10 })).resolves.toBeUndefined();

    // Media path undefined, but message was still dispatched
    expect(capturedCtx.MediaPath).toBeUndefined();
    expect(capturedCtx.MediaPaths).toBeUndefined();
    expect(rt.channel.reply.dispatchReplyFromConfig).toHaveBeenCalled();

    abortController.abort();
    await promise;
  });

  it("places current-message media before reply media in MediaPaths", async () => {
    const fs = await import("node:fs/promises");
    vi.mocked(fs.readFile).mockResolvedValue(Buffer.from("reply-image-data") as any);
    saveMediaBufferMock
      .mockResolvedValueOnce({
        path: "/tmp/oc-media/current.jpg",
        size: 200,
        contentType: "image/jpeg",
      })
      .mockResolvedValueOnce({
        path: "/tmp/oc-media/reply.jpg",
        size: 100,
        contentType: "image/jpeg",
      });
    ...
    mockDc.rpc.getMessage.mockImplementation(async (_accountId: number, msgId: number) => {
      if (msgId === 10) {
        return {
          text: "here is my file too",
          fromId: 2,
          isFromSelf: false,
          systemMessageType: "Unknown",
          isInfo: false,
          timestamp: 1000,
          file: "/dc/blob/current.jpg",
          fileMime: "image/jpeg",
    ...
    // Current-message media first, reply media second
    expect(capturedCtx.MediaPath).toBe("/tmp/oc-media/current.jpg");
    expect(capturedCtx.MediaPaths).toEqual([
      "/tmp/oc-media/current.jpg",
      "/tmp/oc-media/reply.jpg",
    ]);
    expect(capturedCtx.MediaUrls).toEqual(["/tmp/oc-media/current.jpg", "/tmp/oc-media/reply.jpg"]);
