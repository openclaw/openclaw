...
export async function monitorDeltaChatProvider(opts: MonitorDeltaChatOpts = {}): Promise<void> {
  ...
  // Pass accountId so multi-account setups read the correct scoped allowlist.
  const storeAllowFrom = await core.channel.pairing
    .readAllowFromStore({ channel: "deltachat", accountId: opts.accountId ?? "default" })
    .catch(() => []);
  ...
  type DeltachatDebounceEntry = {
    ...
    timestamp?: number;
    mediaUrl?: string;
    // Numeric message ID of the quoted message, used to fetch its media at flush time.
    quotedMsgId?: number;
    replyToId?: string;
    replyToBody?: string;
  ...
  trackListener("IncomingMsg", async ({ chatId, msgId }: { chatId: number; msgId: number }) => {
    ...
    // Extract quote/reply context from message.quote.
    // "WithMessage" has a message ID + author → treat as a quote (partial selection).
    // "JustText" is a text-only quote with no message reference → treat as a reply.
    let replyToId: string | undefined;
    let replyToBody: string | undefined;
    let replyToSender: string | undefined;
    let replyToIsQuote: boolean | undefined;
    let quotedMsgId: number | undefined;
    if (message.quote) {
      const q = message.quote;
      if (q.kind === "WithMessage") {
        replyToId = String(q.messageId);
        replyToBody = q.text.trim() || undefined;
        replyToSender = q.authorDisplayName || undefined;
        replyToIsQuote = true;
        // Store numeric ID so media can be fetched at debounce-flush time.
        quotedMsgId = q.messageId;
      } else {
        // JustText: quoted text only, no originating message reference or author
        replyToBody = q.text.trim() || undefined;
  ...
  // Fetch and save the media file from a quoted message, if any.
  // Called at debounce-flush time (not at event receipt) to avoid downloading
  // media for messages that are still being debounced or combined.
  async function resolveReplyMedia(quotedMessageId: number): Promise<string | undefined> {
    try {
      const quotedMsg = await dc!.rpc.getMessage(account.id, quotedMessageId);
      if (!quotedMsg.file || quotedMsg.isSetupmessage) {
        return undefined;
      }
      const fs = await import("node:fs/promises");
      const buffer = await fs.readFile(quotedMsg.file);
      const saved = await core.channel.media.saveMediaBuffer(
        buffer,
        quotedMsg.fileMime ?? undefined,
        "inbound",
        _mediaMaxBytes,
        quotedMsg.fileName ?? undefined,
      );
      runtime.log?.(
        `[Delta.Chat] Saved reply media from quoted msg ${quotedMessageId}: ${saved.path}`,
      );
      return saved.path;
    } catch (err) {
      runtime.log?.(
        `[Delta.Chat] Failed to fetch reply media for msg ${quotedMessageId}: ${extractErrorMessage(err)}`,
      );
      return undefined;
    }
  }
  ...
  async function handleInboundMessage(entry: DeltachatDebounceEntry): Promise<void> {
    ...
    // Build reply/quote suffix (mirrors Telegram's replySuffix format).
    // "WithMessage" quotes use [Quoting ...] with quoted text in double-quotes.
    // "JustText" replies (no message ID) use [Replying to ...].
    const replySuffix = replyToBody
      ? replyToIsQuote
        ? `\n\n[Quoting ${replyToSender ?? "unknown sender"}${replyToId ? ` id:${replyToId}` : ""}]\n"${replyToBody}"\n[/Quoting]`
        : `\n\n[Replying to ${replyToSender ?? "unknown sender"}${replyToId ? ` id:${replyToId}` : ""}]\n${replyToBody}\n[/Replying]`
      : "";
    ...
    // Fetch media from the quoted message, if any (mirrors Telegram's reply media handling).
    const replyMediaUrl = quotedMsgId != null ? await resolveReplyMedia(quotedMsgId) : undefined;
    // Merge current-message media with quoted-message media; current media comes first.
    const allMediaUrls = [mediaUrl, replyMediaUrl].filter((u): u is string => Boolean(u));
    const ctxMediaPath = allMediaUrls[0];
    const ctxMediaPaths = allMediaUrls.length > 0 ? allMediaUrls : undefined;
    ...
    const ctxPayload = core.channel.reply.finalizeInboundContext({
      ...
      MediaPath: ctxMediaPath,
      MediaUrl: ctxMediaPath,
      MediaPaths: ctxMediaPaths,
      MediaUrls: ctxMediaPaths,
  ...
  // Handle incoming reactions to messages the bot sent.
  // IncomingReaction fires only when another user reacts to the bot's own message,
  // so no "own" mode filter is needed (unlike Telegram which checks wasSentByBot).
  trackListener(
    "IncomingReaction",
    async ({
      chatId,
      ...
      try {
        // Skip removed reactions (empty string means the reaction was cleared).
        if (!reaction?.trim()) {
          return;
        }

        // Build sender label from contact info.
        let senderLabel = `id:${contactId}`;
        try {
          const contact = await dc!.rpc.getContact(account.id, contactId);
          // Skip bot reactions (parallel to Telegram's is_bot check).
          if (contact.isBot) {
            return;
          }
          ...
          if (name && addr && name !== addr) {
            senderLabel = `${name} (${addr})`;
          } else if (addr) {
            senderLabel = addr;
          }
        } catch {
          // Keep fallback label if contact lookup fails.
        }

        // Resolve agent route to get sessionKey (same logic as IncomingMsg).
        const chat = await dc!.rpc.getBasicChatInfo(account.id, chatId);
        const isGroup = chat.chatType === C.DC_CHAT_TYPE_GROUP;
        const route = core.channel.routing.resolveAgentRoute({
          cfg,
        ...
        const text = `Delta.Chat reaction added: ${reaction} by ${senderLabel} on msg ${msgId}`;
        core.system.enqueueSystemEvent(text, {
          sessionKey: route.sessionKey,
          contextKey: `deltachat:reaction:add:${chatId}:${msgId}:${contactId}:${reaction}`,
        });
        runtime.log?.(`deltachat: reaction event enqueued: ${text}`);
      } catch (err) {
        runtime.error?.(`deltachat reaction handler failed: ${String(err)}`);
      }
    },
  );
