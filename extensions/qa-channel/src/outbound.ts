...
/** Validate and deliver every attachment and its caption in one QA bus message. */
export async function sendQaChannelMediaBatch(
  ...
  // Resolve the complete batch before sending so a later invalid attachment
  // cannot leave a visible partial message or duplicate the user's caption.
  ...
  return await sendQaChannelText({
    ...params,
    attachments,
  });
}

/** Deliver media and its caption atomically without exposing host-local paths. */
