...
async function applyGoogleChatInboundAccessPolicy(params) {
  ...
  if (isGroup) {
    ...
    if (groupUsers.length > 0) {
      ...
      warnDeprecatedUsersEmailEntries(logVerbose, normalizedGroupUsers2);
  ...
  const storeAllowFrom = !isGroup && dmPolicy !== "allowlist" && (dmPolicy !== "open" || shouldComputeAuth) ? await pairing.readAllowFromStore().catch(() => []) : [];
