		...
		Boolean(
			groupName &&
				keys.some((key) => {
					...
					return (
						trimmed === groupName ||
						trimmed.toLowerCase() === normalizedGroupName
					);
				}),
		);
...
function extractMentionInfo(
	...
	const mentionAnnotations = annotations.filter(
		(entry) => entry.type === "USER_MENTION",
	);
	const hasAnyMention = mentionAnnotations.length > 0;
	const botTargets = new Set(
		["users/app", botUser?.trim()].filter(Boolean) as string[],
	);
...
> {
	...
	warnMutableGroupKeysConfigured(
		logVerbose,
		account.config.groups ?? undefined,
	);
	...
	if (isGroup) {
		if (groupConfigResolved.deprecatedNameMatch) {
			logVerbose(
				`drop group message (deprecated mutable group key matched, space=${spaceId})`,
			);
			return { ok: false };
		}
		...
		const routeAccess = evaluateGroupRouteAccessForPolicy({
			...
			routeEnabled:
				groupEntry?.enabled !== false && groupEntry?.allow !== false,
		});
		if (!routeAccess.allowed) {
			if (routeAccess.reason === "disabled") {
				logVerbose(
					`drop group message (groupPolicy=disabled, space=${spaceId})`,
				);
	...
	const configAllowFrom = (account.config.dm?.allowFrom ?? []).map((v) =>
		String(v),
	);
	const normalizedGroupUsers = groupUsers.map((v) => String(v));
