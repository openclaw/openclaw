...
vi.mock("./targets.js", () => ({
  normalizeGoogleChatTarget: (raw) => {
    if (!raw?.trim()) return void 0;
    if (raw === "invalid-target") return void 0;
  ...
  isGoogleChatUserTarget: (value) => value.startsWith("users/"),
  isGoogleChatSpaceTarget: (value) => value.startsWith("spaces/"),
...
describe("googlechat outbound cfg threading", () => {
  ...
  it("threads resolved cfg into sendText account resolution", async () => {
    ...
    vi.mocked(resolveGoogleChatAccount).mockReturnValue(account);
