...
function primeCommonDefaults() {
	...
	resolveSenderScopedGroupPolicy.mockImplementation(
...
function allowInboundGroupTraffic(options?: {
	...
	resolveDmGroupAccessWithLists.mockReturnValue({
		decision: "allow",
		effectiveAllowFrom: [],
		effectiveGroupAllowFrom: options?.effectiveGroupAllowFrom ?? [
			"users/alice",
		],
	});
...
async function applyInboundAccessPolicy(
	overrides: Partial<
		Parameters<
			Awaited<
				typeof import("./monitor-access.js")
			>["applyGoogleChatInboundAccessPolicy"]
		>[0]
	>,
) {
	const { applyGoogleChatInboundAccessPolicy } = await import(
		"./monitor-access.js"
	);
...
describe("googlechat inbound access policy", () => {
	...
	it("allows group traffic when sender and mention gates pass", async () => {
		...
		core.channel.commands.resolveCommandAuthorizedFromAuthorizers.mockReturnValue(
			true,
		);
	...
	it("drops unauthorized group control commands", async () => {
		...
		expect(logVerbose).toHaveBeenCalledWith(
			"googlechat: drop control command from users/alice",
		);
	});
