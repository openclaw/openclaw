...
type ParsedGoogleChatInboundPayload =
	...
	| { ok: false };
type ParsedGoogleChatInboundSuccess = Extract<
...
function parseGoogleChatInboundPayload(
	...
	if (
		rawObj.commonEventObject?.hostApp === "CHAT" &&
		rawObj.chat?.messagePayload
	) {
		...
		addOnBearerToken = String(
			rawObj.authorizationEventObject?.systemIdToken ?? "",
		).trim();
	}

	const event = eventPayload as GoogleChatEvent;
	const eventType =
		event.type ?? (eventPayload as { eventType?: string }).eventType;
	...
	if (
		!event.space ||
		typeof event.space !== "object" ||
		Array.isArray(event.space)
	) {
		res.statusCode = 400;
		res.end("invalid payload");
		return { ok: false };
