...
const googleChatConfigAdapter =
	createScopedChannelConfigAdapter<ResolvedGoogleChatAccount>({
		...
		resolveAllowFrom: (account: ResolvedGoogleChatAccount) =>
		...
		resolveDefaultTo: (account: ResolvedGoogleChatAccount) =>
			account.config.defaultTo,
	});

const googlechatActions: ChannelMessageActionAdapter = {
	describeMessageTool: (ctx) =>
		googlechatMessageActions.describeMessageTool?.(ctx) ?? null,
	extractToolSend: (ctx) =>
		...
		messaging: {
			...
			targetResolver: {
				looksLikeId: (raw, normalized) => {
					...
					return (
						isGoogleChatSpaceTarget(value) || isGoogleChatUserTarget(value)
					);
				},
	...
	outbound: {
		...
		attachedResults: {
			...
			sendText: async ({ cfg, to, text, accountId, replyToId, threadId }) => {
				...
				const thread = (threadId ?? replyToId ?? undefined) as
					| string
					| undefined;
				const { sendGoogleChatMessage } = await loadGoogleChatChannelRuntime();
