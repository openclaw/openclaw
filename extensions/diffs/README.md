...
Read-only diff viewer plugin for **Mullusi** agents.
...
Set plugin-wide defaults in `~/.mullusi/mullusi.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/mullusi",
        },
      },
    },
  },
...
Before:
Mullusi supports plugins.

After:
Mullusi supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/mullusi-diffs`).
