...
function getCards() {
  const cards = [];
  for (const card of document.querySelectorAll(".oc-diff-card")) {
    const host = card.querySelector("[data-openclaw-diff-host]");
    const payloadNode = card.querySelector("[data-openclaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
    try {
...
function getHydrateProps(payload) {
  if (payload.fileDiff) {
...
function applyToolbarButtonStyles(button, active) {
  button.style.color = viewerState.theme === "dark" ? "rgba(226, 232, 240, 0.74)" : "rgba(15, 23, 42, 0.52)";
...
function backgroundIcon(active) {
  if (active) {
    return `<svg viewBox="0 0 16 16" aria-hidden="true">
...
function createToolbar() {
  ...
  toolbar.append(
    createToolbarButton({
      title: viewerState.backgroundEnabled ? "Hide background highlights" : "Show background highlights",
...
function createRenderOptions(payload) {
  return {
