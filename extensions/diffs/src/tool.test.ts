...
describe("diffs tool", () => {
	...
	it("accepts mode=file as an alias for file artifact rendering", async () => {
		...
		expect((result?.details as Record<string, unknown>).artifactId).toEqual(
			expect.any(String),
		);
		expect((result?.details as Record<string, unknown>).expiresAt).toEqual(
			expect.any(String),
		);
	});
	...
	it("routes tool context into artifact details for file mode", async () => {
		const screenshotter = createPngScreenshotter();
		const tool = createToolWithScreenshotter(
			store,
			screenshotter,
			DEFAULT_DIFFS_TOOL_DEFAULTS,
			{
