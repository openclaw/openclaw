...
function normalizeElevenLabsProviderConfig(
	...
	return {
		...
		voiceSettings: {
			stability:
				asNumber(rawVoiceSettings?.stability) ??
			...
			style:
				asNumber(rawVoiceSettings?.style) ??
			...
			speed:
				asNumber(rawVoiceSettings?.speed) ??
				DEFAULT_ELEVENLABS_VOICE_SETTINGS.speed,
		},
	};
}

function readElevenLabsProviderConfig(
	...
	return {
		...
		applyTextNormalization:
			(trimToUndefined(config.applyTextNormalization) as
				| "auto"
				| "on"
				| "off"
				| undefined) ?? defaults.applyTextNormalization,
		languageCode: trimToUndefined(config.languageCode) ?? defaults.languageCode,
		voiceSettings: {
			stability:
				asNumber(voiceSettings?.stability) ?? defaults.voiceSettings.stability,
			similarityBoost:
				asNumber(voiceSettings?.similarityBoost) ??
				defaults.voiceSettings.similarityBoost,
			style: asNumber(voiceSettings?.style) ?? defaults.voiceSettings.style,
			useSpeakerBoost:
				asBoolean(voiceSettings?.useSpeakerBoost) ??
	...
	try {
		switch (ctx.key) {
			...
			case "similarity_boost": {
				...
				return {
					handled: true,
					overrides: mergeVoiceSettingsOverride(ctx, {
...
export async function listElevenLabsVoices(params: {
	...
	const res = await fetch(
		`${normalizeElevenLabsBaseUrl(params.baseUrl)}/v1/voices`,
		{
			headers: {
...
export function buildElevenLabsSpeechProvider(): SpeechProviderPlugin {
	return {
		...
		resolveConfig: ({ rawConfig }) =>
			normalizeElevenLabsProviderConfig(rawConfig),
		parseDirectiveToken,
				...
				...(trimToUndefined(talkProviderConfig.languageCode) == null
					...
					: {
							languageCode: normalizeLanguageCode(
								trimToUndefined(talkProviderConfig.languageCode),
							),
						}),
				voiceSettings: {
					...base.voiceSettings,
		...
		resolveTalkOverrides: ({ params }) => {
			...
			const voiceSettings = {
				...
				...(asNumber(params.stability) == null
					? {}
					: { stability: asNumber(params.stability) }),
				...(asNumber(params.similarity) == null
					? {}
					: { similarityBoost: asNumber(params.similarity) }),
				...(asNumber(params.style) == null
					? {}
					: { style: asNumber(params.style) }),
			...
			return {
				...
				...(normalize == null
					...
					: {
							applyTextNormalization:
								normalizeApplyTextNormalization(normalize),
						}),
				...(language == null
					? {}
			...
			const audioBuffer = await elevenLabsTTS({
				...
				voiceSettings: {
					...
					...(asNumber(overrideVoiceSettings?.similarityBoost) == null
						? {}
						: {
								similarityBoost: asNumber(
									overrideVoiceSettings?.similarityBoost,
								),
							}),
					...
					...(asBoolean(overrideVoiceSettings?.useSpeakerBoost) == null
						? {}
						: {
								useSpeakerBoost: asBoolean(
									overrideVoiceSettings?.useSpeakerBoost,
								),
							}),
