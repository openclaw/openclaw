...
Designed for universal compatibility. The Gateway relay supplies 8 kHz G.711
μ-law microphone frames; the plugin converts them to raw PCM audio (16-bit
little-endian, mono) before sending binary WebSocket frames and receives JSON
events on the same connection.

### Client to server

- Binary frames: PCM `pcm_s16le` audio. The default output rate is 16 kHz;
  `sampleRate` controls the server-facing rate.
  ...
  dictation: {
    ...
    providers: {
      "openai-compatible-stt": {
        ...
        // Optional bearer token. Prefer a SecretRef or environment variable.
        apiKey: { source: "env", provider: "default", id: "OPENAI_COMPATIBLE_STT_API_KEY" },
...
The provider appears in the standalone dictation catalog under the label
...
page's Dictation section in the Control UI, or set:
...
- **whisper.cpp** - expose the [whisper.cpp server example][whisper-server]
