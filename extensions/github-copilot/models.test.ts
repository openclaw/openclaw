...
vi.mock("@mariozechner/pi-ai/oauth", async () => {
	const actual = await vi.importActual<
		typeof import("@mariozechner/pi-ai/oauth")
	...
	return {
		...
		modelRegistry: {
			find: (provider: string, id: string) =>
				registryModels[`${provider}/${id}`] ?? null,
		},
...
describe("resolveCopilotForwardCompatModel", () => {
	it("returns undefined for empty modelId", () => {
		...
		expect(
			resolveCopilotForwardCompatModel(createMockCtx("  ")),
		).toBeUndefined();
	});
	...
	it("creates synthetic model for arbitrary unknown model ID", () => {
		...
		expect((result as unknown as Record<string, unknown>).api).toBe(
			"openai-responses",
		);
		expect((result as unknown as Record<string, unknown>).input).toEqual([
			"text",
			"image",
		]);
	});
