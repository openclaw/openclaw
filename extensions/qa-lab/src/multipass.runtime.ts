...
export function renderQaMultipassGuestScript(
  ...
  const lines = [
    ...
    '  curl -fsSL --connect-timeout 15 --max-time 120 "${base_url}/SHASUMS256.txt" -o "${node_tmp_dir}/SHASUMS256.txt" >>"$BOOTSTRAP_LOG" 2>&1',
    ...
    '  curl -fsSL --connect-timeout 15 --max-time 120 "${base_url}/${tarball_name}" -o "${node_tmp_dir}/${tarball_name}" >>"$BOOTSTRAP_LOG" 2>&1',
