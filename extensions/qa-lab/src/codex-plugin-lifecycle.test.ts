...
describe("codex plugin lifecycle: pinned-old codex plugin with new OpenClaw", () => {
  it("blocks with a precise update remediation when the plugin is older than the host", async () => {
    ...
    await seedCodexPluginAt("2026.5.9-beta.1", agentDir);
    ...
    expect(result.remediation).toBe(
      'Codex plugin version 2026.5.9-beta.1 is older than OpenClaw 2026.5.10-beta.1. Run "openclaw plugins update codex" or unpin codex, then rerun "openclaw doctor --fix".',
    );
  });
});
...
describe("codex plugin lifecycle: pinned-new codex plugin with old OpenClaw", () => {
  it("blocks with a precise host-upgrade remediation when the plugin is newer than the host", async () => {
    ...
    await seedCodexPluginAt("2026.5.11-beta.1", agentDir);
    await seedAuthProfiles("oauth-only", agentDir);
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toBe(
      "Codex plugin version 2026.5.11-beta.1 requires a newer OpenClaw host than 2026.5.10-beta.1. Upgrade OpenClaw or install a codex plugin version pinned to 2026.5.10-beta.1.",
    );
  });
});
