...
describe("qa scenario catalog", () => {
  ...
  it("keeps remember-across-conversations isolated and product-only", () => {
    ...
    const serializedFlow = JSON.stringify(scenario.execution.flow);
    const pauseExpression = serializedFlow.match(
      /"expr":"([^"]*candidate\.direction === 'outbound'[^"]*config\.pausedConversationId[^"]*)"/,
    )?.[1];
    expect(pauseExpression).toBeDefined();
    expect(serializedFlow).toContain('"saveAs":"pauseCommandOutbound"');
    expect(serializedFlow).not.toContain("pauseCommandMessages.find");
    const matchesPause = new Function("candidate", "config", `return (${pauseExpression});`) as (
      candidate: unknown,
      config: unknown,
    ) => boolean;
    const pauseConfig = { pausedConversationId: "remember-paused" };
    expect(
      matchesPause(
        { direction: "outbound", conversation: { id: "remember-group", kind: "channel" } },
        pauseConfig,
      ),
    ).toBe(false);
    expect(
      matchesPause(
        { direction: "outbound", conversation: { id: "remember-paused", kind: "direct" } },
        pauseConfig,
      ),
    ).toBe(true);
  });
