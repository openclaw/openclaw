...
`activi.invoke --each`) without adding new Activi code per workflow.
...
This extension depends on Activi internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Activi extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.activi/extensions` as a standalone plugin directory.
