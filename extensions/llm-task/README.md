...
`smart-agent-neo.invoke --each`) without adding new SmartAgentNeo code per workflow.
...
This extension depends on SmartAgentNeo internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SmartAgentNeo extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.smart-agent-neo/extensions` as a standalone plugin directory.
