...
`EasyHub.invoke --each`) without adding new EasyHub code per workflow.
...
This extension depends on EasyHub internal modules (the embedded agent runner).
It is intended to ship as a **bundled** EasyHub extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.easyhub/extensions` as a standalone plugin directory.
