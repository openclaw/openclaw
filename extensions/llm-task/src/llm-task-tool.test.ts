	...
	default: class MockAjv {
		compile(schema: unknown) {
			return (value: unknown) => {
				...
				(
					this as { errors?: Array<{ instancePath: string; message: string }> }
				).errors = undefined;
				return true;
			};
		}
...
describe("llm-task tool (json-only)", () => {
	...
	it("throws on invalid thinking level", async () => {
		const tool = createLlmTaskTool(fakeApi());
		await expect(
			tool.execute("id", { prompt: "x", thinking: "banana" }),
		).rejects.toThrow(/invalid thinking level/i);
	...
	it("throws on unsupported xhigh thinking level", async () => {
		const tool = createLlmTaskTool(fakeApi());
		await expect(
			tool.execute("id", { prompt: "x", thinking: "xhigh" }),
