...
// NOTE: This extension is intended to be bundled with DNA.
// When running from source (tests/dev), DNA internals live under src/.
...
export function createLlmTaskTool(api: DNAPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-llm-task-"));
