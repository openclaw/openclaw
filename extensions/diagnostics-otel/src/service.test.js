...
const sdkStart = vi.hoisted(() => vi.fn().mockResolvedValue(void 0));
const sdkShutdown = vi.hoisted(() => vi.fn().mockResolvedValue(void 0));
const logEmit = vi.hoisted(() => vi.fn());
const logShutdown = vi.hoisted(() => vi.fn().mockResolvedValue(void 0));
...
function createOtelContext(endpoint, { traces = false, metrics = false, logs = false } = {}) {
  return {
    config: {
      diagnostics: {
        enabled: true,
...
async function emitAndCaptureLog(logObj) {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    registeredTransports[0]?.({
      ...
      1: "hello",
      _meta: { logLevelName: "INFO", date: /* @__PURE__ */ new Date() }
    });
    expect(logEmit).toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  test("appends signal path when endpoint contains non-signal /v1 segment", async () => {
    ...
    await service.start(ctx);
    const options = traceExporterCtor.mock.calls[0]?.[0];
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    const emitCall = await emitAndCaptureLog({
      ...
      1: "auth configured",
      _meta: { logLevelName: "DEBUG", date: /* @__PURE__ */ new Date() }
    });
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const attrs = sessionCounter?.add.mock.calls[0]?.[1];
