...
function buildGoogleMeetAudioCommands(
  ...
  return MeetingPlatformAdapter.resolveAudioRuntimeForFormat({
    backend,
    bufferBytes,
    format,
    platform:
...
const DEFAULT_GOOGLE_MEET_AUDIO_COMMANDS = MeetingPlatformAdapter.resolveAudioRuntimeForFormat({
  backend: "blackhole-2ch",
  platform: "darwin",
  bufferBytes: DEFAULT_GOOGLE_MEET_AUDIO_BUFFER_BYTES,
  format: "pcm16-24khz",
});
