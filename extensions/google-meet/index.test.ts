...
import { readGoogleMeetAudioBase64 } from "./src/audio-base64.js";
...
type TestNodeAgentEngineParams = Omit<
  Parameters<typeof startMeetingAgentRealtimeEngine>[0],
  "config" | "consultAgent" | "platform" | "transport"
> & {
  config: GoogleMeetConfig;
...
async function startTestNodeAgentAudioBridge(params: TestNodeAgentEngineParams) {
  const { nodeId, bridgeId, ...engineParams } = params;
  const transport = createNodeMeetingRealtimeAudioTransport({
    runtime: params.runtime,
    nodeId,
...
describe("google-meet plugin", () => {
  ...
  it("rejects malformed paired-node realtime audio chunks before provider input", async () => {
    vi.useFakeTimers();
    const sendAudio = vi.fn();
    const bridge = {
      connect: vi.fn(async () => {}),
      ...
      nodes: {
        invoke: vi.fn(async ({ params }: { params?: { action?: string } }) => {
          if (params?.action === "pullAudio") {
            pullCount += 1;
            if (pullCount === 1) {
              return { bridgeId: "bridge-1", base64: "not-base64!" };
            }
            if (pullCount === 2) {
              return { bridgeId: "bridge-1", base64: Buffer.from([5, 4, 3]).toString("base64") };
            }
    ...
    try {
      ...
      expect(sendAudio).not.toHaveBeenCalledWith(Buffer.from("not-base64!", "base64"));
      expect(bridge.close).not.toHaveBeenCalled();
      await vi.waitFor(() => {
        expect(idlePullStarted).toBe(true);
      });
  ...
  it("rejects malformed paired-node agent audio chunks before transcription input", async () => {
    vi.useFakeTimers();
    const sendAudio = vi.fn();
    const sttSession = {
      connect: vi.fn(async () => {}),
      ...
      nodes: {
        invoke: vi.fn(async ({ params }: { params?: { action?: string } }) => {
          if (params?.action === "pullAudio") {
            ...
            if (pullCount === 2) {
              return { bridgeId: "bridge-1", base64: Buffer.from([6, 5, 4]).toString("base64") };
            }
            idlePullStarted = true;
            await new Promise<void>((resolve) => {
              releaseIdlePull = resolve;
    ...
    let handle: Awaited<ReturnType<typeof startTestNodeAgentAudioBridge>> | undefined;

    try {
      handle = await startTestNodeAgentAudioBridge({
        config: resolveGoogleMeetConfig({
          chrome: { audioFormat: "g711-ulaw-8khz" },
          realtime: { provider: "openai", agentId: "jay", introMessage: "" },
        }),
        fullConfig: {} as never,
        runtime: runtime as never,
      ...
      await vi.advanceTimersByTimeAsync(250);
      await vi.waitFor(() => {
        expect(sendAudio).toHaveBeenCalledWith(Buffer.from([6, 5, 4]));
      });
      expect(sendAudio).not.toHaveBeenCalledWith(Buffer.from("not-base64!", "base64"));
  ...
  it("stops paired-node realtime audio after repeated malformed input chunks", async () => {
    vi.useFakeTimers();
    const bridge = {
      connect: vi.fn(async () => {}),
      sendAudio: vi.fn(),
    ...
    try {
      ...
      for (let attempt = 0; attempt < 6 && bridge.close.mock.calls.length === 0; attempt += 1) {
        await vi.advanceTimersByTimeAsync(250);
      }
      await vi.waitFor(
        () => {
      ...
      expect(health.lastInputError).toBe("pullAudio base64 must be a valid audio payload");
      expect(bridge.sendAudio).not.toHaveBeenCalled();
      const stopCall = runtime.nodes.invoke.mock.calls
        .map(([call]) => call)
        .find((call) => isRecord(call.params) && call.params.action === "stop");
      const stop = requireRecord(stopCall, "malformed pull stop call");
      expect(stop.nodeId).toBe("node-1");
      expect(stop.command).toBe("googlemeet.chrome");
      expect(stop.params).toStrictEqual({ action: "stop", bridgeId: "bridge-1" });
      expect(stop.timeoutMs).toBe(5_000);
