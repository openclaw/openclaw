...
async function resolveDirectRoomId(client: MatrixClient, userId: string): Promise<string> {
  ...
  if (!trimmed.startsWith("@")) {
    throw new Error(`DeJoy user IDs must be fully qualified (got "${trimmed}")`);
  }
...
export async function resolveMatrixRoomId(client: MatrixClient, raw: string): Promise<string> {
  ...
  if (lowered.startsWith("dejoy:")) {
    return await resolveMatrixRoomId(client, target.slice("dejoy:".length));
  }
  ...
  if (target.startsWith("#")) {
    ...
    if (!resolved) {
      throw new Error(`DeJoy alias ${target} could not be resolved`);
    }
    return resolved;
  }
  return target;
