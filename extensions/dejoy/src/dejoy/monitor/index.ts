...
/** Grace window (ms) before process start: events within this window are not dropped on startup/reconnect. */
...
export async function monitorMatrixProvider(opts: MonitorMatrixOpts = {}): Promise<void> {
  ...
  const resolveUserAllowlist = async (
    ...
    if (unresolved.length > 0) {
      runtime.log?.(
        `${label} entries must be full DeJoy IDs (example: @user:server). Unresolved entries are ignored.`,
      );
    }
    return allowList;
  };
