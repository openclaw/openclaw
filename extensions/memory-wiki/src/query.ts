...
function scoreClaimMatch(params: {
  ...
  let score = 0;
  const textLower = normalizeLowercaseStringOrEmpty(params.text);
  const idLower = normalizeLowercaseStringOrEmpty(params.id);
  if (params.signals.queryLower && textLower.includes(params.signals.queryLower)) {
    score += 25;
  }
  score += scoreTokenCoverage(textLower, params.signals, 4, 16);
  if (params.signals.queryLower && idLower.includes(params.signals.queryLower)) {
    score += 10;
  }
  score += scoreTokenCoverage(idLower, params.signals, 2, 6);
...
function scoreWikiMetadataMatch(params: {
  ...
  const idLower = normalizeLowercaseStringOrEmpty(params.id);
  if (titleLower === params.signals.queryLower) {
    score += 50;
  } else if (params.signals.queryLower && titleLower.includes(params.signals.queryLower)) {
    score += 20;
  }
  score += scoreTokenCoverage(titleLower, params.signals, 3, 9);
  if (params.signals.queryLower && pathLower.includes(params.signals.queryLower)) {
    score += 10;
  }
  score += scoreTokenCoverage(pathLower, params.signals, 2, 6);
  if (params.signals.queryLower && idLower.includes(params.signals.queryLower)) {
    score += 20;
  }
  score += scoreTokenCoverage(idLower, params.signals, 3, 9);
  if (
    params.sourceIds.some((sourceId) =>
      matchesQuery(normalizeLowercaseStringOrEmpty(sourceId), params.signals),
    )
  ) {
    score += 12;
  }
