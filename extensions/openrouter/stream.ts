...
function createOpenRouterDeepSeekV4ReplayWrapper(baseStreamFn: StreamFn | undefined): StreamFn {
  return createPayloadPatchStreamWrapper(
    baseStreamFn,
    ({ payload }) => {
...
export function wrapOpenRouterProviderStream(
  ...
  if (!wrapStreamFn) {
    return createOpenRouterBilledCostWrapper(
      createOpenRouterAnthropicPrefillWrapper(
        createOpenRouterDeepSeekV4ReplayWrapper(routedStreamFn),
      ),
    );
  }
  ...
  return createOpenRouterBilledCostWrapper(
    createOpenRouterAnthropicPrefillWrapper(
      createOpenRouterDeepSeekV4ReplayWrapper(wrappedStreamFn),
    ),
  );
}
