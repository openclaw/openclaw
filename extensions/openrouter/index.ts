...
function injectOpenRouterRouting(
	...
	return (model, context, options) =>
		(
			baseStreamFn ??
			((nextModel, _nextContext, _nextOptions) => {
				throw new Error(
		...
		api.registerProvider({
			...
			wrapStreamFn: (ctx) => {
				...
				const providerRouting =
					ctx.extraParams?.provider != null &&
					typeof ctx.extraParams.provider === "object"
				...
				const openRouterThinkingLevel = skipReasoningInjection
					? undefined
					: ctx.thinkingLevel;
