...
export function _setFalFetchGuardForTesting(
	impl: typeof fetchWithSsrFGuard | null,
): void {
	falFetchGuard = impl ?? fetchWithSsrFGuard;
}

function mergeSsrFPolicies(
...
function resolveFalNetworkPolicy(
	...
	const hostPolicy = buildHostnameAllowlistPolicyFromSuffixAllowlist([
		hostSuffix,
	]);
...
function resolveFalImageSize(params: {
	...
	if (normalizedAspectRatio && params.hasInputImages) {
		throw new Error(
			"fal image edit endpoint does not support aspectRatio overrides",
		);
	}
...
async function fetchImageBuffer(
	...
	const downloadPolicy = (() => {
		...
		try {
			...
			return matchesTrustedHostSuffix(parsed.hostname, trustedSuffix)
				? trustedPolicy
				: undefined;
		} catch {
			return undefined;
	...
	return {
		...
		async generateImage(req) {
			...
			if ((req.inputImages?.length ?? 0) > 1) {
				throw new Error(
					"fal image generation currently supports at most one reference image",
				);
			}
