...
describe("fal image-generation provider", () => {
  ...
  it("routes GPT Image 2 edits to /edit with image_urls and image_size", async () => {
    vi.spyOn(providerAuth, "resolveApiKeyForProvider").mockResolvedValue({
      apiKey: "fal-test-key",
      source: "env",
      mode: "api-key",
    ...
    expectFalJsonPost({
      ...
      body: {
        ...
        num_images: 1,
        output_format: "png",
        image_urls: ["data:image/png;base64,b25l", "data:image/png;base64,dHdv"],
      },
    });
  });

  it("routes NB2 edits to /edit with image_urls, aspect_ratio and resolution", async () => {
    vi.spyOn(providerAuth, "resolveApiKeyForProvider").mockResolvedValue({
      apiKey: "fal-test-key",
      source: "env",
      mode: "api-key",
  ...
  it("allows multi-image for GPT Image 2 edit", async () => {
    vi.spyOn(providerAuth, "resolveApiKeyForProvider").mockResolvedValue({
      apiKey: "fal-test-key",
      source: "env",
      mode: "api-key",
    ...
    await provider.generateImage({
      ...
      inputImages: [
        ...
        { buffer: Buffer.from("c"), mimeType: "image/png" },
      ],
    });
  ...
  it("preserves explicit /edit endpoint for non-GPT/NB2 models", async () => {
    vi.spyOn(providerAuth, "resolveApiKeyForProvider").mockResolvedValue({
      apiKey: "fal-test-key",
      source: "env",
      mode: "api-key",
    ...
    await provider.generateImage({
      provider: "fal",
      model: "fal-ai/custom/edit",
      prompt: "edit with custom model",
      cfg: {},
      inputImages: [{ buffer: Buffer.from("x"), mimeType: "image/png" }],
    });

    // Should NOT append /image-to-image; custom /edit must be preserved
    expectFalJsonPost({
      call: 1,
      url: "https://fal.run/fal-ai/custom/edit",
      body: {
