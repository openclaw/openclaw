...
describe("Crabbox cleanup settlement", () => {
  it.each(["complete", "indeterminate"] as const)(
    "does not replay already %s cleanup",
    async (outcome) => {
      const { params } = createSetupFixture();
      const original = new Error("setup failed");
      const settled =
        outcome === "complete"
          ? WorkerProviderError.cleanupComplete(params.inspect.id, original)
          : WorkerProviderError.cleanupIndeterminate(
              params.inspect.id,
              original,
              new Error("stop uncertain"),
            );
      await expect(
        failProvisionAfterCleanup({ ...params, id: params.inspect.id }, settled),
