...
import {
  ...
  type WorkerProfile,
  type WorkerProvider as WorkerProviderContract,
...
} from "./crabbox-worker-config-validation.js";
import {
...
type WorkerProvider = WorkerProviderContract<1>;
    ...
    return async () => {
      ...
      if (project && warmImages.lookupLease(leaseId)?.phase !== "enrolled") {
        ...
        try {
          ...
          // A project grant can retire independently of the initiating authority.
          project.assertCurrent();
          return await failProvisionAfterCleanup({ ...context, id: leaseId, stopLease }, error);
        }
