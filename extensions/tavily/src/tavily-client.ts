...
export async function runTavilySearch(
	...
	const timeoutSeconds = resolveTavilySearchTimeoutSeconds(
	...
	const results = rawResults.map((r: Record<string, unknown>) => ({
		title:
			typeof r.title === "string" ? wrapWebContent(r.title, "web_search") : "",
		url: typeof r.url === "string" ? r.url : "",
		snippet:
			typeof r.content === "string"
				? wrapWebContent(r.content, "web_search")
				: "",
...
export async function runTavilyExtract(
	...
	const baseUrl = resolveTavilyBaseUrl(params.cfg);
	const timeoutSeconds = resolveTavilyExtractTimeoutSeconds(
		params.timeoutSeconds,
	);
		...
		...(Array.isArray(r.images)
			? {
					images: (r.images as string[]).map((img) =>
						wrapExternalContent(String(img), {
							source: "web_fetch",
							includeWarning: false,
						}),
					),
