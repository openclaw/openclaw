...
describe("tavily tools", () => {
	...
	beforeAll(async () => {
		vi.resetModules();
		({ createTavilyWebSearchProvider } = await import(
			"./tavily-search-provider.js"
		));
		...
		({ __testing: tavilyClientTesting } =
			await vi.importActual<typeof import("./tavily-client.js")>(
				"./tavily-client.js",
			));
	});
	...
	beforeEach(() => {
		...
		runTavilySearch.mockImplementation(
			async (params: Record<string, unknown>) => params,
		);
		runTavilyExtract.mockReset();
		runTavilyExtract.mockImplementation(async (params: unknown) => ({
			ok: true,
			params,
		}));
		vi.unstubAllEnvs();
	...
	it("normalizes optional parameters before invoking Tavily", async () => {
		runTavilySearch.mockImplementationOnce(
	...
	it("rejects chunks_per_source without query", async () => {
		...
		await expect(
			...
			"tavily_extract requires query when chunks_per_source is set.",
		);
	...
	it("falls back to environment values and defaults", () => {
		...
		expect(resolveTavilyBaseUrl({} as OpenClawConfig)).not.toBe(
			DEFAULT_TAVILY_BASE_URL,
		);
		expect(resolveTavilySearchTimeoutSeconds()).toBe(
			DEFAULT_TAVILY_SEARCH_TIMEOUT_SECONDS,
		);
		expect(resolveTavilyExtractTimeoutSeconds()).toBe(
			DEFAULT_TAVILY_EXTRACT_TIMEOUT_SECONDS,
		);
	});
	...
	it("accepts positive numeric timeout overrides and floors them", () => {
		...
		expect(resolveTavilySearchTimeoutSeconds(0)).toBe(
			DEFAULT_TAVILY_SEARCH_TIMEOUT_SECONDS,
		);
	...
	it("appends endpoints to reverse-proxy base urls", () => {
		expect(
			tavilyClientTesting.resolveEndpoint(
				"https://proxy.example/api/tavily",
				"/search",
			),
		).toBe("https://proxy.example/api/tavily/search");
		expect(
			tavilyClientTesting.resolveEndpoint(
				"https://proxy.example/api/tavily/",
				"/extract",
