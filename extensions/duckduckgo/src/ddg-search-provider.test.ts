...
describe("duckduckgo web search provider", () => {
	...
	beforeAll(async () => {
		vi.resetModules();
		({ createDuckDuckGoWebSearchProvider } = await import(
			"./ddg-search-provider.js"
		));
		({ __testing: ddgClientTesting } =
			await vi.importActual<typeof import("./ddg-client.js")>(
				"./ddg-client.js",
			));
	...
	beforeEach(() => {
		...
		runDuckDuckGoSearch.mockImplementation(
	...
	it("decodes direct and redirect urls plus common html entities", () => {
		...
		expect(ddgClientTesting.decodeDuckDuckGoUrl("https://example.com")).toBe(
			"https://example.com",
		);
		expect(
			ddgClientTesting.decodeHtmlEntities(
				"Fish &amp; Chips&nbsp;&hellip; &#39;ok&#39;",
			),
		).toBe("Fish & Chips ... 'ok'");
	});
