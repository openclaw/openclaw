...
export const api = {
  ...
  getAgentFiles: (agentId: string) =>
    get<{ files: AgentFileInfo[] }>(`/agents/${agentId}/files`),
  ...
  updateElementPosition: (workflowId: string, elementId: string, position: { x: number; y: number }) =>
  ...
  validateWorkflow: (workflowId: string) =>
    post<{ valid: boolean; errors: BpmnValidationError[] }>(`/workflows/${workflowId}/validate`, {}),
};
