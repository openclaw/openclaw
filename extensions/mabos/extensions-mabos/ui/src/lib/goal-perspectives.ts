...
export function getAllPerspectives(goalModel?: TroposGoalModel): GoalPerspectiveConfig[] {
  const actorPerspective = goalModel
    ...
    : { id: "actor" as const, label: "Actor", description: "Group by Tropos actor", icon: "Users", groups: [] };

  return [levelPerspective, actorPerspective, typePerspective, bscPerspective, goaDomainPerspective];
}
