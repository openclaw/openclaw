...
function getEfficiencyLabel(avgDuration: number): {
  ...
  if (avgDuration <= 2.5) return { label: "Excellent", color: "var(--accent-green)" };
  if (avgDuration <= 4.0) return { label: "Good", color: "var(--accent-blue)" };
  if (avgDuration <= 5.5) return { label: "Average", color: "var(--accent-orange)" };
...
export function AgentPerformance({ agentMetrics, isLoading }: AgentPerformanceProps) {
  ...
  const maxTasks = useMemo(() => Math.max(...metrics.map((m) => m.tasksCompleted), 1), [metrics]);
