...
export default function register(api) {
    ...
    const gatewayAuthConfig = api.config?.gateway?.auth ?? {};
    const resolvedAuth = resolveGatewayAuth({
        authConfig: gatewayAuthConfig,
    });
    const authRateLimiter = resolvedAuth.mode !== "none"
    ...
    async function requireAuth(req, res) {
        ...
        if (referer.includes("/mabos/dashboard"))
            return true;
    ...
    async function readMabosJsonBody(req, res, opts) {
        ...
        if (!result.ok) {
            ...
            const message = result.code === "PAYLOAD_TOO_LARGE" ? "Request body too large" : "Invalid JSON body";
                ...
                try {
                    ...
                    for (const agentId of agents) {
                        ...
                        import("./src/knowledge/typedb-dashboard.js")
                            .then(({ writeBdiCycleResultToTypeDB }) => writeBdiCycleResultToTypeDB(agentId, "mabos", {
                    ...
                    api.logger.warn?.(`[mabos-bdi] Cycle error: ${err instanceof Error ? err.message : String(err)}`);
                }
            };
            // Initial cycle
            await runCycle();
    ...
    api.registerCli(({ program }) => {
        ...
        mabos
            ...
            if (businessName && orchestrateTool) {
                ...
                try {
                    const result = await orchestrateTool.execute("cli", {
        ...
        mabos
            ...
            try {
                ...
                const { migrate } = (await import(/* webpackIgnore: true */ migratePath));
            ...
            try {
                ...
                try {
                    ...
                    if (typedbAgents && typedbAgents.length > 0) {
                        const typedbMap = new Map(typedbAgents.map((a) => [a.id, a]));
    ...
    api.registerHttpRoute({
        ...
        handler: async (_req, res) => {
            ...
            try {
                ...
                allDecisions.sort((a, b) => (urgencyOrder[a.urgency] ?? 2) - (urgencyOrder[b.urgency] ?? 2));
                res.setHeader("Content-Type", "application/json");
    ...
    const registerParamRoute = (pattern, handler) => {
    ...
    registerParamRoute("/mabos/api/decisions/:id/resolve", async (req, res) => {
        ...
        try {
            ...
            const params = await readMabosJsonBody(req, res);
            if (!params)
                return;
            ...
            const idx = queue.findIndex((d) => d.id === params.decision_id);
            if (idx === -1) {
                res.statusCode = 404;
            ...
            // Notify agent
            if (decision.agent) {
                const inboxPath = join(workspaceDir, "businesses", bizId, "agents", decision.agent, "inbox.json");
                const inbox = (await readJsonSafe(inboxPath)) || [];
                inbox.push({
                    id: `DEC-${params.decision_id}-resolved`,
                    from: "stakeholder",
                    to: decision.agent,
                    performative: params.resolution === "approved"
                        ? "ACCEPT"
    ...
    api.registerHttpHandler(async (req, res) => {
        ...
        try {
            const { join } = await import("node:path");
            const { readdir, stat: fsStat, readFile: fsReadFile, writeFile: fsWriteFile, } = await import("node:fs/promises");
    ...
    api.registerHttpHandler(async (req, res) => {
        ...
        try {
            const { join } = await import("node:path");
            const { readFile: fsReadFile, writeFile: fsWriteFile, unlink, stat: fsStat, mkdir, } = await import("node:fs/promises");
    ...
    registerParamRoute("/mabos/api/agents/:id/knowledge", async (req, res) => {
        try {
            ...
            try {
                const { queryKnowledgeStatsFromTypeDB } = await import("./src/knowledge/typedb-dashboard.js");
    ...
    api.registerHttpRoute({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if (typeof params.business_id !== "string" ||
                    params.business_id.length > 64 ||
                    !/^[a-zA-Z0-9_-]+$/.test(params.business_id)) {
                    res.statusCode = 400;
                    res.setHeader("Content-Type", "application/json");
                    res.end(JSON.stringify({
                        error: "Invalid business_id: must be alphanumeric with hyphens/underscores, max 64 chars",
                    }));
                    return;
                }
                ...
                for (const role of ROLES) {
                    ...
                    if (existsSync(templatePersona)) {
                        ...
                        await writeFile(join(agentPath, "Persona.md"), `# Persona — ${role.toUpperCase()}\n\n**Role:** ${role.toUpperCase()}\n**Business:** ${params.name}\n`, "utf-8");
                    }
                    ...
                    if (existsSync(templateCaps)) {
                        await writeFile(join(agentPath, "Capabilities.md"), await readFile(templateCaps, "utf-8"), "utf-8");
                    }
                    ...
                    for (const f of [
                        ...
                        await writeFile(join(agentPath, f), `# ${f.replace(".md", "")} — ${role.toUpperCase()}\n\nInitialized: ${now.split("T")[0]}\nBusiness: ${params.name}\n`, "utf-8");
                    }
                    await writeFile(join(agentPath, "inbox.json"), "[]", "utf-8");
                    await writeFile(join(agentPath, "cases.json"), "[]", "utf-8");
                    await writeFile(join(agentPath, "facts.json"), JSON.stringify({ facts: [], version: 0 }, null, 2), "utf-8");
                    await writeFile(join(agentPath, "rules.json"), JSON.stringify({ rules: [], version: 0 }, null, 2), "utf-8");
                    await writeFile(join(agentPath, "memory-store.json"), JSON.stringify({ working: [], short_term: [], long_term: [], version: 0 }, null, 2), "utf-8");
                }
                // 3. Generate BMC
                const bmc = {
                ...
                await writeFile(join(bizDir, "business-model-canvas.json"), JSON.stringify(bmc, null, 2), "utf-8");
                // 4. Create shared resources
                await writeFile(join(bizDir, "decision-queue.json"), "[]", "utf-8");
                await writeFile(join(bizDir, "metrics.json"), JSON.stringify({ metrics: [], snapshots: [] }, null, 2), "utf-8");
                await writeFile(join(bizDir, "README.md"), `# ${params.name}\n\n**Legal:** ${params.legal_name || params.name}\n**Type:** ${params.type}\n**Created:** ${now}\n\n${params.description || ""}\n`, "utf-8");
                // 5. Orchestrate if requested
                if (params.orchestrate) {
                    // 5a. Spawn domain agents
                    const domainAgentDefs = {
                        ecommerce: [
                            {
                                id: "inventory-mgr",
                    ...
                    for (const agent of agents) {
                        ...
                        await mkdir(agentPath, { recursive: true });
                        await writeFile(join(agentPath, "Persona.md"), `# Persona — ${agent.name}\n\n**Role:** ${agent.name}\n**Agent ID:** ${agent.id}\n**Type:** Domain-specific\n\n## Identity\n${agent.role}\n`, "utf-8");
                        for (const f of [
                            "Capabilities.md",
                            "Beliefs.md",
                            "Desires.md",
                            ...
                            await writeFile(join(agentPath, f), `# ${f.replace(".md", "")} — ${agent.name}\n\nInitialized: ${now.split("T")[0]}\n`, "utf-8");
                        }
                        await writeFile(join(agentPath, "inbox.json"), "[]", "utf-8");
                        await writeFile(join(agentPath, "cases.json"), "[]", "utf-8");
                    }
                    ...
                    try {
                        const { loadOntologies, mergeOntologies, exportSBVRForTypeDB } = await import("./src/ontology/index.js");
                        const ontologies = loadOntologies();
                        const graph = mergeOntologies(ontologies);
                        const sbvrExport = exportSBVRForTypeDB(graph);
                        await writeFile(join(bizDir, "sbvr-export.json"), JSON.stringify(sbvrExport, null, 2), "utf-8");
                    }
                    catch (err) {
                    ...
                    await writeFile(join(bizDir, "onboarding-progress.json"), JSON.stringify(progress, null, 2), "utf-8");
                }
                // 6. Generate Tropos goal model from stakeholder goals if provided
                if (params.goals && params.goals.length > 0) {
                    const goalMapping = params.goals.map((g, i) => {
                    ...
                    await writeFile(join(bizDir, "tropos-goal-model.json"), JSON.stringify(tropos, null, 2), "utf-8");
                }
                res.setHeader("Content-Type", "application/json");
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if (!params.agentId || !params.message || !params.businessId) {
                    res.statusCode = 400;
                    res.setHeader("Content-Type", "application/json");
                    res.end(JSON.stringify({ error: "Missing required fields: agentId, message, businessId" }));
                    return;
                }
                ...
                const inboxPath = join(workspaceDir, "businesses", businessId, "agents", agentId, "inbox.json");
                let inbox = [];
                try {
    ...
    api.registerHttpRoute({
        ...
        handler: async (req, res) => {
            ...
            const unsubscribe = onAgentEvent((evt) => {
                ...
                const matchesAgent = evt.sessionKey?.includes(agentId) || evt.data?.agentId === agentId;
                if (!matchesAgent)
                    return;
                try {
            ...
            const outboxPath = join(workspaceDir, "businesses", businessId, "agents", agentId, "outbox.json");
    ...
    registerParamRoute("/mabos/api/businesses/:id/goals", async (req, res) => {
        try {
            ...
            if (req.method === "PUT") {
                // Update goal model
                const goalModel = await readMabosJsonBody(req, res);
                if (!goalModel)
                    return;
                const { writeFile, mkdir } = await import("node:fs/promises");
                await mkdir(bizDir, { recursive: true });
                await writeFile(join(bizDir, "tropos-goal-model.json"), JSON.stringify(goalModel, null, 2), "utf-8");
                // Cascade: update agent Goals.md files
                if (goalModel.goals && goalModel.actors) {
                    for (const actor of goalModel.actors) {
                        if (actor.type === "agent") {
                            const agentGoals = goalModel.goals.filter((g) => g.actor === actor.id);
                            if (agentGoals.length > 0) {
                                const goalsContent = `# Goals — ${actor.id.toUpperCase()}\n\nUpdated: ${new Date().toISOString().split("T")[0]}\n\n${agentGoals.map((g) => `## ${g.id}: ${g.text}\n- **Type:** ${g.type}\n- **Priority:** ${g.priority}\n- **Status:** active\n`).join("\n")}`;
            ...
            if (!goalModel) {
                ...
                const goals = [];
                const actors = [{ id: "stakeholder", type: "principal", goals: [], x: 400, y: 50 }];
                if (manifest?.agents) {
        ...
        try {
            ...
            for (const agentId of agentEntries) {
                ...
                for (const line of lines) {
                    ...
                    const rowMatch = line.match(/^\|\s*(S-\d+)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|/);
    ...
    registerParamRoute("/mabos/api/businesses/:id/agents", async (req, res) => {
        try {
            const { readFile, readdir, stat: fsStat, writeFile: wf, mkdir: mk, } = await import("node:fs/promises");
            const { join } = await import("node:path");
            const { existsSync } = await import("node:fs");
            const url = new URL(req.url || "", "http://localhost");
            const segments = url.pathname.split("/");
            ...
            if (req.method === "POST") {
                ...
                for (const f of [
                    ...
                    await wf(join(agentPath, f), `# ${f.replace(".md", "")} — ${params.name || newId}\n\nInitialized: ${now.split("T")[0]}\n`, "utf-8");
                }
                await wf(join(agentPath, "Persona.md"), `# Persona — ${params.name || newId}\n\n**Role:** ${params.name || newId}\n**Agent ID:** ${newId}\n**Type:** ${params.type || "domain"}\n`, "utf-8");
    ...
    registerParamRoute("/mabos/api/businesses/:id/agents/:agentId/archive", async (req, res) => {
        ...
        try {
            ...
            // Update manifest
            const manifest = (await readJsonSafe(join(bizDir, "manifest.json"))) || {};
            manifest.agents = (manifest.agents || []).filter((a) => a !== agentId);
            manifest.domain_agents = (manifest.domain_agents || []).filter((a) => a !== agentId);
            await writeFile(join(bizDir, "manifest.json"), JSON.stringify(manifest, null, 2), "utf-8");
            res.setHeader("Content-Type", "application/json");
    ...
    registerParamRoute("/mabos/api/businesses/:id/agents/:agentId", async (req, res) => {
        ...
        try {
            ...
            if (params.autonomy_level !== undefined)
                config.autonomy_level = params.autonomy_level;
        ...
        try {
            ...
            if (req.method === "POST") {
                ...
                if (params.workflowId)
                    newJob.workflowId = params.workflowId;
                if (params.stepId)
                    newJob.stepId = params.stepId;
                jobs.push(newJob);
            ...
            const heartbeatJob = jobs.find((j) => j.id === "CRON-heartbeat");
            if (heartbeatJob) {
            ...
            const filteredJobs = filterWorkflowId
                ? jobs.filter((j) => j.workflowId === filterWorkflowId)
                : jobs;
            res.setHeader("Content-Type", "application/json");
    ...
    registerParamRoute("/mabos/api/businesses/:id/cron/:jobId", async (req, res) => {
        ...
        try {
            ...
            if (params.enabled !== undefined)
                jobs[idx].enabled = params.enabled;
            if (params.schedule !== undefined)
                jobs[idx].schedule = params.schedule;
            if (params.name !== undefined)
                jobs[idx].name = params.name;
            if (params.status !== undefined)
    ...
    api.registerHttpRoute({
        ...
        handler: async (_req, res) => {
            try {
                ...
                res.setHeader("Content-Type", "text/html");
                res.end(`<!DOCTYPE html><html><head><title>MABOS</title></head><body style="background:#0d1117;color:#c9d1d9;font-family:sans-serif;padding:40px"><h1 style="color:#58a6ff">MABOS Dashboard</h1><p>Dashboard files not found. Run <code>cd extensions/mabos/ui && npm run build</code> first.</p></body></html>`);
            }
        },
    });
    ...
    api.registerTool((ctx) => {
        ...
        return {
            ...
            description: "Search agent memories using the native BM25 + vector search engine. " +
            ...
            async execute(toolCallId, params) {
                ...
                return memorySearchTool.execute(toolCallId, {
                    query: params.query,
    ...
    api.registerHttpRoute({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if (req.method === "POST") {
                    // Create workflow
                    const body = await readMabosJsonBody(req, res);
                    if (!body)
                        return;
        ...
        try {
            ...
            if (req.method === "PATCH" || req.method === "PUT") {
                ...
                if (body.position && Object.keys(body).length <= 2) {
                    const typeql = BpmnStoreQueries.updateElementPosition(elementId, body.position.x, body.position.y);
                    await client.insertData(typeql, BPMN_DB);
                }
                else {
                    // Full field update
                    ...
                    if (body.element_type !== undefined)
                        fields.element_type = body.element_type;
                    if (body.task_type_bpmn !== undefined)
                        fields.task_type_bpmn = body.task_type_bpmn;
                    if (body.gateway_type !== undefined)
                        fields.gateway_type = body.gateway_type;
                    if (body.documentation !== undefined)
                        fields.documentation = body.documentation;
    ...
    registerParamRoute("/mabos/api/workflows/:id/validate", async (req, res) => {
        ...
        try {
            ...
            // Check orphan nodes
            try {
                const orphanResult = await client.matchQuery(BpmnStoreQueries.queryOrphanNodes(workflowId), BPMN_DB);
    ...
    api.on("before_agent_start", async (_event, ctx) => {
        if (ctx.workspaceDir) {
            ...
            try {
                ...
                const commitments = await readFile(join(agentDir, "Commitments.md"), "utf-8").catch(() => null);
    ...
    api.on("after_tool_call", async (event, _ctx) => {
        if (event.toolName.startsWith("belief_") ||
