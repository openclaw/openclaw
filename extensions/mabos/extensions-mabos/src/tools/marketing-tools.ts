...
async function apiCall(
  ...
  try {
    ...
    // Some providers are picky about GET requests carrying content headers.
    if (body && method !== "GET" && !requestHeaders["Content-Type"]) {
    ...
    if (body && method !== "GET") {
      opts.body =
        typeof body === "string" || body instanceof URLSearchParams ? body : JSON.stringify(body);
    }
    ...
    // --- Platform Connection ---
    {
      ...
      async execute(_id: string, params: Static<typeof PlatformConnectParams>) {
        ...
        if (params.platform === "meta" || params.platform === "instagram") {
          ...
          // Prefer ad account validation because /user_account requires
          // additional scope (user_accounts:read) that is not needed
          ...
          if (adAccounts.status === 200) {
            ...
            testResult =
              user.status === 200
                ? `✅ Connected as: ${user.data.username || "OK"}`
                : `❌ Error: ${adAccounts.status}`;
          }
