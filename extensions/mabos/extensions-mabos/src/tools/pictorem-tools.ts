  ...
  return [
    // ── Queue List ─────────────────────────────────────────────
    {
      ...
      parameters: Type.Object({
        ...
        limit: Type.Optional(
          Type.Number({ description: "Max items to return (default 50)" }),
        ),
      }),
      async execute(
        _id: string,
    ...
    // ── Order Status ───────────────────────────────────────────
    {
      ...
      description:
        "Get detailed fulfillment status for a specific Shopify order number.",
      parameters: Type.Object({
      ...
      async execute(_id: string, params: { order_number: string }) {
        ...
        if (status === 404)
          return textResult(
            `No fulfillment items found for order #${params.order_number}.`,
          );
        if (status !== 200)
          return textResult(`**Error** (HTTP ${status}): ${JSON.stringify(data)}`);
    ...
    // ── Retry Fulfillment ──────────────────────────────────────
    {
      ...
      async execute(_id: string, params: { order_number: string }) {
        ...
        const lines: string[] = [
          `## Retry Results — Order #${d.order_number}`,
          "",
        ];
    ...
    // ── Trigger Fulfillment ────────────────────────────────────
    {
      ...
      async execute(_id: string, params: { order_number: string }) {
        ...
        if (status === 404)
          return textResult(
            `Order #${params.order_number} not found in Shopify.`,
          );
        if (status !== 200)
          return textResult(`**Error** (HTTP ${status}): ${JSON.stringify(data)}`);
