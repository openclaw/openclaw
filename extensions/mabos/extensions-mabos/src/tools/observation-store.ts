...
export async function saveObservationLog(
  ...
  // Bridge to OpenClaw memory indexer (fire-and-forget)
  void (async () => {
    const { materializeObservations } = await import("./memory-materializer.js");
    await materializeObservations(api, agentId);
  })().catch(() => {});
}
