  ...
  return {
    ...
    parameters: Type.Object({
      ...
      flowId: Type.Optional(
        Type.String({
          description:
            "Managed flow to inspect or resume. Resume uses its saved approval or input checkpoint when token and approvalId are omitted.",
        }),
      ),
    ...
    async execute(_id: string, params: Record<string, unknown>, signal?: AbortSignal) {
      ...
      if (action === "status") {
        const result = await inspectManagedLobsterFlows(
