...
import { resolveReactionSyntheticEvent } from "./monitor.js";
import { setFeishuRuntime } from "./runtime.js";
const handleFeishuMessageMock = vi.hoisted(() => vi.fn(async (_params) => {
}));
...
vi.mock("./bot.js", async () => {
  const actual = await vi.importActual("./bot.js");
  return {
    ...actual,
...
function createFetchedReactionMessage(chatId, chatType) {
  return {
    messageId: "om_msg1",
    chatId,
    chatType,
...
async function resolveNonBotReaction(params) {
  return await resolveReactionSyntheticEvent({
    cfg: params?.cfg ?? cfg,
    accountId: "default",
...
async function enqueueDebouncedMessage(onMessage, event) {
...
describe("resolveReactionSyntheticEvent", () => {
  ...
  it("drops unverified reactions when sender verification times out", async () => {
    ...
    const result = await resolveReactionSyntheticEvent({
      ...
      fetchMessage: async () => await new Promise(() => {
      })
    });
    expect(result).toBeNull();
  });
...
describe("Feishu inbound debounce regressions", () => {
  ...
  it("passes prefetched botName through to handleFeishuMessage", async () => {
    ...
    await vi.advanceTimersByTimeAsync(25);
    expect(handleFeishuMessageMock).toHaveBeenCalledTimes(1);
    const firstParams = handleFeishuMessageMock.mock.calls[0]?.[0];
  ...
  it("excludes previously processed retries from combined debounce text", async () => {
    ...
    const combined = JSON.parse(dispatched.message.content);
