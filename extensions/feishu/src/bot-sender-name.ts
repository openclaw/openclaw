...
function shouldSuppressPermissionErrorNotice(
	permissionError: FeishuPermissionError,
): boolean {
	const message = permissionError.message.toLowerCase();
	return IGNORED_PERMISSION_SCOPE_TOKENS.some((token) =>
...
function resolveSenderLookupIdType(
	senderId: string,
	...
	try {
		...
		if (permErr) {
			if (shouldSuppressPermissionErrorNotice(permErr)) {
				log(
					`feishu: ignoring stale permission scope error: ${permErr.message}`,
				);
				return {};
			}
			log(
		...
		log(
			`feishu: failed to resolve sender name for ${normalizedSenderId}: ${String(err)}`,
		);
		return {};
	}
}
