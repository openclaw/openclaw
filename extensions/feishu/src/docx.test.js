...
describe("feishu_doc image fetch hardening", () => {
  ...
  function resolveFeishuDocTool(context = {}) {
    ...
    const tool = registerTool.mock.calls.map((call) => call[0]).map((candidate) => typeof candidate === "function" ? candidate(context) : candidate).find((candidate) => candidate.name === "feishu_doc");
  ...
  it("falls back to size-based convert chunking for long no-heading markdown", async () => {
    ...
    blockDescendantCreateMock.mockImplementation(async ({ data }) => ({
      code: 0,
      data: {
        children: data.children_id.map((id) => ({
          block_id: id
        }))
      }
    }));
