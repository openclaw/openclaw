...
function getDescribedActions(cfg: OpenClawConfig): string[] {
	return [
		...(feishuPlugin.actions?.describeMessageTool?.({ cfg })?.actions ?? []),
	];
}

function createLegacyFeishuButtonCard(value: {
	command?: string;
	text?: string;
}) {
	return {
...
async function expectLegacyFeishuCardPayloadRejected(
	cfg: OpenClawConfig,
	card: unknown,
) {
	await expect(
...
describe("feishuPlugin actions", () => {
	...
	it("fetches channel info", async () => {
		...
		expect(getChatInfoMock).toHaveBeenCalledWith(
			{ tag: "client" },
			"oc_group_1",
		);
		expect(result?.details).toMatchObject({
	...
	it("fails channel-list when live discovery fails", async () => {
		listFeishuDirectoryGroupsLiveMock.mockRejectedValueOnce(
