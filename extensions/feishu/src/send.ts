...
function sanitizeCardText(raw: string): string {
  // Strip ANSI escape sequences and control characters including \r (CWE-117)
  // eslint-disable-next-line no-control-regex
  return raw.replace(/\x1B\[[0-9;]*[A-Za-z]/g, "").replace(/[\x00-\x08\x0B-\x1F\x7F]/g, "");
}
...
function parseInteractiveCardContent(parsed: unknown): string {
  ...
  // Extract standard header title (card.header.title.content)
  if (!texts.length) {
    const header = card.header as Record<string, unknown> | undefined;
    const headerTitle = header?.title as Record<string, unknown> | undefined;
    const headerContent =
      typeof headerTitle?.content === "string" ? headerTitle.content.trim() : "";
    if (headerContent) {
      const remaining = CARD_MAX_OUTPUT_CHARS - totalChars;
      if (remaining > 0) {
        const clamped =
          headerContent.length > remaining ? headerContent.slice(0, remaining) : headerContent;
        texts.push(clamped);
        totalChars += clamped.length;
      }
    }
