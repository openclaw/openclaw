...
export const FeishuConfigSchema = z
	...
	.superRefine((value, ctx) => {
		...
		const defaultVerificationTokenConfigured = hasConfiguredSecretInput(
			value.verificationToken,
		);
		const defaultEncryptKeyConfigured = hasConfiguredSecretInput(
			value.encryptKey,
		);
		if (defaultConnectionMode === "webhook") {
			...
			if (!defaultEncryptKeyConfigured) {
				ctx.addIssue({
					...
					path: ["encryptKey"],
					message:
						'channels.feishu.connectionMode="webhook" requires channels.feishu.encryptKey',
				});
			}
		}
		...
		for (const [accountId, account] of Object.entries(value.accounts ?? {})) {
			...
			const accountVerificationTokenConfigured =
				...
				defaultVerificationTokenConfigured;
			const accountEncryptKeyConfigured =
				hasConfiguredSecretInput(account.encryptKey) ||
				defaultEncryptKeyConfigured;
