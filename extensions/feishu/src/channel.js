...
const feishuPlugin = {
  ...
  actions: {
    ...
    supportsCards: ({ cfg }) => {
      return cfg.channels?.feishu?.enabled !== false && Boolean(resolveFeishuCredentials(cfg.channels?.feishu));
    },
    handleAction: async (ctx) => {
      const account = resolveFeishuAccount({ cfg: ctx.cfg, accountId: ctx.accountId ?? void 0 });
      if ((ctx.action === "react" || ctx.action === "reactions") && !isFeishuReactionsActionEnabled({ cfg: ctx.cfg, account })) {
      ...
      if (ctx.action === "send" && ctx.params.card) {
        ...
        if (!to) {
          return {
            isError: true,
            content: [{ type: "text", text: "Feishu card send requires a target (to)." }],
            details: { error: "Feishu card send requires a target (to)." }
          };
        }
        const replyToMessageId = typeof ctx.params.replyTo === "string" ? ctx.params.replyTo.trim() || void 0 : void 0;
      ...
      if (ctx.action === "react") {
        const messageId = typeof ctx.params.messageId === "string" && ctx.params.messageId.trim() || typeof ctx.params.message_id === "string" && ctx.params.message_id.trim() || void 0;
        if (!messageId) {
        ...
        if (remove) {
          ...
          if (!ownReaction) {
            return {
              content: [
                { type: "text", text: JSON.stringify({ ok: true, removed: null }) }
              ],
          ...
          return {
            content: [
              { type: "text", text: JSON.stringify({ ok: true, removed: emoji }) }
            ],
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ ok: true, added: emoji }) }],
      ...
      if (ctx.action === "reactions") {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ ok: true, reactions }) }],
