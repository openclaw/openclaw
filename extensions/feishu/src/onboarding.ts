...
import { addWildcardAllowFrom, DEFAULT_ACCOUNT_ID, formatDocsLink } from "smart-agent-neo/plugin-sdk";
...
function setFeishuDmPolicy(cfg: NeobotConfig, dmPolicy: DmPolicy): NeobotConfig {
  const allowFrom =
    dmPolicy === "open"
...
function setFeishuAllowFrom(cfg: NeobotConfig, allowFrom: string[]): NeobotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
function setFeishuGroupAllowFrom(cfg: NeobotConfig, groupAllowFrom: string[]): NeobotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
