...
export function restoreFeishuLifecycleStateDir(
	...
	setFeishuRuntime(
		createPluginRuntimeMock({
			channel: {
				...
				pairing: {
					readAllowFromStore:
						params.readAllowFromStore ?? vi.fn().mockResolvedValue([]),
	...
	await replayFeishuLifecycleEvent({
		...
		waitForSecond: () =>
			vi.waitFor(
				() => {
					expect(params.dispatchReplyFromConfigMock).toHaveBeenCalledTimes(1);
					expect(params.createFeishuReplyDispatcherMock).toHaveBeenCalledTimes(
						1,
					);
				},
				waitTimeoutMs == null ? undefined : { timeout: waitTimeoutMs },
...
export function expectFeishuReplyDispatcherSentFinalReplyOnce(params: {
	...
	const dispatcher = params.createFeishuReplyDispatcherMock.mock.results[0]
		?.value.dispatcher as {
...
export async function setupFeishuLifecycleHandler<
	T extends RuntimeEnv,
>(params: {
