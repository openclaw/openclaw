...
describe("FeishuStreamingSession", () => {
  ...
  it("retains prior visible content when CardKit rejects a divergent close replacement", async () => {
    ...
    await session.update(next);
    // close() distinguishes prior visible content from the rejected requested rewrite.
    await expect(session.close()).resolves.toEqual({
      contentVisible: true,
  ...
  it("logs CardKit body errors for note updates and streaming close settings", async () => {
    ...
    await expect(session.close(undefined, { note: "model note" })).resolves.toEqual({
      contentVisible: true,
  ...
  it.each([
    ...
    async ({ name, stateText, businessErrorCodes, contentVisible }) => {
      ...
      expect(log).toHaveBeenCalledWith(
        `Final ${name} failed: Error: ${name === "update" ? "Update" : "Replace"} card content failed: rejected (code=300317)`,
      );
    },
  );
