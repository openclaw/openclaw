...
vi.mock("./streaming-card.js", async () => {
  const actual = await vi.importActual("./streaming-card.js");
  return {
    ...
    FeishuStreamingSession: class {
      constructor() {
        ...
        this.isActive = vi.fn(() => this.active);
...
describe("createFeishuReplyDispatcher streaming behavior", () => {
  beforeEach(() => {
    ...
    sendMediaFeishuMock.mockResolvedValue(void 0);
    sendStructuredCardFeishuMock.mockResolvedValue(void 0);
    ...
    getFeishuRuntimeMock.mockReturnValue({
      channel: {
        ...
        reply: {
          ...
          resolveHumanDelayConfig: vi.fn(() => void 0)
        }
      }
    });
  });
  ...
  function createDispatcherHarness(overrides = {}) {
  ...
  it("skips typing indicator for stale replayed messages", async () => {
    createFeishuReplyDispatcher({
      ...
      chatId: "oc_chat",
      replyToMessageId: "om_parent",
      messageCreateTimeMs: Date.now() - 3 * 6e4
    });
    const options = createReplyDispatcherWithTypingMock.mock.calls[0]?.[0];
    await options.onReplyStart?.();
    expect(addTypingIndicatorMock).not.toHaveBeenCalled();
  ...
  it("treats second-based timestamps as stale for typing suppression", async () => {
    createFeishuReplyDispatcher({
      ...
      chatId: "oc_chat",
      replyToMessageId: "om_parent",
      messageCreateTimeMs: Math.floor((Date.now() - 3 * 6e4) / 1e3)
    });
    const options = createReplyDispatcherWithTypingMock.mock.calls[0]?.[0];
    await options.onReplyStart?.();
    expect(addTypingIndicatorMock).not.toHaveBeenCalled();
  ...
  it("keeps typing indicator for fresh messages", async () => {
    createFeishuReplyDispatcher({
      ...
      chatId: "oc_chat",
      replyToMessageId: "om_parent",
      messageCreateTimeMs: Date.now() - 3e4
    });
    const options = createReplyDispatcherWithTypingMock.mock.calls[0]?.[0];
    await options.onReplyStart?.();
  ...
  it("streams reasoning content as blockquote before answer", async () => {
    ...
    expect(streamingInstances).toHaveLength(1);
    const updateCalls = streamingInstances[0].update.mock.calls.map((c) => c[0]);
    const reasoningUpdate = updateCalls.find((c) => c.includes("Thinking"));
    expect(reasoningUpdate).toContain("> \u{1F4AD} **Thinking**");
    ...
    expect(streamingInstances[0].close).toHaveBeenCalledTimes(1);
    const closeArg = streamingInstances[0].close.mock.calls[0][0];
    expect(closeArg).toContain("> \u{1F4AD} **Thinking**");
  ...
  it("recovers streaming after start() throws (HTTP 400)", async () => {
    ...
    streamingInstances.push = function(...args) {
      if (shouldFailStart) {
        args[0].start = vi.fn().mockRejectedValue(new Error("Create card request failed with HTTP 400"));
    ...
    try {
      createFeishuReplyDispatcher({
        cfg: {},
        agentId: "agent",
        runtime: { log: vi.fn(), error: errorMock },
        chatId: "oc_chat"
      });
      const options = createReplyDispatcherWithTypingMock.mock.calls[0]?.[0];
      await options.deliver({ text: "```ts\nconst x = 1\n```" }, { kind: "block" });
