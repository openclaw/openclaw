...
import { type ClawdbotConfig, withTempDownloadPath } from "../runtime-api.js";
...
async function readFeishuResponseBuffer(params: {
	...
	if (
		responseWithOptionalFields.code !== undefined &&
		responseWithOptionalFields.code !== 0
	) {
		throw new Error(
	...
	if (
		...
		Buffer.isBuffer(responseWithOptionalFields.data)
	) {
	...
	if (responseWithOptionalFields[Symbol.asyncIterator]) {
		const asyncIterable = responseWithOptionalFields as AsyncIterable<
	...
	throw new Error(
		`${params.errorPrefix}: unexpected response format. Keys: [${keys.join(", ")}]`,
	);
}
