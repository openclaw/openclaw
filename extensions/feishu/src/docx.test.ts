...
import {
	...
	type ToolLike,
} from "./tool-factory-test-harness.js";
...
vi.spyOn(toolAccountModule, "resolveFeishuToolAccount").mockImplementation(
	(...args) => resolveFeishuToolAccountMock(...args),
);
...
describe("feishu_doc image fetch hardening", () => {
	...
	it("reorders convert output by document tree instead of raw block array order", async () => {
		...
		expect(
			(call?.data.descendants as Array<{ block_id: string }>).map(
				(b) => b.block_id,
			),
		).toEqual(["h1", "p1", "h2", "list1", "li1", "li2"]);
	});
	...
	it("rejects traversal paths in upload_file via loadWebMedia sandbox", async () => {
		loadWebMediaMock.mockRejectedValueOnce(
			new Error(
				"Local media path is not under an allowed directory: /etc/passwd",
			),
		);
