						...
						buildFeishuCardButton({
							...
							value: createFeishuCardInteractionEnvelope({
								...
								m: {
									command: "/new",
									prompt:
										"Start a fresh session? This will reset the current chat context.",
								},
								c: context,
							}),
						}),
						buildFeishuCardButton({
							...
							value: createFeishuCardInteractionEnvelope({
								...
								m: {
									command: "/reset",
									prompt:
										"Reset this session now? Any active conversation state will be cleared.",
								},
								c: context,
							}),
						}),
...
export async function maybeHandleFeishuQuickActionMenu(params: {
	...
	const expiresAt =
		(params.now ?? Date.now()) + FEISHU_QUICK_ACTION_CARD_TTL_MS;
	try {
