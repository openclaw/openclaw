...
describe("handleFeishuMessage command authorization", () => {
  ...
  it("keeps quote-only replies to file messages instead of dropping them as empty", async () => {
    mockShouldComputeCommandAuthorized.mockReturnValue(false);
    const quotedRawContent = JSON.stringify({
      file_key: "file_quote_only_payload",
      file_name: "quote-only.docx",
    });
    mockGetMessageFeishu.mockResolvedValueOnce({
      messageId: "om_parent_quote_only_file",
      chatId: "oc-group",
      chatType: "group",
      senderId: "ou-file-sender",
      senderType: "user",
    ...
    mockDownloadMessageResourceFeishu.mockResolvedValueOnce({
      buffer: Buffer.from("quote-only-file"),
      contentType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      fileName: "quote-only.docx",
    });
    mockSaveMediaBuffer.mockResolvedValueOnce({
      id: "quote-only.docx",
      path: "/tmp/quote-only.docx",
      size: Buffer.byteLength("quote-only-file"),
      contentType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    });
    ...
    const event: FeishuMessageEvent = {
      ...
      message: {
        message_id: "om_quote_only_reply_to_file",
        parent_id: "om_parent_quote_only_file",
        chat_id: "oc-group",
        chat_type: "group",
        message_type: "text",
    ...
    expect(mockDownloadMessageResourceFeishu).toHaveBeenCalledWith(
      expect.objectContaining({
        messageId: "om_parent_quote_only_file",
        fileKey: "file_quote_only_payload",
        type: "file",
      }),
    );
    expect(mockDispatchReplyFromConfig).toHaveBeenCalled();
    expect(mockFinalizeInboundContext).toHaveBeenCalledWith(
      expect.objectContaining({
        ReplyToBody: "[file message]",
        MediaPath: "/tmp/quote-only.docx",
        MediaPaths: ["/tmp/quote-only.docx"],
        MediaTypes: ["application/vnd.openxmlformats-officedocument.wordprocessingml.document"],
      }),
    );
  });
