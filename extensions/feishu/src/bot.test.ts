...
type ConfiguredBindingRoute = ReturnType<
	typeof ConversationRuntime.resolveConfiguredBindingRoute
>;
type BoundConversation = ReturnType<
	ReturnType<
		typeof ConversationRuntime.getSessionBindingService
...
function createConfiguredBindingReadiness(
	...
	return (
		ok ? { ok: true } : { ok: false, error: error ?? "unknown error" }
	) as BindingReadiness;
}
...
const resolveAgentRouteMock: PluginRuntime["channel"]["routing"]["resolveAgentRoute"] =
	(params) => mockResolveAgentRoute(params);
const readSessionUpdatedAtMock: PluginRuntime["channel"]["session"]["readSessionUpdatedAt"] =
	(params) => mockReadSessionUpdatedAt(params);
const resolveStorePathMock: PluginRuntime["channel"]["session"]["resolveStorePath"] =
	(params) => mockResolveStorePath(params);
...
const {
	...
	mockResolveAgentRoute: vi.fn((_params?: unknown) =>
		buildDefaultResolveRoute(),
	),
	mockReadSessionUpdatedAt: vi.fn(
		(_params?: unknown): number | undefined => undefined,
	),
	mockResolveStorePath: vi.fn(
		(_params?: unknown) => "/tmp/feishu-sessions.json",
	),
...
vi.mock("openclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
	...
	return {
		...
		resolveConfiguredBindingRoute: (params: unknown) =>
			mockResolveConfiguredBindingRoute(
				params as { route: ResolvedAgentRoute },
			),
...
describe("handleFeishuMessage ACP routing", () => {
	...
	it("routes Feishu topic messages through active bound conversations", async () => {
		...
		expect(mockTouchBinding).toHaveBeenCalledWith(
			"default:oc_group_chat:topic:om_topic_root",
		);
	});
});
...
describe("handleFeishuMessage command authorization", () => {
	...
	it("falls back to Date.now() when create_time is absent", async () => {
		...
		const call = mockFinalizeInboundContext.mock.calls[0]?.[0] as {
			Timestamp: number;
		};
