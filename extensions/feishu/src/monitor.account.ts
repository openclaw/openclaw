...
async function retryBotIdentityProbe(
  ...
  for (let i = 0; i < BOT_IDENTITY_RETRY_DELAYS_MS.length; i++) {
    ...
    await new Promise((r) => setTimeout(r, BOT_IDENTITY_RETRY_DELAYS_MS[i]));
    if (abortSignal?.aborted) return;
  ...
  error(
    `feishu[${accountId}]: bot identity background retry exhausted; bot will respond to all group messages until restart`,
  );
}
