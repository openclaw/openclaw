...
function normalizeFeishuChatType(value: unknown): FeishuChatType | undefined {
	return value === "group" || value === "private" || value === "p2p"
		? value
		: undefined;
}
...
function parseFeishuMessageEventPayload(
	value: unknown,
...
function parseFeishuBotAddedEventPayload(
	value: unknown,
): FeishuBotAddedEvent | null {
	if (
		!isRecord(value) ||
		!readString(value.chat_id) ||
		!isRecord(value.operator_id)
	) {
		return null;
	}
...
function parseFeishuCardActionEventPayload(
	value: unknown,
): FeishuCardActionEvent | null {
	if (!isRecord(value)) {
		return null;
	}
	const operator = value.operator;
...
function registerEventHandlers(
	...
	const recordSuppressedMessageIds = async (
		...
		const suppressedIds = new Set(
			entries
				...
				.filter(
					(id): id is string =>
						Boolean(id) && (!keepMessageId || id !== keepMessageId),
				),
		);
	...
	const isMessageAlreadyProcessed = async (
		entry: FeishuMessageEvent,
	): Promise<boolean> => {
		return await hasProcessedFeishuMessage(
	...
	const inboundDebouncer =
		core.channel.debounce.createInboundDebouncer<FeishuMessageEvent>({
			...
			onFlush: async (entries) => {
				...
				await recordSuppressedMessageIds(
					dedupedEntries,
					dispatchEntry.message.message_id,
				);
			...
			onError: (err, entries) => {
				...
				error(
					`feishu[${accountId}]: inbound debounce flush failed: ${String(err)}`,
				);
			},
		});
	...
	eventDispatcher.register({
		"im.message.receive_v1": async (data) => {
			...
			if (!tryBeginFeishuMessageProcessing(messageId, accountId)) {
				log(
					`feishu[${accountId}]: dropping duplicate event for message ${messageId}`,
				);
				return;
			}
		...
		"application.bot.menu_v6": async (data) => {
			try {
				...
				if (
					...
					log(
						`feishu[${accountId}]: dropping duplicate bot-menu event for ${syntheticMessageId}`,
					);
					return;
				}
				if (!tryBeginFeishuMessageProcessing(syntheticMessageId, accountId)) {
					log(
						`feishu[${accountId}]: dropping in-flight bot-menu event for ${syntheticMessageId}`,
					);
					return;
				}
				...
				const promise = maybeHandleFeishuQuickActionMenu({
					...
					.then(async (handledMenu) => {
						if (handledMenu) {
							await recordProcessedFeishuMessage(
								syntheticMessageId,
								accountId,
								log,
							);
				...
				if (fireAndForget) {
					promise.catch((err) => {
						error(
							`feishu[${accountId}]: error handling bot menu event: ${String(err)}`,
						);
					});
					return;
				}
...
export async function monitorSingleAccount(
	...
	if (connectionMode === "webhook" && !account.encryptKey?.trim()) {
		throw new Error(
			`Feishu account "${accountId}" webhook mode requires encryptKey`,
		);
	}
	...
	if (warmupCount > 0) {
		log(
			`feishu[${accountId}]: dedup warmup loaded ${warmupCount} entries from disk`,
		);
	}

	let threadBindingManager: ReturnType<
		typeof createFeishuThreadBindingManager
	> | null = null;
	try {
