...
function buildMultiAccountWebsocketConfig(accountIds) {
  return {
    channels: {
      feishu: {
        enabled: true,
...
async function waitForStartedAccount(started, accountId) {
...
describe("Feishu monitor startup preflight", () => {
  it("starts account probes sequentially to avoid startup bursts", async () => {
    ...
    probeFeishuMock.mockImplementation(async (account) => {
  ...
  it("does not refetch bot info after a failed sequential preflight", async () => {
    const started = [];
    let releaseBetaProbe;
    const betaProbeReleased = new Promise((resolve) => {
  ...
  it("continues startup when probe layer reports timeout", async () => {
    ...
    probeFeishuMock.mockImplementation((account) => {
      started.push(account.accountId);
      if (account.accountId === "alpha") {
