...
import { type FeishuMessageEvent, handleFeishuMessage } from "./bot.js";
import {
...
export async function handleFeishuCardAction(params: {
	...
	if (!claimedToken) {
		log(
			`feishu[${account.accountId}]: skipping duplicate card action token ${event.token}`,
		);
		return;
	}
	...
	try {
		...
		if (decoded.kind === "structured") {
			...
			if (envelope.a === FEISHU_APPROVAL_REQUEST_ACTION) {
				const command =
					typeof envelope.m?.command === "string"
						? envelope.m.command.trim()
						: "";
				if (!command) {
					await sendInvalidInteractionNotice({
						cfg,
				...
				await sendCardFeishu({
					...
					card: createApprovalCard({
						...
						chatType:
							envelope.c?.t ?? (event.context.chat_id ? "group" : "p2p"),
