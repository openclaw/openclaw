	...
	try {
		...
		if (messageType === "share_chat") {
			if (parsed && typeof parsed === "object") {
				...
				if (
					typeof share.share_chat_id === "string" &&
					share.share_chat_id.trim()
				) {
...
export function parseMergeForwardContent(params: {
	...
	subMessages.sort(
		(a, b) =>
			parseInt(a.create_time || "0", 10) - parseInt(b.create_time || "0", 10),
	);

	const lines = ["[Merged and Forwarded Messages]"];
	for (const item of subMessages.slice(0, maxMessages)) {
		lines.push(
			`- ${formatSubMessageContent(item.body?.content || "", item.msg_type || "text")}`,
		);
	}
		...
		const replacement =
			botStripId && mentionId === botStripId
				...
				: mentionId
					...
					: `@${mention.name}`;
		result = result
			.replace(new RegExp(escaped(mention.key), "g"), () => replacement)
			.trim();
	}
	return result;
}
...
export async function resolveFeishuMediaList(params: {
	...
	const { cfg, messageId, messageType, content, maxBytes, log, accountId } =
		params;
	const mediaTypes = [
		"image",
		"file",
	...
	if (messageType === "post") {
		...
		if (imageKeys.length > 0) {
			log?.(
				`feishu: post message contains ${imageKeys.length} embedded image(s)`,
			);
		}
		if (mediaKeys.length > 0) {
			log?.(
				`feishu: post message contains ${mediaKeys.length} embedded media file(s)`,
			);
		}
		...
		for (const imageKey of imageKeys) {
			try {
				...
				log?.(
					`feishu: downloaded embedded image ${imageKey}, saved to ${saved.path}`,
				);
			} catch (err) {
				log?.(
					`feishu: failed to download embedded image ${imageKey}: ${String(err)}`,
				);
			}
		}
		...
		for (const media of mediaKeys) {
			try {
				...
				log?.(
					`feishu: downloaded embedded media ${media.fileKey}, saved to ${saved.path}`,
				);
			} catch (err) {
				log?.(
					`feishu: failed to download embedded media ${media.fileKey}: ${String(err)}`,
				);
			}
		}
		return out;
