...
export function resolveFeishuReplyRouting(params: {
  ...
  /** Direct (DM) message that arrived inside a thread; reply stays in that thread. */
  directThreadReply?: boolean;
}): FeishuReplyRouting {
  ...
  if (!isGroup && params.directThreadReply) {
    // A DM that arrived inside a thread replies back into that thread, anchored
    // on the thread root when present so the exchange stays threaded.
    const directThreadRootId = params.rootId?.trim() || undefined;
    return {
      replyTargetMessageId: directThreadRootId ?? params.replyTargetMessageId ?? fallbackTarget,
      dispatchRootId: params.rootId,
      dispatchReplyInThread: true,
      threadReply: true,
    };
  }
