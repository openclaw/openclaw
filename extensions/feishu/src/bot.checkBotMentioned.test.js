...
function makeEvent(chatType, mentions, text = "hello") {
  return {
    sender: {
...
function makePostEvent(content) {
  return {
...
function makeShareChatEvent(content) {
  return {
    sender: { sender_id: { user_id: "u1", open_id: "ou_sender" } },
    message: {
      message_id: "msg_1",
...
describe("parseFeishuMessageEvent \u2013 mentionedBot", () => {
  ...
  it("returns mentionedBot=false when botOpenId is undefined (unknown bot)", () => {
    ...
    const ctx = parseFeishuMessageEvent(event, void 0);
    expect(ctx.mentionedBot).toBe(false);
  });
  ...
  it("returns mentionedBot=true for post message with at (no top-level mentions)", () => {
    const BOT_OPEN_ID2 = "ou_bot_123";
    const event = makePostEvent({
      content: [
        [{ tag: "at", user_id: BOT_OPEN_ID2, user_name: "claw" }],
        [{ tag: "text", text: "What does this document say" }]
      ]
    });
    const ctx = parseFeishuMessageEvent(event, BOT_OPEN_ID2);
    expect(ctx.mentionedBot).toBe(true);
  });
