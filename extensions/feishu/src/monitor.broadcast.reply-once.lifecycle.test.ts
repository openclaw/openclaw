...
function createLifecycleAccount(
...
describe("Feishu broadcast reply-once lifecycle", () => {
	beforeEach(() => {
		...
		mockFeishuReplyOnceDispatch({
			...
			shouldSendFinalReply: (ctx) =>
				typeof (ctx as { SessionKey?: string } | undefined)?.SessionKey ===
					"string" &&
	...
	it("uses one active reply path when the same broadcast event reaches two accounts", async () => {
		...
		await runFeishuLifecycleSequence(
			...
			[
				() => {
					expect(dispatchReplyFromConfigMock.mock.calls.length).toBeGreaterThan(
						0,
					);
				},
				() => {
		...
		expect(sessionKeys).toContain(
			"agent:susan:feishu:group:oc_broadcast_group",
		);

		const activeDispatcher = createFeishuReplyDispatcherMock.mock.results[0]
			?.value.dispatcher as {
		...
		dispatchReplyFromConfigMock.mockImplementationOnce(
			async ({ ctx, dispatcher }) => {
				if (
					...
					ctx.SessionKey.includes("agent:susan:")
				) {
