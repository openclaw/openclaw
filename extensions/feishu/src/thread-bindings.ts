...
const FEISHU_THREAD_BINDINGS_STATE_KEY = Symbol.for(
	"openclaw.feishuThreadBindingsState",
);
...
export function createFeishuThreadBindingManager(params: {
	...
	const sessionBindingAdapter: SessionBindingAdapter = {
		...
		bind: async (input) => {
			if (
				input.conversation.channel !== "feishu" ||
...
export function getFeishuThreadBindingManager(
	...
	return (
		getState().managersByAccountId.get(normalizeAccountId(accountId)) ?? null
	);
}
