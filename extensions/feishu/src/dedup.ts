...
import { createDedupeCache, createPersistentDedupe } from "activi/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.ACTIVI_STATE_DIR?.trim() || env.ACTIVI_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["activi-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".activi");
}
