...
export async function handleFeishuMessage(params: {
  ...
  try {
    ...
    // Skip messages with no text content, no direct media, and no quoted
    // context. Feishu can deliver empty-text events (e.g. `{"text":""}`) when a
    // user sends a blank message or when media parsing produces an empty string.
    // Fetch quote context first so quote-only replies to media/text are still
    // delivered to the agent instead of being dropped as blank user turns.
    if (!agentFacingCtx.content.trim() && mediaList.length === 0 && !quotedContent) {
      log(
        `feishu[${account.accountId}]: skipping empty message (no text, no media, no quote) from ${ctx.senderOpenId}`,
      );
      return;
    }
