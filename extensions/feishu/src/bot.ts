...
export function resolveBroadcastAgents(
	cfg: ClawdbotConfig,
	peerId: string,
): string[] | null {
...
export async function handleFeishuMessage(params: {
	...
	if (event.message.message_type === "merge_forward") {
		...
		try {
			...
			if (
				...
				log(
					`feishu[${account.accountId}]: merge_forward API returned no items`,
				);
				ctx = {
					...ctx,
			...
			log(
				`feishu[${account.accountId}]: merge_forward fetch failed: ${String(err)}`,
			);
	...
	if (ctx.mentionTargets && ctx.mentionTargets.length > 0) {
		...
		log(
			`feishu[${account.accountId}]: detected @ forward request, targets: [${names}]`,
		);
	}
	...
	const rawBroadcastAgents = isGroup
		? resolveBroadcastAgents(cfg, ctx.chatId)
		: null;
	...
	if (isGroup) {
		...
		const effectiveSenderAllowFrom =
			perGroupSenderAllowFrom.length > 0
				? perGroupSenderAllowFrom
				: globalSenderAllowFrom;
	...
	try {
		...
		if (isDirect && dmPolicy !== "open" && !dmAllowed) {
			if (dmPolicy === "pairing") {
				await pairing.issueChallenge({
					...
					onCreated: () => {
						log(
							`feishu[${account.accountId}]: pairing request sender=${ctx.senderOpenId}`,
						);
					},
		...
		const parentConversationId = isGroup
			? (parentPeer?.id ?? ctx.chatId)
			: undefined;
		...
		if (feishuAcpConversationSupported) {
			...
			if (threadBinding && boundSessionKey) {
				route = {
					...
					agentId:
						resolveAgentIdFromSessionKey(boundSessionKey) || route.agentId,
		...
		>();
		let rootMessageInfo:
			| Awaited<ReturnType<typeof getMessageFeishu>>
			| undefined;
		...
		const resolveThreadContextForAgent = async (
			...
			const previousThreadSessionTimestamp =
			...
			if (feishuThreadId) {
				log(
					`feishu[${account.accountId}]: resolved thread ID: ${feishuThreadId}`,
				);
			}
			if (!feishuThreadId) {
				log(
			...
			try {
						...
						? threadMessages.filter(
								(msg) =>
									...
									(msg.senderId !== undefined &&
										senderIds.has(msg.senderId.trim())),
							)
						: threadMessages) ?? [];

				const threadStarterBody =
					rootMsg?.content ?? relevantMessages[0]?.content;
				...
				log(
					`feishu[${account.accountId}]: failed to fetch thread history: ${String(err)}`,
				);
			}
		...
		if (broadcastAgents) {
			...
			const strategy =
				(
					(cfg as Record<string, unknown>).broadcast as
			...
			if (strategy === "sequential") {
				...
				const results = await Promise.allSettled(
					broadcastAgents.map(dispatchForAgent),
				);
