...
describe("resolveDefaultFeishuAccountId", () => {
	...
	it("reports selection source for configured defaults and mapped defaults", () => {
		...
		expect(
			resolveDefaultFeishuAccountSelection(explicitDefaultCfg as never),
		).toEqual({
			accountId: "router-d",
		...
		expect(
			resolveDefaultFeishuAccountSelection(mappedDefaultCfg as never),
		).toEqual({
			accountId: "default",
...
describe("resolveFeishuAccount", () => {
	...
	it("throws typed SecretRef errors in runtime account resolution", () => {
		...
		expect((caught as Error).message).toMatch(
			/channels\.feishu\.appSecret: unresolved SecretRef/i,
		);
	});
