...
import { type FeishuDriveParams, FeishuDriveSchema } from "./drive-schema.js";
import {
...
function getDriveInternalClient(
	client: Lark.Client,
): FeishuDriveInternalClient {
						...
						switch (p.action) {
							...
							case "create_folder":
								return jsonToolResult(
									await createFolder(client, p.name, p.folder_token),
								);
							case "move":
								return jsonToolResult(
							...
							case "delete":
								return jsonToolResult(
									await deleteFile(client, p.file_token, p.type),
								);
							default:
								return unknownToolActionResult(
									(p as { action?: unknown }).action,
								);
						}
					} catch (err) {
