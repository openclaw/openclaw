...
const _setFeishuAllowFrom = createTopLevelChannelAllowFromSetter({
	channel,
});
...
function isFeishuConfigured(cfg: OpenClawConfig): boolean {
	...
	const accountConfigured = Object.values(feishuCfg?.accounts ?? {}).some(
		(account) => {
			...
			const accountSecretConfigured = hasOwnAppSecret
				? hasConfiguredSecretInput(
						(account as Record<string, unknown>).appSecret,
					)
				: hasConfiguredSecretInput(feishuCfg?.appSecret);
...
const promptFeishuAllowFrom = createTopLevelChannelParsedAllowFromPrompt({
	...
	mergeEntries: ({ existing, parsed }) =>
		mergeAllowFromEntries(existing, parsed),
});
...
const feishuDmPolicy: ChannelSetupDmPolicy = createTopLevelChannelDmPolicy({
	...
	getCurrent: (cfg) =>
		(cfg.channels?.feishu as FeishuConfig | undefined)?.dmPolicy ?? "pairing",
...
export const feishuSetupWizard: ChannelSetupWizard = {
	...
	finalize: async ({ cfg, prompter, options }) => {
		...
		const hasConfigCreds = Boolean(
			typeof feishuCfg?.appId === "string" &&
				feishuCfg.appId.trim() &&
				hasConfigSecret,
		);
		...
		const currentMode =
			(next.channels?.feishu as FeishuConfig | undefined)?.connectionMode ??
			"websocket";
		...
		if (connectionMode === "webhook") {
			const currentVerificationToken = (
				next.channels?.feishu as FeishuConfig | undefined
			)?.verificationToken;
		...
		const currentDomain =
			(next.channels?.feishu as FeishuConfig | undefined)?.domain ?? "feishu";
		...
		const groupPolicy = (await prompter.select({
			...
			initialValue:
				(next.channels?.feishu as FeishuConfig | undefined)?.groupPolicy ??
				"allowlist",
