...
describe("broadcast dispatch", () => {
	...
	const mockWithReplyDispatcher: PluginRuntime["channel"]["reply"]["withReplyDispatcher"] =
		async ({ dispatcher, run, onSettled }) => {
			try {
				return await run();
			} finally {
	...
	it("dispatches to all broadcast agents when bot is mentioned", async () => {
		...
		const sessionKeys = finalizeInboundContextCalls.map(
			(call) => call.SessionKey,
		);
