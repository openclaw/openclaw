...
const DEFAULT_CREDS = { appId: "cli_123", appSecret: "secret" };
...
function makeRequestFn(response) {
...
async function expectDefaultSuccessResult(creds = DEFAULT_CREDS, expected = DEFAULT_SUCCESS_RESULT) {
...
async function expectFreshDefaultProbeAfter(requestFn, invalidate) {
  await probeFeishu(DEFAULT_CREDS);
...
describe("probeFeishu", () => {
  ...
  it("returns error when appId is missing", async () => {
    const result = await probeFeishu({ appSecret: "secret" });
    expect(result).toEqual({ ok: false, error: "missing credentials (appId, appSecret)" });
  });
  it("returns error when appSecret is missing", async () => {
    const result = await probeFeishu({ appId: "cli_123" });
    expect(result).toEqual({ ok: false, error: "missing credentials (appId, appSecret)" });
  });
  ...
  it("returns timeout error when request exceeds timeout", async () => {
    await withFakeTimers(async () => {
      ...
      createFeishuClientMock.mockReturnValue({ request: requestFn });
      const promise = probeFeishu(DEFAULT_CREDS, { timeoutMs: 1e3 });
  ...
  it("makes a fresh API call after cache expires", async () => {
    await withFakeTimers(async () => {
      ...
      await expectFreshDefaultProbeAfter(requestFn, () => {
        vi.advanceTimersByTime(10 * 60 * 1e3 + 1);
      });
    });
  });
