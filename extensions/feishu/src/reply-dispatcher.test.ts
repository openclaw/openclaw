...
vi.mock("./streaming-card.js", async () => {
	const actual = await vi.importActual<typeof import("./streaming-card.js")>(
		"./streaming-card.js",
	);
	return {
...
describe("createFeishuReplyDispatcher streaming behavior", () => {
	...
	function createDispatcherHarness(
		overrides: Partial<ReplyDispatcherArgs> = {},
	) {
	...
	it("streams reasoning content as blockquote before answer", async () => {
		...
		expect(streamingInstances).toHaveLength(1);
		const updateCalls = streamingInstances[0].update.mock.calls.map(
		...
		const combinedUpdate = updateCalls.find(
			(c) => c.includes("Thinking") && c.includes("---"),
		);
		...
		try {
			...
			await vi.waitFor(() => {
				expect(errorMock).toHaveBeenCalledWith(
					expect.stringContaining("streaming start failed"),
				);
			});
