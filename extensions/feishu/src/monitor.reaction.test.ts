...
const handleFeishuMessageMock = vi.hoisted(() =>
	vi.fn(async (_params: { event?: unknown }) => {}),
);
const createEventDispatcherMock = vi.hoisted(() => vi.fn());
const monitorWebSocketMock = vi.hoisted(() => vi.fn(async () => {}));
const monitorWebhookMock = vi.hoisted(() => vi.fn(async () => {}));
const createFeishuThreadBindingManagerMock = vi.hoisted(() =>
	vi.fn(() => ({ stop: vi.fn() })),
);
...
function createFetchedReactionMessage(
	chatId: string,
...
type FeishuMention = NonNullable<
...
describe("monitorSingleAccount lifecycle", () => {
	...
	it("stops the Feishu thread binding manager when the monitor exits", async () => {
		...
		const manager = createFeishuThreadBindingManagerMock.mock.results[0]
...
describe("Feishu inbound debounce regressions", () => {
	...
	it("keeps bot mention when per-message mention keys collide across non-forward messages", async () => {
		...
		expect(
			mergedMentions.some((mention) => mention.id.open_id === "ou_bot"),
		).toBe(true);
		expect(
			mergedMentions.some((mention) => mention.id.open_id === "ou_user_a"),
		).toBe(false);
	});
	...
	it("does not synthesize mention-forward intent across separate messages", async () => {
		...
		const mergedMentions = dispatched.message.mentions ?? [];
		expect(
			mergedMentions.every((mention) => mention.id.open_id === "ou_bot"),
		).toBe(true);
	});
	...
	it("excludes previously processed retries from combined debounce text", async () => {
		...
		const combined = JSON.parse(dispatched.message.content) as {
			text?: string;
		};
	...
	it("uses latest fresh message id when debounce batch ends with stale retry", async () => {
		vi.spyOn(dedup, "tryBeginFeishuMessageProcessing").mockReturnValue(true);
		const recordSpy = vi
			.spyOn(dedup, "recordProcessedFeishuMessage")
