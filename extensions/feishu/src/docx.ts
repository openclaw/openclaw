	...
	const cleaned = blocks
		.filter((block) => {
			if (UNSUPPORTED_CREATE_TYPES.has(block.block_type)) {
				const typeName =
					BLOCK_TYPE_NAMES[block.block_type] || `type_${block.block_type}`;
...
>;
type DriveMediaUploadFile = NonNullable<
	NonNullable<DriveMediaUploadAllPayload["data"]>["file"]
>;
...
function normalizeInsertedChildBlocks(
	...
	return children.filter(
		(child): child is FeishuDocxBlockChild =>
...
function normalizeConvertedBlockTree(
	...
	if (blocks.length <= 1) {
		const rootIds =
			blocks.length === 1 && typeof blocks[0]?.block_id === "string"
				? [blocks[0].block_id]
				: [];
	...
	const inferredTopLevelIds = blocks
		.filter((block) => {
			...
			const parentId =
				typeof block?.parent_id === "string" ? block.parent_id : "";
	...
	const rootIds = (
		firstLevelIds && firstLevelIds.length > 0
			? firstLevelIds
			: inferredTopLevelIds
	).filter(
		(id, index, arr) =>
			typeof id === "string" && byId.has(id) && arr.indexOf(id) === index,
	);
	...
	return {
		orderedBlocks,
		rootIds: rootIds.filter((id): id is string => typeof id === "string"),
	};
}
...
async function convertMarkdownWithFallback(
	...
	try {
		...
		for (const chunk of chunks) {
			const converted = await convertMarkdownWithFallback(
				client,
				chunk,
				depth + 1,
			);
...
async function chunkedConvertMarkdown(client: Lark.Client, markdown: string) {
	...
	for (const chunk of chunks) {
		...
		const { orderedBlocks, rootIds } = normalizeConvertedBlockTree(
			blocks,
			firstLevelBlockIds,
		);
...
async function insertBlocksWithDescendant(
	...
	firstLevelBlockIds: string[],
	{
		...
		index = -1,
	}: { parentBlockId?: string; index?: number } = {},
...
async function resolveUploadInput(
	...
	if (inputSources.length > 1) {
		throw new Error(
			`Provide only one image source; got: ${inputSources.join(", ")}`,
		);
	}
	...
	if (imageInput) {
		const candidate = imageInput.startsWith("~")
			? imageInput.replace(/^~/, homedir())
			: imageInput;
...
async function uploadImageBlock(
	...
	const imageBlockId = insertRes.data?.children?.find(
		(b) => b.block_type === 27,
	)?.block_id;
	if (!imageBlockId) {
...
async function createDoc(
	...
	if (!docToken) {
		throw new Error(
			"Document creation succeeded but no document_id was returned",
		);
	}
	...
	if (shouldGrantToRequester) {
		if (!requesterOpenId) {
			requesterPermissionSkippedReason =
				"trusted requester identity unavailable";
		} else {
			try {
	...
	return {
		...
		...(shouldGrantToRequester && {
			...
			...(requesterPermissionError && {
				requester_permission_error: requesterPermissionError,
			}),
		}),
	};
}
...
async function writeDoc(
	...
	const { blocks, firstLevelBlockIds } = await chunkedConvertMarkdown(
		client,
		markdown,
	);
	if (blocks.length === 0) {
	...
	logger?.info?.(
		`feishu_doc: Converted to ${blocks.length} blocks, inserting...`,
	);
	const { orderedBlocks, rootIds } = normalizeConvertedBlockTree(
		blocks,
		firstLevelBlockIds,
	...
	logger?.info?.(
		`feishu_doc: Done (${blocks.length} blocks, ${imagesProcessed} images)`,
	);
			...
			children: [
				{
					...
					table: {
						property: {
							...
							...(columnWidth && columnWidth.length > 0
...
export function registerFeishuDocTools(api: OpenClawPluginApi) {
	...
	const getMediaMaxBytes = (
		...
		(resolveFeishuToolAccount({ api, executeParams: params, defaultAccountId })
					...
					async execute(_toolCallId, params) {
						...
						try {
							...
							switch (p.action) {
								...
								case "delete_block":
									return json(
										await deleteBlock(client, p.doc_token, p.block_id),
									);
								case "create_table":
									return json(
										await createTable(
				...
				async execute() {
					try {
						const result = await listAppScopes(
							getClient(undefined, ctx.agentAccountId),
						);
						return json(result);
					} catch (err) {
						return json({
