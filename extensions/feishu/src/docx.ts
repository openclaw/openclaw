...
async function createDoc(
  ...
  // Auto-grant full_access to owner if specified
  let permGranted: string | undefined;
  if (ownerOpenId && docId) {
    try {
      ...
      if (permRes.code === 0) {
        permGranted = ownerOpenId;
      }
    } catch {
      // Non-critical: doc created but permission grant failed
    }
  }
  ...
  return {
    ...
    ...(permGranted && { full_access_granted_to: permGranted }),
  };
}
        ...
        async execute(_toolCallId, params) {
          ...
          try {
            ...
            switch (p.action) {
              ...
              case "create":
                return json(await createDoc(client, p.title, p.folder_token, p.owner_open_id));
              case "list_blocks":
