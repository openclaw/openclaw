...
function containsLegacyFeishuCardCommandValue(node: unknown): boolean {
	...
	return Object.values(node).some((value) =>
		containsLegacyFeishuCardCommandValue(value),
	);
}
...
const collectFeishuSecurityWarnings =
	createAllowlistProviderGroupPolicyWarningCollector<{
		cfg: ClawdbotConfig;
...
function isFeishuReactionsActionEnabled(params: {
	...
	const gate = createActionGate(
		(params.account.config.actions ??
			(params.cfg.channels?.feishu as { actions?: unknown } | undefined)
...
function isSupportedFeishuDirectConversationId(
...
function matchFeishuAcpConversation(params: {
	...
	const binding = normalizeFeishuAcpConversationId(
	...
	const matchesCanonicalConversation =
		binding.conversationId === incoming.canonicalConversationId;
	...
	if (
		!matchesCanonicalConversation &&
		!matchesParentTopicForSenderScopedConversation
	) {
		return null;
	}
	return {
...
function resolveFeishuCommandConversation(params: {
	...
	if (params.threadId) {
		...
		const senderScopedConversationId =
...
function resolveFeishuMemberId(
	params: Record<string, unknown>,
): string | undefined {
	return readFirstString(params, [
		"memberId",
	...
	base: {
		...
		actions: {
			...
			handleAction: async (ctx) => {
				...
				if (
					...
					throw new Error(
						"Feishu reactions are disabled via actions.reactions.",
					);
				}
				if (ctx.action === "send" || ctx.action === "thread-reply") {
					...
					if (card && mediaUrl) {
						throw new Error(
							`Feishu ${ctx.action} does not support card with media.`,
						);
					}
					if (!card && !text && !mediaUrl) {
						throw new Error(
							`Feishu ${ctx.action} requires text/message, media, or card.`,
						);
					}
		...
		messaging: {
			...
			resolveOutboundSessionRoute: (params) =>
				resolveFeishuOutboundSessionRoute(params),
			targetResolver: {
