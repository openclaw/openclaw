...
type DocxDescendantCreate =
	Lark.Client["docx"]["documentBlockDescendant"]["create"];
...
describe("insertBlocksInBatches", () => {
	it("builds the source block map once for large flat trees", async () => {
		...
		const createMock = vi.fn(
			async (
				params?: DocxDescendantCreateParams,
			): Promise<DocxDescendantCreateResponse> => ({
				code: 0,
				data: {
		...
		expect(createMock.mock.calls[0]?.[0]?.data.children_id).toHaveLength(
			BATCH_SIZE,
		);
	...
	it("keeps nested descendants grouped with their root blocks", async () => {
		...
		expect(createMock.mock.calls[0]?.[0]?.data.children_id).toEqual([
			"root_a",
			"root_b",
		]);
		expect(
