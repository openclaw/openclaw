...
type SetFeishuClientRuntimeForTest =
	typeof import("./client.js").setFeishuClientRuntimeForTest;
...
function readCallOptions(
	...
	const call =
		index < 0 ? mock.mock.calls.at(index)?.[0] : mock.mock.calls[index]?.[0];
...
describe("createFeishuClient HTTP timeout", () => {
	...
	it("clamps env timeout override to max bound", async () => {
		process.env[FEISHU_HTTP_TIMEOUT_ENV_VAR] = String(
			FEISHU_HTTP_TIMEOUT_MAX_MS + 123_456,
		);
