...
import { buildInboundBody, processInboundMessage } from "./inbound.js";
    ...
    if (core) {
      ...
      startAgentForStream({
        ...
        target.runtime.error?.(
          `[${target.account.accountId}] wecom agent failed (处理失败): ${String(err)}`,
        );
        streamStore.onStreamFinished(streamId);
      });
    }
  ...
  async function startAgentForStream(params: {
    ...
    if (directLocalPaths.length && looksLikeSendLocalFileIntent(rawBody)) {
        ...
        if (loaded.length > 0) {
          ...
          if (responseUrl) {
            try {
              ...
              logVerbose(
                target,
                `local-path: 已通过 Bot response_url 推送图片 frames=final images=${loaded.length}`,
              );
            } catch (err) {
              target.runtime.error?.(
                `local-path: Bot 主动推送图片失败（将依赖 stream_refresh 拉取）: ${String(err)}`,
              );
            }
          } else {
      ...
      if (otherPaths.length > 0) {
        ...
        const filename =
          otherPaths.length === 1
            ? otherPaths[0]!.split("/").pop()!
            : `${otherPaths.length} 个文件`;
    ...
    logVerbose(
      target,
      `starting agent processing (streamId=${streamId}, agentId=${route.agentId}, peerKind=${chatType}, peerId=${chatId})`,
    );
    logVerbose(
      target,
      `启动 Agent 处理: streamId=${streamId} 路由=${route.agentId} 类型=${chatType} ID=${chatId}`,
    );
    ...
    if (authz.shouldComputeAuth && authz.commandAuthorized !== true) {
      ...
      try {
        ...
        target.runtime.error?.(
          `authz: 未授权命令提示推送失败 streamId=${streamId}: ${String(err)}`,
        );
      }
      streamStore.onStreamFinished(streamId);
      return;
    ...
    const cfgForDispatch = (() => {
      ...
      const existingDeny = Array.isArray((baseSandboxTools as any).deny)
    ...
    logVerbose(
      target,
      `tool-policy: WeCom Bot 会话已禁用 message 工具（tools.sandbox.tools.deny += message，防止绕过 Bot 交付）`,
    );
        ...
        deliver: async (payload) => {
            ...
            try {
              ...
              if (parsed.template_card) {
                ...
                if (responseUrl && isSingleChat) {
                  ...
                  logVerbose(
                    target,
                    `template_card fallback to text (group=${!isSingleChat}, hasUrl=${!!responseUrl})`,
                  );
          ...
          if (nearTimeout) {
            ...
            try {
              ...
              target.runtime.error?.(
                `wecom bot fallback prompt push failed (timeout) streamId=${streamId}: ${String(err)}`,
              );
            }
            return;
          }
          ...
          for (const mediaPath of mediaUrls) {
            try {
              ...
              if (contentType?.startsWith("image/")) {
                ...
                if (agentCfg && !alreadySent && current.userId) {
                  try {
                    ...
                    logVerbose(
                      target,
                      `fallback(media): 文件已通过 Agent 私信发送 user=${current.userId}`,
                    );
                    streamStore.updateStream(streamId, (s) => {
                      s.agentMediaKeys = Array.from(
                        new Set([...(s.agentMediaKeys ?? []), mediaPath]),
                      );
                    });
                  } catch (err) {
                ...
                if (!current.fallbackMode) {
                  ...
                  try {
                    ...
                    target.runtime.error?.(
                      `wecom bot fallback prompt push failed (media) streamId=${streamId}: ${String(err)}`,
                    );
                  }
                }
                return;
              ...
              target.runtime.error?.(
                `Failed to process outbound media: ${mediaPath}: ${String(err)}`,
              );
            }
          }
      ...
      if (!agentCfg) {
        ...
        if (dmText) {
          try {
            ...
            logVerbose(
              target,
              `fallback(timeout): Agent 私信发送完成 user=${finishedState.userId}`,
            );
          } catch (err) {
    ...
    if (chatType === "group") {
      ...
      if (state && hasImages && responseUrl) {
        ...
        try {
          ...
          logVerbose(
            target,
            `final stream pushed via response_url (group) streamId=${streamId}, images=${state.images?.length ?? 0}`,
          );
        } catch (err) {
          target.runtime.error?.(
            `final stream push via response_url failed (group) streamId=${streamId}: ${String(err)}`,
          );
        }
      }
    }
