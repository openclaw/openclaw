...
describe("createSynologyChatPlugin", () => {
  ...
  describe("security.collectWarnings", () => {
    ...
    it("warns when allowInsecureSsl is true", () => {
      ...
      const warnings = plugin.security.collectWarnings({ account });
      expect(warnings.some((w) => w.includes("SSL"))).toBe(true);
    });
    it("warns when dmPolicy is open", () => {
      const plugin = createSynologyChatPlugin();
      const account = makeSecurityAccount({ dmPolicy: "open" });
      const warnings = plugin.security.collectWarnings({ account });
      expect(warnings.some((w) => w.includes("open"))).toBe(true);
    });
    it("warns when dmPolicy is allowlist and allowedUserIds is empty", () => {
      const plugin = createSynologyChatPlugin();
      const account = makeSecurityAccount();
      const warnings = plugin.security.collectWarnings({ account });
      expect(warnings.some((w) => w.includes("empty allowedUserIds"))).toBe(true);
    });
  ...
  describe("agentPrompt", () => {
    it("returns formatting hints", () => {
      ...
      expect(hints.some((h) => h.includes("<URL|display text>"))).toBe(true);
    });
  });
  describe("outbound", () => {
  ...
  describe("gateway", () => {
    function makeStartAccountCtx(accountConfig, abortController = new AbortController()) {
      return {
        abortController,
        ctx: {
          cfg: {
    ...
    async function expectPendingStartAccountPromise(result, abortController) {
    ...
    async function expectPendingStartAccount(accountConfig) {
      const plugin = createSynologyChatPlugin();
