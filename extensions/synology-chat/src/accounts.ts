...
function getChannelConfig(
	cfg: OpenClawConfig,
): SynologyChatChannelConfig | undefined {
...
function resolveImplicitAccountId(
	...
	return channelCfg.token || process.env.SYNOLOGY_CHAT_TOKEN
		? DEFAULT_ACCOUNT_ID
		: undefined;
}
...
function hasExplicitWebhookPath(
	rawAccount: SynologyChatChannelConfig | undefined,
): boolean {
	return (
		typeof rawAccount?.webhookPath === "string" &&
		rawAccount.webhookPath.trim().length > 0
	);
}
...
function resolveWebhookPathSource(params: {
	...
	if (
		...
		hasExplicitWebhookPath(params.channelCfg)
	) {
...
export function resolveAccount(
	...
	const accountOverrides =
		id === DEFAULT_ACCOUNT_ID
			? undefined
			: (channelCfg.accounts?.[id] ?? undefined);
	const rawAccount = getRawAccountConfig(channelCfg, id);
	const merged = resolveMergedAccountConfig<
		Record<string, unknown> & SynologyChatChannelConfig
	>({
		channelConfig: channelCfg as Record<string, unknown> &
			SynologyChatChannelConfig,
		accounts: channelCfg.accounts as
			| Record<
					string,
					Partial<Record<string, unknown> & SynologyChatChannelConfig>
			  >
			| undefined,
		accountId: id,
	});
	...
	const envRateLimitValue = parseRateLimitPerMinute(
		process.env.SYNOLOGY_RATE_LIMIT,
	);
	...
	return {
		...
		allowedUserIds: parseAllowedUserIds(
			merged.allowedUserIds ?? envAllowedUserIds,
		),
