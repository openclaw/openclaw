...
const sendMessage = vi
	.spyOn(clientModule, "sendMessage")
...
async function runDangerousNameMatchReply(
	...
	vi.mocked(resolveLegacyWebhookNameToChatUserId).mockResolvedValueOnce(
...
describe("createWebhookHandler", () => {
	...
	it("rejects excess concurrent pre-auth body reads from the same remote IP", async () => {
		const handler = createWebhookHandler({
			account: makeAccount({
				accountId: `preauth-inflight-test-${Date.now()}`,
			}),
			deliver: vi.fn(),
			log,
		});
	...
	it("rate limits repeated invalid token guesses before the correct token can succeed", async () => {
		...
		(lockedReq.socket as { remoteAddress?: string }).remoteAddress =
			"203.0.113.10";
	...
	it("keeps pre-auth throttling scoped to the remote IP", async () => {
		...
		(invalidReq.socket as { remoteAddress?: string }).remoteAddress =
			"203.0.113.10";
		...
		(validReq.socket as { remoteAddress?: string }).remoteAddress =
			"203.0.113.11";
