...
import { sendFileUrl, sendMessage } from "./client.js";
...
const resolveSynologyChatDmPolicy =
	createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
...
const synologyChatConfigAdapter =
	createHybridChannelConfigAdapter<ResolvedSynologyChatAccount>({
...
> & {
	...
	directory: {
		self?: NonNullable<
			ChannelPlugin<ResolvedSynologyChatAccount>["directory"]
		>["self"];
		listPeers?: NonNullable<
			ChannelPlugin<ResolvedSynologyChatAccount>["directory"]
		>["listPeers"];
		listGroups?: NonNullable<
			ChannelPlugin<ResolvedSynologyChatAccount>["directory"]
		>["listGroups"];
	};
	outbound: {
		...
		sendMedia: (
			ctx: SynologyChannelOutboundContext,
		) => Promise<SynologyChatOutboundResult>;
	};
	gateway: {
	...
	return createChatChannelPlugin({
		base: {
			...
			messaging: {
				...
				targetResolver: {
					looksLikeId: (id: string) => {
						...
						return (
							/^\d+$/.test(trimmed) || /^synology[-_]?chat:/i.test(trimmed)
						);
					},
					hint: "<userId>",
				},
			...
			gateway: {
				startAccount: async (ctx: SynologyChannelGatewayContext) => {
					...
					log?.info?.(
						`Registered HTTP route: ${account.webhookPath} for Synology Chat`,
					);
					...
					return waitUntilAbort(abortSignal, () => {
						log?.info?.(
							`Stopping Synology Chat channel (account: ${accountId})`,
						);
						unregister();
					});
				},
		...
		outbound: {
			...
			sendText: async ({
				...
				accountId,
				cfg,
			}: SynologyChannelSendTextContext) => {
			...
			sendMedia: async ({
				...
				accountId,
				cfg,
			}: SynologyChannelOutboundContext) => {
				const account = resolveOutboundAccount(cfg ?? {}, accountId);
				const incomingUrl = requireIncomingUrl(account);
				if (!mediaUrl) {
