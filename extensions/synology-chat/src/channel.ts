    ...
    security: {
      resolveDmPolicy: ({
        ...
        return {
          ...
          allowFrom: account.allowFrom ?? account.allowedUserIds ?? [],
          policyPath: `${basePath}dmPolicy`,
          allowFromPath: `${basePath}allowedUserIds`,
      ...
      startAccount: async (ctx: any) => {
        ...
        const handler = createWebhookHandler({
          ...
          deliver: async (msg) => {
            ...
            // IMPORTANT: Provider and Surface are required for proper routing
            const msgCtx = {
              Body: msg.body,
              RawBody: msg.body,
            ...
            await rt.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
              ...
              dispatcherOptions: {
                deliver: async (payload, info) => {
                  if (payload.text) {
                    log?.info?.(`Sending ${info.kind} reply to ${msg.from}`);
                    try {
                      await sendMessage(
                        account.incomingUrl,
                        payload.text,
                      ...
                      log?.error?.(`Failed to send ${info.kind} reply: ${String(err)}`);
                    }
                  }
                },
                onError: (err, info) => {
                  log?.error?.(`Synology Chat ${info.kind} reply failed: ${String(err)}`);
                },
              },
            });
