...
class InvalidTokenRateLimiter {
	...
	private normalizeState(
		key: string,
		nowMs: number,
	): InvalidTokenRateLimitState | undefined {
...
function getInvalidTokenRateLimiter(
...
function parsePayload(
	...
	const token =
		pickAlias(bodyFields, ["token"]) ??
		pickAlias(queryFields, ["token"]) ??
		headerToken;
	const userId =
	...
	return {
		token,
		channel_id:
			pickAlias(bodyFields, ["channel_id"]) ??
			pickAlias(queryFields, ["channel_id"]) ??
			undefined,
		channel_name:
		...
		username:
			...
			"unknown",
		post_id:
			pickAlias(bodyFields, ["post_id"]) ??
			pickAlias(queryFields, ["post_id"]) ??
			undefined,
		timestamp:
			pickAlias(bodyFields, ["timestamp"]) ??
			pickAlias(queryFields, ["timestamp"]) ??
			undefined,
		text,
		trigger_word:
...
function authorizeSynologyWebhook(params: {
	...
	if (params.invalidTokenRateLimiter.isLocked(invalidTokenRateLimitKey)) {
		params.log?.warn(
			`Rate limit exceeded for remote IP: ${invalidTokenRateLimitKey}`,
		);
	...
	if (!validateToken(params.payload.token, params.account.token)) {
		if (
			params.invalidTokenRateLimiter.recordFailure(invalidTokenRateLimitKey)
		) {
			params.log?.warn(
				`Rate limit exceeded for remote IP: ${invalidTokenRateLimitKey}`,
			);
...
async function resolveSynologyReplyDeliveryUserId(params: {
	...
	const resolvedChatApiUserId =
		await synologyClient.resolveLegacyWebhookNameToChatUserId({
...
export function createWebhookHandler(deps: WebhookHandlerDeps) {
	...
	return async (req: IncomingMessage, res: ServerResponse) => {
		...
		let authorized: Awaited<
			ReturnType<typeof parseAndAuthorizeSynologyWebhook>
		>;
		try {
