...
vi.mock("node:https", () => {
	...
	const mockGet = vi.fn();
	return {
		default: { request: mockRequest, get: mockGet },
		request: mockRequest,
		get: mockGet,
	};
});
...
function mockUserListResponseImpl(
	...
	const impl = (_url: any, _opts: any, callback: any) => {
		...
		process.nextTick(() => {
			...
			res.emit(
				"data",
				Buffer.from(JSON.stringify({ success: true, data: { users } })),
			);
			res.emit("end");
		});
		const req = new EventEmitter() as any;
...
describe("resolveLegacyWebhookNameToChatUserId", () => {
	...
	it("keeps user cache scoped per incoming URL", async () => {
		mockUserListResponseOnce([
			{ user_id: 4, username: "jmn67", nickname: "jmn" },
		]);
		mockUserListResponseOnce([
			{ user_id: 9, username: "jmn67", nickname: "jmn" },
		]);
