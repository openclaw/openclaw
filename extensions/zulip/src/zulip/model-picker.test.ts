...
describe("resolveZulipModelPickerCallbackAction", () => {
  ...
  it("builds a provider page for mdl_prov", async () => {
    ...
    expect(action.render.spec.buttons).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: "anthropic (1)", callbackData: "mdl_list_anthropic_1" }),
        expect.objectContaining({ label: "openai (2)", callbackData: "mdl_list_openai_1" }),
      ]),
    );
  });

  it("builds a model page with the current override marked", async () => {
    const action = await resolveZulipModelPickerCallbackAction({
      cfg,
    ...
    expect(action.render.spec.buttons).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: expect.stringContaining("gpt-4.1-mini ✓") }),
        expect.objectContaining({ label: "<< Back", callbackData: "mdl_back" }),
      ]),
    );
  });
