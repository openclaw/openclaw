...
export class OpenAIQuicksilverAudioPeer implements OpenAIQuicksilverAudioPeerContract {
  ...
  private inboundResampler = createStreamingPcmResampler(
  ...
  clearOutputAudio(): void {
    ...
    try {
      // Retire every pre-clear packet in order, including gaps, without resetting
      // the Opus decoder or allowing late duplicates to become a new stream.
      ...
      // into the next output generation; only the codec/sequence state survives.
      this.inboundResampler = createStreamingPcmResampler(
        QUICKSILVER_SAMPLE_RATE,
        RELAY_SAMPLE_RATE,
      );
    }
  ...
  private scheduleInboundFlush(state: InboundRtpState): void {
    if (
      this.closed ||
      this.discardingInboundAudio ||
      state.flushTimer ||
      state.pendingPackets.size === 0
    ) {
      return;
    }
  ...
  private emitInboundPcm(decoded: Int16Array): void {
    if (this.closed || this.discardingInboundAudio) {
      return;
    }
