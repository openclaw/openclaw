...
vi.mock("openclaw/plugin-sdk/runtime-env", () => ({
	ensureGlobalUndiciEnvProxyDispatcher:
		runtimeMocks.ensureGlobalUndiciEnvProxyDispatcher,
}));
...
const LIVE_MODEL_ID =
	process.env.OPENCLAW_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL =
	process.env.OPENCLAW_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const LIVE_VISION_MODEL =
	process.env.OPENCLAW_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled =
	OPENAI_API_KEY.trim().length > 0 && process.env.OPENCLAW_LIVE_TEST === "1";
...
describe("openai plugin", () => {
	...
	it("registers the expected provider surfaces", () => {
		...
		expect(mediaProviders.map((provider) => provider.id)).toEqual([
			"openai",
			"openai-codex",
		]);
	...
	it("bootstraps the env proxy dispatcher before refreshing codex oauth credentials", async () => {
		...
		await expect(refreshOpenAICodexToken("refresh-token")).resolves.toBe(
			refreshed,
		);
		...
		expect(
			runtimeMocks.ensureGlobalUndiciEnvProxyDispatcher.mock
	...
	it("generates an image through the registered image provider", async () => {
		...
		try {
			const generated = await imageProvider.generateImage({
				...
				prompt:
					"Create a minimal flat orange square centered on a white background.",
				cfg,
				agentDir,
