...
const OPENAI_CODEX_GPT_54_TEMPLATE_MODEL_IDS = [
	"gpt-5.3-codex",
	"gpt-5.2-codex",
] as const;
...
function normalizeCodexTransport(
	...
	const useCodexTransport =
		!model.baseUrl ||
		isOpenAIApiBaseUrl(model.baseUrl) ||
		isOpenAICodexBaseUrl(model.baseUrl);
	const api =
		useCodexTransport && model.api === "openai-responses"
...
export function buildOpenAICodexProviderPlugin(): ProviderPlugin {
	return {
		...
		isModernModelRef: ({ modelId }) =>
			matchesExactOrPrefix(modelId, OPENAI_CODEX_MODERN_MODEL_IDS),
