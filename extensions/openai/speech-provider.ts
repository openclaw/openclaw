...
function readOpenAIProviderConfig(
	...
	return {
		...
		instructions:
			trimToUndefined(config.instructions) ?? normalized.instructions,
	};
}
...
export function buildOpenAISpeechProvider(): SpeechProviderPlugin {
	return {
		...
		listVoices: async () =>
			OPENAI_TTS_VOICES.map((voice) => ({ id: voice, name: voice })),
		isConfigured: ({ providerConfig }) =>
			Boolean(
				readOpenAIProviderConfig(providerConfig).apiKey ||
