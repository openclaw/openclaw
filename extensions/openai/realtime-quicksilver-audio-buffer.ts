...
/** Bounded owned tail; raw transport bytes retain incomplete PCM or odd mu-law chunks. */
export class OpenAIQuicksilverPendingAudio {
  ...
  constructor(
    private readonly limit = OPENAI_QUICKSILVER_MAX_PENDING_AUDIO_BYTES,
  ...
  append(incoming: Buffer): void {
    ...
    const retainedBytes = Math.min(alignedLength, this.limit);
    const sourceOffset = alignedLength - retainedBytes;
    const storage = (this.storage ??= Buffer.alloc(this.limit));
    const droppedBytes = Math.max(0, this.pendingBytes + retainedBytes - this.limit);
    this.readOffset = (this.readOffset + droppedBytes) % this.limit;
    this.pendingBytes -= droppedBytes;

    const writeOffset = (this.readOffset + this.pendingBytes) % this.limit;
    const firstBytes = Math.min(retainedBytes, this.limit - writeOffset);
  ...
  readInto(target: Buffer): number {
    ...
    const firstBytes = Math.min(readBytes, this.limit - this.readOffset);
    ...
    this.readOffset = (this.readOffset + readBytes) % this.limit;
  ...
  take(limit = this.pendingBytes): Buffer<ArrayBuffer> {
    const bytes = Math.min(limit, this.pendingBytes);
    const output = Buffer.alloc(bytes - (bytes % this.sampleBytes));
