...
const slackPlugin = {
  ...
  resolver: {
    resolveTargets: async ({ cfg, accountId, inputs, kind }) => {
      ...
      if (kind === "group") {
        ...
        return resolved2.map(
          (entry) => toResolvedTarget(entry, entry.archived ? "archived" : void 0)
        );
      }
  ...
  status: {
    ...
    buildAccountSnapshot: ({ account, runtime, probe }) => {
      const mode = account.config.mode ?? "socket";
      const configured = (mode === "http" ? resolveConfiguredFromRequiredCredentialStatuses(account, [
        "botTokenStatus",
        "signingSecretStatus"
