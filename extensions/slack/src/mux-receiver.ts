...
export class MuxReceiver {
  ...
  async start(): Promise<void> {
    ...
    // Retry initial connection with backoff so that boot-time mux
    // unavailability doesn't permanently crash the channel task.
    let attempt = 0;
    while (!this.stopped) {
      try {
        await this.connect();
  ...
  private handleSlackEvent(msg: MuxSlackEvent): void {
    ...
    // Default missing headers to {} to guard against older/buggy mux emitters.
