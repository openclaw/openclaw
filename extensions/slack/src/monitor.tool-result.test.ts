...
const slackSleepMock = vi.hoisted(() => vi.fn(async (_ms?: number) => {}));

vi.mock("./utils/sleep.js", () => ({
  sleep: (ms?: number) => slackSleepMock(ms),
}));
...
beforeEach(() => {
  ...
  slackSleepMock.mockReset();
  slackSleepMock.mockImplementation(async () => {});
});
