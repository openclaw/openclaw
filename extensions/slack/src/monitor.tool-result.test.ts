...
beforeEach(async () => {
	...
	({ HISTORY_CONTEXT_MARKER } = await import(
		"../../../src/auto-reply/reply/history.js"
	));
	({ CURRENT_MESSAGE_MARKER } = await import(
		"../../../src/auto-reply/reply/mentions.js"
	));
...
describe("monitorSlackProvider tool results", () => {
	...
	function expectSingleSendWithThread(threadTs: string | undefined) {
		...
		expect(
			(sendMock.mock.calls[0]?.[2] as { threadTs?: string } | undefined)
				?.threadTs,
		).toBe(threadTs);
	}
	...
	it("restores ack reaction when dispatch fails before any reply is delivered", async () => {
		...
		expect(sendMock).not.toHaveBeenCalled();
		expect(
			reactMock.mock.calls.map(([args]) =>
				String((args as { name: string }).name),
			),
		).toEqual(["eyes", "scream", "eyes", "eyes", "scream"]);
	});
