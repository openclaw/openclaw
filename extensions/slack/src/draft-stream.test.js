...
function createDraftStreamHarness(params = {}) {
  const send = params.send ?? vi.fn(async () => ({
    channelId: "C123",
    messageId: "111.222"
  }));
...
describe("createSlackDraftStream", () => {
  ...
  it("supports forceNewMessage for subsequent assistant messages", async () => {
    const send = vi.fn().mockResolvedValueOnce({ channelId: "C123", messageId: "111.222" }).mockResolvedValueOnce({ channelId: "C123", messageId: "333.444" });
