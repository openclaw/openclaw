...
const prepareSlackMessageMock =
	...
	>();
const dispatchPreparedSlackMessageMock =
	vi.fn<(prepared: unknown) => Promise<void>>();
...
vi.mock("./thread-resolution.js", () => ({
	createSlackThreadTsResolver: () => ({
		resolve: async ({ message }: { message: Record<string, unknown> }) =>
			message,
	}),
}));
...
vi.mock("./message-handler/prepare.js", () => ({
	...
	): ReturnType<typeof prepareSlackMessageMock> =>
		prepareSlackMessageMock(params),
}));
...
async function sendMessageEvent(
	...
	await handler(createSlackEvent({ type: "message", ts, text: "hello" }), {
		source: "message",
	});
}

async function sendMentionEvent(
	handler: ReturnType<typeof createTestHandler>,
	ts: string,
) {
	await handler(
		createSlackEvent({ type: "app_mention", ts, text: "<@U_BOT> hello" }),
		{
			source: "app_mention",
			wasMentioned: true,
		},
...
async function createInFlightMessageScenario(ts: string) {
	...
	const messagePending = handler(
		createSlackEvent({ type: "message", ts, text: "hello" }),
		{
			source: "message",
		},
	);
