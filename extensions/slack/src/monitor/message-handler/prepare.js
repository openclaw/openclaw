...
import { resolveSlackReplyToMode } from "../../accounts.js";
...
function resolveSlackRoutingContext(params) {
  ...
  const roomThreadId = isThreadReply && threadTs ? threadTs : void 0;
...
async function prepareSlackMessage(params) {
  ...
  const implicitMention = Boolean(
    !isDirectMessage && ctx.botUserId && message.thread_ts && (message.parent_user_id === ctx.botUserId || hasSlackThreadParticipation(account.accountId, message.channel, message.thread_ts))
  );
