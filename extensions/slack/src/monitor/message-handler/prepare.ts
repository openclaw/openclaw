...
const mentionRegexCache = new WeakMap<
	SlackMonitorContext,
	Map<string, RegExp[]>
>();
...
async function resolveSlackConversationContext(params: {
	...
	let resolvedChannelType = normalizeSlackChannelType(
...
function resolveSlackRoutingContext(params: {
	...
	const threadKeys = resolveThreadSessionKeys({
		...
		parentSessionKey:
			canonicalThreadId && ctx.threadInheritParent
				? route.sessionKey
				: undefined,
	});
...
export async function prepareSlackMessage(params: {
	...
	const senderNameForAuth = ctx.allowNameMatching
	...
	const ctxPayload = finalizeInboundContext({
		...
		ThreadStarterBody: !threadSessionPreviousTimestamp
			? threadStarterBody
			: undefined,
		ThreadHistoryBody: threadHistoryBody,
		IsFirstThreadTurn:
			isThreadReply && threadTs && !threadSessionPreviousTimestamp
				? true
				: undefined,
