...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  function createThreadReplyMessage(overrides) {
  ...
  function createDmScopeMainSlackCtx() {
    ...
    slackCtx.resolveUserName = async () => ({ name: "Alice" });
    slackCtx.resolveChannelName = async () => ({ name: void 0, type: "im" });
    return slackCtx;
  }
  function createMainScopedDmMessage(overrides) {
    return createSlackMessage({
      channel: "D0ACP6B1T8V",
      user: "U1",
      text: "hello from DM",
  ...
  function expectMainScopedDmClassification(prepared, options) {
  ...
  function createReplyToAllSlackCtx(params) {
    const slackCtx = createInboundSlackCtx({
      ...
      replyToMode: "all",
      ...params?.defaultRequireMention === void 0 ? {} : { defaultRequireMention: params.defaultRequireMention }
    });
    slackCtx.resolveUserName = async () => ({ name: "Alice" });
    if (params?.asChannel) {
...
describe("prepareSlackMessage sender prefix", () => {
  ...
  async function prepareSenderPrefixMessage(ctx, text, ts) {
    return prepareSlackMessage({
      ctx,
      account: { accountId: "default", config: {}, replyToMode: "off" },
      message: {
        type: "message",
        channel: "C1",
        channel_type: "channel",
