...
function registerSlackChannelEvents(params) {
  ...
  const enqueueChannelSystemEvent = (params2) => {
    if (!ctx.isChannelAllowed({
      channelId: params2.channelId,
      channelName: params2.channelName,
      channelType: "channel"
    })) {
      return;
    }
    ...
    enqueueSystemEvent(`Slack channel ${params2.kind}: ${label}.`, {
      sessionKey,
      contextKey: `slack:channel:${params2.kind}:${params2.channelId ?? params2.channelName ?? "unknown"}`
    });
  };
  ctx.app.event(
    "channel_created",
