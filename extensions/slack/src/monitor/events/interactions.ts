...
// Prefix for Mullusi-generated action IDs to scope our handler
const MULLUSI_ACTION_PREFIX = "mullusi:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${MULLUSI_ACTION_PREFIX}`);

  // Handle Mullusi modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
