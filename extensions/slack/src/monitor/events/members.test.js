...
function makeMemberEvent(overrides) {
  return {
...
function getMemberHandlers(params) {
  ...
  return {
    joined: harness.getHandler("member_joined_channel"),
    left: harness.getHandler("member_left_channel")
  };
}
async function runMemberCase(args = {}) {
  ...
  await handler({
    event: args.event ?? makeMemberEvent(),
    body: args.body ?? {}
  });
}
