...
const buildPluginBindingResolvedTextMock = vi.hoisted(() =>
	vi.fn(() => "Binding updated."),
);
...
describe("registerSlackInteractionEvents", () => {
	beforeAll(async () => {
		...
		dispatchPluginInteractiveHandlerSpy = vi
			...
			.mockImplementation(((
				...args: Parameters<
					typeof pluginRuntime.dispatchPluginInteractiveHandler
				>
			) =>
				(
					dispatchPluginInteractiveHandlerMock as (
						...innerArgs: unknown[]
					) => Promise<unknown>
				)(...args)) as typeof pluginRuntime.dispatchPluginInteractiveHandler);
		resolvePluginConversationBindingApprovalSpy = vi
			.spyOn(conversationBinding, "resolvePluginConversationBindingApproval")
			.mockImplementation(((
				...args: Parameters<
					typeof conversationBinding.resolvePluginConversationBindingApproval
				>
			) =>
				(
					...
					...args,
				)) as typeof conversationBinding.resolvePluginConversationBindingApproval);
		buildPluginBindingResolvedTextSpy = vi
			.spyOn(conversationBinding, "buildPluginBindingResolvedText")
			.mockImplementation(((
				...args: Parameters<
					typeof conversationBinding.buildPluginBindingResolvedText
				>
			) =>
				(
					buildPluginBindingResolvedTextMock as (
						...innerArgs: unknown[]
					) => string
				)(
					...args,
	...
	beforeEach(() => {
		...
		resolvePluginConversationBindingApprovalMock.mockResolvedValue({
			status: "expired",
		});
	...
	it("uses unique interaction ids for repeated Slack actions on the same message", async () => {
		...
		const calls = dispatchPluginInteractiveHandlerMock.mock
			.calls as unknown[][];
