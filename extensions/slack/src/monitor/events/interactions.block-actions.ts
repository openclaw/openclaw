...
type SlackBlockActionRespond = NonNullable<
	SlackActionMiddlewareArgs["respond"]
>;
...
async function handleSlackPluginBindingApproval(params: {
	...
	const pluginBindingApproval = parsePluginBindingApprovalCustomId(
		params.pluginInteractionData,
	);
	...
	await respondEphemeral(
		params.respond,
		buildPluginBindingResolvedText(resolved),
	);
	return true;
}
...
async function dispatchSlackPluginInteraction(params: {
	...
	const pluginResult = await dispatchPluginInteractiveHandler({
		...
		ctx: {
			...
			interaction: {
				kind:
					params.parsed.actionSummary.actionType === "button"
						? "button"
						: "select",
				actionId: params.parsed.actionId,
...
function buildSlackConfirmationBlocks(params: {
	...
	let updatedBlocks = params.originalBlocks.map((block) => {
		...
		if (
			typedBlock.type === "actions" &&
			typedBlock.block_id === params.parsed.blockId
		) {
			return {
				type: "context",
				elements: [
...
async function updateSlackLegacyBlockAction(params: {
	...
	try {
		...
		await respondEphemeral(
			params.respond,
			`Button "${params.parsed.actionId}" clicked!`,
		);
	}
}
