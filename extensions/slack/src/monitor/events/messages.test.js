...
vi.mock("../../../../../src/infra/system-events.js", () => ({
  enqueueSystemEvent: (...args) => messageQueueMock(...args)
}));
vi.mock("../../../../../src/pairing/pairing-store.js", () => ({
  readChannelAllowFromStore: (...args) => messageAllowMock(...args)
}));
function createHandlers(eventName, overrides) {
  ...
  return {
    handler: harness.getHandler(eventName),
    handleSlackMessage
  };
}
...
function makeChangedEvent(overrides) {
...
async function runMessageCase(input = {}) {
  resetMessageMocks();
  const { handler } = createHandlers("message", input.overrides);
  expect(handler).toBeTruthy();
  await handler({
    event: input.event ?? makeChangedEvent(),
    body: input.body ?? {}
  });
}
