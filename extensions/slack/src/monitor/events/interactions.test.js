...
function createContext(overrides) {
  ...
  const isChannelAllowed = vi.fn().mockImplementation((params) => overrides?.isChannelAllowed?.(params) ?? true);
  const resolveUserName = vi.fn().mockImplementation((userId) => overrides?.resolveUserName?.(userId) ?? Promise.resolve({}));
  const resolveChannelName = vi.fn().mockImplementation(
...
describe("registerSlackInteractionEvents", () => {
  ...
  it("drops modal lifecycle payloads when mismatch guard triggers", async () => {
    ...
    const ackSubmit = vi.fn().mockResolvedValue(void 0);
    await viewHandler({
      ack: ackSubmit,
      body: {
        user: { id: "U123" },
    ...
    const ackClosed = vi.fn().mockResolvedValue(void 0);
