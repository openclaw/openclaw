...
function readOptionLabels(options) {
  ...
  const labels = options.map(
    (option) => option && typeof option === "object" ? option.text?.text ?? null : null
...
function summarizeAction(action) {
  ...
  const inputKind = actionType === "number_input" ? "number" : actionType === "email_text_input" ? "email" : actionType === "url_text_input" ? "url" : actionType === "rich_text_input" ? "rich_text" : inputValue != null ? "text" : void 0;
  return {
    actionType,
    inputKind,
    value: typed.value,
...
function formatInteractionSelectionLabel(params) {
  ...
  if (typeof params.summary.selectedDateTime === "number") {
    return new Date(params.summary.selectedDateTime * 1e3).toISOString();
  }
