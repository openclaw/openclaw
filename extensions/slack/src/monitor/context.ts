...
export type SlackMonitorContext = {
	...
	slashCommand: Required<
		import("openclaw/plugin-sdk/config-runtime").SlackSlashCommandConfig
	>;
	textLimit: number;
...
export function createSlackMonitorContext(params: {
	...
	const hasChannelAllowlistConfig =
	...
	const isChannelAllowed = (p: {
		...
		if (isGroupDm && groupDmChannels.length > 0) {
			...
			const permitted =
				...
				candidates.some((candidate) =>
					groupDmChannelsLower.includes(candidate),
				);
			if (!permitted) {
				return false;
			}
		...
		if (isRoom && p.channelId) {
			...
			if (
				!channelAllowed &&
				(params.groupPolicy !== "open" || hasExplicitConfig)
			) {
	...
	const shouldDropMismatchedSlackEvent = (body: unknown) => {
		...
		if (
			params.apiAppId &&
			incomingApiAppId &&
			incomingApiAppId !== params.apiAppId
		) {
			logVerbose(
