...
function resolveSlackBoltModule(
	value: unknown,
): SlackBoltResolvedExports | null {
...
function publishSlackConnectedStatus(
...
export async function monitorSlackProvider(opts: MonitorSlackOpts = {}) {
	...
	if (resolveToken) {
		void (async () => {
			...
			const allowEntries = normalizeStringEntries(allowFrom).filter(
				(entry) => entry !== "*",
			);
	...
	try {
		if (slackMode === "socket") {
			...
			while (!opts.abortSignal?.aborted) {
				...
				if (
					disconnect.error &&
					isNonRecoverableSlackAuthError(disconnect.error)
				) {
					runtime.error?.(
...
export const resolveSlackRuntimeGroupPolicy =
	resolveOpenProviderRuntimeGroupPolicy;
