...
let slashCommandsRuntimePromise: Promise<
	typeof import("./slash-commands.runtime.js")
> | null = null;
let slashDispatchRuntimePromise: Promise<
	typeof import("./slash-dispatch.runtime.js")
> | null = null;
...
function loadSlashSkillCommandsRuntime() {
	slashSkillCommandsRuntimePromise ??= import(
			...
			if (isRoom) {
				...
				if (ctx.useAccessGroups) {
					const channelAllowlistConfigured =
						(ctx.channelsConfigKeys?.length ?? 0) > 0;
			...
			ctx.app.command(
				...
				async ({
					...
					const prompt = commandDefinition
						? slashCommandsRuntime.buildCommandTextFromArgs(
								commandDefinition,
								commandArgs,
							)
						: rawText
