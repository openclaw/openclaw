...
export function createSlackMessageHandler(params: {
	...
	const { debounceMs, debouncer } = createChannelInboundDebouncer<{
		...
		shouldDebounce: (entry) =>
			shouldDebounceSlackMessage(entry.message, ctx.cfg),
		onFlush: async (entries) => {
			...
			if (flushedKey && topLevelConversationKey) {
				const pendingKeys = pendingTopLevelDebounceKeys.get(
	...
	return async (message, opts) => {
		...
		const wasSeen = seenMessageKey
			? ctx.markMessageSeen(message.channel, message.ts)
			: false;
		...
		if (seenMessageKey && wasSeen) {
			...
			if (
				...
				!consumeAppMentionRetryKey(seenMessageKey)
			) {
				return;
			}
		}
		...
		const canDebounce =
			debounceMs > 0 && shouldDebounceSlackMessage(resolvedMessage, ctx.cfg);
