...
let mockFetch;
const createSavedMedia = (filePath, contentType) => ({
  id: "saved-media-id",
  path: filePath,
  size: 128,
  contentType
...
describe("resolveSlackMedia", () => {
  ...
  it("overrides video/* MIME to audio/* for slack_audio voice messages", async () => {
    const saveMediaBufferMock = vi.spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(createSavedMedia("/tmp/voice.mp4", "video/mp4"));
  ...
  it("preserves original MIME for non-voice Slack files", async () => {
    const saveMediaBufferMock = vi.spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(createSavedMedia("/tmp/video.mp4", "video/mp4"));
  ...
  it("caps downloads to 8 files for large multi-attachment messages", async () => {
    const saveMediaBufferMock = vi.spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(createSavedMedia("/tmp/x.jpg", "image/jpeg"));
