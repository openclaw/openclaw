...
async function sendSlackOutboundMessage(params: {
	...
	const send =
		resolveOutboundSendDep<typeof sendMessageSlack>(params.deps, "slack") ??
		sendMessageSlack;
	const threadTs =
		params.replyToId ??
...
function resolveSlackBlocks(payload: {
	...
	const renderedInteractive = resolveRenderedInteractiveBlocks(
		payload.interactive,
	);
	if (!slackData || typeof slackData !== "object" || Array.isArray(slackData)) {
		return renderedInteractive;
	}
	const existingBlocks = parseSlackBlocksInput(
