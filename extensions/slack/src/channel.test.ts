...
function requireSlackSendPayload() {
	const sendPayload =
		slackPlugin.outbound?.sendPayload ?? slackOutbound.sendPayload;
	if (!sendPayload) {
...
describe("slackPlugin actions", () => {
	...
	it("owns unified message tool discovery", () => {
		...
		expect(discovery?.capabilities).toEqual(
			expect.arrayContaining(["blocks", "interactive"]),
		);
	...
	it("treats interactive reply payloads as structured Slack payloads", () => {
		const hasStructuredReplyPayload =
			slackPlugin.messaging?.hasStructuredReplyPayload;
...
describe("slackPlugin outbound new targets", () => {
	...
	it("sends to a new user target via DM without erroring", async () => {
		const sendSlack = vi
			.fn()
			.mockResolvedValue({ messageId: "m-new-user", channelId: "D999" });
		const sendText = requireSlackSendText();
	...
	it("sends to a new channel target without erroring", async () => {
		const sendSlack = vi
			.fn()
			.mockResolvedValue({ messageId: "m-new-chan", channelId: "C555" });
		const sendText = requireSlackSendText();
	...
	it("sends media to a new user target without erroring", async () => {
		const sendSlack = vi
			.fn()
			.mockResolvedValue({ messageId: "m-new-media", channelId: "D888" });
		const sendMedia = requireSlackSendMedia();
