...
describe("MuxReceiver", () => {
  ...
  test("retries initial connection with backoff on failure", async () => {
    ...
    // Start in background — it will retry
    const startPromise = receiver.start();

    // Wait for at least one retry log
    await vi.waitFor(
      () =>
        expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("initial connect failed")),
      { timeout: 5000 },
    );

    // Stop the receiver to cancel the retry loop
    await receiver.stop();
    // start() should resolve once stopped (loop exits on this.stopped)
    await startPromise;
  });
