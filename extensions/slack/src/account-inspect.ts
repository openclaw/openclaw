...
export type SlackCredentialStatus =
	| "available"
...
export function inspectSlackAccount(params: {
	...
	const enabled =
		params.cfg.channels?.slack?.enabled !== false && merged.enabled !== false;
	...
	const envBot = allowEnv
		? normalizeSecretInputString(
				params.envBotToken ?? process.env.SLACK_BOT_TOKEN,
			)
		: undefined;
	const envApp = allowEnv
		? normalizeSecretInputString(
				params.envAppToken ?? process.env.SLACK_APP_TOKEN,
			)
		: undefined;
	const envUser = allowEnv
		? normalizeSecretInputString(
				params.envUserToken ?? process.env.SLACK_USER_TOKEN,
			)
		: undefined;
