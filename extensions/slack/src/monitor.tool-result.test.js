...
describe("monitorSlackProvider tool results", () => {
  ...
  function setDirectMessageReplyMode(replyToMode) {
  ...
  function setRequireMentionChannelConfig(mentionPatterns) {
    slackTestState.config = {
      ...mentionPatterns ? {
        messages: {
          responsePrefix: "PFX",
  ...
  async function runDirectMessageEvent(ts, extraEvent = {}) {
  ...
  async function runChannelMessageEvent(text, overrides = {}) {
    await runSlackMessageOnce(monitorSlackProvider, {
      event: makeSlackMessageEvent({
        text,
        channel_type: "channel",
  ...
  function setHistoryCaptureConfig(channels) {
    slackTestState.config = {
  ...
  async function runMonitoredSlackMessages(events) {
  ...
  function setPairingOnlyDirectMessages() {
    const currentConfig = slackTestState.config;
    slackTestState.config = {
      ...currentConfig,
      channels: {
  ...
  function expectSingleSendWithThread(threadTs) {
    expect(sendMock).toHaveBeenCalledTimes(1);
    expect(sendMock.mock.calls[0]?.[2]?.threadTs).toBe(
      threadTs
    );
  }
  async function runDefaultMessageAndExpectSentText(expectedText) {
        ...
        list: [
          {
            id: "main",
            default: true,
            identity: { name: "Mainbot", theme: "space lobster", emoji: "\u{1F99E}" }
          },
          {
            id: "rich",
            identity: { name: "Richbot", theme: "lion bot", emoji: "\u{1F981}" }
          }
        ]
      },
      bindings: [
  ...
  it("updates assistant thread status when replies start", async () => {
    replyMock.mockImplementation(async (...args) => {
      const opts = args[1] ?? {};
  ...
  async function expectMentionPatternMessageAccepted(text) {
  ...
  it("ignores replyToId directive when replyToMode is off", async () => {
    ...
    expectSingleSendWithThread(void 0);
  });
  ...
  it("reacts to mention-gated room messages when ackReaction is enabled", async () => {
    replyMock.mockResolvedValue(void 0);
    const client = getSlackClient();
    if (!client) {
      throw new Error("Slack client not registered");
    }
