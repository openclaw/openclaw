...
vi.mock("../../../src/infra/net/fetch-guard.js", () => ({
	fetchWithSsrFGuard: (...args: unknown[]) =>
		fetchWithSsrFGuard(
			...(args as [params: { url: string; init?: RequestInit }]),
		),
...
describe("sendMessageSlack file upload with user IDs", () => {
	...
	beforeAll(async () => {
		vi.resetModules();
		({ sendMessageSlack, clearSlackDmChannelCache } = await import(
			"./send.js"
		));
	});
	...
	it("resolves bare user ID to DM channel before completing upload", async () => {
		...
		expect(client.files.completeUploadExternal).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				files: [
					expect.objectContaining({ id: "F001", title: "screenshot.png" }),
				],
			}),
		);
	});
	...
	it("uses uploadFileName as the title fallback when uploadTitle is omitted", async () => {
		...
		expect(client.files.completeUploadExternal).toHaveBeenCalledWith(
			expect.objectContaining({
				files: [
					expect.objectContaining({ id: "F001", title: "custom-name.bin" }),
				],
			}),
		);
	});
