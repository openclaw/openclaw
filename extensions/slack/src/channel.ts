...
const resolveSlackDmPolicy =
	createScopedDmSecurityResolver<ResolvedSlackAccount>({
		channelKey: "slack",
...
function resolveSlackProbe() {
	try {
		...
		if (
			...
			error.message === "Slack runtime not initialized"
		) {
			return probeSlack;
		}
		throw error;
...
type SlackSendFn = ReturnType<
	typeof getSlackRuntime
...
function parseSlackExplicitTarget(raw: string) {
	...
	return {
		to: target.id,
		chatType:
			target.kind === "user" ? ("direct" as const) : ("channel" as const),
	};
}
...
async function resolveSlackOutboundSessionRoute(params: {
	...
	const threadId = normalizeOutboundThreadId(
		params.threadId ?? params.replyToId,
	);
	...
	return {
		...
		peer,
		chatType:
			peerKind === "direct" ? ("direct" as const) : ("channel" as const),
		from:
			peerKind === "direct"
				? `slack:${parsed.id}`
				: peerKind === "group"
...
function formatSlackScopeDiagnostic(params: {
	...
	if (params.result.ok && params.result.scopes?.length) {
		return {
			text: `${label}${source}: ${params.result.scopes.join(", ")}`,
		} as const;
	}
	return {
...
const collectSlackSecurityWarnings =
	createOpenProviderConfiguredRouteWarningCollector<ResolvedSlackAccount>({
		...
		missingRouteAllowlist: {
			surface: "Slack channels",
			openBehavior:
				"with no channel allowlist; any channel can trigger (mention-gated)",
			remediation:
				'Set channels.slack.groupPolicy="allowlist" and configure channels.slack.channels',
		},
	});

export const slackPlugin: ChannelPlugin<ResolvedSlackAccount, SlackProbe> =
	createChatChannelPlugin<ResolvedSlackAccount, SlackProbe>({
		base: {
			...createSlackPluginBase({
			...
			messaging: {
				...
				resolveSessionTarget: ({ id }) =>
					normalizeSlackMessagingTarget(`channel:${id}`),
				...
				resolveOutboundSessionRoute: async (params) =>
					await resolveSlackOutboundSessionRoute(params),
			...
			directory: createChannelDirectoryAdapter({
				...
				listGroups: async (params) =>
					listSlackDirectoryGroupsFromConfig(params),
				...createRuntimeDirectoryLiveAdapter({
			...
			>({
				...
				formatCapabilitiesProbe: ({ probe }) => {
					...
					if (slackProbe?.team?.name || slackProbe?.team?.id) {
						...
						lines.push({
							text: `Team: ${slackProbe.team?.name ?? "unknown"}${id}`,
						});
					}
					return lines;
				},
				buildCapabilitiesDiagnostics: async ({ account, timeoutMs }) => {
					...
					lines.push(
						formatSlackScopeDiagnostic({ tokenType: "bot", result: botScopes }),
					);
		...
		outbound: {
			...
			attachedResults: {
				...
				sendText: async ({
					...
					const { send, threadTsValue, tokenOverride } =
						resolveSlackSendContext({
							cfg,
