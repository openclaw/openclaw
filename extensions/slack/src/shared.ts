...
export function isSlackPluginAccountConfigured(
...
export function isSlackSetupAccountConfigured(
	account: ResolvedSlackAccount,
): boolean {
	...
	const hasConfiguredAppToken =
		...
		hasConfiguredSecretInput(account.config.appToken);
	return hasConfiguredBotToken && hasConfiguredAppToken;
}

export const slackConfigAdapter =
	createScopedChannelConfigAdapter<ResolvedSlackAccount>({
