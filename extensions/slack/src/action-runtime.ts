...
export async function handleSlackAction(
	...
	if (reactionsActions.has(action)) {
		...
		if (action === "react") {
			...
			if (isEmpty) {
				const removed = writeOpts
					? await slackActionRuntime.removeOwnSlackReactions(
							channelId,
							messageId,
							writeOpts,
						)
			...
			if (writeOpts) {
				await slackActionRuntime.reactSlackMessage(
					channelId,
					messageId,
					emoji,
					writeOpts,
		...
		const reactions = readOpts
			? await slackActionRuntime.listSlackReactions(
					channelId,
					messageId,
					readOpts,
				)
	...
	if (messagingActions.has(action)) {
		...
		switch (action) {
			...
			case "editMessage": {
				...
				if (writeOpts) {
					await slackActionRuntime.editSlackMessage(
						channelId,
						messageId,
						content ?? "",
						{
			...
			case "deleteMessage": {
				...
				if (writeOpts) {
					await slackActionRuntime.deleteSlackMessage(
						channelId,
						messageId,
						writeOpts,
					);
	...
	if (pinActions.has(action)) {
		...
		if (action === "pinMessage") {
			...
			if (writeOpts) {
				await slackActionRuntime.pinSlackMessage(
					channelId,
					messageId,
					writeOpts,
				);
		...
		if (action === "unpinMessage") {
			...
			if (writeOpts) {
				await slackActionRuntime.unpinSlackMessage(
					channelId,
					messageId,
					writeOpts,
				);
