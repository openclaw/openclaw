...
/** Returns true when a Slack Web API error matches the given error code string. */
function isSlackErrorCode(err: unknown, code: string): boolean {
  return (
    ...
    (err as { data: { error?: unknown } }).data?.error === code
  );
}
...
export async function reactSlackMessage(
  ...
  try {
    ...
    // Slack returns `already_reacted` when the bot already has this reaction on
    // the message.  The desired state (reaction present) is already achieved —
    // treat it as a no-op instead of surfacing a noisy error to the user.
    if (isSlackErrorCode(err, "already_reacted")) return;
    throw err;
  }
}
...
export async function removeSlackReaction(
  ...
  try {
    ...
    // Slack returns `no_reaction` when the bot never had this reaction on the
    // message.  The desired state (reaction absent) is already satisfied —
    // treat it as a no-op.
    if (isSlackErrorCode(err, "no_reaction")) return;
    throw err;
  }
}
...
export async function removeOwnSlackReactions(
  ...
  await Promise.all(
    Array.from(toRemove, (name) =>
      removeSlackReaction(channelId, messageId, name, { client }),
    ),
  );
