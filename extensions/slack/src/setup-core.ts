...
function hasSlackInteractiveRepliesConfig(
	cfg: OpenClawConfig,
	accountId: string,
): boolean {
	const capabilities = resolveSlackAccount({ cfg, accountId }).config
		.capabilities;
...
function setSlackInteractiveReplies(
	...
	const nextCapabilities = Array.isArray(capabilities)
		? interactiveReplies
			...
			: capabilities.filter(
					(entry) =>
						String(entry).trim().toLowerCase() !== "interactivereplies",
				)
		: {
	...
	return {
		...
		inspect: ({
			...
			const configuredValue =
				params.inputKey === "botToken"
					? resolved.config.botToken
					: resolved.config.appToken;
			const resolvedValue =
				params.inputKey === "botToken" ? resolved.botToken : resolved.appToken;
			return {
				accountConfigured:
					Boolean(resolvedValue) || hasConfiguredSecretInput(configuredValue),
	...
	return {
		...
		groupAccess: createAccountScopedGroupAccessSection({
			...
			currentPolicy: ({
				...
				resolveSlackAccount({ cfg, accountId }).config.groupPolicy ??
				"allowlist",
			currentEntries: ({
				cfg,
				accountId,
				...
				Object.entries(
					resolveSlackAccount({ cfg, accountId }).config.channels ?? {},
				)
					.filter(
