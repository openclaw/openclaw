...
function createOllamaFetchMock(params: {
	...
	return vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
		...
		if (url.endsWith("/api/tags")) {
			...
			return jsonResponse({
				models: (params.tags ?? []).map((name) => ({ name })),
			});
		}
		if (url.endsWith("/api/show")) {
			...
			return contextWindow
				? jsonResponse({
						model_info: { "llama.context_length": contextWindow },
					})
				: jsonResponse({});
		}
...
describe("ollama setup", () => {
	...
	it("local mode does not trigger cloud auth", async () => {
		...
		expect(
			fetchMock.mock.calls.some((call) =>
				requestUrl(call[0]).includes("/api/me"),
			),
		).toBe(false);
	});
	...
	it("normalizes ollama/ prefix in non-interactive custom model download", async () => {
		...
		expect(JSON.parse(requestBodyText(pullRequest?.body))).toEqual({
	...
	it("accepts cloud models in non-interactive mode without pulling", async () => {
		...
		expect(fetchMock).toHaveBeenCalledTimes(1);
		expect(
			result.models?.providers?.ollama?.models?.map((model) => model.id),
