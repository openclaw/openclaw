...
function readMicrosoftProviderConfig(
	...
	return {
		...
		outputFormatConfigured:
			asBoolean(config.outputFormatConfigured) ??
			defaults.outputFormatConfigured,
...
function formatMicrosoftVoiceDescription(
	entry: MicrosoftVoiceListEntry,
): string | undefined {
	const personalities =
		entry.VoiceTag?.VoicePersonalities?.filter(Boolean) ?? [];
	...
	return Array.isArray(voices)
		? voices
				.map((voice) => ({
					...
					name:
						voice.FriendlyName?.trim() || voice.ShortName?.trim() || undefined,
					category: voice.VoiceTag?.ContentCategories?.find(
...
export function buildMicrosoftSpeechProvider(): SpeechProviderPlugin {
	return {
		...
		resolveConfig: ({ rawConfig }) =>
			normalizeMicrosoftProviderConfig(rawConfig),
		...
		isConfigured: ({ providerConfig }) =>
			readMicrosoftProviderConfig(providerConfig).enabled,
		synthesize: async (req) => {
			...
			const fallbackOutputFormat =
				outputFormat !== DEFAULT_EDGE_OUTPUT_FORMAT
