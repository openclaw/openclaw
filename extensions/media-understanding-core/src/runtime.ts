...
type MediaUnderstandingOutput = Awaited<
	ReturnType<typeof runCapability>
>["outputs"][number];

const KIND_BY_CAPABILITY: Record<
...
export async function describeImageFileWithModel(params: {
	...
	const provider = providerRegistry.get(
		normalizeMediaProviderId(params.provider),
	);
	if (!provider?.describeImage) {
		throw new Error(
			`Provider does not support image analysis: ${params.provider}`,
		);
	}
