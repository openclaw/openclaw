  ...
  private initializePractices(): void {
    this.practices = [
      ...
      },
      {
        ...
        check: (content: string) => {
          const publicExports = (content.match(/export\s+(?:async\s+)?function|export\s+class/g) || []).length;
          const jsdocComments = (content.match(/\/\*\*[\s\S]*?\*\//g) || []).length;
          
          return {
            passed: jsdocComments >= publicExports * 0.8, // 80% coverage
            message: jsdocComments < publicExports ? `Missing JSDoc: ${publicExports - jsdocComments} exports need documentation` : undefined
          };
        }
      },
      {
        ...
        check: (content: string) => {
          ...
          return {
            ...
            message: hasAsync && !hasTryCatch ? 'Async functions should have try-catch blocks' : undefined
          };
        }
      }
    ];
...
interface SecurityRule {
  ...
  category: 'secret-exposure' | 'injection-risk' | 'misconfiguration' | 'permission-issue' | 'dependency-risk' | 'pattern-violation';
  message: string;
  suggestion?: string;
}
...
export { SecurityScanner, SkillCreator, BestPracticeEnforcer };
export type { SecurityFinding, PracticeCheckResult };
