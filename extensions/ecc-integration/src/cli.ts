  ...
  .addCommand(
    new Command('status')
      ...
      .action(async () => {
        ...
        console.log('\n📋 Governance Status');
        console.log('===================');
        console.log(`Active Rules: ${status.governance.rulesActive}`);
        console.log(`Total Agents: ${status.governance.agents}`);
        console.log(`Total Tasks: ${status.governance.tasks}`);
        
        console.log('\n🤖 Agent Pools');
        console.log('==============');
        for (const [type, counts] of Object.entries(status.orchestration.agents)) {
          console.log(`${type}: ${counts.idle} idle / ${counts.working} working / ${counts.total} total`);
        }
        
        console.log('\n📊 Task Queue');
        console.log('=============');
        console.log(`Total: ${status.orchestration.queue.total}`);
        for (const [status, count] of Object.entries(status.orchestration.queue.byStatus)) {
  ...
  .addCommand(
    new Command('create <type>')
      ...
      .action(async (type: string) => {
        ...
        const agent = await system.orchestrator.createAgent(type as z.infer<typeof AgentTypeSchema>);
    ...
    new Command('scan <path>')
      ...
      .action(async (scanPath: string, options) => {
        ...
        if (options.quick) {
          ...
          console.log(`================`);
          console.log(`Status: ${result.passed ? '✅ PASSED' : '❌ FAILED'}`);
          console.log(`Critical: ${result.criticalCount} | High: ${result.highCount} | Medium: ${result.mediumCount}`);
