...
describe("listAzureSpeechVoices timeout", () => {
  ...
  it.each([
    { name: "configured", options: { timeoutMs: 100 }, deadlineMs: 100 },
    { name: "default", options: {}, deadlineMs: 30_000 },
  ])(
    "aborts a hanging voice list request within the $name timeout",
    async ({ options, deadlineMs }) => {
      let requestCount = 0;
