...
function sampleBandlimited(
	...
	for (let tap = -half; tap <= half; tap++) {
		...
		const lowPass =
			2 * cutoffCyclesPerSample * sinc(2 * cutoffCyclesPerSample * distance);
		const tapIndex = tap + half;
		const window =
			0.5 -
			0.5 * Math.cos((2 * Math.PI * tapIndex) / (RESAMPLE_FILTER_TAPS - 1));
...
export function resamplePcmTo8k(
	...
	const cutoffCyclesPerSample = Math.max(
		0.01,
		downsampleCutoff * RESAMPLE_CUTOFF_GUARD,
	);
