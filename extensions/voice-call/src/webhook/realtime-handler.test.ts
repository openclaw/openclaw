    ...
    try {
      ...
      try {
        ws.send(
          JSON.stringify({
            event: "start",
            start: { streamSid: "MZ-empty-args", callSid: "CA-empty-args" },
          }),
        );
        await vi.waitFor(() => {
          expect(createBridge).toHaveBeenCalled();
        ...
        await vi.waitFor(() => {
          expect(consult).toHaveBeenCalledWith({ question: "Send a Discord message." }, "call-1", {
