...
describe("RealtimeCallHandler path routing", () => {
  ...
  it("submits continuing responses only for realtime agent consult calls", async () => {
    ...
    let timingStartedAt: number | undefined;
    const timingEvents: Array<{ kind: string; at: number }> = [];
    const recordTiming = (kind: string) => {
      if (timingStartedAt !== undefined) {
        timingEvents.push({ kind, at: Date.now() - timingStartedAt });
      }
    };
    const submitToolResult = vi.fn(
    ...
    const sendUserMessage = vi.fn((message: string) => {
      if (message.includes('Answer: "One sec."')) {
        recordTiming("acknowledgement");
      } else if (message.includes('Answer: "The basement lights are on."')) {
        recordTiming("supervisor-phrase");
      }
    });
    ...
    try {
      ...
      try {
        ...
        console.log(
          `[voice-delivery-harness] ${JSON.stringify(
            Object.fromEntries(timingEvents.map((event) => [`${event.kind}Ms`, event.at])),
          )}`,
        );
