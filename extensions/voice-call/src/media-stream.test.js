...
const withTimeout = async (promise, timeoutMs = 2e3) => {
  let timer = null;
...
const startWsServer = async (handler) => {
...
const connectWs = async (url) => {
  ...
  await withTimeout(once(ws, "open"));
  return ws;
};
const waitForClose = async (ws) => {
  const [code, reason] = await withTimeout(once(ws, "close")) ?? [];
  return {
    code,
