...
function resolveDNARoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.DNA_ROOT?.trim() || process.env.DNA_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set DNA_ROOT (or legacy DNA_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveDNARoot();
