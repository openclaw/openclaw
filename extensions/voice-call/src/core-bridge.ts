...
function resolveEasyHubRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.EASYHUB_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set EASYHUB_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveEasyHubRoot(), "dist", "extensionAPI.js");
