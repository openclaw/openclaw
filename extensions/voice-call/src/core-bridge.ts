...
function resolveSmartAgentNeoRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SMART_AGENT_NEO_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["smart-agent-neo"]) {
  ...
  throw new Error("Unable to resolve core root. Set SMART_AGENT_NEO_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSmartAgentNeoRoot(), "dist", "extensionAPI.js");
