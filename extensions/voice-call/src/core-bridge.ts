...
function resolveActiviRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ACTIVI_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ACTIVI_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveActiviRoot(), "dist", "extensionAPI.js");
