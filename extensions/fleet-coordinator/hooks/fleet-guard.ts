...
import { acquireLock, releaseLock, getLockHolder } from "../../shared/fleet-redis.js";
...
const CATASTROPHIC_RM_PATTERNS = [
	/rm\s+-rf\s+\//,
	/rm\s+-rf\s+~/,
];
...
function isBlocked(command: string): { blocked: boolean; reason: string } {
	for (const pattern of DESTRUCTIVE_DOCKER_PATTERNS) {
		if (pattern.test(command)) return { blocked: true, reason: `Blocked: destructive Docker operation (${command.slice(0, 80)})` };
	}
	for (const pattern of FORCE_PUSH_PATTERNS) {
		if (pattern.test(command)) return { blocked: true, reason: "Blocked: force push is prohibited by fleet guard" };
	}
	for (const pattern of CATASTROPHIC_RM_PATTERNS) {
		if (pattern.test(command)) return { blocked: true, reason: "Blocked: catastrophic delete operation" };
	}
