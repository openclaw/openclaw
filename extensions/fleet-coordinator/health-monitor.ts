...
import { setHeartbeat } from "../shared/fleet-redis.js";
...
async function checkHttpHealth(url: string, timeoutMs = 5000): Promise<{ healthy: boolean; latencyMs: number; error?: string }> {
...
async function checkTcpHealth(host: string, port: number, timeoutMs = 5000): Promise<{ healthy: boolean; latencyMs: number; error?: string }> {
	const start = Date.now();
	try {
