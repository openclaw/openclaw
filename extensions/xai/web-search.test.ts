...
describe("xai web search config resolution", () => {
	it("prefers configured api keys and resolves grok scoped defaults", () => {
		expect(
			resolveXaiWebSearchCredential({ grok: { apiKey: "xai-secret" } }),
		).toBe("xai-secret");
	...
	it("uses config apiKey when provided", () => {
		expect(
			resolveXaiWebSearchCredential({ grok: { apiKey: "xai-test-key" } }),
		).toBe("xai-test-key");
	});
	...
	it("treats unresolved non-env SecretRefs as missing credentials instead of throwing", async () => {
		await withEnv({ XAI_API_KEY: undefined }, async () => {
			...
			await expect(
				maybeTool.execute({ query: "OpenClaw" }),
	...
	it("uses config model when provided", () => {
		expect(
			resolveXaiWebSearchModel({ grok: { model: "grok-4-fast-reasoning" } }),
		).toBe("grok-4-fast");
	});
	...
	it("respects inlineCitations config", () => {
		expect(resolveXaiInlineCitations({ grok: { inlineCitations: true } })).toBe(
			true,
		);
		expect(
			resolveXaiInlineCitations({ grok: { inlineCitations: false } }),
		).toBe(false);
	});
		...
		const result = extractXaiWebSearchContent({
			output: [
				{ type: "web_search_call" },
				{
					...
					annotations: [
						{ type: "url_citation", url: "https://example.com/direct" },
					],
				},
			],
		});
...
describe("xai provider models", () => {
	...
	it("publishes Grok 4.20 reasoning and non-reasoning models", () => {
		expect(
			resolveXaiCatalogEntry("grok-4.20-beta-latest-reasoning"),
		).toMatchObject({
		...
		expect(
			resolveXaiCatalogEntry("grok-4.20-beta-latest-non-reasoning"),
		).toMatchObject({
	...
	it("marks current Grok families as modern while excluding multi-agent ids", () => {
		...
		expect(
			isModernXaiModel("grok-4.20-multi-agent-experimental-beta-0304"),
		).toBe(false);
	});
