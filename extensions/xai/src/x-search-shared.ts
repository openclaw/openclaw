...
export function resolveXaiXSearchConfig(
...
export function resolveXaiXSearchInlineCitations(
	config?: Record<string, unknown>,
): boolean {
	return resolveXaiXSearchConfig(config).inlineCitations === true;
}

export function resolveXaiXSearchMaxTurns(
	config?: Record<string, unknown>,
): number | undefined {
...
export function buildXaiXSearchPayload(params: {
	...
	return {
		...
		...(params.options?.enableImageUnderstanding
			? { enableImageUnderstanding: true }
			: {}),
		...(params.options?.enableVideoUnderstanding
