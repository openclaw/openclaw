...
function readPluginCodeExecutionConfig(
	cfg?: OpenClawConfig,
): CodeExecutionConfig | undefined {
...
export function createCodeExecutionTool(options?: {
	...
	return {
		...
		execute: async (_toolCallId: string, args: Record<string, unknown>) => {
			...
			const codeExecutionConfigRecord =
				readCodeExecutionConfigRecord(codeExecutionConfig);
			const model = resolveXaiCodeExecutionModel(codeExecutionConfigRecord);
			const maxTurns = resolveXaiCodeExecutionMaxTurns(
				codeExecutionConfigRecord,
			);
