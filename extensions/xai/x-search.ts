...
type XSearchConfig = NonNullable<
...
function readPluginXaiWebSearchApiKey(
	cfg?: OpenClawConfig,
): string | undefined {
	return readConfiguredSecretString(
		resolveProviderWebSearchPluginConfig(
	...
	return {
		...
		parameters: Type.Object({
			...
			enable_image_understanding: Type.Optional(
				Type.Boolean({
					description:
						"Allow xAI to inspect images attached to matching posts.",
				}),
			),
			enable_video_understanding: Type.Optional(
				Type.Boolean({
					description:
						"Allow xAI to inspect videos attached to matching posts.",
				}),
			),
		}),
		execute: async (_toolCallId: string, args: Record<string, unknown>) => {
			...
			const fromDate = normalizeOptionalIsoDate(
				readStringParam(args, "from_date"),
				"from_date",
			);
			const toDate = normalizeOptionalIsoDate(
				readStringParam(args, "to_date"),
				"to_date",
			);
			...
			const xSearchConfigRecord = xSearchConfig as
				| Record<string, unknown>
				| undefined;
			const model = resolveXaiXSearchModel(xSearchConfigRecord);
			const inlineCitations =
				resolveXaiXSearchInlineCitations(xSearchConfigRecord);
