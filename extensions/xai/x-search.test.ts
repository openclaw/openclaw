...
function parseFirstRequestBody(
	mockFetch: ReturnType<typeof installXSearchFetch>,
) {
...
describe("xai x_search tool", () => {
	...
	it("uses the xAI Responses x_search tool with structured filters", async () => {
		...
		expect(
			(result?.details as { citations?: string[] } | undefined)?.citations,
		).toEqual(["https://x.com/openclaw/status/1"]);
	});
	...
	it("prefers the active runtime config for SecretRef-backed x_search keys", async () => {
		...
		expect(
			(request?.headers as Record<string, string> | undefined)?.Authorization,
		).toBe("Bearer x-search-runtime-key");
	});
