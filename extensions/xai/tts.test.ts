...
class FakeTtsWebSocket extends EventEmitter {
  ...
  addEventListener(
    ...
    const wrapped = (data?: unknown) => {
      ...
      listener(data && typeof data === "object" ? (data as Event) : ({ type: event } as Event));
    };
    if (options?.once) {
...
describe("xai tts", () => {
  ...
  describe("xaiTTSStream", () => {
    ...
    it("rejects connection errors before the text handoff completes", async () => {
      const { sockets, factory } = createSocketHarness();
      const resultPromise = xaiTTSStream({
        text: "hello",
        apiKey: "ok-key",
      ...
      sockets[0]?.error(new Error("socket failed"));
