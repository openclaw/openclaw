...
function resolveXSearchConfigRecord(
	...
	return xSearch && typeof xSearch === "object"
		? (xSearch as Record<string, unknown>)
		: undefined;
}
...
async function runXaiSearchProviderSetup(
	...
	const enableChoice = await ctx.prompter.select<"yes" | "skip">({
		...
		initialValue:
			existingXSearch?.enabled === true || ctx.quickstartDefaults
				? "yes"
				: "skip",
	});
	...
	const existingModel =
		typeof existingXSearch?.model === "string" && existingXSearch.model.trim()
			...
			: "";
	const knownModel = X_SEARCH_MODEL_OPTIONS.find(
		(entry) => entry.value === existingModel,
	)?.value;
...
function resolveXaiWebSearchCredential(
				...
				execute: async (args: Record<string, unknown>) => {
					...
					return await runXaiWebSearch({
						...
						cacheTtlMs: resolveCacheTtlMs(
							(searchConfig?.cacheTtlMinutes as number | undefined) ??
								undefined,
							DEFAULT_CACHE_TTL_MINUTES,
						),
					});
