...
describe("cdp", () => {
	...
	it("preserves query params when connecting via direct WebSocket URL", async () => {
		...
		wsServer.on("connection", (socket) => {
			socket.on("message", (data) => {
				...
				if (msg.method === "Target.createTarget") {
					socket.send(
						JSON.stringify({ id: msg.id, result: { targetId: "T_QP" } }),
					);
				}
			});
		});
	...
	it("rewrites 0.0.0.0 wildcard bind address to remote CDP host", () => {
		...
		expect(normalized).toBe(
			"ws://192.168.1.202:18850/devtools/browser/ABC?token=secret",
		);
	});
...
describe("isWebSocketUrl", () => {
	...
	it("returns false for http:// and https:// URLs", () => {
		...
		expect(
			isWebSocketUrl("https://production-sfo.browserless.io?token=abc"),
		).toBe(false);
	});
...
describe("parseHttpUrl with WebSocket protocols", () => {
	...
	it("rejects unsupported protocols", () => {
		expect(() => parseHttpUrl("ftp://example.com", "test")).toThrow(
			"must be http(s) or ws(s)",
		);
		expect(() => parseHttpUrl("file:///etc/passwd", "test")).toThrow(
			"must be http(s) or ws(s)",
		);
	});
});
