...
const trackedTabsBySession = new Map<
	string,
	Map<string, TrackedSessionBrowserTab>
>();
	...
	for (const tab of tabs) {
		try {
			...
			if (!isIgnorableCloseError(err)) {
				params.onWarn?.(
					`failed to close tracked browser tab ${tab.targetId}: ${String(err)}`,
				);
			}
		}
	}
...
export function __countTrackedSessionBrowserTabsForTests(
	sessionKey?: string,
): number {
