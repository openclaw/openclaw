...
describe("browser config", () => {
	...
	it("preserves wss:// cdpUrl with query params for the default profile", () => {
		...
		expect(profile?.cdpUrl).toBe(
			"wss://connect.browserbase.com/?apiKey=test-key",
		);
	...
	it("preserves loopback direct WebSocket cdpUrl for explicit profiles", () => {
		...
		expect(profile?.cdpUrl).toBe(
			"ws://127.0.0.1:9222/devtools/browser/ABC?token=test-key",
		);
		expect(profile?.cdpPort).toBe(9222);
	...
	it("rejects unsupported protocols", () => {
		expect(() =>
			resolveBrowserConfig({ cdpUrl: "ftp://127.0.0.1:18791" }),
		).toThrow("must be http(s) or ws(s)");
	});
	...
	it("sets usesChromeMcp only for existing-session profiles", () => {
		...
		expect(getBrowserProfileCapabilities(existingSession).usesChromeMcp).toBe(
			true,
		);
