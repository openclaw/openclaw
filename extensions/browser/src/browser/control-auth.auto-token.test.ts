...
const mocks = vi.hoisted(() => ({
	...
	resolveGatewayAuth: vi.fn(
		({
			...
			authConfig?:
				| NonNullable<NonNullable<OpenClawConfig["gateway"]>["auth"]>
				| undefined;
		}) => {
			const token =
...
describe("ensureBrowserControlAuth", () => {
	...
	it("fails when gateway.auth.token SecretRef is unresolved", async () => {
		...
		mocks.ensureGatewayStartupAuth.mockRejectedValueOnce(
			new Error("MISSING_GW_TOKEN"),
		);

		await expect(
			ensureBrowserControlAuth({ cfg, env: {} as NodeJS.ProcessEnv }),
