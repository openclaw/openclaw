...
const chromeMocks = vi.hoisted(() => ({
	getChromeWebSocketUrl: vi.fn(
		async () => "ws://127.0.0.1/devtools/browser/mock",
	),
}));
...
> &
	Pick<
		typeof import("./pw-tools-core.responses.js"),
...
describe("pw-tools-core", () => {
	...
	async function expectAtomicDownloadSave(params: {
		...
		const [savedDirReal, tempDirReal] = await Promise.all([
			fs
				.realpath(path.dirname(String(savedPath)))
				.catch(() => path.dirname(String(savedPath))),
	...
	it("waits for the next download and atomically finalizes explicit output paths", async () => {
		await withTempDir(async (tempDir) => {
			...
			await expect(fs.realpath(res.path)).resolves.toBe(
		...
		async () => {
			await withTempDir(async (tempDir) => {
				...
				await expect(p).rejects.toThrow(
					/alias escape blocked|Hardlinked path is not allowed/i,
				);
