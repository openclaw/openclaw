...
function localMullusiProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "mullusi",
...
function createLocalMullusiResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localMullusiProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveMullusiUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mullusi-reset-"));
    const profileDir = path.join(tempRoot, "mullusi");
    ...
    const ops = createLocalMullusiResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveMullusiUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mullusi-reset-no-own-"));
    const profileDir = path.join(tempRoot, "mullusi");
    fs.mkdirSync(profileDir, { recursive: true });
