...
function extractStructuredPages(
	result: ChromeMcpToolResult,
): ChromeMcpStructuredPage[] {
...
function extractSnapshot(result: ChromeMcpToolResult): ChromeMcpSnapshotNode {
	...
	if (!snapshot) {
		throw new Error(
			"Chrome MCP snapshot response was missing structured snapshot data.",
		);
	}
...
function normalizeChromeMcpUserDataDir(
	userDataDir?: string,
): string | undefined {
...
function cacheKeyMatchesProfileName(
	cacheKey: string,
	profileName: string,
): boolean {
	try {
	...
	const ready = (async () => {
		try {
			...
			if (!tools.tools.some((tool) => tool.name === "list_pages")) {
				throw new Error(
					"Chrome MCP server did not expose the expected navigation tools.",
				);
			}
		} catch (err) {
...
export async function stopAllChromeMcpSessions(): Promise<void> {
	const names = [
		...new Set([...sessions.keys()].map((key) => JSON.parse(key)[0] as string)),
	];
...
export function setChromeMcpSessionFactoryForTest(
	factory: ChromeMcpSessionFactory | null,
): void {
