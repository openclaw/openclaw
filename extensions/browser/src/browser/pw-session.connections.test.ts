...
describe("pw-session connection scoping", () => {
	it("does not share in-flight connectOverCDP promises across different cdpUrls", async () => {
		...
		let resolveA:
			| ((value: import("playwright-core").Browser) => void)
			| undefined;
		...
		connectOverCdpSpy.mockImplementation((async (...args: unknown[]) => {
			...
			if (endpointText === "http://127.0.0.1:9222") {
				return await new Promise<import("playwright-core").Browser>(
					(resolve) => {
						resolveA = resolve;
					},
				);
		...
		const pendingA = listPagesViaPlaywright({
			cdpUrl: "http://127.0.0.1:9222",
		});
		await Promise.resolve();
		const pendingB = listPagesViaPlaywright({
