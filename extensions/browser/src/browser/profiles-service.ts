...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
	...
	const createProfile = async (
		...
		const normalizedUserDataDir = rawUserDataDir
			? resolveUserPath(rawUserDataDir)
			: undefined;
		const driver =
			params.driver === "existing-session" ? "existing-session" : undefined;
		...
		if (rawCdpUrl) {
			...
			if (driver === "existing-session") {
				...
				if (cdpPort === null) {
					throw new BrowserResourceExhaustedError(
						"no available CDP ports in range",
					);
				}
				profileConfig = {
					cdpPort,
	...
	const deleteProfile = async (
		...
		const defaultProfile =
			cfg.browser?.defaultProfile ?? state.resolved.defaultProfile;
