...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "mullusi",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/mullusi" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    mullusi: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchMullusiChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "mullusi";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.MULLUSI_GATEWAY_PORT;
  process.env.MULLUSI_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.MULLUSI_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.MULLUSI_GATEWAY_PASSWORD;
