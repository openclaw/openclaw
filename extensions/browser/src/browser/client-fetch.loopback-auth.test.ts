...
const mocks = vi.hoisted(() => ({
	...
	dispatch: vi.fn(
		async (): Promise<BrowserDispatchResponse> => okDispatchResponse(),
	),
}));
...
vi.mock("./control-service.js", () => ({
	...
	startBrowserControlServiceFromConfig:
		mocks.startBrowserControlServiceFromConfig,
}));
...
describe("fetchBrowserJson loopback auth", () => {
	...
	it("preserves dispatcher error context while keeping no-retry hint", async () => {
		mocks.dispatch.mockRejectedValueOnce(
			new Error("Chrome CDP handshake timeout"),
		);

		await expectThrownBrowserFetchError(
			() => fetchBrowserJson<{ ok: boolean }>("/tabs"),
			{
				contains: [
	...
	it("surfaces 429 from HTTP URL as rate-limit error with no-retry hint", async () => {
		const response = new Response("max concurrent sessions exceeded", {
			status: 429,
		});
	...
	it("keeps Browserbase-specific wording for Browserbase 429 responses", async () => {
		vi.stubGlobal(
			...
			vi.fn(
				async () =>
					new Response("max concurrent sessions exceeded", { status: 429 }),
			),
		);
