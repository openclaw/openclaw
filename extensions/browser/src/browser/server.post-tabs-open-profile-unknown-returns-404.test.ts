...
describe("profile CRUD endpoints", () => {
	...
	it("validates profile create/delete endpoints", async () => {
		...
		const createBadExistingSession = await realFetch(
		...
		const createBadExistingSessionBody =
			(await createBadExistingSession.json()) as {
				error: string;
			};
		expect(createBadExistingSessionBody.error).toContain(
			"driver=existing-session is required",
		);
		...
		const createLegacyDriverBody = (await createLegacyDriver.json()) as {
			error: string;
		};
		expect(createLegacyDriverBody.error).toContain(
			'unsupported profile driver "extension"',
		);
