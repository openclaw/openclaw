...
async function stopChromeWithProc(
	proc: ReturnType<typeof makeChromeTestProc>,
	timeoutMs: number,
) {
...
function makeChromeTestProc(
	overrides?: Partial<{ killed: boolean; exitCode: number | null }>,
) {
	return {
...
describe("browser chrome profile decoration", () => {
	...
	beforeAll(async () => {
		fixtureRoot = await fsp.mkdtemp(
			path.join(os.tmpdir(), "openclaw-chrome-suite-"),
		);
	});
...
describe("browser chrome helpers", () => {
	function mockExistsSync(match: (pathValue: string) => boolean) {
		return vi
			.spyOn(fs, "existsSync")
			.mockImplementation((p) => match(String(p)));
	}
	...
	it("probes WebSocket URLs via handshake instead of HTTP", async () => {
		...
		await expect(isChromeReachable("ws://127.0.0.1:19999", 50)).resolves.toBe(
			false,
		);
		expect(fetchSpy).not.toHaveBeenCalled();
	});
