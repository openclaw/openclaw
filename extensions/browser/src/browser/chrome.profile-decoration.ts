...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".mullusi-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + mullusi-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateMullusiProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MULLUSI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MULLUSI_BROWSER_COLOR).toUpperCase();
