...
function buildExistingSessionWaitPredicate(params: {
	...
	if (params.text) {
		checks.push(
			`Boolean(document.body?.innerText?.includes(${JSON.stringify(params.text)}))`,
		);
	}
	if (params.textGone) {
		checks.push(
			`!document.body?.innerText?.includes(${JSON.stringify(params.textGone)})`,
		);
	}
	if (params.selector) {
		checks.push(
	...
	while (Date.now() < deadline) {
		...
		if (ready && params.url) {
			...
			ready =
				...
				matchBrowserUrlPattern(params.url, currentUrl);
		}
		if (ready) {
			return;
		}
...
function validateBatchTargetIds(
	...
	switch (kind) {
		...
		case "batch": {
			const actions = Array.isArray(raw.actions)
				? raw.actions.map(normalizeBatchAction)
				: [];
			if (!actions.length) {
			...
			if (countBatchActions(actions) > MAX_BATCH_ACTIONS) {
				throw new Error(
					`batch exceeds maximum of ${MAX_BATCH_ACTIONS} actions`,
				);
			}
			const targetId = toStringOrEmpty(raw.targetId) || undefined;
...
export function registerBrowserAgentActRoutes(
	...
	app.post("/act", async (req, res) => {
		...
		const earlyFn =
			kind === "wait" || kind === "evaluate" ? toStringOrEmpty(body.fn) : "";
		if (
			...
			run: async ({ profileCtx, cdpUrl, tab }) => {
				...
				switch (kind) {
						...
						if (isExistingSession) {
							if (delayMs) {
								return jsonError(
									res,
									501,
									"existing-session press does not support delayMs.",
								);
							}
							await pressChromeMcpKey({
								profileName,
					...
					case "drag": {
						...
						const startSelector =
							toStringOrEmpty(body.startSelector) || undefined;
						...
						if (!startRef && !startSelector) {
							return jsonError(
								res,
								400,
								"startRef or startSelector is required",
							);
						}
						if (!endRef && !endSelector) {
					...
					case "select": {
						...
						if ((!ref && !selector) || !values?.length) {
							return jsonError(
								res,
								400,
								"ref/selector and values are required",
							);
						}
						const timeoutMs = toNumber(body.timeoutMs);
						if (isExistingSession) {
					...
					case "wait": {
						...
						if (fn && !evaluateEnabled) {
							return jsonError(
								res,
								403,
								browserEvaluateDisabledMessage("wait"),
							);
						}
						if (
							timeMs === undefined &&
					...
					case "evaluate": {
						if (!evaluateEnabled) {
							return jsonError(
								res,
								403,
								browserEvaluateDisabledMessage("evaluate"),
							);
						}
					...
					case "batch": {
						...
						try {
							actions = Array.isArray(body.actions)
								? body.actions.map(normalizeBatchAction)
								: [];
						} catch (err) {
							return jsonError(
								res,
