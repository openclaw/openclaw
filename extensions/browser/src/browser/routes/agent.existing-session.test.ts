...
const chromeMcpMocks = vi.hoisted(() => ({
	evaluateChromeMcpScript: vi.fn(
		async (_params: { profileName: string; targetId: string; fn: string }) =>
			true,
	),
...
vi.mock("./agent.shared.js", () => ({
	...
	withRouteTabContext: vi.fn(
		async ({ run }: { run: (args: unknown) => Promise<void> }) => {
			await run({
...
beforeAll(async () => {
	...
	({ registerBrowserAgentSnapshotRoutes } = await import(
		"./agent.snapshot.js"
	));
});
...
describe("existing-session browser routes", () => {
	...
	it("allows labeled AI snapshots for existing-session profiles", async () => {
		...
		await handler?.(
			{ params: {}, query: { format: "ai", labels: "1" } },
			response.res,
		);
