...
function formatBrowserProxyTimeoutMessage(params: {
	...
	if (params.status) {
		...
		if (
			typeof params.status.transport === "string" &&
			params.status.transport.trim()
		) {
			statusParts.push(`transport=${params.status.transport}`);
		}
		if (
			typeof params.status.cdpUrl === "string" &&
			params.status.cdpUrl.trim()
		) {
	...
	try {
		...
		throw new Error(
			formatBrowserProxyTimeoutMessage({
				...
				path,
				profile:
					path === "/profiles" ? undefined : profileForStatus || undefined,
				timeoutMs,
