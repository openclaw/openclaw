...
export const __testing = {
	setDepsForTest(
		...
		browserToolDeps.browserArmDialog =
			overrides?.browserArmDialog ?? browserArmDialog;
		browserToolDeps.browserArmFileChooser =
			overrides?.browserArmFileChooser ?? browserArmFileChooser;
		browserToolDeps.browserCloseTab =
			overrides?.browserCloseTab ?? browserCloseTab;
		browserToolDeps.browserFocusTab =
			overrides?.browserFocusTab ?? browserFocusTab;
		browserToolDeps.browserNavigate =
			overrides?.browserNavigate ?? browserNavigate;
		browserToolDeps.browserOpenTab =
			overrides?.browserOpenTab ?? browserOpenTab;
		browserToolDeps.browserPdfSave =
			overrides?.browserPdfSave ?? browserPdfSave;
		browserToolDeps.browserProfiles =
			overrides?.browserProfiles ?? browserProfiles;
		...
		browserToolDeps.imageResultFromFile =
		...
		browserToolDeps.callGatewayTool =
			overrides?.callGatewayTool ?? callGatewayTool;
...
async function callBrowserProxy(params: {
	...
	const gatewayTimeoutMs =
		proxyTimeoutMs + BROWSER_PROXY_GATEWAY_TIMEOUT_SLACK_MS;
		...
		execute: async (_toolCallId, args) => {
			...
			if (isUserBrowserProfile) {
				if (requestedNode || target === "node") {
					throw new Error(
						`profile="${profile}" only supports the local host browser.`,
					);
				}
				if (target === "sandbox") {
					throw new Error(
			...
			switch (action) {
				case "status":
					...
					return jsonResult(
						await browserToolDeps.browserStatus(baseUrl, { profile }),
					);
				case "start":
					if (proxyRequest) {
						await proxyRequest({
				...
				case "open": {
					...
					const opened = await browserToolDeps.browserOpenTab(
						baseUrl,
						targetUrl,
						{ profile },
					);
