...
export function describeBrowserTool(opts: {
  ...
  return [
    ...
    "act:batch runs nested actions[] in one call (stopOnError defaults to stop). For double clicks, set doubleClick=true on act:click. Pass labels=true on snapshot or screenshot to overlay role refs on the image and return an annotations[] array (Playwright-backed profiles only).",
