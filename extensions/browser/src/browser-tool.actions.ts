...
export const __testing = {
	setDepsForTest(
		...
		browserToolActionDeps.browserSnapshot =
			overrides?.browserSnapshot ?? browserSnapshot;
...
function canRetryChromeActWithoutTargetId(
	request: Parameters<typeof browserAct>[1],
): boolean {
...
export async function executeActAction(params: {
	...
	try {
		...
		if (isChromeStaleTargetError(profile, err)) {
			...
			if (
				retryRequest &&
				canRetryChromeActWithoutTargetId(request) &&
				tabs.length === 1
			) {
				try {
