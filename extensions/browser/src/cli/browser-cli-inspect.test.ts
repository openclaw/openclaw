...
const { defaultRuntime: runtime, resetRuntimeCapture } =
...
vi.spyOn(cliCoreApiModule, "loadConfig").mockImplementation(
	configMocks.loadConfig,
);
...
describe("browser cli snapshot defaults", () => {
	...
	beforeAll(async () => {
		({ registerBrowserInspectCommands } = await import(
			"./browser-cli-inspect.js"
		));
	});
