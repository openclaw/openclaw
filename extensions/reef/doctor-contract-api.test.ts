...
describe("Reef doctor contract", () => {
  ...
  it("drops a torn final replay JSON record without a trailing newline", async () => {
    const legacyDir = path.join(stateDir, ".openclaw", "data", "reef");
    const replayPath = path.join(legacyDir, "replay.jsonl");
    fs.mkdirSync(legacyDir, { recursive: true });
    fs.writeFileSync(replayPath, "{broken");
  ...
  it("does not drop a semantically invalid final replay record", async () => {
    const legacyDir = path.join(stateDir, ".openclaw", "data", "reef");
    const replayPath = path.join(legacyDir, "replay.jsonl");
    fs.mkdirSync(legacyDir, { recursive: true });
    fs.writeFileSync(
    ...
    expect(result.changes).toEqual([]);
    expect(result.warnings).toEqual([
      expect.stringContaining("Failed importing Reef replay state"),
