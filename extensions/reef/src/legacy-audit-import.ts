...
export async function validateLegacyReefAuditJournal(
  ...
  let lastHash = "";
  let lastSeq = 0;
  await forEachLegacyReefJsonlRecord(filePath, "reject-torn", (value) => {
...
async function registerAuditEntry(
  ...
  if (recordBytes > MAX_PLUGIN_STATE_VALUE_BYTES) {
    throw new Error(
...
export async function streamLegacyReefAuditWindow(
  ...
  try {
    await forEachLegacyReefJsonlRecord(filePath, "reject-torn", async (value) => {
