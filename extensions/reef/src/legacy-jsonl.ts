...
export async function forEachLegacyReefJsonlRecord(
  ...
  try {
    ...
    // An unterminated final record is the only tolerated torn append. Keep
    // visitor errors outside this catch so semantic, capacity, and storage
    // failures cannot be mistaken for a torn write and silently discarded.
    return;
  }
