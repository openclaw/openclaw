    ...
    return this.withLock(async () => {
      ...
      try {
        ...
        if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
          this.logger.warn(`[memory-hybrid][graph] Load failed: ${String(err)}`);
        }
      ...
      try {
        ...
        if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
          this.logger.warn(`[memory-hybrid][graph] Legacy load failed: ${String(err)}`);
        }
  ...
  try {
    ...
    try {
      ...
      tracer.trace(
        ...
        { raw: cleanJson },
        `Graph JSON Parse Failed: ${String(parseErr)}. Attempting regex rescue.`,
      );
