...
Return ONLY the merged fact as a single plain text string (no JSON, no quotes, no explanation).`;
  ...
  try {
    ...
    // Sanity checks
    if (merged.length < 5 || merged.length > 500) {
      return null;
    }
...
export async function mergeFactsBatch(
  ...
  if (logger) {
    logger.info(`[memory-hybrid] Merging ${clusters.length} clusters...`);
  }
...
${formattedClusters}`;
  ...
  try {
    ...
    if (logger) {
      logger.warn(
        `[memory-hybrid][consolidate] mergeFactsBatch JSON parse failed: ${String(error)}`,
      );
    }
    return [];
  }
