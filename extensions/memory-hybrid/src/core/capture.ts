...
import { DEFAULT_CAPTURE_MAX_CHARS, type MemoryCategory } from "../infra/config.js";
import { TaskPriority } from "../infra/limiter.js";
import { type MemoryTracer, type Logger } from "../infra/tracer.js";
import { escapePrompt } from "../infra/utils.js";
...
export function detectCategory(text: string): MemoryCategory {
  ...
  if (/rozhodli|decided|will use|budeme|вирішили|будемо/i.test(lower)) {
    return "decision";
  }
  if (
    /\+\d{10,}|@[\w.-]+\.\w+|is called|jmenuje se|звати|мій номер|мій email|my name/i.test(lower)
  ) {
    return "entity";
  }
  // Fact detection: possessive + verb, or "The X is/are Y", or "X uses/runs/works"
  if (/\b(his|her|my|our|their|the)\b.+\b(is|are|has|have|was|were)\b/i.test(lower)) {
    return "fact";
  }
  if (/\b(uses|runs|works|supports|requires|contains)\b/i.test(lower)) {
    return "fact";
  }
  if (/\bfact:|note:|fyi:/i.test(lower)) {
    return "fact";
  }
  return "other";
}
  ...
  try {
    ...
    try {
      ...
      tracer.trace(
        ...
        { raw: cleanJson },
        `JSON Parse Failed: ${String(parseErr)}. Attempting regex rescue.`,
      );
    ...
    if (logger) {
      logger.warn(`[memory-hybrid][capture] smartCapture failed: ${String(error)}`);
    }
    return { shouldStore: false, facts: [] };
  }
}
...
export async function generateMemorySummary(
  ...
  if (text.length < 100) {
    return text;
  } // Too short to summarize, keep it as is.
...
Return ONLY the summary text, nothing else.`;
  ...
  try {
    ...
    if (logger) {
      logger.warn(`[memory-hybrid][summary] Failed to generate summary: ${String(error)}`);
    }
