...
import { withRetry as _withRetry } from "../infra/utils.js";
...
export class MemoryDB {
  ...
  private async doInitialize(): Promise<void> {
    ...
    if (tables.includes(TABLE_NAME)) {
      ...
      try {
        ...
        this.logger.warn(
          `[memory-hybrid] FTS index creation failed (text search degraded): ${String(err)}`,
        );
      }
    }
  }
  ...
  async searchWithAMHR(
    ...
    return [...initialResults, ...associativeResults].toSorted((a, b) => b.score - a.score);
  }
  ...
  async flushRecallCounts(): Promise<number> {
    ...
    if (this.recallCountDeltas.size === 0) {
      return 0;
    }
