...
import type { ChatModel } from "../api/chat.js";
import { handleRecall, handleCapture } from "../api/handlers.js";
import type { MemoryDB } from "../core/database.js";
import type { Embeddings } from "../core/embeddings.js";
import type { GraphDB } from "../core/graph.js";
...
export function registerHooks(
  ...
  if (cfg.autoRecall) {
    api.on("before_agent_start", async (event, ctx) => {
      ...
      return handleRecall(event as never, ctx as never, api, deps, tracer);
    });
  }
  ...
  api.on("agent_end", async (event, ctx) => {
    ...
    if (cfg.autoCapture) {
      ...
      memoryQueue.push("auto-capture", () =>
        handleCapture(event as never, ctx as never, api, deps, tracer),
      );
    }
  });
  ...
  return {
    async cleanup() {
      ...
      try {
        ...
        api.logger.error(`memory-hybrid: cleanup error: ${String(err)}`);
      }
    },
  };
}
