...
import { vectorDimsForModel, detectProvider } from "../src/core/embeddings.js";
import { GraphDB } from "../src/core/graph.js";
import { memoryConfigSchema } from "../src/infra/config.js";
import { escapePrompt } from "../src/infra/utils.js";
...
describe("plugin", () => {
  test("should export a valid plugin definition", async () => {
    const { default: memoryPlugin } = await import("../src/index.js");
...
describe("MemoryDB Error Handling", () => {
  test("should handle search failure gracefully", async () => {
    ...
    await expect(db.search(Array.from({ length: 3072 }).fill(0) as number[], 5)).rejects.toThrow(
      "LanceDB connection lost",
    );
  });
  ...
  test("should handle store failure and not corrupt state", async () => {
    ...
    await expect(
      db.store({
        text: "test",
        vector: Array.from({ length: 3072 }).fill(0) as number[],
        importance: 0.5,
        category: "fact",
      }),
  ...
  describe("Concurrency & Safety", () => {
    ...
    test("Bug 4: should NOT drop in-flight recall deltas during flush", async () => {
      const _graph = new GraphDB("/tmp/test-graph", mockTracer, mockLogger as any);
