...
import { GraphDB, type GraphEdge } from "../src/core/graph.js";
import { hybridScore, getGraphEnrichment, type MemoryEntry } from "../src/core/recall.js";
  ...
  test("boosts memories with graph connections", async () => {
    const mockGraphWithEdges = {
      findEdgesForTexts: vi.fn().mockImplementation(async (texts) => {
        if (texts[0] === "Connected text") {
          return [{} as GraphEdge, {} as GraphEdge];
        }
        return [];
      }),
