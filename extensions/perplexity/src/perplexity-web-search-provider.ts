...
function resolvePerplexityBaseUrl(
	...
	const fromConfig =
		typeof perplexity?.baseUrl === "string" ? perplexity.baseUrl.trim() : "";
	if (fromConfig) {
		return fromConfig;
	}
...
function resolvePerplexityModel(perplexity?: PerplexityConfig): string {
	const model =
		typeof perplexity?.model === "string" ? perplexity.model.trim() : "";
...
function isDirectPerplexityBaseUrl(baseUrl: string): boolean {
	try {
		return (
			new URL(baseUrl.trim()).hostname.toLowerCase() === "api.perplexity.ai"
		);
	} catch {
		return false;
	}
...
function resolvePerplexityTransport(perplexity?: PerplexityConfig): {
	...
	const hasLegacyOverride = Boolean(
		perplexity?.baseUrl?.trim() || perplexity?.model?.trim(),
	);
	return {
		...auth,
		baseUrl,
		model,
		transport:
			hasLegacyOverride || !isDirectPerplexityBaseUrl(baseUrl)
				? "chat_completions"
				: "search_api",
	};
}
...
function resolveRuntimeTransport(params: {
	...
	const configuredBaseUrl =
		typeof scoped?.baseUrl === "string" ? scoped.baseUrl.trim() : "";
	const configuredModel =
		typeof scoped?.model === "string" ? scoped.model.trim() : "";
	const baseUrl = (() => {
		...
		if (
			(params.keySource === "config" || params.keySource === "secretRef") &&
			params.resolvedKey
		) {
			return inferPerplexityBaseUrlFromApiKey(params.resolvedKey) ===
				"openrouter"
				? DEFAULT_PERPLEXITY_BASE_URL
				: PERPLEXITY_DIRECT_BASE_URL;
		}
...
function createPerplexitySchema(transport?: PerplexityTransport) {
	...
	return Type.Object({
		...
		country: Type.Optional(
			Type.String({
				description:
					"Native Perplexity Search API only. 2-letter country code.",
			}),
		),
		language: Type.Optional(
			Type.String({
				description:
					"Native Perplexity Search API only. ISO 639-1 language code.",
			}),
		),
		...
		domain_filter: Type.Optional(
			Type.Array(Type.String(), {
				description:
					"Native Perplexity Search API only. Domain filter (max 20).",
			}),
		),
		max_tokens: Type.Optional(
			Type.Number({
				description:
					"Native Perplexity Search API only. Total content budget across all results.",
				minimum: 1,
				maximum: 1000000,
			}),
		),
		max_tokens_per_page: Type.Optional(
			Type.Number({
				description:
					"Native Perplexity Search API only. Max tokens extracted per page.",
				minimum: 1,
			}),
		),
	});
...
function createPerplexityToolDefinition(
	...
	const schemaTransport =
		runtimeTransport ??
		(perplexityConfig.baseUrl || perplexityConfig.model
			? "chat_completions"
			: undefined);
	...
	return {
		...
		execute: async (args) => {
			...
			const freshness = rawFreshness
				? normalizeFreshness(rawFreshness, "perplexity")
				: undefined;
			...
			const maxTokensPerPage = readNumberParam(params, "max_tokens_per_page", {
				integer: true,
			});
			...
			const dateAfter = rawDateAfter
				? normalizeToIsoDate(rawDateAfter)
				: undefined;
			const dateBefore = rawDateBefore
				? normalizeToIsoDate(rawDateBefore)
				: undefined;
				...
				runtime.transport === "chat_completions"
					...
					: {
							...
							results: await runPerplexitySearchApi({
								...
								searchAfterDate: dateAfter
									? isoToPerplexityDate(dateAfter)
									: undefined,
								searchBeforeDate: dateBefore
									? isoToPerplexityDate(dateBefore)
									: undefined,
...
export function createPerplexityWebSearchProvider(): WebSearchProviderPlugin {
	return {
		...
		getCredentialValue: (searchConfig) =>
			getScopedCredentialValue(searchConfig, "perplexity"),
		...
		createTool: (ctx) =>
			createPerplexityToolDefinition(
				...
				ctx.runtimeMetadata?.perplexityTransport as
					| PerplexityTransport
					| undefined,
			),
	};
