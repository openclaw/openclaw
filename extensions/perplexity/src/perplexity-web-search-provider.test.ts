...
describe("perplexity web search provider", () => {
	it("infers provider routing from api key prefixes", () => {
		expect(__testing.inferPerplexityBaseUrlFromApiKey("pplx-abc")).toBe(
			"direct",
		);
		expect(__testing.inferPerplexityBaseUrlFromApiKey("sk-or-v1-abc")).toBe(
			"openrouter",
		);
		expect(
			__testing.inferPerplexityBaseUrlFromApiKey("unknown"),
		).toBeUndefined();
	});

	it("resolves base url from auth source and request model by transport", () => {
		expect(
			__testing.resolvePerplexityBaseUrl(undefined, "perplexity_env"),
		).toBe("https://api.perplexity.ai");
		expect(
			__testing.resolvePerplexityBaseUrl(undefined, "openrouter_env"),
	...
	it("prefers explicit baseUrl over key-based defaults", () => {
		expect(
			__testing.resolvePerplexityBaseUrl(
	...
	it("switches direct Perplexity to chat completions when model override is configured", () => {
		expect(
			__testing.resolvePerplexityModel({
