...
describe("resolveLineGroupLookupIds", () => {
	it("expands raw ids to both prefixed candidates", () => {
		expect(resolveLineGroupLookupIds("abc123")).toEqual([
			"abc123",
			"group:abc123",
			"room:abc123",
		]);
	...
	it("preserves prefixed ids while also checking the raw id", () => {
		expect(resolveLineGroupLookupIds("room:abc123")).toEqual([
			"abc123",
			"room:abc123",
		]);
		expect(resolveLineGroupLookupIds("group:abc123")).toEqual([
			"abc123",
			"group:abc123",
		]);
	});
...
describe("account-scoped LINE groups", () => {
	it("resolves the effective account-scoped groups map", () => {
		...
		expect(
			resolveExactLineGroupConfigKey({ cfg, accountId: "work", groupId: "g1" }),
		).toBe("group:g1");
		expect(
			resolveExactLineGroupConfigKey({
				cfg,
...
describe("line group policy", () => {
	it("matches raw and prefixed LINE group keys for requireMention", () => {
		...
		expect(resolveLineGroupRequireMention({ cfg, groupId: "r123" })).toBe(
			false,
		);
		expect(resolveLineGroupRequireMention({ cfg, groupId: "room:r123" })).toBe(
			false,
		);
		expect(resolveLineGroupRequireMention({ cfg, groupId: "g123" })).toBe(
			false,
		);
		expect(resolveLineGroupRequireMention({ cfg, groupId: "group:g123" })).toBe(
			false,
		);
		expect(resolveLineGroupRequireMention({ cfg, groupId: "other" })).toBe(
			true,
		);
	});
