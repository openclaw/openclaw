...
async function expectSignedRawBodyWins(params: {
	...
	const processedBody = (
		onEvents.mock.calls[0] as unknown as [
...
describe("createLineNodeWebhookHandler", () => {
	...
	it("uses strict pre-auth limits for signed POST requests", async () => {
		...
		const readBody = vi.fn(
			async (_req: IncomingMessage, maxBytes: number, timeoutMs?: number) => {
	...
	it("uses signed raw buffer body instead of a pre-parsed req.body object", async () => {
		await expectSignedRawBodyWins({
			rawBody: Buffer.from(
				JSON.stringify({
					events: [
						{ type: "message", source: { userId: "signed-buffer-user" } },
					],
				}),
				"utf-8",
			),
