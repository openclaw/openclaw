...
describe("buildLineMessageContext", () => {
	...
	it("resolves prefixed-only group config through the inbound message context", async () => {
		...
		expect(context?.ctxPayload.GroupSystemPrompt).toBe(
			"Use the prefixed group config",
		);
	});
	...
	it("routes LINE conversations through active ACP session bindings", async () => {
		...
		expect(context?.route.sessionKey).toBe(
			"agent:codex:acp:binding:line:default:test123",
		);
