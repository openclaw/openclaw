...
function createOutboundRuntime() {
  ...
  const pushMessagesLine = vi.fn(async (_to: string, _messages: unknown[], _options: unknown) =>
    lineResult("batch"),
  );
  const runtime = {
    channel: {
  ...
  const pushes = () => pushMessagesLine.mock.calls.map((call) => call[1]);
  return { runtime, pushMessageLine, pushMessagesLine, pushes };
}
...
describe("LINE carousel fallback delivery", () => {
  it.each(["", "After"])(
    ...
    async (text) => {
      const { runtime, pushes } = createOutboundRuntime();
