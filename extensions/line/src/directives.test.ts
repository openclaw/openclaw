...
describe("parseLineDirectives", () => {
  ...
  describe("confirm", () => {
    ...
    it("handles question mark and special chars", () => {
      ...
      const tmpl = result.lineData.templateMessage as { text: string } | undefined;
      expect(tmpl?.text).toBe("Delete this file? 🗑️");
    });
  });
  ...
  describe("combined directives", () => {
    ...
    it("extracts only one directive type (first match wins for templateMessage)", () => {
      ...
      const tmpl = result.lineData.templateMessage as { type: string } | undefined;
      expect(tmpl?.type).toBe("buttons");
