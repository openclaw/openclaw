...
export const lineSetupWizard: ChannelSetupWizard = {
	...
	status: createStandardChannelSetupStatus({
		...
		resolveConfigured: ({ cfg }) =>
			listLineAccountIds(cfg).some((accountId) =>
			...
			inspect: ({ cfg, accountId }) => {
				...
				return {
					...
					hasConfiguredValue: Boolean(
						resolved.config.channelAccessToken?.trim() ||
							resolved.config.tokenFile?.trim(),
					),
		...
		},
		{
			...
			inspect: ({ cfg, accountId }) => {
				...
				return {
					...
					hasConfiguredValue: Boolean(
						resolved.config.channelSecret?.trim() ||
							resolved.config.secretFile?.trim(),
					),
