...
import type { ChannelPlugin, ResolvedLineAccount } from "../api.js";
...
const lineSecurityAdapter =
	createRestrictSendersChannelSecurity<ResolvedLineAccount>({
		channelKey: "line",
			...
			messaging: {
				...
				targetResolver: {
					looksLikeId: (id) => {
						...
						return (
							/^[UCR][a-f0-9]{32}$/i.test(trimmed) || /^line:/i.test(trimmed)
						);
					},
			...
			bindings: {
				...
				matchInboundConversation: ({ compiledBinding, conversationId }) => {
					const normalizedIncoming =
						normalizeLineConversationId(conversationId);
					if (
						!normalizedIncoming ||
						compiledBinding.conversationId !== normalizedIncoming
					) {
						return null;
					}
					return {
