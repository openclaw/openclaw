...
type LineClientOpts = Pick<
	LineSendOpts,
	"cfg" | "channelAccessToken" | "accountId"
>;
type LinePushOpts = Pick<
	LineSendOpts,
	"cfg" | "channelAccessToken" | "accountId" | "verbose"
>;
	...
	if (mediaUrl) {
		switch (opts.mediaKind) {
			case "video": {
				...
				const trackingId = isLineUserChatId(chatId)
					? opts.trackingId
					: undefined;
				messages.push(
					createVideoMessage(mediaUrl, previewImageUrl, trackingId),
				);
				break;
			}
			case "audio":
			...
			default:
				...
				messages.push(
					createImageMessage(
						mediaUrl,
						opts.previewImageUrl?.trim() || mediaUrl,
					),
				);
				break;
		}
