...
describe("stripMarkdown", () => {
	...
	it("preserves underscores inside words", () => {
		...
		expect(stripMarkdown("use foo_bar_baz in code")).toBe(
	...
	it("strips italic between underscored words", () => {
		expect(stripMarkdown("foo_bar _italic_ baz_qux")).toBe(
			"foo_bar italic baz_qux",
		);
	});
	...
	it("strips standalone italic between non-Latin words", () => {
		expect(stripMarkdown("こんにちは _italic_ テスト")).toBe(
			"こんにちは italic テスト",
		);
	});
