...
describe("validateLineMediaUrl", () => {
	it("accepts HTTPS URL", () => {
		expect(() =>
			validateLineMediaUrl("https://example.com/image.jpg"),
		).not.toThrow();
	});

	it("accepts uppercase HTTPS scheme", () => {
		expect(() =>
			validateLineMediaUrl("HTTPS://EXAMPLE.COM/img.jpg"),
		).not.toThrow();
	});

	it("rejects HTTP URL", () => {
		expect(() => validateLineMediaUrl("http://example.com/image.jpg")).toThrow(
			/must use HTTPS/i,
		);
	});
...
describe("resolveLineOutboundMedia", () => {
	...
	it("infers media kinds from known HTTPS file extensions", async () => {
		await expect(
			resolveLineOutboundMedia("https://example.com/audio.mp3"),
		).resolves.toEqual({
	...
	it("rejects local paths because LINE outbound media requires public HTTPS URLs", async () => {
		await expect(
			resolveLineOutboundMedia("./assets/image.jpg"),
		).rejects.toThrow(/requires a public https url/i);
	});

	it("rejects non-HTTPS URL explicitly", async () => {
		await expect(
			resolveLineOutboundMedia("http://example.com/image.jpg"),
		).rejects.toThrow(/must use HTTPS/i);
	});
});
