...
function hasLineSpecificMediaOptions(
...
export const lineOutboundAdapter: NonNullable<
	ChannelPlugin<ResolvedLineAccount>["outbound"]
> = {
	deliveryMode: "direct",
	chunker: (text, limit) =>
	...
	sendPayload: async ({ to, payload, accountId, cfg }) => {
		...
		const lineData =
			(payload.channelData?.line as LineChannelDataWithMedia | undefined) ?? {};
		...
		if (chunks.length > 0) {
			...
			for (const url of mediaUrls) {
				...
				quickReplyMessages.push(
					buildLineMediaMessageObject(resolved, {
						allowTrackingId: isLineUserTarget(to),
					}),
				);
			}
