...
const { getBotInfoMock, MessagingApiClientMock } = vi.hoisted(() => {
	...
	const MessagingApiClientMock = vi.fn(() => ({ getBotInfo: getBotInfoMock }));
...
describe("linePlugin status.probeAccount", () => {
	it("falls back to the direct probe helper when runtime is not initialized", async () => {
		...
		const { clearLineRuntime: clearFreshLineRuntime } = await import(
			"./runtime.js"
		);
		...
		await expect(
			freshLinePlugin.status?.probeAccount?.(params),
		).resolves.toEqual(await directProbeLineBot("token", 50));
	});
});
...
function startLineAccount(params: {
