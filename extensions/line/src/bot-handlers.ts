...
function pruneLineWebhookReplayCache(
...
function shouldSkipLineReplayEvent(
	...
	if (inFlightResult) {
		logVerbose(
			`line: skipped in-flight replayed webhook event ${candidate.eventId}`,
		);
...
function markLineReplayEventInFlight(
	candidate: LineReplayCandidate,
): LineInFlightReplayResult {
	let resolve!: () => void;
		...
		sendPairingReply: async (text) => {
			if (replyToken) {
				try {
					...
					logVerbose(
						`line pairing reply failed for ${senderId}: ${String(err)}`,
					);
				}
			}
			try {
...
async function shouldProcessLineEvent(
	...
	if (isGroup) {
		...
		if (
			...
			senderGroupAccess.reason === "missing_match_input"
		) {
			logVerbose(
		...
		if (
			!senderGroupAccess.allowed &&
			senderGroupAccess.reason === "not_allowlisted"
		) {
			logVerbose(
...
function isLineBotMentioned(message: MessageEvent["message"]): boolean {
	return getLineMentionees(message).some(
		(m) => m.isSelf === true || m.type === "all",
	);
}
...
async function handleMessageEvent(
	...
	if (isGroup) {
		...
		if (mentionGate.shouldSkip) {
			logVerbose(
				`line: skipping group message (requireMention, not mentioned)`,
			);
...
export async function handleLineWebhookEvents(
	...
	for (const event of events) {
		...
		const replaySkip = replayCandidate
			? shouldSkipLineReplayEvent(replayCandidate)
			: null;
		if (replaySkip?.skip) {
