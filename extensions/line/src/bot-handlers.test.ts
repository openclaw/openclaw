...
type LineWebhookContext = Parameters<
	typeof import("./bot-handlers.js").handleLineWebhookEvents
>[1];

const createRuntime = () => ({ log: vi.fn(), error: vi.fn(), exit: vi.fn() });

function _buildDefaultLineMessageContext() {
	return {
		ctxPayload: { From: "line:group:group-1" },
		replyToken: "reply-token",
		route: { agentId: "default" },
...
describe("handleLineWebhookEvents", () => {
	beforeAll(async () => {
		vi.resetModules();
		({ handleLineWebhookEvents, createLineWebhookReplayCache } = await import(
			"./bot-handlers.js"
		));
	});
	...
	beforeEach(() => {
		...
		buildLinePostbackContextMock.mockImplementation(
		...
		upsertPairingRequestMock.mockImplementation(async () => ({
			code: "CODE",
			created: true,
		}));
	});
	...
	it("blocks group sender not in groupAllowFrom even when sender is paired in DM store", async () => {
		...
		await handleLineWebhookEvents([event], {
			cfg: {
				channels: {
					line: { groupPolicy: "allowlist", groupAllowFrom: ["user-group"] },
				},
			},
			account: {
				accountId: "default",
	...
	it("blocks group messages without sender id when groupPolicy is allowlist", async () => {
		...
		await handleLineWebhookEvents([event], {
			cfg: {
				channels: {
					line: { groupPolicy: "allowlist", groupAllowFrom: ["user-5"] },
				},
			},
			account: {
				accountId: "default",
	...
	it("skips concurrent redeliveries while the first event is still processing", async () => {
		...
		const { firstRun, secondRun } = await startInflightReplayDuplicate({
			event,
			processMessage,
		});
		resolveFirst?.();
	...
	it("mirrors in-flight replay failures so concurrent duplicates also fail", async () => {
		...
		const firstFailure = expect(firstRun).rejects.toThrow(
			"transient inflight failure",
		);
	...
	it("deduplicates redeliveries by LINE message id when webhookEventId changes", async () => {
		...
		const context: Parameters<typeof handleLineWebhookEvents>[1] = {
			cfg: {
				channels: {
					line: { groupPolicy: "allowlist", groupAllowFrom: ["user-dup"] },
				},
			},
			account: {
				accountId: "default",
	...
	it("does not bypass mention gating when non-bot mention is present with control command", async () => {
		...
		const event = createTestMessageEvent({
			message: {
				...
				text: "@other !status",
				mention: {
					mentionees: [{ index: 0, length: 6, type: "user", isSelf: false }],
				},
			} as unknown as MessageEvent["message"],
