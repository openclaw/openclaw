...
function formatVoiceMeta(voice: SpeechVoiceOption): string | undefined {
	...
	const personalities =
		voice.personalities?.filter((value) => value.trim().length > 0) ?? [];
		...
		api.registerCommand({
			...
			handler: async (ctx) => {
				...
				const currentVoiceId =
					asTrimmedString(active.config.voiceId) ||
					asTrimmedString(cfg.talk?.voiceId);
				...
				if (action === "set") {
					...
					if (!chosen) {
						...
						return {
							text: `No voice found for ${hint}. Try: ${commandLabel} list`,
						};
					}
					...
					return {
						text: `✅ ${providerLabel} Talk voice set to ${name}\n${chosen.id}`,
					};
				}
