...
describe("talk-voice plugin", () => {
	function createElevenlabsVoiceSetHarness(
		channel = "webchat",
		scopes?: string[],
	) {
		...
		vi.mocked(runtime.tts.listVoices).mockResolvedValue([
			{ id: "voice-a", name: "Claudia" },
		]);
		return {
			runtime,
			run: async () =>
				await command.handler(
					createCommandContext("set Claudia", channel, scopes),
				),
		};
	}
	...
	it("writes provider voice id without legacy top-level field for microsoft", async () => {
		...
		await command.handler(
			createCommandContext("set Ava", "webchat", ["operator.admin"]),
		);
	...
	it("rejects /voice set from gateway client with only operator.write scope", async () => {
		const { runtime, run } = createElevenlabsVoiceSetHarness("webchat", [
			"operator.write",
		]);
