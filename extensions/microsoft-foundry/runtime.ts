...
async function refreshEntraToken(params?: {
	...
	const rawExpiry = result.expiresOn
...
export async function prepareFoundryRuntimeAuth(
	...
	try {
		...
		const credential = ctx.profileId
			? authStore.profiles[ctx.profileId]
			: undefined;
		const metadata =
			credential?.type === "api_key" ? credential.metadata : undefined;
		const modelId =
			typeof ctx.modelId === "string" && ctx.modelId.trim().length > 0
				? ctx.modelId.trim()
				: typeof metadata?.modelId === "string" &&
						metadata.modelId.trim().length > 0
		...
		const endpoint =
			...
			metadata.endpoint.trim().length > 0
		...
		throw new Error(
			`Failed to refresh Azure Entra ID token via az CLI: ${details}`,
		);
	}
}
