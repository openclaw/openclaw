...
export const COGNITIVE_SERVICES_RESOURCE =
...
export function normalizeFoundryModelName(
	value?: string | null,
): string | undefined {
...
export function requiresFoundryMaxCompletionTokens(
	value?: string | null,
): boolean {
	const normalized = normalizeFoundryModelName(value);
	if (!normalized) {
...
export function isFoundryProviderApi(
	value?: string | null,
): value is FoundryProviderApi {
...
export function resolveFoundryApi(
	...
	const configuredModelName = resolveConfiguredModelNameHint(
		modelId,
		modelNameHint,
	);
	return usesFoundryResponsesByDefault(configuredModelName)
		? DEFAULT_GPT5_API
		: DEFAULT_API;
}
...
export function extractFoundryEndpoint(
...
export function resolveConfiguredModelNameHint(
	...
	const trimmedName =
		typeof modelNameHint === "string" ? modelNameHint.trim() : "";
	if (trimmedName) {
		return trimmedName;
	}
...
export function buildFoundryProviderConfig(
	...
	const runtimeApiKey =
		options?.authMethod === "api-key" ? options.apiKey : undefined;
	...
	return {
		...
		models: deployments.map((deployment) => {
			const configuredName = resolveConfiguredModelNameHint(
				deployment.name,
				deployment.modelName,
			);
			const api = resolveFoundryApi(
				deployment.name,
				configuredName,
				deployment.api,
			);
...
function buildFoundryCredentialMetadata(params: {
	...
	const resolvedApi = resolveFoundryApi(
		params.modelId,
	...
	const modelName = resolveConfiguredModelNameHint(
		params.modelId,
		params.modelNameHint,
	);
	if (modelName) {
...
export function listConfiguredFoundryProfileIds(
...
export function resolveFoundryTargetProfileId(
	...
	return (
		...
		(configuredProfileEntries.length === 1
			? configuredProfileEntries[0]?.[0]
			: undefined)
	);
}
