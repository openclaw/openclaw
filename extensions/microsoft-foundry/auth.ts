	...
	run: async (ctx: ProviderAuthContext): Promise<ProviderAuthResult> => {
		...
		if (selectedSub) {
			...
			if (useDiscoveredResource) {
				...
				const resourceDeployments = listResourceDeployments(
					selectedResource,
					selectedSub.id,
				);
				...
				modelNameHint = resolveConfiguredModelNameHint(
					modelId,
				...
				await ctx.prompter.note(
					[
						...
						`Deployment: ${modelId}`,
						selectedDeployment.modelName
							? `Model: ${selectedDeployment.modelName}`
							: undefined,
				...
				({ endpoint, modelId, modelNameHint, api } =
					await promptEndpointAndModelManually(ctx));
			}
		} else {
			({ endpoint, modelId, modelNameHint, api } =
				await promptEndpointAndModelManually(ctx));
...
export const apiKeyAuthMethod: ProviderAuthMethod = {
	...
	run: async (ctx) => {
		...
		const existingMetadata =
			existing?.type === "api_key" ? existing.metadata : undefined;
		...
		return buildFoundryAuthResult({
			...
			modelNameHint:
				selection.modelNameHint ??
				existingMetadata?.modelName ??
