...
vi.mock("openclaw/plugin-sdk/provider-auth", async () => {
	const actual = await vi.importActual<
		typeof import("openclaw/plugin-sdk/provider-auth")
...
function buildFoundryRuntimeAuthContext(
	...
	return {
		...
		model: buildFoundryModel({
			id: modelId,
			...("model" in overrides ? overrides.model : {}),
		}),
...
describe("microsoft-foundry plugin", () => {
	...
	it("uses the active ordered API key profile when model selection rebinding is needed", async () => {
		...
		expect(config.auth?.order?.["microsoft-foundry"]).toEqual([
			"microsoft-foundry:default",
		]);
	});
	...
	it("keeps other configured Foundry models when switching the selected model", async () => {
		...
		expect(
			config.models?.providers?.["microsoft-foundry"]?.models.map(
				(model) => model.id,
			),
	...
	it("accepts tenant domains as valid tenant identifiers", () => {
		...
		expect(
			isValidTenantIdentifier("00000000-0000-0000-0000-000000000000"),
		).toBe(true);
	...
	it("writes Azure API key header overrides for API-key auth configs", () => {
		...
		expect(
			result.configPatch?.models?.providers?.["microsoft-foundry"],
		).toMatchObject({
			apiKey: "test-api-key",
			authHeader: false,
	...
	it("keeps persisted response-mode routing for custom deployment aliases", async () => {
		...
		expect(config.models?.providers?.["microsoft-foundry"]?.api).toBe(
			"openai-responses",
		);
		...
		expect(
			config.models?.providers?.["microsoft-foundry"]?.models[0]?.api,
	...
	it("persists discovered deployments alongside the selected default model", () => {
		const result = buildFoundryAuthResult({
			...
			deployments: [
				{
					name: "deployment-gpt5",
					modelName: "gpt-5.4",
					api: "openai-responses",
				},
				{
					name: "deployment-gpt4o",
					modelName: "gpt-4o",
					api: "openai-responses",
				},
			],
