...
function buildAzCommandError(
	error: Error,
	stderr: string,
	stdout: string,
): Error {
	const details = summarizeAzErrorMessage(
...
export function getLoggedInAccount(): AzAccount | null {
	try {
		return JSON.parse(
			execAz(["account", "show", "--output", "json"]),
		) as AzAccount;
	} catch {
		return null;
	}
...
export function getAccessTokenResult(
