...
async function createBlueBubblesConfigureAdapter() {
	const { blueBubblesSetupAdapter, blueBubblesSetupWizard } = await import(
		"./setup-surface.js"
	);
	const plugin = {
		id: "bluebubbles",
...
async function runBlueBubblesConfigure(params: {
	cfg: unknown;
...
describe("bluebubbles setup surface", () => {
	it("preserves existing password SecretRef and keeps default webhook path", async () => {
		...
		expect(result.cfg.channels?.bluebubbles?.webhookPath).toBe(
...
describe("BlueBubblesConfigSchema", () => {
	...
	it("defaults account enrichGroupParticipantsFromContacts to true", () => {
		...
		const accountConfig = (
			parsed.data as {
				accounts?: { work?: { enrichGroupParticipantsFromContacts?: boolean } };
			}
		).accounts?.work;
...
describe("bluebubbles group policy", () => {
	it("uses generic channel group policy helpers", () => {
		...
		expect(
			resolveBlueBubblesGroupRequireMention({ cfg, groupId: "chat:primary" }),
		).toBe(false);
		expect(
			resolveBlueBubblesGroupRequireMention({ cfg, groupId: "chat:other" }),
		).toBe(true);
		expect(
			resolveBlueBubblesGroupToolPolicy({ cfg, groupId: "chat:primary" }),
		).toEqual({
			deny: ["exec"],
		});
		expect(
			resolveBlueBubblesGroupToolPolicy({ cfg, groupId: "chat:other" }),
		).toEqual({
			allow: ["message.send"],
		});
	});
...
describe("looksLikeBlueBubblesExplicitTargetId", () => {
	it("treats explicit chat targets as immediate ids", () => {
		expect(looksLikeBlueBubblesExplicitTargetId("chat_guid:ABC-123")).toBe(
			true,
		);
		expect(looksLikeBlueBubblesExplicitTargetId("imessage:+15551234567")).toBe(
			true,
		);
	});

	it("prefers directory fallback for bare handles and phone numbers", () => {
		expect(looksLikeBlueBubblesExplicitTargetId("+1 (555) 123-4567")).toBe(
			false,
		);
		expect(looksLikeBlueBubblesExplicitTargetId("user@example.com")).toBe(
			false,
		);
	});
});
...
describe("inferBlueBubblesTargetChatType", () => {
	it("infers direct chat for handles and dm chat_guids", () => {
		...
		expect(
			inferBlueBubblesTargetChatType("chat_guid:iMessage;-;+15551234567"),
		).toBe("direct");
	});

	it("infers group chat for explicit group targets", () => {
		expect(inferBlueBubblesTargetChatType("chat_id:123")).toBe("group");
		expect(inferBlueBubblesTargetChatType("chat_guid:iMessage;+;chat123")).toBe(
			"group",
		);
	});
});
