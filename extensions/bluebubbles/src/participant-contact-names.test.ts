...
describe("enrichBlueBubblesParticipantsWithContactNames", () => {
	...
	it("enriches unnamed phone participants and reuses cached names across formats", async () => {
		...
		const second = await enrichBlueBubblesParticipantsWithContactNames(
			[{ id: "+15551234567" }],
			{
				platform: "darwin",
				now: () => 2_000,
	...
	it("retries negative cache entries after the short negative ttl expires", async () => {
		...
		const secondResolver = vi.fn(
			async () => new Map([["5551234567", "Alice Example"]]),
		);
		...
		const third = await enrichBlueBubblesParticipantsWithContactNames(
			[{ id: "+15551234567" }],
			{
				platform: "darwin",
				now: () => 1_000 + 6 * 60 * 1000,
	...
	it("queries only the requested phone keys in sqlite", async () => {
		const execFileAsync = vi.fn(
			async (_file: string, _args: string[], _options: unknown) => ({
	...
	it("resolves names through the macOS contacts path across multiple databases", async () => {
		...
		const execFileAsync = vi
			...
			.mockResolvedValueOnce({
				stdout: "5557654321\tBob Example\n",
				stderr: "",
			});

		const resolved =
			await resolveBlueBubblesParticipantContactNamesFromMacOsContactsForTest(
				["5551234567", "5557654321"],
				{
					homeDir: "/Users/tester",
					readdir,
	...
	it("skips contact lookup on non macOS hosts", async () => {
		...
		const result = await enrichBlueBubblesParticipantsWithContactNames(
			participants,
			{
				platform: "linux",
			},
