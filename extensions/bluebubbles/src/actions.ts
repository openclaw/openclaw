...
export const bluebubblesMessageActions: ChannelMessageActionAdapter = {
	describeMessageTool: ({ cfg, currentChannelId }) => {
		...
		for (const action of BLUEBUBBLES_ACTION_NAMES) {
			...
			if (
				"unsupportedOnMacOS26" in spec &&
				spec.unsupportedOnMacOS26 &&
				macOS26
			) {
				continue;
			}
		...
		if (!isGroupTarget) {
			for (const action of BLUEBUBBLES_ACTION_NAMES) {
				if (
					"groupOnly" in BLUEBUBBLES_ACTIONS[action] &&
					BLUEBUBBLES_ACTIONS[action].groupOnly
				) {
	...
	handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
		...
		if (action === "setGroupIcon") {
			...
			await runtime.setGroupIconBlueBubbles(
				resolvedChatGuid,
				buffer,
				filename,
				{
		...
		if (action === "removeParticipant") {
			...
			await runtime.removeBlueBubblesParticipant(
				resolvedChatGuid,
				address,
				opts,
			);
		...
		if (action === "sendAttachment" || action === "upload-file") {
			...
			if (base64Buffer) {
				...
				throw new Error(
					`BlueBubbles ${action} requires buffer (base64) parameter.`,
				);
			}
