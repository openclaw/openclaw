...
describe("bluebubblesMessageActions", () => {
  ...
  const callHandleAction = (ctx) => handleAction({ channel: "bluebubbles", ...ctx });
  const blueBubblesConfig = () => ({
    channels: {
      bluebubbles: {
  ...
  const runReactAction = async (params) => {
  ...
  describe("supportsAction", () => {
    ...
    it("returns false for unsupported actions", () => {
      expect(supportsAction({ action: "delete" })).toBe(false);
      expect(supportsAction({ action: "unknown" })).toBe(false);
    });
  });
