...
function extractBlueBubblesChatGuid(
...
function extractBlueBubblesChatId(
	chat: BlueBubblesChatRecord,
): number | undefined {
...
function extractBlueBubblesChatIdentifier(
	chat: BlueBubblesChatRecord,
): string | undefined {
	const candidates = [
		chat.chatIdentifier,
			...
			const matches =
				...
				(params.chatIdentifier &&
					(chatIdentifier === params.chatIdentifier ||
						chatGuid === params.chatIdentifier));
			if (matches) {
...
export async function processMessage(
	...
	if (message.fromMe) {
		...
		const confirmedAssistantOutbound =
			...
			normalizeSnippet(confirmedOutboundCacheEntry.body ?? "") ===
				normalizeSnippet(rawBody);
	...
	if (isSelfChatMessage && hasBlueBubblesSelfChatCopy(selfChatLookup)) {
		logVerbose(
			core,
			runtime,
			`drop: reflected self-chat duplicate sender=${message.senderId}`,
		);
		return;
	}
	...
	if (accessDecision.decision !== "allow") {
		...
		if (accessDecision.decision === "pairing") {
			await pairing.issueChallenge({
				...
				onCreated: () => {
					runtime.log?.(
						`[bluebubbles] pairing request sender=${message.senderId} created=true`,
					);
					logVerbose(
						core,
						runtime,
	...
	if (isGroup && !message.participants?.length && baseUrl && password) {
		try {
			const fetchedParticipants =
				await fetchBlueBubblesParticipantsForInboundMessage({
					baseUrl,
					password,
					chatGuid: message.chatGuid,
					chatId: message.chatId,
...
export async function processReaction(
	...
	if (requireMention) {
		logVerbose(
			core,
			runtime,
			"bluebubbles: skipping group reaction (requireMention=true)",
		);
		return;
	}
