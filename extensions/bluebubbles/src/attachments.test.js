...
const fetchRemoteMediaMock = vi.fn(
  async (params) => {
    ...
    if (typeof params.maxBytes === "number" && buffer.byteLength > params.maxBytes) {
      const error = new Error(`payload exceeds maxBytes ${params.maxBytes}`);
...
describe("downloadBlueBubblesAttachment", () => {
  ...
  async function expectAttachmentTooLarge(params) {
    ...
    const attachment = { guid: "att-large" };
    await expect(
  ...
  it("encodes guid in URL", async () => {
    ...
    const attachment = { guid: "att/with/special chars" };
    await downloadBlueBubblesAttachment(attachment, {
      serverUrl: "http://localhost:1234",
      password: "test"
    });
  ...
  it("throws on non-ok response", async () => {
    ...
    const attachment = { guid: "att-missing" };
    await expect(
      downloadBlueBubblesAttachment(attachment, {
        serverUrl: "http://localhost:1234",
        password: "test"
  ...
  it("resolves credentials from config when opts not provided", async () => {
    mockSuccessfulAttachmentDownload();
    const attachment = { guid: "att-config" };
    const result = await downloadBlueBubblesAttachment(attachment, {
      cfg: {
        channels: {
          bluebubbles: {
  ...
  it("passes ssrfPolicy with allowPrivateNetwork when config enables it", async () => {
    mockSuccessfulAttachmentDownload();
    const attachment = { guid: "att-ssrf" };
    await downloadBlueBubblesAttachment(attachment, {
      cfg: {
        channels: {
          bluebubbles: {
    ...
    const fetchMediaArgs = fetchRemoteMediaMock.mock.calls[0][0];
  ...
  it("auto-allowlists serverUrl hostname when allowPrivateNetwork is not set", async () => {
    mockSuccessfulAttachmentDownload();
    const attachment = { guid: "att-no-ssrf" };
    await downloadBlueBubblesAttachment(attachment, {
      serverUrl: "http://localhost:1234",
      password: "test"
    });
  ...
  it("auto-allowlists private IP serverUrl hostname when allowPrivateNetwork is not set", async () => {
    mockSuccessfulAttachmentDownload();
    const attachment = { guid: "att-private-ip" };
    await downloadBlueBubblesAttachment(attachment, {
