...
function resolveWebhookAllowRealIpFallback(
...
function resolveWebhookClientIp(
	...
	if (
		...
		!(allowRealIpFallback && req.headers["x-real-ip"])
	) {
...
export async function handleBlueBubblesWebhookRequest(
	...
	const clientIp = resolveWebhookClientIp(
		req,
		trustedProxies,
		allowRealIpFallback,
	);
	...
	return await withResolvedWebhookRequestPipeline({
		...
		handle: async ({ path, targets }) => {
			...
			if (!body.ok) {
				console.warn(
					`[bluebubbles] webhook rejected: status=${res.statusCode}`,
				);
				return true;
			}
			...
			const eventType =
				typeof eventTypeRaw === "string" ? eventTypeRaw.trim() : "";
