...
export function matchBlueBubblesAcpConversation(params: {
	...
	const binding = normalizeBlueBubblesAcpConversationId(
		params.bindingConversationId,
	);
	const conversation = normalizeBlueBubblesAcpConversationId(
...
export function resolveBlueBubblesInboundConversationId(params: {
	...
	const normalized =
		(params.chatGuid &&
			normalizeBlueBubblesAcpConversationId(params.chatGuid)?.conversationId) ||
		(params.chatIdentifier &&
			normalizeBlueBubblesAcpConversationId(params.chatIdentifier)
				?.conversationId) ||
...
export function resolveBlueBubblesConversationIdFromTarget(
	target: string,
): string | undefined {
