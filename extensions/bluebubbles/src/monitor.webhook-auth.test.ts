...
describe("BlueBubbles webhook monitor", () => {
	...
	function setupProtectedWebhookTarget(password = TEST_WEBHOOK_PASSWORD) {
		return setupWebhookTargetAccount(
			createProtectedWebhookTarget(password).account,
		);
	}
	...
	function createProtectedPasswordQueryRequestParams(
	...
	async function dispatchWebhookPayloadWithSetup(
		setup: () => void,
		payload: unknown,
	) {
		setup();
	...
	async function expectProtectedLoopbackWebhookRequestStatus(
		...
		setupProtectedWebhookTarget();
		return expectLoopbackWebhookRequestStatus(
			remoteAddress,
			expectedStatus,
			overrides,
		);
	...
	describe("webhook parsing + auth handling", () => {
		...
		it("rejects requests with invalid JSON", async () => {
			await expectRegisteredWebhookRequestStatus(
				{ body: "invalid json {{" },
				400,
			);
		});
		...
		it("authenticates via password query parameter", async () => {
			await expectProtectedWebhookRequestStatus(
				createProtectedPasswordQueryRequestParams(),
				200,
			);
		});
		...
		it("rejects targets without passwords for loopback and proxied-looking requests", async () => {
			...
			for (const headers of headerVariants) {
				await expectPasswordlessLoopbackWebhookRequestStatus("127.0.0.1", 401, {
					headers,
				});
			}
		});
		...
		it("extracts chatGuid from nested chat object fields (webhook variant)", async () => {
			const { sendMessageBlueBubbles, resolveChatGuidForTarget } = await import(
				"./send.js"
			);
