...
export const LOOPBACK_REMOTE_ADDRESSES_FOR_TEST = [
	"127.0.0.1",
	"::1",
	"::ffff:127.0.0.1",
] as const;
...
export function createProtectedWebhookAccountForTest(
	password = "test-password",
) {
	return createMockAccount({ password });
}

export function createNewMessagePayloadForTest(
...
export function createMessageReactionPayloadForTest(
	dataOverrides: Record<string, unknown> = {},
) {
	return {
...
export function createMockRequest(
	...
	const hasAuthQuery =
		parsedUrl.searchParams.has("guid") ||
...
export function createMockRequestForTest(
...
export async function dispatchWebhookPayloadForTest(
	params: WebhookRequestParams = {},
) {
...
export async function expectWebhookRequestStatusForTest(
	...
	return expectWebhookStatusForTest(
		createMockRequestForTest(params),
		expectedStatus,
		expectedBody,
	);
}

export function trackWebhookRegistrationForTest<
	T extends { unregister: () => void },
>(registration: T, setUnregister: (unregister: () => void) => void) {
