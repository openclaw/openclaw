...
function mockResolvedHandleTarget(guid = "iMessage;-;+15551234567", address = "+15551234567") {
...
function mockSendResponse(body) {
  mockFetch.mockResolvedValueOnce({
    ok: true,
    text: () => Promise.resolve(JSON.stringify(body))
  });
}
function mockNewChatSendResponse(guid) {
  mockFetch.mockResolvedValueOnce({
    ok: true,
...
describe("send", () => {
  describe("resolveChatGuidForTarget", () => {
    const resolveHandleTargetGuid = async (data) => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ data })
      });
    ...
    it("queries chats to resolve chat_id target", async () => {
      ...
      const target = { kind: "chat_id", chatId: 456 };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target
    ...
    it("returns null when chat not found", async () => {
      ...
      const target = { kind: "chat_id", chatId: 999 };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target
    ...
    it("handles API error gracefully", async () => {
      ...
      const target = { kind: "chat_id", chatId: 123 };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target
    ...
    it("paginates through chats to find match", async () => {
      ...
      const target = { kind: "chat_id", chatId: 555 };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target
    ...
    it("extracts guid from various response formats", async () => {
      ...
      const target = { kind: "chat_id", chatId: 100 };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target
  ...
  describe("sendMessageBlueBubbles", () => {
    ...
    it("warns and downgrades private-api features when status is unknown", async () => {
      ...
      setBlueBubblesRuntime({ log: runtimeLog });
