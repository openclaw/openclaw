...
export function createBlueBubblesMonitorTestRuntime(
	...
	return createPluginRuntimeMock({
		system: {
			enqueueSystemEvent:
				mocks.enqueueSystemEvent as PluginRuntime["system"]["enqueueSystemEvent"],
		},
		channel: {
			text: {
				chunkMarkdownText:
				...
				chunkByNewline:
					mocks.chunkByNewline as PluginRuntime["channel"]["text"]["chunkByNewline"],
...
export function resetBlueBubblesMonitorTestState(params: {
	...
	fetchHistoryMock: {
		mockResolvedValue: (value: BlueBubblesHistoryFetchResult) => unknown;
	};
	...
	resolveCommandAuthorizedFromAuthorizersMock: {
	...
	params.upsertPairingRequestMock.mockResolvedValue({
		code: "TESTCODE",
		created: true,
	});
