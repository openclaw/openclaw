...
function resolveBlueBubblesServerUrl(
	...
	return (
		resolveBlueBubblesAccount({ cfg, accountId }).config.serverUrl?.trim() ||
		undefined
	);
}
...
function resolveBlueBubblesWebhookPath(
	...
	return (
		resolveBlueBubblesAccount({ cfg, accountId }).config.webhookPath?.trim() ||
		undefined
	);
}
	...
	prepare: async ({ cfg, accountId, prompter, credentialValues }) => {
		...
		const wantsCustomWebhook = await prompter.confirm({
			...
			initialValue: Boolean(
				existingWebhookPath && existingWebhookPath !== DEFAULT_WEBHOOK_PATH,
			),
		});
		return {
	...
	credentials: [
		{
			...
			inspect: ({ cfg, accountId }) => {
				const existingPassword = resolveBlueBubblesAccount({ cfg, accountId })
					.config.password;
				return {
					accountConfigured: resolveBlueBubblesAccount({ cfg, accountId })
						.configured,
					hasConfiguredValue: hasConfiguredSecretInput(existingPassword),
					resolvedValue:
						normalizeSecretInputString(existingPassword) ?? undefined,
				};
			},
	...
	textInputs: [
		{
			...
			currentValue: ({ cfg, accountId }) =>
				resolveBlueBubblesServerUrl(cfg, accountId),
