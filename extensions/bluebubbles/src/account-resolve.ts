...
export function resolveBlueBubblesServerAccount(
	...
	try {
		const hostname = new URL(
			normalizeBlueBubblesServerUrl(baseUrl),
		).hostname.trim();
		autoAllowPrivateNetwork =
			Boolean(hostname) && isBlockedHostnameOrIp(hostname);
	} catch {
	...
	return {
		...
		allowPrivateNetwork:
			account.config.allowPrivateNetwork === true || autoAllowPrivateNetwork,
	};
}
