...
function resolveParticipantPhoneLookupKey(
	participant: BlueBubblesParticipant,
): string | null {
...
function trimParticipantContactNameCache(now: number): void {
	...
	while (
		participantContactNameCache.size >
		MAX_PARTICIPANT_CONTACT_NAME_CACHE_ENTRIES
	) {
...
function readFreshCacheEntry(
	phoneKey: string,
	now: number,
): ContactNameCacheEntry | null {
...
function writeCacheEntry(
	...
	participantContactNameCache.set(phoneKey, {
		...
		expiresAt:
			now +
			(name ? CONTACT_NAME_CACHE_TTL_MS : NEGATIVE_CONTACT_NAME_CACHE_TTL_MS),
	});
...
async function listContactsDatabases(
	deps: ResolvedParticipantContactNameDeps,
): Promise<string[]> {
...
function resolveLookupDeps(
	deps?: ParticipantContactNameDeps,
): ResolvedParticipantContactNameDeps {
	const merged = {
...
export async function enrichBlueBubblesParticipantsWithContactNames(
	...
	const lookup =
		...
		((phoneKeys: string[]) =>
			resolvePhoneNamesFromMacOsContacts(phoneKeys, resolvedDeps));
	const shouldAttemptLookup =
		Boolean(resolvedDeps.resolvePhoneNames) ||
