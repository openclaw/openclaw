...
describe("BlueBubbles webhook monitor", () => {
    ...
    it("falls back to group:peerId when chatName is missing", async () => {
      ...
      expect(mockFormatInboundEnvelope).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          chatType: "group",
          sender: { name: void 0, id: "+15551234567" }
        })
      );
    });
  ...
  describe("inbound debouncing", () => {
    it("coalesces text-only then attachment webhook events by messageId", async () => {
      ...
      try {
        ...
        const config = {};
        const core = createMockRuntime();
        core.channel.debounce.createInboundDebouncer = vi.fn((params) => {
  ...
  describe("tapback text parsing", () => {
    ...
    it("parses tapback text with custom emoji when metadata is present", async () => {
      ...
      const payload = {
        type: "new-message",
        data: {
          text: 'Reacted \u{1F605} to "nice one"',
          handle: { address: "+15551234567" },
          isGroup: false,
          isFromMe: false,
          guid: "msg-2",
      ...
      expect(mockDispatchReplyWithBufferedBlockDispatcher).toHaveBeenCalled();
      const callArgs = getFirstDispatchCall();
      expect(callArgs.ctx.RawBody).toBe("reacted with \u{1F605}");
      expect(callArgs.ctx.Body).toContain("reacted with \u{1F605}");
  ...
  describe("command gating", () => {
    ...
    it("does not auto-authorize DM control commands in open mode without allowlists", async () => {
      ...
      await flushAsync();
      expect(mockDispatchReplyWithBufferedBlockDispatcher).toHaveBeenCalled();
      const latestDispatch = mockDispatchReplyWithBufferedBlockDispatcher.mock.calls[mockDispatchReplyWithBufferedBlockDispatcher.mock.calls.length - 1]?.[0];
  ...
  describe("reaction events", () => {
    ...
    it("enqueues system event for reaction removed", async () => {
      ...
      const payload = {
        ...
        data: {
          ...
          isFromMe: false,
          associatedMessageGuid: "msg-original-123",
          associatedMessageType: 3e3,
          // Heart reaction removed
          date: Date.now()
        }
      };
      const req = createMockRequest("POST", "/bluebubbles-webhook", payload);
    ...
    it("maps reaction types to correct emojis", async () => {
      ...
      await flushAsync();
      expect(mockEnqueueSystemEvent).toHaveBeenCalledWith(
        expect.stringContaining("\u{1F44D}"),
        expect.any(Object)
      );
    });
  });
  ...
  describe("history backfill", () => {
    it("scopes in-memory history by account to avoid cross-account leakage", async () => {
      mockFetchBlueBubblesHistory.mockImplementation(async (_chatIdentifier, _limit, opts) => {
        if (opts?.accountId === "acc-a") {
          return {
            resolved: true,
            entries: [
              { sender: "A", body: "a-history", messageId: "a-history-1", timestamp: 1e3 }
            ]
          };
        }
        if (opts?.accountId === "acc-b") {
          return {
            resolved: true,
            entries: [
              { sender: "B", body: "b-history", messageId: "b-history-1", timestamp: 1e3 }
            ]
          };
        }
      ...
      const firstHistory = firstCall?.ctx.InboundHistory ?? [];
      const secondHistory = secondCall?.ctx.InboundHistory ?? [];
    ...
    it("dedupes and caps merged history to dmHistoryLimit", async () => {
      mockFetchBlueBubblesHistory.mockResolvedValueOnce({
        resolved: true,
        entries: [
          { sender: "Friend", body: "older context", messageId: "hist-1", timestamp: 1e3 },
          { sender: "Friend", body: "current text", messageId: "msg-1", timestamp: 2e3 }
        ]
      });
      ...
      await flushAsync();
      const callArgs = getFirstDispatchCall();
      const inboundHistory = callArgs.ctx.InboundHistory ?? [];
    ...
    it("uses exponential backoff for unresolved backfill and stops after resolve", async () => {
      ...
      const mkPayload = (guid, text, now2) => ({
        type: "new-message",
        data: {
          text,
          handle: { address: "+15551234567" },
      ...
      try {
        ...
        const thirdHistory = thirdCall?.ctx.InboundHistory ?? [];
    ...
    it("caps inbound history payload size to reduce prompt-bomb risk", async () => {
      ...
      expect(totalChars).toBeLessThanOrEqual(12e3);
