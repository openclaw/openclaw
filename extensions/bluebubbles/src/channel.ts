...
const resolveBlueBubblesDmPolicy =
	createScopedDmSecurityResolver<ResolvedBlueBubblesAccount>({
	...
	base: {
		...
		config: {
			...
			describeAccount: (account): ChannelAccountSnapshot =>
				describeBlueBubblesAccount(account),
		},
		...
		messaging: {
			...
			resolveOutboundSessionRoute: (params) =>
				resolveBlueBubblesOutboundSessionRoute(params),
			targetResolver: {
		...
		gateway: {
			startAccount: async (ctx) => {
				...
				const webhookPath = runtime.resolveWebhookPathFromConfig(
					account.config,
				);
	...
	pairing: {
		text: {
			...
			notify: async ({ cfg, id, message, accountId }) => {
				await (await loadBlueBubblesChannelRuntime()).sendMessageBlueBubbles(
					id,
					message,
					{
						cfg: cfg,
