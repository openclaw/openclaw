...
function readRoomId(params: Record<string, unknown>, required = true): string {
	const direct =
		readStringParam(params, "roomId") ?? readStringParam(params, "channelId");
	if (direct) {
		return direct;
	}
	if (!required) {
...
export async function handleMatrixAction(
	...
	if (pollActions.has(action)) {
		const roomId = readRoomId(params);
		const pollId = readStringAliasParam(params, ["pollId", "messageId"], {
			required: true,
		});
		if (!pollId) {
		...
		const optionIndex = readNumberParam(params, "pollOptionIndex", {
			integer: true,
		});
		const optionIds = [
	...
	if (verificationActions.has(action)) {
		...
		if (action === "verificationRecoveryKey") {
			...
			const result = await verifyMatrixRecoveryKey(
				readStringParam({ recoveryKey }, "recoveryKey", {
					required: true,
					trim: false,
				}),
				clientOpts,
		...
		if (action === "verificationRequest") {
			...
			const ownUser =
				typeof params.ownUser === "boolean" ? params.ownUser : undefined;
