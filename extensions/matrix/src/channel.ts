...
const listMatrixDirectoryPeersFromConfig =
	createResolvedDirectoryEntriesLister<ResolvedMatrixAccount>({
		...
		normalizeId: (entry) => {
			...
			const cleaned = lowered.startsWith("user:")
...
const resolveMatrixDmPolicy =
	createScopedDmSecurityResolver<ResolvedMatrixAccount>({
		channelKey: "matrix",
...
const collectMatrixSecurityWarnings =
	createAllowlistProviderOpenWarningCollector<ResolvedMatrixAccount>({
		providerConfigPresent: (cfg) =>
			(cfg as CoreConfig).channels?.matrix !== undefined,
...
function resolveMatrixAccountConfigPath(
	accountId: string,
	field: string,
): string {
...
function collectMatrixSecurityWarningsForAccount(params: {
	...
	if (params.account.accountId !== DEFAULT_ACCOUNT_ID) {
		const groupPolicyPath = resolveMatrixAccountConfigPath(
			params.account.accountId,
			"groupPolicy",
		);
		const groupsPath = resolveMatrixAccountConfigPath(
			params.account.accountId,
			"groups",
		);
	...
	const autoJoinPath = resolveMatrixAccountConfigPath(
		params.account.accountId,
		"autoJoin",
	);
...
function matchMatrixAcpConversation(params: {
	...
	const binding = normalizeMatrixAcpConversationId(
		...
		base: {
			...
			messaging: {
				...
				resolveOutboundSessionRoute: (params) =>
					resolveMatrixOutboundSessionRoute(params),
				targetResolver: {
					looksLikeId: (raw) => {
			...
			directory: createChannelDirectoryAdapter({
				listPeers: async (params) => {
					...
					return entries.map((entry) => {
						const raw = entry.id.startsWith("user:")
							? entry.id.slice("user:".length)
							: entry.id;
						const incomplete = !raw.startsWith("@") || !raw.includes(":");
						return incomplete
							? { ...entry, name: "incomplete id; expected @user:server" }
							: entry;
					});
				},
				listGroups: async (params) =>
					await listMatrixDirectoryGroupsFromConfig(params),
				...createRuntimeDirectoryLiveAdapter({
			...
			>({
				...
				collectStatusIssues: (accounts) =>
					collectStatusIssuesFromLastError("matrix", accounts),
				...
				probeAccount: async ({ account, timeoutMs, cfg }) => {
					try {
						const { probeMatrix, resolveMatrixAuth } =
