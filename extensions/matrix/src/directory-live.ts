...
function createMatrixDirectoryClient(
...
export async function listMatrixDirectoryPeersLive(
	...
	const res = await requestMatrixJson<MatrixUserDirectoryResponse>(
		context.client,
		{
			method: "POST",
...
export async function listMatrixDirectoryGroupsLive(
	...
	if (directTarget?.startsWith("!")) {
		return [
			createGroupDirectoryEntry({ id: directTarget, name: directTarget }),
		];
	}
