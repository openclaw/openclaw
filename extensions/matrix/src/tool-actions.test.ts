...
vi.mock("./matrix/actions.js", async () => {
	const actual = await vi.importActual<typeof import("./matrix/actions.js")>(
		"./matrix/actions.js",
	);
	return {
		...actual,
...
vi.mock("./matrix/send.js", async () => {
	const actual =
		await vi.importActual<typeof import("./matrix/send.js")>(
			"./matrix/send.js",
		);
	return {
		...actual,
...
vi.mock("./profile-update.js", () => ({
	applyMatrixProfileUpdate: (...args: unknown[]) =>
		mocks.applyMatrixProfileUpdate(...args),
}));
...
describe("handleMatrixAction pollVote", () => {
	beforeEach(() => {
		...
		mocks.listMatrixReactions.mockResolvedValue([
			{ key: "👍", count: 1, users: ["@u:example"] },
		]);
	...
	it("parses snake_case vote params and forwards normalized selectors", async () => {
		...
		expect(mocks.voteMatrixPoll).toHaveBeenCalledWith(
			"!room:example",
			"$poll",
			{
				cfg,
	...
	it("passes account-scoped opts to add reactions", async () => {
		const cfg = {
			channels: { matrix: { actions: { reactions: true } } },
		} as CoreConfig;
		await handleMatrixAction(
			{
				action: "react",
		...
		expect(mocks.reactMatrixMessage).toHaveBeenCalledWith(
			"!room:example",
			"$msg",
			"👍",
			{
	...
	it("passes account-scoped opts to remove reactions", async () => {
		...
		expect(mocks.removeMatrixReactions).toHaveBeenCalledWith(
			"!room:example",
			"$msg",
			{
				cfg,
	...
	it("passes account-scoped opts and limit to reaction listing", async () => {
		...
		expect(mocks.listMatrixReactions).toHaveBeenCalledWith(
			"!room:example",
			"$msg",
			{
				cfg,
	...
	it("passes mediaLocalRoots to profile updates", async () => {
		const cfg = {
			channels: { matrix: { actions: { profile: true } } },
		} as CoreConfig;
		await handleMatrixAction(
			{
				action: "setProfile",
	...
	it("passes account-scoped opts to member and room info actions", async () => {
		...
		const roomCfg = {
			channels: { matrix: { actions: { channelInfo: true } } },
		} as CoreConfig;
		await handleMatrixAction(
			{
				action: "channelInfo",
