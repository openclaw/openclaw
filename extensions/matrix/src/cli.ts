...
} from "./matrix/actions/client.js";
import {
	listMatrixOwnDevices,
	pruneMatrixStaleGatewayDevices,
} from "./matrix/actions/devices.js";
...
function printMatrixOwnDevices(
	...
	for (const device of devices) {
		const labels = [
			device.current ? "current" : null,
			device.displayName,
		].filter(Boolean);
		console.log(
			`- ${device.deviceId}${labels.length ? ` (${labels.join(", ")})` : ""}`,
		);
...
async function addMatrixAccount(params: {
	...
	const input: ChannelSetupInput & { avatarUrl?: string } = {
		...
		initialSyncLimit: parseOptionalInt(
			params.initialSyncLimit,
			"--initial-sync-limit",
		),
	...
	if (accountConfig.encryption === true) {
		const { maybeBootstrapNewEncryptedMatrixAccount } = await import(
			"./setup-bootstrap.js"
		);
	...
	try {
		...
		deviceHealth = {
			currentDeviceId:
				addedDevices.find((device) => device.current)?.deviceId ?? null,
...
function printDirectRoomCandidate(room: MatrixCliDirectRoomCandidate): void {
	const members =
		room.joinedMembers === null
			? "unavailable"
			: room.joinedMembers.join(", ") || "none";
	console.log(
		`- ${room.roomId} [${room.source}] strict=${room.strict ? "yes" : "no"} joined=${members}`,
	);
}

function printDirectRoomInspection(
	result: MatrixCliDirectRoomInspection,
): void {
...
async function runMatrixCliCommand<TResult>(
	...
	try {
		...
		if (config.json) {
			printJson(
				config.onJsonError ? config.onJsonError(message) : { error: message },
			);
		} else {
...
function toCliDirectRoomCandidate(
	room: MatrixDirectRoomCandidate,
...
function printBackupStatus(backup: MatrixCliBackupStatus): void {
	...
	console.log(
		`Backup matches local decryption key: ${yesNoUnknown(backup.matchesDecryptionKey)}`,
	);
	console.log(
		`Backup key cached locally: ${yesNoUnknown(backup.decryptionKeyCached)}`,
	);
	console.log(
		`Backup key load attempted: ${yesNoUnknown(backup.keyLoadAttempted)}`,
	);
...
function printVerificationTrustDiagnostics(status: {
	...
	console.log(
		`Cross-signing verified: ${status.crossSigningVerified ? "yes" : "no"}`,
	);
	console.log(`Signed by owner: ${status.signedByOwner ? "yes" : "no"}`);
}

function printVerificationGuidance(
...
function buildVerificationGuidance(
	...
	if (status.pendingVerifications > 0) {
		nextSteps.add(
			`Complete ${status.pendingVerifications} pending verification request(s).`,
		);
	}
...
function printVerificationStatus(
	...
	if (verbose) {
		...
		console.log(
			`Recovery key stored: ${status.recoveryKeyStored ? "yes" : "no"}`,
		);
...
export function registerMatrixCli(params: { program: Command }): void {
				...
				await runMatrixCliCommand({
					...
					onText: (result) => {
						...
						if (result.verificationBootstrap.attempted) {
							if (result.verificationBootstrap.success) {
								...
								if (result.verificationBootstrap.backupVersion) {
									console.log(
										`Backup version: ${result.verificationBootstrap.backupVersion}`,
									);
								}
							} else {
								console.error(
						...
						if (result.deviceHealth.error) {
							console.error(
								`Matrix device health warning: ${result.deviceHealth.error}`,
							);
						...
						if (result.profile.attempted) {
							if (result.profile.error) {
								...
								if (
									result.profile.convertedAvatarFromHttp &&
									result.profile.resolvedAvatarUrl
								) {
									console.log(
										`Avatar converted and saved as: ${result.profile.resolvedAvatarUrl}`,
									);
								}
							}
						}
	...
	const direct = root
		.command("direct")
		.description("Inspect and repair Matrix direct-room state");
			...
			async (options: {
				...
				await runMatrixCliCommand({
					...
					onText: (result, verbose) => {
						printDirectRoomInspection(result);
						console.log(
							`Encrypted room creation: ${result.encrypted ? "enabled" : "disabled"}`,
						);
	...
	const verify = root
		.command("verify")
		.description("Device verification for Matrix E2EE");
	...
	const backup = verify
		.command("backup")
		.description("Matrix room-key backup health and restore");
			...
			async (options: {
				...
				await runMatrixCliCommand({
					...
					run: async () => {
						if (options.yes !== true) {
							throw new Error(
								"Refusing to reset Matrix room-key backup without --yes",
							);
						}
					...
					onText: (result, verbose) => {
						...
						console.log(
							`Previous backup version: ${result.previousVersion ?? "none"}`,
						);
						console.log(
							`Deleted backup version: ${result.deletedVersion ?? "none"}`,
						);
						console.log(
							`Current backup version: ${result.createdVersion ?? "none"}`,
						);
	...
	backup
		...
		.option(
			"--recovery-key <key>",
			"Optional recovery key to load before restoring",
		)
		.option("--verbose", "Show detailed diagnostics")
		.option("--json", "Output as JSON")
		.action(
			...
			async (options: {
				...
				await runMatrixCliCommand({
					...
					onText: (result, verbose) => {
						...
						console.log(
							`Verified by owner: ${result.verification.verified ? "yes" : "no"}`,
						);
						...
						if (verbose) {
							...
							console.log(
								`Pending verifications: ${result.pendingVerifications}`,
							);
						} else {
							console.log(
	...
	verify
		...
		.action(
			async (
				key: string,
				options: { account?: string; verbose?: boolean; json?: boolean },
			) => {
				const accountId = resolveMatrixCliAccountId(options.account);
				await runMatrixCliCommand({
					verbose: options.verbose === true,
	...
	const devices = root
		.command("devices")
		.description("Inspect and clean up Matrix devices");
			...
			async (options: {
				...
				await runMatrixCliCommand({
					...
					onText: (result, verbose) => {
						...
						console.log(
							`Current device: ${result.currentDeviceId ?? "unknown"}`,
						);
