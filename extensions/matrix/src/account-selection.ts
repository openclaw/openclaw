...
export function findMatrixAccountEntry(
	...
	const entry = resolveNormalizedAccountEntry(
		accounts,
		accountId,
		normalizeAccountId,
	);
...
export function resolveConfiguredMatrixAccountIds(
	...
	return listCombinedAccountIds({
		configuredAccountIds: listConfiguredAccountIds({
			accounts:
				channel && isRecord(channel.accounts) ? channel.accounts : undefined,
			normalizeAccountId,
		}),
...
export function requiresExplicitMatrixDefaultAccount(
	...
	return !(
		configuredDefault && configuredAccountIds.includes(configuredDefault)
	);
}
