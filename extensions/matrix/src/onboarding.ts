...
function resolveMatrixOnboardingAccountId(
	cfg: CoreConfig,
	accountId?: string,
): string {
...
function setMatrixDmPolicy(
	...
	const allowFrom =
		policy === "open"
			? addWildcardAllowFrom(existing.dm?.allowFrom)
			: undefined;
...
function setMatrixGroupPolicy(
	...
	return updateMatrixAccountConfig(
		cfg,
		resolveMatrixOnboardingAccountId(cfg, accountId),
		{
			groupPolicy,
		},
	);
...
const dmPolicy: ChannelSetupDmPolicy = {
	...
	resolveConfigKeys: (cfg, accountId) => {
		const effectiveAccountId = resolveMatrixOnboardingAccountId(
			cfg as CoreConfig,
			accountId,
		);
		return {
	...
	setPolicy: (cfg, policy, accountId) =>
		setMatrixDmPolicy(cfg as CoreConfig, policy, accountId),
...
async function runMatrixConfigure(params: {
	...
	if (params.intent === "add-account") {
		...
		if (override) {
			...
			accountId = await promptAccountId({
				...
				label: "Matrix",
				currentId: accountId,
				listAccountIds: (inputCfg) =>
					listMatrixAccountIds(inputCfg as CoreConfig),
				defaultAccountId,
			});
		}
	}
	...
	const requiresAllowPrivateNetwork =
		requiresMatrixPrivateNetworkOptIn(homeserver);
	...
	const allowPrivateNetwork = shouldPromptAllowPrivateNetwork
		? await params.prompter.confirm({
				message:
					"Allow private/internal Matrix homeserver traffic for this account?",
				initialValue:
					existing.allowPrivateNetwork === true || requiresAllowPrivateNetwork,
			})
		: false;
	...
	if (
		hasConfiguredSecretInput(accessToken) ||
		hasConfiguredSecretInput(password)
	) {
	...
	const existingGroups =
		existingAccountConfig.groups ?? existingAccountConfig.rooms;
...
export const matrixOnboardingAdapter: MatrixOnboardingAdapter = {
	...
	getStatus: async ({ cfg, accountOverrides }) => {
		...
		if (
			!accountOverrides[channel] &&
			requiresExplicitMatrixDefaultAccount(resolvedCfg)
		) {
			return {
				channel,
				configured: false,
		...
		const account = resolveMatrixAccount({
			cfg: resolvedCfg,
			accountId: resolveMatrixOnboardingAccountId(
				resolvedCfg,
				accountOverrides[channel],
			),
		});
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import(
			"./setup-bootstrap.js"
		);
