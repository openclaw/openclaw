...
const MATRIX_GLOBAL_ENV_KEYS = MATRIX_SCOPED_ENV_SUFFIXES.map(
	(suffix) => `MATRIX_${suffix}`,
);
...
function decodeMatrixEnvAccountToken(token: string): string | undefined {
	...
	return resolveMatrixEnvAccountToken(normalized) === token
		? normalized
		: undefined;
}

export function listMatrixEnvAccountIds(
