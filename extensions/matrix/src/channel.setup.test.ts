...
describe("matrix setup post-write bootstrap", () => {
	...
	function applyDefaultAccountConfig(
		input: Record<string, unknown> = defaultPasswordInput,
	) {
	...
	beforeEach(async () => {
		vi.resetModules();
		({ runMatrixSetupBootstrapAfterConfigWrite } = await import(
			"./setup-bootstrap.js"
		));
	...
	it("bootstraps verification for newly added encrypted accounts", async () => {
		const { previousCfg, nextCfg, accountId, input } =
			applyDefaultAccountConfig();
		...
		expect(log).toHaveBeenCalledWith(
			'Matrix verification bootstrap: complete for "default".',
		);
	...
	it("does not bootstrap verification for already configured accounts", async () => {
		...
		expect(
			verificationMocks.bootstrapMatrixVerification,
