...
describe("matrix profile sync", () => {
	...
	it("uploads avatar media from a local path and then updates profile avatar", async () => {
		...
		expect(result.avatarUpdated).toBe(true);
		expect(loadAvatarFromPath).toHaveBeenCalledWith(
			"/tmp/avatar.jpg",
			10 * 1024 * 1024,
		);
		expect(client.setAvatarUrl).toHaveBeenCalledWith(
			"mxc://example/path-avatar",
		);
	});
	...
	it("recognizes supported avatar sources", () => {
		...
		expect(
			isSupportedMatrixAvatarSource("https://example.org/avatar.png"),
		).toBe(true);
		expect(isSupportedMatrixAvatarSource("http://example.org/avatar.png")).toBe(
			true,
		);
		expect(isSupportedMatrixAvatarSource("ftp://example.org/avatar.png")).toBe(
			false,
		);
	});
});
