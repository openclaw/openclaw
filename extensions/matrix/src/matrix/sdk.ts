...
import { normalizeNullableString, uniqueStrings } from "openclaw/plugin-sdk/string-coerce-runtime";
...
export class MatrixClient {
  ...
  async deleteOwnDevices(deviceIds: string[]): Promise<MatrixOwnDeviceDeleteResult> {
    const uniqueDeviceIds = uniqueStrings(deviceIds.map((value) => value.trim()).filter(Boolean));
    const currentDeviceId = this.client.getDeviceId()?.trim() || null;
