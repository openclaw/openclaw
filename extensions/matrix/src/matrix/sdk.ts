...
export type MatrixRecoveryKeyVerificationResult =
...
export class MatrixClient {
	...
	constructor(
		...
		this.syncStore = opts.storagePath
			? new FileBackedMatrixSyncStore(opts.storagePath)
			: undefined;
		...
		this.verificationManager.onSummaryChanged(
		...
		if (!initial.crossSigningPublished || initial.ownDeviceVerified === false) {
			...
			if (status.signedByOwner) {
				...
				try {
					...
					if (
						repaired.crossSigningPublished &&
						repaired.ownDeviceVerified !== false
					) {
						LogService.info(
							"MatrixClientLite",
	...
	private async initializeCryptoIfNeeded(): Promise<void> {
		...
		try {
			...
			LogService.warn(
				"MatrixClientLite",
				"Failed to initialize rust crypto:",
				err,
			);
		}
	}
	...
	async getUserId(): Promise<string> {
		...
		const whoami = (await this.doRequest(
			"GET",
	...
	async getAccountData(
		...
		return (
			(event?.getContent() as Record<string, unknown> | undefined) ?? undefined
		);
	}
	...
	private async runSerializedRoomSend<T>(
		roomId: string,
		task: () => Promise<T>,
	): Promise<T> {
	...
	async getEvent(
		...
		const rawEvent = (await this.client.fetchRoomEvent(
			roomId,
			eventId,
	...
	async getRelations(
		...
		const result = await this.client.relations(
			roomId,
			eventId,
			relationType,
			eventType,
		...
		return {
			originalEvent: result.originalEvent
				? matrixEventToRaw(result.originalEvent)
				: null,
	...
	async hydrateEvents(
		...
		await Promise.all(
			mappedEvents.map((event) => this.client.decryptEventIfNeeded(event)),
		);
	...
	async getRoomKeyBackupStatus(): Promise<MatrixRoomKeyBackupStatus> {
		...
		let { activeVersion, decryptionKeyCached } =
		...
		const shouldLoadBackupKey =
			...
			(decryptionKeyCached === false || matchesDecryptionKey === false);
	...
	async verifyWithRecoveryKey(
		...
		if (!crypto) {
			return await fail(
				"Matrix crypto is not available (start client with encryption enabled)",
			);
		}
		...
		try {
			...
			const backupError = resolveMatrixRoomKeyBackupReadinessError(
				status.backup,
				{
	...
	async restoreRoomKeyBackup(
		...
		try {
			...
			if (typeof crypto.restoreKeyBackup !== "function") {
				this.recoveryKeyStore.discardStagedRecoveryKey();
				return await fail(
					"Matrix crypto backend does not support full key backup restore",
				);
			}
	...
	async resetRoomKeyBackup(): Promise<MatrixRoomKeyBackupResetResult> {
		...
		try {
			...
			await this.recoveryKeyStore.bootstrapSecretStorageWithRecoveryKey(
				crypto,
				{
			...
			if (!createdVersion) {
				return await fail(
					"Matrix room key backup is still missing after reset.",
				);
			}
	...
	async getOwnCrossSigningPublicationStatus(): Promise<MatrixOwnCrossSigningPublicationStatus> {
		...
		try {
			...
			const selfSigningKeyPublished = Boolean(
				response.self_signing_keys?.[userId],
			);
			const userSigningKeyPublished = Boolean(
				response.user_signing_keys?.[userId],
			);
			return {
				userId,
				masterKeyPublished,
	...
	async bootstrapOwnDeviceVerification(params?: {
		...
		const error = success
			? undefined
			: (backupError ?? verificationError ?? undefined);
		return {
			success,
			error,
	...
	async listOwnDevices(): Promise<MatrixOwnDeviceInfo[]> {
		...
		return entries.map((device) => ({
			...
			lastSeenTs:
				typeof device.last_seen_ts === "number" &&
				Number.isFinite(device.last_seen_ts)
					? device.last_seen_ts
					: null,
	...
	async deleteOwnDevices(
		deviceIds: string[],
	): Promise<MatrixOwnDeviceDeleteResult> {
		const uniqueDeviceIds = [
			...new Set(deviceIds.map((value) => value.trim()).filter(Boolean)),
		];
		const currentDeviceId = this.client.getDeviceId()?.trim() || null;
		const protectedDeviceIds = uniqueDeviceIds.filter(
			(deviceId) => deviceId === currentDeviceId,
		);
		if (protectedDeviceIds.length > 0) {
			throw new Error(
				`Refusing to delete the current Matrix device: ${protectedDeviceIds[0]}`,
			);
		}
		...
		const deleteWithAuth = async (
			...
			await this.client.deleteMultipleDevices(
				uniqueDeviceIds,
				authData as never,
			);
		};
	...
	private async resolveRoomKeyBackupTrustState(
		...
		if (typeof crypto.getKeyBackupInfo === "function") {
			...
			if (info && typeof crypto.isKeyBackupTrusted === "function") {
				const trustInfo = await crypto
					.isKeyBackupTrusted(info)
					.catch(() => null);
				trusted =
					typeof trustInfo?.trusted === "boolean" ? trustInfo.trusted : null;
				matchesDecryptionKey =
	...
	private async ensureRoomKeyBackupEnabled(
		crypto: MatrixCryptoBootstrapApi,
	): Promise<void> {
		...
		LogService.info(
			"MatrixClientLite",
			`Room key backup enabled (version ${createdVersion})`,
		);
	}
	...
	private registerBridge(): void {
		...
		this.client.on(ClientEvent.Event, (event: MatrixEvent) => {
			...
			if (isEncryptedEvent) {
				...
				if (
					this.decryptBridge.shouldEmitUnencryptedMessage(roomId, raw.event_id)
				) {
	...
	private emitMembershipForRoom(room: unknown): void {
		...
		const membership =
			roomObj.getMyMembership?.() ?? roomObj.selfMembership ?? undefined;
		const selfUserId = this.client.getUserId() ?? this.selfUserId ?? "";
		if (!selfUserId) {
			return;
		}
