...
export async function pruneMatrixStaleGatewayDevices(
	...
	return await withStartedActionClient(opts, async (client) => {
		...
		const staleGatewayDeviceIds = health.staleOpenClawDevices.map(
		...
		const deleted =
			staleGatewayDeviceIds.length > 0
				...
				: {
						currentDeviceId:
							devices.find((device) => device.current)?.deviceId ?? null,
