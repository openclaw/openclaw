...
function requireCrypto(
	...
	if (!client.crypto) {
		const cfg =
			opts.cfg ?? (getMatrixRuntime().config.loadConfig() as CoreConfig);
		throw new Error(
			formatMatrixEncryptionUnavailableError(cfg, opts.accountId),
		);
	}
...
export async function listMatrixVerifications(
	opts: MatrixActionClientOpts = {},
) {
...
export async function requestMatrixVerification(
	...
	return await withStartedActionClient(params, async (client) => {
		...
		const ownUser =
			params.ownUser ?? (!params.userId && !params.deviceId && !params.roomId);
...
export async function startMatrixVerification(
	...
	return await withStartedActionClient(opts, async (client) => {
		const crypto = requireCrypto(client, opts);
		return await crypto.startVerification(
			resolveVerificationId(requestId),
			opts.method ?? "sas",
		);
	});
}
...
export async function generateMatrixVerificationQr(
	...
	return await withStartedActionClient(opts, async (client) => {
		const crypto = requireCrypto(client, opts);
		return await crypto.generateVerificationQr(
			resolveVerificationId(requestId),
		);
	});
}
...
export async function scanMatrixVerificationQr(
	...
	return await withStartedActionClient(opts, async (client) => {
		...
		return await crypto.scanVerificationQr(
			resolveVerificationId(requestId),
			payload,
		);
	});
...
export async function confirmMatrixVerificationSas(
	...
	return await withStartedActionClient(opts, async (client) => {
		const crypto = requireCrypto(client, opts);
		return await crypto.confirmVerificationSas(
			resolveVerificationId(requestId),
		);
	});
}
...
export async function mismatchMatrixVerificationSas(
	...
	return await withStartedActionClient(opts, async (client) => {
		const crypto = requireCrypto(client, opts);
		return await crypto.mismatchVerificationSas(
			resolveVerificationId(requestId),
		);
	});
}
...
export async function confirmMatrixVerificationReciprocateQr(
	...
	return await withStartedActionClient(opts, async (client) => {
		const crypto = requireCrypto(client, opts);
		return await crypto.confirmVerificationReciprocateQr(
			resolveVerificationId(requestId),
		);
	});
}
...
export async function getMatrixVerificationStatus(
	...
	return await withStartedActionClient(opts, async (client) => {
		...
		const payload = {
			...status,
			pendingVerifications: client.crypto
				? (await client.crypto.listVerifications()).length
				: 0,
		};
		if (!opts.includeRecoveryKey) {
			return payload;
		}
		const recoveryKey = client.crypto
			? await client.crypto.getRecoveryKey()
			: null;
		return {
			...payload,
			recoveryKey: recoveryKey?.encodedPrivateKey ?? null,
...
export async function getMatrixRoomKeyBackupStatus(
	opts: MatrixActionClientOpts = {},
) {
...
export async function resetMatrixRoomKeyBackup(
	...
	return await withStartedActionClient(
		opts,
		async (client) => await client.resetRoomKeyBackup(),
	);
}
