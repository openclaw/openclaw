...
vi.mock("../profile.js", () => ({
	syncMatrixOwnProfile: (...args: unknown[]) =>
		syncMatrixOwnProfileMock(...args),
}));

vi.mock("./client.js", () => ({
	withResolvedActionClient: (...args: unknown[]) =>
		withResolvedActionClientMock(...args),
}));
...
describe("matrix profile actions", () => {
	...
	it("bridges avatar loaders through Matrix runtime media helpers", async () => {
		...
		expect(loadWebMediaMock).toHaveBeenNthCalledWith(
			1,
