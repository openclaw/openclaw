...
function resolveSelectedAnswerIds(params: {
	...
	const selectedByIndex = normalizeOptionIndexes(
		params.optionIndexes ?? [],
	).map((index) => {
	...
	if (answerIds.length === 0) {
		throw new Error(
			"Matrix poll vote requires at least one poll option id or index.",
		);
	}
	...
	const answerMap = new Map(
		parsed.answers.map((answer) => [answer.id, answer.text] as const),
	);
...
export async function voteMatrixPoll(
	...
	return await withResolvedRoomAction(
		...
		async (client, resolvedRoom) => {
			...
			const eventType =
				typeof pollEvent.type === "string" ? pollEvent.type : "";
