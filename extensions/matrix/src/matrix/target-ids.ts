...
export function normalizeMatrixDirectoryUserId(
	raw: string,
): string | undefined {
	...
	return isMatrixQualifiedUserId(normalized)
		? `user:${normalized}`
		: normalized;
}

export function normalizeMatrixDirectoryGroupId(
	raw: string,
): string | undefined {
	const normalized = stripKnownPrefixes(raw, [MATRIX_PREFIX]);
	if (!normalized || normalized === "*") {
...
export function resolveMatrixDirectUserId(params: {
	...
	const userId = stripKnownPrefixes(params.from ?? "", [
		MATRIX_PREFIX,
		USER_PREFIX,
	]);
