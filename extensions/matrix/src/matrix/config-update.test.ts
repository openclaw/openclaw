...
describe("updateMatrixAccountConfig", () => {
	it("resolves account-aware Matrix config field paths", () => {
		expect(
			resolveMatrixConfigFieldPath({} as CoreConfig, "default", "dm.policy"),
		).toBe("channels.matrix.dm.policy");
	...
	it("supports explicit null clears and boolean false values", () => {
		...
		expect(
			updated.channels?.matrix?.accounts?.default?.password,
		).toBeUndefined();
	...
	it("stores and clears Matrix allowBots, allowPrivateNetwork, and proxy settings", () => {
		...
		expect(
			updated.channels?.matrix?.accounts?.default?.allowPrivateNetwork,
		).toBeUndefined();
