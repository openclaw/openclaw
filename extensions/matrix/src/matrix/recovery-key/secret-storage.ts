...
export async function fetchSecretStorageMetadata(
  ...
  try {
    ...
    throw new Error(ERROR_MESSAGES.SECRET_STORAGE_NOT_CONFIGURED, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(ERROR_MESSAGES.SECRET_STORAGE_KEY_NOT_FOUND, { cause: error });
  }
...
export function decryptSecret(
  ...
  // Compute HMAC-SHA256 on ciphertext only (Matrix spec v1.11 section 11.13.2.1)
  // Spec: https://spec.matrix.org/v1.11/client-server-api/#msecret_storagev1aes-hmac-sha2
  // Note: MAC is computed on ciphertext alone, not iv + ciphertext
...
export async function fetchCrossSigningKeys(
  ...
  try {
    ...
    throw new Error(ERROR_MESSAGES.CROSS_SIGNING_NOT_CONFIGURED, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(ERROR_MESSAGES.SELF_SIGNING_KEY_MISSING, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(ERROR_MESSAGES.USER_SIGNING_KEY_MISSING, { cause: error });
  }
