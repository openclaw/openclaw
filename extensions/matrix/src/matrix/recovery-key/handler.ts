  ...
  async verifyWithRecoveryKey(key: string): Promise<VerificationResult> {
    try {
      ...
      if (!deviceId || typeof deviceId !== "string") {
        throw new Error("Device ID not available from crypto client");
      }

      // Use SHA-256 hash instead of HMAC to avoid timing attack surface
      const keyHash = crypto
        .createHash("sha256")
        .update(Buffer.from(recoveryKey))
        .update(Buffer.from(deviceId))
        .digest("hex");
      ...
      try {
        ...
        if (backupInfo) {
          ...
          try {
            ...
            backupKey = await getBackupDecryptionKey(this.client, recoveryKey);
            ...
            // Always zero out backup key, even on error
            if (backupKey) {
              backupKey.fill(0);
            }
          }
