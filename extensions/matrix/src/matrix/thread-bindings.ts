...
async function loadBindingsFromDisk(filePath: string, accountId: string) {
	const { value } =
		await readJsonFileWithFallback<StoredMatrixThreadBindingState | null>(
			filePath,
			null,
		);
	...
	for (const entry of value.bindings) {
		...
		loaded.push({
			...
			idleTimeoutMs:
				typeof entry?.idleTimeoutMs === "number" &&
				Number.isFinite(entry.idleTimeoutMs)
...
export async function createMatrixThreadBindingManager(params: {
	...
	const updateBindingsBySessionKey = (input: {
		targetSessionKey: string;
		update: (
			entry: MatrixThreadBindingRecord,
			now: number,
	...
	const manager: MatrixThreadBindingManager = {
		...
		getByConversation: ({ conversationId, parentConversationId }) =>
			listBindingsForAccount(params.accountId).find((entry) => {
				...
				return (
					(entry.parentConversationId ?? "") === parentConversationId.trim()
				);
			}),
		...
		stop: () => {
			...
			if (
				getMatrixThreadBindingManagerEntry(params.accountId)?.manager ===
				manager
			) {
	...
	const sendFarewellMessages = async (
		...
		reason:
			| string
			| ((record: MatrixThreadBindingRecord) => string | undefined),
	) => {
		await Promise.all(
	...
	const sessionBindingAdapter: SessionBindingAdapter = {
		...
		bind: async (input) => {
			...
			if (input.placement === "current" && introText) {
				...
				const threadId =
					boundParentConversationId &&
					boundParentConversationId !== boundConversationId
						? boundConversationId
						: undefined;
				await sendBindingMessage({
					client: params.client,
	...
	if (params.enableSweeper !== false) {
		sweepTimer = setInterval(() => {
			...
			const reasonByBindingKey = new Map(
				expired.map(({ record, lifecycle }) => [
					resolveBindingKey(record),
					lifecycle.reason,
				]),
			);
			void (async () => {
