...
function createMatrixJsClientStub(): MatrixJsClientStub {
	...
	client.getRoomIdForAlias = vi.fn(async () => ({
	...
	client.uploadContent = vi.fn(async () => ({
...
vi.mock("matrix-js-sdk/lib/matrix.js", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("matrix-js-sdk/lib/matrix.js")>();
	return {
		...actual,
...
describe("MatrixClient request hardening", () => {
	...
	it("blocks absolute endpoints unless explicitly allowed", async () => {
		...
		await expect(
			client.doRequest("GET", "https://matrix.example.org/start"),
		).rejects.toThrow("Absolute Matrix endpoint is blocked by default");
	...
	it("prefers authenticated client media downloads", async () => {
		...
		await expect(
			client.downloadContent("mxc://example.org/media"),
		).resolves.toEqual(payload);

		expect(fetchMock).toHaveBeenCalledTimes(1);
		const firstUrl = String(
			(fetchMock.mock.calls as unknown[][])[0]?.[0] ?? "",
		);
		expect(firstUrl).toContain(
			"/_matrix/client/v1/media/download/example.org/media",
		);
	});
	...
	it("falls back to legacy media downloads for older homeservers", async () => {
		...
		const secondUrl = String(
			(fetchMock.mock.calls as unknown[][])[1]?.[0] ?? "",
		);
		expect(firstUrl).toContain(
			"/_matrix/client/v1/media/download/example.org/media",
		);
	...
	it("maps relations pages back to raw events", async () => {
		...
		const page = await client.getRelations(
			"!room:example.org",
			"$poll",
			"m.reference",
		);

		expect(page.originalEvent).toMatchObject({
			event_id: "$poll",
			type: "m.poll.start",
		});
	...
	it("aborts requests after timeout", async () => {
		...
		const pending = client.doRequest(
			"GET",
	...
	it("wires the sync store into the SDK and flushes it on shutdown", async () => {
		const tempDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "openclaw-matrix-sdk-store-"),
		);
...
describe("MatrixClient crypto bootstrapping", () => {
	...
	it("verifies with a provided recovery key and reports success", async () => {
		...
		const recoveryDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "matrix-sdk-verify-key-"),
		);
		const client = new MatrixClient("https://matrix.example.org", "token", {
			encryption: true,
	...
	it("fails recovery-key verification when the device is only locally trusted", async () => {
		...
		const recoveryDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "matrix-sdk-verify-local-only-"),
		);
		const client = new MatrixClient("https://matrix.example.org", "token", {
			encryption: true,
			recoveryKeyPath: path.join(recoveryDir, "recovery-key.json"),
	...
	it("fails recovery-key verification when backup remains untrusted after device verification", async () => {
		...
		const recoveryDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "matrix-sdk-verify-untrusted-"),
		);
	...
	it("does not overwrite the stored recovery key when recovery-key verification fails", async () => {
		...
		const recoveryDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "matrix-sdk-verify-preserve-"),
		);
		const recoveryKeyPath = path.join(recoveryDir, "recovery-key.json");
		fs.writeFileSync(
			recoveryKeyPath,
		...
		const result = await client.verifyWithRecoveryKey(
			attemptedEncoded as string,
		);
	...
	it("tries loading backup keys from secret storage when key is missing from cache", async () => {
		...
		expect(loadSessionBackupPrivateKeyFromSecretStorage).toHaveBeenCalledTimes(
			1,
		);
	});
	...
	it("reports why backup key loading failed during status checks", async () => {
		...
		expect(backup.keyLoadError).toContain(
			"secret storage key is not available",
		);
	...
	it("resets the current room-key backup and creates a fresh trusted version", async () => {
		...
		vi.spyOn(client, "doRequest").mockImplementation(
			async (method, endpoint) => {
				if (
					method === "GET" &&
					String(endpoint).includes("/room_keys/version")
				) {
					return { version: "21868" };
				}
				if (
					method === "DELETE" &&
					String(endpoint).includes("/room_keys/version/21868")
				) {
					return {};
				}
				return {};
	...
	it("reloads the new backup decryption key after reset when the old cached key mismatches", async () => {
		...
		vi.spyOn(client, "doRequest").mockImplementation(
			async (method, endpoint) => {
				...
				if (
					method === "DELETE" &&
					String(endpoint).includes("/room_keys/version/22245")
				) {
					return {};
				}
				return {};
	...
	it("fails reset when the recreated backup still does not match the local decryption key", async () => {
		...
		expect(result.success).toBe(false);
		expect(result.error).toContain(
			"does not have the matching backup decryption key",
		);
	...
	it("does not recreate key backup during bootstrap when one already exists", async () => {
		...
		const bootstrapSecretStorageCalls = bootstrapSecretStorage.mock
			.calls as Array<unknown[]>;
		expect(
			bootstrapSecretStorageCalls.some((call) =>
				Boolean(
					(call[0] as { setupNewKeyBackup?: boolean })?.setupNewKeyBackup,
				),
			),
		).toBe(false);
	});
	...
	it("does not report bootstrap errors when final verification state is healthy", async () => {
		const encoded = encodeRecoveryKey(
			new Uint8Array(Array.from({ length: 32 }, (_, i) => i + 90)),
		);
		matrixJsClient.getUserId = vi.fn(() => "@bot:example.org");
		matrixJsClient.getDeviceId = vi.fn(() => "DEVICE123");
		matrixJsClient.getCrypto = vi.fn(() => ({
