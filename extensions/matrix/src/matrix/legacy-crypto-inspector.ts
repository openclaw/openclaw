...
function resolveLegacyMachineStorePath(params: {
	...
	if (
		fs.existsSync(path.join(params.cryptoRootDir, "matrix-sdk-crypto.sqlite3"))
	) {
...
export async function inspectLegacyMatrixCryptoStore(params: {
	...
	if (!machineStorePath) {
		throw new Error(
			`Matrix legacy crypto store not found for device ${params.deviceId}`,
		);
	}
	...
	try {
		...
		return {
			...
			roomKeyCounts: roomKeyCounts
				? {
						total:
							typeof roomKeyCounts.total === "number" ? roomKeyCounts.total : 0,
						backedUp:
							typeof roomKeyCounts.backedUp === "number"
								? roomKeyCounts.backedUp
								: 0,
					}
				: null,
			backupVersion:
				typeof backupKeys?.backupVersion === "string" &&
				backupKeys.backupVersion.trim()
					? backupKeys.backupVersion
					: null,
			decryptionKeyBase64:
				typeof backupKeys?.decryptionKeyBase64 === "string" &&
				backupKeys.decryptionKeyBase64.trim()
