...
export function parsePollStart(
	content: PollStartContent,
): ParsedPollStart | null {
	const poll =
...
export function resolvePollReferenceEventId(content: unknown): string | null {
	...
	const relates = (content as { "m.relates_to"?: { event_id?: unknown } })[
		"m.relates_to"
	];
...
export function parsePollResponseAnswerIds(content: unknown): string[] | null {
	...
	const response =
		(content as Record<string, PollResponseSubtype | undefined>)[
			M_POLL_RESPONSE
		] ??
		(content as Record<string, PollResponseSubtype | undefined>)[
			ORG_POLL_RESPONSE
...
export function buildPollResultsSummary(params: {
	...
	const orderedRelationEvents = [...params.relationEvents].sort(
		(left, right) => {
			const leftTs =
				typeof left.origin_server_ts === "number" &&
				Number.isFinite(left.origin_server_ts)
					? left.origin_server_ts
					: Number.POSITIVE_INFINITY;
			const rightTs =
				typeof right.origin_server_ts === "number" &&
				Number.isFinite(right.origin_server_ts)
...
export function formatPollResultsAsText(summary: PollResultsSummary): string {
	...
	for (const [index, entry] of summary.entries.entries()) {
		...
		lines.push(
			`${index + 1}. ${entry.text} (${entry.votes} vote${entry.votes === 1 ? "" : "s"})`,
		);
	}
	lines.push("");
	if (!revealResults) {
