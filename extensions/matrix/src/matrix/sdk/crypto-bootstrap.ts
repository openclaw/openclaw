...
export class MatrixCryptoBootstrapper<TRawEvent extends MatrixRawEvent> {
	...
	async bootstrap(
		...
		const crossSigning = await this.bootstrapCrossSigning(crypto, {
			...
			allowAutomaticCrossSigningReset:
				options.allowAutomaticCrossSigningReset !== false,
			allowSecretStorageRecreateWithoutRecoveryKey:
				options.allowSecretStorageRecreateWithoutRecoveryKey === true,
			strict,
		});
	...
	private async bootstrapCrossSigning(
		...
		if (options.forceResetCrossSigning) {
			try {
				...
				LogService.warn(
					"MatrixClientLite",
					"Forced cross-signing reset failed:",
					err,
				);
				if (options.strict) {
		...
		try {
			...
			if (shouldRepairSecretStorage) {
				...
				try {
					...
					if (options.strict) {
						throw resetErr instanceof Error
							? resetErr
		...
		try {
			...
			LogService.warn(
				"MatrixClientLite",
				"Fallback cross-signing bootstrap failed:",
				err,
			);
			if (options.strict) {
				throw err instanceof Error ? err : new Error(String(err));
	...
	private async bootstrapSecretStorage(
		...
		try {
			...
			LogService.warn(
				"MatrixClientLite",
				"Failed to bootstrap secret storage:",
				err,
			);
			if (options.strict) {
				throw err instanceof Error ? err : new Error(String(err));
	...
	private registerVerificationRequestHandler(
		...
		crypto.on(CryptoEvent.VerificationRequestReceived, async (request) => {
			...
			try {
				this.deps.verificationManager.trackVerificationRequest(
					verificationRequest,
				);
			} catch (err) {
				LogService.warn(
		...
		LogService.info(
			"MatrixClientLite",
			"Verification request handler registered",
		);
	}
	...
	private async ensureOwnDeviceTrust(
		...
		if (!verified && strict) {
			throw new Error(
				`Matrix own device ${deviceId} is not verified by its owner after bootstrap`,
			);
		}
		return verified;
	}
