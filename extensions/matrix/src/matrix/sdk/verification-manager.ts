...
type MatrixVerificationSummaryListener = (
...
export class MatrixVerificationManager {
	...
	private readonly summaryListeners =
		new Set<MatrixVerificationSummaryListener>();
	...
	private pruneVerificationSessions(nowMs: number): void {
		...
		const overflow =
			this.verificationSessions.size - MAX_TRACKED_VERIFICATION_SESSIONS;
	...
	private buildVerificationSummary(
		...
		const accepting = this.readRequestValue(
			request,
			() => request.accepting,
			false,
		);
		const declining = this.readRequestValue(
			request,
			() => request.declining,
			false,
		);
		const pending = this.readRequestValue(
			request,
			() => request.pending,
			false,
		);
		...
		const methods = Array.isArray(methodsRaw)
			...
			: [];
		const sasCallbacks =
			session.sasCallbacks ?? session.activeVerifier?.getShowSasCallbacks();
		if (sasCallbacks) {
			session.sasCallbacks = sasCallbacks;
		}
		const canAccept =
			phase < VerificationPhase.Ready && !accepting && !declining;
		return {
			id: session.id,
			transactionId: this.readRequestValue(
				request,
				() => request.transactionId,
				undefined,
			),
			roomId: this.readRequestValue(request, () => request.roomId, undefined),
			otherUserId: this.readRequestValue(
				request,
				() => request.otherUserId,
				"unknown",
			),
			otherDeviceId: this.readRequestValue(
				request,
				() => request.otherDeviceId,
				undefined,
			),
			isSelfVerification: this.readRequestValue(
				request,
				() => request.isSelfVerification,
				false,
			),
			initiatedByMe: this.readRequestValue(
				request,
			...
			pending,
			methods,
			chosenMethod: this.readRequestValue(
				request,
				() => request.chosenMethod ?? null,
				null,
			),
			canAccept,
	...
	private ensureVerificationRequestTracked(
		...
		session.request.on(VerificationRequestEvent.Change, () => {
			...
			const verifier = this.readRequestValue(
				session.request,
				() => session.request.verifier,
				null,
			);
			if (verifier) {
	...
	private maybeAutoAcceptInboundRequest(
		...
		const initiatedByMe = this.readRequestValue(
			request,
			() => request.initiatedByMe,
			false,
		);
	...
	private maybeAutoStartInboundSas(session: MatrixVerificationSession): void {
		if (
			session.activeVerifier ||
			session.verifyStarted ||
			session.startRequested
		) {
			return;
		}
		if (
			this.readRequestValue(
				session.request,
				() => session.request.initiatedByMe,
				true,
			)
		) {
			return;
		...
		if (
			!this.readRequestValue(
				session.request,
				() => session.request.isSelfVerification,
				false,
			)
		) {
			return;
		...
		const supportsSas =
			methods.includes(VerificationMethod.Sas) ||
	...
	listVerifications(): MatrixVerificationSummary[] {
		this.pruneVerificationSessions(Date.now());
		const summaries = Array.from(this.verificationSessions.values()).map(
			(session) => this.buildVerificationSummary(session),
		);
	...
	async requestVerification(
		...
		if (params.ownUser) {
			...
			request = await crypto.requestDeviceVerification(
				params.userId,
				params.deviceId,
			);
		} else if (params.userId && params.roomId && crypto.requestVerificationDM) {
			request = await crypto.requestVerificationDM(
				params.userId,
				params.roomId,
			);
		} else {
	...
	async startVerification(
		...
		if (method !== "sas") {
			throw new Error(
				"Matrix startVerification currently supports only SAS directly",
			);
		}
		const verifier = await session.request.startVerification(
	...
	async scanVerificationQr(
		...
		const verifier = await session.request.scanQRCode(
			new Uint8ClampedArray(qrBytes),
		);
		this.attachVerifierToVerificationSession(session, verifier);
		this.ensureVerificationStarted(session);
		return this.buildVerificationSummary(session);
	...
	async confirmVerificationSas(id: string): Promise<MatrixVerificationSummary> {
		...
		if (!callbacks) {
			throw new Error(
				"Matrix SAS confirmation is not available for this verification request",
			);
		}
		this.clearSasAutoConfirmTimer(session);
		session.sasCallbacks = callbacks;
	...
	mismatchVerificationSas(id: string): MatrixVerificationSummary {
		...
		if (!callbacks) {
			throw new Error(
				"Matrix SAS mismatch is not available for this verification request",
			);
		}
		this.clearSasAutoConfirmTimer(session);
		session.sasCallbacks = callbacks;
	...
	confirmVerificationReciprocateQr(id: string): MatrixVerificationSummary {
		const session = this.findVerificationSession(id);
		const callbacks =
			session.reciprocateQrCallbacks ??
			session.activeVerifier?.getReciprocateQrCodeCallbacks();
		if (!callbacks) {
			throw new Error(
	...
	getVerificationSas(id: string): {
		...
		if (!callbacks) {
			throw new Error(
				"Matrix SAS data is not available for this verification request",
			);
		}
		session.sasCallbacks = callbacks;
		return {
