...
function resolveDecryptRetryKey(
	roomId: string,
	eventId: string,
): string | null {
	if (!roomId || !eventId) {
...
function isDecryptionFailure(event: MatrixEvent): boolean {
	return (
		typeof (event as { isDecryptionFailure?: () => boolean })
...
export class MatrixDecryptBridge<TRawEvent extends DecryptBridgeRawEvent> {
	...
	attachEncryptedEvent(event: MatrixEvent, roomId: string): void {
		...
		event.on(
			...
			(decryptedEvent: MatrixEvent, err?: Error) => {
	...
	retryPendingNow(reason: string): void {
		...
		LogService.debug(
			"MatrixClientLite",
			`Retrying pending decryptions due to ${reason}`,
		);
	...
	bindCryptoRetrySignals(
		crypto: MatrixCryptoRetrySignalSource | undefined,
	): void {
	...
	async drainPendingDecryptions(reason: string): Promise<void> {
		for (
			let attempts = 0;
			attempts < MATRIX_DECRYPT_RETRY_MAX_ATTEMPTS;
			attempts += 1
		) {
			...
			const hasPendingRetryTimers = Array.from(
				this.decryptRetries.values(),
			).some((state) => state.timer || state.inFlight);
	...
	private handleEncryptedEventDecrypted(params: {
		...
		const retryEventId =
			decryptedRaw.event_id || params.encryptedEvent.getId() || "";
	...
	private async runDecryptRetry(retryKey: string): Promise<void> {
		...
		const canDecrypt =
			typeof this.deps.client.decryptEventIfNeeded === "function";
		if (!canDecrypt) {
			this.clearDecryptRetry(retryKey);
