...
type MatrixCryptoFacadeClient = {
	getRoom: (
		roomId: string,
	) => { hasEncryptionStateEvent: () => boolean } | null;
	getCrypto: () => unknown;
};
...
export type MatrixCryptoFacade = {
	...
	confirmVerificationReciprocateQr: (
		id: string,
	) => Promise<MatrixVerificationSummary>;
...
let matrixCryptoNodeRuntimePromise: Promise<MatrixCryptoNodeRuntime> | null =
	null;
...
export function createMatrixCryptoFacade(deps: {
	...
	return {
		...
		isRoomEncrypted: async (roomId: string): Promise<boolean> => {
			...
			try {
				const event = await deps.getRoomStateEvent(
					roomId,
					"m.room.encryption",
					"",
				);
				return (
					typeof event.algorithm === "string" && event.algorithm.length > 0
				);
			} catch {
				return false;
			}
		...
		requestOwnUserVerification: async (): Promise<unknown | null> => {
			const crypto = deps.client.getCrypto() as
				| MatrixVerificationCryptoApi
				| undefined;
		...
		encryptMedia: async (
			...
			if (!mediaInfoJson) {
				throw new Error(
					"Matrix media encryption failed: missing media encryption info",
				);
			}
		...
		decryptMedia: async (
			...
			const { Attachment, EncryptedAttachment } =
				await loadMatrixCryptoNodeRuntime();
		...
		scanVerificationQr: async (id, qrDataBase64) => {
			return await deps.verificationManager.scanVerificationQr(
				id,
				qrDataBase64,
			);
		},
