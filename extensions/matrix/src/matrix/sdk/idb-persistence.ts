...
function isValidIdbIndexSnapshot(
	value: unknown,
): value is IdbStoreSnapshot["indexes"][number] {
...
function isValidIdbRecordSnapshot(
	value: unknown,
): value is IdbStoreSnapshot["records"][number] {
	if (!value || typeof value !== "object") {
		return false;
	}
...
function isValidIdbDatabaseSnapshot(
	value: unknown,
): value is IdbDatabaseSnapshot {
	if (!value || typeof value !== "object") {
		return false;
	}
...
async function dumpIndexedDatabases(
	databasePrefix?: string,
): Promise<IdbDatabaseSnapshot[]> {
