...
function expectSecretStorageRepairRetry(
	...
	expect(
		deps.recoveryKeyStore.bootstrapSecretStorageWithRecoveryKey,
	).toHaveBeenCalledWith(crypto, {
		allowSecretStorageRecreateWithoutRecoveryKey: true,
...
describe("MatrixCryptoBootstrapper", () => {
	...
	it("bootstraps cross-signing/secret-storage and binds decrypt retry signals", async () => {
		...
		expect(deps.decryptBridge.bindCryptoRetrySignals).toHaveBeenCalledWith(
			crypto,
		);
	});
	...
	it("recreates secret storage and retries cross-signing when explicit bootstrap hits a stale server key", async () => {
		const { deps, crypto, bootstrapCrossSigning } =
			await runExplicitSecretStorageRepairScenario(
	...
	it("fails in strict mode when cross-signing keys are still unpublished", async () => {
		...
		await expect(
			bootstrapper.bootstrap(crypto, { strict: true }),
		).rejects.toThrow(
	...
	it("uses password UIA fallback when null and dummy auth fail", async () => {
		...
		const bootstrapCrossSigningCalls = bootstrapCrossSigning.mock
			.calls as Array<
			[
				{
	...
	it("tracks incoming verification requests from other users", async () => {
		...
		expect(
			deps.verificationManager.trackVerificationRequest,
		).toHaveBeenCalledWith(verificationRequest);
