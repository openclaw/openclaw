			...
			getSecretStorageKey: async ({ keys }) => {
				...
				if (staged?.privateKeyBase64) {
					const privateKey = new Uint8Array(
						Buffer.from(staged.privateKeyBase64, "base64"),
					);
	...
	async bootstrapSecretStorageWithRecoveryKey(
		...
		if (typeof getSecretStorageStatus === "function") {
			try {
				...
				LogService.warn(
					"MatrixClientLite",
					"Failed to read secret storage status:",
					err,
				);
			}
		}
		...
		const hasKnownInvalidSecrets = Object.values(
			status?.secretStorageKeyValidityMap ?? {},
		...
		let recoveryKey: MatrixGeneratedSecretStorageKey | null = sourceRecovery
			? {
					...
					privateKey: new Uint8Array(
						Buffer.from(sourceRecovery.privateKeyBase64, "base64"),
					),
		...
		if (recoveryKey && status?.defaultKeyId) {
			...
			if (
				!stagedRecovery &&
				storedRecovery &&
				storedRecovery.keyId !== defaultKeyId
			) {
				this.saveRecoveryKeyToDisk({
					keyId: defaultKeyId,
		...
		const ensureRecoveryKey =
			async (): Promise<MatrixGeneratedSecretStorageKey> => {
				if (recoveryKey) {
					return recoveryKey;
				}
	...
	private loadStoredRecoveryKey(): MatrixStoredRecoveryKey | null {
		...
		try {
			...
			return {
				...
				encodedPrivateKey:
					typeof parsed.encodedPrivateKey === "string"
						? parsed.encodedPrivateKey
						: undefined,
				...
				keyInfo:
					parsed.keyInfo && typeof parsed.keyInfo === "object"
						? {
								...
								name:
									typeof parsed.keyInfo.name === "string"
										? parsed.keyInfo.name
										: undefined,
							}
						: undefined,
