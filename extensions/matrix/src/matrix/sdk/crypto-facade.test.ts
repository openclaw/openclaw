...
function createFacadeHarness(params?: {
	...
	recoveryKeySummary?: ReturnType<
		MatrixRecoveryKeyStore["getRecoveryKeySummary"]
	>;
	...
	const facade = createMatrixCryptoFacade({
		...
		verificationManager: createVerificationManagerMock(
			params?.verificationManager,
		),
		recoveryKeyStore: createRecoveryKeyStoreMock(
			params?.recoveryKeySummary ?? null,
		),
		getRoomStateEvent,
		downloadContent,
	});
...
describe("createMatrixCryptoFacade", () => {
	it("detects encrypted rooms from cached room state", async () => {
		...
		await expect(facade.isRoomEncrypted("!room:example.org")).resolves.toBe(
			true,
		);
	});
	...
	it("falls back to server room state when room cache has no encryption event", async () => {
		...
		expect(getRoomStateEvent).toHaveBeenCalledWith(
			"!room:example.org",
			"m.room.encryption",
			"",
		);
	...
	it("forwards verification requests and uses client crypto API", async () => {
		...
		await expect(facade.getRecoveryKey()).resolves.toMatchObject({
			keyId: "KEY",
		});
	});
