	...
	return await new Promise((resolve, reject) => {
		...
		req.onsuccess = () => {
			...
			const maybeResolve = () => {
				...
				resolve(
					keys.map((key, index) => ({ key, value: resolvedValues[index] })),
				);
			};
...
describe("Matrix IndexedDB persistence", () => {
	...
	it("persists and restores database contents for the selected prefix", async () => {
		...
		expect(restoredRecords).toEqual([
			{ key: "room-1", value: { session: "abc123" } },
		]);

		const dbs = await indexedDB.databases();
		expect(
			dbs.some((entry) => entry.name === "other-prefix::matrix-sdk-crypto"),
		).toBe(false);
	});
	...
	it("returns false and logs a warning for malformed snapshots", async () => {
		...
		expect(warnSpy).toHaveBeenCalledWith(
			...
			expect.stringContaining(
				`Failed to restore IndexedDB snapshot from ${snapshotPath}:`,
			),
			expect.any(Error),
		);
	});
