...
function cloneMatrixRoomMap(
	...
	return Object.fromEntries(
		Object.entries(rooms).map(([roomId, roomCfg]) => [
			roomId,
			roomCfg ? { ...roomCfg } : roomCfg,
		]),
	);
}
...
export function shouldStoreMatrixAccountAtTopLevel(
	cfg: CoreConfig,
	accountId: string,
): boolean {
...
export function resolveMatrixConfigPath(
	cfg: CoreConfig,
	accountId: string,
): string {
	const normalizedAccountId = normalizeAccountId(accountId);
...
export function updateMatrixAccountConfig(
	...
	if (patch.initialSyncLimit !== undefined) {
		if (patch.initialSyncLimit === null) {
			...
			nextAccount.initialSyncLimit = Math.max(
				0,
				Math.floor(patch.initialSyncLimit),
			);
		}
	}
