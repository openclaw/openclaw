...
async function readMatrixDirectAccountData(
	client: MatrixClient,
): Promise<MatrixDirectAccountData> {
	try {
		const direct = (await client.getAccountData(
			EventType.Direct,
		)) as MatrixDirectAccountData;
...
function normalizeRemoteUserId(remoteUserId: string): string {
	...
	if (!isMatrixQualifiedUserId(normalized)) {
		throw new Error(
			`Matrix user IDs must be fully qualified (got "${remoteUserId}")`,
		);
	}
	return normalized;
}

function normalizeMappedRoomIds(
	direct: MatrixDirectAccountData,
	remoteUserId: string,
): string[] {
...
function buildNextDirectContent(params: {
	...
	const current = normalizeMappedRoomIds(
...
export async function inspectMatrixDirectRooms(params: {
	...
	const remoteUserId = normalizeRemoteUserId(params.remoteUserId);
	const selfUserId =
		(await params.client.getUserId().catch(() => null))?.trim() || null;
	const directContent = await readMatrixDirectAccountData(params.client);
	...
	const discoveredStrictRoomIds = discoveredStrictRooms.map(
		(room) => room.roomId,
	);
	const discoveredExplicit = discoveredStrictRooms.find(
		(room) => room.explicit,
	);
	...
	return {
		...
		activeRoomId:
			mappedStrict?.roomId ??
			discoveredExplicit?.roomId ??
			discoveredStrictRoomIds[0] ??
			null,
	};
}
