...
export function buildMatrixReactionRelationsPath(
	roomId: string,
	messageId: string,
): string {
...
export function extractMatrixReactionAnnotation(
	...
	const eventId =
		typeof relatesTo.event_id === "string" ? relatesTo.event_id.trim() : "";
	return {
		key,
...
export function selectOwnMatrixReactionEventIds(
	events: Iterable<
		Pick<MatrixReactionEventLike, "content" | "event_id" | "sender">
	>,
	userId: string,
	emoji?: string,
): string[] {
	...
	for (const event of events) {
		...
		if (
			targetEmoji &&
			extractMatrixReactionKey(event.content) !== targetEmoji
		) {
			continue;
		}
