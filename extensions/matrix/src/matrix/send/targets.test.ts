...
describe("resolveMatrixRoomId", () => {
	...
	it("prefers joined rooms marked direct in member state over plain strict rooms", async () => {
		...
		const client = {
			...
			getJoinedRooms: vi
				.fn()
				.mockResolvedValue(["!fallback:example.org", "!explicit:example.org"]),
			getJoinedRoomMembers: vi
				.fn()
				.mockResolvedValue(["@bot:example.org", userId]),
			getRoomStateEvent: vi
				...
				.mockImplementation(
					async (roomId: string, _eventType: string, stateKey: string) => ({
						is_direct:
							roomId === "!explicit:example.org" && stateKey === userId,
					}),
				),
	...
	it("scopes direct-room cache per Matrix client", async () => {
		...
		const clientA = {
			...
			getJoinedRoomMembers: vi
				.fn()
				.mockResolvedValue(["@bot-a:example.org", userId]),
			setAccountData: vi.fn(),
			resolveRoom: vi.fn(),
		} as unknown as MatrixClient;
		const clientB = {
			...
			getJoinedRoomMembers: vi
				.fn()
				.mockResolvedValue(["@bot-b:example.org", userId]),
			setAccountData: vi.fn(),
			resolveRoom: vi.fn(),
		} as unknown as MatrixClient;

		await expect(resolveMatrixRoomId(clientA, userId)).resolves.toBe(
			"!room-a:example.org",
		);
		await expect(resolveMatrixRoomId(clientB, userId)).resolves.toBe(
			"!room-b:example.org",
		);
	...
	it("ignores m.direct entries that point at shared rooms", async () => {
		...
		await expect(resolveMatrixRoomId(client, userId)).resolves.toBe(
			"!dm-room:example.org",
		);
	});
	...
	it("revalidates cached direct rooms before reuse when membership changes", async () => {
		...
		const client = {
			...
			getJoinedRooms: vi
				.fn()
				.mockResolvedValue([
					"!dm-room-1:example.org",
					"!dm-room-2:example.org",
				]),
			getJoinedRoomMembers: vi
				.fn()
				.mockImplementation(
					async (roomId: string) => membersByRoom.get(roomId) ?? [],
				),
			setAccountData: vi.fn(),
			resolveRoom: vi.fn(),
		} as unknown as MatrixClient;
