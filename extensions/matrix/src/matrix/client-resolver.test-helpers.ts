...
type MatrixClientResolverMocks = {
	...
	acquireSharedMatrixClientMock: Mock<
		(...args: unknown[]) => Promise<MatrixClient>
	>;
	releaseSharedClientInstanceMock: Mock<
...
export async function expectOneOffSharedMatrixClient(params?: {
	...
	const sharedClient =
		await acquireSharedMatrixClientMock.mock.results[0]?.value;
	expect(sharedClient.prepareForOneOff).toHaveBeenCalledTimes(
		prepareForOneOffCalls,
	);
	expect(sharedClient.start).toHaveBeenCalledTimes(startCalls);
	expect(releaseSharedClientInstanceMock).toHaveBeenCalledWith(
		sharedClient,
		releaseMode,
	);
...
export function expectExplicitMatrixClientConfig(params: {
	cfg: unknown;
	accountId?: string;
}) {
	const {
