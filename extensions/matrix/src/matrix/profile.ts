...
async function uploadAvatarMedia(params: {
	...
	loadAvatar: (
		source: string,
		maxBytes: number,
	) => Promise<MatrixProfileLoadResult>;
}): Promise<string> {
	const media = await params.loadAvatar(
		params.avatarSource,
		params.avatarMaxBytes,
	);
...
export async function syncMatrixOwnProfile(params: {
	...
	try {
		...
		currentDisplayName =
			normalizeOptionalText(currentProfile.displayname) ?? undefined;
		currentAvatarUrl =
			normalizeOptionalText(currentProfile.avatar_url) ?? undefined;
	} catch {
