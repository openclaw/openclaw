...
export function resolveMatrixRoomKeyBackupIssue(
	...
	if (backup.matchesDecryptionKey === false) {
		return {
			...
			message:
				"backup key mismatch (this device does not have the matching backup decryption key)",
		};
	}
...
export function resolveMatrixRoomKeyBackupReadinessError(
	...
	if (issue.code === "missing-server-backup") {
		return opts.requireServerBackup
			? "Matrix room key backup is missing on the homeserver."
			: null;
	}
	if (issue.code === "ok") {
		return null;
