...
function readEnvSecretRefFallback(params: {
	...
	if (providerConfig) {
		...
		if (
			...
			!providerConfig.allowlist.includes(ref.id)
		) {
			throw new Error(
		...
		ref.provider !==
		(params.config?.secrets?.defaults?.env?.trim() || "default")
	) {
		throw new Error(
...
function resolveMatrixBaseConfigFieldPath(
	field: MatrixConfigStringField,
): string {
	return `channels.matrix.${field}`;
}

function shouldAllowEnvSecretRefFallback(
	field: MatrixConfigStringField,
): boolean {
...
function hasConfiguredMatrixAccessTokenSource(params: {
	...
	const account =
		findMatrixAccountConfig(params.cfg, normalizedAccountId) ?? {};
	const scopedAccessTokenVar =
		getMatrixScopedEnvVarNames(normalizedAccountId).accessToken;
	if (
...
function resolveConfiguredMatrixAuthInput(params: {
	...
	if (accountValue !== undefined) {
		return {
			value: accountValue,
			path: resolveMatrixConfigFieldPath(
				params.cfg,
				normalizedAccountId,
				params.field,
			),
	...
	const scopedEnv = resolveScopedMatrixEnvConfig(
		normalizedAccountId,
		params.env,
	);
	...
	if (scopedValue !== undefined) {
		return {
			value: scopedValue,
			path:
				params.field === "accessToken"
					? scopedKeys.accessToken
					: scopedKeys.password,
		};
	}
	...
	if (coerceSecretRef(configured.value, params.cfg.secrets?.defaults)) {
		throw new Error(
			resolved.unresolvedRefReason ??
				`${configured.path} SecretRef could not be resolved.`,
		);
	}
...
function readMatrixAccountConfigField(
	...
	return clean(
		account[field],
		resolveMatrixConfigFieldPath(cfg, accountId, field),
		{
			env: opts?.env,
			config: opts?.config,
			allowEnvSecretRefFallback: shouldAllowEnvSecretRefFallback(field),
...
function buildMatrixNetworkFields(params: {
	...
	"allowPrivateNetwork" | "ssrfPolicy" | "dispatcherPolicy"
> {
	const dispatcherPolicy: PinnedDispatcherPolicy | undefined =
		params.dispatcherPolicy ??
		(params.proxy
			? { mode: "explicit-proxy", proxyUrl: params.proxy }
			: undefined);
...
function resolveGlobalMatrixEnvConfig(env: NodeJS.ProcessEnv): MatrixEnvConfig {
	return {
		...
		accessToken:
			clean(env.MATRIX_ACCESS_TOKEN, "MATRIX_ACCESS_TOKEN") || undefined,
		...
		deviceName:
			clean(env.MATRIX_DEVICE_NAME, "MATRIX_DEVICE_NAME") || undefined,
	};
}
...
function hasScopedMatrixEnvConfig(
	accountId: string,
	env: NodeJS.ProcessEnv,
): boolean {
...
export function validateMatrixHomeserverUrl(
	...
	if (parsed.search || parsed.hash) {
		throw new Error(
			"Matrix homeserver URL must not include query strings or fragments",
		);
	}
	if (
...
export function resolveMatrixConfig(
	...
	const suppressInactivePasswordSecretRef =
		hasConfiguredMatrixAccessTokenSource({
			cfg,
			env,
	...
	const defaultScopedEnv = resolveScopedMatrixEnvConfig(
		DEFAULT_ACCOUNT_ID,
		env,
	);
	...
	const resolvedStrings = resolveMatrixAccountStringValues({
		...
		channel: {
			homeserver: readMatrixBaseConfigField(
				matrix,
				"homeserver",
				fieldReadOptions,
			),
			userId: readMatrixBaseConfigField(matrix, "userId", fieldReadOptions),
			accessToken: readMatrixBaseConfigField(
				matrix,
				"accessToken",
				fieldReadOptions,
			),
			...
			deviceName: readMatrixBaseConfigField(
				matrix,
				"deviceName",
				fieldReadOptions,
			),
	...
	const allowPrivateNetwork =
		matrix.allowPrivateNetwork === true ? true : undefined;
	return {
...
export function resolveMatrixConfigForAccount(
	...
	const accountField = (field: MatrixConfigStringField) =>
		readMatrixAccountConfigField(cfg, normalizedAccountId, account, field, {
			...fieldReadOptions,
			suppressSecretRef:
				field === "password" ? suppressInactivePasswordSecretRef : undefined,
		});
	const resolvedStrings = resolveMatrixAccountStringValues({
		accountId: normalizedAccountId,
		account: {
	...
	const accountInitialSyncLimit = clampMatrixInitialSyncLimit(
		account.initialSyncLimit,
	);
	const initialSyncLimit =
		accountInitialSyncLimit ??
		clampMatrixInitialSyncLimit(matrix.initialSyncLimit);
	const encryption =
		typeof account.encryption === "boolean"
			? account.encryption
			: (matrix.encryption ?? false);
	const allowPrivateNetwork =
		account.allowPrivateNetwork === true || matrix.allowPrivateNetwork === true
			? true
			: undefined;
...
export function resolveMatrixAuthContext(params?: {
	...
	const effectiveAccountId =
		explicitAccountId ?? resolveImplicitMatrixAccountId(cfg, env);
...
export async function resolveMatrixAuth(params?: {
	...
	if (accessToken) {
		...
		const hasMatchingCachedToken =
			cachedCredentials?.accessToken === accessToken;
		...
		if (!userId || !knownDeviceId) {
			...
			const whoami = (await tempClient.doRequest(
				"GET",
	...
	const login = (await loginClient.doRequest(
		"POST",
