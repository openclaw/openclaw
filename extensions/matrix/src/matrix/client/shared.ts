...
async function ensureSharedClientStarted(
  ...
  state.startAbortController = startAbortController;
  const startPromise = (async () => {
    ...
    await state.client.start({ abortSignal: startAbortController.signal });
    throwIfMatrixStartupAborted(startAbortController.signal);
    if (state.phase !== "open") {
      throwIfMatrixStartupAborted(AbortSignal.abort());
    }
    state.started = true;
  })();
...
function beginGenerationRetirement(params: {
  ...
  const owner = Promise.resolve().then(async () => {
    ...
    if (state.poisonError) {
      ...
      if (lateLeaseDrain || (canReplacePoisonedGeneration && decryptionsDrained)) {
        deleteSharedClientState(state);
      }
      throw state.poisonError;
    }
