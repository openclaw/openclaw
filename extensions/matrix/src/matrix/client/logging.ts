...
export function createMatrixJsSdkClientLogger(
	prefix = "matrix",
): MatrixJsSdkLogger {
...
function applyMatrixJsSdkRootLoggerMode(): void {
	...
	rootLogger.methodFactory =
		matrixSdkLogMode === "quiet"
			? matrixSdkSilentMethodFactory
			: matrixSdkRootMethodFactory;
	rootLogger.rebuild?.();
}
...
function createMatrixJsSdkLoggerInstance(prefix: string): MatrixJsSdkLogger {
	const log = (
		...
		(
			matrixSdkBaseLogger[method] as (
				module: string,
				...args: unknown[]
			) => void
		)(prefix, ...messageOrObject);
	};
	...
	return {
		...
		getChild: (namespace: string) => {
			...
			return createMatrixJsSdkLoggerInstance(
				nextNamespace ? `${prefix}.${nextNamespace}` : prefix,
			);
		},
	};
}
