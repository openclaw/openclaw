...
function assertLegacyMigrationAccountSelection(params: {
	...
	const selectedAccountId = normalizeAccountId(
		resolveMatrixDefaultOrOnlyAccountId(cfg),
	);
...
function readStoredRootMetadata(rootDir: string): StoredRootMetadata {
	...
	try {
		...
		if (
			typeof parsed.accessTokenHash === "string" &&
			parsed.accessTokenHash.trim()
		) {
...
function isCompatibleStorageRoot(params: {
	...
	if (
		metadata.accountId &&
		normalizeAccountId(metadata.accountId) !==
			normalizeAccountId(params.accountKey)
	) {
		return false;
	}
	if (
...
export async function maybeMigrateLegacyStorage(params: {
	...
	const logger = getMatrixRuntime().logging.getChildLogger({
	...
	if (moved.length > 0) {
		logger.info(
			`matrix: migrated legacy client storage into ${params.storagePaths.rootDir}\n${moved
				.map(
					(entry) =>
						`- ${entry.label}: ${entry.sourcePath} -> ${entry.targetPath}`,
				)
				.join("\n")}`,
		);
	}
