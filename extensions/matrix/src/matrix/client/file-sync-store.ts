...
function normalizeRoomsData(value: unknown): IRooms | null {
	...
	return {
		[Category.Join]: isRecord(value[Category.Join])
			? (value[Category.Join] as IRooms["join"])
			: {},
...
export class FileBackedMatrixSyncStore extends MemoryStore {
	...
	constructor(private readonly storagePath: string) {
		...
		this.hadCleanShutdownOnLoad =
			this.hadSavedSyncOnLoad && restoredCleanShutdown;
	...
	private markDirtyAndSchedulePersist(): void {
		...
		this.persistTimer = setTimeout(() => {
			...
			void this.flush().catch((err) => {
				LogService.warn(
					"MatrixFileSyncStore",
					"Failed to persist Matrix sync store:",
					err,
				);
			});
		}, PERSIST_DEBOUNCE_MS);
	...
	private async persist(): Promise<void> {
		...
		const payload: PersistedMatrixSyncStore = {
			...
			...(this.savedClientOptions
				? { clientOptions: cloneJson(this.savedClientOptions) }
				: {}),
		};
		try {
