...
describe("resolveMatrixConfig", () => {
	...
	it("does not throw when accessToken uses a non-env SecretRef", () => {
		...
		expect(
			resolveMatrixConfig(cfg, {} as NodeJS.ProcessEnv).accessToken,
		).toBeUndefined();
	});
	...
	it("ignores typoed defaultAccount values that do not map to a real Matrix account", () => {
		...
		expect(resolveImplicitMatrixAccountId(cfg, {} as NodeJS.ProcessEnv)).toBe(
			"default",
		);
		expect(
	...
	it("requires explicit defaultAccount selection when multiple named Matrix accounts exist", () => {
		...
		expect(
			resolveImplicitMatrixAccountId(cfg, {} as NodeJS.ProcessEnv),
		).toBeNull();
		expect(() =>
			resolveMatrixAuthContext({ cfg, env: {} as NodeJS.ProcessEnv }),
		).toThrow(/channels\.matrix\.defaultAccount.*--account <id>/i);
	});
	...
	it("allows explicit non-default account ids backed only by scoped env vars", () => {
		...
		expect(
			resolveMatrixAuthContext({ cfg, env, accountId: "ops" }).accountId,
		).toBe("ops");
	});
	...
	it("rejects insecure public http Matrix homeservers", () => {
		expect(() =>
			validateMatrixHomeserverUrl("http://matrix.example.org"),
		).toThrow(
			"Matrix homeserver must use https:// unless it targets a private or loopback host",
		);
		expect(validateMatrixHomeserverUrl("http://127.0.0.1:8008")).toBe(
			"http://127.0.0.1:8008",
		);
	});
...
describe("resolveMatrixAuth", () => {
	...
	it("surfaces password login errors when account credentials are invalid", async () => {
		const doRequestSpy = vi.spyOn(
			sdkModule.MatrixClient.prototype,
			"doRequest",
		);
		doRequestSpy.mockRejectedValueOnce(
	...
	it("uses cached matching credentials when access token is not configured", async () => {
		...
		vi.mocked(credentialsReadModule.credentialsMatchConfig).mockReturnValue(
			true,
		);
	...
	it("rejects embedded credentials in Matrix homeserver URLs", async () => {
		...
		await expect(
			resolveMatrixAuth({ cfg, env: {} as NodeJS.ProcessEnv }),
		).rejects.toThrow(
	...
	it("resolves file-backed accessToken SecretRefs during Matrix auth", async () => {
		const tempDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "matrix-secret-ref-"),
		);
