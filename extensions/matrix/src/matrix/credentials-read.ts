...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
	try {
		...
		const homeDir =
			env.OPENCLAW_HOME?.trim() || env.HOME?.trim() || os.homedir();
		return path.join(homeDir, ".openclaw");
	}
}

function resolveLegacyMatrixCredentialsPath(
...
function shouldReadLegacyCredentialsForAccount(
	...
	return (
		normalizeAccountId(resolveMatrixDefaultOrOnlyAccountId(cfg)) ===
		normalizedAccountId
	);
}
...
function resolveLegacyMigrationSourcePath(
	...
	return legacyPath === resolveMatrixCredentialsPath(env, accountId)
		? null
		: legacyPath;
}

function parseMatrixCredentialsFile(
	filePath: string,
...
export function credentialsMatchConfig(
	...
	if (!config.userId) {
		...
		return (
			stored.homeserver === config.homeserver &&
			stored.accessToken === config.accessToken
		);
	}
	return (
