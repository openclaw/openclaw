...
export async function readJoinedMatrixMembers(
	...
	try {
		return normalizeJoinedMatrixMembers(
			await client.getJoinedRoomMembers(roomId),
		);
	} catch {
		return null;
	}
...
export async function inspectMatrixDirectRoomEvidence(params: {
	...
	const joinedMembers = await readJoinedMatrixMembers(
		params.client,
		params.roomId,
	);
	...
	return {
		...
		strict,
		viaMemberState:
			(await hasDirectMatrixMemberFlag(
				params.client,
				params.roomId,
				params.remoteUserId,
			)) ||
