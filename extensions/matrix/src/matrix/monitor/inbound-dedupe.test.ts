...
describe("Matrix inbound event dedupe", () => {
	...
	function createStoragePath(): string {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "openclaw-matrix-inbound-dedupe-"),
		);
		tempDirs.push(dir);
	...
	it("persists committed events across restarts", async () => {
		...
		expect(
			first.claimEvent({ roomId: "!room:example.org", eventId: "$event-1" }),
		).toBe(true);
		...
		expect(
			second.claimEvent({ roomId: "!room:example.org", eventId: "$event-1" }),
		).toBe(false);
	});
	...
	it("does not persist released pending claims", async () => {
		...
		expect(
			first.claimEvent({ roomId: "!room:example.org", eventId: "$event-2" }),
		).toBe(true);
		...
		expect(
			second.claimEvent({ roomId: "!room:example.org", eventId: "$event-2" }),
		).toBe(true);
	});
	...
	it("prunes expired and overflowed entries on load", async () => {
		...
		expect(
			deduper.claimEvent({ roomId: "!room:example.org", eventId: "$old" }),
		).toBe(true);
		expect(
			deduper.claimEvent({ roomId: "!room:example.org", eventId: "$keep-1" }),
		).toBe(true);
		expect(
			deduper.claimEvent({ roomId: "!room:example.org", eventId: "$keep-2" }),
		).toBe(false);
		expect(
			deduper.claimEvent({ roomId: "!room:example.org", eventId: "$keep-3" }),
		).toBe(false);
	});
	...
	it("retains replayed backlog events based on processing time", async () => {
		...
		expect(
			first.claimEvent({ roomId: "!room:example.org", eventId: "$backlog" }),
		).toBe(true);
		await first.commitEvent({
			roomId: "!room:example.org",
			eventId: "$backlog",
		...
		expect(
			second.claimEvent({ roomId: "!room:example.org", eventId: "$backlog" }),
		).toBe(false);
	});
