...
function makeRoomTriggerEvent(params: {
	eventId: string;
	body: string;
	ts?: number;
}) {
...
function makeRoomPlainEvent(params: {
	eventId: string;
	body: string;
	ts?: number;
}) {
...
describe("matrix group chat history — scenario 1: basic accumulation", () => {
	it("pending messages appear in InboundHistory; trigger itself does not", async () => {
		...
		const ctx = finalizeInboundContext.mock.calls[0]?.[0] as Record<
			string,
			unknown
		>;
		const history = ctx.InboundHistory as Array<{
			body: string;
			sender: string;
		}>;
	...
	it("multi-agent: each agent has an independent watermark", async () => {
		...
		await handler(
			DEFAULT_ROOM,
			makeRoomTriggerEvent({ eventId: "$c", body: "msg C", ts: 3000 }),
		);
		{
			const ctx = finalizeInboundContext.mock.calls[1]?.[0] as Record<
				string,
				unknown
			>;
			const history = ctx.InboundHistory as Array<{ body: string }>;
			expect(history).toHaveLength(2);
			expect(history.map((h) => h.body).some((b) => b.includes("msg A"))).toBe(
				true,
			);
			expect(history.map((h) => h.body).some((b) => b.includes("msg B"))).toBe(
				true,
			);
		}
		...
		await handler(
			DEFAULT_ROOM,
			makeRoomTriggerEvent({ eventId: "$d", body: "msg D", ts: 4000 }),
		);
		{
			const ctx = finalizeInboundContext.mock.calls[2]?.[0] as Record<
				string,
				unknown
			>;
	...
	it("historyLimit=0 disables history accumulation entirely", async () => {
		...
		await handler(
			DEFAULT_ROOM,
			makeRoomPlainEvent({ eventId: "$p", body: "pending" }),
		);
		await handler(
			DEFAULT_ROOM,
			makeRoomTriggerEvent({ eventId: "$t", body: "trigger" }),
		);
	...
	it("historyLimit=0 does not serialize same-room ingress", async () => {
		...
		const first = handler(
			DEFAULT_ROOM,
			makeRoomTriggerEvent({ eventId: "$a", body: "first" }),
		);
		await Promise.resolve();
		const second = handler(
			DEFAULT_ROOM,
			makeRoomTriggerEvent({ eventId: "$b", body: "second" }),
		);
		await Promise.resolve();
	...
	it("DMs do not accumulate history (group chat only)", async () => {
		...
		await handler(
			DEFAULT_ROOM,
			makeRoomPlainEvent({ eventId: "$dm1", body: "dm message 1" }),
		);
		await handler(
			DEFAULT_ROOM,
			makeRoomPlainEvent({ eventId: "$dm2", body: "dm message 2" }),
		);
	...
	it("includes skipped media-only room messages in next trigger history", async () => {
		...
		await handler(
			DEFAULT_ROOM,
			makeRoomTriggerEvent({
				eventId: "$trigger-media",
				body: "trigger",
				ts: 2000,
			}),
		);
		...
		expect(
			history?.some((entry) =>
				entry.body.includes("[matrix image attachment]"),
			),
		).toBe(true);
	});
...
describe("matrix group chat history — scenario 2: race condition safety", () => {
	it("messages arriving during agent processing are visible on the next trigger", async () => {
		...
		await handler(
			DEFAULT_ROOM,
			makeRoomPlainEvent({ eventId: "$b", body: "msg B", ts: 2000 }),
		);
		...
		expect(finalizeInboundContext).toHaveBeenCalledTimes(2);
		const ctxForC = finalizeInboundContext.mock.calls[1]?.[0] as Record<
			string,
			unknown
		>;
	...
	it("watermark does not advance when final reply delivery fails (retry sees same history)", async () => {
		...
		const { handler } = createMatrixHandlerTestHarness({
			...
			withReplyDispatcher: async <T>(params: {
				...
				capturedOnError?.(new Error("simulated delivery failure"), {
					kind: "final",
				});
	...
	it("retrying the same failed trigger reuses the original history window", async () => {
		...
		expect(finalizeInboundContext).toHaveBeenCalledTimes(2);
		const firstHistory = (
			finalizeInboundContext.mock.calls[0]?.[0] as Record<string, unknown>
		).InboundHistory as Array<{ body: string }>;
		const retryHistory = (
			finalizeInboundContext.mock.calls[1]?.[0] as Record<string, unknown>
		).InboundHistory as Array<{ body: string }>;
	...
	it("records pending history before sender-name lookup resolves", async () => {
		...
		const triggerDone = handler(
			DEFAULT_ROOM,
			makeRoomTriggerEvent({
				eventId: "$trigger-after-slow-name",
				body: "trigger",
				ts: 2000,
			}),
		);
		...
		const history = ctx.InboundHistory as Array<{ body: string }> | undefined;
		expect(
			history?.some((entry) => entry.body.includes("plain before trigger")),
		).toBe(true);
	});
