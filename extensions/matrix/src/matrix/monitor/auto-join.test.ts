...
function createClientStub() {
	...
	return {
		...
		joinRoom: (client as unknown as { joinRoom: ReturnType<typeof vi.fn> })
			.joinRoom,
		resolveRoom: (
			client as unknown as { resolveRoom: ReturnType<typeof vi.fn> }
		).resolveRoom,
	};
}
	...
	it("logs and skips allowlist alias resolution failures", async () => {
		...
		expect(error).toHaveBeenCalledWith(
			expect.stringContaining(
				"matrix: failed resolving allowlisted alias #allowed:example.org:",
			),
		);
	});
