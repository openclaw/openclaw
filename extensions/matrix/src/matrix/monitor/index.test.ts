...
const hoisted = vi.hoisted(() => {
	...
	return {
		...
		inboundDeduper,
		logger,
		registeredOnRoomMessage: null as
			| null
			| ((roomId: string, event: unknown) => Promise<void>),
	...
	resolveSharedMatrixClient: vi.fn(
		async (params: { startClient?: boolean }) => {
			...
			if (!hoisted.callOrder.includes("create-manager")) {
				throw new Error(
					"Matrix client started before thread bindings were registered",
				);
			}
...
describe("monitorMatrixProvider", () => {
	beforeEach(() => {
		...
		hoisted.client.drainPendingDecryptions
			.mockReset()
	...
	it("stops sync, drains decryptions, then waits for in-flight handlers before persisting", async () => {
		...
		const roomMessagePromise = onRoomMessage("!room:example.org", {
			event_id: "$event",
		});
		abortController.abort();
		await vi.waitFor(() => {
