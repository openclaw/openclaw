...
async function readStartupVerificationState(
	...
	const { value } =
		await readJsonFileWithFallback<MatrixStartupVerificationState | null>(
			filePath,
			null,
		);
...
function resolveStateCooldownMs(
	...
	if (state?.outcome === "failed") {
		return Math.min(
			cooldownMs,
			DEFAULT_STARTUP_VERIFICATION_FAILURE_COOLDOWN_MS,
		);
	}
	return cooldownMs;
}
...
export async function ensureMatrixStartupVerification(params: {
	...
	accountConfig: Pick<
		MatrixConfig,
		"startupVerification" | "startupVerificationCooldownHours"
	>;
	env?: NodeJS.ProcessEnv;
	nowMs?: number;
	...
	const mode =
		params.accountConfig.startupVerification ??
	...
	const verifications = await params.client.crypto
		.listVerifications()
		.catch(() => []);
	...
	try {
		const request = await params.client.crypto.requestVerification({
			ownUser: true,
		});
