...
const MATRIX_INBOUND_DEDUPE_TTL_MS = 30 * 24 * 60 * 60 * 1000;
const MATRIX_INBOUND_DEDUPE_MEMORY_MAX = 5_000;
const MATRIX_INBOUND_DEDUPE_STATE_MAX_ENTRIES = 20_000;
...
function buildEventKey(params: { roomId: string; eventId: string }): string | null {
...
export function createMatrixInboundEventDeduper(params: {
  ...
  const namespace = params.auth.accountId.trim() || "default";
  return {
    claimEvent: async (ids) => {
      const key = buildEventKey(ids);
      if (!key) {
