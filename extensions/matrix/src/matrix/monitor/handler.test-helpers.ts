...
export function createMatrixHandlerTestHarness(
	...
	const readAllowFromStore =
		options.readAllowFromStore ?? vi.fn(async () => [] as string[]);
	const upsertPairingRequest =
		options.upsertPairingRequest ??
		vi.fn(async () => ({ code: "ABCDEFGH", created: false }));
	const resolveAgentRoute =
		options.resolveAgentRoute ?? vi.fn(() => DEFAULT_ROUTE);
	const recordInboundSession =
		options.recordInboundSession ?? vi.fn(async () => {});
	const finalizeInboundContext =
		options.finalizeInboundContext ?? vi.fn((ctx) => ctx);
	...
	const handler = createMatrixRoomMessageHandler({
			...
			channel: {
				...
				commands: {
					shouldHandleTextCommands:
						options.shouldHandleTextCommands ?? (() => false),
				},
				text: {
					hasControlCommand: options.hasControlCommand ?? (() => false),
					resolveMarkdownTableMode:
						options.resolveMarkdownTableMode ?? (() => "preserve"),
				},
				routing: {
					resolveAgentRoute,
				},
				...
				session: {
					resolveStorePath:
						options.resolveStorePath ?? (() => "/tmp/session-store"),
					readSessionUpdatedAt:
						options.readSessionUpdatedAt ?? (() => undefined),
					recordInboundSession,
				},
				reply: {
					resolveEnvelopeFormatOptions:
						options.resolveEnvelopeFormatOptions ?? (() => ({})),
					formatAgentEnvelope:
						options.formatAgentEnvelope ??
					...
					resolveHumanDelayConfig:
						options.resolveHumanDelayConfig ?? (() => undefined),
		...
		getMemberDisplayName:
			options.getMemberDisplayName ?? (async () => "sender"),
