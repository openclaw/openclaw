...
function resolveRelationEventId(content: RoomMessageEventContent): string | undefined {
  ...
  if (typeof relates.event_id === "string" && relates.event_id.trim()) {
...
function resolveRelationType(content: RoomMessageEventContent): string | undefined {
  ...
  if (typeof relates.rel_type === "string" && relates.rel_type.trim()) {
...
function isIgnoredSelfDraftEvent(params: {
  ...
  if (eventId && params.ignoredEventIds.has(eventId)) {
    return true;
  }
  if (params.event.type !== EventType.RoomMessage) {
    return false;
  }
  const content = params.event.content as RoomMessageEventContent & Record<string, unknown>;
...
export function createMatrixLatestVisibleTracker(params?: {
  ...
  return {
    ...
    prepareTrigger(_agentId, roomId, event) {
