...
export async function runMatrixStartupMaintenance(params: {
	...
	try {
		const profileSync = await syncMatrixOwnProfile({
			...
			loadAvatarFromUrl: async (url, maxBytes) =>
				await params.loadWebMedia(url, maxBytes),
		});
		if (profileSync.displayNameUpdated) {
			params.logger.info(
				`matrix: profile display name updated for ${params.auth.userId}`,
			);
		}
		if (profileSync.avatarUpdated) {
			params.logger.info(
				`matrix: profile avatar updated for ${params.auth.userId}`,
			);
		}
		if (
			...
			const updatedCfg = updateMatrixAccountConfig(
				latestCfg,
				params.accountId,
				{
		...
		params.logger.warn("matrix: failed to sync profile from config", {
			error: String(err),
		});
	}
	...
	try {
		const deviceHealth = summarizeMatrixDeviceHealth(
		...
		params.logger.debug?.(
			"Failed to inspect matrix device hygiene (non-fatal)",
			{
				error: String(err),
			},
		);
	...
	try {
		...
		params.logger.debug?.(
			"Failed to resolve matrix verification status (non-fatal)",
			{
				error: String(err),
			},
		);
	...
	try {
		...
		params.logger.warn(
			"matrix: failed restoring legacy encrypted-state backup",
			{
				error: String(err),
			},
		);
