...
describe("registerMatrixMonitorEvents verification routing", () => {
  ...
  it.each([
    {
      name: "remembers invite provenance on room invites",
      eventId: "$invite1",
      sender: "@alice:example.org",
      direct: true,
      remember: true,
    ...
    {
      name: "ignores lifecycle-only invite events emitted with self sender ids",
      eventId: "$invite-self",
      sender: "@bot:example.org",
      remember: false,
    },
    {
      name: "remembers invite provenance even when Matrix omits the direct invite hint",
      eventId: "$invite-group",
      sender: "@alice:example.org",
      remember: true,
    },
  ])("$name", ({ eventId, sender, direct, remember }) => {
    ...
    expect(invalidateRoom).toHaveBeenCalledWith("!room:example.org");
    if (remember) {
      expect(rememberInvite).toHaveBeenCalledWith("!room:example.org", sender);
      return;
    }
