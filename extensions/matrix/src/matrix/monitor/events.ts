...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Mullusi runtime is using the same account. " +
    `Check 'mullusi matrix verify status --verbose --account ${accountId}' and 'mullusi matrix devices list --account ${accountId}'.`
  );
}
