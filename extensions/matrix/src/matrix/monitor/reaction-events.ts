...
export function resolveMatrixReactionNotificationMode(params: {
	...
	return (
		accountConfig.reactionNotifications ??
		matrixConfig?.reactionNotifications ??
		"own"
	);
}
...
export async function handleInboundMatrixReaction(params: {
	...
	const targetSender =
		targetEvent && typeof targetEvent.sender === "string"
			? targetEvent.sender.trim()
			: "";
	if (!targetSender) {
		return;
	}
