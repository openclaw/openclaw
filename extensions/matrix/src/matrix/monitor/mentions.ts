...
function checkFormattedBodyMention(params: {
	...
	for (const match of params.formattedBody.matchAll(anchorPattern)) {
		...
		try {
			...
			const fragmentTarget = decodeURIComponent(
				parsed.hash.replace(/^#\/?/, "").trim(),
			);
...
export function resolveMentions(params: {
	...
	const visibleRoomMention =
		hasVisibleRoomMention(params.text) ||
		hasVisibleRoomMention(params.content.formatted_body);
	...
	const metadataBackedRoomMention =
		Boolean(mentions?.room) && visibleRoomMention;
	const explicitMention =
		mentionedInFormattedBody ||
		metadataBackedUserMention ||
		metadataBackedRoomMention;
