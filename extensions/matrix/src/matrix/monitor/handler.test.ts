...
describe("matrix monitor handler pairing account scope", () => {
  ...
  it.each<{
    ...
    {
      name: "processes room messages mentioned via displayName in formatted_body",
      displayName: "Tom Servo",
      eventId: "$display-name-mention",
      body: "Tom Servo: hello",
      formattedBody: '<a href="https://matrix.to/#/@bot:example.org">Tom Servo</a>: hello',
    },
    {
      name: "processes room messages mentioned via @displayName in Unicode formatted_body",
      displayName: "欢欢",
      eventId: "$unicode-display-name-mention",
      body: "@欢欢 please reply",
      formattedBody: '<a href="https://matrix.to/#/@bot:example.org">@欢欢</a> please reply',
      explicitMention: true,
    },
    {
      name: "processes room messages mentioned via bracketed @displayName in formatted_body",
  ...
  ])("$name", async ({ displayName, eventId, body, formattedBody, explicitMention }) => {
    const recordInboundSession = vi.fn(async () => {});
    const { handler } = createMatrixHandlerTestHarness({
      isDirectMessage: false,
      getMemberDisplayName: async () => displayName,
    ...
    await handler(
      ...
      createMatrixRoomMessageEvent({
        ...
        content: {
          ...
          ...(explicitMention ? { "m.mentions": { user_ids: ["@bot:example.org"] } } : {}),
        },
      }),
    );
