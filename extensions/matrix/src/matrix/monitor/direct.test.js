...
function createMockClient(opts) {
  ...
  return {
    dms: {
      isDm: (roomId) => dmRooms[roomId] ?? false,
      update: vi.fn().mockResolvedValue(void 0)
    },
    getUserId: vi.fn().mockResolvedValue(selfUserId),
    getJoinedRoomMembers: vi.fn().mockImplementation(async (roomId) => {
      return membersByRoom[roomId] ?? [];
    }),
    getRoomStateEvent: vi.fn().mockImplementation(async (roomId, eventType, stateKey) => {
      const key = `${roomId}|${eventType}|${stateKey}`;
      const ev = stateEvents[key];
      if (ev === void 0) {
        const err = new Error(`State event not found: ${key}`);
...
function createBrokenDmClient(roomNameEvent) {
  ...
  describe("edge cases", () => {
    ...
    it("treats M_NOT_FOUND for room name as no name (DM)", async () => {
      ...
      client.getRoomStateEvent.mockImplementation(
        async (roomId, eventType, stateKey) => {
