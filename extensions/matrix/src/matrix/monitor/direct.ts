...
const _MAX_TRACKED_DM_MEMBER_FLAGS = 2048;
...
export function createDirectRoomTracker(
	...
	const joinedMembersCache = new Map<
		string,
		{ members: string[]; ts: number }
	>();
	const directMemberFlagCache = new Map<
		string,
		{ isDirect: boolean; ts: number }
	>();
	...
	const resolveDirectMemberFlag = async (
		...
		const isDirect = await hasDirectMatrixMemberFlag(
			client,
			roomId,
			normalizedUserId,
		);
