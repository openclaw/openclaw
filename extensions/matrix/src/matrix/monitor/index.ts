...
export async function monitorMatrixProvider(
	...
	let groupAllowFrom: string[] = (accountConfig.groupAllowFrom ?? []).map(
		String,
	);
	...
	({ allowFrom, groupAllowFrom, roomsConfig } =
	...
	needsRoomAliasesForConfig = Boolean(
		roomsConfig &&
			Object.keys(roomsConfig).some((key) => key.trim().startsWith("#")),
	);
	...
	let cleanedUp = false;
	let threadBindingManager: { accountId: string; stop: () => void } | null =
		null;
	...
	const historyLimit = Math.max(
		0,
		accountConfig.historyLimit ?? globalGroupChatHistoryLimit ?? 0,
	);
	const mediaMaxMb =
		opts.mediaMaxMb ?? accountConfig.mediaMaxMb ?? DEFAULT_MEDIA_MAX_MB;
	...
	const trackRoomMessage = (
		roomId: string,
		event: Parameters<typeof handleRoomMessage>[1],
	) => {
	...
	try {
		...
		await runMatrixStartupMaintenance({
			...
			loadWebMedia: async (url, maxBytes) =>
				await core.media.loadWebMedia(url, maxBytes),
			env: process.env,
		});
