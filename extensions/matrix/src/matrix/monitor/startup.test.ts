...
function createStartupVerificationOutcome(
	...
	overrides: Partial<
		Extract<MatrixStartupVerificationOutcome, { kind: typeof kind }>
	> = {},
...
const hoisted = vi.hoisted(() => ({
	maybeRestoreLegacyMatrixBackup: vi.fn(async () =>
...
describe("runMatrixStartupMaintenance", () => {
	beforeEach(() => {
		...
		hoisted.summarizeMatrixDeviceHealth
			.mockClear()
			.mockReturnValue({ staleOpenClawDevices: [] });
		hoisted.syncMatrixOwnProfile
			.mockClear()
			.mockResolvedValue(createProfileSyncResult());
		hoisted.ensureMatrixStartupVerification
			.mockClear()
			.mockResolvedValue(createStartupVerificationOutcome("verified"));
		hoisted.updateMatrixAccountConfig
			.mockClear()
			.mockImplementation((cfg: unknown) => cfg);
	});
