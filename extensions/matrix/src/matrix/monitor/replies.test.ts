...
vi.mock("../send.js", () => ({
	chunkMatrixText: (text: string, opts?: unknown) =>
		chunkMatrixTextMock(text, opts),
...
describe("deliverMatrixReplies", () => {
	...
	const resolveMarkdownTableModeMock = vi.fn<(params: unknown) => string>(
		() => "code",
	);
	...
	const runtimeStub = {
		...
		channel: {
			text: {
				resolveMarkdownTableMode: (params: unknown) =>
					resolveMarkdownTableModeMock(params),
	...
	it("uses supplied cfg for chunking and send delivery without reloading runtime config", async () => {
		...
		loadConfigMock.mockImplementation(() => {
			throw new Error(
				"deliverMatrixReplies should not reload runtime config when cfg is provided",
			);
		});
	...
	it("passes raw media captions through to sendMessageMatrix without pre-converting them", async () => {
		convertMarkdownTablesMock.mockImplementation(
