...
async function resolvePendingMigrationStatePath(params: {
	...
	auth: Pick<
		MatrixAuth,
		"homeserver" | "userId" | "accessToken" | "accountId" | "deviceId"
	>;
}): Promise<{
	statePath: string;
	...
	const { value: directValue } =
		await readJsonFileWithFallback<MatrixLegacyCryptoMigrationState | null>(
			directStatePath,
			null,
		);
	if (
		isMigrationState(directValue) &&
		directValue.restoreStatus === "pending"
	) {
	...
	try {
		siblingEntries = (
			await fs.readdir(accountStorageDir, { withFileTypes: true })
		)
