...
function readVerificationSignal(event: MatrixRawEvent): {
	...
	const msgtype =
		trimMaybeString((content as { msgtype?: unknown }).msgtype) ?? "";
	const relatedEventId = trimMaybeString(
		(content as { "m.relates_to"?: { event_id?: unknown } })["m.relates_to"]
			?.event_id,
	);
	const transactionId = trimMaybeString(
		(content as { transaction_id?: unknown }).transaction_id,
	);
	if (
		type === EventType.RoomMessage &&
		isMatrixVerificationRequestMsgType(msgtype)
	) {
		return {
			stage: "request",
			flowId:
				trimMaybeString(event.event_id) ?? transactionId ?? relatedEventId,
		};
	}
	...
	const flowId =
		transactionId ?? relatedEventId ?? trimMaybeString(event.event_id);
...
function formatVerificationSasNotice(
	...
	const emojiLine =
		Array.isArray(sas.emoji) && sas.emoji.length > 0
			? `SAS emoji: ${sas.emoji
					.map(
						([emoji, name]) =>
							`${trimMaybeString(emoji) ?? "?"} ${trimMaybeString(name) ?? "?"}`,
					)
					.join(" | ")}`
			: null;
	const decimalLine =
...
function isActiveVerificationSummary(
	summary: MatrixVerificationSummaryLike,
): boolean {
...
async function resolveVerificationSummaryForSignal(
	...
	const activeByUser = list
		.filter(
			(entry) =>
				entry.otherUserId === params.senderId &&
				isActiveVerificationSummary(entry),
		)
...
async function resolveVerificationSasNoticeForSignal(
	...
	summary: MatrixVerificationSummaryLike | null;
	sasNotice: string | null;
}> {
	...
	if (
		immediateNotice ||
		(params.stage !== "ready" && params.stage !== "start")
	) {
		return {
			summary,
	...
	await new Promise((resolve) =>
		setTimeout(resolve, SAS_NOTICE_RETRY_DELAY_MS),
	);
	const retriedSummary = await resolveVerificationSummaryForSignal(
		client,
		params,
	);
	return {
...
export function createMatrixVerificationEventRouter(params: {
	...
	async function resolveActiveDirectRoomId(
		remoteUserId: string,
	): Promise<string | null> {
		const inspection = await inspectMatrixDirectRooms({
	...
	function rememberVerificationRoom(
		roomId: string,
		event: MatrixRawEvent,
		flowId: string | null,
	) {
	...
	function rememberVerificationUserRoom(
		remoteUserId: string,
		roomId: string,
	): void {
	...
	async function resolveSummaryRoomId(
		...
		const mappedRoomId =
			...
			trimMaybeString(
				summary.transactionId
					? verificationFlowRooms.get(summary.transactionId)
					: null,
			) ??
		...
		const recentRoomId = trimMaybeString(
			verificationUserRooms.get(remoteUserId),
		);
	...
	async function routeVerificationSummary(
		summary: MatrixVerificationSummaryLike,
	): Promise<void> {
	...
	function routeVerificationEvent(
		roomId: string,
		event: MatrixRawEvent,
	): boolean {
		...
		void (async () => {
			...
			const { summary, sasNotice } =
				await resolveVerificationSasNoticeForSignal(params.client, {
					roomId,
					event,
					senderId,
					flowId,
			...
			params.logVerboseMessage(
				`matrix: failed routing verification event: ${String(err)}`,
			);
		});
