...
describe("matrixApprovalNativeRuntime", () => {
  ...
  it("preserves custom plugin approval command actions in Matrix fallback text", async () => {
    const view = buildPluginApprovalView({
      actions: [
        {
          kind: "command",
    ...
    expect(pendingPayload.text).not.toContain("/approve plugin:req-1 allow-once");
    expect(pendingPayload.extraContent[MATRIX_APPROVAL_METADATA_KEY].actions).toEqual(view.actions);
  });
