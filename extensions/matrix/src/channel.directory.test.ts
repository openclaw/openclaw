...
describe("matrix directory", () => {
	...
	it("clears stored auth fields when switching a Matrix account to env-backed auth", () => {
		...
		try {
			...
			expect(
				updated.channels?.matrix?.accounts?.ops?.homeserver,
			).toBeUndefined();
			expect(updated.channels?.matrix?.accounts?.ops?.userId).toBeUndefined();
			expect(
				updated.channels?.matrix?.accounts?.ops?.accessToken,
			).toBeUndefined();
			...
			expect(
				updated.channels?.matrix?.accounts?.ops?.deviceName,
			).toBeUndefined();
			expect(
				resolveMatrixConfigForAccount(updated, "ops", process.env),
			).toMatchObject({
	...
	it("clears stale access token when switching an account to password auth", () => {
		...
		expect(updated.channels?.matrix?.accounts?.default?.password).toBe(
			"new-password",
		);
		expect(
			updated.channels?.matrix?.accounts?.default?.accessToken,
		).toBeUndefined();
	});
	...
	it("clears stale password when switching an account to token auth", () => {
		...
		expect(updated.channels?.matrix?.accounts?.default?.accessToken).toBe(
			"new-token",
		);
		expect(
