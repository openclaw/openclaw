...
vi.mock("./matrix/actions/verification.js", () => ({
	bootstrapMatrixVerification: (...args: unknown[]) =>
		bootstrapMatrixVerificationMock(...args),
	getMatrixRoomKeyBackupStatus: (...args: unknown[]) =>
		getMatrixRoomKeyBackupStatusMock(...args),
	getMatrixVerificationStatus: (...args: unknown[]) =>
		getMatrixVerificationStatusMock(...args),
	resetMatrixRoomKeyBackup: (...args: unknown[]) =>
		resetMatrixRoomKeyBackupMock(...args),
	restoreMatrixRoomKeyBackup: (...args: unknown[]) =>
		restoreMatrixRoomKeyBackupMock(...args),
	verifyMatrixRecoveryKey: (...args: unknown[]) =>
		verifyMatrixRecoveryKeyMock(...args),
}));

vi.mock("./matrix/actions/devices.js", () => ({
	listMatrixOwnDevices: (...args: unknown[]) =>
		listMatrixOwnDevicesMock(...args),
	pruneMatrixStaleGatewayDevices: (...args: unknown[]) =>
		pruneMatrixStaleGatewayDevicesMock(...args),
}));

vi.mock("./matrix/client/logging.js", () => ({
	setMatrixSdkConsoleLogging: (...args: unknown[]) =>
		setMatrixSdkConsoleLoggingMock(...args),
	setMatrixSdkLogMode: (...args: unknown[]) => setMatrixSdkLogModeMock(...args),
}));

vi.mock("./matrix/actions/profile.js", () => ({
	updateMatrixOwnProfile: (...args: unknown[]) =>
		updateMatrixOwnProfileMock(...args),
}));

vi.mock("./matrix/accounts.js", () => ({
	resolveMatrixAccount: (...args: unknown[]) =>
		resolveMatrixAccountMock(...args),
	resolveMatrixAccountConfig: (...args: unknown[]) =>
		resolveMatrixAccountConfigMock(...args),
}));

vi.mock("./matrix/client.js", () => ({
	resolveMatrixAuthContext: (...args: unknown[]) =>
		resolveMatrixAuthContextMock(...args),
}));

vi.mock("./setup-core.js", () => ({
	matrixSetupAdapter: {
		applyAccountConfig: (...args: unknown[]) =>
			matrixSetupApplyAccountConfigMock(...args),
		validateInput: (...args: unknown[]) =>
			matrixSetupValidateInputMock(...args),
	},
}));
...
vi.mock("./runtime.js", () => ({
	getMatrixRuntime: () => ({
		config: {
			...
			writeConfigFile: (...args: unknown[]) =>
				matrixRuntimeWriteConfigFileMock(...args),
		},
	}),
}));
...
function formatExpectedLocalTimestamp(value: string): string {
	return (
		formatZonedTimestamp(new Date(value), { displaySeconds: true }) ?? value
	);
}
...
describe("matrix CLI verification commands", () => {
	...
	it("sets non-zero exit code for device verification failures in JSON mode", async () => {
		...
		await program.parseAsync(
			["matrix", "verify", "device", "bad-key", "--json"],
			{
				from: "user",
			},
		);
	...
	it("sets non-zero exit code for backup restore failures in JSON mode", async () => {
		...
		await program.parseAsync(
			["matrix", "verify", "backup", "restore", "--json"],
			{
				from: "user",
			},
		);
	...
	it("sets non-zero exit code for backup reset failures in JSON mode", async () => {
		...
		await program.parseAsync(
			["matrix", "verify", "backup", "reset", "--yes", "--json"],
			{
				from: "user",
			},
		);
	...
	it("lists matrix devices", async () => {
		...
		await program.parseAsync(
			["matrix", "devices", "list", "--account", "poe"],
			{ from: "user" },
		);
	...
	it("prunes stale matrix gateway devices", async () => {
		...
		await program.parseAsync(
			["matrix", "devices", "prune-stale", "--account", "poe"],
			{
				from: "user",
			},
		);
	...
	it("bootstraps verification for newly added encrypted accounts", async () => {
		...
		expect(console.log).toHaveBeenCalledWith(
			"Matrix verification bootstrap: complete",
		);
		expect(console.log).toHaveBeenCalledWith(
	...
	it("warns instead of failing when device-health probing fails after saving the account", async () => {
		listMatrixOwnDevicesMock.mockRejectedValue(
	...
	it("uses --name as fallback account id and prints account-scoped config path", async () => {
		...
		expect(console.log).toHaveBeenCalledWith(
			"Config path: channels.matrix.accounts.main-bot",
		);
	...
	it("sets profile name and avatar via profile set command", async () => {
		...
		expect(console.log).toHaveBeenCalledWith(
			"Config path: channels.matrix.accounts.alerts",
		);
	});

	it("returns JSON errors for invalid account setup input", async () => {
		matrixSetupValidateInputMock.mockReturnValue(
			"Matrix requires --homeserver",
		);
		const program = buildProgram();
	...
	it("prints local timezone timestamps for verify bootstrap and device output in verbose mode", async () => {
		...
		await program.parseAsync(
			["matrix", "verify", "device", "valid-key", "--verbose"],
			{
				from: "user",
			},
		);
	...
	it("prints backup health lines for verify backup status in verbose mode", async () => {
		...
		await program.parseAsync(
			["matrix", "verify", "backup", "status", "--verbose"],
			{
				from: "user",
			},
		);
