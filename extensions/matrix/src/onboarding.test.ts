...
describe("matrix onboarding", () => {
	...
	it("offers env shortcut for non-default account when scoped env vars are present", async () => {
		...
		if (result !== "skip") {
			...
			expect(
				result.cfg.channels?.matrix?.accounts?.ops?.homeserver,
			).toBeUndefined();
			expect(
				result.cfg.channels?.matrix?.accounts?.ops?.accessToken,
			).toBeUndefined();
		}
		expect(
	...
	it("reports account-scoped DM config keys for named accounts", () => {
		const resolveConfigKeys =
			matrixOnboardingAdapter.dmPolicy?.resolveConfigKeys;
