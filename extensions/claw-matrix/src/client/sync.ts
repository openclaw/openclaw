...
function markEventProcessed(eventId: string): void {
  if (processedEventSet.has(eventId)) return;
  processedEventSet.add(eventId);
  processedEventIds.push(eventId);
  // FIFO eviction
  while (processedEventIds.length > DEDUP_MAX) {
    const evicted = processedEventIds.shift()!;
    processedEventSet.delete(evicted);
  }
}

function isEventProcessed(eventId: string): boolean {
  return processedEventSet.has(eventId);
}
...
function loadSyncToken(storePath: string): string | undefined {
  ...
  try {
    ...
    // Restore dedup set from persisted state (capped to prevent memory bloat)
    if (Array.isArray(data.processedEvents)) {
      const events = data.processedEvents.slice(-DEDUP_MAX);
...
export async function runSyncLoop(opts: SyncLoopOpts): Promise<void> {
  ...
  while (!abortSignal.aborted) {
    try {
      ...
      //    oneTimeKeyCounts: Map<string, number>, unusedFallbackKeys: Array<string>)
      ...
      await retryUTDQueue(onMessage, log, opts.backupDecryptionKey, opts.backupVersion);

      // Save sync token early — after crypto state ingestion + UTD retry,
      // before timeline processing. Crash during timeline re-dispatches
      // messages (cheap) rather than re-processing crypto state.
      if (response.next_batch) {
        nextBatch = response.next_batch;
        saveSyncToken(cryptoStorePath, nextBatch);
      }

      // ── Step 2b: Invalidate m.direct cache if account data changed ──
      const globalAccountData = response.account_data?.events ?? [];
      if (globalAccountData.some((e) => e.type === "m.direct")) {
        invalidateMDirectCache();
      }
      ...
      for (const [roomId, room] of Object.entries(joinedRooms)) {
        ...
        if (room.timeline?.events) {
          ...
          // Process message events (with dedup guard)
          for (const event of room.timeline.events) {
            ...
            if (event.event_id) markEventProcessed(event.event_id);
          }

          // 3c. Send read receipt for last event (skip own messages)
          const lastEvent = room.timeline.events[room.timeline.events.length - 1];
          if (lastEvent?.event_id && lastEvent.sender !== userId) {
            sendReadReceipt(roomId, lastEvent.event_id).catch(() => {});
          }
        }
      }

      // ── Step 4: Process rooms.invite ──
      const invitedRooms = response.rooms?.invite ?? {};
      for (const [roomId, inviteData] of Object.entries(invitedRooms)) {
        await handleRoomInvite(roomId, inviteData as any, opts, log);
      }

      // ── Step 5: Process rooms.leave ──
      const leftRooms = response.rooms?.leave ?? {};
      for (const [roomId, leaveData] of Object.entries(leftRooms)) {
        // Extract leave reason from timeline events (kick/ban carry content.reason)
        const leaveEvents = (leaveData as any)?.timeline?.events ?? [];
      ...
      updateCryptoMetrics(utdQueue.length);
      ...
      // Timeouts are expected during long-poll — don't count as failures
      if (err instanceof MatrixTimeoutError) {
        log?.info?.(`[sync] Sync request timed out (expected for long-poll), retrying`);
        continue;
      }
...
async function processTimelineEvent(
  ...
  if (event.type === "m.room.encrypted") {
    ...
    try {
      ...
      if (fullEvent.type === "m.room.message" || fullEvent.type === "m.reaction") {
          ...
          try {
            ...
            if (restored) {
              ...
              try {
                ...
                // Backup restore succeeded but decrypt still failed — stay in queue
              }
            }
          } catch {
            // Backup fetch failed — stay in queue for next cycle
          }
        }
      }
    }
...
async function handleTokenError(
  ...
  if (err.softLogout && opts.password) {
    ...
    await closeMachine();
    // Small delay to let any in-flight FFI callbacks settle
    await new Promise((r) => setTimeout(r, 200));
    try {
      fs.rmSync(opts.cryptoStorePath, { recursive: true, force: true });
    } catch (rmErr: any) {
      log?.warn?.(`[sync] Failed to wipe crypto store: ${rmErr.message}`);
    }
...
let joinCount = 0;
let joinWindowStart = 0;
const JOIN_RATE_LIMIT = 3;
...
async function handleRoomInvite(
  ...
  if (opts.autoJoin === "off" || !opts.autoJoin) {
    log?.info?.(`[sync] Invited to ${roomId} by ${inviter} (auto-join off, ignoring)`);
    return;
  }
  ...
  // Rate limit joins
  const now = Date.now();
  if (now - joinWindowStart > JOIN_RATE_WINDOW_MS) {
    joinCount = 0;
    joinWindowStart = now;
  }
  ...
  try {
    ...
    joinCount++;
    log?.info?.(`[sync] Auto-joined ${roomId} (invited by ${inviter})`);

    // Process invite state to populate caches
    if (inviteEvents.length > 0) {
      processStateEvents(roomId, inviteEvents);
    }
  } catch (err: any) {
    log?.error?.(`[sync] Failed to auto-join ${roomId}: ${err.message}`);
  }
}
