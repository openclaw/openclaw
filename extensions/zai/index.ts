...
async function promptForZaiEndpoint(
	ctx: ProviderAuthContext,
): Promise<ZaiEndpointId> {
...
async function runZaiApiKeyAuth(
	...
	const nextEndpoint =
		detected?.endpoint ?? endpoint ?? (await promptForZaiEndpoint(ctx));
	return {
		profiles: [
			{
...
function buildZaiApiKeyMethod(params: {
	...
	return {
		...
		runNonInteractive: async (ctx) =>
			await runZaiApiKeyAuthNonInteractive(ctx, params.endpoint),
	};
}
	...
	register(api) {
		api.registerProvider({
			...
			resolveUsageAuth: async (ctx) => {
				...
				const legacyToken = resolveLegacyPiAgentAccessToken(ctx.env, [
					"z-ai",
					"zai",
				]);
			...
			fetchUsageSnapshot: async (ctx) =>
				await fetchZaiUsage(ctx.token, ctx.timeoutMs, ctx.fetchFn),
