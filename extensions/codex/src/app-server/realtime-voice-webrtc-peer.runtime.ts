// Lazy Codex realtime media runtime: werift peer plus WASM Opus framing and PCM conversion.
import { randomInt } from "node:crypto";
import { resamplePcm } from "openclaw/plugin-sdk/realtime-voice";
import {
  appendCodexRealtimePendingAudio,
  CODEX_REALTIME_RELAY_FRAME_BYTES,
} from "./realtime-voice-audio-buffer.js";

const CODEX_REALTIME_SAMPLE_RATE = 48_000;
const RELAY_SAMPLE_RATE = 24_000;
const CODEX_REALTIME_CHANNELS = 2;
...
export type CodexRealtimeAudioPeerCallbacks = {
  onAudio: (audio: Buffer) => void;
  onError: (error: Error) => void;
  onRtpPacket?: () => void;
};

export type CodexRealtimeAudioPeerContract = {
...
function convertRelayPcmToCodexRealtimePcm(pcm24kMono: Buffer): Int16Array {
  const mono48k = pcmBufferToInt16(
    resamplePcm(pcm24kMono, RELAY_SAMPLE_RATE, CODEX_REALTIME_SAMPLE_RATE),
  );
  const stereo48k = new Int16Array(mono48k.length * CODEX_REALTIME_CHANNELS);
...
function convertCodexRealtimePcmToRelayPcm(pcm48kStereo: Int16Array): Buffer {
  const frameCount = Math.floor(pcm48kStereo.length / CODEX_REALTIME_CHANNELS);
  ...
  return resamplePcm(mono48k, CODEX_REALTIME_SAMPLE_RATE, RELAY_SAMPLE_RATE);
}
...
export class CodexRealtimeAudioPeer implements CodexRealtimeAudioPeerContract {
  static async create(params: {
    callbacks: CodexRealtimeAudioPeerCallbacks;
    iceServers?: Array<{ urls: string | string[]; username?: string; credential?: string }>;
    signal?: AbortSignal;
  }): Promise<CodexRealtimeAudioPeer> {
    ...
    try {
      encoder = await libopus.createEncoder({
        ...
        channels: CODEX_REALTIME_CHANNELS,
        sampleRate: CODEX_REALTIME_SAMPLE_RATE,
      ...
      return new CodexRealtimeAudioPeer({
  ...
  static convertRelayPcm(pcm24kMono: Buffer): Int16Array {
    return convertRelayPcmToCodexRealtimePcm(pcm24kMono);
  }

  static convertCodexRealtimePcm(pcm48kStereo: Int16Array): Buffer {
    return convertCodexRealtimePcmToRelayPcm(pcm48kStereo);
  }
    ...
    state.peer.connectionStateChange.subscribe((connectionState) => {
      ...
      if (connectionState === "connected") {
        ...
        this.state.callbacks.onError(
          new Error(`Codex realtime WebRTC media connection ${connectionState}`),
        );
      }
    });
  }
  ...
  async createOffer(): Promise<string> {
    ...
    if (!sdp?.trim()) {
      throw new Error("werift did not produce a Codex realtime SDP offer");
    }
    return sdp;
  }
  ...
  sendAudio(audio: Buffer): void {
    ...
    this.pendingAudio = appendCodexRealtimePendingAudio(this.pendingAudio, audio);
  }
  ...
  private handleInboundRtp(packet: WeriftRtpPacket): void {
    ...
    try {
      ...
      if (this.activeInboundSsrc === undefined) {
        ...
        throw new Error("Codex realtime WebRTC audio source changed unexpectedly");
      }
      ...
      if (distance >= RTP_SEQUENCE_HALF_RANGE) {
        ...
        throw new Error("Codex realtime WebRTC RTP sequence changed unexpectedly");
      }
  ...
  private emitInboundPcm(decoded: Int16Array): void {
    const relayPcm = convertCodexRealtimePcmToRelayPcm(decoded);
  ...
  private sendNextAudioFrame(): void {
    ...
    try {
      const opusPacket = this.state.encoder.encode(convertRelayPcmToCodexRealtimePcm(frame), {
        frameSize: OPUS_FRAME_SAMPLES,
      });
  ...
  private takeNextRelayFrame(): Buffer {
    ...
    const frame = Buffer.alloc(CODEX_REALTIME_RELAY_FRAME_BYTES);
    const queuedBytes = Math.min(this.pendingAudio.length, CODEX_REALTIME_RELAY_FRAME_BYTES);
    if (queuedBytes > 0) {
