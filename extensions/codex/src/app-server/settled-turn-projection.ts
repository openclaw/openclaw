...
function createProjectedCallIdResolver(reservedIds: ReadonlySet<string>): (id: string) => string {
  ...
  // or on one an earlier rewrite produced: Codex would see different calls under
  // the same wire id, making a complete transcript ambiguous -- the same fail-closed turn this
...
export class SettledTurnPriorContext {
  ...
  /** IDs leave the projection only here, after streaming preserves early limit rejection. */
  prependTo(current: JsonValue[]): JsonValue[] {
    ...
    return rewriteOverlengthCallIds(
      [
