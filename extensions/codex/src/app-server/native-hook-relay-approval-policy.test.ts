...
describe("resolveCodexNativeHookRelayForApprovalPolicy", () => {
  ...
  it("retains the before-tool policy relay when a policy is active under an explicit never", () => {
    // Explicit yolo/never with a live before_tool_call hook: clearing the relay
    // here would drop the very relay that executes and can block that policy, so
    // the opt-out narrows to the floor instead.
    const warn = vi.fn();
    initializeGlobalHookRunner(
