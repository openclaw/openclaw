...
describe("CodexAppServerEventProjector", () => {
  ...
  it("does not promote synthesized missing-tool-result to run error for cancelled interrupted turn with in-progress command", async () => {
    const projector = await createProjector();
  ...
  it("promotes synthesized missing-tool-result error for non-aborted interrupted turn", async () => {
    const projector = await createProjector();
    ...
    // The run is NOT aborted — no OpenClaw cancellation was observed.
    expect(result.aborted).toBe(false);
    // The synthesized missing-tool-result error IS promoted to promptError
    // because the interruption was not a user cancellation.
    expect(result.promptError).not.toBeNull();
    // The partial tool output meta is preserved for downstream guards.
    expect(result.toolMetas).toEqual([
      expect.objectContaining({
        toolName: "bash",
