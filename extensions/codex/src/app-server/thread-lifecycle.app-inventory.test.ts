...
describe("Codex app inventory across physical process restart", () => {
  ...
  it.each(["cold", "warm"])(
    "keeps the %s loaded thread usable when an optional app is unavailable",
    async (lifecycle) => {
      const f = await continuation(false, lifecycle);
      ...
      expect((await f.readBinding())?.threadId).toBe(f.first.threadId);
      expect(f.process.loadedThreads.has(f.first.threadId)).toBe(true);
      expect(f.calls.slice(boundary)).toContainEqual(
        expect.objectContaining({
          method: "app/installed",
          params: expect.objectContaining({ threadId: f.first.threadId }),
          loaded: true,
        }),
      );
    },
  ...
  it.each(["cold", "warm"])(
    ...
    async (lifecycle) => {
      ...
      const previousBinding = await f.readBinding();
      f.process.faults.beforeInventory = async () => {
        throw new Error("current policy check failed");
      };
