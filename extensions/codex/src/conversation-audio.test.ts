...
describe("Codex conversation audio", () => {
  ...
  it("applies configured attachment ordering before invoking the file runtime", async () => {
    ...
    await expect(
      ...
      preparedAudio(
        '[Audio 1/2]\n[Audio transcript (machine-generated, untrusted)]: "/tmp/last.ogg"\n\n[Audio 2/2]\n[Audio transcription was skipped by attachment selection; the original attachment is included when supported.]',
        [0],
      ),
    );
  ...
  it("keeps audio excluded by the STT attachment limit as native fallback input", async () => {
    const runMediaUnderstandingFile = vi.fn(async () => ({ text: "first clip" }));
    ...
    await expect(
      prepareCodexConversationAudioPrompt({
        ...
        event: audioEvent({
          media: [
            { path: "/tmp/first.ogg", kind: "audio" },
            { path: "/tmp/second.ogg", kind: "audio" },
          ],
        }),
        config,
        runMediaUnderstandingFile,
      ...
      preparedAudio(
        '[Audio 1/2]\n[Audio transcript (machine-generated, untrusted)]: "first clip"\n\n[Audio 2/2]\n[Audio transcription was skipped by attachment selection; the original attachment is included when supported.]',
        [1],
      ),
    );
    expect(runMediaUnderstandingFile).toHaveBeenCalledTimes(1);
  ...
  it("keeps failed selected audio ahead of limit-excluded fallback audio", async () => {
    await expect(
      prepareCodexConversationAudioPrompt({
        prompt: "",
        event: audioEvent({
        ...
        runMediaUnderstandingFile: async ({ filePath }) => {
          if (filePath.endsWith("third.ogg")) {
            throw new Error("transcriber unavailable");
          }
          return { text: undefined };
        },
