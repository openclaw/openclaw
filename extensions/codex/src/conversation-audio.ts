...
export async function prepareCodexConversationAudioPrompt(params: {
  ...
  const selectedIndexes = new Set(selectedAudio.map((audio) => audio.index));
  const unselectedAudio = audioAttachments.filter(
    (audio) => !audio.alreadyTranscribed && !selectedIndexes.has(audio.index),
  );
  const orderedAudioCount = selectedAudio.length + unselectedAudio.length;
  ...
  if (!params.config || !params.runMediaUnderstandingFile) {
    ...
    appendUnselectedAudioFallbacks({
      selectedCount: selectedAudio.length,
      unselectedAudio,
      audioInputAttachmentIndexes,
      promptParts,
    });
    return {
      prompt: appendPreparedAudioPrompt(params.prompt, promptParts, orderedAudioCount),
      audioInputAttachmentIndexes,
    };
  }
...
function appendUnselectedAudioFallbacks(params: {
  selectedCount: number;
  unselectedAudio: CodexConversationAudioAttachment[];
  audioInputAttachmentIndexes: number[];
  promptParts: Array<{ ordinal: number; text: string }>;
}): void {
  for (const [unselectedIndex, audio] of params.unselectedAudio.entries()) {
    params.audioInputAttachmentIndexes.push(audio.index);
    params.promptParts.push({
      ordinal: params.selectedCount + unselectedIndex + 1,
      text: "[Audio transcription was skipped by attachment selection; the original attachment is included when supported.]",
    });
  }
}
