...
async function requestCodexAppServerUsageLazy(options: {
  ...
  authProfileId?: string;
  config?: Parameters<
    typeof import("./src/app-server/request.js").withCodexAppServerRateLimitsClient
  >[0]["config"];
  startOptions?: CodexAppServerStartOptions;
}): Promise<{ rateLimits: unknown; accountEmail?: string }> {
  const { withCodexAppServerRateLimitsClient } = await import("./src/app-server/request.js");
  ...
  // The rate-limit helper leases the auth-matched shared client. The follow-up
  // account read stays on that client, so its identity owns these windows.
  return await withCodexAppServerRateLimitsClient(
    {
...
// Leave a small margin before the shared usage deadline so returning the
// already-read windows wins over a best-effort identity timeout.
...
async function readCodexAccountEmailBestEffort(
  ...
  try {
    ...
    // When the timer wins, leaving the client scope aborts the pending read; its
    // rejection is already swallowed, so there is nothing to leak.
  }
}
