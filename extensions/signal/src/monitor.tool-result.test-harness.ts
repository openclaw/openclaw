...
vi.mock("./daemon.js", async () => {
	const actual =
		await vi.importActual<typeof import("./daemon.js")>("./daemon.js");
	return {
		...actual,
...
vi.mock("openclaw/plugin-sdk/channel-runtime", async () => {
	...
	return {
		...actual,
		enqueueSystemEvent: (
			...args: Parameters<typeof actual.enqueueSystemEvent>
		) => {
...
export function installSignalToolResultTestHooks() {
	beforeEach(async () => {
		const [{ resetInboundDedupe }, { resetSystemEventsForTest }] =
			await Promise.all([
