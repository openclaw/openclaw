...
describe("signalMessageActions", () => {
	...
	it("lists actions based on configured accounts and reaction gates", () => {
		expect(
			signalMessageActions.describeMessageTool?.({ cfg: {} as OpenClawConfig })
				?.actions ?? [],
		).toEqual([]);
	...
	it("skips send for plugin dispatch", () => {
		expect(signalMessageActions.supportsAction?.({ action: "send" })).toBe(
			false,
		);
		expect(signalMessageActions.supportsAction?.({ action: "react" })).toBe(
			true,
		);
	});
	...
	it("maps reaction targets into sendReactionSignal calls", async () => {
		...
		for (const testCase of cases) {
			...
			await signalMessageActions.handleAction?.({
				...
				toolContext:
					"toolContext" in testCase ? testCase.toolContext : undefined,
			});
