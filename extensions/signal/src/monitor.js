...
async function monitorSignalProvider(opts = {}) {
  ...
  const groupAllowFrom = normalizeAllowList(
    opts.groupAllowFrom ?? accountInfo.config.groupAllowFrom ?? (accountInfo.config.allowFrom && accountInfo.config.allowFrom.length > 0 ? accountInfo.config.allowFrom : [])
  );
  ...
  try {
    if (daemonHandle) {
      ...
      const daemonExitError2 = daemonLifecycle.getExitError();
      if (daemonExitError2) {
        throw daemonExitError2;
      }
    }
