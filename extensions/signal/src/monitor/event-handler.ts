...
function formatAttachmentSummaryPlaceholder(
...
export function createSignalEventHandler(deps: SignalEventHandlerDeps) {
		...
		createChannelInboundDebouncer<SignalInboundEntry>({
			...
			shouldDebounce: (entry) => {
				return shouldDebounceTextInbound({
					...
					cfg: deps.cfg,
					hasMedia: Boolean(
						entry.mediaPath || entry.mediaType || entry.mediaPaths?.length,
					),
				});
			},
	...
	return async (event: { event?: string; data?: string }) => {
		...
		const normalizedAccount = deps.account
			? normalizeE164(deps.account)
			: undefined;
		const isOwnMessage =
			(sender.kind === "phone" &&
				normalizedAccount != null &&
				sender.e164 === normalizedAccount) ||
			(sender.kind === "uuid" &&
				deps.accountUuid != null &&
				sender.raw === deps.accountUuid);
		if (isOwnMessage) {
			return;
		}
		...
		const normalizedMessage = renderSignalMentions(
			rawMessage,
		...
		const wasMentioned =
			isGroup && matchesMentionPatterns(messageText, mentionRegexes);
				...
				try {
					...
					if (fetched) {
						...
						if (!mediaPath) {
							...
							mediaType =
								fetched.contentType ?? attachment.contentType ?? undefined;
						}
					}
				} catch (err) {
					deps.runtime.error?.(
						danger(`attachment fetch failed: ${String(err)}`),
					);
				}
			}
		}
