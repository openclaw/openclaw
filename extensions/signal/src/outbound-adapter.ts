...
function resolveSignalSender(deps: OutboundSendDeps | undefined) {
	return (
		resolveOutboundSendDep<typeof sendMessageSignal>(deps, "signal") ??
		sendMessageSignal
	);
}

const resolveSignalMaxBytes =
	createScopedChannelMediaMaxBytesResolver("signal");
type SignalSendOpts = NonNullable<Parameters<typeof sendMessageSignal>[2]>;

function inferSignalTableMode(params: {
	cfg: SignalSendOpts["cfg"];
