	...
	return await new Promise((resolve, reject) => {
		...
		child.on("close", (code) => {
			...
			if (timedOut) {
				reject(
					new Error(
						`command timed out: ${params.command} ${params.args.join(" ")}`,
					),
				);
				return;
			}
...
async function runBackendExec(params: {
	backend: Awaited<
		ReturnType<ReturnType<typeof createOpenShellSandboxBackendFactory>>
	>;
	command: string;
	allowFailure?: boolean;
	timeoutMs?: number;
	...
	it.runIf(process.platform !== "win32" && OPENCLAW_OPENSHELL_E2E)(
		...
		async () => {
			...
			const rootDir = await fs.mkdtemp(
				path.join(os.tmpdir(), "openclaw-openshell-e2e-"),
			);
			...
			try {
				...
				await fs.writeFile(
					path.join(workspaceDir, "seed.txt"),
					"seed-from-local\n",
					"utf8",
				);
				...
				expect(trimTrailingNewline(curlPathResult.stdout.trim())).toMatch(
					/^\/.+\/curl$/,
				);
				...
				if (!bridge) {
					throw new Error(
						"openshell backend did not create a filesystem bridge",
					);
				}
				...
				await expect(
					fs.readFile(
						path.join(workspaceDir, "nested", "remote-only.txt"),
						"utf8",
					),
				).rejects.toThrow();
				await expect(
					bridge.readFile({ filePath: "nested/remote-only.txt" }),
				).resolves.toEqual(Buffer.from("hello-remote\n"));
				...
				expect(
					`${blockedGetResult.stdout}\n${blockedGetResult.stderr}`,
				).toMatch(/403|deny/i);
				...
				expect(allowedGetResult.stdout).toContain(
