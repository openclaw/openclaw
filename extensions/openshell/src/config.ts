...
const OpenShellPluginConfigSchema = z.strictObject({
	...
	gateway: nonEmptyTrimmedString(
		"gateway must be a non-empty string",
	).optional(),
	gatewayEndpoint: nonEmptyTrimmedString(
		"gatewayEndpoint must be a non-empty string",
	).optional(),
	...
	autoProviders: z
		.boolean({ error: "autoProviders must be a boolean" })
		.optional(),
...
export function resolveOpenShellPluginConfig(
	value: unknown,
