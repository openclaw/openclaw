...
describe("replaceDirectoryContents", () => {
	...
	it("excludes specified directories from sync", async () => {
		...
		await fs.writeFile(
			path.join(target, "hooks", "trusted", "handler.js"),
			"// trusted code",
		);
		...
		expect(await fs.readFile(path.join(target, "code.txt"), "utf8")).toBe(
			"legit",
		);

		// Old non-excluded content is removed
		await expect(
			fs.access(path.join(target, "existing.txt")),
		).rejects.toThrow();
		...
		await expect(
			fs.access(path.join(target, "hooks", "evil")),
		).rejects.toThrow();
	});
	...
	it("preserves default excluded directories and repository metadata", async () => {
		...
		await fs.writeFile(
			path.join(source, "git-hooks", "pre-commit"),
			"malicious",
		);
		await fs.mkdir(path.join(source, ".git", "hooks"), { recursive: true });
		await fs.writeFile(
			path.join(source, ".git", "hooks", "post-checkout"),
			"malicious",
		);
		...
		expect(
			await fs.readFile(path.join(target, "hooks", "trusted"), "utf8"),
		).toBe("trusted");
		expect(
			await fs.readFile(path.join(target, "git-hooks", "trusted"), "utf8"),
		).toBe("trusted");
		expect(await fs.readFile(path.join(target, ".git", "HEAD"), "utf8")).toBe(
			"ref: refs/heads/main\n",
		);
		await expect(
			fs.access(path.join(target, ".git", "hooks", "post-checkout")),
		).rejects.toThrow();
	});
	...
	it("skips symbolic links when copying into the host workspace", async () => {
		...
		await expect(
			fs.lstat(path.join(target, "nested", "escaped-dir")),
		).rejects.toThrow();
	});
	...
	it("preserves existing trusted host symlinks", async () => {
		...
		expect(await fs.readFile(path.join(target, "safe.txt"), "utf8")).toBe("ok");
		expect(await fs.readlink(path.join(target, "linked-entry"))).toBe(
			"/tmp/trusted-host-target",
		);
	});
});
...
describe("stageDirectoryContents", () => {
	it("stages upload content without symbolic links", async () => {
		...
		expect(
			await fs.readFile(path.join(staged, "nested", "file.txt"), "utf8"),
		).toBe("nested");
		await expect(fs.lstat(path.join(staged, "escaped-link"))).rejects.toThrow();
		await expect(
			fs.lstat(path.join(staged, "nested", "escaped-dir")),
		).rejects.toThrow();
	});
});
