...
async function lstatIfExists(targetPath: string) {
	return await runLimitedFs(async () => await fs.lstat(targetPath)).catch(
		() => null,
	);
}
...
async function copyTreeWithoutSymlinks(params: {
	...
	const stats = await runLimitedFs(
		async () => await fs.lstat(params.sourcePath),
	);
	...
	if (stats.isDirectory()) {
		await runLimitedFs(
			async () => await fs.mkdir(params.targetPath, { recursive: true }),
		);
		const entries = await runLimitedFs(
			async () => await fs.readdir(params.sourcePath),
		);
		await Promise.all(
	...
	if (stats.isFile()) {
		...
		await runLimitedFs(
			async () => await fs.copyFile(params.sourcePath, params.targetPath),
		);
	}
}
