	...
	return {
		async describeRuntime({ entry, config }) {
			const execContext: OpenShellExecContext = {
				config: resolveOpenShellPluginConfigFromConfig(
					config,
					params.pluginConfig,
				),
...
async function createOpenShellSandboxBackend(params: {
	...
	if ((params.createParams.cfg.docker.binds?.length ?? 0) > 0) {
		throw new Error(
			"OpenShell sandbox backend does not support sandbox.docker.binds.",
		);
	}
	...
	return {
		...
		runShellCommand: async (command) =>
			await impl.runRemoteShellScript(command),
...
class OpenShellSandboxBackendImpl {
	...
	asHandle(): OpenShellSandboxBackend {
		return {
			...
			runShellCommand: async (command) =>
				await this.runRemoteShellScript(command),
			createFsBridge: ({ sandbox }) =>
	...
	async syncLocalPathToRemote(
		...
		if (result.code !== 0) {
			throw new Error(
				result.stderr.trim() || "openshell sandbox upload failed",
			);
		}
	}
	...
	private async ensureSandboxExistsInner(): Promise<void> {
		...
		const createArgs = [
			...
			...(this.params.execContext.config.autoProviders
				? ["--auto-providers"]
				: ["--no-auto-providers"]),
			...this.params.execContext.config.providers.flatMap((provider) => [
				"--provider",
				provider,
			]),
			"--",
		...
		if (createResult.code !== 0) {
			throw new Error(
				createResult.stderr.trim() || "openshell sandbox create failed",
			);
		}
	...
	private async syncWorkspaceFromRemote(): Promise<void> {
		...
		try {
			...
			if (result.code !== 0) {
				throw new Error(
					result.stderr.trim() || "openshell sandbox download failed",
				);
			}
