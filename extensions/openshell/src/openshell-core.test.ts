...
describe("openshell plugin config", () => {
	...
	it("accepts remote mode", () => {
		expect(resolveOpenShellPluginConfig({ mode: "remote" }).mode).toBe(
			"remote",
		);
	});
	...
	it("keeps the runtime json schema in sync with the manifest config schema", () => {
		...
		expect(createOpenShellPluginConfigSchema().jsonSchema).toEqual(
...
describe("openshell cli helpers", () => {
	...
	it("prefers the bundled openshell command when available", () => {
		setBundledOpenShellCommandResolverForTest(
			() => "/tmp/node_modules/.bin/openshell",
		);
		const config = resolveOpenShellPluginConfig(undefined);

		expect(resolveOpenShellCommand("openshell")).toBe(
			"/tmp/node_modules/.bin/openshell",
		);
		expect(buildOpenShellBaseArgv(config)).toEqual([
			"/tmp/node_modules/.bin/openshell",
	...
	it("wraps exec commands with env and workdir", () => {
		...
		expect(command).toContain(
			`'cd '"'"'/sandbox/project'"'"' && pwd && printenv TOKEN'`,
		);
	});
});
...
async function runLocalShell(params: {
	...
	const translatedArgs = (params.args ?? []).map((arg) =>
		translateRemotePath(arg, params.roots),
	);
	const stdinBuffer =
	...
	return {
		...result,
		stdout: Buffer.from(
			rewriteLocalPaths(result.stdout.toString("utf8"), params.roots),
			"utf8",
		),
	};
}
...
async function emulateRemoteShell(params: {
	...
	try {
		if (params.script === 'set -eu\ncat -- "$1"') {
			return {
				stdout: await fs.readFile(params.args[0] ?? ""),
				stderr: Buffer.alloc(0),
				code: 0,
			};
		}
		...
		if (params.script.includes('canonical=$(readlink -f -- "$cursor")')) {
			const canonical = await resolveCanonicalPath(
				params.args[0] ?? "",
				params.args[1] === "1",
			);
			return {
				stdout: Buffer.from(`${canonical}\n`),
				stderr: Buffer.alloc(0),
				code: 0,
			};
		}
...
async function applyMutation(args: string[], stdin?: Buffer) {
	...
	if (operation === "mkdirp") {
		...
		await fs.mkdir(path.join(root ?? "", relativePath ?? ""), {
			recursive: true,
		});
		return;
	}
	...
	if (operation === "rename") {
		const [srcRoot, srcParent, srcBase, dstRoot, dstParent, dstBase, mkdir] =
			args.slice(1);
...
describe("openshell fs bridges", () => {
	it("writes locally and syncs the file to the remote workspace", async () => {
		...
		expect(
			await fs.readFile(path.join(workspaceDir, "nested", "file.txt"), "utf8"),
		).toBe("hello");
	...
	it("maps agent mount paths when the sandbox workspace is read-only", async () => {
		...
		await fs.writeFile(
			path.join(agentWorkspaceDir, "note.txt"),
			"agent",
			"utf8",
		);
		const backend = createMirrorBackendMock();
		...
		expect(await bridge.readFile({ filePath: "/agent/note.txt" })).toEqual(
			Buffer.from("agent"),
		);
	});
	...
	it("writes, reads, renames, and removes files without local host paths", async () => {
		...
		const remoteWorkspaceDir = await makeTempDir(
			"openclaw-openshell-remote-workspace-",
		);
		const remoteAgentDir = await makeTempDir(
			"openclaw-openshell-remote-agent-",
		);
		...
		const { createOpenShellRemoteFsBridge } = await import(
			"./remote-fs-bridge.js"
		);
		...
		expect(
			await fs.readFile(
				path.join(remoteWorkspaceRealDir, "nested", "file.txt"),
				"utf8",
			),
		).toBe("hello");
		...
		expect(await bridge.readFile({ filePath: "nested/file.txt" })).toEqual(
			Buffer.from("hello"),
		);
		...
		expect(
			await fs.readFile(
				path.join(remoteWorkspaceRealDir, "nested", "renamed.txt"),
				"utf8",
			),
		).toBe("hello");
