...
class OpenShellFsBridge implements SandboxFsBridge {
	...
	private resolveTarget(params: {
		...
		const hasAgentMount =
			this.sandbox.workspaceAccess !== "none" && workspaceRoot !== agentRoot;
		const agentContainerRoot = (
			this.backend.remoteAgentWorkspaceDir || "/agent"
		).replace(/\\/g, "/");
		const workspaceContainerRoot = this.sandbox.containerWorkdir.replace(
			/\\/g,
			"/",
		);
		...
		if (
			input.startsWith(`${workspaceContainerRoot}/`) ||
		...
		if (
			hasAgentMount &&
			(input.startsWith(`${agentContainerRoot}/`) ||
				input === agentContainerRoot)
		) {
			const relative = path.posix.relative(agentContainerRoot, input) || "";
			const hostPath = relative
				? path.resolve(agentRoot, ...relative.split("/"))
				: agentRoot;
			return {
				hostPath,
				relativePath: relative
					? `${agentContainerRoot}/${relative}`
					: agentContainerRoot,
				containerPath: relative
		...
		const hostPath = path.isAbsolute(input)
			? path.resolve(input)
			: path.resolve(cwd, input);

		if (isPathInside(workspaceRoot, hostPath)) {
			const relative = path
				.relative(workspaceRoot, hostPath)
				.split(path.sep)
				.join(path.posix.sep);
			return {
				hostPath,
		...
		throw new Error(
			`Path escapes sandbox root (${workspaceRoot}): ${params.filePath}`,
		);
	}
}
...
async function assertLocalPathSafety(params: {
	...
	if (!params.target.hostPath) {
		throw new Error(
			`Missing local host path for ${params.target.containerPath}`,
		);
	}
	...
	for (let index = 0; index < segments.length; index += 1) {
		...
		if (
			...
			(!isFinal || !params.allowFinalSymlinkForUnlink)
		) {
			throw new Error(
				`Sandbox boundary checks failed: ${params.target.containerPath}`,
			);
		}
	}
}
