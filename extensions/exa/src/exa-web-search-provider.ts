...
function normalizeExaFreshness(
	value: string | undefined,
): ExaFreshness | undefined {
	if (!value) {
		return undefined;
	}
...
function resolveExaConfig(searchConfig?: SearchConfigRecord): ExaConfig {
	...
	return exa && typeof exa === "object" && !Array.isArray(exa)
		? (exa as ExaConfig)
		: {};
}
...
function isErrorPayload(
	value: unknown,
): value is { error: string; message: string; docs: string } {
	return Boolean(
		value &&
			typeof value === "object" &&
			"error" in value &&
...
function parseExaContents(
	...
	const parseText = (
		...
		if (!value || typeof value !== "object" || Array.isArray(value)) {
			return invalidContentsPayload(
				"contents.text must be a boolean or an object.",
			);
		}
		...
		if (
			...
			parsePositiveInteger(obj.maxCharacters) === undefined
		) {
			return invalidContentsPayload(
	...
	const parseHighlights = (
		...
		if (
			"numSentences" in obj &&
			parsePositiveInteger(obj.numSentences) === undefined
		) {
			return invalidContentsPayload(
		...
		if (
			...
			parsePositiveInteger(obj.highlightsPerUrl) === undefined
		) {
			return invalidContentsPayload(
		...
		if ("query" in obj && typeof obj.query !== "string") {
			return invalidContentsPayload(
				"contents.highlights.query must be a string.",
			);
		}
		return {
			...(parsePositiveInteger(obj.maxCharacters)
	...
	const parseSummary = (
		...
		if (!value || typeof value !== "object" || Array.isArray(value)) {
			return invalidContentsPayload(
				"contents.summary must be a boolean or an object.",
			);
		}
		const obj = value as Record<string, unknown>;
		for (const key of Object.keys(obj)) {
	...
	return withTrustedWebSearchEndpoint(
		...
		async (res) => {
			if (!res.ok) {
				...
				throw new Error(
					`Exa API error (${res.status}): ${detail || res.statusText}`,
				);
			}
			try {
				return normalizeExaResults(await res.json());
			} catch (error) {
				throw new Error(`Exa API returned invalid JSON: ${String(error)}`, {
					cause: error,
				});
			}
		},
				...
				Type.Object(
					{
						...
						text: Type.Optional(
							Type.Unsafe<ExaTextContentsOption>({
								description:
									"Text config: true, or an object with maxCharacters.",
							}),
						),
		...
		execute: async (args) => {
			...
			const parsedDateRange = parseIsoDateRange({
				...
				invalidDateRangeMessage:
					"date_after must be earlier than or equal to date_before.",
			});
			...
			const payload = {
				...
				results: results.map((entry) => {
					...
					return {
						...
						...(summary
							? { summary: wrapWebContent(summary, "web_search") }
							: {}),
...
export function createExaWebSearchProvider(): WebSearchProviderPlugin {
	return {
		...
		getCredentialValue: (searchConfig) =>
			getScopedCredentialValue(searchConfig, "exa"),
		...
		applySelectionConfig: (config) =>
			enablePluginInConfig(config, "exa").config,
		createTool: (ctx) =>
