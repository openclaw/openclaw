...
export default definePluginEntry({
	...
	register(api) {
		...
		const config: WorkerConfig = {
			...
			maxThreads: (pluginConfig?.maxThreads as number) || Number(process.env.WORKER_MAX_THREADS || "4"),
			pollIntervalMs: (pluginConfig?.pollIntervalMs as number) || Number(process.env.WORKER_POLL_INTERVAL || "30") * 1000,
			worktreeBaseDir: (pluginConfig?.worktreeBaseDir as string) || process.env.WORKTREE_BASE_DIR || "/repo/worktrees",
		...
		if (config.maxThreads <= 0) {
			api.logger.info(`distributed-workers loaded on ${config.nodeName} but disabled (maxThreads=0)`);
			return;
		}
		...
		api.on("gateway_stop", async () => {
			...
			api.logger.info(`distributed-workers shutting down on ${config.nodeName}, ${executor.activeCount} tasks in flight`);
		});
