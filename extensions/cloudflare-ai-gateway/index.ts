		...
		api.registerProvider({
			...
			auth: [
				{
					...
					run: async (ctx) => {
						const metadata = await resolveCloudflareGatewayMetadataInteractive({
							accountId: normalizeOptionalSecretInput(
								ctx.opts?.cloudflareAiGatewayAccountId,
							),
							gatewayId: normalizeOptionalSecretInput(
								ctx.opts?.cloudflareAiGatewayGatewayId,
							),
						...
						await ensureApiKeyFromOptionEnvOrPrompt({
							token: normalizeOptionalSecretInput(
								ctx.opts?.cloudflareAiGatewayApiKey,
							),
					...
					runNonInteractive: async (ctx) => {
						...
						const storedMetadata = resolveMetadataFromCredential(
							authStore.profiles[PROFILE_ID],
						);
						const accountId =
							normalizeOptionalSecretInput(
								ctx.opts.cloudflareAiGatewayAccountId,
						...
						const resolved = await ctx.resolveApiKey({
							provider: PROVIDER_ID,
							flagValue: normalizeOptionalSecretInput(
								ctx.opts.cloudflareAiGatewayApiKey,
							),
						...
						return applyCloudflareAiGatewayConfig(next, {
							accountId,
							gatewayId,
						});
					},
			...
			catalog: {
				...
				run: async (ctx) => {
					...
					const envManagedApiKey = ctx.env[PROVIDER_ENV_VAR]?.trim()
						? PROVIDER_ENV_VAR
						: undefined;
					for (const profileId of listProfilesForProvider(
						authStore,
						PROVIDER_ID,
					)) {
						...
						const apiKey =
							envManagedApiKey ?? resolveApiKeyFromCredential(cred);
						if (!apiKey) {
							continue;
						}
