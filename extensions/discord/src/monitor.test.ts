...
const channelRuntimeModule = await import(
	"openclaw/plugin-sdk/channel-runtime"
);
vi.spyOn(channelRuntimeModule, "enqueueSystemEvent").mockImplementation(
	enqueueSystemEventSpy,
);

const routingModule = await import("openclaw/plugin-sdk/routing");
vi.spyOn(routingModule, "resolveAgentRoute").mockImplementation(
	resolveAgentRouteMock,
);

const { DiscordMessageListener, DiscordReactionListener } = await import(
...
function makeReactionEvent(overrides?: {
	...
	return {
		...
		rawMember: overrides?.memberRoleIds
			? { roles: overrides.memberRoleIds }
			: undefined,
		user: {
			id: userId,
			bot: false,
