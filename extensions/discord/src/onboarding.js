...
async function promptDiscordAllowFrom(params) {
  ...
  return promptLegacyChannelAllowFrom({
    ...
    resolveEntries: ({ token: token2, entries }) => resolveDiscordUserAllowlist({
      token: token2,
      entries
    })
  });
...
const discordOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, options, accountOverrides, shouldPromptAccountIds }) => {
    ...
    const tokenStep = await runSingleChannelSecretStep({
      ...
      applyUseEnv: async (cfg2) => applySingleTokenPromptResult({
        cfg: cfg2,
        channel: "discord",
        accountId: discordAccountId,
        tokenPatchKey: "token",
      ...
      applySet: async (cfg2, value) => applySingleTokenPromptResult({
        cfg: cfg2,
        channel: "discord",
        accountId: discordAccountId,
        tokenPatchKey: "token",
    ...
    next = await configureChannelAccessWithAllowlist({
      ...
      setPolicy: (cfg2, policy) => setAccountGroupPolicyForChannel({
        cfg: cfg2,
        channel: "discord",
        accountId: discordAccountId,
        groupPolicy: policy
      }),
      resolveAllowlist: async ({ cfg: cfg2, entries }) => {
      ...
      applyAllowlist: ({ cfg: cfg2, resolved }) => {
        ...
        return setDiscordGuildChannelAllowlist(cfg2, discordAccountId, allowlistEntries);
      }
    });
