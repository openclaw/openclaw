...
const runFfprobeMock = vi.hoisted(() =>
...
vi.mock("openclaw/plugin-sdk/temp-path", async (_importOriginal) => {
	return {
		resolvePreferredOpenClawTmpDir: () => "/tmp",
	};
});

vi.mock("openclaw/plugin-sdk/media-runtime", async (_importOriginal) => {
	return {
...
describe("ensureOggOpus", () => {
	...
	it("rejects URL/protocol input paths", async () => {
		await expect(
			ensureOggOpus("https://example.com/audio.ogg"),
		).rejects.toThrow(/local file path/i);
