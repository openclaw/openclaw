...
vi.mock("./monitor/provider.runtime.js", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("./monitor/provider.runtime.js")>();
	return {
		...actual,
...
function resolveAccount(cfg: OpenClawConfig): ResolvedDiscordAccount {
	return discordPlugin.config.resolveAccount(
		cfg,
		"default",
...
describe("discordPlugin security", () => {
	it("normalizes dm allowlist entries with trimmed prefixes and mentions", () => {
		...
		const result = resolveDmPolicy({
			cfg,
			account: discordPlugin.config.resolveAccount(
				cfg,
				"default",
			) as ResolvedDiscordAccount,
		});
		...
		expect(result.normalizeEntry?.("  discord:<@!123456789>  ")).toBe(
			"123456789",
		);
