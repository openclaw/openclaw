...
describe("formatMention", () => {
	...
	it("throws when more than one mention id is provided", () => {
		expect(() => formatMention({ userId: "1", roleId: "2" })).toThrow(
			/exactly one/i,
		);
	});
});
...
describe("rewriteDiscordKnownMentions", () => {
	...
	it("is account-scoped", () => {
		...
		const defaultRewrite = rewriteDiscordKnownMentions("@alice", {
			accountId: "default",
		});
		const opsRewrite = rewriteDiscordKnownMentions("@alice", {
			accountId: "ops",
		});
