...
function createMockRateLimitError(retryAfter = 1e-3) {
...
describe("retry rate limits", () => {
  ...
  it("uses retry_after delays when rate limited", async () => {
    ...
    const promise = sendMessageDiscord("channel:789", "hello", {
      rest,
      token: "t",
      retry: { attempts: 2, minDelayMs: 0, maxDelayMs: 1e3, jitter: 0 }
    });
  ...
  it("retries reactions on rate limits", async () => {
    const { rest, putMock } = makeDiscordRest();
    const rateLimitError = createMockRateLimitError(0);
    putMock.mockRejectedValueOnce(rateLimitError).mockResolvedValueOnce(void 0);
  ...
  it("retries media upload without duplicating overflow text", async () => {
    ...
    postMock.mockRejectedValueOnce(rateLimitError).mockResolvedValueOnce({ id: "msg1", channel_id: "789" }).mockResolvedValueOnce({ id: "msg2", channel_id: "789" });
