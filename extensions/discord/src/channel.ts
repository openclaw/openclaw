...
let discordProbeRuntimePromise:
	| Promise<typeof import("./probe.runtime.js")>
	| undefined;
...
const _meta = getChatChannelMeta("discord");
const REQUIRED_DISCORD_PERMISSIONS = ["ViewChannel", "SendMessages"] as const;

const resolveDiscordDmPolicy =
	createScopedDmSecurityResolver<ResolvedDiscordAccount>({
		channelKey: "discord",
...
const discordMessageActions: ChannelMessageActionAdapter = {
	describeMessageTool: (ctx) =>
		getDiscordRuntime().channel.discord.messageActions?.describeMessageTool?.(
			ctx,
		) ?? null,
	extractToolSend: (ctx) =>
		getDiscordRuntime().channel.discord.messageActions?.extractToolSend?.(
			ctx,
		) ?? null,
...
const discordNativeApprovalAdapter =
...
const resolveDiscordAllowlistGroupOverrides =
	createNestedAllowlistOverrideResolver({
		...
		innerLabel: (guildKey, channelKey) =>
			`guild ${guildKey} / channel ${channelKey}`,
...
const resolveDiscordAllowlistNames = createAccountScopedAllowlistNameResolver({
	...
	resolveNames: ({ token, entries }) =>
		resolveDiscordUserAllowlist({ token, entries }),
});
...
function parseDiscordParentChannelFromSessionKey(
	raw: unknown,
): string | undefined {
	...
	base: {
		...
		messaging: {
			...
			resolveSessionTarget: ({ id }) =>
				normalizeDiscordMessagingTarget(`channel:${id}`),
			...
			resolveOutboundSessionRoute: (params) =>
				resolveDiscordOutboundSessionRoute(params),
			targetResolver: {
		...
		>({
			...
			probeAccount: async ({ account, timeoutMs }) =>
				(await loadDiscordProbeRuntime()).probeDiscord(
					account.token,
					timeoutMs,
					{
			...
			buildCapabilitiesDiagnostics: async ({ account, timeoutMs, target }) => {
				...
				const details: Record<string, unknown> = {
					target: {
						...
						channelId:
							parsedTarget?.kind === "channel" ? parsedTarget.id : undefined,
					},
				};
			...
			auditAccount: async ({ account, timeoutMs, cfg }) => {
				const { channelIds, unresolvedChannels } =
					collectDiscordAuditChannelIds({
						cfg,
		...
		gateway: {
			startAccount: async (ctx) => {
				...
				let discordBotLabel = "";
				try {
					const probe = await (await loadDiscordProbeRuntime()).probeDiscord(
						token,
						2500,
						{
							includeApplication: true,
