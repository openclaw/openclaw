...
vi.mock("node:child_process", async (importOriginal) => {
  ...
  const execFileImpl = (file, args, optionsOrCallback, callbackMaybe) => {
    const normalizedArgs = Array.isArray(args) ? [...args] : [];
    const callback = typeof optionsOrCallback === "function" ? optionsOrCallback : callbackMaybe ?? void 0;
    const options = typeof optionsOrCallback === "function" ? void 0 : optionsOrCallback ?? void 0;
    execCalls.push({
      command: file,
      args: normalizedArgs,
      options
  ...
  const execFileWithCustomPromisify = execFileImpl;
  execFileWithCustomPromisify[promisify.custom] = (file, args, options) => new Promise((resolve, reject) => {
...
describe("ensureOggOpus", () => {
  ...
  it("keeps .ogg only when codec is opus and sample rate is 48kHz", async () => {
    ...
    expect(execCalls[0].options?.timeout).toBe(1e4);
  });
  it("re-encodes .ogg opus when sample rate is not 48kHz", async () => {
    ...
    expect(ffmpegCall?.options?.timeout).toBe(45e3);
  });
  it("re-encodes non-ogg input with bounded ffmpeg execution", async () => {
    ...
    expect(ffmpegCalls[0].options?.timeout).toBe(45e3);
