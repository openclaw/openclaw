	...
	switch (action) {
		case "memberInfo": {
			...
			const member = accountId
				? await discordGuildActionRuntime.fetchMemberInfoDiscord(
						guildId,
						userId,
						{ accountId },
					)
		...
		case "voiceStatus": {
			...
			const voice = accountId
				? await discordGuildActionRuntime.fetchVoiceStatusDiscord(
						guildId,
						userId,
						{
							accountId,
		...
		case "eventCreate": {
			...
			const event = accountId
				? await discordGuildActionRuntime.createScheduledEventDiscord(
						guildId,
						payload,
						{
							accountId,
