...
function hasDiscordComponentObjectKeys(
	value: unknown,
...
export async function handleDiscordMessagingAction(
	...
	switch (action) {
		...
		case "reactions": {
			...
			const reactions =
				await discordMessagingActionRuntime.fetchReactionsDiscord(
					channelId,
					messageId,
					{
						...cfgOptions,
		...
		case "poll": {
			...
			const maxSelections = resolvePollMaxSelections(
				answers.length,
				allowMultiselect,
			);
		...
		case "sendMessage": {
			...
			if (componentSpec) {
				...
				const result =
					await discordMessagingActionRuntime.sendDiscordComponentMessage(
						to,
						payload,
						{
							...cfgOptions,
			...
			if (asVoice) {
				...
				const result =
					await discordMessagingActionRuntime.sendVoiceMessageDiscord(
						to,
						mediaUrl,
						{
							...cfgOptions,
		...
		case "pinMessage": {
			...
			if (accountId) {
				await discordMessagingActionRuntime.pinMessageDiscord(
					channelId,
					messageId,
					{
						...cfgOptions,
		...
		case "unpinMessage": {
			...
			if (accountId) {
				await discordMessagingActionRuntime.unpinMessageDiscord(
					channelId,
					messageId,
					{
						...cfgOptions,
		...
		case "listPins": {
			...
			const pins = accountId
				...
				: await discordMessagingActionRuntime.listPinsDiscord(
						channelId,
						cfgOptions,
					);
			return jsonResult({
