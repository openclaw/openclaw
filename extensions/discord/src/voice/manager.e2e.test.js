...
const {
  ...
  return {
    createConnectionMock: createConnectionMock2,
    joinVoiceChannelMock: vi.fn(() => createConnectionMock2()),
    entersStateMock: vi.fn(async (_target, _state, _timeoutMs) => {
      return void 0;
    }),
    ...
    agentCommandMock: vi.fn(async (_opts, _runtime) => ({ payloads: [] })),
    buildProviderRegistryMock: vi.fn(() => ({})),
    createMediaAttachmentCacheMock: vi.fn(() => ({
      cleanup: vi.fn(async () => void 0)
    })),
...
describe("DiscordVoiceManager", () => {
  ...
  beforeEach(() => {
    ...
    entersStateMock.mockResolvedValue(void 0);
  ...
  const createManager = (discordConfig = {}, clientOverride) => new managerModule.DiscordVoiceManager({
    client: clientOverride ?? createClient(),
    cfg: {},
    discordConfig,
    accountId: "default",
    runtime: createRuntime()
  });
  const expectConnectedStatus = (manager, channelId) => {
  ...
  const emitDecryptFailure = (manager) => {
    const entry = manager.sessions.get("g1");
    expect(entry).toBeDefined();
    manager.handleReceiveError(
      entry,
      new Error("Failed to decrypt: DecryptionFailed(UnencryptedWhenPassthroughDisabled)")
    );
  };
  const processVoiceSegment = async (manager, userId) => await manager.processSegment({
    entry: {
      guildId: "g1",
      channelId: "c1",
  ...
  it("keeps the new session when an old disconnected handler fires", async () => {
    ...
    entersStateMock.mockImplementation(async (target, status) => {
  ...
  it("passes senderIsOwner=true for allowlisted voice speakers", async () => {
    ...
    const commandArgs = agentCommandMock.mock.calls.at(-1)?.[0];
