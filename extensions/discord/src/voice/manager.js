...
import { resolveTtsConfig, textToSpeech } from "../../../../src/tts/tts.js";
...
function createOpusDecoder() {
  try {
    const OpusScript = require2("opusscript");
...
function scheduleTempCleanup(tempDir, delayMs = 30 * 60 * 1e3) {
...
class DiscordVoiceManager {
  ...
  async processSegment(params) {
    ...
    const replyText = (result.payloads ?? []).map((payload) => payload.text).filter((text) => typeof text === "string" && text.trim()).join("\n").trim();
    if (!replyText) {
      logVoiceVerbose(
  ...
  handleReceiveError(entry, err) {
    ...
    this.resetDecryptFailureState(entry);
    void this.recoverFromDecryptFailures(entry).catch(
      (recoverErr) => logger.warn(`discord voice: decrypt recovery failed: ${formatErrorMessage(recoverErr)}`)
    ).finally(() => {
...
class DiscordVoiceReadyListener extends ReadyListener {
  constructor(manager) {
    super();
