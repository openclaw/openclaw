...
export function formatMention(params: {
	...
	const userId =
		params.userId == null ? null : normalizeSnowflake(params.userId);
	const roleId =
		params.roleId == null ? null : normalizeSnowflake(params.roleId);
	const channelId =
		params.channelId == null ? null : normalizeSnowflake(params.channelId);
	const values = [
		...
		(entry): entry is { kind: "user" | "role" | "channel"; id: string } =>
			Boolean(entry),
	);
	if (values.length !== 1) {
		throw new Error(
			"formatMention requires exactly one of userId, roleId, or channelId",
		);
	}
