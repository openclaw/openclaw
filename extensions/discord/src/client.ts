...
function resolveToken(params: { accountId: string; fallbackToken?: string }) {
	const fallback = normalizeDiscordToken(
...
export function createDiscordRestClient(
	...
	const explicitToken = normalizeDiscordToken(
		opts.token,
		"channels.discord.token",
	);
	const account = explicitToken
		? resolveAccountWithoutToken({
				cfg: resolvedCfg,
...
export function createDiscordClient(
	...
	const { token, rest, account } = createDiscordRestClient(
		opts,
		opts.cfg ?? cfg,
	);
