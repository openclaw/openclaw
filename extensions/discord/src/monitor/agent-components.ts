...
function buildDiscordComponentConversationLabel(params: {
	...
	return buildGuildLabel({
		...
		channelName:
			params.channelCtx.channelName ?? params.interactionCtx.channelId,
		channelId: params.interactionCtx.channelId,
	});
}

function resolveDiscordComponentChatType(
	interactionCtx: ComponentInteractionContext,
) {
...
async function dispatchPluginDiscordInteractiveEvent(params: {
	...
	const normalizedConversationId =
		params.interactionCtx.rawGuildId ||
		params.channelCtx.channelType === ChannelType.GroupDM
	...
	const updateOriginalMessage = async (input: {
		...
		if (
			!("update" in params.interaction) ||
			typeof params.interaction.update !== "function"
		) {
	...
	if (pluginBindingApproval) {
		...
		const approvalMessageId =
			params.messageId?.trim() || params.interaction.message?.id?.trim();
		if (approvalMessageId) {
			try {
				...
				logError(
					`discord plugin binding approval: failed to clear prompt: ${String(err)}`,
				);
			}
		}
		if (resolved.status !== "approved") {
			try {
				...
				logError(
					`discord plugin binding approval: failed to follow up: ${String(err)}`,
				);
			}
		}
		return "handled";
	...
	const pinnedMainDmOwner = interactionCtx.isDirectMessage
		? resolvePinnedMainDmOwnerFromAllowlist({
				...
				normalizeEntry: (entry: string) => {
					const normalized = normalizeDiscordAllowList(
						[entry],
						["discord:", "user:", "pk:"],
					);
