...
function isTransientDiscordGatewayResponse(
	status: number,
	body: string,
): boolean {
	if (status >= 500) {
...
function createGatewayMetadataError(params: {
	...
	const error = new Error(
			...
			: `Failed to get gateway information from Discord: ${params.detail}`,
		{
			cause:
				params.cause ??
				(params.transient ? new Error(params.detail) : undefined),
		},
...
async function fetchDiscordGatewayInfoWithTimeout(params: {
	...
	const timeoutMs = Math.max(
		1,
		params.timeoutMs ?? DISCORD_GATEWAY_INFO_TIMEOUT_MS,
	);
...
export function createDiscordGatewayPlugin(params: {
	...
	try {
		...
		const ProxyAgentCtor =
			params.__testing?.ProxyAgentCtor ?? undici.ProxyAgent;
		...
		return createGatewayPlugin({
			...
			fetchImpl: (input, init) =>
				(params.__testing?.undiciFetch ?? undici.fetch)(input, init),
