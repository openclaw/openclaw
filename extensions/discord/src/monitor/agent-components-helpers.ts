...
export type AgentComponentInteraction =
...
export async function resolveComponentInteractionContext(params: {
	...
	const isDirectMessage =
		...
		(!rawGuildId && !isGroupDm && channelType == null);
...
export function parseAgentComponentData(
	data: ComponentData,
...
async function ensureDmComponentAuthorized(params: {
	...
	const allowMatch = resolveAllowMatch([
		...(ctx.allowFrom ?? []),
		...storeAllowFrom,
	]);
	if (allowMatch.allowed) {
...
async function ensureGroupDmComponentAuthorized(params: {
	...
	if (!groupDmEnabled) {
		logVerbose(
			`agent ${componentLabel}: blocked group dm ${channelId} (group DMs disabled)`,
		);
		try {
			await interaction.reply({
	...
	logVerbose(
		`agent ${componentLabel}: blocked group dm ${channelId} (not allowlisted)`,
	);
	try {
		await interaction.reply({
			content: `You are not authorized to use this ${componentLabel}.`,
...
export function parseDiscordModalId(
	data: ComponentData,
	customId?: string,
): string | null {
	if (data && typeof data === "object") {
...
export function mapSelectValues(
	entry: DiscordComponentEntry,
	values: string[],
): string[] {
