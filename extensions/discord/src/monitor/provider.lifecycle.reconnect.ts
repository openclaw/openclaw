...
export function createDiscordGatewayReconnectController(params: {
	...
	const shouldStop = () =>
		params.isLifecycleStopping() || params.abortSignal?.aborted;
			...
			void (async () => {
				try {
					...
					const forceFreshIdentify =
						consecutiveHelloStalls >=
						DISCORD_GATEWAY_MAX_CONSECUTIVE_HELLO_STALLS;
					...
					params.runtime.error?.(
						danger(
							`discord: failed to restart stalled gateway socket: ${String(err)}`,
						),
					);
