...
function isBoundThreadBotSystemMessage(params: {
	...
	return DISCORD_BOUND_THREAD_SYSTEM_PREFIXES.some((prefix) =>
...
function mergeFetchedDiscordMessage(
	base: Message,
	fetched: APIMessage,
): Message {
	...
	const referencedMessage = fetched.referenced_message
		? ({
				...
				mentionedRoles:
					fetched.referenced_message.mention_roles ??
					baseReferenced?.mentionedRoles ??
					[],
				mentionedEveryone:
	...
	const rawData = {
		...
		message_snapshots:
			...
			(base as { rawData?: { message_snapshots?: unknown } }).rawData
				?.message_snapshots,
		sticker_items:
			(fetched as { sticker_items?: unknown }).sticker_items ??
			(base as { rawData?: { sticker_items?: unknown } }).rawData
				?.sticker_items,
	};
	return {
		...base,
...
async function hydrateDiscordMessageIfEmpty(params: {
	...
	const rest = params.client.rest as
	...
	try {
		...
		logVerbose(
			`discord: hydrated empty inbound payload via REST for ${params.message.id}`,
		);
		return mergeFetchedDiscordMessage(params.message, fetched);
	} catch (err) {
		logVerbose(
			`discord: failed to hydrate message ${params.message.id}: ${String(err)}`,
		);
		return params.message;
	}
}
...
export async function preflightDiscordMessage(
	...
	let pluralkitInfo: Awaited<ReturnType<typeof fetchPluralKitMessageInfo>> =
		null;
	if (shouldCheckPluralKit) {
		try {
			...
			logVerbose(
				`discord: pluralkit lookup failed for ${message.id}: ${String(err)}`,
			);
		}
	}
	...
	const data =
		message === params.data.message ? params.data : { ...params.data, message };
	logDebug(
	...
	const dmPolicy =
		params.discordConfig?.dmPolicy ??
		params.discordConfig?.dm?.policy ??
		"pairing";
	const useAccessGroups = params.cfg.commands?.useAccessGroups !== false;
	const resolvedAccountId = params.accountId ?? DEFAULT_ACCOUNT_ID;
	const allowNameMatching = isDangerousNameMatchingEnabled(
	...
	if (!isDirectMessage && baseText && hasControlCommand(baseText, params.cfg)) {
		logVerbose(
			`discord: drop text-based slash command ${message.id} (intercepted at gateway)`,
		);
		return null;
	}
	...
	const boundSessionKey = conversationRuntime.isPluginOwnedSessionBindingRecord(
		threadBinding,
	)
		? ""
	...
	const threadChannelSlug = channelName
		? normalizeDiscordSlug(channelName)
		: "";
	const threadParentSlug = threadParentName
	...
	if (
		...
		if (params.groupPolicy === "disabled") {
			...
			logDebug(
				`[discord-preflight] drop: groupPolicy allowlist, no channel allowlist configured`,
			);
			logVerbose(
	...
	if (isGuildMessage) {
		...
		logVerbose(
			`discord: allow channel ${messageChannelId} (${channelMatchMeta})`,
		);
	}
	...
	const threadOwnerId = threadChannel
		? (threadChannel.ownerId ?? channelInfo?.ownerId)
		: undefined;
	...
	if (author.bot && !sender.isPluralKit && allowBotsMode === "mentions") {
		...
		if (!botMentioned) {
			logDebug(
				`[discord-preflight] drop: bot message missing mention (allowBots=mentions)`,
			);
			logVerbose(
