...
const DISCORD_ACP_STALE_RUNNING_ACTIVITY_MS = 2 * 60 * 1e3;
...
function classifyAcpStatusProbeError(params) {
  ...
  return params.isStaleRunning ? { status: "stale", reason: "status-error-running-stale" } : { status: "uncertain", reason: "status-error" };
}
async function probeDiscordAcpBindingHealth(params) {
  ...
  }).then((status) => ({ kind: "status", status })).catch((error) => ({ kind: "error", error }));
  ...
  if (result.kind === "timeout") {
    return isStaleRunning ? { status: "stale", reason: "status-timeout-running-stale" } : { status: "uncertain", reason: "status-timeout" };
  }
...
async function monitorDiscordProvider(opts = {}) {
  ...
  try {
    ...
    class DiscordStatusReadyListener extends ReadyListener {
      async handle(_data, client2) {
        if (autoPresenceController?.enabled) {
          autoPresenceController.refresh();
          return;
        }
        const gateway = client2.getPlugin("gateway");
        if (!gateway) {
          return;
        }
