...
function mockDispatchSingleBlockReply(payload) {
...
beforeEach(() => {
  ...
  recordInboundSession.mockResolvedValue(void 0);
  readSessionUpdatedAt.mockReturnValue(void 0);
  resolveStorePath.mockReturnValue("/tmp/openclaw-discord-process-test-sessions.json");
  threadBindingTesting.resetThreadBindingsForTests();
});
function getLastRouteUpdate() {
  const callArgs = recordInboundSession.mock.calls.at(-1);
  const params = callArgs?.[0];
  return params?.updateLastRoute;
}
function getLastDispatchCtx() {
  const callArgs = dispatchInboundMessage.mock.calls.at(-1);
  const params = callArgs?.[0];
  return params?.ctx;
}
async function runProcessDiscordMessage(ctx) {
  await processDiscordMessage(ctx);
}
async function runInPartialStreamMode() {
...
function getReactionEmojis() {
  return sendMocks.reactMessageDiscord.mock.calls.map((call) => call[2]);
}
...
describe("processDiscordMessage ack reactions", () => {
  ...
  it("sends ack reactions for mention-gated guild messages when mentioned", async () => {
    ...
    await processDiscordMessage(ctx);
    expect(sendMocks.reactMessageDiscord.mock.calls[0]).toEqual(["c1", "m1", "\u{1F440}", { rest: {} }]);
  });
  ...
  it("shows stall emojis for long no-progress runs", async () => {
    ...
    const ctx = await createBaseContext();
    const runPromise = processDiscordMessage(ctx);
    await vi.advanceTimersByTimeAsync(30001);
    releaseDispatch();
    await vi.runAllTimersAsync();
    await runPromise;
    const emojis = sendMocks.reactMessageDiscord.mock.calls.map((call) => call[2]);
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        messages: {
          ackReaction: "\u{1F440}",
          statusReactions: {
            emojis: { queued: "\u{1F7E6}", thinking: "\u{1F9EA}", done: "\u{1F3C1}" },
            timing: { debounceMs: 0 }
          }
        },
    ...
    await processDiscordMessage(ctx);
    const emojis = getReactionEmojis();
    expect(emojis).toContain("\u{1F7E6}");
    expect(emojis).toContain("\u{1F3C1}");
  });
  ...
  it("clears status reactions when dispatch aborts and removeAckAfterReply is enabled", async () => {
    ...
    await processDiscordMessage(ctx);
    await vi.waitFor(() => {
      expect(sendMocks.removeReactionDiscord).toHaveBeenCalledWith("c1", "m1", "\u{1F440}", { rest: {} });
    });
  });
});
...
describe("processDiscordMessage draft streaming", () => {
  async function runSingleChunkFinalScenario(discordConfig) {
    dispatchInboundMessage.mockImplementationOnce(async (params) => {
