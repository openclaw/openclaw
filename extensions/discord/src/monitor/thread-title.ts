...
export async function generateThreadTitle(params: {
	...
	if ("error" in prepared) {
		const modelLabel = prepared.selection
			...
			: "unknown";
		logVerbose(
			`thread-title: ${prepared.error} (agent=${params.agentId}, model=${modelLabel})`,
		);
		return null;
	}
	...
	try {
		...
		const generated = normalizeGeneratedThreadTitle(
			extractAssistantText(response),
		);
		return generated || null;
	} catch (err) {
		logVerbose(
			`thread-title: title generation failed for agent ${params.agentId}: ${String(err)}`,
		);
		return null;
	}
}
...
function resolveThreadTitleTimeoutMs(timeoutMs: number | undefined): number {
	return Math.max(
		100,
		Math.floor(timeoutMs ?? DEFAULT_THREAD_TITLE_TIMEOUT_MS),
	);
}
...
function normalizeTitleContextField(
