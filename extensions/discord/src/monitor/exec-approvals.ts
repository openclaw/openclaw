...
function resolveApprovalAccountId(params: {
	...
	return isPluginApprovalRequest(params.request)
		? resolvePluginApprovalAccountId({
				cfg: params.cfg,
				request: params.request,
			})
		: resolveExecApprovalAccountId({
				cfg: params.cfg,
				request: params.request,
			});
}
...
function buildPluginApprovalMetadataLines(
	request: PluginApprovalRequest,
): string[] {
	const lines: string[] = [];
...
function createExecApprovalRequestContainer(params: {
	...
	const { commandPreview, commandSecondaryPreview } =
		resolveExecApprovalPreviews(params.request.request, 1000, 500);
...
function createPluginApprovalRequestContainer(params: {
	...
	return new ExecApprovalContainer({
		...
		commandSecondaryPreview: formatOptionalCommandPreview(
			params.request.request.description,
			1000,
		),
...
function createExecResolvedContainer(params: {
	...
	const { commandPreview, commandSecondaryPreview } =
		resolveExecApprovalPreviews(params.request.request, 500, 300);
...
export class DiscordExecApprovalHandler {
	...
	private async handleApprovalRequested(
		...
		const pluginRequest: PluginApprovalRequest | null = isPluginApprovalRequest(
			request,
		)
			? request
			: null;
		...
		if (pluginRequest) {
			this.requestCache.set(request.id, {
				kind: "plugin",
		...
		const originatingChannelId =
			sessionKey && target === "dm"
				? extractDiscordChannelId(sessionKey)
				: null;
		...
		if (target === "dm" && originatingChannelId) {
			try {
				...
				logError(
					`discord exec approvals: failed to send DM redirect notice: ${String(err)}`,
				);
			}
		}
	...
	private async finalizeMessage(
		...
		try {
			const { rest, request: discordRequest } = (
				this.opts.__testing?.createDiscordClient ??
				sendShared.createDiscordClient
			)(
				{ token: this.opts.token, accountId: this.opts.accountId },
				this.opts.cfg,
			);
	...
	async resolveApproval(
		...
		logDebug(
			`discord exec approvals: resolving ${approvalId} with ${decision} via ${method}`,
		);
