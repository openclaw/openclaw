...
function resolveDiscordPresenceUpdate(config) {
  ...
  if (hasActivity) {
    ...
    const activity = resolvedType === DEFAULT_CUSTOM_ACTIVITY_TYPE ? { name: CUSTOM_STATUS_NAME, type: resolvedType, state: activityText } : { name: activityText, type: resolvedType };
