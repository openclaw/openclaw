...
function normalizeStickerItems(value) {
  ...
  return value.filter(
    (entry) => Boolean(entry) && typeof entry === "object" && typeof entry.id === "string" && typeof entry.name === "string"
  );
}
...
function resolveDiscordMessageText(message, options) {
  ...
  const rawText = message.content?.trim() || buildDiscordMediaPlaceholder({
    attachments: message.attachments ?? void 0,
    stickers: resolveDiscordMessageStickers(message)
  }) || embedText || options?.fallbackText?.trim() || "";
