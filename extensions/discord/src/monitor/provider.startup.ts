...
type DiscordAutoPresenceController = ReturnType<
	typeof createDiscordAutoPresenceController
>;
...
export function createDiscordMonitorClient(params: {
	...
	const gateway = client.getPlugin<GatewayPlugin>("gateway") as
...
export async function fetchDiscordBotIdentity(params: {
	...
	try {
		...
		const botUserName =
			botUser?.username?.trim() || botUser?.globalName?.trim() || undefined;
...
export function registerDiscordMonitorListeners(params: {
	...
	const reactionListenerOptions: ConstructorParameters<
		typeof DiscordReactionListener
	>[0] = {
		cfg: params.cfg,
		accountId: params.accountId,
		runtime: params.runtime,
