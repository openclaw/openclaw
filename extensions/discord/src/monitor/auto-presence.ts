...
function resolveAutoPresenceConfig(
	...
	const intervalMs = clampPositiveInt(
		config?.intervalMs,
		DEFAULT_INTERVAL_MS,
		MIN_INTERVAL_MS,
	);
...
function isExhaustedUnavailableReason(
...
function resolvePresenceActivities(params: {
	...
	const defaultTemplate = isExhaustedUnavailableReason(
...
export function resolveDiscordAutoPresenceDecision(params: {
	...
	const autoPresence = resolveAutoPresenceConfig(
		params.discordConfig.autoPresence,
	);
...
export function createDiscordAutoPresenceController(params: {
	...
	const loadAuthStore =
		params.loadAuthStore ?? (() => ensureAuthProfileStore());
	const now = params.now ?? (() => Date.now());
