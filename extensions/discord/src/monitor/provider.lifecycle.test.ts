...
const {
	...
	const getDiscordGatewayEmitterMock = vi.fn<() => EventEmitter | undefined>(
		() => undefined,
	);
	return {
		...
		waitForDiscordGatewayStopMock: vi.fn(
			(_params: WaitForDiscordGatewayStopParams) => Promise.resolve(),
		),
...
describe("runDiscordGatewayLifecycle", () => {
		...
		return {
			...
			lifecycleParams: {
				...
				isDisallowedIntentsError:
					params?.isDisallowedIntentsError ?? (() => false),
				voiceManager: null,
	...
	function expectLifecycleCleanup(params: {
		...
		expect(waitForDiscordGatewayStopMock).toHaveBeenCalledTimes(
			params.waitCalls,
		);
	...
	async function emitGatewayOpenAndWait(
		emitter: EventEmitter,
		delayMs = 30000,
	): Promise<void> {
	...
	it("cleans up thread bindings when exec approvals startup fails", async () => {
		const { runDiscordGatewayLifecycle } = await import(
	...
	it("cleans up when gateway wait fails after startup", async () => {
		...
		waitForDiscordGatewayStopMock.mockRejectedValueOnce(
			new Error("gateway wait failed"),
		);
		const { lifecycleParams, start, stop, threadStop, gatewaySupervisor } =
			createLifecycleHarness();
	...
	it("cleans up after successful gateway wait", async () => {
		...
		await expect(
			runDiscordGatewayLifecycle(lifecycleParams),
	...
	it("fails closed when forced terminate still does not close the old socket", async () => {
		...
		try {
			...
			expect(runtimeError).toHaveBeenCalledWith(
				expect.stringContaining(
					"force-stopping instead of opening a parallel socket",
				),
			);
			expect(runtimeError).not.toHaveBeenCalledWith(
				expect.stringContaining(
	...
	it("treats drain timeout as a graceful stop after lifecycle abort", async () => {
		...
		try {
			...
			expect(runtimeError).not.toHaveBeenCalledWith(
				expect.stringContaining(
					"gateway socket did not close within 5000ms before reconnect",
				),
			);
			expectLifecycleCleanup({
				start,
	...
	it("logs queued non-fatal startup gateway errors and continues", async () => {
		...
		const {
			...
			pendingGatewayEvents: [
				createGatewayEvent("other", "transient startup error"),
			],
		});
	...
	it("throws queued non-disallowed fatal gateway errors", async () => {
		...
		const { lifecycleParams, start, stop, threadStop, gatewaySupervisor } =
			createLifecycleHarness({
				pendingGatewayEvents: [
					createGatewayEvent("fatal", "Fatal Gateway error: 4000"),
				],
			});
	...
	it("surfaces fatal startup gateway errors while waiting for READY", async () => {
		...
		try {
			...
			setTimeout(() => {
				pendingGatewayEvents.push(
					createGatewayEvent("fatal", "Fatal Gateway error: 4001"),
				);
			}, 1_000);
			...
			expect(runtimeError).toHaveBeenCalledWith(
				expect.stringContaining(
					"discord gateway error: Error: Fatal Gateway error: 4001",
				),
			);
	...
	it("suppresses reconnect-exhausted already queued before shutdown", async () => {
		...
		const { lifecycleParams, runtimeLog, runtimeError } =
			createLifecycleHarness({
				gateway,
				pendingGatewayEvents,
			});
		...
		expect(runtimeLog).not.toHaveBeenCalledWith(
			expect.stringContaining(
				"ignoring expected reconnect-exhausted during shutdown",
			),
		);
		expect(runtimeError).toHaveBeenCalledWith(
