...
describe("createDiscordGatewayPlugin", () => {
	...
	async function expectGatewayRegisterFallback(response: Response) {
		...
		expect(
			(plugin as unknown as { gatewayInfo?: { url?: string } }).gatewayInfo
				?.url,
		).toBe("wss://gateway.discord.gg/");
	...
	it("uses safe gateway metadata lookup without proxy", async () => {
		...
		await registerGatewayClientWithMetadata({
			plugin,
			fetchMock: globalFetchMock,
		});
	...
	it("refreshes fallback gateway metadata on the next register attempt", async () => {
		...
		expect(
			(plugin as unknown as { gatewayInfo?: { url?: string; shards?: number } })
				.gatewayInfo,
		).toMatchObject({
			url: "wss://gateway.discord.gg/?v=10",
			shards: 8,
