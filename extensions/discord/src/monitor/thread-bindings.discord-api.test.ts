...
const sendWebhookMessageDiscord =
	vi.fn<typeof discordSendModule.sendWebhookMessageDiscord>();
...
beforeAll(async () => {
	({ maybeSendBindingMessage, resolveChannelIdForBinding } = await import(
...
describe("resolveChannelIdForBinding", () => {
	beforeEach(() => {
		...
		sendWebhookMessageDiscord
			.mockReset()
			.mockResolvedValue(DEFAULT_SEND_RESULT);
		...
		vi.spyOn(discordSendModule, "sendMessageDiscord").mockImplementation(
			(...args) => sendMessageDiscord(...args),
		);
		vi.spyOn(discordSendModule, "sendWebhookMessageDiscord").mockImplementation(
			(...args) => sendWebhookMessageDiscord(...args),
		);
	});
	...
	it("forwards cfg when resolving channel id through Discord client", async () => {
		...
		const createDiscordRestClientCalls = createDiscordRestClient.mock
			.calls as unknown[][];
