...
describe("buildDiscordInboundJob", () => {
	it("prefers route session key, then base session key, then channel id for queueing", async () => {
		...
		expect(resolveDiscordInboundJobQueueKey(routed)).toBe(
			"agent:main:discord:direct:routed",
		);
		expect(resolveDiscordInboundJobQueueKey(baseOnly)).toBe(
			"agent:main:discord:direct:base-only",
		);
		expect(resolveDiscordInboundJobQueueKey(channelFallback)).toBe(
			"channel-fallback",
		);
	});
	...
	it("re-materializes the process context with an overridden abort signal", async () => {
		...
		const rematerialized = materializeDiscordInboundJob(
			job,
			overrideAbortController.signal,
		);
