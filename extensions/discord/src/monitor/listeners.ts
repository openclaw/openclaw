...
function formatListenerContextSuffix(
	...
	try {
		timedOut = await runDiscordTaskWithTimeout({
			...
			onTimeout: (resolvedTimeoutMs) => {
				logger.error(
					danger(
						`discord handler timed out after ${formatDurationSeconds(
							resolvedTimeoutMs,
							{
								decimals: 1,
								unit: "seconds",
...
async function handleDiscordReactionEvent(
	...
	try {
		...
		const memberRoleIds = Array.isArray(data.rawMember?.roles)
			...
			: [];
		const reactionIngressBase: Omit<
			DiscordReactionIngressAuthorizationParams,
			"channelConfig"
		> = {
			accountId: params.accountId,
			user,
		...
		if (!isGuildMessage) {
			const ingressAccess =
				await authorizeDiscordReactionIngress(reactionIngressBase);
		...
		const shouldNotifyReaction = (options: {
			...
			channelConfig?: ReturnType<
				typeof resolveDiscordChannelConfigWithFallback
			>;
		}) =>
			shouldEmitDiscordReactionNotification({
				mode: options.mode,
