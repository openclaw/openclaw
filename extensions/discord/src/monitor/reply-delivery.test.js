...
vi.mock("../send.js", () => ({
  sendMessageDiscord: (...args) => sendMessageDiscordMock(...args),
  sendVoiceMessageDiscord: (...args) => sendVoiceMessageDiscordMock(...args),
  sendWebhookMessageDiscord: (...args) => sendWebhookMessageDiscordMock(...args)
}));
vi.mock("../send.shared.js", () => ({
  sendDiscordText: (...args) => sendDiscordTextMock(...args)
}));
describe("deliverDiscordReply", () => {
  ...
  const expectBotSendRetrySuccess = async (status, message) => {
    sendMessageDiscordMock.mockRejectedValueOnce(Object.assign(new Error(message), { status })).mockResolvedValueOnce({ messageId: "msg-1", channelId: "channel-1" });
  ...
  const createBoundThreadBindings = async (overrides = {}) => {
  ...
  it("delivers remaining chunks after a mid-sequence retry", async () => {
    sendMessageDiscordMock.mockResolvedValueOnce({ messageId: "c1" }).mockRejectedValueOnce(Object.assign(new Error("rate limited"), { status: 429 })).mockResolvedValueOnce({ messageId: "c2-retry" }).mockResolvedValueOnce({ messageId: "c3" });
    await deliverDiscordReply({
  ...
  it("touches bound-thread activity after outbound delivery", async () => {
    ...
    try {
      ...
      vi.setSystemTime(/* @__PURE__ */ new Date("2026-02-20T00:02:00.000Z"));
      await deliverDiscordReply({
      ...
      expect(threadBindings.getByThreadId("thread-1")?.lastActivityAt).toBe(
        (/* @__PURE__ */ new Date("2026-02-20T00:02:00.000Z")).getTime()
      );
    } finally {
      vi.useRealTimers();
    }
