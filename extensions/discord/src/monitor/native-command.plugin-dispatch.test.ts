...
vi.mock("openclaw/plugin-sdk/plugin-runtime", async (importOriginal) => {
	...
	return {
		...actual,
		matchPluginCommand: (...args: unknown[]) =>
			runtimeModuleMocks.matchPluginCommand(...args),
		executePluginCommand: (...args: unknown[]) =>
			runtimeModuleMocks.executePluginCommand(...args),
	};
});
...
function createConfiguredRouteState(params: {
	...
	return {
		...
		ReturnType<
			typeof import("./native-command-route.js").resolveDiscordNativeInteractionRouteState
		>
	>;
}
...
function expectBoundSessionDispatch(
	...
	if (
		!dispatchCall.ctx?.SessionKey ||
		!dispatchCall.ctx.CommandTargetSessionKey
	) {
		throw new Error(
			"native command dispatch did not include bound session context",
		);
	}
...
describe("Discord native plugin command dispatch", () => {
	...
	it("allows recovery commands through configured ACP bindings even when ensure fails", async () => {
		...
		expect(dispatchCall.ctx?.SessionKey).toMatch(
