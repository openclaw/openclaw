...
vi.mock("undici", () => {
  ...
  class ProxyAgent {
    ...
    constructor(options: string | { uri?: string }) {
      const proxyUrl = typeof options === "string" ? options : options.uri;
...
describe("resolveDiscordRestFetch", () => {
  ...
  it("uses undici proxy fetch when a proxy URL is configured", async () => {
    ...
    expect(proxyAgentSpy).toHaveBeenCalledWith(
      expect.objectContaining({ uri: "http://127.0.0.1:8080" }),
    );
  ...
  it("uses undici proxy fetch when proxy URL is remote", async () => {
    const runtime = {
      log: vi.fn(),
      error: vi.fn(),
      exit: vi.fn(),
  ...
  it("uses undici proxy fetch when the proxy URL is IPv6 loopback", async () => {
    ...
    expect(proxyAgentSpy).toHaveBeenCalledWith(
      expect.objectContaining({ uri: "http://[::1]:8080" }),
    );
    expect(runtime.error).not.toHaveBeenCalled();
  });
