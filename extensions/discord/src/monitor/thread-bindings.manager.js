...
function createThreadBindingManager(params = {}) {
  ...
  const manager = {
    ...
    touchThread: (touchParams) => {
      ...
      const existing2 = BINDINGS_BY_THREAD_ID.get(key);
      if (!existing2 || existing2.accountId !== accountId) {
        return null;
      }
      const now = Date.now();
      ...
      const nextRecord = {
        ...existing2,
        lastActivityAt: Math.max(existing2.lastActivityAt || 0, at)
      };
    ...
    unbindThread: (unbindParams) => {
      ...
      const existing2 = BINDINGS_BY_THREAD_ID.get(bindingKey);
      if (!existing2 || existing2.accountId !== accountId) {
        return null;
      }
  ...
  registerSessionBindingAdapter({
    ...
    listBySession: (targetSessionKey) => manager.listBySessionKey(targetSessionKey).map((entry) => toSessionBindingRecord(entry, { idleTimeoutMs, maxAgeMs })),
    ...
    unbind: async (input) => {
      if (input.targetSessionKey?.trim()) {
        const removed2 = manager.unbindBySessionKey({
          targetSessionKey: input.targetSessionKey,
          reason: input.reason
        });
        return removed2.map((entry) => toSessionBindingRecord(entry, { idleTimeoutMs, maxAgeMs }));
      }
      const threadId = resolveThreadBindingConversationIdFromBindingId({
        accountId,
