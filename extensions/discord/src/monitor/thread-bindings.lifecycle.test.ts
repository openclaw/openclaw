...
const { __testing, createThreadBindingManager } = await import(
...
const { resolveThreadBindingIntroText } = await import(
	"./thread-bindings.messages.js"
);
const discordClientModule = await import("../client.js");
const discordThreadBindingApi = await import(
...
describe("thread binding lifecycle", () => {
	beforeEach(() => {
		...
		hoisted.createThreadDiscord
			.mockReset()
			.mockResolvedValue({ id: "thread-created" });
			...
			return {
				webhookId:
					typeof created?.id === "string"
						? created.id.trim() || undefined
						: undefined,
				webhookToken:
					typeof created?.token === "string"
						? created.token.trim() || undefined
						: undefined,
			};
		});
		vi.spyOn(
		...
		vi.spyOn(
			...
			await hoisted.sendMessageDiscord(
				`channel:${params.record.threadId}`,
				params.text,
				{
					cfg: params.cfg,
					accountId: params.record.accountId,
		...
		vi.spyOn(acpRuntime, "readAcpSessionEntry").mockImplementation(
	...
	it("uses the active runtime snapshot cfg for manager operations", async () => {
		...
		const usedRefreshedCfg = hoisted.createDiscordRestClient.mock.calls.some(
			(call) => {
		...
		const usedStartupCfg = hoisted.createDiscordRestClient.mock.calls.some(
			(call) => {
