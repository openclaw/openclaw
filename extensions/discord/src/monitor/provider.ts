...
type DiscordProviderSessionRuntimeModule =
	typeof import("./provider-session.runtime.js");
...
let resolveNativeCommandsEnabledForTesting:
	| typeof resolveNativeCommandsEnabled
	| undefined;
let resolveNativeSkillsEnabledForTesting:
	| typeof resolveNativeSkillsEnabled
	| undefined;
let listNativeCommandSpecsForConfigForTesting:
	| typeof listNativeCommandSpecsForConfig
	| undefined;
...
async function loadDiscordProviderSessionRuntime(): Promise<DiscordProviderSessionRuntimeModule> {
	...
	discordProviderSessionRuntimePromise ??= import(
		"./provider-session.runtime.js"
	);
...
function appendPluginCommandSpecs(params: {
	...
	for (const pluginCommand of (
		getPluginCommandSpecsForTesting ?? getPluginCommandSpecs
	)("discord")) {
...
async function probeDiscordAcpBindingHealth(params: {
	...
	const { getAcpSessionManager, isAcpRuntimeError } =
		await loadDiscordProviderSessionRuntime();
	...
	const isStaleRunning =
		params.storedState === "running" &&
		runningForMs >= DISCORD_ACP_STALE_RUNNING_ACTIVITY_MS;
...
async function deployDiscordCommands(params: {
	...
	try {
		...
		params.runtime.error?.(
			danger(
				`discord: failed to deploy native commands: ${formatErrorMessage(err)}${details}`,
			),
		);
	} finally {
		if (restClient.options) {
...
function formatDiscordStartupGatewayState(gateway?: GatewayPlugin): string {
	...
	const reconnectAttempts = (
		gateway as unknown as { reconnectAttempts?: unknown }
	).reconnectAttempts;
...
function logDiscordStartupPhase(params: {
	...
	const suffix = [
		params.details,
		formatDiscordStartupGatewayState(params.gateway),
	]
...
function formatDiscordRejectedDeployEntries(params: {
	...
	const requestBody = Array.isArray(params.requestBody)
		? params.requestBody
		: null;
	const rejectedEntriesSource = resolveDiscordRejectedDeployEntriesSource(
		params.rawBody,
	);
	if (!rejectedEntriesSource || !requestBody || requestBody.length === 0) {
		return [];
	}
	const rawEntries = Object.entries(rejectedEntriesSource).filter(([key]) =>
		/^\d+$/.test(key),
	);
	return rawEntries
		...
		.flatMap(([key, value]) => {
			...
			if (payload.description !== undefined) {
				parts.push(
					`description=${stringifyDiscordDeployField(payload.description)}`,
				);
			}
		...
		try {
			...
			bodyText =
				typeof rawBody === "string"
					? rawBody
					: inspect(rawBody, { depth: 3, breakLength: 120 });
		}
		if (bodyText) {
			const maxLen = 800;
			const trimmed =
				bodyText.length > maxLen ? `${bodyText.slice(0, maxLen)}...` : bodyText;
...
export async function monitorDiscordProvider(opts: MonitorDiscordOpts = {}) {
	...
	const token =
		normalizeDiscordToken(opts.token ?? undefined, "channels.discord.token") ??
		account.token;
	if (!token) {
		throw new Error(
	...
	const discordRestFetch = resolveDiscordRestFetch(
		rawDiscordCfg.proxy,
		runtime,
	);
	...
	const threadBindingIdleTimeoutMs =
		discordProviderSessionRuntime.resolveThreadBindingIdleTimeoutMs({
			channelIdleHoursRaw:
				discordAccountThreadBindings?.idleHours ??
				discordRootThreadBindings?.idleHours,
			sessionIdleHoursRaw: cfg.session?.threadBindings?.idleHours,
		});
	const threadBindingMaxAgeMs =
		discordProviderSessionRuntime.resolveThreadBindingMaxAgeMs({
			channelMaxAgeHoursRaw:
				discordAccountThreadBindings?.maxAgeHours ??
				discordRootThreadBindings?.maxAgeHours,
			sessionMaxAgeHoursRaw: cfg.session?.threadBindings?.maxAgeHours,
		});
	const threadBindingsEnabled =
		discordProviderSessionRuntime.resolveThreadBindingsEnabled({
			channelEnabledRaw:
				discordAccountThreadBindings?.enabled ??
				discordRootThreadBindings?.enabled,
			sessionEnabledRaw: cfg.session?.threadBindings?.enabled,
		});
	const groupDmEnabled = dmConfig?.groupEnabled ?? false;
	const groupDmChannels = dmConfig?.groupChannels;
	const nativeEnabled = (
		resolveNativeCommandsEnabledForTesting ?? resolveNativeCommandsEnabled
	)({
		providerId: "discord",
		providerSetting: discordCfg.commands?.native,
		globalSetting: cfg.commands?.native,
	});
	const nativeSkillsEnabled = (
		resolveNativeSkillsEnabledForTesting ?? resolveNativeSkillsEnabled
	)({
		providerId: "discord",
	...
	const slashCommand = resolveDiscordSlashCommandConfig(
		discordCfg.slashCommand,
	);
	...
	const applicationId = await (
		fetchDiscordApplicationIdForTesting ?? fetchDiscordApplicationId
	)(token, 4000, discordRestFetch);
	if (!applicationId) {
	...
	if (
		...
		commandSpecs = (
			listNativeCommandSpecsForConfigForTesting ??
			listNativeCommandSpecsForConfig
		)(cfg, { skillCommands: [], provider: "discord" });
		commandSpecs = appendPluginCommandSpecs({ commandSpecs, runtime });
		runtime.log?.(
			warn(
	...
	const threadBindings = threadBindingsEnabled
		...
		: discordProviderSessionRuntime.createNoopThreadBindingManager(
				account.accountId,
			);
			...
			await discordProviderSessionRuntime.reconcileAcpThreadBindingsOnStartup({
				...
				healthProbe: async ({ sessionKey, session }) => {
					...
					if (probe.status === "uncertain") {
						uncertainProbeKeys.add(
							`${sessionKey}${probe.reason ? ` (${probe.reason})` : ""}`,
						);
					}
					return probe;
				},
	...
	let gatewaySupervisor:
		| ReturnType<typeof createDiscordGatewaySupervisor>
		| undefined;
	...
	try {
		...
		if (execApprovalsHandler) {
			components.push(
				createExecApprovalButton({ handler: execApprovalsHandler }),
			);
		}
		...
		if (agentComponentsEnabled) {
			...
			components.push(
				createDiscordComponentMentionableSelect(componentContext),
			);
		...
		if (voiceEnabled) {
			const { DiscordVoiceManager, DiscordVoiceReadyListener } =
			...
			registerDiscordListener(
				client.listeners,
				new DiscordVoiceReadyListener(voiceManager),
			);
		}

		const messageHandler =
			discordProviderSessionRuntime.createDiscordMessageHandler({
				cfg,
				discordConfig: discordCfg,
				accountId: account.accountId,
				token,
		...
		if (!lifecycleStarted) {
			try {
				...
				runtime.error?.(
					danger(
						`discord: failed to disconnect gateway during startup cleanup: ${String(err)}`,
					),
				);
			}
		}
...
export const __testing = {
	...
	setCreateDiscordGatewaySupervisor(
		mock?: typeof createDiscordGatewaySupervisor,
	) {
	...
	setLoadDiscordProviderSessionRuntime(
		mock?: () => Promise<DiscordProviderSessionRuntimeModule>,
	) {
	...
	setListNativeCommandSpecsForConfig(
		mock?: typeof listNativeCommandSpecsForConfig,
	) {
...
export const resolveDiscordRuntimeGroupPolicy =
	resolveOpenProviderRuntimeGroupPolicy;
