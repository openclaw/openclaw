...
describe("preflightDiscordMessage configured ACP bindings", () => {
	beforeEach(() => {
		...
		).mockImplementation(resolveConfiguredBindingRouteMock);
		vi.spyOn(
			conversationRuntime,
			"ensureConfiguredBindingRouteReady",
		).mockImplementation(ensureConfiguredBindingRouteReadyMock);
	});
	...
	it("initializes configured ACP bindings only after preflight accepts the message", async () => {
		...
		expect(result?.boundSessionKey).toBe(
			"agent:codex:acp:binding:discord:default:abc123",
		);
	});
