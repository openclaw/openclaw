...
describe("resolveDiscordAllowlistConfig", () => {
	beforeEach(() => {
		vi.restoreAllMocks();
		vi.spyOn(
			resolveChannelsModule,
			"resolveDiscordChannelAllowlist",
		).mockResolvedValue([]);
		vi.spyOn(
			resolveUsersModule,
			"resolveDiscordUserAllowlist",
		).mockImplementation(async (params: { entries: string[] }) =>
	...
	it("canonicalizes resolved user names to ids in runtime config", async () => {
		...
		expect(result.guildEntries?.["*"]?.channels?.["*"]?.users).toEqual([
			"Carol",
			"888",
		]);
		expect(
			resolveUsersModule.resolveDiscordUserAllowlist,
