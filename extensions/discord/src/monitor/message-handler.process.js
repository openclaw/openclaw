...
${forumContextLine}`;
  ...
  const typingChannelId = deliverTarget.startsWith("channel:") ? deliverTarget.slice("channel:".length) : messageChannelId;
  ...
  const deliverChannelId = deliverTarget.startsWith("channel:") ? deliverTarget.slice("channel:".length) : messageChannelId;
  ...
  const resolvePreviewFinalText = (text2) => {
    if (typeof text2 !== "string") {
      return void 0;
    }
    const formatted = convertMarkdownTables(text2, tableMode);
    ...
    if (currentPreviewText && currentPreviewText.startsWith(trimmed) && trimmed.length < currentPreviewText.length) {
      return void 0;
    }
    return trimmed;
  };
  const updateDraftFromPartial = (text2) => {
    if (!draftStream || !text2) {
      return;
    }
    const cleaned = stripReasoningTagsFromText(text2, { mode: "strict", trim: "both" });
    ...
    if (discordStreamMode === "partial") {
      if (lastPartialText && lastPartialText.startsWith(cleaned) && cleaned.length < lastPartialText.length) {
        return;
      }
  ...
  const { dispatcher, replyOptions, markDispatchIdle, markRunComplete } = createReplyDispatcherWithTyping({
    ...
    deliver: async (payload, info) => {
      ...
      if (draftStream && isFinal) {
        ...
        const canFinalizeViaPreviewEdit = !finalizedViaPreviewMessage && !hasMedia && typeof previewFinalText === "string" && typeof previewMessageId === "string" && !payload.isError;
        ...
        if (!finalizedViaPreviewMessage) {
          ...
          if (typeof messageIdAfterStop === "string" && typeof previewFinalText === "string" && !hasMedia && !payload.isError) {
            try {
              await editMessageDiscord(
                deliverChannelId,
                messageIdAfterStop,
