...
vi.mock("openclaw/plugin-sdk/gateway-runtime", async (importOriginal) => {
	...
	return {
		...
		createOperatorApprovalsGatewayClient: async (
			...
			const gatewayUrl =
				params.gatewayUrl?.trim() || envUrl || "ws://127.0.0.1:18789";
			const urlOverrideSource = params.gatewayUrl?.trim()
				? "cli"
				: envUrl
					? "env"
					: undefined;
...
type PluginApprovalRequest =
	import("./exec-approvals.js").PluginApprovalRequest;
type ExecApprovalButtonContext =
	import("./exec-approvals.js").ExecApprovalButtonContext;
...
function mockSuccessfulDmDelivery(params?: {
	...
	mockRestPost.mockImplementation(
		async (route: string, requestParams?: { body?: { content?: string } }) => {
			if (
				params?.noteChannelId &&
				route === Routes.channelMessages(params.noteChannelId)
			) {
				if (params.expectedNoteText) {
					expect(requestParams?.body?.content).toContain(
						params.expectedNoteText,
					);
				}
...
async function expectGatewayAuthStart(params: {
	...
	expect(mockGatewayClientCtor).toHaveBeenCalledWith(
		expect.objectContaining(expectedClientParams),
	);
}
...
describe("DiscordExecApprovalHandler plugin approvals", () => {
	...
	it("delivers plugin approval requests with interactive approval buttons", async () => {
		...
		expect(bodyJson).toContain(
			"execapproval:id=plugin%3Atest-id;action=allow-once",
		);
	...
	it("handles plugin approvals end-to-end via gateway event, button resolve, and card update", async () => {
		...
		try {
			...
			expect(gatewayClientRequests).toHaveBeenCalledWith(
