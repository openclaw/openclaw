...
function createThreadBinding(
	overrides?: Partial<
		import("openclaw/plugin-sdk/conversation-runtime").SessionBindingRecord
	>,
) {
	return {
...
async function runThreadBoundPreflight(params: {
	...
	return preflightDiscordMessage({
		...
		threadBindings: {
			getByThreadId: (id: string) =>
				id === params.threadId ? params.threadBinding : undefined,
...
describe("preflightDiscordMessage", () => {
	...
	it("drops hydrated bound-thread webhook echoes after fetching an empty payload", async () => {
		...
		const result = await preflightDiscordMessage({
			...
			threadBindings: {
				getByThreadId: (id: string) =>
					id === threadId ? threadBinding : undefined,
			} as import("./thread-bindings.js").ThreadBindingManager,
		});
	...
	it("drops guild messages that mention another user when ignoreOtherMentions=true", async () => {
		...
		const result = await runIgnoreOtherMentionsPreflight({
			channelId,
			guildId,
			message,
		});
	...
	it("drops guild message without mention when channel has configuredBinding and requireMention: true", async () => {
		const conversationRuntime = await import(
		...
		try {
			const result = await runGuildPreflight({
				...
				guildEntries: {
					"guild-1": {
						channels: { [channelId]: { allow: true, requireMention: true } },
					},
				},
			});
			expect(result).toBeNull();
