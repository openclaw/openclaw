...
describe("preflightDiscordMessage", () => {
  ...
  it("drops guild messages from unconfigured channels when allowlist is configured (issue #54277)", async () => {
    // When groupPolicy=allowlist and at least one channel is configured, any channel NOT in the
    // channels config should be blocked, not allowed through due to `undefined !== false === true`.
    const message = createDiscordMessage({
      id: "m-unconfigured-ch",
    ...
    const result = await runGuildPreflight({
      ...
      guildId: "guild-1",
      message,
      discordConfig: { groupPolicy: "allowlist" } as DiscordConfig,
      guildEntries: {
        "guild-1": {
          requireMention: true,
          channels: {
    ...
    // Channel "ch-unknown" is NOT in the allowlist → should be dropped
    expect(result).toBeNull();
  });
