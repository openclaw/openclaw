...
let resolveDiscordNativeInteractionRouteStateImpl =
...
function resolveDiscordCommandLogLabel(command: ChatCommandDefinition): string {
	if (
		typeof command.nativeName === "string" &&
		command.nativeName.trim().length > 0
	) {
...
function resolveDiscordNativeCommandAllowlistAccess(params: {
	...
	if (guildId) {
		for (const entry of rawAllowList) {
			...
			if (
				text.startsWith("guild:") &&
				text.slice("guild:".length) === guildId
			) {
			...
			? async (interaction: AutocompleteInteraction) => {
					...
					const filtered = focusValue
						? choices.filter((choice) =>
								choice.label.toLowerCase().includes(focusValue),
							)
						: choices;
					await interaction.respond(
						filtered
...
async function resolveDiscordNativeAutocompleteAuthorized(params: {
	...
	const dmPolicy =
		discordConfig?.dmPolicy ?? discordConfig?.dm?.policy ?? "pairing";
	if (isDirectMessage) {
...
function hasRenderableReplyPayload(payload: ReplyPayload): boolean {
	...
	if (
		...
		discordData.components.length > 0
	) {
		return true;
	}
	return false;
...
async function dispatchDiscordCommandInteraction(params: {
	...
	if (pluginMatch) {
		...
		const messageThreadId =
			!isDirectMessage && isThreadChannel ? channelId : undefined;
	...
	const routeState = await resolveDiscordNativeInteractionRouteStateImpl({
		...
		threadBinding,
		enforceConfiguredBindingReadiness:
			!shouldBypassConfiguredAcpEnsure(commandName),
	});
	...
	const { sessionKey, commandTargetSessionKey } =
		resolveNativeCommandSessionTargets({
