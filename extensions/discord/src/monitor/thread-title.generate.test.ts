...
const extractAssistantTextMock =
	vi.fn<typeof agentRuntimeModule.extractAssistantText>();
...
beforeEach(() => {
	...
	prepareSimpleCompletionModelForAgentMock.mockResolvedValue({
		...
		ReturnType<typeof agentRuntimeModule.prepareSimpleCompletionModelForAgent>
	>);
	...
	vi.spyOn(agentRuntimeModule, "extractAssistantText").mockImplementation(
		(...args) => extractAssistantTextMock(...args),
	);
});
...
describe("generateThreadTitle", () => {
	...
	it("returns null when shared completion prep fails", async () => {
		prepareSimpleCompletionModelForAgentMock.mockResolvedValue({
			error:
				'No API key resolved for provider "anthropic" (auth mode: api-key).',
			selection: {
				provider: "anthropic",
				modelId: "claude-opus-4-6",
				agentDir: "/tmp/openclaw-agent",
	...
	it("builds contextual prompt and forwards completion options", async () => {
		...
		expect(completeWithPreparedSimpleCompletionModelMock).toHaveBeenCalledTimes(
			1,
		);
		expect(
			completeWithPreparedSimpleCompletionModelMock.mock.calls[0]?.[0]?.context,
		).toEqual(
