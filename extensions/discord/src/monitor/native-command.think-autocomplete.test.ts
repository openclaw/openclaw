...
import { type AutocompleteInteraction, ChannelType } from "@buape/carbon";
import {
...
const ensureConfiguredBindingRouteReadyMock = vi.hoisted(() =>
	vi.fn<() => Promise<{ ok: boolean; error?: string }>>(async () => ({
		ok: true,
	})),
);
...
describe("discord native /think autocomplete", () => {
	beforeEach(async () => {
		vi.resetModules();
		({ resolveDiscordNativeChoiceContext } = await import(
			"./native-command-ui.js"
		));
