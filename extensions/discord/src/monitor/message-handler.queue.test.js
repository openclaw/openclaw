...
function createMessageData(messageId, channelId = "ch-1") {
  return {
...
function createHandlerWithDefaultPreflight(overrides) {
  preflightDiscordMessageMock.mockImplementation(
    async (params) => createPreflightContext(params.data.channel_id)
  );
  return createDiscordMessageHandler(createDiscordHandlerParams(overrides));
}
async function createLifecycleStopScenario(params) {
  preflightDiscordMessageMock.mockImplementation(
    async (preflightParams) => createPreflightContext(preflightParams.data.channel_id)
  );
  ...
  await expect(handler(createMessageData("m-1"), {})).resolves.toBeUndefined();
  await vi.waitFor(() => {
...
describe("createDiscordMessageHandler queue behavior", () => {
  ...
  it("returns immediately and tracks busy status while queued runs execute", async () => {
    ...
    await expect(handler(createMessageData("m-2"), {})).resolves.toBeUndefined();
    await vi.waitFor(() => {
  ...
  it("applies explicit inbound worker timeout to queued runs so stalled runs do not block the queue", async () => {
    ...
    try {
      preflightDiscordMessageMock.mockReset();
      processDiscordMessageMock.mockReset();
      processDiscordMessageMock.mockImplementationOnce(async (ctx) => {
      ...
      await expect(
        handler(createMessageData("m-1"), {})
      ).resolves.toBeUndefined();
      await expect(
        handler(createMessageData("m-2"), {})
      ).resolves.toBeUndefined();
      await vi.advanceTimersByTimeAsync(60);
      await vi.waitFor(() => {
        expect(processDiscordMessageMock).toHaveBeenCalledTimes(2);
      });
      const firstCtx = processDiscordMessageMock.mock.calls[0]?.[0];
  ...
  it("preserves non-debounced message ordering by awaiting debouncer enqueue", async () => {
    ...
    processDiscordMessageMock.mockImplementation(async (ctx) => {
      processedMessageIds.push(ctx.messageId ?? "unknown");
    });
    const handler = createDiscordMessageHandler(createDiscordHandlerParams());
    const sequentialDispatch = (async () => {
      await handler(createMessageData("m-1"), {});
      await handler(createMessageData("m-2"), {});
    })();
    await vi.waitFor(() => {
  ...
  it("recovers queue progress after a run failure without leaving busy state stuck", async () => {
    ...
    await expect(handler(createMessageData("m-2"), {})).resolves.toBeUndefined();
    firstRun.resolve();
    await firstRun.promise.catch(() => void 0);
    await vi.waitFor(() => {
      expect(processDiscordMessageMock).toHaveBeenCalledTimes(2);
    });
    await vi.waitFor(() => {
