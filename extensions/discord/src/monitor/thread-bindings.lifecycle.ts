...
async function mapWithConcurrency<TItem, TResult>(params: {
	...
	const workers = Array.from(
		{ length: Math.min(limit, params.items.length) },
		() => runWorker(),
	);
...
export async function reconcileAcpThreadBindingsOnStartup(params: {
	...
	const acpBindings = manager
		...
		.filter(
			(binding) =>
				binding.targetKind === "acp" &&
				binding.metadata?.pluginBindingOwner !== "plugin",
		);
			...
			worker: async ({ binding, sessionKey, session }) => {
				try {
					...
					return {
						...
						status:
							result?.status ??
							("uncertain" satisfies AcpThreadBindingHealthStatus),
					};
				} catch {
