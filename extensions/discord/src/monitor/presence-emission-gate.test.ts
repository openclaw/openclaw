...
describe("DiscordPresenceEmissionGate", () => {
  it("suppresses emission during the reconnect window and logs once", () => {
    ...
    expect(gate.evaluate(1_001, options)).toEqual({
      allowed: false,
      reason: "reconnect-window",
      shouldLog: true,
    });
    expect(gate.evaluate(2_000, options)).toEqual({
      allowed: false,
      reason: "reconnect-window",
      shouldLog: false,
    });
    expect(gate.evaluate(1_000 + options.reconnectSuppressMs, options)).toEqual({ allowed: true });
  });

  it("logs again for each new reconnect window", () => {
    const gate = new DiscordPresenceEmissionGate();
    gate.noteGatewaySessionReset(0);
    expect(gate.evaluate(1, options)).toMatchObject({ shouldLog: true });
    gate.noteGatewaySessionReset(options.reconnectSuppressMs * 2);
    expect(gate.evaluate(options.reconnectSuppressMs * 2 + 1, options)).toMatchObject({
      shouldLog: true,
    });
  });
  ...
  it("does not suppress when the reconnect window is disabled", () => {
    const gate = new DiscordPresenceEmissionGate();
    gate.noteGatewaySessionReset(1_000);
    expect(gate.evaluate(1_001, { ...options, reconnectSuppressMs: 0 })).toEqual({ allowed: true });
  });
  ...
  it("rate-limits emission bursts within the sliding window", () => {
    ...
    expect(gate.evaluate(1_000, burstOptions)).toEqual({ allowed: true });
    expect(gate.evaluate(2_000, burstOptions)).toEqual({ allowed: true });
    expect(gate.evaluate(3_000, burstOptions)).toEqual({
      allowed: false,
      reason: "burst",
      shouldLog: true,
    });
    expect(gate.evaluate(4_000, burstOptions)).toEqual({
      allowed: false,
      reason: "burst",
      shouldLog: false,
    });
    // The window drains as old emissions age out; logging re-arms for the next burst.
    expect(gate.evaluate(12_500, burstOptions)).toEqual({ allowed: true });
    expect(gate.evaluate(12_600, burstOptions)).toEqual({ allowed: true });
    expect(gate.evaluate(12_700, burstOptions)).toEqual({
      allowed: false,
      reason: "burst",
      shouldLog: true,
    });
