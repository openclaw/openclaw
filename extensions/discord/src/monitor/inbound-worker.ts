...
async function processDiscordInboundJob(params: {
	...
	const processDiscordMessageImpl =
		params.testing?.processDiscordMessage ?? processDiscordMessage;
	await runDiscordTaskWithTimeout({
		...
		onTimeout: async (resolvedTimeoutMs) => {
			params.runtime.error?.(
				danger(
					`discord inbound worker timed out after ${formatDurationSeconds(
						resolvedTimeoutMs,
						{
							decimals: 1,
							unit: "seconds",
		...
		onErrorAfterTimeout: (error) => {
			params.runtime.error?.(
				danger(
					`discord inbound worker failed after timeout: ${String(error)}${contextSuffix}`,
				),
			);
		},
	});
	...
	return {
		enqueue(job) {
			void runQueue
				...
				.catch((error) => {
					params.runtime.error?.(
						danger(`discord inbound worker failed: ${String(error)}`),
					);
				});
		},
