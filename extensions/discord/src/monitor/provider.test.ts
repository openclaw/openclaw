...
describe("monitorDiscordProvider", () => {
	...
	const getHealthProbe = () => {
		...
		const firstCall = reconcileAcpThreadBindingsOnStartupMock.mock.calls.at(
			0,
		) as [ReconcileStartupParams] | undefined;
		const reconcileParams = firstCall?.[0];
		if (!reconcileParams?.healthProbe) {
			throw new Error(
				"healthProbe was not wired into ACP startup reconciliation",
			);
		}
		return reconcileParams.healthProbe as NonNullable<
			ReconcileStartupParams["healthProbe"]
		>;
	};
	...
	beforeAll(async () => {
		vi.doMock("../accounts.js", () => ({
			resolveDiscordAccount: (
				...args: Parameters<typeof resolveDiscordAccountMock>
		...
		({ monitorDiscordProvider, __testing: providerTesting } = await import(
			"./provider.js"
		));
	});
	...
	beforeEach(() => {
		...
		providerTesting.setCreateDiscordNativeCommand(((
			...args: Parameters<
				typeof providerTesting.setCreateDiscordNativeCommand
			>[0] extends ((...inner: infer P) => unknown) | undefined
				? P
				: never
		) =>
			createDiscordNativeCommandMock(
			...
			Parameters<typeof providerTesting.setCreateDiscordNativeCommand>[0]
		>);
		providerTesting.setRunDiscordGatewayLifecycle((...args) =>
			monitorLifecycleMock(
				...(args as Parameters<typeof monitorLifecycleMock>),
			),
		);
		...
		providerTesting.setLoadDiscordProviderSessionRuntime(
			(async () =>
				...
				Parameters<
					typeof providerTesting.setLoadDiscordProviderSessionRuntime
				>[0]
			>,
		);
	...
	it("falls back to legacy missing-session message classification", async () => {
		getAcpSessionStatusMock.mockRejectedValue(
			new Error("ACP session metadata missing"),
		);
	...
	it("registers plugin commands as native Discord commands", async () => {
		...
		const commandNames = (
			createDiscordNativeCommandMock.mock.calls as Array<unknown[]>
		)
			.map(
				(call) =>
					(call[0] as { command?: { name?: string } } | undefined)?.command
						?.name,
			)
	...
	it("configures Carbon native deploy by default", async () => {
		...
		expect(clientHandleDeployRequestMock).toHaveBeenCalledTimes(1);
		expect(getConstructedClientOptions().eventQueue?.listenerTimeout).toBe(
			120_000,
		);
	});
	...
	it("reports connected status on startup and shutdown", async () => {
		...
		expect(setStatus.mock.calls).toContainEqual([
	...
	it("logs Discord startup phases and early gateway debug events", async () => {
		...
		expect(
			messages.some((msg) => msg.includes("fetch-application-id:start")),
		).toBe(true);
		expect(
			messages.some((msg) => msg.includes("fetch-application-id:done")),
		).toBe(true);
		expect(messages.some((msg) => msg.includes("deploy-commands:start"))).toBe(
			true,
		);
		expect(messages.some((msg) => msg.includes("deploy-commands:done"))).toBe(
			true,
		);
		expect(
			messages.some((msg) => msg.includes("fetch-bot-identity:start")),
		).toBe(true);
		expect(
			messages.some((msg) => msg.includes("fetch-bot-identity:done")),
		).toBe(true);
		expect(
			messages.some(
				(msg) =>
					msg.includes("gateway-debug") &&
					msg.includes("WebSocket connection opened"),
			),
		).toBe(true);
	});
	...
	it("keeps Discord startup chatter quiet by default", async () => {
		...
		const messages = vi
			.mocked(runtime.log)
			.mock.calls.map((call) => String(call[0]));
		expect(messages.some((msg) => msg.includes("discord startup ["))).toBe(
			false,
		);
	});
});
