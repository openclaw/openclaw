...
const deliveryMocks = vi.hoisted(() => ({
	...
	deliverDiscordReply: vi.fn<(params: unknown) => Promise<void>>(
		async () => {},
	),
...
const configSessionsMocks = vi.hoisted(() => ({
	readSessionUpdatedAt: vi.fn<(params?: unknown) => number | undefined>(
		() => undefined,
	),
...
vi.spyOn(sendMessagesModule, "editMessageDiscord").mockImplementation(
	((
		...
		deliveryMocks.editMessageDiscord(
			channelId,
			messageId,
			payload,
			opts,
...
const conversationRuntimeModule = await import(
	"openclaw/plugin-sdk/conversation-runtime"
);
vi.spyOn(conversationRuntimeModule, "recordInboundSession").mockImplementation(
	((
		params: Parameters<
			typeof conversationRuntimeModule.recordInboundSession
		>[0],
...
function mockDispatchSingleBlockReply(payload: {
	...
	dispatchInboundMessage.mockImplementationOnce(
		async (params?: DispatchInboundParams) => {
...
beforeEach(() => {
	...
	resolveStorePath.mockReturnValue(
		"/tmp/openclaw-discord-process-test-sessions.json",
	);
...
function getLastRouteUpdate():
	...
	| undefined {
	const callArgs = recordInboundSession.mock.calls.at(-1) as
		| unknown[]
		| undefined;
...
function getLastDispatchCtx():
	...
	| undefined {
	const callArgs = dispatchInboundMessage.mock.calls.at(-1) as
		| unknown[]
		| undefined;
	const params = callArgs?.[0] as
...
function getReactionEmojis(): string[] {
	return (
		sendMocks.reactMessageDiscord.mock.calls as unknown as Array<
