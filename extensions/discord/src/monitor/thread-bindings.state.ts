...
const THREAD_BINDINGS_STATE_KEY = Symbol.for(
	"openclaw.discordThreadBindingsState",
);
...
function resolveThreadBindingsGlobalState(): ThreadBindingsGlobalState {
	if (!threadBindingsState) {
		...
		threadBindingsState =
			(globalStore[THREAD_BINDINGS_STATE_KEY] as
				| ThreadBindingsGlobalState
				| undefined) ?? createThreadBindingsGlobalState();
