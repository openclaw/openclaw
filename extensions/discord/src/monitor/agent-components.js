...
function resolveAgentComponentRoute(params) {
  return resolveAgentRoute({
    ...
    parentPeer: params.parentId ? { kind: "channel", id: params.parentId } : void 0
  });
}
...
function buildAgentButtonCustomId(componentId) {
  return `${AGENT_BUTTON_KEY}:componentId=${encodeURIComponent(componentId)}`;
}
function buildAgentSelectCustomId(componentId) {
...
async function dispatchDiscordComponentEvent(params) {
  ...
  const route = resolveAgentRoute({
    ...
    parentPeer: channelCtx.parentId ? { kind: "channel", id: channelCtx.parentId } : void 0
  });
