...
const hookMocks = vi.hoisted(() => ({
  ...
  listThreadBindingsBySessionKey: vi.fn((_params) => []),
...
function registerHandlersForTest(config = {
  channels: {
    discord: {
      threadBindings: {
        spawnSubagentSessions: true
...
function getRequiredHandler(handlers, hookName) {
...
function createSpawnEventWithoutThread() {
  return createSpawnEvent({
    label: "",
    requester: { threadId: void 0 }
  });
}
async function runSubagentSpawning(config, event = createSpawnEventWithoutThread()) {
...
describe("discord subagent hook handlers", () => {
  beforeEach(() => {
    ...
    hookMocks.resolveDiscordAccount.mockImplementation((params) => ({
      accountId: params?.accountId?.trim() || "default",
      config: {
        threadBindings: {
          spawnSubagentSessions: true
