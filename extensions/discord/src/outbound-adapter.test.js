...
vi.mock("./send.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    sendMessageDiscord: (...args) => hoisted.sendMessageDiscordMock(...args),
    sendPollDiscord: (...args) => hoisted.sendPollDiscordMock(...args),
    sendWebhookMessageDiscord: (...args) => hoisted.sendWebhookMessageDiscordMock(...args)
  };
});
vi.mock("./monitor/thread-bindings.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    getThreadBindingManager: (...args) => hoisted.getThreadBindingManagerMock(...args)
  };
});
...
describe("normalizeDiscordOutboundTarget", () => {
  ...
  it("returns error for undefined target", () => {
    expect(normalizeDiscordOutboundTarget(void 0).ok).toBe(false);
  });
  it("trims whitespace", () => {
...
describe("discordOutbound", () => {
  ...
  it("uses webhook persona delivery for bound thread text replies", async () => {
    ...
    expect(
      hoisted.sendWebhookMessageDiscordMock.mock.calls[0]?.[1]?.cfg
    ).toBe(cfg);
