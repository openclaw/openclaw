...
const mocks = vi.hoisted(() => ({
  fetchDiscord: vi.fn(),
  normalizeDiscordToken: vi.fn((token) => token.trim()),
...
describe("discord directory live lookups", () => {
  beforeEach(() => {
    ...
    mocks.normalizeDiscordToken.mockImplementation((token) => token.trim());
  });
  ...
  it("filters group channels by query and respects limit", async () => {
    mocks.fetchDiscord.mockImplementation(async (path) => {
