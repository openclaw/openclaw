  ...
  for (const input of params.entries) {
    ...
    if (parsed.channelId) {
      ...
      if (result.status === "not-found" && parsed.guildId) {
        ...
        if (guild) {
          ...
          const match2 = preferActiveMatch(matches);
          if (match2) {
            results.push({
              input,
              resolved: true,
              ...
              guildName: guild.name,
              channelId: match2.id,
              channelName: match2.name,
              archived: match2.archived
            });
            continue;
          }
        }
    ...
    if (parsed.guildId || parsed.guild) {
      ...
      let matches = channels.filter(
        (channel) => isNumericId ? channel.id === channelQuery : normalizeDiscordSlug(channel.name) === normalizedChannelQuery
      );
