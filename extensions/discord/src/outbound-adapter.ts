...
function normalizeDiscordApprovalPayload<
	T extends { text?: string; channelData?: unknown },
>(payload: T): T {
...
export const discordOutbound: ChannelOutboundAdapter = {
	...
	sendPayload: async (ctx) => {
		...
		const rawComponentSpec =
			...
			buildDiscordInteractiveComponents(payload.interactive);
		...
		const send =
			resolveOutboundSendDep<typeof sendMessageDiscord>(ctx.deps, "discord") ??
			sendMessageDiscord;
	...
	...createAttachedChannelResultAdapter({
		...
		sendText: async ({
			...
			const send =
				resolveOutboundSendDep<typeof sendMessageDiscord>(deps, "discord") ??
				sendMessageDiscord;
