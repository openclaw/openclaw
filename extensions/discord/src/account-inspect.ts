...
export type DiscordCredentialStatus =
	| "available"
...
export function inspectDiscordAccount(params: {
	...
	const enabled =
		params.cfg.channels?.discord?.enabled !== false && merged.enabled !== false;
	...
	const channelToken = inspectDiscordTokenValue(
		params.cfg.channels?.discord?.token,
	);
	if (channelToken) {
		return {
			accountId,
	...
	const envToken = allowEnv
		? normalizeSecretInputString(
				params.envToken ?? process.env.DISCORD_BOT_TOKEN,
			)
		: undefined;
	if (envToken) {
		return {
