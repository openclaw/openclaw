...
describe("chunkDiscordText", () => {
  it("splits tall messages even when under 2000 chars", () => {
    ...
    expect(text.length).toBeLessThan(2e3);
    const chunks = chunkDiscordText(text, { maxChars: 2e3, maxLines: 20 });
...
Done.`;
    const chunks = chunkDiscordText(text, { maxChars: 2e3, maxLines: 10 });
    expect(chunks.length).toBeGreaterThan(1);
    for (const chunk of chunks) {
      expect(hasBalancedFences(chunk)).toBe(true);
      expect(chunk.length).toBeLessThanOrEqual(2e3);
    }
...
_${body}_`;
    const chunks = chunkDiscordText(text, { maxLines: 10, maxChars: 2e3 });
    expect(chunks.length).toBeGreaterThan(1);
    for (const chunk of chunks) {
