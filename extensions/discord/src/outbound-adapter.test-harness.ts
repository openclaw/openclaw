...
type DiscordThreadBindingsModule =
	typeof import("./monitor/thread-bindings.js");
...
export async function installDiscordOutboundModuleSpies(
	hoisted: DiscordOutboundHoisted,
) {
	const sendModule = await import("./send.js");
	const mockedSendModule = await createDiscordSendModuleMock(
		hoisted,
		async () => sendModule,
	);
	...
	vi.spyOn(sendModule, "sendPollDiscord").mockImplementation(
		mockedSendModule.sendPollDiscord,
	);
	...
	const mockedThreadBindingsModule =
		await createDiscordThreadBindingsModuleMock(
			hoisted,
