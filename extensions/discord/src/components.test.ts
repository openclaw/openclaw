...
describe("discord components", () => {
	it("builds v2 containers with modal trigger", () => {
		...
		expect(
			result.entries.find((entry) => entry.kind === "button")?.callbackData,
		).toBe("codex:approve");
...
describe("discord component registry", () => {
	...
	const componentsRegistryModuleUrl = new URL(
	...
	it("shares registry state across duplicate module instances", async () => {
		...
		expect(
			second.resolveDiscordComponentEntry({ id: "btn_shared", consume: false }),
		).toMatchObject({
			id: "btn_shared",
			label: "Shared",
		});
