...
const BLOCK_ALIASES = /* @__PURE__ */ new Map([
  ["row", "actions"],
  ["action-row", "actions"]
]);
...
function readDiscordComponentSpec(raw) {
  ...
  if (modalRaw !== void 0) {
    ...
    const fields = fieldsRaw.map(
      (entry, idx) => parseModalField(entry, `components.modal.fields[${idx}]`, idx)
    );
    modal = {
...
function buildDiscordComponentMessage(params) {
  ...
  if (params.spec.modal) {
    ...
    if (lastChild instanceof Row) {
      const row = lastChild;
      const hasSelect = row.components.some((entry2) => isSelectComponent(entry2));
...
function buildDiscordComponentMessageFlags(components) {
  ...
  return hasV2 ? MessageFlags.IsComponentsV2 : void 0;
}
class DiscordFormModal extends Modal {
  constructor(params) {
    super();
