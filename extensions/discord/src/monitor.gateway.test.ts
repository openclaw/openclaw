...
function startGatewayWait(params?: {
	...
	const promise = waitForDiscordGatewayStop({
		...
		...(params?.onGatewayEvent
			? { onGatewayEvent: params.onGatewayEvent }
			: {}),
...
describe("waitForDiscordGatewayStop", () => {
	it("resolves on abort and disconnects gateway", async () => {
		const { abort, attachLifecycle, detachLifecycle, disconnect, promise } =
			startGatewayWait();
	...
	it("rejects on lifecycle stop events and disconnects", async () => {
		...
		const { detachLifecycle, disconnect, emitGatewayEvent, promise } =
			startGatewayWait();
	...
	it("keeps the lifecycle handler active until disconnect returns on abort", async () => {
		...
		const { abort, detachLifecycle, disconnect, emitGatewayEvent, promise } =
			startGatewayWait({
				onGatewayEvent,
				disconnect: () => {
