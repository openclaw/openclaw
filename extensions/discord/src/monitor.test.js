...
describe("discord mention gating", () => {
  ...
  it("applies autoThread mention rules based on thread ownership", () => {
    const cases = [
      ...
      { name: "unknown thread owner", threadOwnerId: void 0, expected: true }
    ];
...
describe("discord reply target selection", () => {
  it("handles off/first/all reply modes", () => {
    const cases = [
      { name: "off mode", replyToMode: "off", hasReplied: false, expected: void 0 },
      {
...
function makeReactionEvent(overrides) {
  ...
  const messageFetch = overrides?.messageFetch ?? vi.fn(async () => ({
    author: {
  ...
  return {
    ...
    emoji: { name: overrides?.emojiName ?? "\u{1F44D}", id: null },
    guild: overrides?.guild,
    rawMember: overrides?.memberRoleIds ? { roles: overrides.memberRoleIds } : void 0,
    user: {
      id: userId,
      bot: false,
      username: "testuser",
...
function makeReactionClient(options) {
  ...
  const channelName = options?.channelName ?? (channelType === ChannelType.DM ? void 0 : "test-channel");
  const parentId = options?.parentId;
  const parentName = options?.parentName ?? "parent-channel";
  return {
    fetchChannel: vi.fn(async (channelId) => {
      if (parentId && channelId === parentId) {
        return { type: ChannelType.GuildText, name: parentName, parentId: void 0 };
      }
...
describe("discord DM reaction handling", () => {
  ...
  it("processes DM reactions with or without guild allowlists", async () => {
    const cases = [
      { name: "no guild allowlist", guildEntries: void 0 },
      {
    ...
    for (const testCase of cases) {
      ...
      expect(text, testCase.name).toContain("\u{1F44D}");
    ...
    for (const testCase of cases) {
      ...
      const guildEntries = makeEntries({
        [guildId]: {
          ...
          users: testCase.users ? [...testCase.users] : void 0
        }
      });
