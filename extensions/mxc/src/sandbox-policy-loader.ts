...
export function resolveMachineSandboxPolicyPath(): string {
...
export function resolveUserSandboxPolicyPath(homeDir: string = homedir()): string {
  return win32.join(homeDir, ".openclaw", "sandbox-policy.json");
}
...
export function loadSandboxBaselinePolicy(
  ...
  const machinePolicyPath = options.machinePolicyPath ?? resolveMachineSandboxPolicyPath();
