...
async function prepareIsolatedCodexHome(baseDir: string): Promise<string> {
  ...
  await bridgeCodexAuthJson(codexHome);
  return codexHome;
}
...
async function realpathOrResolvedPath(filePath: string): Promise<string> {
  try {
...
async function bridgeCodexAuthJson(codexHome: string): Promise<void> {
  const sourceAuth = sourceCodexAuthPath();
  const targetAuth = path.join(codexHome, "auth.json");
  const [resolvedSourceAuth, resolvedTargetAuth] = await Promise.all([
    realpathOrResolvedPath(sourceAuth),
    realpathOrResolvedPath(targetAuth),
  ]);
  if (resolvedSourceAuth === resolvedTargetAuth) {
    return;
  }
  try {
  ...
  try {
    ...
    await fs.writeFile(targetAuth, await fs.readFile(sourceAuth), { flag: "wx", mode: 0o600 });
    if (process.platform !== "win32") {
