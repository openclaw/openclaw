...
import { type ResolvedAcpxPluginConfig, toAcpMcpServers } from "./config.js";
import { type AcpxVersionCheckResult, checkAcpxVersion } from "./ensure.js";
import {
	parseJsonLines,
...
function findSessionIdentifierEvent(
	events: AcpxJsonObject[],
): AcpxJsonObject | undefined {
	return events.find(
		(event) =>
...
export function decodeAcpxRuntimeHandleState(
	runtimeSessionName: string,
): AcpxHandleState | null {
...
export class AcpxRuntime implements AcpRuntime {
	...
	private async runHelpCheck(): Promise<
		Awaited<ReturnType<typeof spawnAndCollect>>
	> {
	...
	private async shouldReplaceEnsuredSession(params: {
		...
		const noSession = events.some(
			(event) => toAcpxErrorEvent(event)?.code === "NO_SESSION",
		);
		if (noSession) {
			this.logger?.warn?.(
	...
	async ensureSession(input: AcpRuntimeEnsureInput): Promise<AcpRuntimeHandle> {
		...
		const acpxRecordId = ensuredEvent
			? asOptionalString(ensuredEvent.acpxRecordId)
			: undefined;
		const agentSessionId = ensuredEvent
	...
	async getStatus(input: {
		...
		const detail =
			events.find((event) => !toAcpxErrorEvent(event)) ?? events[0];
		if (!detail) {
			return {
		...
		const pid =
			typeof detail.pid === "number" && Number.isFinite(detail.pid)
				? detail.pid
				: null;
		const summary = [
			`status=${status}`,
	...
	async setConfigOption(input: {
		...
		if (!key || !value) {
			throw new AcpRuntimeError(
				"ACP_TURN_FAILED",
				"ACP config option key/value are required.",
			);
		}
		const args = await this.buildVerbArgs({
			agent: state.agent,
	...
	async doctor(): Promise<AcpRuntimeDoctorReport> {
		...
		if (!result.ok && result.failure.kind === "version-check") {
			...
			const details = [
				...
				versionCheck.installedVersion
					? `installed=${versionCheck.installedVersion}`
					: null,
		...
		if (!result.ok && result.failure.kind === "help-check") {
			...
			this.healthy = false;
			if (helpResult.error) {
				const spawnFailure = resolveSpawnFailure(
					helpResult.error,
					this.config.cwd,
				);
				if (spawnFailure === "missing-command") {
			...
			return {
				...
				message:
					...
					`acpx exited with code ${helpResult.code ?? "unknown"}`,
			};
		}
	...
	private resolveHandleState(handle: AcpRuntimeHandle): AcpxHandleState {
		...
		return {
			name: legacyName,
			agent: deriveAgentFromSessionKey(
				handle.sessionKey,
	...
	private async runControlCommand(params: {
		...
		const errorEvent =
			events.map((event) => toAcpxErrorEvent(event)).find(Boolean) ?? null;
		if (errorEvent) {
