...
let missingCommandRuntime = null;
beforeAll(async () => {
...
describe("AcpxRuntime", () => {
  ...
  it("uses sessions new with --resume-session when resumeSessionId is provided", async () => {
    ...
    const resumeArgs = resumeEntry?.args ?? [];
  ...
  it("serializes text plus image attachments into ACP prompt blocks", async () => {
    ...
    const prompt = logs.find(
      (entry) => entry.kind === "prompt" && String(entry.sessionName ?? "") === "agent:codex:acp:with-image"
    );
    expect(prompt).toBeDefined();
  ...
  it("preserves provider auth env vars when runtime uses a custom acpx command", async () => {
    ...
    try {
      ...
      const logs = await readMockRuntimeLogEntries(logPath);
      const prompt = logs.find(
        (entry) => entry.kind === "prompt" && String(entry.sessionName ?? "") === "agent:codex:acp:custom-env"
      );
  ...
  it("preserves leading spaces across streamed text deltas", async () => {
    ...
    const prompt = logs.find(
      (entry) => entry.kind === "prompt" && String(entry.sessionName ?? "") === "agent:codex:acp:space"
    );
    expect(prompt).toBeDefined();
    const promptArgs = prompt?.args ?? [];
  ...
  it("routes ACPX commands through an MCP proxy agent when MCP servers are configured", async () => {
    ...
    try {
      ...
      const logs = await readMockRuntimeLogEntries(logPath);
      const ensureArgs = logs.find((entry) => entry.kind === "ensure")?.args ?? [];
      const setModeArgs = logs.find((entry) => entry.kind === "set-mode")?.args ?? [];
