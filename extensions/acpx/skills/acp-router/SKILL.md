---
name: acp-router
description: Route plain-language requests for Pi, Claude Code, Codex, Cursor, OpenCode, Gemini CLI, or ACP harness work into either OpenClaw ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow). For coding-agent thread requests, read this skill first, then use only `sessions_spawn` for thread creation.
user-invocable: false
---

# ACP Harness Router

When user intent is "run this in Pi/Claude Code/Codex/Cursor/OpenCode/Gemini/Kimi (ACP harness)", do not use subagent runtime or PTY scraping. Route through ACP-aware flows.

## Intent detection

Trigger this skill when the user asks OpenClaw to:

- run something in Pi / Claude Code / Codex / Cursor / OpenCode / Gemini
...
- Before creating any thread for Pi/Claude/Codex/Cursor/OpenCode/Gemini work, read this skill first in the same turn.
...
- PTY scraping of pi/claude/codex/cursor/opencode/gemini/kimi CLIs when `acpx` is available
...
When the user asks to start a coding harness in a thread (for example "start a codex/claude/cursor/pi/kimi thread"), treat that as an ACP runtime request and try to satisfy it end-to-end.
