---
name: acp-router
description: Route plain-language requests for Pi, Claude Code, Codex, OpenCode, Gemini CLI, or ACP harness work into either Bot ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow).
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@bot/acpx` extension.
