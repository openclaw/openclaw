...
async function runMullusiSubprocessOnce(params: {
  execPath: string;
  argv: string[];
  cwd: string;
  timeoutMs: number;
  ...
  const spawnTarget =
    process.platform === "win32"
      ? resolveWindowsMullusiSpawn(execPath, argv, env)
  ...
  return await new Promise<{ stdout: string }>((resolve, reject) => {
    ...
    child.stdout?.on("data", (chunk) => {
      ...
      if (stdoutBytes > maxStdoutBytes) {
        failAndTerminate("mullusi output exceeded maxStdoutBytes");
        return;
      }
      stdout += str;
    });
    ...
    const timer = setTimeout(() => {
      failAndTerminate("mullusi subprocess timed out");
    }, timeoutMs);
    ...
    child.once("exit", (code) => {
      if (code !== 0) {
        settle({
          ok: false,
          error: new Error(`mullusi failed (${code ?? "?"}): ${stderr.trim() || stdout.trim()}`),
        });
        return;
      }
...
function parseEnvelope(stdout: string): MullusiEnvelope {
  ...
  if (parsed === undefined) {
    throw new Error("mullusi returned invalid JSON");
  }

  if (!parsed || typeof parsed !== "object") {
    throw new Error("mullusi returned invalid JSON envelope");
  }

  const ok = (parsed as { ok?: unknown }).ok;
  if (ok === true || ok === false) {
    return parsed as MullusiEnvelope;
  }

  throw new Error("mullusi returned invalid JSON envelope");
}

function buildMullusiArgv(action: string, params: Record<string, unknown>): string[] {
  if (action === "run") {
...
export function createMullusiTool(api: MullusiPluginApi) {
  return {
    name: "mullusi",
    label: "Mullusi Workflow",
    description:
      "Run Mullusi pipelines as a local-first workflow runtime (typed JSON envelope + resumable approvals).",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      const execPath = "mullusi";
      const cwd = resolveCwd(params.cwd);
      const timeoutMs = typeof params.timeoutMs === "number" ? params.timeoutMs : 20_000;
      const maxStdoutBytes =
        typeof params.maxStdoutBytes === "number" ? params.maxStdoutBytes : 512_000;

      const argv = buildMullusiArgv(action, params);

      if (api.runtime?.version && api.logger?.debug) {
        api.logger.debug(`mullusi plugin runtime=${api.runtime.version}`);
      }

      const { stdout } = await runMullusiSubprocessOnce({
        execPath,
        argv,
        cwd,
        timeoutMs,
