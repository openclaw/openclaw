...
function _resolveRegistryDefaultSpeechProviderId(
	cfg?: OpenClawConfig,
): TtsProvider {
...
function resolveRawProviderConfig(
	...
	const direct =
		rawProviders[providerId] ?? (raw as Record<string, unknown>)[providerId];
...
function collectDirectProviderConfigEntries(
	...
	for (const [providerId, value] of Object.entries(rawProviders)) {
		const normalized =
			normalizeConfiguredSpeechProviderId(providerId) ?? providerId;
...
export function resolveTtsProviderOrder(
	...
	const normalizedPrimary =
		canonicalizeSpeechProviderId(primary, cfg) ?? primary;
...
function resolveTtsRequestSetup(params: {
	...
	const provider =
		canonicalizeSpeechProviderId(params.providerOverride, params.cfg) ??
		userProvider;
	return {
		config,
...
export async function synthesizeSpeech(params: {
	...
	const target =
		channelId && OPUS_CHANNELS.has(channelId) ? "voice-note" : "audio-file";
	...
	for (const provider of providers) {
		...
		try {
			...
			const synthesis = await resolvedProvider.synthesize({
				...
				target,
				providerOverrides:
					params.overrides?.providerOverrides?.[resolvedProvider.id],
				timeoutMs: config.timeoutMs,
			});
			return {
				success: true,
