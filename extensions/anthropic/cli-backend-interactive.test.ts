...
describe("buildAnthropicInteractiveCliBackend.normalizeConfig", () => {
  ...
  it("classifies command inherited from the legacy `anthropic-cli` key as a Claude binary, not a Bun override", () => {
    // ClawSweeper P2a regression on PR #81851: `resolveCliBackendConfig`
    // inherits sibling backend config by normalized provider id, and
    // `normalizeLegacyCliBackendKey` folds the legacy alias
    // `anthropic-cli` → `claude-cli` (plain `normalizeProviderId` does NOT —
    // it only normalizes casing/spacing). So an
    // operator who has only `agents.defaults.cliBackends.anthropic-cli`
    // configured (the legacy alias) still gets that command merged into the
    // interactive backend here. The classifier must look up the sibling
    // command by normalized id, not the literal "claude-cli" key —
    // otherwise the inherited claude binary is misclassified as a direct
    // Bun override and the MITM proxy never starts.
    const inherited = "/usr/local/bin/claude";
    const merged = { ...baseConfig, command: inherited };
    const result = backend.normalizeConfig!(merged, {
      ...
      config: {
        agents: {
          defaults: {
            cliBackends: {
              // Legacy alias key — must still be detected via normalized lookup.
...
describe("buildAnthropicInteractiveCliBackend Claude history-import parity", () => {
  ...
  it("appends OpenClaw system prompts on the first turn only (systemPromptWhen='first')", () => {
    // systemPromptMode matches plain claude-cli, but systemPromptWhen does not:
    // the interactive backend intentionally injects once ("first") while plain
    // claude-cli uses "always". Assert each value explicitly rather than
    // asserting systemPromptWhen parity (which no longer holds since plain
    // moved to "always").
