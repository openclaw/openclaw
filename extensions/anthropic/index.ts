...
function buildAnthropicAuthDoctorHint(params: {
	...
	const cfgProvider =
		params.config?.auth?.profiles?.[legacyProfileId]?.provider;
...
async function runAnthropicSetupTokenNonInteractive(ctx: {
	...
	const profileId =
		ctx.opts.tokenProfileId?.trim() ||
		buildTokenProfileId({ provider: PROVIDER_ID, name: "" });
	upsertAuthProfile({
		profileId,
...
async function runAnthropicCliMigration(
	ctx: ProviderAuthContext,
): Promise<ProviderAuthResult> {
	if (!hasClaudeCliAuth()) {
		throw new Error(
	...
	return {
		...
		agents: {
			...
			defaults: {
				...
				model: {
					...(Array.isArray(currentFallbacks)
			...
			auth: [
				{
					...
					run: async (ctx: ProviderAuthContext) =>
						await runAnthropicCliMigration(ctx),
				...
				},
				{
					...
					run: async (ctx: ProviderAuthContext) =>
						await runAnthropicSetupToken(ctx),
					runNonInteractive: async (ctx) =>
