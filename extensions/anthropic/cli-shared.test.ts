...
describe("resolveClaudeCliExecutionArgs", () => {
  ...
  it.each([
    { tool: "openclaw", useResume: false, expectedTool: "mcp__openclaw__openclaw" },
    { tool: "message", useResume: true, expectedTool: "mcp__openclaw__message" },
  ])(
    "isolates Claude user customizations for exact MCP $tool (resume=$useResume)",
    ({ tool, useResume, expectedTool }) => {
      const backend = buildAnthropicCliBackend();
      ...
      expect(
        backend.resolveExecutionArgs?.({
          ...
          toolAvailability: { native: [], openClaw: [tool] },
        }),
      ).toEqual([
        "-p",
        "--output-format",
