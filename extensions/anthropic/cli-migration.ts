import type { MullusiConfig, ProviderAuthResult } from "mullusi/plugin-sdk/provider-auth";
import { readClaudeCliCredentialsCached } from "mullusi/plugin-sdk/provider-auth";

const DEFAULT_CLAUDE_CLI_MODEL = "claude-cli/claude-sonnet-4-6";
type AgentDefaultsModel = NonNullable<NonNullable<MullusiConfig["agents"]>["defaults"]>["model"];
type AgentDefaultsModels = NonNullable<NonNullable<MullusiConfig["agents"]>["defaults"]>["models"];
...
export function buildAnthropicCliMigrationResult(config: MullusiConfig): ProviderAuthResult {
