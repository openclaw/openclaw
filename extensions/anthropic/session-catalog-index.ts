...
export async function readIndexRecords(
  ...
  const { results: rejectedIndexSidechains } = await runTasksWithConcurrency({
    tasks: [...rejectedIndexPaths, ...lateRejectedIndexPaths].map(
      (filePath) => async () =>
        probeRejectedSessionIndex(filePath, () => {
          context.complete = false;
        }),
    ),
    limit: CLAUDE_CATALOG_IO_CONCURRENCY,
  ...
  for (const rejectedSidechainIds of rejectedIndexSidechains) {
    for (const sessionId of rejectedSidechainIds) {
