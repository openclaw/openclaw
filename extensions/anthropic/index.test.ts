...
describe("anthropic provider replay hooks", () => {
  ...
  it.each([
    {
      name: "publishes Claude Opus 5 CLI metadata without downgrading its API contract",
      id: "claude-opus-5",
      label: "Claude Opus 5",
    },
    {
      name: "publishes Claude Sonnet 5 CLI metadata without downgrading its API contract",
      id: "claude-sonnet-5",
      label: "Claude Sonnet 5",
    },
    {
      name: "publishes Claude Fable 5 CLI metadata without downgrading its API contract",
      id: "claude-fable-5",
    ...
    expect(buildClaudeCliCatalogEntries().find((model) => model.id === id)).toMatchObject({
      ...
      maxTokens: 128_000,
      mediaInput: { image: { maxSidePx: 2576, preferredSidePx: 2576, tokenMode: "provider" } },
    });
  });
