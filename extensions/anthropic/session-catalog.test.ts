...
function captureConfiguredCatalog(config: () => OpenClawConfig): SessionCatalogProvider {
  return captureCatalogProvider({ config: { current: config } } as unknown as PluginRuntime);
}
...
describe("Claude session catalog", () => {
  ...
  it("does not advertise creation without a configured Claude CLI route", () => {
    ...
    const provider = captureConfiguredCatalog(() => config);
