// Tenki helper module supports config behavior.
...
const TENKI_MANAGED_REMOTE_ROOTS = [
...
function isManagedTenkiRemotePath(value: string): boolean {
  return TENKI_MANAGED_REMOTE_ROOTS.some(
    (root) => value === root || value.startsWith(`${root}/`),
  );
}

function normalizeTenkiRemotePath(
  ...
  if (!normalized.startsWith("/")) {
    throw new Error(`Tenki ${fieldName} must be absolute: ${candidate}`);
  }
  if (!isManagedTenkiRemotePath(normalized)) {
    throw new Error(
      `Tenki ${fieldName} must stay under ${TENKI_MANAGED_REMOTE_ROOTS.join(" or ")}: ${candidate}`,
    );
  }
  return normalized;
}
...
export function resolveTenkiPluginConfig(value: unknown): ResolvedTenkiPluginConfig {
  if (value === undefined) {
    // The built-in defaults are managed Tenki roots, so they do not need to
    // flow back through normalizeTenkiRemotePath.
    return {
      mode: DEFAULT_MODE,
  ...
  return {
    ...
    remoteWorkspaceDir: normalizeTenkiRemotePath(
      cfg.remoteWorkspaceDir,
      DEFAULT_REMOTE_WORKSPACE_DIR,
      "remoteWorkspaceDir",
    ),
    remoteAgentWorkspaceDir: normalizeTenkiRemotePath(
