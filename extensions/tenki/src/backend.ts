// Tenki plugin module implements backend behavior.
...
import type { TenkiSandboxBackend } from "./backend.types.js";
import {
  buildValidatedExecRemoteCommand,
  buildRemoteWorkdirValidationCommand,
  buildRemoteCommand,
  createTenkiSshSession,
  runTenkiCli,
  buildTenkiGetArgs,
  buildTenkiDeleteArgs,
  buildTenkiCreateArgs,
  type TenkiExecContext,
} from "./cli.js";
import { resolveTenkiPluginConfig, type ResolvedTenkiPluginConfig } from "./config.js";
import { createTenkiFsBridge } from "./fs-bridge.js";
import {
  DEFAULT_TENKI_MIRROR_EXCLUDE_DIRS,
  movePathWithCopyFallback,
  replaceDirectoryContents,
  stageDirectoryContents,
} from "./mirror.js";

type CreateTenkiSandboxBackendFactoryParams = {
...
export function buildTenkiDirectoryUploadArgs(params: {
  sandboxName: string;
  localPath: string;
  remotePath: string;
}): string[] {
...
export const ENSURE_TENKI_REMOTE_REAL_DIRECTORY_SCRIPT = [
  "set -e",
  'target="$1"',
  'root="${2:-$1}"',
...
export function buildTenkiSshExecEnv(): NodeJS.ProcessEnv {
  return sanitizeEnvVars(process.env).allowed;
}

export type { TenkiFsBridgeContext, TenkiSandboxBackend } from "./backend.types.js";

export function createTenkiSandboxBackendFactory(
  params: CreateTenkiSandboxBackendFactoryParams,
): SandboxBackendFactory {
...
export function createTenkiSandboxBackendManager(params: {
  ...
  return {
    async describeRuntime({ entry, config }) {
      const execContext: TenkiExecContext = {
        config: resolveTenkiPluginConfigFromConfig(config, params.pluginConfig),
        sandboxName: entry.containerName,
      };
      const result = await runTenkiCli({
        context: execContext,
        args: buildTenkiGetArgs(entry.containerName),
      });
    ...
    async removeRuntime({ entry }) {
      ...
      await runTenkiCli({
        context: execContext,
        args: buildTenkiDeleteArgs(entry.containerName),
      });
    },
  };
}
...
async function createTenkiSandboxBackend(params: {
  ...
  const sandboxName = buildTenkiSandboxName(params.createParams.scopeKey);
  const execContext: TenkiExecContext = {
    config: params.pluginConfig,
    sandboxName,
  };
...
class TenkiSandboxBackendImpl {
  ...
  constructor(
    private readonly params: {
      createParams: CreateSandboxBackendParams;
      execContext: TenkiExecContext;
      remoteWorkspaceDir: string;
      remoteAgentWorkspaceDir: string;
    },
  ) {}

  asHandle(): TenkiSandboxBackend {
    return {
      id: "tenki",
  ...
  async prepareExec(params: {
    ...
    const sshSession = await createTenkiSshSession({
  ...
  private async runRemoteShellScriptInternal(
    ...
    const session = await createTenkiSshSession({
      context: this.params.execContext,
    });
    try {
  ...
  async syncLocalPathToRemote(localPath: string, remotePath: string): Promise<void> {
    ...
    if (result.code !== 0) {
      throw new Error(result.stderr.trim() || "tenki sandbox upload failed");
    }
  }
  ...
  private resolveRemoteTarget(remotePath: string): { root: string; relativePath: string } {
    ...
    throw new Error(`Remote path escapes Tenki managed roots: ${remotePath}`);
  }
  ...
  private async ensureSandboxExistsInner(): Promise<void> {
    const getResult = await runTenkiCli({
      context: this.params.execContext,
      args: buildTenkiGetArgs(this.params.execContext.sandboxName),
      cwd: this.params.createParams.workspaceDir,
    });
    if (getResult.code === 0) {
      return;
    }
    const createArgs = buildTenkiCreateArgs({
      sandboxName: this.params.execContext.sandboxName,
      config: this.params.execContext.config,
    });
    const createResult = await runTenkiCli({
      context: this.params.execContext,
      args: createArgs,
      cwd: this.params.createParams.workspaceDir,
    ...
    if (createResult.code !== 0) {
      throw new Error(createResult.stderr.trim() || "tenki sandbox create failed");
    }
  ...
  private async syncSkillsWorkspaceToRemote(): Promise<void> {
    ...
    await this.runRemoteShellScriptInternal({
      script: `${ENSURE_TENKI_REMOTE_REAL_DIRECTORY_SCRIPT}\nfind "$1" -mindepth 1 -maxdepth 1 -exec rm -rf -- {} +`,
  ...
  private async syncWorkspaceFromRemote(): Promise<void> {
    await withTempWorkspace(
      { rootDir: resolveTenkiTmpRoot(), prefix: "openclaw-tenki-sync-" },
      async ({ dir: tmpDir }) => {
        const result = await runTenkiCli({
          context: this.params.execContext,
          args: [
        ...
        if (result.code !== 0) {
          throw new Error(result.stderr.trim() || "tenki sandbox download failed");
        }
        ...
        try {
          await replaceDirectoryContents({
            ...
            // the remote sandbox.
            excludeDirs: DEFAULT_TENKI_MIRROR_EXCLUDE_DIRS,
          });
        } finally {
  ...
  private async uploadPathToRemote(localPath: string, remotePath: string): Promise<void> {
    await withTempWorkspace(
      { rootDir: resolveTenkiTmpRoot(), prefix: "openclaw-tenki-upload-" },
      async ({ dir: tmpDir }) => {
        ...
        for (const entry of stagedEntries) {
          const result = await runTenkiCli({
            context: this.params.execContext,
            args: buildTenkiDirectoryUploadArgs({
              sandboxName: this.params.execContext.sandboxName,
...
export function buildTenkiSandboxName(scopeKey: string): string {
...
async function moveMaterializedSkillsShadowAside(params: {
  ...
  const preserveRoot = await fs.mkdtemp(
    path.join(path.dirname(params.tmpDir), "openclaw-tenki-preserve-"),
  );
...
function resolveTenkiTmpRoot(): string {
...
function normalizeRemotePath(remotePath: string): string {
  ...
  if (!path.posix.isAbsolute(normalized)) {
    throw new Error(`Tenki remote path must be absolute: ${remotePath}`);
  }
  return normalized;
}
