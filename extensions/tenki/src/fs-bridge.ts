// Tenki plugin module implements fs bridge behavior.
...
import type { TenkiFsBridgeContext, TenkiSandboxBackend } from "./backend.types.js";
...
export function createTenkiFsBridge(params: {
  sandbox: TenkiFsBridgeContext;
  backend: TenkiSandboxBackend;
}): SandboxFsBridge {
  return new TenkiFsBridge(params.sandbox, params.backend);
}

class TenkiFsBridge implements SandboxFsBridge {
  ...
  constructor(
    private readonly sandbox: TenkiFsBridgeContext,
    private readonly backend: TenkiSandboxBackend,
  ) {}
  ...
  async rename(params: {
    ...
    if (from.mountHostRoot !== to.mountHostRoot) {
      throw new Error("Tenki cross-root mirror renames require pinned fs-safe support");
    }
  ...
  private requireHostPath(target: ResolvedMountPath): string {
    if (!target.hostPath) {
      throw new Error(
        `Tenki mirror bridge requires a local host path: ${target.containerPath}`,
      );
    }
...
async function assertSameDeviceRenameSupported(params: {
  ...
  if (sourceStats.dev !== destinationParentStats.dev) {
    throw new Error("Tenki cross-device mirror renames require pinned fs-safe support");
  }
}
