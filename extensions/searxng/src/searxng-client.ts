...
import { withWebSearchEndpoint } from "openclaw/plugin-sdk/provider-web-search";
...
export async function runSearXNGSearch(
  ...
  const payload = await withWebSearchEndpoint(
    {
      url: url.toString(),
      timeoutSeconds,
      init: {
      ...
      policy: {
        allowPrivateNetwork: resolveSearXNGAllowPrivateNetwork(params.cfg),
      },
    },
