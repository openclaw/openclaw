...
export function resolveSearXNGAllowPrivateNetwork(cfg?: OpenClawConfig): boolean {
  const search = resolveSearXNGSearchConfig(cfg);
  if (typeof search?.allowPrivateNetwork === "boolean") {
    return search.allowPrivateNetwork;
  }
  const global = cfg?.tools?.web?.search as { allowPrivateNetwork?: boolean } | undefined;
  return global?.allowPrivateNetwork ?? false;
}
