...
export function replaceCard(state: WorkboardUiState, card: WorkboardCard) {
  ...
  next.push(card);
  setWorkboardCards(state, applyPendingCardRemovals(next, state.pendingCardRemovals));
}
...
export function applyReferenceUpdatesToPendingCardRemovals(
  pendingRemovals: Map<string, WorkboardCardRemoval>,
  ...
  for (const receipt of referenceUpdates) {
    for (const [pendingCardId, removal] of pendingRemovals) {
      const card = removal.card;
      if (!card || card.id !== receipt.id || card.updatedAt !== receipt.previousUpdatedAt) {
        continue;
      }
      pendingRemovals.set(pendingCardId, {
        ...removal,
        card: { ...card, updatedAt: receipt.updatedAt },
      });
    }
  }
}
