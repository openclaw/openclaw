...
const mocks = vi.hoisted(() => ({
  runDetachedWebhookWork: vi.fn(async <T>(run: () => Promise<T>) => await run()),
}));
...
afterEach(async () => {
  ...
  mocks.runDetachedWebhookWork.mockReset();
  mocks.runDetachedWebhookWork.mockImplementation(async <T>(run: () => Promise<T>) => await run());
});

describe("createRcsIngressSpool", () => {
  it("keeps delivery admitted after the pump task releases its async context", async () => {
    const stateDir = await createStateDir();
    const taskContext = new AsyncLocalStorage<{ released: boolean }>();
    mocks.runDetachedWebhookWork.mockImplementation(async <T>(run: () => Promise<T>) => {
      const task = { released: false };
      try {
        return await taskContext.run(task, async () => {
    ...
    let deliveryContextReleased: boolean | undefined;
    const spool = createRcsIngressSpool({
      ...
      deliver: async (_message, lifecycle) => {
        ...
        deliveryContextReleased = taskContext.getStore()?.released;
    ...
    spool.start();
    await spool.enqueue(form("SM-detached-delivery"));
    await spool.waitForIdle();

    expect(deliveryContextReleased).toBe(false);
    expect(mocks.runDetachedWebhookWork).toHaveBeenCalled();
  });
