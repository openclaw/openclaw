...
const recordRcsStatusEvent = vi.hoisted(() => vi.fn());

vi.mock("./status-store.js", () => ({
...
const ssrfPolicyFromHttpBaseUrlAllowedOrigin = vi.hoisted(() =>
  vi.fn((baseUrl: string) => ({ allowedOrigins: [new URL(baseUrl).origin] })),
);
...
function expectLogCallsToExclude(log: ReturnType<typeof vi.fn>, values: string[]): void {
  const calls = JSON.stringify(log.mock.calls);
...
describe("createRcsWebhookHandler", () => {
  ...
  it("logs replay detection without the Twilio message identifier", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith("RCS webhook ignored replayed message");
    expectLogCallsToExclude(log.warn, ["SM-private-replay"]);
  });
  ...
  it("acks and drops messages after the per-sender rate limit", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith("RCS webhook sender rate limit exceeded");
    expectLogCallsToExclude(log.warn, ["+15551234567", "SM-sender-limit-30"]);
  });
  ...
  it("acks validated callbacks over the IP rate limit with empty TwiML instead of 429", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "RCS webhook rate limit exceeded; acknowledged validated callback without dispatch",
    );
    expectLogCallsToExclude(log.warn, ["203.0.113.31", "+15559990001", "SM-ip-ack-dropped"]);
  });
  ...
  it("keeps 429 for over-limit traffic when signature validation is disabled", async () => {
    ...
    expect(dispatchRcsInboundEvent).toHaveBeenCalledTimes(600);
    expect(log.warn).toHaveBeenCalledWith("RCS webhook rate limit exceeded");
    expectLogCallsToExclude(log.warn, ["203.0.113.33", "+15559990004", "SM-ip-nosig-throttled"]);
  });
});
...
describe("createRcsSharedTwilioWebhookHandler", () => {
  beforeEach(() => {
    ...
    ssrfPolicyFromHttpBaseUrlAllowedOrigin.mockClear();
  });
  ...
  it("forwards non-RCS payloads to the internal SMS webhook path", async () => {
    ...
    const forwardCall = fetchWithSsrFGuard.mock.calls[0]?.[0];
    expect(forwardCall?.url).toBe("http://127.0.0.1:18789/webhooks/sms/native?foo=bar");
    expect(forwardCall?.policy).toEqual({
      allowedOrigins: ["http://127.0.0.1:18789"],
    });
    expect(ssrfPolicyFromHttpBaseUrlAllowedOrigin).toHaveBeenCalledWith("http://127.0.0.1:18789");
  ...
  it("acks validated over-limit callbacks without dispatch or SMS forward", async () => {
    ...
    expect(dispatchRcsInboundEvent).toHaveBeenCalledTimes(600);
    expect(log.warn).toHaveBeenCalledWith(
      "RCS shared webhook rate limit exceeded; acknowledged validated callback without dispatch",
    );
    ...
    expect(fetchWithSsrFGuard).not.toHaveBeenCalled();
    expect(log.warn).toHaveBeenCalledWith(
      "RCS shared webhook rate limit exceeded; acknowledged validated callback without SMS forward",
    );
    expectLogCallsToExclude(log.warn, [
      "203.0.113.40",
      "+15551234567",
      "+15559990005",
      "SM-shared-ack-dropped",
      "SM-shared-sms-dropped",
    ]);
  });
...
describe("createRcsStatusCallbackHandler", () => {
  it("logs delivery status without the Twilio message identifier", async () => {
    recordRcsStatusEvent.mockClear();
    const log = { info: vi.fn() };
    const body =
      "AccountSid=AC123&MessageSid=SM-private-status&MessageStatus=failed&ErrorCode=30008";
    const signature = computeTwilioSignature({
      url: "https://gateway.example.com/webhooks/rcs/status",
      authToken: "secret",
      form: parseTwilioFormBody(body),
    });
    const handler = createRcsStatusCallbackHandler({
      cfg: {},
      account: createAccount(),
      log,
    });
    ...
    await handler(createRequest({ body, signature, url: "/webhooks/rcs/status" }), res);

    expect(res.statusCode).toBe(200);
    expect(recordRcsStatusEvent).toHaveBeenCalledWith(
      "default",
      expect.objectContaining({ messageSid: "SM-private-status", status: "failed" }),
    );
    expect(log.info).toHaveBeenCalledWith("RCS message status=failed error=30008");
    expectLogCallsToExclude(log.info, ["SM-private-status"]);
  });
});
