...
describe("dispatchRcsInboundEvent", () => {
  it("keeps sender identifiers out of access and pairing diagnostics", async () => {
    const unauthorized = createRuntime();
    const unauthorizedLog = { warn: vi.fn() };
    await dispatchRcsInboundEvent({
      cfg: {},
      account: createAccount({ allowFrom: [] }),
      channelRuntime: unauthorized.runtime,
      msg: msg({ body: "hello" }),
      receivedAt: 1_700_000_000_000,
      log: unauthorizedLog,
    });

    expect(unauthorizedLog.warn).toHaveBeenCalledWith("RCS sender is not authorized");
    expect(JSON.stringify(unauthorizedLog.warn.mock.calls)).not.toContain(RCS_FROM);

    const pairing = createRuntime();
    const pairingLog = { info: vi.fn(), warn: vi.fn() };
    await dispatchRcsInboundEvent({
      cfg: {},
      account: createAccount({ dmPolicy: "pairing", allowFrom: [] }),
      channelRuntime: pairing.runtime,
      msg: msg({ body: "hello" }),
      receivedAt: 1_700_000_000_001,
      log: pairingLog,
    });

    expect(pairingLog.info).toHaveBeenCalledWith("RCS pairing request created");
    expect(sendRcsTextChunks).toHaveBeenCalledWith(
      expect.objectContaining({ to: RCS_FROM, text: expect.stringContaining("PAIR123") }),
    );
    expect(JSON.stringify([pairingLog.info.mock.calls, pairingLog.warn.mock.calls])).not.toContain(
      RCS_FROM,
    );

    sendRcsTextChunks.mockRejectedValueOnce(new Error(`provider rejected ${RCS_FROM}`));
    const failedPairing = createRuntime();
    const failedPairingLog = { info: vi.fn(), warn: vi.fn() };
    await dispatchRcsInboundEvent({
      cfg: {},
      account: createAccount({ dmPolicy: "pairing", allowFrom: [] }),
      channelRuntime: failedPairing.runtime,
      msg: msg({ body: "hello" }),
      receivedAt: 1_700_000_000_002,
      log: failedPairingLog,
    });

    expect(failedPairingLog.warn).toHaveBeenCalledWith("RCS pairing reply failed");
    expect(JSON.stringify(failedPairingLog.warn.mock.calls)).not.toContain(RCS_FROM);
  });

  it("keeps sender identifiers out of reply-start diagnostics", async () => {
    const log = { info: vi.fn() };
    const { turn } = await resolveAuthorizedRcsTurn({
      body: "hello",
      receivedAt: 1_700_000_000_003,
      log,
    });

    turn.dispatcherOptions.onReplyStart();

    expect(log.info).toHaveBeenCalledWith("RCS reply started");
