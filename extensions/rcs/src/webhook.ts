...
function rejectRateLimitedRequest(params: {
  ...
  params.log?.warn?.(`${params.scope} rate limit exceeded`);
...
async function dispatchVerifiedRcsForm(params: {
  ...
  if (params.rateLimited) {
    ...
    params.log?.warn?.(
      `${params.scope} rate limit exceeded; acknowledged validated callback without dispatch`,
    );
  ...
  if (
    ...
    params.log?.warn?.("RCS webhook sender rate limit exceeded");
  ...
  if (verdict.duplicate) {
    params.log?.warn?.("RCS webhook ignored replayed message");
  }
  respondTwiml(params.res, 200);
}
...
async function forwardSharedSmsWebhook(params: {
  ...
  try {
    guarded = await fetchWithSsrFGuard({
      url: forwardUrl,
      policy: ssrfPolicyFromHttpBaseUrlAllowedOrigin(localOriginBaseUrl),
...
export function createRcsWebhookHandler(
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      if (rateLimited) {
        return rejectRateLimitedRequest({ scope: "RCS webhook", log: params.log, res });
      }
...
export function createRcsSharedTwilioWebhookHandler(
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      if (rateLimited) {
        return rejectRateLimitedRequest({ scope: "RCS shared webhook", log: params.log, res });
      }
      respondTwiml(res, 400, "Invalid request body");
      return true;
    }
...
export function createRcsStatusCallbackHandler(params: RcsStatusCallbackHandlerParams) {
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    params.log?.info?.(
      `RCS message status=${event.status}${event.errorCode ? ` error=${event.errorCode}` : ""}`,
    );
