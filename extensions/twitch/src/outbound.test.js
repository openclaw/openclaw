...
vi.mock("./utils/twitch.js", () => ({
  normalizeTwitchChannel: (channel) => channel.toLowerCase().replace(/^#/, ""),
  missingTargetError: (channel, hint) => new Error(`Missing target for ${channel}. Provide ${hint}`)
}));
function assertResolvedTarget(result) {
  if (!result.ok) {
    throw result.error;
  }
  return result.to;
}
function expectTargetError(resolveTarget, params, expectedMessage) {
...
describe("outbound", () => {
  ...
  describe("sendText", () => {
    ...
    it("should throw when no channel specified", async () => {
      const { getAccountConfig } = await import("./config.js");
      const accountWithoutChannel = { ...mockAccount, channel: void 0 };
