/**
 * Twitch channel plugin for Activi.
 *
...
import { buildChannelConfigSchema } from "activi/plugin-sdk";
 ...
 * for Activi. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ActiviConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ActiviConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ActiviConfig): boolean => {
