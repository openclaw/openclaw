/**
 * Twitch channel plugin for EasyHub.
 *
...
import { buildChannelConfigSchema } from "EasyHub/plugin-sdk";
import type {
 ...
 * for EasyHub. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: EasyHubConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: EasyHubConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: EasyHubConfig): boolean => {
