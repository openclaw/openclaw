/**
 * Twitch channel plugin for SmartAgentNeo.
 *
...
import { buildChannelConfigSchema } from "smart-agent-neo/plugin-sdk";
import type {
 ...
 * for SmartAgentNeo. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SmartAgentNeoConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SmartAgentNeoConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SmartAgentNeoConfig): boolean => {
