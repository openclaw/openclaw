...
export function formatPendingRequests(
...
function normalizeNotifyState(raw: unknown): NotifyStateFile {
	const root =
		typeof raw === "object" && raw !== null
			...
			: {};
	const subscribersRaw = Array.isArray(root.subscribers)
		? root.subscribers
		: [];
	const notifiedRaw =
		typeof root.notifiedRequestIds === "object" &&
		root.notifiedRequestIds !== null
	...
	for (const item of subscribersRaw) {
		...
		const messageThreadId =
			typeof record.messageThreadId === "string"
				...
				: typeof record.messageThreadId === "number" &&
						Number.isFinite(record.messageThreadId)
...
function upsertNotifySubscriber(
	...
	const index = subscribers.findIndex(
		(entry) => notifySubscriberKey(entry) === key,
	);
...
function buildPairingRequestNotificationText(
...
async function notifySubscriber(params: {
	...
	const adapter =
		await params.api.runtime.channel.outbound.loadAdapter("telegram");
	...
	try {
		await send({
			...
			text: params.text,
			...(params.subscriber.accountId
				? { accountId: params.subscriber.accountId }
				: {}),
...
export async function handleNotifyCommand(params: {
	...
	const current = state.subscribers.find(
