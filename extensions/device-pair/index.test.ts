...
vi.mock("./api.js", () => {
	return {
		...
		resolvePreferredOpenClawTmpDir:
			pluginApiMocks.resolvePreferredOpenClawTmpDir,
...
function createCommandContext(
	params?: Partial<PluginCommandContext>,
): PluginCommandContext {
	return {
		channel: "webchat",
...
describe("device-pair /pair qr", () => {
	beforeEach(async () => {
		...
		await fs.mkdir(pluginApiMocks.resolvePreferredOpenClawTmpDir(), {
			recursive: true,
		});
	});

	afterEach(async () => {
		await fs.rm(pluginApiMocks.resolvePreferredOpenClawTmpDir(), {
			recursive: true,
			force: true,
		});
	});
	...
	it("returns an inline QR image for webchat surfaces", async () => {
		...
		const result = await command.handler(
			createCommandContext({ channel: "webchat" }),
		);
		...
		expect(text).toContain(
			"![OpenClaw pairing QR](data:image/png;base64,ZmFrZXBuZw==)",
		);
	...
	it("reissues the bootstrap token if webchat QR rendering fails before falling back", async () => {
		...
		pluginApiMocks.renderQrPngBase64.mockRejectedValueOnce(
	...
	it.each([
		...
		const sendMessage = vi
			.fn()
			.mockImplementation(async (_target, _caption, opts) => {
				if (opts?.mediaUrl) {
	...
	it("reissues the bootstrap token after QR delivery failure before falling back", async () => {
		...
		expect(text).toContain("Pairing setup code generated.");
		expect(text).toContain(
			"If this code leaks or you are done, run /pair cleanup",
		);
	});
	...
	it("falls back to the setup code instead of ASCII when the channel cannot send media", async () => {
		...
		expect(text).toContain(
			"QR image delivery is not available on this channel",
		);
