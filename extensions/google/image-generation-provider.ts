...
function mapSizeToImageConfig(
	...
	const imageSize =
		longestEdge >= 3072 ? "4K" : longestEdge >= 1536 ? "2K" : undefined;
	...
	return {
		...
		async generateImage(req) {
			...
			const allowPrivate = Boolean(
				req.cfg?.models?.providers?.google?.baseUrl?.trim(),
			);
			...
			const resolvedImageConfig = {
				...
				...(req.aspectRatio?.trim()
					? { aspectRatio: req.aspectRatio.trim() }
					: {}),
			...
			try {
				...
				const images = (payload.candidates ?? [])
					...
					.map((part) => {
						...
						const mimeType =
							inline?.mimeType ?? inline?.mime_type ?? DEFAULT_OUTPUT_MIME;
				...
				if (images.length === 0) {
					throw new Error(
						"Google image generation response missing image data",
					);
				}
