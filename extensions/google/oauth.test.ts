  ...
  `;
	...
	function installGeminiLayout(params: {
		...
		mockExistsSync.mockImplementation((p: string) => {
			...
			if (
				params.oauth2Exists &&
				normalized === normalizePath(layout.oauth2Path)
			) {
				return true;
			}
			return false;
	...
	function installNpmShimLayout(params: {
		oauth2Exists?: boolean;
		oauth2Content?: string;
	}) {
...
describe("loginGeminiCliOAuth", () => {
	...
	beforeEach(() => {
		...
		process.env.OPENCLAW_GEMINI_OAUTH_CLIENT_SECRET =
			"GOCSPX-test-client-secret"; // pragma: allowlist secret
	...
	it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
		...
		await runRemoteLoginExpectingProjectId(
			loginGeminiCliOAuth,
			"daily-project",
		);
		...
		expect(loadRequests.map((request) => request.url)).toEqual([
			LOAD_PROD,
			LOAD_DAILY,
		]);
	...
	it("falls back to GOOGLE_CLOUD_PROJECT when all loadCodeAssist endpoints fail", async () => {
		...
		expect(
			requests.filter((url) => url.includes("v1internal:loadCodeAssist")),
		).toHaveLength(3);
		expect(requests.some((url) => url.includes("v1internal:onboardUser"))).toBe(
			false,
		);
	});
});
