...
const _GOOGLE_GEMINI_CLI_DEFAULT_MODEL =
...
let googleImageGenerationProviderPromise: Promise<ImageGenerationProvider> | null =
	null;
let googleMediaUnderstandingProviderPromise: Promise<MediaUnderstandingProvider> | null =
	null;
...
async function loadGoogleGeminiCliProvider(): Promise<ProviderPlugin> {
	if (!googleGeminiCliProviderPromise) {
		googleGeminiCliProviderPromise = import("./gemini-cli-provider.js").then(
			(mod) => {
				...
				if (!provider) {
					throw new Error(
						"google gemini cli provider missing provider registration",
					);
				}
				return provider;
			},
...
async function loadGoogleImageGenerationProvider(): Promise<ImageGenerationProvider> {
	if (!googleImageGenerationProviderPromise) {
		googleImageGenerationProviderPromise = import(
			"./image-generation-provider.js"
		).then((mod) => mod.buildGoogleImageGenerationProvider());
	}
...
async function loadGoogleMediaUnderstandingProvider(): Promise<MediaUnderstandingProvider> {
	if (!googleMediaUnderstandingProviderPromise) {
		googleMediaUnderstandingProviderPromise = import(
			"./media-understanding-provider.js"
		).then((mod) => mod.googleMediaUnderstandingProvider);
	}
...
async function loadGoogleRequiredMediaUnderstandingProvider(): Promise<GoogleMediaUnderstandingProvider> {
	...
	if (
		...
		throw new Error(
			"google media understanding provider missing required handlers",
		);
	}
...
function createLazyGoogleGeminiCliProvider(): ProviderPlugin {
	return {
		...
		formatApiKey: (cred) =>
			formatGoogleOauthApiKey(cred as GoogleOauthApiKeyCredential),
		...
		fetchUsageSnapshot: async (ctx: ProviderFetchUsageSnapshotContext) => {
			...
			if (!provider.fetchUsageSnapshot) {
				throw new Error(
					"google gemini cli provider missing usage snapshot handler",
				);
			}
...
function createLazyGoogleMediaUnderstandingProvider(): MediaUnderstandingProvider {
	return {
		...
		describeImage: async (...args) =>
			await (
				await loadGoogleRequiredMediaUnderstandingProvider()
			).describeImage(...args),
...
export default definePluginEntry({
	...
	register(api) {
		...
		api.registerMediaUnderstandingProvider(
			createLazyGoogleMediaUnderstandingProvider(),
		);
