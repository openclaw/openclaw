...
export const DEFAULT_GOOGLE_API_BASE_URL =
...
export function normalizeGoogleApiBaseUrl(baseUrl?: string): string {
	const raw = trimTrailingSlashes(
		baseUrl?.trim() || DEFAULT_GOOGLE_API_BASE_URL,
	);
	try {
...
export function normalizeGoogleGenerativeAiBaseUrl(
	baseUrl?: string,
): string | undefined {
	return baseUrl ? normalizeGoogleApiBaseUrl(baseUrl) : baseUrl;
}

export function resolveGoogleGenerativeAiTransport<
	TApi extends string | null | undefined,
>(params: { api: TApi; baseUrl?: string }): { api: TApi; baseUrl?: string } {
	return {
		api: params.api,
...
export function shouldNormalizeGoogleGenerativeAiProviderConfig(
	...
	return (
		provider.models?.some((model) => isGoogleGenerativeAiApi(model?.api)) ??
		false
	);
}
...
export function normalizeGoogleProviderConfig(
	...
	if (
		...
		const modelNormalized = normalizeProviderModels(
			nextProvider,
			normalizeGoogleModelId,
		);
		const normalizedBaseUrl = normalizeGoogleGenerativeAiBaseUrl(
	...
	if (providerKey === "google-antigravity") {
		nextProvider = normalizeProviderModels(
			nextProvider,
			normalizeAntigravityModelId,
		);
	}
