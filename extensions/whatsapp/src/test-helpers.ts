...
function resolveStorePathFallback(store?: string, opts?: { agentId?: string }) {
	...
	return path.resolve(
		store.replaceAll("{agentId}", opts?.agentId?.trim() || "main"),
	);
}
	...
	Object.defineProperties(mockModule, {
		...
		updateLastRoute: {
			...
			value: async (params: {
				...
				const raw = await fs
					.readFile(params.storePath, "utf8")
					.catch(() => "{}");
		...
		loadSessionStore: {
			...
			value: (storePath: string) => {
				try {
					return JSON.parse(fsSync.readFileSync(storePath, "utf8")) as Record<
						string,
						unknown
					>;
				} catch {
...
vi.mock("openclaw/plugin-sdk/state-paths", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("openclaw/plugin-sdk/state-paths")>();
	return {
		...actual,
