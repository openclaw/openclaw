...
export function getComparableIdentityValues(
	...
	return [resolved.e164, resolved.jid, resolved.lid].filter(
...
export function identitiesOverlap(
	...
	return getComparableIdentityValues(right).some((value) =>
		leftValues.has(value),
	);
}

export function getSenderIdentity(
	msg: LegacySenderLike,
	authDir?: string,
): WhatsAppIdentity {
...
export function getMentionIdentities(
	...
	return getMentionJids(msg).map((jid) =>
		resolveComparableIdentity({ jid }, authDir),
	);
}

export function getPrimaryIdentityId(
	identity: WhatsAppIdentity | null | undefined,
): string | null {
