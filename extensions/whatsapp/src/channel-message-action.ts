...
// channel-react-action.js exports the react/upload-file dispatcher under the
// same public name; alias it so this wrapper can layer list-reply on top.
import { handleWhatsAppMessageAction as handleWhatsAppReactOrUploadAction } from "./channel-react-action.js";
...
export async function handleWhatsAppMessageAction(params: WhatsAppMessageActionParams) {
  if (params.action === "upload-file" || params.action === "react") {
    return await handleWhatsAppReactOrUploadAction(params);
  }
