...
function makeMsg() {
  return {
    ...
    to: "+20000000000",
    id: "msg-1",
    reply: vi.fn(async () => void 0),
    sendMedia: vi.fn(async () => void 0)
  };
}
...
function mockFirstSendMediaFailure(msg, message) {
  msg.sendMedia.mockRejectedValueOnce(new Error(message));
}
function mockFirstReplyFailure(msg, message) {
  msg.reply.mockRejectedValueOnce(
    new Error(message)
  );
}
function mockSecondReplySuccess(msg) {
  msg.reply.mockResolvedValueOnce(
    void 0
  );
}
const replyLogger = {
...
async function expectReplySuppressed(replyResult) {
  const msg = makeMsg();
...
describe("deliverWebReply", () => {
  ...
  it("falls back to text-only when the first media send fails", async () => {
    ...
    expect(msg.reply).toHaveBeenCalledTimes(1);
    expect(
      String(msg.reply.mock.calls[0]?.[0])
