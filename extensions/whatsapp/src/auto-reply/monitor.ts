...
export async function monitorWebChannel(
  ...
  try {
    while (true) {
      ...
      const isExpectedWatchdogReconnect =
      ...
      statusController.noteClose({
        ...
        reconnectAttempts: decision.reconnectAttempts,
        healthState: decision.healthState,
        expected: isExpectedWatchdogReconnect,
      });
      reconnectLogger.info(
        {
          connectionId: connection.connectionId,
      ...
      const reconnectMessage = isExpectedWatchdogReconnect
        ? `WhatsApp Web watchdog restarted a quiet connection (status ${decision.normalized.statusLabel}). Retry ${decision.reconnectAttempts}/${reconnectPolicy.maxAttempts || "∞"} in ${formatDurationPrecise(decision.delayMs ?? 0)}.`
      ...
      if (isExpectedWatchdogReconnect) {
