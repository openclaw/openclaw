		...
		outbound: {
			...createWhatsAppOutboundBase({
				...
				sendMessageWhatsApp: async (...args) =>
					await getWhatsAppRuntime().channel.whatsapp.sendMessageWhatsApp(
						...args,
					),
		...
		base: {
			...
			messaging: {
				...
				resolveOutboundSessionRoute: (params) =>
					resolveWhatsAppOutboundSessionRoute(params),
				parseExplicitTarget: ({ raw }) => parseWhatsAppExplicitTarget(raw),
				inferTargetChatType: ({ to }) =>
					parseWhatsAppExplicitTarget(to)?.chatType,
				targetResolver: {
			...
			directory: {
				...
				listPeers: async (params) =>
					listWhatsAppDirectoryPeersFromConfig(params),
				listGroups: async (params) =>
					listWhatsAppDirectoryGroupsFromConfig(params),
			},
			actions: {
				...
				handleAction: async ({
					...
					if (action !== "react") {
						throw new Error(
							`Action ${action} is not supported for provider ${WHATSAPP_CHANNEL}.`,
						);
					}
					...
					const isWhatsAppSource =
						toolContext?.currentChannelProvider === WHATSAPP_CHANNEL;
					const explicitTarget =
						readStringParam(params, "chatJid") ?? readStringParam(params, "to");
					const normalizedTarget = explicitTarget
						? normalizeWhatsAppTarget(explicitTarget)
						: null;
					...
					const scopedContext =
						!isWhatsAppSource || isCrossChat ? undefined : toolContext;
			...
			heartbeat: {
				checkReady: async ({ cfg, accountId, deps }) => {
					...
					const listenerActive = deps?.hasActiveWebListener
						...
						: Boolean(
								(await loadWhatsAppChannelRuntime()).getActiveWebListener(),
							);
					if (!listenerActive) {
				...
				buildChannelSummary: async ({ account, snapshot }) => {
					...
					const linked =
						typeof snapshot.linked === "boolean"
							? snapshot.linked
							: authDir
								? await (await loadWhatsAppChannelRuntime()).webAuthExists(
										authDir,
									)
								: false;
					const authAgeMs =
					...
					return {
						...
						lastInboundAt:
							snapshot.lastInboundAt ?? snapshot.lastMessageAt ?? null,
				...
				resolveAccountSnapshot: async ({ account, runtime }) => {
					...
					return {
						...
						extra: {
							...
							lastInboundAt:
								runtime?.lastInboundAt ?? runtime?.lastMessageAt ?? null,
