...
vi.mock("../../../src/media/image-ops.js", async () => {
  ...
  return {
    ...actual,
    convertHeicToJpeg: (...args) => convertHeicToJpegMock(...args)
  };
});
let fixtureRoot = "";
...
function cloneStatWithDev(stat, dev) {
...
beforeAll(async () => {
  ...
  fallbackPngBuffer = await sharp(raw, { raw: { width: size, height: size, channels: 4 } }).png().toBuffer();
...
describe("web media loading", () => {
  ...
  it("normalizes HEIC local files to JPEG output", async () => {
    ...
    expect(result.buffer[1]).toBe(216);
  });
...
describe("local media root guard", () => {
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    const lstatSpy = vi.spyOn(fs, "lstat").mockResolvedValue(cloneStatWithDev(actualLstat, zeroDev));
