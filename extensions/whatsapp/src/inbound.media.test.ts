...
vi.mock("openclaw/plugin-sdk/config-runtime", async () => {
	const actual = await vi.importActual<
		typeof import("openclaw/plugin-sdk/config-runtime")
...
const HOME = path.join(
	os.tmpdir(),
	`openclaw-inbound-media-${crypto.randomUUID()}`,
);
...
vi.mock("@whiskeysockets/baileys", async (importOriginal) => {
	...
	return {
		...
		extractMessageContent: vi.fn((message: MockMessageInput) =>
			mockExtractMessageContent(message),
		),
		getContentType: vi.fn((message: MockMessageInput) =>
			mockGetContentType(message),
		),
