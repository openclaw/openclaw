...
const hoisted = vi.hoisted(() => ({
	...
	handleWhatsAppAction: vi.fn(async () => ({
...
vi.mock("openclaw/plugin-sdk/setup", async () => {
	const actual = await vi.importActual<
		typeof import("openclaw/plugin-sdk/setup")
...
describe("whatsappPlugin outbound sendPoll", () => {
	...
	it("threads cfg into runtime sendPollWhatsApp call", async () => {
		...
		expectWhatsAppPollSent(hoisted.sendPollWhatsApp, {
			cfg,
			poll,
			to,
			accountId,
...
describe("whatsapp setup wizard", () => {
	...
	beforeEach(() => {
		...
		hoisted.resolveWhatsAppAuthDir.mockReturnValue({
			authDir: "/tmp/openclaw-whatsapp-test",
		});
	});
	...
	it("runs WhatsApp login when not linked and user confirms linking", async () => {
		...
		expect(hoisted.loginWeb).toHaveBeenCalledWith(
			false,
			undefined,
			runtime,
			DEFAULT_ACCOUNT_ID,
...
describe("whatsapp group policy", () => {
	it("uses generic channel group policy helpers", () => {
		...
		expect(
			resolveWhatsAppGroupRequireMention({ cfg, groupId: "1203630@g.us" }),
		).toBe(false);
		expect(
			resolveWhatsAppGroupRequireMention({ cfg, groupId: "other@g.us" }),
		).toBe(true);
		expect(
			resolveWhatsAppGroupToolPolicy({ cfg, groupId: "1203630@g.us" }),
		).toEqual({
			deny: ["exec"],
		});
		expect(
			resolveWhatsAppGroupToolPolicy({ cfg, groupId: "other@g.us" }),
		).toEqual({
			allow: ["message.send"],
		});
	});
...
describe("whatsappPlugin actions.handleAction react messageId resolution", () => {
	...
	it("throws ToolInputError when messageId missing and no toolContext", async () => {
		const err = await whatsappPlugin.actions
			?.handleAction?.({
				channel: "whatsapp",
				action: "react",
				params: { emoji: "👍", to: "+1555" },
