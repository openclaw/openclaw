...
describe("extractMentionedJids", () => {
  ...
  it("deduplicates direct mentions without losing distinct recipients", () => {
    expect(
      extractMentionedJids({
        extendedTextMessage: {
          text: "Hey @bot and @other",
          contextInfo: { mentionedJid: [botJid, botJid, otherJid, botJid] },
        },
      }),
    ).toEqual([botJid, otherJid]);
  });
});
...
describe("web inbound helpers", () => {
  ...
  it.each([
    ...
    ["normalizes tel: prefixes", vcard("Ada Lovelace", ["tel:+15555550123"]), ["+15555550123"]],
    [
      ...
      ["+15555550123", "+15555550124"],
    ],
  ])("%s in WhatsApp vcards", (_name, card, phones) => {
    expect(extractContactContext(message({ contactMessage: { vcard: card } }))).toEqual({
      kind: "contact",
      total: 1,
      contacts: [{ name: "Ada Lovelace", phones }],
    });
  });
