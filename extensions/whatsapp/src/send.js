...
async function sendMessageWhatsApp(to, body, options) {
  ...
  try {
    ...
    if (options.mediaUrl) {
      ...
      if (media.kind === "audio") {
        mediaType = media.contentType === "audio/ogg" ? "audio/ogg; codecs=opus" : media.contentType ?? "application/octet-stream";
    ...
    const accountId = hasExplicitAccountId ? resolvedAccountId : void 0;
