...
const whatsappConfigAdapter =
	createScopedChannelConfigAdapter<ResolvedWhatsAppAccount>({
...
const whatsappResolveDmPolicy =
	createScopedDmSecurityResolver<ResolvedWhatsAppAccount>({
	...
	return createDelegatedSetupWizardProxy({
		...
		resolveShouldPromptAccountIds: (params) =>
			(params.shouldPromptAccountIds ||
				params.options?.promptWhatsAppAccountId) ??
			false,
		credentials: [],
...
export function createWhatsAppPluginBase(params: {
	...
	setupWizard: NonNullable<
		ChannelPlugin<ResolvedWhatsAppAccount>["setupWizard"]
	>;
	setup: NonNullable<ChannelPlugin<ResolvedWhatsAppAccount>["setup"]>;
	isConfigured: NonNullable<
		ChannelPlugin<ResolvedWhatsAppAccount>["config"]
	>["isConfigured"];
}) {
	...
	const base = createChannelPluginBase({
		...
		config: {
			...
			isEnabled: (account, cfg) =>
				account.enabled && cfg.web?.enabled !== false,
