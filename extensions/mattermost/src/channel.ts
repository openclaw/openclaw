...
const mattermostSecurityAdapter =
	createRestrictSendersChannelSecurity<ResolvedMattermostAccount>({
...
>[0]): ChannelMessageToolDiscovery {
	...
	const hasReactionCapableAccount = enabledAccounts.some((account) => {
		const accountActions = account.config.actions as
			| { reactions?: boolean }
			| undefined;
	...
	handleAction: async ({ action, params, cfg, accountId }) => {
		if (action === "react") {
			const resolvedAccountId =
				accountId ?? resolveDefaultMattermostAccountId(cfg);
			const mattermostConfig = cfg.channels?.mattermost as
				| MattermostConfig
				| undefined;
			...
			const reactionsEnabled =
				account.config.actions?.reactions ??
			...
			const { postId, emojiName, remove } =
				parseMattermostReactActionParams(params);
			if (remove) {
		...
		const result = await sendMessageMattermost(to, message, {
			...
			attachmentText:
				typeof params.attachmentText === "string"
...
function parseMattermostReactActionParams(params: Record<string, unknown>): {
	...
	const postId =
		readTrimmedString(params.messageId) ?? readTrimmedString(params.postId);
	if (!postId) {
...
function readMattermostReplyToId(
	params: Record<string, unknown>,
): string | undefined {
	return (
		readTrimmedString(params.replyToId) ?? readTrimmedString(params.replyTo)
	);
}
...
const mattermostConfigAdapter =
	createScopedChannelConfigAdapter<ResolvedMattermostAccount>({
		...
		resolveAllowFrom: (account: ResolvedMattermostAccount) =>
		...
		base: {
			...
			messaging: {
				...
				resolveOutboundSessionRoute: (params) =>
					resolveMattermostOutboundSessionRoute(params),
				targetResolver: {
