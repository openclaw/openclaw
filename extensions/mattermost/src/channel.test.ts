...
type MattermostSendText = NonNullable<
	NonNullable<typeof mattermostPlugin.outbound>["sendText"]
>;
type MattermostSendTextParams = Parameters<MattermostSendText>[0];
type MattermostSendMedia = NonNullable<
	NonNullable<typeof mattermostPlugin.outbound>["sendMedia"]
>;
type MattermostSendMediaParams = Parameters<MattermostSendMedia>[0];

function getDescribedActions(cfg: OpenClawConfig): string[] {
	return [
		...(mattermostPlugin.actions?.describeMessageTool?.({ cfg })?.actions ??
			[]),
	];
}
...
describe("mattermostPlugin", () => {
	beforeEach(async () => {
		...
		({ resetMattermostReactionBotUserCacheForTests } = await import(
			"./mattermost/reactions.js"
		));
	...
	describe("messageActions", () => {
		...
		it("exposes react when mattermost is configured", () => {
			...
			expect(
				mattermostPlugin.actions?.supportsAction?.({ action: "send" }),
			).toBe(true);
		});
		...
		it("keeps buttons optional in message tool schema", () => {
			...
			const discovery = mattermostPlugin.actions?.describeMessageTool?.({
				cfg,
			});
