...
export type MattermostMediaKind =
	| "image"
	| "audio"
	| "video"
	| "document"
...
export function createMattermostMonitorResources(params: {
	...
	mediaKindFromMime: (
		contentType?: string,
	) => MattermostMediaKind | null | undefined;
}) {
	const {
		accountId,
		callbackUrl,
	...
	const channelCache = new Map<
		string,
		{ value: MattermostChannel | null; expiresAt: number }
	>();
	const userCache = new Map<
		string,
		{ value: MattermostUser | null; expiresAt: number }
	>();
	...
	const resolveMattermostMedia = async (
		...
		for (const fileId of ids) {
			try {
				...
				const contentType =
					saved.contentType ?? fetched.contentType ?? undefined;
				out.push({
					path: saved.path,
					contentType,
				...
				logger.debug?.(
					`mattermost: failed to download file ${fileId}: ${String(err)}`,
				);
			}
		}
		return out;
	...
	const resolveChannelInfo = async (
		channelId: string,
	...
	const updateModelPickerPost = async (params: {
		...
		const props = buildModelPickerProps(
			params.channelId,
			params.buttons ?? [],
		) ?? {
			attachments: [],
