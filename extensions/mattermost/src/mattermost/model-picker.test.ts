...
describe("Mattermost model picker", () => {
	it("resolves bare /model and /models entry points", () => {
		expect(resolveMattermostModelPickerEntry("/model")).toEqual({
			kind: "summary",
		});
		expect(resolveMattermostModelPickerEntry("/models")).toEqual({
			kind: "providers",
		});
	...
	it("renders providers and models with Telegram-style navigation", () => {
		...
		const providerTexts = providersView.buttons
			.flat()
