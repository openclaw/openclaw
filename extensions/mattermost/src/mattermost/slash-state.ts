 ...
 */
export function getAllSlashCommandStates(): ReadonlyMap<
	string,
	SlashCommandAccountState
> {
...
export function activateSlashCommands(params: {
	...
	const { account, commandTokens, registeredCommands, triggerMap, api, log } =
		params;
...
export function registerSlashCommandRoute(api: OpenClawPluginApi) {
	const mmConfig = api.config.channels?.mattermost as
		| MattermostConfig
		| undefined;
	...
	const routeHandler = async (req: IncomingMessage, res: ServerResponse) => {
		...
		const match = token
			? resolveSlashHandlerForToken(token)
			: { kind: "none" as const };
	...
	for (const callbackPath of callbackPaths) {
		...
		api.logger.info?.(
			`mattermost: registered slash command callback at ${callbackPath}`,
		);
	}
}
