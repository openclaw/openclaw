...
describe("slash-commands", () => {
	...
	it("resolves command text with trigger map fallback", () => {
		...
		expect(resolveCommandText("oc_status", " now ", triggerMap)).toBe(
			"/status now",
		);
		expect(resolveCommandText("oc_models", " openai ", undefined)).toBe(
			"/models openai",
		);
	...
	it("falls back to localhost callback URL for wildcard bind hosts", () => {
		...
		expect(callbackUrl).toBe(
			"http://localhost:18789/api/channels/mattermost/command",
		);
	});
