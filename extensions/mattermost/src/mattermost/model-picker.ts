...
function buildContext(
	state: MattermostModelPickerState,
...
function getProviderModels(
	...
	return [
		...(data.byProvider.get(normalizeProviderId(provider)) ??
			new Set<string>()),
	].toSorted();
}
...
export function buildMattermostAllowedModelRefs(
	data: ModelsProviderData,
): Set<string> {
...
export function resolveMattermostModelPickerCurrentModel(params: {
	...
	try {
		...
		const provider = (
			override.provider || params.data.resolvedDefault.provider
		).trim();
