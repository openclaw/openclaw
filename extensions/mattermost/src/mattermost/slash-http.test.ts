...
describe("slash-http", () => {
  it("rejects non-POST methods", async () => {
    const handler = createSlashCommandHttpHandler({
      ...
      commandTokenTriggers: new Map([["valid-token", "oc_status"]]),
    });
  ...
  it("fails closed when no command tokens are registered", async () => {
    const response = await runSlashRequest({
      ...
      commandTokenTriggers: new Map<string, string>(),
  ...
  it("rejects a valid token when no registered trigger matches it", async () => {
    const response = await runSlashRequest({
      commandTokens: new Set(["known-token"]),
      commandTokenTriggers: new Map(),
      body: "token=known-token&team_id=t1&channel_id=c1&user_id=u1&command=%2Foc_status&text=",
    });
