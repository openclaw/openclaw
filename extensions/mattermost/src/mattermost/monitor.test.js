...
function resolveRequireMentionForTest(params) {
  ...
  const groupConfig = params.groupId ? groups?.[params.groupId] : void 0;
  const defaultGroupConfig = groups?.["*"];
  const configMention = typeof groupConfig?.requireMention === "boolean" ? groupConfig.requireMention : typeof defaultGroupConfig?.requireMention === "boolean" ? defaultGroupConfig.requireMention : void 0;
