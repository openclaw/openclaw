...
export function normalizeMattermostAllowList(
...
function mapMattermostChannelKind(
...
export function authorizeMattermostCommandInvocation(params: {
	...
	const configAllowFrom = normalizeMattermostAllowList(
		account.config.allowFrom ?? [],
	);
	const configGroupAllowFrom = normalizeMattermostAllowList(
		account.config.groupAllowFrom ?? [],
	);
	const normalizedStoreAllowFrom = normalizeMattermostAllowList(
		storeAllowFrom ?? [],
	);
	...
	const commandDmAllowFrom =
		kind === "direct" ? effectiveAllowFrom : configAllowFrom;
	...
	if (kind === "direct") {
		...
		if (
			!senderGroupAccess.allowed &&
			senderGroupAccess.reason === "empty_allowlist"
		) {
			return {
				ok: false,
