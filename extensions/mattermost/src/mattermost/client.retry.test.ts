...
describe("createMattermostDirectChannelWithRetry", () => {
	...
	function createFetchFailedError(params: {
		message: string;
		code?: string;
	}): TypeError {
	...
	it("succeeds on first attempt without retries", async () => {
		...
		const result = await createMattermostDirectChannelWithRetry(
			client,
			["user-1", "user-2"],
			{
				onRetry,
	...
	it("retries on 5xx even if error message contains 4xx substring", async () => {
		...
		mockFetch
			.mockRejectedValueOnce(
				new Error("Mattermost API 503: upstream returned 404 Not Found"),
			)
			.mockResolvedValueOnce({
				ok: true,
				status: 201,
