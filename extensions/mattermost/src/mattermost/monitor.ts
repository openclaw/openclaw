...
export function resolveMattermostEffectiveReplyToId(params: {
	...
	return params.replyToMode === "all" || params.replyToMode === "first"
		? postId
		: undefined;
}
...
function buildMattermostAttachmentPlaceholder(
	...
	if (mediaList.length === 1) {
		const kind =
			mediaList[0].kind === "unknown" ? "document" : mediaList[0].kind;
		return `<media:${kind}>`;
	}
...
export async function monitorMattermostProvider(
	...
	await runWithReconnect(
		...
		},
		{
			...
			onReconnect: (delayMs) => {
				runtime.log?.(
					`mattermost: API not accessible, retrying in ${Math.round(delayMs / 1000)}s`,
				);
			},
		},
	);
	...
	runtime.log?.(
		`mattermost connected as ${botUsername ? `@${botUsername}` : botUserId}`,
	);
	...
	try {
		...
		if (
			!isLoopbackHost(callbackHost) &&
			allowedInteractionSourceIps.length === 0
		) {
			runtime.error?.(
		...
		handler: createMattermostInteractionHandler({
			...
			dispatchButtonClick: async (opts) => {
				...
				const channelDisplay =
					channelInfo?.display_name ?? channelName ?? opts.channelId;
				const route = core.channel.routing.resolveAgentRoute({
					cfg,
					channel: "mattermost",
					accountId: account.accountId,
					...
					createChannelReplyPipeline({
						...
						typing: {
							start: () =>
								sendTypingIndicator(
									opts.channelId,
									threadContext.effectiveReplyToId,
								),
							onStartError: (err) => {
								logTypingFailure({
				...
				const { dispatcher, replyOptions, markDispatchIdle } =
					core.channel.reply.createReplyDispatcherWithTyping({
						...
						onError: (err, info) => {
							runtime.error?.(
								`mattermost button-click ${info.kind} reply failed: ${String(err)}`,
							);
						},
	...
	const {
		...
		mediaKindFromMime: (contentType) =>
			core.media.mediaKindFromMime(contentType) as MediaKind,
	});
	...
	const runModelPickerCommand = async (params: {
		...
		const ctxPayload = core.channel.reply.finalizeInboundContext({
			...
			GroupSubject:
				params.kind !== "direct"
					? params.channelDisplay || params.roomLabel
					: undefined,
		...
		const { dispatcher, replyOptions, markDispatchIdle } =
			core.channel.reply.createReplyDispatcherWithTyping({
				...
				onError: (err, info) => {
					runtime.error?.(
						`mattermost model picker ${info.kind} reply failed: ${String(err)}`,
					);
				},
				onReplyStart: typingCallbacks?.onReplyStart,
			});
	...
	async function handleModelPickerInteraction(params: {
		...
		const teamId =
			auth.channelInfo.team_id ?? params.payload.team_id ?? undefined;
		const channelName = auth.channelName || undefined;
		const channelDisplay =
			auth.channelDisplay || auth.channelName || params.payload.channel_id;
		...
		void (async () => {
			try {
				...
				runtime.error?.(
					`mattermost model picker select failed: ${String(err)}`,
				);
			}
		})();
	...
	const handlePost = async (
		...
		const allMessageIds = messageIds?.length
			? messageIds
			: post.id
				? [post.id]
				: [];
		...
		const channelInfo = await resolveChannelInfo(channelId);
		const channelType =
			payload.data?.channel_type ?? channelInfo?.type ?? undefined;
		...
		const dmPolicy = account.config.dmPolicy ?? "pairing";
		const normalizedAllowFrom = normalizeMattermostAllowList(
		...
		const commandDmAllowFrom =
			kind === "direct" ? effectiveAllowFrom : normalizedAllowFrom;
		...
		if (accessDecision.decision !== "allow") {
			if (kind === "direct") {
				if (
					...
					DM_GROUP_ACCESS_REASON.DM_POLICY_DISABLED
				) {
					logVerboseMessage(
						`mattermost: drop dm (dmPolicy=disabled sender=${senderId})`,
					);
					return;
				}
			...
			if (
				...
				logVerboseMessage(
					"mattermost: drop group message (groupPolicy=disabled)",
				);
				return;
			}
			if (
				...
				logVerboseMessage(
					"mattermost: drop group message (no group allowlist)",
				);
				return;
			}
			if (
		...
		const channelDisplay =
			payload.data?.channel_display_name ??
		...
		const oncharPrefixes = oncharEnabled
			? resolveOncharPrefixes(account.oncharPrefixes)
			: [];
		...
		const baseText = [bodySource, mediaPlaceholder]
			.filter(Boolean)
			.join("\n")
			.trim();
		...
		const body = core.channel.reply.formatInboundEnvelope({
			...
			from: fromLabel,
			timestamp:
				typeof post.create_at === "number" ? post.create_at : undefined,
			body: textWithId,
			chatType,
			...
			combinedBody = buildPendingHistoryContextFromMap({
				...
				formatEntry: (entry) =>
					core.channel.reply.formatInboundEnvelope({
						...
						body: `${entry.body}${
							entry.messageId
								? ` [id:${entry.messageId} channel:${channelId}]`
								: ""
						}`,
						chatType,
		...
		const ctxPayload = core.channel.reply.finalizeInboundContext({
			...
			Timestamp:
				typeof post.create_at === "number" ? post.create_at : undefined,
			WasMentioned:
				kind !== "direct" ? mentionDecision.effectiveWasMentioned : undefined,
		...
		const { dispatcher, replyOptions, markDispatchIdle } =
			core.channel.reply.createReplyDispatcherWithTyping({
				...
				onError: (err, info) => {
					runtime.error?.(
						`mattermost ${info.kind} reply failed: ${String(err)}`,
					);
				},
			});
	...
	const handleReactionEvent = async (payload: MattermostEventPayload) => {
		...
		const reactionAccess = resolveDmGroupAccessWithLists({
			...
			groupAllowFrom: normalizeMattermostAllowList(
				account.config.groupAllowFrom ?? [],
			),
			storeAllowFrom,
			isSenderAllowed: (allowFrom) =>
	...
	if (slashEnabled) {
		const runAbortCleanup = () => {
			...
			const commands =
				getSlashCommandState(account.accountId)?.registeredCommands ?? [];
		...
		if (opts.abortSignal?.aborted) {
			...
			opts.abortSignal?.addEventListener("abort", runAbortCleanup, {
				once: true,
			});
		}
	}
	...
	try {
		await runWithReconnect(connectOnce, {
			...
			onReconnect: (delayMs) => {
				runtime.log?.(
					`mattermost reconnecting in ${Math.round(delayMs / 1000)}s`,
				);
			},
		});
	} finally {
