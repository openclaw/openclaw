...
const normalizeMattermostBaseUrl = vi.fn((value: string | undefined) =>
	value?.trim(),
);
...
describe("mattermost target resolution", () => {
	...
	afterEach(async () => {
		const { resetMattermostOpaqueTargetCacheForTests } = await import(
	...
	it("recognizes explicit targets and ID-shaped values", async () => {
		...
		expect(
			parseMattermostApiStatus(new Error("Mattermost API 404 Not Found")),
		).toBe(404);
	...
	it("resolves opaque ids as users and caches the result", async () => {
		...
		const { resolveMattermostOpaqueTarget } = await import(
			"./target-resolution.js"
		);
	...
	it("falls back to channel targets on 404 lookups", async () => {
		createMattermostClient.mockReturnValue({ client: true });
		fetchMattermostUser.mockRejectedValue(
			new Error("Mattermost API 404 Not Found"),
		);
