...
export async function registerSlashCommands(params: {
	...
	if (!normalizedCreatorUserId) {
		throw new Error(
			"creatorUserId is required for slash command reconciliation",
		);
	}
	...
	for (const spec of commands) {
		...
		if (existingCmd && existingCmd.url === callbackUrl) {
			log?.(
				`mattermost: command /${spec.trigger} already registered (id=${existingCmd.id})`,
			);
			registered.push({
				id: existingCmd.id,
		...
		if (existingCmd && existingCmd.url !== callbackUrl) {
			...
			try {
				...
				try {
					...
					log?.(
						`mattermost: deleted stale command /${spec.trigger} (id=${existingCmd.id})`,
					);
				} catch (deleteErr) {
					log?.(
		...
		try {
			...
			log?.(
				`mattermost: registered command /${spec.trigger} (id=${created.id})`,
			);
			registered.push({
				id: created.id,
				trigger: spec.trigger,
			...
			log?.(
				`mattermost: failed to register command /${spec.trigger}: ${String(err)}`,
			);
		}
	}
...
export async function cleanupSlashCommands(params: {
	...
	for (const cmd of commands) {
		...
		try {
			...
			log?.(
				`mattermost: failed to delete command /${cmd.trigger}: ${String(err)}`,
			);
		}
	}
}
...
export function parseSlashCommandPayload(
	...
	try {
		if (contentType?.includes("application/json")) {
			...
			const channelId =
				typeof parsed.channel_id === "string" ? parsed.channel_id : "";
			...
			return {
				...
				channel_name:
					typeof parsed.channel_name === "string"
						? parsed.channel_name
						: undefined,
				user_id: userId,
				user_name:
					typeof parsed.user_name === "string" ? parsed.user_name : undefined,
				command,
				text: typeof parsed.text === "string" ? parsed.text : "",
				trigger_id:
					typeof parsed.trigger_id === "string" ? parsed.trigger_id : undefined,
				response_url:
					typeof parsed.response_url === "string"
						? parsed.response_url
						: undefined,
			};
		}
...
export function resolveCommandText(
	...
	const commandName =
		triggerMap?.get(trigger) ??
		(trigger.startsWith("oc_") ? trigger.slice(3) : trigger);
...
export function resolveSlashCommandConfig(
	...
	return {
		...
		callbackPath: normalizeCallbackPath(
			raw?.callbackPath ?? DEFAULT_CALLBACK_PATH,
		),
		callbackUrl: raw?.callbackUrl?.trim() || undefined,
	};
}

export function isSlashCommandsEnabled(
