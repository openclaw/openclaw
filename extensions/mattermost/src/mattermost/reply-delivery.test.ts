...
type DeliverMattermostReplyPayloadParams = Parameters<
	typeof deliverMattermostReplyPayload
>[0];
	...
	return createPluginRuntimeMock({
		channel: {
			text: {
				...
				resolveMarkdownTableMode: vi.fn<() => ReplyDeliveryMarkdownTableMode>(
					() => "off",
				),
...
describe("deliverMattermostReplyPayload", () => {
	it("passes agent-scoped mediaLocalRoots when sending media paths", async () => {
		...
		const stateDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "openclaw-mm-state-"),
		);
