...
describe("createMattermostInteractionHandler", () => {
  ...
  function createReq(params) {
    const body = params.body === void 0 ? "" : JSON.stringify(params.body);
  ...
  async function runHandler(handler, params) {
    const req = createReq({
  ...
  function expectForbiddenResponse(res, expectedMessage) {
    expect(res.statusCode).toBe(403);
    expect(res.body).toContain(expectedMessage);
  }
  function expectSuccessfulApprovalUpdate(res, requestLog) {
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe("{}");
    if (requestLog) {
      expect(requestLog).toEqual([
        { path: "/posts/post-1", method: void 0 },
  ...
  async function runInvalidActionRequest(actionId) {
    const { context, token } = createActionContext();
    const handler = createMattermostInteractionHandler({
      client: {
  ...
  it("forwards fetched post threading metadata to session and button callbacks", async () => {
    ...
    const handler = createMattermostInteractionHandler({
      client: {
        request: async (_path, init) => init?.method === "PUT" ? { id: "post-1" } : fetchedPost
      },
      botUserId: "bot",
      accountId: "acct",
      resolveSessionKey,
  ...
  it("lets a custom interaction handler short-circuit generic completion updates", async () => {
    ...
    expect(requestLog).toEqual([{ path: "/posts/post-1", method: void 0 }]);
