...
function recordMattermostOutboundActivity(accountId: string): void {
	try {
		...
		if (
			...
			error.message !== "Mattermost runtime not initialized"
		) {
			throw error;
		}
	}
...
async function resolveChannelIdByName(params: {
	...
	throw new Error(
		`Mattermost channel "#${name}" not found in any team the bot belongs to`,
	);
}
...
async function resolveTargetChannelId(
	params: ResolveTargetChannelIdParams,
): Promise<string> {
...
async function resolveMattermostSendContext(
	...
	const accountRetryConfig: CreateDmChannelRetryOptions | undefined = account
		.config.dmChannelRetry
		? {
