...
function buildSlashCommands(params: {
	...
	const commandsToRegister: MattermostCommandSpec[] = [
		...DEFAULT_COMMAND_SPECS,
	];
	...
	try {
		...
		params.runtime.error?.(
			`mattermost: failed to list skill commands: ${String(err)}`,
		);
	}
	return commandsToRegister;
}
...
function warnOnSuspiciousCallbackUrl(params: {
	...
	try {
		const mmHost = new URL(
			normalizeMattermostBaseUrl(params.baseUrl) ?? params.baseUrl,
		).hostname;
...
export async function registerMattermostMonitorSlashCommands(params: {
	...
	try {
		const teams = await fetchMattermostUserTeams(
			params.client,
			params.botUserId,
		);
		...
		const { registered, teamRegistrationFailures } =
		...
		params.runtime.error?.(
			`mattermost: failed to register slash commands: ${String(err)}`,
		);
	}
}
