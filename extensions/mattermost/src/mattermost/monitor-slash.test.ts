...
const normalizeMattermostBaseUrl = vi.hoisted(() =>
	vi.fn((value: string | undefined) => value),
);
...
describe("mattermost monitor slash", () => {
	...
	it("returns early when slash commands are disabled", async () => {
		...
		const { registerMattermostMonitorSlashCommands } = await import(
			"./monitor-slash.js"
		);
	...
	it("registers deduped default and native skill commands across teams", async () => {
		...
		fetchMattermostUserTeams.mockResolvedValue([
			{ id: "team-1" },
			{ id: "team-2" },
		]);
