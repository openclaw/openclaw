...
describe("resolveInteractionCallbackUrl", () => {
	...
	it("prefers cached URL from registry", () => {
		...
		expect(resolveInteractionCallbackUrl("cached")).toBe(
			"http://cached:1234/path",
		);
	});
	...
	it("recomputes from config when bypassing the cache explicitly", () => {
		...
		expect(url).toBe(
			"http://gateway.internal:9999/mattermost/interactions/acct",
		);
	});
	...
	it("uses interactions.callbackBaseUrl when configured", () => {
		...
		expect(url).toBe(
			"https://gateway.example.com/openclaw/mattermost/interactions/default",
		);
	});
	...
	it("trims trailing slashes from callbackBaseUrl", () => {
		...
		expect(url).toBe(
			"https://gateway.example.com/root/mattermost/interactions/acct",
		);
	});
	...
	it("falls back to gateway.customBindHost when configured", () => {
		...
		expect(url).toBe(
			"http://gateway.internal:9999/mattermost/interactions/default",
		);
	});
	...
	it("uses default port 18789 when no config provided", () => {
		...
		expect(url).toBe(
			"http://localhost:18789/mattermost/interactions/myaccount",
		);
	});
});
...
describe("resolveInteractionCallbackPath", () => {
	it("builds the per-account callback path", () => {
		expect(resolveInteractionCallbackPath("acct")).toBe(
			"/mattermost/interactions/acct",
		);
	});
});
...
describe("buildButtonAttachments", () => {
	...
	it("merges custom context into integration context", () => {
		const result = buildButtonAttachments({
			callbackUrl: "http://localhost:18789/cb",
			buttons: [
				{ id: "btn", name: "Go", context: { tweet_id: "123", batch: true } },
			],
		});
			...
			buttons: [
				{
					id: "do_action",
					name: "Do",
					context: { tweet_id: "42", category: "ai" },
				},
			],
		});
...
describe("createMattermostInteractionHandler", () => {
	...
	function createMattermostClientMock(
		...
		return {
			...
			request: async <T>(path: string, init?: RequestInit) =>
				(await requestImpl(path, init)) as T,
	...
	function createRes(): ServerResponse & {
		...
		const res = {
			...
			setHeader(name: string, value: string | number | readonly string[]) {
				res.headers[name] = Array.isArray(value)
					? value.join(",")
					: String(value);
				return res;
			},
	...
	it("rejects requests when the fetched post does not belong to the callback channel", async () => {
		...
		const handler = createMattermostInteractionHandler({
			client: createMattermostClientMock(async () =>
				createActionPost({ channelId: "chan-9" }),
			),
			botUserId: "bot",
			accountId: "acct",
		});
	...
	it("forwards fetched post threading metadata to session and button callbacks", async () => {
		...
		const resolveSessionKey = vi
			.fn()
			.mockResolvedValue("session:thread:root-9");
		const dispatchButtonClick = vi.fn();
