...
export function setInteractionCallbackUrl(
	accountId: string,
	url: string,
): void {
...
export function getInteractionCallbackUrl(
	accountId: string,
): string | undefined {
...
export function computeInteractionCallbackUrl(
	...
	let host =
		...
		!isWildcardBindHost(cfg.gateway.customBindHost)
...
export function setInteractionSecret(
	accountIdOrBotToken: string,
	botToken?: string,
): void {
	if (typeof botToken === "string") {
		interactionSecrets.set(
			accountIdOrBotToken,
			deriveInteractionSecret(botToken),
		);
		return;
	}
...
function canonicalizeInteractionContext(value: unknown): unknown {
	...
	if (value && typeof value === "object") {
		const entries = Object.entries(value as Record<string, unknown>)
			...
			.map(([key, entryValue]) => [
				key,
				canonicalizeInteractionContext(entryValue),
			]);
...
export function createMattermostInteractionHandler(params: {
	...
	return async (req: IncomingMessage, res: ServerResponse) => {
		...
		try {
			originalPost = await client.request<MattermostPost>(
			...
			if (clickedButtonName === null) {
				log?.(
					`mattermost interaction: action ${actionId} not found in post ${payload.post_id}`,
				);
				res.statusCode = 403;
				res.setHeader("Content-Type", "application/json");
			...
			log?.(
				`mattermost interaction: failed to validate post ${payload.post_id}: ${String(err)}`,
			);
			res.statusCode = 500;
			res.setHeader("Content-Type", "application/json");
		...
		try {
			...
			log?.(
				`mattermost interaction: system event dispatch failed: ${String(err)}`,
			);
		}
		...
		try {
			...
			log?.(
				`mattermost interaction: failed to update post ${payload.post_id}: ${String(err)}`,
			);
		}
		...
		if (params.dispatchButtonClick) {
			try {
				...
				log?.(
					`mattermost interaction: dispatchButtonClick failed: ${String(err)}`,
				);
			}
		}
	};
