...
describe("mattermost reactions", () => {
  ...
  async function addReactionWithFetch(fetchMock) {
  ...
  async function removeReactionWithFetch(fetchMock) {
  ...
  it("caches the bot user id across reaction mutations", async () => {
    ...
    const usersMeCalls = fetchMock.mock.calls.filter(
      (call) => String(call[0]).endsWith("/api/v4/users/me")
    );
