...
export const mattermostSetupWizard: ChannelSetupWizard = {
	...
	status: createStandardChannelSetupStatus({
		...
		resolveConfigured: ({ cfg }) =>
			listMattermostAccountIds(cfg).some((accountId) =>
				isMattermostConfigured(
					resolveMattermostAccountWithSecrets(cfg, accountId),
				),
			),
	}),
	introNote: {
	...
	envShortcut: {
		...
		isAvailable: ({ cfg, accountId }) => {
			...
			const resolvedAccount = resolveMattermostAccountWithSecrets(
				cfg,
				accountId,
			);
	...
	credentials: [
		{
			...
			inspect: ({ cfg, accountId }) => {
				...
				return {
					...
					hasConfiguredValue: hasConfiguredSecretInput(
	...
	textInputs: [
		{
			...
			shouldPrompt: ({ cfg, accountId, credentialValues, currentValue }) => {
				...
				const tokenConfigured =
					...
					hasConfiguredSecretInput(resolvedAccount.config.botToken);
				return (
					Boolean(credentialValues.botToken) ||
					!tokenConfigured ||
					!currentValue
				);
			},
			...
			normalizeValue: ({ value }) =>
				normalizeMattermostBaseUrl(value) ?? value.trim(),
		},
	],
