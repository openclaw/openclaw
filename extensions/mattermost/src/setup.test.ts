...
const hasConfiguredSecretInput = vi.hoisted(() =>
	vi.fn((value: unknown) => Boolean(value)),
);

vi.mock("./mattermost/accounts.js", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("./mattermost/accounts.js")>();
	return {
		...actual,
		resolveMattermostAccount: (
			...args: Parameters<typeof actual.resolveMattermostAccount>
		) => {
			const mocked = resolveMattermostAccount(...args);
			return mocked === undefined
				? actual.resolveMattermostAccount(...args)
				: mocked;
		},
	};
});
...
vi.mock("./mattermost/client.js", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("./mattermost/client.js")>();
	return {
		...actual,
...
describe("mattermost setup", () => {
	...
	afterEach(() => {
		...
		normalizeMattermostBaseUrl.mockImplementation(
			(value: string | undefined) => value,
		);
		hasConfiguredSecretInput.mockReset();
		hasConfiguredSecretInput.mockImplementation((value: unknown) =>
			Boolean(value),
		);
		vi.unstubAllEnvs();
	});
	...
	it("resolves accounts with unresolved secret refs allowed", async () => {
		...
		const { resolveMattermostAccountWithSecrets } = await import(
			"./setup-core.js"
		);
		const cfg = { channels: { mattermost: {} } };

		expect(
			resolveMattermostAccountWithSecrets(cfg as never, "default"),
		).toEqual({
			accountId: "default",
		});
	...
	])("treats secret-ref tokens plus base url as configured: %s", async (baseUrl) => {
