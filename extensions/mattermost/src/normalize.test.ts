...
describe("normalizeMattermostMessagingTarget", () => {
	...
	it("normalizes channel: prefix", () => {
		expect(normalizeMattermostMessagingTarget("channel:abc123")).toBe(
			"channel:abc123",
		);
		expect(normalizeMattermostMessagingTarget("Channel:ABC")).toBe(
			"channel:ABC",
		);
	});

	it("normalizes group: prefix to channel:", () => {
		expect(normalizeMattermostMessagingTarget("group:abc123")).toBe(
			"channel:abc123",
		);
	});

	it("normalizes user: prefix", () => {
		expect(normalizeMattermostMessagingTarget("user:abc123")).toBe(
			"user:abc123",
		);
	});

	it("normalizes mattermost: prefix to user:", () => {
		expect(normalizeMattermostMessagingTarget("mattermost:abc123")).toBe(
			"user:abc123",
		);
	});
...
describe("looksLikeMattermostTargetId", () => {
	...
	it("recognizes 26-char alphanumeric Mattermost IDs", () => {
		expect(looksLikeMattermostTargetId("abcdefghijklmnopqrstuvwxyz")).toBe(
			true,
		);
		expect(looksLikeMattermostTargetId("12345678901234567890123456")).toBe(
			true,
		);
		expect(looksLikeMattermostTargetId("AbCdEf1234567890abcdef1234")).toBe(
			true,
	...
	it("recognizes DM channel format (26__26)", () => {
		expect(
			looksLikeMattermostTargetId(
				"abcdefghijklmnopqrstuvwxyz__12345678901234567890123456",
			), // pragma: allowlist secret
		).toBe(true);
	});
	...
	it("rejects strings longer than 26 chars that are not DM format", () => {
		expect(looksLikeMattermostTargetId("abcdefghijklmnopqrstuvwxyz1")).toBe(
			false,
		); // pragma: allowlist secret
	});
});
