...
const NextcloudTalkWebhookPayloadSchema: z.ZodType<NextcloudTalkWebhookPayload> =
	z.object({
...
function verifyWebhookSignature(params: {
	...
	if (!isValid) {
		params.authRateLimiter.recordFailure(
			params.clientIp,
			WEBHOOK_AUTH_RATE_LIMIT_SCOPE,
		);
...
export function createNextcloudTalkWebhookServer(
	...
	const server = createServer(
		async (req: IncomingMessage, res: ServerResponse) => {
			...
			if (
				!webhookAuthRateLimiter.check(clientIp, WEBHOOK_AUTH_RATE_LIMIT_SCOPE)
					.allowed
			) {
				res.writeHead(429);
