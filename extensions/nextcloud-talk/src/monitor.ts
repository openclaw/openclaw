...
function decodeWebhookCreateMessage(params: {
  ...
  | { kind: "ignore" }
  | { kind: "invalid" } {
  // Step 1: Try JSON.parse — malformed JSON is always a client error (400).
  let parsed: unknown;
  try {
  ...
  // Step 2: Check for valid non-message events (file shares, calls, system notifications).
  // Nextcloud Talk sends Activity Streams 2.0 events for all room activity. Events whose
  // object.type is not "Note" (e.g. "Document" for file shares) are not chat messages and
  // should be silently acknowledged (200) rather than rejected as malformed (400).
  if (
    ...
    const obj = (parsed as Record<string, unknown>).object;
    if (typeof obj === "object" && obj !== null && "type" in obj) {
      const objType = (obj as Record<string, unknown>).type;
      if (typeof objType === "string" && objType !== "Note") {
        return { kind: "ignore" };
      }
    }
  }

  // Step 3: Zod schema validation — catches malformed Note-shaped payloads (400).
