...
function resolveNextcloudTalkSendContext(opts: NextcloudTalkSendOpts): {
	...
	const cfg = (opts.cfg ??
		getNextcloudTalkRuntime().config.loadConfig()) as CoreConfig;
...
function recordNextcloudTalkOutboundActivity(accountId: string): void {
	try {
		...
		if (
			...
			error.message !== "Nextcloud Talk runtime not initialized"
		) {
			throw error;
		}
	}
...
export async function sendMessageNextcloudTalk(
	...
	const { cfg, account, baseUrl, secret } =
		resolveNextcloudTalkSendContext(opts);
	...
	const { response, release } = await fetchWithSsrFGuard({
		...
		policy: account.config?.allowPrivateNetwork
