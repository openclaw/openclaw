...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as ActiviConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ActiviConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ActiviConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ActiviConfig);
