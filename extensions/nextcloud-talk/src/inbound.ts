...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as EasyHubConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as EasyHubConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as EasyHubConfig);
