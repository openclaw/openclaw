...
type NextcloudTalkSection = NonNullable<
	CoreConfig["channels"]
>["nextcloud-talk"];

export function normalizeNextcloudTalkBaseUrl(
	value: string | undefined,
): string {
	return value?.trim().replace(/\/+$/, "") ?? "";
}

export function validateNextcloudTalkBaseUrl(
	value: string,
): string | undefined {
	if (!value) {
		return "Required";
	...
	return {
		...
		channels: {
			...
			"nextcloud-talk": {
				...
				accounts: {
					...section.accounts,
					[accountId]: nextAccount as NonNullable<
						typeof section.accounts
					>[string],
				},
			},
		},
...
export const nextcloudTalkDmPolicy: ChannelSetupDmPolicy =
	createTopLevelChannelDmPolicy({
		...
		getCurrent: (cfg) =>
			cfg.channels?.["nextcloud-talk"]?.dmPolicy ?? "pairing",
