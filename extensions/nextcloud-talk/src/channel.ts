...
const resolveNextcloudTalkDmPolicy =
	createScopedDmSecurityResolver<ResolvedNextcloudTalkAccount>({
		...
		base: {
			...
			config: {
				...
				describeAccount: (account) =>
					describeWebhookAccountSnapshot({
						account,
						configured: Boolean(
							account.secret?.trim() && account.baseUrl?.trim(),
						),
						extra: {
			...
			messaging: {
				...
				resolveOutboundSessionRoute: (params) =>
					resolveNextcloudTalkOutboundSessionRoute(params),
				targetResolver: {
			...
			gateway: {
				startAccount: async (ctx) => {
					...
					ctx.log?.info(
						`[${account.accountId}] starting Nextcloud Talk webhook server`,
					);
				...
				logoutAccount: async ({ accountId, cfg }) => {
					...
					if (changed) {
						if (nextSection && Object.keys(nextSection).length > 0) {
							...
							const nextChannels = { ...nextCfg.channels } as Record<
								string,
								unknown
							>;
