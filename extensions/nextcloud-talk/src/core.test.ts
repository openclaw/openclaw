...
describe("nextcloud talk core", () => {
	it("accepts SecretRef botSecret and apiPassword at top-level", () => {
		const result = NextcloudTalkConfigSchema.safeParse({
			...
			apiPassword: {
				source: "env",
				provider: "default",
				id: "NEXTCLOUD_TALK_API_PASSWORD",
			},
		});
	...
	it("escapes markdown-sensitive characters", () => {
		expect(escapeNextcloudTalkMarkdown("*hello* [x](y)")).toBe(
			"\\*hello\\* \\[x\\]\\(y\\)",
		);
	});
	...
	it("formats mentions and code consistently", () => {
		...
		expect(formatNextcloudTalkCodeBlock("const x = 1;", "ts")).toBe(
			"```ts\nconst x = 1;\n```",
		);
	...
	it("strips markdown formatting and truncates on word boundaries", () => {
		...
		expect(truncateNextcloudTalkText("alpha beta gamma delta", 14)).toBe(
			"alpha beta...",
		);
	...
	it("normalizes and recognizes supported room target formats", () => {
		...
		expect(stripNextcloudTalkTargetPrefix("nextcloud-talk:room:AbC123")).toBe(
			"AbC123",
		);
		...
		expect(normalizeNextcloudTalkMessagingTarget("room:AbC123")).toBe(
			"nextcloud-talk:abc123",
		);
		expect(normalizeNextcloudTalkMessagingTarget("nc-talk:room:Ops")).toBe(
			"nextcloud-talk:ops",
		);

		expect(looksLikeNextcloudTalkTargetId("nextcloud-talk:room:abc12345")).toBe(
			true,
		);
	...
	it("normalizes trimmed DM allowlist prefixes to lowercase ids", () => {
		...
		expect(
			nextcloudTalkPlugin.pairing?.normalizeAllowEntry?.(
