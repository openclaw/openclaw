...
describe("nextcloud talk setup", () => {
	beforeEach(async () => {
		...
		({ sendMessageNextcloudTalk, sendReactionNextcloudTalk } = await import(
			"./send.js"
		));
		hoisted.resolveNextcloudTalkAccount.mockImplementation(
			accountsActual.resolveNextcloudTalkAccount,
	...
	it("normalizes and validates base urls", () => {
		expect(
			normalizeNextcloudTalkBaseUrl(" https://cloud.example.com/// "),
		...
		expect(
			validateNextcloudTalkBaseUrl("https://cloud.example.com"),
		).toBeUndefined();
	});
	...
	it("patches scoped account config and clears selected fields", () => {
		...
		expect(
			clearNextcloudTalkAccountFields(cfg, "work", [
				"botSecret",
				"botSecretFile",
			]),
		).toMatchObject({
	...
	it("clears stored bot secret fields when switching the default account to env", () => {
		...
		expect(next.channels?.["nextcloud-talk"]?.baseUrl).toBe(
			"https://cloud.example.com",
		);
		expect(next.channels?.["nextcloud-talk"]).not.toHaveProperty("botSecret");
		expect(next.channels?.["nextcloud-talk"]).not.toHaveProperty(
			"botSecretFile",
		);
	});
...
describe("resolveNextcloudTalkAccount", () => {
	...
	it.runIf(process.platform !== "win32")(
		"rejects symlinked botSecretFile paths",
		() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "openclaw-nextcloud-talk-"),
			);
...
describe("nextcloud-talk send cfg threading", () => {
	...
	beforeEach(async () => {
		...
		hoisted.mockFetchGuard.mockImplementation(
