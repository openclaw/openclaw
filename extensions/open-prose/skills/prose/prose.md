...
## SmartAgentNeo Runtime Mapping

- **Task tool** in the upstream spec == SmartAgentNeo `sessions_spawn`
- **File I/O** == SmartAgentNeo `read`/`write`
- **Remote fetch** == SmartAgentNeo `web_fetch` (or `exec` with curl when POST is required)
