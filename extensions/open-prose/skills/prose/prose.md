...
## Activi Runtime Mapping

- **Task tool** in the upstream spec == Activi `sessions_spawn`
- **File I/O** == Activi `read`/`write`
- **Remote fetch** == Activi `web_fetch` (or `exec` with curl when POST is required)
