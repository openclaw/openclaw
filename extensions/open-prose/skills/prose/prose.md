...
## Mullusi Runtime Mapping

- **Task tool** in the upstream spec == Mullusi `sessions_spawn`
- **File I/O** == Mullusi `read`/`write`
- **Remote fetch** == Mullusi `web_fetch` (or `exec` with curl when POST is required)
