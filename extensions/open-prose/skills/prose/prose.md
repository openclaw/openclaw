...
## EasyHub Runtime Mapping

- **Task tool** in the upstream spec == EasyHub `sessions_spawn`
- **File I/O** == EasyHub `read`/`write`
- **Remote fetch** == EasyHub `web_fetch` (or `exec` with curl when POST is required)
