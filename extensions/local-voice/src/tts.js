...
const DEFAULT_VV_CONFIG = {
    endpoint: "http://localhost:50021",
    speakerId: 2, // Default "四国めたん" (Metan) or similar, can be adjusted
};
export function getTTSConfig(globalConfig) {
    const talkVoiceCfg = globalConfig.plugins?.entries?.["talk-voice"]?.config;
    const localVoiceCfg = globalConfig.plugins?.entries?.["local-voice"]?.config;
    const provider = localVoiceCfg?.ttsProvider ?? talkVoiceCfg?.provider ?? "style-bert-vits2";
    const sbv2Partial = talkVoiceCfg?.styleBertVits2 ?? {};
...
async function synthesizeWithSBV2(text, config) {
...
async function synthesizeWithOpenAI(text, apiKey) {
    if (!apiKey) {
...
async function synthesizeWithVoiceVox(text, config) {
    const base = config.endpoint.replace(/\/$/, "");
    // 1. Audio Query
...
export async function playAudioData(data) {
    const { exec } = require("child_process");
...
function getPlayCommand(source, platform) {
    switch (platform) {
        case "darwin":
