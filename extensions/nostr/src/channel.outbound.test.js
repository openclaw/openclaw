...
const mocks = vi.hoisted(() => ({
  normalizePubkey: vi.fn((value) => `normalized-${value.toLowerCase()}`),
  startNostrBus: vi.fn()
}));
...
describe("nostr outbound cfg threading", () => {
  ...
  it("uses resolved cfg when converting markdown tables before send", async () => {
    ...
    const convertMarkdownTables = vi.fn((text) => `converted:${text}`);
    setNostrRuntime({
      channel: {
        text: {
    ...
    mocks.startNostrBus.mockResolvedValueOnce(bus);
