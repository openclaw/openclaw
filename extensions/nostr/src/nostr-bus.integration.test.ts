...
function createTracker(
	overrides?: Partial<Parameters<typeof createSeenTracker>[0]>,
) {
	...
	describe("createMetrics", () => {
		...
		it("tracks circuit breaker state changes", () => {
			...
			metrics.emit("relay.circuit_breaker.open", 1, {
				relay: TEST_RELAY_URL_PRIMARY,
			});

			let snapshot = metrics.getSnapshot();
			expect(snapshot.relays[TEST_RELAY_URL_PRIMARY].circuitBreakerState).toBe(
				"open",
			);
			expect(snapshot.relays[TEST_RELAY_URL_PRIMARY].circuitBreakerOpens).toBe(
				1,
			);
			...
			expect(snapshot.relays[TEST_RELAY_URL_PRIMARY].circuitBreakerCloses).toBe(
				1,
			);
		});
		...
		it("tracks relay message types", () => {
			...
			metrics.emit("relay.message.closed", 1, {
				relay: TEST_RELAY_URL_PRIMARY,
			});
			metrics.emit("relay.message.notice", 1, {
				relay: TEST_RELAY_URL_PRIMARY,
			});
