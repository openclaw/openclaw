...
const resolveNostrDmPolicy =
	createScopedDmSecurityResolver<ResolvedNostrAccount>({
		channelKey: "nostr",
...
const nostrConfigAdapter =
	createTopLevelChannelConfigAdapter<ResolvedNostrAccount>({
		sectionKey: "nostr",
			...
			messaging: {
				...
				targetResolver: {
					looksLikeId: (input) => {
						...
						return (
							trimmed.startsWith("npub1") || /^[0-9a-fA-F]{64}$/.test(trimmed)
						);
					},
					hint: "<npub|hex pubkey|nostr:npub...>",
				},
				resolveOutboundSessionRoute: (params) =>
					resolveNostrOutboundSessionRoute(params),
			},
			status: {
			...
			gateway: {
				startAccount: async (ctx) => {
					...
					const authorizeSender = createPreCryptoDirectDmAuthorizer({
						resolveAccess: async (senderPubkey) =>
							await resolveInboundAccess(senderPubkey, ""),
						issuePairingChallenge: async ({ senderId, reply }) => {
							await pairing.issueChallenge({
								...
								onCreated: () => {
									ctx.log?.debug?.(
										`[${account.accountId}] nostr pairing request sender=${senderId}`,
									);
								},
								onReplyError: (err) => {
									ctx.log?.warn?.(
						...
						onBlocked: ({ senderId, reason }) => {
							ctx.log?.debug?.(
								`[${account.accountId}] blocked Nostr sender ${senderId} (${reason})`,
							);
						},
					});
