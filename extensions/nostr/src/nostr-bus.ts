...
function createFixedWindowRateLimiter(params: {
	...
	const touch = (
		key: string,
		value: { count: number; windowStartMs: number },
	) => {
		state.delete(key);
		state.set(key, value);
	};
...
export async function startNostrBus(
	...
	async function handleEvent(event: Event): Promise<void> {
		try {
			...
			if (
				event.created_at >
				Math.floor(Date.now() / 1000) + guardPolicy.maxFutureSkewSec
			) {
			...
			if (
				...
				guardPolicy.maxCiphertextBytes
			) {
			...
			if (
				Buffer.byteLength(plaintext, "utf8") > guardPolicy.maxPlaintextBytes
			) {
