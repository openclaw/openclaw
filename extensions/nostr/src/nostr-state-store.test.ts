...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SMART_AGENT_NEO_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-nostr-"));
  process.env.SMART_AGENT_NEO_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".smart-agent-neo");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SMART_AGENT_NEO_STATE_DIR = previous;
    }
