...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ACTIVI_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "activi-nostr-"));
  process.env.ACTIVI_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.ACTIVI_STATE_DIR?.trim() || stateEnv.ACTIVI_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".activi");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ACTIVI_STATE_DIR = previous;
    }
