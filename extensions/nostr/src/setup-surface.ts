...
function parseNostrAllowFrom(raw: string): {
	entries: string[];
	error?: string;
} {
	...
	applyAccountConfig: ({ cfg, input }) => {
		...
		return patchTopLevelChannelConfigSection({
			...
			patch: {
				...(typedInput.useEnv
					? {}
					: { privateKey: typedInput.privateKey?.trim() }),
	...
	textInputs: [
		{
			...
			helpLines: [
				"Use ws:// or wss:// relay URLs.",
				"Leave blank to keep the default relay set.",
			],
			currentValue: ({ cfg, accountId }) => {
				const account = resolveNostrAccount({ cfg, accountId });
				const relays =
					cfg.channels?.nostr?.relays && cfg.channels.nostr.relays.length > 0
						? account.relays
						: [];
				return relays.join(", ");
			},
