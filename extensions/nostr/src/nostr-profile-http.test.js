...
function createMockRequest(method, url, body, opts) {
...
describe("nostr-profile-http", () => {
  ...
  describe("PUT /api/channels/nostr/:accountId/profile", () => {
    ...
    function expectBadRequestResponse(res) {
      expect(res._getStatusCode()).toBe(400);
      const data = JSON.parse(res._getData());
      expect(data.ok).toBe(false);
      return data;
    }
    async function expectPrivatePictureRejected(pictureUrl) {
      const ctx = createMockContext();
      const handler = createNostrProfileHttpHandler(ctx);
    ...
    it("caps tracked rate-limit keys to prevent unbounded growth", () => {
      ...
      expect(getNostrProfileRateLimitStateSizeForTest()).toBeLessThanOrEqual(2048);
    });
