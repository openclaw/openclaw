// Comfy workflow file helpers bound local workflowPath reads when the
// operator configures an explicit workflowFileMaxBytes limit.
  ...
  try {
    ...
    while (true) {
      const scratch = Buffer.allocUnsafe(
        maxBytes === undefined
          ? COMFY_WORKFLOW_FILE_READ_CHUNK_BYTES
