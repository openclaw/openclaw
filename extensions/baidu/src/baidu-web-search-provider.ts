...
function resolveBaiduConfig(
  config?: OpenClawConfig,
  searchConfig?: SearchConfigRecord,
): BaiduConfig {
  const pluginConfig = resolveProviderWebSearchPluginConfig(config, "baidu");
  if (pluginConfig) {
    return pluginConfig as BaiduConfig;
  }
  const baidu = (searchConfig as Record<string, unknown> | undefined)?.baidu;
...
function resolveBaiduApiKey(baidu?: BaiduConfig): string | undefined {
  return (
    readConfiguredSecretString(baidu?.apiKey, "plugins.entries.baidu.config.webSearch.apiKey") ??
...
function createBaiduToolDefinition(
  ...
  return {
    ...
    execute: async (args) => {
      ...
      const baiduConfig = resolveBaiduConfig(config, searchConfig);
...
export function createBaiduWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    createTool: (ctx) =>
      createBaiduToolDefinition(ctx.config, ctx.searchConfig as SearchConfigRecord | undefined),
  };
}
