/**
 * Strategy package validation for Findoo Backtest Agent (fep v1.1).
...
/**
 * Validate a strategy package directory per fep v1.1:
...
export async function validateStrategyPackage(dirPath: string): Promise<ValidateResult> {
  ...
  if (!/^\s*fep\s*:/m.test(fepStr)) {
    errors.push("fep.yaml must contain 'fep:' (e.g. fep: \"1.1\")");
  }
  ...
  if (!/\bdef\s+compute\s*\s*\(\s*data\s*\)/.test(strategyStr)) {
    errors.push("scripts/strategy.py must define compute(data) function (per fep v1.1)");
  }
  if (!/\baction\s*/.test(strategyStr) || !/\bamount\b/.test(strategyStr)) {
    warnings.push("compute(data) should return dict with action, amount, price, reason (buy|sell|hold)");
  }
