...
function resolveOauthClientCredentials(): { clientId: string; clientSecret: string } {
  const clientId = process.env[OAUTH_CLIENT_ID_ENV]?.trim() ?? "";
  const clientSecret = process.env[OAUTH_CLIENT_SECRET_ENV]?.trim() ?? "";
  if (!clientId || !clientSecret) {
    throw new Error(
      `Missing OAuth client credentials. Set ${OAUTH_CLIENT_ID_ENV} and ${OAUTH_CLIENT_SECRET_ENV}.`,
    );
  }
