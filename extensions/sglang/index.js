...
const sglangPlugin = {
  ...
  register(api) {
    api.registerProvider({
      ...
      auth: [
        {
          ...
          kind: "custom",
          run: async (ctx) => promptAndConfigureOpenAICompatibleSelfHostedProviderAuth({
            cfg: ctx.config,
          ...
          runNonInteractive: async (ctx) => configureOpenAICompatibleSelfHostedProviderNonInteractive({
            ctx,
            providerId: PROVIDER_ID,
            providerLabel: "SGLang",
            defaultBaseUrl: DEFAULT_BASE_URL,
      ...
      discovery: {
        order: "late",
        run: async (ctx) => discoverOpenAICompatibleSelfHostedProvider({
          ctx,
          providerId: PROVIDER_ID,
...
var sglang_default = sglangPlugin;
export {
  sglang_default as default
};
