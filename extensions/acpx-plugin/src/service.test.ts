...
describe("createAcpxRuntimeService", () => {
  ...
  it("registers and unregisters the acpx-plugin backend", async () => {
    ...
    expect(getAcpRuntimeBackend("acpx-plugin")?.runtime).toBe(runtime);
  ...
  it("marks backend unavailable when runtime health check fails", async () => {
    ...
    expect(() => requireAcpRuntimeBackend("acpx-plugin")).toThrowError(AcpRuntimeError);
    try {
