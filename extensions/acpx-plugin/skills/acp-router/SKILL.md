...
2. First attempt automatic local repair:
   - ensure plugin-local pinned acpx is installed in `extensions/acpx-plugin`
...
1. Prefer plugin-local binary, not global PATH:
   - `./extensions/acpx-plugin/node_modules/.bin/acpx`
2. Resolve pinned version from extension dependency:
   - `node -e "console.log(require('./extensions/acpx-plugin/package.json').dependencies.acpx)"`
3. If binary is missing or version mismatched, install plugin-local pinned version:
   - `cd extensions/acpx-plugin && npm install --omit=dev --no-save acpx@<pinnedVersion>`
4. Verify before use:
   - `./extensions/acpx-plugin/node_modules/.bin/acpx --version`
...
```bash
ACPX_CMD="./extensions/acpx-plugin/node_modules/.bin/acpx"
```
...
- `acpx: command not found`:
  - for thread-spawn ACP requests, install plugin-local pinned acpx in `extensions/acpx-plugin` immediately
