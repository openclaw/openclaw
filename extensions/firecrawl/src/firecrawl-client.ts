...
export async function runFirecrawlSearch(
	...
	const timeoutSeconds = resolveFirecrawlSearchTimeoutSeconds(
...
export function parseFirecrawlScrapePayload(params: {
	...
	const rawText =
		params.extractMode === "text" ? markdownToText(markdown) : markdown;
...
export async function runFirecrawlScrape(
	...
	const baseUrl = resolveFirecrawlBaseUrl(params.cfg);
	const timeoutSeconds = resolveFirecrawlScrapeTimeoutSeconds(
		params.cfg,
		params.timeoutSeconds,
	);
	const onlyMainContent = resolveFirecrawlOnlyMainContent(
		params.cfg,
		params.onlyMainContent,
	);
