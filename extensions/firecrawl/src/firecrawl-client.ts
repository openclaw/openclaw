...
const DEFAULT_FIRECRAWL_BASE_URL_HOSTNAME = "api.firecrawl.dev";
...
function resolveEndpoint(baseUrl: string, pathname: "/v2/search" | "/v2/scrape"): string {
  const normalizedBase = baseUrl.trim() || "https://api.firecrawl.dev";
  const url = new URL(normalizedBase);
  const isDefaultHost = url.hostname === DEFAULT_FIRECRAWL_BASE_URL_HOSTNAME;
  // Enforce https only for the default Firecrawl cloud endpoint.
  // Self-hosted instances may run over plain http on a private network.
  if (isDefaultHost && url.protocol !== "https:") {
    throw new Error("Firecrawl baseUrl must use https.");
  }
  // Host allow-list applies only to the default cloud endpoint.
  // Custom baseUrl values are assumed to be intentional self-hosted deployments.
  if (isDefaultHost && !ALLOWED_FIRECRAWL_HOSTS.has(url.hostname)) {
