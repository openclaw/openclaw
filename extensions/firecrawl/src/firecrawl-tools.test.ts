...
describe("firecrawl tools", () => {
	...
	beforeAll(async () => {
		vi.resetModules();
		({ createFirecrawlWebSearchProvider } = await import(
			"./firecrawl-search-provider.js"
		));
		({ createFirecrawlSearchTool } = await import(
			"./firecrawl-search-tool.js"
		));
		({ createFirecrawlScrapeTool } = await import(
			"./firecrawl-scrape-tool.js"
		));
		({ __testing: firecrawlClientTesting } = await vi.importActual<
			typeof import("./firecrawl-client.js")
		>("./firecrawl-client.js"));
	});

	beforeEach(() => {
		runFirecrawlSearch.mockReset();
		runFirecrawlSearch.mockImplementation(
			async (params: Record<string, unknown>) => params,
		);
		runFirecrawlScrape.mockReset();
		runFirecrawlScrape.mockImplementation(
	...
	it("normalizes optional search parameters before invoking Firecrawl", async () => {
		runFirecrawlSearch.mockImplementationOnce(
			async (params: Record<string, unknown>) => ({
				ok: true,
				params,
			}),
	...
	it("falls back to environment and defaults for fetch config values", () => {
		...
		expect(resolveFirecrawlScrapeTimeoutSeconds()).toBe(
			DEFAULT_FIRECRAWL_SCRAPE_TIMEOUT_SECONDS,
		);
		expect(resolveFirecrawlSearchTimeoutSeconds()).toBe(
			DEFAULT_FIRECRAWL_SEARCH_TIMEOUT_SECONDS,
		);
		expect(resolveFirecrawlBaseUrl({} as OpenClawConfig)).not.toBe(
			DEFAULT_FIRECRAWL_BASE_URL,
		);
	});
