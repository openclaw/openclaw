...
export function createFirecrawlScrapeTool(api: OpenClawPluginApi) {
	return {
		...
		execute: async (
			...
			const extractMode =
				readStringParam(rawParams, "extractMode") === "text"
					? "text"
					: "markdown";
			...
			const onlyMainContent =
				typeof rawParams.onlyMainContent === "boolean"
					? rawParams.onlyMainContent
					: undefined;
			const storeInCache =
				typeof rawParams.storeInCache === "boolean"
					? rawParams.storeInCache
					: undefined;
