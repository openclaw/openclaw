...
export function resolveAnthropicVertexRegion(
...
function hasAnthropicVertexMetadataServerAdc(
	env: NodeJS.ProcessEnv = process.env,
): boolean {
	const explicitMetadataOptIn = normalizeOptionalSecretInput(
		env.ANTHROPIC_VERTEX_USE_GCP_METADATA,
	);
	return (
		explicitMetadataOptIn === "1" ||
		explicitMetadataOptIn?.toLowerCase() === "true"
	);
}

function resolveAnthropicVertexDefaultAdcPath(
	env: NodeJS.ProcessEnv = process.env,
): string {
...
function resolveAnthropicVertexAdcCredentialsPath(
	...
	const explicitCredentialsPath = normalizeOptionalSecretInput(
		env.GOOGLE_APPLICATION_CREDENTIALS,
	);
	if (explicitCredentialsPath) {
		return existsSync(explicitCredentialsPath)
			? explicitCredentialsPath
			: undefined;
	}
...
export function hasAnthropicVertexCredentials(
	env: NodeJS.ProcessEnv = process.env,
): boolean {
	return (
...
export function resolveAnthropicVertexConfigApiKey(
	...
	return hasAnthropicVertexAvailableAuth(env)
