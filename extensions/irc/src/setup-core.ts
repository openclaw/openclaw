...
export function setIrcDmPolicy(
	cfg: CoreConfig,
	dmPolicy: DmPolicy,
): CoreConfig {
...
export function setIrcAllowFrom(
	cfg: CoreConfig,
	allowFrom: string[],
): CoreConfig {
...
export const ircSetupAdapter: ChannelSetupAdapter = {
	...
	applyAccountConfig: ({ cfg, accountId, input }) => {
		...
		const portInput =
			typeof setupInput.port === "number"
				? String(setupInput.port)
				: String(setupInput.port ?? "");
		const patch: Partial<IrcAccountConfig> = {
			enabled: true,
			host: setupInput.host?.trim(),
			port: portInput
				? parsePort(portInput, setupInput.tls === false ? 6667 : 6697)
				: undefined,
			tls: setupInput.tls,
