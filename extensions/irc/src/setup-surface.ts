...
export const ircSetupWizard: ChannelSetupWizard = {
	...
	prepare: async ({ cfg, accountId, credentialValues, prompter }) => {
		...
		const envReady = Boolean(
			envHost && envNick && !resolved.config.host && !resolved.config.nick,
		);
	...
	textInputs: [
		{
			...
			currentValue: ({ cfg, accountId }) =>
				resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.host ||
				undefined,
		...
		},
		{
			...
			message: "IRC nick",
			currentValue: ({ cfg, accountId }) =>
				resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.nick ||
				undefined,
			shouldPrompt: ({ credentialValues }) =>
				credentialValues[USE_ENV_FLAG] !== "1",
			validate: ({ value }) =>
	...
	groupAccess: {
		...
		updatePrompt: ({ cfg, accountId }) =>
			Boolean(
				resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.groups,
			),
	...
	allowFrom: createAllowFromSection({
		...
		apply: async ({ cfg, allowFrom }) =>
			setIrcAllowFrom(cfg as CoreConfig, allowFrom),
	}),
