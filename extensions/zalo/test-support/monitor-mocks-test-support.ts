...
const secretInputModuleUrl = new URL("../src/secret-input.ts", import.meta.url)
	.href;
...
async function importSecretInputModule(
	cacheBust: string,
): Promise<SecretInputModule> {
	return (await import(
...
export function setLifecycleRuntimeCore(
	channel: NonNullable<
		NonNullable<Parameters<typeof createPluginRuntimeMock>[0]>["channel"]
	>,
) {
...
export async function startWebhookLifecycleMonitor(params: {
	...
	const { normalizeSecretInputString } =
		await importSecretInputModule("secret-input");
	const webhookSecret =
		params.webhookSecret ??
		normalizeSecretInputString(params.account.config?.webhookSecret);
	...
	await vi.waitFor(() => {
		if (
			setWebhookMock.mock.calls.length !== 1 ||
			registry.httpRoutes.length !== 1
		) {
