...
async function waitForPollingLoopStart() {
...
async function startLifecycleMonitor(options = {}) {
...
describe("monitorZaloProvider lifecycle", () => {
  ...
  it("waits for webhook deletion before finishing webhook shutdown", async () => {
    ...
    expect(deleteWebhookMock).toHaveBeenCalledWith("test-token", void 0, 5e3);
    expect(settled).toBe(false);
    expect(registry.httpRoutes).toHaveLength(1);
