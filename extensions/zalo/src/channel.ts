...
const zaloRawSendResultAdapter = createRawChannelSendResultAdapter({
	...
	sendText: async ({ to, text, accountId, cfg }) =>
		await (await loadZaloChannelRuntime()).sendZaloText({
			to,
			text,
...
const zaloConfigAdapter = createScopedChannelConfigAdapter<ResolvedZaloAccount>(
	{
		...
		resolveAllowFrom: (account: ResolvedZaloAccount) =>
...
const collectZaloSecurityWarnings =
	createOpenProviderGroupPolicyWarningCollector<{
		cfg: OpenClawConfig;
		...
		collect: ({ account, groupPolicy }) => {
			...
			const explicitGroupAllowFrom = mapAllowFromEntries(
		...
		base: {
			...
			messaging: {
				...
				resolveOutboundSessionRoute: (params) =>
					resolveZaloOutboundSessionRoute(params),
				targetResolver: {
			...
			>({
				...
				probeAccount: async ({ account, timeoutMs }) =>
					await (await loadZaloChannelRuntime()).probeZaloAccount({
						account,
						timeoutMs,
					}),
		...
		pairing: {
			text: {
				...
				normalizeAllowEntry: (entry) =>
					entry.trim().replace(/^(zalo|zl):/i, ""),
				notify: async (params) =>
					await (await loadZaloChannelRuntime()).notifyZaloPairingApproval(
						params,
					),
			},
		},
