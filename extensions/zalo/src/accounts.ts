...
export function listZaloAccountIds(cfg: DNAConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: DNAConfig): string {
...
function mergeZaloAccountConfig(cfg: DNAConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: DNAConfig): ResolvedZaloAccount[] {
