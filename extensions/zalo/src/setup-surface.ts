...
async function noteZaloTokenHelp(
	prompter: Parameters<
		NonNullable<ChannelSetupWizard["finalize"]>
	>[0]["prompter"],
): Promise<void> {
	await prompter.note(
		[
	...
	finalize: async ({ cfg, accountId, forceAllowFrom, options, prompter }) => {
		...
		if (wantsWebhook) {
			...
			let webhookSecretResult = await promptSingleChannelSecretInput({
				...
				secretInputMode: options?.secretInputMode,
				...buildSingleChannelSecretPromptState({
					accountConfigured: hasConfiguredSecretInput(
			...
			while (
				...
				(webhookSecretResult.value.length < 8 ||
					webhookSecretResult.value.length > 256)
			) {
				await prompter.note(
					"Webhook secret must be between 8 and 256 characters.",
					"Zalo webhook",
				);
