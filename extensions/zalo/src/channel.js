...
const zaloDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) => mapAllowFromEntries(resolveZaloAccount({ cfg, accountId }).config.allowFrom),
...
const zaloPlugin = {
  ...
  setup: {
    ...
    applyAccountConfig: ({ cfg, accountId, input }) => {
      ...
      const patch = input.useEnv ? {} : input.tokenFile ? { tokenFile: input.tokenFile } : input.token ? { botToken: input.token } : {};
