...
const webhookRequestHandler = async (req, res) => {
...
function createPairingAuthCore(params) {
  ...
  const upsertPairingRequest = vi.fn().mockResolvedValue({ code: "PAIRCODE", created: params?.pairingCreated ?? false });
  const core = {
    logging: {
...
async function postUntilRateLimited(params) {
  ...
  for (let i = 0; i < attempts; i += 1) {
    const url = params.withNonceQuery ? `${params.baseUrl}${params.path}?nonce=${i}` : `${params.baseUrl}${params.path}`;
