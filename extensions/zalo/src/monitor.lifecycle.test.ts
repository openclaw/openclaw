...
const getWebhookInfoMock = vi.fn(async () => ({
	ok: true,
	result: { url: "" },
}));
const deleteWebhookMock = vi.fn(async () => ({
	ok: true,
	result: { url: "" },
}));
...
describe("monitorZaloProvider lifecycle", () => {
	...
	it("continues polling when webhook inspection returns 404", async () => {
		...
		getWebhookInfoMock.mockRejectedValueOnce(
			new ZaloApiError("Not Found", 404, "Not Found"),
		);
		...
		expect(runtime.log).toHaveBeenCalledWith(
			expect.stringContaining(
				"webhook inspection unavailable; continuing without webhook cleanup",
			),
		);
