...
export async function handleZaloWebhookRequest(
	...
	return handleZaloWebhookRequestInternal(
		req,
		res,
		async ({ update, target }) => {
			await processUpdate({
...
function startPollingLoop(params: ZaloPollingLoopParams) {
	...
	runtime.log?.(
		`[${account.accountId}] Zalo polling loop started timeout=${String(pollTimeout)}s`,
	);
	...
	const poll = async () => {
		...
		try {
			...
			if (err instanceof ZaloApiError && err.isPollingTimeout) {
				...
				runtime.error?.(
					`[${account.accountId}] Zalo polling error: ${formatZaloError(err)}`,
				);
...
async function processUpdate(
	params: ZaloUpdateProcessingParams,
): Promise<void> {
	const {
		update,
		token,
	...
	switch (event_name) {
		...
		case "message.sticker.received":
			logVerbose(
				core,
				runtime,
				`[${account.accountId}] Received sticker from ${message.from.id}`,
			);
			break;
...
async function handleImageMessage(
	...
	if (photo_url) {
		try {
			...
			runtime.error?.(
				`[${account.accountId}] Failed to download Zalo image: ${String(err)}`,
			);
		}
	}
...
async function authorizeZaloMessage(
	...
	const configuredGroupAllowFrom = (account.config.groupAllowFrom ?? []).map(
		(v) => String(v),
	);
	...
	if (groupAccess) {
		warnMissingProviderGroupPolicyFallbackOnce({
			providerMissingFallbackApplied:
				groupAccess.providerMissingFallbackApplied,
			providerKey: "zalo",
			accountId: account.accountId,
		...
		if (!groupAccess.allowed) {
			if (groupAccess.reason === "disabled") {
				logVerbose(
					core,
					runtime,
					`zalo: drop group ${chatId} (groupPolicy=disabled)`,
				);
				...
				logVerbose(
					core,
					runtime,
					`zalo: drop group sender ${senderId} (groupPolicy=allowlist)`,
				);
			}
			return;
		}
	...
	if (directDmOutcome === "disabled") {
		logVerbose(
			core,
			runtime,
			`Blocked zalo DM from ${senderId} (dmPolicy=disabled)`,
		);
		return;
	}
...
async function processMessageWithPipeline(
	params: ZaloMessagePipelineParams,
): Promise<void> {
	const {
		message,
	...
	const { isGroup, chatId, senderId, senderName, rawBody, commandAuthorized } =
		authorization;
	...
	if (
		...
		logVerbose(
			core,
			runtime,
			`zalo: drop control command from unauthorized sender ${senderId}`,
		);
		return;
	}
...
export async function monitorZaloProvider(
	...
	const fetcher =
		fetcherOverride ?? resolveZaloProxyFetch(account.config.proxy);
	...
	try {
		if (useWebhook) {
			if (!webhookUrl || !webhookSecret) {
				throw new Error(
					"Zalo webhookUrl and webhookSecret are required for webhook mode",
				);
			}
			...
			cleanupWebhook = async () => {
				if (!webhookCleanupPromise) {
					webhookCleanupPromise = (async () => {
						runtime.log?.(
							`[${account.accountId}] Zalo stopping; deleting webhook`,
						);
						try {
							...
							const detail =
								err instanceof Error && err.name === "AbortError"
									...
									: formatZaloError(err);
							runtime.error?.(
								`[${account.accountId}] Zalo webhook delete failed: ${detail}`,
							);
						}
					})();
				}
			...
			runtime.log?.(
				`[${account.accountId}] Zalo webhook registered path=${path}`,
			);
		...
		runtime.log?.(
			`[${account.accountId}] Zalo polling mode: clearing webhook before startup`,
		);
		try {
			try {
				...
				if (!currentWebhookUrl) {
					runtime.log?.(
						`[${account.accountId}] Zalo polling mode ready (no webhook configured)`,
					);
				} else {
					runtime.log?.(
					...
					runtime.log?.(
						`[${account.accountId}] Zalo polling mode ready (webhook disabled)`,
					);
				}
			} catch (err) {
