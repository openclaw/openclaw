...
describe("collectTelegramStatusIssues", () => {
	it("reports privacy-mode and wildcard unmentioned-group configuration risks", () => {
		...
		expect(issues.some((issue) => issue.message.includes("privacy mode"))).toBe(
			true,
		);
		expect(issues.some((issue) => issue.message.includes('uses "*"'))).toBe(
			true,
		);
		expect(
			issues.some((issue) => issue.message.includes("unresolvedGroups=2")),
		).toBe(true);
	});
