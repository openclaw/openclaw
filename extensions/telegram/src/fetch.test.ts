...
beforeAll(async () => {
	...
	({ resolveTelegramFetch, resolveTelegramTransport } = await import(
		"./fetch.js"
	));
});
...
function getDispatcherFromUndiciCall(nth: number) {
	const call = undiciFetch.mock.calls[nth - 1] as
...
function buildFetchFallbackError(code: string) {
	const connectErr = Object.assign(
		new Error(`connect ${code} api.telegram.org:443`),
		{
			code,
		},
	);
...
async function runDefaultStickyIpv4FallbackProbe(
	code = "EHOSTUNREACH",
): Promise<void> {
	undiciFetch
...
function primeStickyFallbackRetry(
	code = "EHOSTUNREACH",
	successCount = 2,
): void {
...
function expectPinnedIpv4ConnectDispatcher(args: {
	...
	if (args.firstCall) {
		expect(getDispatcherFromUndiciCall(args.firstCall)).not.toBe(
			pinnedDispatcher,
		);
	}
	if (args.followupCall) {
		expect(getDispatcherFromUndiciCall(args.followupCall)).toBe(
			pinnedDispatcher,
		);
	}
}
...
function expectCallerDispatcherPreserved(
	callIndexes: number[],
	dispatcher: unknown,
) {
...
async function expectNoStickyRetryWithSameDispatcher(params: {
	...
	await expect(
		params.resolved("https://api.telegram.org/botx/sendMessage"),
...
describe("resolveTelegramFetch", () => {
	...
	it("emits default transport decisions at debug level", () => {
		...
		expect(loggerInfo).not.toHaveBeenCalledWith(
			"dnsResultOrder=ipv4first (default-node22)",
		);
	...
	it("arms sticky IPv4 fallback when env proxy init falls back to direct Agent", async () => {
		...
		EnvHttpProxyAgentCtor.mockImplementationOnce(
			function ThrowingEnvProxyAgent() {
	...
	it("does not retry when error codes do not match fallback rules", async () => {
		...
		await expect(
			resolved("https://api.telegram.org/botx/sendMessage"),
		).rejects.toThrow("fetch failed");
