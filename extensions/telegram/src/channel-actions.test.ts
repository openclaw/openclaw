...
const originalHandleTelegramAction =
	telegramMessageActionRuntime.handleTelegramAction;
	...
	it("normalizes reaction message identifiers before dispatch", async () => {
		...
		for (const testCase of cases) {
			...
			if (testCase.expectedMessageId === undefined) {
				...
				expect(String(call?.messageId), testCase.name).toBe(
					testCase.expectedMessageId,
				);
			}
		}
	});
