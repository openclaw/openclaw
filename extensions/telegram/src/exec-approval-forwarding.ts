...
export function shouldSuppressTelegramExecApprovalForwardingFallback(params: {
	...
	const channel =
		normalizeMessageChannel(params.target.channel) ?? params.target.channel;
	if (channel !== "telegram") {
		return false;
	}
	const requestChannel = normalizeMessageChannel(
...
export function buildTelegramExecApprovalPendingPayload(params: {
	...
	return buildExecApprovalPendingReplyPayload({
		...
		command: resolveExecApprovalCommandDisplay(params.request.request)
			.commandText,
