...
type TelegramActionName =
	(typeof TELEGRAM_ACTION_ALIASES)[keyof typeof TELEGRAM_ACTION_ALIASES];
type TelegramForumTopicIconColor =
	(typeof TELEGRAM_FORUM_TOPIC_ICON_COLORS)[number];
...
function readTelegramForumTopicIconColor(
	...
	if (
		!TELEGRAM_FORUM_TOPIC_ICON_COLORS.includes(
			iconColor as TelegramForumTopicIconColor,
		)
	) {
		throw new Error(
...
export async function handleTelegramAction(
	...
	const { action, accountId } = {
		action: normalizeTelegramActionName(
			readStringParam(params, "action", { required: true }),
		),
	...
	if (action === "react") {
		...
		let reactionResult: Awaited<
			ReturnType<typeof telegramActionRuntime.reactMessageTelegram>
		>;
		try {
	...
	if (action === "sendMessage") {
		...
		const result = await telegramActionRuntime.sendMessageTelegram(
			to,
			content,
			{
				cfg,
	...
	if (action === "poll") {
		...
		const allowMultiselect =
			readBooleanParam(params, "allowMultiselect") ??
			readBooleanParam(params, "pollMulti");
	...
	if (action === "deleteMessage") {
		...
		await telegramActionRuntime.deleteMessageTelegram(
			chatId ?? "",
			messageId ?? 0,
			{
				cfg,
	...
	if (action === "sendSticker") {
		...
		const fileId =
			readStringParam(params, "fileId") ??
			readStringArrayParam(params, "stickerId")?.[0];
		if (!fileId) {
	...
	if (action === "createForumTopic") {
		...
		const result = await telegramActionRuntime.createForumTopicTelegram(
			chatId ?? "",
			name,
			{
				cfg,
