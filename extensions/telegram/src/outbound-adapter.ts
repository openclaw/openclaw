...
function resolveTelegramPayloadSendShape(payload: ReplyPayload) {
  const telegramData = payload.channelData?.telegram as TelegramPayloadChannelData | undefined;
...
export async function sendTelegramPayloadMessages(params: {
  ...
  const { quoteText, reactionEmoji, text, mediaUrls, buttons } = resolveTelegramPayloadSendShape(
    params.payload,
  );
