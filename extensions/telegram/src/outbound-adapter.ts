...
export async function sendTelegramPayloadMessages(params: {
  ...
  let reactionResult: Awaited<ReturnType<TelegramReactionFn>> | undefined;
  if (reactionEmoji && typeof replyToMessageId === "number") {
    reactionResult = await params.react(params.to, replyToMessageId, reactionEmoji, {
  ...
  if (reactionEmoji && !text && mediaUrls.length === 0 && !buttons?.length) {
    ...
    if (!reactionResult?.ok) {
      throw new Error(reactionResult?.warning ?? "Telegram reaction was not delivered");
    }
