...
} from "./test-support/plugin-command.js";
...
describe("registerTelegramNativeCommands", () => {
	...
	it("scopes skill commands when account binding exists", () => {
		...
		registerTelegramNativeCommands(
			createNativeCommandTestParams(cfg, { accountId: "bot-a" }),
		);
	...
	it("normalizes hyphenated native command names for Telegram registration", async () => {
		...
		expect(
			registeredCommands.some((entry) => entry.command === "export-session"),
		).toBe(false);
	...
	it("registers only Telegram-safe command names across native, custom, and plugin sources", async () => {
		...
		expect(
			registeredCommands.some((entry) => entry.command === "custom_backup"),
		).toBe(true);
		expect(
			registeredCommands.some((entry) => entry.command === "plugin_status"),
		).toBe(true);
		expect(
			registeredCommands.some((entry) => entry.command === "plugin-status"),
		).toBe(false);
		expect(
			registeredCommands.some((entry) => entry.command === "custom-bad"),
		).toBe(false);
	});
	...
	it("prefixes native command menu callback data so callback handlers can preserve native routing", async () => {
		...
		expect(parseTelegramNativeCommandCallbackData("tgcmd:/fast status")).toBe(
			"/fast status",
		);
		expect(
			parseTelegramNativeCommandCallbackData("tgcmd:fast status"),
		).toBeNull();
	});
	...
	it("passes agent-scoped media roots for plugin command replies with media", async () => {
		...
		const firstDeliverRepliesCall = deliverReplies.mock.calls.at(0) as
			| [unknown]
			| undefined;
