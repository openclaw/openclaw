...
vi.mock("../../../src/infra/outbound/session-binding-service.js", async (importOriginal) => {
  ...
  return {
    ...
    getSessionBindingService: () => ({
      ...
      listBySession: vi.fn(),
      resolveByConversation: (ref) => hoisted.resolveByConversationMock(ref),
      touch: (bindingId, at) => hoisted.touchMock(bindingId, at),
      unbind: vi.fn()
    })
  };
});
...
describe("buildTelegramMessageContext bound conversation override", () => {
  ...
  it("routes forum topic messages to the bound session", async () => {
    ...
    expect(hoisted.touchMock).toHaveBeenCalledWith("default:-100200300:topic:77", void 0);
  });
  it("treats named-account bound conversations as explicit route matches", async () => {
    ...
    expect(hoisted.touchMock).toHaveBeenCalledWith("work:-100200300:topic:77", void 0);
  });
  it("routes dm messages to the bound session", async () => {
    ...
    expect(hoisted.touchMock).toHaveBeenCalledWith("default:1234", void 0);
  });
});
