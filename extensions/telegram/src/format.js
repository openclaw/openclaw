...
const FILE_EXTENSIONS_WITH_TLD = /* @__PURE__ */ new Set([
  "md",
  // Markdown (Moldova) - very common in repos
  "go",
  // Go language - common in Go projects
  "py",
  // Python (Paraguay) - common in Python projects
  "pl",
  // Perl (Poland) - common in Perl projects
  "sh",
  // Shell (Saint Helena) - common for scripts
  "am",
  // Automake files (Armenia)
  "at",
  // Assembly (Austria)
  "be",
  // Backend files (Belgium)
  "cc"
  // C++ source (Cocos Islands)
]);
...
function wrapSegmentFileRefs(text, codeDepth, preDepth, anchorDepth) {
  ...
  return wrappedStandalone.replace(
    ...
    (match, prefix, tld) => prefix === ">" ? match : `${prefix}<code>${escapeHtml(tld)}</code>`
  );
}
...
const TELEGRAM_SELF_CLOSING_HTML_TAGS = /* @__PURE__ */ new Set(["br"]);
...
function splitTelegramChunkByHtmlLimit(chunk, htmlLimit, renderedHtmlLength) {
  ...
  const splitLimit = Number.isFinite(candidateLimit) && candidateLimit > 0 ? candidateLimit : Math.max(1, Math.floor(currentTextLength / 2));
