  ...
  if (menu && commandDefinition) {
    ...
    for (let index = 0; index < menu.choices.length; index += 2) {
      const row = menu.choices.slice(index, index + 2).flatMap((choice) => {
        // Telegram model buttons are session-only. Keep every model choice on the canonical
        // callback path so callback length cannot change runtime or authorization semantics.
