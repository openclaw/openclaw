...
describe("telegram stickers", () => {
  const STICKER_TEST_TIMEOUT_MS = process.platform === "win32" ? 3e4 : 2e4;
  beforeEach(() => {
    cacheStickerSpy.mockClear();
    getCachedStickerSpy.mockClear();
    describeStickerImageSpy.mockClear();
    getCachedStickerSpy.mockReturnValue(void 0);
    describeStickerImageSpy.mockReturnValue(void 0);
  });
  it(
    ...
    async () => {
      ...
      expect(payload.Sticker?.emoji).toBe("\u{1F389}");
...
describe("telegram text fragments", () => {
  ...
  const TEXT_FRAGMENT_TEST_TIMEOUT_MS = process.platform === "win32" ? 45e3 : 2e4;
