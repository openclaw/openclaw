...
function normalizeDnsResultOrder(
	value: string | null,
): TelegramDnsResultOrder | null {
...
function buildTelegramConnectOptions(params: {
	...
	if (params.forceIpv4) {
		...
		connect.autoSelectFamilyAttemptTimeout =
...
function createTelegramDispatcher(policy: PinnedDispatcherPolicy): {
	...
	if (policy.mode === "explicit-proxy") {
		const requestTlsOptions = withPinnedLookup(
			policy.proxyTls,
	...
	if (policy.mode === "env-proxy") {
		const connectOptions = withPinnedLookup(
			policy.connect,
			policy.pinnedHostname,
		);
		const proxyTlsOptions = withPinnedLookup(
			policy.proxyTls,
			policy.pinnedHostname,
		);
		const proxyOptions =
...
function logResolverNetworkDecisions(params: {
	autoSelectDecision: ReturnType<
		typeof resolveTelegramAutoSelectFamilyDecision
	>;
	...
	if (params.autoSelectDecision.value !== null) {
		const sourceLabel = params.autoSelectDecision.source
			...
			: "";
		log.debug(
			`autoSelectFamily=${params.autoSelectDecision.value}${sourceLabel}`,
		);
	}
	if (params.dnsDecision.value !== null) {
		const sourceLabel = params.dnsDecision.source
			? ` (${params.dnsDecision.source})`
			: "";
	...
	attempts.push({
		createDispatcher: () => {
			if (!fallbackIpDispatcher) {
				fallbackIpDispatcher =
					createTelegramDispatcher(fallbackIpPolicy).dispatcher;
			}
...
export function resolveTelegramTransport(
	...
	const explicitProxyUrl = proxyFetch
		? getProxyUrlFromFetch(proxyFetch)
		: undefined;
	...
	const resolvedFetch = (async (
		...
		try {
			return await sourceFetch(
				...
				withDispatcherIfMissing(
					init,
					transportAttempts[startIndex].createDispatcher(),
				),
			);
		} catch (caught) {
			err = caught;
		...
		for (
			...
			nextIndex < transportAttempts.length;
			nextIndex += 1
		) {
	...
	return {
		fetch: resolvedFetch,
		sourceFetch,
		dispatcherAttempts: transportAttempts.map(
			(attempt) => attempt.exportAttempt,
		),
	};
}
