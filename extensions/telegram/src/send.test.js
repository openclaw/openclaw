...
installTelegramSendTestHooks();
const { botApi, botCtorSpy, loadConfig, loadWebMedia, maybePersistResolvedTelegramTarget } = getTelegramSendTestMocks();
const {
  buildInlineKeyboard,
...
async function expectChatNotFoundWithChatId(action, expectedChatId) {
  try {
    ...
    if (error instanceof Error && error.message === "Expected action to reject with chat-not-found context") {
      throw error;
    }
...
describe("sendMessageTelegram", () => {
  ...
  it("falls back to plain text when Telegram rejects HTML and preserves send params", async () => {
    ...
    const cases = [
      ...
      },
      {
        ...
        messageId: 60,
        options: { messageThreadId: 271, replyToMessageId: 100 },
        firstCall: {
          parse_mode: "HTML",
          message_thread_id: 271,
  ...
  it("keeps link_preview_options disabled for both html and plain-text fallback", async () => {
    ...
    const cases = [
      ...
      },
      {
        ...
        text: "_oops_",
        sendMessage: vi.fn().mockRejectedValueOnce(parseErr).mockResolvedValueOnce({ message_id: 42, chat: { id: "123" } }),
        expectedCalls: [
          [
            "123",
            "<i>oops</i>",
    ...
    for (const testCase of cases) {
      ...
      const api = { sendMessage: testCase.sendMessage };
  ...
  it("applies reply markup and thread options to split video-note sends", async () => {
    ...
    for (const testCase of cases) {
      ...
      if ("replyToMessageId" in testCase.options && testCase.options.replyToMessageId !== void 0) {
  ...
  it.each([
    ...
    const api = { sendAnimation, sendDocument, sendPhoto };
    mockLoadedMedia({
  ...
  it("routes audio media to sendAudio/sendVoice based on voice compatibility", async () => {
    ...
    for (const testCase of cases) {
      ...
      await sendMessageTelegram(testCase.chatId, testCase.text, {
        ...
        ..."messageThreadId" in testCase && testCase.messageThreadId !== void 0 ? { messageThreadId: testCase.messageThreadId } : {},
        ..."replyToMessageId" in testCase && testCase.replyToMessageId !== void 0 ? { replyToMessageId: testCase.replyToMessageId } : {}
      });
  ...
  it("retries sends without message_thread_id on thread-not-found", async () => {
    ...
    for (const testCase of cases) {
      const sendMessage = vi.fn().mockRejectedValueOnce(threadErr).mockResolvedValueOnce({
        message_id: testCase.messageId,
        chat: { id: testCase.chatId }
      });
      const api = { sendMessage };
  ...
  it("keeps disable_notification on plain-text fallback when silent is true", async () => {
    ...
    const sendMessage = vi.fn().mockRejectedValueOnce(parseErr).mockResolvedValueOnce({ message_id: 2, chat: { id: chatId } });
    const api = { sendMessage };
  ...
  it("retries media sends without message_thread_id when thread is missing", async () => {
    const chatId = "-100123";
    const threadErr = new Error("400: Bad Request: message thread not found");
    const sendPhoto = vi.fn().mockRejectedValueOnce(threadErr).mockResolvedValueOnce({
      message_id: 59,
      chat: { id: chatId }
    });
    const api = { sendPhoto };
  ...
  it("chunks long html-mode text and keeps buttons on the last chunk only", async () => {
    const chatId = "123";
    const htmlText = `<b>${"A".repeat(5e3)}</b>`;
    const sendMessage = vi.fn().mockResolvedValueOnce({ message_id: 90, chat: { id: chatId } }).mockResolvedValueOnce({ message_id: 91, chat: { id: chatId } });
    const api = { sendMessage };
    ...
    expect(firstCall[1].length).toBeLessThanOrEqual(4e3);
    expect(secondCall[1].length).toBeLessThanOrEqual(4e3);
  ...
  it("preserves caller plain-text fallback across chunked html parse retries", async () => {
    ...
    const sendMessage = vi.fn().mockRejectedValueOnce(parseErr).mockResolvedValueOnce({ message_id: 90, chat: { id: chatId } }).mockRejectedValueOnce(parseErr).mockResolvedValueOnce({ message_id: 91, chat: { id: chatId } });
    const api = { sendMessage };
    const res = await sendMessageTelegram(chatId, htmlText, {
      token: "tok",
      api,
  ...
  it("keeps malformed leading ampersands on the chunked plain-text fallback path", async () => {
    const chatId = "123";
    const htmlText = `&${"A".repeat(5e3)}`;
    const plainText = "fallback!!";
    const parseErr = new Error(
      "400: Bad Request: can't parse entities: Can't find end of the entity starting at byte offset 0"
    );
    const sendMessage = vi.fn().mockRejectedValueOnce(parseErr).mockResolvedValueOnce({ message_id: 92, chat: { id: chatId } }).mockRejectedValueOnce(parseErr).mockResolvedValueOnce({ message_id: 93, chat: { id: chatId } });
    const api = { sendMessage };
    const res = await sendMessageTelegram(chatId, htmlText, {
      token: "tok",
      api,
  ...
  it("cuts over to plain text when fallback text needs more chunks than html", async () => {
    const chatId = "123";
    const htmlText = `<b>${"A".repeat(5e3)}</b>`;
    const plainText = "P".repeat(9e3);
    const sendMessage = vi.fn().mockResolvedValueOnce({ message_id: 94, chat: { id: chatId } }).mockResolvedValueOnce({ message_id: 95, chat: { id: chatId } }).mockResolvedValueOnce({ message_id: 96, chat: { id: chatId } });
    const api = { sendMessage };
    const res = await sendMessageTelegram(chatId, htmlText, {
      token: "tok",
      api,
    ...
    expect(sendMessage.mock.calls.every((call) => call[2]?.parse_mode === void 0)).toBe(true);
...
describe("reactMessageTelegram", () => {
  ...
  it("resolves legacy telegram targets before reacting", async () => {
    const setMessageReaction = vi.fn().mockResolvedValue(void 0);
    const getChat = vi.fn().mockResolvedValue({ id: -100123 });
    const api = { setMessageReaction, getChat };
...
describe("sendStickerTelegram", () => {
  ...
  for (const testCase of positiveSendCases) {
    it(testCase.name, async () => {
      ...
      expect(sendSticker).toHaveBeenCalledWith(chatId, testCase.expectedFileId, void 0);
  ...
  it("retries sticker sends without message_thread_id when thread is missing", async () => {
    const chatId = "-100123";
    const threadErr = new Error("400: Bad Request: message thread not found");
    const sendSticker = vi.fn().mockRejectedValueOnce(threadErr).mockResolvedValueOnce({
      message_id: 109,
      chat: { id: chatId }
    });
    const api = { sendSticker };
    ...
    expect(sendSticker).toHaveBeenNthCalledWith(2, chatId, "fileId123", void 0);
...
describe("editMessageTelegram", () => {
  ...
  it("retries editMessageTelegram on Telegram 5xx errors", async () => {
    botApi.editMessageText.mockRejectedValueOnce(Object.assign(new Error("502: Bad Gateway"), { error_code: 502 })).mockResolvedValueOnce({ message_id: 1, chat: { id: "123" } });
    await expect(
      editMessageTelegram("123", 1, "hi", {
        token: "tok",
        cfg: {},
...
describe("sendPollTelegram", () => {
  ...
  it("retries without message_thread_id on thread-not-found", async () => {
    const api = {
      sendPoll: vi.fn(
        async (_chatId, _question, _options, params) => {
          const p = params;
    ...
    expect(
      api.sendPoll.mock.calls[1]?.[3]?.message_thread_id
    ).toBeUndefined();
  });
  ...
  const cases = [
    {
      ...
      expectedCall: ["-1001234567890", "x", void 0],
      expectedResult: {
        topicId: 272,
        name: "Build Updates",
        chatId: "-1001234567890"
    ...
    },
    {
      ...
      expectedCall: [
        "-1001234567890",
        "Roadmap",
        { icon_color: 7322096, icon_custom_emoji_id: "1234567890" }
      ],
      expectedResult: {
        topicId: 300,
        name: "Roadmap",
