...
const telegramDepsForTest: TelegramBotDeps = {
	...
	enqueueSystemEvent:
		enqueueSystemEvent as TelegramBotDeps["enqueueSystemEvent"],
	dispatchReplyWithBufferedBlockDispatcher:
		dispatchReplyWithBufferedBlockDispatcher as TelegramBotDeps["dispatchReplyWithBufferedBlockDispatcher"],
	buildModelsProviderData:
		buildModelsProviderData as TelegramBotDeps["buildModelsProviderData"],
	...
	editMessageTelegram:
		editMessageTelegram as TelegramBotDeps["editMessageTelegram"],
};
...
describe("dispatchTelegramMessage draft streaming", () => {
	...
	const createDraftStream = (messageId?: number) =>
		createTestDraftStream({ messageId });
	...
	it("does not inject approval buttons in local dispatch once the monitor owns approvals", async () => {
		...
		const deliveredPayload = (
			deliverReplies.mock.calls[0]?.[0] as { replies?: Array<unknown> }
	...
	it("does not overwrite finalized preview when additional final payloads are sent", async () => {
		...
		deliverReplies.mockResolvedValue({ delivered: true });
		editMessageTelegram.mockResolvedValue({
			ok: true,
			chatId: "123",
			messageId: "999",
		});
		...
		expect(deliverReplies).toHaveBeenCalledWith(
			expect.objectContaining({
				replies: [
					expect.objectContaining({ text: "⚠️ Recovered tool error details" }),
				],
			}),
		);
	...
	])("disables block streaming when streamMode is off ($label)", async ({
		telegramCfg,
	}) => {
		dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
			async ({ dispatcherOptions }) => {
	...
	it.each([
		"block",
		"partial",
	] as const)("forces new message when assistant message restarts (%s mode)", async (streamMode) => {
		const draftStream = createDraftStream(999);
		createTelegramDraftStream.mockReturnValue(draftStream);
		dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
			async ({ dispatcherOptions, replyOptions }) => {
	...
	it("queues late partials behind async boundary materialization", async () => {
		...
		dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
			async ({ replyOptions }) => {
				...
				const startPromise = replyOptions?.onAssistantMessageStart?.();
				const nextPartialPromise = replyOptions?.onPartialReply?.({
		...
		const boundaryRotationOrder =
			answerDraftStream.forceNewMessage.mock.invocationCallOrder[0];
	...
	it("does not skip message-start rotation when pre-rotation did not force a new message", async () => {
		...
		expect(
			(bot.api.deleteMessage as ReturnType<typeof vi.fn>).mock.calls,
		).toHaveLength(0);
	});
	...
	it("does not trigger late pre-rotation mid-message after an explicit assistant message start", async () => {
		...
		expect(answerDraftStream.update).toHaveBeenNthCalledWith(
			1,
			"Message B first chunk",
		);
		expect(answerDraftStream.update).toHaveBeenNthCalledWith(
			2,
			"Message B second chunk",
		);
	});
	...
	it.each([
		"partial",
		"block",
	] as const)("keeps finalized text preview when the next assistant message is media-only (%s mode)", async (streamMode) => {
	...
	it.each([
		"block",
		"partial",
	] as const)("splits reasoning lane only when a later reasoning block starts (%s mode)", async (streamMode) => {
	...
	it.each([
		"block",
		"partial",
	] as const)("does not split reasoning lane on reasoning end without a later reasoning block (%s mode)", async (streamMode) => {
		const { reasoningDraftStream } = setupDraftStreams({
			answerMessageId: 999,
			reasoningMessageId: 111,
		});
	...
	it.each([
		undefined,
		null,
	] as const)("skips outbound send when final payload text is %s and has no media", async (emptyText) => {
	...
	it.each([
		"partial",
		"block",
	] as const)("does not duplicate reasoning final after reasoning end (%s mode)", async (streamMode) => {
	...
	it("routes think-tag partials to reasoning lane and keeps answer lane clean", async () => {
		...
		expect(
			answerDraftStream.update.mock.calls.some((call) =>
				String(call[0] ?? "").includes("<think>"),
			),
		).toBe(false);
		expect(editMessageTelegram).toHaveBeenCalledWith(
			123,
	...
	it("clears preview for error-only finals", async () => {
		...
		dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
			async ({ dispatcherOptions }) => {
				...
				await dispatcherOptions.deliver(
					{ text: "another error", isError: true },
					{ kind: "final" },
				);
				return { queuedFinal: true };
			},
	...
	it("shows compacting reaction during auto-compaction and resumes thinking", async () => {
		...
		expect(
			statusReactionController.setCompacting.mock.invocationCallOrder[0],
		).toBeLessThan(
