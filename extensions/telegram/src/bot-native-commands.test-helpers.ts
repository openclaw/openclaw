...
type MatchPluginCommandFn =
	typeof import("openclaw/plugin-sdk/plugin-runtime").matchPluginCommand;
...
const pluginCommandMocks = vi.hoisted(() => ({
	...
	executePluginCommand: vi.fn<ExecutePluginCommandFn>(async () => ({
		text: "ok",
	})),
}));
...
const replyPipelineMocks = vi.hoisted(() => {
	...
	return {
		...
		recordInboundSessionMetaSafe: vi.fn<RecordInboundSessionMetaSafeFn>(
			async () => undefined,
		),
	};
});
...
vi.mock("openclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
	...
	return {
		...
		recordInboundSessionMetaSafe:
			replyPipelineMocks.recordInboundSessionMetaSafe,
...
vi.mock(
	...
	async (importOriginal) => {
		const actual =
			await importOriginal<
				typeof import("openclaw/plugin-sdk/channel-reply-pipeline")
			>();
		return {
			...actual,
