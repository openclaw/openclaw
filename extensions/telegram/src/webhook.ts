...
function resolveSingleHeaderValue(
...
function isTrustedProxyAddress(
	...
	return blockList.check(
		candidate,
		net.isIP(candidate) === 6 ? "ipv6" : "ipv4",
	);
}
	...
	const server = createServer((req, res) => {
		...
		if (
			!applyBasicWebhookRequestGuards({
				...
				rateLimitKey: resolveTelegramWebhookRateLimitKey(
					req,
					path,
					opts.config,
				),
		...
		const secretHeader = resolveSingleHeaderValue(
