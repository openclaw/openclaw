...
const buildTelegramMessageContext = async ({
  ...
  const requiresExplicitAccountBinding = (candidate) => candidate.accountId !== DEFAULT_ACCOUNT_ID && candidate.matchedBy === "default";
  ...
  if (!baseAccess.allowed) {
    ...
    logVerbose(
      isGroup ? `Blocked telegram group sender ${senderId || "unknown"} (group allowFrom override)` : `Blocked telegram DM sender ${senderId || "unknown"} (DM allowFrom override)`
    );
    return null;
  }
  ...
  const threadKeys = dmThreadId != null ? resolveThreadSessionKeys({ baseSessionKey, threadId: `${chatId}:${dmThreadId}` }) : null;
  ...
  const ackReactionPromise = statusReactionController ? shouldAckReaction() ? Promise.resolve(statusReactionController.setQueued()).then(
    () => true,
    () => false
  ) : null : shouldAckReaction() && msg.message_id && reactionApi ? withTelegramApiErrorLogging({
