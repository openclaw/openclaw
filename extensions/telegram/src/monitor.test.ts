...
const makeAbortRunner = (
	abort: AbortController,
	beforeAbort?: () => void,
): RunnerStub => makeRunnerStub({ task: createAbortTask(abort, beforeAbort) });
...
async function runMonitorAndCaptureStartupOrder(params?: {
	...
	if (params && "persistedOffset" in params) {
		readTelegramUpdateOffsetSpy.mockResolvedValueOnce(
			params.persistedOffset ?? null,
		);
	}
	const abort = new AbortController();
	const order: string[] = [];
...
async function monitorWithAutoAbort(
	opts: Omit<MonitorTelegramOpts, "abortSignal"> = {},
) {
	const abort = new AbortController();
	mockRunOnceAndAbort(abort);
...
vi.mock("openclaw/plugin-sdk/config-runtime", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("openclaw/plugin-sdk/config-runtime")>();
	return {
		...actual,
		loadConfig,
	};
...
describe("monitorTelegramProvider (grammY)", () => {
	...
	it("force-restarts polling when unhandled network rejection stalls runner", async () => {
		...
		expect(emitUnhandledRejection(await makeTaggedPollingFetchError())).toBe(
			true,
		);
	...
	it("rebuilds the resolved transport after a stalled polling restart", async () => {
		...
		try {
			...
			await vi.waitFor(() =>
				expect(createTelegramBotCalls.length).toBeGreaterThanOrEqual(1),
			);
			...
			expect(createTelegramBotCalls[0]?.telegramTransport).toBe(
				telegramTransport,
			);
			expect(createTelegramBotCalls[1]?.telegramTransport).toBe(
				rebuiltTransport,
			);
		} finally {
			vi.useRealTimers();
