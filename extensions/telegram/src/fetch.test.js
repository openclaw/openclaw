...
const AgentCtor = vi.hoisted(
  () => vi.fn(function MockAgent(options) {
    this.options = options;
  })
);
const EnvHttpProxyAgentCtor = vi.hoisted(
  () => vi.fn(function MockEnvHttpProxyAgent(options) {
    this.options = options;
  })
);
const ProxyAgentCtor = vi.hoisted(
  () => vi.fn(function MockProxyAgent(options) {
    this.options = options;
  })
);
...
function resolveTelegramFetchOrThrow(proxyFetch, options) {
  return resolveTelegramFetch(proxyFetch, options);
}
function getDispatcherFromUndiciCall(nth) {
  const call = undiciFetch.mock.calls[nth - 1];
  if (!call) {
...
function buildFetchFallbackError(code) {
...
async function runDefaultStickyIpv4FallbackProbe(code = "EHOSTUNREACH") {
  undiciFetch.mockRejectedValueOnce(buildFetchFallbackError(code)).mockResolvedValueOnce({ ok: true }).mockResolvedValueOnce({ ok: true });
  const resolved = resolveTelegramFetchOrThrow(void 0, STICKY_IPV4_FALLBACK_NETWORK);
  await resolved("https://api.telegram.org/botx/sendMessage");
  await resolved("https://api.telegram.org/botx/sendChatAction");
}
function primeStickyFallbackRetry(code = "EHOSTUNREACH", successCount = 2) {
...
function expectStickyAutoSelectDispatcher(dispatcher, field = "connect") {
...
function expectCallerDispatcherPreserved(callIndexes, dispatcher) {
  for (const callIndex of callIndexes) {
    const callInit = undiciFetch.mock.calls[callIndex - 1]?.[1];
...
describe("resolveTelegramFetch", () => {
  ...
  it("uses resolver-scoped Agent dispatcher with configured transport policy", async () => {
    ...
    const resolved = resolveTelegramFetchOrThrow(void 0, {
      network: {
        autoSelectFamily: true,
  ...
  it("treats ALL_PROXY-only env as direct transport and arms sticky IPv4 fallback", async () => {
    ...
    undiciFetch.mockRejectedValueOnce(buildFetchFallbackError("EHOSTUNREACH")).mockResolvedValueOnce({ ok: true }).mockResolvedValueOnce({ ok: true });
    const transport = resolveTelegramTransport(void 0, {
      network: {
        autoSelectFamily: true,
        dnsResultOrder: "ipv4first"
      }
  ...
  it("preserves caller-provided dispatcher across fallback retry", async () => {
    ...
    undiciFetch.mockRejectedValueOnce(fetchError).mockResolvedValueOnce({ ok: true });
    const resolved = resolveTelegramFetchOrThrow(void 0, {
      network: {
        autoSelectFamily: true
      }
  ...
  it("keeps per-resolver transport policy isolated across multiple accounts", async () => {
    undiciFetch.mockResolvedValue({ ok: true });
    const resolverA = resolveTelegramFetchOrThrow(void 0, {
      network: {
        autoSelectFamily: false,
        dnsResultOrder: "ipv4first"
      }
    });
    const resolverB = resolveTelegramFetchOrThrow(void 0, {
      network: {
        autoSelectFamily: true,
        dnsResultOrder: "verbatim"
      }
