...
describe("isRecoverableTelegramNetworkError", () => {
	...
	it.each([
		...
		expect(
			isRecoverableTelegramNetworkError(errorWithCode(message, code)),
		).toBe(true);
	});
	...
	it("normalizes blank tagged origins to null and finds nested tags", () => {
		...
		expect(getTelegramNetworkErrorOrigin(outer)).toEqual({
			method: null,
			url: null,
		});
...
describe("isSafeToRetrySendError", () => {
	...
	it("detects pre-connect error nested in cause chain", () => {
		const root = Object.assign(new Error("ECONNREFUSED"), {
			code: "ECONNREFUSED",
		});
...
describe("isTelegramServerError", () => {
	it.each([
		...
		expect(
			isTelegramServerError(errorWithTelegramCode(message, errorCode)),
		).toBe(expected);
	});

	it("returns false for plain Error", () => {
		expect(isTelegramServerError(new Error("500: Internal Server Error"))).toBe(
			false,
		);
	});
});
...
describe("isTelegramClientRejection", () => {
	it.each([
		...
		expect(
			isTelegramClientRejection(errorWithTelegramCode(message, errorCode)),
		).toBe(expected);
	});

	it("returns false for plain Error", () => {
		expect(isTelegramClientRejection(new Error("400: Bad Request"))).toBe(
			false,
		);
	});
