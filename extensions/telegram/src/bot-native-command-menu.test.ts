...
type SyncMenuOptions = {
	...
	commandsToRegister: Parameters<
		typeof syncTelegramMenuCommands
	>[0]["commandsToRegister"];
	accountId: string;
	botIdentity: string;
...
describe("bot-native-command-menu", () => {
	...
	it("ignores malformed plugin specs without crashing", () => {
		const malformedSpecs = [
			...
			typeof buildPluginTelegramMenuCommands
		>[0]["specs"];
	...
	it.each([
		{
			label: "description envelope",
			error: { description: "BOT_COMMANDS_TOO_MUCH" },
		},
		{ label: "message envelope", error: { message: "BOT_COMMANDS_TOO_MUCH" } },
	])("retries when Telegram returns a plain-object $label error", async ({
		error,
	}) => {
		const deleteMyCommands = vi.fn(async () => undefined);
		const setMyCommands = vi
