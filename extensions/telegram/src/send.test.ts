...
async function expectTelegramMembershipErrorWithChatId(
	...
	try {
		...
		if (
			error instanceof Error &&
			error.message ===
				"Expected action to reject with membership error context"
		) {
			throw error;
		}
		const message = error instanceof Error ? error.message : String(error);
		expect(message).toMatch(
			/not a member of the chat, was blocked, or was kicked/i,
		);
...
describe("sent-message-cache", () => {
	...
	it("shares sent-message state across distinct module instances", async () => {
		const cacheA = await importFreshModule<
			typeof import("./sent-message-cache.js")
		>(import.meta.url, "./sent-message-cache.js?scope=shared-a");
		const cacheB = await importFreshModule<
			typeof import("./sent-message-cache.js")
		>(import.meta.url, "./sent-message-cache.js?scope=shared-b");
...
describe("buildInlineKeyboard", () => {
	it("normalizes keyboard inputs", () => {
		...
		for (const testCase of cases) {
			const input = testCase.input?.map((row) =>
				row.map((button) => ({ ...button })),
			);
			expect(buildInlineKeyboard(input), testCase.name).toEqual(
				testCase.expected,
			);
		}
	});
...
describe("sendMessageTelegram", () => {
			...
			expect(botCtorSpy, testCase.name).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					client: expect.objectContaining({
						timeoutSeconds: testCase.expectedTimeout,
					}),
				}),
			);
		}
	...
	it("keeps link_preview_options disabled for both html and plain-text fallback", async () => {
		...
		for (const testCase of cases) {
			...
			expect(testCase.sendMessage.mock.calls, testCase.name).toEqual(
	...
	it("applies reply markup and thread options to split video-note sends", async () => {
		...
		for (const testCase of cases) {
			...
			const sendOptions: NonNullable<
				Parameters<typeof sendMessageTelegram>[2]
			> = {
				token: "tok",
				api,
				mediaUrl: "https://example.com/video.mp4",
	...
	it.each([
		...
		expect(sendDocument, testCase.name).toHaveBeenCalledWith(
			chatId,
			expect.anything(),
			{
				caption: "caption",
	...
	])("sends images as documents when Telegram rejects $name", async ({
		width,
		height,
	}) => {
		const chatId = "123";
	...
	it("routes audio media to sendAudio/sendVoice based on voice compatibility", async () => {
		...
		for (const testCase of cases) {
			...
			const called =
				testCase.expectedMethod === "sendVoice" ? sendVoice : sendAudio;
			const notCalled =
				testCase.expectedMethod === "sendVoice" ? sendAudio : sendVoice;
	...
	it("keeps message_thread_id for forum/private/group sends", async () => {
		...
		for (const testCase of cases) {
			...
			expect(sendMessage, testCase.name).toHaveBeenCalledWith(
				testCase.chatId,
				testCase.text,
				{
					parse_mode: "HTML",
	...
	it("does not retry on non-retriable thread/chat errors", async () => {
		const cases: Array<{
			...
			expectedCallArgs: [
				string,
				string,
				{ parse_mode: "HTML"; message_thread_id?: number },
			];
		}> = [
			{
				chatId: "123",
	...
	it("preserves caller plain-text fallback across chunked html parse retries", async () => {
		...
		const plainFallbackCalls = [
			sendMessage.mock.calls[1],
			sendMessage.mock.calls[3],
		];
		expect(
			plainFallbackCalls.map((call) => String(call?.[1] ?? "")).join(""),
		).toBe(plainText);
		expect(
			plainFallbackCalls.every(
				(call) => !String(call?.[1] ?? "").includes("<"),
			),
		).toBe(true);
		expect(res.messageId).toBe("91");
	});
	...
	it("keeps malformed leading ampersands on the chunked plain-text fallback path", async () => {
		...
		expect(
			plainFallbackCalls.every((call) => String(call?.[1] ?? "").length > 0),
		).toBe(true);
	...
	it("cuts over to plain text when fallback text needs more chunks than html", async () => {
		...
		expect(
			sendMessage.mock.calls.every((call) => call[2]?.parse_mode === undefined),
		).toBe(true);
		expect(
			sendMessage.mock.calls.map((call) => String(call[1] ?? "")).join(""),
		).toBe(plainText);
...
describe("sendStickerTelegram", () => {
	...
	it("throws error when fileId is blank", async () => {
		for (const fileId of ["", "   "]) {
			await expect(
				sendStickerTelegram("123", fileId, { token: "tok" }),
	...
	it("retries sticker sends without message_thread_id when thread is missing", async () => {
		...
		expect(sendSticker).toHaveBeenNthCalledWith(
			2,
			chatId,
			"fileId123",
			undefined,
...
describe("shared send behaviors", () => {
	...
	it("omits invalid reply_to_message_id values before calling Telegram", async () => {
		const invalidReplyToMessageIds = [
			"session-meta-id",
			"123abc",
			Number.NaN,
		] as const;
...
describe("editMessageTelegram", () => {
	it.each([
		...
		},
		{
			...
			expectedCalls: 1,
			firstExpectReplyMarkup: { inline_keyboard: [] } as Record<
				string,
				unknown
			>,
			parseFallback: false,
		},
		{
			...
			firstExpectReplyMarkup: { inline_keyboard: [] } as Record<
				...
				unknown
			>,
			secondExpectReplyMarkup: { inline_keyboard: [] } as Record<
				string,
				unknown
			>,
			parseFallback: true,
		...
		await editMessageTelegram("123", 1, testCase.text, {
			...
			buttons: testCase.buttons
				? testCase.buttons.map((row) => [...row])
				: testCase.buttons,
		});

		expect(botCtorSpy, testCase.name).toHaveBeenCalledTimes(1);
		expect(botCtorSpy.mock.calls[0]?.[0], testCase.name).toBe("tok");
		expect(botApi.editMessageText, testCase.name).toHaveBeenCalledTimes(
			testCase.expectedCalls,
		);

		const firstParams = (botApi.editMessageText.mock.calls[0] ??
			[])[3] as Record<string, unknown>;
		expect(firstParams, testCase.name).toEqual(
			expect.objectContaining({ parse_mode: "HTML" }),
		);
		if (
			"firstExpectNoReplyMarkup" in testCase &&
			testCase.firstExpectNoReplyMarkup
		) {
			expect(firstParams, testCase.name).not.toHaveProperty("reply_markup");
		}
		if (
			"firstExpectReplyMarkup" in testCase &&
			testCase.firstExpectReplyMarkup
		) {
			expect(firstParams, testCase.name).toEqual(
				expect.objectContaining({
					reply_markup: testCase.firstExpectReplyMarkup,
				}),
			);
		}

		if (
			"secondExpectReplyMarkup" in testCase &&
			testCase.secondExpectReplyMarkup
		) {
			const secondParams = (botApi.editMessageText.mock.calls[1] ??
				[])[3] as Record<string, unknown>;
			expect(secondParams, testCase.name).toEqual(
				expect.objectContaining({
					reply_markup: testCase.secondExpectReplyMarkup,
				}),
			);
		}
	});
	...
	it("retries editMessageTelegram on Telegram 5xx errors", async () => {
		botApi.editMessageText
			.mockRejectedValueOnce(
				Object.assign(new Error("502: Bad Gateway"), { error_code: 502 }),
			)
			.mockResolvedValueOnce({ message_id: 1, chat: { id: "123" } });
...
describe("sendPollTelegram", () => {
	...
	it("retries without message_thread_id on thread-not-found", async () => {
		...
		expect(api.sendPoll.mock.calls[0]?.[3]).toMatchObject({
			message_thread_id: 99,
		});
		expect(
			(
				api.sendPoll.mock.calls[1]?.[3] as
...
describe("createForumTopicTelegram", () => {
	...
	for (const testCase of cases) {
		it(testCase.name, async () => {
			...
			const result = await createForumTopicTelegram(
				testCase.target,
				testCase.title,
				{
					token: "tok",
