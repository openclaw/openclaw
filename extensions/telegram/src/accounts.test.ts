...
function resolveAccountWithEnv(
	...
	return withEnv(env, () =>
		resolveTelegramAccount({ cfg, ...(accountId ? { accountId } : {}) }),
	);
}
...
beforeEach(() => {
	...
	vi.spyOn(runtimeEnvModule, "createSubsystemLogger").mockImplementation(() => {
		...
		return logger as unknown as ReturnType<
			typeof runtimeEnvModule.createSubsystemLogger
		>;
	});
});
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
	...
	const createDefaultAccountGroupsConfig = (
		includeDevAccount: boolean,
	): OpenClawConfig => ({
		channels: {
			telegram: {
