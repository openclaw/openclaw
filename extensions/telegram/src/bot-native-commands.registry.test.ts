...
describe("registerTelegramNativeCommands real plugin registry", () => {
	beforeAll(async () => {
		...
		({ clearPluginCommands, registerPluginCommand } = await import(
			"../../../src/plugins/commands.js"
		));
		({ registerTelegramNativeCommands } = await import(
	...
	it("registers and executes plugin commands through the real plugin registry", async () => {
		const { bot, commandHandlers, sendMessage, setMyCommands } =
			createCommandBot();
	...
	it("round-trips Telegram native aliases through the real plugin registry", async () => {
		...
		expect(registeredCommands).toEqual(
			expect.arrayContaining([
				{ command: "pair_device", description: "Pair device" },
			]),
		);
		...
		await handler?.(
			createPrivateCommandContext({ match: "now", messageId: 2 }),
		);
	...
	it("allows requireAuth:false plugin commands for unauthorized senders through the real registry", async () => {
		...
		registerTelegramNativeCommands({
			...createNativeCommandTestParams({
				commands: {
					allowFrom: { telegram: ["999"] },
				} as OpenClawConfig["commands"],
			}),
			bot,
			allowFrom: ["999"],
			nativeEnabled: false,
