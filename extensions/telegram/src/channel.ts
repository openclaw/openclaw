...
function resolveTelegramProbe() {
	return (
		...
		probeModule.probeTelegram
	);
}

function resolveTelegramAuditCollector() {
	return (
		getOptionalTelegramRuntime()?.channel?.telegram
...
function resolveTelegramAuditMembership() {
	return (
		getOptionalTelegramRuntime()?.channel?.telegram
			?.auditTelegramGroupMembership ?? auditModule.auditTelegramGroupMembership
	);
}
...
const telegramMessageActions: ChannelMessageActionAdapter = {
	describeMessageTool: (ctx) =>
		getOptionalTelegramRuntime()?.channel?.telegram?.messageActions?.describeMessageTool?.(
			ctx,
		) ??
		telegramMessageActionsImpl.describeMessageTool?.(ctx) ??
		null,
	extractToolSend: (ctx) =>
		getOptionalTelegramRuntime()?.channel?.telegram?.messageActions?.extractToolSend?.(
			ctx,
		) ??
	...
	handleAction: async (ctx) => {
		const runtimeHandleAction =
			getOptionalTelegramRuntime()?.channel?.telegram?.messageActions
				?.handleAction;
...
function matchTelegramAcpConversation(params: {
	...
	const binding = normalizeTelegramAcpConversationId(
...
function resolveTelegramOutboundSessionRoute(params: {
	...
	const resolvedThreadId =
		parsed.messageThreadId ?? parseTelegramThreadId(fallbackThreadId);
	const isGroup =
...
const telegramNativeApprovalAdapter =
...
const resolveTelegramAllowlistGroupOverrides =
	createNestedAllowlistOverrideResolver({
...
export const telegramPlugin = createChatChannelPlugin({
	base: {
		...
		messaging: {
			...
			resolveOutboundSessionRoute: (params) =>
				resolveTelegramOutboundSessionRoute(params),
			targetResolver: {
		...
		lifecycle: {
			onAccountConfigChanged: async ({ prevCfg, nextCfg, accountId }) => {
				const previousToken = resolveTelegramAccount({
					cfg: prevCfg,
					accountId,
				}).token.trim();
				const nextToken = resolveTelegramAccount({
					cfg: nextCfg,
					accountId,
				}).token.trim();
				if (previousToken !== nextToken) {
					const { deleteTelegramUpdateOffset } = await import(
		...
		>({
			...
			resolveAccountSnapshot: ({ account, cfg, runtime, audit }) => {
				...
				const duplicateTokenReason = ownerAccountId
					...
					: null;
				const configured =
					(configuredFromStatus ?? Boolean(account.token?.trim())) &&
					!ownerAccountId;
				const groups =
					cfg.channels?.telegram?.accounts?.[account.accountId]?.groups ??
					cfg.channels?.telegram?.groups;
	...
	outbound: {
		base: {
			...
			shouldSkipPlainTextSanitization: ({ payload }) =>
