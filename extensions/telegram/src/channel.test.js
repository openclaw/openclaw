...
function installGatewayRuntime(params) {
  const monitorTelegramProvider = vi.fn(async () => void 0);
  const probeTelegram = vi.fn(
    async () => params?.probeOk ? { ok: true, bot: { username: params.botUsername ?? "bot" } } : { ok: false }
  );
...
function configureOpsProxyNetwork(cfg) {
...
function installSendMessageRuntime(sendMessageTelegram) {
  setTelegramRuntime({
    channel: {
      telegram: {
        sendMessageTelegram
...
describe("telegramPlugin duplicate token guard", () => {
  ...
  it("passes account proxy and network settings into Telegram probes", async () => {
    ...
    expect(probeTelegram).toHaveBeenCalledWith("token-ops", 5e3, {
      accountId: "ops",
  ...
  it("sends outbound payload media lists and keeps buttons on the first message only", async () => {
    const sendMessageTelegram = installSendMessageRuntime(
      vi.fn().mockResolvedValueOnce({ messageId: "tg-3", chatId: "12345" }).mockResolvedValueOnce({ messageId: "tg-4", chatId: "12345" })
    );
    const result = await telegramPlugin.outbound.sendPayload({
      cfg: createCfg(),
      to: "12345",
    ...
    expect(
      sendMessageTelegram.mock.calls[1]?.[2]?.buttons
    ).toBeUndefined();
