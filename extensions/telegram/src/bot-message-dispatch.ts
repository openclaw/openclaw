...
function resolveTelegramReasoningLevel(params: {
	...
	try {
		...
		const store = (telegramDeps.loadSessionStore ?? loadSessionStore)(
			storePath,
			{
				skipCache: true,
			},
...
export const dispatchTelegramMessage = async ({
	...
	const createDraftLane = (
		...
		const stream = enabled
			? (telegramDeps.createTelegramDraftStream ?? createTelegramDraftStream)({
					...
					thread: threadSpec,
					previewTransport: useMessagePreviewTransportForDm
						? "message"
						: "auto",
	...
	const ingestDraftLaneSegments = async (text: string | undefined) => {
		...
		const hasAnswerSegment = split.segments.some(
		...
		if (
			...
			skipNextAnswerMessageStartRotation =
	...
	const emitPreviewFinalizedHook = (result: LaneDeliveryResult) => {
		...
		(telegramDeps.emitInternalMessageSentHook ?? emitInternalMessageSentHook)({
			sessionKeyForInternalHooks:
				deliveryBaseOptions.sessionKeyForInternalHooks,
	...
	const isDmTopic =
		!isGroup && threadSpec.scope === "dm" && threadSpec.id != null;
	...
	if (isDmTopic) {
		try {
			...
			if (sessionKey) {
				...
				logVerbose(
					"auto-topic-label: SessionKey is absent, skipping first-turn detection",
				);
			}
		} catch (err) {
			logVerbose(
		...
		for (const laneState of lanesToCleanup) {
			...
			const hasBoundaryFinalizedActivePreview =
				...
				archivedAnswerPreviews.some(
					(p) =>
						p.deleteIfUnused === false &&
						p.messageId === activePreviewMessageId,
				);
			const shouldClear =
				!retainPreviewOnCleanupByLane[laneState.laneName] &&
	...
	if (
		...
		const result = await (telegramDeps.deliverReplies ?? deliverReplies)({
			...
			silent:
				silentErrorReplies &&
				(dispatchError != null || hadErrorReplyFailureOrSkip),
			mediaLoader: telegramDeps.loadWebMedia,
		});
	...
	if (isDmTopic && isFirstTurnInSession) {
		const userMessage = (ctxPayload.RawBody ?? ctxPayload.Body ?? "").slice(
			0,
			500,
		);
		if (userMessage.trim()) {
			...
			if (autoTopicConfig) {
				...
				void (async () => {
					try {
						...
						logVerbose(
							`auto-topic-label: generated label (len=${label.length})`,
						);
						await bot.api.editForumTopic(chatId, topicThreadId, {
							name: label,
						});
						logVerbose(
							`auto-topic-label: renamed topic ${chatId}/${topicThreadId}`,
						);
					} catch (err) {
						logVerbose(
