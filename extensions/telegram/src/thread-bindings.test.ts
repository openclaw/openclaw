...
describe("telegram thread bindings", () => {
	...
	it("registers a telegram binding adapter and binds current conversations", async () => {
		...
		expect(manager.getByConversationId("-100200300:topic:77")?.boundBy).toBe(
			"user-1",
		);
	});
	...
	it("shares binding state across distinct module instances", async () => {
		const bindingsA = await importFreshModule<
			typeof import("./thread-bindings.js")
		>(import.meta.url, "./thread-bindings.js?scope=shared-a");
		const bindingsB = await importFreshModule<
			typeof import("./thread-bindings.js")
		>(import.meta.url, "./thread-bindings.js?scope=shared-b");
	...
	it("updates lifecycle windows by session key", async () => {
		...
		expect(maxAgeUpdated[0]?.lastActivityAt).toBe(
			Date.parse("2026-03-06T12:00:00.000Z"),
		);
		expect(
			manager.listBySessionKey("agent:main:subagent:child-1")[0]?.maxAgeMs,
		).toBe(6 * 60 * 60 * 1000);
	});
	...
	it("does not persist lifecycle updates when manager persistence is disabled", async () => {
		stateDirOverride = fs.mkdtempSync(
			path.join(os.tmpdir(), "openclaw-telegram-bindings-"),
		);
