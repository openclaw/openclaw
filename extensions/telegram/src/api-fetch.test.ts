...
const EnvHttpProxyAgent =
	require("undici/lib/dispatcher/env-http-proxy-agent.js") as {
		new (opts?: Record<string, unknown>): Record<PropertyKey, unknown>;
	};
const { kHttpsProxyAgent, kNoProxyAgent } =
	require("undici/lib/core/symbols.js") as {
...
describe("undici env proxy semantics", () => {
	it("uses proxyTls rather than connect for proxied HTTPS transport settings", () => {
		...
		const noProxyAgent = withoutProxyTls[kNoProxyAgent] as Record<
			PropertyKey,
			unknown
		>;
		const httpsProxyAgent = withoutProxyTls[kHttpsProxyAgent] as Record<
			PropertyKey,
			unknown
		>;
		...
		expect(
			getOwnSymbolValue(httpsProxyAgent, "proxy tls settings"),
		).toBeUndefined();
		...
		const httpsProxyAgentWithProxyTls = withProxyTls[
			kHttpsProxyAgent
		] as Record<PropertyKey, unknown>;

		expect(
			getOwnSymbolValue(httpsProxyAgentWithProxyTls, "proxy tls settings"),
		).toEqual(expect.objectContaining(connect));
	});
});
