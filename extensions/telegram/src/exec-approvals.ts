...
function normalizeTelegramDirectApproverId(
	value: string | number,
): string | undefined {
...
export function isTelegramExecApprovalClientEnabled(params: {
	...
	return Boolean(
		config?.enabled && getTelegramExecApprovalApprovers(params).length > 0,
	);
}
...
export function isTelegramExecApprovalTargetRecipient(params: {
	...
	return targets.some((target) => {
		...
		if (
			accountId &&
			target.accountId &&
			normalizeAccountId(target.accountId) !== accountId
		) {
			return false;
		}
