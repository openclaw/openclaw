...
> &
	Partial<
		Pick<DeliverRepliesParams, "replyToMode" | "textLimit" | "mediaLoader">
	>;
...
vi.mock("../../../../src/hooks/internal-hooks.js", async () => {
	const actual = await vi.importActual<
		typeof import("../../../../src/hooks/internal-hooks.js")
...
describe("deliverReplies", () => {
	...
	it("skips malformed replies and continues with valid entries", async () => {
		...
		const sendMessage = vi
			.fn()
			.mockResolvedValue({ message_id: 1, chat: { id: "123" } });
		const bot = createBot({ sendMessage });
		...
		await deliverWith({
			replies: [
				undefined,
				{ text: "hello" },
			] as unknown as DeliverRepliesParams["replies"],
			runtime,
			bot,
		});
	...
	it("passes accountId into message hooks", async () => {
		...
		const sendMessage = vi
			.fn()
			.mockResolvedValue({ message_id: 9, chat: { id: "123" } });
		const bot = createBot({ sendMessage });
	...
	it("does not emit internal message:sent without a session key", async () => {
		...
		const sendMessage = vi
			.fn()
			.mockResolvedValue({ message_id: 11, chat: { id: "123" } });
		const bot = createBot({ sendMessage });
	...
	it("keeps disable_notification on voice fallback text when silent is true", async () => {
		...
		const sendVoice = vi
			.fn()
			.mockRejectedValue(createVoiceMessagesForbiddenError());
		const sendMessage = vi.fn().mockResolvedValue({
			message_id: 5,
			chat: { id: "123" },
		});
	...
	it("continues when pinning fails", async () => {
		...
		const sendMessage = vi
			.fn()
			.mockResolvedValue({ message_id: 201, chat: { id: "123" } });
