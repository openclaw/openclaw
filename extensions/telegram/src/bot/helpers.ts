...
export async function resolveTelegramForumFlag(params: {
	...
	try {
		return (
			extractTelegramForumFlag(await params.getChat(params.chatId)) === true
		);
	} catch {
		return false;
	}
...
export async function resolveTelegramGroupAllowFromContext(params: {
	...
	const storeAllowFrom = await (
		params.readChannelAllowFromStore ?? readChannelAllowFromStore
	)("telegram", process.env, accountId).catch(() => []);
