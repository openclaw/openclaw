...
describe("resolveMedia getFile retry", () => {
	...
	it.each([
		...
		"photo",
		"video",
	] as const)("returns null for %s when getFile exhausts retries so message is not dropped", async (mediaField) => {
		const getFile = vi
			.fn()
	...
	it.each([
		"audio",
		"voice",
	] as const)("returns null for %s when file is too big", async (mediaField) => {
	...
	it("uses caller-provided fetch impl for file downloads", async () => {
		const getFile = vi
			.fn()
			.mockResolvedValue({ file_path: "documents/file_42.pdf" });
	...
	it("uses caller-provided fetch impl for sticker downloads", async () => {
		const getFile = vi
			.fn()
			.mockResolvedValue({ file_path: "stickers/file_0.webp" });
		const callerFetch = vi.fn() as unknown as typeof fetch;
		const callerTransport = { fetch: callerFetch, sourceFetch: callerFetch };
		fetchRemoteMedia.mockResolvedValueOnce({
			buffer: Buffer.from("sticker-data"),
	...
	it("uses local absolute file paths directly for media downloads", async () => {
		const getFile = vi
			.fn()
			.mockResolvedValue({ file_path: "/var/lib/telegram-bot-api/file.pdf" });
	...
	it("uses local absolute file paths directly for sticker downloads", async () => {
		const getFile = vi.fn().mockResolvedValue({
			file_path: "/var/lib/telegram-bot-api/sticker.webp",
		});
...
describe("resolveMedia original filename preservation", () => {
	...
	it("passes document.file_name to saveMediaBuffer instead of server-side path", async () => {
		...
		expect(result).toEqual(
			expect.objectContaining({ path: "/tmp/business-plan---uuid.pdf" }),
		);
	});
	...
	it("passes audio.file_name to saveMediaBuffer", async () => {
		const getFile = vi
			.fn()
			.mockResolvedValue({ file_path: "music/file_99.mp3" });
		fetchRemoteMedia.mockResolvedValueOnce({
	...
	it("passes video.file_name to saveMediaBuffer", async () => {
		const getFile = vi
			.fn()
			.mockResolvedValue({ file_path: "videos/file_55.mp4" });
		fetchRemoteMedia.mockResolvedValueOnce({
