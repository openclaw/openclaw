...
function createBot(sendMessage: ReturnType<typeof vi.fn>): Bot {
  // Upstream send path goes through the rich-message raw API
  // (`getTelegramRichRawApi(bot.api).sendRichMessage`), not `bot.api.sendMessage`.
  return { api: { raw: { sendRichMessage: sendMessage } } } as unknown as Bot;
}
