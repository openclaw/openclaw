...
function createRuntime(withLog = true) {
  return {
    error: vi.fn(),
...
function createBot(api = {}) {
  return { api };
}
async function deliverWith(params) {
  await deliverReplies({
    ...baseDeliveryParams,
    ...params
  });
}
function mockMediaLoad(fileName, contentType, data) {
...
describe("deliverReplies", () => {
  ...
  it("reports message_sent success=false when hooks blank out a text-only reply", async () => {
    messageHookRunner.hasHooks.mockImplementation(
      (name) => name === "message_sending" || name === "message_sent"
    );
  ...
  it("passes media metadata to message_sending hooks", async () => {
    messageHookRunner.hasHooks.mockImplementation((name) => name === "message_sending");
    const runtime = createRuntime(false);
  ...
  it("retries DM topic sends without message_thread_id when thread is missing", async () => {
    const runtime = createRuntime();
    const sendMessage = vi.fn().mockRejectedValueOnce(createThreadNotFoundError("sendMessage")).mockResolvedValueOnce({
      message_id: 7,
      chat: { id: "123" }
    });
    const bot = createBot({ sendMessage });
  ...
  it("falls back to plain text when markdown renders to empty HTML in threaded mode", async () => {
    const runtime = createRuntime();
    const sendMessage = vi.fn(async (_chatId, text) => {
      if (text === "") {
  ...
  it("pins the first delivered text message when telegram pin is requested", async () => {
    const runtime = createRuntime();
    const sendMessage = vi.fn().mockResolvedValueOnce({ message_id: 101, chat: { id: "123" } }).mockResolvedValueOnce({ message_id: 102, chat: { id: "123" } });
