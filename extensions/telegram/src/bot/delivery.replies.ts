...
function filterEmptyTelegramTextChunks<T extends { text: string }>(
	chunks: readonly T[],
): T[] {
...
async function deliverTextReply(params: {
	...
	const chunks = filterEmptyTelegramTextChunks(
		params.chunkText(params.replyText),
	);
	...
	for (const mediaUrl of params.mediaList) {
		...
		if (isGif) {
			...
			if (useVoice) {
				...
				try {
					await sendVoiceMedia(
						mediaParams,
						(err) => !isVoiceMessagesForbidden(err),
					);
				} catch (voiceErr) {
...
async function maybePinFirstDeliveredMessage(params: {
	...
	try {
		await params.bot.api.pinChatMessage(
			params.chatId,
			params.firstDeliveredMessageId,
			{
