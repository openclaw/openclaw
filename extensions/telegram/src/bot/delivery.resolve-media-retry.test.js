...
function makeCtx(mediaField, getFile, opts) {
  const msg = {
    message_id: 1,
    date: 0,
...
function setupTransientGetFileRetry() {
  const getFile = vi.fn().mockRejectedValueOnce(new Error("Network request for 'getFile' failed!")).mockResolvedValueOnce({ file_path: "voice/file_0.oga" });
...
function mockPdfFetchAndSave(fileName) {
  fetchRemoteMedia.mockResolvedValueOnce({
...
function createFileTooBigError() {
...
describe("resolveMedia getFile retry", () => {
  ...
  it("retries getFile for stickers on transient failure", async () => {
    const getFile = vi.fn().mockRejectedValueOnce(new Error("Network request for 'getFile' failed!")).mockResolvedValueOnce({ file_path: "stickers/file_0.webp" });
    fetchRemoteMedia.mockResolvedValueOnce({
