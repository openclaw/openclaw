...
function resolveTelegramFileName(
...
function resolveRequiredTelegramTransport(
...
function resolveOptionalTelegramTransport(
	transport?: TelegramTransport,
...
async function downloadAndSaveTelegramFile(params: {
	...
	const originalName =
		params.telegramFileName ?? fetched.fileName ?? params.filePath;
...
> {
	...
	try {
		...
		const cached = sticker.file_unique_id
			? getCachedSticker(sticker.file_unique_id)
			: null;
		if (cached) {
