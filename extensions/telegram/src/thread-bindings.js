...
const TELEGRAM_THREAD_BINDINGS_STATE_KEY = /* @__PURE__ */ Symbol.for("openclaw.telegramThreadBindingsState");
const threadBindingsState = resolveGlobalSingleton(
...
function createTelegramThreadBindingManager(params = {}) {
  ...
  const manager = {
    ...
    touchConversation: (conversationIdRaw, at) => {
      ...
      const existing2 = BINDINGS_BY_ACCOUNT_CONVERSATION.get(key);
      if (!existing2) {
        return null;
      }
      const nextRecord = {
  ...
  registerSessionBindingAdapter({
    ...
    listBySession: (targetSessionKeyRaw) => {
      ...
      return manager.listBySessionKey(targetSessionKey).map(
        (entry) => toSessionBindingRecord(entry, {
          idleTimeoutMs,
          maxAgeMs
        })
      );
