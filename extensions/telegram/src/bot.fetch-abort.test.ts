...
const { telegramBotRuntimeForTest } = await import(
	"./bot.create-telegram-bot.test-harness.js"
);
const {
...
function createWrappedTelegramClientFetch(proxyFetch: typeof fetch) {
	...
	const clientFetch = (
		botCtorSpy.mock.calls.at(-1)?.[1] as { client?: { fetch?: unknown } }
	)?.client?.fetch as (
...
describe("createTelegramBot fetch abort", () => {
		...
		const fetchSpy = vi.fn(
			(_input: RequestInfo | URL, init?: RequestInit) =>
				new Promise<AbortSignal>((resolve) => {
					...
					signal.addEventListener("abort", () => resolve(signal), {
						once: true,
					});
				}),
		);
	...
	it("tags wrapped Telegram fetch failures with the Bot API method", async () => {
		...
		await expect(
			clientFetch("https://api.telegram.org/bot123456:ABC/getUpdates"),
