...
type LoadConfigFn = typeof import("mullusi/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("mullusi/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("mullusi/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("mullusi/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/mullusi-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
function createModelsProviderDataFromConfig(cfg: MullusiConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: MullusiConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: MullusiConfig) => {
