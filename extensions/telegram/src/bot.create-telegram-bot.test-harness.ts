...
type ResolveStorePathFn =
	typeof import("openclaw/plugin-sdk/config-runtime").resolveStorePath;
...
const { readChannelAllowFromStore, upsertChannelPairingRequest } = vi.hoisted(
	(): {
		readChannelAllowFromStore: MockFn<
...
vi.doMock(
	"openclaw/plugin-sdk/conversation-runtime.js",
	async (importOriginal) => {
		const actual =
			await importOriginal<
				typeof import("openclaw/plugin-sdk/conversation-runtime")
...
async function dispatchHarnessReplies(
	...
	const dispatcher = createReplyDispatcher({
		...
		enableSlackInteractiveReplies:
			params.dispatcherOptions.enableSlackInteractiveReplies,
		responsePrefixContextProvider:
			params.dispatcherOptions.responsePrefixContextProvider,
		...
		vi.fn<DispatchReplyWithBufferedBlockDispatcherFn>(
			async (params: DispatchReplyHarnessParams) =>
				await dispatchHarnessReplies(params, async (dispatchParams) => {
					return await replySpyHoisted.replySpy(
						dispatchParams.ctx,
...
export const listSkillCommandsForAgents =
	skillCommandListHoisted.listSkillCommandsForAgents;
const buildModelsProviderData =
	modelProviderDataHoisted.buildModelsProviderData;
...
const systemEventsHoisted = vi.hoisted(() => ({
	enqueueSystemEventSpy: vi.fn<TelegramBotDeps["enqueueSystemEvent"]>(
		() => false,
	),
}));
export const enqueueSystemEventSpy: MockFn<
	TelegramBotDeps["enqueueSystemEvent"]
...
const grammySpies = vi.hoisted(() => ({
	...
	botCtorSpy: vi.fn(
		(_: string, __?: { client?: { fetch?: typeof fetch } }) => undefined,
	),
	...
	editMessageReplyMarkupSpy: vi.fn(async () => ({
		message_id: 88,
	})) as AnyAsyncMock,
...
export const answerCallbackQuerySpy: AnyAsyncMock =
	grammySpies.answerCallbackQuerySpy;
...
export const editMessageReplyMarkupSpy: AnyAsyncMock =
	grammySpies.editMessageReplyMarkupSpy;
export const sendMessageDraftSpy: AnyAsyncMock =
	grammySpies.sendMessageDraftSpy;
export const setMessageReactionSpy: AnyAsyncMock =
	grammySpies.setMessageReactionSpy;
...
export const telegramBotDepsForTest: TelegramBotDeps = {
	...
	enqueueSystemEvent:
		enqueueSystemEventSpy as TelegramBotDeps["enqueueSystemEvent"],
	dispatchReplyWithBufferedBlockDispatcher,
...
beforeEach(() => {
	...
	runnerHoisted.sequentializeMiddleware.mockImplementation(
		async (_ctx, next) => {
			if (typeof next === "function") {
				await next();
			}
	...
	sequentializeSpy.mockImplementation(
		() => runnerHoisted.sequentializeMiddleware,
	);
