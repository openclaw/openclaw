...
function resolveCommandHashPath(
	accountId?: string,
	botIdentity?: string,
): string {
	const stateDir = resolveStateDir(process.env, os.homedir);
	const normalizedAccount =
		accountId?.trim().replace(/[^a-z0-9._-]+/gi, "_") || "default";
	const botHash = hashBotIdentity(botIdentity);
	return path.join(
		stateDir,
		"telegram",
		`command-hash-${normalizedAccount}-${botHash}.txt`,
	);
}
...
async function readCachedCommandHash(
	...
	try {
		return (
			await fs.readFile(resolveCommandHashPath(accountId, botIdentity), "utf-8")
		).trim();
	} catch {
		return null;
	}
...
export function syncTelegramMenuCommands(params: {
	...
	const sync = async () => {
		...
		if (commandsToRegister.length === 0) {
			if (!deleteSucceeded) {
				runtime.log?.(
					"telegram: deleteMyCommands failed; skipping empty-menu hash cache write",
				);
				return;
			}
