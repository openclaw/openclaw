...
// Permanent error: a stale bundled JS chunk cannot be regenerated by retrying
// the same update. See openclaw#92980 — without this classification the
// spooled-update retry loop released the claim and immediately re-picked the
// same update, saturating the gateway.
const MODULE_NOT_FOUND_ERROR_CODE = "ERR_MODULE_NOT_FOUND";
// Poison retry cap: after this many consecutive same-signature failures on a
// single update id, dead-letter it regardless of error type. Bounded so a
// transient classification gap (e.g. a new non-retryable error shape) still
// fails closed instead of looping the gateway. Reset on success.
const TELEGRAM_SPOOLED_UPDATE_POISON_RETRY_LIMIT = 3;
const TELEGRAM_SPOOLED_UPDATE_POISON_RETRY_DEFAULT_SIGNATURE = "unknown";
// Module-scoped retry counter for spooled update ids. Keyed by `updateId` so
// reorders and concurrent lanes cannot cross-contaminate poison state. Bounded
// to the live-claim set: once an update is dead-lettered or deleted, the entry
// is cleared. Cleared in bulk by the test helper.
const telegramSpooledUpdatePoisonState = new Map<number, { count: number; signature: string }>();
...
function resolveNonRetryableSpooledUpdateFailure(
  ...
  for (const candidate of collectErrorGraphCandidates(err, (current) => [
    ...
    if (extractErrorCode(candidate) === MODULE_NOT_FOUND_ERROR_CODE) {
      // Module-resolution failures never resolve on retry — the missing chunk
      // either reappears on reinstall or stays missing. Keep the classification
      // first inside the loop so a wrapped cause does not get masked by an
      // outer retryable error.
...
// Stable failure signature for poison-counter grouping. We only need a
// fingerprint that re-uses the same error kind on the same update, not a
// collision-resistant hash: collecting error name + first normalized cause
// message keeps related transient network blips grouped and unrelated ones
// split. Stays narrow on purpose so a real bug surfaces quickly in logs.
function signatureForTelegramSpooledUpdateFailure(
  err: unknown,
  nonRetryable: NonRetryableSpooledUpdateFailure | null,
): string {
  if (nonRetryable) {
    return nonRetryable.reason;
  }
  const candidates = collectErrorGraphCandidates(err, (current) => [current.cause, current.error]);
  ...
  return TELEGRAM_SPOOLED_UPDATE_POISON_RETRY_DEFAULT_SIGNATURE;
}

function recordTelegramSpooledUpdateFailure(params: {
  updateId: number;
  err: unknown;
  nonRetryable: NonRetryableSpooledUpdateFailure | null;
}): { count: number; signature: string } {
  const signature = signatureForTelegramSpooledUpdateFailure(params.err, params.nonRetryable);
  const previous = telegramSpooledUpdatePoisonState.get(params.updateId);
  if (previous && previous.signature === signature) {
    previous.count += 1;
    return { count: previous.count, signature };
  }
  const next = { count: 1, signature };
  telegramSpooledUpdatePoisonState.set(params.updateId, next);
  return next;
}

function clearTelegramSpooledUpdateFailureRecord(updateId: number): void {
  telegramSpooledUpdatePoisonState.delete(updateId);
}

function isTelegramSpooledUpdatePoisoned(count: number): boolean {
  return count >= TELEGRAM_SPOOLED_UPDATE_POISON_RETRY_LIMIT;
}
...
export class TelegramPollingSession {
  ...
  async #handleClaimedSpooledUpdate(params: {
    ...
    // Successful delivery clears the in-memory poison counter for this
    // update id. Without this, an update that succeeded once could still
    // dead-letter itself on a later transient hiccup counted against the
    // historical run.
    clearTelegramSpooledUpdateFailureRecord(params.update.updateId);
  ...
  async #releaseFailedSpooledUpdate(params: {
    ...
    // Failure classified as retryable — check the per-update poison counter
    // before requeueing. After N consecutive same-signature failures the
    // update is dead-lettered even though the error type is unknown to the
    // classifier. Without this, a new non-retryable error shape (or a
    // gap in the classifier) can reproduce the openclaw#92980 retry loop
    // and freeze the gateway.
    const poison = recordTelegramSpooledUpdateFailure({
      updateId: params.update.updateId,
      err: params.err,
      nonRetryable,
    });
    if (isTelegramSpooledUpdatePoisoned(poison.count)) {
      const poisonReason = "poison-retry-exceeded";
      const poisonMessage = `Telegram spooled update ${params.update.updateId} failed ${poison.count} consecutive times with signature "${poison.signature}"; dead-lettering with reason ${poisonReason} to break the retry loop.`;
      try {
        const failed = await failTelegramSpooledUpdateClaim({
          update: params.update,
          reason: poisonReason,
        ...
        if (!failed) {
          this.opts.log(
            `[telegram][diag] ${poisonMessage} No processing marker remained, falling back to release.`,
          );
        } else {
          clearTelegramSpooledUpdateFailureRecord(params.update.updateId);
          this.opts.log(
            `[telegram][diag] spooled update ${params.update.updateId} failed with non-retryable ${poisonReason}; dead-lettered: ${poisonMessage}`,
          );
          return;
        }
      } catch (failErr) {
        this.opts.log(
          `[telegram][diag] ${poisonMessage} Dead-letter attempt failed: ${formatErrorMessage(failErr)}`,
        );
      }
    }
    try {
...
export const testing = {
  ...
  resetTelegramSpooledUpdatePoisonStateForTests: (): void => {
    telegramSpooledUpdatePoisonState.clear();
  },
  getTelegramSpooledUpdatePoisonCountForTests: (updateId: number): number =>
    telegramSpooledUpdatePoisonState.get(updateId)?.count ?? 0,
