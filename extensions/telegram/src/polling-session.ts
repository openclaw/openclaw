...
export class TelegramPollingSession {
	...
	async #createPollingBot(): Promise<TelegramBot | undefined> {
		...
		try {
			...
			await this.#waitBeforeRetryOnRecoverableSetupError(
				err,
	...
	async #runPollingCycle(bot: TelegramBot): Promise<"continue" | "exit"> {
		...
		const _stopSequenceLogged = false;
		...
		bot.api.config.use(async (prev, method, payload, signal) => {
			...
			try {
				...
				lastGetUpdatesOutcome = Array.isArray(result)
					? `ok:${result.length}`
					: "ok";
				return result;
			} catch (err) {
		...
		const watchdog = setInterval(() => {
			...
			const idleElapsed =
				inFlightGetUpdates > 0
					? 0
					: now - (lastGetUpdatesFinishedAt ?? lastGetUpdatesAt);
			...
			if (
				...
				if (
					stallDiagLoggedAt &&
					now - stallDiagLoggedAt < POLL_STALL_THRESHOLD_MS / 2
				) {
					return;
				}
