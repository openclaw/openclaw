...
describe("recordOutboundMessageForPromptContext", () => {
  ...
  it("records forum and channel Direct Messages replies with distinct native scope bindings", async () => {
    const chatId = -1001;
    ...
    const cache = createPromptContextCache();
    const forum = await cache.get({ accountId: "default", chatId, messageId: "710" });
    const direct = await cache.get({ accountId: "default", chatId, messageId: "711" });
