...
function createHarness(params?: {
	...
	const answer =
		params?.answerStream ??
		createTestDraftStream({ messageId: params?.answerMessageId });
	...
	const flushDraftLane = vi
		.fn()
		.mockImplementation(async (lane: DraftLaneState) => {
			await lane.stream?.flush();
		});
	const stopDraftLane = vi
		.fn()
		.mockImplementation(async (lane: DraftLaneState) => {
			if (
				lane === lanes.answer &&
				params?.answerMessageIdAfterStop !== undefined
			) {
				(
					answer as { setMessageId?: (value: number | undefined) => void }
...
async function deliverFinalAnswer(
...
async function expectFinalPreviewRetained(params: {
	...
	const result = await deliverFinalAnswer(
		params.harness,
		params.text ?? HELLO_FINAL,
	);
...
describe("createLaneTextDeliverer", () => {
	...
	it("keeps stop-created preview when follow-up final edit fails", async () => {
		...
		expect(harness.log).toHaveBeenCalledWith(
			expect.stringContaining(
				"failed after stop flush; keeping existing preview",
			),
		);
	});
	...
	it("treats 'message is not modified' preview edit errors as delivered", async () => {
		...
		expect(harness.log).toHaveBeenCalledWith(
			expect.stringContaining(
				'edit returned "message is not modified"; treating as delivered',
			),
		);
	});
	...
	it("retains preview when an existing preview final edit fails with ambiguous error", async () => {
		...
		await expectFinalPreviewRetained({
			harness,
			expectedLogSnippet:
				"ambiguous error; keeping existing preview to avoid duplicate",
		});
		expect(harness.editPreview).toHaveBeenCalledTimes(1);
	});
	...
	it("falls back when Telegram reports the current final edit target missing", async () => {
		...
		await expectFinalEditFallbackToSend({
			...
			text: "Hello final",
			expectedLogSnippet:
				"edit target missing with no alternate preview; falling back",
		});
	});
	...
	it("keeps preview when the final edit times out after the request may have landed", async () => {
		...
		await expectFinalPreviewRetained({
			harness,
			expectedLogSnippet:
				"may have landed despite network error; keeping existing preview",
		});
	});
	...
	it("falls back to normal delivery when final text exceeds preview edit limit", async () => {
		...
		expect(harness.log).toHaveBeenCalledWith(
			expect.stringContaining("preview final too long"),
		);
	});
	...
	it("falls back to normal send when draft materialize returns no message id", async () => {
		...
		expect(harness.log).toHaveBeenCalledWith(
			expect.stringContaining(
				"draft preview materialize produced no message id",
			),
		);
	});
	...
	it("keeps the active preview when an archived final edit target is missing", async () => {
		...
		expect(harness.log).toHaveBeenCalledWith(
			expect.stringContaining(
				"edit target missing; keeping alternate preview without fallback",
			),
		);
	});
	...
	it("retains when sendMayHaveLanded is true and a prior preview was visible", async () => {
		...
		await expectFinalPreviewRetained({
			harness,
			expectedLogSnippet:
				"preview send may have landed despite missing message id",
		});
	});
