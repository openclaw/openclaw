...
type TelegramEditMessageTextParams = Parameters<
	TelegramApi["editMessageText"]
>[3];
type TelegramCreateForumTopicParams = NonNullable<
	Parameters<TelegramApi["createForumTopic"]>[2]
>;
...
function splitTelegramPlainTextFallback(
	text: string,
	chunkCount: number,
	limit: number,
): string[] {
...
function buildTelegramClientOptionsCacheKey(params: {
	...
	const autoSelectFamilyKey =
		typeof autoSelectFamily === "boolean"
			? String(autoSelectFamily)
			: "default";
	const dnsResultOrderKey =
		params.account.config.network?.dnsResultOrder ?? "default";
...
function setCachedTelegramClientOptions(
	...
	if (
		telegramClientOptionsCache.size > MAX_TELEGRAM_CLIENT_OPTIONS_CACHE_SIZE
	) {
...
function buildTelegramThreadReplyParams(params: {
	...
	const threadParams: TelegramThreadReplyParams = threadIdParams
		? { ...threadIdParams }
		: {};
...
export async function sendMessageTelegram(
	...
	const buildChunkedTextPlan = (
		...
		try {
			...
			return splitTelegramPlainTextChunks(fallbackText, 4000).map(
				(plainText) => ({ plainText }),
			);
		}
	...
	const sendChunkedText = async (rawText: string, context: string) =>
		await sendTelegramTextChunks(
			buildChunkedTextPlan(rawText, context),
			context,
		);

	async function shouldSendTelegramImageAsPhoto(
		buffer: Buffer,
	): Promise<boolean> {
		try {
			...
			if (typeof width !== "number" || typeof height !== "number") {
				sendLogger.warn(
					"Photo dimensions are unavailable. Sending as document instead.",
				);
				return false;
			}
...
export async function sendTypingTelegram(
	...
	const threadParams = buildTypingThreadParams(
		target.messageThreadId ?? opts.messageThreadId,
	);
...
export async function unpinMessageTelegram(
	...
	const messageId =
		messageIdInput === undefined
			? undefined
			: normalizeMessageId(messageIdInput);
	const requestWithDiag = createTelegramRequestWithDiag({
		cfg,
		account,
		retry: opts.retry,
...
export async function editForumTopicTelegram(
	...
	if (trimmedName && trimmedName.length > 128) {
		throw new Error(
			"Telegram forum topic name must be 128 characters or fewer",
		);
	}
	...
	logVerbose(
		`[telegram] Edited forum topic ${messageThreadId} in chat ${chatId}`,
	);
	return {
		ok: true,
		chatId,
...
export async function renameForumTopicTelegram(
	...
	const result = await editForumTopicTelegram(
		chatIdInput,
...
export async function editMessageReplyMarkupTelegram(
	...
	logVerbose(
		`[telegram] Edited reply markup for message ${messageId} in chat ${chatId}`,
	);
	return { ok: true, messageId: String(messageId), chatId };
}
