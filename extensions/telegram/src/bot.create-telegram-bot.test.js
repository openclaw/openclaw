...
describe("createTelegramBot", () => {
  ...
  it("routes callback_query payloads as messages and answers callbacks", async () => {
    createTelegramBot({ token: "tok" });
    const callbackHandler = onSpy.mock.calls.find((call) => call[0] === "callback_query")?.[1];
  ...
  it("wraps inbound message with Telegram envelope", async () => {
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      ...
      expect(replySpy).toHaveBeenCalledTimes(1);
      const payload = replySpy.mock.calls[0][0];
      const expectedTimestamp = formatEnvelopeTimestamp(/* @__PURE__ */ new Date("2025-01-09T00:00:00Z"));
  ...
  it("blocks unauthorized DM media before download and sends pairing reply", async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(
      async () => new Response(new Uint8Array([255, 216, 255, 0]), {
        status: 200,
  ...
  it("does not persist update offset past pending updates", async () => {
    ...
    const middlewares = middlewareUseSpy.mock.calls.map((call) => call[0]).filter((fn) => typeof fn === "function");
    const runMiddlewareChain = async (ctx, finalNext) => {
      let idx = -1;
  ...
  it("threads forum replies only when a topic id exists", async () => {
    const threadCases = [
      { name: "General topic reply", threadId: void 0, expectedMessageThreadId: void 0 },
  ...
  it("honors threaded replies for replyToMode=first/all", async () => {
    for (const [mode, messageId] of [
      ...
      for (const [index, call] of sendMessageSpy.mock.calls.entries()) {
        const actual = call[2]?.reply_to_message_id;
        if (mode === "all" || index === 0) {
  ...
  it("buffers channel_post media groups and processes them together", async () => {
    ...
    try {
      createTelegramBot({ token: "tok", testTimings: TELEGRAM_TEST_TIMINGS });
      const handler = getOnHandler("channel_post");
      const first = handler({
        channelPost: {
          chat: { id: -100777111222, type: "channel", title: "Wake Channel" },
          message_id: 201,
      ...
      const flushTimer = flushTimerCallIndex >= 0 ? setTimeoutSpy.mock.calls[flushTimerCallIndex]?.[0] : void 0;
      if (flushTimerCallIndex >= 0) {
        clearTimeout(
          setTimeoutSpy.mock.results[flushTimerCallIndex]?.value
        );
      }
  ...
  it("notifies users when media download fails for direct messages", async () => {
    ...
    replySpy.mockClear();
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(
      async () => Promise.reject(new Error("MediaFetchError: Failed to fetch media"))
    );
    try {
      createTelegramBot({ token: "tok" });
      const handler = getOnHandler("message");
