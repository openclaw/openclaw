...
function resolveAccount(
	...
	return telegramPlugin.config.resolveAccount(
		cfg,
		accountId,
...
function startTelegramAccount(cfg: OpenClawConfig, accountId: string) {
	return telegramPlugin.gateway?.startAccount?.(
		createStartTelegramContext(cfg, accountId),
	);
}
	...
	const probeTelegram = vi
		...
		.mockImplementation(async () =>
			params?.probeOk
				? {
						ok: true,
						bot: { username: params.botUsername ?? "bot" },
						elapsedMs: 0,
					}
...
describe("telegramPlugin duplicate token guard", () => {
	...
	it("blocks startup for duplicate token accounts before polling starts", async () => {
		...
		await expect(startTelegramAccount(cfg, "work")).rejects.toThrow(
	...
	it("preserves accountId for pairing approval sends", async () => {
		...
		const resolveTelegramToken = vi.fn(() => ({
			token: "token-ops",
			source: "config",
		}));
		const cfg = createCfg();
	...
	it("sends outbound payload media lists and keeps buttons on the first message only", async () => {
		...
		expect(
			(sendMessageTelegram.mock.calls[1]?.[2] as Record<string, unknown>)
				?.buttons,
		).toBeUndefined();
	...
	it("reports not configured for unknown binding-created accountId in multi-bot setup", async () => {
		...
		expect(await telegramPlugin.config.isConfigured?.(account, cfg)).toBe(
			false,
		);
		expect(telegramPlugin.config.unconfiguredReason?.(account, cfg)).toContain(
			"unknown accountId",
		);
	});
