...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
	...
	it("registers only allowlisted skills for the bound agent menu", async () => {
		...
		const actualSkillCommands = await import(
			"../../../src/auto-reply/skill-commands.js"
		);
		...
		expect(
			registeredCommands.some((entry) => entry.command === "alpha_skill"),
		).toBe(true);
		expect(
			registeredCommands.some((entry) => entry.command === "beta_skill"),
		).toBe(false);
	});
});
