...
const TELEGRAM_WEBHOOK_RATE_LIMIT_BURST =
	WEBHOOK_RATE_LIMIT_DEFAULTS.maxRequests + 10;
...
async function postWebhookHeadersOnly(params: {
	...
	return await new Promise((resolve, reject) => {
		...
		const timeout = setTimeout(() => {
			req.destroy(
				new Error(
					`webhook header-only post timed out after ${params.timeoutMs ?? 5_000}ms`,
				),
			);
		...
		await withStartedWebhook(
			...
			async () => {
				...
				if (
					certificate &&
					"path" in certificate &&
					typeof certificate.path === "string"
				) {
