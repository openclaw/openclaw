...
export function buildTelegramNativeCommandCallbackData(
	commandText: string,
): string {
	return `${TELEGRAM_NATIVE_COMMAND_CALLBACK_PREFIX}${commandText}`;
}

export function parseTelegramNativeCommandCallbackData(
	data?: string | null,
): string | null {
	if (!data) {
		return null;
	...
	const commandText = trimmed
		.slice(TELEGRAM_NATIVE_COMMAND_CALLBACK_PREFIX.length)
		.trim();
...
async function resolveTelegramCommandAuth(params: {
	...
	const requireTopic = (groupConfig as TelegramDirectConfig | undefined)
		?.requireTopic;
	if (!isGroup && requireTopic === true && dmThreadId == null) {
		logVerbose(
			`Blocked telegram command in DM ${chatId}: requireTopic=true but no topic present`,
		);
		return null;
	}
	...
	const commandsAllowFromConfigured =
		...
		(Array.isArray(commandsAllowFrom.telegram) ||
...
export const registerTelegramNativeCommands = ({
	...
	const reservedCommands = new Set(
		listNativeCommandSpecs().map((command) =>
	...
	const loadFreshRuntimeConfig = (): OpenClawConfig =>
		telegramDeps.loadConfig();
	const resolveFreshTelegramConfig = (
	...
	const allCommandsFull: Array<{ command: string; description: string }> = [
		...nativeCommands
			...
			.filter(
				(cmd): cmd is { command: string; description: string } => cmd !== null,
			),
		...
		for (const command of nativeCommands) {
			...
			bot.command(
				...
				async (ctx: TelegramNativeCommandContext) => {
					...
					const commandDefinition = findCommandByNativeName(
						command.name,
						"telegram",
					);
					...
					const dmThreadId =
						threadSpec.scope === "dm" ? threadSpec.id : undefined;
					const threadKeys =
						dmThreadId != null
							? resolveThreadSessionKeys({
									baseSessionKey,
					...
					await recordInboundSessionMetaSafe({
						...
						onError: (err) =>
							runtime.error?.(
								danger(
									`telegram slash: failed updating session meta: ${String(err)}`,
								),
							),
					});
						...
						dispatcherOptions: {
							...
							deliver: async (payload, _info) => {
								...
								const result = await deliverReplies({
									...
									...deliveryBaseOptions,
									silent:
										runtimeTelegramCfg.silentErrorReplies === true &&
