...
describe("probeTelegram retry logic", () => {
	...
	const installFetchMock = (): Mock => {
		...
		resolveTelegramFetch.mockImplementation(
			(proxyFetch?: typeof fetch) => proxyFetch ?? fetch,
		);
		makeProxyFetch.mockImplementation(
	...
	beforeAll(async () => {
		vi.resetModules();
		({ probeTelegram, resetTelegramProbeFetcherCacheForTests } = await import(
			"./probe.js"
		));
	});
	...
	])("succeeds after retry pattern %#", async ({
		errors,
		expectedCalls,
		retryCount,
	}) => {
