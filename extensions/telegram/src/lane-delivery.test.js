...
import { createLaneTextDeliverer } from "./lane-delivery.js";
...
function createHarness(params) {
  ...
  const flushDraftLane = vi.fn().mockImplementation(async (lane) => {
    await lane.stream?.flush();
  });
  const stopDraftLane = vi.fn().mockImplementation(async (lane) => {
    if (lane === lanes.answer && params?.answerMessageIdAfterStop !== void 0) {
      answer.setMessageId?.(
  ...
  const editPreview = vi.fn().mockResolvedValue(void 0);
  const deletePreviewMessage = vi.fn().mockResolvedValue(void 0);
  const log = vi.fn();
...
async function deliverFinalAnswer(harness, text) {
...
function seedArchivedAnswerPreview(harness) {
...
describe("createLaneTextDeliverer", () => {
  ...
  it("falls back to normal send when draft materialize returns no message id", async () => {
    ...
    answerStream.materialize.mockResolvedValue(void 0);
    const harness = createHarness({
      answerStream,
      answerHasStreamedMessage: true,
      answerLastPartialText: "Hello final"
  ...
  it("retains when sendMayHaveLanded is true and a prior preview was visible", async () => {
    ...
    harness.stopDraftLane.mockImplementation(async (lane) => {
      stream.setMessageId(void 0);
      await lane.stream?.stop();
    });
    await expectFinalPreviewRetained({
      harness,
