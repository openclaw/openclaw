...
describe("buildTelegramMessageContext per-topic agentId routing", () => {
  ...
  beforeEach(() => {
    vi.mocked(loadConfig).mockReturnValue(defaultRouteConfig);
  });
  ...
  it("different topics route to different agents", async () => {
    const buildForTopic = async (threadId, agentId) => await buildForumContext({ threadId, topicConfig: { agentId } });
