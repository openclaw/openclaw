...
export async function deliverTelegramTextPage<TPlain, THtml, TRich>(params: {
  ...
  warn: (message: string) => void;
  sender: {
    sendPlain: (text: string, fallback?: { index: number; count: number }) => Promise<TPlain>;
  ...
  if (page.richMessage) {
    ...
    try {
      return [{ result: await params.sender.sendRich(page.richMessage), page }];
    } catch (error) {
      const fallback = buildTelegramPlainFallbackPlan({
      ...
      for (let index = 0; index < fallback.chunks.length; index += 1) {
        const text = fallback.chunks[index] ?? "";
        delivered.push({
          result: await params.sender.sendPlain(text, {
            index,
            count: fallback.chunks.length,
          }),
  ...
  if (page.htmlText) {
    try {
      return [{ result: await params.sender.sendHtml(page.htmlText), page }];
    } catch (error) {
      ...
      return [
        {
          ...
          page: fallbackPage(page.plainText),
        },
      ];
    }
  }
