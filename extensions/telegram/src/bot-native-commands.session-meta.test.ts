...
const persistentBindingMocks = vi.hoisted(() => ({
	resolveConfiguredBindingRoute: vi.fn<ResolveConfiguredBindingRouteFn>(
		({ route }) => ({
			bindingResolution: null,
			route,
		}),
	),
	ensureConfiguredBindingRouteReady: vi.fn<EnsureConfiguredBindingRouteReadyFn>(
		async () => ({
			ok: true,
		}),
	),
...
vi.mock("openclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
	...
	return {
		...
		resolveConfiguredBindingRoute:
			persistentBindingMocks.resolveConfiguredBindingRoute,
		ensureConfiguredBindingRouteReady:
			persistentBindingMocks.ensureConfiguredBindingRouteReady,
		recordInboundSessionMetaSafe: vi.fn(
			async (params: {
				...
				const storePath = sessionMocks.resolveStorePath(
		...
		upsertChannelPairingRequest:
			conversationStoreMocks.upsertChannelPairingRequest,
		getSessionBindingService: () => ({
			bind: vi.fn(),
			getCapabilities: vi.fn(),
			listBySession: vi.fn(),
			resolveByConversation: (ref: unknown) =>
				sessionBindingMocks.resolveByConversation(ref),
			touch: (bindingId: string, at?: number) =>
				sessionBindingMocks.touch(bindingId, at),
			unbind: vi.fn(),
		}),
	};
});
...
vi.mock("../../../src/config/sessions.js", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("../../../src/config/sessions.js")>();
	return {
		...actual,
...
function registerAndResolveCommandHandlerBase(params: {
	...
	const telegramDeps: TelegramBotDeps = {
		...
		resolveStorePath:
			sessionMocks.resolveStorePath as TelegramBotDeps["resolveStorePath"],
		readChannelAllowFromStore: vi.fn(async () => []),
		upsertChannelPairingRequest: vi.fn(async () => ({
			code: "PAIRCODE",
			created: true,
...
function expectUnauthorizedNewCommandBlocked(
	sendMessage: ReturnType<typeof vi.fn>,
) {
	expect(
		replyMocks.dispatchReplyWithBufferedBlockDispatcher,
...
describe("registerTelegramNativeCommands — session metadata", () => {
	...
	beforeEach(() => {
		...
		persistentBindingMocks.resolveConfiguredBindingRoute.mockImplementation(
			({ route }) => createConfiguredBindingRoute(route, null),
		);
	...
	it("routes Telegram native commands through configured ACP topic bindings", async () => {
		...
		const dispatchCall = (
			replyMocks.dispatchReplyWithBufferedBlockDispatcher.mock
	...
	it.each([
		"new",
		"reset",
	] as const)("preserves the topic-qualified origin target for native /%s in forum topics", async (commandName) => {
