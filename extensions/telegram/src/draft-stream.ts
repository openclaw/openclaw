...
function hasNumericMessageThreadId(
	...
	return (
		...
		params !== null &&
		typeof (params as { message_thread_id?: unknown }).message_thread_id ===
			"number"
	);
}
 ...
 */
const TELEGRAM_DRAFT_STREAM_STATE_KEY = Symbol.for(
	"openclaw.telegramDraftStreamState",
);
...
function shouldFallbackFromDraftTransport(err: unknown): boolean {
	...
	return (
		DRAFT_METHOD_UNAVAILABLE_RE.test(text) ||
		DRAFT_CHAT_UNSUPPORTED_RE.test(text)
	);
}
...
export function createTelegramDraftStream(params: {
	...
	let streamDraftId = usesDraftTransport
		...
		: undefined;
	let previewTransport: "message" | "draft" = usesDraftTransport
		? "draft"
		: "message";
	let lastSentText = "";
	...
	const sendRenderedMessageWithThreadFallback = async (sendArgs: {
		...
		try {
			...
			const threadlessParams: TelegramSendMessageParams = {
				...(sendParams ?? {}),
			};
	...
	const sendMessageTransportPreview = async ({
		...
		let sent: Awaited<
			ReturnType<typeof sendRenderedMessageWithThreadFallback>
		>["sent"];
		try {
	...
	return {
		...
		forceNewMessage,
		sendMayHaveLanded: () =>
			messageSendAttempted && typeof streamMessageId !== "number",
	};
}
