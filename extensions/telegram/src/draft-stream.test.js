...
function createMockDraftApi(sendMessageImpl) {
  return {
...
function createForumDraftStream(api) {
  return createThreadedDraftStream(api, { id: 99, scope: "forum" });
}
function createThreadedDraftStream(api, thread) {
  return createDraftStream(api, { thread });
}
function createDraftStream(api, overrides = {}) {
...
function expectDmMessagePreviewViaSendMessage(api, text = "Hello") {
...
function createForceNewMessageHarness(params = {}) {
...
describe("createTelegramDraftStream", () => {
  ...
  it("omits message_thread_id for general topic id", async () => {
    ...
    stream.update("Hello");
    await vi.waitFor(() => expect(api.sendMessage).toHaveBeenCalledWith(123, "Hello", void 0));
  });
  ...
  it("retries DM message preview send without thread when thread is not found", async () => {
    const api = createMockDraftApi();
    api.sendMessage.mockRejectedValueOnce(new Error("400: Bad Request: message thread not found")).mockResolvedValueOnce({ message_id: 17 });
    const warn = vi.fn();
    const stream = createDraftStream(api, {
      thread: { id: 42, scope: "dm" },
    ...
    expect(api.sendMessage).toHaveBeenNthCalledWith(2, 123, "Hello", void 0);
    expect(warn).toHaveBeenCalledWith(
  ...
  it("retries materialize send without thread when dm thread lookup fails", async () => {
    const api = createMockDraftApi();
    api.sendMessage.mockRejectedValueOnce(new Error("400: Bad Request: message thread not found")).mockResolvedValueOnce({ message_id: 55 });
    const warn = vi.fn();
    const stream = createDraftStream(api, {
      thread: { id: 42, scope: "dm" },
      previewTransport: "draft",
  ...
  it("rotates draft_id when forceNewMessage races an in-flight DM draft send", async () => {
    ...
    const firstDraftSend = new Promise((resolve) => {
  ...
  it("shares draft-id allocation across distinct module instances", async () => {
    const draftA = await importFreshModule(
      import.meta.url,
      "./draft-stream.js?scope=shared-a"
    );
    const draftB = await importFreshModule(
      import.meta.url,
  ...
  it("creates new message after forceNewMessage is called", async () => {
    ...
    expect(api.sendMessage).toHaveBeenLastCalledWith(123, "After thinking", void 0);
  });
  it("sends first update immediately after forceNewMessage within throttle window", async () => {
    vi.useFakeTimers();
    try {
      const { api, stream } = createForceNewMessageHarness({ throttleMs: 1e3 });
      stream.update("Hello");
      await vi.waitFor(() => expect(api.sendMessage).toHaveBeenCalledTimes(1));
      stream.update("Hello edited");
      expect(api.editMessageText).not.toHaveBeenCalled();
      ...
      expect(api.sendMessage).toHaveBeenLastCalledWith(123, "Second message", void 0);
    } finally {
      vi.useRealTimers();
    }
  });
  it("does not rebind to an old message when forceNewMessage races an in-flight send", async () => {
    ...
    expect(api.sendMessage).toHaveBeenCalledTimes(2);
    expect(api.sendMessage).toHaveBeenNthCalledWith(2, 123, "Message B partial", void 0);
  ...
  async function expectSendMayHaveLandedStateAfterFirstFailure(error, expected) {
    const api = createMockDraftApi();
...
describe("draft stream initial message debounce", () => {
  ...
  function createDebouncedStream(api, minInitialChars = 30) {
    return createTelegramDraftStream({
      api,
      chatId: 123,
      minInitialChars
  ...
  describe("isFinal has highest priority", () => {
    it("sends immediately on stop() even with 1 character", async () => {
      ...
      await stream.stop();
      await stream.flush();
      expect(api.sendMessage).toHaveBeenCalledWith(123, "Y", void 0);
    });
    it("sends immediately on stop() with short sentence", async () => {
      ...
      await stream.stop();
      await stream.flush();
      expect(api.sendMessage).toHaveBeenCalledWith(123, "Ok.", void 0);
    });
  });
  ...
  describe("default behavior without debounce params", () => {
    it("sends immediately without minInitialChars set (backward compatible)", async () => {
      ...
      stream.update("Hi");
      await stream.flush();
      expect(api.sendMessage).toHaveBeenCalledWith(123, "Hi", void 0);
    });
  });
});
