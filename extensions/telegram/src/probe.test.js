...
describe("probeTelegram retry logic", () => {
  ...
  const installFetchMock = () => {
    ...
    resolveTelegramFetch.mockImplementation((proxyFetch) => proxyFetch ?? fetch);
    makeProxyFetch.mockImplementation(() => fetchMock);
    return fetchMock;
  };
  function mockGetMeSuccess(fetchMock) {
  ...
  function mockGetWebhookInfoSuccess(fetchMock) {
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: vi.fn().mockResolvedValue({ ok: true, result: { url: "" } })
    });
  }
  async function expectSuccessfulProbe(fetchMock, expectedCalls, retryCount = 0) {
    const probePromise = probeTelegram(token, timeoutMs);
    if (retryCount > 0) {
      await vi.advanceTimersByTimeAsync(retryCount * 1e3);
    }
