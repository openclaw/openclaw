...
describe("buildTelegramMessageContext named-account DM fallback", () => {
	...
	async function buildNamedAccountDmContext(
		accountId = "atlas",
		messageId = 1,
	) {
	...
	it("keeps named-account fallback lastRoute on the isolated DM session", async () => {
		...
		expect(getLastUpdateLastRoute()?.sessionKey).toBe(
			"agent:main:telegram:atlas:direct:814912386",
		);
	});
	...
	it("isolates sessions between named accounts that share the default agent", async () => {
		...
		expect(atlas?.ctxPayload?.SessionKey).toBe(
			"agent:main:telegram:atlas:direct:814912386",
		);
		expect(skynet?.ctxPayload?.SessionKey).toBe(
			"agent:main:telegram:skynet:direct:814912386",
		);
		expect(atlas?.ctxPayload?.SessionKey).not.toBe(
			skynet?.ctxPayload?.SessionKey,
		);
	});
	...
	it("keeps identity-linked peer canonicalization in the named-account fallback path", async () => {
		...
		expect(ctx?.ctxPayload?.SessionKey).toBe(
			"agent:main:telegram:atlas:direct:alice-shared",
		);
	});
