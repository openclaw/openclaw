...
async function withIsolatedStateDirAsync<T>(fn: () => Promise<T>): Promise<T> {
	const stateDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "openclaw-telegram-state-"),
	);
...
async function withConfigPathAsync<T>(
	cfg: unknown,
	fn: () => Promise<T>,
): Promise<T> {
...
describe("createTelegramBot", () => {
	...
	it("preserves same-chat reply order when a debounced run is still active", async () => {
		...
		replySpy.mockImplementation(
			async (ctx: MsgContext, opts?: GetReplyOptions) => {
		...
		const extractLatestDebounceFlush = () => {
			...
			clearTimeout(
				setTimeoutSpy.mock.results[debounceCallIndex]?.value as ReturnType<
					typeof setTimeout
				>,
			);
	...
	it("reloads callback model routing bindings without recreating the bot", async () => {
		const buildModelsProviderDataMock =
			telegramBotDepsForTest.buildModelsProviderData as unknown as ReturnType<
				typeof vi.fn
			>;
				...
				upsertChannelPairingRequest.mockImplementation(async () => {
					const result =
						testCase.pairingUpsertResults[
							Math.min(
								pairingUpsertCall,
								testCase.pairingUpsertResults.length - 1,
							)
						];
					pairingUpsertCall += 1;
					return result;
		...
		for (const testCase of cases) {
			await withConfigPathAsync(testCase.config, async () => {
				...
				expect(payload.SessionKey).toContain(
					testCase.expectedSessionKeyFragment,
				);
			});
		}
	});
