...
const isRecoverableTelegramNetworkErrorMock = vi.hoisted(() =>
	vi.fn(() => true),
);
...
vi.mock("./api-logging.js", () => ({
	withTelegramApiErrorLogging: async ({ fn }: { fn: () => Promise<unknown> }) =>
		await fn(),
}));
...
function expectTelegramBotTransportSequence(
	firstTransport: unknown,
	secondTransport: unknown,
) {
	expect(createTelegramBotMock).toHaveBeenCalledTimes(2);
	expect(createTelegramBotMock.mock.calls[0]?.[0]?.telegramTransport).toBe(
		firstTransport,
	);
	expect(createTelegramBotMock.mock.calls[1]?.[0]?.telegramTransport).toBe(
		secondTransport,
	);
}
...
describe("TelegramPollingSession", () => {
	...
	it("rebuilds the transport after a stalled polling cycle", async () => {
		...
		createTelegramBotMock
			.mockReturnValueOnce(firstBot)
			.mockReturnValueOnce(secondBot);
	...
	it("does not trigger stall restart when non-getUpdates API calls are active", async () => {
		...
		try {
			...
			if (apiMiddleware) {
				...
				await apiMiddleware(fakePrev, "sendMessage", {
					chat_id: 123,
					text: "hello",
				});
			}
	...
	it("does not trigger stall restart while a recent non-getUpdates API call is in-flight", async () => {
		...
		try {
			...
			if (apiMiddleware) {
				...
				const sendPromise = apiMiddleware(slowPrev, "sendMessage", {
					chat_id: 123,
					text: "hello",
				});
