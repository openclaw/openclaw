...
function resolvePreviewTarget(
	params: ResolvePreviewTargetParams,
...
export function createLaneTextDeliverer(params: CreateLaneTextDelivererParams) {
	...
	const canMaterializeDraftFinal = (
		...
		const hasPreviewButtons = Boolean(
		...
		if (infoKind === "final") {
			...
			if (
				...
				if (canMaterializeDraftFinal(lane, previewButtons)) {
					const materializedMessageId =
						await tryMaterializeDraftPreviewForFinal({
							lane,
							laneName,
							text,
						});
