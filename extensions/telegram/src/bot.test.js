...
const { listNativeCommandSpecs, listNativeCommandSpecsForConfig } = await import("../../../src/auto-reply/commands-registry.js");
const { loadSessionStore } = await import("../../../src/config/sessions.js");
const { normalizeTelegramCommandName } = await import("../../../src/config/telegram-custom-commands.js");
...
describe("createTelegramBot", () => {
  ...
  it("merges custom commands with native commands", async () => {
    ...
    const registered = setMyCommandsSpy.mock.calls.at(-1)?.[0];
    ...
    await callbackHandler({
      callbackQuery: {
        ...
        message: {
          ...
          message_id: 21,
          text: [
            "\u{1F9E9} Yep-needs approval again.",
            "",
            "Run:",
  ...
  it("isolates inbound debounce by DM topic thread id", async () => {
    ...
    try {
      ...
      expect(replySpy).not.toHaveBeenCalled();
      const debounceTimerIndexes = setTimeoutSpy.mock.calls.map((call, index) => ({ index, delay: call[1] })).filter((entry) => entry.delay === DEBOUNCE_MS).map((entry) => entry.index);
      ...
      for (const index of debounceTimerIndexes) {
        const flushTimer = setTimeoutSpy.mock.calls[index]?.[0];
        await flushTimer?.();
      }
      await vi.waitFor(() => {
        expect(replySpy).toHaveBeenCalledTimes(2);
      });
      const threadIds = replySpy.mock.calls.map((call) => call[0].MessageThreadId).toSorted((a, b) => (a ?? 0) - (b ?? 0));
  ...
  it("propagates forwarded origin from external_reply targets", async () => {
    ...
    expect(payload.ReplyToForwardedDate).toBe(5e5);
  ...
  it("sets command target session key for dm topic commands", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = commandSpy.mock.calls.find((call) => call[0] === "status")?.[1];
    if (!handler) {
  ...
  it("enqueues system event for reaction", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({
    ...
    expect(enqueueSystemEventSpy).toHaveBeenCalledWith(
      "Telegram reaction added: \u{1F44D} by Ada (@ada_bot) on msg 42",
      expect.objectContaining({
  ...
  it.each([
    ...
    },
    {
      ...
      reaction: {
        ...
        date: 1736380800,
        old_reaction: [],
        new_reaction: [{ type: "emoji", emoji: "\u{1F525}" }]
      },
      expectedEnqueueCalls: 0
    }
  ...
  it("allows reaction in all mode regardless of message sender", async () => {
    ...
    await handler({
      ...
      messageReaction: {
        ...
        date: 1736380800,
        old_reaction: [],
        new_reaction: [{ type: "emoji", emoji: "\u{1F389}" }]
      }
    });
    expect(enqueueSystemEventSpy).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEventSpy).toHaveBeenCalledWith(
      "Telegram reaction added: \u{1F389} by Ada on msg 99",
      expect.any(Object)
    );
  });
  ...
  it("enqueues one event per added emoji reaction", async () => {
    ...
    expect(enqueueSystemEventSpy.mock.calls.map((call) => call[0])).toEqual([
      "Telegram reaction added: \u{1F525} by Ada on msg 42",
      "Telegram reaction added: \u{1F389} by Ada on msg 42"
    ]);
  });
  it("routes forum group reactions to the general topic (thread id not available on reactions)", async () => {
    ...
    expect(enqueueSystemEventSpy).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEventSpy).toHaveBeenCalledWith(
      "Telegram reaction added: \u{1F525} by Bob (@bob_user) on msg 100",
      expect.objectContaining({
  ...
  it("uses correct session key for forum group reactions in general topic", async () => {
    ...
    await handler({
      ...
      messageReaction: {
        ...
        date: 1736380800,
        old_reaction: [],
        new_reaction: [{ type: "emoji", emoji: "\u{1F440}" }]
      }
    });
    expect(enqueueSystemEventSpy).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEventSpy).toHaveBeenCalledWith(
      "Telegram reaction added: \u{1F440} by Bob on msg 101",
      expect.objectContaining({
        sessionKey: expect.stringContaining("telegram:group:5678:topic:1"),
  ...
  it("uses correct session key for regular group reactions without topic", async () => {
    ...
    const eventOptions = enqueueSystemEventSpy.mock.calls[0]?.[1];
