...
function resolveRequestSessionTarget(params: {
	...
	return resolveExecApprovalSessionTarget({
		...
		turnSourceAccountId:
			params.request.request.turnSourceAccountId ?? undefined,
...
function resolveTelegramSourceTarget(params: {
	...
	const turnSourceChannel =
		params.request.request.turnSourceChannel?.trim().toLowerCase() || "";
	...
	if (turnSourceChannel === "telegram" && turnSourceTo) {
		if (
			turnSourceAccountId &&
			normalizeAccountId(turnSourceAccountId) !==
...
export class TelegramExecApprovalHandler {
	...
	constructor(
		...
		this.editReplyMarkup =
			deps.editReplyMarkup ?? editMessageReplyMarkupTelegram;
	}
	...
	async handleRequested(request: ExecApprovalRequest): Promise<void> {
		...
		for (const target of resolvedTargets) {
			try {
				...
				log.error(
					`telegram exec approvals: failed to send request ${request.id}: ${String(err)}`,
				);
			}
		}
