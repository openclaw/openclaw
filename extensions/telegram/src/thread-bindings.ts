...
export type TelegramThreadBindingManager = {
	...
	getByConversationId: (
		conversationId: string,
	) => TelegramThreadBindingRecord | undefined;
 ...
 */
const TELEGRAM_THREAD_BINDINGS_STATE_KEY = Symbol.for(
	"openclaw.telegramThreadBindingsState",
);
...
function toSessionBindingTargetKind(
	raw: TelegramBindingTargetKind,
): BindingTargetKind {
	return raw === "subagent" ? "subagent" : "session";
}

function toTelegramTargetKind(
...
function fromSessionBindingInput(params: {
	...
	if (
		typeof metadata.idleTimeoutMs === "number" &&
		Number.isFinite(metadata.idleTimeoutMs)
	) {
	...
	if (
		typeof metadata.maxAgeMs === "number" &&
...
function summarizeLifecycleForLog(
	...
	const idleLabel = formatThreadBindingDurationLabel(
		Math.max(0, Math.floor(idleTimeoutMs)),
	);
	const maxAgeLabel = formatThreadBindingDurationLabel(
	...
	try {
		...
		for (const entry of parsed.bindings) {
			...
			const targetSessionKey =
				typeof entry?.targetSessionKey === "string"
					? entry.targetSessionKey.trim()
					: "";
		...
		if (code !== "ENOENT") {
			logVerbose(
				`telegram thread bindings load failed (${accountId}): ${String(err)}`,
			);
		}
		return [];
	}
...
function listBindingsForAccount(
	accountId: string,
): TelegramThreadBindingRecord[] {
	return [
		...getThreadBindingsState().bindingsByAccountConversation.values(),
...
function enqueuePersistBindings(params: {
	...
	const previous =
		getThreadBindingsState().persistQueueByAccountId.get(params.accountId) ??
		Promise.resolve();
	const next = previous
		.catch(() => undefined)
		.then(async () => {
...
function shouldExpireByIdle(params: {
	...
	return (
		params.now >=
		Math.max(params.record.lastActivityAt, params.record.boundAt) +
			idleTimeoutMs
	);
}
...
export function createTelegramThreadBindingManager(
	...
	const maxAgeMs = normalizeDurationMs(
		params.maxAgeMs,
	...
	const manager: TelegramThreadBindingManager = {
		...
		touchConversation: (conversationIdRaw, at) => {
			...
			const existing =
				getThreadBindingsState().bindingsByAccountConversation.get(key);
			if (!existing) {
				return null;
			}
		...
		unbindConversation: (unbindParams) => {
			...
			const key = resolveBindingKey({ accountId, conversationId });
			const removed =
				getThreadBindingsState().bindingsByAccountConversation.get(key) ?? null;
			if (!removed) {
				return null;
			}
		...
		stop: () => {
			...
			const existingManager =
				getThreadBindingsState().managersByAccountId.get(accountId);
...
export function getTelegramThreadBindingManager(
	...
	return (
		getThreadBindingsState().managersByAccountId.get(
...
function updateTelegramBindingsBySessionKey(params: {
	...
	targetSessionKey: string;
	update: (
		entry: TelegramThreadBindingRecord,
		now: number,
	) => TelegramThreadBindingRecord;
}): TelegramThreadBindingRecord[] {
