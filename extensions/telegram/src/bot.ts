...
export function setTelegramBotRuntimeForTest(
...
export function createTelegramBot(opts: TelegramBotOptions) {
	...
	let finalFetch: TelegramCompatFetch | undefined = shouldProvideFetch
		? fetchForClient
		: undefined;
	if (finalFetch || opts.fetchAbortSignal) {
		const baseFetch =
			finalFetch ??
			asTelegramCompatFetch(asTelegramClientFetch(globalThis.fetch));
	...
	const mediaMaxBytes =
		(opts.mediaMaxMb ?? telegramCfg.mediaMaxMb ?? 100) * 1024 * 1024;
