...
export const evaluateTelegramGroupPolicyAccess = (params: {
	...
	if (groupPolicy === "allowlist" && params.enforceAllowlistAuthorization) {
		...
		if (
			!senderAuthorization.allowed &&
			senderAuthorization.reason === "missing_match_input"
		) {
			return {
				allowed: false,
		...
		if (
			!senderAuthorization.allowed &&
			senderAuthorization.reason === "empty_allowlist"
		) {
			return {
				allowed: false,
		...
		if (
			!senderAuthorization.allowed &&
			senderAuthorization.reason === "not_allowlisted"
		) {
			return {
				allowed: false,
