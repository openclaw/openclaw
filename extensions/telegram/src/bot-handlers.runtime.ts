...
export const registerTelegramHandlers = ({
	...
	const buildSyntheticContext = (
		...
		const getFile =
			typeof ctx.getFile === "function"
				? ctx.getFile.bind(ctx as object)
				: async () => ({});
	...
	const inboundDebouncer = createInboundDebouncer<TelegramDebounceEntry>({
		...
		onFlush: async (entries) => {
			...
			const messageIdOverride = last.msg.message_id
				? String(last.msg.message_id)
				: undefined;
		...
		onError: (err, items) => {
			...
			if (chatId != null) {
				...
				void bot.api
					...
					.catch((sendErr) => {
						logVerbose(
							`telegram: error fallback send failed: ${String(sendErr)}`,
						);
					});
			}
		},
		...
		try {
			...
			for (const { ctx } of entry.messages) {
				...
				try {
					...
					runtime.log?.(
						warn(
							`media group: skipping photo that failed to fetch: ${String(mediaErr)}`,
						),
					);
					continue;
				}
	...
	const loadStoreAllowFrom = async () =>
		telegramDeps
			.readChannelAllowFromStore("telegram", process.env, accountId)
			.catch(() => []);
	...
	const resolveReplyMediaForMessage = async (
		...
		try {
			...
			logger.warn(
				{ chatId: msg.chat.id, error: String(err) },
	...
	type TelegramEventAuthorizationMode =
		...
		| "callback-scope"
		| "callback-allowlist";
	type TelegramEventAuthorizationResult =
		| { allowed: true }
	...
	> = {
		...
		"callback-allowlist": {
			...
			deniedGroupReason:
				"callback unauthorized by inlineButtonsScope allowlist",
		},
	};
	...
	const resolveTelegramEventAuthorizationContext = async (params: {
		...
		const effectiveDmPolicy =
			...
			"dmPolicy" in groupAllowContext.groupConfig
				? (groupAllowContext.groupConfig.dmPolicy ??
		...
		if (!isGroup && enforceDirectAuthorization) {
			...
			if (dmPolicy !== "open") {
				...
				if (
					...
					logVerbose(
						`Blocked telegram direct sender ${senderId || "unknown"} (${deniedDmReason})`,
					);
	...
	bot.on("message_reaction", async (ctx) => {
		try {
			...
			if (
				reactionMode === "own" &&
				!telegramDeps.wasSentByBot(chatId, messageId)
			) {
				return;
			}
			...
			if (!isGroup) {
				const requireTopic = (
					eventAuthContext.groupConfig as TelegramDirectConfig | undefined
				)?.requireTopic;
			...
			const senderName = user
				? [user.first_name, user.last_name].filter(Boolean).join(" ").trim() ||
					user.username
				: undefined;
			const senderUsernameLabel = user?.username
				? `@${user.username}`
				: undefined;
	...
	const processInboundMessage = async (params: {
		...
		if (text && !isCommandLike) {
			...
			if (existing) {
				...
				const idGap =
					typeof lastMsgId === "number" ? msg.message_id - lastMsgId : Infinity;
		...
		const conversationKey =
			conversationThreadId != null
				? `${chatId}:topic:${conversationThreadId}`
				: String(chatId);
	...
	bot.on("callback_query", async (ctx) => {
		...
		try {
			...
			const { resolvedThreadId, dmThreadId, storeAllowFrom, groupConfig } =
				eventAuthContext;
			const requireTopic = (
				groupConfig as { requireTopic?: boolean } | undefined
			)?.requireTopic;
			...
			const pluginCallback = await dispatchPluginInteractiveHandler({
				...
				ctx: {
					...
					parentConversationId:
						callbackThreadId != null ? String(chatId) : undefined,
					senderId: senderId || undefined,
			...
			if (isApprovalCallback) {
				...
				const isPluginApprovalCallback =
					approvalId?.startsWith("plugin:") ?? false;
				if (
					!isTelegramExecApprovalAuthorizedSender({
						cfg: runtimeCfg,
						accountId,
				...
				try {
					...
					if (
						...
						logVerbose(
							`telegram: failed to clear approval callback buttons: ${errStr}`,
						);
					}
				}
			}

			const paginationMatch = data.match(
				/^commands_page_(\d+|noop)(?::(.+))?$/,
			);
			if (paginationMatch) {
				...
				const agentId =
					paginationMatch[2]?.trim() || resolveDefaultAgentId(runtimeCfg);
			...
			if (modelCallback) {
				...
				if (modelCallback.type === "select") {
					...
					try {
						...
						const storePath = telegramDeps.resolveStorePath(
							cfg.session?.store,
							{
	...
	bot.on("message:migrate_to_chat_id", async (ctx) => {
		try {
			...
			if (
				!resolveChannelConfigWrites({ cfg, channelId: "telegram", accountId })
			) {
				runtime.log?.(
					warn(
	...
	const handleInboundMessageLike = async (event: InboundTelegramEvent) => {
		try {
			...
			if (
				event.requireConfiguredGroup &&
				(!groupConfig || groupConfig.enabled === false)
			) {
				logVerbose(
					`Blocked telegram channel ${event.chatId} (channel disabled)`,
				);
				return;
			}
	...
	bot.on("message", async (ctx) => {
		...
		await handleInboundMessageLike({
			...
			senderId:
				normalizedMsg.from?.id != null ? String(normalizedMsg.from.id) : "",
