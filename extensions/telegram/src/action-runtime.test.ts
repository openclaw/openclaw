...
describe("handleTelegramAction", () => {
  ...
  function telegramConfig(overrides?: Record<string, unknown>): MullusiConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as MullusiConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: MullusiConfig, accountId = "media") {
    await handleTelegramAction(
