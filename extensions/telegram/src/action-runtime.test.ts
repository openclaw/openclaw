...
describe("handleTelegramAction", () => {
	...
	it("surfaces non-fatal reaction warnings", async () => {
		...
		const parsed = JSON.parse(
			(textPayload as { type: "text"; text: string }).text,
		) as {
			ok: boolean;
			warning?: string;
			added?: string;
	...
	it.each([
		"off",
		"ack",
	] as const)("soft-fails reactions when reactionLevel is %s", async (level) => {
		const result = await handleTelegramAction(
			{
				action: "react",
				chatId: "123",
	...
	])("forwards resolved cfg for $name action", async ({
		params,
		cfg,
		assertCall,
	}) => {
	...
	])("blocks inline buttons when $name", async ({
		to,
		inlineButtons,
		expectedMessage,
	}) => {
...
describe("readTelegramButtons", () => {
	...
	it("rejects unsupported button style", () => {
		expect(() =>
			readTelegramButtons({
				buttons: [
					[{ text: "Option A", callback_data: "cmd:a", style: "secondary" }],
				],
			}),
...
describe("handleTelegramAction per-account gating", () => {
	function accountTelegramConfig(params: {
		...
		return {
			channels: {
				telegram: {
					...(params.topLevelBotToken
						? { botToken: params.topLevelBotToken }
						: {}),
					...(params.topLevelActions
						? { actions: params.topLevelActions }
						: {}),
	...
	async function expectAccountStickerSend(
		cfg: OpenClawConfig,
		accountId = "media",
	) {
		await handleTelegramAction(
	...
	it("allows account to explicitly re-enable top-level disabled reaction gate", async () => {
		const cfg = accountTelegramConfig({
			...
			accounts: {
				media: {
					botToken: "tok-media",
					actions: { sticker: true, reactions: true },
				},
			},
		});
