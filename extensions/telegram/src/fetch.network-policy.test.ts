...
describe("fetchRemoteMedia telegram network policy", () => {
	...
	beforeAll(async () => {
		vi.resetModules();
		({ TEST_UNDICI_RUNTIME_DEPS_KEY } = await import(
			"../../../src/infra/net/undici-runtime.js"
		));
	...
	it("keeps explicit proxy routing for file downloads", async () => {
		...
		const telegramTransport = resolveTelegramTransport(
			makeProxyFetch("http://127.0.0.1:7890"),
			{
				network: {
