...
describe("telegram exec approvals", () => {
	...
	it("matches approvers by normalized sender id", () => {
		...
		expect(isTelegramExecApprovalApprover({ cfg, senderId: "789" })).toBe(
			false,
		);
	});
	...
	it("infers approvers from allowFrom and direct defaultTo", () => {
		...
		expect(isTelegramExecApprovalApprover({ cfg, senderId: "12345" })).toBe(
			true,
		);
		expect(isTelegramExecApprovalApprover({ cfg, senderId: "67890" })).toBe(
			true,
		);
	});
	...
	it("defaults target to dm", () => {
		expect(
			resolveTelegramExecApprovalTarget({
				cfg: buildConfig({ enabled: true, approvers: ["1"] }),
			}),
		).toBe("dm");
	});

	it("only injects approval buttons on eligible telegram targets", () => {
		const dmCfg = buildConfig({
			enabled: true,
			approvers: ["123"],
			target: "dm",
		});
		const channelCfg = buildConfig({
			enabled: true,
			approvers: ["123"],
			target: "channel",
		});
		const bothCfg = buildConfig({
			enabled: true,
			approvers: ["123"],
			target: "both",
		});
		...
		expect(
			shouldInjectTelegramExecApprovalButtons({ cfg: dmCfg, to: "123" }),
		).toBe(true);
		expect(
			shouldInjectTelegramExecApprovalButtons({ cfg: dmCfg, to: "-100123" }),
		).toBe(false);
		expect(
			shouldInjectTelegramExecApprovalButtons({
				cfg: channelCfg,
		...
		expect(
			shouldInjectTelegramExecApprovalButtons({ cfg: channelCfg, to: "123" }),
		).toBe(false);
		expect(
			shouldInjectTelegramExecApprovalButtons({ cfg: bothCfg, to: "123" }),
		).toBe(true);
		expect(
			shouldInjectTelegramExecApprovalButtons({ cfg: bothCfg, to: "-100123" }),
		).toBe(true);
	});
	...
	it("does not require generic inlineButtons capability to enable exec approval buttons", () => {
		...
		expect(shouldEnableTelegramExecApprovalButtons({ cfg, to: "123" })).toBe(
			true,
		);
	});
	...
	describe("isTelegramExecApprovalTargetRecipient", () => {
		...
		it("accepts sender who is a DM target", () => {
			...
			expect(
				isTelegramExecApprovalTargetRecipient({ cfg, senderId: "12345" }),
			).toBe(true);
		});
		...
		it("rejects sender not in any target", () => {
			const cfg = buildTargetConfig([{ channel: "telegram", to: "12345" }]);
			expect(
				isTelegramExecApprovalTargetRecipient({ cfg, senderId: "99999" }),
			).toBe(false);
		});

		it("rejects group targets", () => {
			const cfg = buildTargetConfig([
				{ channel: "telegram", to: "-100123456" },
			]);
			expect(
				isTelegramExecApprovalTargetRecipient({ cfg, senderId: "123456" }),
			).toBe(false);
		});
		...
		it("returns false when senderId is empty or null", () => {
			const cfg = buildTargetConfig([{ channel: "telegram", to: "12345" }]);
			expect(isTelegramExecApprovalTargetRecipient({ cfg, senderId: "" })).toBe(
				false,
			);
			expect(
				isTelegramExecApprovalTargetRecipient({ cfg, senderId: null }),
			).toBe(false);
		...
		it("matches across multiple targets", () => {
			...
			expect(
				isTelegramExecApprovalTargetRecipient({ cfg, senderId: "67890" }),
			).toBe(true);
			expect(
				isTelegramExecApprovalTargetRecipient({ cfg, senderId: "11111" }),
			).toBe(true);
			expect(
				isTelegramExecApprovalTargetRecipient({ cfg, senderId: "U12345" }),
			).toBe(false);
		});
		...
		it("normalizes accountId matching", () => {
			const cfg = buildTargetConfig([
				{ channel: "telegram", to: "12345", accountId: "Work Bot" },
			]);
			expect(
				isTelegramExecApprovalTargetRecipient({
					cfg,
	...
	describe("isTelegramExecApprovalAuthorizedSender", () => {
		it("accepts explicit approvers", () => {
			...
			expect(
				isTelegramExecApprovalAuthorizedSender({ cfg, senderId: "123" }),
			).toBe(true);
		});
		...
		it("accepts active forwarded DM targets", () => {
			...
			expect(
				isTelegramExecApprovalAuthorizedSender({ cfg, senderId: "12345" }),
			).toBe(true);
		});
	});
});
