...
function buildMemoryLanceDbManifestCandidates(modulePath: string): string[] {
	...
	while (true) {
		candidates.add(
			path.join(cursor, "extensions", "memory-lancedb", "package.json"),
		);
...
export function resolveLanceDbDependencySpec(
	...
	for (const manifestPath of buildMemoryLanceDbManifestCandidates(modulePath)) {
		const lanceDbSpec =
			readPackageJson(manifestPath)?.dependencies?.["@lancedb/lancedb"];
		if (lanceDbSpec) {
			return lanceDbSpec;
		}
	}
...
const MEMORY_LANCEDB_RUNTIME_MANIFEST: RuntimeManifest = (() => {
	const lanceDbSpec = resolveLanceDbDependencySpec(
...
function defaultResolveRuntimeEntry(params: {
	...
	if (
		!manifestsMatch(readRuntimeManifest(runtimePackagePath), params.manifest)
	) {
		return null;
	}
	try {
...
async function defaultInstallRuntime(params: {
	...
	if (!manifestsMatch(currentManifest, params.manifest)) {
		...
		await fs.promises.rm(path.join(params.runtimeDir, "package-lock.json"), {
			force: true,
		});
	}
	...
	if ((install.code ?? 0) !== 0) {
		...
		throw new Error(
			detail || `npm exited with code ${install.code ?? "unknown"}`,
		);
	}
	...
	if (!resolved) {
		throw new Error(
			"installed LanceDB runtime is missing the @lancedb/lancedb entry",
		);
	}
	params.logger?.info?.(
		`memory-lancedb: installed LanceDB runtime under ${params.runtimeDir}`,
	);
	return resolved;
}
...
export function createLanceDbRuntimeLoader(
	overrides: Partial<LanceDbRuntimeLoaderDeps> = {},
): {
	...
	const deps: LanceDbRuntimeLoaderDeps = {
		...
		runtimeManifest:
			overrides.runtimeManifest ?? MEMORY_LANCEDB_RUNTIME_MANIFEST,
		importBundled:
			overrides.importBundled ?? (() => import("@lancedb/lancedb")),
		importResolved: overrides.importResolved ?? defaultImportResolved,
		resolveRuntimeEntry:
			overrides.resolveRuntimeEntry ?? defaultResolveRuntimeEntry,
