...
export class MemoryDB {
  ...
  private async doInitialize(): Promise<void> {
    ...
    if (tables.includes(MEMORY_TABLE_NAME)) {
      this.table = await this.db.openTable(MEMORY_TABLE_NAME);
      if (!hasAgentScopeColumn(await this.table.schema())) {
        this.table.close();
        this.db.close();
        this.table = null;
        this.db = null;
    ...
    this.table = await this.db.createTable(MEMORY_TABLE_NAME, [
      {
    ...
    await this.table.delete(`id = ${quoteLanceSqlString(SCHEMA_SENTINEL_ID)}`);
  }
  ...
  async search(
    ...
    // LanceDB 0.30 applies this metadata predicate before vector ranking. That
    // keeps another agent's rows outside both the candidate set and top-K.
  ...
  async query(agentId: string, options: MemoryQueryOptions): Promise<Record<string, unknown>[]> {
    ...
    let query = this.table!.query()
      // LanceDB before 0.31 replaces, rather than combines, repeated where()
      // calls. Keep scope and operator filter in one predicate.
