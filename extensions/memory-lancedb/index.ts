...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    const resolveRuntimeConfig = (): OpenClawConfig | undefined =>
      api.runtime.config?.current?.() as OpenClawConfig | undefined;
    const resolveEnabledAgentId = (
      rawAgentId: string | undefined,
      runtimeCfg = resolveRuntimeConfig(),
    ): string | undefined => {
      // Tool discovery without an execution agent cannot safely select a
      // private namespace. Hooks follow the same fail-closed rule.
      if (!rawAgentId?.trim()) {
        return undefined;
      }
      const agentId = normalizeAgentId(rawAgentId);
      const overrides = runtimeCfg
        ? resolveAgentConfig(runtimeCfg, agentId)?.memorySearch
        : undefined;
      const enabled =
        overrides?.enabled ?? runtimeCfg?.agents?.defaults?.memorySearch?.enabled ?? true;
    ...
    const resolveCliAgentId = (rawAgentId: unknown): string => {
      ...
      return resolveConfiguredDefaultAgentId(resolveRuntimeConfig() ?? api.config);
    };
    ...
    api.registerTool(
      (ctx) => {
        const agentId = resolveEnabledAgentId(
          ctx.agentId,
          ctx.getRuntimeConfig?.() ?? ctx.runtimeConfig ?? ctx.config,
        );
        if (!agentId) {
          return null;
        }
