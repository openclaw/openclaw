...
describe("resolveLanceDbDependencySpec", () => {
	it("reads dependency from source-layout sibling manifest", () => {
		...
		expect(resolveLanceDbDependencySpec(modulePath, readPackageJson)).toBe(
			"^0.27.1",
		);
	});
	...
	it("throws when no candidate package manifest declares @lancedb/lancedb", () => {
		...
		expect(() =>
			resolveLanceDbDependencySpec(modulePath, readPackageJson),
		).toThrow('memory-lancedb package.json is missing "@lancedb/lancedb"');
	});
});
