...
export function createMSTeamsMessageHandler(deps: MSTeamsMessageHandlerDeps) {
	...
	const handleTeamsMessageNow = async (params: MSTeamsDebounceEntry) => {
		...
		if (!isDirectMessage && msteamsCfg) {
			...
			if (
				...
				senderGroupAccess.reason === "disabled"
			) {
		...
		const { commandAuthorized, shouldBlock } =
			resolveDualTextControlCommandGate({
				useAccessGroups,
		...
		const clientInfo = activity.entities?.find(
			(e) => e.type === "clientInfo",
		) as
			| {
					timezone?: string;
		...
		const mediaList = await resolveMSTeamsInboundMedia({
			...
			log,
			preserveFilenames: (cfg as { media?: { preserveFilenames?: boolean } })
		...
		const ctxPayload = core.channel.reply.finalizeInboundContext({
			...
			WasMentioned:
				isDirectMessage || params.wasMentioned || params.implicitMention,
		...
		await core.channel.session.recordInboundSession({
			...
			onRecordError: (err) => {
				logVerboseMessage(
					`msteams: failed updating session meta: ${String(err)}`,
				);
			},
		});
	...
	const inboundDebouncer =
		core.channel.debounce.createInboundDebouncer<MSTeamsDebounceEntry>({
			...
			buildKey: (entry) => {
				...
				const senderId =
					entry.context.activity.from?.aadObjectId ??
					entry.context.activity.from?.id ??
					"";
	...
	return async function handleTeamsMessage(context: MSTeamsTurnContext) {
		...
		const implicitMention = Boolean(
			conversationId &&
				replyToId &&
				wasMSTeamsMessageSent(conversationId, replyToId),
		);
