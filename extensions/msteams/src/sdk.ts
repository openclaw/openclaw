 ...
 */
export function createMSTeamsApp(
...
function normalizeOutboundActivity(
	textOrActivity: string | object,
...
function createSendContext(params: {
	...
	return {
		...
		async updateActivity(
			...
			const nextActivity = activityUpdate as { id?: string } & Record<
				string,
				unknown
			>;
...
function createProcessContext(params: {
	...
	const conversationId = (
		params.activity?.conversation as Record<string, unknown>
	...
	const bot: MSTeamsBotIdentity | undefined =
		params.activity?.recipient && typeof params.activity.recipient === "object"
			? {
					id: (params.activity.recipient as Record<string, unknown>).id as
						| string
						| undefined,
					name: (params.activity.recipient as Record<string, unknown>).name as
						| string
						| undefined,
				}
			: undefined;
 ...
 */
export async function createBotFrameworkJwtValidator(
	creds: MSTeamsCredentials,
): Promise<{
