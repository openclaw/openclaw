...
const privateResolve = (ip) => async () => ({ address: ip });
const failingResolve = async () => {
  throw new Error("DNS failure");
};
function mockFetchWithRedirect(redirectMap, finalBody = "ok") {
...
describe("msteams attachment allowlists", () => {
  ...
  ])("malformed/expanded %s \u2192 %s (SDK fails closed)", (ip, expected) => {
