...
function mockFetchWithRedirect(
...
describe("msteams attachment allowlists", () => {
	...
	it("requires https and host suffix match", () => {
		...
		expect(
			isUrlAllowed("https://contoso.sharepoint.com/file.png", allowHosts),
		).toBe(true);
		expect(
			isUrlAllowed("http://contoso.sharepoint.com/file.png", allowHosts),
		).toBe(false);
		expect(isUrlAllowed("https://evil.example.com/file.png", allowHosts)).toBe(
			false,
		);
	});
...
describe("resolveAndValidateIP", () => {
	it("accepts a hostname resolving to a public IP", async () => {
		const ip = await resolveAndValidateIP(
	...
	it("rejects a hostname resolving to 10.x.x.x", async () => {
		await expect(
			resolveAndValidateIP("evil.test", privateResolve("10.0.0.1")),
	...
	it("rejects a hostname resolving to loopback", async () => {
		await expect(
			resolveAndValidateIP("evil.test", privateResolve("127.0.0.1")),
		).rejects.toThrow("private/reserved IP");
	});

	it("rejects a hostname resolving to IPv6 loopback", async () => {
		await expect(
			resolveAndValidateIP("evil.test", privateResolve("::1")),
		).rejects.toThrow("private/reserved IP");
	});

	it("throws on DNS resolution failure", async () => {
		await expect(
			resolveAndValidateIP("nonexistent.test", failingResolve),
...
describe("safeFetch", () => {
	...
	it("follows a redirect to an allowlisted host with public IP", async () => {
		const fetchMock = mockFetchWithRedirect({
			"https://teams.sharepoint.com/file.pdf":
				"https://cdn.sharepoint.com/storage/file.pdf",
		});
		await expectSafeFetchStatus({
			fetchMock,
			url: "https://teams.sharepoint.com/file.pdf",
	...
	it("blocks a redirect to an allowlisted host that resolves to a private IP (DNS rebinding)", async () => {
		...
		const fetchMock = mockFetchWithRedirect({
			"https://teams.sharepoint.com/file.pdf":
				"https://evil.trafficmanager.net/metadata",
		});
		await expect(
			safeFetch({
				url: "https://teams.sharepoint.com/file.pdf",
	...
	it("blocks redirect to HTTP (non-HTTPS)", async () => {
		const fetchMock = mockFetchWithRedirect({
			"https://teams.sharepoint.com/file":
				"http://internal.sharepoint.com/file",
		});
		await expect(
			safeFetch({
