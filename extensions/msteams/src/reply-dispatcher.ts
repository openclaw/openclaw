...
export function createMSTeamsReplyDispatcher(params: {
	...
	const feedbackLoopEnabled =
		params.cfg.channels?.msteams?.feedbackEnabled !== false;
		...
		try {
			...
			for (const msg of toSend) {
				try {
					...
					failed += 1;
					params.log.debug?.(
						"individual message send failed, continuing with remaining blocks",
					);
				}
			}
			if (failed > 0) {
				params.log.warn?.(
					`failed to deliver ${failed} of ${total} message blocks`,
					{
						failed,
						total,
					},
	...
	const markDispatchIdle = (): Promise<void> => {
		return flushPendingMessages()
			.catch((err) => {
				...
				const hint = formatMSTeamsSendErrorHint(classification);
				params.runtime.error?.(
					`msteams flush reply failed: ${errMsg}${hint ? ` (${hint})` : ""}`,
				);
