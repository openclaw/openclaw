...
export function renderReplyPayloadsToMessages(
	...
	for (const payload of replies) {
		const reply = resolveSendableOutboundReplyParts(payload, {
			text: getMSTeamsRuntime().channel.text.convertMarkdownTables(
				payload.text ?? "",
				tableMode,
			),
		});
	...
	if (msg.mediaUrl) {
		...
		if (isLocalPath(msg.mediaUrl)) {
			...
			if (!isPersonal && !isImage && tokenProvider && sharePointSiteId) {
				...
				const chatId =
					conversationRef.graphChatId ?? conversationRef.conversation?.id;
...
export async function sendMSTeamsMessages(params: {
	...
	const sendMessageBatchInContext = async (
		...
		for (const [idx, message] of batch.entries()) {
			messageIds.push(
				await sendMessageInContext(ctx, message, startIndex + idx),
			);
		}
		return messageIds;
	};
	...
	const sendProactively = async (
		...
		await params.adapter.continueConversation(
			...
			async (ctx) => {
				messageIds.push(
					...(await sendMessageBatchInContext(ctx, batch, startIndex)),
				);
			},
		);
		return messageIds;
		...
		for (const [idx, message] of messages.entries()) {
			const result = await withRevokedProxyFallback({
				...
				onRevoked: async () => {
					...
					return {
						ids:
							remaining.length > 0 ? await sendProactively(remaining, idx) : [],
						fellBack: true,
					};
				},
			});
