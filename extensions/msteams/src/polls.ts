...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["activiPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["activi", "pollId"]) ??
    readNestedString(value, ["activi", "poll", "id"]) ??
    readNestedString(value, ["data", "activiPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "activi", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "activi poll vote",
          displayText: "Vote recorded",
          value: { activiPollId: pollId, pollId },
        },
      },
    ],
  };
