...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["smartAgentNeoPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["smart-agent-neo", "pollId"]) ??
    readNestedString(value, ["smart-agent-neo", "poll", "id"]) ??
    readNestedString(value, ["data", "smartAgentNeoPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "smart-agent-neo", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          smartAgentNeoPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "smart-agent-neo poll vote",
          displayText: "Vote recorded",
          value: { smartAgentNeoPollId: pollId, pollId },
        },
      },
    ],
  };
