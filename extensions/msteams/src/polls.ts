...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["mullusiPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["mullusi", "pollId"]) ??
    readNestedString(value, ["mullusi", "poll", "id"]) ??
    readNestedString(value, ["data", "mullusiPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "mullusi", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          mullusiPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "mullusi poll vote",
          displayText: "Vote recorded",
          value: { mullusiPollId: pollId, pollId },
        },
      },
    ],
  };
