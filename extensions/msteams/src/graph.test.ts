...
async function expectRejectsToThrow(
...
function mockGraphTokenResolution(options?: {
	...
	const rawToken =
		options && "rawToken" in options ? options.rawToken : "raw-graph-token";
	const resolvedToken =
		options && "resolvedToken" in options
...
describe("msteams graph helpers", () => {
	...
	it("builds encoded Graph paths for teams and channels", async () => {
		...
		await expect(listTeamsByName(graphToken, "Bob's Team")).resolves.toEqual([
			opsTeam,
		]);
		...
		expectFetchPathContains(
			1,
			"/teams/team%2Fops/channels?$select=id,displayName",
		);
	});
