...
describe("TeamsHttpStream", () => {
	...
	it("sends final message activity on finalize", async () => {
		...
		expect(entities).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ additionalType: ["AIGeneratedContent"] }),
			]),
		);
	...
	it("sets feedbackLoopEnabled on final message", async () => {
		...
		stream.update(
			"A response long enough to pass the minimum character threshold for streaming.",
		);
		await new Promise((r) => setTimeout(r, 700));
		await stream.finalize();
	...
	it("informative update establishes streamId for subsequent chunks", async () => {
		...
		stream.update(
			"Hello, this is a long enough response for streaming to begin.",
		);
	...
	it("double finalize is a no-op", async () => {
		...
		stream.update(
			"A response long enough to pass the minimum character threshold.",
		);
		await stream.finalize();
