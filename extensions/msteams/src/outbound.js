...
const msteamsOutbound = {
  ...
  sendText: async ({ cfg, to, text, deps }) => {
    const send = resolveOutboundSendDep(deps, "msteams") ?? ((to2, text2) => sendMessageMSTeams({ cfg, to: to2, text: text2 }));
    const result = await send(to, text);
    return { channel: "msteams", ...result };
  },
  sendMedia: async ({ cfg, to, text, mediaUrl, mediaLocalRoots, deps }) => {
    const send = resolveOutboundSendDep(deps, "msteams") ?? ((to2, text2, opts) => sendMessageMSTeams({
      cfg,
      to: to2,
      text: text2,
