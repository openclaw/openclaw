    ...
    getDelegatedAccessToken: async (request) => {
      ...
      if (cached && !isTokenExpired(cached, params.now)) {
        if (!matchesRequestedJwtClaims(cached.token, request.audience, request.scopes)) {
          ...
          params.onDebug?.("msteams delegated auth cached token rejected by requested claims", {
...
function claimAudienceMatches(value: unknown, expected: string): boolean {
  ...
  if (typeof value === "string") {
    return audienceValueMatches(value, expectedAudiences);
  }
  if (Array.isArray(value)) {
    return value.some(
      (entry) => typeof entry === "string" && audienceValueMatches(entry, expectedAudiences),
    );
  }
  return false;
}
