...
describe("msteams delegated auth context", () => {
  ...
  it("rechecks Bot Framework when cached token claims do not satisfy a later request", async () => {
    ...
    const { deps, calls } = createSsoDeps([
      ...
      { status: 412, body: "consent required for second api" },
      {
        status: 200,
        body: {
          signInLink: "https://signin.example.test/start-second",
          tokenExchangeResource: { uri: "api://second-api" },
        },
      },
    ]);
    ...
    await expect(
      auth.getDelegatedAccessToken({
        provider: "msteams",
        audience: "api://second-api",
        scopes: ["second.access"],
      }),
    ).resolves.toEqual({
      ok: false,
    ...
    expect(calls[1]?.url).toContain("/api/usertoken/GetToken");
    expect(calls[2]?.url).toContain("/api/botsignin/GetSignInResource");
    expect(sendActivity).toHaveBeenCalledWith(
      expect.objectContaining({
        text: expect.stringContaining("https://signin.example.test/start-second"),
      }),
    );
  });
  ...
  it("continues user-id fallback when the first candidate token is expired", async () => {
  ...
  it("continues user-id fallback when the first candidate token has wrong claims", async () => {
    const wrongToken = createJwt({
      aud: "api://other-api",
      scp: "other.access",
    });
    const validToken = createJwt({
