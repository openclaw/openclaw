...
describe("msteams conversation store (fs)", () => {
  it("filters and prunes expired entries (but keeps legacy ones)", async () => {
    ...
    const ref: StoredConversationReference = {
      conversation: { id: "19:[redacted-email]2" },
      channelId: "msteams",
      serviceUrl: "https://service.example.com",
      user: { id: "u1", aadObjectId: "aad1" },
    };

    await store.upsert("19:[redacted-email]2", ref);
    ...
    json.conversations["19:[redacted-email]2"] = {
      ...ref,
      conversation: { id: "19:[redacted-email]2" },
    ...
    expect(ids).toEqual(["19:[redacted-email]2", "19:[redacted-email]2"]);

    expect(await store.get("19:[redacted-email]2")).toBeNull();
    expect(await store.get("19:[redacted-email]2")).not.toBeNull();

    await store.upsert("19:[redacted-email]2", {
      ...ref,
      conversation: { id: "19:[redacted-email]2" },
    });
