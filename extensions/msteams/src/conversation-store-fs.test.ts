...
import type { PluginRuntime } from "dna/plugin-sdk";
...
const runtimeStub = {
  state: {
    resolveStateDir: (env: NodeJS.ProcessEnv = process.env, homedir?: () => string) => {
      const override = env.DNA_STATE_DIR?.trim();
      if (override) return override;
      const resolvedHome = homedir ? homedir() : os.homedir();
      return path.join(resolvedHome, ".dna");
    },
  },
...
describe("msteams conversation store (fs)", () => {
  ...
  it("filters and prunes expired entries (but keeps legacy ones)", async () => {
    const stateDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "dna-msteams-store-"));

    const env: NodeJS.ProcessEnv = {
      ...process.env,
      DNA_STATE_DIR: stateDir,
    };
