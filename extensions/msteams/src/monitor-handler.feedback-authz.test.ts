...
vi.mock("./feedback-reflection.js", async () => {
	const actual = await vi.importActual<
		typeof import("./feedback-reflection.js")
			...
			value: {
				...
				actionValue: {
					...
					feedback: JSON.stringify({
						feedbackText: params.comment ?? "feedback text",
					}),
				},
				replyToId: "bot-msg-1",
			},
...
async function withFeedbackHandler(params: {
	...
	assertResult: (args: {
		tmpDir: string;
		originalRun: ReturnType<typeof vi.fn>;
	}) => Promise<void>;
}) {
	const tmpDir = await mkdtemp(
		path.join(tmpdir(), "openclaw-msteams-feedback-"),
	);
	try {
...
describe("msteams feedback invoke authz", () => {
	beforeEach(() => {
		...
		feedbackReflectionMockState.runFeedbackReflection.mockResolvedValue(
			undefined,
		);
	});
		...
		await withFeedbackHandler({
			...
			assertResult: async ({ tmpDir, originalRun }) => {
				await expectFileMissing(
					path.join(tmpDir, "msteams_direct_attacker-aad.jsonl"),
				);
				expect(
