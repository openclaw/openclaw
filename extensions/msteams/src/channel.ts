...
const collectMSTeamsSecurityWarnings =
	createAllowlistProviderGroupPolicyWarningCollector<{
		cfg: OpenClawConfig;
	}>({
...
function jsonMSTeamsActionResult(
	action: string,
...
function jsonMSTeamsOkActionResult(
	action: string,
	data: Record<string, unknown> = {},
) {
...
function resolveActionUploadFilePath(
	params: Record<string, unknown>,
): string | undefined {
...
function resolveRequiredActionMessageTarget(params: {
	...
	if (!to || !messageId) {
		return actionError(
			`${params.actionLabel} requires a target (to) and messageId.`,
		);
	}
	return { to, messageId };
}
...
function resolveRequiredActionPinnedMessageTarget(params: {
	...
	if (!to || !pinnedMessageId) {
		return actionError(
			`${params.actionLabel} requires a target (to) and pinnedMessageId.`,
		);
	}
	...
	base: {
		...
		messaging: {
			...
			resolveOutboundSessionRoute: (params) =>
				resolveMSTeamsOutboundSessionRoute(params),
			targetResolver: {
				looksLikeId: (raw) => {
		...
		directory: createChannelDirectoryAdapter({
			...
			listGroups: async ({ cfg, query, limit }) =>
				listDirectoryEntriesFromSources({
					...
					limit,
					normalizeId: (raw) =>
						`conversation:${raw.replace(/^conversation:/i, "").trim()}`,
				}),
			...createRuntimeDirectoryLiveAdapter({
		...
		resolver: {
			resolveTargets: async ({ cfg, inputs, kind, runtime }) => {
				...
				const resolvePending = async <T>(
					...
					applyResolvedEntry: (
						target: ResolveTargetResultEntry,
						entry: T,
					) => void,
				) => {
					if (pending.length === 0) {
		...
		actions: {
			...
			handleAction: async (ctx) => {
				...
				if (ctx.action === "upload-file") {
					...
					if (!mediaUrl) {
						return actionError(
							"Upload-file requires media, filePath, or path.",
						);
					}
					return await runWithRequiredActionTarget({
				...
				if (ctx.action === "react") {
					return await runWithRequiredActionMessageTarget({
						...
						run: async (target) => {
							const emoji =
								typeof ctx.params.emoji === "string"
									? ctx.params.emoji.trim()
									: "";
							const remove =
								typeof ctx.params.remove === "boolean"
									? ctx.params.remove
									: false;
							if (!emoji) {
								return {
				...
				if (ctx.action === "search") {
					return await runWithRequiredActionTarget({
						...
						run: async (to) => {
							...
							const limit =
								typeof ctx.params.limit === "number"
									? ctx.params.limit
									: undefined;
							const from =
								typeof ctx.params.from === "string"
									? ctx.params.from.trim()
									: undefined;
							const { searchMessagesMSTeams } =
								await loadMSTeamsChannelRuntime();
		...
		>({
			...
			probeAccount: async ({ cfg }) =>
				await (await loadMSTeamsChannelRuntime()).probeMSTeams(
			...
			formatCapabilitiesProbe: ({ probe }) => {
				...
				const appId =
					typeof teamsProbe?.appId === "string" ? teamsProbe.appId.trim() : "";
				if (appId) {
				...
				if (graph) {
					...
					if (graph.ok === false) {
						...
						if (roles.length > 0) {
							lines.push({
								text: `Graph roles: ${roles.map(formatPermission).join(", ")}`,
							});
						}
						if (scopes.length > 0) {
							lines.push({
								text: `Graph scopes: ${scopes.map(formatPermission).join(", ")}`,
							});
						}
