...
import { type PluginRuntime, SILENT_REPLY_TOKEN } from "../runtime-api.js";
...
function requireConversationId(ref: { conversation?: { id?: string } }) {
	if (!ref.conversation?.id) {
		throw new Error(
			"expected Teams top-level send to preserve conversation id",
		);
	}
...
function requireSentMessage(
	sent: Array<{ text?: string; entities?: unknown[] }>,
) {
