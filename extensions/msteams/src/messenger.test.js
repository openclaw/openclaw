...
import { SILENT_REPLY_TOKEN } from "openclaw/plugin-sdk/msteams";
...
vi.mock("./graph-upload.js", async () => {
  const actual = await vi.importActual("./graph-upload.js");
  return {
    ...actual,
...
const createRecordedSendActivity = (sink, failFirstWithStatusCode) => {
  let attempts = 0;
  return async (activity) => {
    ...
    sink.push(content);
    attempts += 1;
    if (failFirstWithStatusCode !== void 0 && attempts === 1) {
...
const createFallbackAdapter = (proactiveSent) => ({
...
describe("msteams messenger", () => {
  ...
  describe("renderReplyPayloadsToMessages", () => {
    ...
    it("does not filter non-exact silent reply prefixes", () => {
      const messages = renderReplyPayloadsToMessages(
        ...
        { textChunkLimit: 4e3, tableMode: "code" }
      );
    ...
    it("supports inline media mode", () => {
      const messages = renderReplyPayloadsToMessages(
        [{ text: "hi", mediaUrl: "https://example.com/a.png" }],
        { textChunkLimit: 4e3, mediaMode: "inline", tableMode: "code" }
      );
      expect(messages).toEqual([{ text: "hi", mediaUrl: "https://example.com/a.png" }]);
    });
  ...
  describe("sendMSTeamsMessages", () => {
    function createRevokedThreadContext(params) {
      let attempt = 0;
      return {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      ...
      try {
        ...
        expect(sent[0]?.text).toContain(
          "\u{1F4CE} [upload.txt](https://onedrive.example.com/share/item123)"
        );
