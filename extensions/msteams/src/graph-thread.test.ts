...
describe("stripHtmlFromTeamsMessage", () => {
	it("preserves @mention display names from <at> tags", () => {
		expect(stripHtmlFromTeamsMessage("<at>Alice</at> hello")).toBe(
			"@Alice hello",
		);
	});

	it("strips other HTML tags", () => {
		expect(stripHtmlFromTeamsMessage("<p>Hello <b>world</b></p>")).toBe(
			"Hello world",
		);
	});
	...
	it("handles <at> tags with attributes", () => {
		expect(
			stripHtmlFromTeamsMessage('<at id="123">Bob</at> please review'),
		).toBe("@Bob please review");
	});
...
describe("fetchChannelMessage", () => {
	...
	it("fetches the parent message with correct path", async () => {
		...
		const result = await fetchChannelMessage(
			"tok",
			"group-1",
			"channel-1",
			"msg-1",
	...
	it("returns undefined on fetch error", async () => {
		vi.mocked(fetchGraphJson).mockRejectedValueOnce(
