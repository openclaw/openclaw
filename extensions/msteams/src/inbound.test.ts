...
describe("msteams inbound", () => {
	...
	describe("htmlToPlainText", () => {
		it("strips tags and decodes entities", () => {
			expect(htmlToPlainText("<strong>Hello &amp; world</strong>")).toBe(
				"Hello & world",
			);
		});
	...
	describe("extractMSTeamsQuoteInfo", () => {
		const replyAttachment = (overrides?: {
			content?: string;
			contentType?: string;
		}) => ({
		...
		it("returns undefined when no reply blockquote is present", () => {
			expect(
				extractMSTeamsQuoteInfo([
					{ contentType: "text/html", content: "<p>just a message</p>" },
				]),
			).toBeUndefined();
		});
		...
		it("skips non-string content values", () => {
			expect(
				extractMSTeamsQuoteInfo([
					{ contentType: "application/json", content: { foo: "bar" } },
				]),
			).toBeUndefined();
		});
