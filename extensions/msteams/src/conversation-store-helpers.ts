...
export function parseStoredConversationTimestamp(
...
export function findPreferredDmConversationByUserId(
	...
	for (const entry of entries) {
		if (
			entry.reference.user?.aadObjectId === target ||
			entry.reference.user?.id === target
		) {
			matches.push(entry);
		}
	}
	...
	matches.sort((a, b) => {
		const aType =
			a.reference.conversation?.conversationType?.toLowerCase() ?? "";
		const bType =
			b.reference.conversation?.conversationType?.toLowerCase() ?? "";
