...
async function isFeedbackInvokeAuthorized(
	...
	if (
		...
		deps.log.debug?.(
			"dropping feedback invoke (not in team/channel allowlist)",
			{
				conversationId,
	...
	if (!isDirectMessage && !resolved.senderGroupAccess.allowed) {
		deps.log.debug?.(
			"dropping feedback invoke (group sender not allowlisted)",
			{
				sender: senderId,
				conversationId,
			},
...
async function handleFeedbackInvoke(
	...
	if (value.actionValue?.feedback) {
		try {
			const parsed = JSON.parse(value.actionValue.feedback) as {
				feedbackText?: string;
			};
	...
	try {
		...
		await fs
			.appendFile(transcriptFile, `${JSON.stringify(feedbackEvent)}\n`, "utf-8")
			.catch(() => {
...
export function registerMSTeamsHandlers<T extends MSTeamsActivityHandler>(
	...
	if (originalRun) {
		handler.run = async (context: unknown) => {
			...
			if (
				ctx.activity?.type === "invoke" &&
				ctx.activity?.name === "message/submitAction"
			) {
		...
		for (const member of membersAdded) {
			if (member.id === botId) {
				...
				const conversationType =
					ctx.activity?.conversation?.conversationType?.toLowerCase() ??
					"personal";
				...
				if (isPersonal && msteamsCfg?.welcomeCard !== false) {
					...
					try {
						...
						deps.log.debug?.("failed to send welcome card", {
							error: String(err),
						});
					}
					...
					try {
						...
						deps.log.debug?.("failed to send group welcome", {
							error: String(err),
						});
					}
				} else {
					deps.log.debug?.(
						"skipping welcome (disabled by config or conversation type)",
					);
				}
			} else {
