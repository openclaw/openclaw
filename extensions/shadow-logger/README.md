...
3.  **Direct Persistence:** The payload is sent via a direct, encrypted HTTPS connection (using `@supabase/supabase-js`) to the `public.dialogues` table in Supabase.
...
`[External Event (e.g. Telegram)]` $\rightarrow$ `[MCP Layer]` $\rightarrow$ `[OpenClaw Core]` $\rightarrow$ `[Internal Hook Trigger]` $\rightarrow$ **`[Shadow Logger Plugin]`** $\rightarrow$ `[Supabase (public.dialogues)]`

### Schema Design

The plugin writes to a specialized landing zone: the `public.dialogues` table. This table is designed to capture the raw, unadulterated context of every interaction.
...
The data recorded by this plugin serves as the raw material for **Layer 2: Semantic Distillation**. A separate background process (the "Refiner") will periodically consume the `dialogues` table to extract:
