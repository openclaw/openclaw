...
const openVikingPluginConfigSchema: OpenClawPluginConfigSchema = {
  uiHints: {
    baseUrl: {
      label: "Base URL",
  ...
  jsonSchema: {
    ...
    properties: {
      ...
      readLimit: { type: "integer", minimum: 1, maximum: 200 },
      injectAbstractsOnly: { type: "boolean" },
      systemPromptHeader: { type: "string" },
      requestTimeoutMs: { type: "integer", minimum: 1000, maximum: 60000 },
      logQueries: { type: "boolean" },
      logMisses: { type: "boolean" },
      logWriteback: { type: "boolean" },
      writebackEnabled: { type: "boolean" },
      writebackMode: {
        type: "string",
        enum: ["session-api", "workspace-memory", "hybrid"],
      },
      writebackDirectory: { type: "string" },
      writebackMaxChars: { type: "integer", minimum: 100, maximum: 4000 },
      diagnosticEnabled: { type: "boolean" },
      diagnosticFile: { type: "string" },
    },
  },
};
