...
export class OpenVikingContextEngine implements ContextEngine {
  ...
  async afterTurn(params: {
    ...
    if (mode === "workspace-memory" || mode === "hybrid") {
      try {
        ...
        if (workspaceDir) {
          ...
          this.logger?.warn?.("openviking: workspace-memory writeback skipped: missing workspaceDir");
        }
      } catch (error) {
    ...
    await this.writeDiagnosticSnapshotBestEffort(params);
    if (this.config.logWriteback) {
      this.logger?.info?.(`openviking: writeback stored digest=${candidate.digest} via ${outcomes.join(", ")}`);
    }
  }
  ...
  private async readBestEffort(uri: string, fallbackAbstract?: string): Promise<string | undefined> {
  ...
  private rememberDiagnosticState(
    sessionId: string,
    next: OpenVikingDiagnosticState,
  ): void {
