...
async function withStore<T>(run: (store: DashboardStore) => Promise<T> | T): Promise<T> {
...
function docWithPendingWidget(store: DashboardStore): WorkspaceDoc {
  return store.mutate(
    (draft) => {
...
describe("DashboardStore", () => {
  ...
  it("evicts the oldest undo snapshot past the ring size", async () => {
    await withStore((store) => {
      ...
      expect(() => store.undo()).toThrow("no dashboard undo snapshot available");
    });
  });
