// Wire-as-you-go integration test for the dashboard UI <-> gateway seam.
//
...
//   1. a real DashboardStore over a temp stateDir, seeded via the store's real
//      replace() API, plus a real JSON data file under `<stateDir>/dashboard/data`;
//   2. the real gateway handlers from registerDashboardGatewayMethods();
...
import {
  ...
  setWidgetCollapsed,
  updateWidgetTitle,
} from "../../../ui/src/lib/dashboard/index.ts";
import { registerDashboardGatewayMethods } from "./gateway.js";
import type { DashboardWidget, WorkspaceDoc } from "./schema.js";
...
async function withTempStateDir<T>(run: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-dashboard-integ-"));
  try {
...
function registerHandlers(store: DashboardStore, stateDir: string): RegisteredMethods {
  ...
  registerDashboardGatewayMethods({ api, store, dataRead: { stateDir } });
  return methods;
}
...
function findWidget(doc: WorkspaceDoc, slug: string, id: string): DashboardWidget {
...
describe("dashboard UI <-> gateway integration seam", () => {
  it("round-trips real UI client params through real handlers into the persisted store", async () => {
    await withTempStateDir(async (stateDir) => {
      ...
      const got = (await client.request("dashboard.workspace.get", {})) as { doc: WorkspaceDoc };
  ...
  it("catches the shipped contract drift: the pre-fix `{ slug, widgetId }` mutation shape is rejected and never mutates the store", async () => {
    await withTempStateDir(async (stateDir) => {
      ...
      await expect(
        client.request("dashboard.widget.update", {
          slug: "ops",
          widgetId: "revenue-card",
