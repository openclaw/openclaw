...
const ZALOUSER_ALLOW_FROM_PLACEHOLDER =
	"Alice, 123456789, or leave empty to configure later";
const ZALOUSER_GROUPS_PLACEHOLDER =
	"Family, Work, 123456789, or leave empty for now";
...
async function noteZalouserHelp(
	prompter: Parameters<
		NonNullable<ChannelSetupWizard["prepare"]>
	>[0]["prompter"],
): Promise<void> {
	await prompter.note(
		[
...
async function promptZalouserAllowFrom(params: {
	...
	while (true) {
		const entry = await prompter.text({
			...
			initialValue:
				existingAllowFrom.length > 0 ? existingAllowFrom.join(", ") : undefined,
		});
...
const zalouserDmPolicy: ChannelSetupDmPolicy = createTopLevelChannelDmPolicy({
	...
	getCurrent: (cfg) =>
		(cfg.channels?.zalouser?.dmPolicy ?? "pairing") as DmPolicy,
...
async function promptZalouserQuickstartDmPolicy(params: {
	...
	const { cfg, prompter, accountId } = params;
	const resolved = resolveZalouserAccountSync({ cfg, accountId });
	const existingPolicy = (cfg.channels?.zalouser?.dmPolicy ??
		"pairing") as DmPolicy;
	const existingAllowFrom = resolved.config.allowFrom ?? [];
	const existingLabel =
...
export const zalouserSetupWizard: ChannelSetupWizard = {
	...
	groupAccess: {
		...
		currentPolicy: ({ cfg, accountId }) =>
			resolveZalouserAccountSync({ cfg, accountId }).config.groupPolicy ??
			"allowlist",
		currentEntries: ({ cfg, accountId }) =>
			Object.keys(
				resolveZalouserAccountSync({ cfg, accountId }).config.groups ?? {},
			),
