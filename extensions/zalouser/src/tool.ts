...
function resolveAmbientZalouserTarget(context?: ZalouserToolContext): {
	...
	if (
		(deliveryContext?.channel === undefined ||
			deliveryContext.channel === "zalouser") &&
...
function resolveZalouserSendTarget(
	params: ToolParams,
...
export function createZalouserTool(
	context?: ZalouserToolContext,
): AnyAgentTool {
	return {
		name: "zalouser",
