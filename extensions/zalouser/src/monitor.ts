...
function resolveZalouserInboundSessionKey(params: {
	...
	return hasLegacySession && !hasDirectSession
		? legacySessionKey
		: directSessionKey;
}
...
function resolveGroupRequireMention(params: {
	...
	groups: Record<
		string,
		{ allow?: boolean; enabled?: boolean; requireMention?: boolean }
	>;
...
async function processMessage(
	...
	if (!senderId) {
		logVerbose(
			core,
			runtime,
			`zalouser: drop message ${chatId} (missing senderId)`,
		);
		return;
	}
	...
	const groupMembers =
		groupContext?.members?.slice(0, 20).join(", ") || undefined;
	...
	if (message.eventMessage) {
		try {
			...
			logVerbose(
				core,
				runtime,
				`zalouser: delivery/seen ack failed for ${chatId}: ${String(err)}`,
			);
		}
	}
		...
		if (!routeAccess.allowed) {
			if (routeAccess.reason === "disabled") {
				logVerbose(
					core,
					runtime,
					`zalouser: drop group ${chatId} (groupPolicy=disabled)`,
				);
				...
				logVerbose(
					core,
					runtime,
					`zalouser: drop group ${chatId} (not allowlisted)`,
				);
			} else if (routeAccess.reason === "route_disabled") {
				logVerbose(
					core,
					runtime,
					`zalouser: drop group ${chatId} (group disabled)`,
				);
			}
			return;
		}
	...
	const shouldComputeCommandAuth =
		core.channel.commands.shouldComputeCommandAuthorized(commandBody, config);
	...
	if (isGroup && accessDecision.decision !== "allow") {
		if (
			...
			logVerbose(
				core,
				runtime,
				"Blocked zalouser group message (no group allowlist)",
			);
		} else if (
			accessDecision.reasonCode ===
	...
	if (!isGroup && accessDecision.decision !== "allow") {
			...
			await pairing.issueChallenge({
				...
				onCreated: () => {
					logVerbose(
						core,
						runtime,
						`zalouser pairing request sender=${senderId}`,
					);
				},
		...
		if (
			...
			logVerbose(
				core,
				runtime,
				`Blocked zalouser DM from ${senderId} (dmPolicy=disabled)`,
			);
		} else {
			logVerbose(
				core,
	...
	const hasControlCommand = core.channel.commands.isControlCommandMessage(
		commandBody,
		config,
	);
	...
	const canDetectMention =
		mentionRegexes.length > 0 || explicitMention.canResolveExplicit;
	...
	if (isGroup && mentionGate.shouldSkip) {
		...
		logVerbose(
			core,
			runtime,
			`zalouser: skip group ${chatId} (mention required, not mentioned)`,
		);
		return;
	}
	...
	const normalizedTo = isGroup
		? `zalouser:group:${chatId}`
		: `zalouser:${chatId}`;
		...
		typing: {
			...
			onStartError: (err) => {
				...
				logVerbose(
					core,
					runtime,
					`zalouser typing failed for ${chatId}: ${String(err)}`,
				);
			},
		},
	});
...
export async function monitorZalouserProvider(
	...
	try {
		...
		runtime.log?.(
			`zalouser resolve failed; using config entries. ${String(err)}`,
		);
	}
		...
		listener = await startZaloListener({
			...
			onMessage: (msg) => {
				...
				void inboundQueue
					...
					.catch((err) => {
						runtime.error(
							`[${account.accountId}] Failed to process message: ${String(err)}`,
						);
					});
			},
			onError: (err) => {
				...
				runtime.error(
					`[${account.accountId}] Zalo listener error: ${String(err)}`,
				);
				settleFailure(err);
			},
		});
...
export const __testing = {
	processMessage: async (params: {
		...
		const groupHistories =
			params.historyState?.groupHistories ?? new Map<string, HistoryEntry[]>();
		await processMessage(
			params.message,
			params.account,
			params.config,
