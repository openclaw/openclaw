...
function mergeZalouserAccountConfig(
	...
	const merged = resolveMergedAccountConfig<ZalouserAccountConfig>({
		...
		accounts: (cfg.channels?.zalouser as ZalouserConfig | undefined)
			?.accounts as Record<string, Partial<ZalouserAccountConfig>> | undefined,
		accountId,
...
export async function resolveZalouserAccount(params: {
	...
	const { accountId, enabled, merged, profile } =
		resolveZalouserAccountBase(params);
