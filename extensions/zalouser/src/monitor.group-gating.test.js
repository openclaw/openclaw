...
function installRuntime(params) {
  ...
  const resolveAgentRoute = vi.fn((input) => {
  ...
  setZalouserRuntime({
    ...
    channel: {
      ...
      commands: {
        shouldComputeCommandAuthorized: vi.fn((body) => body.trim().startsWith("/")),
        resolveCommandAuthorizedFromAuthorizers,
        isControlCommandMessage: vi.fn((body) => body.trim().startsWith("/")),
      ...
      groups: {
        resolveRequireMention: vi.fn((input) => {
          ...
          const groupEntry = input.groupId ? groupCfg[input.groupId] : void 0;
      ...
      reply: {
        resolveEnvelopeFormatOptions: vi.fn(() => void 0),
...
function installGroupCommandAuthRuntime() {
  return installRuntime({
    resolveCommandAuthorizedFromAuthorizers: ({ useAccessGroups, authorizers }) => useAccessGroups && authorizers.some((entry) => entry.configured && entry.allowed)
  });
}
...
describe("zalouser monitor group mention gating", () => {
  ...
  async function expectSkippedGroupMessage(message) {
  ...
  async function expectGroupCommandAuthorizers(params) {
    const { dispatchReplyWithBufferedBlockDispatcher, resolveCommandAuthorizedFromAuthorizers } = installGroupCommandAuthRuntime();
  ...
  it("routes DM messages with direct peer kind", async () => {
    const { dispatchReplyWithBufferedBlockDispatcher, resolveAgentRoute, buildAgentSessionKey } = await processOpenDmMessage();
  ...
  it("reuses the legacy DM session key when only the old group-shaped session exists", async () => {
    const { dispatchReplyWithBufferedBlockDispatcher } = await processOpenDmMessage({
      readSessionUpdatedAt: (input) => input?.sessionKey === "agent:main:zalouser:group:321" ? 123 : void 0
    });
    const callArg = dispatchReplyWithBufferedBlockDispatcher.mock.calls[0]?.[0];
