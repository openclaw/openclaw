...
describe("zalouser send helpers", () => {
  ...
  it("rechunks normalized markdown text before sending to avoid transport truncation", async () => {
    ...
    mockSendText.mockResolvedValueOnce({ ok: true, messageId: "mid-2c-1" }).mockResolvedValueOnce({ ok: true, messageId: "mid-2c-2" });
    const result = await sendMessageZalouser("thread-2c", text, {
      profile: "p2c",
      isGroup: false,
      textMode: "markdown"
    });
    expect(formatted.text.length).toBeGreaterThan(2e3);
    expect(mockSendText).toHaveBeenCalledTimes(2);
    expect(mockSendText.mock.calls.map((call) => call[1]).join("")).toBe(formatted.text);
    expect(mockSendText.mock.calls.every((call) => call[1].length <= 2e3)).toBe(true);
  ...
  it("preserves text styles when splitting long formatted markdown", async () => {
    ...
    mockSendText.mockResolvedValueOnce({ ok: true, messageId: "mid-2d-1" }).mockResolvedValueOnce({ ok: true, messageId: "mid-2d-2" });
    ...
    expect(mockSendText).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        isGroup: false,
        textMode: "markdown",
        textStyles: [{ start: 0, len: 2e3, st: TextStyle.Bold }]
      })
    );
    expect(mockSendText).toHaveBeenNthCalledWith(
      2,
...
second paragraph`;
    const formatted = parseZalouserTextStyles(text);
    mockSendText.mockResolvedValueOnce({ ok: true, messageId: "mid-2d-3" }).mockResolvedValueOnce({ ok: true, messageId: "mid-2d-4" });
...
`,
  ...
  it("respects an explicit text chunk limit when splitting formatted markdown", async () => {
    ...
    mockSendText.mockResolvedValueOnce({ ok: true, messageId: "mid-2d-5" }).mockResolvedValueOnce({ ok: true, messageId: "mid-2d-6" });
  ...
  it("sends overflow markdown captions as follow-up text after the media message", async () => {
    ...
    mockSendText.mockResolvedValueOnce({ ok: true, messageId: "mid-2e-1" }).mockResolvedValueOnce({ ok: true, messageId: "mid-2e-2" });
  ...
  it("delegates reaction helper to JS transport", async () => {
    ...
    expect(result).toEqual({ ok: true, error: void 0 });
  });
