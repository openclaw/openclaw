...
describe("zalouser message sid helpers", () => {
	...
	it("formats message sid fields for context payload", () => {
		expect(formatZalouserMessageSidFull({ msgId: "1", cliMsgId: "2" })).toBe(
			"1:2",
		);
	...
	it("resolves primary message sid with fallback timestamp", () => {
		expect(
			resolveZalouserMessageSid({ msgId: "1", cliMsgId: "2", fallback: "t" }),
		).toBe("1");
		expect(resolveZalouserMessageSid({ cliMsgId: "2", fallback: "t" })).toBe(
			"2",
		);
