...
const groupContextCache = new Map<
	string,
	{ value: ZaloGroupContext; expiresAt: number }
>();
...
function extractOwnMentionSpans(
	...
	for (const entry of rawMentions) {
		...
		const startRaw = toNonNegativeInteger(
			record.pos ?? record.start ?? record.offset,
		);
...
function resolveGroupNameFromMessageData(
...
function mapGroup(
	groupId: string,
	group: GroupInfo & Record<string, unknown>,
): ZaloGroup {
	const totalMember =
...
function readCredentials(profile: string): StoredZaloCredentials | null {
	...
	try {
		...
		return {
			...
			lastUsedAt:
				typeof parsed.lastUsedAt === "string" ? parsed.lastUsedAt : undefined,
		};
	} catch {
		return null;
	}
...
function touchCredentials(profile: string): void {
	...
	fs.writeFileSync(
		resolveCredentialsPath(profile),
...
function readCachedGroupContext(
	profile: string,
	groupId: string,
): ZaloGroupContext | null {
...
function writeCachedGroupContext(
	profile: string,
	context: ZaloGroupContext,
): void {
	const now = Date.now();
...
function toInboundMessage(
	...
	return {
		...
		senderId,
		senderName:
			typeof data.dName === "string"
				? data.dName.trim() || undefined
				: undefined,
...
export async function checkZaloAuthenticated(
	...
	try {
		...
		await withTimeout(
			...
			12_000,
			"Timed out checking Zalo session",
		);
		return true;
	} catch {
...
export async function listZaloGroupMembers(
	...
	const memVerIds = Array.isArray(groupInfo.memVerList)
		...
		: [];
	const currentMembers = Array.isArray(groupInfo.currentMems)
	...
	if (uniqueIds.length > 0) {
		...
		for (const [rawId, profileValue] of Object.entries(profileEntries)) {
			const id =
				toNumberId(rawId) || toNumberId((profileValue as { id?: unknown })?.id);
			if (!id || !profileValue) {
				continue;
			}
			profileMap.set(id, {
	...
	return uniqueIds.map((id) => ({
		userId: id,
		displayName:
			profileMap.get(id)?.displayName || currentById.get(id)?.displayName || id,
	...
	try {
		if (options.mediaUrl?.trim()) {
			...
			if (media.kind === "audio") {
				...
				const attachmentFileName = fileName.includes(".")
					? fileName
					: `${fileName}.bin`;
...
export async function sendZaloTypingEvent(
	...
	if ("sendTypingEvent" in api && typeof api.sendTypingEvent === "function") {
		await (api as API & ApiTypingCapability).sendTypingEvent(
			trimmedThreadId,
			type,
		);
		return;
	...
	throw new Error(
		"Zalo typing indicator is not supported by current API session",
	);
}
...
export async function startZaloQrLogin(params: {
					...
					(event: LoginQRCallbackEvent) => {
						...
						switch (event.type) {
							case LoginQRCallbackEventType.QRCodeGenerated: {
								const image = event.data.image.replace(
							...
							case LoginQRCallbackEventType.QRCodeExpired: {
								try {
									...
									current.error =
										"QR expired before confirmation. Start login again.";
								}
								break;
							}
	...
	const timeoutMs = Math.max(
		params.timeoutMs ?? DEFAULT_QR_START_TIMEOUT_MS,
		3000,
	);
...
export async function waitForZaloQrLogin(params: {
	...
	if (!active) {
		...
		return {
			connected,
			message: connected
				? "Zalo session is ready."
				: "No active Zalo QR login in progress.",
		};
	}
	...
	const timeoutMs = Math.max(
		params.timeoutMs ?? DEFAULT_QR_WAIT_TIMEOUT_MS,
		1000,
	);
	const deadline = Date.now() + timeoutMs;
...
export async function resolveZaloGroupsByEntries(params: {
	...
	return params.entries.map((input) => {
		...
		return match
			? { input, resolved: true, id: match.groupId }
...
export async function resolveZaloAllowFromEntries(params: {
	...
	Array<{ input: string; resolved: boolean; id?: string; note?: string }>
> {
...
export async function clearProfileRuntimeArtifacts(
	profileInput?: string | null,
): Promise<void> {
	const profile = normalizeProfile(profileInput);
	resetQrLogin(profile);
	...
	invalidateApi(profile);
	await fsp
		.mkdir(resolveCredentialsDir(), { recursive: true })
		.catch(() => undefined);
}
