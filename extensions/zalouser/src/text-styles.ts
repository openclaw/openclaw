...
export function parseZalouserTextStyles(input: string): {
	text: string;
	styles: Style[];
} {
	...
	for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
		...
		const { text: unquotedLine, indent: baseIndent } =
			stripQuotePrefix(rawLine);
			...
			processedLines.push(
				escapeLiteralText(
					normalizeCodeBlockLeadingWhitespace(
						stripCodeFenceIndent(codeLine, activeFence.indent),
					),
					escapeMap,
				),
			);
		...
		if (isIndentedCodeBlockLine(line)) {
			...
			processedLines.push(
				escapeLiteralText(normalizeCodeBlockLeadingWhitespace(line), escapeMap),
			);
			continue;
		}

		const { text: markdownLine, size: markdownPadding } =
			stripOptionalMarkdownPadding(line);
...
function stripQuotePrefix(
	...
	while (
		removedDepth < maxDepth &&
		consumedCursor < line.length &&
		line[consumedCursor] === ">"
	) {
		removedDepth += 1;
		consumedCursor += 1;
...
function parseInlineSegments(
	...
	while (cursor < text.length) {
		...
		if (nextMatch.match.index > cursor) {
			pushSegment(
				segments,
				text.slice(cursor, nextMatch.match.index),
				inheritedStyles,
			);
		}
