...
function installRuntime(params: {
	...
	const resolveAgentRoute = vi.fn(
		(input: { peer?: { kind?: string; id?: string } }) => {
	...
	const readSessionUpdatedAt = vi.fn(
		(_params?: { storePath: string; sessionKey: string }): number | undefined =>
			undefined,
	);
	...
	setZalouserRuntime({
		...
		channel: {
			...
			commands: {
				shouldComputeCommandAuthorized: vi.fn((body: string) =>
			...
			groups: {
				resolveRequireMention: vi.fn((input) => {
					...
					const groupEntry = input.groupId
						? groupCfg[input.groupId]
						: undefined;
...
describe("zalouser monitor group mention gating", () => {
	...
	async function expectSkippedGroupMessage(
	...
	async function processOpenDmMessage(params?: {
		...
		if (params?.readSessionUpdatedAt) {
			runtime.readSessionUpdatedAt.mockImplementation(
				params.readSessionUpdatedAt,
			);
		}
		...
		await processMessageWithDefaults({
			...
			account: {
				...
				config: {
					...
					...(params.dangerouslyAllowNameMatching
	...
	it("accepts mutable group-name matches only when dangerouslyAllowNameMatching is enabled", async () => {
		const dispatchReplyWithBufferedBlockDispatcher =
			await expectDangerousNameMatching({
				dangerouslyAllowNameMatching: true,
		...
		const { dispatchReplyWithBufferedBlockDispatcher } =
			await processOpenDmMessage({
				readSessionUpdatedAt: (input?: {
					...
					input?.sessionKey === "agent:main:zalouser:group:321"
						? 123
						: undefined,
			});
	...
	it("includes skipped group messages as InboundHistory on the next processed message", async () => {
		...
		expect(dispatchReplyWithBufferedBlockDispatcher).toHaveBeenCalledTimes(1);
		const firstDispatch =
			dispatchReplyWithBufferedBlockDispatcher.mock.calls[0]?.[0];
		...
		expect(String(firstDispatch?.ctx?.Body ?? "")).toContain(
			"first unmentioned line",
		);
		...
		const secondDispatch =
			dispatchReplyWithBufferedBlockDispatcher.mock.calls[1]?.[0];
