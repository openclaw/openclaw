...
const zalouserRawSendResultAdapter = createRawChannelSendResultAdapter({
	...
	sendText: async ({ to, text, accountId, cfg }) => {
		...
		return await sendMessageZalouser(target.threadId, text, {
			...
			textChunkLimit: resolveZalouserOutboundTextChunkLimit(
				cfg,
				account.accountId,
			),
		});
...
function resolveZalouserOutboundChunkMode(
	cfg: OpenClawConfig,
	accountId?: string,
) {
	return getZalouserRuntime().channel.text.resolveChunkMode(
		cfg,
		"zalouser",
		accountId,
	);
...
function resolveZalouserOutboundTextChunkLimit(
	cfg: OpenClawConfig,
	accountId?: string,
) {
	return getZalouserRuntime().channel.text.resolveTextChunkLimit(
		cfg,
		"zalouser",
		accountId,
		{
...
const resolveZalouserDmPolicy =
	createScopedDmSecurityResolver<ResolvedZalouserAccount>({
		channelKey: "zalouser",
...
const zalouserMessageActions: ChannelMessageActionAdapter = {
	...
	handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
		...
		const ids = resolveZalouserReactionMessageIds({
			...
			cliMsgId:
				typeof params.cliMsgId === "string" ? params.cliMsgId : undefined,
	...
	base: {
		...
		directory: {
			...
			listGroupMembers: async ({ cfg, accountId, groupId, limit }) => {
				...
				const members = await listZaloGroupMembers(
					account.profile,
					normalizedGroupId,
				);
			...
			resolveTargets: async ({ cfg, accountId, inputs, kind, runtime }) => {
				...
				for (const input of inputs) {
					...
					try {
						...
						if (kind === "user") {
							const friends = await listZaloFriendsMatching(
								account.profile,
								trimmed,
							);
							...
							results.push({
								...
								note:
									friends.length > 1
										? "multiple matches; chose first"
										: undefined,
							});
						} else {
							const groups = await listZaloGroupsMatching(
								account.profile,
								trimmed,
							);
							const best =
		...
		gateway: {
			startAccount: async (ctx) => {
				...
				ctx.log?.info(
					`[${account.accountId}] starting zalouser provider${userLabel}`,
				);
	...
	outbound: {
		...
		sendPayload: async (ctx) =>
			await sendPayloadWithChunkedTextAndMedia({
				...
				sendMedia: (nextCtx) =>
					zalouserRawSendResultAdapter.sendMedia?.(nextCtx),
