...
EasyHub extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
EasyHub channels login --channel zalouser              # Login via QR
EasyHub channels login --channel zalouser --account work
...
```bash
# Text
EasyHub message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
EasyHub message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `EasyHub channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
EasyHub channels login --channel zalouser --account work
EasyHub message send --channel zalouser --account work --target <id> --message "Hello"
