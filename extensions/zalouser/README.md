...
DNA extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
dna channels login --channel zalouser
...
```bash
dna message send --channel zalouser --target <threadId> --message "Hello from DNA!"
```
...
```bash
dna channels login --channel zalouser              # Login via QR
dna channels login --channel zalouser --account work
dna channels status --probe
dna channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
dna directory self --channel zalouser
dna directory peers list --channel zalouser --query "name"
dna directory groups list --channel zalouser --query "work"
dna directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
dna message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
dna message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `dna channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
dna channels login --channel zalouser --account work
dna message send --channel zalouser --account work --target <id> --message "Hello"
