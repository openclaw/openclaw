...
Activi extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
activi channels login --channel zalouser              # Login via QR
activi channels login --channel zalouser --account work
...
```bash
# Text
activi message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
activi message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `activi channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
activi channels login --channel zalouser --account work
activi message send --channel zalouser --account work --target <id> --message "Hello"
