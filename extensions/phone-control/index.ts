import type { SmartAgentNeoPluginApi, SmartAgentNeoPluginService } from "smart-agent-neo/plugin-sdk";
...
function normalizeDenyList(cfg: SmartAgentNeoPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SmartAgentNeoPluginApi["config"]): string[] {
...
export default function register(api: SmartAgentNeoPluginApi) {
  ...
  const timerService: SmartAgentNeoPluginService = {
