...
import type { ActiviPluginApi, ActiviPluginService } from "activi/plugin-sdk";
...
function normalizeDenyList(cfg: ActiviPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ActiviPluginApi["config"]): string[] {
...
export default function register(api: ActiviPluginApi) {
  ...
  const timerService: ActiviPluginService = {
