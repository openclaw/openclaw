...
function normalizeDenyList(cfg: MullusiPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: MullusiPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: MullusiPluginApi) {
    ...
    const timerService: MullusiPluginService = {
