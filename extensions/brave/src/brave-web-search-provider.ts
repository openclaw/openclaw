...
function resolveBraveConfig(searchConfig?: SearchConfigRecord): BraveConfig {
	...
	return brave && typeof brave === "object" && !Array.isArray(brave)
		? (brave as BraveConfig)
		: {};
}
...
function normalizeBraveSearchLang(
	...
	const canonical =
		BRAVE_SEARCH_LANG_ALIASES[trimmed.toLowerCase()] ?? trimmed.toLowerCase();
	...
	return withTrustedWebSearchEndpoint(
		...
		async (res) => {
			if (!res.ok) {
				...
				throw new Error(
					`Brave LLM Context API error (${res.status}): ${detail || res.statusText}`,
				);
			}
...
async function runBraveWebSearch(params: {
	...
	if (params.freshness) {
		...
		url.searchParams.set(
			"freshness",
			`${params.dateAfter}to${params.dateBefore}`,
		);
	...
	return {
		...
		execute: async (args) => {
			...
			const freshness = rawFreshness
				? normalizeFreshness(rawFreshness, "brave")
				: undefined;
