...
describe("plugin registration", () => {
	it("registers memory tools + cli through extension-local modules", () => {
		...
		expect(registerMemoryPromptSection).toHaveBeenCalledWith(
			buildPromptSection,
		);
		...
		const searchFactory = registerTool.mock.calls[0]?.[0] as
			| ((ctx: unknown) => unknown)
			| undefined;
		const getFactory = registerTool.mock.calls[1]?.[0] as
			| ((ctx: unknown) => unknown)
			| undefined;
		...
		expect((searchFactory?.(ctx) as { name?: string } | null)?.name).toBe(
			"memory_search",
		);
		expect((getFactory?.(ctx) as { name?: string } | null)?.name).toBe(
			"memory_get",
		);
...
describe("buildMemoryFlushPlan", () => {
	...
	it("defaults to safe prompts and gating values", () => {
		...
		expect(plan?.forceFlushTranscriptBytes).toBe(
			DEFAULT_MEMORY_FLUSH_FORCE_TRANSCRIPT_BYTES,
		);
