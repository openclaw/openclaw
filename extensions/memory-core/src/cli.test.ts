	...
	async (importOriginal) => {
		const actual =
			await importOriginal<
				typeof import("openclaw/plugin-sdk/memory-core-host-runtime-core")
			>();
		return {
			...actual,
			loadConfig,
...
vi.mock(
	"openclaw/plugin-sdk/memory-core-host-runtime-cli",
	async (importOriginal) => {
		const actual =
			await importOriginal<
				typeof import("openclaw/plugin-sdk/memory-core-host-runtime-cli")
			>();
		return {
			...actual,
...
beforeAll(async () => {
	...
	({ defaultRuntime, isVerbose, setVerbose } = await import(
		"openclaw/plugin-sdk/memory-core-host-runtime-cli"
	));
});
...
describe("memory cli", () => {
	const inactiveMemorySecretDiagnostic =
		"agents.defaults.memorySearch.remote.apiKey inactive"; // pragma: allowlist secret
	...
	function hasLoggedInactiveSecretDiagnostic(spy: ReturnType<typeof vi.spyOn>) {
		return spy.mock.calls.some(
			(call: unknown[]) =>
				...
				call[0].includes(inactiveMemorySecretDiagnostic),
		);
	}
