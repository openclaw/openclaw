...
async function collectSessionIngestionBatches(params: {
  ...
  if (agentIds.length > 0) {
    const sessionStores = new Map<
      string,
      Record<string, { sessionFile?: unknown; sessionId?: unknown; spawnedBy?: unknown }>
    >();
    for (const agentId of agentIds) {
      const sessionsDir = resolveSessionTranscriptsDirForAgent(agentId);
      try {
        const raw = await fs.readFile(path.join(sessionsDir, "sessions.json"), "utf-8");
        sessionStores.set(agentId, JSON.parse(raw) as Record<string, { spawnedBy?: unknown }>);
      } catch {
        // Swallow; secondary check is best-effort.
      }
    }
    if (sessionStores.size > 0) {
      const cronKeyCache = new Map<string, boolean>();
      function isCronDescendant(
        key: string,
        ...
        if (isCronRunSessionKey(key)) {
          cronKeyCache.set(key, true);
          return true;
        }
        if (typeof entry.spawnedBy === "string" && entry.spawnedBy.trim().length > 0) {
          cronKeyCache.set(key, false); // Guard against cycles.
          const result = isCronDescendant(entry.spawnedBy, store);
          cronKeyCache.set(key, result);
          return result;
        }
        cronKeyCache.set(key, false);
        return false;
