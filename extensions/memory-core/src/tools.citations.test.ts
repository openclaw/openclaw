...
describe("memory tools", () => {
  ...
  it("returns structured error when corpus=wiki supplement get throws", async () => {
    // memory_get corpus=wiki bypasses memory read entirely and calls
    // getSupplementMemoryReadResult directly. If the supplement's get()
    // throws, the tool must return a structured disabled result instead
  ...
  it("returns unavailable result when all supplements fail to search", async () => {
    // When every registered supplement's search() throws, the function
    // must surface the outage via unavailable-phase handling instead of
    // silently returning an empty success.
    registerMemoryCorpusSupplement("broken-a", {
      search: async () => {
        throw new Error("supplement A failed");
      },
      get: async () => null,
    });
    registerMemoryCorpusSupplement("broken-b", {
      search: async () => {
        throw new Error("supplement B failed");
      },
      get: async () => null,
    });

    const tool = createMemorySearchToolOrThrow();
    const result = await tool.execute("call_search_all_both_broken", {
      query: "anything",
      corpus: "all",
    });

    expectUnavailableMemorySearchDetails(result.details, {
      error: expect.stringContaining("All corpus supplements failed to search"),
      warning: expect.stringContaining("unavailable"),
      action: expect.any(String),
    });
  });

  it("isolates per-supplement search failures so one broken supplement does not discard results from others", async () => {
    // Two supplements: one throws on search(), one returns valid results.
    // Memory search returns nothing so only supplement results are visible.
    // The broken supplement must not prevent the healthy one's results
    // from being returned.
    setMemorySearchImpl(async () => []);
    registerMemoryCorpusSupplement("broken-wiki", {
      search: async () => {
        throw new Error("broken supplement search failed");
      },
      get: async () => null,
    });
    ...
    const tool = createMemorySearchToolOrThrow();
    const result = await tool.execute("call_search_all_one_broken", {
      query: "alpha",
      corpus: "all",
    });
    ...
    expect(details.results).toHaveLength(1);
    expect(details.results[0]?.path).toBe("entities/alpha.md");
  });
});
