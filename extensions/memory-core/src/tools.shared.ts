...
export async function searchMemoryCorpusSupplements(params: {
  ...
  const results = (
    await Promise.all(
      supplements.map(async (registration) => {
        try {
          return await registration.supplement.search(params);
        } catch {
          // Isolate per-supplement search failures so a single broken
          // supplement does not discard results from other supplements.
          failureCount++;
          return [];
        }
      }),
  ...
  if (failureCount === supplements.length) {
    // All registered supplements failed; surface the outage so the
    // caller's unavailable-phase handling can return an explicit
    // unavailable result instead of silently reporting empty success.
    throw new Error("All corpus supplements failed to search");
  }
  return results
    .toSorted((left, right) => {
