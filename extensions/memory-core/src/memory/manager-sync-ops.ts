...
export abstract class MemoryManagerSyncOps {
			...
			try {
							...
							for (const extension of getMemoryMultimodalExtensions(modality)) {
								watchPaths.add(
									path.join(
										entry,
										"**",
										buildCaseInsensitiveExtensionGlob(extension),
									),
								);
							}
						}
				...
				if (
					...
					(entry.toLowerCase().endsWith(".md") ||
						classifyMemoryMultimodalPath(entry, this.settings.multimodal) !==
							null)
				) {
		...
		for (const sessionFile of pending) {
			...
			const bytesHit =
				bytesThreshold <= 0
					...
					: delta.pendingBytes >= bytesThreshold;
			const messagesHit =
				messagesThreshold <= 0
			...
			delta.pendingBytes =
				bytesThreshold > 0
					? Math.max(0, delta.pendingBytes - bytesThreshold)
					: 0;
			delta.pendingMessages =
				messagesThreshold > 0
					? Math.max(0, delta.pendingMessages - messagesThreshold)
					: 0;
			shouldSync = true;
		}
		if (shouldSync) {
	...
	private async updateSessionDelta(sessionFile: string): Promise<{
		...
		if (size < state.lastSize) {
			...
			const shouldCountMessages =
				...
				(thresholds.deltaBytes <= 0 ||
					state.pendingBytes < thresholds.deltaBytes);
	...
	private async syncMemoryFiles(params: {
		...
		const selectSourceFileState = this.db.prepare(
			`SELECT path, hash FROM files WHERE source = ?`,
		);
		...
		const existingHashes = new Map(
			existingRows.map((row) => [row.path, row.hash]),
		);
		...
		const tasks = fileEntries.map((entry) => async () => {
			if (
				...
				existingHashes.get(entry.path) === entry.hash
			) {
				if (params.progress) {
	...
	private async syncSessionFiles(params: {
		...
		const existingHashes =
			existingRows === null
				? null
				: new Map(existingRows.map((row) => [row.path, row.hash]));
		const indexAll =
			params.needsFullReindex ||
	...
	protected async runSync(params?: {
		...
		const targetSessionFiles = this.normalizeTargetSessionFiles(
			params?.sessionFiles,
		);
		...
		if (
			...
			try {
				...
				const activated =
					this.shouldFallbackOnError(reason) &&
					(await this.activateFallbackProvider(reason));
				if (activated) {
					if (
		...
		try {
			...
			if (shouldSyncSessions) {
				await this.syncSessionFiles({
					needsFullReindex,
					targetSessionFiles: targetSessionFiles
						? Array.from(targetSessionFiles)
						: undefined,
					progress: progress ?? undefined,
				});
	...
	protected resolveBatchConfig(): {
		...
		const enabled = Boolean(
			batch?.enabled && this.provider && this.providerRuntime?.batchEmbed,
		);
		return {
			enabled,
	...
	private async runSafeReindex(params: {
		...
		const restoreOriginalState = () => {
			...
			this.vector.available = originalDbClosed
				? null
				: originalState.vectorAvailable;
		...
		try {
			...
			if (!nextMeta) {
				throw new Error(
					"Failed to compute memory index metadata for reindexing.",
				);
			}
	...
	private metaSourcesDiffer(
		...
		return metaSources.some(
			(source, index) => source !== configuredSources[index],
		);
	}
}
