...
function listAutoSelectAdapters(
	...
	return listMemoryEmbeddingProviders()
		...
		.filter((adapter) =>
			adapter.id === "local"
				? canAutoSelectLocal(options.local?.modelPath)
				: true,
		)
		.toSorted(
			(a, b) =>
...
export function resolveEmbeddingProviderFallbackModel(
	...
	const adapter =
		getMemoryEmbeddingProvider(providerId) ??
		getBuiltinMemoryEmbeddingProviderAdapter(providerId);
...
export async function createEmbeddingProvider(
	...
	try {
		...
		if (
			...
			options.fallback !== options.provider
		) {
			...
			try {
				...
				const fallbackReason = formatProviderError(
					fallbackAdapter,
					fallbackErr,
				);
