...
import { memoryWatchMode } from "./watch-policy.js";
...
describe("Memory watch configuration", () => {
  ...
  it.each(["true", "1", "false", "0"])("honors explicit polling mode %s", (value) => {
    vi.stubEnv("CHOKIDAR_USEPOLLING", value);
    vi.stubEnv("CHOKIDAR_INTERVAL", "250");
    expect(memoryWatchMode()).toEqual(
      value === "true" || value === "1" ? { mode: "poll", intervalMs: 250 } : { mode: "node" },
    );
  });
});
