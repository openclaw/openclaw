...
describe("memory manager readonly recovery", () => {
	...
	async function expectReadonlyRetry(params: {
		firstError: unknown;
		...
		await MemoryIndexManager.prototype.sync.call(
			harness as unknown as MemoryIndexManager,
			{
				reason: "test",
			},
		);
	...
	it("does not retry non-readonly sync errors", async () => {
		...
		await expect(
			MemoryIndexManager.prototype.sync.call(
				harness as unknown as MemoryIndexManager,
				{
					reason: "test",
				},
