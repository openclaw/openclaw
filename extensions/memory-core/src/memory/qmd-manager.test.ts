...
const originalWindowsPath = (
	process.env as NodeJS.ProcessEnv & { Path?: string }
).Path;
...
describe("QmdMemoryManager", () => {
	...
	afterEach(async () => {
		...
		if (originalWindowsPath === undefined) {
			...
			(process.env as NodeJS.ProcessEnv & { Path?: string }).Path =
				originalWindowsPath;
		}
	...
	it("runs qmd sync when watched collection files change", async () => {
		...
		const initialUpdateCalls = spawnMock.mock.calls.filter(
			(call) => call[1]?.[0] === "update",
		);
	...
	it("avoids destructive rebind when qmd only reports collection names", async () => {
		...
		const removeCalls = commands.filter(
			(args) => args[0] === "collection" && args[1] === "remove",
		);
		expect(removeCalls).toHaveLength(0);

		const addCalls = commands.filter(
			(args) => args[0] === "collection" && args[1] === "add",
		);
	...
	it("migrates unscoped legacy collections before adding scoped names", async () => {
		...
		>([
			...
			[
				"memory-dir",
				{ path: path.join(workspaceDir, "memory"), pattern: "**/*.md" },
			],
		]);
		...
		spawnMock.mockImplementation((_cmd: string, args: string[]) => {
			...
			if (args[0] === "collection" && args[1] === "add") {
				...
				const pattern =
					args[args.indexOf("--glob") + 1] ??
					args[args.indexOf("--mask") + 1] ??
					"";
	...
	it("rebinds conflicting collection name when path+pattern slot is already occupied", async () => {
		...
		expect(logWarnMock).toHaveBeenCalledWith(
			expect.stringContaining("rebinding"),
		);
	});
		...
		try {
			...
			const qmdCalls = spawnMock.mock.calls.filter((call: unknown[]) => {
				...
				return (
					...
					args.some(
						(token) =>
							token === "update" || token === "search" || token === "query",
					)
				);
			});
		...
		cfg = {
			...
			memory: {
				...
				qmd: {
					...
					paths: [
						{
							path: path.join(workspaceDir, "notes-a"),
							pattern: "**/*.md",
							name: "workspace-a",
						},
						{
							path: path.join(workspaceDir, "notes-b"),
							pattern: "**/*.md",
							name: "workspace-b",
						},
					],
	...
	it("does not pin v1 fallback when only the serialized query text contains tool-not-found words", async () => {
		...
		await manager.search("hello again", {
			sessionKey: "agent:main:slack:dm:u123",
		});
	...
	it("does not pin v1 fallback when a timed out query contains tool-not-found words", async () => {
		...
		const originalRunMcporter =
			managerWithPrivate.runMcporter.bind(managerWithPrivate);
	...
	it("resolves mcporter to a direct Windows entrypoint without enabling shell mode", async () => {
		...
		try {
			...
			await fs.mkdir(shimDir, { recursive: true });
			await fs.writeFile(
				path.join(shimDir, "mcporter.cmd"),
				"@echo off\r\n",
				"utf8",
			);
			await fs.writeFile(
	...
	it("fails closed on Windows EINVAL cmd-shim failures instead of retrying through the shell", async () => {
		...
		try {
			...
			const attemptedCmdShim = (firstCallCommand ?? "")
				.toLowerCase()
				.endsWith(".cmd");
			if (attemptedCmdShim) {
				expect(
	...
	it("runs periodic embed maintenance even when regular update scheduling is disabled", async () => {
		...
		const commandCallsBefore = spawnMock.mock.calls.filter(
			(call: unknown[]) => {
	...
	it("returns collection-scoped qmd paths when session exports live under the workspace qmd directory", async () => {
		...
		const lstatSpy = vi
			...
			.mockImplementation(async (target, options) => {
				if (
					typeof target === "string" &&
					path.resolve(target) === exportedSessionPath
				) {
					return {
						isFile: () => true,
	...
	it("sets busy_timeout on qmd sqlite connections", async () => {
		...
		const db = (
			manager as unknown as { ensureDb: () => DatabaseSync }
		).ensureDb();
