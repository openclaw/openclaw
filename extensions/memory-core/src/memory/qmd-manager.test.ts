...
const MCPORTER_STATE_KEY = Symbol.for("mullusi.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("mullusi.qmdEmbedQueueTail");
...
vi.mock("mullusi/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("mullusi/plugin-sdk/file-lock")>(
    "mullusi/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: MullusiConfig }) {
