...
describe("QmdMemoryManager", () => {
  ...
  it("refreshes qmd index config with quoted collection values during update repair", async () => {
    ...
    expect(indexConfig).toContain('  "notes":');
  ...
  it("forces repair remove/add even when managed collections are still listed", async () => {
    ...
    expect(updateCalls).toBe(2);
    // Names are unscoped now: the legacy scoped collections listed (memory-root-main /
    // memory-dir-main) are removed by the scoped→unscoped migration first, then the
    // duplicate-document-constraint repair rebuilds the unscoped managed collections.
    expect(removeCalls).toEqual([
      "memory-root-main",
      "memory-dir-main",
      "memory-root",
    ...
    expect(addCalls).toEqual(["memory-root", "memory-dir", "memory-root", "memory-dir"]);
  ...
  it("keeps per-result and aggregate QMD snippet limits UTF-16 safe", async () => {
    ...
    const searchWithLimits = async (limits: {
      ...
      inner.db = {
        prepare: () => ({
          all: () => [{ collection: "workspace", path: "notes/unicode.md" }],
        }),
        close: () => {},
      };
  ...
  it("maps exported QMD artifacts to the persisted session identity", async () => {
    ...
    const identity = resolveQmdSessionArtifactIdentity({
      ...
      collection: "sessions",
      indexPath,
      searchPath: "qmd/sessions/actual-session.md",
    });
