...
const SPLITTABLE_MEMORY_EMBEDDING_BATCH_ERROR_RE =
  /(request_headers_too_large|request header fields too large|other side closed|ECONNRESET|EPIPE|UND_ERR_SOCKET|socket hang up|socket terminated|read ECONN|connection (?:reset|aborted)|\bembeddings (?:api input limit exceeded:\s*max\s+\d+\s*,\s*got\s+\d+|max input length is\s+\d+)\b|batch size is invalid|input 数组最大不得超过|不得超过\s*\d+\s*条|不得超过\s*\d+|超过\s*\d+\s*条|max\s+不得超过\s*\d+)/i;
