...
import { MemoryWatchPolicy, memoryWatchMode, type MemoryObservation } from "./watch-policy.js";
...
// fs-safe owns one worker per Node subscription. This is a domain admission
// ceiling, not a measured host limit; retiring/failed-close workers still count.
const MAX_MEMORY_WATCH_WORKERS = 16;
let memoryWatchWorkers = 0;
class MemoryWatchWorkerLimitError extends Error {
  constructor() {
    super("memory observation worker limit reached (" + MAX_MEMORY_WATCH_WORKERS + ")");
  }
}
...
export class MemoryFileWatcher {
        ...
        while (this.refreshRequested && !this.closed && !this.recovering) {
          ...
          for (const { id, group } of groups) {
            ...
            if (entry) {
              ...
              if (entry.key !== key) {
                entry.key = key;
                ready.push(entry.subscription!.update(scopes));
              }
              continue;
            }
            const mode = memoryWatchMode();
            if (mode.mode === "node" && memoryWatchWorkers >= MAX_MEMORY_WATCH_WORKERS) {
              throw new MemoryWatchWorkerLimitError();
            }
            ...
            entry = {
              ...
              workerReserved: mode.mode === "node",
            };
            if (entry.workerReserved) {
              memoryWatchWorkers++;
            }
  ...
  private dirty(group: MemoryObservation, hint: WatchDirty): void {
    if (!hint.changes) {
      this.markDirty();
      void this.refresh();
      return;
  ...
  private health(health: WatchHealth): void {
    if (health.state === "unavailable") {
      const code =
        health.failure?.operation === "watch"
          ? getFileWatchCapacityCode({ syscall: "watch", code: health.failure.code })
          : null;
      this.unavailable(health.error, code ?? undefined);
      return;
    }
    ...
    const count = facts.reduce(
      (total, fact) => total + (polling ? fact.observedDirectories : fact.directories),
      0,
    );
    warnIfMemoryWatchPressureHigh(
      this.pressure,
      count,
      polling ? "observed directories" : "registered directories",
      polling
  ...
  private unavailable(error: unknown, capacityCode?: string): void {
    ...
    const workerLimit = error instanceof MemoryWatchWorkerLimitError;
    this.degraded ||= Boolean(capacityCode) || workerLimit;
    this.options.onUnavailable();
    this.markDirty();
    log.warn(
      workerLimit
        ? String(error) + "; watching disabled, memory will refresh on search"
        : capacityCode
  ...
  private retire(entries: Observation[]): Promise<void> {
    ...
    const closes = entries.map((entry) => {
      ...
      const close = async () => {
        ...
        // A constructor rejection before returning a handle created no worker.
        // Otherwise only successful actual close releases admission, not abort,
        // unavailable health, logical removal, or a rejected close.
        if (entry.workerReserved) {
          entry.workerReserved = false;
          memoryWatchWorkers--;
        }
      };
