...
async function runEmbeddingOperationWithTimeout<T>(params: {
  ...
  signal?: AbortSignal;
  /**
   * Owned phases (managed local-service readiness) pause this watchdog through
   * the shared search deadline control so a slow readyTimeoutMs-owned startup
   * cannot consume the embedding budget. Cancellation via params.signal is
   * unaffected.
   */
