...
describe("memory manager cache hydration", () => {
	...
	beforeAll(async () => {
		...
		({
			...
			MemoryIndexManager: RawMemoryIndexManager,
	...
	it("evicts cached managers during global teardown", async () => {
		...
		const secondManager = await RawMemoryIndexManager.get({
			cfg,
			agentId: "main",
		});
	...
	it("does not identity-cache status-only managers", async () => {
		...
		const first = await RawMemoryIndexManager.get({
			cfg,
			agentId: "main",
			purpose: "status",
		});
		const second = await RawMemoryIndexManager.get({
			cfg,
			agentId: "main",
			purpose: "status",
		});
