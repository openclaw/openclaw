...
export class MemoryManagerRegistry<T extends ClosableMemoryManager> {
  ...
  async acquire(
    ...
    return await this.runScopeOperation(params, async () => {
      ...
      if (cachedManager && (await prepared.reuse(cachedManager))) {
        ...
        this.assertManagerAvailable(prepared.key, cachedManager);
      ...
      if (params.purpose !== "search") {
        ...
        if (existing) {
          this.assertManagerAvailable(prepared.key, existing);
          return existing;
        }
      ...
      try {
        ...
        this.assertManagerAvailable(prepared.key, candidate, { requireCached: false });
      } catch (err) {
        try {
  ...
  private assertManagerAvailable(
    key: string,
    manager: T,
    options: { requireCached?: boolean } = {},
  ): void {
    const owner = this.managers.get(manager);
    ...
    if (
      !owner ||
      owner.retiring ||
      (options.requireCached !== false && this.cache.get(key) !== manager) ||
      reload?.retireRuntime ||
      (reload && this.getProbeOwners(manager).some((adapter) => reload.adapters.has(adapter)))
    ) {
      throw new MemoryManagerReloadError();
    }
  }
