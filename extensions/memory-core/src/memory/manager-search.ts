...
export async function searchPathKeyword(params: {
  ...
  const loadFilteredLexicalRows = (matchQuery: string | null, terms: string[]) => {
    ...
    if (!filter.normalizedClause) {
      return params.db
        ...
        .all(...queryParams, params.limit) as PathLexicalRow[];
    }
    ...
    return params.db
      .prepare(
        `WITH path_candidates AS MATERIALIZED (\n` +
          ...
          `   WHERE 1=1${filter.normalizedClause}\n` +
          `)\n` +
      ...
      .all(...queryParams, ...filter.normalizedParams, params.limit) as PathLexicalRow[];
  };
  const loadLexicalRows = (lexicalPlan: (typeof pathPlans)[number]) => {
    if (lexicalPlan.matchQuery) {
      try {
        const rows = loadFilteredLexicalRows(lexicalPlan.matchQuery, lexicalPlan.substringTerms);
        ...
        const rows = loadFilteredLexicalRows(null, allTerms);
        return { rows, usedMatch: false };
      }
    }
    const rows = loadFilteredLexicalRows(null, lexicalPlan.substringTerms);
    return { rows, usedMatch: false };
  };
  ...
  const lexicalResults = [...lexicalById.values()];
