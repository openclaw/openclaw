...
const MEMORY_INDEX_MANAGER_CACHE_KEY = Symbol.for(
	"openclaw.memoryIndexManagerCache",
);
	...
	implements MemorySearchManager
{
	...
	async search(
		...
		if (!this.provider) {
			...
			const merged = [...seenIds.values()].toSorted(
				(a, b) => b.score - a.score,
			);
	...
	private hasIndexedContent(): boolean {
		const chunkRow = this.db
			.prepare(`SELECT 1 as found FROM chunks LIMIT 1`)
			.get() as
			| {
					found?: number;
			  }
		...
		const ftsRow = this.db
			.prepare(`SELECT 1 as found FROM ${FTS_TABLE} LIMIT 1`)
			.get() as
			| {
					found?: number;
			  }
	...
	async sync(params?: {
		...
		if (this.syncing) {
			if (
				params?.sessionFiles?.some(
					(sessionFile) => sessionFile.trim().length > 0,
				)
			) {
	...
	status(): MemoryProviderStatus {
		...
		const searchMode =
			this.provider || !this.providerInitialized ? "hybrid" : "fts-only";
