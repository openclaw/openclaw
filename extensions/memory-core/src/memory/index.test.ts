						...
						? {
								embedBatchInputs: async (
									...
									return inputs.map((input) => {
										const inlineData = input.parts?.find(
											(part) => part.type === "inline-data",
										);
										if (
											inlineData?.type === "inline-data" &&
											inlineData.data.length > 9000
										) {
...
describe("memory index", () => {
	...
	beforeAll(async () => {
		...
		indexSourceChangePath = path.join(
			workspaceDir,
			"index-source-change.sqlite",
		);
	...
	async function getFreshManager(cfg: TestCfg): Promise<MemoryIndexManager> {
		const { getRequiredMemoryIndexManager } = await import(
	...
	it("indexes multimodal image and audio files from extra paths with Gemini structured inputs", async () => {
		...
		expect(
			imageResults.some((result) => result.path.endsWith("diagram.png")),
		).toBe(true);

		const audioResults = await manager.search("audio");
		expect(
			audioResults.some((result) => result.path.endsWith("meeting.wav")),
		).toBe(true);
	});
	...
	it("skips oversized multimodal inputs without aborting sync", async () => {
		...
		const cfg = createCfg({
			storePath: path.join(
				workspaceDir,
				`index-oversize-${randomUUID()}.sqlite`,
			),
			provider: "gemini",
			model: "gemini-embedding-2-preview",
			extraPaths: [mediaDir],
		...
		const imageResults = await manager.search("image");
		expect(
			imageResults.some((result) => result.path.endsWith("huge.png")),
		).toBe(false);

		const alphaResults = await manager.search("alpha");
		expect(
			alphaResults.some((result) =>
				result.path.endsWith("memory/2026-01-12.md"),
			),
		).toBe(true);
	...
	it("reindexes a multimodal file after a transient mid-sync disappearance", async () => {
				...
				if (typeof targetPath === "string" && targetPath === imagePath) {
					...
					if (imageReads === 2) {
						const err = Object.assign(
							new Error(
								`ENOENT: no such file or directory, open '${imagePath}'`,
							),
							{
								code: "ENOENT",
							},
		...
		expect(results.some((result) => result.path.endsWith("diagram.png"))).toBe(
			true,
		);
	...
	it("targets explicit session files during post-compaction sync", async () => {
		...
		const storePath = path.join(
			workspaceDir,
			`index-targeted-${randomUUID()}.sqlite`,
		);
		const previousStateDir = process.env.OPENCLAW_STATE_DIR;
		process.env.OPENCLAW_STATE_DIR = stateDir;
		...
		await fs.writeFile(
			...
			`${JSON.stringify({
				...
				message: {
					role: "user",
					content: [{ type: "text", text: "first transcript v1" }],
				},
			})}\n`,
		);
		await fs.writeFile(
			...
			`${JSON.stringify({
				...
				message: {
					role: "user",
					content: [{ type: "text", text: "second transcript v1" }],
				},
			})}\n`,
		);
		...
		try {
			...
			const secondOriginalHash = getSessionHash(
				"sessions/targeted-second.jsonl",
			);
			...
			await fs.writeFile(
				...
				`${JSON.stringify({
					...
					message: {
						role: "user",
						content: [
							{ type: "text", text: "first transcript v2 after compaction" },
						],
					},
				})}\n`,
			);
				...
				`${JSON.stringify({
					...
					message: {
						...
						content: [
							{
								type: "text",
								text: "second transcript v2 should stay untouched",
							},
						],
					},
				})}\n`,
			...
			db.prepare = ((sql: string) => {
				...
				if (sql === `SELECT path, hash FROM files WHERE source = ?`) {
					if (!statement.all) {
						throw new Error(
							"expected sqlite statement.all for bulk session state query",
						);
					}
			...
			expect(getSessionHash("sessions/targeted-first.jsonl")).not.toBe(
				firstOriginalHash,
			);
			expect(getSessionHash("sessions/targeted-second.jsonl")).toBe(
				secondOriginalHash,
			);
	...
	it("preserves unrelated dirty sessions after targeted post-compaction sync", async () => {
		const stateDir = path.join(
			fixtureRoot,
			`state-targeted-dirty-${randomUUID()}`,
		);
		const sessionDir = path.join(stateDir, "agents", "main", "sessions");
		const firstSessionPath = path.join(
			sessionDir,
			"targeted-dirty-first.jsonl",
		);
		const secondSessionPath = path.join(
			sessionDir,
			"targeted-dirty-second.jsonl",
		);
		const storePath = path.join(
			workspaceDir,
			`index-targeted-dirty-${randomUUID()}.sqlite`,
		);
		const previousStateDir = process.env.OPENCLAW_STATE_DIR;
		process.env.OPENCLAW_STATE_DIR = stateDir;
		...
		try {
			...
			const firstOriginalHash = getSessionHash(
				"sessions/targeted-dirty-first.jsonl",
			);
			const secondOriginalHash = getSessionHash(
				"sessions/targeted-dirty-second.jsonl",
			);
			...
			await fs.writeFile(
				...
				`${JSON.stringify({
					...
					message: {
						role: "user",
						content: [
							{ type: "text", text: "second transcript v2 still pending" },
						],
					},
				})}\n`,
			);
			...
			expect(getSessionHash("sessions/targeted-dirty-first.jsonl")).not.toBe(
				firstOriginalHash,
			);
			expect(getSessionHash("sessions/targeted-dirty-second.jsonl")).toBe(
				secondOriginalHash,
			);
			...
			expect(getSessionHash("sessions/targeted-dirty-second.jsonl")).not.toBe(
				secondOriginalHash,
			);
			await manager.close?.();
		} finally {
	...
	it("queues targeted session sync when another sync is already in progress", async () => {
		const stateDir = path.join(
			fixtureRoot,
			`state-targeted-queued-${randomUUID()}`,
		);
		const sessionDir = path.join(stateDir, "agents", "main", "sessions");
		const sessionPath = path.join(sessionDir, "targeted-queued.jsonl");
		const storePath = path.join(
			workspaceDir,
			`index-targeted-queued-${randomUUID()}.sqlite`,
		);
		const previousStateDir = process.env.OPENCLAW_STATE_DIR;
		process.env.OPENCLAW_STATE_DIR = stateDir;
		...
		await fs.writeFile(
			...
			`${JSON.stringify({
				...
				message: {
					role: "user",
					content: [{ type: "text", text: "queued transcript v1" }],
				},
			})}\n`,
		);
		...
		try {
			...
			const originalRunSync =
				internal.runSyncWithReadonlyRecovery.bind(manager);
			...
			await fs.writeFile(
				...
				`${JSON.stringify({
					...
					message: {
						role: "user",
						content: [
							{ type: "text", text: "queued transcript v2 after compaction" },
						],
					},
				})}\n`,
			);
			...
			expect(getSessionHash("sessions/targeted-queued.jsonl")).not.toBe(
				originalHash,
			);
			await manager.close?.();
		} finally {
	...
	it("runs a full reindex after fallback activates during targeted sync", async () => {
		const stateDir = path.join(
			fixtureRoot,
			`state-targeted-fallback-${randomUUID()}`,
		);
		const sessionDir = path.join(stateDir, "agents", "main", "sessions");
		const sessionPath = path.join(sessionDir, "targeted-fallback.jsonl");
		const storePath = path.join(
			workspaceDir,
			`index-targeted-fallback-${randomUUID()}.sqlite`,
		);
		const previousStateDir = process.env.OPENCLAW_STATE_DIR;
		process.env.OPENCLAW_STATE_DIR = stateDir;
		...
		await fs.writeFile(
			...
			`${JSON.stringify({
				...
				message: {
					role: "user",
					content: [{ type: "text", text: "fallback transcript v1" }],
				},
			})}\n`,
		);
		...
		try {
			...
			const originalShouldFallbackOnError =
				internal.shouldFallbackOnError.bind(manager);
			const originalActivateFallbackProvider =
				internal.activateFallbackProvider.bind(manager);
			...
			expect(activateFallbackProvider).toHaveBeenCalledWith(
	...
	it("snapshots builtin file hashes with a single sqlite query per sync", async () => {
		...
		const cfg = createCfg({
			storePath: path.join(
				workspaceDir,
				`index-prepare-reuse-${randomUUID()}.sqlite`,
			),
			onSearch: false,
		});
	...
	it("uses a single sqlite aggregation query for status counts", async () => {
		const cfg = createCfg({
			storePath: path.join(
				workspaceDir,
				`index-status-aggregate-${randomUUID()}.sqlite`,
			),
			sources: ["memory", "sessions"],
			sessionMemory: true,
			onSearch: false,
		...
		await fs.writeFile(
			...
			`${JSON.stringify({
				...
				message: {
					role: "user",
					content: [{ type: "text", text: "session status line" }],
				},
			})}\n`,
		);
		...
		db.prepare = ((sql: string) => {
			if (
				sql.includes(
					`SELECT 'files' AS kind, source, COUNT(*) as c FROM files`,
				) &&
			...
			if (
				...
				sql ===
					`SELECT source, COUNT(*) as c FROM files WHERE 1=1 GROUP BY source` ||
				sql ===
					`SELECT source, COUNT(*) as c FROM chunks WHERE 1=1 GROUP BY source`
			) {
		...
		try {
			...
			expect(
				status.sourceCounts?.find((entry) => entry.source === "sessions")
					?.files,
			).toBeGreaterThan(0);
		} finally {
			db.prepare = originalPrepare;
	...
	it("reindexes when extraPaths change", async () => {
		const storePath = path.join(
			workspaceDir,
			`index-scope-extra-${randomUUID()}.sqlite`,
		);
		...
		expect(
			results.some((result) => result.path.endsWith("scope-extra-b/b.md")),
		).toBe(true);
		expect(
			results.some((result) => result.path.endsWith("scope-extra-a/a.md")),
		).toBe(false);
		await secondManager.close?.();
	});
	...
	it("reindexes when multimodal settings change", async () => {
		const storePath = path.join(
			workspaceDir,
			`index-scope-multimodal-${randomUUID()}.sqlite`,
		);
		...
		expect(
			results.some((result) => result.path.endsWith("scope-media/diagram.png")),
		).toBe(true);
		await secondManager.close?.();
	});
	...
	it("triggers full reindex and cleans up old-model FTS rows when switching from provider to FTS-only", async () => {
		const sharedStorePath = path.join(
			workspaceDir,
			"index-provider-to-fts-only.sqlite",
		);

		// Phase 1: sync with a real provider — FTS rows stored under model = "mock-embed"
		const providerCfg = createCfg({
			storePath: sharedStorePath,
			hybrid: { enabled: true },
		});
		const providerResult = await getMemorySearchManager({
			cfg: providerCfg,
			agentId: "main",
		});
		...
		const providerFtsRows = providerDb
			.prepare(
				"SELECT COUNT(*) as c FROM chunks_fts WHERE model = 'mock-embed'",
			)
			.get();
		...
		const ftsOnlyResult = await getMemorySearchManager({
			cfg: ftsOnlyCfg,
			agentId: "main",
		});
