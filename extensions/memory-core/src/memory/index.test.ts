...
describe("memory index", () => {
  ...
  it("keeps overlapping keyword-backed hybrid hits when all scores are below minScore", async () => {
    const cfg = createCfg({
      storePath: path.join(workspaceDir, "index-hybrid-overlap-relaxed-fallback.sqlite"),
      minScore: 0.35,
      hybrid: { enabled: true, vectorWeight: 0.7, textWeight: 0.3 },
    });
    const manager = await getFreshManager(cfg);
  ...
  it("preserves hybrid merger order when retaining keyword-only hits", async () => {
    const cfg = createCfg({
      storePath: path.join(workspaceDir, "index-hybrid-keyword-preserves-merge-order.sqlite"),
      minScore: 0.35,
      hybrid: { enabled: true, vectorWeight: 0.7, textWeight: 0.3 },
    });
    const manager = await getFreshManager(cfg);
