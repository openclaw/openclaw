...
vi.mock("mullusi/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("mullusi/plugin-sdk/memory-core-host-engine-foundation")
  >("mullusi/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
import { resolveMemoryBackendConfig } from "mullusi/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager slugified path resolution", () => {
  ...
  async function createManager(params?: { cfg?: MullusiConfig }) {
  ...
  beforeEach(async () => {
    ...
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-qmd-slugified-"));
