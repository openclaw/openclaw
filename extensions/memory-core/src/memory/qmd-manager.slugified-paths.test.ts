...
describe("QmdMemoryManager slugified path resolution", () => {
			...
			prepare: (query: string) => ({
				all: (...args: unknown[]) => {
					...
					if (query.includes("collection = ? AND active = 1")) {
						...
						return (params.allPaths ?? [params.actualPath]).map(
							(pathValue) => ({
								path: pathValue,
							}),
						);
	...
	beforeEach(async () => {
		...
		tmpRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "openclaw-qmd-slugified-"),
		);
	...
	it("maps slugified workspace qmd URIs back to the indexed filesystem path", async () => {
		const actualRelative =
			"extra-docs/Category/Sub Category/Topic Name/Topic Name.md";
		...
		await expect(
			manager.readFile({ relPath: results[0]?.path }),
		).resolves.toEqual({
			path: actualRelative,
			text: "line-1\nline-2\nline-3",
		});
