...
export function createTlonApprovalRuntime(params: {
	...
	const addToChannelAllowlist = async (
		ship: string,
		channelNest: string,
	): Promise<void> => {
		const normalizedShip = normalizeShip(ship);
	...
	const isShipBlocked = async (ship: string): Promise<boolean> => {
		const normalizedShip = normalizeShip(ship);
		try {
			const blocked = (await api.scry("/chat/blocked.json")) as
				| string[]
				| undefined;
			return (
				Array.isArray(blocked) &&
				blocked.some((item) => normalizeShip(item) === normalizedShip)
			);
		} catch (err) {
	...
	const queueApprovalRequest = async (
		...
		const existingIndex = approvals.findIndex(
			(item) =>
				...
				(approval.type !== "channel" ||
					item.channelNest === approval.channelNest) &&
	...
	const handleApprovalResponse = async (text: string): Promise<boolean> => {
		...
		if (parsed.action === "approve") {
			...
			await sendOwnerNotification(
				formatApprovalConfirmation(approval, "block"),
			);
		} else {
		...
		setPendingApprovals(
			removePendingApproval(getPendingApprovals(), approval.id),
		);
		await savePendingApprovals();
		return true;
	};
	...
	const handleAdminCommand = async (text: string): Promise<boolean> => {
		...
		switch (command.type) {
			case "blocked": {
				...
				runtime.log?.(
					`[tlon] Owner requested blocked ships list (${blockedShips.length} ships)`,
				);
				return true;
			}
			case "pending":
			...
			case "unblock": {
				...
				await sendOwnerNotification(
					success
						? `Unblocked ${shipToUnblock}.`
						: `Failed to unblock ${shipToUnblock}.`,
				);
				return true;
			}
		}
