...
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "bot/plugin-sdk";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as BotConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Docs: https://docs.hanzo.bot/concepts/session#secure-dm-mode`;
