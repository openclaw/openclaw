import type { OpenClawConfig, ReplyPayload, RuntimeEnv } from "../../api.js";
...
export async function monitorTlonProvider(
	...
	try {
		...
		runtime.log?.(
			`[tlon] Could not fetch nickname: ${error?.message ?? String(error)}`,
		);
	}
	...
	groupChannels = mergeUniqueStrings(
		groupChannels,
				...
				deliver: async (payload: ReplyPayload) => {
					...
					if (isGroup && groupChannel) {
						...
						if (parentId) {
							...
							runtime.log?.(
								`[tlon] Now tracking thread for future replies: ${parentId}`,
							);
						}
					} else {
						await sendDm({
	...
	const handleChatFirehose = async (event: any) => {
		try {
			...
			if (isOwner(senderShip) && isApprovalResponse(messageText)) {
				...
				if (handled) {
					runtime.log?.(
						`[tlon] Processed approval response from owner: ${messageText}`,
					);
					return;
				}
			}
			...
			if (isOwner(senderShip) && isAdminCommand(messageText)) {
				...
				if (handled) {
					runtime.log?.(
						`[tlon] Processed admin command from owner: ${messageText}`,
					);
					return;
				}
			}
		...
		// Always subscribe so we can hot-reload the setting via settings store
		{
			...
			const processPendingInvites = async (foreigns: Foreigns) => {
				...
				for (const [groupFlag, foreign] of Object.entries(foreigns)) {
					...
					const isAllowed = isGroupInviteAllowed(
						inviterShip,
						effectiveGroupInviteAllowlist,
					);
