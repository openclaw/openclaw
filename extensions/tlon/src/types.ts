...
function resolveTlonChannelConfig(
	cfg: OpenClawConfig,
): TlonAccountConfig | undefined {
...
function resolveMergedTlonAccountConfig(
	...
	return resolveMergedAccountConfig<
		Record<string, unknown> & TlonAccountConfig
	>({
		channelConfig: (channel ?? {}) as Record<string, unknown> &
			TlonAccountConfig,
...
export function resolveTlonAccount(
	...
	const allowPrivateNetwork = (merged.allowPrivateNetwork ?? null) as
		| boolean
		| null;
	...
	const autoDiscoverChannels = (merged.autoDiscoverChannels ?? null) as
		| boolean
		| null;
	const showModelSignature = (merged.showModelSignature ?? null) as
		| boolean
		| null;
	const autoAcceptDmInvites = (merged.autoAcceptDmInvites ?? null) as
		| boolean
		| null;
	const autoAcceptGroupInvites = (merged.autoAcceptGroupInvites ?? null) as
		| boolean
		| null;
...
export function listTlonAccountIds(cfg: OpenClawConfig): string[] {
	...
	return listCombinedAccountIds({
		configuredAccountIds: Object.keys(base.accounts ?? {}).map(
			normalizeAccountId,
		),
