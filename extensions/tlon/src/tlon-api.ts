...
function isHostedShipUrl(shipUrl: string): boolean {
	try {
		...
		return (
			hostname.endsWith("tlon.network") ||
			hostname.endsWith(".test.tlon.systems")
		);
	} catch {
		return (
			shipUrl.endsWith("tlon.network") || shipUrl.endsWith(".test.tlon.systems")
		);
	}
}
