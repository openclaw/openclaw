...
vi.mock("./channel-ops.js", () => ({
  ensureUrbitChannelOpen: vi.fn().mockResolvedValue(void 0),
  pokeUrbitChannel: vi.fn().mockResolvedValue(void 0),
...
describe("UrbitSSEClient", () => {
  ...
  describe("subscribe", () => {
    it("sends subscriptions added after connect", async () => {
      ...
      const body = JSON.parse(callArgs.init?.body);
  ...
  describe("reconnection", () => {
    ...
    it("resets reconnect attempts on successful connect", async () => {
      ...
      const client = new UrbitSSEClient("https://example.com", "urbauth-~zod=123", {
        autoReconnect: false
        // Disable to prevent reconnect loop
      });
  ...
  describe("event acking", () => {
    it("tracks lastHeardEventId and ackThreshold", () => {
      const client = new UrbitSSEClient("https://example.com", "urbauth-~zod=123");
      const lastHeardEventId = client.lastHeardEventId;
      const ackThreshold = client.ackThreshold;
  ...
  describe("constructor", () => {
    ...
    it("sets default reconnection parameters", () => {
      ...
      expect(client.reconnectDelay).toBe(1e3);
      expect(client.maxReconnectDelay).toBe(3e4);
    });
    it("allows overriding reconnection parameters", () => {
      ...
      expect(client.maxReconnectDelay).toBe(1e4);
    });
  });
});
