...
describe("Security: Group Invite Allowlist", () => {
	describe("isGroupInviteAllowed", () => {
		...
		it("normalizes ship names (with/without ~ prefix)", () => {
			...
			expect(
				isGroupInviteAllowed("~nocsyx-lassul", allowlistWithoutTilde),
			).toBe(true);
		});
...
describe("Security: Bot Mention Detection", () => {
	describe("isBotMentioned", () => {
		...
		it("detects direct ship mention", () => {
			...
			expect(isBotMentioned("~sampel-palnet can you help?", botShip)).toBe(
				true,
			);
			expect(isBotMentioned("hello ~sampel-palnet how are you", botShip)).toBe(
				true,
			);
		});
...
describe("Security: Channel Authorization Logic", () => {
	...
	it("settings should override file config for channel rules", () => {
		...
		const fileRules: Record<string, ChannelRule> = {
			"chat/~zod/test": { mode: "restricted" },
		};
		const settingsRules: Record<string, ChannelRule> = {
			"chat/~zod/test": { mode: "open" },
		};
	...
	async function resolveAllCitesForPoC(
		...
		for (const cite of cites) {
			...
			const data = (await api.scry(
				`/channels/v4/${cite.nest}/posts/post/${cite.postId}.json`,
			)) as {
				essay?: { content?: unknown };
			};
			const text = data?.essay?.content
				? extractMessageText(data.essay.content)
				: "";
			if (text) {
	...
	it("resolves channel cites after sender authorization passes", async () => {
		...
		const api = {
			scry: vi.fn(async (path: string) => {
				expect(path).toBe(
					`/channels/v4/${secretNest}/posts/post/${postId}.json`,
				);
				return {
					essay: {
						content: [{ inline: ["TOP-SECRET: migration key is rotate-me"] }],
					},
				};
			}),
		};
		...
		expect(messageText).toContain(
			"> ~victim-ship wrote: TOP-SECRET: migration key is rotate-me",
		);
	});
...
describe("Security: Sender Role Identification", () => {
	...
	describe("impersonation prevention", () => {
		it("approved user cannot get [owner] label through ship name tricks", () => {
			...
			expect(getSenderRole("~nocsyx-lassul-fake", "~nocsyx-lassul")).toBe(
				"user",
			);
			expect(getSenderRole("~fake-nocsyx-lassul", "~nocsyx-lassul")).toBe(
				"user",
			);
		});
