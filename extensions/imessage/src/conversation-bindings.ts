...
type IMessageConversationBindingManager = {
	...
	getByConversationId: (
		conversationId: string,
	) => IMessageConversationBindingRecord | undefined;
	listBySessionKey: (
		targetSessionKey: string,
	) => IMessageConversationBindingRecord[];
...
function toSessionBindingTargetKind(
	raw: IMessageBindingTargetKind,
): BindingTargetKind {
	return raw === "subagent" ? "subagent" : "session";
}

function toIMessageTargetKind(
	raw: BindingTargetKind,
): IMessageBindingTargetKind {
...
function toSessionBindingRecord(
	...
	const idleExpiresAt =
		defaults.idleTimeoutMs > 0
			? record.lastActivityAt + defaults.idleTimeoutMs
			: undefined;
	const maxAgeExpiresAt =
		defaults.maxAgeMs > 0 ? record.boundAt + defaults.maxAgeMs : undefined;
	const expiresAt =
...
export function createIMessageConversationBindingManager(params: {
	...
	const manager: IMessageConversationBindingManager = {
		...
		unbindBySessionKey: (targetSessionKey) => {
			...
			for (const record of [
				...getState().bindingsByAccountConversation.values(),
			]) {
				if (
					record.accountId !== accountId ||
					record.targetSessionKey !== targetSessionKey
	...
	const sessionBindingAdapter: SessionBindingAdapter = {
		...
		bind: async (input) => {
			if (
				input.conversation.channel !== "imessage" ||
			...
			return bound
				? toSessionBindingRecord(bound, { idleTimeoutMs, maxAgeMs })
				: null;
		},
		...
		resolveByConversation: (ref) => {
			...
			return found
				? toSessionBindingRecord(found, { idleTimeoutMs, maxAgeMs })
				: null;
		},
		touch: (bindingId, at) => {
