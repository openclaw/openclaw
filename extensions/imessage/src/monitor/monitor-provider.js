...
async function monitorIMessageProvider(opts = {}) {
  ...
  const groupAllowFrom = normalizeAllowList(
    opts.groupAllowFrom ?? imessageCfg.groupAllowFrom ?? (imessageCfg.allowFrom && imessageCfg.allowFrom.length > 0 ? imessageCfg.allowFrom : [])
  );
  ...
  async function handleMessageNow(message) {
    ...
    if (decision.kind === "drop") {
      const isLoopDrop = decision.reason === "echo" || decision.reason === "self-chat echo" || decision.reason === "reflected assistant content" || decision.reason === "from me";
      if (isLoopDrop) {
    ...
    if (shouldLogVerbose()) {
      ...
      logVerbose(
        `imessage inbound: chatId=${chatId ?? "unknown"} from=${ctxPayload.From} len=${String(ctxPayload.Body ?? "").length} preview="${preview}"`
      );
    }
