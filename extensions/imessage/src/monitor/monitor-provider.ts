...
export async function monitorIMessageProvider(
	...
	async function handleMessageNow(message: IMessagePayload) {
		...
		const conversationKey =
			chatId != null ? `group:${chatId}` : `dm:${senderForKey}`;
		...
		if (
			...
			loopRateLimiter.isRateLimited(rateLimitKey)
		) {
			logVerbose(
				`imessage: rate-limited conversation ${conversationKey} (echo loop detected)`,
			);
			return;
		}
...
export const resolveIMessageRuntimeGroupPolicy =
	resolveOpenProviderRuntimeGroupPolicy;
