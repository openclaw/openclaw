...
import { normalizeScpRemoteHost } from "mullusi/plugin-sdk/host-runtime";
import { waitForTransportReady } from "mullusi/plugin-sdk/infra-runtime";
import { isInboundPathAllowed, kindFromMime } from "mullusi/plugin-sdk/media-runtime";
import {
...
import { dispatchInboundMessage } from "mullusi/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "mullusi/plugin-sdk/reply-runtime";
import { danger, logVerbose, shouldLogVerbose, warn } from "mullusi/plugin-sdk/runtime-env";
 ...
 *   exec ssh -T mullusi@192.168.64.3 /opt/homebrew/bin/imsg "$@"
...
async function detectRemoteHostFromCliPath(cliPath: string): Promise<string | undefined> {
  try {
    ...
    // Match user@host pattern first (e.g., mullusi@192.168.64.3)
