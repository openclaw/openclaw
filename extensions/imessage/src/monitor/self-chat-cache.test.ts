...
describe("createSelfChatCache", () => {
	...
	it("expires entries after the ttl window", () => {
		...
		expect(cache.has({ ...directLookup, text: "hello", createdAt: 123 })).toBe(
			false,
		);
	});
	...
	it("evicts older entries when the cache exceeds its cap", () => {
		...
		expect(
			cache.has({ ...directLookup, text: "message-0", createdAt: 0 }),
		).toBe(false);
		expect(
			cache.has({ ...directLookup, text: "message-512", createdAt: 512 }),
		).toBe(true);
	});
	...
	it("does not collide long texts that differ only in the middle", () => {
		...
		expect(
			cache.has({ ...directLookup, text: longTextA, createdAt: 123 }),
		).toBe(true);
		expect(
			cache.has({ ...directLookup, text: longTextB, createdAt: 123 }),
		).toBe(false);
	});
});
