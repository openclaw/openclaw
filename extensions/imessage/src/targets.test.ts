...
describe("imessage group policy", () => {
	it("uses generic channel group policy helpers", () => {
		...
		expect(
			resolveIMessageGroupRequireMention({ cfg, groupId: "chat:family" }),
		).toBe(false);
		expect(
			resolveIMessageGroupRequireMention({ cfg, groupId: "chat:other" }),
		).toBe(true);
		expect(
			resolveIMessageGroupToolPolicy({ cfg, groupId: "chat:family" }),
		).toEqual({
			deny: ["exec"],
		});
		expect(
			resolveIMessageGroupToolPolicy({ cfg, groupId: "chat:other" }),
		).toEqual({
			allow: ["message.send"],
		});
	});
	...
	it("marks unknown rpc subcommand as fatal", async () => {
		const createIMessageRpcClientMock = vi
			...
			.mockResolvedValue({
				...
				ReturnType<typeof clientModule.createIMessageRpcClient>
			>);
