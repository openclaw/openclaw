...
function baseCfg(): MullusiConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
    ...
    messages: {
      groupChat: { mentionPatterns: ["@mullusi"] },
    },
...
function buildDispatchContextPayload(params: { cfg: MullusiConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@mullusi hello",
      bodyText: "@mullusi hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
