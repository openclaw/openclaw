...
import { O as promptAccountId, Q as splitSetupEntries, v as mergeAllowFromEntries } from "./setup-wizard-helpers-BszlehmC.js";
import { t as promptChannelAccessConfig } from "./setup-group-access-C3JVwPsk.js";
...
import { r as resolveMatrixEnvAuthReadiness } from "./env-auth-M9RzhDwp.js";
import { i as resolveMatrixAccount, r as resolveDefaultMatrixAccountId, t as listMatrixAccountIds } from "./accounts-BLqmzEWw.js";
import { r as moveSingleMatrixAccountConfigToNamedAccount, t as resolveMatrixSetupDmAllowFrom } from "./setup-dm-policy-CpwtBfRA.js";
import { t as resolveMatrixConfigFieldPath } from "./config-paths-DLEnzEzV.js";
import { t as updateMatrixAccountConfig } from "./config-update-xadELhXY.js";
import { n as validateMatrixHomeserverUrl, r as isPrivateOrLoopbackHost, t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-B_D427nZ.js";
...
async function promptMatrixAllowFrom(params) {
	...
	while (true) {
		...
		if (pending.length > 0) {
			const { resolveMatrixTargets } = await import("./resolve-targets-zbh-hntT.js");
	...
	if (accessConfig) if (accessConfig.policy !== "allowlist") next = setMatrixGroupPolicy(next, accessConfig.policy, params.accountId);
		...
		if (accessConfig.entries.length > 0) try {
			...
			for (const entry of accessConfig.entries) {
				...
				const { listMatrixDirectoryGroupsLive } = await import("./directory-live-BDW5vVgk.js");
...
const matrixOnboardingAdapter = {
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import("./setup-bootstrap-DFma0aUy.js");
