import { l as onAgentEvent, t as clearAgentRunContext } from "./agent-events-G96-vmZx.js";
import { n as onSessionTranscriptUpdate } from "./transcript-events-DBR6CAJi.js";
import { n as onSessionLifecycleEvent } from "./session-lifecycle-events-Df0Iylxr.js";
import { r as onHeartbeatEvent } from "./heartbeat-events-C1OIvEdU.js";
...
function startGatewayEventSubscriptions(params) {
	...
	const getAgentEventHandler = () => {
		agentEventHandlerPromise ??= Promise.all([import("./server-chat-MyQeF2-S.js"), import("./server-session-key-Cbity020.js")]).then(([{ createAgentEventHandler }, { resolveSessionKeyForRun }]) => createAgentEventHandler({
	...
	const getTranscriptUpdateHandler = () => {
		transcriptUpdateHandlerPromise ??= import("./server-session-events-BdgLiyl5.js").then(({ createTranscriptUpdateBroadcastHandler }) => createTranscriptUpdateBroadcastHandler({
			broadcastToConnIds: params.broadcastToConnIds,
			sessionEventSubscribers: params.sessionEventSubscribers,
	...
	const getLifecycleEventHandler = () => {
		lifecycleEventHandlerPromise ??= import("./server-session-events-BdgLiyl5.js").then(({ createLifecycleEventBroadcastHandler }) => createLifecycleEventBroadcastHandler({
			broadcastToConnIds: params.broadcastToConnIds,
			sessionEventSubscribers: params.sessionEventSubscribers
		}));
