...
import { r as resolveAgentConfig } from "./agent-scope-config-DKR4wP5w.js";
import "./agent-scope-ByE5d_BM.js";
import { X as resolveOwnerDisplaySetting } from "./io-q2yb7vWQ.js";
import { l as listDeliverableMessageChannels } from "./message-channel-CAROpxkS.js";
import { n as buildMemoryPromptSection } from "./memory-state-Ta1XBGV3.js";
import { n as normalizeStructuredPromptSection, t as normalizePromptCapabilityIds } from "./prompt-cache-stability-B8KWL9Hu.js";
import { n as SILENT_REPLY_TOKEN } from "./tokens-BBeT4wJ7.js";
import { i as isKnownNativeApprovalPromptChannel, r as hasNativeApprovalPromptRuntimeCapability } from "./native-approval-prompt-BZadHGNa.js";
import { t as sanitizeForPromptLiteral } from "./sanitize-for-prompt-D8qu449B.js";
import { n as buildTtsSystemPromptHint } from "./tts-runtime-DBMO6awq.js";
import "./tts-D14cTPgc.js";
import { t as SYSTEM_PROMPT_CACHE_BOUNDARY } from "./system-prompt-cache-boundary-CGFRRusC.js";
import { t as buildModelAliasLines } from "./model-alias-lines-DeKIuu-u.js";
...
function buildMessagingSection(params) {
	...
	return [
		...
		hasSessionsSpawn ? hasSubagents ? "- Sub-agent orchestration → use `sessions_spawn(...)` to start delegated work; use `subagents(action=list|steer|kill)` to manage already-spawned children." : "- Sub-agent orchestration → use `sessions_spawn(...)` to start delegated work." : hasSubagents ? "- Sub-agent orchestration → use `subagents(action=list|steer|kill)` to manage already-spawned children." : "",
...
function buildAgentSystemPrompt(params) {
	...
	const lines = [cacheStablePromptPrefix(hashStablePromptInput({
		...
		const lines = [
			...
			...nativeCommandGuidanceLines,
			...availableTools.has("continue_delegate") ? ["For background, delayed, silent, or compaction-aware delegate work, prefer `continue_delegate` over shell sleeps, ad-hoc `openclaw ...` CLI calls, or manual relay patterns."] : [],
	...
	if (!isMinimal && params.continuationEnabled) lines.push("## Continuation & Delegation", "### Self-elected turns", ...availableTools.has("continue_work") ? ["Use the `continue_work` tool to request another turn with structured `reason` and optional `delaySeconds`.", "Fallback bracket syntax remains available: CONTINUE_WORK or CONTINUE_WORK:30."] : ["End your response with CONTINUE_WORK to request another turn after a delay.", "End with CONTINUE_WORK:30 to specify delay in seconds."], "Use this when the same session should keep working later, after yielding to human input first.", "This is the sequential path: your main session keeps the thread of work itself.", "Use CONTINUE_WORK when you want your own next turn; use `continue_delegate` when the work", "should leave your head-session, run in background shards, and inform future turns later.", "", "### Delegated continuation", ...availableTools.has("continue_delegate") ? [
	...
	], "", "Delegates let the main session stay free while background shards do legwork.", "Silent returns may simply color later replies; they do not need immediate visible output.", "They can quietly inform future blind inquiry, later synthesis, or post-compaction recovery.", "Use `silent` for ambient enrichment or future recall. Use `silent-wake` when the return", "should silently enrich context and immediately wake you to synthesize or fan out again.", "", ...availableTools.has("request_compaction") ? [
		"### Compaction",
		...
		""
	] : [], "### When to use CONTINUE_DELEGATE vs sessions_spawn", "Use sessions_spawn for immediate, explicit workers you want to manage directly, for ACP", "runtime spawns, or when the shard needs inline attachments / explicit spawn-time controls.", "Use `continue_delegate` (or `[[CONTINUE_DELEGATE:]]` bracket syntax) when you need:", "  - Delayed dispatch — schedule work for N seconds from now", "  - Silent return — result arrives as internal context, no channel output", "  - Wake-on-return — silent result that triggers your next turn", "  - Background fan-out — multiple narrow shards while the main session keeps thinking/responding", "  - Compaction handoff — preserve working state or partial results across compaction", "  - Chain tracking — gateway enforces cost cap and depth limit across linked dispatches", "", "Do not use `exec`, shell sleeps, or manual `openclaw ...` commands to imitate delayed", "delegate scheduling when `continue_delegate` or `sessions_spawn` already fits the job.", "", "### Cooperative yield", "Use `sessions_yield` to end your turn immediately, aborting any queued tool calls.", "The session parks until an external event (subagent result, user message) arrives.", "This is useful after dispatching delegates when you should stop and wait for results,", "rather than requesting another turn on a timer.", "", "### Context pressure", "When you receive a [system:context-pressure] event, your context window is approaching capacity.", ...availableTools.has("continue_delegate") ? [
	...
	if (isMinimal && params.continuationEnabled) {
		...
		if (availableTools.has("continue_delegate")) lines.push("Use the `continue_delegate` tool to dispatch follow-up sub-agents from this chain.", "The tool supports multi-delegate fan-out, structured parameters (`task`, `delaySeconds`, `mode`),", "and `post-compaction` mode. The parent/main session stays free while your branch keeps working.", "", "Fallback bracket syntax (if the tool call fails or is unavailable):", "  [[CONTINUE_DELEGATE: task description]]", "  [[CONTINUE_DELEGATE: task +30s]]          — delayed spawn", "  [[CONTINUE_DELEGATE: task | silent]]       — silent return (no channel output)", "  [[CONTINUE_DELEGATE: task | silent-wake]]  — silent return + triggers parent turn", "", "Prefer the tool. Use brackets only as fallback.", "The gateway handles chain tracking and depth limits.", "");
		else lines.push("To dispatch a follow-up sub-agent from your output, end your ENTIRE response with:", "  [[CONTINUE_DELEGATE: task description]]", "", "Use this to keep a delegate tree moving without asking the parent to relay every hop.", "The parent/main session stays free while your branch keeps working.", "", "Optional modifiers:", "  [[CONTINUE_DELEGATE: task +30s]]          — delayed spawn", "  [[CONTINUE_DELEGATE: task | silent]]       — silent return (no channel output)", "  [[CONTINUE_DELEGATE: task | silent-wake]]  — silent return + triggers parent turn", "", "Use `| silent` when the result should only enrich the parent's future context.", "Use `| silent-wake` when the result should enrich the parent and wake it to act.", "", "Emit exactly ONE bracket per response. Do not nest brackets inside brackets.", "The gateway handles chain tracking and depth limits.", "");
	}
