...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-X2BXJBel.js";
import { t as createSubsystemLogger } from "./subsystem-CqiJqOXy.js";
import { t as isContainerEnvironment } from "./container-environment-Bk7eZ65v.js";
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-I-1keREy.js";
import { t as parsePort } from "./parse-port-BX_QYjNu.js";
import { i as withDiagnosticPhase } from "./diagnostic-phase-CpxGjk65.js";
import { r as withProgress } from "./progress-Dbtadrnx.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-BUTm9Gou.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-C6-033wS.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-BHDLxl05.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-B39xdQm8.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-rvKXBqZ5.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-DlVLivmV.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-DqcKKM6l.js");
	...
	if (!Number.isFinite(port) || port <= 0) {
		defaultRuntime.error(`Gateway port is invalid in config. Set it with ${formatCliCommand("openclaw config set gateway.port 3000")} or pass --port 3000.`);
		defaultRuntime.exit(1);
	}
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-dbQtzeA6.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-CKSbY2G-.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-Bqvh8Fhr.js");
	...
	const { resolveGatewayAuth } = await import("./auth-_Tpjt-7T.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-C81M-N2u.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-DzhZtcr8.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-CDxPUHkC.js");
