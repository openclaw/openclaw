...
const LONG_TEXT_TRANSLATIONS: Record<string, string> = {
  ...
  "Google Workspace Chat app with HTTP webhook":
    "通过 HTTP webhook 使用 Google Workspace Chat 应用",
  ...
  "iMessage via the BlueBubbles mac app + REST API":
    "通过 BlueBubbles Mac 应用 + REST API 使用 iMessage",
...
export function translateBackendMessage(message: string): string {
  ...
  if (
    trimmedMessage.includes("Security warning — please read") ||
  ...
  if (
    trimmedMessage.includes("Gateway service install failed") ||
  ...
  if (trimmedMessage.includes("Model not found:")) {
    return trimmedMessage
      ...
      .replace(
        "Update agents.defaults.model or run /models list.",
        "请更新 agents.defaults.model 或运行 /models list。",
      );
  }
  ...
  result = result.replace(
    /register a bot with @BotFather and get going/g,
    "通过 @BotFather 注册机器人即可使用",
  );
  ...
  result = result.replace(
    /Mattermost: self-hosted Slack-style chat/g,
    "Mattermost: 自托管的 Slack 风格聊天",
  );
  result = result.replace(/install the plugin to enable/g, "安装插件以启用");
  result = result.replace(
    /Nextcloud Talk: Self-hosted chat via Nextcloud Talk webhook bots/g,
    "Nextcloud Talk: 通过 Nextcloud Talk webhook 机器人的自托管聊天",
  );
  result = result.replace(/Matrix: open protocol/g, "Matrix: 开放协议");
  result = result.replace(
    /BlueBubbles: iMessage via the BlueBubbles mac app \+ REST API/g,
    "BlueBubbles: 通过 BlueBubbles Mac 应用 + REST API 使用 iMessage",
  );
  result = result.replace(
    /LINE: LINE Messaging API bot for Japan\/Taiwan\/Thailand markets/g,
    "LINE: 用于日本/台湾/泰国市场的 LINE 消息 API 机器人",
  );
  result = result.replace(
    /Zalo: Vietnam-focused messaging platform with Bot API/g,
    "Zalo: 越南主流消息平台，使用 Bot API",
  );
  result = result.replace(
    /Zalo Personal: Zalo personal account via QR code login/g,
    "Zalo 个人版: 通过二维码登录的 Zalo 个人账户",
  );
  result = result.replace(
    /Tlon: decentralized messaging on Urbit/g,
    "Tlon: Urbit 上的去中心化消息",
  );
  result = result.replace(
    /Google Chat: Google Workspace Chat app with HTTP webhook/g,
    "Google Chat: 通过 HTTP webhook 使用 Google Workspace Chat 应用",
  );

  // Translate DM security section
  result = result.replace(
    /DM security: default is pairing; unknown DMs get a pairing code\./g,
    "私信安全：默认使用配对；未知私信会获得配对码。",
  );
  result = result.replace(
    /Approve with: openclaw pairing approve <channel> <code>/g,
    "批准命令：openclaw pairing approve <channel> <code>",
  );
  result = result.replace(
    /Public DMs require dmPolicy="open" \+ allowFrom=\["\*"\]\./g,
    '公开私信需要 dmPolicy="open" + allowFrom=["*"]。',
  );
  result = result.replace(
    /Multi-user DMs: set session\.dmScope="per-channel-peer"/g,
    '多用户私信：设置 session.dmScope="per-channel-peer"',
  );
  result = result.replace(
    /\(or "per-account-channel-peer" for multi-account channels\) to isolate sessions\./g,
...
export function translateHint(hint: string): string {
  ...
  result = result.replace(
    /DeepSeek, GLM, Qwen, Llama via siliconflow\.cn/g,
