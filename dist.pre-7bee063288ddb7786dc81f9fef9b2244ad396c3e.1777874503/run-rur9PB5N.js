...
import { o as hasConfiguredSecretInput } from "./types.secrets-CyuYZNvB.js";
import { n as defaultRuntime } from "./runtime-Dv8n03pi.js";
import { S as setVerbose } from "./logger-BK1OY7lv.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-B_aJN9s5.js";
import { t as createSubsystemLogger } from "./subsystem-D1_uRCZT.js";
import "./globals-C7I_COwU.js";
import { t as isContainerEnvironment } from "./container-environment-CTUfWq7l.js";
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-xqHkLqZe.js";
import { r as formatConfigIssueSummary } from "./issue-format-u844k3jY.js";
import { t as parsePort } from "./parse-port-jiBPIaCY.js";
import { r as withProgress } from "./progress-CA-UKFN8.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-DjWPB1JF.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-_DF75mS4.js";
			...
			try {
				if (isRestart) {
					...
					if (activeTasks > 0 || activeRuns > 0) {
						...
						if (taskBlockers) gatewayLog$1.warn(`restart blocked by active task run(s): ${taskBlockers}`);
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-Damckaca.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-B52askLG.js");
...
async function readGatewayStartupConfig(params) {
	const { readConfigFileSnapshotWithPluginMetadata, recoverConfigFromLastKnownGood, recoverConfigFromJsonRootSuffix } = await import("./config/config.js");
	let snapshotRead = await params.startupTrace.measure("cli.config-snapshot", () => readConfigFileSnapshotWithPluginMetadata().catch(() => null));
	let snapshot = snapshotRead?.snapshot ?? null;
	if (snapshot?.exists && !snapshot.valid) {
		...
		if (await params.startupTrace.measure("cli.config-recovery", () => recoverConfigFromLastKnownGood({
			...
			const issueSummary = formatConfigIssueSummary([...invalidSnapshot.issues, ...invalidSnapshot.legacyIssues]);
			gatewayLog.warn(`gateway: restored invalid effective config from last-known-good backup: ${invalidSnapshot.path}${issueSummary ? `; Rejected validation details: ${issueSummary}.` : ""}`);
			try {
				const { writeRestartSentinel } = await import("./restart-sentinel-WK2noNL-.js");
			...
			snapshotRead = await params.startupTrace.measure("cli.config-snapshot-reload", () => readConfigFileSnapshotWithPluginMetadata().catch(() => null));
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-BfamHafB.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-Bczo_Ivr.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-Crq6JwiU.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-BgHGstlW.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-DNFXDbOu.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-B8RGegek.js");
	...
	const { resolveGatewayAuth } = await import("./auth-CqNJLTj4.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-CEnXEioc.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-Bi--uqpB.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-CNzcWgwK.js");
