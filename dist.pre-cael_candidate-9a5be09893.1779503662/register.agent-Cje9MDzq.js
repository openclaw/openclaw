...
async function loadAgentCliCommand() {
	return (await import("./agent-via-gateway-Cuf_pI0L.js")).agentCliCommand;
}
async function loadAgentsAddCommand() {
	return (await import("./agents.commands.add-BpeV5LpZ.js")).agentsAddCommand;
}
async function loadAgentsBindCommand() {
	return (await import("./agents.commands.bind-Dos27bbL.js")).agentsBindCommand;
}
async function loadAgentsBindingsCommand() {
	return (await import("./agents.commands.bind-Dos27bbL.js")).agentsBindingsCommand;
}
async function loadAgentsUnbindCommand() {
	return (await import("./agents.commands.bind-Dos27bbL.js")).agentsUnbindCommand;
}
async function loadAgentsDeleteCommand() {
	return (await import("./agents.commands.delete-CZVqzcRl.js")).agentsDeleteCommand;
}
async function loadAgentsSetIdentityCommand() {
	return (await import("./agents.commands.identity-DuDS0Yrh.js")).agentsSetIdentityCommand;
}
async function loadAgentsListCommand() {
	return (await import("./agents.commands.list-C2u5tYcR.js")).agentsListCommand;
}
async function loadCreateDefaultDeps() {
	return (await import("./deps-B6AmVUfY.js")).createDefaultDeps;
}
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openclaw.ai/cli/agent")}`).action(async (opts) => {
		...
		await runCommandWithRuntime(defaultRuntime, async () => {
			(await loadSetVerbose())(verboseLevel === "on");
			const deps = (await loadCreateDefaultDeps())();
			await (await loadAgentCliCommand())(opts, defaultRuntime, deps);
		});
	});
...
`).action(async (opts) => {
	...
	agents.action(async () => {
		await runCommandWithRuntime(defaultRuntime, async () => {
			await (await loadAgentsListCommand())({}, defaultRuntime);
		});
	});
}
//#endregion
