...
import { a as findTailscaleBinary } from "./tailscale-WKukbm1z.js";
import { t as randomToken } from "./random-token-DxG3QWGd.js";
import { o as formatPortRangeHint } from "./error-format-CzNExHro.js";
import { t as resolveSecretInputModeForEnvSelection } from "./provider-auth-mode-qF5peWvp.js";
import { n as promptSecretRefForSetup } from "./provider-auth-ref-D16TVvQz.js";
import { t as DEFAULT_DANGEROUS_NODE_COMMANDS } from "./node-command-policy-B_vsERP7.js";
import { t as maskApiKey } from "./mask-api-key-CSSv_gTI.js";
import { c as normalizeGatewayTokenInput, p as validateGatewayPasswordInput } from "./onboard-helpers-CdTyygwd.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-kCxN6WmW.js";
import { a as maybeAddTailnetOriginToControlUiAllowedOrigins, i as TAILSCALE_MISSING_BIN_NOTE_LINES, n as TAILSCALE_DOCS_LINES, r as TAILSCALE_EXPOSURE_OPTIONS, t as validateIPv4AddressInput } from "./ipv4-re_0on6A.js";
import { n as ensureControlUiAllowedOriginsForNonLoopbackBind } from "./gateway-control-ui-origins-DMhny2d7.js";
...
async function configureGatewayForSetup(opts) {
	...
	let bind = flow === "quickstart" ? quickstartGateway.bind : await prompter.select({
		...
		options: [
			...
			},
			{
				value: "lan",
				label: "LAN (0.0.0.0)",
				hint: "Reachable on your local network"
			},
			{
				value: "tailnet",
				label: "Tailnet (Tailscale IP)",
				hint: "Reachable over Tailscale"
			},
			{
				value: "auto",
				label: "Auto (Loopback -> LAN)",
				hint: "Try loopback first"
			},
			{
				value: "custom",
				label: "Custom IP",
				hint: "Bind to one local address"
			}
		]
	});
	...
	if (tailscaleMode !== "off" && bind !== "loopback") {
		await prompter.note("Tailscale exposure requires bind=loopback. I will switch the bind address to loopback.", "Gateway bind");
		bind = "loopback";
		customBindHost = void 0;
	}
	if (tailscaleMode === "funnel" && authMode !== "password") {
		await prompter.note("Tailscale Funnel requires password auth. I will switch Gateway auth to password.", "Gateway auth");
		authMode = "password";
	}
	let gatewayToken;
