...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.MULLUSI_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "MULLUSI_TEST_FAST", value: process.env.MULLUSI_TEST_FAST },
    ...
    { key: "MULLUSI_STATE_DIR", value: process.env.MULLUSI_STATE_DIR },
    { key: "MULLUSI_CONFIG_PATH", value: process.env.MULLUSI_CONFIG_PATH },
    { key: "MULLUSI_GATEWAY_PORT", value: process.env.MULLUSI_GATEWAY_PORT },
    { key: "MULLUSI_BRIDGE_ENABLED", value: process.env.MULLUSI_BRIDGE_ENABLED },
    { key: "MULLUSI_BRIDGE_HOST", value: process.env.MULLUSI_BRIDGE_HOST },
    { key: "MULLUSI_BRIDGE_PORT", value: process.env.MULLUSI_BRIDGE_PORT },
    { key: "MULLUSI_CANVAS_HOST_PORT", value: process.env.MULLUSI_CANVAS_HOST_PORT },
    { key: "MULLUSI_TEST_HOME", value: process.env.MULLUSI_TEST_HOME },
    { key: "MULLUSI_AGENT_DIR", value: process.env.MULLUSI_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "mullusi-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MULLUSI_TEST_HOME = tempHome;
  ...
  delete process.env.MULLUSI_BRIDGE_ENABLED;
  delete process.env.MULLUSI_BRIDGE_HOST;
  delete process.env.MULLUSI_BRIDGE_PORT;
  delete process.env.MULLUSI_CANVAS_HOST_PORT;
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.MULLUSI_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".mullusi");
  const priorIsolatedHome = params.env.MULLUSI_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".mullusi")
  ) {
    realStateDir = path.join(params.realHome, ".mullusi");
  }
  const tempStateDir = path.join(params.tempHome, ".mullusi");
  fs.mkdirSync(tempStateDir, { recursive: true });

  const realConfigPath = params.env.MULLUSI_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.MULLUSI_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "mullusi.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "mullusi.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.MULLUSI_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.MULLUSI_LIVE_USE_REAL_HOME);
