...
function loadProfileEnv(homeDir = os.homedir()): void {
	...
	try {
		...
		if (
			applied > 0 &&
			!isTruthyEnvValue(process.env.OPENCLAW_LIVE_TEST_QUIET)
		) {
...
function stageLiveTestState(params: {
	...
	copyDirIfExists(
		path.join(realStateDir, "credentials"),
	...
	for (const authDir of LIVE_EXTERNAL_AUTH_DIRS) {
		copyDirIfExists(
			path.join(params.realHome, authDir),
			path.join(params.tempHome, authDir),
		);
	}
}
