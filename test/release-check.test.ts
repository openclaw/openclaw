...
import {
	...
	bundledPluginFile,
} from "./helpers/bundled-plugin-paths.js";
...
const requiredPluginSdkPackPaths = [
...
describe("collectBundledExtensionManifestErrors", () => {
	...
	it("flags non-object install metadata instead of throwing", () => {
		expect(
			...
			"bundled extension 'broken' manifest invalid | openclaw.install must be an object",
		]);
	});
});
...
describe("collectPackUnpackedSizeErrors", () => {
	it("accepts pack results within the unpacked size budget", () => {
		expect(
			collectPackUnpackedSizeErrors([
				makePackResult("openclaw-2026.3.14.tgz", 120_354_302),
			]),
		).toEqual([]);
	});
	...
	it("flags oversized pack results that risk low-memory startup failures", () => {
		expect(
			collectPackUnpackedSizeErrors([
				makePackResult("openclaw-2026.3.12.tgz", 224_002_564),
			]),
		).toEqual([
