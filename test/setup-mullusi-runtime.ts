...
const WORKER_RUNTIME_STATE = Symbol.for("mullusi.testSetupRuntimeState");
const WORKER_CLEANUP_DEPS = Symbol.for("mullusi.testSetupCleanupDeps");
...
const createStubPlugin = (params: {
  ...
  threading: params.resolveReplyToMode
    ...
    : undefined,
  config: {
    listAccountIds: (cfg: MullusiConfig) => {
    ...
    resolveAccount: (cfg: MullusiConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: MullusiConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
