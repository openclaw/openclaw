...
describe("plugin extension import boundary inventory", () => {
	it("keeps dedicated web-search registry shims out of the remaining inventory", async () => {
		...
		expect(
			inventory.some(
				(entry) => entry.file === "src/plugins/web-search-providers.ts",
			),
		).toBe(false);
		expect(
			inventory.some(
				(entry) => entry.file === "src/plugins/bundled-web-search-registry.ts",
			),
		).toBe(false);
	});
	...
	it("ignores boundary shims by scope", async () => {
		...
		expect(
			inventory.some((entry) => entry.file.startsWith("src/plugin-sdk/")),
		).toBe(false);
		expect(
	...
	it("matches the checked-in baseline", async () => {
		...
		expect(diffInventory(baseline, actual)).toEqual({
			missing: [],
			unexpected: [],
		});
	});
