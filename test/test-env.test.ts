...
function createTempHome(): string {
	const tempDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "openclaw-test-env-real-home-"),
	);
...
describe("installTestEnv", () => {
	it("keeps live tests on a temp HOME while copying config and auth state", () => {
		...
		writeFile(
			path.join(realHome, ".openclaw", "credentials", "token.txt"),
			"secret\n",
		);
		writeFile(
			path.join(
			...
			JSON.stringify(
				{ version: 1, profiles: { default: { provider: "openai" } } },
				null,
				2,
			),
		);
		...
		const copiedConfig = JSON.parse(
			fs.readFileSync(copiedConfigPath, "utf8"),
		) as {
			agents?: {
		...
		expect(copiedConfig.models?.providers?.custom).toEqual({
		...
		expect(
			fs.existsSync(
				path.join(testEnv.tempHome, ".openclaw", "credentials", "token.txt"),
			),
		).toBe(true);
		expect(
			fs.existsSync(
		...
		expect(
			fs.existsSync(
				path.join(testEnv.tempHome, ".claude", ".credentials.json"),
			),
		).toBe(true);
	});
