...
describe("fetchNpmRegistryPackumentWithRetry", () => {
  ...
  it("rejects oversized npm packuments by content length", async () => {
    await expect(
      fetchNpmRegistryPackumentWithRetry({
        packageName: "@openclaw/meta-provider",
        packageUrl: "https://registry.npmjs.org/%40openclaw%2Fmeta-provider",
        ...
        maxBytes: 8,
        fetchImpl: async () =>
          new Response('{"versions":{}}', { headers: { "content-length": "64" } }),
        createSignal: () => new AbortController().signal,
      }),
    ).rejects.toThrow("npm packument response body exceeded 8 bytes");
  });

  it("rejects oversized streamed npm packuments", async () => {
    await expect(
      fetchNpmRegistryPackumentWithRetry({
        packageName: "@openclaw/meta-provider",
        packageUrl: "https://registry.npmjs.org/%40openclaw%2Fmeta-provider",
        attempts: 1,
        maxBytes: 8,
        fetchImpl: async () => new Response('{"versions":{}}'),
        createSignal: () => new AbortController().signal,
      }),
    ).rejects.toThrow("npm packument response body exceeded 8 bytes");
  });
