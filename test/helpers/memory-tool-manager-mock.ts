...
let readFileImpl: (params: MemoryReadParams) => Promise<MemoryReadResult> =
	async (params) => ({
		text: "",
		path: params.relPath,
	});
...
const getMemorySearchManagerMock = vi.fn(async () => ({
	manager: stubManager,
}));
...
const { memoryIndexModuleId, memoryToolsRuntimeModuleId } = vi.hoisted(() => ({
	...
	memoryToolsRuntimeModuleId:
		"../../extensions/memory-core/src/tools.runtime.js",
}));
