...
function resolvePluginSdkAliasTarget(
	root: string,
	subpath: string,
): string | null {
...
function collectPluginSdkAliases(params: {
		...
		for (const ref of collectSourceModuleRefs(filePath)) {
			if (ref.specifier.startsWith(PLUGIN_SDK_SPECIFIER_PREFIX)) {
				const shouldKeepReal =
					rootModule &&
					!ref.typeOnly &&
					(explicitRealSpecifiers.size === 0 ||
						explicitRealSpecifiers.has(ref.specifier));
				if (shouldKeepReal) {
					realSpecifiers.add(ref.specifier);
					const subpath = ref.specifier.slice(
						PLUGIN_SDK_SPECIFIER_PREFIX.length,
					);
	...
	for (const specifier of listPluginSdkExportedSubpaths(params.root).map(
		...
		if (realSpecifiers.has(specifier)) {
			...
			aliasEntries.set(
				specifier,
				resolvePluginSdkAliasTarget(params.root, subpath) ?? stubPath,
			);
			continue;
		}
