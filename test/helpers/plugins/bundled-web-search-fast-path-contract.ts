...
function toComparableEntry(params: {
	...
	return {
		...
		hasApplySelectionConfig:
			typeof params.provider.applySelectionConfig === "function",
		hasResolveRuntimeMetadata:
			typeof params.provider.resolveRuntimeMetadata === "function",
	};
}

function sortComparableEntries(
	entries: ComparableProvider[],
): ComparableProvider[] {
	...
	describe(`${pluginId} bundled web search fast-path contract`, () => {
		...
		it("keeps fast-path provider metadata aligned with the bundled runtime registry", async () => {
			...
			expect(
				...
				sortComparableEntries(
					bundledProviderEntries.map(
						({ pluginId: entryPluginId, ...provider }) =>
							toComparableEntry({
			...
			for (const fastPathProvider of fastPathProviders) {
				...
				contractProvider.setConfiguredCredentialValue?.(
					contractConfig,
					"test-key",
				);
				expect(fastConfig).toEqual(contractConfig);
				expect(
					fastPathProvider.getConfiguredCredentialValue?.(fastConfig),
				).toEqual(
					contractProvider.getConfiguredCredentialValue?.(contractConfig),
				);

				if (
					fastPathProvider.applySelectionConfig ||
					contractProvider.applySelectionConfig
				) {
					expect(
						fastPathProvider.applySelectionConfig?.({} as OpenClawConfig),
					).toEqual(
				...
				if (
					fastPathProvider.resolveRuntimeMetadata ||
					contractProvider.resolveRuntimeMetadata
				) {
					const metadataCases = [
						{
						...
						},
						{
							searchConfig: {
								...fastSearchConfig,
								perplexity: {
									...(fastSearchConfig.perplexity as
