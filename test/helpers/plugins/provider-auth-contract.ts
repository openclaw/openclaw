...
const loginOpenAICodexOAuthMock = vi.hoisted(() =>
	vi.fn<LoginOpenAICodexOAuth>(),
);
const githubCopilotLoginCommandMock = vi.hoisted(() =>
	vi.fn<GithubCopilotLoginCommand>(),
);
const ensureAuthProfileStoreMock = vi.hoisted(() =>
	vi.fn<EnsureAuthProfileStore>(),
);
const listProfilesForProviderMock = vi.hoisted(() =>
	vi.fn<ListProfilesForProvider>(),
);
	...
	describe("openai-codex provider auth contract", () => {
		...
		async function expectStableFallbackProfile(params: {
			...
			const provider = requireProvider(
				registerProviders(openAIPlugin),
				"openai-codex",
			);
		...
		it("uses a stable fallback id when JWT email is missing", async () => {
			...
			const expectedStableId = Buffer.from(
				"user-123__acct-456",
				"utf8",
			).toString("base64url");
		...
		it("uses iss and sub to build a stable fallback id when auth claims are missing", async () => {
			...
			const expectedStableId = Buffer.from(
				"https://accounts.openai.com|user-abc",
			).toString("base64url");
			await expectStableFallbackProfile({
				access,
				profileId: `openai-codex:id-${expectedStableId}`,
		...
		it("keeps OAuth failures non-fatal at the provider layer", async () => {
			const provider = await getProvider();
			loginOpenAICodexOAuthMock.mockRejectedValueOnce(
				new Error("oauth failed"),
			);

			await expect(
				provider.auth[0]?.run(buildAuthContext() as never),
			).resolves.toEqual({
				profiles: [],
			});
		});
...
export function describeGithubCopilotProviderAuthContract() {
	...
	describe("github-copilot provider auth contract", () => {
		...
		async function getProvider() {
			const { default: githubCopilotPlugin } =
