...
import {
	registerProviders,
	requireProvider,
} from "../../../src/plugins/contracts/testkit.js";
...
const bundledProviderModules = vi.hoisted(() => ({
	...
	minimaxIndexModuleUrl: new URL(
		"../../../extensions/minimax/index.ts",
		import.meta.url,
	).href,
	modelStudioIndexModuleUrl: new URL(
		"../../../extensions/modelstudio/index.ts",
		import.meta.url,
	).href,
	ollamaApiModuleId: new URL(
		"../../../extensions/ollama/api.js",
		import.meta.url,
	).pathname,
	ollamaIndexModuleUrl: new URL(
		"../../../extensions/ollama/index.ts",
		import.meta.url,
	).href,
	sglangApiModuleId: new URL(
		"../../../extensions/sglang/api.js",
		import.meta.url,
	).pathname,
	sglangIndexModuleUrl: new URL(
		"../../../extensions/sglang/index.ts",
		import.meta.url,
	).href,
	vllmApiModuleId: new URL("../../../extensions/vllm/api.js", import.meta.url)
		.pathname,
	vllmIndexModuleUrl: new URL(
...
function runCatalog(
	...
	return state.runProviderCatalog({
		...
		resolveProviderApiKey:
			params.resolveProviderApiKey ?? (() => ({ apiKey: undefined })),
		resolveProviderAuth:
...
function installDiscoveryHooks(state: DiscoveryState) {
	beforeEach(async () => {
		...
		vi.doMock(bundledProviderModules.ollamaApiModuleId, async () => {
			const actual = await vi.importActual<object>(
				bundledProviderModules.ollamaApiModuleId,
			);
			return {
				...actual,
				buildOllamaProvider: (...args: unknown[]) =>
					buildOllamaProviderMock(...args),
			};
		});
		vi.doMock(bundledProviderModules.vllmApiModuleId, async () => {
			const actual = await vi.importActual<object>(
				bundledProviderModules.vllmApiModuleId,
			);
			return {
				...actual,
				buildVllmProvider: (...args: unknown[]) =>
					buildVllmProviderMock(...args),
			};
		});
		vi.doMock(bundledProviderModules.sglangApiModuleId, async () => {
			const actual = await vi.importActual<object>(
				bundledProviderModules.sglangApiModuleId,
			);
			return {
				...actual,
				buildSglangProvider: (...args: unknown[]) =>
					buildSglangProviderMock(...args),
			};
		});
		({ runProviderCatalog: state.runProviderCatalog } = await import(
			"../../../src/plugins/provider-discovery.js"
		));
		const [
		...
		state.ollamaProvider = requireProvider(
			registerProviders(ollamaPlugin),
			"ollama",
		);
		state.vllmProvider = requireProvider(registerProviders(vllmPlugin), "vllm");
		state.sglangProvider = requireProvider(
			registerProviders(sglangPlugin),
			"sglang",
		);
		state.minimaxProvider = requireProvider(
