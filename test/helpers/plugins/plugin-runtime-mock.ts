	...
	const base: PluginRuntime = {
		...
		agent: {
			...
			ensureAgentWorkspace: vi
				...
				.mockResolvedValue(
					undefined,
				) as unknown as PluginRuntime["agent"]["ensureAgentWorkspace"],
			session: {
		...
		system: {
			enqueueSystemEvent:
				vi.fn() as unknown as PluginRuntime["system"]["enqueueSystemEvent"],
			requestHeartbeatNow:
		...
		mediaUnderstanding: {
			runFile:
				vi.fn() as unknown as PluginRuntime["mediaUnderstanding"]["runFile"],
			describeImageFile:
		...
		imageGeneration: {
			generate:
				vi.fn() as unknown as PluginRuntime["imageGeneration"]["generate"],
			listProviders:
				vi.fn() as unknown as PluginRuntime["imageGeneration"]["listProviders"],
		},
		webSearch: {
			listProviders:
				vi.fn() as unknown as PluginRuntime["webSearch"]["listProviders"],
			search: vi.fn() as unknown as PluginRuntime["webSearch"]["search"],
		},
		stt: {
			transcribeAudioFile:
				vi.fn() as unknown as PluginRuntime["stt"]["transcribeAudioFile"],
		},
		channel: {
			text: {
				chunkByNewline: vi.fn((text: string) => (text ? [text] : [])),
				chunkMarkdownText: vi.fn((text: string) => [text]),
				chunkMarkdownTextWithMode: vi.fn((text: string) =>
					text ? [text] : [],
				),
			...
			outbound: {
				loadAdapter:
					vi.fn() as unknown as PluginRuntime["channel"]["outbound"]["loadAdapter"],
			},
			threadBindings: {
		...
		events: {
			onAgentEvent: vi.fn(
				() => () => {},
			) as unknown as PluginRuntime["events"]["onAgentEvent"],
		...
		modelAuth: {
			getApiKeyForModel:
				vi.fn() as unknown as PluginRuntime["modelAuth"]["getApiKeyForModel"],
