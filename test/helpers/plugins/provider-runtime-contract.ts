...
const providerRuntimeContractModules = vi.hoisted(() => ({
	anthropicIndexModuleUrl: new URL(
	...
	googleIndexModuleUrl: new URL(
		"../../../extensions/google/index.ts",
		import.meta.url,
	).href,
	openAIIndexModuleUrl: new URL(
	...
	openRouterIndexModuleUrl: new URL(
		"../../../extensions/openrouter/index.ts",
		import.meta.url,
	).href,
	veniceIndexModuleUrl: new URL(
		"../../../extensions/venice/index.ts",
		import.meta.url,
	).href,
	xAIIndexModuleUrl: new URL(
		"../../../extensions/xai/index.ts",
		import.meta.url,
	).href,
	zaiIndexModuleUrl: new URL(
...
vi.mock(
	providerRuntimeContractModules.openAICodexProviderRuntimeModuleId,
	() => ({
		refreshOpenAICodexToken: refreshOpenAICodexTokenMock,
	}),
);
...
function createModel(
	overrides: Partial<ProviderRuntimeModel> & Pick<ProviderRuntimeModel, "id">,
) {
	return {
		id: overrides.id,
...
function requireProviderContractProvider(providerId: string): ProviderPlugin {
	...
	if (!provider) {
		throw new Error(
			`provider runtime contract fixture missing for ${providerId}`,
		);
	}
	return provider;
}
...
export function describeAnthropicProviderRuntimeContract() {
	describe("anthropic provider runtime contract", {
		...
		it("owns auth doctor hint generation", () => {
			...
			expect(hint).toContain(
				"suggested profile: anthropic:oauth-user@example.com",
			);
...
export function describeXAIProviderRuntimeContract() {
	describe("xai provider runtime contract", {
		timeout: CONTRACT_SETUP_TIMEOUT_MS,
	}, () => {
		installRuntimeHooks();
...
export function describeVeniceProviderRuntimeContract() {
	describe("venice provider runtime contract", {
		timeout: CONTRACT_SETUP_TIMEOUT_MS,
	}, () => {
		installRuntimeHooks();
...
export function describeZAIProviderRuntimeContract() {
	describe("zai provider runtime contract", {
		timeout: CONTRACT_SETUP_TIMEOUT_MS,
	}, () => {
		installRuntimeHooks();
		...
		it("falls back to legacy pi auth tokens for usage auth", async () => {
			...
			const home = await fs.mkdtemp(
				path.join(os.tmpdir(), "openclaw-zai-contract-"),
			);
		...
		it("owns usage snapshot fetching", async () => {
			...
			await expect(
				...
				displayName: "z.ai",
				windows: [
					{ label: "Tokens (6h)", usedPercent: 25, resetAt: 1_767_765_600_000 },
				],
				plan: "Pro",
			});
		});
