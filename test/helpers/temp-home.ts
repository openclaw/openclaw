...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("EASYHUB_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.EASYHUB_STATE_DIR = path.join(base, ".easyhub");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "EasyHub-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".easyhub", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
