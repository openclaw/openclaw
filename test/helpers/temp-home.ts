...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    activiHome: process.env.ACTIVI_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ACTIVI_HOME", snapshot.activiHome);
  restoreKey("ACTIVI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ACTIVI_HOME.
  delete process.env.ACTIVI_HOME;
  process.env.ACTIVI_STATE_DIR = path.join(base, ".activi");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "activi-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".activi", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
