...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("DNA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.DNA_STATE_DIR = path.join(base, ".dna");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "dna-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".dna", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
