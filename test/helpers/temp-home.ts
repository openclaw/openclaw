...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MULLUSI_HOME.
  delete process.env.MULLUSI_HOME;
  process.env.MULLUSI_STATE_DIR = path.join(base, ".mullusi");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "mullusi-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".mullusi", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
