...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `dna-e2e-${name}-`));
  const configDir = path.join(homeDir, ".dna");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          DNA_SKIP_CHANNELS: "1",
          DNA_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        DNA_GATEWAY_TOKEN: inst.gatewayToken,
        DNA_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          DNA_GATEWAY_PORT: String(gwA.port),
          DNA_GATEWAY_TOKEN: gwA.gatewayToken,
          DNA_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          DNA_GATEWAY_PORT: String(gwB.port),
          DNA_GATEWAY_TOKEN: gwB.gatewayToken,
          DNA_GATEWAY_PASSWORD: "",
        }),
