...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `EasyHub-e2e-${name}-`));
  const configDir = path.join(homeDir, ".easyhub");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          EASYHUB_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        EASYHUB_GATEWAY_TOKEN: inst.gatewayToken,
        EASYHUB_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          EASYHUB_GATEWAY_PORT: String(gwA.port),
          EASYHUB_GATEWAY_TOKEN: gwA.gatewayToken,
          EASYHUB_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          EASYHUB_GATEWAY_PORT: String(gwB.port),
          EASYHUB_GATEWAY_TOKEN: gwB.gatewayToken,
          EASYHUB_GATEWAY_PASSWORD: "",
        }),
