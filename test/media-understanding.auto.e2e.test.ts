...
import type { SmartAgentNeoConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("smart-agent-neo-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("smart-agent-neo-bin-sherpa-");
      const modelDir = await makeTempDir("smart-agent-neo-sherpa-model-");
      ...
      const cfg: SmartAgentNeoConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("smart-agent-neo-bin-whispercpp-");
      const modelDir = await makeTempDir("smart-agent-neo-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("smart-agent-neo-bin-gemini-");
      ...
      const cfg: SmartAgentNeoConfig = { tools: { media: { image: {} } } };
