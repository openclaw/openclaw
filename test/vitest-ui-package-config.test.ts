...
describe("ui package vitest config", () => {
  ...
  it("keeps native Chromium files out of root jsdom without dropping Node-driven Playwright files", async () => {
    ...
    vi.resetModules();
    const noBrowserConfig = (await import("../ui/vitest.config.ts")).default;
    expect(probe).not.toHaveBeenCalled();
    const noBrowserProject = (requireTestConfig(noBrowserConfig).projects ?? [])
      .map(requireTestConfig)
      .find((project) => project.browser?.enabled);
    expect(noBrowserProject?.include).toEqual([]);
    expect(noBrowserProject?.browser?.provider?.prewarm).toBeUndefined();
    ...
    const fullBrowser = requireTestConfig(createUiBrowserVitestConfig({}));
    expect(fullBrowser.browser?.provider?.prewarm).toEqual(expect.any(Function));
    expect(probe).toHaveBeenCalledWith(includeFile, ["--version"], { stdio: "ignore" });
    expect(fullBrowser.browser?.provider?.options).toMatchObject({
      launchOptions: {
    ...
    for (const [files, shouldPrewarm] of [
      ...
      writeFileSync(includeFile, JSON.stringify(files));
      const selectedBrowser = requireTestConfig(
        createUiBrowserVitestConfig({
          OPENCLAW_VITEST_INCLUDE_FILE: includeFile,
        }),
      );
      expect(selectedBrowser.exclude).toEqual(fullBrowser.exclude);
      expect(selectedBrowser.browser).toMatchObject({
        enabled: true,
        provider: {
          name: "playwright",
          options: fullBrowser.browser?.provider?.options,
          providerFactory: expect.any(Function),
          serverFactory: fullBrowser.browser?.provider?.serverFactory,
        },
      });
      expect(typeof selectedBrowser.browser?.provider?.prewarm).toBe(
