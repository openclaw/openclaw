...
describe("release wrapper scripts", () => {
  ...
  it("runs trusted harness planning against an explicit old release target root", () => {
    const oldTarget = createOldReleaseTarget();
    const repositoryRoot = process.cwd();
    const releaseSha = "a".repeat(40);
    const plan = runTsxScript(join(repositoryRoot, "scripts/openclaw-release-clawhub-plan.ts"), [
      "--bootstrap-workflow-sha",
      "b".repeat(40),
      "--release-tag",
      "v2026.7.1-beta.3",
