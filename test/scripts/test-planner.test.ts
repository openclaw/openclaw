...
describe("test planner", () => {
	it("builds a capability-aware plan for mid-memory local runs", () => {
		...
		expect(
			plan.selectedUnits.some((unit) => unit.id.startsWith("unit-fast")),
		).toBe(true);
		expect(
			plan.selectedUnits.some((unit) => unit.id.startsWith("extensions")),
		).toBe(true);
	...
	it("scales down mid-tier local concurrency under saturated load", () => {
		...
		expect(saturatedPlan.executionBudget.topLevelParallelLimitNoIsolate).toBe(
			4,
		);
		expect(saturatedPlan.executionBudget.topLevelParallelLimitIsolated).toBe(1);
		expect(saturatedPlan.topLevelParallelLimit).toBeLessThan(
			baselinePlan.topLevelParallelLimit,
		);
	...
	it("coalesces saturated high-memory local unit bursts into fewer shared batches", () => {
		...
		expect(saturatedSharedUnitBatches.length).toBeLessThan(
			baselineSharedUnitBatches.length,
		);
		baselineArtifacts.cleanupTempArtifacts();
		saturatedArtifacts.cleanupTempArtifacts();
	});
	...
	it("keeps full local unit runs phased when isolated and heavy lanes are present", () => {
		...
		expect(
			plan.serialPrefixUnits.some((unit) => unit.serialPhase === "unit-fast"),
		).toBe(true);
	...
	it("does not leak default-plan shard assignments into targeted units with the same id", () => {
		...
		const defaultUnitWithSameId = plan.allUnits.find(
			(unit) => unit.id === targetedUnit?.id,
		);
		...
		expect(
			plan.topLevelSingleShardAssignments.get(targetedUnit),
		).toBeUndefined();
		expect(
			plan.topLevelSingleShardAssignments.get(defaultUnitWithSameId),
		).toBeDefined();
	...
	it("assigns single include-file CI batches to one shard instead of over-sharding them", () => {
		...
		expect(plan.topLevelSingleShardAssignments.get(singleFileBatch)).toBeTypeOf(
			"number",
		);
	...
	it("adds the push-only compat lane to push manifests", () => {
		...
		expect(
			manifest.jobs.checks.matrix.include.some(
				(entry) => entry.task === "compat-node22",
			),
		).toBe(true);
	});
});
