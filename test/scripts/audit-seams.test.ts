...
describe("audit-seams cron seam classification", () => {
    ...
    `;
		...
		expect(describeSeamKinds("src/cron/isolated-agent/run.ts", source)).toEqual(
			["cron-agent-handoff", "cron-outbound-delivery"],
		);
	});
    ...
    `;
		...
		expect(describeSeamKinds("src/cron/service/ops.ts", source)).toContain(
...
describe("audit-seams subagent seam classification", () => {
    ...
    `;
		...
		expect(
			describeSeamKinds("src/agents/subagent-registry.ts", source),
		).toEqual(["subagent-announce-delivery", "subagent-lifecycle-registry"]);
	});
    ...
    `;
		...
		expect(
			describeSeamKinds("src/agents/acp-spawn-parent-stream.ts", source),
	...
	it("keeps subagent seam statuses conservative when nearby tests exist", () => {
		expect(
			determineSeamTestStatus(
				...
				[
					{
						file: "src/agents/subagent-spawn.workspace.test.ts",
