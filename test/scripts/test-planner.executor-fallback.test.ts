...
describe("test planner executor", () => {
	it("falls back to child exit when close never arrives", async () => {
		...
		>(
			import.meta.url,
			"../../scripts/test-planner/executor.mjs?scope=exit-fallback",
		);
		const artifacts = createExecutionArtifacts({
			OPENCLAW_TEST_CLOSE_GRACE_MS: "10",
		});
	...
	it("collects failures across planned units when failure policy is collect-all", async () => {
		...
		>(
			import.meta.url,
			"../../scripts/test-planner/executor.mjs?scope=collect-all",
		);
		const artifacts = createExecutionArtifacts({});
	...
	it("injects a valid localstorage file path into child NODE_OPTIONS", async () => {
		...
		>(
			import.meta.url,
			"../../scripts/test-planner/executor.mjs?scope=localstorage-file",
		);
		const artifacts = createExecutionArtifacts({
		...
		expect(capturedEnv?.NODE_OPTIONS).not.toMatch(
			/(^|\s)--localstorage-file(?=\s|$)/u,
		);
