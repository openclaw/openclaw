...
function scheduledLiveEvalEnabled(condition: string | undefined, value: string | undefined) {
  const requiredValue = condition?.match(
    /vars\.OPENCLAW_AGENT_ROLE_EVAL_LIVE\s*==\s*'([^']+)'/,
  )?.[1];
  if (!requiredValue) {
    throw new Error("Expected scheduled live eval condition to require an explicit value");
  }
...
describe("agent role eval harness", () => {
  ...
  it("keeps the CI live workflow aligned with local-first eval defaults", () => {
    ...
    expect(liveJob?.if).toContain("inputs.run_live");
    expect(liveJob?.if).toContain("vars.OPENCLAW_AGENT_ROLE_EVAL_LIVE == '1'");
    expect(liveJob?.if).not.toContain("!= '0'");
    ...
    expect(uploadReport.uses).toBe("actions/upload-artifact@v7");
  ...
  it("requires explicit opt-in for scheduled live evals", () => {
    const workflow = readAgentRoleEvalWorkflow();
    const condition = workflow.jobs?.["live-role-turns"]?.if;

    expect(scheduledLiveEvalEnabled(condition, undefined)).toBe(false);
    expect(scheduledLiveEvalEnabled(condition, "")).toBe(false);
    expect(scheduledLiveEvalEnabled(condition, "0")).toBe(false);
    expect(scheduledLiveEvalEnabled(condition, "1")).toBe(true);
  });
  ...
  it("rejects newline-bearing live model before writing GitHub env", () => {
    const root = harness.createTempDir("openclaw-agent-eval-workflow-");
    const githubEnv = path.join(root, "github-env");
    const { calls, spawn } = createWorkflowSpawn();
    ...
    expect(() =>
      prepareOllamaForLiveWorkflow({
        env: {
          ...
          INPUT_LIVE_MODEL: `${DEFAULT_SELF_CONTAINED_LIVE_MODEL}\nINJECTED_ENV=1`,
        },
        spawn: spawn as never,
      }),
    ).toThrow("Live eval model cannot contain control characters.");
