...
async function createTempRoot() {
	const dir = await fs.mkdtemp(
		path.join(os.tmpdir(), "openclaw-runtime-postbuild-"),
	);
...
describe("runtime postbuild static assets", () => {
	...
	it("writes stable aliases for hashed root runtime modules", async () => {
		...
		expect(
			await fs.readFile(
				path.join(distDir, "runtime-model-auth.runtime.js"),
				"utf8",
			),
		).toBe('export * from "./runtime-model-auth.runtime-XyZ987.js";\n');
		expect(
			await fs.readFile(path.join(distDir, "runtime-tts.runtime.js"), "utf8"),
		).toBe('export * from "./runtime-tts.runtime-AbCd1234.js";\n');
