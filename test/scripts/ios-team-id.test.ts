...
function pickTeamIdFromCandidates(params: {
	...
	if (preferredTeamId) {
		const preferred = params.candidates.find(
			(candidate) => candidate.teamId === preferredTeamId,
		);
		if (preferred) {
	...
	const preferredTeamName = (params.preferredTeamName ?? "")
		.trim()
		.toLowerCase();
	if (preferredTeamName) {
		const preferredByName = params.candidates.find(
			(candidate) =>
				candidate.teamName.trim().toLowerCase() === preferredTeamName,
		);
		if (preferredByName) {
...
describe("scripts/ios-team-id.sh", () => {
	beforeAll(async () => {
		...
		await mkdir(path.join(sharedHomeDir, "Library", "Preferences"), {
			recursive: true,
		});
		await writeFile(
			path.join(
