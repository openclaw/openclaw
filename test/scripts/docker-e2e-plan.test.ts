...
describe("scripts/lib/docker-e2e-plan", () => {
  it("plans package-backed Compose and package artifact proofs", () => {
    const plan = planFor({
      selectedLaneNames: ["docker-compose-setup", "package-openclaw-for-docker"],
    });

    expect(plan.lanes.map(summarizeLane)).toEqual([
      {
        command: "OPENCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:docker-compose-setup",
        imageKind: "functional",
        live: false,
      ...
      {
        command: "OPENCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:package-openclaw-for-docker",
        imageKind: "bare",
        live: false,
        name: "package-openclaw-for-docker",
        resources: ["docker", "npm"],
        stateScenario: "empty",
        timeoutMs: 1_200_000,
        weight: 3,
