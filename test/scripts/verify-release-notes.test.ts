...
describe("release-note verification", () => {
  it("rejects partial GraphQL data when GitHub also reports errors", () => {
    expect(() =>
      githubGraphqlData({
        data: { commit0: null },
        errors: [{ message: "Something went wrong while executing your query." }],
      }),
    ).toThrow("GitHub GraphQL response included errors");
  });
  ...
  it("accepts canonical cherry-pick origins and rejects malformed provenance", () => {
    const origin = "a".repeat(40);
    const secondOrigin = "b".repeat(40);
    expect(
      cherryPickedFromCommits(`Fix release behavior\n\n(cherry picked from commit ${origin})`),
    ).toEqual([origin]);
    expect(cherryPickedFromCommits("Fix release behavior")).toEqual([]);
    expect(
      cherryPickedFromCommits(
        `(cherry picked from commit ${origin})\n(cherry picked from commit ${secondOrigin})`,
      ),
    ).toEqual([origin, secondOrigin]);
    expect(() => cherryPickedFromCommits("(cherry picked from commit abc123)")).toThrow(
      "invalid cherry-pick provenance",
    );
  });
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "### Complete contribution record",
          "",
          "This stale generated record must be replaced without validation.",
          "",
          "- **PR #999999999** stale: no longer resolvable.",
        ].join("\n"),
      );
      ...
      expect(readFileSync(join(cwd, "CHANGELOG.md"), "utf8")).not.toContain("#999999999");
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });

  it("does not replace CHANGELOG.md when generated-ledger validation fails", () => {
    const cwd = mkdtempSync(join(tmpdir(), "openclaw-release-notes-"));
    try {
      git(cwd, ["init", "-q"]);
      const original = [
      ...
      expect(JSON.parse(result.stdout).errors).toContain(
        "### Highlights must contain 5-8 top-level bullets; found 1",
      );
