...
describe("release CI summary child correlation", () => {
  ...
  it("selects one immutable manifest artifact bound to the exact parent run", () => {
    ...
    expect(source).toContain("execFileSync(resolvePlainGhBin(), args");
    expect(source).toContain('token="$("$OPENCLAW_PLAIN_GH_BIN" auth token)"');
    expect(source).not.toContain('token="$(gh auth token)"');
  ...
  it("requires v3 manifests to record blocking performance evidence", () => {
    const workflowSha = "b".repeat(40);
    const advisory = rawManifest({ version: 3, workflowSha });
    advisory.controls.performanceBlocking = false;
    expect(() =>
    ...
    ).toThrow("release validation manifest performance blocking control is invalid");
  });
  ...
  it("validates manifest child workflow, dispatch tuple, branch, and attempt", () => {
    ...
    ).toBe(maintainerRerun);
    expect(() =>
      validateManifestChildRun(
        { ...maintainerRerun, triggering_actor: { login: "other-maintainer" } },
        child,
        "101",
        parentManifest,
        parentJobs,
