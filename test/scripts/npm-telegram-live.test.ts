...
describe("package Telegram live Docker E2E", () => {
  ...
  it("keeps private QA harness imports local while using the installed package dist", () => {
    ...
    expect(preparePackage).not.toContain('pkg.exports["./plugin-sdk/qa-channel"]');
    expect(preparePackage).not.toContain('pkg.exports["./plugin-sdk/qa-channel-protocol"]');
    ...
    expect(qaRuntimeApi).toContain('from "@openclaw/qa-channel/api.js"');
  });
