...
describe("full release validation evidence", () => {
  it("rejects manifests without blocking performance evidence", () => {
    expect(() => validate({}, { controls: { performanceBlocking: false } })).toThrow(
      "does not record blocking product performance evidence",
    );
  });
  ...
  it("rejects a direct release workflow branch at a different commit", () => {
    expect(() =>
      validate(
        { head_branch: "release/2026.7.1", head_sha: "c".repeat(40) },
        {
          workflowRef: "release/2026.7.1",
          workflowSha: "c".repeat(40),
          workflowFullRef: "refs/heads/release/2026.7.1",
          targetRef: "v2026.7.1-beta.3",
        },
      ),
    ).toThrow("does not match trusted");
  });
