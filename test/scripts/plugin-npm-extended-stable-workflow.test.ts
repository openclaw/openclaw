...
describe("plugin npm extended-stable workflow", () => {
  ...
  it("publishes extended-stable with OIDC only and verifies every package tag", () => {
    const parsed = workflow();
    const setup = step(parsed.jobs?.publish_plugins_npm, "Setup trusted publication dependencies");
    const publish = step(parsed.jobs?.publish_plugins_npm, "Publish with trusted publisher");
    const versionCheck = step(parsed.jobs?.publish_plugins_npm, "Check OIDC npm package version");
    expect(versionCheck.env).toMatchObject({
      EXPECTED_NPM_INTEGRITY: "${{ steps.publication_evidence.outputs.npm_integrity }}",
      EXPECTED_NPM_SHASUM: "${{ steps.publication_evidence.outputs.npm_shasum }}",
    });
    expect(versionCheck.run).toContain(".dist.integrity");
    expect(versionCheck.run).toContain(".dist.shasum");
    expect(versionCheck.run).toContain("do not match immutable publication evidence");
    expect(setup.if).toContain("npm-oidc");
    expect(publish.env).toMatchObject({
      OPENCLAW_NPM_PUBLISH_AUTH_MODE: "trusted-publisher",
      OPENCLAW_PLUGIN_NPM_DEFER_DIST_TAG_MIRRORS: "1",
      SOURCE_PACKAGE_DIR: "${{ steps.publication_evidence.outputs.source_package_dir }}",
      TARBALL_PATH: "${{ steps.publication_evidence.outputs.tarball_path }}",
      TARBALL_SHA256: "${{ steps.publication_evidence.outputs.tarball_sha256 }}",
    });
    expect(publish.env?.NODE_AUTH_TOKEN).toBeUndefined();
    expect(publish.env?.NPM_TOKEN).toBeUndefined();
    const mirrorRepair = step(parsed.jobs?.publish_plugins_npm, "Repair required npm mirrors");
    expect(mirrorRepair.if).toContain("npm-oidc");
    expect(mirrorRepair.env?.NPM_TOKEN).toBe("${{ secrets.NPM_TOKEN }}");
    expect(mirrorRepair.run).toContain("resolveNpmPublishPlan");
    expect(mirrorRepair.run).toContain('[[ "$mirror_dist_tags" != "beta" ]]');
    expect(mirrorRepair.run).toContain("NPM_CONFIG_USERCONFIG");
    expect(mirrorRepair.run).toContain("unset NODE_AUTH_TOKEN NPM_TOKEN NODE_OPTIONS");
    expect(mirrorRepair.run).toContain(
    ...
    expect(consume.run).toContain(
      '"$WORKFLOW_REF" != "refs/heads/release-publish/${WORKFLOW_SHA}"',
    );
    ...
    expect(
      parsed.jobs?.publish_plugins_npm?.steps?.map((candidate) => candidate.name),
    ).not.toContain("Checkout OIDC publication target");
    expect(
      parsed.jobs?.publish_plugins_npm?.steps?.map((candidate) => candidate.name),
    ).not.toContain("Setup OIDC publication target");
    ...
    expect(publish.run).toContain('sha256sum "$TARBALL_PATH"');
    expect(publish.run).toContain('"$actual_tarball_sha256" != "$TARBALL_SHA256"');
    expect(publish.run).toContain(
      'plugin-npm-publish.sh --publish "$SOURCE_PACKAGE_DIR" "$TARBALL_PATH"',
    );
