...
const targetedChannelProxyFiles = [
	...
	bundledPluginFile(
		"whatsapp",
		"src/monitor-inbox.streams-inbound-messages.test.ts",
	),
];
...
function createLocalPlannerEnv(
	overrides: NodeJS.ProcessEnv = {},
): NodeJS.ProcessEnv {
...
function runManifestOutputWriter(
	workflow: string,
	envOverrides: NodeJS.ProcessEnv = {},
): string {
	const outputPath = path.join(
		os.tmpdir(),
		`openclaw-${workflow}-output-${Date.now()}.txt`,
	);
	try {
		execFileSync(
			"node",
			["scripts/ci-write-manifest-outputs.mjs", "--workflow", workflow],
			{
				cwd: REPO_ROOT,
...
describe("scripts/test-parallel fatal output guard", () => {
	...
	it("preserves explicit non-zero exits", () => {
		expect(resolveTestRunExitCode({ code: 2, signal: null, output: "" })).toBe(
			2,
		);
	});
...
describe("scripts/test-parallel lane planning", () => {
	...
	it("uses higher shared extension worker counts on high-memory local hosts", () => {
		...
		const highSharedBatches = getPlanLines(
			highMemoryOutput,
			"extensions-batch-",
		);
		...
		expect(
			midSharedBatches.every((line) => /filters=\d+ maxWorkers=3/.test(line)),
		).toBe(true);
		expect(
			highSharedBatches.every((line) => /filters=\d+ maxWorkers=5/.test(line)),
		).toBe(true);
		expect(highSharedBatches.length).toBeLessThanOrEqual(
			midSharedBatches.length,
		);
	});
	...
	it("uses earlier targeted channel batching on high-memory local hosts", () => {
		...
		expect(
			channelBatchLines.every((line) => line.includes("maxWorkers=5")),
		).toBe(true);
		expect(Math.max(...channelBatchFilterCounts)).toBeLessThan(30);
		expect(
			channelBatchFilterCounts.reduce((sum, count) => sum + count, 0),
		).toBe(sharedTargetedChannelProxyFiles.length);
	});
	...
	it("uses targeted unit batching on high-memory local hosts", () => {
		...
		expect(unitBatchLines.every((line) => line.includes("maxWorkers=6"))).toBe(
			true,
		);
		...
		expect(output).toContain(
			"unit-qr-dashboard.integration-isolated filters=1 maxWorkers=2",
		);
	});
	...
	it("rejects wrapper --files values that look like options", () => {
		expect(() =>
			runPlannerPlan(["--plan", "--files", "--config"]),
		).toThrowError(/Invalid --files value/u);
	});

	it("rejects missing --profile values", () => {
		expect(() => runPlannerPlan(["--plan", "--profile"])).toThrowError(
			/Invalid --profile value/u,
		);
	});

	it("rejects missing --surface values", () => {
		expect(() => runPlannerPlan(["--plan", "--surface"])).toThrowError(
			/Invalid --surface value/u,
		);
	});

	it("rejects missing --explain values", () => {
		expect(() => runPlannerPlan(["--explain"])).toThrowError(
			/Invalid --explain value/u,
		);
	});
	...
	it("rejects explicit existing files that are not known test files", () => {
		const tempFilePath = path.join(
			os.tmpdir(),
			`openclaw-non-test-${Date.now()}.ts`,
		);
		fs.writeFileSync(tempFilePath, "export const notATest = true;\n", "utf8");

		try {
			expect(() =>
				runPlannerPlan(["--plan", "--files", tempFilePath]),
