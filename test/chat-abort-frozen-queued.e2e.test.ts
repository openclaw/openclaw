 ...
 * state: "final") as the synchronization barrier. chat.send returns a client
 * run id that the Gateway finalizes immediately upon queue admission (an
 * enqueue "final" event with no message); the followup's own delivery
 * "final" event carries the assistant message text. Synchronizing on the
 * delivery message (QUEUED_TURN_COMPLETE) skips the enqueue final and
 * guarantees commitTerminalOutcome (freeze +
 * retireFollowupRunCancellation) and delivery have both completed before
 * the late abort. Because Gateway does not broadcast an event between
 * freeze-retirement and delivery-settlement, an E2E test cannot observe
 * the exact freeze-to-settlement interval. The late chat.abort after the
 * delivery "final" barrier therefore proves the terminal state
 * (aborted=false, delivery completed) but cannot distinguish retirement
 * (abortable=false, entry present) from deletion (entry removed).
 *
...
async function waitForChatFinal(
  ...
  while (Date.now() < deadline) {
    for (const event of events) {
      ...
      if (payload.state === "final" && collectFinalText(payload).includes(textMarker)) {
        return payload;
      }
    }
  ...
  throw new Error(
    `timed out waiting for chat final with text marker=${textMarker}; events=${JSON.stringify(
      events.slice(-12),
      null,
      2,
    )}`,
...
async function waitForChatRunActive(
  events: Array<{ event?: string; payload?: unknown }>,
  runId: string,
  timeoutMs = 30_000,
): Promise<void> {
  ...
  while (Date.now() < deadline) {
    for (const event of events) {
      ...
      const payload = event.payload as ChatEventPayload & { phase?: string };
      if (
        payload.runId === runId &&
  ...
  throw new Error(
    `timed out waiting for chat run active (state=status, phase=starting_model) runId=${runId}; events=${JSON.stringify(
      events.slice(-12),
      null,
      2,
    )}`,
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        // Synchronize on the Gateway's real "starting_model" status event so the
        // first turn is provably the active embedded run before the second turn
        // is submitted. This replaces a fixed sleep with an observable state
        // barrier: the second turn can only be queued behind an active run.
        await waitForChatRunActive(events, firstRunId);
        ...
        //    retire + delivery). chat.send returns a client run id that the
        //    Gateway finalizes immediately upon queue admission (an enqueue
        //    "final" event with no message); the followup's own delivery
        //    "final" event carries the assistant message text. Synchronizing
        //    on the delivery message (QUEUED_TURN_COMPLETE) skips the enqueue
        //    final and guarantees commitTerminalOutcome (freeze +
        //    retireFollowupRunCancellation) and delivery have both completed
        //    before the test issues a late abort.
        const finalEvent = await waitForChatFinal(events, "QUEUED_TURN_COMPLETE");
        trace.push(
          `followup run terminalized (chat state=final, message has QUEUED_TURN_COMPLETE)`,
        );
