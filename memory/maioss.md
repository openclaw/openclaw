...
- `78a8b65 chore: sync local changes (03-02)`
- `15d65f3 fix: Cover sheet Rev.5 template alignment (02-23)`
- `5ab809a fix: Details sheet Rev.5 template alignment (02-23)`
- `812c390 fix: Rev.5 report표기 개선 - ECU 타입, Rev.5 마킹, CVE+CAVD AND 모드 명시 (02-23)`
- `0d639f9 fix: call set_rev5_config() before generating Rev.5 ES10500 report (02-23)`
...
- **도달성 분석(Reachability Analysis)**: 2026 OSS 보안 핵심 트렌드 — 실제 호출 여부 분석으로 오탐 대폭 감소. MAIOSS v3 차별화 기능 후보
- **OSV-Scanner v2**: guided remediation 기능 — 자동 교정 안내 참고
- **Trivy 멀티스캐너**: 취약점+IaC+시크릿 단일 패스 — MAIOSS 올인원 방향 벤치마크
- **Grype EPSS 리스크 스코어링**: 리스크 점수 산출 모델 참고

### Tech Intelligence 인사이트 (2026-03-10)

- **🔴 GitHub Security Lab Taskflow Agent** (2026.03.09 공개): 오픈소스 AI 프레임워크, Auth Bypass/IDOR/Token Leak 자동 탐지. 67개 OSS 프로젝트 보안 감사 결과 공개. GitHub 플랫폼 내재화 = 배포 우위. Claude Code Security와 함께 MAIOSS의 두 번째 대형 경쟁자.
- **Augustus (Praetorian)**: LLM 취약점 스캐너 — 210+ 공격패턴, 28개 LLM 프로바이더. "Buff" 시스템으로 동적 프로브 변환. MAIOSS에 LLM 보안 스캐닝 통합 시 차별화 가능.
- **경쟁 환경 요약**: (1) Claude Code Security — AI 추론 기반, (2) GitHub Taskflow Agent — GitHub 생태계 내재, (3) Snyk/Trivy/Grype — 전통 OSS 스캐너. MAIOSS 차별화: **자동차 규정(ES10500/CAVD) 특화 + SBOM 의존성 체인 + LLM 보안 확장**.
- **DefectDojo**: 200+ 보안 도구 결과 통합 플랫폼. MAIOSS 스캔 결과를 DefectDojo로 내보내는 통합 검토.

### Tech Intelligence 인사이트 (2026-03-11)

- **🔴 OpenAI Codex Security 정식 롤아웃** (2026.03.06): 1.2M 커밋 스캔, 792 critical + 10,561 high-severity 발견 공개. 자동 취약점 발견 + 수정 제안 에이전트. MAIOSS 3번째 빅테크 경쟁자. 시장 내러티브가 "AI가 OSS 보안을 자동화한다"로 급속 수렴 중.
- **🔴 OpenAnt (Knostic, 2026.03.06 공개)**: 오픈소스 LLM 취약점 스캐너. 2-stage 파이프라인(발견 → 실제 공격 시뮬레이션). false positive 최소화 설계. MAIOSS와 직접 경쟁하면서도 코드 오픈이라 기법 참고 가능.
- **🟡 Cloudflare AI API Vulnerability Scanner** (2026.03.09): AI로 API 콜 그래프 구축 → 로직 결함 탐지. API 보안은 MAIOSS 미진 영역 — 모듈 추가 검토.
- **경쟁 환경 심화 (3주 연속 빅테크 진입)**: Week 1 Claude Code Security → Week 2 GitHub Taskflow → Week 3 OpenAI Codex Security. MAIOSS 포지셔닝 재검토 시급. 차별화 축: (1) 자동차 규정 특화(ES10500/CAVD), (2) 커뮤니티 + self-hosted 오픈소스, (3) auto-fix 자동 수정, (4) API 보안 확장.
