...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-D3q9sw1e.js";
...
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-DpXvrs-2.js";
...
import { i as withDiagnosticPhase } from "./diagnostic-phase-DdT2_8lT.js";
import { r as withProgress } from "./progress-C-BqssOb.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-DhJXWH_V.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-0ePzyOT-.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-Cb8IdHFk.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-Cl1wIRxJ.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-DK7vPYwX.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-DCoAFKXM.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-KiVz022j.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-J9MYpLxM.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-HM8A2SWC.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-DmxmoNmz.js");
	...
	const { resolveGatewayAuth } = await import("./auth-xWq7N3G2.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-5qQoT1zH.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-WcK9sroX.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-cXuvkyfP.js");
