...
import { i as listBundledChannelIdsForPackageState, n as listBundledChannelIdsWithConfiguredState, r as hasBundledChannelPackageState, t as hasBundledChannelConfiguredState } from "./configured-state-DkaFsShr.js";
import { t as resolveAgentRuntimePolicy } from "./agent-runtime-policy-sM853hqM.js";
import { i as listBundledChannelIds } from "./bootstrap-registry-Bm_6DD70.js";
import { i as hasNonEmptyString } from "./channel-target-DDtX5fgx.js";
...
function createConfiguredStateChannelIdSet() {
	return new Set(listBundledChannelIdsWithConfiguredState().map((channelId) => normalizeOptionalLowercaseString(channelId)).filter((channelId) => Boolean(channelId)));
}
function listBundledConfiguredChannelEnvSignalIds(channelIds, env) {
	const configuredStateChannelIds = createConfiguredStateChannelIdSet();
	return channelIds.map((channelId) => normalizeOptionalLowercaseString(channelId)).filter((channelId) => Boolean(channelId)).filter((channelId) => configuredStateChannelIds.has(channelId) && hasBundledChannelConfiguredState({
		channelId,
		cfg: {},
		env
	}));
}
function listFallbackChannelEnvPrefixes(channelIds) {
	const configuredStateChannelIds = createConfiguredStateChannelIdSet();
	return listChannelEnvPrefixes(channelIds.filter((channelId) => {
...
function hasEnvConfiguredChannel(cfg, env, options = {}) {
	const channelIds = options.channelIds ?? listBundledChannelIds(env);
	if (listBundledConfiguredChannelEnvSignalIds(channelIds, env).length > 0) return true;
	const channelEnvPrefixes = listFallbackChannelEnvPrefixes(channelIds);
	for (const [key, value] of Object.entries(env)) {
		if (!hasNonEmptyString(value)) continue;
