...
if (-not ($hfStatus -and $hfStatus.tokenPresent)) {
    foreach ($url in $gatedHuggingFaceTargets) {
        Write-Host "  🔒 " -ForegroundColor Yellow -NoNewline
        Write-Host "$url - gated Hugging Face model; queue-að sjálfkrafa þegar HF token er virkt" -ForegroundColor Gray
        $skipped++
    }
}
...
Write-Host "   ❌ Mistókust: $failed" -ForegroundColor Red
Write-Host "   🔒 Sleppt gated: $skipped" -ForegroundColor Yellow
Write-Host "   📋 Alls: $($targets.Count + $skipped)" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "`n🌐 Sjá rauntíma progress á: $ControlUiUrl" -ForegroundColor Cyan
    Write-Host "💡 Tip: Opnaðu UI-ið til að sjá logs streyma inn!`n" -ForegroundColor Yellow
}
catch {
    Write-Host "⚠️ Gat ekki sótt stats, en targets voru sendir!" -ForegroundColor Yellow
}
Write-Host "🔥 ALPHABET-VELDIÐ ER VIRKT! 🔥" -ForegroundColor Green
