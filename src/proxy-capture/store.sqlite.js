...
export class DebugProxyCaptureStore {
    ...
    upsertSession(session) {
        ...
        ON CONFLICT(id) DO UPDATE SET
          ...
          source_process=excluded.source_process`)
            .run(session.id, session.startedAt, session.endedAt ?? null, session.mode, session.sourceScope, session.sourceProcess, session.proxyUrl ?? null, session.dbPath, session.blobDir);
    }
    ...
    recordEvent(event) {
        this.db
          ...
          data_text, data_blob_id, data_sha256, error_text, meta_json
            ...
            .run(event.sessionId, event.ts, event.sourceScope, event.sourceProcess, event.protocol, event.direction, event.kind, event.flowId, event.method ?? null, event.host ?? null, event.path ?? null, event.status ?? null, event.closeCode ?? null, event.contentType ?? null, event.headersJson ?? null, event.dataText ?? null, event.dataBlobId ?? null, event.dataSha256 ?? null, event.errorText ?? null, event.metaJson ?? null);
    }
    ...
    purgeAll() {
        const sessionCount = this.db.prepare(`SELECT COUNT(*) AS count FROM capture_sessions`).get()
            .count ?? 0;
        const eventCount = this.db.prepare(`SELECT COUNT(*) AS count FROM capture_events`).get()
            .count ?? 0;
    ...
    deleteSessions(sessionIds) {
        ...
        const remainingBlobRefs = candidateBlobIds.length > 0
            ? new Set(this.db
                .prepare(`SELECT DISTINCT data_blob_id AS blobId
                   FROM capture_events
...
let cachedKey = "";
export function getDebugProxyCaptureStore(dbPath, blobDir) {
...
export function closeDebugProxyCaptureStore() {
    if (!cachedStore) {
        return;
    }
    cachedStore.close();
...
export function persistEventPayload(store, params) {
    if (params.data == null) {
        return {};
    }
