...
import { z } from "zod";
import { createMullusiChannelMcpServer, MullusiChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createMullusiChannelMcpServer({
...
describe("mullusi channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new MullusiChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
