...
function candidateBinDirs(opts: EnsureDNAPathOpts): string[] {
  ...
  // Bundled macOS app: `dna` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingDNA = path.join(execDir, "dna");
    if (isExecutable(siblingDNA)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/dna` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "dna"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `dna` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDNACliOnPath(opts: EnsureDNAPathOpts = {}) {
  if (isTruthyEnvValue(process.env.DNA_PATH_BOOTSTRAPPED)) return;
