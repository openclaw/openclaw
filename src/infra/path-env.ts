...
function candidateBinDirs(opts: EnsureMullusiPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `mullusi` lives next to the executable (process.execPath).
  try {
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MULLUSI_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "mullusi"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `mullusi`
...
/**
 * Best-effort PATH bootstrap so skills that require the `mullusi` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMullusiCliOnPath(opts: EnsureMullusiPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MULLUSI_PATH_BOOTSTRAPPED)) {
    return;
  }
