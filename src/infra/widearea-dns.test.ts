...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  ...
  gatewayPort: 18790,
  displayName: "Mac Studio (Mullusi)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "mullusi.internal", expected: "mullusi.internal." },
    { value: "mullusi.internal.", expected: "mullusi.internal." },
    { value: "  mullusi.internal  ", expected: "mullusi.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { MULLUSI_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { MULLUSI_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("mullusi.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "mullusi.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN mullusi.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_mullusi-gw._tcp IN PTR studio-london._mullusi-gw._tcp`,
      `studio-london._mullusi-gw._tcp IN SRV 0 0 18790 studio-london`,
      `displayName=Mac Studio (Mullusi)`,
      `gatewayPort=18790`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/mullusi`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN mullusi.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._mullusi-gw._tcp IN TXT`,
        `displayName=Mac Studio (Mullusi)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("mullusi.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("mullusi.internal."),
