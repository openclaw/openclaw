...
export function buildCronEventPrompt(pendingEvents, opts) {
...
export function buildExecEventPrompt(pendingEvents, opts) {
    const deliverToUser = opts?.deliverToUser ?? true;
    const rawEventText = pendingEvents.join("\n").trim();
    const eventText = rawEventText.length > MAX_EXEC_EVENT_PROMPT_CHARS
    ...
    if (!deliverToUser) {
        return ("An async command completion event was triggered, but user delivery is disabled for this run. " +
...
export function isExecCompletionEvent(evt) {
    ...
    return (/^exec finished(?::|\s*\()/.test(normalized) ||
        /^exec (completed|failed) \([a-z0-9_-]{1,64}, (code -?\d+|signal [^)]+)\)( :: .*)?$/.test(normalized));
}
