...
import { normalizeExecApprovalAgentId } from "./exec-approvals.js";
...
export function resolveTrustAuditPath(agentId?: string): string {
  return expandHomePrefix(`~/.openclaw/trust-audit-${normalizeExecApprovalAgentId(agentId)}.jsonl`);
}
...
export function tryAppendTrustAuditEntry(params: {
  agentId?: string;
  command: string;
  exitCode?: number | null;
  durationMs?: number | null;
  ...
  try {
    appendTrustAuditEntry(params);
  } catch (error) {
    // Best-effort audit; command outcome is authoritative.
    logInfo(
      `exec: trust audit append failed${params.logLabel ? ` (${params.logLabel})` : ""}: ${String(error)}`,
    );
  }
}
