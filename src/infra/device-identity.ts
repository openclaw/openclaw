...
export function loadOrCreateDeviceIdentity(): DeviceIdentity {
  try {
    const parsed = getCoreSettingFromDb<StoredIdentity>(SCOPE);
    if (
      parsed?.version === 1 &&
      ...
      if (derivedId && derivedId !== parsed.deviceId) {
        // Self-heal: re-derive device ID from public key
        const updated: StoredIdentity = { ...parsed, deviceId: derivedId };
        setCoreSettingInDb(SCOPE, "", updated);
        return {
          deviceId: derivedId,
  ...
  setCoreSettingInDb(SCOPE, "", stored);
  return identity;
}
