...
const UNIX_BASE_TRUSTED_DIRS = ["/usr/bin", "/bin", "/usr/sbin", "/sbin"];
...
const WIN_PATHEXT = [".exe", ".cmd", ".bat", ".com"];
const resolvedCacheStrict = new Map();
const resolvedCacheStandard = new Map();
 ...
 */
export function resolveSystemBin(name, opts) {
...
export function _getTrustedDirs(trust = "strict") {
    return getTrustedDirs(trust);
}
/** Reset cache and optionally override the executable-check function (for tests). */
export function _resetResolveSystemBin(overrideIsExecutable) {
