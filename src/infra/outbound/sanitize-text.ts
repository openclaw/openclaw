...
export function sanitizeForPlainText(text: string, options: { style?: "markdown" } = {}): string {
  ...
  for (const region of codeRegions) {
    sanitized += convertHtmlOutsideCode(prepared.slice(cursor, region.start), options);
    sanitized += prepared.slice(region.start, region.end);
    cursor = region.end;
  }
  return sanitized + convertHtmlOutsideCode(prepared.slice(cursor), options);
}
