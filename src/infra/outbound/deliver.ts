...
function normalizePayloadsForChannelDelivery(
  ...
  const canInjectTelegramButtons =
    channel === "telegram" && resolveTelegramInlineButtonsScope({ cfg, accountId }) !== "off";
...
async function deliverOutboundPayloadsCore(
  ...
  const normalizedPayloads = normalizePayloadsForChannelDelivery(payloads, channel, cfg, accountId);
