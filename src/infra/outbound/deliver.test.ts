...
const { deliverOutboundPayloads, normalizeOutboundPayloads } =
...
describe("deliverOutboundPayloads", () => {
  ...
  it("clamps telegram text chunk size to protocol max even with higher config", async () => {
    ...
    expect(sentHtmlChunks.every((message) => message.length <= 4096)).toBe(
      true,
    );
  });
  ...
  it("includes OpenClaw tmp root in imessage mediaLocalRoots", async () => {
    const sendIMessage = vi
      .fn()
      .mockResolvedValue({ messageId: "i1", chatId: "chat-1" });
  ...
  it("mirrors delivered output when mirror options are provided", async () => {
    ...
    expect(
      mocks.appendAssistantMessageToSessionTranscript,
    ).toHaveBeenCalledWith(expect.objectContaining({ text: "report.pdf" }));
  });
  ...
  it("emits message_sent success for sendPayload deliveries", async () => {
    ...
    const sendPayload = vi
      .fn()
      .mockResolvedValue({ channel: "matrix", messageId: "mx-1" });
  ...
  it("emits message_sent failure when delivery errors", async () => {
    ...
    const sendWhatsApp = vi
      .fn()
      .mockRejectedValue(new Error("downstream failed"));
