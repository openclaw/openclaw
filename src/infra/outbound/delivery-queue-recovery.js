...
import { ackDelivery, failDelivery, loadPendingDelivery, loadPendingDeliveries, moveToFailed, } from "./delivery-queue-storage.js";
const MAX_RETRIES = 5;
...
export async function withActiveDeliveryClaim(entryId, fn) {
    ...
    try {
        ...
        for (const entry of matchingEntries) {
            ...
            try {
                ...
                if (result === "recovered") {
                    opts.log.info(`${opts.logLabel}: drained delivery ${currentEntry.id} on ${currentEntry.channel}`);
                }
            }
            finally {
