...
describe("message action sandbox media hydration", () => {
  maybeIt(
    "rejects symlink retarget escapes after sandbox media normalization",
    async () => {
      const sandboxRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "msg-params-sandbox-"),
      );
      const outsideRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "msg-params-outside-"),
      );
      try {
        const insideDir = path.join(sandboxRoot, "inside");
        await fs.mkdir(insideDir, { recursive: true });
        await fs.writeFile(
          path.join(insideDir, "note.txt"),
          "INSIDE_SECRET",
          "utf8",
        );
        await fs.writeFile(
          path.join(outsideRoot, "note.txt"),
          "OUTSIDE_SECRET",
          "utf8",
        );
