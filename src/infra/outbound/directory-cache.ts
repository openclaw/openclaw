...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MullusiConfig | null = null;
  ...
  get(key: string, cfg: MullusiConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MullusiConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: MullusiConfig): void {
