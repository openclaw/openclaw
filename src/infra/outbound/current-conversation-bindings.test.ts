...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.MULLUSI_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-current-bindings-"));
    process.env.MULLUSI_STATE_DIR = testStateDir;
