...
describe("delivery-index", () => {
  ...
  it("adds and queries entries", () => {
    ...
    const telegramEntries = queryIndex({ channel: "telegram" });
    expect(telegramEntries).toHaveLength(1);
    expect(telegramEntries[0].id).toBe("d1");
  });

  it("removes entries", () => {
    addToIndex({ id: "d1", channel: "telegram", enqueuedAt: 1000, lanePriority: "user-visible" });
    removeFromIndex("d1");
    expect(getIndexSize()).toBe(0);
  });

  it("filters by channel and accountId", () => {
    addToIndex({
      id: "d1",
      channel: "telegram",
      accountId: "default",
    ...
    const filtered = queryIndex({ channel: "telegram", accountId: "default" });
    expect(filtered).toHaveLength(1);
    expect(filtered[0].id).toBe("d1");
  });

  it("returns entries sorted by enqueuedAt", () => {
    addToIndex({ id: "d2", channel: "telegram", enqueuedAt: 2000, lanePriority: "user-visible" });
    addToIndex({ id: "d1", channel: "telegram", enqueuedAt: 1000, lanePriority: "user-visible" });
    const all = queryIndex();
