...
import { mergeDeliveryContext, normalizeDeliveryContext, } from "../utils/delivery-context.shared.js";
const MAX_EVENTS = 20;
...
function areDeliveryContextsEqual(left, right) {
    ...
    return ((left.channel ?? undefined) === (right.channel ?? undefined) &&
...
export function consumeSystemEventEntries(sessionKey, consumedEntries) {
    const key = requireSessionKey(sessionKey);
    const entry = getSessionQueue(key);
    if (!entry || entry.queue.length === 0 || consumedEntries.length === 0) {
        return [];
    }
    if (consumedEntries.length > entry.queue.length ||
