...
const PRIMARY_PACKAGE_NAME = "mullusi";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const MULLUSI_MAIN_PACKAGE_SPEC = "github:mullusi/mullusi#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Mullusi", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.MULLUSI_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.MULLUSI_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return MULLUSI_MAIN_PACKAGE_SPEC;
  }
