...
async function maybeUpgradeCnOnlyCert(params: {
  ...
  try {
    const certPem = await fs.readFile(params.certPath, "utf8");
    if (!isOpenClawGeneratedCnOnlyCert(certPem)) {
      return false;
    }
    params.log?.warn?.("gateway tls: upgrading CN-only certificate to include subjectAltName");
    // Remove old files so generateSelfSignedCert can create fresh ones.
    await fs.unlink(params.certPath).catch(() => {});
    await fs.unlink(params.keyPath).catch(() => {});
