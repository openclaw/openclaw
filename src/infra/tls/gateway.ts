...
export async function loadGatewayTlsRuntime(
  ...
  try {
    ...
    if (
      autoGenerate &&
      caPath === undefined &&
      (await maybeUpgradeCnOnlyCert({ certPath, keyPath, log }))
    ) {
      // cert was regenerated — re-read the fresh material
      const freshCert = await fs.readFile(certPath, "utf8");
      const freshKey = await fs.readFile(keyPath, "utf8");
      const built = buildTlsResult(certPath, keyPath, undefined, freshCert, freshKey, undefined);
      if ("error" in built) {
        return {
          enabled: false,
          required: true,
