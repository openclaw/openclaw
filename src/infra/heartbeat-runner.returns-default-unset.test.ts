...
describe("runHeartbeatOnce", () => {
  ...
  it("runs heartbeat when HEARTBEAT.md does not exist (model decides what to do)", async () => {
    const tmpDir = await createCaseDir("openclaw-hb");
    const storePath = path.join(tmpDir, "sessions.json");
    const workspaceDir = path.join(tmpDir, "workspace");
    const replySpy = vi.spyOn(replyModule, "getReplyFromConfig");
    try {
      await fs.mkdir(workspaceDir, { recursive: true });
      // Don't create HEARTBEAT.md - per docs, heartbeat still runs
      ...
      // HEARTBEAT.md is optional — heartbeat should still run
      // and the model responds with HEARTBEAT_OK (nothing to do)
      expect(res.status).toBe("ran");
      expect(replySpy).toHaveBeenCalled();
    } finally {
      replySpy.mockRestore();
