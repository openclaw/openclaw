...
describe("normalizePackageTagInput", () => {
  const packageNames = ["evox", "@openclaw/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("evox@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@openclaw/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("evox@   ", packageNames)).toBeNull();
  });
