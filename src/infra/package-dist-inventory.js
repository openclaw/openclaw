...
function isPackagedDistPath(relativePath) {
    ...
    if (OMITTED_PRIVATE_QA_PLUGIN_SDK_PREFIXES.some((prefix) => relativePath.startsWith(prefix)) ||
...
async function collectRelativeFiles(rootDir, baseDir) {
    ...
    try {
        ...
        if (!rootStats.isDirectory() || rootStats.isSymbolicLink()) {
            throw new Error(`Unsafe package dist path: ${normalizeRelativePath(path.relative(baseDir, rootDir))}`);
        }
...
export async function collectPackageDistInventory(packageRoot) {
    return await collectRelativeFiles(path.join(packageRoot, "dist"), packageRoot);
}
export async function writePackageDistInventory(packageRoot) {
    const inventory = [
        ...new Set([
...
export async function readPackageDistInventory(packageRoot) {
    const inventoryPath = path.join(packageRoot, PACKAGE_DIST_INVENTORY_RELATIVE_PATH);
...
export async function readPackageDistInventoryIfPresent(packageRoot) {
    try {
...
export async function collectPackageDistInventoryErrors(packageRoot) {
