...
// Heredoc is a common pattern for multi-line scripts generated by agents.
// Only flag when the heredoc body contains secondary obfuscation indicators.
const HEREDOC_SUSPICIOUS_PATTERNS = [
  /base64\s+(?:-d|--decode)\b/i,
  /\beval\b.*(?:base64|decode|\\x[0-9a-f])/i,
  /\$'(?:[^']*\\[0-7]{3}){2,}/,
  /\$'(?:[^']*\\x[0-9a-fA-F]{2}){2,}/,
  /(?:curl|wget)\s+.*\|\s*(?:sh|bash|zsh)\b/i,
  /\bxxd\b.*-r.*\|\s*(?:sh|bash|zsh)\b/i,
  /\bprintf\b.*\\x[0-9a-f].*\|\s*(?:sh|bash|zsh)\b/i,
];

function shouldSuppressHeredocExec(command: string): boolean {
  return !HEREDOC_SUSPICIOUS_PATTERNS.some((pattern) => pattern.test(command));
}
...
export function detectCommandObfuscation(command: string): ObfuscationDetection {
  ...
  for (const pattern of OBFUSCATION_PATTERNS) {
    ...
    const suppressed =
      ...
      (pattern.id === "shell-heredoc-exec" && shouldSuppressHeredocExec(normalizedCommand));
