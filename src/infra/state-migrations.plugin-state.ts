...
export async function migrateLegacyInstalledPluginIndex(params: {
  ...
  if (current) {
    // SQLite ledger existence marks migration completion, so JSON-only records are stale residue.
    // Archive the retired store without parsing it so startup has one canonical source.
