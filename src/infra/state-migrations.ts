...
async function migrateLegacyMemoryStore(params: {
  ...
  if (targetExists) {
    try {
      ...
      const db = new requireNodeSqlite().DatabaseSync(targetPath, { readOnly: true });
  ...
  try {
    ...
    const checkLegacySchema = (db: DatabaseSync): boolean => {
      const hasMeta =
    ...
    const legacyDb = new requireNodeSqlite().DatabaseSync(legacyPath, { readOnly: true });
    ...
    const db = new requireNodeSqlite().DatabaseSync(targetPath);
