...
function shouldBlockWorkspaceDotEnvKey(key) {
    ...
    return (shouldBlockWorkspaceRuntimeDotEnvKey(upper) ||
...
export function loadRuntimeDotEnvFile(filePath, opts) {
...
export function loadWorkspaceDotEnvFile(filePath, opts) {
    const parsed = readDotEnvFile({
        filePath,
...
export function loadGlobalRuntimeDotEnvFiles(opts) {
    ...
    const hasExplicitNonDefaultStateDir = process.env.OPENCLAW_STATE_DIR?.trim() !== undefined &&
    ...
    if (!hasExplicitNonDefaultStateDir) {
        parsedFiles.push(readDotEnvFile({
