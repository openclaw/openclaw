...
const buildAccountDetails = (params: {
  ...
  if (snapshot.botTokenSource && snapshot.botTokenSource !== "none") {
    details.push(`bot:${snapshot.botTokenSource}`); // pragma: allowlist secret (status string)
  }
  if (snapshot.appTokenSource && snapshot.appTokenSource !== "none") {
    details.push(`app:${snapshot.appTokenSource}`); // pragma: allowlist secret (status string)
  }
  if (snapshot.signingSecretSource && snapshot.signingSecretSource !== "none") {
    details.push(`signing:${snapshot.signingSecretSource}`); // pragma: allowlist secret (status string)
  }
