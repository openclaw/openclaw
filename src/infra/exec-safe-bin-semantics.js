...
export function normalizeSafeBinName(raw) {
...
export function getSafeBinSemanticRule(binName) {
...
export function validateSafeBinSemantics(params) {
    return getSafeBinSemanticRule(params.binName)?.validate?.(params) ?? true;
}
export function listRiskyConfiguredSafeBins(entries) {
    const hits = new Map();
