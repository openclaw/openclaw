...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/mullusi-state", "devices")).toEqual({
      dir: path.join("/tmp/mullusi-state", "devices"),
      pendingPath: path.join("/tmp/mullusi-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/mullusi-state", "devices", "paired.json"),
    });
  });
