...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__mullusi_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "mullusi-root");
...
function setPackageRoot(root: string, name = "mullusi") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./mullusi-root.js").resolveMullusiPackageRootSync,
  asyncResolver: typeof import("./mullusi-root.js").resolveMullusiPackageRoot,
  opts: Parameters<typeof import("./mullusi-root.js").resolveMullusiPackageRootSync>[0],
...
vi.mock("./mullusi-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveMullusiPackageRoot", () => {
  let resolveMullusiPackageRoot: typeof import("./mullusi-root.js").resolveMullusiPackageRoot;
  let resolveMullusiPackageRootSync: typeof import("./mullusi-root.js").resolveMullusiPackageRootSync;
  ...
  beforeAll(async () => {
    ({ resolveMullusiPackageRoot, resolveMullusiPackageRootSync } =
      await import("./mullusi-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "mullusi");
        const pkgRoot = path.join(project, "node_modules", "mullusi");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "mullusi");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "mullusi.mjs")));
    ...
    {
      name: "falls through from a non-mullusi moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-mullusi");
    ...
    {
      name: "returns null for non-mullusi package roots",
      setup: () => {
        const pkgRoot = fx("not-mullusi");
        setPackageRoot(pkgRoot, "not-mullusi");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "mullusi.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "mullusi");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
