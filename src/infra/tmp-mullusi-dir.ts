...
export const POSIX_MULLUSI_TMP_DIR = "/tmp/mullusi";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredMullusiTmpDirOptions = {
...
export function resolvePreferredMullusiTmpDir(
  options: ResolvePreferredMullusiTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "mullusi" : `mullusi-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[mullusi] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Mullusi temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Mullusi temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_MULLUSI_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_MULLUSI_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_MULLUSI_TMP_DIR)) {
      return POSIX_MULLUSI_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_MULLUSI_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_MULLUSI_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_MULLUSI_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_MULLUSI_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_MULLUSI_TMP_DIR;
