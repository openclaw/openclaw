...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.SMART_AGENT_NEO_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.SMART_AGENT_NEO_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.SMART_AGENT_NEO_TEST_TAILSCALE_BINARY;
    } else {
      process.env.SMART_AGENT_NEO_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
