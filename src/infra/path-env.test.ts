...
let ensureActiviCliOnPath: typeof import("./path-env.js").ensureActiviCliOnPath;

describe("ensureActiviCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ACTIVI_PATH_BOOTSTRAPPED",
    "ACTIVI_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureActiviCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling activi exists", () => {
    const tmp = abs("/tmp/activi-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "activi");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.ACTIVI_PATH_BOOTSTRAPPED;

    ensureActiviCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.ACTIVI_PATH_BOOTSTRAPPED = "1";
    ensureActiviCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/activi-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "activi");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/activi-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "activi");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "activi");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/activi-path/case-linuxbrew");
