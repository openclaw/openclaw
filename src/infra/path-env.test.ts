...
import { ensureDNACliOnPath } from "./path-env.js";

describe("ensureDNACliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling dna exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "dna-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "dna");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env.DNA_PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env.DNA_PATH_BOOTSTRAPPED;
      try {
        ensureDNACliOnPath({
          execPath: cliPath,
          cwd: tmp,
        ...
        if (originalFlag === undefined) delete process.env.DNA_PATH_BOOTSTRAPPED;
        else process.env.DNA_PATH_BOOTSTRAPPED = originalFlag;
      }
    } finally {
  ...
  it("is idempotent", () => {
    ...
    process.env.DNA_PATH_BOOTSTRAPPED = "1";
    try {
      ensureDNACliOnPath({
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "dna");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "dna");
