...
const VITEST_FS_BASE = path.join(
...
const isFixturePath = (p: string) => {
  ...
  return (
    resolved === vitestRootWithSep.slice(0, -1) ||
    resolved.startsWith(vitestRootWithSep)
  );
};
...
describe("resolveOpenClawPackageRoot", () => {
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOpenClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(
      pkgRoot,
    );
  });
  ...
  it("async resolver returns null when no package roots exist", async () => {
    await expect(
      resolveOpenClawPackageRoot({ cwd: fx("missing") }),
    ).resolves.toBeNull();
  });
});
