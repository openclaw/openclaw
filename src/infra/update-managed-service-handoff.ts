...
function recordUpdateHandoffOutcome(reason, restored, completedStatus, expectedRevision) {
  ...
  // Cancellation must preserve a refusal or late failure already recorded by the Gateway.
  if (reason === "managed-service-handoff-cancelled" && run?.reason && run.steps.some(({ step, status }) =>
      status === "failed" && ["requested", "managed-service-handoff-finalization"].includes(step))) reason = run.reason;
