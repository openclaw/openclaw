...
import { resolveMissingRequestedScope, resolveScopeOutsideRequestedRoles, roleScopesAllow, } from "../shared/operator-scope-compat.js";
import { createAsyncLock, pruneExpiredPending, readJsonFile, reconcilePendingPairingRequests, resolvePairingPaths, writeJsonAtomic, } from "./pairing-files.js";
...
export function formatDevicePairingForbiddenMessage(result) {
...
function listActiveTokenRoles(tokens) {
    ...
    return mergeRoles(Object.values(tokens)
        .filter((entry) => !entry.revokedAtMs)
        .map((entry) => entry.role));
}
export function listApprovedPairedDeviceRoles(device) {
...
export function listEffectivePairedDeviceRoles(device) {
...
export function hasEffectivePairedDeviceRole(device, role) {
...
function samePendingApprovalSnapshot(existing, incoming) {
    ...
    if (!sameStringSet(resolveRequestedRoles(existing), resolveRequestedRoles(incoming)) ||
...
export async function getPendingDevicePairing(requestId, baseDir) {
    const state = await loadState(baseDir);
...
export async function approveDevicePairing(requestId, optionsOrBaseDir, maybeBaseDir) {
    const options = typeof optionsOrBaseDir === "string" || optionsOrBaseDir === undefined
        ? undefined
        : optionsOrBaseDir;
...
export async function approveBootstrapDevicePairing(requestId, bootstrapProfile, baseDir) {
    ...
    return await withLock(async () => {
        ...
        for (const roleForToken of approvedRoles) {
            const existingToken = tokens[roleForToken];
            const tokenScopes = roleForToken === OPERATOR_ROLE
