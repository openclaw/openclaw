...
import { resolveSmartAgentNeoPackageRoot, resolveSmartAgentNeoPackageRootSync } from "./smart-agent-neo-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveSmartAgentNeoPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "smart-agent-neo" + dist/control-ui/index.html
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (fs.existsSync(pkgJsonPath)) {
      try {
        ...
        if (parsed.name === "smart-agent-neo") {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveSmartAgentNeoPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // smart-agent-neo.mjs or dist/<bundle>.js
