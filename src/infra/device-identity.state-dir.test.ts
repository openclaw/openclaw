...
describe("device identity state dir defaults", () => {
  ...
  it("writes the default identity file under SMART_AGENT_NEO_STATE_DIR", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-identity-state-"));
