...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-update-check-"));
    process.env.EASYHUB_STATE_DIR = tempDir;
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveEasyHubPackageRoot } = await import("./easyhub-root.js");
    ...
    vi.mocked(resolveEasyHubPackageRoot).mockResolvedValue("/opt/EasyHub");
