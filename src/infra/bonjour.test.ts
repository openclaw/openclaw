...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.EASYHUB_MDNS_HOSTNAME = "test-host";
    process.env.EASYHUB_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("EasyHub-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("EasyHub (EasyHub)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("EasyHub");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("EasyHub.local");
