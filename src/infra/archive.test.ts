...
describe("archive utils", () => {
  it("detects archive kinds", () => {
    ...
    for (const testCase of cases) {
      expect(resolveArchiveKind(testCase.input), testCase.input).toBe(
        testCase.expected,
      );
    }
  });
  ...
  it.each([{ ext: "zip" as const }, { ext: "tar" as const }])(
    ...
    async ({ ext }) => {
      await withArchiveCase(
        ext,
        async ({ workDir, archivePath, extractDir }) => {
  ...
  it("fails resolvePackedRootDir when extract dir has multiple root dirs", async () => {
    ...
    await expect(resolvePackedRootDir(extractDir)).rejects.toThrow(
      /unexpected archive layout/i,
    );
  });
