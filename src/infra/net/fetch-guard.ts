...
export type GuardedFetchOptions = {
  ...
  /** Resolve a per-hop override so redirects can change proxy or direct routing. */
...
async function fetchWithSsrFGuardInternal(
  ...
  while (true) {
    ...
    try {
      ...
      const resolvedPolicy = params.resolveDispatcherPolicy?.(parsedUrl);
      const dispatcherPolicy =
        (isPromiseLike<PinnedDispatcherPolicy | undefined>(resolvedPolicy)
          ? await racePromiseWithAbortSignal(Promise.resolve(resolvedPolicy), signal)
          : resolvedPolicy) ?? params.dispatcherPolicy;
      signal?.throwIfAborted();
