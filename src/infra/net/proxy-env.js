 ...
 */
export function resolveEnvHttpProxyUrl(protocol, env = process.env) {
...
export function hasEnvHttpProxyConfigured(protocol = "https", env = process.env) {
    return resolveEnvHttpProxyUrl(protocol, env) !== undefined;
}
export function shouldUseEnvHttpProxyForUrl(targetUrl, env = process.env) {
    let protocol;
    try {
 ...
 * SSRF bypass.
 */
export function matchesNoProxy(targetUrl, env = process.env) {
