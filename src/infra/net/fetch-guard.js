...
import { assertHostnameAllowedWithPolicy, closeDispatcher, createPinnedDispatcher, resolvePinnedHostnameWithPolicy, SsrFBlockedError, } from "./ssrf.js";
...
export function withTrustedExplicitProxyGuardedFetchMode(params) {
...
function assertExplicitProxySupportsPinnedDns(url, dispatcherPolicy, pinDns) {
    if (pinDns !== false &&
        ...
        url.protocol !== "https:") {
        throw new Error("Explicit proxy SSRF pinning requires HTTPS targets; plain HTTP targets are not supported");
    }
}
function createPolicyDispatcherWithoutPinnedDns(dispatcherPolicy, timeoutMs) {
    ...
    if (dispatcherPolicy.mode === "direct") {
        return createHttp1Agent(dispatcherPolicy.connect ? { connect: { ...dispatcherPolicy.connect } } : undefined, timeoutMs);
    }
    ...
    if (dispatcherPolicy.proxyTls) {
        return createHttp1ProxyAgent({ uri: proxyUrl, requestTls: { ...dispatcherPolicy.proxyTls } }, timeoutMs);
    }
...
function isAmbientGlobalFetch(params) {
    return (typeof params.fetchImpl === "function" &&
        typeof params.globalFetch === "function" &&
        params.fetchImpl === params.globalFetch);
}
export function retainSafeHeadersForCrossOriginRedirectHeaders(headers) {
...
function rewriteRedirectInitForMethod(params) {
    ...
    const shouldForceGet = status === 303
...
export async function fetchWithSsrFGuard(params) {
    ...
    while (true) {
        ...
        let dispatcher = null;
        try {
            const usesTrustedExplicitProxyMode = mode === GUARDED_FETCH_MODE.TRUSTED_EXPLICIT_PROXY &&
            ...
            const canUseTrustedEnvProxy = mode === GUARDED_FETCH_MODE.TRUSTED_ENV_PROXY &&
            ...
            if (canUseTrustedEnvProxy) {
                ...
                dispatcher = createPinnedDispatcher(pinned, params.dispatcherPolicy, params.policy, timeoutMs);
            }
            const init = {
            ...
            const supportsDispatcherInit = (params.fetchImpl !== undefined &&
