...
export function startCallTraceWriter(config: OpenClawConfig): (() => void) | undefined {
  ...
  if (!logLlmUsage && !logLlmCalls && !logToolCalls) return undefined;
  ...
  const unsub = onDiagnosticEvent((evt: DiagnosticEventPayload) => {
    if (evt.type === "model.usage" && !logLlmUsage) return;
    if (evt.type === "model.call" && !logLlmCalls) return;
    if (evt.type === "tool.call" && !logToolCalls) return;
    if (evt.type !== "model.usage" && evt.type !== "model.call" && evt.type !== "tool.call") return;
