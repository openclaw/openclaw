...
export function startCallTraceWriter(config: OpenClawConfig): (() => void) | undefined {
  ...
  if (!logLlmCalls && !logToolCalls) return undefined;
  ...
  if (logLlmCalls) ensureDir(llmDir);
  if (logToolCalls) ensureDir(toolDir);

  // Purge stale files on startup (best-effort).
  if (logLlmCalls) purgeOldFiles(llmDir, retainDays);
  if (logToolCalls) purgeOldFiles(toolDir, retainDays);
