...
function shouldTriggerForSession(params: {
  ...
  // Must have activity
  if (!entry.updatedAt) {
    log.debug("shouldTrigger: no updatedAt", { sessionKey, triggerKey });
    return false;
  }
  ...
  if (typeof lastTriggeredAt === "number" && entry.updatedAt <= lastTriggeredAt) {
    log.debug("shouldTrigger: already triggered (no new activity)", {
      sessionKey,
      triggerKey,
      updatedAt: entry.updatedAt,
      lastTriggeredAt,
      diff: entry.updatedAt - lastTriggeredAt,
    });
    return false;
  }
