...
export function triggerMullusiRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.MULLUSI_SYSTEMD_UNIT,
      process.env.MULLUSI_PROFILE,
    );
  ...
  const label =
    process.env.MULLUSI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MULLUSI_PROFILE);
