...
export function triggerEasyHubRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.EASYHUB_SYSTEMD_UNIT,
        process.env.EASYHUB_PROFILE,
      );
  ...
  const label =
    process.env.EASYHUB_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.EASYHUB_PROFILE);
