...
export function triggerDNARestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.DNA_SYSTEMD_UNIT,
        process.env.DNA_PROFILE,
      );
  ...
  const label =
    process.env.DNA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.DNA_PROFILE);
