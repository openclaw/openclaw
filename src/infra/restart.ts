...
export function triggerActiviRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ACTIVI_SYSTEMD_UNIT,
        process.env.ACTIVI_PROFILE,
      );
  ...
  const label =
    process.env.ACTIVI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ACTIVI_PROFILE);
