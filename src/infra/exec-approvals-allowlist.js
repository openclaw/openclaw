...
import { analyzeShellCommand, isWindowsPlatform, matchAllowlist, resolveExecutionTargetCandidatePath, resolveExecutionTargetResolution, resolveCommandResolutionFromArgv, resolvePolicyTargetCandidatePath, resolvePolicyTargetResolution, splitCommandChain, splitCommandChainWithOperators, } from "./exec-approvals-analysis.js";
import { detectInterpreterInlineEvalArgv, isInterpreterLikeAllowlistPattern, } from "./exec-inline-eval.js";
import { DEFAULT_SAFE_BINS, SAFE_BIN_PROFILES, validateSafeBinArgv, } from "./exec-safe-bin-policy.js";
import { isTrustedSafeBinPath } from "./exec-safe-bin-trust.js";
import { extractShellWrapperInlineCommand, isShellWrapperExecutable, normalizeExecutableToken, POWERSHELL_WRAPPERS, } from "./exec-wrapper-resolution.js";
...
function buildSkillBinTrustIndex(entries) {
    ...
    for (const entry of entries) {
        ...
        const paths = trustByName.get(name) ?? new Set();
...
function resolveAllowlistedSkillWrapperId(segment) {
    const execution = resolveExecutionTargetResolution(segment.resolution);
    const executableName = normalizeExecutableToken(execution?.executableName ?? segment.argv[0] ?? "");
...
function resolveShellWrapperScriptArgv(params) {
    ...
    if (idx === -1) {
        idx = params.effectiveArgv.findIndex((a) => normalizeLowercaseStringOrEmpty(path.basename(a)) === scriptBase);
    }
...
function resolveSegmentAllowlistMatch(params) {
    const effectiveArgv = params.segment.resolution?.effectiveArgv && params.segment.resolution.effectiveArgv.length > 0
        ? params.segment.resolution.effectiveArgv
        : params.segment.argv;
    const allowlistSegment = effectiveArgv === params.segment.argv
        ? params.segment
    ...
    const executableMatch = isPositionalCarrierInvocation
        ? null
        : matchAllowlist(params.context.allowlist, candidateResolution, effectiveArgv, params.context.platform);
    ...
    const shellPositionalArgvMatch = shellPositionalArgvCandidatePath
        ? matchAllowlist(params.context.allowlist, {
        ...
        }, undefined, params.context.platform)
        : null;
    const shellScriptCandidatePath = inlineCommand === null
    ...
    const shellScriptMatch = shellScriptCandidatePath && shellScriptArgv
        ...
        }, shellScriptArgv, params.context.platform)
        : null;
    return {
        effectiveArgv,
        inlineCommand,
...
function buildScriptArgPatternFromArgv(argv, scriptPath, cwd, platform) {
    ...
    if (scriptIdx === -1) {
        scriptIdx = argv.findIndex((arg) => normalizeLowercaseStringOrEmpty(path.basename(arg)) === scriptBase);
    }
...
function addAllowAlwaysPattern(out, pattern, argPattern) {
    const exists = out.some((p) => p.pattern === pattern && (p.argPattern ?? undefined) === (argPattern ?? undefined));
    if (!exists) {
...
function collectAllowAlwaysPatterns(params) {
    ...
    const segment = trustPlan.argv === params.segment.argv
        ? params.segment
        : {
    ...
    if (isInterpreterLikeAllowlistPattern(candidatePath)) {
        ...
        if (params.strictInlineEval !== true ||
    ...
    const isPowerShellFileInvocation = POWERSHELL_WRAPPERS.has(normalizeExecutableToken(segment.argv[0] ?? "")) &&
        segment.argv.some((t) => {
...
export function evaluateShellAllowlist(params) {
    ...
    const allowSkillPreludeAtIndex = new Set();
    ...
    for (let index = finalizedEvaluations.length - 1; index >= 0; index -= 1) {
        ...
        const preludeSkillIds = isPreludeOnly
            ...
            : new Set();
        const reachesTrustedSkillExecution = opToNext === "&&" &&
