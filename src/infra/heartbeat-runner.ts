...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ActiviConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ActiviConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ActiviConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: ActiviConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ActiviConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ActiviConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ActiviConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
