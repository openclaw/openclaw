...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SmartAgentNeoConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: SmartAgentNeoConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: SmartAgentNeoConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: SmartAgentNeoConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SmartAgentNeoConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SmartAgentNeoConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SmartAgentNeoConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
