...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: MullusiConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: MullusiConfig) {
...
function resolveHeartbeatAgents(cfg: MullusiConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: MullusiConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: MullusiConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: MullusiConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
