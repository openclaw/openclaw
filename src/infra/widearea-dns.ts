...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.EASYHUB_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*EasyHub-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "EasyHub";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "EasyHub");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "EasyHub-gw");
  ...
  records.push(`_EasyHub-gw._tcp IN PTR ${instanceLabel}._EasyHub-gw._tcp`);
  records.push(`${instanceLabel}._EasyHub-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._EasyHub-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; EasyHub-content-hash: ${contentHash}\n${contentBody}`;
}
