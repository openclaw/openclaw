...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ACTIVI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*activi-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "activi";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "activi");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "activi-gw");
  ...
  records.push(`_activi-gw._tcp IN PTR ${instanceLabel}._activi-gw._tcp`);
  records.push(`${instanceLabel}._activi-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._activi-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; activi-content-hash: ${contentHash}\n${contentBody}`;
}
