...
export const WIDE_AREA_DISCOVERY_DOMAIN = "dna.internal.";
export const WIDE_AREA_ZONE_FILENAME = "dna.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*dna-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "dna";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "dna");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "dna-gw");
  ...
  records.push(`_dna-gw._tcp IN PTR ${instanceLabel}._dna-gw._tcp`);
  records.push(`${instanceLabel}._dna-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._dna-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; dna-content-hash: ${contentHash}\n${contentBody}`;
}
