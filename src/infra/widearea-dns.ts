...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SMART_AGENT_NEO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*smart-agent-neo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "smart-agent-neo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "smart-agent-neo");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "smart-agent-neo-gw");
  ...
  records.push(`_smart-agent-neo-gw._tcp IN PTR ${instanceLabel}._smart-agent-neo-gw._tcp`);
  records.push(`${instanceLabel}._smart-agent-neo-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._smart-agent-neo-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; smart-agent-neo-content-hash: ${contentHash}\n${contentBody}`;
}
