...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MULLUSI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*mullusi-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "mullusi";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "mullusi");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "mullusi-gw");
  ...
  records.push(`_mullusi-gw._tcp IN PTR ${instanceLabel}._mullusi-gw._tcp`);
  records.push(`${instanceLabel}._mullusi-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._mullusi-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; mullusi-content-hash: ${contentHash}\n${contentBody}`;
}
