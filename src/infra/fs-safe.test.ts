...
describe("fs-safe", () => {
  ...
  it("removes a file within root through the shared helper", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
  ...
  it("removes an empty directory within root through the shared helper", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
  ...
  it("rejects non-recursive removal of non-empty directories through the shared helper", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
  ...
  it("removes directory trees recursively through the shared helper", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
  ...
  it("rejects symlink and junction targets through the shared helper", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
