...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("excludes pids whose command does not include 'evox'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple evox pids from a single lsof output block", () => {
    ...
    it("returns [] when status 0 but only non-openclaw pids present", () => {
      ...
      // only tracks evox processes — non-openclaw listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty evox stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty evox stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + evox pid in stdout — container-restricted lsof reports partial results
          return createOpenClawBusyResult(stalePid, {
            status: 1,
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'evox'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("evox")
      // inside the mid-loop flush: a non-openclaw cmd between two entries must not
      // be pushed, but the following evox entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-openclaw entry first, then evox entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid evox entry must still be found.
      const stdout = `p0\ncopenclaw-gateway\np${stalePid}\ncopenclaw-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero evox pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-openclaw stdout (line 145)", () => {
    it("treats status 1 + non-openclaw stdout as port-free (not an evox process)", () => {
      // status 1 + non-empty stdout where no evox pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill evox pids) it is effectively free.
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no evox pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
