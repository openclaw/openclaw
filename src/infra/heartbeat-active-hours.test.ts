...
describe("isWithinActiveHours", () => {
  ...
  it("returns true when activeHours start/end are invalid", () => {
    ...
    expect(
      isWithinActiveHours(
        cfg,
        heartbeatWindow("08:00", "24:30", "UTC"),
        Date.UTC(2025, 0, 1, 9),
      ),
    ).toBe(true);
  });
  ...
  it("respects user timezone windows for normal ranges", () => {
    ...
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 7, 0, 0)),
    ).toBe(false);
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 8, 0, 0)),
    ).toBe(true);
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 23, 59, 0)),
    ).toBe(true);
  });
  ...
  it("supports overnight ranges", () => {
    ...
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 23, 0, 0)),
    ).toBe(true);
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 5, 30, 0)),
    ).toBe(true);
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 12, 0, 0)),
    ).toBe(false);
  });
  ...
  it("respects explicit non-user timezones", () => {
    ...
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 15, 0, 0)),
    ).toBe(true);
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 23, 30, 0)),
    ).toBe(false);
  });
  ...
  it("falls back to user timezone when activeHours timezone is invalid", () => {
    ...
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 9, 0, 0)),
    ).toBe(true);
    expect(
      isWithinActiveHours(cfg, heartbeat, Date.UTC(2025, 0, 1, 11, 0, 0)),
    ).toBe(false);
  });
});
