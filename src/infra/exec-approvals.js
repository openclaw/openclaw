...
import { normalizeLowercaseStringOrEmpty, normalizeOptionalLowercaseString, normalizeOptionalString, readStringValue, } from "../shared/string-coerce.js";
...
export function normalizeExecTarget(value) {
    const normalized = normalizeOptionalLowercaseString(value);
    if (normalized === "auto") {
        return normalized;
    }
...
export const DEFAULT_EXEC_APPROVAL_ASK_FALLBACK = "full";
...
function sanitizeExecApprovalPolicy(policy) {
    ...
    return {
        security: security === "deny" || security === "allowlist" || security === "full" ? security : undefined,
        ask: ask === "off" || ask === "on-miss" || ask === "always" ? ask : undefined,
        askFallback: askFallback === "deny" || askFallback === "allowlist" || askFallback === "full"
            ? askFallback
            : undefined,
...
export function normalizeExecApprovals(file) {
    ...
    for (const [key, agent] of Object.entries(agents)) {
        ...
        const agentChanged = allowlist !== agent.allowlist ||
...
export function restoreExecApprovalsSnapshot(snapshot) {
    if (!snapshot.exists) {
...
function hasExactCommandDurableExecApproval(params) {
    ...
    return (params.allowlist ?? []).some((entry) => entry.source === "allow-always" &&
...
export function addAllowlistEntry(approvals, agentId, pattern, options) {
    const target = agentId ?? DEFAULT_AGENT_ID;
    const agents = approvals.agents ?? {};
    const existing = agents[target] ?? {};
    const allowlist = Array.isArray(existing.allowlist) ? existing.allowlist : [];
    ...
    const existingEntry = allowlist.find((entry) => entry.pattern === trimmed && (entry.argPattern ?? undefined) === trimmedArgPattern);
    if (existingEntry && (!options?.source || existingEntry.source === options.source)) {
        return;
    }
    const now = Date.now();
    const nextAllowlist = existingEntry
        ? allowlist.map((entry) => entry.pattern === trimmed
            ? {
                ...entry,
...
export function addDurableCommandApproval(approvals, agentId, commandText) {
...
export function resolveExecApprovalRequestAllowedDecisions(params) {
    const explicit = Array.isArray(params?.allowedDecisions)
        ? params.allowedDecisions.filter((decision) => decision === "allow-once" || decision === "allow-always" || decision === "deny")
        : [];
