...
describe("watch-node script", () => {
  ...
  it("isBuildReadyForRestart returns false when build stamp HEAD mismatches git HEAD", () => {
    ...
    const readFileSync = vi
      .fn()
      .mockReturnValue(JSON.stringify({ head: fakeHead, builtAt: Date.now() }));
    const fsMock = {
  ...
  it("isBuildReadyForRestart returns true when build is fresh (entry + stamp + runtime stamp)", () => {
    ...
    const readFileSync = vi.fn().mockImplementation((p: string) => {
      if (p.includes(".runtime-postbuildstamp"))
        return JSON.stringify({ head: sharedHead, syncedAt: now + 1000 });
    ...
    const fsMock = {
      existsSync: vi.fn(
        (p: string) =>
          p.includes("entry.js") ||
          p.includes(".buildstamp") ||
          p.includes(".runtime-postbuildstamp"),
      ),
      readFileSync,
    } as MockFs;
