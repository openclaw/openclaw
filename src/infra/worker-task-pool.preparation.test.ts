...
describe("public worker task preparation custody", () => {
  ...
  it.each(["abort", "close"])(
    "reports delayed callback failure through close after %s rejects the result",
    async (ending) => {
      ...
      try {
        ...
        if (!closing) {
          ...
          await expect(pool.close()).rejects.toBe(cleanupError);
        } else {
          expect(await closeOutcome).toBe(cleanupError);
        }
        await pool.close();
        expect(consumed).toHaveBeenCalledOnce();
        expect(pool.getSnapshot().pendingTasks).toBe(0);
