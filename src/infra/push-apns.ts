...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ACTIVI_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ACTIVI_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ACTIVI_APNS_TEAM_ID and ACTIVI_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ACTIVI_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ACTIVI_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ACTIVI_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ACTIVI_APNS_PRIVATE_KEY_P8 or ACTIVI_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ACTIVI_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createActiviPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    activi: createActiviPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
