...
describe("isVolatileBackupPath", () => {
  it.each([
    ...
    [`${stateDir}/agents/main/agent/tmp/tool-output`, true],
    [`${stateDir}/agents/main/agent/.tmp/inflight`, true],
    [`${stateDir}/agents/main/agent/cache/provider-cache.bin`, true],
    [`${stateDir}/agents/main/agent/shell-snapshots/bash.snapshot`, true],
    [`${stateDir}/agents/main/agent/shell_snapshots/bash.snapshot`, true],
    [`${stateDir}/cache/shell-snapshots/zsh.snapshot`, true],
    [`${stateDir}/cache/shell_snapshots/zsh.snapshot`, true],
    // volatile: managed browser cache paths, but not the whole profile
    ...
    [`${stateDir}/browser/openclaw/user-data/Default/GPUCache/cache.bin`, true],
    [`${stateDir}/browser/openclaw/user-data/Default/Service Worker/CacheStorage/index`, true],
    [`${stateDir}/browser/openclaw/user-data/Default/Service Worker/ScriptCache/index`, true],
    [`${stateDir}/browser/openclaw/user-data/Default/Web Applications/app/Resources/icon`, true],
    // volatile: retired archive paths
    [`${stateDir}/archived/old-session.jsonl`, true],
    ...
    [`${stateDir}/locks/task.lock`, true],
    [`${stateDir}/downloads/download.partial`, true],
    [`${stateDir}/cache/queue/item.wal`, true],
    [`${stateDir}/cache/queue/item-wal`, true],
    ...
    [`${stateDir}/agents/main/agent/openclaw-agent.sqlite`, false],
    [`${stateDir}/agents/main/agent/auth-profiles.json`, false],
    [`${stateDir}/openclaw.sqlite-wal`, false],
    [`${stateDir}/openclaw.sqlite-shm`, false],
    [`${stateDir}/openclaw.sqlite-journal`, false],
    [`${stateDir}/browser/openclaw/user-data/Default/Preferences`, false],
    [`${stateDir}/browser/openclaw/user-data/Default/Cookies`, false],
    [`${stateDir}/browser/openclaw/user-data/Default/Local Storage/leveldb/000003.log`, false],
    [`${stateDir}/extensions/example/Cargo.lock`, false],
    [`${stateDir}/extensions/example/download.partial`, false],
    [`${stateDir}/task.lock`, false],
    [`${stateDir}/download.partial`, false],
    [`${stateDir}/queue/item.wal`, false],
