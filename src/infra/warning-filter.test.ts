  ...
  it("installs once and suppresses known warnings at emit time", async () => {
    ...
    try {
      ...
      const emitWarning = (...args: unknown[]) =>
        (process.emitWarning as unknown as (...warningArgs: unknown[]) => void)(
          ...args,
        );
      ...
      expect(
        seenWarnings.find((warning) => warning.code === "DEP0060"),
      ).toBeUndefined();

      emitWarning("Visible warning", {
        type: "Warning",
      ...
      expect(
        seenWarnings.find(
          (warning) => warning.code === "OPENCLAW_TEST_WARNING",
        ),
      ).toBeDefined();
    } finally {
