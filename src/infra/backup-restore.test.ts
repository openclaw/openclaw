...
describe("backup restore", () => {
  ...
  async function rewriteArchiveManifest(
    archivePath: string,
    mutate: (manifest: Record<string, unknown>) => Record<string, unknown>,
  ): Promise<string> {
    const tar = await import("tar");
    const extractDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-restore-rewrite-"));
    await tar.x({ file: archivePath, cwd: extractDir, gzip: true });
    const [archiveRoot] = await fs.readdir(extractDir);
    const manifestPath = path.join(extractDir, archiveRoot, "manifest.json");
    const raw = await fs.readFile(manifestPath, "utf8");
    const mutated = mutate(JSON.parse(raw) as Record<string, unknown>);
    await fs.writeFile(manifestPath, JSON.stringify(mutated, null, 2), "utf8");

    const repackPath = path.join(
      path.dirname(archivePath),
      `rewritten-${path.basename(archivePath)}`,
    );
    await tar.c({ file: repackPath, cwd: extractDir, gzip: true, portable: true }, [archiveRoot]);
    await fs.rm(extractDir, { recursive: true, force: true });
    return repackPath;
  }

  it("rejects manifest where sourcePath does not match encoded archivePath", async () => {
    const stateDir = path.join(tempHome.home, ".openclaw");
    const archivePath = await createTestBackup(stateDir);
    await fs.rm(stateDir, { recursive: true, force: true });

    const tamperedArchive = await rewriteArchiveManifest(archivePath, (manifest) => {
      const assets = manifest.assets as Array<Record<string, unknown>>;
      // Rewrite sourcePath to a different absolute location, leaving archivePath alone.
      assets[0].sourcePath = "/tmp/evil-restore-target";
      return manifest;
    });

    try {
      await expect(planRestore({ archive: tamperedArchive })).rejects.toThrow(
        /sourcePath does not match archivePath/i,
      );
    } finally {
      await fs.rm(path.dirname(archivePath), { recursive: true, force: true });
    }
  ...
  it("rejects manifest with zero assets", async () => {
    const stateDir = path.join(tempHome.home, ".openclaw");
    const archivePath = await createTestBackup(stateDir);

    const emptyAssetsArchive = await rewriteArchiveManifest(archivePath, (manifest) => {
      manifest.assets = [];
      return manifest;
    });

    try {
      await expect(planRestore({ archive: emptyAssetsArchive })).rejects.toThrow(
        /no assets to restore/i,
      );
    } finally {
      await fs.rm(path.dirname(archivePath), { recursive: true, force: true });
    }
  ...
  it("rejects archive created on a different platform", async () => {
    const stateDir = path.join(tempHome.home, ".openclaw");
    const archivePath = await createTestBackup(stateDir);

    const otherPlatform = process.platform === "linux" ? "darwin" : "linux";
    const wrongPlatformArchive = await rewriteArchiveManifest(archivePath, (manifest) => {
      manifest.platform = otherPlatform;
      return manifest;
    });

    try {
      await expect(planRestore({ archive: wrongPlatformArchive })).rejects.toThrow(
        new RegExp(`created on ${otherPlatform}.*current platform is ${process.platform}`, "i"),
      );
    } finally {
      await fs.rm(path.dirname(archivePath), { recursive: true, force: true });
    }
  ...
  it("re-checks conflict at execution time (TOCTOU window)", async () => {
    const stateDir = path.join(tempHome.home, ".openclaw");
    const archivePath = await createTestBackup(stateDir);

    // Plan while state is missing (no conflict)
    await fs.rm(stateDir, { recursive: true, force: true });
    const plan = await planRestore({ archive: archivePath, force: true });
    expect(plan.assets.every((a) => !a.conflict)).toBe(true);

    // Race: another process recreates the target between plan and execute
    await fs.mkdir(stateDir, { recursive: true });
    await fs.writeFile(path.join(stateDir, "state.txt"), "raced-content\n", "utf8");

    // With force, restore should still take a snapshot of the raced-in state
    await executeRestore(plan, { archive: archivePath, force: true });
    ...
    // Pre-restore snapshot should contain the raced-in content, not the original
    const snapshotDir = path.join(path.dirname(stateDir), preRestoreEntries[0]);
    const snapshotState = await fs.readFile(path.join(snapshotDir, "state.txt"), "utf8");
    expect(snapshotState).toBe("raced-content\n");
