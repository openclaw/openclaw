...
import { resolveDNAPackageRoot } from "./dna-root.js";
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveDNAPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("dna update")}`,
      );
