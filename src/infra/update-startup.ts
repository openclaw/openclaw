...
import { resolveSmartAgentNeoPackageRoot } from "./smart-agent-neo-root.js";
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveSmartAgentNeoPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("smart-agent-neo update")}`,
      );
