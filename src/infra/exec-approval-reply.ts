...
export function buildExecApprovalUnavailableReplyPayload(
  ...
  if (params.reason === "initiating-platform-disabled") {
    ...
    lines.push(
      "Approve it from the Web UI or terminal UI, or enable a native chat approval client such as Discord, Slack, or Telegram. If those accounts already know your owner ID via allowFrom or owner config, Mullusi can often infer approvers automatically.",
    );
