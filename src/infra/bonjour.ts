...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ACTIVI_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Activi";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Activi\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.ACTIVI_MDNS_HOSTNAME?.trim() ||
    process.env.ACTIVI_MDNS_HOSTNAME?.trim() ||
    "activi";
  const hostname =
    hostnameRaw
