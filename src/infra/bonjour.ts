...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SMART_AGENT_NEO_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "SmartAgentNeo";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(SmartAgentNeo\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.SMART_AGENT_NEO_MDNS_HOSTNAME?.trim() ||
    process.env.NEOBOT_MDNS_HOSTNAME?.trim() ||
    "smart-agent-neo";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
      .trim() || "smart-agent-neo";
  const instanceName =
    typeof opts.instanceName === "string" && opts.instanceName.trim()
      ? opts.instanceName.trim()
      : `${hostname} (SmartAgentNeo)`;
