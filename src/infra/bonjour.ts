...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MULLUSI_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Mullusi";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Mullusi\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.MULLUSI_MDNS_HOSTNAME?.trim() || "mullusi";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
