...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.EASYHUB_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "EasyHub";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(EasyHub\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.EASYHUB_MDNS_HOSTNAME?.trim() ||
    process.env.EASYHUB_MDNS_HOSTNAME?.trim() ||
    "EasyHub";
  const hostname =
    hostnameRaw
