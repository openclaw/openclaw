...
export function ensureExecApprovals(): ExecApprovalsFile {
  ...
  // Preserve loaded defaults if the file has a defaults field (even if empty),
  // to avoid losing allow-always allowlist entries when normalizeExecApprovals
  // drops undefined fields. An empty defaults object {} means the file was saved
  // with defaults present, so we preserve it.
  const hasDefaults = loaded.defaults != null;
  const updated: ExecApprovalsFile = {
    ...next,
    defaults: hasDefaults ? loaded.defaults : next.defaults,
    socket: {
