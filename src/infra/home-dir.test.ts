...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ACTIVI_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands ACTIVI_HOME when set to ~", () => {
    const env = {
      ACTIVI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ACTIVI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ACTIVI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ACTIVI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ACTIVI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { ACTIVI_HOME: "/srv/activi-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/activi-home")}/x`);
  });
