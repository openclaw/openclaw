...
describe("resolveNpmIntegrityDrift", () => {
  ...
  it("warns by default when no callback is provided", async () => {
    ...
    expect(warn).toHaveBeenCalledWith({ spec: "@bot/test@1.0.0" });
  ...
  it("formats default warning and abort error messages", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "Integrity drift detected for @bot/test@1.0.0: expected sha512-old, got sha512-new",
    );
    ...
    expect(abortResult.error).toBe(
      "aborted: npm package integrity drift detected for @bot/test@1.0.0",
    );
  });
});
