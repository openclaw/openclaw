...
const GATEWAY_DEVICE_ID_HEADER = "x-mullusi-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-mullusi-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-mullusi-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.MULLUSI_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "MULLUSI_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or MULLUSI_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.MULLUSI_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require MULLUSI_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.MULLUSI_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
