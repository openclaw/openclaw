...
describe("exec safe bin policy knownLongFlags from config", () => {
  it("accepts known boolean long flags provided via fixture knownLongFlags", () => {
    ...
    const profile = expectDefined(profiles.mycli, "profiles.mycli test invariant");
  ...
  it("works identically to auto-derivation when knownLongFlags duplicates allow/deny", () => {
    ...
    expect(
      expectDefined(withExplicit.mycli, "withExplicit.mycli test invariant").knownLongFlagsSet,
    ).toEqual(
      expectDefined(withoutExplicit.mycli, "withoutExplicit.mycli test invariant").knownLongFlagsSet,
    );
  });
