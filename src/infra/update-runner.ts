...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "easyhub.mjs");
    ...
    const doctorStep = await runStep(
      step("EasyHub doctor", doctorArgv, gitRoot, { EASYHUB_UPDATE_IN_PROGRESS: "1" }),
    );
