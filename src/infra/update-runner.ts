...
const DEFAULT_PACKAGE_NAME = "activi";
...
function normalizeTag(tag?: string) {
  ...
  if (trimmed.startsWith("activi@")) {
    return trimmed.slice("activi@".length);
  }
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "activi-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "activi.mjs");
    ...
    const doctorStep = await runStep(
      step("activi doctor", doctorArgv, gitRoot, { ACTIVI_UPDATE_IN_PROGRESS: "1" }),
    );
