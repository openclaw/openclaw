...
const DEFAULT_PACKAGE_NAME = "smart-agent-neo";
...
function normalizeTag(tag?: string) {
  ...
  if (trimmed.startsWith("smart-agent-neo@")) {
    return trimmed.slice("smart-agent-neo@".length);
  }
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && !pkgRoot) {
    return {
      ...
      mode: "unknown",
      root: gitRoot,
      reason: "not-smart-agent-neo-root",
      steps: [],
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "smart-agent-neo.mjs");
    ...
    if (!doctorEntryExists) {
      steps.push({
        name: "smart-agent-neo doctor entry",
    ...
    const doctorStep = await runStep(
      step("smart-agent-neo doctor", doctorArgv, gitRoot, { SMART_AGENT_NEO_UPDATE_IN_PROGRESS: "1" }),
    );
