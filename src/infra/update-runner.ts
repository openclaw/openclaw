...
const DEFAULT_PACKAGE_NAME = "mullusi";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["mullusi", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "mullusi.mjs");
    ...
    const doctorStep = await runStep(
      step("mullusi doctor", doctorArgv, gitRoot, { MULLUSI_UPDATE_IN_PROGRESS: "1" }),
    );
