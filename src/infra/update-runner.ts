...
const DEFAULT_PACKAGE_NAME = "evox";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["evox", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "evox-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "evox.mjs");
    ...
    const doctorStep = await runStep(
      step("evox doctor", doctorArgv, gitRoot, { OPENCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
