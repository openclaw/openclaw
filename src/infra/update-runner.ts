...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (globalManager) {
    ...
    const installTarget = await resolveGlobalInstallTarget({
      ...
      timeoutMs,
      pkgRoot,
      honorPackageRoot: channel === "extended-stable",
      packageName,
    });
    ...
    const globalInstallEnv = await createGlobalInstallEnv(undefined, {
      registryUrl: extendedStable?.status === "resolved" ? extendedStable.registryUrl : undefined,
    });
    const spec =
      extendedStable?.status === "resolved"
