...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dna-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "dna doctor",
        managerScriptArgs(manager, "dna", ["doctor", "--non-interactive"]),
        gitRoot,
        { DNA_UPDATE_IN_PROGRESS: "1" },
      ),
    );
