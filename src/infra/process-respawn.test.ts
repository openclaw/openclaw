...
describe("respawnGatewayProcessForUpdate", () => {
  ...
  it("rewrites pnpm versioned entry path to stable wrapper in detached spawn", () => {
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      process.execPath,
      [
        expect.stringMatching(/node_modules\/openclaw\/openclaw\.mjs$/),
        "gateway",
        "run",
      ],
      expect.any(Object),
...
describe("rewritePnpmVersionedEntryPath", () => {
  ...
  it("returns non-pnpm paths unchanged", () => {
    expect(rewritePnpmVersionedEntryPath("/repo/src/entry.ts")).toBe(
      "/repo/src/entry.ts",
    );
  });
  ...
  it("returns paths without .pnpm marker unchanged", () => {
    ...
    ).toBe("/app/node_modules/openclaw/dist/entry.js");
  });
