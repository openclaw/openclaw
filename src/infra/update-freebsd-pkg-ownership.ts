...
export class FreeBsdPkgOwnershipError extends Error {
  constructor(
    ...
    const code =
      diagnostic && "cause" in diagnostic
        ? normalizeSupportDiagnosticErrorCode(extractErrorCode(diagnostic.cause))
        : undefined;
    ...
    super(
      ...
      diagnostic && "cause" in diagnostic ? { cause: diagnostic.cause } : undefined,
    );
...
export function createFreeBsdPkgOwnershipInspection(
  ...
  const readPath = <T>(operation: () => Promise<T>, label: "lstat" | "realpath") =>
    read(
      () => {
        if (++pathReads > 50_000) {
      ...
      () => {
        ...
        if (!canonical) {
          canonical = (async () => {
            ...
            return path.resolve(
              await readPath(() => fs.realpath(ancestor), "realpath"),
  ...
  const assertUnowned = async (lexicalRoot: string, entryOnly: boolean) => {
    ...
    const inventory = await read(() => (files ??= readPkgFiles(budget)), "database", "pkg query");
