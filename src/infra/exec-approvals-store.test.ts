...
describe("exec approvals store helpers", () => {
  ...
  it("respects OPENCLAW_STATE_DIR environment variable for file path", () => {
    const customStateDir = "/custom/state/dir";
    const env = { OPENCLAW_STATE_DIR: customStateDir } as NodeJS.ProcessEnv;

    expect(path.normalize(resolveExecApprovalsPath(env))).toBe(
      path.normalize(path.join(customStateDir, "exec-approvals.json")),
    );
  });

  it("respects OPENCLAW_STATE_DIR environment variable for socket path", () => {
    const customStateDir = "/custom/state/dir";
    const env = { OPENCLAW_STATE_DIR: customStateDir } as NodeJS.ProcessEnv;

    expect(path.normalize(resolveExecApprovalsSocketPath(env))).toBe(
      path.normalize(path.join(customStateDir, "exec-approvals.sock")),
    );
  });
