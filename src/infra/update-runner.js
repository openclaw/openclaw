...
import { collectInstalledGlobalPackageErrors, cleanupGlobalRenameDirs, createGlobalInstallEnv, detectGlobalInstallManagerForRoot, globalInstallArgs, globalInstallFallbackArgs, resolveExpectedInstalledVersionFromSpec, resolveGlobalInstallTarget, resolveGlobalInstallSpec, } from "./update-global.js";
import { managerInstallIgnoreScriptsArgs, managerInstallArgs, managerScriptArgs, resolveUpdateBuildManager, } from "./update-package-manager.js";
...
function isSupersededInstallFailure(step, steps) {
    ...
    if (step.name === "deps install") {
        return steps.some((candidate) => candidate.name === "deps install (ignore scripts)" && candidate.exitCode === 0);
    }
...
export async function runGatewayUpdate(opts = {}) {
    ...
    if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
        ...
        if (channel === "dev") {
            ...
            if (devTargetRef) {
                ...
                for (const targetRefCandidate of buildDevTargetRefResolutionCandidates(devTargetRef)) {
                    const targetShaStep = await runStep(step(`git rev-parse ${targetRefCandidate}`, ["git", "-C", gitRoot, "rev-parse", targetRefCandidate], gitRoot));
            ...
            const manager = await resolveUpdateBuildManager((argv, options) => runCommand(argv, { timeoutMs: options.timeoutMs, env: options.env }), gitRoot, timeoutMs, defaultCommandEnv, "require-preferred");
            ...
            const worktreeStep = await runStep(step("preflight worktree", ["git", "-C", gitRoot, "worktree", "add", "--detach", worktreeDir, preflightBaseSha], gitRoot));
            ...
            try {
                for (const sha of candidates) {
                    ...
                    const depsStepArgv = preflightIgnoreScripts && preflightIgnoreScriptsArgv
                    ...
                    const depsStep = await runStep(step(depsStepName, depsStepArgv, worktreeDir, manager.env));
                    steps.push(depsStep);
                ...
                if (removeStep.exitCode !== 0 &&
                    ...
                    removeStep.stderrTail = trimLogTail([removeStep.stderrTail, "windows fallback cleanup removed preflight tree"]
                        .filter(Boolean)
                        .join("\n"), MAX_LOG_CHARS);
                }
        ...
        try {
            const depsStep = await runStep(step("deps install", managerInstallArgs(manager.manager, {
                compatFallback: manager.fallback && manager.manager === "npm",
            }), gitRoot, manager.env));
            steps.push(depsStep);
            let finalDepsStep = depsStep;
    ...
    if (globalManager) {
        ...
        const failedStep = finalStep.exitCode !== 0
            ? finalStep
