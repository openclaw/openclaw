...
function resolveShellWrapperScriptArgv(params: {
  ...
  const scriptBase = targetPath.basename(params.shellScriptCandidatePath).toLowerCase();
  const cwdBase = params.cwd && params.cwd.trim() ? params.cwd.trim() : process.cwd();
  const resolveArgPath = (a: string): string =>
    targetPath.isAbsolute(a) ? a : targetPath.resolve(cwdBase, a);
  ...
  if (idx === -1) {
    idx = params.effectiveArgv.findIndex(
      (a) => targetPath.basename(a).toLowerCase() === scriptBase,
    );
  }
  ...
  const shellPositionalArgvMatch = shellPositionalArgvCandidatePath
    ? matchAllowlist(
        params.context.allowlist,
        {
          ...
          executableName: getPathModuleForPlatform(params.context.platform).basename(
...
function buildScriptArgPatternFromArgv(
  ...
  const scriptBase = targetPath.basename(scriptPath).toLowerCase();
  const base = cwd && cwd.trim() ? cwd.trim() : process.cwd();
  const resolveArgPath = (arg: string): string =>
    targetPath.isAbsolute(arg) ? arg : targetPath.resolve(base, arg);
  let scriptIdx = argv.findIndex((arg) => resolveArgPath(arg) === scriptPath);
  if (scriptIdx === -1) {
    scriptIdx = argv.findIndex((arg) => targetPath.basename(arg).toLowerCase() === scriptBase);
  }
