...
export type MemoryAssessmentResult = {
  ...
  source: "system" | "cgroupv2" | "cgroupv1" | "unlimited";
};
...
export function parseCgroupLimitBytes(raw: string): number | null {
  ...
  // Treat values >= 128PB as unlimited (practical upper bound)
  if (parsed >= 128n * 1024n * 1024n * 1024n * 1024n) {
    return null;
  }
...
export function assessGatewayStartupMemory(): MemoryAssessmentResult {
  ...
  // If cgroup reports unlimited, use system memory
  const actualEffectiveBytes = source === "unlimited" ? totalMemoryBytes : effectiveMemoryBytes;

  let status: MemoryAssessmentStatus;
  if (actualEffectiveBytes >= GATEWAY_RECOMMENDED_MEMORY_BYTES) {
    status = "ok";
  } else if (actualEffectiveBytes >= GATEWAY_MIN_MEMORY_BYTES) {
    status = "warn";
  } else {
    status = "error";
  }
  ...
  return {
    status,
    totalMemoryBytes,
    effectiveMemoryBytes: actualEffectiveBytes,
    source: source === "unlimited" ? "system" : source,
  };
}
