...
vi.mock("./install-source-utils.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("./install-source-utils.js")>();
  return {
    ...actual,
    withTempDir: vi.fn(
...
describe("installFromNpmSpecArchive", () => {
  ...
  it("returns pack errors without invoking installer", async () => {
    ...
    expect(withTempDir).toHaveBeenCalledWith(
      "openclaw-test-",
      expect.any(Function),
    );
  });
  ...
  it("returns resolution metadata and installer result on success", async () => {
    ...
    const installFromArchive = vi.fn(async () => ({
      ok: true as const,
      target: "done",
    }));
    ...
    const okResult = expectWrappedOkResult(result, {
      ok: true,
      target: "done",
    });
