 ...
 * returning control to the caller (typically `triggerMullusiRestart` →
...
/**
 * Parse mullusi gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("mullusi")) {
 ...
 * Returns only PIDs that belong to mullusi gateway processes (not the current process).
 */
