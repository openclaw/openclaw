...
import { NPM_UPDATE_COMPAT_SIDECAR_PATHS, NPM_UPDATE_OMITTED_BUNDLED_PLUGIN_ROOTS, } from "./npm-update-compat-sidecars.js";
import { collectPackageDistInventory, PACKAGE_DIST_INVENTORY_RELATIVE_PATH, readPackageDistInventoryIfPresent, } from "./package-dist-inventory.js";
...
export function isMainPackageTarget(value) {
    return normalizeLowercaseStringOrEmpty(normalizePackageTarget(value)) === "main";
}
export function isExplicitPackageInstallSpec(value) {
...
export function resolveExpectedInstalledVersionFromSpec(packageName, spec) {
...
async function collectCriticalInstalledPackageDistPaths(packageRoot) {
    ...
    await Promise.all(BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
        ...
        if ((await pathExists(path.join(packageRoot, pluginRoot, "package.json"))) ||
...
export function canResolveRegistryVersionForPackageTarget(value) {
    const trimmed = normalizePackageTarget(value);
    if (!trimmed) {
        return true;
    }
...
export async function createGlobalInstallEnv(env) {
    ...
    const requiresMergedEnv = pathPrepend.length > 0 || process.platform === "win32" || !hasCorepackDownloadPromptSetting;
    if (!requiresMergedEnv) {
        return env;
    }
    const merged = Object.fromEntries(Object.entries(sourceEnv)
...
export function resolveGlobalInstallCommand(manager, pkgRoot) {
    return {
        manager,
...
export async function resolveGlobalRoot(managerOrCommand, runCommand, timeoutMs, pkgRoot) {
...
export async function resolveGlobalPackageRoot(managerOrCommand, runCommand, timeoutMs, pkgRoot) {
...
export function globalInstallArgs(managerOrCommand, spec, pkgRoot) {
    const resolved = normalizeGlobalInstallCommand(managerOrCommand, pkgRoot);
...
export function globalInstallFallbackArgs(managerOrCommand, spec, pkgRoot) {
    const resolved = normalizeGlobalInstallCommand(managerOrCommand, pkgRoot);
