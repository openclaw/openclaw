  ...
  try {
    ...
    try {
      ...
      if (!params.signal) {
        ...
        throwIfAborted(params.signal);
        await pipeline(
          createReadStream(params.tempArchivePath),
          createWriteStream(params.outputPath, { flags: "wx" }),
...
async function writeArchiveWithSignal(params: {
  tempArchivePath: string;
  manifestPath: string;
  archiveRoot: string;
  assetPaths: string[];
...
export async function createBackupArchive(
  ...
  try {
    ...
    throwIfAborted(opts.signal);
    await publishTempArchive({ tempArchivePath, outputPath, signal: opts.signal });
  } finally {
