...
export const POSIX_EVOX_TMP_DIR = "/tmp/evox";
...
export function resolvePreferredOpenClawTmpDir(
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "evox" : `evox-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[evox] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback EVOX.sh temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback EVOX.sh temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_EVOX_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_EVOX_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_EVOX_TMP_DIR)) {
      return POSIX_EVOX_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_EVOX_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_EVOX_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_EVOX_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_EVOX_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_EVOX_TMP_DIR;
