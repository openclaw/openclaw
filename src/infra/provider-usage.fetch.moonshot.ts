...
async function fetchMoonshotUsageRaw(
  ...
  try {
    ...
    if (kimi.response.ok) {
      try {
        const body = (await kimi.response.clone().json()) as KimiGatewayUsageResponse;
        if (parseKimiGatewayWindows(body).length > 0) {
          logVerbose("[usage:kimi] usage resolved via kimi.com billing endpoint");
          return { ...kimi, errors };
        }
        logVerbose(
          "[usage:kimi] kimi.com billing endpoint returned 200 but no usable windows; trying fallbacks",
        );
      } catch {
        logVerbose(
          "[usage:kimi] kimi.com billing endpoint returned 200 but body could not be parsed; trying fallbacks",
        );
      }
    } else {
