...
const activeDeferralPolls = new Set();
...
export function clearGatewayRestartIntentSync(env = process.env) {
...
export function consumeGatewayRestartIntentSync(env = process.env, now = Date.now()) {
    const intentPath = resolveGatewayRestartIntentPath(env);
    let raw;
    try {
        const stat = fs.lstatSync(intentPath);
...
export function deferGatewayRestartUntilIdle(opts) {
    ...
    const maxWaitMs = typeof opts.maxWaitMs === "number" && Number.isFinite(opts.maxWaitMs) && opts.maxWaitMs > 0
