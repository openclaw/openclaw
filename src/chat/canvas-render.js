...
function coerceCanvasPreview(record) {
    ...
    const preferredHeight = normalizePreferredHeight(getRecordNumberField(presentation, "preferred_height") ??
    ...
    const className = getRecordStringField(presentation, "class_name") ??
...
function parseCanvasAttributes(raw) {
    const attrs = {};
...
function previewFromShortcode(attrs) {
    ...
    const preferredHeight = attrs.height && Number.isFinite(Number(attrs.height))
...
export function extractCanvasFromText(outputText, _toolName) {
    const parsed = tryParseJsonRecord(outputText);
    return coerceCanvasPreview(parsed);
}
export function extractCanvasShortcodes(text) {
