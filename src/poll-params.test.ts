...
describe("poll params", () => {
	...
	it.each([
		...
		{ key: "pollAnonymous" },
		{ key: "pollPublic" },
	])("treats $key=true as poll creation intent", ({ key }) => {
		expect(
			hasPollCreationParams({
				[key]: true,
			}),
	...
	it("treats non-zero finite numeric poll params as poll creation intent", () => {
		...
		expect(hasPollCreationParams({ pollDurationHours: Number.NaN })).toBe(
			false,
		);
		expect(hasPollCreationParams({ pollDurationSeconds: Infinity })).toBe(
			false,
		);
	...
	it("detects snake_case poll fields as poll creation intent", () => {
		...
		expect(hasPollCreationParams({ poll_option: ["Pizza", "Sushi"] })).toBe(
			true,
		);
	...
	it("resolves telegram poll visibility flags", () => {
		...
		expect(() =>
			resolveTelegramPollVisibility({ pollAnonymous: true, pollPublic: true }),
