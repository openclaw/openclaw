...
const createTempDir = () => tempDirs.make("mullusi-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets MULLUSI_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "mullusi",
    });

    expect(env.MULLUSI_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("mullusi");
  });

  it("overrides pre-existing MULLUSI_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      MULLUSI_SHELL: "wrong",
    });
    expect(env.MULLUSI_SHELL).toBe("acp-client");
  ...
  it("preserves MULLUSI_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        MULLUSI_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["MULLUSI_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Mullusi bridge", () => {
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "mullusi",
        serverArgs: ["acp"],
        defaultServerCommand: "mullusi",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("MULLUSI_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "mullusi", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "mullusi", "dist", "entry.js");
    const shimPath = path.join(dir, "mullusi.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\mullusi\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/mullusi-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/mullusi-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/mullusi-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
