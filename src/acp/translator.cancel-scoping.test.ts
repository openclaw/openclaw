...
function createHarness(
	...
	return {
		...
		sessionUpdateSpy: connection.sessionUpdate as unknown as ReturnType<
			typeof vi.fn
		>,
		sessionStore,
		sentRunIds,
...
async function cancelAndExpectAbortForPendingRun(
	...
	await expect(pending.promptPromise).resolves.toEqual({
...
describe("acp translator cancel and run scoping", () => {
	it("cancel passes active runId to chat.abort", async () => {
		...
		await cancelAndExpectAbortForPendingRun(
			harness,
			"session-1",
			sessionKey,
			pending,
	...
	it("cancel skips chat.abort when there is no active run and no pending prompt", async () => {
		...
		const abortCalls = harness.requestSpy.mock.calls.filter(
	...
	it("cancel from a session without active run does not abort another session sharing the same key", async () => {
		...
		expect(abortCalls).toHaveLength(0);
		expect(harness.sessionStore.getSession("session-2")?.activeRunId).toBe(
			pending2.runId,
		);
	...
	it("drops chat events when runId does not match the active prompt", async () => {
		...
		expect(harness.sessionStore.getSession("session-1")?.activeRunId).toBe(
			pending.runId,
		);
	...
	it("routes events to the pending prompt that matches runId when session keys are shared", async () => {
		...
		await expect(pending1.promptPromise).resolves.toEqual({
			stopReason: "end_turn",
		});
	});
});
