...
export function validateRuntimeThinkingInput(rawThinking) {
    return validateBoundedText({
        value: rawThinking,
...
export function parseRuntimeTimeoutSecondsInput(rawTimeout) {
...
export function resolveRuntimeOptionsFromMeta(meta) {
...
export function buildRuntimeControlSignature(options) {
...
export function buildRuntimeConfigOptionPairs(options) {
    const normalized = normalizeRuntimeOptions(options);
    const pairs = new Map();
    if (normalized.model) {
...
export function inferRuntimeOptionPatchFromConfigOption(key, value) {
