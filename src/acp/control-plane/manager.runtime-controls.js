...
import { buildRuntimeConfigOptionPairs, buildRuntimeControlSignature, normalizeText, resolveRuntimeOptionsFromMeta, } from "./runtime-options.js";
...
export async function applyManagerRuntimeControls(params) {
    ...
    const advertisedKeys = new Set((capabilities.configOptionKeys ?? [])
        .map((entry) => normalizeText(entry))
        .filter(Boolean));
    await withAcpRuntimeErrorBoundary({
        run: async () => {
            ...
            if (configOptions.length > 0) {
                if (!capabilities.controls.includes("session/set_config_option") ||
                ...
                for (const [key, value] of configOptions) {
                    if (advertisedKeys.size > 0 && !advertisedKeys.has(key)) {
                        throw new AcpRuntimeError("ACP_BACKEND_UNSUPPORTED_CONTROL", `ACP backend "${backend}" does not accept config key "${key}".`);
                    }
