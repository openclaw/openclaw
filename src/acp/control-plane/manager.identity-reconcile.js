...
import { createIdentityFromHandleEvent, createIdentityFromStatus, identityEquals, mergeSessionIdentity, resolveRuntimeHandleIdentifiersFromIdentity, resolveSessionIdentityFromMeta, } from "../runtime/session-identity.js";
...
export async function reconcileManagerRuntimeSessionIdentifiers(params) {
    ...
    if (!runtimeStatus && params.runtime.getStatus) {
        try {
            runtimeStatus = await withAcpRuntimeErrorBoundary({
                run: async () => await params.runtime.getStatus({
    ...
    const handleChanged = handleIdentifiers.backendSessionId !== params.handle.backendSessionId ||
    ...
    const metaChanged = !identityEquals(currentIdentity, nextIdentity) || hasLegacyAcpIdentityProjection(params.meta);
    if (!metaChanged) {
        return {
            handle: nextHandle,
            meta: params.meta,
