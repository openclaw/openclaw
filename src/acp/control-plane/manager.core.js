...
import { createIdentityFromEnsure, identityEquals, isSessionIdentityPending, mergeSessionIdentity, resolveRuntimeHandleIdentifiersFromIdentity, resolveSessionIdentityFromMeta, } from "../runtime/session-identity.js";
...
import { createUnsupportedControlError, hasLegacyAcpIdentityProjection, normalizeAcpErrorCode, normalizeActorKey, normalizeSessionKey, resolveAcpAgentFromSessionKey, resolveMissingMetaError, resolveRuntimeIdleTtlMs, } from "./manager.utils.js";
...
export class AcpSessionManager {
        ...
        await this.withSessionActor(sessionKey, async () => {
            ...
            try {
                ...
                for await (const event of runtime.runTurn({
                    ...
                    if (event.type === "error") {
                        streamError = new AcpRuntimeError(normalizeAcpErrorCode(event.code), event.message?.trim() || "ACP turn failed before completion.");
                    }
                    if (input.onEvent) {
    ...
    async withSessionActor(sessionKey, op) {
        const actorKey = normalizeActorKey(sessionKey);
