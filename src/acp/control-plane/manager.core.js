...
import { createRunningTaskRun, completeTaskRunByRunId, failTaskRunByRunId, startTaskRunByRunId, } from "../../tasks/detached-task-runtime.js";
import { AcpRuntimeError, toAcpRuntimeError, withAcpRuntimeErrorBoundary, } from "../runtime/errors.js";
import { createIdentityFromEnsure, identityHasStableSessionId, identityEquals, isSessionIdentityPending, mergeSessionIdentity, resolveRuntimeResumeSessionId, resolveRuntimeHandleIdentifiersFromIdentity, resolveSessionIdentityFromMeta, } from "../runtime/session-identity.js";
import { reconcileManagerRuntimeSessionIdentifiers } from "./manager.identity-reconcile.js";
import { applyManagerRuntimeControls, resolveManagerRuntimeCapabilities, } from "./manager.runtime-controls.js";
import { DEFAULT_DEPS, } from "./manager.types.js";
import { canonicalizeAcpSessionKey, createUnsupportedControlError, hasLegacyAcpIdentityProjection, normalizeAcpErrorCode, normalizeActorKey, requireReadySessionMeta, resolveAcpAgentFromSessionKey, resolveAcpSessionResolutionError, resolveMissingMetaError, resolveRuntimeIdleTtlMs, } from "./manager.utils.js";
import { RuntimeCache } from "./runtime-cache.js";
import { inferRuntimeOptionPatchFromConfigOption, mergeRuntimeOptions, normalizeRuntimeOptions, normalizeText, resolveRuntimeOptionsFromMeta, runtimeOptionsEqual, validateRuntimeConfigOptionInput, validateRuntimeModeInput, validateRuntimeOptionPatch, } from "./runtime-options.js";
...
function resolveBackgroundTaskTerminalResult(progressSummary) {
    ...
    if (/\bneed a writable session\b/i.test(normalized) ||
...
export class AcpSessionManager {
    ...
    actorTailBySession = this.actorQueue.getTailMapForTesting();
    ...
    errorCountsByCode = new Map();
    evictedRuntimeCount = 0;
    lastEvictedAt;
    ...
    async setSessionConfigOption(params) {
        ...
        return await this.withSessionActor(sessionKey, async () => {
            ...
            if (advertisedKeys.size > 0 && !advertisedKeys.has(key)) {
                throw new AcpRuntimeError("ACP_BACKEND_UNSUPPORTED_CONTROL", `ACP backend "${handle.backend || meta.backend}" does not accept config key "${key}".`);
            }
            await withAcpRuntimeErrorBoundary({
    ...
    async runTurn(input) {
        ...
        await this.withSessionActor(sessionKey, async () => {
            ...
            const taskContext = input.mode === "prompt"
            ...
            for (let attempt = 0; attempt < 2; attempt += 1) {
                ...
                try {
                    ...
                    const combinedSignal = input.signal && typeof AbortSignal.any === "function"
                    ...
                    const turnPromise = (async () => {
                        for await (const event of runtime.runTurn({
                            ...
                            if (event.type === "error") {
                                streamError = new AcpRuntimeError(normalizeAcpErrorCode(event.code), normalizeText(event.message) || "ACP turn failed before completion.");
                            }
    ...
    resolveTurnTimeoutMs(params) {
        ...
        if (typeof runtimeTimeoutSeconds === "number" &&
            ...
            if (outcome === timeoutToken) {
                void observedTurnPromise.then((lateOutcome) => {
                    if (lateOutcome.kind === "error") {
                        logVerbose(`acp-manager: detached late turn error after timeout for ${params.sessionKey}: ${String(lateOutcome.error)}`);
                    }
                });
                await params.onTimeout();
                throw new AcpRuntimeError("ACP_TURN_FAILED", `ACP turn timed out after ${Math.max(1, Math.round(params.timeoutLabelMs / 1_000))}s.`);
            }
            if (outcome.kind === "error") {
                throw outcome.error;
            }
        ...
        try {
            ...
            if (outcome === timeoutToken) {
                void observedCleanupPromise.then((lateOutcome) => {
                    if (lateOutcome.kind === "error") {
                        logVerbose(`acp-manager: detached timed-out turn ${params.label} cleanup failed for ${params.sessionKey}: ${String(lateOutcome.error)}`);
                    }
                });
            ...
            if (outcome.kind === "error") {
                logVerbose(`acp-manager: timed-out turn ${params.label} cleanup failed for ${params.sessionKey}: ${String(outcome.error)}`);
            }
            return true;
        }
        finally {
    ...
    async closeSession(input) {
        ...
        return await this.withSessionActor(sessionKey, async () => {
            ...
            const shouldSkipRuntimeClose = input.discardPersistentState &&
    ...
    async ensureRuntimeHandle(params) {
        const agent = normalizeText(params.meta.agent) || resolveAcpAgentFromSessionKey(params.sessionKey, "main");
        ...
        const persistedResumeSessionId = mode === "persistent" ? resolveRuntimeResumeSessionId(previousIdentity) : undefined;
        const shouldPrepareFreshPersistentSession = mode === "persistent" &&
        ...
        const shouldPersistMeta = previousMeta.backend !== nextMeta.backend ||
    ...
    enforceConcurrentSessionLimit(params) {
        ...
        if (activeCount >= limit) {
            throw new AcpRuntimeError("ACP_SESSION_INIT_FAILED", `ACP max concurrent sessions reached (${activeCount}/${limit}).`);
        }
    }
    ...
    isRecoverableMissingPersistentSessionError(message) {
        ...
        return (/persistent acp session .* could not be resumed/i.test(normalized) &&
