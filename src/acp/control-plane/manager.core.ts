...
import { type CachedRuntimeState, RuntimeCache } from "./runtime-cache.js";
import {
...
function appendBackgroundTaskProgressSummary(
	current: string,
	chunk: string,
): string {
...
function resolveBackgroundTaskFailureStatus(
...
function resolveBackgroundTaskTerminalResult(progressSummary: string): {
	...
	const normalized = normalizeText(progressSummary)
		?.replace(/\s+/g, " ")
		.trim();
	if (!normalized) {
		return {};
	...
	if (permissionDeniedMatch) {
		const path = permissionDeniedMatch.groups?.path
			?.trim()
			.replace(/[.,;:!?]+$/, "");
		return {
			terminalOutcome: "blocked",
			terminalSummary: path
...
export class AcpSessionManager {
	...
	getObservabilitySnapshot(
		...
		const completedTurns =
			this.turnLatencyStats.completed + this.turnLatencyStats.failed;
		...
		return {
			...
			errorsByCode: Object.fromEntries(
				[...this.errorCountsByCode.entries()].toSorted(([a], [b]) =>
					a.localeCompare(b),
				),
			),
		};
	...
	async reconcilePendingSessionIdentities(params: {
		...
		let acpSessions: Awaited<
			ReturnType<AcpSessionManagerDeps["listAcpSessions"]>
		>;
		try {
				...
				const becameResolved = await this.withSessionActor(
					...
					async () => {
						...
						return !isSessionIdentityPending(
							resolveSessionIdentityFromMeta(reconciled.meta),
						);
					},
				);
				if (becameResolved) {
	...
	async setSessionConfigOption(params: {
		...
		const normalizedOption = validateRuntimeConfigOptionInput(
			params.key,
			params.value,
		);
					...
					try {
						...
						if (taskContext) {
							const terminalResult =
								resolveBackgroundTaskTerminalResult(taskProgressSummary);
						...
						if (
							activeTurn &&
							this.activeTurnBySession.get(actorKey) === activeTurn
						) {
	...
	private resolveTurnTimeoutMs(params: {
		...
		const runtimeTimeoutSeconds = resolveRuntimeOptionsFromMeta(
			params.meta,
		).timeoutSeconds;
		if (
	...
	private async cleanupTimedOutTurn(params: {
		...
		void Promise.allSettled([
			params.activeTurn.cancelPromise,
			closePromise,
		]).then(() => {
			this.clearCachedRuntimeStateIfHandleMatches({
				sessionKey: params.sessionKey,
	...
	private async awaitCleanupWithGrace(params: {
		...
		const timeoutPromise = new Promise<typeof timeoutToken>((resolve) => {
			timer = setTimeout(
				...
				ACP_TURN_TIMEOUT_CLEANUP_GRACE_MS,
			);
			timer.unref?.();
		});
	...
	private async ensureRuntimeHandle(params: {
		...
		if (cached) {
			const backendMatches =
				!configuredBackend || cached.backend === configuredBackend;
		...
		const nextMeta: SessionAcpMeta = {
			...
			...(Object.keys(nextRuntimeOptions).length > 0
		...
		const shouldPersistMeta =
			...
			!runtimeOptionsEqual(
				previousMeta.runtimeOptions,
				nextMeta.runtimeOptions,
			) ||
	...
	private isRuntimeStatusUnavailable(
		...
		const detailsStatus =
			typeof status.details?.status === "string"
				? status.details.status.trim().toLowerCase()
				: "";
	...
	private recordTurnCompletion(params: {
		...
		this.turnLatencyStats.maxMs = Math.max(
			this.turnLatencyStats.maxMs,
			durationMs,
		);
		if (params.errorCode) {
	...
	private recordErrorCode(code: string): void {
		...
		this.errorCountsByCode.set(
			normalized,
			(this.errorCountsByCode.get(normalized) ?? 0) + 1,
		);
	}
	...
	private isRecoverableAcpxExitError(message: string): boolean {
		return /^acpx exited with (code \d+|signal [a-z0-9]+)/i.test(
			message.trim(),
		);
	}
	...
	private async evictIdleRuntimeHandles(params: {
		...
		for (const candidate of candidates) {
			await this.actorQueue.run(candidate.actorKey, async () => {
				...
				const lastTouchedAt = this.runtimeCache.getLastTouchedAt(
					candidate.actorKey,
				);
	...
	private async applyRuntimeControls(params: {
		...
		await applyManagerRuntimeControls({
			...
			getCachedRuntimeState: (sessionKey) =>
				this.getCachedRuntimeState(sessionKey),
		});
	}
	...
	private async reconcileRuntimeSessionIdentifiers(params: {
		...
		return await reconcileManagerRuntimeSessionIdentifiers({
			...
			writeSessionMeta: async (writeParams) =>
				await this.writeSessionMeta(writeParams),
		});
	}
	...
	private resolveBackgroundTaskContext(params: {
		...
		return {
			requesterSessionKey,
			requesterOrigin:
				parentEntry?.deliveryContext ?? childEntry?.deliveryContext,
	...
	private markBackgroundTaskRunning(
		...
		try {
			...
			logVerbose(
				`acp-manager: failed updating background task for ${runId}: ${String(error)}`,
			);
		}
	}
