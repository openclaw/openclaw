...
const OPTIONAL_THINKING_CONFIG_KEYS = new Set([
  "thinking",
  "effort",
  "reasoning_effort",
  "thought_level",
...
function isOptionalThinkingConfigKey(key: string): boolean {
  return OPTIONAL_THINKING_CONFIG_KEYS.has(normalizeLowercaseStringOrEmpty(key));
}
...
function isUnsupportedOptionalThinkingConfigRejection(key: string, error: unknown): boolean {
  if (!isOptionalThinkingConfigKey(key)) {
    return false;
  }
  const errorCode = error && typeof error === "object" ? (error as { code?: unknown }).code : null;
  if (errorCode === "ACP_BACKEND_UNSUPPORTED_CONTROL") {
      ...
      if (configOptions.length > 0) {
        ...
        for (const [key, value] of configOptions) {
          ...
          try {
            ...
            if (isUnsupportedOptionalThinkingConfigRejection(key, error)) {
              continue;
            }
            throw error;
          }
