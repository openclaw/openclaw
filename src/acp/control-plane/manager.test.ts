...
vi.mock("../runtime/session-meta.js", () => ({
	listAcpSessionEntries: (params: unknown) =>
		hoisted.listAcpSessionEntriesMock(params),
	readAcpSessionEntry: (params: unknown) =>
		hoisted.readAcpSessionEntryMock(params),
	upsertAcpSessionMeta: (params: unknown) =>
		hoisted.upsertAcpSessionMetaMock(params),
}));

vi.mock("../runtime/registry.js", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("../runtime/registry.js")>();
	return {
		...actual,
...
async function withAcpManagerTaskStateDir(
...
function extractRuntimeOptionsFromUpserts(): Array<
...
describe("AcpSessionManager", () => {
	...
	it("serializes concurrent turns for the same ACP session", async () => {
		...
		let inFlight = 0;
		let maxInFlight = 0;
		runtimeState.runTurn.mockImplementation(async function* (_input: {
			requestId: string;
		}) {
			inFlight += 1;
	...
	it("rejects a queued turn promptly when its caller aborts before the actor is free", async () => {
		...
		await first;
		await vi.waitFor(() => {
			expect(manager.getObservabilitySnapshot(baseCfg).turns.queueDepth).toBe(
				0,
			);
		});
	...
	it("keeps timed-out runtime handles counted until timeout cleanup finishes", async () => {
		...
		try {
			...
			expect(
				manager.getObservabilitySnapshot(cfg).runtimeCache.activeSessions,
			).toBe(1);
	...
	it("passes persisted ACP backend session identity back into ensureSession for configured bindings after restart", async () => {
		...
		hoisted.readAcpSessionEntryMock.mockImplementation(
			(paramsUnknown: unknown) => {
				const key =
					(paramsUnknown as { sessionKey?: string }).sessionKey ?? sessionKey;
				return {
					sessionKey: key,
					storeSessionKey: key,
					acp: {
	...
	it("falls back to a fresh ensure when reopening a persisted ACP backend session id fails", async () => {
		...
		const sessionKey =
			"agent:codex:acp:binding:demo-binding:default:retry-fresh";
	...
	it("drops cached runtime handles when close sees a stale acpx process-exit error", async () => {
		const runtimeState = createRuntime();
		runtimeState.close.mockRejectedValueOnce(
			new Error("acpx exited with code 1"),
		);
		hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
			id: "acpx",
			runtime: runtimeState.runtime,
	...
	it("rolls back ensured runtime sessions when metadata persistence fails", async () => {
		...
		hoisted.upsertAcpSessionMetaMock.mockRejectedValueOnce(
			new Error("disk full"),
		);
	...
	it("persists runtime mode changes through setSessionRuntimeMode", async () => {
		...
		expect(
			extractRuntimeOptionsFromUpserts().some(
				(entry) => entry?.runtimeMode === "plan",
			),
		).toBe(true);
	});
	...
	it("reconciles pending ACP identities during startup scan", async () => {
		...
		const manager = new AcpSessionManager();
		const result = await manager.reconcilePendingSessionIdentities({
			cfg: baseCfg,
		});
