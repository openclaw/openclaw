...
describe("AcpSessionManager", () => {
  ...
  it.each([
    ...
    [
      "timed-out replay drain",
      "Persistent ACP session acpx-sid-stale could not be resumed: Timed out waiting for session replay drain after 5000ms",
    ],
  ])(
    "retries once with a fresh persistent session after an early %s turn failure",
    async (_label, resumeErrorMessage) => {
      const runtimeState = createRuntime();
      hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
        id: "acpx",
        runtime: runtimeState.runtime,
  ...
  it("clears stale persistent resume state without retrying after replay output was observed", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
    });
    const sessionKey = "agent:claude:acp:binding:discord:default:retry-replay-output";
    let currentMeta: SessionAcpMeta = {
      ...readySessionMeta({
        agent: "claude",
      }),
    ...
    runtimeState.runTurn.mockImplementationOnce(async function* () {
      yield { type: "text_delta" as const, text: "replayed output", stream: "content" as const };
      yield {
        type: "error" as const,
        code: "NO_SESSION",
        message:
