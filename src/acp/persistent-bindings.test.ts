...
function parseFeishuConversationIdForTest(params: {
	...
	const topicSenderMatch = /^(.+):topic:([^:]+):sender:([^:]+)$/.exec(
		conversationId,
	);
	if (topicSenderMatch) {
	...
	matchInboundConversation: ({
		...
		if (
			...
			(incoming.scope !== "group_topic" &&
				...
				!isSupportedFeishuDirectConversationId(
		...
		const matchesParentTopicForSenderScopedConversation =
			...
			compiledBinding.conversationId ===
				`${incoming.chatId}:topic:${incoming.topicId}`;
		if (
...
function createDiscordPersistentSpec(
	overrides: Partial<BindingSpec> = {},
): BindingSpec {
	return {
		channel: "discord",
		accountId: defaultDiscordAccountId,
...
beforeEach(async () => {
	...
	persistentBindingsResolveModule = await import(
		"./persistent-bindings.resolve.js"
	);
	...
	persistentBindings = {
		...
		ensureConfiguredAcpBindingSession:
			lifecycleBindingsModule.ensureConfiguredAcpBindingSession,
...
describe("resolveConfiguredAcpBindingRecord", () => {
	...
	it("falls back to parent discord channel when conversation is a thread id", () => {
		...
		expect(resolved?.record.conversation.conversationId).toBe(
			"channel-parent-1",
		);
	});
	...
	it("resolves telegram forum topic bindings using canonical conversation ids", () => {
		...
		expect(splitIds?.record.targetSessionKey).toBe(
			canonical?.record.targetSessionKey,
		);
	});
	...
	it("resolves Feishu DM bindings using direct peer ids", () => {
		...
		expect(resolved?.record.targetSessionKey).toContain(
			"agent:codex:acp:binding:feishu:default:",
		);
	});
	...
	it("resolves Feishu topic bindings with parent chat ids", () => {
		...
		expect(resolved?.spec.agentId).toBe("claude");
		expect(resolved?.record.conversation.parentConversationId).toBe(
			"oc_group_chat",
		);
	});
	...
	it("derives configured binding cwd from an explicit agent workspace", () => {
		const cfg = createCfgWithBindings(
			...
			],
			{
				agents: {
					list: [
						{ id: "codex", workspace: "/workspace/openclaw" },
						{ id: "claude" },
					],
				},
			},
...
describe("resetAcpSessionInPlace", () => {
	...
	it("preserves harness agent ids during in-place reset even when not in agents.list", async () => {
		...
		const sessionKey =
			"agent:coding:acp:binding:discord:default:9373ab192b2317f4";
		sessionMetaMocks.readAcpSessionEntry.mockReturnValue({
			acp: {
				agent: "codex",
				mode: "persistent",
