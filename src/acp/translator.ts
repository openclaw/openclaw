...
function buildSessionMetadata(params: {
	...
	const updatedAt =
		...
		Number.isFinite(params.row.updatedAt)
...
export class AcpGatewayAgent implements Agent {
	...
	async loadSession(params: LoadSessionRequest): Promise<LoadSessionResponse> {
		...
		const [sessionSnapshot, transcript] = await Promise.all([
			...
			this.getSessionTranscript(session.sessionKey).catch((err) => {
				this.log(
					`session transcript fallback for ${session.sessionKey}: ${String(err)}`,
				);
				return [];
			}),
		]);
	...
	async setSessionConfigOption(
		...
		const sessionPatch = this.resolveSessionConfigPatch(
			params.configId,
			params.value,
		);
	...
	async prompt(params: PromptRequest): Promise<PromptResponse> {
		...
		const systemInputProvenance =
			provenanceMode === "off"
				? undefined
				: buildSystemInputProvenance(params.sessionId);
		...
		if (phase === "update") {
			...
			await this.connection.sessionUpdate({
				...
				update: {
					...
					locations: extractToolCallLocations(
						toolState?.locations,
						partialResult,
					),
				},
	...
	private async handleChatEvent(evt: EventFrame): Promise<void> {
		...
		const shouldHandleMessageSnapshot =
			messageData && (state === "delta" || state === "final");
	...
	private async finishPrompt(
		...
		try {
			...
			this.log(
				`session snapshot update failed for ${sessionId}: ${String(err)}`,
			);
		}
	...
	private async getSessionSnapshot(
		...
		try {
			...
			this.log(
				`session presentation fallback for ${sessionKey}: ${String(err)}`,
			);
			return {
		...
		switch (configId) {
			...
			case ACP_RESPONSE_USAGE_CONFIG_ID:
				return {
					...
					overrides: {
						responseUsage:
							value as GatewaySessionPresentationRow["responseUsage"],
					},
				};
			...
			default:
				throw new Error(
					`ACP bridge mode does not support session config option "${configId}".`,
				);
		}
	}
	...
	private async getSessionTranscript(
		...
		const result = await this.gateway.request<{ messages?: unknown[] }>(
			"sessions.get",
			{
				key: sessionKey,
	...
	private assertSupportedSessionSetup(
