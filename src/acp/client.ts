...
import { ensureDNACliOnPath } from "../infra/path-env.js";
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  ensureDNACliOnPath({ cwd });
  const serverCommand = opts.serverCommand ?? "dna";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "dna-acp-client", version: "1.0.0" },
  });
