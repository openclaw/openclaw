...
import { ensureEasyHubCliOnPath } from "../infra/path-env.js";
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  ensureEasyHubCliOnPath({ cwd });
  const serverCommand = opts.serverCommand ?? "EasyHub";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "EasyHub-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("EasyHub ACP client");
