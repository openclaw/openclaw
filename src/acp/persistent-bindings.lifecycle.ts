...
function sessionMatchesConfiguredBinding(params: {
	...
	const desiredAgent = (params.spec.acpAgentId ?? params.spec.agentId)
		.trim()
		.toLowerCase();
	...
	const desiredBackend =
		params.spec.backend?.trim() || params.cfg.acp?.backend?.trim() || "";
	if (desiredBackend) {
	...
	if (desiredCwd !== undefined) {
		const currentCwd = (
			params.meta.runtimeOptions?.cwd ??
			params.meta.cwd ??
			""
		).trim();
...
export async function resetAcpSessionInPlace(params: {
	...
	try {
		...
		await acpManager.initializeSession({
			...
			cwd,
			backendId:
				normalizeText(meta.backend) ?? normalizeText(params.cfg.acp?.backend),
		});
		...
		const message = error instanceof Error ? error.message : String(error);
		logVerbose(
			`acp-configured-binding: failed reset for ${sessionKey}: ${message}`,
		);
		return {
			ok: false,
			error: message,
