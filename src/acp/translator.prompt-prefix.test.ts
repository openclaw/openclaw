...
describe("acp prompt cwd prefix", () => {
  async function runPromptWithCwd(cwd: string) {
    ...
    const previousActiviHome = process.env.ACTIVI_HOME;
    ...
    try {
      ...
      if (previousActiviHome === undefined) {
        delete process.env.ACTIVI_HOME;
      } else {
        process.env.ACTIVI_HOME = previousActiviHome;
      }
  ...
  it("redacts home directory in prompt prefix", async () => {
    const requestSpy = await runPromptWithCwd(path.join(os.homedir(), "activi-test"));
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringMatching(/\[Working directory: ~[\\/]activi-test\]/),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("keeps backslash separators when cwd uses them", async () => {
    const requestSpy = await runPromptWithCwd(`${os.homedir()}\\activi-test`);
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringContaining("[Working directory: ~\\activi-test]"),
      }),
      { expectFinal: true },
    );
  });
