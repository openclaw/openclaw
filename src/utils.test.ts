...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-agent-neo-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "smart-agent-neo-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "smart-agent-neo-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.smart-agent-neo when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers SMART_AGENT_NEO_HOME over HOME", () => {
    vi.stubEnv("SMART_AGENT_NEO_HOME", "/srv/smart-agent-neo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/smart-agent-neo-home"));
...
describe("shortenHomePath", () => {
  it("uses $SMART_AGENT_NEO_HOME prefix when SMART_AGENT_NEO_HOME is set", () => {
    vi.stubEnv("SMART_AGENT_NEO_HOME", "/srv/smart-agent-neo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/smart-agent-neo-home")}/.smart-agent-neo/smart-agent-neo.json`)).toBe(
      "$SMART_AGENT_NEO_HOME/.smart-agent-neo/smart-agent-neo.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $SMART_AGENT_NEO_HOME replacement when SMART_AGENT_NEO_HOME is set", () => {
    vi.stubEnv("SMART_AGENT_NEO_HOME", "/srv/smart-agent-neo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/smart-agent-neo-home")}/.smart-agent-neo/smart-agent-neo.json`),
    ).toBe("config: $SMART_AGENT_NEO_HOME/.smart-agent-neo/smart-agent-neo.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/smart-agent-neo")).toBe(path.resolve(os.homedir(), "smart-agent-neo"));
  });
  ...
  it("prefers SMART_AGENT_NEO_HOME for tilde expansion", () => {
    vi.stubEnv("SMART_AGENT_NEO_HOME", "/srv/smart-agent-neo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/smart-agent-neo")).toBe(path.resolve("/srv/smart-agent-neo-home", "smart-agent-neo"));
