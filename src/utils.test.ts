...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "EasyHub-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "EasyHub-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "EasyHub-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "EasyHub-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.EasyHub when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "EasyHub-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers EASYHUB_HOME over HOME", () => {
    vi.stubEnv("EASYHUB_HOME", "/srv/EasyHub-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/EasyHub-home"));
...
describe("shortenHomePath", () => {
  it("uses $EASYHUB_HOME prefix when EASYHUB_HOME is set", () => {
    vi.stubEnv("EASYHUB_HOME", "/srv/EasyHub-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/EasyHub-home")}/.easyhub/easyhub.json`)).toBe(
      "$EASYHUB_HOME/.easyhub/easyhub.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $EASYHUB_HOME replacement when EASYHUB_HOME is set", () => {
    vi.stubEnv("EASYHUB_HOME", "/srv/EasyHub-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/EasyHub-home")}/.easyhub/easyhub.json`),
    ).toBe("config: $EASYHUB_HOME/.easyhub/easyhub.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/EasyHub")).toBe(path.resolve(os.homedir(), "EasyHub"));
  });
  ...
  it("prefers EASYHUB_HOME for tilde expansion", () => {
    vi.stubEnv("EASYHUB_HOME", "/srv/EasyHub-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/EasyHub")).toBe(path.resolve("/srv/EasyHub-home", "EasyHub"));
