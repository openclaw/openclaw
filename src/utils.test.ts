...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("activi-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("activi-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("activi-lid-a-", (first) => {
      withTempDirSync("activi-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.activi when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "activi-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers ACTIVI_HOME over HOME", () => {
    vi.stubEnv("ACTIVI_HOME", "/srv/activi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/activi-home"));
...
describe("shortenHomePath", () => {
  it("uses $ACTIVI_HOME prefix when ACTIVI_HOME is set", () => {
    vi.stubEnv("ACTIVI_HOME", "/srv/activi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/activi-home")}/.activi/activi.json`)).toBe(
      "$ACTIVI_HOME/.activi/activi.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ACTIVI_HOME replacement when ACTIVI_HOME is set", () => {
    vi.stubEnv("ACTIVI_HOME", "/srv/activi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/activi-home")}/.activi/activi.json`),
    ).toBe("config: $ACTIVI_HOME/.activi/activi.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/activi")).toBe(path.resolve(os.homedir(), "activi"));
  });
  ...
  it("prefers ACTIVI_HOME for tilde expansion", () => {
    vi.stubEnv("ACTIVI_HOME", "/srv/activi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/activi")).toBe(path.resolve("/srv/activi-home", "activi"));
