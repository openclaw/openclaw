...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.EASYHUB_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.EASYHUB_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.EASYHUB_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[EasyHub] ${parsed.error}`);
    process.exit(2);
  }
