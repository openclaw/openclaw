...
function resolveSystemdServiceName(env: Record<string, string | undefined>): string {
  const override = env.EASYHUB_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.EASYHUB_PROFILE);
}
...
export async function installSystemdService({
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: env.EASYHUB_PROFILE,
      version: environment?.EASYHUB_SERVICE_VERSION ?? env.EASYHUB_SERVICE_VERSION,
    });
  ...
  const serviceName = resolveGatewaySystemdServiceName(env.EASYHUB_PROFILE);
