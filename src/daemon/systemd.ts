...
function resolveSystemdServiceName(env: Record<string, string | undefined>): string {
  const override = env.DNA_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.DNA_PROFILE);
}
...
export async function installSystemdService({
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: env.DNA_PROFILE,
      version: environment?.DNA_SERVICE_VERSION ?? env.DNA_SERVICE_VERSION,
    });
  ...
  const serviceName = resolveGatewaySystemdServiceName(env.DNA_PROFILE);
