...
export async function auditScheduledTaskDefinition(
  ...
  if (expectedCommand) {
    ...
    if (
      ...
      const taskSupervisor = command?.environment?.OPENCLAW_SERVICE_KIND === "gateway";
      // v2026.9.6 wrote a waiting direct-path wrapper; only exact generated bytes migrate.
      const releasedWaiting = [
        'Set shell = CreateObject("WScript.Shell")',
        ...(taskSupervisor
          ? ['shell.Environment("Process")("OPENCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER") = "wscript"']
          : []),
        `WScript.Quit shell.Run("""${sourcePath.replaceAll('"', '""')}""", 0, True)`,
      ].join("\n");
      const generated = buildHiddenLauncherScript({ scriptPath: sourcePath, taskSupervisor });
