...
import { LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES, resolveGatewayServiceDescription, resolveGatewaySystemdServiceName, } from "./constants.js";
import { execFileUtf8 } from "./exec-file.js";
import { formatLine, toPosixPath, writeFormattedLines } from "./output.js";
import { resolveHomeDir } from "./paths.js";
import { parseKeyValueOutput } from "./runtime-parse.js";
import { enableSystemdUserLinger, readSystemdUserLingerStatus, } from "./systemd-linger.js";
import { classifySystemdUnavailableDetail, isSystemctlMissingDetail, isSystemdUserBusUnavailableDetail, } from "./systemd-unavailable.js";
...
function isGenericSystemctlIsEnabledFailure(detail) {
    ...
    const normalized = normalizeLowercaseStringOrEmpty(detail);
    return (normalized.startsWith("command failed: systemctl") &&
...
export function isNonFatalSystemdInstallProbeError(error) {
...
export async function isSystemdUserServiceAvailable(env = process.env) {
...
async function writeSystemdGatewayEnvironmentFile(params) {
    ...
    for (const [key, value] of entries) {
        if (/[\r\n]/.test(value)) {
            throw new Error(`state-dir .env contains a multiline value for ${key}; systemd EnvironmentFile values must be single-line`);
        }
    }
...
export async function stageSystemdService({ stdout, ...args }) {
...
export async function installSystemdService(args) {
    const { unitPath, backedUp } = await writeSystemdUnit(args);
