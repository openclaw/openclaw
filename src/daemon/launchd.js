...
import { GATEWAY_LAUNCH_AGENT_LABEL, resolveGatewayServiceDescription, resolveGatewayLaunchAgentLabel, resolveLegacyGatewayLaunchAgentLabels, } from "./constants.js";
...
import { isCurrentProcessLaunchdServiceLabel, scheduleDetachedLaunchdRestartHandoff, } from "./launchd-restart-handoff.js";
...
function isUnsupportedGuiDomain(detail) {
    const normalized = normalizeLowercaseStringOrEmpty(detail);
    return (normalized.includes("domain does not support specified action") ||
...
async function bootoutLaunchAgentOrThrow(params) {
    ...
    if (bootout.code !== 0 && !isLaunchctlNotLoaded(bootout)) {
        throw new Error(`${params.warning}; launchctl bootout failed: ${formatLaunchctlResultDetail(bootout)}`);
    }
...
async function probeLaunchAgentState(serviceTarget) {
    ...
    if (normalizeLowercaseStringOrEmpty(runtime.state) === "running" ||
...
export async function stopLaunchAgent({ stdout, env }) {
    ...
    if (stopState.state !== "stopped" && stopState.state !== "not-loaded") {
        const warning = stopState.state === "unknown"
...
export async function stageLaunchAgent({ stdout, ...args }) {
