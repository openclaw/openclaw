...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.SMART_AGENT_NEO_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.SMART_AGENT_NEO_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.SMART_AGENT_NEO_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.SMART_AGENT_NEO_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
