...
function resolveTaskName(env: GatewayServiceEnv): string {
  const override = env.ACTIVI_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.ACTIVI_PROFILE);
}

export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = env.ACTIVI_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.ACTIVI_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
