...
export function buildServiceEnvironment(params: {
  ...
  return {
    ...
    MULLUSI_PROFILE: profile,
    MULLUSI_GATEWAY_PORT: String(port),
    MULLUSI_LAUNCHD_LABEL: resolvedLaunchdLabel,
    MULLUSI_SYSTEMD_UNIT: systemdUnit,
    MULLUSI_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    MULLUSI_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    MULLUSI_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    MULLUSI_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = env.MULLUSI_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    MULLUSI_GATEWAY_TOKEN: gatewayToken,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    MULLUSI_STATE_DIR: sharedEnv.stateDir,
    MULLUSI_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.MULLUSI_STATE_DIR;
  const configPath = env.MULLUSI_CONFIG_PATH;
