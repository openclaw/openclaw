...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.SMART_AGENT_NEO_STATE_DIR;
  const configPath = env.SMART_AGENT_NEO_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    SMART_AGENT_NEO_PROFILE: profile,
    SMART_AGENT_NEO_STATE_DIR: stateDir,
    SMART_AGENT_NEO_CONFIG_PATH: configPath,
    SMART_AGENT_NEO_GATEWAY_PORT: String(port),
    SMART_AGENT_NEO_GATEWAY_TOKEN: token,
    SMART_AGENT_NEO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    SMART_AGENT_NEO_SYSTEMD_UNIT: systemdUnit,
    SMART_AGENT_NEO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    SMART_AGENT_NEO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    SMART_AGENT_NEO_SERVICE_VERSION: VERSION,
  };
}
