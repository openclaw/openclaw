...
export function resolveLaunchdRestartTarget(env = process.env) {
...
export function isCurrentProcessLaunchdServiceLabel(label, env = process.env) {
    const launchdLabel = normalizeOptionalString(env.LAUNCH_JOB_LABEL) ||
...
export function scheduleDetachedLaunchdRestartHandoff(params) {
    ...
    const waitForPid = typeof params.waitForPid === "number" && Number.isFinite(params.waitForPid)
