...
`,
  ...
  const result = execFileSync(
    ...
    ],
    {
      encoding: "utf8",
      timeout: 10_000,
      env: { PATH: bin, TMPDIR: root, TEST_QUERY_MARKER: path.join(root, "queried") },
    },
  );
...
describe.skipIf(process.platform !== "darwin")("detached launchd native fallback", () => {
  ...
  it("does not coerce non-string labels or trim label bytes", () => {
    expect(runFallback(plist("<key>Label</key><integer>42</integer>"))).toBe("");
