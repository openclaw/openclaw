...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "smart-agent-neo" | "neobot" | "neobot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["smart-agent-neo", "neobot", "neobot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["smart-agent-neo_service_marker"];
  const kindKeys = ["smart-agent-neo_service_kind"];
...
function isSmartAgentNeoGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.smartagentneo.");
}

function isSmartAgentNeoGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("smart-agent-neo-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isSmartAgentNeoGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("smart-agent-neo gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("neobot") || lower.includes("neobot");
}
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "neobot" : "neobot",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "smart-agent-neo" && isSmartAgentNeoGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "smart-agent-neo" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "smart-agent-neo" && isSmartAgentNeoGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isSmartAgentNeoGatewayTaskName(name)) {
        continue;
      }
