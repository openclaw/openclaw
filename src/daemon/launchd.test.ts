...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.smartagentneo.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", SMART_AGENT_NEO_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    const env: Record<string, string | undefined> = {
      HOME: "/Users/test",
      SMART_AGENT_NEO_PROFILE: "default",
    };
    const repair = await repairLaunchAgentBootstrap({ env });
    expect(repair.ok).toBe(true);

    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const label = "ai.smartagentneo.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when SMART_AGENT_NEO_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.smartagentneo.gateway.plist",
    );
  });

  it("uses profile-specific label when SMART_AGENT_NEO_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", SMART_AGENT_NEO_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.smartagentneo.jbphoenix.plist",
    );
  });

  it("prefers SMART_AGENT_NEO_LAUNCHD_LABEL over SMART_AGENT_NEO_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      SMART_AGENT_NEO_PROFILE: "jbphoenix",
      SMART_AGENT_NEO_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from SMART_AGENT_NEO_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      SMART_AGENT_NEO_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty SMART_AGENT_NEO_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      SMART_AGENT_NEO_PROFILE: "myprofile",
      SMART_AGENT_NEO_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.smartagentneo.myprofile.plist",
    );
  });
});
