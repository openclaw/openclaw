...
function shouldFallbackToStartupEntry(params) {
    return (/access is denied/i.test(params.detail) ||
        params.code === 124 ||
...
export function deriveScheduledTaskRuntimeStatus(parsed) {
...
async function resolveScheduledTaskPort(env) {
    ...
    return (parsePortFromProgramArguments(command?.programArguments) ??
...
async function resolveScheduledTaskGatewayListenerPids(port) {
    ...
    const matchedGatewayPids = Array.from(new Set(diagnostics.listeners
        .filter((listener) => typeof listener.pid === "number" &&
    ...
    return Array.from(new Set(diagnostics.listeners
        .map((listener) => listener.pid)
...
async function waitForGatewayPortRelease(port, timeoutMs = 5_000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const diagnostics = await inspectPortUsage(port).catch(() => null);
...
async function terminateBusyPortListeners(port) {
    ...
    const pids = Array.from(new Set(diagnostics.listeners
        .map((listener) => listener.pid)
        .filter((pid) => typeof pid === "number" && Number.isFinite(pid) && pid > 0)));
    for (const pid of pids) {
        await terminateGatewayProcessTree(pid, 300);
...
async function resolveFallbackRuntime(env) {
    ...
    return {
        ...
        detail: diagnostics.status === "busy"
...
export async function stageScheduledTask({ stdout, ...args }) {
...
async function shouldFallbackScheduledTaskLaunch(params) {
    ...
    const hasLaunchEvidence = async () => {
        ...
        const matchingTaskScriptProcess = entries.some((entry) => normalizeLowercaseStringOrEmpty(entry.CommandLine ?? "")
            .replaceAll("/", "\\")
...
async function runScheduledTaskOrThrow(params) {
    ...
    if (!(await shouldFallbackScheduledTaskLaunch({ env: params.env, scriptPath: params.scriptPath }))) {
        return;
    }
...
export async function installScheduledTask(args) {
