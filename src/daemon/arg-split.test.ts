...
describe("splitArgsPreservingQuotes", () => {
  ...
  it("groups single-quoted tokens in backslash mode", () => {
    expect(
      splitArgsPreservingQuotes("/usr/bin/env 'FOO=bar baz' /usr/bin/openclaw", {
        escapeMode: "backslash",
      }),
    ).toEqual(["/usr/bin/env", "FOO=bar baz", "/usr/bin/openclaw"]);
  });
});
