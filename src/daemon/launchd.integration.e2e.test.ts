...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `mullusi-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      MULLUSI_LAUNCHD_LABEL: `ai.mullusi.launchd-int-${testId}`,
      MULLUSI_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
