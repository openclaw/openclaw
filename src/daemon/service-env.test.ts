...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/.openclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/.openclaw-rescue"));
  });
  ...
  it("uses OPENCLAW_HOME for the default state dir", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/srv/openclaw-home/.openclaw"));
  });

  it("expands ~/ in OPENCLAW_HOME before building the default state dir", () => {
    const env = { OPENCLAW_HOME: "~/svc", HOME: "/Users/test" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/svc/.openclaw"));
  });
