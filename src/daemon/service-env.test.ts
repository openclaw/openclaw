...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.DNA_GATEWAY_PORT).toBe("18789");
    expect(env.DNA_GATEWAY_TOKEN).toBe("secret");
    expect(env.DNA_SERVICE_MARKER).toBe("dna");
    expect(env.DNA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.DNA_SERVICE_VERSION).toBe("string");
    expect(env.DNA_SYSTEMD_UNIT).toBe("dna-gateway.service");
    if (process.platform === "darwin") {
      expect(env.DNA_LAUNCHD_LABEL).toBe("bot.molt.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", DNA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.DNA_SYSTEMD_UNIT).toBe("dna-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.DNA_LAUNCHD_LABEL).toBe("bot.molt.work");
    }
  });
});
