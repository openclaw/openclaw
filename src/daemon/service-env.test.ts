...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ACTIVI_GATEWAY_PORT).toBe("18789");
    expect(env.ACTIVI_GATEWAY_TOKEN).toBe("secret");
    expect(env.ACTIVI_SERVICE_MARKER).toBe("activi");
    expect(env.ACTIVI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ACTIVI_SERVICE_VERSION).toBe("string");
    expect(env.ACTIVI_SYSTEMD_UNIT).toBe("activi-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ACTIVI_LAUNCHD_LABEL).toBe("ai.activi.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ACTIVI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ACTIVI_SYSTEMD_UNIT).toBe("activi-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ACTIVI_LAUNCHD_LABEL).toBe("ai.activi.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".activi"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ACTIVI_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".activi-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ACTIVI_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".activi"));
  });

  it("uses ACTIVI_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ACTIVI_STATE_DIR: "/var/lib/activi" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/activi"));
  });

  it("expands ~ in ACTIVI_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ACTIVI_STATE_DIR: "~/activi-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/activi-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ACTIVI_STATE_DIR: "C:\\State\\activi" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\activi");
  });
});
