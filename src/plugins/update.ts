...
function buildLoadPathHelpers(
	existing: string[],
	env: NodeJS.ProcessEnv = process.env,
) {
	let paths = [...existing];
	const resolveSet = () =>
		new Set(paths.map((entry) => resolveUserPath(entry, env)));
...
export async function updateNpmInstalledPlugins(params: {
	...
	for (const pluginId of targets) {
		...
		const effectiveSpec =
			record.source === "npm"
				? (params.specOverrides?.[pluginId] ?? record.spec)
				: record.spec;
		...
		if (record.source === "npm") {
			...
			next = recordPluginInstall(next, {
				...
				source: "clawhub",
				spec:
					effectiveSpec ?? record.spec ?? `clawhub:${record.clawhubPackage!}`,
				installPath: result.targetDir,
				version: nextVersion,
			...
			next = recordPluginInstall(next, {
				...
				version: nextVersion,
				marketplaceName:
					marketplaceResult.marketplaceName ?? record.marketplaceName,
				marketplaceSource: record.marketplaceSource,
				marketplacePlugin: record.marketplacePlugin,
			});
		}
...
export async function syncPluginsForUpdateChannel(params: {
	...
	let next = params.config;
	const loadHelpers = buildLoadPathHelpers(
