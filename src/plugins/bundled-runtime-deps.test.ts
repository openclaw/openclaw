...
describe("installBundledRuntimeDeps", () => {
  ...
  it("strips workspace:* devDependencies from the source manifest before spawning npm and restores them after", () => {
    const installRoot = makeTempDir();
    const manifestPath = path.join(installRoot, "package.json");
    const sourceManifest = {
      name: "@openclaw/test-plugin",
      version: "1.0.0",
      dependencies: { tokenjuice: "0.6.1" },
      devDependencies: {
    ...
    let manifestSeenByNpm: Record<string, unknown> | null = null;
    spawnSyncMock.mockImplementation((_command, _args, options) => {
      const cwd = String(options?.cwd ?? "");
      manifestSeenByNpm = JSON.parse(fs.readFileSync(path.join(cwd, "package.json"), "utf8"));
      fs.mkdirSync(path.join(cwd, "node_modules", "tokenjuice"), { recursive: true });
      fs.writeFileSync(
        path.join(cwd, "node_modules", "tokenjuice", "package.json"),
        JSON.stringify({ name: "tokenjuice", version: "0.6.1" }),
    ...
    expect((manifestSeenByNpm as Record<string, unknown>).devDependencies).toEqual({
      "some-other-dev": "1.0.0",
    });

    // The original on-disk manifest is restored verbatim after install
    expect(JSON.parse(fs.readFileSync(manifestPath, "utf8"))).toEqual(sourceManifest);
  });

  it("leaves the source manifest untouched when there are no workspace:* devDependencies", () => {
    const installRoot = makeTempDir();
    const manifestPath = path.join(installRoot, "package.json");
    const sourceManifest = {
      name: "@openclaw/clean-plugin",
      version: "1.0.0",
      dependencies: { tokenjuice: "0.6.1" },
      devDependencies: { "some-other-dev": "1.0.0" },
    };
    const originalRaw = `${JSON.stringify(sourceManifest, null, 2)}\n`;
    fs.writeFileSync(manifestPath, originalRaw, "utf8");
    ...
    expect(fs.readFileSync(manifestPath, "utf8")).toBe(originalRaw);
  });
