...
export function loadPluginManifestRegistry(params: {
  ...
  for (const candidate of candidates) {
    ...
    if (existing) {
      ...
      const winningCandidate = candidateWins ? candidate : existing.candidate;
      const losingCandidate = candidateWins ? existing.candidate : candidate;
      const messagePrefix =
        winningCandidate.origin === "bundled" || losingCandidate.origin === "bundled"
          ? ""
      ...
      diagnostics.push({
        ...
        pluginId: manifest.id,
        source: candidate.source,
        message: `${messagePrefix}${losingCandidate.origin} plugin will be overridden by ${winningCandidate.origin} plugin (${candidate.source})`,
      });
    } else {
