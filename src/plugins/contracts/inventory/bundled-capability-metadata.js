...
import { normalizeBundledPluginStringList, resolveBundledPluginScanDir, } from "../../bundled-plugin-scan.js";
...
function readBundledCapabilityManifest(pluginDir) {
    ...
    const extensions = normalizeBundledPluginStringList(packageJson?.openclaw && typeof packageJson.openclaw === "object"
...
function normalizeStringListRecord(record) {
    ...
    return Object.fromEntries(Object.entries(record)
        .map(([key, values]) => [
        key.trim(),
        uniqueStrings(Array.isArray(values) ? values : [], (value) => typeof value === "string" ? value.trim() : ""),
    ])
        .filter(([key, values]) => key && values.length > 0)
        .toSorted(([left], [right]) => left.localeCompare(right)));
}
export function buildBundledPluginContractSnapshot(manifest) {
    return {
...
export function hasBundledPluginContractSnapshotCapabilities(entry) {
    return (entry.cliBackendIds.length > 0 ||
...
export const BUNDLED_PLUGIN_CONTRACT_SNAPSHOTS = BUNDLED_CAPABILITY_MANIFESTS.map(buildBundledPluginContractSnapshot)
    .filter(hasBundledPluginContractSnapshotCapabilities)
    .toSorted((left, right) => left.pluginId.localeCompare(right.pluginId));
export const BUNDLED_LEGACY_PLUGIN_ID_ALIASES = Object.fromEntries(BUNDLED_CAPABILITY_MANIFESTS.flatMap((manifest) => (manifest.legacyPluginIds ?? []).map((legacyPluginId) => [legacyPluginId, manifest.id])).toSorted(([left], [right]) => left.localeCompare(right)));
export const BUNDLED_AUTO_ENABLE_PROVIDER_PLUGIN_IDS = Object.fromEntries(BUNDLED_CAPABILITY_MANIFESTS.flatMap((manifest) => (manifest.autoEnableWhenConfiguredProviders ?? []).map((providerId) => [
    providerId,
    manifest.id,
])).toSorted(([left], [right]) => left.localeCompare(right)));
export function resolveBundledContractSnapshotPluginIds(key) {
