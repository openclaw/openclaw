...
describe("plan-mode parity follow-ups (Seams B-F)", () => {
  beforeEach(() => {
  ...
  describe("Seam B: resolveAttachmentDelivery channel hint precedence", () => {
    ...
    it("plumbs Telegram silent through sendPluginSessionAttachment via channelHints", async () => {
      ...
      // sendMessage records the resolved `silent` flag for the assertion.
        ...
        await withTempConfig({
          ...
          run: async () => {
            ...
            const result = await sendPluginSessionAttachment({
              ...
              files: [{ path: filePath }],
              channelHints: { telegram: { disableNotification: true } },
            });
  ...
  describe("Seam D: scheduler tags + payloadExtras + tag-based cleanup", () => {
    it("auto-prefixes the cron job name with the plugin id and tag", () => {
      ...
      ).toBe("plugin:plan-mode:tag:nudge:agent:main:main:abc");
    });
    ...
    it("falls back to a non-tagged name when no tag is supplied", () => {
      ...
      ).toBe("plugin:plan-mode:agent:main:main:xyz");
    });
    ...
    it("merges payloadExtras into the cron payload and persists the tagged name", async () => {
      ...
      const handle = await schedulePluginSessionTurn({
        ...
        schedule: {
          ...
          delayMs: 1_000,
          tag: "nudge",
          payloadExtras: { archetype: "plan-snapshot", version: 2 },
        },
      });
      ...
      expect(job.name).toMatch(/^plugin:plan-mode:tag:nudge:agent:main:main:/);
      const payload = job.payload as Record<string, unknown>;
      expect(payload.kind).toBe("agentTurn");
      expect(payload.message).toBe("wake");
      expect(payload.archetype).toBe("plan-snapshot");
    ...
    it("listing-then-removing by tag only touches matching jobs in the same session", async () => {
      ...
      workflowMocks.callGatewayTool.mockImplementation(
        async (method: string, _opts: unknown, body: unknown) => {
          if (method === "cron.list") {
            return {
              jobs: [
                {
                  id: "job-a",
                  name: "plugin:plan-mode:tag:nudge:agent:main:main:1",
                  sessionTarget: "session:agent:main:main",
                },
                {
                  id: "job-b",
                  name: "plugin:plan-mode:tag:nudge:agent:main:main:2",
                  sessionTarget: "session:agent:main:main",
                },
                {
                  id: "job-c",
                ...
                },
                {
                  ...
                  name: "plugin:plan-mode:tag:nudge:agent:other:main:1",
          ...
          if (method === "cron.remove") {
            removed.push(((body as { id?: string }).id ?? "") as string);
            return { removed: true };
          }
          return undefined;
        },
      ...
      expect(removed.sort()).toEqual(["job-a", "job-b"]);
    });
      ...
      registerTestPlugin({
        ...
        register(api) {
          api.registerSessionExtension({
            ...
            sessionEntrySlotKey: "planSnapshot",
        ...
        await withTempConfig({
          ...
          run: async () => {
            ...
            const patchResult = await patchPluginSessionExtension({
              ...
              namespace: "workflow",
              value: { state: "executing", title: "Plan A", internal: 7 },
            });
            expect(patchResult.ok).toBe(true);
            const afterPatch = loadSessionStore(storePath, { skipCache: true });
            expect(
              (afterPatch["agent:main:main"] as unknown as Record<string, unknown>).planSnapshot,
            ).toEqual({ state: "executing", title: "Plan A" });
            ...
            expect(
              (afterUnset["agent:main:main"] as unknown as Record<string, unknown>).planSnapshot,
            ).toBeUndefined();
          },
        });
      } finally {
      ...
      try {
        ...
        await withTempConfig({
          ...
          run: async () => {
            ...
            expect(entry.planSnapshot).toBeUndefined();
          },
        });
      } finally {
        if (previousStateDir === undefined) {
