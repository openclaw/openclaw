...
describe("host runtime sync reads", () => {
  ...
  describe("getHostRunContext snapshot", () => {
    ...
    it("throws when the namespace is not the canonical host one", () => {
      expect(() =>
        // oxlint-disable-next-line typescript/no-explicit-any -- intentional negative test
        resolveHostRunContextSnapshot("run-x", "_host.future" as any),
      ).toThrowError(/unknown host run-context namespace/);
    });
