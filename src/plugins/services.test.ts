...
import type { MullusiPluginService, MullusiPluginServiceContext } from "./types.js";
...
function createRegistry(services: MullusiPluginService[]) {
...
function expectServiceContext(
  ctx: MullusiPluginServiceContext,
...
function expectServiceLogger(ctx: MullusiPluginServiceContext) {
...
function expectServiceContexts(
  contexts: MullusiPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: MullusiPluginServiceContext[] = [];
