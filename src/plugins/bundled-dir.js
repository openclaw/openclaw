...
export function resolveBundledPluginsDir(env = process.env) {
    ...
    try {
        ...
        const packageRoots = (preferSourceCheckout ? [cwdRoot, argvRoot, moduleRoot] : [argvRoot, cwdRoot, moduleRoot]).filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index);
