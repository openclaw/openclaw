...
import { getActivePluginRegistry, getActivePluginRegistryKey, getActivePluginRuntimeSubagentMode, } from "./runtime.js";
...
export function setPluginToolMeta(tool, meta) {
    pluginToolMeta.set(tool, meta);
}
export function getPluginToolMeta(tool) {
    return pluginToolMeta.get(tool);
}
export function copyPluginToolMeta(source, target) {
    ...
    for (const entry of registry.tools) {
        ...
        if (!resolved) {
            if (entry.names.length > 0) {
                context.logger.debug?.(`plugin tool factory returned null (${entry.pluginId}): [${entry.names.join(", ")}]`);
            }
            continue;
        }
