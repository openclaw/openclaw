...
import { createPluginIdScopeSet, normalizePluginIdScope, serializePluginIdScope, } from "./plugin-scope.js";
function comparePluginProvidersAlphabetically(left, right) {
    return left.id.localeCompare(right.id) || left.pluginId.localeCompare(right.pluginId);
}
export function sortPluginProviders(providers) {
    return providers.toSorted(comparePluginProvidersAlphabetically);
}
export function sortPluginProvidersForAutoDetect(providers) {
...
export function buildWebProviderSnapshotCacheKey(params) {
    const envKey = typeof params.envKey === "string"
        ? params.envKey
...
export function mapRegistryProviders(params) {
    const onlyPluginIdSet = createPluginIdScopeSet(normalizePluginIdScope(params.onlyPluginIds));
    return params.sortProviders(params.entries
