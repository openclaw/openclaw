...
import { clearPluginCommands, clearPluginCommandsForPlugin, getPluginCommandSpecs, isPluginCommandRegistryLocked, listProviderPluginCommandSpecs, pluginCommands, } from "./command-registry-state.js";
/**
 ...
 */
export function validatePluginCommandDefinition(command) {
...
export function listPluginInvocationKeys(command) {
    const keys = new Set();
    const push = (value) => {
...
export function registerPluginCommand(pluginId, command, opts) {
    ...
    for (const invocationKey of invocationKeys) {
        const existing = pluginCommands.get(invocationKey) ??
            Array.from(pluginCommands.values()).find((candidate) => listPluginInvocationKeys(candidate).includes(invocationKey));
        if (existing) {
            return {
                ok: false,
