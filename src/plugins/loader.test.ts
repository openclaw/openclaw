...
import { loadDNAPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.DNA_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `dna-plugin-${randomUUID()}`);
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.DNA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadDNAPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.DNA_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadDNAPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadDNAPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@dna/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        dna: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
