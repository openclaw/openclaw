...
import { normalizeBundledPluginStringList, trimBundledPluginString, } from "./bundled-plugin-scan.js";
...
const PUBLIC_CONFIG_SURFACE_BASENAMES = ["channel-config-api", "runtime-api", "api"];
...
export function collectBundledChannelConfigs(params) {
    ...
    const existingChannelConfigs = params.manifest.channelConfigs && Object.keys(params.manifest.channelConfigs).length > 0
    ...
    for (const channelId of channelIds) {
        ...
        existingChannelConfigs[channelId] = {
            ...
            ...((trimBundledPluginString(existing?.label) ?? trimBundledPluginString(channelMeta?.label))
                ? {
                    label: trimBundledPluginString(existing?.label) ??
            ...
            ...((trimBundledPluginString(existing?.description) ??
                ...
                ? {
                    description: trimBundledPluginString(existing?.description) ??
                        trimBundledPluginString(channelMeta?.blurb),
                }
                : {}),
