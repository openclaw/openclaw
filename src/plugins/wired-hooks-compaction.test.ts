...
describe("compaction hook wiring", () => {
	...
	beforeEach(async () => {
		...
		({ handleAutoCompactionStart, handleAutoCompactionEnd } = await import(
			"../agents/pi-embedded-subscribe.handlers.compaction.js"
		));
	});
	...
	function expectCompactionEvent(params: {
		call:
			| ReturnType<typeof getBeforeCompactionCall>
			| ReturnType<typeof getAfterCompactionCall>;
		expectedEvent: Record<string, unknown>;
		expectedSessionKey?: string;
	}) {
		expect(params.call.event).toEqual(
