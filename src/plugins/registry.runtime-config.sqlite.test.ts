...
// derives the agent scope (and the incognito sentinel store path) from the key
// so the real accessor can open the store instead of throwing "Cannot resolve
// SQLite session scope without an agent id".
...
describe("embedded-run session ownership resolves through the real SQLite session store", () => {
  it("scopes the key-only ownership listing to the derived agent and its store (#120178)", async () => {
