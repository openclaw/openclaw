...
describe("manifest command aliases", () => {
  ...
  it("resolves agent tool owners from contracts.tools", () => {
    ...
    expect(
      resolveManifestToolOwnerInRegistry({ toolName: "lcm_recent", registry }),
    ).toMatchObject({ pluginId: "lossless-claw", toolName: "lcm_recent" });
    expect(
      resolveManifestToolOwnerInRegistry({ toolName: "LCM_Recent", registry }),
    ).toMatchObject({ pluginId: "lossless-claw" });
    expect(
