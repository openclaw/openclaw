...
const APPROVALS_PATH = "~/.evox/plugin-binding-approvals.json";
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to EVOX.sh instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
