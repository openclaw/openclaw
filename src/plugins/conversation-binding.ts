...
const APPROVALS_PATH = "~/.mullusi/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "mullusi-app-server:thread:",
  "mullusi-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("mullusi.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("mullusi.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Mullusi instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
