...
import { hasExplicitPluginConfig, normalizePluginsConfig, resolveEffectivePluginActivationState, } from "./config-state.js";
...
function loadBundleCommandsFromRoot(params) {
    ...
    for (const filePath of listMarkdownFilesRecursive(params.commandRoot)) {
        ...
        const rawName = normalizeOptionalString(frontmatter.name) ||
            toDefaultCommandName(params.commandRoot, filePath);
        if (!rawName) {
            continue;
        }
        const description = normalizeOptionalString(frontmatter.description) ||
