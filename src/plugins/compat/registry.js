export const PLUGIN_COMPAT_RECORDS = [
    ...
    },
    {
        ...
        releaseNote: "`plugins.installs` remains readable as a legacy compatibility fallback while new plugin install metadata moves to the state-managed install ledger.",
    },
];
const pluginCompatRecordByCode = new Map(PLUGIN_COMPAT_RECORDS.map((record) => [record.code, record]));
export function listPluginCompatRecords() {
    return PLUGIN_COMPAT_RECORDS;
}
export function getPluginCompatRecord(code) {
...
export function isPluginCompatCode(code) {
    return pluginCompatRecordByCode.has(code);
}
export function listDeprecatedPluginCompatRecords() {
    return PLUGIN_COMPAT_RECORDS.filter((record) => ["deprecated", "removal-pending"].includes(record.status));
}
