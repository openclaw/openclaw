...
import { DEFAULT_MEMORY_DREAMING_PLUGIN_ID, resolveMemoryDreamingConfig, resolveMemoryDreamingPluginConfig, resolveMemoryDreamingPluginId, } from "../memory-host-sdk/dreaming.js";
import { normalizeOptionalLowercaseString } from "../shared/string-coerce.js";
import { hasExplicitChannelConfig } from "./channel-presence-policy.js";
import { resolveEffectivePluginActivationState } from "./config-state.js";
import { createPluginRegistryIdNormalizer, loadPluginRegistrySnapshot, normalizePluginsConfigWithRegistry, } from "./plugin-registry.js";
...
function resolveExplicitMemorySlotStartupPluginId(config, normalizePluginId) {
...
function hasConfiguredStartupChannel(params) {
    return params.plugin.contributions.channels.some((channelId) => params.configuredChannelIds.has(channelId));
}
function canStartConfiguredChannelPlugin(params) {
    ...
    const explicitBundledChannelConfig = params.plugin.origin === "bundled" &&
        params.plugin.contributions.channels.some((channelId) => hasExplicitChannelConfig({
            config: params.activationSource.rootConfig ?? params.config,
            channelId,
        }));
    if (params.pluginsConfig.allow.length > 0 &&
...
export function resolveConfiguredDeferredChannelPluginIds(params) {
    ...
    return index.plugins
        .filter((plugin) => hasConfiguredStartupChannel({ plugin, configuredChannelIds }) &&
...
export function resolveGatewayStartupPluginIds(params) {
    ...
    const explicitMemorySlotStartupPluginId = resolveExplicitMemorySlotStartupPluginId(activationSourceConfig, createPluginRegistryIdNormalizer(index));
    return index.plugins
        .filter((plugin) => {
        ...
        if (plugin.startup.agentHarnesses.some((runtime) => requiredAgentHarnessRuntimes.has(runtime))) {
            const activationState = resolveEffectivePluginActivationState({
                id: plugin.pluginId,
