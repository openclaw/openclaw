...
function createNpmInstallRecord(
	pluginId = "my-plugin",
	installPath?: string,
): PluginInstallRecord {
	return {
...
async function expectPathAccessState(
	pathToCheck: string,
	expected: "exists" | "missing",
) {
...
describe("resolveUninstallChannelConfigKeys", () => {
	...
	it("keeps explicit empty channelIds as remove-nothing", () => {
		expect(
			resolveUninstallChannelConfigKeys("telegram", { channelIds: [] }),
		).toEqual([]);
	});
...
describe("uninstallPlugin", () => {
	...
	it("returns a warning when directory deletion fails unexpectedly", async () => {
		...
		try {
			...
			expect(successfulResult.warnings[0]).toContain(
