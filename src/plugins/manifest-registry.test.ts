...
describe("loadPluginManifestRegistry", () => {
  it("emits duplicate warning for truly distinct plugins with same id (same origin)", () => {
  ...
  it("suppresses duplicate warning for plugins with same id but different origins (bundled vs global)", () => {
    const dirA = makeTempDir();
    const dirB = makeTempDir();
    const manifest = { id: "test-plugin", configSchema: { type: "object" } };
    writeManifest(dirA, manifest);
    ...
    // Different origins indicate intentional override - no warning needed
