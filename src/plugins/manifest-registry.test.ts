...
describe("loadPluginManifestRegistry", () => {
  ...
  it("suppresses legacy global shadow duplicates for bundled qveris", () => {
    const bundledDir = makeTempDir();
    const stateDir = makeTempDir();
    const globalDir = path.join(stateDir, "extensions", "qveris");
    mkdirSafe(globalDir);
    const manifest = { id: "qveris", configSchema: { type: "object" } };
    writeManifest(bundledDir, manifest);
    writeManifest(globalDir, manifest);
    ...
    expect(
      registry.diagnostics.some((diag) => diag.message.includes("duplicate plugin id detected")),
    ).toBe(false);
    expect(registry.plugins.filter((plugin) => plugin.id === "qveris")).toHaveLength(1);
    expect(registry.plugins.find((plugin) => plugin.id === "qveris")?.origin).toBe("bundled");
  });
