...
export function createNetworkRegistrars(state: PluginRegistryState) {
  ...
  const registerChannel = (
    ...
    const id = plugin.id;
    // A fresh factory identifies each registration, including same-owner updates.
    // Capture the activation at invocation: initial registration precedes activation.
    ...
    if (registrationCapabilities.runtimeChannel && existingRuntime) {
      if (existingRuntime.pluginId === record.id) {
        ...
        existingRuntime.trustedOfficialInstall = record.trustedOfficialInstall;
