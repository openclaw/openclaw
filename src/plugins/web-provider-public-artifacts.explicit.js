...
const WEB_SEARCH_RUNTIME_ARTIFACT_CANDIDATES = ["web-search-provider.js", "web-search.js"];
...
function hasBundledPublicArtifactCandidate(params) {
    return params.artifactCandidates.some((artifactBasename) => Boolean(resolveBundledPluginPublicArtifactPath({ dirName: params.dirName, artifactBasename })));
}
export function hasBundledWebSearchProviderPublicArtifact(pluginId) {
    return hasBundledPublicArtifactCandidate({
        dirName: pluginId,
        artifactCandidates: WEB_SEARCH_ARTIFACT_CANDIDATES,
    });
}
export function hasBundledWebFetchProviderPublicArtifact(pluginId) {
    return hasBundledPublicArtifactCandidate({
        dirName: pluginId,
        artifactCandidates: WEB_FETCH_ARTIFACT_CANDIDATES,
    });
