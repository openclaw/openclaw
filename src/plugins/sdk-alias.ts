...
function hasTrustedMullusiRootIndicator(params: {
  packageRoot: string;
  ...
  const hasMullusiBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("mullusi")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.mullusi === "string");
  const hasMullusiEntrypoint = fs.existsSync(path.join(params.packageRoot, "mullusi.mjs"));
  return hasCliEntryExport || hasMullusiBin || hasMullusiEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedMullusiRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedMullusiRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveMullusiPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedMullusiRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveMullusiPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedMullusiRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias ? { "mullusi/extension-api": extensionApiAlias } : {}),
