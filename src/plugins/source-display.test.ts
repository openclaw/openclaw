...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "mullusi", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".mullusi", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".mullusi", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "mullusi-home");
    expectResolvedSourceRoots({
      homeDir,
      env: {
        MULLUSI_BUNDLED_PLUGINS_DIR: "~/bundled",
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".mullusi", "extensions"),
      },
    });
  });
});
