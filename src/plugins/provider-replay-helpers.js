...
export function buildOpenAICompatibleReplayPolicy(modelApi, options = {}) {
...
export function buildStrictAnthropicReplayPolicy(options = {}) {
    const sanitizeToolCallIds = options.sanitizeToolCallIds ?? true;
    return {
        sanitizeMode: "full",
        ...(sanitizeToolCallIds
 ...
 */
export function shouldPreserveThinkingBlocks(modelId) {
...
export function buildAnthropicReplayPolicyForModel(modelId) {
...
export function buildNativeAnthropicReplayPolicyForModel(modelId) {
    const isClaude = normalizeLowercaseStringOrEmpty(modelId).includes("claude");
    return buildStrictAnthropicReplayPolicy({
        dropThinkingBlocks: isClaude && !shouldPreserveThinkingBlocks(modelId),
        sanitizeToolCallIds: true,
...
export function buildHybridAnthropicOrOpenAIReplayPolicy(ctx, options = {}) {
    if (ctx.modelApi === "anthropic-messages" || ctx.modelApi === "bedrock-converse-stream") {
        ...
        return buildStrictAnthropicReplayPolicy({
            dropThinkingBlocks: options.anthropicModelDropThinkingBlocks &&
                isClaude &&
...
export function buildGoogleGeminiReplayPolicy() {
    return {
        sanitizeMode: "full",
        sanitizeToolCallIds: true,
        toolCallIdMode: "strict",
...
export function buildPassthroughGeminiSanitizingReplayPolicy(modelId) {
...
export function sanitizeGoogleGeminiReplayHistory(ctx) {
    const messages = sanitizeGoogleAssistantFirstOrdering(ctx.messages);
    if (messages !== ctx.messages &&
        ctx.sessionState &&
...
export function resolveTaggedReasoningOutputMode() {
    return "tagged";
}
