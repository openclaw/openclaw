...
function buildWebSearchSnapshotCacheKey(params: {
	...
	return JSON.stringify({
		...
		onlyPluginIds: [...new Set(params.onlyPluginIds ?? [])].toSorted(
...
function pluginManifestDeclaresWebSearch(
	...
	if (
		configUiHintKeys.some(
			(key) => key === "webSearch" || key.startsWith("webSearch."),
		)
	) {
		return true;
	}
...
function mapRegistryWebSearchProviders(params: {
	...
	return sortWebSearchProviders(
		params.registry.webSearchProviders
			.filter(
				(entry) => !onlyPluginIdSet || onlyPluginIdSet.has(entry.pluginId),
			)
			.map((entry) => ({
				...entry.provider,
...
export function resolvePluginWebSearchProviders(params: {
	...
	const shouldMemoizeSnapshot =
		...
		params.cache !== true &&
		shouldUsePluginSnapshotCache(env);
...
export function resolveRuntimeWebSearchProviders(params: {
	...
	const runtimeRegistry = resolveRuntimePluginRegistry(
		params.config === undefined
			? undefined
			: resolveWebSearchLoadOptions(params),
	);
	if (runtimeRegistry) {
