...
export async function schedulePluginSessionTurn(params: {
  ...
  // Explicit name overrides — accept it as-is so callers can pin a known cron
  // name. Otherwise build a deterministic plugin-prefixed name (with optional
  // tag) so listing/cleanup can find this job.
  const name =
