...
function resolveProviderDefaultEnvSecretRef(provider, config) {
    ...
    if (!envVar) {
        throw new Error(`Provider "${provider}" does not have a default env var mapping for secret-input-mode=ref.`);
    }
...
export function buildApiKeyCredential(provider, input, metadata, options) {
...
export function applyAuthProfileConfig(cfg, params) {
    ...
    const configuredProviderProfiles = Object.entries(cfg.auth?.profiles ?? {})
        .filter(([, profile]) => resolveProviderIdForAuth(profile.provider, { config: cfg }) === normalizedProvider)
    ...
    const existingProviderOrder = matchingProviderOrderEntries.length > 0
    ...
    const derivedProviderOrder = existingProviderOrder === undefined && preferProfileFirst && hasMixedConfiguredModes
        ...
        : undefined;
    const baseOrder = matchingProviderOrderEntries.length > 0
        ? Object.fromEntries(Object.entries(cfg.auth?.order ?? {}).filter(([providerId]) => resolveProviderIdForAuth(providerId, { config: cfg }) !== normalizedProvider))
        : cfg.auth?.order;
