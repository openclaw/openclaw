...
describe("resolvePluginTools optional tools", () => {
  ...
  it("gates delegated auth for manifest-optional factory entries", async () => {
    const delegatedAuth: OpenClawPluginAuthContext = {
      getDelegatedAccessToken: vi.fn(async () => ({
        ok: true as const,
        token: "delegated-token",
    ...
    const anchorFactory = vi.fn(() => makeTool("anchor_tool"));
    const observedOptionalFactoryAuth: Array<OpenClawPluginAuthContext | undefined> = [];
    const optionalFactory = vi.fn((rawCtx: unknown) => {
      const ctx = rawCtx as { auth?: OpenClawPluginAuthContext };
      observedOptionalFactoryAuth.push(ctx.auth);
      return {
        ...makeTool("optional_auth_tool"),
        async execute() {
    ...
    setRegistry([
      {
        pluginId: "manifest-optional-auth",
        optional: false,
        source: "/tmp/manifest-optional-auth-anchor.js",
        names: ["anchor_tool"],
        factory: anchorFactory,
      },
      {
        pluginId: "manifest-optional-auth",
        optional: false,
        source: "/tmp/manifest-optional-auth-optional.js",
        names: ["optional_auth_tool"],
        declaredNames: ["optional_auth_tool"],
        factory: optionalFactory,
      },
    ]);
    const base = createContext();
    const config = {
      ...
      plugins: {
        ...base.config.plugins,
        enabled: true,
        allow: ["manifest-optional-auth"],
        entries: {
          "manifest-optional-auth": {
            auth: { delegatedAccess: { enabled: true } },
          },
        },
      },
    ...
    installToolManifestSnapshot({
      config: config as never,
      plugin: {
        id: "manifest-optional-auth",
        origin: "bundled",
        enabledByDefault: true,
        channels: [],
        providers: [],
        contracts: { tools: ["anchor_tool", "optional_auth_tool"] },
        toolMetadata: {
          optional_auth_tool: {
            optional: true,
          },
    ...
    const defaultTools = resolvePluginTools({
      context: {
        ...createContext(),
        auth: delegatedAuth,
        config,
    ...
    expectResolvedToolNames(defaultTools, ["anchor_tool"]);
    expect(anchorFactory).toHaveBeenCalledTimes(1);
    expect(optionalFactory).toHaveBeenCalledTimes(1);
    expect(observedOptionalFactoryAuth[0]).toBeUndefined();

    const allowlistedTools = resolvePluginTools({
      context: {
        ...createContext(),
        auth: delegatedAuth,
        config,
      } as never,
      toolAllowlist: [DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY, "optional_auth_tool"],
    });

    expectResolvedToolNames(allowlistedTools, ["anchor_tool", "optional_auth_tool"]);
    expect(optionalFactory).toHaveBeenCalledTimes(2);
    expect(observedOptionalFactoryAuth[1]).toBeDefined();
    await expect(executeToolDetails(allowlistedTools[1], {})).resolves.toEqual({
      ok: true,
      token: "delegated-token",
    });
  ...
  it("does not let a no-auth resolution poison cached delegated auth tools", async () => {
    ...
    expect(factory).toHaveBeenCalledTimes(2);
    expect(observedAuth[0]).toBeUndefined();
    expect(observedAuth[1]).toBeDefined();
    await expect(executeToolDetails(noAuthTools[0], {})).resolves.toBeUndefined();
  ...
  it("accepts Entra app-id audience claims for api scheme delegated auth policies", async () => {
    ...
    const uppercaseDownstreamAppId = downstreamAppId.toUpperCase();
    const allowedToken = createJwt({
      aud: downstreamAppId,
      scp: "allowed.scope",
    });
          ...
          allowed: {
            auth: {
              delegatedAccess: {
                enabled: true,
                providers: ["msteams"],
                audiences: [`api://${uppercaseDownstreamAppId}`],
                scopes: ["allowed.scope"],
              },
            },
          },
    ...
    await expect(
      executeToolDetails(tools[0], {
        provider: "msteams",
        audience: uppercaseDownstreamAppId,
        scopes: ["allowed.scope"],
      }),
    ).resolves.toEqual({
      ok: true,
  ...
  it("accepts returned delegated tokens with extra granted scopes and audiences", async () => {
  ...
  it("rejects returned delegated tokens missing requested scopes or with wrong audiences", async () => {
    const missingRequestedScopeToken = createJwt({
      aud: "api://allowed",
      scp: "other.scope",
    });
    const wrongAudienceToken = createJwt({
      aud: ["api://other", "api://untrusted"],
      scp: "allowed.scope",
    });
    const wrongRequestedAudienceToken = createJwt({
      aud: "api://secondary",
      scp: "allowed.scope",
    });
    const delegatedAuth: OpenClawPluginAuthContext = {
      getDelegatedAccessToken: vi
        ...
        .mockResolvedValueOnce({
          ok: true as const,
          token: wrongRequestedAudienceToken,
        }),
    };
    let pluginContext: { auth?: OpenClawPluginAuthContext } | undefined;
    setRegistry([
          ...
          allowed: {
            auth: {
              delegatedAccess: {
                enabled: true,
                providers: ["msteams"],
                audiences: ["api://allowed", "api://secondary"],
                scopes: ["allowed.scope"],
              },
            },
          },
    ...
    expect(delegatedAuth.getDelegatedAccessToken).toHaveBeenCalledTimes(3);
  });
