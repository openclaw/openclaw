...
type WebSearchProvidersRuntimeModule =
	typeof import("./web-search-providers.runtime.js");
...
type WebSearchProvidersSharedModule =
	typeof import("./web-search-providers.shared.js");
...
function toRuntimeProviderKeys(
	providers: ReturnType<
		WebSearchProvidersRuntimeModule["resolvePluginWebSearchProviders"]
	>,
) {
...
function expectRuntimeProviderResolution(
	providers: ReturnType<
		WebSearchProvidersRuntimeModule["resolveRuntimeWebSearchProviders"]
	>,
...
describe("resolvePluginWebSearchProviders", () => {
	beforeAll(async () => {
		...
		webSearchProvidersSharedModule = await import(
	...
	it("loads plugin web-search providers from the auto-enabled config snapshot", () => {
		...
		resolvePluginWebSearchProviders(
			createSnapshotParams({ config: rawConfig }),
		);
	...
	it.each([
		...
		},
		{
			...
			mutate: (
				_config: { plugins?: Record<string, unknown> },
				env: NodeJS.ProcessEnv,
			) => {
	...
	it.each([
		{
			title:
				"skips web-search snapshot memoization when plugin cache opt-outs are set",
			env: {
		...
		{
			title:
				"skips web-search snapshot memoization when discovery cache ttl is zero",
			env: {
	...
	it.each([
		...
		},
		{
			...
			setupRegistry: () => {
				...
				const { cacheKey } =
					loaderModule.__testing.resolvePluginLoadCacheContext({
						config,
