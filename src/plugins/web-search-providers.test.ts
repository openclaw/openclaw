...
function expectBundledWebSearchProviders(
	...
	expect(toProviderKeys(providers)).toEqual(
		EXPECTED_BUNDLED_WEB_SEARCH_PROVIDER_KEYS,
	);
...
describe("resolveBundledPluginWebSearchProviders", () => {
	it.each([
		...
		{
			title:
				"can resolve bundled providers through the manifest-scoped loader path",
			options: {
				bundledAllowlistCompat: true,
			},
		},
	] as const)("$title", { timeout: WEB_SEARCH_PROVIDER_TEST_TIMEOUT_MS }, ({
		options,
	}) => {
		...
		expectBundledWebSearchProviders(providers);
		expect(
			providers.find((provider) => provider.id === "firecrawl")
				?.applySelectionConfig,
		).toEqual(expect.any(Function));
		expect(
			providers.find((provider) => provider.id === "perplexity")
	...
	it.each([
		...
		{
			title:
				"does not return bundled providers excluded by a restrictive allowlist without compat",
			params: {
				config: {
					plugins: {
						allow: ["demo-other-plugin"],
