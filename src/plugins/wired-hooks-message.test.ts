...
async function expectMessageHookCall(params: {
	...
	const handler =
		params.hookResult === undefined
			? vi.fn()
			: vi.fn().mockReturnValue(params.hookResult);
	...
	if (params.hookName === "message_sending") {
		...
		expect(result).toEqual(
			expect.objectContaining(params.expectedResult ?? {}),
		);
	} else {
		await runner.runMessageSent(
			params.event as PluginHookMessageSentEvent,
			params.channelCtx,
		);
	}
