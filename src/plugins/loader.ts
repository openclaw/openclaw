...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SmartAgentNeoPluginDefinition;
  register?: SmartAgentNeoPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SmartAgentNeoPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SmartAgentNeoPluginDefinition;
...
export function loadSmartAgentNeoPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSmartAgentNeoPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "smart-agent-neo/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "smart-agent-neo/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SmartAgentNeoPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as SmartAgentNeoPluginModule;
    } catch (err) {
