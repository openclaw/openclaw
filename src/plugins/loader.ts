...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: EasyHubPluginDefinition;
  register?: EasyHubPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as EasyHubPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as EasyHubPluginDefinition;
...
export function loadEasyHubPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverEasyHubPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "EasyHub/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: EasyHubPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as EasyHubPluginModule;
    } catch (err) {
