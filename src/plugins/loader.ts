...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ActiviPluginDefinition;
  register?: ActiviPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ActiviPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ActiviPluginDefinition;
...
export function loadActiviPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverActiviPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "activi/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "activi/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ActiviPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as ActiviPluginModule;
    } catch (err) {
