...
function buildCacheKey(params: {
  ...
  return `${roots.workspace ?? ""}::${roots.global ?? ""}::${roots.stock ?? ""}::${JSON.stringify({
    ...
    installs,
    loadPaths,
  })}::${scopeKey}::${setupOnlyKey}::${startupChannelMode}::${params.runtimeSubagentMode ?? "default"}::${params.providerOnly ? "provider-only" : "full"}`;
}
...
export function loadOpenClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
    ...
    runtimeSubagentMode:
      options.runtimeOptions?.allowGatewaySubagentBinding === true
        ...
        : options.runtimeOptions?.subagent
          ? "explicit"
          : "default",
    providerOnly: options.providerOnly === true,
  });
  ...
  for (const candidate of orderedCandidates) {
    ...
    // Note: "setup-runtime" is intentionally not downgraded here; channel
    // plugins in setup-runtime mode do not register providers regardless, so
    // this is a pre-existing limitation rather than a regression.
