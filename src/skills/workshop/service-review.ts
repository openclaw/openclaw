...
export async function reviewSkillProposal(input: {
  ...
  try {
    ...
    if (error instanceof SkillProposalIntegrityError && error.record) {
      return {
        record: error.record,
        revisionHash: hashSkillProposalRevision(error.record),
        mode: "unavailable",
  ...
  if ("reason" in skillTarget) {
    return { record, revisionHash, mode: "unavailable", reason: skillTarget.reason };
  }
  ...
  if (record.kind === "create") {
    ...
    for (const file of supportFiles) {
      ...
      if ("reason" in supportTarget) {
        return { record, revisionHash, mode: "unavailable", reason: supportTarget.reason };
      }
