...
import {
  isIgnoredSkillsWatchPath,
  isSkillDiscoveryFileWatchPath,
  resolveSkillsWatcherPollingInterval,
...
// One fs-safe Node subscription owns one worker. Keep the domain ceiling
// independent of workspace retention and OS capacity, including retiring workers.
const MAX_SKILLS_WATCH_WORKERS = 16;
...
function createSkillsPathWatcher(
  ...
  const retireSubscription = async () => {
    ...
    // Construction has settled before this runs. No handle means construction
    // failed before acquiring a worker; failed actual close retains its slot.
    if (workerReserved) {
  ...
  const failed = (error: unknown, failure?: WatchHealth["failure"], workerLimit = false) => {
    ...
    state.verified = false;
    const capacity =
      failure?.operation === "watch" && ["ENOSPC", "EMFILE", "ENFILE"].includes(failure.code ?? "");
    if (capacity && !resolveSkillsWatcherUsePolling()) {
    ...
    if (!workerLimit || !state.unavailable) {
    ...
    if (!workerLimit && isCurrent() && !state.recovering && subscriber !== undefined) {
  ...
  starting = runInSkillsWatcherContext(() =>
    Promise.resolve().then(async () => {
      try {
        ...
        const polling = resolveSkillsWatcherUsePolling();
        if (!polling && skillsWatchWorkers >= MAX_SKILLS_WATCH_WORKERS) {
          // This is application admission, not ENOSPC. Keep healthy peers and
          // refresh this target during preparation; the next turn may retry.
          failed(
            new Error(
              "skills observation worker limit reached (" +
                MAX_SKILLS_WATCH_WORKERS +
                "); refreshing skills during agent preparation",
            ),
            undefined,
            true,
        ...
        workerReserved = !polling;
        if (workerReserved) {
          skillsWatchWorkers++;
        }
        subscription = watch(authority, {
          scopes: [scope],
          mode: polling ? "poll" : "node",
          intervalMs: polling ? resolveSkillsWatcherPollingInterval() : undefined,
          ...
          onDirty: (hint) => {
            ...
            for (const change of hint.changes) {
              ...
              } else if (isSkillDiscoveryFileWatchPath(relative)) {
          ...
          onHealth: (health) => {
            ...
            if (health.state === "unavailable") {
              failed(health.error, health.failure);
            }
          },
        });
      } finally {
