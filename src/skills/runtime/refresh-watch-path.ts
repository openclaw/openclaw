...
import { defaultFsObservationUsePolling } from "../../infra/fs-observation-mode.js";
...
export function resolveSkillsWatcherUsePolling(): boolean {
  ...
  if (envPolling === undefined) {
    return defaultFsObservationUsePolling();
  }
...
export function resolveSkillsWatcherPollingInterval(): number {
  const value = Number.parseInt(process.env.CHOKIDAR_INTERVAL ?? "", 10);
  return Number.isInteger(value) && value > 0 && value <= 2_147_483_647 ? Math.max(20, value) : 100;
}
