...
async function runStateLeaseOwnerInScope<T>(
  ...
  const timerObservation =
    invocation.kind === "worker" && validated.heartbeat !== "worker"
      ? new BigInt64Array(new SharedArrayBuffer(4 * BigInt64Array.BYTES_PER_ELEMENT))
      : undefined;
  const workerStorage =
    invocation.kind === "worker"
  ...
  if (workerStorage) {
    workerOperations = createOpenClawStateLeaseWorkerOwner({
      ...
      expiryObservation: timerObservation?.buffer,
      assertCurrent(purpose) {
        ...
        if (purpose === "acquire") {
          ...
          if (validated.waitMs > 0 && performance.now() >= deadline) {
            throw leaseError(
      ...
      if (invocation.kind === "worker") {
        ...
        result = await exclusion.runWithOwnerScope(() => {
          const lease: OpenClawStateLeaseContext = {
            ...
            withDatabaseFileMutation: (operation) => exclusion.runMutation(operation),
            signal: operationSignal,
