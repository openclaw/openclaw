...
describe("openclaw agent database", () => {
  ...
  it("rejects reusing an open explicit path for another agent", () => {
    const stateDir = createTempStateDir();
    const env = { OPENCLAW_STATE_DIR: stateDir };
    const sharedPath = path.join(stateDir, "relocated", "shared.sqlite");
    openOpenClawAgentDatabase({
      agentId: "worker-1",
      env,
      path: sharedPath,
  ...
  it("rejects reopening an existing agent database for another agent", () => {
    const stateDir = createTempStateDir();
    const env = { OPENCLAW_STATE_DIR: stateDir };
    const sharedPath = path.join(stateDir, "relocated", "shared.sqlite");
    openOpenClawAgentDatabase({
