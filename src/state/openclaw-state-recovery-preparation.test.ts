...
import { withStateDatabaseCoordinatorRuntimeDirectory } from "../infra/state-database-coordinator-runtime.js";
...
vi.mock("../infra/tmp-openclaw-dir.js", async (importOriginal) => ({
  ...
  resolvePreferredOpenClawTmpDir: () => {
    return requirePrivateCoordinatorRoot();
  },
}));

function requirePrivateCoordinatorRoot(): string {
  const root = privateCoordinator.root;
  if (!root || fsSync.realpathSync(root) !== root || !fsSync.lstatSync(root).isDirectory()) {
    throw new Error("Recovery fixture requires its physically private coordinator directory");
  }
  return root;
}
...
describe("shared-state recovery preparation", () => {
  it.each([
    ...
    async ({ older, newer, change }) =>
      withStateDatabaseCoordinatorRuntimeDirectory(requirePrivateCoordinatorRoot(), () =>
      ...
      `);
        ...
        `);
          ...
          if (change === "preserve" || change === "baseline worker purpose") {
            ...
            try {
              expect(prepared.prepare("PRAGMA user_version").get()).toEqual({
                user_version: older,
              });
