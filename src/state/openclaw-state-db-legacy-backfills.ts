...
import {
  repairLegacySubagentBackingMetadata,
  type LegacySubagentBackingMetadataRepairDisposition,
} from "./openclaw-state-db-subagent-backing-migration.js";
...
export type LegacySubagentTaskBindingRepairDisposition =
  LegacySubagentBackingMetadataRepairDisposition;

/** Recover stable released task ownership metadata before runtime hydration. */
export function repairLegacySubagentTaskBindings(
  db: DatabaseSync,
): LegacySubagentTaskBindingRepairDisposition {
  const disposition: LegacySubagentTaskBindingRepairDisposition = {
  ...
  return runSqliteImmediateTransactionSync(db, () => repairLegacySubagentBackingMetadata(db));
}
