...
function fixture(options: { pauseAcquisition?: boolean; firstCleanupError?: Error } = {}) {
...
describe("async lease maintenance ownership", () => {
  ...
  it("registers before acquisition so an external canonical close can drain the pending lease", async () => {
    ...
    try {
      ...
      const closing = observe(Promise.all([...f.registered].map((resource) => resource.close())));
      expect(observed.settled()).toBe(false);
      expect(closing.settled()).toBe(false);
