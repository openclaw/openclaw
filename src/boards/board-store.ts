...
export function createBoardWidgetPutSnapshot(
  ...
  layout = insertBoardWidget(
    layout,
    {
      ...
      grantState: declaredSummary ? "pending" : "none",
...
export class InMemoryBoardStore implements BoardStore {
  ...
  putWidget(params: BoardWidgetPutParams): BoardSnapshot {
    ...
    documents.set(params.name, createBoardWidgetDocument(params.content, widgetRevision));
  ...
  grant(sessionKey: string, name: string, decision: "granted" | "rejected"): BoardSnapshot {
    ...
    const snapshot = createBoardGrantSnapshot(current.snapshot, name, decision);
  ...
  deleteSession(sessionKey: string, _agentId?: string): void {
    this.boards.delete(sessionKey);
  }
}
