...
function renderLiveStatus(status: LiveStatusSnapshot): string[] {
  ...
  const lines = [
    ...
    `${colorize(rich, theme.muted, "Runtime loaded commit:")} ${status.runtime.loadedCommit?.slice(0, 7) ?? "unknown"}`,
    `${colorize(rich, theme.muted, "Runtime matches live commit:")} ${renderBooleanLabel(status.runtime.matchesLiveCommit)}`,
...
function renderLiveSyncStatus(status: LiveSyncStatus): string[] {
  ...
  const lines = [
    ...
    `${colorize(rich, theme.muted, "Runtime loaded commit:")} ${status.runtimeLoadedCommit?.slice(0, 7) ?? "unknown"}`,
