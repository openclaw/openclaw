...
const SANDBOX_EXAMPLES = {
  main: [
    ["EasyHub sandbox list", "List all sandbox containers."],
    ["EasyHub sandbox list --browser", "List only browser containers."],
    ["EasyHub sandbox recreate --all", "Recreate all containers."],
    ["EasyHub sandbox recreate --session main", "Recreate a specific session."],
    ["EasyHub sandbox recreate --agent mybot", "Recreate agent containers."],
    ["EasyHub sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["EasyHub sandbox list", "List all sandbox containers."],
    ["EasyHub sandbox list --browser", "List only browser containers."],
    ["EasyHub sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["EasyHub sandbox recreate --all", "Recreate all containers."],
    ["EasyHub sandbox recreate --session main", "Recreate a specific session."],
    ["EasyHub sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["EasyHub sandbox recreate --browser --all", "Recreate only browser containers."],
    ["EasyHub sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["EasyHub sandbox explain", "Show effective sandbox config."],
    ["EasyHub sandbox explain --session agent:main:main", "Explain a specific session."],
    ["EasyHub sandbox explain --agent work", "Explain an agent sandbox."],
    ["EasyHub sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.EasyHub.ai/cli/sandbox")}\n`,
    )
    .action(() => {
