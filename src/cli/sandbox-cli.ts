...
const SANDBOX_EXAMPLES = {
  main: [
    ["activi sandbox list", "List all sandbox containers."],
    ["activi sandbox list --browser", "List only browser containers."],
    ["activi sandbox recreate --all", "Recreate all containers."],
    ["activi sandbox recreate --session main", "Recreate a specific session."],
    ["activi sandbox recreate --agent mybot", "Recreate agent containers."],
    ["activi sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["activi sandbox list", "List all sandbox containers."],
    ["activi sandbox list --browser", "List only browser containers."],
    ["activi sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["activi sandbox recreate --all", "Recreate all containers."],
    ["activi sandbox recreate --session main", "Recreate a specific session."],
    ["activi sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["activi sandbox recreate --browser --all", "Recreate only browser containers."],
    ["activi sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["activi sandbox explain", "Show effective sandbox config."],
    ["activi sandbox explain --session agent:main:main", "Explain a specific session."],
    ["activi sandbox explain --agent work", "Explain an agent sandbox."],
    ["activi sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.activi.ai/cli/sandbox")}\n`,
    )
    .action(() => {
