...
describe("browser extension install", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "dna-ext-"));
  ...
  it("copies extension path to clipboard", async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "dna-ext-path-"));
    process.env.DNA_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.DNA_STATE_DIR;
      else process.env.DNA_STATE_DIR = prev;
    }
  });
});
