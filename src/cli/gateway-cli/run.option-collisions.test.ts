...
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/mullusi-control-ui" as string | null,
}));

const { runtimeErrors, defaultRuntime, resetRuntimeCapture } = createCliRuntimeCapture();

vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/mullusi-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/mullusi-control-ui";
  ...
  it("forwards parent-captured options to `gateway run` subcommand", async () => {
    ...
    expect(forceFreePortAndWait).toHaveBeenCalledWith(18790, expect.anything());
  ...
  it.each([
    ...
    delete process.env.MULLUSI_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.MULLUSI_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode even if loadConfig still says local", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `mullusi onboard --mode local` or `mullusi setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
