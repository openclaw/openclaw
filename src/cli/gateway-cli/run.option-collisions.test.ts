...
const startGatewayServer = vi.fn(async (..._args: unknown[]) => ({
  close: vi.fn(async () => {}),
}));
const setGatewayWsLogStyle = vi.fn((_style?: unknown) => {});
const setVerbose = vi.fn((_enabled?: unknown) => {});
const forceFreePortAndWait = vi.fn(async (..._args: unknown[]) => ({
  killed: [],
  waitedMs: 0,
