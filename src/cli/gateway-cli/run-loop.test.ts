...
describe("runGatewayLoop", () => {
  ...
  it("waits for active runtime-deps installs before restart close", async () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      // The PR moves markGatewayDraining() to AFTER the runtime-deps idle
      // wait + followup queue drain so queued followups aren't dropped while
      // draining — see run-loop.ts comment near markGatewayDraining(). The
      // pre-PR upstream order was draining-first, so the runtime-deps wait
      // is observed here while close() is still blocked.
