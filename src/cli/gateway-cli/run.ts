...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  if (!bind) {
    defaultRuntime.error(
      'Invalid --bind (use "loopback", "lan", "tailnet", "zerotier", "wireguard", "overlay", "auto", or "custom")',
    );
    defaultRuntime.exit(1);
    return;
  }
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--bind <mode>",
      'Bind mode ("loopback"|"lan"|"tailnet"|"zerotier"|"wireguard"|"overlay"|"auto"|"custom"). Defaults to config gateway.bind (or loopback).',
    )
    .option(
      "--token <token>",
