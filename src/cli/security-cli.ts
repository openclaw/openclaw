...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["evox security audit", "Run a local security audit."],
          ["evox security audit --deep", "Include best-effort live Gateway probe checks."],
          ["evox security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "evox security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["evox security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["evox security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.evox.sh/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("EVOX.sh security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("evox security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("evox security audit --fix")}`));
        if (!fixResult) {
