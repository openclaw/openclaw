...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mullusi security audit", "Run a local security audit."],
          ["mullusi security audit --deep", "Include best-effort live Gateway probe checks."],
          ["mullusi security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "mullusi security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["mullusi security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["mullusi security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.mullusi.com/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Mullusi security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("mullusi security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("mullusi security audit --fix")}`));
        if (!fixResult) {
