...
const STEP_LABELS: Record<string, string> = {
  ...
  "dna doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "dna";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "dna"]);
const CLI_NAME = resolveCliName();
const DNA_REPO_URL = "https://github.com/dna/dna.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "dna");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("dna@")) return trimmed.slice("dna@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.DNA_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", DNA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `DNA_GIT_DIR points at a non-git directory: ${params.dir}. Set DNA_GIT_DIR to an empty folder or a dna checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`DNA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("DNA update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating DNA..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this DNA install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("dna doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("dna gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g dna@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g dna@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("dna gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("dna doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("dna gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("dna gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `dna update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `DNA_GIT_DIR points at a non-git directory: ${gitDir}. Set DNA_GIT_DIR to an empty folder or a dna checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via DNA_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update DNA to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["dna update", "Update a source checkout (git)"],
        ["dna update --channel beta", "Switch to beta channel (git + npm)"],
        ["dna update --channel dev", "Switch to dev channel (git + npm)"],
        ["dna update --tag beta", "One-off update to a dist-tag or version"],
        ["dna update --no-restart", "Update without restarting the service"],
        ["dna update --json", "Output result as JSON"],
        ["dna update --yes", "Non-interactive (accept downgrade prompts)"],
        ["dna update wizard", "Interactive update wizard"],
        ["dna --update", "Shorthand for dna update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run dna update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.molt.bot/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["dna update status", "Show channel + version status."],
          ["dna update status --json", "JSON output."],
          ["dna update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
