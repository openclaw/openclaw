...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Activi and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["activi update", "Update a source checkout (git)"],
        ["activi update --channel beta", "Switch to beta channel (git + npm)"],
        ["activi update --channel dev", "Switch to dev channel (git + npm)"],
        ["activi update --tag beta", "One-off update to a dist-tag or version"],
        ["activi update --no-restart", "Update without restarting the service"],
        ["activi update --json", "Output result as JSON"],
        ["activi update --yes", "Non-interactive (accept downgrade prompts)"],
        ["activi update wizard", "Interactive update wizard"],
        ["activi --update", "Shorthand for activi update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run activi update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.activi.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["activi update status", "Show channel + version status."],
          ["activi update status --json", "JSON output."],
          ["activi update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.activi.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
