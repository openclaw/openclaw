...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update EVOX.sh and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["evox update", "Update a source checkout (git)"],
        ["evox update --channel beta", "Switch to beta channel (git + npm)"],
        ["evox update --channel dev", "Switch to dev channel (git + npm)"],
        ["evox update --tag beta", "One-off update to a dist-tag or version"],
        ["evox update --tag main", "One-off package install from GitHub main"],
        ["evox update --dry-run", "Preview actions without changing anything"],
        ["evox update --no-restart", "Update without restarting the service"],
        ["evox update --json", "Output result as JSON"],
        ["evox update --yes", "Non-interactive (accept downgrade prompts)"],
        ["evox update wizard", "Interactive update wizard"],
        ["evox --update", "Shorthand for evox update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run evox update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.evox.sh/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["evox update status", "Show channel + version status."],
          ["evox update status --json", "JSON output."],
          ["evox update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.evox.sh/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
