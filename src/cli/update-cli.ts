...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Mullusi and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["mullusi update", "Update a source checkout (git)"],
        ["mullusi update --channel beta", "Switch to beta channel (git + npm)"],
        ["mullusi update --channel dev", "Switch to dev channel (git + npm)"],
        ["mullusi update --tag beta", "One-off update to a dist-tag or version"],
        ["mullusi update --tag main", "One-off package install from GitHub main"],
        ["mullusi update --dry-run", "Preview actions without changing anything"],
        ["mullusi update --no-restart", "Update without restarting the service"],
        ["mullusi update --json", "Output result as JSON"],
        ["mullusi update --yes", "Non-interactive (accept downgrade prompts)"],
        ["mullusi update wizard", "Interactive update wizard"],
        ["mullusi --update", "Shorthand for mullusi update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run mullusi update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.mullusi.com/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mullusi update status", "Show channel + version status."],
          ["mullusi update status --json", "JSON output."],
          ["mullusi update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.mullusi.com/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
