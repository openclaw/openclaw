...
export const COMPLETION_SHELLS = ["zsh", "bash", "powershell", "fish"];
export function isCompletionShell(value) {
    return COMPLETION_SHELLS.includes(value);
}
export function resolveShellFromEnv(env = process.env) {
...
export function resolveCompletionCachePath(shell, binName) {
...
export async function completionCacheExists(shell, binName = "openclaw") {
...
export async function isCompletionInstalled(shell, binName = "openclaw") {
 ...
 */
export async function usesSlowDynamicCompletion(shell, binName = "openclaw") {
    const profilePath = getShellProfilePath(shell);
    if (!(await pathExists(profilePath))) {
        return false;
    }
...
export async function installCompletion(shell, yes, binName = "openclaw") {
    const home = process.env.HOME || os.homedir();
    let profilePath = "";
    let sourceLine = "";
