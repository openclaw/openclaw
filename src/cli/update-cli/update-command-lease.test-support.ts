...
export async function runUpdateLeaseChild(): Promise<void> {
  ...
  let postCoreResult: { path: string; contents: string } | undefined;
  try {
      ...
      const sdk = await import(
        pathToFileURL(
          path.join(
            ...
            "dist/extensions/node_modules/openclaw/plugin-sdk/demo.js",
          ),
        ).href
      );
    ...
    if (command === "update") {
      ...
      postCoreResult = { path: resultPath, contents: JSON.stringify(scenario.pluginUpdate) };
      return;
    }
    if (command === "doctor") {
    ...
    // The parent may terminate a writer as soon as its result commits. Finish
    // fixture-owned storage before publishing that handoff.
    if (postCoreResult) {
      await fs.writeFile(postCoreResult.path, postCoreResult.contents);
    }
  }
}
