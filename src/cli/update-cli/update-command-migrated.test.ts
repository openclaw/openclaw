...
it.each([
  ...
  "fences migrated candidate finalization (json=$json, legacy=$legacy, parentOwns=$parentOwns, retained=$retained, original=$original)",
  async ({ json, legacy, parentOwns, retained, original }) => {
