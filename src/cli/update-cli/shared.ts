...
const SMART_AGENT_NEO_REPO_URL = "https://github.com/betterbrand/smart-agent-neo.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "smart-agent-neo";
...
export function resolveGitInstallDir(): string {
  const override = process.env.SMART_AGENT_NEO_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveSmartAgentNeoPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SMART_AGENT_NEO_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SMART_AGENT_NEO_GIT_DIR points at a non-git directory: ${params.dir}. Set SMART_AGENT_NEO_GIT_DIR to an empty folder or an smart-agent-neo checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SMART_AGENT_NEO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "smart-agent-neo.mjs");
