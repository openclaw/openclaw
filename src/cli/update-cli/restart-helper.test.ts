...
describe("restart-helper", () => {
  ...
  async function prepareAndReadScript(env: Record<string, string>, gatewayPort = 18790) {
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18790) {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'mullusi-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses MULLUSI_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        MULLUSI_PROFILE: "default",
        MULLUSI_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.mullusi.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.mullusi.gateway'");
      expect(content).toContain("launchctl bootstrap 'gui/501'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses MULLUSI_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        MULLUSI_PROFILE: "default",
        MULLUSI_LAUNCHD_LABEL: "com.custom.mullusi",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.mullusi'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Mullusi Gateway"');
      expect(content).toContain('schtasks /Run /TN "Mullusi Gateway"');
    ...
    it("uses MULLUSI_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        MULLUSI_PROFILE: "default",
        MULLUSI_WINDOWS_TASK_NAME: "Mullusi Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "Mullusi Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "Mullusi Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        MULLUSI_PROFILE: "production",
      });
      expect(content).toContain("mullusi-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        MULLUSI_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.mullusi.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Mullusi Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        MULLUSI_LAUNCHD_LABEL: "ai.mullusi.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.mullusi.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
