...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "EVOX.sh Gateway"');
      expect(content).toContain('schtasks /Run /TN "EVOX.sh Gateway"');
    ...
    it("uses OPENCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "EVOX.sh Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "EVOX.sh Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "EVOX.sh Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
