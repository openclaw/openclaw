...
import { coerceSecretRef, isValidEnvSecretRefId, resolveSecretInputRef, } from "../config/types.secrets.js";
import { collectUnsupportedSecretRefPolicyIssues, validateConfigObjectRaw, } from "../config/validation.js";
import { SecretProviderSchema } from "../config/zod-schema.core.js";
import { danger, info, success } from "../globals.js";
import { writeRuntimeJson } from "../runtime.js";
import { defaultRuntime } from "../runtime.js";
import { formatExecSecretRefIdValidationMessage, isValidExecSecretRefId, isValidFileSecretRefId, isValidSecretProviderAlias, secretRefKey, validateExecSecretRefId, } from "../secrets/ref-contract.js";
import { resolveSecretRefValue } from "../secrets/resolve.js";
import { discoverConfigSecretTargets, resolveConfigSecretTargetByPath, } from "../secrets/target-registry.js";
import { normalizeOptionalString } from "../shared/string-coerce.js";
import { formatDocsLink } from "../terminal/links.js";
import { theme } from "../terminal/theme.js";
import { shortenHomePath } from "../utils.js";
import { formatCliCommand } from "./command-format.js";
import { hasBatchMode, hasProviderBuilderOptions, hasRefBuilderOptions, parseBatchSource, } from "./config-set-input.js";
...
function assertNonDestructiveReplacement(params) {
    ...
    if (isProtectedMapReplacementPath(params.path) && isPlainRecord(existing.value)) {
        ...
        if (removed.length > 0) {
            throw new Error(`Refusing to replace ${pathLabel}; it would remove existing entries: ${formatRemovedEntries(removed)}. Use --merge to merge object values or --replace to replace intentionally.`);
        }
    }
    if (isProtectedArrayReplacementPath(params.path)) {
        ...
        if (removed.length > 0) {
            throw new Error(`Refusing to replace ${pathLabel}; it would remove existing entries: ${formatRemovedEntries(removed)}. Use --merge to merge by id or --replace to replace intentionally.`);
        }
    }
}
...
function pruneInactiveGatewayAuthCredentials(params) {
    const touchedGatewayAuthMode = params.operations.some((operation) => pathEquals(operation.requestedPath, GATEWAY_AUTH_MODE_PATH));
...
function parseSecretRefBuilder(params) {
    ...
    if (!isValidSecretProviderAlias(provider)) {
        throw new Error(`${params.fieldPrefix}.provider must match /^[a-z][a-z0-9_-]{0,63}$/ (example: "default").`);
    }
    ...
    if (source === "file" && !isValidFileSecretRefId(id)) {
        throw new Error(`${params.fieldPrefix}.id must be an absolute JSON pointer (or "value" for singleValue mode).`);
    }
    if (source === "exec") {
...
function parseProviderAliasPath(path) {
    const expectedPrefixMatches = path.length === 3 &&
        path[0] === SECRET_PROVIDER_PATH_PREFIX[0] &&
        path[1] === SECRET_PROVIDER_PATH_PREFIX[1];
    if (!expectedPrefixMatches) {
        throw new Error('Provider builder mode requires path "secrets.providers.<alias>" (example: secrets.providers.vault).');
    }
    const alias = path[2] ?? "";
    if (!isValidSecretProviderAlias(alias)) {
        throw new Error(`Provider alias "${alias}" must match /^[a-z][a-z0-9_-]{0,63}$/ (example: "default").`);
    }
    return alias;
}
    ...
    if (source === "env") {
        ...
        for (const envName of allowlist) {
            if (!isValidEnvSecretRefId(envName)) {
                throw new Error(`--provider-allowlist entry "${envName}" must match /^[A-Z][A-Z0-9_]{0,127}$/.`);
            }
        }
        provider = {
            source: "env",
...
function buildSingleSetOperations(params) {
    ...
    if (modeResolution.mode === "ref_builder") {
        ...
        if (!params.opts.refProvider || !params.opts.refSource || !params.opts.refId) {
            throw modeError("ref builder mode requires --ref-provider <alias>, --ref-source <env|file|exec>, and --ref-id <id>.");
        }
...
function dedupeDryRunErrors(errors) {
    ...
    for (const error of errors) {
        const key = error.kind === "resolvability"
...
function formatDryRunFailureMessage(params) {
    ...
    if (skippedExecRefs > 0) {
        lines.push(`Dry run note: skipped ${skippedExecRefs} exec SecretRef resolvability check(s). Re-run with --allow-exec to execute exec providers during dry-run.`);
    }
    return lines.join("\n");
}
export async function runConfigSet(opts) {
    ...
    try {
        ...
        const policyIssueLines = formatConfigIssueLines(policyIssues, "", { normalizeRoot: true }).map((line) => line.trim());
        if (opts.cliOptions.dryRun) {
            ...
            const refs = hasJsonMode || hasBuilderMode
                ? collectDryRunRefs({
                    config: nextConfig,
                    operations,
                })
            ...
            if ((!hasJsonMode || !requiresFullSchemaValidation) && policyIssueLines.length > 0) {
                errors.push(...policyIssueLines.map((message) => ({
                    kind: "schema",
                    message,
                })));
            }
            if (requiresFullSchemaValidation) {
                errors.push(...collectDryRunSchemaErrors({
                    config: nextConfig,
                    operations,
                }));
            }
            ...
            if (opts.cliOptions.json) {
                ...
                if (!dryRunResult.checks.schema && !dryRunResult.checks.resolvability) {
                    runtime.log(info("Dry run note: value mode does not run schema/resolvability checks. Use --strict-json, builder flags, or batch mode to enable validation checks."));
                }
                if (dryRunResult.skippedExecRefs > 0) {
                    runtime.log(info(`Dry run note: skipped ${dryRunResult.skippedExecRefs} exec SecretRef resolvability check(s). Re-run with --allow-exec to execute exec providers during dry-run.`));
                }
        ...
        if (removedGatewayAuthPaths.length > 0) {
            runtime.log(info(`Removed inactive ${removedGatewayAuthPaths.join(", ")} for gateway.auth.mode=${nextConfig.gateway?.auth?.mode ?? "<unset>"}.`));
        }
...
export async function runConfigFile(opts) {
    const runtime = opts.runtime ?? defaultRuntime;
    try {
        const snapshot = await readConfigFileSnapshot();
...
export async function runConfigSchema(opts = {}) {
    const runtime = opts.runtime ?? defaultRuntime;
    try {
...
export function registerConfigCli(program) {
    const cmd = program
        .command("config")
        .description("Non-interactive config helpers (get/set/unset/file/schema/validate). Run without subcommand for guided setup.")
        .addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`)
        .option("--section <section>", "Configuration sections for guided setup (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], [])
        .action(async (opts) => {
    ...
    setCommandJsonMode(cmd.command("set"), "parse-only")
        ...
        .option("--dry-run", "Validate changes without writing openclaw.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)", false)
        .option("--allow-exec", "Dry-run only: allow exec SecretRef resolvability checks (may execute provider commands)", false)
        .option("--merge", "Merge object/map values instead of replacing the target path", false)
        .option("--replace", "Allow full replacement of protected map/list paths such as agents.defaults.models", false)
        ...
        .option("--provider-allowlist <envVar>", "Provider builder (env): allowlist entry (repeatable)", (value, previous) => [...previous, value], [])
        ...
        .option("--provider-arg <arg>", "Provider builder (exec): command arg (repeatable)", (value, previous) => [...previous, value], [])
        ...
        .option("--provider-env <key=value>", "Provider builder (exec): env assignment (repeatable)", (value, previous) => [...previous, value], [])
        .option("--provider-pass-env <envVar>", "Provider builder (exec): pass host env var (repeatable)", (value, previous) => [...previous, value], [])
        .option("--provider-trusted-dir <path>", "Provider builder (exec): trusted directory (repeatable)", (value, previous) => [...previous, value], [])
        .option("--provider-allow-insecure-path", "Provider builder (file|exec): bypass strict path permission checks", false)
        .option("--provider-allow-symlink-command", "Provider builder (exec): allow command symlink path", false)
