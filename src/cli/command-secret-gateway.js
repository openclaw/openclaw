...
function collectConfiguredTargetRefPaths(params) {
    ...
    const configuredTargetRefPaths = new Set();
...
function classifyConfiguredTargetRefs(params) {
    ...
    const inactiveWarningsByPath = new Map();
...
function isUnsupportedSecretsResolveError(err) {
    ...
    return (message.includes("does not support required method") ||
...
async function resolveTargetSecretLocally(params) {
    ...
    try {
        ...
        assertExpectedResolvedSecretValue({
            ...
            errorMessage: params.target.entry.expectedResolvedValue === "string"
        ...
        if (!enforcesResolvedSecrets(params.mode)) {
            params.localResolutionDiagnostics.push(`${params.commandName}: failed to resolve ${params.target.path} locally (${formatErrorMessage(error)}).`);
        }
    }
}
export async function resolveCommandSecretRefsViaGateway(params) {
    ...
    try {
        ...
        try {
            ...
            const fallbackMessage = recoveredLocally && !fallback.hadUnresolvedTargets
        ...
        if (isUnsupportedSecretsResolveError(err)) {
            throw new Error(`${params.commandName}: active gateway does not support secrets.resolve (${formatErrorMessage(err)}). Update the gateway or run without SecretRefs.`, { cause: err });
        }
    ...
    const inactiveRefPaths = parsed.inactiveRefPaths.length > 0
    ...
    if (analyzed.unresolved.length > 0) {
        try {
            ...
            const recoveredPaths = new Set(Object.entries(localFallback.targetStatesByPath)
            ...
            if (stillUnresolved.length > 0) {
                if (enforcesResolvedSecrets(mode)) {
                    throw new Error(`${params.commandName}: ${stillUnresolved[0]?.path ?? "target"} is unresolved in the active runtime snapshot.`);
                }
                ...
                diagnostics = dedupeDiagnostics([
                    ...diagnostics,
                    `${params.commandName}: resolved ${recoveredPaths.size} secret ${recoveredPaths.size === 1 ? "path" : "paths"} locally after the gateway snapshot was incomplete.`,
                ]);
            }
        }
        catch (error) {
