...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(resolveEasyHubPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("EasyHub update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-update-"));
    try {
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "EasyHub", version: "1.0.0" }),
        "utf-8",
      );
      ...
      vi.mocked(resolveEasyHubPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("requires confirmation on downgrade when non-interactive", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "EasyHub", version: "2.0.0" }),
        "utf-8",
      );
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-update-wizard-"));
    const previousGitDir = process.env.EASYHUB_GIT_DIR;
    try {
      setTty(true);
      process.env.EASYHUB_GIT_DIR = tempDir;
      ...
      process.env.EASYHUB_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
