...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveDNAPackageRoot } = await import("../infra/dna-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveDNAPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-update-"));
    try {
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "dna", version: "1.0.0" }),
        "utf-8",
      );
      ...
      vi.mocked(resolveDNAPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("requires confirmation on downgrade when non-interactive", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "dna", version: "2.0.0" }),
        "utf-8",
      );
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-update-wizard-"));
    const previousGitDir = process.env.DNA_GIT_DIR;
    try {
      setTty(true);
      process.env.DNA_GIT_DIR = tempDir;
      ...
      process.env.DNA_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
