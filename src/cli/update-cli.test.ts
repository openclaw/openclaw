...
describe("update-cli", () => {
  ...
  it("updateCommand prefers dist/index.js over dist/entry.js for service refresh (#46621)", async () => {
    const root = createCaseDir("openclaw-updated-root");
    const indexPath = path.join(root, "dist", "index.js");
    pathExists.mockImplementation(async (candidate: string) => candidate === indexPath);
  ...
  it("updateCommand picks dist/index.js when both index.js and entry.js exist (#46621)", async () => {
    const root = createCaseDir("openclaw-updated-root");
    const indexPath = path.join(root, "dist", "index.js");
    const entryPath = path.join(root, "dist", "entry.js");
    pathExists.mockImplementation(
    ...
    // Should use index.js (checked first) even though entry.js also exists
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [expect.stringMatching(/node/), indexPath, "gateway", "install", "--force"],
      expect.objectContaining({ cwd: root, timeoutMs: 60_000 }),
    );
