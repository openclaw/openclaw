    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["activi memory status", "Show index and provider status."],
          ["activi memory index --force", "Force a full reindex."],
          ['activi memory search --query "deployment notes"', "Search indexed memory entries."],
          ["activi memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.activi.ai/cli/memory")}\n`,
    );
