...
describe("native hook relay CLI", () => {
  ...
  it("keeps loop-detection-only PreToolUse unavailable handling observational", async () => {
    const callGateway = vi.fn(async () => {
      throw new Error("gateway closed");
    });
    const stdout = createWritableTextBuffer();
  ...
  it("keeps skill workshop lifecycle approvals fail-closed when loop-detection-only relay is unavailable", async () => {
    const callGateway = vi.fn(async () => {
      throw new Error("gateway closed");
    });
    const stdout = createWritableTextBuffer();
  ...
  it("honors auto-approved skill workshop lifecycle actions when loop-detection-only relay is unavailable", async () => {
    const callGateway = vi.fn(async () => {
      throw new Error("gateway closed");
    });
    const stdout = createWritableTextBuffer();
    ...
    const exitCode = await runNativeHookRelayCli(
      {
        ...
        event: "pre_tool_use",
        preToolUseUnavailable: "loop-detection-only",
        skillWorkshopApprovalPolicy: "auto",
      },
      {
        stdin: createReadableTextStream(
          JSON.stringify({
