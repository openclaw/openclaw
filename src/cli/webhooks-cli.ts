...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.smart-agent-neo.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SmartAgentNeo hooks")
    ...
    .option("--hook-url <url>", "SmartAgentNeo hook URL")
    .option("--hook-token <token>", "SmartAgentNeo hook token")
