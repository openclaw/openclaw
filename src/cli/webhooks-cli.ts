...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.mullusi.com/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Mullusi hooks")
    ...
    .option("--hook-url <url>", "Mullusi hook URL")
    .option("--hook-token <token>", "Mullusi hook token")
