...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.evox.sh/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + EVOX.sh hooks")
    ...
    .option("--hook-url <url>", "EVOX.sh hook URL")
    .option("--hook-token <token>", "EVOX.sh hook token")
