...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "EasyHub-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      EASYHUB_NO_RESPAWN: "1",
      EASYHUB_STATE_DIR: stateDir,
      EASYHUB_SKIP_CHANNELS: "1",
      EASYHUB_SKIP_GMAIL_WATCHER: "1",
      EASYHUB_SKIP_CRON: "1",
      EASYHUB_SKIP_BROWSER_CONTROL_SERVER: "1",
      EASYHUB_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "EasyHub-entry-bootstrap.mjs");
