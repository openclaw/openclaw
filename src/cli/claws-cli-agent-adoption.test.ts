...
describe("claws configured-agent adoption CLI", () => {
  ...
  it("adopts an agent added on disk before the runtime snapshot converges", async () => {
    const { root, workspace } = await cliTestHelpers.writePackageFixture(tempDirs);
    await mkdir(workspace);
    await writeFile(join(workspace, "AGENTS.md"), "# Demo\n", "utf8");
    mocks.loadConfig.mockReturnValue({});
  ...
  it("protects the workspace still served by a stale runtime agent", async () => {
    const { root } = await cliTestHelpers.writePackageFixture(tempDirs);
    const oldWorkspace = tempDirs.make("openclaw-claws-runtime-workspace-");
    const newWorkspace = tempDirs.make("openclaw-claws-source-workspace-");
    await writeFile(join(oldWorkspace, "AGENTS.md"), "# Demo\n", "utf8");
    mocks.loadConfig.mockReturnValue({
      agents: { entries: { incumbent: { workspace: oldWorkspace } } },
    });
    mocks.readConfigFileSnapshot.mockResolvedValue({
      sourceConfig: {
        agents: { entries: { incumbent: { workspace: newWorkspace } } },
      },
    });
  ...
  it("resumes a created agent without colliding with its own committed workspace", async () => {
    const { root, workspace } = await cliTestHelpers.writePackageFixture(tempDirs);
    vi.stubEnv(
      "OPENCLAW_STATE_DIR",
      join(tempDirs.make("openclaw-claws-created-resume-"), "state"),
    );
    mocks.loadConfig.mockReturnValue({});
    await runAdd(root, { dryRun: true, workspace, json: true });
    const originalPlan = JSON.parse(mocks.logs[0] ?? "{}");
    persistClawInstallRecord(originalPlan, { status: "config_committed", nowMs: 1 });
    const { id: _id, ...committedAgent } = originalPlan.agent.config;
    mocks.loadConfig.mockReturnValue({
      agents: { entries: { "demo-agent": committedAgent } },
    });
    mocks.logs.length = 0;
    mocks.runtime.exit.mockClear();
    mocks.applyClawAddPlan.mockClear();
    ...
    expect(mocks.applyClawAddPlan).toHaveBeenCalledWith(
      expect.objectContaining({ planIntegrity: originalPlan.planIntegrity, blockers: [] }),
      expect.objectContaining({
        resumeRecord: expect.objectContaining({ status: "config_committed" }),
      }),
    );
