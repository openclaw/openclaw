...
import type { ConfigFileSnapshot, SmartAgentNeoConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `smart-agent-neo config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<[], Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<[SmartAgentNeoConfig], Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: SmartAgentNeoConfig) => mockWriteConfigFile(cfg),
}));
...
function setSnapshot(resolved: SmartAgentNeoConfig, config: SmartAgentNeoConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: SmartAgentNeoConfig = {
        agents: {
      ...
      const runtimeMerged: SmartAgentNeoConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
