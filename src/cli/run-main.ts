...
const MABOS_OWNED_CORE_CONFLICTS = new Set(["onboard", "agents", "dashboard"]);

export function shouldDeferCorePrimaryToMabosPlugin(params: {
  primary: string | null;
  ...
  if (!isMabosProduct(params.env)) {
    return false;
  }
  return MABOS_OWNED_CORE_CONFLICTS.has(params.primary);
}
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const deferCorePrimaryToMabos = shouldDeferCorePrimaryToMabosPlugin({
    primary,
    env: process.env,
  });
  if (primary) {
    ...
    if (ctx && !deferCorePrimaryToMabos) {
