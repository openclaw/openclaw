...
${formatHelpExamples([
  ['evox agent --to +15555550123 --message "status update"', "Start a new session."],
  ['evox agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['evox agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'evox agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.evox.sh/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.evox.sh/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['evox agents set-identity --agent main --name "EVOX.sh" --emoji "🦞"', "Set name + emoji."],
  ["evox agents set-identity --agent main --avatar avatars/openclaw.png", "Set avatar path."],
  [
    "evox agents set-identity --workspace ~/.openclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "evox agents set-identity --identity-file ~/.openclaw/workspace/IDENTITY.md --agent main",
