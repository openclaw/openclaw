...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mullusi status", "Show channel health + session summary."],
          ["mullusi status --all", "Full diagnosis (read-only)."],
          ["mullusi status --json", "Machine-readable output."],
          ["mullusi status --usage", "Show model provider usage/quota snapshots."],
          [
            "mullusi status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["mullusi status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.mullusi.com/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.mullusi.com/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mullusi sessions", "List all sessions."],
          ["mullusi sessions --agent work", "List sessions for one agent."],
          ["mullusi sessions --all-agents", "Aggregate sessions across agents."],
          ["mullusi sessions --active 120", "Only last 2 hours."],
          ["mullusi sessions --json", "Machine-readable output."],
          ["mullusi sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.mullusi.com/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mullusi sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "mullusi sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["mullusi sessions cleanup --enforce", "Apply maintenance now."],
          ["mullusi sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["mullusi sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "mullusi sessions cleanup --enforce --store ./tmp/sessions.json",
