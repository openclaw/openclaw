...
function printStatus(statuses: ProviderLimitsStatus[]): void {
  ...
  for (const status of statuses) {
    ...
    if (dailyBudget) {
      const pct =
        dailyBudget.limit > 0 ? Math.round((dailyBudget.used / dailyBudget.limit) * 100) : 0;
      console.log(
        `    Daily tokens: ${dailyBudget.used.toLocaleString()} / ${dailyBudget.limit.toLocaleString()} (${pct}%)`,
      );
    } else {
    ...
    if (monthlyBudget) {
      const pct =
        monthlyBudget.limit > 0 ? Math.round((monthlyBudget.used / monthlyBudget.limit) * 100) : 0;
      console.log(
        `    Monthly tokens: ${monthlyBudget.used.toLocaleString()} / ${monthlyBudget.limit.toLocaleString()} (${pct}%)`,
      );
    } else {
...
export function registerLimitsCli(program: Command): void {
  const limits = program.command("limits").description("Rate limiting & cost controls");
  ...
  limits
    ...
    .action(async (provider?: string) => {
      try {
        ...
        console.error("Failed to reset rate limits:", err instanceof Error ? err.message : err);
        process.exitCode = 1;
      }
    });
}
