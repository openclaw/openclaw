    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["evox nodes status", "List known nodes with live status."],
          ["evox nodes pairing pending", "Show pending node pairing requests."],
          ['evox nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["evox nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.evox.sh/cli/nodes")}\n`,
    );
