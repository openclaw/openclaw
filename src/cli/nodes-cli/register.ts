    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["activi nodes status", "List known nodes with live status."],
          ["activi nodes pairing pending", "Show pending node pairing requests."],
          ['activi nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["activi nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.activi.ai/cli/nodes")}\n`,
    );
