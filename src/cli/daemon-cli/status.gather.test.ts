...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    MULLUSI_STATE_DIR: "/tmp/mullusi-daemon",
    MULLUSI_CONFIG_PATH: "/tmp/mullusi-daemon/mullusi.json",
  },
}));
...
const resolveGatewayPort = vi.fn((_cfg?: unknown, _env?: unknown) => 18790);
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.MULLUSI_STATE_DIR ?? "/tmp/mullusi-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.MULLUSI_CONFIG_PATH ?? `${stateDir}/mullusi.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/mullusi-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.MULLUSI_STATE_DIR = "/tmp/mullusi-cli";
    process.env.MULLUSI_CONFIG_PATH = "/tmp/mullusi-cli/mullusi.json";
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/mullusi-cli/mullusi.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.MULLUSI_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.MULLUSI_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.MULLUSI_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
