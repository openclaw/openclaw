...
import { DEFAULT_RESTART_HEALTH_ATTEMPTS, DEFAULT_RESTART_HEALTH_DELAY_MS, renderGatewayPortHealthDiagnostics, renderRestartDiagnostics, terminateStaleGatewayPids, waitForGatewayHealthyListener, waitForGatewayHealthyRestart, } from "./restart-health.js";
...
async function assertUnmanagedGatewayRestartEnabled(port) {
    ...
    if (!isRestartEnabled(probe.configSnapshot)) {
        throw new Error("Gateway restart is disabled in the running gateway config (commands.restart=false); unmanaged SIGUSR1 restart would be ignored");
    }
}
...
async function restartGatewayWithoutServiceManager(port) {
    ...
    if (pids.length > 1) {
        throw new Error(`multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "openclaw gateway status --deep" before retrying restart`);
    }
...
export async function runDaemonStart(opts = {}) {
    return await runServiceStart({
        ...
        onNotLoaded: process.platform === "darwin"
