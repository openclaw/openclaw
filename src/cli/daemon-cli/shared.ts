...
const SAFE_DAEMON_ENV_KEYS = [
  "SMART_AGENT_NEO_PROFILE",
  "SMART_AGENT_NEO_STATE_DIR",
  "SMART_AGENT_NEO_CONFIG_PATH",
  "SMART_AGENT_NEO_GATEWAY_PORT",
  "SMART_AGENT_NEO_NIX_MODE",
];
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("smart-agent-neo gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const unit = resolveGatewaySystemdServiceName(env.SMART_AGENT_NEO_PROFILE);
      hints.push(`Logs: journalctl --user -u ${unit}.service -n 200 --no-pager`);
    } else if (process.platform === "win32") {
      const task = resolveGatewayWindowsTaskName(env.SMART_AGENT_NEO_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("smart-agent-neo gateway install", env),
    formatCliCommand("smart-agent-neo gateway", env),
  ];
  const profile = env.SMART_AGENT_NEO_PROFILE;
