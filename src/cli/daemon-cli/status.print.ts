...
export function printDaemonStatus(
  ...
  try {
    ...
    defaultRuntime.log(
      `${label("File logs:")} ${infoText(shortenHomePath(logFile))}`,
    );
  } catch {
  ...
  if (status.config) {
    ...
    if (status.config.daemon) {
      ...
      defaultRuntime.log(
        `${label("Config (service):")} ${infoText(daemonCfg)}`,
      );
  ...
  if (service.runtime?.cachedLabel) {
    ...
    defaultRuntime.error(
      errorText(
        `Then reinstall: ${formatCliCommand("openclaw gateway install")}`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      ...
      defaultRuntime.error(
        `${errorText("Logs:")} ${shortenHomePath(logs.stdoutPath)}`,
      );
      defaultRuntime.error(
        `${errorText("Errors:")} ${shortenHomePath(logs.stderrPath)}`,
      );
    }
    spacer();
  }
  ...
  defaultRuntime.log(
    `${label("Troubles:")} run ${formatCliCommand("openclaw status")}`,
  );
  defaultRuntime.log(
    `${label("Troubleshooting:")} https://docs.openclaw.ai/troubleshooting`,
  );
}
