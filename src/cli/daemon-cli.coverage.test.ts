...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    EASYHUB_STATE_DIR: process.env.EASYHUB_STATE_DIR,
    EASYHUB_CONFIG_PATH: process.env.EASYHUB_CONFIG_PATH,
    EASYHUB_GATEWAY_PORT: process.env.EASYHUB_GATEWAY_PORT,
    EASYHUB_PROFILE: process.env.EASYHUB_PROFILE,
  };

  beforeEach(() => {
    process.env.EASYHUB_STATE_DIR = "/tmp/EasyHub-cli-state";
    process.env.EASYHUB_CONFIG_PATH = "/tmp/EasyHub-cli-state/easyhub.json";
    delete process.env.EASYHUB_GATEWAY_PORT;
    delete process.env.EASYHUB_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.EASYHUB_STATE_DIR !== undefined) {
      process.env.EASYHUB_STATE_DIR = originalEnv.EASYHUB_STATE_DIR;
    } else {
      delete process.env.EASYHUB_STATE_DIR;
    }

    if (originalEnv.EASYHUB_CONFIG_PATH !== undefined) {
      process.env.EASYHUB_CONFIG_PATH = originalEnv.EASYHUB_CONFIG_PATH;
    } else {
      delete process.env.EASYHUB_CONFIG_PATH;
    }

    if (originalEnv.EASYHUB_GATEWAY_PORT !== undefined) {
      process.env.EASYHUB_GATEWAY_PORT = originalEnv.EASYHUB_GATEWAY_PORT;
    } else {
      delete process.env.EASYHUB_GATEWAY_PORT;
    }

    if (originalEnv.EASYHUB_PROFILE !== undefined) {
      process.env.EASYHUB_PROFILE = originalEnv.EASYHUB_PROFILE;
    } else {
      delete process.env.EASYHUB_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        EASYHUB_PROFILE: "dev",
        EASYHUB_STATE_DIR: "/tmp/EasyHub-daemon-state",
        EASYHUB_CONFIG_PATH: "/tmp/EasyHub-daemon-state/easyhub.json",
        EASYHUB_GATEWAY_PORT: "19001",
      },
