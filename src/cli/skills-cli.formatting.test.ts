...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["ACTIVI_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "activi-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "activi-bundled-skills-test-"));
    process.env.ACTIVI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
