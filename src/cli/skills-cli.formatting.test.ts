...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["MULLUSI_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "mullusi-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "mullusi-bundled-skills-test-"));
    process.env.MULLUSI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
