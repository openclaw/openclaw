...
import { readMcpClientTlsFile } from "../agents/mcp-client-tls.js";
import {
  ...
  if (!disabled) {
    ...
    if (resolved?.kind === "http") {
      ...
      for (const [label, filePath] of [
        ["client certificate", resolved.clientCert],
        ["client key", resolved.clientKey],
      ] as const) {
        if (!filePath) {
          continue;
        }
        try {
          await readMcpClientTlsFile(filePath);
        } catch (error) {
          issues.push(issue("error", `${label} file cannot be read: ${formatErrorMessage(error)}`));
        }
      }
    }
  }
