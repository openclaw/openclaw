...
export function registerCronAddCommand(cron: Command) {
  addGatewayClientOptions(
    cron
      ...
      .option(
        "--announce",
        "Announce summary to a chat (subagent-style)",
        false,
      )
      .option(
        "--deliver",
      ...
      .action(
        async (
          ...
          try {
            const staggerRaw =
              typeof opts.stagger === "string" ? opts.stagger.trim() : "";
            ...
            const wakeModeRaw =
              typeof opts.wake === "string" ? opts.wake : "now";
            ...
            const deliveryFlagCount = [hasAnnounce, hasNoDeliver].filter(
              Boolean,
            ).length;
            ...
            const inferredSessionTarget =
              payload.kind === "agentTurn" ? "isolated" : "main";
            const sessionTarget =
              sessionSource === "cli"
                ? sessionTargetRaw || ""
            ...
            if (
              ...
              throw new Error(
                "--account requires an isolated agentTurn job with delivery.",
              );
            }
