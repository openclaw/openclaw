...
export function registerCronAddCommand(cron: Command) {
  addGatewayClientOptions(
    cron
      ...
      .option("--command <command>", "Shell command payload (isolated/current/session jobs)")
      .option(
        "--arg <value>",
      ...
      .action(async (opts: GatewayRpcOpts & Record<string, unknown>, cmd?: Command) => {
        try {
          ...
          const payload = (() => {
            ...
            const commandText = normalizeOptionalString(opts.command) ?? "";
            const chosen = [Boolean(systemEvent), Boolean(message), Boolean(commandText)].filter(
              Boolean,
            ).length;
            if (chosen !== 1) {
              throw new Error(
            ...
            if (commandText) {
              const args = Array.isArray(opts.arg)
                ? opts.arg.map((value) => String(value).trim()).filter(Boolean)
                : [];
              if (opts.model || opts.thinking || opts.lightContext || opts.tools) {
                throw new Error(
                  "--model, --thinking, --light-context, and --tools only apply to --message jobs",
                );
              }
              return {
          ...
          if (isIsolatedLikeSessionTarget && payload.kind === "systemEvent") {
            throw new Error(
              "Isolated/current/custom-session jobs require --message or --command (non-systemEvent).",
            );
          }
          if (
            (opts.announce || typeof opts.deliver === "boolean") &&
            (!isIsolatedLikeSessionTarget || payload.kind === "systemEvent")
          ) {
            throw new Error(
              "--announce/--no-deliver require an isolated/current/session non-systemEvent job.",
            );
          }

          const accountId = normalizeOptionalString(opts.account);

          if (accountId && (!isIsolatedLikeSessionTarget || payload.kind === "systemEvent")) {
            throw new Error("--account requires a non-main, non-systemEvent job with delivery.");
          }
