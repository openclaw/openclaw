...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# EVOX.sh Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(evox completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# EVOX.sh Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(evox completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.evox.sh/cli/completion")}\n`,
    )
    .addOption(
...
export async function installCompletion(shell: string, yes: boolean, binName = "evox") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
