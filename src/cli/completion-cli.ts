...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Mullusi Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(mullusi completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Mullusi Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(mullusi completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.mullusi.com/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $MULLUSI_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "mullusi") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
