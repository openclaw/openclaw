...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("dna node install"), formatCliCommand("dna node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("dna node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    DNA_LOG_PREFIX: baseEnv.DNA_LOG_PREFIX ?? "node",
