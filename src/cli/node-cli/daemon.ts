...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("EasyHub node install"), formatCliCommand("EasyHub node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("EasyHub node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    EASYHUB_LOG_PREFIX: baseEnv.EASYHUB_LOG_PREFIX ?? "node",
