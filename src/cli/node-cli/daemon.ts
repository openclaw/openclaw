...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("activi node install"), formatCliCommand("activi node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("activi node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ACTIVI_LOG_PREFIX: baseEnv.ACTIVI_LOG_PREFIX ?? "node",
