    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "activi node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["activi node status", "Check node host service status."],
          ["activi node install", "Install the node host service."],
          ["activi node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.activi.ai/cli/node")}\n`,
    );
