...
function normalizeSessionIdMatches(matches, normalizedSessionId) {
    return matches.map(([sessionKey, entry]) => {
        ...
        return {
            ...
            isStructural: normalizedSessionKey.endsWith(`:${normalizedSessionId}`) ||
...
export function resolveSessionIdMatchSelection(matches, sessionId) {
    if (matches.length === 0) {
        return { kind: "none" };
    }
...
export function resolvePreferredSessionKeyForSessionIdMatches(matches, sessionId) {
