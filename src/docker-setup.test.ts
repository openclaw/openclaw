...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "smart-agent-neo-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    SMART_AGENT_NEO_GATEWAY_TOKEN: "test-token",
    SMART_AGENT_NEO_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SMART_AGENT_NEO_WORKSPACE_DIR: join(sandbox.rootDir, "smart-agent-neo"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        SMART_AGENT_NEO_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        SMART_AGENT_NEO_EXTRA_MOUNTS: undefined,
        SMART_AGENT_NEO_HOME_VOLUME: "smart-agent-neo-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SMART_AGENT_NEO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("SMART_AGENT_NEO_EXTRA_MOUNTS=");
    expect(envFile).toContain("SMART_AGENT_NEO_HOME_VOLUME=smart-agent-neo-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("smart-agent-neo-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("smart-agent-neo-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SMART_AGENT_NEO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
