...
describe("extractLinksFromMessage", () => {
  ...
  it("blocks localhost and common loopback addresses", () => {
    ...
    expect(
      extractLinksFromMessage("http://localhost.localdomain/secret"),
    ).toEqual([]);
  ...
  it("blocks link-local and cloud metadata addresses", () => {
    expect(
      extractLinksFromMessage("http://169.254.169.254/latest/meta-data/"),
    ).toEqual([]);
  ...
  it("blocks private and mapped IPv6 addresses", () => {
    ...
    expect(
      extractLinksFromMessage("http://[2001:db8:1234::5efe:127.0.0.1]/secret"),
    ).toEqual([]);
  ...
  it("allows legitimate public URLs", () => {
    ...
    expect(extractLinksFromMessage("https://8.8.8.8/dns")).toEqual([
      "https://8.8.8.8/dns",
    ]);
  });
});
