...
function formatAttachmentSummaryPlaceholder(contentTypes: Array<string | undefined>): string {
  ...
  for (const contentType of contentTypes) {
    const kind = mediaKindFromMime(contentType) ?? "attachment";
...
export function createSignalEventHandler(deps: SignalEventHandlerDeps) {
  ...
  return async (event: { event?: string; data?: string }) => {
    ...
    } else if (attachmentContentTypes.length > 1) {
      placeholder = formatAttachmentSummaryPlaceholder(attachmentContentTypes);
    } else if (kind) {
