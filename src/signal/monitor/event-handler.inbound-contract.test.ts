...
const { dispatchInboundMessageMock } = vi.hoisted(() => {
  ...
  return { dispatchInboundMessageMock };
});
...
beforeEach(() => {
  ...
  dispatchInboundMessageMock.mockImplementation(async (params: { ctx: MsgContext }) => {
...
describe("signal createSignalEventHandler inbound contract", () => {
  ...
  it("forwards all fetched attachments via MediaPaths/MediaTypes", async () => {
    ...
    expect(capturedCtx).toBeTruthy();
    expect(capturedCtx?.MediaPath).toBe("/tmp/a1.dat");
    expect(capturedCtx?.MediaType).toBe("image/jpeg");
    expect(capturedCtx?.MediaPaths).toEqual(["/tmp/a1.dat", "/tmp/a2.dat"]);
    expect(capturedCtx?.MediaUrls).toEqual(["/tmp/a1.dat", "/tmp/a2.dat"]);
    expect(capturedCtx?.MediaTypes).toEqual(["image/jpeg", "application/octet-stream"]);
  });
