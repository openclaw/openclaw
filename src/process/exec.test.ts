...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        MULLUSI_BASE_ENV: "base",
        MULLUSI_TO_REMOVE: undefined,
      },
      env: {
        MULLUSI_TEST_ENV: "ok",
      },
    ...
    expect(resolved.MULLUSI_BASE_ENV).toBe("base");
    expect(resolved.MULLUSI_TEST_ENV).toBe("ok");
    expect(resolved.MULLUSI_TO_REMOVE).toBeUndefined();
    expect(resolved.MULLUSI_CLI).toBe(MULLUSI_CLI_ENV_VALUE);
  });
