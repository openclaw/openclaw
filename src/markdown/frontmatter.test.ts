...
`;
    ...
    expect(parsed.EasyHub?.emoji).toBe("disk");
  });

  it("preserves inline JSON values", () => {
    const content = `---
name: inline-json
metadata: {"EasyHub": {"events": ["test"]}}
---
`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"EasyHub": {"events": ["test"]}}');
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "");
    expect(parsed.EasyHub?.events).toEqual(["command:new"]);
  });
