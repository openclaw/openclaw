...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { dna?: { emoji?: string } };
    expect(parsed.dna?.emoji).toBe("disk");
  });

  it("preserves inline JSON values", () => {
    const content = `---
name: inline-json
metadata: {"dna": {"events": ["test"]}}
---
`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"dna": {"events": ["test"]}}');
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { dna?: { events?: string[] } };
    expect(parsed.dna?.events).toEqual(["command:new"]);
  });
