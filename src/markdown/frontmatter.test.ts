...
`;
    ...
    expect(parsed.mullusi?.emoji).toBe("disk");
  });

  it("preserves inline JSON values", () => {
    const content = `---
name: inline-json
metadata: {"mullusi": {"events": ["test"]}}
---
`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"mullusi": {"events": ["test"]}}');
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "");
    expect(parsed.mullusi?.events).toEqual(["command:new"]);
  });
...
---`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"mullusi":true}');
  });
