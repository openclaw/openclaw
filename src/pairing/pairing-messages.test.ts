...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MULLUSI_CONTAINER_HINT", "MULLUSI_PROFILE"]);
    delete process.env.MULLUSI_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:mullusi|mullusi) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
