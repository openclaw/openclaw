...
function resolveGatewayPort(cfg: SmartAgentNeoConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: SmartAgentNeoConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    env.SMART_AGENT_NEO_GATEWAY_TOKEN?.trim() ||
    env.NEOBOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    env.SMART_AGENT_NEO_GATEWAY_PASSWORD?.trim() ||
    env.NEOBOT_GATEWAY_PASSWORD?.trim() ||
