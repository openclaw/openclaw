// Anthropic Microsoft Foundry bearer constructor guard-specific proof:
// SSRF-blocks a private IP before the SDK's default global fetch is ever
// reached, then streams Anthropic SSE end-to-end through
// buildGuardedModelFetch against a real local HTTP server.
//
...
const SSRF_BLOCKED_FOUNDRY_MODEL = {
...
describe("Anthropic Microsoft Foundry guard-specific SSRF blocking proof", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });
...
describe("Anthropic Microsoft Foundry guard real wire loopback proof (http.createServer)", () => {
  it("streams Anthropic SSE end-to-end through buildGuardedModelFetch against a real loopback server", async () => {
    ...
    const sseEvents: Array<Record<string, unknown>> = [
      ...
      },
      {
        ...
        index: 0,
        delta: { type: "text_delta", text: "Hello from local Foundry Anthropic loopback." },
      },
      {
    ...
    try {
      ...
      // Foundry bearer constructor sets apiKey:null and uses authToken, which
      // the SDK sends as `Authorization: Bearer <token>`.
