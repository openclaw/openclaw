...
function createBackendEntry(params: {
	...
	return {
		...
		backend: {
			...
			...(params.normalizeConfig
				? { normalizeConfig: params.normalizeConfig }
				: {}),
		},
	};
}
			...
			normalizeConfig: (config) => {
				const normalizeArgs = (args: string[] | undefined) => {
					...
					const hasPermissionMode = next.some(
						(arg, index) =>
							arg === "--permission-mode" ||
							next[index - 1]?.startsWith("--permission-mode="),
					);
					return hasPermissionMode
						? next
						: [...next, "--permission-mode", "bypassPermissions"];
				};
				return {
					...config,
...
describe("resolveCliBackendConfig reliability merge", () => {
	...
	it("deep-merges reliability watchdog overrides for codex", () => {
		...
		expect(resolved).not.toBeNull();
		expect(
			resolved?.config.reliability?.watchdog?.resume?.noOutputTimeoutMs,
		).toBe(42_000);
		// Ensure defaults are retained when only one field is overridden.
		expect(
			resolved?.config.reliability?.watchdog?.resume?.noOutputTimeoutRatio,
		).toBe(0.3);
		...
		expect(
			resolved?.config.reliability?.watchdog?.fresh?.noOutputTimeoutRatio,
		).toBe(0.8);
	});
});
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
	it("uses non-interactive permission-mode defaults for fresh and resume args", () => {
		...
		expect(resolved?.config.args).not.toContain(
		...
		expect(resolved?.config.resumeArgs).not.toContain(
			"--dangerously-skip-permissions",
		);
	});
...
describe("resolveCliBackendConfig google-gemini-cli defaults", () => {
	it("uses Gemini CLI json args and existing-session resume mode", () => {
		...
		expect(resolved?.config.sessionIdFields).toEqual([
			"session_id",
			"sessionId",
		]);
