...
import { buildSessionAsyncTaskStatusDetails, findActiveSessionTask, } from "./session-async-task-status.js";
export function isActiveMediaGenerationTask(params) {
    return (params.task.runtime === "cli" &&
        params.task.scopeKind === "session" &&
        params.task.taskKind === params.taskKind &&
        (params.task.status === "queued" || params.task.status === "running"));
}
export function getMediaGenerationTaskProviderId(task, sourcePrefix) {
