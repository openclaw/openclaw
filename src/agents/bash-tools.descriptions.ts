...
export function describeExecTool(params?: { agentId?: string; hasCronTool?: boolean }): string {
  const base = [
    ...
    "Use yieldMs/background to continue later via process tool; backgrounded commands still use the exec timeout, so set timeout=0 for intentionally long-lived local/sandbox/gateway servers, agents, or PTYs, and use a large positive timeout for host=node.",
