...
type StdinState = "writable" | "closed" | "unavailable";
...
function resolveStdinState(stdin: WritableStdin | undefined): StdinState {
  if (!stdin) {
    return "unavailable";
  }
  if (
...
function isWritableStdin(stdin: WritableStdin | undefined): stdin is WritableStdin {
  return resolveStdinState(stdin) === "writable";
}

function runningSessionInputDetails(runtime: RunningSessionRuntime) {
  return {
    stdinState: runtime.stdinState,
...
export function createProcessTool(
  ...
  const describeRunningSession = (session: ProcessSession): RunningSessionRuntime => {
    ...
    const lastOutputAt = Math.max(
      ...
      record?.lastOutputAtMs ?? session.startedAt,
    );
    const idleMs = Math.max(0, Date.now() - lastOutputAt);
    const stdinState = resolveStdinState(resolveSessionStdin(session));
    const stdinWritable = stdinState === "writable";
    return {
      stdinState,
      stdinWritable,
  ...
  const buildInputWaitHint = (
    ...
    const inputHint =
      context === "attach"
        ...
        : "Use process attach to inspect the transcript, then write, send-keys, submit, or paste to provide input.";
          ...
          if (scopedSession) {
            ...
            const inputControls =
              runtime.stdinWritable && !runtime.waitingForInput
                ? "\n\nStdin is writable. Use process write, send-keys, submit, or paste to provide input."
                : "";
            return {
              content: [
                {
                  ...
                  text:
                    ...
                    buildInputWaitHint(runtime, "attach") +
                    inputControls,
                },
              ],
              details: {
            ...
            content: [
              {
                ...
                text:
                  ...
                  (exited
                    ...
                    : buildInputWaitHint(runtime, "follow-up") || "\n\nProcess still running."),
              },
            ],
            details: {
              status,
