...
const processSchema = Type.Object({
	...
	timeout: Type.Optional(
		Type.Number({
			description:
				"For poll: wait up to this many milliseconds before returning",
			minimum: 0,
		}),
	),
});
...
function recordPollRetrySuggestion(
	sessionId: string,
			...
			const resolveBackgroundedWritableStdin = () => {
				if (!scopedSession) {
					return {
						ok: false as const,
						result: failedResult(
							`No active session found for ${params.sessionId}`,
						),
					};
				}
				...
				if (!stdin || stdin.destroyed) {
					return {
						ok: false as const,
						result: failedResult(
							`Session ${params.sessionId} stdin is not writable.`,
						),
					};
				}
				return {
			...
			switch (params.action) {
					...
					if (pollWaitMs > 0 && !scopedSession.exited) {
						...
						while (!scopedSession.exited && Date.now() < deadline) {
							await new Promise((resolve) =>
								setTimeout(
									resolve,
									Math.max(0, Math.min(250, deadline - Date.now())),
								),
							);
						}
					}
				...
				case "log": {
					if (scopedSession) {
						...
						const logDefaultTailNote = defaultTailNote(
							totalLines,
							window.usingDefaultTail,
						);
						return {
							content: [
								{
				...
				case "send-keys": {
					...
					if (
						...
						hasCursorModeSensitiveKeys(request)
					) {
						return failText(
