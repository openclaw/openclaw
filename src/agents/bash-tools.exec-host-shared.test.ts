...
describe("sendExecApprovalFollowupResult", () => {
	beforeEach(async () => {
		...
		({
			...
			MAX_EXEC_APPROVAL_FOLLOWUP_FAILURE_LOG_KEYS:
				maxExecApprovalFollowupFailureLogKeys,
	...
	it("logs repeated followup dispatch failures once per approval id and error message", async () => {
		vi.mocked(sendExecApprovalFollowup).mockRejectedValue(
	...
	it("evicts oldest followup failure dedupe keys after reaching the cap", async () => {
		...
		expect(logWarn).toHaveBeenCalledTimes(
			maxExecApprovalFollowupFailureLogKeys + 2,
		);
