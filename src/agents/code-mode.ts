...
function createCodeModeExecDescription(
  ...
  return (
    'Run JavaScript or TypeScript in code mode. Use `return` to pass the final value back. Quick-index arrows show trusted declared output hints; `-> ?` means never guess result field names. For declared fields, process them in the first exec; do not spend another exec inspecting them. Perform dependent reads, checks, and follow-up calls in order; parallelize independent work only. For an unknown output, including a final dependent call after declared-output calls, return the raw tool value unchanged; do not wrap it in the requested answer shape or guess fields; filter or map it only in a later exec. Nested calls enforce normal tool policy and approvals. `ALL_TOOLS` is the complete compact catalog. Select exact ids directly or with `tools.search(query: string, options?)`; use `tools.describe(id: string)` only when needed. Never invent or transform a tool id. `tools.callValue(id: string, args?)` returns its JSON value directly; `tools.call(id: string, args?)` preserves `{ tool, result }`. Example: `const hit = ALL_TOOLS.find(x=>x.name==="read") ?? (await tools.search("read"))[0]; return tools.callValue(hit.id,{});`. Node.js modules: `require`/`import` are NOT available; use enabled catalog tools allowed by policy for external actions. `setTimeout` and `setInterval`: unavailable in the QuickJS guest, like all Node/browser timers. For asynchronous terminal output, use separate tool turns: `await yield_control(...)`, then resume the returned run with `wait`; never invent sleep or timer globals.' +
    apiGuidance +
    mcpGuidance +
    swarmGuidance +
    nodesGuidance +
