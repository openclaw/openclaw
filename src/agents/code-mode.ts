...
function createCodeModeExecDescription(
  ...
  return (
    ...
    shellGuidance +
    ` Enabled tools are async global functions. Compose deterministic dependent work in one cell and await calls in order; use Promise.all only for independent work. Declared output fields may feed later calls in the same program. Emit output with \`text(value)\` or \`json(value)\`. Return the final value; otherwise \`null\`. \`-> ?\` means the output contract is unknown: return the raw value for an extra observation only when the actual output is genuinely ambiguous. If authority changes between calls, observe the current state before choosing the next action. Never replay a mutation whose outcome is uncertain. For omitted tools, use \`catalog.search(query)\`; results are callable: \`const [tool] = await catalog.search("..."); return await tool({...});\`. Use handle \`describe()\` for schemas. \`setTimeout\` and \`clearTimeout\` work. \`TextEncoder\` and \`TextDecoder\` support local text encoding without network access. Console log/info/warn/error/debug emit bounded text. Nested calls enforce normal tool policy and approvals. Tool failures are catchable JavaScript errors; use a typed failed result to correct code or choose another tool. Nested results are intact or throw catchable resource errors. Replies share a ${Math.min(config.memoryLimitBytes, config.maxSnapshotBytes)}-byte cell inbox; consume replies or paginate if full. Cumulative output and the final value or error share ${maxOutputBytes} bytes across waits. Context limits preserve complete status and continuation. Truncation reports a normalized JSON prefix and omitted bytes; rerun with narrower args. Output is incremental; changed cumulative summaries replace earlier ones. Node.js modules and \`require\`/\`import\` are NOT available; use tools for external actions.` +
    apiGuidance +
    mcpGuidance +
    swarmGuidance +
    nodesGuidance +
