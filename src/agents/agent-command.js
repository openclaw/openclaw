import { formatThinkingLevels, isThinkingLevelSupported, normalizeThinkLevel, normalizeVerboseLevel, resolveSupportedThinkingLevel, } from "../auto-reply/thinking.js";
...
import { listAgentIds, resolveAgentDir, resolveEffectiveModelFallbacks, resolveSessionAgentId, resolveAgentSkillsFilter, resolveAgentWorkspaceDir, } from "./agent-scope.js";
import { clearSessionAuthProfileOverride } from "./auth-profiles/session-override.js";
import { ensureAuthProfileStore } from "./auth-profiles/store.js";
import { persistSessionEntry as persistSessionEntryBase, prependInternalEventContext, } from "./command/attempt-execution.shared.js";
...
import { buildAllowedModelSet, modelKey, normalizeModelRef, parseModelRef, resolveConfiguredModelRef, resolveDefaultModelForAgent, resolveThinkingDefault, } from "./model-selection.js";
...
async function prepareAgentCommandExecution(opts, runtime) {
    ...
    const subagentLane = AGENT_LANE_SUBAGENT;
    const isSubagentLane = laneRaw === subagentLane;
    const timeoutSecondsRaw = opts.timeout !== undefined ? Number.parseInt(opts.timeout, 10) : isSubagentLane ? 0 : undefined;
    ...
    const { sessionId, sessionKey, sessionEntry: sessionEntryRaw, sessionStore, storePath, isNewSession, persistedThinking, persistedVerbose, } = sessionResolution;
    const sessionAgentId = agentIdOverride ??
...
async function agentCommandInternal(opts, runtime = defaultRuntime, deps) {
    ...
    try {
        ...
        const explicitProviderOverride = typeof opts.provider === "string"
            ? normalizeExplicitOverrideInput(opts.provider, "provider")
            : undefined;
        const explicitModelOverride = typeof opts.model === "string"
        ...
        if (hasExplicitRunOverride) {
            ...
            if (!allowAnyModel && !allowedModelKeys.has(explicitKey)) {
                throw new Error(`Model override "${sanitizeForLog(explicitRef.provider)}/${sanitizeForLog(explicitRef.model)}" is not allowed for agent "${sessionAgentId}".`);
            }
            ...
            try {
                ...
                if (err instanceof LiveSessionModelSwitchError) {
                    ...
                    if (!allowAnyModel && !allowedModelKeys.has(switchKey)) {
                        log.info(`Live session model switch in subagent run ${runId}: ` +
...
export async function agentCommandFromIngress(opts, runtime = defaultRuntime, deps) {
