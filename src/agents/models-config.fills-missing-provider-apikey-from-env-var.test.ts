...
function createAgentSeedProvider(
	overrides: Partial<MergeSeedProvider> = {},
): MergeSeedProvider {
	return {
...
function createOpenAiConfigWithResolvedApiKey(
	mergeMode = false,
): OpenClawConfig {
	return {
		models: {
...
async function expectOpenAiEnvMarkerApiKey(options?: {
	seedMergedProvider?: boolean;
}) {
	await withEnvVar(
		"OPENAI_API_KEY",
	...
	it("fills anthropic-vertex apiKey with the ADC sentinel when models exist", async () => {
		await withTempHome(async () => {
			...
			try {
				...
				expect(parsed.providers["anthropic-vertex"]?.apiKey).toBe(
