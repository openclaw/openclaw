...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits extended sections in minimal prompt mode", () => {
    ...
    expect(prompt).toContain(
      "For long waits, avoid rapid poll loops: use exec with enough yieldMs plus a higher timeout; use timeout=0 only for local/sandbox/gateway long-lived commands, not host=node; or use process(action=poll, timeout=<ms>).",
    );
