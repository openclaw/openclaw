...
describe("buildAgentSystemPrompt", () => {
  ...
  it("teaches direct status answers only on the full Control UI surface", () => {
    ...
    expect(minimalWebchatPrompt).not.toContain(
  ...
  it("guides subagent workflows to avoid polling loops", () => {
    ...
    expect(prompt).toContain(
      "Never loop-poll `subagents list`/`sessions_list`",
    );
  ...
  it("guides harness requests to ACP thread-bound spawns", () => {
    ...
    expect(prompt).toContain(
      "never route ACP via `subagents`/`agents_list`/local PTY",
    );
    expect(prompt).toContain(
  ...
  it("delegates system changes when openclaw tool is present", () => {
    ...
    expect(prompt).toContain(
      "Never write own config; OpenClaw is system expert.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain(
      "`agents_list`/`subagents` = OpenClaw runtime=subagent only",
    );
    expect(prompt).toContain("Subagent results auto-announce");
    expect(prompt).toContain(
      "never sessions_list/history, exec sleep, or poll loops",
    );
