...
import { createEmbeddedRunReplayState, mergeEmbeddedRunReplayState, } from "./pi-embedded-runner/replay-state.js";
import { createEmbeddedPiSessionEventHandler } from "./pi-embedded-subscribe.handlers.js";
import { consumePendingAssistantReplyDirectivesIntoReply, consumePendingToolMediaIntoReply, readPendingToolMediaReply, } from "./pi-embedded-subscribe.handlers.messages.js";
...
export function subscribeEmbeddedPiSession(params) {
    ...
    const pendingBlockReplyTasks = new Set();
    const replyDirectiveAccumulator = createStreamingDirectiveAccumulator();
    const partialReplyDirectiveAccumulator = createStreamingDirectiveAccumulator();
    const shouldAllowSilentTurnText = (text) => Boolean(text && isSilentReplyText(text, SILENT_REPLY_TOKEN));
    const emitBlockReplySafely = (payload, options) => {
        if (!params.onBlockReply) {
            return;
        }
        try {
            const taggedPayload = options?.assistantMessageIndex !== undefined
    ...
    const resolveAssistantUsage = (usageLike) => {
        ...
        if (usageLike && typeof usageLike === "object") {
            ...
            const partial = record.partial && typeof record.partial === "object"
                ? record.partial
                : undefined;
        ...
        for (const candidate of candidates) {
            const usage = normalizeUsage((candidate ?? undefined));
