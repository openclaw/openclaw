...
describe("moonshot implicit provider (#33637)", () => {
	it("uses explicit CN baseUrl when provided", async () => {
		...
		try {
			...
			expect(
				providers?.moonshot?.models?.[0]?.compat?.supportsUsageInStreaming,
			).toBeUndefined();
		} finally {
			envSnapshot.restore();
		}
	...
	it("opts native Moonshot baseUrls into streaming usage only after the final compat pass", () => {
		...
		expect(
			defaultProviders.moonshot?.models?.[0]?.compat?.supportsUsageInStreaming,
		).toBe(true);
		...
		expect(
			cnProviders.moonshot?.models?.[0]?.compat?.supportsUsageInStreaming,
		).toBe(true);
		...
		expect(
			customProviders.moonshot?.models?.[0]?.compat?.supportsUsageInStreaming,
		).toBeUndefined();
	});
});
