...
import { composeSplitTurnInstructions, resolveCompactionInstructions, } from "./compaction-instructions.js";
import { appendSummarySection, auditSummaryQuality, buildCompactionStructureInstructions, buildStructuredFallbackSummary, extractOpaqueIdentifiers, wrapUntrustedInstructionBlock, } from "./compaction-safeguard-quality.js";
import { getCompactionSafeguardRuntime, setCompactionSafeguardCancelReason, } from "./compaction-safeguard-runtime.js";
...
const PREVIOUS_SUMMARY_REDISTILL_PREFIX = "Previous compaction summary to re-distill with the current conversation. " +
    ...
    api.on("session_before_compact", async (event, ctx) => {
        ...
        if (providerId) {
            ...
            if (compactionProvider) {
                ...
                log.warn(`Compaction provider "${providerId}" is configured but not registered. Falling back to LLM.`);
            }
        }
        // -----------------------------------------------------------------------
        ...
        if (!model) {
            if (!ctx.model && !runtime?.model && !missedModelWarningSessions.has(ctx.sessionManager)) {
                ...
                log.warn("[compaction-safeguard] Both ctx.model and runtime.model are undefined. " +
            ...
            for (let attempt = 0; attempt < totalAttempts; attempt += 1) {
                ...
                try {
                    ...
                    if (lastSuccessfulSummary && attempt > 0) {
                        log.warn(`Compaction safeguard: quality retry failed on attempt ${attempt + 1}; ` +
                ...
                const qualityFeedbackInstruction = identifierPolicy === "strict"
