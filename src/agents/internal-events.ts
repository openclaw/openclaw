...
function formatChildResultDataBlock(value: string): string {
  // Keep every task-completion prompt projection within one fixed budget while
  // preserving the authoritative full result held by the completion event.
  return (
    wrapPromptDataBlock({
      label: "Child result",
      text: value,
      maxChars: MAX_TASK_COMPLETION_RESULT_CHARS,
