...
export function resolveFastModeParam(
	...
	return normalizeFastMode(
		(extraParams?.fastMode ?? extraParams?.fast_mode) as
			| string
			| boolean
			| null
			| undefined,
...
export function resolveConfiguredFastMode(params: {
	...
	return (
		normalizeFastMode(
			resolveConfiguredFastModeRaw(params) as
				| string
				| boolean
				| null
				| undefined,
...
export function resolveFastModeState(params: {
	...
	const configured = normalizeFastMode(
		configuredRaw as string | boolean | null | undefined,
	);
