...
export function buildSkillWorkshopPromptSection(): string[] {
  return [
    ...
    "When the user has already explicitly approved a low-risk internal skill creation, update, correction, or addition in the current workstream, create/update/revise the proposal and then apply it in the same turn after the Skill Workshop scan is clean. Do not ask for a second approval loop.",
    "Do not auto-apply proposals that are destructive, external/public, privacy-sensitive, credential-related, scope-ambiguous, or scanner-blocked; leave them pending and state the real decision or blocker.",
    "Use `action=apply`, `action=reject`, or `action=quarantine` after the user explicitly asks to approve/use/apply, reject, or quarantine a specific proposal, or when the current approved low-risk workstream requires same-turn auto-apply. Pass `proposal_id`; if it is not known, use `action=list` or `action=inspect` first.",
