...
describe("markAuthProfileFailure", () => {
	it("does not overwrite fresher on-disk credentials with a stale runtime snapshot", async () => {
		...
		try {
			...
			expect(
				staleCredential && "key" in staleCredential
					? staleCredential.key
					: undefined,
			).toBe("sk-expired-old");
			...
			expect(
				reloadedCredential && "key" in reloadedCredential
					? reloadedCredential.key
					: undefined,
			).toBe("sk-fresh-new");
			expect(
				typeof reloaded.usageStats?.["openai:default"]?.cooldownUntil,
			).toBe("number");
		} finally {
			clearRuntimeAuthProfileStoreSnapshots();
			fs.rmSync(agentDir, { recursive: true, force: true });
