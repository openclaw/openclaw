...
function resolveExecConfig(params: { cfg?: ActiviConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: ActiviConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createActiviCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createActiviReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
