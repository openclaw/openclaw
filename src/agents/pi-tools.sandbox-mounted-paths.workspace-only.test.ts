  ...
  it("defaults to allowing sandbox mounts outside the workspace root", async () => {
    await withUnsafeMountedSandboxHarness(
      async ({ sandboxRoot, agentRoot, sandbox }) => {
        ...
        const readResult = await readTool?.execute("t1", {
          path: "/agent/secret.txt",
        });
        expect(getTextContent(readResult)).toContain("shh");

        await writeTool?.execute("t2", {
          path: "/agent/owned.txt",
          content: "x",
        });
        expect(
          await fs.readFile(path.join(agentRoot, "owned.txt"), "utf8"),
        ).toBe("x");
      },
    );
  });
  ...
  it("rejects sandbox mounts outside the workspace root when enabled", async () => {
    await withUnsafeMountedSandboxHarness(
      async ({ sandboxRoot, agentRoot, sandbox }) => {
        ...
        await expect(
          readTool?.execute("t1", { path: "/agent/secret.txt" }),
        ...
        await expect(
          fs.stat(path.join(agentRoot, "owned.txt")),
        ...
        expect(
          await fs.readFile(path.join(agentRoot, "secret.txt"), "utf8"),
        ).toBe("shh");
      },
    );
  });
...
*** End Patch`;
        ...
        await expect(
          applyPatchTool.execute("t1", { input: patch }),
        ).rejects.toThrow(/Path escapes sandbox root/i);
        await expect(
          fs.stat(path.join(agentRoot, "pwned.txt")),
        ).rejects.toMatchObject({
          code: "ENOENT",
        });
      },
...
*** End Patch`;
        ...
        expect(
          await fs.readFile(path.join(agentRoot, "pwned.txt"), "utf8"),
        ).toBe("owned-by-apply-patch\n");
      },
    );
  });
});
