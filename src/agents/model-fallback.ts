...
export function isFallbackSummaryError(
	err: unknown,
): err is FallbackSummaryError {
...
function shouldProbePrimaryDuringCooldown(params: {
	...
	const soonest = getSoonestCooldownExpiry(
		params.authStore,
		params.profileIds,
		{
			now: params.now,
...
function resolveCooldownDecision(params: {
	...
	const isPersistentAuthIssue =
		inferredReason === "auth" || inferredReason === "auth_permanent";
	...
	for (let i = 0; i < candidates.length; i += 1) {
		...
		if (authStore) {
			...
			if (profileIds.length > 0 && !isAnyProfileAvailable) {
				...
				if (shouldAllowCooldownProbeForReason(decision.reason)) {
					...
					const isTransientCooldownReason = shouldUseTransientCooldownProbeSlot(
						decision.reason,
					);
					if (
		...
		if ("success" in attemptRun) {
			...
			const notFoundAttempt =
				i > 0
					? attempts.find((a) => a.reason === "model_not_found")
					: undefined;
			if (notFoundAttempt) {
				log.warn(
