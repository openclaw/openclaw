...
describe("analyzeBootstrapBudget", () => {
	it("reports per-file and total-limit causes", () => {
		...
		const agents = analysis.truncatedFiles.find(
			(file) => file.name === "AGENTS.md",
		);
		const soul = analysis.truncatedFiles.find(
			(file) => file.name === "SOUL.md",
		);
...
describe("bootstrap prompt warnings", () => {
	...
	it("improves cache-relevant system prompt stability versus legacy warning injection", () => {
		const contextFiles = [
			{ path: "AGENTS.md", content: "Follow AGENTS guidance." },
		];
		...
		const cacheHitRate = (turns: string[]) => {
			const hits = turns
				.slice(1)
				.filter((turn, index) => turn === turns[index]).length;
