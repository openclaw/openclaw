...
describe("main-session-restart-recovery", () => {
  ...
  it("skips the interrupted trajectory event when capture is disabled but still marks the session", async () => {
    // OPENCLAW_TRAJECTORY=0 is the shipped capture opt-out: the state owner must
    // still commit its interruption marking while recording stays off.
    const sessionsDir = await makeSessionsDir();
    const storePath = path.join(sessionsDir, "sessions.json");
    await writeStorePath(storePath, {
      "agent:main:main": runningSessionEntry("main-session", { lifecycleRunId: "restart-run" }),
    });
    const previousTrajectory = process.env.OPENCLAW_TRAJECTORY;
    process.env.OPENCLAW_TRAJECTORY = "0";
    let result: { marked: number; skipped: number } | undefined;
    try {
      result = await markRestartAbortedMainSessions({
        resolveGatewayContext,
        stateDir: tmpDir,
        activeRuns: [activeRestartRun()],
      });
    } finally {
      if (previousTrajectory === undefined) {
        delete process.env.OPENCLAW_TRAJECTORY;
      } else {
        process.env.OPENCLAW_TRAJECTORY = previousTrajectory;
      }
    }
    expect(result).toEqual({ marked: 1, skipped: 0 });
    const store = readStore(storePath);
    expect(store["agent:main:main"]?.abortedLastRun).toBe(true);
    await expect(
      loadSqliteTrajectoryRuntimeEvents({ sessionId: "main-session", storePath }),
    ).resolves.toEqual([]);
  });
    ...
    const dispatchAgent = vi.fn<GatewayRecoveryRuntime["dispatchAgent"]>(
      async (request, _timeoutMs, options) => {
        ...
        await commitMainSessionRecovery({
          ...
          requireWriteSuccess: true,
          target: { agentId: "ops", sessionKey: "global", storePath },
        });
        acceptedObserver = options?.onAccepted;
        acceptedObserver?.({ runId, status: "accepted" });
        throw new Error("detached execution failed before provider start");
