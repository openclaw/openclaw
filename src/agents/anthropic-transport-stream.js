import { calculateCost, getEnvApiKey, parseStreamingJson, } from "@mariozechner/pi-ai";
...
import { coerceTransportToolCallArguments, createEmptyTransportUsage, createWritableTransportEventStream, failTransportStream, finalizeTransportStream, mergeTransportHeaders, sanitizeTransportPayloadText, } from "./transport-stream-shared.js";
...
const CLAUDE_CODE_TOOL_LOOKUP = new Map(CLAUDE_CODE_TOOLS.map((tool) => [normalizeLowercaseStringOrEmpty(tool), tool]));
...
function supportsAdaptiveThinking(modelId) {
    return (isClaudeOpus47Model(modelId) ||
...
function buildAnthropicParams(model, context, isOAuthToken, options) {
    ...
    if (maxTokens === undefined) {
        throw new Error(`Anthropic Messages transport requires a positive maxTokens value for ${model.provider}/${model.id}`);
    }
    ...
    return (rawModel, context, rawOptions) => {
        ...
        void (async () => {
            ...
            try {
                ...
                const anthropicStream = client.messages.stream({ ...params, stream: true }, transportOptions.signal ? { signal: transportOptions.signal } : undefined);
                    ...
                    if (event.type === "content_block_start") {
                        ...
                        if (contentBlock?.type === "tool_use") {
                            const block = {
                                ...
                                name: typeof contentBlock.name === "string"
                                    ? isOAuthToken
                                        ? fromClaudeCodeName(contentBlock.name, context.tools)
                                        : contentBlock.name
                                    : "",
                                arguments: contentBlock.input && typeof contentBlock.input === "object"
                                    ? contentBlock.input
                                    : {},
                                partialJson: "",
                                index,
