  ...
  return {
    ...
    execute: async (toolCallId, args, signal, onUpdate) => {
      ...
      for (const key of pathParamKeys) {
        ...
        const includedRoots = guardedRoot === root ? allowedRoots.slice(1) : [];
        let guardRoots = [guardedRoot, ...additionalRoots, ...includedRoots];
        if (includedRoots.length > 0 && path.isAbsolute(sandboxPath)) {
          try {
            const match = resolvePathWithinRoots(guardRoots, sandboxPath, {
              cwd: guardedRoot,
              boundaryLabel: "allowed work roots",
            });
            guardRoots = [match.root];
          } catch {
            // Fall through to the normal guard path so error wording stays consistent.
          }
        }
        ...
        const guardedPath = sandboxResult.resolved;
