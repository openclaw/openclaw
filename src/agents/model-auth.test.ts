...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-oauth-"));

    try {
      process.env.EASYHUB_STATE_DIR = tempDir;
      process.env.EASYHUB_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.EASYHUB_AGENT_DIR, {
      ...
      const apiKey = await getApiKeyForModel({
        ...
        store,
        agentDir: process.env.EASYHUB_AGENT_DIR,
      });
