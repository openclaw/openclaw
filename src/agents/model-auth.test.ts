...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-oauth-"));

    try {
      process.env.DNA_STATE_DIR = tempDir;
      process.env.DNA_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.DNA_AGENT_DIR, {
      ...
      const apiKey = await getApiKeyForModel({
        ...
        store,
        agentDir: process.env.DNA_AGENT_DIR,
      });
