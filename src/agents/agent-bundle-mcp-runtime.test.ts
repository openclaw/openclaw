...
describe("session MCP runtime", () => {
  ...
  it("catalogs tools whose inputSchema omits type by backfilling object (issue #63602)", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bundle-mcp-typeless-schema-"));
    const serverPath = path.join(tempDir, "typeless-schema.mjs");
    const logPath = path.join(tempDir, "server.log");
    await writeListToolsMcpServer({
      filePath: serverPath,
      logPath,
    ...
    const runtime = await getOrCreateSessionMcpRuntime({
      sessionId: "session-typeless-schema",
      sessionKey: "agent:test:session-typeless-schema",
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
          servers: {
    ...
    try {
      ...
      expect(catalog.servers.legacySdk?.toolCount).toBe(1);
      expect(catalog.diagnostics).toBeUndefined();
    } finally {
      await runtime.dispose();
      await fs.rm(tempDir, { recursive: true, force: true });
