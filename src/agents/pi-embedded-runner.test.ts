...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-embedded-agent-"));
...
const ensureModels = (cfg: EasyHubConfig) => ensureEasyHubModelsJson(cfg, agentDir) as unknown;
