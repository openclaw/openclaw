...
describe("native session binding generation", () => {
  it("fences an already-readable binding when its admitted session generation rotates", async () => {
    const fixture = await createOpenClawTestState({
      prefix: "native-readable-authority-", layout: "state-only", applyEnv: false,
    });
    const storePath = path.join(fixture.stateDir, "sessions.json");
    const target = {
      agentId: "main", sessionId: "session-current", sessionKey: "agent:main:readable",
    };
    ...
    try {
      ...
      const resolved = await resolveNativeSessionBinding({
        target, storePath, readBinding: () => binding, createSupersededError,
      });
  ...
  it("rejects an already-readable binding owned by a stale admitted session", async () => {
    const fixture = await createOpenClawTestState({
      prefix: "native-readable-stale-", layout: "state-only", applyEnv: false,
    });
    const storePath = path.join(fixture.stateDir, "sessions.json");
    const target = {
      agentId: "main", sessionId: "session-stale", sessionKey: "agent:main:readable",
    };
    try {
      await upsertSessionEntryCore({
        agentId: target.agentId, sessionKey: target.sessionKey, storePath,
      }, { sessionId: "session-current", updatedAt: 1 });
      await expect(resolveNativeSessionBinding({
        target, storePath, readBinding: () => ({ value: "stale-native-owner" }), createSupersededError,
  ...
  it("preserves caller authority for a scoped session with no durable row", async () => {
    const fixture = await createOpenClawTestState({
      prefix: "native-readable-ephemeral-", layout: "state-only", applyEnv: false,
    });
    const storePath = path.join(fixture.stateDir, "sessions.json");
    const target = {
      agentId: "main", sessionId: "session-ephemeral", sessionKey: "agent:main:ephemeral",
    };
    let active = true;
    try {
      await upsertSessionEntryCore({
        agentId: target.agentId, sessionKey: "agent:main:other", storePath,
  ...
  it("does not bridge two generations when the host rotates during a predecessor wait", async () => {
    const fixture = await createOpenClawTestState({
      prefix: "native-predecessor-wait-", layout: "state-only", applyEnv: false,
    });
    const storePath = path.join(fixture.stateDir, "sessions.json");
    const previous = {
      agentId: "main", sessionId: "previous", sessionKey: "agent:main:compaction",
    };
    ...
    const preparationReleased = new Promise<void>((resolve) => { finishPreparation = resolve; });
    ...
    try {
      ...
      const outcome = reclaimNativeSessionGeneration({
        target, storePath, generation, reclaimStale: false, createSupersededError,
      }).catch((error: unknown) => error);
      await preparationStarted;
      await patchSessionEntryCore(scope, () => ({ sessionId: next.sessionId }));
      finishPreparation();

      expect(await outcome).toMatchObject({ message: `Session generation is no longer current: ${target.sessionId}` });
      ...
      expect(loadSessionEntryReadOnly(scope)).toMatchObject({
        sessionId: next.sessionId, previousSessionId: target.sessionId,
      });
      await expect(reclaimNativeSessionGeneration({
        target: next, storePath, generation, reclaimStale: false, createSupersededError,
      })).resolves.toBe(false);
    } finally {
      await fixture.cleanup();
    }
