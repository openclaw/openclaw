...
type NativeHookRelaySkillWorkshopApprovalPolicy = "pending" | "auto";
  ...
  const handle: ActiveNativeHookRelayRegistrationHandle = {
    ...
    commandForEvent: (event, options) =>
      buildNativeHookRelayCommand({
        ...
        skillWorkshopApprovalPolicy: resolveNativeHookRelaySkillWorkshopApprovalPolicy({
          registration,
          event,
        }),
...
export function buildNativeHookRelayCommand(params: {
  ...
  skillWorkshopApprovalPolicy?: NativeHookRelaySkillWorkshopApprovalPolicy;
  timeoutMs?: number;
  executable?: string;
  nice?: number | false;
  nodeExecutable?: string;
  ...
  return shellQuoteArgs([
    ...
    ...(params.event === "pre_tool_use" && params.preToolUseUnavailable
      ...
      : []),
    ...(params.event === "pre_tool_use" && params.skillWorkshopApprovalPolicy
      ? ["--skill-workshop-approval-policy", params.skillWorkshopApprovalPolicy]
      : []),
    "--timeout",
    String(timeoutMs),
  ]);
...
function resolveNativeHookRelaySkillWorkshopApprovalPolicy(params: {
  registration: NativeHookRelayRegistration;
  event: NativeHookRelayEvent;
}): NativeHookRelaySkillWorkshopApprovalPolicy | undefined {
  if (params.event !== "pre_tool_use") {
    return undefined;
  }
  const approvalPolicy = resolveSkillWorkshopConfig(params.registration.config).approvalPolicy;
  return approvalPolicy === "auto" ? "auto" : undefined;
}
...
export function renderNativeHookRelayUnavailableResponse(params: {
  ...
  skillWorkshopApprovalPolicy?: unknown;
  rawPayload?: unknown;
  message?: string;
}): NativeHookRelayProcessResponse {
  const provider = readNativeHookRelayProvider(params.provider);
  ...
  if (event === "pre_tool_use") {
    ...
    if (
      ...
      if (
        nativePreToolUseUnavailableRequiresCoreApproval({
          adapter,
          rawPayload: params.rawPayload,
          skillWorkshopApprovalPolicy: params.skillWorkshopApprovalPolicy,
        })
      ) {
...
function nativePreToolUseUnavailableRequiresCoreApproval(params: {
  adapter: NativeHookRelayProviderAdapter;
  skillWorkshopApprovalPolicy?: unknown;
  rawPayload?: unknown;
}): boolean {
  const payload = isJsonValue(params.rawPayload) ? params.rawPayload : null;
  const metadata = params.adapter.normalizeMetadata(payload);
  const toolName = normalizeNativeHookToolName(metadata.toolName);
  const toolInput = params.adapter.readToolInput(payload);
  const skillWorkshopApprovalPolicy =
    params.skillWorkshopApprovalPolicy === "auto" ? "auto" : "pending";
  return (
    resolveSkillWorkshopToolApproval({
      toolName,
      toolParams: toolInput,
      config: { skills: { workshop: { approvalPolicy: skillWorkshopApprovalPolicy } } },
    }) !== undefined
  );
}
