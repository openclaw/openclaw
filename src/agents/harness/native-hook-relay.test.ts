...
describe("native hook relay registry", () => {
  ...
  it("records auto skill workshop approval policy on pre-tool relay commands", () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      sessionKey: "agent:main:session-1",
      runId: "run-1",
      config: { skills: { workshop: { approvalPolicy: "auto" } } } as never,
      command: {
        executable: "/opt/Open Claw/openclaw.mjs",
        nodeExecutable: "/usr/local/bin/node",
        timeoutMs: 1234,
    ...
    expect(relay.commandForEvent("pre_tool_use")).toBe(
      "/usr/local/bin/node '/opt/Open Claw/openclaw.mjs' hooks relay --provider codex --relay-id " +
        `${relay.relayId} --generation ${relay.generation} --event pre_tool_use --pre-tool-use-unavailable loop-detection-only --skill-workshop-approval-policy auto --timeout 1234`,
    );
  });
