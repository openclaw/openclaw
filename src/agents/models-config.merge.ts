...
function resolvePreferredTokenLimit(params: {
	...
	if (
		params.explicitPresent &&
		isPositiveFiniteTokenLimit(params.explicitValue)
	) {
	...
	return isPositiveFiniteTokenLimit(params.explicitValue)
		? params.explicitValue
		: undefined;
}
...
export function mergeProviderModels(
	...
	const implicitHeaders =
		implicit.headers &&
		typeof implicit.headers === "object" &&
		!Array.isArray(implicit.headers)
			? implicit.headers
			: undefined;
	const explicitHeaders =
		explicit.headers &&
		typeof explicit.headers === "object" &&
		!Array.isArray(explicit.headers)
			? explicit.headers
			: undefined;
...
function resolveModelApiSurface(
	entry: { models?: unknown } | undefined,
): string | undefined {
...
function shouldPreserveExistingApiKey(params: {
	...
	const { providerKey, existing, nextEntry, secretRefManagedProviders } =
		params;
	const nextApiKey =
		typeof nextEntry.apiKey === "string" ? nextEntry.apiKey : "";
