...
import { DEFAULT_MAX_OUTPUT, DEFAULT_PATH, DEFAULT_PENDING_MAX_OUTPUT, applyPathPrepend, applyShellPath, normalizeExecAsk, normalizeExecSecurity, normalizeExecTarget, normalizePathPrepend, resolveExecTarget, resolveApprovalRunningNoticeMs, runExecProcess, execSchema, } from "./bash-tools.exec-runtime.js";
...
function stripPreflightEnvPrefix(argv) {
    ...
    while (idx < argv.length) {
        ...
        if (PREFLIGHT_ENV_OPTIONS_WITH_VALUES.has(option) &&
            !token.includes("=") &&
            idx < argv.length) {
            idx += 1;
        }
...
function extractInterpreterScriptPathsFromSegment(rawSegment) {
    ...
    const withoutLeadingKeyword = /^(?:if|then|do|elif|else|while|until|time)$/i.test(argv[0] ?? "")
        ? argv.slice(1)
        : argv;
    const target = extractInterpreterScriptTargetFromArgv(stripPreflightEnvPrefix(withoutLeadingKeyword));
...
function extractScriptTargetFromCommand(command) {
    ...
    const shouldUseWindowsPathTokenizer = process.platform === "win32" &&
...
function resolveLeadingShellSegmentExecutable(rawSegment) {
    ...
    let commandIdx = 0;
    while (commandIdx < normalizedArgv.length &&
...
function analyzeInterpreterHeuristicsFromUnquoted(raw) {
    ...
    const hasComplexSyntax = hasUnescapedSequence(raw, "|") ||
...
function shouldFailClosedInterpreterPreflight(command) {
    ...
    const isDirectPythonExecutable = Boolean(directExecutable && /^python(?:3(?:\.\d+)?)?$/i.test(directExecutable));
    ...
    const hasInterpreterSegmentScriptHint = hasInterpreterAndScriptHintInSameSegment(raw) ||
        (shellWrappedPayload !== null && hasInterpreterAndScriptHintInSameSegment(shellWrappedPayload));
    const hasInterpreterPipelineScriptHint = hasInterpreterPipelineScriptHintInSameSegment(raw) ||
    ...
    const hasShellWrappedInterpreterInvocation = (nested.hasPython || nested.hasNode) &&
    ...
    const hasInterpreterInvocation = isDirectInterpreterCommand ||
...
async function validateScriptFileForShellBleed(params) {
    ...
    if (!target) {
        ...
        if (hasInterpreterInvocation &&
            ...
            throw new Error("exec preflight: complex interpreter invocation detected; refusing to run without script preflight validation. " +
...
function parseExecApprovalShellCommand(raw) {
    ...
    const match = normalized.match(/^\/approve(?:@[^\s]+)?\s+([A-Za-z0-9][A-Za-z0-9._:-]*)\s+(allow-once|allow-always|always|deny)\b/i);
    if (!match) {
        return null;
    }
    return {
        approvalId: match[1],
        decision: normalizeLowercaseStringOrEmpty(match[2]) === "always"
            ? "allow-always"
