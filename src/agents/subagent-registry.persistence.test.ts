...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["MULLUSI_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-subagent-"));
  ...
  it("uses isolated temp state when MULLUSI_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "mullusi-test-state"));
  });
});
