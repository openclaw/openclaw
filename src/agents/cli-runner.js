...
export async function runCliAgent(params) {
    ...
    const executeCliWithSession = async (cliSessionIdToUse) => {
            ...
            const output = await enqueueCliRun(queueKey, async () => {
                ...
                if (result.exitCode !== 0 || result.reason !== "exit") {
                    if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
                        ...
                        log.warn(`cli watchdog timeout: provider=${params.provider} model=${modelId} session=${resolvedSessionId ?? params.sessionId} noOutputTimeoutMs=${noOutputTimeoutMs} pid=${managedRun.pid ?? "unknown"}`);
    ...
    try {
        ...
        if (err instanceof FailoverError) {
            ...
            if (err.reason === "session_expired" && params.cliSessionId && params.sessionKey) {
                log.warn(`CLI session expired, clearing session ID and retrying: provider=${params.provider} session=${redactRunIdentifier(params.cliSessionId)}`);
