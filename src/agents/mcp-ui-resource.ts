...
export async function fetchMcpAppView(params: {
  ...
  try {
    if (!params.runtime.readResource) {
      logWarn(`mcp-app: runtime does not support resource reads for "${params.serverName}"`);
      return undefined;
    }
    const resource = (await params.runtime.readResource(
      params.serverName,
      params.uiResourceUri,
    )) as { contents?: Array<Record<string, unknown>> };
    const contents = Array.isArray(resource.contents) ? resource.contents : [];
    const content =
      contents.find((item) => item.uri === params.uiResourceUri) ?? contents[0];
    if (!content) {
      logWarn(
