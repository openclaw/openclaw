...
const modelsConfigDeps: PlanOpenClawModelsJsonDeps = {
  ...
  resolveImplicitProviders: async ({ env }) => {
...
async function runEnvProviderCase(params: {
  ...
  try {
    await ensureOpenClawModelsJson({}, undefined, modelsConfigDeps);
...
describe("models-config", () => {
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      await ensureOpenClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG, undefined, modelsConfigDeps);
