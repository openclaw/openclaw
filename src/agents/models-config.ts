...
type ModelsConfig = NonNullable<ActiviConfig["models"]>;
...
export async function ensureActiviModelsJson(
  config?: ActiviConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveActiviAgentDir();
