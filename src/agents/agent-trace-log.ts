...
export function createAgentTraceLogger(params: {
  ...
  const wrapStreamFn: AgentTraceLogger["wrapStreamFn"] = (streamFn) => {
    const wrapped: StreamFn = (model, context, options) => {
      ...
      if (typeof (context as unknown as { system?: unknown })?.system === "string") {
        sysTextFromContext = (context as unknown as { system: string }).system as string;
      }
      const messages = Array.isArray((context as any)?.messages) ? (context as any).messages : [];
      const tools = Array.isArray((context as any)?.tools) ? (context as any).tools : [];
      ...
      const nextOnPayload = (payload: unknown) => {
        ...
        if (payload && typeof payload === "object") {
          ...
          if (Array.isArray(p.messages)) {
            const sysMsg = p.messages.find((m: any) => m.role === "system");
            if (sysMsg) {
        ...
        record({
          ...
          ...(cfg.verbose
            ? {
                payload: redactImageDataForDiagnostics(payload ?? {
                  messages,
                  tools,
                  system: finalSysText,
                }),
  ...
  const logSessionEvent: AgentTraceLogger["logSessionEvent"] = (evt) => {
    ...
    switch (type) {
      case "message_start":
        if (role === "user") break; // Skip user messages
        record({
          ...base,
          ts: new Date().toISOString(),
          type: "message_start",
      ...
      case "message_update": {
        ...
        if (typeof content === "string") {
          ...
          text = content
            ...
            .join("");
          reasoning = content
            .filter((b: unknown) => b && typeof b === "object" && (b as { type?: unknown }).type === "thinking")
