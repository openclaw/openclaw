...
describe("createOpenClawCodingTools deferred follow-up guidance", () => {
  it("keeps cron-specific guidance when cron survives filtering", () => {
    ...
    expect(exec.description).toBe(
      "Execute shell commands with background continuation for work that starts now. Use yieldMs/background to continue later via process tool. For long-running work started now, rely on automatic completion wake when it is enabled and the command emits output or fails; otherwise use process to confirm completion. Use process whenever you need logs, status, input, or intervention. Do not use exec sleep or delay loops for reminders or deferred follow-ups; use cron instead. Search safety: do not run broad recursive `rg`, `grep -R`, or `find` over home directories, OpenClaw/Codex state, session archives, repo parents, or external workspace roots; narrow to a repo/task/evidence path, prefer indexed/search tools when available, and cap output with focused flags or head/sed ranges. Use pty=true for TTY-required commands (terminal UIs, coding agents).",
    );
  ...
  it("drops cron-specific guidance when cron is unavailable", () => {
    ...
    expect(exec.description).toBe(
      "Execute shell commands with background continuation for work that starts now. Use yieldMs/background to continue later via process tool. For long-running work started now, rely on automatic completion wake when it is enabled and the command emits output or fails; otherwise use process to confirm completion. Use process whenever you need logs, status, input, or intervention. Search safety: do not run broad recursive `rg`, `grep -R`, or `find` over home directories, OpenClaw/Codex state, session archives, repo parents, or external workspace roots; narrow to a repo/task/evidence path, prefer indexed/search tools when available, and cap output with focused flags or head/sed ranges. Use pty=true for TTY-required commands (terminal UIs, coding agents).",
    );
    expect(process.description).toBe(
