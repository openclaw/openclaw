...
export async function runBeforeToolCallHook(args: {
  ...
  if (args.ctx?.sessionKey) {
    ...
    recordToolCall(sessionState, toolName, params, effectiveToolCallId, args.ctx.loopDetection);
  }
  ...
  try {
    ...
    const toolCallId = effectiveToolCallId ? ` toolCallId=${effectiveToolCallId}` : "";
...
export function wrapToolWithBeforeToolCallHook(
  ...
  const wrappedTool: AnyAgentTool = {
    ...
    execute: async (toolCallId, params, signal, onUpdate) => {
      ...
      if (toolCallId) {
        adjustedParamsByToolCallId.set(hookToolCallId, outcome.params);
