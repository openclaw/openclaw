...
import { evaluateToolPermission } from "./tool-permission-contracts.js";
...
function evaluateContractPolicy(args: {
  ...
  if (decision.blocked) {
    log.warn(
      `tool policy denied: actor=${decision.actor ?? "unknown"} tool=${args.toolName} permission=${decision.permissionTool} reason=${decision.reason ?? "unknown"}`,
    );
