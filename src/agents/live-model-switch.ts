...
export class LiveSessionModelSwitchError extends Error {
	...
	constructor(selection: LiveSessionModelSelection) {
		super(
			`Live session model switch requested: ${selection.provider}/${selection.model}`,
		);
...
export function resolveLiveSessionModelSelection(params: {
	...
	const model =
		runtimeModel || entry?.modelOverride?.trim() || defaultModelRef.model;
...
export function hasDifferentLiveSessionModelSelection(
	...
	return (
		...
		(current.authProfileId?.trim()
			? current.authProfileIdSource
			: undefined) !== next.authProfileIdSource
	);
}
