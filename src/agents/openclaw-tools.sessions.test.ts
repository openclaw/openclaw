...
const getSessionsListTool = (opts?: Parameters<typeof createSessionsListTool>[0]) =>
  createSessionsListTool({
    config: TEST_CONFIG,
...
const getSessionsHistoryTool = (opts?: Parameters<typeof createSessionsHistoryTool>[0]) =>
  createSessionsHistoryTool({
    config: TEST_CONFIG,
    callGateway: callGatewayMock as never,
    ...opts,
...
const getSessionsSendTool = (opts?: Parameters<typeof createSessionsSendTool>[0]) =>
  createSessionsSendTool({
    config: TEST_CONFIG,
    callGateway: callGatewayMock as never,
    ...opts,
...
const getSubagentsTool = (opts?: Parameters<typeof createSubagentsTool>[0]) =>
  createSubagentsTool(opts);
...
describe("sessions tools", () => {
  ...
  it("uses number (not integer) in tool schemas for Gemini compatibility", () => {
    const tools = [
      createSessionsHistoryTool({ config: TEST_CONFIG, callGateway: callGatewayMock as never }),
      createSessionsListTool({ config: TEST_CONFIG, callGateway: callGatewayMock as never }),
      createSessionsSendTool({ config: TEST_CONFIG, callGateway: callGatewayMock as never }),
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = getSessionsListTool();
  ...
  it("subagents lists active and recent runs", async () => {
    ...
    const tool = getSubagentsTool({ agentSessionKey: "agent:main:main" });
