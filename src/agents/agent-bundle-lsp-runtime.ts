...
export function spawnLspServerProcess(config: StdioMcpServerLaunchConfig): ChildProcess {
  // Rockie isolation: the spawned child must receive the strict owned-child env
  // allowlist (tenant identity preserved, secret-like keys scrubbed), not the
  // upstream host-exec sanitizer. The sanitized env is still used for Windows
  // program resolution so PATH-based shim lookup keeps working.
  const resolverEnv = sanitizeHostExecEnv({ baseEnv: process.env, overrides: config.env ?? null });
  const ownedEnv = buildOwnedChildEnv({ overrides: config.env ?? undefined });
