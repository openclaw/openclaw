...
describe("failover-error", () => {
	it("infers failover reason from HTTP status", () => {
		...
		expect(
			resolveFailoverReasonFromError({
				status: 402,
				message:
					"HTTP 402: request reached organization usage limit, try again later",
			}),
		).toBe("rate_limit");
		...
		expect(
			resolveFailoverReasonFromError({
				status: 402,
				message:
					"HTTP 402: You have exceeded your current quota. Please add more credits.",
			}),
		).toBe("billing");
	...
	it("keeps status-only 503s conservative unless the payload is clearly overloaded", () => {
		...
		expect(
			resolveFailoverReasonFromError({
				status: 503,
				message:
					'{"error":{"message":"The model is overloaded. Please try later"}}',
			}),
		).toBe("overloaded");
	});
	...
	it("keeps temporary 402 spend limits retryable without downgrading explicit billing", () => {
		expect(
			resolveFailoverReasonFromError({
				status: 402,
				message:
					"Monthly spend limit reached. Please visit your billing settings.",
			}),
		).toBe("rate_limit");
		expect(
			resolveFailoverReasonFromError({
	...
	it("infers timeout from common node error codes", () => {
		...
		expect(resolveFailoverReasonFromError({ code: "EHOSTDOWN" })).toBe(
			"timeout",
		);
	...
	it("infers timeout from abort/error stop-reason messages", () => {
		...
		expect(
			resolveFailoverReasonFromError({ message: "stop reason: error" }),
		).toBe("timeout");
		expect(resolveFailoverReasonFromError({ message: "reason: abort" })).toBe(
			"timeout",
		);
		expect(resolveFailoverReasonFromError({ message: "reason: error" })).toBe(
			"timeout",
		);
		expect(
			resolveFailoverReasonFromError({
	...
	it("infers timeout from connection/network error messages", () => {
		expect(
			resolveFailoverReasonFromError({ message: "Connection error." }),
		).toBe("timeout");
		expect(resolveFailoverReasonFromError({ message: "fetch failed" })).toBe(
			"timeout",
		);
		expect(
			resolveFailoverReasonFromError({
