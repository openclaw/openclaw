...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.MULLUSI_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".mullusi", `workspace-${profile}`);
  }
  return path.join(home, ".mullusi", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".mullusi";
