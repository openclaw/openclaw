...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ACTIVI_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".activi", `workspace-${profile}`);
  }
  return path.join(home, ".activi", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".activi";
