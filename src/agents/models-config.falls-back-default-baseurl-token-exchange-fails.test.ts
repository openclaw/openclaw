...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "EasyHub-models-" });
}

const _MODELS_CONFIG: EasyHubConfig = {
  models: {
    providers: {
      "custom-proxy": {
  ...
  it("falls back to default baseUrl when token exchange fails", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        const { ensureEasyHubModelsJson } = await import("./models-config.js");
        const { resolveEasyHubAgentDir } = await import("./agent-paths.js");

        await ensureEasyHubModelsJson({ models: { providers: {} } });

        const agentDir = resolveEasyHubAgentDir();
  ...
  it("uses agentDir override auth profiles for copilot injection", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        ...
        await ensureEasyHubModelsJson({ models: { providers: {} } }, agentDir);
