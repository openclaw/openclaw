...
export function resolveAcpSpawnRuntimePolicyError(params: {
	...
	const requesterSandboxed =
		params.requesterSandboxed === true || requesterRuntime.sandboxed;
...
function isHeartbeatEnabledForSessionAgent(params: {
	...
	const trimmedEvery =
		typeof heartbeatEvery === "string" ? heartbeatEvery.trim() : "";
	if (!trimmedEvery) {
		return false;
	}
	try {
...
function hasSessionLocalHeartbeatRelayRoute(params: {
	...
	if (
		typeof heartbeat?.accountId === "string" &&
		heartbeat.accountId.trim().length > 0
	) {
		return false;
	}
...
function resolveConversationIdForThreadBinding(params: {
	...
	if (channel === "line") {
		const prefixed = target.match(
			/^line:(?:(?:user|group|room):)?([UCR][a-f0-9]{32})$/i,
		)?.[1];
		if (prefixed) {
			return prefixed;
		}
...
function prepareAcpThreadBinding(params: {
	...
	const placement = requiresNativeThreadContextForThreadHere(policy.channel)
		? "child"
		: "current";
	if (
		!capabilities.bindSupported ||
		!capabilities.placements.includes(placement)
	) {
		return {
			ok: false,
...
function resolveAcpSpawnRequesterState(params: {
	...
	const isSubagentSession =
		...
		isSubagentSessionKey(params.parentSessionKey);
	const hasActiveSubagentBinding =
		isSubagentSession && params.parentSessionKey
			? bindingService
					...
					.some(
						(record) =>
							record.targetKind === "subagent" && record.status !== "ended",
					)
			: false;
...
function resolveAcpSpawnStreamPlan(params: {
	...
	return {
		...
		effectiveStreamToParent:
			params.streamToParentRequested || implicitStreamToParent,
	};
}
...
async function bindPreparedAcpThread(params: {
		...
		metadata: {
			...
			introText: resolveThreadBindingIntroText({
				...
				sessionCwd: resolveAcpSessionCwd(
					params.initializedRuntime.initialized.meta,
				),
	...
	if (
		params.initializedRuntime.sessionId &&
		params.preparedBinding.placement === "child"
	) {
		const boundThreadId =
			String(binding.conversation.conversationId).trim() || undefined;
		if (boundThreadId) {
			sessionEntry = await persistAcpSpawnSessionFileBestEffort({
...
function resolveAcpSpawnBootstrapDeliveryPlan(params: {
	...
	const bindingMatchesRequesterConversation = Boolean(
		params.requester.origin?.channel &&
			params.binding?.conversation.channel ===
				params.requester.origin.channel &&
			params.binding?.conversation.accountId ===
				(params.requester.origin.accountId ?? "default") &&
	...
	const boundDeliveryTarget = resolveConversationDeliveryTarget({
		channel:
			params.requester.origin?.channel ?? params.binding?.conversation.channel,
	...
	const hasDeliveryTarget = Boolean(
		params.requester.origin?.channel && inferredDeliveryTo,
	);
	...
	return {
		...
		accountId: useInlineDelivery
			? (params.requester.origin?.accountId ?? undefined)
			: undefined,
	...
	if (effectiveStreamToParent && parentSessionKey) {
		...
		try {
			createRunningTaskRun({
				...
				task: params.task,
				preferMetadata: true,
				deliveryStatus: requesterInternalKey.trim()
					? "pending"
					: "parent_missing",
				startedAt: Date.now(),
			});
