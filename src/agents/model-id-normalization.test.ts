...
describe("normalizeXaiModelId", () => {
	it("maps deprecated grok 4.20 beta ids to GA ids", () => {
		expect(
			normalizeXaiModelId("grok-4.20-experimental-beta-0304-reasoning"),
		).toBe("grok-4.20-beta-latest-reasoning");
		expect(
			normalizeXaiModelId("grok-4.20-experimental-beta-0304-non-reasoning"),
	...
	it("maps older fast and 4.20 ids to the current Pi-backed ids", () => {
		...
		expect(normalizeXaiModelId("grok-4-1-fast-reasoning")).toBe(
			"grok-4-1-fast",
		);
