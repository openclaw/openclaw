...
describe("ensureAuthProfileStore", () => {
	function withTempAgentDir<T>(
		prefix: string,
		run: (agentDir: string) => T,
	): T {
	...
	] as const)("normalizes auth-profiles credential aliases with canonical-field precedence: $name", ({
		name,
		profile,
		expected,
	}) => {
	...
	it("merges legacy oauth.json into auth-profiles.json", () => {
		const root = fs.mkdtempSync(
			path.join(os.tmpdir(), "openclaw-oauth-migrate-"),
		);
