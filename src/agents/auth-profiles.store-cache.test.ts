...
function withAgentDirEnv(prefix: string, run: (agentDir: string) => void | Promise<void>) {
  ...
  const previousAgentDir = process.env.MULLUSI_AGENT_DIR;
  const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
  try {
    process.env.MULLUSI_AGENT_DIR = agentDir;
    ...
    if (previousAgentDir === undefined) {
      ...
      process.env.MULLUSI_AGENT_DIR = previousAgentDir;
    }
...
describe("auth profile store cache", () => {
  ...
  it("reuses the synced auth store while auth-profiles.json is unchanged", async () => {
    await withAgentDirEnv("mullusi-auth-store-cache-", (agentDir) => {
  ...
  it("refreshes the cached auth store after auth-profiles.json changes", async () => {
    await withAgentDirEnv("mullusi-auth-store-refresh-", async (agentDir) => {
  ...
  it("re-syncs external CLI credentials after the cache ttl when auth-profiles.json is absent", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "mullusi-auth-store-missing-"));
    const previousAgentDir = process.env.MULLUSI_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    vi.useFakeTimers();
