...
const LIVE_HEARTBEAT_MS = Math.max(
	1_000,
	toInt(process.env.OPENCLAW_LIVE_HEARTBEAT_MS, 30_000),
);
...
async function withLiveHeartbeat<T>(
	operation: Promise<T>,
	context: string,
): Promise<T> {
	...
	const timer = setInterval(() => {
		heartbeatCount += 1;
		logProgress(
			`${context}: still running (${formatElapsedSeconds(Date.now() - startedAt)})`,
		);
	}, LIVE_HEARTBEAT_MS);
	timer.unref?.();
	try {
		...
		if (heartbeatCount > 0) {
			logProgress(
				`${context}: completed after ${formatElapsedSeconds(Date.now() - startedAt)}`,
			);
		}
	}
}
		...
		async () => {
			...
			const models = await withLiveStageTimeout(
				Promise.resolve().then(() =>
					discoverModels(authStorage, agentDir).getAll(),
				),
			...
			logProgress(
				`[live-models] selection=${useExplicit ? "explicit" : "high-signal"}`,
			);
				...
				for (let attempt = 0; attempt < attemptMax; attempt += 1) {
					...
					try {
						...
						if (
							...
							skipped.push({ model: id, reason: message });
							logProgress(
								`${progressLabel}: skip (codex refresh token reused)`,
							);
							break;
						}
						if (
