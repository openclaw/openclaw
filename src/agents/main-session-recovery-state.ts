...
import type { RestartRecoveryRun } from "../config/sessions/types.js";
...
export function transitionMainSessionRecovery(
  ...
  switch (command.kind) {
    ...
    case "prepare_attempt": {
      ...
      if (
        state.chargedAttempts >= MAIN_RESTART_RECOVERY_MAX_AUTOMATIC_ATTEMPTS ||
        command.attempt > MAIN_RESTART_RECOVERY_MAX_AUTOMATIC_ATTEMPTS
      ) {
        return { kind: "rejected", reason: "attempt_budget_exhausted" };
      }
    ...
    case "claim_foreground": {
      ...
      const currentTokens =
        state.foregroundClaims?.lifecycleGeneration === command.lifecycleGeneration
          ...
          : [];
      const tokens = [...new Set([...currentTokens, command.claimId])].toSorted((a, b) =>
        a.localeCompare(b),
      );
      entry.mainRestartRecovery = {
        ...state,
