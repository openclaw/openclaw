...
export function applyDiscoveredContextWindows(params) {
    for (const model of params.models) {
        ...
        const discoveredContextTokens = typeof model.contextTokens === "number"
...
export function applyConfiguredContextWindows(params) {
    ...
    for (const provider of Object.values(providers)) {
        ...
        for (const model of provider.models) {
            ...
            const contextTokens = typeof model?.contextTokens === "number"
                ? model.contextTokens
                : typeof model?.contextWindow === "number"
                    ? model.contextWindow
                    : undefined;
...
export function shouldEagerWarmContextWindowCache(argv = process.argv) {
...
export function lookupContextTokens(modelId, options) {
    if (!modelId) {
        return undefined;
    }
    ...
    function findContextTokens(matchProviderId) {
        for (const [providerId, providerConfig] of Object.entries(providers)) {
            ...
            for (const m of providerConfig.models) {
                const contextTokens = typeof m?.contextTokens === "number"
                    ? m.contextTokens
