...
const defaultTools = createActiviCodingTools({ senderIsOwner: true });
...
describe("createActiviCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createActiviCodingTools({
      config,
    ...
    const anthropicTools = createActiviCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createActiviCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createActiviCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
