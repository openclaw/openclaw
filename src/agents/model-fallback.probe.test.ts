...
async function loadModelFallbackProbeModules() {
	...
	mockedEnsureAuthProfileStore = vi.mocked(
		authProfilesModule.ensureAuthProfileStore,
	);
	mockedGetSoonestCooldownExpiry = vi.mocked(
		authProfilesModule.getSoonestCooldownExpiry,
	);
	...
	mockedResolveAuthProfileOrder = vi.mocked(
		authProfilesModule.resolveAuthProfileOrder,
	);
...
describe("runWithModelFallback – probe logic", () => {
	...
	beforeEach(async () => {
		...
		mockedIsProfileInCooldown.mockImplementation(
			(_store: AuthProfileStore, profileId: string) => {
	...
	it("logs primary metadata on probe success and failure fallback decisions", async () => {
		...
		setLoggerOverride({
			...
			file: path.join(
				os.tmpdir(),
				`openclaw-model-fallback-probe-${Date.now()}.log`,
			),
		});
		...
		expect(fallbackRun).toHaveBeenNthCalledWith(
			2,
			"anthropic",
			"claude-haiku-3-5",
		);
	...
	it("attempts non-primary fallbacks during overloaded cooldown after primary probe failure", async () => {
		await expectProbeFailureFallsBack({
			reason: "overloaded",
			probeError: Object.assign(new Error("service overloaded"), {
				status: 503,
			}),
		});
	});
	...
	it("keeps walking remaining fallbacks after an abort-wrapped RESOURCE_EXHAUSTED probe failure", async () => {
		...
		const run = vi
			...
			.mockRejectedValueOnce(
				Object.assign(new Error("fallback still rate limited"), {
					status: 429,
				}),
			)
			.mockRejectedValueOnce(
				Object.assign(new Error("final fallback still rate limited"), {
					status: 429,
				}),
			);
	...
	it("prunes stale probe throttle entries before checking eligibility", () => {
		...
		expect(_probeThrottleInternals.isProbeThrottleOpen(NOW, "fresh")).toBe(
			false,
		);
