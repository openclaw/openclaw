...
describe("message-tool-only source reply delivery (openclaw-p3j)", () => {
  afterEach(() => {
    setActivePluginRegistry(createTestRegistry());
  });

  function registerSlackTestChannel() {
    setActivePluginRegistry(
      createTestRegistry([
        {
          pluginId: "slack",
  ...
  it("counts an explicit-route send to the current source as source-reply evidence", async () => {
    registerSlackTestChannel();
    const { ctx } = createTestContext();
    const onDeliveredMessageToolOnlySourceReply = vi.fn();
    ctx.params.sourceReplyDeliveryMode = "message_tool_only";
    ctx.params.messageChannel = "slack";
    ctx.params.currentChannelId = "C0B2EDDPW95";
    ctx.params.onDeliveredMessageToolOnlySourceReply = onDeliveredMessageToolOnlySourceReply;
    const toolCallId = "tool-explicit-route-current-source";

    await handleToolExecutionStart(ctx, {
      type: "tool_execution_start",
      toolName: "message",
      toolCallId,
      args: { action: "send", channel: "slack", to: "C0B2EDDPW95", message: "explicit echo" },
    });
    await handleToolExecutionEnd(ctx, {
      type: "tool_execution_end",
      toolName: "message",
  ...
  it("does not count an explicit-route send to a different conversation as source-reply evidence", async () => {
    registerSlackTestChannel();
    const { ctx } = createTestContext();
    const onDeliveredMessageToolOnlySourceReply = vi.fn();
    ctx.params.sourceReplyDeliveryMode = "message_tool_only";
    ...
    ctx.params.currentChannelId = "C0B2EDDPW95";
    ctx.params.onDeliveredMessageToolOnlySourceReply = onDeliveredMessageToolOnlySourceReply;
    const toolCallId = "tool-explicit-route-other-conversation";

    await handleToolExecutionStart(ctx, {
      type: "tool_execution_start",
      toolName: "message",
      toolCallId,
      args: { action: "send", channel: "slack", to: "C-SOMEWHERE-ELSE", message: "explicit echo" },
    });
    await handleToolExecutionEnd(ctx, {
      type: "tool_execution_end",
      toolName: "message",
