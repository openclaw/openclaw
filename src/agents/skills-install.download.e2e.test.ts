...
const originalSmartAgentNeoStateDir = process.env.SMART_AGENT_NEO_STATE_DIR;

afterEach(() => {
  if (originalSmartAgentNeoStateDir === undefined) {
    delete process.env.SMART_AGENT_NEO_STATE_DIR;
  } else {
    process.env.SMART_AGENT_NEO_STATE_DIR = originalSmartAgentNeoStateDir;
  }
});
