 ...
 */
export function detectCursorKeyMode(raw) {
...
export const DEFAULT_PENDING_MAX_OUTPUT = clampWithDefault(readEnvInt("OPENCLAW_BASH_PENDING_MAX_OUTPUT_CHARS"), 30_000, 1_000, 200_000);
...
export function resolveExecTarget(params) {
    ...
    if (requestedTarget &&
        ...
        const allowedConfig = Array.from(new Set(configuredTarget === "auto" &&
...
function maybeNotifyOnExit(session, status) {
    ...
    requestHeartbeatNow(scopedHeartbeatWakeOptions(sessionKey, { reason: "exec-event", coalesceMs: 0 }));
}
        ...
        if (opts.sandbox) {
            ...
            return {
                ...
                stdinMode: backendExecSpec?.stdinMode ??
                    (opts.usePty ? "pipe-open" : "pipe-closed"),
            };
        }
