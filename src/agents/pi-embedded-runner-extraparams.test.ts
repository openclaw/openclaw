...
describe("restricted reasoning_effort mapping (#46622)", () => {
  function runReasoningEffortCase(params: {
    provider: string;
    modelId: string;
  ...
  it.each(["nvidia", "vllm", "sglang"])(
    'remaps reasoning_effort "minimal" to "low" for %s provider',
    (provider) => {
  ...
  it.each(["nvidia", "vllm", "sglang"])(
    'preserves reasoning_effort "low" unchanged for %s provider',
    (provider) => {
      const payload = runReasoningEffortCase({
        provider,
        modelId: "some-model",
  ...
  it.each(["nvidia", "vllm", "sglang"])(
    'preserves reasoning_effort "medium" unchanged for %s provider',
    (provider) => {
      const payload = runReasoningEffortCase({
        provider,
        modelId: "some-model",
  ...
  it.each(["nvidia", "vllm", "sglang"])(
    'preserves reasoning_effort "high" unchanged for %s provider',
    (provider) => {
      const payload = runReasoningEffortCase({
        provider,
        modelId: "some-model",
  ...
  it("does not remap reasoning_effort for non-restricted providers", () => {
    const payload = runReasoningEffortCase({
      provider: "openai",
      modelId: "gpt-5",
      reasoningEffort: "minimal",
    });
    // OpenAI supports "minimal" natively — should pass through unchanged
