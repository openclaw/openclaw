...
import { resolveChannelDefaultBindingPlacement, resolveInboundConversationResolution, } from "../channels/conversation-resolution.js";
...
import { DEFAULT_SUBAGENT_MAX_CHILDREN_PER_AGENT, DEFAULT_SUBAGENT_MAX_SPAWN_DEPTH, } from "../config/agent-limits.js";
...
import { isSubagentSessionKey, normalizeAgentId, parseAgentSessionKey, resolveAgentIdFromSessionKey, } from "../routing/session-key.js";
import { normalizeOptionalLowercaseString, normalizeOptionalString, } from "../shared/string-coerce.js";
import { createRunningTaskRun } from "../tasks/detached-task-runtime.js";
import { listTasksForOwnerKey } from "../tasks/runtime-internal.js";
import { deliveryContextFromSession, formatConversationTarget, resolveConversationDeliveryTarget, } from "../utils/delivery-context.js";
...
import { isSubagentEnvelopeSession, resolveSubagentCapabilities, resolveSubagentCapabilityStore, } from "./subagent-capabilities.js";
...
export function isSpawnAcpAcceptedResult(result) {
...
function countUntrackedActiveAcpRunsForOwner(ownerKey) {
    ...
    const trackedChildSessionKeys = new Set(tasks
    ...
    const activeAcpChildSessionKeys = new Set(tasks.flatMap((task) => {
...
function resolveTargetAcpAgentId(params) {
    ...
    if (requested) {
        ...
        if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
            return {
                ok: false,
                error: `agentId "${requested}" is an OpenClaw config agent, not an ACP harness. ` +
...
function prepareAcpThreadBinding(params) {
    ...
    const placementToUse = pluginPlacement ??
...
function resolveAcpSubagentEnvelopeState(params) {
    ...
    const maxChildren = params.cfg.agents?.defaults?.subagents?.maxChildrenPerAgent ??
    ...
    const requireAgentId = resolveAgentConfig(params.cfg, requesterAgentId)?.subagents?.requireAgentId ??
    ...
    if (params.targetAgentId !== requesterAgentId) {
        const allowAgents = resolveAgentConfig(params.cfg, requesterAgentId)?.subagents?.allowAgents ??
...
async function initializeAcpSpawnRuntime(params) {
    ...
    const initialized = await getAcpSessionManager().initializeSession({
        ...
        runtimeOptions: params.model || params.thinking
            ? {
...
async function bindPreparedAcpThread(params) {
    ...
    if (!binding.conversation.conversationId) {
        throw new Error(params.preparedBinding.placement === "child"
...
function resolveAcpSpawnBootstrapDeliveryPlan(params) {
    ...
    const fallbackThreadId = fallbackThreadIdRaw != null ? normalizeOptionalString(String(fallbackThreadIdRaw)) : undefined;
    ...
    const bindingMatchesRequesterConversation = Boolean(params.requester.origin?.channel &&
    ...
    const useInlineDelivery = hasDeliveryTarget && !params.effectiveStreamToParent && params.spawnMode === "session";
    return {
        useInlineDelivery,
...
export async function spawnAcpDirect(params, ctx) {
    ...
    if (spawnMode === "session" && !requestThreadBinding) {
        return createAcpSpawnFailure({
            ...
            error: 'sessions_spawn(runtime="acp", mode="session") requires thread=true so the ACP session can stay bound to a channel thread. ' +
    ...
    if (!targetAgentResult.ok) {
        return createAcpSpawnFailure({
            status: "error",
            errorCode: params.agentId && normalizeOptionalAgentId(params.agentId)
                ? "runtime_agent_mismatch"
                : "target_agent_required",
    ...
    const parentDeliveryCtx = effectiveStreamToParent && parentSessionKey
        ? deliveryContextFromSession(loadSessionStore(resolveStorePath(cfg.session?.store, {
