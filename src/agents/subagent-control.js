...
import { readLatestAssistantReplySnapshot, waitForAgentRunAndReadUpdatedAssistantReply, } from "./run-wait.js";
import { resolveStoredSubagentCapabilities } from "./subagent-capabilities.js";
import { buildLatestSubagentRunIndex, buildSubagentList, createPendingDescendantCounter, isActiveSubagentRun, resolveSessionEntryForKey, } from "./subagent-list.js";
import { subagentRuns } from "./subagent-registry-memory.js";
import { getLatestSubagentRunByChildSessionKey, listSubagentRunsForController, } from "./subagent-registry-read.js";
import { getSubagentRunsSnapshotForRead } from "./subagent-registry-state.js";
import { clearSubagentRunSteerRestart, countPendingDescendantRuns, markSubagentRunTerminated, markSubagentRunForSteerRestart, replaceSubagentRunAfterSteer, } from "./subagent-registry.js";
...
export function listControlledSubagentRuns(controllerSessionKey) {
    ...
    const filtered = Array.from(latestByChildSessionKey.values()).filter((entry) => {
        const latestControllerSessionKey = entry.controllerSessionKey?.trim() || entry.requesterSessionKey?.trim();
...
async function killSubagentRun(params) {
    ...
    if (cleared.followupCleared > 0 || cleared.laneCleared > 0) {
        logVerbose(`subagents control kill: cleared followups=${cleared.followupCleared} lane=${cleared.laneCleared} keys=${cleared.keys.join(",")}`);
    }
    if (resolved.entry) {
        try {
...
export async function killAllControlledSubagentRuns(params) {
    ...
    return { status: "ok", killed, labels: killedLabels };
}
...
export async function steerControlledSubagentRun(params) {
    ...
    if (cleared.followupCleared > 0 || cleared.laneCleared > 0) {
        logVerbose(`subagents control steer: cleared followups=${cleared.followupCleared} lane=${cleared.laneCleared} keys=${cleared.keys.join(",")}`);
    }
    try {
...
export async function sendControlledSubagentMessage(params) {
    ...
    if (ownershipError) {
        return { status: "forbidden", error: ownershipError };
    }
    if (params.controller.controlScope !== "children") {
        return {
            status: "forbidden",
    ...
    try {
        ...
        return { status: "ok", runId, replyText: result.replyText };
    }
    catch (err) {
        const error = formatErrorMessage(err);
...
export function resolveControlledSubagentTarget(runs, token, options) {
