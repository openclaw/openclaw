...
vi.mock("./pi-embedded-runner/run/attempt.js", async (importOriginal) => {
	const actual =
		await importOriginal<
			typeof import("./pi-embedded-runner/run/attempt.js")
		>();
	return {
		...actual,
...
vi.mock("../infra/backoff.js", async (importOriginal) => {
	...
	return {
		...
		sleepWithAbort: (ms: number, abortSignal?: AbortSignal) =>
			sleepWithAbortMock(ms, abortSignal),
	};
});
...
async function withAgentWorkspace<T>(
	...
	const root = await fs.mkdtemp(
		path.join(os.tmpdir(), "openclaw-model-fallback-"),
	);
...
async function readUsageStats(agentDir: string) {
	...
	return JSON.parse(raw).usageStats as Record<
		string,
...
function expectOpenAiThenGroqAttemptOrder(params?: {
	expectOpenAiAuthProfileId?: string;
}) {
...
function mockAllProvidersOverloaded() {
	runEmbeddedAttemptMock.mockImplementation(async (params: unknown) => {
		...
		if (
			attemptParams.provider === "openai" ||
			attemptParams.provider === "groq"
		) {
			return makeEmbeddedRunnerAttempt({
				assistantTexts: [],
				lastAssistant: buildEmbeddedRunnerAssistant({
...
describe("runWithModelFallback + runEmbeddedPiAgent overload policy", () => {
	it("falls back across providers after overloaded primary failure and persists transient cooldown", async () => {
		await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
			...
			expect(usageStats["openai:p1"]?.failureCounts).toMatchObject({
				overloaded: 1,
			});
	...
	it("surfaces a bounded overloaded summary when every fallback candidate is overloaded", async () => {
		await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
			...
			expect(usageStats["groq:p1"]?.failureCounts).toMatchObject({
				overloaded: 1,
			});
	...
	it("probes a provider already in overloaded cooldown before falling back", async () => {
		await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
			...
			expect(result.provider).toBe("groq");
			expectOpenAiThenGroqAttemptOrder({
				expectOpenAiAuthProfileId: "openai:p1",
			});
		});
	});
