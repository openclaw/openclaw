...
export function resolveBootstrapWarningSignaturesSeen(report?: {
	...
	const seenFromReport = normalizeSeenSignatures(
		truncation?.warningSignaturesSeen,
	);
...
export function analyzeBootstrapBudget(params: {
	...
	const bootstrapTotalMaxChars = normalizePositiveLimit(
		params.bootstrapTotalMaxChars,
	);
	...
	const injectedChars = nonMissing.reduce(
		(sum, file) => sum + file.injectedChars,
		0,
	);
	const totalNearLimit =
		injectedChars >= Math.ceil(bootstrapTotalMaxChars * nearLimitRatio);
	...
	const files = params.files.map((file) => {
		...
		const nearLimit =
			file.rawChars >= Math.ceil(bootstrapMaxChars * nearLimitRatio);
...
export function formatBootstrapTruncationWarningLines(params: {
	...
	const maxFiles =
		...
		Number.isFinite(params.maxFiles) &&
		params.maxFiles > 0
			? Math.floor(params.maxFiles)
			: DEFAULT_BOOTSTRAP_PROMPT_WARNING_MAX_FILES;
	const lines: string[] = [];
	const duplicateNameCounts = params.analysis.truncatedFiles.reduce(
		(acc, file) => {
	...
	for (const file of topFiles) {
		const pct =
			file.rawChars > 0
				? Math.round(
						((file.rawChars - file.injectedChars) / file.rawChars) * 100,
					)
				: 0;
		const causeText =
			file.causes.length > 0
				...
				: "";
		const nameLabel =
			(duplicateNameCounts.get(file.name) ?? 0) > 1 &&
...
export function buildBootstrapPromptWarning(params: {
	...
	if (
		params.previousSignature &&
		!seenSignatures.includes(params.previousSignature)
	) {
		seenSignatures = appendSeenSignature(
			seenSignatures,
			params.previousSignature,
		);
	}
	const hasSeenSignature = Boolean(
		signature && seenSignatures.includes(signature),
	);
	const warningShown =
		params.mode !== "off" &&
		Boolean(signature) &&
		(params.mode === "always" || !hasSeenSignature);
