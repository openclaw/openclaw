...
const DEFAULT_SUBAGENT_ANNOUNCE_TIMEOUT_MS = 60000;
const MAX_TIMER_SAFE_TIMEOUT_MS = 2147000000;
...
async function runAnnounceDeliveryWithRetry(params) {
    ...
    for (;;) {
        ...
        try {
            ...
            defaultRuntime.log(`[warn] Subagent announce ${params.operation} transient failure, retrying ${nextAttempt}/${maxAttempts} in ${Math.round(delayMs / 1000)}s: ${summarizeDeliveryError(err)}`);
            retryIndex += 1;
...
async function waitForSubagentOutputChange(params) {
    ...
    const RETRY_INTERVAL_MS = FAST_TEST_MODE ? FAST_TEST_RETRY_INTERVAL_MS : 100;
    const deadline = Date.now() + Math.max(0, Math.min(params.maxWaitMs, 5000));
...
async function resolveSubagentCompletionOrigin(params) {
    ...
    if (route.mode === "bound" && route.binding) {
        const boundOrigin = {
            ...
            threadId: requesterOrigin?.threadId != null && requesterOrigin.threadId !== ""
    ...
    try {
        ...
        if (params.expectsCompletionMessage &&
            ...
            if (!forceBoundSessionDirectDelivery) {
                ...
                try {
                    const { countPendingDescendantRuns, countPendingDescendantRunsExcludingRun, countActiveDescendantRuns, } = await import("./subagent-registry.js");
                    if (params.currentRunId && typeof countPendingDescendantRunsExcludingRun === "function") {
                        pendingDescendantRuns = Math.max(0, countPendingDescendantRunsExcludingRun(canonicalRequesterSessionKey, params.currentRunId));
                    }
                    else {
                        pendingDescendantRuns = Math.max(0, typeof countPendingDescendantRuns === "function"
        ...
        const shouldDeliverExternally = !params.requesterIsSubagent &&
...
export async function runSubagentAnnounceFlow(params) {
    ...
    try {
        ...
        try {
            ...
            pendingChildDescendantRuns = Math.max(0, typeof countPendingDescendantRuns === "function"
        ...
        if (requesterDepth >= 1 && reply?.trim()) {
            ...
            reply = await waitForSubagentOutputChange({
                sessionKey: params.childSessionKey,
                baselineReply: reply,
                maxWaitMs: Math.max(minReplyChangeWaitMs, Math.min(params.timeoutMs, 2000)),
            });
        }
        // Build status label
