...
describe("sessions_spawn minRunTimeoutSeconds enforcement", () => {
  it("raises agent-supplied timeout to the configured minimum", async () => {
    const tool = createSessionsSpawnTool({ agentSessionKey: "agent:test:main" });
    // Agent passes 30s but config minimum is 120s
    const result = await tool.execute("call-1", { task: "hello", runTimeoutSeconds: 30 });
  ...
  it("allows agent-supplied timeout above the minimum", async () => {
    const tool = createSessionsSpawnTool({ agentSessionKey: "agent:test:main" });
    // Agent passes 300s which is above 120s minimum
  ...
  it("uses config default when agent omits timeout, default respects minimum", async () => {
    const tool = createSessionsSpawnTool({ agentSessionKey: "agent:test:main" });
    // Agent omits timeout; config default is 600s which is above 120s minimum
    const result = await tool.execute("call-3", { task: "hello" });
    expect(result.details).toMatchObject({ status: "accepted" });

    const calls = await getGatewayCalls();
    const agentCall = findLastCall(calls, (call) => call.method === "agent");
    expect(agentCall?.params?.timeout).toBe(600);
  });
});
