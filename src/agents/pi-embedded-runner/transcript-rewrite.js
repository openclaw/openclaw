...
function appendBranchEntry(params) {
    ...
    if (entry.type === "message") {
        return appendMessage(entry.message);
    }
    ...
    if (entry.type === "custom_message") {
        return sessionManager.appendCustomMessageEntry(entry.customType, entry.content, entry.display, entry.details);
    }
    ...
    if (entry.type === "branch_summary") {
        return sessionManager.branchWithSummary(remapEntryId(entry.parentId, rewrittenEntryIds), entry.summary, entry.details, entry.fromHook);
    }
...
export function rewriteTranscriptEntriesInSessionManager(params) {
    const replacementsById = new Map(params.replacements
...
export async function rewriteTranscriptEntriesInSessionFile(params) {
    ...
    try {
        ...
        if (result.changed) {
            ...
            log.info(`[transcript-rewrite] rewrote ${result.rewrittenEntries} entr` +
