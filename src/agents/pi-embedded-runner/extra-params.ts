...
/**
 * Providers that only accept `low`, `medium`, or `high` as `reasoning_effort`
 * values and reject `minimal` (e.g. NVIDIA NIM, vLLM).
 * See: openclaw/openclaw#46622
 */
const RESTRICTED_REASONING_EFFORT_PROVIDERS = new Set(["nvidia", "vllm", "sglang"]);

/**
 * Map `reasoning_effort` values that are not accepted by restricted providers.
 * `minimal` is mapped to `low`; other values pass through unchanged.
 */
function normalizeRestrictedReasoningEffort(value: unknown): unknown {
  if (value === "minimal") {
    return "low";
  }
  return value;
...
/**
 * Create a streamFn wrapper that remaps unsupported `reasoning_effort` values
 * for providers that only accept `low`, `medium`, `high`.
 *
 * NVIDIA NIM and vLLM reject `minimal` with a 400 error:
 *   Input should be 'low', 'medium' or 'high'
 *
 * This wrapper intercepts the outgoing payload and maps `minimal` → `low`.
 * See: openclaw/openclaw#46622
 */
function createRestrictedReasoningEffortWrapper(baseStreamFn: StreamFn | undefined): StreamFn {
  const underlying = baseStreamFn ?? streamSimple;
  return (model, context, options) => {
    const originalOnPayload = options?.onPayload;
    return underlying(model, context, {
      ...
      onPayload: (payload) => {
        if (payload && typeof payload === "object") {
          ...
          if (payloadObj.reasoning_effort !== undefined) {
            payloadObj.reasoning_effort = normalizeRestrictedReasoningEffort(
...
export function applyExtraParamsToAgent(
  ...
  // NVIDIA NIM and vLLM only accept low/medium/high for reasoning_effort —
  // remap unsupported values (e.g. "minimal" → "low") to prevent 400 errors.
  // See: openclaw/openclaw#46622
  if (RESTRICTED_REASONING_EFFORT_PROVIDERS.has(provider)) {
    log.debug(`applying restricted reasoning_effort mapping for ${provider}/${modelId}`);
    agent.streamFn = createRestrictedReasoningEffortWrapper(agent.streamFn);
  }
