...
import { testing as subagentAnnounceTesting } from "../subagent-announce-delivery.js";
...
describe("sessions_yield orchestration", () => {
  ...
  it("end-to-end: yielded parent + child completion via direct-pending → durable inbox → next-turn drain", async () => {
    ...
    const yieldResult = await runEmbeddedPiAgent({
      ...overflowBaseRunParams,
    ...
    expect(isEmbeddedPiRunActive(parentSessionKey)).toBe(false);
    ...
    const steerProbe = queueEmbeddedPiMessageWithOutcome(parentSessionKey, "would-be-steer");
    ...
    // primitive `drainFormattedSystemEvents` wraps — what the auto-reply
    // pipeline calls when building the next prompt for an idle parent.
    ...
    // Cleanup test deps — leaving stubbed deps in place would leak into
    // any subsequent suite that imports subagent-announce-delivery.
    subagentAnnounceTesting.setDepsForTest({});
    resetSystemEventsForTest();
  }, 20_000);
});
