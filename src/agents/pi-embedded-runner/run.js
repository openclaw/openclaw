...
export async function runEmbeddedPiAgent(params) {
    ...
    return enqueueSession(() => enqueueGlobal(async () => {
        ...
        let apiKeyInfo = null;
        let lastProfileId;
        const copilotTokenState = model.provider === "github-copilot" ? { githubToken: "", expiresAt: 0 } : null;
        ...
        const refreshCopilotToken = async (reason) => {
            if (!copilotTokenState) {
                return;
            }
            ...
            copilotTokenState.refreshInFlight = (async () => {
                ...
                log.debug(`Copilot token refreshed; expires in ${Math.max(0, Math.floor(remaining / 1000))}s.`);
            })()
                .catch((err) => {
        ...
        const maybeRefreshCopilotForAuthError = async (errorText, retried) => {
        ...
        try {
            ...
            while (true) {
                ...
                if (authFailure &&
                    (await maybeRefreshCopilotForAuthError(lastAssistant?.errorMessage ?? "", copilotAuthRetry))) {
                    authRetryPending = true;
                    continue;
                }
