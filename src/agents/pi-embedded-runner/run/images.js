...
import { createSandboxBridgeReadFile, resolveSandboxedBridgeMediaPath, } from "../../sandbox-media-paths.js";
...
export function injectHistoryImagesIntoMessages(messages, historyImagesByIndex) {
    ...
    for (const [index, images] of historyImagesByIndex) {
        ...
        for (const image of images) {
            ...
            const hasImage = contentArr.some((block) => typeof block === "object" &&
                block !== null &&
                block.type === "image" &&
                block.data === image.data);
            if (!hasImage) {
                contentArr.push(image);
                didMutate = true;
            }
        }
    }
