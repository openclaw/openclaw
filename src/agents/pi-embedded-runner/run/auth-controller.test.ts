...
type SetRuntimeApiKey =
  Parameters<typeof createEmbeddedRunAuthController>[0]["authStorage"]["setRuntimeApiKey"];
...
type RuntimeApiKeySetter = Mock<SetRuntimeApiKey>;
...
describe("createEmbeddedRunAuthController", () => {
  ...
  it("applies runtime request overrides on the first auth exchange", async () => {
    ...
    const setRuntimeApiKey = vi.fn<SetRuntimeApiKey>();
  ...
  it("rejects privileged runtime transport overrides on the first auth exchange", async () => {
    ...
    const controller = createEmbeddedRunAuthController({
      ...
      authStorage: { setRuntimeApiKey: vi.fn<SetRuntimeApiKey>() },
