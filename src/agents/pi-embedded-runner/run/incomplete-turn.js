...
import { isStrictAgenticSupportedProviderModel, stripProviderPrefix, } from "../../execution-contract.js";
...
export const ACK_EXECUTION_FAST_PATH_INSTRUCTION = "The latest user message is a short approval to proceed. Do not recap or restate the plan. Start with the first concrete tool action immediately. Keep any user-facing follow-up brief and natural.";
export const STRICT_AGENTIC_BLOCKED_TEXT = "Agent stopped after repeated plan-only turns without taking a concrete action. No concrete tool action or external side effect advanced the task.";
function hasStringEntry(values) {
    return (Array.isArray(values) &&
        values.some((value) => typeof value === "string" && value.trim().length > 0));
}
export function hasCommittedUserVisibleToolDelivery(attempt) {
    return (hasStringEntry(attempt.messagingToolSentTexts) ||
...
export function buildAttemptReplayMetadata(params) {
    ...
    const hadPotentialSideEffects = hadMutatingTools ||
...
export function resolveIncompleteTurnPayloadText(params) {
    ...
    if (!incompleteTerminalAssistant &&
...
export function resolveRunLivenessState(params) {
    ...
    if (params.attempt.promptErrorSource === "compaction" ||
        params.attempt.timedOutDuringCompaction) {
        return "paused";
    }
...
export function isReasoningOnlyAssistantTurn(message) {
...
export function isLikelyExecutionAckPrompt(text) {
...
export function extractPlanningOnlyPlanDetails(text) {
    const trimmed = text.trim();
    if (!trimmed) {
        return null;
    }
...
function hasSingleRetrySafeNonPlanTool(toolMetas) {
    ...
    return (nonPlanToolNames.length === 1 &&
...
function isSingleActionThenNarrativePattern(params) {
    ...
    return (SINGLE_ACTION_EXPLICIT_CONTINUATION_RE.test(text) ||
        SINGLE_ACTION_MULTI_STEP_PROMISE_RE.test(text));
}
export function resolvePlanningOnlyRetryLimit(executionContract) {
...
export function resolvePlanningOnlyRetryInstruction(params) {
    ...
    if (!hasStructuredPlanningFormat &&
