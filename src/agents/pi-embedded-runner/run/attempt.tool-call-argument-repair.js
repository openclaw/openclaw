import { createHtmlEntityToolCallArgumentDecodingWrapper, decodeHtmlEntitiesInObject, } from "../../../plugin-sdk/provider-stream-shared.js";
...
function hasMeaningfulToolCallArgumentsInMessage(message, contentIndex) {
    ...
    return (typedBlock.arguments !== null &&
...
function wrapStreamRepairMalformedToolCallArguments(stream) {
    ...
    const hadPreexistingArgsByIndex = new Set();
    ...
    wrapStreamObjectEvents(stream, (event) => {
        if (typeof event.contentIndex === "number" &&
            ...
            const shouldReevaluateRepair = shouldAttemptMalformedToolCallRepair(nextPartialJson, event.delta) ||
            ...
            if (shouldReevaluateRepair) {
                ...
                if (repair) {
                    ...
                    if (!loggedRepairIndices.has(event.contentIndex) && repair.kind === "repaired") {
                        ...
                        log.warn(`repairing malformed tool call arguments with ${repair.leadingPrefix.length} leading chars and ${repair.trailingSuffix.length} trailing chars`);
                    }
                }
                else {
                    repairedArgsByIndex.delete(event.contentIndex);
                    ...
                    const hadPreexistingArgs = hadPreexistingArgsByIndex.has(event.contentIndex) ||
                        (!hadRepairState &&
                            (hasMeaningfulToolCallArgumentsInMessage(event.partial, event.contentIndex) ||
                                hasMeaningfulToolCallArgumentsInMessage(event.message, event.contentIndex)));
...
export function shouldRepairMalformedToolCallArguments(params) {
    return ((normalizeProviderId(params.provider ?? "") === "kimi" &&
