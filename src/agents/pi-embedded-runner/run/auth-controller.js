...
import { resolveProviderRequestConfig, sanitizeRuntimeProviderRequestOverrides, } from "../../provider-request-config.js";
import { clampRuntimeAuthRefreshDelayMs } from "../../runtime-auth-refresh.js";
import { RUNTIME_AUTH_REFRESH_MARGIN_MS, RUNTIME_AUTH_REFRESH_MIN_DELAY_MS, RUNTIME_AUTH_REFRESH_RETRY_MS, } from "./helpers.js";
export function createEmbeddedRunAuthController(params) {
    const applyPreparedRuntimeRequestOverrides = (paramsForApply) => {
        ...
        const runtimeRequestConfig = resolveProviderRequestConfig({
            ...
            providerHeaders: paramsForApply.runtimeModel.headers &&
    ...
    const refreshRuntimeAuth = async (reason) => {
        ...
        refreshPromise = (async () => {
            ...
            if (!preparedAuth?.apiKey) {
                throw new Error(`Provider "${runtimeModel.provider}" does not support runtime auth refresh.`);
            }
            ...
            if (preparedAuth.expiresAt) {
                ...
                params.log.debug(`Runtime auth refreshed for ${runtimeModel.provider}; expires in ${Math.max(0, Math.floor(remaining / 1000))}s.`);
            }
        })()
            .catch((err) => {
            const runtimeModel = params.getRuntimeModel();
    ...
    const throwAuthProfileFailover = (failoverParams) => {
        ...
        const message = failoverParams.message?.trim() ||
    ...
    const initializeAuthProfile = async () => {
        try {
            const autoProfileCandidates = params.profileCandidates.filter((candidate) => typeof candidate === "string" &&
                candidate.length > 0 &&
                candidate !== params.lockedProfileId);
            const modelId = params.getModelId();
            const allAutoProfilesInCooldown = autoProfileCandidates.length > 0 &&
                autoProfileCandidates.every((candidate) => isProfileInCooldown(params.authStore, candidate, undefined, modelId));
            const unavailableReason = allAutoProfilesInCooldown
                ...
                : null;
            const allowTransientCooldownProbe = params.allowTransientCooldownProbe &&
