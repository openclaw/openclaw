...
export function resolveLlmIdleTimeoutMs(params) {
    ...
    if (typeof agentTimeoutSeconds === "number" &&
 ...
 */
export function streamWithIdleTimeout(baseFn, timeoutMs, onIdleTimeout) {
                    ...
                    const createTimeoutPromise = () => {
                        return new Promise((_, reject) => {
                            idleTimer = setTimeout(() => {
                                const error = new Error(`LLM idle timeout (${Math.floor(timeoutMs / 1000)}s): no response from model`);
                                onIdleTimeout?.(error);
                                reject(error);
                            }, timeoutMs);
                        });
