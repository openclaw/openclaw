...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  while ((match = mediaAttachedPattern.exec(prompt)) !== null) {
    ...
    if (pathMatch?.[1]) {
      ...
      // Fallback: match extensionless paths when followed by an image MIME type hint
      // e.g., "[media attached: /tmp/openclaw/media/a3f29b1c (image/jpeg)]"
      // saveMediaBuffer may produce hash-based filenames without extensions
      const mimeMatch = content.match(
        /^\s*(\S+)\s*\(\s*image\/(?:png|jpe?g|gif|webp|bmp|tiff?|heic|heif)\s*\)/i,
      );
      if (mimeMatch?.[1]) {
        const rawPath = mimeMatch[1].trim();
        // Bypass the extension check for extensionless paths detected via MIME hint
        const dedupeKey = normalizeRefForDedupe(rawPath);
        if (rawPath && !seen.has(dedupeKey)) {
          if (!rawPath.startsWith("http://") && !rawPath.startsWith("https://")) {
            seen.add(dedupeKey);
            const resolved = rawPath.startsWith("~") ? resolveUserPath(rawPath) : rawPath;
            refs.push({ raw: rawPath, type: "path", resolved });
          }
        }
      }
    }
