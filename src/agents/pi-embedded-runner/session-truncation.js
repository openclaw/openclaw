...
export async function truncateSessionAfterCompaction(params) {
    ...
    const summarizedBranchIds = new Set();
    ...
    const lines = [JSON.stringify(header), ...keptEntries.map((e) => JSON.stringify(e))];
    ...
    log.info(`[session-truncation] Truncated session file: ` +
