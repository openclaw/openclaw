...
export const PREEMPTIVE_CONTEXT_OVERFLOW_MESSAGE = "Context overflow: estimated context size exceeds safe threshold during tool loop.";
const TOOL_RESULT_ESTIMATE_TO_TEXT_RATIO = 4 / TOOL_RESULT_CHARS_PER_TOKEN_ESTIMATE;
export function formatContextLimitTruncationNotice(truncatedChars) {
...
function truncateToolResultToChars(msg, maxChars, cache) {
    ...
    if (!rawText) {
        const omittedChars = Math.max(1, estimateBudgetToTextBudget(Math.max(estimatedChars - maxChars, 1)));
...
export function installToolResultContextGuard(params) {
    ...
    const maxContextChars = Math.max(1_024, Math.floor(contextWindowTokens * CHARS_PER_TOKEN_ESTIMATE * PREEMPTIVE_OVERFLOW_RATIO));
