...
export function isAnthropicModelRef(modelId) {
...
export function isAnthropicBedrockModel(modelId) {
    ...
    if (BEDROCK_APP_INFERENCE_PROFILE_ARN_RE.test(normalized) &&
...
export function isOpenRouterAnthropicModelRef(provider, modelId) {
    return (normalizeOptionalLowercaseString(provider) === "openrouter" && isAnthropicModelRef(modelId));
}
...
export function resolveAnthropicCacheRetentionFamily(params) {
    ...
    if (normalizedProvider === "amazon-bedrock" &&
        ...
        if (BEDROCK_APP_INFERENCE_PROFILE_ARN_RE.test(normalizeLowercaseStringOrEmpty(params.modelId)) &&
