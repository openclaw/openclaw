...
import { resolveImplicitCopilotProvider } from "../models-config.providers.js";
...
/**
 * Resolve the narrative model from mind-memory plugin config, if set.
 * Returns the Model object and injects its API key into authStorage so that
 * session.compact() (which uses the main modelRegistry) can authenticate it.
 * Returns undefined if narrative model is not configured or resolution fails.
 */
async function resolveNarrativeModelForCompaction(params: {
  ...
  const [narrativeProvider, narrativeModelId] =
    sep > 0
      ? [narrativeModelStr.slice(0, sep), narrativeModelStr.slice(sep + 1)]
      : [DEFAULT_PROVIDER, narrativeModelStr];
  let { model: nModel, error: nError } = resolveModel(
    narrativeProvider,
    narrativeModelId,
    params.agentDir,
    params.config,
  );
  // github-copilot is an implicit provider not registered in models.json; if the sync
  // registry lookup fails, resolve the provider asynchronously and construct the model.
  if ((!nModel || nError) && narrativeProvider === "github-copilot") {
    try {
      const copilotProvider = await resolveImplicitCopilotProvider({
        agentDir: params.agentDir,
      });
      if (copilotProvider) {
        nModel = {
          id: narrativeModelId,
          name: narrativeModelId,
          provider: "github-copilot",
          api: "openai-responses" as const,
          baseUrl: copilotProvider.baseUrl,
          reasoning: false,
          input: ["text" as const],
  ...
  if (!nModel || nError) {
    log.warn(
      `[compaction] could not resolve narrative model for compaction: ${nError ?? "unknown"}`,
    );
    return undefined;
  }
  try {
    const nApiKeyInfo = await getApiKeyForModel({
      model: nModel,
      cfg: params.config,
      profileId: params.authProfileId,
      agentDir: params.agentDir,
    });
    if (!nApiKeyInfo.apiKey) {
      if (nApiKeyInfo.mode !== "aws-sdk") {
        log.warn(
          `[compaction] no API key for narrative model ${narrativeProvider}/${narrativeModelId}; using main model`,
        );
        return undefined;
      }
    } else if (nModel.provider === "github-copilot") {
      const { resolveCopilotApiToken } = await import("../../providers/github-copilot-token.js");
      const copilotToken = await resolveCopilotApiToken({ githubToken: nApiKeyInfo.apiKey });
      params.mainAuthStorage.setRuntimeApiKey(nModel.provider, copilotToken.token);
    } else {
      params.mainAuthStorage.setRuntimeApiKey(nModel.provider, nApiKeyInfo.apiKey);
    }
    return nModel;
  } catch (err) {
    log.warn(
      `[compaction] failed to resolve narrative model API key: ${String(err)}; using main model`,
    );
    return undefined;
  }
}
...
export async function compactEmbeddedPiSessionDirect(
  ...
  // Resolve narrative model for compaction (fire-and-forget on failure — falls back to main model)
  const compactionModel = await resolveNarrativeModelForCompaction({
    config: params.config,
    agentDir,
    authProfileId: params.authProfileId,
    mainAuthStorage: authStorage,
  });
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // Temporarily swap to narrative model if configured, so the summary is
        // generated by the cheaper/dedicated model rather than the main agent model.
        const originalModel = session.model;
        if (compactionModel) {
          log.info(
            `[compaction] using auxiliary model: ${compactionModel.provider}/${compactionModel.id}`,
          );
          session.agent.setModel(compactionModel);
        } else {
          log.warn(
            `[compaction] auxiliaryModel resolution failed — using main model: ${originalModel?.provider ?? "?"}/${originalModel?.id ?? "?"}`,
          );
        }
        let compactionResult;
        try {
          compactionResult = await compactWithSafetyTimeout(() =>
            session.compact(params.customInstructions),
          );
        } finally {
          if (compactionModel && originalModel) {
            session.agent.setModel(originalModel);
          }
        }
        // compactionResult is always defined here — if compact() threw, we never reach this line
