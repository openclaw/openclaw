...
import { resolveSmartAgentNeoDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveSmartAgentNeoAgentDir();
  await ensureSmartAgentNeoModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createSmartAgentNeoCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveSmartAgentNeoDocsPath({
      workspaceDir: effectiveWorkspace,
