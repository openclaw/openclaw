...
import { resolveDNADocsPath } from "../docs-path.js";
import type { ExecElevatedDefaults } from "../bash-tools.js";
import { DEFAULT_MODEL, DEFAULT_PROVIDER } from "../defaults.js";
import { getApiKeyForModel, resolveModelAuthMode } from "../model-auth.js";
import { ensureDNAModelsJson } from "../models-config.js";
import {
  ensureSessionHeader,
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveDNAAgentDir();
  await ensureDNAModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createDNACodingTools({
      exec: {
    ...
    const docsPath = await resolveDNADocsPath({
      workspaceDir: effectiveWorkspace,
