...
function createStreamFnWithExtraParams(baseStreamFn, extraParams, provider) {
    ...
    const providerRouting = provider === "openrouter" &&
...
function resolveOpenAIResponsesCompactThreshold(model) {
    ...
    if (contextWindow) {
        return Math.max(1000, Math.floor(contextWindow * 0.7));
    }
    return 80000;
}
 ...
 */
export function applyExtraParamsToAgent(agent, cfg, provider, modelId, extraParamsOverride, thinkingLevel, agentId) {
