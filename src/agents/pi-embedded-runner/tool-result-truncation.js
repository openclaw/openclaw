...
export function calculateMaxToolResultCharsWithCap(contextWindowTokens, hardCapChars) {
 ...
 */
export function truncateOversizedToolResultsInMessages(messages, contextWindowTokens, maxCharsOverride) {
...
function buildAggregateToolResultReplacements(params) {
    ...
    const candidates = params.branch
        ...
        .filter((item) => item.entry.type === "message" &&
 ...
 */
export function isOversizedToolResult(msg, contextWindowTokens, maxCharsOverride) {
    if (msg.role !== "toolResult") {
        return false;
    }
    const maxChars = Math.max(1, maxCharsOverride ?? calculateMaxToolResultChars(contextWindowTokens));
