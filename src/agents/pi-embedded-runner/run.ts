...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      // Forward global agent events that match this run to params.onAgentEvent.
      // This allows plugins/hooks emitting onAgentEvent({ runId }) to reach callers.
      const cleanupGlobalEvents = onAgentEvent((evt) => {
      ...
      try {
        ...
        while (true) {
          ...
          const prompt = pluginExtraSystemPrompt
            ? `${pluginExtraSystemPrompt}\n\n${basePrompt}`
            : basePrompt;
