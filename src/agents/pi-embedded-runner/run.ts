    ...
    enqueueGlobal(async () => {
      ...
      const resolveAuthProfileFailoverReason = (params: {
        ...
        if (params.allInCooldown) {
          const latestCooldownReason = profileCandidates
            .filter((candidate): candidate is string => typeof candidate === "string")
            .map((candidate) => authStore.usageStats?.[candidate])
            .filter((stats): stats is NonNullable<typeof stats> => Boolean(stats))
            .toSorted(
              (a, b) => (b.lastFailureAt ?? 0) - (a.lastFailureAt ?? 0),
            )[0]?.lastFailureReason;
          if (latestCooldownReason && latestCooldownReason !== "unknown") {
            return latestCooldownReason;
          }
          const classifiedCooldownReason = classifyFailoverReason(params.message);
          return classifiedCooldownReason ?? "unknown";
        }
          ...
          if (shouldRotate) {
            if (lastProfileId) {
              ...
              if (timedOut && !isProbeSession) {
                log.warn(
                  `Profile ${lastProfileId} timed out. Marking profile cooldown and trying next account...`,
                );
              }
