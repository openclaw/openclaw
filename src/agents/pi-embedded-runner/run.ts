...
export async function runEmbeddedPiAgent(
  ...
  const providerLane = `provider:${normalizeProviderId((params.provider ?? DEFAULT_PROVIDER).trim() || DEFAULT_PROVIDER)}`;
  const enqueueProvider =
    params.enqueue ?? ((task, opts) => enqueueCommandInLane(providerLane, task, opts));
