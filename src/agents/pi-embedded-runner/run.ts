...
import { runWithDebugProxyCaptureContext } from "../../proxy-capture/runtime.js";
...
function buildDebugProxyRuntimeCaptureContext(params: RunEmbeddedPiAgentParams): {
  sessionKey?: string;
  ...
  const resolvedMessageId =
    params.currentMessageId != null
      ? normalizeOptionalString(String(params.currentMessageId))
      : undefined;
  const normalizedSessionKey = normalizeOptionalString(params.sessionKey);
  return {
    sessionKey: normalizedSessionKey,
    parentSessionKey: normalizeOptionalString(params.spawnedBy),
    topLevelUserRequestId: resolvedMessageId,
    messageId: resolvedMessageId,
    agentId: normalizedSessionKey ?? normalizeOptionalString(params.agentId),
    agentName: normalizeOptionalString(params.agentId),
    spawnedBy: normalizeOptionalString(params.spawnedBy),
    trigger: normalizeOptionalString(params.trigger),
    inputProvenanceKind: normalizeOptionalString(params.inputProvenance?.kind),
    originSessionId: normalizeOptionalString(params.inputProvenance?.originSessionId),
    sourceSessionKey: normalizeOptionalString(params.inputProvenance?.sourceSessionKey),
    sourceTool: normalizeOptionalString(params.inputProvenance?.sourceTool),
  };
}
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(async () => {
      return runWithDebugProxyCaptureContext(
        buildDebugProxyRuntimeCaptureContext(params),
        async () => {
          throwIfAborted();
