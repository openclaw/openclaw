...
export async function runEmbeddedPiAgent(
  ...
  const notifyLaneWait =
    (scope: "session" | "global", lane: string) =>
    (waitMs: number, queuedAhead: number): void => {
      if (!params.onCommandLaneWait) {
        return;
      }
      const payload = {
        lane,
      ...
      try {
        const maybePromise = params.onCommandLaneWait(payload);
        if (maybePromise && typeof (maybePromise as Promise<unknown>).then === "function") {
          void (maybePromise as Promise<unknown>).catch((error) => {
            log.debug(
              `[lane-wait] callback failed run=${params.runId} lane=${lane} scope=${scope}: ${String(error)}`,
            );
          });
        }
      } catch (error) {
  ...
  return enqueueSession(
    () =>
      enqueueGlobal(
        ...
        { onWait: notifyLaneWait("global", globalLane) },
      ),
    { onWait: notifyLaneWait("session", sessionLane) },
  );
}
