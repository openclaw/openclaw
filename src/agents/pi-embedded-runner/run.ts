      ...
      try {
        ...
        while (true) {
          ...
          if (promptError && !aborted) {
            ...
            // message and complete the run so polling does not stall (#43607). Match all
            // variants that failover-matches recognizes (stop reason / reason: sensitive).
            if (
              /\b(?:unhandled\s+)?stop\s+reason:\s*sensitive\b|\breason:\s*sensitive\b/i.test(
                errorText,
              )
            ) {
              return {
