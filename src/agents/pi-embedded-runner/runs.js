import { abortActiveReplyRuns, abortReplyRunBySessionId, getActiveReplyRunCount, isReplyRunActiveForSessionId, isReplyRunStreamingForSessionId, listActiveReplyRunSessionIds, queueReplyRunMessage, resolveActiveReplyRunSessionId, waitForReplyRunEndBySessionId, } from "../../auto-reply/reply/reply-run-registry.js";
...
export function abortEmbeddedPiRun(sessionId, opts) {
...
export function resolveActiveEmbeddedRunSessionId(sessionKey) {
    const normalizedSessionKey = sessionKey.trim();
    if (!normalizedSessionKey) {
        return undefined;
    }
    return (resolveActiveReplyRunSessionId(normalizedSessionKey) ??
...
export function getActiveEmbeddedRunSnapshot(sessionId) {
    return ACTIVE_EMBEDDED_RUN_SNAPSHOTS.get(sessionId);
}
export function requestEmbeddedRunModelSwitch(sessionId, request) {
    ...
    diag.debug(`model switch requested: sessionId=${normalizedSessionId} provider=${provider} model=${model}`);
    return true;
}
export function consumeEmbeddedRunModelSwitch(sessionId) {
    const normalizedSessionId = sessionId.trim();
 ...
 */
export async function waitForActiveEmbeddedRuns(timeoutMs, opts) {
    const pollMsRaw = opts?.pollMs ?? 250;
    const pollMs = Math.max(10, Math.floor(pollMsRaw));
    if (timeoutMs !== undefined && timeoutMs <= 0) {
        return { drained: getActiveEmbeddedRunCount() === 0 };
    }
    const maxWaitMs = typeof timeoutMs === "number" && Number.isFinite(timeoutMs)
...
export function updateActiveEmbeddedRunSnapshot(sessionId, snapshot) {
    if (!ACTIVE_EMBEDDED_RUNS.has(sessionId)) {
        return;
    }
