...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("mullusi-bundled-");
  const workspaceDir = await createTempDir("mullusi-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: MullusiConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.MULLUSI_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
