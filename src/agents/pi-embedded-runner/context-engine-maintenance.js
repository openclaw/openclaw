...
import { completeTaskRunByRunId, createQueuedTaskRun, failTaskRunByRunId, recordTaskRunProgressByRunId, setDetachedTaskDeliveryStatusByRunId, startTaskRunByRunId, } from "../../tasks/detached-task-runtime.js";
import { cancelTaskByIdForOwner, findTaskByRunIdForOwner, updateTaskNotifyPolicyForOwner, } from "../../tasks/task-owner-access.js";
import { findActiveSessionTask } from "../session-async-task-status.js";
import { resolveSessionLane } from "./lanes.js";
import { log } from "./logger.js";
import { rewriteTranscriptEntriesInSessionFile, rewriteTranscriptEntriesInSessionManager, } from "./transcript-rewrite.js";
...
const activeDeferredTurnMaintenanceRuns = new Map();
...
export function createDeferredTurnMaintenanceAbortSignal(params) {
    ...
    const handleTerminationSignal = (signalName) => {
        const shouldReraise = typeof processLike.listenerCount === "function"
            ? processLike.listenerCount(signalName) === 1
            : false;
        for (const activeController of state.controllers) {
            if (!activeController.signal.aborted) {
                activeController.abort(new Error(`received ${signalName} while waiting for deferred maintenance`));
            }
        }
...
export function resetDeferredTurnMaintenanceStateForTest() {
        ...
        for (;;) {
            while (getQueueSize(sessionLane) > 0) {
                ...
                if (now - startedWaitingAt >= TURN_MAINTENANCE_LONG_WAIT_MS &&
                    ...
                    surfaceMaintenanceUpdate("Waiting for the session lane to go idle.", surfacedUserNotice
...
function scheduleDeferredTurnMaintenance(params) {
    ...
    log.info(`[context-engine] deferred turn maintenance ${reusableTask ? "resuming" : "queued"} ` +
