...
describe("model-selection", () => {
  ...
  describe("buildAllowedModelSet", () => {
    ...
    it("resolves fallback aliases through the caller's manifest normalization context", () => {
      // Regression: the alias-first fallback index must honor the manifest
      // plugins the caller passed, not silently fall back to the global
      // snapshot. /models callers pass a captured snapshot, so an alias whose
      // target needs that normalization would otherwise list under the raw,
      // pre-normalization model id.
      const cfg: OpenClawConfig = {
        agents: {
          defaults: {
            model: {
            ...
            models: {
              "openai-codex/gpt-5.5": {},
              "anthropic/opus-special": { alias: "my-opus" },
            },
          },
        },
      } as unknown as OpenClawConfig;
      ...
      const result = buildAllowedModelSet({
        ...
        defaultProvider: "openai-codex",
        defaultModel: "gpt-5.5",
        // Normalization supplied only via the caller's snapshot; the global
        // test snapshot has no "opus-special" alias.
        manifestPlugins: [
          {
            modelIdNormalization: {
              providers: {
                anthropic: { aliases: { "opus-special": "claude-opus-4-6" } },
              },
            },
          },
        ],
      ...
      expect(result.allowedKeys.has("anthropic/claude-opus-4-6")).toBe(true);
      expect(result.allowedKeys.has("anthropic/opus-special")).toBe(false);
    });
  });
