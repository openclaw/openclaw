...
import { hasBinary as defaultHasBinary, loadWorkspaceSkillEntries as defaultLoadWorkspaceSkillEntries, resolveSkillsInstallPreferences as defaultResolveSkillsInstallPreferences, } from "./skills.js";
...
async function installGoViaApt(timeoutMs) {
    ...
    if (!getSkillsInstallDeps().hasBinary("sudo")) {
        return createInstallFailure({
            message: "go not installed — apt-get is available but sudo is not installed. Install manually: https://go.dev/doc/install",
        });
    }
...
export async function installSkill(params) {
    ...
    if (!trustedInstallSources.has(skillSource)) {
        warnings.push(`WARNING: Skill "${params.skillName}" install triggered from non-bundled source "${skillSource}". Verify the install recipe is trusted.`);
    }
    if (!spec) {
        return withWarnings({
            ok: false,
