...
vi.mock("./pi-embedded.js", () => ({
	abortEmbeddedPiRun: (...args: unknown[]) =>
		state.abortEmbeddedPiRunMock(...args),
}));
...
describe("live model switch", () => {
	beforeEach(() => {
		...
		state.resolveStorePathMock
			.mockReset()
			.mockReturnValue("/tmp/session-store.json");
	});
	...
	it("resolves persisted session overrides ahead of agent defaults", async () => {
		...
		expect(state.resolveStorePathMock).toHaveBeenCalledWith(
	...
	it("queues a live switch only when an active run was aborted", async () => {
		...
		expect(state.requestEmbeddedRunModelSwitchMock).toHaveBeenCalledWith(
			"session-1",
			{
				provider: "openai",
				model: "gpt-5.4",
	...
	it("does not track persisted live selection when the run started on a transient model override", async () => {
		const { shouldTrackPersistedLiveSessionModelSelection } =
			await loadModule();
