...
async function withOutsideHardlinkInMullusiTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside mullusi tmp root",
      media: path.join(os.tmpdir(), "outside-mullusi", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Mullusi tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Mullusi tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMullusiTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Mullusi tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMullusiTmp(
