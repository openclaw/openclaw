...
export function normalizeUsage(raw) {
    ...
    const usesOpenAIStylePromptTotals = raw.cached_tokens !== undefined ||
    ...
    const normalizedInput = rawInput !== undefined && usesOpenAIStylePromptTotals && cacheRead !== undefined
        ? rawInput - cacheRead
        : rawInput;
    const input = normalizeTokenCount(normalizedInput);
    const output = normalizeTokenCount(raw.output ??
        raw.outputTokens ??
        raw.output_tokens ??
 ...
 */
export function toOpenAiChatCompletionsUsage(usage) {
    ...
    const aggregateTotal = typeof aggregateRaw === "number" && Number.isFinite(aggregateRaw)
        ? Math.max(0, aggregateRaw)
        : undefined;
    const totalTokens = aggregateTotal !== undefined ? Math.max(componentTotal, aggregateTotal) : componentTotal;
    return {
