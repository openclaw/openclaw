...
function extractToolInput(toolName: string, args: unknown): Record<string, unknown> | undefined {
  ...
  if (n === "exec" || n === "bash") {
    ...
    if (cmd != null) return { command: cmd };
  }
  // read / write
  if (n === "read" || n === "write") {
    const p = a.path ?? a.file ?? a.filePath ?? a.file_path;
    if (p != null) return { path: p };
  }
  // edit
  if (n === "edit") {
    const p = a.path ?? a.file ?? a.filePath ?? a.file_path;
  ...
  if (n === "message") {
    ...
    if (a.action != null) result.action = a.action;
    if (a.target != null) result.target = a.target;
    if (a.channel != null) result.channel = a.channel;
    return Object.keys(result).length > 0 ? result : undefined;
  }
  // tts
  if (n === "tts") {
    const t = a.text;
    if (t != null) return { text: typeof t === "string" ? t.slice(0, 100) : t };
  }
  // image_generate / image
  if (n === "image_generate" || n === "image") {
    const p = a.prompt;
    if (p != null) return { prompt: typeof p === "string" ? p.slice(0, 100) : p };
  }
  // memory_search
  if (n === "memory_search") {
    const q = a.query;
  ...
  if (n === "sessions_spawn" || n === "sessions_send") {
    const result: Record<string, unknown> = {};
    if (a.agentId != null) result.agentId = a.agentId;
    if (a.label != null) result.label = a.label;
    if (a.sessionKey != null) result.sessionKey = a.sessionKey;
    return Object.keys(result).length > 0 ? result : undefined;
  }
  return undefined;
}
