...
import { __testing, createSmartAgentNeoCodingTools } from "./pi-tools.js";
...
describe("createSmartAgentNeoCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSmartAgentNeoCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "smart-agent-neo-outside.txt");
