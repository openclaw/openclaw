...
export function buildSubagentList(params: {
  ...
  const buildListEntry = (entry: SubagentRunRecord, runtimeMs: number) => {
    ...
    const runtime = formatDurationCompact(runtimeMs) ?? "0s";
...
> {
  const controlTimeoutMs = resolveSubagentControlTimeoutMs(params.cfg);
  const settleGatewayTimeoutMs = resolveSubagentWaitGatewayTimeoutMs(
    params.cfg,
  ...
  try {
    await subagentControlDeps.callGateway({
      ...
      timeoutMs: settleGatewayTimeoutMs,
    });
  } catch {
