...
import { __testing, createDNACodingTools } from "./pi-tools.js";
...
describe("createDNACodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createDNACodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "dna-outside.txt");
