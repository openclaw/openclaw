...
function isReplayableReasoningType(
	value: unknown,
): value is "reasoning" | `reasoning.${string}` {
	return (
		typeof value === "string" &&
		(value === "reasoning" || value.startsWith("reasoning."))
	);
}
...
function encodeThinkingSignature(
	signature: ReplayableReasoningSignature,
): string {
...
export function planTurnInput(params: {
	...
	if (params.previousResponseId && params.lastContextLength > 0) {
		...
		return {
			...
			inputItems: convertMessagesToInputItems(
