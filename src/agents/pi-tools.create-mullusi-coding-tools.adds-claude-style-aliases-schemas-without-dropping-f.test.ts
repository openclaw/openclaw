...
describe("createMullusiCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-alias-"));
    try {
      const tools = createMullusiCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts broader file/edit alias variants", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-alias-broad-"));
    try {
      const tools = createMullusiCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-structured-write-"));
    try {
      const tools = createMullusiCodingTools({ workspaceDir: tmpDir });
      ...
      await writeTool?.execute("tool-structured-write", {
        ...
        content: [
          ...
          { type: "input_text", text: "const root = path.join(process.env.HOME, 'mullusi');\n" },
        ],
      });

      const written = await fs.readFile(path.join(tmpDir, "structured-write.js"), "utf8");
      expect(written).toBe(
        "const path = require('path');\nconst root = path.join(process.env.HOME, 'mullusi');\n",
      );
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-structured-edit-"));
    try {
