...
import { formatExecDeniedUserMessage, isExecDeniedResultText, parseExecApprovalResultText, } from "./exec-approval-result.js";
...
export function buildExecApprovalFollowupPrompt(resultText) {
...
export async function sendExecApprovalFollowup(params) {
    ...
    const sessionOnlyOriginChannel = normalizedTurnSourceChannel && isGatewayMessageChannel(normalizedTurnSourceChannel)
        ? normalizedTurnSourceChannel
        : undefined;
    let sessionError = null;
    if (sessionKey) {
        try {
            await callGatewayTool("agent", { timeoutMs: 60_000 }, buildAgentFollowupArgs({
