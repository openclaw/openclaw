...
export function reconcileOrphanedRun(params: {
  ...
  const now = Date.now();
  let changed = false;
  // NOTE: Earlier revisions attempted a transcript-based "recovered completed"
  // path here using `hasCompletedTranscript`.  That helper required a known
  // `sessionId` to locate the transcript, but the only orphan reasons reaching
  // this function are `missing-session-entry` and `missing-session-id` — both of
  // which imply no sessionId is available.  The recovery branch was therefore
  // unreachable, so it has been removed.  If we later need to rescue runs that
  // completed but were left registry-orphaned, add a new orphan reason (e.g.
  // reached from a path where sessionId is still known) and reintroduce a
  // targeted recovery step there.
