...
describe("appendPrioritizedDynamicLiveModels", () => {
  ...
  it("materializes prioritized refs from provider dynamic model hooks", async () => {
    ...
    expect(prepareParam?.context.providerConfig).toBe(config.models?.providers?.[DYNAMIC_PROVIDER]);
    ...
    expect(resolveParam?.context.providerConfig).toBe(config.models?.providers?.[DYNAMIC_PROVIDER]);
