...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SMART_AGENT_NEO_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SMART_AGENT_NEO_HOME: "/srv/smart-agent-neo-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/smart-agent-neo-home"), ".smart-agent-neo", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".smart-agent-neo", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("smart-agent-neo-workspace-");
