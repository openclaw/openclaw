...
export function escapeInternalRuntimeContextDelimiters(value) {
    return value
...
export function stripInternalRuntimeContext(text) {
    if (!text) {
        return text;
    }
    const withoutDelimitedBlocks = stripDelimitedBlock(text, INTERNAL_RUNTIME_CONTEXT_BEGIN, INTERNAL_RUNTIME_CONTEXT_END);
    return stripLegacyInternalRuntimeContext(withoutDelimitedBlocks);
}
export function hasInternalRuntimeContext(text) {
    if (!text) {
        return false;
    }
    return (findDelimitedTokenIndex(text, INTERNAL_RUNTIME_CONTEXT_BEGIN, 0) !== -1 ||
