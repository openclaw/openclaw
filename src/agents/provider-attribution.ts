...
const MULLUSI_ATTRIBUTION_PRODUCT = "Mullusi";
const MULLUSI_ATTRIBUTION_ORIGINATOR = "mullusi";
...
function formatMullusiUserAgent(version: string): string {
  return `${MULLUSI_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: MULLUSI_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: MULLUSI_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatMullusiUserAgent(identity.version),
    },
  };
}
