...
let createMullusiCodingTools: typeof import("./pi-tools.js").createMullusiCodingTools;

describe("createMullusiTools plugin context", () => {
  beforeEach(async () => {
    resolvePluginToolsMock.mockClear();
    vi.resetModules();
    ({ createMullusiTools } = await import("./mullusi-tools.js"));
    ({ createMullusiCodingTools } = await import("./pi-tools.js"));
  });
  ...
  it("does not inject messageThreadId defaults for missing params objects", async () => {
    ...
    const wrapped = createMullusiTools({
      config: {} as never,
      agentThreadId: "77",
