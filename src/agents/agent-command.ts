...
function normalizeExplicitOverrideInput(
	raw: string,
	kind: "provider" | "model",
): string {
	...
	if (trimmed.length > OVERRIDE_VALUE_MAX_LENGTH) {
		throw new Error(
			`${label} override exceeds ${String(OVERRIDE_VALUE_MAX_LENGTH)} characters.`,
		);
	}
	...
	try {
		...
		const explicitModelOverride =
			typeof opts.model === "string"
				...
				: undefined;
		const hasExplicitRunOverride = Boolean(
		...
		const needsModelCatalog =
			hasAllowlist || hasStoredOverride || hasExplicitRunOverride;
		...
		try {
			...
			const fallbackResult = await runWithModelFallback({
				...
				run: async (providerOverride, modelOverride, runOptions) => {
					...
					return runAgentAttempt({
						...
						sessionHasHistory:
							!isNewSession || (await sessionFileHasContent(sessionFile)),
						onAgentEvent: (evt) => {
			...
			if (!lifecycleEnded) {
				...
				if (stopReason && stopReason !== "end_turn") {
					console.error(
						`[agent] run ${runId} ended with stopReason=${stopReason}`,
					);
				}
				emitAgentEvent({
					runId,
