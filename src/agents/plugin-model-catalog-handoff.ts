...
export async function capturePluginModelCatalogHandoff(
  ...
  // Best-effort metadata: a store this process cannot read (missing, owned by
  // another agent id, unreadable) carries no catalogs instead of failing a run
  // that never needed them.
  let catalogs: PersistedPluginModelCatalog[];
  try {
