...
const DEFAULT_PROVIDER_CATALOG_ROOT = path.resolve(
	import.meta.dirname,
	"../..",
);
...
let providerCatalogEntriesCache: ReadonlyArray<BundledProviderCatalogEntry> | null =
	null;
let providerCatalogModulesPromise: Promise<
	Readonly<Record<string, ProviderCatalogModule>>
> | null = null;
let providerCatalogExportMapPromise: Promise<
	Readonly<ProviderCatalogExportMap>
> | null = null;
...
export function resolveBundledProviderCatalogEntries(params?: {
	...
	for (const entry of listBundledPluginMetadata({ rootDir })) {
		if (
			!entry.publicSurfaceArtifacts?.includes(
				PROVIDER_CATALOG_ARTIFACT_BASENAME,
			)
		) {
			continue;
		}
