...
describe("OpenAI WebSocket e2e", () => {
	...
	testFn(
		...
		async () => {
			...
			const streamFn = openAIWsStreamModule.createOpenAIWebSocketStreamFn(
				API_KEY!,
				sid,
			);
	...
	testFn(
		...
		async () => {
			...
			const toolCall = firstDone.content.find(
		...
		async () => {
			...
			openAIWsStreamModule.__testing.setDepsForTest({
				createManager: (options) => {
					const manager = new openAIWsConnectionModule.OpenAIWebSocketManager(
						options,
					);
			...
			expect(
				thinkingBlocks.map((block) =>
					parseReasoningSignature(block.thinkingSignature),
				),
			).toEqual(
				replayableReasoningItems.map((item) =>
					toExpectedReasoningSignature(item),
				),
			);
	...
	testFn(
		...
		async () => {
			...
			await collectEvents(
				streamFn(model, makeContext("Say hello."), { transport: "websocket" }),
			);
