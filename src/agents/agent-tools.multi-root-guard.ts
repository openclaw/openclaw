...
async function validateCanonicalTargetAgainstRoots(
  ...
  const canonicalPath = path.resolve(resolvePathViaExistingAncestorSync(resolvedPath));
  const canonicalRoots = sortRootsForMatching(
    roots.map((root) => ({
      ...root,
      resolvedPath: path.resolve(resolvePathViaExistingAncestorSync(root.resolvedPath)),
    })),
  );
