...
vi.mock("./pi-embedded-runner/model.js", () => ({
	resolveModelWithRegistry: (...args: unknown[]) =>
		resolveModelWithRegistryMock(...args),
}));
...
vi.mock("./pi-embedded-runner/runs.js", () => ({
	getActiveEmbeddedRunSnapshot: (...args: unknown[]) =>
		getActiveEmbeddedRunSnapshotMock(...args),
}));
...
describe("runBtwSideQuestion", () => {
	...
	it("forces provider reasoning off even when the session think level is adaptive", async () => {
		streamSimpleMock.mockImplementation(
			(_model, _input, options?: { reasoning?: unknown }) => {
	...
	it("fails when the current branch has no messages", async () => {
		...
		await expect(runSideQuestion()).rejects.toThrow(
			...
			messages: [
				{
					...
					content: [
						{
							type: "text",
							text: "write some things then wait 30 seconds and write more",
						},
					],
					timestamp: 1,
				},
	...
	it("uses the in-flight prompt as background only when there is no prior transcript context", async () => {
		...
		expect(result).toEqual({
			text: "You're building a tic-tac-toe game in Brainfuck.",
		});
		expect(streamSimpleMock).toHaveBeenCalledWith(
			expect.anything(),
			expect.objectContaining({
	...
	it("falls back when the active run snapshot leaf no longer exists", async () => {
		...
		expect(diagDebugMock).toHaveBeenCalledWith(
			expect.stringContaining(
				"btw snapshot leaf unavailable: sessionId=session-1",
			),
		);
	});
	...
	it("excludes tool results from BTW context to avoid replaying raw tool output", async () => {
		...
		expect(
			(context as { messages?: Array<{ role?: string }> }).messages,
		).not.toEqual(
