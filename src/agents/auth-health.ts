...
export function resolveAuthProfileSource(
	_profileId: string,
): AuthProfileSource {
	return "store";
}
...
export function buildAuthHealthSummary(params: {
	...
	const providerFilter = params.providers
		? new Set(
				params.providers.map((p) => normalizeProviderId(p)).filter(Boolean),
			)
		: null;
	...
	const profiles = Object.entries(params.store.profiles)
		.filter(([_, cred]) =>
			providerFilter
				? providerFilter.has(normalizeProviderId(cred.provider))
				: true,
		)
