...
export function resolveTranscriptPolicy(params: {
  ...
  // so always drop them.  OpenAI (and OpenAI-compatible providers like
  // github-copilot) need reasoning blocks for tool-id pairing, so keep them.
  // Drop for all other providers (Gemini, Mistral, etc.) that do not
  // understand thinking blocks.
  const dropThinkingBlocks = !isOpenAi && !isOpenAiApi(params.modelApi);
