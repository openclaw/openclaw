...
function resolveLiveProviderCatalogTimeoutMs(
	env: NodeJS.ProcessEnv,
): number | null {
	const live =
		env.OPENCLAW_LIVE_TEST === "1" ||
		env.OPENCLAW_LIVE_GATEWAY === "1" ||
		env.LIVE === "1";
	if (!live) {
		return null;
	}
...
function resolveLiveProviderDiscoveryFilter(
	env: NodeJS.ProcessEnv,
): string[] | undefined {
	const live =
		env.OPENCLAW_LIVE_TEST === "1" ||
...
async function resolvePluginImplicitProviders(
	...
	for (const provider of byOrder[order]) {
				...
				!findNormalizedProviderValue(
					{
						[provider.id]: true,
						...Object.fromEntries(
							(provider.aliases ?? []).map((alias) => [alias, true]),
						),
						...Object.fromEntries(
							(provider.hookAliases ?? []).map((alias) => [alias, true]),
						),
					},
					resolvedProviderId,
				)
		...
		for (const [providerId, implicitProvider] of Object.entries(
			normalizedResult,
		)) {
...
function buildPluginCatalogConfig(
	ctx: ImplicitProviderContext,
): OpenClawConfig {
	if (
		!ctx.explicitProviders ||
		Object.keys(ctx.explicitProviders).length === 0
	) {
		return ctx.config ?? {};
	}
	return {
...
export async function resolveImplicitProviders(
	...
	for (const order of PLUGIN_DISCOVERY_ORDERS) {
		mergeImplicitProviderSet(
			providers,
			await resolvePluginImplicitProviders(context, order),
		);
	}
