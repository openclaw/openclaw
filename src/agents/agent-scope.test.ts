...
describe("resolveAgentConfig", () => {
	...
	it("resolves explicit and effective model primary separately", () => {
		...
		expect(
			resolveAgentExplicitModelPrimary(cfgWithStringDefault, "main"),
		).toBeUndefined();
		expect(
			resolveAgentEffectiveModelPrimary(cfgWithStringDefault, "main"),
		...
		expect(
			resolveAgentExplicitModelPrimary(cfgWithObjectDefault, "main"),
		).toBeUndefined();
		expect(
			resolveAgentEffectiveModelPrimary(cfgWithObjectDefault, "main"),
		).toBe("openai/gpt-5.4");
		...
		expect(
			resolveAgentExplicitModelPrimary(cfgNoDefaults, "main"),
		).toBeUndefined();
		expect(
			resolveAgentEffectiveModelPrimary(cfgNoDefaults, "main"),
		).toBeUndefined();
	});
	...
	it("supports per-agent model primary+fallbacks", () => {
		...
		expect(resolveAgentExplicitModelPrimary(cfg, "linus")).toBe(
			"anthropic/claude-sonnet-4-6",
		);
		expect(resolveAgentEffectiveModelPrimary(cfg, "linus")).toBe(
			"anthropic/claude-sonnet-4-6",
		);
	...
	it("uses OPENCLAW_HOME for default agentDir", () => {
		...
		expect(agentDir).toBe(
			path.join(path.resolve(home), ".openclaw", "agents", "main", "agent"),
		);
	});
});
...
describe("resolveAgentIdByWorkspacePath", () => {
	it("returns the most specific workspace match for a directory", () => {
		...
		expect(resolveAgentIdByWorkspacePath(cfg, `${opsWorkspace}/src`)).toBe(
			"ops",
		);
	});
	...
	it("returns undefined when directory has no matching workspace", () => {
		...
		expect(
			resolveAgentIdByWorkspacePath(
				cfg,
				`/tmp/openclaw-agent-scope-${Date.now()}-unrelated`,
			),
		).toBeUndefined();
	});
	...
	it("matches workspace paths through symlink aliases", () => {
		const tempRoot = fs.mkdtempSync(
			path.join(os.tmpdir(), "openclaw-agent-scope-"),
		);
		...
		try {
			...
			expect(
				resolveAgentIdByWorkspacePath(
					cfg,
					path.join(aliasWorkspaceRoot, "projects", "ops"),
				),
			).toBe("ops");
			expect(
				resolveAgentIdByWorkspacePath(
					cfg,
					path.join(aliasWorkspaceRoot, "projects", "ops", "src"),
				),
			).toBe("ops");
		} finally {
...
describe("resolveAgentIdsByWorkspacePath", () => {
	it("returns matching workspaces ordered by specificity", () => {
		...
		expect(
			resolveAgentIdsByWorkspacePath(cfg, `${opsDevWorkspace}/pkg`),
		).toEqual(["ops-dev", "ops", "main"]);
	});
});
