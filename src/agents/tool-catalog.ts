...
function buildCoreToolGroupMap() {
  ...
  const mullusiTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInMullusiGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:mullusi": mullusiTools,
