...
describe("createCacheTrace", () => {
	...
	it("sanitizes cache-trace payloads before writing", () => {
		...
		const optionsImages = (
			((event.options as { images?: unknown[] } | undefined)?.images ??
		...
		const firstMessage = ((event.messages as
		...
		const source = (((firstMessage?.content as
			| Array<Record<string, unknown>>
		...
		expect(source.sha256).toBe(
			crypto.createHash("sha256").update("U0VDUkVU").digest("hex"),
		);
	});
