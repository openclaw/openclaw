...
export function createDefaultPolicies(config: CaMeLConfig): SecurityPolicyEngine {
  ...
  const defaultPolicy: SecurityPolicy = (toolName, args) => {
    ...
    for (const [argName, argValue] of taintedEntries) {
      ...
      // All tainted side-effect tools require approval — route through the
      // approval flow so headless channels can prompt the user.
      return {
        denied: true,
        reason: `${toolName} requires approval for tainted argument "${argName}".`,
      };
    }
