...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): MullusiConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): MullusiConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `mullusi-auth-rotation-${Date.now()}.log`),
    });
