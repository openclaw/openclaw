...
function resolveAcpStreamLogPathFromSessionFile(
	sessionFile: string,
	sessionId: string,
): string {
...
export function startAcpSpawnParentStreamRelay(params: {
	...
	const unsubscribe = onAgentEvent((event) => {
		...
		if (event.stream === "assistant") {
			...
			if (
				pendingText.length >= STREAM_SNIPPET_MAX_CHARS ||
				delta.includes("\n\n")
			) {
				flushPending();
				return;
		...
		const phase = toTrimmedString(
			(event.data as { phase?: unknown } | undefined)?.phase,
		);
		...
		if (phase === "end") {
			...
			const endedAt = toFiniteNumber(
				(event.data as { endedAt?: unknown } | undefined)?.endedAt,
			);
			const durationMs =
		...
		if (phase === "error") {
			flushPending();
			const errorText = toTrimmedString(
				(event.data as { error?: unknown } | undefined)?.error,
			);
			if (errorText) {
				emit(
					`${relayLabel} run failed: ${errorText}`,
					`${contextPrefix}:error`,
				);
			} else {
