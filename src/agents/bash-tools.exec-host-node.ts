...
export async function executeNodeHostCommand(
	...
	const inlineEvalHit =
		params.strictInlineEval === true
			? (baseAllowlistEval.segments
					.map((segment) =>
						detectInterpreterInlineEvalArgv(
							segment.resolution?.effectiveArgv ?? segment.argv,
						),
					)
	...
	if (requiresAsk) {
		...
		void (async () => {
			...
			const deniedReason = initialDeniedReason;
			...
			try {
				...
				const output = normalizeNotifyOutput(
					combined.slice(-DEFAULT_NOTIFY_TAIL_CHARS),
				);
				const exitLabel = payload.timedOut
					? "timeout"
