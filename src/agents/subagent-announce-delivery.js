...
import { mergeDeliveryContext, normalizeDeliveryContext, resolveConversationDeliveryTarget, } from "../utils/delivery-context.js";
import { INTERNAL_MESSAGE_CHANNEL, isDeliverableMessageChannel, isGatewayMessageChannel, isInternalMessageChannel, normalizeMessageChannel, } from "../utils/message-channel.js";
import { buildAnnounceIdempotencyKey, resolveQueueAnnounceId } from "./announce-idempotency.js";
import { callGateway, createBoundDeliveryRouter, getGlobalHookRunner, isEmbeddedPiRunActive, loadConfig, loadSessionStore, queueEmbeddedPiMessage, resolveActiveEmbeddedRunSessionId, resolveAgentIdFromSessionKey, resolveConversationIdFromTargets, resolveExternalBestEffortDeliveryTarget, resolveQueueSettings, resolveStorePath, sendMessage, } from "./subagent-announce-delivery.runtime.js";
...
function resolveBoundConversationOrigin(params) {
    ...
    if (conversation.channel === "matrix" &&
    ...
    const inferredThreadId = boundTarget.threadId ??
...
export async function resolveSubagentCompletionOrigin(params) {
    ...
    if (route.mode === "bound" && route.binding) {
        return mergeDeliveryContext(resolveBoundConversationOrigin({
...
async function maybeQueueSubagentAnnounce(params) {
    ...
    if (shouldSteer) {
        const steered = subagentAnnounceDeliveryDeps.queueEmbeddedPiMessage(sessionId, params.steerMessage);
        if (steered) {
            return "steered";
        }
    }
...
export function extractThreadCompletionFallbackText(internalEvents) {
...
function hasVisibleGatewayAgentPayload(response) {
    const result = response && typeof response === "object" && "result" in response
        ? response.result
        : undefined;
    const payloads = result && typeof result === "object" && "payloads" in result
        ? result.payloads
        : undefined;
...
async function sendSubagentAnnounceDirectly(params) {
    ...
    try {
        ...
        const normalizedSessionOnlyOriginChannel = !params.requesterIsSubagent
            ...
            : undefined;
        const sessionOnlyOriginChannel = normalizedSessionOnlyOriginChannel &&
        ...
        if (params.expectsCompletionMessage && requesterActivity.sessionId) {
            const woke = requesterActivity.sessionId
                ? subagentAnnounceDeliveryDeps.queueEmbeddedPiMessage(requesterActivity.sessionId, params.triggerMessage)
                : false;
            if (woke) {
                return {
                    delivered: true,
        ...
        const completionFallbackText = params.expectsCompletionMessage && deliveryTarget.deliver
