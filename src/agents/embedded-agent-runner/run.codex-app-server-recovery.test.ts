...
describe("runEmbeddedAgent Codex app-server recovery", () => {
  ...
  it("recovers a completed source reply after an external send races the completion timeout", async () => {
    ...
    const result = await runEmbeddedAgent({
      ...
      provider: "codex",
      model: "gpt-5.5",
      runId: "run-codex-timeout-external-message-final-assistant-recovered",
    });
