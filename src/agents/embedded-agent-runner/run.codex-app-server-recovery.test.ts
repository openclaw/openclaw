...
describe("runEmbeddedAgent Codex app-server recovery", () => {
  ...
  it("does not deliver assistant output from non-completion Codex turn watch timeouts", async () => {
    const assistantText = "This progress-timeout answer must not be delivered.";
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(
      codexTurnCompletionIdleTimeoutAttempt({
    ...
    const result = await runEmbeddedAgent({
      ...
      provider: "codex",
      model: "gpt-5.5",
      runId: "run-codex-progress-idle-timeout-assistant-output",
    });
    ...
    expect(result.payloads?.some((payload) => payload.text === assistantText)).toBe(false);
    expect(mockedRunEmbeddedAttempt).toHaveBeenCalledTimes(1);
    expect(mockedMarkAuthProfileFailure).not.toHaveBeenCalled();
  });
  ...
  it("delivers safe assistant text after a missing turn/completed timeout", async () => {
    ...
    expect(
      result.payloads?.some((payload) => payload.text === CODEX_MISSING_TERMINAL_MESSAGE),
    ).toBe(false);
    expect(mockedRunEmbeddedAttempt).toHaveBeenCalledTimes(1);
    expect(mockedMarkAuthProfileFailure).not.toHaveBeenCalled();
  });

  it("does not deliver assistant output while Codex item lifecycle is active", async () => {
    const assistantText = "This active-item answer must stay withheld.";
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(
      codexTurnCompletionIdleTimeoutAttempt({
        assistantTexts: [assistantText],
        itemLifecycle: {
    ...
    const result = await runEmbeddedAgent({
      ...
      provider: "codex",
      model: "gpt-5.5",
      runId: "run-codex-turn-completion-idle-timeout-active-item",
    });
  ...
  it.each([
    [
      ...
      (result: Awaited<ReturnType<typeof runEmbeddedAgent>>) => {
        expect(result.heartbeatToolResponse?.summary).toBe("Still working");
      },
    ],
    [
      "tool media",
      ...
      (result: Awaited<ReturnType<typeof runEmbeddedAgent>>) => {
        expect(
          result.payloads?.some((payload) => payload.mediaUrl === "file:///tmp/render.png"),
        ).toBe(true);
      },
    ],
    ["tool media block replies", { hasToolMediaBlockReply: true }, undefined],
    [
      "source reply payloads",
      { messagingToolSourceReplyPayloads: [{ text: "Delivered through the source reply." }] },
      (result: Awaited<ReturnType<typeof runEmbeddedAgent>>) => {
        expect(result.messagingToolSourceReplyPayloads?.[0]?.text).toBe(
          "Delivered through the source reply.",
        );
      },
    ],
    [
      "accepted session spawns",
    ...
    ["cron writes", { successfulCronAdds: 1 }, undefined],
    [
      "committed messaging delivery",
    ...
    [
      string,
      Partial<EmbeddedRunAttemptResult>,
      ((result: Awaited<ReturnType<typeof runEmbeddedAgent>>) => void) | undefined,
    ]
  >)("does not recover assistant output over terminal %s", async (_label, attemptState, verify) => {
    const assistantText = "This recovered assistant output must not replace terminal state.";
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(
      codexTurnCompletionIdleTimeoutAttempt({
        assistantTexts: [assistantText],
        ...attemptState,
    ...
    const result = await runEmbeddedAgent({
      ...
      provider: "codex",
      model: "gpt-5.5",
      runId: `run-codex-turn-completion-idle-timeout-terminal-${_label.replaceAll(" ", "-")}`,
    });
    ...
    if (!result.payloads) {
      throw new Error("expected terminal timeout payloads");
    }
    verify?.(result);
    expect(mockedRunEmbeddedAttempt).toHaveBeenCalledTimes(1);
    expect(mockedMarkAuthProfileFailure).not.toHaveBeenCalled();
