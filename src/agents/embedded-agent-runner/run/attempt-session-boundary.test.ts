...
describe("prepareEmbeddedAttemptSessionBoundary", () => {
  ...
  it("drops an empty aborted assistant leaf during a persisted retry", () => {
    const repairedMessages: AgentMessage[] = [
      { role: "user", content: [{ type: "text", text: "repaired" }], timestamp: 2 },
    ];
    const { activeSession } = createActiveSession();
  ...
  it("preserves an aborted assistant leaf for a newly admitted user turn", () => {
    const { activeSession } = createActiveSession();
    const branch = vi.fn();
    const assistantLeaf = {
      id: "assistant-leaf",
      parentId: "cancelled-user-leaf",
      type: "message",
      timestamp: "2026-07-20T00:00:00.000Z",
      message: {
        role: "assistant",
    ...
    const boundary = prepareEmbeddedAttemptSessionBoundary({
      activeSession,
      attempt: { prompt: "different prompt", trigger: "user" },
      getUserTranscriptContexts: () => undefined,
      isRawModelRun: false,
      preparedUserTurnMessage: undefined,
      sessionManager,
