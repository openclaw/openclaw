...
describe("normalizeAssistantReplayContent", () => {
  ...
  it("drops leaked bracketed tool-call text from assistant replay", () => {
    const messages = [
      userMessage("first"),
      bedrockAssistant('[tool:web_search] {"query":"funny chicken","count":5}', "stop", {
        input: 1,
        output: 1,
        totalTokens: 2,
      }),
  ...
  it("strips leaked bracketed tool calls while preserving visible assistant prose", () => {
    const messages = [
      userMessage("first"),
      bedrockAssistant(
        [{ type: "text", text: 'Let me check.\n[tool:web_search] {"query":"x","count":5}' }],
        "stop",
    ...
    const out = normalizeAssistantReplayContent(messages);
    const normalized = out[1] as AgentMessage & { content: unknown[] };
    expect(normalized.content).toEqual([{ type: "text", text: "Let me check." }]);
  });

  it("drops leaked hybrid XML parameter tool-call text from assistant replay", () => {
    const messages = [
      userMessage("first"),
      bedrockAssistant(
        [
  ...
  it("drops serialized tool result payloads that were replayed as assistant text", () => {
    const payload = JSON.stringify({
      provider: "search-searxng",
      results: [{ title: "Result", url: "https://example.test/" }],
      filtering: { blocked: 0 },
    });
    const messages = [userMessage("first"), bedrockAssistant(payload), userMessage("second")];
    const out = normalizeAssistantReplayContent(messages);
    expect(out).toEqual([messages[0], messages[2]]);
  });

  it("strips replay control markers from assistant replay text", () => {
    const messages = [
      userMessage("first"),
      bedrockAssistant("[[reply_to_current]]\n\nVisible reply", "stop", {
        input: 1,
        output: 1,
        totalTokens: 2,
      }),
    ...
    const out = normalizeAssistantReplayContent(messages);
    const normalized = out[1] as AgentMessage & { content: unknown[] };
    expect(normalized.content).toEqual([{ type: "text", text: "Visible reply" }]);
  });
