...
describe("resolveAuthProfileOrder", () => {
	...
	it.each([
		"store",
		"config",
	] as const)("pushes cooldown profiles to the end even with %s order", (orderSource) => {
		const now = Date.now();
	...
	it.each([
		"store",
		"config",
	] as const)("pushes disabled profiles to the end even with %s order", (orderSource) => {
		const now = Date.now();
		const order = resolveWithAnthropicOrderAndUsage({
			orderSource,
			usageStats: {
	...
	it.each([
		"store",
		"config",
	] as const)("keeps OpenRouter explicit order even when cooldown fields exist (%s)", (orderSource) => {
		const now = Date.now();
