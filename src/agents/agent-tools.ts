...
export function createOpenClawCodingTools(options?: {
  ...
  const hookContext = {
    ...
    // delivered back to the originating chat (parity with bash exec approval).
    // Without these, embedded-runner plugin approvals reach
    ...
    //
    // `messageChannel` is the canonical transport channel for approval routing;
    // `messageProvider` is the tool-policy provider that the embedded runner can
    // intentionally diverge from the transport (for example `discord-voice`
    // policy on a `discord` transport). Falling back to `messageProvider`
    // preserves the prior single-channel behavior for callers that have not
    // split the two.
