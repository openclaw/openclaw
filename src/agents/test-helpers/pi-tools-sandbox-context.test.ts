...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("mullusi-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("mullusi-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("mullusi-sbx-");
