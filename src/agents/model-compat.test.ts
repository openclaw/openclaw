...
vi.mock("../plugins/provider-runtime.js", () => ({
	resolveProviderModernModelRef:
		providerRuntimeMocks.resolveProviderModernModelRef,
}));
...
function supportsDeveloperRole(model: Model<Api>): boolean | undefined {
	return (model.compat as { supportsDeveloperRole?: boolean } | undefined)
...
function supportsStrictMode(model: Model<Api>): boolean | undefined {
	return (model.compat as { supportsStrictMode?: boolean } | undefined)
		?.supportsStrictMode;
}
...
function expectSupportsUsageInStreamingForcedOff(
	overrides?: Partial<Model<Api>>,
): void {
	const model = { ...baseModel(), ...overrides };
	delete (model as { compat?: unknown }).compat;
...
function expectSupportsStrictModeForcedOff(
	overrides?: Partial<Model<Api>>,
): void {
	const model = { ...baseModel(), ...overrides };
	delete (model as { compat?: unknown }).compat;
...
describe("isModernModelRef", () => {
	it("uses provider runtime hooks before fallback heuristics", () => {
		...
		expect(
			isModernModelRef({ provider: "openrouter", id: "claude-opus-4-6" }),
		).toBe(false);
	});
	...
	it("includes plugin-advertised modern models", () => {
		providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(
			({ provider, context }) =>
				provider === "openai" &&
				["gpt-5.4", "gpt-5.4-pro", "gpt-5.4-mini", "gpt-5.4-nano"].includes(
					context.modelId,
				)
					? true
		...
		expect(isModernModelRef({ provider: "openai", id: "gpt-5.4-pro" })).toBe(
			true,
		);
		expect(isModernModelRef({ provider: "openai", id: "gpt-5.4-mini" })).toBe(
			true,
		);
		expect(isModernModelRef({ provider: "openai", id: "gpt-5.4-nano" })).toBe(
			true,
		);
		expect(isModernModelRef({ provider: "openai-codex", id: "gpt-5.4" })).toBe(
			true,
		);
		expect(
		...
		expect(isModernModelRef({ provider: "opencode-go", id: "kimi-k2.5" })).toBe(
			true,
		);
		expect(isModernModelRef({ provider: "opencode-go", id: "glm-5" })).toBe(
			true,
		);
		expect(
			isModernModelRef({ provider: "opencode-go", id: "minimax-m2.7" }),
		).toBe(true);
	});
	...
	it("excludes provider-declined modern models", () => {
		providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(
			({ provider, context }) =>
				provider === "opencode" && context.modelId === "minimax-m2.7"
					? false
					: undefined,
		);

		expect(isModernModelRef({ provider: "opencode", id: "minimax-m2.7" })).toBe(
			false,
		);
	});
});
...
describe("isHighSignalLiveModelRef", () => {
	it("keeps modern higher-signal Claude families", () => {
		providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(
			({ provider, context }) =>
				provider === "anthropic" &&
				["claude-sonnet-4-5", "claude-opus-4-5"].includes(context.modelId)
					? true
					: undefined,
		);
