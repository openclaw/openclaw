...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["SMART_AGENT_NEO_STATE_DIR"]);
  let tempStateDir: string | null = null;

  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-subagent-"));
    process.env.SMART_AGENT_NEO_STATE_DIR = tempStateDir;
  ...
  it("uses isolated temp state when SMART_AGENT_NEO_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "smart-agent-neo-test-state"));
  });
});
