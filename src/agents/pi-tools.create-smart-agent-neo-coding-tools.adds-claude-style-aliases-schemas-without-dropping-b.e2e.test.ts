...
import { createSmartAgentNeoCodingTools } from "./pi-tools.js";

const defaultTools = createSmartAgentNeoCodingTools();

describe("createSmartAgentNeoCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    const config: SmartAgentNeoConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createSmartAgentNeoCodingTools({
      config,
    ...
    const anthropicTools = createSmartAgentNeoCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createSmartAgentNeoCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createSmartAgentNeoCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
  ...
  it("keeps canonical tool names for Anthropic OAuth (pi-ai remaps on the wire)", () => {
    const tools = createSmartAgentNeoCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
