...
import { loadSessionStore, resolveAgentIdFromSessionKey, resolveStorePath, updateSessionStore, } from "../config/sessions.js";
import { callGateway } from "../gateway/call.js";
import { readSessionMessages } from "../gateway/session-utils.fs.js";
import { formatErrorMessage } from "../infra/errors.js";
import { createSubsystemLogger } from "../logging/subsystem.js";
import { buildAnnounceIdempotencyKey } from "./announce-idempotency.js";
import { deliverSubagentAnnouncement, isInternalAnnounceRequesterSession, loadRequesterSessionEntry, } from "./subagent-announce-delivery.js";
import { resolveAnnounceOrigin } from "./subagent-announce-origin.js";
import { finalizeInterruptedSubagentRun, replaceSubagentRunAfterSteer, } from "./subagent-registry-steer-runtime.js";
...
function buildResumeMessage(task, lastHumanMessage) {
    ...
    let message = `[System] Your previous turn was interrupted by a gateway reload. ` +
...
function buildRecoveryProgressPrompt(params) {
    ...
    const taskLabel = params.task.length > maxTaskLen ? `${params.task.slice(0, maxTaskLen)}...` : params.task;
    return (`A spawned subagent task was interrupted by a gateway restart or connection loss. ` +
...
export async function recoverOrphanedSubagentSessions(params) {
    ...
    const notifiedRecoverySessionKeys = params.notifiedRecoverySessionKeys ?? new Set();
    ...
    try {
        ...
        for (const [runId, runRecord] of activeRuns.entries()) {
            ...
            try {
                ...
                if (typeof runRecord.endedAt === "number" &&
                    runRecord.endedAt > 0 &&
    ...
    if (result.recovered > 0 || result.failed > 0) {
        log.info(`orphan recovery complete: recovered=${result.recovered} failed=${result.failed} skipped=${result.skipped}`);
    }
    return result;
}
...
function buildRecoveryFailureMessage(params) {
    const base = `Subagent run was interrupted by a gateway restart or connection loss. ` +
...
export function scheduleOrphanRecovery(params) {
    ...
    const notifiedRecoverySessionKeys = new Set();
    const attemptRecovery = (attempt, delay) => {
        setTimeout(() => {
            void recoverOrphanedSubagentSessions({
                ...
                void Promise.allSettled(result.failedRuns.map((run) => finalizeInterruptedSubagentRun({
                    runId: run.runId,
