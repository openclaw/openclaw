...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "activi-oauth-"));
    ...
    try {
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.ACTIVI_AGENT_DIR, {
          ...
          const apiKey = await getApiKeyForModel({
            ...
            store,
            agentDir: process.env.ACTIVI_AGENT_DIR,
          });
