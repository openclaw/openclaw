...
describe("getApiKeyForModel", () => {
  ...
  it("resolves synthetic local auth key for configured ollama provider without apiKey", async () => {
    await withEnvAsync({ OLLAMA_API_KEY: undefined }, async () => {
      ...
      expect(resolved.apiKey).toBe("ollama-local"); // pragma: allowlist secret (fixture key)
  ...
  it("prefers explicit OLLAMA_API_KEY over synthetic local key", async () => {
    await withEnvAsync({ OLLAMA_API_KEY: "env-ollama-key" }, async () => { // pragma: allowlist secret (fixture key)
      const resolved = await resolveApiKeyForProvider({
        provider: "ollama",
        store: { version: 1, profiles: {} },
        cfg: {
      ...
      expect(resolved.apiKey).toBe("env-ollama-key"); // pragma: allowlist secret (fixture key)
