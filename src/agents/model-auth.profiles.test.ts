...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-oauth-"));
    ...
    try {
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.MULLUSI_AGENT_DIR, {
          ...
          const apiKey = await getApiKeyForModel({
            ...
            store,
            agentDir: process.env.MULLUSI_AGENT_DIR,
          });
  ...
  it("resolveEnvApiKey('anthropic-vertex') accepts GOOGLE_APPLICATION_CREDENTIALS with project_id", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-adc-"));
