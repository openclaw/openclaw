...
import { buildApiErrorObservationFields, buildTextObservationFields, sanitizeForConsole, } from "./pi-embedded-error-observation.js";
import { classifyFailoverReason, formatAssistantErrorText } from "./pi-embedded-helpers.js";
import { isIncompleteTerminalAssistantTurn } from "./pi-embedded-runner/run/incomplete-turn.js";
import { consumePendingToolMediaReply, hasAssistantVisibleReply, } from "./pi-embedded-subscribe.handlers.messages.js";
...
export function handleAgentEnd(ctx) {
    ...
    const hasAssistantVisibleText = Array.isArray(ctx.state.assistantTexts) &&
        ctx.state.assistantTexts.some((text) => hasAssistantVisibleReply({ text }));
    const hadDeterministicSideEffect = ctx.state.hadDeterministicSideEffect === true ||
    ...
    const replayInvalid = ctx.state.replayState.replayInvalid || incompleteTerminalAssistant ? true : undefined;
    ...
    if (isError && lastAssistant) {
        ...
        const safeErrorText = buildTextObservationFields(errorText, {
            provider: lastAssistant.provider,
    ...
    const flushPendingMediaAndChannel = () => {
        ...
        if (isPromiseLike(postMediaFlushResult)) {
    ...
    const emitLifecycleTerminalOnce = () => {
        ...
        let beforeLifecycleTerminal = undefined;
        try {
        ...
        if (isPromiseLike(beforeLifecycleTerminal)) {
    ...
    try {
        ...
        finalizeAgentEnd();
        const flushPendingMediaAndChannelResult = isPromiseLike(flushBlockReplyBufferResult)
        ...
        if (isPromiseLike(flushPendingMediaAndChannelResult)) {
            return Promise.resolve(flushPendingMediaAndChannelResult).then(() => emitLifecycleTerminalOnce(), (error) => {
