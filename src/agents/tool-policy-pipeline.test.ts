...
describe("agent tool replay safety", () => {
  test("allows only audited unconditional core tools", () => {
  ...
  test("requires extension-owned tools to opt in even when they reuse an audited name", () => {
  ...
  test("accepts opted-in extension tools only for audited names", () => {
  ...
  test("accepts owner-declared concrete tools for restart-safe turns", () => {
  ...
  test("rejects memory_search because it records durable recall signals", () => {
    expect(
      isAgentToolReplaySafe(
        { name: "memory_search" },
        {
  ...
  test("rejects duplicate names from name-only replay metadata", () => {
  ...
  test("retains canonical owners only for unique side-effecting tool names", () => {
