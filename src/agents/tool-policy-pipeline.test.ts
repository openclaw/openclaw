...
describe("tool-policy-pipeline", () => {
  ...
  test("warns about unknown allowlist entries", () => {
    ...
    expect(warnings[0]).toContain("unknown entries (unknown: wat)");
  });
  ...
  test("warns gated core tools as unavailable instead of plugin-only unknowns", () => {
    ...
    expect(warnings.length).toBe(1);
    expect(warnings[0]).toContain("unavailable core entries (unavailable core: apply_patch)");
  ...
  test("warns mixed unavailable core + unknown entries with split labels", () => {
    const warnings: string[] = [];
    const tools = [{ name: "exec" }] as unknown as DummyTool[];
    applyToolPolicyPipeline({
      // oxlint-disable-next-line typescript/no-explicit-any
    ...
    expect(warnings.length).toBe(1);
    expect(warnings[0]).toContain("unavailable core and unknown entries");
    expect(warnings[0]).toContain("unavailable core: apply_patch");
    expect(warnings[0]).toContain("unknown: wat");
  });
