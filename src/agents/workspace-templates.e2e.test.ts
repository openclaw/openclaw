...
async function makeTempRoot(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-templates-"));
}
...
describe("resolveWorkspaceTemplateDir", () => {
  it("resolves templates from package root when module url is dist-rooted", async () => {
    ...
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "smart-agent-neo" }));
