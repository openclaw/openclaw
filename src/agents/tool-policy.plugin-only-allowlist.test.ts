...
const pluginGroups: PluginToolGroups = {
  all: ["flow", "workflow_tool"],
  byPlugin: new Map([["flow", ["flow", "workflow_tool"]]]),
};
...
describe("stripPluginOnlyAllowlist", () => {
  it("strips allowlist when it only targets plugin tools", () => {
    const policy = stripPluginOnlyAllowlist({ allow: ["flow"] }, pluginGroups, coreTools);
  ...
  it("keeps allowlist when it mixes plugin and core entries", () => {
    const policy = stripPluginOnlyAllowlist({ allow: ["flow", "read"] }, pluginGroups, coreTools);
    expect(policy.policy?.allow).toEqual(["flow", "read"]);
    expect(policy.unknownAllowlist).toEqual([]);
  });

  it("strips allowlist with unknown entries when no core tools match", () => {
    const emptyPlugins: PluginToolGroups = { all: [], byPlugin: new Map() };
    const policy = stripPluginOnlyAllowlist({ allow: ["flow"] }, emptyPlugins, coreTools);
    expect(policy.policy?.allow).toBeUndefined();
    expect(policy.unknownAllowlist).toEqual(["flow"]);
  });

  it("keeps allowlist with core tools and reports unknown entries", () => {
    const emptyPlugins: PluginToolGroups = { all: [], byPlugin: new Map() };
    const policy = stripPluginOnlyAllowlist({ allow: ["read", "flow"] }, emptyPlugins, coreTools);
    expect(policy.policy?.allow).toEqual(["read", "flow"]);
    expect(policy.unknownAllowlist).toEqual(["flow"]);
  });
});
