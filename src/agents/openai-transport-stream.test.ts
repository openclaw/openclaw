...
type OpenAICompletionsStreamSink = Parameters<typeof __testing.processOpenAICompletionsStream>[3];
type CapturedStreamEvent = { type?: string; delta?: string };

const DEEPSEEK_DSML_STREAM_CREATED = 1_778_043_848;

function createDeepSeekCompletionsModel(
  id: "deepseek-v4-flash" | "deepseek-v4-pro",
): Model<"openai-completions"> {
  return {
    id,
    name: id === "deepseek-v4-pro" ? "DeepSeek V4 Pro" : "DeepSeek V4 Flash",
...
function collectStreamEvents(): {
  ...
  stream: OpenAICompletionsStreamSink;
} {
...
function createContentChunk(params: {
  content: string;
  ...
  return {
    ...
    created: DEEPSEEK_DSML_STREAM_CREATED,
    model: params.model.id,
    choices: [
      {
        index: 0,
        delta: { role: "assistant" as const, content: params.content },
        logprobs: null,
        finish_reason: null,
      },
    ],
...
function createStopChunk(params: {
  model: Model<"openai-completions">;
  streamId: string;
}): ChatCompletionChunk {
  return {
    ...
    created: DEEPSEEK_DSML_STREAM_CREATED,
    model: params.model.id,
    choices: [{ index: 0, delta: {}, logprobs: null, finish_reason: "stop" as const }],
  };
}

async function* streamContentChunks(params: {
  contents: readonly string[];
  model: Model<"openai-completions">;
  streamId: string;
}): AsyncGenerator<ChatCompletionChunk> {
  for (const content of params.contents) {
    yield createContentChunk({ content, model: params.model, streamId: params.streamId });
  }
  yield createStopChunk({ model: params.model, streamId: params.streamId });
}
...
describe("openai transport stream", () => {
  ...
  it("suppresses streamed DeepSeek DSML tool-use-error text before assistant events", async () => {
    const model = createDeepSeekCompletionsModel("deepseek-v4-pro");
    const output = createAssistantOutput(model);
    const { events, stream } = collectStreamEvents();

    await __testing.processOpenAICompletionsStream(
      streamContentChunks({
        model,
        streamId: "chatcmpl-deepseek-dsml",
        contents: [
          "before ",
          "<｜DS",
          "ML｜tool_use_error>\n<tool_name>write</tool_name>\n",
          '<tool_parameter name="path">/tmp/timer-manual.md</tool_param>\n',
          "</｜DSML｜tool_use_error>",
          " after",
        ],
      }),
    ...
    expect(JSON.stringify(output.content)).not.toContain("tool_name");
  });

  it("turns DSML-only DeepSeek tool-use-error text into an empty assistant turn", async () => {
    const model = createDeepSeekCompletionsModel("deepseek-v4-flash");
    const output = createAssistantOutput(model);
    const { events, stream } = collectStreamEvents();
    ...
    await __testing.processOpenAICompletionsStream(
      streamContentChunks({
        model,
        streamId: "chatcmpl-deepseek-dsml-empty",
        contents: [
          "<｜DSML｜tool_use_error>\n",
          "<tool_name>write</tool_name>\n",
          '<tool_parameter name="path">/tmp/timer-manual.md</tool_param>\n',
  ...
  it("suppresses ASCII DSML tool-use-error text for DeepSeek-compatible providers", async () => {
    const model = {
      ...createDeepSeekCompletionsModel("deepseek-v4-pro"),
    ...
    await __testing.processOpenAICompletionsStream(
      streamContentChunks({
        model,
        streamId: "chatcmpl-deepseek-compatible-dsml",
        contents: [
          "visible ",
          "<|DSML|tool_use_error>\n",
          "<tool_name>write</tool_name>\n",
    ...
    expect(output.content).toEqual([{ type: "text", text: "visible  done" }]);
    expect(
      events.filter((event) => event.type === "text_delta").map((event) => event.delta),
    ).toEqual(["visible ", " done"]);
    expect(JSON.stringify(events)).not.toContain("DSML");
    expect(JSON.stringify(output.content)).not.toContain("tool_name");
  });
