...
import { CANONICAL_ROOT_MEMORY_FILENAME, exactWorkspaceEntryExists, } from "../memory/root-memory-files.js";
...
async function workspaceProfileLooksConfigured(params) {
    const profileFileDiffs = await Promise.all(WORKSPACE_ONBOARDING_PROFILE_FILENAMES.map(async (fileName) => fileContentDiffersFromTemplate(path.join(params.dir, fileName), await loadTemplate(fileName))));
    return (profileFileDiffs.some(Boolean) ||
...
async function reconcileWorkspaceBootstrapCompletionState(params) {
    ...
    if (typeof params.state.setupCompletedAt === "string" &&
...
async function readWorkspaceSetupState(statePath, opts) {
    try {
        ...
        if (opts?.persistLegacyMigration &&
            parsed &&
...
export async function resolveWorkspaceBootstrapStatus(dir) {
...
export async function isWorkspaceBootstrapPending(dir) {
    return (await resolveWorkspaceBootstrapStatus(dir)) === "pending";
}
export async function reconcileWorkspaceBootstrapCompletion(dir) {
    const resolvedDir = resolveUserPath(dir);
    const statePath = resolveWorkspaceStatePath(resolvedDir);
    const bootstrapPath = path.join(resolvedDir, DEFAULT_BOOTSTRAP_FILENAME);
    const state = await readWorkspaceSetupState(statePath, {
...
export async function ensureAgentWorkspace(params) {
    ...
    if (!state.bootstrapSeededAt && !state.setupCompletedAt && !bootstrapExists) {
        ...
        if (await workspaceProfileLooksConfigured({
            dir,
...
export async function loadWorkspaceBootstrapFiles(dir) {
    ...
    for (const entry of entries) {
        if (entry.name === DEFAULT_MEMORY_FILENAME &&
