...
function createRequesterContext(
...
describe("spawnAcpDirect", () => {
	beforeEach(() => {
		...
		getAcpSessionManagerSpy.mockReset().mockReturnValue({
			initializeSession: async (
				params: Parameters<
					ReturnType<
						typeof acpSessionManager.getAcpSessionManager
					>["initializeSession"]
				>[0],
		...
		hoisted.resolveStorePathMock
			.mockReset()
			.mockReturnValue("/tmp/codex-sessions.json");
		resolveStorePathSpy
			.mockReset()
			.mockImplementation((store, opts) =>
				hoisted.resolveStorePathMock(store, opts),
			);
		...
		loadSessionStoreSpy
			...
			.mockImplementation((storePath) =>
				hoisted.loadSessionStoreMock(storePath),
			);
		...
		resolveSessionTranscriptFileSpy
			...
			.mockImplementation(
				async (params) =>
					await hoisted.resolveSessionTranscriptFileMock(params),
			);
	...
	it("spawns ACP session, binds a new thread, and dispatches initial task", async () => {
		...
		const transcriptCalls =
			hoisted.resolveSessionTranscriptFileMock.mock.calls.map(
				...
				createSessionBinding({
					...
					conversation: {
						...
						parentConversationId:
							input.conversation.parentConversationId ?? "!room:example",
					},
					metadata: {
						boundBy:
							typeof input.metadata?.boundBy === "string"
	...
	])("resolves LINE ACP conversation ids from $name", async ({
		agentTo,
	...
	it("keeps ACP spawn running when session-file persistence fails", async () => {
		hoisted.resolveSessionTranscriptFileMock.mockRejectedValueOnce(
			new Error("disk full"),
		);
	...
	it("does not implicitly stream for thread-bound subagent requester sessions", async () => {
		hoisted.sessionBindingListBySessionMock.mockImplementation(
			(targetSessionKey: string) => {
