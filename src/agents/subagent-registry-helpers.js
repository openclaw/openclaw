...
export function capFrozenResultText(resultText) {
...
export function resolveAnnounceRetryDelayMs(retryCount) {
...
export function logAnnounceGiveUp(entry, reason) {
...
export async function persistSubagentSessionTiming(entry) {
    ...
    const endedAt = typeof entry.endedAt === "number" && Number.isFinite(entry.endedAt) ? entry.endedAt : undefined;
    const runtimeMs = endedAt !== undefined
...
export function resolveSubagentRunOrphanReason(params) {
    ...
    try {
        ...
        if (params.includeStaleUnended === true &&
...
export async function safeRemoveAttachmentsDir(entry) {
...
export function reconcileOrphanedRun(params) {
    ...
    const shouldDeleteAttachments = params.entry.cleanup === "delete" || !params.entry.retainAttachmentsOnKeep;
...
export function resolveArchiveAfterMs(cfg) {
