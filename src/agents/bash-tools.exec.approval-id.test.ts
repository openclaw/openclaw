...
async function loadExecApprovalModules() {
	...
	({ detectCommandObfuscation } = await import(
		"../infra/exec-obfuscation-detect.js"
	));
	({ getExecApprovalApproverDmNoticeText } = await import(
...
async function writeOpenClawConfig(
	...
	await fs.writeFile(
		configPath,
		JSON.stringify(config, null, pretty ? 2 : undefined),
	);
}
...
async function expectGatewayExecWithoutApproval(options: {
	...
	const result = await tool.execute("call-no-approval", {
...
describe("exec approvals", () => {
	...
	it("reuses approval id as the node runId", async () => {
		...
		await expect
			.poll(() => agentParams, { timeout: 2_000, interval: 20 })
			.toBeTruthy();
	});
	...
	it("starts a direct agent follow-up after approved gateway exec completes", async () => {
		...
		expect(result.details.status).toBe("approval-pending");
		await expect
			.poll(() => agentCalls.length, { timeout: 3_000, interval: 20 })
			.toBe(1);
	...
	it("executes approved commands and emits a session-only followup in webchat-only mode", async () => {
		const agentCalls: Array<Record<string, unknown>> = [];
		const tempDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "openclaw-exec-followup-sidefx-"),
		);
	...
	it("keeps Telegram approvals in the initiating chat even when Discord DM approvals are also enabled", async () => {
		...
		expect(getResultText(result)).toContain(
			`/approve ${details.approvalSlug} allow-once`,
		);
