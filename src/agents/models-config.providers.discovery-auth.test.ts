...
describe("provider discovery auth marker guardrails", () => {
	...
	async function createAgentDirWithAuthProfiles(
		profiles: AuthProfilesFile["profiles"],
	) {
		const agentDir = mkdtempSync(join(tmpdir(), "openclaw-test-"));
		await writeFile(
			join(agentDir, "auth-profiles.json"),
			JSON.stringify(
				{ version: 1, profiles } satisfies AuthProfilesFile,
				null,
				2,
			),
			"utf8",
	...
	it("keeps all-caps plaintext API keys for authenticated discovery", async () => {
		...
		const vllmCall = fetchMock.mock.calls.find(([url]) =>
			String(url).includes(":8000"),
		);
		const request = vllmCall?.[1] as
			| { headers?: Record<string, string> }
			| undefined;
