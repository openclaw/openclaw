...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-state-"));
    const bundledDir = path.join(stateDir, "bundled-skills");
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.EASYHUB_STATE_DIR = stateDir;
    process.env.EASYHUB_BUNDLED_SKILLS_DIR = bundledDir;
    vi.resetModules();

    const { resolveSandboxContext } = await import("./sandbox.js");

    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-workspace-"));
    await writeSkill({
