...
export function appendImagePathsToPrompt(prompt, paths, prefix = "") {
    if (!paths.length) {
        return prompt;
    }
...
export async function writeCliSystemPromptFile(params) {
    if (!params.backend.systemPromptFileArg?.trim() &&
        !params.backend.systemPromptFileConfigKey?.trim()) {
...
export async function prepareCliPromptImagePayload(params) {
    ...
    const resolvedImages = params.images && params.images.length > 0
        ? params.images
...
export function buildCliArgs(params) {
    ...
    else if (!params.useResume &&
        params.systemPrompt &&
        params.systemPromptFilePath &&
        params.backend.systemPromptFileConfigKey) {
        args.push(params.backend.systemPromptFileConfigArg ?? "-c", formatTomlConfigOverride(params.backend.systemPromptFileConfigKey, params.systemPromptFilePath));
    }
