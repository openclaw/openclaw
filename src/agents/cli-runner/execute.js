...
import { createCliJsonlStreamingParser, extractCliErrorMessage, parseCliOutput, } from "../cli-output.js";
...
import { buildCliSupervisorScopeKey, buildCliArgs, resolveCliRunQueueKey, enqueueCliRun, prepareCliPromptImagePayload, resolveCliNoOutputTimeoutMs, resolvePromptInput, resolveSessionIdToSend, resolveSystemPromptUsage, writeCliSystemPromptFile, } from "./helpers.js";
import { cliBackendLog, CLI_BACKEND_LOG_OUTPUT_ENV, LEGACY_CLAUDE_CLI_LOG_OUTPUT_ENV, } from "./log.js";
const executeDeps = {
...
export function setCliRunnerExecuteTestDeps(overrides) {
...
export function buildCliEnvAuthLog(childEnv) {
...
export async function executePreparedCliRun(context, cliSessionIdToUse) {
    ...
    const systemPromptFile = !useResume && systemPromptArg
        ? await writeCliSystemPromptFile({
            backend,
            systemPrompt: systemPromptArg,
        })
        : undefined;
    let prompt = applyPluginTextReplacements(prependBootstrapPromptWarning(params.prompt, context.bootstrapPromptWarningLines, {
    ...
    const args = buildCliArgs({
        backend,
        baseArgs: claudeSkillsPlugin.args.length > 0
            ...
            try {
                ...
                const env = (() => {
                    ...
                    if (backend.env && Object.keys(backend.env).length > 0) {
                        Object.assign(next, sanitizeHostExecEnv({
                            baseEnv: {},
                ...
                const managedRun = await supervisor.spawn({
                    ...
                    env,
                    input: stdinPayload,
                    onStdout: streamingParser ? (chunk) => streamingParser.push(chunk) : undefined,
                });
                ...
                if (result.exitCode !== 0 || result.reason !== "exit") {
                    ...
                    const structuredError = extractCliErrorMessage(primaryErrorText) ??
