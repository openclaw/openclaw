...
async function readExternalMcpConfig(
	configPath: string,
...
export async function prepareCliBundleMcpConfig(params: {
	...
	const existingMcpConfigPath =
		findMcpConfigPath(params.backend.resumeArgs) ??
		findMcpConfigPath(params.backend.args);
	...
	for (const diagnostic of bundleConfig.diagnostics) {
		params.warn?.(
			`bundle MCP skipped for ${diagnostic.pluginId}: ${diagnostic.message}`,
		);
	}
	mergedConfig = applyMergePatch(
		mergedConfig,
