...
const prepareDeps = {
    ...
    resolveOpenClawReferencePaths: async (params) => (await import("../docs-path.js")).resolveOpenClawReferencePaths(params),
};
export function setCliRunnerPrepareTestDeps(overrides) {
    Object.assign(prepareDeps, overrides);
}
export function shouldSkipLocalCliCredentialEpoch(params) {
    return Boolean(params.authEpochMode === "profile-only" &&
...
export async function prepareCliRunContext(params) {
    ...
    const effectiveAuthProfileId = requestedAuthProfileId ?? backendResolved.defaultAuthProfileId?.trim() ?? undefined;
    let authCredential;
    ...
    const extraSystemPromptHash = params.extraSystemPromptStatic !== undefined
    ...
    const preparedBackend = await prepareCliBundleMcpConfig({
        ...
        env: mcpLoopbackRuntime
            ? {
                OPENCLAW_MCP_TOKEN: params.senderIsOwner === true
    ...
    const preparedBackendEnv = preparedExecution?.env && Object.keys(preparedExecution.env).length > 0
        ...
        : preparedBackend.env;
    const preparedBackendCleanup = preparedBackend.cleanup || preparedExecution?.cleanup
        ? async () => {
            try {
    ...
    const transformedSystemPrompt = backendResolved.transformSystemPrompt?.({
        config: params.config,
        workspaceDir,
        provider: params.provider,
        modelId,
