...
const liveSessionCreates = new Map();
function sha256(value) {
    return crypto.createHash("sha256").update(value).digest("hex");
}
export function resetClaudeLiveSessionsForTest() {
...
export function closeClaudeLiveSessionForContext(context) {
...
export function shouldUseClaudeLiveSession(context) {
    return (context.backendResolved.id === "claude-cli" &&
...
export function buildClaudeLiveArgs(params) {
    return appendArg(upsertArgValue(upsertArgValue(stripLiveProcessArgs(params.args, params.backend, params.useResume), "--input-format", "stream-json"), "--permission-prompt-tool", "stdio"), "--replay-user-messages");
}
...
function resetNoOutputTimer(session) {
    ...
    turn.noOutputTimer = setTimeout(() => {
        closeLiveSession(session, "abort", createTimeoutError(session, `CLI produced no output for ${Math.round(session.noOutputTimeoutMs / 1000)}s and was terminated.`));
...
function handleClaudeLiveLine(session, line) {
    ...
    if (turn.rawChars > CLAUDE_LIVE_MAX_TURN_RAW_CHARS ||
...
async function createClaudeLiveSession(params) {
    ...
    void managedRun.wait().then((exit) => handleClaudeExit(session, exit.exitCode), (error) => {
        if (session) {
            closeLiveSession(session, "abort", error);
        }
    });
...
function createTurn(params) {
    ...
    turn.noOutputTimer = setTimeout(() => {
        closeLiveSession(params.session, "abort", createTimeoutError(params.session, `CLI produced no output for ${Math.round(params.noOutputTimeoutMs / 1000)}s and was terminated.`));
    }, params.noOutputTimeoutMs);
    turn.timeoutTimer = setTimeout(() => {
        closeLiveSession(params.session, "abort", createTimeoutError(params.session, `CLI exceeded timeout (${Math.round(params.context.params.timeoutMs / 1000)}s) and was terminated.`));
