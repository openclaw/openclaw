...
/**
 * Cleanup suspended EasyHub CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: EasyHubConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-cli-images-"));
  const paths: string[] = [];
