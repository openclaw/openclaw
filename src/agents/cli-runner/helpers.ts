...
/**
 * Cleanup suspended DNA CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: DNAConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-cli-images-"));
  const paths: string[] = [];
