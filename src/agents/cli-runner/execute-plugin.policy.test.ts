...
describe("plugin-owned CLI same-turn input", () => {
  ...
  it("reports a write refused before the transcript as an unconfirmed commit", async () => {
    const { context } = await createExecution();
    let handle: ReplyBackendHandle | undefined;
    context.params.replyOperation = {
      attachBackend: (backend: ReplyBackendHandle) => {
    ...
    await vi.waitFor(() => expect(handle?.messageInjectionV2?.isAvailable()).toBe(true));
    const injection = handle!.messageInjectionV2!;
    // before_message_write rejected the turn: nothing was written, and the text
    // is already in the model's hands, so the run must stop rather than replay it.
    const blocked = {
      hasPersisted: vi.fn(() => false),
      isBlocked: vi.fn(() => false),
      resolveMessage: vi.fn(async () => ({ role: "user" })),
      markBlocked: vi.fn(),
      persistApproved: vi.fn(async () => false),
    };

    await expect(
      injection.queueMessage(
        "blocked",
        { userTurnTranscriptRecorder: blocked } as unknown as Parameters<
          typeof injection.queueMessage
        >[1],
        () => {},
        "run",
    ...
    expect(blocked.markBlocked).toHaveBeenCalledTimes(1);
  });
