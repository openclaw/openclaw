...
import { resolveProviderRequestCapabilities, resolveProviderRequestPolicy, } from "./provider-attribution.js";
...
export function sanitizeConfiguredProviderRequest(request) {
...
export function sanitizeConfiguredModelProviderRequest(request) {
...
export function mergeProviderRequestOverrides(...overrides) {
    const merged = {};
    let hasMerged = false;
...
export function mergeModelProviderRequestOverrides(...overrides) {
...
export function mergeProviderRequestHeaders(...headerSets) {
    let merged;
...
export function sanitizeRuntimeProviderRequestOverrides(request) {
    if (!request) {
        return undefined;
    }
...
export function buildProviderRequestDispatcherPolicy(request) {
...
export function buildProviderRequestTlsClientOptions(request) {
    return toTlsConnectOptions(request.tls);
}
export function resolveProviderRequestPolicyConfig(params) {
    ...
    const unprotectedCallerHeaders = params.callerHeaders
        ? Object.fromEntries(Object.entries(params.callerHeaders).filter(([key]) => !protectedAttributionKeys.has(normalizeLowercaseStringOrEmpty(key))))
        : undefined;
    const mergedDefaults = mergeProviderRequestHeaders(extraHeaders, policy.attributionHeaders);
    const headers = params.precedence === "caller-wins"
...
export function attachModelProviderRequestTransport(model, request) {
    if (!request) {
        return model;
    }
    const next = { ...model };
