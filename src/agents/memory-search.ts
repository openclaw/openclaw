...
function mergeConfig(
	...
	const primaryAdapter =
		provider === "auto" ? undefined : getMemoryEmbeddingProvider(provider);
	const defaultRemote = defaults?.remote;
	const overrideRemote = overrides?.remote;
	const fallback = overrides?.fallback ?? defaults?.fallback ?? "none";
	const fallbackAdapter =
		fallback && fallback !== "none"
			? getMemoryEmbeddingProvider(fallback)
			: undefined;
	...
	const remote = includeRemote
		...
		: undefined;
	const modelDefault =
		provider === "auto" ? undefined : primaryAdapter?.defaultModel;
	const model = overrides?.model ?? defaults?.model ?? modelDefault ?? "";
	const outputDimensionality =
		overrides?.outputDimensionality ?? defaults?.outputDimensionality;
	const local = {
	...
	const multimodal = normalizeMemoryMultimodalSettings({
		...
		modalities:
			overrides?.multimodal?.modalities ?? defaults?.multimodal?.modalities,
		maxFileBytes:
			overrides?.multimodal?.maxFileBytes ?? defaults?.multimodal?.maxFileBytes,
	});
	const vector = {
		enabled:
	...
	const fts = {
		tokenizer:
			overrides?.store?.fts?.tokenizer ??
			defaults?.store?.fts?.tokenizer ??
			"unicode61",
	};
	const store = {
...
export function resolveMemorySearchConfig(
	...
	const multimodalProvider =
		resolved.provider === "auto"
			? undefined
			: getMemoryEmbeddingProvider(resolved.provider);
	if (
		multimodalActive &&
