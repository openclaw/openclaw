...
/**
 * Tag an error as a context overflow so that downstream guards can detect it
 * structurally (in addition to regex matching on the message text).
 */
export function tagContextOverflowError<T extends Error>(err: T): T {
  (err as Record<string, unknown>).isContextOverflow = true;
  return err;
}

/**
 * Check whether an error was structurally tagged as a context overflow.
 */
export function hasContextOverflowTag(err: unknown): boolean {
  return (
    typeof err === "object" &&
    err !== null &&
    (err as Record<string, unknown>).isContextOverflow === true
...
/**
 * Internal overflow errors generated by OpenClaw's own context guards
 * (tool-result-context-guard, preemptive-compaction) always start with this
 * prefix.  Recognizing them before any exclusion guard runs ensures they are
 * never misclassified as rate-limit, billing, or reasoning errors.
 */
const INTERNAL_CONTEXT_OVERFLOW_PREFIX = "context overflow:";
...
export function isLikelyContextOverflowError(errorMessage?: string): boolean {
  ...
  // Fast path: internal overflow errors from OpenClaw's own guards must always
  // be recognized, regardless of rate-limit/billing/reasoning exclusions below.
  if (errorMessage.toLowerCase().startsWith(INTERNAL_CONTEXT_OVERFLOW_PREFIX)) {
    return true;
  }
