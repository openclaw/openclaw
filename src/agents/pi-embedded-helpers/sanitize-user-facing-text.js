import { extractLeadingHttpStatus, formatRawAssistantErrorForUi, isCloudflareOrHtmlErrorPage, parseApiErrorInfo, parseApiErrorPayload, } from "../../shared/assistant-error-format.js";
...
import { isBillingErrorMessage, isOverloadedErrorMessage, isRateLimitErrorMessage, isTimeoutErrorMessage, } from "./failover-matches.js";
...
function extractProviderRateLimitMessage(raw) {
    ...
    const candidate = info?.message ?? (extractLeadingHttpStatus(withoutPrefix)?.rest || withoutPrefix);
...
export function formatRateLimitOrOverloadedErrorCopy(raw) {
...
export function formatTransportErrorCopy(raw) {
    if (!raw) {
        return undefined;
    }
...
export function formatDiskSpaceErrorCopy(raw) {
    if (!raw) {
        return undefined;
    }
    const lower = normalizeLowercaseStringOrEmpty(raw);
    if (/\benospc\b/i.test(raw) ||
        lower.includes("no space left on device") ||
        lower.includes("disk full")) {
        return ("OpenClaw could not write local session data because the disk is full. " +
...
export function isInvalidStreamingEventOrderError(raw) {
    if (!raw) {
        return false;
    }
    const lower = normalizeLowercaseStringOrEmpty(raw);
    return (lower.includes("unexpected event order") &&
...
export function isLikelyHttpErrorText(raw) {
    if (isCloudflareOrHtmlErrorPage(raw)) {
        return true;
    }
