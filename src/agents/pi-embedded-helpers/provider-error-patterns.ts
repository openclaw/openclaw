...
type ProviderErrorPattern = {
  ...
  /** Optional: only match when this provider is in use. */
  provider?: string;
};

/**
 * Provider-specific context overflow patterns not covered by the generic
 * `isContextOverflowError()` in errors.ts. These catch provider-specific
 * wording that the generic regex misses.
 */
...
export const PROVIDER_SPECIFIC_PATTERNS: readonly ProviderErrorPattern[] = [
  // Google Vertex / Gemini: RESOURCE_EXHAUSTED is already handled as rate_limit
  // in failover-matches.ts, but INVALID_ARGUMENT with token messages is context overflow.
  {
    test: /INVALID_ARGUMENT.*(?:exceeds? the (?:maximum|max)|input.*too (?:long|large))/i,
    reason: "format", // context overflow manifests as format error in Vertex
  },
  ...
  // AWS Bedrock: ModelNotReadyException
  {
    test: /ModelNotReadyException|model is not ready/i,
    reason: "overloaded",
  },
 ...
 * Call this from `isLikelyContextOverflowError()` to catch provider-specific wording.
 */
