...
describe("same-line duplicate collapse", () => {
  it("removes duplicated sentences joined after tag stripping", () => {
    const input =
      "Running on Opus 4.6. What do you need?Running on Opus 4.6. What do you need?";
    expect(sanitizeUserFacingText(input)).toBe(
      "Running on Opus 4.6. What do you need?",
    );
  });
  ...
  it("keeps trailing different text after duplicate", () => {
    ...
    expect(sanitizeUserFacingText(input)).toBe(
      "Atlas here, Dion. What do you need?Fair point.",
    );
  });
