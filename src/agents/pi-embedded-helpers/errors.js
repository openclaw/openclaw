...
import { extractLeadingHttpStatus, formatRawAssistantErrorForUi, } from "../../shared/assistant-error-format.js";
...
import { isAuthErrorMessage, isAuthPermanentErrorMessage, isBillingErrorMessage, isOverloadedErrorMessage, isPeriodicUsageLimitErrorMessage, isRateLimitErrorMessage, isServerErrorMessage, isTimeoutErrorMessage, matchesFormatErrorPattern, } from "./failover-matches.js";
import { classifyProviderSpecificError, matchesProviderContextOverflow, } from "./provider-error-patterns.js";
import { formatBillingErrorMessage, formatDiskSpaceErrorCopy, formatRateLimitOrOverloadedErrorCopy, formatTransportErrorCopy, isInvalidStreamingEventOrderError, isLikelyHttpErrorText, isRawApiErrorPayload, } from "./sanitize-user-facing-text.js";
...
export function isReasoningConstraintErrorMessage(raw) {
    if (!raw) {
        return false;
    }
...
export function extractObservedOverflowTokenCount(errorMessage) {
    if (!errorMessage) {
        return undefined;
    }
...
export function inferSignalStatus(signal) {
...
export function isUnclassifiedNoBodyHttpSignal(signal) {
...
function isHtmlErrorResponse(raw, status) {
    ...
    const candidate = extractLeadingHttpStatus(trimmed) ? trimmed : stripErrorPrefix(trimmed);
    const inferred = typeof status === "number" && Number.isFinite(status)
        ? status
...
function isOAuthRefreshContentionMessage(raw) {
    return (/\brefresh_contention\b/i.test(raw) ||
...
function hasKnownBareLeading402Signal(text) {
    return (hasQuotaRefreshWindowSignal(text) ||
...
export function classifyFailoverReasonFromHttpStatus(status, message, opts) {
    const messageClassification = message
        ? classifyFailoverClassificationFromMessage(message, opts?.provider)
        : null;
    return failoverReasonFromClassification(classifyFailoverClassificationFromHttpStatus(status, message, messageClassification, status));
}
...
function isOpenRouterProviderReturnedError(raw, provider) {
    return (isProvider(provider, "openrouter") &&
...
export function classifyProviderRuntimeFailureKind(signal) {
    ...
    if (failoverClassification?.kind === "reason" &&
...
export function formatAssistantErrorText(msg, opts) {
    ...
    if (providerRuntimeFailureKind === "replay_invalid") {
        return ("Session history or replay state is invalid. " +
...
export function classifyFailoverReason(raw, opts) {
    return failoverReasonFromClassification(classifyFailoverSignal({
        message: raw,
        provider: opts?.provider,
    }));
}
export function isFailoverErrorMessage(raw, opts) {
