...
type CliBackendRuntimeModule =
	typeof import("../plugins/cli-backends.runtime.js");
...
export function isCliProvider(provider: string, cfg?: OpenClawConfig): boolean {
	...
	const cliBackends =
		loadCliBackendRuntime()?.resolveRuntimeCliBackends() ?? [];
	if (
		cliBackends.some(
			(backend) => normalizeProviderId(backend.id) === normalized,
		)
	) {
		return true;
	}
