...
function normalizeProviderModelId(provider: string, model: string): string {
  ...
  // OpenRouter-compatible native routing providers (OpenRouter, Dgrid) need
  // the full "<provider>/<name>" as the model ID sent to the API. Models from
  // external providers already contain a slash (e.g.
  // "anthropic/claude-sonnet-4-5") and are passed through as-is (#12924).
  if ((provider === "openrouter" || provider === "dgrid") && !model.includes("/")) {
    return `${provider}/${model}`;
  }
  return model;
}
