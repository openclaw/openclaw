...
import { isActiveUnusableWindow, isAuthCooldownBypassedForProvider, resolveProfileUnusableUntil, } from "./usage-state.js";
...
function applyWhamCooldownResult(params) {
    ...
    const existingActiveCooldownUntil = typeof existingCooldownUntil === "number" &&
...
export async function probeWhamForCooldown(store, profileId) {
...
function resolveAuthCooldownConfig(params) {
    ...
    const billingBackoffHours = resolvePositiveNumber(billingOverride ?? cooldowns?.billingBackoffHours, defaults.billingBackoffHours);
...
function computeNextProfileUsageStats(params) {
    ...
    if (disabledFailureReason) {
        ...
        const existingCooldownActive = typeof params.existing.cooldownUntil === "number" &&
