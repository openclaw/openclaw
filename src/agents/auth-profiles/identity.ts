...
function resolveStoredMetadata(
	...
	return {
		displayName:
			"displayName" in profile
				? trimOptionalString(profile.displayName)
				: undefined,
...
export function buildAuthProfileId(params: {
	...
	const profilePrefix =
		trimOptionalString(params.profilePrefix) ?? params.providerId;
...
export function resolveAuthProfileMetadata(params: {
	...
	return {
		displayName:
			trimOptionalString(configured?.displayName) ?? stored.displayName,
