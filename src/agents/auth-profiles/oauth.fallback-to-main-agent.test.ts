...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveDNAAgentDir() returns mainAgentDir
    process.env.DNA_STATE_DIR = tmpDir;
    process.env.DNA_AGENT_DIR = mainAgentDir;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.DNA_AGENT_DIR;
    else process.env.DNA_AGENT_DIR = previousAgentDir;
