...
// keep it store-scoped rather than generalizing prematurely. Kept as its own
// raw SQLite read (rather than layered on `inspectAuthProfileJsonCellReadOnly`
// below) because it preserves the caught `error` object on the unreadable
// path, which `PersistedAuthProfileStoreInspection`'s `unreadable` variant
// does not carry — the fingerprint path's callers want that detail.
