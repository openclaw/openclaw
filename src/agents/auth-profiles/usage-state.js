...
export function isAuthCooldownBypassedForProvider(provider) {
...
export function isActiveUnusableWindow(until, now) {
    return typeof until === "number" && Number.isFinite(until) && until > 0 && now < until;
}
export function shouldBypassModelScopedCooldown(stats, now, forModel) {
    return !!(forModel &&
 ...
 */
export function isProfileInCooldown(store, profileId, now, forModel) {
 ...
 */
export function getSoonestCooldownExpiry(store, profileIds, options) {
    ...
    for (const id of profileIds) {
        ...
        const matchingModelScopedCooldown = options?.forModel &&
            stats.cooldownReason === "rate_limit" &&
...
export function clearExpiredCooldowns(store, now) {
    ...
    for (const [profileId, stats] of Object.entries(usageStats)) {
        ...
        const cooldownExpired = typeof stats.cooldownUntil === "number" &&
            Number.isFinite(stats.cooldownUntil) &&
            stats.cooldownUntil > 0 &&
            ts >= stats.cooldownUntil;
        const disabledExpired = typeof stats.disabledUntil === "number" &&
