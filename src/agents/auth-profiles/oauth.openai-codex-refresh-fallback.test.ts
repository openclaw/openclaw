  ...
  return {
    ...
    profiles: {
      [params.profileId]: {
        ...
        access: params.access ?? "cached-access-token", // pragma: allowlist secret (fixture token)
        refresh: "refresh-token", // pragma: allowlist secret (fixture token)
...
describe("resolveApiKeyForProfile openai-codex refresh fallback", () => {
  ...
  it("falls back to cached access token when openai-codex refresh fails on accountId extraction", async () => {
    ...
    expect(result).toEqual({
      apiKey: "cached-access-token", // pragma: allowlist secret (fixture token)
      provider: "openai-codex",
      email: undefined,
    });
