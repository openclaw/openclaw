		...
		async () => {
			await withTempDir(async (dir) => {
				...
				const outside = await fs.mkdtemp(
					path.join(os.tmpdir(), "openclaw-patch-outside-"),
				);
				...
				try {
					await withRealpathSymlinkRebindRace({
						shouldFlip: (realpathInput) =>
							realpathInput.endsWith(path.join("slot")),
						symlinkPath: slot,
...
*** End Patch`;
				...
				try {
					await withRealpathSymlinkRebindRace({
						shouldFlip: (realpathInput) =>
							realpathInput.endsWith(
								path.join("slot", "nested", "deep", "file.txt"),
							),
						symlinkPath: slot,
						symlinkTarget: outside,
						timing: "before-realpath",
					...
					await expect(
						fs.stat(path.join(outside, "nested")),
