...
import { loadSessionStore, resolveAgentIdFromSessionKey, resolveStorePath, } from "../config/sessions.js";
...
import { ANNOUNCE_EXPIRY_MS, MAX_ANNOUNCE_RETRY_COUNT, reconcileOrphanedRestoredRuns, reconcileOrphanedRun, resolveAnnounceRetryDelayMs, resolveSubagentRunOrphanReason, safeRemoveAttachmentsDir, } from "./subagent-registry-helpers.js";
...
function resolveCompletionFromSessionEntry(sessionEntry, fallbackEndedAt) {
    ...
    const endedAt = typeof sessionEntry?.endedAt === "number" && Number.isFinite(sessionEntry.endedAt)
...
async function resolveSubagentRegistryContextEngine(cfg) {
    ...
    const ensureContextEnginesInitialized = subagentRegistryDeps.ensureContextEnginesInitialized ??
...
const { clearScheduledResumeTimers, completeCleanupBookkeeping, completeSubagentRun, finalizeResumedAnnounceGiveUp, refreshFrozenResultFromSession, startSubagentAnnounceCleanupFlow, } = subagentLifecycleController;
