...
import { resolveRoots } from "./agent-tools.fs-roots.js";
import {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await expect(readTool.execute("tc-roots-read-symlink", { path: aliasFile })).rejects.toThrow(
        // main's fs-safe rejects escaping symlinks with "symlink open blocked"; the
        // PR's older wording is kept too so the assertion stays implementation-agnostic.
        /outside workspace root|regular file under root|symlink/i,
      );
    },
  );
});
