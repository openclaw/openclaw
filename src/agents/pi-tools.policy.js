...
import { resolveSubagentCapabilityStore, resolveStoredSubagentCapabilities, } from "./subagent-capabilities.js";
...
export function resolveSubagentToolPolicyForSession(cfg, sessionKey, opts) {
    const configured = cfg?.tools?.subagents?.tools;
    ...
    const explicitAllow = new Set([...(allow ?? []), ...(alsoAllow ?? [])].map((toolName) => normalizeToolName(toolName)));
    const deny = [
        ...resolveSubagentDenyListForRole(capabilities.role).filter((toolName) => !explicitAllow.has(normalizeToolName(toolName))),
        ...(Array.isArray(configured?.deny) ? configured.deny : []),
    ];
    const mergedAllow = allow && alsoAllow ? Array.from(new Set([...allow, ...alsoAllow])) : allow;
    return { allow: mergedAllow, deny };
...
export function resolveGroupContextFromSessionKey(sessionKey) {
...
function resolveImplicitProfileAlsoAllow(params) {
    ...
    if (hasExplicitToolSection(params.agentTools?.exec) ||
        hasExplicitToolSection(params.globalTools?.exec)) {
        implicit.add("exec");
        implicit.add("process");
    }
    if (hasExplicitToolSection(params.agentTools?.fs) ||
...
export function resolveEffectiveToolPolicy(params) {
    ...
    const profileAlsoAllow = explicitProfileAlsoAllow || implicitProfileAlsoAllow
        ? Array.from(new Set([...(explicitProfileAlsoAllow ?? []), ...(implicitProfileAlsoAllow ?? [])]))
        : undefined;
    return {
        agentId,
