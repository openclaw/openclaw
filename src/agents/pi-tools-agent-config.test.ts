...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "activi-pi-tools-"));
    ...
    try {
      ...
      const tools = createActiviCodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createActiviCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createActiviCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createActiviCodingTools({
      config: cfg,
    ...
    const defaultTools = createActiviCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createActiviCodingTools({
      config: cfg,
    ...
    const bobTools = createActiviCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createActiviCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createActiviCodingTools({
      config: cfg,
