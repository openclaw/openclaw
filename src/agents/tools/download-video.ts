  ...
  execute: async (
    ...
    try {
      ...
      if (typedParams.quality === "720") {
        ...
        typedParams.quality &&
        typeof typedParams.quality === "string" &&
        typedParams.quality.includes("[")
      ) {
        formatSelector = typedParams.quality;
      }

      const { stdout: rawTitle } = await execFileAsync(ytDlpPath, [
        "--get-title",
      ...
      const { stdout: ytDlpOutput } = await execFileAsync(
        ytDlpPath,
        [
          "-f",
          formatSelector,
