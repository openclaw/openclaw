...
import { extractBasicHtmlContent, htmlToMarkdown, markdownToText, truncateText, } from "./web-fetch-utils.js";
import { DEFAULT_CACHE_TTL_MINUTES, DEFAULT_TIMEOUT_SECONDS, normalizeCacheKey, readCache, readResponseText, resolveCacheTtlMs, resolveTimeoutSeconds, writeCache, } from "./web-shared.js";
...
function normalizeProviderWebFetchPayload(params) {
    ...
    const status = typeof payload.status === "number" && Number.isFinite(payload.status)
        ...
        : 200;
    const contentType = typeof payload.contentType === "string" ? normalizeContentType(payload.contentType) : undefined;
    const title = typeof payload.title === "string" ? wrapWebFetchField(payload.title) : undefined;
    const warning = typeof payload.warning === "string" ? wrapWebFetchField(payload.warning) : undefined;
    const extractor = typeof payload.extractor === "string" && payload.extractor.trim()
        ? payload.extractor
        : params.providerId;
    return {
        url,
        ...
        tookMs: typeof payload.tookMs === "number" && Number.isFinite(payload.tookMs)
