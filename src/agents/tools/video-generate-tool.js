...
import { resolveVideoGenerationMode, resolveVideoGenerationModeCapabilities, } from "../../video-generation/capabilities.js";
...
import { applyVideoGenerationModelConfigDefaults, buildMediaReferenceDetails, buildTaskRunDetails, normalizeMediaReferenceInputs, readBooleanToolParam, readGenerationTimeoutMs, resolveCapabilityModelConfigForTool, resolveGenerateAction, resolveMediaToolLocalRoots, resolveRemoteMediaSsrfPolicy, resolveSelectedCapabilityProvider, } from "./media-tool-shared.js";
import { createSandboxBridgeReadFile, resolveSandboxedBridgeMediaPath, } from "./tool-runtime.helpers.js";
import { completeVideoGenerationTaskRun, createVideoGenerationTaskRun, failVideoGenerationTaskRun, recordVideoGenerationTaskProgress, wakeVideoGenerationTaskCompletion, } from "./video-generate-background.js";
import { createVideoGenerateDuplicateGuardResult, createVideoGenerateListActionResult, createVideoGenerateStatusActionResult, } from "./video-generate-tool.actions.js";
...
const VideoGenerateToolSchema = Type.Object({
    ...
    imageRoles: Type.Optional(Type.Array(Type.String(), {
        description: "Optional semantic roles for the combined reference image list, parallel by index. " +
    ...
    videoRoles: Type.Optional(Type.Array(Type.String(), {
        description: "Optional semantic roles for the combined reference video list, parallel by index. " +
    ...
    audioRef: Type.Optional(Type.String({
        description: "Optional single reference audio path or URL (e.g. background music).",
    })),
    audioRefs: Type.Optional(Type.Array(Type.String(), {
        description: `Optional reference audios (up to ${MAX_INPUT_AUDIOS}).`,
    })),
    audioRoles: Type.Optional(Type.Array(Type.String(), {
        description: "Optional semantic roles for the combined reference audio list, parallel by index. " +
    ...
    providerOptions: Type.Optional(Type.Record(Type.String(), Type.Unknown(), {
        description: 'Optional provider-specific options as a JSON object, e.g. `{"seed": 42, "draft": true}`. ' +
...
function parseRoleArray(params) {
    ...
    if (!Array.isArray(params.raw)) {
        throw new ToolInputError(`${params.kind} must be a JSON array of role strings, parallel to the reference list.`);
    }
    const roles = params.raw.map((entry) => (typeof entry === "string" ? entry.trim() : ""));
    if (roles.length > params.assetCount) {
        throw new ToolInputError(`${params.kind} has ${roles.length} entries but only ${params.assetCount} reference ${params.kind === "imageRoles" ? "image" : params.kind === "videoRoles" ? "video" : "audio"}${params.assetCount === 1 ? "" : "s"} were provided; extra roles cannot be aligned positionally.`);
    }
    return roles;
}
...
function validateVideoGenerationCapabilities(params) {
    ...
    if (params.inputVideoCount > 0) {
        ...
        if (params.inputVideoCount > maxInputVideos) {
            throw new ToolInputError(`${provider.id} supports at most ${maxInputVideos} reference video${maxInputVideos === 1 ? "" : "s"}.`);
        }
    }
...
async function loadReferenceAssets(params) {
    ...
    for (const rawInput of params.inputs) {
        ...
        if (refInfo.hasUnsupportedScheme) {
            throw new ToolInputError(`Unsupported ${params.expectedKind} reference: ${rawInput}. Use a file path, a file:// URL, a data: URL, or an http(s) URL.`);
        }
        if (params.sandboxConfig && isHttpUrl) {
            throw new ToolInputError(`Sandboxed video_generate does not allow remote ${params.expectedKind} URLs.`);
        }
        const resolvedInput = (() => {
...
async function executeVideoGenerationJob(params) {
    ...
    const warning = ignoredOverrides.length > 0
        ...
        : undefined;
    const normalizedDurationSeconds = result.normalization?.durationSeconds?.applied ??
    ...
    const supportedDurationSeconds = result.normalization?.durationSeconds?.supportedValues ??
        (Array.isArray(result.metadata?.supportedDurationSeconds)
            ? result.metadata.supportedDurationSeconds.filter((entry) => typeof entry === "number" && Number.isFinite(entry))
            : undefined);
    const normalizedSize = result.normalization?.size?.applied ??
        (typeof result.metadata?.normalizedSize === "string" && result.metadata.normalizedSize.trim()
            ? result.metadata.normalizedSize
            : undefined);
    const normalizedAspectRatio = result.normalization?.aspectRatio?.applied ??
        (typeof result.metadata?.normalizedAspectRatio === "string" &&
            ...
            : undefined);
    const normalizedResolution = result.normalization?.resolution?.applied ??
        (typeof result.metadata?.normalizedResolution === "string" &&
            ...
            : undefined);
    const sizeTranslatedToAspectRatio = result.normalization?.aspectRatio?.derivedFrom === "size" ||
        (!normalizedSize &&
...
export function createVideoGenerateTool(options) {
    ...
    return {
        ...
        execute: async (_toolCallId, rawArgs) => {
            ...
            if (providerOptionsRaw != null &&
                (typeof providerOptionsRaw !== "object" || Array.isArray(providerOptionsRaw))) {
                throw new ToolInputError("providerOptions must be a JSON object keyed by provider-specific option name.");
            }
            const providerOptions = providerOptionsRaw != null ? providerOptionsRaw : undefined;
