...
export function readConfiguredSecretString(value, path) {
...
export async function postTrustedWebToolsJson(params, parseResponse) {
    const withTrustedWebToolsEndpoint = await loadTrustedWebToolsEndpoint();
    return withTrustedWebToolsEndpoint({
        url: params.url,
        timeoutSeconds: params.timeoutSeconds,
...
export async function throwWebSearchApiError(res, providerLabel) {
...
export const RECENCY_TO_FRESHNESS = {
    day: "pd",
    week: "pw",
    month: "pm",
    year: "py",
...
export function isoToPerplexityDate(iso) {
...
export function normalizeFreshness(value, provider) {
    if (!value) {
        return undefined;
    }
    const trimmed = value.trim();
...
export function buildSearchCacheKey(parts) {
    return normalizeCacheKey(parts.map((part) => (part === undefined ? "default" : String(part))).join(":"));
}
...
export function buildUnsupportedSearchFilterResponse(params, provider, docs = "https://docs.openclaw.ai/tools/web") {
