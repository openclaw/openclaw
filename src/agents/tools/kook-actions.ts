...
function classifyKookActionError(error: unknown): Error {
  const message = error instanceof Error ? error.message : String(error);
  if (/disabled/i.test(message)) {
    return new Error(`KOOK action blocked by config: ${message}`);
  }
  if (
    /required|missing|invalid|unknown kook action|unknown kook .* action|target format|channelId or to/i.test(
      message,
    )
  ) {
    return new Error(`KOOK action invalid params: ${message}`);
  }
  return new Error(`KOOK platform/runtime error: ${message}`);
}
...
export async function handleKookAction(
  ...
  try {
    ...
    if (action === "getGuildUsers" && !isActionEnabled("getGuildUsers")) {
    ...
    throw classifyKookActionError(error);
  }
}
