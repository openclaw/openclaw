...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "evox" }),
                      },
                    },
                  ],
                },
    ...
    const result = await tool?.execute?.("call-1", { query: "latest evox release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://evox.sh/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://evox.sh/docs"]);
