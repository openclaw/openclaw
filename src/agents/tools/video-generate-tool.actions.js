...
import { buildVideoGenerationTaskStatusDetails, buildVideoGenerationTaskStatusText, findActiveVideoGenerationTaskForSession, } from "../video-generation-task-status.js";
...
function summarizeVideoGenerationCapabilities(provider) {
    ...
    const maxInputAudios = generate?.maxInputAudios ??
...
export function createVideoGenerateListActionResult(config) {
...
export function createVideoGenerateStatusActionResult(sessionKey) {
    return videoGenerateTaskStatusActions.createStatusActionResult(sessionKey);
}
export function createVideoGenerateDuplicateGuardResult(sessionKey) {
