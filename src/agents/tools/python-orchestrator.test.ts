...
describe("python_orchestrator tool", () => {
  ...
  it("executes simple Python code", async () => {
    ...
    expect((result.details as { exit_code: number }).exit_code).toBe(0);
  });
  ...
  it("handles Python errors", async () => {
    ...
    expect((result.content[0] as { type: string; text: string }).text).toContain("failed");
    expect((result.details as { exit_code: number }).exit_code).not.toBe(0);
  });
