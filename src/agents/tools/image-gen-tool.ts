...
const SGLANG_DIFFUSION_DEFAULT_BASE_URL = "http://127.0.0.1:30000/v1";
...
const ImageGenToolSchema = Type.Object({
  ...
  num_inference_steps: Type.Optional(
    Type.Number({ description: "Number of diffusion denoising steps." }),
  ),
...
function resolveSglangDiffusionConfig(cfg?: OpenClawConfig): {
  ...
  if (provider) {
    ...
    let apiKey = resolveSecretInputString(provider.apiKey);
    // Resolve ${ENV_VAR} or bare UPPER_SNAKE_CASE env var names (onboarding writes bare names)
    const envWrapMatch = /^\$\{([A-Z0-9_]+)\}$/.exec(apiKey);
    if (envWrapMatch?.[1]) {
      apiKey = process.env[envWrapMatch[1]]?.trim() ?? "";
    } else if (/^[A-Z][A-Z0-9_]*$/.test(apiKey)) {
...
export function createImageGenTool(opts?: { config?: OpenClawConfig }): AnyAgentTool | null {
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal) => {
      ...
      try {
        ...
        const revisedNote = imageData.revised_prompt
