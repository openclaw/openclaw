...
import { resolveSubagentLabel, resolveSubagentTargetFromRuns, sortSubagentRuns, } from "../../auto-reply/reply/subagents-utils.js";
...
import { clearSubagentRunSteerRestart, countPendingDescendantRuns, listSubagentRunsForRequester, markSubagentRunTerminated, markSubagentRunForSteerRestart, replaceSubagentRunAfterSteer, } from "../subagent-registry.js";
...
const STEER_RATE_LIMIT_MS = 2000;
const STEER_ABORT_SETTLE_TIMEOUT_MS = 5000;
...
export function createSubagentsTool(opts) {
    return {
        ...
        execute: async (_toolCallId, args) => {
            ...
            if (action === "list") {
                const now = Date.now();
                const recentCutoff = now - recentMinutes * 60000;
                const cache = new Map();
                const pendingDescendantCache = new Map();
                const hasPendingDescendants = (sessionKey) => {
