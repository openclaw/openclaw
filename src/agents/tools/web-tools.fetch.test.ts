...
function errorHtmlResponse(
  ...
  return {
    ...
    url,
    headers: contentType
      ? makeHeaders({ "content-type": contentType })
      : makeHeaders({}),
    text: async () => html,
  };
}
...
describe("web_fetch extraction fallbacks", () => {
    ...
    installMockFetch((input: RequestInfo | URL) => {
      ...
      if (url.includes("api.firecrawl.dev")) {
        return Promise.resolve(
          firecrawlResponse("firecrawl content"),
        ) as Promise<Response>;
      }
      return Promise.resolve(
        htmlResponse(
  ...
  it("normalizes firecrawl Authorization header values", async () => {
    const fetchSpy = installMockFetch((input: RequestInfo | URL) => {
      ...
      if (url.includes("api.firecrawl.dev/v2/scrape")) {
        return Promise.resolve(
          firecrawlResponse("firecrawl normalized"),
        ) as Promise<Response>;
      }
      return Promise.resolve(
        htmlResponse(
  ...
  it("uses firecrawl when direct fetch fails", async () => {
    installMockFetch((input: RequestInfo | URL) => {
      ...
      if (url.includes("api.firecrawl.dev")) {
        return Promise.resolve(
          firecrawlResponse("firecrawl fallback", url),
        ) as Promise<Response>;
      }
      return Promise.resolve({
        ok: false,
  ...
  it("wraps external content and clamps oversized maxChars", async () => {
    ...
    installMockFetch(
      (input: RequestInfo | URL) =>
        Promise.resolve(
          textResponse(large, requestUrl(input)),
        ) as Promise<Response>,
    );
  ...
  it("strips HTML errors when content-type is missing", async () => {
    ...
    installMockFetch(
      (input: RequestInfo | URL) =>
        Promise.resolve(
          errorHtmlResponse(html, 500, requestUrl(input), null),
        ) as Promise<Response>,
    );
