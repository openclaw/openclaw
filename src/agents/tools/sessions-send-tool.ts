...
function resolveGatewayCaller(
  ...
  override?: GatewayCaller,
): GatewayCaller {
  if (!config) {
    return override ?? callGateway;
  }
  const gatewayCaller = override ?? callGateway;
  return (async <T = Record<string, unknown>>(request: Parameters<GatewayCaller>[0]) =>
    await gatewayCaller<T>({
      config,
      ...request,
    })) as GatewayCaller;
}
...
export function createSessionsSendTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const gatewayCall = resolveGatewayCaller(cfg, opts?.callGateway);
      const rpcTimeoutMs = resolveGatewayRpcTimeoutMs(cfg ?? loadConfig());
      ...
      try {
        const wait = await gatewayCall<{ status?: string; error?: string }>({
          ...
          timeoutMs: resolveGatewayWaitCallTimeoutMs(cfg, timeoutMs),
        });
