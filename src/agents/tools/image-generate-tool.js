...
import { resolveConfiguredMediaMaxBytes, resolveGeneratedMediaMaxBytes, } from "../../media/configured-max-bytes.js";
...
import { applyImageGenerationModelConfigDefaults, buildMediaReferenceDetails, isCapabilityProviderConfigured, normalizeMediaReferenceInputs, readGenerationTimeoutMs, resolveRemoteMediaSsrfPolicy, resolveCapabilityModelConfigForTool, resolveGenerateAction, resolveMediaToolLocalRoots, resolveSelectedCapabilityProvider, } from "./media-tool-shared.js";
...
function normalizeQuality(raw) {
    ...
    if (SUPPORTED_QUALITIES.includes(normalized)) {
        return normalized;
    }
...
function normalizeOutputFormat(raw) {
    ...
    if (SUPPORTED_OUTPUT_FORMATS.includes(normalized)) {
        return normalized;
    }
...
function normalizeOpenAIBackground(raw) {
    ...
    if (SUPPORTED_BACKGROUNDS.includes(normalized)) {
        return normalized;
    }
...
function normalizeOpenAIModeration(raw) {
    ...
    if (SUPPORTED_OPENAI_MODERATIONS.includes(normalized)) {
        return normalized;
    }
...
export function createImageGenerateTool(options) {
    ...
    return {
        ...
        execute: async (_toolCallId, args) => {
            ...
            const modeCaps = inputImages.length > 0
