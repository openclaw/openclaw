...
export async function handleDiscordMessagingAction(action, params, isActionEnabled, options) {
    ...
    switch (action) {
        ...
        case "sendMessage": {
            ...
            const componentSpec = rawComponents && typeof rawComponents === "object" && !Array.isArray(rawComponents)
                ? readDiscordComponentSpec(rawComponents)
                : null;
            const components = Array.isArray(rawComponents) || typeof rawComponents === "function"
                ? rawComponents
                : undefined;
            const content = readStringParam(params, "content", {
