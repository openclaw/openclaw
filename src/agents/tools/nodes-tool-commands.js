...
export async function executeNodeCommandAction(params) {
    switch (params.action) {
        ...
        case "device_health": {
            ...
            const payload = payloadRaw && typeof payloadRaw === "object" && payloadRaw !== null ? payloadRaw : {};
        ...
        case "notifications_action": {
            ...
            if (notificationAction !== "open" &&
                notificationAction !== "dismiss" &&
                notificationAction !== "reply") {
                throw new Error("notificationAction must be open|dismiss|reply");
            }
            const notificationReplyText = typeof params.input.notificationReplyText === "string"
        ...
        case "location_get": {
            const node = readStringParam(params.input, "node", { required: true });
            const maxAgeMs = typeof params.input.maxAgeMs === "number" && Number.isFinite(params.input.maxAgeMs)
                ? params.input.maxAgeMs
                : undefined;
            const desiredAccuracy = params.input.desiredAccuracy === "coarse" ||
                params.input.desiredAccuracy === "balanced" ||
                params.input.desiredAccuracy === "precise"
                ? params.input.desiredAccuracy
                : undefined;
            const locationTimeoutMs = typeof params.input.locationTimeoutMs === "number" &&
        ...
        case "invoke": {
            ...
            if (BLOCKED_INVOKE_COMMANDS.has(invokeCommandNormalized)) {
                throw new Error(`invokeCommand "${invokeCommand}" is reserved for shell execution; use exec with host=node instead`);
            }
            ...
            if (dedicatedAction && !params.allowMediaInvokeCommands) {
                throw new Error(`invokeCommand "${invokeCommand}" returns media payloads and is blocked to prevent base64 context bloat; use action="${dedicatedAction}"`);
            }
            const invokeParamsJson = typeof params.input.invokeParamsJson === "string"
