...
function isLocalMacNode(node) {
    return (normalizeOptionalLowercaseString(node.platform)?.startsWith("mac") === true &&
...
export function selectDefaultNodeFromList(nodes, options = {}) {
...
export async function resolveNode(opts, query, allowDefault = false) {
