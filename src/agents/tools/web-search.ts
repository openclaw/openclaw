...
const SEARCH_PROVIDERS = ["brave", "gemini", "grok", "kimi", "mistral", "perplexity"] as const;
...
const MISTRAL_AGENT_CACHE_KEY = "mistral_dynamic_agent_id"; // key to cache dynamically created agent-ids
...
async function runMistralSearch(params: {
  ...
  if (!agent_id) {
    const cachedAgent = readCache(SEARCH_CACHE, MISTRAL_AGENT_CACHE_KEY);
    if (cachedAgent?.cached) {
      ...
      agent_id = agentRes;
      writeCache(SEARCH_CACHE, MISTRAL_AGENT_CACHE_KEY, { id: agent_id }, 3600000); // TTL: 1 hour
    }
  }
