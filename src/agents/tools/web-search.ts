...
type WebSearchConfig = NonNullable<ActiviConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ActiviConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "perplexity") {
    return {
      ...
      docs: "https://docs.activi.ai/tools/web",
    };
  }
  if (provider === "grok") {
    return {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("activi configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.activi.ai/tools/web",
  };
}
