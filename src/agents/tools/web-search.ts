...
type WebSearchConfig = NonNullable<SmartAgentNeoConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SmartAgentNeoConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "perplexity") {
    return {
      ...
      docs: "https://docs.smart-agent-neo.ai/tools/web",
    };
  }
  if (provider === "grok") {
    return {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("smart-agent-neo configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.smart-agent-neo.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "X-Title": "SmartAgentNeo Web Search",
    },
