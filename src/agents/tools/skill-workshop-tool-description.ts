...
export function buildSkillWorkshopToolDescription(params: {
  ...
  return `${SKILL_WORKSHOP_TOOL_DISPLAY_SUMMARY} Stage pending proposals to create or update reusable-procedure skills in your agent's Workshop directory. Create and update do not publish or activate skills; a later apply step makes the proposal active. Read and prepare_patch establish content and exact-span prerequisites only; they do not count as using a skill for foreground repair. A foreground patch requires a genuine run-usage receipt from normal skill activation. For planned authoring, use update with the complete skill body to stage a proposal, then apply it according to the configured approval policy. Read, prepare an exact bounded patch, patch, revise, inspect, evaluate, and apply Workshop proposals. The operator edits all other skills directly. Restore a retained backup from the previous collection-review implementation when the user asks. New reviews use automation history and do not create collection backups. ${repairPolicy}\n\n${SKILL_AUTHORING_STANDARDS_PROMPT}`;
}
