...
function collectChangedConfigPaths(currentValue, nextValue, basePath = "", out = new Set()) {
    ...
    if (isPlainObject(currentValue) && isPlainObject(nextValue)) {
        ...
        for (const key of keys) {
            collectChangedConfigPaths(currentValue[key], nextValue[key], basePath ? `${basePath}.${key}` : key, out);
        }
        return out;
    }
    out.add(normalizeGatewayConfigPath(basePath));
...
function assertGatewayConfigMutationAllowed(params) {
    ...
    const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig);
