...
export function createMessageTool(options?: MessageToolOptions): AnyAgentTool {
  ...
  const description = options?.sourceReplyOnly
    ...
    : `${buildMessageToolDescription(actions)}${currentChannelIsInternal ? ' When the user asks whether you can perform an action or install a capability, use action="send" with clawhub={intent:"recommend",query:"capability"} to check official plugins and skills and present installation cards. Omit channel, target, and ordinary send fields; use clawhub.intro for optional introductory text. Installed capabilities show their current status; the card opens the listing inside Control UI.' : ""}`;
