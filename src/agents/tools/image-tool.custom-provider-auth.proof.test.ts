...
import { hasAuthForProvider, hasProviderAuthForTool } from "./model-config.helpers.js";
...
async function withEmptyAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-image-auth-proof-"));
  try {
...
function simulatePreFixCandidateFilter(params: { agentDir: string; candidate: string }): boolean {
  const provider = params.candidate.slice(0, params.candidate.indexOf("/")).trim();
...
describe("image custom provider auth fix proof", () => {
  ...
  it("documents why pre-fix auth filtering dropped the candidate", async () => {
    await withEmptyAgentDir(async (agentDir) => {
      const cfg = createUserReportedConfig();
      expect(
        simulatePreFixCandidateFilter({
          agentDir,
          candidate: USER_PRIMARY,
        }),
      ).toBe(false);
      expect(hasProviderAuthForTool({ provider: USER_PROVIDER, cfg })).toBe(true);
    });
