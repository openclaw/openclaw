    ...
    models: {
      providers: {
        "sglang-diffusion": {
          ...
          models: [{ id: "FLUX.1-dev", name: "FLUX.1-dev" }],
        },
      },
    },
  })),
...
describe("createImageGenTool", () => {
  it("returns null when no provider is configured", () => {
    // Pass an empty config explicitly — no sglang-diffusion provider, env fallback returns null.
  ...
  it("creates tool when sglang-diffusion provider is configured", () => {
    // Default mock returns config with sglang-diffusion provider.
    const tool = createImageGenTool();
