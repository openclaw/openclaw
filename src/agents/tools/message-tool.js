...
import { channelSupportsMessageCapability, channelSupportsMessageCapabilityForChannel, listCrossChannelSchemaSupportedMessageActions, resolveChannelMessageToolSchemaProperties, } from "../../channels/plugins/message-action-discovery.js";
...
function buildSendSchema(options) {
    ...
    if (options.includeDeliveryPin) {
        props.delivery = Type.Optional(Type.Object({
            pin: Type.Optional(Type.Union([
                Type.Boolean(),
                Type.Object({
...
function resolveMessageToolSchemaActions(params) {
    ...
    if (currentChannel) {
        ...
        for (const plugin of listChannelPlugins()) {
            ...
            for (const action of listCrossChannelSchemaSupportedMessageActions(buildMessageActionDiscoveryInput(params, plugin.id))) {
...
function resolveIncludeCapability(params, capability) {
    const currentChannel = normalizeMessageChannel(params.currentChannelProvider);
    if (currentChannel) {
        return channelSupportsMessageCapabilityForChannel(buildMessageActionDiscoveryInput(params, currentChannel), capability);
    }
...
function buildMessageToolSchema(params) {
    ...
    const extraProperties = resolveChannelMessageToolSchemaProperties(buildMessageActionDiscoveryInput(params, normalizeMessageChannel(params.currentChannelProvider) ?? undefined));
...
function buildMessageToolDescription(options) {
    ...
    if (messageToolDiscoveryParams) {
        ...
        if (actions.length > 0) {
            return appendMessageToolReadHint(`${baseDescription} Supports actions: ${actions.join(", ")}.`, actions);
        }
    }
