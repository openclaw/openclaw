...
import { createCurrentTurnDelivery } from "./current-turn-delivery.js";
import {
...
function createPluginToolDelivery(params: {
  ...
  const delivery = createCurrentTurnDelivery({
    ...
    messageActionTurnCapability: params.options?.messageActionTurnCapability,
  });
  return delivery && { send: async (request) => void (await delivery.send(request)) };
}
