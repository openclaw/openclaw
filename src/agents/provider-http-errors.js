...
export function truncateErrorDetail(detail, limit = 220) {
    return detail.length <= limit ? detail : `${detail.slice(0, limit - 1)}…`;
}
export async function readResponseTextLimited(response, limitBytes = 16 * 1024) {
    if (limitBytes <= 0) {
        return "";
    }
...
export function formatProviderErrorPayload(payload) {
    ...
    const message = trimToUndefined(subject.message) ??
...
export async function extractProviderErrorDetail(response) {
...
export function extractProviderRequestId(response) {
    return (trimToUndefined(response.headers.get("x-request-id")) ??
...
export function formatProviderHttpErrorMessage(params) {
    ...
    return (`${label} (${statusPrefix}${status})` +
        (detail ? `: ${detail}` : "") +
        (requestId ? ` [request_id=${requestId}]` : ""));
}
export async function createProviderHttpError(response, label, options) {
...
export async function assertOkOrThrowHttpError(response, label) {
    if (response.ok) {
        return;
    }
