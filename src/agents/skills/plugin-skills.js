...
import { normalizePluginsConfigWithResolver, resolveEffectivePluginActivationState, resolveMemorySlotDecision, } from "../../plugins/config-policy.js";
import { loadPluginManifestRegistry, } from "../../plugins/manifest-registry.js";
import { hasKind } from "../../plugins/slots.js";
import { isPathInsideWithRealpath } from "../../security/scan-paths.js";
const log = createSubsystemLogger("skills");
function buildRegistryPluginIdAliases(registry) {
    return Object.fromEntries(registry.plugins
        .flatMap((record) => [
        ...record.providers
            .filter((providerId) => providerId !== record.id)
            .map((providerId) => [providerId, record.id]),
        ...(record.legacyPluginIds ?? []).map((legacyPluginId) => [legacyPluginId, record.id]),
    ])
