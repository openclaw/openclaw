...
import { resolveEffectiveAgentSkillFilter, resolveEffectiveAgentSkillsLimits, } from "./agent-filter.js";
...
function resolveCompactHomePrefixes() {
    const homes = [resolveHomeDir(), resolveUserHomeDir(), resolveNativeUserHomeDir()].filter((home) => !!home);
...
function resolveSkillsLimits(config, agentId) {
    ...
    return {
        ...
        maxSkillsPromptChars: agentSkillsLimits?.maxSkillsPromptChars ??
...
function warnEscapedSkillPath(params) {
    ...
    const rootResolved = path.resolve(params.rootDir) === params.rootRealPath
        ? ""
    ...
    skillsLogger.warn("Skipping escaped skill path outside its configured root.", {
        ...
        consoleMessage: `Skipping escaped skill path outside its configured root: ` +
...
function unwrapLoadedSkillRecords(loaded) {
    if (Array.isArray(loaded)) {
        return loaded.map((skill) => ({ skill }));
    }
    if (loaded && typeof loaded === "object" && "skills" in loaded) {
        const skills = loaded.skills;
        if (Array.isArray(skills)) {
            const loadedResult = loaded;
            const frontmatterByFilePath = loadedResult.frontmatterByFilePath instanceof Map
...
function loadSkillEntries(workspaceDir, opts) {
    ...
    const skillEntries = Array.from(merged.values())
        .sort((a, b) => a.skill.name.localeCompare(b.skill.name, "en"))
        .map((record) => {
...
export function loadVisibleWorkspaceSkillEntries(workspaceDir, opts) {
    const entries = loadSkillEntries(workspaceDir, opts);
    const effectiveSkillFilter = resolveEffectiveWorkspaceSkillFilter(opts);
    return filterSkillEntries(entries, opts?.config, effectiveSkillFilter, opts?.eligibility);
}
...
export function filterWorkspaceSkillEntriesWithOptions(entries, opts) {
