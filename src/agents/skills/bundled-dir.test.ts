...
describe("resolveBundledSkillsDir", () => {
  ...
  it("returns MULLUSI_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-bundled-override-"));
    process.env.MULLUSI_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-bundled-"));
