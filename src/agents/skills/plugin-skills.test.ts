    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "mullusi.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "mullusi.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "mullusi.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("mullusi-");
  const acpxRoot = await tempDirs.make("mullusi-acpx-plugin-");
  const helperRoot = await tempDirs.make("mullusi-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("mullusi-");
  const pluginRoot = await tempDirs.make("mullusi-plugin-");
  const outsideDir = await tempDirs.make("mullusi-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("mullusi-");
    const pluginRoot = await tempDirs.make("mullusi-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("mullusi-");
    const pluginRoot = await tempDirs.make("mullusi-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
