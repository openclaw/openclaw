...
function resolveNestedSkillsRoot(dir: string): { baseDir: string; note?: string } {
  ...
  for (const name of nestedDirs.slice(0, 25)) {
...
function loadSkillEntries(
  ...
  const loadSkills = (params: { dir: string; source: string }): Skill[] => {
    ...
    // If the root itself is a skill directory, just load it directly.
