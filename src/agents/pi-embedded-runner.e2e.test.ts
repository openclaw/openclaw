...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-embedded-agent-"));
...
const ensureModels = (cfg: SmartAgentNeoConfig) => ensureSmartAgentNeoModelsJson(cfg, agentDir) as unknown;
