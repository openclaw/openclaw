 ...
 * limit.
 */
export async function sessionFileHasContent(
	...
	try {
		...
		try {
			...
			for await (const line of rl) {
				...
				if (
					rec?.type === "message" &&
					(rec.message as Record<string, unknown> | undefined)?.role ===
						"assistant"
				) {
					return true;
				}
...
export function createAcpVisibleTextAccumulator() {
	...
	let visibleText = "";
	const startsWithWordChar = (chunk: string): boolean =>
...
export function runAgentAttempt(params: {
	...
	if (isCliProvider(params.providerOverride, params.cfg)) {
		...
		return runCliWithSession(cliSessionBinding?.sessionId).catch(
			async (err) => {
				if (
