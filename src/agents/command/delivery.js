...
import { createOutboundPayloadPlan, formatOutboundPayloadLog, projectOutboundPayloadPlanForJson, projectOutboundPayloadPlanForOutbound, } from "../../infra/outbound/payloads.js";
...
async function normalizeReplyMediaPathsForDelivery(params) {
    ...
    const agentId = params.outboundSession?.agentId ??
...
export function normalizeAgentCommandReplyPayloads(params) {
    ...
    const channel = params.deliveryChannel && !isInternalMessageChannel(params.deliveryChannel)
    ...
    const transformReplyPayload = deliveryPlugin?.messaging?.transformReplyPayload
        ? (payload) => deliveryPlugin.messaging?.transformReplyPayload?.({
            payload,
            cfg: params.cfg,
            accountId: params.accountId,
        }) ?? payload
...
export async function deliverAgentCommandResult(params) {
    ...
    const deliveryPlugin = deliver && !isInternalMessageChannel(deliveryChannel)
    ...
    const resolvedThreadTarget = replyTransport && Object.hasOwn(replyTransport, "threadId")
    ...
    const mediaNormalizedReplyPayloads = deliver && !isInternalMessageChannel(deliveryChannel)
