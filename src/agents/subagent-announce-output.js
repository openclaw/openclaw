...
import { captureSubagentCompletionReplyUsing, readLatestSubagentOutputWithRetryUsing, } from "./subagent-announce-capture.js";
import { callGateway, loadConfig, loadSessionStore, resolveAgentIdFromSessionKey, resolveStorePath, } from "./subagent-announce.runtime.js";
...
export function withSubagentOutcomeTiming(outcome, timing) {
...
export async function readSubagentOutput(sessionKey, outcome) {
...
export async function waitForSubagentRunOutcome(runId, timeoutMs) {
...
export async function captureSubagentCompletionReply(sessionKey, options) {
...
export function buildChildCompletionFindings(children) {
    ...
    for (const [index, child] of sorted.entries()) {
        ...
        sections.push([`${index + 1}. ${title}`, `status: ${outcome}`, formatUntrustedChildResult(resultText)].join("\n"));
    }
...
export function dedupeLatestChildCompletionRows(children) {
...
export function filterCurrentDirectChildCompletionRows(children, params) {
