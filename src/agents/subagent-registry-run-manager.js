...
import { emitSubagentEndedHookOnce, shouldUpdateRunOutcome, } from "./subagent-registry-completion.js";
...
export function createSubagentRunManager(params) {
    const waitForSubagentCompletion = async (runId, waitTimeoutMs, expectedEntry) => {
        try {
            ...
            const baseOutcome = wait.status === "error"
    ...
    const markSubagentRunTerminated = (markParams) => {
        ...
        for (const runId of runIds) {
            ...
            entry.endedAt = now;
            entry.outcome = withSubagentOutcomeTiming({ status: "error", error: reason }, {
                startedAt: entry.startedAt,
                endedAt: now,
            });
        ...
        if (updated > 0) {
            ...
            for (const entry of entriesByChildSessionKey.values()) {
                ...
                const cfg = params.loadConfig();
                void Promise.resolve(params.ensureRuntimePluginsLoaded({
                    config: cfg,
                    workspaceDir: entry.workspaceDir,
