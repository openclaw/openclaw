...
describe("vercel-ai-gateway provider resolution", () => {
	it("adds the provider with GPT-5.4 models when AI_GATEWAY_API_KEY is present", async () => {
		...
		try {
			...
			expect(
				provider?.models?.some((model) => model.id === "openai/gpt-5.4"),
			).toBe(true);
			expect(
				provider?.models?.some((model) => model.id === "openai/gpt-5.4-pro"),
			).toBe(true);
		} finally {
			envSnapshot.restore();
		}
	...
	it("prefers env keyRef marker over runtime plaintext for persistence", async () => {
		...
		try {
			const providers = await resolveImplicitProvidersForTest({ agentDir });
			expect(providers?.["vercel-ai-gateway"]?.apiKey).toBe(
				"AI_GATEWAY_API_KEY",
			);
		} finally {
			envSnapshot.restore();
