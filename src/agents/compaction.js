...
const generateSummaryCompat = piGenerateSummary;
...
export function splitMessagesByTokenShare(messages, parts = DEFAULT_PARTS) {
    ...
    const splitCurrentAtPendingBoundary = () => {
        if (pendingChunkStartIndex === null ||
...
function generateSummary(currentMessages, model, reserveTokens, apiKey, headers, signal, customInstructions, previousSummary) {
    if (piGenerateSummary.length >= 8) {
        return generateSummaryCompat(currentMessages, model, reserveTokens, apiKey, headers, signal, customInstructions, previousSummary);
    }
