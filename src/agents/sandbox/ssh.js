...
function normalizeInlineSshMaterial(contents, filename) {
    ...
    const normalizedEscapedNewlines = normalizedNewlines
        .replace(/\\r\\n/g, "\\n")
        .replace(/\\r/g, "\\n");
    const expanded = filename === "identity" || filename === "certificate.pub"
...
function buildSshFailureMessage(stderr, exitCode) {
    ...
    if (trimmed.includes("error in libcrypto") &&
...
export function buildRemoteCommand(argv) {
...
export function buildExecRemoteCommand(params) {
    const body = params.workdir
        ...
        : params.command;
    const argv = Object.keys(params.env).length > 0
        ? [
            "env",
...
export async function createSshSandboxSessionFromSettings(settings) {
...
export async function disposeSshSandboxSession(session) {
    await fs.rm(path.dirname(session.configPath), { recursive: true, force: true });
}
export async function runSshSandboxCommand(params) {
