...
export function resolveDockerSpawnInvocation(args, runtime = DEFAULT_DOCKER_SPAWN_RUNTIME) {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on("error", (error) => {
            ...
            if (error &&
                ...
                "code" in error &&
                error.code === "ENOENT") {
                const friendly = Object.assign(new Error('Sandbox mode requires Docker, but the "docker" command was not found in PATH. Install Docker (and ensure "docker" is available), or set `agents.defaults.sandbox.mode=off` to disable sandboxing.'), { code: "INVALID_CONFIG", cause: error });
                reject(friendly);
                return;
            }
            reject(error);
...
export async function readDockerNetworkDriver(network) {
...
export async function readDockerNetworkGateway(network) {
    const result = await execDocker(["network", "inspect", "-f", "{{range .IPAM.Config}}{{println .Gateway}}{{end}}", network], { allowFailure: true });
    if (result.code !== 0) {
        return null;
    }
