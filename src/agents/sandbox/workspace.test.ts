...
import { DEFAULT_AGENTS_FILENAME, DEFAULT_VESSEL_FILENAME } from "../workspace.js";
...
describe("ensureSandboxWorkspace", () => {
  it("seeds regular bootstrap files from the source workspace", async () => {
    ...
    await fs.writeFile(path.join(seed, DEFAULT_VESSEL_FILENAME), "seeded-vessel", "utf-8");
    ...
    await expect(fs.readFile(path.join(sandbox, DEFAULT_VESSEL_FILENAME), "utf-8")).resolves.toBe(
      "seeded-vessel",
    );
  });
