...
export function resolveSandboxEnvironmentSkillExclusions(sandbox: SandboxContext): string[] {
  ...
  const delivered = inputs.skillUsagePaths;
  const roots = [
    ...new Set([
...
export async function mergeSandboxEnvironmentSkillCatalog(params: {
  assertCurrent?: () => void;
  skillsPrompt: string;
  candidates: Skill[];
  ...
  warn: (message: string) => void;
}): Promise<{ skillsPrompt: string; candidates: Skill[] }> {
  ...
  const environment = await buildSkillSnapshot(params.workspaceDir, {
