...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "smart-agent-neo.sandbox=1");
  args.push("--label", `smart-agent-neo.sessionKey=${params.scopeKey}`);
  args.push("--label", `smart-agent-neo.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `smart-agent-neo.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "smart-agent-neo.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`smart-agent-neo sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`smart-agent-neo sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("smart-agent-neo sandbox recreate --all");
}
