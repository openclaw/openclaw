...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "activi.sandbox=1");
  args.push("--label", `activi.sessionKey=${params.scopeKey}`);
  args.push("--label", `activi.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `activi.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "activi.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`activi sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`activi sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("activi sandbox recreate --all");
}
