...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mullusi.sandbox=1");
  args.push("--label", `mullusi.sessionKey=${params.scopeKey}`);
  args.push("--label", `mullusi.createdAtMs=${createdAtMs}`);
  args.push("--label", `mullusi.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `mullusi.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markMullusiExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "mullusi.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mullusi sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mullusi sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mullusi sandbox recreate --all");
}
