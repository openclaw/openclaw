      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `evox sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `evox sandbox recreate --browser --agent ${agentId}`;
          }
          return "evox sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
