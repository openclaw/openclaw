...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_SMART_AGENT_NEO_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_SMART_AGENT_NEO_BROWSER_PROFILE_NAME,
    profiles: {
      [DEFAULT_SMART_AGENT_NEO_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "smart-agent-neo.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `smart-agent-neo sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `smart-agent-neo sandbox recreate --browser --agent ${agentId}`;
          }
          return "smart-agent-neo sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "smart-agent-neo.sandboxBrowser": "1" },
    ...
    args.push("-e", `SMART_AGENT_NEO_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `SMART_AGENT_NEO_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `SMART_AGENT_NEO_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `SMART_AGENT_NEO_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `SMART_AGENT_NEO_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_SMART_AGENT_NEO_BROWSER_PROFILE_NAME)
    : null;
