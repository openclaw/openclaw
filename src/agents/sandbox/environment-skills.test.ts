...
describe("environment skill policy and catalogs", () => {
  ...
  it("uses existing invocation/config/agent policy and executor requirements, never Gateway facts", async () => {
    ...
    const merged = await mergeSandboxEnvironmentSkillCatalog({
      skillsPrompt: "",
      candidates: [],
  ...
  it("keeps native catalog priority, honors session overlay, and bounds the merged prompt", async () => {
    ...
    const native = await buildSkillSnapshot(root, { entries: [entries[0]!] });
    const merged = await mergeSandboxEnvironmentSkillCatalog({
  ...
  it("keeps context compaction after merging environment skills without dropping resources", async () => {
    ...
    const snapshot = await buildSkillSnapshot(root, { entries: native });
    const merged = await mergeSandboxEnvironmentSkillCatalog({
  ...
  it("isolates malformed and oversized skills and exposes valid remote paths to the existing reader", async () => {
    ...
    const snapshot = await buildSkillSnapshot(root, { entries });
