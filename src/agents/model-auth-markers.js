...
export function listKnownNonSecretApiKeyMarkers() {
    knownNonSecretApiKeyMarkersCache ??= [
        ...new Set([
            ...CORE_NON_SECRET_API_KEY_MARKERS,
            ...loadPluginManifestRegistry({ cache: true }).plugins.flatMap((plugin) => plugin.origin === "bundled" ? (plugin.nonSecretAuthMarkers ?? []) : []),
        ]),
    ];
...
export function resolveOAuthApiKeyMarker(providerId) {
    return `${OAUTH_API_KEY_MARKER_PREFIX}${providerId.trim()}`;
}
export function isOAuthApiKeyMarker(value) {
...
export function isNonSecretApiKeyMarker(value, opts) {
    ...
    const isKnownMarker = isOAuthApiKeyMarker(trimmed) ||
