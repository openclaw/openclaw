...
import { resolveProviderTransportTurnStateWithPlugin, resolveProviderWebSocketSessionPolicyWithPlugin, } from "../plugins/provider-runtime.js";
import { encodeAssistantTextSignature, normalizeAssistantPhase, } from "../shared/chat-message-content.js";
...
import { buildAssistantMessageFromResponse, convertMessagesToInputItems, convertResponseToInputItems, convertTools, } from "./openai-ws-message-conversion.js";
import { buildOpenAIWebSocketResponseCreatePayload, planOpenAIWebSocketRequestPayload, } from "./openai-ws-request.js";
...
function resolveOpenAIWebSocketStrictToolSetting(model) {
    return resolveOpenAIStrictToolSetting(model, {
        transport: "websocket",
        supportsStrictMode: model.compat && typeof model.compat === "object"
    ...
    await new Promise((resolve, reject) => {
        ...
        const unsubscribe = params.manager.onMessage((event) => {
            if (event.type === "response.completed") {
                ...
                cleanup();
                reject(new Error(`warm-up failed: ${formatOpenAIWebSocketResponseFailure(event.response)}`));
            }
            else if (event.type === "error") {
                cleanup();
        ...
        const run = async () => {
            ...
            while (true) {
                ...
                if (transport !== "websocket" && isWsSessionDegraded(session)) {
                    log.debug(`[ws-stream] session=${sessionId} in websocket cool-down; using HTTP fallback until ${new Date(session.degradedUntil).toISOString()}`);
                    return fallbackToHttp(model, context, options, apiKey, eventStream, opts.signal, {
                ...
                fullPayload = mergeTransportMetadata((nextPayload ?? fullPayload), turnState?.metadata);
                ...
                if (plannedPayload.mode === "incremental") {
                    log.debug(`[ws-stream] session=${sessionId}: incremental send (${plannedInputItems.length} items) previous_response_id=${plannedPayload.payload.previous_response_id}`);
                }
                else {
                    log.debug(`[ws-stream] session=${sessionId}: full context send (${plannedInputItems.length} items)`);
                }
                ...
                try {
                    ...
                    if (transport !== "websocket" &&
                        ...
                        runtimeRetries++;
                        log.warn(`[ws-stream] retrying websocket turn after send failure for session=${sessionId} (${runtimeRetries}/${MAX_AUTO_WS_RUNTIME_RETRIES}). error=${normalizedErr.message}`);
                        resetWsSession({
                            session,
                            createManager: () => createWsManager(opts.managerOptions, sessionHeaders),
                        });
                    ...
                    if (transport !== "websocket") {
                        log.warn(`[ws-stream] send failed for session=${sessionId}; falling back to HTTP. error=${normalizedErr.message}`);
                        markWsSessionDegraded(session);
                        resetWsSession({
                            session,
                            createManager: () => createWsManager(opts.managerOptions, sessionHeaders),
                ...
                const outputItemPhaseById = new Map();
                const outputTextByPart = new Map();
                ...
                try {
                        ...
                        const unsubscribe = session.manager.onMessage((event) => {
                            ...
                            if (event.type === "response.output_item.added" ||
                                ...
                                if (typeof event.item.id === "string") {
                                    const itemPhase = event.item.type === "message"
                    ...
                    if (transport !== "websocket" &&
                        ...
                        runtimeRetries < MAX_AUTO_WS_RUNTIME_RETRIES) {
                        runtimeRetries++;
                        log.warn(`[ws-stream] retrying websocket turn after retryable runtime failure for session=${sessionId} (${runtimeRetries}/${MAX_AUTO_WS_RUNTIME_RETRIES}). error=${normalizedErr.message}`);
                        resetWsSession({
                            session,
                            createManager: () => createWsManager(opts.managerOptions, sessionHeaders),
                        });
                    ...
                    if (transport !== "websocket" && !signal?.aborted && !sawWsOutput) {
                        log.warn(`[ws-stream] session=${sessionId} runtime failure before output; falling back to HTTP. error=${normalizedErr.message}`);
                        markWsSessionDegraded(session);
                        resetWsSession({
                            session,
                            createManager: () => createWsManager(opts.managerOptions, sessionHeaders),
...
async function fallbackToHttp(model, context, streamOptions, apiKey, eventStream, signal, fallbackOptions) {
        ...
        ...(fallbackOptions?.turnState?.metadata
            ? {
                onPayload: async (payload, payloadModel) => {
                    ...
                    const resolvedPayload = (nextPayload ?? payload);
    ...
    const httpStreamFn = openAIWsStreamDeps.createHttpFallbackStreamFn(model) ??
...
export const __testing = {
    ...
    setWsDegradeCooldownMsForTest(nextMs) {
