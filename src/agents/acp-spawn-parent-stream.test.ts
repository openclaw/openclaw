...
vi.mock("../infra/heartbeat-wake.js", async (importOriginal) => {
	return await mergeMockedModule(
		...
		() => ({
			requestHeartbeatNow: (...args: unknown[]) =>
...
vi.mock("../acp/runtime/session-meta.js", async (importOriginal) => {
	return await mergeMockedModule(
		...
		() => ({
			readAcpSessionEntry: (...args: unknown[]) =>
				readAcpSessionEntryMock(...args),
		}),
	);
});
...
vi.mock("../config/sessions/paths.js", async (importOriginal) => {
	return await mergeMockedModule(
		...
		() => ({
			resolveSessionFilePath: (...args: unknown[]) =>
				resolveSessionFilePathMock(...args),
...
describe("startAcpSpawnParentStreamRelay", () => {
	beforeEach(async () => {
		...
		resolveSessionFilePathOptionsMock.mockImplementation(
			(value: unknown) => value,
		);
		({
			emitAgentEvent,
	...
	it("relays assistant progress and completion to the parent session", () => {
		...
		expect(texts.some((text) => text.includes("Started codex session"))).toBe(
			true,
		);
		expect(texts.some((text) => text.includes("codex: hello from child"))).toBe(
			true,
		);
		expect(
			texts.some((text) => text.includes("codex run completed in 2s")),
		).toBe(true);
	...
	it("emits a no-output notice and a resumed notice when output returns", () => {
		...
		expect(
			collectedTexts().some((text) =>
				text.includes("has produced no output for 1s"),
			),
		).toBe(true);
		...
		expect(texts.some((text) => text.includes("codex: resumed output"))).toBe(
			true,
		);
		...
		expect(
			collectedTexts().some((text) => text.includes("run failed: boom")),
		).toBe(true);
		relay.dispose();
	});
	...
	it("auto-disposes stale relays after max lifetime timeout", () => {
		...
		expect(
			collectedTexts().some((text) =>
				text.includes("stream relay timed out after 1s"),
			),
		).toBe(true);
	...
	it("supports delayed start notices", () => {
		...
		expect(
			collectedTexts().some((text) => text.includes("Started codex session")),
		).toBe(false);
	...
	it("preserves delta whitespace boundaries in progress relays", () => {
		...
		const texts = collectedTexts();
		expect(texts.some((text) => text.includes("codex: hello world"))).toBe(
			true,
		);
		relay.dispose();
	});
	...
	it("resolves ACP spawn stream log path from session metadata", () => {
		...
		expect(resolved).toBe(
			"/tmp/openclaw/agents/codex/sessions/sess-123.acp-stream.jsonl",
		);
