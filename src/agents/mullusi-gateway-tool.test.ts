...
async function loadFreshMullusiToolsModuleForTest() {
  vi.resetModules();
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshMullusiToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-test-"));

    try {
      await withEnvAsync(
        { MULLUSI_STATE_DIR: stateDir, MULLUSI_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: mullusi --profile isolated doctor --non-interactive",
          );
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/mullusi" } } }',
      }),
    ).rejects.toThrow(
