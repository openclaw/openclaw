...
const log = createSubsystemLogger("zenmux-models");
...
export async function discoverZenmuxModels(): Promise<ModelDefinitionConfig[]> {
  ...
  try {
    ...
    if (!response.ok) {
      log.warn(`Failed to discover ZenMux models: HTTP ${response.status}, using static catalog`);
      return staticZenmuxModelDefinitions();
    }
    const data = (await response.json()) as ZenmuxModelsResponse;
    if (!data.data || data.data.length === 0) {
      log.warn("No ZenMux models found, using static catalog");
      return staticZenmuxModelDefinitions();
    }
