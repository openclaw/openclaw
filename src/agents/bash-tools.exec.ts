...
function buildExecForegroundResult(params: {
	...
	return textResult(
		`${warningText}${params.outcome.aggregated || "(no output)"}`,
		{
			status: "completed",
...
export function createExecTool(
	...
	return {
		...
		execute: async (_toolCallId, args, signal, onUpdate) => {
			...
			const configuredAsk =
				defaults?.ask ?? loadExecApprovals().defaults?.ask ?? "on-miss";
			...
			if (
				...
				if (invalidKeys.length > 0) {
					details.push(
						`invalid non-portable override keys: ${invalidKeys.join(", ")}`,
					);
				}
