...
export function describeExecTool(params?: { agentId?: string; hasCronTool?: boolean }): string {
  const base = [
    ...
    "For long-running work started now, rely on automatic completion wake when the command emits output or fails; use process to confirm quiet successful completions and when you need logs, status, input, or intervention.",
    params?.hasCronTool
