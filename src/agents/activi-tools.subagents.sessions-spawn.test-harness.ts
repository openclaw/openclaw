...
type CreateActiviTools = (typeof import("./activi-tools.js"))["createActiviTools"];
export type CreateActiviToolsOpts = Parameters<CreateActiviTools>[0];
...
export async function getSessionsSpawnTool(opts: CreateActiviToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createActiviTools } = await import("./activi-tools.js");
  const tool = createActiviTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
