...
function limitedOutput(
  buffer: Buffer,
  maxOutputBytes: number | { stdout?: number; stderr?: number } | undefined,
  stream: "stdout" | "stderr",
) {
  const limit = typeof maxOutputBytes === "number" ? maxOutputBytes : maxOutputBytes?.[stream];
  if (limit === undefined || buffer.length <= limit) {
    return { buffer, truncatedBytes: 0 };
  }
  return {
    buffer: buffer.subarray(buffer.length - limit),
    truncatedBytes: buffer.length - limit,
  };
}
...
async function runSandboxGit(params: {
  ...
  signal?: AbortSignal;
  beforeRun?: () => void;
}): Promise<{ result: GitResult; stdout: Buffer; stderr: Buffer }> {
  ...
  const stdout = limitedOutput(command.stdout, params.maxOutputBytes, "stdout");
  const stderr = limitedOutput(command.stderr, params.maxOutputBytes, "stderr");
  return {
    result: {
...
export async function createWorktreeGitExecutor(params: {
  ...
  try {
    ...
    const admitSnapshotIndex = async (requestedEnv: NodeJS.ProcessEnv | undefined) => {
      ...
      if (!snapshotDir) {
        if (admittedSnapshotDir) {
          admittedSnapshotDir = undefined;
          backend = await createBackend(currentMounts());
        }
        return;
      }
    ...
    return {
      async run(cwd, args, options) {
        ...
        if (args[0] === "worktree" && args[1] === "remove") {
          ...
          const forceRequested = args.some((arg) => arg === "--force" || arg === "-f");
          if (!forceRequested) {
            const status = await runSandboxGit({
              backend,
              configuredEnv: cfg.docker.env,
              cwd: destination,
            ...
            if (status.result.code !== 0 || status.result.termination !== "exit") {
              return status.result;
            }
            if (status.result.stdout.length > 0) {
              return {
                stdout: "",
                stderr: `fatal: '${destination}' contains modified or untracked files, use --force to delete it\n`,
                code: 1,
                signal: null,
                killed: false,
                termination: "exit",
      ...
      async runBuffered(cwd, args, options) {
        await admitSnapshotIndex(options?.env);
        const executed = await runSandboxGit({
          backend,
          configuredEnv: cfg.docker.env,
        ...
        const termination =
          executed.result.termination === "no-output-timeout"
            ? "timeout"
            : executed.result.termination;
        return {
          stdout: executed.stdout,
          stderr: executed.stderr,
          code: executed.result.code,
          signal: executed.result.signal,
          killed: executed.result.killed,
          termination,
        };
      },
