...
describe("managed worktree sandbox Git executor", () => {
  ...
  it("preserves a dirty worktree for non-force removal and rechecks authority", async () => {
    const root = await createRepositoryFixture();
    const config = sandboxConfig(root);
    const commonDir = path.join(root, ".git");
    const fingerprint = hashTextSha256(`${commonDir}\n`).slice(0, 16);
    const destination = path.join(config.worktreeRoot!, fingerprint, "dirty");
    await fs.mkdir(destination, { recursive: true });
    await fs.writeFile(path.join(destination, "late-untracked.txt"), "keep me");
    const beforeRun = vi.fn();
    const restore = registerSandboxBackend("docker", async () => ({
      id: "docker",
      runtimeId: "dirty-removal-test",
      runtimeLabel: "dirty-removal-test",
      workdir: "/workspace",
      buildExecSpec: vi.fn(),
      async runShellCommand(command) {
        if (command.args?.includes("rev-parse")) {
        ...
        if (command.args?.includes("status")) {
          return {
            stdout: Buffer.from("?? late-untracked.txt\n"),
            stderr: Buffer.alloc(0),
            code: 0,
          };
        }
    ...
    const executor = await createWorktreeGitExecutor({
      isolation: { config, sessionKey: "agent:main:subagent:test" },
      repoRoot: root,
      allocationRoot: destination,
      env: { OPENCLAW_STATE_DIR: path.join(root, "state") },
    });

    const result = await executor?.run(root, ["worktree", "remove", "--", destination], {
      beforeRun,
    });

    expect(result?.code).toBe(1);
    expect(result?.stderr).toContain("contains modified or untracked files");
    await expect(fs.readFile(path.join(destination, "late-untracked.txt"), "utf8")).resolves.toBe(
      "keep me",
    );
  ...
  it("retries failed runtime revocation before declaring the pool disposed", async () => {
    let disposalAttempts = 0;
    const pool = createProvisioningRuntimePool({
      backendId: "docker",
      async create() {
        return {
          id: "docker",
          runtimeId: "retry-revocation",
          runtimeLabel: "retry-revocation",
          workdir: "/workspace",
          buildExecSpec: vi.fn(),
    ...
    await pool.create([]);
    await expect(pool.dispose()).rejects.toThrow(
  ...
  it("keeps failed recovery fenced until the exact registered runtime is revoked", async () => {
    const root = await createRepositoryFixture();
    const stateDir = path.join(root, "state");
    vi.stubEnv("OPENCLAW_STATE_DIR", stateDir);
    const entry = {
      containerName: "stranded-provisioning-runtime",
      backendId: "test-recovery",
      backendTarget: { key: "podman:builder", globalArgs: ["--connection", "builder"] },
      runtimeLabel: "stranded-provisioning-runtime",
      sessionKey: `${PROVISIONING_SCOPE_PREFIX}recovery-test`,
      createdAtMs: 1,
      lastUsedAtMs: 1,
      image: "openclaw-sandbox:test",
      configLabelKind: "Image",
    };
    insertSandboxRegistryEntryIfMissing(entry);
    let attempts = 0;
    const restore = registerSandboxBackend("test-recovery", {
      async factory() {
      ...
      manager: {
        ...
        async removeRuntime(params) {
          attempts += 1;
          expect(params.entry).toEqual(entry);
          if (attempts === 1) {
            throw new Error("runtime still owns writable mounts");
          }
        },
      },
    });
    ...
    await expect(revokeStaleProvisioningRuntimes(sandboxConfig(root))).rejects.toThrow(
      "remains fenced",
    );
    await expect(readRegistryEntry(entry.containerName)).resolves.toEqual(entry);
    await expect(revokeStaleProvisioningRuntimes(sandboxConfig(root))).resolves.toBeUndefined();
