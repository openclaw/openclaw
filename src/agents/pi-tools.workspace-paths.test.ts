...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("EasyHub-ws-", async (workspaceDir) => {
      await withTempDir("EasyHub-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createEasyHubCodingTools({ workspaceDir });
    ...
    await withTempDir("EasyHub-ws-", async (workspaceDir) => {
      await withTempDir("EasyHub-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello EasyHub");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("EasyHub-ws-", async (workspaceDir) => {
      const tools = createEasyHubCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("EasyHub-ws-", async (workspaceDir) => {
      await withTempDir("EasyHub-override-", async (overrideDir) => {
        const tools = createEasyHubCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("EasyHub-sandbox-", async (sandboxDir) => {
      await withTempDir("EasyHub-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "EasyHub-sbx-test",
        ...
        const tools = createEasyHubCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
