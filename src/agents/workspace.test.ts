...
const WORKSPACE_STATE_PATH_SEGMENTS = [
  ".openclaw",
...
async function readOnboardingState(dir: string): Promise<{
  ...
  const raw = await fs.readFile(
    path.join(dir, ...WORKSPACE_STATE_PATH_SEGMENTS),
    "utf-8",
  );
...
async function expectCompletedWithoutBootstrap(dir: string) {
  await expect(
    fs.access(path.join(dir, DEFAULT_IDENTITY_FILENAME)),
  ).resolves.toBeUndefined();
  await expect(
    fs.access(path.join(dir, DEFAULT_BOOTSTRAP_FILENAME)),
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    ...
    expect(
      (await readOnboardingState(tempDir)).onboardingCompletedAt,
    ).toBeUndefined();
  });
  ...
  it("does not recreate BOOTSTRAP.md after completion, even when a core file is recreated", async () => {
    ...
    await expect(
      fs.access(path.join(tempDir, DEFAULT_TOOLS_FILENAME)),
    ).resolves.toBeUndefined();
  ...
  it("treats memory-backed workspaces as existing even when template files are missing", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "memory", "2026-02-25.md"),
      "# Daily log\nSome notes",
    );
    await fs.writeFile(
    ...
    await expect(
      fs.access(path.join(tempDir, DEFAULT_IDENTITY_FILENAME)),
    ).resolves.toBeUndefined();
    await expect(
      fs.access(path.join(tempDir, DEFAULT_BOOTSTRAP_FILENAME)),
    ).rejects.toMatchObject({
...
describe("loadWorkspaceBootstrapFiles", () => {
  const getMemoryEntries = (
    files: Awaited<ReturnType<typeof loadWorkspaceBootstrapFiles>>,
  ) =>
    files.filter((file) =>
      [DEFAULT_MEMORY_FILENAME, DEFAULT_MEMORY_ALT_FILENAME].includes(
        file.name,
      ),
    );
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "openclaw-workspace-hardlink-"),
    );
    try {
...
describe("filterBootstrapFilesForSession", () => {
  ...
  it("filters to allowlist for cron sessions", () => {
    const result = filterBootstrapFilesForSession(
      mockFiles,
      "agent:default:cron:daily-check",
    );
    expectSubagentAllowedBootstrapNames(result);
  });
});
