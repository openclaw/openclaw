...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MULLUSI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MULLUSI_HOME: "/srv/mullusi-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/mullusi-home"), ".mullusi", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".mullusi", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("mullusi-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("mullusi-workspace-");
    await fs.mkdir(path.join(tempDir, ".mullusi"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-workspace-hardlink-"));
    try {
