...
let createMullusiTools: typeof import("./mullusi-tools.js").createMullusiTools;
...
function getNodesTool(options?: { modelHasVision?: boolean; allowMediaInvokeCommands?: boolean }) {
  ...
  const tool = createMullusiTools(toolOptions).find((candidate) => candidate.name === "nodes");
  if (!tool) {
...
describe("nodes camera_snap", () => {
  ...
  it("omits inline base64 image blocks when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/mullusi-camera-snap-front-.*\.jpg$/);
  });
...
describe("nodes photos_latest", () => {
  ...
  it("returns MEDIA paths and no inline images when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/mullusi-camera-snap-.*\.jpg$/);
  });
...
async function loadMullusiToolsForTest(): Promise<void> {
  vi.resetModules();
  await import("./test-helpers/fast-core-tools.js");
  ({ createMullusiTools } = await import("./mullusi-tools.js"));
}
