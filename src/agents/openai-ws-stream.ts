...
type OpenAIWsStreamDeps = {
	createManager: (
		options?: OpenAIWebSocketManagerOptions,
	) => OpenAIWebSocketManager;
...
export function createOpenAIWebSocketStreamFn(
	...
	return (model, context, options) => {
		...
		const run = async () => {
			...
			if (resolveWsWarmup(options) && !session.warmUpAttempted) {
				...
				if (warmupFailed && !session.manager.isConnected()) {
					...
					try {
						...
						session.everConnected = true;
						log.debug(
							`[ws-stream] reconnected after warm-up failure for session=${sessionId}`,
						);
					} catch (reconnectErr) {
						session.broken = true;
						wsRegistry.delete(sessionId);
			...
			const supportsStore = (model as { compat?: { supportsStore?: boolean } })
