...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    try {
      ...
      // Sanitize reasoning_content (and related fields) in outbound payloads.
      // Models like Qwen 3.5 return reasoning_content containing control
      // characters that cause strict JSON parsers (oMLX / FastAPI / Pydantic)
      // to reject the request with a 422 parse error on subsequent turns.
      activeSession.agent.streamFn = createReasoningContentSanitizer(activeSession.agent.streamFn);
