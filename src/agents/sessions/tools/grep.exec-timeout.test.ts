...
// Grep tool exec-timeout escalation tests run a real managed-bin stub that
...
describePosix("grep tool exec-timeout escalation", () => {
  afterEach(() => {
  ...
  it("rejects at the deadline, reaps a SIGTERM-ignoring ripgrep via escalation, and releases its pipes", async () => {
    ...
    const tool = createGrepToolDefinition(dir, { execTimeoutMs: 100 });
    ...
    await expect(result).rejects.toThrow(/ripgrep timed out/);
