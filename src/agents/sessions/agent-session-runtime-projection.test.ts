...
describe("AgentSession login recovery", () => {
  it.each(["refresh", "access", "cancel", "choice", "test-provider"])(
    "offers sign-in when the %s provider supplies no OAuth request key for a branch summary",
    async (provider) => {
      ...
      const refreshToken = vi.fn(async () => {
        throw new Error("Unexpected refresh of current credentials");
      });
      ...
      modelRegistry.registerProvider(provider, {
        ...
        oauth: {
          name: provider,
          async login() {
            throw new Error("Unexpected login during branch summary");
          },
          refreshToken,
          getApiKey,
        },
      ...
      const targetId = sessionManager.appendMessage(
        createAssistant(model, [{ type: "text", text: "target branch" }]),
      );
      ...
      try {
        ...
        await expect(session.navigateTree(targetId, { summarize: true })).rejects.toThrow(
          provider === "test-provider"
            ? "Run '/login test-provider' to re-authenticate."
  ...
  it("keeps missing-auth prompt guidance and session state without starting a model", async () => {
    const model = { ...testModel, provider: "missing-auth-fixture" };
    ...
    try {
      ...
      await expect(session.prompt("Continue this conversation.")).rejects.toThrow(
        "No API key found for missing-auth-fixture.\n\nUse /login to log into a provider via OAuth or API key.",
      );
      expect(streamMocks.streamSimple).not.toHaveBeenCalled();
      expect(sessionManager.getEntries()).toEqual(before);
      expect(sessionManager.getLeafId()).toBe(beforeLeafId);
