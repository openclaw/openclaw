import { diagnosticErrorCategory, diagnosticHttpStatusCode, } from "../infra/diagnostic-error-metadata.js";
        ...
        if (hookResult?.requireApproval) {
            ...
            try {
                ...
                if (hasImmediateDecision) {
                    ...
                    const waitPromise = callGatewayTool("plugin.approval.waitDecision", 
                    // Buffer beyond the approval timeout so the gateway can clean up
                    // and respond before the client-side RPC timeout fires.
                    { timeoutMs: (approval.timeoutMs ?? 120_000) + 10_000 }, { id });
                    let waitResult;
                    if (args.signal) {
                        let onAbort;
                ...
                const resolution = decision === PluginApprovalResolutions.ALLOW_ONCE ||
...
export function consumeAdjustedParamsForToolCall(toolCallId, runId) {
