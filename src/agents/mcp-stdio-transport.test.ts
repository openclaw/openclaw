...
describe("OpenClawStdioClientTransport", () => {
  ...
  it("keeps failed owner cleanup uncertain through repeated disposal", async () => {
    ...
    await withAgentCleanupOutcome(async () => {
      await expect(
        disposeMcpClient({
          transport,
          transportType: "stdio",
    ...
    expect(receipt).toHaveBeenCalledWith("uncertain");
  });
  ...
  it.each([
    ...
    async ({ confirmed, outcome }) => {
      ...
      await transport.start();
      const receipt = vi.fn();
      const disposal = withAgentCleanupOutcome(
        () =>
          disposeMcpClient(
            {
              transport,
      ...
      expect(receipt).toHaveBeenCalledWith(outcome);
    },
  );
