...
} from "./pi-embedded-block-chunker.js";
...
function buildBtwQuestionPrompt(
	question: string,
...
export async function runBtwSideQuestion(
	...
	for await (const event of stream) {
		finalEvent =
			event.type === "done" || event.type === "error" ? event : finalEvent;

		if (
			!assistantStarted &&
			(event.type === "text_start" || event.type === "start")
		) {
	...
	const finalMessage =
		finalEvent?.type === "done" ? finalEvent.message : undefined;
	if (finalMessage) {
		if (!sawTextEvent) {
