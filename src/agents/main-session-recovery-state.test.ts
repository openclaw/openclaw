...
describe("main session recovery state", () => {
  ...
  it("rejects an attempt beyond the automatic recovery budget", () => {
    const entry = interruptedEntry({
      mainRestartRecovery: recoveryState({
        revision: 7,
        chargedAttempts: 3,
    ...
    expect(
      transitionMainSessionRecovery(entry, {
        ...
        lifecycleGeneration: "generation-1",
        now: 200,
        observation: { sessionId: "session-1", cycleId: "cycle-1", revision: 7 },
        runId: "recovery-4",
      }),
    ).toEqual({ kind: "rejected", reason: "attempt_budget_exhausted" });
    expect(entry.mainRestartRecovery).toEqual(
      recoveryState({
        revision: 7,
        chargedAttempts: 3,
  ...
  it("does not let a delayed interruption revive a tombstoned session", () => {
    const entry = interruptedEntry({
      abortedLastRun: false,
      status: "failed",
      mainRestartRecovery: recoveryState({
