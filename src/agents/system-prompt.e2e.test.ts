...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits extended sections in minimal prompt mode", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      docsPath: "/tmp/smart-agent-neo/docs",
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## SmartAgentNeo CLI Quick Reference");
    expect(prompt).toContain("smart-agent-neo gateway restart");
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("SmartAgentNeo docs: /tmp/smart-agent-neo/docs");
    expect(prompt).toContain(
  ...
  // See: https://github.com/neobot/neobot/commit/66eec295b894bce8333886cfbca3b960c57c4946
  // Agents should use session_status or message timestamps to determine the date/time.
  // Related: https://github.com/neobot/neobot/issues/1897
  //          https://github.com/neobot/neobot/issues/3658
  it("does NOT include a date or time in the system prompt (cache stability)", () => {
    ...
    // https://github.com/neobot/neobot/issues/3658 for the preferred approach:
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## SmartAgentNeo Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/smart-agent-neo. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
