...
/** Check whether a message carries non-boilerplate content (text or media). */
function hasMeaningfulText(message: AgentMessage): boolean {
  const content = (message as { content?: unknown }).content;
  // Image-only or media-only messages are real conversation even without text.
  if (Array.isArray(content)) {
    for (const block of content) {
      if (!block || typeof block !== "object") continue;
      const type = (block as { type?: string }).type;
      if (type === "image" || type === "image_url" || type === "file" || type === "document") {
        return true;
      }
    }
  }
