...
const missedModelWarningSessions = new WeakSet();
...
export default function compactionSafeguardExtension(api) {
    api.on("session_before_compact", async (event, ctx) => {
        ...
        try {
            ...
            log.warn(`Compaction summarization failed; cancelling compaction to preserve history: ${error instanceof Error ? error.message : String(error)}`);
            return { cancel: true };
        }
    });
}
