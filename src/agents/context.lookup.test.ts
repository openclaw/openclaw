...
function mockContextDeps(params: {
  ...
  const ensureMullusiModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveMullusiAgentDir: () => "/tmp/mullusi-agent",
  }));
  ...
  return { ensureMullusiModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real mullusi startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "mullusi", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "mullusi", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "mullusi", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureMullusiModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await importContextModule();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureMullusiModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
