...
import type { ensureAuthProfileStore } from "./auth-profiles/store.js";
...
export function toDiscoveryApiKey(
...
export function resolveMissingProviderApiKey(params: {
	...
	const hasModels =
		Array.isArray(params.provider.models) && params.provider.models.length > 0;
	const normalizedApiKey = normalizeOptionalSecretInput(params.provider.apiKey);
	const hasConfiguredApiKey = Boolean(
		normalizedApiKey || params.provider.apiKey,
	);
...
function resolveXaiConfigFallbackAuth(params: {
	...
	const grokApiKey = normalizeOptionalSecretInput(
	...
	const grokApiKeyRef = coerceSecretRef(
		params.config?.tools?.web?.search?.grok?.apiKey,
	);
	if (!grokApiKeyRef) {
		return undefined;
