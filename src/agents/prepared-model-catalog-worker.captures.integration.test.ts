...
describe("Gateway catalog worker captures", () => {
  ...
  it("reuses one Gateway catalog worker and source graph across agent publications", async () => {
    ...
    try {
      ...
      const nativeArtifacts = [...captures].map((file) =>
        path.join(path.dirname(file), "payload.bin"),
      );
      ...
      expect(initialFootprint.references).toEqual([
        expect.objectContaining({ bytes: 1024 * 1024 }),
        expect.objectContaining({ bytes: 1024 * 1024 }),
      ]);
      expect(initialFootprint.bytes).toBeGreaterThanOrEqual(2 * 1024 * 1024);
