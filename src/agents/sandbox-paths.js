...
import { assertNoWindowsNetworkPath, hasEncodedFileUrlSeparator, safeFileURLToPath, } from "../infra/local-file-access.js";
...
export async function resolveAllowedManagedMediaPath(candidate) {
    const expanded = expandPath(candidate);
    ...
    if (/^file:\/\//i.test(candidate)) {
        ...
        if (workspaceMappedFromUrl) {
            ...
            try {
                ...
                throw new Error(`Invalid file:// URL for sandboxed media: ${err.message}`, {
                    cause: err,
                });
            }
        }
...
function mapContainerWorkspaceFileUrl(params) {
    ...
    if (normalizedPathname !== SANDBOX_CONTAINER_WORKDIR &&
