...
const GOOGLE_LIVE_SINGLE_KEY = "MULLUSI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MULLUSI_LIVE_ANTHROPIC_KEY",
    listVar: "MULLUSI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MULLUSI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MULLUSI_LIVE_${base}_KEY`;
