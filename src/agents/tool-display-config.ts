...
export const TOOL_DISPLAY_CONFIG: ToolDisplayConfig = {
  ...
  tools: {
    ...
    enter_plan_mode: {
      ...
      detailKeys: ["sessionKey", "runtimeMode"],
    },
    exit_plan_mode: {
      emoji: "✅",
    ...
    todo_write: {
      emoji: "📝",
      title: "Todo Write",
      detailKeys: ["planState.todos.0.text", "planState.content"],
    },
    task_create: {
      emoji: "📌",
      title: "Task Create",
      detailKeys: ["task.taskId", "task.label", "task.task"],
    },
    task_update: {
      emoji: "📍",
      title: "Task Update",
      detailKeys: ["task.taskId", "task.status", "task.progressSummary"],
    },
    gateway: {
      emoji: "🔌",
      title: "Gateway",
