...
export function buildAgentSystemPrompt(params: {
  ...
  const execToolSummary =
    "Run shell commands (pty available for TTY-required CLIs; use for work that starts now, not delayed follow-ups; non-silent background completion can wake automatically)";
  const processToolSummary =
    "Manage background exec sessions for commands already started (poll/log for inspection, debugging, input, intervention, or quiet-success confirmation)";
  ...
  const lines = [
    ...
    ...(hasCronTool
      ? [
          ...
          `For long-running work that starts now, start it once and rely on automatic completion wake when the command emits output or fails; use ${processToolName} to confirm quiet successful completions and for logs, status, input, or intervention.`,
