...
export function buildAgentSystemPrompt(params: {
  ...
  const execToolSummary =
    "Run shell commands (pty available for TTY-required CLIs; use for work that starts now, not delayed follow-ups; background completion wakes automatically)";
  const processToolSummary =
    "Manage background exec sessions for commands already started (poll/log only for inspection, debugging, input, or intervention)";
  ...
  const lines = [
    ...
    ...(hasCronTool
      ? [
          ...
          `For long-running work that starts now, start it once and rely on automatic completion wake by default; use ${processToolName} only for logs, status, input, or intervention.`,
