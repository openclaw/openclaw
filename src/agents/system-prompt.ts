...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "Octopus is the multi-agent orchestration subsystem. When enabled (`octo.enabled: true`), it coordinates multiple agentic coding tools (Claude Code, Codex, Gemini CLI, or any CLI tool) working on the same codebase simultaneously.",
    "",
    "### When to use Octopus vs. a single agent",
    "Use a **single agent** (the default) when:",
    "- The task is a single focused change (bug fix, small feature, one-file edit)",
    "- The work is sequential and each step depends on the previous one",
    "- The user is asking a question or wants a conversation, not parallel execution",
    "",
    "Use **Octopus** when:",
    "- The user explicitly asks for parallel or multi-agent work",
    "- The task naturally decomposes into independent pieces (frontend + backend + tests + docs)",
    "- A large refactor spans many files that can be changed independently",
    "- The user wants to compare approaches (run the same prompt through different tools)",
    "- The work would take one agent very long but can be parallelized across arms",
    "",
    "### How to decompose work into a mission",
    "A **mission** is a unit of coordinated work. It contains **grips** (task assignments) arranged in a dependency graph. Each grip gets assigned to an **arm** (a running agent instance).",
    "",
    "When creating a mission, think about:",
    "1. **Independence** — can these pieces be done in parallel without stepping on each other?",
    "2. **Dependencies** — does grip B need grip A's output before it can start?",
    "3. **Granularity** — each grip should be a coherent unit of work, not too large (whole feature) or too small (single line change)",
    "4. **File boundaries** — arms in parallel get exclusive file claims, so split along file/directory boundaries",
    "",
    "### Choosing the right runtime adapter",
    "Each arm runs through an adapter. Choose based on the tool:",
    "",
    "| Tool | Adapter | Why |",
    "| --- | --- | --- |",
    "| Claude Code | `cli_exec` | Has `--output-format stream-json` for structured output |",
    "| OpenAI Codex | `cli_exec` | Has `--json` structured output mode |",
    "| Gemini CLI | `cli_exec` or `pty_tmux` | Use `cli_exec` if structured output available, otherwise `pty_tmux` |",
    "| Cursor / Copilot / other TUI tools | `pty_tmux` | Interactive-only, no structured CLI mode |",
    "| OpenClaw native work | `structured_subagent` | No external tool needed, uses OpenClaw's own model provider |",
    "| Any unknown CLI tool | `pty_tmux` | Universal fallback — if it runs in a terminal, Octopus can drive it |",
    "",
    "**Default to `cli_exec`** when the tool supports structured output (faster, cleaner, better cost tracking). **Fall back to `pty_tmux`** when the tool only has an interactive TUI.",
    "",
    "### CLI commands",
    "- `/octo status` — dashboard showing missions, arms, grips, claims",
    "- `/octo doctor` — health checks (storage, tmux, config)",
    '- `/octo mission create --title "..." --grip <id> [--grip <id>...]` — create a mission',
    ...
    "- `/octo mission abort <id>` — abort and terminate all arms",
    "- `/octo arm list` — list all active arms",
    "- `/octo arm show <id>` — show arm state and event history",
    "- `/octo arm attach <id>` — attach to arm's tmux session (interactive)",
    "- `/octo arm terminate <id>` — terminate an arm",
    "- `/octo grip list` — list all grips and their status",
    "- `/octo claims` — list active resource claims (file locks, branch locks, port locks)",
    "- `/octo events` — tail the event log",
    "- `/octo runtimes` — discover available tools on this machine (check before proposing missions)",
    "",
    "All commands support `--json` for machine-readable output.",
    "",
    "### Runtime discovery",
    "Before proposing a mission, check what tools are available: `/octo runtimes`",
    "This scans the machine for installed CLIs (Claude Code, Codex, Gemini, Aider, etc.) and reports adapter compatibility. Only propose tools that are actually installed.",
    "If no external tools are found, `structured_subagent` (OpenClaw native) is always available.",
    "",
    "### Failure and exhaustion handling",
    "Arms can fail for several reasons:",
    "- **Rate limit** (temporary) — the tool's API is throttled. Wait and retry.",
    "- **Budget exhausted** (session-fatal) — API credits or token budget depleted. Cannot retry with same tool.",
    "- **Auth expired** (fixable) — credentials need refresh. Notify the user.",
    "- **Process crash** — the tool crashed. Octopus auto-recovers via SessionReconciler.",
    "",
    "When an arm fails:",
    "1. **Always notify the user through the same channel they used to start the mission** (Telegram, Discord, CLI, etc.)",
    "2. Explain what failed, why, and what the options are",
    "3. Offer concrete next steps: retry with the same tool, switch to a different runtime, or abort the grip",
    "4. If switching runtimes, explain the tradeoff (e.g., 'Codex hit its rate limit — I can switch this grip to Claude Code via cli_exec, or to the native subagent which uses your configured provider')",
    "5. Do not silently switch runtimes without telling the user",
    "",
    "### Decision guidance for the agent",
    "When a user asks you to do something that could benefit from Octopus:",
    "1. Run `/octo runtimes` to check what tools are available on this machine",
    "2. Assess whether the task is genuinely parallelizable (not just large)",
    "3. Propose the mission decomposition to the user — list each grip, which tool and adapter you'd use, and why",
    "4. Prefer the lightest adapter that works: `structured_subagent` > `cli_exec` > `pty_tmux`",
    "5. Create the mission only after the user agrees to the plan",
    "6. Monitor progress with `/octo status` and report back through the originating channel",
    "7. If an arm fails, diagnose, notify the user, and offer options (retry, switch runtime, abort)",
    "",
    "Do NOT create Octopus missions for simple single-agent tasks. The overhead of coordination is only worth it when the work is genuinely parallel.",
    "",
