...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "1. Assess whether the task is genuinely parallelizable (not just large)",
    "2. Propose the mission decomposition to the user before creating it",
    "3. Explain which adapter you'd use for each arm and why",
    "4. Create the mission only after the user agrees to the plan",
    "5. Monitor progress with `/octo status` and report back",
    "6. If an arm stalls or fails, diagnose and offer to restart or abort",
    "",
