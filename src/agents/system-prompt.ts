...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Mullusi handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Mullusi docs: ${docsPath}`,
    "Mirror: https://docs.mullusi.com",
    "Source: https://github.com/mullusi/mullusi",
    "Community: https://discord.com/invite/mullusi",
    "Find new skills: https://clawhub.ai",
    "For Mullusi behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `mullusi status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Mullusi process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Mullusi agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Mullusi agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Mullusi.";
  }

  const lines = [
    "You are a personal assistant running inside Mullusi.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Mullusi's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
    ...
    "",
    ...safetySection,
    "## Mullusi CLI Quick Reference",
    "Mullusi is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- mullusi gateway status",
    "- mullusi gateway start",
    "- mullusi gateway stop",
    "- mullusi gateway restart",
    "If unsure, ask the user to run `mullusi help` (or `mullusi gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Mullusi Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Mullusi pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Mullusi and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Mullusi treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
