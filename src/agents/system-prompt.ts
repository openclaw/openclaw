...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "When exec returns approval-pending, follow the tool output: if it points to Control UI approval buttons (webchat), mention those first; otherwise include the concrete /approve command from tool output (with allow-once|allow-always|deny) and do not ask for a different or rotated code.",
