...
export function splitShellWords(input, maxWords = 48) {
    if (!input) {
        return [];
    }
    const words = [];
...
export function optionValue(words, names) {
...
export function positionalArgs(words, from = 1, optionsWithValue = []) {
    const args = [];
...
export function firstPositional(words, from = 1, optionsWithValue = []) {
    return positionalArgs(words, from, optionsWithValue)[0];
}
export function trimLeadingEnv(words) {
    if (words.length === 0) {
        return words;
    }
    let index = 0;
...
export function scanTopLevelChars(command, visit) {
    let quote;
    let escaped = false;
...
export function splitTopLevelStages(command) {
    const parts = [];
    let start = 0;
...
export function splitTopLevelPipes(command) {
    const parts = [];
    let start = 0;
    scanTopLevelChars(command, (char, index) => {
