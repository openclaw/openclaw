...
type CreateSmartAgentNeoTools = (typeof import("./smart-agent-neo-tools.js"))["createSmartAgentNeoTools"];
export type CreateSmartAgentNeoToolsOpts = Parameters<CreateSmartAgentNeoTools>[0];
...
export async function getSessionsSpawnTool(opts: CreateSmartAgentNeoToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createSmartAgentNeoTools } = await import("./smart-agent-neo-tools.js");
  const tool = createSmartAgentNeoTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
