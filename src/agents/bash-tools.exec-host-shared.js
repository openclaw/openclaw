...
import { minSecurity, maxAsk, resolveExecApprovalAllowedDecisions, resolveExecApprovals, } from "../infra/exec-approvals.js";
...
const loggedExecApprovalFollowupFailures = new Set();
...
export function enforceStrictInlineEvalApprovalBoundary(params) {
    if (!params.baseDecision.timedOut ||
...
export function shouldResolveExecApprovalUnavailableInline(params) {
    return (isHeadlessExecTrigger(params.trigger) &&
...
export async function sendExecApprovalFollowupResult(target, resultText, deps = {}) {
