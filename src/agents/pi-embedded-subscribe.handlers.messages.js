...
import { parseAssistantTextSignature, resolveAssistantMessagePhase, } from "../shared/chat-message-content.js";
...
import { extractAssistantText, extractAssistantThinking, extractAssistantVisibleText, extractThinkingFromTaggedStream, extractThinkingFromTaggedText, formatReasoningMessage, promoteThinkingTagsToBlocks, } from "./pi-embedded-utils.js";
...
function resolveAssistantStreamItemId(params) {
    ...
    const contentIndex = typeof params.contentIndex === "number" &&
        ...
        ? params.contentIndex
        : undefined;
    const candidateBlocks = contentIndex !== undefined ? [content[contentIndex]] : content.toReversed();
...
export function consumePendingToolMediaIntoReply(state, payload) {
...
export function consumePendingToolMediaReply(state) {
...
export function readPendingToolMediaReply(state) {
    if (state.pendingToolMediaUrls.length === 0 &&
        !state.pendingToolAudioAsVoice &&
        !state.pendingToolTrustedLocalMedia) {
        return null;
...
export function recordPendingAssistantReplyDirectives(state, parsed) {
...
export function consumePendingAssistantReplyDirectivesIntoReply(state, payload) {
...
export function handleMessageUpdate(ctx, evt) {
    ...
    const partialAssistant = assistantRecord?.partial && typeof assistantRecord.partial === "object"
        ? assistantRecord.partial
        : msg;
    ...
    const next = phaseAwareVisibleText ||
    ...
    if (next) {
        ...
        const finalParsedDelta = evtType === "text_end" ? ctx.consumePartialReplyDirectives("", { final: true }) : null;
...
export function handleMessageEnd(ctx, evt) {
    ...
    const silentExpectedWithoutSentinel = ctx.params.silentExpected && !isSilentReplyText(trimmedText, SILENT_REPLY_TOKEN);
    ...
    const shouldEmitReasoning = Boolean(!ctx.params.silentExpected &&
        !suppressDeterministicApprovalOutput &&
    ...
    if (!ctx.params.silentExpected &&
        ...
        if (hasBufferedBlockReply && ctx.blockChunker?.hasBuffered()) {
            ...
            if (ctx.state.blockReplyBreak === "text_end" && ctx.state.lastBlockReplyText != null) {
                ctx.log.debug(`Skipping message_end safety send for text_end channel - content already delivered via text_end`);
            }
            else {
