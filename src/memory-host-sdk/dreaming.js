...
import { lowercasePreservingWhitespace, normalizeLowercaseStringOrEmpty, normalizeOptionalLowercaseString, normalizeStringifiedOptionalString, } from "../shared/string-coerce.js";
...
function normalizeStringArray(value, allowed, fallback) {
    ...
    for (const entry of value) {
        ...
        if (!normalizedEntry || !allowedSet.has(normalizedEntry)) {
            continue;
        }
...
function resolveExecutionConfig(value, fallback) {
    ...
    const temperature = typeof temperatureRaw === "number" && Number.isFinite(temperatureRaw) && temperatureRaw >= 0
...
export function resolveMemoryDreamingPluginId(cfg) {
...
export function resolveMemoryDreamingPluginConfig(cfg) {
    const root = asNullableRecord(cfg);
    const plugins = asNullableRecord(root?.plugins);
...
export function resolveMemoryDreamingConfig(params) {
    ...
    const timezone = normalizeTrimmedString(dreaming?.timezone) ??
    ...
    return {
        ...
        phases: {
            light: {
                ...
                sources: normalizeStringArray(light?.sources, ["daily", "sessions", "recall"], DEFAULT_MEMORY_LIGHT_DREAMING_SOURCES),
            ...
            deep: {
                ...
                minRecallCount: normalizeNonNegativeInt(deep?.minRecallCount, DEFAULT_MEMORY_DEEP_DREAMING_MIN_RECALL_COUNT),
                minUniqueQueries: normalizeNonNegativeInt(deep?.minUniqueQueries, DEFAULT_MEMORY_DEEP_DREAMING_MIN_UNIQUE_QUERIES),
                recencyHalfLifeDays: normalizeNonNegativeInt(deep?.recencyHalfLifeDays, DEFAULT_MEMORY_DEEP_DREAMING_RECENCY_HALF_LIFE_DAYS),
                ...(typeof maxAgeDays === "number"
                    ...
                    : typeof DEFAULT_MEMORY_DEEP_DREAMING_MAX_AGE_DAYS === "number"
                        ...
                        : {}),
                sources: normalizeStringArray(deep?.sources, ["daily", "memory", "sessions", "logs", "recall"], DEFAULT_MEMORY_DEEP_DREAMING_SOURCES),
                recovery: {
            ...
            rem: {
                ...
                sources: normalizeStringArray(rem?.sources, ["memory", "daily", "deep"], DEFAULT_MEMORY_REM_DREAMING_SOURCES),
...
export function formatMemoryDreamingDay(epochMs, timezone) {
    if (!timezone) {
...
export function isSameMemoryDreamingDay(firstEpochMs, secondEpochMs, timezone) {
    return (formatMemoryDreamingDay(firstEpochMs, timezone) ===
        formatMemoryDreamingDay(secondEpochMs, timezone));
}
export function resolveMemoryDreamingWorkspaces(cfg) {
