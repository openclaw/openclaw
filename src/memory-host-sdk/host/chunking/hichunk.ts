 ...
 */
export function replaceHeadingMarkers(line: string, replacement: string | null | undefined): string {
	if (replacement == null) return line;
	const match = HEADING_PREFIX_RE.exec(line);
	if (!match?.[1] || match[1].trim() === "") return line;
 ...
 */
function pointsToClips(points: number[], startIdx: number, endIdx: number): Array<[number, number]> {
...
export function parseAnswerChunkingPoints(answer: string, maxLevel: number): MultiLevelChunkPoints {
	...
	for (const line of answer.split("\n")) {
		...
		if (levelIdx == null || levelIdx >= maxLevel) continue;
...
function buildResidualLines(
	...
	if (recurrentType === 0 || recurrentType === 1) return [];
	if (recurrentType !== 2) return [];
...
function buildInputInstruction(
	...
	while (gIdx < sentences.length) {
		...
		if (curTokens + lineTokens > windowSize) break;
 ...
 */
function postProcess(originSentences: HiSentenceEntry[], globalChunkPoints: MultiLevelChunkPoints): MemoryChunk[] {
...
export class HiChunkStrategy implements ChunkingStrategy {
	...
	async chunk(content: string, cfg: ChunkingConfig): Promise<MemoryChunk[]> {
		...
		// console.log("inputSentences", inputSentences);
