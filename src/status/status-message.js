...
import { buildModelAliasIndex, isCliProvider, resolveConfiguredModelRef, resolveModelRefFromString, } from "../agents/model-selection.js";
...
import { formatProviderModelRef, resolveSelectedAndActiveModel, } from "../auto-reply/model-runtime.js";
import { resolveChannelModelOverride } from "../channels/model-overrides.js";
import { resolveMainSessionKey, resolveSessionFilePath, resolveSessionFilePathOptions, resolveSessionPluginStatusLines, resolveSessionPluginTraceLines, } from "../config/sessions.js";
...
function resolveAgentRuntimeLabel(args) {
    ...
    const providerRaw = normalizeOptionalString(args.sessionEntry?.modelProvider) ??
    ...
    if (provider && isCliProvider(provider, args.config)) {
        return (AGENT_RUNTIME_LABELS[normalizeOptionalLowercaseString(providerRaw) ?? ""] ??
            `${provider} (cli)`);
    }
...
const readUsageFromSessionLog = (sessionId, sessionEntry, agentId, sessionKey, storePath) => {
    ...
    try {
        const snapshot = readLatestSessionUsageFromTranscript(sessionId, storePath, sessionEntry?.sessionFile, agentId ?? (sessionKey ? resolveAgentIdFromSessionKey(sessionKey) : undefined));
        if (!snapshot) {
            return undefined;
        }
...
export function buildStatusMessage(args) {
    ...
    if (runtimeModelRaw && !runtimeProviderRaw && runtimeModelRaw.includes("/")) {
        ...
        const fallbackMatchesRuntimeModel = initialFallbackState.active &&
            normalizeLowercaseStringOrEmpty(runtimeModelRaw) ===
                normalizeLowercaseStringOrEmpty(normalizeOptionalString(entry?.fallbackNoticeActiveModel ?? "") ?? "");
        const runtimeMatchesSelectedModel = normalizeLowercaseStringOrEmpty(runtimeModelRaw) ===
    ...
    const pluginStatusLine = pluginStatusLines.length > 0 || pluginTraceLines.length > 0
