import { resolveAgentConfig, resolveAgentDir, resolveDefaultAgentId, resolveSessionAgentId, resolveAgentModelFallbacksOverride, } from "../agents/agent-scope.js";
...
export async function buildStatusText(params) {
        ...
        try {
            ...
            const usageSummary = await Promise.race([
                ...
                new Promise((_, reject) => {
                    usageTimeout = setTimeout(() => reject(new Error("usage summary timeout")), usageSummaryTimeoutMs);
                }),
            ]).finally(() => {
                if (usageTimeout) {
    ...
    const effectiveFastMode = resolvedFastMode ??
    ...
    const effectiveHarness = params.resolvedHarness ??
    ...
    const explicitThinkingDefault = agentConfig?.thinkingDefault ??
    ...
    return buildStatusMessage({
        ...
        agentId: statusAgentId,
        explicitConfiguredContextTokens: typeof agentDefaults.contextTokens === "number" && agentDefaults.contextTokens > 0
