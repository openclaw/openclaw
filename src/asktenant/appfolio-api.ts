 ...
 */
export async function fetchAppFolioApi<T = unknown>(endpoint: string, options?: RequestInit): Promise<T> {
...
export async function getCurrentBalance(residentId: string, unitId: string) {
  return fetchAppFolioApi(`/api/resident/${encodeURIComponent(residentId)}/unit/${encodeURIComponent(unitId)}/balance`);
}
