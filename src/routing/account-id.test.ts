...
describe("account id normalization", () => {
	...
	function expectNormalizedAccountIdCase(params: {
		...
		const normalize = params.optional
			? normalizeOptionalAccountId
			: normalizeAccountId;
	...
	it.each([
		...
		{
			name: "defaults null to default account",
			input: null,
			expected: DEFAULT_ACCOUNT_ID,
		},
		{
		...
		{
			name: "normalizes valid ids to lowercase",
			input: "  Business_1  ",
			expected: "business_1",
		},
		{
	...
	it.each([
		{
			name: "keeps undefined optional values unset",
			input: undefined,
			expected: undefined,
		},
		{
			name: "keeps blank optional values unset",
			input: "   ",
			expected: undefined,
		},
		{
			name: "keeps invalid optional values unset",
			input: " !!! ",
			expected: undefined,
		},
		...reservedAccountIdCases.map(({ name, input }) => ({
		...
		{
			name: "normalizes valid optional values",
			input: "  Business  ",
			expected: "business",
		},
	] as const)("$name", ({ input, expected }) => {
