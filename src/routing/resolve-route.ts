...
function buildEvaluatedBindingsIndex(
	...
	for (const binding of bindings) {
		if (binding.match.peer.state === "valid") {
			for (const key of peerLookupKeys(
...
function getEvaluatedBindingsForChannelAccount(
	...
	const evaluated = mergeEvaluatedBindingsInSourceOrder(
		accountScoped,
		anyAccount,
	);

	cache.byChannelAccount.set(cacheKey, evaluated);
	cache.byChannelAccountIndex.set(
		cacheKey,
		buildEvaluatedBindingsIndex(evaluated),
	);
...
function resolveRouteCacheForConfig(
	cfg: OpenClawConfig,
): Map<string, ResolvedAgentRoute> {
...
export function resolveAgentRoute(
	...
	const bindingsIndex = getEvaluatedBindingIndexForChannelAccount(
		input.cfg,
		channel,
		accountId,
	);
	...
	const tiers: Array<{
		...
		},
		{
			...
			scopePeer: peer,
			candidates: guildId
				? (bindingsIndex.byGuildWithRoles.get(guildId) ?? [])
				: [],
			predicate: (candidate) =>
				hasGuildConstraint(candidate.match) &&
				hasRolesConstraint(candidate.match),
		},
		{
