...
function listAgents(cfg: ActiviConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: ActiviConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ActiviConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<ActiviConfig, EvaluatedBindingsCache>();
