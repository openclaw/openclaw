...
function listAgents(cfg: MullusiConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: MullusiConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<MullusiConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: MullusiConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: MullusiConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MullusiConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<MullusiConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  MullusiConfig,
  {
    bindingsRef: MullusiConfig["bindings"];
    agentsRef: MullusiConfig["agents"];
    sessionRef: MullusiConfig["session"];
...
function resolveRouteCacheForConfig(cfg: MullusiConfig): Map<string, ResolvedAgentRoute> {
