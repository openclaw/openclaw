...
function listAgents(cfg: SmartAgentNeoConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: SmartAgentNeoConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SmartAgentNeoConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<SmartAgentNeoConfig, EvaluatedBindingsCache>();
