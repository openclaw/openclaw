...
const resolveRoute = (
	params: Omit<Parameters<typeof resolveAgentRoute>[0], "cfg"> & {
		cfg?: OpenClawConfig;
	},
) =>
	resolveAgentRoute({
...
function createCompatPeer(kind: CompatRoutePeerKind, id: string) {
	return { kind, id } as unknown as NonNullable<
		Parameters<typeof resolveAgentRoute>[0]["peer"]
	>;
}
...
describe("resolveAgentRoute", () => {
	...
	const expectRouteResolutionCase = (params: {
		routeParams: Omit<Parameters<typeof resolveRoute>[0], "cfg"> & {
			cfg: OpenClawConfig;
		};
		expected: ResolvedRouteExpectation;
	}) => {
	...
	])("dmScope=%s controls direct-message session key isolation", ({
		dmScope,
		expected,
	}) => {
		const cfg: OpenClawConfig = {
...
] as const)("dmScope=per-account-channel-peer $name", ({
	accountId,
	expected,
}) => {
	const route = resolveAgentRoute({
