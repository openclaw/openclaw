...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("smart-agent-neo-triggers-", async (home) => {
      const inboundDir = join(home, ".smart-agent-neo", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "smart-agent-neo"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("smart-agent-neo-triggers-bypass-", async (home) => {
      // Sensitive host file outside .smart-agent-neo
