...
const piEmbeddedMocks = getSharedMocks("mullusi.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("mullusi.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("mullusi.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("mullusi.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    mullusiHome: process.env.MULLUSI_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("MULLUSI_HOME", snapshot.mullusiHome);
  restoreKey("MULLUSI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.MULLUSI_STATE_DIR = join(home, ".mullusi");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "mullusi-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".mullusi", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): MullusiConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "mullusi"),
