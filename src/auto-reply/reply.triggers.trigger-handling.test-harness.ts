...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openclawHome: process.env.NEXUS_HOME,
    stateDir: process.env.NEXUS_STATE_DIR,
  };
}
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("NEXUS_HOME", snapshot.openclawHome);
  restoreKey("NEXUS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.NEXUS_STATE_DIR = join(home, ".nexus-agent");
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".nexus-agent", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
