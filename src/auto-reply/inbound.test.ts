...
describe("flushAllInboundDebouncers", () => {
  ...
  it("keeps registered debouncers across restart sweeps so late ingress is still flushable", async () => {
    vi.useFakeTimers();

    // Use a debounceMs longer than five minutes so the timer does not fire
    // and the debouncer cannot actually drain on the first sweep.
    createInboundDebouncer<{ key: string; id: string }>({
    ...
    // Advance the clock well past any prior staleness window.
    vi.advanceTimersByTime(5 * 60 * 1000 + 1);

    // First sweep with a zero-ms deadline drains nothing.
    await flushAllInboundDebouncers({ timeoutMs: 0 });

    // Second sweep finds the same handle still registered. There were never
    // any buffered messages enqueued, so flushedCount is 0; the point is that
    // the handle was not evicted between sweeps and could still rescue any
    // message that arrives before server.close().
  ...
  it("keeps debouncers with recent activity registered across sweeps", async () => {
    vi.useFakeTimers();
    ...
    // Flush with zero timeout — debouncer can't drain on the tight deadline,
    // but it must remain registered for a subsequent full flush to pick up
