...
export function buildBuiltinChatCommands(
  ...
  const commands: ChatCommandDefinition[] = [
    ...
    defineChatCommand({
      ...
      nativeName: "messaging_window",
      description: "Set inbound message batching window.",
      textAliases: ["/messaging_window", "/messaging-window"],
      category: "options",
      tier: "standard",
      acceptsArgs: true,
      args: [
        {
          name: "action",
          description: "messaging window",
          type: "string",
          choices: [
            { value: "status", label: "View settings" },
            { value: "1s", label: "All channels: 1s" },
            { value: "3s", label: "All channels: 3s" },
            { value: "5s", label: "All channels: 5s" },
            { value: "10s", label: "All channels: 10s" },
            { value: "off", label: "All channels: off" },
            { value: "current 1s", label: "This channel: 1s" },
            { value: "current 3s", label: "This channel: 3s" },
            { value: "current 5s", label: "This channel: 5s" },
            { value: "current off", label: "This channel: off" },
          ],
        },
        {
          name: "value",
          description: "duration, channel name, or reset target",
          type: "string",
          captureRemaining: true,
        },
      ],
      argsMenu: {
        arg: "action",
        title: "How long should OpenClaw wait for follow-up messages before replying?",
      },
    }),
    defineChatCommand({
      key: "allowlist",
