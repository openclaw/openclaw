...
export function isCommandEnabled(cfg: EasyHubConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: EasyHubConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: EasyHubConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: EasyHubConfig) {
