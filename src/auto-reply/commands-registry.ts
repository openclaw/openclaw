...
export function isCommandEnabled(cfg: DNAConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: DNAConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: DNAConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: DNAConfig) {
