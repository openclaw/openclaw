...
describe("listThinkingLevels", () => {
  it("excludes minimal for codex models that do not support it", () => {
    expect(listThinkingLevels("openai-codex", "gpt-5.3-codex")).not.toContain("minimal");
    expect(listThinkingLevels("github-copilot", "gpt-5.2-codex")).not.toContain("minimal");
  });
  ...
  it("returns binary level set for zai providers", () => {
    expect(listThinkingLevels("zai", "glm-4.7")).toEqual(["off", "low"]);
  });
});
