...
describe("trigger handling", () => {
  it("allows elevated off in groups without mention", async () => {
    await withTempHome(async (home) => {
      ...
      expect(store["agent:main:whatsapp:group:[redacted-email]"]?.elevatedLevel).toBe("off");
    });
  });
  it("allows elevated directive in groups when mentioned", async () => {
    await withTempHome(async (home) => {
      ...
      expect(store["agent:main:whatsapp:group:[redacted-email]"]?.elevatedLevel).toBe("on");
    });
  });
