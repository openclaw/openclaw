...
export async function runPreparedReply(
  ...
  const replyRunPriorityLaneRaw = normalizeOptionalString(ctx.ReplyRunPriorityLane);
  const replyRunPriorityLane: "foreground" | "ambient" | undefined =
    replyRunPriorityLaneRaw === "foreground" || replyRunPriorityLaneRaw === "ambient"
      ? replyRunPriorityLaneRaw
      : undefined;
  const replyRunBaseKey =
    normalizeOptionalString(ctx.ReplyRunBaseKey) ?? sessionKey ?? sessionIdFinal;
  const replyRunKey =
    normalizeOptionalString(ctx.ReplyRunKey) ??
    (replyRunPriorityLane ? `${replyRunBaseKey}:reply-lane:${replyRunPriorityLane}` : undefined) ??
    sessionKey ??
    sessionIdFinal;
  ...
  const sessionLaneKey = piRuntime ? piRuntime.resolveEmbeddedSessionLane(replyRunKey) : undefined;
  ...
  if (resolvedQueue.mode === "interrupt" && sessionLaneKey && laneSize > 0) {
    ...
    const activeSessionId = piRuntime?.resolveActiveEmbeddedRunSessionId(replyRunKey);
  ...
  const queueKey = replyRunKey;
  preparedSessionState = resolvePreparedSessionState();
  const resolveActiveQueueSessionId = () =>
    piRuntime?.resolveActiveEmbeddedRunSessionId(replyRunKey) ?? preparedSessionState.sessionId;
  ...
  const followupRun = {
    ...
    run: {
      ...
      replyRunKey,
      sessionLaneKey: replyRunKey,
      replyRunPriorityLane,
  ...
  return runReplyAgent({
    ...
    isRunActive: () => {
      ...
      const latestActiveSessionId =
        piRuntime?.resolveActiveEmbeddedRunSessionId(replyRunKey) ?? latestSessionState.sessionId;
