...
import { setChannelConversationBindingIdleTimeoutBySessionKey, setChannelConversationBindingMaxAgeBySessionKey, } from "../../channels/plugins/conversation-bindings.js";
...
import { buildRestartSuccessContinuation, formatDoctorNonInteractiveHint, removeRestartSentinelFile, writeRestartSentinel, } from "../../infra/restart-sentinel.js";
...
function resolveUpdatedBindingExpiry(params) {
    const expiries = params.bindings
        ...
        if (params.action === SESSION_ACTION_IDLE) {
            const idleTimeoutMs = typeof binding.idleTimeoutMs === "number" && Number.isFinite(binding.idleTimeoutMs)
        ...
        const maxAgeMs = typeof binding.maxAgeMs === "number" && Number.isFinite(binding.maxAgeMs)
...
export const handleFastCommand = async (params, allowTextCommands) => {
    if (!allowTextCommands) {
        return null;
    }
    const normalized = params.command.commandBodyNormalized;
    ...
    if (!rawMode || rawMode === "status") {
        ...
        const suffix = state.source === "agent"
            ? " (agent)"
            : state.source === "config"
                ? " (config)"
...
export const handleSessionCommand = async (params, allowTextCommands) => {
    ...
    const channelId = params.command.channelId ??
    ...
    const commandSupportsLifecycleUpdate = action === SESSION_ACTION_IDLE
    ...
    const supportsLifecycleUpdate = action === SESSION_ACTION_IDLE
    ...
    const idleTimeoutMs = resolveSessionBindingDurationMs(activeBinding, "idleTimeoutMs", 24 * 60 * 60 * 1000);
    ...
    if (!durationArgRaw) {
        if (action === SESSION_ACTION_IDLE) {
            if (typeof idleExpiresAt === "number" &&
    ...
    if (updatedBindings.length === 0) {
        return {
            shouldContinue: false,
            reply: {
                text: action === SESSION_ACTION_IDLE
    ...
    const expiryLabel = typeof nextExpiry === "number" && Number.isFinite(nextExpiry)
