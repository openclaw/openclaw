...
import { resolveAcpCommandAccountId, resolveAcpCommandBindingContext, resolveAcpCommandThreadId, } from "./context.js";
...
async function bindSpawnedAcpSessionToThread(params) {
    ...
    if (threadId) {
        ...
        const boundBy = typeof existingBinding?.metadata?.boundBy === "string"
...
function resolveAcpSessionForCommandOrStop(params) {
    ...
    if (resolved.kind === "none") {
        return stopWithText(collectAcpErrorText({
            error: new AcpRuntimeError("ACP_SESSION_INIT_FAILED", `Session is not ACP-enabled: ${params.sessionKey}`),
            fallbackCode: "ACP_SESSION_INIT_FAILED",
