...
import { ACP_DOCTOR_USAGE, ACP_INSTALL_USAGE, ACP_SESSIONS_USAGE, formatAcpCapabilitiesText, resolveAcpInstallCommandHint, resolveConfiguredAcpBackendId, stopWithText, } from "./shared.js";
import { resolveBoundAcpThreadSessionKey } from "./targets.js";
export async function handleAcpDoctorAction(params, restTokens) {
    ...
    const errorStatsText = Object.entries(managerSnapshot.errorsByCode)
...
export function handleAcpInstallAction(params, restTokens) {
    if (restTokens.length > 0) {
...
export function handleAcpSessionsAction(params, restTokens) {
    if (restTokens.length > 0) {
    ...
    const rows = Object.entries(store)
        ...
        const bindingThreadId = bindingService
            .listBySession(key)
            .find((binding) => (!normalizedChannel || binding.conversation.channel === normalizedChannel) &&
            (!normalizedAccountId || binding.conversation.accountId === normalizedAccountId))?.conversation.conversationId;
