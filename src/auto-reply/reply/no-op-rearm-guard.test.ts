...
describe("defensive backstop", () => {
  it("flags a room-event provider run with no prior admission, once", () => {
    ...
    const first = guard.assertEvaluatedBeforeProvider({
      sessionKey: "s",
      inboundEventKind: "room_event",
      site: "followup-runner",
    });
    expect(first?.code).toBe("noop-rearm-admission-bypass");
    const second = guard.assertEvaluatedBeforeProvider({
      sessionKey: "s",
      inboundEventKind: "room_event",
      site: "followup-runner",
    });
  ...
  it("ignores non-room-event runs", () => {
    const guard = new NoOpRearmGuard();
    expect(
      guard.assertEvaluatedBeforeProvider({
        sessionKey: "s",
        inboundEventKind: "user_request",
        site: "followup-runner",
      }),
