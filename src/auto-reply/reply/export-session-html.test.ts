...
describe("export-session HTML template (#22595)", () => {
  ...
  it("full generateHtml pipeline produces valid self-contained HTML", () => {
    ...
    // Use arrow-function replacer to avoid $& special pattern interpretation
    // in vendor JS content (highlight.min.js contains "$&" in R language regex)
    const html = template
      .replace("{{CSS}}", () => css)
    ...
    // 1. No unreplaced placeholders remain ({{HIGHLIGHT_JS}} is critical —
    //    highlight.min.js contains "$&" which re-inserts the matched pattern
    //    if a plain string replacer is used instead of a function replacer)
