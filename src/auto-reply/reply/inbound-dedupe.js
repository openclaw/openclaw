...
const inboundDedupeInFlight = resolveGlobalSingleton(INBOUND_DEDUPE_INFLIGHT_KEY, () => new Set());
...
export function claimInboundDedupe(ctx, opts) {
    const key = buildInboundDedupeKey(ctx);
    if (!key) {
...
export function commitInboundDedupe(key, opts) {
    const cache = opts?.cache ?? inboundDedupeCache;
    cache.check(key, opts?.now);
    const inFlight = opts?.inFlight ?? inboundDedupeInFlight;
    inFlight.delete(key);
}
export function releaseInboundDedupe(key, opts) {
    const inFlight = opts?.inFlight ?? inboundDedupeInFlight;
    inFlight.delete(key);
}
