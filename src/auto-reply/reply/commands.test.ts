...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-commands-"));
...
function buildParams(commandBody: string, cfg: DNAConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
