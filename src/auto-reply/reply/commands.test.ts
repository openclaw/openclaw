...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-commands-"));
...
function buildParams(commandBody: string, cfg: EasyHubConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
