...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop mullusi!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@mullusi_bot", { botUsername: "mullusi_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@mullusi_bot", { botUsername: "mullusi_bot" })).toBe(true);
