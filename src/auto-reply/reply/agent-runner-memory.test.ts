...
describe("runMemoryFlushIfNeeded", () => {
  ...
  it("does not suppress byte-threshold compaction when the active session file changes", async () => {
  ...
  it("records byte retry metadata against the active transcript when the entry file is stale", async () => {
    ...
    expect(compactEmbeddedAgentSessionMock).toHaveBeenCalledTimes(1);
    expect(firstEntry?.sessionFile).toBe(staleSessionFile);
