...
export async function readPostCompactionContext(workspaceDir: string): Promise<string | null> {
  ...
  try {
    ...
    return (
      ...
      "If startup instructions include placeholders like YYYY-MM-DD, resolve them using today in the user's timezone (and call session_status if you need an explicit current date/time).\n\n" +
