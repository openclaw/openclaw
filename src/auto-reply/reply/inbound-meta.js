...
function buildLocationContextPayload(ctx) {
    const payload = {
        ...
        accuracy_m: typeof ctx.LocationAccuracy === "number" && Number.isFinite(ctx.LocationAccuracy)
...
function resolveInboundFormattingHints(ctx) {
    ...
    const agentPrompt = getLoadedChannelPluginById(normalizedChannel)
        ?.agentPrompt;
    return agentPrompt?.inboundFormattingHints?.({
        accountId: normalizePromptMetadataString(ctx.AccountId) ?? undefined,
    });
}
export function buildInboundMetaSystemPrompt(ctx, options) {
    ...
    const payload = {
        ...
        response_format: options?.includeFormattingHints === false ? undefined : resolveInboundFormattingHints(ctx),
    };
...
export function buildInboundUserContextPrefix(ctx, envelope) {
    const blocks = [];
    const chatType = normalizeChatType(ctx.ChatType);
    const isDirect = !chatType || chatType === "direct";
    ...
    const conversationInfo = {
        ...
        topic_id: ctx.MessageThreadId != null
    ...
    if (Object.values(conversationInfo).some((v) => v !== undefined)) {
        blocks.push(formatUntrustedJsonBlock("Conversation info (untrusted metadata):", conversationInfo));
    }
    const senderInfo = {
    ...
    if (threadStarterBody) {
        blocks.push(formatUntrustedJsonBlock("Thread starter (untrusted, for context):", {
            body: threadStarterBody,
        }));
    }
    const replyToBody = sanitizePromptBody(ctx.ReplyToBody);
    if (replyToBody) {
        blocks.push(formatUntrustedJsonBlock("Replied message (untrusted, for context):", {
    ...
    if (forwardedFrom) {
        blocks.push(formatUntrustedJsonBlock("Forwarded message context (untrusted metadata):", forwardedContext));
    }
    ...
    if (boundedHistory.length > 0) {
        blocks.push(formatUntrustedJsonBlock("Chat history since last reply (untrusted, for context):", boundedHistory.map((entry) => ({
