...
export function finalizeInboundContext(ctx, opts = {}) {
    ...
    if (Array.isArray(normalized.UntrustedContext)) {
        const normalizedUntrusted = normalized.UntrustedContext.map((entry) => sanitizeInboundSystemTags(normalizeInboundTextNewlines(entry))).filter((entry) => Boolean(entry));
