...
import { BILLING_ERROR_USER_MESSAGE, formatRateLimitOrOverloadedErrorCopy, isCompactionFailureError, isContextOverflowError, isBillingErrorMessage, isLikelyContextOverflowError, isOverloadedErrorMessage, isRateLimitErrorMessage, isTransientHttpError, } from "../../agents/pi-embedded-helpers.js";
...
import { resolveGroupSessionKey, resolveSessionTranscriptPath, updateSessionStore, } from "../../config/sessions.js";
import { logVerbose } from "../../globals.js";
import { emitAgentEvent, registerAgentRunContext } from "../../infra/agent-events.js";
import { formatErrorMessage } from "../../infra/errors.js";
import { CommandLaneClearedError, GatewayDrainingError } from "../../process/command-queue.js";
import { defaultRuntime } from "../../runtime.js";
import { hasNonEmptyString, normalizeLowercaseStringOrEmpty, normalizeOptionalString, readStringValue, } from "../../shared/string-coerce.js";
import { sanitizeForLog } from "../../terminal/ansi.js";
import { isMarkdownCapableMessageChannel, resolveMessageChannel, } from "../../utils/message-channel.js";
import { isInternalMessageChannel } from "../../utils/message-channel.js";
import { stripHeartbeatToken } from "../heartbeat.js";
import { HEARTBEAT_TOKEN, isSilentReplyPrefixText, isSilentReplyText, SILENT_REPLY_TOKEN, startsWithSilentToken, stripLeadingSilentToken, } from "../tokens.js";
import { resolveRunAuthProfile } from "./agent-runner-auth-profile.js";
import { buildEmbeddedRunExecutionParams, resolveQueuedReplyRuntimeConfig, resolveModelFallbackOptions, } from "./agent-runner-utils.js";
...
function isToolResultTurnMismatchError(message) {
    ...
    return (lower.includes("toolresult") &&
...
function formatForwardedExternalRunFailureText(message) {
    ...
    const detail = sanitized.length > EXTERNAL_RUN_FAILURE_DETAIL_MAX_CHARS
...
function scoreChattyFinalReplyText(text) {
    ...
    if (/\b(?:in summary|to summarize|here(?:'s| is) what|what changed|what I verified)\b/i.test(trimmed)) {
        score += 1;
    }
    return score;
}
...
export async function runAgentTurnWithFallback(params) {
    ...
    const effectiveRun = runtimeConfig === params.followupRun.run.config
    ...
    const replyMediaContext = params.replyMediaContext ??
    ...
    const shouldSurfaceToControlUi = isInternalMessageChannel(params.followupRun.run.messageProvider ??
    ...
    const persistFallbackCandidateSelection = async (provider, model) => {
        ...
        const isUserModelOverride = activeSessionEntry.modelOverrideSource === "user" ||
        ...
        return async () => {
            const rolledBackInMemory = rollbackFallbackSelectionStateIfUnchanged(activeSessionEntry, nextState, previousState);
    ...
    while (true) {
        try {
            const normalizeStreamingText = (payload) => {
                ...
                if (isSilentReplyPrefixText(text, SILENT_REPLY_TOKEN) ||
            ...
            const fallbackResult = await runWithModelFallback({
                ...
                run: async (provider, model, runOptions) => {
                    ...
                    if (isCliProvider(cliExecutionProvider, runtimeConfig)) {
                        ...
                        const authProfile = resolveRunAuthProfile(params.followupRun.run, cliExecutionProvider, {
                            config: runtimeConfig,
                        });
                    ...
                    return (async () => {
                        ...
                        try {
                            const result = await runEmbeddedPiAgent({
                                ...
                                groupChannel: normalizeOptionalString(params.sessionCtx.GroupChannel) ??
                                ...
                                onAgentEvent: async (evt) => {
                                    ...
                                    if (evt.stream === "command_output") {
                                        await params.opts?.onCommandOutput?.({
                                            ...
                                            status: readStringValue(evt.data.status),
                                            exitCode: typeof evt.data.exitCode === "number" || evt.data.exitCode === null
                                                ? evt.data.exitCode
                                                : undefined,
            ...
            if (err instanceof LiveSessionModelSwitchError) {
                ...
                if (liveModelSwitchRetries > MAX_LIVE_SWITCH_RETRIES) {
                    ...
                    // See: https://github.com/openclaw/openclaw/issues/58348
                    defaultRuntime.error(`Live model switch failed after ${MAX_LIVE_SWITCH_RETRIES} retries ` +
            ...
            const rateLimitOrOverloadedCopy = !isFallbackSummary || isPureTransientSummary
    ...
    if (runResult) {
        const hasNonErrorContent = runResult.payloads?.some((p) => !p.isError && !p.isReasoning && hasOutboundReplyContent(p, { trimText: true }));
