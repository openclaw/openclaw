...
describe("persistSessionUsageUpdate", () => {
  ...
  it("can persist selected runtime model while usage came from a fallback model", async () => {
    const storePath = await createStorePath("openclaw-usage-model-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
      sessionKey,
    ...
    await persistSessionUsageUpdate({
      storePath,
      sessionKey,
      usage: { input: 1_000, output: 100, total: 1_100 },
      modelUsed: "gpt-5.4",
      providerUsed: "openai",
