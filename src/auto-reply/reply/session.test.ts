...
describe("initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  it("/new clears per-session model and provider overrides (#21725)", async () => {
    ...
    expect(result.resetTriggered).toBe(true);
    expect(result.sessionId).not.toBe(existingSessionId);
    // Model override should NOT carry over — /new resets to default model
    expect(result.sessionEntry.modelOverride).toBeUndefined();
    expect(result.sessionEntry.providerOverride).toBeUndefined();
    // Behavior overrides (verbose, thinking, etc.) should still carry over
    expect(result.sessionEntry.verboseLevel).toBe("on");
  });
});
