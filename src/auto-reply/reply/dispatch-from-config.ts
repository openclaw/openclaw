...
import { isPowernapDraining, recordDrainedMessage } from "./powernap-drain.js";
...
export async function dispatchReplyFromConfig(
  ...
  if (isPowernapDraining()) {
    ...
    recordDrainedMessage(
      ctx.From ?? ctx.MessageParticipant ?? "<unknown>",
      ctx.OriginatingChannel ?? ctx.Provider ?? ctx.Surface ?? "<unknown>",
    );
