...
export async function dispatchReplyFromConfig(
  ...
  try {
    ...
    if (suppressDelivery) {
      logVerbose(
        `Delivery suppressed by send policy for session ${sessionStoreEntry.sessionKey ?? sessionKey ?? "unknown"} â agent will still process the message`,
      );
    }
...
ipluginFallbackReason && hookRunnermport { resolveSendableOutboundReplyParts } from "openclaw/plugin-sdk/reply-payload";
import { isParentOwnedBackgroundAcpSession } from "../../acp/session-interaction-mode.js";
import {
  resolveAgentConfig,
  resolveAgentWorkspaceDir,
