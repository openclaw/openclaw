...
import { formatThinkingLevels, isThinkingLevelSupported, resolveSupportedThinkingLevel, } from "../thinking.js";
import { resolveModelSelectionFromDirective } from "./directive-handling.model-selection.js";
import { maybeHandleModelDirectiveInfo } from "./directive-handling.model.js";
import { maybeHandleQueueDirective } from "./directive-handling.queue-validation.js";
import { canPersistInternalExecDirective, canPersistInternalVerboseDirective, formatDirectiveAck, formatElevatedRuntimeHint, formatElevatedUnavailableText, formatInternalExecPersistenceDeniedText, formatInternalVerboseCurrentReplyOnlyText, formatInternalVerbosePersistenceDeniedText, enqueueModeSwitchEvents, withOptions, } from "./directive-handling.shared.js";
...
export async function handleDirectiveOnly(params) {
    ...
    const effectiveFastModeSource = directives.fastMode !== undefined ? "session" : fastModeState.source;
    ...
    if (directives.hasTraceDirective && !directives.traceLevel) {
        if (!directives.rawTraceLevel) {
            const level = sessionEntry.traceLevel ?? "off";
            return {
    ...
    if (directives.hasFastDirective && directives.fastMode === undefined) {
        if (!directives.rawFastMode ||
            ...
            const sourceSuffix = effectiveFastModeSource === "config"
                ? " (config)"
                : effectiveFastModeSource === "default"
                    ? " (default)"
                    : "";
            return {
                text: withOptions(`Current fast mode: ${effectiveFastMode ? "on" : "off"}${sourceSuffix}.`, "status, on, off"),
            };
        }
        return {
    ...
    if (directives.hasExecDirective) {
        ...
        if (!directives.hasExecOptions) {
            ...
            return {
                text: withOptions(`Current exec defaults: host=${renderExecTargetLabel(execDefaults.host)}, effective=${execDefaults.effectiveHost}, security=${execDefaults.security}, ask=${execDefaults.ask}, ${nodeLabel}.`, "host=auto|sandbox|gateway|node, security=deny|allowlist|full, ask=off|on-miss|always, node=<id>"),
            };
        }
    }
    ...
    const remappedUnsupportedThinkLevel = !directives.hasThinkDirective &&
        nextThinkLevel &&
        !isThinkingLevelSupported({
            provider: resolvedProvider,
            model: resolvedModel,
    ...
    const shouldPersistSessionEntry = (directives.hasThinkDirective && Boolean(directives.thinkLevel)) ||
    ...
    const fastModeChanged = directives.hasFastDirective &&
    ...
    if (directives.hasThinkDirective && directives.thinkLevel) {
        ...
        parts.push(displayedThinkLevel === "off"
            ? "Thinking disabled."
            : `Thinking level set to ${displayedThinkLevel}.`);
        if (directives.thinkLevel === "max" && displayedThinkLevel !== "max") {
            parts.push(`max not supported for ${resolvedProvider}/${resolvedModel}; using ${displayedThinkLevel}.`);
        }
    }
    if (directives.hasFastDirective && directives.fastMode !== undefined) {
        parts.push(directives.fastMode
            ? formatDirectiveAck("Fast mode enabled.")
            : formatDirectiveAck("Fast mode disabled."));
    }
    if (directives.hasVerboseDirective && directives.verboseLevel) {
        parts.push(!allowInternalVerbosePersistence
    ...
    if (directives.hasTraceDirective && directives.traceLevel) {
        parts.push(directives.traceLevel === "off"
            ? formatDirectiveAck("Trace disabled.")
            : directives.traceLevel === "raw"
                ? formatDirectiveAck("Trace set to raw. Warning: trace output may contain sensitive information.")
                : formatDirectiveAck("Trace enabled. Warning: trace output may contain sensitive information."));
    }
    if (directives.hasVerboseDirective &&
        directives.verboseLevel &&
        !allowInternalVerbosePersistence) {
