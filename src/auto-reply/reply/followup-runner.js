...
import { buildAgentRuntimeDeliveryPlan, buildAgentRuntimeOutcomePlan, } from "../../agents/runtime-plan/build.js";
...
import { resolveQueuedReplyExecutionConfig, resolveQueuedReplyRuntimeConfig, resolveRunAuthProfile, } from "./agent-runner-utils.js";
...
export function createFollowupRunner(params) {
    ...
    const sendFollowupPayloads = async (payloads, queued, resolvedRun) => {
        ...
        for (const payload of sendablePayloads) {
            ...
            const deliveryRoute = providerRoute?.route === "origin" && shouldRouteToOriginating
                ? "origin"
        ...
        if (crossChannelRouteFailureNeedsNotice &&
            !routedAnyCrossChannelPayloadToOrigin &&
            opts?.onBlockReply) {
            await opts.onBlockReply({
                text: "Follow-up completed, but OpenClaw could not deliver it to the originating " +
    ...
    return async (queued) => {
        ...
        const runtimeConfig = resolveQueuedReplyRuntimeConfig(queued.run.config);
        const effectiveQueued = runtimeConfig === queued.run.config
            ? queued
        ...
        try {
            ...
            let activeSessionEntry = (sessionKey ? sessionStore?.[sessionKey] : undefined) ?? sessionEntry;
            ...
            try {
                ...
                const fallbackResult = await runWithModelFallback({
                    ...
                    classifyResult: ({ result, provider, model }) => outcomePlan.classifyRunResult({ result, provider, model }),
                    run: async (provider, model, runOptions) => {
                        ...
                        try {
                            const result = await runEmbeddedPiAgent({
                                ...
                                currentThreadTs: queued.originatingThreadId != null
            ...
            if (autoCompactionCount > 0) {
                ...
                const refreshedSessionEntry = sessionKey && sessionStore ? sessionStore[sessionKey] : undefined;
