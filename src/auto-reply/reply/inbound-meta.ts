...
export function buildInboundMetaSystemPrompt(
  ...
  const payload = {
    ...
    requester_identity: resolveTrustedRequesterIdentity({
      ctx,
  ...
  return [
    ...
    "When requester_identity is present, it is resolved from stable provider ids matched against trusted configuration such as session.identityLinks, not from display names or quoted text.",
