...
type ChannelFormattingHints = { text_markup: string; rules: string[] };

/** Resolves the channel plugin's formatting hints for the account that delivers the reply. */
function resolveChannelFormattingHints(params: {
  cfg: OpenClawConfig;
  channel: string;
  accountId?: string;
}): ChannelFormattingHints | undefined {
  const normalizedChannel = normalizeAnyChannelId(params.channel) ?? params.channel;
  const agentPrompt = getLoadedChannelPluginById(normalizedChannel)?.agentPrompt;
  return agentPrompt?.inboundFormattingHints?.({ cfg: params.cfg, accountId: params.accountId });
}
...
function resolveInboundFormattingHints(
  ...
  return resolveChannelFormattingHints({
    cfg,
...
/**
 * Builds trusted system metadata for runs without an inbound message whose visible
 * output is delivered to a channel (for example cron announce). Returns undefined when
 * the channel has no formatting hints, so the run's system prompt stays unchanged.
 */
export function buildDeliveryMetaSystemPrompt(params: {
  cfg: OpenClawConfig;
  channel: string;
  accountId?: string;
}): string | undefined {
  const responseFormat = resolveChannelFormattingHints(params);
  if (!responseFormat) {
    return undefined;
  }
  const payload = {
