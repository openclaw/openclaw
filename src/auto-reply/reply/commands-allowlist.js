...
async function readAllowlistConfig(params) {
    const plugin = getChannelPlugin(params.channelId);
    return ((await plugin?.allowlist?.readConfig?.({
        cfg: params.cfg,
        accountId: params.accountId,
    })) ?? {});
}
export const handleAllowlistCommand = async (params, allowTextCommands) => {
    ...
    if (parsed.action === "list") {
        ...
        const resolvedDm = parsed.resolve && dmDisplay.length > 0
            ...
            : undefined;
        const resolvedGroup = parsed.resolve && groupOverrideDisplay.length > 0
            ? await resolveAllowlistNames({
                cfg: params.cfg,
                channelId,
                accountId,
