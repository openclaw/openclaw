...
describe("runReplyAgent memory flush during heartbeat", () => {
  it("skips memory flush during heartbeat by default", async () => {
    ...
    // Should only have the main prompt, no memory flush
    expect(calls.map((call) => call.prompt)).toEqual(["hello"]);
  });

  it("runs memory flush during heartbeat when runDuringHeartbeats is true", async () => {
    runEmbeddedPiAgentMock.mockReset();
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-flush-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    ...
    // Should have memory flush prompt first, then main prompt
