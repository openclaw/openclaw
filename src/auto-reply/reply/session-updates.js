...
import { getSkillsSnapshotVersion, shouldRefreshSnapshotForVersion, } from "../../agents/skills/refresh-state.js";
import { ensureSkillsWatcher } from "../../agents/skills/refresh.js";
import { resolveSessionFilePath, resolveSessionFilePathOptions, updateSessionStore, } from "../../config/sessions.js";
...
export async function ensureSkillSnapshot(params) {
    ...
    const shouldRefreshSnapshot = shouldRefreshSnapshotForVersion(existingSnapshot?.version, snapshotVersion) ||
    ...
    const hasFreshSnapshotInEntry = Boolean(nextEntry?.skillsSnapshot) &&
...
function resolveCompactionSessionFile(params) {
    ...
    const normalizedRewrittenSessionFile = rewrittenSessionFile && path.isAbsolute(rewrittenSessionFile)
        ? canonicalizeAbsoluteSessionFilePath(rewrittenSessionFile)
        : rewrittenSessionFile;
    return resolveSessionFilePath(params.newSessionId, normalizedRewrittenSessionFile ? { sessionFile: normalizedRewrittenSessionFile } : undefined, pathOpts);
}
...
function rewriteSessionFileForNewSessionId(params) {
    ...
    if (withoutExt.startsWith(`${params.previousSessionId}-topic-`)) {
        return path.join(path.dirname(trimmed), `${params.nextSessionId}${base.slice(params.previousSessionId.length)}`);
    }
