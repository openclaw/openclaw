...
import { evaluateSessionFreshness, resolveChannelResetConfig, resolveSessionResetPolicy, resolveSessionResetType, resolveThreadFlag, } from "../../config/sessions/reset.js";
...
import { forkSessionFromParent, resolveParentForkMaxTokens, resolveParentForkTokenCount, } from "./session-fork.js";
...
function resolveStaleSessionEndReason(params) {
    ...
    const staleIdle = params.freshness.idleExpiresAt != null && params.now > params.freshness.idleExpiresAt;
    if (staleIdle) {
        return "idle";
    }
    if (staleDaily) {
...
export async function initSessionState(params) {
    ...
    const commandTargetSessionKey = ctx.CommandSource === "native"
    ...
    const targetSessionKey = commandTargetSessionKey ??
    ...
    if (ingressTimingEnabled) {
        log.info(`session-init store-load agent=${agentId} session=${sessionCtxForState.SessionKey ?? "(no-session)"} ` +
    ...
    const entryFreshness = entry
        ...
        : undefined;
    const softResetAllowed = softReset.matched &&
        resetAuthorized &&
        !isAcpSessionKey(resolveEffectiveResetTargetSessionKey({
            cfg,
    ...
    if (parentSessionKey &&
        ...
        const parentTokens = parentForkMaxTokens > 0
        ...
        if (parentForkMaxTokens > 0 &&
            ...
            log.warn(`skipping parent fork (parent too large): parentKey=${parentSessionKey} → sessionKey=${sessionKey} ` +
                `parentTokens=${parentTokens} maxTokens=${parentForkMaxTokens}`);
            sessionEntry.forkedFromParent = true;
        }
        else {
            log.warn(`forking from parent session: parentKey=${parentSessionKey} → sessionKey=${sessionKey} ` +
    ...
    const fallbackSessionFile = !sessionEntry.sessionFile
        ? resolveSessionTranscriptPath(sessionEntry.sessionId, agentId, ctx.MessageThreadId ?? threadIdFromSessionKey)
        : undefined;
