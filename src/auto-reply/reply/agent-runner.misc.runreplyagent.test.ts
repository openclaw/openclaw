...
describe("runReplyAgent billing error classification", () => {
  ...
  it("applies outbound transforms to final fallback payloads", async () => {
    runEmbeddedPiAgentMock.mockRejectedValueOnce(
      new Error("402 Payment Required: request token limit exceeded for this billing plan"),
    );
    hookRunnerMocks.getGlobalHookRunner.mockReturnValue({
      runOutboundTransforms: (text: string) => text.replace("billing error", "account issue"),
    });
    ...
    const payload = Array.isArray(result) ? result[0] : result;
    expect(payload?.text).toContain("account issue");
    expect(payload?.text).not.toContain("billing error");
  });
});
