...
describe("followup enqueue durable-admission failure", () => {
  ...
  it.each([
    ...
    "re-admits the same message id after a failed durable write $label",
    async ({ withLifecycle }) => {
      ...
      expect(await enqueueFollowupRun(key, first, settings)).toBe(false);
      ...
      expect(await enqueueFollowupRun(key, retry, settings)).toBe(true);
