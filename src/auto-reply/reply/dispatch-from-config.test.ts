...
describe("dispatchReplyFromConfig", () => {
  ...
  it("routes to primary channel when primary routing is configured (primary-only mode)", async () => {
    mocks.tryFastAbortFromMessage.mockResolvedValue({
      handled: false,
      aborted: false,
    });
    ...
    const replyResolver = async () => ({ text: "routed to telegram" }) satisfies ReplyPayload;
    await dispatchReplyFromConfig({ ctx, cfg, dispatcher, replyResolver });
  ...
  it("routes to both source and primary in mirror mode", async () => {
    mocks.tryFastAbortFromMessage.mockResolvedValue({
      handled: false,
      aborted: false,
    });
    ...
    const replyResolver = async () => ({ text: "mirrored" }) satisfies ReplyPayload;
    await dispatchReplyFromConfig({ ctx, cfg, dispatcher, replyResolver });
    ...
    expect(dispatcher.sendFinalReply).toHaveBeenCalledWith(
      expect.objectContaining({ text: "mirrored" }),
    );
  });

  it("does not route when message comes from primary channel", async () => {
    mocks.tryFastAbortFromMessage.mockResolvedValue({
      handled: false,
      aborted: false,
    });
    ...
    const replyResolver = async () => ({ text: "normal reply" }) satisfies ReplyPayload;
    await dispatchReplyFromConfig({ ctx, cfg, dispatcher, replyResolver });

    expect(mocks.routeReply).not.toHaveBeenCalled();
    expect(dispatcher.sendFinalReply).toHaveBeenCalledWith(
      expect.objectContaining({ text: "normal reply" }),
    );
  });
