...
describe("persistFollowupQueues tool-policy restore", () => {
  ...
  it("fail-closes restored items whose intersection lacks toolsAllow", async () => {
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "intersection-only-secret")).toBe(
        false,
      );
  ...
  it("fail-closes overflow summary sources whose intersection lacks toolsAllow", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
    ...
    expect(
      await followupQueueEntryContainsPrompt(TEST_KEY, "intersection-only-overflow-secret"),
    ).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "kept-item")).toBe(true);
  });

  it("fail-closes overflow elision sources whose intersection lacks toolsAllow", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
    ...
    expect(
      await followupQueueEntryContainsPrompt(TEST_KEY, "intersection-only-elision-secret"),
    ).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "live-item")).toBe(true);
  });
});
