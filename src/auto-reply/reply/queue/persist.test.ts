...
describe("persistFollowupQueues / restoreFollowupQueues", () => {
  ...
  it("writes shared SQLite state when a non-empty queue exists", async () => {
    ...
    const persisted = (await readPersistedQueueEntry(TEST_KEY)) as { items?: unknown[] };
    expect(Array.isArray(persisted?.items)).toBe(true);
  });

  it("clears shared SQLite rows when all restored queues are empty", async () => {
  ...
  it("retains durable rows this process has not restored", async () => {
    await replaceFollowupQueueEntries({
      entries: [
    ...
    expect((await listFollowupQueueKeys()).toSorted()).toEqual(
      ["other-process-key", TEST_KEY].toSorted(),
    );
  });

  it("keeps incognito queues out of the durable snapshot", async () => {
    const incognitoKey = "agent:main:dashboard:incognito-abc123";
    const queue = await getFollowupQueue(incognitoKey, SETTINGS);
    queue.items.push(makeFollowupRun("secret prompt"));
    await persistFollowupQueuesOrThrow();
    expect(await listFollowupQueueKeys()).not.toContain(incognitoKey);
    expect(await followupQueueEntryContainsPrompt(incognitoKey, "secret prompt")).toBe(false);
  });

  it("removes incognito rows leaked by an older build", async () => {
    ...
    expect(await listFollowupQueueKeys()).toContain(incognitoKey);
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("round-trips prompt and routing through persist+restore", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("round-trips reply-target and closed input provenance through persist+restore", async () => {
    const run = makeRun();
    run.inputProvenance = {
      kind: "external_user",
  ...
  it("round-trips media facts through persist+restore", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push({
  ...
  it("round-trips summarySources and summaryElisions through persist+restore", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("persistFollowupQueuesOrThrow rethrows write failures", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    try {
      await expect(persistFollowupQueuesOrThrow()).rejects.toThrow();
    } finally {
      process.env.OPENCLAW_STATE_DIR = previousStateDir;
    }
  });

  it("rejects enqueue and rolls back when persistence fails", async () => {
    const blocker = path.join(tmpDir, "not-a-directory");
    fs.writeFileSync(blocker, "file");
    const previousStateDir = process.env.OPENCLAW_STATE_DIR;
    process.env.OPENCLAW_STATE_DIR = path.join(blocker, "child");
  ...
  it("releases the message-id reservation when durable admission fails", async () => {
    const blocker = path.join(tmpDir, "not-a-directory");
    fs.writeFileSync(blocker, "file");
    const previousStateDir = process.env.OPENCLAW_STATE_DIR;
    const failedRun = {
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(blocker, "child");
    try {
      const admitted = await enqueueFollowupRun(TEST_KEY, failedRun, SETTINGS);
      expect(admitted).toBe(false);
      expect(FOLLOWUP_QUEUES.get(TEST_KEY)?.items ?? []).toEqual([]);
    } finally {
      process.env.OPENCLAW_STATE_DIR = previousStateDir;
    ...
    const retried = await enqueueFollowupRun(
      TEST_KEY,
  ...
  it("restores prior overflow mutations when admission persistence fails", async () => {
    ...
    expect(await enqueueFollowupRun(TEST_KEY, kept, tight, "none")).toBe(true);
  ...
  it("defers summary-elision lifecycle completion until admission persistence succeeds", async () => {
    const tight: QueueSettings = {
      mode: "steer",
      debounceMs: 0,
      cap: 1,
    ...
    expect(await enqueueFollowupRun(TEST_KEY, makeTracked("first"), tight, "none")).toBe(true);
    expect(await enqueueFollowupRun(TEST_KEY, makeTracked("second"), tight, "none")).toBe(true);
    expect(await enqueueFollowupRun(TEST_KEY, makeTracked("third"), tight, "none")).toBe(true);
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(blocker, "child");
    try {
      const admitted = await enqueueFollowupRun(TEST_KEY, makeTracked("fourth"), tight, "none");
      expect(admitted).toBe(false);
    } finally {
      process.env.OPENCLAW_STATE_DIR = previousStateDir;
    }
  ...
  it("keeps in-flight items in SQLite until settle-after-success", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "in flight")).toBe(true);
  });

  it("keeps admitted in-flight items after they leave the pending list", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "admitted aggregate source")).toBe(
      true,
    );
  });

  it("clears a restored-drain marker with its queue", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    expect(await clearFollowupQueue(TEST_KEY)).toBe(1);

    expect(peekRestoredPendingDrainKeys().has(TEST_KEY)).toBe(false);
  });

  it("omits current inbound prompt context from persist+restore", async () => {
    ...
    await persistFollowupQueues();
    expect(JSON.stringify((await readPersistedQueueEntry(TEST_KEY)) ?? {})).not.toContain(
      "Current event:",
    );
    FOLLOWUP_QUEUES.delete(TEST_KEY);
    clearFollowupQueuesRestoredFlagForTest();
  ...
  it("omits quoted inbound context and keeps the audio flag through persist+restore", async () => {
    const inboundContext = {
      text: "Quoted:\n> prior message\n\nreplying in thread",
      promptJoiner: "\n\n" as const,
    };
  ...
  it("persists cap-driven elision trimming before a rejected enqueue survives restart", async () => {
    ...
    const queue = await getFollowupQueue(TEST_KEY, wide);
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "oldest")).toBe(true);

    expect(
      await enqueueFollowupRun(
        TEST_KEY,
        { ...makeFollowupRun("live-turn"), messageId: "live-1" },
        tight,
      ),
    ).toBe(false);
    ...
    const persistedAfterReject = (await readPersistedQueueEntry(TEST_KEY)) as {
  ...
  it("round-trips chat delivery identity and execution context through persist+restore", async () => {
    const run = makeRun();
    run.chatType = "direct";
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "nested-static-do-not-leak")).toBe(
      false,
    );
    expect(JSON.stringify((await readPersistedQueueEntry(TEST_KEY)) ?? {})).not.toContain(
  ...
  it("strips nested extraSystemPromptStatic from legacy cliSessionBindingFacts on restore", async () => {
    await replaceFollowupQueueEntries({
      entries: [
        [
          TEST_KEY,
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "LEGACY-CLI-PROMPT-SENTINEL")).toBe(
      true,
    );
  ...
  it("skips malformed persisted queue rows during restore", async () => {
    await replaceFollowupQueueEntries({
      entries: [[TEST_KEY, { items: [{ prompt: 7, run: {} }], mode: "steer" }]],
    });

    await expect(restoreFollowupQueues()).resolves.not.toThrow();
    expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();
  });

  it("round-trips bare session-reset transcript without current-turn context", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push({
  ...
  it("does not restore abortSignal (runtime-only field stripped on persist)", async () => {
  ...
  it("is a no-op when no persisted queue rows exist", async () => {
    expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();
    await expect(restoreFollowupQueues()).resolves.not.toThrow();
    expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();
  });

  it("guards restore against split-module re-evaluation (restore-once)", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("retries restore after a transient SQLite initialization failure", async () => {
  ...
  it("notifies the restored-queue listener after a successful restore retry", async () => {
    vi.useFakeTimers();
  ...
  it("retains unreadable SQLite rows across ordinary persist mutations", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    expect(await listFollowupQueueKeys()).toEqual(
      expect.arrayContaining(["agent:main:dm:corrupt-sqlite", TEST_KEY]),
    );
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "live-queue")).toBe(true);
    expect((await loadFollowupQueueEntries()).map(([key]) => key)).toEqual([TEST_KEY]);
  });

  it("resumes restore after the flag is explicitly cleared (test hook)", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("registers restored keys in the pending-drain set when queue has items", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("does not add to pending-drain set when restored queue is empty", async () => {
    await replaceFollowupQueueEntries({
      entries: [
        [
          TEST_KEY,
  ...
  it("registers summary-only restored queues in the pending-drain set", async () => {
  ...
  it("persists queue data in shared SQLite state", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push(makeFollowupRun("private"));
    await persistFollowupQueues();

    expect(fs.existsSync(resolveOpenClawStateSqlitePath())).toBe(true);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "private")).toBe(true);
  });

  it("strips secret-bearing runtime fields but persists auth and reply context selectors", async () => {
    const run = makeRun();
    run.config = {
    ...
    const entries = await loadFollowupQueueEntries();
  ...
  it("does not persist sensitive delivery grouping fields after real overflow", async () => {
    ...
    for (const [index, secret] of ["alpha-secret", "beta-secret", "gamma-secret"].entries()) {
      ...
      expect(await enqueueFollowupRun(TEST_KEY, item, settings, "none")).toBe(true);
    }

    const raw = JSON.stringify(await loadFollowupQueueEntries());
  ...
  it("round-trips auth profile selection through restore", async () => {
    const run = makeRun();
  ...
  it("round-trips session tool overrides and prepared model-routing facts", async () => {
    const run = makeRun();
    run.toolOverrides = {
      webSearch: false,
