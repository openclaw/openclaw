...
describe("persistFollowupQueues / restoreFollowupQueues", () => {
  ...
  it("restores persisted summary counters but resets runtime-only elision state", () => {
    replaceFollowupQueueEntries({
      entries: [
        [
          TEST_KEY,
          {
            items: [{ ...makeFollowupRun("ok"), run: makeRun() }],
            mode: "steer",
            droppedCount: 0,
            summaryLines: ["Earlier messages summarized"],
            evictedSummaryCount: 3,
            lastEnqueuedAt: 1,
          },
        ],
      ],
    ...
    restoreFollowupQueues();
    const restored = FOLLOWUP_QUEUES.get(TEST_KEY);
    expect(restored?.summaryLines).toEqual(["Earlier messages summarized"]);
