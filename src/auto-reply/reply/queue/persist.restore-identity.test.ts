...
describe("persistFollowupQueues restore identity", () => {
  ...
  it("does not persist sender-bound turns, raw channel identities, or owner privilege", async () => {
  ...
  it("marks restored identity-less collect entries for individual draining", async () => {
    const collectSettings = { ...SETTINGS, mode: "collect" as const, debounceMs: 0 };
    const queue = await getFollowupQueue(TEST_KEY, collectSettings);
  ...
  it("does not restore persisted exec elevation or exec overrides", async () => {
    const run = makeRun();
  ...
  it("fences exec authority even if an older SQLite row still serialized it", async () => {
  ...
  it("does not restore client-bound caps, tool bindings, or approval device ids", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
  ...
  it("fail-closes role-dependent follow-ups instead of draining without member roles", async () => {
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "discord-role-gated")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "telegram-plain")).toBe(true);
    expect(peekRestoredPendingDrainKeys().has(TEST_KEY)).toBe(true);
  });

  it("drops a leading role-dependent overflow source with its summary line", async () => {
    const validRun = makeRun();
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "role-gated-overflow-text")).toBe(
      false,
    );
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "safe-overflow-text")).toBe(true);
  });

  it("does not wake a queue that only contained role-dependent work", async () => {
    const roleRun = makeRun();
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "role-only")).toBe(true);

    await restorePersistedQueueForTest();

    expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();
    expect(peekRestoredPendingDrainKeys().has(TEST_KEY)).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "role-only")).toBe(false);
    expect(await hasPersistedFollowupQueues()).toBe(false);
  });

  it("fail-closes legacy memberRoleIds rows that lack the roleDependent marker", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
    ...
    expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();
    expect(peekRestoredPendingDrainKeys().has(TEST_KEY)).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "legacy-role-row")).toBe(false);
  });

  it("fail-closes canceled follow-ups instead of replaying them after restart", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push({
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "canceled-after-ack-failure")).toBe(
      true,
    );
  ...
  it("fail-closes delivered follow-ups instead of replaying them after restart", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push({
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "delivered-before-omit-ack")).toBe(
      true,
    );
  ...
  it("fail-closes canceled overflow summary sources instead of replaying them after restart", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "canceled-overflow-source")).toBe(true);

    await restorePersistedQueueForTest();

    expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();
    expect(peekRestoredPendingDrainKeys().has(TEST_KEY)).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "canceled-overflow-source")).toBe(
      false,
    );
  });

  it("fail-closes canceled overflow elision sources instead of replaying them after restart", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.droppedCount = 1;
    queue.summaryLines = [];
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "canceled-overflow-elision")).toBe(
      true,
    );
  ...
  it("does not log queued prompts when fail-closing canceled follow-ups", async () => {
  ...
  it("does not log queued prompts when fail-closing role-dependent follow-ups", async () => {
  ...
  it("round-trips terminal reply expectation without restoring delegated authority", async () => {
    const run = makeRun();
  ...
  it("fail-closes delegated-authority follow-ups instead of restoring copied claims", async () => {
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "handoff-turn")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "grant-turn")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "ordinary-turn")).toBe(true);
  });

  it("fail-closes restored follow-ups whose trusted handoff or plugin grant is invalid", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "invalid-handoff")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "invalid-grant")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "invalid-expectation")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "invalid-provenance")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "marked-delegated")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "ordinary-turn")).toBe(true);
  });

  it("strips invalid closed envelopes from restored lastRun instead of leaking them", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
  ...
  it("strips an incomplete session permission pair from restored lastRun instead of leaking it", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
  ...
  it("strips raw source-session provenance from restored rows and rewrites SQLite", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
  ...
  it("fail-closes legacy rows that carry raw channel identity and scrubs them from SQLite", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
    ...
    expect(restored?.lastRun?.senderId).toBeUndefined();
    expect(restored?.lastRun?.channelContext).toBeUndefined();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "legacy-identity")).toBe(false);
  ...
  it("does not persist or restore handoff-derived source-session provenance", async () => {
    const run = makeRun();
    run.inputProvenance = {
      kind: "inter_session",
  ...
  it("strips currentInboundContext from restored rows and rewrites SQLite", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
