...
describe("persistFollowupQueues restore guards", () => {
  ...
  it("round-trips toolsAllow, disableTools, and allowlist intersection across restore", async () => {
  ...
  it("round-trips explicit skill selections across restore", async () => {
  ...
  it("fail-closes follow-ups whose explicit skill selections cannot be revalidated", async () => {
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "invalid-skill-selection")).toBe(
        false,
      );
  ...
  it("never persists inline image payloads and fail-closes the marked follow-up on restore", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push({
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "with-inline-image")).toBe(false);
      const logged = errorSpy.mock.calls
        .map((call) => (typeof call[0] === "string" ? call[0] : ""))
        .join("\n");
  ...
  it("fail-closes legacy rows that still carry raw image payloads and scrubs them from SQLite", async () => {
    await replaceFollowupQueueEntries({
      entries: [
        [
          TEST_KEY,
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "legacy-image-row")).toBe(false);
      const persisted = await readPersistedQueueEntry(TEST_KEY);
  ...
  it("fail-closes restored follow-ups older than the bounded retention window", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push({
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "stale-followup")).toBe(false);
      const logged = errorSpy.mock.calls
        .map((call) => (typeof call[0] === "string" ? call[0] : ""))
        .join("\n");
  ...
  it("fail-closes follow-ups whose selected skill is gone after restart", async () => {
    const workspaceDir = path.join(tmpDir, "workspace");
    const skillPath = writeWorkspaceSkill(workspaceDir, "calendar");
    setRuntimeConfigSnapshot({} as OpenClawConfig);
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "moved-skill-selection")).toBe(true);
    ...
    try {
      ...
      expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();
      expect(peekRestoredPendingDrainKeys().has(TEST_KEY)).toBe(false);
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "moved-skill-selection")).toBe(false);
      const logged = errorSpy.mock.calls
        .map((call) => (typeof call[0] === "string" ? call[0] : ""))
        .join("\n");
      expect(logged).toContain(TEST_KEY);
  ...
  it("fail-closes follow-ups whose selected skill is disabled after restart", async () => {
    const workspaceDir = path.join(tmpDir, "workspace");
    const skillPath = writeWorkspaceSkill(workspaceDir, "calendar");
    setRuntimeConfigSnapshot({} as OpenClawConfig);
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    await persistFollowupQueues();
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "disabled-skill-selection")).toBe(true);
    ...
    try {
      ...
      expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();
      expect(peekRestoredPendingDrainKeys().has(TEST_KEY)).toBe(false);
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "disabled-skill-selection")).toBe(
        false,
      );
      const logged = errorSpy.mock.calls
        .map((call) => (typeof call[0] === "string" ? call[0] : ""))
  ...
  it("does not restore an accepted steer after consume settles to SQLite", async () => {
    const followup = makeFollowupRun("accepted-steer");
    const parked = await parkSteerCandidate(TEST_KEY, followup, SETTINGS, async () => undefined);
    expect(parked).toBeDefined();
    expect(FOLLOWUP_QUEUES.get(TEST_KEY)?.items).toHaveLength(1);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "accepted-steer")).toBe(true);
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "accepted-steer")).toBe(false);
  ...
  it("persists the original queue cap after deferred steer overflow fallback", async () => {
    ...
    const parked = await parkSteerCandidate(
      TEST_KEY,
    ...
    expect(parked).toBeDefined();
    expect(await enqueueFollowupRun(TEST_KEY, makeFollowupRun("suffix-a"), tight, "none")).toBe(
      true,
    );
    expect(await enqueueFollowupRun(TEST_KEY, makeFollowupRun("suffix-b"), tight, "none")).toBe(
      true,
    );
    ...
    const persisted = (await readPersistedQueueEntry(TEST_KEY)) as { cap?: number };
  ...
  it("rolls back deferred overflow when SQLite persistence fails", async () => {
    const tight: QueueSettings = {
      mode: "steer",
      debounceMs: 500,
      cap: 1,
    ...
    expect(parked).toBeDefined();
    expect(await enqueueFollowupRun(TEST_KEY, makeTracked("suffix-a"), tight, "none")).toBe(true);
    expect(await enqueueFollowupRun(TEST_KEY, makeTracked("suffix-b"), tight, "none")).toBe(true);
    expect(FOLLOWUP_QUEUES.get(TEST_KEY)?.items.map((item) => item.prompt)).toEqual([
      "steer-anchor",
      "suffix-a",
      "suffix-b",
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "suffix-a")).toBe(true);
      expect(await followupQueueEntryContainsPrompt(TEST_KEY, "suffix-b")).toBe(true);
  ...
  it("round-trips scheduledToolPolicy through persist+restore", async () => {
    const run = makeRun();
  ...
  it("fail-closes restored follow-ups whose scheduledToolPolicy cannot be normalized", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "invalid-scheduled-policy")).toBe(
      false,
    );
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "ordinary-turn")).toBe(true);
  });

  it("round-trips session permission mode and root through persist+restore", async () => {
    const run = makeRun();
  ...
  it("persists a root-only live session policy so restore fail-closes the half-pair", async () => {
    const run = makeRun();
    run.sessionRoot = "/tmp/workspace/project";
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push({
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "root-only-live")).toBe(false);
  });

  it("fail-closes restored follow-ups whose session permission pair is incomplete or invalid", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "mode-without-root")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "root-without-mode")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "invalid-mode")).toBe(false);
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "ordinary-turn")).toBe(true);
  });

  it("round-trips Skill Workshop revision constraints through persist+restore", async () => {
    const revision = {
      agentId: "main",
  ...
  it("fail-closes restored follow-ups whose Skill Workshop revision constraint is invalid", async () => {
    const validRun = makeRun();
    await replaceFollowupQueueEntries({
      entries: [
        [
    ...
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "invalid-workshop-revision")).toBe(
      false,
    );
    expect(await followupQueueEntryContainsPrompt(TEST_KEY, "ordinary-turn")).toBe(true);
  });
  ...
  it("restores Skill Workshop revision turns as individually drained and rejects a stale hash", async () => {
    ...
    expect(
      await enqueueFollowupRun(TEST_KEY, ordinary, drainSettings, "none", undefined, false),
    ).toBe(true);
    expect(
      await enqueueFollowupRun(TEST_KEY, workshop, drainSettings, "none", undefined, false),
    ).toBe(true);
  ...
  it("shares restored pending drain keys with later bundled runtime copies", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("drops overflow elision groups whose sources fail restore validation", async () => {
    const validRun = {
      agentId: "main",
  ...
  it("maps isolated heartbeat session keys back to restored queue keys", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("defers restored drain until the active reply owner clears", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
  ...
  it("reconnects restored items and overflow sources to queue cancellation", async () => {
    const queue = await getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    await clearFollowupQueue(TEST_KEY);
  ...
  it("rehydrates run.config from the live runtime snapshot on restore", async () => {
    const liveConfig = {
  ...
  it("picks up a refreshed snapshot across separate restore passes", async () => {
  ...
  it("skips entries with missing or invalid items array", async () => {
    await replaceFollowupQueueEntries({
      entries: [
  ...
  it("skips restored items with mismatched sessionKey or incomplete originating routes", async () => {
