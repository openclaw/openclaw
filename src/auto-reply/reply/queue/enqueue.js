...
function isRunAlreadyQueued(run, items, allowPromptFallback = false) {
    ...
    const messageId = normalizeOptionalString(run.messageId);
    if (messageId) {
        return items.some((item) => normalizeOptionalString(item.messageId) === messageId && hasSameRouting(item));
    }
...
export function enqueueFollowupRun(key, run, settings, dedupeMode = "message-id", runFollowup, restartIfIdle = true) {
...
export function resetRecentQueuedMessageIdDedupe() {
