...
describe("followup queue restart round-trip (real shared SQLite state)", () => {
  ...
  it("persists a mid-turn Telegram followup, restores it after a simulated restart, redelivers exactly once with routing intact, and does not replay after a second restart", async () => {
    ...
    try {
      ...
      expect(await hasPersistedFollowupQueues()).toBe(true);
      expect(await followupQueueEntryContainsPrompt(key, "summarize the thread so far")).toBe(true);
      ...
      rememberFollowupDrainCallback(key, async (run) => {
        persistedDuringSend = await followupQueueEntryContainsPrompt(key, run.prompt);
      ...
      const keyDrainedOnDisk = async () => {
        if (!(await hasFollowupQueueEntries())) {
          return true;
        }
        const entry = (await loadFollowupQueueEntries()).find(
          ([entryKey]) => entryKey === key,
        )?.[1] as { items?: unknown[] } | undefined;
        return !entry || (entry.items?.length ?? 0) === 0;
      };
      const deadline = Date.now() + 5_000;
      while (!(await keyDrainedOnDisk()) && Date.now() < deadline) {
  ...
  it("drains restored collect entries individually after a restart", async () => {
    ...
    const key = "agent:main:telegram:group:collect-restore";
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(key, "first queued turn")).toBe(true);
      expect(await followupQueueEntryContainsPrompt(key, "second queued turn")).toBe(true);

      simulateGatewayRestart();
      await restoreFollowupQueues();
      const restored = FOLLOWUP_QUEUES.get(key);
      expect(restored?.items).toHaveLength(2);
      expect(restored?.items.every((item) => item.disableCollectBatching === true)).toBe(true);
      expect(restored?.items.every((item) => item.run.senderId === undefined)).toBe(true);

      const deliveries: FollowupRun[] = [];
      rememberFollowupDrainCallback(key, async (run) => {
        deliveries.push(run);
      });
      kickFollowupDrainIfIdle(key);
