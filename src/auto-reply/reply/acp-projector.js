...
import { isAcpTagVisible, resolveAcpProjectionSettings, resolveAcpStreamingConfig, } from "./acp-stream-settings.js";
import { createBlockReplyPipeline } from "./block-reply-pipeline.js";
const ACP_BLOCK_REPLY_TIMEOUT_MS = 15000;
...
export function createAcpReplyProjector(params) {
    ...
    const toolLifecycleById = new Map();
    ...
    const emitTruncationNotice = async () => {
        ...
        await emitSystemStatus("output truncated", {
    ...
    const onEvent = async (event) => {
        ...
        if (event.type === "status") {
            ...
            if (event.tag === "usage_update" && settings.repeatSuppression) {
                const usageTuple = typeof event.used === "number" && typeof event.size === "number"
