...
import { resolveAgentIdFromSessionKey, resolveFreshSessionTotalTokens, resolveSessionFilePath, resolveSessionFilePathOptions, updateSessionStoreEntry, } from "../../config/sessions.js";
import { readSessionMessages } from "../../gateway/session-utils.fs.js";
import { logVerbose } from "../../globals.js";
import { registerAgentRunContext } from "../../infra/agent-events.js";
import { resolveMemoryFlushPlan } from "../../plugins/memory-state.js";
import { normalizeOptionalString } from "../../shared/string-coerce.js";
import { buildEmbeddedRunExecutionParams, resolveModelFallbackOptions, } from "./agent-runner-utils.js";
import { hasAlreadyFlushedForCurrentCompaction, resolveMemoryFlushContextWindowTokens, shouldRunMemoryFlush, shouldRunPreflightCompaction, } from "./memory-flush.js";
...
export function setAgentRunnerMemoryTestDeps(overrides) {
...
export function estimatePromptTokensForMemoryFlush(prompt) {
    const trimmed = normalizeOptionalString(prompt);
    if (!trimmed) {
        return undefined;
    }
    const message = { role: "user", content: trimmed, timestamp: Date.now() };
...
export function resolveEffectivePromptTokens(basePromptTokens, lastOutputTokens, promptTokenEstimate) {
...
function deriveTranscriptUsageSnapshot(usage) {
    ...
    const outputTokens = typeof outputRaw === "number" && Number.isFinite(outputRaw) && outputRaw > 0
        ? outputRaw
        : undefined;
...
export async function readPromptTokensFromSessionLog(sessionId, sessionEntry, sessionKey, opts) {
...
export async function runPreflightCompactionIfNeeded(params) {
    ...
    const hasPersistedTotalTokens = typeof persistedTotalTokens === "number" &&
    ...
    const transcriptPromptTokens = typeof freshPersistedTokens === "number"
        ? undefined
    ...
    const sessionFile = resolveSessionLogPath(entry.sessionId, entry.sessionFile ? entry : { ...entry, sessionFile: params.followupRun.run.sessionFile }, params.sessionKey ?? params.followupRun.run.sessionKey, { storePath: params.storePath });
    const result = await memoryDeps.compactEmbeddedPiSession({
        ...
        agentHarnessId: entry.sessionId === params.followupRun.run.sessionId ? entry.agentHarnessId : undefined,
...
export async function runMemoryFlushIfNeeded(params) {
    ...
    const persistedPromptTokens = typeof persistedPromptTokensRaw === "number" &&
    ...
    const projectedTokenCount = hasFreshPromptTokensSnapshot
        ? resolveEffectivePromptTokens(promptTokensSnapshot, transcriptOutputTokens, promptTokenEstimate)
        : undefined;
    ...
    logVerbose(`memoryFlush check: sessionKey=${params.sessionKey} ` +
    ...
    const shouldFlushMemory = (memoryFlushWritable &&
        !params.isHeartbeat &&
        !isCli &&
