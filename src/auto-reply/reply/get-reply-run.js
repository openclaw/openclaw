...
import { formatThinkingLevels, isThinkingLevelSupported, normalizeThinkLevel, resolveSupportedThinkingLevel, } from "../thinking.js";
...
export function buildExecOverridePromptHint(params) {
    ...
    const fullAccessLine = params.fullAccessAvailable === false
...
function hasInboundHistoryBody(ctx) {
    return (Array.isArray(ctx.InboundHistory) &&
...
export async function runPreparedReply(params) {
    ...
    const directChatContext = sessionCtx.ChatType === "direct" || sessionCtx.ChatType === "dm"
    ...
    const inboundMetaPrompt = buildInboundMetaSystemPrompt(isNewSession ? sessionCtx : { ...sessionCtx, ThreadStarterBody: undefined }, { includeFormattingHints: !useFastReplyRuntime });
    ...
    const isBareSessionReset = softResetTriggered ||
        (isNewSession &&
    ...
    const inboundUserContext = buildInboundUserContextPrefix(isNewSession
        ...
        : { ...sessionCtx, ThreadStarterBody: undefined }, envelopeOptions);
    ...
    const resolvePreparedSessionState = () => {
        const latestSessionEntry = sessionStore && sessionKey
    ...
    if (isActive && activeRunQueueAction === "run-now") {
        const queueState = await resolvePreparedReplyQueueState({
            ...
            waitForActiveRunEnd: (activeRunSessionId) => piRuntime?.waitForEmbeddedPiRunEnd(activeRunSessionId) ?? Promise.resolve(undefined),
    ...
    const followupRun = {
        ...
        run: {
            ...
            groupChannel: normalizeOptionalString(sessionCtx.GroupChannel) ??
            ...
            traceAuthorized: (forceSenderIsOwnerFalseFromSystemEvents ? false : command.senderIsOwner) ||
