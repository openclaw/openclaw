...
import { markSessionActive, markSessionInactive } from "../../config/sessions/active-sessions.js";
import { createSessionCheckpoint } from "../../config/sessions/checkpoint.js";
...
export async function runAgentTurnWithFallback(params: {
  ...
  // Mark session as active to prevent GC from deleting it
  if (params.sessionKey) {
    const agentId = resolveAgentIdFromSessionKey(params.followupRun.run.sessionKey);
    markSessionActive(params.sessionKey, agentId);
  ...
  try {
    while (true) {
      try {
        ...
        if (
          ...
          try {
            // Create checkpoint before deleting transcript (enables recovery)
            const agentId = resolveAgentIdFromSessionKey(params.followupRun.run.sessionKey);
            if (sessionKey && sessionEntry) {
              try {
                await createSessionCheckpoint(agentId, sessionKey, sessionEntry, "pre-reset");
                defaultRuntime.log?.(`Created checkpoint for session ${sessionKey} before reset`);
              } catch (checkpointErr) {
                defaultRuntime.error(
                  `Failed to create checkpoint before reset: ${String(checkpointErr)}`,
                );
                // Continue with reset even if checkpoint fails
              }
            }

            // Record session reset metric
            recordSessionReset(agentId);
    ...
    // Mark session as inactive when agent finishes (regardless of success/failure)
    if (params.sessionKey) {
      markSessionInactive(params.sessionKey);
    }
  }
}
