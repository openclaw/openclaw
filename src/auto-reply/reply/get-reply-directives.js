...
import { reserveSkillCommandNames, resolveConfiguredDirectiveAliases, } from "./get-reply-directive-aliases.js";
...
import { createFastTestModelSelectionState, createModelSelectionState, resolveContextTokens, } from "./model-selection.js";
...
function resolveDirectiveCommandText(params) {
    const commandSource = params.sessionCtx.BodyForCommands ??
        ...
        params.ctx.RawBody ??
        "";
    const promptSource = params.sessionCtx.BodyForAgent ??
        params.sessionCtx.BodyStripped ??
        params.sessionCtx.Body ??
        "";
    return {
...
export async function resolveReplyDirectives(params) {
    ...
    const hasConfiguredModelAliases = commandTextHasSlash &&
        Object.values(cfg.agents?.defaults?.models ?? {}).some((entry) => Boolean(normalizeOptionalString(entry.alias)));
    ...
    const resolvedVerboseLevel = directives.verboseLevel ??
        targetSessionEntry?.verboseLevel ??
    ...
    const useFastModelSelection = useFastReplyRuntime &&
    ...
    const thinkingExplicitlySet = directives.thinkLevel !== undefined ||
    ...
    const isModelListAlias = directives.hasModelDirective &&
        ["status", "list"].includes(normalizeLowercaseStringOrEmpty(normalizeOptionalString(directives.rawModelDirective)));
