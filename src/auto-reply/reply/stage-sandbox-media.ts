...
export async function stageSandboxMedia(params: {
  ...
  for (const raw of rawPaths) {
    ...
    // Use path.posix for sandbox-relative paths (sandbox paths are always POSIX).
    const inboundSubdirPosix = ctx.AccountId
      ? path.posix.join("inbound", ctx.AccountId)
      : "inbound";
    const relativeDest = sandbox
      ? path.join("media", ...inboundSubdirPosix.split("/"), fileName)
      : fileName;
    ...
    const stagedPath = sandbox ? path.posix.join("media", inboundSubdirPosix, fileName) : dest;
