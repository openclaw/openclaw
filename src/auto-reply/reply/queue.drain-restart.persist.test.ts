...
describe("followup queue drain restart persistence", () => {
  ...
  it("persists the queue after dropAbortedFollowups removes an aborted item", async () => {
    ...
    try {
      ...
      expect(await hasFollowupQueueEntries()).toBe(true);
      expect(await followupQueueEntryContainsPrompt(key, "aborted item")).toBe(true);
      ...
      expect(await followupQueueEntryContainsPrompt(key, "aborted item")).toBe(false);
  ...
  it("retains canceled tombstones when acknowledgement fails and does not execute them after restart", async () => {
    ...
    const replaceSpy = vi
      ...
      .mockImplementation(async (params) => {
        ...
        if (!hasAborted && retainedEntries === undefined) {
          retainedEntries = await followupQueueSqlite.loadFollowupQueueEntries();
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(key, abortedPrompt)).toBe(true);
      aborter.abort();
      ...
      await originalReplace({ entries: retainedEntries ?? [] });
      FOLLOWUP_QUEUES.delete(key);
      clearRestoredPendingDrainKeysForTest();
      clearFollowupQueuesRestoredFlagForTest();
      await restoreFollowupQueues();
  ...
  it("persists canceled overflow sources before dropping them", async () => {
    ...
    try {
      const aborter = new AbortController();
      const queue = await getFollowupQueue(key, settings);
      const source = {
        ...createRun({
          prompt: abortedPrompt,
          originatingChannel: "telegram",
      ...
      await vi.waitFor(async () => {
        expect(await followupQueueEntryContainsPrompt(key, abortedPrompt)).toBe(false);
      });
  ...
  it("keeps the durable row when an orderly restart retires the queue", async () => {
    ...
    try {
      const queue = await getFollowupQueue(key, { mode: "followup", debounceMs: 0, cap: 50 });
      ...
      const other = await getFollowupQueue(otherKey, { mode: "followup", debounceMs: 0, cap: 50 });
