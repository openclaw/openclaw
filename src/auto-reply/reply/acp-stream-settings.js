...
const DEFAULT_ACP_MAX_OUTPUT_CHARS = 24000;
const DEFAULT_ACP_MAX_SESSION_UPDATE_CHARS = 320;
export const ACP_TAG_VISIBILITY_DEFAULTS = {
...
export function resolveAcpProjectionSettings(cfg) {
    const stream = cfg.acp?.stream;
    const deliveryMode = resolveAcpDeliveryMode(stream?.deliveryMode);
    const hiddenBoundaryFallback = deliveryMode === "live"
...
export function isAcpTagVisible(settings, tag) {
    if (!tag) {
        return true;
    }
