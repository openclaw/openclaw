...
import { loadSessionStore, resolveSessionStoreEntry, resolveStorePath, updateSessionStore, } from "../../config/sessions.js";
...
import { applyAbortCutoffToSessionEntry, resolveAbortCutoffFromContext, shouldPersistAbortCutoff, } from "./abort-cutoff.js";
...
export function stopSubagentsForRequester(params) {
    ...
    const dedupedRunsByChildKey = new Map();
    for (const run of abortDeps.listSubagentRunsForController(requesterKey)) {
        ...
        const latestControllerSessionKey = normalizeOptionalString(latest?.controllerSessionKey) ??
    ...
    for (const run of runs) {
        ...
        if (!run.endedAt) {
            ...
            const aborted = (childKey ? replyRunRegistry.abort(childKey) : false) ||
                (sessionId ? abortDeps.abortEmbeddedPiRun(sessionId) : false);
            const markedTerminated = abortDeps.markSubagentRunTerminated({
                runId: run.runId,
...
export async function tryFastAbortFromMessage(params) {
    ...
    const stripped = isGroup
        ? stripMentions(raw, ctx, cfg, resolveSessionAgentId({
    ...
    if (targetKey) {
        ...
        const aborted = replyRunRegistry.abort(resolvedTargetKey) ||
            (sessionId ? abortDeps.abortEmbeddedPiRun(sessionId) : false);
