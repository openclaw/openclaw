...
describe("followup queue restart exec policy", () => {
  ...
  it("restores a live restrictive exec overlay after persist+restore", async () => {
    ...
    try {
      ...
      expect(await followupQueueEntryContainsPrompt(key, "stay denied after restart")).toBe(true);
