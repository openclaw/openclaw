...
describe("buildInboundUserContextPrefix", () => {
  it("omits conversation label block for direct chats", () => {
    const result = buildInboundUserContextPrefix({
      ChatType: "direct",
  ...
  it("includes message identifiers for direct external-channel chats", () => {
    ...
    const conversationInfo = parseConversationInfoPayload(result.text);
  ...
  it("keeps conversation label for group chats", () => {
    ...
    expect(result.text).toContain("Conversation info (untrusted metadata):");
    expect(result.text).toContain('"conversation_label": "ops-room"');
  });
  ...
  it("includes sender metadata block for direct chats", () => {
    ...
    const senderInfo = parseSenderInfoPayload(result.text);
