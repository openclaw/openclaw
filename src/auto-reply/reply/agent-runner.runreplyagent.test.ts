...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { BOT_STATE_DIR: process.env.BOT_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.BOT_STATE_DIR === undefined) {
      delete process.env.BOT_STATE_DIR;
    } else {
      process.env.BOT_STATE_DIR = snapshot.BOT_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "bot-typing-heartbeat-"));
  });
