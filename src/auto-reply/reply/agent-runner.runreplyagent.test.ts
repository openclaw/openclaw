...
beforeEach(() => {
  ...
  vi.stubEnv("SMART_AGENT_NEO_TEST_FAST", "1");
});
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { SMART_AGENT_NEO_STATE_DIR: process.env.SMART_AGENT_NEO_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.SMART_AGENT_NEO_STATE_DIR === undefined) {
      delete process.env.SMART_AGENT_NEO_STATE_DIR;
    } else {
      process.env.SMART_AGENT_NEO_STATE_DIR = snapshot.SMART_AGENT_NEO_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "smart-agent-neo-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "smart-agent-neo-memory-flush-"));
  });
