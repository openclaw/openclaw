...
function resolveDiscordChannelEntry(channels, params) {
    ...
    return ((groupId ? channels[groupId] : undefined) ??
...
export async function resolveGroupRequireMention(params) {
    ...
    const groupId = groupResolution?.id ?? extractExplicitGroupId(rawGroupId) ?? (rawGroupId || undefined);
...
export function buildDirectChatContext(params) {
    ...
    if (params.silentReplyPolicy === "allow") {
        ...
        lines.push(`If no response is needed, reply with exactly "${params.silentToken}" (and nothing else) so OpenClaw can send a short fallback reply.`);
    }
    else {
...
export function buildGroupIntro(params) {
    ...
    const silenceLine = activation === "always" && canUseSilentReply
        ...
        : undefined;
    const toolSilenceLine = activation === "always" && canUseSilentReply
        ...
        : undefined;
    const cautionLine = activation === "always" && params.silentReplyPolicy === "allow"
    ...
    const styleLine = "Write like a human. Avoid Markdown tables. Minimize empty lines and use normal chat conventions, not document-style spacing. Don't type literal \\n sequences; use real line breaks sparingly.";
