...
import { scheduleGatewaySigusr1Restart, triggerMullusiRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Mullusi in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMullusiRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Mullusi via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
