...
export const handleTotalUsageCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  const args =
    normalized === "/total_usage"
      ? []
      : normalized.slice("/total_usage".length).trim().split(/\s+/).filter(Boolean);
  ...
  if (rangeStr === "all") {
    ...
    if (match) {
      ...
      if (unit === "h") {
        ...
        label = `Last ${value} hour${value === 1 ? "" : "s"}`;
      } else {
        durationMs = value * 24 * 60 * 60 * 1000;
        label = `Last ${value} day${value === 1 ? "" : "s"}`;
      }
    }
  }
  ...
  if (isByModel) {
    const modelLines = summary.models
      .map((modelSummary) => {
        const cost = formatUsd(modelSummary.totalCost);
        const tokens = formatTokenCount(modelSummary.totalTokens);
        return `🤖 *${modelSummary.model}*\n   ├ Usage: \`${tokens}\` tokens\n   └ Cost: \`${cost}\` (${modelSummary.sessionCount} session${modelSummary.sessionCount === 1 ? "" : "s"})`;
      })
      .slice(0, 15)
      .join("\n\n");
  ...
  const modelDistributionLines = topModels
    .map((modelSummary) => {
      const percent =
        summary.totals.totalTokens > 0
          ? ((modelSummary.totalTokens / summary.totals.totalTokens) * 100).toFixed(1)
          : "0.0";
      const tokens = formatTokenCount(modelSummary.totalTokens);
      const namePart = modelSummary.model.padEnd(maxModelNameLen, " ");
