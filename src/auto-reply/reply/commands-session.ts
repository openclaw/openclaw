...
import { scheduleGatewaySigusr1Restart, triggerActiviRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Activi in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerActiviRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Activi via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
