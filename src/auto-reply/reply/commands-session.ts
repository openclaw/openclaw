...
import { scheduleGatewaySigusr1Restart, triggerSmartAgentNeoRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting SmartAgentNeo in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSmartAgentNeoRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting SmartAgentNeo via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
