...
import { scheduleGatewaySigusr1Restart, triggerEasyHubRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting EasyHub in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerEasyHubRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting EasyHub via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
