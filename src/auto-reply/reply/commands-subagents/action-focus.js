...
import { formatThreadBindingDisabledError, formatThreadBindingSpawnDisabledError, resolveThreadBindingIdleTimeoutMsForChannel, resolveThreadBindingMaxAgeMsForChannel, resolveThreadBindingPlacementForCurrentContext, resolveThreadBindingSpawnPolicy, } from "../../../channels/thread-bindings-policy.js";
...
import { resolveFocusTargetSession, stopWithText } from "./shared.js";
...
export async function handleSubagentsFocusAction(ctx) {
    ...
    if (bindingContext.placement === "child") {
        ...
        if (!spawnPolicy.enabled) {
            return stopWithText(`⚠️ ${formatThreadBindingDisabledError({
        ...
        if (bindingContext.placement === "child" && !spawnPolicy.spawnEnabled) {
            return stopWithText(`⚠️ ${formatThreadBindingSpawnDisabledError({
                channel: spawnPolicy.channel,
                accountId: spawnPolicy.accountId,
                kind: "subagent",
            })}`);
    ...
    const acpMeta = focusTarget.targetKind === "acp"
        ? readAcpSessionEntry({
            cfg: params.cfg,
        ...
        binding = await bindingService.bind({
            ...
            metadata: {
                ...
                introText: resolveThreadBindingIntroText({
                    ...
                    sessionDetails: focusTarget.targetKind === "acp"
    ...
    const actionText = bindingContext.placement === "child"
