import { sendControlledSubagentMessage, steerControlledSubagentRun, } from "../commands-subagents-control.runtime.js";
...
export async function handleSubagentsSendAction(ctx, steerRequested) {
    ...
    if (steerRequested) {
        ...
        if (result.status === "accepted") {
            return stopWithText(`steered ${formatRunLabel(targetResolution.entry)} (run ${result.runId.slice(0, 8)}).`);
        }
