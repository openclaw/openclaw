...
describe("maybeGenerateSessionTitle", () => {
  beforeEach(() => {
    ...
    updateSessionStoreEntry.mockImplementation(
      async (params: { update: (entry: SessionEntry) => unknown }) => {
        await params.update({ sessionId: "abc" } as SessionEntry);
      },
    );
  });
  ...
  it("notifies callers after persisting a generated title", async () => {
    const transcriptPath = createTranscript(["first", "second", "third"]);
    const onTitleGenerated = vi.fn();
    resolveSessionTranscriptCandidates.mockReturnValue([transcriptPath]);
    updateSessionStoreEntry.mockImplementationOnce(
      async (params: { update: (entry: SessionEntry) => unknown }) => {
        await params.update({ sessionId: "abc" } as SessionEntry);
  ...
  it("skips stale async title writes after the session key rotates", async () => {
    const transcriptPath = createTranscript(["first", "second", "third"]);
    const onTitleGenerated = vi.fn();
    ...
    updateSessionStoreEntry.mockImplementationOnce(
      async (params: { update: (entry: SessionEntry) => unknown }) => {
        updateResult = await params.update({ sessionId: "new-session" } as SessionEntry);
        return updateResult;
      },
    );
    ...
    maybeGenerateSessionTitle({
      cfg: { sessionTitle: { enabled: true, turnsBeforeTitle: 3 } },
      sessionKey: "agent:main:abc",
      sessionEntry: { sessionId: "old-session" } as SessionEntry,
      storePath: "/tmp/store.json",
      agentId: "main",
      onTitleGenerated,
    });

    await vi.waitFor(() => expect(updateSessionStoreEntry).toHaveBeenCalledOnce());
    await Promise.resolve();
