...
describe("stripInboundMetadata", () => {
  ...
  it("strips compact direct metadata lines", () => {
    const input = `Direct message context (untrusted metadata): channel="telegram" chat_id="telegram:123" message_id="777" sender_id="999" sender_label="Ada Lovelace"\n\nCan you help me?`;
    expect(stripInboundMetadata(input)).toBe("Can you help me?");
  });
...
describe("extractInboundSenderLabel", () => {
  ...
  it("extracts compact direct sender labels", () => {
    const input = `Direct message context (untrusted metadata): channel="telegram" chat_id="telegram:123" message_id="777" sender_id="999" sender_label="Ada Lovelace (@ada)"\n\nHello from user`;
    expect(extractInboundSenderLabel(input)).toBe("Ada Lovelace (@ada)");
  });
});
