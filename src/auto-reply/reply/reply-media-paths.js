...
import { assertMediaNotDataUrl, resolveAllowedManagedMediaPath, resolveSandboxedMediaSource, } from "../../agents/sandbox-paths.js";
...
export function createReplyMediaPathNormalizer(params) {
    ...
    const persistedMediaBySource = new Map();
    ...
    const normalizeMediaSource = async (raw) => {
        ...
        const isRelativeLocalMedia = isLikelyLocalMediaSource(media) &&
        ...
        if (sandboxRoot) {
            ...
            try {
                ...
                if (FILE_URL_RE.test(media)) {
                    throw new Error("Host-local MEDIA file URLs are blocked in normal replies. Use a safe path or the message tool.", { cause: err });
                }
                throw err;
            }
        ...
        if (FILE_URL_RE.test(media)) {
            throw new Error("Host-local MEDIA file URLs are blocked in normal replies. Use a safe path or the message tool.");
        }
...
export function createReplyMediaContext(params) {
    return {
