...
import { resolveAgentConfig, resolveAgentWorkspaceDir, resolveSessionAgentId, } from "../../agents/agent-scope.js";
import { resolveConversationBindingRecord, touchConversationBindingRecord, } from "../../bindings/records.js";
import { normalizeChatType } from "../../channels/chat-type.js";
import { shouldSuppressLocalExecApprovalPrompt } from "../../channels/plugins/exec-approval-local.js";
import { parseSessionThreadInfoFast } from "../../config/sessions/thread-info.js";
import { logVerbose } from "../../globals.js";
import { fireAndForgetHook } from "../../hooks/fire-and-forget.js";
import { deriveInboundMessageHookContext, toPluginInboundClaimContext, toPluginInboundClaimEvent, toInternalMessageReceivedContext, toPluginMessageContext, toPluginMessageReceivedEvent, } from "../../hooks/message-hook-mappers.js";
...
import { buildPluginBindingDeclinedText, buildPluginBindingErrorText, buildPluginBindingUnavailableText, hasShownPluginBindingFallbackNotice, isPluginOwnedSessionBindingRecord, markPluginBindingFallbackNoticeShown, toPluginConversationBinding, } from "../../plugins/conversation-binding.js";
...
import { normalizeTtsAutoMode, resolveConfiguredTtsMode, shouldAttemptTtsPayload, } from "../../tts/tts-config.js";
import { INTERNAL_MESSAGE_CHANNEL, normalizeMessageChannel } from "../../utils/message-channel.js";
import { getReplyPayloadMetadata } from "../reply-payload.js";
import { normalizeVerboseLevel } from "../thinking.js";
import { resolveConversationBindingContextFromMessage } from "./conversation-binding-input.js";
import { createInternalHookEvent, loadSessionStore, resolveSessionStoreEntry, resolveStorePath, triggerInternalHook, } from "./dispatch-from-config.runtime.js";
...
export async function dispatchReplyFromConfig(params) {
    ...
    const shouldEmitVerboseProgress = createShouldEmitVerboseProgress({
        ...
        fallbackLevel: normalizeVerboseLevel(sessionStoreEntry.entry?.verboseLevel ??
    ...
    const inboundClaimEvent = toPluginInboundClaimEvent(hookContext, {
        commandAuthorized: typeof ctx.CommandAuthorized === "boolean" ? ctx.CommandAuthorized : undefined,
    ...
    const isInternalWebchatTurn = normalizedCurrentSurface === INTERNAL_MESSAGE_CHANNEL &&
        (normalizedSurfaceChannel === INTERNAL_MESSAGE_CHANNEL || !normalizedSurfaceChannel) &&
        ctx.ExplicitDeliverRoute !== true;
    const hasRouteReplyCandidate = Boolean(!suppressAcpChildUserDelivery &&
    ...
    const routeReplyToOriginating = async (payload, options) => {
        ...
        return await routeReplyRuntime.routeReply({
            ...
            policySessionKey: ctx.CommandSource === "native"
                ? (ctx.CommandTargetSessionKey ?? ctx.SessionKey)
                : ctx.SessionKey,
    ...
    const sendBindingNotice = async (payload, mode) => {
        ...
        if (result) {
            if (!result.ok) {
                logVerbose(`dispatch-from-config: route-reply (plugin binding notice) failed: ${result.error ?? "unknown error"}`);
            }
            return result.ok;
        }
    ...
    const pluginOwnedBindingRecord = inboundClaimContext.conversationId && inboundClaimContext.channelId
        ? resolveConversationBindingRecord({
            channel: inboundClaimContext.channelId,
            accountId: inboundClaimContext.accountId ??
                cfg.channels?.[inboundClaimContext.channelId]?.defaultAccount ??
                "default",
    ...
    const sendPolicy = resolveSendPolicy({
        ...
        channel: (shouldRouteToOriginating ? routeReplyChannel : undefined) ??
    ...
    if (pluginOwnedBinding) {
        ...
        if (suppressDelivery) {
            ...
            logVerbose(`plugin-bound inbound skipped under sendPolicy: deny (plugin=${pluginOwnedBinding.pluginId} session=${sessionKey ?? "unknown"}); falling through to suppressed agent processing`);
        }
        else {
            ...
            const targetedClaimOutcome = hookRunner?.runInboundClaimForPluginOutcome
                ? await hookRunner.runInboundClaimForPluginOutcome(pluginOwnedBinding.pluginId, inboundClaimEvent, { ...inboundClaimContext, pluginBinding: pluginOwnedBinding })
                : (() => {
                    const pluginLoaded = getGlobalPluginRegistry()?.plugins.some((plugin) => plugin.id === pluginOwnedBinding.pluginId && plugin.status === "loaded") ?? false;
                    return pluginLoaded
                        ? { status: "no_handler" }
            ...
            switch (targetedClaimOutcome.status) {
                ...
                case "no_handler": {
                    ...
                    if (!hasShownPluginBindingFallbackNotice(pluginOwnedBinding.bindingId)) {
                        const didSendNotice = await sendBindingNotice({ text: buildPluginBindingUnavailableText(pluginOwnedBinding) }, "additive");
                        if (didSendNotice) {
    ...
    if (hookRunner?.hasHooks("message_received")) {
        fireAndForgetHook(hookRunner.runMessageReceived(toPluginMessageReceivedEvent(hookContext), toPluginMessageContext(hookContext)), "dispatch-from-config: message_received plugin hook failed");
    }
    ...
    try {
        ...
        if (fastAbort.handled) {
            ...
            if (!suppressDelivery) {
                ...
                logVerbose(`dispatch-from-config: fast_abort reply suppressed by sendPolicy: deny (session=${sessionKey ?? "unknown"})`);
            }
        ...
        const shouldSendVerboseProgressMessages = !isSlackNonDirectSurface && (ctx.ChatType !== "group" || ctx.IsForum === true);
        ...
        if (suppressDelivery) {
            logVerbose(`Delivery suppressed by send policy for session ${sessionStoreEntry.sessionKey ?? sessionKey ?? "unknown"} — agent will still process the message`);
        }
        const toolStartStatusesSent = new Set();
        ...
        const replyResult = await replyResolver(ctx, {
            ...
            onToolResult: (payload) => {
                const run = async () => {
                    ...
                    if (suppressDefaultToolProgressMessages) {
                        ...
                        const execApproval = deliveryPayload.channelData &&
                            ...
                            : undefined;
                        const hasExecApproval = execApproval && typeof execApproval === "object" && !Array.isArray(execApproval);
            ...
            onBlockReply: (payload, context) => {
                const run = async () => {
                    ...
                    const queuedContext = payloadMetadata?.assistantMessageIndex !== undefined
                        ? {
                            ...context,
        ...
        recordProcessed("completed", pluginFallbackReason ? { reason: pluginFallbackReason } : undefined);
        markIdle("message_completed");
        return { queuedFinal, counts };
    }
    catch (err) {
