...
describe("followup queue restart session permission policy", () => {
  ...
  it("restores the queued session permission pair into the embedded execution policy", async () => {
    ...
    try {
      ...
      await enqueueFollowupRun(key, queued, settings, "message-id", undefined, false);

      expect(await followupQueueEntryContainsPrompt(key, "stay inside the workspace")).toBe(true);
  ...
  it("fail-closes a live root-only session policy after persist+restore", async () => {
    ...
    try {
      ...
      const persisted = (await loadFollowupQueueEntries()).find(
      ...
      expect(await followupQueueEntryContainsPrompt(key, "root-only live policy")).toBe(true);

      simulateGatewayRestart();
      await restoreFollowupQueues();

      expect(FOLLOWUP_QUEUES.get(key)?.items ?? []).toEqual([]);
      expect(await followupQueueEntryContainsPrompt(key, "root-only live policy")).toBe(false);
    } finally {
      simulateGatewayRestart();
      if (originalStateDir === undefined) {
        delete process.env.OPENCLAW_STATE_DIR;
