...
const CURRENT_MESSAGE_SENTINEL = "Current message:";
const [CONVERSATION_INFO_SENTINEL, SENDER_INFO_SENTINEL] = INBOUND_META_SENTINELS;

// Matches lines like "Chat window (untrusted, ...):" or any label "(untrusted, ...):"
// These are generated by formatChatWindowStructuredContext.
const CHAT_WINDOW_CONTEXT_RE = /^\S.*\(untrusted,\s*[^)]+\):\s*$/;

// Pre-compiled fast-path regex — avoids line-by-line parse when no blocks present.
const SENTINEL_FAST_RE = new RegExp(
  [
    ...INBOUND_META_SENTINELS,
    CURRENT_MESSAGE_SENTINEL,
    ...MESSAGE_TOOL_DELIVERY_HINTS,
    UNTRUSTED_CONTEXT_HEADER,
  ]
    .map((s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
    .concat(
      // Chat window pattern: <label> (untrusted, <order>, <relation>):
      "\\(untrusted,\\s*[^)]+\\):",
    )
    .join("|"),
);
...
function isInboundMetaSentinelLine(line: string): boolean {
  ...
  return (
    ...
    trimmed === CURRENT_MESSAGE_SENTINEL ||
    CHAT_WINDOW_CONTEXT_RE.test(trimmed)
  );
}

function isNonFencedMetaBlockLine(line: string): boolean {
  const trimmed = line.trim();
  return trimmed === CURRENT_MESSAGE_SENTINEL || CHAT_WINDOW_CONTEXT_RE.test(trimmed);
}
...
export function stripInboundMetadata(text: string): string {
  ...
  let inNonFencedBlock = false; // true when stripping non-fenced blocks (Current message, Chat window)
  ...
  for (let i = 0; i < strippedLeadingPrefixLines.length; i++) {
    ...
    if (!inMetaBlock && isInboundMetaSentinelLine(line)) {
      if (isNonFencedMetaBlockLine(line)) {
        // Non-JSON-fence blocks (Current message, Chat window, etc.)
        // Skip the sentinel line and all continuation lines until next empty line.
        inMetaBlock = true;
        inNonFencedBlock = true;
        continue;
      }
    ...
    if (inMetaBlock) {
      if (inNonFencedBlock) {
        // Inside a non-fenced block (Current message, Chat window).
        // End of block: empty line or start of a new sentinel block.
        if (line.trim() === "" || isInboundMetaSentinelLine(line)) {
          inMetaBlock = false;
          inNonFencedBlock = false;
        ...
        // Continuation line of the non-fenced block — skip it.
        continue;
      }
...
export function stripLeadingInboundMetadata(text: string): string {
  ...
  while (index < lines.length) {
    ...
    if (isNonFencedMetaBlockLine(line)) {
      // Non-fenced block: skip sentinel line and all continuation lines
      // until an empty line or a line that starts a new sentinel block.
      index++;
      while (index < lines.length) {
        const current = lines[index].trim();
        if (current === "" || isInboundMetaSentinelLine(current)) {
          break;
        }
        index++;
      }
