...
import { loadSessionStore, resolveSessionPluginStatusLines, resolveSessionPluginTraceLines, updateSessionStoreEntry, } from "../../config/sessions.js";
...
import { estimateUsageCost, formatTokenCount, resolveModelCostConfig, } from "../../utils/usage-format.js";
...
import { createReplyOperation, ReplyRunAlreadyActiveError, replyRunRegistry, } from "./reply-run-registry.js";
import { createReplyToModeFilterForChannel, resolveReplyToMode } from "./reply-threading.js";
import { incrementRunCompactionCount, persistRunSessionUsage } from "./session-run-accounting.js";
import { createTypingSignaler } from "./typing-mode.js";
const BLOCK_REPLY_SEND_TIMEOUT_MS = 15_000;
function buildInlinePluginStatusPayload(params) {
    const statusLines = params.entry?.verboseLevel && params.entry.verboseLevel !== "off"
        ? resolveSessionPluginStatusLines(params.entry)
        : [];
    const traceLines = params.includeTraceLines &&
...
function mergeExecutionTrace(params) {
    const attempts = [
        ...(params.fallbackAttempts ?? []).map((attempt) => Object.assign({
            provider: attempt.provider,
            model: attempt.model,
            result: inferFallbackAttemptResult(attempt),
        }, attempt.reason ? { reason: attempt.reason } : {}, typeof attempt.status === `number` ? { status: attempt.status } : {})),
    ...
    if (winnerProvider &&
        winnerModel &&
        !attempts.some((attempt) => attempt.provider === winnerProvider &&
...
function formatRequestContextTraceBlock(params) {
    ...
    const headroom = typeof limit === "number" &&
        Number.isFinite(limit) &&
        typeof used === "number" &&
        Number.isFinite(used)
        ? Math.max(0, limit - used)
        : undefined;
    const percent = typeof limit === "number" &&
        Number.isFinite(limit) &&
        limit > 0 &&
        typeof used === "number" &&
        Number.isFinite(used)
...
export async function runReplyAgent(params) {
    ...
    if (shouldSteer && isStreaming) {
        const steerSessionId = (sessionKey ? replyRunRegistry.resolveSessionId(sessionKey) : undefined) ??
    ...
    try {
        ...
        const resetSession = async ({ failureLabel, buildLogMessage, cleanupTranscripts, }) => await resetReplyRunSession({
            ...
            activeSessionStore,
            storePath,
            messageThreadId: typeof sessionCtx.MessageThreadId === "string" ? sessionCtx.MessageThreadId : undefined,
            followupRun,
        ...
        const cliSessionBinding = isCliProvider(providerUsed, cfg)
            ...
            : undefined;
        const runtimeContextTokens = typeof runResult.meta?.agentMeta?.contextTokens === "number" &&
            ...
            : undefined;
        const contextTokensUsed = runtimeContextTokens ??
        ...
        if (isDiagnosticsEnabled(cfg) && hasNonzeroUsage(usage)) {
            ...
            emitTrustedDiagnosticEvent({
                ...
                ...(runResult.diagnosticTrace
                    ? {
                        trace: freezeDiagnosticTraceContext(createChildDiagnosticTraceContext(runResult.diagnosticTrace)),
                    }
                    : {}),
                sessionKey,
                sessionId: followupRun.run.sessionId,
        ...
        const rawUserText = runResult.meta?.finalPromptText ??
        ...
        const requestShaping = {
            authMode: runResult.meta?.requestShaping?.authMode ??
                (cfg?.models?.providers && providerUsed in cfg.models.providers
                    ...
                    : undefined),
            thinking: runResult.meta?.requestShaping?.thinking ??
                normalizeOptionalString(followupRun.run.thinkLevel),
            reasoning: runResult.meta?.requestShaping?.reasoning ??
            ...
            trace: runResult.meta?.requestShaping?.trace ??
                normalizeOptionalString(activeSessionEntry?.traceLevel),
            fallbackEligible: runResult.meta?.requestShaping?.fallbackEligible ??
                hasConfiguredModelFallbacks({
                    cfg,
                    agentId: followupRun.run.agentId,
                    sessionKey: followupRun.run.sessionKey,
                }),
            blockStreaming: runResult.meta?.requestShaping?.blockStreaming ??
        ...
        const completion = runResult.meta?.completion ??
        ...
        const contextManagement = {
            ...
            ...(runResult.meta?.contextManagement &&
                ...
                ? {
                    postCompactionContextInjected: runResult.meta.contextManagement.postCompactionContextInjected,
                }
                : {}),
        };
        const sessionUsage = traceAuthorized && activeSessionEntry?.traceLevel === "raw"
            ...
            : undefined;
        const traceEnabledForSender = traceAuthorized &&
        ...
        if (shouldAppendTracePayload) {
            ...
            const rawTracePayload = traceAuthorized && activeSessionEntry?.traceLevel === "raw"
