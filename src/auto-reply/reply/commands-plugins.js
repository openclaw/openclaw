...
import { buildPreferredClawHubSpec, createPluginInstallLogger, decidePreferredClawHubFallback, resolveFileNpmSpecToLocalPath, } from "../../cli/plugins-command-helpers.js";
...
export const handlePluginsCommand = async (params, allowTextCommands) => {
    ...
    const allowInternalReadOnly = (pluginsCommand.action === "list" || pluginsCommand.action === "inspect") &&
    ...
    if (pluginsCommand.action === "inspect") {
        ...
        if (normalizeOptionalLowercaseString(pluginsCommand.name) === "all") {
            return {
                shouldContinue: false,
                reply: {
                    text: renderJsonBlock("🔌 Plugins", buildAllPluginInspectJson({ ...loaded, installRecords })),
                },
            };
        }
    ...
    const next = setPluginEnabledInConfig(structuredClone(loaded.config), plugin.id, pluginsCommand.action === "enable");
