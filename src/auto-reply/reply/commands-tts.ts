...
function ttsUsage(): ReplyPayload {
  ...
  return {
    text:
      ...
      `• cartesia — Sonic voice (requires API key)\n\n` +
...
export const handleTtsCommands: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (action === "provider") {
    ...
    if (!args.trim()) {
      ...
      const hasCartesia = Boolean(resolveTtsApiKey(config, "cartesia"));
      return {
        shouldContinue: false,
        reply: {
          text:
            ...
            `Cartesia key: ${hasCartesia ? "✅" : "❌"}\n` +
            `Usage: /tts provider openai | elevenlabs | edge | cartesia`,
        },
      };
    }
