...
function ttsUsage(): ReplyPayload {
  ...
  return {
    text:
      ...
      `• sarvam — Indic text+voice (requires API key)\n\n` +
...
export const handleTtsCommands: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (action === "provider") {
    ...
    if (!args.trim()) {
      ...
      const hasSarvam = Boolean(resolveTtsApiKey(config, "sarvam"));
      ...
      return {
        ...
        reply: {
          text:
            ...
            `Sarvam key: ${hasSarvam ? "✅" : "❌"}\n` +
            `Edge enabled: ${hasEdge ? "✅" : "❌"}\n` +
            `Usage: /tts provider openai | elevenlabs | sarvam | edge`,
        },
      };
    }
