...
type HomeEnvSnapshot = {
  ...
  MULLUSI_STATE_DIR: string | undefined;
  MULLUSI_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    MULLUSI_AGENT_DIR: process.env.MULLUSI_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".mullusi", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.MULLUSI_STATE_DIR = path.join(home, ".mullusi");
    process.env.MULLUSI_AGENT_DIR = path.join(home, ".mullusi", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".mullusi", "agent");
