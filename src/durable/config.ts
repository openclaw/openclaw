// Durable runtime feature flag and path resolution.
...
export function isDurableRuntimesEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return ENABLED_VALUES.has((env.OPENCLAW_DURABLE_RUNTIME ?? "").trim().toLowerCase());
}

export function isDurableWorkerEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    isDurableRuntimesEnabled(env) &&
