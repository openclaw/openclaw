...
describe("worker runtime", () => {
  ...
  it
    ...
    .each(["worker", "anchor", "node-host"] as const)(
    "stops registered background execs when their %s crashes",
    { timeout: 120_000 },
    async (crashed) => {
      const { gateway, launch, workspaceDir } = await setup({
      ...
      await writeFile(
        ...
        [
          ...
          'process.on("SIGTERM", () => {});',
          'fs.writeFileSync("exec-lineage-env.txt", process.env.OPENCLAW_WORKER_LINEAGE_FDS ?? "absent");',
      ...
      try {
        ...
        expect(await readFile(path.join(workspaceDir, "exec-lineage-env.txt"), "utf8")).toBe(
          "absent",
        );
