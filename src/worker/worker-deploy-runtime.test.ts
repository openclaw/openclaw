...
vi.mock("../infra/runtime-process-url.js", () => ({
  registerSealedRuntimeProcessEntrypoint: vi.fn(),
}));
vi.mock("../infra/sealed-runtime-registry.js", () => ({
  registerSealedRuntime: () => state.calls.push("sealed:registered"),
}));
...
vi.mock("./worker-deploy-runtime-registry.js", () => ({
  setWorkerDeployHighlightJs: () => state.calls.push("highlight:registered"),
}));
...
it("pins the sealed worker to bundled JavaScript before registering its runtime", async () => {
  ...
  expect(state.calls).toEqual(["fs-safe:off", "sealed:registered", "highlight:registered"]);
});
