...
export function listIMessageAccountIds(cfg: DNAConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: DNAConfig): string {
...
function mergeIMessageAccountConfig(cfg: DNAConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: DNAConfig): ResolvedIMessageAccount[] {
