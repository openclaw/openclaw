...
function baseCfg(): ActiviConfig {
  return {
    ...
    messages: {
      groupChat: { mentionPatterns: ["@activi"] },
    },
...
function buildDispatchContextPayload(params: { cfg: ActiviConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@activi hello",
      bodyText: "@activi hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
