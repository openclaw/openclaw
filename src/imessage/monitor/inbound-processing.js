...
import { DM_GROUP_ACCESS_REASON, resolveDmGroupAccessWithLists, } from "../../security/dm-policy-shared.js";
...
export function resolveIMessageInboundDecision(params) {
    ...
    if (params.echoCache && (messageText || inboundMessageId)) {
        ...
        if (params.echoCache.has(echoScope, {
            ...
            params.logVerbose?.(describeIMessageEchoDropLog({ messageText, messageId: inboundMessageId }));
    ...
    const ownerAllowedForCommands = commandDmAllowFrom.length > 0
...
export function buildIMessageInboundContext(params) {
    ...
    const inboundHistory = decision.isGroup && decision.historyKey && params.historyLimit > 0
