...
import { redactPathForSupport, sanitizeSupportSnapshotValue, } from "../logging/diagnostic-support-redaction.js";
...
function buildSkillsCapture(skillsSnapshot, redaction) {
    ...
    const entries = skillsSnapshot.resolvedSkills && skillsSnapshot.resolvedSkills.length > 0
...
export function buildTrajectoryRunMetadata(params) {
    const env = params.env ?? process.env;
    const redaction = buildTrajectorySupportRedaction(env);
    const os = resolveOsSummary();
    const plugins = buildPluginsFromActiveRegistry() ??
    ...
    return {
        ...
        harness: {
            ...
            name: "OpenClaw",
            version: VERSION,
            gitSha: resolveCommitHash({ cwd: params.workspaceDir, env, moduleUrl: import.meta.url }) ??
                undefined,
            os,
            runtime: {
...
export function buildTrajectoryArtifacts(params) {
    return {
        capturedAt: new Date().toISOString(),
