...
import { jsonSupportBundleFile, jsonlSupportBundleFile, supportBundleContents, textSupportBundleFile, writeSupportBundleDirectory, } from "../logging/diagnostic-support-bundle.js";
import { redactSupportString, } from "../logging/diagnostic-support-redaction.js";
import { safeJsonStringify } from "../utils/safe-json.js";
import { TRAJECTORY_RUNTIME_FILE_MAX_BYTES, resolveTrajectoryFilePath, resolveTrajectoryPointerFilePath, safeTrajectorySessionFileName, } from "./paths.js";
        ...
        for (const entry of entries) {
            ...
            if (entry.type === "compaction" && typeof mutable.firstKeptEntryIndex === "number") {
                ...
                if (target && target.type !== "session") {
                    mutable.firstKeptEntryId = target.id;
                }
...
function parseJsonlFile(filePath, params) {
    ...
    if (stat.size > params.maxBytes) {
        throw new Error(`Trajectory runtime file is too large to export (${stat.size} bytes; limit ${params.maxBytes})`);
    }
    ...
    for (const row of rows) {
        if (parsed.length >= params.maxEvents) {
            throw new Error(`Trajectory runtime file has too many events to export (limit ${params.maxEvents})`);
        }
        try {
...
function isRuntimeTrajectoryEventForSession(value, sessionId) {
    ...
    return (value.traceSchema === "openclaw-trajectory" &&
...
function readRuntimePointerFile(sessionFile, sessionId) {
    ...
    try {
        ...
        const defaultRuntimeFile = path.resolve(resolveTrajectoryFilePath({
            env: {},
            sessionFile,
            sessionId,
        }));
...
function redactEventForExport(event, redaction) {
    return {
        ...
        workspaceDir: event.workspaceDir
            ...
            : undefined,
        data: event.data
            ? redactLocalPathValues(event.data, redaction)
            : undefined,
    };
}
function resolveRuntimeContext(runtimeEvents) {
    ...
    const toolsValue = Array.isArray(runtimeData?.tools)
        ...
        : undefined;
    return {
        systemPrompt: typeof runtimeData?.systemPrompt === "string" ? runtimeData.systemPrompt : undefined,
        tools: toolsValue,
    };
}
...
function markInvokedSkills(params) {
    ...
    const entries = skillsRecord.entries.map((entry) => {
        ...
        const skillDirName = rawPath?.replaceAll("\\", "/").split("/").slice(-2, -1)[0]?.toLowerCase() ?? undefined;
        const invoked = normalizedPath
            ? [...normalizedInvokedPaths].some((candidate) => candidate === normalizedPath ||
...
function buildPromptsCapture(params) {
    ...
    const systemPrompt = (typeof latestCompiled?.systemPrompt === "string" ? latestCompiled.systemPrompt : undefined) ??
        params.runtimeContext.systemPrompt;
    const skillsPrompt = runtimeMetadata?.prompting &&
        typeof runtimeMetadata.prompting === "object" &&
        typeof runtimeMetadata.prompting.skillsPrompt === "string"
        ? runtimeMetadata.prompting.skillsPrompt
        : undefined;
    const userPromptPrefixText = runtimeMetadata?.prompting &&
        typeof runtimeMetadata.prompting === "object" &&
        typeof runtimeMetadata.prompting.userPromptPrefixText === "string"
        ? runtimeMetadata.prompting.userPromptPrefixText
        : undefined;
    const promptReport = runtimeMetadata?.prompting &&
        typeof runtimeMetadata.prompting === "object" &&
        typeof runtimeMetadata.prompting.systemPromptReport === "object"
        ? runtimeMetadata.prompting.systemPromptReport
        : undefined;
...
export function exportTrajectoryBundle(params) {
    ...
    if (sessionStat.size > MAX_TRAJECTORY_SESSION_FILE_BYTES) {
        throw new Error(`Trajectory session file is too large to export (${sessionStat.size} bytes; limit ${MAX_TRAJECTORY_SESSION_FILE_BYTES})`);
    }
    ...
    if (totalEventCount > maxTotalEvents) {
        throw new Error(`Trajectory export has too many events (${totalEventCount}; limit ${maxTotalEvents})`);
    }
    ...
    const manifest = {
        ...
        sourceFiles: {
            ...
            runtime: runtimeFile && isRegularNonSymlinkFile(runtimeFile)
    ...
    if (bundleRuntimeContext.systemPrompt) {
        files.push(textSupportBundleFile("system-prompt.txt", redactLocalPathValues(bundleRuntimeContext.systemPrompt, redaction)));
    }
    if (bundleRuntimeContext.tools) {
        files.push(jsonSupportBundleFile("tools.json", redactLocalPathValues(bundleRuntimeContext.tools, redaction)));
    }
    const contents = [...supportBundleContents(files)];
