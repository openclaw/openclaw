...
export function appendInterruptedSessionTrajectoryEndSync(params: {
  ...
  // Honor the shipped OPENCLAW_TRAJECTORY=0 capture opt-out with the same
  // parseBooleanValue semantics as the normal runtime recorder: a
  // capture-disabled process skips the terminal event but must still commit
  // its recovery-state transition, so this returns without throwing.
