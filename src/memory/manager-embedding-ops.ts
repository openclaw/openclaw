  ...
  protected async indexFile(
    ...
    if (vectorReady) {
      try {
        ...
        log.warn(`Failed to delete vector entries for ${entry.path}`, { error: String(err) });
      }
    }
    if (this.fts.enabled && this.fts.available) {
      try {
        ...
        log.warn(`Failed to delete FTS entries for ${entry.path}`, { error: String(err) });
      }
    }
    this.db
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (vectorReady && embedding.length > 0) {
        try {
          ...
          log.warn(`Failed to delete vector entry for chunk ${id}`, { error: String(err) });
        }
        this.db
