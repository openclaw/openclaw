...
export function parseCrestodianOperation(input) {
...
export function isPersistentCrestodianOperation(operation) {
    return (operation.kind === "set-default-model" ||
...
export function describeCrestodianPersistentOperation(operation) {
...
export function formatCrestodianPersistentPlan(operation) {
...
async function resolveTuiAgentId(params) {
    ...
    const match = overview.agents.find((agent) => {
        return (normalizeAgentId(agent.id) === requested ||
            (agent.name ? normalizeAgentId(agent.name) === requested : false));
    });
    return match?.id ?? requested;
}
export async function executeCrestodianOperation(operation, runtime, opts = {}) {
    ...
    if (operation.kind === "config-set") {
        ...
        const { readConfigFileSnapshot } = await import("../config/config.js");
        const before = await readConfigFileSnapshot();
        const runConfigSet = opts.deps?.runConfigSet ??
            (async (setOpts) => {
    ...
    if (operation.kind === "create-agent") {
        ...
        const before = await readConfigFileSnapshot();
        const workspace = resolveUserPath(operation.workspace ?? process.cwd());
        const runAgentsAdd = opts.deps?.runAgentsAdd ??
    ...
    if (operation.kind === "gateway-restart") {
        ...
        const runGatewayRestart = opts.deps?.runGatewayRestart ?? (() => runGatewayLifecycle("restart"));
    ...
    if (operation.kind === "open-tui") {
        ...
        if (result?.exitReason === "return-to-crestodian") {
            runtime.log(result.crestodianMessage
