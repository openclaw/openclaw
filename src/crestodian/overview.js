import { listAgentEntries, resolveAgentEffectiveModelPrimary, resolveDefaultAgentId, } from "../agents/agent-scope.js";
import { OPENCLAW_DOCS_URL, OPENCLAW_SOURCE_URL, resolveOpenClawReferencePaths, } from "../agents/docs-path.js";
import { readConfigFileSnapshot, resolveConfigPath, resolveGatewayPort, } from "../config/config.js";
...
export async function loadCrestodianOverview(opts = {}) {
    const env = opts.env ?? process.env;
    const snapshot = await readConfigFileSnapshot();
    const cfg = snapshot.runtimeConfig ?? snapshot.sourceConfig ?? {};
    const defaultAgentId = resolveDefaultAgentId(cfg);
    const defaultModel = resolveAgentEffectiveModelPrimary(cfg, defaultAgentId) ??
...
export function formatCrestodianOverview(overview) {
    ...
    const configStatus = overview.config.valid
        ...
        : "invalid";
    const issueLines = overview.config.issues.length > 0
...
export function recommendCrestodianNextStep(overview) {
...
export function formatCrestodianStartupMessage(overview) {
