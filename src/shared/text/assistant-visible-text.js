...
export function stripToolCallXmlTags(text) {
    ...
    for (let idx = 0; idx < text.length; idx += 1) {
        ...
        if (!inToolCallBlock) {
            ...
            const payloadKind = tag.tagName === "tool_call" || tag.tagName === "function"
...
export function sanitizeAssistantVisibleTextWithProfile(text, profile = "delivery") {
    return applyAssistantVisibleTextStagePipeline(text, ASSISTANT_VISIBLE_TEXT_PIPELINE_OPTIONS[profile]);
}
export function stripAssistantInternalScaffolding(text) {
 ...
 */
export function sanitizeAssistantVisibleTextWithOptions(text, options) {
