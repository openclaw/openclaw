...
describe("shared/string-normalization", () => {
	...
	it("normalizes mixed allow-list entries to lowercase", () => {
		expect(normalizeStringEntriesLower([" A ", "MiXeD", 7])).toEqual([
			"a",
			"mixed",
			"7",
		]);
	...
	it("normalizes slug-like labels while preserving supported symbols", () => {
		...
		expect(normalizeHyphenSlug(" #My_Channel + Alerts ")).toBe(
			"#my_channel-+-alerts",
		);
	...
	it("collapses repeated separators and trims leading/trailing punctuation", () => {
		expect(normalizeHyphenSlug("  ...Hello   /  World---  ")).toBe(
			"hello-world",
		);
		expect(normalizeHyphenSlug(" ###Team@@@Room### ")).toBe(
			"###team@@@room###",
		);
	});

	it("normalizes @/# prefixed slugs used by channel allowlists", () => {
		expect(normalizeAtHashSlug(" #My_Channel + Alerts ")).toBe(
			"my-channel-alerts",
		);
