...
describe("shared/frontmatter", () => {
	...
	test("resolveOpenClawManifestBlock returns undefined for invalid input", () => {
		...
		expect(
			resolveOpenClawManifestBlock({ frontmatter: { metadata: "123" } }),
		).toBeUndefined();
		expect(
			resolveOpenClawManifestBlock({ frontmatter: { metadata: "[]" } }),
		).toBeUndefined();
		expect(
			resolveOpenClawManifestBlock({
	...
	it("normalizes manifest requirement and os lists", () => {
		...
		expect(
			resolveOpenClawManifestOs({ os: [" darwin ", "linux", ""] }),
	...
	it("parses and applies install common fields", () => {
		...
		expect(
			parseOpenClawManifestInstallBase({ kind: "bad" }, ["brew"]),
		).toBeUndefined();
		expect(
			applyOpenClawManifestInstallCommonFields<{
				extra: boolean;
