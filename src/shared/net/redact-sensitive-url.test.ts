...
describe("redactSensitiveUrl", () => {
	it("redacts userinfo and sensitive query params from valid URLs", () => {
		expect(
			redactSensitiveUrl(
				"https://user:pass@example.com/mcp?token=secret&safe=value",
			),
		).toBe("https://***:***@example.com/mcp?token=***&safe=value");
	});

	it("treats query param names case-insensitively", () => {
		expect(
			redactSensitiveUrl("https://example.com/mcp?Access_Token=secret"),
		).toBe("https://example.com/mcp?Access_Token=***");
	});
...
describe("redactSensitiveUrlLikeString", () => {
	it("redacts invalid URL-like strings", () => {
		expect(
			redactSensitiveUrlLikeString(
				"//user:pass@example.com/mcp?client_secret=secret",
			),
		).toBe("//***:***@example.com/mcp?client_secret=***");
	});
});
...
describe("sensitive URL config metadata", () => {
	...
	it("uses an explicit url-secret hint tag", () => {
		...
		expect(hasSensitiveUrlHintTag({ tags: [SENSITIVE_URL_HINT_TAG] })).toBe(
			true,
		);
