...
export function hasSensitiveUrlHintTag(hint) {
...
export function redactSensitiveUrlLikeString(value) {
    ...
    return value
        .replace(/\/\/([^@/?#]+)@/, "//***:***@")
        .replace(/([?&])([^=&]+)=([^&]*)/g, (match, prefix, key) => isSensitiveUrlQueryParamName(key) ? `${prefix}${key}=***` : match);
}
