...
export function resolveAvatarMime(filePath) {
...
export function looksLikeAvatarPath(value) {
    if (/[\\/]/.test(value)) {
        return true;
    }
    return AVATAR_PATH_EXT_RE.test(value);
}
export function isSupportedLocalAvatarExtension(filePath) {
    const ext = path.extname(filePath).toLowerCase();
