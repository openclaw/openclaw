...
describe("shared/subagents-format", () => {
	...
	it("resolves token totals and io breakdowns from valid numeric fields only", () => {
		...
		expect(
			resolveTotalTokens({ inputTokens: Number.NaN, outputTokens: 5 }),
		).toBeUndefined();
		...
		expect(
			resolveIoTokens({ inputTokens: Number.NaN, outputTokens: 0 }),
		).toBeUndefined();
	});
	...
	it("formats io and prompt-cache usage displays with fallback branches", () => {
		...
		expect(formatTokenUsageDisplay({ totalTokens: 500 })).toBe(
