...
function isLegacyClawdbotClient(clientId: string | undefined): boolean {
	const normalized = clientId?.trim().toLowerCase() ?? "";
	return (
		normalized.startsWith("clawdbot-") || normalized.startsWith("moldbot-")
	);
}
...
function pickPreferredLegacyMigrationMatch(
	...
	const current = matches.filter((match) =>
		isCurrentOpenClawClient(match.clientId),
	);
	if (current.length !== 1) {
		return undefined;
	}
	const legacyCount = matches.filter((match) =>
		isLegacyClawdbotClient(match.clientId),
	).length;
...
export function resolveNodeIdFromCandidates(
	...
	const topMatchScore = Math.max(
		...rawMatches.map((match) => match.matchScore),
	);
	const strongestMatches = rawMatches.filter(
		(match) => match.matchScore === topMatchScore,
	);
	if (strongestMatches.length === 1) {
		return strongestMatches[0]?.node.nodeId ?? "";
	}

	const topSelectionScore = Math.max(
		...strongestMatches.map((match) => match.selectionScore),
	);
	const matches = strongestMatches.filter(
		(match) => match.selectionScore === topSelectionScore,
	);
	if (matches.length === 1) {
		return matches[0]?.node.nodeId ?? "";
	}

	const preferred = pickPreferredLegacyMigrationMatch(
