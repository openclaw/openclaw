...
describe("config-eval helpers", () => {
	...
	it("resolves nested config paths and missing branches safely", () => {
		...
		expect(
			resolveConfigPath("not-an-object", "browser.enabled"),
		).toBeUndefined();
	});
	...
	it("uses defaults only when config paths are unresolved", () => {
		...
		expect(
			isConfigPathTruthyWithDefaults(config, "browser.enabled", {
				"browser.enabled": true,
			}),
		).toBe(false);
		expect(
			isConfigPathTruthyWithDefaults(config, "browser.missing", {
				"browser.missing": true,
			}),
		).toBe(true);
		expect(isConfigPathTruthyWithDefaults(config, "browser.other", {})).toBe(
			false,
		);
	});
	...
	it("checks PATHEXT candidates on Windows", () => {
		...
		expect(hasBinary("tool")).toBe(true);
		expect(
			accessSpy.mock.calls.map(([candidate]) => String(candidate)),
		).toEqual([plainCandidate, exeCandidate, cmdCandidate]);
	});
});
