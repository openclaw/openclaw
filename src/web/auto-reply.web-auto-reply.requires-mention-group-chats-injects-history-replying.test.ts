...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-web-home-"));
...
const _makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-session-"));
...
describe("web auto-reply", () => {
  ...
  it("detects LID mentions using authDir mapping", async () => {
    ...
    const authDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-wa-auth-"));
