...
describe("web monitor inbox", () => {
  ...
  it("allows messages from senders in allowFrom list", async () => {
    ...
    const upsert = {
      ...
      messages: [
        {
          key: { id: "auth1", fromMe: false, remoteJid: "[redacted-email]" },
  ...
  it("allows same-phone messages even if not in allowFrom", async () => {
    ...
    // Message from self (sock.user.id is "[redacted-email]" in mock)
    const upsert = {
      type: "notify",
      messages: [
        {
          key: { id: "self1", fromMe: false, remoteJid: "[redacted-email]" },
  ...
  it("normalizes participant phone numbers to JIDs in sendReaction", async () => {
    ...
    await listener.sendReaction("[redacted-email]", "msg123", "👍", false, "+6421000000");
