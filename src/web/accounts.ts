...
export function listWhatsAppAuthDirs(cfg: DNAConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: DNAConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: DNAConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: DNAConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: DNAConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: DNAConfig): ResolvedWhatsAppAccount[] {
