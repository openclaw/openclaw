...
export async function sendMessageWhatsApp(to, body, options) {
    ...
    try {
        ...
        logger.error({ err: String(err), to: redactedTo, hasMedia: Boolean(options.mediaUrl) }, "failed to send via web session");
        throw err;
    }
}
export async function sendReactionWhatsApp(chatJid, messageId, emoji, options) {
    ...
    try {
        ...
        logger.error({ err: String(err), chatJid: redactedChatJid, messageId, emoji }, "failed to send reaction via web session");
        throw err;
    }
}
