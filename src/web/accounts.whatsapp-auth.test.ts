...
describe("hasAnyWhatsAppAuth", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ACTIVI_OAUTH_DIR"]);
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "activi-oauth-"));
    process.env.ACTIVI_OAUTH_DIR = tempOauthDir;
  });
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "activi-wa-auth-"));
    try {
