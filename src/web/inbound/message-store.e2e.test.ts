...
describe("MessageStore E2E - Direct History Read", () => {
  ...
  it("should verify authentication and connectivity", async () => {
    ...
    try {
      ...
      if (targetChat) {
        ...
        process.stdout.write(
          `        - Created: ${new Date((targetChat.creation || 0) * 1000).toLocaleString()}\n`,
        );
      } else {
        process.stdout.write(
          `     ⚠️  Target chat not found in group list (set TEST_GROUP_JID env var)\n`,
        );
      }
      process.stdout.write(`\n`);
    } catch (error: any) {
    ...
    const messageHandler = (update: any) => {
      ...
      process.stdout.write(
        `     📨 Received ${update.messages.length} message(s) (type: ${update.type})\n`,
      );
    };
  ...
  it("should fetch message history using our patched implementation", async () => {
    ...
    process.stdout.write(
      `   Note: Set TEST_GROUP_JID environment variable to test with a specific group\n`,
    );
    process.stdout.write(
      `   Strategy: Wait for a new message, then fetch history from that point\n\n`,
    );
    ...
    try {
      ...
      process.stdout.write(
        `   The fetchMessageHistory call succeeded and returned a message ID,\n`,
      );
      process.stdout.write(`   but no history sync notification was received.\n\n`);

      process.stdout.write(`   Possible reasons:\n`);
      process.stdout.write(
        `   1. On-demand history sync may only work for primary devices (phones)\n`,
      );
      ...
      process.stdout.write(
        `   4. There's a rate limit or the request was rejected server-side\n\n`,
      );
      ...
      process.stdout.write(
        `   - Use syncFullHistory on connection (syncs recent chats automatically)\n`,
      );
  ...
  it("should demonstrate message store usage", async () => {
    ...
    if (storedMessages.length > 0) {
      ...
      storedMessages.slice(0, 3).forEach((stored, idx) => {
        ...
        const body =
          msg.message?.conversation || msg.message?.extendedTextMessage?.text || "(media/other)";
