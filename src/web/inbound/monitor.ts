...
export async function monitorWebInbox(options: {
  ...
  // Identity-resolved message before access control.
  // Split from access control so message_observed hook can fire in between.
  type ResolvedInboundMessage = {
    id?: string;
    remoteJid: string;
    group: boolean;
  ...
  type NormalizedInboundMessage = ResolvedInboundMessage & {
    access: Awaited<ReturnType<typeof checkInboundAccessControl>>;
  };

  // Phase 1: identity resolution + dedup (no access control)
  const resolveInboundIdentity = async (msg: WAMessage): Promise<ResolvedInboundMessage | null> => {
  ...
  // Phase 2: access control (runs after message_observed hook)
  const applyAccessControl = async (
    ...
    const access = await checkInboundAccessControl({
      ...
      messageTimestampMs: resolved.messageTimestampMs,
      connectedAtMs,
  ...
  const handleMessagesUpsert = async (upsert: { type?: string; messages?: Array<WAMessage> }) => {
    ...
    for (const msg of upsert.messages ?? []) {
      ...
      // Phase 1: identity resolution + dedup (before access control)
      const resolved = await resolveInboundIdentity(msg);
      if (!resolved) {
        continue;
      }
          ...
          const observedResult = await hookRunner.runMessageObserved(
            {
              ...
              timestamp: resolved.messageTimestampMs,
              fromMe: Boolean(msg.key?.fromMe),
              metadata: {
                remoteJid: resolved.remoteJid,
                pushName: msg.pushName ?? undefined,
                senderE164: resolved.senderE164 ?? undefined,
                group: resolved.group,
                groupSubject: resolved.groupSubject ?? undefined,
              },
            },
            {
              channelId: "whatsapp",
              accountId: options.accountId,
              conversationId: resolved.from,
            },
          );
          if (observedResult?.handled) {
            logVerbose(
              `monitor: message_observed hook handled message from ${resolved.from}, skipping pipeline`,
            );
            continue;
          }
        } catch (err) {
      ...
      // Phase 2: access control (after message_observed hook)
      const inbound = await applyAccessControl(msg, resolved);
      if (!inbound) {
        continue;
      }
