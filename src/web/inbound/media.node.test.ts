...
describe("downloadInboundMedia", () => {
  ...
  it("uses explicit mimetype from audioMessage when present", async () => {
    await expectMimetype(
      { audioMessage: { mimetype: "audio/mp4", ptt: true } },
      "audio/mp4",
    );
  });
  ...
  it.each([
    {
      name: "voice messages without explicit MIME",
  ...
  it.each([
    ...
    "defaults MIME for $name messages without explicit MIME",
    async ({ message, mimetype }) => {
