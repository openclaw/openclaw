...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "DNA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("DNA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv DNA_GATEWAY_TOKEN");
    expect(message).not.toContain("DNA_GATEWAY_PASSWORD");
  });
