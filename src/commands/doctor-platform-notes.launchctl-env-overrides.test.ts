...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "EASYHUB_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("EASYHUB_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv EASYHUB_GATEWAY_TOKEN");
    expect(message).not.toContain("EASYHUB_GATEWAY_PASSWORD");
  });
