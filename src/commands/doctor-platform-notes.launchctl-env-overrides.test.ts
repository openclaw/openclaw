...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ACTIVI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ACTIVI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ACTIVI_GATEWAY_TOKEN");
    expect(message).not.toContain("ACTIVI_GATEWAY_PASSWORD");
  });
