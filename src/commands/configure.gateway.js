...
import { maybeAddTailnetOriginToControlUiAllowedOrigins, TAILSCALE_DOCS_LINES, TAILSCALE_EXPOSURE_OPTIONS, TAILSCALE_MISSING_BIN_NOTE_LINES, } from "../gateway/gateway-config-prompts.shared.js";
...
export async function promptGatewayConfig(cfg, runtime) {
    ...
    let next = cfg;
    if (authMode === "token") {
        const tokenInputMode = guardCancel(await select({
