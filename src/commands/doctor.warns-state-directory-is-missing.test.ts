...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "EasyHub-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.EASYHUB_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when EASYHUB_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/easyhub.json",
      exists: true,
      raw: "{}",
    ...
    process.env.EASYHUB_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
