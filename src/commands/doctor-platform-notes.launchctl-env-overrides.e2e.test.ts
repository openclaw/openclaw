...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SMART_AGENT_NEO_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SMART_AGENT_NEO_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SMART_AGENT_NEO_GATEWAY_TOKEN");
    expect(message).not.toContain("SMART_AGENT_NEO_GATEWAY_PASSWORD");
  });
