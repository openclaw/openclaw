...
type EnvSnapshot = {
  HOME?: string;
  MULLUSI_HOME?: string;
  MULLUSI_STATE_DIR?: string;
  MULLUSI_OAUTH_DIR?: string;
};
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    MULLUSI_OAUTH_DIR: process.env.MULLUSI_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: MullusiConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: MullusiConfig) {
...
async function runStateIntegrityText(cfg: MullusiConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "mullusi-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.MULLUSI_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when MULLUSI_OAUTH_DIR is explicitly configured", async () => {
    process.env.MULLUSI_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: MullusiConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints mullusi-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: MullusiConfig = {};
    ...
    expect(text).toMatch(/mullusi sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/mullusi sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /mullusi sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
