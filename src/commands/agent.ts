...
async function persistAcpTurnTranscript(params: {
  ...
  const hadSessionFile = await fsPromises
    .access(sessionFile)
    .then(() => true)
    .catch(() => false);
...
async function agentCommandInternal(
  ...
  try {
    ...
    try {
      ...
      // Run beforeMessage hook for the selected model (e.g. WOL + KV cache restore).
      {
        const startupModelKey = modelKey(provider, model);
        const hookCfg =
          agentCfg?.models?.[startupModelKey]?.beforeMessage ?? agentCfg?.beforeMessage;
        if (hookCfg?.command) {
      ...
      // Run afterResponse hook for the model that actually handled the turn (fire-and-forget).
      {
        const usedModelKey = modelKey(fallbackProvider, fallbackModel);
        const hookCfg = agentCfg?.models?.[usedModelKey]?.afterResponse ?? agentCfg?.afterResponse;
        if (hookCfg?.command) {
          void runBeforeModelChangeHook({
            hookCfg,
            previousProvider: fallbackProvider,
