...
import {
  buildAihubmixProvider,
  buildXiaomiProvider,
  AIHUBMIX_DEFAULT_MODEL_ID,
...
export function applyAihubmixProviderConfig(cfg: OpenClawConfig): OpenClawConfig {
  const models = { ...cfg.agents?.defaults?.models };
  models[AIHUBMIX_DEFAULT_MODEL_REF] = {
    ...models[AIHUBMIX_DEFAULT_MODEL_REF],
    alias: models[AIHUBMIX_DEFAULT_MODEL_REF]?.alias ?? "AIHubMix",
  };

  const providers = { ...cfg.models?.providers };
  const existingProvider = providers.aihubmix;
  const defaultProvider = buildAihubmixProvider();
  const existingModels = Array.isArray(existingProvider?.models) ? existingProvider.models : [];
  const defaultModels = defaultProvider.models ?? [];
  const hasDefaultModel = existingModels.some((model) => model.id === AIHUBMIX_DEFAULT_MODEL_ID);
  const mergedModels =
    existingModels.length > 0
      ? hasDefaultModel
        ? existingModels
...
export function applyAihubmixConfig(cfg: OpenClawConfig): OpenClawConfig {
  const next = applyAihubmixProviderConfig(cfg);
  const existingModel = next.agents?.defaults?.model;
  return {
    ...next,
    agents: {
      ...
      defaults: {
        ...
        model: {
          ...(existingModel && "fallbacks" in (existingModel as Record<string, unknown>)
            ...
            : undefined),
          primary: AIHUBMIX_DEFAULT_MODEL_REF,
        },
      },
    },
  };
