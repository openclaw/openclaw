...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "MULLUSI_GATEWAY_TOKEN",
          placeholder: "MULLUSI_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like MULLUSI_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Mullusi will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Mullusi trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.mullusi.com/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
