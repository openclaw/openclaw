...
export function listLegacyXSearchConfigPaths(raw) {
...
export function migrateLegacyXSearchConfig(raw) {
    if (!isRecord(raw)) {
    ...
    if (auth !== undefined) {
        ...
        if (!existingWebSearch) {
            ...
            changes.push(`Removed ${X_SEARCH_LEGACY_PATH}.apiKey (${XAI_WEB_SEARCH_PLUGIN_KEY_PATH} already set).`);
        }
    }
