...
export function isStalePluginAutoRepairBlocked(cfg, env) {
    return collectPluginRegistryState(cfg, env).hasDiscoveryErrors;
}
export function scanStalePluginConfig(cfg, env) {
...
export function collectStalePluginConfigWarnings(params) {
    ...
    if (params.autoRepairBlocked) {
        lines.push(`- Auto-removal is paused because plugin discovery currently has errors. Fix plugin discovery first, then rerun "${params.doctorFixCommand}".`);
    }
    else {
        lines.push(`- Run "${params.doctorFixCommand}" to remove stale plugins.allow and plugins.entries ids.`);
    }
    return lines.map((line) => sanitizeForLog(line));
}
export function maybeRepairStalePluginConfig(cfg, env) {
    const plugins = asObjectRecord(cfg.plugins);
    if (!plugins) {
    ...
    if (allowIds.length > 0) {
        changes.push(`- plugins.allow: removed ${allowIds.length} stale plugin id${allowIds.length === 1 ? "" : "s"} (${allowIds.join(", ")})`);
    }
    if (entryIds.length > 0) {
        changes.push(`- plugins.entries: removed ${entryIds.length} stale plugin entr${entryIds.length === 1 ? "y" : "ies"} (${entryIds.join(", ")})`);
    }
