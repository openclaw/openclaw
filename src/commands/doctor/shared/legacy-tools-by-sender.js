...
export function scanLegacyToolsBySenderKeys(cfg) {
    const hits = [];
...
export function maybeRepairLegacyToolsBySenderKeys(cfg) {
    ...
    for (const [pathLabel, row] of summary) {
        ...
        if (row.dropped > 0) {
            changes.push(`- ${pathLabel}: removed ${row.dropped} legacy key${row.dropped === 1 ? "" : "s"} where typed id: entries already existed.`);
        }
    }
