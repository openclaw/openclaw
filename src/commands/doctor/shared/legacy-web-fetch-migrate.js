...
export function listLegacyWebFetchConfigPaths(raw) {
    const fetch = resolveLegacyFetchConfig(raw);
...
export function normalizeLegacyWebFetchConfig(raw) {
    if (!isRecord(raw)) {
        return raw;
    }
    const fetch = resolveLegacyFetchConfig(raw);
...
export function migrateLegacyWebFetchConfig(raw) {
...
export function resolvePluginWebFetchConfig(config, pluginId) {
