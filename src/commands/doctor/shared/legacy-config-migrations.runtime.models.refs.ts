...
export function migrateExplicitDefaultModelAllowPolicy(
  ...
  // When the migration marker is already stamped, computeModelPolicyAllowlist
  // returns null — treat this as "already migrated" and skip.
  if (defaultAllow === null) {
    return;
  }
  // When every legacy key is rejected (all-bare map), do NOT write the empty
  // allowlist or stamp the migration marker. Stamping would retire the legacy
