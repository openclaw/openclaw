...
import { formatChannelAccountsDefaultPath, formatSetExplicitDefaultInstruction, formatSetExplicitDefaultToConfiguredInstruction, } from "../../../routing/default-account-warnings.js";
...
function collectChannelsMissingDefaultAccount(cfg) {
    ...
    for (const [channelKey, rawChannel] of Object.entries(channels)) {
        ...
        const normalizedAccountIds = Array.from(new Set(Object.keys(accounts)
...
export function collectMissingDefaultAccountBindingWarnings(cfg) {
...
export function collectMissingExplicitDefaultAccountWarnings(cfg) {
    const warnings = [];
    for (const { channelKey, channel, normalizedAccountIds } of collectChannelsMissingDefaultAccount(cfg)) {
        ...
        const preferredDefault = normalizeOptionalAccountId(typeof channel.defaultAccount === "string" ? channel.defaultAccount : undefined);
        if (preferredDefault) {
