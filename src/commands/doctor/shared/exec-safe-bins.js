...
import { getTrustedSafeBinDirs, isTrustedSafeBinPath, normalizeTrustedSafeBinDirs, } from "../../../infra/exec-safe-bin-trust.js";
...
function normalizeConfiguredSafeBins(entries) {
    ...
    return Array.from(new Set(entries
        ...
        .filter((entry) => entry.length > 0))).toSorted();
}
...
export function scanExecSafeBinCoverage(cfg) {
    const hits = [];
...
export function scanExecSafeBinTrustedDirHints(cfg) {
    const hits = [];
    for (const scope of collectExecSafeBinScopes(cfg)) {
        for (const bin of scope.safeBins) {
...
export function collectExecSafeBinCoverageWarnings(params) {
    ...
    if (interpreterHits.length > 0) {
        for (const hit of interpreterHits.slice(0, 5)) {
            lines.push(`- ${sanitizeForLog(hit.scopePath)}.safeBins includes interpreter/runtime '${sanitizeForLog(hit.bin)}' without profile.`);
        }
    ...
    if (customHits.length > 0) {
        for (const hit of customHits.slice(0, 5)) {
            lines.push(`- ${sanitizeForLog(hit.scopePath)}.safeBins entry '${sanitizeForLog(hit.bin)}' is missing safeBinProfiles.${sanitizeForLog(hit.bin)}.`);
        }
    ...
    if (riskyHits.length > 0) {
        for (const hit of riskyHits.slice(0, 5)) {
            lines.push(`- ${sanitizeForLog(hit.scopePath)}.safeBins includes '${sanitizeForLog(hit.bin)}': ${sanitizeForLog(hit.warning ?? "prefer explicit allowlist entries or approval-gated runs.")}`);
        }
...
export function collectExecSafeBinTrustedDirHintWarnings(hits) {
    if (hits.length === 0) {
        return [];
    }
    const lines = hits
        .slice(0, 5)
        .map((hit) => `- ${sanitizeForLog(hit.scopePath)}.safeBins entry '${sanitizeForLog(hit.bin)}' resolves to '${sanitizeForLog(hit.resolvedPath)}' outside trusted safe-bin dirs.`);
    if (hits.length > 5) {
...
export function maybeRepairExecSafeBinProfiles(cfg) {
