import { defineLegacyConfigMigration, getRecord, mergeMissing, } from "../../../config/legacy.shared.js";
import { isBlockedObjectKey } from "../../../config/prototype-keys.js";
const LEGACY_TTS_PROVIDER_KEYS = ["openai", "elevenlabs", "microsoft", "edge"];
...
export const LEGACY_CONFIG_MIGRATIONS_RUNTIME_TTS = [
    defineLegacyConfigMigration({
        ...
        apply: (raw, changes) => {
            ...
            for (const [pluginId, entryValue] of Object.entries(pluginEntries)) {
                ...
                const entry = getRecord(entryValue);
                const config = getRecord(entry?.config);
                migrateLegacyTtsConfig(getRecord(config?.tts), `plugins.entries.${pluginId}.config.tts`, changes);
            }
        },
    }),
];
