...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  for (const [channelId, rawChannel] of Object.entries(channels)) {
    ...
    // Skip channels that already have named accounts (non-default).
    // These configs already have a proper multi-account structure and should not
    // have top-level keys migrated into a new accounts.default.
    const namedAccountCount = accountKeys.filter(
      (key) => normalizeOptionalLowercaseString(key) !== DEFAULT_ACCOUNT_ID,
    ).length;
    if (namedAccountCount > 0) {
      continue;
    }
