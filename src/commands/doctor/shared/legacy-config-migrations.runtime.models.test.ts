...
describe("stale contextWindow migration", () => {
  ...
  it.each(["deepseek-v4-flash", "deepseek/deepseek-v4-flash"])(
    "repairs stale native %s context windows",
    (id) => {
      const model = { id, contextWindow: 200_000, maxTokens: 61_440 };
      const raw = { models: { providers: { deepseek: { models: [model] } } } };
      const changes: string[] = [];
      expect(migration!.legacyRules?.[0]?.match?.(raw.models.providers, raw)).toBe(true);

      migration!.apply(raw, changes);

      expect(model).toEqual({ id, contextWindow: 1_000_000, maxTokens: 61_440 });
      expect(changes).toHaveLength(1);
  ...
  it.each([
    { provider: "deepseek", id: "deepseek-v4-flash", contextWindow: 500_000 },
    { provider: "custom", id: "deepseek-v4-flash", contextWindow: 200_000 },
    { provider: "openrouter", id: "deepseek/deepseek-v4-flash", contextWindow: 200_000 },
    { provider: "openai", id: "gpt-4o", contextWindow: 128_000 },
  ])("preserves non-stale $provider/$id at $contextWindow", ({ provider, id, contextWindow }) => {
    const model = { id, contextWindow };
    const raw = { models: { providers: { [provider]: { models: [model] } } } };
    const changes: string[] = [];

    migration!.apply(raw, changes);

    expect(model).toEqual({ id, contextWindow });
  ...
  it.each([
    {},
    { models: { providers: { deepseek: { models: "not-an-array" } } } },
    { models: { providers: { deepseek: { models: [{ contextWindow: 200_000 }] } } } },
  ])("leaves malformed model config untouched: %j", (raw) => {
    const before = structuredClone(raw);
    const changes: string[] = [];
