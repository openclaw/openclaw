...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      ...
      "- If the Codex plugin is not installed, run `openclaw plugins install npm:@openclaw/codex --acknowledge-non-clawhub-install` first. Then run `openclaw migrate plan codex` to inventory them. Applying that migration copies skills into the current OpenClaw agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
