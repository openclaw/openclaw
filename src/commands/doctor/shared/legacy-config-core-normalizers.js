...
export function normalizeLegacyCommandsConfig(cfg, changes) {
...
export function normalizeLegacyBrowserConfig(cfg, changes) {
    ...
    if (isRecord(rawSsrFPolicy) && "allowPrivateNetwork" in rawSsrFPolicy) {
        ...
        if (typeof legacyAllowPrivateNetwork === "boolean" ||
...
export function seedMissingDefaultAccountsFromSingleAccountBase(cfg, changes) {
    ...
    for (const [channelId, rawChannel] of Object.entries(channels)) {
        ...
        const hasDefault = accountKeys.some((key) => normalizeOptionalLowercaseString(key) === DEFAULT_ACCOUNT_ID);
        if (hasDefault) {
            continue;
        }
...
function normalizeLegacyRuntimeAgentModelConfig(raw) {
    ...
    const next = { ...raw, primary: migratedPrimary.ref };
...
function ensureEmbeddedHarnessRuntime(raw, selectedRuntime) {
    ...
    if (!currentRuntime || currentRuntime === "auto") {
        return {
            value: { ...raw, runtime: selectedRuntime },
...
export function normalizeLegacyRuntimeModelRefs(cfg, changes) {
...
export function normalizeLegacyOpenAICodexModelsAddMetadata(cfg, changes) {
...
export function normalizeLegacyNanoBananaSkill(cfg, changes) {
    ...
    const legacyApiKey = legacyEnvApiKey ||
...
export function normalizeLegacyCrossContextMessageConfig(cfg, changes) {
    ...
    if (legacyAllowCrossContextSend) {
        ...
        changes.push("Removed tools.message.allowCrossContextSend=false (default cross-context policy already matches canonical settings).");
    }
    return {
        ...cfg,
        tools: {
...
function migrateLegacyDeepgramCompat(params) {
    ...
    params.changes.push(hadCanonicalDeepgram
...
export function normalizeLegacyMediaProviderOptions(cfg, changes) {
    const rawTools = cfg.tools;
    if (!isRecord(rawTools)) {
        return cfg;
    }
...
export function normalizeLegacyMistralModelMaxTokens(cfg, changes) {
    ...
    for (const [providerId, rawProvider] of Object.entries(rawProviders)) {
        ...
        const nextModels = rawModels.map((model, index) => {
            ...
            const contextWindow = typeof model.contextWindow === "number" && Number.isFinite(model.contextWindow)
                ? model.contextWindow
                : null;
            const maxTokens = typeof model.maxTokens === "number" && Number.isFinite(model.maxTokens)
                ? model.maxTokens
                : null;
