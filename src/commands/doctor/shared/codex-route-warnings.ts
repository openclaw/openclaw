...
function recordCodexModelHit(params: {
  ...
  const runtime = normalizeString(params.runtime);
  if (runtime !== "codex") {
    return undefined;
  }
  const canonicalModel = toOpenAICodexModelRef(params.model);
  if (!canonicalModel) {
    return undefined;
  }
  params.hits.push({
...
function collectStringModelSlot(params: {
  ...
  if (!model || !isOpenAIModelRef(model)) {
    return false;
  }
...
export function collectCodexRouteWarnings(params: {
  ...
  return [
    [
      "- Generic `openai/*` model refs are paired with the Codex runtime.",
      ...hits.map(
        (hit) =>
          `- ${hit.path}: ${hit.model} will use runtime "${hit.runtime}"; use ${hit.canonicalModel} to keep the Codex OAuth/provider route, or switch the runtime/provider intentionally.`,
      ),
      "- Run `openclaw doctor --fix` to rewrite these risky config model refs to `openai-codex/*` without adding global runtime policy.",
    ].join("\n"),
  ];
}
...
function rewriteSessionModelPair(params: {
  ...
  const runtime = normalizeString(params.entry.agentRuntimeOverride ?? params.entry.agentHarnessId);
  ...
  if (model && isOpenAIModelRef(model) && runtime === "codex") {
    const canonicalModel = toOpenAICodexModelRef(model);
    if (canonicalModel) {
...
export function repairCodexSessionStoreRoutes(params: {
  ...
  for (const [sessionKey, entry] of Object.entries(params.store)) {
    ...
    const changedRuntimePins = changedFallbackNotice ? clearStaleSessionRuntimePins(entry) : false;
