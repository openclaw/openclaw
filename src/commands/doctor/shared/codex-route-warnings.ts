...
type PreRepairRuntimePin = {
  runtimeId: string;
  // Doctor-local refinement: resolver source "model" can mean an agent model map
  // or a provider catalog entry, but only the latter needs an eager policy write.
...
function providerModelExplicitNonDefaultRuntimeId(params: {
  ...
  for (const [providerId, providerConfig] of Object.entries(providers ?? {})) {
    ...
    for (const model of models) {
      ...
      // Explicit Codex catalog pins already match the repaired OpenAI default.
