...
import { countStaleDreamingJobs, migrateLegacyDreamingPayloadShape, } from "./doctor-cron-dreaming-payload-migration.js";
...
function formatLegacyIssuePreview(issues) {
    ...
    if (issues.legacyPayloadProvider) {
        lines.push(`- ${pluralize(issues.legacyPayloadProvider, "job")} still uses payload \`provider\` as a delivery alias`);
    }
...
export async function maybeRepairLegacyCronStore(params) {
    ...
    if (dreamingStaleCount > 0) {
        previewLines.push(`- ${pluralize(dreamingStaleCount, "managed dreaming job")} still has the legacy heartbeat-coupled shape`);
    }
    ...
    if (changed) {
        ...
        if (dreamingMigration.rewrittenCount > 0) {
            note(`Rewrote ${pluralize(dreamingMigration.rewrittenCount, "managed dreaming job")} to run as an isolated agent turn so dreaming no longer requires heartbeat.`, "Doctor changes");
        }
    }
