...
const ensureSmartAgentNeoModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveSmartAgentNeoAgentDir = vi.fn().mockReturnValue("/tmp/smart-agent-neo-agent");
const ensureAuthProfileStore = vi.fn().mockReturnValue({ version: 1, profiles: {} });
const listProfilesForProvider = vi.fn().mockReturnValue([]);
const resolveAuthProfileDisplayLabel = vi.fn(({ profileId }: { profileId: string }) => profileId);
const resolveAuthStorePathForDisplay = vi
  .fn()
  .mockReturnValue("/tmp/smart-agent-neo-agent/auth-profiles.json");
...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "/tmp/smart-agent-neo.json",
  STATE_DIR: "/tmp/smart-agent-neo-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureSmartAgentNeoModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveSmartAgentNeoAgentDir,
}));
