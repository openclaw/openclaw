...
beforeEach(() => {
  originalAgentDir = process.env.SMART_AGENT_NEO_AGENT_DIR;
  originalPiAgentDir = process.env.PI_CODING_AGENT_DIR;
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-agent-neo-auth-"));
  process.env.SMART_AGENT_NEO_AGENT_DIR = tempAgentDir;
  process.env.PI_CODING_AGENT_DIR = tempAgentDir;
});

afterEach(() => {
  if (originalAgentDir === undefined) {
    delete process.env.SMART_AGENT_NEO_AGENT_DIR;
  } else {
    process.env.SMART_AGENT_NEO_AGENT_DIR = originalAgentDir;
  }
