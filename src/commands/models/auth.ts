...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("mullusi plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("mullusi plugins install")}\`.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is a legacy/manual path in Mullusi.");
    runtime.log(
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("mullusi plugins install")}\`.`,
    );
  }
