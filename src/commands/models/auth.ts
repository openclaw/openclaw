...
import { readConfigFileSnapshot, type DNAConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: DNAConfig, model: string): DNAConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("dna plugins install")}\`.`,
    );
  }
