...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveDNAAgentDir: vi.fn().mockReturnValue("/tmp/dna-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveDNAAgentDir: mocks.resolveDNAAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/dna-agent/auth-profiles.json");
