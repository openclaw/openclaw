...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMullusiAgentDir: vi.fn().mockReturnValue("/tmp/mullusi-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/mullusi-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/mullusi-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/mullusi-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/mullusi-dev/mullusi.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveMullusiAgentDir: mocks.resolveMullusiAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/mullusi-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMullusiAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/mullusi-dev/mullusi.json");
    expect(payload.auth.storePath).toBe("/tmp/mullusi-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/mullusi-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/mullusi-agent-custom");
