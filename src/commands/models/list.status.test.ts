...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveActiviAgentDir: vi.fn().mockReturnValue("/tmp/activi-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/activi-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveActiviAgentDir: mocks.resolveActiviAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/activi-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveActiviAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/activi-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/activi-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/activi-agent-custom");
