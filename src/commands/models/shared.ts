...
export async function loadValidConfigOrThrow(): Promise<ActiviConfig> {
...
export async function updateConfig(
  mutator: (cfg: ActiviConfig) => ActiviConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ActiviConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ActiviConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("activi agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
