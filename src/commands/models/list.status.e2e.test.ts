...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSmartAgentNeoAgentDir: vi.fn().mockReturnValue("/tmp/smart-agent-neo-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/smart-agent-neo-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSmartAgentNeoAgentDir: mocks.resolveSmartAgentNeoAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSmartAgentNeoAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/smart-agent-neo-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/smart-agent-neo-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/smart-agent-neo-agent-custom");
