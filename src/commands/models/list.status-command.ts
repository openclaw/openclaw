...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveEasyHubAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("EasyHub models auth setup-token")}\` or \`${formatCliCommand("EasyHub configure")}\`.`
          : `Run \`${formatCliCommand("EasyHub configure")}\` or set an API key env var.`;
