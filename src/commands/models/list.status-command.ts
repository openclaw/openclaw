...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("dna models auth setup-token")}\` or \`${formatCliCommand("dna configure")}\`.`
          : `Run \`${formatCliCommand("dna configure")}\` or set an API key env var.`;
