...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSmartAgentNeoAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("smart-agent-neo models auth setup-token")}\` or \`${formatCliCommand("smart-agent-neo configure")}\`.`
          : `Run \`${formatCliCommand("smart-agent-neo configure")}\` or set an API key env var.`;
