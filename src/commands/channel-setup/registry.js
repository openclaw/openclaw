...
const setupWizardAdapters = new WeakMap();
function isChannelSetupWizardAdapter(setupWizard) {
    return Boolean(setupWizard &&
...
export function resolveChannelSetupWizardAdapterForPlugin(plugin) {
    if (!plugin) {
        return undefined;
    }
...
export function getChannelSetupWizardAdapter(channel) {
    return getChannelSetupWizardAdapterMap().get(channel);
}
export function listChannelSetupWizardAdapters() {
