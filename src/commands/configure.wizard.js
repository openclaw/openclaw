...
import { applyWizardMetadata, DEFAULT_WORKSPACE, ensureWorkspaceAndSessions, guardCancel, probeGatewayReachable, resolveControlUiLinks, summarizeExistingConfig, waitForGatewayReachable, } from "./onboard-helpers.js";
...
async function promptWebToolsConfig(nextConfig, runtime, prompter) {
    ...
    if (enableSearch) {
        ...
        if (codexRelevant) {
            ...
            const enableCodexNative = guardCancel(await confirm({
...
export async function runConfigureWizard(opts, runtime = defaultRuntime) {
    try {
        ...
        const configureWorkspace = async () => {
            ...
            workspaceDir = resolveUserPath(normalizeOptionalString(workspaceInput ?? "") || DEFAULT_WORKSPACE);
            if (!snapshot.exists) {
                const indicators = ["MEMORY.md", "memory", ".git"].map((name) => nodePath.join(workspaceDir, name));
                const hasExistingContent = (await Promise.all(indicators.map(async (candidate) => {
                    try {
        ...
        const newPassword = process.env.OPENCLAW_GATEWAY_PASSWORD ??
            (await resolveGatewaySecretInputForWizard({
                cfg: nextConfig,
                value: nextConfig.gateway?.auth?.password,
                path: "gateway.auth.password",
            }));
        const oldPassword = process.env.OPENCLAW_GATEWAY_PASSWORD ??
            (await resolveGatewaySecretInputForWizard({
                cfg: baseConfig,
                value: baseConfig.gateway?.auth?.password,
                path: "gateway.auth.password",
