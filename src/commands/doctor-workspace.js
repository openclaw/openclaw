...
import { CANONICAL_ROOT_MEMORY_FILENAME, LEGACY_ROOT_MEMORY_FILENAME, resolveCanonicalRootMemoryPath, resolveLegacyRootMemoryPath, resolveRootMemoryRepairDir, } from "../memory/root-memory-files.js";
...
export async function detectRootMemoryFiles(workspaceDir) {
...
export function formatRootMemoryFilesWarning(detection) {
...
async function moveLegacyRootMemoryFileToArchive(params) {
    ...
    const archiveDir = path.join(repairDir, new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-"));
...
export async function migrateLegacyRootMemoryFile(workspaceDir) {
...
export async function noteWorkspaceMemoryHealth(cfg) {
    try {
