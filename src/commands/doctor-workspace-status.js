...
function noteFlowRecoveryHints() {
    const suspicious = listTaskFlowRecords().flatMap((flow) => {
        ...
        if (flow.syncMode === "managed" &&
            ...
            findings.push(`${flow.flowId}: running managed TaskFlow has no linked tasks or wait state; inspect or cancel it manually.`);
        }
        if (flow.status === "blocked" &&
            flow.blockedTaskId &&
            !tasks.some((task) => task.taskId === flow.blockedTaskId)) {
            findings.push(`${flow.flowId}: blocked TaskFlow points at missing task ${flow.blockedTaskId}; inspect before retrying.`);
        }
        return findings;
    });
