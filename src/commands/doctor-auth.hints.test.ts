...
describe("resolveUnusableProfileHint", () => {
  it("returns billing guidance for disabled billing profiles", () => {
    expect(
      resolveUnusableProfileHint({ kind: "disabled", reason: "billing" }),
    ).toBe("Top up credits (provider billing) or switch provider.");
  });
  ...
  it("returns credential guidance for permanent auth disables", () => {
    ...
    ).toBe("Refresh or replace credentials, then retry.");
  });

  it("falls back to cooldown guidance for non-billing disable reasons", () => {
    expect(
      resolveUnusableProfileHint({ kind: "disabled", reason: "unknown" }),
    ).toBe("Wait for cooldown or switch provider.");
  });
