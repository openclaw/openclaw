  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        `  Set locally: OPENCLAW_STATE_DIR=~/.openclaw ${formatCliCommand("evox doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`evox sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`evox sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`evox sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
