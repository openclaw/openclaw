...
export const resolveSmartAgentNeoPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadSmartAgentNeoPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/smart-agent-neo-root.js", () => ({
  resolveSmartAgentNeoPackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveSmartAgentNeoPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SMART_AGENT_NEO_STATE_DIR;
  originalUpdateInProgress = process.env.SMART_AGENT_NEO_UPDATE_IN_PROGRESS;
  process.env.SMART_AGENT_NEO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-agent-neo-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SMART_AGENT_NEO_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SMART_AGENT_NEO_UPDATE_IN_PROGRESS;
  } else {
    process.env.SMART_AGENT_NEO_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
