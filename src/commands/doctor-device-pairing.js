...
import { listApprovedPairedDeviceRoles, listDevicePairing, summarizeDeviceTokens, } from "../infra/device-pairing.js";
...
function describeDevice(params) {
    const label = sanitizeTerminalText(params.displayName?.trim() || "") ||
...
function collectPairedRecordIssues(snapshot) {
    ...
    for (const device of snapshot.paired) {
        ...
        if (approvedRoles.includes("operator") && approvedScopes.length === 0) {
            lines.push(`- Paired device ${deviceLabel} is missing its approved operator scope baseline. Scope upgrades can get stuck in pairing-required until the device repairs or is re-approved.`);
        }
