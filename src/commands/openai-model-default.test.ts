...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: MullusiConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: MullusiConfig) => { changed: boolean; next: MullusiConfig };
  defaultModel: string;
  overrideConfig: MullusiConfig;
  alreadyDefaultConfig: MullusiConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: MullusiConfig) => config,
      applyDefaultConfig: (config: MullusiConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: MullusiConfig = { agents: { defaults: {} } };
