...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: ActiviConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: ActiviConfig) => { changed: boolean; next: ActiviConfig };
  defaultModel: string;
  overrideConfig: ActiviConfig;
  alreadyDefaultConfig: ActiviConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: ActiviConfig) => config,
      applyDefaultConfig: (config: ActiviConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: ActiviConfig = { agents: { defaults: {} } };
