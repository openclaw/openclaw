...
export async function noteMemorySearchHealth(cfg: OpenClawConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`openclaw models auth login --provider ${resolved.provider}`)}`,
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("openclaw models auth login --provider openai")}`,
