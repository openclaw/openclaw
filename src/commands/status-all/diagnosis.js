...
import { formatPortDiagnostics, isDualStackLoopbackGatewayListeners, } from "../../infra/ports.js";
import { summarizeRestartSentinel, } from "../../infra/restart-sentinel.js";
import { formatPluginCompatibilityNotice, } from "../../plugins/status.js";
...
export async function appendStatusAllDiagnosis(params) {
    ...
    if (params.portUsage) {
        ...
        if (!portOk) {
            ...
            lines.push(`  ${muted("Detected dual-stack loopback listeners (127.0.0.1 + ::1) for one gateway process.")}`);
        }
    }
    {
    ...
    if (params.channelsStatus) {
        ...
        emitCheck(`Channel issues skipped (node-only mode; query ${params.nodeOnlyGateway.gatewayTarget})`, "ok");
    }
    else {
