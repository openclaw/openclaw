...
const buildAccountNotes = (params: {
  ...
  if (snapshot.tokenSource && snapshot.tokenSource !== "none") {
    notes.push(`token:${snapshot.tokenSource}`); // pragma: allowlist secret (status string)
  }
  if (snapshot.botTokenSource && snapshot.botTokenSource !== "none") {
    notes.push(`bot:${snapshot.botTokenSource}`); // pragma: allowlist secret (status string)
  }
  if (snapshot.appTokenSource && snapshot.appTokenSource !== "none") {
    notes.push(`app:${snapshot.appTokenSource}`); // pragma: allowlist secret (status string)
  }
  if (snapshot.signingSecretSource && snapshot.signingSecretSource !== "none") {
    notes.push(`signing:${snapshot.signingSecretSource}`); // pragma: allowlist secret (status string)
  }
