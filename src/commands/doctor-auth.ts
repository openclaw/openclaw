...
function pruneAuthProfiles(
  cfg: EasyHubConfig,
  profileIds: Set<string>,
): { next: EasyHubConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("EasyHub models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("EasyHub models auth setup-token")} or ${formatCliCommand(
      "EasyHub configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "EasyHub models auth login --provider openai-codex",
    )} or ${formatCliCommand("EasyHub configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("EasyHub configure")}\` or \`${formatCliCommand("EasyHub onboard")}\`.`;
}
