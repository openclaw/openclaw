...
function pruneAuthProfiles(
  cfg: ActiviConfig,
  profileIds: Set<string>,
): { next: ActiviConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("activi models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("activi models auth setup-token")} or ${formatCliCommand(
      "activi configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "activi models auth login --provider openai-codex",
    )} or ${formatCliCommand("activi configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("activi configure")}\` or \`${formatCliCommand("activi onboard")}\`.`;
}
