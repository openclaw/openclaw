...
function pruneAuthProfiles(
  cfg: SmartAgentNeoConfig,
  profileIds: Set<string>,
): { next: SmartAgentNeoConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("smart-agent-neo models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("smart-agent-neo models auth setup-token")} or ${formatCliCommand(
  ...
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "smart-agent-neo models auth login --provider openai-codex",
    )} or ${formatCliCommand("smart-agent-neo configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("smart-agent-neo configure")}\` or \`${formatCliCommand("smart-agent-neo onboard")}\`.`;
}
