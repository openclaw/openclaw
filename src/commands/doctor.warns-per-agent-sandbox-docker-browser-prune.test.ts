...
beforeEach(() => {
  ...
  resolveEasyHubPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.EASYHUB_STATE_DIR;
  originalUpdateInProgress = process.env.EASYHUB_UPDATE_IN_PROGRESS;
  process.env.EASYHUB_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "EasyHub-doctor-state-"));
  process.env.EASYHUB_STATE_DIR = tempStateDir;
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.EASYHUB_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.EASYHUB_UPDATE_IN_PROGRESS;
  } else {
    process.env.EASYHUB_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveEasyHubPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadEasyHubPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  ...
  it("does not warn when only the active workspace is present", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      ...
      valid: true,
      config: {
        agents: { defaults: { workspace: "/Users/steipete/EasyHub" } },
      },
      issues: [],
      legacyIssues: [],
    });
