...
function collectExecPolicyConflictWarnings(cfg) {
    ...
    const maybeWarn = (params) => {
        ...
        const snapshot = resolveExecPolicyScopeSnapshot({
            ...
            scopeExecConfig,
            globalExecConfig,
            configPath: params.scopeLabel === "tools.exec"
                ? "tools.exec"
        ...
        const securityConflict = securityConfigured &&
...
export async function noteSecurityWarnings(cfg) {
    ...
    if (cfg.approvals?.exec?.enabled === false) {
        warnings.push("- Note: approvals.exec.enabled=false disables approval forwarding only.", "  Host exec gating still comes from ~/.openclaw/exec-approvals.json.", `  Check local policy with: ${formatCliCommand("openclaw approvals get --gateway")}`);
    }
    ...
    if (isExposed) {
        if (!hasSharedSecret) {
            ...
            warnings.push(`- WARNING: Gateway bound to ${bindDescriptor} (network-accessible).`, `  Ensure your auth credentials are strong and not exposed.`, ...saferRemoteAccessLines);
        }
    }
    const warnDmPolicy = async (params) => {
        ...
        if (dmScope === "main" && isMultiUserDm) {
            warnings.push(`- ${params.label} DMs: multiple senders share the main session; run: ` +
