...
import { ensureAuthProfileStore, hasAnyAuthProfileStoreSource, } from "../agents/auth-profiles/store.js";
import { readClaudeCliCredentialsCached } from "../agents/cli-credentials.js";
import { formatCliCommand } from "../cli/command-format.js";
import { resolveExecutablePath } from "../infra/executable-path.js";
import { normalizeOptionalLowercaseString, normalizeOptionalString, resolvePrimaryStringValue, } from "../shared/string-coerce.js";
...
function hasClaudeCliConfigSignals(cfg) {
    ...
    if (Object.keys(backendConfig).some((key) => normalizeOptionalLowercaseString(key) === CLAUDE_CLI_PROVIDER)) {
        return true;
    }
...
export function noteClaudeCliHealth(cfg, deps) {
    ...
    const store = deps?.store ??
        (hasConfigSignals || hasAnyAuthProfileStoreSource()
            ...
            : { version: 1, profiles: {} });
    const readClaudeCliCredentials = deps?.readClaudeCliCredentials ??
    ...
    const resolveCommandPath = deps?.resolveCommandPath ??
    ...
    if (!storedProfile) {
        ...
        lines.push(`- OpenClaw auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`);
    }
    lines.push(formatWorkspaceHealthLine(workspaceDir, workspaceHealth));
    if (workspaceHealth === "readonly" ||
