...
const findExtraGatewayServices = vi.hoisted(() =>
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  beforeEach(() => {
    ...
    findExtraGatewayServices.mockResolvedValue([]);
  ...
  it("skips gateway service install when a system-level OpenClaw gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findExtraGatewayServices.mockResolvedValue([
      {
    ...
    expect(findExtraGatewayServices).toHaveBeenCalledWith(process.env, { deep: true });
    expect(service.install).not.toHaveBeenCalled();
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
