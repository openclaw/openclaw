import type { SmartAgentNeoConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: SmartAgentNeoConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.SMART_AGENT_NEO_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.SMART_AGENT_NEO_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: SmartAgentNeoConfig) {
