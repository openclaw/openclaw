...
import { ensureAuthProfileStore, hasAnyAuthProfileStoreSource, resolveApiKeyForProfile, resolveProfileUnusableUntilForDisplay, } from "../agents/auth-profiles.js";
import { formatAuthDoctorHint } from "../agents/auth-profiles/doctor.js";
import { buildOAuthRefreshFailureLoginCommand, classifyOAuthRefreshFailure, } from "../agents/auth-profiles/oauth-refresh-failure.js";
...
export function noteLegacyCodexProviderOverride(cfg) {
...
export async function resolveAuthIssueHint(issue, cfg, store) {
...
export async function noteAuthProfileHealth(params) {
    if (Object.keys(params.cfg.auth?.profiles ?? {}).length === 0 &&
    ...
    if (issues.length > 0) {
        const issueLines = await Promise.all(issues.map((issue) => formatAuthIssueLine({
            profileId: issue.profileId,
            provider: issue.provider,
            status: issue.status,
