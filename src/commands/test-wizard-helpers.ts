...
export function requireSmartAgentNeoAgentDir(): string {
  const agentDir = process.env.SMART_AGENT_NEO_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SMART_AGENT_NEO_AGENT_DIR not set");
  }
  return agentDir;
}
