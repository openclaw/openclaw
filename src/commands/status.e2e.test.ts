...
beforeAll(() => {
  envSnapshot = captureEnv(["SMART_AGENT_NEO_PROFILE"]);
  process.env.SMART_AGENT_NEO_PROFILE = "isolated";
});
...
vi.mock("../infra/smart-agent-neo-root.js", () => ({
  resolveSmartAgentNeoPackageRoot: vi.fn().mockResolvedValue("/tmp/smart-agent-neo"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/smart-agent-neo/pnpm-lock.yaml",
      markerPath: "/tmp/smart-agent-neo/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("SmartAgentNeo status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("smart-agent-neo status --all") ||
          l.includes("smart-agent-neo --profile isolated status --all") ||
          l.includes("smart-agent-neo status --all") ||
          l.includes("smart-agent-neo --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    const prevToken = process.env.SMART_AGENT_NEO_GATEWAY_TOKEN;
    process.env.SMART_AGENT_NEO_GATEWAY_TOKEN = "abcd1234";
    try {
