...
function stripUnknownConfigKeys(config: DNAConfig): {
  config: DNAConfig;
  removed: string[];
} {
  const parsed = DNASchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as DNAConfig;
...
function noteOpencodeProviderOverrides(cfg: DNAConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.DNA_CONFIG_PATH?.trim() || env.DNA_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "dna.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "dna.json");
  ...
  let cfg: DNAConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as DNAConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("dna doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("dna doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "dna doctor --fix" to remove these keys.');
    }
  }
