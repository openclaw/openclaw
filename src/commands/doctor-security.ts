...
import type { DNAConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: DNAConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("dna security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("dna configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("dna config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("dna doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("dna config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
