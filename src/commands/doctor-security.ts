...
import type { SmartAgentNeoConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: SmartAgentNeoConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("smart-agent-neo security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("smart-agent-neo configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("smart-agent-neo config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("smart-agent-neo doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("smart-agent-neo config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('smart-agent-neo config set session.dmScope "per-channel-peer"') +
