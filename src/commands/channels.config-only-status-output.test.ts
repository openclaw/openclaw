  ...
  return {
    ...
    config: {
      ...
      inspectAccount: () => ({
        ...
        configured: true,
        mode: "http",
        botToken: "resolved-bot", // pragma: allowlist secret (fixture token)
        botTokenSource: "config",
        botTokenStatus: "available",
        signingSecret: "", // pragma: allowlist secret (fixture)
...
describe("config-only channels status output", () => {
  ...
  it("shows configured-but-unavailable credentials distinctly from not configured", async () => {
    ...
    expect(joined).toContain("token:config (unavailable)"); // pragma: allowlist secret (status string)
  });
