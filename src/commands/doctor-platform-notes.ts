...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".smart-agent-neo", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: SmartAgentNeoConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["NEOBOT_GATEWAY_TOKEN", await getenv("NEOBOT_GATEWAY_TOKEN")],
    ["NEOBOT_GATEWAY_PASSWORD", await getenv("NEOBOT_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`SMART_AGENT_NEO_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["SMART_AGENT_NEO_GATEWAY_TOKEN", await getenv("SMART_AGENT_NEO_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["SMART_AGENT_NEO_GATEWAY_PASSWORD", await getenv("SMART_AGENT_NEO_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "SMART_AGENT_NEO_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("NEOBOT_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
  ...
  const lines = [
    ...
    "- Use SMART_AGENT_NEO_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> SMART_AGENT_NEO_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
