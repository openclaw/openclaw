...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".mullusi", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: MullusiConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["MULLUSI_GATEWAY_TOKEN", await getenv("MULLUSI_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["MULLUSI_GATEWAY_PASSWORD", await getenv("MULLUSI_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "MULLUSI_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.MULLUSI_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- MULLUSI_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
  ...
  const suggestions = [
    ...
    "  export NODE_COMPILE_CACHE=/var/tmp/mullusi-compile-cache",
    "  mkdir -p /var/tmp/mullusi-compile-cache",
    "  export MULLUSI_NO_RESPAWN=1",
