...
function validateGatewayWebSocketUrl(value: string): string | undefined {
  ...
  if (
    !isSecureWebSocketUrl(trimmed, {
      allowPrivateWs: process.env.MULLUSI_ALLOW_INSECURE_PRIVATE_WS === "1",
    })
  ) {
    return (
      "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel. " +
      "Break-glass: MULLUSI_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks."
    );
  }
  return undefined;
}
  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        "Docs: https://docs.mullusi.com/gateway/discovery",
      ].join("\n"),
      "Discovery",
    );
  }
      ...
      if (mode === "direct") {
        ...
        if (trusted) {
          ...
          await prompter.note(
            [
              ...
              "If your gateway is loopback-only, choose SSH tunnel and keep ws://127.0.0.1:18790.",
            ].join("\n"),
            "Direct remote",
          );
        } else {
        ...
        await prompter.note(
          [
            ...
            `ssh -N -L 18790:127.0.0.1:18790 <user>@${host}${target.sshPort ? ` -p ${target.sshPort}` : ""}`,
  ...
  if (authChoice === "token") {
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the token directly in Mullusi config",
      },
    });
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForSetup({
        ...
        config: cfg,
        prompter,
        preferredEnvVar: "MULLUSI_GATEWAY_TOKEN",
        copy: {
          sourceMessage: "Where is this gateway token stored?",
          envVarPlaceholder: "MULLUSI_GATEWAY_TOKEN",
        },
      });
      token = resolved.ref;
    } else {
    ...
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the password directly in Mullusi config",
      },
    });
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForSetup({
        ...
        config: cfg,
        prompter,
        preferredEnvVar: "MULLUSI_GATEWAY_PASSWORD",
        copy: {
          sourceMessage: "Where is this gateway password stored?",
          envVarPlaceholder: "MULLUSI_GATEWAY_PASSWORD",
        },
      });
