...
function resolveMode(cfg: SmartAgentNeoConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: SmartAgentNeoConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("smart-agent-neo configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("smart-agent-neo config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("smart-agent-neo setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("smart-agent-neo doctor --fix")}" to apply changes.`);
  }
