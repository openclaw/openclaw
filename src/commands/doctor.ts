...
function resolveMode(cfg: ActiviConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: ActiviConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("activi configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("activi config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("activi setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("activi doctor --fix")}" to apply changes.`);
  }
