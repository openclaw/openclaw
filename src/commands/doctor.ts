...
function resolveMode(cfg: EasyHubConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: EasyHubConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("EasyHub configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("EasyHub config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("EasyHub setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("EasyHub doctor --fix")}" to apply changes.`);
  }
