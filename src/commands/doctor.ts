...
function resolveMode(cfg: DNAConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: DNAConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("dna configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("dna config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("dna setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("dna doctor --fix")}" to apply changes.`);
  }
