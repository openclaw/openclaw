...
export async function setupInternalHooks(
  ...
  await prompter.note(
    [
      ...
      "",
      "Learn more: https://docs.hanzo.bot/automation/hooks",
    ].join("\n"),
    "Hooks",
  );
  ...
  await prompter.note(
    [
      ...
      `  ${formatCliCommand("bot hooks list")}`,
      `  ${formatCliCommand("bot hooks enable <name>")}`,
      `  ${formatCliCommand("bot hooks disable <name>")}`,
    ].join("\n"),
    "Hooks Configured",
  );
