...
function captureEnv(): EnvSnapshot {
  return {
    ...
    disableConfigCache: process.env.EASYHUB_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("EASYHUB_STATE_DIR", prev.stateDir);
  restoreEnvVar("EASYHUB_CONFIG_PATH", prev.configPath);
  restoreEnvVar("EASYHUB_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("EASYHUB_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("EASYHUB_SKIP_CRON", prev.skipCron);
  restoreEnvVar("EASYHUB_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("EASYHUB_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("EASYHUB_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("EASYHUB_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.EASYHUB_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "easyhub.json");
  process.env.HOME = tempHome;
  process.env.EASYHUB_STATE_DIR = tempHome;
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("EasyHub-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("EasyHub-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("EasyHub-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("EasyHub-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("EasyHub-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("EasyHub-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
