...
import { resolveSessionTotalTokens } from "../config/sessions/types.js";
...
const loadStatusSummaryRuntimeModule = createLazyRuntimeSurface(() => import("./status.summary.runtime.js"), ({ statusSummaryRuntime }) => statusSummaryRuntime);
...
export async function getStatusSummary(options = {}) {
    ...
    const channelScopeConfig = options.sourceConfig === undefined
        ? { config: cfg }
    ...
    const linkContext = needsChannelPlugins
        ? await loadLinkChannelModule().then(({ resolveLinkChannelContext }) => resolveLinkChannelContext(cfg, { sourceConfig: options.sourceConfig }))
        : null;
    ...
    const channelSummary = needsChannelPlugins
        ? await loadChannelSummaryModule().then(({ buildChannelSummary }) => buildChannelSummary(cfg, {
            colorize: true,
