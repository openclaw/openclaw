...
export const formatPromptCacheCompact = (sess) => {
    const cacheStats = resolvePromptCacheStats(sess);
    if (!cacheStats) {
        return "";
    }
...
function resolvePromptCacheStats(sess) {
    const cacheRead = typeof sess.cacheRead === "number" && Number.isFinite(sess.cacheRead) && sess.cacheRead >= 0
        ? sess.cacheRead
        : 0;
    const cacheWrite = typeof sess.cacheWrite === "number" && Number.isFinite(sess.cacheWrite) && sess.cacheWrite >= 0
        ? sess.cacheWrite
        : 0;
    if (cacheRead <= 0 && cacheWrite <= 0) {
        return null;
    }
    const inputTokens = typeof sess.inputTokens === "number" &&
    ...
    // tokens.
    const total = promptTokensFromParts ??
...
export const formatDaemonRuntimeShort = (runtime) => {
    ...
    const noisyLaunchctlDetail = runtime.missingUnit === true &&
