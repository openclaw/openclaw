...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByEvox ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByEvox ? "installed · " : "";
  ...
  runtime.log(theme.heading("EVOX.sh status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`evox devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("evox devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("evox devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("evox security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("evox security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.evox.sh/faq");
  runtime.log("Troubleshooting: https://docs.evox.sh/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("evox status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("evox logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("evox status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("evox gateway probe")}`);
  }
}
