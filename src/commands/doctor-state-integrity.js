...
import { formatSessionArchiveTimestamp, isPrimarySessionTranscriptFileName, } from "../config/sessions/artifacts.js";
...
export function detectLinuxSdBackedStateDir(stateDir, deps) {
...
export function formatLinuxSdBackedStateDirWarning(displayStateDir, linuxSdBackedStateDir) {
    const displayMountPoint = linuxSdBackedStateDir.mountPoint === "/"
        ? "/"
...
export function detectMacCloudSyncedStateDir(stateDir, deps) {
    const platform = deps?.platform ?? process.platform;
    if (platform !== "darwin") {
        return null;
    }
    ...
    if (existsDir(sessionsDir)) {
        ...
        if (orphanTranscriptPaths.length > 0 && !suppressOrphanTranscriptWarning) {
            ...
            if (archiveOrphans) {
                ...
                if (archived > 0) {
                    changes.push(`- Archived ${countLabel(archived, "orphan transcript file")} in ${displaySessionsDir} as .deleted timestamped backups.`);
                }
            }
        }
    }
