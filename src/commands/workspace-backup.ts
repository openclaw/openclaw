...
async function assertWorkspaceTreeHasNoSymlinks(rootPath: string): Promise<void> {
  const rootStat = await fs.lstat(rootPath);
  if (rootStat.isSymbolicLink()) {
    throw new Error(`Workspace backup source contains symbolic links: ${rootPath}`);
  }
  ...
  while (pending.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) {
        throw new Error(`Workspace backup source contains symbolic links: ${entryPath}`);
      }
...
export async function workspaceBackupRunCommand(
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    try {
      await assertWorkspaceTreeHasNoSymlinks(workspaceDir);
      ...
      await assertWorkspaceTreeHasNoSymlinks(stagedDir);
