...
async function ensureDirectoryPathWithoutSymlinks(dirPath: string): Promise<void> {
  ...
  while (true) {
    try {
      ...
      if (stat.isSymbolicLink()) {
        throw new Error(`Refusing to restore through a symbolic link path segment: ${probe}`);
      }
      if (!stat.isDirectory()) {
        throw new Error(`Refusing to restore through a non-directory path segment: ${probe}`);
      }
      break;
    } catch (error) {
  ...
  let current = probe;
  for (const segment of pendingSegments.toReversed()) {
    current = path.join(current, segment);
    try {
      const stat = await fs.lstat(current);
      if (stat.isSymbolicLink()) {
        throw new Error(`Refusing to restore through a symbolic link path segment: ${current}`);
      }
      if (!stat.isDirectory()) {
        throw new Error(`Refusing to restore through a non-directory path segment: ${current}`);
      }
      continue;
    } catch (error) {
      if (!isNotFoundError(error)) {
...
async function copySourceToTarget(sourcePath: string, targetPath: string): Promise<void> {
  ...
  await ensureDirectoryPathWithoutSymlinks(targetParent);
  const tempTargetPath = path.join(targetParent, `.openclaw-restore-${randomUUID()}`);
  if (stat.isDirectory()) {
    await assertTreeContainsNoSymlinks(sourcePath);
    try {
      await fs.mkdir(tempTargetPath);
      await fs.cp(sourcePath, tempTargetPath, {
        recursive: true,
        dereference: false,
        force: true,
      });
  ...
  try {
    await fs.copyFile(sourcePath, tempTargetPath, fsConstants.COPYFILE_EXCL);
    await fs.rename(tempTargetPath, targetPath);
  } catch (error) {
