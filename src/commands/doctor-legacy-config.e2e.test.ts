...
describe("normalizeLegacyConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.SMART_AGENT_NEO_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-agent-neo-oauth-"));
    process.env.SMART_AGENT_NEO_OAUTH_DIR = tempOauthDir;
  });

  afterEach(() => {
    if (previousOauthDir === undefined) {
      delete process.env.SMART_AGENT_NEO_OAUTH_DIR;
    } else {
      process.env.SMART_AGENT_NEO_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "smart-agent-neo-wa-auth-"));
    try {
