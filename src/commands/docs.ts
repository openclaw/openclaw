...
const SEARCH_TOOL = "https://docs.mullusi.com/mcp.SearchMullusi";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.mullusi.com");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('mullusi docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.mullusi.com/");
      runtime.log(`Search: ${formatCliCommand('mullusi docs "your query"')}`);
    }
    return;
  }
