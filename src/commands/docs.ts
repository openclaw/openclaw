...
const SEARCH_TOOL = "https://docs.evox.sh/mcp.SearchEVOX.sh";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.evox.sh");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('evox docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.evox.sh/");
      runtime.log(`Search: ${formatCliCommand('evox docs "your query"')}`);
    }
    return;
  }
