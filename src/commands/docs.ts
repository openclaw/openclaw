...
const SEARCH_TOOL = "https://docs.smart-agent-neo.ai/mcp.SearchSmartAgentNeo";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.smart-agent-neo.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('smart-agent-neo docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.smart-agent-neo.ai/");
      runtime.log(`Search: ${formatCliCommand('smart-agent-neo docs "your query"')}`);
    }
    return;
  }
