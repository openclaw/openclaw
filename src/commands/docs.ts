...
const SEARCH_TOOL = "https://docs.activi.ai/mcp.SearchActivi";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.activi.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('activi docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.activi.ai/");
      runtime.log(`Search: ${formatCliCommand('activi docs "your query"')}`);
    }
    return;
  }
