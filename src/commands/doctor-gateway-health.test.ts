...
describe("checkGatewayHealth", () => {
  ...
  it.each([false, true])(
    ...
    async (rejectStatus) => {
      ...
      await expect(
        checkGatewayHealth({ runtime, cfg: {} as OpenClawConfig }),
  ...
  it("uses a lightweight status RPC for the restart liveness gate", async () => {
    ...
    try {
      await expect(checkGatewayHealth({ runtime, cfg, timeoutMs: 3000 })).resolves.toEqual({
        authenticated: true,
        healthOk: true,
        status: { ok: true },
      });
  ...
  it("renders the shared redacted telemetry exporter summary", async () => {
    ...
    await checkGatewayHealth({ runtime, cfg, timeoutMs: 3000 });
  ...
  it("reports broken egress certificates even when Gateway RPC is healthy", async () => {
    callGateway
      ...
      .mockResolvedValue({});
    expect(await checkGatewayHealth({ runtime, cfg })).toMatchObject({
      healthOk: true,
      authenticated: true,
    });
    expect(note).toHaveBeenCalledWith(
