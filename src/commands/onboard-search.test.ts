...
describe("setupSearch", () => {
  ...
  it("sets provider and key for kimi", async () => {
    ...
    expect(result.tools?.web?.search?.enabled).toBe(true);
    expect(result.tools?.web?.search?.kimi?.apiKey).toBe("sk-moonshot"); // pragma: allowlist secret (fixture key)
  });
  ...
  it("keeps existing key when user leaves input blank", async () => {
    const cfg: OpenClawConfig = {
      tools: {
        web: {
          search: {
            provider: "perplexity",
            perplexity: { apiKey: "existing-key" }, // pragma: allowlist secret (fixture key)
          },
        },
      },
    };
    ...
    const result = await setupSearch(cfg, runtime, prompter);
    expect(result.tools?.web?.search?.perplexity?.apiKey).toBe("existing-key"); // pragma: allowlist secret (fixture key)
    expect(result.tools?.web?.search?.enabled).toBe(true);
  });
  ...
  it("quickstart skips key prompt when config key exists", async () => {
    const cfg: OpenClawConfig = {
      tools: {
        web: {
          search: {
            provider: "perplexity",
            perplexity: { apiKey: "stored-pplx-key" }, // pragma: allowlist secret (fixture key)
          },
        },
      },
    };
    ...
    expect(result.tools?.web?.search?.provider).toBe("perplexity");
    expect(result.tools?.web?.search?.perplexity?.apiKey).toBe("stored-pplx-key"); // pragma: allowlist secret (fixture key)
    expect(result.tools?.web?.search?.enabled).toBe(true);
