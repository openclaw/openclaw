...
import { normalizeLowercaseStringOrEmpty, normalizeOptionalLowercaseString, normalizeOptionalString, normalizeOptionalStringifiedId, } from "../shared/string-coerce.js";
...
export function normalizeStoredCronJobs(jobs) {
    const issues = {};
    let mutated = false;
    for (const raw of jobs) {
        const jobIssues = new Set();
        ...
        if ("sessionKey" in raw) {
            const sessionKey = typeof raw.sessionKey === "string" ? normalizeOptionalString(raw.sessionKey) : undefined;
        ...
        const hadLegacyTopLevelPayloadFields = "model" in raw ||
            "thinking" in raw ||
            ...
            "command" in raw ||
            "timeout" in raw;
        const hadLegacyTopLevelDeliveryFields = "deliver" in raw ||
            "channel" in raw ||
            "to" in raw ||
            "threadId" in raw ||
        ...
        if (schedule && typeof schedule === "object" && !Array.isArray(schedule)) {
            ...
            if ((kind === "every" || sched.kind === "every") && everyMs !== null) {
                ...
                const normalizedAnchor = anchorCoerced !== undefined
        ...
        const isIsolatedAgentTurn = sessionTarget === "isolated" ||
