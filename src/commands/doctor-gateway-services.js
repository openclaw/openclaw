...
import { auditGatewayServiceConfig, needsNodeRuntimeMigration, readEmbeddedGatewayToken, SERVICE_AUDIT_CODES, } from "../daemon/service-audit.js";
...
export async function maybeRepairGatewayServiceConfig(cfg, mode, runtime, prompter) {
    ...
    if (gatewayTokenResolution.unavailableReason) {
        note(`Unable to verify gateway service token drift: ${gatewayTokenResolution.unavailableReason}`, "Gateway service config");
    }
    ...
    const normalizedCurrentEntrypoint = currentEntrypoint
        ...
        : null;
    if (normalizedExpectedEntrypoint &&
    ...
    const configuredGatewayToken = typeof cfg.gateway?.auth?.token === "string"
    ...
    if (!updateRepairMode &&
        ...
        try {
            ...
            note(expectedGatewayToken
                ? "Persisted gateway.auth.token from environment before reinstalling service."
                : "Persisted gateway.auth.token from existing service definition before reinstalling service.", "Gateway");
        }
        catch (err) {
