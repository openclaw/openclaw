...
vi.mock("../infra/dna-root.js", () => ({
  resolveDNAPackageRoot: vi.fn().mockResolvedValue("/tmp/dna"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/dna/pnpm-lock.yaml",
      markerPath: "/tmp/dna/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("DNA status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("dna status --all") ||
          l.includes("dna --profile isolated status --all") ||
          l.includes("dna status --all") ||
          l.includes("dna --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    const prevToken = process.env.DNA_GATEWAY_TOKEN;
    process.env.DNA_GATEWAY_TOKEN = "abcd1234";
    try {
      ...
      if (prevToken === undefined) delete process.env.DNA_GATEWAY_TOKEN;
      else process.env.DNA_GATEWAY_TOKEN = prevToken;
    }
  });
