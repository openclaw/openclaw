...
describe("workspace backup commands", () => {
  ...
  it("rejects mirrored workspaces that contain symbolic links", async () => {
    const stateDir = path.join(tempHome.home, ".openclaw");
    const workspaceDir = path.join(tempHome.home, "workspace");
    const outsideDir = path.join(tempHome.home, "outside");
    targetDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-workspace-target-"));
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.mkdir(outsideDir, { recursive: true });
    await fs.writeFile(path.join(outsideDir, "secret.txt"), "secret\n", "utf8");
    await fs.symlink(path.join(outsideDir, "secret.txt"), path.join(workspaceDir, "secret-link"));
    await fs.writeFile(
      path.join(stateDir, "openclaw.json"),
      JSON.stringify({
        agents: {
    ...
    await workspaceBackupInitCommand(runtime, { target: targetDir });
    await expect(workspaceBackupRunCommand(runtime, {})).rejects.toThrow(
