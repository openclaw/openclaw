...
function resolveAgentName(cfg: MullusiConfig, agentId: string) {
...
function resolveAgentModel(cfg: MullusiConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MullusiConfig): AgentSummary[] {
