...
type AgentEntry = NonNullable<NonNullable<EasyHubConfig["agents"]>["list"]>[number];

export type AgentIdentity = AgentIdentityFile;

export function listAgentEntries(cfg: EasyHubConfig): AgentEntry[] {
...
function resolveAgentName(cfg: EasyHubConfig, agentId: string) {
...
function resolveAgentModel(cfg: EasyHubConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: EasyHubConfig): AgentSummary[] {
