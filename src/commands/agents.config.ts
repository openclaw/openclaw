...
type AgentEntry = NonNullable<NonNullable<SmartAgentNeoConfig["agents"]>["list"]>[number];

export type AgentIdentity = AgentIdentityFile;

export function listAgentEntries(cfg: SmartAgentNeoConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SmartAgentNeoConfig, agentId: string) {
...
function resolveAgentModel(cfg: SmartAgentNeoConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SmartAgentNeoConfig): AgentSummary[] {
