...
function resolveAgentName(cfg: BotConfig, agentId: string) {
...
function resolveAgentModel(cfg: BotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: BotConfig): AgentSummary[] {
