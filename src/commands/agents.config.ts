...
type AgentEntry = NonNullable<NonNullable<DNAConfig["agents"]>["list"]>[number];

export type AgentIdentity = AgentIdentityFile;

export function listAgentEntries(cfg: DNAConfig): AgentEntry[] {
...
function resolveAgentName(cfg: DNAConfig, agentId: string) {
...
function resolveAgentModel(cfg: DNAConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DNAConfig): AgentSummary[] {
