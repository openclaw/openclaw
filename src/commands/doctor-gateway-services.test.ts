...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("flags and repairs entrypoint mismatch from entry.js to index.js (#46621)", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      expect.stringContaining("entry.js -> /opt/openclaw/dist/index.js"),
      "Gateway service config",
    );
    expect(mocks.install).toHaveBeenCalledTimes(1);
    expect(mocks.install).toHaveBeenCalledWith(
      expect.objectContaining({
        programArguments: expect.arrayContaining(["/opt/openclaw/dist/index.js"]),
      }),
    );
  });
