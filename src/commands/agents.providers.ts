...
function resolveDefaultAccountId(cfg: DNAConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: DNAConfig): boolean {
...
export function summarizeBindings(cfg: DNAConfig, bindings: AgentBinding[]): string[] {
