...
beforeEach(() => {
  ...
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclaw-doctor-exit-"));
...
const healthCommand = vi.fn().mockResolvedValue(undefined);
...
describe("doctor exit codes", () => {
  ...
  it("exits with code 1 when gateway health check fails", async () => {
    ...
    healthCommand.mockRejectedValue(new Error("gateway closed"));
