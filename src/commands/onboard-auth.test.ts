...
const requireAgentDir = () => {
  const agentDir = process.env.EASYHUB_AGENT_DIR;
  if (!agentDir) {
    throw new Error("EASYHUB_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.EASYHUB_STATE_DIR;
  const previousAgentDir = process.env.EASYHUB_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.EASYHUB_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.EASYHUB_AGENT_DIR;
    } else {
      process.env.EASYHUB_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.EASYHUB_OAUTH_DIR;
  });

  it("writes auth-profiles.json under EASYHUB_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-oauth-"));
    process.env.EASYHUB_STATE_DIR = tempStateDir;
    process.env.EASYHUB_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.EASYHUB_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to EASYHUB_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-minimax-"));
    process.env.EASYHUB_STATE_DIR = tempStateDir;
    process.env.EASYHUB_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.EASYHUB_AGENT_DIR;
