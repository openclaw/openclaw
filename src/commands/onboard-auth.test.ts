...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ACTIVI_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("activi-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "activi-oauth-sync-"));
    process.env.ACTIVI_STATE_DIR = tempStateDir;
    ...
    process.env.ACTIVI_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "activi-oauth-nosync-"));
    process.env.ACTIVI_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ACTIVI_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "activi-oauth-external-"));
    process.env.ACTIVI_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ACTIVI_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ACTIVI_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("activi-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
