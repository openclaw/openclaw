import { buildPairingConnectRecoveryTitle, describePairingConnectRequirement, } from "../gateway/protocol/connect-error-details.js";
...
export function buildStatusAgentsValue(params) {
    const pending = params.agentStatus.bootstrapPendingCount > 0
...
export function buildStatusTasksValue(params) {
    ...
    return [
        ...
        params.summary.tasks.failures > 0
            ? params.warn(`${params.summary.tasks.failures} issue${params.summary.tasks.failures === 1 ? "" : "s"}`)
            : params.muted("no issues"),
        params.summary.taskAudit.errors > 0
            ? params.warn(`audit ${params.summary.taskAudit.errors} error${params.summary.taskAudit.errors === 1 ? "" : "s"} · ${params.summary.taskAudit.warnings} warn`)
...
export function buildStatusHeartbeatValue(params) {
...
export function buildStatusSecurityAuditLines(params) {
    ...
    if (importantFindings.length === 0) {
        ...
        const severityLabel = (sev) => sev === "critical"
...
export function buildStatusPairingRecoveryLines(params) {
    ...
    return [
        ...
        ...(params.pairingRecovery.reason
            ? [
                params.muted(`Reason: ${describePairingConnectRequirement(params.pairingRecovery.reason)}.`),
            ]
            : []),
        ...
        ...(params.pairingRecovery.requestId
            ? [
                params.muted(`Recovery: ${params.formatCliCommand(`openclaw devices approve ${params.pairingRecovery.requestId}`)}`),
            ]
            : []),
