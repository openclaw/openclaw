...
it("keeps startup post-session execution bound to the admitted external plugin inventory", async () => {
  await withHome(async (home) => {
    const stateDir = path.join(home, ".openclaw");
    const bundledRoot = path.join(home, "bundled");
    const externalRoot = path.join(stateDir, "extensions");
    const actions = [
  ...
  migrateLegacyState: () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(prepared.stepReceipts.at(-1)).toMatchObject({
          id: "plugin-doctor-post-session-state",
          outcome: "completed",
          changes: ["migrated acpx"],
          warnings: [],
        });
        expect(
          fs.readFileSync(path.join(externalRoot, "acpx", "migrated-session-state"), "utf8"),
        ).toBe("retained session state");
        for (const pluginRoot of [
          path.join(externalRoot, "codex"),
