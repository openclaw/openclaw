...
import { resolveApiKeyForProvider, resolveEnvApiKey, resolveUsableCustomProviderApiKey, } from "../agents/model-auth.js";
...
import { auditDreamingArtifacts, auditShortTermPromotionArtifacts, repairDreamingArtifacts, repairShortTermPromotionArtifacts, } from "../plugin-sdk/memory-core-engine-runtime.js";
import { getActiveMemorySearchManager, resolveActiveMemoryBackendConfig, } from "../plugins/memory-runtime.js";
...
function listAutoSelectMemoryEmbeddingProviderDoctorMetadata() {
    return BUNDLED_MEMORY_EMBEDDING_PROVIDER_DOCTOR_METADATA.filter((provider) => typeof provider.autoSelectPriority === "number")
...
async function resolveRuntimeMemoryAuditContext(cfg) {
    ...
    try {
        ...
        return {
            ...
            qmdCollections: typeof customQmd?.collections === "number" ? customQmd.collections : undefined,
        };
    }
    finally {
...
export async function noteMemoryRecallHealth(cfg) {
    try {
 ...
 */
export async function noteMemorySearchHealth(cfg, opts) {
...
async function hasApiKeyForProvider(provider, cfg, agentDir) {
    ...
    if (resolveEnvApiKey(authProviderId) ||
