...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.ACTIVI_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.ACTIVI_GATEWAY_PASSWORD;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.activi.ai/gateway/health",
        "https://docs.activi.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from activi.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Activi update wizard" : "Activi configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("activi doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.ACTIVI_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.ACTIVI_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.ACTIVI_GATEWAY_TOKEN;
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.activi.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
