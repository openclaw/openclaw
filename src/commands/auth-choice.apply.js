...
async function normalizeTokenProviderChoice(params) {
    ...
    const { normalizeApiKeyTokenProviderAuthChoice } = await import("./auth-choice.apply.api-providers.js");
...
async function formatDeprecatedProviderChoiceError(authChoice, params) {
    ...
    const { resolveManifestDeprecatedProviderAuthChoice } = await import("../plugins/provider-auth-choices.js");
...
export async function applyAuthChoice(params) {
    const normalizedAuthChoice = (await normalizeLegacyChoice(params.authChoice, {
        config: params.config,
        env: params.env,
    })) ?? params.authChoice;
    ...
    const normalizedParams = normalizedProviderAuthChoice === params.authChoice
        ? params
    ...
    if (normalizedParams.authChoice === "oauth") {
        throw new Error('Auth choice "oauth" is no longer supported directly. Use "setup-token" for Anthropic legacy token auth or a provider-specific OAuth entry.');
    }
