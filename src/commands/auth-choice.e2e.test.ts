...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    const env = await setupAuthTestEnv("smart-agent-neo-auth-");
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireSmartAgentNeoAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
