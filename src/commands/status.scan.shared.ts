...
export function hasExplicitMemorySearchConfig(cfg: MullusiConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: MullusiConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: MullusiConfig, agentId: string) => { store: { path: string } } | null;
