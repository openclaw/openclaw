...
export function clearContextEnginesForOwner(owner) {
    const normalizedOwner = requireContextEngineOwner(owner);
    const registry = getContextEngineRegistryState().engines;
...
export async function resolveContextEngine(config) {
    ...
    if (!entry) {
        ...
        console.error(`[context-engine] Context engine "${sanitizeForLog(engineId)}" is not registered; ` +
    ...
    try {
        ...
        console.error(`[context-engine] Context engine "${sanitizeForLog(engineId)}" factory threw during resolution: ` +
    ...
    try {
        ...
        console.error(`[context-engine] Context engine "${sanitizeForLog(engineId)}" contract validation threw: ` +
...
async function resolveDefaultContextEngine(defaultEngineId) {
    ...
    if (!defaultEntry) {
        throw new Error(`[context-engine] fallback failed: default engine "${defaultEngineId}" is not registered. ` +
            `Available engines: ${listContextEngineIds().join(", ") || "(none)"}`);
    }
