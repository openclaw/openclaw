...
import { buildMediaGenerationNormalizationMetadata, buildNoCapabilityModelConfiguredMessage, recordCapabilityCandidateFailure, resolveCapabilityModelCandidates, throwCapabilityGenerationFailure, } from "../media-generation/runtime-shared.js";
...
export function listRuntimeVideoGenerationProviders(params) {
...
export async function generateVideo(params) {
    ...
    for (const candidate of candidates) {
        ...
        if (params.providerOptions &&
            ...
            const declaredOptions = optCaps?.providerOptions ?? provider.capabilities.providerOptions ?? undefined;
        ...
        try {
            ...
            for (const [index, video] of result.videos.entries()) {
                if (!video.buffer && !video.url) {
                    throw new Error(`Video generation provider returned an undeliverable asset at index ${index}: neither buffer nor url is set.`);
                }
            }
            return {
