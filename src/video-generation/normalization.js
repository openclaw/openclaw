...
import { normalizeVideoGenerationDuration, resolveVideoGenerationSupportedDurations, } from "./duration-support.js";
export function resolveVideoGenerationOverrides(params) {
    ...
    if (caps) {
        ...
        if (aspectRatio && (caps.aspectRatios?.length ?? 0) > 0 && caps.supportsAspectRatio) {
            ...
            const derivedSize = caps.supportsSize && !size
                ? resolveClosestSize({
    ...
    const requestedDurationSeconds = typeof params.durationSeconds === "number" && Number.isFinite(params.durationSeconds)
    ...
    if (typeof requestedDurationSeconds === "number" &&
    ...
    return {
        ...
        watermark,
        ignoredOverrides,
        normalization: hasMediaNormalizationEntry(normalization.size) ||
