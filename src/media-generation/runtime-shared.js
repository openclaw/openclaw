...
export function hasMediaNormalizationEntry(entry) {
...
export function resolveCapabilityModelCandidates(params) {
    ...
    const autoProviderFallbackEnabled = params.autoProviderFallback ??
...
export function deriveAspectRatioFromSize(size) {
...
export function resolveClosestAspectRatio(params) {
    ...
    const requested = parseAspectRatioValue(params.requestedAspectRatio) ??
...
export function normalizeDurationToClosestMax(durationSeconds, maxDurationSeconds) {
...
export function buildMediaGenerationNormalizationMetadata(params) {
    ...
    if (normalization?.aspectRatio?.applied !== undefined) {
        ...
        if (normalization.aspectRatio.derivedFrom === "size" &&
    ...
    if (normalization?.resolution?.requested !== undefined &&
    ...
    if (normalization?.durationSeconds?.requested !== undefined &&
        ...
        if (params.includeSupportedDurationSeconds &&
...
export function throwCapabilityGenerationFailure(params) {
    ...
    const summary = params.attempts.length > 0
        ? params.attempts
            ...
            .join(" | ")
        : "unknown";
    throw new Error(`All ${params.capabilityLabel} models failed (${params.attempts.length}): ${summary}`, {
