...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of SmartAgentNeo")).toBe("friends-of-smart-agent-neo");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of SmartAgentNeo"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-smart-agent-neo" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-smart-agent-neo" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of SmartAgentNeo"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-smart-agent-neo");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-smart-agent-neo": { slug: "friends-of-smart-agent-neo" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of SmartAgentNeo"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["smart-agent-neo-dm"],
        channelId: "1",
        channelName: "SmartAgentNeo DM",
        channelSlug: "smart-agent-neo-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
