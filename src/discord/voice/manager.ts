...
function resolveVoiceTtsConfig(params: { cfg: ActiviConfig; override?: TtsConfig }): {
  cfg: ActiviConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredActiviTmpDir(), "discord-voice-"));
