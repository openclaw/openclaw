...
describe("agent components", () => {
  const createCfg = (): SmartAgentNeoConfig => ({}) as SmartAgentNeoConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): SmartAgentNeoConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/smart-agent-neo-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/smart-agent-neo",
    });
  });
});
