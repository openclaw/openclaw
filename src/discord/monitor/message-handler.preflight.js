...
import { allowListMatches, isDiscordGroupAllowedByPolicy, normalizeDiscordAllowList, normalizeDiscordSlug, resolveDiscordChannelConfigWithFallback, resolveDiscordGuildEntry, resolveDiscordMemberAccessState, resolveDiscordShouldRequireMention, resolveGroupDmAllow, } from "./allow-list.js";
...
export function shouldIgnoreBoundThreadWebhookMessage(params) {
    ...
    const boundWebhookId = typeof params.threadBinding?.metadata?.webhookId === "string"
...
export async function preflightDiscordMessage(params) {
    ...
    if (isDirectMessage) {
        ...
        if (dmAccess.decision !== "allow") {
            const allowMatchMeta = formatAllowlistMatchMeta(dmAccess.allowMatch.allowed ? dmAccess.allowMatch : undefined);
