import { Button, Container, Row, Separator, StringSelectMenu, TextDisplay, } from "@buape/carbon";
...
export const DISCORD_MODEL_PICKER_PROVIDER_PAGE_SIZE = DISCORD_COMPONENT_MAX_BUTTONS_PER_ROW * (DISCORD_COMPONENT_MAX_ROWS - 1);
// When providers fit in one page, we can use all button rows and hide nav controls.
export const DISCORD_MODEL_PICKER_PROVIDER_SINGLE_PAGE_MAX = DISCORD_COMPONENT_MAX_BUTTONS_PER_ROW * DISCORD_COMPONENT_MAX_ROWS;
...
function buildProviderRows(params) {
    const rows = chunkProvidersForRows(params.page.items).map((providers) => new Row(providers.map((provider) => {
...
function buildModelRows(params) {
    ...
    const shouldDisableReset = Boolean(parsedCurrentModel) &&
        parsedCurrentModel?.provider === resolvedDefault.provider &&
        parsedCurrentModel?.model === resolvedDefault.model;
    const hasPendingSelection = Boolean(parsedPendingModel) &&
 ...
 */
export async function loadDiscordModelPickerData(cfg) {
...
export function buildDiscordModelPickerCustomId(params) {
    ...
    const providerPage = typeof params.providerPage === "number" && Number.isFinite(params.providerPage)
    ...
    const modelIndex = typeof params.modelIndex === "number" && Number.isFinite(params.modelIndex)
        ? Math.max(1, Math.floor(params.modelIndex))
        : undefined;
    const recentSlot = typeof params.recentSlot === "number" && Number.isFinite(params.recentSlot)
    ...
    if (customId.length > DISCORD_CUSTOM_ID_MAX_CHARS) {
        throw new Error(`Discord model picker custom_id exceeds ${DISCORD_CUSTOM_ID_MAX_CHARS} chars (${customId.length})`);
    }
    return customId;
}
export function parseDiscordModelPickerCustomId(customId) {
...
export function parseDiscordModelPickerData(data) {
...
export function buildDiscordModelPickerProviderItems(data) {
...
export function renderDiscordModelPickerProvidersView(params) {
...
export function renderDiscordModelPickerModelsView(params) {
...
export function renderDiscordModelPickerRecentsView(params) {
...
export function toDiscordModelPickerMessagePayload(view) {
