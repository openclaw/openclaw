...
function buildTaskScriptCommand(
  taskPacketPath: string | undefined,
  harnessRepoCwd: string,
  ...
  const absoluteScriptPath = path
    .join(harnessRepoCwd, scriptRelativePath)
    .split(path.sep)
    .join("/");
  return `node --import tsx ${absoluteScriptPath} --task ${normalizedPacketPath}`;
}
