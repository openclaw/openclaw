...
export function buildPullRequestHeadRefName(input: {
  branchName: string;
  repoUrl: string;
  pushRepoUrl?: string | null;
}) {
  const repoSlug = parseRepo(input.repoUrl);
  const pushRepoSlug = input.pushRepoUrl ? parseRepo(input.pushRepoUrl) : repoSlug;
  if (repoSlug === pushRepoSlug) {
    return input.branchName;
  }
  return `${repoOwner(pushRepoSlug)}:${input.branchName}`;
}
...
export async function findPullRequestByBranch(input: {
  ...
  const refs = Array.from(
    new Set(
      [input.headRefName, input.branchName].filter(
...
export async function ensurePullRequest(input: {
  ...
  const repo = parseRepo(input.repoUrl);
  const headRefName = input.headRefName ?? input.branchName;
  const existing = await findPullRequestByBranch({
