...
import { buildPullRequestHeadRefName, findPullRequestByBranch } from "./github-pr.js";
...
Use the $paperclip skill immediately. Read the assigned issue description and comments, follow the task packet exactly, perform the local implementation or review work, collect the required evidence, run validationCommand, then run finalizeCommand so the branch, commit, push, and PR state are all real before you report completion. Update the issue with a concise evidence-backed comment, and then exit the heartbeat.
The task packet is the source of truth for repoCwd, branchName, artifactDir, startupCommand, validationCommand, finalizeCommand, browser steps, PR requirements, and evidence paths.
`);
...
async function syncBuilderPullRequest(
  ...
  const pushRepoUrl = await runCommand(
    "git",
    ["remote", "get-url", "--push", taskPacket.gitRemoteName],
    taskPacket.repoCwd,
  ).catch(() => null);
  const headRefName = buildPullRequestHeadRefName({
...
async function reconcileBuilderCompletion(
  provisioned: ProvisionResult,
  group: ManagedIssueGroup,
): Promise<void> {
  const builderIssue = findSubtaskByRole(group, "builder");
  const taskPacket = builderIssue?.taskPacket;
  if (!builderIssue || !taskPacket || builderIssue.status === "done" || builderIssue.activeRun) {
    return;
  }
  const prUrl = (await syncBuilderPullRequest(provisioned, group)) ?? taskPacket.prUrl ?? null;
  if (!prUrl) {
    return;
  }
  ...
  await provisioned.paperclip.updateIssue(builderIssue.id, {
    status: "done",
    comment: trimPrompt(`
Operator reconciled the builder task after verifying the pushed branch, open PR, and required evidence.
...
export async function operatorStatus(
  ...
  for (const group of groups) {
    ...
    await reconcileBuilderCompletion(provisioned, group).catch(() => null);
    await reconcileParentCompletion(provisioned, group);
  }
  const refreshedGroups = groupManagedIssues(await listHarnessIssues(provisioned));
  const evidenceByParentId = await collectEvidenceStatus(refreshedGroups);
  const summary = summarizeGroups(refreshedGroups, evidenceByParentId);
  ...
  runtime.log(renderStatusTable(refreshedGroups, evidenceByParentId));
}
...
export async function operatorRequestReview(
  ...
  await syncBuilderPullRequest(provisioned, group).catch(() => null);
  await reconcileBuilderCompletion(provisioned, group).catch(() => null);
  const refreshedGroup = await findGroupByTicketKey(provisioned, ticketKey);
  if (!refreshedGroup) {
    throw new Error(`Managed Paperclip issue disappeared for ${ticketKey}.`);
  }
  const builderTask = refreshedGroup ? findSubtaskByRole(refreshedGroup, "builder") : null;
  if (!builderTask || builderTask.status !== "done") {
    throw new Error(`Builder task is not done for ${ticketKey}. Review cannot start yet.`);
  }
  const reviewIssues = await ensureReviewSubtasks(provisioned, refreshedGroup);
...
export async function operatorSpotCheck(
  ...
  if (taskPacket.finalizeCommand) {
    const [command, ...args] = taskPacket.finalizeCommand.split(" ");
    await runCommand(command, args, taskPacket.repoCwd);
  }
  const artifactCheck = await checkArtifacts(taskPacket.artifactDir, taskPacket.requiredArtifacts);
