...
async function ensureRemoteUrls(config: HarnessConfig, cwd: string) {
  if (config.workspace.repoUrl) {
    await runGit(["remote", "set-url", "origin", config.workspace.repoUrl], cwd);
  }
  if (config.workspace.pushRepoUrl) {
    await runGit(["remote", "set-url", "--push", "origin", config.workspace.pushRepoUrl], cwd);
  }
}
...
async function ensureClone(config: HarnessConfig, cwd: string) {
  ...
  if (await pathExists(gitDir)) {
    await ensureRemoteUrls(config, cwd);
    return;
  }
...
async function hasRemoteBranch(cwd: string, remote: string, branchName: string) {
  try {
    const output = await runGit(["ls-remote", "--heads", remote, branchName], cwd);
...
async function fetchRemoteBranch(
  ...
  remote: string,
  branchName: string,
  destinationRef = `refs/remotes/origin/${branchName}`,
) {
  await runGit(["fetch", remote, `refs/heads/${branchName}:${destinationRef}`], cwd);
}

async function ensureBranchAvailable(config: HarnessConfig, cwd: string, branchName: string) {
  await runGit(["fetch", "origin", "--prune"], cwd);
  if (await hasRemoteBranch(cwd, "origin", branchName)) {
    await fetchRemoteBranch(cwd, "origin", branchName);
    return;
  }
  if (
    config.workspace.pushRepoUrl &&
    (await hasRemoteBranch(cwd, config.workspace.pushRepoUrl, branchName))
  ) {
    await fetchRemoteBranch(cwd, config.workspace.pushRepoUrl, branchName);
    return;
  }
...
async function ensureBuilderBranch(
  ...
  try {
    await ensureBranchAvailable(config, cwd, branchName);
...
async function ensureReviewBranch(config: HarnessConfig, cwd: string, branchName: string) {
  const dirty = await isDirty(cwd);
  const branch = await currentBranch(cwd);
  if (dirty) {
    if (branch !== branchName) {
...
export async function prepareRoleWorkspace(input: {
  ...
  if (input.role === "builder") {
    ...
    await ensureReviewBranch(input.config, cwd, branchName);
  }
