...
const configSchema: z.ZodType<HarnessConfig> = z.object({
  ...
  workspace: z.object({
    ...
    pushRepoUrl: z.string().url().optional(),
  ...
  return Object.fromEntries(
    Object.entries(env).map(([key, value]) => {
      if (value.startsWith("./") || value.startsWith("../")) {
        return [key, resolveMaybeRelative(value, configDir)];
      }
      return [key, value];
    }),
  );
  ...
  return {
    ...
    agents: {
      builder: {
        ...
        env: resolveEnvBindings(config.agents.builder.env, configDir),
      },
      qa: {
        ...config.agents.qa,
        instructionsFile: resolveMaybeRelative(config.agents.qa.instructionsFile, configDir),
        env: resolveEnvBindings(config.agents.qa.env, configDir),
      },
      ...(config.agents.ux
        ? {
            ux: {
              ...
              env: resolveEnvBindings(config.agents.ux.env, configDir),
            },
          }
        : {}),
      ...(config.agents.ai
        ? {
            ai: {
              ...
              env: resolveEnvBindings(config.agents.ai.env, configDir),
            },
          }
        : {}),
    },
