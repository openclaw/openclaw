...
export const SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT = [
  ...
  "Switching: If the user prefers masked channel-secret prompts, hand off with `open channel wizard for <channel>`. If CLI web-search or Gateway setup reaches a credential, hand off with `open search wizard` or `open gateway wizard`; gateway chat masks the credential field in place. Shared provider/auth onboarding cannot run inside this session.",
