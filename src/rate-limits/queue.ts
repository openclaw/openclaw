...
export class RateLimitQueue {
  ...
  enqueue<T>(
    ...
    if (queue.length >= this.maxSize) {
      return Promise.reject(
        new RateLimitQueueFullError(`Rate limit queue full for ${key} (max ${this.maxSize})`),
      );
    }
  ...
  private scheduleDrain(key: string, delayMs: number, acquireFn: () => AcquireResult): void {
