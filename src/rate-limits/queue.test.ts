...
describe("RateLimitQueue", () => {
    ...
    describe("enqueue", () => {
        ...
        it("rejects with queue-full error when maxSize exceeded", async () => {
            const acquireFn = vi.fn<() => AcquireResult>().mockReturnValue({ allowed: false, retryAfterMs: 60000 });
        ...
        it("rejects entries that time out in the queue", async () => {
            const acquireFn = vi.fn<() => AcquireResult>().mockReturnValue({ allowed: false, retryAfterMs: 1000 });
            const fn = vi.fn().mockResolvedValue("result");
