...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".activi-profile-decorated");
}
 ...
 */
export function decorateActiviProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ACTIVI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ACTIVI_BROWSER_COLOR).toUpperCase();
