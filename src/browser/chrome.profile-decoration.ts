...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".smart-agent-neo-profile-decorated");
}
 ...
 */
export function decorateSmartAgentNeoProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SMART_AGENT_NEO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SMART_AGENT_NEO_BROWSER_COLOR).toUpperCase();
