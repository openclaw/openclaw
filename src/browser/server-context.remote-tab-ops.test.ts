...
function makeState(
  profile: "remote" | "smart-agent-neo",
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("smart-agent-neo");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const smart-agent-neo = ctx.forProfile("smart-agent-neo");

    const opened = await smart-agent-neo.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("smart-agent-neo")?.lastTargetId).toBe("CREATED");
  });
});
