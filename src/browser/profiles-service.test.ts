...
vi.mock("./chrome.js", () => ({
  resolveEasyHubUserDataDir: vi.fn(() => "/tmp/EasyHub-test/EasyHub/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "EasyHub-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveEasyHubUserDataDir).mockReturnValue(userDataDir);
