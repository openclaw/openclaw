...
vi.mock("./chrome.js", () => ({
  resolveSmartAgentNeoUserDataDir: vi.fn(() => "/tmp/smart-agent-neo-test/smart-agent-neo/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "smart-agent-neo-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveSmartAgentNeoUserDataDir).mockReturnValue(userDataDir);
