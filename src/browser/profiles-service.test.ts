...
vi.mock("./chrome.js", () => ({
  resolveActiviUserDataDir: vi.fn(() => "/tmp/activi-test/activi/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "activi-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveActiviUserDataDir).mockReturnValue(userDataDir);
