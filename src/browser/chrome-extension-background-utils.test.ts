...
describe("chrome extension background utils", () => {
  ...
  it("builds websocket url with derived relay token", async () => {
    ...
    expect(url).toMatch(
      /^ws:\/\/127\.0\.0\.1:18792\/extension\?token=[0-9a-f]{64}$/,
    );
  });

  it("throws when gateway token is missing", async () => {
    await expect(buildRelayWsUrl(18792, "")).rejects.toThrow(
  ...
  it("keeps transient network errors retryable", () => {
    expect(
      isRetryableReconnectError(new Error("WebSocket connect timeout")),
    ).toBe(true);
    expect(
      isRetryableReconnectError(new Error("Relay server not reachable")),
    ).toBe(true);
  });
});
