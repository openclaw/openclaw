import type { BrowserConfig, BrowserProfileConfig, ActiviConfig } from "../config/config.js";
...
import {
  DEFAULT_ACTIVI_BROWSER_COLOR,
  DEFAULT_ACTIVI_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_ACTIVI_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "activi" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_ACTIVI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "activi" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ACTIVI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ACTIVI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Activi browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for activi/activi CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ActiviConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ACTIVI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "activi";
