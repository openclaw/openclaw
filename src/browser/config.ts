import type { BrowserConfig, BrowserProfileConfig, SmartAgentNeoConfig } from "../config/config.js";
...
import {
  DEFAULT_SMART_AGENT_NEO_BROWSER_COLOR,
  DEFAULT_SMART_AGENT_NEO_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_SMART_AGENT_NEO_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "smart-agent-neo" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SMART_AGENT_NEO_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "smart-agent-neo" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SMART_AGENT_NEO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SMART_AGENT_NEO_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SmartAgentNeo browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for betterbrand/smart-agent-neo CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SmartAgentNeoConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SMART_AGENT_NEO_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "smart-agent-neo";
