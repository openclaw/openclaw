...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("evox");
    expect(profile?.driver).toBe("evox");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const evox = resolveProfile(resolved, "evox");
    expect(evox?.driver).toBe("evox");
    expect(evox?.cdpPort).toBe(18800);
    expect(evox?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from OPENCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ OPENCLAW_GATEWAY_PORT: "19001" }, () => {
      ...
      const evox = resolveProfile(resolved, "evox");
      expect(evox?.cdpPort).toBe(19012);
      expect(evox?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ OPENCLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const evox = resolveProfile(resolved, "evox");
      expect(evox?.cdpPort).toBe(19022);
      expect(evox?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(evox?.cdpPort).toBe(19000);
    expect(evox?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "evox");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "evox")!;
  ...
  describe("default profile preference", () => {
    it("defaults to evox profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("evox");
    });

    it("keeps evox default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("evox");
    ...
    it("keeps evox default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("evox");
    ...
    it("keeps evox default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
