  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          activi: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ACTIVI_GATEWAY_PORT;
    process.env.ACTIVI_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ACTIVI_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ACTIVI_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPassword === undefined) {
      ...
      process.env.ACTIVI_GATEWAY_PASSWORD = prevGatewayPassword;
    }
