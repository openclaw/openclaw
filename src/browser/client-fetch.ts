...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-smart-agent-neo-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-smart-agent-neo-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-smart-agent-neo-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the SmartAgentNeo gateway (SmartAgentNeo.app menubar, or \`${formatCliCommand("smart-agent-neo gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the SmartAgentNeo browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the SmartAgentNeo browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
