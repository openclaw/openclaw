  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-smart-agent-neo-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-smart-agent-neo-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-smart-agent-neo-labels", "1");
          ...
          tag.setAttribute("data-smart-agent-neo-labels", "1");
