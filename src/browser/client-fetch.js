...
function enhanceBrowserFetchError(url, err, timeoutMs) {
    ...
    if (looksLikeTimeout) {
        return new Error(`Can't reach the OpenClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`);
    }
    return new Error(`Can't reach the OpenClaw browser control service. ${operatorHint} ${modelHint} (${msg})`);
}
