...
export async function startBrowserControlServerFromConfig(): Promise<BrowserServerState | null> {
  ...
  const server = await new Promise<Server>((resolve, reject) => {
    ...
    logServer.error(
      `openclaw browser server failed to bind 127.0.0.1:${port}: ${String(err)}`,
    );
    return null;
  });
  ...
  const authMode = browserAuth.token
    ? "token"
    : browserAuth.password
      ? "password"
      : "off";
  logServer.info(
    `Browser control listening on http://127.0.0.1:${port}/ (auth=${authMode})`,
  );
  return state;
}
