...
export async function launchOpenClawChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`🦞 evox browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`evox browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`evox browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 evox browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
