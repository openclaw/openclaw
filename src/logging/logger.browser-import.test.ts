...
describe("logging/logger browser-safe import", () => {
  const expectedLogDir = path.join(path.sep, "tmp", "openclaw");
  const expectedLogFile = path.join(expectedLogDir, "openclaw.log");
  ...
  it("does not resolve the preferred temp dir at import time when node fs is unavailable", async () => {
    ...
    expect(module.DEFAULT_LOG_DIR).toBe(expectedLogDir);
    expect(module.DEFAULT_LOG_FILE).toBe(expectedLogFile);
  });
