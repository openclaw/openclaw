...
import { getDiagnosticStabilitySnapshot, MAX_DIAGNOSTIC_STABILITY_LIMIT, } from "./diagnostic-stability.js";
...
export function resolveDiagnosticStabilityBundleDir(options = {}) {
...
function readOptionalMemorySummary(value) {
    ...
    const latest = memory.latest === undefined
        ? undefined
    ...
    return {
        ...
        ...(memory.maxHeapUsedBytes !== undefined
            ? {
                maxHeapUsedBytes: readNumber(memory.maxHeapUsedBytes, "snapshot.summary.memory.maxHeapUsedBytes"),
            }
            : {}),
...
export function listDiagnosticStabilityBundleFilesSync(options = {}) {
...
export function readDiagnosticStabilityBundleFileSync(file) {
    try {
        const stat = fs.statSync(file);
        if (stat.size > MAX_DIAGNOSTIC_STABILITY_BUNDLE_BYTES) {
            throw new Error(`Stability bundle is too large: ${stat.size} bytes exceeds ${MAX_DIAGNOSTIC_STABILITY_BUNDLE_BYTES}`);
        }
...
export function readLatestDiagnosticStabilityBundleSync(options = {}) {
    try {
...
export function writeDiagnosticStabilityBundleSync(options) {
    try {
...
export function writeDiagnosticStabilityBundleForFailureSync(reason, error, options = {}) {
...
export function installDiagnosticStabilityFatalHook(options = {}) {
...
export function uninstallDiagnosticStabilityFatalHook() {
    fatalHookUnsubscribe?.();
    fatalHookUnsubscribe = null;
}
export function resetDiagnosticStabilityBundleForTest() {
