...
let originalMullusiLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.MULLUSI_TEST_FILE_LOG;
  originalMullusiLogLevel = process.env.MULLUSI_LOG_LEVEL;
  delete process.env.MULLUSI_TEST_FILE_LOG;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.MULLUSI_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalMullusiLogLevel === undefined) {
    delete process.env.MULLUSI_LOG_LEVEL;
  } else {
    process.env.MULLUSI_LOG_LEVEL = originalMullusiLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.MULLUSI_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();
