...
type ConsoleConfigLoader = () => SmartAgentNeoConfig["logging"] | undefined;
const loadConfigFallbackDefault: ConsoleConfigLoader = () => {
  try {
    const loaded = requireConfig("../config/config.js") as {
      loadConfig?: () => SmartAgentNeoConfig;
    };
...
function normalizeConsoleLevel(level?: string): LogLevel {
  ...
  if (!level && process.env.VITEST === "true" && process.env.SMART_AGENT_NEO_TEST_CONSOLE !== "1") {
    return "silent";
  }
...
function resolveConsoleSettings(): ConsoleSettings {
  let cfg: SmartAgentNeoConfig["logging"] | undefined =
