...
function isPrivateSupportField(key) {
    return (SECRET_SUPPORT_FIELD_RE.test(key) ||
...
export function redactPathForSupport(file, options) {
    if (file.startsWith("$")) {
        return file;
    }
...
export function redactTextForSupport(value) {
...
function redactUrlSecretsForSupport(value) {
    return value
        .replace(URL_USERINFO_RE, (_match, scheme, _username, password) => password ? `${scheme}<redacted>:<redacted>@` : `${scheme}<redacted>@`)
        .replace(URL_PARAM_RE, (match, prefix, key) => isSensitiveUrlQueryParamName(key) ? `${prefix}${key}=<redacted>` : match);
}
...
export function redactSupportString(value, redaction, options = {}) {
...
export function sanitizeSupportSnapshotValue(value, redaction, key = "", depth = 0) {
...
export function sanitizeSupportConfigValue(value, redaction, key = "", depth = 0) {
    if (value == null || typeof value === "boolean") {
        return value;
    }
    if (typeof value === "number") {
