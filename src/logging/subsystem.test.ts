...
describe("createSubsystemLogger date rollover", () => {
  it("creates a new file logger when the date changes", () => {
    vi.useFakeTimers();
    // Use timestamps ≥48h apart so `formatLocalDate` returns different days
    // regardless of the machine's local timezone (worst-case offset is ±14h).
    ...
    // Advance the date by two full days (guaranteed different local date)
    vi.setSystemTime(new Date("2025-06-03T10:00:00Z"));
