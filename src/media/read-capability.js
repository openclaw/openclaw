...
import { getAgentScopedMediaLocalRoots, getAgentScopedMediaLocalRootsForSources, } from "./local-roots.js";
...
export function createAgentScopedHostMediaReadFile(params) {
    ...
    const inferredWorkspaceDir = params.workspaceDir ??
...
export function resolveAgentScopedOutboundMediaAccess(params) {
    ...
    const resolvedWorkspaceDir = params.workspaceDir ??
        params.mediaAccess?.workspaceDir ??
        (params.agentId ? resolveAgentWorkspaceDir(params.cfg, params.agentId) : undefined);
    const readFile = params.mediaAccess?.readFile ??
