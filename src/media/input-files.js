...
async function normalizeInputImage(params) {
    ...
    const sourceMime = (detectedMime && HEIC_INPUT_IMAGE_MIMES.has(detectedMime)) ||
    ...
    if (normalizedBuffer.byteLength > params.limits.maxBytes) {
        throw new Error(`Image too large after HEIC conversion: ${normalizedBuffer.byteLength} bytes (limit: ${params.limits.maxBytes} bytes)`);
    }
    return {
        type: "image",
