...
describe("image input pixel guard", () => {
  ...
  it("classifies image processor availability errors centrally", () => {
    ...
    expect(
      isImageProcessorUnavailableError(new Error("RASTERMILL_IMAGE_PROCESSOR_UNAVAILABLE")),
    ).toBe(true);
  });
  ...
  it("resizes grayscale alpha PNGs through Rastermill", async () => {
  ...
  it("honors PNG compression levels through Rastermill", async () => {
