...
import { isSafeOpenError, readLocalFileSafely } from "./store.runtime.js";
...
export async function cleanOldMedia(ttlMs = DEFAULT_TTL_MS, options = {}) {
        ...
        resolvePinnedHostnameImpl(parsedUrl.hostname)
            ...
            const req = requestImpl(parsedUrl, { headers, lookup: pinned.lookup }, (res) => {
                if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400) {
                    ...
                    const redirectHeaders = new URL(redirectUrl).origin === parsedUrl.origin
                        ? headers
...
function toSaveMediaSourceError(err, maxBytes = MAX_BYTES) {
    switch (err.code) {
        ...
        case "too-large":
            return new SaveMediaSourceError("too-large", `Media exceeds ${formatMediaLimitMb(maxBytes)} limit`, { cause: err });
        case "not-found":
...
export async function saveMediaSource(source, headers, subdir = "", maxBytes = MAX_BYTES) {
 ...
 *                regular file.
 */
export async function resolveMediaBufferPath(id, subdir = "inbound") {
 ...
 * @param subdir The subdirectory the file was saved into (default "inbound").
 */
export async function deleteMediaBuffer(id, subdir = "inbound") {
