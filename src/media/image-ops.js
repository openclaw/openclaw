...
function isMediaAttachmentImageOps(value) {
    ...
    return (typeof candidate.getImageMetadata === "function" &&
...
async function loadMediaAttachmentImageOps() {
    if (!mediaAttachmentImageOpsPromise) {
        mediaAttachmentImageOpsPromise = Promise.resolve()
            .then(async () => {
            const { loadBundledPluginPublicArtifactModuleSync } = await import("../plugins/public-surface-loader.js");
...
function createImagePixelLimitError(meta) {
    ...
    const detail = pixelCount === null
        ? `${meta.width}x${meta.height}`
        : `${meta.width}x${meta.height} (${pixelCount} pixels)`;
    return new Error(`Image dimensions exceed the ${MAX_IMAGE_INPUT_PIXELS.toLocaleString("en-US")} pixel input limit: ${detail}`);
}
