...
function resolvePluginConfigSchema<TConfigSchema>(
  configSchema: TConfigSchema | (() => TConfigSchema),
): TConfigSchema {
  return typeof configSchema === "function"
    ? (configSchema as () => TConfigSchema)()
    : configSchema;
}
...
export function defineChannelPluginEntry<TPlugin>({
  ...
  const getConfigSchema = (): ChannelEntryConfigSchema<TPlugin> => {
    resolvedConfigSchema ??= resolvePluginConfigSchema(
      (configSchema ?? emptyChannelConfigSchema) as
        | ChannelEntryConfigSchema<TPlugin>
        | (() => ChannelEntryConfigSchema<TPlugin>),
    ) as ChannelEntryConfigSchema<TPlugin>;
