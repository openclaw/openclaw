...
// external channel plugins (such as packages that import createTypingCallbacks
// via the openclaw/plugin-sdk/channel-runtime subpath) keep resolving on
// source checkouts of main.
...
describe("plugin-sdk channel-runtime compatibility alias", () => {
  ...
  it("does not re-export unrelated channel-outbound helpers", () => {
    ...
    expect(Object.keys(channelRuntime).toSorted()).toEqual([...RUNTIME_ALIAS_EXPORTS].toSorted());
  });
});
