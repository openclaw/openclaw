...
import { createPluginActivationSource, normalizePluginsConfig, resolveEffectivePluginActivationState, } from "../plugins/config-state.js";
...
let cachedManifestRegistryByKey = new Map();
const cachedFacadeManifestRecordsByKey = new Map();
...
function readFacadeBoundaryConfigSafely() {
    try {
        ...
        if (cachedBoundaryConfigFileState &&
        ...
        const rawConfig = parsed && typeof parsed === "object"
            ? parsed
...
function getFacadeBoundaryResolvedConfig() {
    ...
    if (cachedBoundaryResolvedConfig &&
    ...
    try {
        ...
        return {
            ...
            channels: Array.isArray(raw.channels)
                ? raw.channels.filter((entry) => typeof entry === "string")
                : [],
        };
    }
    catch {
...
function resolveBundledMetadataManifestRecord(params) {
    ...
    if (params.location.modulePath.startsWith(`${params.sourceExtensionsRoot}${path.sep}`)) {
        const relativeToExtensions = path.relative(params.sourceExtensionsRoot, params.location.modulePath);
    ...
    const relativeToBundledDir = path.relative(normalizedBundledPluginsDir, params.location.modulePath);
...
function resolveBundledPluginManifestRecord(params) {
    ...
    const resolved = (params.location
        ? registry.find((plugin) => {
            ...
            return (normalizedModulePath === normalizedRootDir ||
...
export function resetFacadeActivationCheckRuntimeStateForTest() {
