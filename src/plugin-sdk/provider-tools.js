...
export function findUnsupportedSchemaKeywords(schema, path, unsupportedKeywords) {
    if (!schema || typeof schema !== "object") {
        return [];
    }
    if (Array.isArray(schema)) {
        return schema.flatMap((item, index) => findUnsupportedSchemaKeywords(item, `${path}[${index}]`, unsupportedKeywords));
    }
    const record = schema;
    const violations = [];
    ...
    if (properties) {
        for (const [key, value] of Object.entries(properties)) {
            violations.push(...findUnsupportedSchemaKeywords(value, `${path}.properties.${key}`, unsupportedKeywords));
        }
    }
    for (const [key, value] of Object.entries(record)) {
        ...
        if (value && typeof value === "object") {
            violations.push(...findUnsupportedSchemaKeywords(value, `${path}.${key}`, unsupportedKeywords));
        }
    }
    return violations;
}
export function normalizeGeminiToolSchemas(ctx) {
...
export function inspectGeminiToolSchemas(ctx) {
...
export function normalizeOpenAIToolSchemas(ctx) {
...
function normalizeOpenAIStrictCompatSchemaRecursive(schema, options) {
    ...
    const hasObjectShapeHints = !("type" in normalized) &&
    ...
    const hasEmptyProperties = normalized.properties &&
        typeof normalized.properties === "object" &&
        !Array.isArray(normalized.properties) &&
...
export function findOpenAIStrictSchemaViolations(schema, path, options) {
    if (Array.isArray(schema)) {
...
export function inspectOpenAIToolSchemas(ctx) {
    if (!shouldApplyOpenAIToolCompat(ctx)) {
        return [];
    }
