...
export function listSlackAccountIds(cfg: DNAConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: DNAConfig): string {
...
function mergeSlackAccountConfig(cfg: DNAConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: DNAConfig): ResolvedSlackAccount[] {
