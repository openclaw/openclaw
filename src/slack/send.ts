...
export async function sendMessageSlack(
  ...
  if (opts.mediaUrl) {
    ...
    for (const chunk of chunks.length ? chunks : [""]) {
      // Include explicit section blocks so receiving bots get full text.
      // Slack truncates both message.text AND auto-generated rich_text blocks
      // for bot messages, but preserves explicit section block content.
      // Split chunk into sub-chunks of ≤SLACK_SECTION_TEXT_LIMIT (3000) chars
      // since Slack silently truncates section block text beyond 3000 chars.
      const sectionBlocks = chunk
        ? (() => {
            const blocks: { type: "section"; text: { type: "mrkdwn"; text: string } }[] = [];
            for (let i = 0; i < chunk.length; i += SLACK_SECTION_TEXT_LIMIT) {
              blocks.push({ type: "section", text: { type: "mrkdwn", text: chunk.slice(i, i + SLACK_SECTION_TEXT_LIMIT) } });
            }
            return blocks;
          })()
        : undefined;
