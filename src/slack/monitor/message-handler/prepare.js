...
import { resolveSlackAttachmentContent, MAX_SLACK_MEDIA_FILES, resolveSlackMedia, resolveSlackThreadHistory, resolveSlackThreadStarter, } from "../media.js";
...
export async function prepareSlackMessage(params) {
    ...
    const fileOnlyFallback = !mediaPlaceholder && (message.files?.length ?? 0) > 0
        ? message
    ...
    const botAttachmentText = isBotMessage && !attachmentContent?.text
        ? (message.attachments ?? [])
