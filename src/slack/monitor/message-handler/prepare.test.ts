...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "activi-slack-thread-"));
  });
  ...
  function createThreadSlackCtx(params: { cfg: ActiviConfig; replies: unknown }) {
...
describe("prepareSlackMessage sender prefix", () => {
  ...
  it("prefixes channel bodies with sender label", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/activi", enabled: true },
    });
