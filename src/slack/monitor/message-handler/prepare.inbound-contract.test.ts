...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  it("excludes thread_ts from top-level messages", async () => {
    ...
    const message: SlackMessageEvent = {
      ...
      ts: "1.000",
      // No thread_ts - this is not a reply
    } as SlackMessageEvent;
  ...
  it("includes only thread_ts when parent_user_id is missing", async () => {
    const slackCtx = createSlackMonitorContext({
      cfg: {
        channels: { slack: { enabled: true } },
      } as OpenClawConfig,
    ...
    expect(prepared).toBeTruthy();
    // Verify thread_ts is present but parent_user is not in the message footer
    expect(prepared!.ctxPayload.Body).toMatch(
      /\[slack message id: 1\.002 channel: D123 thread_ts: 1\.000\]/,
    );
    expect(prepared!.ctxPayload.Body).not.toContain("parent_user");
  });
});
