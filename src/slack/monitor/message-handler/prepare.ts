...
export async function prepareSlackMessage(params: {
  ...
  // For bot messages, the Events API may truncate both message.text and blocks.
  // Re-fetch via conversations.history to get the full message content.
  let effectiveText = (message.text ?? "").trim();
  let effectiveBlocks = message.blocks;
  if (message.bot_id && message.channel && message.ts) {
    try {
      const historyResult = await ctx.app.client.conversations.history({
        token: ctx.botToken,
      ...
      const fullMsg = historyResult.messages?.[0];
      if (fullMsg?.text && fullMsg.text.length > effectiveText.length) {
        effectiveText = fullMsg.text.trim();
      }
      if (fullMsg?.blocks && Array.isArray(fullMsg.blocks) && fullMsg.blocks.length > 0) {
        effectiveBlocks = fullMsg.blocks as unknown as typeof message.blocks;
      }
    } catch {
      // Fall back to event payload text if API call fails
    }
  }
  const blocksText = effectiveBlocks?.length ? extractFullTextFromBlocks(effectiveBlocks) : undefined;
  const blocksTextLen = blocksText?.length ?? 0;
  const effectiveTextLen = effectiveText.length;
  const primaryText = (blocksTextLen >= effectiveTextLen) ? blocksText : effectiveText;
  const which = (blocksTextLen >= effectiveTextLen) ? "blocksText" : "effectiveText";
  if (blocksTextLen !== effectiveTextLen) {
    console.log('[SLACK-TRUNC] blocksText=%d effectiveText=%d using=%s', blocksTextLen, effectiveTextLen, which);
  }
  const rawBody =
    [primaryText || effectiveText, attachmentContent?.text, mediaPlaceholder, fileOnlyPlaceholder]
      .filter(Boolean)
      .join("\n") || "";
  if (!rawBody) {
    return null;
