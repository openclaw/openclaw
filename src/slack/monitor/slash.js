...
import { createSlackExternalArgMenuStore, SLACK_EXTERNAL_ARG_MENU_PREFIX, } from "./external-arg-menu-store.js";
...
export async function registerSlackMonitorSlashCommands(params) {
    ...
    const handleSlashCommand = async (p) => {
        ...
        try {
            ...
            const { allowFromLower: effectiveAllowFromLower } = await resolveSlackEffectiveAllowFrom(ctx, {
    ...
    if (nativeCommands.length > 0) {
        ...
        ctx.app.command(buildSlackSlashCommandMatcher(slashCommand.name), async ({ command, ack, respond, body }) => {
            await handleSlashCommand({
                command,
                ack,
                respond,
