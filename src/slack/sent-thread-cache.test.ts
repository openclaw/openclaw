...
describe("slack sent-thread-cache", () => {
  ...
  it("records and checks thread participation", () => {
    ...
    expect(hasSlackThreadParticipation("A1", "C123", "1700000000.000001")).toBe(
      true,
    );
  });
  ...
  it("distinguishes different channels and threads", () => {
    recordSlackThreadParticipation("A1", "C123", "1700000000.000001");
    expect(hasSlackThreadParticipation("A1", "C123", "1700000000.000002")).toBe(
      false,
    );
    expect(hasSlackThreadParticipation("A1", "C456", "1700000000.000001")).toBe(
      false,
    );
  });
  ...
  it("scopes participation by accountId", () => {
    recordSlackThreadParticipation("A1", "C123", "1700000000.000001");
    expect(hasSlackThreadParticipation("A2", "C123", "1700000000.000001")).toBe(
      false,
    );
    expect(hasSlackThreadParticipation("A1", "C123", "1700000000.000001")).toBe(
      true,
  ...
  it("ignores empty accountId, channelId, or threadTs", () => {
    ...
    expect(hasSlackThreadParticipation("", "C123", "1700000000.000001")).toBe(
      false,
    );
    expect(hasSlackThreadParticipation("A1", "", "1700000000.000001")).toBe(
      false,
    );
  ...
  it("clears all entries", () => {
    ...
    expect(hasSlackThreadParticipation("A1", "C456", "1700000000.000002")).toBe(
      false,
    );
  });
  ...
  it("enforces maximum entries by evicting oldest fresh entries", () => {
    for (let i = 0; i < 5001; i += 1) {
      recordSlackThreadParticipation(
        "A1",
        "C123",
        `1700000000.${String(i).padStart(6, "0")}`,
      );
    }

    expect(hasSlackThreadParticipation("A1", "C123", "1700000000.000000")).toBe(
      false,
    );
    expect(hasSlackThreadParticipation("A1", "C123", "1700000000.005000")).toBe(
      true,
    );
  });
});
