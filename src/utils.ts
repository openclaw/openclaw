...
export function resolveConfigDir(
  ...
  const override = env.MULLUSI_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, homedir);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".mullusi");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.MULLUSI_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$MULLUSI_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via MULLUSI_STATE_DIR.
