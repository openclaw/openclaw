...
export function resolveConfigDir(
  ...
  const override = env.ACTIVI_STATE_DIR?.trim() || env.ACTIVI_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".activi");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ACTIVI_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ACTIVI_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ACTIVI_STATE_DIR.
