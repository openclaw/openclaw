...
export function resolveConfigDir(
  ...
  const override = env.EASYHUB_STATE_DIR?.trim() || env.EASYHUB_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".easyhub");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.EASYHUB_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$EASYHUB_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via EASYHUB_STATE_DIR.
