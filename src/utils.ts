...
export function resolveConfigDir(
  ...
  const override = env.SMART_AGENT_NEO_STATE_DIR?.trim() || env.NEOBOT_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".smart-agent-neo");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SMART_AGENT_NEO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SMART_AGENT_NEO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SMART_AGENT_NEO_STATE_DIR.
