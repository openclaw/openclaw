...
export const zh_TW = {
  ...
  wizard: {
    ...
    migration: {
      ...
      includeCredentials: "同時匯入支援的驗證憑據？",
      previewTitle: "遷移預覽",
      setupModelSeparately: "單獨設定模型",
      setupModelSeparatelyHint: "不匯入其他 agent，直接設定模型驗證",
    ...
    setup: {
      agentAssistedInstructions:
        "你正在引導 OpenClaw 輔助設定。本機 Gateway 已安全設定，並將在本次設定工作階段期間保持執行；不要要求使用者先設定 Gateway 才能設定頻道。臨時本機 Gateway 會在本次 TUI 工作階段期間占用已設定連接埠。不要執行 Gateway 安裝、啟動、重新啟動或停止命令，也不要從本 TUI 安裝持久背景服務。如果使用者需要持久服務，請讓其退出本 TUI 後執行 `openclaw onboard --install-daemon`。第一輪只問一個簡短問題，並明確讓使用者選擇要設定的項目：訊息頻道；Gateway 網路存取和 Tailscale；模型提供者和認證；Web 搜尋；技能和外掛；Hooks 和自動化；Control UI；語音/TTS；或立即開始使用 OpenClaw。使用者選擇後，檢查目前設定，只詢問必要的決定，並在使用者確認後進行所需變更。所有選用步驟都應可略過。對於每個訊息頻道設定請求，不要依賴記憶或靜態頻道清單。先執行 `openclaw channels list --all --json`。當所選頻道有 `docsPath` 時，透過 `web_fetch` 閱讀 `https://docs.openclaw.ai${docsPath}` 的完整官方指南。如果缺少 `docsPath` 或無法閱讀完整指南，請說明無法驗證頻道專屬說明，並改用引導式設定。檢查 `openclaw channels add --help`，瞭解目前安裝版本公開的非互動參數。當使用者詢問如何設定頻道時，先根據已驗證的指南回答，再請求必要輸入。說明提供者端的先決條件和使用者下一步操作。引導式設定請執行不帶參數的 `openclaw channels add` 並在精靈中選擇頻道。只有在已驗證的官方指南有記錄，且目前安裝版本說明公開所有必要參數時，才使用非互動的 `openclaw channels add --channel <id> ...` 命令。最後透過 `openclaw channels status --probe` 驗證。不要臆測頻道專屬設定說明；如果有環境變數、檔案、登入或引導式設定路徑，不要要求使用者在聊天中貼上密鑰。第一輪不要描述計畫或進行變更。",
      agentAssistedMessage: "幫我完成 OpenClaw 設定。",
      agentAssistedOpening: "核心設定和 Gateway 已就緒。正在開啟 agent 輔助設定。",
      agentAssistedGatewayReady: "Gateway 已設定，並將在本次輔助設定工作階段期間保持執行。",
      agentAssistedGatewayReadyTitle: "Gateway 已就緒",
      agentAssistedSkipped:
        "核心設定已完成。執行 `openclaw` 開始使用 agent，或重新執行 `openclaw onboard` 進行 agent 輔助設定。",
      agentNotRunnable: "所選模型認證未能讓預設 agent 正常執行。",
