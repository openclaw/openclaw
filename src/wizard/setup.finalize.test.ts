...
const probeGatewayReachable = vi.hoisted(() =>
	vi.fn<() => Promise<{ ok: boolean; detail?: string }>>(async () => ({
		ok: true,
	})),
);
...
const gatewayServiceRestart = vi.hoisted(() =>
	vi.fn<() => Promise<{ outcome: "completed" } | { outcome: "scheduled" }>>(
		async () => ({
			outcome: "completed",
		}),
	),
...
const resolveExistingKey = vi.hoisted(() =>
	vi.fn<(config: OpenClawConfig, provider: string) => string | undefined>(
		() => undefined,
	),
);
...
const hasKeyInEnv = vi.hoisted(() =>
	vi.fn<(entry: Pick<PluginWebSearchProviderEntry, "envVars">) => boolean>(
		() => false,
	),
);
...
vi.mock("../daemon/service.js", () => ({
	describeGatewayServiceRestart: vi.fn(
		(serviceNoun: string, result: { outcome: string }) =>
...
function expectFirstOnboardingInstallPlanCallOmitsToken() {
	const [firstArg] =
		(buildGatewayInstallPlan.mock.calls.at(0) as
...
describe("finalizeSetupWizard", () => {
	beforeEach(() => {
		...
		readSystemdUserLingerStatus.mockResolvedValue({
			user: "test-user",
			linger: "yes",
		});
	...
	it("resolves gateway password SecretRef for probe and TUI", async () => {
		...
		resolveSetupSecretInputString.mockResolvedValueOnce(
	...
	it("stops after a scheduled restart instead of reinstalling the service", async () => {
		...
		expect(progressStop).toHaveBeenCalledWith(
			"Gateway service restart scheduled.",
		);
	});
	...
	it("only reports legacy auto-detect for runtime-visible providers", async () => {
		...
		hasExistingKey.mockImplementation(
			(_config, provider) => provider === "perplexity",
		);
	...
	it("uses configured provider resolution instead of the active runtime registry", async () => {
		...
		hasExistingKey.mockImplementation(
			(_config, provider) => provider === "firecrawl",
		);
