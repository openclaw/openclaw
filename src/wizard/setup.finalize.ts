...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      try {
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.mullusi.com/gateway/health",
            "https://docs.mullusi.com/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("mullusi gateway run")}`,
          `Or rerun with: ${formatCliCommand("mullusi onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("mullusi onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.mullusi.com/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: $MULLUSI_CONFIG_PATH (default: ~/.mullusi/mullusi.json) under gateway.auth.token, or in MULLUSI_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("mullusi config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("mullusi doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("mullusi dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ...
            : "Copy/paste this URL in a browser on this machine to control Mullusi.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("mullusi dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.mullusi.com/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.mullusi.com/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("mullusi configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("mullusi configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.mullusi.com/tools/web"}`,
          "Docs: https://docs.mullusi.com/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("mullusi configure --section web")}`,
          "",
          "Docs: https://docs.mullusi.com/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://mullusi.com/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Mullusi."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Mullusi.",
  );
