...
describe("setupPluginConfig", () => {
  ...
  it.each([
    { name: "number", values: [1, 2], selectedIndex: 1, expected: 2 },
    { name: "boolean", values: [true, false], selectedIndex: 1, expected: false },
    {
      name: "object with a duplicate label",
      values: [{ mode: "first" }, { mode: "second" }],
      selectedIndex: 1,
      expected: { mode: "second" },
    },
  ])("preserves a selected $name enum value", async ({ values, selectedIndex, expected }) => {
