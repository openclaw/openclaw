...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Mullusi is a hobby project and still in beta. Expect sharp edges.",
      "By default, Mullusi is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Mullusi is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Mullusi.",
      ...
      "",
      "Must read: https://docs.mullusi.com/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("Mullusi setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: MullusiConfig = snapshot.valid
    ? snapshot.exists
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.mullusi.com/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("mullusi doctor")}`,
        `Inspect: ${formatCliCommand("mullusi plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("mullusi configure")}.`;
  ...
  let localGatewayToken = process.env.MULLUSI_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.MULLUSI_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: MullusiConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
