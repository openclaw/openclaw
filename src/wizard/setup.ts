...
async function resolveAuthChoiceModelSelectionPolicy(params: {
	...
	const matchedProvider =
		...
		(preferredProvider
			? providers.find(
					(provider) => provider.id.trim() === preferredProvider.trim(),
				)
			: undefined);
	const setupPolicy =
		resolvedChoice?.wizard?.modelSelection ??
		matchedProvider?.wizard?.setup?.modelSelection;

	return {
		preferredProvider,
		promptWhenAuthChoiceProvided:
			setupPolicy?.promptWhenAuthChoiceProvided === true,
...
export async function runSetupWizard(
	...
	let remoteGatewayToken = normalizeSecretInputString(
		baseConfig.gateway?.remote?.token,
	);
	try {
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import(
			"../commands/onboard-remote.js"
		);
	...
	const { applyLocalSetupWorkspaceConfig } = await import(
	...
	const { ensureAuthProfileStore } = await import(
		"../agents/auth-profiles.runtime.js"
	);
	const { promptAuthChoiceGrouped } = await import(
		"../commands/auth-choice-prompt.js"
	);
	const { promptCustomApiConfig } = await import(
		"../commands/onboard-custom.js"
	);
	const {
		applyAuthChoice,
	...
	const { applyPrimaryModel, promptDefaultModel } = await import(
		"../commands/model-picker.js"
	);
	...
	const shouldPromptModelSelection =
		...
		(authChoiceFromPrompt ||
			authChoiceModelSelectionPolicy?.promptWhenAuthChoiceProvided === true);
	...
	const { configureGatewayForSetup } = await import(
