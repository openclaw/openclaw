...
export async function runSetupWizard(
  ...
  if (authChoice === "codexbar") {
    ...
    // Users will connect CodexBar as a device after onboarding completes.
    await prompter.note(
      [
        ...
        "",
        "You can also connect CodexBar as a device:",
        "  openclaw devices approve codexbar",
      ].join("\n"),
      "CodexBar LM Manager",
    );
    // No LM config changes — CodexBar injects them after setup
  ...
  const authChoiceModelSelectionPolicy =
    authChoice === "custom-api-key" || authChoice === "codexbar"
      ? undefined
