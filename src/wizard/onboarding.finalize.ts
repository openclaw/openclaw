...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.easyhub/easyhub.json (gateway.auth.token) or EASYHUB_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("EasyHub config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("EasyHub doctor --generate-gateway-token")}`,
        "Web UI stores a copy in this browser's localStorage (EasyHub.control.settings.v1).",
        `Open the dashboard anytime: ${formatCliCommand("EasyHub dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ...
            : "Copy/paste this URL in a browser on this machine to control EasyHub.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("EasyHub dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.EasyHub.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.EasyHub.ai/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ...
      : [
          ...
          "",
          "EasyHub uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand("EasyHub configure --section web")}`,
  ...
  await prompter.note(
    'What now: https://EasyHub.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control EasyHub."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control EasyHub.",
  );
