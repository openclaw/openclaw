...
function collectConfigSecrets(params: {
  ...
  for (const target of discoverConfigSecretTargets(params.config)) {
    ...
    // Only apply the non-secret apiKey marker exemption to actual apiKey fields
    // (e.g., models.providers.*.apiKey) — not to other secret targets that
    // happen to carry a known marker value. See Sweeper P1 feedback.
    if (target.path.endsWith(".apiKey") && isNonSecretApiKeyMarker(target.value)) {
      continue;
    }
    addFinding(params.collector, {
      code: "PLAINTEXT_FOUND",
