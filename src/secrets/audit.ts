...
import {
	getSkippedExecRefStaticError,
	selectRefsForExecPolicy,
} from "./exec-resolution-policy.js";
...
import { describeUnknownError, isNonEmptyString, isRecord } from "./shared.js";
import {
...
function collectModelsJsonSecrets(params: {
	...
	for (const [providerId, providerValue] of Object.entries(parsed.providers)) {
		...
		const headers = isRecord(providerValue.headers)
...
async function collectUnresolvedRefFindings(params: {
	...
	for (const refsForProvider of refsByProvider.values()) {
		...
		const fallback = await runTasksWithConcurrency({
			tasks,
			limit: Math.min(
				REF_RESOLVE_FALLBACK_CONCURRENCY,
				selectedRefs.refsToResolve.length,
			),
			errorMode: "continue",
...
export async function runSecretsAudit(
	...
	if (snapshot.valid) {
		...
		for (const modelsJsonPath of listAgentModelsJsonPaths(
			config,
			stateDir,
			env,
		)) {
