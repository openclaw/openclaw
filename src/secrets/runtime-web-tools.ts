...
export async function resolveRuntimeWebTools(params: {
	...
	const tools = isRecord(params.sourceConfig.tools)
	...
	const rawProvider =
		typeof search?.provider === "string"
			...
			: "";
	const configuredBundledPluginId =
		resolveBundledWebSearchPluginId(rawProvider);
	...
	if (searchEnabled) {
		...
		for (const provider of candidates) {
			...
			if (
				resolution.secretRefConfigured &&
				resolution.fallbackUsedAfterRefFailure
			) {
				const diagnostic: RuntimeWebDiagnostic = {
			...
			if (
				resolution.secretRefConfigured &&
				!resolution.value &&
				resolution.unresolvedRefReason
			) {
		...
		const failUnresolvedSearchNoFallback = (unresolved: {
			...
			throw new Error(
				`[WEB_SEARCH_KEY_UNRESOLVED_NO_FALLBACK] ${unresolved.reason}`,
			);
		};
	...
	if (xSearchEnabled) {
		...
		if (configuredInlineValue) {
			...
			const envFallback = readNonEmptyEnvValue(params.context.env, [
				"XAI_API_KEY",
			]);
