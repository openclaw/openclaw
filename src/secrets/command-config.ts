...
export function analyzeCommandSecretAssignmentsFromSnapshot(params: {
	...
	for (const target of discoverConfigSecretTargetsByIds(
		params.sourceConfig,
		params.targetIds,
	)) {
		...
		const inlineCandidateRef = explicitRef
			? coerceSecretRef(target.value, defaults)
			: null;
		if (!ref) {
			continue;
		}
		...
		const hasCompetingSiblingRef =
			...
			explicitRef &&
			inlineCandidateRef; // pragma: allowlist secret
