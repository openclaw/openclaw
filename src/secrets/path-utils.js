    ...
    for (let index = 0; index < params.segments.length - 1; index += 1) {
        ...
        if (Array.isArray(cursor)) {
            if (!isArrayIndexSegment(segment)) {
                throw new Error(`Invalid array index segment "${segment}" at ${params.segments.join(".")}.`);
            }
            const arrayIndex = Number.parseInt(segment, 10);
            if (params.requireExistingSegment && (arrayIndex < 0 || arrayIndex >= cursor.length)) {
                throw new Error(`Path segment does not exist at ${params.segments.slice(0, index + 1).join(".")}.`);
            }
            cursor = cursor[arrayIndex];
            continue;
        }
        if (!isRecord(cursor)) {
            throw new Error(`Invalid path shape at ${params.segments.slice(0, index).join(".") || "<root>"}.`);
        }
...
export function setPathCreateStrict(root, segments, value) {
    if (segments.length === 0) {
        throw new Error("Target path is empty.");
    }
    let cursor = root;
...
export function setPathExistingStrict(root, segments, value) {
...
export function deletePathStrict(root, segments) {
