...
type CredentialMatrixEntry = {
  id: string;
  configFile: "mullusi.json" | "auth-profiles.json";
  path: string;
  refPath?: string;
...
export type SecretRefCredentialMatrixDocument = {
  ...
  scope: "Credentials that are strictly user-supplied and not minted/rotated by Mullusi runtime.";
...
export function buildSecretRefCredentialMatrix(): SecretRefCredentialMatrixDocument {
  ...
  return {
    ...
    pathSyntax: 'Dot path with "*" for map keys and "[]" for arrays.',
    scope:
      "Credentials that are strictly user-supplied and not minted/rotated by Mullusi runtime.",
