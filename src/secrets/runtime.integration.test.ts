...
function expectResolvedOpenAIRuntime(agentDir: string) {
	...
	expect(
		ensureAuthProfileStore(agentDir).profiles["openai:default"],
	).toMatchObject({
		type: "api_key",
		key: "sk-file-runtime",
	});
...
describe("secrets runtime snapshot integration", () => {
	...
	it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
		await withEnvAsync(
			...
			async () => {
				...
				expect(
					ensureAuthProfileStore("/tmp/openclaw-agent-main").profiles[
						"openai:default"
					],
				).toMatchObject({
					type: "api_key",
	...
	it("keeps active secrets runtime snapshots resolved after config writes", async () => {
		...
		await withTempHome("openclaw-secrets-runtime-write-", async (home) => {
			const { secretFile, agentDir } =
				await createOpenAIFileRuntimeFixture(home);
	...
	it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
		...
		await withTempHome(
			"openclaw-secrets-runtime-refresh-fail-",
			async (home) => {
				const { secretFile, agentDir } =
					await createOpenAIFileRuntimeFixture(home);
				...
				expectResolvedOpenAIRuntime(agentDir);
				expect(
					activeAfterFailure?.sourceConfig.models?.providers?.openai?.apiKey,
				).toEqual(OPENAI_FILE_KEY_REF);
			},
		);
	});
	...
	it(
		...
		async () => {
			await withTempHome(
				"openclaw-secrets-runtime-web-reload-lkg-",
				async (home) => {
					const prepared = await prepareSecretsRuntimeSnapshot({
						config: asConfig({
							tools: {
					...
					expect(
						activeAfterFailure?.sourceConfig.tools?.web?.search?.gemini?.apiKey,
					).toEqual({
						source: "env",
						provider: "default",
						id: "WEB_SEARCH_GEMINI_API_KEY",
					...
					const persistedGoogleWebSearchConfig = persistedConfig.plugins
						?.entries?.google?.config as
	...
	it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
		await withTempHome("openclaw-secrets-runtime-agent-dirs-", async (home) => {
			...
			activateSecretsRuntimeSnapshot(prepared);
			expect(
				ensureAuthProfileStore(opsAgentDir).profiles["anthropic:ops"],
			).toBeUndefined();
