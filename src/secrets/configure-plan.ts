...
function getSecretProviders(config: MullusiConfig): Record<string, SecretProviderConfig> {
...
export function buildConfigureCandidates(config: MullusiConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredMullusiConfig?: MullusiConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredMullusiConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const mullusiCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...mullusiCandidates, ...authCandidates].toSorted((a, b) =>
