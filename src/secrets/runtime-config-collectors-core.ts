...
function collectAgentMemorySearchAssignments(params: {
	...
	const defaultsConfig = isRecord(agents.defaults)
		? agents.defaults
		: undefined;
	...
	for (const rawAgent of list) {
		...
		const memorySearch = isRecord(rawAgent.memorySearch)
			? rawAgent.memorySearch
			: undefined;
		...
		const remote = isRecord(memorySearch.remote)
			? memorySearch.remote
			: undefined;
	...
	if (defaultsMemorySearch && isRecord(defaultsMemorySearch.remote)) {
		...
		collectSecretInputAssignment({
			...
			active:
				defaultsEnabled &&
				(hasEnabledAgentWithoutOverride || list.length === 0),
	...
	list.forEach((rawAgent, index) => {
		...
		const enabled =
			rawAgent.enabled !== false && memorySearch.enabled !== false;
		collectSecretInputAssignment({
			value: remote.apiKey,
...
function collectSandboxSshAssignments(params: {
	...
	const agents = isRecord(params.config.agents)
	...
	const defaultsSandbox = isRecord(defaultsAgent?.sandbox)
	...
	const defaultsBackend =
		typeof defaultsSandbox?.backend === "string"
			? defaultsSandbox.backend
			: undefined;
	const defaultsMode =
		typeof defaultsSandbox?.mode === "string"
			? defaultsSandbox.mode
			: undefined;
	...
	const list = Array.isArray(agents.list) ? agents.list : [];
	list.forEach((rawAgent, index) => {
		const agentRecord = isRecord(rawAgent)
			? (rawAgent as Record<string, unknown>)
			: null;
		if (!agentRecord || agentRecord.enabled === false) {
			return;
		}
		const sandbox = isRecord(agentRecord.sandbox)
			? agentRecord.sandbox
			: undefined;
		...
		const effectiveMode =
			...
			"off";
		const active =
			effectiveBackend.trim().toLowerCase() === "ssh" &&
			effectiveMode !== "off";
		for (const key of [
			"identityData",
			"certificateData",
	...
	const defaultsActive =
		(defaultsBackend?.trim().toLowerCase() === "ssh" &&
			defaultsMode !== "off") ||
