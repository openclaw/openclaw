...
import { formatExecSecretRefIdValidationMessage, isValidExecSecretRefId, SINGLE_VALUE_FILE_REF_ID, resolveDefaultSecretProviderAlias, secretRefKey, } from "./ref-contract.js";
...
export function isProviderScopedSecretResolutionError(value) {
...
export async function resolveSecretRefValues(refs, options) {
    ...
    for (const ref of refs) {
        ...
        if (ref.source === "exec" && !isValidExecSecretRefId(id)) {
            throw new Error(`${formatExecSecretRefIdValidationMessage()} (ref: ${ref.source}:${ref.provider}:${id}).`);
        }
