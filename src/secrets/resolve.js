...
const DEFAULT_EXEC_NO_OUTPUT_TIMEOUT_MS = 2000;
...
function resolveConfiguredProvider(ref, config) {
    ...
    if (providerConfig.source !== ref.source) {
        throw new Error(`Secret provider "${ref.provider}" has source "${providerConfig.source}" but ref requests "${ref.source}".`);
    }
...
async function assertSecurePath(params) {
    ...
    if (process.platform === "win32" && perms.source === "unknown") {
        throw new Error(`${params.label} ACL verification unavailable on Windows for ${effectivePath}.`);
    }
...
async function resolveEnvRefs(params) {
    ...
    for (const ref of params.refs) {
        if (allowlist && !allowlist.has(ref.id)) {
            throw new Error(`Environment variable "${ref.id}" is not allowlisted in secrets.providers.${params.providerName}.allowlist.`);
        }
    ...
    if (mode === "singleValue") {
        for (const ref of params.refs) {
            if (ref.id !== SINGLE_VALUE_FILE_REF_ID) {
                throw new Error(`singleValue file provider "${params.providerName}" expects ref id "${SINGLE_VALUE_FILE_REF_ID}".`);
            }
        ...
        const append = (chunk, target) => {
            ...
            if (outputBytes > params.maxOutputBytes) {
                ...
                if (!settled) {
                    settled = true;
                    clearTimers();
                    reject(new Error(`Exec provider output exceeded maxOutputBytes (${params.maxOutputBytes}).`));
                }
                return;
            }
            if (target === "stdout") {
    ...
    for (const id of params.ids) {
        if (responseErrors && id in responseErrors) {
            ...
            if (isRecord(entry) && typeof entry.message === "string" && entry.message.trim()) {
                throw new Error(`Exec provider "${params.providerName}" failed for id "${id}" (${entry.message.trim()}).`);
            }
...
async function resolveExecRefs(params) {
    ...
    if (ids.length > params.limits.maxRefsPerProvider) {
        throw new Error(`Exec provider "${params.providerName}" exceeded maxRefsPerProvider (${params.limits.maxRefsPerProvider}).`);
    }
    ...
    if (Buffer.byteLength(input, "utf8") > params.limits.maxBatchBytes) {
        throw new Error(`Exec provider "${params.providerName}" request exceeded maxBatchBytes (${params.limits.maxBatchBytes}).`);
    }
    const childEnv = {};
    ...
    if (result.termination === "no-output-timeout") {
        throw new Error(`Exec provider "${params.providerName}" produced no output for ${noOutputTimeoutMs}ms.`);
    }
    if (result.code !== 0) {
        throw new Error(`Exec provider "${params.providerName}" exited with code ${String(result.code)}.`);
    }
...
export async function resolveSecretRefValues(refs, options) {
    if (refs.length === 0) {
        return new Map();
    }
    ...
    const tasks = [...grouped.values()].map((group) => async () => {
        if (group.refs.length > limits.maxRefsPerProvider) {
            throw new Error(`Secret provider "${group.providerName}" exceeded maxRefsPerProvider (${limits.maxRefsPerProvider}).`);
        }
    ...
    for (const result of taskResults.results) {
        for (const ref of result.group.refs) {
            if (!result.values.has(ref.id)) {
                throw new Error(`Secret provider "${result.group.providerName}" did not return id "${ref.id}".`);
            }
...
export async function resolveSecretRefValue(ref, options) {
    const cache = options.cache;
    const key = secretRefKey(ref);
    if (cache?.resolvedByRefKey?.has(key)) {
        return await cache.resolvedByRefKey.get(key);
    }
    const promise = (async () => {
...
export async function resolveSecretRefString(ref, options) {
    const resolved = await resolveSecretRefValue(ref, options);
    if (!isNonEmptyString(resolved)) {
        throw new Error(`Secret reference "${ref.source}:${ref.provider}:${ref.id}" resolved to a non-string or empty value.`);
    }
    return resolved;
}
