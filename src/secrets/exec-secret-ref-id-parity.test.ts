...
describe("exec SecretRef id parity", () => {
	...
	for (const id of [
		...
		it(`keeps config/plan/gateway/plugin parity for exec id "${id}"`, () => {
			...
			expect(
				validateGatewaySecretRef({ source: "exec", provider: "vault", id }),
			).toBe(expected);
			expect(
				pluginSdkSecretInput.safeParse({
					source: "exec",
					provider: "vault",
					id,
				}).success,
	...
	function classifyTargetClass(id: string): string {
		...
		if (
			id.startsWith("plugins.entries.") &&
			id.includes(".config.webSearch.apiKey")
		) {
	...
	function samplePathSegments(pathPattern: string): string[] {
		...
		if (!segments) {
			throw new Error(
				`failed to sample path segments for pattern "${pathPattern}"`,
			);
		}
		return segments;
	}
	...
	const sampledTargetsByClass = [
		...new Set(
			registryPlanTargets.map((entry) => classifyTargetClass(entry.id)),
		),
	]
			...
			targets: [
				{
					...
					...(params.configFile === "auth-profiles.json"
						? { agentId: "main" }
						: {}),
				},
			],
