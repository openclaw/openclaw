...
async function promptOptionalPositiveInt(params: {
	...
	const raw = assertNoCancel(
		await text({
			message: params.message,
			initialValue:
				params.initialValue === undefined ? "" : String(params.initialValue),
			validate: (value) => {
...
function resolveCandidateProviderHint(
	candidate: ConfigureCandidate,
): string | undefined {
	if (
		typeof candidate.authProfileProvider === "string" &&
		candidate.authProfileProvider.trim()
	) {
...
function resolveSuggestedEnvSecretId(
	candidate: ConfigureCandidate,
): string | undefined {
...
function resolveConfigureAgentId(
	config: OpenClawConfig,
...
function normalizeAuthStoreForConfigure(
	...
	return {
		...
		...(isRecord(raw.order)
			? { order: raw.order as AuthProfileStore["order"] }
			: {}),
		...(isRecord(raw.lastGood)
			? { lastGood: raw.lastGood as AuthProfileStore["lastGood"] }
			: {}),
...
async function promptNewAuthProfileCandidate(
	agentId: string,
): Promise<ConfigureCandidate> {
...
export async function runSecretsConfigureInteractive(
	...
	if (!params.providersOnly) {
		const configureAgentId = resolveConfigureAgentId(
			snapshot.config,
			params.agentId,
		);
		...
		if (candidates.length === 0) {
			throw new Error(
				"No configurable secret-bearing fields found for this agent scope.",
			);
		}

		const sourceChoices = toSourceChoices(stagedConfig);
		const hasDerivedCandidates = candidates.some(
			(candidate) => candidate.isDerived === true,
		);
		...
		while (true) {
			...
			if (hasDerivedCandidates) {
				options.push({
					...
					label: showDerivedCandidates
						? "Hide derived targets"
			...
			if (selectedPath === "__create_auth_profile__") {
				const createdCandidate =
					await promptNewAuthProfileCandidate(configureAgentId);
				const key = configureCandidateKey(createdCandidate);
				const existingIndex = candidates.findIndex(
					(entry) => configureCandidateKey(entry) === key,
				);
				if (existingIndex >= 0) {
			...
			if (!suggestedId && source === "file") {
				const configuredProvider =
					stagedConfig.secrets?.providers?.[providerAlias];
				if (
					configuredProvider?.source === "file" &&
					configuredProvider.mode === "singleValue"
				) {
					suggestedId = "value";
				}
			}
