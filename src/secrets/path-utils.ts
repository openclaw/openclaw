...
function parseArrayLeafTarget(
	...
	if (!isArrayIndexSegment(leaf)) {
		throw new Error(
			`Invalid array index segment "${leaf}" at ${segments.join(".")}.`,
		);
	}
...
export function setPathCreateStrict(
	...
	for (let index = 0; index < segments.length - 1; index += 1) {
		...
		if (Array.isArray(cursor)) {
			if (!isArrayIndexSegment(segment)) {
				throw new Error(
					`Invalid array index segment "${segment}" at ${segments.join(".")}.`,
				);
			}
			const arrayIndex = Number.parseInt(segment, 10);
			...
			if (existing === undefined || existing === null) {
				...
				needs === "array" ? !Array.isArray(existing) : !isRecord(existing)
			) {
				throw new Error(
					`Invalid path shape at ${segments.slice(0, index + 1).join(".")}.`,
				);
			}
			cursor = cursor[arrayIndex];
			continue;
		...
		if (!isRecord(cursor)) {
			throw new Error(
				`Invalid path shape at ${segments.slice(0, index).join(".") || "<root>"}.`,
			);
		}
	...
	if (!isRecord(cursor)) {
		throw new Error(
			`Invalid path shape at ${segments.slice(0, -1).join(".") || "<root>"}.`,
		);
	}
...
export function setPathExistingStrict(
	...
	if (arrayTarget) {
		if (
			arrayTarget.index < 0 ||
			arrayTarget.index >= arrayTarget.array.length
		) {
...
export function deletePathStrict(
	root: OpenClawConfig,
	segments: string[],
): boolean {
	const cursor = traverseToLeafParent({
