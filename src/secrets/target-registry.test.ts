...
describe("secret target registry", () => {
	...
	it("stays in sync with docs/reference/secretref-credential-surface.md", () => {
		...
		const matrix = JSON.parse(matrixRaw) as ReturnType<
			typeof buildSecretRefCredentialMatrix
		>;
		...
		const readMarkedCredentialList = (params: {
			start: string;
			end: string;
		}): Set<string> => {
		...
		const supportedFromMatrix = new Set(
			matrix.entries.map((entry) =>
				entry.configFile === "auth-profiles.json" && entry.refPath
					? entry.refPath
					: entry.path,
			),
		);
		const unsupportedFromMatrix = new Set(
			matrix.excludedMutableOrRuntimeManaged,
		);

		expect([...supportedFromDocs].toSorted()).toEqual(
			[...supportedFromMatrix].toSorted(),
		);
		expect([...unsupportedFromDocs].toSorted()).toEqual(
			[...unsupportedFromMatrix].toSorted(),
		);
	});
	...
	it("resolves config targets by exact path including sibling ref metadata", () => {
		...
		expect(target?.refPathSegments).toEqual([
			"channels",
			"googlechat",
			"serviceAccountRef",
		]);
	...
	it("returns null when no config target path matches", () => {
		expect(
			resolveConfigSecretTargetByPath(["gateway", "auth", "mode"]),
		).toBeNull();
	});
});
