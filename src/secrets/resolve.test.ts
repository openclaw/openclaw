...
describe("secret ref resolver", () => {
	...
	itPosix(
		"uses timeoutMs as the default no-output timeout for exec providers",
		async () => {
	...
	itPosix(
		"supports non-JSON single-value exec output when jsonOnly is false",
		async () => {
			const value = await resolveExecSecret(execPlainScriptPath, {
				jsonOnly: false,
			});
	...
	itPosix(
		"rejects symlink command paths unless allowSymlinkCommand is enabled",
		async () => {
			...
			await expect(
				resolveExecSecret(symlinkPath, { jsonOnly: false }),
			).rejects.toThrow("must not be a symlink");
		},
	);

	itPosix(
		"allows symlink command paths when allowSymlinkCommand is enabled",
		async () => {
	...
	itPosix(
		...
		async () => {
			...
			await expect(
				resolveExecSecret(symlinkCommand, { args: ["brew"] }),
			).rejects.toThrow("must not be a symlink");
	...
	itPosix(
		"rejects exec refs with invalid JSON when jsonOnly is true",
		async () => {
			await expect(
				resolveExecSecret(execInvalidJsonScriptPath, { jsonOnly: true }),
	...
	it("rejects invalid exec ids before provider resolution", async () => {
		for (const id of INVALID_EXEC_SECRET_REF_IDS) {
			await expect(
				...
				/Exec secret reference id must match|Secret reference id is empty/,
			);
		}
	});
});
