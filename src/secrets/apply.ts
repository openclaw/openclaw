...
function planContainsExecReferences(plan: SecretsApplyPlan): boolean {
	...
	return Object.values(plan.providerUpserts ?? {}).some(
		(provider) => provider.source === "exec",
	);
}
	...
	for (const { target, resolved } of resolvedTargets) {
		if (resolved.entry.configFile === "auth-profiles.json") {
			...
			if (authStoreChanged) {
				...
				if (!agentId) {
					throw new Error(
						`Missing required agentId for auth-profiles target ${target.path}.`,
					);
				}
		...
		if (usesSiblingRef) {
			...
			const deletedLegacy = deletePathStrict(
				params.nextConfig,
				targetPathSegments,
			);
...
function scrubAuthStoresForProviderTargets(params: {
	...
	for (const authStorePath of listAuthProfileStorePaths(
		params.nextConfig,
		params.stateDir,
	)) {
		...
		for (const profile of iterateAuthProfileCredentials(nextStore.profiles)) {
			...
			if (
				profile.kind === "oauth" &&
				(profile.hasAccess || profile.hasRefresh)
			) {
...
function resolveAuthStoreForTarget(params: {
	...
	if (!agentId) {
		throw new Error(
			`Missing required agentId for auth-profiles target ${params.target.path}.`,
		);
	}
...
function ensureAuthProfileContainer(params: {
	...
	const wroteProfile = setPathCreateStrict(
		asConfigPathRoot(params.store),
		profilePathSegments,
		{
			type: expectedType,
...
function applyAuthProfileTargetMutation(params: {
	...
	if (usesSiblingRef) {
		...
		if (!refPathSegments) {
			throw new Error(
				`Missing sibling ref path for auth-profiles target ${params.target.path}.`,
			);
		}
		const wroteRef = setPathCreateStrict(
		...
		const deletedPlaintext = deletePathStrict(
			asConfigPathRoot(store),
			targetPathSegments,
		);
...
async function validateProjectedSecretsState(params: {
	...
	for (const { target, resolved: resolvedTarget } of params.resolvedTargets) {
		if (
			!params.write &&
			target.ref.source === "exec" &&
			!params.allowExecInDryRun
		) {
			skippedExecRefs += 1;
	...
	return {
		...
		resolvabilityComplete:
			params.write || params.allowExecInDryRun || skippedExecRefs === 0,
	};
}
