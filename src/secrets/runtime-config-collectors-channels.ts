...
function resolveChannelAccountSurface(
	...
	if (!isRecord(accounts) || Object.keys(accounts).length === 0) {
		return {
			...
			channelEnabled,
			accounts: [
				{ accountId: "default", account: channel, enabled: channelEnabled },
			],
		};
	}
...
function hasConfiguredSecretInputValue(
	...
	return (
		normalizeSecretStringValue(value).length > 0 ||
...
function collectTelegramAssignments(params: {
	...
	const baseTokenFile =
		typeof telegram.tokenFile === "string" ? telegram.tokenFile.trim() : "";
	...
	collectConditionalChannelFieldAssignments({
		...
		topLevelInheritedAccountActive: ({ account, enabled }) => {
			...
			return (
				!accountBotTokenConfigured && accountTokenFile(account).length === 0
			);
		},
		accountActive: ({ account, enabled }) =>
			enabled && accountTokenFile(account).length === 0,
		topInactiveReason:
			"no enabled Telegram surface inherits this top-level botToken (tokenFile is configured).",
		accountInactiveReason:
			"Telegram account is disabled or tokenFile is configured.",
	});
	const baseWebhookUrl =
		typeof telegram.webhookUrl === "string" ? telegram.webhookUrl.trim() : "";
	...
	collectConditionalChannelFieldAssignments({
		...
		accountActive: ({ account, enabled }) =>
			enabled && accountWebhookUrl(account).length > 0,
		topInactiveReason:
...
function collectSlackAssignments(params: {
	...
	const baseMode =
		slack.mode === "http" || slack.mode === "socket" ? slack.mode : "socket";
	...
	const resolveAccountMode = (account: Record<string, unknown>) =>
		account.mode === "http" || account.mode === "socket"
			? account.mode
			: baseMode;
	collectConditionalChannelFieldAssignments({
		channelKey: "slack",
		...
		topLevelInheritedAccountActive: ({ account, enabled }) =>
			enabled &&
			!hasOwnProperty(account, "appToken") &&
			resolveAccountMode(account) !== "http",
		accountActive: ({ account, enabled }) =>
			enabled && resolveAccountMode(account) !== "http",
		topInactiveReason:
			"no enabled Slack socket-mode surface inherits this top-level appToken.",
		accountInactiveReason:
			"Slack account is disabled or not running in socket mode.",
	});
	collectConditionalChannelFieldAssignments({
		channelKey: "slack",
		field: "signingSecret",
		...
		accountActive: ({ account, enabled }) =>
			enabled && resolveAccountMode(account) === "http",
		topInactiveReason:
			"no enabled Slack HTTP-mode surface inherits this top-level signingSecret.",
		accountInactiveReason:
			"Slack account is disabled or not running in HTTP mode.",
	});
}
...
function collectDiscordAssignments(params: {
	...
	collectSimpleChannelFieldAssignments({
		...
		context: params.context,
		topInactiveReason:
			"no enabled account inherits this top-level Discord token.",
	...
	collectNestedChannelFieldAssignments({
		...
		accountActive: ({ account, enabled }) =>
			enabled &&
			isRecord(account.pluralkit) &&
			isEnabledFlag(account.pluralkit),
		accountInactiveReason:
			"Discord account is disabled or PluralKit is disabled for this account.",
	});
	collectNestedChannelTtsAssignments({
		...
		accountInactiveReason:
			"Discord account is disabled or voice is disabled for this account.",
	});
}
...
function collectIrcAssignments(params: {
	...
	collectSimpleChannelFieldAssignments({
		...
		context: params.context,
		topInactiveReason:
			"no enabled account inherits this top-level IRC password.",
	...
	collectNestedChannelFieldAssignments({
		...
		accountInactiveReason:
			"IRC account is disabled or NickServ is disabled for this account.",
	});
}
...
function collectBlueBubblesAssignments(params: {
	...
	collectSimpleChannelFieldAssignments({
		...
		context: params.context,
		topInactiveReason:
			"no enabled account inherits this top-level BlueBubbles password.",
...
function collectMattermostAssignments(params: {
	...
	collectSimpleChannelFieldAssignments({
		...
		context: params.context,
		topInactiveReason:
			"no enabled account inherits this top-level Mattermost botToken.",
...
function collectMatrixAssignments(params: {
	...
	const envAccessTokenConfigured =
		normalizeSecretStringValue(params.context.env.MATRIX_ACCESS_TOKEN).length >
		0;
	...
	for (const { accountId, account, enabled } of surface.accounts) {
		...
		collectSecretInputAssignment({
			...
			inactiveReason:
				"Matrix account is disabled or this account has an accessToken configured.",
			apply: (value) => {
...
function collectZaloAssignments(params: {
	...
	collectConditionalChannelFieldAssignments({
		...
		topInactiveReason:
			"no enabled Zalo surface inherits this top-level botToken.",
...
function collectFeishuAssignments(params: {
	...
	const baseConnectionMode =
		normalizeSecretStringValue(feishu.connectionMode) === "webhook"
			? "webhook"
			: "websocket";
	const resolveAccountMode = (account: Record<string, unknown>) =>
	...
	collectConditionalChannelFieldAssignments({
		...
		accountActive: ({ account, enabled }) =>
			enabled && resolveAccountMode(account) === "webhook",
		topInactiveReason:
			"no enabled Feishu webhook-mode surface inherits this top-level encryptKey.",
		accountInactiveReason:
			"Feishu account is disabled or not running in webhook mode.",
	});
	collectConditionalChannelFieldAssignments({
		channelKey: "feishu",
...
function collectNextcloudTalkAssignments(params: {
	...
	collectConditionalChannelFieldAssignments({
		...
		accountActive: ({ enabled }) => enabled,
		topInactiveReason:
			"no enabled Nextcloud Talk surface inherits this top-level botSecret.",
	...
	collectConditionalChannelFieldAssignments({
		...
		accountActive: ({ enabled }) => enabled,
		topInactiveReason:
			"no enabled Nextcloud Talk surface inherits this top-level apiPassword.",
		accountInactiveReason: "Nextcloud Talk account is disabled.",
	});
}
...
function collectGoogleChatAssignments(params: {
	...
	collectGoogleChatAccountAssignment({
		...
		inactiveReason:
			"no enabled account inherits this top-level Google Chat serviceAccount.",
	});
	if (!surface.hasExplicitAccounts) {
		return;
	}
