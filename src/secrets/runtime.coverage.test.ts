...
type SecretRegistryEntry = ReturnType<
	typeof listSecretTargetRegistryEntries
>[number];
...
function buildConfigForOpenClawTarget(
	...
	if (
		entry.id === "gateway.remote.token" ||
		entry.id === "gateway.remote.password"
	) {
	...
	if (entry.id === "channels.slack.accounts.*.signingSecret") {
		setPathCreateStrict(
			config,
			["channels", "slack", "accounts", "sample", "mode"],
			"http",
		);
	}
...
function buildAuthStoreForTarget(
	entry: SecretRegistryEntry,
	envId: string,
): AuthProfileStore {
...
describe("secrets runtime target coverage", () => {
	...
	beforeEach(async () => {
		vi.resetModules();
		({ clearSecretsRuntimeSnapshot, prepareSecretsRuntimeSnapshot } =
	...
	it("handles every openclaw.json registry target when configured as active", async () => {
		...
		for (const [index, entry] of entries.entries()) {
			...
			const resolved = getPath(
				snapshot.config,
				toConcretePathSegments(entry.pathPattern),
			);
