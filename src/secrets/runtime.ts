...
function cloneRefreshContext(
	context: SecretsRuntimeRefreshContext,
): SecretsRuntimeRefreshContext {
	return {
		env: { ...context.env },
		explicitAgentDirs: context.explicitAgentDirs
			? [...context.explicitAgentDirs]
			: null,
...
function resolveLoadablePluginOrigins(params: {
	...
	return new Map(
		manifestRegistry.plugins.map((record) => [record.id, record.origin]),
	);
}
...
function mergeSecretsRuntimeEnv(
	...
	const merged = { ...(env ?? process.env) } as Record<
		string,
		string | undefined
	>;
...
export async function prepareSecretsRuntimeSnapshot(params: {
	...
	const candidateDirs = params.agentDirs?.length
		? [
				...new Set(
					params.agentDirs.map((entry) => resolveUserPath(entry, runtimeEnv)),
				),
			]
...
export function getActiveSecretsRuntimeSnapshot(): PreparedSecretsRuntimeSnapshot | null {
	...
	if (activeRefreshContext) {
		preparedSnapshotRefreshContext.set(
			snapshot,
			cloneRefreshContext(activeRefreshContext),
		);
	}
	return snapshot;
}
