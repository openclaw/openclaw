...
function createTestProvider(params: {
	...
	return {
		...
		getConfiguredCredentialValue: (config) =>
			(
				config?.plugins?.entries?.[params.pluginId]?.config as {
					webSearch?: { apiKey?: unknown };
				}
			)?.webSearch?.apiKey,
		setConfiguredCredentialValue: (configTarget, value) => {
			const plugins = (configTarget.plugins ??= {}) as {
				entries?: Record<string, unknown>;
			};
			const entries = (plugins.entries ??= {});
			const entry = (entries[params.pluginId] ??= {}) as {
...
function loadAuthStoreWithProfiles(
...
describe("secrets runtime snapshot", () => {
	...
	it("resolves env refs for config and auth profiles", async () => {
			...
			gateway: {
				...
				remote: {
					...
					token: {
						source: "env",
						provider: "default",
						id: "REMOTE_GATEWAY_TOKEN",
					},
					password: {
						source: "env",
						provider: "default",
						id: "REMOTE_GATEWAY_PASSWORD",
					},
				},
			},
			channels: {
				telegram: {
					...
					webhookSecret: {
						source: "env",
						provider: "default",
						id: "TELEGRAM_WEBHOOK_SECRET_REF",
					},
					accounts: {
						work: {
							botToken: {
		...
		expect(snapshot.config.models?.providers?.openai?.apiKey).toBe(
			"sk-env-openai",
		);
		expect(
			snapshot.config.models?.providers?.openai?.headers?.Authorization,
		).toBe("Bearer sk-env-header");
		expect(snapshot.config.skills?.entries?.["review-pr"]?.apiKey).toBe(
			"sk-skill-ref",
		);
		expect(snapshot.config.agents?.defaults?.memorySearch?.remote?.apiKey).toBe(
			"mem-ref-key",
		);
		...
		expect(snapshot.config.gateway?.remote?.password).toBe(
			"remote-password-ref",
		);
		...
		expect(snapshot.config.channels?.telegram?.webhookSecret).toBe(
			"telegram-webhook-ref",
		);
		expect(snapshot.config.channels?.telegram?.accounts?.work?.botToken).toBe(
			"telegram-work-ref",
		);
		expect(snapshot.config.channels?.slack?.signingSecret).toBe(
			"slack-signing-ref",
		);
		expect(snapshot.config.channels?.slack?.accounts?.work?.botToken).toBe(
			"slack-work-bot-ref",
		);
		...
		expect(
			snapshot.authStores[0]?.store.profiles["openai:default"],
		).toMatchObject({
			type: "api_key",
			key: "sk-env-openai",
		});
		expect(
			snapshot.authStores[0]?.store.profiles["github-copilot:default"],
		).toMatchObject({
			type: "token",
			token: "ghp-env-token",
		});
		expect(
			snapshot.authStores[0]?.store.profiles["openai:inline"],
		).toMatchObject({
			type: "api_key",
			key: "sk-env-openai",
		});
		...
		expect(
			(
				snapshot.authStores[0].store.profiles["openai:inline"] as Record<
					string,
					unknown
				>
			).keyRef,
	...
	it("resolves top-level Matrix accessToken refs even when named accounts exist", async () => {
		...
		expect(snapshot.config.channels?.matrix?.accessToken).toBe(
			"default-matrix-token",
		);
	});
	...
	])("ignores top-level Matrix password refs shadowed by $name", async ({
		config,
		env,
	}) => {
		const snapshot = await prepareSecretsRuntimeSnapshot({
	...
	it.each([
	...
	])("ignores default-account Matrix password refs shadowed by $name", async ({
		config,
		env,
	}) => {
		const snapshot = await prepareSecretsRuntimeSnapshot({
		...
		expect(
			snapshot.config.channels?.matrix?.accounts?.default?.password,
		).toEqual({
			source: "env",
			provider: "default",
			id: "MATRIX_DEFAULT_PASSWORD",
	...
	it("treats sandbox ssh secret refs as inactive when ssh backend is not selected", async () => {
		...
		expect(
			snapshot.config.agents?.defaults?.sandbox?.ssh?.identityData,
		).toEqual({
			source: "env",
			provider: "default",
			id: "SSH_IDENTITY_DATA",
	...
	it("normalizes inline SecretRef object on token to tokenRef", async () => {
		...
		const profile = snapshot.authStores[0]?.store.profiles[
			"custom:inline-token"
	...
	it("resolves selected web search provider ref even when provider config is disabled", async () => {
		...
		expect(snapshot.config.tools?.web?.search?.gemini?.apiKey).toBe(
	...
	it("resolves file refs via configured file provider", async () => {
		...
		const root = await fs.mkdtemp(
			path.join(os.tmpdir(), "openclaw-secrets-file-provider-"),
		);
	...
	it("fails when file provider payload is not a JSON object", async () => {
		...
		const root = await fs.mkdtemp(
			path.join(os.tmpdir(), "openclaw-secrets-file-provider-bad-"),
		);
		const secretsPath = path.join(root, "secrets.json");
		try {
			await fs.writeFile(
				...
				web: {
					search: {
						...
						apiKey: {
							source: "env",
							provider: "default",
							id: "DISABLED_WEB_SEARCH_API_KEY",
						},
						gemini: {
							apiKey: {
								source: "env",
	...
	it("treats gateway.auth.password ref as active when mode is unset and no token is configured", async () => {
		...
		expect(snapshot.config.gateway?.auth?.password).toBe(
			"resolved-gateway-password",
		);
		expect(snapshot.warnings.map((warning) => warning.path)).not.toContain(
			"gateway.auth.password",
		);
	...
	it.each([
		"none",
		"trusted-proxy",
	] as const)("treats gateway.remote refs as inactive in local mode when auth mode is %s", async (mode) => {
		const snapshot = await prepareSecretsRuntimeSnapshot({
			config: asConfig({
				gateway: {
					mode: "local",
	...
	it("treats gateway.remote.token ref as active in local mode when no local credentials are configured", async () => {
		...
		expect(snapshot.config.gateway?.remote?.token).toBe(
			"resolved-remote-token",
		);
		expect(snapshot.warnings.map((warning) => warning.path)).not.toContain(
			"gateway.remote.token",
		);
	...
	it("treats top-level Zalo botToken refs as active even when tokenFile is configured", async () => {
		...
		expect(snapshot.config.channels?.zalo?.botToken).toBe(
			"resolved-zalo-token",
		);
		expect(snapshot.warnings.map((warning) => warning.path)).not.toContain(
	...
	it("treats account-level Nextcloud Talk botSecret and apiPassword refs as active when file paths are configured", async () => {
					...
					"nextcloud-talk": {
						accounts: {
							work: {
								...
								apiPasswordFile:
									"/tmp/missing-nextcloud-work-api-password-file",
							},
						},
					},
				},
		...
		expect(
			snapshot.config.channels?.["nextcloud-talk"]?.accounts?.work?.botSecret,
		).toBe("resolved-nextcloud-work-bot-secret");
		expect(
			snapshot.config.channels?.["nextcloud-talk"]?.accounts?.work?.apiPassword,
		).toBe("resolved-nextcloud-work-api-password");
		expect(snapshot.warnings.map((warning) => warning.path)).not.toContain(
	...
	it("treats gateway.remote refs as active when tailscale serve is enabled", async () => {
		...
		expect(snapshot.config.gateway?.remote?.token).toBe(
			"tailscale-remote-token",
		);
		expect(snapshot.config.gateway?.remote?.password).toBe(
			"tailscale-remote-password",
		);
		expect(snapshot.warnings.map((warning) => warning.path)).not.toContain(
			"gateway.remote.token",
		);
	...
	it("treats defaults memorySearch ref as inactive when all enabled agents disable memorySearch", async () => {
		...
		expect(
			snapshot.config.agents?.defaults?.memorySearch?.remote?.apiKey,
		).toEqual({
			source: "env",
			provider: "default",
			id: "DEFAULT_MEMORY_REMOTE_API_KEY",
	...
	it("fails when enabled channel surfaces contain unresolved refs", async () => {
		await expect(
			...
			'Environment variable "MISSING_ENABLED_TELEGRAM_TOKEN" is missing or empty.',
		);
	});
	...
	it("treats Telegram account botToken refs as inactive when account tokenFile is configured", async () => {
		...
		expect(
			snapshot.config.channels?.telegram?.accounts?.work?.botToken,
		).toEqual({
			source: "env",
			provider: "default",
			id: "MISSING_TELEGRAM_WORK_BOT_TOKEN",
	...
	it("treats top-level Telegram botToken refs as active when account botToken is blank", async () => {
		...
		expect(snapshot.config.channels?.telegram?.botToken).toBe(
			"telegram-base-token",
		);
		expect(snapshot.config.channels?.telegram?.accounts?.work?.botToken).toBe(
			"",
	...
	it("treats IRC account nickserv password refs as inactive when nickserv is disabled", async () => {
		...
		expect(
			snapshot.config.channels?.irc?.accounts?.work?.nickserv?.password,
		).toEqual({
			source: "env",
			provider: "default",
			id: "MISSING_IRC_WORK_NICKSERV_PASSWORD",
	...
	it("treats top-level Google Chat serviceAccount as inactive when enabled accounts use serviceAccountRef", async () => {
		...
		expect(
			snapshot.config.channels?.googlechat?.accounts?.work?.serviceAccount,
		).toBe("work-service-account-json");
		expect(snapshot.warnings.map((warning) => warning.path)).toContain(
	...
	it("fails when non-default Discord account inherits an unresolved top-level token ref", async () => {
		await expect(
			...
			'Environment variable "MISSING_DISCORD_BASE_TOKEN" is missing or empty.',
		);
	});
	...
	it("treats Discord voice TTS refs as inactive when voice is disabled", async () => {
		...
		expect(
			snapshot.config.channels?.discord?.voice?.tts?.providers?.openai?.apiKey,
		).toEqual({
			source: "env",
			provider: "default",
			id: "MISSING_DISCORD_VOICE_TTS_OPENAI",
		...
		expect(
			snapshot.config.channels?.discord?.accounts?.work?.voice?.tts?.providers
				?.openai?.apiKey,
		).toEqual({
			source: "env",
			provider: "default",
	...
	it("handles Discord nested inheritance for enabled and disabled accounts", async () => {
		...
		expect(
			snapshot.config.channels?.discord?.voice?.tts?.providers?.openai?.apiKey,
		).toBe("base-tts-openai");
		expect(snapshot.config.channels?.discord?.pluralkit?.token).toBe(
			"base-pk-token",
		);
		expect(
			snapshot.config.channels?.discord?.accounts?.enabledOverride?.voice?.tts
				?.providers?.openai?.apiKey,
		).toBe("enabled-override-tts-openai");
		expect(
			snapshot.config.channels?.discord?.accounts?.disabledOverride?.voice?.tts
				?.providers?.openai?.apiKey,
		).toEqual({
			source: "env",
			provider: "default",
		...
		expect(
			snapshot.config.channels?.discord?.accounts?.disabledOverride?.pluralkit
				?.token,
		).toEqual({
			source: "env",
			provider: "default",
	...
	it("does not write inherited auth stores during runtime secret activation", async () => {
		const root = await fs.mkdtemp(
			path.join(os.tmpdir(), "openclaw-secrets-runtime-"),
		);
		const stateDir = path.join(root, ".openclaw");
		const mainAgentDir = path.join(stateDir, "agents", "main", "agent");
		const workerStorePath = path.join(
			stateDir,
			"agents",
			"worker",
			"agent",
		...
		try {
			...
			await expect(fs.access(workerStorePath)).rejects.toMatchObject({
				code: "ENOENT",
			});
		} finally {
