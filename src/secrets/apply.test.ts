...
function buildFixturePaths(rootDir: string) {
  const stateDir = path.join(rootDir, ".mullusi");
  return {
    rootDir,
    stateDir,
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      MULLUSI_STATE_DIR: paths.stateDir,
      MULLUSI_CONFIG_PATH: paths.configPath,
