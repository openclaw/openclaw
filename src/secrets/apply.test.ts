...
describe("secrets apply", () => {
	...
	it("skips exec SecretRef checks during dry-run unless explicitly allowed", async () => {
		...
		const dryRunSkipped = await runSecretsApply({
			plan,
			env: fixture.env,
			write: false,
		});
		...
		await expect(fs.stat(execLogPath)).rejects.toMatchObject({
			code: "ENOENT",
		});
	...
	it("rejects write mode for exec plans unless allowExec is set", async () => {
		...
		await expect(
			runSecretsApply({ plan, env: fixture.env, write: true }),
		).rejects.toThrow(
	...
	it("applies model provider header targets", async () => {
		...
		expect(
			nextConfig.models?.providers?.openai?.headers?.["x-api-key"],
