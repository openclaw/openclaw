...
const COMPILED_SECRET_TARGET_REGISTRY = SECRET_TARGET_REGISTRY.map(
	compileTargetRegistryEntry,
);
const OPENCLAW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
	(entry) => entry.configFile === "openclaw.json",
);
const AUTH_PROFILES_COMPILED_SECRET_TARGETS =
	COMPILED_SECRET_TARGET_REGISTRY.filter(
...
const KNOWN_TARGET_IDS = new Set(
	COMPILED_SECRET_TARGET_REGISTRY.map((entry) => entry.id),
);

function buildConfigTargetIdIndex(): Map<
	string,
...
function buildAuthProfileTargetIdIndex(): Map<
	string,
	CompiledTargetRegistryEntry[]
> {
	const byId = new Map<string, CompiledTargetRegistryEntry[]>();
...
function normalizeAllowedTargetIds(
...
function discoverSecretTargetsFromEntries(
	...
	for (const entry of discoveryEntries) {
		...
		for (const match of expanded) {
			const resolved = toResolvedPlanTarget(
				entry,
				match.segments,
				match.captures,
			);
...
export function listSecretTargetRegistryEntries(): SecretTargetRegistryEntry[] {
	return COMPILED_SECRET_TARGET_REGISTRY.map((entry) => ({
		...
		...(entry.targetTypeAliases
			? { targetTypeAliases: [...entry.targetTypeAliases] }
			: {}),
		...
		...(entry.authProfileType
			? { authProfileType: entry.authProfileType }
			: {}),
...
export function resolvePlanTargetAgainstRegistry(candidate: {
	...
	for (const entry of entries) {
		...
		if (candidate.providerId && candidate.providerId.trim().length > 0) {
			if (
				!resolved.providerId ||
				resolved.providerId !== candidate.providerId
			) {
				continue;
			}
		}
...
export function listAuthProfileSecretTargetEntries(): SecretTargetRegistryEntry[] {
	return COMPILED_SECRET_TARGET_REGISTRY.filter(
		(entry) =>
			entry.configFile === "auth-profiles.json" && entry.includeInAudit,
	);
}
