...
function countDynamicPatternTokens(tokens: PathPatternToken[]): number {
	return tokens.filter(
		(token) => token.kind === "wildcard" || token.kind === "array",
	).length;
}
...
export function compileTargetRegistryEntry(
	...
	const refPathTokens = entry.refPathPattern
		? parsePathPattern(entry.refPathPattern)
		: undefined;
	const refPathDynamicTokenCount = refPathTokens
		? countDynamicPatternTokens(refPathTokens)
		: 0;
	...
	if (requiresSiblingRefPath && !refPathTokens) {
		throw new Error(
			`Missing refPathPattern for sibling_ref target: ${entry.id}`,
		);
	}
	if (refPathTokens && refPathDynamicTokenCount !== pathDynamicTokenCount) {
		throw new Error(
			`Mismatched wildcard shape for target ref path: ${entry.id}`,
		);
	}
	return {
		...entry,
