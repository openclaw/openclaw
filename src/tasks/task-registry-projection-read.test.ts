...
import { configureTaskRegistryRuntime, onTaskRegistryChange } from "./task-registry.store.js";
    ...
    await withReadState(async () => {
      ...
      try {
        ...
        await withTestTimeout(
          Promise.race([
            publicationCaptured.promise,
            mutation.then(() => {
              throw new Error("Mutation settled before publication capture", {
                cause: mutationFailure,
              });
            }),
          ]),
          5_000,
          "Newer publication captured",
        );
