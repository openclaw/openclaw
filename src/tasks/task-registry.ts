...
let deliveryRuntimePromise: Promise<
...
function cloneTaskDeliveryState(state: TaskDeliveryState): TaskDeliveryState {
	return {
		...
		...(state.requesterOrigin
			? { requesterOrigin: { ...state.requesterOrigin } }
			: {}),
	};
}
...
function emitTaskRegistryHookEvent(
	createEvent: () => TaskRegistryHookEvent,
): void {
...
function ensureNotifyPolicy(params: {
	...
	const deliveryStatus =
		params.deliveryStatus ?? ensureDeliveryStatus(params.requesterSessionKey);
	...
	const exact = runId
		? getTasksByRunId(runId).find(
				(task) =>
					...
					normalizeComparableText(task.label) ===
						normalizeComparableText(params.label) &&
					normalizeComparableText(task.task) ===
						normalizeComparableText(params.task),
			)
		: undefined;
	if (exact) {
		return exact;
...
function taskTerminalDeliveryIdempotencyKey(task: TaskRecord): string {
	const outcome =
		task.status === "succeeded"
			? (task.terminalOutcome ?? "default")
			: "default";
	return `task-terminal:${task.taskId}:${task.status}:${outcome}`;
}

function flowTerminalDeliveryIdempotencyKey(
	flowId: string,
	task: TaskRecord,
): string {
	const outcome =
...
function resolveTaskTerminalIdempotencyKey(
	task: TaskRecord,
	owner: TaskDeliveryOwner,
): string {
	return owner.flowId
...
function resolveTaskDeliveryOwner(task: TaskRecord): TaskDeliveryOwner {
	const flow = task.parentFlowId?.trim()
		? getFlowById(task.parentFlowId)
		: undefined;
	return {
		sessionKey:
			flow?.ownerSessionKey?.trim() || task.requesterSessionKey.trim(),
...
export function markTaskLostById(params: {
	...
	return updateTask(params.taskId, {
		...
		lastEventAt: params.lastEventAt ?? params.endedAt,
		...(params.error !== undefined ? { error: params.error } : {}),
		...(params.cleanupAfter !== undefined
			? { cleanupAfter: params.cleanupAfter }
			: {}),
	});
}

function updateTasksByRunId(
	runId: string,
	patch: Partial<TaskRecord>,
): TaskRecord[] {
	const ids = taskIdsByRunId.get(runId.trim());
			...
			if (evt.stream === "lifecycle") {
				...
				const startedAt =
					typeof evt.data?.startedAt === "number"
						...
						: current.startedAt;
				const endedAt =
					typeof evt.data?.endedAt === "number" ? evt.data.endedAt : undefined;
				if (startedAt) {
				...
				patch.error =
					typeof evt.data?.error === "string" ? evt.data.error : current.error;
			}
...
export function createTaskRecord(params: {
	...
	if (
		...
		record.cleanupAfter =
			(record.endedAt ?? record.lastEventAt ?? record.createdAt) +
	...
	for (const taskId of ids) {
			...
			(nextStatus === "failed"
				...
				: nextStatus === "succeeded"
					? normalizeTaskSummary(
							params.terminalSummary ?? current.terminalSummary,
						)
					: undefined);
		...
		const nextEvent = shouldAppendEvent
			? appendTaskEvent({
					...
					kind:
						params.status &&
						normalizeTaskStatus(params.status) !== current.status
							? normalizeTaskStatus(params.status)
							: "progress",
...
export function getTaskRegistrySnapshot(): TaskRegistrySnapshot {
	return {
		...
		deliveryStates: [...taskDeliveryStates.values()].map((state) =>
			cloneTaskDeliveryState(state),
		),
	};
}
...
export function findLatestTaskForSessionKey(
	sessionKey: string,
): TaskRecord | undefined {
...
export function resolveTaskForLookupToken(
	token: string,
): TaskRecord | undefined {
