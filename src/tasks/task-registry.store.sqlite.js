...
function hasTaskRunsColumn(db, columnName) {
    const rows = db.prepare(`PRAGMA table_info(task_runs)`).all();
...
export function saveTaskRegistryStateToSqlite(snapshot) {
...
export function upsertTaskRegistryRecordToSqlite(task) {
...
export function deleteTaskRegistryRecordFromSqlite(taskId) {
    const store = openTaskRegistryDatabase();
    store.statements.deleteRow.run(taskId);
    store.statements.deleteDeliveryState.run(taskId);
}
export function deleteTaskAndDeliveryStateFromSqlite(taskId) {
    withWriteTransaction((statements) => {
        statements.deleteRow.run(taskId);
        statements.deleteDeliveryState.run(taskId);
    });
}
export function upsertTaskDeliveryStateToSqlite(state) {
    const store = openTaskRegistryDatabase();
    store.statements.replaceDeliveryState.run(bindTaskDeliveryState(state));
}
export function deleteTaskDeliveryStateFromSqlite(taskId) {
    const store = openTaskRegistryDatabase();
    store.statements.deleteDeliveryState.run(taskId);
}
