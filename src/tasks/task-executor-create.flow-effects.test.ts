...
it.each(["metadata", "removal", "replacement", "adoption"] as const)(
...
it.each(["adopted", "unknown outcome", "acknowledged"] as const)(
  ...
  async (outcome) => {
    ...
    vi.spyOn(f.store, "runInitialMutationAsync").mockImplementation(
      async (context, command, assertCurrent) => {
        if (
          ...
          if (outcome === "acknowledged") {
            const receipt = await original(context, command, assertCurrent);
            releaseAdoption = bindTaskRunOwner(second.task, async () => err("Synthetic successor"));
            return receipt;
          }
        }
        return original(context, command, assertCurrent);
      },
    );
