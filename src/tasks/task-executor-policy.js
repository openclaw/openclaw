...
function resolveTaskDisplayTitle(task) {
    return formatTaskStatusTitleText(task.label?.trim() ||
        (task.runtime === "acp"
            ? "ACP background task"
...
export function formatTaskTerminalMessage(task) {
...
export function formatTaskBlockedFollowupMessage(task) {
    if (task.status !== "succeeded" || task.terminalOutcome !== "blocked") {
        return null;
    }
    const title = resolveTaskDisplayTitle(task);
    const runLabel = resolveTaskRunLabel(task);
    const summary = sanitizeTaskStatusText(task.terminalSummary, { errorContext: true }) ||
        "Task is blocked and needs follow-up.";
    return `Task needs follow-up: ${title}${runLabel}. ${summary}`;
}
export function formatTaskStateChangeMessage(task, event) {
    const title = resolveTaskDisplayTitle(task);
...
export function shouldAutoDeliverTaskTerminalUpdate(task) {
...
export function shouldAutoDeliverTaskStateChange(task) {
    return (task.notifyPolicy === "state_changes" &&
