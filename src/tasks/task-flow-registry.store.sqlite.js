...
import { resolveTaskFlowRegistryDir, resolveTaskFlowRegistrySqlitePath, } from "./task-flow-registry.paths.js";
...
function hasFlowRunsColumn(db, columnName) {
    const rows = db.prepare(`PRAGMA table_info(flow_runs)`).all();
...
export function saveTaskFlowRegistryStateToSqlite(snapshot) {
...
export function upsertTaskFlowRegistryRecordToSqlite(flow) {
...
export function deleteTaskFlowRegistryRecordFromSqlite(flowId) {
    const store = openFlowRegistryDatabase();
