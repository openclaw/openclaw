...
vi.mock("../agents/subagent-control.js", () => ({
	killSubagentRunAdmin: (params: unknown) =>
		hoisted.killSubagentRunAdminMock(params),
}));
...
describe("task-registry", () => {
	...
	it("records delivery failure and queues a session fallback when direct delivery misses", async () => {
		await withTempDir({ prefix: "openclaw-task-registry-" }, async (root) => {
			process.env.OPENCLAW_STATE_DIR = root;
			resetTaskRegistryForTests();
			hoisted.sendMessageMock.mockRejectedValueOnce(
		...
		await withTempDir({ prefix: "openclaw-task-registry-" }, async (root) => {
			...
			await waitForAssertion(() =>
				expect(hoisted.sendMessageMock).toHaveBeenCalledWith(
					expect.objectContaining({
						content:
							"Background task done: ACP background task (run run-deta).",
					}),
				),
			);
		});
	...
	it("keeps distinct task records when different producers share a runId", async () => {
		await withTempDir({ prefix: "openclaw-task-registry-" }, async (root) => {
			...
			expect(
				listTaskRecords().filter((task) => task.runId === "run-shared"),
			).toHaveLength(2);
		...
		await withTempDir({ prefix: "openclaw-task-registry-" }, async (root) => {
			...
			expect(
				listTaskRecords().filter(
					(task) => task.runId === "run-shared-delivery",
				),
			).toHaveLength(1);
	...
	it("collapses ACP run-owned task creation onto the existing spawned task", async () => {
		await withTempDir({ prefix: "openclaw-task-registry-" }, async (root) => {
			...
			expect(
				listTaskRecords().filter((task) => task.runId === "run-collapse"),
			).toHaveLength(1);
	...
	it("indexes tasks by session key for latest and list lookups", async () => {
		await withTempDir({ prefix: "openclaw-task-registry-" }, async (root) => {
			...
			expect(findLatestTaskForSessionKey("agent:main:main")?.taskId).toBe(
				latest.taskId,
			);
			expect(
				listTasksForSessionKey("agent:main:main").map((task) => task.taskId),
			).toEqual([latest.taskId, older.taskId]);
			expect(
				findLatestTaskForSessionKey("agent:main:subagent:child-1")?.taskId,
			).toBe(older.taskId);
		});
	});
	...
	it("previews and repairs missing cleanup timestamps during maintenance", async () => {
		await withTempDir({ prefix: "openclaw-task-registry-" }, async (root) => {
			...
			expect(getTaskById("task-missing-cleanup")?.cleanupAfter).toBeGreaterThan(
				now,
			);
		});
	});
				...
				expect(hoisted.sendMessageMock).toHaveBeenCalledWith(
					expect.objectContaining({
						...
						idempotencyKey: expect.stringContaining(
							`flow-event:${flow.flowId}:${task.taskId}:`,
						),
						mirror: expect.objectContaining({
			...
			await waitForAssertion(() =>
				expect(hoisted.sendMessageMock).toHaveBeenCalledWith(
					expect.objectContaining({
						...
						to: "telegram:123",
						content:
							"Background task cancelled: ACP background task (run run-canc).",
					}),
				),
			);
		});
