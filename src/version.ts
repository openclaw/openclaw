...
declare const __ACTIVI_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "activi";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["ACTIVI_VERSION"],
      env["ACTIVI_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Activi version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __ACTIVI_VERSION__ === "string" && __ACTIVI_VERSION__) ||
  process.env.ACTIVI_BUNDLED_VERSION ||
