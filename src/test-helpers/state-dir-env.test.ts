...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    activi: process.env.ACTIVI_STATE_DIR,
    legacy: process.env.ACTIVI_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.ACTIVI_STATE_DIR).toBe(snapshot.activi);
  expect(process.env.ACTIVI_STATE_DIR).toBe(snapshot.legacy);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips ACTIVI_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/activi-state-dir-test");
    expect(process.env.ACTIVI_STATE_DIR).toBe("/tmp/activi-state-dir-test");
    expect(process.env.ACTIVI_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("activi-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.ACTIVI_STATE_DIR).toBe(stateDir);
      expect(process.env.ACTIVI_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("activi-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
  ...
  it("withStateDirEnv restores both env vars when legacy var was previously set", async () => {
    ...
    process.env.ACTIVI_STATE_DIR = "/tmp/original-activi";
    process.env.ACTIVI_STATE_DIR = "/tmp/original-legacy";
    const prev = snapshotCurrentStateDirVars();
