...
export class GatewayChatClient {
	...
	constructor(connection: ResolvedGatewayConnection) {
		...
		this.client = new GatewayClient({
			...
			deviceIdentity: connection.allowInsecureLocalOperatorUi
				? null
				: undefined,
...
export async function resolveGatewayConnection(
	...
	if (isRemoteMode) {
		...
		const password =
			explicitAuth.password ?? envPassword ?? remotePassword.value;
		if (!token && !password) {
	...
	if (gatewayAuthMode === "password") {
		...
		const password =
			explicitAuth.password ?? envPassword ?? localPassword.value;
		if (!password) {
			throwGatewayAuthResolutionError(
	...
	const shouldUsePassword =
		Boolean(passwordCandidate) ||
		(hasConfiguredPassword && !hasConfiguredToken);
	...
	if (shouldUsePassword) {
		...
		const password =
			explicitAuth.password ?? localPassword.value ?? envPassword;
		if (!password) {
			throwGatewayAuthResolutionError(
				localPassword.unresolvedRefReason ?? "Missing gateway auth password.",
			);
