...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set MULLUSI_GATEWAY_TOKEN/MULLUSI_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.MULLUSI_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.MULLUSI_GATEWAY_PASSWORD);
