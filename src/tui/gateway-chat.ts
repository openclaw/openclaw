...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set NEXUS_GATEWAY_TOKEN/NEXUS_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.NEXUS_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.NEXUS_GATEWAY_PASSWORD);
