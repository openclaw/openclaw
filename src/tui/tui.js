...
export function resolveLocalAuthCliInvocation(params) {
    const hasDistEntry = params?.hasDistEntry ??
...
export function stopTuiSafely(stop) {
    try {
        stop();
    }
    catch (error) {
...
export async function drainAndStopTuiSafely(tui) {
...
export async function runTui(opts) {
    ...
    const runAuthFlow = isLocalMode
        ? async (params) => await withTuiSuspended(async () => await new Promise((resolve, reject) => {
            const provider = params.provider?.trim() || undefined;
            // Codex owns its auth store; delegate when the CLI is available.
            const codexBin = provider === OPENAI_CODEX_PROVIDER ||
    ...
    client.onConnected = () => {
        ...
        void (async () => {
            ...
            updateHeader();
            await loadHistory();
            setConnectionStatus(isLocalMode ? "local ready" : reconnected ? "gateway reconnected" : "gateway connected", 4000);
            tui.requestRender();
