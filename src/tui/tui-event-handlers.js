...
export function createEventHandlers(context) {
    ...
    const streamingWatchdogMs = typeof context.streamingWatchdogMs === "number" &&
    ...
    const armStreamingWatchdog = (runId) => {
        ...
        streamingWatchdogTimer = setTimeout(() => {
            ...
            chatLog.addSystem(`streaming watchdog: no stream updates for ${Math.round(streamingWatchdogMs / 1000)}s; resetting status. The backend may have dropped this run silently — send a new message to resync.`);
            tui.requestRender();
        }, streamingWatchdogMs);
