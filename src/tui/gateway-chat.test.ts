...
describe("resolveGatewayConnection", () => {
  ...
  it("uses BOT_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ BOT_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers BOT_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ BOT_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
