...
const { GatewayChatClient, resolveGatewayConnection } = await import(
	"./gateway-chat.js"
);
...
async function withModeExecProviderFixture(
	...
	const tempDir = await fs.mkdtemp(
		path.join(os.tmpdir(), `openclaw-tui-mode-${label}-`),
	);
...
describe("resolveGatewayConnection", () => {
	...
	])("uses explicit $label when url override is set", async ({
		auth,
		expected,
	}) => {
		loadConfig.mockReturnValue({ gateway: { mode: "local" } });
	...
	it.runIf(process.platform !== "win32")(
		...
		async () => {
			const tempDir = await fs.mkdtemp(
				path.join(os.tmpdir(), "openclaw-tui-file-secret-"),
			);
...
describe("GatewayChatClient", () => {
	it("identifies the TUI as a tui client and skips device identity on insecure local ui paths", () => {
		...
		expect(
			(
				client as unknown as {
					client: { opts: { clientName?: string; mode?: string } };
				}
		...
		expect(
			(client as unknown as { client: { opts: { deviceIdentity?: unknown } } })
				.client.opts.deviceIdentity,
		).toBeUndefined();
	});
});
