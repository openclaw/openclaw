...
function createHarness(params?: {
	...
	const refreshSessionInfo =
		params?.refreshSessionInfo ?? vi.fn().mockResolvedValue(undefined);
	const applySessionInfoFromPatch =
		params?.applySessionInfoFromPatch ?? vi.fn();
...
describe("tui command handlers", () => {
	...
	it("creates unique session for /new and resets shared session for /reset", async () => {
		...
		const setSessionMock = vi
			.fn()
			.mockResolvedValue(undefined) as SetSessionMock;
		...
		expect(setSessionMock).toHaveBeenCalledWith(
			expect.stringMatching(
				/^tui-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/,
			),
		);
	...
	it("sanitizes control sequences in /new and /reset failures", async () => {
		const setSession = vi
			.fn()
			.mockRejectedValue(new Error("\u001b[31mboom\u001b[0m"));
	...
	it("patches the session for valid /activation values", async () => {
		...
		const patchSession = vi
			.fn()
			.mockResolvedValue({ groupActivation: "always" });
		const { handleCommand, addSystem } = createHarness({
			patchSession,
			refreshSessionInfo,
			applySessionInfoFromPatch,
