...
describe("resolveInitialTuiAgentId", () => {
  const cfg: MullusiConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/mullusi" },
        { id: "ops", workspace: "/tmp/mullusi/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/mullusi/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run mullusi devices list");
    expect(state.pairingHint).toContain("mullusi devices list");
  });
