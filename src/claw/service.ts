...
import type {
  ...
  ClawAuditOutcome,
  ClawAuditRole,
  ClawAuditSideEffectClass,
  ClawContinuationPhase,
  ClawControlState,
  ClawDecisionAction,
  ClawInboxItem,
...
type ClawMissionStateRecord = Omit<
  ClawMissionDetail,
  "requiresAttention" | "packet" | "continuationPhase"
> & {
  version: 1;
  missionPlan: ClawMissionPlan;
  runnerSessionId: string | null;
  runnerSessionFile: string | null;
  resumeTargetStatus: Extract<ClawMissionStatus, "running" | "recovering" | "verifying"> | null;
...
const TERMINAL_MISSION_STATUSES = new Set<ClawMissionStatus>(["done", "failed", "cancelled"]);
...
const AWAITING_APPROVAL_STEP = "Awaiting approval for unattended continuation.";
const PRE_APPROVAL_BLOCKERS_STEP =
  "Resolve preflight blockers before unattended continuation is approved.";
const MISSION_LIFECYCLE_NOTE =
...
function resolveQueuedCurrentStep(control: ClawControlState): string {
  ...
  return "Queued for autonomous continuation.";
}
...
type ClawResumeTargetStatus = Extract<ClawMissionStatus, "running" | "recovering" | "verifying">;
type ClawRecoveryTargetStatus = Extract<ClawMissionStatus, "running" | "verifying">;

type ClawAuditEntryInput = Omit<ClawAuditEntry, "id" | "missionId" | "at"> & { at?: string };
...
function parseRunnerDecision(text: string): ClawRunnerDecision {
  ...
  return {
    ...
    blockerSummary: typeof parsed.blockerSummary === "string" ? parsed.blockerSummary.trim() : null,
...
function buildFallbackMissionPlan(params: { title: string; goal: string }): ClawMissionPlan {
...
function parsePlanningDecision(text: string, fallback: ClawMissionPlan): ClawPlanningDecision {
  const parsed = extractJsonObject(text);
  if (!parsed) {
...
type ClawExternalAuthReadiness = {
  ready: boolean;
  summary: string;
  detail: string | null;
};
...
function inferMissionCapabilityNeeds(goal: string): ClawMissionCapabilityNeeds {
  ...
  const requiredAuthProviders: ClawMissionCapabilityNeeds["requiredAuthProviders"] = [];
  const requiredProviderIds = new Set<string>();
  const providers: Array<{ label: string; providerId: string; pattern: RegExp }> = [
    {
      label: "GitHub",
      providerId: "github",
      pattern: /\bgithub\b|\bpull request\b|\bpr\b|\bbranch\b|\bcommit\b|\bpush\b/,
    },
    { label: "AWS", providerId: "aws", pattern: /\baws\b|\bec2\b|\bs3\b|\blambda\b/ },
    { label: "Cloudflare", providerId: "cloudflare", pattern: /\bcloudflare\b|\bworkers\b/ },
    { label: "Vercel", providerId: "vercel", pattern: /\bvercel\b/ },
    { label: "Render", providerId: "render", pattern: /\brender\b/ },
    { label: "Netlify", providerId: "netlify", pattern: /\bnetlify\b/ },
    { label: "Stripe", providerId: "stripe", pattern: /\bstripe\b/ },
    { label: "Supabase", providerId: "supabase", pattern: /\bsupabase\b/ },
  ];
  for (const provider of providers) {
    if (provider.pattern.test(normalizedGoal)) {
      likelyExternalSystems.add(provider.label);
      likelyAuthDomains.add(provider.label);
      if (!requiredProviderIds.has(provider.providerId)) {
        requiredProviderIds.add(provider.providerId);
        requiredAuthProviders.push({
...
function normalizeAuthProviderKey(value: string): string {
...
}): ClawExternalAuthReadiness | null {
  if (params.capabilityNeeds.requiredAuthProviders.length === 0) {
    return null;
  }
  const labels = params.capabilityNeeds.requiredAuthProviders.map((provider) => provider.label);
  try {
    ...
    const summary = buildAuthHealthSummary({
      store,
      cfg: params.cfg,
      providers: params.capabilityNeeds.requiredAuthProviders.map(
        (provider) => provider.providerId,
      ),
    });
    const providerById = new Map(
      summary.providers.map((provider) => [normalizeAuthProviderKey(provider.provider), provider]),
    );
    const proven: string[] = [];
    const missing: string[] = [];
    const detailParts: string[] = [];
    for (const provider of params.capabilityNeeds.requiredAuthProviders) {
      const providerHealth = providerById.get(normalizeAuthProviderKey(provider.providerId));
      const providerStatus = providerHealth?.status ?? "missing";
      if (providerStatus === "ok" || providerStatus === "static" || providerStatus === "expiring") {
        proven.push(provider.label);
        detailParts.push(
          `${provider.label}: ${
            providerStatus === "expiring"
              ? "credentials are available but expiring soon"
        ...
        detailParts.push(
          `${provider.label}: ${providerStatus === "expired" ? "stored credentials are expired" : "credentials are not proven from current auth state"}`,
        );
      }
    }
    return {
      ...
      summary:
        missing.length === 0
          ? `Required external access is already proven for: ${proven.join(", ")}.`
          : `Required external access is not yet proven for: ${missing.join(", ")}.`,
      detail:
        detailParts.length > 0
          ? detailParts.join(" | ")
          : `No provider-backed external access was proven for: ${labels.join(", ")}.`,
    };
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    return {
      ready: false,
      summary: `Required external access is not yet proven for: ${labels.join(", ")}.`,
      detail: `Claw could not inspect stored auth readiness: ${detail}`,
    };
  }
}
...
async function defaultInspectBrowserReadiness(cfg: OpenClawConfig): Promise<ClawBrowserReadiness> {
  ...
  try {
    ...
    if (status.attachOnly) {
      return {
        ready: true,
        summary:
          "Browser control is ready in attach-only mode and can attach to an existing session.",
      };
    }
    const chosenBrowser =
...
async function buildPreflightChecks(params: {
  ...
  const externalAuthReadiness = inspectExternalAuthReadiness({
    cfg: params.cfg,
    capabilityNeeds,
  });
  ...
  const authSummary = capabilityNeeds.manualAuthRequired
    ? "This goal explicitly mentions login, CAPTCHA, MFA, OAuth, or similar manual authentication. Operator setup is required before unattended continuation makes sense."
    : externalAuthReadiness
      ? externalAuthReadiness.summary
      : capabilityNeeds.likelyAuthDomains.length > 0
        ...
        : "No required external authentication was inferred from the goal text.";
  const authDetail = capabilityNeeds.manualAuthRequired
    ? "Manual-auth requirements are treated as true blockers for unattended continuation, even when full tool access is granted globally."
    : (externalAuthReadiness?.detail ?? null);
  ...
  return [
    ...
    },
    {
      ...
      status: "ready",
      summary:
        "Mission state is mirrored through the managed Task Flow registry for restart safety.",
    },
    {
      id: "gateway-runtime",
      category: "runtime",
    ...
    },
    {
      id: "likely-auth",
      category: "auth",
      title: "Required external authentication readiness",
      status: authReady ? (externalAuthReadiness ? "ready" : "info") : "needs_setup",
      summary: authSummary,
      detail: authDetail,
      blocker: !authReady,
    },
...
function buildStartApprovalDecision(nowIso: string): ClawPendingDecision {
  return {
    ...
    kind: "start_approval",
    title: START_APPROVAL_TITLE,
    summary: START_APPROVAL_SUMMARY,
    requestedAt: nowIso,
    status: "pending",
  };
}
...
function buildRecoveryUncertainSummary(state: ClawMissionStateRecord): string {
  ...
  if (state.verifyCycleCount > 0) {
    reasons.push(`verifier cycles completed: ${state.verifyCycleCount}`);
  }
  if (state.lastFailureSummary) {
    reasons.push(`last mission error: ${state.lastFailureSummary}`);
  }
  if (state.lastVerifierRejectionSignature) {
    reasons.push(`last verifier rejection: ${state.lastVerifierRejectionSignature}`);
  }
  const suffix = reasons.length > 0 ? ` Last checkpoint details: ${reasons.join(" | ")}.` : "";
  return `Mission recovery needs operator confirmation because active mission work may have left partially applied side effects before restart.${suffix}`;
}
...
function renderMissionMarkdown(state: ClawMissionStateRecord): string {
  return [
    ...
    `- Autonomous continuation approved: ${state.approvedAt ? `yes (${state.approvedAt})` : "no"}`,
    `- Runner started: ${state.startedAt ?? "not yet"}`,
  ...
  const nextCheckpoint =
    state.status === "awaiting_setup"
      ...
      : state.status === "awaiting_approval"
        ...
        : state.status === "queued"
          ? "Wait for the runner to claim the mission and continue autonomously."
...
function renderTasksMarkdown(state: ClawMissionStateRecord): string {
  ...
  return [
    ...
    `- [${state.approvedAt ? "x" : " "}] Approve unattended continuation`,
...
function renderStatusMarkdown(state: ClawMissionStateRecord): string {
  return [
    ...
    `- Autonomous Continuation Approved: ${state.approvedAt ? "yes" : "no"}`,
...
function renderDoneCriteriaMarkdown(state: ClawMissionStateRecord): string {
  ...
  return ["# Project Done Criteria", "", ...criteria, "", `Mission: ${state.title}`].join("\n");
}

function resolveContinuationPhase(
  state: Pick<ClawMissionStateRecord, "status" | "resumeTargetStatus">,
): ClawContinuationPhase | null {
  if (state.status === "running" || state.status === "recovering" || state.status === "verifying") {
    return state.status;
  }
  return state.resumeTargetStatus ?? null;
}

function buildMissionPacket(state: ClawMissionStateRecord): ClawMissionPacket {
  return {
    source: state.missionPlan.source,
    summary: state.missionPlan.summary,
    scopeIn: [...state.missionPlan.scopeIn],
    scopeOut: [...state.missionPlan.scopeOut],
    phases: [...state.missionPlan.phases],
    tasks: [...state.missionPlan.tasks],
    doneCriteria: [...state.missionPlan.doneCriteria],
    lifecycleNote: MISSION_LIFECYCLE_NOTE,
  };
}
...
function toSummary(state: ClawMissionStateRecord) {
  return {
    ...
    goal: state.goal,
    status: state.status,
    continuationPhase: resolveContinuationPhase(state),
...
function toDetail(state: ClawMissionStateRecord): ClawMissionDetail {
  return {
    ...
    files: [...state.files],
    packet: buildMissionPacket(state),
...
function assertMissionStateRecord(raw: unknown): ClawMissionStateRecord {
  ...
  return {
    ...
    resumeTargetStatus:
      state.resumeTargetStatus === "running" ||
      state.resumeTargetStatus === "recovering" ||
      state.resumeTargetStatus === "verifying"
        ? state.resumeTargetStatus
        : null,
    recoveryTargetStatus:
      state.recoveryTargetStatus === "running" || state.recoveryTargetStatus === "verifying"
        ? state.recoveryTargetStatus
        : null,
    recentEvidence: Array.isArray(state.recentEvidence)
      ? normalizeEvidence(state.recentEvidence)
      : [],
    ...
    missionPlan:
      state.missionPlan && typeof state.missionPlan === "object"
        ? {
            ...
            scopeOut: normalizeTextList(
              state.missionPlan.scopeOut,
              6,
              fallbackMissionPlan.scopeOut,
            ),
...
export function createClawMissionService(deps: ClawServiceDeps = {}) {
  ...
  async function writeStaticMissionDocs(state: ClawMissionStateRecord): Promise<void> {
    await Promise.all([
      fs.writeFile(
        path.join(state.missionDir, "PROJECT_SCOPE.md"),
        renderScopeMarkdown(state),
        "utf-8",
      ),
      fs.writeFile(
  ...
  async function writeDynamicMissionDocs(state: ClawMissionStateRecord): Promise<void> {
    ...
    await Promise.all([
      fs.writeFile(
        path.join(state.missionDir, "MISSION.md"),
        renderMissionMarkdown(state),
        "utf-8",
      ),
      fs.writeFile(
      ...
      fs.writeFile(
        path.join(state.missionDir, "PROJECT_PLAN.md"),
  ...
  function resolveAuditRole(
    entry: Pick<ClawAuditEntryInput, "actor" | "type" | "role">,
  ): ClawAuditRole {
    if (entry.role) {
      return entry.role;
    }
    ...
    if (entry.type.includes("verifier")) {
      return "verifier";
    }
    if (entry.type.includes("helper") || entry.type === "mission.replanned") {
      return "helper";
    }
    if (entry.type.includes("runner")) {
      return "runner";
    }
    if (entry.type.includes("packet") || entry.type === "mission.preflighting") {
      return "planner";
    }
    return "system";
  }

  function resolveAuditPhase(
    state: Pick<ClawMissionStateRecord, "status"> | null,
    entry: Pick<ClawAuditEntryInput, "phase" | "type">,
  ): ClawMissionStatus | "planning" | null {
    if (entry.phase !== undefined) {
      return entry.phase;
    }
    if (
      entry.type === "mission.created" ||
      entry.type === "mission.preflighting" ||
      entry.type === "mission.packetPlanned" ||
      entry.type === "mission.packetFallback"
    ) {
      return "planning";
    }
  ...
  function resolveAuditSideEffectClass(
    entry: Pick<ClawAuditEntryInput, "type" | "sideEffectClass">,
  ): ClawAuditSideEffectClass | null {
    if (entry.sideEffectClass !== undefined) {
      return entry.sideEffectClass;
    }
    if (entry.type.startsWith("decision.")) {
      return "decision_request";
    }
    if (
      entry.type === "mission.paused" ||
      entry.type === "mission.pauseAll" ||
      entry.type === "mission.stopAllNow" ||
      entry.type === "mission.resumed" ||
      entry.type === "mission.cancelled"
    ) {
      return "control_change";
    }
    if (
      entry.type === "mission.created" ||
      entry.type === "mission.started" ||
      entry.type === "mission.recovering" ||
      entry.type === "mission.recovered" ||
      entry.type === "mission.verifying" ||
      entry.type === "mission.done" ||
      entry.type === "mission.failed" ||
      entry.type === "mission.blocked" ||
      entry.type === "mission.replanned" ||
      entry.type === "mission.runnerCycle" ||
      entry.type === "mission.helperCycle" ||
      entry.type === "mission.verifierCycle" ||
      entry.type === "mission.verifierRejected"
    ) {
  ...
  function resolveAuditOutcome(
    entry: Pick<ClawAuditEntryInput, "type" | "outcome">,
  ): ClawAuditOutcome {
    if (entry.outcome) {
      return entry.outcome;
    }
    if (entry.type === "decision.requested") {
      return "requested";
    }
    if (entry.type === "mission.blocked") {
      return "blocked";
    }
    if (entry.type === "mission.failed" || entry.type === "mission.runnerError") {
      return "failed";
    }
    if (entry.type === "mission.verifierRejected") {
      return "rejected";
    }
    if (entry.type === "mission.resumed" || entry.type === "mission.recovered") {
      return "resumed";
    }
    return "completed";
  }
  ...
  function buildAuditEntry(
    state: Pick<
      ...
      "id" | "status" | "currentStep" | "title" | "flowRevision"
    > | null,
    entry: ClawAuditEntryInput & {
      existingId?: string | null;
      existingMissionId?: string | null;
    },
  ): ClawAuditEntry {
    return {
      id: entry.existingId?.trim() || crypto.randomUUID(),
      missionId: entry.existingMissionId?.trim() || state?.id || "",
      at: entry.at ?? isoNow(now),
      actor: entry.actor,
      role: resolveAuditRole(entry),
      phase: resolveAuditPhase(state, entry),
      type: entry.type,
      actionType: entry.actionType ?? entry.type,
      toolName: entry.toolName ?? null,
      targetSummary: entry.targetSummary ?? state?.currentStep ?? state?.title ?? null,
      sideEffectClass: resolveAuditSideEffectClass(entry),
      intentSummary: entry.intentSummary ?? entry.summary,
      outcome: resolveAuditOutcome(entry),
      errorSummary: entry.errorSummary ?? null,
      artifactRefs: [...(entry.artifactRefs ?? [])],
      checkpointRevision: entry.checkpointRevision ?? state?.flowRevision ?? null,
  ...
  async function recordAudit(
    ...
    const auditEntry = buildAuditEntry(state, entry);
  ...
  async function reconcileMissionStateForControl(
    ...
    if (control.stopAllNowRequestedAt && ACTIVE_MISSION_STATUSES.has(state.status)) {
      const resumeIntent = resolveResumeIntentForWaitingTransition(state);
      moveMissionToWaiting({
        ...
        resumeTargetStatus: resumeIntent.resumeTargetStatus,
        recoveryTargetStatus: resumeIntent.recoveryTargetStatus,
      });
    ...
    if (state.status === "awaiting_setup") {
      if (!blockingSummary) {
        moveMissionToWaiting({
          ...
          currentStep: AWAITING_APPROVAL_STEP,
          waitKind: "approval",
        });
      ...
      if (blockingSummary) {
        moveMissionToWaiting({
          ...
          currentStep: PRE_APPROVAL_BLOCKERS_STEP,
      ...
      if (pausedByGlobalControl && shouldStartMissionImmediately(control)) {
        moveMissionToQueued(state, "Queued after the global control was cleared.", {
          resumeTargetStatus: state.resumeTargetStatus,
  ...
  function resolvePendingDecisionByKind(
    ...
    return state.decisions.find(
      (decision) => decision.kind === kind && decision.status === "pending",
    );
  }
  ...
  function buildFlowStateJson(params: {
    state: ClawMissionStateRecord;
    missionStatus: ClawMissionStatus;
    resumeTargetStatus?: ClawResumeTargetStatus | null;
    recoveryTargetStatus?: ClawRecoveryTargetStatus | null;
  }) {
    return {
      missionId: params.state.id,
      missionStatus: params.missionStatus,
      resumeTargetStatus: params.resumeTargetStatus ?? null,
      recoveryTargetStatus: params.recoveryTargetStatus ?? null,
    };
  }

  function resolveResumeIntentFromBlockedState(state: ClawMissionStateRecord): {
    resumeTargetStatus: ClawResumeTargetStatus | null;
    recoveryTargetStatus: ClawRecoveryTargetStatus | null;
  } {
    if (state.resumeTargetStatus === "recovering") {
      return {
        resumeTargetStatus: "recovering",
        recoveryTargetStatus: state.recoveryTargetStatus ?? "running",
      };
    }
    if (state.resumeTargetStatus === "running" || state.resumeTargetStatus === "verifying") {
      return {
        resumeTargetStatus: state.resumeTargetStatus,
    ...
    const waitKind = getFlowWaitKind(state);
    if (waitKind === "verification_blocker") {
      return {
        resumeTargetStatus: "verifying",
        recoveryTargetStatus: null,
      };
    }
    if (waitKind === "recovery_uncertain") {
      return {
        resumeTargetStatus: "recovering",
        recoveryTargetStatus: state.recoveryTargetStatus ?? "running",
      };
    }
    if (waitKind === "runtime_blocker" || waitKind === "helper_blocker") {
      return {
        resumeTargetStatus: "running",
        recoveryTargetStatus: null,
      };
    }
    return {
  ...
  function resolveResumeIntentForWaitingTransition(state: ClawMissionStateRecord): {
    resumeTargetStatus: ClawResumeTargetStatus | null;
    recoveryTargetStatus: ClawRecoveryTargetStatus | null;
  } {
    ...
    if (state.status === "blocked" || state.status === "paused") {
      return resolveResumeIntentFromBlockedState(state);
    }
    return {
      resumeTargetStatus: state.resumeTargetStatus ?? null,
      recoveryTargetStatus: state.recoveryTargetStatus ?? null,
    };
  }
  ...
  function moveMissionToWaiting(params: {
    ...
    const resumeTargetStatus = params.resumeTargetStatus ?? null;
    const recoveryTargetStatus = params.recoveryTargetStatus ?? null;
    const result = setFlowWaiting({
      flowId: flow.flowId,
      expectedRevision: flow.revision,
      currentStep: params.currentStep,
      stateJson: buildFlowStateJson({
        state: params.state,
    ...
    params.state.resumeTargetStatus = resumeTargetStatus;
    params.state.recoveryTargetStatus = recoveryTargetStatus;
  }

  function moveMissionToQueued(
    state: ClawMissionStateRecord,
    currentStep: string,
    options?: {
      resumeTargetStatus?: ClawResumeTargetStatus | null;
    ...
    const resumeTargetStatus = options?.resumeTargetStatus ?? null;
    const recoveryTargetStatus = options?.recoveryTargetStatus ?? null;
    ...
    state.resumeTargetStatus = resumeTargetStatus;
  ...
  async function executeVerifierCycle(state: ClawMissionStateRecord): Promise<void> {
    ...
    moveMissionToRunning(
      ...
      decision.nextStep?.trim() ||
        decision.summary ||
        "Continue the mission after verifier rejection.",
    );
    await recordAudit(state, {
      actor: "system",
  ...
  async function claimQueuedMission(queued: ClawMissionStateRecord): Promise<void> {
    if (queued.resumeTargetStatus === "recovering") {
      moveMissionToRecovering(
        queued,
        queued.recoveryTargetStatus ?? "running",
    ...
    } else if (queued.resumeTargetStatus === "verifying") {
      moveMissionToVerifying(queued, "Mission verification resumed.");
      await recordAudit(queued, {
        actor: "system",
        type: "mission.resumed",
        summary: "Mission verification resumed.",
      });
    } else if (queued.resumeTargetStatus === "running") {
      moveMissionToRunning(queued, "Mission execution resumed.");
      await recordAudit(queued, {
        actor: "system",
        type: "mission.resumed",
        summary: "Mission execution resumed.",
      });
    } else {
  ...
  async function createMission(params: {
    ...
    await recordAudit(state, {
      ...
      summary:
        "Mission packet created from the requested goal. Packet planning and preflight may continue before unattended continuation is approved.",
    });
    if (cfg.claw?.enabled === true) {
      ...
      await recordAudit(state, {
        ...
        summary: planningAttempt.usedFallback
          ? "Mission packet planning fell back to the deterministic goal-derived packet before unattended continuation approval."
          : "Mission packet planning completed before unattended continuation approval.",
    ...
    await recordAudit(state, {
      ...
      summary: "Mission preflight started before unattended continuation approval.",
    });
    ...
    state.currentStep = awaitingSetup ? PRE_APPROVAL_BLOCKERS_STEP : AWAITING_APPROVAL_STEP;
    ...
    await recordAudit(state, {
      ...
      summary: awaitingSetup
        ...
        : "Autonomous continuation approval requested.",
    });
    await saveMissionState(state);
    await writeDynamicMissionDocs(state);
    return await getMissionSnapshot(missionId, workspaceDir);
  ...
  async function approveMissionStart(missionId: string): Promise<ClawMissionDetailSnapshot> {
    return await updateMission(missionId, async (state) => {
      ...
      if (!approval) {
        throw new Error(
          `Mission ${missionId} does not have a pending autonomous-continuation approval.`,
        );
      }
      ...
      await recordAudit(state, {
        ...
        summary: "Mission approved for autonomous continuation.",
      });
      return state;
    });
  }
  ...
  async function getAudit(missionId: string, limit?: number): Promise<ClawAuditEntry[]> {
    ...
    const entries = raw
      ...
      .filter(Boolean)
      .map((line) => {
        const parsed = JSON.parse(line) as Partial<ClawAuditEntry>;
        return buildAuditEntry(
          {
            ...
            title: mission.title,
            flowRevision: mission.flowRevision,
          },
          {
            existingId: typeof parsed.id === "string" ? parsed.id : null,
            existingMissionId: typeof parsed.missionId === "string" ? parsed.missionId : mission.id,
            at: parsed.at,
            actor: parsed.actor === "operator" ? "operator" : "system",
            role: parsed.role ?? null,
            phase: parsed.phase ?? null,
            type: typeof parsed.type === "string" ? parsed.type : "mission.audit",
            actionType: parsed.actionType ?? null,
            toolName: parsed.toolName ?? null,
            targetSummary: parsed.targetSummary ?? null,
            sideEffectClass: parsed.sideEffectClass ?? null,
            ...
            checkpointRevision:
              typeof parsed.checkpointRevision === "number" ? parsed.checkpointRevision : null,
            summary:
