...
async function writeRawAtomically(
  ...
  try {
    ...
    if (code === "EPERM" || code === "EEXIST") {
      await ioFs.promises.copyFile(tmpPath, configPath);
      await ioFs.promises.chmod(configPath, 0o600).catch(() => {
        // best-effort
      });
      await ioFs.promises.unlink(tmpPath).catch(() => {
        // best-effort
      });
      return;
    }
...
export async function runConfigWriteTransaction(
  ...
  try {
    ...
    const rollbackMatches =
      beforeSnapshot.exists && beforeHash ? restoredHash === beforeHash : !beforeSnapshot.exists;
    if (!rollbackMatches) {
      return {
        ok: false,
        transactionId,
