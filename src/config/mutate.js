...
import { readConfigFileSnapshotForWrite, resolveConfigSnapshotHash, writeConfigFile, } from "./io.js";
...
async function writeJsonFileAtomic(filePath, value) {
    ...
    try {
        ...
        await fs.access(filePath).then(async () => await maintainConfigBackups(filePath, fs), () => undefined);
...
async function tryWriteSingleTopLevelIncludeMutation(params) {
    ...
    if (!validated.ok) {
        throw createInvalidConfigError(params.snapshot.path, formatInvalidConfigDetails(validated.issues));
    }
    await writeJsonFileAtomic(includePath, nextConfigRecord[key]);
    return true;
}
export async function replaceConfigFile(params) {
    const prepared = params.snapshot && params.writeOptions
...
export async function mutateConfigFile(params) {
    const { snapshot, writeOptions } = await readConfigFileSnapshotForWrite();
    const previousHash = assertBaseHashMatches(snapshot, params.baseHash);
    const baseConfig = params.base === "runtime" ? snapshot.runtimeConfig : snapshot.sourceConfig;
    const draft = structuredClone(baseConfig);
    const result = (await params.mutate(draft, { snapshot, previousHash }));
    const wroteInclude = await tryWriteSingleTopLevelIncludeMutation({
        snapshot,
        nextConfig: draft,
    });
