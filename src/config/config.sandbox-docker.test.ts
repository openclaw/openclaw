...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("mullusi-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "mullusi-sandbox-browser-global" },
      agentBrowser: { network: "mullusi-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("mullusi-sandbox-browser-agent");
  });
