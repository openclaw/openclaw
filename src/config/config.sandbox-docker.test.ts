...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("activi-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "activi-sandbox-browser-global" },
      agentBrowser: { network: "activi-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("activi-sandbox-browser-agent");
  });
