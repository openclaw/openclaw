...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    const shared = path.join(tmpdir(), "smart-agent-neo-shared-agentdir");
        ...
        JSON.stringify(
          {
            agents: {
              list: [
                { id: "a", agentDir: "~/.smart-agent-neo/agents/shared/agent" },
                { id: "b", agentDir: "~/.smart-agent-neo/agents/shared/agent" },
              ],
            },
