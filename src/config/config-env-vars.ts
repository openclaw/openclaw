...
function collectConfigEnvVarsByTarget(cfg?: MullusiConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: MullusiConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: MullusiConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: MullusiConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.MULLUSI_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
