...
describe("config file adapter", () => {
  ...
  beforeEach(() => {
    ...
    // These are mode/recovery boundary tests, not host-native qualification.
    vi.stubGlobal("process", {
      ...process,
      platform: "linux",
      release: { ...process.release, name: "node" },
      versions: { ...process.versions, bun: undefined, deno: undefined },
    });
  ...
  function createHarness(included: string[] = []) {
    ...
    const dirty = async (name?: string, reason: WatchDirty["reason"] = "event") => {
  ...
  function fakeObserver(options: WatchOptions) {
    ...
    return {
      ...
      options,
      ready: () => ready.resolve(),
      dirty: (relative?: string, reason: WatchDirty["reason"] = "event") =>
        options.onDirty({
          generation: 1,
      ...
      fail: (
        ...
        failure: WatchHealth["failure"] = { operation: "watch", code: "ENOSPC" },
      ) =>
        options.onHealth?.({
          ...subscription.health(),
          state: "unavailable",
  ...
  it("settles a canonical file change without rebuilding an unrelated indirect alias scope", async () => {
    ...
    const structural = (relative: string) =>
      h.current().options.onDirty({
        generation: 1,
        scopes: h.current().options.scopes,
        reason: "reconcile",
  ...
  it.each([
    ...
    "selects the default config mode for $platform/$runtime",
    async ({ platform, runtime, mode }) => {
      ...
      expect(h.current().options.intervalMs).toBe(mode === "poll" ? 100 : undefined);
    },
  );

  it.each(["false", "0", ""])(
    "preserves explicit native-only config on unsupported hosts (%s)",
    async (setting) => {
      ...
      for (const delay of [500, 2000, 5000]) {
        h.current().fail(unavailable, { operation: "watch" });
        await vi.advanceTimersByTimeAsync(delay);
      }
      h.current().fail(unavailable, { operation: "watch" });
      await vi.runAllTimersAsync();
      expect(h.adapter.status()).toBe("disabled");
      expect(h.watch.mock.calls.every(([, options]) => options.mode === "node")).toBe(true);
      expect(h.log.error).toHaveBeenCalledWith(
        expect.stringContaining("select polling explicitly"),
      );
    },
  );
  ...
  ])("keeps explicit config polling cadence $configured", async ({ configured, interval }) => {
    vi.stubEnv("CHOKIDAR_USEPOLLING", "TrUe");
    vi.stubEnv("CHOKIDAR_INTERVAL", configured);
    const h = createHarness();
    await h.start();
    expect(h.current().options).toMatchObject({ mode: "poll", intervalMs: interval });
  });

  it.each([
    { setting: undefined, modes: ["node", "poll"] },
    { setting: "1", modes: ["poll"] },
    { setting: "TrUe", modes: ["poll"] },
    { setting: "0", modes: ["node"] },
    { setting: "FALSE", modes: ["node"] },
  ...
  it.each([undefined, "false"])(
    "bounds native workers and preserves fallback policy (%s)",
    async (setting) => {
      vi.stubEnv("CHOKIDAR_USEPOLLING", setting);
      const h = createHarness(
        Array.from({ length: 512 }, (_, index) => p(`include-${index}.json`)),
      );
      await h.start();
      expect(h.watch).toHaveBeenCalledTimes(4);
      expect(h.log.warn).toHaveBeenCalledWith(
        expect.stringContaining("Config observation worker limit reached (4)"),
      );
      for (const delay of [500, 2000, 5000]) {
        await vi.advanceTimersByTimeAsync(delay);
      }
      expect(h.watch).toHaveBeenCalledTimes(16);
      if (setting === "false") {
        expect(h.adapter.status()).toBe("disabled");
        expect(h.watch.mock.calls.every(([, options]) => options.mode === "node")).toBe(true);
      } else {
        await vi.advanceTimersByTimeAsync(500);
        expect(h.watch).toHaveBeenCalledTimes(21);
        expect(h.watch.mock.calls.slice(-5).every(([, options]) => options.mode === "poll")).toBe(
          true,
        );
        for (const observer of h.observers.slice(-5)) {
          observer.ready();
        }
        await vi.advanceTimersByTimeAsync(0);
        expect(h.adapter.status()).toBe("active");
