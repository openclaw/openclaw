...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-smart-agent-neo",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-smart-agent-neo#general");
  });
  ...
  it("derives session transcripts dir from SMART_AGENT_NEO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SMART_AGENT_NEO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.SMART_AGENT_NEO_STATE_DIR = "/custom/state";
    try {
