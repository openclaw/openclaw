...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mullusi-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ MULLUSI_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-mullusi",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-mullusi#general");
  });
  ...
  it("derives session transcripts dir from MULLUSI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MULLUSI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when MULLUSI_STATE_DIR differs from stored paths", () => {
  ...
  it("resolveSessionFilePathOptions keeps explicit agentId alongside absolute store path", () => {
    const storePath = "/tmp/mullusi/agents/main/sessions/sessions.json";
