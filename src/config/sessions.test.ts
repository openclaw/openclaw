...
import { useSessionStoreTestDb } from "./sessions/test-helpers.sqlite.js";
...
describe("sessions", () => {
  ...
  function makeStorePath(prefix: string): string {
    ...
    return path.join(fixtureRoot, "agents", agentId, "sessions", "sessions.json");
  }

  function seedStore(storePath: string, entries: Record<string, Record<string, unknown>>): void {
    testDb.seed(storePath, entries as Record<string, SessionEntry>);
  }
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const storePath = makeStorePath("updateLastRoute");
    seedStore(storePath, {
  ...
  it("updateSessionStoreEntry preserves existing fields when patching", async () => {
    ...
    const storePath = makeStorePath("updateSessionStoreEntry");
    seedStore(storePath, {
      [sessionKey]: {
        sessionId: "sess-1",
        updatedAt: 100,
  ...
  it("updateSessionStoreEntry returns null when session key does not exist", async () => {
    const storePath = makeStorePath("updateSessionStoreEntry-missing");
    seedStore(storePath, {});
  ...
  it("updateSessionStoreEntry keeps existing entry when patch callback returns null", async () => {
    const sessionKey = "agent:main:main";
    const storePath = makeStorePath("updateSessionStoreEntry-noop");
    seedStore(storePath, {
      [sessionKey]: {
        sessionId: "sess-1",
        updatedAt: 123,
  ...
  it("updateSessionStore preserves concurrent additions", async () => {
    const storePath = makeStorePath("updateSessionStore");
    seedStore(storePath, {});
