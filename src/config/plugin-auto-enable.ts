...
function isTelegramConfigured(cfg: DNAConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: DNAConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: DNAConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: DNAConfig): boolean {
...
function isIMessageConfigured(cfg: DNAConfig): boolean {
...
function isWhatsAppConfigured(cfg: DNAConfig): boolean {
...
function isGenericChannelConfigured(cfg: DNAConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: DNAConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: DNAConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: DNAConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: DNAConfig, pluginId: string): boolean {
...
function enablePluginEntry(cfg: DNAConfig, pluginId: string): DNAConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
