...
const EnvSecretRefSchema = z
    ...
    provider: z
        .string()
        .regex(SECRET_PROVIDER_ALIAS_PATTERN, 'Secret reference provider must match /^[a-z][a-z0-9_-]{0,63}$/ (example: "default").'),
    id: z
        .string()
        .regex(ENV_SECRET_REF_ID_PATTERN, 'Env secret reference id must match /^[A-Z][A-Z0-9_]{0,127}$/ (example: "OPENAI_API_KEY").'),
})
    .strict();
const FileSecretRefSchema = z
    ...
    id: z
        .string()
        .refine(isValidFileSecretRefId, 'File secret reference id must be an absolute JSON pointer (example: "/providers/openai/apiKey"), or "value" for singleValue mode.'),
})
    .strict();
const ExecSecretRefSchema = z
    ...
    id: z
        .string()
        .regex(EXEC_SECRET_REF_ID_PATTERN, 'Exec secret reference id must match /^[A-Za-z0-9][A-Za-z0-9._:/-]{0,255}$/ (example: "vault/openai/api-key").'),
})
    .strict();
