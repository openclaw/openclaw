...
export const OpenClawSchema = z
  .object({
      ...
      .object({
        ...
        stream: z
          .object({
            ...
            deliveryMode: z
              .union([z.literal("live"), z.literal("final_only")])
              .optional(),
    ...
    cron: z
      .object({
        ...
        retry: z
          .object({
            ...
            retryOn: z
              .array(
                z.enum(["rate_limit", "network", "timeout", "server_error"]),
              )
              .min(1)
              .optional(),
          })
      ...
      .superRefine((val, ctx) => {
        if (
          val.sessionRetention !== undefined &&
          val.sessionRetention !== false
        ) {
          try {
            parseDurationMs(String(val.sessionRetention).trim(), {
              defaultUnit: "h",
            });
          } catch {
            ctx.addIssue({
        ...
        if (val.runLog?.maxBytes !== undefined) {
          try {
            parseByteSize(String(val.runLog.maxBytes).trim(), {
              defaultUnit: "b",
            });
          } catch {
            ctx.addIssue({
