...
describe("config transaction e2e", () => {
  ...
  it("rejects stale expected base hash writes", async () => {
    await withConfigCase({
      raw: '{\n  "gateway": { "mode": "local" }\n}\n',
      run: async () => {
        const snapshot = await readConfigFileSnapshot();
        const expectedBaseHash = resolveConfigSnapshotHash(snapshot);
        expect(expectedBaseHash).toBeTruthy();

        await runConfigWriteTransaction({
          config: {
            gateway: { mode: "local", bind: "loopback" },
          },
        });
        const staleWrite = await runConfigWriteTransaction({
          config: {
            gateway: { mode: "local", bind: "loopback" },
          },
          expectedBaseHash,
        ...
        expect(staleWrite.stage).toBe("prepare");
        expect(staleWrite.error).toContain("config base hash mismatch");
      },
    });
  });
});
