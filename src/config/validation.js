...
import { collectRelevantDoctorPluginIds, collectRelevantDoctorPluginIdsForTouchedPaths, listPluginDoctorLegacyConfigRules, } from "../plugins/doctor-contract-registry.js";
...
function extractBindingsSpecificUnionIssue(record, parentPath) {
    ...
    for (const errGroup of record.errors) {
        ...
        for (const issue of branch) {
            ...
            const issueIsBetter = issuePathLen > branchBestPathLen
                ? true
 ...
 */
export function validateConfigObjectRaw(raw, opts) {
    ...
    const legacyIssues = findLegacyConfigIssues(normalizedRaw, normalizedRaw, extraLegacyRules, opts?.touchedPaths);
    ...
    for (const pluginId of allow) {
        ...
        if (!knownIds.has(pluginId)) {
            ...
            if (commandAlias?.pluginId && knownIds.has(commandAlias.pluginId)) {
                warnings.push({
                    path: "plugins.allow",
                    message: `"${pluginId}" is not a plugin — it is a command provided by the "${commandAlias.pluginId}" plugin. ` +
