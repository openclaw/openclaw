...
describe("resolveChannelGroupPolicy listen-only", () => {
  it("allows group messages with listenOnly flag when groupPolicy=listen-only", () => {
    const cfg = {
      channels: {
        whatsapp: {
    ...
    expect(policy.allowed).toBe(true);
    expect(policy.listenOnly).toBe(true);
  });

  it("respects account-scoped listen-only override", () => {
    const cfg = {
      channels: {
        whatsapp: {
          groupPolicy: "open",
  ...
  it("does not set listenOnly for open policy", () => {
    const cfg = {
      channels: {
        whatsapp: {
          groupPolicy: "open",
    ...
    expect(policy.allowed).toBe(true);
    expect(policy.listenOnly).toBeUndefined();
  });
});
