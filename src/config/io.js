...
import { appendConfigAuditRecord, appendConfigAuditRecordSync, createConfigWriteAuditRecordBase, finalizeConfigWriteAuditRecord, formatConfigOverwriteLogMessage, } from "./io.audit.js";
import { throwInvalidConfig } from "./io.invalid-config.js";
import { maybeRecoverSuspiciousConfigRead, maybeRecoverSuspiciousConfigReadSync, promoteConfigSnapshotToLastKnownGood as promoteConfigSnapshotToLastKnownGoodWithDeps, recoverConfigFromLastKnownGood as recoverConfigFromLastKnownGoodWithDeps, } from "./io.observe-recovery.js";
import { persistGeneratedOwnerDisplaySecret } from "./io.owner-display-secret.js";
import { collectChangedPaths, createMergePatch, formatConfigValidationFailure, projectSourceOntoRuntimeShape, restoreEnvRefsFromMap, resolvePersistCandidateForWrite, resolveWriteEnvSnapshotForPath, unsetPathForWrite, } from "./io.write-prepare.js";
...
import { clearRuntimeConfigSnapshot as clearRuntimeConfigSnapshotState, finalizeRuntimeSnapshotWrite, getRuntimeConfigSnapshot as getRuntimeConfigSnapshotState, getRuntimeConfigSourceSnapshot as getRuntimeConfigSourceSnapshotState, loadPinnedRuntimeConfig, notifyRuntimeConfigWriteListeners, registerRuntimeConfigWriteListener, resetConfigRuntimeState as resetConfigRuntimeStateState, selectApplicableRuntimeConfig, setRuntimeConfigSnapshot as setRuntimeConfigSnapshotState, getRuntimeConfigSnapshotRefreshHandler as getRuntimeConfigSnapshotRefreshHandlerState, setRuntimeConfigSnapshotRefreshHandler as setRuntimeConfigSnapshotRefreshHandlerState, } from "./runtime-snapshot.js";
import { resolveShellEnvExpectedKeys } from "./shell-env-expected-keys.js";
import { validateConfigObjectRawWithPlugins, validateConfigObjectWithPlugins, } from "./validation.js";
import { shouldWarnOnTouchedVersion } from "./version.js";
export { clearRuntimeConfigSnapshotState as clearRuntimeConfigSnapshot, getRuntimeConfigSnapshotState as getRuntimeConfigSnapshot, getRuntimeConfigSourceSnapshotState as getRuntimeConfigSourceSnapshot, resetConfigRuntimeStateState as resetConfigRuntimeState, selectApplicableRuntimeConfig, setRuntimeConfigSnapshotState as setRuntimeConfigSnapshot, setRuntimeConfigSnapshotRefreshHandlerState as setRuntimeConfigSnapshotRefreshHandler, };
...
async function observeConfigSnapshot(deps, snapshot) {
    ...
    if (suspicious.length === 0) {
        if (snapshot.valid) {
            ...
            if (!sameFingerprint(entry.lastKnownGood, current) ||
...
async function persistPrefixedConfigRecovery(params) {
    ...
    params.deps.logger.warn(`Config auto-stripped non-JSON prefix: ${params.configPath}` +
...
async function recoverConfigFromJsonRootSuffixWithDeps(params) {
    ...
    const legacyResolution = resolveLegacyConfigForRead(readResolution.resolvedConfigRaw, suffixRecovery.parsed);
...
export function createConfigIO(overrides = {}) {
    ...
    function loadConfig() {
        try {
            ...
            for (const w of readResolution.envWarnings) {
                deps.logger.warn(`Config (${configPath}): missing env var "${w.varName}" at ${w.configPath} - feature using this value will be unavailable`);
            }
            ...
            if (validated.warnings.length > 0) {
                const details = validated.warnings
                    .map((iss) => `- ${sanitizeTerminalText(iss.path || "<root>")}: ${sanitizeTerminalText(iss.message)}`)
                    .join("\n");
    ...
    async function readSourceConfigBestEffort() {
        ...
        try {
            ...
            const readResolution = resolveConfigForRead(resolved, deps.env);
            const legacyResolution = resolveLegacyConfigForRead(readResolution.resolvedConfigRaw, recovered.parsed);
        ...
        try {
            ...
            try {
                ...
                if (code === "EPERM" || code === "EEXIST") {
                    ...
                    await appendWriteAudit("copy-fallback", undefined, await deps.fs.promises.stat(configPath).catch(() => null));
            ...
            logConfigOverwrite();
            logConfigWriteAnomalies();
            await appendWriteAudit("rename", undefined, await deps.fs.promises.stat(configPath).catch(() => null));
            return { persistedHash: nextHash, persistedConfig: stampedOutputConfig };
        }
        catch (err) {
...
export function registerConfigWriteListener(listener) {
...
export function projectConfigOntoRuntimeSourceSnapshot(config) {
    ...
    const projectedSource = coerceConfig(projectSourceOntoRuntimeShape(runtimeConfigSourceSnapshot, runtimeConfigSnapshot));
...
export async function readSourceConfigBestEffort() {
    return await createConfigIO().readSourceConfigBestEffort();
}
export async function readConfigFileSnapshot() {
    return await createConfigIO().readConfigFileSnapshot();
}
export async function promoteConfigSnapshotToLastKnownGood(snapshot) {
...
export async function writeConfigFile(cfg, options = {}) {
    ...
    if (options.skipRuntimeSnapshotRefresh &&
        !hadRuntimeSnapshot &&
