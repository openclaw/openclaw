...
export const FIELD_HELP: Record<string, string> = {
  ...
  "agents.defaults.compaction.fallbacks":
    "Ordered list of provider/model references to try when the compaction summarization call fails on quota, rate-limit, or transient provider errors. Each entry is a `provider/model` string (same shape as `agents.defaults.model.fallbacks`). Entries equal to `compaction.model` are skipped, and duplicates are removed. When unset, compaction inherits `agents.defaults.model.fallbacks`. When all candidates fail, compaction falls through to the existing truncate-only behavior.",
