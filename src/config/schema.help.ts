...
export const FIELD_HELP: Record<string, string> = {
  ...
  "gateway.tools.directInvoke.hostFsRead":
    "Allow the `read` coding tool to be materialized for direct-invoke surfaces (HTTP POST /tools/invoke + SDK RPC tools.invoke). Requires gateway.tools.allow to also include `read` AND the request to come from an owner/admin sender (senderIsOwner === true) — a non-owner trusted-proxy caller (e.g. operator.write) is refused the built-in read tool even when both config keys are set (the owner gate scopes only built-in host-FS materialization; a same-named allowlisted plugin tool follows the normal gateway.tools.allow/deny policy). Default false. Exposes host filesystem reads outside the workspace unless tools.fs.workspaceOnly is enabled.",
  "gateway.tools.directInvoke.hostFsWrite":
    "Allow the host-filesystem write coding tools (`write`, `edit`) to be materialized for direct-invoke surfaces. Requires gateway.tools.allow to also include each tool name AND the request to come from an owner/admin sender (senderIsOwner === true) — a non-owner trusted-proxy caller (e.g. operator.write) is refused the built-in write/edit tools even when both config keys are set (the owner gate scopes only built-in host-FS materialization; a same-named allowlisted plugin tool follows the normal gateway.tools.allow/deny policy). Default false. Exposes host filesystem writes outside the workspace unless tools.fs.workspaceOnly is enabled. STRONGLY recommend pairing with tools.fs.workspaceOnly: true. `apply_patch` is intentionally NOT controlled by this flag yet (no factory entry).",
