...
import { hasSensitiveUrlHintTag, isSensitiveUrlConfigPath, redactSensitiveUrlLikeString, } from "../shared/net/redact-sensitive-url.js";
        ...
        for (const [key, value] of Object.entries(obj)) {
            ...
            for (const candidate of [path, wildcardPath]) {
                ...
                if (lookup.has(candidate)) {
                    ...
                    else if (hints[candidate]?.sensitive === true &&
                        value !== undefined &&
                        value !== null) {
...
function redactObjectGuessing(obj, prefix, values, hints) {
    ...
    if (isObjectRecord(obj)) {
        ...
        for (const [key, value] of Object.entries(obj)) {
            ...
            else if (!isExplicitlyNonSensitivePath(hints, [dotPath, wildcardPath]) &&
                isSensitivePath(dotPath) &&
...
function maybeRestoreSecretRefId(params) {
    ...
    if (!isSecretRefWithProvider(originalObj)) {
        if (isSecretRefShape(originalObj)) {
            throw new RedactionError(params.path, `SecretRef at ${params.path} requires a provider field to restore the redacted id automatically (original ref lacks provider).`);
        }
    ...
    if (!isSecretRefWithProvider(incomingObj)) {
        throw new RedactionError(params.path, `SecretRef at ${params.path} must include source, provider, and id when redacted placeholders are present.`);
    }
    if (incomingObj.source !== originalObj.source || incomingObj.provider !== originalObj.provider) {
        throw new RedactionError(params.path, `SecretRef at ${params.path} changed source/provider while id is redacted. Provide an explicit id when changing source/provider.`);
    }
...
function shouldRestoreSensitiveGuessingPath(path, hintPaths, hints) {
    return (!isExplicitlyNonSensitivePath(hints, hintPaths) &&
