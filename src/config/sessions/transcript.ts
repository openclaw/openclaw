...
function normalizeDeliveryMirrorComparisonText(text: string): string {
  // Delivery may rejoin streamed blocks with whitespace the persisted reply does
  // not have; compare on collapsed whitespace so a pure formatting difference
  // does not defeat the duplicate guard.
  return text.trim().replace(/\s+/g, " ");
}

function redactedDeliveryMirrorText(text: string, config?: OpenClawConfig): string | null {
  // Redact the candidate exactly as appendSessionTranscriptMessage will before
  // it is written, so a secret-shaped reply compares against the redacted tail
  // row instead of its raw form and the duplicate guard cannot be bypassed.
  const redacted = redactTranscriptMessage(
    { role: "assistant", content: [{ type: "text", text }] } as AgentMessage,
    config,
  );
  return extractAssistantMessageText(redacted as unknown as SessionTranscriptAssistantMessage);
}

/**
 * Returns true when appending `deliveredText` as a delivery-mirror row would
 * duplicate the current turn's tail assistant reply. Stops at the first message
 * entry (a user line marks a new turn, so an identical later reply is a
 * legitimate repeat, not a gap-fill duplicate) and redacts both sides so secret
 * redaction asymmetry between the raw candidate and the persisted row cannot
 * defeat the guard.
 */
export async function isDeliveryMirrorTailDuplicate(
  transcriptPath: string,
  deliveredText: string,
  config?: OpenClawConfig,
): Promise<boolean> {
  const expected = redactedDeliveryMirrorText(deliveredText, config);
  if (!expected) {
    return false;
  }
  const expectedNormalized = normalizeDeliveryMirrorComparisonText(expected);

  for await (const line of streamSessionTranscriptLinesReverse(transcriptPath)) {
    let parsed: { message?: unknown };
    try {
      parsed = JSON.parse(line) as { message?: unknown };
    } catch {
      continue;
    }
    // Skip non-message metadata entries (e.g. openclaw.cache-ttl) so a marker
    // line written after the assistant turn does not hide the tail reply.
    if (!parsed.message || typeof parsed.message !== "object") {
      continue;
    }
    const candidate = parsed.message as SessionTranscriptAssistantMessage;
    ...
    return (
      candidateText !== null &&
      normalizeDeliveryMirrorComparisonText(candidateText) === expectedNormalized
    );
  }
  return false;
}
