...
describe("session goals", () => {
  ...
  it("migrates legacy plugin-owned goal mirror slots before creating core goals", async () => {
    await upsertSessionEntry({
      storePath: fixture.storePath(),
      sessionKey,
      entry: {
        ...
        pluginExtensionSlotKeys: {
          "legacy-plugin": {
            workflow: "goal",
            other: "approvalSnapshot",
          },
        },
        goal: { state: "plugin-owned" },
      } as unknown as Parameters<typeof upsertSessionEntry>[0]["entry"],
    });

    const goal = await createSessionGoal({
      storePath: fixture.storePath(),
      sessionKey,
      objective: "take over core goal",
      now: 10,
    });
    const entry = getSessionEntry({ storePath: fixture.storePath(), sessionKey });

    expect(goal.objective).toBe("take over core goal");
    expect(entry?.goal?.schemaVersion).toBe(1);
    expect(entry?.goal?.id).toBe(goal.id);
    expect(entry?.pluginExtensions?.["legacy-plugin"]?.workflow).toEqual({ state: "waiting" });
  ...
  it("clears legacy plugin-owned goal mirror slots when reading goal status", async () => {
    await upsertSessionEntry({
      storePath: fixture.storePath(),
      sessionKey,
      entry: {
    ...
    const snapshot = await getSessionGoal({ storePath: fixture.storePath(), sessionKey, now: 10 });
    const entry = getSessionEntry({ storePath: fixture.storePath(), sessionKey });
