...
async function makeFsSafeTmp(
  ...
  const tp = path.join(
    ...
    `.${path.basename(storePath)}.${pid}.${token}.tmp`,
  );
  await fs.writeFile(tp, makeStore(entries), "utf-8");
  const oldTime = new Date(Date.now() - ageMs);
  await fs.utimes(tp, oldTime, oldTime);
...
describe("session store recovery", () => {
  ...
  it("main parse error -> recovery from real fs-safe residual tmp", async () => {
    await withTempDir({ prefix: "pr2-" }, async (dir) => {
      const sp = path.join(dir, "sessions.json");
      await fs.writeFile(sp, "{bad json!!!", "utf-8");
      await makeFsSafeTmp(
      ...
      const store = loadSessionStore(sp, { skipCache: true });
      expect(Object.keys(store)).toHaveLength(2);
      expect(store.fs1?.sessionId).toBe("a");
      expect(store.fs2?.sessionId).toBe("b");
    });
  });
  ...
  it("sourceRank: .bak(3) preferred over stale tmp(2)", async () => {
    await withTempDir({ prefix: "pr2-" }, async (dir) => {
      ...
      await fs.writeFile(`${sp}.bak`, makeStore([["bak", "recovered"]]), "utf-8");
  ...
  it("stale real fs-safe tmp preferred over fresh legacy tmp", async () => {
    await withTempDir({ prefix: "pr2-" }, async (dir) => {
      const sp = path.join(dir, "sessions.json");
      await fs.writeFile(sp, "{bad", "utf-8");
      const freshLegacy = `${sp}.fresh.tmp`;
      await fs.writeFile(freshLegacy, makeStore([["fresh-legacy", "fresh"]]), "utf-8");
      await makeFsSafeTmp(sp, "5678", "fedcba", [["stale-fssafe", "stale"]], 10000);
      const store = loadSessionStore(sp, { skipCache: true });
      expect(store["stale-fssafe"]?.sessionId).toBe("stale");
      expect(store["fresh-legacy"]).toBeUndefined();
    });
  });
  ...
  it(".bak always included even with 60+ stale tmps", async () => {
    await withTempDir({ prefix: "pr2-" }, async (dir) => {
      ...
      for (let i = 0; i < 60; i++) {
        ...
        for (let j = 0; j <= i; j++) entries.push([`t${i}s${j}`, `id-${j}`]);
  ...
  it("candidate with only non-object entries -> rejected", async () => {
    await withTempDir({ prefix: "pr2-" }, async (dir) => {
      ...
      await fs.writeFile(sp, "{bad json", "utf-8");
      const tp = `${sp}.bad.tmp`;
      await fs.writeFile(tp, JSON.stringify({ foo: "bar", bad: 123 }), "utf-8");
  ...
  it("candidate with object entries but no sessionId -> rejected", async () => {
    await withTempDir({ prefix: "pr2-" }, async (dir) => {
      ...
      await fs.writeFile(tp, JSON.stringify({ junk: { foo: "bar" } }), "utf-8");
      const oldTime = new Date(Date.now() - 10000);
      await fs.utimes(tp, oldTime, oldTime);
      const store = loadSessionStore(sp, { skipCache: true });
      expect(Object.keys(store)).toHaveLength(0);
  ...
  it("parseable object with invalid entries does not trigger recovery", async () => {
    await withTempDir({ prefix: "pr2-" }, async (dir) => {
      ...
      await fs.writeFile(sp, JSON.stringify({ junk: { foo: "bar" } }), "utf-8");
      await fs.writeFile(`${sp}.bak`, makeStore([["s1", "a"]]), "utf-8");
      const store = loadSessionStore(sp, { skipCache: true });
