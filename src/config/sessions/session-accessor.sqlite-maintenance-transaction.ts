...
export function reclaimSessionMaintenanceInTransaction(
  ...
  // Slow-hold diagnostics need the store and the operation to be attributable (#157686).
  const transactionOptions = (operation: string) => ({
    ...(plan.databaseOptions.path ? { databaseLabel: plan.databaseOptions.path } : {}),
    operationLabel: `session.reclamation.${operation}`,
  });
  if (plan.kind === "maintenance-statistics") {
    ...
    runWithSqliteBusyTimeout(database.db, 0, () =>
      runOpenClawAgentWriteTransaction(
        ...
        { busyTimeoutMs: 0, ...transactionOptions("statistics") },
      ),
    );
  ...
  if (plan.kind === "maintenance-plan") {
    ...
    try {
      return runOpenClawAgentWriteTransaction(
        ...
        plan.databaseOptions,
        transactionOptions("maintenance-plan"),
      );
    } catch (error) {
