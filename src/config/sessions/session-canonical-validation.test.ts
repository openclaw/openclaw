...
it("leaves a marker when serialized writer bindings do not match the stored row", async () => {
  await withOpenClawTestState({ scenario: "minimal" }, async ({ env }) => {
    const scope = { agentId: "main", env, sessionKey: "agent:main:conditional" };
    replaceSessionEntrySync(scope, { sessionId: "conditional", updatedAt: 1 });
    const database = openOpenClawAgentDatabase(scope);
    const entryJson = String(
      database.db
        .prepare("SELECT entry_json FROM session_nodes WHERE session_key = ?")
        .get(scope.sessionKey)?.entry_json,
    );
    database.db
      .prepare("INSERT INTO session_canonical_validation_pending VALUES (?)")
      .run(scope.sessionKey);
    const row = {
      session_key: scope.sessionKey,
      current_session_id: "conditional",
    ...
    runOpenClawAgentWriteTransaction((writer) => {
      certifyCanonicalSessionValidationEntry(writer, {
        ...row,
        entry_json: JSON.stringify({ sessionId: "conditional", updatedAt: 2 }),
      });
      expect(hasPendingCanonicalSessionValidation(writer)).toBe(true);
      certifyCanonicalSessionValidationEntry(writer, row);
      expect(hasPendingCanonicalSessionValidation(writer)).toBe(false);
    }, scope);
