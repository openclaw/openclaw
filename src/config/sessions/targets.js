...
import { resolveAgentSessionDirsFromAgentsDir, resolveAgentSessionDirsFromAgentsDirSync, } from "../../agents/session-dirs.js";
...
function shouldSkipDiscoveredAgentDirName(dirName, agentId) {
    ...
    return (agentId === DEFAULT_AGENT_ID && normalizeLowercaseStringOrEmpty(dirName) !== DEFAULT_AGENT_ID);
}
...
export function resolveAllAgentSessionStoreTargetsSync(cfg, params = {}) {
    const env = params.env ?? process.env;
    const { configuredTargets, agentsRoots } = resolveSessionStoreDiscoveryState(cfg, env);
    const realAgentsRoots = new Map();
...
export async function resolveAllAgentSessionStoreTargets(cfg, params = {}) {
    const env = params.env ?? process.env;
    const { configuredTargets, agentsRoots } = resolveSessionStoreDiscoveryState(cfg, env);
    const realAgentsRoots = new Map();
...
export function resolveSessionStoreTargets(cfg, opts, params = {}) {
    const env = params.env ?? process.env;
    ...
    if (hasAgent) {
        ...
        if (!knownAgents.includes(requested)) {
            throw new Error(`Unknown agent id "${opts.agent}". Use "openclaw agents list" to see configured agents.`);
        }
        return [
            {
                agentId: requested,
