...
describe("session artifact helpers", () => {
  it("classifies archived artifact file names", () => {
    expect(
      isSessionArchiveArtifactName(
        "abc.jsonl.deleted.2026-01-01T00-00-00.000Z",
      ),
    ).toBe(true);
    expect(
      isSessionArchiveArtifactName("abc.jsonl.reset.2026-01-01T00-00-00.000Z"),
    ).toBe(true);
    expect(
      isSessionArchiveArtifactName("abc.jsonl.bak.2026-01-01T00-00-00.000Z"),
    ).toBe(true);
    expect(isSessionArchiveArtifactName("sessions.json.bak.1737420882")).toBe(
      true,
    );
  ...
  it("classifies primary transcript files", () => {
    ...
    expect(isPrimarySessionTranscriptFileName("keep.deleted.keep.jsonl")).toBe(
      true,
    );
    expect(
      isPrimarySessionTranscriptFileName(
  ...
  it("formats and parses archive timestamps", () => {
    ...
    expect(
      parseSessionArchiveTimestamp("keep.deleted.keep.jsonl", "deleted"),
    ).toBeNull();
  });
});
