...
type BoundedMessageTailReadOptions = {
  byteOverflowPolicy: "include-newest" | "skip";
  maxBytes: number;
  maxMessages: number;
  offset: number;
};
...
function readBoundedMessageTailFromProjection(
  projection: CurrentProjection,
  options: BoundedMessageTailReadOptions,
): SessionTranscriptBoundedMessageTailPage {
  const totalMessages = resolveVisibleMessagePositions(projection).total;
  const offset = Math.min(options.offset, totalMessages);
  const endExclusive = Math.max(0, totalMessages - offset);
  const start = Math.max(0, endExclusive - options.maxMessages);
  const positions = resolveVisibleMessagePositionRange(projection, start, endExclusive);
  if (positions.length === 0 || options.maxBytes === 0 || options.maxMessages === 0) {
    return {
  ...
  for (const row of metadata) {
    ...
    if (selectedPositions.length >= options.maxMessages) {
      break;
    }
    if (serializedBytes + row.serialized_bytes > options.maxBytes) {
      if (options.byteOverflowPolicy === "skip") {
        continue;
      }
      // Recent history must consume an oversized newest row so offset pagination can advance.
...
export function readRecentSessionTranscriptMessageEvents(
  ...
  return withCurrentProjectionSnapshot(scope, (projection) => {
    ...
    const { activeLeafEntryId, events, totalMessages } = readBoundedMessageTailFromProjection(
      projection,
      {
        byteOverflowPolicy: "include-newest",
        maxBytes,
        maxMessages: Math.min(maxLines, maxMessages),
        offset: 0,
      },
    );
    return { activeLeafEntryId, events, totalMessages };
  });
}
...
export function readSessionTranscriptBoundedMessageTailPage(
  ...
  return withCurrentProjectionSnapshot(scope, (projection) => {
    const offset = Math.max(0, Math.floor(Number.isFinite(options.offset) ? options.offset : 0));
    const maxMessages = Math.min(
      MAX_VISIBLE_MESSAGE_MAX_MESSAGES,
      Math.max(0, Math.floor(Number.isFinite(options.maxMessages) ? options.maxMessages : 0)),
    );
    ...
    return readBoundedMessageTailFromProjection(projection, {
      byteOverflowPolicy: "skip",
      maxBytes,
      maxMessages,
      offset,
    });
