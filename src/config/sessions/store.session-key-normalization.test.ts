...
describe("session store key normalization", () => {
  ...
  it("preserves user display names when inbound group metadata changes", async () => {
    await fs.writeFile(
      storePath,
      JSON.stringify(
        {
          [SIGNAL_GROUP_KEY]: {
            sessionId: "existing-signal-session",
            updatedAt: 1,
            chatType: "group",
            channel: "signal",
    ...
    await recordSessionMetaFromInbound({
      ...
      sessionKey: SIGNAL_GROUP_KEY,
      ctx: {
        ...createSignalGroupContext(),
        GroupSubject: "New Group",
      } as MsgContext,
    });

    const store = loadSessionStore(storePath, { skipCache: true });
    expect(store[SIGNAL_GROUP_KEY]?.displayName).toBe("Research Plan");
    expect(store[SIGNAL_GROUP_KEY]?.subject).toBe("New Group");
  });

  it("refreshes channel-owned display names when inbound group metadata changes", async () => {
    await fs.writeFile(
      storePath,
      JSON.stringify(
        {
          [SIGNAL_GROUP_KEY]: {
            ...
            groupId: SIGNAL_GROUP_ID,
            subject: "Old Group",
            displayName: "signal:g-old-group",
          },
        },
        null,
        2,
    ...
    const store = loadSessionStore(storePath, { skipCache: true });
    expect(store[SIGNAL_GROUP_KEY]?.displayName).toBe("signal:g-new-group");
    expect(store[SIGNAL_GROUP_KEY]?.subject).toBe("New Group");
  });
