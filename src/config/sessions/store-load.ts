...
function collectSessionStoreRecoveryCandidates(storePath: string): {
  ...
  // Supports both legacy/writeTextAtomic temps `sessions.json.<uuid>.tmp`
  // and fs-safe atomic replace temps `.<basename>.<pid>.<uuid>.tmp`
  // created by buildAtomicWriteTempPath in src/infra/fs-safe.ts.
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const isLegacyTmp = entry.startsWith(`${storeBase}.`);

      // Real fs-safe atomic replace temps: `.<basename>.<pid>.<uuid>.tmp`
      const isFsSafeTmp = entry.startsWith(`.${storeBase}.`);

      if (!isLegacyTmp && !isFsSafeTmp) continue;
