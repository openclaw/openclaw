...
describe("resolveSessionStoreTargets", () => {
  ...
  it("prepares each SQLite identity once for a large configured and discovered fleet", async () => {
    await withTempHome(async (home) => {
      const configuredTargets = Array.from({ length: 65 }, (_, index) => ({
        agentId: `agent-${index}`,
        storePath: path.join(
          home,
          ".openclaw",
      ...
      const databaseDirs = new Set(
        configuredTargets.map((target) =>
          path.join(path.dirname(path.dirname(target.storePath)), "agent"),
        ),
      );
      await Promise.all(
      ...
      try {
        expect(
          dedupeSessionStoreTargetsBySqliteTarget(
            [...configuredTargets, ...configuredTargets.map((target) => ({ ...target }))],
            { defaultAgentId: configuredTargets[0]!.agentId },
          ),
        ).toEqual(configuredTargets);
        const preparedPaths = realpathSpy.mock.calls.flatMap(([pathname]) =>
          typeof pathname === "string" && databaseDirs.has(pathname) ? [pathname] : [],
        );
        expect(preparedPaths).toHaveLength(configuredTargets.length);
