...
// Keep integration tests deterministic: never read a real smart-agent-neo.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.SMART_AGENT_NEO_SESSION_CACHE_TTL_MS;
    process.env.SMART_AGENT_NEO_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.SMART_AGENT_NEO_SESSION_CACHE_TTL_MS;
    } else {
      process.env.SMART_AGENT_NEO_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
