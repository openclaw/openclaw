 ...
 */
export async function hardenBackupPermissions(configPath, ioFs) {
    if (!ioFs.chmod) {
        return;
    }
    const backupBase = `${configPath}.bak`;
 ...
 */
export async function cleanOrphanBackups(configPath, ioFs) {
    if (!ioFs.readdir) {
        return;
    }
 ...
 */
export async function maintainConfigBackups(configPath, ioFs) {
