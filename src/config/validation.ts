...
import type { SmartAgentNeoConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SmartAgentNeoConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: SmartAgentNeoConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SmartAgentNeoSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SmartAgentNeoConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SmartAgentNeoConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as SmartAgentNeoConfig,
  };
}
