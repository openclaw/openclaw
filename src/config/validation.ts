...
import type { MullusiConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.mullusi.com/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: MullusiConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: MullusiConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: MullusiConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MullusiSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as MullusiConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: MullusiConfig | null | undefined;
  ...
  const ensureCompatConfig = (): MullusiConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Mullusi config schema.
