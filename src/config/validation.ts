...
import type { DNAConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: DNAConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: DNAConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DNASchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as DNAConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as DNAConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as DNAConfig)),
    ),
  };
}
