...
const NEW_STATE_DIRNAME = ".nexus-agent";
const CONFIG_FILENAME = "nexus-agent.json";
...
/** Build a homedir thunk that respects NEXUS_HOME for the given env. */
 ...
 * Default: ~/.nexus-agent/nexus-agent.json (or $NEXUS_STATE_DIR/nexus-agent.json)
 */
...
export function resolveConfigPath(
  ...
  const stateOverride = env.NEXUS_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.NEXUS_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const nexusStateDir = env.NEXUS_STATE_DIR?.trim();
  if (nexusStateDir) {
    const resolved = resolveUserPath(nexusStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/nexus-agent-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `nexus-agent-${uid}` : "nexus-agent";
 ...
 *
 * Precedence:
 * - `NEXUS_OAUTH_DIR` (explicit override)
