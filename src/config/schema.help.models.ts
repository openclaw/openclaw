...
export const MODEL_FIELD_HELP: Record<string, string> = {
  ...
  "agents.defaults.experimental.decisionAssistance":
    "Global opt-in for automatic Decision experiments such as heartbeat question mode (default: false). Also requires an effective decisionModel for each owning agent and the consumer’s own mode setting. Does not gate the explicit decision_evaluate tool, select providers, or grant actions.",
