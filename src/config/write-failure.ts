...
function extractReasonFromLegacyMessage(message: string): string {
  ...
  tail = tail.replace(/^rollbackTargetHash=([A-Fa-f0-9]{7,128}|unknown);?\s*/, "");
  const firstSegment = tail.split(";")[0]?.trim();
  return firstSegment || "unknown error";
}

function extractRollbackTargetHashFromLegacyMessage(message: string): string | null {
  const hashMatch = message.match(/\brollbackTargetHash=([A-Fa-f0-9]{7,128}|unknown);?/);
  const hash = hashMatch?.[1]?.trim();
  if (!hash || hash.toLowerCase() === "unknown") {
    return null;
  }
  return hash;
}
...
export class ConfigWriteTransactionError extends Error {
  ...
  constructor(result: ConfigWriteTransactionResult) {
    ...
    const rollbackTargetLabel = result.rolledBack
      ? ` rollbackTargetHash=${result.beforeHash ?? "unknown"};`
      : "";
    super(
      `writeConfigFile transaction failed;${stageLabel}${rollbackLabel}${rollbackTargetLabel} ${reason}; last config update failed (${recoveryHint}); please retry`,
    );
    this.name = "ConfigWriteTransactionError";
    this.transactionId = result.transactionId;
    this.stage = result.stage;
    this.rolledBack = result.rolledBack;
    this.beforeHash = result.beforeHash;
    this.reason = reason;
...
export function describeConfigWriteFailure(error: unknown): ConfigWriteFailureDetails | null {
  ...
  // Compatibility shim: older and third-party callers may still throw plain
  // Error strings. Prefer ConfigWriteTransactionError for structured handling.
  ...
  return {
    ...
    beforeHash: extractRollbackTargetHashFromLegacyMessage(message),
...
export function formatConfigWriteFailureForCli(error: unknown): string | null {
  ...
  const rollbackTargetText =
    details.rolledBack && details.beforeHash
      ? ` Rolled back config version hash=${details.beforeHash}.`
      : "";
  const reasonText = details.reason ? ` Error: ${details.reason}.` : "";
  return `${stateText}${rollbackTargetText}${reasonText} Retry the command.`;
}
...
export function formatConfigWriteFailureForChannel(error: unknown): string | null {
  ...
  return `⚠️ Config update failed. ${stateText}${rollbackTargetText}${reasonText} Please retry.`;
}
