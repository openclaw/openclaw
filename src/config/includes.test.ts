...
const ETC_SMART_AGENT_NEO_DIR = path.join(ROOT_DIR, "etc", "smart-agent-neo");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "smart-agent-neo.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSmartAgentNeoPath(...parts: string[]) {
  return path.join(ETC_SMART_AGENT_NEO_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcSmartAgentNeoPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "smart-agent-neo.json"))).toEqual({
      shared: true,
    });
  });
});
