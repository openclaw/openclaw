...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = SmartAgentNeoSchema.safeParse({
      $schema: "https://smart-agent-neo.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://smart-agent-neo.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = SmartAgentNeoSchema.safeParse({ $schema: 123 });
