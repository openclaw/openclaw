  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.dna/logs/dna.log" },
        hooks: {
          path: "~/.dna/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".dna", "logs", "dna.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".dna", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".dna", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".dna", "agents", "main"));
