  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.smart-agent-neo/logs/smart-agent-neo.log" },
        hooks: {
          path: "~/.smart-agent-neo/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.smart-agent-neo/telegram.token",
              },
            },
          },
          imessage: {
          ...
          list: [
            {
              id: "main",
              workspace: "~/ws-agent",
              agentDir: "~/.smart-agent-neo/agents/main",
              identity: {
                name: "~not-a-path",
              },
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".smart-agent-neo", "logs", "smart-agent-neo.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".smart-agent-neo", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".smart-agent-neo", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".smart-agent-neo", "agents", "main"));
