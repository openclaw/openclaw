  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.hanzo/bot/logs/bot.log" },
        hooks: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".hanzo", "bot", "logs", "bot.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".hanzo", "bot", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".hanzo", "bot", "telegram.token"),
      );
