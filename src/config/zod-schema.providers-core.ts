...
export const DingTalkGatewayChannelSchema = z
  .object({
    requireMention: z.boolean().optional(),
    tools: ToolPolicySchema,
    toolsBySender: ToolPolicyBySenderSchema,
...
export const DingTalkGatewayGroupSchema = z
  .object({
    requireMention: z.boolean().optional(),
    tools: ToolPolicySchema,
    toolsBySender: ToolPolicyBySenderSchema,
    channels: z.record(z.string(), DingTalkGatewayChannelSchema.optional()).optional(),
  })
  .strict();

export const DingTalkGatewayConfigSchema = z
  .object({
    enabled: z.boolean().optional(),
    capabilities: z.array(z.string()).optional(),
    markdown: MarkdownConfigSchema,
    ...
    kafkaBrokers: z.union([z.string(), z.array(z.string())]).optional(),
    dmPolicy: DmPolicySchema.optional().default("pairing"),
    allowFrom: z.array(z.string()).optional(),
    groupAllowFrom: z.array(z.string()).optional(),
    groupPolicy: GroupPolicySchema.optional().default("allowlist"),
    ...
    dmHistoryLimit: z.number().int().min(0).optional(),
    dms: z.record(z.string(), DmConfigSchema.optional()).optional(),
    groups: z.record(z.string(), DingTalkGatewayGroupSchema.optional()).optional(),
    mediaMaxMb: z.number().positive().optional(),
    heartbeat: ChannelHeartbeatVisibilitySchema,
  })
  .strict()
  .superRefine((value, ctx) => {
    requireOpenAllowFrom({
      ...
      path: ["allowFrom"],
      message:
        'channels.dingtalk-gateway.dmPolicy="open" requires channels.dingtalk-gateway.allowFrom to include "*"',
    });
  });
