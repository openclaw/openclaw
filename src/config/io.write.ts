...
import {
  migrateBlankAgentCwdForWrite,
  remapLegacyListExplicitPaths,
} from "./legacy.blank-agent-cwd.js";
...
export async function writeConfigFileFromContext(
  ...
  const resolveValidationCandidate = (candidate: unknown) => {
    ...
    const preflight = containsConfigIncludeDirective(config)
      ...
      : config;
    // A saved blank cwd (authored or included, restored by
    ...
    return migrateBlankAgentCwdForWrite(preflight as OpenClawConfig, resolveExplicitSet()).config;
  };
