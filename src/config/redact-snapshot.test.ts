...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.smart-agent-neo/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/smart-agent-neo/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/smart-agent-neo/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/smart-agent-neo/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/smart-agent-neo/nickserv-password.txt");
