...
export function parseDNAVersion(raw: string | null | undefined): DNAVersion | null {
  if (!raw) return null;
...
export function compareDNAVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseDNAVersion(a);
  const parsedB = parseDNAVersion(b);
