...
export const resolveDefaultAgentIdFromRaw = (raw) => {
    ...
    const firstEntry = list.find((entry) => isRecord(entry) && normalizeOptionalString(entry.id) !== undefined);
    if (firstEntry) {
...
export const defineLegacyConfigMigration = (migration) => migration;
