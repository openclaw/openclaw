...
import { hasPotentialConfiguredChannels, listPotentialConfiguredChannelIds, } from "../channels/config-presence.js";
...
function resolveAgentHarnessOwnerPluginIds(registry, runtime) {
    ...
    return registry.plugins
        .filter((plugin) => (plugin.activation?.onAgentHarnesses ?? []).some((entry) => normalizeOptionalLowercaseString(entry) === normalizedRuntime))
...
function hasConfiguredWebSearchPluginEntry(cfg) {
    ...
    return (!!entries &&
        ...
        Object.values(entries).some((entry) => isRecord(entry) && isRecord(entry.config) && isRecord(entry.config.webSearch)));
}
function hasConfiguredWebFetchPluginEntry(cfg) {
    const entries = cfg.plugins?.entries;
    return (!!entries &&
        typeof entries === "object" &&
        Object.values(entries).some((entry) => isRecord(entry) && isRecord(entry.config) && isRecord(entry.config.webFetch)));
}
...
function resolveRelevantSetupAutoEnablePluginIds(cfg) {
    ...
    if (isRecord(cfg.plugins?.entries?.xai) ||
...
function hasSetupAutoEnableRelevantConfig(cfg) {
    ...
    if (isRecord(cfg.tools?.web) && isRecord(cfg.tools.web.x_search)) {
        return true;
    }
...
export function configMayNeedPluginAutoEnable(cfg, env) {
    if (hasPluginAllowlistWithEntries(cfg)) {
        return true;
    }
    if (hasConfiguredPluginConfigEntry(cfg)) {
...
export function resolvePluginAutoEnableCandidateReason(candidate) {
...
export function resolveConfiguredPluginAutoEnableCandidates(params) {
    ...
    for (const [providerId, pluginId] of Object.entries(resolveAutoEnableProviderPluginIds(params.registry))) {
    ...
    const webFetchProvider = typeof params.config.tools?.web?.fetch?.provider === "string"
...
function materializeConfiguredPluginEntryAllowlist(params) {
    ...
    for (const pluginId of Object.keys(entries).toSorted((left, right) => left.localeCompare(right))) {
        ...
        if (!hasMaterialPluginEntryConfig(entry) ||
...
export function materializePluginAutoEnableCandidatesInternal(params) {
    let next = params.config ?? {};
    const changes = [];
    const autoEnabledReasons = new Map();
    if (next.plugins?.enabled === false) {
        return { config: next, changes, autoEnabledReasons: {} };
    }
    const preferOverCache = new Map();
