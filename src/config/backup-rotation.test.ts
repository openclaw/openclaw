...
describe("backup-rotation", () => {
  ...
  describe("rotateConfigBackups", () => {
    ...
    it("rotates backups in correct order", async () => {
      ...
      expect(operations).toContain(
        "rename:/config/openclaw.json.bak->/config/openclaw.json.bak.1",
      );
    });
    ...
    it("handles errors gracefully (best-effort)", async () => {
      ...
      // Should not throw
      await expect(
        rotateConfigBackups("/config/openclaw.json", mockFs),
  ...
  describe("hardenBackupPermissions", () => {
    ...
    it("hardens permissions on all backup files", async () => {
      ...
      expect(chmodCalls[0]).toEqual({
        path: "/config/openclaw.json.bak",
        mode: 0o600,
      });
      expect(chmodCalls[1]).toEqual({
        path: "/config/openclaw.json.bak.1",
        mode: 0o600,
      });
      expect(chmodCalls[2]).toEqual({
        path: "/config/openclaw.json.bak.2",
        mode: 0o600,
      });
      expect(chmodCalls[3]).toEqual({
        path: "/config/openclaw.json.bak.3",
        mode: 0o600,
      });
      expect(chmodCalls[4]).toEqual({
        path: "/config/openclaw.json.bak.4",
        mode: 0o600,
      });
    });
  ...
  describe("cleanOrphanBackups", () => {
    it("does nothing when readdir is not available", async () => {
      ...
      await expect(
        cleanOrphanBackups("/config/openclaw.json", mockFs),
      ).resolves.toBeUndefined();
    });
  ...
  describe("maintainConfigBackups", () => {
    it("runs full maintenance cycle in correct order", async () => {
      ...
      expect(operations).toContain(
        "copyFile:/config/openclaw.json->/config/openclaw.json.bak",
      );
    });
    ...
    it("handles errors gracefully at each step", async () => {
      ...
      await expect(
        maintainConfigBackups("/config/openclaw.json", mockFs),
      ).resolves.toBeUndefined();
    });
  });
});
