...
import type { EasyHubConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareEasyHubVersions } from "./version.js";
...
function coerceConfig(value: unknown): EasyHubConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as EasyHubConfig;
}
...
function stampConfigVersion(cfg: EasyHubConfig): EasyHubConfig {
...
function warnIfConfigFromFuture(cfg: EasyHubConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareEasyHubVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer EasyHub (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: EasyHubConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): EasyHubConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as EasyHubConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as EasyHubConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: EasyHubConfig) {
    clearConfigCache();
...
// module scope. `EASYHUB_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.EASYHUB_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.EASYHUB_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): EasyHubConfig {
...
export async function writeConfigFile(cfg: EasyHubConfig): Promise<void> {
  clearConfigCache();
