...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  bot config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  bot config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function stampConfigVersion(cfg: BotConfig): BotConfig {
...
function warnIfConfigFromFuture(cfg: BotConfig, logger: Pick<typeof console, "warn">): void {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as BotConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): BotConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as BotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: BotConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    try {
      if (deps.fs.existsSync(configPath)) {
        ...
        if (parsedRes.ok) {
          ...
          cfgToWrite = restoreEnvVarRefs(cfgToWrite, parsedRes.parsed, envForRestore) as BotConfig;
        }
      }
    } catch {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as BotConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.BOT_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.BOT_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.BOT_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.BOT_WATCH_SESSION === "string" &&
        deps.env.BOT_WATCH_SESSION.trim().length > 0
          ? deps.env.BOT_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.BOT_WATCH_COMMAND === "string" &&
        deps.env.BOT_WATCH_COMMAND.trim().length > 0
          ? deps.env.BOT_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `BOT_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: BotConfig | null = null;
let runtimeConfigSourceSnapshot: BotConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.BOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.BOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): BotConfig | null {
