...
import {
  appendConfigSecurityAuditEntry,
...
import { verifyConfigHmac, writeConfigHmacSig } from "./io.hmac-integrity.js";
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenClawConfig {
    try {
      ...
      // HMAC integrity check: verify config was not modified outside the gateway.
      ...
      if (typeof effectiveConfigRaw !== "object" || effectiveConfigRaw === null) {
        observeLoadConfigSnapshot({
          ...createConfigFileSnapshot({
            ...
            integrityWarning: configIntegrityWarning,
          }),
        });
        return {};
      }
    ...
    try {
      ...
      try {
        ...
        if (code === "EPERM" || code === "EEXIST") {
          ...
          await writeConfigHmacSig(configPath, json);
          await appendConfigSecurityAuditEntry({
            env: deps.env,
            homedir: deps.homedir,
            actor: "gateway",
            changedPaths: changedPaths ? [...changedPaths] : [],
            sourceHash: previousHash ?? null,
            resultHash: nextHash,
          });
