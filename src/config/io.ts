...
import type { ActiviConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareActiviVersions } from "./version.js";
...
function coerceConfig(value: unknown): ActiviConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ActiviConfig;
}
...
function stampConfigVersion(cfg: ActiviConfig): ActiviConfig {
...
function warnIfConfigFromFuture(cfg: ActiviConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareActiviVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Activi (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function normalizeExecSafeBinProfilesInConfig(cfg: ActiviConfig): void {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ActiviConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ActiviConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ActiviConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ActiviConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ActiviConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ACTIVI_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ACTIVI_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.ACTIVI_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.ACTIVI_WATCH_SESSION === "string" &&
        deps.env.ACTIVI_WATCH_SESSION.trim().length > 0
          ? deps.env.ACTIVI_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.ACTIVI_WATCH_COMMAND === "string" &&
        deps.env.ACTIVI_WATCH_COMMAND.trim().length > 0
          ? deps.env.ACTIVI_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `ACTIVI_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ACTIVI_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ACTIVI_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): ActiviConfig {
