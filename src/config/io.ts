    ...
    try {
      ...
      try {
        ...
        if (code === "EPERM" || code === "EEXIST") {
          ...
          await deps.fs.promises.chmod(configPath, 0o600).catch((err: unknown) => {
            deps.logger.warn(
              `Failed to set config file permissions to 0600: ${err instanceof Error ? err.message : String(err)}`,
            );
          });
