...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "activi.plugin.json"),
...
describe("config plugin validation", () => {
  const fixtureRoot = path.join(os.tmpdir(), "activi-config-plugin-validation");
  let caseIndex = 0;
  ...
  const validateInHome = (home: string, raw: unknown) => {
    process.env.ACTIVI_STATE_DIR = path.join(home, ".activi");
