...
function processFormationPipeline(entityId: string, request: FormationRequest): void {
  ...
  if (request.requestEin) {
    advanceStep(entity, "ein_application", () => {
      ...
      return {
        ...
        actionRequired:
          "EIN application submitted. IRS processes online applications instantly on weekdays 7am-10pm ET.",
      };
    });
  }
  ...
  advanceStep(entity, "compliance_calendar_setup", () => {
    entity.complianceCalendar = generateComplianceCalendar(
      entityId,
      request.state,
      request.entityType,
    );
