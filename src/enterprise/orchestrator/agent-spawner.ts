 ...
 */
function registerCronJob(agentId: string, _spec: AgentSpec): string {
...
export function modifyAgent(agentId: string, changes: Partial<AgentSpec>): DeployedAgent | null {
  ...
  if (changes.systemPrompt !== undefined) {
    agent.spec.systemPrompt = changes.systemPrompt;
  }
  if (changes.tools !== undefined) {
    agent.spec.tools = changes.tools;
  }
  if (changes.channels !== undefined) {
    agent.spec.channels = changes.channels;
  }
  if (changes.name !== undefined) {
    agent.spec.name = changes.name;
  }
  if (changes.purpose !== undefined) {
    agent.spec.purpose = changes.purpose;
  }
 ...
 */
export function executeAgentRun(agentId: string, _input?: string): AgentRunResult {
  const agent = agentRegistry.get(agentId);
  if (!agent) {
    return {
