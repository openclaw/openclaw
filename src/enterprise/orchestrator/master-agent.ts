...
export const MASTER_AGENT_TOOLS: MasterAgentTool[] = [
	...
	{
		name: "create_agent",
		description: "Create and deploy a revenue-generating agent. Every agent should have a clear revenue purpose.",
		parameters: {
			name: { type: "string", description: "Agent name" },
			purpose: { type: "string", description: "Revenue purpose (e.g., 'close inbound leads via WhatsApp')" },
			system_prompt: { type: "string", description: "Detailed behavior instructions" },
			tools: { type: "array", description: "Tools: web_search, send_message, image_generate, http_request, calendar, email, stripe_create_invoice, crm_update", items: { type: "string" } },
			channels: { type: "array", description: "Channels: whatsapp, telegram, slack, discord, email, webchat", items: { type: "string" } },
			...
			revenue_target_cents: { type: "number", description: "Monthly revenue target for this agent (cents)" },
		},
	...
	{
		name: "form_business",
		description: "Start automated business formation (LLC, Corp, DBA). Handles filing, EIN, registered agent, bank account.",
		parameters: {
		...
		required: ["company_name", "entity_type", "state", "business_purpose", "member_name", "member_email"],
	...
	{
		name: "record_revenue",
		description: "Record real revenue earned by an agent (payment received, deal closed, invoice paid).",
		parameters: {
			...
			source: { type: "string", description: "Source: client_payment, subscription, product_sale, lead_conversion, affiliate, consulting_fee, service_delivery, invoice_paid" },
	...
	},
	{
		...
		parameters: {
			target_cents: { type: "number", description: "Target revenue in cents (e.g., 1000000 = $10,000)" },
...
async function handleStopAgent(orgId: OrgId, params: Record<string, unknown>): Promise<string> {
	...
	if (!agent || agent.orgId !== orgId) return `Agent ${agentId} not found.`;
...
async function handleModifyAgent(orgId: OrgId, params: Record<string, unknown>): Promise<string> {
	...
	if (params.cron_schedule) changes.cronSchedule = params.cron_schedule === "none" ? undefined : (params.cron_schedule as string);
...
async function handleFormBusiness(orgId: OrgId, params: Record<string, unknown>): Promise<string> {
	...
	const lines = [
		...
		"",
		"**Steps in progress:**",
		...entity.steps.map((s) => `  ${s.status === "completed" ? "done" : s.status === "requires_action" ? "ACTION NEEDED" : "pending"} — ${s.step.replace(/_/g, " ")}`),
		"",
...
async function handleFormationStatus(orgId: OrgId, params: Record<string, unknown>): Promise<string> {
	const entityId = params.entity_id as string;
	const entity = getFormationStatus(entityId);
	if (!entity || entity.orgId !== orgId) return `Entity ${entityId} not found.`;
	...
	const lines = [
		...
		"**Steps:**",
		...entity.steps.map((s) => {
			const icon = s.status === "completed" ? "[done]" : s.status === "failed" ? "[FAILED]" : s.status === "requires_action" ? "[ACTION]" : "[...]";
			let line = `  ${icon} ${s.step.replace(/_/g, " ")}`;
			if (s.actionRequired) line += ` — ${s.actionRequired}`;
...
async function handleBusinessDashboard(orgId: OrgId): Promise<string> {
	...
	if (health.topAgent) {
		lines.push(`**Top agent:** ${health.topAgent.agentId} — $${(health.topAgent.revenueCents / 100).toFixed(2)} revenue (30d)`);
	}

	if (health.goals.length > 0) {
		lines.push("", "**Revenue Goals:**");
		for (const g of health.goals) {
			const pct = g.targetAmountCents > 0 ? ((g.currentAmountCents / g.targetAmountCents) * 100).toFixed(0) : "0";
			lines.push(`  ${g.period}: $${(g.currentAmountCents / 100).toFixed(2)} / $${(g.targetAmountCents / 100).toFixed(2)} (${pct}%) — ${g.status.replace(/_/g, " ")}`);
		}
	}
...
async function handleCreateDeal(orgId: OrgId, params: Record<string, unknown>): Promise<string> {
	const deal = createDeal({
		...
		expectedCloseDate: params.expected_close_date ? new Date(params.expected_close_date as string) : new Date(Date.now() + 30 * 86_400_000),
	});
...
async function handleSetRevenueGoal(orgId: OrgId, params: Record<string, unknown>): Promise<string> {
...
async function handleCheckBalance(orgId: OrgId): Promise<string> {
	...
	const lines = [
		...
		"",
		"**Recent transactions:**",
		...recentTx.map((tx) => `  ${tx.type === "credit" ? "+" : "-"}$${(tx.amountCents / 100).toFixed(2)} — ${tx.description}`),
	];
