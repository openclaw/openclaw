...
import { resolveSessionConversation, resolveSessionConversationRef, } from "./plugins/session-conversation.js";
function resolveProviderEntry(modelByChannel, channel) {
    ...
    return (modelByChannel?.[normalized] ??
        modelByChannel?.[Object.keys(modelByChannel ?? {}).find((key) => {
            const normalizedKey = normalizeMessageChannel(key) ?? normalizeOptionalLowercaseString(key) ?? "";
...
function buildChannelCandidates(params) {
    ...
    const parentOverrideFallbacks = channelPlugin?.conversationBindings?.buildModelOverrideParentCandidates?.({
    ...
    const groupConversationKind = normalizeChatType(params.groupChatType ?? undefined) === "channel"
        ? "channel"
