...
export function buildChannelSetupWizardAdapterFromSetupWizard(params) {
    ...
    return {
        ...
        configure: async ({ cfg, runtime, prompter, options, accountOverrides, shouldPromptAccountIds, forceAllowFrom, }) => {
            const defaultAccountId = plugin.config.defaultAccountId?.(cfg) ??
            ...
            const shouldShowIntro = !usedEnvShortcut &&
            ...
            const runCredentialSteps = async () => {
                ...
                for (const credential of wizard.credentials) {
                    ...
                    const credentialResult = await runSingleChannelSecretStep({
                        ...
                        onMissingConfigured: credential.helpLines && credential.helpLines.length > 0
                            ? async () => {
                                await prompter.note(credential.helpLines.join("\n"), credential.helpTitle ?? credential.credentialLabel);
                            }
                            : undefined,
                        applyUseEnv: async (currentCfg) => credential.applyUseEnv
            ...
            const runTextInputSteps = async () => {
                for (const textInput of wizard.textInputs ?? []) {
                    let currentValue = normalizeOptionalString(typeof credentialValues[textInput.inputKey] === "string"
                        ? credentialValues[textInput.inputKey]
                        : undefined);
                    if (!currentValue && textInput.currentValue) {
                        currentValue = normalizeOptionalString(await textInput.currentValue({
                            cfg: next,
                            accountId,
                            credentialValues,
                        }));
            ...
            if (wizard.groupAccess) {
                ...
                next = await configureChannelAccessWithAllowlist({
                    ...
                    resolveAllowlist: access.resolveAllowlist
                        ? async ({ cfg: currentCfg, entries }) => await access.resolveAllowlist({
                            cfg: currentCfg,
                            accountId,
                            credentialValues,
                            entries,
                    ...
                    applyAllowlist: access.applyAllowlist
                        ? ({ cfg: currentCfg, resolved }) => access.applyAllowlist({
                            cfg: currentCfg,
                            accountId,
                            resolved,
                        })
            ...
            if (forceAllowFrom && wizard.allowFrom) {
                ...
                if (allowFrom.helpLines && allowFrom.helpLines.length > 0) {
                    await prompter.note(allowFrom.helpLines.join("\n"), allowFrom.helpTitle ?? `${plugin.meta.label} allowlist`);
                }
                const existingAllowFrom = plugin.config.resolveAllowFrom?.({
                    cfg: next,
                    accountId,
                }) ?? [];
            ...
            const shouldShowCompletionNote = wizard.completionNote &&
