...
describe("normalizeTelegramMessagingTarget", () => {
  it("normalizes t.me links to prefixed usernames", () => {
    expect(normalizeTelegramMessagingTarget("https://t.me/MyChannel")).toBe(
      "telegram:@mychannel",
    );
  });
  ...
  it("keeps legacy prefixed topic targets valid", () => {
    ...
    ).toBe("telegram:group:-1001234567890:topic:456");
    expect(
      normalizeTelegramMessagingTarget("tg:group:-1001234567890:topic:456"),
    ).toBe("telegram:group:-1001234567890:topic:456");
  });
});
...
describe("looksLikeTelegramTargetId", () => {
  ...
  it("recognizes legacy prefixed topic targets", () => {
    expect(
      looksLikeTelegramTargetId("telegram:group:-1001234567890:topic:456"),
    ).toBe(true);
    expect(looksLikeTelegramTargetId("tg:group:-1001234567890:topic:456")).toBe(
      true,
    );
  });
