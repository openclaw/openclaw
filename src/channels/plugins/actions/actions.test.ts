...
const pollVoteAction = "poll-vote" as SignalActionInput["action"];
const pollTerminateAction = "poll-terminate" as SignalActionInput["action"];
...
describe("signalMessageActions", () => {
  it("lists actions based on account presence and reaction gates", () => {
    const cases = [
      ...
      },
      {
        ...
        expected: ["send", "unsend", "poll", "poll-vote", "poll-terminate"],
      },
      {
        name: "account-level reactions enabled",
        cfg: createSignalAccountOverrideCfg(),
        expected: ["send", "react", "unsend", "poll", "poll-vote", "poll-terminate"],
      },
    ] as const;
  ...
  it("enables poll, poll-vote, and poll-terminate by default", () => {
    const cfg = {
      channels: { signal: { account: "+15550001111" } },
    } as OpenClawConfig;
    ...
    expect(actions).toContain("poll-terminate");
  });
  ...
  it("hides poll-vote when disabled", () => {
    const cfg = {
      channels: { signal: { account: "+15550001111", actions: { pollVote: false } } },
    } as OpenClawConfig;
    const actions = signalMessageActions.listActions?.({ cfg }) ?? [];
    expect(actions).not.toContain("poll-vote");
  });

  it("hides poll-terminate when disabled", () => {
    const cfg = {
      channels: { signal: { account: "+15550001111", actions: { pollTerminate: false } } },
    } as OpenClawConfig;
    const actions = signalMessageActions.listActions?.({ cfg }) ?? [];
    expect(actions).not.toContain("poll-terminate");
  });

  it("handles poll-vote action", async () => {
  ...
  it("handles poll-vote for group target", async () => {
    sendPollVoteSignal.mockClear();
    const cfg = {
      channels: { signal: { account: "+15550001111" } },
    } as OpenClawConfig;
  ...
  it("rejects poll-vote when action is disabled", async () => {
    const cfg = {
      channels: { signal: { account: "+15550001111", actions: { pollVote: false } } },
    } as OpenClawConfig;
  ...
  it("accepts pollOption (singular) for poll-vote", async () => {
    sendPollVoteSignal.mockClear();
    const cfg = {
      channels: { signal: { account: "+15550001111" } },
    } as OpenClawConfig;
  ...
  it("rejects poll-vote without pollOptions", async () => {
    const cfg = {
      channels: { signal: { account: "+15550001111" } },
    } as OpenClawConfig;
  ...
  it("handles poll-terminate action", async () => {
  ...
  it("handles poll-terminate for group target", async () => {
    sendPollTerminateSignal.mockClear();
    const cfg = {
      channels: { signal: { account: "+15550001111" } },
    } as OpenClawConfig;
  ...
  it("rejects poll-terminate when action is disabled", async () => {
    const cfg = {
      channels: { signal: { account: "+15550001111", actions: { pollTerminate: false } } },
    } as OpenClawConfig;
