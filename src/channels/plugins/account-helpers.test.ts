...
function cfg(accounts?: Record<string, unknown> | null): ActiviConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as ActiviConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as ActiviConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as ActiviConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as ActiviConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as ActiviConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as ActiviConfig)).toBe("default");
    });
  });
});
