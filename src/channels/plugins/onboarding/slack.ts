...
async function noteSlackTokenHelp(prompter: WizardPrompter, botName: string): Promise<void> {
  ...
  // Print the manifest through the prompter transport so gateway-backed
  // onboarding sessions receive it (fixes #32493).
  await prompter.note(manifest, "Slack App Manifest (JSON)");
}
