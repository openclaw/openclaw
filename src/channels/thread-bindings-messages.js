...
export function formatThreadBindingDurationLabel(durationMs) {
    if (durationMs <= 0) {
        return "disabled";
    }
    if (durationMs < 60000) {
...
export function resolveThreadBindingIntroText(params) {
    ...
    const lifecycle = [];
    if (idleTimeoutMs > 0) {
        lifecycle.push(`idle auto-unfocus after ${formatThreadBindingDurationLabel(idleTimeoutMs)} inactivity`);
    }
    if (maxAgeMs > 0) {
        lifecycle.push(`max age ${formatThreadBindingDurationLabel(maxAgeMs)}`);
    }
    const intro = lifecycle.length > 0
...
export function resolveThreadBindingFarewellText(params) {
    ...
    if (params.reason === "idle-expired") {
        ...
        return prefixSystemMessage(`Session ended automatically after ${label} of inactivity. Messages here will no longer be routed.`);
    }
    if (params.reason === "max-age-expired") {
        ...
        return prefixSystemMessage(`Session ended automatically at max age of ${label}. Messages here will no longer be routed.`);
    }
