...
import { formatTrimmedAllowFromEntries, formatWhatsAppConfigAllowFromEntries, resolveIMessageConfigAllowFrom, resolveIMessageConfigDefaultTo, resolveWhatsAppConfigAllowFrom, resolveWhatsAppConfigDefaultTo, } from "../plugin-sdk/channel-config-helpers.js";
...
import { resolveWhatsAppGroupIntroHint, resolveWhatsAppMentionStripPatterns, } from "./plugins/whatsapp-shared.js";
...
function resolveCaseInsensitiveAccount(accounts, accountId) {
    ...
    return (accounts[normalized] ??
        accounts[Object.keys(accounts).find((key) => key.toLowerCase() === normalized.toLowerCase()) ?? ""]);
}
...
const DOCKS = {
    telegram: {
        ...
        config: {
            resolveAllowFrom: ({ cfg, accountId }) => stringifyAllowFrom(resolveTelegramAccount({ cfg, accountId }).config.allowFrom ?? []),
            formatAllowFrom: ({ allowFrom }) => trimAllowFromEntries(allowFrom)
        ...
        threading: {
            ...
            buildToolContext: ({ context, hasRepliedRef }) => {
                ...
                const currentMessageId = typeof rawCurrentMessageId === "number"
                    ? rawCurrentMessageId
    ...
    discord: {
        ...
        config: {
            ...
            resolveDefaultTo: ({ cfg, accountId }) => resolveDiscordAccount({ cfg, accountId }).config.defaultTo?.trim() || undefined,
        },
        groups: {
    ...
    googlechat: {
        ...
        threading: {
            ...
            buildToolContext: ({ context, hasRepliedRef }) => buildThreadToolContextFromMessageThreadOrReply({ context, hasRepliedRef }),
        },
    },
    slack: {
        id: "slack",
        ...
        config: {
            ...
            resolveDefaultTo: ({ cfg, accountId }) => resolveSlackAccount({ cfg, accountId }).config.defaultTo?.trim() || undefined,
        },
        groups: {
    ...
    signal: {
        ...
        streaming: DEFAULT_BLOCK_STREAMING_COALESCE,
        config: {
            resolveAllowFrom: ({ cfg, accountId }) => stringifyAllowFrom(resolveSignalAccount({ cfg, accountId }).config.allowFrom ?? []),
            formatAllowFrom: ({ allowFrom }) => trimAllowFromEntries(allowFrom)
                .map((entry) => (entry === "*" ? "*" : normalizeE164(entry.replace(/^signal:/i, ""))))
                .filter(Boolean),
            resolveDefaultTo: ({ cfg, accountId }) => resolveSignalAccount({ cfg, accountId }).config.defaultTo?.trim() || undefined,
        },
        threading: {
