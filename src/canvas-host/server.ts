...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.dnaCanvasA2UIAction ||
        window.webkit.messageHandlers.dnaCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.dnaCanvasA2UIAction &&
        typeof window.dnaCanvasA2UIAction.postMessage === "function") ||
      (window.dnaCanvasA2UIAction &&
        typeof window.dnaCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.dnaSendUserAction === "function" ? window.dnaSendUserAction : undefined;
  if (!window.dnaSendUserAction && legacySend) {
    window.dnaSendUserAction = legacySend;
  }
  if (!window.dnaSendUserAction && typeof window.dnaSendUserAction === "function") {
    window.dnaSendUserAction = window.dnaSendUserAction;
  }
  const hasHelper = () =>
    typeof window.dnaSendUserAction === "function" ||
    typeof window.dnaSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("dna:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android DNA node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.dnaSendUserAction === "function"
        ? window.dnaSendUserAction
        : window.dnaSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DNA_SKIP_CANVAS_HOST)) return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>DNA Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
