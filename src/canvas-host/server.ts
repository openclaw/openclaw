...
<title>SmartAgentNeo Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.smartAgentNeoCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.smartAgentNeoCanvasA2UIAction &&
        typeof window.smartAgentNeoCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.smartAgentNeoSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("smart-agent-neo:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SmartAgentNeo node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.smartAgentNeoSendUserAction === "function"
        ? window.smartAgentNeoSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SMART_AGENT_NEO_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SMART_AGENT_NEO_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SmartAgentNeo Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
