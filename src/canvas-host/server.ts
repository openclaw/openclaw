...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.EasyHubCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.EasyHubCanvasA2UIAction &&
        typeof window.EasyHubCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.EasyHubSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("EasyHub:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android EasyHub node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.EasyHubSendUserAction === "function"
        ? window.EasyHubSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.EASYHUB_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.EASYHUB_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>EasyHub Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
