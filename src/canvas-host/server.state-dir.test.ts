...
describe("canvas host state dir defaults", () => {
  ...
  it("uses MULLUSI_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("mullusi-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Mullusi Canvas");
      } finally {
        await handler.close();
      }
    });
