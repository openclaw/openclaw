...
describe("canvas host state dir defaults", () => {
  ...
  it("uses EASYHUB_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("EasyHub Canvas");
    } finally {
      await handler.close();
