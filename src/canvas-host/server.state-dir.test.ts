...
describe("canvas host state dir defaults", () => {
  it("uses ACTIVI_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("activi-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Activi Canvas");
      } finally {
        await handler.close();
      }
    });
