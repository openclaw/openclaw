...
describe("ui i18n locale registry", () => {
  it("lists supported locales", () => {
    expect(SUPPORTED_LOCALES).toEqual(["en", "zh-CN", "zh-TW", "pt-BR", "de", "es", "uk"]);
  ...
  it("loads lazy locale translations from the registry", async () => {
    ...
    const uk = await loadLazyLocaleTranslation("uk");
    ...
    expect(getNestedTranslation(uk, "common", "health")).toBe("Стан");
    expect(getNestedTranslation(uk, "languages", "uk")).toBe("Українська (Ukrainian)");
