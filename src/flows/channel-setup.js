...
import { getChannelSetupPlugin, listActiveChannelSetupPlugins, listChannelSetupPlugins, } from "../channels/plugins/setup-registry.js";
import { formatCliCommand } from "../cli/command-format.js";
import { resolveChannelSetupEntries, shouldShowChannelInSetup, } from "../commands/channel-setup/discovery.js";
import { ensureChannelSetupPluginInstalled, loadChannelSetupPluginRegistrySnapshotForChannel, } from "../commands/channel-setup/plugin-install.js";
...
import { maybeConfigureDmPolicies, promptConfiguredAction, promptRemovalAccountId, formatAccountLabel, } from "./channel-setup.prompts.js";
import { collectChannelStatus, noteChannelPrimer, resolveChannelSelectionNoteLines, resolveChannelSetupSelectionContributions, resolveQuickstartDefault, } from "./channel-setup.status.js";
...
export async function setupChannels(cfg, runtime, prompter, options) {
    ...
    const scopedPluginsById = new Map();
    ...
    const activePluginsById = new Map();
    const rememberActivePlugin = (plugin) => {
        activePluginsById.set(plugin.id, plugin);
        return plugin;
    };
    const getVisibleChannelPlugin = (channel) => scopedPluginsById.get(channel) ??
    ...
    const loadScopedChannelPlugin = async (channel, pluginId, setup) => {
        ...
        const plugin = snapshot.channelSetups.find((entry) => entry.plugin.id === channel)?.plugin ??
    ...
    const preloadConfiguredExternalPlugins = async () => {
        ...
        for (const entry of listTrustedChannelPluginCatalogEntries({ cfg: next, workspaceDir })) {
            ...
            const explicitlyEnabled = next.plugins?.entries?.[entry.pluginId ?? channel]?.enabled === true;
    ...
    const statusSummary = deferStatusUntilSelection
        ? { statusByChannel: new Map(), statusLines: [] }
    ...
    const resolveConfigDisabledHint = (channel) => {
        ...
        if (typeof next.channels?.[channel]
            ?.enabled === "boolean") {
            return next.channels[channel]?.enabled === false
                ? "disabled"
                : undefined;
        }
        return undefined;
