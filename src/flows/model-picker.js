...
import { isModelPickerVisibleModelRef, isModelPickerVisibleProvider, } from "../agents/model-picker-visibility.js";
...
function createPreferredProviderMatcher(params) {
    ...
    const preferredOwnerPluginIdSet = preferredOwnerPluginIds
        ...
        : undefined;
    const entryProviderCache = new Map();
    return (entryProvider) => {
        ...
        const value = !!preferredOwnerPluginIdSet &&
...
async function resolveProviderPluginSetupOptions(params) {
    ...
    const providerModelPickerOptions = "resolveProviderModelPickerContributions" in runtime &&
...
async function maybeHandleProviderPluginSelection(params) {
    ...
    if (params.selection.startsWith("provider-plugin:")) {
        ...
        pluginResolution = pluginProviders.some((provider) => normalizeProviderId(provider.id) === normalizeProviderId(params.selection))
            ? params.selection
            : null;
    }
    if (!pluginResolution) {
...
export async function promptDefaultModel(params) {
    ...
    if (includeProviderPluginSetups && params.agentDir) {
        options.push(...(await resolveProviderPluginSetupOptions({
            cfg,
            workspaceDir: params.workspaceDir,
            env: params.env,
        })));
...
export function applyModelAllowlist(cfg, models, opts = {}) {
    ...
    if (normalized.length === 0) {
        ...
        if (scopeKeySet) {
            ...
            return {
                ...
                agents: {
                    ...cfg.agents,
                    defaults: Object.keys(nextModels).length > 0 ? { ...defaults, models: nextModels } : restDefaults,
                },
            };
        }
        const { models: _ignored, ...restDefaults } = defaults;
