...
describe("doctor health contributions", () => {
  ...
  describe("--deep mode filtering", () => {
    it("marks 10 contributions as deepOnly", () => {
      const contributions = resolveDoctorHealthContributions();
      const deepOnlyIds = new Set(
      ...
      expect(deepOnlyIds.has("doctor:shell-completion")).toBe(true);
      expect(deepOnlyIds.has("doctor:browser")).toBe(true);
      expect(deepOnlyIds.has("doctor:hooks-model")).toBe(true);
      expect(deepOnlyIds.has("doctor:systemd-linger")).toBe(true);
      expect(deepOnlyIds.has("doctor:memory-search")).toBe(true);
      expect(deepOnlyIds.has("doctor:device-pairing")).toBe(true);
      expect(deepOnlyIds.has("doctor:workspace-suggestions")).toBe(true);
    });

    it("does not mark critical contributions as deepOnly", () => {
      const contributions = resolveDoctorHealthContributions();
      const criticalIds = [
      ...
      for (const id of criticalIds) {
        ...
        expect(contribution?.mode).toBe("always");
      }
    });

    it("defaults mode to always when not explicitly set", () => {
      const contributions = resolveDoctorHealthContributions();

      for (const contribution of contributions) {
        expect(contribution.mode).toBeDefined();
        expect(["always", "deepOnly"]).toContain(contribution.mode);
      }
    });
  });
});
