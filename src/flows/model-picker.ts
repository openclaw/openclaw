...
export type PromptDefaultModelResult = { model?: string; config?: MullusiConfig };
...
function resolveConfiguredModelRaw(cfg: MullusiConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: MullusiConfig): string[] {
...
export function applyModelAllowlist(cfg: MullusiConfig, models: string[]): MullusiConfig {
