...
describe("node worker supervisor", () => {
      ...
      async () => {
        const expectedWorkerEnv: NodeJS.ProcessEnv = {
          ...
          ...(process.platform === "linux" || process.platform === "darwin"
            ? {
                [INHERITED_PROCESS_LINEAGE_FD_ENV]: expect.stringMatching(/^[0-9]+,[0-9]+$/u),
              }
            : {}),
        };
        ...
        // This legacy fixture records startup metadata before current worker bootstrap consumes it.
        if (process.platform !== "linux" && process.platform !== "darwin") {
          expect(workerEnv).not.toHaveProperty(INHERITED_PROCESS_LINEAGE_FD_ENV);
        } else {
          const lineage = workerEnv[INHERITED_PROCESS_LINEAGE_FD_ENV]!.split(",").map(Number);
          expect(lineage).toHaveLength(2);
          expect(new Set(lineage).size).toBe(2);
          for (const fd of lineage) {
  ...
  it("does not open or signal a child after markRunning observes its terminal receipt", async () => {
    ...
    const captureAdapter = observeWorkerAdapters((child) => {
      adapter = child;
  ...
  it.each([
    ...
    async (operation, state) => {
      ...
      try {
        ...
        throw new Error(`${operation} during startup failed: ${stderr}`, { cause: error });
      } finally {
        captureAdapter.mockRestore();
      }
    },
