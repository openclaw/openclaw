...
const execHostEnforced = process.env.EASYHUB_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.EASYHUB_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureEasyHubCliOnPath({ pathEnv: process.env.PATH ?? "" });
