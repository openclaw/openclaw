...
const execHostEnforced = process.env.DNA_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.DNA_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureDNACliOnPath({ pathEnv: process.env.PATH ?? "" });
