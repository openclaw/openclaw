...
const { detectChangedScope, listChangedPaths } = (await import(
...
describe("detectChangedScope", () => {
	...
	it("keeps node lane off for native-only changes", () => {
		...
		expect(
			detectChangedScope(["apps/shared/OpenClawKit/Sources/Foo.swift"]),
		).toEqual({
			runNode: false,
			runMacos: true,
			runAndroid: true,
	...
	it("runs changed-smoke for install and packaging surfaces", () => {
		...
		expect(
			detectChangedScope([bundledPluginFile("matrix", "package.json")]),
		).toEqual({
			runNode: true,
			runMacos: false,
			runAndroid: false,
