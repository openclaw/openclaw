...
describe("docs-link-audit", () => {
	it("normalizes route fragments away", () => {
		expect(
			normalizeRoute("/plugins/building-plugins#registering-agent-tools"),
		).toBe("/plugins/building-plugins");
		expect(normalizeRoute("/plugins/building-plugins?tab=all")).toBe(
	...
	it("resolves redirects that land on anchored sections", () => {
		const redirects = new Map([
			[
				"/plugins/agent-tools",
				"/plugins/building-plugins#registering-agent-tools",
			],
		]);
				...
				navigation: [
					...
					},
					{
						...
						tabs: [
							{
								tab: "中文",
								groups: [{ group: "帮助", pages: ["zh-CN/help/testing"] }],
							},
						],
					},
				],
	...
	it("builds an English-only docs tree for anchor audits", () => {
		const fixtureRoot = fs.mkdtempSync(
			path.join(os.tmpdir(), "docs-link-audit-fixture-"),
		);
					...
					navigation: [
						{
							...
							tabs: [
								{
									tab: "Docs",
									groups: [{ group: "Help", pages: ["help/testing"] }],
								},
							],
						},
						{
		...
		fs.writeFileSync(
			path.join(docsRoot, "zh-CN", "help", "testing.md"),
			"# 测试\n",
			"utf8",
		);
		...
		try {
			expect(
				fs.existsSync(path.join(anchorDocsDir, "help", "testing.md")),
			).toBe(true);
