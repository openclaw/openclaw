...
describe("Claw agent-origin provenance", () => {
  ...
  it("claims a pending configured-agent adoption with its config-committed phase", async () => {
    const root = tempDirs.make("openclaw-claw-origin-claim-");
    const { plan } = await makeProvenancePlan(root, {
      schemaVersion: 1,
      agent: { id: "worker" },
    });
    ...
    const pending = persistClawInstallRecord(adoptedPlan(plan), {
      ...options,
      status: "pending",
      nowMs: 1,
    });
    expect(pending.agentClaimed).toBe(false);
    expect(JSON.parse(rawOriginPayload(root).agent_owned_paths_json)).toMatchObject({
      claimed: false,
    });
    ...
    expect(readClawInstallRecord("worker", options)).toMatchObject({
  ...
  it("treats historical v3 envelopes without a claim flag as claimed and updates them", async () => {
    const root = tempDirs.make("openclaw-claw-origin-legacy-claim-");
    const { plan } = await makeProvenancePlan(root, {
      schemaVersion: 1,
      agent: { id: "worker" },
    });
    ...
    openOpenClawStateDatabase({ env: stateEnv(root) })
      .db.prepare("UPDATE claw_installs SET agent_owned_paths_json = ? WHERE agent_id = ?")
      .run(JSON.stringify({ origin: "adopted", paths: ['agents.entries["worker"]'] }), "worker");

    expect(readClawInstallRecord("worker", { env: stateEnv(root) })?.agentClaimed).toBe(true);

    const updated = updateClawInstallRecord(adoptedPlan(plan), {
      env: stateEnv(root),
      nowMs: 2,
    });

    expect(updated).toMatchObject({ agentOrigin: "adopted", agentClaimed: true, updatedAtMs: 2 });
    expect(JSON.parse(rawOriginPayload(root).agent_owned_paths_json)).toMatchObject({
      origin: "adopted",
      claimed: true,
    });
  ...
  it("binds adopted resume to origin, agent digest, workspace, package identity, and plan", async () => {
    ...
    expect(record.agentClaimed).toBe(false);
