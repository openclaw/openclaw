...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `smart-agent-neo-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.SMART_AGENT_NEO_CONFIG_PATH = path.join(root, "smart-agent-neo.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "smart-agent-neo.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "smart-agent-neo-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadSmartAgentNeoPlugins: () => pluginRegistryState.registry,
  };
});
