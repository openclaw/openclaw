...
export function resolveGatewayReloadSettings(cfg: MullusiConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: MullusiConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: MullusiConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: MullusiConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: MullusiConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: MullusiConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: MullusiConfig) => {
