...
import type { ActiviConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: ActiviConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: ActiviConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: ActiviConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: ActiviConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: ActiviConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: ActiviConfig) => {
