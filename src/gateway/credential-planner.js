 ...
 */
export function trimCredentialToUndefined(value) {
...
export function hasGatewayPasswordEnvCandidate(env = process.env) {
...
export function createGatewayCredentialPlan(params) {
    ...
    const passwordCanWin = authMode === "password" ||
        (authMode !== "token" && authMode !== "none" && authMode !== "trusted-proxy" && !tokenCanWin);
    const localTokenSurfaceActive = localTokenCanWin &&
        !envToken &&
        (authMode === "token" ||
