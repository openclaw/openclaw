...
function verifyDecodedSize(buffer, estimatedBytes, label) {
    if (Math.abs(buffer.byteLength - estimatedBytes) > 3) {
        throw new Error(`attachment ${label}: base64 contains invalid characters ` +
...
function assertSavedMedia(value, label) {
    if (value !== null &&
        ...
        if (id.includes("/") || id.includes("\\") || id.includes("\0")) {
            throw new Error(`attachment ${label}: saveMediaBuffer returned an unsafe media ID ` +
    ...
    try {
        for (const [idx, att] of attachments.entries()) {
            ...
            if (shouldForceOffload && offloadedRefs.length >= TEXT_ONLY_OFFLOAD_LIMIT) {
                log?.warn(`attachment ${label}: dropping image because text-only offload limit ` +
            ...
            if (shouldForceOffload || sizeBytes > OFFLOAD_THRESHOLD_BYTES) {
                ...
                if (!isSupportedForOffload) {
                    if (shouldForceOffload) {
                        log?.warn(`attachment ${label}: format ${finalMime} cannot be offloaded for ` +
                            "text-only model, dropping");
                        continue;
                    }
                    // Passing this inline would reintroduce the OOM risk this PR prevents.
                    throw new Error(`attachment ${label}: format ${finalMime} is too large to pass inline ` +
                ...
                try {
                    ...
                    throw new MediaOffloadError(`[Gateway Error] Failed to save intercepted media to disk: ${errorMessage}`, { cause: err });
                }
            }
            if (isOffloaded) {
                continue;
