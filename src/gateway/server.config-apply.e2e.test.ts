...
beforeAll(async () => {
  previousToken = process.env.DNA_GATEWAY_TOKEN;
...
afterAll(async () => {
  await server.close();
  if (previousToken === undefined) delete process.env.DNA_GATEWAY_TOKEN;
  else process.env.DNA_GATEWAY_TOKEN = previousToken;
});
...
describe("gateway config.apply", () => {
  it("writes config, stores sentinel, and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".dna", "restart-sentinel.json");
