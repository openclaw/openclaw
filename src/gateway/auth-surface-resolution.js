...
export async function resolveGatewayInteractiveSurfaceAuth(params) {
    ...
    if (params.surface === "remote") {
        ...
        const remotePassword = explicitPassword || envPassword
        ...
        return token || password
            ? { token, password }
            : {
                failureReason: remoteToken.unresolvedRefReason ??
    ...
    const resolvePassword = async () => {
        const localPassword = explicitPassword || envPassword
            ? { value: explicitPassword ?? envPassword }
            : await resolveGatewayCredential({
                config: params.config,
                env,
    ...
    const shouldUsePassword = Boolean(explicitPassword ?? envPassword) || (hasConfiguredPassword && !hasConfiguredToken);
