    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "smart-agent-neo", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-smart-agent-neo-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-smart-agent-neo-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "smart-agent-neo", messages: [{ role: "user", content: "hi" }] },
          {
            "x-smart-agent-neo-agent-id": "beta",
            "x-smart-agent-neo-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
