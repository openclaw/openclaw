...
export function resolveGatewayScopedTools(params: {
  ...
  const runtimePolicySessionKey =
    params.runtimePolicySessionKey?.trim() || params.sessionKey;
  const {
    agentId,
    globalPolicy,
  ...
  const requesterPolicies = resolveRequesterToolPolicies({
    ...
    accountId:
      params.scheduledToolPolicy?.ownerAccountId ?? params.accountId ?? null,
    senderId,
  ...
  const mediatedToolNames = new Set(
    Array.from(params.mediatedToolNames ?? [], (name) =>
      ...
      ? DEFAULT_GATEWAY_HTTP_TOOL_DENY.filter(
          ...
          (name) =>
            !gatewayToolsCfg?.allow?.some(
              (allowed) =>
                normalizeToolName(allowed) === normalizeToolName(name),
            ),
        )
      : [];
  ...
  const toolsWithMediatedCoding = [
    ...
    ...baseTools.filter(
      (tool) => !mediatedToolNames.has(normalizeToolName(tool.name)),
    ),
    ...mediatedCodingTools,
  ];
