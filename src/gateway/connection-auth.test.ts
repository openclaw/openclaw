...
function cfg(input: Partial<MullusiConfig>): MullusiConfig {
  return input as MullusiConfig;
}
...
const DEFAULT_ENV = {
  MULLUSI_GATEWAY_TOKEN: "env-token",
  MULLUSI_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when MULLUSI env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when MULLUSI env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when MULLUSI env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
