...
import { clearActiveMcpLoopbackRuntimeByOwnerToken, setActiveMcpLoopbackRuntime, } from "./mcp-http.loopback-runtime.js";
import { jsonRpcError } from "./mcp-http.protocol.js";
import { readMcpHttpBody, resolveMcpRequestContext, validateMcpLoopbackRequest, } from "./mcp-http.request.js";
...
function shouldLogMcpLoopbackTraffic() {
    return (isTruthyEnvValue(process.env.OPENCLAW_CLI_BACKEND_LOG_OUTPUT) ||
...
export async function startMcpLoopbackServer(port = 0) {
        ...
        void (async () => {
            try {
                ...
                for (const message of messages) {
                    ...
                    if (response !== null) {
                        const toolName = message.method === "tools/call" && isRecord(message.params)
                            ? message.params.name
                            : undefined;
...
export async function ensureMcpLoopbackServer(port = 0) {
...
export async function closeMcpLoopbackServer() {
    const server = activeMcpLoopbackServer ??
