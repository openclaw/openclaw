...
export class GatewayClient {
    ...
    sendConnect() {
        ...
        const resolvedDeviceToken = explicitDeviceToken ?? (!explicitGatewayToken ? (storedToken ?? undefined) : undefined);
    ...
    queueConnect() {
        ...
        const connectChallengeTimeoutMs = typeof rawConnectDelayMs === "number" && Number.isFinite(rawConnectDelayMs)
            ? Math.max(250, Math.min(10000, rawConnectDelayMs))
            : 2000;
        if (this.connectTimer) {
            clearTimeout(this.connectTimer);
        }
    ...
    startTickWatch() {
        ...
        const minInterval = typeof rawMinInterval === "number" && Number.isFinite(rawMinInterval)
            ? Math.max(1, Math.min(30000, rawMinInterval))
            : 1000;
