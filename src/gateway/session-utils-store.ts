...
export function resolveCanonicalSessionStoreMatchFromStoreKeys(
  ...
  for (const key of storeKeys) {
    ...
    if (selected) {
      throw duplicateCanonicalSessionKeyError(storeKeys[0] ?? key);
    }
    selected = match;
  }
  if (selected && selected.key !== storeKeys[0]) {
    throw nonCanonicalSessionKeyRowError(storeKeys[0] ?? selected.key);
  }
  return selected;
}
...
export function resolveCanonicalGatewaySessionStoreKey(params: {
  ...
  if (canonicalMatch && canonicalMatch.key !== primaryKey) {
    throw nonCanonicalSessionKeyRowError(primaryKey);
  }
