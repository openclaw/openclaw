...
const { createMullusiTools } = await import("../agents/mullusi-tools.js");
...
type SessionSendTool = ReturnType<typeof createMullusiTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createMullusiTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["MULLUSI_GATEWAY_PORT", "MULLUSI_GATEWAY_TOKEN"]);
  ...
  process.env.MULLUSI_GATEWAY_PORT = String(gatewayPort);
  process.env.MULLUSI_GATEWAY_TOKEN = gatewayToken;
