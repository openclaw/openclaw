...
describe("gateway server sessions", () => {
  ...
  test("sessions.reset preserves spawned session ownership metadata", async () => {
    ...
    expect(store["agent:main:subagent:child"]?.providerOverride).toBeUndefined();
    expect(store["agent:main:subagent:child"]?.modelOverride).toBeUndefined();
    expect(store["agent:main:subagent:child"]?.authProfileOverride).toBeUndefined();
    expect(store["agent:main:subagent:child"]?.authProfileOverrideSource).toBeUndefined();
    expect(store["agent:main:subagent:child"]?.authProfileOverrideCompactionCount).toBeUndefined();
