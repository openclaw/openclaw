...
export function scheduleRestoredFollowupQueueRecovery(
  ...
  const wakeOrLog = () => {
    ...
    // Restore dispatches to the shared-state worker, so this startup hook has no
    // caller to await it. Keep the failure visible in the log instead.
    void (async () => {
      try {
        // Restore is deferred to this timer rather than run as an import side
        // effect: opening shared state SQLite during module evaluation
        // invalidated read admissions already held by other startup owners.
        // Enqueues that land first stay correct through per-key reconciliation.
