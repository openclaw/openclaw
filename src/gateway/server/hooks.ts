...
export function createGatewayHooksRequestHandler(params: {
  ...
  const dispatchWakeHook = (value: {
    ...
    if (value.sessionKey) {
      ...
      // No sessionKey provided — enqueue to main session but fan-out the heartbeat
      // to ALL agents (don't scope requestHeartbeatNow to a single session).
      const mainKey = resolveMainSessionKeyFromConfig();
      enqueueSystemEvent(value.text, { sessionKey: mainKey });
      if (value.mode === "now") {
