...
import {
  ...
  DEFAULT_COMPACTION_CHECKPOINT_MAX_RETAINED_BYTES_PER_SESSION,
...
describe("session-compaction-checkpoints", () => {
  ...
  test("persist trims retained checkpoint snapshots by total byte budget", async () => {
    ...
    const sessionKey = "agent:main:main";
    const now = Date.now();
    const checkpointSize = Math.floor(
      DEFAULT_COMPACTION_CHECKPOINT_MAX_RETAINED_BYTES_PER_SESSION / 6,
    );
  ...
  test("persist honors session.maintenance.compactionCheckpointMaxBytes override", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-checkpoint-byte-cfg-"));
    tempDirs.push(dir);
    ...
    const sessionKey = "agent:main:main";
    const now = Date.now();
    const tightBudgetBytes = 32 * 1024; // 32kb cap to force eviction of older snapshots
    const checkpointSize = Math.floor(tightBudgetBytes / 2);
    const existingCheckpoints = await Promise.all(
    ...
    const currentSnapshotFile = path.join(
      dir,
      "sess.checkpoint.99999999-aaaa-4aaa-8aaa-aaaaaaaaaaaa.jsonl",
    );
    await fs.writeFile(currentSnapshotFile, "", "utf-8");
    await fs.truncate(currentSnapshotFile, checkpointSize);
    ...
    await persistSessionCompactionCheckpoint({
      cfg: {
        session: {
          store: storePath,
          maintenance: { compactionCheckpointMaxBytes: `${tightBudgetBytes}b` },
        },
        agents: { list: [{ id: "main", default: true }] },
      } as OpenClawConfig,
      sessionKey: "main",
