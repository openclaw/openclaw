...
const loadMullusiPlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadMullusiPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadMullusiPlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadMullusiPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", async () => {
    ...
    expect(loadMullusiPlugins).not.toHaveBeenCalled();
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.mullusi.com/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", async () => {
    ...
    loadMullusiPlugins.mockReturnValue(pluginRegistry);
