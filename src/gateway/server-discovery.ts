...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) {
    return "SmartAgentNeo";
  }
  if (/smart-agent-neo/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (SmartAgentNeo)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SMART_AGENT_NEO_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const siblingCli = path.join(execDir, "smart-agent-neo");
  ...
  const binCli = path.join(cwd, "bin", "smart-agent-neo");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SMART_AGENT_NEO_TAILNET_DNS?.trim();
