...
import { listAgentIds, resolveAgentEffectiveModelPrimary, resolveAgentModelFallbacksOverride, resolveAgentWorkspaceDir, resolveDefaultAgentId, } from "../agents/agent-scope.js";
import { lookupContextTokens, resolveContextTokensForModel } from "../agents/context.js";
import { DEFAULT_CONTEXT_TOKENS, DEFAULT_MODEL, DEFAULT_PROVIDER } from "../agents/defaults.js";
import { inferUniqueProviderFromConfiguredModels, normalizeStoredOverrideModel, parseModelRef, resolveConfiguredModelRef, resolveDefaultModelForAgent, resolvePersistedSelectedModelRef, } from "../agents/model-selection.js";
import { countActiveDescendantRuns, getSessionDisplaySubagentRunByChildSessionKey, getSubagentSessionRuntimeMs, getSubagentSessionStartedAt, isSubagentRunLive, listSubagentRunsForController, resolveSubagentSessionStatus, } from "../agents/subagent-registry-read.js";
import { RECENT_ENDED_SUBAGENT_CHILD_SESSION_MS, shouldKeepSubagentRunChildLink, } from "../agents/subagent-run-liveness.js";
import { listThinkingLevelOptions, resolveThinkingDefaultForModel, } from "../auto-reply/thinking.js";
import { loadConfig } from "../config/config.js";
import { resolveAgentModelFallbackValues } from "../config/model-input.js";
import { resolveStateDir } from "../config/paths.js";
import { buildGroupDisplayName, loadSessionStore, resolveAllAgentSessionStoreTargetsSync, resolveAgentMainSessionKey, resolveFreshSessionTotalTokens, resolveStorePath, } from "../config/sessions.js";
...
import { AVATAR_MAX_BYTES, isAvatarDataUrl, isAvatarHttpUrl, isPathWithinRoot, isWorkspaceRelativeAvatarPath, resolveAvatarMime, } from "../shared/avatar-policy.js";
...
import { resolveSessionStoreAgentId, resolveSessionStoreKey, resolveStoredSessionKeyForAgentStore, } from "./session-store-key.js";
import { readLatestSessionUsageFromTranscript, readSessionTitleFieldsFromTranscript, } from "./session-utils.fs.js";
...
function shouldKeepStoreOnlyChildLink(entry, now) {
    if (isTerminalSessionStatus(entry.status) || isFinitePositiveTimestamp(entry.endedAt)) {
        ...
        return (isFinitePositiveTimestamp(endedAt) && now - endedAt <= RECENT_ENDED_SUBAGENT_CHILD_SESSION_MS);
    }
    if (entry.status === "running" || isFinitePositiveTimestamp(entry.startedAt)) {
        return true;
    }
    return (isFinitePositiveTimestamp(entry.updatedAt) &&
 ...
 */
export function resolveDeletedAgentIdFromSessionKey(cfg, sessionKey) {
...
export function resolveFreshestSessionStoreMatchFromStoreKeys(store, storeKeys) {
    let freshest;
...
export function resolveFreshestSessionEntryFromStoreKeys(store, storeKeys) {
...
function resolveExplicitDeletedLegacyMainStoreTarget(params) {
    ...
    const lookupSeeds = Array.from(new Set([params.key, canonicalKey, agentMainKey, legacyAgentMainKey]));
    let best;
...
export async function resolveGatewayModelSupportsImages(params) {
    ...
    try {
        ...
        const modelEntry = catalog.find((entry) => entry.id === params.model && (!params.provider || entry.provider === params.provider));
        ...
        if (modelEntry) {
            ...
            if (normalizedProvider === "microsoft-foundry" &&
                normalizedCandidates.some((candidate) => candidate.startsWith("gpt-") ||
            ...
            if (normalizedProvider === "claude-cli" &&
                normalizedCandidates.some((candidate) => candidate === "opus" ||
                    candidate === "sonnet" ||
                    candidate === "haiku" ||
...
export function resolveSessionModelIdentityRef(cfg, entry, agentId, fallbackModelRef) {
...
export function buildGatewaySessionRow(params) {
    ...
    const subagentOwner = normalizeOptionalString(subagentRun?.controllerSessionKey) ||
    ...
    const needsTranscriptTotalTokens = resolvePositiveNumber(resolveFreshSessionTotalTokens(entry)) === undefined;
    ...
    const transcriptUsage = needsTranscriptTotalTokens || needsTranscriptContextTokens || needsTranscriptEstimatedCostUsd
    ...
    const shouldUseTranscriptModelIdentity = runtimeModelPresent &&
    ...
    const totalTokensFresh = typeof totalTokens === "number" && Number.isFinite(totalTokens) && totalTokens > 0
        ? true
    ...
    const contextTokens = resolvePositiveNumber(entry?.contextTokens) ??
        resolvePositiveNumber(transcriptUsage?.contextTokens) ??
        resolvePositiveNumber(resolveContextTokensForModel({
            cfg,
            provider: modelProvider,
            model,
            // Gateway/session listing is read-only; don't start async model discovery.
...
export function loadGatewaySessionRow(sessionKey, options) {
...
export function listSessionsFromStore(params) {
    ...
    let sessions = Object.entries(store)
        ...
        return (shouldKeepStoreOnlyChildLink(entry, now) &&
    ...
    if (search) {
        sessions = sessions.filter((s) => {
            ...
            return fields.some((f) => typeof f === "string" && normalizeLowercaseStringOrEmpty(f).includes(search));
        });
    }
