...
export { resolveEffectiveSharedGatewayAuth, resolveGatewayAuth, } from "./auth-resolve.js";
...
export function hasForwardedRequestHeaders(req) {
    if (!req) {
        return false;
    }
    return Boolean(req.headers?.forwarded ||
        req.headers?.["x-forwarded-for"] ||
        req.headers?.["x-forwarded-proto"] ||
        req.headers?.["x-real-ip"] ||
        req.headers?.["x-forwarded-host"]);
}
export function isLocalDirectRequest(req, _trustedProxies, _allowRealIpFallback = false) {
    if (!req) {
        return false;
    }
...
export function assertGatewayAuthConfigured(auth, rawAuthConfig) {
    ...
    if (auth.mode === "password" && !auth.password) {
        if (rawAuthConfig?.password != null && // pragma: allowlist secret
    ...
    if (auth.mode === "trusted-proxy") {
        ...
        if (auth.token) {
            throw new Error("gateway auth mode is trusted-proxy, but a shared token is also configured; remove gateway.auth.token / OPENCLAW_GATEWAY_TOKEN because trusted-proxy and token auth are mutually exclusive");
        }
    }
}
/**
