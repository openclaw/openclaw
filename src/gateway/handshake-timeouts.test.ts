...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers MULLUSI_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        MULLUSI_HANDSHAKE_TIMEOUT_MS: "75",
        MULLUSI_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        MULLUSI_HANDSHAKE_TIMEOUT_MS: "",
        MULLUSI_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
  ...
  test("getConnectChallengeTimeoutMsFromEnv reads MULLUSI_CONNECT_CHALLENGE_TIMEOUT_MS", () => {
    expect(getConnectChallengeTimeoutMsFromEnv({})).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ MULLUSI_CONNECT_CHALLENGE_TIMEOUT_MS: "15000" }),
    ).toBe(15_000);
    expect(
      getConnectChallengeTimeoutMsFromEnv({ MULLUSI_CONNECT_CHALLENGE_TIMEOUT_MS: "garbage" }),
    ).toBeUndefined();
  });

  test("resolveConnectChallengeTimeoutMs falls back to env override", () => {
    const original = process.env.MULLUSI_CONNECT_CHALLENGE_TIMEOUT_MS;
    try {
      process.env.MULLUSI_CONNECT_CHALLENGE_TIMEOUT_MS = "5000";
      ...
      if (original === undefined) {
        delete process.env.MULLUSI_CONNECT_CHALLENGE_TIMEOUT_MS;
      } else {
        process.env.MULLUSI_CONNECT_CHALLENGE_TIMEOUT_MS = original;
      }
    }
  });
});
