...
export async function startGatewayServer(
  ...
  const hasControlUiOrigins = (cfgAtStart.gateway?.controlUi?.allowedOrigins ?? []).length > 0;
  ...
  if (isNonLoopbackBind && !hasControlUiOrigins && !hasControlUiFallback) {
    const bindPort = cfgAtStart.gateway?.port ?? DEFAULT_GATEWAY_PORT;
    const seededOrigins = new Set<string>([
      `http://localhost:${bindPort}`,
      `http://127.0.0.1:${bindPort}`,
    ]);
    if (controlUiBind === "custom" && cfgAtStart.gateway?.customBindHost) {
      seededOrigins.add(`http://${cfgAtStart.gateway.customBindHost}:${bindPort}`);
    }
    cfgAtStart = {
      ...cfgAtStart,
      gateway: {
