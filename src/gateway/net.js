...
export function resolveRequestClientIp(req, trustedProxies, allowRealIpFallback = false) {
    if (!req) {
        return undefined;
    }
 ...
 */
export function isLocalishHost(hostHeader) {
...
function parseHostForAddressChecks(host) {
    ...
    return {
        ...
        unbracketedHost: normalizedHost.startsWith("[") && normalizedHost.endsWith("]")
 ...
 */
export function isSecureWebSocketUrl(url, opts) {
    let parsed;
    try {
        parsed = new URL(url);
    }
    ...
    if (opts?.allowPrivateWs) {
        ...
        const hostForIpCheck = parsed.hostname.startsWith("[") && parsed.hostname.endsWith("]")
