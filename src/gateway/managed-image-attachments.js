...
import { getImageMetadata, hasAlphaChannel, resizeToJpeg, resizeToPng, } from "../media/image-ops.js";
...
import { authorizeGatewayHttpRequestOrReply, resolveOpenAiCompatibleHttpOperatorScopes, } from "./http-utils.js";
...
export function resolveManagedImageAttachmentLimits(config) {
    return {
...
function isManagedImageAttachmentSafeError(error) {
    ...
    return (error.message.startsWith("Managed image attachment ") ||
...
function validateManagedImageBuffer(buffer, alt, limits) {
    if (buffer.byteLength > limits.maxBytes) {
        throw createManagedImageAttachmentError(`Managed image attachment ${JSON.stringify(alt)} exceeds the ${formatLimitMiB(limits.maxBytes)} byte limit`);
    }
}
...
async function requesterOwnsManagedImageSession(params) {
    ...
    return (subagentRun.requesterSessionKey === params.requesterSessionKey ||
...
export async function cleanupManagedOutgoingImageRecords(params) {
    ...
    let deletedFileCount = 0;
    let retainedCount = 0;
    const retainedReferencedPaths = new Set();
...
function buildManagedImageResizeWarningBlock(params) {
    return {
        type: "text",
        text: `[Image warning] ${params.alt} exceeded gateway dimension/pixel limits and was resized from ` +
...
function getCachedSessionManagedOutgoingAttachmentIndex(sessionKey, stat) {
    ...
    if (cached.transcriptPath !== stat.transcriptPath ||
...
function setCachedSessionManagedOutgoingAttachmentIndex(sessionKey, stat, index) {
    ...
    while (sessionManagedOutgoingAttachmentIndexCache.size >
...
async function getSessionManagedOutgoingAttachmentIndex(sessionKey, cache) {
    ...
    for (const message of messages) {
        ...
        for (const ref of collectManagedOutgoingAttachmentRefs(Array.isArray(message?.content)
            ? message.content
            : [], sessionKey)) {
...
async function recordMatchesTranscriptMessage(record, cache) {
    ...
    return (index?.has(buildManagedOutgoingAttachmentRefKey(record.messageId, record.attachmentId)) ?? false);
}
...
export async function createManagedOutgoingImageBlocks(params) {
    ...
    for (const [index, mediaUrl] of mediaUrls.entries()) {
        ...
        try {
            ...
            let savedOriginal = parsedDataUrl.kind === "image-data-url"
                ? await saveMediaBuffer(parsedDataUrl.buffer, parsedDataUrl.contentType, "outgoing/originals", limits.maxBytes, `generated-image-${index + 1}`)
                : await (async () => {
                    const localMediaPath = resolveLocalMediaPath(mediaUrl);
                    if (localMediaPath) {
                        await assertLocalMediaAllowed(localMediaPath, params.localRoots);
                    }
                    return await saveMediaSource(mediaUrl, undefined, "outgoing/originals", Math.max(limits.maxBytes, MEDIA_MAX_BYTES));
                })();
            ...
            let originalBuffer = parsedDataUrl.kind === "image-data-url"
...
export async function handleManagedOutgoingImageHttpRequest(req, res, opts) {
