...
import { hasHookTemplateExpressions, resolveHookMappings, } from "./hooks-mapping.js";
...
export function resolveHooksConfig(cfg) {
    ...
    if (hasEffectiveTemplatedHookSessionKeyMapping(mappings) && !allowedSessionKeyPrefixes) {
        throw new Error("hooks.allowedSessionKeyPrefixes is required when a hook mapping sessionKey uses templates, even if hooks.allowRequestSessionKey=true");
    }
    return {
        basePath: trimmed,
        token,
...
export function isSessionKeyAllowedByPrefix(sessionKey, prefixes) {
...
export function resolveHookIdempotencyKey(params) {
    return (resolveOptionalHookIdempotencyKey(params.headers?.["idempotency-key"]) ||
...
export const getHookSessionKeyRequestPolicyError = () => "sessionKey is disabled for externally supplied hook payload values; set hooks.allowRequestSessionKey=true to enable";
...
export function resolveHookSessionKey(params) {
    ...
    if (requested) {
        if ((params.source === "request" || params.source === "mapping-templated") &&
