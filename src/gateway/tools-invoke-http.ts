...
import { createSmartAgentNeoTools } from "../agents/smart-agent-neo-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-smart-agent-neo-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-smart-agent-neo-account-id")?.trim() || undefined;
  ...
  const allTools = createSmartAgentNeoTools({
