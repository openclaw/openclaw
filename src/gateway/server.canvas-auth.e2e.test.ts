...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.EASYHUB_CONFIG_PATH;
  const prevDisableCache = process.env.EASYHUB_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "EasyHub-canvas-auth-test-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.EASYHUB_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.EASYHUB_DISABLE_CONFIG_CACHE;
    } else {
      process.env.EASYHUB_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
