...
import { CursorError, decodeCursor, encodeCursor } from "../util/cursor.js";
...
export function collectArtifactsFromMessages(params: {
  ...
  for (const message of params.messages) {
    ...
    // P2 provenance fix: only filter by per-message runId/taskId when explicitly present.
    // Transcript writers attach only `id` and `seq` to __openclaw metadata — runId/taskId
    // are never persisted to transcript entries (#74926 review). The session-level routing
    // (resolveQuerySessionKey) already scoped the session correctly, so silently dropping
    // all artifacts when the field is absent was wrong. Applying the filter only when the
    // field is present allows future writers to persist provenance without breaking anything.
    if (params.runId && messageRunId !== undefined && messageRunId !== params.runId) {
      continue;
    }
    if (params.taskId && messageTaskId !== undefined && messageTaskId !== params.taskId) {
      continue;
    }
...
export const artifactsHandlers: GatewayRequestHandlers = {
  "artifacts.list": ({ params, respond }) => {
    ...
    // Apply type filter (empty array means nothing matches)
    const hasTypesFilter = Array.isArray(params.types);
    const typesSet = hasTypesFilter ? new Set(params.types) : null;
    const filtered = typesSet !== null ? artifacts.filter((a) => typesSet.has(a.type)) : artifacts;
    const total = filtered.length;

    // Apply cursor: drop artifacts whose messageSeq is at or before the cursor value
    let afterCursor = filtered;
    if (params.cursor) {
      let cursorSeq: number;
      try {
        cursorSeq = decodeCursor(params.cursor);
      } catch (e) {
        if (e instanceof CursorError) {
          respond(false, undefined, artifactError("invalid_cursor", e.message));
          return;
        }
        respond(false, undefined, artifactError("invalid_cursor", "malformed cursor value"));
        return;
      }
      afterCursor = filtered.filter((a) => (a.messageSeq ?? 0) > cursorSeq);
    }

    // Apply limit (default 50, hard cap 200)
    const limit = Math.min(params.limit ?? 50, 200);
    const page = afterCursor.slice(0, limit);
    const hasMore = afterCursor.length > limit;

    const nextCursor =
      hasMore && page.length > 0 ? encodeCursor(page[page.length - 1].messageSeq ?? 0) : undefined;
