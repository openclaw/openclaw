  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    try {
      ...
      let pendingDispatchLifecycleError:
        ...
        | undefined;
      let releaseDispatchErrorRootContinuation: (() => void) | null = null;
      ...
      void gatewayWorkAdmission
        ...
        .catch(async (err: unknown) => {
          ...
          // Force abortability closed before awaited transcript fallback so a
          // late chat.abort cannot resurrect this run as aborted.
          context.chatAbortedRuns.delete(clientRunId);
          if (pendingDispatchLifecycleError && !releaseDispatchErrorRootContinuation) {
            // Error ownership must close abortability before transcript fallback, but
            // restart-drain still needs a retained root until lifecycle error projection lands.
            releaseDispatchErrorRootContinuation = retainGatewayRootWorkAdmissionContinuation();
          }
          cleanupAdmittedRun({ force: true });
          removeActiveChatRun();
        ...
        .finally(() => {
          ...
          void persistDispatchLifecycleError()
            ...
            .finally(() => {
              releaseDispatchErrorRootContinuation?.();
              releaseDispatchErrorRootContinuation = null;
            });
        });
    } catch (err) {
      cleanupAdmittedRun({ force: true });
