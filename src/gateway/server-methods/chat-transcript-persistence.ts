...
function isCurrentAbortedPartialSession(failure: AbortedPartialPersistenceFailure): boolean {
  try {
    const current = loadSessionEntry(
      failure.sessionKey,
      failure.agentId ? { agentId: failure.agentId } : undefined,
    ).entry;
    return (
      current?.sessionId === failure.sessionId &&
      (current.lifecycleRevision ?? null) === failure.lifecycleRevision
    );
  } catch {
    // Abort already completed. If the incarnation cannot be revalidated, do
    // not turn its reporting path into a failed abort or leak into a new one.
    return false;
  }
}
 ...
 *
 * Each event is fenced to the captured session incarnation and bound to the
 * originating agent (when one exists). A reset can reuse the same queue key and
 * even retain the same session id, so both the id and lifecycle revision must
 * still match immediately before enqueueing. The notice text is fixed and
 * bounded; the run identifier is kept only in the non-model-visible dedupe
 * context key, never interpolated into prompt text, because chat-send run ids
 * are caller-supplied and may be unbounded or contain newlines.
 */
export function queueAbortedPartialPersistenceFailures(
  ...
  for (const failure of failures) {
    ...
    if (!isCurrentAbortedPartialSession(failure)) {
      continue;
    }
    const options = {
  ...
  for (const snapshot of params.snapshots) {
    ...
    if (!appended.ok) {
      ...
      failed.push({
        ...
        lifecycleRevision: snapshot.value.expectedLifecycleRevision ?? null,
