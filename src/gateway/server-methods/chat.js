...
import { appendLocalMediaParentRoots, getAgentScopedMediaLocalRoots, } from "../../media/local-roots.js";
...
import { stripInlineDirectiveTagsForDisplay, stripInlineDirectiveTagsFromMessageForDisplay, sanitizeReplyDirectiveId, } from "../../utils/directive-tags.js";
import { INTERNAL_MESSAGE_CHANNEL, isGatewayCliClient, isWebchatClient, normalizeMessageChannel, } from "../../utils/message-channel.js";
import { abortChatRunById, isChatStopCommandText, registerChatAbortController, } from "../chat-abort.js";
import { parseMessageWithAttachments, } from "../chat-attachments.js";
import { MediaOffloadError } from "../chat-attachments.js";
import { stripEnvelopeFromMessage, stripEnvelopeFromMessages } from "../chat-sanitize.js";
import { augmentChatHistoryWithCliSessionImports } from "../cli-session-history.js";
import { isSuppressedControlReplyText } from "../control-reply-text.js";
import { attachManagedOutgoingImagesToMessage, cleanupManagedOutgoingImageRecords, createManagedOutgoingImageBlocks, } from "../managed-image-attachments.js";
import { ADMIN_SCOPE } from "../method-scopes.js";
import { GATEWAY_CLIENT_CAPS, GATEWAY_CLIENT_MODES, GATEWAY_CLIENT_NAMES, hasGatewayClientCap, } from "../protocol/client-info.js";
...
import { capArrayByJsonBytes, loadSessionEntry, resolveGatewayModelSupportsImages, resolveDeletedAgentIdFromSessionKey, readSessionMessages, resolveSessionModelRef, } from "../session-utils.js";
import { formatForLog } from "../ws-log.js";
import { injectTimestamp, timestampOptsFromConfig } from "./agent-timestamp.js";
import { setGatewayDedupeEntry } from "./agent-wait-dedupe.js";
import { normalizeRpcAttachmentsToChatAttachments } from "./attachment-normalize.js";
import { appendInjectedAssistantMessageToTranscript } from "./chat-transcript-inject.js";
import { buildWebchatAssistantMessageFromReplyPayloads, buildWebchatAudioContentBlocksFromReplyPayloads, } from "./chat-webchat-media.js";
...
const chatHistoryManagedImageCleanupState = new Map();
...
export function resolveEffectiveChatHistoryMaxChars(cfg, maxChars) {
...
function replaceAssistantContentTextBlocks(content, transcriptMediaMessage) {
    const transcriptTextBlocks = (transcriptMediaMessage?.content ?? []).filter((block) => Boolean(block) &&
        typeof block === "object" &&
        block.type === "text" &&
...
function resolveChatSendOriginatingRoute(params) {
    ...
    const routeChannelCandidate = normalizeMessageChannel(params.entry?.deliveryContext?.channel ??
    ...
    const routeAccountIdCandidate = params.entry?.deliveryContext?.accountId ??
        params.entry?.lastAccountId ??
        params.entry?.origin?.accountId ??
        undefined;
    const routeThreadIdCandidate = params.entry?.deliveryContext?.threadId ??
    ...
    const hasLegacyChannelPeerShape = !isChannelScopedSession &&
    ...
    const hasClientMetadata = (typeof params.client?.mode === "string" && params.client.mode.trim().length > 0) ||
    ...
    const canInheritConfiguredMainRoute = isConfiguredMainSessionScope &&
    ...
    const canInheritDeliverableRoute = Boolean(!isFromWebchatClient &&
        sessionChannelHint &&
...
function isAcpBridgeClient(client) {
    ...
    return (info?.id === GATEWAY_CLIENT_NAMES.CLI &&
...
function extractTranscriptUserText(content) {
    ...
    const textBlocks = content
        .map((block) => block && typeof block === "object" && "text" in block ? block.text : undefined)
...
async function rewriteChatSendUserTurnMediaPaths(params) {
    ...
    const target = [...branch].toReversed().find((entry) => {
        ...
        const existingPaths = Array.isArray(entry.message.MediaPaths)
            ? entry.message.MediaPaths
            : undefined;
        if ((typeof entry.message.MediaPath === "string" &&
            entry.message.MediaPath) ||
...
function sanitizeChatHistoryMessage(message, maxChars = DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS) {
    ...
    const preserveExactToolPayload = role === "toolresult" ||
        role === "tool_result" ||
        role === "tool" ||
        role === "function" ||
    ...
    if (typeof entry.content === "string") {
        ...
        const updated = entry.content.map((block) => sanitizeChatHistoryContentBlock(block, { preserveExactToolPayload, maxChars }));
...
export function sanitizeChatHistoryMessages(messages, maxChars = DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS) {
...
function appendCanvasBlockToAssistantHistoryMessage(params) {
    ...
    const alreadyPresent = baseContent.some((block) => {
        ...
        const typed = block;
        return (typed.type === "canvas" &&
            typed.preview &&
            typeof typed.preview === "object" &&
            ((typed.preview.viewId &&
                typed.preview.viewId === preview.viewId) ||
                (typed.preview.url &&
                    typed.preview.url === preview.url)));
    });
    if (!alreadyPresent) {
        baseContent.push({
            type: "canvas",
...
export function augmentChatHistoryWithCanvasBlocks(messages) {
    if (messages.length === 0) {
        return messages;
    }
    const next = [...messages];
...
export function buildOversizedHistoryPlaceholder(message) {
    const role = message &&
...
export function enforceChatHistoryFinalBudget(params) {
...
function normalizeExplicitChatSendOrigin(params) {
    ...
    const hasAnyExplicitOriginField = Boolean(originatingChannel || originatingTo || accountId || messageThreadId);
...
function isBtwReplyPayload(payload) {
    return (typeof payload?.btw?.question === "string" &&
...
export const chatHandlers = {
    ...
    "chat.send": async ({ params, respond, context, client }) => {
        ...
        if ((p.systemInputProvenance || p.systemProvenanceReceipt || explicitOriginResult.value) &&
            !canInjectSystemProvenance(client)) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, p.systemInputProvenance || p.systemProvenanceReceipt
        ...
        if (deletedAgentId !== null) {
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, `Agent "${deletedAgentId}" no longer exists in configuration`));
            return;
        }
        ...
        if (normalizedAttachments.length > 0) {
            ...
            const supportsImages = supportsSessionModelImages ||
            ...
            try {
                ...
                respond(false, undefined, errorShape(err instanceof MediaOffloadError ? ErrorCodes.UNAVAILABLE : ErrorCodes.INVALID_REQUEST, String(err)));
                return;
            }
        }
        try {
            ...
            const pluginBoundMediaFields = explicitOriginTargetsPlugin && parsedImages.length > 0
            ...
            const appendWebchatAgentMediaTranscriptIfNeeded = async (payload) => {
                ...
                const mediaLocalRoots = appendLocalMediaParentRoots(getAgentScopedMediaLocalRoots(cfg, agentId), resolvedTranscriptPath ? [resolvedTranscriptPath] : undefined);
                ...
                const persistedContentForAppend = hasAssistantDisplayMediaContent(persistedAssistantContent)
                    ...
                    : undefined;
                const transcriptReply = mediaMessage?.transcriptText ??
            ...
            void dispatchInboundMessage({
                ...
                if (!agentRunStarted) {
                    ...
                    if (btwReplies.length > 0 && btwText) {
                        ...
                        const persistedAssistantContent = replaceAssistantContentTextBlocks(hasSensitiveMedia
                        ...
                        const broadcastAssistantContent = hasAssistantDisplayMediaContent(assistantContent)
                            ...
                            : hasAssistantDisplayMediaContent(mediaMessage?.content)
                                ...
                                : assistantContent;
                        const displayReply = extractAssistantDisplayTextFromContent(assistantContent) ??
                        ...
                        if (transcriptReply ||
                            ...
                            if (appended.ok) {
                                ...
                                const fallbackAssistantContent = stripManagedOutgoingAssistantContentBlocks(persistedAssistantContent) ??
