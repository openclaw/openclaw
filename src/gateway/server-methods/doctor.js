...
import { isSameMemoryDreamingDay, resolveMemoryDeepDreamingConfig, resolveMemoryLightDreamingConfig, resolveMemoryDreamingPluginConfig, resolveMemoryDreamingConfig, resolveMemoryDreamingWorkspaces, resolveMemoryRemDreamingConfig, } from "../../memory-host-sdk/dreaming.js";
import { getActiveMemorySearchManager } from "../../plugins/memory-runtime.js";
import { formatError } from "../server-utils.js";
import { dedupeDreamDiaryEntries, removeBackfillDiaryEntries, removeGroundedShortTermCandidates, previewGroundedRemMarkdown, repairDreamingArtifacts, writeBackfillDiaryEntries, } from "./doctor.memory-core-runtime.js";
...
function isShortTermMemoryPath(filePath) {
    ...
    if (/(?:^|\/)memory\/\.dreams\/session-corpus\/(\d{4})-(\d{2})-(\d{2})\.(?:md|txt)$/.test(normalized)) {
        return true;
    }
...
async function loadDreamingStoreStats(workspaceDir, nowMs, timezone) {
    ...
    try {
        ...
        for (const [entryKey, value] of Object.entries(entries)) {
            ...
            const snippet = normalizeTrimmedString(entry.snippet) ??
...
export const doctorHandlers = {
    "doctor.memory.status": async ({ respond, context }) => {
        ...
        try {
            ...
            const storeStats = allWorkspaces.length > 0
                ? mergeDreamingStoreStats(await Promise.all(allWorkspaces.map((entry) => loadDreamingStoreStats(entry, nowMs, dreamingConfig.timezone))))
                : {
                    shortTermCount: 0,
                    recallSignalCount: 0,
                    dailySignalCount: 0,
