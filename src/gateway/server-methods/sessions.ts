...
function scoreDiagnoseCandidate(params: {
  ...
  let score = 0;
  if (gatewayRun.hasActiveRun || embeddedRun.active || activity.activeWorkKind) {
    score += 50;
  }
  if ((diagnostic.queueDepth ?? 0) > 0 || lane.queuedCount > 0) {
    score += 40;
  ...
  if (
    terminal &&
    (gatewayRun.hasActiveRun || embeddedRun.active || diagnostic.state === "processing")
  ) {
    score += 80;
  }
  if (
...
function buildDiagnoseFindings(params: {
  row: DiagnoseRow;
  gatewayRun: ReturnType<typeof collectTrackedActiveSessionRunSnapshot>;
  embeddedRun: ReturnType<typeof getEmbeddedRunDiagnosticSnapshot>;
  diagnostic: ReturnType<typeof getDiagnosticSessionStateSnapshot> &
    ReturnType<typeof getDiagnosticSessionActivitySnapshot>;
  lane: ReturnType<typeof getCommandLaneSnapshot>;
...
function summarizeDiagnose(params: {
  ...
  row: DiagnoseRow;
}): SessionsDiagnoseResult["summary"] {
...
async function buildDiagnoseResult(params: {
  ...
  return {
    ...
    transcript: {
      ...
      source: target.entry.sessionFile ? "sessionFile" : "store",
      ...(tail ? { recentEventCount: tail.lines.length } : {}),
    },
...
export const sessionsHandlers: GatewayRequestHandlers = {
  ...
  "sessions.diagnose": async ({ params, respond, context }) => {
    ...
    const p = params as DiagnoseParams;
    const cfg = context.getRuntimeConfig();
    try {
