...
export function respondUnavailableOnNodeInvokeError<
  T extends { ok: boolean; error?: unknown },
>(respond: RespondFn, res: T): res is T & { ok: true } {
  if (res.ok) {
    return true;
  }
  const nodeError =
    res.error && typeof res.error === "object"
      ...
      : null;
  const nodeCode =
    typeof nodeError?.code === "string" ? nodeError.code.trim() : "";
  const nodeMessage =
    typeof nodeError?.message === "string" &&
    nodeError.message.trim().length > 0
