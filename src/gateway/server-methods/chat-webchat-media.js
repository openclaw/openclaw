 ...
 */
export async function buildWebchatAudioContentBlocksFromReplyPayloads(payloads, options) {
    const seen = new Set();
    const blocks = [];
...
export async function buildWebchatAssistantMessageFromReplyPayloads(payloads, options) {
    const content = [];
    ...
    for (const payload of payloads) {
        ...
        const text = visibleText && !isSuppressedControlReplyText(visibleText) ? visibleText : undefined;
        ...
        const needsSyntheticText = payloadMediaBlocks.length > 0 &&
