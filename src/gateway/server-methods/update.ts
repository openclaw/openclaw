  ...
  "update.run": async ({ params, respond, client, context, sessionMutationCommitGuard }) => {
    ...
    if (managedHandoffOwner) {
      try {
        ...
        let failureHistoryIncomplete = false;
        // Cancellation settles the helper's ledger; persist the distinct late cause first.
        result = recordHandoffFailure(
          runId,
          error,
          result,
          (message) => {
            failureHistoryIncomplete = true;
            warn(message);
          },
        ...
        if (failureHistoryIncomplete) {
          outcomeMessage =
            "The update failed, but its failure history could not be fully saved. Check openclaw update status before retrying.";
        }
        try {
          warn(
            `update.run handoff transfer failed: ${formatErrorMessage(error)}${
              cancellation === "restored-in-process" ? "" : "; handoff cancellation is unconfirmed"
            }`,
          );
        } catch {
          // A diagnostic sink must not replace the retained handoff failure.
        }
      }
    }
