      ...
      try {
        ...
        providerServer = createServer((request, response) => {
          void (async () => {
            if (request.url === "/after-turn") {
              afterTurnCallCount += 1;
              if (afterTurnCallCount === 1) {
                // Predecessor's commitTurn: acknowledge immediately so the
                // predecessor can finish and the queue can drain.
                response.writeHead(204).end();
                return;
              }
              // Successor's commitTurn: block to create the pending-delivery
              // window. This fires after commitTerminalOutcome →
              // retireFollowupRunCancellation, so blocking here proves
              // retirement has already executed.
        ...
        await fs.writeFile(
          ...
          [
            ...
            "        `after-turn gate failed: ${response.status}`); return { status: 'committed' }; },",
            "    }));",
            "  },",
            "};",
            "",
        ...
        // received one request (for the held first turn). Snapshot this
        // before releasing the predecessor — after release, the provider will
        // receive the second request and the count will be 2.
        expect(modelRequests).toHaveLength(1);
        const queuedPreconditionSnapshot = modelRequests.length === 1;
        trace.push(
        ...
        //    starts executing. The predecessor's commitTurn acknowledges
        //    immediately (gate is successor-specific). The followup reaches
        //    commitTerminalOutcome (retireFollowupRunCancellation →
        //    entry.abortable=false), then its commitTurn blocks via HTTP
        //    POST — creating the pending-delivery window.
        ...
        const verdict = {
          ...
          queuedPrecondition: queuedPreconditionSnapshot,
          commitTurnCallCount: afterTurnCallCount,
