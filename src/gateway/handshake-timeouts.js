...
export function clampConnectChallengeTimeoutMs(timeoutMs) {
    return Math.max(MIN_CONNECT_CHALLENGE_TIMEOUT_MS, Math.min(MAX_CONNECT_CHALLENGE_TIMEOUT_MS, timeoutMs));
}
export function getConnectChallengeTimeoutMsFromEnv(env = process.env) {
...
export function resolveConnectChallengeTimeoutMs(timeoutMs) {
...
export function getPreauthHandshakeTimeoutMsFromEnv(env = process.env) {
