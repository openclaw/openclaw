...
describe("gateway auth", () => {
  it("resolves token/password from SMART_AGENT_NEO gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          SMART_AGENT_NEO_GATEWAY_TOKEN: "env-token",
          SMART_AGENT_NEO_GATEWAY_PASSWORD: "env-password",
  ...
  it("does not resolve legacy NEOBOT gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          NEOBOT_GATEWAY_TOKEN: "legacy-token",
          NEOBOT_GATEWAY_PASSWORD: "legacy-password",
        } as NodeJS.ProcessEnv,
      }),
    ).toMatchObject({
      mode: "none",
