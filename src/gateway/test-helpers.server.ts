...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.DNA_STATE_DIR;
  previousConfigPath = process.env.DNA_CONFIG_PATH;
  previousSkipBrowserControl = process.env.DNA_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.DNA_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.DNA_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "dna-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.DNA_STATE_DIR = path.join(tempHome, ".dna");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "dna-test-"))
    : path.join(tempHome, ".dna-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.DNA_STATE_DIR;
    else process.env.DNA_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.DNA_CONFIG_PATH;
    else process.env.DNA_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.DNA_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.DNA_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.DNA_SKIP_GMAIL_WATCHER;
    else process.env.DNA_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.DNA_SKIP_CANVAS_HOST;
    else process.env.DNA_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.DNA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.DNA_GATEWAY_TOKEN = fallbackToken;
  }
