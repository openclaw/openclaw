...
const mocks = vi.hoisted(() => ({
  replaceConfigFile: vi.fn(async (_params: { nextConfig: MullusiConfig }) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: MullusiConfig) {
  ...
  async function expectNoTokenGeneration(cfg: MullusiConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): MullusiConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
  ...
  it("resolves env-template gateway.auth.token before env-token short-circuiting", async () => {
    await expectResolvedToken({
      ...
      env: {
        MULLUSI_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${MULLUSI_GATEWAY_TOKEN}",
    });
  });

  it("uses MULLUSI_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
  ...
  it("uses MULLUSI_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await ensureGatewayStartupAuth({
      cfg: {
        gateway: {
          auth: {
