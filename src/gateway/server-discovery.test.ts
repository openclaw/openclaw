...
describe("resolveTailnetDnsHint", () => {
  ...
  beforeEach(() => {
    prevTailnetDns.value = process.env.SMART_AGENT_NEO_TAILNET_DNS;
    delete process.env.SMART_AGENT_NEO_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.SMART_AGENT_NEO_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.SMART_AGENT_NEO_TAILNET_DNS = "studio.tailnet.ts.net.";
