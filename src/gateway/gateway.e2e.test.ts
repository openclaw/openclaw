...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.DNA_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "dna-gw-mock-home-"));
      ...
      process.env.DNA_SKIP_BROWSER_CONTROL_SERVER = "1";
      ...
      const toolProbePath = path.join(workspaceDir, `.dna-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".dna");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "dna.json");
      ...
      process.env.DNA_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.DNA_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "dna-wizard-home-"));
    process.env.HOME = tempHome;
    delete process.env.DNA_STATE_DIR;
    delete process.env.DNA_CONFIG_PATH;
