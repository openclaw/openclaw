...
export function isRollingLogFile(file: string): boolean {
...
export async function readLogSlice(params: {
  ...
  const previousFile =
    typeof params.previousFile === "string" && params.previousFile.trim().length > 0
      ? params.previousFile.trim()
      : undefined;
  let cursor =
  ...
  if (previousFile && previousFile !== params.file) {
    reset = true;
