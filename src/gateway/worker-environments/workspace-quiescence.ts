...
export function createWorkerWorkspaceQuiescence(params: {
  ...
  return async (remoteWorkspaceDir, authorize) => {
    ...
    try {
      ...
      // The dispatched acquisition may have succeeded. Release its exact nonce without
      // borrowing the closed caller authority or starting independent renewals.
      try {
        await releaseRemote();
      } catch (releaseError) {
        throw new AggregateError(
          [error, releaseError],
          "Worker workspace quiescence release failed after authority closed",
          { cause: releaseError },
        );
      }
      throw error;
