...
describe("recipient-specific chat notification suppression", () => {
  ...
  it("preserves recipient projections and their rejection before deriving suppression", () => {
    const f = fixture();
    f.phone.client.preparedRecipientProfileId = "alice";
    const projected = { ...f.payload, message: { role: "assistant", content: "projected" } };
    const broadcaster = createGatewayBroadcaster({
      clients: f.clients,
      prepareSessionEventProjection: () => (client) =>
        client === f.otherPhone.client ? undefined : { ...projected, suppressNotification: true },
    });

    broadcaster.broadcast("chat", f.payload);

    expect(sentFrame(f.phone.send)).toMatchObject({
      recipientProfileId: "alice",
      payload: { ...projected, suppressNotification: true },
    });
    expect(f.otherPhone.send).not.toHaveBeenCalled();
    expect(sentFrame(f.viewer.send).payload).toEqual(projected);
    expect(f.payload.message.content).toBe("done");
  });
