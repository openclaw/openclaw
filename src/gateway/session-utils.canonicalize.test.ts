...
describe("canonicalizeWakeSessionKey", () => {
  ...
  it("falls back to main session key when agent ID does not match", () => {
    mocks.loadConfig.mockReturnValue({});
    // Key with a different agent prefix than the default (main)
    const result = canonicalizeWakeSessionKey("agent:other:discord:channel:1");
    expect(result).toBe("agent:main:main");
  });
  ...
  it("falls back to main when key targets different configured agent", () => {
    mocks.loadConfig.mockReturnValue({
      agents: { list: [{ id: "ops", default: true }] },
    });
    const result = canonicalizeWakeSessionKey("agent:other:hook:test");
