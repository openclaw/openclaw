...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.BOT_STATE_DIR = path.join(homeDir, ".bot");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-home-"));
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["BOT_CANVAS_HOST_PORT", "BOT_GATEWAY_TOKEN"]);
    try {
      ...
      process.env.BOT_CANVAS_HOST_PORT = String(canvasPort);
