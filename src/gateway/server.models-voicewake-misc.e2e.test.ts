...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    const prevStateDir = process.env.SMART_AGENT_NEO_STATE_DIR;
    const prevUserProfile = process.env.USERPROFILE;
    const prevHomeDrive = process.env.HOMEDRIVE;
    const prevHomePath = process.env.HOMEPATH;
    process.env.HOME = homeDir;
    process.env.SMART_AGENT_NEO_STATE_DIR = path.join(homeDir, ".smart-agent-neo");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["smart-agent-neo", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".smart-agent-neo", "settings", "voicewake.json"), "utf8"),
  ...
  test("pushes voicewake.changed to nodes on connect and on updates", async () => {
    ...
    const setRes = await rpcReq<{ triggers: string[] }>(ws, "voicewake.set", {
      triggers: ["smart-agent-neo", "computer"],
    });
    expect(setRes.ok).toBe(true);
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["SMART_AGENT_NEO_CANVAS_HOST_PORT", "SMART_AGENT_NEO_GATEWAY_TOKEN"]);
    try {
      ...
      process.env.SMART_AGENT_NEO_CANVAS_HOST_PORT = String(canvasPort);
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.SMART_AGENT_NEO_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing SMART_AGENT_NEO_CONFIG_PATH");
    }
