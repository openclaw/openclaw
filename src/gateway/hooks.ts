...
export function resolveHooksConfig(cfg: EasyHubConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-EasyHub-token"] === "string"
      ? req.headers["x-EasyHub-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
