...
export function resolveHooksConfig(cfg: MullusiConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MullusiConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-mullusi-token"] === "string"
      ? req.headers["x-mullusi-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-mullusi-idempotency-key"]) ||
