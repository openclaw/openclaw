...
export function resolveHooksConfig(cfg: ActiviConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ActiviConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-activi-token"] === "string"
      ? req.headers["x-activi-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
