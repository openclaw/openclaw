...
const gatewayHttpLog = createSubsystemLogger("gateway/http");
...
export async function runGatewayHttpRequestStages(
  ...
  for (const stage of stages) {
    try {
      ...
      gatewayHttpLog.error(`stage "${stage.name}" threw — skipping`, { err });
    }
  }
  return false;
}
...
export function createGatewayHttpServer(opts: {
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      gatewayHttpLog.error("unhandled error in request handler", { err });
      res.statusCode = 500;
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
