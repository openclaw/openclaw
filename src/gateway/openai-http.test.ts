...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("MULLUSI_CONFIG_PATH is required for gateway config tests");
  }
    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "mullusi", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-mullusi-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "mullusi", messages: [{ role: "user", content: "hi" }] },
          {
            "x-mullusi-agent-id": "beta",
            "x-mullusi-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        expect(json.error?.message).toBe(
          "Invalid `model`. Use `mullusi` or `mullusi/<agentId>`.",
        );
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-mullusi-model": "openai/" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-mullusi-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Mullusi.");
      }
