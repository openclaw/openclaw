...
import { ensureDNAModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { DNAConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.DNA_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.DNA_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.DNA_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<DNAConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveDNAAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.dna-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDNAAgentDir();
  const sanitizedCfg: DNAConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "DNA live tool probe (local, safe): " +
    ...
    process.env.DNA_CONFIG_PATH = previous.configPath;
    process.env.DNA_GATEWAY_TOKEN = previous.token;
    process.env.DNA_SKIP_CHANNELS = previous.skipChannels;
    process.env.DNA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.DNA_SKIP_CRON = previous.skipCron;
    process.env.DNA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.DNA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.DNA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.DNA_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.dna-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
