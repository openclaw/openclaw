...
import type { SmartAgentNeoConfig, ModelProviderConfig } from "../config/types.js";
...
import { ensureSmartAgentNeoModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SMART_AGENT_NEO_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SMART_AGENT_NEO_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SMART_AGENT_NEO_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: SmartAgentNeoConfig;
  agentDir: string;
}): SmartAgentNeoConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<SmartAgentNeoConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveSmartAgentNeoAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.smart-agent-neo-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSmartAgentNeoAgentDir();
  const sanitizedCfg: SmartAgentNeoConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-live-"));
  ...
  await ensureSmartAgentNeoModelsJson(nextCfg);
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "SmartAgentNeo live tool probe (local, safe): " +
    ...
    process.env.SMART_AGENT_NEO_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SMART_AGENT_NEO_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.SMART_AGENT_NEO_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.smart-agent-neo-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
