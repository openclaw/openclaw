/**
 * Display-cap truncation boundary tests.
 *
 ...
 * no longer carries into a user-visible reply. The Gateway and TUI paths keep the
 * scope aligned with the buffer they cap, so these are defensive boundaries: they
 * become reachable only if a truncation shortens the retained buffer without
 * maintaining those offsets, and no such producer is known today.
 */
...
describe("display-cap truncation boundary", () => {
  ...
  it("repairs a stale scope before the next offset read that follows an append", () => {
    // The merger appends deltas to the retained buffer without reading scope
    // offsets, so a stale prefix can survive that merge; the repair has to land
    // before the next merge reads the offsets, not just on the capped path.
    const appended = mergeAssistantText(
      {
        text: "CCCC",
        scope: { itemId: "answer", prefix: "AA", boundaryNewlines: 0, separatorLength: 0 },
      },
      { itemId: "answer", delta: "!" },
      "live",
    );
    expect(appended.text).toBe("CCCC!");

    const corrected = mergeAssistantText(appended, { itemId: "answer", text: "next" }, "live");
    expect(corrected.text).toBe("next");
    expect(corrected.text).not.toContain("AA");
  });
});
