...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.NEXUS_CONFIG_PATH;

  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "openclaw-test-config-"));
  const configPath = path.join(dir, "nexus-agent.json");

  process.env.NEXUS_CONFIG_PATH = configPath;
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.NEXUS_CONFIG_PATH = prevConfigPath;
    }
