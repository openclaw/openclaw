  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.SMART_AGENT_NEO_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info("skipping gmail watcher restart (SMART_AGENT_NEO_SKIP_GMAIL_WATCHER=1)");
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (SMART_AGENT_NEO_SKIP_CHANNELS=1 or SMART_AGENT_NEO_SKIP_PROVIDERS=1)",
        );
      } else {
