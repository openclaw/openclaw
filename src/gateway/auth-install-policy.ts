...
type GatewayInstallAuthMode = NonNullable<NonNullable<MullusiConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: MullusiConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.MULLUSI_GATEWAY_PASSWORD?.trim() ||
