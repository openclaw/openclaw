...
declare const __EASYHUB_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __EASYHUB_VERSION__ === "string" && __EASYHUB_VERSION__) ||
  process.env.EASYHUB_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `EasyHub --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.EASYHUB_GATEWAY_PORT ??
    process.env.EASYHUB_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.EASYHUB_GATEWAY_BIND ??
    process.env.EASYHUB_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
