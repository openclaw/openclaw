    ...
    try {
        ...
        const createPollingBot = async () => {
            try {
                ...
                const shouldRetry = await waitBeforeRetryOnRecoverableSetupError(err, "Telegram setup network error");
                if (!shouldRetry) {
                    return undefined;
                }
                return undefined;
        ...
        const ensureWebhookCleanup = async (bot) => {
            ...
            try {
                ...
                const shouldRetry = await waitBeforeRetryOnRecoverableSetupError(err, "Telegram webhook cleanup failed");
        ...
        const runPollingCycle = async (bot) => {
            ...
            try {
                ...
                const shouldRestart = await waitBeforeRestart((delay) => `Telegram polling runner stopped (${reason}); restarting in ${delay}.`);
                ...
                const shouldRestart = await waitBeforeRestart((delay) => `Telegram ${reason}: ${errMsg}; retrying in ${delay}.`);
                return shouldRestart ? "continue" : "exit";
            }
            finally {
