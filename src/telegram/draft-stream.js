...
export function createTelegramDraftStream(params) {
    ...
    const sendStreamPreview = usesDraftTransport
        ? async ({ renderedText, renderedParseMode, }) => {
            ...
            await resolvedDraftApi(chatId, draftId, renderedText, Object.keys(draftParams).length > 0 ? draftParams : undefined);
            return true;
        }
        : async ({ renderedText, renderedParseMode, sendGeneration, }) => {
            ...
            try {
                ...
                const hasThreadParam = "message_thread_id" in (sendParams ?? {}) &&
                ...
                params.warn?.("telegram stream preview send failed with message_thread_id, retrying without thread");
                sent = await params.api.sendMessage(chatId, renderedText, Object.keys(threadlessParams).length > 0 ? threadlessParams : undefined);
            }
