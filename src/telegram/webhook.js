...
const TELEGRAM_WEBHOOK_BODY_TIMEOUT_MS = 30000;
const TELEGRAM_WEBHOOK_CALLBACK_TIMEOUT_MS = 10000;
...
function resolveWebhookPublicUrl(params) {
    ...
    if (address && typeof address !== "string") {
        const resolvedHost = params.host === "0.0.0.0" || address.address === "0.0.0.0" || address.address === "::"
            ? "localhost"
            : address.address;
...
export async function startTelegramWebhook(opts) {
    ...
    if (!secret) {
        throw new Error("Telegram webhook mode requires a non-empty secret token. " +
