...
describe("isRecoverableTelegramNetworkError", () => {
  ...
  it("detects additional recoverable error codes", () => {
    ...
    const network = Object.assign(new Error("network"), {
      code: "ERR_NETWORK",
    });
  ...
  it("treats undici fetch failed errors as recoverable in send context", () => {
    ...
    expect(isRecoverableTelegramNetworkError(err, { context: "send" })).toBe(
      true,
    );
    expect(
      isRecoverableTelegramNetworkError(new Error("TypeError: fetch failed"), {
    ...
    expect(isRecoverableTelegramNetworkError(err, { context: "polling" })).toBe(
      true,
    );
  });

  it("skips broad message matches for send context", () => {
    const networkRequestErr = new Error(
      "Network request for 'sendMessage' failed!",
    );
    expect(
      isRecoverableTelegramNetworkError(networkRequestErr, { context: "send" }),
    ).toBe(false);
    expect(
      isRecoverableTelegramNetworkError(networkRequestErr, {
        context: "polling",
      }),
    ).toBe(true);
    ...
    expect(
      isRecoverableTelegramNetworkError(undiciSnippetErr, { context: "send" }),
    ).toBe(false);
    expect(
      isRecoverableTelegramNetworkError(undiciSnippetErr, {
        context: "polling",
      }),
    ).toBe(true);
  });
  ...
  it("returns false for unrelated errors", () => {
    expect(isRecoverableTelegramNetworkError(new Error("invalid token"))).toBe(
      false,
    );
  });
