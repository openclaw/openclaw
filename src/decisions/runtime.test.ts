...
describe("registered decision capability", () => {
  it.each([
    ...
    await expect(
      host.api.runtime.decisions.evaluateV2(decisionBatchV1ToV2(batch)!, { ...options(), agentId }),
    ).rejects.toThrow(reason);
  ...
  it("requires a current Gateway binding for scoped operator decisions", async () => {
    ...
    ).rejects.toThrow("Plugin model completion requires its current Gateway binding.");
    expect(evaluate).not.toHaveBeenCalled();
  });
  ...
  it.each([
    ...
    async ({ model, source }) => {
      ...
      expect(evaluate).toHaveBeenCalledOnce();
      expect(evaluate.mock.calls[0]?.[1].model).toBe("fixture-v1");
    },
  );
