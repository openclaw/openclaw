...
export function normalizeMessagePresentation(raw) {
    const record = toRecord(raw);
    if (!record) {
        return undefined;
    }
...
export function hasMessagePresentationBlocks(value) {
    return Boolean(normalizeMessagePresentation(value));
}
export function presentationToInteractiveReply(presentation) {
    const blocks = [];
...
export function interactiveReplyToPresentation(interactive) {
