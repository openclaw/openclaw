...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: SmartAgentNeoConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.SMART_AGENT_NEO_GATEWAY_PASSWORD;
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: SmartAgentNeoConfig = { session: { dmScope: "main" } };
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: SmartAgentNeoConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.SMART_AGENT_NEO_GATEWAY_TOKEN;
    process.env.SMART_AGENT_NEO_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: SmartAgentNeoConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.smart-agent-neo",
      configPath: "/Users/test/Dropbox/.smart-agent-neo/smart-agent-neo.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: SmartAgentNeoConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "smart-agent-neo.json"),
      });
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        smart-agent-neo: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        smart-agent-neo: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    it("prefers env password over remote password", async () => {
      process.env.SMART_AGENT_NEO_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: SmartAgentNeoConfig = {
        gateway: {
          mode: "remote",
