...
`,
  ...
  it("evaluates workspace skill path escape findings", async () => {
    const cases = [
      ...
      {
        name: "warns when skill resolves inside workspace root but outside skills directory",
        supported: !isWindows,
        setup: async () => {
          const tmp = await makeTmpDir("workspace-skill-inside-ws-outside-skills");
          const stateDir = path.join(tmp, "state");
          const workspaceDir = path.join(tmp, "workspace");
          // Create a skill directory inside the workspace root but outside workspace/skills/
          const insideWsSkillDir = path.join(
            workspaceDir,
            "other-dir",
            "shared-skills",
            "my-skill",
          ...
          await fs.mkdir(insideWsSkillDir, { recursive: true });

          const outsideSkillsSkillPath = path.join(insideWsSkillDir, "SKILL.md");
          await fs.writeFile(
            outsideSkillsSkillPath,
            "# inside workspace, outside skills\n",
            "utf-8",
          );
          await fs.symlink(
            outsideSkillsSkillPath,
          ...
          return { stateDir, workspaceDir, outsideSkillsSkillPath };
        },
        assert: (
          res: SecurityAuditReport,
          fixture: { stateDir: string; workspaceDir: string; outsideSkillsSkillPath?: string },
        ) => {
          const finding = res.findings.find((f) => f.checkId === "skills.workspace.symlink_escape");
          expect(finding?.severity).toBe("warn");
          expect(fixture.outsideSkillsSkillPath).toBeTruthy();
          expect(finding?.detail).toContain(fixture.outsideSkillsSkillPath ?? "");
        },
      },
      {
