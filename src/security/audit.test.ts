...
const execDockerRawUnavailable: NonNullable<
	...
	return {
		...
		config: {
			...
			isConfigured: (account, cfg) =>
				params.isConfigured?.(account, cfg) ?? true,
		},
	};
}
...
const zalouserPlugin = stubChannelPlugin({
	...
	label: "Zalo Personal",
	listAccountIds: (cfg) => {
		const channel = (cfg.channels as Record<string, unknown> | undefined)
			?.zalouser as { accounts?: Record<string, unknown> } | undefined;
...
const synologyChatPlugin = stubChannelPlugin({
	...
	resolveAccount: (cfg, accountId) => {
		...
		const account =
			cfg.channels?.["synology-chat"]?.accounts?.[resolvedAccountId] ?? {};
...
async function audit(
	...
	extra?: Omit<SecurityAuditOptions, "config"> & {
		preserveExecApprovals?: boolean;
	},
): Promise<SecurityAuditReport> {
	if (!extra?.preserveExecApprovals) {
		saveExecApprovals({ version: 1, agents: {} });
	}
	const { preserveExecApprovals: _preserveExecApprovals, ...options } =
		extra ?? {};
...
async function runConfigAuditCases<T extends { cfg: OpenClawConfig }>(
	...
	options?: (testCase: T) => Omit<SecurityAuditOptions, "config"> & {
		preserveExecApprovals?: boolean;
	},
) {
	await runAuditCases(
...
function hasFinding(
	...
	return res.findings.some(
		(f) =>
			f.checkId === checkId && (severity == null || f.severity === severity),
	);
}
...
function expectFindingSet(params: {
	...
	for (const checkId of params.expectedPresent ?? []) {
		expect(
			hasFinding(params.res, checkId, severity),
			`${params.name}:${checkId}`,
		).toBe(true);
	}
	for (const checkId of params.expectedAbsent ?? []) {
		expect(hasFinding(params.res, checkId), `${params.name}:${checkId}`).toBe(
			false,
		);
	}
}
...
async function expectSeverityByExposureCases(params: {
	...
	await Promise.all(
		params.cases.map(async (testCase) => {
			...
			expect(
				hasFinding(res, params.checkId, testCase.expectedSeverity),
				testCase.name,
			).toBe(true);
		}),
	);
...
describe("security audit", () => {
	...
	const previousPathResolutionEnv: Partial<
		Record<(typeof pathResolutionEnvKeys)[number], string>
	> = {};
	...
	const withChannelSecurityStateDir = async (
		fn: (tmp: string) => Promise<void>,
	) => {
		const credentialsDir = path.join(
			sharedChannelSecurityStateDir,
			"credentials",
		);
		await fs
			.rm(credentialsDir, { recursive: true, force: true })
			.catch(() => undefined);
		await fs.mkdir(credentialsDir, { recursive: true, mode: 0o700 });
		await withEnvAsync(
			{ OPENCLAW_STATE_DIR: sharedChannelSecurityStateDir },
...
`,
	...
	beforeAll(async () => {
		fixtureRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "openclaw-security-audit-"),
		);
		isolatedHome = path.join(fixtureRoot, "home");
		const isolatedEnv = createPathResolutionEnv(isolatedHome, {
		...
		sharedChannelSecurityStateDir = path.join(
			channelSecurityRoot,
			"state-shared",
		);
		...
		sharedExtensionsStateDir = path.join(
			fixtureRoot,
			"shared-extensions-state",
		);
		await fs.mkdir(
			path.join(sharedExtensionsStateDir, "extensions", "some-plugin"),
			{
				recursive: true,
				mode: 0o700,
			},
		);
		sharedInstallMetadataStateDir = path.join(
			fixtureRoot,
			"shared-install-metadata-state",
		);
	...
	it("includes an attack surface summary (info)", async () => {
		...
		const summary = res.findings.find(
		...
		const cases = [
			{
				...
				assert: (res: SecurityAuditReport) => {
					expect(hasFinding(res, "gateway.bind_no_auth", "critical")).toBe(
						true,
					);
				},
			},
			...
			},
			{
				...
				assert: (res: SecurityAuditReport) => {
					expect(hasFinding(res, "gateway.auth_no_rate_limit", "warn")).toBe(
						true,
					);
				},
			},
	...
	it("warns when sandbox exec host is selected while sandbox mode is off", async () => {
		...
		await runConfigAuditCases(cases, (res, testCase) => {
			expect(hasFinding(res, testCase.checkId, "warn"), testCase.name).toBe(
				true,
			);
		});
	});
	...
	it("evaluates safeBinTrustedDirs risk findings", async () => {
		const riskyGlobalTrustedDirs =
			process.platform === "win32"
				? [
						String.raw`C:\Users\ci-user\bin`,
						String.raw`C:\Users\ci-user\.local\bin`,
					]
	...
	it("evaluates loopback control UI and logging exposure findings", async () => {
		...
		await runConfigAuditCases(
			...
			(res, testCase) => {
				expect(
					hasFinding(res, testCase.checkId, testCase.severity),
					testCase.name,
				).toBe(true);
			},
			...
			},
			{
				...
				assert: (res: SecurityAuditReport) => {
					expect(
						res.findings.some(
							(f) =>
								f.checkId === "fs.state_dir.perms_readable" &&
								f.severity === "warn",
						),
					).toBe(true);
				},
	...
	it("evaluates sandbox browser findings", async () => {
		const cases = [
				...
				run: async () => {
					...
					return runSecurityAudit({
						...
						execDockerRawFn: (async (args: string[]) => {
							...
							if (
								args[0] === "inspect" &&
								args.at(-1) === "openclaw-sbx-browser-old"
							) {
								return {
							...
							if (
								args[0] === "inspect" &&
								args.at(-1) === "openclaw-sbx-browser-missing-hash"
							) {
								return {
			...
			},
			{
				...
				assert: (res: SecurityAuditReport) => {
					expect(
						hasFinding(res, "sandbox.browser_container.hash_label_missing"),
					).toBe(false);
					expect(
						hasFinding(res, "sandbox.browser_container.hash_epoch_stale"),
					).toBe(false);
				},
			},
			{
				...
				run: async () => {
					...
					return runSecurityAudit({
						...
						execDockerRawFn: (async (args: string[]) => {
							...
							if (
								args[0] === "inspect" &&
								args.at(-1) === "openclaw-sbx-browser-exposed"
							) {
								return {
									stdout: Buffer.from(
										"hash123\t2026-02-21-novnc-auth-default\n",
									),
									stderr: Buffer.alloc(0),
									code: 0,
								};
			...
			},
			{
				...
				assert: (res: SecurityAuditReport) => {
					expect(
						hasFinding(res, "sandbox.browser_cdp_bridge_unrestricted"),
					).toBe(false);
				},
			},
		] as const;
	...
	it("uses symlink target permissions for config checks", async () => {
		...
		expect(res.findings).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ checkId: "fs.config.symlink" }),
			]),
		);
		expect(
			res.findings.some((f) => f.checkId === "fs.config.perms_writable"),
		).toBe(false);
		expect(
			res.findings.some((f) => f.checkId === "fs.config.perms_world_readable"),
		).toBe(false);
		expect(
			res.findings.some((f) => f.checkId === "fs.config.perms_group_readable"),
		).toBe(false);
	});
		...
		const cases = [
			{
				...
				setup: async () => {
					...
					await fs.mkdir(stateDir, { recursive: true, mode: 0o700 });
					await fs.mkdir(path.join(workspaceDir, "skills", "leak"), {
						recursive: true,
					});
				...
				assert: (
					...
					fixture: {
						stateDir: string;
						workspaceDir: string;
						outsideSkillPath?: string;
					},
				) => {
					const finding = res.findings.find(
						(f) => f.checkId === "skills.workspace.symlink_escape",
					);
					expect(finding?.severity).toBe("warn");
			...
			},
			{
				...
				setup: async () => {
					...
					const workspaceDir = path.join(tmp, "workspace");
					await fs.mkdir(stateDir, { recursive: true, mode: 0o700 });
					await fs.mkdir(path.join(workspaceDir, "skills", "safe"), {
						recursive: true,
					});
					await fs.writeFile(
	...
	it("scores small-model risk by tool/sandbox exposure", async () => {
		...
		await runConfigAuditCases(cases, (res, testCase) => {
			const finding = res.findings.find(
				(f) => f.checkId === "models.small_params",
			);
	...
	it("evaluates sandbox docker config findings", async () => {
		...
		await runConfigAuditCases(cases, (res, testCase) => {
			...
			expectFindingSet({
				...
				name: testCase.name,
				expectedAbsent:
					"expectedAbsent" in testCase ? testCase.expectedAbsent : [],
			});
		});
	});
	...
	it("flags tools.elevated allowFrom wildcard as critical", async () => {
		...
		expectFinding(
			res,
			"tools.elevated.allowFrom.whatsapp.wildcard",
			"critical",
		);
	});
	...
	it("adds a read-only resolution warning when channel account resolveAccount throws", async () => {
		...
		const finding = res.findings.find(
			(entry) =>
				entry.checkId === "channels.zalouser.account.read_only_resolution",
		);
		expect(finding?.severity).toBe("warn");
		expect(finding?.title).toContain("could not be fully resolved");
		expect(finding?.detail).toContain(
			"zalouser:default: failed to resolve account",
		);
	...
	it.each([
		...
		await withChannelSecurityStateDir(async (tmp) => {
			...
			const nameBasedFinding = res.findings.find(
				(entry) =>
					entry.checkId === "channels.discord.allowFrom.name_based_entries",
			);
			...
			if (testCase.expectNoNameBasedFinding) {
				...
				if (testCase.expectNameBasedSeverity) {
					expect(nameBasedFinding?.severity).toBe(
						testCase.expectNameBasedSeverity,
					);
				}
			...
			if (testCase.expectFindingMatch) {
				expect(res.findings).toEqual(
					expect.arrayContaining([
						expect.objectContaining(testCase.expectFindingMatch),
					]),
				);
			}
		});
	...
	it.each([
		...
		await withChannelSecurityStateDir(async () => {
			const res = await runChannelSecurityAudit(testCase.cfg, [
				synologyChatPlugin,
			]);
			expect(res.findings).toEqual(
				expect.arrayContaining([
					expect.objectContaining(testCase.expectedMatch),
				]),
			);
		});
	});
	...
	it("does not treat prototype properties as explicit Discord account config paths", async () => {
		await withChannelSecurityStateDir(async () => {
			...
			const dangerousMatchingFinding = res.findings.find(
				(entry) =>
					entry.checkId ===
					"channels.discord.allowFrom.dangerous_name_matching_enabled",
			);
			expect(dangerousMatchingFinding).toBeDefined();
			expect(dangerousMatchingFinding?.title).not.toContain(
				"(account: toString)",
			);
			...
			expect(nameBasedFinding).toBeDefined();
			expect(nameBasedFinding?.detail).toContain(
				"channels.discord.allowFrom:Alice#1234",
			);
			expect(nameBasedFinding?.detail).not.toContain(
				"channels.discord.accounts.toString",
			);
		});
	});
		...
		await runAuditCases(
			cases.map((testCase) => ({
				...
				assert: (res: SecurityAuditReport) => {
					...
					expect(
						hasFinding(res, "gateway.probe_failed", "warn"),
						testCase.name,
					).toBe(true);
				},
			})),
		...
		await runConfigAuditCases(
			...
			(res, testCase) => {
				for (const expected of testCase.expectedFindings ?? []) {
					expect(
						hasFinding(res, expected.checkId, expected.severity),
						testCase.name,
					).toBe(true);
				}
				if (testCase.expectedAbsentCheckId) {
					expect(
						hasFinding(res, testCase.expectedAbsentCheckId),
						testCase.name,
					).toBe(false);
				}
			},
			...
			cases.map((testCase) => ({
				...
				assert: (res: SecurityAuditReport) => {
					expectFindingSet({
						...
						name: testCase.name,
						expectedPresent:
							"expectedPresent" in testCase ? testCase.expectedPresent : [],
						expectedAbsent:
							"expectedAbsent" in testCase ? testCase.expectedAbsent : [],
					});
				},
		...
		const cases = [
			...
			},
			{
				...
				assert: (res: SecurityAuditReport) => {
					expect(
						res.findings.some(
							(f) => f.checkId === "plugins.tools_reachable_permissive_policy",
						),
					).toBe(false);
				},
			},
		...
		const cases = [
			{
				...
				assert: (result: SecurityAuditReport) => {
					expect(
						result.findings.some((f) => f.checkId === "plugins.code_safety"),
					).toBe(false);
				},
			},
			{
				...
				assert: (
					result: Awaited<
						ReturnType<typeof collectPluginsCodeSafetyFindings>
					> extends never
						? never
						: {
								pluginFindings: Awaited<
									ReturnType<typeof collectPluginsCodeSafetyFindings>
								>;
								skillFindings: Awaited<
									ReturnType<typeof collectInstalledSkillsCodeSafetyFindings>
								>;
							},
				) => {
					const pluginFinding = result.pluginFindings.find(
						(finding) =>
							finding.checkId === "plugins.code_safety" &&
			...
			},
			{
				...
				assert: (
					...
					expect(
						findings.some(
							(f) => f.checkId === "plugins.code_safety.entry_escape",
						),
					).toBe(true);
				},
			},
			{
				...
				assert: (
					...
					expect(
						findings.some(
							(f) => f.checkId === "plugins.code_safety.scan_failed",
						),
					).toBe(true);
				},
			},
	...
	describe("maybeProbeGateway auth selection", () => {
		...
		it("applies gateway auth precedence across local/remote modes", async () => {
			const cases: Array<{
			...
			}> = [
				{
					...
					cfg: {
						gateway: { mode: "local", auth: { token: "local-token-abc123" } },
					},
		...
		it("adds warning finding when probe auth SecretRef is unavailable", async () => {
			...
			const warning = res.findings.find(
				(finding) =>
					finding.checkId === "gateway.probe_auth_secretref_unavailable",
			);
