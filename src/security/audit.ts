...
type ExecDockerRawFn =
	typeof import("../agents/sandbox/docker.js").execDockerRaw;
...
let channelPluginsModulePromise:
	| Promise<typeof import("../channels/plugins/index.js")>
	| undefined;
let auditNonDeepModulePromise:
	| Promise<typeof import("./audit.nondeep.runtime.js")>
	| undefined;
let auditDeepModulePromise:
	| Promise<typeof import("./audit.deep.runtime.js")>
	| undefined;
...
function isFeishuDocToolEnabled(cfg: OpenClawConfig): boolean {
	...
	for (const accountValue of Object.values(accounts)) {
		...
		const accountConfigured =
			...
			(hasConfiguredSecretInput(account.appSecret, cfg.secrets?.defaults) ||
				baseAppSecret);
...
function collectGatewayConfigFindings(
	...
	const envPasswordConfigured = hasNonEmptyString(
	...
	const passwordConfigured =
		hasPassword ||
		(passwordCanWin && (envPasswordConfigured || passwordConfiguredFromConfig));
...
function collectExecRuntimeFindings(
	...
	const fullExecScopes = effectiveExecScopes.filter(
		(entry) => entry.security === "full",
	);
	const execEnabledScopes = effectiveExecScopes.filter(
		(entry) => entry.security !== "deny",
	);
	...
	const interpreterAllowlistHits = collectInterpreterAllowlistHits({
		...
		strictInlineEvalForAgentId: (agentId) => {
			...
			return (
				agent?.tools?.exec?.strictInlineEval === true || globalStrictInlineEval
			);
		},
	});
	...
	if (globalSafeBins.length > 0) {
		...
		for (const hit of listRiskyConfiguredSafeBins(globalSafeBins)) {
			riskySemanticSafeBinHits.push(
				`- tools.exec.safeBins: ${hit.bin} (${hit.warning})`,
			);
		}
	}
	...
	if (riskySemanticSafeBinHits.length > 0) {
		findings.push({
			...
			severity: "warn",
			title:
				"safeBins includes binaries with broader semantics than low-risk stream filters",
			detail:
...
function collectInterpreterAllowlistHits(params: {
	...
	const hits: string[] = [];
	for (const [agentId, agent] of Object.entries(
		params.approvals.agents ?? {},
	)) {
...
async function maybeProbeGateway(params: {
	...
	if (authResolution.warning && !res.ok) {
		res.error = res.error
			? `${res.error}; ${authResolution.warning}`
...
export async function runSecurityAudit(
	...
	findings.push(
		...auditNonDeep.collectSyncedFolderFindings({ stateDir, configPath }),
	);
	...
	findings.push(
		...auditNonDeep.collectGatewayHttpSessionKeyOverrideFindings(cfg),
	);
	...
	if (context.includeFilesystem) {
		...
		findings.push(
			...(await auditNonDeep.collectPluginsTrustFindings({ cfg, stateDir })),
		);
		if (context.deep) {
	...
	if (shouldAuditChannelSecurity) {
		const channelPlugins =
			context.plugins ?? (await loadChannelPlugins()).listChannelPlugins();
	...
	const deepProbeResult = context.deep
		? await maybeProbeGateway({
				...
				probe:
					context.probeGatewayFn ?? (await loadGatewayProbeDeps()).probeGateway,
