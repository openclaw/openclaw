...
export function collectGatewayConfigFindings(cfg, sourceConfig, env, options = {}) {
    const findings = [];
    ...
    const passwordCanWin = explicitAuthMode === "password" ||
    ...
    const hasSharedSecret = explicitAuthMode === "token"
        ? tokenConfigured
    ...
    if (bind !== "loopback" &&
        ...
        findings.push({
            ...
            detail: "Control UI is enabled on a non-loopback bind but gateway.controlUi.allowedOrigins is empty. " +
                "Strict origin policy requires explicit allowed origins for non-loopback deployments.",
            remediation: "Set gateway.controlUi.allowedOrigins to full trusted origins (for example https://control.example.com). " +
    ...
    if (dangerouslyAllowHostHeaderOriginFallback) {
        ...
        findings.push({
            ...
            detail: "gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true enables Host-header origin fallback " +
    ...
    if (allowRealIpFallback) {
        ...
        findings.push({
            ...
            detail: "gateway.allowRealIpFallback=true trusts X-Real-IP when trusted proxies omit X-Forwarded-For. " +
                "Misconfigured proxies that forward client-supplied X-Real-IP can spoof source IP and local-client checks.",
            remediation: "Keep gateway.allowRealIpFallback=false (default). Only enable this when your trusted proxy " +
    ...
    if (mdnsMode === "full") {
        ...
        findings.push({
            ...
            detail: 'discovery.mdns.mode="full" publishes cliPath/sshPort in local-network TXT records. ' +
