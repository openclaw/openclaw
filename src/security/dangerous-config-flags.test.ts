...
describe("collectEnabledInsecureOrDangerousFlags", () => {
  ...
  it("collects manifest-declared boolean dangerous plugin config values", () => {
    loadPluginManifestRegistryMock.mockReturnValue({
      plugins: [
        {
          id: "voice-call",
          configContracts: {
            dangerousFlags: [{ path: "skipSignatureVerification", equals: true }],
          },
        },
      ],
      diagnostics: [],
    ...
    ).toContain("plugins.entries.voice-call.config.skipSignatureVerification=true");
  });
  ...
  it.each([
    {
      name: "ignores dangerous plugin config values when the plugin entry is disabled",
      cfg: asConfig({
        plugins: {
          entries: {
            "voice-call": {
    ...
    {
      name: "ignores dangerous plugin config values when plugins are globally disabled",
      cfg: asConfig({
        plugins: {
          enabled: false,
          entries: {
