...
describe("temp path guard", () => {
	...
	it("detects dynamic and ignores static fixtures", () => {
		...
		expect(
			dynamicFixtures.every((fixture) => hasDynamicTmpdirJoin(fixture)),
		).toBe(true);
		expect(
			staticFixtures.every((fixture) => !hasDynamicTmpdirJoin(fixture)),
		).toBe(true);
	});
	...
	it("enforces runtime guardrails for tmpdir joins and weak randomness", async () => {
		...
		for (const file of files) {
			...
			if (
				mightContainWeakRandom &&
				WEAK_RANDOM_SAME_LINE_PATTERN.test(source)
			) {
