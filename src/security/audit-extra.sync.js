...
import { DEFAULT_DANGEROUS_NODE_COMMANDS, resolveNodeCommandAllowlist, } from "../gateway/node-command-policy.js";
import { normalizeOptionalLowercaseString, normalizeOptionalString, normalizeStringifiedOptionalString, } from "../shared/string-coerce.js";
...
export function collectHooksHardeningFindings(cfg, env = process.env) {
    ...
    const openclawGatewayToken = typeof env.OPENCLAW_GATEWAY_TOKEN === "string" && env.OPENCLAW_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(cfg) {
    ...
    findings.push({
        ...
        detail: `${enabledEndpoints.join(", ")} accept x-openclaw-session-key for per-request session routing. ` +
...
export function collectSandboxDangerousConfigFindings(cfg) {
    ...
    for (const { source, docker } of configs) {
        ...
        if (isDangerousNetworkMode(network)) {
            ...
            const detail = normalizedNetwork === "host"
            ...
            findings.push({
                ...
                detail,
                remediation: `Set ${source}.network to "bridge", "none", or a custom bridge network name.` +
...
export function collectNodeDangerousAllowCommandFindings(cfg) {
    const findings = [];
    ...
    findings.push({
        ...
        remediation: "Remove these entries from gateway.nodes.allowCommands (recommended). " +
...
export function collectLikelyMultiUserSetupFindings(cfg) {
    const findings = [];
    ...
    const riskyContextsDetail = riskyContexts.length > 0
