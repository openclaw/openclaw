...
describe("security audit gateway exposure findings", () => {
  it("warns on insecure or dangerous flags", () => {
    const cases = [
      ...
      {
        name: "voice-call skipSignatureVerification is treated as a dangerous break-glass flag",
        cfg: {
          plugins: {
            entries: {
              "voice-call": {
  ...
  it.each([
    ...
    expect(
      findings.some((finding) => finding.checkId === "config.insecure_or_dangerous_flags"),
    ).toBe(false);
  });
