	...
	expect(
		findings.some(
			(finding) =>
				...
				(expected.messageIncludes == null ||
					finding.message.includes(expected.messageIncludes)),
		),
	).toBe(true);
}
...
describe("scanDirectoryWithSummary", () => {
	it.each([
		...
		const summary = await scanDirectoryWithSummary(
			root,
			normalizeSkillScanOptions(options),
		);
