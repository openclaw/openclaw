...
function summarizeGroupPolicy(cfg: ActiviConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: ActiviConfig): boolean {
...
function collectModels(cfg: ActiviConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: ActiviConfig): Set<string> {
  const baseCfg: ActiviConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: ActiviConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ActiviConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ActiviConfig): boolean {
...
function isBrowserEnabled(cfg: ActiviConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ActiviConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: ActiviConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep ACTIVI_STATE_DIR on a local-only volume and re-run "${formatCliCommand("activi security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: ActiviConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer ACTIVI_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const activiGatewayToken =
    typeof env.ACTIVI_GATEWAY_TOKEN === "string" && env.ACTIVI_GATEWAY_TOKEN.trim()
      ? env.ACTIVI_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-activi-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: ActiviConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: ActiviConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: activi-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: ActiviConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: ActiviConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: ActiviConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: ActiviConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
