...
function summarizeGroupPolicy(cfg: BotConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: BotConfig): boolean {
...
function listKnownNodeCommands(cfg: BotConfig): Set<string> {
...
function hasWebSearchKey(cfg: BotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: BotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: BotConfig): boolean {
...
function isBrowserEnabled(cfg: BotConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: BotConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: BotConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: BotConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: BotConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep BOT_STATE_DIR on a local-only volume and re-run "${formatCliCommand("bot security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectHooksHardeningFindings(
  ...
  const botGatewayToken =
    typeof env.BOT_GATEWAY_TOKEN === "string" && env.BOT_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-bot-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: BotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: BotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: bot-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: BotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: BotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: BotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
