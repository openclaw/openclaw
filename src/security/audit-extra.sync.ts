...
function summarizeGroupPolicy(cfg: SmartAgentNeoConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: SmartAgentNeoConfig): boolean {
...
function collectModels(cfg: SmartAgentNeoConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: SmartAgentNeoConfig): Set<string> {
  const baseCfg: SmartAgentNeoConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: SmartAgentNeoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SmartAgentNeoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SmartAgentNeoConfig): boolean {
...
function isBrowserEnabled(cfg: SmartAgentNeoConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: SmartAgentNeoConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: SmartAgentNeoConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SMART_AGENT_NEO_STATE_DIR on a local-only volume and re-run "${formatCliCommand("smart-agent-neo security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: SmartAgentNeoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer SMART_AGENT_NEO_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const smartAgentNeoGatewayToken =
    typeof env.SMART_AGENT_NEO_GATEWAY_TOKEN === "string" && env.SMART_AGENT_NEO_GATEWAY_TOKEN.trim()
      ? env.SMART_AGENT_NEO_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-smart-agent-neo-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: SmartAgentNeoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: SmartAgentNeoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: SmartAgentNeoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: SmartAgentNeoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: SmartAgentNeoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: SmartAgentNeoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
