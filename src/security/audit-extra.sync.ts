...
function suggestKnownNodeCommands(
	unknown: string,
	known: Set<string>,
): string[] {
	...
	const prefix = needle.includes(".")
		? needle.split(".").slice(0, 2).join(".")
		: needle;
...
export function collectNodeDenyCommandPatternFindings(
	...
	if (unknownExact.length > 0) {
		...
		detailParts.push(
			`Unknown command names (not in defaults/allowCommands): ${unknownDetails}`,
		);
	}
