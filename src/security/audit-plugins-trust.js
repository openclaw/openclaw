...
async function isChannelPluginConfigured(cfg, plugin) {
    ...
    for (const accountId of candidates) {
        const inspected = plugin.config.inspectAccount?.(cfg, accountId) ??
            (await inspectReadOnlyChannelAccount({
                channelId: plugin.id,
                cfg,
                accountId,
            }));
        const inspectedRecord = inspected && typeof inspected === "object" && !Array.isArray(inspected)
            ? inspected
            : null;
        ...
        let enabled = typeof inspectedRecord?.enabled === "boolean"
            ? inspectedRecord.enabled
            : resolvedAccount != null;
        if (typeof inspectedRecord?.enabled !== "boolean" &&
            resolvedAccount != null &&
        ...
        let configured = typeof inspectedRecord?.configured === "boolean"
            ? inspectedRecord.configured
            : resolvedAccount != null;
        if (typeof inspectedRecord?.configured !== "boolean" &&
            resolvedAccount != null &&
...
async function listInstalledPluginDirs(params) {
    ...
    const st = await fs.stat(extensionsDir).catch((err) => {
...
function resolveToolPolicies(params) {
    ...
    if (params.sandboxMode === "all") {
        policies.push(params.deps.resolveSandboxToolPolicyForAgent(params.cfg, params.agentId ?? undefined));
    }
    return policies;
}
    ...
    if (pluginDirs.length > 0) {
        ...
        if (!allowConfigured) {
            const skillCommandsLikelyExposed = (await Promise.all(listChannelPlugins().map(async (plugin) => {
                if (plugin.capabilities.nativeCommands !== true &&
