...
function expectSinglePrincipal(
...
function expectSummaryCounts(
	...
	expected: {
		trusted?: number;
		untrustedWorld?: number;
		untrustedGroup?: number;
	},
...
Successfully processed 1 files`;
		...
		] as const)("detects write permissions correctly for %s", ({
			rights,
			canWrite,
			canRead,
		}) => {
	...
	describe("inspectWindowsAcl", () => {
		it("returns parsed ACL entries on success", async () => {
			...
			expect(mockExec).toHaveBeenCalledWith("icacls", [
				"C:\\test\\file.txt",
				"/sid",
			]);
		});
		...
		it("resolves current user SID via whoami when USERSID is missing", async () => {
			...
			expect(result.untrustedGroup).toHaveLength(0);
			expect(mockExec).toHaveBeenNthCalledWith(1, "icacls", [
				"C:\\test\\file.txt",
				"/sid",
			]);
			expect(mockExec).toHaveBeenNthCalledWith(2, "whoami", [
				"/user",
				"/fo",
				"csv",
				"/nh",
