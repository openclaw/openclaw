...
let skillsModulePromise:
	| Promise<typeof import("../agents/skills.js")>
	| undefined;
let configModulePromise:
	| Promise<typeof import("../config/config.js")>
	| undefined;
...
function buildCodeSafetySummaryCacheKey(params: {
	...
	const includeFiles = (params.includeFiles ?? [])
		...
		.filter(Boolean);
	const includeKey =
		includeFiles.length > 0 ? includeFiles.toSorted().join("\u0000") : "";
...
async function getCodeSafetySummary(params: {
	...
	if (cache) {
		...
		if (hit) {
			return (await hit) as Awaited<
				ReturnType<typeof skillScanner.scanDirectoryWithSummary>
			>;
		}
...
export async function collectWorkspaceSkillSymlinkEscapeFindings(params: {
	...
	for (const workspaceDir of workspaceDirs) {
		...
		for (const skillFilePath of skillFilePaths) {
			...
			const skillRealPath = await fs
				.realpath(canonicalSkillPath)
				.catch(() => null);
			if (!skillRealPath) {
				continue;
			}
