 ...
 */
function realpathWithTimeout(p, timeoutMs = 2000) {
    let timerHandle;
...
let auditPluginsTrustModulePromise;
...
async function listSandboxBrowserContainers(execDockerRawFn) {
    try {
        const result = await execDockerRawFn(["ps", "-a", "--filter", "label=openclaw.sandboxBrowser=1", "--format", "{{.Names}}"], { allowFailure: true });
        if (result.code !== 0) {
            return null;
        }
    ...
    for (const workspaceDir of workspaceDirs) {
        ...
        if (truncated) {
            ...
            findings.push({
                ...
                detail: `The skills/ directory scan in ${workspacePath} stopped early after reaching the ` +
                    `BFS visit cap. Skill files in the unscanned portion of the tree were not checked ` +
                    "for symlink escapes.",
                remediation: "Flatten or simplify the skills/ directory hierarchy to stay within the scan budget, " +
    ...
    findings.push({
        ...
        detail: "Detected workspace `skills/**/SKILL.md` paths whose realpath escapes their workspace root:\n" +
            escapedSkillFiles
                ...
                .map((entry) => `- workspace=${entry.workspaceDir}\n` +
...
export async function collectStateDeepFilesystemFindings(params) {
    ...
    const agentIds = Array.isArray(params.cfg.agents?.list)
        ? params.cfg.agents?.list
            .map((a) => normalizeOptionalString(a && typeof a === "object" ? a.id : undefined) ?? "")
            .filter(Boolean)
        : [];
