...
export function resolvePinnedMainDmOwnerFromAllowlist(params) {
    ...
    const normalizedOwners = Array.from(new Set(rawAllowFrom
...
export function resolveEffectiveAllowFromLists(params) {
    ...
    const effectiveAllowFrom = normalizeStringEntries(mergeDmAllowFromSources({
        allowFrom,
        storeAllowFrom,
        dmPolicy: params.dmPolicy ?? undefined,
    }));
    // Group auth is explicit (groupAllowFrom fallback allowFrom). Pairing store is DM-only.
    const effectiveGroupAllowFrom = normalizeStringEntries(resolveGroupAllowFromSources({
        allowFrom,
        groupAllowFrom,
...
export function resolveDmGroupAccessDecision(params) {
    ...
    const groupPolicy = params.groupPolicy === "open" || params.groupPolicy === "disabled"
        ? params.groupPolicy
        : "allowlist";
