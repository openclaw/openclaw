...
export function collectGatewayConfigFindings(cfg, sourceConfig, env) {
...
export async function collectPluginSecurityAuditFindings(context) {
    ...
    if (collectors.length === 0) {
        ...
        const snapshot = (await loadPluginMetadataRegistryLoaderModule()).loadPluginMetadataRegistrySnapshot({
            config: autoEnabled.config,
            activationSourceConfig: context.sourceConfig,
            env: context.env,
            workspaceDir: context.workspaceDir,
    ...
    const collectorResults = await Promise.all(collectors.map(async (entry) => {
        try {
...
export function collectLoggingFindings(cfg) {
...
export function collectElevatedFindings(cfg) {
    const findings = [];
...
export function collectExecRuntimeFindings(cfg) {
    const findings = [];
    ...
    if (fullExecScopes.length > 0) {
        findings.push({
            ...
            detail: `Full exec trust is enabled for: ${fullExecScopes.map((entry) => entry.id).join(", ")}.` +
    ...
    for (const entry of agents) {
        ...
        if (interpreters.length === 0) {
            for (const hit of listRiskyConfiguredSafeBins(agentSafeBins)) {
                riskySemanticSafeBinHits.push(`- agents.list.${entry.id}.tools.exec.safeBins: ${hit.bin} (${hit.warning})`);
            }
            continue;
        }
