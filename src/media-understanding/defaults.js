...
const configRegistryCache = new WeakMap();
...
function resolveConfiguredImageProviderModel(params) {
    ...
    for (const [providerKey, providerCfg] of Object.entries(providers)) {
        ...
        const match = models.find((model) => Boolean(normalizeOptionalString(model?.id)) &&
...
function resolveConfiguredImageProviderIds(cfg) {
    ...
    for (const [providerKey, providerCfg] of Object.entries(providers)) {
        ...
        const models = providerCfg?.models ?? [];
        const hasImageModel = models.some((model) => Array.isArray(model?.input) && model.input.includes("image"));
        if (hasImageModel) {
...
export function resolveDefaultMediaModel(params) {
    if (!params.providerRegistry) {
        const configuredImageModel = params.capability === "image"
