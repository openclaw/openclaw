...
const TEMP_MEDIA_PREFIX = "mullusi-media-";
...
function createGroqAudioConfig(): MullusiConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): MullusiConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): MullusiConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: MullusiConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredMullusiTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        MULLUSI_AGENT_DIR: isolatedAgentDir,
