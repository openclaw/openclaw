...
describe("runCapability auto audio entries", () => {
  ...
  it("uses bailian when only DASHSCOPE_API_KEY is configured", async () => {
    const priorEnv: Record<string, string | undefined> = {
      OPENAI_API_KEY: process.env.OPENAI_API_KEY,
      GROQ_API_KEY: process.env.GROQ_API_KEY,
      DEEPGRAM_API_KEY: process.env.DEEPGRAM_API_KEY,
    ...
    let runResult: Awaited<ReturnType<typeof runCapability>> | undefined;
    try {
      await withAudioFixture("openclaw-auto-audio-bailian", async ({ ctx, media, cache }) => {
        const providerRegistry = buildProviderRegistry({
          openai: {
            id: "openai",
            capabilities: ["audio"],
          ...
          bailian: {
            id: "bailian",
            capabilities: ["audio"],
            transcribeAudio: async (req) => ({ text: "bailian", model: req.model ?? "unknown" }),
          },
        });
        const cfg = {
          models: {
    ...
    if (!runResult) {
      throw new Error("Expected auto audio bailian result");
    }
    expect(runResult.decision.outcome).toBe("success");
    expect(runResult.outputs[0]?.provider).toBe("bailian");
    expect(runResult.outputs[0]?.model).toBe("qwen3-asr-flash");
    expect(runResult.outputs[0]?.text).toBe("bailian");
  });
});
