...
export class MediaAttachmentCache {
    ...
    async ensureLocalStat(entry) {
        ...
        if (!isInboundPathAllowed({ filePath: entry.resolvedPath, roots: this.localPathRoots })) {
            ...
            if (shouldLogVerbose()) {
                logVerbose(`Blocked attachment path outside allowed roots: ${entry.attachment.path ?? entry.attachment.url ?? "(unknown)"}`);
            }
        ...
        try {
            ...
            if (!isInboundPathAllowed({ filePath: canonicalPath, roots: canonicalRoots })) {
                entry.resolvedPath = undefined;
                if (shouldLogVerbose()) {
                    logVerbose(`Blocked canonicalized attachment path outside allowed roots: ${canonicalPath}`);
                }
                throw new MediaUnderstandingSkipError("blocked", `Attachment ${entry.attachment.index + 1} path is outside allowed roots.`);
            }
    ...
    async readLocalBuffer(params) {
        ...
        try {
            ...
            if (!stat.isFile()) {
                throw new MediaUnderstandingSkipError("empty", `Attachment ${params.attachmentIndex + 1} path is not a regular file.`);
            }
            const canonicalPath = await this.resolveCanonicalLocalPath(params.filePath);
            if (!canonicalPath) {
                throw new MediaUnderstandingSkipError("blocked", `Attachment ${params.attachmentIndex + 1} could not be canonicalized.`);
            }
            const canonicalRoots = await this.getCanonicalLocalPathRoots();
            if (!isInboundPathAllowed({ filePath: canonicalPath, roots: canonicalRoots })) {
    ...
    async resolveCanonicalLocalPath(filePath) {
        try {
            ...
            if (shouldLogVerbose()) {
                logVerbose(`Blocked attachment path when canonicalization failed: ${filePath} (${String(err)})`);
            }
            return undefined;
        }
    }
