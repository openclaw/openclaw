...
import { buildModelCatalogMergeKey, buildModelCatalogRef, normalizeModelCatalogProviderId, } from "./refs.js";
...
function normalizeModelCatalogApi(value) {
    ...
    return MODEL_CATALOG_APIS.has(api) ? api : undefined;
}
...
function normalizeModelCatalogStatus(value) {
    ...
    return MODEL_CATALOG_STATUSES.has(status) ? status : undefined;
}
...
export function normalizeModelCatalog(value, params) {
    if (!isRecord(value)) {
        return undefined;
    }
...
export function normalizeModelCatalogRows(params) {
    return Object.entries(params.providers)
        .flatMap(([provider, providerCatalog]) => normalizeModelCatalogProviderRows({ provider, providerCatalog, source: params.source }))
