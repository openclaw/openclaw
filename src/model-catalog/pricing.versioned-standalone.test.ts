...
beforeEach(() => {
    ...
    createPluginMetadataSnapshotFixture({
      plugins: [
        ...
        },
        {
          ...
          modelCatalog: {
            providers: {
              vendor: { models: [{ id: "catalogued" }, { id: "rowpriced" }, { id: "rowfree" }] },
            },
          },
        ...
        },
        {
          ...
          modelPricing: { providers: { openrouter: { openRouter: {}, liteLLM: false } } },
        },
        {
          id: "owner",
          providers: ["owner"],
  ...
  setRemoteModelCatalogOverlaySourcesForTest({
    ...
    readStoredCatalog: () => ({
      id: 1,
      source_url: `https://catalog.openclaw.ai/models/v${wireVersion}/catalog.json`,
      bundle_json: JSON.stringify({
        ...
        upstreamPricing: {
          "vendor/rowfree": { input: 5, output: 10, source: "openRouter", passthroughOnly: true },
          "vendor/listed": {
            input: 2,
            output: 4,
            source: "openRouter",
        ...
        providerPricing: {
          "openrouter/typesafe/free-unlisted": { input: 0, output: 0, source: "openRouter" },
          "vendor/free-untrusted": { input: 0, output: 0, source: "liteLLM" },
...
it.each(
  [
    {
      name: "gateway passes through a vendor rate",
      ref: "gateway/vendor/listed",
      cost: rates(2, 4),
    },
    {
    ...
    {
      name: "known free vendor row wins over a positive upstream tariff",
      ref: "gateway/vendor/rowfree",
      cost: rates(0, 0),
    },
    {
      name: "standalone native OpenRouter zero remains known",
      ref: "openrouter/typesafe/free-unlisted",
      cost: rates(0, 0),
    },
    { name: "standalone third-party zero remains unknown", ref: "vendor/free-untrusted" },
  ...
  ].flatMap((entry) => ([2, 3] as const).map((wire) => Object.assign({ wire }, entry))),
)("v$wire $name", ({ ref, cost, wire }) => {
  wireVersion = wire;
  ...
  expect(
    resolveModelCostConfig({
      config: {
        models: {
          catalogRefresh: {
            url: `https://catalog.openclaw.ai/models/v${wireVersion}/catalog.json`,
          },
        },
      },
