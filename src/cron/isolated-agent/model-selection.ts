  ...
  if (modelOverride !== undefined && modelOverride.length > 0) {
    ...
    if ("error" in resolvedOverride) {
      if (resolvedOverride.error.startsWith("model not allowed:")) {
        getLog().error(
          `cron: explicit payload.model '${modelOverride}' (provider=${resolvedOverride.ref?.provider ?? "?"}, model=${resolvedOverride.ref?.model ?? "?"}) is not in the allowlist. ` +
            `Falling back to provider='${provider}' model='${model}'. ` +
            `This may cause unexpected API calls to wrong provider. ` +
            `Fix: add the model to your allowlist or check agents.defaults.model config. ` +
            `(resolved key: ${resolvedOverride.error})`,
        );
        return {
          ok: true,
          provider,
          model,
          warning: `cron: payload.model '${modelOverride}' not allowed, falling back to defaults (${provider}/${model})`,
        };
      }
