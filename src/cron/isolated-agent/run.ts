...
export async function runCronIsolatedAgentTurn(params: {
  ...
  try {
    ...
    // which in turn take priority over agents.defaults.model.fallbacks.
    // The global default fallbacks are resolved explicitly here so that cron
    // sessions always honor the configured fallback chain even when the merged
    // cfgWithAgentDefaults config is not read by runWithModelFallback (e.g.
    // when the runtime model provider differs from the configured primary
    // provider — see resolveFallbackCandidates provider-mismatch guard).
    const payloadFallbacks =
      params.job.payload.kind === "agentTurn" && Array.isArray(params.job.payload.fallbacks)
        ...
        : undefined;
    const defaultModelFallbacks = resolveAgentModelFallbackValues(
    ...
    const runPrompt = async (promptText: string) => {
      const fallbackResult = await runWithModelFallback({
        ...
        fallbacksOverride:
          ...
          (defaultModelFallbacks.length > 0 ? defaultModelFallbacks : undefined),
