...
export async function runCronIsolatedAgentTurn(params: {
  ...
  // Exclude tool-error warning payloads (isError: true) from summary/output — they are
  // diagnostic annotations, not the agent's actual response text.
  const nonErrorPayloads = payloads.filter((p) => !p?.isError);
  const firstText = nonErrorPayloads[0]?.text ?? payloads[0]?.text ?? "";
  ...
  // `buildEmbeddedRunPayloads` appends tool-error warning payloads *after* the assistant
  // text, so checking only for a non-error payload *after* the last error never fires in
  // the common write-error case (write fails → agent replies normally → warning appended
  // last).  We treat the error as transient when any non-error text payload exists
  // anywhere in the run — before *or* after the error — as long as there was no
  // run-level model/API failure.
  const hasSuccessfulPayloadRelativeToError =
    !runLevelError &&
    lastErrorPayloadIndex >= 0 &&
    payloads.some(
      (payload, idx) =>
        idx !== lastErrorPayloadIndex &&
        payload?.isError !== true &&
        Boolean(payload?.text?.trim()),
    );
  // Tool wrappers can emit transient/false-positive error payloads before or after a
  // valid assistant payload.  Only treat payload errors as fatal when (a) the run itself
  // reported a model/context-level error or (b) there is no non-error text payload
  // anywhere in the run.
  const hasFatalErrorPayload = hasErrorPayload && !hasSuccessfulPayloadRelativeToError;
