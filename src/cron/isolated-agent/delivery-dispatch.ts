...
export async function dispatchCronDelivery(
  ...
  if (params.deliveryRequested && !params.skipHeartbeatDelivery && !skipMessagingToolDelivery) {
    ...
    // Suppress NO_REPLY for direct delivery (thread/structured content) where
    // there is no descendant finalization step.  For the text delivery path,
    // let the flow continue to finalizeTextDelivery which waits for active
    // descendants that may produce real output.
    if (useDirectDelivery && synthesizedText?.toUpperCase() === SILENT_REPLY_TOKEN.toUpperCase()) {
      return {
        result: params.withRunSession({
          status: "ok",
          summary,
