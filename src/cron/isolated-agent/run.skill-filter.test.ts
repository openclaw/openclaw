...
describe("runCronIsolatedAgentTurn — skill filter", () => {
  ...
  function expectDefaultModelCall(params: {
    primary: string;
    fallbacks: string[];
  }) {
  ...
  it("passes agent-level skillFilter to buildWorkspaceSkillSnapshot", async () => {
    ...
    await runSkillFilterCase({
      cfg: {
        agents: {
          list: [{ id: "scout", skills: ["meme-factory", "weather"] }],
        },
      },
      agentId: "scout",
    });
    expect(buildWorkspaceSkillSnapshotMock).toHaveBeenCalledOnce();
    expect(buildWorkspaceSkillSnapshotMock.mock.calls[0][1]).toHaveProperty(
      "skillFilter",
  ...
  it("omits skillFilter when agent has no skills config", async () => {
    ...
    expect(
      buildWorkspaceSkillSnapshotMock.mock.calls[0][1].skillFilter,
    ).toBeUndefined();
  });
  ...
  it("refreshes cached snapshot when skillFilter changes without version bump", async () => {
    ...
    resolveCronSessionMock.mockReturnValue({
      ...
      sessionEntry: {
        ...
        skillsSnapshot: {
          prompt:
            "<available_skills><skill>meme-factory</skill></available_skills>",
  ...
  it("reuses cached snapshot when version and normalized skillFilter are unchanged", async () => {
    ...
    await runSkillFilterCase({
      cfg: {
        agents: {
          list: [{ id: "weather-bot", skills: ["weather", "meme-factory"] }],
        },
      },
      agentId: "weather-bot",
    });
  ...
  describe("model fallbacks", () => {
    ...
    async function expectPrimaryOverridePreservesDefaults(
    ...
    it("preserves defaults when agent overrides primary as string", async () => {
      await expectPrimaryOverridePreservesDefaults(
  ...
  describe("CLI session handoff (issue #29774)", () => {
    ...
    it("reuses stored cliSessionId on continuation runs (isNewSession=false)", async () => {
      ...
      runCliAgentMock.mockResolvedValue({
        ...
        meta: {
          agentMeta: {
            sessionId: "existing-cli-session-def",
            usage: { input: 5, output: 10 },
          },
        },
      });
