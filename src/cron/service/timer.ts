...
export function applyJobResult(
  ...
  if (!shouldDelete) {
    if (job.schedule.kind === "at") {
      ...
      if (transient && !(opts?.preserveSchedule && normalNext !== undefined)) {
        ...
        // Permanent error or preserveSchedule: respect natural schedule.
        // resolveCronNextRunWithLowerBound adds a safety guard for cron-kind jobs
        // that silences unclear schedule expressions instead of entering a tight loop.
