    ...
    if (job.wakeMode === "now" && state.deps.runHeartbeatOnce) {
      ...
      if (heartbeatResult.status === "skipped") {
        if (heartbeatResult.reason === "requests-in-flight") {
          // The main lane is busy, but the event is queued and will be processed
          // when the lane becomes available. Return success to avoid deadlock.
          return { status: "ok", summary: text };
        } else {
          // For other skip reasons (disabled, quiet-hours, etc.), preserve the skip status
