...
import { computeJobNextRunAtMs, nextWakeAtMs, recomputeNextRunsForMaintenance, recordScheduleComputeError, resolveJobPayloadTextForMain, } from "./jobs.js";
...
const DEFAULT_FAILURE_ALERT_COOLDOWN_MS = 60 * 60000; // 1 hour
...
const DEFAULT_BACKOFF_SCHEDULE_MS = [
    30000, // 1st error  →  30 s
    60000, // 2nd error  →   1 min
    5 * 60000, // 3rd error  →   5 min
    15 * 60000, // 4th error  →  15 min
    60 * 60000, // 5th+ error →  60 min
];
...
function resolveRetryConfig(cronConfig) {
    ...
    return {
        ...
        backoffMs: Array.isArray(retry?.backoffMs) && retry.backoffMs.length > 0
            ? retry.backoffMs
...
function resolveFailureAlert(state, job) {
    ...
    return {
        ...
        channel: normalizeCronMessageChannel(jobConfig?.channel) ??
 ...
 */
export function applyJobResult(state, job, result) {
    ...
    if (result.status === "error") {
        ...
        if (alertConfig && job.state.consecutiveErrors >= alertConfig.after) {
            ...
            if (!isBestEffort) {
                ...
                const inCooldown = typeof lastAlert === "number" && now - lastAlert < Math.max(0, alertConfig.cooldownMs);
                if (!inCooldown) {
...
export function armTimer(state) {
    ...
    if (!nextAt) {
        ...
        const withNextRun = state.store?.jobs.filter((j) => j.enabled &&
            typeof j.state.nextRunAtMs === "number" &&
            Number.isFinite(j.state.nextRunAtMs)).length ?? 0;
    ...
    try {
        ...
        const runDueJob = async (params) => {
            ...
            try {
                ...
                state.deps.log.warn({ jobId: id, jobName: job.name, timeoutMs: jobTimeoutMs ?? null }, `cron: job failed: ${errorText}`);
                return {
                    jobId: id,
                    status: "error",
                    error: errorText,
...
export async function executeJobCore(state, job, abortSignal) {
    ...
    if (job.sessionTarget === "main") {
        ...
        if (job.wakeMode === "now" && state.deps.runHeartbeatOnce) {
            ...
            const maxWaitMs = state.deps.wakeNowHeartbeatBusyMaxWaitMs ?? 2 * 60000;
