...
function getCronActiveJobState() {
    return resolveGlobalSingleton(CRON_ACTIVE_JOB_STATE_KEY, () => ({
        activeJobIds: new Set(),
    }));
}
export function markCronJobActive(jobId) {
    if (!jobId) {
        return;
    }
    getCronActiveJobState().activeJobIds.add(jobId);
}
export function clearCronJobActive(jobId) {
    if (!jobId) {
        return;
    }
    getCronActiveJobState().activeJobIds.delete(jobId);
}
export function isCronJobActive(jobId) {
    if (!jobId) {
        return false;
    }
