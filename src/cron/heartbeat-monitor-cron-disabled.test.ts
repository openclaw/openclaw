...
describe("heartbeat monitors are inert while the cron scheduler is off", () => {
  ...
  it("still wakes the agent from the same row when the scheduler is on", async () => {
    ...
    try {
      ...
      const job = await seedMonitor(seeded.cron, 60_000);
      seeded.cron.stop();
      ...
      // Register before advancing: the due tick only reserves the run, and a later
      // scheduler pass dispatches it, so the wait has to be armed up front.
      const finishedRun = enabled.finished.waitForOk(job.id);
