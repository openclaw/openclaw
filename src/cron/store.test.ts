...
const { makeStorePath } = createCronStoreHarness({ prefix: "mullusi-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MULLUSI_HOME for tilde expansion", () => {
    vi.stubEnv("MULLUSI_HOME", "/srv/mullusi-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/mullusi-home", "cron", "jobs.json"));
  });
});
