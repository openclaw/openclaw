...
export async function sendFailureNotificationAnnounce(
  ...
  try {
    // Phase 4 REWORK (origin-respect): failure notifications respect the
    // resolved delivery target instead of rerouting into an operator bucket.
    // If the cron config supplies a valid target, post the failure there;
    // otherwise the caller already short-circuited above.
