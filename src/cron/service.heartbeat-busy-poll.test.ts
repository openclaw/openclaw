...
async function createPollFixture(options: { scratch?: string; isolated?: boolean } = {}) {
  ...
  async function close() {
    ...
    try {
      if (vi.isFakeTimers()) await vi.advanceTimersByTimeAsync(1_000);
    ...
    async ({ scratch }) => {
      await withPollFixture(
        async ({ cron, monitor, storePath, reply, runOnce, request, finished, holdLane }) => {
          ...
          const nextTick = firstTick + EVERY_MS;
          for (const job of [
    ...
    async (outcome) => {
      await withPollFixture(
        async ({ cron, monitor, reply, runOnce, request, finished, holdLane }) => {
          ...
          expect(finished().map((event) => ({ jobId: event.jobId, status: event.status }))).toEqual(
