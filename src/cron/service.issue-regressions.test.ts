...
describe("Cron issue regressions", () => {
  ...
  it("drops a queued manual cron.run result when the job is removed during execution", async () => {
    vi.useRealTimers();
    clearCommandLane(CommandLane.Cron);
    setCommandLaneConcurrency(CommandLane.Cron, 1);
    ...
    let now = dueAt;
    const runDeferred = createDeferred<{ status: "ok"; summary: string; delivered: true }>();
    ...
    const state = createCronServiceState({
      ...
      requestHeartbeatNow: vi.fn(),
      runIsolatedAgentJob: vi.fn(async () => {
        const result = await runDeferred.promise;
        now += 10;
        return result;
      }),
    });
    ...
    await vi.waitFor(() => expect(state.store?.jobs[0]?.state.runningAtMs).toBe(dueAt));
    await fs.writeFile(store.storePath, JSON.stringify({ version: 1, jobs: [] }), "utf-8");
    runDeferred.resolve({ status: "ok", summary: "should be dropped", delivered: true });
  ...
  it("only drops the deleted queued manual run and still executes the next job", async () => {
    vi.useRealTimers();
    clearCommandLane(CommandLane.Cron);
    setCommandLaneConcurrency(CommandLane.Cron, 1);
    ...
    const store = makeStorePath();
    const dueAt = Date.parse("2026-02-06T10:05:02.000Z");
    const first = createDueIsolatedJob({ id: "queued-delete-a", nowMs: dueAt, nextRunAtMs: dueAt });
    const second = createDueIsolatedJob({ id: "queued-keep-b", nowMs: dueAt, nextRunAtMs: dueAt });
    await fs.writeFile(
      store.storePath,
      JSON.stringify({ version: 1, jobs: [first, second] }),
      "utf-8",
    ...
    let now = dueAt;
    const firstRun = createDeferred<{ status: "ok"; summary: string; delivered: true }>();
    const secondRun = createDeferred<{ status: "ok"; summary: string }>();
    const secondStarted = createDeferred<void>();
    const log = createNoopLogger();
    const runIsolatedAgentJob = vi.fn(async (params: { job: { id: string } }) => {
    ...
    await vi.waitFor(() => expect(runIsolatedAgentJob).toHaveBeenCalledTimes(1));
    await fs.writeFile(store.storePath, JSON.stringify({ version: 1, jobs: [second] }), "utf-8");
    firstRun.resolve({ status: "ok", summary: "A dropped", delivered: true });
    ...
    secondRun.resolve({ status: "ok", summary: "B delivered" });
    await vi.waitFor(() => {
      const jobs = state.store?.jobs ?? [];
      expect(jobs.find((job) => job.id === first.id)).toBeUndefined();
      expect(jobs.find((job) => job.id === second.id)?.state.lastStatus).toBe("ok");
    });
