...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "EasyHub-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".easyhub", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<EasyHubConfig> = {},
): EasyHubConfig {
  const base: EasyHubConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".easyhub", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
