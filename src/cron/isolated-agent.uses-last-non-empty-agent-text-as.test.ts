...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("keeps defaults fallbacks when agent model override is a string", async () => {
    await withTempHome(async (home) => {
      const storePath = await writeSessionStore(home);
      const deps: CliDeps = {
        sendMessageWhatsApp: vi.fn(),
      ...
      const billingErr = new Error("credit balance too low") as Error & { status?: number };
      billingErr.status = 402;
      vi.mocked(runEmbeddedPiAgent)
        .mockRejectedValueOnce(billingErr)
        .mockResolvedValueOnce({
          payloads: [{ text: "ok-after-fallback" }],
          meta: {
            durationMs: 5,
      ...
      const res = await runCronIsolatedAgentTurn({
        cfg: makeCfg(home, storePath, {
          agents: {
            ...
            list: [{ id: "main", default: true, model: "openai/gpt-5.3-codex" }],
          },
        }),
        deps,
        job: makeJob({
      ...
      const first = vi.mocked(runEmbeddedPiAgent).mock.calls[0]?.[0] as {
        provider?: string;
        model?: string;
      };
      const second = vi.mocked(runEmbeddedPiAgent).mock.calls[1]?.[0] as {
        provider?: string;
        model?: string;
      };
      expect(first?.provider).toBe("openai");
      expect(first?.model).toBe("gpt-5.3-codex");
