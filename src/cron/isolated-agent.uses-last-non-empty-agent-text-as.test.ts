...
describe("runCronIsolatedAgentTurn", () => {
  ...
  // Regression tests for #30773 — write tool appends error warning LAST (after assistant
  // text), so the old "only look after the last error" check never cleared the error flag.
  it("treats write-tool error as non-fatal when agent produced text before the warning (issue #30773)", async () => {
    await withTempHome(async (home) => {
      // buildEmbeddedRunPayloads always appends tool-error warnings after assistant texts.
      // The write succeeded on disk but the tool reported a partial-write error; the agent
      // still produced its normal text reply.
      mockEmbeddedPayloads([
        { text: "Memory updated successfully.", isError: false },
        {
          text: "⚠️ ✍️ Write: `to ~/agents/main/agent/memory/2026-03-01.md (682 chars)` failed",
          isError: true,
        },
      ]);
      const { res } = await runCronTurn(home, {
      ...
      expect(res.status).toBe("ok");
      expect(res.summary).toBe("Memory updated successfully.");
    });
  });

  it("treats write-tool error as fatal when it is the only payload in the run (issue #30773)", async () => {
    await withTempHome(async (home) => {
      mockEmbeddedPayloads([
        {
          text: "⚠️ ✍️ Write: `to ~/agents/main/agent/memory/2026-03-01.md (682 chars)` failed",
  ...
  it("keeps write-tool error fatal when run-level error is also present (issue #30773)", async () => {
    await withTempHome(async (home) => {
      vi.mocked(runEmbeddedPiAgent).mockResolvedValue({
        payloads: [
