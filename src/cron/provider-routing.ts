...
import type { CronJob, CronPacingProviderTarget } from "./types.js";
...
type ScoreCard = Record<CronPacingProviderTarget, number>;
...
export function inferCronProviderTarget(
  ...
  const ranked = (Object.entries(scores) as Array<[CronPacingProviderTarget, number]>).toSorted(
    (a, b) => b[1] - a[1],
  );
...
function matchesProviderTargetRef(
  providerTarget: CronPacingProviderTarget,
  rawRef: string,
): boolean {
...
export function resolveCronProviderRouting(params: {
  job: Pick<CronJob, "agentId" | "name" | "description" | "payload" | "pacing">;
  provider: string;
  model: string;
  configuredModelRefs?: string[];
