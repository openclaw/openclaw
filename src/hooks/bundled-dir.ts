...
export function resolveBundledHooksDir(): string | undefined {
  const override = process.env.SMART_AGENT_NEO_BUNDLED_HOOKS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  // This path works when installed via npm: node_modules/smart-agent-neo/dist/hooks/bundled-dir.js
  try {
