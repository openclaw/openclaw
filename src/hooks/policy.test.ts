...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "mullusi-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "mullusi-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "mullusi-bundled");
      const plugin = makeHookEntry("shared", "mullusi-plugin");
      const managed = makeHookEntry("shared", "mullusi-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("mullusi-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "mullusi-managed");
      const workspace = makeHookEntry("shared", "mullusi-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "mullusi-workspace");
      const second = makeHookEntry("shared", "mullusi-workspace");
      second.hook.handlerPath = "/tmp/mullusi-workspace/shared/handler-2.js";
