...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "activi-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ACTIVI_BUNDLED_HOOKS_DIR"]);
    process.env.ACTIVI_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
