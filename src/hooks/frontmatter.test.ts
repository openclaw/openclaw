...
`;
    ...
    expect(parsed.mullusi.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.mullusi.emoji).toBe("📝");
    expect(parsed.mullusi.events).toEqual(["command"]);
    expect(parsed.mullusi.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.mullusi.install[0].kind).toBe("bundled");
  });
...
describe("resolveMullusiMetadata", () => {
  it("extracts mullusi metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMullusiMetadata(frontmatter);
  ...
  it("returns undefined when mullusi key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        mullusi: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Mullusi" },
            { id: "npm", kind: "npm", package: "@mullusi/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@mullusi/hook");
  });
...
`;
    ...
    const mullusi = resolveMullusiMetadata(frontmatter);
    expect(mullusi).toBeDefined();
    expect(mullusi?.emoji).toBe("💾");
    expect(mullusi?.events).toEqual(["command:new", "command:reset"]);
    expect(mullusi?.requires?.config).toEqual(["workspace.dir"]);
    expect(mullusi?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const mullusi = resolveMullusiMetadata(frontmatter);
    expect(mullusi?.emoji).toBe("disk");
    expect(mullusi?.events).toEqual(["command:new"]);
  });
});
