...
`;
    ...
    expect(parsed.EasyHub.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.EasyHub.emoji).toBe("📝");
    expect(parsed.EasyHub.events).toEqual(["command"]);
    expect(parsed.EasyHub.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.EasyHub.install[0].kind).toBe("bundled");
  });
...
describe("resolveEasyHubMetadata", () => {
  it("extracts EasyHub metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveEasyHubMetadata(frontmatter);
  ...
  it("returns undefined when EasyHub key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        EasyHub: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with EasyHub" },
            { id: "npm", kind: "npm", package: "@easyhub/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@easyhub/hook");
  });
...
`;
    ...
    const EasyHub = resolveEasyHubMetadata(frontmatter);
    expect(EasyHub).toBeDefined();
    expect(EasyHub?.emoji).toBe("💾");
    expect(EasyHub?.events).toEqual(["command:new"]);
    expect(EasyHub?.requires?.config).toEqual(["workspace.dir"]);
    expect(EasyHub?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const EasyHub = resolveEasyHubMetadata(frontmatter);
    expect(EasyHub?.emoji).toBe("disk");
    expect(EasyHub?.events).toEqual(["command:new"]);
  });
});
