...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.dna.emoji).toBe("📝");
    expect(parsed.dna.events).toEqual(["command"]);
    expect(parsed.dna.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.dna.install[0].kind).toBe("bundled");
  });
...
describe("resolveDNAMetadata", () => {
  it("extracts dna metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveDNAMetadata(frontmatter);
  ...
  it("returns undefined when dna key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        dna: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with DNA" },
            { id: "npm", kind: "npm", package: "@dna/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@dna/hook");
  });
...
`;
    ...
    const dna = resolveDNAMetadata(frontmatter);
    expect(dna).toBeDefined();
    expect(dna?.emoji).toBe("💾");
    expect(dna?.events).toEqual(["command:new"]);
    expect(dna?.requires?.config).toEqual(["workspace.dir"]);
    expect(dna?.install?.[0].kind).toBe("bundled");
  });
