...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevBundledDir = process.env.EASYHUB_BUNDLED_HOOKS_DIR;
    process.env.EASYHUB_STATE_DIR = path.join(baseDir, "state");
    process.env.EASYHUB_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.EASYHUB_BUNDLED_HOOKS_DIR;
    } else {
      process.env.EASYHUB_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          EasyHub: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
