...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "dna-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevBundledDir = process.env.DNA_BUNDLED_HOOKS_DIR;
    process.env.DNA_STATE_DIR = path.join(baseDir, "state");
    process.env.DNA_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.DNA_BUNDLED_HOOKS_DIR;
    } else {
      process.env.DNA_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
