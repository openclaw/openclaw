...
This directory contains hooks that ship with Activi. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.activi/activi.json`:
...
2. Restart gateway: `pkill -9 -f 'activi.*gateway' && pnpm activi gateway`
3. Enable the hook: `activi hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.activi.ai/automation/hooks
