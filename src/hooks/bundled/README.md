...
This directory contains hooks that ship with SmartAgentNeo. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.smart-agent-neo/smart-agent-neo.json`:
...
2. Restart gateway: `pkill -9 -f 'smart-agent-neo.*gateway' && pnpm smart-agent-neo gateway`
3. Enable the hook: `smart-agent-neo hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.smart-agent-neo.ai/automation/hooks
