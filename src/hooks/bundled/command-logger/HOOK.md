...
homepage: https://docs.activi.ai/automation/hooks#command-logger
metadata:
  {
    "activi":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.activi/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.activi/logs/commands.log ~/.activi/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/activi`:
   ```
   /home/username/.activi/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
