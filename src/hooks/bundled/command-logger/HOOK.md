...
2. **Appends to log file** - Writes a JSON line to `~/.dna/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.dna/logs/commands.log ~/.dna/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/dna`:
   ```
   /home/username/.dna/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
