...
homepage: https://docs.mullusi.com/automation/hooks#command-logger
metadata:
  {
    "mullusi":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.mullusi/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.mullusi/logs/commands.log ~/.mullusi/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/mullusi`:
   ```
   /home/username/.mullusi/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
