...
function hasEntryCredential(
	...
	if (configuredRef?.source === "env") {
		return Boolean(
			normalizeSecretInput(process.env[configuredRef.id]) ||
				readProviderEnvValue(provider.envVars),
		);
	}
...
export function resolveWebSearchDefinition(
	...
	const runtimeWebSearch =
		options?.runtimeWebSearch ?? getActiveRuntimeWebToolsMetadata()?.search;
