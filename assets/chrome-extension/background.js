...
function scheduleReconnect() {
  ...
  console.log(
    `Scheduling reconnect attempt ${reconnectAttempt} in ${Math.round(delay)}ms`,
  );
...
async function onRelayMessage(text) {
  ...
  if (
    ...
    msg.id === relayConnectRequestId
  ) {
    relayConnectRequestId = null;
    if (!msg.ok) {
      const detail =
        msg?.error?.message || msg?.error || "gateway connect failed";
  ...
  if (method === "Target.closeTarget") {
    ...
    try {
      ...
      if (isLastRemainingTab(allTabs, toClose)) {
        console.warn(
          "Refusing to close the last tab: this would kill the browser process",
        );
    ...
    try {
      ...
      if (!relayUp) {
        ...
        void chrome.action.setTitle({
          tabId,
          title:
            "OpenClaw Browser Relay: attached, waiting for relay reconnect…",
        });
      }
      return;
    } catch {
...
chrome.debugger.onEvent.addListener(
  (...args) => void whenReady(() => onDebuggerEvent(...args)),
);
chrome.debugger.onDetach.addListener(
  (...args) => void whenReady(() => onDebuggerDetach(...args)),
);
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  for (const [tabId, tab] of tabs.entries()) {
    if (tab.state === "connected") {
      setBadge(
        tabId,
        relayWs && relayWs.readyState === WebSocket.OPEN ? "on" : "connecting",
      );
    }
  }
