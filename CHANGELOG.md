...
- Security audit/Windows ACL localization: treat localized `NT AUTHORITY\\SYSTEM` account names (for example French and Russian variants) as trusted during ACL classification, preventing false-positive writable-config findings on localized Windows installs. Fixes #29681.
