...
- Agents/Session transcript repair: when persisted `toolResult.toolName` is blank/whitespace, recover it from the matching assistant tool call during pairing repair and guard persistence-time writes to avoid re-saving empty tool names, reducing repeated `Tool  not found` loops in bridged sessions. (#30723)
