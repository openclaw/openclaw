...
- Sandbox/Workspace mkdirp: allow `mkdir -p` against existing in-workspace directories in the sandbox FS bridge so `write`/`apply_patch` flows with `workspaceAccess: "rw"` no longer fail with boundary-check errors on pre-existing subdirectories. (#30582)
