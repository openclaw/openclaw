...
- Telegram/native command sync: avoid `deleteMyCommands` before non-empty `setMyCommands` updates so restart-time command sync uses one API call instead of two, reducing 429 risk during repeated gateway restarts while still clearing menus when command registration is empty. (#32017) Thanks @liuxiaopai-ai.
- Webchat/chat.history silent token filtering: hide assistant `NO_REPLY`-only transcript entries from chat history responses so internal/silent turns are not rendered as visible chat bubbles. (#32015) Thanks @liuxiaopai-ai.
