...
- Sandbox/FS bridge mkdirp: map container-style `cwd` values (for example `/workspace`) to host mount roots before relative path resolution so directory creation inside mounted workspaces no longer fails boundary checks. (Related #31438)
