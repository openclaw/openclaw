...
- Agents/Failover: classify `network_error` stop-reason errors (`Unhandled stop reason: network_error`) as timeout-class failures so configured model fallback chains trigger instead of surfacing raw network failures.
- Agents/Failover: classify `model_context_window_exceeded` stop-reason errors (`Unhandled stop reason: model_context_window_exceeded`) as timeout-class failures so configured model fallback chains trigger, which can help if fallback models have larger context windows.
