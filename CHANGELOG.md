...
- Telegram/Outbound recipient allowlist: add optional `channels.telegram.allowTo` (plus per-account override) to hard-enforce outbound recipient chat IDs in `message.send`/Telegram send paths, blocking sends when the resolved `chat_id` is not allowlisted while preserving default unrestricted behavior when unset. (#30560)
