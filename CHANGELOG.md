...
- Config/runtime snapshot refresh: update the in-memory runtime config snapshot after config writes so back-to-back gateway mutations on separate connections see newly persisted agents/config instead of stale pre-write state. (#37175) Thanks @liuxiaopai-ai.
