...
- Browser/CDP loopback probes: bypass proxy dispatchers for loopback `/json/version` reachability checks so browser control startup is not broken by global `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY` settings. (#31219)
