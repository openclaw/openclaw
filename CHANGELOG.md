...
- Telegram: normalize hyphenated command names to underscores so skills like `export-session` register as valid `/export_session` bot commands, and apply normalization consistently to handler registration, `reservedCommands`, and `existingCommands` collision detection. (#18683)
