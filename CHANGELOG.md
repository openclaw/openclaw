...
- Slack/implicit mention session timeout: thread participation now respects session timeout config (e.g., `reset: { mode: "idle", idleMinutes: 1 }`), so the bot stops auto-replying in stale threads without an explicit @mention. Prevents unwanted noise in channels where `requireMention: true` should apply to all messages including thread replies. Optimized to cache session timestamp from freshness check and reuse it for thread context loading, reducing I/O from 2 reads to 1 read per thread message (~25-30ms savings). (from #34389)
