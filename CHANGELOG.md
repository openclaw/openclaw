...
- WhatsApp/sendPolicy: persist inbound messages to session transcript when `sendPolicy` is `deny`, so read-only logging configurations (spy mode) still record group messages to `.jsonl` history instead of silently dropping them before persistence. (#32341) Thanks @stone-jin.
