...
- Agents/Compaction safeguard: when no real conversation messages are available to summarize, return a minimal compaction result instead of cancelling the compaction flow, avoiding embedded-run `400 status code (no body)` failures while preserving context reduction. (#32759) Thanks @liuxiaopai-ai.
