...
- Thinking/custom model compat: allow `models.providers.*.models[*].compat.supportsXHighThinking` to opt custom model refs into `xhigh`, so OpenClaw no longer hard-downgrades those refs to `high` just because they are not in the built-in allowlist.
