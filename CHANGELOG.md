...
- CLI/status: break the `/session` plugin-runtime import cycle that could send built `status --json` into a hot CPU loop when configured channels triggered plugin-registry loading, and add a built-CLI regression test for parseable JSON output on that path.
