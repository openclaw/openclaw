...
- Google/cache: parse Gemini CLI JSON `response` and `stats` fields so `google-gemini-cli` replies keep their text plus cache-aware usage reporting, and pass explicit `cachedContent` handles through direct Google transport params so prebuilt Gemini caches can be targeted again. Thanks @vincentkoc.
