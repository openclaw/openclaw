...
- Tools/Image custom providers: resolve image models from `openclaw.json` provider definitions when registry discovery misses providers using `apiKey` SecretInput values (for example `{ env: "..." }`), so configured custom image models no longer fail with `Unknown model`. (#31486)
