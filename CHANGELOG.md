...
- Browser/Profile defaults: prefer `openclaw` as the default browser profile on desktop installs (including GUI environments) when `browser.defaultProfile` is not explicitly set, while keeping the `chrome` relay profile available for explicit opt-in. (#31907)
