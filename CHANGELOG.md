...
- Gateway/WebSocket chat routing: scope `chat` event fanout to each websocket client's session interests learned from `chat.send`/`chat.history`, while preserving legacy full-feed behavior for unbound clients and admin monitor scopes. (#32579)
- iMessage/self-chat echo suppression: cache dropped inbound `is_from_me=true` payloads into the monitor echo cache so mirrored `is_from_me=false` self-chat duplicates are filtered before dispatch, preventing self-addressed echo loops. (#32166) Thanks @liuxiaopai-ai.
