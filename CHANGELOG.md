...
- Daemon/service PATH hygiene: include `~/.npm-global/bin` and Linux `~/.nvm/current/bin` in generated service PATH only when those directories actually exist, preventing stale/non-existent entries from being reintroduced by `doctor --fix`. Fixes #32448. Thanks @cliffchen.
