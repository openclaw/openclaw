...
- Agents/Gemini tool schema cleaning: apply Google tool-schema keyword stripping to `google-generative-ai` runs as well as `google-gemini-cli`, preventing `patternProperties` / `additionalProperties` payload rejections that could brick Gemini-backed sessions after a model switch. (openclaw#36672)
