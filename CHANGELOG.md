...
- Gateway/Connection overrides: honor `OPENCLAW_GATEWAY_URL` (fallback `CLAWDBOT_GATEWAY_URL`) and treat CLI `--url` as the explicit override requiring explicit token/password. (#23005) Thanks @vincentkoc.
...
- Subagents/runtime session tracking: use the sandbox session key consistently for subagent runtime handoffs, tool session wiring, and completion propagation to avoid cross-session routing drift. (#21113) Thanks @vincentkoc.
- Sandbox env injection: make sure `sandbox.docker.env` values are passed as `--env` during container creation to keep setup/entrypoint commands seeing resolved container env. (#17923) Thanks @vincentkoc.
...
- Browser sandbox hardening: disable 3D APIs/extensions/software rasterizer path, run without zygote, and cap renderer processes for Docker/LXC stability. (#23889) Thanks @vincentkoc.
