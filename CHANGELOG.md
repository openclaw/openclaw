...
- Gateway/macOS LaunchAgent restart: when `launchctl kickstart` returns a transient "Could not find service" right after bootstrap, briefly wait for launchd registration before failing so `openclaw gateway restart` does not false-fail on some macOS setups. (#31280)
