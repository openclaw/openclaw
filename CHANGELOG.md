...
- Config/plugins memory backend migration: auto-migrate legacy `plugins.slots.memory="memory"` to `memory-core` during config load so upgraded installs stop selecting the legacy mem0 plugin path that fails with sqlite binding resolution on current jiti runtime. (#33048)
