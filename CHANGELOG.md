...
- Daemon/macOS LaunchAgent install: preflight LaunchAgent stdout/stderr log targets for writability and fail with explicit guidance before bootstrap, preventing silent `EX_CONFIG` crash loops when log files are not writable.
