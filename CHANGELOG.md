...
- Logging/file path expansion: expand `~` in `logging.file` before logger directory creation so gateway startup does not fail with `ENOENT` when users configure tilde-prefixed log paths. (#30401) Thanks @liuxiaopai-ai.
