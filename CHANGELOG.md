...
- Telegram/native command sync: replace startup `deleteMyCommands` preflight with a single `setMyCommands` write for non-empty menus (and skip API calls for empty menus) to reduce restart-time API pressure and lower 429 lockout risk during frequent gateway restarts. (#32017) Thanks @liuxiaopai-ai.
