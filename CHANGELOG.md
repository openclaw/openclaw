...
- Agents/Google Gemini 3.1 forward-compat: extend the gemini-3.1-pro/flash forward-compat fallback to also cover the `google` (API key / `google-generative-ai`) provider so `google/gemini-3.1-flash-lite-preview` and similar models in fallback arrays no longer fail with "Unknown model" — previously only the `google-gemini-cli` (OAuth) path was covered. (#36111)
