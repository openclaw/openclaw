...
- Anthropic transcript replay signatures: preserve Anthropic/Bedrock thinking signatures in transcript policy (`preserveSignatures=true` for Anthropic-compatible providers) so replayed thinking blocks stay byte-identical and avoid Anthropic API rejection on modified `thinking`/`redacted_thinking` content. (#32526)
