...
- Nextcloud Talk/plugin-sdk compatibility: export `waitForAbortSignal` from `openclaw/plugin-sdk` and switch Nextcloud Talk to consume the public export so built/distributed installs no longer fail on missing relative `src/infra` import paths. Fixes #30057.
