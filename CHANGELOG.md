...
- Doctor/Migrations: auto-prune stale removed `google-antigravity-auth` plugin references from `plugins.entries`, `plugins.installs`, `plugins.allow`, `plugins.load.paths`, and `plugins.slots.memory` during doctor compatibility normalization, so upgrades no longer require manual config cleanup after plugin removal. (#27371) Thanks @vincentkoc.
