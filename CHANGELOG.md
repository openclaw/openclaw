...
- CLI/Startup follow-up: add root `--help` fast-path bootstrap bypass, lazily resolve CLI channel options only when commands need them, generate build-time startup metadata (`dist/cli-startup-metadata.json`) so runtime can avoid catalog scans on low-power hosts, add an experimental bundled CLI entry build (`dist/entry.bundle.mjs`) with env-gated runtime opt-in (`OPENCLAW_CLI_BUNDLED_ENTRY=1`), and add an experimental snapshot build helper (`pnpm build:cli-snapshot`). (#30975) Thanks @vincentkoc.
