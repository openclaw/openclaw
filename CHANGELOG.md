...
- Telegram/reply hook parity: run `message_sending` and `message_sent` plugin hooks from the Telegram `deliverReplies` path so direct Telegram reply sends no longer bypass outbound hook integrations used by plugins. (#32581)
