...
=======
- Gateway/CLI: use `wss://` instead of `ws://` when `gateway.tls.enabled` is true, fixing connection failures when TLS is configured; also fix `onboard-remote` to suggest `wss://` when beacon advertises TLS. (#21736) Thanks @arosstale.
>>>>>>> b1b996d40 (fix(gateway): use wss:// when gateway.tls.enabled is true)
