...
- TUI/chat history: hide `user` transcript entries tagged with `inputProvenance.kind="internal_system"` (including pre-compaction memory flush prompts), and tag memory-flush runs with internal-system provenance so internal prompts no longer appear in interactive chat history surfaces. (#32057) Thanks @liuxiaopai-ai.
