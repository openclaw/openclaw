...
- Models/Merge-mode provider sync: when refreshing `agents/*/agent/models.json`, keep existing provider `apiKey` but no longer pin stale `baseUrl` over an updated config `models.providers.*.baseUrl`, so endpoint changes in `openclaw.json` are applied on rebuild. (#30667) Thanks @liuxiaopai-ai.
