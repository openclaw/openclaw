...
- Gateway/chat.inject: create transcript file when missing (pass `createIfMissing: true`) so ACP oneshot/run sessions that don't pre-create the transcript can receive injected messages and have working history, instead of hard-failing with "transcript file not found". (#36170)
