...
- WhatsApp/Reactions messageId fallback: auto-inject `messageId` for WhatsApp `react` actions from inbound tool context when omitted, and accept context aliases (`CurrentMessageId`, `MessageSid*`) in the WhatsApp action handler so reactions no longer fail with `messageId required` on inbound-triggered turns. (#31303) Thanks @VCSMO.
