...
- Model selection/OpenAI-compatible routing: infer a unique provider for unprefixed `agents.defaults.model.primary` entries from configured providers/auth profiles (instead of always forcing Anthropic), and allow `openai/*` unknown-model passthrough when `OPENAI_BASE_URL` (or `models.providers.openai.baseUrl`) is set so OpenAI-compatible endpoints can accept custom model IDs like `deepseek-chat`. (Related #31850)
