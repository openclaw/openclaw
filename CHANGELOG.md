...
- Sandbox/FS bridge mkdirp: allow pre-existing directory targets during boundary validation so sandboxed `write`/`edit` flows can create parent directories under `/workspace` without false `Sandbox boundary checks failed` errors when `workspaceAccess: rw`. (#30513)
