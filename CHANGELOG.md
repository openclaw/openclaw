...
- Usage telemetry/fail-open: guard provider usage fetch tasks so network/provider errors (including Anthropic geo-block responses) return provider error snapshots instead of rejecting the whole usage-summary load path. (#30672) Thanks @liuxiaopai-ai.
