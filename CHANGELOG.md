...
- Agents/OpenAI-compatible tool IDs: enable strict tool-call ID sanitization for `openai-completions` transcript policy paths, capping tool call IDs to provider-safe lengths so OpenAI-compatible chat-completions requests no longer fail on overlong `tool_call_id` values. (#31934)
- CLI/Profile dotenv ordering: apply `--profile` env defaults before `loadDotEnv()` in direct `runCli` entry paths, so profile-scoped `.env` files are selected correctly instead of always loading the default profile `.env` first. (#31936)
