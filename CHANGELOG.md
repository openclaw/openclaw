...
- CLI/agent local browser cleanup: stop the in-process browser control service after embedded `openclaw agent` runs (including gateway-fallback-to-local), so Browser tool usage no longer leaves CLI turns hanging after final output. (#36388) Thanks @liuxiaopai-ai.
...
- Skills/skill-creator trigger guidance: broaden metadata trigger wording to include edit/refactor workflows and add an explicit early trigger rule for `SKILL.md`/skill-directory edits so skill-authoring tasks load the correct guidance more reliably. (#36039) Thanks @haynzz.
- Doctor/config repair baseline: preserve account-only Telegram configs by applying repair writes from pre-default resolved config, preventing synthetic top-level `dmPolicy/groupPolicy/streaming` injection and false empty-group-allowlist warnings during doctor-driven repairs. (#35560)
- Discord/gateway shutdown safety: attach a fallback gateway `error` listener so late reconnect-limit emissions (for example `Max reconnect attempts (0) reached`) do not surface as uncaught process-level exceptions during monitor stop/reload races. (#36055)
- Gateway/health-monitor stale-socket detection: avoid treating channels as stale before their first observed event so idle long-lived transports (for example BlueBubbles) are not restarted in false-positive stale cycles. (#36059)
- Gateway/origin diagnostics: include observed websocket `origin` and `host` values in `origin not allowed` handshake errors to make LAN/reverse-proxy Control UI allowlist mismatches easier to fix. (#36056)
...
- Daemon/systemd user-scope fallback: retry `systemctl --user is-enabled` via machine-user scope when direct user-scope output is only a generic `Command failed: systemctl --user ...` wrapper, while preserving disabled/not-found state short-circuit handling. (#36008) Thanks @liuxiaopai-ai.
- Telegram/media download resilience: retry recoverable `fetch_failed` file-download errors once before skipping media-group items, reducing intermittent network/undici dropouts on multi-photo batches. (#36010) Thanks @liuxiaopai-ai.
- Matrix/send rate-limit handling: retry one send attempt on `M_LIMIT_EXCEEDED` using `retry_after_ms` (with bounded fallback) so transient homeserver throttling does not silently drop replies. (#36027) Thanks @liuxiaopai-ai.
- Memory/SQLite crash resilience: default memory index databases to `journal_mode=WAL` at open time (best-effort), reducing corruption risk during abrupt restarts. (#36035) Thanks @nguathen.
