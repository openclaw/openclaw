...
- Security/MSTeams: close sender allowlist bypass when route allowlist is configured so an empty sender allowlist with an active route allowlist now means deny-all rather than allow-all. (GHSA-g7cr-9h7q-4qxq) Thanks @02-Coding.
...
- Security/session kill HTTP scopes: require `operator.admin` for `/sessions/:sessionKey/kill` HTTP endpoint so non-admin callers cannot terminate arbitrary sessions. Thanks @AntAISecurityLab and @vincentkoc.
