...
- Gateway/readiness probes: make `/ready` and `/readyz` report managed channel readiness with startup grace while keeping `/health` and `/healthz` as shallow liveness probes, so orchestrators wait for channels to come online before routing traffic. (follow-up to #31272; from #27139) Thanks @mahsumaktas.
...
- Gateway/readiness probes: make `/ready` and `/readyz` report channel-backed readiness with startup grace, `503` on disconnected managed channels, and typed internal-fallback responses, while keeping `/health` and `/healthz` as shallow liveness probes. Thanks @mahsumaktas.
