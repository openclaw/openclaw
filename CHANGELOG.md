...
- Discord/Reactions: pass `accountId` to `reactMessageDiscord` and `removeReactionDiscord` in `processDiscordMessage` so ack reactions work correctly with named accounts that have no root `channels.discord.token`. Previously the token resolver fell back to account `"default"`, which has no token in named-account setups, causing ack reactions to silently fail. (#22938) Thanks @arosstale.
