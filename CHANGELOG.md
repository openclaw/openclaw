...
- Elevated mode: default `elevatedDefault` to `"off"` instead of `"on"` when `tools.elevated.enabled` is true, so exec commands are not silently routed through the approval gate in non-interactive sessions. (#18177)
