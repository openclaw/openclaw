...
- Plugins/duplicate bundled-vs-global id: stop emitting a false-positive "duplicate plugin id" warning when a user-installed (global) plugin has the same ID as a bundled channel plugin; silently prefer the higher-precedence global version (matching the declared `config > workspace > global > bundled` priority) so users who install a feishu/zalo/etc. extension don't see repeated warnings on every config reload. (#35884)
