...
- Plugins/Jiti native sqlite loading: mark `sqlite3` and `better-sqlite3` as native Jiti modules in plugin-loader imports so memory-slot plugins that rely on sqlite native bindings avoid addon path resolution failures on global installs. (#32095) Thanks @liuxiaopai-ai.
