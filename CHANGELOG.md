...
- Agents/system prompt: add **The Angle** section (pressure naming, frame matching, complexity assessment, urgency, self-check) and rewrite the Memory section as calibration with a four-zone MEMORY.md structure (recovery / routing / state / index) and four moments (WRITE / RETRIEVE / MAINTAIN / CHECK). Also extends the MS Teams thumbs-down reflection prompt with pressure / frame / failure-mode questions.
