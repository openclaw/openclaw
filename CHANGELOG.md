...
- Discord/Slash command completion: send fallback interaction acknowledgement when native command dispatch emits no reply payload (for example when `message` tool already delivered output), preventing slash commands from hanging at "Bot is responding...". (#31246)
