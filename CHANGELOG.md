...
- Tests/Subagent announce fast mode: set `OPENCLAW_TEST_FAST=1` before importing `subagent-announce` in `subagent-announce.format.test.ts` so Windows shards consistently use fast polling constants and avoid timing-flaky retries in completion synthesis assertions. (#31298) Thanks @ktamas77.
