...
- Exec approvals/wildcard allowlist fanout: make `openclaw approvals allowlist add|remove --agent "*"` apply to existing concrete agent IDs (instead of writing only `agents["*"]`), so wildcard CLI updates are reflected in runtime-checked agent allowlists. (#33268) Thanks @aboveUAV.
