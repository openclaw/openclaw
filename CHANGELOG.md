...
- Agents/sessions_spawn: within 60s, return the prior `childSessionKey`/`runId` when the same requester repeats an identical spawn (target agent, task text, UTC minute bucket, and spawn variant), with `agents/sessions-spawn-dedup` logs on hits; skipped for attachments, subagent attach mounts, and ACP `resumeSessionId`.
- Config/agents: add optional `agents.list[].tools.maxResponseTokens` to cap completion/output tokens (merged as `min(existing maxTokens, maxResponseTokens)` into the embedded stream).
