...
- CLI/Memory search: run a foreground memory sync before `openclaw memory search` when the index is dirty, so one-shot CLI searches do not return stale empty results while async sync is still in progress. (#30587) Thanks @StarMatrixSky.
