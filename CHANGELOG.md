...
- CLI/Update: prevent double restart after service-env refresh — mark restart as initiated after `refreshGatewayServiceEnv` succeeds so `runRestartScript`/`runDaemonRestart` do not also fire and trigger a second startup-hook cycle. (#21591) Thanks @irchelper.
