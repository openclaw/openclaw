...
- Gateway: add node command allowlists (default-deny unknown node commands; configurable via `gateway.nodes.allowCommands` / `gateway.nodes.denyCommands`).
- Plugins: add `device-pair` (Telegram `/pair` flow) and `phone-control` (iOS/Android node controls).
...
- Gateway: stabilize chat routing by canonicalizing node session keys for node-originated chat methods.
