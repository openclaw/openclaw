...
- Matrix/Tool action routing: fix `accountId` not propagated through the Matrix tool adapter chain; isolated cron and subagent sessions were using the first active account by Map insertion order instead of the agent's bound account, silently routing messages from the wrong Matrix identity and triggering `M_FORBIDDEN` errors in account-restricted rooms. (#26457)
