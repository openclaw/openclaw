...
- Control UI/Config form union support: treat `string | object` union schemas (including `SecretInputSchema`) as editable in Form mode, so `models.providers` no longer collapses to unsupported when provider `apiKey` supports secret refs. Fixes #31490.
