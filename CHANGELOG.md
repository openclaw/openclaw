...
- Slack/App Home tab: add App Home tab support with default view (agent name, status, uptime, getting started instructions), per-user version caching, in-flight dedup, account-scoped state, and agent-driven `updateHomeTab`/`resetHomeTab` actions for dynamic Block Kit content. Configurable via `channels.slack.homeTab`. (#23268) Thanks @slurpyone.
