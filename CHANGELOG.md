...
- Daemon/Linux gateway install: treat `systemctl --user is-enabled` `not-found` stdout as a disabled/missing-unit state even when stderr is replaced by generic command-failure text, so fresh installs proceed instead of aborting. (#32635) Thanks @neo-operator.
