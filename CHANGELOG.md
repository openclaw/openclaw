...
- Agents/background exec chat notifications: treat chat-context background exec completions as notify-on-empty-success by default so `exit 0` tasks without stdout/stderr stop looking like "started, then silence" on chat surfaces.
