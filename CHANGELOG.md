...
- Hooks/session-memory on delete: trigger the `session-memory` hook before session deletion so conversations from non-TUI channels (Telegram, Discord, Feishu, etc.) that rely on session timeouts are archived to workspace memory instead of being silently lost. (#37027)
