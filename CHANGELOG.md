...
- Media/relative path resolution: resolve relative `MEDIA:` paths against configured allowed media roots instead of the gateway CWD, making agent-generated relative file references deterministic and correcting the misleading prompt guidance about supported path formats. (#37111)
...
- Agents/Anthropic effort override: pass through user-configured `params.effort` to the Anthropic API payload (`output_config.effort`) so adaptive thinking with explicit effort levels (e.g. `effort: "max"` for Opus 4.6) is honored instead of defaulting to the mapped thinking-level effort. (#37097)
...
- Telegram/NO_REPLY streaming leak: suppress `NO_REPLY` partial streaming fragments (e.g. the transient `NO` prefix) from reaching the draft stream so they cannot be flushed as a real Telegram message when the stream finalises (bypassing the minInitialChars debounce). (#36811)
