...
- Gateway/Plugin HTTP ordering: move canvas-http catch-all handler registration to after plugin request stages so plugin POST endpoints are no longer intercepted with 405 before plugin auth/routing runs. (#32682)
...
- Agents/Owner-only tool policy: remove dead wrapping pass in `applyOwnerOnlyToolPolicy` when sender is not owner; owner-only tools are now filtered directly without creating error-throwing wrappers that are immediately discarded.
- Agents/OpenAI Codex OAuth error: improve no-API-key error message for OpenAI provider when Codex OAuth is configured to also mention `openai/gpt-5.3-codex` as an option alongside `gpt-5.1-codex`.
- Agents/Forward-compat fallback: add `gpt-5.1-codex` as a secondary template fallback for `gpt-5.3-codex` forward-compat resolution so users with older model registries lacking `gpt-5.2-codex` are not forced to the hardcoded defaults.
- Feishu/Interactive card quoting: fix `parseInteractiveCardContent` to extract text from schema 2.0 interactive cards (body.elements) introduced in v2026.3.1 so quoted card messages are not shown as "[Interactive Card]". (#32712)
- TypeScript/Skills runtime test: fix TS2556 spread-type error in `skills-runtime.test.ts` mock wrapper for `loadWorkspaceSkillEntries`.
- Discord/ACP inline action: register acp and subagents command action arg as autocomplete so Discord passes typed values through instead of requiring dropdown selection, fixing "/acp close" ignoring the inline arg. (#32753)
