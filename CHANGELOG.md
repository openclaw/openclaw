...
- Gateway/Profiles: treat `OPENCLAW_PROFILE` as a first-class profile source for state/config derivation, accept `--profile` after subcommand position, and hard-stop `gateway --dev --reset` when targets are not dev-isolated so dev resets never trash default-profile state. (#17833)
