...
- Config/agent MCP schema: accept `agents.list[].mcp.servers` in `openclaw.json` so agent-level MCP server blocks no longer fail strict config validation with `Unrecognized key: "mcp"`. (#32583)
