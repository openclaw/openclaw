...
- Tools/Brave search_lang validation: align Brave `web_search` language guidance and normalization with provider-supported localized codes (for example `zh-hans`, `zh-hant`, `en-gb`, `pt-br`) so Chinese searches no longer fail from the old 2-letter-only constraint. (#37260) Thanks @liuxiaopai-ai.
