...
- Web UI/config form: treat `additionalProperties: true` object schemas as editable map entries (JSON values) instead of flagging them unsupported, so Telegram fields like Accounts/Capabilities/Custom Commands render in form mode. (#31970) Thanks @liuxiaopai-ai.
