...
- ACP/native thread binding: choose channel-appropriate current-vs-child placement for `sessions_spawn` with `runtime: "acp"` and `thread: true` so Telegram topics stay in-topic, Discord DMs stay in the DM, and parent-channel Discord spawns still create child threads. Fixes #40800, #41004, #41116.
