...
- Browser/Profile defaults: default to `openclaw` (managed standalone browser) for all environments when `browser.defaultProfile` is not explicitly configured; simplify default-resolution to a single constant lookup. The `chrome` relay profile remains available for explicit opt-in. (#31907)
