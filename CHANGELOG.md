...
- Export: restore `{{MARKED_JS}}`, `{{HIGHLIGHT_JS}}`, and `{{JS}}` placeholders in export-session HTML template that were broken by formatter, and use function replacers to prevent `$&` pattern injection from vendor JS, fixing non-functional exported HTML files. (#22595)
