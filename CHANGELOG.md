...
- Agents/Bootstrap cache freshness: add a TTL-based reload guard for cached workspace bootstrap files (default 5 minutes, configurable via `OPENCLAW_BOOTSTRAP_CACHE_TTL_MS`, with `0` disabling cache) so long-running sessions pick up external `AGENTS.md`/`MEMORY.md`/`TODO.md` edits without requiring session reset. (#30896)
