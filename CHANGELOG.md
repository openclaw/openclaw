...
- Gateway/health + doctor allowlist checks: merge live channel runtime snapshot fields into gateway `health` snapshots (so running/token-source state matches `channels status`) and skip empty-allowlist warnings for disabled channel/account scopes in doctor analysis. (#32689)
