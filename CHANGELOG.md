...
- Plugins/uninstall cleanup: allow uninstall to remove default `~/.openclaw/extensions/<pluginId>` directories even for entry-only records (no install metadata), so stale duplicate plugin directories no longer survive uninstall and keep triggering duplicate-id warnings. (#35884)
