...
- CLI/Completions: install fish completions to `~/.config/fish/completions/` (auto-loaded by fish) instead of sourcing in `config.fish`, clean up legacy source lines on upgrade, and fix orphaned `# OpenClaw Completion` header when removing old entries. (#27135) Thanks @hou-rong.
