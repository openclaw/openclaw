...
- Telegram/replyToMode first chunking: apply `reply_to_message_id` only to the first physical send in `replyToMode=first` (text chunking, multi-media sends, and media follow-up chunk sends), with regression coverage for split text and caption-overflow follow-up paths. (#31039) Thanks @liuxiaopai.
