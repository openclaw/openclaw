...
- Config/transactional write safety: run config mutations through a transactional pipeline (`prepare -> commit -> verify -> rollback`) across CLI/gateway/internal write paths, plus backup-first restart preflight and config-guard recovery to avoid self-bricking on invalid config edits. (#35221) thanks @leeekyrie.
