...
- WhatsApp/Reactions context fallback: allow `message.react` to use inbound `MessageSid` when explicit `messageId` is missing so WhatsApp reaction calls no longer fail with `messageId required` on inbound turns. Fixes #31303.
