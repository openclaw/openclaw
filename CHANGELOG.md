...
- Daemon/doctor service PATH hygiene: only include user bin paths that actually exist when generating gateway/node service environments, preventing stale `~/.npm-global/bin` and missing `~/.nvm/current/bin` entries from being reintroduced by `openclaw doctor --fix`. (#32448) Thanks @cliffchen.
