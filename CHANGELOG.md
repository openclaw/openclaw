...
- Agents/ToolPolicy: add missing `group:fs` (`read`, `write`, `edit`, `apply_patch`) and `group:runtime` (`exec`, `process`) tools to `group:openclaw`. The group is documented as "all built-in OpenClaw tools" but was silently omitting filesystem and execution tools, causing `deny: ["group:openclaw"]` policies to leave those tools unblocked.
