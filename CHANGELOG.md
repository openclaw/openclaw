...
- Browser/Snapshot resilience: when AI snapshots fail with Playwright CDP connect timeout on an already-open tab, fall back to a direct CDP aria snapshot (with `fallbackFrom: "ai"`) instead of failing the request outright. (#30686) Thanks @liuxiaopai-ai.
