...
- Slack: replace `files.uploadV2` with 3-step upload flow (`getUploadURLExternal` → POST → `completeUploadExternal`) to fix `missing_scope` errors when uploading files despite `files:write` being granted. (#17558)
