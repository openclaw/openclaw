...
- Reply/NO_REPLY stream prefix handling: treat uppercase alpha lead fragments (`N`, `NO`) as silent-token prefixes so streaming responders no longer surface transient `NO` leaks before the full `NO_REPLY` token arrives, while keeping mixed-case natural-language partials (for example `No`) visible. (#32168) Thanks @liuxiaopai-ai.
