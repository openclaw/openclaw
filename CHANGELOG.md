...
- Cron/announce heartbeat noise: suppress heartbeat-only (`HEARTBEAT_OK`) isolated-run summaries from fallback main-session enqueue so cron ack tokens do not leak into active user conversations during announce delivery. (#32013) Thanks @liuxiaopai-ai.
