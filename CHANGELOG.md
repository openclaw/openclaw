...
- Docker/Compose gateway targeting: run `openclaw-cli` in the `openclaw-gateway` service network namespace, require gateway startup ordering, pin Docker setup to `gateway.mode=local`, and sync `gateway.bind` from `OPENCLAW_GATEWAY_BIND` so Docker CLI calls avoid stale non-loopback targets while host-published access remains explicit. (#12504) Thanks @bvanderdrift and @vincentkoc.
