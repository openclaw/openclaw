...
- Feishu/chat list action: add `list` action to `feishu_chat` tool for listing recent chats (groups and DMs) with pagination, and fix per-agent account routing so `feishu_chat` uses the correct Feishu account in multi-agent setups. (#39477) Thanks @ohmyskyhigh.
- Feishu/group broadcast dispatch: add configurable multi-agent group broadcast dispatch with observer-session isolation, cross-account dedupe safeguards, and non-mention history buffering rules that avoid duplicate replay in broadcast/topic workflows. (#39477) Thanks @ohmyskyhigh.
