...
import { gn as TtsAutoMode } from "./types.channels-BQaK8GCG.js";
import { t as ChannelId } from "./channel-id.types-CSSXKH7K.js";
import { t as DeliveryContext } from "./delivery-context.types-D3hESzPU.js";
...
type SessionPostCompactionDelegate = {
  task: string;
  createdAt: number; /** Stable original arm time, preserved across re-stage/restart cycles. */
  firstArmedAt?: number;
  /**
...
type SessionEntry = {
  ...
  pluginNextTurnInjections?: Record<string, SessionPluginNextTurnInjection[]>; /** Internal one-shot traceparent for a freshly spawned child agent run. */
  ...
  acp?: SessionAcpMeta; /** Number of continuation turns completed in the current chain. Reset on external message. */
  continuationChainCount?: number; /** Timestamp (ms) when the current continuation chain started. */
  continuationChainStartedAt?: number; /** Accumulated token usage across the current continuation chain. Reset on external message. */
   ...
   */
  continuationChainId?: string; /** Post-compaction delegates staged for execution after context compaction. */
