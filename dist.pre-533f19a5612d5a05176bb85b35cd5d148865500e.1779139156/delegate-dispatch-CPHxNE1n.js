...
import { t as spawnSubagentDirect } from "./subagent-spawn-DXI9Mvbh.js";
import { r as hasCrossSessionDelegateTargeting } from "./targeting-pure-DQnVhnO6.js";
import { g as startContinuationDelegateSpan, i as emitContinuationDisabledSpan, m as resolveContinuationTraceparent } from "./continuation-tracer-BMyqgPEb.js";
import { t as generateChainId } from "./secure-random-CKyp7Fy_.js";
import { a as enqueueSystemEvent } from "./system-events-CvlpBn9J.js";
import { i as failFlow } from "./task-flow-runtime-internal-3MCc_0JU.js";
import { i as consumePendingDelegates, l as highestDelayedContinuationReservationHop, u as peekSoonestUnmaturedDelegateDueAt } from "./delegate-store-BGSfZWyq.js";
import { n as resolveContinuationRuntimeConfig } from "./config-4qzXUepR.js";
import { l as retainContinuationTimerRef, o as registerContinuationTimerHandle, u as unregisterContinuationTimerHandle } from "./state-DkYgbKsI.js";
...
function armHedgeTimer(sessionKey, fireAt, params) {
	...
	const handle = setTimeout(() => {
		...
		dispatchToolDelegates({
			...
			...params.reservedDelegateSlots !== void 0 ? { reservedDelegateSlots: params.reservedDelegateSlots } : {},
...
async function dispatchToolDelegates(params) {
	...
	const delegateSlotsAvailable = Math.max(0, maxDelegatesPerTurn - (params.reservedDelegateSlots ?? 0));
...
*/
async function dispatchStagedPostCompactionDelegates(delegates, sessionKey, spawnCtx, options) {
	let dispatched = 0;
	let failed = 0;
	...
	for (const dropped of delegatesOverLimit) {
		postCompactionLog.warn(`[continuation:post-compaction-policy-rejected] cap.delegates_per_turn maxDelegatesPerTurn=${config.maxDelegatesPerTurn} session=${sessionKey} task=${dropped.task.slice(0, 80)}`);
		enqueueSystemEvent(`[continuation] Post-compaction delegate rejected: maxDelegatesPerTurn exceeded (${config.maxDelegatesPerTurn}). Task: ${dropped.task}`, {
			sessionKey,
			trusted: true
		});
		emitContinuationDisabledSpan({
	...
	for (const delegate of delegatesWithinLimit) {
		...
		if (budgetCheck) {
			...
			const summary = budgetCheck === "chain-capped" ? `chain length ${config.maxChainLength} reached` : `cost cap exceeded (${accumulatedChainTokens} > ${config.costCapTokens})`;
			postCompactionLog.warn(`[continuation:post-compaction-policy-rejected] ${disabledReason} session=${sessionKey} task=${delegate.task.slice(0, 80)}`);
			enqueueSystemEvent(`[continuation] Post-compaction delegate rejected: ${summary}. Task: ${delegate.task}`, {
				sessionKey,
				trusted: true
			});
			emitContinuationDisabledSpan({
