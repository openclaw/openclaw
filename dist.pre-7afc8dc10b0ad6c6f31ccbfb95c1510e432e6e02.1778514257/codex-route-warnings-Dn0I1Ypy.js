import { t as AGENT_MODEL_CONFIG_KEYS } from "./model-refs-BUOdHq4p.js";
...
function rewriteAgentModelRefs(params) {
	...
	for (const key of AGENT_MODEL_CONFIG_KEYS) rewriteModelConfigSlot({
		hits: params.hits,
...
function clearConfigLegacyAgentRuntimePolicies(cfg) {
	...
	for (const [index, agent] of (cfg.agents?.list ?? []).entries()) clearLegacyAgentRuntimePolicy(agent, `agents.list.${typeof agent.id === "string" && agent.id.trim() ? agent.id.trim() : String(index)}`, changes);
	return changes;
}
...
async function maybeRepairCodexSessionRoutes(params) {
	...
	return {
		...
		repairedSessions,
		warnings: [],
		changes: repairedSessions > 0 ? [`Repaired Codex session routes: moved ${repairedSessions} session${repairedSessions === 1 ? "" : "s"} across ${repairedStores} store${repairedStores === 1 ? "" : "s"} to openai/* while preserving runtime policy.`] : []
	};
}
//#endregion
