...
import { d as markMigrationItemSkipped, g as redactMigrationPlan, v as summarizeMigrationItems } from "./migration-k8fmTkLI.js";
...
const MIGRATION_SELECTION_SKIP = "__openclaw_migrate_skip_for_now__";
...
function reconcileInteractiveMigrationSkillToggleValues(selectedValues, activatedValue, selectableValues) {
	if (activatedValue === "__openclaw_migrate_skip_for_now__") return selectedValues.includes("__openclaw_migrate_skip_for_now__") ? [MIGRATION_SELECTION_SKIP] : [];
	...
	if (activatedValue !== void 0 && selectableValues.includes(activatedValue)) return selectedValues.filter((value) => value !== "__openclaw_migrate_toggle_all_on__" && value !== "__openclaw_migrate_toggle_all_off__" && value !== "__openclaw_migrate_skip_for_now__");
...
function reconcileInteractiveMigrationShortcutValues(previousValues, selectedValues, selectableValues, key) {
	...
	if (key === "a" && !previousValues.includes("__openclaw_migrate_skip_for_now__") && previousSelectable.length === selectableValues.length) return [MIGRATION_SELECTION_TOGGLE_ALL_OFF];
...
//#endregion
export { reconcileInteractiveMigrationSkillToggleValues as C, reconcileInteractiveMigrationShortcutValues as S, resolveInteractiveMigrationSkillSelection as T, getDefaultMigrationSkillSelectionValues as _, MIGRATION_SELECTION_SKIP as a, getSelectableMigrationPluginItems as b, applyMigrationPluginSelection as c, applyMigrationSkillSelection as d, formatMigrationPluginSelectionHint as f, getDefaultMigrationPluginSelectionValues as g, formatMigrationSkillSelectionLabel as h, writeApplyResult as i, applyMigrationSelectedPluginItemIds as l, formatMigrationSkillSelectionHint as m, assertConflictFreePlan as n, MIGRATION_SELECTION_TOGGLE_ALL_OFF as o, formatMigrationPluginSelectionLabel as p, formatMigrationPlan as r, MIGRATION_SELECTION_TOGGLE_ALL_ON as s, assertApplySucceeded as t, applyMigrationSelectedSkillItemIds as u, getMigrationPluginSelectionValue as v, resolveInteractiveMigrationPluginSelection as w, getSelectableMigrationSkillItems as x, getMigrationSkillSelectionValue as y };
