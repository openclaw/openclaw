...
import { U as AUTO_MANAGED_CONFIG_META_PATHS, u as readConfigFileSnapshot, w as validateConfigObjectRawWithPlugins, x as collectUnsupportedSecretRefPolicyIssues } from "./io-CINDrze3.js";
...
import { n as redactConfigObject } from "./redact-snapshot-DOFJIUXS.js";
import { n as readBestEffortRuntimeConfigSchema } from "./runtime-schema-DcI0Ohox.js";
import { t as buildGatewayReloadPlan } from "./config-reload-plan-CxOcGK3p.js";
import { n as setCommandJsonMode } from "./json-mode-DsKKFPQ7.js";
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/patch/unset/file/schema/validate). Run without subcommand for guided setup.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`).option("--section <section>", "Configuration sections for guided setup (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-CVV2srOu.js");
