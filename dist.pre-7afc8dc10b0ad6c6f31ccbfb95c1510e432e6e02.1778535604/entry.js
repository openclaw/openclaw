#!/usr/bin/env node
import { i as normalizeEnv, t as isTruthyEnvValue } from "./env-R7Vd4Uso.js";
import { d as isRootHelpInvocation, f as isRootVersionInvocation, r as getCommandPositionalsWithRootOptions } from "./argv-MdBfPgbe.js";
...
import { a as resolveCliContainerTarget, i as parseCliContainerArgs, n as parseCliProfileArgs, t as applyCliProfileEnv } from "./profile-v-og3OEu.js";
...
import { t as ensureOpenClawExecMarkerOnProcess } from "./openclaw-exec-env-COA04KLd.js";
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-DhqtXiRQ.js"), import("./git-commit-ePaFOyOQ.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
if (!isMainModule({
	...
	if (!respawnWithoutOpenClawCompileCacheIfNeeded({
		...
		normalizeEnv();
		if (!isRootHelpInvocation(process$1.argv) && !isRootVersionInvocation(process$1.argv)) assertNotRoot();
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		if (!(deps.outputPrecomputedRootHelpText ?? (await import("./root-help-metadata-CBh1mhht.js")).outputPrecomputedRootHelpText)()) {
			const { outputRootHelp } = await import("./root-help-BMEFdBQO.js");
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-Dl_XQOCJ.js");
