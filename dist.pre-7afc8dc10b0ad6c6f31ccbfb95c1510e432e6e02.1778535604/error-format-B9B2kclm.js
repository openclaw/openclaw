...
function formatGatewayCommandFailure(params) {
	const message = (params.error instanceof Error ? params.error.message : String(params.error)).replace(/\s*Run [`"]?openclaw doctor[`"]? for diagnostics\.?/gi, "").replace(/\s+/g, " ").trim().replace(/[.。]+$/u, "");
