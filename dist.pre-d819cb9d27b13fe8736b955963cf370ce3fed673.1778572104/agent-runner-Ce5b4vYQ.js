...
import { r as enqueueCommitmentExtraction } from "./runtime-DKA42H3X.js";
...
import { u as isLikelyExecutionAckPrompt } from "./selection-BKw7T0R_.js";
...
import { i as dispatchPostCompactionDelegates, s as persistPendingPostCompactionDelegates } from "./post-compaction-delegate-dispatch-2BK8M1jl.js";
...
async function releaseQueuedCompactionCompletion(params) {
	...
	const { dispatchPostCompactionDelegates } = await import("./post-compaction-delegate-dispatch-4HO_o8BO.js");
						...
						const result = await runEmbeddedPiAgentDefault$1({
							...
							requestCompactionOpts: runtimeConfig?.agents?.defaults?.continuation?.enabled === true ? {
								...
								triggerCompaction: async (request) => {
									...
									try {
										const { compactEmbeddedPiSession } = await import("./compact.queued-CCsazk2D.js");
...
const piEmbeddedRuntimeLoader = createLazyImportLoader(() => import("./pi-embedded-BDWjVeKf.js"));
		...
		try {
			...
			if (runtimeConfig?.agents?.defaults?.continuation?.enabled === true && sessionKey) {
				const [{ dispatchToolDelegates }, { resolveLiveContinuationRuntimeConfig }, { loadContinuationChainState, persistContinuationChainState }, { updateSessionStore, resolveSessionStoreEntry }] = await Promise.all([
					import("./delegate-dispatch-BoDrdh6_.js"),
