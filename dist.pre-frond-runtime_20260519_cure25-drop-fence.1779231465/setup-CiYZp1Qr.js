...
import { d as normalizeSecretInputString } from "./types.secrets-11Owfj8f.js";
import { r as normalizeProviderId } from "./provider-id-Cz7K6wgK.js";
import { n as defaultRuntime } from "./runtime-DDH_zqCr.js";
import { r as createConfigIO } from "./io-CINDrze3.js";
import { i as replaceConfigFile } from "./mutate-CxFtvytV.js";
import "./config-BpFERVfB.js";
import { t as WizardCancelledError } from "./prompts-giVBRodb.js";
import { n as t } from "./i18n-C72PqHsa.js";
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-Dm1R__CV.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-D5tyx51X.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-XM8U7HpA.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-CHsNpqNs.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-BLVXYapz.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-BbwbnpOf.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-DlnVJkJX.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-Dprb43Zf.js"), import("./setup-registry-BSSOnUpL.js")]);
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-ovGXTfri.js");
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-CcniYeuS.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DX5ml6iq.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-neU2QCvE.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-neU2QCvE.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-Xg4ipS4x.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-ea_bussa.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-mVvrXW9c.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-CrANPHnz.js");
		const { setupChannels } = await import("./onboard-channels-adqpao8Q.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-B823zQS-.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-CAlXpHFd.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-Cgsw4h9v.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-cyRQxTiq.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-CMqsVo8C.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-C3A_3ovH.js");
