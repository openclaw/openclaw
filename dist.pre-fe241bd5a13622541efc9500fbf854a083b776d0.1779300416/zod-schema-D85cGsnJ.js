...
import { C as SecretInputSchema, E as SecretsConfigSchema, F as VisibleRepliesSchema, P as TypingModeSchema, _ as NativeCommandsSettingSchema, c as GroupChatSchema, d as HumanDelaySchema, g as ModelsConfigSchema, i as ContextVisibilityModeSchema, j as TtsConfigSchema, k as TranscribeAudioSchema, l as GroupPolicySchema, n as BlockStreamingCoalesceSchema, p as InboundDebounceSchema, r as CliBackendSchema, t as BlockStreamingChunkSchema, u as HexColorSchema, y as QueueSchema, z as createAllowDenyChannelRulesSchema } from "./zod-schema.core-BrhV4JOv.js";
import { t as sensitive } from "./zod-schema.sensitive-3GVmnUbm.js";
import { t as parseDurationMs } from "./parse-duration-CD4d_yk2.js";
import { a as AgentRuntimePolicySchema, c as HeartbeatSchema, d as ToolsSchema, f as AgentModelSchema, i as AgentRunRetriesConfigSchema, l as MemorySearchSchema, n as AgentEmbeddedHarnessSchema, o as AgentSandboxSchema, p as AgentToolModelSchema, r as AgentEntrySchema, s as ElevatedAllowFromSchema, t as AgentContextLimitsSchema } from "./zod-schema.agent-runtime-RxtzzDCJ.js";
import { n as ChannelHeartbeatVisibilitySchema, r as ApprovalsSchema } from "./zod-schema.channels-BJfpHgPH.js";
...
const AgentDefaultsSchema = object({
	...
	continuation: object({
		enabled: boolean().optional(),
		defaultDelayMs: number().int().nonnegative().optional(),
		minDelayMs: number().int().nonnegative().optional(),
		maxDelayMs: number().int().nonnegative().optional(),
		maxChainLength: number().int().positive().optional(),
		costCapTokens: number().int().nonnegative().optional(),
		maxDelegatesPerTurn: number().int().positive().optional(),
		crossSessionTargeting: union([literal("disabled"), literal("enabled")]).default("disabled"),
		contextPressureThreshold: number().gt(0, "contextPressureThreshold must be > 0 (0 would fire on empty sessions)").max(1).optional(),
		earlyWarningBand: number().min(0).max(1).default(.3125)
	}).strict().refine((cfg) => {
