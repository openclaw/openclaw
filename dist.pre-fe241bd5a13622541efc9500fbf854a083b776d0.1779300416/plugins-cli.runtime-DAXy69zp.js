...
import { s as tracePluginLifecyclePhaseAsync } from "./discovery-BWEGE8ra.js";
...
import { n as collectConfiguredRuntimePluginIds, r as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-DYZ_4THb.js";
...
async function runPluginsEnableCommand(id) {
	...
	const { enablePluginInConfig } = await import("./enable-BD-axBGO.js");
	const { normalizePluginId } = await import("./config-state-D9shXnSd.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-B3REzR9E.js");
	const { applySlotSelectionForPlugin, logSlotWarnings } = await import("./plugins-command-helpers-DjkpJ1jl.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-DhPkc2kp.js");
...
async function runPluginsDisableCommand(id) {
	...
	const { normalizePluginId } = await import("./config-state-D9shXnSd.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-B3REzR9E.js");
	const { setPluginEnabledInConfig } = await import("./plugins-config-CIvxatAV.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-DhPkc2kp.js");
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-Dpqk3LUs.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-CBl6pfyx.js");
...
async function runPluginsDoctorCommand() {
	const { buildPluginCompatibilityNotices, buildPluginDiagnosticsReport, formatPluginCompatibilityNotice } = await import("./status-B3REzR9E.js");
	const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-B0XBaCBF.js");
	const cfg = getRuntimeConfig();
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-XVxgh_xd.js");
	const { createPluginInstallLogger } = await import("./plugins-command-helpers-DjkpJ1jl.js");
