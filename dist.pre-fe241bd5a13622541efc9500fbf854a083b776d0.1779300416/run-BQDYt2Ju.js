...
import { t as CONFIG_PATH, u as resolveGatewayPort, v as resolveStateDir } from "./paths-r6w2eKyy.js";
...
import { S as setVerbose } from "./logger-RkfmvLxA.js";
import "./globals-xAdlid3k.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-BIEzsfPC.js";
...
import { i as withDiagnosticPhase } from "./diagnostic-phase-Bn3KJTUs.js";
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-Bij0aCQe.js";
import { r as withProgress } from "./progress-CE2g1zbv.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-zK0zdycs.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-BsBVO_Eq.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-DzqZ_3kB.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-Cl2y0gDr.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-CjHCzZeX.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-CiQjn_U7.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-CAo01Rbr.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-DVrQ5dFO.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-DX7q440e.js");
	...
	const { resolveGatewayAuth } = await import("./auth-2aaXgCy9.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-DnQZXBLY.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-C-81vEJG.js");
