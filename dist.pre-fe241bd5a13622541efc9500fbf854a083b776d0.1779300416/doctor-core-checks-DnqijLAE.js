...
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-Dx7tFU11.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-C6PeMX3M.js";
import { i as disableUnavailableSkillsInConfig, t as collectUnavailableAgentSkills } from "./doctor-skills-CaB4ZKzT.js";
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-DR7rBvYS.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		...
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-CGS7VMnY.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./pi-embedded-helpers-BBmVWTFM.js");
...
const securityCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectSecurityWarnings } = await import("./doctor-security-BEc52EqJ.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./doctor-cron-5TiEm8me.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-DOJGZXoS.js");
...
const workspaceStatusCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyWorkspaceDirs } = await import("./doctor-workspace-Cp9nx811.js");
		const workspaceDir = resolveAgentWorkspaceDir(ctx.cfg, resolveDefaultAgentId(ctx.cfg));
...
const convertedWorkflowChecks = [
	...
	bootstrapSizeCheck,
	{
		...
		source: "doctor",
		async detect(ctx) {
			const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-CV8noWT4.js");
