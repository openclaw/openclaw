...
app.get('/api/az/accounts/:unit/column/:column', (req, res) => {
  const account = findAccountByUnit(req.params.unit);
  if (!account) {return res.status(404).json({ error: 'Not found' });}
  const col = accountsARColumns.find(c => c.toLowerCase() === req.params.column.replace(/_/g, ' ').toLowerCase());
...
accountsARColumns.forEach(col => {
  const endpoint = '/api/az/accounts/:unit/' + col.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
...
app.get('/api/az/accounts/:unit/special-assessment-active', (req, res) => {
  const account = findAccountByUnit(req.params.unit);
  if (!account) {return res.status(404).json({ error: 'Not found' });}
  res.json({ unit: req.params.unit, specialAssessmentActive: account['Special Assessment Active'] });
});
...
app.get('/api/az/accounts/:unit/special-assessment-balance', (req, res) => {
  const account = findAccountByUnit(req.params.unit);
  if (!account) {return res.status(404).json({ error: 'Not found' });}
  res.json({ unit: req.params.unit, specialAssessmentBalance: account['Special Assessment Balance ($)'] });
});
...
const excelPath = '/Users/cliffordvaughn/AskTenant Workspace/openclaw/Azure_Bay_Residences_Demo_Data_850_Units.xlsx';
let excelAccounts = [];
try {
  ...
  Object.entries(global.sheetRows || {}).forEach(([sheetName, rows]) => {
    ...
    const tabSlug = sheetName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
    ...
    columns.forEach(col => {
      ...
      const colSlug = col.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
      const endpoint = `/api/az/${tabSlug}/:${keyField.toLowerCase().replace(/ /g, '-')}/` + colSlug;
      app.get(endpoint, (req, res) => {
        const value = req.params[keyField.toLowerCase().replace(/ /g, '-')];
        const row = rows.find(r => r[keyField] && String(r[keyField]).trim().toLowerCase() === value.trim().toLowerCase());
        if (!row) {return res.status(404).json({ error: 'Not found' });}
        res.json({ [keyField]: value, [col]: row[col] });
      });
    });
    // Only add a generic column endpoint for safe sheets, and only once per sheet
    app.get(`/api/az/${tabSlug}/:${keyField.toLowerCase().replace(/ /g, '-')}/column/:column`, (req, res) => {
      const value = req.params[keyField.toLowerCase().replace(/ /g, '-')];
      const col = columns.find(c => c.toLowerCase() === req.params.column.replace(/_/g, ' ').toLowerCase());
      if (!col || !isSafeForRoute(col)) {return res.status(400).json({ error: 'Unknown or unsafe column' });}
      const row = rows.find(r => r[keyField] && String(r[keyField]).trim().toLowerCase() === value.trim().toLowerCase());
      if (!row) {return res.status(404).json({ error: 'Not found' });}
      res.json({ [keyField]: value, [col]: row[col] });
    });
...
app.get('/api/az/accounts/phone/:phone', (req, res) => {
  ...
  const phoneFields = ['Phone', 'Phone Number', 'Resident Phone', 'Primary Phone', 'Contact Phone', 'phone', 'phone_number', 'ResidentPhone', 'Owner_Phone', 'Owner Phone'];
...
app.get('/api/az/accounts/:unit', (req, res) => {
  ...
  const unitFields = ['Unit', 'Unit #', 'Unit Number', 'UnitNumber', 'Unit#', 'unit', 'unit_id', 'unit number'];
  const account = excelAccounts.find(a => {
...
app.get('/api/az/units/:unit', (req, res) => {
  ...
  if (!unitObj) {return res.status(404).json({ error: 'Not found' });}
  res.json(unitObj);
});
...
app.get('/api/az/work-orders', (req, res) => {
  ...
  if (unit) {results = results.filter(w => w.unit_id == unit || w.unit == unit);}
  res.json({ results });
});
