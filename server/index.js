...
app.get("/api/homeowners", (req, res) => {
  ...
  if (req.query.property)
    {results = results.filter((r) =>
      norm(r.Property).includes(norm(req.query.property))
    );}
  if (req.query.q) {
...
app.get("/api/homeowners/:unit", (req, res) => {
  ...
  if (!records.length) {return res.status(404).json({ error: "Unit not found" });}
...
app.get("/api/violations", (req, res) => {
  ...
  if (req.query.homeowner)
    {results = results.filter((r) =>
      norm(r.Homeowner).includes(norm(req.query.homeowner))
    );}
  res.json(paginate(results, req));
});
...
app.get("/api/violations/:unit", (req, res) => {
  ...
  const records = db.violations.filter(
    (r) =>
      norm(r["Unit Address"]).includes(q) ||
      norm(r.Homeowner).includes(q)
  );
  if (!records.length)
...
app.get("/api/work-orders", (req, res) => {
  ...
  if (req.query.priority)
    {results = results.filter(
      (r) => norm(r.Priority) === norm(req.query.priority)
    );}
  if (req.query.unit)
    {results = results.filter((r) => norm(r.Unit) === norm(req.query.unit));}
  if (req.query.vendor)
    {results = results.filter((r) =>
      norm(r.Vendor).includes(norm(req.query.vendor))
    );}
  res.json(paginate(results, req));
});
...
app.get("/api/work-orders/:number", (req, res) => {
  const wo = db.work_orders.find(
    (r) => norm(r["Work Order Number"]) === norm(req.params.number)
  );
...
app.get("/api/letters", (req, res) => {
  ...
  if (req.query.recipient)
    {results = results.filter((r) =>
      norm(r.Recipients).includes(norm(req.query.recipient))
    );}
  if (req.query.association)
    {results = results.filter((r) =>
      norm(r.Association).includes(norm(req.query.association))
    );}
  res.json(paginate(results, req));
});
...
app.get("/api/az/units/:unit", (req, res) => {
  ...
  const boatslip = db.az_boatslips.filter((r) => norm(r["Assigned Unit"]) === norm(req.params.unit));

  if (!unit.length) {return res.status(404).json({ error: "Unit not found" });}
...
app.get("/api/az/accounts", (req, res) => {
  ...
  if (req.query.delinquency_bucket)
    {results = results.filter((r) => norm(r["Delinquency Bucket [Formula]"]) === norm(req.query.delinquency_bucket));}
  ...
  if (req.query.special_assessment)
    {results = results.filter((r) => norm(r["Special Assessment Active"]) === norm(req.query.special_assessment));}
  if (req.query.min_balance)
    {results = results.filter((r) => (r["Current Balance ($)"] || 0) >= parseFloat(req.query.min_balance));}
  res.json(paginate(results, req));
});
...
app.get("/api/az/violations", (req, res) => {
  ...
  if (req.query.hearing_required)
    {results = results.filter((r) => norm(r["Hearing Required"]) === norm(req.query.hearing_required));}
  res.json(paginate(results, req));
});
...
app.get("/api/az/work-orders/:id", (req, res) => {
  const wo = db.az_work_orders.find(
    (r) => norm(r["Work Order ID"]) === norm(req.params.id)
  );
  if (!wo) {return res.status(404).json({ error: "Work order not found" });}
  res.json(wo);
});
...
app.get("/api/az/parking/:unit", (req, res) => {
  const records = db.az_parking.filter(
...
app.get("/api/az/boat-slips/:unit", (req, res) => {
  const records = db.az_boatslips.filter(
    (r) => norm(r["Assigned Unit"]) === norm(req.params.unit)
  );
...
app.post("/api/az/identity/resolve", (req, res) => {
  ...
  res.json({
    ...
    decision: candidates.length === 0
      ? "no_match"
      : ambiguous ? "ask_clarification" : "allow_with_verification",
    note: ambiguous
      ? "Multiple subjects found. active_subject must be explicitly selected before protected resolvers execute."
      : candidates.length === 0
...
app.get("/api/az/pm/summary", (req, res) => {
  ...
  accounts.forEach((r) => {
    ...
    if (r["Special Assessment Active"] === "Y") {special_assessment_units++;}
  });
