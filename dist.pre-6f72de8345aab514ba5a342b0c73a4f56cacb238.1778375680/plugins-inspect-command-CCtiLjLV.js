...
import { o as tracePluginLifecyclePhase, s as tracePluginLifecyclePhaseAsync } from "./discovery-C_DZWe8p.js";
...
import { u as quietPluginJsonLogger } from "./plugins-command-helpers-CJtfOP4h.js";
...
async function runPluginsInspectCommand(id, opts) {
	const { buildAllPluginInspectReports, buildPluginDiagnosticsReport, buildPluginInspectReport, buildPluginSnapshotReport, formatPluginCompatibilityNotice } = await import("./status-C2gl5AiA.js");
	const { loadInstalledPluginIndexInstallRecords } = await import("./installed-plugin-index-records-B-kOqRDH.js");
	...
	if (!targetPlugin) {
		defaultRuntime.error(`Plugin not found: ${id}. Run ${formatCliCommand("openclaw plugins list")} to see installed plugins.`);
		return defaultRuntime.exit(1);
	}
	const report = runtimeInspect ? tracePluginLifecyclePhase("runtime plugin registry load", () => buildPluginDiagnosticsReport({
		config: cfg,
	...
	if (!inspect) {
		defaultRuntime.error(`Plugin not found: ${id}. Run ${formatCliCommand("openclaw plugins list --json")} to inspect raw discovery state.`);
		return defaultRuntime.exit(1);
	}
