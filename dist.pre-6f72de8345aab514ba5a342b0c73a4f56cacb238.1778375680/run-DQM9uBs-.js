...
import { s as hasConfiguredSecretInput } from "./types.secrets-DYAh0rlj.js";
...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-OUQtxr3y.js";
import { t as createSubsystemLogger } from "./subsystem--HE_VOOc.js";
import { t as isContainerEnvironment } from "./container-environment-Dv6Wz22u.js";
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-DrO5tffj.js";
import { t as parsePort } from "./parse-port-eDEBLHoO.js";
import { i as withDiagnosticPhase } from "./diagnostic-phase-D_0tIO_h.js";
import { r as withProgress } from "./progress-CHYRSgwI.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-Dfsx8oaD.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-udc7UXHI.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-Y3QLvZ6b.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-DN61NizY.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-BN3frRoh.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-DeqyCqW8.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-E7atwifi.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-vkKeGcjB.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-pP-HH3mJ.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-oDwIC3h7.js");
	...
	const { resolveGatewayAuth } = await import("./auth--eayBWbQ.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-DN8VGIQ7.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-DlqPVRCp.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-DEKjYROn.js");
