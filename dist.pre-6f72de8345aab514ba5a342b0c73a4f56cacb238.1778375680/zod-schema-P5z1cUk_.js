...
import { t as InstallRecordShape } from "./zod-schema.installs-BJ6stiox.js";
import { t as parseDurationMs } from "./parse-duration-m3bv-WFH.js";
import { a as AgentSandboxSchema, c as MemorySearchSchema, d as AgentModelSchema, i as AgentRuntimePolicySchema, n as AgentEmbeddedHarnessSchema, o as ElevatedAllowFromSchema, r as AgentEntrySchema, s as HeartbeatSchema, t as AgentContextLimitsSchema, u as ToolsSchema } from "./zod-schema.agent-runtime-eYABEOrC.js";
import { C as SecretInputSchema, E as SecretsConfigSchema, F as VisibleRepliesSchema, P as TypingModeSchema, R as createAllowDenyChannelRulesSchema, _ as NativeCommandsSettingSchema, c as GroupChatSchema, d as HumanDelaySchema, g as ModelsConfigSchema, i as ContextVisibilityModeSchema, j as TtsConfigSchema, k as TranscribeAudioSchema, l as GroupPolicySchema, n as BlockStreamingCoalesceSchema, p as InboundDebounceSchema, r as CliBackendSchema, t as BlockStreamingChunkSchema, u as HexColorSchema, y as QueueSchema } from "./zod-schema.core-Bo8Y8c53.js";
import { t as sensitive } from "./zod-schema.sensitive-BzkXscPR.js";
import { l as ChannelHeartbeatVisibilitySchema } from "./zod-schema.providers-whatsapp-CBTMFdxJ.js";
...
const AgentDefaultsSchema = z.object({
	...
	continuation: z.object({
		...
		contextPressureThreshold: z.number().gt(0, "contextPressureThreshold must be > 0 (0 would fire on empty sessions)").max(1).optional(),
		earlyWarningBand: z.number().min(0).max(1).default(.3125)
	}).strict().refine((cfg) => {
		const min = cfg.minDelayMs;
		const def = cfg.defaultDelayMs;
		const max = cfg.maxDelayMs;
		if (min !== void 0 && def !== void 0 && min > def) return false;
		if (def !== void 0 && max !== void 0 && def > max) return false;
		if (min !== void 0 && max !== void 0 && min > max) return false;
		return true;
	}, { message: "continuation delay bounds violate minDelayMs ≤ defaultDelayMs ≤ maxDelayMs" }).optional()
}).strict().optional();
//#endregion
...
const ProxyLoopbackModeSchema = z.enum([
	"gateway-only",
	"proxy",
	"block"
]);
const ProxyConfigSchema = z.object({
	enabled: z.boolean().optional(),
	proxyUrl: z.url().refine(isHttpProxyUrl, { message: "proxyUrl must use http://" }).register(sensitive).optional(),
...
const LegacyCanvasHostSchema = z.object({
	enabled: z.boolean().optional(),
...
const TalkRealtimeSchema = z.object({
	provider: z.string().optional(),
