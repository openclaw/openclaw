import { a as OPENCLAW_RUNTIME_CONTEXT_NOTICE, i as OPENCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE, o as OPENCLAW_RUNTIME_EVENT_HEADER, r as OPENCLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER } from "./internal-runtime-context-DcCVbFSf.js";
//#region src/agents/pi-embedded-runner/run/runtime-context-prompt.ts
const OPENCLAW_RUNTIME_EVENT_USER_PROMPT = "Continue the OpenClaw runtime event.";
function formatReplyContextAsText(reply) {
	const lines = [];
	if (reply.senderLabel) lines.push(reply.isQuote ? `Quoting ${reply.senderLabel}:` : `Reply from ${reply.senderLabel}:`);
	else if (reply.isQuote) lines.push("Quoting earlier message:");
	else lines.push("Reply target of current user message:");
...
function buildCurrentTurnPromptContextPrefix(context) {
	...
	if (context.reply) return formatReplyContextAsText(context.reply).trim();
	return "";
}
...
//#endregion
export { resolveRuntimeContextPromptParts as a, queueRuntimeContextForNextTurn as i, buildRuntimeContextSystemContext as n, buildRuntimeEventSystemContext as r, buildCurrentTurnPromptContextPrefix as t };
