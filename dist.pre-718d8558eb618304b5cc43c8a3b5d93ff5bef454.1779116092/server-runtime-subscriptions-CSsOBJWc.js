import { l as onAgentEvent, t as clearAgentRunContext } from "./agent-events-CzW8tQZJ.js";
import { n as onSessionTranscriptUpdate } from "./transcript-events-B2y1CNV1.js";
import { n as onSessionLifecycleEvent } from "./session-lifecycle-events-DE1ElpZs.js";
import { r as onHeartbeatEvent } from "./heartbeat-events-DRWCmNtU.js";
...
function startGatewayEventSubscriptions(params) {
	...
	const getAgentEventHandler = () => {
		agentEventHandlerPromise ??= Promise.all([import("./server-chat-Dy9zxgvq.js"), import("./server-session-key-Hz7FksdV.js")]).then(([{ createAgentEventHandler }, { resolveSessionKeyForRun }]) => createAgentEventHandler({
	...
	const getTranscriptUpdateHandler = () => {
		transcriptUpdateHandlerPromise ??= import("./server-session-events-c1VHaFev.js").then(({ createTranscriptUpdateBroadcastHandler }) => createTranscriptUpdateBroadcastHandler({
			broadcastToConnIds: params.broadcastToConnIds,
			sessionEventSubscribers: params.sessionEventSubscribers,
	...
	const getLifecycleEventHandler = () => {
		lifecycleEventHandlerPromise ??= import("./server-session-events-c1VHaFev.js").then(({ createLifecycleEventBroadcastHandler }) => createLifecycleEventBroadcastHandler({
			broadcastToConnIds: params.broadcastToConnIds,
			sessionEventSubscribers: params.sessionEventSubscribers
		}));
