...
当用户发送语音时，自动执行：
```bash
cd /Users/lizhihong/WalkThink && python3 scripts/auto_process.py {audio_path} --source telegram --timestamp {timestamp} --user-id 5242887805 --message-id {message_id}
```
...
- **Telegram 通道**：✓ 已配置（用户ID: 5242887805）
- **实时处理**：✓ 支持（MacBook "open" 状态时）
- **离线处理**：✗ 不支持（简化设计）

## 使用说明
1. **自动处理**：发送语音到 Telegram → 自动处理
