...
If still empty, check `/data/.hanzo-bot/bot.json`:

```
cat /data/.hanzo-bot/bot.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Bot dashboard or in ~/.hanzoai/bot.json under skills.entries.gh-issues."
...
7. **Check claim-based in-progress tracking:**
   ...
   ```
   CLAIMS_FILE="/data/.hanzo-bot/gh-issues-claims.json"
   if [ ! -f "$CLAIMS_FILE" ]; then
     mkdir -p /data/.hanzo-bot
     echo '{}' > "$CLAIMS_FILE"
   fi
   ```
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  ```
  CURSOR_FILE="/data/.hanzo-bot/gh-issues-cursor-{SOURCE_REPO_SLUG}.json"
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., bot-bot)
  ```
...
GH_TOKEN=$(cat ~/.hanzoai/bot.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.hanzo-bot/bot.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.hanzo-bot/bot.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")
