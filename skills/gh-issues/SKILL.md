...
- owner/repo — optional. This is the source repo to fetch issues from. If omitted, detect from the current git remote:
  ...
  - SSH: [redacted-email]:owner/repo.git → owner/repo
...
3. **Verify remote access:**
   ...
   - Verify the fork remote exists. Check if a git remote named `fork` exists:
     ...
     ```
     git remote add fork https://x-access-token:$[redacted-email]/{PUSH_REPO}.git
     ```
...
git remote set-url {PUSH_REMOTE} https://x-access-token:$[redacted-email]/{PUSH_REPO}.git
Then push:
