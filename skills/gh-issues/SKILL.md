...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Mullusi. Pass it as a Bearer token in all API calls:
...
If still empty, check `/data/.mullusi/mullusi.json`:

```
cat /data/.mullusi/mullusi.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Mullusi dashboard or in the active Mullusi config path (`$MULLUSI_CONFIG_PATH`, default `~/.mullusi/mullusi.json`) under `skills.entries.gh-issues`."
...
7. **Check claim-based in-progress tracking:**
   ...
   ```
   CLAIMS_FILE="/data/.mullusi/gh-issues-claims.json"
   if [ ! -f "$CLAIMS_FILE" ]; then
     mkdir -p /data/.mullusi
     echo '{}' > "$CLAIMS_FILE"
   fi
   ```
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  ```
  CURSOR_FILE="/data/.mullusi/gh-issues-cursor-{SOURCE_REPO_SLUG}.json"
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., mullusi-mullusi)
  ```
...
CONFIG_PATH="${MULLUSI_CONFIG_PATH:-${MULLUSI_STATE_DIR:-$HOME/.mullusi}/mullusi.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.mullusi/mullusi.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.mullusi/mullusi.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${MULLUSI_CONFIG_PATH:-${MULLUSI_STATE_DIR:-$HOME/.mullusi}/mullusi.json}"
