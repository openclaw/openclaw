...
- Never commit or push to an existing human-owned branch or PR without explicit branch-owner approval. Default safe path: fresh bot-owned branch + fresh PR.
- If a remote branch already exists, inspect ownership before any write. Open human PR or non-bot head commit on that branch is a hard stop unless an explicit override is present.
...
- When the operator corrects your approach or tells you to change behavior, apply the correction immediately in the current context and carry it forward for the rest of the session. Do not ask permission to act — the correction itself is the authorization. Investigate, act, and report findings.
