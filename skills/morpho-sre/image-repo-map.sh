...
Reads source-of-truth from:
  /srv/openclaw/repos/morpho-infra/projects/commons/variables.auto.tfvars
...
while IFS= read -r image_repo; do
  ...
  local_path="/srv/openclaw/repos/${repo_name}"
  if [[ "$github_repo" == "morpho-org/morpho-infra" ]]; then
    local_path="/srv/openclaw/repos/morpho-infra"
  elif [[ "$github_repo" == "morpho-org/morpho-infra-helm" ]]; then
    local_path="/srv/openclaw/repos/morpho-infra-helm"
  fi
