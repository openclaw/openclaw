...
aws_cmd() {
  AWS_PAGER="" aws "${aws_base_args[@]}" "$@"
}

aws_timeout_cmd() {
  timeout "$AWS_TIMEOUT_SECONDS" env AWS_PAGER="" aws "${aws_base_args[@]}" "$@"
}

if ! aws_timeout_cmd sts get-caller-identity >/dev/null 2>&1; then
...
if [[ -n "$instance_ids" ]]; then
  ...
  if [[ "${#instance_id_args[@]}" -gt 0 ]]; then
    ec2_status_json="$(aws_timeout_cmd ec2 describe-instance-status --include-all-instances --instance-ids "${instance_id_args[@]}" 2>/dev/null || true)"
    ...
    if [[ -n "$ec2_status_json" ]]; then
      ...
      volume_ids="$(aws_timeout_cmd ec2 describe-instances --instance-ids "${instance_id_args[@]}" 2>/dev/null \
      ...
      if [[ -n "$volume_ids" ]]; then
        ...
        if [[ "${#volume_args[@]}" -gt 0 ]]; then
          volume_status_json="$(aws_timeout_cmd ec2 describe-volume-status --volume-ids "${volume_args[@]}" 2>/dev/null || true)"
      ...
      spot_notice_json="$(aws_timeout_cmd ec2 describe-instance-status --instance-ids "${instance_id_args[@]}" 2>/dev/null || true)"
...
if [[ -z "$cluster_names" ]]; then
  cluster_names="$(aws_timeout_cmd eks list-clusters 2>/dev/null | jq -r '.clusters[]?' | head -n 5 || true)"
fi

if [[ -n "$cluster_names" ]]; then
  while IFS= read -r cluster; do
    [[ -z "$cluster" ]] && continue
    nodegroups_json="$(aws_timeout_cmd eks list-nodegroups --cluster-name "$cluster" 2>/dev/null || true)"
    ...
    while IFS= read -r ng; do
      [[ -z "$ng" ]] && continue
      ng_json="$(aws_timeout_cmd eks describe-nodegroup --cluster-name "$cluster" --nodegroup-name "$ng" 2>/dev/null || true)"
      if [[ -z "$ng_json" ]]; then
