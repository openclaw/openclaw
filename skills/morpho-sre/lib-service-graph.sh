...
_sg_dependency_health_json() {
  local raw_json="${SERVICE_DEPENDENCY_HEALTH_JSON:-}"
  local file_path="${SERVICE_DEPENDENCY_HEALTH_FILE:-}"

  if [[ -z "$raw_json" && -n "$file_path" && -f "$file_path" ]]; then
    raw_json="$(cat "$file_path" 2>/dev/null || true)"
  fi

  if [[ -z "$raw_json" ]] || ! command -v jq >/dev/null 2>&1; then
    printf '{}\n'
    return 0
  fi
...
apply_service_dependency_health() {
  local services_json="$1"
  ...
  local health_json
  health_json="$(_sg_dependency_health_json)"

  printf '%s\n' "$services_json" | jq --argjson health "$health_json" '
    def degraded_status($status):
    ...
    def annotate_edges($edges):
      ...
      | map(
          . as $edge
          | ($health[$edge.service] // {}) as $h
          | . + (if ($h.status // "") != "" then {health_status: $h.status} else {} end)
              + (if ($h.reason // "") != "" then {health_reason: $h.reason} else {} end)
              + (if degraded_status($h.status) then {cascade_candidate: true} else {} end)
        );
    with_entries(
      .value.depends_on = annotate_edges(.value.depends_on)
      | .value.depended_by = annotate_edges(.value.depended_by)
    )
  '
}
...
discover_service_graph() {
  ...
  services="$(apply_service_dependency_health "$services")"
