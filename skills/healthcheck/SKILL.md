---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SmartAgentNeo deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SmartAgentNeo cron scheduling for periodic checks, or version status checks on a machine running SmartAgentNeo (laptop, workstation, Pi, VPS).
---

# SmartAgentNeo Host Hardening

## Overview

Assess and harden the host running SmartAgentNeo, then align it to a user-defined risk tolerance without breaking access. Use SmartAgentNeo security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SmartAgentNeo changes the host firewall, SSH, or OS updates; it does not.
...
5. SmartAgentNeo gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SmartAgentNeo can access sensitive data.
...
### 2) Run SmartAgentNeo security audits (read-only)

As part of the default read-only checks, run `smart-agent-neo security audit --deep`. Only offer alternatives if the user requests them:

1. `smart-agent-neo security audit` (faster, non-probing)
2. `smart-agent-neo security audit --json` (structured output)

Offer to apply SmartAgentNeo safe defaults (numbered):

1. `smart-agent-neo security audit --fix`

Be explicit that `--fix` only tightens SmartAgentNeo defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SmartAgentNeo version/update status (read-only)

As part of the default read-only checks, run `smart-agent-neo update status`.
...
- Credential hygiene notes (location of SmartAgentNeo creds, prefer disk encryption)
...
- SmartAgentNeo security audit (re-run)
...
After SmartAgentNeo install or first hardening pass, run at least one baseline audit and version check:

- `smart-agent-neo security audit`
- `smart-agent-neo security audit --deep`
- `smart-agent-neo update status`

Ongoing monitoring is recommended. Use the SmartAgentNeo cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `smart-agent-neo cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `smart-agent-neo update status`
...
Before creating, `smart-agent-neo cron list` and match on exact `name`. If found, `smart-agent-neo cron edit <id> ...`.
If not found, `smart-agent-neo cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `smart-agent-neo update status` (preferred for source checkouts and channels)
2. `npm view smart-agent-neo version` (published npm version)

## SmartAgentNeo command accuracy

Use only supported commands and flags:

- `smart-agent-neo security audit [--deep] [--fix] [--json]`
- `smart-agent-neo status` / `smart-agent-neo status --deep`
- `smart-agent-neo health --json`
- `smart-agent-neo update status`
- `smart-agent-neo cron add|list|runs|run`

Do not invent CLI flags or imply SmartAgentNeo enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SmartAgentNeo compaction:
