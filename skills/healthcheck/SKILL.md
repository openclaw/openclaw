---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Activi deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Activi cron scheduling for periodic checks, or version status checks on a machine running Activi (laptop, workstation, Pi, VPS).
---

# Activi Host Hardening

## Overview

Assess and harden the host running Activi, then align it to a user-defined risk tolerance without breaking access. Use Activi security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Activi changes the host firewall, SSH, or OS updates; it does not.
...
5. Activi gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Activi can access sensitive data.
...
### 2) Run Activi security audits (read-only)

As part of the default read-only checks, run `activi security audit --deep`. Only offer alternatives if the user requests them:

1. `activi security audit` (faster, non-probing)
2. `activi security audit --json` (structured output)

Offer to apply Activi safe defaults (numbered):

1. `activi security audit --fix`

Be explicit that `--fix` only tightens Activi defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Activi version/update status (read-only)

As part of the default read-only checks, run `activi update status`.
...
- Credential hygiene notes (location of Activi creds, prefer disk encryption)
...
After Activi install or first hardening pass, run at least one baseline audit and version check:

- `activi security audit`
- `activi security audit --deep`
- `activi update status`

Ongoing monitoring is recommended. Use the Activi cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `activi cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `activi update status`
...
Before creating, `activi cron list` and match on exact `name`. If found, `activi cron edit <id> ...`.
If not found, `activi cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `activi update status` (preferred for source checkouts and channels)
2. `npm view activi version` (published npm version)

## Activi command accuracy

Use only supported commands and flags:

- `activi security audit [--deep] [--fix] [--json]`
- `activi status` / `activi status --deep`
- `activi health --json`
- `activi update status`
- `activi cron add|list|runs|run`

Do not invent CLI flags or imply Activi enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Activi compaction:
