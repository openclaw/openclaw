---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Mullusi deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Mullusi cron scheduling for periodic checks, or version status checks on a machine running Mullusi (laptop, workstation, Pi, VPS).
---

# Mullusi Host Hardening

## Overview

Assess and harden the host running Mullusi, then align it to a user-defined risk tolerance without breaking access. Use Mullusi security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Mullusi changes the host firewall, SSH, or OS updates; it does not.
...
5. Mullusi gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Mullusi can access sensitive data.
...
### 2) Run Mullusi security audits (read-only)

As part of the default read-only checks, run `mullusi security audit --deep`. Only offer alternatives if the user requests them:

1. `mullusi security audit` (faster, non-probing)
2. `mullusi security audit --json` (structured output)

Offer to apply Mullusi safe defaults (numbered):

1. `mullusi security audit --fix`

Be explicit that `--fix` only tightens Mullusi defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Mullusi version/update status (read-only)

As part of the default read-only checks, run `mullusi update status`.
...
- Credential hygiene notes (location of Mullusi creds, prefer disk encryption)
...
After Mullusi install or first hardening pass, run at least one baseline audit and version check:

- `mullusi security audit`
- `mullusi security audit --deep`
- `mullusi update status`

Ongoing monitoring is recommended. Use the Mullusi cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `mullusi cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `mullusi update status`
...
Before creating, `mullusi cron list` and match on exact `name`. If found, `mullusi cron edit <id> ...`.
If not found, `mullusi cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `mullusi update status` (preferred for source checkouts and channels)
2. `npm view mullusi version` (published npm version)

## Mullusi command accuracy

Use only supported commands and flags:

- `mullusi security audit [--deep] [--fix] [--json]`
- `mullusi status` / `mullusi status --deep`
- `mullusi health --json`
- `mullusi update status`
- `mullusi cron add|list|runs|run`

Do not invent CLI flags or imply Mullusi enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Mullusi compaction:
