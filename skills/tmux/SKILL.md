...
metadata:
  { "EasyHub": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${EASYHUB_TMUX_SOCKET_DIR:-${EASYHUB_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/EasyHub-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/EasyHub.sock"
SESSION=EasyHub-python
...
- Use `EASYHUB_TMUX_SOCKET_DIR` (legacy `EASYHUB_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$EASYHUB_TMUX_SOCKET_DIR/EasyHub.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `EASYHUB_TMUX_SOCKET_DIR`).
