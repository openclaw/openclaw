...
metadata:
  { "smart-agent-neo": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---

# tmux Skill (SmartAgentNeo)
...
```bash
SOCKET_DIR="${SMART_AGENT_NEO_TMUX_SOCKET_DIR:-${NEOBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/smart-agent-neo-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/smart-agent-neo.sock"
SESSION=smart-agent-neo-python
...
- Use `SMART_AGENT_NEO_TMUX_SOCKET_DIR` (legacy `NEOBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$SMART_AGENT_NEO_TMUX_SOCKET_DIR/smart-agent-neo.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `SMART_AGENT_NEO_TMUX_SOCKET_DIR`).
