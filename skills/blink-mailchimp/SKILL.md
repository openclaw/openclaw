...
```bash
bash scripts/call.sh /lists GET '{"count":20}'
```

## Get audience details
```bash
bash scripts/call.sh /lists/{list_id} GET
```

## Get subscribers in an audience
```bash
bash scripts/call.sh /lists/{list_id}/members GET '{"count":20,"status":"subscribed"}'
```

## Add a subscriber
```bash
bash scripts/call.sh /lists/{list_id}/members POST '{"email_address":"user@example.com","status":"subscribed","merge_fields":{"FNAME":"John","LNAME":"Doe"}}'
```

## Get campaigns
```bash
bash scripts/call.sh /campaigns GET '{"count":10,"sort_field":"send_time","sort_dir":"DESC"}'
```

## Get campaign report
```bash
bash scripts/call.sh /reports/{campaign_id} GET
```

## Send a campaign
```bash
bash scripts/call.sh /campaigns/{campaign_id}/actions/send POST
```

## Search members
```bash
bash scripts/call.sh /search-members GET '{"query":"john@example.com"}'
```
