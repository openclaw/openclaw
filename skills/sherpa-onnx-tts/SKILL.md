...
1. Download the runtime for your OS (extracts into `$MULLUSI_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.mullusi/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$MULLUSI_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.mullusi/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Mullusi config file (`$MULLUSI_CONFIG_PATH`, default `~/.mullusi/mullusi.json`):
