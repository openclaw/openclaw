...
function getSessionsList(activeMinutes = 60) {
  try {
    const output = runOpenClaw(
      ["sessions", "list", "--active", activeMinutes.toString(), "--json"],
      true,
    );
    return JSON.parse(output);
  } catch (e) {
...
function cmdDashboard(options) {
  ...
  if (agents.length === 0) {
    ...
    agents.forEach((agent) => {
      ...
      console.log(
        `│ ${agentId.padEnd(9)} │ ${status.padEnd(12)} │ ${String(agentSessions.length).padEnd(11)} │ ${"configured".padEnd(12)} │`,
      );
    });
  ...
  if (teams.length === 0) {
    ...
    teams.forEach((team) => {
      ...
      console.log(
        `│ ${team.name.padEnd(9)} │ ${String(memberCount).padEnd(11)} │ ${orchestrator.padEnd(10)} │ ${status.padEnd(12)} │`,
      );
    });
...
function cmdSessionsList() {
  ...
  const officeSessions =
    sessions.sessions?.filter((s) => s.key?.includes(":office:") || s.key?.includes("subagent")) ||
    [];
