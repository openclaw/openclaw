---
name: node-connect
description: Diagnose Mullusi node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Mullusi is advertising that route, then fix pairing/auth.
...
- whether `mullusi devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `mullusi qr --json`. It uses the same setup-code payload Android scans.

```bash
mullusi config get gateway.mode
mullusi config get gateway.bind
mullusi config get gateway.tailscale.mode
mullusi config get gateway.remote.url
mullusi config get gateway.auth.mode
mullusi config get gateway.auth.allowTailscale
mullusi config get plugins.entries.device-pair.config.publicUrl
mullusi qr --json
mullusi devices list
mullusi nodes status
```

If this Mullusi instance is pointed at a remote gateway, also run:

```bash
mullusi qr --remote --json
```
...
If `mullusi qr --json` says `Gateway is only bound to loopback`:
...
- `mullusi devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run mullusi qr again, rescan, then approve the pending device pairing.`
