...
const PARSED_JSON_CACHE_MAX_ENTRIES = 1e4;
const parsedJsonCache = /* @__PURE__ */ new Map();
function cacheParsedJson(filePath, mtimeNs, size, parsed) {
	if (parsedJsonCache.size >= PARSED_JSON_CACHE_MAX_ENTRIES) {
		const oldestKey = parsedJsonCache.keys().next().value;
		if (oldestKey !== void 0) parsedJsonCache.delete(oldestKey);
	}
...
function readJsonObject(filePath) {
	...
	try {
		...
		if (cached && cached.mtimeNs === -1n && cached.size === -1n) return cached.parsed;
		cacheParsedJson(filePath, -1n, -1n, void 0);
		return;
	}
	...
	try {
		...
		parsed = isRecord(raw) ? raw : void 0;
	} catch {
		parsed = void 0;
	}
