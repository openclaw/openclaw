...
import { C as BrowserTabNotFoundError, b as BrowserProfileUnavailableError, f as redactCdpUrl } from "./cdp.helpers-CUJF6Yhn.js";
...
async function createRealSession(profileName, options = normalizeChromeMcpOptions()) {
	...
	const ready = (async () => {
		try {
			...
			throw new BrowserProfileUnavailableError(`Chrome MCP existing-session attach failed for profile "${profileName}". Make sure ${options.browserUrl ? `the configured Chrome endpoint (${redactToolPayloadText(redactCdpUrl(options.browserUrl) ?? options.browserUrl)})` : options.userDataDir ? `the configured Chromium user data dir (${options.userDataDir})` : "Google Chrome's default profile"} is running locally with remote debugging enabled. Details: ${redactChromeMcpDiagnosticText(err instanceof Error ? err.message : String(err))}`);
		}
	})();
	ready.catch(() => {});
	return {
