...
type ReplyPayload = {
  ...
  isFallbackNotice?: boolean; /** Channel-specific payload data (per-channel envelope). */
...
type ContinuationTrigger = "work-wake" | "delegate-return";
...
type GetReplyOptions = {
  /** Override run id for agent events (defaults to random UUID). */runId?: string; /** Parent run id when this run was spawned by a previous agent turn. */
  parentRunId?: string; /** Abort signal for the underlying agent run. */
   ...
   */
  continuationTrigger?: ContinuationTrigger; /** Policy-level typing control for run classes (user/system/internal/heartbeat). */
...
//#endregion
export { QueuedReplyDeliveryCorrelation as a, SourceReplyDeliveryMode as c, PromptImageOrderEntry as d, PartialReplyPayload as i, TypingController as l, ContinuationTrigger as n, QueuedReplyLifecycle as o, GetReplyOptions as r, ReplyThreadingPolicy as s, BlockReplyContext as t, ReplyPayload as u };