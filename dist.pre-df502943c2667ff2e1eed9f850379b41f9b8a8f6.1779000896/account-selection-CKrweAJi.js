...
function hasConfiguredDefaultAccountSource(cfg) {
	const telegram = cfg.channels?.telegram;
	if (!telegram) return false;
	const botToken = telegram.botToken;
	if (typeof botToken === "string") return botToken.trim().length > 0;
	if (botToken && typeof botToken === "object") return true;
...
function listTelegramAccountIds(cfg) {
	return combineAccountIds({
		...
		additionalAccountIds: [...listBoundAccountIds(cfg, "telegram"), ...hasConfiguredDefaultAccountSource(cfg) ? [DEFAULT_ACCOUNT_ID] : []]
	});
}
