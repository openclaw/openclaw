import { n as CLAUDE_CLI_DEFAULT_ALLOWLIST_REFS, t as CLAUDE_CLI_BACKEND_ID } from "./cli-constants-H_8CLcxT.js";
//#region extensions/anthropic/config-defaults.ts
const ANTHROPIC_PROVIDER_API = "anthropic-messages";
const ANTHROPIC_API_KEY_DEFAULT_ALLOWLIST_REFS = ["anthropic/claude-haiku-4-5"];
...
function toClaudeCliRuntimeModelRef(raw) {
	...
	if (!normalizeLowercaseStringOrEmpty(parsed.model).startsWith("claude-")) return null;
...
function collectClaudeCliRuntimeRefs(model) {
	...
	const primary = typeof model?.primary === "string" ? toClaudeCliRuntimeModelRef(model.primary) : null;
...
function applyAnthropicConfigDefaults(params) {
	...
	if (authMode === "oauth" && usesClaudeCliModelSelection(params.config)) {
		const nextModels = defaults.models ? { ...defaults.models } : {};
		let modelsMutated = false;
		const runtimeRefs = new Set(collectClaudeCliRuntimeRefs(defaults.model));
