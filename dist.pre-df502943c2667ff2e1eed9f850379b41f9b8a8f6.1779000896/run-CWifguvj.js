...
import { i as isLoopbackHost, p as resolveGatewayBindHost, t as defaultGatewayBindMode, v as isContainerEnvironment } from "./net-DW8WQG5I.js";
...
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-C61hcJEB.js";
import { r as withProgress } from "./progress-exju-4wx.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-ZohB0zxa.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-BbBJJ3YZ.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-c1JhK_FW.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-GSQIS0VW.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-Ce6fTPMw.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-CDI0gH_E.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-Cn9QDKiW.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-Cun35_ML.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-EBcc6LIO.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-MM4UWbXI.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared--gmSpOl7.js");
