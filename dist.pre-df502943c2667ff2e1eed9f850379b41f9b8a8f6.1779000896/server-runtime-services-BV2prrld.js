import { n as isVitestRuntimeEnv } from "./env-GwU3cB_J.js";
...
import { i as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-DUZSrsXy.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-CoRjYACy.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-D8K9NlJ-.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-D63117J0.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-CJTgbbD7.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-BnGw_oCe.js");
		if (stopped) return;
