...
import { a as isDiagnosticsEnabled, n as emitDiagnosticEvent, t as areDiagnosticsEnabledForProcess } from "./diagnostic-events-QP40Y1ku.js";
...
import { r as resolveAllAgentSessionStoreTargetsSync } from "./targets-CBd7TCsR.js";
import { i as resetDiagnosticRunActivityForTest, t as getDiagnosticSessionActivitySnapshot } from "./diagnostic-run-activity-ChMqWM5j.js";
import { t as getDiagnosticContinuationQueueMetrics } from "./diagnostic-continuation-queues-BvNWQuP5.js";
import { i as resetDiagnosticStabilityRecorderForTest, o as startDiagnosticStabilityRecorder, s as stopDiagnosticStabilityRecorder } from "./diagnostic-stability-tdqSzjqG.js";
import { a as installDiagnosticStabilityFatalHook, d as uninstallDiagnosticStabilityFatalHook, f as writeDiagnosticMemoryPressureBundleSync, l as resetDiagnosticStabilityBundleForTest } from "./diagnostic-stability-bundle-DEIlcBO3.js";
import { n as getRecentDiagnosticPhases, r as resetDiagnosticPhasesForTest, t as getCurrentDiagnosticPhase } from "./diagnostic-phase-DOI6ZwF7.js";
import { a as markDiagnosticActivity, n as getLastDiagnosticActivityAt, o as resetDiagnosticActivityForTest, t as diagnosticLogger } from "./diagnostic-runtime-CFFwo_Of.js";
import { a as pruneDiagnosticSessionStates, i as isDiagnosticSessionStateCurrent, n as getDiagnosticSessionState, o as resetDiagnosticSessionStateForTest, r as getDiagnosticSessionStateCountForTest$1, t as diagnosticSessionStates } from "./diagnostic-session-state-CyhsYPdX.js";
...
function recoveryOutcomeClearsQueuedSessionState(outcome) {
	return outcome.status === "released" || outcome.status === "noop" && outcome.reason === "no_active_work";
}
...
function applyRecoveryOutcomeToDiagnosticState(params) {
	...
	state.queueDepth = recoveryOutcomeClearsQueuedSessionState(params.outcome) ? 0 : Math.max(0, state.queueDepth - 1);
	emitDiagnosticEvent({
		type: "session.state",
...
function shouldEmitDiagnosticLivenessWarning(now, work, continuationQueue) {
	if (!hasOpenDiagnosticWork(work) && !hasContinuationQueueActivity(continuationQueue)) return false;
...
function formatContinuationQueueNumber(value) {
	return typeof value === "number" && Number.isFinite(value) ? value.toFixed(2) : "n/a";
}
function hasContinuationQueueActivity(continuationQueue) {
	return continuationQueue !== void 0 && (continuationQueue.totalQueued > 0 || continuationQueue.enqueuedSinceLastSample > 0 || continuationQueue.drainedSinceLastSample > 0 || continuationQueue.failedSinceLastSample > 0);
}
function hasContinuationQueueMotion(continuationQueue) {
	return continuationQueue !== void 0 && (continuationQueue.enqueuedSinceLastSample > 0 || continuationQueue.drainedSinceLastSample > 0 || continuationQueue.failedSinceLastSample > 0);
}
function safeGetDiagnosticContinuationQueueMetrics(now) {
	try {
		return getDiagnosticContinuationQueueMetrics(now);
	} catch (err) {
		diagnosticLogger.debug(`continuation queue diagnostics failed: ${String(err)}`);
		return;
	}
}
function formatContinuationQueueTopQueues(continuationQueue) {
	return continuationQueue.topQueues.map((queue) => `${queue.sessionKey}(total=${queue.totalQueued},runnable=${queue.pendingRunnable},scheduled=${queue.pendingScheduled},staged=${queue.stagedPostCompaction},invalid=${queue.invalidQueued})`).join(",");
}
function formatContinuationQueueHistory(continuationQueue) {
	return JSON.stringify(continuationQueue.queueDepthHistory.map((point) => ({
...
function formatContinuationQueueLogSuffix(continuationQueue) {
	if (!hasContinuationQueueActivity(continuationQueue)) return "";
...
function emitDiagnosticContinuationQueueSample(continuationQueue) {
	emitDiagnosticEvent({
...
function emitDiagnosticLivenessWarning(sample, work, continuationQueue) {
	...
	if (hasBlockingWork || hasOpenDiagnosticWork(work) && hasSustainedEventLoopDelay || hasContinuationQueueWarn) diagnosticLogger.warn(message);
...
function startDiagnosticHeartbeat(config, opts) {
	...
	heartbeatInterval = setInterval(() => {
		...
		const shouldEmitLivenessWarning = livenessSample !== null && shouldEmitDiagnosticLivenessWarning(now, work, continuationQueue);
		...
		const shouldRecordMemorySample = shouldEmitLivenessReport || hasRecentDiagnosticActivity(now) || hasOpenDiagnosticWork(work) || hasContinuationActivity;
		...
		if (shouldEmitLivenessReport && livenessSample) emitDiagnosticLivenessWarning(livenessSample, work, continuationQueue);
		if (hasContinuationActivity) emitDiagnosticContinuationQueueSample(continuationQueue);
		diagnosticLogger.debug(`heartbeat: webhooks=${webhookStats.received}/${webhookStats.processed}/${webhookStats.errors} active=${work.activeCount} waiting=${work.waitingCount} queued=${work.queuedCount}${formatContinuationQueueLogSuffix(continuationQueue)}`);
		emitDiagnosticEvent({
