...
import { r as formatDurationPrecise } from "./format-duration-D2mwWJew.js";
import "./runtime-env-BvVavUHC.js";
import "./string-coerce-runtime-DBT298Ik.js";
import { i as isRecoverableTelegramNetworkError, t as TELEGRAM_GET_UPDATES_REQUEST_TIMEOUT_MS } from "./request-timeouts-qaoOavRm.js";
import { a as listTelegramSpooledUpdates, c as resolveTelegramIngressSpoolDir, i as listTelegramSpooledUpdateClaims, n as deleteTelegramSpooledUpdate, o as recoverStaleTelegramSpooledUpdateClaims, r as isTelegramSpooledUpdateClaimOwnedByOtherLiveProcess, s as releaseTelegramSpooledUpdateClaim, t as claimTelegramSpooledUpdate } from "./telegram-ingress-spool-gm16fhuG.js";
import { t as drainPendingDeliveries } from "./delivery-queue-runtime-C2RvzNBX.js";
...
import { n as readTelegramUpdateOffset, r as writeTelegramUpdateOffset, t as deleteTelegramUpdateOffset } from "./update-offset-store-ybW41HCZ.js";
import { O as withTelegramApiErrorLogging } from "./send-DK2rOL6m.js";
import { n as getTelegramSequentialKey, t as createTelegramBot } from "./bot-CydcGmAo.js";
...
var TelegramPollingSession = class {
	...
	#detectStaleSpooledHandler(blockedHandlerKeys) {
		const now = Date.now();
		let stale = null;
		for (const handlerKey of blockedHandlerKeys) {
			...
			if (!stale || ageMs > stale.ageMs) stale = {
				...handler,
				ageMs
			};
		}
	...
	async #runIsolatedIngressCycle(bot) {
		...
		const unsubscribe = worker.onMessage((message) => {
			...
			if (message.type === "poll-success") {
				if (stalledBacklogKeys.size === 0) this.#status.notePollSuccess(message.finishedAt);
		...
		const drainOnce = async () => {
			if (drainActive || this.opts.abortSignal?.aborted) return;
			drainActive = true;
			try {
