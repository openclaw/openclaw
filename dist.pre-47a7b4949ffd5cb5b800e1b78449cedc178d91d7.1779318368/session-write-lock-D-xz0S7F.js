...
import { n as createFileLockManager } from "./file-lock-yskus6Nd.js";
import { n as isPidAlive, t as getProcessStartTime } from "./pid-alive-C0-jAJSa.js";
import { r as readGatewayProcessArgsSync } from "./gateway-processes-D5Lk8sjZ.js";
...
async function acquireSessionWriteLock(params) {
	...
	while (true) {
		const remainingTimeoutMs = timeoutMs === Number.POSITIVE_INFINITY ? Number.POSITIVE_INFINITY : Math.max(0, timeoutMs - (Date.now() - startedAtMs));
		if (remainingTimeoutMs <= 0) await throwTimeout();
		try {
			...
			if (isFileLockError(err, "file_lock_stale")) {
				...
				const retryDelayMs = timeoutMs === Number.POSITIVE_INFINITY ? 50 : Math.min(50, Math.max(0, timeoutMs - (Date.now() - startedAtMs)));
				if (retryDelayMs <= 0) await throwTimeout();
