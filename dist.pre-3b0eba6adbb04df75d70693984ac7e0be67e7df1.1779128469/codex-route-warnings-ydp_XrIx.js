...
import { r as normalizeConfiguredProviderCatalogModelId } from "./model-ref-shared-BpLaMszT.js";
import { n as DEFAULT_MODEL, r as DEFAULT_PROVIDER } from "./defaults-DQNxPE51.js";
import { t as AGENT_MODEL_CONFIG_KEYS } from "./model-refs-dUnyf8Yy.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-F8J_hk_t.js";
import { t as resolveModelRuntimePolicy } from "./model-runtime-policy-K6ldDu8_.js";
import { t as normalizeEmbeddedAgentRuntime } from "./runtime-BRQxtJYr.js";
import { s as openAIProviderUsesCodexRuntimeByDefault } from "./openai-codex-routing-8M9hE9Ml.js";
import { t as loadSessionStore } from "./store-load-mEo4yxSu.js";
import { s as updateSessionStore } from "./store-PGV-10Zw.js";
import { i as resolveAllAgentSessionStoreTargetsSync } from "./targets-CvEriifS.js";
import { t as resolveConfiguredProviderFallback } from "./configured-provider-fallback-TJz6l49i.js";
...
function resolveRuntime(params) {
	return normalizeRuntimeString(params.env?.OPENCLAW_AGENT_RUNTIME) ?? normalizeRuntimeString(params.agentRuntime?.id) ?? normalizeRuntimeString(params.defaultsRuntime?.id);
}
...
function resolveConfiguredModelAliasRef(params) {
	...
	return resolveAliasFromModelsMap(asMutableRecord(params.cfg.agents?.defaults?.models), aliasKey, defaultProvider);
}
function resolveDefaultProviderForAliasContext(params) {
	...
	if (primaryModelRef) {
		...
		return normalizeProviderId((parseModelRef(resolveAliasFromModelsMap(asMutableRecord(params.cfg.agents?.defaults?.models), normalizeString(effectivePrimaryModelRef) ?? "", "openai") ?? compatModelRef ?? legacyCodexModel ?? effectivePrimaryModelRef) ?? parseModelRef(resolveConfiguredBareModelRef({
			cfg: params.cfg,
...
function findAgentById(cfg, agentId) {
	...
	return (Array.isArray(cfg.agents?.list) ? cfg.agents.list : []).map((agent) => asMutableRecord(agent)).find((agent) => normalizeAgentId(typeof agent?.id === "string" ? agent.id : void 0) === normalizedAgentId);
}
function resolveAliasFromModelsMap(models, aliasKey, defaultProvider) {
	for (const [modelRef, entry] of Object.entries(models ?? {})) {
		if (normalizeString(asMutableRecord(entry)?.alias) !== aliasKey) continue;
...
function collectUnsupportedCodexCompactionOverridesForAgent(params) {
	...
	if (normalizeString(compaction?.provider ?? inheritedCompaction?.provider) === LOSSLESS_CONTEXT_ENGINE_ID) return [];
...
function collectLegacyLosslessCompactionForAgent(params) {
	...
	if (normalizeString(providerValue) !== LOSSLESS_CONTEXT_ENGINE_ID) return [];
...
function collectLegacyLosslessCompactionConfigs(params) {
	...
	const defaultsRuntime = params.ignoreLegacyAgentRuntimePins ? void 0 : defaults?.agentRuntime;
...
function sharedDefaultLosslessCompactionHasNonCodexConsumer(params) {
	const defaults = params.cfg.agents?.defaults;
	const defaultCompaction = asMutableRecord(defaults?.compaction);
	const hasDefaultLosslessProvider = normalizeString(defaultCompaction?.provider) === LOSSLESS_CONTEXT_ENGINE_ID;
	const hasDefaultModel = typeof defaultCompaction?.model === "string" && defaultCompaction.model.trim();
...
function isCodexPluginUnavailableByConfig(cfg) {
	...
	if (asMutableRecord(asMutableRecord(cfg.plugins?.entries)?.codex)?.enabled === false) return true;
...
function collectDisabledCodexPluginRouteHits(cfg) {
	...
	for (const [index, agent] of agents.entries()) {
		...
		const inheritedModelRefs = inheritedDefaultAuxRefs.filter((ref) => {
			if (ref.path === "agents.defaults.heartbeat.model") return !normalizeString(asMutableRecord(agentRecord.heartbeat)?.model);
			if (ref.path.startsWith("agents.defaults.subagents.model")) return !readModelConfigPrimaryRef(asMutableRecord(agentRecord.subagents)?.model);
			return true;
		});
...
function enableCodexPluginForRequiredRoutes(params) {
	...
	if (Array.isArray(plugins.allow) && plugins.allow.length > 0 && !plugins.allow.some((id) => normalizeString(id) === "codex")) {
...
function rewriteModelsMap(params) {
	...
	for (const legacyRef of Object.keys(params.models)) {
		...
		params.models[canonicalModel] = legacyRecord && canonicalRecord ? {
			...legacyRecord,
			...canonicalRecord
...
function collectCodexRuntimeModelPolicyRefs(params) {
	...
	for (const [modelRef, entry] of Object.entries(record)) {
		const trimmed = modelRef.trim();
		if (!trimmed) continue;
		if (normalizeEmbeddedAgentRuntime(normalizeString(asMutableRecord(asMutableRecord(entry)?.agentRuntime)?.id)) === "codex") params.refs.push({
...
function agentExplicitlyReferencesCanonicalModel(agent, modelRef) {
	...
	if (modelConfigContainsRef(asMutableRecord(record.heartbeat)?.model, modelRef)) return true;
	if (modelConfigContainsRef(asMutableRecord(record.subagents)?.model, modelRef)) return true;
	const compaction = asMutableRecord(record.compaction);
	return modelConfigContainsRef(compaction?.model, modelRef) || modelConfigContainsRef(asMutableRecord(compaction?.memoryFlush)?.model, modelRef) || asMutableRecord(record.models)?.[modelRef] !== void 0;
}
...
function setModelRuntimePolicy(params) {
	...
	if (normalizeString(priorRuntime?.id) === params.runtimeId) return;
	entry.agentRuntime = {
		...priorRuntime,
		id: params.runtimeId
	};
...
function rewriteAgentModelRefs(params) {
	...
	if (agentUsesCodexRuntimeForCompaction({
		...
		currentRuntime: params.currentRuntime,
		inheritedModelRef: params.inheritedModelRef
	})) if (normalizeString(compaction?.provider ?? inheritedCompaction?.provider) === LOSSLESS_CONTEXT_ENGINE_ID) {
		const start = params.hits.length;
...
function removeUnsupportedCodexCompactionOverrides(params) {
	...
	if (normalizeString(params.compaction.provider) === LOSSLESS_CONTEXT_ENGINE_ID) return;
...
function readCompactionOwnerForPath(cfg, ownerPath) {
	if (ownerPath === "agents.defaults") return asMutableRecord(cfg.agents?.defaults);
	...
	if (agentWithId) return asMutableRecord(agentWithId);
	const index = Number(label);
	const candidate = Number.isInteger(index) ? agents[index] : void 0;
	if (candidate) return asMutableRecord(candidate);
}
...
function readLosslessSummaryModel(plugins) {
	const config = asMutableRecord(asMutableRecord(asMutableRecord(plugins?.entries)?.[LOSSLESS_CONTEXT_ENGINE_ID])?.config);
...
function ensureCodexRuntimePolicy(params) {
	...
	const entry = asMutableRecord(models[params.modelRef]) ?? {};
	if (models[params.modelRef] !== entry) models[params.modelRef] = entry;
	const runtimeId = normalizeString(asMutableRecord(entry.agentRuntime)?.id);
	if (runtimeId && runtimeId !== "auto" && runtimeId !== "default") return;
	setModelRuntimePolicy({
		agent: params.agent,
...
function rewriteConfigModelRefsWithCompactionPolicy(params) {
	...
	const defaultsRuntime = ignoreLegacyAgentRuntimePins ? void 0 : nextConfig.agents?.defaults?.agentRuntime;
...
function formatDisabledCodexPluginWarning(params) {
	const fixHint = params.blockedOutsideEntry ? "- Enable plugin loading and remove `codex` from plugins.deny, or set the affected OpenAI models to a PI runtime policy." : "- Run `openclaw doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to a PI runtime policy.";
	return [
...
function scanCodexSessionStoreRoutes(store) {
	return Object.entries(store).flatMap(([sessionKey, entry]) => {
		if (!entry) return [];
		return normalizeString(entry.modelProvider) === "openai-codex" || normalizeString(entry.providerOverride) === "openai-codex" || isOpenAICodexModelRef(entry.model) || isOpenAICodexModelRef(entry.modelOverride) || isOpenAICodexModelRef(entry.fallbackNoticeSelectedModel) || isOpenAICodexModelRef(entry.fallbackNoticeActiveModel) ? [sessionKey] : [];
	});
}
async function maybeRepairCodexSessionRoutes(params) {
	...
	if (!params.shouldRepair) {
		const stale = targets.flatMap((target) => {
			return scanCodexSessionStoreRoutes(loadSessionStore(target.storePath)).map((sessionKey) => `${target.agentId}:${sessionKey}`);
		});
		return {
	...
	for (const target of targets) {
		if (scanCodexSessionStoreRoutes(loadSessionStore(target.storePath)).length === 0) continue;
