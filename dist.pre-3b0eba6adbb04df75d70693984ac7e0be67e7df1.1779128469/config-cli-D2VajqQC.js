...
import { l as isValidEnvSecretRefId, m as resolveSecretInputRef, o as coerceSecretRef } from "./types.secrets-11Owfj8f.js";
...
import { n as discoverConfigSecretTargets, s as resolveConfigSecretTargetByPath } from "./target-registry-CUu9liyz.js";
import { a as formatExecSecretRefIdValidationMessage, c as isValidSecretProviderAlias, d as validateExecSecretRefId, o as isValidExecSecretRefId, s as isValidFileSecretRefId, u as secretRefKey } from "./ref-contract-Da5XKQDh.js";
import { a as resolveSecretRefValue } from "./resolve-DeoFl9S6.js";
import { n as info, o as success, t as danger } from "./globals-DaPK6X5S.js";
import { U as AUTO_MANAGED_CONFIG_META_PATHS, u as readConfigFileSnapshot, w as validateConfigObjectRawWithPlugins, x as collectUnsupportedSecretRefPolicyIssues } from "./io-CaHWZ8wj.js";
import { n as formatConfigIssueLines, o as normalizeConfigIssues } from "./issue-format-C780MyYm.js";
import { r as normalizeConfiguredProviderCatalogModelId } from "./model-ref-shared-BpLaMszT.js";
import { w as SecretProviderSchema } from "./zod-schema.core-DX5KkB1Y.js";
import { i as replaceConfigFile } from "./mutate-C2_btZYz.js";
import "./config-CoAMbpNB.js";
import { s as formatStrictJsonParseFailure } from "./error-format-W6MULocR.js";
import { n as redactConfigObject } from "./redact-snapshot-BOe-JYnI.js";
import { n as readBestEffortRuntimeConfigSchema } from "./runtime-schema-BDfoWmLx.js";
import { t as buildGatewayReloadPlan } from "./config-reload-plan-CdOruyBL.js";
import { n as setCommandJsonMode } from "./json-mode-DJXxnjPi.js";
...
function normalizeModelProviderRefsForConfigMutation(providers) {
	if (!isPlainRecord(providers)) return providers;
	let mutated = false;
...
function formatConfigReloadHint(plan) {
	if (plan.restartGateway) return "Restart the gateway to apply.";
	if (plan.hotReasons.length > 0) return plan.noopPaths.length > 0 ? "Gateway hot reload and dynamic reads will apply." : "Gateway hot reload will apply.";
	return "No gateway restart required.";
}
function resolveConfigReloadHintForPaths(paths) {
	return formatConfigReloadHint(buildGatewayReloadPlan([...paths].map(toDotPath).filter(Boolean)));
}
...
async function runConfigOperations(params) {
	...
	for (const operation of operations) {
		...
		if (operation.mutation === "merge" || options.merge && operation.mutation !== "replace") mergeAtPath(next, operation.setPath, operation.value, { numericObjectKeys: params.successMode === "patch" });
		else {
			...
			setAtPath(next, operation.setPath, operation.value, { numericObjectKeys: params.successMode === "patch" });
		}
	}
	...
	const reloadHint = resolveConfigReloadHintForPaths(operations.map((operation) => operation.setPath));
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/patch/unset/file/schema/validate). Run without subcommand for guided setup.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`).option("--section <section>", "Configuration sections for guided setup (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-CmTdKkXZ.js");
