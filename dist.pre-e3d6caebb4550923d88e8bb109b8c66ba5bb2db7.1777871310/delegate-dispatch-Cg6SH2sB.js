...
import { n as spawnSubagentDirect } from "./subagent-spawn-CW7s_sjG.js";
import { n as resolveContinuationRuntimeConfig } from "./config-Cko-TbTj.js";
import { i as failFlow } from "./task-flow-runtime-internal-B95uf9oE.js";
import { a as enqueueSystemEvent } from "./system-events-DLvm6-8V.js";
import { i as consumePendingDelegates, l as highestDelayedContinuationReservationHop, u as peekSoonestUnmaturedDelegateDueAt } from "./delegate-store-l1CMPLYy.js";
import { l as retainContinuationTimerRef, o as registerContinuationTimerHandle, u as unregisterContinuationTimerHandle } from "./state-d9jDSKQp.js";
...
async function dispatchStagedPostCompactionDelegates(delegates, sessionKey, spawnCtx) {
	...
	for (const delegate of delegates) try {
		...
		enqueueSystemEvent(`[continuation] Post-compaction delegate spawn ${spawnResult.status}: delegation was not accepted. Task: ${delegate.task}`, { sessionKey });
		failed++;
	} catch (err) {
		...
		enqueueSystemEvent(`[continuation] Post-compaction delegate spawn failed: ${String(err)}. Task: ${delegate.task}`, { sessionKey });
		failed++;
	}
	return {
		dispatched,
