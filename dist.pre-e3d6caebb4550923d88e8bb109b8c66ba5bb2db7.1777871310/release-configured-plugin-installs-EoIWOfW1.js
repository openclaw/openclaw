...
import { t as compareOpenClawVersions } from "./version-g1ztCCWD.js";
import "./registry-aY1NGwGo.js";
import { a as listPotentialConfiguredChannelPresenceSignals, o as collectConfiguredAgentHarnessRuntimes } from "./config-presence-Db5MsoBF.js";
import { t as isChannelConfigured } from "./channel-configured-CtwLAe-2.js";
import { r as detectPluginAutoEnableCandidates } from "./plugin-auto-enable-DGR2X1Ud.js";
...
import { n as repairMissingPluginInstallsForIds } from "./missing-configured-plugin-install-Gj8ekv-d.js";
...
function collectConfiguredProviderIds(cfg) {
	...
	const collectModelRef = (value) => {
		const ref = normalizeId(value);
		const slash = ref?.indexOf("/") ?? -1;
		if (ref && slash > 0) add(ref.slice(0, slash));
	};
	const collectModelConfig = (value) => {
		...
		if (Array.isArray(record.fallbacks)) for (const fallback of record.fallbacks) collectModelRef(fallback);
	};
	const collectAgent = (agent) => {
		...
		for (const modelRef of Object.keys(asObjectRecord(record.models) ?? {})) collectModelRef(modelRef);
	};
	collectAgent(cfg.agents?.defaults);
	for (const agent of Array.isArray(cfg.agents?.list) ? cfg.agents.list : []) collectAgent(agent);
	return ids;
}
