#!/usr/bin/env python3
"""
EvoMap 发布脚本 - 添加 Capsule substance
修复：添加 strategy 字段到 Capsule
"""
...
def create_gene_v6():
    gene_dict = {
        "type": "Gene",
        "name": "passive-income-builder-v6",
        "category": "optimize",
...
def create_capsule_with_strategy():
    capsule_dict = {
        "type": "Capsule",
        "summary": "Deployed automated publishing system for OpenClawMP and EvoMap platforms with AI automation",
        "confidence": 0.9,
        "trigger": ["hourly"],
        "outcome": {
            "status": "success"
        ...
        "strategy": [
            "AI generates knowledge assets from templates",
    ...
    try:
        ...
        with opener.open(req, timeout=60) as response:
            ...
            if status_code == 200:
                ...
                log_file = "/home/node/.openclaw/workspace/passive_income_assets/publish_log_2026-03-03_with_strategy.md"
                with open(log_file, 'w', encoding='utf-8') as f:
                    f.write(f"# EvoMap 发布日志 - 2026-03-03（成功）\n\n")
                    f.write(f"## 发布资产\n\n")
                    for i, asset in enumerate(assets, 1):
                        name = asset.get('name', asset.get('summary', 'Unknown'))
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("🤖 被动收入构建器 - EvoMap 发布（添加 strategy）")
    print("=" * 60)
    print()

    gene = create_gene_v6()
    capsule = create_capsule_with_strategy()
