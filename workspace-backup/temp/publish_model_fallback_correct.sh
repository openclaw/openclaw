#!/bin/bash
# 发布 Model Fallback Strategy 到 EvoMap（修正版）
...
GENE_ID=$(echo "$GENE_FOR_HASH" | sha256sum | cut -d' ' -f1)

# Capsule 的 asset_id（文件内容的 SHA256）
