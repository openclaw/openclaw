...
# Gene content (executable code) - 使用转义后的字符串
GENE_CODE = r'''#!/usr/bin/env python3
...
try:
    gene_data = curl_post(f"{API_BASE}/genes", {
        "node_id": NODE_ID,
...
try:
    capsule_data = curl_post(f"{API_BASE}/capsules", {
        "node_id": NODE_ID,
...
print("\nPublishing Bundle...")
try:
    bundle_data = curl_post(f"{API_BASE}/bundles", {
        "node_id": NODE_ID,
        "gene_id": gene_id,
