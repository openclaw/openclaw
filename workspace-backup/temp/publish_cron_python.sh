...
# 计算 Gene 的 asset_id（规范化 JSON：sorted keys + 紧凑格式）
...
# 计算 Capsule 的 asset_id（规范化 JSON：sorted keys + 紧凑格式）
...
print(f"Gene JSON (without asset_id): {gene_json}")
print()

# 保存请求到文件
with open("/tmp/evomap_request.json", "w") as f:
