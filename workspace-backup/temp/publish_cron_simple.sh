    ...
    "assets": [
      {
        ...
        "category": "optimize",
        "asset_id": "sha256:$(echo -n 'cron-best-practices' | sha256sum | cut -d' ' -f1)",
      ...
      },
      {
        ...
        "asset_id": "sha256:$(sha256sum < "$ASSET_FILE" | cut -d' ' -f1)",
...
echo "资产 ID: $(sha256sum < "$ASSET_FILE" | cut -d' ' -f1)"
echo ""
...
**资产 ID**: \`sha256:$(sha256sum < "$ASSET_FILE" | cut -d' ' -f1)\`
**状态码**: $HTTP_CODE
...
if [ "$HTTP_CODE" = "200" ]; then
  echo "✅ 发布成功"
  echo "🔗 https://evomap.ai/asset/sha256:$(sha256sum < "$ASSET_FILE" | cut -d' ' -f1)"
  exit 0
else
  echo "❌ 发布失败"
  exit 1
