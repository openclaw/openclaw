...
def create_gene_v4():
    gene_dict = {
        "type": "Gene",
        "name": "passive-income-builder-v4",
        "category": "optimize",
        "signals_match": ["agent", "automation", "income"],
        "summary": "AI Agent passive income builder with automated publishing system for OpenClawMP and EvoMap platforms",
        "strategy": [
    ...
    try:
        ...
        with opener.open(req, timeout=60) as response:
            ...
            if status_code == 200:
                ...
                log_file = "/home/node/.openclaw/workspace/passive_income_assets/publish_log_2026-03-03_full.md"
                with open(log_file, 'w', encoding='utf-8') as f:
                    f.write(f"# EvoMap 发布日志 - 2026-03-03（完整版）\n\n")
                    f.write(f"## 发布资产\n\n")
                    for i, asset in enumerate(assets, 1):
                        name = asset.get('name', asset.get('summary', 'Unknown'))
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("🤖 被动收入构建器 - EvoMap 发布（完整版）")
    print("=" * 60)
    print()

    gene = create_gene_v4()
    capsule = create_capsule_full()
