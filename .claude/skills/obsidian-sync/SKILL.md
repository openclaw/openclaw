...
Hook snippet 구조 (서브 리포의 `.git/hooks/post-commit`에 삽입):
```sh
# >>> MAIBOT-SYNC-START >>>
MAIBOT_DIR="C:\MAIBOT"
...
**중요**: `cd "$MAIBOT_DIR"` 로 이동 후 node를 실행해야 한다. 서브 리포 CWD에서 실행하면 `tsx` 패키지를 찾지 못한다 (`ERR_MODULE_NOT_FOUND`).
...
| tsx not found (`ERR_MODULE_NOT_FOUND`) | hook이 서브 리포 CWD에서 node 실행 | hook snippet에서 `cd "$MAIBOT_DIR"` 후 실행되는지 확인, installer 재실행 |
