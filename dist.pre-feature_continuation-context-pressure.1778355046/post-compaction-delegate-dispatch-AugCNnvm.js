...
import { t as generateChainId } from "./secure-random-UYE7uWu5.js";
import { n as resolveContinuationRuntimeConfig } from "./config-D9MPOF2I.js";
import { c as loadPendingSessionDelivery, i as enqueuePostCompactionDelegateDelivery, l as moveSessionDeliveryToFailed, o as failSessionDelivery, r as ackSessionDelivery, s as loadPendingSessionDeliveries, u as pruneFailedOlderThan } from "./targeting-DFUCVhIS.js";
import { a as enqueueSystemEvent } from "./system-events-CoCLzqF0.js";
import { t as consumeStagedPostCompactionDelegates } from "./continuation-delegate-store-XlKhp10v.js";
...
async function deliverQueuedPostCompactionDelegate(params, deps = defaultPostCompactionDelegateDeliveryDeps) {
	...
	const spawnResult = await deps.spawnSubagentDirect({
		...
		...params.entry.traceparent ? { traceparent: params.entry.traceparent } : {},
		...params.entry.attachments && params.entry.attachments.length > 0 ? { attachments: params.entry.attachments } : {},
		...params.entry.attachAs?.mountPath ? { attachMountPath: params.entry.attachAs.mountPath } : {}
	}, {
	...
	if (spawnResult.status !== "accepted") throw new Error(spawnResult.error ? `post-compaction delegate spawn ${spawnResult.status}: ${spawnResult.error}` : `post-compaction delegate spawn ${spawnResult.status}`);
