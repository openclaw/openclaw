...
import { a as SESSIONS_SPAWN_ATTACHMENT_METADATA_KEYS, c as isRedactableInlineAttachmentToolName, i as REDACTED_SESSIONS_SPAWN_ATTACHMENT_CONTENT, l as isRedactedSessionsSpawnAttachment, n as extractToolResultId, s as isAllowedToolCallName, t as extractToolCallsFromAssistant, u as normalizeAllowedToolNames } from "./tool-call-id-D1XJLn0C.js";
...
function sanitizeToolCallBlock(block) {
	...
	if (!isRedactableInlineAttachmentToolName(normalizedName)) {
		...
		for (const block of msg.content) {
			...
			if (isRawToolCallBlock(block)) {
				if (RAW_TOOL_CALL_BLOCK_TYPES.has(block.type ?? "")) {
					if (isRedactableInlineAttachmentToolName(typeof block.name === "string" ? block.name.trim() : void 0)) {
