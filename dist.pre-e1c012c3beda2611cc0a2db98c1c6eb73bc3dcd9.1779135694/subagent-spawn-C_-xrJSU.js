...
import { c as parseAgentSessionKey, i as isCronSessionKey } from "./session-key-utils-CJRKuBJA.js";
import { c as normalizeAgentId, s as isValidAgentId } from "./session-key-CQewiu8n.js";
import { a as resolveAgentDir, o as resolveAgentWorkspaceDir, r as resolveAgentConfig } from "./agent-scope-config-DdvF1onI.js";
import { t as privateFileStore } from "./private-file-store-BexMuv_j.js";
import { a as normalizeDiagnosticTraceparent } from "./diagnostic-trace-context-pure-DiETCmyi.js";
import { i as getRuntimeConfig } from "./io-DxVmbF3R.js";
import { p as normalizeThinkLevel, t as formatThinkingLevels } from "./thinking-Bzr_rdQG.js";
import "./config-CBeYX-pH.js";
import "./operator-scopes-D5DA_QCa.js";
import { r as isAdminOnlyMethod } from "./method-scopes-DfV0ICXA.js";
...
import { p as resolveSubagentSpawnModelSelection } from "./model-selection-BGs61_ek.js";
import { t as isAcpRuntimeSpawnAvailable } from "./availability-BoizeP8y.js";
import { u as resolveThreadBindingSpawnPolicy } from "./thread-bindings-policy-Btk9SdbL.js";
import { a as resolveSubagentCapabilities, f as inheritedToolAllowPatch, h as normalizeInheritedToolDenylist, m as normalizeInheritedToolAllowlist, p as inheritedToolDenyPatch, s as getSubagentDepthFromSessionStore } from "./subagent-capabilities-p9yM-kRv.js";
import { a as normalizeSpawnedRunMetadata, i as mapToolContextToSpawnedRunMetadata, r as resolveSubagentTargetPolicy, s as resolveSpawnedWorkspaceInheritance, t as resolveRequesterOriginForChild } from "./spawn-requester-origin-BR86G80s.js";
import { n as countActiveRunsForSession, r as registerSubagentRun } from "./subagent-registry-spawn-runtime-CLrsr0SO.js";
import { c as resolveMainSessionAlias, o as resolveDisplaySessionKey, s as resolveInternalSessionKey } from "./sessions-helpers-Bw2JkI8j.js";
import { n as resolveParentForkDecision, t as forkSessionFromParent } from "./session-fork-Dl48bKWC.js";
import { t as ensureContextEnginesInitialized } from "./init-DgBbglL-.js";
import { m as resolveGatewaySessionStoreTarget, u as pruneLegacyStoreKeys } from "./session-utils-CWqQa0gp.js";
...
//#endregion
//#region src/agents/subagent-traceparent-handoff.ts
const SUBAGENT_TRACEPARENT_HANDOFF_TTL_MS = 300 * 1e3;
const traceparentHandoffs = /* @__PURE__ */ new Map();
function handoffKey(params) {
	return `${params.idempotencyKey}\0${params.sessionKey}`;
}
function pruneExpiredSubagentTraceparentHandoffs(nowMs) {
	for (const [key, entry] of traceparentHandoffs) if (entry.expiresAtMs <= nowMs) traceparentHandoffs.delete(key);
}
function registerSubagentTraceparentHandoff(params) {
	const idempotencyKey = normalizeOptionalString(params.idempotencyKey);
	const sessionKey = normalizeOptionalString(params.sessionKey);
	const traceparent = normalizeDiagnosticTraceparent(params.traceparent);
	if (!idempotencyKey || !sessionKey || !traceparent) return;
...
function consumeSubagentTraceparentHandoff(params) {
	const idempotencyKey = normalizeOptionalString(params.idempotencyKey);
	const sessionKey = normalizeOptionalString(params.sessionKey);
	if (!idempotencyKey || !sessionKey) return;
	const nowMs = params.nowMs ?? Date.now();
	pruneExpiredSubagentTraceparentHandoffs(nowMs);
...
function buildDirectChildSessionPatch(patch) {
	...
	const continuationTraceparent = normalizeDiagnosticTraceparent(typeof patch.continuationTraceparent === "string" ? patch.continuationTraceparent : void 0);
	if (continuationTraceparent) entry.continuationTraceparent = continuationTraceparent;
...
async function spawnSubagentDirect(params, ctx) {
	...
	const inheritedWorkspaceDir = targetAgentId !== requesterAgentId ? void 0 : toolSpawnMetadata.workspaceDir;
...
//#endregion
export { SUBAGENT_SPAWN_SANDBOX_MODES as a, normalizeSubagentTaskName as c, SUBAGENT_SPAWN_SESSION_ACCEPTED_NOTE as d, decodeStrictBase64 as f, SUBAGENT_SPAWN_MODES as i, resolveSubagentSpawnOwnership as l, testing as n, splitModelRef as o, SUBAGENT_SPAWN_CONTEXT_MODES as r, consumeSubagentTraceparentHandoff as s, spawnSubagentDirect as t, SUBAGENT_SPAWN_ACCEPTED_NOTE as u };
