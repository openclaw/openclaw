import { f as sweepStaleRunContexts } from "./agent-events-DkpIHRmj.js";
import { r as cleanOldMedia } from "./store-C25GuYX2.js";
import { t as abortChatRunById } from "./chat-abort-DS0xFwab.js";
import { r as HEALTH_REFRESH_INTERVAL_MS, s as TICK_INTERVAL_MS, t as DEDUPE_MAX } from "./server-constants-BUku2xyU.js";
import { n as pruneStaleControlPlaneBuckets } from "./control-plane-rate-limit-Cvf1DsiN.js";
import { t as formatError } from "./server-utils-Bk3BAMDO.js";
import { s as setBroadcastHealthUpdate } from "./health-state-C1IUpMiv.js";
...
function startGatewayMaintenanceTimers(params) {
	...
	const dedupeCleanup = setInterval(() => {
		...
		for (const [k, v] of params.dedupe) {
			if (isActiveRunDedupeKey(k, v)) continue;
			if (now - v.ts > 3e5) params.dedupe.delete(k);
		}
		if (params.dedupe.size > 1e3) {
			const excess = params.dedupe.size - DEDUPE_MAX;
			const oldestKeys = [...params.dedupe.entries()].filter(([key, entry]) => !isActiveRunDedupeKey(key, entry)).toSorted(([, left], [, right]) => left.ts - right.ts).slice(0, excess).map(([key]) => key);
		...
		const clearAgentThrottleStateForRun = (runId) => {
