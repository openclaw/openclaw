...
import { a as parseAssistantTextSignature, s as resolveAssistantMessagePhase } from "./chat-message-content-ZRefYnI2.js";
import "./internal-runtime-context-tBUvZkew.js";
import { g as stripEnvelopeFromMessages } from "./session-utils.fs-DC7uJpmA.js";
...
import { n as isSuppressedControlReplyText } from "./control-reply-text-BXxGWwnL.js";
...
function hasAssistantMixedToolVisibleText(message) {
	...
	for (const block of content) {
		...
		if (entry.type === "text" && typeof entry.text === "string" && entry.text.trim()) hasText = true;
	}
...
function projectChatDisplayMessages(messages, options) {
	return sanitizeChatHistoryMessages(mergeTtsSupplementMessages(filterVisibleProjectedHistoryMessages(toProjectedMessages(sanitizeChatHistoryMessages(options?.stripEnvelope === false ? messages : stripEnvelopeFromMessages(messages), Number.MAX_SAFE_INTEGER)))), options?.maxChars ?? 8e3);
}
