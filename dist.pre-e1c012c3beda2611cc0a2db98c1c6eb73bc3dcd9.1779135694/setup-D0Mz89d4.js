...
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-Cara5-ZM.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-CH7IjYSv.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-BNa9KgSu.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-rGAqs4qV.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-BPgBaExs.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-C4IeD04B.js");
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-M4lIcq3C.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-Cqg2wbnC.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { setupChannels } = await import("./onboard-channels-DGOQmulS.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-DZVOkHsa.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-13bCdzXp.js");
