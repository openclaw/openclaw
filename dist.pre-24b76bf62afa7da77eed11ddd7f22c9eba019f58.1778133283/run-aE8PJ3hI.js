...
import { S as setVerbose } from "./logger-CQ0p51p4.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-DH24jjKy.js";
...
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-CEscmVCn.js";
import { t as parsePort } from "./parse-port-BBpv2kmd.js";
import { i as withDiagnosticPhase } from "./diagnostic-phase-CEdgrVPf.js";
import { r as withProgress } from "./progress-UGUqAH-r.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-IgUZUJkZ.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-Blkfa-96.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-BwMoZrK4.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-BYz5qqUz.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-D4Xi0R4X.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-D-vSzVnI.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-Dlh2A7ws.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-BEuz-EeT.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-bfdpZd60.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-Bfv5_FzG.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-CvK0C68c.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-Bx3D7dYK.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-DnA7j8ss.js");
