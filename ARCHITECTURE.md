...
## 4. Core Patches (~400+ lines, ~20 files)
...
```
CORE PATCHES (~400+ lines, ~20 files):
...
- Push webhook verification token is optional; when unset, endpoint security relies on gateway auth + network boundary controls
...
├── OpenClaw (forked, ~400+ lines changed)
...
**Total core diff: ~400+ lines across ~20 files.**
