import { n as isVitestRuntimeEnv } from "./env-p2fClQGo.js";
...
import { i as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-BAyp62ec.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-sL3hQA17.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-CXjgVYXl.js");
		const { deliverOutboundPayloads } = await import("./deliver-CEAzg2HF.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-BaRg1bnA.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-BSK8q8ns.js");
		if (stopped) return;
