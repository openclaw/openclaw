...
import { S as setVerbose } from "./logger-rPxGv0hY.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-FbMq-ElM.js";
...
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-BMdoYZVC.js";
import { t as parsePort } from "./parse-port-lOseyz1J.js";
import { i as withDiagnosticPhase } from "./diagnostic-phase-C_rZCGRH.js";
import { r as withProgress } from "./progress-Cwglgs0e.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-3BFzHJTF.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-CU9Klr-6.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-OqlZsMsG.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-Cffxi_kW.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-CNlpijev.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BHd8PTfA.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-drU-uAii.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-Ctd9u65r.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-DBLCAg-i.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-ezBxgP5c.js");
	...
	const { resolveGatewayAuth } = await import("./auth-WWVe_yJt.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-CltvimE4.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-C_bS6mMo.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-DoqZ9ZKD.js");
