#!/usr/bin/env node
import { i as normalizeEnv, t as isTruthyEnvValue } from "./env-DZStR-gf.js";
...
import { a as resolveCliContainerTarget, i as parseCliContainerArgs, n as parseCliProfileArgs, t as applyCliProfileEnv } from "./profile-Bfe-LcFR.js";
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-DhqtXiRQ.js"), import("./git-commit-DT00-tRJ.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		if (!(deps.outputPrecomputedRootHelpText ?? (await import("./root-help-metadata-COAYXjav.js")).outputPrecomputedRootHelpText)()) {
			const { outputRootHelp } = await import("./root-help-CrrXTkut.js");
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-BUSW2dkI.js");
