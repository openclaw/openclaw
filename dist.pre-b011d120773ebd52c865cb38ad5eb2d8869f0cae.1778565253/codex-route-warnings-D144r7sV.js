import { t as AGENT_MODEL_CONFIG_KEYS } from "./model-refs-kVKyT56i.js";
import { t as resolveModelRuntimePolicy } from "./model-runtime-policy-BeXwz0TB.js";
import { a as openAIProviderUsesCodexRuntimeByDefault } from "./openai-codex-routing-DLqe3PZx.js";
import { t as loadSessionStore } from "./store-load-CfT9U9P4.js";
import { s as updateSessionStore } from "./store-DWV_PSjG.js";
import { r as resolveAllAgentSessionStoreTargetsSync } from "./targets-BRdMnLMu.js";
...
function resolveRuntime(params) {
	return normalizeString(params.env?.OPENCLAW_AGENT_RUNTIME) ?? normalizeString(params.agentRuntime?.id) ?? normalizeString(params.defaultsRuntime?.id);
}
...
function resolveCurrentRuntimeIdForCanonicalModel(params) {
	...
	const configured = normalizeString(resolveModelRuntimePolicy({
		config: params.cfg,
...
function rewriteConfigModelRefs(params) {
	...
	const runtimePinChanges = hits.length > 0 ? clearConfigLegacyAgentRuntimePolicies(nextConfig) : [];
	return {
		cfg: hits.length > 0 || runtimePolicyChanges.length > 0 || runtimePinChanges.length > 0 ? nextConfig : params.cfg,
		changes: hits,
		runtimePinChanges,
		runtimePolicyChanges
	};
