...
import { S as setVerbose } from "./logger-tDhXRPrK.js";
import "./globals-DaPK6X5S.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-GeI1UuV8.js";
import { t as createSubsystemLogger } from "./subsystem-BCvJ25zm.js";
import { i as isLoopbackHost, p as resolveGatewayBindHost, t as defaultGatewayBindMode, v as isContainerEnvironment } from "./net-CQP-kC3g.js";
...
import { i as withDiagnosticPhase } from "./diagnostic-phase-Cp12dDAc.js";
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-BBKjfMnE.js";
import { r as withProgress } from "./progress-CWqykc_n.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-D0Ht0DBi.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-Dx6qJcEJ.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-DCMEZDuh.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-DC9GOpa_.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-rB7h-_ID.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BLpXZ14c.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-CFGcEtRV.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-_hyIN_oP.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-B_4UypZK.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-ZO0UHt3l.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-DnjZVANz.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-CmcmlrLH.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-Cw4f1sYV.js");
