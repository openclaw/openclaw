...
import { m as mergeSsrFPolicies } from "./ssrf-DzOdS68g.js";
import { n as fetchWithSsrFGuard } from "./fetch-guard-DO86P4h3.js";
import { i as assertOkOrThrowProviderError, r as assertOkOrThrowHttpError } from "./provider-http-errors-ho1SUoaN.js";
import { p as resolveProviderHttpRequestConfig } from "./shared-xkyON1a3.js";
import { l as isProviderApiKeyConfigured } from "./provider-auth-D-JuU8Uc.js";
import "./string-coerce-runtime-DVkpEjUr.js";
import { n as buildHostnameAllowlistPolicyFromSuffixAllowlist, u as ssrfPolicyFromDangerouslyAllowPrivateNetwork } from "./ssrf-policy-DYNSxqZe.js";
import "./ssrf-runtime-Cu4zbqxY.js";
import { o as resolveApiKeyForProvider } from "./provider-auth-runtime-BZXyHGf7.js";
import "./provider-http-Dbl2jV7_.js";
import { a as imageFileExtensionForMimeType, u as toImageDataUrl } from "./image-generation-wBOi7ZhG.js";
...
function buildFalImageGenerationProvider() {
	return {
		...
		async generateImage(req) {
			...
			if (hasInputImages && isGptImageEditModel && inputImageCount > GPT_IMAGE_EDIT_MAX_INPUT_IMAGES) throw new Error(`fal GPT Image edit supports at most ${GPT_IMAGE_EDIT_MAX_INPUT_IMAGES} reference images (requested ${inputImageCount})`);
			if (hasInputImages && isNanoBananaEditModel && inputImageCount > NANO_BANANA_EDIT_MAX_INPUT_IMAGES) throw new Error(`fal Nano Banana edit supports at most ${NANO_BANANA_EDIT_MAX_INPUT_IMAGES} reference images (requested ${inputImageCount})`);
			if (hasInputImages && !isGptImageEditModel && !isNanoBananaEditModel) {
				if (inputImageCount > 1) throw new Error("fal flux image generation currently supports at most one reference image");
			...
			if (imageSize !== void 0) if (model.startsWith("fal-ai/nano-banana-") && hasInputImages) {
				if (req.aspectRatio) requestBody.aspect_ratio = req.aspectRatio;
				if (req.resolution) requestBody.resolution = req.resolution;
			} else requestBody.image_size = imageSize;
			if (hasInputImages) {
				const [input] = req.inputImages ?? [];
				if (!input) throw new Error("fal image edit request missing reference image");
				if (isGptImageEditModel || isNanoBananaEditModel) requestBody.image_urls = req.inputImages.map((img) => toImageDataUrl(img));
				else requestBody.image_url = toImageDataUrl(input);
			}
			const { response, release } = await falFetchGuard({
				url: `${baseUrl}/${model}`,
				init: {
