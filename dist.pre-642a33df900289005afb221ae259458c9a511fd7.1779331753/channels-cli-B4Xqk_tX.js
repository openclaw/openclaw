...
import { t as danger } from "./globals-DENIQrbE.js";
import { i as getRuntimeConfig, u as readConfigFileSnapshot } from "./io-CmeoeBvq.js";
import "./config-B3BdVqTi.js";
import { l as formatUnsupportedChannelActionMessage } from "./error-format-BfFDdqsq.js";
import { i as GATEWAY_CLIENT_NAMES, r as GATEWAY_CLIENT_MODES } from "./client-info-BVWE_ra1.js";
import "./message-channel-DIO4k9AW.js";
import { r as callGateway } from "./call-C4cvr8It.js";
import { t as applyPluginAutoEnable } from "./plugin-auto-enable-8qFd3zxN.js";
import { a as normalizeChannelId, i as listChannelPlugins } from "./registry-CbQuetgM.js";
...
import { t as commitConfigWithPendingPluginInstalls } from "./plugins-install-record-commit-CWEZ_otY.js";
import { t as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-Co2_M1ih.js";
import { t as formatCliChannelOptions } from "./channel-options-F4viywp8.js";
...
const channelsCommandsLoader = createLazyImportLoader(() => import("./channels-BMxGkMfd.js"));
const bundledPackageChannelMetadataLoader = createLazyImportLoader(() => import("./bundled-package-channel-metadata-B7sXoHPn.js"));
...
async function registerChannelsCli(program, argv = process.argv, options = {}) {
	...
	channels.command("list").description("List chat channels (configured by default; pass --all for installable catalog)").option("--all", "Include bundled and installable catalog channels", false).option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsListCommand } = await import("./list-CnwZ9gGs.js");
	...
	channels.command("status").description("Show gateway channel status (use status --deep for local)").option("--channel <name>", `Only show one channel (${formatCliChannelOptions(["all"])})`).option("--probe", "Probe channel credentials", false).option("--timeout <ms>", "Timeout in ms", "10000").option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsStatusCommand } = await import("./status-4Wg0FqeM.js");
