#!/usr/bin/env node
import { i as normalizeEnv, t as isTruthyEnvValue } from "./env-BE3qwYMq.js";
...
import { a as resolveCliContainerTarget, i as parseCliContainerArgs, n as parseCliProfileArgs, t as applyCliProfileEnv } from "./profile-DmxPejo0.js";
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-C1-6jjhw.js"), import("./git-commit-BDb0UpsR.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		if (!(deps.outputPrecomputedRootHelpText ?? (await import("./root-help-metadata-D00z1S1d.js")).outputPrecomputedRootHelpText)()) {
			const { outputRootHelp } = await import("./root-help-DjhJvZOA.js");
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-E6SqplO0.js");
