...
import { r as withProgress } from "./progress-BCZgbmna.js";
//#region src/cli/daemon-cli/probe.ts
const probeGatewayModuleLoader = createLazyImportLoader(() => import("./probe-DlnEM5Ez.js"));
...
async function probeGatewayStatus(opts) {
	...
	try {
		const result = await withProgress({
			...
			if (opts.requireRpc) {
				const { callGateway } = await import("./call-D7gJQgj1.js");
				await callGateway({
					url: opts.url,
					token: opts.token,
					password: opts.password,
