...
import { t as getPluginRuntimeGatewayRequestScope } from "./gateway-request-scope-5Nb6fnfy.js";
...
async function resolveAgentId(params) {
	...
	const { resolveDefaultAgentId } = await import("./agent-scope-7LV8YhEs.js");
...
function createRuntimeLlm(options = {}) {
	...
	return { complete: async (params) => {
		...
		const [{ prepareSimpleCompletionModelForAgent, completeWithPreparedSimpleCompletionModel, resolveSimpleCompletionSelectionForAgent }, cfg] = await Promise.all([import("./simple-completion-runtime-CDJ54l9d.js"), Promise.resolve(resolveRuntimeConfig(options))]);
