...
import { d as markMigrationItemSkipped, g as redactMigrationPlan, v as summarizeMigrationItems } from "./migration-D8r0JUFO.js";
...
function reconcileInteractiveMigrationEnterValues(selectedValues, activatedValue, selectableValues, opts = {}) {
	if (activatedValue === "__openclaw_migrate_skip_for_now__") return [MIGRATION_SELECTION_SKIP];
	if (activatedValue === "__openclaw_migrate_toggle_all_on__") return [MIGRATION_SELECTION_TOGGLE_ALL_ON, ...selectableValues];
	if (activatedValue === "__openclaw_migrate_toggle_all_off__") return [MIGRATION_SELECTION_TOGGLE_ALL_OFF];
	if (activatedValue !== void 0 && selectableValues.includes(activatedValue)) {
		const selectedSelectableValues = selectedValues.filter((value) => value !== "__openclaw_migrate_toggle_all_on__" && value !== "__openclaw_migrate_toggle_all_off__" && value !== "__openclaw_migrate_skip_for_now__");
