import { r as getFsSafePythonConfig, t as canFallbackFromPythonError } from "./fs-safe-defaults-DsMJI6H_.js";
import { i as isPathInside, n as hasNodeErrorCode, o as isSymlinkOpenError, p as FsSafeError, r as isNotFoundPathError, t as assertNoNulPathInput } from "./path-Bv_HaZDb.js";
import { t as sameFileIdentity } from "./file-identity-Cff8f9mS.js";
import { n as normalizeOptionalString } from "./string-coerce-b8me7dqA.js";
import { c as assertAsyncDirectoryGuard, d as createNearestExistingDirectoryGuard, n as registerTempPathForExit, s as withAsyncDirectoryGuards, t as serializePathWrite, u as createAsyncDirectoryGuard } from "./write-queue-BOD4ekDx.js";
import { r as resolveRootPath, t as ROOT_PATH_ALIAS_POLICIES } from "./root-path-sLzD43u1.js";
...
async function mkdirPathComponentsWithGuards(params) {
	...
	if (isPathEscape(relative)) throw new FsSafeError("outside-workspace", "directory is outside workspace root");
	let current = root;
	for (const part of relative.split(path.sep).filter(Boolean)) {
		...
		if (!isSameOrChildPath(path.resolve(await fs$1.realpath(next)), root)) throw new FsSafeError("outside-workspace", "directory escaped workspace root");
...
function getWorker() {
	...
	child.once("error", (error) => {
		rejectPending(isSpawnUnavailable(error) ? new FsSafeError("helper-unavailable", "Python helper is unavailable", { cause: error }) : error instanceof Error ? error : new Error(String(error)));
	});
...
async function runPinnedWriteFallback(params) {
	...
	if (params.mkdir) await withAsyncDirectoryGuards([parentGuard], async () => {
		await fs$1.mkdir(parentPath, { recursive: true });
	});
...
//#endregion
export { resolveOpenedFileRealPathForHandle as a, PATH_ALIAS_POLICIES as c, resolveHomeRelativePath as d, resolveUserPath as f, readLocalFileSafely as i, assertNoPathAliasEscape as l, DEFAULT_ROOT_MAX_BYTES as n, root as o, openLocalFileSafely as r, getFsSafeTestHooks as s, resolveSecureTempRoot as t, expandHomePrefix as u };
