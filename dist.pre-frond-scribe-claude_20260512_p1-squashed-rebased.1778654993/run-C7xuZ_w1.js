...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-Ce_TY4Ls.js";
import { t as createSubsystemLogger } from "./subsystem-DT3cKML_.js";
import { t as isContainerEnvironment } from "./container-environment-nBlcuOod.js";
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-Dpr-pD-p.js";
...
import { i as withDiagnosticPhase } from "./diagnostic-phase-B7WG05Lz.js";
import { r as withProgress } from "./progress-aEUO5DaE.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-DbirQXth.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-Uuh6h7IY.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-DC61wmc_.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-vygDfsOZ.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-Csy377ex.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BsyJeGVq.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-DKFpMcFd.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-Bdqht31Y.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-D4DqBKUI.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-BvIkzcSm.js");
	...
	const { resolveGatewayAuth } = await import("./auth-DUYchMYb.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-C_JU8m-u.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-bDTgnNH3.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-Dt1Qo138.js");
