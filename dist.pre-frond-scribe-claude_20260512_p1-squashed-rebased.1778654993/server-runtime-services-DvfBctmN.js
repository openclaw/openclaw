import { n as isVitestRuntimeEnv } from "./env-AQ99Lilo.js";
import { t as createSubsystemLogger } from "./subsystem-DT3cKML_.js";
import { t as isGatewayModelPricingEnabled } from "./model-pricing-config-c5M7NSmp.js";
import { i as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-jgIHEI5Q.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-Dq7EoD7Q.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-Du8JaIFw.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-CMIiDqYt.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-CF0Y8Fvb.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-BH4L88_U.js");
		if (stopped) return;
