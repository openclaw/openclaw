...
import { d as normalizeSecretInputString } from "./types.secrets-0GsiwJ5t.js";
import { r as normalizeProviderId } from "./provider-id-BvxMxU5i.js";
import { n as defaultRuntime } from "./runtime-E6YfuGL9.js";
import { r as createConfigIO } from "./io-D39r-SPY.js";
...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-CEoZojgv.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-CwlXPIEL.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-Dxf5v_aP.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-CWpbzRmI.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-DpKOwl_D.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-L4g8kb-M.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-CfGH0M89.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-D5uKRwA9.js"), import("./setup-registry-BaRSKQPi.js")]);
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-CUfCwjLj.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CHtdeeui.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-CHj7XGHh.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-CHj7XGHh.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-8tZA6tLl.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-BL0VpShY.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-DiaGScOn.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-BsGvzX2i.js");
		const { setupChannels } = await import("./onboard-channels-Cb21uQSc.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BOT7Tqrb.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-CoRI_ChU.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-SVzfuHc7.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-B2Uz5QWS.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-C1mlVvyz.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BsRhx0tG.js");
