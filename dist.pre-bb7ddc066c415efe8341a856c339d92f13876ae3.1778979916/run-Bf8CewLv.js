...
import { t as CONFIG_PATH, u as resolveGatewayPort, v as resolveStateDir } from "./paths-Cnwfh6dH.js";
...
import { S as setVerbose } from "./logger-CdnoaNwY.js";
import "./globals-Cnlrc0S3.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-gTFG3fFl.js";
...
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-CLmerLvz.js";
import { n as formatInvalidConfigPort, r as formatInvalidPortOption } from "./error-format-C27-4rQ7.js";
import { t as parsePort } from "./parse-port-B_miNtJR.js";
import { i as withDiagnosticPhase } from "./diagnostic-phase-BLv_11Yd.js";
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-QzdbhtSX.js";
import { r as withProgress } from "./progress-Ba4gvSqF.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-Cnmvcsmi.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-UFiLjJmK.js";
			...
			try {
				if (isRestart) {
					...
					await measureGatewayRestartTrace("restart.drain", async () => {
						const { abortEmbeddedPiRun, getInspectableActiveTaskRestartBlockers, getActiveEmbeddedRunCount, getActiveTaskCount, waitForActiveEmbeddedRuns, waitForActiveTasks } = await loadGatewayLifecycleRuntimeModule();
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-BBEInGyG.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-DqgcyZfD.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-BidWDJuh.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-_sLJOQ9e.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-BgHtLWuH.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-DxNUJb0E.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-Cer1iucI.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-D7sDdfVF.js");
	...
	const { resolveGatewayAuth } = await import("./auth-C2KHQGwr.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-B0taeNMH.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-b5cMiwZA.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-CfENGcCX.js");
