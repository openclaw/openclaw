...
const SecuritySchema = z.object({ audit: z.object({ suppressions: z.array(z.object({
...
const McpServerSchema = z.object({
	...
	codex: z.object({
		agents: z.array(z.string().trim().regex(/^[a-z0-9][a-z0-9_-]{0,63}$/i)).min(1).optional(),
		defaultToolsApprovalMode: z.enum([
			"auto",
			"prompt",
			"approve"
		]).optional(),
		default_tools_approval_mode: z.enum([
			"auto",
			"prompt",
			"approve"
		]).optional()
...
const OpenClawSchema = z.object({
	...
	surfaces: z.record(z.string(), z.object({ silentReply: SilentReplyPolicyConfigSchema.optional() }).strict()).optional(),
