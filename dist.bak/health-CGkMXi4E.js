...
import { Ar as setHeartbeatWakeHandler, Dr as peekSystemEventEntries, Hn as buildOutboundSessionContext, It as resolveHeartbeatDeliveryTarget, Lt as resolveHeartbeatSenderContext, Mr as resolveHeartbeatReasonKind, Sr as stripHeartbeatToken, Yn as resolveUserTimezone, _r as CommandLane, br as isHeartbeatContentEffectivelyEmpty, dr as getQueueSize, jr as isHeartbeatEventDrivenReason, kr as requestHeartbeatNow, r as appendCronStyleCurrentTimeLine, t as getReplyFromConfig, vr as DEFAULT_HEARTBEAT_ACK_MAX_CHARS, xr as resolveHeartbeatPrompt$1, yr as DEFAULT_HEARTBEAT_EVERY } from "./reply-D8YvfZps.js";
import { d as resolveDefaultAgentId, i as resolveAgentConfig, u as resolveAgentWorkspaceDir, y as DEFAULT_HEARTBEAT_FILENAME } from "./agent-scope-D8K2SjR7.js";
...
import { dn as parseDurationMs, zt as loadConfig } from "./model-selection-DILdVnl8.js";
import { t as isTruthyEnvValue } from "./env-BqIeOdP-.js";
import { i as hasErrnoCode, n as formatErrorMessage } from "./errors-8IxbaLwV.js";
import { X as canonicalizeMainSessionAlias, Z as resolveAgentMainSessionKey, c as loadSessionStore, h as updateSessionStore, p as saveSessionStore } from "./sessions-DUzDEcXs.js";
import { n as listChannelPlugins, t as getChannelPlugin } from "./plugins-B9xwwhdE.js";
import { a as resolvePreferredAccountId, t as buildChannelAccountBindings } from "./bindings-DXaMWXSi.js";
import { l as resolveStorePath, r as resolveSessionFilePath } from "./paths-B_bX6Iw-.js";
import { t as HEARTBEAT_TOKEN } from "./tokens-LFopHMoh.js";
import { t as deliverOutboundPayloads } from "./deliver-B0fNH-zB.js";
import { n as callGateway, t as buildGatewayConnectionDetails } from "./call-BP56BqJF.js";
import { o as resolveEffectiveMessagesConfig } from "./reply-prefix-CdgjH0e2.js";
...
async function runHeartbeatOnce(opts) {
	...
	if (!isHeartbeatEventDrivenReason(opts.reason)) {
...
function startHeartbeatRunner(opts) {
	...
	const run = async (params) => {
		...
		if (!heartbeatsEnabled && !isEventDriven) return {
			status: "skipped",
			reason: "disabled"
		};
		if (state.agents.size === 0 && !isEventDriven) return {
			status: "skipped",
			reason: "disabled"
		};
		...
		if (requestedSessionKey || requestedAgentId) {
			...
			const effectiveHeartbeat = targetAgent?.heartbeat ?? resolveHeartbeatConfig(state.cfg, targetAgentId);
			try {
				...
				if (targetAgent && (res.status !== "skipped" || res.reason !== "disabled")) advanceAgentSchedule(targetAgent, now);
				scheduleNext();
				...
				if (targetAgent) advanceAgentSchedule(targetAgent, now);
				scheduleNext();
				return {
					status: "failed",
					reason: errMsg
