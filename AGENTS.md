...
- Avoid schema changes. When one is needed, prefer a same-version additive change (a new table, or a bare nullable column; see [versioning](docs/reference/database-schemas/versioning.md)). A schema-version bump needs maintainer discussion before implementation and matching older-updater support.
