...
- **Main CI runs hourly.** Pushes to `main` run only `security-fast`; full `main` validation is the hourly `workflow_dispatch` CI run. A green push run is not a green `main` — watch the hourly run, and a red hourly run is red `main`.
