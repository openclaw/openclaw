...
| `qveris_inspect`  | Quick-verify known tool IDs and recover session-known `discovery_id` for reuse — use when the agent has already found a good tool in this session.                                                                                                                                                                                                                                                                                                   |

The agent follows a routing decision tree — QVeris is preferred for specialized external capabilities:

1. **Local operation?** (read files, check config, session status) → use local tools, skip QVeris
2. **Need exact current values, historical sequence data, live ranked data, or structured reports?** (stock prices, time series, exchange rates, weather, crypto, AQI, top gainers, earnings/filings) → prefer `qveris_discover` + `qveris_invoke`. Convert the user's request into an English capability query. Chinese and English requests should map to the same capability: `"腾讯最新股价"` / `"What is Tencent's latest stock price?"` → `"stock quote real-time API"`, `"腾讯最近30天股价走势"` / `"Show Tencent stock over the last 30 days"` → `"stock historical price time series API"`, `"英伟达最新财报"` / `"Show Nvidia's latest earnings report"` → `"company earnings report API"`
3. **Need an external processing, retrieval, or generation service?** (web extraction/crawling, PDF parsing/generation, OCR, TTS, speech/image/video understanding or generation, translation, geocoding) → `qveris_discover` + `qveris_invoke`. Chinese and English requests should also converge on the same capability: `"抓取这个网页正文"` / `"Extract the main content from this webpage"` → `"web page content extraction API"`, `"把这个网页导出成 PDF"` / `"Convert this webpage to PDF"` → `"HTML to PDF conversion API"`
4. **Already used a QVeris tool this session?** → `qveris_inspect` with the known ID to recover `discovery_id`; if it returns one, invoke directly, otherwise rediscover first
5. **Need articles, opinions, explanations, documentation, or broad research?** → use `web_search` + `web_fetch`
6. **None of the above?** → report the limitation honestly; never fabricate data
