...
function normalizeRawCredentialEntry(raw) {
	...
	if (entry.type === "oauth") {
		...
		const expires = normalizeExpiryField(entry.expires);
		if (expires !== void 0) normalized.expires = expires;
		if (isOAuthProfileSecretRef(entry.oauthRef)) normalized.oauthRef = entry.oauthRef;
		return normalized;
	}
	return entry;
}
...
function shouldReadMacKeychainForOAuthProfileSecrets(params) {
	const env = params?.env ?? process.env;
	return (params?.platform ?? process.platform) === "darwin" && env.VITEST !== "true" && env.VITEST_WORKER_ID === void 0;
}
function resolveOAuthProfileSecretKeySeedWithDeps(options, deps) {
	const externalKey = deps.env[OAUTH_PROFILE_SECRET_KEY_ENV]?.trim();
	if (externalKey) return externalKey;
	if (deps.env.NODE_ENV === "test" && deps.env.VITEST === "true") return "openclaw-test-oauth-profile-secret-key";
	if (shouldReadMacKeychainForOAuthProfileSecrets({
		env: deps.env,
	...
	const fileKey = deps.readFile() ?? (options?.create === true ? deps.createFile() : void 0);
