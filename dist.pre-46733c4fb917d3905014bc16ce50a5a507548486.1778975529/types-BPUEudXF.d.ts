import { a as PromptMode, d as AgentStreamParams, f as ClientToolDefinition, l as AgentInternalEvent, p as ExecElevatedDefaults } from "./params-DEguTk67.js";
import { c as SourceReplyDeliveryMode, d as PromptImageOrderEntry, n as ContinuationTrigger } from "./get-reply-options.types-CGgi3jrA.js";
import { t as InputProvenance } from "./input-provenance-DOTdpOTI.js";
import { O as ChannelOutboundTargetMode } from "./types.core-BHltg72J.js";
...
type AgentCommandOpts = {
  ...
  bashElevated?: ExecElevatedDefaults; /** Whether this caller is authorized for owner-only tools (defaults true for local CLI calls). */
  ...
  lane?: string;
  runId?: string; /** Structured lifecycle metadata for internally-triggered turns. */
  continuationTrigger?: ContinuationTrigger; /** When true, the run drains the pending continuation delegate queue after completion. */
  drainsContinuationDelegateQueue?: boolean; /** Inherited continuation trace context for this run. */
  traceparent?: string;
  ...
  inputProvenance?: InputProvenance; /** Visible source replies must be sent through the message tool when set. */
  sourceReplyDeliveryMode?: SourceReplyDeliveryMode; /** Per-call stream param overrides (best-effort). */
  streamParams?: AgentStreamParams; /** Explicit workspace directory override (for subagents to inherit parent workspace). */
  workspaceDir?: SpawnedRunMetadata["workspaceDir"]; /** Force bundled MCP teardown when a one-shot local run completes. */
...
type AgentCommandIngressOpts = Omit<AgentCommandOpts, "senderIsOwner" | "allowModelOverride" | "resultMetaOverrides"> & {
  /** Ingress callsites must always pass explicit owner-tool authorization state. */senderIsOwner: boolean; /** Ingress callsites must always pass explicit model-override authorization state. */
