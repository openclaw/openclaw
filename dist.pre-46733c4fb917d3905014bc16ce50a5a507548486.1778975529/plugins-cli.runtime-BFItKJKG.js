...
async function runPluginsEnableCommand(id) {
	...
	const { applySlotSelectionForPlugin, logSlotWarnings } = await import("./plugins-command-helpers-DgussqnO.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-Bs_3XWx0.js");
...
async function runPluginsDisableCommand(id) {
	...
	const { normalizePluginId } = await import("./config-state-CDowxzeJ.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-FAwjTWmx.js");
	const { setPluginEnabledInConfig } = await import("./plugins-config-BGMQx4ep.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-Bs_3XWx0.js");
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-Cnlr9aQd.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-RU8vbWAK.js");
...
async function runPluginsDoctorCommand() {
	const { buildPluginCompatibilityNotices, buildPluginDiagnosticsReport, formatPluginCompatibilityNotice } = await import("./status-FAwjTWmx.js");
	const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-DZ9a5VSy.js");
	const cfg = getRuntimeConfig();
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-CoMMAhiu.js");
	const { createPluginInstallLogger } = await import("./plugins-command-helpers-DgussqnO.js");
