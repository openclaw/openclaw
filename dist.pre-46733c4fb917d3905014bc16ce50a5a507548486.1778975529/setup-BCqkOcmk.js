...
import { d as normalizeSecretInputString } from "./types.secrets-Co1lNtpa.js";
import { r as normalizeProviderId } from "./provider-id-Cz7K6wgK.js";
import { n as defaultRuntime } from "./runtime-DDH_zqCr.js";
import { r as createConfigIO } from "./io-DEZ0cwqj.js";
import { i as replaceConfigFile } from "./mutate-9b4cZaiQ.js";
import "./config-qlJph4Y8.js";
import { t as WizardCancelledError } from "./prompts-Ck_pZJ0x.js";
import { n as t } from "./i18n-DyU5xbsE.js";
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-D47dIgAB.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-cxZgCTYZ.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-CphQztFu.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-DpUFFVrm.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-BgOO-kVD.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-Dm_qgTOk.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-BgXJPwl3.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-CC_M2osP.js"), import("./setup-registry-CI1Oyosw.js")]);
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-DMEYnpR5.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-Sx0lvqiF.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-BlXKJHL-.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-BlXKJHL-.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-BPhH8k61.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-fnyvafzW.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-DY6PoLZ3.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-DGL2FIeU.js");
		const { setupChannels } = await import("./onboard-channels-BvPUKWUL.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-B9nVNNTv.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-DAHWKloF.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-CQ7BGBA2.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-CizPl_PU.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-Dx4Kpoel.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-C8RmErqy.js");
