...
import { n as CHAT_CHANNEL_ORDER } from "./ids-DbFBIQba.js";
import { t as sanitizeTerminalText } from "./safe-text-D4vNi2Ke.js";
import { t as enablePluginInConfig } from "./enable-D6tREeS9.js";
import { n as listChatChannels } from "./chat-meta-DNnFqIhM.js";
import { n as formatChannelSelectionLine, t as formatChannelPrimerLine } from "./registry-BkmpEVNf.js";
import { d as listBundledChannelSetupPlugins, i as getBundledChannelSetupPlugin } from "./bundled-kjKUfT4R.js";
import { t as isChannelConfigured } from "./channel-configured-CVTJvV2o.js";
import { t as getActivePluginChannelRegistry, v as requireActivePluginRegistry } from "./runtime-DsnS2jud.js";
import { i as resolveChannelDefaultAccountId } from "./helpers-C42hXXcT.js";
import { t as buildChannelSetupWizardAdapterFromSetupWizard } from "./setup-wizard-D5MohG79.js";
import { i as resolveBundledPluginSources, n as findBundledPluginSourceInMap } from "./bundled-sources-wRst8EEf.js";
import { n as loadChannelSetupPluginRegistrySnapshotForChannel, t as ensureChannelSetupPluginInstalled } from "./plugin-install-CZl3BkRF.js";
import { r as listTrustedChannelPluginCatalogEntries, t as getTrustedChannelPluginCatalogEntry } from "./trusted-catalog-BUqclZAC.js";
import { i as shouldShowChannelInSetup, r as resolveChannelSetupEntries } from "./discovery-C2FtyKui.js";
...
async function noteChannelPrimer(prompter, channels) {
	...
	await prompter.note([
		...
		"For multi-user DMs, isolate sessions with: " + formatCliCommand("openclaw config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels).",
		`Docs: ${formatDocsLink("/channels/pairing", "channels/pairing")}`,
		"",
		...channelLines
	].join("\n"), "How channels work");
...
async function setupChannels(cfg, runtime, prompter, options) {
	...
	const enableBundledPluginForSetup = async (channel) => {
		...
		if (disabledHint) {
			await prompter.note(`${channel} cannot be configured while ${disabledHint}. Enable it, then run ${formatCliCommand("openclaw channels add")} again.`, "Channel setup");
			return false;
		}
		const result = enablePluginInConfig(next, channel);
		next = result.config;
		if (!result.enabled) {
			await prompter.note(`Cannot enable ${channel}: ${result.reason ?? "plugin disabled"}. Run ${formatCliCommand("openclaw plugins list")} to inspect plugin state.`, "Channel setup");
			return false;
		}
	...
	const configureChannel = async (channel) => {
		...
		const adapter = getVisibleSetupFlowAdapter(channel);
		if (!adapter) {
			await prompter.note(`${channel} does not have an interactive setup screen yet. Run ${formatCliCommand(`openclaw channels add --channel ${channel} --help`)} for supported flags.`, "Channel setup");
			return;
		}
