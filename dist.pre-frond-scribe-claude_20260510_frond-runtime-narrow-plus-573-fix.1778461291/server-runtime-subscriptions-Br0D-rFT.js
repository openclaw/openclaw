import { l as onAgentEvent, t as clearAgentRunContext } from "./agent-events-DJVWZi1F.js";
import { n as onSessionTranscriptUpdate } from "./transcript-events-WgHlH1S0.js";
import { n as onSessionLifecycleEvent } from "./session-lifecycle-events-9VdtqbhN.js";
import { r as onHeartbeatEvent } from "./heartbeat-events-CrnZJ8Qx.js";
...
function startGatewayEventSubscriptions(params) {
	...
	const getAgentEventHandler = () => {
		agentEventHandlerPromise ??= Promise.all([import("./server-chat-DZ5kp0Eb.js"), import("./server-session-key-Dxbhr4mf.js")]).then(([{ createAgentEventHandler }, { resolveSessionKeyForRun }]) => createAgentEventHandler({
	...
	const getTranscriptUpdateHandler = () => {
		transcriptUpdateHandlerPromise ??= import("./server-session-events-C_hM8n_8.js").then(({ createTranscriptUpdateBroadcastHandler }) => createTranscriptUpdateBroadcastHandler({
			broadcastToConnIds: params.broadcastToConnIds,
			sessionEventSubscribers: params.sessionEventSubscribers,
	...
	const getLifecycleEventHandler = () => {
		lifecycleEventHandlerPromise ??= import("./server-session-events-C_hM8n_8.js").then(({ createLifecycleEventBroadcastHandler }) => createLifecycleEventBroadcastHandler({
			broadcastToConnIds: params.broadcastToConnIds,
			sessionEventSubscribers: params.sessionEventSubscribers
		}));
