...
import { a as resetGlobalHookRunner } from "./hook-runner-global-Dn0qn0nC.js";
...
function installChannelOutboundPayloadContractSuite(params) {
	...
	it("single media delegates to sendMedia", async () => {
		...
		const result = await run();
		expect(sendMock).toHaveBeenCalledTimes(1);
		expect(sendMock).toHaveBeenCalledWith(to, "cap", expect.objectContaining({ mediaUrl: "https://example.com/a.jpg" }));
		expect(result).toMatchObject({ channel: params.channel });
	});
	it("multi-media iterates URLs with caption on first", async () => {
		...
		expect(sendMock).toHaveBeenNthCalledWith(1, to, "caption", expect.objectContaining({ mediaUrl: "https://example.com/1.jpg" }));
		expect(sendMock).toHaveBeenNthCalledWith(2, to, "", expect.objectContaining({ mediaUrl: "https://example.com/2.jpg" }));
