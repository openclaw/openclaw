...
import { t as clearRuntimeAuthProfileStoreSnapshots } from "../store-DSz4632-.js";
import { i as runProviderCatalog } from "../provider-discovery-BF4zVqVF.js";
import { s as listSupportedMusicGenerationModes } from "../openclaw-tools-D-G6kiLc.js";
import { r as createCapturedPluginRegistration } from "../bundled-capability-runtime-CXGYLffP.js";
import { a as resolveWebFetchProviderContractEntriesForPluginId, i as resolveProviderContractProvidersForPluginIds, n as providerContractLoadError, o as resolveWebSearchProviderContractEntriesForPluginId, s as resolveBundledExplicitProviderContractsFromPublicArtifacts, t as pluginRegistrationContractRegistry } from "../registry-B_YLL4jc.js";
import { i as resolveBundledExplicitWebSearchProvidersFromPublicArtifacts, r as resolveBundledExplicitWebFetchProvidersFromPublicArtifacts } from "../web-provider-public-artifacts.explicit-IL-6Gyzh.js";
import { r as listSupportedVideoGenerationModes } from "../duration-support-DS10xX0C.js";
import "../provider-onboard-BUyG9wYK.js";
import "../runtime-BH1nnZGN.js";
import "../agent-runtime-DZt2NBNQ.js";
import { b as registerSingleProviderPlugin, v as registerProviderPlugin, x as requireRegisteredProvider, y as registerProviderPlugins } from "../plugin-setup-wizard-CTZmep56.js";
import { a as expectedAugmentedOpenaiCodexCatalogEntriesWithGpt55, i as expectCodexMissingAuthHint, o as expectedOpenaiPluginCodexCatalogEntriesWithGpt55, r as expectAugmentedCodexCatalog } from "../testing-DKAOjXOT.js";
import { i as resolveProviderWizardOptions, n as resolveProviderModelPickerEntries, o as setProviderWizardProvidersResolverForTest, t as buildProviderPluginMethodChoice } from "../provider-wizard-BSwYICil.js";
import { r as resolveProviderPluginChoice } from "../provider-auth-choice.runtime-DcpbL_Sk.js";
import { i as makeResponse, r as createProviderUsageFetch } from "../frozen-time-CEQ4VRsk.js";
import { n as loadBundledPluginPublicSurfaceSync, t as loadBundledPluginPublicSurface } from "../public-surface-loader-Dqi6_JcK.js";
import "../plugin-test-runtime-DOwxwcRW.js";
import "../test-env-BLO1o1eE.js";
...
function describeOpenAICodexProviderAuthContract(load) {
	...
	describe("openai-codex provider auth contract", () => {
		...
		it("keeps OAuth failures non-fatal at the provider layer", async () => {
			...
			await expect(provider.auth[0]?.run(buildAuthContext())).resolves.toEqual({ profiles: [] });
		});
	});
}
	...
	describe("github-copilot provider discovery contract", () => {
		...
		it("keeps env-token base URL resolution provider-owned", async () => {
			...
			expect(resolveCopilotApiTokenMock).toHaveBeenCalledWith({
				...
				env: expect.objectContaining({ GITHUB_TOKEN: "github-env-token" })
			});
		});
	});
}
...
function describeMinimaxProviderDiscoveryContract(load) {
	...
	describe("minimax provider discovery contract", () => {
		...
		it("keeps API catalog provider-owned", async () => {
			await expect(state.runProviderCatalog({
				...
				authHeader: true,
				apiKey: "minimax-key",
				models: expect.arrayContaining([expect.objectContaining({ id: "MiniMax-M2.7" }), expect.objectContaining({ id: "MiniMax-M2.7-highspeed" })])
			} });
		});
...
function describeOpenAIProviderRuntimeContract(load) {
	describe("openai provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
		...
		it("owns direct openai transport normalization", () => {
			expect(requireProviderContractProvider("openai").normalizeResolvedModel?.({
				provider: "openai",
				modelId: "gpt-5.4",
				model: createModel({
					id: "gpt-5.4",
...
function describeVeniceProviderRuntimeContract(load) {
	describe("venice provider runtime contract", { timeout: CONTRACT_SETUP_TIMEOUT_MS }, () => {
		...
		it("owns xai downstream compat flags for grok-backed Venice models", () => {
			expect(requireProviderContractProvider("venice").normalizeResolvedModel?.({
				provider: "venice",
