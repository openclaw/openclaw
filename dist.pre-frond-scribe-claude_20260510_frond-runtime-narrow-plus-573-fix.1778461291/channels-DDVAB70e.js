...
import "./message-channel-JLy5SBEA.js";
import { i as callGateway } from "./call-ClRxJfOt.js";
import { i as getBundledChannelSetupPlugin } from "./bundled-kjKUfT4R.js";
import { a as normalizeChannelId$1, n as getLoadedChannelPlugin, t as getChannelPlugin } from "./registry-xj8wnFVP.js";
...
import { l as resolveMessageActionDiscoveryForPlugin, r as createMessageActionDiscoveryContext } from "./message-action-discovery-BmnhCQSX.js";
...
import { i as resolveChannelDefaultAccountId, r as parseOptionalDelimitedEntries } from "./helpers-C42hXXcT.js";
import { c as moveSingleAccountChannelSectionToDefaultAccount } from "./setup-helpers-Bsl8k4FM.js";
import { t as createClackPrompter } from "./clack-prompter-Bhmb1ny0.js";
import { t as commitConfigWithPendingPluginInstalls } from "./plugins-install-record-commit-D0PRNqiA.js";
import { t as refreshPluginRegistryAfterConfigMutation } from "./plugins-registry-refresh-D1P2vbM2.js";
import { t as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-BdW6Gol3.js";
import { t as resolveCommandConfigWithSecrets } from "./command-config-resolution-B-VHJ-Ru.js";
import { n as resolveLogFile } from "./log-tail-DdAxJxGp.js";
import { t as parseLogLine } from "./parse-log-line-Btj2ZynU.js";
import { a as describeBinding, t as applyAgentBindings } from "./agents.bindings-NUD4GEsB.js";
import { t as requireValidConfigFileSnapshot } from "./config-validation-fW3X_yom.js";
import { c as shouldUseWizard, o as formatChannelAccountLabel, s as requireValidConfig } from "./shared-CmOSnk36.js";
import { t as channelsListCommand } from "./list-Da1ljlYV.js";
import { n as formatGatewayChannelsStatusLines, t as channelsStatusCommand } from "./status-BHJoXTMF.js";
...
const channelSetupPluginInstallLoader = createLazyImportLoader(() => import("./plugin-install-k3lGqQu0.js"));
const onboardChannelsLoader = createLazyImportLoader(() => import("./onboard-channels-ohf5TJM2.js"));
...
async function resolveCatalogChannelEntry(raw, cfg) {
	...
	if (!trimmed) return;
	return (cfg ? await import("./trusted-catalog-B_5XRcLX.js").then(({ listTrustedChannelPluginCatalogEntries }) => listTrustedChannelPluginCatalogEntries({
		cfg,
		workspaceDir: resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg))
	})) : await import("./catalog-D8EgQ_92.js").then(({ listChannelPluginCatalogEntries }) => listChannelPluginCatalogEntries({ excludeWorkspace: true }))).find((entry) => {
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	if (shouldUseWizard(params)) {
		const [{ buildAgentSummaries }, onboardChannels] = await Promise.all([import("./agents.config-DvEJdWE5.js"), loadOnboardChannels()]);
	...
	if (!channel && catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-C-pAr_po.js");
	...
	if (!channel) {
		const hint = catalogEntry ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run openclaw doctor --fix, then retry openclaw channels add.` : `Unknown channel: ${rawChannel}. Run openclaw channels list --all to see configured and installable channels.`;
		runtime.error(hint);
		runtime.exit(1);
		return;
	}
	...
	if (!plugin?.setup?.applyAccountConfig) {
		runtime.error(`Channel ${channel} does not support non-interactive add. Run openclaw channels add with no flags for guided setup, or openclaw channels list --all to inspect available channels.`);
		runtime.exit(1);
		return;
	}
...
async function channelsCapabilitiesCommand(opts, runtime = defaultRuntime) {
	...
	if (!selected || selected.length === 0) {
		runtime.error(danger(`Unknown channel "${rawChannel}". Run ${formatCliCommand("openclaw channels list")} to see installed channels.`));
		runtime.exit(1);
		return;
	}
	const reports = [];
...
async function channelsRemoveCommand(opts, runtime = defaultRuntime, params) {
	...
	const resolvedPluginState = Boolean(lookupChannel || channel) ? await (async () => {
		const { resolveInstallableChannelPlugin } = await import("./channel-plugin-resolution-D-yEr2NX.js");
	...
	if (!resolvedChannel) {
		runtime.error(`Unknown channel "${rawChannel}". Run ${formatCliCommand("openclaw channels list")} to see supported channels.`);
		runtime.exit(1);
		return;
	}
	...
	if (deleteConfig) {
		if (!plugin.config.deleteAccount) {
			runtime.error(`Channel "${channel}" does not support delete. Use disable/remove without --delete.`);
			runtime.exit(1);
			return;
		}
		...
		if (!plugin.config.setAccountEnabled) {
			runtime.error(`Channel "${channel}" does not support disable. Use --delete only if you want to remove config.`);
			runtime.exit(1);
			return;
		}
...
async function channelsResolveCommand(opts, runtime) {
	...
	if (!plugin?.resolver?.resolveTargets) {
		...
		throw new Error(`Channel ${channelText} does not support resolve. Run ${formatCliCommand("openclaw channels capabilities --channel " + channelText)} to inspect supported actions.`);
	}
