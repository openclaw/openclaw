...
import { i as createLazyRuntimeNamedExport } from "./lazy-runtime-OJqeil7F.js";
...
import { n as resolveChannelGroupRequireMention, r as resolveChannelGroupToolsPolicy } from "./group-policy-BbKPc6Se.js";
import { t as readBooleanParam } from "./boolean-param-VJw8D0Ji.js";
...
import { n as IMESSAGE_ACTIONS, r as IMESSAGE_ACTION_NAMES, t as describeIMessageMessageTool } from "./message-tool-api-B1PqeTOk.js";
import "./client-rE_PORUI.js";
import { t as getCachedIMessagePrivateApiStatus } from "./private-api-status-DI1_2gIF.js";
import { s as parseIMessageTarget } from "./targets-B5kz3YxZ.js";
import { t as findLatestIMessageEntryForChat } from "./monitor-reply-cache-BwNT8TQK.js";
import "./probe-l6igR5M4.js";
//#region extensions/imessage/src/actions.ts
const loadIMessageActionsRuntime = createLazyRuntimeNamedExport(() => import("./actions.runtime-DSR526zc.js"), "imessageActionsRuntime");
	...
	handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
		...
		const assertPrivateApiEnabled = async () => {
			if (privateApiStatus?.available !== true) {
				const { probeIMessagePrivateApi } = await import("./probe-DyGsQ4rC.js");
				privateApiStatus = await probeIMessagePrivateApi(cliPathForProbe, account.config.probeTimeoutMs ?? 1e4);
			}
			if (!privateApiStatus?.available) throw new Error(`iMessage ${action} requires the imsg private API bridge. Run imsg launch, then openclaw channels status to refresh capability detection.`);
		};
		const opts = {
		...
		if (action === "reply") {
			...
			return jsonResult({
				ok: true,
				messageId: (await runtime.sendRichMessage({
					chatGuid: resolvedChatGuid,
					text,
		...
		if (action === "sendAttachment" || action === "upload-file") {
			...
			return jsonResult({
				ok: true,
				messageId: (await runtime.sendAttachment({
					chatGuid: resolvedChatGuid,
					buffer: decodeBase64Buffer(params, action),
					filename,
