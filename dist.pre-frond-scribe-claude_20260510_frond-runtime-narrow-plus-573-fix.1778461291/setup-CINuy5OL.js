...
import { d as normalizeSecretInputString } from "./types.secrets-m6J7qPGz.js";
import { r as normalizeProviderId } from "./provider-id-C3dkeX_L.js";
import { n as defaultRuntime } from "./runtime-Bd9_VI2J.js";
import { r as createConfigIO } from "./io-BedkIX2f.js";
import { r as replaceConfigFile } from "./mutate-DkqBz8pD.js";
import "./config-DX4vfpUR.js";
import { t as WizardCancelledError } from "./prompts-BKq7NR7y.js";
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-Bl4sLmRp.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-D0PRNqiA.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-BtBt7UqQ.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-C4LE87ny.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-DVMRNebF.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-zeSk9MkN.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-0rVzwUp_.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-DT0evL03.js"), import("./setup-registry-Bu_TsJda.js")]);
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-C-1FNzrf.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-jvRecIu_.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-aUyTvUad.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-lxdE1n-3.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-lxdE1n-3.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-BTQFnPEV.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-B_BTSCht.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-GUzeaKVm.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-DIn_wWDK.js");
		const { setupChannels } = await import("./onboard-channels-ohf5TJM2.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-6ov2hsAC.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-D4vh_ygU.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-DgJD5W1X.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-DnGhxF1P.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-CpjbAzXk.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-lsQPwotZ.js");
