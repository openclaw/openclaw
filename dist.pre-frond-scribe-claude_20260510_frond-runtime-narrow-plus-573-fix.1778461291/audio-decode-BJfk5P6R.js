...
const decoders = {
	...
	async mp3(buf) {
		...
		if (!decoder) {
			const { MPEGDecoder } = await import("./mpg123-decoder-nQ7Ris3f.js");
	...
	async opus(buf) {
		...
		if (!decoder) {
			const { OggOpusDecoder } = await import("./ogg-opus-decoder-C8JQusHU.js");
	...
	async wav(buf) {
		...
		if (!decode) {
			let module = await import("./node-wav-x_zeevaO.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
	...
	async qoa(buf) {
		...
		if (!decode) decoders.qoa.decode = decode = (await import("./qoa-format-ByTbMH5s.js")).decode;
		return buf && createBuffer(await decode(buf));
	}
};
