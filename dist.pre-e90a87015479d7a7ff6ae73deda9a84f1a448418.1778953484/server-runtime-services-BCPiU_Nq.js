import { n as isVitestRuntimeEnv } from "./env-DRvegM_Y.js";
...
import { r as startHeartbeatRunner } from "./heartbeat-runner-7G8b-7iQ.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-DkKmTKT8.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-CsQSorn1.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-4lzJsQq4.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-BOQrJi2N.js");
		if (stopped) return;
