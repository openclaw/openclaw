...
import { n as listBuiltRuntimeEntryCandidates } from "./package-entrypoints-CxjrFxsc.js";
import { t as extensionUsesSkippedScannerPath } from "./scan-paths-QIB2erM6.js";
import { t as getGlobalHookRunner } from "./hook-runner-global-CkZcU59h.js";
import { i as scanDirectoryWithSummary } from "./skill-scanner-CiuaJj1T.js";
...
function rebuildBuiltinScanCounts(scan) {
	let critical = 0;
	let warn = 0;
	let info = 0;
	for (const finding of scan.findings) if (finding.severity === "critical") critical += 1;
...
const DEFAULT_INSTALLED_PACKAGE_CODE_SCAN_MAX_FILES = 25e3;
...
function resolveInstalledPackageCodeScanMaxFiles() {
	return readPositiveIntegerEnv("OPENCLAW_INSTALL_SCAN_MAX_CODE_FILES", DEFAULT_INSTALLED_PACKAGE_CODE_SCAN_MAX_FILES);
}
...
function normalizeRelativeScanPath(relativePath) {
	return relativePath.split(path.sep).join("/");
}
function isKnownBenignLanceDbFinding(params) {
	const relativePath = normalizeRelativeScanPath(path.relative(params.packageDir, params.finding.file));
	const evidence = params.finding.evidence ?? "";
	if (params.finding.ruleId === "dangerous-exec" && relativePath === "dist/native.js") return evidence.includes("child_process") && /\bexecSync\(\s*['"](?:ldd --version|which ldd)['"]/.test(evidence);
	if (params.finding.ruleId === "dynamic-code-execution" && relativePath === "dist/embedding/transformers.js") return /\beval\(\s*['"]import\(["']@huggingface\/transformers["']\)['"]\s*\)/.test(evidence);
	return false;
}
async function suppressKnownBenignInstalledDependencyFindings(params) {
	if (params.builtinScan.status !== "ok" || params.builtinScan.findings.length === 0) return params.builtinScan;
	if ((await tryReadJson(path.join(params.packageDir, "package.json")))?.name !== "@lancedb/lancedb") return params.builtinScan;
	const findings = params.builtinScan.findings.filter((finding) => !isKnownBenignLanceDbFinding({
		finding,
		packageDir: params.packageDir
	}));
	if (findings.length === params.builtinScan.findings.length) return params.builtinScan;
...
async function scanInstalledPackageDependencyTreeRuntime(params) {
	const directoryScanRoots = await collectNonOverlappingPackageScanRoots(await collectInstalledPackageScanRoots({
	...
	const pluginRootRealPath = await fs.realpath(params.packageDir).catch(() => path.resolve(params.packageDir));
	for (const packageDir of directoryScanRoots) {
		if (remainingMaxFiles <= 0) return resolveBuiltinScanDecision({
			builtinScan: buildBuiltinScanFromError("code safety scan reached file limit (configured limit)"),
			logger: params.logger,
			dangerouslyForceUnsafeInstall: params.dangerouslyForceUnsafeInstall,
			trustedSourceLinkedOfficialInstall: params.trustedSourceLinkedOfficialInstall,
			targetLabel: `Plugin "${params.pluginId}" installation`
		});
		const isPluginRoot = await fs.realpath(packageDir).catch(() => path.resolve(packageDir)) === pluginRootRealPath;
		...
		if (params.trustedSourceLinkedOfficialInstall !== true && builtinScan.status === "ok") {
			if (builtinScan.critical > 0) params.logger.warn?.(`${installedTreeWarningMessage}: ${buildCriticalDetails({ findings: builtinScan.findings })}`);
			else if (builtinScan.warn > 0) params.logger.warn?.(installedTreeSuspiciousMessage.replace("{count}", String(builtinScan.warn)).replace("{target}", params.pluginId));
		}
		const builtinBlocked = resolveBuiltinScanDecision({
			builtinScan,
			logger: params.logger,
		...
		if (builtinBlocked) return builtinBlocked;
