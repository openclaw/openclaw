...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-8WW2qZJU.js";
...
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-BYZmrkLW.js";
...
import { i as withDiagnosticPhase } from "./diagnostic-phase-BWunwXyx.js";
import { r as withProgress } from "./progress-N8EyzJgH.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-DhkYI3e8.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-DCqsLvc5.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-CWBxex59.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-C-dZsKCq.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-D0-dImjw.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-CRDq6X53.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-BKqWs40I.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-DnTxJGvn.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-Dn0Sr7m9.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-BTC4lsxq.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-DK2oYvYB.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-BKIYAe0x.js");
