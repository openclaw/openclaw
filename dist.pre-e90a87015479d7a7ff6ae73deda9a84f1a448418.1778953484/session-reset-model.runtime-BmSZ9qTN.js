...
import { l as isModelKeyAllowedBySet } from "./model-selection-shared-DvIdVzM0.js";
import { t as applyModelOverrideToSessionEntry } from "./model-overrides-CATeiDEF.js";
import { n as resolveModelDirectiveSelection, r as resolveModelRefFromDirectiveString, t as modelKey } from "./model-selection-directive-Bk8TseEC.js";
...
async function loadResetModelCatalog(cfg) {
	const { loadModelCatalog } = await import("./model-catalog-Du_YCouG.js");
	return loadModelCatalog({ config: cfg });
}
async function resolveResetFallbackModels(params) {
	if (params.agentId) {
		const { resolveAgentModelFallbacksOverride } = await import("./agent-scope-DSLu8euX.js");
...
async function buildResetAllowedModelKeys(params) {
	if (Object.keys(params.cfg.agents?.defaults?.models ?? {}).length > 0 || params.cfg.models?.providers) {
		const { buildAllowedModelSetWithFallbacks } = await import("./model-selection-shared-BEqhNCnG.js");
