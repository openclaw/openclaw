...
function classifyComplexity(message: string, context: any = {}): {
  complexity: string;
  config: any;
  score: number;
  reasoning: string;
...
const tokenOptimizerPlugin = {
  ...
  version: '1.0.0',
  description: 'Pre-flight complexity classification and intelligent routing for token savings',
  ...
  register: (api: any) => {
    ...
    api.on('message_received', async (event: any, ctx: any) => {
      ...
      if (api.pluginConfig?.logClassifications) {
        api.logger.info(`[Token Optimizer] ${classification.complexity} | ${classification.reasoning}`);
      }
      ...
      if (api.pluginConfig?.enableBudgetDowngrade) {
        effectiveTier = (tokenBudget as any).getRecommendedTier(classification.complexity);
        if (effectiveTier !== classification.complexity) {
          api.logger.info(`[Token Optimizer] Budget constraint: ${classification.complexity} -> ${effectiveTier}`);
        }
      }
      ...
      api.logger.debug(`[Token Optimizer] Classified: ${classification.complexity} -> ${effectiveTier} (${content.length} chars)`);
    });
    
    // Register a gateway method to get classification info
    api.registerGatewayMethod('token_optimizer_classify', async (params: any) => {
    ...
    api.registerCommand({
      ...
      handler: async (args: any, options: any) => {
        ...
        const percentUsed = ((tokenBudget as any).dailyLimit - remaining) / (tokenBudget as any).dailyLimit * 100;
        ...
        return {
          ...
          dailyLimit: (tokenBudget as any).dailyLimit,
          shouldDowngrade: (tokenBudget as any).shouldDowngrade(),
          lastClassification: (globalThis as any).tokenOptimizerLastClassification,
        };
      },
    });
