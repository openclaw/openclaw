...
- regenerates apps/ios/Mullusi.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${MULLUSI_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid MULLUSI_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid MULLUSI_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid MULLUSI_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid MULLUSI_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing MULLUSI_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(MULLUSI_URL_SLASH)$(MULLUSI_URL_SLASH)#g'
)"
...
MULLUSI_CODE_SIGN_STYLE = Automatic
MULLUSI_DEVELOPMENT_TEAM = ${TEAM_ID}
MULLUSI_IOS_SELECTED_TEAM = ${TEAM_ID}
MULLUSI_APP_BUNDLE_ID = ai.mullusi.client
MULLUSI_SHARE_BUNDLE_ID = ai.mullusi.client.share
MULLUSI_ACTIVITY_WIDGET_BUNDLE_ID = ai.mullusi.client.activitywidget
MULLUSI_WATCH_APP_BUNDLE_ID = ai.mullusi.client.watchkitapp
MULLUSI_WATCH_EXTENSION_BUNDLE_ID = ai.mullusi.client.watchkitapp.extension
MULLUSI_APP_PROFILE =
MULLUSI_SHARE_PROFILE =
MULLUSI_PUSH_TRANSPORT = relay
MULLUSI_PUSH_DISTRIBUTION = official
MULLUSI_URL_SLASH = /
MULLUSI_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
MULLUSI_PUSH_APNS_ENVIRONMENT = production
EOF
