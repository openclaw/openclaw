/**
 * openclaw-capital-se-dashboard-dashboardserver-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/dashboard/DashboardServer.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-dashboard-dashboardserver.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "dashboard", "DashboardServer.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-dashboard-dashboardserver-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeDashboardDashboardserverGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeDashboardDashboardserverGate({
