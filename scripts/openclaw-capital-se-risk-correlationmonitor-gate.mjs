/**
 * openclaw-capital-se-risk-correlationmonitor-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/risk/CorrelationMonitor.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-risk-correlationmonitor.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "risk", "CorrelationMonitor.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-risk-correlationmonitor-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeRiskCorrelationmonitorGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeRiskCorrelationmonitorGate({
