/**
 * openclaw-capital-se-strategies-breakoutstrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/BreakoutStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-breakoutstrategy.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategies", "BreakoutStrategy.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-breakoutstrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesBreakoutstrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesBreakoutstrategyGate({
