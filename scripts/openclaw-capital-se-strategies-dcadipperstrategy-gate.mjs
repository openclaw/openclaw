/**
 * openclaw-capital-se-strategies-dcadipperstrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/DcaDipperStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-dcadipperstrategy.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategies", "DcaDipperStrategy.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-dcadipperstrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesDcadipperstrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesDcadipperstrategyGate({
