 ...
 * `activi update`. Use it to verify the completion installation flow
...
\`activi update\`, \`activi doctor\`, and \`activi onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Activi Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`activi.completion --install\` later to enable.`));
    return;
  }
