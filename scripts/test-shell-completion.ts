 ...
 * `EasyHub update`. Use it to verify the completion installation flow
...
\`EasyHub update\`, \`EasyHub doctor\`, and \`EasyHub onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# EasyHub Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`EasyHub completion --install\` later to enable.`));
    return;
  }
