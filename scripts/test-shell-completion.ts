 ...
 * `smart-agent-neo update`. Use it to verify the completion installation flow
...
const CLI_NAME = "smart-agent-neo";
...
\`smart-agent-neo update\`, \`smart-agent-neo doctor\`, and \`smart-agent-neo onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SmartAgentNeo Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`smart-agent-neo completion --install\` later to enable.`));
    return;
  }
