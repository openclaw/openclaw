 ...
 * `mullusi update`. Use it to verify the completion installation flow
...
\`mullusi update\`, \`mullusi doctor\`, and \`mullusi onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Mullusi Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`mullusi completion --install\` later to enable.`));
    return;
  }
