...
from scripts.memory_hub.openclaw_hook_bridge import OPENCLAW_HUB_ROOT, build_task_completed_event, build_user_confirmed_event

INGEST = ROOT / "scripts" / "memory_hub_ingest_event.py"


def run_ingest(event: dict) -> None:
    event_json = OPENCLAW_HUB_ROOT / "tmp-openclaw-hook-event.json"
    event_json.parent.mkdir(parents=True, exist_ok=True)
    event_json.write_text(json.dumps(event, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    subprocess.run(
        [sys.executable, str(INGEST), "--hub-root", str(OPENCLAW_HUB_ROOT), "--event-json", str(event_json)],
        check=False,
