...
# Mullusi Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://mullusi.com/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Mullusi."
...
GUM_VERSION="${MULLUSI_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Mullusi Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://mullusi.com/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$MULLUSI_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.mullusi.com/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_mullusi_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.mullusi-* "$npm_root"/mullusi 2>/dev/null || true
}

extract_mullusi_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_mullusi_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/mullusi" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/mullusi"|"/usr/local/bin/mullusi")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/mullusi/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale mullusi symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing mullusi binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Mullusi package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_mullusi_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Mullusi npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*mullusi" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_mullusi_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_mullusi_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Mullusi npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an mullusi binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("I'll butter your workflow like a mullusi roll: messy, delicious, effective.")
...
pick_tagline() {
    ...
    if [[ -n "${MULLUSI_TAGLINE_INDEX:-}" ]]; then
        if [[ "${MULLUSI_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((MULLUSI_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${MULLUSI_NO_ONBOARD:-0}
NO_PROMPT=${MULLUSI_NO_PROMPT:-0}
DRY_RUN=${MULLUSI_DRY_RUN:-0}
INSTALL_METHOD=${MULLUSI_INSTALL_METHOD:-}
MULLUSI_VERSION=${MULLUSI_VERSION:-latest}
USE_BETA=${MULLUSI_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/mullusi"
GIT_DIR=${MULLUSI_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${MULLUSI_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${MULLUSI_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${MULLUSI_VERBOSE:-0}"
VERIFY_INSTALL="${MULLUSI_VERIFY_INSTALL:-0}"
MULLUSI_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Mullusi installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://mullusi.com/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/mullusi)
...
Environment variables:
  MULLUSI_INSTALL_METHOD=git|npm
  MULLUSI_VERSION=latest|next|main|<semver>|<spec>
  MULLUSI_BETA=0|1
  MULLUSI_GIT_DIR=...
  MULLUSI_GIT_UPDATE=0|1
  MULLUSI_NO_PROMPT=1
  MULLUSI_VERIFY_INSTALL=1
  MULLUSI_DRY_RUN=1
  MULLUSI_NO_ONBOARD=1
  MULLUSI_VERBOSE=1
  MULLUSI_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://mullusi.com/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://mullusi.com/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://mullusi.com/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Mullusi checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Mullusi source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_mullusi_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"mullusi"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://mullusi.com/install.sh | bash"
}
...
ensure_mullusi_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/mullusi" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/mullusi" ]]; then
        ln -sf "$npm_root/mullusi/dist/entry.js" "${npm_bin}/mullusi"
        ui_info "Created mullusi bin link at ${npm_bin}/mullusi"
    fi
    return 0
}

# Check for existing Mullusi installation
check_existing_mullusi() {
    if [[ -n "$(type -P mullusi 2>/dev/null || true)" ]]; then
        ui_info "Existing Mullusi installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make mullusi show as \"command not found\" in new terminals."
...
warn_mullusi_not_found() {
    ui_warn "Installed, but mullusi is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t mullusi 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named mullusi; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_mullusi_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P mullusi 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/mullusi" ]]; then
        echo "${npm_bin}/mullusi"
        return 0
    fi
...
install_mullusi_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/mullusi/mullusi.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Mullusi from git checkout: ${repo_dir}"
    else
        ui_info "Installing Mullusi from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Mullusi" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Mullusi" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/mullusi" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/mullusi"
    ui_success "Mullusi wrapper installed to \$HOME/.local/bin/mullusi"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view mullusi dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:mullusi/mullusi#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_mullusi() {
    local package_name="mullusi"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            MULLUSI_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="mullusi"
        else
            MULLUSI_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${MULLUSI_VERSION}" ]]; then
        MULLUSI_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${MULLUSI_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${MULLUSI_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Mullusi v${resolved_version}"
    else
        ui_info "Installing Mullusi (${MULLUSI_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${MULLUSI_VERSION}")"

    if ! install_mullusi_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_mullusi_paths
        install_mullusi_npm "${install_spec}"
    fi

    if [[ "${MULLUSI_VERSION}" == "latest" && "${package_name}" == "mullusi" ]]; then
        if ! resolve_mullusi_bin &> /dev/null; then
            ui_warn "npm install mullusi@latest failed; retrying mullusi@next"
            cleanup_npm_mullusi_paths
            install_mullusi_npm "mullusi@next"
        fi
    fi

    ensure_mullusi_bin_link || true

    ui_success "Mullusi installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${MULLUSI_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_mullusi_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (mullusi not on PATH yet)"
        warn_mullusi_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${MULLUSI_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.mullusi/workspace-${profile}"
    else
        echo "${HOME}/.mullusi/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${MULLUSI_CONFIG_PATH:-$HOME/.mullusi/mullusi.json}"
    if [[ -f "${config_path}" || -f "$HOME/.mullusi/mullusi.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run mullusi onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but mullusi not on PATH; skipping onboarding"
        warn_mullusi_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run mullusi onboard to retry"
        return
    }
}
...
if ! declare -F extract_mullusi_semver >/dev/null 2>&1; then
...
resolve_mullusi_version() {
    ...
    local claw="${MULLUSI_BIN:-}"
    if [[ -z "$claw" ]] && command -v mullusi &> /dev/null; then
        claw="$(command -v mullusi)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_mullusi_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/mullusi/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/mullusi/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: mullusi not on PATH yet"
        warn_mullusi_not_found
        return 1
    fi

    run_quiet_step "Checking Mullusi version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: mullusi gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_mullusi_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Mullusi checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set MULLUSI_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_mullusi; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Mullusi"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g mullusi &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g mullusi 2>/dev/null || true
        ...
        install_mullusi_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/mullusi" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/mullusi"
    ...
    MULLUSI_BIN="$(resolve_mullusi_bin || true)"
    ...
    installed_version=$(resolve_mullusi_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Mullusi installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Mullusi installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            "Leveled up! New skills unlocked. You're welcome."
            "Fresh code, same mullusi. Miss me?"
            ...
            "The mullusi has molted. Harder shell, sharper claws."
        ...
        local completion_messages=(
            ...
            "The mullusi has landed. Your terminal will never be the same."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/mullusi"
        ui_kv "Update command" "mullusi update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://mullusi.com/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running mullusi doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                MULLUSI_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                MULLUSI_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                MULLUSI_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run mullusi doctor and mullusi plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run mullusi onboard later"
        else
            local config_path="${MULLUSI_CONFIG_PATH:-$HOME/.mullusi/mullusi.json}"
            if [[ -f "${config_path}" || -f "$HOME/.mullusi/mullusi.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (mullusi not on PATH yet)"
                    warn_mullusi_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run mullusi onboard to finish setup"
            return 0
        fi
    fi

    if command -v mullusi &> /dev/null; then
        local claw="${MULLUSI_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_mullusi_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (mullusi daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if MULLUSI_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: mullusi daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${MULLUSI_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
