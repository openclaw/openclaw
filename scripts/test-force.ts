...
function runTests() {
  const isolatedLock =
    process.env.SMART_AGENT_NEO_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `smart-agent-neo-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      SMART_AGENT_NEO_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.SMART_AGENT_NEO_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
