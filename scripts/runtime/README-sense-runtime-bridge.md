...
For log aggregation and dashboard use, `sense-runtime-log-aggregate.sh` provides a thin read-only view over runtime entry observability records. It groups by `route_signature`, reports per-route counts, latest occurrence timestamp, per-owner counts, actionable counts, max recovery rank, an owner x bucket cross-tab summary, and an owner x bucket x actionable summary over the filtered record set. `top_n` trims only the per-route detail list; the cross-tab and actionable summary stay based on the full filtered set. This helper is observational only and does not execute runtime work.
