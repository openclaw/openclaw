    ...
    while attempts < max(1, args.max_attempts):
        ...
        if next_step == 'sense_runtime_start':
            runtime_result = run_json([
                str(manager_tool), '--intent', 'sense runtime start', '--token', args.token,
                *( ['--timeout', str(args.timeout)] if args.timeout is not None else [] ),
                *( ['--wait-timeout', str(args.wait_timeout)] if args.wait_timeout is not None else [] ),
                *( ['--poll-interval', str(args.poll_interval)] if args.poll_interval is not None else [] ),
            ...
            runtime_result = run_json([
                str(manager_tool), '--intent', 'sense runtime status', '--token', args.token,
                *( ['--timeout', str(args.timeout)] if args.timeout is not None else [] ),
                *( ['--wait-timeout', str(args.wait_timeout)] if args.wait_timeout is not None else [] ),
                *( ['--poll-interval', str(args.poll_interval)] if args.poll_interval is not None else [] ),
                *( ['--input', args.input] if args.input else [] ),
            ...
            runtime_result = run_json([
                str(manager_tool), '--intent', 'sense sandbox status', '--token', args.token,
