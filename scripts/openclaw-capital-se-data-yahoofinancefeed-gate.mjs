/**
 * openclaw-capital-se-data-yahoofinancefeed-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/data/YahooFinanceFeed.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-data-yahoofinancefeed.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "data", "YahooFinanceFeed.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-data-yahoofinancefeed-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeDataYahoofinancefeedGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeDataYahoofinancefeedGate({
