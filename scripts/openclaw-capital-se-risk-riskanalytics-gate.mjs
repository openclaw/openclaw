/**
 * openclaw-capital-se-risk-riskanalytics-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/risk/RiskAnalytics.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-risk-riskanalytics.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "risk", "RiskAnalytics.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-risk-riskanalytics-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeRiskRiskanalyticsGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeRiskRiskanalyticsGate({
