/**
 * openclaw-capital-se-strategy-config-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategy-config.json
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategy-config.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategy-config.json"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategy-config-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategyConfigGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategyConfigGate({
