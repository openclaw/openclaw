...
# Build and bundle Activi into a minimal .app we can open.
# Outputs to dist/Activi.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Activi.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Activi"
BUNDLE_ID="${BUNDLE_ID:-ai.activi.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/activi/activi/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Activi/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ActiviBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ActiviGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Activi"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Activi"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Activi"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Activi" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Activi/Resources/Activi.icns" "$APP_ROOT/Contents/Resources/Activi.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Activi/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ActiviKit resources"
ACTIVIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ActiviKit_ActiviKit.bundle"
if [ -d "$ACTIVIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ActiviKit_ActiviKit.bundle"
  cp -R "$ACTIVIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ActiviKit_ActiviKit.bundle"
else
  echo "WARN: ActiviKit resource bundle not found at $ACTIVIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Activi"
killall -q Activi 2>/dev/null || true
