...
# Build and bundle Hanzo Bot into a minimal .app we can open.
# Outputs to dist/Hanzo Bot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Hanzo Bot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Hanzo Bot"
BUNDLE_ID="${BUNDLE_ID:-ai.bot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/hanzoai/bot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Hanzo Bot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :Hanzo BotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :Hanzo BotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Hanzo Bot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Hanzo Bot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Hanzo Bot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Hanzo Bot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Hanzo Bot/Resources/Hanzo Bot.icns" "$APP_ROOT/Contents/Resources/Hanzo Bot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Hanzo Bot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying Hanzo BotKit resources"
HANZO BOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/Hanzo BotKit_Hanzo BotKit.bundle"
if [ -d "$HANZO BOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/Hanzo BotKit_Hanzo BotKit.bundle"
  cp -R "$HANZO BOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/Hanzo BotKit_Hanzo BotKit.bundle"
else
  echo "WARN: Hanzo BotKit resource bundle not found at $HANZO BOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Hanzo Bot"
killall -q Hanzo Bot 2>/dev/null || true
