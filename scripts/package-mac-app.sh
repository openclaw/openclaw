...
# Build and bundle SmartAgentNeo into a minimal .app we can open.
# Outputs to dist/SmartAgentNeo.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SmartAgentNeo.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SmartAgentNeo"
BUNDLE_ID="${BUNDLE_ID:-ai.smartagentneo.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/betterbrand/smart-agent-neo/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SmartAgentNeo/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SmartAgentNeoBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SmartAgentNeoGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SmartAgentNeo"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SmartAgentNeo"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SmartAgentNeo"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SmartAgentNeo" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SmartAgentNeo/Resources/SmartAgentNeo.icns" "$APP_ROOT/Contents/Resources/SmartAgentNeo.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SmartAgentNeo/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SmartAgentNeoKit resources"
SMARTAGENTNEOKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SmartAgentNeoKit_SmartAgentNeoKit.bundle"
if [ -d "$SMARTAGENTNEOKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SmartAgentNeoKit_SmartAgentNeoKit.bundle"
  cp -R "$SMARTAGENTNEOKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SmartAgentNeoKit_SmartAgentNeoKit.bundle"
else
  echo "WARN: SmartAgentNeoKit resource bundle not found at $SMARTAGENTNEOKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SmartAgentNeo"
killall -q SmartAgentNeo 2>/dev/null || true
