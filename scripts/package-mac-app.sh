...
# Build and bundle EasyHub into a minimal .app we can open.
# Outputs to dist/EasyHub.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/EasyHub.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="EasyHub"
BUNDLE_ID="${BUNDLE_ID:-ai.EasyHub.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/EasyHub/EasyHub/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/EasyHub/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :EasyHubBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :EasyHubGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/EasyHub"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/EasyHub"
fi
chmod +x "$APP_ROOT/Contents/MacOS/EasyHub"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/EasyHub" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/EasyHub/Resources/EasyHub.icns" "$APP_ROOT/Contents/Resources/EasyHub.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/EasyHub/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying EasyHubKit resources"
EasyHubKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/EasyHubKit_EasyHubKit.bundle"
if [ -d "$EasyHubKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/EasyHubKit_EasyHubKit.bundle"
  cp -R "$EasyHubKIT_BUNDLE" "$APP_ROOT/Contents/Resources/EasyHubKit_EasyHubKit.bundle"
else
  echo "WARN: EasyHubKit resource bundle not found at $EasyHubKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running EasyHub"
killall -q EasyHub 2>/dev/null || true
