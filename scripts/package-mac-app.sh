...
# Build and bundle Mullusi into a minimal .app we can open.
# Outputs to dist/Mullusi.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Mullusi.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Mullusi"
BUNDLE_ID="${BUNDLE_ID:-ai.mullusi.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/mullusi/mullusi/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Mullusi/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MullusiBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MullusiGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Mullusi"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Mullusi"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Mullusi"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Mullusi" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Mullusi/Resources/Mullusi.icns" "$APP_ROOT/Contents/Resources/Mullusi.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Mullusi/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MullusiKit resources"
MULLUSIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MullusiKit_MullusiKit.bundle"
if [ -d "$MULLUSIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MullusiKit_MullusiKit.bundle"
  cp -R "$MULLUSIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MullusiKit_MullusiKit.bundle"
else
  echo "WARN: MullusiKit resource bundle not found at $MULLUSIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Mullusi"
killall -q Mullusi 2>/dev/null || true
