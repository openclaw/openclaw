...
#
# Suggested cron: */30 * * * * /home/admin/smart-agent-neo/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SmartAgentNeo notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., smart-agent-neo-alerts)
...
STATE_FILE="$HOME/.smart-agent-neo/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SmartAgentNeo if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use smart-agent-neo
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SmartAgentNeo to $NOTIFY_PHONE..."
            smart-agent-neo send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: SmartAgentNeo Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SmartAgentNeo is down. Run: ssh l36 '~/smart-agent-neo/scripts/mobile-reauth.sh'" "urgent"
    exit 1
