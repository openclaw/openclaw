...
#
# Suggested cron: */30 * * * * /home/admin/dna/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send DNA notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., dna-alerts)
...
STATE_FILE="$HOME/.dna/auth-monitor-state"
...
send_notification() {
    ...
    # Send via DNA if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use dna
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via DNA to $NOTIFY_PHONE..."
            dna send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! DNA is down. Run: ssh l36 '~/dna/scripts/mobile-reauth.sh'" "urgent"
    exit 1
