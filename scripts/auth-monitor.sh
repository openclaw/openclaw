...
#
# Suggested cron: */30 * * * * /home/admin/activi/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Activi notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., activi-alerts)
...
STATE_FILE="$HOME/.activi/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Activi if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use activi
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Activi to $NOTIFY_PHONE..."
            activi send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Activi is down. Run: ssh l36 '~/activi/scripts/mobile-reauth.sh'" "urgent"
    exit 1
