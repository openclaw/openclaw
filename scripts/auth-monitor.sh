...
#
# Suggested cron: */30 * * * * /home/admin/mullusi/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Mullusi notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., mullusi-alerts)
...
STATE_FILE="$HOME/.mullusi/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Mullusi if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use mullusi
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Mullusi to $NOTIFY_PHONE..."
            mullusi send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Mullusi is down. Run: ssh l36 '~/mullusi/scripts/mobile-reauth.sh'" "urgent"
    exit 1
