...
#
# Suggested cron: */30 * * * * /home/admin/EasyHub/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send EasyHub notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., EasyHub-alerts)
...
STATE_FILE="$HOME/.easyhub/auth-monitor-state"
...
send_notification() {
    ...
    # Send via EasyHub if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use EasyHub
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via EasyHub to $NOTIFY_PHONE..."
            EasyHub send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! EasyHub is down. Run: ssh l36 '~/EasyHub/scripts/mobile-reauth.sh'" "urgent"
    exit 1
