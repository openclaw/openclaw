...
import { defaultFsObservationUsePolling } from "../src/infra/fs-observation-mode.ts";
...
export function createSourceObserver(paths: string[], options: WatchOptions) {
  ...
  const polling = options.env.CHOKIDAR_USEPOLLING?.toLowerCase();
  const mode =
    polling === undefined
      ? defaultFsObservationUsePolling()
        ? "poll"
        : "node"
      : polling && polling !== "false" && polling !== "0"
        ? "poll"
        : "node";
  const pollingInterval = Number.parseInt(options.env.CHOKIDAR_INTERVAL ?? "", 10);
  const intervalMs =
    mode === "poll"
      ? Number.isInteger(pollingInterval) && pollingInterval > 0 && pollingInterval <= 2_147_483_647
        ? Math.max(20, pollingInterval)
        : 100
      : undefined;
  ...
  const install = async (groups: SourceTargetGroup[]) => {
    ...
    // Retire removed workers before admitting replacements. Never overlap two
    // subscriptions for one Root or grow beyond the discovery Root ceiling.
      ...
      if (existing) {
        if (existing.signature !== signature) {
          ...
          if (existing.subscription) {
            admissions.push(existing.subscription.update(group.scopes));
          }
        }
        continue;
      }
      const entry: Physical = { group, signature };
      ...
      entry.subscription = watch(group.authority, {
        ...
        onHealth: (health) => {
          if (!terminal && health.state === "unavailable") {
            fail(health.error ?? new Error("Source observation unavailable"));
          }
        },
      });
      admissions.push(entry.subscription.ready);
      if (terminal) {
        void retire(entry);
      }
    }
