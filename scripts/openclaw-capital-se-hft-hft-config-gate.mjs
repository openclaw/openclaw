/**
 * openclaw-capital-se-hft-hft-config-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/hft/hft-config.json
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-hft-hft-config.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "hft", "hft-config.json"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-hft-hft-config-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeHftHftConfigGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeHftHftConfigGate({
