...
const tempDir = await fs.mkdtemp(
  path.join(os.tmpdir(), "openclaw-capital-operator-heartbeat-runner-"),
);
...
const fixtureReportPath = path.join(tempDir, "operator-heartbeat-runner-report.fixture.json");
...
const disabled = await runHeartbeat({
...
assert.equal(disabled.report.schema, "openclaw.capital.live-trading-operator-heartbeat-runner.v1");
assert.equal(disabled.report.status, "guard_live_disabled_idle");
assert.equal(disabled.report.action, "status");
assert.equal(disabled.report.applied, false);
assert.equal(disabled.report.enabledAfter, false);
assert.equal(disabled.report.sentOrder, false);
assert.equal(disabled.report.intervalSec, 60);
await fs.access(fixtureReportPath);
await fs.access(`${fixtureReportPath}.sha256`);

const expiredDryRun = await runHeartbeat({
  approvalPath: fixtureApprovalPath,
...
const expiredExecute = await runHeartbeat({
  approvalPath: fixtureApprovalPath,
  riskControlsPath: fixtureRiskExpiredPath,
  reportPath: fixtureReportPath,
  intervalSec: 60,
...
process.stdout.write(
  [
    "CAPITAL_LIVE_TRADING_OPERATOR_HEARTBEAT_RUNNER_CHECK=OK",
