/**
 * openclaw-capital-test-p1p2-modules-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/test-p1p2-modules.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.test-p1p2-modules.v1";
const SOURCE_REL_PARTS = ["scripts", "test-p1p2-modules.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-test-p1p2-modules-gate-latest.json",
);
function hasFlag(f) {
...
export async function runTestP1p2ModulesGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runTestP1p2ModulesGate({
