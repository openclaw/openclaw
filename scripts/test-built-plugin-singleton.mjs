...
const warningFilterKey = Symbol.for("mullusi.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadMullusiPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadMullusiPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mullusi-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@mullusi/build-smoke-plugin",
      type: "module",
      mullusi: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "mullusi.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'mullusi/plugin-sdk';",
...
const registry = loadMullusiPlugins({
  ...
  env: {
    ...process.env,
    MULLUSI_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
