/**
 * openclaw-capital-se-news-newsfetcher-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/news/NewsFetcher.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-news-newsfetcher.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "news", "NewsFetcher.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-news-newsfetcher-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeNewsNewsfetcherGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeNewsNewsfetcherGate({
