...
try {
  ...
  if (
    ...
    !state.matchingLookupStarted ||
    !state.matchingLookupCompleted ||
    !state.stalledLookupStarted ||
    !state.stalledLookupSocketClosed ||
    fallbackSendCount !== 0
  ) {
    throw new Error(
      "native progress proof did not preserve delivery and settle the stalled lookup",
    );
  }
  console.log(
