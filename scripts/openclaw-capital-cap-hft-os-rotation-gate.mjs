/**
 * openclaw-capital-cap-hft-os-rotation-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/openclaw-capital-hft-os-rotation.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.cap-hft-os-rotation.v1";
const SOURCE_REL_PARTS = ["scripts", "openclaw-capital-hft-os-rotation.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-cap-hft-os-rotation-gate-latest.json",
);
function hasFlag(f) {
...
export async function runCapHftOsRotationGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runCapHftOsRotationGate({
