...
const EXPECTED_REPOSITORY_URL = "https://github.com/mullusi/mullusi";
...
const skipPackValidationEnv = "MULLUSI_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "mullusi") {
    errors.push(`package.json name must be "mullusi"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.mullusi !== "mullusi.mjs") {
    errors.push(
      `package.json bin.mullusi must be "mullusi.mjs"; found "${pkg.bin?.mullusi ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`mullusi-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `mullusi-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
