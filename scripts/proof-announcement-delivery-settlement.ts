...
let currentGatewayResult = gatewayResultFor("D000000101");
...
async function startServer(user: string) {
  const server = createServer((_request, response) => {
    ...
    response.end(
      JSON.stringify({
        ok: true,
        channel: { id: user === "U000000001" ? "D000000101" : "D000000102", is_im: true, user },
      }),
    );
  });
...
async function main() {
  ...
  try {
    ...
    if (!matching.delivered) {
      throw new Error("matching recipient was not credited as source delivery");
    }
    console.log(
  ...
  currentGatewayResult = gatewayResultFor("D000000102");
  try {
    const wrong = await announce(sourceOrigin, "proof:settlement:wrong-recipient");
    if (wrong.delivered) {
      throw new Error("wrong recipient was incorrectly credited as source delivery");
    }
