...
// A native subscription costs about 17 MiB in the candidate probe. This is a
// developer compatibility lane, not an unbounded worker per symbolic link.
...
export function createSourceTargetDiscovery(
  ...
  const admit = async (canonical: string, signal: AbortSignal) => {
    ...
    if (pinned.size >= SOURCE_OBSERVATION_LIMITS.roots) {
      throw new RangeError(
        "Source observation exceeds the 4 lifetime-pinned Root budget; restart the watcher or reduce linked source roots",
      );
    }
