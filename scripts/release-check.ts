...
function checkBundledExtensionMetadata() {
	...
	if (errors.length > 0) {
		console.error(
			"release-check: bundled extension manifest validation failed:",
		);
...
export function collectBundledExtensionRootDependencyMirrorErrors(
	...
	for (const extension of extensions) {
		...
		const extensionRuntimeDeps = collectRuntimeDependencySpecs(
	...
	return requiredPathGroups
		.flatMap((group) => {
			if (Array.isArray(group)) {
				return group.some((path) => available.has(path))
					? []
					: [group.join(" or ")];
			}
		...
		.filter(
			(path) =>
				...
				(/node_modules\//.test(path) &&
					!isAllowedBundledPluginNodeModulesPath(path)),
		)
...
async function collectDistPluginSdkExports(): Promise<Set<string>> {
	...
	try {
		...
		console.error(
			"release-check: dist/plugin-sdk directory not found (build missing?).",
		);
		process.exit(1);
		return new Set();
	}
...
async function checkPluginSdkExports() {
	...
	const missingExports = requiredPluginSdkExports.filter(
