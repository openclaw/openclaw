/**
 * openclaw-capital-se-backtest-backtester-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/backtest/Backtester.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-backtest-backtester.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "backtest", "Backtester.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-backtest-backtester-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeBacktestBacktesterGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeBacktestBacktesterGate({
