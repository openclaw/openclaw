/**
 * openclaw-capital-sim-trading-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/sim-trading.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.sim-trading.v1";
const SOURCE_REL_PARTS = ["scripts", "sim-trading.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-sim-trading-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSimTradingGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSimTradingGate({
