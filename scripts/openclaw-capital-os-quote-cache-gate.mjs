/**
 * openclaw-capital-os-quote-cache-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/os-quote-cache.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.os-quote-cache.v1";
const SOURCE_REL_PARTS = ["scripts", "os-quote-cache.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-os-quote-cache-gate-latest.json",
);
function hasFlag(f) {
...
export async function runOsQuoteCacheGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runOsQuoteCacheGate({
