/**
 * openclaw-capital-se-strategies-pivotreversalstrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/PivotReversalStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-pivotreversalstrategy.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategies", "PivotReversalStrategy.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-pivotreversalstrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesPivotreversalstrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesPivotreversalstrategyGate({
