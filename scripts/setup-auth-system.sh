#!/bin/bash
# Setup DNA Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'dna-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  DNA message: Send warning via DNA itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/dna-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/dna-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/dna-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now dna-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status dna-auth-monitor.timer"
echo ""
echo "Quick commands:"
