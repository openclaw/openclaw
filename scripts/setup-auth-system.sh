#!/bin/bash
# Setup Activi Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'activi-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Activi message: Send warning via Activi itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/activi-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/activi-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/activi-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now activi-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status activi-auth-monitor.timer"
echo ""
echo "Quick commands:"
