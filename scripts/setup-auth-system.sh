#!/bin/bash
# Setup Mullusi Auth Management System
...
echo "=== Mullusi Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'mullusi-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Mullusi message: Send warning via Mullusi itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/mullusi-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/mullusi-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/mullusi-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now mullusi-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status mullusi-auth-monitor.timer"
echo ""
echo "Quick commands:"
