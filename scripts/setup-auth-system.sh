#!/bin/bash
# Setup SmartAgentNeo Auth Management System
...
echo "=== SmartAgentNeo Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'smart-agent-neo-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SmartAgentNeo message: Send warning via SmartAgentNeo itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/smart-agent-neo-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/smart-agent-neo-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/smart-agent-neo-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now smart-agent-neo-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/NeoAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/NeoAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Neo*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (NeoAuth) shows status and opens auth URL if needed."
echo "The full widget (NeoAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status smart-agent-neo-auth-monitor.timer"
echo ""
echo "Quick commands:"
