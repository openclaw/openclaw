#!/bin/bash
# Setup EasyHub Auth Management System
...
echo "=== EasyHub Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'EasyHub-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  EasyHub message: Send warning via EasyHub itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/EasyHub-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/EasyHub-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/EasyHub-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now EasyHub-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status EasyHub-auth-monitor.timer"
echo ""
echo "Quick commands:"
