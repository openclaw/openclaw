/**
 * openclaw-capital-se-hft-riskguard-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/hft/RiskGuard.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-hft-riskguard.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "hft", "RiskGuard.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-hft-riskguard-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeHftRiskguardGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeHftRiskguardGate({
