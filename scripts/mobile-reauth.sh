...
case "$AUTH_STATUS" in
    OK)
        ...
        exit 0
        ;;
    CLAUDE_EXPIRING|SMART_AGENT_NEO_EXPIRING|NEOBOT_EXPIRING)
...
if claude setup-token; then
    ...
    # Restart smart-agent-neo service if running
    if systemctl --user is-active smart-agent-neo >/dev/null 2>&1; then
        echo ""
        echo "Restarting smart-agent-neo service..."
        systemctl --user restart smart-agent-neo
