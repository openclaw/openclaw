...
elif [[ -n "${MULLUSI_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(mullusi_live_collect_auth_dirs_from_csv "${MULLUSI_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(mullusi_live_collect_auth_files_from_csv "${MULLUSI_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e MULLUSI_LIVE_GATEWAY_MODELS="${MULLUSI_LIVE_GATEWAY_MODELS:-modern}" \
  -e MULLUSI_LIVE_GATEWAY_PROVIDERS="${MULLUSI_LIVE_GATEWAY_PROVIDERS:-}" \
  -e MULLUSI_LIVE_GATEWAY_SMOKE="${MULLUSI_LIVE_GATEWAY_SMOKE:-1}" \
  -e MULLUSI_LIVE_GATEWAY_MAX_MODELS="${MULLUSI_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e MULLUSI_LIVE_GATEWAY_STEP_TIMEOUT_MS="${MULLUSI_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e MULLUSI_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${MULLUSI_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
  -v "$ROOT_DIR":/src:ro \
