/**
 * openclaw-capital-se-notify-notify-config-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/notify/notify-config.json
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-notify-notify-config.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "notify", "notify-config.json"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-notify-notify-config-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeNotifyNotifyConfigGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeNotifyNotifyConfigGate({
