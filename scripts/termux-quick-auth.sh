...
SERVER="${DNA_SERVER:-l36}"

STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/dna/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "DNA Re-Auth" -c "After getting key, run: ssh $SERVER '~/dna/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
