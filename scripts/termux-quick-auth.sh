...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/mullusi/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Mullusi Re-Auth" -c "After getting key, run: ssh $SERVER '~/mullusi/scripts/mobile-reauth.sh'" --id mullusi-auth
        ;;
    *)
