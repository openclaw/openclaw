...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/activi/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Activi Re-Auth" -c "After getting key, run: ssh $SERVER '~/activi/scripts/mobile-reauth.sh'" --id activi-auth
        ;;
    *)
