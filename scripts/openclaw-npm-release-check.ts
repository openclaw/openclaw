...
export function resolveNpmPublishPlan(
	...
	return resolveNpmPublishPlanBase(
		version,
		currentBetaVersion,
	) as NpmPublishPlan;
}

export function parseReleaseTagVersion(
	version: string,
): ParsedReleaseTag | null {
...
export function utcCalendarDayDistance(left: Date, right: Date): number {
	return Math.round(
		Math.abs(startOfUtcDay(left) - startOfUtcDay(right)) / 86_400_000,
	);
}
...
export function collectReleasePackageMetadataErrors(
	...
	if (pkg.name !== "openclaw") {
		errors.push(
			`package.json name must be "openclaw"; found "${pkg.name ?? ""}".`,
		);
	}
	...
	if (pkg.license !== "MIT") {
		errors.push(
			`package.json license must be "MIT"; found "${pkg.license ?? ""}".`,
		);
	}
...
export function collectReleaseTagErrors(params: {
	...
	if (!releaseTag.startsWith("v")) {
		errors.push(
			`Release tag must start with "v"; found "${releaseTag || "<missing>"}".`,
		);
	}
...
export function resolveNpmCommandInvocation(
	...
	const npmCommand =
		(params.platform ?? process.platform) === "win32" ? "npm.cmd" : "npm";

	if (
		typeof npmExecPath === "string" &&
		npmExecPath.length > 0 &&
		isNpmExecPath(npmExecPath)
	) {
...
export function collectControlUiPackErrors(paths: Iterable<string>): string[] {
	...
	const assetPaths = [...packedPaths].filter((path) =>
		path.startsWith(CONTROL_UI_ASSET_PREFIX),
	);
	const errors: string[] = [];
