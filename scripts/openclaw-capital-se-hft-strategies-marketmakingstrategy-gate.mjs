/**
 * openclaw-capital-se-hft-strategies-marketmakingstrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/hft/strategies/MarketMakingStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-hft-strategies-marketmakingstrategy.v1";
const SOURCE_REL_PARTS = [
  "scripts",
  "strategy-engine",
  "hft",
  "strategies",
  "MarketMakingStrategy.mjs",
];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-hft-strategies-marketmakingstrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeHftStrategiesMarketmakingstrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeHftStrategiesMarketmakingstrategyGate({
