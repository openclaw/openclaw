...
// Forward-hygiene guard: prevent new large files from entering the tree.
// Current tracked files under the threshold stay under the threshold without
// this guard; the point of the check is to make future commits that add
// oversize assets fail CI unless the file is explicitly allowlisted below.
...
// Windows-safe script-entry detection. Compare resolved filesystem paths via
// fileURLToPath(import.meta.url) rather than raw `new URL(...).pathname`,
// which keeps the `/C:/...` URL form on win32 and fails path-equality.
