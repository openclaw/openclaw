/**
 * openclaw-capital-se-arbitrage-triangulararbitragestrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/arbitrage/TriangularArbitrageStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-arbitrage-triangulararbitragestrategy.v1";
const SOURCE_REL_PARTS = [
  "scripts",
  "strategy-engine",
  "arbitrage",
  "TriangularArbitrageStrategy.mjs",
];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-arbitrage-triangulararbitragestrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeArbitrageTriangulararbitragestrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeArbitrageTriangulararbitragestrategyGate({
