#!/usr/bin/env bash
# Reset SmartAgentNeo like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${SMART_AGENT_NEO_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="SmartAgentNeo.app/Contents/MacOS/SmartAgentNeo"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/SmartAgentNeo"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/SmartAgentNeo"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/SmartAgentNeo"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.smartagentneo.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/smart-agent-neo-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${SMART_AGENT_NEO_RESTART_LOG:-/tmp/smart-agent-neo-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${SMART_AGENT_NEO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.smart-agent-neo/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  SMART_AGENT_NEO_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node smart-agent-neo.mjs daemon install --force --runtime node"
      log "  node smart-agent-neo.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.smart-agent-neo/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.smartagentneo.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing SmartAgentNeo instances"
kill_all_smart-agent-neo
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product SmartAgentNeo"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/SmartAgentNeo.app" ]]; then
    APP_BUNDLE="/Applications/SmartAgentNeo.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/SmartAgentNeo.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/SmartAgentNeo.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/SmartAgentNeo.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set SMART_AGENT_NEO_APP_BUNDLE to your installed SmartAgentNeo.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node smart-agent-neo.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node smart-agent-neo.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".smart-agent-neo", "smart-agent-neo.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: SmartAgentNeo is running."
else
