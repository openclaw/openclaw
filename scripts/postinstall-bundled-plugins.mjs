...
const BAILEYS_MEDIA_DISPATCHER_FETCH_AGENT_GUARD_RE =
  /const\s+dispatcher\s*=\s*typeof\s+agent\?\.dispatch\s*===\s*['"]function['"]\s*\?\s*agent\s*:\s*undefined;\s*\n[\s\S]*\.\.\.\(dispatcher\s*\?\s*\{\s*dispatcher\s*\}\s*:\s*\{\s*\}\),/u;
  ...
  try {
    ...
    const dispatcherAlreadyPatched =
      patchedText.includes(
        "...(typeof fetchAgent?.dispatch === 'function' ? { dispatcher: fetchAgent } : {}),",
      ) || BAILEYS_MEDIA_DISPATCHER_FETCH_AGENT_GUARD_RE.test(patchedText);
