/**
 * openclaw-capital-se-risk-riskcontroller-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/risk/RiskController.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-risk-riskcontroller.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "risk", "RiskController.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-risk-riskcontroller-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeRiskRiskcontrollerGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeRiskRiskcontrollerGate({
