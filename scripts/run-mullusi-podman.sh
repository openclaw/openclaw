#!/usr/bin/env bash
# Rootless Mullusi in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-mullusi-podman.sh launch        # Start gateway
#   ./scripts/run-mullusi-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   mullusi --container mullusi dashboard --no-open
#   mullusi --container mullusi channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      MULLUSI_GATEWAY_TOKEN|MULLUSI_PODMAN_CONTAINER|MULLUSI_PODMAN_IMAGE|MULLUSI_IMAGE|MULLUSI_PODMAN_PULL|MULLUSI_PODMAN_GATEWAY_HOST_PORT|MULLUSI_GATEWAY_PORT|MULLUSI_PODMAN_BRIDGE_HOST_PORT|MULLUSI_BRIDGE_PORT|MULLUSI_GATEWAY_BIND|MULLUSI_PODMAN_USERNS|MULLUSI_BIND_MOUNT_OPTIONS|MULLUSI_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-mullusi-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${MULLUSI_CONFIG_DIR:-$EFFECTIVE_HOME/.mullusi}"
ENV_FILE="${MULLUSI_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${MULLUSI_CONFIG_DIR:-$EFFECTIVE_HOME/.mullusi}"
ENV_FILE="${MULLUSI_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${MULLUSI_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${MULLUSI_PODMAN_CONTAINER:-mullusi}"
MULLUSI_IMAGE="${MULLUSI_PODMAN_IMAGE:-${MULLUSI_IMAGE:-mullusi:local}}"
PODMAN_PULL="${MULLUSI_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${MULLUSI_PODMAN_GATEWAY_HOST_PORT:-${MULLUSI_GATEWAY_PORT:-18790}}"
HOST_BRIDGE_PORT="${MULLUSI_PODMAN_BRIDGE_HOST_PORT:-${MULLUSI_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${MULLUSI_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$MULLUSI_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v mullusi >/dev/null 2>&1; then
    return 0
  fi
  MULLUSI_CONTAINER="" MULLUSI_CONFIG_DIR="$config_dir" \
    mullusi config get gateway.bind 2>/dev/null || true
}
...
# MULLUSI_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${MULLUSI_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'MULLUSI_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v mullusi >/dev/null 2>&1; then
    echo "Warning: mullusi not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    MULLUSI_CONTAINER="" MULLUSI_CONFIG_DIR="$config_dir" \
      mullusi config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    MULLUSI_CONTAINER="" MULLUSI_CONFIG_DIR="$config_dir" \
      mullusi config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  MULLUSI_CONTAINER="" MULLUSI_CONFIG_DIR="$config_dir" \
    mullusi config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${MULLUSI_GATEWAY_TOKEN:-}" ]]; then
  export MULLUSI_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "MULLUSI_GATEWAY_TOKEN" "$MULLUSI_GATEWAY_TOKEN"
  echo "Generated MULLUSI_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/mullusi.json"
...
PODMAN_USERNS="${MULLUSI_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported MULLUSI_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (MULLUSI_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${MULLUSI_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${MULLUSI_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$MULLUSI_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.mullusi/.npm \
    -e MULLUSI_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.mullusi:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.mullusi/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$MULLUSI_IMAGE" \
...
podman run --pull="$PODMAN_PULL" -d --replace \
  ...
  -v "$CONFIG_DIR:/home/node/.mullusi:rw${SELINUX_MOUNT_OPTS}" \
  -v "$WORKSPACE_DIR:/home/node/.mullusi/workspace:rw${SELINUX_MOUNT_OPTS}" \
  -p "${PUBLISH_HOST}:${HOST_GATEWAY_PORT}:18790" \
  -p "${PUBLISH_HOST}:${HOST_BRIDGE_PORT}:18790" \
  "$MULLUSI_IMAGE" \
  node dist/index.js gateway --bind "$GATEWAY_BIND" --port 18790 >/dev/null

echo "Container $CONTAINER_NAME started: http://127.0.0.1:${HOST_GATEWAY_PORT}/"
echo "podman exec -it $CONTAINER_NAME mullusi dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME mullusi devices approve --latest  # if pairing required"
