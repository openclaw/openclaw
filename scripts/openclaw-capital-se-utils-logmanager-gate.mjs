/**
 * openclaw-capital-se-utils-logmanager-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/utils/LogManager.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-utils-logmanager.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "utils", "LogManager.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-utils-logmanager-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeUtilsLogmanagerGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeUtilsLogmanagerGate({
