...
export function selectDispatchedWorkflowRunId(runs, expected) {
  const expectedPath = `.github/workflows/${expected.workflowFile}`;
  const matches = runs.filter(
    (workflowRun) =>
      workflowRun.display_title === expected.correlationId &&
      workflowRun.event === "workflow_dispatch" &&
      workflowRun.head_branch === expected.workflowRef &&
      workflowRun.head_sha === expected.headSha &&
      String(workflowRun.path ?? "").split("@", 1)[0] === expectedPath &&
      /^[1-9][0-9]*$/u.test(String(workflowRun.id ?? "")),
  );
  if (matches.length > 1) {
    throw new Error(
      `workflow dispatch correlation ${expected.correlationId} matched multiple runs`,
    );
  }
...
async function dispatchWorkflow(repo, workflowFile, workflowRef, headSha, fields) {
  ...
  const output = runAndEcho("gh", args);
  const directRunId = parseRunIdFromDispatchOutput(output);
  if (directRunId) {
    return directRunId;
  }
  ...
  for (let attempt = 1; attempt <= 20; attempt += 1) {
    const response = await githubApi(
      `repos/${repo}/actions/workflows/${encodedWorkflow}/runs?${query.toString()}`,
    );
    const runId = selectDispatchedWorkflowRunId(response.workflow_runs ?? [], {
      correlationId,
      headSha,
      workflowFile,
      workflowRef,
  ...
  throw new Error(
    `workflow ${workflowFile} was dispatched with correlation ${correlationId}, but its run ID did not become visible; recover that exact run before retrying`,
  );
}
...
export function buildPublishCommand(options) {
  for (const [label, value] of [
    ["npm preflight run ID", options.npmPreflightRunId],
    ["npm preflight run attempt", options.npmPreflightRunAttempt],
    ["Full Release Validation run ID", options.fullReleaseRunId],
    ["Full Release Validation run attempt", options.fullReleaseRunAttempt],
  ]) {
    if (!/^[1-9][0-9]*$/u.test(String(value ?? ""))) {
      throw new Error(`${label} must be a positive integer before building the publish command`);
    }
  }
  const alphaRelease = options.tag.includes("-alpha.");
  const workflowRef = alphaRelease
    ? options.workflowRef
    : `release-publish/${options.trustedToolingSha}`;
  if (alphaRelease && !TIDECLAW_ALPHA_WORKFLOW_REF_PATTERN.test(workflowRef)) {
    throw new Error(
  ...
  if (!alphaRelease && !/^[a-f0-9]{40}$/u.test(options.trustedToolingSha ?? "")) {
    throw new Error("regular release publish requires an exact trusted tooling SHA");
  }
  ...
  const pushCommand = [
    ...
    "push",
    "origin",
    `${options.trustedToolingSha}:refs/heads/${workflowRef}`,
  ]
    .map(shellQuote)
    .join(" ");
  return `${pushCommand}\n${workflowCommand}`;
...
async function runTelegramIfNeeded(options, artifactName, trustedToolingSha) {
...
export function validateTrustedWorkflowRunHead(workflowRun, params) {
  const shaPinnedRun =
    params.allowShaPinnedWorkflowRef && isShaPinnedReleaseValidationBranch(workflowRun.headBranch);
  const expectedSha = shaPinnedRun ? params.targetSha : params.trustedToolingSha;
  if (workflowRun.headSha !== expectedSha) {
    throw new Error(
      `${params.label} workflow SHA mismatch: expected=${expectedSha} run=${workflowRun.headSha}`,
    );
  }
}
...
async function main() {
  ...
  if (!options.fullReleaseRunId && !options.skipDispatch) {
    ...
    options.fullReleaseRunId = await dispatchWorkflow(
      options.repo,
      workflowFile,
      options.workflowRef,
      trustedToolingSha,
      {
        ...
        run_release_soak:
          options.releaseProfile === "stable" || options.releaseProfile === "full"
            ? "true"
            : "false",
        rerun_group: "all",
      },
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    ...
    options.npmPreflightRunId = await dispatchWorkflow(
      options.repo,
      workflowFile,
      options.workflowRef,
      trustedToolingSha,
  ...
  validateTrustedWorkflowRunHead(npmRun, {
    label: "npm preflight",
    trustedToolingSha,
  });
  validateTrustedWorkflowRunHead(fullRun, {
    allowShaPinnedWorkflowRef: true,
  ...
  const npmArtifactName = `openclaw-npm-preflight-${options.npmPreflightRunId}-${npmRun.runAttempt}`;
  downloadArtifact(options.repo, options.npmPreflightRunId, npmArtifactName, npmDir);
  ...
  validateFullReleaseValidationEvidence({
    ...
    expectedWorkflowSha: trustedToolingSha,
  ...
  const npmTelegram = await runTelegramIfNeeded(options, npmArtifactName, trustedToolingSha);
