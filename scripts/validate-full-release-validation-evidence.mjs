...
export function validateFullReleaseValidationEvidence({
  ...
  if (!pinnedMatch) {
    ...
    if (directBranches.has(run.headBranch)) {
      ...
      if (
        run.headBranch !== "main" &&
        (!SHA_PATTERN.test(expectedWorkflowSha ?? "") || run.headSha !== expectedWorkflowSha)
      ) {
        throw new Error(
          `Direct ${run.headBranch} validation workflow SHA ${run.headSha} does not match trusted ${expectedWorkflowSha ?? "<missing>"}.`,
        );
      }
