...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "mullusi.plugin.json");
    ...
    if (packageJson.mullusi && "extensions" in packageJson.mullusi) {
      packageJson.mullusi = {
        ...packageJson.mullusi,
        extensions: rewritePackageExtensions(packageJson.mullusi.extensions),
        ...(typeof packageJson.mullusi.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.mullusi.setupEntry) }
          : {}),
      };
    }
