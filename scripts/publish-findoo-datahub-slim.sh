...
# Findoo DataHub Slim Plugin Publisher
# Publishes @openfinclaw/findoo-datahub-slim-plugin to npm
#
# Usage:
#   export NPM_TOKEN=npm_xxxxx
#   pnpm plugins:publish:findoo-slim
#
# Required: NPM_TOKEN (Granular Access Token with 2FA bypass enabled)

PLUGIN_NAME="@openfinclaw/findoo-datahub-slim-plugin"

echo "Findoo DataHub Slim Plugin Publisher"
echo "====================================="
echo ""
...
if [ -z "${NPM_TOKEN:-}" ]; then
  ...
  echo "  pnpm plugins:publish:findoo-slim"
  exit 1
fi
