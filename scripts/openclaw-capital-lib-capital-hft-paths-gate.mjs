/**
 * openclaw-capital-lib-capital-hft-paths-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/lib/capital-hft-paths.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.lib-capital-hft-paths.v1";
const SOURCE_REL_PARTS = ["scripts", "lib", "capital-hft-paths.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-lib-capital-hft-paths-gate-latest.json",
);
function hasFlag(f) {
...
export async function runLibCapitalHftPathsGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runLibCapitalHftPathsGate({
