/**
 * openclaw-capital-se-strategies-threebarreversalstrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/ThreeBarReversalStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-threebarreversalstrategy.v1";
const SOURCE_REL_PARTS = [
  "scripts",
  "strategy-engine",
  "strategies",
  "ThreeBarReversalStrategy.mjs",
];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-threebarreversalstrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesThreebarreversalstrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesThreebarreversalstrategyGate({
