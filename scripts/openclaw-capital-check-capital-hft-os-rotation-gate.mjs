/**
 * openclaw-capital-check-capital-hft-os-rotation-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/check-capital-hft-os-rotation.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.check-capital-hft-os-rotation.v1";
const SOURCE_REL_PARTS = ["scripts", "check-capital-hft-os-rotation.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-check-capital-hft-os-rotation-gate-latest.json",
);
function hasFlag(f) {
...
export async function runCheckCapitalHftOsRotationGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runCheckCapitalHftOsRotationGate({
