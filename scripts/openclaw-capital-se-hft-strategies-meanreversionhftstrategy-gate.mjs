/**
 * openclaw-capital-se-hft-strategies-meanreversionhftstrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/hft/strategies/MeanReversionHftStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-hft-strategies-meanreversionhftstrategy.v1";
const SOURCE_REL_PARTS = [
  "scripts",
  "strategy-engine",
  "hft",
  "strategies",
  "MeanReversionHftStrategy.mjs",
];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-hft-strategies-meanreversionhftstrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeHftStrategiesMeanreversionhftstrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeHftStrategiesMeanreversionhftstrategyGate({
