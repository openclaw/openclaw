...
BASE_IMAGE="${BASE_IMAGE:-smart-agent-neo-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-smart-agent-neo-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=smart-agent-neo.sandbox=1)
NOTE
