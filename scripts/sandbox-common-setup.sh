...
BASE_IMAGE="${BASE_IMAGE:-activi-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-activi-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=activi.sandbox=1)
NOTE
