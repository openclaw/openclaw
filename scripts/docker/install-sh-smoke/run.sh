...
SMOKE_PREVIOUS_VERSION="${MULLUSI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MULLUSI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="mullusi"
PACKAGE_NAME="${MULLUSI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MULLUSI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MULLUSI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MULLUSI_INSTALL_LATEST_OUT:-}"
fi
