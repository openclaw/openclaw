...
SMOKE_PREVIOUS_VERSION="${DNA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${DNA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="dna"
if [[ -z "${DNA_INSTALL_PACKAGE:-}" && "$INSTALL_URL" == *"clawd.bot"* ]]; then
  DEFAULT_PACKAGE="dna"
fi
PACKAGE_NAME="${DNA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
if [[ "$PACKAGE_NAME" == "dna" ]]; then
  ALT_PACKAGE_NAME="dna"
else
  ALT_PACKAGE_NAME="dna"
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (DNA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${DNA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "$DNA_INSTALL_LATEST_OUT"
fi
