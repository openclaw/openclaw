...
INSTALL_URL="${SMART_AGENT_NEO_INSTALL_URL:-https://smart-agent-neo.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SMART_AGENT_NEO_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SMART_AGENT_NEO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="smart-agent-neo"
PACKAGE_NAME="${SMART_AGENT_NEO_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SMART_AGENT_NEO_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SMART_AGENT_NEO_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SMART_AGENT_NEO_INSTALL_LATEST_OUT:-}"
fi
