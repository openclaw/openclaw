...
INSTALL_URL="${EASYHUB_INSTALL_URL:-https://EasyHub.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${EASYHUB_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${EASYHUB_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="EasyHub"
PACKAGE_NAME="${EASYHUB_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (EASYHUB_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${EASYHUB_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${EASYHUB_INSTALL_LATEST_OUT:-}"
fi
