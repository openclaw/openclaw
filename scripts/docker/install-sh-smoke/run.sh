...
INSTALL_URL="${ACTIVI_INSTALL_URL:-https://activi.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ACTIVI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ACTIVI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="activi"
PACKAGE_NAME="${ACTIVI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ACTIVI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ACTIVI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ACTIVI_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
