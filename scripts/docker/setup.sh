...
DEFAULT_VOLUMES_DIR="$ROOT_DIR/volumes"
...
CODEX_OAUTH_PROFILES_JSON="${OPENCLAW_CODEX_OAUTH_PROFILES_JSON:-}"
CODEX_OAUTH_PROFILES_FILE="${OPENCLAW_CODEX_OAUTH_PROFILES_FILE:-}"
...
prepare_codex_oauth_profiles_import_file() {
	local import_file="$OPENCLAW_CONFIG_DIR/openai-codex-oauth-profiles.import.json"
	if [[ -n "$CODEX_OAUTH_PROFILES_FILE" ]]; then
		cp "$CODEX_OAUTH_PROFILES_FILE" "$import_file"
		chmod 600 "$import_file"
		printf '%s' "/home/node/.openclaw/$(basename "$import_file")"
		return 0
	fi
	if [[ -n "$CODEX_OAUTH_PROFILES_JSON" ]]; then
		if command -v python3 >/dev/null 2>&1; then
			OPENCLAW_CODEX_OAUTH_PROFILES_JSON="$CODEX_OAUTH_PROFILES_JSON" python3 - "$import_file" <<'PY'
import os
import sys

path = sys.argv[1]
with open(path, "w", encoding="utf-8") as f:
    f.write(os.environ.get("OPENCLAW_CODEX_OAUTH_PROFILES_JSON", ""))
PY
		elif command -v node >/dev/null 2>&1; then
			OPENCLAW_CODEX_OAUTH_PROFILES_JSON="$CODEX_OAUTH_PROFILES_JSON" node - "$import_file" <<'NODE'
const fs = require("node:fs");
const path = process.argv[2];
fs.writeFileSync(path, process.env.OPENCLAW_CODEX_OAUTH_PROFILES_JSON ?? "", "utf8");
NODE
		else
			fail "OPENCLAW_CODEX_OAUTH_PROFILES_JSON requires python3 or node to stage the import file."
		fi
		chmod 600 "$import_file"
		printf '%s' "/home/node/.openclaw/$(basename "$import_file")"
	fi
...
seed_codex_oauth_profiles() {
	if [[ -z "$CODEX_OAUTH_PROFILES_JSON" && -z "$CODEX_OAUTH_PROFILES_FILE" ]]; then
		return 0
	fi
	local import_file
	import_file="$(prepare_codex_oauth_profiles_import_file)"
	if [[ -z "$import_file" ]]; then
		fail "Failed to prepare Codex OAuth profile import file."
	fi
	run_prestart_cli models auth import-oauth --provider openai-codex --profiles-file "$import_file" >/dev/null
	rm -f "$OPENCLAW_CONFIG_DIR/openai-codex-oauth-profiles.import.json"
	echo "Seeded OpenAI Codex OAuth profiles into auth-profiles.json."
}
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$DEFAULT_VOLUMES_DIR/config}"
OPENCLAW_WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-$DEFAULT_VOLUMES_DIR/workspace}"

validate_mount_path_value "OPENCLAW_CONFIG_DIR" "$OPENCLAW_CONFIG_DIR"
validate_mount_path_value "OPENCLAW_WORKSPACE_DIR" "$OPENCLAW_WORKSPACE_DIR"
if [[ -n "$CODEX_OAUTH_PROFILES_FILE" ]]; then
	validate_mount_path_value "OPENCLAW_CODEX_OAUTH_PROFILES_FILE" "$CODEX_OAUTH_PROFILES_FILE"
	if [[ ! -f "$CODEX_OAUTH_PROFILES_FILE" ]]; then
		fail "OPENCLAW_CODEX_OAUTH_PROFILES_FILE must point to an existing file."
	fi
fi
if [[ -n "$CODEX_OAUTH_PROFILES_JSON" && -n "$CODEX_OAUTH_PROFILES_FILE" ]]; then
	fail "Provide only one of OPENCLAW_CODEX_OAUTH_PROFILES_JSON or OPENCLAW_CODEX_OAUTH_PROFILES_FILE."
fi
