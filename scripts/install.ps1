...
function Install-OpenClawNpm {
    ...
    $PackageName = if ($env:QVERISBOT_INSTALL_PACKAGE) { $env:QVERISBOT_INSTALL_PACKAGE } `
                   elseif ($env:OPENCLAW_INSTALL_PACKAGE) { $env:OPENCLAW_INSTALL_PACKAGE } `
                   else { "@qverisai/qverisbot" }
    Write-Host "Installing $PackageName@$Version..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g "$PackageName@$Version" --no-fund --no-audit 2>&1
        Write-Host "$PackageName installed" -Level success
        return $true
    } catch {
...
function Main {
    ...
    $noOnboard = $NoOnboard -or $env:QVERISBOT_NO_ONBOARD -eq "1" -or $env:OPENCLAW_NO_ONBOARD -eq "1"
    if (!$noOnboard -and !$DryRun) {
        Write-Host ""
        if (Get-Command qverisbot -ErrorAction SilentlyContinue) {
            Write-Host "Run 'qverisbot onboard' to complete setup" -Level info
        } elseif (Get-Command openclaw -ErrorAction SilentlyContinue) {
            Write-Host "Run 'openclaw onboard' to complete setup" -Level info
        } else {
            Write-Host "Run the CLI onboard command to complete setup" -Level info
        }
    }
    
    Write-Host ""
    Write-Host "🦞 Installed successfully!" -Level success
}
