# Mullusi Installer for Windows (PowerShell)
# Usage: iwr -useb https://mullusi.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://mullusi.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\mullusi",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 Mullusi Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Mullusi.$NC" -Level info
    Write-Host ""
}
...
function Install-MullusiGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Mullusi from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/mullusi/mullusi.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\mullusi\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\mullusi.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Mullusi from git to $GitDir" -Level info
        } else {
            Install-MullusiGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Mullusi via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-MullusiNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'mullusi onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 Mullusi installed successfully!" -Level success
}
