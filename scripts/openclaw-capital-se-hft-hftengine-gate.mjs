/**
 * openclaw-capital-se-hft-hftengine-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/hft/HftEngine.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-hft-hftengine.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "hft", "HftEngine.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-hft-hftengine-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeHftHftengineGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeHftHftengineGate({
