/**
 * openclaw-capital-se-strategies-macdstrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/MacdStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-macdstrategy.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategies", "MacdStrategy.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-macdstrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesMacdstrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesMacdstrategyGate({
