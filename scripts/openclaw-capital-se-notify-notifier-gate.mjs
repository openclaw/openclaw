/**
 * openclaw-capital-se-notify-notifier-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/notify/Notifier.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-notify-notifier.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "notify", "Notifier.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-notify-notifier-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeNotifyNotifierGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeNotifyNotifierGate({
