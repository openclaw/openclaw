/**
 * openclaw-capital-se-strategies-turtlestrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/TurtleStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-turtlestrategy.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategies", "TurtleStrategy.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-turtlestrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesTurtlestrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesTurtlestrategyGate({
