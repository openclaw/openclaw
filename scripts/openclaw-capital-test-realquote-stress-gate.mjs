/**
 * openclaw-capital-test-realquote-stress-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/test-realquote-stress.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.test-realquote-stress.v1";
const SOURCE_REL_PARTS = ["scripts", "test-realquote-stress.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-test-realquote-stress-gate-latest.json",
);
function hasFlag(f) {
...
export async function runTestRealquoteStressGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runTestRealquoteStressGate({
