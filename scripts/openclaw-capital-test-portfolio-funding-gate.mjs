/**
 * openclaw-capital-test-portfolio-funding-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/test-portfolio-funding.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.test-portfolio-funding.v1";
const SOURCE_REL_PARTS = ["scripts", "test-portfolio-funding.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-test-portfolio-funding-gate-latest.json",
);
function hasFlag(f) {
...
export async function runTestPortfolioFundingGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runTestPortfolioFundingGate({
