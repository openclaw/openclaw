...
import type { ApiContributor, Entry, MapConfig, User } from "./update-neo-contributors.types.js";

const REPO = "betterbrand/smart-agent-neo";
const PER_LINE = 10;

const mapPath = resolve("scripts/neo-contributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing neo-contributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README neo-contributors: ${entries.length} entries`);
