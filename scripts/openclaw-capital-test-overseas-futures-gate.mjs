/**
 * openclaw-capital-test-overseas-futures-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/test-overseas-futures.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.test-overseas-futures.v1";
const SOURCE_REL_PARTS = ["scripts", "test-overseas-futures.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-test-overseas-futures-gate-latest.json",
);
function hasFlag(f) {
...
export async function runTestOverseasFuturesGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runTestOverseasFuturesGate({
