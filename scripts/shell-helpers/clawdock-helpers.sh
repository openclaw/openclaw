#!/usr/bin/env bash
# ClawDock - Docker helpers for Bot
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Bot at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/bot"
      return 1
    fi
...
clawdock-shell() {
  _clawdock_compose exec hanzo-bot-gateway \
    bash -c 'echo "alias bot=\"./hanzo-bot.mjs\"" > /tmp/.bashrc_bot && bash --rcfile /tmp/.bashrc_bot'
}
...
clawdock-cli() {
  _clawdock_compose run --rm bot-cli "$@"
}
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm bot-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Bot${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (bot alias ready)${_CLR_RESET}"
