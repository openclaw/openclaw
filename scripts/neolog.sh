...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "neolog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
neolog - SmartAgentNeo Logging Utility

USAGE:
    neolog [OPTIONS]

DESCRIPTION:
    View SmartAgentNeo logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    SmartAgentNeo logs flow through the macOS unified log (subsystem: ai.smart-agent-neo).
...
QUICK START:
    neolog -n 100             Show last 100 lines from all components
    neolog -f                 Follow logs in real-time
    neolog -e                 Show only errors
    neolog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    neolog                   Show last 50 lines from past 5 minutes (default)
    neolog -f                Stream logs continuously
    neolog -n 100            Show last 100 lines
    neolog -e                Show only recent errors
    neolog -l 30m -n 200     Show last 200 lines from past 30 minutes
    neolog -c ServerManager  Show recent ServerManager logs
    neolog -s "fail"         Search for "fail" in recent logs
    neolog --server -e       Show recent server errors
    neolog -f -d             Stream debug logs continuously
