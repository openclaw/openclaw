/**
 * openclaw-capital-se-risk-stoplossmanager-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/risk/StopLossManager.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-risk-stoplossmanager.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "risk", "StopLossManager.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-risk-stoplossmanager-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeRiskStoplossmanagerGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeRiskStoplossmanagerGate({
