...
export function buildPublishedInstallScenarios(
	version: string,
): PublishedInstallScenario[] {
...
function installSpec(prefixDir: string, spec: string, cwd: string): void {
	npmExec(
		["install", "-g", "--prefix", prefixDir, spec, "--no-fund", "--no-audit"],
		cwd,
	);
}
...
function verifyScenario(
	...
	const workingDir = mkdtempSync(
		join(tmpdir(), `openclaw-postpublish-${scenario.name}.`),
	);
	...
	try {
		...
		console.log(
			`openclaw-npm-postpublish-verify: ${scenario.name} OK (${version})`,
		);
	} finally {
