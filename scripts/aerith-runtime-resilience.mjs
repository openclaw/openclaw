...
export function nativeHookRelayBridgeDir(
...
function printHuman(report) {
  ...
  console.log(
    `relay_records=${report.relay.summary.total} alive=${report.relay.summary.alive} stale=${report.relay.summary.stale}`,
  );
  console.log(`memory_bytes=${report.memory.bytes} bootstrap_risk=${report.memory.bootstrapRisk}`);
  console.log(
    `installed_codex_generation_persistence=${report.installedCodex.persistsNativeHookRelayGeneration}`,
  );
