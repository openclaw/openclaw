/**
 * openclaw-capital-test-hot-reload-dashboard-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/test-hot-reload-dashboard.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.test-hot-reload-dashboard.v1";
const SOURCE_REL_PARTS = ["scripts", "test-hot-reload-dashboard.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-test-hot-reload-dashboard-gate-latest.json",
);
function hasFlag(f) {
...
export async function runTestHotReloadDashboardGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runTestHotReloadDashboardGate({
