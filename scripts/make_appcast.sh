...
ZIP=${1:?"Usage: $0 SmartAgentNeo-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/betterbrand/smart-agent-neo/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^SmartAgentNeo-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/betterbrand/smart-agent-neo/releases/download/v${VERSION}/"}
