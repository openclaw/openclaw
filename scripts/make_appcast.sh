...
ZIP=${1:?"Usage: $0 DNA-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/dna/dna/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^DNA-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/dna/dna/releases/download/v${VERSION}/"}
