...
ZIP=${1:?"Usage: $0 EasyHub-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/EasyHub/EasyHub/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^EasyHub-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/EasyHub/EasyHub/releases/download/v${VERSION}/"}
