...
ZIP=${1:?"Usage: $0 Activi-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/activi/activi/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Activi-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/activi/activi/releases/download/v${VERSION}/"}
