...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "smart-agent-neo-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "smart-agent-neo.json");
  ...
  const sessionId =
    process.env.SMART_AGENT_NEO_ZAI_FALLBACK_SESSION_ID ??
    process.env.NEOBOT_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();

  const baseEnv: NodeJS.ProcessEnv = {
    ...process.env,
    SMART_AGENT_NEO_CONFIG_PATH: configPath,
    SMART_AGENT_NEO_STATE_DIR: stateDir,
    NEOBOT_CONFIG_PATH: configPath,
    NEOBOT_STATE_DIR: stateDir,
    ZAI_API_KEY: zaiKey,
    Z_AI_API_KEY: "",
  };
  ...
  const run1 = await runCommand(
    "run1",
    ["smart-agent-neo", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["smart-agent-neo", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
