...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "EasyHub-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "easyhub.json");
  ...
  const sessionId =
    process.env.EASYHUB_ZAI_FALLBACK_SESSION_ID ??
    process.env.EASYHUB_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();

  const baseEnv: NodeJS.ProcessEnv = {
    ...process.env,
    EASYHUB_CONFIG_PATH: configPath,
    EASYHUB_STATE_DIR: stateDir,
    EASYHUB_CONFIG_PATH: configPath,
    EASYHUB_STATE_DIR: stateDir,
    ZAI_API_KEY: zaiKey,
    Z_AI_API_KEY: "",
  ...
  const run1 = await runCommand(
    "run1",
    ["EasyHub", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["EasyHub", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
