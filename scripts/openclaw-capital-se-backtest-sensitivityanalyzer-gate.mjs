/**
 * openclaw-capital-se-backtest-sensitivityanalyzer-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/backtest/SensitivityAnalyzer.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-backtest-sensitivityanalyzer.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "backtest", "SensitivityAnalyzer.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-backtest-sensitivityanalyzer-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeBacktestSensitivityanalyzerGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeBacktestSensitivityanalyzerGate({
