...
function isBuiltChannelConfigSchema(value: unknown): value is {
...
function resolveConfigSchemaExport(imported: Record<string, unknown>): {
	schema: Record<string, unknown>;
	uiHints?: Record<string, unknown>;
} | null {
	for (const [name, value] of Object.entries(imported)) {
		if (
			name.endsWith("ChannelConfigSchema") &&
			isBuiltChannelConfigSchema(value)
		) {
			return value;
		}
	}
	...
	for (const [name, value] of Object.entries(imported)) {
		if (
			!name.endsWith("ConfigSchema") ||
			name.endsWith("AccountConfigSchema")
		) {
			continue;
		}
...
function shouldRetryViaIsolatedCopy(error: unknown): boolean {
	...
	return (
		...
		message.includes(`${path.sep}node_modules${path.sep}`)
	);
}
...
function copyModuleImportGraphWithoutNodeModules(params: {
	...
	const isolatedRoot = fs.mkdtempSync(
		path.join(tempParent, `${path.basename(packageRoot)}-`),
	);
...
export async function loadChannelConfigSurfaceModule(
    ...
    `;
		...
		if (result.status !== 0) {
			throw new Error(
				result.stderr ||
					result.stdout ||
					`bun loader failed for ${candidatePath}`,
			);
		}
	...
	try {
		...
		const isolatedCopy = copyModuleImportGraphWithoutNodeModules({
			modulePath,
			repoRoot,
		});
		try {
