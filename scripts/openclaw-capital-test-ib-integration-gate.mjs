/**
 * openclaw-capital-test-ib-integration-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/test-ib-integration.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.test-ib-integration.v1";
const SOURCE_REL_PARTS = ["scripts", "test-ib-integration.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-test-ib-integration-gate-latest.json",
);
function hasFlag(f) {
...
export async function runTestIbIntegrationGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runTestIbIntegrationGate({
