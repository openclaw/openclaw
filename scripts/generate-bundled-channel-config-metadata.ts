...
function resolveRootLabel(
	...
	if (
		typeof source.manifest?.name === "string" &&
		source.manifest.name.trim()
	) {
...
function resolveRootDescription(
	...
	if (
		typeof source.manifest?.description === "string" &&
		source.manifest.description.trim()
	) {
...
function formatTypeScriptModule(
	source: string,
	outputPath: string,
	repoRoot: string,
): string {
...
export async function collectBundledChannelConfigMetadata(params?: {
	repoRoot?: string;
}) {
