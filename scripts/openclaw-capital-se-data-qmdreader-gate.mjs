/**
 * openclaw-capital-se-data-qmdreader-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/data/QmdReader.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-data-qmdreader.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "data", "QmdReader.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-data-qmdreader-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeDataQmdreaderGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeDataQmdreaderGate({
