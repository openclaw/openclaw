...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.mullusi) ? packageJson.mullusi : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.mullusi?.channel?.id) || left.name;
    const rightId = trimString(right.mullusi?.channel?.id) || right.name;
