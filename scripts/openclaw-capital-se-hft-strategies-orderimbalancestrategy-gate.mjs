/**
 * openclaw-capital-se-hft-strategies-orderimbalancestrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/hft/strategies/OrderImbalanceStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-hft-strategies-orderimbalancestrategy.v1";
const SOURCE_REL_PARTS = [
  "scripts",
  "strategy-engine",
  "hft",
  "strategies",
  "OrderImbalanceStrategy.mjs",
];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-hft-strategies-orderimbalancestrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeHftStrategiesOrderimbalancestrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeHftStrategiesOrderimbalancestrategyGate({
