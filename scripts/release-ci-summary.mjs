...
export function validateParentManifest(value, expected) {
  ...
  if (value.version === 3 && controls.performanceBlocking !== true) {
    throw new Error("release validation manifest performance blocking control is invalid");
  }
...
export function validateManifestChildRun(
  ...
  // GitHub preserves the bot as `actor` on a rerun and records the requester
  // separately. Only the original parent operator may retry that exact child.
  const allowedTriggeringActors = new Set(
    ["github-actions[bot]", parentActorLogin].filter(
  ...
  if (
    ...
    !allowedTriggeringActors.has(run.triggering_actor?.login) ||
...
function validateStrictChildRun({ child, client, parentEvidence, parentJobs, repository, runId }) {
  ...
  validateManifestChildRun(
    ...
    parentEvidence.parentRun.actor?.login,
    repository,
  );
  if (
