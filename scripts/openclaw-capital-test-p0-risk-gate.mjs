/**
 * openclaw-capital-test-p0-risk-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/test-p0-risk.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.test-p0-risk.v1";
const SOURCE_REL_PARTS = ["scripts", "test-p0-risk.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-test-p0-risk-gate-latest.json",
);
function hasFlag(f) {
...
export async function runTestP0RiskGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runTestP0RiskGate({
