...
if [ "${OPENCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE:-0}" = "1" ]; then
  ...
  if [ "$CANDIDATE_IS_CURRENT" = "1" ] && [ -z "${OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_DIR:-}" ]; then
    ...
    registry_planner_lane="published-upgrade-survivor"
    if [ "${OPENCLAW_UPGRADE_SURVIVOR_UPDATE_CHANNEL:-stable}" = "extended-stable" ]; then
      registry_planner_lane="extended-stable-upgrade-survivor"
    fi
    OPENCLAW_DOCKER_ALL_LANES="$registry_planner_lane" \
  ...
  docker_e2e_run_with_harness \
    ...
    -e OPENCLAW_UPGRADE_SURVIVOR_UPDATE_CHANNEL="${OPENCLAW_UPGRADE_SURVIVOR_UPDATE_CHANNEL:-stable}" \
    -e OPENCLAW_UPGRADE_SURVIVOR_EXEC_APPROVAL_OWNER="${OPENCLAW_UPGRADE_SURVIVOR_EXEC_APPROVAL_OWNER:-sqlite}" \
    -e OPENCLAW_UPGRADE_SURVIVOR_DISCORD_DM_OWNER="${OPENCLAW_UPGRADE_SURVIVOR_DISCORD_DM_OWNER:-canonical}" \
    -e OPENCLAW_UPGRADE_SURVIVOR_SESSION_METADATA_OWNER="${OPENCLAW_UPGRADE_SURVIVOR_SESSION_METADATA_OWNER:-sqlite}" \
    -e OPENCLAW_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_OUTCOME="${OPENCLAW_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_OUTCOME:-preserved}" \
    -e OPENCLAW_UPGRADE_SURVIVOR_PREPUBLISH_PLUGIN_SOURCE="${OPENCLAW_UPGRADE_SURVIVOR_PREPUBLISH_PLUGIN_SOURCE:-npm}" \
    -e OPENCLAW_UPDATE_FIXTURE_ALLOW_MISSING_LEGACY_COMPAT="${OPENCLAW_UPDATE_FIXTURE_ALLOW_MISSING_LEGACY_COMPAT:-0}" \
    -e OPENCLAW_UPGRADE_SURVIVOR_DOCTOR_REPAIRED_SERVICE="${OPENCLAW_UPGRADE_SURVIVOR_DOCTOR_REPAIRED_SERVICE:-0}" \
    -e OPENCLAW_UPGRADE_SURVIVOR_MOCK_CONFIG_DIALECT="${OPENCLAW_UPGRADE_SURVIVOR_MOCK_CONFIG_DIALECT:-current}" \
    -e OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_INCLUDE_CORE="${OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_INCLUDE_CORE:-0}" \
