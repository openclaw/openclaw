...
export type ClaudeChannelNotification = z.infer<
	typeof ClaudeChannelNotificationSchema
>["params"];
...
export async function connectGateway(params: {
	...
	ws.once("close", (code, reason) => {
		const error = new Error(
			`gateway closed (${code}): ${rawDataToString(reason)}`,
		);
...
export async function maybeApprovePendingBridgePairing(
	...
	const pendingRequest = pairingState.pending?.find(
		(entry) => entry.role === "operator",
	);
	if (!pendingRequest?.requestId) {
		return false;
	}
	await gateway.request("device.pair.approve", {
