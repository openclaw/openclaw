...
IMAGE_NAME="smart-agent-neo-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/smart-agent-neo-bin:$PATH"
  mkdir -p /tmp/smart-agent-neo-bin

  cat > /tmp/smart-agent-neo-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/smart-agent-neo-bin/systemctl

  cat > /tmp/smart-agent-neo-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/smart-agent-neo-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/smart-agent-neo"
	  npm_root="/tmp/npm-prefix/lib/node_modules/smart-agent-neo"
	  ...
	  git_cli="/app/smart-agent-neo.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/smart-agent-neo-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/smart-agent-neo-gateway.service"
    if [ ! -f "$unit_path" ]; then
