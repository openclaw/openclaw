...
IMAGE_NAME="dna-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/dna-bin:$PATH"
  mkdir -p /tmp/dna-bin

  cat > /tmp/dna-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/dna-bin/systemctl

  cat > /tmp/dna-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_bin="/tmp/npm-prefix/bin/dna"
  npm_entry="/tmp/npm-prefix/lib/node_modules/dna/dna.mjs"
  git_entry="/app/dna.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/dna-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/dna-gateway.service"
    if [ ! -f "$unit_path" ]; then
