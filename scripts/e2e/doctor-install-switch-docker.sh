...
IMAGE_NAME="EasyHub-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/EasyHub-bin:$PATH"
  mkdir -p /tmp/EasyHub-bin

  cat > /tmp/EasyHub-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/EasyHub-bin/systemctl

  cat > /tmp/EasyHub-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/EasyHub-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/EasyHub"
	  npm_root="/tmp/npm-prefix/lib/node_modules/EasyHub"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/EasyHub-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/EasyHub-gateway.service"
    if [ ! -f "$unit_path" ]; then
