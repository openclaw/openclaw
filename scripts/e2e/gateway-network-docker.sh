...
IMAGE_NAME="smart-agent-neo-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="smart-agent-neo-net-e2e-$$"
GW_NAME="smart-agent-neo-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SMART_AGENT_NEO_GATEWAY_TOKEN=$TOKEN" \
	  -e "SMART_AGENT_NEO_SKIP_CHANNELS=1" \
	  -e "SMART_AGENT_NEO_SKIP_GMAIL_WATCHER=1" \
	  -e "SMART_AGENT_NEO_SKIP_CRON=1" \
	  -e "SMART_AGENT_NEO_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
