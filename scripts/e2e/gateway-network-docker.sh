...
IMAGE_NAME="dna-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="dna-net-e2e-$$"
GW_NAME="dna-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "DNA_GATEWAY_TOKEN=$TOKEN" \
	  -e "DNA_SKIP_CHANNELS=1" \
	  -e "DNA_SKIP_GMAIL_WATCHER=1" \
	  -e "DNA_SKIP_CRON=1" \
	  -e "DNA_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
