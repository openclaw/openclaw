...
IMAGE_NAME="activi-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="activi-net-e2e-$$"
GW_NAME="activi-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ACTIVI_GATEWAY_TOKEN=$TOKEN" \
	  -e "ACTIVI_SKIP_CHANNELS=1" \
	  -e "ACTIVI_SKIP_GMAIL_WATCHER=1" \
	  -e "ACTIVI_SKIP_CRON=1" \
	  -e "ACTIVI_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
