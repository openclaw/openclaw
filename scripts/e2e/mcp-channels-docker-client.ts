	...
	try {
		...
		const attachmentMessage = messages.find((entry) => {
			...
			return (
				raw &&
				typeof raw === "object" &&
				(raw as { id?: unknown }).id === "msg-attachment"
			);
		});
		...
		assert(
			...
			"expected assistant event role",
		);
		assert(
			assistantEvent.text === "assistant live event",
			"expected assistant event text",
		);
		const assistantCursor =
			typeof assistantEvent.cursor === "number" ? assistantEvent.cursor : 0;
		...
		assert(
			...
			"expected raw gateway session.message after chat.send",
		);
		...
		assert(
			helpNotification.content === channelMessage,
			"expected Claude channel content",
		);
		...
		const permission = await waitFor(
			...
			() =>
				mcpHandle.rawMessages
					...
					.find(
						(entry) =>
							entry.success && entry.data.params.request_id === "abcde",
					)?.data.params,
		);
