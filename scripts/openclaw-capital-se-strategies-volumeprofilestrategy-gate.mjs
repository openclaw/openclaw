/**
 * openclaw-capital-se-strategies-volumeprofilestrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/VolumeProfileStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-volumeprofilestrategy.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategies", "VolumeProfileStrategy.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-volumeprofilestrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesVolumeprofilestrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesVolumeprofilestrategyGate({
