/**
 * openclaw-capital-se-strategies-macrossstrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/MaCrossStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-macrossstrategy.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategies", "MaCrossStrategy.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-macrossstrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesMacrossstrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesMacrossstrategyGate({
