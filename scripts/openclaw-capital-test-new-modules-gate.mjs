/**
 * openclaw-capital-test-new-modules-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/test-new-modules.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.test-new-modules.v1";
const SOURCE_REL_PARTS = ["scripts", "test-new-modules.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-test-new-modules-gate-latest.json",
);
function hasFlag(f) {
...
export async function runTestNewModulesGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runTestNewModulesGate({
