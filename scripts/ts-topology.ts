...
function parseArgs(argv: string[]): CliOptions {
	...
	for (const arg of args) {
		...
		switch (flag) {
			...
			case "--report":
				options.report =
					(value as TopologyReportName | undefined) ?? options.report;
				break;
			case "--limit":
...
function resolveScope(options: CliOptions): TopologyScope {
	...
	if (options.scopeId === "custom") {
		if (!options.customEntrypointRoot || !options.customImportPrefix) {
			throw new Error(
				"--scope=custom requires --entrypoint-root and --import-prefix",
			);
		}
...
function assertValidReport(
	report: string,
): asserts report is TopologyReportName {
