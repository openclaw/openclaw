/**
 * openclaw-capital-se-strategies-divergencestrategy-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/strategies/DivergenceStrategy.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-strategies-divergencestrategy.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "strategies", "DivergenceStrategy.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-strategies-divergencestrategy-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSeStrategiesDivergencestrategyGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSeStrategiesDivergencestrategyGate({
