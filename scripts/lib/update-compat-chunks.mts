...
const GENERATED_HEADER = "// Generated by update-compat-inventory; do not edit.";
...
export function isUpdateCompatibilityChunk(source: string): boolean {
...
export function recordUpdateCompatibilityRelease(params: {
  ...
  if (!chunks.size) {
    throw new Error("Release package has no recorded post-swap import surface");
  }
  return {
...
export function readUpdateCompatibilityInventory(file: string): UpdateCompatibilityInventory {
  ...
  if (
    ...
    value.releases.length > 2
  ) {
    throw new Error(`Invalid bounded update compatibility inventory: ${file}`);
  }
  ...
  const stable = releases.filter((release) => /^\d{4}\.\d+\.\d+(?:-\d+)?$/.test(release.version));
  const beta = releases.filter((release) => /^\d{4}\.\d+\.\d+-beta\.\d+$/.test(release.version));
  if (stable.length !== 1 || stable.length + beta.length !== releases.length) {
    throw new Error(
      "Inventory must contain one previous stable release and at most one current beta release",
    );
  }
...
export function writeUpdateCompatibilityChunks(params: {
  ...
  for (const release of params.inventory.releases) {
    for (const chunk of release.chunks) {
      ...
      for (const entry of chunk.exports) {
        ...
        if (
          ...
          throw new Error(`Release inventories disagree about ${chunk.path}:${entry.exported}`);
        }
        if (!previous) {
  ...
  const required = [...requiredByPath.values()];
