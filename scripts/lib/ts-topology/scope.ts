...
function classifyUsageBucketForRoots(
	...
	const internalRoots = [
		...new Set(
			entrypoints.map((entrypoint) =>
				path.posix.dirname(entrypoint.sourcePath),
			),
		),
	];
	const publicSpecifiers = new Set(
		entrypoints.map((entrypoint) => entrypoint.importSpecifier),
	);
	return {
		id,
		description,
...
export function createPluginSdkScope(_repoRoot: string): TopologyScope {
	...
	return buildScopeFromEntrypoints(
		"plugin-sdk",
		"OpenClaw plugin-sdk public surface",
		entrypoints,
	);
}
		...
		return {
			...
			importSpecifier:
				entrypoint === "index"
					...
					: `${options.importPrefix}/${entrypoint}`,
		};
	});
	return buildScopeFromEntrypoints(
		options.id,
