...
function computeSharednessScore(record: TopologyRecord): number {
	...
	const testOnlyPenalty =
		record.productionRefCount === 0 && record.testRefCount > 0 ? 25 : 0;
	return clampScore(
		extensionWeight +
			packageWeight +
			internalWeight +
...
function computeMoveBackToOwnerScore(record: TopologyRecord): number {
	const singleExtensionWeight =
		record.productionExtensions.length === 1 ? 45 : 0;
	const noNonExtensionOwnersWeight =
		record.productionPackages.length === 0 ? 20 : 0;
...
function createRecord(
	info: ReturnType<typeof canonicalSymbolInfo>,
): TopologyRecord {
	return {
...
function buildScopeMaps(context: ProgramContext, scope: TopologyScope) {
	...
	const recordBySpecifierAndExportName = new Map<
		string,
		Map<string, TopologyRecord>
	>();
	...
	for (const entrypoint of scope.entrypoints) {
		...
		for (const exportedSymbol of context.checker.getExportsOfModule(
			moduleSymbol,
		)) {
...
function collectReferenceEvents(
	...
	for (const sourceFile of context.program.getSourceFiles()) {
		...
		if (
			!normalizedFileName.startsWith(context.normalizePath(context.repoRoot))
		) {
			continue;
		}
		...
		for (const statement of sourceFile.statements) {
			...
			if (ts.isNamedImports(clause.namedBindings)) {
				for (const element of clause.namedBindings.elements) {
					...
					events.push({
						...
						usageCount: countIdentifierUsages(
							context,
							sourceFile,
							localSymbol,
							element.name.text,
						...
						extensionId:
							bucket === "production" ? scope.extensionForPath(relPath) : null,
						packageOwner:
							bucket === "production"
								? scope.packageOwnerForPath(relPath)
								: null,
					});
				}
				continue;
			...
			if (ts.isNamespaceImport(clause.namedBindings)) {
				const namespaceSymbol = context.checker.getSymbolAtLocation(
...
function buildRankedCandidates(
	...
	return {
		candidateToMove: records
			...
			.toSorted(
				(left, right) => right.moveBackToOwnerScore - left.moveBackToOwnerScore,
			)
			.slice(0, limit),
		duplicatedPublicExports: records
			...
			.toSorted(
				(left, right) =>
					right.publicSpecifiers.length - left.publicSpecifiers.length,
			)
			.slice(0, limit),
		singleOwnerShared: records
			...
			.toSorted(
				(left, right) => right.productionRefCount - left.productionRefCount,
			)
			.slice(0, limit),
	};
}
...
export function analyzeTopology(options: {
	...
	const { recordByCanonicalKey, recordBySpecifierAndExportName } =
		buildScopeMaps(context, options.scope);
	...
	return {
		...
		totals: {
			...
			usedByProduction: allRecords.filter(
				(record) => record.productionImportCount > 0,
			).length,
			usedByTests: allRecords.filter((record) => record.testImportCount > 0)
				.length,
			usedInternally: allRecords.filter(
				(record) => record.internalImportCount > 0,
			).length,
