...
} from "../openclaw-npm-release-check.ts";
...
export function parsePluginReleaseSelection(
...
export function parsePluginReleaseArgs(
	argv: string[],
): ParsedPluginReleaseArgs {
	...
	if (selectionMode && (baseRef || headRef)) {
		throw new Error(
			"Use either --selection-mode or --base-ref/--head-ref, not both.",
		);
	}
...
export function collectPublishablePluginPackages(
	...
	for (const dir of dirs) {
		...
		publishable.push({
			...
			installNpmSpec:
				packageJson.openclaw?.install?.npmSpec?.trim() || undefined,
		});
	}
	...
	return publishable.toSorted((left, right) =>
...
export function resolveSelectedPublishablePluginPackages(params: {
	...
	const byName = new Map(
		params.plugins.map((plugin) => [plugin.packageName, plugin]),
	);
	...
	if (missing.length > 0) {
		throw new Error(
			`Unknown or non-publishable plugin package selection: ${missing.join(", ")}.`,
		);
	}
...
export function collectChangedExtensionIdsFromPaths(
...
export function isPluginVersionPublished(
	packageName: string,
	version: string,
): boolean {
...
export function collectPluginReleasePlan(params?: {
	...
	const all = selectedPublishable.map((plugin) => ({
		...plugin,
		alreadyPublished: isPluginVersionPublished(
			plugin.packageName,
			plugin.version,
		),
	}));
