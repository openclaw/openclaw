...
const ANDROID_WEAR_VERSION_CODE_SUFFIX = "1";
...
export function canonicalAndroidWearVersionCode(rawVersionCode: number, version: string): number {
  ...
  if (
    ...
    wearVersionCode <= 0 ||
    wearVersionCode > ANDROID_VERSION_CODE_MAX
  ) {
    throw new Error(
      `Unable to derive Wear versionCode from Android versionCode ${rawVersionCode}.`,
    );
  }
