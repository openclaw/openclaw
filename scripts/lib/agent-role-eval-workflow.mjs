...
function requireNoControlCharacters(value, label) {
...
export function resolveLiveWorkflowConfig(env = process.env) {
  ...
  return {
    model: requireNoControlCharacters(model, "Live eval model"),
...
function writeGitHubEnv(env, key, value) {
  ...
  if (!/^[A-Z_][A-Z0-9_]*$/.test(key)) {
    throw new Error(`Invalid GitHub env key: ${JSON.stringify(key)}.`);
  }
  const safeValue = requireNoControlCharacters(value, `GitHub env ${key}`);
  fs.appendFileSync(env.GITHUB_ENV, `${key}=${safeValue}\n`, "utf8");
}
