  ...
  docker_e2e_docker_run_cmd() {
    if [ "${1:-}" = "run" ]; then
      ...
      if declare -F docker_e2e_timeout_cmd >/dev/null 2>&1; then
        docker_e2e_timeout_cmd "${DOCKER_COMMAND_TIMEOUT:-${OPENCLAW_DOCKER_E2E_RUN_TIMEOUT:-3600s}}" docker run "${DOCKER_E2E_RUN_RESOURCE_ARGS[@]+"${DOCKER_E2E_RUN_RESOURCE_ARGS[@]}"}" "$@"
        return
      fi
      set -- run "${DOCKER_E2E_RUN_RESOURCE_ARGS[@]+"${DOCKER_E2E_RUN_RESOURCE_ARGS[@]}"}" "$@"
    fi
    if declare -F docker_e2e_timeout_cmd >/dev/null 2>&1; then
...
docker_e2e_run_with_harness() {
  ...
  local harness_stdin_fd=9
  local harness_stdin_fd_open=0
  ...
  cleanup_harness_run() {
    ...
    docker_e2e_cleanup_package_mount_args
    if [ "$harness_stdin_fd_open" = "1" ]; then
      eval "exec ${harness_stdin_fd}<&-"
      harness_stdin_fd_open=0
    fi
    restore_harness_traps
    if [ "$exit_after_cleanup" = "1" ]; then
      exit "$cleanup_status"
