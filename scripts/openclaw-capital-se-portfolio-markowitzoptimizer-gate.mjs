/**
 * openclaw-capital-se-portfolio-markowitzoptimizer-gate.mjs — adapter gate（自動生成）
 * 來源: scripts/strategy-engine/portfolio/MarkowitzOptimizer.mjs
 */
...
const SCHEMA = "openclaw.capital.adapter-gate.se-portfolio-markowitzoptimizer.v1";
const SOURCE_REL_PARTS = ["scripts", "strategy-engine", "portfolio", "MarkowitzOptimizer.mjs"];
...
const DEFAULT_REPORT = path.join(
  ...
  "hermes-agent",
  "state",
  "openclaw-capital-se-portfolio-markowitzoptimizer-gate-latest.json",
);
function hasFlag(f) {
...
export async function runSePortfolioMarkowitzoptimizerGate(options = {}) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === currentFile) {
  const { report } = await runSePortfolioMarkowitzoptimizerGate({
