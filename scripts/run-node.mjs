...
const compiler = env.DNA_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.DNA_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.DNA_RUNNER_LOG === "0") return;
  process.stderr.write(`[dna] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["dna.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
