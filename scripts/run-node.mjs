...
const shouldBuild = (deps) => {
  if (deps.env.SMART_AGENT_NEO_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.SMART_AGENT_NEO_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[smart-agent-neo] ${message}\n`);
};

const runSmartAgentNeo = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["smart-agent-neo.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runSmartAgentNeo(deps);
  }
