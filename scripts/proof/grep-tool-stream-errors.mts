// Real behavior proof: grep tool handles stdout/stderr stream error events
// without crashing the agent runtime.
//
// The proof patches child_process.spawn so the spawned ripgrep is a real process
// whose stdout/stderr streams emit error events after the grep tool attaches
// listeners. With the fix the tool promise rejects with the stream error; without
...
const { createGrepToolDefinition } = await import(
  path.join(repoRoot, "src/agents/sessions/tools/grep.ts")
);

async function runProof(stream: "stdout" | "stderr"): Promise<boolean> {
  const tool = createGrepToolDefinition(repoRoot);
  const resultPromise = tool.execute(
    `call-${stream}`,
    { pattern: "TODO", path: repoRoot },
    undefined,
    undefined,
    {} as never,
  );
  ...
  try {
    await resultPromise;
    console.log(`FAIL (${stream}): grep resolved instead of rejecting`);
    return false;
  } catch (err) {
...
console.log("=== Proof: grep tool stream error handling ===\n");
