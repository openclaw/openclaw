...
// Supported:
const supportedFromMatrix = matrixData.entries.map((entry) =>
  entry.configFile === "auth-profiles.json" && entry.refPath ? entry.refPath : entry.path
);
const supportedSet = new Set(supportedFromMatrix);

// Unsupported:
const unsupportedSet = new Set(matrixData.excludedMutableOrRuntimeManaged);

// Read the current surface MD file
const originalSurface = fs.readFileSync(surfacePath, "utf8");

// Parse the current lines to map keys to their full line format so we preserve comments/descriptions
...
// Generate new lists
const newSupportedLines = [...supportedSet].toSorted().map(key => {
...
if (WRITE) {
  ...
  console.log("SecretRef reference docs generated successfully.");
} else if (CHECK) {
  ...
  console.error("Please specify --check or --write");
  process.exit(1);
}
