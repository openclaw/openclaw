---
summary: "Technical RFC for the OpenClaw x Agent Apps integration model, runtime boundaries, compaction semantics, and rollout plan"
owner: "codex"
status: "draft"
last_updated: "2026-03-07"
title: "OpenClaw x Agent Apps Technical RFC"
---

# OpenClaw x Agent Apps Technical RFC

## 0. Relationship to the principles document

This RFC operationalizes the rules in `OPENCLAW_Agent Apps_DESIGN_PRINCIPLES.md`.
...
OpenClaw already contains a functioning Agent Apps integration layer.
...
- a gateway-scoped Agent Apps runtime
- session-bound desktops
- per-run adapter installation
- snapshot-to-messages projection
- snapshot-to-tools projection
- tool routing back into the Agent Apps kernel

The question is no longer "should OpenClaw integrate Agent Apps".
The question is:
...
- Agent Apps is a stateful runtime surface for agent-facing apps.
- Agent Apps is not durable truth storage.
- Agent Apps is a volatile operational scratchpad.
...
- Agent Apps owns live app execution, live tool surfacing, and live state projection.
- The LLM sees Agent Apps as part of its current world, not as the canonical session archive.

This is the only framing that remains stable under scale.

If Agent Apps is treated as permanent state, token costs climb until the system fights itself.
If Agent Apps is treated as disposable world state with semantic carryover, the system can remain fast, compressible, and recoverable.
...
- OpenClaw-side Agent Apps runtime ownership
- session/desktop identity model
- tool and message projection model
- compaction behavior
- app policy/config ownership
- state-loss model for Agent Apps apps
- rollout and validation plan

This RFC does not attempt to redesign upstream Agent Apps runtime internals.
...
- preserve the full Agent Apps internal tree across compaction
- make Agent Apps a second transcript layer
- keep every Agent Apps app installed for every agent
...
`src/gateway/server.impl.ts` starts one gateway-level Agent Apps runtime via `startAotuiGatewayRuntime()`.
...
- it keeps Agent Apps as a bounded integration layer
...
The Agent Apps integration exposes runtime options that are not fully productized.

Unused config surface weakens clarity and should be removed until real.

## 7. Target architecture

The target OpenClaw x Agent Apps architecture should be composed of six layers.

### 7.1 Layer 1: Runtime Service

Responsibility:

- own the gateway-scoped Agent Apps kernel
- manage lifecycle
...
- map canonical OpenClaw session identity to one Agent Apps desktop identity
...
- decide which Agent Apps apps an agent is allowed to use
...
- define what happens to Agent Apps state when OpenClaw compacts context
...
Agent Apps app installation and projection policy must move fully into OpenClaw config.
...
1. OpenClaw starts one Agent Apps kernel service.
...
5. Adapter replaces old Agent Apps-injected runtime messages with the new ones.
6. Adapter replaces tool surface with base tools + current Agent Apps tools.
...
sequenceDiagram
    ...
    participant AD as Agent Apps Adapter
    participant DM as Desktop Manager
    participant TP as Turn Projector
    participant K as Agent Apps Kernel
...
When a projected Agent Apps tool is called:

1. Adapter resolves the binding from the active projection.
2. Adapter acquires desktop lock.
3. Adapter executes the Agent Apps operation on that desktop.
4. Adapter releases the lock.

This preserves OpenClaw as caller and Agent Apps as execution target.

## 11. Projection semantics

### 11.1 Message roles

System-like content emitted from Agent Apps must not use `role: "user"`.
...
The Agent Apps projector should expose the latest relevant world.
...
- Agent Apps is a working surface
...
This means OpenClaw does not need a separate structured Agent Apps compaction-summary artifact as a baseline requirement.
...
## 13. App contract for OpenClaw-integrated Agent Apps apps
...
5. How many prompt tokens came from Agent Apps projection?
...
- Agent Apps projection message/tool counts
- token contribution estimates for Agent Apps projection
...
- compaction must have an explicit Agent Apps-state policy hook
...
They are required if the team wants the OpenClaw x Agent Apps architecture to remain clean over time instead of gradually degrading into host-side hacks.

These requests are directed at the Agent Apps runtime and SDK layer, not the OpenClaw integration layer.
...
- Agent Apps projection budget telemetry
...
- old Agent Apps injected messages are replaced cleanly
...
The OpenClaw x Agent Apps architecture is considered aligned when all of the following are true:

- OpenClaw config fully determines Agent Apps app exposure
...
- compaction has an explicit Agent Apps policy and may reinitialize desktop apps
...
OpenClaw should treat Agent Apps as a gateway-owned, session-bound, agent-facing runtime surface that exposes live state and tools to the model but does not own durable truth; the system must preserve semantic continuity, allow `appState` reclamation during compaction, derive one turn from one snapshot epoch, and keep all capability and lifecycle authority inside OpenClaw rather than in hidden external control planes.
