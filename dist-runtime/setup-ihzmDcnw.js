...
import { Wb as readConfigFileSnapshot, Yb as writeConfigFile } from "./auth-profiles-DqxBs6Au.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-BXq8Wrym.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CMOK7rEA.js");
		const { logConfigUpdated } = await import("./logging-0_nMYSFr.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-Hd0Edtnu.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-BGCo9bR0.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-BziBleRh.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DLvEob5p.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-DNF1vrxj.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) {await prompter.note("Skipping channel setup.", "Channels");}
		...
		const { listChannelPlugins } = await import("./plugins-Bo5F-qAl.js");
		const { setupChannels } = await import("./onboard-channels-Dkyduvaw.js");
	...
	if (opts.skipSearch) {await prompter.note("Skipping search setup.", "Search");}
		...
		const { setupSearch } = await import("./onboard-search-C5V0CGzG.js");
	...
	if (opts.skipSkills) {await prompter.note("Skipping skills setup.", "Skills");}
		...
		const { setupSkills } = await import("./onboard-skills-CmuYk2tg.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-xYeAGbbI.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-Dmqy-lUj.js");
