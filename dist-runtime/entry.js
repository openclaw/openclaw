...
import { G as isRootHelpInvocation, H as hasHelpOrVersion, K as isRootVersionInvocation, W as init_argv } from "./globals-B6h30oSy.js";
...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-DMWiCyPj.js");
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-BYFu2MF9.js").then(({ outputRootHelp }) => {
		outputRootHelp();
	}).catch(handleError);
	return true;
}
function runMainOrRootHelp(argv) {
	...
	import("./run-main-C5flUylF.js").then(({ runCli }) => runCli(argv)).catch((error) => {
