...
import { $d as resolveEffectiveMessagesConfig, Aa as resolveHeartbeatSenderContext, Ba as buildOutboundSessionContext, Bd as resolveUserTimezone, Cp as canonicalizeMainSessionAlias, Gf as saveSessionStore, U as withProgress, Uf as loadSessionStore, Va as deliverOutboundPayloads, Vg as readBestEffortConfig, Wd as HEARTBEAT_TOKEN, ac as setHeartbeatWakeHandler, bd as resolveHeartbeatPrompt$1, cc as buildGatewayConnectionDetails, ec as peekSystemEventEntries, gn as getReplyFromConfig, ic as requestHeartbeatNow, ja as resolveOutboundTarget, ka as resolveHeartbeatDeliveryTarget, lc as callGateway, qa as extractReplyToTag, qf as updateSessionStore, qr as appendCronStyleCurrentTimeLine, rc as areHeartbeatsEnabled, sc as resolveHeartbeatReasonKind, sd as CommandLane, td as getQueueSize, wp as resolveAgentMainSessionKey, xd as stripHeartbeatToken, yd as isHeartbeatContentEffectivelyEmpty, zg as loadConfig } from "./auth-profiles-CAw5gP6f.js";
...
function extractHeartbeatToTag(text) {
	const routeRegex = /\[\[\s*heartbeat_to\s*:\s*([^\]]+?)\s*\]\]/giu;
	let targetTo;
	return {
		cleaned: text.replace(routeRegex, (_match, rawTarget) => {
			if (targetTo) return "";
			const candidate = String(rawTarget ?? "").trim();
			if (candidate) targetTo = candidate;
			return "";
		}),
		targetTo
	};
...
function resolveHeartbeatDeliveryOverride(params) {
	...
	const allowlist = Array.isArray(rawAllowlist) ? rawAllowlist.filter((entry) => typeof entry === "string").map((entry) => entry.trim()).filter(Boolean) : [];
	if (allowlist.length > 0) {
		const allowedTargets = /* @__PURE__ */ new Set();
...
function normalizeHeartbeatReply(payload, responsePrefix, ackMaxChars) {
	const { cleaned: textWithoutRouteTags, targetTo } = extractHeartbeatToTag(typeof payload.text === "string" ? payload.text : "");
	const stripped = stripHeartbeatToken(stripLeadingHeartbeatResponsePrefix(textWithoutRouteTags.includes("[[") ? extractReplyToTag(textWithoutRouteTags).cleaned : textWithoutRouteTags, responsePrefix), {
		mode: "heartbeat",
...
function sanitizeHeartbeatOutboundText(text) {
	if (!text) return text;
...
async function runHeartbeatOnce(opts) {
	...
	try {
		...
		const sanitizedReasoningPayloads = reasoningPayloads.map((payload) => {
			if (typeof payload.text !== "string" || !payload.text.trim()) return payload;
			const sanitizedText = sanitizeHeartbeatOutboundText(payload.text);
			return sanitizedText === payload.text ? payload : {
				...payload,
				text: sanitizedText
			};
		});
		const sanitizedReasoningEdits = sanitizedReasoningPayloads.reduce((count, payload, index) => payload.text !== reasoningPayloads[index]?.text ? count + 1 : count, 0);
		const originalMainText = normalized.text;
		const sanitizedMainText = shouldSkipMain ? originalMainText : sanitizeHeartbeatOutboundText(originalMainText);
		const mainPayloadRedacted = !shouldSkipMain && sanitizedMainText !== originalMainText;
		if (!shouldSkipMain) normalized.text = sanitizedMainText;
		if (sanitizedReasoningEdits > 0 || mainPayloadRedacted) log.info("heartbeat: redacted sensitive content in outbound payload", {
		...
		const previewText = shouldSkipMain ? sanitizedReasoningPayloads.map((payload) => payload.text).filter((text) => Boolean(text?.trim())).join("\n") : normalized.text;
		...
		if (normalized.targetTo && effectiveDelivery.to !== delivery.to && effectiveDelivery.channel !== "none") log.info("heartbeat: applying heartbeat_to override", {
			baseTo: delivery.to,
			overrideTo: effectiveDelivery.to,
			channel: effectiveDelivery.channel
		});
		const effectiveVisibility = effectiveDelivery.channel !== "none" ? resolveHeartbeatVisibility({
			cfg,
			channel: effectiveDelivery.channel,
		...
		if (!effectiveVisibility.showAlerts) {
			...
			emitHeartbeatEvent({
				...
				hasMedia: mediaUrls.length > 0,
				accountId: effectiveDelivery.accountId,
				indicatorType: effectiveVisibility.useIndicator ? resolveIndicatorType("sent") : void 0
			});
			return {
				status: "ran",
				durationMs: Date.now() - startedAt
		...
		await deliverOutboundPayloads({
			...
			payloads: [...sanitizedReasoningPayloads, ...shouldSkipMain ? [] : [{
