...
import { At as resolveControlUiLinks, O_ as writeConfigFile, Pt as waitForGatewayReachable, St as handleReset, gt as applyWizardMetadata, ht as DEFAULT_WORKSPACE, kt as randomToken, w_ as readConfigFileSnapshot, wt as normalizeGatewayTokenInput, yt as ensureWorkspaceAndSessions } from "./reply-kqyKJg4x.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-D_xCZoBv.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-C6v4Ab7y.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-e8qtG4SB.js").then((n) => n.i);
