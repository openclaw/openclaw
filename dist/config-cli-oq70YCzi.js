import { d as success, h as theme, n as info, r as isDevMode, t as danger } from "./globals-DqM7Q4km.js";
import { t as CONFIG_PATH } from "./paths-BMo6kTge.js";
import { d as defaultRuntime } from "./subsystem-BXiL6bA6.js";
import "./boolean-DtWR5bt3.js";
import { G as writeConfigFile, V as readConfigFileSnapshot } from "./auth-profiles-C39jSzPb.js";
...
import { n as formatConfigIssueLines, r as normalizeConfigIssues } from "./issue-format-COD5qdhp.js";
import { t as redactConfigObject } from "./redact-snapshot-ZOKKEi5E.js";
...
async function runConfigGet(opts) {
	...
	try {
		...
		const res = getAtPath(isDevMode() ? snapshot.config : redactConfigObject(snapshot.config), parsedPath);
		if (!res.found) {
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/unset/file/validate). Run without subcommand for the setup wizard.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`).option("--section <section>", "Configure wizard sections (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-BRXPVBJs.js").then((n) => n.t);
