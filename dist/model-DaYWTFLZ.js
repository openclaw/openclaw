...
import { Fi as DEFAULT_CONTEXT_TOKENS, d as normalizeProviderId, i as findNormalizedProviderValue, xi as resolveOpenClawAgentDir } from "./model-selection-COYmqEoi.js";
import { n as discoverModels, t as discoverAuthStorage } from "./pi-model-discovery-Btdks6K9.js";
...
function applyConfiguredProviderOverrides(params) {
	...
	if (!providerConfig) return discoveredModel;
	const configuredModel = providerConfig.models?.find((candidate) => candidate.id === modelId);
	if (!configuredModel && !providerConfig.baseUrl && !providerConfig.api && !providerConfig.headers) return discoveredModel;
	return {
		...discoveredModel,
		...
		headers: providerConfig.headers || configuredModel?.headers ? {
...
function resolveModel(provider, modelId, agentDir, cfg) {
	...
	return {
		model: normalizeModelCompat(applyConfiguredProviderOverrides({
