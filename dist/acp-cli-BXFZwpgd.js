import { h as theme, r as isDevMode } from "./globals-DqM7Q4km.js";
...
import { t as GatewayClient } from "./client-DCYSexfL.js";
import { s as resolveGatewayCredentialsWithSecretInputs, t as buildGatewayConnectionDetails } from "./call-NWndvBdo.js";
...
import { t as DANGEROUS_ACP_TOOLS } from "./dangerous-tools-BHEZs5NH.js";
...
var AcpGatewayAgent = class {
	...
	async prompt(params) {
		...
		const meta = parseSessionMeta(params._meta);
		const maxBytes = getMaxPromptBytes();
		const userText = extractTextFromPrompt(params.prompt, maxBytes);
		const attachments = extractAttachmentsFromPrompt(params.prompt);
		const prefixCwd = meta.prefixCwd ?? this.opts.prefixCwd ?? true;
		const displayCwd = shortenHomePath(session.cwd);
		const message = prefixCwd ? `[Working directory: ${displayCwd}]\n\n${userText}` : userText;
		if (Buffer.byteLength(message, "utf-8") > maxBytes) throw new Error(`Prompt exceeds maximum allowed size of ${maxBytes} bytes`);
