import { m as resolveSecretInputRef } from "./types.secrets-DwPik3M8.js";
...
import { r as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-DXdptBl7.js";
import "./auth-DZ1o6pTQ.js";
import { n as resolveGatewayAuth } from "./auth-resolve-BEn3Se6k.js";
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-BjTMU4_N.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-D2clMNPw.js";
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-BJWQ32it.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-CKHH4hDi.js");
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-COQALFKi.js");
...
const bootstrapSizeCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { buildBootstrapInjectionStats, analyzeBootstrapBudget } = await import("./bootstrap-budget-Dq9hxfds.js");
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-BETPEwEg.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./pi-embedded-helpers-DiLbg8AE.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./oauth-tls-preflight-CiZDfYZH.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./doctor-cron-D28Ic9eq.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-D5iYDjsD.js");
...
const workspaceStatusCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyWorkspaceDirs } = await import("./doctor-workspace-B9I8OuOH.js");
		const workspaceDir = resolveAgentWorkspaceDir(ctx.cfg, resolveDefaultAgentId(ctx.cfg));
...
//#endregion
export { registerCoreHealthChecks as i, configValidationIssuesToHealthFindings as n, createCoreHealthChecks as r, CORE_HEALTH_CHECKS as t };
