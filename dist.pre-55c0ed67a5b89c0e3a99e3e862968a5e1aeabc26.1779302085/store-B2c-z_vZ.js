...
import { d as resolveConfigDir } from "./utils-DX02THHb.js";
import { n as replaceFileAtomic } from "./replace-file-Dr-OmBmA.js";
...
function normalizeCronStoreFile(parsed) {
	return {
		version: 1,
		jobs: (Array.isArray(parsed) ? parsed : isRecord(parsed) && Array.isArray(parsed.jobs) ? parsed.jobs : []).filter(isRecord)
	};
}
