...
import { n as isPathInside, t as extensionUsesSkippedScannerPath } from "./scan-paths-BipPJnh1.js";
import { t as getGlobalHookRunner } from "./hook-runner-global-j8teApmQ.js";
import { i as scanDirectoryWithSummary } from "./skill-scanner-vlHFmSMz.js";
...
function logTrustedSourceLinkedOfficialInstall(params) {
	params.logger.warn?.(`WARNING: ${params.targetLabel} allowed because it is an official OpenClaw package: ${buildCriticalDetails({ findings: params.findings })}`);
}
function resolveBuiltinScanDecision(params) {
	...
	else if (params.trustedSourceLinkedOfficialInstall && params.builtinScan.critical > 0) logTrustedSourceLinkedOfficialInstall({
		findings: params.builtinScan.findings,
		logger: params.logger,
		targetLabel: params.targetLabel
	});
