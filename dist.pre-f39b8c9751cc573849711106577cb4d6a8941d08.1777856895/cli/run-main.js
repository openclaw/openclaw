...
import { i as normalizeEnv, t as isTruthyEnvValue } from "../env-CNalhgr6.js";
import { n as resolveManifestCommandAliasOwnerInRegistry } from "../manifest-command-aliases-S5HPuXVA.js";
import { t as assertSupportedRuntime } from "../runtime-guard-CIw_BLPB.js";
import { t as ensureOpenClawCliOnPath } from "../path-env-kQdTkCHv.js";
import { a as shouldSkipPluginCommandRegistration, r as shouldRegisterPrimaryCommandOnly, t as isReservedNonPluginCommandRoot } from "../command-registration-policy-DAgDS0dS.js";
import { a as consumeGatewayFastPathRootOptionToken, n as resolveCliNetworkProxyPolicy, o as consumeGatewayRunOptionToken, t as resolveCliCommandPathPolicy } from "../command-path-policy-CIW8d3gX.js";
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { resolveCliStartupPolicy }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-s-7D1keG.js"),
		import("../version-MSU-txfy.js"),
		import("../banner-BlobV7an.js"),
		import("../command-startup-policy-C-3JTjXE.js")
	]));
...
async function closeCliMemoryManagers() {
	try {
		...
		const { closeActiveMemorySearchManagers } = await import("../memory-runtime-Co4KPnW_.js");
...
async function ensureCliEnvProxyDispatcher() {
	try {
		const { hasEnvHttpProxyAgentConfigured } = await import("../proxy-env-DEDLePu-.js");
		if (!hasEnvHttpProxyAgentConfigured()) return;
		const { ensureGlobalUndiciEnvProxyDispatcher } = await import("../undici-global-dispatcher-CUXR4Hhf.js");
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-C59kGKV-.js"), import("../coverage-wqAet9Ps.js")]));
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) await startupTrace.measure("dotenv", async () => {
		const { loadCliDotEnv } = await import("../dotenv-BlhK7cVx.js");
	...
	const stopStartedProxy = async () => {
		...
		proxyHandle = null;
		if (handle) {
			const { stopProxy } = await import("../proxy-lifecycle-CMG3i5f8.js");
	...
	if (shouldStartProxyForCli(normalizedArgv)) {
		const [{ readBestEffortConfig }, { startProxy }] = await Promise.all([import("../io-DSe-kZ6h.js"), import("../proxy-lifecycle-CMG3i5f8.js")]);
		proxyHandle = await startProxy((await readBestEffortConfig())?.proxy ?? void 0);
	}
	let onSigterm = null;
	let onSigint = null;
	let onExit = null;
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("../root-help-metadata-J1F1d8gh.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("../root-help-BFzh5jnF.js");
				await outputRootHelp();
			}
			return;
		}
		if (shouldUseBrowserHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedBrowserHelpText } = await import("../root-help-metadata-J1F1d8gh.js");
		...
		if (shouldRunBareRootCrestodian) {
			...
			const { createCliProgress } = await import("../progress-CNaBvwDy.js");
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-D-OfqHC8.js"));
		...
		try {
			const { enableConsoleCapture } = await import("../logging-CIMynkV-.js");
			...
			const [{ buildProgram }, { formatUncaughtError }, { runFatalErrorHooks }, { installUnhandledRejectionHandler, isBenignUncaughtExceptionError, isUncaughtExceptionHandled }, { restoreTerminalState }] = await startupTrace.measure("core-imports", () => Promise.all([
				import("../program-BPcKF6JV.js"),
				import("../infra/errors.js"),
				import("../fatal-error-hooks-DxqbkWSf.js"),
				import("../unhandled-rejections-D-ysSEgx.js"),
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				const { getProgramContext } = await import("../program-context-G_cn_9eZ.js");
				const ctx = getProgramContext(program);
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-NIXirOuC.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-BHnvVmX4.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				if (config) {
					if (primary && !program.commands.some((command) => command.name() === primary || command.aliases().includes(primary))) {
						const { resolveManifestCommandAliasOwner } = await import("../manifest-command-aliases.runtime-C3S-3qHR.js");
