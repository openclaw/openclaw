...
function stableComparableObject(value, path = []) {
	...
	return Object.fromEntries(Object.entries(value).filter(([key, entry]) => {
		...
		return true;
	}).toSorted(([a], [b]) => a.localeCompare(b)).map(([key, entry]) => [key, stableComparableObject(entry, [...path, key])]));
}
...
function sanitizeDiscordRestPath(path) {
	const [pathname = path] = path.split("?");
	const parts = pathname.replace(/^\/+/, "").split("/");
	if (parts[0] === "interactions" && parts.length >= 4) return `/${[
		"interactions",
		parts[1] || "<unknown>",
		"<token>",
		...parts.slice(3)
	].join("/")}`;
	if (parts[0] === "webhooks" && parts.length >= 3) return `/${[
		"webhooks",
		parts[1] || "<unknown>",
		"<token>",
		...parts.slice(3)
...
function formatDiscordRestAbortMessage(params) {
	...
	return `Discord REST request aborted: method=${method} path=${sanitizeDiscordRestPath(params.path)} route=${method} ${sanitizeDiscordRestPath(params.routeKey.replace(/^\S+\s+/, ""))} timeoutMs=${params.timeoutMs} elapsedMs=${params.elapsedMs} timedOut=${params.timedOut}`;
}
...
//#endregion
export { createThread as $, ChannelSelectMenu as A, removeGuildMember as At, StringSelectMenu as B, GatewayIntentBits as Bt, serializePayload as C, listGuildActiveThreads as Ct, RadioGroup as D, listGuildScheduledEvents as Dt, Modal as E, listGuildRoles as Et, MentionableSelectMenu as F, ButtonStyle as Ft, parseCustomId as G, StickerFormatType as Gt, Thumbnail as H, MessageFlags as Ht, RoleSelectMenu as I, ChannelType as It, getCurrentUser as J, createChannelWebhook as K, TextInputStyle as Kt, Row as L, ComponentType as Lt, File as M, timeoutGuildMember as Mt, LinkButton as N, overwriteApplicationCommands as Nt, TextInput as O, moveGuildChannels as Ot, MediaGallery as P, ApplicationCommandOptionType as Pt, createChannelMessage as Q, Section as R, GatewayCloseCodes as Rt, User as S, getGuildVoiceState as St, Label as T, listGuildEmojis as Tt, UserSelectMenu as U, MessageType as Ut, TextDisplay as V, GatewayOpcodes as Vt, BaseMessageInteractiveComponent as W, PermissionFlagsBits as Wt, deleteOwnMessageReaction as X, createOwnMessageReaction as Y, listMessageReactionUsers as Z, readRetryAfter as _, createGuildScheduledEvent as _t, PresenceUpdateListener as a, getChannelMessage as at, Guild as b, getGuild as bt, ThreadUpdateListener as c, listChannelPins as ct, Plugin as d, sendChannelTyping as dt, deleteChannel as et, RequestClient as f, unpinChannelMessage as ft, readDiscordMessage as g, createGuildEmoji as gt, readDiscordCode as h, createGuildChannel as ht, MessageReactionRemoveListener as i, getChannel as it, Container as j, removeGuildMemberRole as jt, Button as k, putChannelPermission as kt, Embed as l, pinChannelMessage as lt, RateLimitError as m, createGuildBan as mt, MessageCreateListener as n, editChannel as nt, ReadyListener as o, listChannelArchivedThreads as ot, DiscordError as p, addGuildMemberRole as pt, createUserDmChannel as q, require_v10$4 as qt, MessageReactionAddListener as r, editChannelMessage as rt, ResumedListener as s, listChannelMessages as st, InteractionCreateListener as t, deleteChannelMessage as tt, Client as u, searchGuildMessages as ut, Command as v, createGuildSticker as vt, CheckboxGroup as w, listGuildChannels as wt, Message as x, getGuildMember as xt, CommandWithSubcommands as y, deleteChannelPermission as yt, Separator as z, GatewayDispatchEvents as zt };
