...
import { l as jsonResult } from "../../common-O8axt5-B.js";
...
import { t as registerShortTermPromotionDreaming } from "../../dreaming-BDa58kQm.js";
import { i as registerBuiltInMemoryEmbeddingProviders } from "../../provider-adapters-0SzFgjki.js";
...
function loadMemoryToolsModule() {
	memoryToolsModulePromise ??= import("../../tools-CzShGW1S.js");
	return memoryToolsModulePromise;
}
function loadRuntimeProviderModule() {
	runtimeProviderModulePromise ??= import("../../runtime-provider-Bgyg0Oby.js");
	...
	register(api) {
		...
		api.registerMemoryCapability({
			...
			publicArtifacts: { async listArtifacts(params) {
				const { listMemoryCorePublicArtifacts } = await import("../../public-artifacts-CBuXYpci.js");
		...
		api.registerCommand({
			...
			acceptsArgs: true,
			handler: async (ctx) => {
				const { handleDreamingCommand } = await import("../../dreaming-command-DKQ_NsQi.js");
				return await handleDreamingCommand(api, ctx);
			}
		});
		api.registerCli(async ({ program }) => {
			const { registerMemoryCli } = await import("../../cli-CvbZbJDd.js");
