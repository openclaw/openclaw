...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-CPDcWe01.js";
...
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-BA-PIAub.js";
import { r as formatConfigIssueSummary } from "./issue-format-CIl7cU3M.js";
...
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-BTGs1NjO.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-D_w9fvyf.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-9bwbfhUe.js");
...
async function readGatewayStartupConfig(params) {
	...
	if (snapshot?.exists && !snapshot.valid) {
		...
		if (await params.startupTrace.measure("cli.config-recovery", () => recoverConfigFromLastKnownGood({
			...
			try {
				const { writeRestartSentinel } = await import("./restart-sentinel-VdJl2X7G.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-CqkuXOtU.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BtEb4_mo.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-D6H8pxOO.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-CtS2EEgt.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-BZhoXHO_.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-CoTJmyt4.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-e8CifmUa.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-Cq-KfNbZ.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-BnSNypbM.js");
