...
''',
    ...
    def test_codex_provider_routing_requires_a_complete_responses_route(self) -> None:
        routing = argparse.Namespace(
            codex_provider_id="provider.with.dot",
            codex_provider_name="Review Provider",
            codex_provider_base_url="https://provider.example.invalid/v1",
            codex_wire_api="responses",
        )

        self.assertEqual(
            self.helper["codex_provider_routing_overrides"](routing),
            [
                'model_provider="provider.with.dot"',
                'model_providers={"provider.with.dot"={base_url="https://provider.example.invalid/v1", name="Review Provider", wire_api="responses"}}',
            ],
        )
        ...
        for field in (
            ...
            with self.subTest(field=field), self.assertRaisesRegex(
                ...
                self.helper["codex_provider_routing_overrides"](partial)
        ...
        for field, value in (
            ...
            with self.subTest(field=field), self.assertRaisesRegex(
                SystemExit,
                "provider name must be non-empty" if field == "codex_provider_name" else "wire API must be responses",
            ):
                self.helper["codex_provider_routing_overrides"](invalid)

    def test_codex_provider_routing_is_present_in_actual_argv(self) -> None:
        args = argparse.Namespace(
            codex_bin="codex",
            web_search=False,
            model="review-model",
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with mock.patch.dict(
                ...
                {
                    "resolve_command": lambda *_args: "/trusted/codex",
        ...
        routing_flags = [
            ...
            if value == "-c" and command[index + 1].startswith(("model_provider=", "model_providers="))
        ]
        self.assertEqual(
            routing_flags,
            [
