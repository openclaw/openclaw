---
name: mullusi-release-maintainer
description: Maintainer workflow for Mullusi releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# Mullusi Release Maintainer
...
- Core `mullusi` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/mullusi/mullusi/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `mullusi` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `mullusi YYYY.M.D-beta.N`
...
```bash
  MULLUSI_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/mullusi-npm-postpublish-verify.ts <published-version>
```
...
- Always validate the Mullusi npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `mullusi/mullusi` and the real mac preflight in
  `mullusi/releases-private` for every release.
...
- Mullusi publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` is an explicit mode on
  `.github/workflows/mullusi-npm-release.yml`, but it still needs a valid
...
- `.github/workflows/macos-release.yml` in `mullusi/mullusi` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `mullusi/releases-private/.github/workflows/mullusi-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `mullusi/releases-private/.github/workflows/mullusi-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@mullusi/mullusi-release-managers` before publish continues.
- Mac publish uses
  `mullusi/releases-private/.github/workflows/mullusi-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `mullusi/mullusi`
  automatically when `MULLUSI_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for Mullusi releases.
- `@mullusi/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/mullusi-npm-release.yml` with `preflight_only=true`
...
10. Start `.github/workflows/macos-release.yml` in `mullusi/mullusi` and wait
...
14. Start `.github/workflows/mullusi-npm-release.yml` with the same tag for
...
15. Wait for `npm-release` approval from `@mullusi/mullusi-release-managers`.
16. If the stable release was published to `beta`, start
    `.github/workflows/mullusi-npm-release.yml` again after beta validation
...
- Use `mullusi-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
