...
```bash
export MULLUSI_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.mullusi/mullusi.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env MULLUSI_PARALLELS_DISCORD_TOKEN \
...
- Use the `mullusi` wrapper for guest `message send/read`; `node mullusi.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/mullusi-parallels-smoke.*`
...
- host inbound nonce appears in `mullusi message read` output
