...
function fetchGithubApi(args) {
  try {
    return JSON.parse(run("gh", ["api", ...args]).replace(/\u001B\[[0-?]*[ -/]*[@-~]/g, ""));
  } catch (error) {
...
export function cherryPickedFromCommits(message) {
  const markerLines = message
    .split(/\r?\n/u)
    .map((line) => line.trim())
    .filter((line) => /^\(cherry picked from commit\b/iu.test(line));
  const hashes = [];
  for (const markerLine of markerLines) {
    const match = /^\(cherry picked from commit (?<hash>[a-f0-9]{40})\)$/iu.exec(markerLine);
    if (!match?.groups?.hash) {
      fail("commit has invalid cherry-pick provenance");
    }
    hashes.push(match.groups.hash.toLowerCase());
  }
...
function sourceCommits(base, target) {
  ...
  for (const record of output.split("\x1e")) {
    ...
    const provenanceHashes = cherryPickedFromCommits(body);
  ...
  for (const commit of commits.values()) {
    if (commit.isRevert && isActive(commit.hash)) {
      ...
      activeCommits.push({
        ...
        isRevert: true,
        pullRequests: [],
        provenanceHashes: commit.provenanceHashes,
        references: [],
  ...
  const associationHashes = [
    ...new Set(activeCommits.flatMap((commit) => [commit.hash, ...commit.provenanceHashes])),
  ];
  const requiredProvenanceHashes = new Set(
    activeCommits.flatMap((commit) => commit.provenanceHashes),
  );
  const activePullRequests = resolveAssociatedPullRequests(
    associationHashes,
    targetTimestamp,
    requiredProvenanceHashes,
  );
  ...
  for (const commit of activeCommits) {
    const associatedPullRequests = [
      ...new Set(
        [commit.hash, ...commit.provenanceHashes].flatMap(
          (hash) => activePullRequests.get(hash) ?? [],
        ),
      ),
    ];
...
export function githubGraphqlData(response) {
  const errors = Array.isArray(response?.errors)
    ? response.errors.map((error) => error?.message).filter(Boolean)
    : [];
  if (errors.length > 0) {
    throw new Error(`GitHub GraphQL response included errors:\n${errors.join("\n")}`);
  }
  if (response?.data && typeof response.data === "object") {
    return response.data;
  }
  const detail = [response?.message].filter(Boolean).join("\n");
  throw new Error(
    detail
...
function graphql(query) {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    try {
      ...
      return githubGraphqlData(response);
    } catch (error) {
      lastError = error;
...
function resolveAssociatedPullRequests(
  commitHashes,
  targetTimestamp,
  requiredCommitHashes = new Set(),
) {
  ...
  for (let index = 0; index < commitHashes.length; index += commitAssociationQueryBatchSize) {
    ...
    for (let offset = 0; offset < chunk.length; offset += 1) {
      const commitHash = chunk[offset];
      const object = data[`c${index + offset}`]?.object;
      if (!object && requiredCommitHashes.has(commitHash)) {
        fail(`cherry-pick provenance commit ${commitHash} is not in ${repo}`);
      }
      appendPullRequests(commitHash, object?.associatedPullRequests);
    }
  }
...
function main() {
  ...
  const renderedRecord = options.writeLedger
    ? { legacyIssues: new Map(), pullRequests: new Map() }
    : contributionRecordFor(section);
  ...
  const effectiveRenderedRecord = renderedRecord;
  ...
  const candidateChangelog = options.writeLedger
    ? replaceLedger(
        changelog,
        section,
        ledger.ledger,
    ...
    : changelog;
  const candidateSection = options.writeLedger
    ? sectionFor(candidateChangelog, options.version)
    : section;
  ...
  if (options.writeLedger && errors.length === 0) {
    const tempPath = `CHANGELOG.md.${process.pid}.tmp`;
    try {
      writeFileSync(tempPath, candidateChangelog);
      renameSync(tempPath, "CHANGELOG.md");
    } finally {
      rmSync(tempPath, { force: true });
    }
  }
