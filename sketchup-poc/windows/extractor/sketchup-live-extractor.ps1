...
function Get-BootstrapArtifactPaths {
    ...
    $responseArtifactPath = Get-PropertyValue -Object $Artifacts -Name 'responseArtifactPath'
    if (-not (Test-NonEmptyString -Value $responseArtifactPath)) {
        throw 'Bootstrap artifact path shaping requires artifacts.responseArtifactPath.'
    }

    $responseArtifactPath = [string]$responseArtifactPath
    $responseDirectory = Split-Path -Parent $responseArtifactPath
    if (-not (Test-NonEmptyString -Value $responseDirectory)) {
        try {
            $responseDirectory = Split-Path -Parent ([System.IO.Path]::GetFullPath($responseArtifactPath))
        }
        catch {
    ...
    if (-not (Test-NonEmptyString -Value $responseDirectory)) {
        throw ("Could not derive a parent directory from artifacts.responseArtifactPath: {0}" -f $responseArtifactPath)
    }

    $bootstrapDirectory = Join-Path -Path $responseDirectory -ChildPath 'bootstrap'

    return [ordered]@{
        directory = $bootstrapDirectory
        manifestPath = Join-Path -Path $bootstrapDirectory -ChildPath ("{0}.bootstrap-plan.json" -f $RequestId)
        rubyScriptPath = Join-Path -Path $bootstrapDirectory -ChildPath ("{0}.bootstrap.rb" -f $RequestId)
        contextPath = Join-Path -Path $bootstrapDirectory -ChildPath ("{0}.bootstrap-context.json" -f $RequestId)
        bootstrapStatusPath = Join-Path -Path $bootstrapDirectory -ChildPath ("{0}.bootstrap-status.json" -f $RequestId)
    }
}
...
function Get-SketchUpExecutableCandidates {
    ...
    $target = if ($null -ne $Request) { Get-PropertyValue -Object $Request -Name 'target' } else { $null }
    $options = if ($null -ne $Request) { Get-PropertyValue -Object $Request -Name 'options' } else { $null }

    foreach ($candidate in @(
        $(if ($null -ne $target) { Get-PropertyValue -Object $target -Name 'sketchupExecutablePathHint' } else { $null }),
        $(if ($null -ne $options) { Get-PropertyValue -Object $options -Name 'sketchupExePath' } else { $null })
    )) {
    ...
    foreach ($root in @('C:\Program Files\SketchUp')) {
    ...
    return @($candidates | Where-Object { Test-NonEmptyString -Value $_ } | Select-Object -Unique)
}

function Resolve-SketchUpExecutablePath {
    param(
        [AllowNull()]
        [object]$Request
    )

    foreach ($candidate in @(Get-SketchUpExecutableCandidates -Request $Request)) {
...
function New-BootstrapContext {
    ...
    $resolvedSketchUpExecutablePath = Resolve-SketchUpExecutablePath -Request $Request
    ...
    return [ordered]@{
        ...
        target = [ordered]@{
            sketchupExecutablePathHint = if (Test-NonEmptyString -Value $resolvedSketchUpExecutablePath) { $resolvedSketchUpExecutablePath } elseif ($null -ne $target) { Get-PropertyValue -Object $target -Name 'sketchupExecutablePathHint' } else { $null }
...
function New-BootstrapPlan {
    ...
    $sketchupExecutablePath = Resolve-SketchUpExecutablePath -Request $Request
...
function Get-LiveExtractorPreflight {
    ...
    $sketchupExecutableHint = Resolve-SketchUpExecutablePath -Request $Request

    $documentPathExists = $false
    if ($runningOnWindows -and (Test-NonEmptyString -Value $documentPathCandidate)) {
        try {
            $documentPathExists = Test-Path -LiteralPath $documentPathCandidate
        }
        catch {
            $documentPathExists = $false
        }
    ...
    $documentRequired = ($selectedStrategyKey -ne 'ruby-startup-empty-session')
    ...
    if ($documentRequired) {
        ...
        if ($runningOnWindows -and (Test-NonEmptyString -Value $documentPathCandidate) -and -not $documentPathExists) { $environmentIssues.Add('document-path-not-found') }
    }
    ...
    if ($environmentIssues.Count -eq 0) {
        $checks.Add((New-PreflightCheck -Key 'environment-readiness' -Category 'environment' -Status 'pass' -Code 'environment-ready' -Message 'Host and probe-derived target hints are sufficient for a bootstrap attempt.' -Details ([ordered]@{
            ...
            documentPathExists = $documentPathExists
        })))
    }
    else {
...
function Remove-StaleArtifactFile {
    ...
    $stalePath = if (Test-NonEmptyString -Value $directory) {
        Join-Path -Path $directory -ChildPath $staleName
    }
    else {
        $staleName
    }
...
function Invoke-BootstrapAck {
    ...
    if (-not (Test-Path -LiteralPath $executablePath)) {
        throw (New-Object System.Management.Automation.RuntimeException(("SketchUp executable path does not exist on host: {0}" -f $executablePath)))
    }
    if (-not (Test-Path -LiteralPath $rubyScriptPath)) {
        throw (New-Object System.Management.Automation.RuntimeException(("Ruby bootstrap script was not materialized at expected path: {0}" -f $rubyScriptPath)))
    }
    if (Test-NonEmptyString -Value $documentPath -and -not (Test-Path -LiteralPath $documentPath)) {
        throw (New-Object System.Management.Automation.RuntimeException(("Requested document path does not exist on host, aborting before SketchUp launch: {0}" -f $documentPath)))
    }
    ...
    try {
        ...
        if ($_.Exception -is [System.TimeoutException]) {
            $processStateSummary = if ($null -ne $process) {
                try {
                    $process.Refresh()
                    if ($process.HasExited) {
                        "process exited before ack (pid=$($process.Id), exitCode=$($process.ExitCode))"
                    }
                    else {
                        "process still running without ack (pid=$($process.Id))"
                    }
                }
                catch {
            ...
            $documentExists = if (Test-NonEmptyString -Value $documentPath) {
                Test-Path -LiteralPath $documentPath
            }
            else {
                $null
            }
            throw (New-Object System.Management.Automation.RuntimeException(("Timed out waiting for SketchUp bootstrap acknowledgment: {0}. Diagnostics: executableExists={1}; rubyScriptExists={2}; documentPath={3}; documentExists={4}; {5}; bootstrapStatusPath={6}" -f $_.Exception.Message, (Test-Path -LiteralPath $executablePath), (Test-Path -LiteralPath $rubyScriptPath), $documentPath, $documentExists, $processStateSummary, $bootstrapStatusPath)))
        }
        throw
    }
}
