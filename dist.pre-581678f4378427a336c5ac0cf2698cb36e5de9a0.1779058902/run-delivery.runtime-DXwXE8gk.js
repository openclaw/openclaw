...
import { i as isCronSessionKey, u as parseThreadSessionSuffix } from "./session-key-utils-CJRKuBJA.js";
import { u as resolveAgentIdFromSessionKey } from "./session-key-CQewiu8n.js";
import { i as resolveMainSessionKey, n as resolveAgentMainSessionKey, t as canonicalizeMainSessionAlias } from "./main-session-4-6sWu5o.js";
import { h as stringifyRouteThreadId } from "./channel-route-vpaTrtym.js";
import { u as resolveStorePath } from "./paths-789ECJN2.js";
import { o as isAudioFileName } from "./mime-B9SCuRKX.js";
import { s as resolveMirroredTranscriptText } from "./transcript-Co5UP2SS.js";
import { n as sleepWithAbort } from "./backoff-GJF0X7y_.js";
import { a as isSilentReplyText, c as stripLeadingSilentToken, l as stripSilentToken, n as SILENT_REPLY_TOKEN, o as startsWithSilentToken } from "./tokens-2q03qiGF.js";
...
import { o as normalizeTargetForProvider } from "./target-id-resolution-wvjhh0nO.js";
import { s as projectOutboundPayloadPlanForMirror, t as createOutboundPayloadPlan } from "./payloads-DNfEhAOv.js";
import { r as shouldAttemptTtsPayload } from "./tts-config-CDuPQ5rU.js";
import "./inbound.runtime-f5TN1sgb.js";
import { n as pickLastNonEmptyTextFromPayloads, r as pickSummaryFromOutput } from "./helpers-TWkZB_6F.js";
import { _ as resolveDeliveryTarget, l as hasScheduledNextRunAtMs } from "./jobs-PkGF69Ir.js";
import { t as createCronExecutionId } from "./run-id-Kr9U0YsE.js";
import { n as isLikelyInterimCronMessage, t as expectsSubagentFollowup } from "./subagent-followup-hints-DR00ilS4.js";
...
const outboundSessionRuntimeLoader = createLazyImportLoader(() => import("./outbound-session-MTI9XAxC.js"));
...
const ttsRuntimeLoader = createLazyImportLoader(() => import("./tts.runtime-xDjgZxva.js"));
...
function isTtsAudioMirrorOnly(params) {
	return (params.payload.audioAsVoice === true || !!params.payload.hookContent) && isAudioFileName(params.mediaUrl);
}
...
async function resolveDirectCronDeliverySessionKey(params) {
	...
	try {
		...
		await logCronDeliveryWarn(`[cron:${params.job.id}] failed to resolve destination session for direct delivery mirror: ${formatErrorMessage(err)}`);
...
async function dispatchCronDelivery(params) {
	...
	const deliverViaDirect = async (delivery, options) => {
		...
		try {
			...
			const mirrorTargetsAwarenessMainSession = isSameSessionKey(deliverySessionKey, resolveCronAwarenessMainSessionKey({
				cfg: params.cfgWithAgentDefaults,
				agentId: params.agentId
			}));
			...
			const runDelivery = async () => {
				...
				if (send.status === "failed" || !params.deliveryBestEffort && send.status === "partial_failed") throw send.error;
				if (send.status === "partial_failed") hadPartialFailure = true;
			...
			const awarenessText = shouldQueueAwarenessForDelivery ? resolveCronAwarenessText({
				outputText,
				synthesizedText,
				deliveryPayloads: payloadsForDelivery,
				outboundPayloads: attemptedPayloadsForMirror
			}) : void 0;
			const deliveryWillReachAwarenessMainSession = mirrorTargetsAwarenessMainSession && shouldQueueAwarenessForDelivery && !!awarenessText;
			...
			if (delivered && shouldQueueAwarenessForDelivery) await queueCronAwarenessSystemEvent({
				cfg: params.cfgWithAgentDefaults,
				jobId: params.job.id,
				agentId: params.agentId,
