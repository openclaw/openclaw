...
import { L as resolveProviderTransportTurnStateWithPlugin } from "./provider-runtime-CD5virdi.js";
import { r as resolveProviderRequestCapabilities } from "./provider-attribution-D-0WKaTm.js";
import { c as resolveProviderRequestPolicyConfig, i as mergeModelProviderRequestOverrides, n as buildProviderRequestDispatcherPolicy, r as getModelProviderRequestTransport } from "./provider-request-config-CaQqVmIZ.js";
import { c as detectOpenAICompletionsCompat } from "./provider-model-compat-DHhPic5e.js";
...
import { r as resolveDebugProxySettings } from "./env-Chn_Bh31.js";
import { n as ensureModelProviderLocalService } from "./provider-local-service-5IYzT4I9.js";
import { n as repairToolUseResultPairing } from "./session-transcript-repair-DXvalsBA.js";
...
async function requestBodyHasStreamTrue(request, init) {
	...
	let text;
	if (request) text = await request.clone().text().catch(() => void 0);
	else if (typeof init?.body === "string") text = init.body;
...
function buildGuardedModelFetch(model, timeoutMs, options) {
	...
	return async (input, init) => {
		...
		return options?.sanitizeSse === false ? response : sanitizeOpenAISdkSseResponse(response, { synthesizeJsonAsSse });
	};
}
//#endregion
