...
import { a as isSessionArchiveArtifactName, d as parseParentSessionIdFromCheckpointFileName, f as parseSessionArchiveTimestamp, i as isPrimarySessionTranscriptFileName, l as isUsageCountedSessionTranscriptFileName, n as isCheckpointSessionTranscriptFileName, p as parseUsageCountedSessionIdFromFileName } from "./artifacts-CCkMJjUb.js";
import { i as resolveSessionFilePath, l as resolveSessionTranscriptsDirForAgent } from "./paths-D2tVOYHR.js";
import { a as resolveModelCostConfig, n as estimateUsageCost, o as resolveModelCostConfigFingerprint } from "./usage-format-C4AMhu2s.js";
import { t as asFiniteNumber } from "./number-coercion-CADwxRWz.js";
import { o as normalizeUsage } from "./usage-CbtPd3vL.js";
import { _ as stripEnvelope, b as extractToolCallNames, v as stripMessageIdHints, y as countToolResults } from "./session-utils.fs-66nI1FFR.js";
...
async function loadCostUsageSummary(params) {
	...
	const entries = await fs.promises.readdir(sessionsDir, { withFileTypes: true }).catch(() => []);
	const files = (await Promise.all(entries.filter((entry) => entry.isFile() && isUsageCountedSessionTranscriptFileName(entry.name) && !isCheckpointSessionTranscriptFileName(entry.name)).map(async (entry) => {
		const filePath = path.join(sessionsDir, entry.name);
		const stats = await fs.promises.stat(filePath).catch(() => null);
...
async function discoverAllSessions(params) {
	...
	for (const entry of entries) {
		...
		if (isCheckpointSessionTranscriptFileName(entry.name)) {
			...
			if (!existing) discovered.set(parentId, {
				sessionId: parentId,
				sessionFile: filePath,
				mtime: stats.mtimeMs,
				firstUserMessage: void 0
			});
			else if (stats.mtimeMs > existing.mtime) {
				const existingIsPrimary = isPrimarySessionTranscriptFileName(path.basename(existing.sessionFile));
