import { n as isVitestRuntimeEnv } from "./env-6yd24Fv9.js";
...
import { i as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-453t4TpO.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-4Na5zLFq.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-VT8OmhZO.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-MD0V6O1P.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-PF2sOhXc.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-DheJo3Mh.js");
		if (stopped) return;
