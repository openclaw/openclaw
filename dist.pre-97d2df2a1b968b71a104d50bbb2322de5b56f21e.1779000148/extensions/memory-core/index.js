...
import { l as jsonResult } from "../../common-DHPeQHu2.js";
...
import { t as registerShortTermPromotionDreaming } from "../../dreaming-Cah0GSsx.js";
import { i as registerBuiltInMemoryEmbeddingProviders } from "../../provider-adapters-BzeTnEA0.js";
...
function loadMemoryToolsModule() {
	memoryToolsModulePromise ??= import("../../tools-1Bp2woEb.js");
	return memoryToolsModulePromise;
}
function loadRuntimeProviderModule() {
	runtimeProviderModulePromise ??= import("../../runtime-provider-DAYynBAf.js");
	...
	register(api) {
		...
		api.registerMemoryCapability({
			...
			publicArtifacts: { async listArtifacts(params) {
				const { listMemoryCorePublicArtifacts } = await import("../../public-artifacts-DgzVix8_.js");
		...
		api.registerCommand({
			...
			acceptsArgs: true,
			handler: async (ctx) => {
				const { handleDreamingCommand } = await import("../../dreaming-command-cQ9cDsBU.js");
				return await handleDreamingCommand(api, ctx);
			}
		});
		api.registerCli(async ({ program }) => {
			const { registerMemoryCli } = await import("../../cli-3IIQEm3v.js");
