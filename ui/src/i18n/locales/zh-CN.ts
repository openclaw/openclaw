...
export const zh_CN: TranslationMap = {
  ...
  agents: {
    ...
    overview: {
      ...
      inheritDefaultNamed: "继承默认值 ({default})",
      fallbacks: "回退",
      fallbackPlaceholder: "提供商/模型",
    },
    skills: {
      title: "技能",
      ...
      enableAll: "全部启用",
      disableAll: "全部禁用",
      resetTitle: "移除每个代理的允许列表并使用所有技能",
      reset: "重置",
      loadConfigHint: "加载网关配置以设置每个代理的技能。",
      customAllowlist: "此代理使用自定义技能允许列表。",
      allEnabled: "所有技能已启用。禁用任何技能将创建每个代理的允许列表。",
  ...
  agentTools: {
    ...
    liveNowBadge: "实时",
    title: "工具权限",
    subtitle: "此代理的配置文件与单工具覆盖设置。",
    enabledSuffix: "已启用。",
    enableAll: "全部启用",
    disableAll: "全部禁用",
    loadConfigHint: "加载网关配置以调整工具配置文件。",
    agentAllowlistHint: "此代理在配置中使用了显式允许列表。工具覆盖在配置标签页中管理。",
    globalAllowHint: "全局 tools.allow 已设置。代理覆盖无法启用被全局屏蔽的工具。",
    loadingCatalog: "正在加载运行时工具目录…",
    catalogError: "无法加载运行时工具目录。改显示内置回退列表。",
    availableNow: "当前可用",
    availableNowSub: "此代理在当前聊天会话中可以使用的工具。",
    noSession: "无会话",
    switchChatHint: "切换到此代理的聊天以查看其实时运行工具。",
    loadingTools: "正在加载可用工具…",
    toolsError: "无法加载此会话的可用工具。",
    noToolsAvailable: "当前此会话没有可用工具。",
    moreLiveToolsTitle: "下面分组中还有 {count} 个实时工具可用。",
    ...
    detail: {
      ...
      defaultPresets: "默认预设",
      currentSession: "当前会话",
      availableNow: "当前通过 {badge} 可用。",
      notAvailable: "当前在此聊天会话中不可用。",
      switchChat: "切换到此代理的聊天以查看实时可用性。",
      link: "链接到此工具",
    },
  },
  nav: {
