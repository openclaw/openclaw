...
export function renderSessions(widget: DashboardWidget, value: unknown): TemplateResult {
      ...
      ${model.rows.map(
        (row) => html`
          <li class="dashboard-list__row">
            <a class="dashboard-list__link" href=${`${chatPath}${searchForSession(row.key)}`}>
              <span
              ...
              ${row.updatedAt !== null
                ? html`<span class="dashboard-list__meta">${formatDateTimeMs(row.updatedAt)}</span>`
                : nothing}
            </a>
          </li>
        `,
