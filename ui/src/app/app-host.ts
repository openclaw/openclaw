...
class OpenClawShell extends LitElement {
  ...
  override render() {
    ...
    const pluginPageRef =
      activeRoute === "plugin"
        ? pluginPageRefFromSearch(this.routeState.location?.search ?? "")
        : null;
    const activePluginTabId =
      activeRoute === "plugin"
        ? pluginNavigationKeyFromSearch(this.routeState.location?.search ?? "")
        : "";
    const currentLabel =
      pluginPageRef && isPluginEntryPointRef(pluginPageRef) ? pluginPageRef.label : "";
