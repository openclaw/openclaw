...
function sanitizeRestoredSessionSelection(
  ...
  fallback: Pick<ScopedSessionSelection, "sessionKey">,
): ScopedSessionSelection {
  const fallbackSessionKey = fallback.sessionKey || "main";
  ...
  return {
    sessionKey,
    lastActiveSessionKey: lastActiveSessionKey || sessionKey || fallbackSessionKey,
  };
}
