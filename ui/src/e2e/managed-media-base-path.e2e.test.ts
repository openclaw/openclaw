...
describe("Control UI managed media under a UI base path", () => {
  ...
  it("keeps origin-root managed-media APIs outside the UI base path", async () => {
    ...
    const previewMediaPath =
      "/api/chat/media/outgoing/agent%3Amain%3Amain/00000000-0000-4000-8000-000000000001/thumbnail";
    ...
    await page.route("**/api/chat/media/outgoing/**", async (route) => {
      ...
      if (requestPath === previewMediaPath) {
      ...
      historyMessages: [
        {
          ...
          content: [
            ...
            { type: "image", url: sourceMediaPath, alt: "Managed proof image" },
          ],
          timestamp: 1,
        },
        {
    ...
    try {
      ...
      expect(requests).toEqual([{ contentType: "image/png", path: previewMediaPath }]);
