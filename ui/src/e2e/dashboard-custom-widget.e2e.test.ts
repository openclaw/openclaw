...
<script>
  ...
  window.addEventListener("message", (event) => {
    const m = event.data;
    if (!m || m.v !== 1) return;
    if (m.type === "dashboard:data" || m.type === "dashboard:push") {
      valueNode.textContent = JSON.stringify(m.data);
    } else if (m.type === "dashboard:error") {
...
async function routeWidgetAssets(page: Page, html: string): Promise<void> {
  await page.route("**/plugins/dashboard/widgets/revenue-chart/**", (route: Route) => {
...
async function gotoWorkspaces(page: Page, e2eServer: ControlUiE2eServer) {
  const response = await page.goto(`${e2eServer.baseUrl}plugin?plugin=dashboard&id=workspaces`);
...
describeControlUiE2e("Control UI custom-widget host mocked Gateway E2E", () => {
  ...
  it("renders a pending widget as an approval card with NO iframe, then approves it", async () => {
    ...
    try {
      ...
      await page.locator('[data-test-id="dashboard-custom-pending"]').waitFor({ timeout: 10_000 });
      expect(await page.locator("iframe").count()).toBe(0);

      // Approve → dashboard.widget.approve RPC, then a broadcast delivers the
      // approved doc; the iframe host mounts.
      await gateway.deferNext("dashboard.workspace.get");
      await page.locator('[data-test-id="dashboard-custom-approve"]').click();
      await gateway.waitForRequest("dashboard.widget.approve");
      await gateway.emitGatewayEvent("plugin.dashboard.changed", { workspaceVersion: 2 });
      await gateway.resolveDeferred("dashboard.workspace.get", workspaceDoc(2, "approved"));

      const frame = page.locator('[data-test-id="dashboard-custom-widget-frame"]');
  ...
  it("round-trips getData with the fixture widget and blocks fetch() via CSP", async () => {
    ...
    try {
      await gotoWorkspaces(page, server);
      const frameEl = page.locator('[data-test-id="dashboard-custom-widget-frame"]');
      await frameEl.waitFor({ timeout: 10_000 });
      const frame = page.frameLocator('[data-test-id="dashboard-custom-widget-frame"]');
  ...
  it("drops a postMessage from a foreign window (identity accept filter)", async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        window.postMessage(
          { v: 1, type: "dashboard:data", requestId: "x", bindingId: "value", data: "SPOOFED" },
          "*",
        );
      });
  ...
  it("isolates a throwing widget to its own cell (kill test)", async () => {
    ...
    try {
      ...
      await page
        .locator('[data-test-id="dashboard-custom-widget-frame"]')
        .waitFor({ timeout: 10_000 });
      await page.locator('[data-test-id="dashboard-widget"]').waitFor({ timeout: 10_000 });
      // The parent SHELL survives the widget's throw: the dashboard section and its
      ...
      await expect.poll(async () => page.locator('[data-test-id="dashboard"]').count()).toBe(1);
      await expect
        .poll(async () => page.locator('[data-test-id="dashboard-tab"]').count())
        .toBeGreaterThan(0);
      // No error leaked into the PARENT document body (the throw text stays inside
      // the sandboxed frame, never rendered by the shell).
      expect(await page.locator('[data-test-id="dashboard"]').textContent()).not.toContain(
        "widget boom",
      );
    } finally {
      await page.context().close();
