  ...
  it.each(["live", "history"] as const)(
    ...
    async (source) => {
      ...
      const safeDiagnostic =
        displayPrefix + diagnostic.replace("password=synthetic-password", "password=[redacted]");
      const gateway = await installMockGateway(currentPage, {
        sessionKey,
        ...(source === "history"
          ? {
      ...
      await currentPage.goto(controlUiSessionUrl(suite.server.baseUrl, sessionKey));
      if (source === "live") {
        await currentPage.locator(".agent-chat__input textarea").fill("Inspect the project skills");
      ...
      await alert.waitFor();
      await captureMockStopProof(currentPage, `run-error-${source}-collapsed`);
      ...
      await captureMockStopProof(currentPage, `run-error-${source}-expanded`);
      ...
      await expect
        ...
        .toBe(true);
      await captureMockStopProof(currentPage, `run-error-${source}-mobile`);
      await summary.press("Enter");
