...
function formatRecentSessionAccessibleLabel(session: {
  label: string;
  meta: string;
  active: boolean;
  hasActiveRun: boolean;
  ...
  return [
    session.label,
    session.meta,
    session.active ? t("common.active") : "",
...
export class AppSidebar extends LitElement {
  ...
  private getSessionNavigationState() {
    ...
    const toSidebarSession = (row: SessionsListResult["sessions"][number]) => {
      ...
      const active = row.key === navigation.activeRowKey;
      const hasActiveRun = Boolean(row.hasActiveRun);
      ...
      return {
        ...
        accessibleLabel: formatRecentSessionAccessibleLabel({
          label,
          meta,
          active,
          hasActiveRun,
  ...
  private renderRecentSession(session: SidebarRecentSession) {
    ...
    const row = html`
      <div
        ...
        role="listitem"
        aria-label=${session.accessibleLabel}
        draggable="true"
      ...
      >
        <a
          ...
          draggable="false"
          title=${`${session.accessibleLabel} · ${session.key}`}
          aria-label=${session.accessibleLabel}
