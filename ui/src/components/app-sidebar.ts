...
class AppSidebar extends LitElement {
  ...
  private renderPluginUiEntryPoint(entryPoint: PluginControlUiEntryPoint) {
    ...
    const link = html`
      <a
        href=${href}
        class="nav-item nav-item--plugin ${active ? "nav-item--active" : ""}"
        @click=${(event: MouseEvent) => {
          if (!shouldHandleNavigationClick(event)) {
            return;
          }
      ...
      >
        ...
        ${!this.collapsed
          ? html`<span class="nav-item__text">${entryPoint.label}</span>`
          : nothing}
      </a>
    `;
    return this.collapsed
      ? html`<openclaw-tooltip .content=${entryPoint.label}>${link}</openclaw-tooltip>`
      : link;
  }
    ...
    return html`
      <section class="nav-section nav-section--more ${expanded ? "" : "nav-section--collapsed"}">
        ...
        <div class="nav-section__items">
          ...
          ${this.pluginUiEntryPoints.map((entryPoint) =>
            this.renderPluginUiEntryPoint(entryPoint),
          )}
          <button
            type="button"
