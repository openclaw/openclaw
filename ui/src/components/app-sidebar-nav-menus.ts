...
type SidebarMenuNavigationHandlers = {
  ...
  onActivatePluginEntryPoint: (entryPoint: PluginControlUiEntryPoint) => void;
...
type SidebarMoreMenuParams = SidebarMenuNavigationHandlers & {
  ...
  pluginEntryPoints: readonly PluginControlUiEntryPoint[];
...
function renderMoreMenuPluginEntryPoint(
  params: SidebarMoreMenuParams,
  entryPoint: PluginControlUiEntryPoint,
) {
  const ref = {
    entryPoint: true as const,
  ...
  const key = pluginEntryPointKey(ref);
  const active = params.activeRouteId === "plugin" && params.activePluginTabId === key;
  return html`
    <wa-dropdown-item
      value=${key}
      class="sidebar-customize-menu__item ${active ? "sidebar-customize-menu__item--active" : ""}"
    ...
    >
      <a href=${`${pathForRoute("plugin", params.basePath)}${search}`} tabindex="-1">
        ...
        <span class="sidebar-customize-menu__text">${entryPoint.label}</span>
      </a>
    </wa-dropdown-item>
  `;
}
  ...
  return html`
    <openclaw-menu-surface>
      <wa-dropdown
        ...
        @wa-select=${(event: CustomEvent<{ item: HTMLElement & { value?: string } }>) => {
          ...
          if (value?.startsWith("entry:")) {
            const entryPoint = params.pluginEntryPoints.find(
              (candidate) =>
                pluginEntryPointKey({
            ...
            if (entryPoint) {
              params.onActivatePluginEntryPoint(entryPoint);
            }
            return;
          }
      ...
      >
        ...
        ${params.pluginEntryPoints.map((entryPoint) =>
          renderMoreMenuPluginEntryPoint(params, entryPoint),
        )}
