...
describe("sidebar attention refresh ownership", () => {
  ...
  it("finishes an agent auth refresh when a cron event arrives mid-switch", async () => {
    ...
    const responses = {
      ...
      "models.authStatus": [Promise.resolve({ ts: 1, providers: [] }), switchedAuth.promise],
    };
    ...
    switchedAuth.resolve(writerAuth);
    await waitForFast(() => expect(request).toHaveBeenCalledTimes(5));
  ...
  it("projects a lifecycle event burst without another cron.list", async () => {
    ...
    expect(element.cronJobs.every((job) => job.state.lastRunStatus === "ok")).toBe(true);
  ...
  it("coalesces a missing-snapshot event burst into one fallback cron.list", async () => {
    ...
    await waitForFast(() => expect(element.cronJobs.map((job) => job.id)).toEqual(["failed"]));
    ...
    await waitForFast(() => expect(element.cronJobs).toEqual([]));
    await new Promise<void>((resolve) => {
