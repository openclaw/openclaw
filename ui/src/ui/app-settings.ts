...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as DNAApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as DNAApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as DNAApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as DNAApp);
    await loadDevices(host as unknown as DNAApp);
    await loadConfig(host as unknown as DNAApp);
    await loadExecApprovals(host as unknown as DNAApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as DNAApp);
    await loadConfig(host as unknown as DNAApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as DNAApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as DNAApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__DNA_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as DNAApp, false),
    loadPresence(host as unknown as DNAApp),
    loadSessions(host as unknown as DNAApp),
    loadCronStatus(host as unknown as DNAApp),
    loadDebug(host as unknown as DNAApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as DNAApp, true),
    loadConfigSchema(host as unknown as DNAApp),
    loadConfig(host as unknown as DNAApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as DNAApp, false),
    loadCronStatus(host as unknown as DNAApp),
    loadCronJobs(host as unknown as DNAApp),
  ]);
}
