...
export function stripAnsi(value: string): string {
  ...
  // - CSI: ESC [ ... cmd
  // - OSC: ESC ] ... BEL or ST
  // eslint-disable-next-line no-control-regex -- intentional: matching ANSI CSI sequences
  const csi = /[\u001B\u009B][[\]()#;?]*(?:\d{1,4}(?:;\d{0,4})*)?[0-9A-ORZcf-nq-uy=><]/g;
  // eslint-disable-next-line no-control-regex -- intentional: matching ANSI OSC sequences
  const osc = /\u001B\][^\u0007]*(?:\u0007|\u001B\\)/g;
...
export function normalizeTerminalText(value: string): string {
  ...
  // eslint-disable-next-line no-control-regex -- intentional: stripping non-printable control chars
  out = out.replace(/[\u0000-\u0008\u000B-\u001A\u001C-\u001F\u007F]/g, "");
  return out;
}
