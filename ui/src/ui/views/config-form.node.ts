...
export function renderNode(params: {
  ...
  const help = rawHelp ? t(rawHelp) : undefined;
...
function renderArray(params: {
  ...
  const itemLabel = arr.length === 1 ? t("item") : t("items");

  return html`
    <div class="cfg-array">
      <div class="cfg-array__header">
        ${showLabel ? html`<span class="cfg-array__label">${label}</span>` : nothing}
        <span class="cfg-array__count">${arr.length} ${itemLabel}</span>
        <button
          type="button"
      ...
      ${
        arr.length === 0
          ? html`
              <div class="cfg-array__empty">${t('No items yet. Click "Add" to create one.')}</div>
            `
          : html`
  ...
  return html`
    <div class="cfg-map">
      <div class="cfg-map__header">
        <span class="cfg-map__label">${t("Custom entries")}</span>
        <button
          type="button"
          class="cfg-map__add"
          ?disabled=${disabled}
      ...
      ${
        entries.length === 0
          ? html`
              <div class="cfg-map__empty">${t("No custom entries.")}</div>
            `
          : html`
