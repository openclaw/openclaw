  ...
  return html`
    <button
      ...
      title=${state.canReveal
        ? state.isRevealed
          ? t("configFormView.hideValue")
          : t("configFormView.revealValue")
        : t("configFormView.disableStreamReveal")}
...
export function renderNode(params: {
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">${t("configFormView.unsupportedNode")}</div>
    </div>`;
  }
  if (
    criteria &&
  ...
  return html`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${label}</div>
      <div class="cfg-field__error">${t("configFormView.unsupportedType", { type })}</div>
    </div>
  `;
}
...
function renderTextInput(params: {
  ...
  const placeholder = effectiveRedacted
    ? isStructuredSecretRef
      ? rawAvailable
        ? t("configFormView.secretRefRawMode")
        : t("configFormView.secretRefConfigFile")
      : REDACTED_PLACEHOLDER
    : (hint?.placeholder ??
      (schema.default !== undefined
        ? t("configFormView.defaultPlaceholder", { value: formatUnknownText(schema.default) })
        : ""));
        ...
        ${schema.default !== undefined
          ? html`
              <button
                ...
                title=${t("configFormView.resetToDefault")}
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        <option value=${unset}>${t("configFormView.selectPlaceholder")}</option>
...
function renderArray(params: {
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error">${t("configFormView.unsupportedArray")}</div>
      </div>
    `;
  }
    ...
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count"
          >${t(arr.length === 1 ? "configFormView.itemCount" : "configFormView.itemCountPlural", {
        ...
        >
          ...
          ${t("configFormView.add")}
        </button>
      </div>
      ${help ? html`<div class="cfg-array__help">${help}</div>` : nothing}
      ${arr.length === 0
        ? html` <div class="cfg-array__empty">${t("configFormView.noItemsYet")}</div> `
        : html`
                  ...
                  <div class="cfg-array__item">
                    <div class="cfg-array__item-header">
                      ...
                      <button
                        ...
                        title=${t("configFormView.removeItem")}
                        ?disabled=${disabled}
                        @click=${() => {
                          const next = [...arr];
                          next.splice(idx, 1);
  ...
  return html`
    <div class="cfg-map">
      <div class="cfg-map__header">
        <span class="cfg-map__label">${t("configFormView.customEntries")}</span>
        <button
          type="button"
          class="cfg-map__add"
          ?disabled=${disabled}
        ...
        >
          ...
          ${t("configFormView.addEntry")}
        </button>
      </div>

      ${visibleEntries.length === 0
        ? html` <div class="cfg-map__empty">${t("configFormView.noCustomEntries")}</div> `
        : html`
                    ...
                    <div class="cfg-map__item-header">
                      <div class="cfg-map__item-key">
                        <input
                          ...
                          placeholder=${t("configFormView.key")}
                          .value=${key}
                          ?disabled=${disabled}
                          @change=${(e: Event) => {
                      ...
                      <button
                        ...
                        title=${t("configFormView.removeEntry")}
                        ?disabled=${disabled}
                        @click=${() => {
                          const next = { ...value };
                          delete next[key];
