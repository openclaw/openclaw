...
export function renderConfigForm(props: ConfigFormProps) {
  if (!props.schema) {
    return html`
      <div class="muted">${msg("Schema unavailable.", { id: "config.schemaUnavailable" })}</div>
    `;
  }
  ...
  if (schemaType(schema) !== "object" || !schema.properties) {
    return html`
      <div class="callout danger">${msg("Unsupported schema. Use Raw.", { id: "config.schemaUnsupported" })}</div>
    `;
  }
    ...
    return html`
      <div class="config-empty">
        ...
        <div class="config-empty__text">
          ${
            searchQuery
              ? msg('No settings match "{query}"', {
                  id: "config.searchEmpty",
                  args: { query: searchQuery },
                })
              : msg("No settings in this section", { id: "config.sectionEmpty" })
          }
        </div>
      </div>
    `;
      ...
      ${
        subsectionContext
          ...
          : filteredEntries.map(([key, node]) => {
              ...
              const metaLabel = msg(meta.label, { id: `config.section.${key}.label` });
              const metaDesc = meta.description
                ? msg(meta.description, { id: `config.section.${key}.desc` })
                : "";
              ...
              <section class="config-section-card" id="config-section-${key}">
                <div class="config-section-card__header">
                  ...
                  <div class="config-section-card__titles">
                    ...
                    ${
                      metaDesc
                        ? html`<p class="config-section-card__desc">${metaDesc}</p>`
                        : nothing
                    }
                  </div>
                </div>
