...
const SKILL_SOURCE_GROUPS: Array<{ id: string; label: LocalizedLabel; sources: string[] }> = [
  {
    id: "workspace",
    label: ["Workspace Skills", "Kỹ năng workspace"],
    sources: ["openclaw-workspace"],
  },
  {
    id: "built-in",
    label: ["Built-in Skills", "Kỹ năng tích hợp"],
    sources: ["openclaw-bundled"],
  },
  {
    id: "installed",
    label: ["Installed Skills", "Kỹ năng đã cài"],
    sources: ["openclaw-managed"],
  },
  {
    id: "extra",
    label: ["Extra Skills", "Kỹ năng bổ sung"],
...
export function groupSkills(skills: SkillStatusEntry[]): SkillGroup[] {
  ...
  for (const def of SKILL_SOURCE_GROUPS) {
    groups.set(def.id, { id: def.id, label: uiText(...def.label), skills: [] });
  }
