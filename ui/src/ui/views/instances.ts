...
function renderEntry(entry: PresenceEntry) {
  const lastInput =
    entry.lastInputSeconds != null
      ? t("instancesView.secondsAgo", { count: String(entry.lastInputSeconds) })
      : t("common.na");
  const mode = entry.mode ?? t("instancesView.unknownMode");
  const roles = Array.isArray(entry.roles) ? entry.roles.filter(Boolean) : [];
  const scopes = Array.isArray(entry.scopes) ? entry.scopes.filter(Boolean) : [];
  const scopesLabel =
    scopes.length > 0
      ? scopes.length > 3
        ? t("instancesView.scopesCount", { count: String(scopes.length) })
        : t("instancesView.scopesList", { value: scopes.join(", ") })
      : null;
  return html`
    <div class="list-item">
      <div class="list-main">
      ...
      <div class="list-meta">
        ...
        <div class="muted">${t("instancesView.lastInput", { value: lastInput })}</div>
        <div class="muted">${t("instancesView.reason", { value: entry.reason ?? "" })}</div>
      </div>
    </div>
  `;
}
