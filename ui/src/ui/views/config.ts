...
const CATEGORISED_KEYS = new Set(
	SECTION_CATEGORIES.flatMap((c) => c.sections.map((s) => s.key)),
);
...
export function renderConfig(props: ConfigProps) {
	...
	const include = props.includeSections?.length
		? new Set(props.includeSections)
		: null;
	const exclude = props.excludeSections?.length
		? new Set(props.excludeSections)
		: null;
	...
	const requestUpdate =
		props.onRequestUpdate ?? (() => props.onRawChange(props.raw));
	...
	const visibleCategories = SECTION_CATEGORIES.map((cat) => ({
		...
		sections: cat.sections.filter(
			(s) =>
				(includeVirtualSections && VIRTUAL_SECTIONS.has(s.key)) ||
				s.key in schemaProps,
		),
