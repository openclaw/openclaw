...
export function renderLogs(props: LogsProps) {
  ...
  return html`
    <section class="card">
      ...
      ${
        props.file
          ? html`<div class="muted" style="margin-top: 10px;">${t("logsView.file", { path: props.file })}</div>`
          : nothing
      }
      ${
        props.truncated
          ? html`
              <div class="callout" style="margin-top: 10px">${t("logsView.truncated")}</div>
            `
          : nothing
      }
      ${
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${props.onScroll}>
        ${
          filtered.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("logsView.noEntries")}</div>
              `
            : filtered.map(
                (entry) => html`
                <div class="log-row">
