...
export function renderUsage(props: UsageProps) {
	...
	const hourFilteredSessions =
		filters.selectedHours.length > 0
			? dayFilteredSessions.filter((s) =>
					sessionTouchesHours(s, filters.selectedHours),
				)
			: dayFilteredSessions;

	// Filter sessions by query (client-side)
	const queryResult = filterSessionsByQuery(
	...
	const toolOptions = unique(
		data.aggregates?.tools.tools.map((tool) => tool.name) ?? [],
	).slice(0, 12);
	...
	const computeDailyTotals = (days: string[]): UsageTotals => {
		...
		return matchingDays.reduce(
			(acc, day) => addUsageTotals(acc, day),
			createEmptyUsageTotals(),
		);
	};
          ...
          <div class="usage-filter-actions">
            ...
            <button
              ...
              @click=${(e: Event) => {
								...
								e.stopPropagation();
								filterActions.onQueryDraftChange(
									setQueryTokensForKey(filters.queryDraft, key, []),
								);
							}}
          ...
          <div class="usage-controls">
            ...
            <div class="usage-date-range">
              <input
                ...
                @change=${(e: Event) =>
									filterActions.onStartDateChange(
