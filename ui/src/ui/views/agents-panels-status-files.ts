...
export function renderAgentChannels(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(params.context, t("agents.context.subtitle"))}
      <section class="card">
...
export function renderAgentCron(params: {
  ...
  return html`
      ...
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">${t("agents.scheduler.subtitle")}</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t("common.refreshing") : t("common.refresh")}
          </button>
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          ...
          <div class="stat">
            <div class="stat-label">${t("agents.scheduler.jobs")}</div>
    ...
    <section class="card">
      ...
      <div class="card-sub">${t("agents.cronJobs.subtitle")}</div>
      ${
        jobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">${t("agents.scheduler.noJobs")}</div>
            `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${jobs.map(
...
export function renderAgentFiles(params: {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("agents.coreFiles.title")}</div>
          <div class="card-sub">${t("agents.coreFiles.subtitle")}</div>
        </div>
        <button
          class="btn btn--sm"
      ...
      ${
        list
          ? html`<div class="muted mono" style="margin-top: 8px;">${t("agents.coreFiles.workspace", { workspace: list.workspace })}</div>`
          : nothing
      }
      ${
              ...
              <div class="agent-files-grid" style="margin-top: 16px;">
                <div class="agent-files-list">
                  ${
                    files.length === 0
                      ? html`
                          <div class="muted">${t("agents.coreFiles.noFiles")}</div>
                        `
                      : files.map((file) =>
                ...
                <div class="agent-files-editor">
                  ${
                    !activeEntry
                      ? html`
                          <div class="muted">${t("agents.coreFiles.selectFile")}</div>
                        `
                      : html`
                            ...
                            activeEntry.missing
                              ? html`
                                  <div class="callout info" style="margin-top: 10px">
                                    ${t("agents.coreFiles.missingHint")}
                                  </div>
                                `
                              : nothing
                          }
                          <label class="field" style="margin-top: 12px;">
                            <span>${t("agents.coreFiles.content")}</span>
                            <textarea
                              .value=${draft}
                              @input=${(e: Event) =>
  ...
  return html`
    ...
    >
      ...
      ${
        file.missing
          ? html`
              <span class="agent-pill warn">${t("agents.coreFiles.missingBadge")}</span>
            `
          : nothing
      }
    </button>
