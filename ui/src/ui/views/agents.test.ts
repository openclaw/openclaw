...
describe("renderAgents", () => {
  ...
  it("shows cron job ids and running state in the agent cron panel", async () => {
    ...
    const jobRow = Array.from(container.querySelectorAll(".list-item")).find((row) =>
      row.textContent?.includes("Beta wakeup"),
    );
    const rowText = jobRow?.textContent?.replace(/\s+/g, " ").trim();
