...
import { type ChatProps, renderChat } from "./chat.ts";
import { type OverviewProps, renderOverview } from "./overview.ts";
...
function createChatHeaderState(
	...
	const catalog =
		overrides.models ?? createModelCatalog(...DEFAULT_CHAT_MODEL_CATALOG);
	const request = vi.fn(
...
function createOverviewProps(
	overrides: Partial<OverviewProps> = {},
): OverviewProps {
	return {
		connected: false,
		hello: null,
...
describe("chat view", () => {
	...
	it("uses the assistant avatar URL for the welcome state when the identity avatar is only initials", () => {
		...
		const welcomeImage = container.querySelector<HTMLImageElement>(
	...
	it("shows sender labels from sanitized gateway messages instead of generic You", () => {
		...
		const senderLabels = Array.from(
			container.querySelectorAll(".chat-sender-name"),
	...
	it("opens delete confirm on the right for assistant messages", () => {
		...
		expect(confirm).not.toBeNull();
		expect(confirm?.classList.contains("chat-delete-confirm--right")).toBe(
			true,
		);
	});
	...
	it("reloads effective tools after a chat-header model switch for the active tools panel", async () => {
		...
		expect(state.toolsEffectiveResultKey).toBe(
			"main:main:model=openai/gpt-5-mini",
		);
		vi.unstubAllGlobals();
	});
	...
	it("prefers the session label over displayName in the grouped chat session selector", () => {
			...
			sessions: [
				{
					...
					displayName:
						"webchat:g-agent-main-subagent-4f2146de-887b-4176-9abe-91140082959b",
				},
			],
		};
		const container = document.createElement("div");
		...
		const [sessionSelect] = Array.from(
	...
	it("keeps agent-prefixed labels unique when a custom label already matches the prefix", () => {
		...
		expect(
			labels.filter((label) => label === "Deep Chat (alpha) / main"),
		).toHaveLength(1);
