...
function renderFallbackIndicator(
	status: FallbackIndicatorStatus | null | undefined,
) {
	if (!status) {
		return nothing;
	}
		...
		if (
			...
			switch (e.key) {
				...
				case "Tab":
					e.preventDefault();
					selectSlashArg(
						vs.slashMenuArgItems[vs.slashMenuIndex],
						props,
						requestUpdate,
						false,
					);
			...
			switch (e.key) {
				...
				case "Tab":
					...
					tabCompleteSlashCommand(
						vs.slashMenuItems[vs.slashMenuIndex],
						props,
						requestUpdate,
					);
					return;
...
function groupMessages(items: ChatItem[]): Array<ChatItem | MessageGroup> {
	...
	for (const item of items) {
		...
		const senderLabel =
			role.toLowerCase() === "user" ? (normalized.senderLabel ?? null) : null;
...
function buildChatItems(props: ChatProps): Array<ChatItem | MessageGroup> {
	...
	for (let i = historyStart; i < history.length; i++) {
		...
		if (
			vs.searchOpen &&
			vs.searchQuery.trim() &&
			!messageMatchesSearchQuery(msg, vs.searchQuery)
		) {
			continue;
		}
