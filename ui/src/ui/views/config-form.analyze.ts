import { type JsonSchema, pathKey, schemaType } from "./config-form.shared.ts";
...
function normalizeSecretInputUnion(
	...
	const stringIndex = remaining.findIndex(
		(entry) => schemaType(entry) === "string",
	);
	if (stringIndex < 0) {
		return null;
	}
...
function normalizeUnion(
	...
	const secretInput = normalizeSecretInputUnion(
		schema,
		path,
		remaining,
		nullable,
