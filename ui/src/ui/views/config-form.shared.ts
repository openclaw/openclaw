...
export function isSensitiveConfigPath(path: string): boolean {
	...
	const whitelisted = SENSITIVE_KEY_WHITELIST_SUFFIXES.some((suffix) =>
		lowerPath.endsWith(suffix),
	);
	return (
		!whitelisted && SENSITIVE_PATTERNS.some((pattern) => pattern.test(path))
	);
}
...
export function hasSensitiveConfigData(
	...
	if (Array.isArray(value)) {
		return value.some((item, index) =>
			hasSensitiveConfigData(item, [...path, index], hints),
		);
	}
...
export function countSensitiveConfigValues(
	...
	if (Array.isArray(value)) {
		return value.reduce(
			(count, item, index) =>
				count + countSensitiveConfigValues(item, [...path, index], hints),
			0,
		);
	}
	...
	if (value && typeof value === "object") {
		return Object.entries(value as Record<string, unknown>).reduce(
			(count, [childKey, childValue]) =>
				count +
				countSensitiveConfigValues(childValue, [...path, childKey], hints),
			0,
		);
	}
