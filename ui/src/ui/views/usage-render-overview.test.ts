...
describe("renderDailyChartCompact", () => {
  it("keeps daily bars keyboard-selectable", () => {
    const container = document.createElement("div");
    ...
    const bar = container.querySelector<HTMLElement>(".daily-bar-wrapper");
    expect(bar?.getAttribute("role")).toBe("button");
    expect(bar?.getAttribute("tabindex")).toBe("0");
    expect(bar?.getAttribute("aria-pressed")).toBe("true");

    bar?.dispatchEvent(new KeyboardEvent("keydown", { key: "Enter", bubbles: true }));
    bar?.dispatchEvent(new KeyboardEvent("keydown", { key: " ", bubbles: true, shiftKey: true }));

    expect(onSelectDay).toHaveBeenNthCalledWith(1, "2026-02-05", false);
    expect(onSelectDay).toHaveBeenNthCalledWith(2, "2026-02-05", true);
  });
});
