...
function setTestLocation(params: {
	protocol: string;
	host: string;
	pathname: string;
}) {
...
describe("loadSettings default gateway URL derivation", () => {
	...
	it("infers base path from nested pathname when configured base path is not set", async () => {
		...
		expect(loadSettings().gatewayUrl).toBe(
			expectedGatewayUrl("/apps/openclaw"),
		);
	});
	...
	it("ignores and scrubs legacy persisted tokens", async () => {
		...
		const scopedKey =
			"openclaw.control.settings.v1:wss://gateway.example:8443/openclaw";
	...
	it("caps persisted session scopes to the most recent gateways", async () => {
		...
		localStorage.setItem(
			scopedKey,
			JSON.stringify({ sessionsByGateway: staleEntries }),
		);
