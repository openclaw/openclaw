...
const TERMINAL_VISIBILITY_MS = 8_000;
...
export function resolveChatActivityState(evidence: ChatActivityEvidence): ChatActivityState {
  ...
  const sessionRunning = evidence.sessionStatus === "running" && evidence.sessionEndedAt == null;
  const hasVisibleStream =
    typeof evidence.stream === "string" && evidence.stream.trim().length > 0;
  const terminalVisible =
    evidence.lastTerminalAt != null && now - evidence.lastTerminalAt < TERMINAL_VISIBILITY_MS;
  ...
  const terminalSuppressesBusy =
    ...
    (latestNonTerminalActivityAt == null || evidence.lastTerminalAt >= latestNonTerminalActivityAt);
