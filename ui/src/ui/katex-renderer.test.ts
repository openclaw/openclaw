...
describe("findInlineMathEnd", () => {
  ...
  it("returns -1 for $5 and $10 (boundary check prevents closing $)", () => {
    expect(findInlineMathEnd("$5 and $10", 0)).toBe(-1);
  });

  // ── Bug 1 regression: findInlineMathEnd rejects pure numeric math (e.g. $1,2$) as "currency" ──

  it("extracts pure numeric inline math: $1,2$", () => {
    expect(findInlineMathEnd("$1,2$", 0)).toBe(4);
  });

  it("extracts single digit inline math: $3$", () => {
    expect(findInlineMathEnd("$3$", 0)).toBe(2);
  });

  it("extracts double digit inline math: $42$", () => {
    expect(findInlineMathEnd("$42$", 0)).toBe(3);
  });
...
describe("extractMathBlocks", () => {
  ...
  it("does NOT extract $x^2$ inside tilde fence closed by shorter fence", () => {
    const { mathBlocks } = extractMathBlocks("~~~~\n$x^2$\n~~~");
    expect(mathBlocks.size).toBe(0);
  });

  it("does NOT extract $x^2$ inside 4-space indented code block", () => {
    const { mathBlocks, protectedText } = extractMathBlocks("    some code $x^2$ here");
    expect(mathBlocks.size).toBe(0);
    expect(protectedText).toBe("    some code $x^2$ here");
  });

  it("does NOT extract $y$ inside tab-indented code block", () => {
    const { mathBlocks, protectedText } = extractMathBlocks("\tsome code $y$ here");
    expect(mathBlocks.size).toBe(0);
    expect(protectedText).toBe("\tsome code $y$ here");
  });

  it("extracts math after indented code block ends", () => {
    const { mathBlocks, protectedText } = extractMathBlocks("    code $x^2$\noutside $y$ end");
    expect(mathBlocks.size).toBe(1);
    const [[ph, { tex }]] = [...mathBlocks.entries()];
    expect(tex).toBe("y");
    expect(protectedText).toBe(`    code $x^2$\noutside ${ph} end`);
  });

  it("does NOT extract $z$ inside 3-space line (not indented code)", () => {
    const { mathBlocks } = extractMathBlocks("   $z$");
    expect(mathBlocks.size).toBe(1);
  });

  it("skips multiple consecutive indented code lines", () => {
    const { mathBlocks, protectedText } = extractMathBlocks(
      "    line1 $a$\n    line2 $b$\ntext $c$",
    );
    expect(mathBlocks.size).toBe(1);
    const [[ph, { tex }]] = [...mathBlocks.entries()];
    expect(tex).toBe("c");
    expect(protectedText).toBe(`    line1 $a$\n    line2 $b$\ntext ${ph}`);
  });
  ...
  it("extracts $1,2$ and $\\alpha_1$ without cascading misalignment", () => {
    ...
    expect(entries[1][1].tex).toBe("\\alpha_1");
  });
...
// ── Inline math with Markdown formatting boundaries ──
//
// Regression tests: findInlineMathEnd's boundary regex at line 108 lacks
// `*`, `_`, and `~`.  These 11 tests ALL FAIL until the regex is fixed.
//
//   /[\s.,;:!?)\]}"'`-]/
//         ↑ add * _ ~ here
//

describe("inline math with Markdown formatting boundaries", () => {
  ...
  it("extracts $x$ followed by *italic* text without space", () => {
    const { mathBlocks } = extractMathBlocks("$x$*not italic*");
    expect(mathBlocks.size).toBe(1);
  });

  it("extracts $x$ followed by _text_ without space", () => {
    const { mathBlocks } = extractMathBlocks("$x$_text_");
    expect(mathBlocks.size).toBe(1);
  });

  it("extracts $x$ followed by ~text~ without space", () => {
    const { mathBlocks } = extractMathBlocks("$x$~text~");
    expect(mathBlocks.size).toBe(1);
  });

  it("extracts two inline math blocks inside **bold** — second $b$ is followed by **", () => {
    const { mathBlocks } = extractMathBlocks("**$a$ and $b$**");
    expect(mathBlocks.size).toBe(2);
  });
...
const renderToStringMock = vi.fn(
  (
    ...
    if (options?.displayMode) {
      return `<span class="katex-display"><span class="katex"><span class="katex-mathml"><math display="block"><semantics><mrow><mi>${tex}</mi></mrow><annotation encoding="application/x-tex">${tex}</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="mord"><span class="hide-tail" style="height:0.522em;min-width:1.469em;"><svg xmlns="http://www.w3.org/2000/svg" width="400em" height="0.522em" viewBox="0 0 400000 522" preserveAspectRatio="xMaxYMin slice"><path d="M0 241v40h399891c-47.3 35.3-84 78-110 128"></path></svg>${tex}</span></span></span></span></span>`;
    }
    return `<span class="katex"><span class="katex-mathml"><math><semantics><mrow><mi>${tex}</mi></mrow><annotation encoding="application/x-tex">${tex}</annotation></semantics></math></span><span class="katex-html" aria-hidden="true">${tex}</span></span>`;
  },
);
...
describe("toSanitizedMarkdownHtmlWithKatex — KaTeX rendering", () => {
  ...
  // ── Bug 2 regression: DOMPurify strips preserveAspectRatio, aria-hidden, display, encoding ──

  it("preserves preserveAspectRatio attribute on SVG in KaTeX output", async () => {
    await preloadKatex();
    clearMarkdownCache();

    const html = toSanitizedMarkdownHtmlWithKatex("$$\\sqrt{x}$$", {
      mathRendering: "katex",
    });
  ...
  it("preserves aria-hidden attribute on katex-html span", async () => {
    await preloadKatex();
    clearMarkdownCache();

    const html = toSanitizedMarkdownHtmlWithKatex("$x$", {
      mathRendering: "katex",
    });
  ...
  it("preserves display attribute on math element in display mode", async () => {
    await preloadKatex();
    clearMarkdownCache();
  ...
  it("preserves encoding attribute on annotation element", async () => {
    await preloadKatex();
    clearMarkdownCache();
