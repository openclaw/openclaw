...
function emitRetryableTokenMismatch(
	ws: MockWebSocket,
...
async function startRetriedDeviceTokenConnect(params: {
	...
	const { ws: firstWs, connectFrame: firstConnect } =
...
describe("GatewayBrowserClient", () => {
	...
	it("prefers explicit shared auth over cached device tokens", async () => {
		...
		expect(signDevicePayloadMock).toHaveBeenCalledWith(
			"private-key",
			expect.any(String),
		);
	...
	it("retries once with device token after token mismatch when shared token is explicit", async () => {
		...
		expect(
			loadDeviceAuthToken({ deviceId: "device-1", role: "operator" })?.token,
