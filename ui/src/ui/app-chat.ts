...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as DNAApp);
}
...
async function sendChatMessageNow(
  ...
  const ok = await sendChatMessage(host as unknown as DNAApp, message, opts?.attachments);
...
export async function refreshChat(host: ChatHost) {
  await Promise.all([
    loadChatHistory(host as unknown as DNAApp),
    loadSessions(host as unknown as DNAApp, { activeMinutes: 0 }),
