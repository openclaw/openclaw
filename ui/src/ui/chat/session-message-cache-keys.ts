...
export function readChatMessageCacheSessionDefaults(
  ...
  const { hello } = host as { hello?: unknown };
  if (!hello || typeof hello !== "object" || !("snapshot" in hello)) {
    return undefined;
  }
  const { snapshot } = hello as { snapshot?: unknown };
  if (!snapshot || typeof snapshot !== "object" || !("sessionDefaults" in snapshot)) {
    return undefined;
  }
  const sessionDefaults = (snapshot as { sessionDefaults?: unknown }).sessionDefaults;
  if (!sessionDefaults || typeof sessionDefaults !== "object") {
    return undefined;
  }
  const sd = sessionDefaults as Record<string, unknown>;
  return {
    defaultAgentId: typeof sd.defaultAgentId === "string" ? sd.defaultAgentId : undefined,
    mainKey: typeof sd.mainKey === "string" ? sd.mainKey : undefined,
    mainSessionKey: typeof sd.mainSessionKey === "string" ? sd.mainSessionKey : undefined,
  };
}
