...
async function executeThink(
	...
	if (!rawLevel) {
		try {
			const { session, models } = await loadThinkingCommandState(
				client,
				sessionKey,
			);
			return {
...
async function executeUsage(
	...
	try {
		const sessions = await client.request<SessionsListResult>(
			"sessions.list",
			{},
		);
		const session = resolveCurrentSession(sessions, sessionKey);
...
async function executeKill(
	...
	try {
		...
		const matched = resolveKillTargets(
			sessions?.sessions ?? [],
			sessionKey,
			target,
		);
		...
		const results = await Promise.allSettled(
			matched.map((key) =>
				client.request<{ aborted?: boolean }>("chat.abort", {
					sessionKey: key,
				}),
			),
		);
		...
		const successCount = results.filter(
			(entry) =>
				...
				(entry.value as { aborted?: boolean })?.aborted !== false,
		).length;
		if (successCount === 0) {
...
function resolveKillTargets(
	...
	const currentAgentId =
		...
		(normalizedCurrentSessionKey === DEFAULT_MAIN_KEY
			? DEFAULT_AGENT_ID
			: undefined);
...
function isWithinCurrentSessionSubtree(
	...
	const currentAliases = resolveEquivalentSessionKeys(
		currentSessionKey,
		currentAgentId,
	);
	const seen = new Set<string>();
	let parentSessionKey = normalizeSessionKey(
		sessionIndex.get(candidateSessionKey)?.spawnedBy,
	);
	while (parentSessionKey && !seen.has(parentSessionKey)) {
		...
		parentSessionKey = normalizeSessionKey(
			sessionIndex.get(parentSessionKey)?.spawnedBy,
		);
	}
	...
	if (spaceIdx > 0) {
		...
		if (rest && maybeTarget.toLowerCase() !== "all") {
			const sessions =
				...
				(await client.request<SessionsListResult>("sessions.list", {}));
			const matched = resolveSteerSubagent(
				sessions?.sessions ?? [],
				sessionKey,
				maybeTarget,
			);
			...
			if (matched.length > 1) {
				return {
					error: `Multiple sub-agents match \`${maybeTarget}\`. Be more specific.`,
				};
			}
		}
	}
...
async function executeSteer(
	...
	try {
		const resolved = await resolveSteerTarget(
			client,
			sessionKey,
			args,
			context,
