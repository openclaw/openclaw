...
describe("applyConfigSnapshot", () => {
  it("does not clobber form edits while dirty", () => {
    ...
    state.configForm = { gateway: { mode: "local", port: 18790 } };
    state.configRaw = "{\n}\n";
...
describe("updateConfigFormValue", () => {
  it("seeds from snapshot when form is null", () => {
    ...
    updateConfigFormValue(state, ["gateway", "port"], 18790);
...
describe("saveConfig", () => {
  it("coerces schema-typed values before config.set in form mode", async () => {
    ...
    state.configFormMode = "form";
    state.configForm = {
      gateway: { port: "18790", enabled: "false" },
    };
    state.configSchema = {
      type: "object",
      properties: {
