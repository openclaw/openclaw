...
export async function handleWhatsAppStart(host: ActiviApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ActiviApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ActiviApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ActiviApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ActiviApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ActiviApp): string {
...
function resolveGatewayHttpAuthHeader(host: ActiviApp): string | null {
...
function buildGatewayHttpHeaders(host: ActiviApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: ActiviApp) {
...
export function handleNostrProfileToggleAdvanced(host: ActiviApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ActiviApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ActiviApp) {
  const state = host.nostrProfileFormState;
