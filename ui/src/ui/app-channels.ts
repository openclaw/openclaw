...
export async function handleWhatsAppStart(host: MullusiApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: MullusiApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: MullusiApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: MullusiApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: MullusiApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: MullusiApp): string {
...
function resolveGatewayHttpAuthHeader(host: MullusiApp): string | null {
...
function buildGatewayHttpHeaders(host: MullusiApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: MullusiApp) {
...
export function handleNostrProfileToggleAdvanced(host: MullusiApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: MullusiApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: MullusiApp) {
  const state = host.nostrProfileFormState;
