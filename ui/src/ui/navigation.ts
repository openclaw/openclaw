 ...
 */
export function getVisibleTabsWithOverrides(mode: AppMode, overrides: TabVisibility): readonly Tab[] {
  if (mode === "configure") {
  ...
  const filteredTabs = baseSet.filter((tab) => {
  ...
  return TAB_GROUPS.map((group) => ({
    ...group,
    tabs: group.tabs.filter((tab) => filteredTabs.includes(tab)),
  })).filter((group) => group.tabs.length > 0);
}
