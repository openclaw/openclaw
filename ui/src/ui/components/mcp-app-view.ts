...
export class McpAppView extends LitElement {
  ...
  @property({ attribute: false }) mcpAppToolName = "";
  @property({ attribute: false }) mcpUiResourceUri = "";
  @property({ attribute: false }) mcpViewUrl = "";
  ...
  private _iframeMount = createRef<HTMLDivElement>();
  ...
  private async _setupBridge() {
    ...
    try {
      const container = this._iframeMount.value;
      if (!container) {
        return;
      }
      ...
      if (this.mcpServerName && this._appViewParams()) {
        bridge.oncalltool = async (params) => {
          return await this._requestMcpProxy("mcp.callTool", {
        ...
        bridge.setRequestHandler(ListToolsRequestSchema, async (request) => {
          return await this._requestMcpProxy("mcp.listTools", {
            ...(request.params?.cursor ? { cursor: request.params.cursor } : {}),
          });
        });

        bridge.onlistresources = async (params) => {
          return await this._requestMcpProxy("mcp.listResources", {
            ...(params?.cursor ? { cursor: params.cursor } : {}),
          });
        };

        bridge.onlistresourcetemplates = async (params) => {
          return await this._requestMcpProxy("mcp.listResourceTemplates", {
            ...(params?.cursor ? { cursor: params.cursor } : {}),
          });
        };

        bridge.onreadresource = async (params) => {
          return await this._requestMcpProxy("mcp.readResource", {
            uri: params.uri,
          });
        };
      }
      ...
      iframe.setAttribute("csp", "navigate-to 'none'; form-action 'none'");
      ...
      const loadPromise = new Promise<void>((resolve, reject) => {
        ...
        iframe.addEventListener("load", () => {
          if (initialLoadSeen) {
            this._closeAfterNavigation(iframe);
            return;
          }
  ...
  private _closeAfterNavigation(iframe: HTMLIFrameElement) {
    if (this._disposed || iframe !== this._iframe) {
      return;
    }
    this._teardown();
    this._disposed = false;
    this._error = "MCP app iframe navigated";
  }
  ...
  private _appViewParams(): { appToolName: string; uiResourceUri: string; viewUrl: string } | null {
    const viewUrl = this.mcpViewUrl || this.src;
    return this.mcpAppToolName && this.mcpUiResourceUri && viewUrl
      ? {
          appToolName: this.mcpAppToolName,
          uiResourceUri: this.mcpUiResourceUri,
          viewUrl,
        }
      : null;
  }

  private async _requestMcpProxy(
    method: string,
    params: Record<string, unknown>,
  ): Promise<unknown> {
    const ctx = getMcpAppContext();
    const sessionKey = this.mcpSessionKey || ctx.sessionKey || "";
    const appViewParams = this._appViewParams();
    if (!ctx.client || !sessionKey || !this.mcpServerName || !appViewParams) {
      throw new Error("MCP app gateway unavailable");
    }
    return await ctx.client.request(method, {
      sessionKey,
  ...
  override render() {
    return html`
      <div class="mcp-app-container">
        <div ${ref(this._iframeMount)} class="mcp-app-frame-mount"></div>
        ${this._error
