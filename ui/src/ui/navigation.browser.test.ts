...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/EasyHub/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/EasyHub");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/EasyHub/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__EASYHUB_CONTROL_UI_BASE_PATH__ = "/EasyHub";
    const app = mountApp("/EasyHub/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/EasyHub");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/EasyHub/sessions");
  });
