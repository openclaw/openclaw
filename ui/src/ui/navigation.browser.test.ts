...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/activi/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/activi");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/activi/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ACTIVI_CONTROL_UI_BASE_PATH__ = "/activi";
    const app = mountApp("/activi/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/activi");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/activi/sessions");
  });
