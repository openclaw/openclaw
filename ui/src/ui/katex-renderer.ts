...
export function extractMathBlocks(markdown: string): {
  ...
  while (i < len) {
    // Indented code blocks: CommonMark treats lines starting with 4+ spaces
    // (or 1+ tab) as indented code. Skip these lines entirely so that
    // $...$ inside indented code is not extracted as math.
    if (
      (i === 0 || markdown[i - 1] === "\n") &&
      (markdown[i] === "\t" ||
        (markdown[i] === " " &&
      ...
      result.push(markdown.slice(lineStart, i));
      continue;
    }
