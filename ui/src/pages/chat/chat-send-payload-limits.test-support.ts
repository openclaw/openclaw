...
import { GatewayPayloadLimitError } from "../../api/gateway.ts";
...
import { makeChatHost, requireRecord } from "./chat-host.test-support.js";
...
type ChatSendPayloadLimitTestDeps = {
  row: (key: string, overrides?: Partial<GatewaySessionRow>) => GatewaySessionRow;
  idleChatHistory: (sessionKey?: string) => unknown;
};

export function registerChatSendPayloadLimitTests(deps: ChatSendPayloadLimitTestDeps): void {
  ...
  it("defers a recipient-only queue replacement made during the browser input yield", async () => {
    ...
    const host = makeChatHost({
      ...
      requestHandlers: {
        "chat.history": () => deps.idleChatHistory(),
          ...
          return Promise.resolve({
            messages:
              historyRequests > 2
                ...
                : [],
            sessionInfo: deps.row("agent:main", { hasActiveRun: false, status: "done" }),
          });
        },
        "chat.send": (params: unknown) => {
