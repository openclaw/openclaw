...
describe("task detail panel", () => {
  it.each([false, true])(
    "applies the remote image policy to task replies (recovered: %s)",
    async (recovered) => {
      const markdown =
        "![Allowed](https://images.example.test/allowed.png) ![Other](https://other.example.test/blocked.png)";
      const task: TaskSummary = {
        ...
        runtime: "subagent",
        agentId: "worker",
        childSessionKey: "agent:worker:subagent:images",
        title: "Image reply",
      };
        ...
        client: createGatewayBrowserClientFixture({
          request: vi.fn().mockResolvedValue({
            messages: [
              {
                role: "assistant",
                content: recovered ? "Preview" : markdown,
                ...(recovered
                  ? { __openclaw: { id: "image-reply", truncated: true, reason: "display-cap" } }
                  : {}),
              },
            ],
          }),
      ...
      const loader = vi
        .fn()
        .mockResolvedValue({ ok: true, message: { role: "assistant", content: markdown } });
      const container = document.body.appendChild(document.createElement("div"));
      const rerender = (remoteImageOrigins?: readonly string[]) =>
        render(
      ...
      if (recovered) {
        await vi.waitFor(() =>
          expect(host.taskDetailState?.fullMessages.get("image-reply")?.status).toBe("loaded"),
        );
        rerender();
      }
      expect(container.querySelector("img")).toBeNull();
      rerender(["https://images.example.test"]);
      expect(
        [...container.querySelectorAll("img")].map((image) => image.getAttribute("src")),
      ).toEqual(["https://images.example.test/allowed.png"]);
      expect(
        container.querySelector('a[href="https://other.example.test/blocked.png"]'),
      ).not.toBeNull();
      rerender([]);
      expect(container.querySelector("img")).toBeNull();
    },
