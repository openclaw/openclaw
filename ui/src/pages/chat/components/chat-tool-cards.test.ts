...
describe("tool-cards", () => {
  ...
  it("validates widget prompt text", () => {
    expect(resolveWidgetPromptText("  Run the tests  ")).toBe("Run the tests");
    ...
    expect(resolveWidgetPromptText("/approve")).toBeNull();
    expect(resolveWidgetPromptText("  /stop")).toBeNull();
    expect(resolveWidgetPromptText("x".repeat(WIDGET_PROMPT_MAX_CHARS + 1))).toBeNull();
    expect(resolveWidgetPromptText("x".repeat(WIDGET_PROMPT_MAX_CHARS))).not.toBeNull();
  });

  it("rate limits widget prompts per widget document", () => {
    const src = `/__openclaw__/canvas/documents/cv_rate_${Math.random()}/index.html`;
    const start = 1_000_000;
    for (let index = 0; index < 10; index += 1) {
      expect(allowWidgetPrompt(src, start + index)).toBe(true);
    }
    expect(allowWidgetPrompt(src, start + 10)).toBe(false);
    // The rolling window frees budget once earlier sends age out.
    expect(allowWidgetPrompt(src, start + 61_000)).toBe(true);
  });
  ...
  it("adopts the bridge's prompt port offer and forwards validated prompts", async () => {
    ...
    // jsdom has no layout and cannot focus iframes; emulate a visible frame
    // the user clicked into, which is what the host checks require.
    ...
    try {
      port.postMessage({ type: "openclaw:widget-prompt", prompt: "  Show details  " });
      await flushPorts();
      expect(received).toEqual(["Show details"]);
      // Slash commands never reach the send path.
      port.postMessage({ type: "openclaw:widget-prompt", prompt: "/approve" });
      await flushPorts();
      expect(received).toEqual(["Show details"]);
      ...
      frame.dispatchEvent(new Event("load"));
      lateOfferPort.postMessage({ type: "openclaw:widget-prompt", prompt: "From takeover" });
      await flushPorts();
      expect(received).toEqual(["Show details"]);
      // Without focus on the frame there is no user-activation signal; drop.
      delete (document as unknown as Record<string, unknown>).activeElement;
      port.postMessage({ type: "openclaw:widget-prompt", prompt: "Auto send" });
      await flushPorts();
      expect(received).toEqual(["Show details"]);
    } finally {
      delete (document as unknown as Record<string, unknown>).activeElement;
  ...
  it("adopts a prompt offer that arrives after the frame's load event", async () => {
    ...
    try {
      port.postMessage({ type: "openclaw:widget-prompt", prompt: "Late but valid" });
      await flushPorts();
  ...
  it("never adopts prompt offers from externally allowed embed URLs", async () => {
    ...
    try {
      port.postMessage({ type: "openclaw:widget-prompt", prompt: "External send" });
      await flushPorts();
