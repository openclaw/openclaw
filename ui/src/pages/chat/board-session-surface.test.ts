...
import { MockBoardProvider } from "../../lib/board/provider.ts";
...
describe("board session shell", () => {
  it("derives divider position from adjacent board surface panels", () => {
    expect(boardDividerRatio(420, 580)).toBe(0.42);
    expect(boardDividerRatio(580, 420, "left")).toBe(0.42);
    expect(boardDividerRatio(0, 0)).toBeNull();
  });
  ...
  it("shows the face toggle only when a board exists", () => {
    ...
    expect(withoutBoard.querySelector("[role=tablist]")).toBeNull();
    expect(withBoard.querySelectorAll("[role=tab]")).toHaveLength(2);
  });
  ...
  it("supports keyboard face selection", () => {
    ...
    container
      .querySelector<HTMLElement>('[data-board-face="chat"]')
