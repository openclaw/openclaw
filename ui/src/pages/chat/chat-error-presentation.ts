...
export function readTranscriptRunError(message: unknown): string | null {
  ...
  // Contention retains its recoverable Check status notice, not a terminal error card.
  if (customFailure && asNullableRecord(record.details)?.errorKind === "state_contention") {
    return null;
  }
...
export function hasTranscriptRunError(messages: readonly unknown[], error: ChatRunError): boolean {
  if (error.kind === "state_contention" || !error.runId) {
    return false;
  }
