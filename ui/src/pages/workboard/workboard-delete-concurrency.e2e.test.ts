...
suite.define(() => {
  it("keeps dependency cleanup while a child card move is pending", async () => {
    ...
    try {
      ...
      await cardInColumn(recorded.page, "todo", child.title).click();
      const dependencyDetails = recorded.page.locator(".workboard-detail__dependencies");
      await expect.poll(() => dependencyDetails.count()).toBe(1);
      await capture(recorded.page, artifacts, "01-before-delete");
      await recorded.page.locator(".workboard-detail__close").click();
      ...
      const parentCard = cardInColumn(recorded.page, "done", parent.title);
      await parentCard.getByRole("button", { name: "Card actions" }).click({ force: true });
      ...
      await cardInColumn(recorded.page, "todo", child.title).click();
      await expect.poll(() => dependencyDetails.count()).toBe(0);
      await recorded.page.locator(".workboard-detail__close").click();
      ...
      await cardInColumn(recorded.page, "todo", child.title)
        ...
        .selectOption("blocked", { force: true });
      ...
      await cardInColumn(recorded.page, "blocked", child.title).click();
      await expect.poll(() => dependencyDetails.count()).toBe(0);
