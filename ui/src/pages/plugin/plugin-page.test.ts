...
describe("PluginPage", () => {
  ...
  it("launches an in-app entry point through the scoped gateway RPC", async () => {
    ...
    page.pluginId = "notes";
    page.tabId = "notes";
    page.entryPointPath = "/plugins/notes";
    ...
    try {
      await vi.waitFor(() =>
        expect(request).toHaveBeenCalledWith("plugins.uiEntryPointLaunch", {
          id: "notes",
          pluginId: "notes",
          path: "/plugins/notes",
          sessionKey: "agent:main:main",
  ...
  it("fails closed when an in-app entry-point launch is rejected", async () => {
    ...
    (page as unknown as { context: ApplicationContext<RouteId> }).context = {
      gateway: { snapshot, subscribe: () => () => undefined },
      config: { current: { embedSandboxMode: "strict" } },
      sessions: { state: { result: { sessions: [], defaults: {} } } },
    } as unknown as ApplicationContext<RouteId>;
    ...
    try {
      ...
      expect(page.innerHTML).not.toContain('src="/plugins/notes"');
    } finally {
      page.remove();
    }
  });
