...
function pairingPendingKind(
  requestId: string,
  props: ChannelsProps,
): ChannelsProps["channels"]["pairingBusyKind"] {
  return props.channels.pairingBusyRequestId === requestId ? props.channels.pairingBusyKind : null;
}

function pairingConfirmLabel(
  approving: boolean,
  pendingKind: ChannelsProps["channels"]["pairingBusyKind"],
): string {
...
export function renderChannelPairingPrompt(props: ChannelsProps) {
  ...
  const pendingKind = busy ? (props.channels.pairingBusyKind ?? prompt.kind) : null;
