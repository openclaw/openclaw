...
server.tool(
  ...
  {
    variantIds: z.array(z.string()).describe("Array of variant IDs to retrieve"),
  },
  async ({ variantIds }) => {
    const client = new ShopifyClient();
    try {
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: success ? "Successfully tagged customer" : "Failed to tag customer",
          },
        ],
      };
    } catch (error) {
...
server.tool(
  ...
  async ({ first, after, query, sortKey, reverse }) => {
    const client = new ShopifyClient();
    try {
      const response = await client.loadOrders(SHOPIFY_ACCESS_TOKEN, MYSHOPIFY_DOMAIN, {
        first,
        after,
        query,
        sortKey,
...
server.tool(
  ...
  async ({ orderId }) => {
    const client = new ShopifyClient();
    try {
      const order = await client.loadOrder(SHOPIFY_ACCESS_TOKEN, MYSHOPIFY_DOMAIN, { orderId });
      return {
...
server.tool(
  ...
  "Create a basic discount code",
  {
    ...
    valueType: z.enum(["percentage", "fixed_amount"]).describe("Type of discount"),
  ...
  async ({ title, code, valueType, value, startsAt, endsAt, appliesOncePerCustomer }) => {
    const client = new ShopifyClient();
    try {
...
server.tool(
  ...
  async ({ draftOrderId, variantId }) => {
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify(completedOrder, null, 2) }],
      };
    } catch (error) {
...
server.tool(
  ...
  async ({ limit, name }) => {
    const client = new ShopifyClient();
    try {
      const collections = await client.loadCollections(SHOPIFY_ACCESS_TOKEN, MYSHOPIFY_DOMAIN, {
        limit,
        name,
      });
      return {
...
server.tool(
  ...
  async () => {
    const client = new ShopifyClient();
    try {
      const shopDetails = await client.loadShopDetail(SHOPIFY_ACCESS_TOKEN, MYSHOPIFY_DOMAIN);
      return {
...
server.tool(
  ...
  {
    action: z.enum(["subscribe", "find", "unsubscribe"]).describe("Action to perform with webhook"),
    callbackUrl: z.string().url().describe("Webhook callback URL"),
    topic: z.nativeEnum(ShopifyWebhookTopic).describe("Webhook topic to subscribe to"),
    webhookId: z.string().optional().describe("Webhook ID (required for unsubscribe)"),
  },
    ...
    try {
      switch (action) {
        ...
        case "unsubscribe": {
          ...
          await client.unsubscribeWebhook(SHOPIFY_ACCESS_TOKEN, MYSHOPIFY_DOMAIN, webhookId);
          return {
            content: [{ type: "text", text: "Webhook unsubscribed successfully" }],
          };
        }
      }
    } catch (error) {
...
server.tool("get-navigation-menus", "Get all navigation menus", {}, async () => {
  const client = new ShopifyClient();
  try {
...
server.tool("get-themes", "Get all themes in the store", {}, async () => {
  const client = new ShopifyClient();
  try {
...
server.tool(
  ...
  async ({ themeId, name }) => {
    const client = new ShopifyClient();
    try {
      const theme = await client.duplicateTheme(
        SHOPIFY_ACCESS_TOKEN,
        MYSHOPIFY_DOMAIN,
        themeId,
        name,
...
server.tool(
  ...
  async ({ themeId, assetKey }) => {
    const client = new ShopifyClient();
    try {
      const asset = await client.getThemeAsset(
        SHOPIFY_ACCESS_TOKEN,
        MYSHOPIFY_DOMAIN,
        themeId,
        assetKey,
...
server.tool(
  ...
  async ({ themeId, assetKey, value, attachment }) => {
    ...
    try {
      ...
      const asset = await client.updateThemeAsset(
        SHOPIFY_ACCESS_TOKEN,
        MYSHOPIFY_DOMAIN,
        assetData,
      );
...
server.tool(
  ...
  async ({ themeId, settings }) => {
    const client = new ShopifyClient();
    try {
      const updatedSettings = await client.updateThemeSettings(
        SHOPIFY_ACCESS_TOKEN,
        MYSHOPIFY_DOMAIN,
        themeId,
        settings,
