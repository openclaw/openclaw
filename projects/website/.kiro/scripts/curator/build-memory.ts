 ...
 * 4. 記錄資料來源位置
 * 5. 計算統計資料
 * 6. 儲存到 .kiro/personas/curator/memory.json
 */
 ...
 * 提取課程的 Highlight 圖片
 */
function extractHighlights(product: any): Highlight[] {
  return Array.from({ length: 6 }).map((_, i) => {
    const n = i + 1;
    const title = product[`content_highlight${n}`] || null;
    const description = product[`content_highlight${n}_description`] || null;
    const image = product[`content_highlight${n}_image`] || product.image || null;
  ...
  }).filter(h => h.title !== null); // 只保留有標題的
}

/**
 * 建立視覺記憶
 */
function buildVisualMemory(courses: CourseMemory[]) {
  ...
  courses.forEach(course => {
    ...
    if (course.images.main_image) {
      allImages.push(course.images.main_image);
      courseImages.push('main_image');
    }

    if (course.images.content_video) {
      allImages.push(course.images.content_video);
      courseImages.push('content_video');
    }

    course.images.highlights.forEach((h, idx) => {
      if (h.image) {
        allImages.push(h.image);
...
async function buildCuratorMemory() {
  ...
  for (const p of products) {
    try {
      ...
      coursesWithDetails.push({
        ...
        images: {
          main_image: details.image || null,
          content_video: details.content_video || null,
  ...
  const visualMemory = buildVisualMemory(coursesWithDetails);
...
(async () => {
  ...
  console.log('🎉 完成！Curator 現在可以回答關於課程的問題了。');
})().catch((error) => {
