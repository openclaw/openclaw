...
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-19aCWWFt.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-kCxN6WmW.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-Bo0VX9hs.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-CKquv7eZ.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-3ER4nbOL.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-ieA37-v9.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-BO85Mr8D.js"), import("./setup-registry-CUNM9iFY.js")]);
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-LUr78Egh.js");
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-CP6lQRPv.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-Bm8n4G83.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-3Hp-vwdR.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-3Hp-vwdR.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-fgVB3beR.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-CsZyzZNw.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-34UMN9Ob.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-BoIFkc9h.js");
		const { setupChannels } = await import("./onboard-channels-ctrIXMx6.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BX7sfXsA.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-CGgWMAH3.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-BSk2e8YL.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-DSZNt80B.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-DgCPnJss.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-MWBOHsNM.js");
