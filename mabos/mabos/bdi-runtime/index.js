...
export function createBdiService(opts) {
    ...
    async function tick() {
        ...
        if (totalPruned > 0 || totalPrioritized > 0) {
            opts.logger.info(`[mabos-bdi] Cycle complete: ${agents.length} agents, ${totalPruned} intentions pruned, ${totalPrioritized} desires re-sorted`);
        }
    }
    return {
        id: "mabos-bdi-heartbeat",
        start: async () => {
            opts.logger.info(`[mabos-bdi] BDI heartbeat started (interval: ${opts.intervalMinutes}min, workspace: ${opts.workspaceDir})`);
