export async function createInvoice(pg, params) {
    const lineItems = params.line_items ?? [];
    const amount = lineItems.reduce((sum, item) => sum + (item.quantity ?? 1) * (item.unit_price ?? 0), 0);
    const result = await pg.query(`INSERT INTO erp.invoices (id, customer_id, status, amount, currency, due_date, line_items)
...
export async function getInvoice(pg, id) {
    const result = await pg.query("SELECT * FROM erp.invoices WHERE id = $1", [id]);
    return result.rows[0] ?? null;
}
export async function listInvoices(pg, params) {
    const conditions = [];
    const values = [];
    let idx = 1;
    if (params.status) {
    ...
    const result = await pg.query(`SELECT i.*, c.name AS customer_name
     FROM erp.invoices i
     LEFT JOIN erp.contacts c ON i.customer_id = c.id
     ${where} ORDER BY i.created_at DESC LIMIT $${idx}`, [...values, params.limit ?? 50]);
    return result.rows;
}
export async function recordPayment(pg, params) {
    const result = await pg.query(`INSERT INTO erp.payments (id, invoice_id, amount, method, status, processed_at)
     VALUES (gen_random_uuid(), $1, $2, $3, 'completed', now()) RETURNING *`, [params.invoice_id, params.amount, params.method ?? null]);
    // Update invoice status if fully paid
    await pg.query(`UPDATE erp.invoices SET status = CASE
       WHEN (SELECT COALESCE(SUM(amount), 0) FROM erp.payments WHERE invoice_id = $1 AND status = 'completed') >= amount THEN 'paid'
       ELSE 'partial'
     END WHERE id = $1`, [params.invoice_id]);
    return result.rows[0];
}
export async function postLedgerEntry(pg, params) {
    // Simple single-entry if account_id provided
    if (params.account_id) {
        const result = await pg.query(`INSERT INTO erp.ledger_entries (id, account_id, debit, credit, description, reference_type, reference_id)
    ...
    if (params.debit_account) {
        await pg.query(`INSERT INTO erp.ledger_entries (id, account_id, debit, credit, description, reference_type, reference_id)
...
export async function getAccountBalance(pg, accountId) {
    const result = await pg.query("SELECT * FROM erp.accounts WHERE id = $1", [accountId]);
    return result.rows[0] ?? null;
}
export async function profitLoss(pg, from, to) {
    const revenue = await pg.query("SELECT COALESCE(SUM(credit - debit), 0) as total FROM erp.ledger_entries WHERE posted_at >= $1 AND posted_at <= $2", [from, to]);
    return { from, to, net: revenue.rows[0]?.total ?? 0 };
}
export async function listAccounts(pg, params) {
    const conditions = [];
    const values = [];
    let idx = 1;
    if (params.type) {
    ...
    const where = conditions.length > 0 ? `WHERE ${conditions.join(" AND ")}` : "";
    const result = await pg.query(`SELECT * FROM erp.accounts ${where} ORDER BY name ASC LIMIT $${idx}`, [...values, params.limit ?? 50]);
    return result.rows;
}
export async function balanceSheet(pg) {
    const result = await pg.query(`SELECT id, name, type, currency, balance
     FROM erp.accounts
...
export async function cashFlowStatement(pg, from, to) {
    const cashId = "e0000001-0001-4000-8000-000000000006";
    const result = await pg.query(`SELECT debit, credit, description, reference_type, posted_at
     FROM erp.ledger_entries
     WHERE account_id = $1 AND posted_at >= $2 AND posted_at <= $3
     ORDER BY posted_at`, [cashId, from, to]);
    const sections = {
        operating: [],
        investing: [],
        financing: [],
...
export async function expenseReport(pg, from, to) {
    const result = await pg.query(`SELECT le.debit, le.description, le.posted_at, a.name AS category
     ...
     ORDER BY a.name, le.posted_at`, [from, to]);
...
export async function budgetVsActual(pg, from, to) {
    const result = await pg.query(`SELECT b.id, a.name AS account_name, a.type AS account_type,
            b.budgeted_amount,
            COALESCE(SUM(
     ...
     ORDER BY a.type DESC, a.name`, [from, to]);
...
export async function createAccount(pg, params) {
    const result = await pg.query(`INSERT INTO erp.accounts (id, name, type, currency, parent_id)
     VALUES (gen_random_uuid(), $1, $2, $3, $4) RETURNING *`, [params.name, params.type, params.currency ?? "USD", params.parent_id ?? null]);
    return result.rows[0];
}
