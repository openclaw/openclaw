...
const actions: ErpAction[] = [
  {
    ...
    handler: async (params, ctx) => {
      const employee = await q.createEmployee(ctx.pg, params);
  ...
  },
  {
    ...
    params: {},
    handler: async (params, ctx) => {
      const employee = await q.getEmployee(ctx.pg, params.id);
  ...
  },
  {
    ...
    params: {},
    handler: async (params, ctx) => {
      return { success: true, data: await q.listEmployees(ctx.pg, params) };
    },
  },
  {
    name: "update",
  ...
  },
  {
    ...
    params: {},
    handler: async (params, ctx) => {
      const employee = await q.deleteEmployee(ctx.pg, params.id);
      if (!employee) {
        return { error: "Employee not found" };
      }
      await writeAuditLog(ctx.pg, {
  ...
  },
  {
    ...
    params: {},
    handler: async (params, ctx) => {
      const payroll = await q.runPayroll(ctx.pg, params);
      await writeAuditLog(ctx.pg, {
        domain: "hr",
        entityType: "payroll",
        entityId: payroll.id,
