...
>
> **Last updated:** 2026-07-05 (post-purge audit: refreshed from currently open GitHub issues/PRs and rebuilt from PR #49126 format)
...
| ------------------------- | ------ | ------ | ------- | ------ | --------- |
| MS Teams (channel plugin) | 12     | 45     | 57      | 0      | 57        |
| Windows platform          | 54     | 38     | 92      | 0      | 92        |
| WSL                       | 9      | 0      | 9       | 0      | 9         |
| Azure                     | 9      | 4      | 13      | 0      | 13        |
| SharePoint / M365         | 0      | 0      | 0       | 0      | 0         |
| **Total**                 | **84** | **87** | **171** | **0**  | **171**   |
...
| [ ]       | P2       | #42099 | fix(plugins): false-positive duplicate plugin ID warning on gateway start (msteams)                                                                         | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:linked-pr-open` `clawsweeper:needs-live-repro` +2       |          |
...
| --------- | -------- | ------- | --------------------------------------------------------------------------------------------------------------------- | ---- | ---------- |
| [ ]       | P0       | #99905  | fix(channels): honor configured read target policies                                                                  | XL   |            |
...
| [ ]       | P2       | #100166 | fix(msteams): paginate thread replies to include newest context (#98870)                                              | S    |            |
...
| [ ]       | P2       | #94859  | [codex] Use Executing for default progress label                                                                      | M    |            |
...
| [ ]       | P1       | #98573  | [Bug]: spawn gemini ENOENT on Windows (missing .cmd extension handling)                                                                                                                 | `bug` `bug:behavior` `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:fix-shape-clear` `clawsweeper:needs-maintainer-review` +5                                                     |             |
...
| [ ]       | P1       | #84600  | Bug: Windows heartbeat cmd window not hidden - 'findstr /I /C:"Running"' stays visible                                                                                                  | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:source-repro` `clawsweeper:linked-pr-open` `impact:crash-loop` `issue-rating: 🦞 diamond lobster` +1                               |             |
...
| [ ]       | P1       | #71865  | Auth login blocked by size-drop guard when openclaw.json was created by PowerShell (verbose/BOM format)                                                                                 | `stale` `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:source-repro` `clawsweeper:linked-pr-open` `impact:data-loss` +3                                                           |             |
...
| [ ]       | P2       | #99994  | [Bug]: heartbeat fails with EAGAIN (Unknown system error -11) on macOS/Linux — loadSessionStore + fileContentDiffersFromTemplate lack retry on non-Windows                              | `no-stale` `P1` `clawsweeper:fix-shape-clear` `clawsweeper:queueable-fix` `clawsweeper:source-repro` `impact:session-state` +3                                                   |             |
...
| [ ]       | P2       | #40694  | Browser-opened temporary tabs/windows should close automatically after task completion                                                                                                  | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:fix-shape-clear` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:source-repro` +3          |             |
...
| Resolved? | Priority | #      | Title                                                                                     | Size | Assignee    |
| --------- | -------- | ------ | ----------------------------------------------------------------------------------------- | ---- | ----------- |
| [ ]       | P0       | #98952 | fix(process): add gemini and claude to Windows .cmd shim allowlist                        | S    |             |
| [ ]       | P0       | #98609 | fix(process): resolve Gemini CLI Windows shim                                             | XS   |             |
...
| [ ]       | P2       | #90365 | test(browser): replace broad win32 skip with dynamic directory symlink check              | XS   |             |
...
| Windows platform          | pr    | P0       | #98952 | fix(process): add gemini and claude to Windows .cmd shim allowlist                                                                                       |
| Windows platform          | pr    | P0       | #98609 | fix(process): resolve Gemini CLI Windows shim                                                                                                            |
