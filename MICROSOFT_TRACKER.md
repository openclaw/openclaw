...
>
> **Last updated:** 2026-07-15 (post-purge audit: refreshed from currently open GitHub issues/PRs and rebuilt from PR #49126 format)
...
| ------------------------- | ------ | ------ | ------- | ------ | --------- |
| MS Teams (channel plugin) | 15     | 43     | 58      | 0      | 58        |
| Windows platform          | 50     | 27     | 77      | 0      | 77        |
| WSL                       | 5      | 1      | 6       | 0      | 6         |
| Azure                     | 12     | 5      | 17      | 0      | 17        |
| SharePoint / M365         | 0      | 0      | 0       | 0      | 0         |
| **Total**                 | **82** | **76** | **158** | **0**  | **158**   |
...
| [ ]       | P0       | #91722  | Refactor HTTP egress around external proxy enforcement                                                                | XL   | @RomneyDa         |
...
| [ ]       | P2       | #103692 | fix(msteams): strip internal tool-trace banners from outbound text                                                    | S    |                   |
...
| [ ]       | P2       | #84560  | feat(cli): support --dm-policy and --dm-allowlist in channels add                                                     |      | @steipete         |
...
| [ ]       | P2       | #108133 | Windows Git Bash commands can miss bundled coreutils                                                                                                                                    |                                                                                                                                                                                     |             |
| [ ]       | P2       | #108049 | Windows installer can fail when TEMP uses an unresolved short path                                                                                                                      |                                                                                                                                                                                     |             |
| [ ]       | P2       | #108021 | Windows update restart can force-kill a non-OpenClaw listener on the gateway port                                                                                                       |                                                                                                                                                                                     |             |
| [ ]       | P2       | #107416 | gateway.vbs/gateway.cmd encoding issue on Windows with CJK username paths                                                                                                               | `no-stale` `clawsweeper:fix-shape-clear` `clawsweeper:queueable-fix` `clawsweeper:source-repro` `P0` `issue-rating: 🦞 diamond lobster` +1                                          |             |
...
| [ ]       | P1       | #107751 | fix(daemon): gateway fails to launch on Windows when the profile path contains CJK characters | M    |          |
...
| [ ]       | P2       | #108073 | fix(infra): scope Windows path realpath caches                                                | M    |          |
| [ ]       | P2       | #108050 | fix(install): normalize Windows temp short paths                                              | M    |          |
| [ ]       | P2       | #108023 | fix(update): verify Windows restart listener owners                                           | M    |          |
...
| [ ]       | P1       | #108250 | Gateway startup fails on WSL2: EROFS chmod '/home/junl/.openclaw/state' in subagent-registry                                                    |                                                                                                                                                       |          |
...
| Resolved? | Priority | #       | Title                                                   | Size | Assignee |
| --------- | -------- | ------- | ------------------------------------------------------- | ---- | -------- |
| [ ]       | P2       | #108258 | fix(state): gateway starts when WSL chmod returns EROFS | XS   |          |
...
| Windows platform          | pr    | P1       | #107751 | fix(daemon): gateway fails to launch on Windows when the profile path contains CJK characters                                                                                           |
...
| WSL                       | issue | P1       | #108250 | Gateway startup fails on WSL2: EROFS chmod '/home/junl/.openclaw/state' in subagent-registry                                                                                            |
