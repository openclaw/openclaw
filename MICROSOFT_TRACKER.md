...
>
> **Last updated:** 2026-07-16 (post-purge audit: refreshed from currently open GitHub issues/PRs and rebuilt from PR #49126 format)
...
| ------------------------- | ------ | ------ | ------- | ------ | --------- |
| MS Teams (channel plugin) | 15     | 38     | 53      | 0      | 53        |
| Windows platform          | 51     | 29     | 80      | 0      | 80        |
| WSL                       | 4      | 0      | 4       | 0      | 4         |
| Azure                     | 12     | 5      | 17      | 0      | 17        |
| SharePoint / M365         | 0      | 0      | 0       | 0      | 0         |
| **Total**                 | **82** | **72** | **154** | **0**  | **154**   |
...
| [ ]       | P2       | #84560  | feat(cli): support --dm-policy and --dm-allowlist in channels add                                                     | L    | @steipete    |
...
| Resolved? | Priority | #       | Title                                                                                                                                                                                   | Labels                                                                                                                                                                              | Assignee    |
| --------- | -------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| [ ]       | P0       | #108021 | Windows update restart can force-kill a non-OpenClaw listener on the gateway port                                                                                                       | `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-security-review` `clawsweeper:source-repro` `clawsweeper:linked-pr-open` `impact:security` +1                                   |             |
...
| [ ]       | P2       | #108774 | Bug: gateway.cmd breaks with non-ASCII Windows usernames (mojibake)                                                                                                                     | `no-stale` `clawsweeper:fix-shape-clear` `clawsweeper:queueable-fix` `clawsweeper:needs-live-repro` `P0` `issue-rating: 🐚 platinum hermit` +1                                      |             |
| [ ]       | P2       | #108133 | Windows Git Bash commands can miss bundled coreutils                                                                                                                                    | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:source-repro` `clawsweeper:linked-pr-open` `issue-rating: 🦞 diamond lobster` `impact:ux-friction`                                    |             |
| [ ]       | P2       | #108049 | Windows installer can fail when TEMP uses an unresolved short path                                                                                                                      | `clawsweeper:no-new-fix-pr` `clawsweeper:source-repro` `clawsweeper:linked-pr-open` `P0` `issue-rating: 🦞 diamond lobster` `impact:ux-release-blocker`                             |             |
...
| [ ]       | P2       | #7057  | Flaky tests on Windows/WSL: timeouts and ENOENT in pi-tools workspace-paths & safe-bins                                            | `enhancement` `P3` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:linked-pr-open` `clawsweeper:not-repro-on-main` +2                             |          |
...
| [ ]       | P0       | #108023 | fix(update): verify Windows restart listener owners                                   | L    |          |
...
| [ ]       | P2       | #108967 | fix: gateway.cmd launcher garbles non-ASCII profile paths on non-CJK Windows locales  | M    |          |
| [ ]       | P2       | #108837 | fix(installer): bound Windows installer HTTP requests                                 | XS   |          |
| [ ]       | P2       | #108242 | fix(gateway): fill bounded transcript windows across short reads                      | M    |          |
| [ ]       | P2       | #108050 | fix(install): normalize Windows temp short paths                                      | S    |          |
| [ ]       | P2       | #107656 | refactor(channels)!: close the streaming flat-key and intro-hint deprecation windows  | L    |          |
| [ ]       | P2       | #105274 | fix(process): UTF-8 command output corrupts at Windows byte limits                    | S    |          |
...
| [ ]       | P2       | #107070 | refactor(whatsapp): centralize inbound turn admission and history finalization | L    |          |
...
| Windows platform          | issue | P0       | #108021 | Windows update restart can force-kill a non-OpenClaw listener on the gateway port                                                    |
...
| Windows platform          | pr    | P0       | #108023 | fix(update): verify Windows restart listener owners                                                                                  |
