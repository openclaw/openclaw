...
>
> **Last updated:** 2026-07-13 (post-purge audit: refreshed from currently open GitHub issues/PRs and rebuilt from PR #49126 format)
...
| ------------------------- | ------ | ------ | ------- | ------ | --------- |
| MS Teams (channel plugin) | 15     | 42     | 57      | 0      | 57        |
| Windows platform          | 51     | 23     | 74      | 0      | 74        |
| WSL                       | 6      | 0      | 6       | 0      | 6         |
| Azure                     | 11     | 5      | 16      | 0      | 16        |
| SharePoint / M365         | 0      | 0      | 0       | 0      | 0         |
| **Total**                 | **83** | **70** | **153** | **0**  | **153**   |
...
| [ ]       | P2       | #106026 | fix(channels): progress drafts only for long tasks, with the model's own preamble as the status headline              | XL   |             |
| [ ]       | P2       | #104691 | fix(msteams): proactive sends fail after conversation migration                                                       | S    |             |
| [ ]       | P2       | #104288 | fix(msteams): add timeouts to SharePoint file send requests                                                           | L    |             |
...
| [ ]       | P1       | #106311 | [Bug]: Gateway refuses to start after startup migrations on Windows (2026.7.1-beta.6)                                                                                                   | `bug`                                                                                                                                                                               |             |
...
| [ ]       | P1       | #74378  | [Bug]: OpenClaw CLI commands remain alive as node.exe processes after execution on Windows                                                                                              | `bug` `stale` `regression` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` +2                                                                                |             |
...
| [ ]       | P2       | #7057  | Flaky tests on Windows/WSL: timeouts and ENOENT in pi-tools workspace-paths & safe-bins                                            | `enhancement` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:linked-pr-open` `issue-rating: 🦪 silver shellfish` +1                         |          |
...
| [ ]       | P2       | #106202 | fix(installer): validate upgraded Windows SQLite runtime                              | S    | @vincentkoc |
...
| Windows platform          | issue | P1       | #106311 | [Bug]: Gateway refuses to start after startup migrations on Windows (2026.7.1-beta.6)                                                                                                   |
