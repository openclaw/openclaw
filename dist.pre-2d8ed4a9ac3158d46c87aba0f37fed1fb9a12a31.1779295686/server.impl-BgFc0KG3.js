...
import { A as applyConfigOverrides, c as promoteConfigSnapshotToLastKnownGood, i as getRuntimeConfig, u as readConfigFileSnapshot, v as registerConfigWriteListener } from "./io-Dn3XTR_G.js";
...
import { m as setPreRestartDeferralCheck, p as setGatewaySigusr1RestartPolicy } from "./restart-SvWHK9cF.js";
...
import { C as createPluginGatewayMethodDescriptors, S as createGatewayMethodRegistry, x as createGatewayMethodDescriptorsFromHandlers } from "./loader-Cq-u8i4x.js";
import { d as pinActivePluginChannelRegistry, f as pinActivePluginHttpRouteRegistry, h as releasePinnedPluginHttpRouteRegistry, m as releasePinnedPluginChannelRegistry } from "./runtime-CuTYW44m.js";
...
import { _ as stopDiagnosticHeartbeat, g as startDiagnosticHeartbeat } from "./diagnostic-FvvsVS2u.js";
...
import { a as getActiveSecretsRuntimeSnapshot, n as clearSecretsRuntimeSnapshot } from "./runtime-state-CXcf51ue.js";
import { o as setFallbackGatewayContextResolver } from "./server-plugins-f5nTlYk1.js";
import { r as resolveCronStorePath } from "./store-DSi9mGQW.js";
import { t as getTotalPendingReplies } from "./dispatcher-registry-BjdUWdbl.js";
import { c as recordGatewayRestartTraceDetail, d as resumeGatewayRestartTraceFromHandoff, i as finishGatewayRestartTrace, l as recordGatewayRestartTraceSpan, n as collectGatewayProcessMemoryUsageMb, u as resumeGatewayRestartTraceFromEnv } from "./restart-trace-BzEL-ZBE.js";
import { t as ensureOpenClawCliOnPath } from "./path-env-CrbflQnQ.js";
import { s as normalizePluginNodeCapabilityScopedUrl } from "./plugin-node-capability-Dw9wBTun.js";
import { i as summarizeAgentEventForWsLog, n as logWs, r as shouldLogWs } from "./ws-log-CKsgGAR2.js";
...
import { c as setDefaultSecurityHeaders, r as sendGatewayAuthFailure } from "./http-common-ldB0iri4.js";
import { i as MAX_BUFFERED_BYTES, o as MAX_PREAUTH_PAYLOAD_BYTES } from "./server-constants-TTWK0ss4.js";
import { t as GatewayLockError } from "./gateway-lock-ChG_gEZs.js";
import { c as resolveControlUiRootSync, n as isPackageProvenControlUiRootSync, s as resolveControlUiRootOverrideSync, t as ensureControlUiAssetsBuilt } from "./control-ui-assets-DhZ-Huil.js";
...
import { a as createToolEventRecipientRegistry, n as createChatRunState } from "./server-chat-state-y2ofT6aR.js";
import { a as incrementPresenceVersion, i as getPresenceVersion, n as getHealthCache, o as refreshGatewayHealthSnapshot, r as getHealthVersion } from "./health-state-CIBtAS4t.js";
import { n as applyGatewayLaneConcurrency, t as resolveHookClientIpConfig } from "./hook-client-ip-config-CpTpKOus.js";
import { t as GATEWAY_EVENTS } from "./server-methods-list-CkLwAvDH.js";
import { t as resolveSharedGatewaySessionGeneration } from "./ws-shared-generation-CG1EoQVl.js";
import { n as isProtectedPluginRoutePathFromContext, r as resolvePluginRoutePathContext } from "./route-match-BvpZtazk.js";
import { n as shouldEnforceGatewayAuthForPluginPath } from "./route-auth-7B7LvtAY.js";
import { t as findMatchingPluginNodeCapabilityRoute } from "./route-capability-C-oczmRw.js";
...
function createLazyGatewayCronState(params) {
	...
	const load = async () => {
		...
		loading ??= import("./server-cron-Ber6Yt5b.js").then(({ buildGatewayCronService }) => {
			loaded = {
...
function getIdentityAvatarModule() {
	identityAvatarModulePromise ??= import("./identity-avatar-UYFFZ97u.js");
	return identityAvatarModulePromise;
}
function getControlUiModule() {
	controlUiModulePromise ??= import("./control-ui-CXDiN3JU.js");
	return controlUiModulePromise;
}
function getEmbeddingsHttpModule() {
	embeddingsHttpModulePromise ??= import("./embeddings-http-DJj4Dov2.js");
	return embeddingsHttpModulePromise;
}
function getManagedImageAttachmentsModule() {
	managedImageAttachmentsModulePromise ??= import("./managed-image-attachments-DjnoYpxW.js");
	return managedImageAttachmentsModulePromise;
}
function getModelsHttpModule() {
	modelsHttpModulePromise ??= import("./models-http-DWeiEXoT.js");
	return modelsHttpModulePromise;
}
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-DUXkGC94.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-DDa7VpLW.js");
	return openResponsesHttpModulePromise;
}
function getSessionHistoryHttpModule() {
	sessionHistoryHttpModulePromise ??= import("./sessions-history-http-BSLDVxig.js");
	return sessionHistoryHttpModulePromise;
}
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-CZn1pT_2.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-BwyaaK1f.js");
	return toolsInvokeHttpModulePromise;
}
function getPluginNodeCapabilityAuthModule() {
	pluginNodeCapabilityAuthModulePromise ??= import("./plugin-node-capability-auth-j1gjFVje.js");
	return pluginNodeCapabilityAuthModulePromise;
}
function getHttpAuthUtilsModule() {
	httpAuthUtilsModulePromise ??= import("./http-auth-utils-BJmyIYu-.js");
	return httpAuthUtilsModulePromise;
}
function getPluginRouteRuntimeScopesModule() {
	pluginRouteRuntimeScopesModulePromise ??= import("./plugin-route-runtime-scopes-BniRcXnp.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-Qkfee2dz.js");
		...
		const handlePluginRequest = async (req, res, pathContext, dispatchContext) => {
			...
			if (!loadedPluginRequestHandler) {
				const { createGatewayPluginRequestHandler } = await import("./plugins-http-CEQC3phz.js");
		...
		const handlePluginUpgrade = async (req, socket, head, pathContext, dispatchContext) => {
			...
			if (!loadedPluginUpgradeHandler) {
				const { createGatewayPluginUpgradeHandler } = await import("./plugins-http-CEQC3phz.js");
...
async function resetModelCatalogCacheForTest() {
	const { resetModelCatalogCacheForTest } = await import("./server-model-catalog-hEfG_LHg.js");
...
function loadGatewayStartupEarlyModule() {
	gatewayStartupEarlyModulePromise ??= import("./server-startup-early-g2xEsD1m.js");
	return gatewayStartupEarlyModulePromise;
}
function loadGatewayStartupPostAttachModule() {
	gatewayStartupPostAttachModulePromise ??= import("./server-startup-post-attach-CXOK9TxI.js");
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-CuB9Gc11.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
	return cachedChannelRuntimePromise;
}
function getStartupChannelRuntime() {
	cachedStartupChannelRuntimePromise ??= import("./channel-runtime-contexts-CLud0Dqj.js").then(({ createChannelRuntimeContextRegistry }) => ({ runtimeContexts: createChannelRuntimeContextRegistry() }));
...
const loadGatewayModelCatalog = async (...args) => {
	gatewayModelCatalogModulePromise ??= import("./server-model-catalog-hEfG_LHg.js");
...
async function stopTaskRegistryMaintenanceOnDemand() {
	const { stopTaskRegistryMaintenance } = await import("./task-registry.maintenance-KgxOcwtx.js");
...
const runDefaultSetupWizard = async (...args) => {
	const { runSetupWizard } = await import("./setup-C4u8JCMY.js");
	return runSetupWizard(...args);
};
async function startGatewayServer(port = 18789, opts = {}) {
	const { bootstrapGatewayNetworkRuntime } = await import("./server-network-runtime-B6hf8Jl1.js");
	...
	const startupConfigModulePromise = import("./server-startup-config-BsRR7woa.js");
	let startupPluginsModulePromise = null;
	const loadStartupPluginsModule = () => {
		startupPluginsModulePromise ??= import("./server-startup-plugins-lwafefR6.js");
	...
	const runtimeConfig = await startupTrace.measure("runtime.config", async () => {
		const { resolveGatewayRuntimeConfig } = await import("./server-runtime-config-ziUyftza.js");
	...
	const { createChannelManager } = await import("./server-channels-4O2RxqZF.js");
	...
	const { createGatewayNodeSessionRuntime } = await import("./server-node-session-runtime-D_TpqFd7.js");
	...
	try {
		...
		const [{ startGatewayEventSubscriptions }, gatewayRuntimeServices] = await Promise.all([import("./server-runtime-subscriptions-DBXNGvp2.js"), import("./server-runtime-services-DSEDjL2B.js")]);
		...
		const { createGatewayAuxHandlers } = await import("./server-aux-handlers-BRB7ie-T.js");
		const { coreGatewayHandlers } = await import("./server-methods-WuUh0YhB.js");
		...
		const reloadAttachedGatewayPlugins = async (params) => {
			...
			const [{ loadPluginLookUpTable }, { prepareGatewayPluginLoad }, { startPluginServices }] = await Promise.all([
				import("./plugin-lookup-table-oJUtxaDo.js"),
				import("./server-plugin-bootstrap-DHIh5mPR.js"),
				import("./services-Emtmsl-z.js")
			]);
		...
		const { createGatewayRequestContext } = await import("./server-request-context-iSF5goZo.js");
		...
		if (!minimalTestGateway) {
			if (runtimePluginsLoaded && deferredConfiguredChannelPluginIds.length > 0) {
				const { reloadDeferredGatewayPlugins } = await import("./server-plugin-bootstrap-DHIh5mPR.js");
		...
		const { attachGatewayWsHandlers } = await import("./server-ws-runtime-DXWbhIMB.js");
		const { listPluginNodeCapabilities } = await import("./route-capability-BePz4qr3.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-BfCrh3Kc.js");
