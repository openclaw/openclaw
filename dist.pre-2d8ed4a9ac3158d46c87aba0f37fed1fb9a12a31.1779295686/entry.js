...
import { d as isRootHelpInvocation, f as isRootVersionInvocation, r as getCommandPositionalsWithRootOptions } from "./argv-DEHjUPTf.js";
import { t as isMainModule } from "./is-main-myFrAEl9.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-Co9T_Qxk.js";
import { a as resolveCliContainerTarget, i as parseCliContainerArgs, n as parseCliProfileArgs, t as applyCliProfileEnv } from "./profile-Cjg3P6nK.js";
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-B_M93VzP.js"), import("./git-commit-0t7yW3nY.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		const liveRootHelpOptions = await (deps.loadRootHelpRenderOptionsForConfigSensitivePlugins ?? (await import("./root-help-live-config-B4xIw3Df.js")).loadRootHelpRenderOptionsForConfigSensitivePlugins)(deps.env);
		if (!liveRootHelpOptions) {
			if ((deps.outputPrecomputedRootHelpText ?? (await import("./root-help-metadata-DBVvAsCk.js")).outputPrecomputedRootHelpText)()) return true;
		}
		await (deps.outputRootHelp ?? (await import("./root-help-B7ABht3C.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
		handleError(error);
		return true;
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-uElKoQr3.js");
