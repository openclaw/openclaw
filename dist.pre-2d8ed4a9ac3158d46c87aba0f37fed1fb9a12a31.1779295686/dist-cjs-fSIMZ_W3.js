...
import { B as client_exports, Ct as parseRfc7231DateTime, D as init_endpoints, Dt as fromUtf8, Et as toBase64, F as init_config, Ot as fromBase64, P as config_exports, Q as init_schema, S as serde_exports, St as parseRfc3339DateTimeWithOffset, T as endpoints_exports, Tt as toUtf8, U as getValueFromTextNode, V as init_client, Y as decorateServiceException, _ as SerdeContext, _t as nv, b as generateIdempotencyToken, bt as dateToUtcString, c as HttpInterceptingShapeSerializer, d as determineTimestampFormat, dt as ProviderError, et as TypeRegistry, f as requestBuilder, g as HttpRequest, gt as NumericValue, h as HttpResponse, l as HttpInterceptingShapeDeserializer, m as HttpBindingProtocol, n as protocols_exports$1, ot as getSmithyContext$1, p as RpcProtocol, rt as deref, t as init_protocols$1, tt as NormalizedSchema, u as FromStringShapeDeserializer, ut as require_dist_cjs$9, v as extendedEncodeURIComponent, vt as _parseEpochTimestamp, wt as expectUnion, x as init_serde, xt as parseEpochTimestamp, y as collectBody, yt as LazyJsonString } from "./protocols-F3YwbM04.js";
import { a as retry_exports, i as init_retry, n as init_client$1, t as client_exports$1, u as setCredentialFeature } from "./client-BSOxYSjG.js";
...
var require_recursionDetectionMiddleware = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const protocols_1 = (init_protocols$1(), __toCommonJS(protocols_exports$1));
	const TRACE_ID_HEADER_NAME = "X-Amzn-Trace-Id";
	const ENV_LAMBDA_FUNCTION_NAME = "AWS_LAMBDA_FUNCTION_NAME";
	const ENV_TRACE_ID = "_X_AMZN_TRACE_ID";
	const recursionDetectionMiddleware = () => (next) => async (args) => {
		const { request } = args;
		if (!protocols_1.HttpRequest.isInstance(request)) return next(args);
...
var init_parseJsonBody = __esmMin((() => {
	...
	sanitizeErrorCode = (rawValue, removeNamespace = true) => {
		let cleanValue = rawValue;
		if (typeof cleanValue === "number") cleanValue = cleanValue.toString();
		if (cleanValue.indexOf(",") >= 0) cleanValue = cleanValue.split(",")[0];
		if (cleanValue.indexOf(":") >= 0) cleanValue = cleanValue.split(":")[0];
		if (removeNamespace && cleanValue.indexOf("#") >= 0) cleanValue = cleanValue.split("#")[1];
		return cleanValue;
	};
	loadRestJsonErrorCode = (output, data) => {
		return loadErrorCode(output, data, true, [
			"header",
			"code",
			"type"
		]);
	};
	loadJsonRpcErrorCode = (output, data, removeNamespace, queryCompat = false) => {
		return loadErrorCode(output, data, removeNamespace, queryCompat ? [
			"code",
			"header",
			"type"
		] : [
	...
	loadErrorCode = ({ headers }, data, removeNamespace, order) => {
		while (order.length > 0) switch (order.shift()) {
			case "header":
				const headerKey = findKey(headers ?? {}, "x-amzn-errortype");
				if (headerKey !== void 0) return sanitizeErrorCode(headers[headerKey], removeNamespace);
				break;
			case "code":
				const codeKey = findKey(data ?? {}, "code");
				if (codeKey && data[codeKey] !== void 0) return sanitizeErrorCode(data[codeKey], removeNamespace);
				break;
			case "type":
				if (data?.__type !== void 0) return sanitizeErrorCode(data.__type, removeNamespace);
				break;
		}
	};
}));
...
var import_dist_cjs$5, XmlShapeDeserializer;
var init_XmlShapeDeserializer = __esmMin((() => {
	import_dist_cjs$5 = require_dist_cjs$5();
	init_client();
	init_protocols$1();
	init_schema();
	init_serde();
	...
	XmlShapeDeserializer = class extends SerdeContextConfig {
		...
		parseXml(xml) {
			if (xml.length) {
				let parsedObj;
				try {
					parsedObj = (0, import_dist_cjs$5.parseXML)(xml);
				} catch (e) {
...
var import_dist_cjs$4, parseXmlBody, parseXmlErrorBody, loadRestXmlErrorCode;
var init_parseXmlBody = __esmMin((() => {
	import_dist_cjs$4 = require_dist_cjs$5();
	init_client();
	init_common();
...
var import_dist_cjs$3, XmlShapeSerializer;
var init_XmlShapeSerializer = __esmMin((() => {
	...
	XmlShapeSerializer = class extends SerdeContextConfig {
		...
		writeStruct(ns, value, parentXmlns) {
			...
			const structXmlNode = import_dist_cjs$3.XmlNode.of(name);
			...
			for (const [memberName, memberSchema] of ns.structIterator()) {
				...
				if (val != null || memberSchema.isIdempotencyToken()) {
					...
					if (memberSchema.isListSchema()) this.writeList(memberSchema, val, structXmlNode, xmlns);
						...
						const memberNode = import_dist_cjs$3.XmlNode.of(memberSchema.getMergedTraits().xmlName ?? memberSchema.getMemberName());
			...
			if ($unknown && ns.isUnionSchema() && Array.isArray($unknown) && Object.keys(value).length === 1) {
				const [k, v] = $unknown;
				const node = import_dist_cjs$3.XmlNode.of(k);
				if (typeof v !== "string") if (value instanceof import_dist_cjs$3.XmlNode || value instanceof import_dist_cjs$3.XmlText) structXmlNode.addChildNode(value);
		...
		writeList(listMember, array, container, parentXmlns) {
			...
			const writeItem = (container, value) => {
				if (listValueSchema.isListSchema()) this.writeList(listValueSchema, Array.isArray(value) ? value : [value], container, xmlns);
					...
					const listItemNode = import_dist_cjs$3.XmlNode.of(flat ? listTraits.xmlName ?? listMember.getMemberName() : listValueTraits.xmlName ?? "member");
			...
			if (flat) {
				...
				const listNode = import_dist_cjs$3.XmlNode.of(listTraits.xmlName ?? listMember.getMemberName());
		...
		writeMap(mapMember, map, container, parentXmlns, containerIsMap = false) {
			...
			const addKeyValue = (entry, key, val) => {
				const keyNode = import_dist_cjs$3.XmlNode.of(keyTag, key);
				const [keyXmlnsAttr, keyXmlns] = this.getXmlnsAttribute(mapKeySchema, xmlns);
				if (keyXmlns) keyNode.addAttribute(keyXmlnsAttr, keyXmlns);
				entry.addChildNode(keyNode);
				let valueNode = import_dist_cjs$3.XmlNode.of(valueTag);
			...
			if (flat) for (const key in map) {
				const val = map[key];
				if (sparse || val != null) {
					const entry = import_dist_cjs$3.XmlNode.of(mapTraits.xmlName ?? mapMember.getMemberName());
				...
				let mapNode;
				if (!containerIsMap) {
					mapNode = import_dist_cjs$3.XmlNode.of(mapTraits.xmlName ?? mapMember.getMemberName());
				...
				for (const key in map) {
					const val = map[key];
					if (sparse || val != null) {
						const entry = import_dist_cjs$3.XmlNode.of("entry");
						addKeyValue(entry, key, val);
		...
		writeSimpleInto(_schema, value, into, parentXmlns) {
			...
			const ns = NormalizedSchema.of(_schema);
			const content = new import_dist_cjs$3.XmlText(nodeContents);
			const [xmlnsAttr, xmlns] = this.getXmlnsAttribute(ns, parentXmlns);
...
var import_dist_cjs$2, getSmithyContext;
var init_getSmithyContext = __esmMin((() => {
	import_dist_cjs$2 = require_dist_cjs$9();
	getSmithyContext = (context) => context[import_dist_cjs$2.SMITHY_CONTEXT_KEY] || (context[import_dist_cjs$2.SMITHY_CONTEXT_KEY] = {});
}));
//#endregion
...
var import_dist_cjs$1, HttpApiKeyAuthSigner;
var init_httpApiKeyAuth = __esmMin((() => {
	...
	HttpApiKeyAuthSigner = class {
		async sign(httpRequest, identity, signingProperties) {
			...
			if (signingProperties.in === import_dist_cjs$1.HttpApiKeyAuthLocation.QUERY) clonedRequest.query[signingProperties.name] = identity.apiKey;
			else if (signingProperties.in === import_dist_cjs$1.HttpApiKeyAuthLocation.HEADER) clonedRequest.headers[signingProperties.name] = signingProperties.scheme ? `${signingProperties.scheme} ${identity.apiKey}` : identity.apiKey;
...
var require_dist_cjs$3 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const isVirtualHostableS3Bucket = (value, allowSubDomains = false) => {
		...
		if (!endpoints.isValidHostLabel(value)) return false;
		if (value.length < 3 || value.length > 63) return false;
		if (value !== value.toLowerCase()) return false;
		if (endpoints.isIpAddress(value)) return false;
		return true;
	};
	const ARN_DELIMITER = ":";
...
var require_dist_cjs$2 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var utilEndpoints = require_dist_cjs$3();
	var protocols = (init_protocols$1(), __toCommonJS(protocols_exports$1));
	...
	const userAgentMiddleware = (options) => (next, context) => async (args) => {
		const { request } = args;
		if (!protocols.HttpRequest.isInstance(request)) return next(args);
...
var init_resolveAwsSdkSigV4Config = __esmMin((() => {
	init_client$1();
	init_dist_es();
	import_dist_cjs = require_dist_cjs$4();
...
var require_dist_cjs$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var middlewareUserAgent = require_dist_cjs$2();
	...
	const getTypeScriptUserAgentPair = async () => {
		...
		try {
			isTypeScriptDetectionDisabled = config.booleanSelector(process.env, "AWS_SDK_JS_TYPESCRIPT_DETECTION_DISABLED", config.SelectorType.ENV) || false;
		} catch {}
...
//#endregion
export { AwsRestJsonProtocol as C, require_dist_cjs$8 as E, AwsQueryProtocol as S, require_dist_cjs$7 as T, getHttpSigningPlugin as _, resolveAwsSdkSigV4Config as a, init_protocols as b, NODE_AUTH_SCHEME_PREFERENCE_OPTIONS as c, require_dist_cjs$2 as d, require_dist_cjs$3 as f, DefaultIdentityProviderConfig as g, NoAuthSigner as h, init_httpAuthSchemes as i, AwsSdkSigV4ASigner as l, init_dist_es as m, require_dist_cjs$1 as n, NODE_SIGV4A_CONFIG_OPTIONS as o, dist_es_exports as p, httpAuthSchemes_exports as r, resolveAwsSdkSigV4AConfig as s, require_dist_cjs as t, AwsSdkSigV4Signer as u, getHttpAuthSchemeEndpointRuleSetPlugin as v, require_dist_cjs$6 as w, protocols_exports as x, require_dist_cjs$4 as y };
