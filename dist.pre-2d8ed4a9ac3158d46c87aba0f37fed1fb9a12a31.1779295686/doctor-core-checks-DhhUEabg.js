...
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-BXVLXLwO.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-c2HkdMEn.js";
import { i as disableUnavailableSkillsInConfig, t as collectUnavailableAgentSkills } from "./doctor-skills-DVb__yE-.js";
import { r as registerHealthCheck } from "./health-check-registry-CJuqImUu.js";
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-DfySb-EM.js");
...
const bootstrapSizeCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { buildBootstrapInjectionStats, analyzeBootstrapBudget } = await import("./bootstrap-budget-CCTUVdkf.js");
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-kF4kQvfL.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./pi-embedded-helpers-C-7tp3Se.js");
...
const securityCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectSecurityWarnings } = await import("./doctor-security-CDG4Sxzw.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./oauth-tls-preflight-BZzQxbt6.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./doctor-cron-Dv2aO6a6.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-D0uDohoU.js");
...
const workspaceStatusCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyWorkspaceDirs } = await import("./doctor-workspace-CDEIGcj4.js");
		const workspaceDir = resolveAgentWorkspaceDir(ctx.cfg, resolveDefaultAgentId(ctx.cfg));
...
const convertedWorkflowChecks = [
	...
	bootstrapSizeCheck,
	{
		...
		source: "doctor",
		async detect(ctx) {
			const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-CERl_S8o.js");
