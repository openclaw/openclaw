...
import { n as isCanvasHostEnabled, t as canvasConfigSchema } from "../../config-BPxD0sqy.js";
import { n as CANVAS_HOST_PATH, r as CANVAS_WS_PATH, t as A2UI_PATH } from "../../a2ui-shared-CzaY1whh.js";
import { t as CanvasToolSchema } from "../../tool-schema-B0faqWZQ.js";
...
function createLazyCanvasTool(params) {
	...
	const loadTool = async () => {
		toolPromise ??= import("../../tool-0j_6goXF.js").then(({ createCanvasTool }) => createCanvasTool({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			...
			const loadHttpRouteHandler = async () => {
				httpRouteHandlerPromise ??= import("../../http-route-D5_S6duh.js").then(({ createCanvasHttpRouteHandler }) => createCanvasHttpRouteHandler({
					config: api.config,
			...
			api.registerHostedMediaResolver(async (mediaUrl) => {
				resolveCanvasHttpPathToLocalPathPromise ??= import("../../documents-chcPRBTE.js").then(({ resolveCanvasHttpPathToLocalPath }) => resolveCanvasHttpPathToLocalPath);
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-B4fJDsPM.js");
