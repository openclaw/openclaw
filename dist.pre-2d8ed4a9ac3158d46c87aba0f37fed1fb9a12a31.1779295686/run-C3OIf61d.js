...
import { i as withDiagnosticPhase } from "./diagnostic-phase-BOGKr7tE.js";
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-BzEL-ZBE.js";
import { r as withProgress } from "./progress-CjeJKwbS.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-ChBDRL3E.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-ChG_gEZs.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-D2M2tC70.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-BmZ6WQjl.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-Tn5Dm5-b.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-DFCwy6gx.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-DoaqS_el.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-CTYidhtm.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-DV6Ultf0.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			const { maybeExplainGatewayServiceStop } = await import("./shared-DSX370pC.js");
