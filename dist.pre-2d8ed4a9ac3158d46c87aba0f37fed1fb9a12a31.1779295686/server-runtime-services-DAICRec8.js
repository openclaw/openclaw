import { n as isVitestRuntimeEnv } from "./env-C3v-quV5.js";
import { t as createSubsystemLogger } from "./subsystem-BCvJ25zm.js";
import { t as isGatewayModelPricingEnabled } from "./model-pricing-config-BBzP2gNS.js";
import { i as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-PUX8se0g.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-BvKZ92UE.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-HiqSe3Cv.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-NwL6IAWu.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-BWXue2Ra.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-DF5NgeS8.js");
		if (stopped) return;
