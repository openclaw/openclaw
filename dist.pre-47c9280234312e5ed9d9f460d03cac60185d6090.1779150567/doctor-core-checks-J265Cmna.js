...
import { t as buildWorkspaceSkillStatus } from "./skills-status-CKNDEuoG.js";
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-f6aeXefl.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-DoDh1WD4.js";
import { i as disableUnavailableSkillsInConfig, t as collectUnavailableAgentSkills } from "./doctor-skills-C8jbYmVd.js";
import { r as registerHealthCheck } from "./health-check-registry-CLK-uyON.js";
...
const hooksModelCheck = {
	...
	async detect(ctx) {
		...
		const { loadModelCatalog } = await import("./model-catalog-dU5n0VTt.js");
		const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } = await import("./model-selection-Y5GzidjE.js");
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-GCVArpDI.js");
...
const claudeCliCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { noteClaudeCliHealth } = await import("./doctor-claude-cli-pScpLSKn.js");
...
const securityCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectSecurityWarnings } = await import("./doctor-security-C95gt1Tc.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./oauth-tls-preflight-CGLv7CWw.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./doctor-cron-DVAZ0ziR.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-BnA3RS1R.js");
...
const browserCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { noteChromeMcpBrowserReadiness } = await import("./doctor-browser-KrgNLHvK.js");
...
const workspaceStatusCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyWorkspaceDirs } = await import("./doctor-workspace-InkO1MjJ.js");
		const workspaceDir = resolveAgentWorkspaceDir(ctx.cfg, resolveDefaultAgentId(ctx.cfg));
...
const convertedWorkflowChecks = [
	...
	bootstrapSizeCheck,
	{
		...
		source: "doctor",
		async detect(ctx) {
			const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-KNxxQdUL.js");
			const { MEMORY_SYSTEM_PROMPT, shouldSuggestMemorySystem } = await import("./doctor-workspace-InkO1MjJ.js");
			const workspaceDir = resolveAgentWorkspaceDir(ctx.cfg, resolveDefaultAgentId(ctx.cfg));
			const findings = [];
...
//#endregion
export { registerCoreHealthChecks as n, configValidationIssuesToHealthFindings as t };
