"""CLI tagline system with holiday support."""
...
# Holiday taglines
HOLIDAY_TAGLINES = {
    "new_year": "New Year's Day: New year, new config—same old EADDRINUSE, but this time we resolve it like grown-ups.",
    "lunar_new_year": "Lunar New Year: May your builds be lucky, your branches prosperous, and your merge conflicts chased away with fireworks.",
...
TAGLINES = [
    ...
    # Holiday taglines are added below
    *HOLIDAY_TAGLINES.values(),
]


def _on_month_day(month: int, day: int) -> Callable[[date], bool]:
    """Create a rule for a specific month/day."""
    return lambda d: d.month == month and d.day == day


def _on_specific_dates(dates: list[tuple[int, int, int]], duration_days: int = 1) -> Callable[[date], bool]:
    """Create a rule for specific dates with optional duration."""
    def check(d: date) -> bool:
        for year, month, day in dates:
            ...
            # Simple range check
            current_ordinal = d.toordinal()
            start_ordinal = start.toordinal()
            if start_ordinal <= current_ordinal < start_ordinal + duration_days:
                return True
        return False
    return check


def _is_fourth_thursday_of_november(d: date) -> bool:
    """Check if date is Thanksgiving (4th Thursday of November)."""
    if d.month != 11:
        return False
    # Find first day of November
    first_day = date(d.year, 11, 1)
    # Calculate offset to first Thursday (weekday 3)
    offset_to_thursday = (3 - first_day.weekday() + 7) % 7
    # Fourth Thursday is first Thursday + 21 days
    fourth_thursday_day = 1 + offset_to_thursday + 21
    return d.day == fourth_thursday_day


# Holiday rules (tagline -> date checker)
HOLIDAY_RULES: dict[str, Callable[[date], bool]] = {
    HOLIDAY_TAGLINES["new_year"]: _on_month_day(1, 1),
    HOLIDAY_TAGLINES["lunar_new_year"]: _on_specific_dates([
        (2025, 1, 29),
        (2026, 2, 17),
        (2027, 2, 6),
    ]),
    ...
    HOLIDAY_TAGLINES["hanukkah"]: _on_specific_dates([
        (2025, 12, 15),
        (2026, 12, 5),
        (2027, 12, 25),
    ], duration_days=8),
    HOLIDAY_TAGLINES["halloween"]: _on_month_day(10, 31),
    HOLIDAY_TAGLINES["thanksgiving"]: _is_fourth_thursday_of_november,
    HOLIDAY_TAGLINES["valentines"]: _on_month_day(2, 14),
    HOLIDAY_TAGLINES["christmas"]: _on_month_day(12, 25),
}


def _is_tagline_active(tagline: str, today: date) -> bool:
    """Check if a tagline is active on the given date."""
...
def active_taglines(today: date | None = None) -> list[str]:
    ...
    Returns:
        List of active taglines
    """
    if not TAGLINES:
        return [DEFAULT_TAGLINE]

    check_date = today or date.today()
    filtered = [t for t in TAGLINES if _is_tagline_active(t, check_date)]
...
def pick_tagline(today: date | None = None, rng: random.Random | None = None) -> str:
    ...
    """
    # Check for override
    override = os.environ.get("OPENCLAW_TAGLINE_INDEX")
    if override:
        try:
            index = int(override)
            if index >= 0:
    ...
    pool = active_taglines(today)
    if not rng:
        rng = random.Random()

    return rng.choice(pool) if pool else DEFAULT_TAGLINE
