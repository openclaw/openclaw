...
import { Nt as validateSecretsResolveResult } from "./protocol-B19qkHL1.js";
import { i as callGateway } from "./call-Ch2fB07i.js";
import { i as resolveBundledExplicitWebSearchProvidersFromPublicArtifacts } from "./web-provider-public-artifacts.explicit-D9Rz2aXW.js";
import { t as assertExpectedResolvedSecretValue } from "./secret-value-Pxa0BPgI.js";
import { t as analyzeCommandSecretAssignmentsFromSnapshot } from "./command-config-V9FHnVRG.js";
import { t as collectConfigAssignments } from "./runtime-config-collectors-B5E30wtd.js";
import { r as createResolverContext } from "./runtime-shared-Bfyyw9aP.js";
import { t as resolveRuntimeWebTools } from "./runtime-web-tools-55kadQ4s.js";
...
function applyProviderOverridesToConfig(config, overrides) {
	if (!normalizeOptionalString(overrides?.webSearch) && !normalizeOptionalString(overrides?.webFetch)) return config;
...
function isProviderOverridePath(params) {
	...
	if (webSearch) {
		...
		if (directSearchProvider) return directSearchProvider === webSearch;
		const pluginId = pluginIdFromRuntimeWebPath(params.path);
		if (pluginId && params.path.endsWith(".config.webSearch.apiKey")) return commandSecretGatewayDeps.resolveManifestContractOwnerPluginId({
			contract: "webSearchProviders",
			value: webSearch,
			origin: "bundled",
			config: params.config
	...
	if (webFetch) {
		...
		if (directFetchProvider) return directFetchProvider === webFetch;
		const pluginId = pluginIdFromRuntimeWebPath(params.path);
		if (pluginId && params.path.endsWith(".config.webFetch.apiKey")) return commandSecretGatewayDeps.resolveManifestContractOwnerPluginId({
...
function classifyRuntimeWebTargetPathState(params) {
	if ((normalizeOptionalString(params.providerOverrides?.webSearch) || normalizeOptionalString(params.providerOverrides?.webFetch)) && isDirectRuntimeWebTargetPath(params.path)) return isProviderOverridePath({
		config: params.config,
		path: params.path,
	...
	if (!directFetchMatch) return "unknown";
	const fetch = params.config.tools?.web?.fetch;
	if (fetch?.enabled === false) return "inactive";
	const configuredProvider = normalizeLowercaseStringOrEmpty(fetch?.provider);
	if (!configuredProvider) return "active";
...
function describeInactiveRuntimeWebTargetPath(params) {
	if (params.config.tools?.web?.search?.enabled === false && (params.path === "tools.web.search.apiKey" || params.path.startsWith("tools.web.search.") || params.path.includes(".webSearch."))) return "tools.web.search is disabled.";
	if (params.config.tools?.web?.fetch?.enabled === false && (params.path.startsWith("tools.web.fetch.") || params.path.includes(".webFetch."))) return "tools.web.fetch is disabled.";
	const webSearchOverride = normalizeOptionalString(params.providerOverrides?.webSearch);
	if (webSearchOverride && params.path.includes(".webSearch.")) return `tools.web.search.provider is "${webSearchOverride}".`;
	if (webSearchOverride && params.path.startsWith("tools.web.search.")) return `tools.web.search.provider is "${webSearchOverride}".`;
	const webFetchOverride = normalizeOptionalString(params.providerOverrides?.webFetch);
	if (webFetchOverride && params.path.includes(".webFetch.")) return `tools.web.fetch.provider is "${webFetchOverride}".`;
	if (webFetchOverride && params.path.startsWith("tools.web.fetch.")) return `tools.web.fetch.provider is "${webFetchOverride}".`;
	...
	if (directSearchMatch) {
		...
		if (search?.enabled === false) return "tools.web.search is disabled.";
		const configuredProvider = normalizeLowercaseStringOrEmpty(search?.provider);
		if (configuredProvider && configuredProvider !== directSearchMatch[1]) return `tools.web.search.provider is "${configuredProvider}".`;
		return;
	}
	const directFetchMatch = /^tools\.web\.fetch\.([^.]+)\.apiKey$/.exec(params.path);
	if (!directFetchMatch) return;
	const fetch = params.config.tools?.web?.fetch;
	if (fetch?.enabled === false) return "tools.web.fetch is disabled.";
	const configuredProvider = normalizeLowercaseStringOrEmpty(fetch?.provider);
	if (configuredProvider && configuredProvider !== directFetchMatch[1]) return `tools.web.fetch.provider is "${configuredProvider}".`;
}
...
function isDirectRuntimeWebTargetPath(path) {
	return path === "tools.web.search.apiKey" || /^plugins\.entries\.[^.]+\.config\.(webSearch|webFetch)\.apiKey$/.test(path) || /^tools\.web\.(search|fetch)\.[^.]+\.apiKey$/.test(path);
}
