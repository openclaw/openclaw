...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-C1-6jjhw.js"), import("./git-commit-BXkAZ01R.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		if (!(deps.outputPrecomputedRootHelpText ?? (await import("./root-help-metadata-CYi2YIJp.js")).outputPrecomputedRootHelpText)()) {
			const { outputRootHelp } = await import("./root-help-eOWRyZK_.js");
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-DQ2vflAX.js");
