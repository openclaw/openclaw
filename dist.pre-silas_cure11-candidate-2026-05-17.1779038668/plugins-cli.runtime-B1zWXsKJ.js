...
import { n as collectConfiguredRuntimePluginIds, r as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-BSKrIQdU.js";
...
async function runPluginsEnableCommand(id) {
	...
	const { enablePluginInConfig } = await import("./enable-D8Gd_M59.js");
	...
	const { applySlotSelectionForPlugin, logSlotWarnings } = await import("./plugins-command-helpers-DvwWRDnU.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-C9T-w3_I.js");
...
async function runPluginsDisableCommand(id) {
	...
	const { normalizePluginId } = await import("./config-state-CDowxzeJ.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-CQUbLivV.js");
	const { setPluginEnabledInConfig } = await import("./plugins-config-Bb8X3FS0.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-C9T-w3_I.js");
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-Dz4cYyoP.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-Cs24fazM.js");
...
async function runPluginsDoctorCommand() {
	const { buildPluginCompatibilityNotices, buildPluginDiagnosticsReport, formatPluginCompatibilityNotice } = await import("./status-CQUbLivV.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-trU50Qiv.js");
	const { createPluginInstallLogger } = await import("./plugins-command-helpers-DvwWRDnU.js");
