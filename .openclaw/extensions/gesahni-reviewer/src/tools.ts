...
import { createGesahniReviewerClient, type BridgeAuthMode, type BridgeMethod } from "./client.js";
import { resolveGesahniReviewerConfig } from "./config.js";

type ToolSpec = {
  name: GesahniReviewerToolName;
  description: string;
  method: BridgeMethod;
  auth: BridgeAuthMode;
  pathTemplate: string;
...
const ReviewArtifactSchema = {
  ...JsonObjectSchema,
  description:
    "Bridge artifact create payload. Must represent a review_notes artifact and include content_json with the required review contract.",
} as const;
...
const TaskStatusSchema = {
  ...JsonObjectSchema,
  description:
    "Bridge task status payload object for PATCH /tasks/{task_id}/status. Include status transitions only for the assigned review task.",
} as const;
const ProjectEventSchema = {
  ...JsonObjectSchema,
  description:
    "Bridge project event payload object. Include event/event_type plus review-stage details; this tool maps legacy fields to the bridge schema.",
} as const;
...
export const GESAHNI_REVIEWER_TOOL_NAMES = [
...
export type GesahniReviewerToolName = (typeof GESAHNI_REVIEWER_TOOL_NAMES)[number];
...
const TOOL_SPECS: readonly ToolSpec[] = [
  ...
  },
  {
    ...
    description:
      "Create a review_notes artifact for the current project. Artifact payload must satisfy the review contract and must not drift into approval or deploy behavior.",
    method: "POST",
    auth: "write",
  ...
  },
  {
    ...
    description:
      "Attach outputs to the assigned review task only. Keep payload narrow and include the created artifact id.",
    method: "POST",
    auth: "write",
  ...
  },
  {
    ...
    description: "Update status only for the assigned review task.",
    method: "PATCH",
    auth: "write",
  ...
  },
  {
    ...
    description: "Append a project event tied to the review completion flow.",
    method: "POST",
    auth: "write",
...
function readStringArray(
  ...
  for (const entry of value) {
    if (typeof entry !== "string" || !entry.trim()) {
      throw new Error(`${errorPrefix}.${key} must be non-empty strings`);
    }
...
function readStringEnum<T extends string>(
  ...
  if (typeof value !== "string" || !allowed.includes(value as T)) {
    throw new Error(`${errorPrefix}.${key} invalid`);
  }
  return value as T;
}
...
function readFindings(
  ...
  const value = object[key];
  if (!Array.isArray(value)) {
    throw new Error(`artifact.content_json.${key} required`);
  }

  return value.map((entry, index) => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      throw new Error(`artifact.content_json.${key}[${index}] invalid`);
    }
    const finding = entry as Record<string, unknown>;
    return {
      severity: readStringEnum(
        finding,
        "severity",
        ["info", "warning", "blocking"],
        `artifact.content_json.${key}[${index}]`,
      ),
      area: readStringEnum(
        finding,
        "area",
        ["content", "structure", "implementation", "readiness", "other"],
        `artifact.content_json.${key}[${index}]`,
      ),
      note: readRequiredStringField(finding, "note", `artifact.content_json.${key}[${index}]`),
    };
  });
}
...
function assertMatchingId(
  ...
  if (typeof value !== "string" || value.trim() !== expected) {
    throw new Error(`artifact.content_json.${key} mismatch`);
  }
}
...
function buildCreateArtifactBody(params: ToolArgs) {
  ...
  if (artifactType !== undefined) {
    if (typeof artifactType !== "string" || artifactType.trim() !== "review_notes") {
      throw new Error("artifact.artifact_type must be review_notes");
    }
  }
  ...
  assertMatchingId(contentJson, "project_id", projectId);
  assertMatchingId(contentJson, "task_id", taskId);

  const buildArtifactId = readRequiredStringField(
    contentJson,
    "build_artifact_id",
  ...
  const summaryFromContentJson =
    typeof contentJson.summary === "string" ? contentJson.summary.trim() : "";
  ...
  const previewJson = {
    ...
    task_id: taskId,
    build_artifact_id: buildArtifactId,
    summary,
    findings: readFindings(contentJson, "findings"),
    strengths: readStringArray(contentJson, "strengths", "artifact.content_json"),
    issues: readStringArray(contentJson, "issues", "artifact.content_json"),
    revision_requests: readStringArray(contentJson, "revision_requests", "artifact.content_json"),
    approval_recommendation: readStringEnum(
      contentJson,
      "approval_recommendation",
      ["approve", "revise", "blocked"],
      "artifact.content_json",
    ) as ApprovalRecommendation,
    recommended_next_step: readStringEnum(
      contentJson,
      "recommended_next_step",
      ["approval", "revision", "preview_ready", "blocked"],
      "artifact.content_json",
    ) as RecommendedNextStep,
  };
...
function buildRequestBody(spec: ToolSpec, params: ToolArgs): unknown {
  ...
  if (spec.name === "append_project_event") {
    ...
    const taskId = readRequiredStringField(event, "task_id", "event");
    ...
    for (const [key, value] of Object.entries(event)) {
      ...
      if (!allowedPayloadKeys.has(key)) {
        throw new Error(`event.${key} not allowed`);
      }
      payload[key] = value;
    }
...
function buildBridgeResult(
  payload: Awaited<ReturnType<ReturnType<typeof createGesahniReviewerClient>["request"]>>,
) {
  if (payload.ok) {
