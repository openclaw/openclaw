---
description: Update Neobot from upstream when branch has diverged (ahead/behind)
---

# Neobot Upstream Sync Workflow
...
# Move old version
mv /Applications/Neobot.app /tmp/Neobot-backup.app

# Install new build
cp -R dist/Neobot.app /Applications/

# Launch
open /Applications/Neobot.app
```
...
ps aux | grep -E "(neobot|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm neobot agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
morph-mcp_warpgrep_codebase_search search_string="Find deprecated FileManager.default and Thread.isMainThread usages in Swift files" repo_path="/Volumes/Main SSD/Developer/neois"
morph-mcp_warpgrep_codebase_search search_string="Locate Peekaboo submodule and macOS app Swift files with concurrency issues" repo_path="/Volumes/Main SSD/Developer/neois"
```
...
cd /Volumes/Main SSD/Developer/neois
...
# Update neobot.json with fallback chains
...
morph-mcp_warpgrep_codebase_search search_string="Find all Swift files using deprecated FileManager.default or Thread.isMainThread" repo_path="/Volumes/Main SSD/Developer/neois"
```
...
pnpm neobot agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
