---
description: Update DNA from upstream when branch has diverged (ahead/behind)
---

# DNA Upstream Sync Workflow
...
# Move old version
mv /Applications/DNA.app /tmp/DNA-backup.app

# Install new build
cp -R dist/DNA.app /Applications/

# Launch
open /Applications/DNA.app
```
...
ps aux | grep -E "(dna|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm dna agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update dna.json with fallback chains
...
pnpm dna agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
