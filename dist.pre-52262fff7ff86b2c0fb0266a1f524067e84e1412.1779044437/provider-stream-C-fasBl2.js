...
import { M as resolveProviderStreamFn } from "./provider-runtime-DrNAJwj9.js";
import { n as resolveProviderEndpoint } from "./provider-attribution-BKdrgqZp.js";
import { r as getModelProviderRequestTransport } from "./provider-request-config-Q8oB6bbT.js";
import { t as MALFORMED_STREAMING_FRAGMENT_ERROR_MESSAGE } from "./assistant-error-format-xfd465ce.js";
	...
	for (let i = 0; i < transformedMessages.length; i += 1) {
		...
		if (msg.role === "assistant") {
			...
			if (blocks.length > 0) {
				...
				else if (allowReasoningContentReplay) assistantMsg.reasoning_content = "";
...
function buildAnthropicParams(model, context, isOAuthToken, options) {
	...
	const params = {
		model: model.id,
		messages: ensureNonEmptyAnthropicMessages(convertAnthropicMessages(context.messages, model, isOAuthToken, { allowReasoningContentReplay: supportsReasoningContentReplay(model) && options?.thinkingEnabled === true })),
	...
	return (rawModel, context, rawOptions) => {
		...
		(async () => {
			...
			try {
				...
				const allowReasoningContentReplay = supportsReasoningContentReplay(model) && transportOptions.thinkingEnabled === true;
