...
### Q12: 记忆提取是全自动的吗？实体识别怎么做的？

**问**：OpenClaw 有后台进程自动扫描对话并提取实体（Entity）吗？
**答**：**不是全自动，而是 LLM 驱动的半自动机制。**

- **机制**：没有独立的 NLP/NER 模型在后台跑。
- **触发**：主要依赖 **System Prompt 指令** + **Pre-Compaction Flush（静默回合）**。
- **流程**：
  1.  **指令驱动**：System Prompt 明确告知："Decisions, preferences... go to MEMORY.md."
  2.  **自我回顾**：在对话中或压缩前，LLM 回顾上下文。
  3.  **In-Context Learning**：LLM 利用自身的通用能力识别出"这是一个人名"或"这是一个重要决策"，然后主动调用写入工具。
- **结论**：智能体既是思考者，也是记录员。没有上帝视角的第三方进程。
