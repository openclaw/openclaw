...
5. Direct chats: ပို့ရာတွင် `user:<id>` (သို့မဟုတ် `<@id>` mention) ကို အသုံးပြုပါ။ အပြန်အလှန်အားလုံးသည် မျှဝေထားသော `main` session သို့ ဝင်ရောက်ပါမည်။ သာမန် နံပါတ် ID များသည် မရှင်းလင်းသဖြင့် ငြင်းပယ်ပါမည်။
6. Guild ချန်နယ်များ: ပို့ရာတွင် `channel:<channelId>` ကို အသုံးပြုပါ။ မူလအားဖြင့် mention လိုအပ်ပြီး guild သို့မဟုတ် ချန်နယ်အလိုက် သတ်မှတ်နိုင်သည်။
7. Direct chats: `channels.discord.dm.policy` ဖြင့် မူလအားဖြင့် လုံခြုံစေထားသည် (default: `"pairing"`)။ မသိသော ပို့သူများသည် pairing code ရရှိမည် (၁ နာရီအတွင်း သက်တမ်းကုန်)။ `openclaw pairing approve discord <code>` ဖြင့် အတည်ပြုပါ။
...
10. Optional native commands: `commands.native` သည် မူလအားဖြင့် `"auto"` ဖြစ်သည် (Discord/Telegram အတွက် on၊ Slack အတွက် off)။ `channels.discord.commands.native: true|false|"auto"` ဖြင့် override လုပ်နိုင်ပြီး `false` သည် ယခင် register လုပ်ထားသော commands များကို ရှင်းလင်းသည်။ Text commands များကို `commands.text` ဖြင့် ထိန်းချုပ်ပြီး သီးခြား `/...` မက်ဆေ့ချ်များအဖြစ် ပို့ရပါမည်။ Command များအတွက် access-group စစ်ဆေးမှုကို ကျော်လွှားလိုပါက `commands.useAccessGroups: false` ကို အသုံးပြုပါ။
    - Command အပြည့်အစုံ + config: [Slash commands](/tools/slash-commands)
11. Optional guild context history: mention ကို ပြန်ကြားရာတွင် နောက်ဆုံး N ခု guild မက်ဆေ့ချ်များကို context အဖြစ် ထည့်သွင်းရန် `channels.discord.historyLimit` (default 20၊ `messages.groupChat.historyLimit` သို့ fallback) ကို သတ်မှတ်ပါ။ ပိတ်ရန် `0` ကို သတ်မှတ်ပါ။
...
မှတ်ချက်: အမည် → id ဖြေရှင်းမှုသည် guild member search ကို အသုံးပြု၍ Server Members Intent လိုအပ်ပါသည်။ ဘော့တ်က member များကို မရှာနိုင်ပါက id များ သို့မဟုတ် `<@id>` mentions ကို အသုံးပြုပါ။
မှတ်ချက်: Slug များသည် အောက်ခြေစာလုံးဖြစ်ပြီး space များကို `-` ဖြင့် အစားထိုးထားသည်။ Channel အမည်များကို ဦးဆောင်သော `#` မပါဘဲ slug ပြုလုပ်ထားသည်။
မှတ်ချက်: Guild context `[from:]` စာကြောင်းများတွင် ping အဆင်သင့် ပြန်ကြားနိုင်ရန် `author.tag` + `id` ပါဝင်သည်။
...
ပုံမှန်အားဖြင့် **Presence Intent** မလိုအပ်ပါ။ ဘော့တ်၏ ကိုယ်ပိုင် presence (`setPresence` action) ကို သတ်မှတ်ခြင်းသည် gateway OP3 ကို အသုံးပြုပြီး ဤ intent မလိုအပ်ပါ။ အခြား guild members များ၏ presence updates များကို လက်ခံလိုမှသာ လိုအပ်ပါသည်။
...
Multi-account ပံ့ပိုးမှု: per-account tokens နှင့် optional `name` ဖြင့် `channels.discord.accounts` ကို အသုံးပြုပါ။ မျှဝေထားသော pattern အတွက် [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) ကို ကြည့်ပါ။
...
- Threads များသည် parent channel config (allowlist, `requireMention`, skills, prompts စသည်) ကို အမွေဆက်ခံပါသည်။ Thread channel id ကို သီးခြား ထည့်သွင်းမထားလျှင်။
- Owner hint: per-guild သို့မဟုတ် per-channel `users` allowlist သည် ပို့သူနှင့် ကိုက်ညီပါက OpenClaw သည် ထိုပို့သူကို system prompt တွင် owner အဖြစ် သတ်မှတ်ပါသည်။ ချန်နယ်အနှံ့ global owner တစ်ဦးအတွက် `commands.ownerAllowFrom` ကို သတ်မှတ်ပါ။
...
- သတိပေးချက်: အခြား bots များသို့ ပြန်ကြားခွင့် (`channels.discord.allowBots=true`) ပေးပါက bot-to-bot reply loops မဖြစ်စေရန် `requireMention`, `channels.discord.guilds.*.channels.<id>.users` allowlists နှင့်/သို့မဟုတ် `AGENTS.md` နှင့် `SOUL.md` တွင် guardrails များကို ရှင်းလင်းထားပါ။
...
- `channels.discord.groupPolicy` သည် မူလအားဖြင့် **allowlist** ဖြစ်သည်; `"open"` သို့ သတ်မှတ်ပါ သို့မဟုတ် `channels.discord.guilds` အောက်တွင် guild entry တစ်ခု ထည့်ပါ (လိုအပ်ပါက `channels.discord.guilds.<id>.channels` အောက်တွင် channels များကို ကန့်သတ်နိုင်သည်)။
  - `DISCORD_BOT_TOKEN` ကိုသာ သတ်မှတ်ပြီး `channels.discord` အပိုင်းကို မဖန်တီးခဲ့ပါက runtime သည်
    `groupPolicy` ကို `open` အဖြစ် default သတ်မှတ်မည်။ `channels.discord.groupPolicy`,
    `channels.defaults.groupPolicy` သို့မဟုတ် guild/channel allowlist တစ်ခု ထည့်၍ လုံခြုံစေပါ။
- `requireMention` သည် `channels.discord.guilds` အောက် (သို့မဟုတ် သီးခြား channel) တွင်သာ ရှိရပါမည်။ Top level တွင် `channels.discord.requireMention` သည် လျစ်လျူရှုပါသည်။
- **Permission audits** (`channels status --probe`) သည် နံပါတ် channel IDs များကိုသာ စစ်ဆေးပါသည်။ Slugs/အမည်များကို `channels.discord.guilds.*.channels` keys အဖြစ် အသုံးပြုပါက audit သည် ခွင့်ပြုချက်များကို မစစ်နိုင်ပါ။
...
- **Discord တွင် Exec approvals**: Discord သည် DMs တွင် exec approvals အတွက် **button UI** ကို ပံ့ပိုးပါသည် (Allow once / Always allow / Deny)။ `/approve <id> ...` သည် forwarded approvals အတွက်သာ ဖြစ်ပြီး Discord ၏ button prompts များကို မဖြေရှင်းနိုင်ပါ။ `❌ Failed to submit approval: Error: unknown approval id` ကို တွေ့ပါက သို့မဟုတ် UI မပေါ်လာပါက အောက်ပါအချက်များကို စစ်ဆေးပါ:
...
Outbound Discord API calls များသည် rate limits (429) တွင် Discord `retry_after` ကို ရရှိနိုင်ပါက အသုံးပြုပြီး exponential backoff နှင့် jitter ဖြင့် retry လုပ်ပါသည်။ `channels.discord.retry` ဖြင့် ဖွဲ့စည်းပြင်ဆင်နိုင်သည်။ [Retry policy](/concepts/retry) ကို ကြည့်ပါ။
...
Ack reactions များကို global အနေဖြင့် `messages.ackReaction` +
`messages.ackReactionScope` ဖြင့် ထိန်းချုပ်ပါသည်။ ဘော့တ် ပြန်ကြားပြီးနောက်
...
- `dm.policy`: DM access control (`pairing` ကို အကြံပြု)။ `"open"` သည် `dm.allowFrom=["*"]` လိုအပ်သည်။
- `dm.allowFrom`: DM allowlist (user ids သို့မဟုတ် အမည်များ)။ `dm.policy="allowlist"` နှင့် `dm.policy="open"` validation အတွက် အသုံးပြုသည်။ Wizard သည် usernames များကို လက်ခံပြီး ဘော့တ်က members များကို ရှာနိုင်ပါက ids သို့ ဖြေရှင်းပါသည်။
...
- `guilds.<id>.slug`: ပြသအမည်အတွက် optional friendly slug။
- `guilds.<id>.users`: optional per-guild user allowlist (ids သို့မဟုတ် အမည်များ)။
- `guilds.<id>.tools`: channel override မရှိပါက အသုံးပြုမည့် optional per-guild tool policy overrides (`allow`/`deny`/`alsoAllow`)။
- `guilds.<id>.toolsBySender`: guild အဆင့်တွင် per-sender tool policy overrides (channel override မရှိပါက အသုံးပြု; `"*"` wildcard ကို ပံ့ပိုး)။
- `guilds.<id>.channels.<channel>.allow`: `groupPolicy="allowlist"` ဖြစ်သည့်အခါ channel ကို ခွင့်ပြု/ငြင်းပယ်။
- `guilds.<id>.channels.<channel>.requireMention`: channel အတွက် mention gating။
- `guilds.<id>.channels.<channel>.tools`: optional per-channel tool policy overrides (`allow`/`deny`/`alsoAllow`)။
- `guilds.<id>.channels.<channel>.toolsBySender`: channel အတွင်း per-sender tool policy overrides (`"*"` wildcard ကို ပံ့ပိုး)။
- `guilds.<id>.channels.<channel>.users`: optional per-channel user allowlist။
- `guilds.<id>.channels.<channel>.skills`: skill filter (မဖော်ပြပါက = skills အားလုံး၊ empty = မရှိ)။
- `guilds.<id>.channels.<channel>.systemPrompt`: channel အတွက် extra system prompt။ Discord channel topics များကို **untrusted** context အဖြစ် ထည့်သွင်းပါသည် (system prompt မဟုတ်)။
- `guilds.<id>.channels.<channel>.enabled`: `false` ကို သတ်မှတ်ပါက channel ကို ပိတ်သည်။
- `guilds.<id>.channels`: channel စည်းမျဉ်းများ (keys များသည် channel slugs သို့မဟုတ် ids)။
- `guilds.<id>.requireMention`: per-guild mention requirement (per channel override လုပ်နိုင်)။
...
- `maxLinesPerMessage`: မက်ဆေ့ချ်တစ်ခုအတွက် soft max line count။ Default: 17။
...
- `dmHistoryLimit`: DM history limit (user turns)။ Per-user overrides: `dms["<user_id>"].historyLimit`။
...
- `execApprovals`: Discord-only exec approval DMs (button UI)။ `enabled`, `approvers`, `agentFilter`, `sessionFilter` ကို ပံ့ပိုးသည်။

Reaction notifications များသည် `guilds.<id>.reactionNotifications` ကို အသုံးပြုသည်:
...
- `allowlist`: မက်ဆေ့ချ်အားလုံးပေါ်ရှိ `guilds.<id>.users` မှ reactions (empty list သည် ပိတ်သည်)။

### PluralKit (PK) support

PK lookups ကို ဖွင့်ပြီး proxied messages များကို အခြေခံ system + member သို့ ဖြေရှင်းပါ။
ဖွင့်ထားပါက OpenClaw သည် allowlists အတွက် member identity ကို အသုံးပြုပြီး
မတော်တဆ Discord pings မဖြစ်စေရန် ပို့သူကို `Member (PK:System)` အဖြစ် label လုပ်ပါသည်။
...
- `dm.allowFrom`, `guilds.<id>.users` သို့မဟုတ် per-channel `users` တွင် `pk:<memberId>` ကို အသုံးပြုပါ။
...
- `replyToMode`: `off` (default), `first`, သို့မဟုတ် `all`။ Model တွင် reply tag ပါရှိသောအခါမှသာ အသုံးချပါသည်။
...
- `[[reply_to:<id>]]` — context/history မှ message id တစ်ခုကို reply လုပ်ပါ။
  လက်ရှိ message ids များကို `[message_id: …]` အဖြစ် prompt များတွင် ထည့်သွင်းထားပြီး history entries များတွင် ids ပါဝင်ပြီးသား ဖြစ်သည်။
...
- `guilds.<id>.channels` ရှိပါက စာရင်းမပါသော channels များကို မူလအားဖြင့် ငြင်းပယ်သည်။
- `guilds.<id>.channels` မပါပါက allowlisted guild အတွင်းရှိ channels အားလုံးကို ခွင့်ပြုသည်။
...
Discord message ids များကို injected context (`[discord message id: …]` နှင့် history lines) တွင် ဖော်ပြထားသဖြင့် agent သည် ၎င်းတို့ကို ဦးတည်နိုင်ပါသည်။
Emoji များကို unicode (ဥပမာ `✅`) သို့မဟုတ် `<:party_blob:1234567890>` ကဲ့သို့ custom emoji syntax ဖြင့် အသုံးပြုနိုင်ပါသည်။
