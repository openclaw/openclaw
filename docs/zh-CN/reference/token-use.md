...
完整分解参见[系统提示词](/zh-CN/concepts/system-prompt)。
...
有关实际分解（每个注入文件、工具、Skills 和系统提示词大小），使用 `/context list` 或 `/context detail`。参见[上下文](/zh-CN/concepts/context)。
...
精确的 skill 列表开销公式参见 [Skills](/zh-CN/tools/skills)。
