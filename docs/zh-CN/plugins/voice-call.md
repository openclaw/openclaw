...
通过插件为 EasyHub 提供语音通话。支持出站通知和带有入站策略的多轮对话。
...
- 使用 `EasyHub voicecall ...` 或 `voice_call` 工具
...
```bash
EasyHub plugins install @easyhub/voice-call
```
...
```bash
EasyHub plugins install ./extensions/voice-call
...
```bash
EasyHub voicecall call --to "+15555550123" --message "Hello from EasyHub"
EasyHub voicecall continue --call-id <id> --message "Any questions?"
EasyHub voicecall speak --call-id <id> --message "One moment"
EasyHub voicecall end --call-id <id>
EasyHub voicecall status --call-id <id>
EasyHub voicecall tail
