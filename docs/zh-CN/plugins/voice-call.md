...
通过插件为 SmartAgentNeo 提供语音通话。支持出站通知和带有入站策略的多轮对话。
...
- 使用 `smart-agent-neo voicecall ...` 或 `voice_call` 工具
...
```bash
smart-agent-neo plugins install @smart-agent-neo/voice-call
```
...
```bash
smart-agent-neo plugins install ./extensions/voice-call
...
```bash
smart-agent-neo voicecall call --to "+15555550123" --message "Hello from SmartAgentNeo"
smart-agent-neo voicecall continue --call-id <id> --message "Any questions?"
smart-agent-neo voicecall speak --call-id <id> --message "One moment"
smart-agent-neo voicecall end --call-id <id>
smart-agent-neo voicecall status --call-id <id>
smart-agent-neo voicecall tail
