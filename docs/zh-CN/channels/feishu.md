...
```bash
smart-agent-neo plugins install @smart-agent-neo/feishu
```

本地 checkout（在 git 仓库内运行）：

```bash
smart-agent-neo plugins install ./extensions/feishu
```
...
如果您刚安装完 SmartAgentNeo，可以直接运行向导，根据提示添加飞书：
...
- `smart-agent-neo gateway status` - 查看网关运行状态
- `smart-agent-neo logs --follow` - 查看实时日志
...
- `smart-agent-neo gateway status` - 查看网关运行状态
- `smart-agent-neo gateway restart` - 重启网关以应用新配置
- `smart-agent-neo logs --follow` - 查看实时日志
...
1. 运行 `smart-agent-neo channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `smart-agent-neo gateway status` 检查状态）
...
编辑 `~/.smart-agent-neo/smart-agent-neo.json`：
...
```bash
smart-agent-neo pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  smart-agent-neo pairing list feishu      # 查看待审批列表
  smart-agent-neo pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `smart-agent-neo logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `smart-agent-neo logs --follow` 查看日志中的 `open_id`

**方法二**：
查看配对请求列表，其中包含用户的 Open ID：

```bash
smart-agent-neo pairing list feishu
```
...
| -------------------------- | ----------------- |
| `smart-agent-neo gateway status`  | 查看网关运行状态  |
| `smart-agent-neo gateway install` | 安装/启动网关服务 |
| `smart-agent-neo gateway stop`    | 停止网关服务      |
| `smart-agent-neo gateway restart` | 重启网关服务      |
| `smart-agent-neo logs --follow`   | 实时查看日志输出  |
...
4. 查看日志：`smart-agent-neo logs --follow`
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`smart-agent-neo gateway status`
6. 查看实时日志：`smart-agent-neo logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "neo-fan",
        workspace: "/home/user/neo-fan",
        agentDir: "/home/user/.smart-agent-neo/agents/neo-fan/agent",
      },
      {
        id: "neo-xi",
        workspace: "/home/user/neo-xi",
        agentDir: "/home/user/.smart-agent-neo/agents/neo-xi/agent",
      },
    ],
  },
  bindings: [
