...
```bash
EasyHub plugins install @easyhub/feishu
```

本地 checkout（在 git 仓库内运行）：

```bash
EasyHub plugins install ./extensions/feishu
```
...
如果您刚安装完 EasyHub，可以直接运行向导，根据提示添加飞书：
...
- `EasyHub gateway status` - 查看网关运行状态
- `EasyHub logs --follow` - 查看实时日志
...
- `EasyHub gateway status` - 查看网关运行状态
- `EasyHub gateway restart` - 重启网关以应用新配置
- `EasyHub logs --follow` - 查看实时日志
...
1. 运行 `EasyHub channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `EasyHub gateway status` 检查状态）
...
```bash
EasyHub pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  EasyHub pairing list feishu      # 查看待审批列表
  EasyHub pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `EasyHub logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `EasyHub logs --follow` 查看日志中的 `open_id`

**方法二**：
查看配对请求列表，其中包含用户的 Open ID：

```bash
EasyHub pairing list feishu
```
...
| -------------------------- | ----------------- |
| `EasyHub gateway status`  | 查看网关运行状态  |
| `EasyHub gateway install` | 安装/启动网关服务 |
| `EasyHub gateway stop`    | 停止网关服务      |
| `EasyHub gateway restart` | 重启网关服务      |
| `EasyHub logs --follow`   | 实时查看日志输出  |
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`EasyHub gateway status`
6. 查看实时日志：`EasyHub logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.easyhub/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.easyhub/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
