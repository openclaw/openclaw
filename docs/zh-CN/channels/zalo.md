...
- 通过 CLI 安装：`smart-agent-neo plugins install @smart-agent-neo/zalo`
...
1. 安装 Zalo 插件：
   - 从源代码检出：`smart-agent-neo plugins install ./extensions/zalo`
   - 从 npm（如果已发布）：`smart-agent-neo plugins install @smart-agent-neo/zalo`
...
- 通过以下方式批准：
  - `smart-agent-neo pairing list zalo`
  - `smart-agent-neo pairing approve zalo <CODE>`
...
- 使用聊天 id 作为目标。
- 示例：`smart-agent-neo message send --channel zalo --target 123456789 --message "hi"`。

## 故障排除

**Bot 不响应：**

- 检查 token 是否有效：`smart-agent-neo channels status --probe`
- 验证发送者已被批准（配对或 allowFrom）
- 检查 Gateway 网关日志：`smart-agent-neo logs --follow`
