...
1. 创建专用 Apple ID（例如：`[redacted-email]`）。
...
```bash
#!/usr/bin/env bash
exec ssh -T [redacted-email] imsg "$@"
```

注意事项：

- 确保 Mac 已登录"信息"，并已启用远程登录。
- 使用 SSH 密钥使 `ssh [redacted-email]` 无需提示即可工作。
...
- `chat_guid:...`
- `chat_identifier:...`
- 直接 handle：`imessage:+1555` / `sms:+1555` / `[redacted-email]`
