---
read_when:
  - 为 SmartAgentNeo 设置 Zalo Personal
...
- 通过 CLI 安装：`smart-agent-neo plugins install @smart-agent-neo/zalouser`
- 或从源码检出安装：`smart-agent-neo plugins install ./extensions/zalouser`
- 详情：[插件](/tools/plugin)
...
- `smart-agent-neo pairing list zalouser`
- `smart-agent-neo pairing approve zalouser <code>`
...
- 配置向导可以提示输入群组允许列表。
- 启动时，SmartAgentNeo 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`smart-agent-neo channels logout --channel zalouser && smart-agent-neo channels login --channel zalouser`
