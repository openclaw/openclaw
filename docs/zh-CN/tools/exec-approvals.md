...
- **gateway 主机** → gateway 机器上的 `mullusi` 进程
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.mullusi/exec-approvals.sock",
...
如果节点尚未通告执行审批，请直接编辑其本地的 `~/.mullusi/exec-approvals.json`。

CLI：`mullusi approvals` 支持 gateway 或 node 编辑（参见 [Approvals CLI](/cli/approvals)）。
