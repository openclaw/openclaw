...
read_when:
  - 添加或修改智能体 CLI 入口点
summary: 直接 `mullusi agent` CLI 运行（带可选投递）
title: Agent Send
x-i18n:
...
# `mullusi agent`（直接智能体运行）

`mullusi agent` 运行单个智能体回合，无需入站聊天消息。
...
- 输出：
  ...
  - `--json`：打印结构化负载 + 元数据
- 可选使用 `--deliver` + `--channel` 将回复投递回渠道（目标格式与 `mullusi message --target` 匹配）。
...
```bash
mullusi agent --to +15555550123 --message "status update"
mullusi agent --agent ops --message "Summarize logs"
mullusi agent --session-id 1234 --message "Summarize inbox" --thinking medium
mullusi agent --to +15555550123 --message "Trace logs" --verbose on --json
mullusi agent --to +15555550123 --message "Summon reply" --deliver
mullusi agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
```
