...
read_when:
  - 添加或修改智能体 CLI 入口点
summary: 直接 `smart-agent-neo agent` CLI 运行（带可选投递）
title: Agent Send
x-i18n:
...
# `smart-agent-neo agent`（直接智能体运行）

`smart-agent-neo agent` 运行单个智能体回合，无需入站聊天消息。
...
- 输出：
  ...
  - `--json`：打印结构化负载 + 元数据
- 可选使用 `--deliver` + `--channel` 将回复投递回渠道（目标格式与 `smart-agent-neo message --target` 匹配）。
...
```bash
smart-agent-neo agent --to +15555550123 --message "status update"
smart-agent-neo agent --agent ops --message "Summarize logs"
smart-agent-neo agent --session-id 1234 --message "Summarize inbox" --thinking medium
smart-agent-neo agent --to +15555550123 --message "Trace logs" --verbose on --json
smart-agent-neo agent --to +15555550123 --message "Summon reply" --deliver
smart-agent-neo agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
```
