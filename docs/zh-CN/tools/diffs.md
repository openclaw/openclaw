---
title: "Diffs"
summary: "只读差异查看器和文件渲染器（可选插件工具）"
description: "使用可选的 Diffs 插件将修改前后的文本或统一补丁渲染为网关托管的差异视图、文件（PNG 或 PDF），或两者兼有。"
read_when:
  - 您希望代理以差异形式显示代码或 Markdown 编辑
  - 您需要可嵌入 Canvas 的查看器 URL 或渲染的差异文件
  - 您需要受控的、临时的差异文件，且具有安全的默认设置
---

# Diffs 差异工具

`diffs` 是一个可选的插件工具，将变更内容转换为代理的只读差异文件。
...
- 用于 Canvas 展示的网关查看器 URL
- 渲染的文件路径（PNG 或 PDF）用于消息传递
- 一次调用返回两种输出
