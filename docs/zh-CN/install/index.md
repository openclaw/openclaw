...
read_when:
  - 安装 Activi
  - 你想从 GitHub 安装
summary: 安装 Activi（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
```bash
curl -fsSL https://activi.ai/install.sh | bash -s -- --help
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g activi@latest
```
...
pnpm approve-builds -g                # 批准 activi、node-llama-cpp、sharp 等
pnpm add -g activi@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm activi ...` 运行仓库命令。
...
- 运行新手引导：`activi onboard --install-daemon`
- 快速检查：`activi doctor`
- 检查 Gateway 网关健康状态：`activi status` + `activi health`
- 打开仪表板：`activi dashboard`

## 安装方式：npm vs git（安装器）

安装器支持两种方式：

- `npm`（默认）：`npm install -g activi@latest`
...
```bash
# 显式 npm
curl -fsSL https://activi.ai/install.sh | bash -s -- --install-method npm

# 从 GitHub 安装（源代码 checkout）
curl -fsSL https://activi.ai/install.sh | bash -s -- --install-method git
```

常用标志：

- `--install-method npm|git`
- `--git-dir <path>`（默认：`~/activi`）
...
- `ACTIVI_INSTALL_METHOD=git|npm`
- `ACTIVI_GIT_DIR=...`
- `ACTIVI_GIT_UPDATE=0|1`
- `ACTIVI_NO_PROMPT=1`
- `ACTIVI_DRY_RUN=1`
- `ACTIVI_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `activi`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `activi`）。
