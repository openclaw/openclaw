...
read_when:
  - 安装 SmartAgentNeo
  - 你想从 GitHub 安装
summary: 安装 SmartAgentNeo（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
通过 npm 全局安装 `smart-agent-neo` 并运行新手引导。
...
```bash
curl -fsSL https://smart-agent-neo.ai/install.sh | bash -s -- --help
```
...
```bash
curl -fsSL https://smart-agent-neo.ai/install.sh | bash -s -- --no-onboard
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g smart-agent-neo@latest
```
...
pnpm approve-builds -g                # 批准 smart-agent-neo、node-llama-cpp、sharp 等
pnpm add -g smart-agent-neo@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm smart-agent-neo ...` 运行仓库命令。
...
- 运行新手引导：`smart-agent-neo onboard --install-daemon`
- 快速检查：`smart-agent-neo doctor`
- 检查 Gateway 网关健康状态：`smart-agent-neo status` + `smart-agent-neo health`
- 打开仪表板：`smart-agent-neo dashboard`

## 安装方式：npm vs git（安装器）

安装器支持两种方式：

- `npm`（默认）：`npm install -g smart-agent-neo@latest`
...
```bash
# 显式 npm
curl -fsSL https://smart-agent-neo.ai/install.sh | bash -s -- --install-method npm

# 从 GitHub 安装（源代码 checkout）
curl -fsSL https://smart-agent-neo.ai/install.sh | bash -s -- --install-method git
```

常用标志：

- `--install-method npm|git`
- `--git-dir <path>`（默认：`~/smart-agent-neo`）
...
- `SMART_AGENT_NEO_INSTALL_METHOD=git|npm`
- `SMART_AGENT_NEO_GIT_DIR=...`
- `SMART_AGENT_NEO_GIT_UPDATE=0|1`
- `SMART_AGENT_NEO_NO_PROMPT=1`
- `SMART_AGENT_NEO_DRY_RUN=1`
- `SMART_AGENT_NEO_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `smart-agent-neo`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `smart-agent-neo`）。
