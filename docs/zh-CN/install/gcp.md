---
read_when:
  - 你想在 GCP 上 24/7 运行 Mullusi
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 Mullusi Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 Mullusi（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 Mullusi Gateway 网关，具有持久状态、内置二进制文件和安全的重启行为。

如果你想要"Mullusi 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
- 安装 Docker（隔离的应用运行时）
- 在 Docker 中启动 Mullusi Gateway 网关
- 在主机上持久化 `~/.mullusi` + `~/.mullusi/workspace`（重启/重建后仍保留）
...
```bash
gcloud projects create my-mullusi-project --name="Mullusi Gateway"
gcloud config set project my-mullusi-project
```
...
```bash
gcloud compute instances create mullusi-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh mullusi-gateway --zone=us-central1-a
```
...
```bash
MULLUSI_IMAGE=mullusi:latest
MULLUSI_GATEWAY_TOKEN=change-me-now
MULLUSI_GATEWAY_BIND=lan
MULLUSI_GATEWAY_PORT=18790

MULLUSI_CONFIG_DIR=/home/$USER/.mullusi
MULLUSI_WORKSPACE_DIR=/home/$USER/.mullusi/workspace

GOG_KEYRING_PASSWORD=change-me-now
XDG_CONFIG_HOME=/home/node/.mullusi
```
...
services:
  mullusi-gateway:
    ...
    environment:
      ...
      - NODE_ENV=production
      - TERM=xterm-256color
      - MULLUSI_GATEWAY_BIND=${MULLUSI_GATEWAY_BIND}
      - MULLUSI_GATEWAY_PORT=${MULLUSI_GATEWAY_PORT}
      - MULLUSI_GATEWAY_TOKEN=${MULLUSI_GATEWAY_TOKEN}
    ...
    ports:
      ...
      - "127.0.0.1:${MULLUSI_GATEWAY_PORT}:18790"
...
```bash
gcloud compute ssh mullusi-gateway --zone=us-central1-a -- -L 18790:127.0.0.1:18790
```
...
Mullusi 在 Docker 中运行，但 Docker 不是真实来源。
所有长期状态必须在重启、重建和重启后仍然存在。

| 组件             | 位置                              | 持久化机制    | 说明                        |
| ---------------- | --------------------------------- | ------------- | --------------------------- |
| Gateway 网关配置 | `/home/node/.mullusi/`           | 主机卷挂载    | 包括 `mullusi.json`、令牌  |
| 模型认证配置文件 | `/home/node/.mullusi/`           | 主机卷挂载    | OAuth 令牌、API 密钥        |
| Skill 配置       | `/home/node/.mullusi/skills/`    | 主机卷挂载    | Skill 级别状态              |
| 智能体工作区     | `/home/node/.mullusi/workspace/` | 主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话    | `/home/node/.mullusi/`           | 主机卷挂载    | 保留 QR 登录                |
| Gmail 密钥环     | `/home/node/.mullusi/`           | 主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
...
```bash
# 首先停止 VM
gcloud compute instances stop mullusi-gateway --zone=us-central1-a

# 更改机器类型
gcloud compute instances set-machine-type mullusi-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# 启动 VM
gcloud compute instances start mullusi-gateway --zone=us-central1-a
```
...
1. 创建服务账户：
   ...
   ```bash
   gcloud iam service-accounts create mullusi-deploy \
     --display-name="Mullusi Deployment"
   ```

2. 授予 Compute Instance Admin 角色（或更窄的自定义角色）：
   ```bash
   gcloud projects add-iam-policy-binding my-mullusi-project \
     --member="serviceAccount:mullusi-deploy@my-mullusi-project.iam.gserviceaccount.com" \
