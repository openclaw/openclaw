...
当你使用 `--channel` **显式**切换渠道时，SmartAgentNeo 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/smart-agent-neo`，可通过 `SMART_AGENT_NEO_GIT_DIR` 覆盖），
...
当你使用 `smart-agent-neo update` 切换渠道时，SmartAgentNeo 还会同步插件来源：
