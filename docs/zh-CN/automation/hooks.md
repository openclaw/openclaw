...
- 在发出 `/new` 或 `/reset` 时将会话上下文保存到工作区笔记
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
...
- **💾 session-memory**：当你发出 `/new` 或 `/reset` 时将会话上下文保存到智能体工作区（默认 `~/.openclaw/workspace/memory/`）
...
当你发出 `/new` 或 `/reset` 时将会话上下文保存到工作区笔记。

**事件**：`command:new`、`command:reset`

**要求**：必须配置 `workspace.dir`

**输出**：默认写入 `<workspace>/memory/YYYY-MM-DD-slug.md`，也可以配置为其他工作区内相对目录，例如 `<workspace>/session-archive/YYYY-MM-DD-slug.md`

**功能**：

1. 使用预重置会话条目定位正确的记录
2. 提取最后 15 条 user/assistant 消息
3. 使用 LLM 生成描述性文件名 slug
4. 将会话元数据保存到配置目录下的带日期 Markdown 文件
...
`outputDir` 必须是工作区根目录下的相对路径。非法值会回退到 `memory`。
