---
read_when:
  - 你想在 SmartAgentNeo 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 SmartAgentNeo 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
smart-agent-neo plugins enable minimax-portal-auth  # 如果已加载则跳过
smart-agent-neo gateway restart  # 如果 Gateway 网关已在运行则重启
smart-agent-neo onboard --auth-choice minimax-portal
```
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/betterbrand/smart-agent-neo/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `smart-agent-neo models list` 和 `smart-agent-neo models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `smart-agent-neo configure` 并选择 **MiniMax M2.1**，或
