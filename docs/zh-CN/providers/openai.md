---
read_when:
  - 你想在 SmartAgentNeo 中使用 OpenAI 模型
  - 你想使用 Codex 订阅认证而非 API 密钥
summary: 在 SmartAgentNeo 中通过 API 密钥或 Codex 订阅使用 OpenAI
title: OpenAI
x-i18n:
...
```bash
smart-agent-neo onboard --auth-choice openai-api-key
# 或非交互式
smart-agent-neo onboard --openai-api-key "$OPENAI_API_KEY"
```
...
```bash
# 在向导中运行 Codex OAuth
smart-agent-neo onboard --auth-choice openai-codex

# 或直接运行 OAuth
smart-agent-neo models auth login --provider openai-codex
```
