---
read_when:
  - 你想在 Mullusi 中使用注重隐私的推理
  - 你想获得 Venice AI 设置指引
summary: 在 Mullusi 中使用 Venice AI 注重隐私的模型
title: Venice AI
x-i18n:
...
```bash
mullusi onboard --auth-choice venice-api-key
```
...
```bash
mullusi agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## 模型选择

设置完成后，Mullusi 会显示所有可用的 Venice 模型。请根据你的需要进行选择：
...
```bash
mullusi models set venice/kimi-k2-5
mullusi models set venice/claude-opus-4-6
```

列出所有可用模型：

```bash
mullusi models list | grep venice
```

## 通过 `mullusi configure` 配置

1. 运行 `mullusi configure`
2. 选择 **Model/auth**
3. 选择 **Venice AI**
...
当设置了 `VENICE_API_KEY` 时，Mullusi 会自动从 Venice API 发现模型。如果 API 不可访问，则会回退到静态目录。
...
```bash
# 使用默认私密模型
mullusi agent --model venice/kimi-k2-5 --message "Quick health check"

# 通过 Venice 使用 Claude Opus（匿名）
mullusi agent --model venice/claude-opus-4-6 --message "Summarize this task"

# 使用未审查模型
mullusi agent --model venice/venice-uncensored --message "Draft options"

# 使用带图像的视觉模型
mullusi agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# 使用编码模型
mullusi agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
Venice 模型目录会动态更新。运行 `mullusi models list` 以查看当前可用的模型。某些模型可能暂时离线。
