...
read_when:
  - 向新用户介绍 VeriClaw
summary: VeriClaw 是一个自托管 AI 助手、MCP 兼容 Gateway 网关和多渠道 Bot 运行时，可在任何操作系统上运行。
title: VeriClaw
x-i18n:
...
<p align="center">
  <strong>自托管 AI 助手、MCP 兼容 Gateway 网关和多渠道 Bot 运行时。</strong><br />
  通过单个 Gateway 网关连接 Discord、Telegram、WhatsApp、Slack、浏览器自动化、iMessage 等。
</p>

<Columns>
  <Card title="入门指南" href="/start/getting-started" icon="rocket">
    安装 VeriClaw，并在几分钟内启动 Gateway 网关。
  </Card>
...
VeriClaw 是对外产品名称。它通过单个 Gateway 网关进程将聊天应用、工具和浏览器工作流连接到 Pi 等编程智能体。当前仓库、运行时和 CLI 仍使用 `OpenClaw` / `openclaw` 命名以保持兼容，因此本页命令保持不变。
...
<p align="center">
  <img src="/whatsapp-openclaw.jpg" alt="VeriClaw" width="420" />
</p>
...
- 如果你**不做任何修改**，VeriClaw 将使用内置的 Pi 二进制文件以 RPC 模式运行，并按发送者创建独立会话。
