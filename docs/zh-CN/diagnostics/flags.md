...
```bash
MULLUSI_DIAGNOSTICS=telegram.http,telegram.payload
```

禁用所有标志：

```bash
MULLUSI_DIAGNOSTICS=0
```
...
```
/tmp/mullusi/mullusi-YYYY-MM-DD.log
```
...
```bash
ls -t /tmp/mullusi/mullusi-*.log | head -n 1
```

过滤 Telegram HTTP 诊断：

```bash
rg "telegram http error" /tmp/mullusi/mullusi-*.log
```

或在复现时使用 tail：

```bash
tail -f /tmp/mullusi/mullusi-$(date +%F).log | rg "telegram http error"
```

对于远程 Gateway 网关，你也可以使用 `mullusi logs --follow`（参见 [/cli/logs](/cli/logs)）。
