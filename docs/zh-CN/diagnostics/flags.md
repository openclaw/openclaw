...
```bash
ACTIVI_DIAGNOSTICS=telegram.http,telegram.payload
```

禁用所有标志：

```bash
ACTIVI_DIAGNOSTICS=0
```
...
```
/tmp/activi/activi-YYYY-MM-DD.log
```
...
```bash
ls -t /tmp/activi/activi-*.log | head -n 1
```

过滤 Telegram HTTP 诊断：

```bash
rg "telegram http error" /tmp/activi/activi-*.log
```

或在复现时使用 tail：

```bash
tail -f /tmp/activi/activi-$(date +%F).log | rg "telegram http error"
```

对于远程 Gateway 网关，你也可以使用 `activi logs --follow`（参见 [/cli/logs](/cli/logs)）。
