...
你可以在 `~/.activi/activi.json` 中覆盖此设置：
...
所有日志配置都在 `~/.activi/activi.json` 的 `logging` 下。
...
- Activi 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `activi plugins enable diagnostics-otel` 启用插件。
...
- `activi.tokens`（计数器，属性：`activi.token`、`activi.channel`、`activi.provider`、`activi.model`）
- `activi.cost.usd`（计数器，属性：`activi.channel`、`activi.provider`、`activi.model`）
- `activi.run.duration_ms`（直方图，属性：`activi.channel`、`activi.provider`、`activi.model`）
- `activi.context.tokens`（直方图，属性：`activi.context`、`activi.channel`、`activi.provider`、`activi.model`）

消息流：

- `activi.webhook.received`（计数器，属性：`activi.channel`、`activi.webhook`）
- `activi.webhook.error`（计数器，属性：`activi.channel`、`activi.webhook`）
- `activi.webhook.duration_ms`（直方图，属性：`activi.channel`、`activi.webhook`）
- `activi.message.queued`（计数器，属性：`activi.channel`、`activi.source`）
- `activi.message.processed`（计数器，属性：`activi.channel`、`activi.outcome`）
- `activi.message.duration_ms`（直方图，属性：`activi.channel`、`activi.outcome`）

队列 + 会话：

- `activi.queue.lane.enqueue`（计数器，属性：`activi.lane`）
- `activi.queue.lane.dequeue`（计数器，属性：`activi.lane`）
- `activi.queue.depth`（直方图，属性：`activi.lane` 或 `activi.channel=heartbeat`）
- `activi.queue.wait_ms`（直方图，属性：`activi.lane`）
- `activi.session.state`（计数器，属性：`activi.state`、`activi.reason`）
- `activi.session.stuck`（计数器，属性：`activi.state`）
- `activi.session.stuck_age_ms`（直方图，属性：`activi.state`）
- `activi.run.attempt`（计数器，属性：`activi.attempt`）
...
- `activi.webhook.error`
  - `activi.channel`、`activi.webhook`、`activi.chatId`、`activi.error`
- `activi.message.processed`
  - `activi.channel`、`activi.outcome`、`activi.chatId`、`activi.messageId`、`activi.sessionKey`、`activi.sessionId`、`activi.reason`
- `activi.session.stuck`
  - `activi.state`、`activi.ageMs`、`activi.queueDepth`、`activi.sessionKey`、`activi.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `activi doctor`。
