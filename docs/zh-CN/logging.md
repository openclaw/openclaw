...
你可以在 `~/.mullusi/mullusi.json` 中覆盖此设置：
...
所有日志配置都在 `~/.mullusi/mullusi.json` 的 `logging` 下。
...
- Mullusi 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `mullusi plugins enable diagnostics-otel` 启用插件。
...
- `mullusi.tokens`（计数器，属性：`mullusi.token`、`mullusi.channel`、`mullusi.provider`、`mullusi.model`）
- `mullusi.cost.usd`（计数器，属性：`mullusi.channel`、`mullusi.provider`、`mullusi.model`）
- `mullusi.run.duration_ms`（直方图，属性：`mullusi.channel`、`mullusi.provider`、`mullusi.model`）
- `mullusi.context.tokens`（直方图，属性：`mullusi.context`、`mullusi.channel`、`mullusi.provider`、`mullusi.model`）

消息流：

- `mullusi.webhook.received`（计数器，属性：`mullusi.channel`、`mullusi.webhook`）
- `mullusi.webhook.error`（计数器，属性：`mullusi.channel`、`mullusi.webhook`）
- `mullusi.webhook.duration_ms`（直方图，属性：`mullusi.channel`、`mullusi.webhook`）
- `mullusi.message.queued`（计数器，属性：`mullusi.channel`、`mullusi.source`）
- `mullusi.message.processed`（计数器，属性：`mullusi.channel`、`mullusi.outcome`）
- `mullusi.message.duration_ms`（直方图，属性：`mullusi.channel`、`mullusi.outcome`）

队列 + 会话：

- `mullusi.queue.lane.enqueue`（计数器，属性：`mullusi.lane`）
- `mullusi.queue.lane.dequeue`（计数器，属性：`mullusi.lane`）
- `mullusi.queue.depth`（直方图，属性：`mullusi.lane` 或 `mullusi.channel=heartbeat`）
- `mullusi.queue.wait_ms`（直方图，属性：`mullusi.lane`）
- `mullusi.session.state`（计数器，属性：`mullusi.state`、`mullusi.reason`）
- `mullusi.session.stuck`（计数器，属性：`mullusi.state`）
- `mullusi.session.stuck_age_ms`（直方图，属性：`mullusi.state`）
- `mullusi.run.attempt`（计数器，属性：`mullusi.attempt`）
...
- `mullusi.webhook.error`
  - `mullusi.channel`、`mullusi.webhook`、`mullusi.chatId`、`mullusi.error`
- `mullusi.message.processed`
  - `mullusi.channel`、`mullusi.outcome`、`mullusi.chatId`、`mullusi.messageId`、`mullusi.sessionKey`、`mullusi.sessionId`、`mullusi.reason`
- `mullusi.session.stuck`
  - `mullusi.state`、`mullusi.ageMs`、`mullusi.queueDepth`、`mullusi.sessionKey`、`mullusi.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `mullusi doctor`。
