...
read_when:
  - 你需要定时作业和唤醒功能
  - 你正在调试 cron 执行和日志
summary: "`activi cron` 的 CLI 参考（调度和运行后台作业）"
title: cron
x-i18n:
...
提示：运行 `activi cron --help` 查看完整的命令集。
...
```bash
activi cron edit <job-id> --announce --channel telegram --to "123456789"
```

为隔离的作业禁用投递：

```bash
activi cron edit <job-id> --no-deliver
```
