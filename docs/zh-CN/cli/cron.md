...
read_when:
  - 你需要定时作业和唤醒功能
  - 你正在调试 cron 执行和日志
summary: "`mullusi cron` 的 CLI 参考（调度和运行后台作业）"
title: cron
x-i18n:
...
提示：运行 `mullusi cron --help` 查看完整的命令集。
...
```bash
mullusi cron edit <job-id> --announce --channel telegram --to "123456789"
```

为隔离的作业禁用投递：

```bash
mullusi cron edit <job-id> --no-deliver
```
