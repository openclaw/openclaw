...
read_when:
  - 你需要定时作业和唤醒功能
  - 你正在调试 cron 执行和日志
summary: "`smart-agent-neo cron` 的 CLI 参考（调度和运行后台作业）"
title: cron
x-i18n:
...
提示：运行 `smart-agent-neo cron --help` 查看完整的命令集。
...
```bash
smart-agent-neo cron edit <job-id> --announce --channel telegram --to "123456789"
```

为隔离的作业禁用投递：

```bash
smart-agent-neo cron edit <job-id> --no-deliver
```
