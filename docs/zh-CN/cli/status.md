...
read_when:
  ...
  - 你想获取可粘贴的"all"状态用于调试
summary: "`smart-agent-neo status` 的 CLI 参考（诊断、探测、使用量快照）"
title: status
x-i18n:
...
```bash
smart-agent-neo status
smart-agent-neo status --all
smart-agent-neo status --deep
smart-agent-neo status --usage
```
...
- 更新信息显示在概览中；如果有可用更新，status 会打印提示运行 `smart-agent-neo update`（参见[更新](/install/updating)）。
