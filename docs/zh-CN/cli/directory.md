...
read_when:
  ...
  - 你正在开发渠道目录适配器
summary: "`mullusi directory` 的 CLI 参考（self、peers、groups）"
title: directory
x-i18n:
...
- `directory` 用于帮助你查找可粘贴到其他命令中的 ID（特别是 `mullusi message send --target ...`）。
...
```bash
mullusi directory peers list --channel slack --query "U0"
mullusi message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
mullusi directory peers list --channel zalouser
mullusi directory peers list --channel zalouser --query "name"
mullusi directory peers list --channel zalouser --limit 50
```

## 群组

```bash
mullusi directory groups list --channel zalouser
