...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.activi/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.activi/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置：`~/.activi/activi.json`（或 `ACTIVI_CONFIG_PATH`）
- 状态目录：`~/.activi`（或 `ACTIVI_STATE_DIR`）
- 工作区：`~/.activi/workspace`（或 `~/.activi/workspace-<agentId>`）
- 智能体目录：`~/.activi/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.activi/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Activi 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.activi/workspace`（或当设置了 `ACTIVI_PROFILE` 时为 `~/.activi/workspace-<profile>`）。
- 状态默认为 `~/.activi/agents/main/agent`。
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.activi/workspace-alex" },
      { id: "mia", workspace: "~/.activi/workspace-mia" },
    ],
  },
  bindings: [
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.activi/workspace-home",
        agentDir: "~/.activi/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.activi/workspace-work",
        agentDir: "~/.activi/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.activi/credentials/whatsapp/personal
          // authDir: "~/.activi/credentials/whatsapp/personal",
        },
        biz: {
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.activi/workspace-chat",
        model: "anthropic/claude-sonnet-4-5",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.activi/workspace-opus",
