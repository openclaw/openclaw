...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.mullusi/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.mullusi/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置：`~/.mullusi/mullusi.json`（或 `MULLUSI_CONFIG_PATH`）
- 状态目录：`~/.mullusi`（或 `MULLUSI_STATE_DIR`）
- 工作区：`~/.mullusi/workspace`（或 `~/.mullusi/workspace-<agentId>`）
- 智能体目录：`~/.mullusi/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.mullusi/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Mullusi 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.mullusi/workspace`（或当设置了 `MULLUSI_PROFILE` 时为 `~/.mullusi/workspace-<profile>`）。
- 状态默认为 `~/.mullusi/agents/main/agent`。
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.mullusi/workspace-alex" },
      { id: "mia", workspace: "~/.mullusi/workspace-mia" },
    ],
  },
  bindings: [
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.mullusi/workspace-home",
        agentDir: "~/.mullusi/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.mullusi/workspace-work",
        agentDir: "~/.mullusi/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.mullusi/credentials/whatsapp/personal
          // authDir: "~/.mullusi/credentials/whatsapp/personal",
        },
        biz: {
          // 可选覆盖。默认：~/.mullusi/credentials/whatsapp/biz
          // authDir: "~/.mullusi/credentials/whatsapp/biz",
        },
      },
    },
  },
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.mullusi/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.mullusi/workspace-opus",
