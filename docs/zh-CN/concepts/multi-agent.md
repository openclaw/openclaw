...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.smart-agent-neo/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.smart-agent-neo/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置：`~/.smart-agent-neo/smart-agent-neo.json`（或 `SMART_AGENT_NEO_CONFIG_PATH`）
- 状态目录：`~/.smart-agent-neo`（或 `SMART_AGENT_NEO_STATE_DIR`）
- 工作区：`~/.smart-agent-neo/workspace`（或 `~/.smart-agent-neo/workspace-<agentId>`）
- 智能体目录：`~/.smart-agent-neo/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.smart-agent-neo/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，SmartAgentNeo 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.smart-agent-neo/workspace`（或当设置了 `SMART_AGENT_NEO_PROFILE` 时为 `~/.smart-agent-neo/workspace-<profile>`）。
- 状态默认为 `~/.smart-agent-neo/agents/main/agent`。
...
```bash
smart-agent-neo agents add work
```
...
```bash
smart-agent-neo agents list --bindings
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.smart-agent-neo/workspace-alex" },
      { id: "mia", workspace: "~/.smart-agent-neo/workspace-mia" },
    ],
  },
  bindings: [
...
`~/.smart-agent-neo/smart-agent-neo.json`（JSON5）：
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.smart-agent-neo/workspace-home",
        agentDir: "~/.smart-agent-neo/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.smart-agent-neo/workspace-work",
        agentDir: "~/.smart-agent-neo/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.smart-agent-neo/credentials/whatsapp/personal
          // authDir: "~/.smart-agent-neo/credentials/whatsapp/personal",
        },
        biz: {
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.smart-agent-neo/workspace-chat",
        model: "anthropic/claude-sonnet-4-5",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.smart-agent-neo/workspace-opus",
