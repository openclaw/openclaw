...
在合并向量和关键词分数后，两个可选的后处理阶段会在结果发送给智能体之前对其进行优化：
...
当混合搜索返回结果时多个块可能包含相似或重叠的内容。例如，搜索"家庭网络设置"可能返回五个几乎相同的片段，来自不同日期的笔记都提到了相同的路由器配置。

**MMR（最大边际相关性）**对结果进行重排序以平衡相关性和多样性，确保顶部结果覆盖查询的不同方面，而不是重复相同的信息。
...
拥有每日笔记的智能体会随着时间积累数百个带日期的文件。如果没有衰减六个月前措辞良好的笔记可能超过同一主题的昨天更新。
