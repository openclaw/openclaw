...
mullusi nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
配对状态存储在 Gateway 网关状态目录下（默认 `~/.mullusi`）：

- `~/.mullusi/nodes/paired.json`
- `~/.mullusi/nodes/pending.json`

如果你覆盖了 `MULLUSI_STATE_DIR`，`nodes/` 文件夹会随之移动。
