...
smart-agent-neo nodes reject <requestId>
smart-agent-neo nodes status
smart-agent-neo nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
配对状态存储在 Gateway 网关状态目录下（默认 `~/.smart-agent-neo`）：

- `~/.smart-agent-neo/nodes/paired.json`
- `~/.smart-agent-neo/nodes/pending.json`

如果你覆盖了 `SMART_AGENT_NEO_STATE_DIR`，`nodes/` 文件夹会随之移动。
