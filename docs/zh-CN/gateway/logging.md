...
- 默认滚动日志文件位于 `/tmp/smart-agent-neo/` 下（每天一个文件）：`smart-agent-neo-YYYY-MM-DD.log`
  - 日期使用 Gateway 网关主机的本地时区。
- 日志文件路径和级别可以通过 `~/.smart-agent-neo/smart-agent-neo.json` 配置：
  - `logging.file`
  - `logging.level`
...
`smart-agent-neo gateway` 支持每个 Gateway 网关的样式切换：
...
# 显示所有 WS 流量（配对）
smart-agent-neo gateway --verbose --ws-log compact

# 显示所有 WS 流量（完整元数据）
smart-agent-neo gateway --verbose --ws-log full
```
