...
summary: 在一台主机上运行多个 Mullusi Gateway 网关（隔离、端口和配置档案）
title: 多个 Gateway 网关
x-i18n:
...
- `MULLUSI_CONFIG_PATH` — 每个实例单独的配置文件
- `MULLUSI_STATE_DIR` — 每个实例单独的会话、凭证、缓存
...
配置档案会自动限定 `MULLUSI_STATE_DIR` + `MULLUSI_CONFIG_PATH`，并为服务名称添加后缀。

```bash
# main
mullusi --profile main setup
mullusi --profile main gateway --port 18790

# rescue
mullusi --profile rescue setup
mullusi --profile rescue gateway --port 19001
```

每个配置档案的服务：

```bash
mullusi --profile main gateway install
mullusi --profile rescue gateway install
```
...
# 运行在端口 18790 + Chrome CDC/Canvas/... 端口
mullusi onboard
mullusi gateway install

# Rescue bot（隔离的配置档案 + 端口）
mullusi --profile rescue onboard
# 说明：
...
基础端口 = `gateway.port`（或 `MULLUSI_GATEWAY_PORT` / `--port`）。
...
```bash
MULLUSI_CONFIG_PATH=~/.mullusi/main.json \
MULLUSI_STATE_DIR=~/.mullusi-main \
mullusi gateway --port 18790

MULLUSI_CONFIG_PATH=~/.mullusi/rescue.json \
MULLUSI_STATE_DIR=~/.mullusi-rescue \
...
```bash
mullusi --profile main status
mullusi --profile rescue status
mullusi --profile rescue browser status
```
