...
summary: 在同一主机上运行多个 Activi Gateway 网关（隔离、端口和配置文件）
title: 多 Gateway 网关
x-i18n:
...
- `ACTIVI_CONFIG_PATH` — 每个实例的配置文件
- `ACTIVI_STATE_DIR` — 每个实例的会话、凭证、缓存
...
配置文件自动限定 `ACTIVI_STATE_DIR` + `ACTIVI_CONFIG_PATH` 范围并为服务名称添加后缀。

```bash
# main
activi --profile main setup
activi --profile main gateway --port 18789

# rescue
activi --profile rescue setup
activi --profile rescue gateway --port 19001
```

按配置文件的服务：

```bash
activi --profile main gateway install
activi --profile rescue gateway install
```
...
基础端口 = `gateway.port`（或 `ACTIVI_GATEWAY_PORT` / `--port`）。
...
```bash
ACTIVI_CONFIG_PATH=~/.activi/main.json \
ACTIVI_STATE_DIR=~/.activi-main \
activi gateway --port 18789

ACTIVI_CONFIG_PATH=~/.activi/rescue.json \
ACTIVI_STATE_DIR=~/.activi-rescue \
...
```bash
activi --profile main status
activi --profile rescue status
activi --profile rescue browser status
```
