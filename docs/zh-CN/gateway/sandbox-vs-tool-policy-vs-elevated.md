...
使用检查器查看 EasyHub *实际*在做什么：

```bash
EasyHub sandbox explain
EasyHub sandbox explain --session agent:main:main
EasyHub sandbox explain --agent work
EasyHub sandbox explain --json
```
...
- `group:nodes`：`nodes`
- `group:EasyHub`：所有内置 EasyHub 工具（不包括提供商插件）
