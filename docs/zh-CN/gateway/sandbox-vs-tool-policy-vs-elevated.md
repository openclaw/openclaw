...
使用检查器查看 Activi *实际*在做什么：

```bash
activi sandbox explain
activi sandbox explain --session agent:main:main
activi sandbox explain --agent work
activi sandbox explain --json
```
...
- `group:nodes`：`nodes`
- `group:activi`：所有内置 Activi 工具（不包括提供商插件）
