...
使用检查器查看 SmartAgentNeo *实际*在做什么：

```bash
smart-agent-neo sandbox explain
smart-agent-neo sandbox explain --session agent:main:main
smart-agent-neo sandbox explain --agent work
smart-agent-neo sandbox explain --json
```
...
- `group:nodes`：`nodes`
- `group:smart-agent-neo`：所有内置 SmartAgentNeo 工具（不包括提供商插件）
