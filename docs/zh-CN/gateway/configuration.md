...
> **初次接触配置？** 请查阅[配置示例](/zh-CN/gateway/configuration-examples)指南，获取带有详细说明的完整示例！
...
参见 [/environment](/zh-CN/help/environment) 了解优先级和来源详情。
...
另请参阅：[/concepts/oauth](/zh-CN/concepts/oauth)
...
参见[多智能体沙箱与工具](/zh-CN/tools/multi-agent-sandbox-tools)了解优先级和更多示例。
...
- `/reasoning stream` 将推理过程流式传输到草稿中，然后发送最终答案。
  重试策略默认值和行为记录在[重试策略](/zh-CN/concepts/retry)中。
...
控制入站/出站前缀和可选的确认反应。
参见[消息](/zh-CN/concepts/messages)了解排队、会话和流式上下文。
...
参见 [/concepts/session-pruning](/zh-CN/concepts/session-pruning) 了解行为细节。

#### `agents.defaults.compaction`（预留空间 + 记忆刷新）

`agents.defaults.compaction.mode` 选择压缩摘要策略。默认为 `default`；设为 `safeguard` 可为超长历史启用分块摘要。参见 [/concepts/compaction](/zh-CN/concepts/compaction)。
...
- `agents.defaults.humanDelay`：第一条之后**块回复**之间的随机延迟。
  ...
  ```
  参见 [/concepts/streaming](/zh-CN/concepts/streaming) 了解行为 + 分块细节。
...
- `session.typingIntervalSeconds`：每会话的刷新间隔覆盖。
  参见 [/concepts/typing-indicators](/zh-CN/concepts/typing-indicators) 了解行为细节。
...
详情：[沙箱](/zh-CN/gateway/sandboxing)
...
- 工具策略：仅允许 `exec`、`process`、`read`、`write`、`edit`、`apply_patch`、`sessions_list`、`sessions_history`、`sessions_send`、`sessions_spawn`、`session_status`（deny 优先）
  ...
  - 沙箱策略中支持工具组简写：`group:runtime`、`group:fs`、`group:sessions`、`group:memory`（参见[沙箱 vs 工具策略 vs 提升](/zh-CN/gateway/sandbox-vs-tool-policy-vs-elevated#tool-groups-shorthands)）
...
按提供商的概述 + 示例：[/concepts/model-providers](/zh-CN/concepts/model-providers)。
...
参见 [/gateway/local-models](/zh-CN/gateway/local-models) 了解当前本地指南。简而言之：在高性能硬件上通过 LM Studio Responses API 运行 MiniMax M2.1；保留托管模型合并作为回退。
...
参见 [/plugin](/zh-CN/tools/plugin) 了解详情。
...
参见 [Gateway 网关运维手册](/zh-CN/gateway) 了解派生的端口映射（gateway/browser/canvas）。
参见[多 Gateway 网关](/zh-CN/gateway/multiple-gateways) 了解浏览器/CDP 端口隔离细节。
...
Cron 是 Gateway 网关自有的唤醒和定时任务调度器。参见 [Cron 任务](/zh-CN/automation/cron-jobs) 了解功能概述和 CLI 示例。
...
_下一步：[智能体运行时](/zh-CN/concepts/agent)_ 🦞
