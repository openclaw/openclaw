---
read_when:
  - 在 DigitalOcean 上设置 Activi
  - 寻找便宜的 VPS 托管来运行 Activi
summary: 在 DigitalOcean 上运行 Activi（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Activi

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 Activi Gateway 网关。
...
# Check service
systemctl --user status activi-gateway.service

# View logs
journalctl --user -u activi-gateway.service -f
```
...
```bash
activi config set gateway.bind tailnet
activi gateway restart
```
...
```bash
activi pairing list telegram
activi pairing approve telegram <CODE>
```

### WhatsApp

```bash
activi channels login whatsapp
# Scan QR code
```
...
- `~/.activi/` — 配置、凭证、会话数据
- `~/.activi/workspace/` — 工作区（SOUL.md、记忆等）
...
journalctl -u activi --no-pager -n 50
```
