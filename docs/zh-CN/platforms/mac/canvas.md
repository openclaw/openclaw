...
- `~/Library/Application Support/Mullusi/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `mullusi-canvas://<session>/<path>`

示例：

- `mullusi-canvas://main/` → `<canvasRoot>/main/index.html`
- `mullusi-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `mullusi-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
mullusi nodes canvas present --node <id>
mullusi nodes canvas navigate --node <id> --url "/"
mullusi nodes canvas eval --node <id> --js "document.title"
mullusi nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__mullusi__/a2ui/
```
...
mullusi nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
mullusi nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "mullusi://agent?message=Review%20this%20design";
```
