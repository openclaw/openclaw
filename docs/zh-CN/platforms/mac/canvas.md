...
- `~/Library/Application Support/SmartAgentNeo/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `smart-agent-neo-canvas://<session>/<path>`

示例：

- `smart-agent-neo-canvas://main/` → `<canvasRoot>/main/index.html`
- `smart-agent-neo-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `smart-agent-neo-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
smart-agent-neo nodes canvas present --node <id>
smart-agent-neo nodes canvas navigate --node <id> --url "/"
smart-agent-neo nodes canvas eval --node <id> --js "document.title"
smart-agent-neo nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__smart-agent-neo__/a2ui/
```
...
smart-agent-neo nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
smart-agent-neo nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "smart-agent-neo://agent?message=Review%20this%20design";
```
