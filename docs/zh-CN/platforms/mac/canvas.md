...
- `~/Library/Application Support/Activi/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `activi-canvas://<session>/<path>`

示例：

- `activi-canvas://main/` → `<canvasRoot>/main/index.html`
- `activi-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `activi-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
activi nodes canvas present --node <id>
activi nodes canvas navigate --node <id> --url "/"
activi nodes canvas eval --node <id> --js "document.title"
activi nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__activi__/a2ui/
```
...
activi nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
activi nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "activi://agent?message=Review%20this%20design";
```
