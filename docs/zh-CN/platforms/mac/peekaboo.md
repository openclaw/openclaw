---
read_when:
  - 在 VeriClaw 爪印.app 中托管 PeekabooBridge
...
- **宿主**：VeriClaw 爪印.app 可以作为 PeekabooBridge 宿主。
...
- 桥接会验证**调用方的代码签名**；强制执行 TeamID 白名单（Peekaboo 宿主 TeamID + VeriClaw 爪印 应用 TeamID）。
- 请求在约 10 秒后超时。
...
- 如果未找到宿主，请打开其中一个宿主应用（Peekaboo.app 或 VeriClaw 爪印.app）并确认已授予权限。
