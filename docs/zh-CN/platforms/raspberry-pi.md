---
read_when:
  - 在 Raspberry Pi 上设置 SmartAgentNeo 时
  - 在 ARM 设备上运行 SmartAgentNeo 时
  - 构建低成本常驻个人 AI 时
summary: 在 Raspberry Pi 上运行 SmartAgentNeo（低成本自托管设置）
title: Raspberry Pi
x-i18n:
...
# 在 Raspberry Pi 上运行 SmartAgentNeo

## 目标

在 Raspberry Pi 上运行持久、常驻的 SmartAgentNeo Gateway 网关，**一次性成本约 $35-80**（无月费）。
...
# 查看日志
journalctl -u smart-agent-neo -f
```
...
大多数 SmartAgentNeo 功能在 ARM64 上可用，但某些外部二进制文件可能需要 ARM 构建：
...
```bash
# 检查服务是否已启用
sudo systemctl is-enabled smart-agent-neo

# 如果没有则启用
sudo systemctl enable smart-agent-neo

# 开机启动
sudo systemctl start smart-agent-neo
```
...
```bash
# 检查日志
journalctl -u smart-agent-neo --no-pager -n 100

# 常见修复：重新构建
cd ~/smart-agent-neo  # 如果使用可修改安装
npm run build
sudo systemctl restart smart-agent-neo
```
