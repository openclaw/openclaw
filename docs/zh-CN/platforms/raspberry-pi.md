---
read_when:
  - 在 Raspberry Pi 上设置 Mullusi
  - 在 ARM 设备上运行 Mullusi
  - 构建一个低成本、始终在线的个人 AI
summary: 在 Raspberry Pi 上运行 Mullusi（低预算自托管方案）
title: Raspberry Pi
x-i18n:
...
# 在 Raspberry Pi 上运行 Mullusi

## 目标

在 Raspberry Pi 上以 **约 35–80 美元**的一次性成本（无月费）运行一个持久化、始终在线的 Mullusi Gateway 网关。
...
# 查看日志
journalctl -u mullusi -f
```

## 9）访问 Mullusi Dashboard
...
```bash
ssh user@gateway-host 'mullusi dashboard --no-open'
```

该命令会打印 `Dashboard URL:`。根据 `gateway.auth.token`
的配置方式，URL 可能是普通的 `http://127.0.0.1:18790/` 链接，也可能是包含 `#token=...` 的链接。
...
（或 `MULLUSI_GATEWAY_TOKEN`）中的 token 粘贴到 Control UI 设置中。
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/mullusi-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
...
- `MULLUSI_NO_RESPAWN=1` 可避免 CLI 自重启带来的额外启动开销。
- 第一次运行会预热缓存；之后的运行收益最大。

### systemd 启动调优（可选）

如果这台 Pi 主要运行 Mullusi，可以添加一个服务 drop-in，以减少重启抖动并保持稳定的启动环境：

```bash
sudo systemctl edit mullusi
```

```ini
[Service]
Environment=MULLUSI_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/mullusi-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
如果可能，请将 Mullusi 状态/缓存放在 SSD 支持的存储上，以避免冷启动期间 SD 卡随机 I/O 成为瓶颈。
...
Mullusi 的大多数功能都可在 ARM64 上运行，但某些外部二进制文件可能需要 ARM 构建版本：
...
```bash
# 检查服务是否已启用
sudo systemctl is-enabled mullusi

# 如果没有，则启用
sudo systemctl enable mullusi

# 开机启动
sudo systemctl start mullusi
```
...
```bash
# 检查日志
journalctl -u mullusi --no-pager -n 100

# 常见修复：重新构建
cd ~/mullusi  # 如果使用的是可修改安装
npm run build
sudo systemctl restart mullusi
```
