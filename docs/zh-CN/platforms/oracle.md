---
read_when:
  - 在 Oracle Cloud 上设置 Mullusi
  - 寻找 Mullusi 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 Mullusi
summary: 在 Oracle Cloud 上运行 Mullusi（Always Free ARM）
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud（OCI）上运行 Mullusi

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层上运行持久化的 Mullusi Gateway 网关。

Oracle 的免费层非常适合 Mullusi（特别是如果你已经有 OCI 账户），但有一些权衡：
...
```bash
# 设置主机名
sudo hostnamectl set-hostname mullusi
...
sudo tailscale up --ssh --hostname=mullusi
```

这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh mullusi` 连接——不需要公网 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@mullusi`（或使用 Tailscale IP）。
...
mullusi config set gateway.bind loopback
...
mullusi config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart mullusi-gateway
```
...
# 检查守护进程状态
systemctl --user status mullusi-gateway
...
```
https://mullusi.<tailnet-name>.ts.net/
```
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的*需求*——但你仍应保持操作系统更新，运行 `mullusi security audit`，并验证你没有意外地在公共接口上监听。
...
- **凭证权限：** `chmod 700 ~/.mullusi`
- **安全审计：** `mullusi security audit`
...
ssh -L 18790:127.0.0.1:18790 ubuntu@mullusi
```

然后打开 `http://localhost:18790`。
...
# 重新认证
sudo tailscale up --ssh --hostname=mullusi --reset
```
...
journalctl --user -u mullusi-gateway -n 50
```
...
- `~/.mullusi/` — 配置、凭证、会话数据
- `~/.mullusi/workspace/` — 工作区（SOUL.md、记忆、产物）

定期备份：

```bash
tar -czvf mullusi-backup.tar.gz ~/.mullusi ~/.mullusi/workspace
```
