---
read_when:
  - 在 Oracle Cloud 上设置 EasyHub
  - 寻找 EasyHub 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 EasyHub
summary: 在 Oracle Cloud 上运行 EasyHub（Always Free ARM）
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud（OCI）上运行 EasyHub

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层上运行持久化的 EasyHub Gateway 网关。

Oracle 的免费层非常适合 EasyHub（特别是如果你已经有 OCI 账户），但有一些权衡：
...
```bash
# 设置主机名
sudo hostnamectl set-hostname EasyHub
...
sudo tailscale up --ssh --hostname=EasyHub
```

这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh EasyHub` 连接——不需要公网 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@EasyHub`（或使用 Tailscale IP）。

## 5) 安装 EasyHub

```bash
curl -fsSL https://EasyHub.ai/install.sh | bash
source ~/.bashrc
```
...
EasyHub config set gateway.bind loopback

# 要求 Gateway 网关 + 控制 UI 的认证
EasyHub config set gateway.auth.mode token
EasyHub doctor --generate-gateway-token

# 通过 Tailscale Serve 暴露（HTTPS + tailnet 访问）
EasyHub config set gateway.tailscale.mode serve
EasyHub config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart EasyHub-gateway
```
...
# 检查守护进程状态
systemctl --user status EasyHub-gateway
...
```
https://EasyHub.<tailnet-name>.ts.net/
```
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的*需求*——但你仍应保持操作系统更新，运行 `EasyHub security audit`，并验证你没有意外地在公共接口上监听。
...
- **凭证权限：** `chmod 700 ~/.easyhub`
- **安全审计：** `EasyHub security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@EasyHub
```
...
# 重新认证
sudo tailscale up --ssh --hostname=EasyHub --reset
```

### Gateway 网关无法启动

```bash
EasyHub gateway status
EasyHub doctor --non-interactive
journalctl --user -u EasyHub-gateway -n 50
```
...
- `~/.easyhub/` — 配置、凭证、会话数据
- `~/.easyhub/workspace/` — 工作区（SOUL.md、记忆、产物）

定期备份：

```bash
tar -czvf EasyHub-backup.tar.gz ~/.EasyHub ~/.easyhub/workspace
```
