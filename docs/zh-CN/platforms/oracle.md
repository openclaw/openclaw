---
read_when:
  - 在 Oracle Cloud 上设置 SmartAgentNeo
  - 寻找 SmartAgentNeo 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 SmartAgentNeo
summary: 在 Oracle Cloud 上运行 SmartAgentNeo（Always Free ARM）
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud（OCI）上运行 SmartAgentNeo

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层上运行持久化的 SmartAgentNeo Gateway 网关。

Oracle 的免费层非常适合 SmartAgentNeo（特别是如果你已经有 OCI 账户），但有一些权衡：
...
```bash
# 设置主机名
sudo hostnamectl set-hostname smart-agent-neo
...
sudo tailscale up --ssh --hostname=smart-agent-neo
```

这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh smart-agent-neo` 连接——不需要公网 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@smart-agent-neo`（或使用 Tailscale IP）。
...
smart-agent-neo config set gateway.bind loopback

# 要求 Gateway 网关 + 控制 UI 的认证
smart-agent-neo config set gateway.auth.mode token
smart-agent-neo doctor --generate-gateway-token

# 通过 Tailscale Serve 暴露（HTTPS + tailnet 访问）
smart-agent-neo config set gateway.tailscale.mode serve
smart-agent-neo config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart smart-agent-neo-gateway
```
...
```bash
# 检查版本
smart-agent-neo --version

# 检查守护进程状态
systemctl --user status smart-agent-neo-gateway
...
```
https://smart-agent-neo.<tailnet-name>.ts.net/
```
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的*需求*——但你仍应保持操作系统更新，运行 `smart-agent-neo security audit`，并验证你没有意外地在公共接口上监听。
...
- **凭证权限：** `chmod 700 ~/.smart-agent-neo`
- **安全审计：** `smart-agent-neo security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@smart-agent-neo
```
...
# 重新认证
sudo tailscale up --ssh --hostname=smart-agent-neo --reset
```
...
smart-agent-neo doctor --non-interactive
journalctl --user -u smart-agent-neo-gateway -n 50
```
...
- `~/.smart-agent-neo/` — 配置、凭证、会话数据
- `~/.smart-agent-neo/workspace/` — 工作区（SOUL.md、记忆、产物）

定期备份：

```bash
tar -czvf smart-agent-neo-backup.tar.gz ~/.smart-agent-neo ~/.smart-agent-neo/workspace
```
