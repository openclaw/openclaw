...
summary: 设置指南：在保持最新的同时保持你的 SmartAgentNeo 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.smart-agent-neo/workspace`（工作区）+ `~/.smart-agent-neo/smart-agent-neo.json`（配置）。
...
- **配置：** `~/.smart-agent-neo/smart-agent-neo.json`（JSON/JSON5 格式）
- **工作区：** `~/.smart-agent-neo/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm smart-agent-neo setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **SmartAgentNeo.app**（菜单栏）。
...
- 运行 `smart-agent-neo setup`，然后 `smart-agent-neo channels login`，然后手动启动 Gateway 网关（`smart-agent-neo gateway`）。
...
- **配对允许列表**：`~/.smart-agent-neo/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.smart-agent-neo/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.smart-agent-neo/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.smart-agent-neo/workspace` 和 `~/.smart-agent-neo/` 保持为"你的东西"；不要将个人提示/配置放入 `smart-agent-neo` 仓库。
