...
summary: 用于 Mullusi CLI 的脚本化新手引导和智能体设置
title: CLI 自动化
x-i18n:
...
使用 `--non-interactive` 自动化 `mullusi onboard`。
...
使用 `mullusi agents add <name>` 创建一个单独的智能体，它拥有自己的工作区、
会话和认证配置文件。不带 `--workspace` 运行会启动向导。

```bash
mullusi agents add work \
  --workspace ~/.mullusi/workspace-work \
...
- 默认工作区遵循 `~/.mullusi/workspace-<agentId>`。
