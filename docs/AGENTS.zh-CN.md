...
此目录负责文档编写、Mintlify 链接规则和文档国际化政策。

## Mintlify 规则

- 文档托管在 Mintlify 上（`https://docs.openclaw.ai`）。
- `docs/**/*.md` 中的内部文档链接必须保持根相对路径，且无 `.md` 或 `.mdx` 后缀（示例：`[配置](/configuration)`）。
- 章节交叉引用应使用根相对路径上的锚点（示例：`[钩子](/configuration#hooks)`）。
- 文档标题应避免使用破折号和撇号，因为 Mintlify 的锚点生成在这些情况下很脆弱。
...
- 保持捆绑插件命名与根目录 `AGENTS.md` 中的仓库范围插件术语规则一致。
...
- `pnpm docs:check-i18n-glossary` 是检查已更改的英文文档标题和简短内部文档标签的守卫。
