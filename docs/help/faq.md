...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Paperclip or another `openclaw_gateway` client fails on first run with `openclaw_gateway_pairing_required`. What do I do?](#paperclip-or-another-openclaw_gateway-client-fails-on-first-run-with-openclaw_gateway_pairing_required-what-do-i-do)
...
### Paperclip or another `openclaw_gateway` client fails on first run with `openclaw_gateway_pairing_required`. What do I do?

That error usually means the new client reached the Gateway, but the device has
not been approved yet.

Do this once after the first connection attempt:
...
If the client is connecting to a remote Gateway, pass the remote endpoint and credentials explicitly:

```bash
openclaw devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```

Then retry the client run.

Notes:

- This is expected for first-time remote/browser/client pairings. The Gateway
  uses device approval to make new operator devices explicit.
- You can also approve the pending device from the Control UI devices screen if
  that UI is already paired.
- If the same client asks for approval on every restart, it is likely generating
  a new device identity each time. Persist the client-side device key so the
  same device ID is reused. For Paperclip's `openclaw_gateway` adapter, that
  means keeping `adapterConfig.devicePrivateKeyPem` stable.

Related: [Devices CLI](/cli/devices), [Gateway troubleshooting](/gateway/troubleshooting#gateway-control-ui-connectivity), [Control UI pairing](/web/control-ui#device-pairing-first-connection).
