...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Paperclip openclaw gateway first run says pairing required what should I do?](#paperclip-openclaw-gateway-first-run-says-pairing-required-what-should-i-do)
...
### Paperclip openclaw gateway first run says pairing required what should I do

Approve the Paperclip adapter as a gateway device once, then retry the run:

```bash
openclaw devices approve --latest
```

If you are approving from a different machine than the gateway host, target the gateway directly:
...
- The `openclaw_gateway` adapter creates a device identity on first connect.
- The gateway rejects unapproved devices with `pairing required`.
- Auto-pairing only works when the adapter already has enough gateway auth to approve itself.

If the first run works once but later starts asking for approval again, persist the adapter's
device key in Paperclip config:
...
That keeps the same device identity across restarts so you do not have to approve every run.

Docs: [Gateway troubleshooting](/gateway/troubleshooting), [Devices CLI](/cli/devices), [Gateway remote access](/gateway/remote).
