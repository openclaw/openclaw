...
- [Quick start and first-run setup]
  ...
  - [I can't access docs.bot.ai (SSL error). What now?](#i-cant-access-docsbotai-ssl-error-what-now)
  ...
  - [Can multiple people use one WhatsApp number with different Bot instances?](#can-multiple-people-use-one-whatsapp-number-with-different-bot-instances)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two Bot instances talk to each other (local + VPS)?](#can-two-bot-instances-talk-to-each-other-local-vps)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of Bot instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-bot-instances-one-ceo-and-many-agents)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do Bot, Flawd, and Krill use for models?](#what-do-bot-flawd-and-krill-use-for-models)
...
```bash
curl -fsSL https://bot.ai/install.sh | bash -s -- --install-method git
```
...
### What's the recommended way to install and set up Bot

The repo recommends running from source and using the onboarding wizard:

```bash
curl -fsSL https://bot.ai/install.sh | bash
...
- Retrieve it from the gateway host: `bot config get gateway.auth.token` (or generate one: `bot doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `bot gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
...
3. Copy your workspace (default: `~/.hanzo/workspace`).
...
under `~/.hanzo/` (for example `~/.hanzo/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-bot-store-its-data),
...
### I can't access docs.bot.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.bot.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.bot.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docshanzoai-shows-an-ssl-error-comcastxfinity).
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://bot.ai/install.ps1](https://bot.ai/install.ps1)
...
```bash
curl -fsSL https://bot.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://bot.ai/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://bot.ai/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://bot.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `bot models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Yes - via **setup-token**. Bot no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Bot supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. Bot fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
### Can multiple people use one WhatsApp number with different Bot instances
...
(`~/.hanzo/bot`) and workspace (`~/.hanzo/workspace`) stay untouched.
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.hanzo/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.hanzo/bot.json`). Precedence is `<workspace>/skills` > `~/.hanzo/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.hanzo/bot.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.hanzo/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Bot treats as `<workspace>/skills`.
...
Not directly. macOS skills are gated by `metadata.bot.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
...
3. Override the skill metadata (workspace or `~/.hanzo/skills`) to allow Linux:
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured Bot workspace); Bot treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.hanzo/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [Skills Configuration](/tools/skills-config).
...
Legacy single-agent path: `~/.hanzo/agent/*` (migrated by `bot doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.hanzo/workspace`).
...
- **State dir (`~/.hanzo/bot`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.hanzo/skills`).

Default workspace is `~/.hanzo/workspace`, configurable via:
...
Bot reads an optional **JSON5** config from `$BOT_CONFIG_PATH` (default: `~/.hanzo/bot.json`):

```
$BOT_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.hanzo/workspace`).
...
Bot enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `bot doctor --generate-gateway-token`.
...
- Daemons read env vars from `~/.hanzo/.env` (or the service environment).
...
### Can two Bot instances talk to each other local VPS
...
- Restore from backup (git or a copied `~/.hanzo/bot.json`).
...
- a global fallback `.env` from `~/.hanzo/.env` (aka `$BOT_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.hanzo/.env` so they're picked up even when the service doesn't inherit your shell env.
...
### Is there a way to make a team of Bot instances one CEO and many agents
...
### Why doesn't Bot reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.hanzo/agents/<agentId>/sessions/`.
...
Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Bot currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `bot configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.hanzo/bot.json`
...
- **Bot + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
...
```
~/.hanzo/agents/<agentId>/agent/auth-profiles.json
```
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.hanzo/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.hanzo/agent/*` (migrated by `bot doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.hanzo/.env` or enable `env.shellEnv`.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `bot models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `bot models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.hanzo/.env` on the **gateway host**.
...
bot models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
bot models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
bot models auth order set --provider anthropic --agent main anthropic:default
```
...
- Fastest: `bot dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
...
- Use `bot --profile <name> …` per instance (auto-creates `~/.hanzo/bot-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `bot --profile <name> gateway install`.

Profiles also suffix service names (`ai.bot.<profile>`; legacy `com.bot.*`, `bot-gateway-<profile>.service`, `Bot Gateway (<profile>)`).
...
- macOS: `$BOT_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.hanzo/logs/...`; profiles use `~/.hanzo/bot-<profile>/logs/...`)
...
- Default behavior on DM-capable channels is **pairing**:
  ...
  - Approve with: `bot pairing approve --channel <channel> [--account <id>] <code>`
  - Pending requests are capped at **3 per channel**; check `bot pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
...
Still stuck? Ask in [Discord](https://discord.gg/hanzo) or open a [GitHub discussion](https://github.com/hanzoai/bot/discussions).
