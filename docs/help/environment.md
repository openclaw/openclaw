...
| `OPENCLAW_GIT_DIR`       | Override the dev-channel source checkout used by `openclaw update --channel dev`. When unset, dev updates use `$OPENCLAW_HOME/openclaw` if `OPENCLAW_HOME` is set, otherwise `~/openclaw`. |
