...
- `OPENCLAW_AGENT_ID`: always set for commands run through the `exec` tool; scripts can obtain the calling agent id (e.g., `main`) from it when one is available, or an empty string otherwise.
- `OPENCLAW_SESSION_KEY`: always set for commands run through the `exec` tool; scripts can obtain the session key (e.g., `agent:main:main`) from it when one is available, or an empty string otherwise.
