---
summary: "Where VeriClaw loads environment variables and the precedence order"
read_when:
...
VeriClaw pulls environment variables from multiple sources. The rule is **never override existing values**.
...
VeriClaw also injects context markers into spawned child processes:
...
- `COLORFGBG`: if your terminal exports it, VeriClaw uses the background color hint to auto-pick the TUI palette.
...
VeriClaw supports two env-driven patterns:
...
| Variable               | Purpose                                                                                                                                                                          |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`        | Override the home directory used for all internal path resolution (`~/.openclaw/`, agent dirs, sessions, credentials). Useful when running VeriClaw as a dedicated service user. |
