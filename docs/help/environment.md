...
- `OPENCLAW_AGENT_ID`: always set for commands run through the `exec` tool; contains the calling agent id (e.g., `it`) when one is available, or an empty string otherwise. This allows scripts to reliably identify the calling agent.
- `OPENCLAW_SESSION_KEY`: always set for commands run through the `exec` tool; contains the session key (e.g., `agent:it:main`) when one is available, or an empty string otherwise. This allows scripts to reliably identify their session context.
