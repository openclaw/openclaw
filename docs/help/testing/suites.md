...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - Changed core-test typechecks run the changed tests' owning root shards first, followed by the remaining selected shards in canonical order. Mixed source/test changes still check every core-test shard; test-only changes still include every importing shard. Ordering reuses the boundary check's compiler inventory and does not prioritize transitive consumers or skip unknown/deleted paths.
