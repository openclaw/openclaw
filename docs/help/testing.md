...
- Command: `pnpm test`
- Config: twenty sequential shard runs (`vitest.full-*.config.ts`) over the existing scoped Vitest projects
...
- Projects note:
  - Untargeted `pnpm test` now runs twenty smaller shard configs (`core-unit-fast`, `core-unit-src`, `core-unit-security`, `core-unit-ui`, `core-unit-support`, `core-support-boundary`, `core-contracts`, `core-bundled`, `core-runtime`, `agentic`, `auto-reply`, `extension-channel-roots`, `extension-browser`, `extension-providers`, `extension-web-search`, `extension-media-tools`, `extension-qa-channel`, `extension-qa-lab-core`, `extension-qa-lab-runtime`, `extension-dev-tools`) instead of one giant native root-project process. This cuts peak RSS on loaded machines and avoids auto-reply/extension work starving unrelated suites.
