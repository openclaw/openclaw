...
- Docker observability smoke: `pnpm test:docker:observability` (script: `scripts/e2e/docker-observability-smoke.sh`) runs QA-lab OTEL and Prometheus diagnostics checks inside the shared built-app Docker image. Set `OPENCLAW_DOCKER_OBSERVABILITY_LOOPS=<count>` to repeat both checks in one container run, or reuse a prebuilt image with `OPENCLAW_DOCKER_OBSERVABILITY_E2E_SKIP_BUILD=1`.
