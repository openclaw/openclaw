...
- The scheduled nightly live/E2E workflow dispatches the reusable live/E2E workflow from a logged wrapper job instead of using a top-level reusable workflow job. Its stable default narrows Docker E2E to `config-reload`, `mcp-channels`, and `plugin-update`, disables Open WebUI, and runs only the OpenAI live-model smoke. Use manual release validation or the reusable workflow's `full` profile for broad provider/media/ACP/Codex sweeps.
