...
- Command: `pnpm test`
- Config: fifteen sequential shard runs (`vitest.full-*.config.ts`) over the existing scoped Vitest projects
...
- Projects note:
  - Untargeted `pnpm test` now runs fifteen smaller shard configs (`core-unit-fast`, `core-unit-src`, `core-unit-security`, `core-unit-ui`, `core-unit-support`, `core-support-boundary`, `core-contracts`, `core-bundled`, `core-runtime`, `agentic`, `auto-reply`, `extension-channel-roots`, `extension-browser`, `extension-providers`, `extensions`) instead of one giant native root-project process. This cuts peak RSS on loaded machines and avoids auto-reply/extension work starving unrelated suites.
