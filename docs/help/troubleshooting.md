---
summary: "Symptom first troubleshooting hub for VeriClaw"
read_when:
  - VeriClaw is not working and you need the fastest path to a fix
...
is using an old shape that the current VeriClaw runtime no longer accepts.
...
```mermaid
flowchart TD
  A[VeriClaw is not working] --> B{What breaks first}
  B --> C[No replies]
