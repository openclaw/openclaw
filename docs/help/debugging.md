...
`pnpm gateway:watch` leaves sync-I/O tracing off by default so normal watch logs
stay quiet and interactive. Set `OPENCLAW_TRACE_SYNC_IO=1` when you want the
stacks printed directly in the terminal.
