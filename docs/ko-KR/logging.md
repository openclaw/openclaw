...
- 게이트웨이에서 작성한 **파일 로그**(JSON 줄)입니다.
- **콘솔 출력**은 터미널과 Control UI에 표시됩니다.
...
Control UI의 **로그** 탭은 `logs.tail`를 사용하여 동일한 파일을 추적합니다.
...
콘솔 로그는 **TTY를 인식**하며 읽기 쉽도록 형식이 지정됩니다.

- 하위 시스템 접두사(예: `gateway/channels/whatsapp`)
- 레벨 색상 지정(정보/경고/오류)
- 선택적 컴팩트 또는 JSON 모드

콘솔 형식은 `logging.consoleStyle`에 의해 제어됩니다.
...
- `compact`: 더 엄격한 출력(긴 세션에 가장 적합)
- `json`: 행당 JSON(로그 프로세서용).
...
수정은 **콘솔 출력에만** 영향을 미치며 파일 로그는 변경하지 않습니다.

## 진단 + OpenTelemetry

진단은 모델 실행을 위한 구조화되고 기계 판독이 가능한 이벤트입니다 **및**
메시지 흐름 원격 분석(웹후크, 대기열, 세션 상태). **그렇지 않습니다**
로그 교체; 메트릭, 추적 및 기타 내보내기를 제공하기 위해 존재합니다.

진단 이벤트는 프로세스 중에 내보내지지만 내보내기는 다음 경우에만 연결됩니다.
진단 + 내보내기 플러그인이 활성화되었습니다.

### OpenTelemetry와 OTLP 비교

- **OpenTelemetry(OTel)**: 데이터 모델 + 추적, 측정항목 및 로그용 SDK.
- **OTLP**: OTel 데이터를 수집기/백엔드로 내보내는 데 사용되는 유선 프로토콜입니다.
- 현재 **OTLP/HTTP(protobuf)**를 통해 OpenClaw를 내보냅니다.

### 내보낸 신호

- **메트릭**: 카운터 + 히스토그램(토큰 사용량, 메시지 흐름, 대기열).
- **추적**: 모델 사용 + 웹훅/메시지 처리 범위입니다.
- **로그**: ​​`diagnostics.otel.logs`가 활성화되면 OTLP를 통해 내보냅니다. 로그
  볼륨이 높을 수 있습니다. `logging.level` 및 내보내기 필터를 염두에 두세요.
...
- `message.queued`: 메시지가 처리 대기열에 추가되었습니다.
...
- `session.stuck`: 세션이 경고 + 수명으로 중단되었습니다.
...
`logging.level`를 발생시키지 않고 추가 대상 디버그 로그를 활성화하려면 플래그를 사용하세요.
...
- 플래그 로그는 표준 로그 파일로 이동합니다(`logging.file`와 동일).
- 출력은 여전히 ​​`logging.redactSensitive`에 따라 수정됩니다.
- 전체 가이드: [/diagnostics/flags](/diagnostics/flags).

### OpenTelemetry로 내보내기

진단은 `diagnostics-otel` 플러그인(OTLP/HTTP)을 통해 내보낼 수 있습니다. 이
...
- `openclaw plugins enable diagnostics-otel`를 사용하여 플러그인을 활성화할 수도 있습니다.
...
- 추적/메트릭은 `traces` / `metrics`로 전환할 수 있습니다(기본값: 켜짐). 흔적
  활성화된 경우 모델 사용 범위와 웹훅/메시지 처리 범위를 포함합니다.
- 수집기에 인증이 필요한 경우 `headers`를 설정하세요.
- 지원되는 환경변수 : `OTEL_EXPORTER_OTLP_ENDPOINT`,
...
- 추적 샘플링: `diagnostics.otel.sampleRate` (0.0–1.0, 루트 범위만).
- 지표 내보내기 간격: `diagnostics.otel.flushIntervalMs`(최소 1000ms).

### 프로토콜 참고 사항

- OTLP/HTTP 엔드포인트는 `diagnostics.otel.endpoint`를 통해 설정할 수 있습니다.
  `OTEL_EXPORTER_OTLP_ENDPOINT`.
- 엔드포인트에 이미 `/v1/traces` 또는 `/v1/metrics`가 포함되어 있으면 그대로 사용됩니다.
- 엔드포인트에 이미 `/v1/logs`가 포함되어 있는 경우 로그에는 그대로 사용됩니다.
- `diagnostics.otel.logs`는 메인 로거 출력에 대한 OTLP 로그 내보내기를 활성화합니다.

### 로그 내보내기 동작

- OTLP 로그는 `logging.file`에 기록된 것과 동일한 구조화된 레코드를 사용합니다.
- `logging.level` (파일 로그 수준)을 존중합니다. 콘솔 수정은 적용되지 **않습니다**
  OTLP 로그에.
...
- **로그가 비어 있습니까?** 게이트웨이가 실행 중이고 파일 경로에 쓰고 있는지 확인하세요.
  `logging.file`에서.
- **자세한 내용이 필요하십니까?** `logging.level`를 `debug` 또는 `trace`로 설정하고 다시 시도하세요.
