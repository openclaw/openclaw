...
3. IDE에서 stdio를 통해 `openclaw acp`를 실행하도록 지정하세요.
...
세션; 재정의하지 않는 한 ACP는 기본적으로 격리된 `acp:<uuid>` 세션으로 설정됩니다.
키 또는 라벨.

## Zed 편집기 설정

`~/.config/zed/settings.json`에 사용자 정의 ACP 에이전트를 추가합니다(또는 Zed의 설정 UI 사용).
...
[/concepts/session](/concepts/session)에서 세션 키에 대해 자세히 알아보세요.
...
- `--session <key>`: 기본 세션 키.
- `--session-label <label>`: 해결할 기본 세션 레이블입니다.
- `--require-existing`: 세션 키/레이블이 없으면 실패합니다.
- `--reset-session`: 처음 사용하기 전에 세션 키를 재설정합니다.
- `--no-prefix-cwd`: 작업 디렉터리 앞에 프롬프트를 붙이지 마세요.
...
- `--server-args <args...>`: ACP 서버에 추가 인수가 전달됩니다.
