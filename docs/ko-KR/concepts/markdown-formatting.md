...
- **안전한 청킹:** 렌더링하기 전에 텍스트를 분할하므로 인라인 형식 지정이 절대 발생하지 않습니다.
  덩어리로 나누어집니다.
...
1. **마크다운 구문 분석 -> IR**
   - IR은 일반 텍스트에 스타일 범위(굵게/기울임꼴/스트라이크/코드/스포일러) 및 링크 범위를 더합니다.
   - 오프셋은 UTF-16 코드 단위이므로 신호 스타일 범위가 해당 API와 일치합니다.
   - 채널이 테이블 변환을 선택한 경우에만 테이블이 구문 분석됩니다.
2. **청크 IR(포맷 우선)**
   - 렌더링 전에 IR 텍스트에 청킹이 발생합니다.
   - 인라인 형식은 여러 청크로 분할되지 않습니다. 스팬은 청크별로 슬라이스됩니다.
3. **채널별 렌더링**
   - **Slack:** mrkdwn 토큰(굵게/기울임꼴/스트라이크/코드), `<url|label>`로 링크됩니다.
   - **텔레그램:** HTML 태그(`<b>`, `<i>`, `<s>`, `<code>`, `<pre><code>`, `<a href>`).
   - **신호:** 일반 텍스트 + `text-style` 범위; 라벨이 다르면 링크는 `label (url)`가 됩니다.
...
- **느슨함:** `[label](url)` -> `<url|label>`; 베어 URL은 베어 남아 있습니다. 자동링크
...
스포일러 마커(`||spoiler||`)는 Signal에 대해서만 구문 분석되며, 여기서는 다음과 매핑됩니다.
스포일러 스타일 범위. 다른 채널에서는 이를 일반 텍스트로 처리합니다.

## 채널 포맷터를 추가하거나 업데이트하는 방법

1. **한 번 구문 분석:** 채널에 적합한 공유 `markdownToIR(...)` 도우미를 사용합니다.
   옵션(자동링크, 제목 스타일, 인용부호 접두어).
2. **렌더링:** `renderMarkdownWithMarkers(...)`를 사용하여 렌더러를 구현하고
   스타일 마커 맵(또는 신호 스타일 범위).
3. **청크:** 렌더링하기 전에 `chunkMarkdownIR(...)`를 호출합니다. 각 청크를 렌더링합니다.
4. **와이어 어댑터:** 새 청커를 사용하도록 채널 아웃바운드 어댑터를 업데이트합니다.
   그리고 렌더러.
5. **테스트:** 형식 테스트와 아웃바운드 전달 테스트를 추가하거나 업데이트합니다.
   채널은 청킹을 사용합니다.

## 일반적인 문제

- Slack 꺾쇠 괄호 토큰(`<@U123>`, `<#C123>`, `<https://...>`)은 다음과 같아야 합니다.
  보존; 원시 HTML을 안전하게 탈출하세요.
- 텔레그램 HTML에서는 마크업 손상을 방지하기 위해 태그 외부의 텍스트를 이스케이프해야 합니다.
- 신호 스타일 범위는 UTF-16 오프셋에 따라 다릅니다. 코드 포인트 오프셋을 사용하지 마십시오.
- 울타리로 둘러싸인 코드 블록에 대한 후행 줄 바꿈을 유지하여 닫는 마커가 위에 놓이도록 합니다.
  그들만의 라인.
