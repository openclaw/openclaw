...
- 비밀은 `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`(레거시: `~/.openclaw/agent/auth-profiles.json`)에 있습니다.
- 구성 `auth.profiles` / `auth.order`는 **메타데이터 + 라우팅 전용**(비밀 없음)입니다.
- 레거시 가져오기 전용 OAuth 파일: `~/.openclaw/credentials/oauth.json` (처음 사용 시 `auth-profiles.json`로 가져옴).
...
- `type: "oauth"` → `{ provider, access, refresh, expires, email? }` (+ `projectId`/`enterpriseUrl` 일부 공급자의 경우)

## 프로필 ID

OAuth 로그인은 여러 계정이 공존할 수 있도록 고유한 프로필을 생성합니다.

- 기본값: `provider:default` 이메일을 사용할 수 없는 경우.
- 이메일을 사용한 OAuth: `provider:<email>` (예: `google-antigravity:user@gmail.com`).

프로필은 `profiles` 아래 `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`에 있습니다.
...
2. **구성된 프로필**: `auth.profiles` 공급자별로 필터링됩니다.
3. **저장된 프로필**: 공급자에 대한 `auth-profiles.json` 항목입니다.

명시적인 순서가 구성되지 않은 경우 OpenClaw는 라운드 로빈 순서를 사용합니다.

- **기본 키:** 프로필 유형(**API 키 이전의 OAuth**).
- **보조 키:** `usageStats.lastUsed` (각 유형 내에서 오래된 것부터).
- **쿨다운/비활성화된 프로필**은 만료가 빠른 순서대로 끝으로 이동됩니다.

### 세션 고정성(캐시 친화적)

OpenClaw는 **세션당 선택한 인증 프로필을 고정**하여 공급자 캐시를 따뜻하게 유지합니다.
모든 요청에 ​​대해 회전하지 **않습니다**. 고정된 프로필은 다음 때까지 재사용됩니다.

- 세션이 재설정됩니다. (`/new` / `/reset`)
- 압축이 완료되었습니다(압축 횟수 증가).
- 프로필이 대기 중이거나 비활성화되었습니다.

`/model …@<profileId>`을 통한 수동 선택은 해당 세션에 대한 **사용자 재정의**를 설정합니다.
새 세션이 시작될 때까지 자동 순환되지 않습니다.

자동 고정 프로필(세션 라우터에 의해 선택됨)은 **기본 설정**으로 처리됩니다.
...
- 프로필 재정의와 함께 `/model …`를 통해 세션별 재정의를 사용합니다(UI/채팅 화면에서 지원하는 경우).
...
청구/크레딧 실패(예: "크레딧 부족"/ "크레딧 잔액 너무 낮음")는 장애 조치가 가능한 것으로 간주되지만 일반적으로 일시적이지 않습니다. 짧은 쿨다운 대신 OpenClaw는 프로필을 **비활성화**(더 긴 백오프 사용)로 표시하고 다음 프로필/공급자로 순환합니다.
...
- 청구 백오프는 **5시간**에 시작되며, 청구 실패당 두 배로 증가하고 **24시간**으로 제한됩니다.
...
다음 내용은 [게이트웨이 구성](/gateway/configuration)을 참조하세요.
...
더 폭넓은 모델 선택 및 대체 개요는 [모델](/concepts/models)을 참조하세요.
