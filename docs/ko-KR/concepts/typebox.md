...
TypeBox는 TypeScript 우선 스키마 라이브러리입니다. **Gateway를 정의하는 데 사용합니다.
WebSocket 프로토콜**(핸드셰이크, 요청/응답, 서버 이벤트). 해당 스키마
드라이브 **런타임 검증**, **JSON 스키마 내보내기** 및 **Swift 코드 생성**
...
첫 번째 프레임은 **반드시** `connect` 요청이어야 합니다. 그 후 고객이 전화할 수 있습니다.
메소드(예: `health`, `send`, `chat.send`) 및 이벤트 구독(예:
`presence`, `tick`, `agent`).
...
| 카테고리 | 예                                                        | 메모                           |
| -------- | --------------------------------------------------------- | ------------------------------ |
| 코어     | `connect`, `health`, `status`                             | `connect`가 먼저 나와야 합니다 |
| 메시징   | `send`, `poll`, `agent`, `agent.wait`                     | 부작용은 `idempotencyKey` 필요 |
| 채팅     | `chat.history`, `chat.send`, `chat.abort`, `chat.inject`  | WebChat은 다음을 사용합니다    |
| 세션     | `sessions.list`, `sessions.patch`, `sessions.delete`      | 세션 관리자                    |
| 노드     | `node.list`, `node.invoke`, `node.pair.*`                 | 게이트웨이 WS + 노드 작업      |
| 이벤트   | `tick`, `presence`, `agent`, `chat`, `health`, `shutdown` | 서버 푸시                      |

신뢰할 수 있는 목록은 `src/gateway/server.ts` (`METHODS`, `EVENTS`)에 있습니다.

## 스키마가 있는 곳

- 출처 : `src/gateway/protocol/schema.ts`
- 런타임 유효성 검사기(AJV): `src/gateway/protocol/index.ts`
- 서버 핸드쉐이크 + 메소드 디스패치: `src/gateway/server.ts`
...
- **서버 측**: 모든 인바운드 프레임이 AJV로 검증됩니다. 악수만
  매개변수가 `ConnectParams`와 일치하는 `connect` 요청을 수락합니다.
- **클라이언트 측**: JS 클라이언트는 이전에 이벤트 및 응답 프레임의 유효성을 검사합니다.
  그들을 사용합니다.
- **메소드 표면**: 게이트웨이는 지원되는 `methods`를 광고하고
  `events` `hello-ok`에 있습니다.
...
## 작업한 예: end-to-end 메서드 추가

예: `{ ok: true, text }`를 반환하는 새로운 `system.echo` 요청을 추가합니다.
...
`ProtocolSchemas` 및 내보내기 유형에 둘 다 추가합니다.
...
`src/gateway/protocol/index.ts`에서 AJV 유효성 검사기를 내보냅니다.
...
`src/gateway/server-methods.ts`에 등록하세요(이미 `systemHandlers`를 병합함).
그런 다음 `src/gateway/server.ts`의 `METHODS`에 `"system.echo"`를 추가하세요.
...
`src/gateway/server.*.test.ts`에 서버 테스트를 추가하고 문서에 방법을 기록해 둡니다.

## Swift 코드 생성 동작

Swift 생성기는 다음을 방출합니다.

- `GatewayFrame` `req`, `res`, `event` 및 `unknown` 사례가 포함된 열거형
...
- `NonEmptyString`는 ID 및 메소드/이벤트 이름의 기본값입니다.
- 최상위 레벨 `GatewayFrame`는 `type`에 **판별기**를 사용합니다.
- 부작용이 있는 방법에는 일반적으로 매개변수에 `idempotencyKey`가 필요합니다.
