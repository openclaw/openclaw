...
`openclaw doctor`는 OpenClaw용 복구 + 마이그레이션 도구입니다. 오래된 문제를 해결합니다
...
- 추가 작업공간 디렉토리 감지(`~/openclaw`).
- 샌드박싱이 활성화된 경우 샌드박스 이미지 복구.
- 레거시 서비스 마이그레이션 및 추가 게이트웨이 감지.
- 게이트웨이 런타임 검사(서비스가 설치되었지만 실행되지 않음, 캐시된 실행 라벨).
...
- 개방형 DM 정책에 대한 보안 경고.
- `gateway.auth.token`가 설정되지 않은 경우 게이트웨이 인증 경고(로컬 모드, 토큰 생성 제공)
- Linux에서 systemd 지연 검사를 수행합니다.
...
- `routing.bindings` → 최상위 레벨 `bindings`
- `routing.agents`/`routing.defaultAgentId` → `agents.list` + `agents.list[].default`
- `routing.agentToAgent` → `tools.agentToAgent`
- `routing.transcribeAudio` → `tools.media.audio.models`
- `bindings[].match.accountID` → `bindings[].match.accountId`
- `identity` → `agents.list[].identity`
- `agent.*` → `agents.defaults` + `tools.*` (도구/상승/exec/샌드박스/하위 에이전트)
...
`@mariozechner/pi-ai`의 내장 OpenCode Zen 카탈로그를 재정의합니다. 그럴 수 있어
...
- 세션 저장소 + 기록:
  - `~/.openclaw/sessions/`에서 `~/.openclaw/agents/<agentId>/sessions/`까지
- 상담원 주소:
  - `~/.openclaw/agent/`에서 `~/.openclaw/agents/<agentId>/agent/`까지
...
이러한 마이그레이션은 최선을 다하고 멱등성을 갖습니다. 의사는 다음과 같은 경우 경고를 발합니다.
...
`openclaw doctor`를 통해 마이그레이션되었습니다.
...
- **상태 디렉토리 누락**: 심각한 상태 손실에 대해 경고하고 재생성하라는 메시지를 표시합니다.
  누락된 데이터를 복구할 수 없음을 알려줍니다.
- **디렉터리 권한 상태**: 쓰기 가능성을 확인합니다. 권한 복구 제안
  (소유자/그룹 불일치가 감지되면 `chown` 힌트를 내보냅니다).
- **세션 디렉토리 누락**: `sessions/` 및 세션 저장소 디렉토리는 다음과 같습니다.
  기록을 유지하고 `ENOENT` 충돌을 방지하는 데 필요합니다.
- **기록 불일치**: 최근 세션 항목이 누락되면 경고합니다.
  성적표 파일.
- **기본 세션 “1줄 JSONL”**: 기본 성적표에 하나만 있는 경우 플래그를 지정합니다.
  라인(이력은 누적되지 않습니다).
- **다중 상태 디렉토리**: `~/.openclaw` 폴더가 여러 개 존재할 때 경고합니다.
  홈 디렉토리 또는 `OPENCLAW_STATE_DIR`가 다른 곳을 가리킬 때(기록은
  설치 간에 분할됨).
- **원격 모드 알림**: `gateway.mode=remote`인 경우 의사가 달리도록 알려줍니다.
  원격 호스트에 있습니다(상태는 거기에 있습니다).
- **구성 파일 권한**: `~/.openclaw/openclaw.json`이 다음인 경우 경고합니다.
  그룹/전 세계에서 읽을 수 있으며 `600`로 강화할 것을 제안합니다.

### 5) 모델 인증 상태(OAuth 만료)

의사는 인증 저장소에서 OAuth 프로필을 검사하고 토큰이 있을 때 경고합니다.
만료/만료되었으며 안전할 때 새로 고칠 수 있습니다. 인류 클로드 코드라면
프로필이 오래되었으므로 `claude setup-token` 실행(또는 설정 토큰 붙여넣기)을 제안합니다.
...
`hooks.gmail.model`가 설정되면 의사는 모델 참조를 검증합니다.
...
의사는 로컬 게이트웨이에서 `gateway.auth`가 누락되면 경고하고 다음을 제안합니다.
토큰을 생성합니다. `openclaw doctor --generate-gateway-token`를 사용하여 토큰을 강제합니다.
자동화의 창조.
...
- `openclaw doctor`는 감독자 구성을 다시 작성하기 전에 메시지를 표시합니다.
- `openclaw doctor --yes`는 기본 수리 프롬프트를 수락합니다.
- `openclaw doctor --repair`는 프롬프트 없이 권장 수정 사항을 적용합니다.
- `openclaw doctor --repair --force`는 사용자 정의 감독자 구성을 덮어씁니다.
...
게이트웨이 포트(기본값 `18789`)에서 가능한 원인을 보고합니다(게이트웨이는 이미
실행 중, SSH 터널).
...
전체 가이드는 [/concepts/agent-workspace](/concepts/agent-workspace)를 참조하세요.
