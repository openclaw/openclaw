...
- 또는 온보딩 중에 **Zalo**를 선택하고 설치 프롬프트를 확인하세요.
...
1. Zalo 플러그인을 설치합니다:
   ...
   - 또는 온보딩에서 **Zalo**를 선택하고 설치 프롬프트를 확인하세요.
2. 토큰을 설정합니다:
   - 환경: `ZALO_BOT_TOKEN=...`
...
- 그룹은 아직 지원되지 않습니다(Zalo 문서에는 "곧 출시 예정"이라고 나와 있습니다).

## 설정(빠른 경로)

### 1) 봇 토큰 생성(Zalo Bot Platform)

1. [https://bot.zaloplatforms.com](https://bot.zaloplatforms.com)에 접속하여 로그인하세요.
...
다중 계정 지원: 계정별 토큰 및 선택 사항인 `name`와 함께 `channels.zalo.accounts`를 사용합니다.

3. 게이트웨이를 다시 시작합니다. Zalo는 토큰이 확인되면(env 또는 config) 시작됩니다.
4. DM접속은 기본적으로 페어링으로 되어있습니다. 봇에 처음 접속할 때 코드를 승인하세요.

## 작동 방식(행동)

- 인바운드 메시지는 미디어 자리 표시자가 있는 공유 채널 봉투로 정규화됩니다.
- 답변은 항상 동일한 Zalo 채팅으로 다시 라우팅됩니다.
- 기본적으로 긴 폴링; `channels.zalo.webhookUrl`로 웹훅 모드를 사용할 수 있습니다.
...
- 기본값 : `channels.zalo.dmPolicy = "pairing"`. 알 수 없는 발신자는 페어링 코드를 받습니다. 메시지는 승인될 때까지 무시됩니다(코드는 1시간 후에 만료됩니다).
- 승인 방법:
  - `openclaw pairing list zalo`
  - `openclaw pairing approve zalo <CODE>`
- 페어링은 기본 토큰 교환입니다. 세부정보: [페어링](/channels/pairing)
- `channels.zalo.allowFrom`는 숫자로 된 사용자 ID를 허용합니다(사용자 이름 조회가 가능하지 않음).
...
- 웹훅 모드: `channels.zalo.webhookUrl` 및 `channels.zalo.webhookSecret`를 설정합니다.
  ...
  - Zalo는 확인을 위해 `X-Bot-Api-Secret-Token` 헤더가 포함된 이벤트를 보냅니다.
  - 게이트웨이 HTTP는 `channels.zalo.webhookPath`에서 웹훅 요청을 처리합니다(기본값은 웹훅 URL 경로).

**참고:** getUpdates(폴링) 및 웹훅은 Zalo API 문서별로 상호 배타적입니다.

## 지원되는 메시지 유형

- **문자 메시지**: 2000자 청크를 완벽하게 지원합니다.
- **이미지 메시지**: 인바운드 이미지를 다운로드하고 처리합니다. `sendPhoto`를 통해 이미지를 보냅니다.
...
- 구성된 경로에서 게이트웨이 HTTP 엔드포인트에 연결할 수 있는지 확인합니다.
...
- `channels.zalo.allowFrom`: DM 허용 목록(사용자 ID)입니다. `open`에는 `"*"`가 필요합니다. 마법사는 숫자 ID를 요구합니다.
- `channels.zalo.mediaMaxMb`: 인바운드/아웃바운드 미디어 캡(MB, 기본값 5).
