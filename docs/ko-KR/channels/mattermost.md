...
[mattermost.com](https://mattermost.com)에서 제품 세부정보 및 다운로드를 확인하세요.
...
Mattermost는 DM에 자동으로 응답합니다. 채널 동작은 `chatmode`에 의해 제어됩니다:
...
- `onchar`는 여전히 명시적인 @멘션에 응답합니다.
- `channels.mattermost.requireMention`는 레거시 구성에 적용되지만 `chatmode`가 선호됩니다.
