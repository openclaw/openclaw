...
- Oracle Cloud 계정([가입](https://www.oracle.com/cloud/free/)) — 문제가 발생한 경우 [커뮤니티 가입 가이드](https://gist.github.com/rssnyder/51e3cfedd730e7dd5f4a816143b25dbd)를 참조하세요.
...
2. **컴퓨팅 → 인스턴스 → 인스턴스 생성**으로 이동합니다.
3. 구성:
   - **이름:** `openclaw`
   - **이미지:** 우분투 24.04(aarch64)
...
**참고:** `build-essential`는 일부 종속성의 ARM 컴파일에 필요합니다.
...
이렇게 하면 Tailscale SSH가 활성화되므로 tailnet의 모든 장치에서 `ssh openclaw`를 통해 연결할 수 있습니다. 공용 IP가 필요하지 않습니다.
...
**이제부터 Tailscale:** `ssh ubuntu@openclaw` (또는 Tailscale IP 사용)를 통해 연결하세요.
...
"봇을 어떻게 부화하시겠습니까?"라는 메시지가 표시되면 **"나중에 수행"**을 선택합니다.
...
1. OCI 콘솔에서 **네트워킹 → 가상 클라우드 네트워크**로 이동합니다.
2. VCN → **보안 목록** → 기본 보안 목록을 클릭합니다.
3. 다음을 제외한 모든 수신 규칙을 **제거**합니다.
...
`<tailnet-name>`를 테일넷 이름으로 바꾸세요(`tailscale status`에 표시됨).
...
이 설정은 순전히 인터넷 전체의 SSH 무차별 대입을 중지하기 위해 추가 호스트 기반 방화벽 규칙에 대한 *필요*를 제거합니다. 하지만 여전히 OS를 업데이트하고 `openclaw security audit`를 실행하고 실수로 공용 인터페이스에서 수신 대기하고 있지 않은지 확인해야 합니다.
...
- **자격증명 권한:** `chmod 700 ~/.openclaw`
- **보안 감사:** `openclaw security audit`
- **시스템 업데이트:** `sudo apt update && sudo apt upgrade` 정기적
- **Tailscale 모니터링:** [Tailscale 관리 콘솔](https://login.tailscale.com/admin)에서 장치를 검토합니다.
...
- [게이트웨이 원격 접속](/gateway/remote) — 기타 원격 접속 패턴
- [Tailscale 통합](/gateway/tailscale) — 전체 Tailscale 문서
- [게이트웨이 구성](/gateway/configuration) — 모든 구성 옵션
- [DigitalOcean 가이드](/platforms/digitalocean) — 유료 + 간편한 가입을 원하는 경우
