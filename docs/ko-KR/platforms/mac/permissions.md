...
- 동일 경로: 고정된 위치(OpenClaw의 경우 `dist/OpenClaw.app`)에서 앱을 실행합니다.
...
## 메시지가 사라질 때의 복구 체크리스트

1. 앱을 종료합니다.
2. 시스템 설정 -> 개인 정보 보호 및 보안에서 앱 항목을 제거하세요.
3. 동일한 경로에서 앱을 다시 실행하고 권한을 다시 부여해 주세요.
4. 여전히 프롬프트가 나타나지 않으면 `tccutil`를 사용하여 TCC 항목을 재설정하고 다시 시도하십시오.
