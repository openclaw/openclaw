...
macOS 앱에는 `WKWebView`를 사용하여 에이전트가 제어하는 **캔버스 패널**이 포함되어 있습니다. 그것
...
캔버스 패널은 **사용자 정의 URL 구성표**를 통해 해당 파일을 제공합니다.
...
루트에 `index.html`가 없으면 앱은 **내장 스캐폴드 페이지**를 표시합니다.
...
설정 → **캔버스 허용**에서 캔버스를 비활성화할 수 있습니다. 비활성화되면 캔버스
노드 명령은 `CANVAS_DISABLED`를 반환합니다.

## 에이전트 API 표면

캔버스는 **Gateway WebSocket**을 통해 노출되므로 에이전트는 다음을 수행할 수 있습니다.
...
- `canvas.navigate`는 **로컬 캔버스 경로**, `http(s)` URL 및 `file://` URL을 허용합니다.
- `"/"`를 전달하면 캔버스에 로컬 스캐폴드 또는 `index.html`가 표시됩니다.
