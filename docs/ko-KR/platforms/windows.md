...
Windows의 OpenClaw는 **WSL2를 통해** 권장됩니다(Ubuntu 권장). 는
...
**WSL 내부**(SSH, 로컬 TTS 서버 또는 게이트웨이)를 실행하려면 다음을 수행해야 합니다.
...
- 다른 시스템의 SSH는 **Windows 호스트 IP**(예: `ssh user@windows-host -p 2222`)를 대상으로 합니다.
- 원격 노드는 **접근 가능한** 게이트웨이 URL(`127.0.0.1` 아님)을 가리켜야 합니다. 사용하다
  `openclaw status --all` 확인합니다.
- LAN 접속을 위해서는 `listenaddress=0.0.0.0`를 사용하세요. `127.0.0.1` 로컬로만 유지합니다.
- 자동으로 실행하려면 예약된 작업을 등록하여 새로 고침을 실행하세요.
  로그인 단계.
