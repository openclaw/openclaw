...
Rows generated by broken bridges, wrong ingestion paths, superseded code, or other bad generated sources must not remain recallable. The operational model is deactivate immediately, quarantine outside active recall, then prune/delete from active recall-producing tables after 24 hours. This rule is meant to prevent stale code from reactivating bad paths.

Legitimate operator/source history remains protected. If an audit trail is needed for a pruned generated row, use a minimal tombstone or non-live audit record that cannot participate in normal recall and cannot be selected by bridge reactivation logic.
