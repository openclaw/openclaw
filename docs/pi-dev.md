...
This guide summarizes a sane workflow for working on the pi integration in EVOX.sh.
...
- Trigger the agent directly:
  - `pnpm evox agent --message "Hello" --thinking low`
...
State lives under the EVOX.sh state directory. Default is `~/.openclaw`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.
