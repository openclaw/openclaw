...
This guide summarizes a sane workflow for working on the pi integration in Mullusi.
...
State lives under the Mullusi state directory. Default is `~/.mullusi`. If `MULLUSI_STATE_DIR` is set, use that directory instead.

To reset everything:

- `mullusi.json` for config
