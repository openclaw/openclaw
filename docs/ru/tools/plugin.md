...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T10:56:40Z
---
...
Плагины OpenClaw — это **модули TypeScript**, загружаемые во время выполнения через jiti. **Валидация конфига не выполняет код плагина**; вместо этого используется манифест плагина и JSON Schema. См. [Манифест плагина](/plugins/manifest).
...
- команды CLI
- фоновые сервисы
- необязательную валидацию конфига
...
- `deny`: список запрещённых (необязательно; запрет имеет приоритет)
