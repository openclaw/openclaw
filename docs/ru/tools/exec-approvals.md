...
только с входящим потоком. Цепочки shell и перенаправления не разрешаются автоматически в режиме allowlist.
...
не поддерживаются в режиме allowlist. Подстановка команд (`$()` / обратные кавычки) отклоняется
