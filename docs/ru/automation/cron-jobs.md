...
Необязательно: одноразовые задания (`schedule.kind = "at"`) по умолчанию удаляются после успешного выполнения.
Установите `deleteAfterRun: false`, чтобы сохранить их (после успеха они будут отключены).
...
принимается для совместимости. Одноразовые задания по умолчанию автоматически удаляются после успеха;
