...
Для практической разбивки (по каждому внедрённому файлу, инструментам, skills и размеру системной подсказки) используйте `/context list` или `/context detail`. См. [Context](/concepts/context).
...
См. [Skills](/tools/skills) для точной формулы накладных расходов списка skills.
