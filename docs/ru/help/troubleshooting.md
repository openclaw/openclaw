...
Выполните эту точную цепочку команд по порядку:
...
<AccordionGroup>
  ...
  <Accordion title="Gateway (шлюз) не запускается или сервис установлен, но не работает">
    ```bash
    openclaw status
    openclaw gateway status
    openclaw logs --follow
  ...
  <Accordion title="Узел сопряжён, но инструмент не выполняет camera/canvas/screen/exec">
    ```bash
    openclaw status
    openclaw gateway status
