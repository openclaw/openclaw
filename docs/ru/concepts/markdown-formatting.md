...
Если вам нужно больше деталей о поведении разбиения на чанки между каналами, см.
[Streaming + chunking](/concepts/streaming).

## Политика ссылок

- **Slack:** `[label](url)` -> `<url|label>`; «голые» URL остаются без изменений.
  Автоссылки отключены на этапе парсинга, чтобы избежать двойного линкования.
