...
См. [/concepts/model-failover](/concepts/model-failover) для ротации профилей аутентификации,
...
для профилей, найденных в хранилище аутентификации (по умолчанию предупреждает за 24 часа).
`--plain` выводит только разрешённую основную модель.
Статус OAuth показывается всегда (и включается в вывод `--json`).
Если у настроенного провайдера нет учётных данных, `models status` выводит раздел
**Missing auth**.
