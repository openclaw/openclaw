...
используемый файловыми инструментами и для контекста рабочего пространства.
Держите его приватным и относитесь к нему как к памяти.

Оно отделено от `~/.openclaw/`, где хранятся конфиг, учётные данные и
сеансы.

**Важно:** рабочее пространство — это **cwd по умолчанию**, а не жёсткий sandbox.
Инструменты разрешают относительные пути относительно рабочего пространства,
...
(например, `trash ~/openclaw`). Если вы намеренно используете несколько рабочих
...
См. [Memory](/concepts/memory) для описания рабочего процесса и автоматической
очистки памяти.
...
- Маршрутизация для нескольких агентов может использовать разные рабочие
  пространства для каждого агента. См.
  [Channel routing](/channels/channel-routing) для настройки маршрутизации.
