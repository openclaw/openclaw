...
Вне этого окна (до 9:00 или после 22:00 по восточному времени) heartbeat пропускаются.
Следующий запланированный тик внутри окна выполнится как обычно.
...
- `session` влияет только на контекст запуска; доставка контролируется
  `target` и `to`.
- Для доставки в конкретный канал/получателю задайте `target` + `to`.
  При `target: "last"` доставка использует последний внешний канал для этого сеанса.
...
По умолчанию подтверждения `HEARTBEAT_OK` подавляются, а содержимое алертов доставляется.
Вы можете настроить это для каждого канала или аккаунта:
...
чтобы сэкономить API‑вызовы. Если файла нет, heartbeat всё равно выполняется, и
...
Heartbeat выполняют полноценные ходы агента. Более короткие интервалы сжигают больше токенов.
Держите `HEARTBEAT.md` небольшим и рассмотрите более дешёвую `model` или
