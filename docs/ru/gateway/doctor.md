...
авторизация и модели попадали в путь для конкретного агента без ручного запуска doctor.
Авторизация WhatsApp намеренно мигрируется только через `openclaw doctor`.
...
предлагает удалить их и установить сервис OpenClaw, используя текущий порт gateway.
Он также может сканировать наличие дополнительных gateway-подобных сервисов и выводить
подсказки по очистке. Сервисы gateway OpenClaw с именами профилей считаются
...
управляемом version-manager (`nvm`, `fnm`, `volta`, `asdf` и т. д.).
Каналы WhatsApp и Telegram требуют Node, а пути version-manager могут ломаться после обновлений,
...
См. [/concepts/agent-workspace](/concepts/agent-workspace) для полного руководства по структуре
