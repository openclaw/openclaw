...
- Хост canvas — это HTTP‑сервер файлов на `canvasHost.port` (по умолчанию `18793`), обслуживающий `/__openclaw__/canvas/` для WebView узлов. См. [Gateway configuration](/gateway/configuration) (`canvasHost`).
- Удалённое использование обычно осуществляется через SSH‑туннель или VPN tailnet. См. [Remote access](/gateway/remote) и [Discovery](/gateway/discovery).
