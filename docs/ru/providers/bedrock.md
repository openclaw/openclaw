...
Bedrock, которые поддерживают **потоковую передачу** и **текстовый вывод**.
Обнаружение использует `bedrock:ListFoundationModels` и кэшируется (по умолчанию: 1 час).
...
использует службу метаданных инстанса (IMDS) для аутентификации. Однако текущая
