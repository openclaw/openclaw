...
Поддержка нескольких аккаунтов: используйте `channels.telegram.accounts` с токенами для каждого аккаунта и необязательным `name`. См. [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) для общего шаблона.
...
1. Запустите Gateway (шлюз) и напишите боту в личные сообщения.
