...
Поддержка нескольких аккаунтов: используйте `channels.discord.accounts` с токенами для каждого аккаунта и необязательным `name`. См. [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) для общего шаблона.
...
- Треды наследуют конфигурацию родительского канала (список разрешённых, `requireMention`, навыки, промпты и т. д.), если вы явно не добавите id треда.
...
| emojiUploads    | enabled      | Загрузка эмодзи                         |
