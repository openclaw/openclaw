...
- выводит необязательные подсказки по настройке провайдеров
...
`/home/node`, сохраняя стандартные bind-монты для конфига/рабочего пространства.
Используйте здесь именованный том (а не bind-путь); для bind-монтов используйте
...
кешировались. Это позволяет не перезапускать `pnpm install`, пока не изменятся
lock-файлы:
...
См. [Multi-Agent Sandbox & Tools](/tools/multi-agent-sandbox-tools) для примеров,
