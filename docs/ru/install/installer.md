...
<Note>
Если установка прошла успешно, но `openclaw` не найден в новом терминале, см. [устранение неполадок Node.js](/install/node#troubleshooting).
</Note>
...
<Steps>
  ...
  <Step title="Обеспечение Node.js 22+">
  ...
  <Step title="Пост-установочные задачи">
...
<Steps>
  <Step title="Установка локального рантайма Node">
...
<Steps>
  <Step title="Обеспечение PowerShell и среды Windows">
...
<Tabs>
  ...
  <Tab title="install.ps1 (пропустить онбординг)">
    ```powershell
...
<AccordionGroup>
  ...
  <Accordion title="Почему npm получает EACCES на Linux?">
  ...
  <Accordion title="Проблемы sharp/libvips">
