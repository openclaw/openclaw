...
- Необязательные учётные данные провайдеров
  - QR-код WhatsApp
  - Токен бота Telegram
  - OAuth Gmail
...
| Внешние бинарные файлы         | `/usr/local/bin/`                 | Docker-образ             | Должны быть встроены при сборке      |
| Среда выполнения Node          | Файловая система контейнера       | Docker-образ             | Пересобирается при каждой сборке     |
