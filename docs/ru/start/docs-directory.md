---
summary: "Подборка ссылок на наиболее используемую документацию OpenClaw."
read_when:
  - Вам нужен быстрый доступ к ключевым страницам документации
title: "Каталог документации"
x-i18n:
  source_path: start/docs-directory.md
  source_hash: 44d94b7a654801e9
  provider: openai
  model: gpt-5.2-chat-latest
  workflow: v1
  generated_at: 2026-02-08T10:55:57Z
---

<Note>
Эта страница — курируемый указатель. Если вы новичок, начните с [Начало работы](/start/getting-started).
Для полной карты документации см. [Хабы документации](/start/hubs).
</Note>

## Начните здесь

- [Хабы документации (все страницы со ссылками)](/start/hubs)
- [Помощь](/help)
- [Конфигурация](/gateway/configuration)
- [Примеры конфигурации](/gateway/configuration-examples)
- [Слэш-команды](/tools/slash-commands)
- [Маршрутизация нескольких агентов](/concepts/multi-agent)
- [Обновление и откат](/install/updating)
- [Сопряжение (личные сообщения и узлы)](/channels/pairing)
- [Режим Nix](/install/nix)
- [Настройка ассистента OpenClaw](/start/openclaw)
- [Skills](/tools/skills)
- [Конфиг Skills](/tools/skills-config)
- [Шаблоны рабочих пространств](/reference/templates/AGENTS)
- [RPC-адаптеры](/reference/rpc)
- [Руководство по эксплуатации Gateway (шлюз)](/gateway)
- [Узлы (iOS и Android)](/nodes)
- [Веб‑поверхности (Control UI)](/web)
- [Discovery и транспорты](/gateway/discovery)
- [Удалённый доступ](/gateway/remote)

## Провайдеры и UX

- [WebChat](/web/webchat)
- [Control UI (браузер)](/web/control-ui)
- [Telegram](/channels/telegram)
- [Discord](/channels/discord)
- [Mattermost (плагин)](/channels/mattermost)
- [BlueBubbles (iMessage)](/channels/bluebubbles)
- [iMessage (legacy)](/channels/imessage)
- [Группы](/channels/groups)
- [Групповые сообщения WhatsApp](/channels/group-messages)
- [Медиа: изображения](/nodes/images)
- [Медиа: аудио](/nodes/audio)

## Сопутствующие приложения

- [Приложение для macOS](/platforms/macos)
- [Приложение для iOS](/platforms/ios)
- [Приложение для Android](/platforms/android)
- [Windows (WSL2)](/platforms/windows)
- [Приложение для Linux](/platforms/linux)

## Операции и безопасная работа

- [Сеансы](/concepts/session)
- [Задания Cron](/automation/cron-jobs)
- [Вебхуки](/automation/webhook)
- [Gmail hooks (Pub/Sub)](/automation/gmail-pubsub)
- [Безопасность](/gateway/security)
- [Устранение неполадок](/gateway/troubleshooting)
