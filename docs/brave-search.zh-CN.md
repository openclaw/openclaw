...
旧版 `tools.web.search.apiKey` 仍然通过兼容层加载，但它不再是规范的配置路径。

`webSearch.mode` 控制 Brave 传输方式：

- `web`（默认）：带有标题、URL 和摘要的正常 Brave 网络搜索
- `llm-context`：带有预提取文本块和用于基础的来源的 Brave LLM Context API
...
- OpenClaw 使用 Brave **Search** 计划。如果你有旧版订阅（例如，每月 2,000 次查询的原始免费计划），它仍然有效，但不包括 LLM Context 或更高速率限制等较新功能。
- 每个 Brave 计划包括 **每月 5 的免费信用**（自动续期）。Search 计划每 1,000 次请求收费 5，因此信用额度涵盖每月 1,000 次查询。在 Brave 仪表板中设置使用限制以避免意外费用。有关当前计划，请参阅 [Brave API 门户](https://brave.com/search/api/)。
