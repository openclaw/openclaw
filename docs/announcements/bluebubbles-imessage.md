...
- With `groupPolicy: "allowlist"`, copy the old `groups` block, including any `"*"` wildcard entry. Group sender allowlists and the group registry are separate gates; a `groups` block with entries but no matching `chat_id` (or no `"*"`) drops the message at runtime, and an empty (or absent) `groups` block still lets messages through as long as the sender gate passes. The startup warning fires on a different condition: `groupPolicy: "allowlist"` with an empty effective group sender allowlist. See [Group registry footgun](/channels/imessage-from-bluebubbles#group-registry-footgun).
