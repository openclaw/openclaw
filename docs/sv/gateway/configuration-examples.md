...
Spara till `~/.openclaw/openclaw.json` så kan du DM:a boten från det numret.
...
### Konfiguration för flera plattformar
...
### Säkert DM-läge (delad inkorg / DMer för flera användare)

Om mer än en person kan DM:a din bot (flera poster i `allowFrom`, parningsgodkännanden för flera personer, eller `dmPolicy: "open"`), aktivera **säkert DM-läge** så att DMer från olika avsändare inte delar samma kontext som standard:
...
### OAuth med API-nyckel som reserv
...
### Anthropic-prenumeration + API-nyckel, MiniMax som reserv
