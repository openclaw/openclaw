...
Tänk på **Gateway-värden** som ”där agenten bor”. Den äger sessioner, autentiseringsprofiler, kanaler och tillstånd.
Din laptop/desktop (och noder) ansluter till den värden.
...
Laptopen kör **inte** agenten. Den ansluter på distans:
...
En gateway-tjänst äger tillstånd + kanaler. Noder är perifera.
...
Obs: när du skickar `--url` faller CLI inte tillbaka till konfig- eller miljöuppgifter.
...
WebChat använder inte längre en separat HTTP-port. SwiftUI-chattgränssnittet ansluter direkt till Gateway WebSocket.
...
- **Tailscale Serve** kan autentisera via identitetshuvuden när `gateway.auth.allowTailscale: true`.
  Sätt det till `false` om du i stället vill använda tokens/lösenord.
