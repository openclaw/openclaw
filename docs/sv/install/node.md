...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T08:17:42Z
---

# Node.js

OpenClaw kräver **Node 22 eller senare**. [Installationsskriptet](/install#install-methods) identifierar och installerar Node automatiskt — den här sidan är till för när du vill konfigurera Node själv och säkerställa att allt är korrekt uppkopplat (versioner, PATH, globala installationer).
...
Om detta skriver ut `v22.x.x` eller högre är allt klart. Om Node inte är installerat eller om versionen är för gammal, välj en installationsmetod nedan.
...
<Accordion title="Använda en versionshanterare (nvm, fnm, mise, asdf)">
  Versionshanterare gör det enkelt att växla mellan Node-versioner. Populära alternativ:
...
```
  ...
  <Warning>
  Se till att din versionshanterare initieras i din skalets startfil (`~/.zshrc` eller `~/.bashrc`). Om den inte gör det kan `openclaw` inte hittas i nya terminalsessioner eftersom PATH inte kommer att innehålla Nodes bin-katalog.
  </Warning>
</Accordion>
...
<Steps>
  <Step title="Hitta ditt globala npm-prefix">
    ```bash
    npm prefix -g
    ```
  </Step>
  <Step title="Kontrollera om det finns på din PATH">
    ```bash
    echo "$PATH"
    ```
  ...
  <Step title="Lägg till det i din skalets startfil">
    <Tabs>
