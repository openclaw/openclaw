...
Starter plan has 512MB RAM, which may be insufficient depending on enabled features and plugins. The Blueprint includes `NODE_OPTIONS: "--max-old-space-size=384"` to help, but if you still see heap errors, upgrade to Standard plan (1GB+ RAM).
