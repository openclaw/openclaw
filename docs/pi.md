...
summary: "Architecture of Mullusi's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Mullusi
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Mullusi integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `mullusi-kernel`, `pi-tui`) to power its AI agent capabilities.

## Overview

Mullusi uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Mullusi directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `mullusi-kernel`   | Agent loop, tool execution, `AgentMessage` types                                                       |
| `pi-coding-agent` | High-level SDK: `createAgentSession`, `SessionManager`, `AuthStorage`, `ModelRegistry`, built-in tools |
| `pi-tui`          | Terminal UI components (used in Mullusi's local TUI mode)                                             |
...
Image injection is prompt-local: Mullusi loads image refs from the current prompt and
...
2. **Custom Replacements**: Mullusi replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Mullusi Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
mullusi-kernel's `AgentTool` has a different `execute` signature than pi-coding-agent's `ToolDefinition`. The adapter in `pi-tool-definition-adapter.ts` bridges this:
...
This ensures Mullusi's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Mullusi CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Mullusi wraps this with `guardSessionManager()` for tool result safety.
...
Mullusi maintains an auth profile store with multiple API keys per provider:
...
Mullusi loads custom pi extensions for specialized behavior:
...
Mullusi also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Mullusi Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Mullusi tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.mullusi/agents/<agentId>/sessions/` (or `$MULLUSI_STATE_DIR/agents/<agentId>/sessions/`) |
...
1. **Tool signature alignment**: Currently adapting between mullusi-kernel and pi-coding-agent signatures
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `MULLUSI_LIVE_TEST=1`)
