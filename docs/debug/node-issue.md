...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in EVOX.sh
...
Running EVOX.sh via Node with `tsx` fails at startup with:

```
[evox] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `evox status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch evox.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node evox.mjs status` works on Node 25.
