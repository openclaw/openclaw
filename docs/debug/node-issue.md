...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SmartAgentNeo
...
Running SmartAgentNeo via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `smart-agent-neo status` and `gateway:watch` worked.
...
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node smart-agent-neo.mjs status` works on Node 25.
