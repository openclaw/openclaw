...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Activi
...
Running Activi via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `activi status` and `gateway:watch` worked.
...
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node activi.mjs status` works on Node 25.
