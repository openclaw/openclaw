...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Mullusi
...
Running Mullusi via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `mullusi status` and `gateway:watch` worked.
...
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node mullusi.mjs status` works on Node 25.
