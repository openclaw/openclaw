...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in EasyHub
...
Running EasyHub via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `EasyHub status` and `gateway:watch` worked.
...
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node easyhub.mjs status` works on Node 25.
