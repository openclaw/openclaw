...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in DNA
---

# Node + tsx "__name is not a function" crash

## Summary
Running DNA via Node with `tsx` fails at startup with:

```
[dna] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `dna status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch dna.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node dna.mjs status` works on Node 25.
