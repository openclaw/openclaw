...
**Pini-pin ng OpenClaw ang napiling auth profile bawat session** upang manatiling mainit ang mga cache ng provider.
Hindi ito nagro-rotate sa bawat request. Ang naka-pin na profile ay muling ginagamit hanggang:
...
Ang mga auto‑pinned na profile (pinili ng session router) ay tinatrato bilang isang **preference**:
sila ang unang sinusubukan, ngunit maaaring mag-rotate ang OpenClaw sa ibang profile kapag may rate limit/timeout.
Ang mga user‑pinned na profile ay nananatiling naka-lock sa profile na iyon; kung pumalya ito at may naka-configure
na model fallback, lilipat ang OpenClaw sa susunod na model sa halip na magpalit ng profile.

### Bakit maaaring “magmukhang nawawala” ang OAuth

Kung mayroon kang parehong OAuth profile at API key profile para sa iisang provider, maaaring magpalipat-lipat ang round‑robin sa pagitan nila sa mga mensahe maliban kung naka-pin. Para pilitin ang iisang profile:
...
Kapag pumalya ang isang profile dahil sa auth/rate‑limit na error (o timeout na mukhang rate limiting),
minamarkahan ito ng OpenClaw na nasa cooldown at lilipat sa susunod na profile.
Ang mga format/invalid‑request na error (halimbawa, Cloud Code Assist tool call ID
validation failures) ay itinuturing na karapat-dapat sa failover at gumagamit ng parehong mga cooldown.
...
Ang mga billing/credit failure (halimbawa “insufficient credits” / “credit balance too low”) ay itinuturing na karapat-dapat sa failover, ngunit kadalasan ay hindi transient. Sa halip na maikling cooldown, minamarkahan ng OpenClaw ang profile bilang **disabled** (na may mas mahabang backoff) at nagro-rotate sa susunod na profile/provider.
...
Kung pumalya ang lahat ng profile para sa isang provider, lilipat ang OpenClaw sa susunod na model sa
`agents.defaults.model.fallbacks`. Nalalapat ito sa mga auth failure, rate limit, at
mga timeout na umubos sa profile rotation (ang iba pang error ay hindi uma-advance ng fallback).
