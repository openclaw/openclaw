...
**용어 정리**:
- **Moltbot**: 설치하는 오픈소스 엔진 (GitHub 리포, CLI 명령어 `moltbot`)
...
### 0.2 Moltbot GitHub 리포지토리 클론

**방법 A: Clone Repository 버튼 (권장)**
- [ ] 시작 화면에서 **Clone Repository** 선택
- [ ] URL 입력: `https://github.com/moltbot/moltbot`
- [ ] Clone/Import 실행

**방법 B: Agent 채팅 요청**
- [ ] Agent 입력창에 입력: `이 리포지토리를 워크스페이스로 클론해줘: https://github.com/moltbot/moltbot`

✅ **완료 기준**: 파일 트리에 Moltbot 프로젝트 구조 (README, src 등) 표시
...
- [ ] 설정 파일 생성 확인: `%USERPROFILE%/.clawdbot/moltbot.json`
...
# Repo-local
npm run moltbot -- configure --section web
```
...
**Repo-local 실행 시**:
```bash
npm run moltbot -- doctor
npm run moltbot -- status
npm run moltbot -- health
npm run moltbot -- channels status --probe
```
...
# Repo-local
npm run moltbot -- agent --message "안녕하세요" --thinking high
```
...
```bash
# Repo-local
npm run moltbot -- pairing list discord
npm run moltbot -- pairing approve discord <CODE>
...
```bash
# 글로벌
moltbot memory prune  # 오래된 세션 정리
moltbot memory --help

# Repo-local
npm run moltbot -- memory prune
```
...
**글로벌 설치 시**:
```bash
moltbot update --channel stable  # 안정 버전
moltbot update --channel beta    # 베타 버전
moltbot update --channel dev     # 개발 버전
```
...
**해결**:
- [ ] 온보딩 재실행: `npm run moltbot -- onboard`
