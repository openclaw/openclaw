...
<AccordionGroup>
  ...
  <Accordion title="Gateway, services, and supervisors">
    ...
    - Codex route repair for legacy `openai-codex/*` model refs in primary models, fallbacks, image/video generation models, heartbeat/subagent/compaction overrides, hooks, channel model overrides, and session route pins; `--fix` rewrites only routes that the shared migration plan marks safe to migrate to `openai/*`, migrates matching `openai-codex:*` auth profiles/order to `openai:*`, removes stale session/whole-agent runtime pins for migrated routes, and leaves canonical OpenAI agent refs on the default Codex harness. When legacy provider/auth/session state cannot be represented safely on canonical OpenAI, doctor retains the legacy route and emits a manual-migration warning instead of advertising an automatic fix.
