...
`activi doctor` is the repair + migration tool for Activi. It fixes stale
...
- Extra workspace dir detection (`~/activi`).
...
you to run `activi doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.activi/activi.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.activi/sessions/` to `~/.activi/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.activi/agent/` to `~/.activi/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.activi/credentials/*.json` (except `oauth.json`)
  - to `~/.activi/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `activi doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.activi` folders exist across
  home directories or when `ACTIVI_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.activi/activi.json` is
...
offers to remove them and install the Activi service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Activi gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `activi doctor --generate-gateway-token` to force token
...
- `activi doctor` prompts before rewriting supervisor config.
- `activi doctor --yes` accepts the default repair prompts.
- `activi doctor --repair` applies recommended fixes without prompts.
- `activi doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `activi gateway install --force`.
