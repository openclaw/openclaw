...
`EasyHub doctor` is the repair + migration tool for EasyHub. It fixes stale
...
- Extra workspace dir detection (`~/EasyHub`).
...
you to run `EasyHub doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.easyhub/easyhub.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.easyhub/sessions/` to `~/.easyhub/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.easyhub/agent/` to `~/.easyhub/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.easyhub/credentials/*.json` (except `oauth.json`)
  - to `~/.easyhub/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `EasyHub doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.easyhub` folders exist across
  home directories or when `EASYHUB_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.easyhub/easyhub.json` is
...
offers to remove them and install the EasyHub service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named EasyHub gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `EasyHub doctor --generate-gateway-token` to force token
...
- `EasyHub doctor` prompts before rewriting supervisor config.
- `EasyHub doctor --yes` accepts the default repair prompts.
- `EasyHub doctor --repair` applies recommended fixes without prompts.
- `EasyHub doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `EasyHub gateway install --force`.
