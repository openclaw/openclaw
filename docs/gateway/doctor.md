...
<AccordionGroup>
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - On macOS, a detected same-label system LaunchDaemon blocks user LaunchAgent install, start, restart, and bootstrap repair. Doctor reports the system owner and stops service recovery; `--force` does not bypass this ownership boundary. Detection limits are described in [Existing system LaunchDaemons](/gateway#existing-system-launchdaemons).
