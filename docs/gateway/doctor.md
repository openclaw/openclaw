...
<AccordionGroup>
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - Linux systemd installs record generated-unit provenance in OpenClaw state. During update-mode repairs, doctor auto-stages a refreshed unit only when the live unit still matches the last OpenClaw-generated bytes; operator-modified or legacy units are left unchanged and get a `.suggested` unit plus review/apply commands.
