...
<AccordionGroup>
  ...
  <Accordion title="8. Gateway service migrations and cleanup hints">
    ...
    On Linux, if the user-level gateway service is missing but `--deep` discovery finds a system-level OpenClaw gateway-like service, doctor does not install a second user-level service automatically. Inspect with `openclaw gateway status --deep` / `openclaw doctor --deep`, then remove the duplicate or set `OPENCLAW_SERVICE_REPAIR_POLICY=external` when an external/system supervisor owns the gateway lifecycle.
