...
summary: "Complete reference for every Mullusi config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.mullusi/mullusi.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Mullusi uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `mullusi doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `mullusi doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["mullusi-dm"] },
      guilds: {
        "123456789012345678": {
...
- Mullusi additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `mullusi plugins install @mullusi/mattermost`.
...
- `commands.callbackUrl` must resolve to the Mullusi gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, Mullusi rejects callbacks with
...
Mullusi spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `mullusi channels add` (or channel onboarding) while still on a single-account top-level channel config, Mullusi promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `mullusi doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@mullusi", "mullusi"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@mullusi"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `mullusi.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Mullusi auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/mullusi" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.4`). If you omit the provider, Mullusi tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, Mullusi falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Mullusi aborts it. Default: `900`.
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Mullusi materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.mullusi/sandboxes`
...
In `remote` mode, host-local edits made outside Mullusi are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `mullusi.json`.
noVNC observer access uses VNC auth by default and Mullusi emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `mullusi-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `MULLUSI_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `MULLUSI_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `MULLUSI_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.mullusi/agents/main/agent",
...
For `type: "acp"` entries, Mullusi resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Mullusi starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Mullusi treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:mullusi`   | All built-in tools (excludes provider plugins)                                                                          |
...
- Files are materialized into the child workspace at `.mullusi/attachments/<uuid>/` with a `.manifest.json`.
...
Mullusi uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.mullusi/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `MULLUSI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Mullusi forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `mullusi onboard --auth-choice opencode-zen` or `mullusi onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `mullusi onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `mullusi onboard --auth-choice moonshot-api-key-cn`.

Native Moonshot endpoints advertise streaming usage compatibility on the shared
`openai-completions` transport, and Mullusi now keys that off endpoint
...
Anthropic-compatible, built-in provider. Shortcut: `mullusi onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `mullusi onboard --auth-choice synthetic-api-key`.
...
`mullusi onboard --auth-choice minimax-global-api` or
`mullusi onboard --auth-choice minimax-cn-api`.
The model catalog now defaults to M2.7 only.
On the Anthropic-compatible streaming path, Mullusi disables MiniMax thinking
...
- `install.nodeManager`: node installer preference for `metadata.mullusi.install`
...
- Loaded from `~/.mullusi/extensions`, `<workspace>/.mullusi/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Mullusi plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Mullusi plugin schema when available).
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Mullusi applies those as sanitized agent settings, not as raw Mullusi config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `mullusi plugins update`.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Mullusi to discover `/json/version`; use WS(S)
...
- Local managed `mullusi` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or MULLUSI_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `MULLUSI_GATEWAY_PORT` > `gateway.port` > `18790`.
- `bind`: `auto`, `loopback` (default), `lan` (`0.0.0.0`), `tailnet` (Tailscale IP only), or `custom`.
- **Legacy bind aliases**: use bind mode values in `gateway.bind` (`auto`, `loopback`, `lan`, `tailnet`, `custom`), not host aliases (`0.0.0.0`, `127.0.0.1`, `localhost`, `::`, `::1`).
- **Docker note**: the default `loopback` bind listens on `127.0.0.1` inside the container. With Docker bridge networking (`-p 18790:18790`), traffic arrives on `eth0`, so the gateway is unreachable. Use `--network host`, or set `bind: "lan"` (or `bind: "custom"` with `customBindHost: "0.0.0.0"`) to listen on all interfaces.
...
- `MULLUSI_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `MULLUSI_APNS_RELAY_BASE_URL` / `MULLUSI_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `MULLUSI_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
Convenience flags: `--dev` (uses `~/.mullusi-dev` + port `19001`), `--profile <name>` (uses `~/.mullusi-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/mullusi/tls/server.crt",
      keyPath: "/etc/mullusi/tls/server.key",
      caPath: "/etc/mullusi/tls/ca-bundle.crt",
    },
  },
}
```
...
Auth: `Authorization: Bearer <token>` or `x-mullusi-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `MULLUSI_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.mullusi/workspace/canvas",
    liveReload: true,
    // enabled: false, // or MULLUSI_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__mullusi__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__mullusi__/a2ui/`
...
- Also serves A2UI at `/__mullusi__/a2ui/`.
...
- Hostname defaults to `mullusi`. Override with `MULLUSI_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.mullusi/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `mullusi dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.mullusi/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${MULLUSI_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `mullusi.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      vault: {
        source: "exec",
        command: "/usr/local/bin/mullusi-vault-resolver",
...
- Legacy OAuth imports from `~/.mullusi/credentials/oauth.json`.
...
- Default log file: `/tmp/mullusi/mullusi-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Mullusi.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `MULLUSI_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `mullusi doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `mullusi.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
