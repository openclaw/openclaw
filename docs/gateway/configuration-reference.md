...
- `memoryPressureBundle`: writes a redacted pre-OOM stability bundle when memory pressure reaches `critical` (default: `true`). Set to `false` to keep memory pressure events but skip the bundle file scan/write.
