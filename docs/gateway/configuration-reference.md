...
description: "Complete field-by-field reference for ~/.activi/activi.json"
---

# Configuration Reference

Every field available in `~/.activi/activi.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Activi uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `activi doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["activi-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `activi plugins install @activi/mattermost`.
...
Activi spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@activi"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `activi.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Activi auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Activi assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.activi/sandboxes`
...
noVNC observer access uses VNC auth by default and Activi emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `activi-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.activi/agents/main/agent",
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:activi`   | All built-in tools (excludes provider plugins)                                           |
...
Activi uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.activi/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ACTIVI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `activi onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `activi onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `activi onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `activi onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `activi onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `activi onboard --auth-choice minimax-api`.
...
- Loaded from `~/.activi/extensions`, `<workspace>/.activi/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ACTIVI_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ACTIVI_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.activi-dev` + port `19001`), `--profile <name>` (uses `~/.activi-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-activi-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ACTIVI_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or ACTIVI_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__activi__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__activi__/a2ui/`
...
- Also serves A2UI at `/__activi__/a2ui/`.
...
- Hostname defaults to `activi`. Override with `ACTIVI_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.activi/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `activi dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.activi/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ACTIVI_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.activi/credentials/oauth.json`.
...
- Default log file: `/tmp/activi/activi-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `activi doctor --fix` can strip unknown keys).
