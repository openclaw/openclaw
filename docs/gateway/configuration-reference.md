...
- `gateway.auth.rateLimit`: failed-auth limiter config. Applies per client IP and per auth scope (shared-secret and device-token are tracked independently). Blocked attempts return `429` + `Retry-After`.
  - Non-loopback shared-secret gateway binds (`token` / `password`) get the default limiter automatically when you do not configure one explicitly: `{ maxAttempts: 10, windowMs: 60000, lockoutMs: 300000, exemptLoopback: true }`.
