...
- `plugins.entries.<id>.hooks.allowConversationAccess`: when `true`, trusted non-bundled plugins may read raw conversation content from typed hooks such as `before_model_call`, `llm_input`, `llm_output`, `before_agent_finalize`, and `agent_end`.
