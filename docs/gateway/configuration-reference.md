...
- `gateway.auth.mode: "trusted-proxy"`: delegate browser/user auth to an identity-aware reverse proxy and trust identity headers from `gateway.trustedProxies` (see [Trusted Proxy Auth](/gateway/trusted-proxy-auth)). This mode expects a **non-loopback** proxy source by default; same-host loopback reverse proxies require explicit `gateway.auth.trustedProxy.allowLoopback = true`. Direct internal callers must go through the proxy or use a different auth mode; trusted-proxy mode does not use `gateway.auth.password` / `OPENCLAW_GATEWAY_PASSWORD` as a local direct fallback, and `gateway.auth.token` remains mutually exclusive with trusted-proxy mode.
