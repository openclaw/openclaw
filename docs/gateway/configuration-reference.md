...
description: "Complete field-by-field reference for ~/.hanzo/bot.json"
summary: "Complete reference for every Bot config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.hanzo/bot.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Bot uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.hanzo/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `bot doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["bot-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-bot",
          requireMention: false,
...
- Bot additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `bot plugins install @hanzo/mattermost`.
...
- If you add a non-default account via `bot channels add` (or channel onboarding) while still on a single-account top-level channel config, Bot moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `bot doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@bot", "bot"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@bot"] },
      },
    ],
  },
}
...
Optional repository root shown in the system prompt's Runtime line. If unset, Bot auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Bot assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.hanzo/sandboxes`
...
noVNC observer access uses VNC auth by default and Bot emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `bot-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.hanzo/workspace",
        agentDir: "~/.hanzo/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.hanzo/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.hanzo/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.hanzo/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.hanzo/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Bot starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.hanzo/settings/tts.json",
      elevenlabs: {
...
Bot uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.hanzo/agents/<agentId>/agent/models.json`.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `bot onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `bot onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `bot onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `bot onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `bot onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `bot onboard --auth-choice minimax-api`.
...
- `plugins.installs`: CLI-managed install metadata used by `bot plugins update`.
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.hanzo/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Writes a unicast DNS-SD zone under `~/.hanzo/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `bot dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.hanzo/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.hanzo/credentials/oauth.json`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `bot doctor --fix` can strip unknown keys).
