...
Sub-agent bootstrap currently uses the runtime's reduced allowlist from the
workspace bootstrap set. Separate `agents.defaults.subagents.agentsFile` and
`agents.defaults.subagents.agentsFilesByModel` config keys are not part of the
documented schema today.
