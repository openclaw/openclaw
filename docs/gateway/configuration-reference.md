...
description: "Complete field-by-field reference for ~/.smart-agent-neo/smart-agent-neo.json"
---

# Configuration Reference

Every field available in `~/.smart-agent-neo/smart-agent-neo.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — SmartAgentNeo uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `smart-agent-neo doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["smart-agent-neo-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `smart-agent-neo plugins install @smart-agent-neo/mattermost`.
...
SmartAgentNeo spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@smart-agent-neo"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `smart-agent-neo.json`).
...
Default: `~/.smart-agent-neo/workspace`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, SmartAgentNeo auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, SmartAgentNeo assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.smart-agent-neo/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.smart-agent-neo/agents/main/agent",
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:smart-agent-neo`   | All built-in tools (excludes provider plugins)                                           |
...
SmartAgentNeo uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.smart-agent-neo/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SMART_AGENT_NEO_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `smart-agent-neo onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `smart-agent-neo onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `smart-agent-neo onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `smart-agent-neo onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `smart-agent-neo onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `smart-agent-neo onboard --auth-choice minimax-api`.
...
- Loaded from `~/.smart-agent-neo/extensions`, `<workspace>/.smart-agent-neo/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SMART_AGENT_NEO_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SMART_AGENT_NEO_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.smart-agent-neo-dev` + port `19001`), `--profile <name>` (uses `~/.smart-agent-neo-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-smart-agent-neo-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SMART_AGENT_NEO_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or SMART_AGENT_NEO_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__smart-agent-neo__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__smart-agent-neo__/a2ui/`
...
- Also serves A2UI at `/__smart-agent-neo__/a2ui/`.
...
- Hostname defaults to `smart-agent-neo`. Override with `SMART_AGENT_NEO_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.smart-agent-neo/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `smart-agent-neo dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.smart-agent-neo/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SMART_AGENT_NEO_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.smart-agent-neo/credentials/oauth.json`.
...
- Default log file: `/tmp/betterbrand/smart-agent-neo-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `smart-agent-neo doctor --fix` can strip unknown keys).
