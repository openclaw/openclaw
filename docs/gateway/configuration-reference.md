...
- `subagents.allowAgents` is valid only on `agents.list[]` entries. Do not use `subagents.allow` or `subagents.allowlist` here, and do not put target-agent allowlists under `agents.defaults.subagents`.
