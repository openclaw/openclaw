...
- `memoryPressureSnapshot`: captures a redacted pre-OOM stability snapshot when memory pressure reaches `critical` (default: `true`). Set to `false` to keep memory pressure events but skip the stability bundle file scan/write.
