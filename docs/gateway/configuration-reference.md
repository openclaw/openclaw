...
- `controlUi.title`: custom browser tab title for the Control UI. Default: `"OpenClaw Control"` (the static HTML title). Set this when running multiple OpenClaw instances to distinguish their tabs. **Rollback note**: if you set this key and then roll back to a release that does not know about it, the gateway will fail to start due to schema validation. Remove the key before rolling back.
