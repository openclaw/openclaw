...
- `channels.discord.subagentProgress` controls Discord source-message progress for spawned subagents. It is default off; set `enabled=true` to opt in. When enabled, active tracked subagents show one count reaction (`1️⃣` through `🔟` by default), count changes replace the previous reaction, zero active subagents removes the count reaction, non-ok completions leave `🔴`, and Discord typing is refreshed while any tracked subagent remains active. Use `reactions.enabled=false`, `typing.enabled=false`, `typing.intervalMs`, `typing.maxDurationMs`, `reactions.runningOrdinals`, or `reactions.failure` to tune it; per-account overrides live under `channels.discord.accounts.<accountId>.subagentProgress`.
