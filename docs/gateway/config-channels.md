...
- In multi-account Telegram setups, root `channels.telegram.groups` can still participate in generic group ID allowlisting while an account has no local groups. Add account-local groups under the active account, for example `channels.telegram.accounts.default.groups` or `channels.telegram.accounts.alerts.groups`, when a named bot account needs account-scoped group options such as `requireMention`, `topics`, or per-group `allowFrom`; once an account-local `groups` map exists, that account uses it instead of the root map, so copy every allowed group or use `"*"`. `openclaw doctor` warns when root groups exist and enabled allowlisted accounts have no account-local groups.
...
- Base channel settings apply to all accounts unless overridden per account. Telegram group allowlists have a split contract in multi-account setups: root `channels.telegram.groups` can still participate in generic group ID allowlisting while an account has no local groups, but configure `channels.telegram.accounts.<accountId>.groups` for each named bot account that needs account-scoped group options. Once an account-local `groups` map exists, copy every allowed group there or use `"*"`, because that account uses the local map instead of the root map.
