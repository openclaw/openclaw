...
- Bind targets that shadow the reserved container mount points (`/workspace`, `/agent`) are also blocked by default. On the Docker and Podman backends, the generated read-only skills destination (`<workdir>/.openclaw-skills`, `/workspace/.openclaw-skills` by default) is reserved the same way: binds that target it, descend from it, or cover it are rejected so the managed skills mount stays authoritative. After an upgrade, a bind that used to be accepted can now hit this reservation — relocate the bind to another container path instead of overriding the check. If you fully understand the consequences, the reservation can be overridden with `agents.defaults.sandbox.docker.dangerouslyAllowReservedContainerTargets: true`, but that flag exists for exceptional cases, not as the normal remedy.
