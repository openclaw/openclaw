...
A Gateway restart that interrupts pending provisioning leaves the nonactivated placement in `provisioning`, but does not automatically resume provider preparation, setup, or activation without the initiating authority. The environment reports that the interrupted placement was retained. Inspect the child, use **Stop cloud worker…** to finish cleanup, then retry with a fresh authorized request. A destruction request already recorded before the restart continues under its existing cleanup owner.

An allocation that already became active remains owned by the child/session: independent reconnect, renewal, and normal registered-run recovery are preserved. Activation does not prove that the initial task was accepted. If its reply was lost before run registration, saved input may remain interrupted and require a fresh submission. Retained allocations may continue incurring costs until explicit Stop, provider policy, or configured idle cleanup; `suspendAfter` is optional and does not clean unfinished provisioning placements. No automatic exact orphan cleanup is promised.
