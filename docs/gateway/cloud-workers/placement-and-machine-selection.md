...
A worker profile's `settings.target` and `settings.class` remain its defaults for RPC calls that omit overrides; an omitted Crabbox target means Linux. The Control UI preselects the available `small` machine class for a new cloud placement, even when the configured class is larger. If Small reports 10 or more vCPUs, or is absent, the picker prefers the first reported class below 10 vCPUs. When neither is available, it retains the configured default or first catalog choice rather than inventing an unsupported size. Unknown Small dimensions remain unknown: a class name alone does not guarantee a CPU ceiling, particularly for macOS and provider-native size pins. Explicitly selected larger machines remain available. In the Control UI New Session picker, hover or focus a configurable profile under **Cloud** to open its **Operating system** and **Machine** options. Only available operating systems appear. Before the profile is selected, dashed outlines identify its preselected choices. Choosing any option selects that profile and activates the preselection for the other setting. The machine list shows classes for the selected operating system, plus any classes that apply to every system, with reported vCPU and RAM when available and the default marked. Changing the operating system clears a selected class if it is unavailable for that system.

The selected profile row and the closed picker show the operating system and machine in muted text beside the profile name, using the selected values or picker preselection. New Session and the Dispatch, Move, and Restart dialogs submit the displayed machine and operating system explicitly, so a configured larger default cannot silently replace the preselection. Existing placements and recovered requests keep their admitted choices. Only metadata reported by the provider is shown. To override the operating system or size for one new placement over RPC, pass `os` and/or `machineClass` with `profileId`:
