...
The bundled Crabbox provider supports Linux on every supported Crabbox version. It also advertises Windows (WSL2), Windows, and macOS when the selected backend reports `windows/wsl2`, `windows/normal`, or `macos`, respectively, and the resolved Crabbox binary reports Crabbox 0.53.1 or newer. This gate compares the numeric major, minor, and patch components, ignoring prerelease and build metadata suffixes. For example, `0.53.1-dev`, `0.53.1`, and `0.54.0` qualify; `0.53.0` and unversioned `dev` builds do not. Older or unrecognized binaries advertise Linux only and reject non-Linux profile defaults, dispatches, and moves before allocation with an upgrade message. Restart the Gateway after upgrading the binary to refresh cached metadata. Desktop and warm images remain Linux only. The **Operating system** section appears when a provider advertises more than one target; `environments.list` omits `operatingSystems` for a single target, and each Crabbox machine option identifies its `os`.
