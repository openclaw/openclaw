...
- `subagents.model`: per-agent model default for native `sessions_spawn` when this agent is the target. Use `"inherit"` to follow the requester session's active model unless `"inherit"` is a configured model alias.
