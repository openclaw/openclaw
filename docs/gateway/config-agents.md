...
- **`reset`**: primary reset policy. `daily` resets at `atHour` local time; `idle` resets after `idleMinutes`; `adaptive` waits for both the daily boundary and `idleMinutes`. With `daily` plus `idleMinutes`, whichever expires first wins.
