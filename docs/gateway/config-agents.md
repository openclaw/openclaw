...
- `subagents.model`: default model for native `sessions_spawn` runs when the requester agent does not set `agents.list[].subagents.model` and the tool call omits `model`. Use `"inherit"` to follow the requester session's active model; otherwise use an alias or `provider/model` ref. Model aliases are resolved first, so an existing alias named `"inherit"` still selects that concrete model. ACP spawns only use concrete configured subagent models; explicit `model: "inherit"` is rejected for `runtime: "acp"`. Resolution order is explicit tool `model`, per-agent `subagents.model`, default `subagents.model`, target-agent `model`, then global/runtime default.
...
- `subagents.model`: per-agent model default for native `sessions_spawn` when this agent is the target. Use `"inherit"` to follow the requester session's active model unless `"inherit"` is a configured model alias. ACP spawns use concrete subagent model defaults only; explicit `model: "inherit"` is rejected for `runtime: "acp"`.
