  ...
  agents: {
    defaults: {
      compaction: {
        ...
        fallbacks: ["openai/gpt-5.5"], // optional compaction-only replacement fallback chain
...
- `fallbacks`: optional ordered replacement fallback chain for compaction summarization only. When unset, an explicit `model` remains exact and does not inherit the session model fallback chain. When set, OpenClaw tries the resolved compaction model first, then these fallback models. Set `[]` to keep compaction strict after the primary compaction model fails.
