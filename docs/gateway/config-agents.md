...
- `models`: optional per-agent model catalog/allowlist for `/model` and runtime model overrides. When omitted, the agent inherits `agents.defaults.models`; when present, it replaces the default allowlist for that agent and inherited default fallbacks do not punch through it. Entries use the same shape as `agents.defaults.models` (`alias`, `params`, `streaming`).
