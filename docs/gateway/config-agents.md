...
- `maxActiveTranscriptTokens`: optional Codex app-server native-thread token reuse guard (`number` or strings like `"120k"`). Defaults to `70000` when unset, starts a fresh native thread when the bound Codex transcript reaches the threshold, and may be set to `0` to disable only this proactive token guard while preserving byte limits and semantic binding invalidation.
