...
- `params.safetyIdentifier`/`params.safety_identifier`: OpenAI [safety identifier](https://developers.openai.com/api/docs/guides/safety-best-practices#safety_identifiers) sent as `safety_identifier` on native OpenAI Platform (`api.openai.com`) Responses and Chat Completions requests so OpenAI can attribute abuse-monitoring signals to your deployment or end user. Use a stable hashed value (for example a SHA-256 of the user id), never raw PII. Ignored for the ChatGPT/Codex backend, Azure OpenAI, and OpenAI-compatible proxies; caller-provided `safety_identifier` values win.
