...
- `reasoningDefault`: optional per-agent default reasoning visibility (`on | off | stream | verbose`). Overrides `agents.defaults.reasoningDefault` for this agent when no per-message or session reasoning override is set.
