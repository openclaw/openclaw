...
- As a text fallback, a standalone final `NO_REPLY` line also suppresses a heartbeat reply when the preceding text is at most 300 characters (after trimming). Inline mentions, longer replies, ordinary chat replies, execution-completion relays, and explicit errors are not suppressed by this fallback. Attachments and structured reply content remain deliverable. Use `heartbeat_respond` for an explicit notification decision rather than relying on this tolerance.
