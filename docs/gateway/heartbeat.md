...
Scheduled heartbeats require automations. When `cron.enabled` is `false` or `OPENCLAW_SKIP_CRON=1`, the gateway logs a startup warning and does not run scheduled heartbeats. Its heartbeat monitor rows converge to disabled, so `openclaw cron list` shows no scheduled heartbeat; the configured cadence is retained and restores when the scheduler is enabled again. Manual and event-driven heartbeat wakes remain available. There is no separate heartbeat fallback timer.
