...
- During heartbeat runs, EVOX.sh treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `evox sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, EVOX.sh skips the heartbeat run entirely (no model call).
...
headers like `# Heading`), EVOX.sh skips the heartbeat run to save API calls.
...
```bash
evox system event --text "Check for urgent follow-ups" --mode now
```
