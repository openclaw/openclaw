...
- During heartbeat runs, DNA treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `dna sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, DNA skips the heartbeat run entirely (no model call).
...
headers like `# Heading`), DNA skips the heartbeat run to save API calls.
...
```bash
dna system event --text "Check for urgent follow-ups" --mode now
```
