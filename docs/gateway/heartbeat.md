...
- During heartbeat runs, EasyHub treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `EasyHub sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, EasyHub skips the heartbeat run entirely (no model call).
...
headers like `# Heading`), EasyHub skips the heartbeat run to save API calls.
