...
- During heartbeat runs, SmartAgentNeo treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `smart-agent-neo sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SmartAgentNeo skips the heartbeat run entirely (no model call).
...
headers like `# Heading`), SmartAgentNeo skips the heartbeat run to save API calls.
