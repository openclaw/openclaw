...
- `web_search_options`: object; request-scoped options for provider-native `web_search` on eligible OpenAI-family Responses backends. Supports `search_context_size: "low" | "medium" | "high"` and OpenAI Chat-style approximate `user_location`. Unsupported fields or invalid values return `400 invalid_request_error`.
...
`web_search_options` does not select a managed OpenClaw search provider. It only customizes native OpenAI/Codex `web_search` when that native search path is already allowed by the selected model, `tools.web.search` config, and tool policy. If a request includes `web_search_options` but the selected backend cannot honor native search, the run fails explicitly instead of silently falling back to managed `web_search`. See [Web search](/tools/web#native-openai-web-search) for native search activation rules.
