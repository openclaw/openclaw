---
summary: "All configuration options for ~/.dna/dna.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

DNA reads an optional **JSON5** config from `~/.dna/dna.json` (comments + trailing commas allowed).

If the file is missing, DNA uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
DNA only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `dna doctor`, `dna logs`, `dna health`, `dna status`, `dna service`, `dna help`).
- Run `dna doctor` to see the exact issues.
- Run `dna doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `dna config set`. Keep a backup of `~/.dna/dna.json`.
...
```bash
dna gateway call config.get --params '{}' # capture payload.hash
dna gateway call config.apply --params '{
...
```bash
dna gateway call config.get --params '{}' # capture payload.hash
dna gateway call config.patch --params '{
...
{ "$include": "/etc/dna/base.json5" } // absolute
...
```json5
// ~/.dna/clients/mueller/agents.json5
[
...
```json5
// ~/.dna/clients/mueller/broadcast.json5
{
...
DNA reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, DNA runs your login shell and imports only the missing expected keys (never overrides).
...
DNA stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.dna/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.dna/credentials/oauth.json` (or `$DNA_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.dna/agent/*` (migrated by `dna doctor` into `~/.dna/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `DNA_OAUTH_DIR`
- Agent dir (default agent root override): `DNA_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, DNA imports `oauth.json` entries into `auth-profiles.json`.
...
If set, DNA derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/dna/dna-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/dna/dna.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.dna/credentials/whatsapp/biz
          // authDir: "~/.dna/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `dna doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "dna", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.dna/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `dna.json`).
...
DNA starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
DNA starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `dna plugins install @dna/mattermost` (or `./extensions/mattermost` from a git checkout).
...
DNA starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
DNA spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, DNA
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/dna" } }
}
```
...
When a file exceeds this limit, DNA logs a warning and injects a truncated
...
message envelopes). If unset, DNA uses the host timezone at runtime.
...
`[dna]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[dna]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[dna]"`, DNA will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, DNA generates audio
...
DNA also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, DNA currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `dna configure --section web`, or use `BRAVE_API_KEY` env var)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.dna/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "dna-sandbox-browser:bookworm-slim",
          containerPrefix: "dna-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
DNA uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.dna/agents/<agentId>/agent/models.json` or by defining the same schema inside your
DNA config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, DNA writes/merges a `models.json` into
`~/.dna/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. DNA uses
...
- Shortcut: `dna onboard --auth-choice opencode-zen`.
...
Shortcut: `dna onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `dna onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `dna onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `dna onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `dna onboard --auth-choice minimax-api`.
...
- Override the agent config root with `DNA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.dna/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.dna/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, DNA stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.dna/skills` (workspace skills
...
from `~/.dna/extensions`, `<workspace>/.dna/extensions`, plus any
...
DNA can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (DNA.app menubar, or `dna gateway`).
...
- When a connection comes from one of these IPs, DNA uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `dna gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `DNA_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `DNA_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. DNA
...
macOS app behavior:
- DNA.app watches `~/.dna/dna.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.dna/dna.json` (or `DNA_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.dna/dna.json` (or `DNA_CONFIG_PATH`)
...
- `DNA_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `dna --dev …` → uses `~/.dna-dev` + shifts ports from base `19001`
- `dna --profile <name> …` → uses `~/.dna-<name>` (port via config/env/flags)
...
Gmail helper config (used by `dna webhooks gmail setup` / `run`):
...
- Set `DNA_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, DNA defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__dna/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__dna__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.dna/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.dna/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.dna/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_dna-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_dna-bridge._tcp` under `~/.dna/dns/` using the standard discovery domain `dna.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `dna.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `dna.internal` via that server
