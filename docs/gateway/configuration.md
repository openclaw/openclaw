---
summary: "All configuration options for ~/.easyhub/easyhub.json with examples"
read_when:
...
EasyHub reads an optional **JSON5** config from `~/.easyhub/easyhub.json` (comments + trailing commas allowed).

If the file is missing, EasyHub uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.easyhub/workspace`). You usually only need a config to:
...
EasyHub only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `EasyHub doctor`, `EasyHub logs`, `EasyHub health`, `EasyHub status`, `EasyHub service`, `EasyHub help`).
- Run `EasyHub doctor` to see the exact issues.
- Run `EasyHub doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `EasyHub config set`. Keep a backup of `~/.easyhub/easyhub.json`.
...
{ "$include": "/etc/EasyHub/base.json5" } // absolute
...
EasyHub reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, EasyHub runs your login shell and imports only the missing expected keys (never overrides).
...
EasyHub stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.easyhub/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.easyhub/credentials/oauth.json` (or `$EASYHUB_STATE_DIR/credentials/oauth.json`)
...
- `~/.easyhub/agent/*` (migrated by `EasyHub doctor` into `~/.easyhub/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `EASYHUB_OAUTH_DIR`
- Agent dir (default agent root override): `EASYHUB_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, EasyHub imports `oauth.json` entries into `auth-profiles.json`.
...
If set, EasyHub derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/EasyHub/EasyHub-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/EasyHub/easyhub.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.easyhub/credentials/whatsapp/biz
...
- The legacy single-account Baileys auth dir is migrated by `EasyHub doctor` into `whatsapp/default`.
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.easyhub/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.easyhub/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `easyhub.json`).
...
EasyHub starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["EasyHub-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
EasyHub starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `EasyHub plugins install @easyhub/mattermost` (or `./extensions/mattermost` from a git checkout).
...
EasyHub starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
EasyHub spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, EasyHub
...
When a file exceeds this limit, EasyHub logs a warning and injects a truncated
...
message envelopes). If unset, EasyHub uses the host timezone at runtime.
...
`[EasyHub]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[EasyHub]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[EasyHub]"`, EasyHub will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, EasyHub generates audio
...
EasyHub also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, EasyHub currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `EasyHub configure --section web`, or use `BRAVE_API_KEY` env var)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.easyhub/sandboxes`
...
EasyHub uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.easyhub/agents/<agentId>/agent/models.json` or by defining the same schema inside your
EasyHub config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, EasyHub writes/merges a `models.json` into
`~/.easyhub/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. EasyHub uses
...
- Shortcut: `EasyHub onboard --auth-choice opencode-zen`.
...
Shortcut: `EasyHub onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `EasyHub onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `EasyHub onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `EasyHub onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `EasyHub onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `EasyHub onboard --auth-choice minimax-api`.
...
- Override the agent config root with `EASYHUB_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.easyhub/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.easyhub/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, EasyHub stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.easyhub/skills` (workspace skills
...
from `~/.easyhub/extensions`, `<workspace>/.easyhub/extensions`, plus any
...
### `browser` (EasyHub-managed browser)

EasyHub can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for EasyHub and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (EasyHub.app menubar, or `EasyHub gateway`).
...
- Examples: `"/ui"`, `"/EasyHub"`, `"/apps/EasyHub"`.
...
- When a connection comes from one of these IPs, EasyHub uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `EasyHub gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `EASYHUB_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `EASYHUB_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. EasyHub
...
- EasyHub.app watches `~/.easyhub/easyhub.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.easyhub/easyhub.json` (or `EASYHUB_CONFIG_PATH`) and applies changes automatically.
...
- `~/.easyhub/easyhub.json` (or `EASYHUB_CONFIG_PATH`)
...
- `EASYHUB_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `EasyHub --dev …` → uses `~/.easyhub-dev` + shifts ports from base `19001`
- `EasyHub --profile <name> …` → uses `~/.EasyHub-<name>` (port via config/env/flags)
...
Gmail helper config (used by `EasyHub webhooks gmail setup` / `run`):
...
- Set `EASYHUB_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, EasyHub defaults `serve.path` to `/` so
...
Default root: `~/.easyhub/workspace/canvas`  
Default port: `18793` (chosen to avoid the EasyHub browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__EASYHUB__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__EASYHUB__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `EASYHUB_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.easyhub/bridge/tls/bridge-{cert,key}.pem when omitted.
...
Controls LAN mDNS discovery broadcasts (`_EasyHub-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `EasyHub` (advertises `EasyHub.local`). Override with `EASYHUB_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_EasyHub-gw._tcp` under `~/.easyhub/dns/` using the configured discovery domain (example: `EasyHub.internal.`).
