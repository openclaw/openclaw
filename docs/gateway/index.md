...
Installed gateway services record the resolved `--port` in their supervisor
metadata. After changing `gateway.port`, run `openclaw gateway install --force`
and restart the gateway service so launchd/systemd/schtasks starts the process
on the new port.
...
Do not also let `openclaw doctor --fix` install a user-level gateway service for
the same profile/port. Doctor refuses that automatic install when it finds a
system-level OpenClaw gateway-like service; use `OPENCLAW_SERVICE_REPAIR_POLICY=external`
when the system unit owns the lifecycle.
