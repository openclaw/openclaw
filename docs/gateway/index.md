...
- Replaces the legacy `gateway` command. CLI entry point: `EasyHub gateway`.
...
EasyHub gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.easyhub/easyhub.json` (or `EASYHUB_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__EASYHUB__/canvas/` from `~/.easyhub/workspace/canvas`. Disable with `canvasHost.enabled=false` or `EASYHUB_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `EASYHUB_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `EASYHUB_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.EasyHub.*` may still exist)
- Linux: `EasyHub-gateway-<profile>.service`
- Windows: `EasyHub Gateway (<profile>)`

Install metadata is embedded in the service config:

- `EASYHUB_SERVICE_MARKER=EasyHub`
- `EASYHUB_SERVICE_KIND=gateway`
- `EASYHUB_SERVICE_VERSION=<version>`
...
```bash
EasyHub --dev setup
EasyHub --dev gateway --allow-unconfigured
...
- `EASYHUB_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.easyhub/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `EASYHUB_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `EASYHUB_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `EASYHUB_CONFIG_PATH`
- unique `EASYHUB_STATE_DIR`
...
```bash
EasyHub --profile main gateway install
EasyHub --profile rescue gateway install
```

Example:

```bash
EASYHUB_CONFIG_PATH=~/.easyhub/a.json EASYHUB_STATE_DIR=~/.EasyHub-a EasyHub gateway --port 19001
EASYHUB_CONFIG_PATH=~/.easyhub/b.json EASYHUB_STATE_DIR=~/.EasyHub-b EasyHub gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `EasyHub health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `EasyHub`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `EasyHub gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.EasyHub.*` is cleaned up).
  - `EasyHub doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are EasyHub profile services.
  ...
  - Cleanup: `EasyHub gateway uninstall` (current service) and `EasyHub doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `EasyHub gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- EasyHub.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.EasyHub.*` labels still unload cleanly).
- To stop it cleanly, use `EasyHub gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `EasyHub gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `EasyHub gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

EasyHub installs a **systemd user service** by default on Linux/WSL2. We
...
`EasyHub gateway install` writes the user unit. `EasyHub doctor` audits the
...
Restart=always
RestartSec=5
Environment=EASYHUB_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/EasyHub-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now EasyHub-gateway[-<profile>].service
```
...
- `EasyHub gateway health|status` — request health/status over the Gateway WS.
- `EasyHub message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `EasyHub agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `EasyHub gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `EasyHub gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `EasyHub gateway` and the legacy TCP control port.
