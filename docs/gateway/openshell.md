---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Mullusi agents"
read_when:
...
OpenShell is a managed sandbox backend for Mullusi. Instead of running Docker
containers locally, Mullusi delegates sandbox lifecycle to the `openshell` CLI,
...
- Mullusi Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Mullusi creates an OpenShell
...
- Before `exec`, Mullusi syncs the local workspace into the OpenShell sandbox.
- After `exec`, Mullusi syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace
  remains the source of truth between turns.

Best for:

- You edit files locally outside Mullusi and want those changes visible in the
...
- When the sandbox is first created, Mullusi seeds the remote workspace from
...
- Mullusi does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Mullusi after the initial seed,
the remote sandbox does **not** see those changes. Use
`mullusi sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"mullusi"`  | Sandbox source for first-time create                  |
...
1. Mullusi calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Mullusi calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `mullusi sandbox` commands
