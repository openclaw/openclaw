---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for EVOX.sh agents"
read_when:
...
OpenShell is a managed sandbox backend for EVOX.sh. Instead of running Docker
containers locally, EVOX.sh delegates sandbox lifecycle to the `openshell` CLI,
...
- EVOX.sh Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, EVOX.sh creates an OpenShell
...
- Before `exec`, EVOX.sh syncs the local workspace into the OpenShell sandbox.
- After `exec`, EVOX.sh syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace
  remains the source of truth between turns.

Best for:

- You edit files locally outside EVOX.sh and want those changes visible in the
...
- When the sandbox is first created, EVOX.sh seeds the remote workspace from
...
- EVOX.sh does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside EVOX.sh after the initial seed,
the remote sandbox does **not** see those changes. Use
`evox sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"evox"`      | Sandbox source for first-time create                  |
...
1. EVOX.sh calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. EVOX.sh calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `evox sandbox` commands
