...
The CA is generated once per Gateway start under the state directory with a ten-year certificate validity window. Its key is still process-owned, not retained for ten years. One-day leaf certificates renew on demand within their final hour without replacing the CA or interrupting established TLS connections. This keeps already-running subprocesses trusting the same issuer across renewal. Its directory is mode `0700`, its private keys are mode `0600`, it is removed during Gateway shutdown, and OpenClaw never installs it in a system trust store. Requests fail closed when a sentinel cannot be authenticated or resolved; the proxy never forwards or silently strips an unresolved sentinel.

The proxy also preserves the request framing an origin depends on. A body the client declares at or below 1 MiB is substituted in bounded memory and forwarded with the substituted byte length as `Content-Length`, never the client's declared length, because substitution can resize a body. Origins that reject chunked uploads require that final length; GitHub's attachment endpoint answers `400 Bad Content-Length` without it. Larger and unknown-length bodies keep streaming with a bounded carry window and chunked framing, so substitution also works when a sentinel crosses chunk boundaries or appears in a large upload. A body refused while it is prepared never reaches the upstream transport, because the proxy sends nothing until the substitution completes.
