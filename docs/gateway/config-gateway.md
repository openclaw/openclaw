...
- `controlUi.remoteImageOrigins`: exact bare HTTP or HTTPS origins allowed for browser-direct chat Markdown images. Origins are normalized and exact-match scheme, hostname, and effective port; wildcards, credentials, paths, queries, and fragments are rejected. This does not proxy images or provide Gateway-side SSRF or redirect enforcement. HTTP can be blocked as mixed content on an HTTPS Control UI page.
