...
```json5
{
  gateway: {
    ...
    controlUi: {
      ...
      // remoteImageOrigins: ["https://images.example.com"], // exact browser-direct Markdown image origins
...
- `controlUi.remoteImageOrigins`: exact bare HTTP or HTTPS origins allowed for browser-direct chat Markdown images. Origins are normalized and exact-match scheme, hostname, and effective port; wildcards, credentials, paths, queries, and fragments are rejected. This does not proxy images or provide Gateway-side SSRF or redirect enforcement. `remoteImageOrigins` cannot override browser mixed-content protection: an HTTPS Control UI page may block or upgrade an allowed HTTP image. To avoid mixed-content blocking for a plain HTTP image endpoint, load the Control UI over HTTP too or serve that endpoint over HTTPS.
