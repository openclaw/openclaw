...
- `gateway.auth.allowTailscale`: when `true`, Tailscale Serve identity headers can satisfy Control UI/WebSocket auth (verified via `tailscale whois`). HTTP API endpoints do **not** use that Tailscale header auth; they follow the gateway's normal HTTP auth mode instead. This tokenless flow assumes the gateway host is trusted. Defaults to `true` when `tailscale.mode = "serve"`, except under `gateway.auth.mode: "password"` or `"trusted-proxy"`, which keep their explicit auth boundary until you set `allowTailscale` yourself. See [Optional identity-header auth](/gateway/stable-https-url#optional-identity-header-auth).
