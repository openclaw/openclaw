...
Mullusi can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Mullusi requires a shared password.
...
- `token` (default when `MULLUSI_GATEWAY_TOKEN` is set)
- `password` (shared secret via `MULLUSI_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Mullusi verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Mullusi only treats a request as Serve when it arrives from loopback with
...
- Control UI: `http://<tailscale-ip>:18790/`
- WebSocket: `ws://<tailscale-ip>:18790`

Note: loopback (`http://127.0.0.1:18790`) will **not** work in this mode.
...
Prefer `MULLUSI_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
mullusi gateway --tailscale serve
mullusi gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Mullusi to undo `tailscale serve`
