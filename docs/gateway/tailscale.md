...
EVOX.sh can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. EVOX.sh requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. EVOX.sh verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
EVOX.sh only treats a request as Serve when it arrives from loopback with
...
```bash
evox gateway --tailscale serve
evox gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want EVOX.sh to undo `tailscale serve`
