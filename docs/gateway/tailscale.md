...
Activi can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Activi requires a shared password.
...
- `token` (default when `ACTIVI_GATEWAY_TOKEN` is set)
- `password` (shared secret via `ACTIVI_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
Control UI/WebSocket auth can use Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Activi verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Activi only treats a request as Serve when it arrives from loopback with
...
Prefer `ACTIVI_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
activi gateway --tailscale serve
activi gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Activi to undo `tailscale serve`
