...
<AccordionGroup>
  ...
  <Accordion title="Operator terminal">
    - `terminal.open` starts a host PTY for an explicit `agentId`, the agent encoded in `sessionKey`, or the default agent when the fleet has one, and returns the resolved agent, working directory, shell, and confinement state. Passing `sessionKey` binds the PTY to that exact agent session and attaches the calling connection as its first viewer; omitting it creates a connection-owned operator terminal.
