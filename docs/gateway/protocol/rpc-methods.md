...
<AccordionGroup>
  ...
  <Accordion title="Session control">
    ...
    - `sessions.groups.list`, `sessions.groups.put`, `sessions.groups.rename`, and `sessions.groups.delete` manage the selected agent's custom session group catalog (names + display order). All six group methods accept optional `agentId` without changing protocol version 4. Omitted-owner reads use the ambient system owner (`agents.defaults.systemAgent.agentId`, with the existing legacy or sole-agent fallback); omitted-owner mutations require exactly one configured agent, otherwise they return `INVALID_REQUEST`. Explicit unknown agents are rejected. Clients should send the selected agent on every group request. The list response adds optional `agentId` identifying the resolved catalog owner, so clients need not infer it from roster order. The read-scoped list result is intentionally path-free. `sessions.groups.defaults` and `sessions.groups.update` require `operator.write` and read or replace one custom group's optional working-directory and worktree defaults. Non-admin callers can save only directories inside a configured agent workspace; other absolute Gateway paths require `operator.admin`. Membership stays on each session's `category` field; rename and delete update only that agent's member sessions server-side, even when agents share a physical session store. Identically named groups on another agent remain independent. `sessions.groups.put` replaces only the name list and order, and rejects dropping a group that still has member sessions — delete it explicitly first. Dropping a group participates in the same member-session authorization as delete.
