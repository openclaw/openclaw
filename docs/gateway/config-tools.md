...
<AccordionGroup>
  ...
  <Accordion title="Model catalog entries">
    ...
    - `models.providers.*.models.*.compat.requiresReasoningContentOnAssistantMessages`: optional compatibility hint for DeepSeek-style Chat Completions backends that require prior assistant messages to keep their `reasoning_content` field on replay (otherwise the backend rejects the turn or drops reasoning context). When `true`, OpenClaw preserves `reasoning_content` on outgoing assistant messages instead of stripping it. Use this when wiring a custom DeepSeek-compatible proxy and validation rejects requests that strip reasoning. Default `false`.
