...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    Imports of the pre-June `tasks/runs.sqlite`, `flows/registry.sqlite`, and `plugin-state/state.sqlite` databases are retired. Doctor leaves these files untouched. Older installations that need their records should [migrate through `2026.6.1`](/install/updating) before updating further; June-and-later releases already use the shared state database.
