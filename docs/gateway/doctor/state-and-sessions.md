...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    Standalone SDK installations from 2026.9.x keep using a nonempty legacy `agent/` directory until Doctor migrates it, provided the canonical agent directory does not exist yet. Once the canonical directory exists, it takes precedence; Doctor reports and repairs any leftover legacy files. Explicit `OPENCLAW_AGENT_DIR` overrides remain authoritative.
