...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “SmartAgentNeo runs”).
...
- `smart-agent-neo health` and `smart-agent-neo status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `smart-agent-neo gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`SMART_AGENT_NEO_GATEWAY_PORT`).
