...
<AccordionGroup>
  ...
  <Accordion title="Session control">
    ...
    - `tasks.list` returns the background-task ledger with optional `status`, `agentId`, `sessionKey`, and `limit` filters. `tasks.get` reads one stable task id. `tasks.cancel` requests cancellation and returns `{ found, cancelled, reason?, task? }`.
