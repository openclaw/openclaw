...
- `params.safetyIdentifier`: OpenAI [safety identifier](https://developers.openai.com/api/docs/guides/safety-best-practices#safety_identifiers) sent as `safety_identifier` on native OpenAI Platform (`api.openai.com`) Responses and Chat Completions requests so OpenAI can attribute abuse-monitoring signals to your deployment or end user. Use a stable hashed value of at most 64 characters (OpenAI's documented limit; for example a truncated SHA-256 of the user id), never raw PII. Longer, empty, or non-string values are ignored with a warning. Ignored for the ChatGPT/Codex backend, Azure OpenAI, and OpenAI-compatible proxies; caller-provided `safety_identifier` values win. Only this camelCase spelling is recognised, so scope precedence (global, then per-model, then per-agent) applies by a single key.
