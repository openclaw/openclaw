...
- In strict mode, use `allowedHostnames` for explicit exceptions, including wildcard patterns such as `*.example.com` (which do not match the bare domain) and otherwise-blocked names like `localhost`.
