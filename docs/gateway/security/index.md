...
When `gateway.auth.allowTailscale` is `true` (default for OpenClaw-managed Serve), OpenClaw accepts the Tailscale Serve identity header `tailscale-user-login` for Control UI/WebSocket authentication after resolving `x-forwarded-for` through the local Tailscale daemon (`tailscale whois`) and matching the login. Separately managed Serve must also configure its loopback peer in `gateway.trustedProxies` and safely overwrite the forwarded chain. The permission setting alone never makes generic proxy headers trustworthy.
...
Do not forward Tailscale identity headers from a generic reverse proxy. Use shared-secret auth or [Trusted Proxy Auth](/gateway/trusted-proxy-auth) instead. The separately managed Serve exception requires explicit proxy trust, complete Serve headers, and a local WhoIs login match.
