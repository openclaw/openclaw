...
## Quick check: `smart-agent-neo security audit`
...
```bash
smart-agent-neo security audit
...
- Tighten local perms (`~/.smart-agent-neo` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

SmartAgentNeo is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, SmartAgentNeo also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.smart-agent-neo/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.smart-agent-neo/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.smart-agent-neo/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.smart-agent-neo/credentials/oauth.json`
...
`smart-agent-neo security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${SMART_AGENT_NEO_GATEWAY_PASSWORD}
```
...
SmartAgentNeo stores session transcripts on disk under `~/.smart-agent-neo/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.smart-agent-neo` (see the audit section below). If you need
...
SmartAgentNeo can refresh the skills list mid-session:
...
- If you install plugins from npm (`smart-agent-neo plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.smart-agent-neo/extensions/<pluginId>/` (or `$SMART_AGENT_NEO_STATE_DIR/extensions/<pluginId>/`).
  - SmartAgentNeo uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
smart-agent-neo pairing approve <channel> <code>
```
...
By default, SmartAgentNeo routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
SmartAgentNeo has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.smart-agent-neo/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.smart-agent-neo or your logs.”
...
4. **Re-run audit**
   - `smart-agent-neo security audit --deep` and confirm the report is clean.

## Lessons Learned (The Hard Way)

### The `find ~` Incident 🦞

On Day 1, a friendly tester asked Neo to run `find ~` and share the output. Neo happily dumped the entire home directory structure to a group chat.
...
- `~/.smart-agent-neo/smart-agent-neo.json`: `600` (user read/write only)
- `~/.smart-agent-neo`: `700` (user only)

`smart-agent-neo doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `SMART_AGENT_NEO_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__smart-agent-neo__/canvas/` and `/__smart-agent-neo__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_smart-agent-neo-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `SMART_AGENT_NEO_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `smart-agent-neo doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `SMART_AGENT_NEO_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `SMART_AGENT_NEO_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), SmartAgentNeo
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. SmartAgentNeo verifies the identity by resolving the
...
- SmartAgentNeo will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.smart-agent-neo/` (or `$SMART_AGENT_NEO_STATE_DIR/`) may contain secrets or private data:

- `smart-agent-neo.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `smart-agent-neo status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@smart-agent-neo", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.smart-agent-neo/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `smart-agent-neo` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only SmartAgentNeo clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default SmartAgentNeo limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `smart-agent-neo gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `SMART_AGENT_NEO_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/betterbrand/smart-agent-neo-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.smart-agent-neo/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + SmartAgentNeo version
...
```mermaid
flowchart TB
    A["Owner (Peter)"] -- Full trust --> B["AI (Neo)"]
...
Found a vulnerability in SmartAgentNeo? Please report responsibly:

1. Email: [security@smart-agent-neo.ai](mailto:security@smart-agent-neo.ai)
