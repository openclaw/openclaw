...
The verified Serve identity also satisfies the Control UI's own HTTP read routes: the dashboard bootstrap config (`/control-ui-config.json`), assistant-media reads, and avatar reads. These are read-only requests the dashboard issues while loading, and any request on them that carries a browser `Origin` header must additionally pass the Control UI browser-origin policy (same-origin with the request host or `gateway.controlUi.allowedOrigins`) - the same boundary the WebSocket surface enforces, so a cross-origin page cannot ride the ambient Tailscale identity.

All other HTTP API endpoints (`/v1/*`, `/tools/invoke`, `/api/channels/*`) do not use Tailscale identity-header auth - they follow the gateway's configured HTTP auth mode.
