...
- **Placement is `failed` after a dispatch or bootstrap failure** — a `failed` placement reached before the worker took ownership (no active worker yet) is not a dead end: the next turn reclaims it to `local` and runs locally so the session keeps working, and `sessions.dispatch` accepts `local` again once the underlying cause is fixed. A failure reached after the worker ran (during reconciliation) still holds worker ownership and recovers through drain/reconcile so remote workspace changes are not discarded.
