...
By default, only **caller-provided client tools** appear as `function_call` items
in the response `output`. Built-in tools the gateway-hosted agent runs itself
(for example `bash`, `read`, `grep`, `write`, `find`, `ls`, plus bundled plugin
tools) are executed inside the agent and do not show up in the response, so the
default response shape stays byte-identical for clients that have not opted in.
...
- **Audit-only.** Only the tool-start event (`phase: "start"`) is captured.
  Tool results are not surfaced, and there is no way to feed a
  `function_call_output` back for one of these calls — the agent has already
...
- **Ordering.** In non-streaming responses, audit `function_call` items are
  ...
  In streaming responses, each capture emits its own
  `response.output_item.added` and `response.output_item.done` events at the
  next free `output_index`, and the same items appear in the final
