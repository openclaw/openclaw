...
- `mcp.servers.<name>.sslVerify`, `clientCert`, `clientKey`: HTTP TLS controls
  for private endpoints and mutual TLS. Each client certificate/key PEM file is
  limited to 64 KiB; `openclaw mcp doctor` reports unreadable or oversized files.
