...
read_when:
  - Running SmartAgentNeo behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of SmartAgentNeo
...
- You run SmartAgentNeo behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. SmartAgentNeo checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. SmartAgentNeo extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://smart-agent-neo.example.com
    to: http://smart-agent-neo-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
smart-agent-neo.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy smart-agent-neo:18789 {
...
location / {
    ...
    proxy_pass http://smart-agent-neo:18789;
...
`smart-agent-neo security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update SmartAgentNeo config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `smart-agent-neo security audit` and review findings
