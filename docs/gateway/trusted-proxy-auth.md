...
read_when:
  - Running Mullusi behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Mullusi
...
- You run Mullusi behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Mullusi checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Mullusi extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that Mullusi can verify.
...
- Mullusi can stay on loopback HTTP behind the proxy.
...
If Mullusi itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://mullusi.example.com
    to: http://mullusi-gateway:18790
    policy:
      - allow:
          or:
            - email:
...
Mullusi rejects ambiguous configurations where both a `gateway.auth.token` (or `MULLUSI_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-mullusi-scopes`.

Examples:

- `x-mullusi-scopes: operator.read`
- `x-mullusi-scopes: operator.read,operator.write`
- `x-mullusi-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Mullusi honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-mullusi-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-mullusi-scopes` explicitly when you want a trusted-proxy request to
...
`mullusi security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
Mullusi rejected a loopback-source trusted-proxy request.
...
3. Update Mullusi config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `mullusi security audit` and review findings
