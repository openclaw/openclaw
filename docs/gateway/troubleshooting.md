...
4. **Optional: prevent maintenance sleep entirely with `caffeinate`** while the gateway is running. macOS ships a `caffeinate` utility that asserts power-management flags to keep the system awake. It can bind those assertions to a process in one of two ways: run the target as a child utility (`caffeinate ... -- <command> ...`), in which case the assertions are held for exactly as long as that child runs and released when it exits; or, with no utility argument, watch an existing process by PID (`caffeinate ... -w <pid>`) so the assertions release when that PID exits. Note that `-w` is ignored when a utility argument is also supplied — the child's lifetime is what bounds the assertion in that case. This is an out-of-tree operator pattern — OpenClaw does not ship a `caffeinate` wrapper, and two prior proposals to add one as a built-in ([#15444](https://github.com/openclaw/openclaw/issues/15444), [#40846](https://github.com/openclaw/openclaw/pull/40846)) did not land — but it is well established for always-on Mac services. Plex Media Server's community recipe prepends `/usr/bin/caffeinate -ims --` to the Plex `LaunchAgent` `ProgramArguments` for the same reason; the general pattern is covered by the `caffeinate(8)` manpage shipped with macOS (`man caffeinate`; source in Apple's [`PowerManagement` distribution](https://github.com/apple-oss-distributions/PowerManagement/blob/main/caffeinate/caffeinate.c), also mirrored as a third-party reference at [ss64.com](https://ss64.com/mac/caffeinate.html)) and helpers such as the open-source [`alwaysBeCaffeinating`](https://github.com/thomstratton/alwaysBeCaffeinating) LaunchDaemon.
   ...
   - Effective, on the observed host: holds `PreventSystemSleep` and `PreventUserIdleSystemSleep` as long as the gateway runs. On an Apple Silicon Mac mini, across a ~20 hour window after enabling, en0 did not flap and no ENETDOWN failures occurred: zero Maintenance Sleep events, zero new uncaught-exception bundles. This is a single-host observation, not a guarantee that caffeinate prevents interface flaps or ENETDOWN in general — the surrounding guide above only says a connection during Maintenance Sleep _can_ fail with ENETDOWN, and this mitigation reduces that exposure rather than eliminating the underlying driver behavior.
