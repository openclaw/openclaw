...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - `existing-session dialog handling does not support dialogId.` → dialog hooks on Chrome MCP profiles accept bounded timeout budgets but do not answer pending dialogs by `dialogId`.
