...
- `mullusi gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `mullusi doctor` reports no blocking config/service issues.
- `mullusi channels status --probe` shows live per-account transport status and,
...
mullusi config get agents.defaults.models
```
...
| ---------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `mullusi config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `mullusi devices list` then `mullusi devices approve <requestId>`.                                                                                                                                                                                            |
...
If `mullusi devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `mullusi devices rotate --scope ...` can only request operator scopes that
...
mullusi logs --follow
mullusi doctor
mullusi gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `mullusi onboard --mode local` / `mullusi setup` to restamp the expected local-mode config. If you are running Mullusi via Podman, the default config path is `~/.mullusi/mullusi.json`.
...
Use this when `mullusi gateway probe` reaches something, but still prints a warning block.

```bash
mullusi gateway probe
mullusi gateway probe --json
mullusi gateway probe --ssh user@gateway-host
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Mullusi skips the model call.
...
mullusi approvals get --node <idOrNameOrIp>
mullusi logs --follow
mullusi status
```
...
mullusi browser start --browser-profile mullusi
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `mullusi browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
```bash
mullusi gateway status
mullusi config get gateway.mode
mullusi config get gateway.remote.url
mullusi config get gateway.auth.mode
```
...
```bash
mullusi config get gateway.bind
mullusi config get gateway.auth.mode
