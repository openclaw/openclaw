...
- Critical memory pressure appears in logs but no bundle exists → check whether `diagnostics.enabled: false` or `diagnostics.memoryPressureBundle.enabled: false` is set.
