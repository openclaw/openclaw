---
summary: "Quick troubleshooting guide for common DNA failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When DNA misbehaves, here's how to fix it.
...
|---|---|---|
| `dna status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `dna status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `dna status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `dna gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `dna channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `dna gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `dna logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `dna status --all` (it redacts tokens). If you paste `dna status`, consider setting `DNA_SHOW_SECRETS=0` first (token previews).
...
```bash
dna models auth paste-token --provider anthropic
dna models status
```
...
**Logs:**
- Preferred: `dna logs --follow`
- File logs (always): `/tmp/dna/dna-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$DNA_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u dna-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "DNA Gateway (<profile>)" /V /FO LIST`
...
- Or set it directly:
  ```bash
  dna config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  dna config set gateway.mode remote
  dna config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `dna setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.dna/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `dna doctor`
...
- then run `dna sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `dna gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `dna gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `dna gateway status` and `dna doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `DNA_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `dna gateway status` shows a config mismatch**
...
- Fix: rerun `dna gateway install --force` from the same `--profile` / `DNA_STATE_DIR` you want the service to use.

**If `dna gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `dna doctor` to update it (or `dna gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `DNA_GATEWAY_TOKEN`) and restart the service.

**If `dna gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/dna` on disk.
...
Symptoms: `pwd` or file tools show `~/.dna/sandboxes/...` even though you
...
DNA intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `dna models list` or
  `dna models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${DNA_CONFIG_PATH:-$HOME/.dna/dna.json}"
```
...
tail -f "$(ls -t /tmp/dna/dna-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
dna logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.dna/agents/<agentId>/sessions/
```
...
dna logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
dna channels logout
trash "${DNA_STATE_DIR:-$HOME/.dna}/credentials" # if logout can't cleanly remove everything
dna channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/dna/dna-*.log | head -1)" | tail -20
```

### High Memory Usage

DNA keeps conversation history in memory.
...
DNA now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `dna doctor` reports every invalid entry.
- `dna doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `dna logs`, `dna health`, `dna status`, `dna gateway status`, and `dna gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/dna/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `dna models status` (CLI).
...
5) Run `dna channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. DNA scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/dna/dna/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
dna gateway status
dna gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.dna.* still works)
```
...
Ensure the global `dna` CLI is installed and matches the app version:
```bash
dna --version
npm install -g dna@<version>
```
...
#   ${DNA_CONFIG_PATH:-$HOME/.dna/dna.json} -> { logging: { level: "trace" } }
#
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/dna/dna-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$DNA_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.dna/logs/...`; profiles use `~/.dna-<profile>/logs/...`)<br />Linux: `journalctl --user -u dna-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "DNA Gateway (<profile>)" /V /FO LIST` |
| Session files | `$DNA_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$DNA_STATE_DIR/media/` |
| Credentials | `$DNA_STATE_DIR/credentials/` |
...
# dna gateway uninstall

trash "${DNA_STATE_DIR:-$HOME/.dna}"
dna channels login         # re-pair WhatsApp
dna gateway restart           # or: dna gateway
```
...
1. Check logs first: `/tmp/dna/` (default: `dna-YYYY-MM-DD.log`, or your configured `logging.file`)
