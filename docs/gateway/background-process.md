...
- `attach`: read the aggregated output and show input recovery state
...
- `process list`, `attach`, `poll`, and `log` report `waitingForInput` only
  when the session still has writable stdin and has been idle since the last
  recorded output longer than the input-wait threshold.
