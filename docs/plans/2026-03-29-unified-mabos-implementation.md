...
export class BudgetLedger {
      ...
      if (daily) {
        ...
        if (params.estimatedCostUsd > available) {
          throw new BudgetExhaustedError(
            ...
            "daily",
            daily.limit_usd,
            daily.spent_usd + daily.reserved_usd,
      ...
      if (monthly) {
        ...
        if (params.estimatedCostUsd > available) {
          ...
          throw new BudgetExhaustedError(
            ...
            "monthly",
            monthly.limit_usd,
            monthly.spent_usd + monthly.reserved_usd,
            params.estimatedCostUsd,
          );
        }
        this.db
  ...
  settleReservation(reservationId: string, actualCostUsd: number): void {
    const settle = this.db.transaction(() => {
      const event = this.db
        .prepare(`SELECT * FROM cost_events WHERE id = ? AND event_type = 'reservation'`)
      ...
      `,
        ...
        .run(
          generatePrefixedId("cost"),
          event.company_id,
          event.agent_id,
          event.session_id,
          actualCostUsd,
          JSON.stringify({ reservationId, estimated, actual: actualCostUsd }),
        );
    });
  ...
  return [
    {
      ...
      label: "Budget Status",
      description:
        "Check remaining budget for an agent. Shows daily and monthly limits, spend, and remaining balance.",
      ...
      async execute(_id: string, params: Static<typeof BudgetStatusParams>) {
        ...
        if (status.daily) {
          lines.push(
            `  Daily:   $${status.daily.spent.toFixed(2)} / $${status.daily.limit.toFixed(2)} (remaining: $${status.daily.remaining.toFixed(2)})`,
          );
        }
        if (status.monthly) {
          lines.push(
            `  Monthly: $${status.monthly.spent.toFixed(2)} / $${status.monthly.limit.toFixed(2)} (remaining: $${status.monthly.remaining.toFixed(2)})`,
          );
        }
    ...
    },
    {
      ...
      description:
        "Search the governance audit log for past actions, budget events, and security incidents.",
...
export class ModelRegistry {
  ...
  estimateCost(modelId: string, inputTokens: number, outputTokens: number): number {
    ...
    if (!spec) return 0;
    return (
      (inputTokens / 1000) * spec.inputPricePer1kTokens +
...
export class ModelResolver {
  ...
  resolve(requested: string): ResolvedModel {
    ...
    for (const fallback of this.config.fallbackChain ?? []) {
      const [, fModelId] = fallback.includes("/") ? fallback.split("/", 2) : [undefined, fallback];
