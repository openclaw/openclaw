...
Top-level fields (`severity`, `canonical_category`, `summary`, `root_cause`, `hypotheses`, `rca_confidence`) remain identical. The `rca_confidence` field (0-100) is derived from the top hypothesis confidence in chain mode and from the legacy heuristic confidence in single-shot mode. **`mode` field enum contract**: The `mode` field is an open string enum — existing consumers MUST treat unknown values as opaque (log and continue, not fail). Current values: `"single"`, `"dual"`, `"heuristic"` (legacy). New values added by the chain: `"chain_v2"` (full chain), `"chain_v2_partial"` (short-circuited chain). The existing `_llm_attach_mode()` function already sets `mode` as a free-form string; no consumer in the codebase does strict enum matching (verified: cron extraction ignores `mode`, Linear ticket creation ignores `mode`, Slack formatting ignores `mode`). **Important**: the cron job extracts `primary_category` from the `=== ranked_hypotheses ===` text section in sentinel-triage.sh stdout, NOT from Step 11 JSON directly. This text section is generated from the heuristic `HYPOTHESES` array (populated before Step 11, at sentinel-triage.sh line ~1804) — it does NOT depend on Step 11 JSON output. Therefore, backward-compat is maintained trivially: the chain changes Step 11 internals but does not affect the `ranked_hypotheses` text section or its cron extraction. The `primary_category` in the final JSON output uses `step11_dedup_category` (line ~2531), which is also set before Step 11. Linear ticket creation (lib-linear-ticket.sh) reads from Step 11 JSON and uses fallback jq paths (`.summary // .brief_description`, `.root_cause // .top_hypothesis.description`) that are satisfied by the chain output's top-level fields. New `chain_metadata` is additive — ignored by existing consumers.
...
| `RCA_CHAIN_TOTAL_TIMEOUT_MS`     | `60000`                             | Total chain budget (ms) — used **only when `RCA_CHAIN_ENABLED=1`**. If exceeded, return best partial result. Default 60s leaves headroom within the 180s cron deadline. May be lowered (e.g., 30s for faster cycles with shallower results) but must not exceed `cron_deadline - max_evidence_time - safety_margin` (~100s). **Not used in legacy mode**: when `RCA_CHAIN_ENABLED=0`, the initial budget check uses `RCA_LLM_TIMEOUT_MS` (existing variable, default 15s) as the only timeout — `RCA_CHAIN_TOTAL_TIMEOUT_MS` is ignored entirely.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
**Timeout budget and cron deadline**: The cron `activeDeadlineSeconds` must be increased from 120s to **180s** (Helm values change). Budget breakdown:

```
Evidence steps 0-10:  30-60s (existing, unchanged)
Chain (A→B→C→D→E):    ≤60s (RCA_CHAIN_TOTAL_TIMEOUT_MS=60000)
Learning extraction:  ≤10s (only on resolution, most cycles skip)
Safety margin:        50-80s (= 180 - evidence - chain - learning)
─────────────────────────────────
Total:                ≤130s (within 180s deadline)
```
...
This defense-in-depth matches the quarantine approach used for overlay suggestions (see Layer 3) and applies to all output sinks. The existing `_output_redact()` call in sentinel-triage.sh is extended to run both stages for chain output fields and all output sinks (Slack posting, Linear ticket creation via lib-linear-ticket.sh, and any future sinks).
...
When the bot discovers a failure mode not in the service overlay, it writes a suggestion to `pending-overlay-suggestions.jsonl` (not auto-modify). Same flock + atomic replace write pattern. **Idempotency**: Each suggestion has a deterministic `suggestion_key` (format: `{namespace}:{service}:{proposed_entry.id}`) used for upsert — if a suggestion with the same key already exists in the file, the existing entry is updated (timestamp refreshed, confidence updated to latest) rather than appending a duplicate. This prevents repeated heartbeat runs from spamming identical suggestions and evicting distinct entries from the 50-entry cap. The upsert uses a read-modify-write cycle within the flock, which is safe because writes are already serialized. **Redaction**: The entire JSON line is passed through `_rca_prompt_scrub()` before persistence (same as incident memory writes) — required because suggestions are surfaced in the weekly Slack digest and could contain secret fragments from evidence.

```json
{
  "suggestion_key": "production:api-gateway:oom-upload-buffering",
...
| 3   | CRITICAL | Latency blows cron deadline              | Per-stage timeout (10s) + total hard budget (60s) + severity-adaptive depth + partial result fallback + 180s cron deadline                                            |
...
| 7   | CRITICAL | Cron category extraction breaks          | Output JSON preserves same top-level fields; `chain_metadata` is additive only                                                                                        |
...
| 11  | HIGH     | Model routing no config surface          | Added `RCA_STAGE_MODEL_FAST`, `RCA_STAGE_MODEL_STRONG`, `RCA_STAGE_TIMEOUT_MS`, `RCA_CHAIN_TOTAL_TIMEOUT_MS`                                                          |
