...
2. **Continuity match (drift-tolerant):** `namespace` matches AND `primary_category` differs. The match requires the time window PLUS at least one corroborating dimension (workload or signal key overlap). **Truth table for empty/non-empty permutations:**

   | Workloads      | Signal Keys    | Required conditions                                             |
   | -------------- | -------------- | --------------------------------------------------------------- |
   | Both available | Both available | Time ≤60m AND workload Jaccard ≥50% AND signal key Jaccard ≥30% |
   | Either empty   | Both available | Time ≤60m AND signal key Jaccard ≥50% (raised from 30%)         |
   | Both available | Either empty   | Time ≤60m AND workload Jaccard ≥70% (raised from 50%)           |
   | Either empty   | Either empty   | **No continuity match possible** — fall through to new incident |
