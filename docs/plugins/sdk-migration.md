...
read_when:
  - You see the MULLUSI_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the MULLUSI_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Mullusi plugin
---

# Plugin SDK Migration

Mullusi has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`mullusi/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`mullusi/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`mullusi/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `mullusi/plugin-sdk/slack`, `mullusi/plugin-sdk/discord`,
`mullusi/plugin-sdk/signal`, `mullusi/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`mullusi/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `mullusi/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "mullusi/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "mullusi/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "mullusi/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `MullusiSchema` |
...
```bash
MULLUSI_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 mullusi gateway run
MULLUSI_SUPPRESS_EXTENSION_API_WARNING=1 mullusi gateway run
```
