...
| 25  | `buildAuthDoctorHint`              | Repair hint appended when OAuth refresh fails                                                                  | Provider needs provider-owned OAuth auth repair guidance after refresh failure                                                                |
| 26  | `buildStaticAuthProfileDoctorHint` | Repair hint for invalid static API-key auth profiles                                                           | Provider can identify a provider-owned static API-key profile problem that doctor should surface                                              |
