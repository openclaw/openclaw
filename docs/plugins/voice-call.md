...
read_when:
  - You want to place an outbound voice call from EVOX.sh
...
Voice calls for EVOX.sh via a plugin. Supports outbound notifications and
...
- Use `evox voicecall ...` or the `voice_call` tool
...
```bash
evox plugins install ./extensions/voice-call
...
```bash
evox voicecall call --to "+15555550123" --message "Hello from EVOX.sh"
evox voicecall start --to "+15555550123"   # alias for call
evox voicecall continue --call-id <id> --message "Any questions?"
evox voicecall speak --call-id <id> --message "One moment"
evox voicecall end --call-id <id>
evox voicecall status --call-id <id>
evox voicecall tail
evox voicecall latency                     # summarize turn latency from logs
