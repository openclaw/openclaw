...
read_when:
  - You want to place an outbound voice call from Mullusi
...
Voice calls for Mullusi via a plugin. Supports outbound notifications and
...
- Use `mullusi voicecall ...` or the `voice_call` tool
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `mullusi doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `mullusi doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `mullusi doctor --fix`:
...
mullusi voicecall start --to "+15555550123"   # alias for call
...
mullusi voicecall latency                     # summarize turn latency from logs
