...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    `api.runtime.agent.session.listSessionEntries({ agentId, storePath, readOnly: true, involvingProfileId })` optionally selects entries involving an exact source-local profile ID or merge alias. The host applies the same named-person Activity semantics (effective human owner, profile participant, or recorded mention) before returning entries. Personal hide/show state does not change this selection. Missing profiles, names, logins, and shortened display references match nothing. Identity resolution respects the reader's optional `env` scope. Omit `involvingProfileId` for an unfiltered read; an empty value matches nothing. Call again after awaited work to reauthorize publication against current identity and involvement. This selection does not grant viewer access or replace the publishing plugin's explicit filters and safety exclusions.
