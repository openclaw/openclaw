...
- Use `approvalCapability.getActionAvailabilityState` for same-chat approval auth availability. Core also uses this hook as the plugin approval initiating-surface state, so native-delivery channels should return `disabled` for `{ approvalKind: "plugin" }` when the channel cannot deliver plugin approval UI, even if approvers are configured.
...
- Use `approvalCapability.describePluginApprovalSetup` when plugin approval failure guidance differs from exec approval guidance. Core appends this text to plugin approval no-route and timeout failures only after the originating plugin approval surface resolves as `disabled`; `createApproverRestrictedNativeApprovalCapability(...)` defaults it to `describeExecApprovalSetup` for channels where plugin and exec approvals use the same native setup.
