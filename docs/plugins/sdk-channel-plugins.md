...
- Use `approvalCapability.describePluginApprovalSetup` when plugin approval failure guidance differs from exec approval guidance. Core can append this text to plugin approval no-route and timeout failures when the originating approval surface cannot use native delivery; `createApproverRestrictedNativeApprovalCapability(...)` defaults it to `describeExecApprovalSetup` for channels where plugin and exec approvals use the same native setup.
