...
- Use `outbound.validateDeliveryResults` only for channel-owned, fail-closed proof checks that require provider-confirmed send results after the native API call, such as verifying a platform-returned thread id. The hook can fail a delivery after one or more messages were already sent, so keep advisory side effects in `afterDeliverPayload`.
