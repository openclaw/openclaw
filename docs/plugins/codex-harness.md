...
- Codex app-server `0.144.0` or newer. The plugin manages a compatible
...
**The app-server is rejected:** use Codex app-server `0.144.0` or newer.
Same-version prereleases or build-suffixed versions such as
`0.144.0-alpha.2` or `0.144.0+custom` are rejected because OpenClaw tests
the stable `0.144.0` protocol floor.
