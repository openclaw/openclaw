...
completions arm the assistant-output release: if Codex then goes quiet for
`appServer.turnAssistantCompletionIdleTimeoutMs` without `turn/completed`,
OpenClaw best-effort interrupts the native turn and releases the session lane.
The assistant-completion idle timeout defaults to 10000 ms and can be raised for
trusted workloads where Codex may need longer to publish `turn/completed` after
the last assistant item. Replay-safe stdio app-server failures, including
