...
| `nativeHookRelay`                             | `{ enabled: true }`                                    | Native hook relay controls. `enabled: false` removes the relay entirely (`features.hooks=false`, cleared hook arrays) when the **effective** approval policy for the thread is `never` — the runtime-resolved value, which includes a stock `mode: "yolo"` deployment and any forced override (unknown-model or exec-mode reviewer forcing, forced guardian reviewer, forced danger-full-access sandbox). When that policy prompts, the opt-out instead narrows to the required before-tool policy relay (`events: ["pre_tool_use"]`) and logs a warning naming the effective policy; this applies even to a configured `approvalPolicy: "never"` whose runtime forced prompting. `events` is different: an explicit list is honored **verbatim under every policy** — nothing added, removed, or reordered, and no floor appended — because that is the pre-existing relay contract (see #116117, which deliberately keeps an explicit `permission_request` relay alive under a prompting policy so approval promotion cannot block unattended runs). An empty `events` list means the default set. Approvals themselves ride the app-server approval bridge and cannot be disabled by this key; the policy-aware default still excludes `permission_request` while approvals are active, so scope it only when you intend to own that escalation. |
