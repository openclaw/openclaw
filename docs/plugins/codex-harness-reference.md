...
from the configured agent workspace uses the normal bounded turn-context path
for native Codex turns, even when memory tools are available for that workspace.
The injected root memory is intended for sparse bootstrap facts; Codex should
still use `memory_search` or `memory_get` when durable memory is relevant beyond
that bounded bootstrap context.
