...
| `loopDetectionPreToolUseRelay`                | `true`                                                 | Install the Codex `PreToolUse` subprocess used for OpenClaw loop detection and its explicit no-policy marker. Set `false` to suppress only that PreToolUse relay. When loop detection is enabled, its `PostToolUse` outcome relay remains installed for no-progress detection; plugin hooks, trusted-tool policy, and result middleware also install their required relays.                     |
