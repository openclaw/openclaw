...
| `loopDetectionPreToolUseRelay`                | `true`                                                 | Install the Codex `PreToolUse` subprocess used for OpenClaw loop detection and its explicit no-policy marker. When effective loop detection is enabled, also install the companion `PostToolUse` outcome relay required for no-progress detection. Set `false` to suppress loop-detection-only pre/post fan-out. Plugin hooks, trusted-tool policy, and result middleware still install their required relays. |
