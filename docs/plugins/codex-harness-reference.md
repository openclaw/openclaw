...
| `nativeHookRelay`                             | `{ enabled: true }`                                    | Native hook relay controls. `enabled: false` disables the relay entirely for attempts and side questions (`features.hooks=false`, cleared hook arrays). `events` narrows relayed events to an explicit list (e.g. `["post_tool_use"]`). Guardrail: while an approval policy is active (`approvalPolicy` other than `never`, default `on-request`), `permission_request` is retained even if omitted from `events`, so scoping cannot silently drop approval enforcement — fully removing it requires `enabled: false` or `approvalPolicy: never`. An empty `events` list means the default set. |
