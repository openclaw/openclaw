...
| `nativeHookRelay`                             | `{ enabled: true }`                                    | Native hook relay controls. `enabled: false` disables the relay entirely for attempts and side questions (`features.hooks=false`, cleared hook arrays). `events` narrows relayed events to an explicit list (e.g. `["post_tool_use"]`), overriding the approval-policy-aware default — including force-adding `permission_request` as described in the runtime guide. An empty `events` list means the default set. |
