...
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, post-tool raw assistant progress, or a post-tool raw reasoning `rawResponseItem/completed` with no active items, while OpenClaw waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis or extended reasoning can legitimately stay quiet longer than the final assistant release budget. |
