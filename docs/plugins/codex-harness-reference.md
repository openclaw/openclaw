...
| `loopDetectionPreToolUseRelay`                | `true`                                                 | Install the Codex `PreToolUse` subprocess used for OpenClaw loop detection and its explicit no-policy marker. Set `false` to suppress the paired `PreToolUse` / `PostToolUse` loop-detection relays. Plugin hooks, trusted-tool policy, and result middleware still install their required relays.                                                                                              |
