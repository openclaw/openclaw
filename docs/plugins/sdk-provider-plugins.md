...
<Steps>
  ...
  <Step title="Register the provider">
    ...
    | Network limits | Fetches use OpenClaw's SSRF guard. Canonical HTTPS provider catalogs honor eligible HTTP(S) proxy environment variables, including `NO_PROXY` bypasses; configured custom and cleartext catalog origins stay on the strict direct path. Direct requests retain pinned DNS. One 5-second timeout budget applies across pagination, with a 4 MiB response limit per page and a 50-page limit. Cross-origin pagination links are rejected; credentials are removed after a cross-origin redirect. |
