...
final-output constraint. The host supplies a detached JSON-safe snapshot only
when it fits within 1 KiB; otherwise it is not forwarded natively. Harnesses
may forward the snapshot when their transport supports structured output. A
