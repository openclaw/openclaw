...
| `mcpServers`                         | No       | `Record<string, object>`     | MCP servers attached to agent sessions while this plugin is enabled. Entries use the same shape as `mcp.servers.<name>` config (`transport`, `url`, `command`, …). Relative stdio commands resolve against the plugin root. User `mcp.servers` entries with the same name win, and `mcp.servers.<name>.enabled: false` disables a plugin-declared server. |
