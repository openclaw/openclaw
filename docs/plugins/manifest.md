...
read_when:
  - You are building an Mullusi plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (mullusi.plugin.json)

This page is for the **native Mullusi plugin manifest** only.
...
Mullusi auto-detects those bundle layouts too, but they are not validated
against the `mullusi.plugin.json` schema described here.

For compatible bundles, Mullusi currently reads bundle metadata plus declared
...
Mullusi runtime expectations.

Every native Mullusi plugin **must** ship a `mullusi.plugin.json` file in the
**plugin root**. Mullusi uses this manifest to validate configuration
...
`mullusi.plugin.json` is the metadata Mullusi reads before it loads your
plugin code.
...
| `providerAuthEnvVars`               | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Mullusi can inspect without loading plugin code.                                                                                                    |
...
Mullusi reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Mullusi falls back to `choiceId`.                                        |
...
Use `contracts` only for static capability ownership metadata that Mullusi can
...
Use `modelSupport` when Mullusi should infer your provider plugin from
...
Mullusi applies this precedence:
...
Legacy top-level capability keys are deprecated. Use `mullusi doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `mullusi.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `mullusi` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Mullusi must know it before loading plugin code, put it in `mullusi.plugin.json`
...
`mullusi` block instead of `mullusi.plugin.json`.

Important examples:

| Field                                                             | What it means                                                                          |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `mullusi.extensions`                                             | Declares native plugin entrypoints.                                                    |
| `mullusi.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding and deferred channel startup. |
| `mullusi.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.   |
| `mullusi.install.npmSpec` / `mullusi.install.localPath`         | Install/update hints for bundled and externally published plugins.                     |
| `mullusi.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                    |
| `mullusi.install.minHostVersion`                                 | Minimum supported Mullusi host version, using a semver floor like `>=2026.3.22`.      |
| `mullusi.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.         |
| `mullusi.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.   |

`mullusi.install.minHostVersion` is enforced during install and manifest
registry loading. Invalid values are rejected; newer-but-valid values skip the
plugin on older hosts.

`mullusi.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
- The manifest is **required for native Mullusi plugins**, including local filesystem loads.
