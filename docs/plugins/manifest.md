...
- Exclusive plugin kinds are selected through `plugins.slots.*`: `kind: "memory"` via `plugins.slots["memory.recall"]`, `kind: "context-engine"` via `plugins.slots.contextEngine` (default `legacy`).
