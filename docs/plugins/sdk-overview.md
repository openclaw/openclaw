...
| `api.registerCommand(...)`                                                           | Scoped plugin commands; command results can set `continueAgent: true` or `suppressReply: true` (Telegram-only, skips the fallback when the command delivers its own reply); Discord native commands support `descriptionLocalizations` |
