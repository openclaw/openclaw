...
Providers opting into prepared workspaces through `resolvePreparationTarget` must also supply `runScriptWithBudget(createScript, signal)` to `project.prepare`. Select the command's remaining timeout immediately before invocation, call the pure synchronous `createScript(timeoutMs)` factory, and run its output with that same timeout. The budget must be a positive safe integer within the platform timer limit. Core requires this capability before starting preparation; the existing `runScript(script, signal)` and `upload` contract remains available for seed-only providers. Repository setup consumes the command budget, and cancellation must still settle owned commands before returning.
