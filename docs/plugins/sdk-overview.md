...
read_when:
  ...
  - You want a reference for all registration methods on MullusiPluginApi
...
prefer `mullusi/plugin-sdk/channel-core`; keep `mullusi/plugin-sdk/core` for
...
`mullusi/plugin-sdk/slack`, `mullusi/plugin-sdk/discord`,
`mullusi/plugin-sdk/signal`, `mullusi/plugin-sdk/whatsapp`, or
...
| `plugin-sdk/config-schema`  | `MullusiSchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `mullusi.json` Zod schema export (`MullusiSchema`) |
...
The `register(api)` callback receives an `MullusiPluginApi` object with these
methods:
...
- User config still wins. Mullusi merges `agents.defaults.cliBackends.<id>` over the
...
| `api.config`             | `MullusiConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `mullusi/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Mullusi is already running. If no runtime
...
- `@mullusi/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `mullusi/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `mullusi/plugin-sdk/speech`, `.../provider-model-shared`, or another
