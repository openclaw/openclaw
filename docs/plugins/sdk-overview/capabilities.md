...
Core supplies invocation-bound `assertCurrent()` to both `prepareProvision`
and `provision`, including the returned allocation function. Call it before
external effects and after awaited work before further effects or durable writes.
It rejects when the operation owner changes, destruction is requested, the service
stops, or the invocation closes (including timeout). The same operation's node
enrollment does not retire its provisioning guard. Retaining the actual allocation
function does not extend its authority. Cleanup and renewal use their independent
lifecycle owner, never a closed provisioning guard. Version 1 also requires the
existing SSH identity-resolution guard. An omitted version keeps legacy lifecycle
and placement support; unknown declarations are rejected. See
...
For provider-owned SSH identity resolution, core supplies the optional `request.assertCurrent` callback. It checks the current lease and local identity invocation; initializing SSH callers also bind it to their preparation lifetime. Core rejects late identity results and closes retained callbacks after resolution or a caller-visible timeout. Version 1 resolvers must call it before effects and after awaits. Legacy resolvers remain supported without a capability declaration, but core cannot prevent internal effects in a resolver that ignores the callback. This does not change provisioning, renewal, or cleanup authority.
