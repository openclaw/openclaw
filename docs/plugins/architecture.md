...
read_when:
  - Building or debugging native EVOX.sh plugins
...
This page covers the internal architecture of the EVOX.sh plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside EVOX.sh. Every
native EVOX.sh plugin registers against one or more capability types:
...
EVOX.sh classifies every loaded plugin into a shape based on its actual
...
Use `evox plugins inspect <id>` to see a plugin's shape and capability
...
When you run `evox doctor` or `evox plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `evox status --all` and `evox plugins doctor`.

## Architecture overview

EVOX.sh's plugin system has four layers:

1. **Manifest + discovery**
   EVOX.sh finds candidate plugins from configured paths, workspace roots,
...
3. **Runtime loading**
   Native EVOX.sh plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of EVOX.sh reads the registry to expose tools, channels, provider
...
That split lets EVOX.sh validate config, explain missing/disabled plugins, and
...
normal chat actions. EVOX.sh keeps one shared `message` tool in core, and
...
EVOX.sh treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's EVOX.sh-facing
  surfaces
...
So if EVOX.sh adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If EVOX.sh has shared
...
EVOX.sh already treats image/audio/video understanding as one shared
...
If EVOX.sh adds a new domain later, such as video generation, use the same
...
2. **contract tests**
   ...
   EVOX.sh can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that EVOX.sh knows, up front, which plugin owns which
...
Native EVOX.sh plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because EVOX.sh currently treats them
...
EVOX.sh exports capabilities, not implementation convenience.
...
At startup, EVOX.sh does roughly this:
...
The manifest is the control-plane source of truth. EVOX.sh uses it to:
...
EVOX.sh keeps short in-process caches for:
...
EVOX.sh still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, EVOX.sh calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | EVOX.sh tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on EVOX.sh's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before EVOX.sh's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as EVOX.sh adds those
...
- If EVOX.sh adds a new capability such as video generation later, define the
...
- EVOX.sh only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "EVOX.sh plugin runtime helpers",
    count: 5,
  },
});
```
...
- Bundled extension internals remain private. External plugins should use only
  `openclaw/plugin-sdk/*` subpaths. EVOX.sh core/test code may use the repo
...
`catalog.run(...)` returns the same shape EVOX.sh writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to EVOX.sh's
...
- if both `catalog` and `discovery` are registered, EVOX.sh uses `catalog`
...
- Read-only command paths such as `evox status`, `evox status --all`,
  `evox channels status`, `evox channels resolve`, and doctor/config
...
Security note: `evox plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `openclaw.setupEntry` can point at a lightweight setup-only module.
When EVOX.sh needs setup surfaces for a disabled channel plugin, or
...
this flag. Keep the plugin on the default behavior and let EVOX.sh load the
...
EVOX.sh can also merge **external channel catalogs** (for example, an MPM
...
contain `{ "entries": [ { "name": "@scope/pkg", "evox": { "channel": {...}, "install": {...} } } ] }`.
...
This is how EVOX.sh stays opinionated without becoming hardcoded to one
