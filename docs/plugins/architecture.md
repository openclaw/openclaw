...
Gateway shutdown also joins actual harness, MCP, LSP, embedding, and media cleanup after their initial grace periods. Plugin host cleanup hooks can advance to later hooks after a timeout, but registry resets and shared database closure wait for their actual completion. These waits preserve resources for cleanup; they do not restore a retired plugin's runtime authority.
