...
<AccordionGroup>
  ...
  <Accordion title="Auth and security subpaths">
    ...
    | `plugin-sdk/secret-input` | Secret input parsing helpers (`coerceSecretRef`, `isSecretRef`, `resolveSecretInputString`, `buildSecretInputSchema`, etc.) plus built-in source type guards (`isEnvSecretProviderConfig`, `isFileSecretProviderConfig`, `isExecSecretProviderConfig`) for narrowing the open `SecretProviderConfig` union |
    | `plugin-sdk/secret-input-runtime` | Runtime variant of `secret-input` with the same exports; for use on lazy import paths |
