...
are private-local.
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-catalog-hybrid-runtime` | Private-local after July 2026; Hybrid catalog helpers for gateway ID lists plus remote full-metadata slices (for example models.dev): `buildHybridProviderConfig`, `buildHybridModelDefinitions`, `fetchModelsDevProviderSlice`, `ScopedHybridModelCache` plus `createScopedHybridDynamicModelHooks` for profile-scoped dynamic resolution, process-sticky metadata TTL vs short gateway/hybrid success TTL, and static fallback without sticky-caching empty discovery |
