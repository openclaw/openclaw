...
<AccordionGroup>
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/agent-config-helpers` | Agent config resolution helpers: `resolveConfigReasoningDefault` resolves per-agent then global `reasoningDefault` with `"off"` fallback |
