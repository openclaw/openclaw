...
在`runEmbeddedAttempt()`（由`runEmbeddedPiAgent()`调用）内部，使用pi SDK：
...
`subscribeEmbeddedPiSession()`订阅pi的`AgentSession`事件：
...
SDK处理完整的代理循环：发送到LLM、执行工具调用、流式响应。
...
pi-agent-core的`AgentTool`与pi-coding-agent的`ToolDefinition`具有不同的`execute`签名。`pi-tool-definition-adapter.ts`中的适配器桥接了这一点：
...
会话是带有树结构（id/parentId链接）的JSONL文件。Pi的`SessionManager`处理持久性：

```typescript
const sessionManager = SessionManager.open(params.sessionFile);
```

OpenClaw用`guardSessionManager()`包装此功能以确保工具结果安全。

### 会话缓存

`session-manager-cache.ts`缓存SessionManager实例以避免重复文件解析：
...
`limitHistoryTurns()`根据通道类型（DM vs 群组）修剪对话历史。
...
OpenClaw维护一个认证配置文件存储，每个提供商有多个API密钥：
...
`src/agents/pi-hooks/compaction-safeguard.ts`为压缩添加保护，包括自适应令牌预算以及工具失败和文件操作摘要：
...
`src/agents/pi-hooks/context-pruning.ts`实现基于缓存-TTL的上下文修剪：
...
`EmbeddedBlockChunker`管理将流式文本转换为离散回复块：
...
解析并提取`[[media:url]]`、`[[voice]]`、`[[reply:id]]`等回复指令：
...
OpenClaw还具有直接使用pi-tui组件的本地TUI模式：
...
3. **扩展加载**：可以更直接地使用pi的`ResourceLoader`
4. **流式处理程序复杂性**：`subscribeEmbeddedPiSession`已经变得很大
5. **提供商特性**：许多提供商特定的代码路径，pi可能可以处理
