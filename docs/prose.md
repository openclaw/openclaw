---
summary: "OpenProse: .prose workflows, slash commands, and state in EasyHub"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In EasyHub it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
Dev/local checkout: `EasyHub plugins install ./extensions/open-prose`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses EasyHub tools under the hood.
...
OpenProse programs map to EasyHub primitives:

| OpenProse concept         | EasyHub tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use EasyHub tool allowlists and approval gates to control side effects.
