...
<Steps>
  ...
  <Step title="OpenClaw’ı önek altında kur">
    `--prefix <prefix>` kullanarak npm ile kurar, ardından sarmalayıcıyı `<prefix>/bin/openclaw` konumuna yazar.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Node.js 22+ sağla">
    Eksikse sırasıyla winget, ardından Chocolatey, sonra Scoop ile kurmayı dener.
  </Step>
  <Step title="OpenClaw’ı kur">
...
<AccordionGroup>
  ...
  <Accordion title="Linux’ta npm neden EACCES alıyor?">
