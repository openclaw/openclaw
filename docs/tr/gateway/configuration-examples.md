...
Aşağıdaki örnekler mevcut yapılandırma şemasıyla uyumludur. Kapsamlı başvuru ve alan bazlı notlar için [Yapılandırma](/gateway/configuration) bölümüne bakın.
...
`~/.openclaw/openclaw.json` dosyasına kaydedin ve bu numaradan bota DM gönderebilirsiniz.
...
Birden fazla kişi botunuza DM gönderebiliyorsa (`allowFrom` içinde birden fazla giriş, birden fazla kişi için eşleştirme onayları veya `dmPolicy: "open"`), farklı gönderenlerden gelen DM'lerin varsayılan olarak tek bir bağlamı paylaşmaması için **güvenli DM modu**nu etkinleştirin:
...
### API anahtarı yedeklemeli OAuth
...
### Anthropic aboneliği + API anahtarı, MiniMax yedeği
...
- `dmPolicy: "open"` ayarlarsanız, eşleşen `allowFrom` listesi `"*"` öğesini içermelidir.
