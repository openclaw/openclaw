...
<AccordionGroup>
  ...
  <Accordion title="Gateway başlatılamıyor veya servis yüklü ancak çalışmıyor">
    ```bash
    openclaw status
    openclaw gateway status
    openclaw logs --follow
  ...
  <Accordion title="Cron veya heartbeat tetiklenmedi ya da iletilmedi">
    ```bash
    openclaw status
    openclaw gateway status
  ...
  <Accordion title="Düğüm eşleştirilmiş ancak araç camera canvas screen exec başarısız">
    ```bash
    openclaw status
    openclaw gateway status
  ...
  <Accordion title="Tarayıcı aracı başarısız">
    ```bash
    openclaw status
    openclaw gateway status
