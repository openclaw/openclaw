...
| Ağ hatalarıyla gönderim başarısız             | Telegram API çağrı hataları için günlükleri inceleyin    | DNS/IPv6/proxy yönlendirmesini `api.telegram.org` için düzeltin.                |
...
| Grup mesajları yok sayılıyor       | Günlüklerde bahsetme kapılaması düşüşlerini kontrol edin | Botu etiketleyin veya guild/kanal için `requireMention: false` ayarlayın. |
