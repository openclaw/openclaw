...
ریپو روٹ سے `pnpm` (Node 22+) استعمال کریں۔ ٹیگ لگانے/شائع کرنے سے پہلے ورکنگ ٹری کو صاف رکھیں۔
...
- **npm pack/publish اٹک جاتا ہے یا بہت بڑا ٹاربال بناتا ہے**: macOS ایپ بنڈل `dist/OpenClaw.app` میں (اور ریلیز zip فائلیں) پیکیج میں شامل ہو جاتی ہیں۔ حل: `package.json` `files` کے ذریعے پبلش مواد کی اجازت فہرست بنائیں (dist سب ڈائریکٹریز، docs، skills شامل کریں؛ ایپ بنڈلز خارج کریں)۔ `npm pack --dry-run` سے تصدیق کریں کہ `dist/OpenClaw.app` فہرست میں نہیں۔
...
ہم صرف **موجودہ npm پلگ اِنز** کو `@openclaw/*` اسکوپ کے تحت شائع کرتے ہیں۔ بنڈل کیے گئے
پلگ اِنز جو npm پر نہیں ہیں وہ **صرف ڈسک ٹری** ہی رہتے ہیں (تاہم
`extensions/**` میں شامل رہتے ہیں)۔
