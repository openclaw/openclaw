...
یہ **براؤزر آٹومیشن نہیں** ہیں۔ JS-بھاری سائٹس یا لاگ اِن کے لیے
[Browser tool](/tools/browser) استعمال کریں۔
...
- `web_fetch` سادہ HTTP GET کرتا ہے اور قابلِ مطالعہ مواد نکالتا ہے
  (HTML → markdown/text)۔ یہ JavaScript **نہیں** چلاتا۔
...
**ماحولیاتی متبادل:** Gateway پروسیس کے ماحول میں `BRAVE_API_KEY` سیٹ کریں۔
گیٹ وے انسٹال کے لیے، اسے `~/.openclaw/.env` (یا اپنے سروس ماحول) میں رکھیں۔
دیکھیں [Env vars](/help/faq#how-does-openclaw-load-environment-variables)۔

## Perplexity کا استعمال (براہِ راست یا OpenRouter کے ذریعے)

Perplexity Sonar ماڈلز میں بلٹ اِن ویب سرچ صلاحیتیں ہوتی ہیں اور یہ حوالہ جات کے ساتھ
AI-مرکب جوابات واپس کرتے ہیں۔ آپ انہیں OpenRouter کے ذریعے استعمال کر سکتے ہیں
(کریڈٹ کارڈ درکار نہیں — کرپٹو/پری پیڈ سپورٹ)۔
...
**ماحولیاتی متبادل:** Gateway کے ماحول میں `OPENROUTER_API_KEY` یا `PERPLEXITY_API_KEY` سیٹ کریں۔
گیٹ وے انسٹال کے لیے، اسے `~/.openclaw/.env` میں رکھیں۔
...
- `country` (اختیاری): علاقائی نتائج کے لیے 2 حرفی ملکی کوڈ (مثلاً "DE"، "US"، "ALL")۔ اگر چھوڑ دیا جائے تو Brave اپنی بطورِ طے شدہ علاقہ منتخب کرتا ہے۔
...
- `web_fetch` پہلے Readability (مرکزی مواد کا اخراج) استعمال کرتا ہے، پھر Firecrawl (اگر کنفیگر ہو)۔ اگر دونوں ناکام ہوں تو اوزار غلطی واپس کرتا ہے۔
