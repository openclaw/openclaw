...
حالت: تجرباتی۔ یہ انضمام `zca-cli` کے ذریعے **ذاتی Zalo اکاؤنٹ** کو خودکار بناتا ہے۔

> **انتباہ:** یہ ایک غیر سرکاری انضمام ہے اور اس کے نتیجے میں اکاؤنٹ معطلی/بین ہو سکتی ہے۔ اپنے خطرے پر استعمال کریں۔
...
چینل آئی ڈی `zalouser` ہے تاکہ واضح رہے کہ یہ **ذاتی Zalo صارف اکاؤنٹ** (غیر سرکاری) کو خودکار بناتا ہے۔ ہم `zalo` کو مستقبل میں ممکنہ سرکاری Zalo API انضمام کے لیے محفوظ رکھتے ہیں۔
...
`channels.zalouser.dmPolicy` معاونت کرتا ہے: `pairing | allowlist | open | disabled` (بطورِ طے شدہ: `pairing`)۔
`channels.zalouser.allowFrom` صارف آئی ڈیز یا نام قبول کرتا ہے۔ وِزارڈ دستیاب ہونے پر ناموں کو `zca friend find` کے ذریعے آئی ڈیز میں حل کرتا ہے۔
...
- بطورِ طے شدہ: `channels.zalouser.groupPolicy = "open"` (گروپس کی اجازت)۔ اگر غیر متعین ہو تو بطورِ طے شدہ کو اوور رائیڈ کرنے کے لیے `channels.defaults.groupPolicy` استعمال کریں۔
...
اکاؤنٹس zca پروفائلز سے میپ ہوتے ہیں۔ مثال:
