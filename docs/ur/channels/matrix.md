...
Matrix ایک کھلا اور غیر مرکزیت یافتہ میسجنگ پروٹوکول ہے۔ OpenClaw کسی بھی homeserver پر Matrix **user** کے طور پر کنیکٹ ہوتا ہے، اس لیے بوٹ کے لیے آپ کو ایک Matrix اکاؤنٹ درکار ہے۔ لاگ اِن ہونے کے بعد، آپ بوٹ کو براہِ راست DM کر سکتے ہیں یا اسے کمروں (Matrix کے "groups") میں مدعو کر سکتے ہیں۔ Beeper بھی ایک درست کلائنٹ آپشن ہے، لیکن اس کے لیے E2EE کا فعال ہونا ضروری ہے۔

حیثیت: پلگ اِن (@vector-im/matrix-bot-sdk) کے ذریعے معاونت یافتہ۔ براہِ راست پیغامات، کمرے، تھریڈز، میڈیا، ری ایکشنز،
پولز (بھیجنا + poll-start بطور متن)، لوکیشن، اور E2EE (کرپٹو سپورٹ کے ساتھ)۔
...
6. کسی بھی Matrix کلائنٹ سے بوٹ کے ساتھ DM شروع کریں یا اسے کسی کمرے میں مدعو کریں
   (Element، Beeper، وغیرہ؛ دیکھیں [https://matrix.org/ecosystem/clients/](https://matrix.org/ecosystem/clients/))۔ Beeper کے لیے E2EE درکار ہے،
   اس لیے `channels.matrix.encryption: true` سیٹ کریں اور ڈیوائس کی توثیق کریں۔
...
- کلید شیئرنگ فعال کرنے کے لیے کسی دوسرے Matrix کلائنٹ (Element وغیرہ) میں ڈیوائس کی توثیق کریں۔
...
کرپٹو اسٹیٹ ہر اکاؤنٹ + ایکسیس ٹوکن کے مطابق محفوظ ہوتی ہے
`~/.openclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
(SQLite ڈیٹابیس) میں۔ Sync اسٹیٹ اس کے ساتھ ہی
`bot-storage.json` میں رہتی ہے۔
اگر ایکسیس ٹوکن (ڈیوائس) بدل جائے تو نیا اسٹور بنایا جاتا ہے اور بوٹ کو
انکرپٹڈ کمروں کے لیے دوبارہ ویریفائی کرنا ہوتا ہے۔

**ڈیوائس ویریفیکیشن:**
جب E2EE فعال ہو، بوٹ اسٹارٹ اپ پر آپ کی دیگر سیشنز سے ویریفیکیشن کی درخواست کرے گا۔
Element (یا کسی اور کلائنٹ) کو کھولیں اور اعتماد قائم کرنے کے لیے ویریفیکیشن درخواست منظور کریں۔
ویریفائی ہونے کے بعد، بوٹ انکرپٹڈ کمروں میں پیغامات ڈکرپٹ کر سکتا ہے۔
...
- ڈیفالٹ: `channels.matrix.dm.policy = "pairing"`۔ نامعلوم ارسال کنندگان کو pairing کوڈ ملتا ہے۔
- منظوری دیں بذریعہ:
  - `openclaw pairing list matrix`
  - `openclaw pairing approve matrix <CODE>`
- عوامی DMs: `channels.matrix.dm.policy="open"` کے ساتھ `channels.matrix.dm.allowFrom=["*"]`۔
- `channels.matrix.dm.allowFrom` مکمل Matrix صارف IDs قبول کرتا ہے (مثال: `@user:server`)۔ وِزارڈ ڈسپلے ناموں کو صارف IDs میں ریزولو کرتا ہے جب ڈائریکٹری سرچ ایک واحد درست میچ تلاش کرے۔

## Rooms (groups)

- ڈیفالٹ: `channels.matrix.groupPolicy = "allowlist"` (mention-gated)۔ جب غیر سیٹ ہو تو ڈیفالٹ کو اوور رائیڈ کرنے کے لیے `channels.defaults.groupPolicy` استعمال کریں۔
...
- `channels.matrix.dm.allowFrom`: DM allowlist (مکمل Matrix صارف IDs)۔ `open` کے لیے `"*"` درکار ہے۔ وِزارڈ جہاں ممکن ہو ناموں کو IDs میں ریزولو کرتا ہے۔
