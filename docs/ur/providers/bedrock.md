...
OpenClaw، pi‑ai کے **Bedrock Converse** اسٹریمنگ فراہم کنندہ کے ذریعے **Amazon Bedrock** ماڈلز استعمال کر سکتا ہے۔ Bedrock کی تصدیق **AWS SDK default credential chain** استعمال کرتی ہے، API کلید نہیں۔
...
اگر AWS اسناد کا پتا چل جائے تو OpenClaw خود بخود ایسے Bedrock ماڈلز دریافت کر سکتا ہے جو **streaming** اور **text output** کو سپورٹ کرتے ہوں۔ دریافت کے لیے `bedrock:ListFoundationModels` استعمال ہوتا ہے اور یہ کیش کیا جاتا ہے (بطورِ طے شدہ: 1 گھنٹہ)۔
...
جب OpenClaw کسی EC2 انسٹینس پر چل رہا ہو جس کے ساتھ IAM رول منسلک ہو، تو AWS SDK خود بخود تصدیق کے لیے instance metadata service (IMDS) استعمال کرے گا۔ تاہم، OpenClaw کی اسناد کی شناخت فی الحال صرف environment variables کو چیک کرتی ہے، IMDS اسناد کو نہیں۔

**Workaround:** `AWS_PROFILE=default` سیٹ کریں تاکہ یہ اشارہ ملے کہ AWS اسناد دستیاب ہیں۔ اصل تصدیق اب بھی IMDS کے ذریعے instance role ہی استعمال کرتی ہے۔
