...
Cron، Gateway کا بلٹ اِن شیڈیولر ہے۔ یہ جابز کو محفوظ رکھتا ہے، درست وقت پر ایجنٹ کو بیدار کرتا ہے، اور اختیاری طور پر آؤٹ پٹ کو واپس چیٹ تک پہنچا سکتا ہے۔
...
بطورِ طے شدہ، کرون جابز Gateway ہوسٹ پر `~/.openclaw/cron/jobs.json` میں محفوظ ہوتی ہیں۔
Gateway فائل کو میموری میں لوڈ کرتا ہے اور تبدیلیوں پر واپس لکھتا ہے، اس لیے دستی ترامیم
صرف اس وقت محفوظ ہیں جب Gateway بند ہو۔ تبدیلیوں کے لیے `openclaw cron add/edit` یا کرون
ٹول کال API کو ترجیح دیں۔
...
اختیاری: ون شاٹ جابز (`schedule.kind = "at"`) کامیابی کے بعد بطورِ طے شدہ حذف ہو جاتی ہیں۔ انہیں رکھنے کے لیے
`deleteAfterRun: false` سیٹ کریں (کامیابی کے بعد غیر فعال ہو جائیں گی)۔
...
جابز کی شناخت ایک مستحکم `jobId` سے ہوتی ہے (CLI/Gateway APIs میں استعمال ہوتا ہے)۔
ایجنٹ ٹول کالز میں `jobId` معیاری ہے؛ مطابقت کے لیے قدیم `id` قبول کیا جاتا ہے۔
ون شاٹ جابز بطورِ طے شدہ کامیابی کے بعد خود بخود حذف ہو جاتی ہیں؛ انہیں رکھنے کے لیے `deleteAfterRun: false` سیٹ کریں۔
...
کرون ایکسپریشنز `croner` استعمال کرتی ہیں۔ اگر ٹائم زون شامل نہ ہو تو Gateway ہوسٹ کا
مقامی ٹائم زون استعمال ہوتا ہے۔

### مین بمقابلہ isolated عمل درآمد

#### مین سیشن جابز (سسٹم ایونٹس)

مین جابز ایک سسٹم ایونٹ قطار میں ڈالتی ہیں اور اختیاری طور پر ہارٹ بیٹ رنر کو بیدار کرتی ہیں۔
انہیں `payload.kind = "systemEvent"` استعمال کرنا لازم ہے۔
...
یہ اس وقت بہترین ہے جب آپ عام ہارٹ بیٹ پرامپٹ + مین سیشن سیاق چاہتے ہوں۔
دیکھیں: [Heartbeat](/gateway/heartbeat)۔
...
اعلان ڈیلیوری اس رن کے لیے میسجنگ ٹول کے بھیجے گئے پیغامات کو دبا دیتی ہے؛ چیٹ کو ہدف بنانے کے لیے
`delivery.channel`/`delivery.to` استعمال کریں۔ جب `delivery.mode = "none"` ہو تو مین سیشن میں کوئی خلاصہ پوسٹ نہیں ہوتا۔

اگر isolated جابز کے لیے `delivery` شامل نہ ہو تو OpenClaw بطورِ طے شدہ `announce` استعمال کرتا ہے۔

#### اعلان ڈیلیوری کا بہاؤ

جب `delivery.mode = "announce"` ہو تو کرون آؤٹ باؤنڈ چینل ایڈاپٹرز کے ذریعے براہِ راست ڈیلیور کرتا ہے۔
پیغام تیار کرنے یا آگے بھیجنے کے لیے مین ایجنٹ نہیں چلایا جاتا۔
...
نوٹ: آپ مین سیشن جابز پر بھی `model` سیٹ کر سکتے ہیں، مگر یہ مشترکہ مین
سیشن ماڈل کو بدل دیتا ہے۔ غیر متوقع سیاق تبدیلیوں سے بچنے کے لیے ہم ماڈل اووررائیڈز
صرف isolated جابز کے لیے تجویز کرتے ہیں۔
...
Telegram، `message_thread_id` کے ذریعے فورم ٹاپکس سپورٹ کرتا ہے۔ کرون ڈیلیوری کے لیے، آپ
ٹاپک/تھریڈ کو `to` فیلڈ میں اینکوڈ کر سکتے ہیں:
...
Gateway `cron.*` ٹولز کو براہِ راست کال کرتے وقت (ایجنٹ ٹول کالز یا RPC) یہ ساختیں استعمال کریں۔
CLI فلیگز انسانی دورانیے جیسے `20m` قبول کرتے ہیں، مگر ٹول کالز کے لیے
`schedule.at` کے لیے ISO 8601 اسٹرنگ اور `schedule.everyMs` کے لیے ملی سیکنڈز استعمال کریں۔
