...
TypeBox ایک TypeScript‑first اسکیمہ لائبریری ہے۔ ہم اسے **Gateway
WebSocket پروٹوکول** (ہینڈشیک، ریکویسٹ/ریسپانس، سرور ایونٹس) کی تعریف کے لیے استعمال کرتے ہیں۔ یہ اسکیمائیں **رن ٹائم ویلیڈیشن**، **JSON Schema ایکسپورٹ**، اور macOS ایپ کے لیے **Swift کوڈ جنریشن** کو چلاتی ہیں۔ واحد ماخذِ حقیقت؛ باقی سب کچھ اسی سے تیار ہوتا ہے۔
...
پہلا فریم **لازم** طور پر `connect` ریکویسٹ ہونا چاہیے۔ اس کے بعد، کلائنٹس
میتھڈز کال کر سکتے ہیں (مثلاً `health`, `send`, `chat.send`) اور ایونٹس کو سبسکرائب کر سکتے ہیں (مثلاً
`presence`, `tick`, `agent`)۔
...
- **سرور سائیڈ**: ہر اِن باؤنڈ فریم کو AJV کے ساتھ ویلیڈیٹ کیا جاتا ہے۔ ہینڈشیک صرف `connect` ریکویسٹ قبول کرتا ہے جس کے پیرامیٹرز `ConnectParams` سے میچ کرتے ہوں۔
...
جنریٹ شدہ JSON Schema ریپو میں `dist/protocol.schema.json` پر موجود ہے۔ شائع شدہ را فائل عموماً یہاں دستیاب ہوتی ہے:
