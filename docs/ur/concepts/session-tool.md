...
`global` اور `unknown` محفوظ اقدار ہیں اور کبھی فہرست میں شامل نہیں کی جاتیں۔ اگر `session.scope = "global"` ہو تو ہم تمام ٹولز کے لیے اسے `main` کے طور پر عرف دیتے ہیں تاکہ کالرز کبھی `global` نہ دیکھیں۔
...
- اگر انتظار ٹائم آؤٹ ہو جائے: `{ runId, status: "timeout", error }`۔ رَن جاری رہتا ہے؛ بعد میں `sessions_history` کال کریں۔
...
- `agents.list[].subagents.allowAgents`: `agentId` کے ذریعے اجازت یافتہ ایجنٹ ids کی فہرست (`["*"]` کسی کو بھی اجازت دینے کے لیے)۔ ڈیفالٹ: صرف درخواست گزار ایجنٹ۔
