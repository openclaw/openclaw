...
Synthetic eksponerer Anthropic-kompatible endpoints. OpenClaw registrerer det som
`synthetic`-udbyderen og bruger Anthropic Messages API.
...
Bemærk: OpenClaws Anthropic-klient tilføjer `/v1` til basis-URL’en, så brug
`https://api.synthetic.new/anthropic` (ikke `/anthropic/v1`). Hvis Synthetic ændrer
sin basis-URL, skal du tilsidesætte `models.providers.synthetic.baseUrl`.
