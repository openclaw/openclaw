...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.smart-agent-neo/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.smart-agent-neo/node.json` (node id + pairing token).
...
Path: `~/.smart-agent-neo/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.smart-agent-neo/exec-approvals.sock",
...
- Unix socket at `~/.smart-agent-neo/exec-approvals.sock` (0600).
