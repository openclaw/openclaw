...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.activi/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.activi/node.json` (node id + pairing token).
...
Path: `~/.activi/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.activi/exec-approvals.sock",
...
- Unix socket at `~/.activi/exec-approvals.sock` (0600).
