...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.easyhub/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.easyhub/node.json` (node id + pairing token).
...
Path: `~/.easyhub/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.easyhub/exec-approvals.sock",
...
- Unix socket at `~/.easyhub/exec-approvals.sock` (0600).
