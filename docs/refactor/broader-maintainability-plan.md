...
- Stage 3B: started with a shared gateway reachability and health-check workflow, a shared workspace resolution and workspace-config seam, and a shared gateway mode probe summary reused by configure and setup flows.
...
2. Stage 3B: extract shared gateway exposure safety normalization for tailscale, bind, and auth constraints before touching broader onboarding branching.
