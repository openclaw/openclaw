...
- Publish as `activi/plugin-sdk` (or export from core under `activi/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ActiviPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ActiviConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ActiviConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ActiviConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ActiviConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ActiviConfig): string;
  };
};
```
...
- Introduce `activi/plugin-sdk`.
- Add `api.runtime` to `ActiviPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `activiRuntime: ">=2026.2.0"`).
