...
- Publish as `smart-agent-neo/plugin-sdk` (or export from core under `smart-agent-neo/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SmartAgentNeoPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SmartAgentNeoConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SmartAgentNeoConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SmartAgentNeoConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SmartAgentNeoConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SmartAgentNeoConfig): string;
  };
};
```
...
- Introduce `smart-agent-neo/plugin-sdk`.
- Add `api.runtime` to `SmartAgentNeoPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `smartAgentNeoRuntime: ">=2026.2.0"`).
