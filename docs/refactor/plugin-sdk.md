...
- Publish as `EasyHub/plugin-sdk` (or export from core under `EasyHub/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `EasyHubPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: EasyHubConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: EasyHubConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: EasyHubConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: EasyHubConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: EasyHubConfig): string;
  };
};
```
...
- Introduce `EasyHub/plugin-sdk`.
- Add `api.runtime` to `EasyHubPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `EasyHubRuntime: ">=2026.2.0"`).
