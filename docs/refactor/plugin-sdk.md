...
Delivery:
- Publish as `@dna/plugin-sdk` (or export from core under `dna/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `DNAPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: DNAConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: DNAConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: DNAConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: DNAConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: DNAConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: DNAConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@dna/plugin-sdk`.
- Add `api.runtime` to `DNAPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `dnaRuntime: ">=2026.2.0"`).
