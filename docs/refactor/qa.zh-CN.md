...
将 OpenClaw QA 从拆分定义模型转变为单一真实来源：
...
理想的最终状态是一个通用的 QA 测试工具，加载强大的场景定义文件，而不是在 TypeScript 中硬编码大多数行为。

## 当前状态

主要真实来源现在位于 `qa/scenarios/index.md` 加上 `qa/scenarios/<theme>/*.md` 下的每个场景一个文件。
...
因此，真实来源的拆分已经修复，但执行仍然主要由处理程序支持，而不是完全声明性的。
...
这些类别很重要，因为它们驱动 DSL 要求。提示 + 预期文本的平面列表是不够的。

## 方向

### 单一真实来源

使用 `qa/scenarios/index.md` 加上 `qa/scenarios/<theme>/*.md` 作为编写的真实来源。
...
使用 Markdown 作为顶级格式，其中包含结构化的 YAML。
...
没有变量支持，测试工具将继续将场景逻辑泄漏回 TypeScript。

## 什么应该作为逃生通道

在第 1 阶段，完全纯声明性的运行器是不现实的。
...
- 85-90% 声明性
- 对于硬剩余部分，使用显式的 `customHandler` 步骤
- 仅命名和记录的自定义处理程序
- 场景文件中没有匿名内联代码
...
- 变量、工件、工具断言、请求日志断言已验证

### 阶段 5：将硬场景保留在自定义处理程序上
...
当前 UI 已经可以渲染线程和反应，因此附件渲染应该分层到相同的消息卡片模型上。
...
- 设置/清理是否应该是命名部分还是只是有序的操作列表
- 工件引用是否应该在架构中强类型或基于字符串
- 自定义处理程序是否应该住在一个注册表中还是每个表面的注册表中
