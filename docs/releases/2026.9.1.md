...
macOS now treats detected AI access as a choice, not consent: it does not test or select a detected Codex, Claude, key, or local model until you click it. The provider picker includes installable official provider plugins, supports Gateway-owned custom OpenAI/Anthropic-compatible endpoint setup, and asks fresh installs before showing existing native provider conversations.
