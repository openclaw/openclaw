...
<AccordionGroup>
  ...
  <Accordion title="Talk mode (browser realtime)">
    ...
    Gateway relay providers can emit inbound transcript updates while audio is still streaming. For xAI, the bridge maps the cumulative `conversation.item.input_audio_transcription.updated` event to user transcript updates so Talk and meeting transcription surfaces can consume live inbound speech content.
