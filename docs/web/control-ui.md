...
Windows path insertion supports PowerShell, `cmd.exe`, and recognized POSIX shells such as Git Bash. Other Windows shell overrides are refused because their quoting rules and path namespace cannot be inferred safely; run the Gateway inside WSL for a native WSL terminal and Linux upload paths. `cmd.exe` paths containing `%` or `!` are also refused because that shell expands those characters even inside double quotes.
