...
If the page fails to load, start the Gateway first: `smart-agent-neo gateway`.
...
you revoke it with `smart-agent-neo devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.smart-agent-neo/smart-agent-neo.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SmartAgentNeo
...
```bash
smart-agent-neo gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
SmartAgentNeo **blocks** Control UI connections without device identity.
...
```bash
SMART_AGENT_NEO_CONTROL_UI_BASE_PATH=/smart-agent-neo/ pnpm ui:build
```
