...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - Control UI deep links can request a one-shot hidden bootstrap turn with `?autostart=<value>` or `#autostart=<value>`. Supported values are `1`, `true`, `yes`, `on`, and `bootstrap`; unsupported/custom values are ignored after the URL is stripped. Autostart waits for a successful empty-session `chat.history` load, then sends the default bootstrap prompt through `chat.send` with `hideUserMessage: true`.
    - `hideUserMessage: true` requires an `operator.admin` session, hides only the triggering user prompt, and cannot be combined with `deliver: true` or attachments. Normal sends should omit the flag.
