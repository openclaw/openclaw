...
- Live `chat` events also suppress commentary-phase assistant stream text, including signature-only phase metadata, so internal progress text does not briefly appear and then disappear after a history reload.
