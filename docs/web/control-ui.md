...
If the page fails to load, start the Gateway first: `evox gateway`.
...
created. Re-run `evox devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `evox devices revoke --device <id> --role <role>`. See
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop evox`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. EVOX.sh
...
```bash
evox gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
EVOX.sh **blocks** Control UI connections without device identity.
