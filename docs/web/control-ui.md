...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - The sidebar groups sessions into Pinned and Chats with a New Session action and an All Sessions link. Pinned sessions always stay visible; the Chats group shows the nine most recent sessions in a stable recency order, with the open session highlighted in place instead of jumping to the top. A new dashboard session asynchronously gets a concise generated title from its first non-command message; explicit names are never replaced. Set `agents.defaults.utilityModel` (or `agents.list[].utilityModel`) to route this separate model call to a lower-cost model. With multiple agents, a compact scope control in the Chats header switches agents; the list shows only sessions tied to the selected agent and falls back to that agent's main session when it has no saved dashboard sessions yet.
    - Session search lives in the command palette (⌘K, or the search button in the top bar): typing a query lists matching chats across agents next to navigation commands, and selecting one opens it. The All Sessions page keeps the full searchable list with filters.
    - Each sidebar session row can pin or archive the session; renaming and bulk management live on the All Sessions page. An active run and an agent's main session cannot be archived. Archiving the currently selected session switches Chat back to that agent's main session.
    - In the macOS app, the OpenClaw brand mark sits in the native titlebar next to the window controls instead of occupying a sidebar row.
