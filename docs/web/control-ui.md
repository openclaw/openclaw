...
If the page fails to load, start the Gateway first: `EasyHub gateway`.
...
you revoke it with `EasyHub devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.easyhub/easyhub.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. EasyHub
...
```bash
EasyHub gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
EasyHub **blocks** Control UI connections without device identity.
...
```bash
EASYHUB_CONTROL_UI_BASE_PATH=/EasyHub/ pnpm ui:build
```
