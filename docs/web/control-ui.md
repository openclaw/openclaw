...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - When `gateway.controlUi.unifiedSession` is enabled, the Control UI joins the agent's single main session (`agent:{agentId}:main`) on connect and hides per-conversation session switching (new-session button, session picker, and recent-sessions list). Web chat and channel DMs then share one continuous session per agent. Default off.
