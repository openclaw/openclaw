...
- Control UI captures the rendered `sessionId` and `activeLeafEntryId` together for each send. The leaf is an opaque logical branch/CAS token and can identify a reset or compaction control event that is not shown in `messages`; `null` authoritatively means there is no active leaf. Normal background appends remain sendable when the captured token is a same-path ancestor in the same reset epoch. A branch switch or reset-epoch change makes Control UI park the message for review and refresh the transcript instead of posting it to the new branch. Ordinary queued and restored-outbox sends retain that submit-time revision. When an ordinary queued item is promoted to a steer, Control UI discards the old revision and freezes the target run ID; every attempt uses only the current `sessionId` and `activeLeafEntryId` pair for that same run. If that pair is unavailable, the steer sends only `expectedRunId` instead of borrowing the displayed transcript revision. A restored legacy steer without a frozen run target fails visibly rather than being redirected to a successor run. Legacy ordinary stored rows without a revision keep their previous replay behavior.
