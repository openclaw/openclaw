...
- [http://127.0.0.1:18790/](http://127.0.0.1:18790/) (or [http://localhost:18790/](http://localhost:18790/))
...
- Re-open anytime: `mullusi dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- **Localhost**: open `http://127.0.0.1:18790/`.
- **Shared-secret token source**: `gateway.auth.token` (or
  `MULLUSI_GATEWAY_TOKEN`); `mullusi dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `mullusi dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `mullusi dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `MULLUSI_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `mullusi status`; remote: SSH tunnel `ssh -N -L 18790:127.0.0.1:18790 user@host` then open `http://127.0.0.1:18790/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `mullusi config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `MULLUSI_GATEWAY_TOKEN` in this shell, then rerun `mullusi dashboard`
  - No shared secret configured: `mullusi doctor --generate-gateway-token`
