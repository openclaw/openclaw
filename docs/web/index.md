...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/EasyHub`)
...
{
  gateway: {
    controlUi: { enabled: true, basePath: "/EasyHub" }, // basePath optional
  },
}
```
...
```json5
{
  gateway: {
    ...
    auth: { mode: "password" }, // or EASYHUB_GATEWAY_PASSWORD
  },
}
```
