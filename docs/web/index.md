...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/activi`)
...
{
  gateway: {
    controlUi: { enabled: true, basePath: "/activi" }, // basePath optional
  },
}
```
...
```json5
{
  gateway: {
    ...
    auth: { mode: "password" }, // or ACTIVI_GATEWAY_PASSWORD
  },
}
```
