...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/smart-agent-neo`)
...
{
  gateway: {
    controlUi: { enabled: true, basePath: "/smart-agent-neo" }, // basePath optional
  },
}
```
...
```json5
{
  gateway: {
    ...
    auth: { mode: "password" }, // or SMART_AGENT_NEO_GATEWAY_PASSWORD
  },
}
```
