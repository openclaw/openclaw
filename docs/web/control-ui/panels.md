...
To save an asset opened in the panel, click **Download file** beside the pencil and pointer icons. The icon becomes a spinner while downloading, and the preview stays open without a progress or success message below the toolbar. Mac tabs show a Save dialog and use the tab's signed-in browser session. Agent browser tabs download through your current web browser; the asset server must allow access from that browser. If the download fails, the panel shows an error with an option to use **Open in your browser** instead. Blank and loading tabs cannot be downloaded.
