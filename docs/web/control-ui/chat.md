...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - **Split view:** open it from the chat title bar (beside the thread diff, background tasks, and thread files toggles), then split the active pane right or down for as many panes as fit. Each pane has its own thread, transcript, composer, and tool stream. New splits share their row or column evenly until you drag a divider; **Balance panes** (the grid button in the pane title bar, the layout menu, or ⌘⌥⇧B / Ctrl+Alt+Shift+B) resets every column and stacked pane to equal shares and clears manual sizing. A pane whose session has a colour carries it as a top edge, a tinted header and a colour wash over the whole pane so panes read apart at a glance.
