...
When the agent's primary model runs on a CLI runtime such as `claude-cli` and no utility model is set explicitly, Side chat answers through that same CLI runtime, so subscription-only installs need no provider API key. On that route each answer is a tool-free, one-shot side question that uses the bounded session snapshot and observer notes instead of the read-only session tools, and image questions are not supported.
