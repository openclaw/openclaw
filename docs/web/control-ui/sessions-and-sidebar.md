...
at the top in chip mode while pinned. Plugin links shown by default can be reordered but not
dragged out of Pages to unpin them; optional plugin destinations can still be unpinned.

**Unpin Home.** Open **Edit pinned items** from the Pages customize menu (or right-click the navigation area) and clear **Home**. The shortcut disappears and the same conversation becomes available in the session list, where it can be assigned to a custom group. Check **Home** again to restore the shortcut; the ordinary main-session row is hidden again, except when needed to expose subagent children. Its transcript, group metadata, and message routing are unchanged. Home stays pinned by default, and **Reset pinned items** restores it. This choice uses the same preference persistence as other navigation pins.
