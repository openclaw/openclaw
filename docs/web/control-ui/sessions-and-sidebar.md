...
The destination picker opens with **Search environments** focused. Its compact, single-line results stay ordered **Local → Devices → Auto → Cloud**, with cloud profiles grouped under a **Cloud** heading. Local, devices, Auto, and cloud profiles use house, device, shuffle, and cloud icons respectively. Connected Macs named MacBook, Mac mini, or Mac Studio use the matching hardware outline; unknown devices use a monitor. The macOS app shell supplies native SF Symbols for those three shapes; other clients keep the web outlines. This is a display-name hint, not hardware detection, and never changes placement eligibility. Local means the Gateway host, not necessarily the computer running your browser. Search matches destination names, types, IDs, and device facts, including capabilities and unavailable reasons. Devices retain their worker-slot indicator and busy-count tooltip when selected, with a checkmark beside the indicator. Selectable device rows show a short platform status. Disabled environment rows show their status and reason inline on one line, truncated with an ellipsis when needed; the tooltip keeps the full text. **Auto** is a single-select row after the devices. Selecting Local, a device, or a cloud profile turns Auto off. The selected row has a distinct highlight, separated from neighboring hover highlights. **Connect a machine** stays below a separator. Closing and reopening the picker clears its search.
