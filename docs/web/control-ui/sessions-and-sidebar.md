...
When available, the original send error stays visible alongside the latest delivery-check result. Updating the Control UI preserves existing tab-local startup recovery without a data migration: records that lack the optional original-error field still restore their prompt, attachments, and delivery state. The storage namespace, Gateway credential scope, and tab lifetime are unchanged. An update cannot recover an error that an older client already discarded. Incognito recovery remains in memory only and does not survive a reload.
