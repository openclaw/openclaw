...
```bash
EasyHub tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `EasyHub tui --deliver`
...
- Check the Gateway logs: `EasyHub logs --follow`.
- Confirm the agent can run: `EasyHub status` and `EasyHub models status`.
...
- No agents in picker: check `EasyHub agents list` and your routing config.
