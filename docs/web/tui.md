...
```bash
evox tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `evox tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `evox tui`.
...
- Check the Gateway logs: `evox logs --follow`.
- Confirm the agent can run: `evox status` and `evox models status`.
...
- No agents in picker: check `evox agents list` and your routing config.
