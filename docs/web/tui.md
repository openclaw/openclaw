...
```bash
activi tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `activi tui --deliver`
...
- Check the Gateway logs: `activi logs --follow`.
- Confirm the agent can run: `activi status` and `activi models status`.
...
- No agents in picker: check `activi agents list` and your routing config.
