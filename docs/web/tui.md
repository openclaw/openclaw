...
```bash
smart-agent-neo tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `smart-agent-neo tui --deliver`
...
- Check the Gateway logs: `smart-agent-neo logs --follow`.
- Confirm the agent can run: `smart-agent-neo status` and `smart-agent-neo models status`.
...
- No agents in picker: check `smart-agent-neo agents list` and your routing config.
