...
```bash
mullusi tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `mullusi tui --deliver`
...
- Local shell commands receive `MULLUSI_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `MULLUSI_THEME=light` before launching `mullusi tui`.
- To force the original dark palette instead, set `MULLUSI_THEME=dark`.
...
- Check the Gateway logs: `mullusi logs --follow`.
- Confirm the agent can run: `mullusi status` and `mullusi models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `mullusi agents list` and your routing config.
