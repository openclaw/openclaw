...
6) Optional allowlist: use `telegram.allowFrom` for direct chats by chat id (`123456789` or `telegram:123456789`) and `telegram.allowFromGroups` to restrict which group chats the bot will accept messages from (group ids or `telegram:<id>`).
...
- Outbound: text and media (photo/video/audio/document) with optional caption; chunked to limits. Typing cue sent best-effort.
  - Config: `TELEGRAM_BOT_TOKEN` env or `telegram.botToken` required; `telegram.groups`, `telegram.allowFrom`, `telegram.allowFromGroups`, `telegram.mediaMaxMb`, `telegram.replyToMode`, `telegram.proxy`, `telegram.webhookSecret`, `telegram.webhookUrl`, `telegram.webhookPath` supported.
...
```json5
{
  telegram: {
    ...
    allowFromGroups: ["-123456789"], // group chat ids, allowed (or "*")
    mediaMaxMb: 5,
