...
### Impact
This collides directly with the project rule in `AGENTS.md`: *"Bind-mount, never rebuild images."* The build pipeline produces JS files that look correct on the host but are invisible inside the gateway container. Code changes silently fail to deploy with no error indication — a developer-experience footgun that can also waste hours of debugging.
...
const sameSchema  = existing?.payload?.payload_schema_version === point.payload.payload_schema_version;
if (sameHash && sameSchema) { unchanged.push(point); continue; }
```
