...
- `cache:state` includes `options.snapshot.toolFingerprints`: a name-sorted list of tool names and their optional `descriptionDigest` and `schemaDigest` hashes. Compare entries by name across requests to locate descriptor changes even when `toolNames` and `toolCount` stay the same. These fingerprints reuse the bounded diagnostic schema projection; they do not contain raw descriptions or schemas and are not a full provider-payload equality check. They identify which descriptor changed, not why a capability was selected.
