...
- When `contextPruning.mode: "cache-ttl"` is configured, direct OpenAI models can use cache-TTL pruning with the automatic prompt cache path. OpenClaw records the cache-touch marker for pruning timing only; it does not add Anthropic `cache_control` blocks or manage an OpenAI cache resource.
