...
OpenClaw 使用专用的工作区目录来存放代理。默认：`~/.openclaw/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
3. 可选：如果你想要个人助手技能列表，用此文件替换 AGENTS.md：
...
- 读取 `SOUL.md`、`USER.md` 和 `memory/` 中的今天和昨天的内容。
- 当存在 `MEMORY.md` 时读取它；仅当 `MEMORY.md` 不存在时才回退到小写的 `memory.md`。
- 在响应之前执行此操作。
...
- 小写的 `memory.md` 仅作为遗留回退；不要故意同时保留两个根文件。
...
- 在 `TOOLS.md` 中保留环境特定的注释（技能注释）。

## 备份提示（推荐）

如果你将此工作区视为 Clawd 的“记忆”，将其设为 git 仓库（理想情况下是私有的），以便 `AGENTS.md` 和你的记忆文件得到备份。
...
- 运行 WhatsApp 网关 + Pi 编码代理，使助手能够通过主机 Mac 读取/写入聊天、获取上下文并运行技能。
...
- **oracle** — 支持 OpenAI 的代理 CLI，具有会话重放和浏览器控制。
- **eightctl** — 从终端控制你的睡眠。
...
- **OpenHue CLI** — 用于场景和自动化的飞利浦 Hue 灯光控制。
...
- 保持心跳启用，以便助手可以安排提醒、监控收件箱和触发相机捕获。
- Canvas UI 以全屏运行，带有原生覆盖。避免在左上角/右上角/底部边缘放置关键控件；在布局中添加显式装订线，不要依赖安全区域插入。
- 对于浏览器驱动的验证，使用 `openclaw browser`（标签/状态/截图）和 OpenClaw 管理的 Chrome 配置文件。
- 对于 DOM 检查，使用 `openclaw browser eval|query|dom|snapshot`（当你需要机器输出时使用 `--json`/`--out`）。
- 对于交互，使用 `openclaw browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要快照引用；使用 `evaluate` 进行 CSS 选择器）。
