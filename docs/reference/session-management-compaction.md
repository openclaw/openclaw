...
This document explains how EVOX.sh manages sessions end-to-end:
...
EVOX.sh is designed around a single **Gateway process** that owns session state.
...
EVOX.sh persists sessions in two layers:
...
EVOX.sh resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, EVOX.sh reports potential evictions but does not mutate the store/files.
...
EVOX.sh intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (EVOX.sh consumes the events, but Pi decides when to compact).
...
EVOX.sh also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, EVOX.sh bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, EVOX.sh leaves it alone.
...
EVOX.sh supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- EVOX.sh strips/suppresses this in the delivery layer.

As of `2026.1.10`, EVOX.sh also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
EVOX.sh uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but EVOX.sh’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `evox status`.
