...
This document explains how Mullusi manages sessions end-to-end:
...
Mullusi is designed around a single **Gateway process** that owns session state.
...
Mullusi persists sessions in two layers:
...
Mullusi resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Mullusi reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.mullusi/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Mullusi intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Mullusi splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Mullusi
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Mullusi preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Mullusi consumes the events, but Pi decides when to compact).
...
Mullusi also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Mullusi bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Mullusi leaves it alone.
...
Mullusi supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Mullusi strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Mullusi also suppresses **draft/typing streaming** when a
...
Mullusi uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Mullusi’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `mullusi status`.
