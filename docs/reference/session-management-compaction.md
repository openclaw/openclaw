...
This document explains how SmartAgentNeo manages sessions end-to-end:
...
SmartAgentNeo is designed around a single **Gateway process** that owns session state.
...
SmartAgentNeo persists sessions in two layers:
...
- Transcripts: `~/.smart-agent-neo/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

SmartAgentNeo resolves these via `src/config/sessions.ts`.
...
SmartAgentNeo intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (SmartAgentNeo consumes the events, but Pi decides when to compact).
...
SmartAgentNeo also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, SmartAgentNeo bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, SmartAgentNeo leaves it alone.
...
SmartAgentNeo supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- SmartAgentNeo strips/suppresses this in the delivery layer.

As of `2026.1.10`, SmartAgentNeo also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
SmartAgentNeo uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but SmartAgentNeo’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `smart-agent-neo status`.
