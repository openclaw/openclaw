...
This document explains how Gemmaclaw manages sessions end-to-end:
...
Gemmaclaw is designed around a single **Gateway process** that owns session state.
...
Gemmaclaw persists sessions in two layers:
...
Gemmaclaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Gemmaclaw reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
gemmaclaw sessions cleanup --dry-run
gemmaclaw sessions cleanup --enforce
```
...
Gemmaclaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Gemmaclaw splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Gemmaclaw
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Gemmaclaw preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Gemmaclaw consumes the events, but Pi decides when to compact).
...
Gemmaclaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Gemmaclaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Gemmaclaw leaves it alone.
...
- If the provider fails or returns an empty result, Gemmaclaw falls back to built-in LLM summarization automatically.
...
- `gemmaclaw status` (CLI)
- `gemmaclaw sessions` / `sessions --json`
...
Gemmaclaw supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Gemmaclaw strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Gemmaclaw also suppresses **draft/typing streaming** when a
...
Gemmaclaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Gemmaclaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `gemmaclaw status`.
