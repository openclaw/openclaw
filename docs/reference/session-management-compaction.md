...
- `compactionCheckpointMaxBytes`: per-session disk budget for retained compaction checkpoint snapshots (default `128mb`); the newest snapshot is always kept and older snapshots are evicted oldest-first once the budget is exceeded
