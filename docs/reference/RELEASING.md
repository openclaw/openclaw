...
older tooling without `npm-publish` or a self-sweeping parent receives exact
child-approval or stale-child sweep commands for the operator; `sync-beta` runs the beta-to-stable
...
otherwise the sweep commands are printed before dispatch), a child
`npm-publish` environment at the tooling SHA that lets receipt-authorized
npm children skip their own approval job (otherwise
...
6. **Flip GitHub as soon as npm is out.** The moment `openclaw@YYYY.M.PATCH`
   ...
   publish jobs start. npm children skip their approval job and publish in
   `npm-publish`, whose trusted publishers and deployment policy require a
   protected `release-publish/*` tag. Before publication they wait for the
   parent attempt's approval receipt and require it to remain `in_progress`.
   Direct human dispatch keeps its own `npm-release` approval job and does not
   use the receipt; it must also use a protected tooling tag. Never
...
SHA. Parent and child provenance must carry that same full ref. The parent
requires one `npm-release` human approval; receipt-authorized npm children
publish in `npm-publish` without additional reviewers. Bind every npm trusted
publisher (`openclaw`, gateway packages, and `@openclaw/*` plugins) to
`npm-publish` and its existing workflow file. That environment admits only
protected `release-publish/*` tags. Direct manual/recovery child dispatches
also require a tooling tag and a separate `npm-release` approval job before
publishing in `npm-publish`. Artifact-only preflights retain their current
refs; the plugin OIDC preflight requires the tooling tag and `npm-publish`.
...
Tideclaw alpha branch publication is blocked: its non-main SHA cannot receive
a tooling tag through `ensureReleasePublishToolingTag`, and the parent's
approval receipt rejects alpha branch refs. Keep its validation evidence and
recover through the owning nightly workflow when that route is repaired;
do not widen the environment policy.
...
- `plugin_npm_run_id`: successful exact-candidate `Plugin NPM Release` run id; required for a real `extended-stable` core publish. Direct core recovery accepts a plugin recovery run on the same protected tooling tag bound to that candidate.
...
`plugins` input, a final patch at or above `33`, and the exact tip of the canonical
`extended-stable/YYYY.M.33` branch as package source, dispatched from a protected
`release-publish/<sha12>-<epoch>` tooling tag with that canonical branch named
in `release_candidate_branch`. Approved direct workflow recovery uses the same
protected tooling tag and explicit `release_candidate_branch`, with its own
`npm-release` approval job. It never moves plugin
