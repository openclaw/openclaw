...
DNA features that can generate provider usage or paid API calls.
...
- `dna status --usage` and `dna channels list` show provider **usage windows**
...
DNA can pick up credentials from:
...
### 8) Model scan / probe
`dna models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
