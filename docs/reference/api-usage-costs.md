...
- **Ollama Web Search**: prefers a reachable Ollama host, then can fall back to Ollama Cloud when `OLLAMA_API_KEY` is available; host-backed setups may still require `ollama signin`
