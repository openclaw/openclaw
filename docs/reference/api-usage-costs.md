...
Activi features that can generate provider usage or paid API calls.
...
- `activi status --usage` and `activi channels list` show provider **usage windows**
...
Activi can pick up credentials from:
...
`activi models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
