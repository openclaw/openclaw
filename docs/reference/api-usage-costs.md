...
SmartAgentNeo features that can generate provider usage or paid API calls.
...
- `smart-agent-neo status --usage` and `smart-agent-neo channels list` show provider **usage windows**
...
SmartAgentNeo can pick up credentials from:
...
`smart-agent-neo models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
