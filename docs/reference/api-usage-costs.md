...
EVOX.sh features that can generate provider usage or paid API calls.
...
- `evox status --usage` and `evox channels list` show provider **usage windows**
...
EVOX.sh can pick up credentials from:
...
`evox models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
