...
EasyHub features that can generate provider usage or paid API calls.
...
- `EasyHub status --usage` and `EasyHub channels list` show provider **usage windows**
...
EasyHub can pick up credentials from:
...
`EasyHub models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
