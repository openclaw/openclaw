...
- If the latest transcript usage entry includes `usage.cost.total`, `/status`
  uses that transcript-backed cost directly instead of recalculating from token
  counts and local pricing.
