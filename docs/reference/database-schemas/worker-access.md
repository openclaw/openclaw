...
Mention Inbox snapshots and recipient policy facts use the shared fixed-read worker; exact session policy rows use the existing session reader. The Inbox serializes accepted operations, rereads its durable sources and canonical recipient identities inside the shared-state worker transaction, and requests live host grants at transaction and commit. Expiry and alias reconciliation do not depend on preparing unrelated session targets. Projection installation and recipient notifications follow acknowledged results. Explicit session invalidation uses this same target owner, and list-driven durable synchronization refreshes connected views. Pending view updates survive failed policy preparation or publication. Retention is scheduled before notification callbacks; a rejected view publication or push adapter does not suppress independent callbacks. Cached target facts retain canonical close custody after native readers settle; pruning, invalidation, failed preparation, and disposal release that custody. Personal session involvement remains a separate agent-database operation through the canonical agent executor, not a cross-database transaction. Transcript persistence joins accepted mention callbacks, and Gateway shutdown fences and drains Inbox work. Push delivery prepares current facts asynchronously and retains a synchronous, SQL-free final currentness check. Existing source codecs, recipient-centered excerpts, replay tombstones, retention, and update behavior are unchanged.
