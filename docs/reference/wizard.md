...
This is the full reference for `evox onboard`.
...
<Steps>
  <Step title="Existing config detection">
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `evox doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `evox pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `evox configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `evox health`.
    - Tip: `evox status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
`evox agents add` writes `agents.list[]` and optional `bindings`.
