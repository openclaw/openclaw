---
summary: "How EVOX.sh builds prompt context and reports token usage + costs"
read_when:
...
EVOX.sh tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

EVOX.sh assembles its own system prompt on every run. It includes:
...
For images, EVOX.sh downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from EVOX.sh session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `evox status --usage` and `evox channels list` show
...
`cacheWrite`. If pricing is missing, EVOX.sh shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. EVOX.sh can
...
Anthropic's 1M context window is currently beta-gated. EVOX.sh can inject the
...
EVOX.sh skips the `context-1m-*` beta header because Anthropic currently
