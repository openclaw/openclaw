---
summary: "How Activi builds prompt context and reports token usage + costs"
read_when:
...
Activi tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Activi assembles its own system prompt on every run. It includes:
...
For images, Activi downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Activi session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `activi status --usage` and `activi channels list` show
...
`cacheWrite`. If pricing is missing, Activi shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Activi can
...
Anthropic's 1M context window is currently beta-gated. Activi can inject the
