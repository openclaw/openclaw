...
Mullusi integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Mullusi owns lifecycle when `channels.signal.autoStart=true`.
...
- Mullusi spawns `imsg rpc` as a child process (legacy iMessage integration).
