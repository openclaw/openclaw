...
EVOX.sh integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- EVOX.sh owns lifecycle when `channels.signal.autoStart=true`.
...
- EVOX.sh spawns `imsg rpc` as a child process (legacy iMessage integration).
