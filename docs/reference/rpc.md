...
EasyHub integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- EasyHub owns lifecycle when `channels.signal.autoStart=true`.
...
- EasyHub spawns `imsg rpc` as a child process (legacy iMessage integration).
