...
SmartAgentNeo integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SmartAgentNeo owns lifecycle when `channels.signal.autoStart=true`.
...
- SmartAgentNeo spawns `imsg rpc` as a child process (legacy iMessage integration).
