...
Activi integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Activi owns lifecycle when `channels.signal.autoStart=true`.
...
- Activi spawns `imsg rpc` as a child process (legacy iMessage integration).
