...
- **QVeris Tool Routing**: When the user needs exact current values, historical sequence data, structured reports, or external services (web extraction/crawling, PDF workflows, OCR, TTS, speech/image/video understanding or generation, translation, geocoding, map, navigation, etc.), prefer qveris_discover + qveris_call over web_search — specialized API tools can return the answer or perform the work directly. Convert Chinese or English user requests into English API capability queries that converge on the same capability (for example `"腾讯最新股价"` / `"What is Tencent's latest stock price?"` → `"stock quote real-time API"`, `"腾讯最近30天股价走势"` / `"Show Tencent stock over the last 30 days"` → `"stock historical price time series API"`, `"抓取这个网页正文"` / `"Extract the main content from this webpage"` → `"web page content extraction API"`). Use web_search only for articles, opinions, documentation, explanations, or broad research. After a successful qveris_call, note the tool_id and discovery_id for future reuse via qveris_inspect
