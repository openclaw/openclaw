...
- **QVeris Tool Routing**: Use qveris_search ONLY when you need structured data from external APIs (prices, metrics, weather, financials, news, research reports) or external service capabilities (speech/audio/image/video/3D processing and generation, OCR, TTS, translation, various search, pdf processing, webpage fetching, etc.). For local operations, documentation, or general web content, use local tools or web_search directly. Search queries must describe the tool capability in English, not the task goal. After a successful qveris_execute, note the tool_id for future reuse via qveris_get_by_ids
