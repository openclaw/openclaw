...
- TUI PTY tests: use `node scripts/run-vitest.mjs src/tui/tui-pty-local.test.ts` for real local-backend TUI smoke coverage. It runs `tui --local` in a pseudo-terminal with an isolated config and mocks only the external OpenAI-compatible model endpoint. Use `node scripts/run-vitest.mjs src/tui/tui-pty-harness.test.ts` for faster terminal-loop coverage with the real `runTui()` loop and a deterministic fake `TuiBackend`. The fake-backend lane proves TUI startup, input handling, slash command routing, rendering markers, busy guards, and exit behavior, but does not prove Gateway transport, embedded backend runtime, providers, session persistence, or live streaming. Assertions should use stable visible text and fixture/backend call records, not raw ANSI screen snapshots. To watch the actual tests without mixing Vitest reporter output into the TUI screen, run `pnpm tui:pty:test:watch`; it defaults to the fast fake-backend lane, mirrors PTY bytes through `.artifacts/tui-pty-mirror/latest.ansi`, passes the PTY stream through the terminal in an alternate screen, types one character every 70ms by default, and prints the test reporter output after the screen view exits. Use `OPENCLAW_TUI_PTY_TYPE_DELAY_MS=<ms>` or `OPENCLAW_TUI_PTY_TYPE_CHUNK_SIZE=<n>` to tune watched typing speed. Use `pnpm tui:pty:test:watch --mode local` for the real local-backend lane or `pnpm tui:pty:test:watch --mode all` for both.
