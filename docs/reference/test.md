...
- TUI PTY tests: use `node scripts/run-vitest.mjs run --config test/vitest/vitest.tui-pty.config.ts`. The local lane runs `tui --local` and mocks only the external model endpoint; the fake lane runs `runTui()` with a deterministic `TuiBackend` fixture. Assert stable visible text or fixture calls, not raw ANSI snapshots. To watch the run, use `pnpm tui:pty:test:watch`; add `--mode local` or `--mode all` when needed.
