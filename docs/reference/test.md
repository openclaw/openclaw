...
Script: [`scripts/bench-model.ts`](https://github.com/sonpiaz/evox-sh/blob/main/scripts/bench-model.ts)
...
Script: [`scripts/bench-cli-startup.ts`](https://github.com/sonpiaz/evox-sh/blob/main/scripts/bench-cli-startup.ts)
...
This script drives the interactive wizard via a pseudo-tty, verifies config/workspace/session files, then starts the gateway and runs `evox health`.
