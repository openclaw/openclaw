...
- `OPENCLAW_HEAVY_CHECK_LOCK_SCOPE=common <local-heavy-check command>`: keeps heavy-check serialization inside the current Git common dir.
- `OPENCLAW_HEAVY_CHECK_LOCK_SCOPE=worktree <local-heavy-check command>`: keeps heavy-check serialization inside the current worktree. Use it only on high-capacity local hosts when you intentionally run independent checks across linked worktrees.
- By default, local heavy-check commands use one machine-wide lock so independent checkouts do not exhaust memory by running type-aware lint or tsgo at the same time.
