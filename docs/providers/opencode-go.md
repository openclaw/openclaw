...
```bash
evox onboard --auth-choice opencode-go
# or non-interactive
evox onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
...
EVOX.sh handles per-model routing automatically when the model ref uses `opencode-go/...`.
