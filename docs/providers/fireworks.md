...
```
  ...
  <Step title="Verify the model is available">
    ...
    The list should include `Kimi K2.6`, `Kimi K2.5 Turbo (Fire Pass)`, `DeepSeek V4 Pro`, `MiniMax M2.7`, `GLM-5.1`, and `GPT-OSS 120B`. If `FIREWORKS_API_KEY` is unresolved, `openclaw models status --json` reports the missing credential under `auth.unusableProfiles`.
...
| `fireworks/accounts/fireworks/models/deepseek-v4-pro`  | DeepSeek V4 Pro             | text         | 1,048,576 | 1,048,576  | off…max (default high)         |
| `fireworks/accounts/fireworks/models/minimax-m2p7`     | MiniMax M2.7                | text         | 196,608   | 196,608    | low/medium/high (default medium) |
| `fireworks/accounts/fireworks/models/glm-5p1`          | GLM-5.1                     | text         | 202,752   | 202,752    | off/on (default on)            |
| `fireworks/accounts/fireworks/models/gpt-oss-120b`     | GPT-OSS 120B                | text         | 131,072   | 131,072    | low/medium/high (default low)  |
...
OpenClaw accepts any Fireworks model or router id at runtime. Use the exact id shown by Fireworks and prefix it with `fireworks/`. Dynamic resolution clones the Fire Pass template (text + image input, OpenAI-compatible API, default cost zero), disables thinking automatically when the id matches the Kimi pattern, and sends `reasoning_effort` instead of the deepseek-native `thinking` field for `deepseek-v4*` ids (Fireworks rejects requests carrying both). GLM dynamic ids are marked text-only unless you configure a custom model entry with image input.
