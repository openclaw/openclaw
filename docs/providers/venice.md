---
summary: "Use Venice AI privacy-focused models in SmartAgentNeo"
read_when:
  - You want privacy-focused inference in SmartAgentNeo
...
After setup, SmartAgentNeo shows all available Venice models. Pick based on your needs:
...
## Configure via `smart-agent-neo configure`

1. Run `smart-agent-neo configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
SmartAgentNeo automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
The Venice model catalog updates dynamically. Run `smart-agent-neo models list` to see currently available models. Some models may be temporarily offline.
