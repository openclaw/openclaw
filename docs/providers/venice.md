---
summary: "Use Venice AI privacy-focused models in DNA"
read_when:
  - You want privacy-focused inference in DNA
...
```bash
dna onboard --auth-choice venice-api-key
```
...
```bash
dna chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, DNA shows all available Venice models. Pick based on your needs:
...
```bash
dna models set venice/claude-opus-45
dna models set venice/llama-3.3-70b
```

List all available models:

```bash
dna models list | grep venice
```

## Configure via `dna configure`
...
DNA automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
dna chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
dna chat --model venice/claude-opus-45

# Use uncensored model
dna chat --model venice/venice-uncensored

# Use vision model with image
dna chat --model venice/qwen3-vl-235b-a22b

# Use coding model
dna chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `dna models list` to see currently available models. Some models may be temporarily offline.
