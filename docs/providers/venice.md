---
summary: "Use Venice AI privacy-focused models in EVOX.sh"
read_when:
  - You want privacy-focused inference in EVOX.sh
...
```bash
evox onboard --auth-choice venice-api-key
```
...
```bash
evox agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, EVOX.sh shows all available Venice models. Pick based on your needs:
...
```bash
evox models set venice/kimi-k2-5
evox models set venice/claude-opus-4-6
```

List all available models:

```bash
evox models list | grep venice
```
...
EVOX.sh automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
evox agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
evox agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
evox agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
evox agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
evox agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `evox models list` to see currently available models. Some models may be temporarily offline.
