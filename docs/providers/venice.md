---
summary: "Use Venice AI privacy-focused models in Activi"
read_when:
  - You want privacy-focused inference in Activi
...
After setup, Activi shows all available Venice models. Pick based on your needs:
...
## Configure via `activi configure`

1. Run `activi configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Activi automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
The Venice model catalog updates dynamically. Run `activi models list` to see currently available models. Some models may be temporarily offline.
