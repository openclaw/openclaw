---
summary: "Use Synthetic's Anthropic-compatible API in EVOX.sh"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. EVOX.sh registers it as the
...
```bash
evox onboard --auth-choice synthetic-api-key
```
...
Note: EVOX.sh's Anthropic client appends `/v1` to the base URL, so use
