---
summary: "Use Synthetic's Anthropic-compatible API in DNA"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. DNA registers it as the
...
```bash
dna onboard --auth-choice synthetic-api-key
```
...
Note: DNA's Anthropic client appends `/v1` to the base URL, so use
