---
summary: "Use MiniMax M2.1 in Activi"
read_when:
  - You want MiniMax models in Activi
...
```bash
activi plugins enable minimax-portal-auth  # skip if already loaded.
activi gateway restart  # restart if gateway is already running
...
See [MiniMax OAuth plugin README](https://github.com/activi/activi/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `activi.json`:
...
- Use `activi models list` and `activi models set minimax/MiniMax-M2.1` to switch.
...
- Running `activi configure` and selecting **MiniMax M2.1**, or
