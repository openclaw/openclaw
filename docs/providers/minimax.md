---
summary: "Use MiniMax M2.1 in SmartAgentNeo"
read_when:
  - You want MiniMax models in SmartAgentNeo
...
```bash
smart-agent-neo plugins enable minimax-portal-auth  # skip if already loaded.
smart-agent-neo gateway restart  # restart if gateway is already running
...
See [MiniMax OAuth plugin README](https://github.com/betterbrand/smart-agent-neo/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `smart-agent-neo.json`:
...
- Use `smart-agent-neo models list` and `smart-agent-neo models set minimax/MiniMax-M2.1` to switch.
...
- Running `smart-agent-neo configure` and selecting **MiniMax M2.1**, or
