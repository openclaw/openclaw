...
<AccordionGroup>
  ...
  <Accordion title="Thinking defaults">
    ...
    MiniMax-M3 (and M3.x) is exempt — M3 emits proper Anthropic thinking blocks and returns an empty `content` array with `stop_reason: "end_turn"` when thinking is disabled, so the wrapper passes M3's thinking config through unchanged.
