---
summary: "Use MiniMax M2.1 in EasyHub"
read_when:
  - You want MiniMax models in EasyHub
...
```bash
EasyHub plugins enable minimax-portal-auth  # skip if already loaded.
EasyHub gateway restart  # restart if gateway is already running
...
See [MiniMax OAuth plugin README](https://github.com/EasyHub/EasyHub/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `easyhub.json`:
...
- Use `EasyHub models list` and `EasyHub models set minimax/MiniMax-M2.1` to switch.
...
- Running `EasyHub configure` and selecting **MiniMax M2.1**, or
