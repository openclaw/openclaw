---
summary: "Use Qwen OAuth (free tier) in EVOX.sh"
read_when:
  - You want to use Qwen with EVOX.sh
...
```bash
evox plugins enable qwen-portal-auth
```
...
```bash
evox models set qwen-portal/coder-model
```

## Reuse Qwen Code CLI login

If you already logged in with the Qwen Code CLI, EVOX.sh will sync credentials
