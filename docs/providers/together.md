...
read_when:
  - You want to use Together AI with SmartAgentNeo
...
```bash
smart-agent-neo onboard --auth-choice together-api-key
```
...
is available to that process (for example, in `~/.neobot/.env` or via
`env.shellEnv`).
