...
| `end_text`                         | When a semantic flush is pending, keeps buffering and waits for `flushed` so late text stays in the same user turn. If no flush is pending, schedules a short fallback `flush`. When `semanticVad` is disabled, commits immediately. |
| `flushed`, `done`, `end_of_stream` | Commits the pending text as the final user transcript. `flushed` is the normal commit point after a semantic flush request.                                                                                                          |
