---
summary: "Use xAI Grok models in Mullusi"
read_when:
  - You want to use Grok models in Mullusi
...
Mullusi ships a bundled `xai` provider plugin for Grok models.
...
```bash
mullusi onboard --auth-choice xai-api-key
```
...
Mullusi now uses the xAI Responses API as the bundled xAI transport. The same
...
Mullusi now includes these xAI model families out of the box:
...
```bash
mullusi config set tools.web.search.provider grok
```

## Known limits

- Auth is API-key only today. There is no xAI OAuth/device-code flow in Mullusi yet.
- `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the normal xAI provider path because it requires a different upstream API surface than the standard Mullusi xAI transport.

## Notes

- Mullusi applies xAI-specific tool-schema and tool-call compatibility fixes automatically on the shared runner path.
...
- `web_search`, `x_search`, and `code_execution` are exposed as Mullusi tools. Mullusi enables the specific xAI built-in it needs inside each tool request instead of attaching all native tools to every chat turn.
