...
OpenProse คือรูปแบบเวิร์กโฟลว์แบบพกพาที่ยึด Markdown เป็นหลัก สำหรับการควบคุมการทำงานของเซสชัน AI ใน OpenClaw จะมาในรูปแบบปลั๊กอินที่ติดตั้งแพ็ก Skills ของ OpenProse พร้อมคำสั่งสแลช `/prose` โปรแกรมจะอยู่ในไฟล์ `.prose` และสามารถสร้างซับเอเจนต์หลายตัวพร้อมการควบคุมโฟลว์อย่างชัดเจน
...
ปลั๊กอินที่มาพร้อมระบบจะถูกปิดใช้งานเป็นค่าเริ่มต้น ให้เปิดใช้งาน OpenProse:
...
OpenProse ลงทะเบียน `/prose` เป็นคำสั่ง Skills ที่ผู้ใช้เรียกได้ โดยจะส่งต่อไปยังคำสั่งของ OpenProse VM และใช้เครื่องมือของ OpenClaw เบื้องหลัง
...
URL แบบตรงจะถูกดึงตามที่เป็นอยู่ ใช้เครื่องมือ `web_fetch` (หรือ `exec` สำหรับ POST)
...
หาก allowlist ของเครื่องมือบล็อกเครื่องมือเหล่านี้ โปรแกรม OpenProse จะล้มเหลว ดู [Skills config](/tools/skills-config)

## ความปลอดภัยและการอนุมัติ

ปฏิบัติต่อไฟล์ `.prose` เสมือนโค้ด ตรวจทานก่อนรัน ใช้ allowlist ของเครื่องมือ OpenClaw และเกตการอนุมัติเพื่อควบคุมผลข้างเคียง
