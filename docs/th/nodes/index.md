...
- โหนดเป็น**อุปกรณ์ต่อพ่วง**ไม่ใช่เกตเวย์ พวกมันไม่รันบริการเกตเวย์
...
จะสร้างคำขอจับคู่อุปกรณ์สำหรับ `role: node` อนุมัติผ่านCLIของอุปกรณ์(หรือUI)
...
ใช้**โฮสต์โหนด**เมื่อGatewayรันอยู่บนเครื่องหนึ่งและคุณต้องการให้คำสั่งไปรันบนอีกเครื่องหนึ่ง โมเดลยังคงคุยกับ**gateway**; gateway
...
หากGatewayผูกกับloopback(`gateway.bind=loopback` ค่าเริ่มต้นในโหมดโลคัล) โฮสต์โหนดระยะไกลจะเชื่อมต่อโดยตรงไม่ได้ ให้สร้างอุโมงค์SSHแล้วชี้โฮสต์โหนดไปยังปลายโลคัลของอุโมงค์
...
การอนุมัติการรันเป็นแบบ**ต่อโฮสต์โหนด** เพิ่มรายการอนุญาตจากgateway:
...
โหนดเปิดเผย `screen.record`(mp4) ตัวอย่าง:
...
- บนmacOS โฮสต์โหนดแบบไม่มีหน้าจอจะเลือกใช้โฮสต์รันของแอปคู่หูเมื่อเข้าถึงได้ และจะถอยกลับไปใช้การรันในเครื่องหากแอปไม่พร้อมใช้งาน ตั้งค่า `OPENCLAW_NODE_EXEC_HOST=app` เพื่อบังคับใช้แอป หรือ `OPENCLAW_NODE_EXEC_FALLBACK=0` เพื่อปิดการถอยกลับ
