...
- ค่าเริ่มต้นของ `maxChars` สำหรับเสียงคือ **unset** (ทรานสคริปต์เต็ม) ตั้งค่า `tools.media.audio.maxChars` หรือ `maxChars` รายการต่อรายการเพื่อตัดเอาต์พุต
...
- กฎขอบเขตใช้หลักการจับคู่ครั้งแรกเป็นผู้ชนะ `chatType` จะถูกทำให้เป็นมาตรฐานเป็น `direct`, `group` หรือ `room`
