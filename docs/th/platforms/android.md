...
การควบคุมระบบ(launchd/systemd)อยู่บนโฮสต์Gateway ดูที่[Gateway](/gateway)
...
การค้นหาAndroid NSD/mDNSไม่ข้ามเครือข่าย หากโหนดAndroidและGatewayอยู่คนละเครือข่ายแต่เชื่อมต่อผ่านTailscale ให้ใช้Wide-Area Bonjour/unicast DNS-SDแทน:
...
- ปลายทางแบบกำหนดเอง(ถ้าเปิดใช้)หรือ
...
- `canvas.eval`, `canvas.snapshot`, `canvas.navigate`(ใช้`{"url":""}`หรือ`{"url":"/"}`เพื่อกลับสู่scaffoldเริ่มต้น)`canvas.snapshot`คืนค่า`{ format, base64 }`(ค่าเริ่มต้น`format="jpeg"`)
