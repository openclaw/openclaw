...
- **Push-to-talk (กดค้าง Option ขวา)**: กดค้างปุ่ม Option ขวาเพื่อเริ่มบันทึกทันที—ไม่ต้องใช้ทริกเกอร์ โอเวอร์เลย์จะแสดงระหว่างกดค้าง; เมื่อปล่อยจะสรุปและส่งต่อหลังดีเลย์สั้นๆ เพื่อให้ปรับข้อความได้

## พฤติกรรมขณะรัน (โหมดคำปลุก)

- ตัวรู้จำคำพูดทำงานอยู่ใน `VoiceWakeRuntime`.
- ทริกเกอร์จะทำงานก็ต่อเมื่อมี **ช่วงหยุดที่มีนัยสำคัญ** ระหว่างคำปลุกกับคำถัดไป (~ช่องว่าง 0.55วินาที) โอเวอร์เลย์/เสียงเตือนอาจเริ่มที่ช่วงหยุดได้แม้ก่อนคำสั่งจะเริ่ม
...
- **Hold Cmd+Fn to talk**: เปิดใช้งานตัวเฝ้าดู push-to-talk ปิดใช้งานบน macOS < 26
...
- **Sounds**: เสียงเตือนเมื่อพบทริกเกอร์และเมื่อส่ง ค่าเริ่มต้นคือเสียงระบบ macOS “Glass” คุณสามารถเลือกไฟล์ที่ `NSSound` โหลดได้ (เช่น MP3/WAV/AIFF) สำหรับแต่ละอีเวนต์ หรือเลือก **No Sound**
...
- คำตอบจะถูกส่งไปยัง **ผู้ให้บริการหลักที่ใช้ล่าสุด** (WhatsApp/Telegram/Discord/WebChat) หากการส่งล้มเหลว จะบันทึกข้อผิดพลาดและยังเห็นรันได้ผ่าน WebChat/บันทึกเซสชัน

## เพย์โหลดการส่งต่อ

- `VoiceWakeForwarder.prefixedTranscript(_:)` จะเติมคำใบ้ของเครื่องไว้ด้านหน้าก่อนส่ง ใช้ร่วมกันทั้งเส้นทางคำปลุกและ push-to-talk
