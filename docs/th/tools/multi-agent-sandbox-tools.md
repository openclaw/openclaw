...
สำหรับการดีบักว่า “ทำไมจึงถูกบล็อก?” ดู [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) และ `openclaw sandbox explain`
...
แต่ละระดับสามารถจำกัดเครื่องมือเพิ่มเติมได้ แต่ไม่สามารถคืนสิทธิ์เครื่องมือที่ถูกปฏิเสธจากระดับก่อนหน้าได้
หากตั้งค่า `agents.list[].tools.sandbox.tools` จะมาแทนที่ `tools.sandbox.tools` สำหรับเอเจนต์นั้น
หากตั้งค่า `agents.list[].tools.profile` จะมีลำดับเหนือ `tools.profile` สำหรับเอเจนต์นั้น
...
`tools.elevated` คือฐานส่วนกลาง (allowlistตามผู้ส่ง) โดย `agents.list[].tools.elevated` สามารถจำกัดเพิ่มเติมสำหรับเอเจนต์เฉพาะได้ (ทั้งสองต้องอนุญาต)
...
ให้ตั้งค่า `agents.list[].sandbox.mode: "off"`
