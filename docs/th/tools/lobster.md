...
ผู้ช่วยของคุณสามารถสร้างเครื่องมือที่จัดการตัวเองได้ ขอเวิร์กโฟลว์หนึ่งครั้ง และภายใน 30 นาทีคุณจะได้ CLI พร้อมไปป์ไลน์ที่รันได้ด้วยการเรียกเพียงครั้งเดียว Lobster คือชิ้นส่วนที่ขาดหายไป: ไปป์ไลน์ที่กำหนดผลลัพธ์ได้แน่นอน การอนุมัติที่ชัดเจน และสถานะที่กลับมาทำต่อได้

## Why

ปัจจุบัน เวิร์กโฟลว์ที่ซับซ้อนต้องเรียกเครื่องมือไปกลับหลายครั้ง การเรียกแต่ละครั้งใช้โทเคน และ LLM ต้องควบคุมทุกขั้นตอน Lobster ย้ายการควบคุมนั้นไปไว้ในรันไทม์แบบมีชนิด:
...
Lobster ถูกออกแบบให้เล็กโดยตั้งใจ เป้าหมายไม่ใช่ “ภาษาใหม่” แต่เป็นสเปกไปป์ไลน์ที่คาดเดาได้ เป็นมิตรกับ AI และมีการอนุมัติและโทเคนสำหรับทำต่อเป็นองค์ประกอบชั้นหนึ่ง
...
- **ยังเขียนโปรแกรมได้**: แต่ละขั้นสามารถเรียก CLI หรือสคริปต์ใดก็ได้ หากต้องการ JS/TS ให้สร้างไฟล์ `.lobster` จากโค้ด
...
สร้างคำสั่งเล็กๆ ที่สื่อสารด้วย JSON แล้วเชื่อมต่อเป็นการเรียก Lobster ครั้งเดียว (ชื่อตัวอย่างด้านล่าง — เปลี่ยนเป็นของคุณได้)
...
AI เป็นผู้กระตุ้นเวิร์กโฟลว์; Lobster รันขั้นตอนต่างๆ เกตการอนุมัติช่วยให้ผลข้างเคียงชัดเจนและตรวจสอบได้
...
`llm-task` แล้วเรียกจาก Lobster วิธีนี้ทำให้เวิร์กโฟลว์ยังคงกำหนดผลลัพธ์ได้แน่นอน ขณะเดียวกันก็ยังจัดประเภท/สรุป/ร่างด้วยโมเดลได้
...
Lobster สามารถรันไฟล์เวิร์กโฟลว์ YAML/JSON ที่มีฟิลด์ `name`, `args`, `steps`, `env`, `condition`, และ `approval` ในการเรียกเครื่องมือของ OpenClaw ให้ตั้งค่า `pipeline` เป็นพาธของไฟล์
...
หมายเหตุ: allowlist เป็นแบบ opt-in สำหรับปลั๊กอินเสริม หาก allowlist ของคุณระบุเฉพาะ
เครื่องมือปลั๊กอิน (เช่น `lobster`) OpenClaw จะยังคงเปิดใช้งานเครื่องมือแกนหลักไว้ หากต้องการจำกัดเครื่องมือแกนหลัก
ให้ใส่เครื่องมือหรือกลุ่มแกนหลักที่ต้องการลงใน allowlist ด้วย
...
เวิร์กโฟลว์เดียว กำหนดผลลัพธ์ได้แน่นอน ปลอดภัย
...
ใช้ `approve --preview-from-stdin --limit N` เพื่อแนบตัวอย่าง JSON ไปกับคำขออนุมัติโดยไม่ต้องใช้ jq/heredoc แบบกำหนดเอง โทเคนสำหรับทำต่อมีขนาดเล็กลงแล้ว: Lobster เก็บสถานะการทำต่อของเวิร์กโฟลว์ไว้ใต้ไดเรกทอรีสถานะของตน และส่งคืนคีย์โทเคนขนาดเล็ก

## OpenProse

OpenProse ทำงานคู่กับ Lobster ได้ดี: ใช้ `/prose` เพื่อจัดการเตรียมงานหลายเอเจนต์ จากนั้นรันไปป์ไลน์ Lobster เพื่อการอนุมัติที่กำหนดผลลัพธ์ได้แน่นอน หากโปรแกรม Prose ต้องใช้ Lobster ให้อนุญาตเครื่องมือ `lobster` สำหรับซับเอเจนต์ผ่าน `tools.subagents.tools` ดู [OpenProse](/prose)
