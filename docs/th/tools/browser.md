...
เบราว์เซอร์นี้ **ไม่ใช่** เบราว์เซอร์ใช้งานประจำวันของคุณ แต่เป็นพื้นผิวที่ปลอดภัยและแยกออกมา
...
- บริการควบคุมเบราว์เซอร์ผูกกับ loopback บนพอร์ตที่ได้มาจาก `gateway.port`
  (ค่าเริ่มต้น: `18791` ซึ่งคือ gateway + 2) ส่วน relay ใช้พอร์ตถัดไป (`18792`)
...
- โปรไฟล์เริ่มต้นคือ `chrome` (extension relay) ใช้ `defaultProfile: "openclaw"` สำหรับเบราว์เซอร์แบบจัดการ
...
OpenClaw จะใช้งานโดยอัตโนมัติ ตั้งค่า `browser.executablePath` เพื่อแทนที่การตรวจจับอัตโนมัติ:
...
- **Remote CDP:** ตั้งค่า `browser.profiles.<name>.cdpUrl` (หรือ `browser.cdpUrl`)
...
แทนการคอมมิตโทเคนลงในไฟล์คอนฟิก
...
OpenClaw รองรับหลายโปรไฟล์ที่ตั้งชื่อได้ (routing configs) โปรไฟล์สามารถเป็น:
...
`OPENCLAW_HOME_VOLUME` หรือ bind mount ดู [Docker](/install/docker)
...
- `browser act kind=evaluate` / `openclaw browser evaluate` และ `wait --fn`
  ...
  ปิดด้วย `browser.evaluateEnabled=false` หากไม่จำเป็น
