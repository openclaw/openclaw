...
Hooks มอบระบบแบบขยายได้ที่ขับเคลื่อนด้วยอีเวนต์สำหรับทำงานอัตโนมัติเพื่อตอบสนองต่อคำสั่งและอีเวนต์ของเอเจนต์ Hooks จะถูกค้นพบโดยอัตโนมัติจากไดเรกทอรี และสามารถจัดการได้ผ่านคำสั่ง CLI คล้ายกับวิธีที่ Skills ทำงานใน OpenClaw

## ทำความเข้าใจเบื้องต้น

Hooks คือสคริปต์ขนาดเล็กที่ทำงานเมื่อมีบางสิ่งเกิดขึ้น โดยมีอยู่สองประเภท:

- **Hooks** (หน้านี้): ทำงานภายใน Gateway เมื่อเกิดอีเวนต์ของเอเจนต์ เช่น `/new`, `/reset`, `/stop` หรืออีเวนต์วงจรชีวิต
- **Webhooks**: HTTP webhook ภายนอกที่เปิดให้ระบบอื่นเรียกใช้งานใน OpenClaw ดู [Webhook Hooks](/automation/webhook) หรือใช้ `openclaw webhooks` สำหรับคำสั่งผู้ช่วย Gmail
...
หากคุณเขียนฟังก์ชัน TypeScript เล็กๆได้ คุณก็สามารถเขียน hook ได้ Hooks จะถูกค้นพบโดยอัตโนมัติ และคุณสามารถเปิดหรือปิดการใช้งานผ่าน CLI
...
ระหว่างการเริ่มต้นใช้งาน (`openclaw onboard`) ระบบจะขอให้คุณเปิดใช้งาน hook ที่แนะนำ ตัวช่วยจะค้นหา hook ที่เข้าเกณฑ์โดยอัตโนมัติและแสดงให้เลือก
...
`package.json` ติดตั้งด้วยคำสั่ง:
...
- **`tool_result_persist`**: แปลงผลลัพธ์ของเครื่องมือก่อนเขียนลง transcript ของเซสชัน ต้องเป็น synchronous; ส่งคืน payload ผลลัพธ์ที่อัปเดตแล้ว หรือ `undefined` เพื่อคงค่าเดิม ดู [Agent Loop](/concepts/agent-loop)
...
**การย้ายระบบ**: สำหรับ hook ใหม่ แนะนำให้ใช้ระบบค้นพบแบบใหม่ Legacy handler จะถูกโหลดหลัง hook แบบไดเรกทอรี
...
Hooks ทำงานระหว่างการประมวลผลคำสั่ง ควรทำให้มีน้ำหนักเบา:
...
### จากคอนฟิกเดิมสู่การค้นพบแบบใหม่
