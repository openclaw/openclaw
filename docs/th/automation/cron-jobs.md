...
Cron คือระบบตั้งเวลาในตัวของ Gateway มันจัดเก็บงานไว้ ปลุกเอเจนต์ในเวลาที่เหมาะสม และสามารถส่งเอาต์พุตกลับไปยังแชตได้ตามต้องการ
...
API การเรียกเครื่องมือ cron สำหรับการเปลี่ยนแปลง
...
`deleteAfterRun: false` เพื่อเก็บไว้ (งานจะถูกปิดใช้งานหลังสำเร็จ)
...
งานจะถูกระบุด้วย `jobId` ที่คงที่ (ใช้โดย CLI/API ของ Gateway)
ในการเรียกเครื่องมือของเอเจนต์ `jobId` เป็นรูปแบบหลัก; แบบเดิม `id` ยังรองรับเพื่อความเข้ากันได้
งานแบบครั้งเดียวจะลบอัตโนมัติหลังสำเร็จเป็นค่าเริ่มต้น ตั้งค่า `deleteAfterRun: false` เพื่อเก็บไว้
...
นิพจน์ cron ใช้ `croner` หากไม่ระบุโซนเวลา จะใช้โซนเวลาท้องถิ่นของโฮสต์Gateway
...
เพื่อกำหนดเป้าหมายไปยังแชตแทน เมื่อเป็น `delivery.mode = "none"` จะไม่โพสต์สรุปไปยังเซสชันหลัก
...
หมายเหตุ: คุณสามารถตั้งค่า `model` กับงานเซสชันหลักได้เช่นกัน แต่จะเปลี่ยนโมเดลของเซสชันหลักที่ใช้ร่วมกัน เราแนะนำให้โอเวอร์ไรด์โมเดลเฉพาะงานแยกอิสระเพื่อหลีกเลี่ยงการเปลี่ยนบริบทโดยไม่คาดคิด
...
Telegram รองรับหัวข้อฟอรัมผ่าน `message_thread_id` สำหรับการส่งมอบด้วย cron คุณสามารถเข้ารหัสหัวข้อ/เธรดไว้ในฟิลด์ `to`:
