...
สถานะจะอยู่ภายใต้ไดเรกทอรีสถานะของ OpenClaw ค่าเริ่มต้นคือ `~/.openclaw` หากตั้งค่า `OPENCLAW_STATE_DIR` ให้ใช้ไดเรกทอรีนั้นแทน
...
หากคุณต้องการรีเซ็ตเฉพาะเซสชัน ให้ลบ `agents/<agentId>/sessions/` และ `agents/<agentId>/sessions.json` สำหรับเอเจนต์นั้น เก็บ `credentials/` ไว้หากคุณไม่ต้องการยืนยันตัวตนใหม่
