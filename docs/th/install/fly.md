...
- ข้อมูลประจำตัวของช่องทาง: โทเคนบอตDiscord, โทเคนTelegram ฯลฯ
...
**เคล็ดลับ:** เลือกภูมิภาคที่ใกล้คุณ ตัวเลือกที่พบบ่อย: `lhr`(ลอนดอน), `iad`(เวอร์จิเนีย), `sjc`(ซานโฮเซ)

## 2) กำหนดค่าfly.toml

แก้ไข`fly.toml`ให้ตรงกับชื่อแอปและความต้องการของคุณ

**หมายเหตุด้านความปลอดภัย:** คอนฟิกเริ่มต้นจะเปิดเผยURLสาธารณะ สำหรับการดีพลอยที่แข็งแกร่งโดยไม่มีIPสาธารณะ ดู[การดีพลอยแบบส่วนตัว](#private-deployment-hardened)หรือใช้`fly.private.toml`.
...
- **แนะนำให้ใช้ตัวแปรสภาพแวดล้อมแทนไฟล์คอนฟิก**สำหรับคีย์APIและโทเคนทั้งหมด เพื่อป้องกันไม่ให้Secretsอยู่ใน`openclaw.json`ซึ่งอาจถูกเปิดเผยหรือบันทึกโดยไม่ตั้งใจ
...
การดีพลอยครั้งแรกจะสร้างอิมเมจDocker(~2-3นาที) การดีพลอยครั้งถัดไปจะเร็วขึ้น
...
หากใช้ตัวแปรสภาพแวดล้อม ไม่จำเป็นต้องเพิ่มโทเคนในคอนฟิก Gatewayจะอ่าน`DISCORD_BOT_TOKEN`อัตโนมัติ
...
คอนเทนเนอร์รีสตาร์ตซ้ำหรือถูกฆ่า สัญญาณ: `SIGABRT`, `v8::internal::Runtime_AllocateInYoungGeneration`หรือรีสตาร์ตแบบเงียบ
...
**หมายเหตุ:** 512MBเล็กเกินไป 1GBอาจใช้ได้แต่มีโอกาสOOMเมื่อมีโหลดหรือบันทึกแบบละเอียด **แนะนำ2GB**
...
หากใช้`--allow-unconfigured`Gatewayจะสร้างคอนฟิกขั้นต่ำ คอนฟิกที่กำหนดเองของคุณที่`/data/openclaw.json`ควรถูกอ่านหลังรีสตาร์ต
...
คำสั่ง`fly ssh console -C`ไม่รองรับการเปลี่ยนเส้นทางเชลล์ เพื่อเขียนไฟล์คอนฟิก:
...
**หมายเหตุ:** หลังจาก`fly deploy`คำสั่งของเครื่องอาจรีเซ็ตกลับเป็นค่าที่อยู่ใน`fly.toml` หากคุณแก้ไขด้วยตนเอง ให้ปรับใช้ซ้ำหลังดีพลอย

## การดีพลอยแบบส่วนตัว(Hardened)

โดยค่าเริ่มต้น FlyจะจัดสรรIPสาธารณะ ทำให้Gatewayของคุณเข้าถึงได้ที่`https://your-app.fly.dev` ซึ่งสะดวกแต่ทำให้การดีพลอยของคุณถูกค้นพบโดยสแกนเนอร์อินเทอร์เน็ต(Shodan, Censys ฯลฯ)
...
หากต้องการการเรียกกลับของเว็บฮุค(Twilio, Telnyx ฯลฯ)โดยไม่เปิดเผยสาธารณะ:
...
ngrok tunnelทำงานภายในคอนเทนเนอร์และให้URLเว็บฮุคสาธารณะโดยไม่เปิดเผยแอปFlyเอง ตั้งค่า`webhookSecurity.allowedHosts`เป็นชื่อโฮสต์ของท่อทางสาธารณะเพื่อยอมรับส่วนหัวโฮสต์ที่ถูกส่งต่อ
