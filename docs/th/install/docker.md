...
Docker เป็นตัวเลือก**ไม่บังคับ** ใช้เฉพาะเมื่อคุณต้องการ Gateway แบบคอนเทนเนอร์หรือเพื่อตรวจสอบขั้นตอนการทำงานของ Docker
...
- **ไม่**: คุณรันบนเครื่องของตัวเองและต้องการรอบการพัฒนาที่เร็วที่สุด ให้ใช้ขั้นตอนการติดตั้งปกติแทน
- **หมายเหตุเรื่อง sandboxing**: agent sandboxing ก็ใช้ Docker เช่นกัน แต่**ไม่จำเป็น**ต้องรัน Gateway ทั้งหมดใน Docker ดู [Sandboxing](/gateway/sandboxing)
...
หมายเหตุ: รัน `docker compose ...` จากโฟลเดอร์รากของรีโป หากคุณเปิดใช้งาน
...
`openclaw-gateway` และ `openclaw-cli` โดยสร้างไฟล์ `docker-compose.extra.yml`
...
- `docker-compose.extra.yml` ถูกสร้างอัตโนมัติ อย่าแก้ไขด้วยมือ

### ทำให้ home ของคอนเทนเนอร์ทั้งหมดคงอยู่ (ไม่บังคับ)

หากคุณต้องการให้ `/home/node` คงอยู่แม้จะสร้างคอนเทนเนอร์ใหม่ ให้ตั้งค่า named
...
`node` ซึ่งช่วยลดพื้นที่โจมตี แต่หมายความว่า:
...
อิมเมจรันด้วย `node` (uid 1000) หากคุณพบข้อผิดพลาดด้านสิทธิ์บน
...
แล้ววางกลับเข้าไปในวิซาร์ดเพื่อจบการยืนยันตัวตน
...
- Dockerfile CMD ใช้ `--allow-unconfigured`; คอนฟิกที่เมานต์ด้วย `gateway.mode` ไม่ใช่ `local` จะยังเริ่มทำงานได้ ให้ override CMD หากต้องการบังคับใช้ guard
...
คอนเทนเนอร์ Docker Gateway ยังคงอยู่บนโฮสต์ แต่การรันเครื่องมือจะถูกแยกออก:
...
คำเตือน: `scope: "shared"` จะปิดการแยกระหว่างเซสชัน ทุกเซสชันใช้คอนเทนเนอร์และเวิร์กสเปซเดียวกัน
...
ช่วยให้คุณรันระดับการเข้าถึงที่หลากหลายใน Gateway เดียว:
...
- `user` ต้องเป็น root สำหรับ `apt-get` (ละ `user` หรือกำหนด `user: "0:0"`)
  ...
  จะบันทึกคำเตือนพร้อมคำสั่ง `openclaw sandbox recreate ...` ที่แน่นอน
...
จะสร้าง `openclaw-sandbox-common:bookworm-slim` วิธีใช้งาน:
...
จะสร้าง `openclaw-sandbox-browser:bookworm-slim` โดยใช้
...
- ไม่พบเครื่องมือกำหนดเอง: OpenClaw รันคำสั่งด้วย `sh -lc` (login shell)
