...
| `OPENCLAW_NO_PROMPT=1`                      | ปิดการถามแบบโต้ตอบ                              |
...
<Steps>
  <Step title="ติดตั้ง Node runtime ภายในเครื่อง">
...
<Tabs>
  ...
  <Tab title="กำหนด prefix + เวอร์ชันเอง">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://openclaw.ai/install-cli.sh | bash -s -- --prefix /opt/openclaw --version latest
    ```
  </Tab>
  <Tab title="เอาต์พุต JSON สำหรับอัตโนมัติ">
    ```bash
...
<Steps>
  <Step title="ตรวจสอบ PowerShell + สภาพแวดล้อม Windows">
...
<AccordionGroup>
  ...
  <Accordion title="ปัญหา sharp/libvips">
    สคริปต์ตั้งค่า `SHARP_IGNORE_GLOBAL_LIBVIPS=1` เป็นค่าเริ่มต้นเพื่อหลีกเลี่ยงการบิลด์ sharp กับ libvips ของระบบ หากต้องการ override:
