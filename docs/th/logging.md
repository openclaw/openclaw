...
แต่ละบรรทัดในไฟล์ล็อกเป็นออบเจ็กต์ JSON CLI และ Control UI จะพาร์สรายการเหล่านี้เพื่อแสดงผลลัพธ์แบบมีโครงสร้าง (เวลา ระดับ ระบบย่อย ข้อความ)
...
Diagnostics คือเหตุการณ์แบบมีโครงสร้างและอ่านได้ด้วยเครื่อง สำหรับการรันโมเดล **และ** เทเลเมทรีของการไหลของข้อความ (webhooks การเข้าคิว สถานะเซสชัน) โดย **ไม่ได้** ใช้แทนล็อก แต่มีไว้เพื่อป้อนข้อมูลให้เมตริก ทรซ และตัวส่งออกอื่นๆ
...
- **Logs**: ส่งออกผ่าน OTLP เมื่อเปิด `diagnostics.otel.logs` ปริมาณล็อกอาจสูง ควรคำนึงถึง `logging.level` และตัวกรองของตัวส่งออก
...
Diagnostics สามารถส่งออกผ่านปลั๊กอิน `diagnostics-otel` (OTLP/HTTP) ใช้ได้กับคอลเลกเตอร์/แบ็กเอนด์ OpenTelemetry ใดๆที่รับ OTLP/HTTP
...
- `protocol` รองรับเฉพาะ `http/protobuf` ในปัจจุบัน `grpc` จะถูกละเลย
...
- Traces/metrics สามารถเปิดปิดได้ด้วย `traces` / `metrics` (ค่าเริ่มต้น: เปิด) Traces รวมสแปนการใช้โมเดลและสแปนการประมวลผล webhook/ข้อความเมื่อเปิดใช้งาน
...
- เคารพ `logging.level` (ระดับไฟล์ล็อก) การปกปิดของคอนโซล **ไม่มีผล** กับล็อก OTLP
