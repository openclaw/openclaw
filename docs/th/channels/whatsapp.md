...
สถานะ: ใช้ WhatsApp Web ผ่าน Baileys เท่านั้น Gateway เป็นผู้ถือครองเซสชัน
...
WhatsApp ต้องการหมายเลขมือถือจริงเพื่อยืนยันตัวตน หมายเลขVoIPและหมายเลขเสมือนมักถูกบล็อก มีสองวิธีที่รองรับในการรันOpenClawบนWhatsApp:

### หมายเลขเฉพาะ(แนะนำ)

ใช้ **หมายเลขโทรศัพท์แยกต่างหาก** สำหรับOpenClaw ประสบการณ์ผู้ใช้ดีที่สุด การกำหนดเส้นทางสะอาด ไม่มีปัญหาแชตกับตัวเอง การตั้งค่าที่เหมาะสม: **โทรศัพท์Androidสำรอง/เก่า + eSIM** เปิดทิ้งไว้บนWi‑Fiและไฟ แล้วเชื่อมโยงผ่านคิวอาร์

**WhatsApp Business:** คุณสามารถใช้WhatsApp Businessบนอุปกรณ์เดียวกันด้วยหมายเลขอื่น เหมาะสำหรับแยกWhatsAppส่วนตัว—ติดตั้งWhatsApp BusinessและลงทะเบียนหมายเลขOpenClawที่นั่น
...
ทางลัด: รันOpenClawบน **หมายเลขของคุณเอง** ส่งข้อความถึงตัวเอง(WhatsApp “Message yourself”)เพื่อทดสอบเพื่อไม่สแปมรายชื่อ คาดว่าจะต้องอ่านรหัสยืนยันบนโทรศัพท์หลักระหว่างการตั้งค่าและทดลอง **ต้องเปิดโหมดแชตกับตัวเอง**
...
**เคล็ดลับ:** หมายเลขต้องรับSMSยืนยันเพียงครั้งเดียว หลังจากนั้นเซสชันWhatsApp Webจะคงอยู่ผ่าน `creds.json`.
...
ไม่ ค่าเริ่มต้นของนโยบายDMคือ **การจับคู่** ดังนั้นผู้ส่งที่ไม่รู้จักจะได้เพียงโค้ดจับคู่และข้อความของพวกเขา **จะไม่ถูกประมวลผล** OpenClawจะตอบเฉพาะแชตที่ได้รับ หรือการส่งที่คุณทริกเกอร์เอง(เอเจนต์/CLI)
...
**ทำไมวิซาร์ดจึงขอหมายเลขโทรศัพท์ของฉัน?**  
...
WhatsAppสามารถส่งรีแอ็กชันอีโมจิไปยังข้อความขาเข้าทันทีเมื่อรับ ก่อนที่บอตจะสร้างคำตอบ เพื่อให้ผู้ใช้ทราบทันทีว่าข้อความถูกรับแล้ว
...
- `emoji`(สตริง): อีโมจิสำหรับการตอบรับ(เช่น "👀","✅","📨") เว้นว่างหรือไม่ระบุ=ปิดฟีเจอร์
...
- ผลลัพธ์ดีที่สุด: OGG/Opus OpenClawจะเขียน `audio/ogg` ใหม่เป็น `audio/ogg; codecs=opus`
...
- `channels.whatsapp.allowFrom`(allowlistDM) WhatsAppใช้หมายเลขโทรศัพท์E.164(ไม่มีชื่อผู้ใช้)
...
- `channels.whatsapp.dmHistoryLimit`(ขีดจำกัดประวัติDMในจำนวนเทิร์นผู้ใช้) การแทนที่ต่อผู้ใช้: `channels.whatsapp.dms["<phone>"].historyLimit`
...
- วิธีแก้ไข: `openclaw doctor`(หรือรีสตาร์ตGateway) หากยังเป็นอยู่ ให้เชื่อมโยงใหม่ผ่าน `channels login` และตรวจสอบ `openclaw logs --follow`

**รันไทม์Bun**

- **ไม่แนะนำ** ให้ใช้Bun WhatsApp(Baileys)และTelegramทำงานไม่น่าเชื่อถือบนBun
  ให้รันGatewayด้วย **Node** (ดูหมายเหตุรันไทม์ในเริ่มต้นใช้งาน)
