...
รองรับแชท Twitch ผ่านการเชื่อมต่อ IRC โดย OpenClaw จะเชื่อมต่อในฐานะผู้ใช้ Twitch (บัญชีบอต) เพื่อรับและส่งข้อความในช่องทางต่างๆ
...
**⚠️ สำคัญ:** เพิ่มการควบคุมการเข้าถึง (`allowFrom` หรือ `allowedRoles`) เพื่อป้องกันผู้ใช้ที่ไม่ได้รับอนุญาตสั่งงานบอต ค่าเริ่มต้นของ `requireMention` คือ `true`.
...
ไม่จำเป็นต้องลงทะเบียนแอปด้วยตนเอง โทเคนจะหมดอายุหลังจากหลายชั่วโมง
...
แนะนำให้ใช้ `allowFrom` สำหรับ allowlist แบบเข้มงวด หากต้องการควบคุมการเข้าถึงตามบทบาท ให้ใช้ `allowedRoles` แทน

**บทบาทที่รองรับ:** `"moderator"`, `"owner"`, `"vip"`, `"subscriber"`, `"all"`.

**ทำไมต้องใช้ user ID?** ชื่อผู้ใช้สามารถเปลี่ยนได้ ทำให้เกิดการสวมรอยได้ แต่ user ID เป็นค่าถาวร
...
ใช้ `channels.twitch.accounts` พร้อมโทเคนแยกต่อบัญชี ดู [`gateway/configuration`](/gateway/configuration) สำหรับรูปแบบที่ใช้ร่วมกัน
...
โดยค่าเริ่มต้น `requireMention` คือ `true` หากต้องการปิดและให้ตอบทุกข้อความ:
