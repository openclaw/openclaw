...
สถานะ: ทดลองใช้งาน รองรับเฉพาะข้อความส่วนตัว; กลุ่มจะมาเร็วๆนี้ตามเอกสารของZalo
...
3. รีสตาร์ตGateway Zaloจะเริ่มทำงานเมื่อสามารถแก้ไขโทเคนได้(envหรือคอนฟิก)
4. การเข้าถึงDMค่าเริ่มต้นคือการจับคู่ อนุมัติรหัสเมื่อบอตถูกติดต่อครั้งแรก
...
- ค่าเริ่มต้น: `channels.zalo.dmPolicy = "pairing"` ผู้ส่งที่ไม่รู้จักจะได้รับรหัสจับคู่; ข้อความจะถูกเพิกเฉยจนกว่าจะอนุมัติ(รหัสหมดอายุหลัง1ชั่วโมง)
- อนุมัติผ่าน:
...
- โหมดเว็บฮุค: ตั้งค่า `channels.zalo.webhookUrl` และ `channels.zalo.webhookSecret`
  - ซีเคร็ตของเว็บฮุคต้องยาว8-256อักขระ
...
- ยืนยันว่าโทเคนลับยาว8-256อักขระ
...
- `channels.zalo.allowFrom`: allowlistสำหรับDM(รหัสผู้ใช้) `open` ต้องใช้ `"*"` ตัวช่วยจะถามรหัสตัวเลข
