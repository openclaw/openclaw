...
6. ช่องกิลด์: ใช้ `channel:<channelId>` สำหรับการส่งมอบ ต้องมีการกล่าวถึงโดยค่าเริ่มต้น และสามารถตั้งค่าเป็นรายกิลด์หรือรายช่องได้
...
หมายเหตุ: Slugเป็นตัวพิมพ์เล็กและแทนที่ช่องว่างด้วย `-`. ชื่อช่องจะถูกทำเป็นslugโดยไม่มี `#` นำหน้า
...
โดยทั่วไปคุณ **ไม่จำเป็น** ต้องใช้ **Presence Intent** การตั้งค่าสถานะของบอตเอง (การกระทำ `setPresence`) ใช้gateway OP3 และไม่ต้องใช้intentนี้; จำเป็นเฉพาะเมื่อคุณต้องการรับอัปเดตสถานะของสมาชิกกิลด์คนอื่น
...
รองรับหลายบัญชี: ใช้ `channels.discord.accounts` พร้อมโทเคนต่อบัญชีและ `name` (ไม่บังคับ) ดู [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) สำหรับรูปแบบที่ใช้ร่วมกัน
...
- เธรดจะสืบทอดคอนฟิกจากช่องแม่ (รายการอนุญาต, `requireMention`, skills, พรอมป์ต์ ฯลฯ) เว้นแต่คุณจะเพิ่มidช่องเธรดโดยตรง
- คำใบ้เจ้าของ: เมื่อรายการอนุญาต `users` ระดับกิลด์หรือระดับช่องตรงกับผู้ส่ง OpenClawจะถือว่าผู้ส่งนั้นเป็นเจ้าของในsystem prompt สำหรับเจ้าของแบบส่วนกลางข้ามช่อง ให้ตั้งค่า `commands.ownerAllowFrom`
...
- `requireMention` ต้องอยู่ภายใต้ `channels.discord.guilds` (หรือช่องเฉพาะ) `channels.discord.requireMention` ที่ระดับบนสุดจะถูกละเว้น
- **การตรวจสอบสิทธิ์** (`channels status --probe`) ตรวจสอบเฉพาะidช่องตัวเลข หากคุณใช้slug/ชื่อเป็นคีย์ `channels.discord.guilds.*.channels` การตรวจสอบจะยืนยันสิทธิ์ไม่ได้
...
- **การอนุมัติการรันคำสั่งในDiscord**: Discordรองรับ **UIปุ่ม** สำหรับการอนุมัติในDM (Allow once / Always allow / Deny) `/approve <id> ...` ใช้สำหรับการส่งต่อการอนุมัติเท่านั้นและจะไม่แก้ไขพรอมป์ต์ปุ่มของDiscord หากคุณเห็น `❌ Failed to submit approval: Error: unknown approval id` หรือUIไม่แสดง ตรวจสอบ:
...
- การตอบในกิลด์ถูกจำกัดด้วยการกล่าวถึงโดยค่าเริ่มต้นเพื่อหลีกเลี่ยงบอตรบกวน
...
การเรียกDiscord APIขาออกจะรีทรายเมื่อถูกจำกัดอัตรา (429) โดยใช้ `retry_after` ของDiscordเมื่อมี พร้อมการถอยหลังแบบเอ็กซ์โปเนนเชียลและjitter กำหนดค่าผ่าน `channels.discord.retry` ดู [Retry policy](/concepts/retry)
...
- `dm.policy`: การควบคุมการเข้าถึงDM (`pairing` แนะนำ) `"open"` ต้องใช้ `dm.allowFrom=["*"]`
- `dm.allowFrom`: รายการอนุญาตDM (user idหรือชื่อ) ใช้โดย `dm.policy="allowlist"` และสำหรับการตรวจสอบ `dm.policy="open"` ตัวช่วยตั้งค่ารับชื่อผู้ใช้และแปลงเป็นidเมื่อบอตค้นหาสมาชิกได้
...
- `guilds.<id>.channels.<channel>.systemPrompt`: system promptเพิ่มเติมสำหรับช่อง หัวข้อช่องDiscordจะถูกแทรกเป็นบริบท **ไม่เชื่อถือได้** (ไม่ใช่system prompt)
...
- `textChunkLimit`: ขนาดการแบ่งข้อความขาออก (อักขระ) ค่าเริ่มต้น: 2000
- `chunkMode`: `length` (ค่าเริ่มต้น) จะแบ่งเฉพาะเมื่อเกิน `textChunkLimit`; `newline` จะแบ่งตามบรรทัดว่างก่อนการแบ่งตามความยาว
- `maxLinesPerMessage`: จำนวนบรรทัดสูงสุดแบบนุ่มต่อข้อความ ค่าเริ่มต้น: 17
...
- `dmHistoryLimit`: ขีดจำกัดประวัติDMเป็นจำนวนเทิร์นผู้ใช้ การแทนที่รายผู้ใช้: `dms["<user_id>"].historyLimit`
...
- `execApprovals`: DMการอนุมัติการรันคำสั่งเฉพาะDiscord (UIปุ่ม) รองรับ `enabled`, `approvers`, `agentFilter`, `sessionFilter`
...
| emojiUploads   | enabled  | อัปโหลดอีโมจิ                      |
