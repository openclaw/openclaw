...
หน้านี้อธิบายพฤติกรรมCLIปัจจุบัน หากคำสั่งมีการเปลี่ยนแปลง ให้ปรับปรุงเอกสารนี้
...
- `muted` (#8B7F77): ลดการเน้น เมตาดาตา
...
การเปลี่ยนแปลงปลั๊กอินส่วนใหญ่ต้องรีสตาร์ทGateway ดู [/plugin](/tools/plugin)
...
ข้อความแชตรองรับคำสั่ง `/...` (ข้อความและเนทีฟ) ดู [/tools/slash-commands](/tools/slash-commands)
...
ตัวช่วยคอนฟิกแบบไม่โต้ตอบ (get/set/unset) การรัน `openclaw config` โดยไม่มีคำสั่งย่อยจะเปิดวิซาร์ด
...
การตั้งค่า+ตัวรัน Gmail Pub/Sub hook ดู [/automation/gmail-pubsub](/automation/gmail-pubsub)
...
ตัวช่วยDNSสำหรับการค้นหาในวงกว้าง (CoreDNS+Tailscale) ดู [/gateway/discovery](/gateway/discovery)
...
เพิ่มเอเจนต์แบบแยกใหม่ จะรันวิซาร์ดแบบแนะนำเว้นแต่ส่งแฟล็ก (หรือ `--non-interactive`); โหมดไม่โต้ตอบต้องใช้ `--workspace`
...
สเปกการผูกใช้ `channel[:accountId]` เมื่อไม่ระบุ `accountId` สำหรับWhatsApp จะใช้ idบัญชีค่าเริ่มต้น
...
- `gateway status` ยังแสดงบริการGatewayเดิมหรือเพิ่มเติมเมื่อสามารถตรวจพบได้ (`--deep` เพิ่มการสแกนระดับระบบ) บริการOpenClawที่ตั้งชื่อตามโปรไฟล์ถือเป็นชั้นหนึ่งและไม่ถูกทำเครื่องหมายว่า “เพิ่มเติม”
...
จัดการงานตามกำหนดเวลา (GatewayRPC) ดู [/automation/cron-jobs](/automation/cron-jobs)
...
`nodes` สื่อสารกับGatewayและกำหนดเป้าหมายโหนดที่จับคู่ ดู [/nodes](/nodes)
...
CLIควบคุมเบราว์เซอร์ (Chrome/Brave/Edge/Chromiumแบบเฉพาะ) ดู [`openclaw browser`](/cli/browser) และ [เครื่องมือBrowser](/tools/browser)
