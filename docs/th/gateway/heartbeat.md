...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T10:52:39Z
---
...
- ช่วงเวลา: `30m` (หรือ `1h` เมื่อโหมดการยืนยันตัวตนที่ตรวจพบคือ Anthropic OAuth/setup-token) ตั้งค่า `agents.defaults.heartbeat.every` หรือแบบต่อเอเจนต์ `agents.list[].heartbeat.every`; ใช้ `0m` เพื่อปิดใช้งาน
...
- PromptของHeartbeatจะถูกส่ง **ตามต้นฉบับ** เป็นข้อความผู้ใช้ โดยsystem prompt
  จะมีส่วน “Heartbeat” และการรันจะถูกทำเครื่องหมายภายใน
...
(ดังนั้นคุณสามารถตั้งค่าร่วมครั้งเดียวและแทนที่ต่อเอเจนต์ได้)
...
รอบถัดไปที่อยู่ในช่วงเวลาจะทำงานตามปกติ
...
- `accountId`: ตัวเลือกidบัญชีสำหรับช่องทางหลายบัญชี เมื่อ `target: "last"` idบัญชีจะถูกใช้กับช่องทางล่าสุดที่รองรับบัญชี; มิฉะนั้นจะถูกละเว้น หากidบัญชีไม่ตรงกับบัญชีที่คอนฟิกไว้สำหรับช่องทางนั้น การส่งจะถูกข้าม
- `prompt`: แทนที่เนื้อหาpromptเริ่มต้น(ไม่ถูกรวม)
- `ackMaxChars`: จำนวนอักขระสูงสุดที่อนุญาตหลัง `HEARTBEAT_OK` ก่อนส่ง
- `activeHours`: จำกัดการรันHeartbeatให้อยู่ในช่วงเวลา อ็อบเจ็กต์ที่มี `start` (HH:MM รวม), `end` (HH:MM ไม่รวม; อนุญาต `24:00` สำหรับสิ้นวัน), และตัวเลือก `timezone`
...
- Heartbeatจะรันในเซสชันหลักของเอเจนต์ตามค่าเริ่มต้น(`agent:<id>:<mainKey>`)
  ...
  เซสชันช่องทางเฉพาะ(Discord/WhatsApp/etc.)
- `session` มีผลเฉพาะบริบทการรัน; การส่งมอบถูกควบคุมโดย `target` และ `to`
- หากต้องการส่งไปยังช่องทาง/ผู้รับเฉพาะ ให้ตั้งค่า `target` + `to` พร้อมกับ
...
คุณสามารถปรับต่อช่องทางหรือบัญชีได้:
...
คิดซะว่าเป็น “เช็กลิสต์Heartbeat” ของคุณ: เล็ก คงที่ และปลอดภัยที่จะใส่ทุก30นาที
...
เมื่อเปิดใช้งาน Heartbeatจะส่งข้อความแยกต่างหากที่ขึ้นต้นด้วย
`Reasoning:` (รูปแบบเดียวกับ `/reasoning on`) สิ่งนี้มีประโยชน์เมื่อเอเจนต์
จัดการหลายเซสชัน/โคเด็กซ์และคุณอยากเห็นเหตุผลที่มันตัดสินใจติดต่อคุณ
แต่ก็อาจเปิดเผยรายละเอียดภายในมากกว่าที่ต้องการ ควรปิดไว้ในแชตกลุ่ม

## การตระหนักถึงค่าใช้จ่าย

Heartbeatรันagent turnsเต็มรูปแบบ ช่วงเวลาที่สั้นลงจะใช้โทเคนมากขึ้น
