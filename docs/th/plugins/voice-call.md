...
การสนทนาแบบหลายเทิร์นพร้อมนโยบายสำหรับสายเรียกเข้า
...
- `tunnel.allowNgrokFreeTierLoopbackBypass: true` อนุญาต webhook ของ Twilio ที่มีลายเซ็นไม่ถูกต้อง **เฉพาะเมื่อ** `tunnel.provider="ngrok"` และ `serve.bind` เป็น loopback (ngrok local agent) ใช้สำหรับ dev ภายในเครื่องเท่านั้น
- URL ของ ngrok ระดับฟรีอาจเปลี่ยนหรือเพิ่มพฤติกรรมคั่นกลาง; หาก `publicUrl` เปลี่ยน Twilio signature จะล้มเหลว สำหรับโปรดักชัน แนะนำโดเมนที่เสถียรหรือ Tailscale funnel
...
นโยบายสายเรียกเข้าเริ่มต้นเป็น `disabled` หากต้องการเปิดใช้งานสายเรียกเข้า ให้ตั้งค่า:
