...
OpenClaw สร้าง system prompt แบบกำหนดเองสำหรับการรันเอเจนต์ทุกครั้ง โดย prompt นี้เป็น **ของ OpenClaw** และไม่ใช้ default prompt ของ p-coding-agent
...
การ์ดเรลด้านความปลอดภัยใน system prompt เป็นเชิงแนะนำ ใช้เพื่อชี้นำพฤติกรรมของโมเดลแต่ไม่บังคับใช้นโยบาย การบังคับใช้แบบเข้มควรใช้ tool policy, การอนุมัติการรันคำสั่ง, sandboxing และ channel allowlists; ผู้ดูแลสามารถปิดสิ่งเหล่านี้ได้ตามการออกแบบ

## โหมดของ prompt

OpenClaw สามารถเรนเดอร์ system prompt ที่เล็กลงสำหรับซับเอเจนต์ รันไทม์จะตั้งค่า
...
- `minimal`: ใช้สำหรับซับเอเจนต์; ตัด **Skills**, **Memory Recall**, **OpenClaw
  ...
  **Messaging**, **Silent Replies** และ **Heartbeats** ออก โดยยังคงมี Tooling, **Safety**,
  Workspace, Sandbox, Current Date & Time (เมื่อทราบ), Runtime และบริบทที่แทรกไว้
...
ไฟล์ขนาดใหญ่จะถูกตัดทอนพร้อมตัวบ่งชี้ ขนาดสูงสุดต่อไฟล์ถูกควบคุมโดย
`agents.defaults.bootstrapMaxChars` (ค่าเริ่มต้น: 20000) ไฟล์ที่หายไปจะถูกแทรกด้วย
ตัวบ่งชี้ไฟล์หายแบบสั้น
...
เพื่อดูว่าไฟล์ที่แทรกแต่ละไฟล์มีส่วนต่อบริบทเท่าใด (ดิบ vs ที่แทรก, การตัดทอน และโอเวอร์เฮดของสคีมาเครื่องมือ) ให้ใช้ `/context list` หรือ `/context detail` ดู [Context](/concepts/context)

## การจัดการเวลา

system prompt จะมีส่วน **Current Date & Time** โดยเฉพาะเมื่อทราบโซนเวลาของผู้ใช้ เพื่อให้แคชของ prompt มีความเสถียร ขณะนี้จะรวมเฉพาะ
...
(ถามผู้ใช้เฉพาะเมื่อไม่สามารถเข้าถึงได้)
