...
- การ override ระดับช่องทาง (`channels.whatsapp.*`, `channels.telegram.*` ฯลฯ) สำหรับขีดจำกัดและสวิตช์การสตรีม

ดู [Configuration](/gateway/configuration) สำหรับสคีมาฉบับเต็ม

## Inbound dedupe

ช่องทางอาจส่งข้อความเดิมซ้ำหลังจากการเชื่อมต่อใหม่ OpenClaw จะเก็บแคชระยะสั้น
...
และใช้ข้อความล่าสุดสำหรับการผูกเธรดการตอบกลับ/ID
...
หลายอุปกรณ์/ช่องทางสามารถแมปไปยังเซสชันเดียวกันได้ แต่ประวัติจะไม่ถูกซิงก์กลับไปยังทุกไคลเอนต์อย่างสมบูรณ์
คำแนะนำ: ใช้อุปกรณ์หลักเพียงหนึ่งเครื่องสำหรับบทสนทนายาว เพื่อหลีกเลี่ยงบริบทที่แยกจากกัน
Control UI และ TUI จะแสดงทรานสคริปต์เซสชันที่อ้างอิงจาก Gateway เสมอ จึงเป็นแหล่งข้อมูลที่เชื่อถือได้
...
- `Body`: ข้อความ prompt ที่ส่งไปยังเอเจนต์ ซึ่งอาจรวมซองข้อความของช่องทางและ
  wrapper ประวัติแบบไม่บังคับ
...
มีความสอดคล้องกันใน prompt ของเอเจนต์
