...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T10:52:19Z
---
...
TypeBox คือไลบรารีสคีมาที่เน้น TypeScript เป็นหลัก เราใช้มันเพื่อกำหนด **โปรโตคอล WebSocket ของGateway** (แฮนด์เชค, คำขอ/คำตอบ, อีเวนต์จากเซิร์ฟเวอร์) สคีมาเหล่านี้ขับเคลื่อน **การตรวจสอบที่รันไทม์**, **การส่งออก JSON Schema** และ **การสร้างโค้ด Swift** สำหรับแอปmacOS แหล่งความจริงเดียว; อย่างอื่นทั้งหมดถูกสร้างขึ้นอัตโนมัติ
...
เฟรมแรก **ต้อง** เป็นคำขอ `connect` หลังจากนั้นไคลเอนต์สามารถเรียกเมธอด (เช่น `health`, `send`, `chat.send`) และสมัครรับอีเวนต์ (เช่น
`presence`, `tick`, `agent`)
...
| Sessions  | `sessions.list`, `sessions.patch`, `sessions.delete`      | การดูแลเซสชัน                         |
...
- **ฝั่งเซิร์ฟเวอร์**: ทุกเฟรมขาเข้าถูกตรวจสอบด้วย AJV แฮนด์เชครับเฉพาะคำขอ `connect` ที่พารามิเตอร์ตรงกับ `ConnectParams`
...
JSON Schema ที่สร้างแล้วอยู่ในรีโปที่ `dist/protocol.schema.json` โดยไฟล์ดิบที่เผยแพร่มักจะอยู่ที่:
