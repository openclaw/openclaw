...
เป้าหมาย: มีเอเจนต์ที่ _แยกจากกัน_ หลายตัว(เวิร์กสเปซ+`agentDir`+เซสชันแยก)พร้อมบัญชีช่องทางหลายบัญชี(เช่น WhatsApp สองบัญชี)ภายในGatewayที่กำลังรันหนึ่งตัว ขาเข้าจะถูกกำหนดเส้นทางไปยังเอเจนต์ผ่านการผูก(binding) Inbound is routed to an agent via bindings.
...
โปรไฟล์การยืนยันตัวตนเป็นแบบ**ต่อเอเจนต์** เอเจนต์แต่ละตัวจะอ่านจากของตัวเอง: Each agent reads from its own:

```
~/.openclaw/agents/<agentId>/agent/auth-profiles.json
```

Main agent credentials are **not** shared automatically. 1. ห้ามใช้ `agentDir` ซ้ำ
ระหว่างเอเจนต์ (จะทำให้เกิดการชนกันของการยืนยันตัวตน/เซสชัน) ข้อมูลรับรองของเอเจนต์หลักจะ**ไม่**ถูกแชร์โดยอัตโนมัติ ห้ามนำ`agentDir`มาใช้ซ้ำ
...
Skills เป็นแบบต่อเอเจนต์ผ่านโฟลเดอร์`skills/`ของแต่ละเวิร์กสเปซ โดยมี Skills ที่ใช้ร่วมกันได้จาก`~/.openclaw/skills` ดู[Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) 2. ดู [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills)

Gatewayสามารถโฮสต์**เอเจนต์เดียว**(ค่าเริ่มต้น)หรือ**หลายเอเจนต์**แบบขนานกันได้

**หมายเหตุเวิร์กสเปซ:** เวิร์กสเปซของแต่ละเอเจนต์เป็น**cwd ค่าเริ่มต้น**ไม่ใช่sandboxแบบเข้มงวด เส้นทางสัมพัทธ์จะอ้างอิงภายในเวิร์กสเปซ แต่เส้นทางสัมบูรณ์สามารถเข้าถึงตำแหน่งอื่นของโฮสต์ได้ เว้นแต่จะเปิดsandboxing ดู[Sandboxing](/gateway/sandboxing) 3. พาธแบบสัมพัทธ์จะอ้างอิงภายใน workspace แต่พาธแบบสัมบูรณ์สามารถ
เข้าถึงตำแหน่งอื่นบนโฮสต์ได้ เว้นแต่จะเปิดใช้งาน sandboxing 4. ดู
...
7. ช่องทางที่รองรับ **หลายบัญชี** (เช่น WhatsApp) ใช้ `accountId` เพื่อระบุ
   การล็อกอินแต่ละครั้ง ช่องทางที่รองรับ**หลายบัญชี**(เช่น WhatsApp)จะใช้`accountId`เพื่อระบุการล็อกอินแต่ละครั้ง `accountId`แต่ละตัวสามารถกำหนดเส้นทางไปยังเอเจนต์ต่างกันได้ ทำให้เซิร์ฟเวอร์เดียวโฮสต์หลายหมายเลขได้โดยไม่ปะปนเซสชัน
...
- 8. รายการอนุญาต/ปฏิเสธของเครื่องมือเป็น **tools** ไม่ใช่ skills รายการอนุญาต/ปฏิเสธเครื่องมือเป็น**เครื่องมือ**ไม่ใช่Skills หากSkillต้องรันไบนารี ให้แน่ใจว่าอนุญาต`exec`และมีไบนารีอยู่ในsandbox
...
9. หมายเหตุ: `setupCommand` อยู่ภายใต้ `sandbox.docker` และรันเพียงครั้งเดียวเมื่อสร้างคอนเทนเนอร์
...
10. หมายเหตุ: `tools.elevated` เป็น **แบบโกลบอล** และอิงตามผู้ส่ง; ไม่สามารถกำหนดค่าแยกตามเอเจนต์ได้
11. หากต้องการขอบเขตแยกตามเอเจนต์ ให้ใช้ `agents.list[].tools` เพื่อปฏิเสธ `exec`
