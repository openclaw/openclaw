...
Session pruning จะตัด **ผลลัพธ์เครื่องมือเก่า** ออกจากบริบทในหน่วยความจำก่อนการเรียก LLM แต่ละครั้ง โดย **ไม่** เขียนทับประวัติเซสชันที่บันทึกบนดิสก์ (`*.jsonl`).
...
- **ทำไมต้องตัดแต่ง:** การแคชพรอมป์ของ Anthropic ใช้ได้เฉพาะภายใน TTL หากเซสชันว่างเกิน TTL คำขอถัดไปจะต้องแคชพรอมป์ทั้งหมดใหม่ เว้นแต่คุณจะตัดแต่งก่อน.
...
การตัดแต่งใช้การประมาณขนาดหน้าต่างบริบท (ตัวอักษร ≈ โทเคน × 4) โดยหน้าต่างฐานจะถูกกำหนดตามลำดับนี้:
...
- Deny มีผลเหนือกว่า.
- การจับคู่ไม่สนใจตัวพิมพ์เล็กใหญ่.
...
- Compaction เป็นคนละเรื่อง: compaction จะสรุปและบันทึกถาวร ส่วน pruning เป็นแบบชั่วคราวต่อคำขอ ดู [/concepts/compaction](/concepts/compaction).
