---
summary: "SmartAgentNeo on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up SmartAgentNeo on a Raspberry Pi
  - Running SmartAgentNeo on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# SmartAgentNeo on Raspberry Pi

## Goal

Run a persistent, always-on SmartAgentNeo Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most SmartAgentNeo features work on ARM64, but some external binaries may need ARM builds:
...
# Common fix: rebuild
cd ~/smart-agent-neo  # if using hackable install
npm run build
sudo systemctl restart smart-agent-neo
```
