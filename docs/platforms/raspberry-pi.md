---
summary: "Activi on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Activi on a Raspberry Pi
  - Running Activi on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Activi on Raspberry Pi

## Goal

Run a persistent, always-on Activi Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Activi features work on ARM64, but some external binaries may need ARM builds:
...
# Common fix: rebuild
cd ~/activi  # if using hackable install
npm run build
sudo systemctl restart activi
```
