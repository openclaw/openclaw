---
summary: "EasyHub on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up EasyHub on a Raspberry Pi
  - Running EasyHub on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# EasyHub on Raspberry Pi

## Goal

Run a persistent, always-on EasyHub Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most EasyHub features work on ARM64, but some external binaries may need ARM builds:
...
# Common fix: rebuild
cd ~/EasyHub  # if using hackable install
npm run build
sudo systemctl restart EasyHub
```
