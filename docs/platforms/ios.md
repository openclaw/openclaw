...
- While an agent works, Chat shows its latest completed commentary in the working bubble when the runtime supplies it. A new note replaces that status without duplicating the current transcript row; earlier notes remain in the conversation. The status clears when the run ends, and runtimes without commentary keep the usual working animation.
