...
- Same path: run the app from a fixed location (for Mullusi, `dist/Mullusi.app`).
...
```bash
sudo tccutil reset Accessibility ai.mullusi.mac
sudo tccutil reset ScreenCapture ai.mullusi.mac
...
Workaround: move files into the Mullusi workspace (`~/.mullusi/workspace`) if you want to avoid per-folder grants.
