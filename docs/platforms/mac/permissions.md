...
- Same path: run the app from a fixed location (for EVOX.sh, `dist/EVOX.sh.app`).
...
Workaround: move files into the EVOX.sh workspace (`~/.openclaw/workspace`) if you want to avoid per-folder grants.
