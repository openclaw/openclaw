...
| Dashboard works but Mac capabilities are offline | The operator/control connection is healthy, but the companion node connection is not connected or is missing its command surface. Open the menu bar device section and check whether the Mac is `paired · disconnected`. After an upgrade, the app automatically reissues a missing Gateway-scoped node credential when the operator and node use the same paired device identity; SecretRef-backed Gateway credentials remain inside the Gateway. If that guarded repair fails, choose **Repair Mac node** from the menu bar. Distinct node identities still require separate pairing. Direct `wss://` operator and node connections use the same configured or stored certificate policy. For trusted `wss://*.ts.net` Tailscale Serve endpoints, stale stored leaf pins are replaced after certificate rotation and retried automatically. Configured pins never rotate automatically; update `gateway.remote.tlsFingerprint` after reviewing the new certificate, or switch to **Remote over SSH**. |
