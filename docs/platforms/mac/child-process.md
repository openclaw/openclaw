...
service via the external `mullusi` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.mullusi.gateway`
  (or `ai.mullusi.<profile>` when using `--profile`/`MULLUSI_PROFILE`; legacy `com.mullusi.*` is supported).
...
```bash
launchctl kickstart -k gui/$UID/ai.mullusi.gateway
launchctl bootout gui/$UID/ai.mullusi.gateway
```

Replace the label with `ai.mullusi.<profile>` when running a named profile.
