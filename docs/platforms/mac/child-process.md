...
service via the external `EasyHub` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`EASYHUB_PROFILE`; legacy `com.EasyHub.*` is supported).
