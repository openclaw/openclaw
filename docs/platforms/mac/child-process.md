...
service via the external `dna` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`DNA_PROFILE`; legacy `com.dna.*` is supported).
