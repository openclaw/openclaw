...
service via the external `activi` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`ACTIVI_PROFILE`; legacy `com.activi.*` is supported).
