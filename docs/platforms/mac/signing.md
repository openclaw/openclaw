...
- `SIGN_IDENTITY` also accepts a certificate SHA-1 hash (`export SIGN_IDENTITY=<40-hex>`), which is how you disambiguate two certificates that share a common name. A hash and a name that select the same certificate sign identically.
