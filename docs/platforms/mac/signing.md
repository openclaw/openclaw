...
- `SIGN_IDENTITY` also accepts a certificate SHA-1 hash (`export SIGN_IDENTITY="7BB27EA163FB4783A0EBEB579DC66FBBACE96453"`), which is how you disambiguate two certificates that share a common name. A hash and a name that select the same certificate sign identically.
- `CODESIGN_TIMESTAMP=auto` (default) enables trusted timestamps only for Developer ID Application signatures, whether that certificate is selected by name or by hash. Set `on`/`off` to force either way.
