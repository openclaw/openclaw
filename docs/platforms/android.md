...
- Reading: scrolling up or using **View all**, **Start of code**, or **End of code** pauses automatic following. Incoming content and window-size changes preserve your reading position, moving text or images into view when space shrinks. Resizing an idle conversation so that all content fits resumes following. **Jump to latest** floats above the composer while newer content is below the visible history and resumes following when tapped. It hides if the pane is too short to fit the action above the composer.
