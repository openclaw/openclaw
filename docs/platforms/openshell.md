...
#
#    On recent OpenShell community images, the sandbox runtime already
#    injects `SSL_CERT_FILE=/etc/openshell-tls/ca-bundle.pem` (along with
#    CURL_CA_BUNDLE, REQUESTS_CA_BUNDLE, NODE_EXTRA_CA_CERTS) pointing at a
#    pre-combined bundle (system roots + OpenShell CA). For that case, the
#    block below is a no-op — `SSL_CERT_FILE` is already correct and
#    persistent at a non-`/tmp` path.
#
#    The rebuild is kept as a defensive fallback for custom sandbox images
#    that skip the env injection or put the OpenShell CA at a different
#    path. It self-heals `/tmp/combined-ca.pem` if it is missing after a
#    sandbox/pod restart and only overrides `SSL_CERT_FILE` when the
#    runtime did not already set it.
if [ -z "${SSL_CERT_FILE:-}" ] && [ -f /etc/ssl/certs/ca-certificates.crt ] && [ -f /opt/openshell/ca.pem ]; then
...
**The typical case — recent community images**: check your sandbox environment first.

```bash
$ env | grep -iE 'SSL_CERT|CURL_CA|NODE_EXTRA|REQUESTS_CA'
...
If all four are set and `/etc/openshell-tls/ca-bundle.pem` exists
(it's a pre-combined bundle with system roots plus the OpenShell CA, so
`SSL_CERT_FILE` is already correct and persistent), you do **not** need
the manual rebuild below. Go binaries, Node, Python `requests`, and
curl will pick up the injected bundle automatically. Note that `git`
remains a separate problem — see `GIT_SSL_CAINFO` in the troubleshooting
section below.

**The fallback — custom images or older runtimes**: if the env vars are
missing or `/etc/openshell-tls/ca-bundle.pem` is not present, combine
the system store with the OpenShell CA yourself:
...
`/tmp` is intentionally ephemeral in an OpenShell sandbox, so the
fallback block in `start-gateway.sh` (step 2) regenerates the bundle on
every launcher run — but only when the runtime has not already set
`SSL_CERT_FILE`. If you prefer a persistent location under your
control, write to `/sandbox/combined-ca.pem` instead; any change to the
