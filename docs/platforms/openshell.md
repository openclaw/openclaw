...
(filename glob, not literal), and safe to run on every start. Remove it
...
fetch guard or a custom tool). Confirm with:

```bash
openshell sandbox exec --name agent -- sh -lc 'getent hosts api.openai.com'
```

If this fails, the proxy cannot see the hostname from its egress view —
add it to the network policy, not to `/etc/hosts`.
