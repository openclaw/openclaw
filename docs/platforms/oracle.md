---
summary: "Mullusi on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up Mullusi on Oracle Cloud
  - Looking for low-cost VPS hosting for Mullusi
  - Want 24/7 Mullusi on a small server
title: "Oracle Cloud (Platform)"
---

# Mullusi on Oracle Cloud (OCI)

## Goal

Run a persistent Mullusi Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for Mullusi (especially if you already have an OCI account), but it comes with tradeoffs:
...
This enables Tailscale SSH, so you can connect via `ssh mullusi` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@mullusi` (or use the Tailscale IP).
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `mullusi security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.mullusi`
- **Security audit:** `mullusi security audit`
...
journalctl --user -u mullusi-gateway.service -n 50
```
...
- `~/.mullusi/workspace/` — workspace (SOUL.md, memory, artifacts)
