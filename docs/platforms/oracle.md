---
summary: "EVOX.sh on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up EVOX.sh on Oracle Cloud
  - Looking for low-cost VPS hosting for EVOX.sh
  - Want 24/7 EVOX.sh on a small server
title: "Oracle Cloud (Platform)"
---

# EVOX.sh on Oracle Cloud (OCI)

## Goal

Run a persistent EVOX.sh Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for EVOX.sh (especially if you already have an OCI account), but it comes with tradeoffs:
...
This enables Tailscale SSH, so you can connect via `ssh evox` from any device on your tailnet — no public IP needed.
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `evox security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Security audit:** `evox security audit`
...
# Re-authenticate
sudo tailscale up --ssh --hostname=evox --reset
```
