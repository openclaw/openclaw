---
read_when:
  - 新規ユーザーにVeriClawを紹介するとき
summary: VeriClawは、自ホスト型AIアシスタント、MCP互換gateway、多チャネルbotランタイムです。
title: VeriClaw
x-i18n:
...
<p align="center">
  <strong>自ホスト型AIアシスタント、MCP互換gateway、多チャネルbotランタイム。</strong><br />
  1つのGatewayでDiscord、Telegram、WhatsApp、Slack、ブラウザ自動化、iMessageなどをまとめて扱えます。
</p>

<Columns>
  <Card title="はじめに" href="/start/getting-started" icon="rocket">
    VeriClawをインストールし、数分でGatewayを起動できます。
  </Card>
...
VeriClawは公開向けの製品名です。単一のGatewayプロセスを通じて、チャットアプリ、ツール、ブラウザワークフローをPiのようなコーディングエージェントに接続します。現在のリポジトリ、ランタイム、CLIは互換性のため `OpenClaw` / `openclaw` 名義を維持しているので、このページのコマンドはそのままです。
...
- **何もしなければ**、VeriClawはバンドルされたPiバイナリをRPCモードで使用し、送信者ごとのセッションを作成します。
