---
read_when:
  - 新規ユーザーにSmartAgentNeoを紹介するとき
summary: SmartAgentNeoは、あらゆるOSで動作するAIエージェント向けのマルチチャネルgatewayです。
title: SmartAgentNeo
x-i18n:
...
<Columns>
  <Card title="はじめに" href="/start/getting-started" icon="rocket">
    SmartAgentNeoをインストールし、数分でGatewayを起動できます。
  </Card>
  <Card title="ウィザードを実行" href="/start/wizard" icon="sparkles">
    `smart-agent-neo onboard`とペアリングフローによるガイド付きセットアップ。
  </Card>
...
SmartAgentNeoは、単一のGatewayプロセスを通じてチャットアプリをPiのようなコーディングエージェントに接続します。SmartAgentNeoアシスタントを駆動し、ローカルまたはリモートのセットアップをサポートします。
...
<Steps>
  <Step title="SmartAgentNeoをインストール">
    ```bash
...
設定は`~/.smart-agent-neo/smart-agent-neo.json`にあります。

- **何もしなければ**、SmartAgentNeoはバンドルされたPiバイナリをRPCモードで使用し、送信者ごとのセッションを作成します。
