...
<AccordionGroup>
  <Accordion title="Rules">
    ...
    - Gateway/WebChat video uploads (MP4/M4V/MOV/WebM) are sent as native model media when the active chat provider/model advertises `video` input. If it does not, OpenClaw stages them as managed `MediaPaths`/`MediaTypes`, so video media-understanding can describe them.
