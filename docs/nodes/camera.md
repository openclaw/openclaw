...
Mullusi supports **camera capture** for agent workflows:
...
```bash
mullusi nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
mullusi nodes camera snap --node <id> --facing front
mullusi nodes camera clip --node <id> --duration 3000
mullusi nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`mullusi.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `mullusi` CLI to invoke camera commands on the macOS node.

Examples:

```bash
mullusi nodes camera list --node <id>            # list camera ids
mullusi nodes camera snap --node <id>            # prints MEDIA:<path>
mullusi nodes camera snap --node <id> --max-width 1280
mullusi nodes camera snap --node <id> --delay-ms 2000
mullusi nodes camera snap --node <id> --device-id <id>
mullusi nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
mullusi nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
mullusi nodes camera clip --node <id> --device-id <id>
mullusi nodes camera clip --node <id> --no-audio
```

Notes:

- `mullusi nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
mullusi nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
