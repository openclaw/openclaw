...
EVOX.sh supports **camera capture** for agent workflows:
...
```bash
evox nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
evox nodes camera snap --node <id> --facing front
evox nodes camera clip --node <id> --duration 3000
evox nodes camera clip --node <id> --no-audio
```
...
Use the main `evox` CLI to invoke camera commands on the macOS node.

Examples:

```bash
evox nodes camera list --node <id>            # list camera ids
evox nodes camera snap --node <id>            # prints MEDIA:<path>
evox nodes camera snap --node <id> --max-width 1280
evox nodes camera snap --node <id> --delay-ms 2000
evox nodes camera snap --node <id> --device-id <id>
evox nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
evox nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
evox nodes camera clip --node <id> --device-id <id>
evox nodes camera clip --node <id> --no-audio
```

Notes:

- `evox nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
evox nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
