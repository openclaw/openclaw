...
`realtime.providers.openai.apiKey` must be a direct OpenAI Platform key for OpenAI Realtime against `api.openai.com`, not an Azure AI Foundry / Azure OpenAI model key or OpenAI-compatible proxy key. Use `gateway-relay` with `azureEndpoint` + `azureDeployment` for Azure Realtime, or an endpoint-only `azureEndpoint` for a custom OpenAI-compatible Realtime proxy. `OPENCLAW_OPENAI_REALTIME_ALLOW_UNVALIDATED_KEY` is an emergency override for unusual direct OpenAI Platform key formats that are incorrectly rejected before an `api.openai.com` Realtime request; prefer fixing the configured auth path instead of leaving this enabled.
