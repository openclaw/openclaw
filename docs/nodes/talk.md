  ...
  talk: {
    ...
    realtime: {
      ...
      voiceSession: {
        ...
        confirmationPolicy: "high-impact-outbound",
        postCallSummary: "mutations",
      },
    },
  },
}
...
| `realtime.consultPolicy`                   | `auto`                                     | `substantive` consults OpenClaw for personal memory, current facts, workspace context, or actions while ordinary conversation stays in Realtime. `always` consults before substantive answers.                                                                             |
| `realtime.toolPolicy`                      | `owner`                                    | `owner` exposes the Gateway-mediated `openclaw_agent_consult` tool with the routed agent's normal permissions. `none` disables it.                                                                                                                                         |
| `realtime.voiceSession.enabled`            | `false`                                    | Assigns one logical id across provider reconnects, accepts finalized transcript items, and recovers abandoned sessions on a later client create.                                                                                                                           |
| `realtime.voiceSession.persistTranscript`  | `false`                                    | Imports the complete role-preserving finalized transcript into the normal agent session on logical close.                                                                                                                                                                  |
| `realtime.voiceSession.confirmationPolicy` | `none`                                     | `high-impact-outbound` binds a fresh spoken confirmation to the exact paused high-impact or outbound tool call; changed arguments and replayed confirmations fail closed.                                                                                                  |
| `realtime.voiceSession.postCallSummary`    | `off`                                      | `mutations` sends a durable last-route summary only when a mutating tool execution started; read-only or conversational calls stay quiet.                                                                                                                                  |
...
When `agents.defaults.realtimeContext.enabled` (or its per-agent override) is
set, client-owned Talk creation also injects the agent's cached context pack.
Use this for identity plus pre-synthesized workspace snapshots; see
[agent realtime context](/gateway/config-agents#agentsdefaultsrealtimecontext).
