...
- `realtime.providers.openai.apiKey`: must be a direct OpenAI Platform key for OpenAI Realtime against `api.openai.com`, not an Azure AI Foundry / Azure OpenAI model key or OpenAI-compatible proxy key. Use `gateway-relay` with `azureEndpoint` + `azureDeployment` for Azure Realtime, or an endpoint-only `azureEndpoint` for a custom OpenAI-compatible Realtime proxy.
