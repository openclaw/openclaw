...
- For trusted self-hosted STT endpoints, set `tools.media.audio.request.allowPrivateNetwork: true` explicitly. This keeps local network access opt-in while allowing OpenAI-compatible local Whisper servers on `127.0.0.1` when `provider: "openai"` is used.
- OpenAI loopback STT endpoints can run without provider auth, matching local chat-completion auth handling. Other audio providers still use their normal provider credential path. `tools.media.audio.request.auth` can provide request-level bearer/header auth for proxy-backed or local endpoints when the endpoint expects it; OpenClaw will not reuse the real upstream OpenAI API key for local OpenAI audio `baseUrl` values.
