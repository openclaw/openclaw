...
- On macOS, a connected native node with Accessibility permission reports input idle time at a coalesced cadence. The Gateway derives `lastActiveAtMs`, marks the freshest connected Mac as `active`, and exposes both through `node.list`, `node.describe`, and `openclaw nodes status`. Input events themselves never leave the Mac.
- The agent prompt receives only the stable authenticated active-node id; node-supplied labels and exact activity timestamps stay behind the `nodes` tool, avoiding prompt injection and churn during continuous mouse or keyboard use.
- Node connection alerts are Gateway-routed: the active Mac is tried first, then other notification-capable Macs receive a delayed fallback only if primary delivery is unavailable or fails. A reconnect cooldown suppresses alert flapping.
