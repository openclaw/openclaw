...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T09:27:13Z
---

# BlueBubbles（macOS REST）

狀態：內建外掛，透過 HTTP 與 BlueBubbles macOS 伺服器通訊。**建議用於 iMessage 整合**，相較於舊版 imsg 頻道，其 API 更豐富且設定更簡單。

## 概覽

- 透過 BlueBubbles 輔助應用程式在 macOS 上執行（[bluebubbles.app](https://bluebubbles.app)）。
- 建議/測試環境：macOS Sequoia（15）。macOS Tahoe（26）可運作；但目前在 Tahoe 上編輯功能損壞，且群組圖示更新可能回報成功但不會同步。
- OpenClaw 透過其 REST API 與之通訊（`GET /api/v1/ping`、`POST /message/text`、`POST /chat/:id/*`）。
- 來訊透過 webhook 傳入；回覆、輸入中指示、已讀回條與點回應皆為 REST 呼叫。
- 附件與貼圖會作為入站媒體匯入（並在可能時提供給代理程式）。
...
某些 macOS VM／常駐設定可能會讓 Messages.app 進入「閒置」狀態（直到開啟/前景化前，來訊事件會停止）。一個簡單的因應方式是使用 AppleScript + LaunchAgent **每 5 分鐘戳一下 Messages**。
...
- 第一次執行可能會觸發 macOS 的**自動化**提示（`osascript` → Messages）。請在執行該 LaunchAgent 的同一使用者工作階段中核准。
...
- 配對是預設的權杖交換機制。詳情請見：[Pairing](/channels/pairing)
...
- **sendAttachment**：傳送媒體/檔案（`to`、`buffer`、`filename`、`asVoice`）
  - 語音備忘錄：設定 `asVoice: true` 並使用 **MP3** 或 **CAF** 音訊即可作為 iMessage 語音訊息傳送。BlueBubbles 在傳送語音備忘錄時會將 MP3 轉換為 CAF。
...
- 直接識別碼：`+15555550123`、`user@example.com`
  - 若直接識別碼尚無既有私訊聊天室，OpenClaw 會透過 `POST /api/v1/chat/new` 建立。這需要啟用 BlueBubbles 私有 API。

## 安全性

- Webhook 請求會透過比對 `guid`/`password` 查詢參數或標頭與 `channels.bluebubbles.password` 來進行驗證。來自 `localhost` 的請求也會被接受。
- 請妥善保管 API 密碼與 webhook 端點（視同憑證）。
- Localhost 信任代表同一主機上的反向代理可能會無意間繞過密碼。若你對 Gateway 閘道器進行代理，請在代理端要求驗證並設定 `gateway.trustedProxies`。請參閱 [Gateway security](/gateway/security#reverse-proxy-configuration)。
...
- 編輯/收回需要 macOS 13+ 與相容的 BlueBubbles 伺服器版本。在 macOS 26（Tahoe）上，因私有 API 變更，目前編輯功能損壞。
- 在 macOS 26（Tahoe）上，群組圖示更新可能不穩定：API 可能回傳成功但新圖示不會同步。
- OpenClaw 會根據 BlueBubbles 伺服器的 macOS 版本自動隱藏已知損壞的動作。若在 macOS 26（Tahoe）上仍顯示編輯功能，請使用 `channels.bluebubbles.actions.edit=false` 手動停用。
