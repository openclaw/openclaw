...
- 如果在此命令路徑中所需的認證 SecretRef 未解析，則探針認證可能會失敗；請明確傳遞 `--token`/`--password` 或先解析秘密來源。
...
- 當 token 認證需要一個 token 且 `gateway.auth.token` 由 SecretRef 管理時，`gateway install` 驗證 SecretRef 是否可解析，但不會將解析後的 token 持久化到服務環境元數據中。
- 如果 token 認證需要一個 token 且設定的 token SecretRef 無法解析，安裝將會失敗，而不是持久化回退的明文。
...
- 多播 DNS-SD: `local.`
- 單播 DNS-SD (廣域 Bonjour): 選擇一個網域 (範例: `openclaw.internal.`) 並設置分割 DNS + DNS 伺服器; 請參見 [/gateway/bonjour](/gateway/bonjour)

只有啟用 Bonjour 探索的閘道（預設設定）會廣播信標。

[[BLOCK_1]] 廣域發現記錄包括 (TXT): [[BLOCK_1]]
