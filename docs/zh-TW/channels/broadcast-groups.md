...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T09:27:02Z
---
...
廣播群組可讓多個代理程式同時處理並回覆相同的訊息。這讓你能在單一 WhatsApp 群組或私訊中建立協同運作的專業代理程式團隊——全部使用同一個電話號碼。

目前範圍：**僅支援 WhatsApp**（Web 頻道）。

廣播群組會在頻道允許清單與群組啟用規則之後進行評估。在 WhatsApp 群組中，這表示當 OpenClaw 通常會回覆時才會進行廣播（例如：依你的群組設定，在被提及時）。
...
新增一個最上層的 `broadcast` 區段（與 `bindings` 並列）。鍵為 WhatsApp 對等 ID：
