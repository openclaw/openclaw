...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T09:27:52Z
---

# Discord（Bot API）

狀態：已可透過官方 Discord 機器人 Gateway 閘道器，用於私訊（DM）與伺服器（guild）文字頻道。
...
3. 為 OpenClaw 設定權杖：
   ...
   - 若兩者同時設定，設定檔優先（環境變數回退僅適用於預設帳號）。
4. 將機器人邀請至你的伺服器，並賦予傳訊權限（若只想使用私訊，可建立私人伺服器）。
5. 啟動 Gateway 閘道器。
6. 私訊存取預設為配對模式；首次聯繫時請核准配對碼。
...
2. 將機器人邀請至你的伺服器，並賦予你希望使用之頻道的讀取/傳送訊息權限。
...
4. 執行 Gateway 閘道器；當權杖可用時（設定檔優先、環境變數回退），且 `channels.discord.enabled` 不是 `false`，Discord 頻道會自動啟動。
   - 若偏好使用環境變數，請設定 `DISCORD_BOT_TOKEN`（設定區塊可選）。
5. 私訊：投遞時使用 `user:<id>`（或 `<@id>` 提及）；所有回合都會進入共用的 `main` 工作階段。單純的數字 ID 具有歧義，會被拒絕。
6. 伺服器頻道：投遞時使用 `channel:<channelId>`。預設需要提及，可依伺服器或頻道設定。
7. 私訊：預設透過 `channels.discord.dm.policy` 強化安全性（預設：`"pairing"`）。未知寄件者會收到配對碼（1 小時後失效）；透過 `openclaw pairing approve discord <code>` 核准。
...
10. 選用的原生命令：`commands.native` 預設為 `"auto"`（Discord/Telegram 開啟，Slack 關閉）。可用 `channels.discord.commands.native: true|false|"auto"` 覆寫；`false` 會清除先前註冊的命令。文字命令由 `commands.text` 控制，且必須以獨立的 `/...` 訊息傳送。使用 `commands.useAccessGroups: false` 可略過命令的存取群組檢查。
    - 完整命令清單與設定：[Slash commands](/tools/slash-commands)
11. 選用的伺服器情境歷史：設定 `channels.discord.historyLimit`（預設 20，回退至 `messages.groupChat.historyLimit`），在回覆提及時包含最近 N 則伺服器訊息作為情境。設定 `0` 可停用。
...
注意：名稱 → id 解析使用伺服器成員搜尋，需啟用 Server Members Intent；若機器人無法搜尋成員，請使用 id 或 `<@id>` 提及。
注意：Slug 皆為小寫，空白以 `-` 取代。頻道名稱在 slug 化時不包含前導 `#`。
注意：伺服器情境的 `[from:]` 行會包含 `author.tag` + `id`，以方便產生可直接 ping 的回覆。
...
通常**不需要** **Presence Intent**。設定機器人自身狀態（`setPresence` 動作）使用 Gateway OP3，無需此 intent；只有在你想接收其他成員的狀態更新時才需要。
...
多帳號支援：使用 `channels.discord.accounts`，為每個帳號設定權杖，並可選擇 `name`。共享模式請見 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)。
...
- 使用 `"*"` 的頻道項目可套用至所有頻道；明確的頻道項目會覆寫萬用設定。
- 討論串會繼承父頻道的設定（允許清單、`requireMention`、Skills、提示等），除非你明確加入討論串的頻道 id。
- 擁有者提示：當每個伺服器或頻道的 `users` 允許清單符合寄件者時，OpenClaw 會在系統提示中將該寄件者視為擁有者。若要跨頻道的全域擁有者，請設定 `commands.ownerAllowFrom`。
...
- `channels.discord.groupPolicy` 預設為 **allowlist**；請將其設為 `"open"`，或在 `channels.discord.guilds` 下新增一個伺服器項目（可選擇在 `channels.discord.guilds.<id>.channels` 列出頻道以限制）。
  - 若你只設定了 `DISCORD_BOT_TOKEN`，卻從未建立 `channels.discord` 區段，執行時
    ...
    `channels.defaults.groupPolicy`，或伺服器/頻道允許清單以鎖定行為。
- `requireMention` 必須位於 `channels.discord.guilds`（或特定頻道）之下。最上層的 `channels.discord.requireMention` 會被忽略。
- **權限稽核**（`channels status --probe`）只會檢查數字頻道 ID。若你使用 slug/名稱作為 `channels.discord.guilds.*.channels` 的鍵，稽核將無法驗證權限。
- **私訊無法運作**：`channels.discord.dm.enabled=false`、`channels.discord.dm.policy="disabled"`，或你尚未被核准（`channels.discord.dm.policy="pairing"`）。
- **Discord 中的 Exec 核准**：Discord 在私訊中支援 **按鈕 UI** 進行 exec 核准（Allow once / Always allow / Deny）。`/approve <id> ...` 僅用於轉送的核准，無法解決 Discord 的按鈕提示。若你看到 `❌ Failed to submit approval: Error: unknown approval id` 或 UI 從未出現，請檢查：
...
- 伺服器回覆預設需提及，以避免吵雜的機器人。
- 當訊息參照另一則訊息時，會注入回覆情境（引用內容 + id）。
- 原生回覆串接預設為 **關閉**；使用 `channels.discord.replyToMode` 與回覆標籤啟用。

## 重試策略

對外的 Discord API 呼叫在遇到速率限制（429）時，會在可用時使用 Discord 的 `retry_after`，並搭配指數退避與抖動重試。可透過 `channels.discord.retry` 設定。請見 [Retry policy](/concepts/retry)。
...
- `dm.allowFrom`：私訊允許清單（使用者 id 或名稱）。供 `dm.policy="allowlist"` 與 `dm.policy="open"` 驗證使用。精靈接受使用者名稱，並在機器人可搜尋成員時解析為 id。
...
- `guilds.<id>.channels.<channel>.systemPrompt`：頻道的額外系統提示。Discord 頻道主題會以 **不可信** 情境注入（非系統提示）。
...
- `dmHistoryLimit`：私訊歷史上限（以使用者回合計）。每使用者覆寫：`dms["<user_id>"].historyLimit`。
...
- `execApprovals`：僅限 Discord 的 exec 核准私訊（按鈕 UI）。支援 `enabled`、`approvers`、`agentFilter`、`sessionFilter`。
...
啟用 PK 查詢，使代理訊息能解析為底層系統 + 成員。
啟用後，OpenClaw 會使用成員身分進行允許清單比對，並將
寄件者標示為 `Member (PK:System)`，以避免意外的 Discord 提及。
...
Discord 訊息 id 會顯示在注入的情境中（`[discord message id: …]` 與歷史行），以便代理程式能精準指定。
表情符號可為 Unicode（例如 `✅`），或自訂表情語法如 `<:party_blob:1234567890>`。

## 安全性與營運

- 將 Bot 權杖視為密碼；在受監督的主機上優先使用 `DISCORD_BOT_TOKEN` 環境變數，或鎖定設定檔權限。
- 僅授予機器人所需的權限（通常為讀取/傳送訊息）。
