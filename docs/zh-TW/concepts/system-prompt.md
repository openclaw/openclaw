...
OpenClaw 會為每一次代理程式執行建構一個自訂的系統提示。此提示由 **OpenClaw 所擁有**，並且不使用 p-coding-agent 的預設提示。
...
系統提示中的安全防護屬於建議性質。它們引導模型行為，但不會強制執行政策。若需硬性強制，請使用工具政策、exec 核准、沙箱隔離，以及頻道允許清單；營運者可依設計選擇停用這些機制。

## 提示模式

OpenClaw 可為子代理程式產生較小的系統提示。執行階段會為每次執行設定
`promptMode`（非使用者可見的設定）：
...
若要檢視每個注入檔案的貢獻量（原始 vs 注入後、截斷情況，以及工具結構描述的額外負擔），請使用 `/context list` 或 `/context detail`。請參閱 [Context](/concepts/context)。
