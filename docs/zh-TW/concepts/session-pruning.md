---
summary: 「工作階段修剪：透過修剪工具結果來降低內容膨脹」
read_when:
...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T09:27:45Z
---

# 工作階段修剪

工作階段修剪會在每次 LLM 呼叫前，從記憶體中的內容修剪 **較舊的工具結果**。它 **不會** 重寫磁碟上的工作階段歷史（`*.jsonl`）。
...
- 修剪完成後，TTL 視窗會重設，因此後續請求會保留快取，直到 `ttl` 再次到期。
...
- **為什麼要修剪：** Anthropic 的提示快取只在 TTL 內生效。若工作階段在 TTL 之後閒置，下一次請求會重新快取完整提示，除非你先修剪它。
- **哪些會更便宜：** 修剪可降低 TTL 到期後第一次請求的 **cacheWrite** 大小。
- **為什麼 TTL 重設很重要：** 一旦修剪執行，快取視窗會重設，後續請求即可重用新快取的提示，而不必再次快取完整歷史。
- **它不會做的事：** 修剪不會增加權杖或造成「加倍」成本；它只會改變 TTL 後第一次請求被快取的內容。
...
- 若助手訊息數量不足以建立切點，將跳過修剪。
- 含有 **影像區塊** 的工具結果會被略過（永不修剪／清除）。

## 內容視窗估算

修剪使用估算的內容視窗（字元 ≈ 權杖 × 4）。基礎視窗依下列順序解析：
...
- 僅在最後一次 Anthropic 呼叫早於 `ttl`（預設 `5m`）時才會修剪。
- 執行時：與先前相同的軟修剪 + 硬清除行為。
...
- 內建工具已會截斷自身輸出；工作階段修剪是額外的一層，可防止長時間聊天在模型內容中累積過多工具輸出。
- 壓縮（Compaction）是分開的：壓縮會摘要並持久化；修剪則是每次請求的暫態行為。請參閱 [/concepts/compaction](/concepts/compaction)。
