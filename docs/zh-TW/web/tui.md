...
- 目前的代理程式 + 工作階段會永遠顯示在頁尾。

## 傳送 + 投遞

- 訊息會傳送到 Gateway 閘道器；預設不會投遞到提供者。
- 開啟投遞：
  - `/deliver on`
  - 或使用設定面板
...
其他 Gateway 閘道器斜線指令（例如 `/context`）會轉送至 Gateway 閘道器，並以系統輸出顯示。請參閱 [Slash commands](/tools/slash-commands)。

## 本機殼層指令

- 在一行前加上 `!`，即可在 TUI 主機上執行本機殼層指令。
- TUI 每個工作階段只會提示一次是否允許本機執行；若拒絕，該工作階段會停用 `!`。
...
- 工作階段選擇器為空：你可能在全域範圍，或尚未建立任何工作階段。
