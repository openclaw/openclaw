...
- `sessionKey` 選填（string）：用於識別代理程式工作階段的鍵。預設為隨機的 `hook:<uuid>`。使用一致的鍵可在 Hook 情境中進行多回合對話。
- `wakeMode` 選填（`now` | `next-heartbeat`）：是否立即觸發心跳（預設 `now`），或等待下一次定期檢查。
- `deliver` 選填（boolean）：若為 `true`，代理程式的回應將送至訊息頻道。預設為 `true`。僅為心跳確認的回應會自動略過。
- `channel` 選填（string）：投遞用的訊息頻道。其一：`last`、`whatsapp`、`telegram`、`discord`、`slack`、`mattermost`（外掛）、`signal`、`imessage`、`msteams`。預設為 `last`。
- `to` 選填（string）：該頻道的收件者識別碼（例如：WhatsApp/Signal 的電話號碼、Telegram 的 chat ID、Discord/Slack/Mattermost（外掛）的 channel ID、MS Teams 的 conversation ID）。預設為主要工作階段中的最後一位收件者。
- `model` 選填（string）：模型覆寫（例如：`anthropic/claude-3-5-sonnet` 或別名）。若有限制，必須包含於允許的模型清單中。
...
自訂 Hook 名稱會透過 `hooks.mappings` 解析（見設定）。映射可將任意 Payload 轉換為 `wake` 或 `agent` 動作，並可搭配範本或程式碼轉換。
...
- 使用專用的 Hook 權杖；不要重複使用 Gateway 閘道器的驗證權杖。
- 避免在 Webhook 記錄中包含敏感的原始 Payload。
- Hook Payload 預設視為不受信任，並以安全邊界包裝。
  若必須為特定 Hook 停用，請在該 Hook 的映射中設定 `allowUnsafeExternalContent: true`
  （危險）。
