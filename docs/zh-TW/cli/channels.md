...
當您將一個非預設帳戶添加到仍在使用單帳戶頂層設定的頻道（尚未有 `channels.<channel>.accounts` 條目）時，OpenClaw 會將帳戶範圍的單帳戶頂層值移動到 `channels.<channel>.accounts.default`，然後寫入新的帳戶。這樣可以在轉向多帳戶形狀的同時保留原始帳戶的行為。

Routing behavior stays consistent:

- 現有的僅通道綁定（無 `accountId`）將繼續匹配預設帳戶。
- `channels add` 在非互動模式下不會自動創建或重寫綁定。
- 互動設置可以選擇性地添加帳戶範圍的綁定。

如果您的設定已經處於混合狀態（存在命名帳戶，但缺少 `default`，並且頂層的單一帳戶值仍然設置），請執行 `openclaw doctor --fix` 將帳戶範圍的值移動到 `accounts.default`。
...
- `openclaw channels status` 在無法連接到網關時回退到僅設定的摘要。如果透過 SecretRef 設定了受支援的通道憑證，但在當前命令路徑中不可用，則會報告該帳戶已設定為降級註解，而不是顯示為未設定。
...
- `channels resolve` 是唯讀的。如果所選帳戶是通過 SecretRef 設定的，但該憑證在當前命令路徑中不可用，則命令會返回降級的未解析結果，並附上說明，而不是中止整個執行。
