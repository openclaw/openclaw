...
- `--fix`（`--repair` 的別名）將備份寫入 `~/.openclaw/openclaw.json.bak` 並刪除未知的設定鍵，列出每個移除專案。
...
- 如果啟用了沙盒模式但 Docker 不可用，doctor 會報告高信號警告並提供修復建議 (`install Docker` 或 `openclaw config set agents.defaults.sandbox.mode off`)。
