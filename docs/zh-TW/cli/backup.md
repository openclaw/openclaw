...
建立 OpenClaw 狀態、設定、憑證、會話的本地備份檔案，並可選擇備份工作區。
...
如果您只需要設定檔案的副本，`--only-config` 在設定檔案格式錯誤時也能正常運作，因為它不依賴於解析設定來進行工作區的發現。
...
- 目標路徑的檔案系統行為。OpenClaw 偏好不覆寫的硬連結發佈步驟，當硬連結不受支援時則回退到獨佔複製
