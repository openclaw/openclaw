...
read_when:
  - 當你想使用 Perplexity Sonar 進行網頁搜尋時
  - 當你需要 PERPLEXITY_API_KEY 或 OpenRouter 設定時
title: "Perplexity Sonar"
---

# Perplexity Sonar

OpenClaw 可以將 Perplexity Sonar 用於 `web_search` 工具。你可以透過 Perplexity 的直接 API 或經由 OpenRouter 連接。
...
如果同時設定了 `PERPLEXITY_API_KEY` 和 `OPENROUTER_API_KEY`，請設定 `tools.web.search.perplexity.baseUrl`（或 `tools.web.search.perplexity.apiKey`）以消除歧義。
