...
- Worker exec is full-permission within the box when the Gateway-issued turn authority includes it. The worker box replaces sandbox-local execution, so that path retains exec; the Gateway omits worker exec when execution depends on a Gateway allowlist or approval, is explicitly denied, or targets a paired node. The worker does not reproduce those external policy owners. The box is disposable and credential-free, so no second per-command approval runs inside it; the guarded boundaries are Gateway admission, inbound reconciliation, and audit. Exec never traverses the gateway node-approval path.
