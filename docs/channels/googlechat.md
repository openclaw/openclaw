...
- Message actions expose text `send` and `edit`. To edit, pass the full `spaces/<spaceId>/messages/<messageId>` resource name returned by send as `messageId`, a target resolving to the same space (case-sensitive), and replacement `message` text. Only messages created by the selected Chat app can be edited, using the existing `chat.bot` scope. Edits preserve cards and attachments; API errors are returned without sending a replacement message.
- Google Chat attachment upload requires user authentication, while this plugin uses service-account authentication, so outbound file upload is not exposed.
