...
- `typingIndicator`: `message` (default) posts a `_<Bot> is typing..._` placeholder and edits it into the first reply. `live` posts the same placeholder and edits it in place with live progress (tool started, thinking, ...). When the turn ends, it edits the placeholder to a "done" status and posts the answer as a new message, so Google Chat notifies the user. An in-place edit does not notify. `none` disables it. `reaction` requires user OAuth and currently falls back to `message` with a logged error under service-account auth. `message` and `live` use only the `chat.bot` scope (`spaces.messages.patch`) and need no user OAuth.
