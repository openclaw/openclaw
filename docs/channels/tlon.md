...
Tlon is a decentralized messenger built on Urbit. DNA connects to your Urbit ship and can
...
```bash
dna plugins install @dna/tlon
```

Local checkout (when running from a git repo):

```bash
dna plugins install ./extensions/tlon
```
...
Use these with `dna message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, DNA replies in-thread.
