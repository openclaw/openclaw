...
Tlon is a decentralized messenger built on Urbit. EasyHub connects to your Urbit ship and can
...
```bash
EasyHub plugins install @easyhub/tlon
```

Local checkout (when running from a git repo):

```bash
EasyHub plugins install ./extensions/tlon
```
...
Use these with `EasyHub message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, EasyHub replies in-thread.
