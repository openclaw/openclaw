...
Tlon is a decentralized messenger built on Urbit. Mullusi connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Mullusi releases, so normal packaged
...
```bash
mullusi plugins install @mullusi/tlon
```

Local checkout (when running from a git repo):

```bash
mullusi plugins install ./path/to/local/tlon-plugin
```
...
By default, Mullusi blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `mullusi message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Mullusi replies in-thread.
