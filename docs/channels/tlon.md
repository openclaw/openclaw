...
Tlon is a decentralized messenger built on Urbit. Activi connects to your Urbit ship and can
...
```bash
activi plugins install @activi/tlon
```

Local checkout (when running from a git repo):

```bash
activi plugins install ./extensions/tlon
```
...
By default, Activi blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `activi message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Activi replies in-thread.
