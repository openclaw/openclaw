...
Discord subagent progress on the original message is default off. Set `channels.discord.subagentProgress.enabled=true` to opt in. When enabled, active tracked subagents show one count reaction (`1️⃣` through `🔟` by default); count changes replace the previous reaction; zero active subagents removes the count reaction; non-ok completions leave `🔴`; Discord typing stays active while tracked subagents are still running. Configure this with `channels.discord.subagentProgress` or per-account `channels.discord.accounts.<accountId>.subagentProgress`.
