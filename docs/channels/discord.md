...
- Set `voice.realtime.bootstrapContextFiles` to include small agent profile files in the realtime provider instructions. Use `["IDENTITY.md", "USER.md", "SOUL.md"]` when `bidi` should answer fast direct turns with the same identity, user grounding, and persona as the routed OpenClaw agent. The supported realtime bootstrap files are limited to those profile files; `AGENTS.md` stays in the normal agent context. The injected profile context is capped by `voice.realtime.bootstrapContextMaxChars` (default: `4000`) and does not replace `openclaw_agent_consult` for workspace work, current facts, memory lookup, or tool-backed actions.
