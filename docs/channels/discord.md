...
```
    ...
    - `streaming.progress.commentary` (default `false`) opts into raw assistant commentary in the temporary progress draft. The default preamble/narration status line is independent of this option; when commentary is enabled, the latest preamble remains the headline and also appears in the bounded commentary history. Commentary is cleaned before display, stays transient, and does not change final answer delivery.
