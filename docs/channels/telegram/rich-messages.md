...
```
    ...
    Rich media accepts HTTPS URLs and absolute local paths under the caller's allowed media roots. Local photos up to 10 MB, videos, and audio upload inside the rich message; a local path in `<img src="/path/photo.png"/>` or Markdown image syntax embeds the file. Code examples remain literal and do not read local files. Unsupported local types in media tags are rejected; send them as ordinary attachments instead.

    With rich messages enabled, a text payload with eligible local media embeds those files in rich blocks. Remaining attachments retain ordinary delivery. Explicit voice-note, video-note, and forced-document sends retain their selected mode. If Telegram rejects the rich message, OpenClaw falls back to plain text and resends the local files as ordinary attachments.
