...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ...
    - `streaming.progress.commentary` (default: `false`) opts into a bounded assistant commentary/preamble timeline in the temporary progress draft; the latest preamble can supply the status headline whether this timeline is enabled or disabled
