...
read_when:
  - You want EasyHub to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables EasyHub to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`EasyHub onboard`) and `EasyHub channels add` list optional channel plugins.
...
```bash
EasyHub plugins install @easyhub/nostr
```

Use a local checkout (dev workflows):

```bash
EasyHub plugins install --link <path-to-EasyHub>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "EasyHub@example.com",
        "lud16": "EasyHub@example.com"
      }
    }
  }
}
