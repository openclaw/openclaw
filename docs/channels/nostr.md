...
read_when:
  - You want Activi to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Activi to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`activi onboard`) and `activi channels add` list optional channel plugins.
...
```bash
activi plugins install @activi/nostr
```

Use a local checkout (dev workflows):

```bash
activi plugins install --link <path-to-activi>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "activi@example.com",
        "lud16": "activi@example.com"
      }
    }
  }
}
