...
read_when:
  - You want EVOX.sh to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables EVOX.sh to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`evox onboard`) and `evox channels add` list optional channel plugins.
...
```bash
evox plugins install @openclaw/nostr
```

Use a local checkout (dev workflows):

```bash
evox plugins install --link <path-to-openclaw>/extensions/nostr
```
...
```bash
evox channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
evox channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "evox@example.com",
        lud16: "evox@example.com",
      },
    },
  },
}
