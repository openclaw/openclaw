...
read_when:
  - You want DNA to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables DNA to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`dna onboard`) and `dna channels add` list optional channel plugins.
...
```bash
dna plugins install @dna/nostr
```

Use a local checkout (dev workflows):

```bash
dna plugins install --link <path-to-dna>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "dna@example.com",
        "lud16": "dna@example.com"
      }
    }
  }
}
