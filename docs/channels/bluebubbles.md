...
| Layer               | Key                                          | Effect                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------------- | -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Channel disablement | `channels.bluebubbles.enabled: false`        | Opts out of inbound webhook routing, channel auto-enable, and bundled-channel setup discovery. Note: explicit outbound sends (e.g., `messages.send_to_channel`) can still resolve the bundled plugin if credentials are configured, because outbound channel resolution falls back to bundled channel plugins regardless of this flag (`src/infra/outbound/channel-resolution.ts:52-74`). The plugin row may still appear in `openclaw plugins list` (it is bundled). |
| Plugin disablement  | `plugins.entries.bluebubbles.enabled: false` | Keeps the bundled plugin from registering at all. No webhook handler, no auto-enable on startup, no setup probes, and explicit outbound sends cannot resolve the channel either.                                                                                                                                                                                                                                                                                      |

Channel disablement alone keeps inbound webhook routing and setup discovery off, but explicit outbound sends (for example `messages.send_to_channel`) can still resolve the bundled plugin when credentials are configured, because outbound channel resolution falls back to bundled plugins regardless of `channels.<id>.enabled`. Setting both layers is the recommended full opt-out: it keeps the bundled plugin module from registering at all, blocks both inbound and explicit-outbound paths, and is preferred for tight-control deployments that audit plugin registration counts at startup.
