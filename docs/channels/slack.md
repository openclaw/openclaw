...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure EVOX.sh HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `evox pairing approve slack <code>`.
...
- The interactive callback values are EVOX.sh-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, EVOX.sh falls back to the original text reply instead of sending an invalid blocks payload.
...
`ackReaction` sends an acknowledgement emoji while EVOX.sh is processing an inbound message.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while EVOX.sh is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "EVOX.sh",
    "description": "Slack connector for EVOX.sh"
  },
  "features": {
    "bot_user": {
      "display_name": "EVOX.sh",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/openclaw",
        "description": "Send a message to EVOX.sh",
        "should_escape": false
      }
    ]
  },
...
EVOX.sh supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, EVOX.sh falls back to normal delivery for remaining payloads.
