...
3. Set tokens for EasyHub and start the gateway.
...
7. Slash Commands → create `/EasyHub` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
EasyHub can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, EasyHub falls back to the user token.
...
```json
{
  "display_information": {
    "name": "EasyHub",
    "description": "Slack connector for EasyHub"
  },
  "features": {
    "bot_user": {
      "display_name": "EasyHub",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/EasyHub",
        "description": "Send a message to EasyHub",
        "should_escape": false
      }
    ]
  },
...
By default, EasyHub replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, EasyHub infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `EasyHub pairing approve slack <code>`.
...
- On startup, EasyHub resolves channel/user names in allowlists to IDs (when tokens allow)
