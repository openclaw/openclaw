...
- With `channels.slack.replyToMode="all"`, top-level channel/group messages also root their own `:thread:<messageTs>` session so later thread replies stay on the same session from the first turn.
