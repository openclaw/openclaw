...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects SmartAgentNeo to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed SmartAgentNeo, run the wizard:
...
2. Configuring app credentials in SmartAgentNeo
...
1. You already ran `smart-agent-neo channels add` for Feishu
2. The gateway is running (`smart-agent-neo gateway status`)
...
## Step 2: Configure SmartAgentNeo
...
Edit `~/.smart-agent-neo/smart-agent-neo.json`:
...
```bash
smart-agent-neo pairing approve feishu <CODE>
```
...
2. Run `smart-agent-neo logs --follow` and look for `chat_id`
...
2. Run `smart-agent-neo logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `smart-agent-neo gateway status`  | Show gateway status           |
| `smart-agent-neo gateway install` | Install/start gateway service |
| `smart-agent-neo gateway stop`    | Stop gateway service          |
| `smart-agent-neo gateway restart` | Restart gateway service       |
| `smart-agent-neo logs --follow`   | Tail gateway logs             |
...
4. Check logs: `smart-agent-neo logs --follow`
...
5. Ensure the gateway is running: `smart-agent-neo gateway status`
6. Check logs: `smart-agent-neo logs --follow`
