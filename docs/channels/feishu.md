...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects EasyHub to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed EasyHub, run the wizard:
...
2. Configuring app credentials in EasyHub
...
1. You already ran `EasyHub channels add` for Feishu
2. The gateway is running (`EasyHub gateway status`)
...
```bash
EasyHub pairing approve feishu <CODE>
```
...
2. Run `EasyHub logs --follow` and look for `chat_id`
...
2. Run `EasyHub logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `EasyHub gateway status`  | Show gateway status           |
| `EasyHub gateway install` | Install/start gateway service |
| `EasyHub gateway stop`    | Stop gateway service          |
| `EasyHub gateway restart` | Restart gateway service       |
| `EasyHub logs --follow`   | Tail gateway logs             |
...
4. Check logs: `EasyHub logs --follow`
...
5. Ensure the gateway is running: `EasyHub gateway status`
6. Check logs: `EasyHub logs --follow`
