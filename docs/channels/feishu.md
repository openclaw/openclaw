...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects EVOX.sh to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current EVOX.sh releases, so no separate plugin install
is required.
...
```bash
evox plugins install @openclaw/feishu
```
...
If you just installed EVOX.sh, run onboarding:
...
2. Configuring app credentials in EVOX.sh
...
1. You already ran `evox channels add` for Feishu
2. The gateway is running (`evox gateway status`)
...
```bash
evox pairing approve feishu <CODE>
```
...
2. Run `evox logs --follow` and look for `chat_id`
...
2. Run `evox logs --follow` and look for `open_id`
...
| `evox gateway restart` | Restart gateway service       |
| `evox logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `evox gateway status`
6. Check logs: `evox logs --follow`
