...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Mullusi to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Mullusi releases, so no separate plugin install
is required.
...
If you just installed Mullusi, run onboarding:
...
2. Configuring app credentials in Mullusi
...
1. You already ran `mullusi channels add` for Feishu
2. The gateway is running (`mullusi gateway status`)
...
2. Run `mullusi logs --follow` and look for `chat_id`
...
2. Run `mullusi logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `mullusi gateway status`  | Show gateway status           |
| `mullusi gateway install` | Install/start gateway service |
| `mullusi gateway stop`    | Stop gateway service          |
| `mullusi gateway restart` | Restart gateway service       |
| `mullusi logs --follow`   | Tail gateway logs             |
...
4. Check logs: `mullusi logs --follow`
...
5. Ensure the gateway is running: `mullusi gateway status`
6. Check logs: `mullusi logs --follow`
