...
- Activation modes: `mention` (default) or `always`. `mention` requires a ping. A ping is a real WhatsApp @-mention (`mentionedJids`), a configured regex pattern, or the bot's E.164 digits anywhere in the text. A quoted reply to one of the bot's messages also counts, except in shared-number self-chat setups. `always` wakes the agent on every message. The injected group prompt then tells it to reply only when it adds value. Otherwise the agent returns the exact silent token `NO_REPLY` (case-insensitive). Defaults come from config (`channels.whatsapp.groups` `requireMention`) and can be overridden per group via `/activation`.
...
- Per-group sessions: session keys look like `agent:<agentId>:whatsapp:group:<jid>`. Non-default accounts append `:thread:whatsapp-account-<accountId>`. Directives such as `/verbose on`, `/trace on`, or `/think high` are therefore scoped to that group when sent as standalone messages. Personal DM state is untouched.
...
- Group system prompt: the first turn of a group session injects activation guidance into the system prompt. Any turn after `/activation` changes the mode does the same. The guidance reads `Activation: trigger-only ...` or `Activation: always-on ...`, plus "address the specific sender". Persistent group-chat delivery guidance ("You are in a WhatsApp group chat...") is always included.
...
- Manual smoke:
  - Send an `@openclaw` ping in the group and check that the reply references the sender name.
  - Send a second ping and check that the history block is included, then cleared on the next turn.
- Check gateway logs (run with `--verbose`) for `inbound web message` entries that carry `from: <groupJid>` and the sender-labeled body.
...
- Typing indicators follow `agents.entries.*.typingMode` / `agents.defaults.typingMode`. When visible replies are opted into message-tool-only mode, typing starts immediately by default. Group members can then see the agent working even if no automatic final reply is posted. Explicit typing-mode config still wins.
