...
In `progress` mode, the temporary post uses Mattermost type
...
<AccordionGroup>
  <Accordion title="Streaming modes">
    ...
    - `progress` shows a temporary status preview while generating, sends the final answer as a new post, and removes the status only after confirmed delivery.
