...
QQ 机器人通过官方 QQ 机器人 API（WebSocket 网关）连接到 OpenClaw。该插件支持 C2C 私聊、群组 @消息和频道消息，以及富媒体（图片、语音、视频、文件）。

状态：内置插件。支持私信、群聊、频道和媒体。不支持反应和线程。

## 内置插件

当前 OpenClaw 版本内置了 QQ 机器人，因此正常的打包构建不需要单独的 `openclaw plugins install` 步骤。
...
> AppSecret 不会以明文形式存储 — 如果你离开页面而没有保存，将不得不重新生成一个新的。
...
- `openclaw channels add --channel qqbot --token-file ...` 仅提供 AppSecret；AppID 必须已经在配置或 `QQBOT_APP_ID` 中设置。
...
- **无入站消息**：验证 `appId` 和 `clientSecret` 正确，且机器人在 QQ 开放平台上已启用。
- **使用 `--token-file` 设置后仍显示未配置**：`--token-file` 仅设置 AppSecret。你仍需要在配置或 `QQBOT_APP_ID` 中设置 `appId`。
- **主动消息未到达**：如果用户最近没有互动，QQ 可能会拦截机器人发起的消息。
- **语音未转录**：确保 STT 已配置且提供者可访问。