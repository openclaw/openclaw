...
```bash
smart-agent-neo plugins install @smart-agent-neo/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
smart-agent-neo plugins install ./extensions/nextcloud-talk
```

If you choose Nextcloud Talk during configure/onboarding and a git checkout is detected,
SmartAgentNeo will offer the local install path automatically.
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "SmartAgentNeo" "<shared-secret>" "<webhook-url>" --feature reaction
   ```

3. Enable the bot in the target room settings.
4. Configure SmartAgentNeo:
...
- Approve via:
  - `smart-agent-neo pairing list nextcloud-talk`
  - `smart-agent-neo pairing approve nextcloud-talk <CODE>`
