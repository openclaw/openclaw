...
```bash
evox plugins install @openclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
evox plugins install ./extensions/nextcloud-talk
```

If you choose Nextcloud Talk during setup and a git checkout is detected,
EVOX.sh will offer the local install path automatically.
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "EVOX.sh" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `evox pairing list nextcloud-talk`
  - `evox pairing approve nextcloud-talk <CODE>`
