...
```bash
activi plugins install @activi/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
activi plugins install ./extensions/nextcloud-talk
```

If you choose Nextcloud Talk during configure/onboarding and a git checkout is detected,
Activi will offer the local install path automatically.
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Activi" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `activi pairing list nextcloud-talk`
  - `activi pairing approve nextcloud-talk <CODE>`
