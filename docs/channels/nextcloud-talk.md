...
```bash
dna plugins install @dna/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
dna plugins install ./extensions/nextcloud-talk
```

If you choose Nextcloud Talk during configure/onboarding and a git checkout is detected,
DNA will offer the local install path automatically.
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "DNA" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `dna pairing list nextcloud-talk`
  - `dna pairing approve nextcloud-talk <CODE>`
