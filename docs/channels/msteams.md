...
```bash
evox plugins install @openclaw/msteams
```

Local checkout (when running from a git repo):

```bash
evox plugins install ./extensions/msteams
```
...
3. Configure EVOX.sh with those credentials.
...
- Talk to EVOX.sh via Teams DMs, group chats, or channels.
...
- On startup, EVOX.sh resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring EVOX.sh, you need to create an Azure Bot resource.
...
1. **Install the Microsoft Teams plugin**
   - From npm: `evox plugins install @openclaw/msteams`
   - From a local checkout: `evox plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "EVOX.sh in Teams", full: "EVOX.sh in Teams" },
...
EVOX.sh handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, EVOX.sh only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
EVOX.sh sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `evox message poll --channel msteams --target conversation:<id> ...`
...
evox message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
evox message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
evox message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
evox message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
