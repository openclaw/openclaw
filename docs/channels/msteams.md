...
3. Configure Activi with those credentials.
...
- Talk to Activi via Teams DMs, group chats, or channels.
...
- On startup, Activi resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.activi/activi.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Activi, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `activi-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `activi plugins install @activi/msteams`
   - From a local checkout: `activi plugins install ./extensions/msteams`
...
Activi handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Activi only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ActiviShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Activi sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `activi message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.activi/msteams-polls.json`.
