...
3. Configure SmartAgentNeo with those credentials.
...
- Talk to SmartAgentNeo via Teams DMs, group chats, or channels.
...
- On startup, SmartAgentNeo resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.smart-agent-neo/smart-agent-neo.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring SmartAgentNeo, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `smart-agent-neo-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `smart-agent-neo plugins install @smart-agent-neo/msteams`
   - From a local checkout: `smart-agent-neo plugins install ./extensions/msteams`
...
SmartAgentNeo handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, SmartAgentNeo only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SmartAgentNeoShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

SmartAgentNeo sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `smart-agent-neo message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.smart-agent-neo/msteams-polls.json`.
