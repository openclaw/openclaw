...
read_when:
  - Setting up Twitch chat integration for Activi
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Activi connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `ACTIVI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "activi", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ACTIVI_TWITCH_ACCESS_TOKEN env var)
...
```bash
ACTIVI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
