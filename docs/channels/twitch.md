...
read_when:
  - Setting up Twitch chat integration for EasyHub
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. EasyHub connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `EASYHUB_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "EasyHub", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use EASYHUB_TWITCH_ACCESS_TOKEN env var)
...
```bash
EASYHUB_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
