...
read_when:
  - Setting up Twitch chat integration for DNA
---
# Twitch (plugin)

Twitch chat support via IRC connection. DNA connects as a Twitch user (bot account) to receive and send messages in channels.
...
4) Configure the token:
   - Env: `DNA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "dna",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use DNA_TWITCH_ACCESS_TOKEN env var)
...
```bash
DNA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
