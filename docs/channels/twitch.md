...
read_when:
  - Setting up Twitch chat integration for SmartAgentNeo
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SmartAgentNeo connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `SMART_AGENT_NEO_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "smart-agent-neo", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SMART_AGENT_NEO_TWITCH_ACCESS_TOKEN env var)
...
```bash
SMART_AGENT_NEO_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
