...
read_when:
  - Setting up Twitch chat integration for EVOX.sh
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. EVOX.sh connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
evox plugins install @openclaw/twitch
```

Local checkout (when running from a git repo):

```bash
evox plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "evox", // Bot's Twitch account
