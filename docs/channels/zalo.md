...
Zalo ships as a bundled plugin in current Mullusi releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `mullusi plugins install @mullusi/zalo`
- Or from a source checkout: `mullusi plugins install ./path/to/local/zalo-plugin`
...
This page reflects current Mullusi behavior for **Zalo Bot Creator / Marketplace bots**.
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in Mullusi.
...
- Example: `mullusi message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `mullusi channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `mullusi logs --follow`
