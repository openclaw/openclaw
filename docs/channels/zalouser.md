...
read_when:
  - Setting up Zalo Personal for Mullusi
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Mullusi.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current Mullusi releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `mullusi plugins install @mullusi/zalouser`
- Or from a source checkout: `mullusi plugins install ./path/to/local/zalouser-plugin`
...
- On startup, Mullusi resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Mullusi stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Mullusi state. Example:
...
- Mullusi sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Mullusi sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `mullusi channels logout --channel zalouser && mullusi channels login --channel zalouser`
...
- The channel now runs fully in Mullusi without external CLI binaries.
