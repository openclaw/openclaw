...
read_when:
  - Setting up Zalo Personal for EVOX.sh
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside EVOX.sh.
...
- Install via CLI: `evox plugins install @openclaw/zalouser`
- Or from a source checkout: `evox plugins install ./extensions/zalouser`
...
- `evox pairing list zalouser`
- `evox pairing approve zalouser <code>`
...
- On startup, EVOX.sh resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, EVOX.sh stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in EVOX.sh state. Example:
...
- EVOX.sh sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, EVOX.sh sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `evox channels logout --channel zalouser && evox channels login --channel zalouser`
...
- The channel now runs fully in EVOX.sh without external CLI binaries.
