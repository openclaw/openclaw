...
read_when:
  - Setting up Zalo Personal for SmartAgentNeo
...
- Install via CLI: `smart-agent-neo plugins install @smart-agent-neo/zalouser`
- Or from a source checkout: `smart-agent-neo plugins install ./extensions/zalouser`
...
- On startup, SmartAgentNeo resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `smart-agent-neo channels logout --channel zalouser && smart-agent-neo channels login --channel zalouser`
