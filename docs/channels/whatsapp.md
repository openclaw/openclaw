...
`messages.ackReaction` sends an immediate reaction on inbound receipt, gated by `channels.whatsapp.reactionLevel` (suppressed when `"off"`). `messages.ackReactionScope` selects which conversations get one:
