...
read_when:
  - You want to connect Mullusi to LINE
...
LINE connects to Mullusi via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current Mullusi releases, so normal
...
```bash
mullusi plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Mullusi applies strict pre-auth body limits and timeout before verification.
- Mullusi processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
