...
- DAVE encryption defaults its native pool to one thread before an encrypted voice worker starts. Workers with `voice.daveEncryption=false` do not set a thread limit. The native pool survives voice disconnects and is separate from JavaScript Worker diagnostics.
- **Process-wide side effect:** unless either `RAYON_NUM_THREADS` or `RAYON_RS_NUM_CPUS` is already set, encrypted voice startup sets `RAYON_NUM_THREADS=1` in the Gateway environment for the rest of the process lifetime. Later children that inherit this environment, including direct launches and ordinary brokered spawns, receive the same limit; other Rayon pools initialized afterward can also be affected. Either operator-provided variable prevents OpenClaw from setting the default. The native binding initializes its pool lazily during a membership rekey and exposes no pool-size or initialization API, so clearing the variable at voice readiness would not safely retain the one-thread limit.
