...
- DAVE encryption defaults its process-wide native pool to one thread before the first voice worker starts, preserving an existing native thread-limit setting. This prevents a voice membership change from creating one retained thread per CPU. The native pool survives voice disconnects and is separate from JavaScript Worker diagnostics.
