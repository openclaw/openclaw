...
- EVOX.sh installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "EVOX.sh"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure EVOX.sh and restart the gateway.
5. Send a first DM and approve pairing (`evox pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "EVOX.sh"` then scan the QR in Signal.
...
4. Configure EVOX.sh, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `evox pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point EVOX.sh at it:
...
This skips auto-spawn and the startup wait inside EVOX.sh. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `evox pairing list signal`
  - `evox pairing approve signal <CODE>`
...
- **Typing indicators**: EVOX.sh sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, EVOX.sh forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `evox doctor --fix`.
