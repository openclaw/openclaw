...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ...
    - `streaming.preview.commandText` controls command/exec detail inside those tool-progress lines: `raw` (default, preserves released behavior) or `status` (hide raw command text; operand-free safe summaries may still render)
