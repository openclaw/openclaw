...
- DM messages can carry `message_thread_id`; OpenClaw preserves the thread ID for replies and defaults `channels.telegram.dm.threadReplies` to `"auto"`. Bots with `getMe.has_topics_enabled=true` use thread-scoped DM sessions by default; bots without that capability keep DMs on the flat session. Configure `"off"`, `"inbound"`, `"always"`, `requireTopic: true`, or a matching topic config when you need fixed behavior.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**: `/acp spawn <agent> --thread here|auto` binds the current topic to a new ACP session; follow-ups route there directly. OpenClaw pins the spawn confirmation in-topic. Requires `channels.telegram.threadBindings.spawnSessions` to remain enabled (default: `true`).
    ...
    Template context exposes `MessageThreadId` and `IsForum`. DM chats with `message_thread_id` keep DM routing and reply metadata; with the default `threadReplies: "auto"`, they use thread-aware session keys only when Telegram reports `getMe.has_topics_enabled=true`. Use `threadReplies: "off"`, `"inbound"`, or `"always"` as fixed-policy overrides. Use top-level `channels.telegram.dm.threadReplies` for the account default, or `direct.<chatId>.threadReplies` for one DM.
