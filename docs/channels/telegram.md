...
EasyHub can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, EasyHub retries the same message as plain text.

## Commands (native + custom)

EasyHub registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; EasyHub does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `EasyHub logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). EasyHub can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. EasyHub:
...
Private chats can include `message_thread_id` in some edge cases. EasyHub keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2. Run `EasyHub logs --follow` and look for `from.id`.
...
EasyHub defaults to audio files for backward compatibility.
...
EasyHub defaults to video files.
...
EasyHub supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, EasyHub handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, EasyHub caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.easyhub/telegram/sticker-cache.json`
...
EasyHub uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, EasyHub:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by EasyHub)
...
- Example: `EasyHub message send --channel telegram --target 123456789 --message "hi"`.
...
- `EasyHub channels status` shows a warning when config expects unmentioned group messages.
- `EasyHub channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `EasyHub logs --follow` (look for "skipping group message")
...
- Upgrade to a EasyHub build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
