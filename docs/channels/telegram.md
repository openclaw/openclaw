...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    `webhookPath` must start with `/`, must not be `/healthz` (reserved for the listener health check), and must not contain `#` (fragments are not part of an HTTP request target). Run `openclaw doctor --fix` to repair persisted configuration that predates these constraints.
