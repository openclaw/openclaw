...
- If Telegram rejects the HTML payload, DNA retries the same message as plain text.

## Commands (native + custom)
DNA registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; DNA does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `dna logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.

## Config writes
By default, Telegram is allowed to write config updates triggered by channel events or `/config set|unset`.

This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). DNA can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. DNA:
...
Private chats can include `message_thread_id` in some edge cases. DNA keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2) Run `dna logs --follow` and look for `from.id`.
...
DNA defaults to audio files for backward compatibility.
...
DNA supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, DNA handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, DNA caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.dna/telegram/sticker-cache.json`
...
DNA uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, DNA:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by DNA)
...
- Example: `dna message send --channel telegram --target 123456789 --message "hi"`.
...
- `dna channels status` shows a warning when config expects unmentioned group messages.
- `dna channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `dna logs --follow` (look for "skipping group message")
...
- Upgrade to a DNA build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
