...
<AccordionGroup>
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `Bad Gateway` from `getUpdates`, `deleteWebhook`, or other startup Bot API calls, treat it as a transient Telegram server-side failure. Polling, webhook cleanup, and command-menu setup paths retry server 5xx responses instead of exiting the channel; non-idempotent sends remain conservative to avoid duplicate visible messages.
