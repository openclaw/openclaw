...
Matrix is an open, decentralized messaging protocol. EasyHub connects as a Matrix **user**
...
```bash
EasyHub plugins install @easyhub/matrix
```

Local checkout (when running from a git repo):

```bash
EasyHub plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `EasyHub plugins install @easyhub/matrix`
   - From a local checkout: `EasyHub plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: EasyHub calls the same
     login endpoint, stores the access token in `~/.easyhub/credentials/matrix/credentials.json`,
...
- On first connection, EasyHub requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  EasyHub logs a warning.
...
`~/.easyhub/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, EasyHub resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
