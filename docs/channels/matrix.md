...
Matrix is an open, decentralized messaging protocol. DNA connects as a Matrix **user**
...
```bash
dna plugins install @dna/matrix
```

Local checkout (when running from a git repo):

```bash
dna plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `dna plugins install @dna/matrix`
   - From a local checkout: `dna plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: DNA calls the same
     login endpoint, stores the access token in `~/.dna/credentials/matrix/credentials.json`,
...
- On first connection, DNA requests device verification from your other sessions.
...
`~/.dna/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, DNA resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
