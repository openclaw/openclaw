...
Matrix is an open, decentralized messaging protocol. Activi connects as a Matrix **user**
...
```bash
activi plugins install @activi/matrix
```

Local checkout (when running from a git repo):

```bash
activi plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `activi plugins install @activi/matrix`
   - From a local checkout: `activi plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Activi calls the same
     login endpoint, stores the access token in `~/.activi/credentials/matrix/credentials.json`,
...
- On first connection, Activi requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Activi logs a warning.
...
`~/.activi/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Activi resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
