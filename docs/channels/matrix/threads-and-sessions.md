...
- Native sub-agents and agent-spawned ACP children never bind a thread, including through manual native commands. Delegated results return through the parent; ordinary Matrix threads and user-owned ACP bindings remain supported.
...
`--bind here` does not create a child Matrix thread. `threadBindings.spawnSessions` gates user-run `/acp spawn --thread auto|here`, where OpenClaw needs to create or bind a child thread.
...
- `threadBindings.spawnSessions`: gates user ACP thread spawns (`/acp spawn --thread`). Delegated workers never bind a thread.
...
- `threadBindings.defaultSpawnContext`: deprecated and ignored; accepted for existing config compatibility. Native sub-agents start with isolated context unless the spawn passes `context: "fork"`.

User ACP thread spawns default on. Set `threadBindings.spawnSessions: false` to block `/acp spawn --thread` from creating/binding Matrix threads. Saved legacy bindings to native sub-agents or agent-spawned ACP children are ignored by runtime routing and delivery, not migrated or deleted by a background sweep.
