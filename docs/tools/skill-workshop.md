...
| `Proposal body is empty`                           | The proposal body must contain skill content after frontmatter is stripped. Revise the proposal with complete skill content.                                                                                                                                                                                                                                                                            |
| `Proposal body does not contain valid frontmatter` | The proposal must be complete skill content with valid YAML frontmatter (name, description), not a plan or change description.                                                                                                                                                                                                                                                                          |
| `Update requires explicit approval`                | The proposal body shares insufficient content with the current SKILL.md (low-continuity update). Substantial continuity requires at least 3 shared non-heading body lines and at least 50% of original non-heading content retained. When `approvalPolicy` is `auto`, low-continuity updates require explicit operator approval before apply. Intentional complete rewrites can proceed after approval. |
| Accidental apply overwrites skill content          | The rollback file at `<OPENCLAW_STATE_DIR>/skill-workshop/proposals/<proposal-id>/rollback.json` contains the previous `SKILL.md` content in `previousContent`. Restore it manually or use `openclaw skills workshop restore` when available.                                                                                                                                                           |
