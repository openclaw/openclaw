...
Use `action=create` only for a new workspace skill. The built-in agent tool
rejects create calls, and revisions to pending create proposals, when the
proposed markdown references an existing workspace skill path such as
`skills/<name>/SKILL.md` or `skills/<name>/scripts/...`. Use `action=update`
with `skill_name` for that existing skill. Split multi-skill changes into one
update proposal per live skill.
