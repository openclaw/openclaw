---
summary: "Chrome extension: let SmartAgentNeo drive your existing Chrome tab"
read_when:
...
The SmartAgentNeo Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate smart-agent-neo-managed Chrome profile.
...
SmartAgentNeo then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SmartAgentNeo release (npm package) as static files. There is no separate “build” step.

After upgrading SmartAgentNeo:

- Re-run `smart-agent-neo browser extension install` to refresh the installed files under your SmartAgentNeo state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

SmartAgentNeo ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want SmartAgentNeo to control.
...
- `ON`: attached; SmartAgentNeo can drive that tab.
...
Debugging: `smart-agent-neo sandbox explain`
...
`smart-agent-neo browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `smart-agent-neo browser extension install` first to copy the extension to a stable location under your SmartAgentNeo state directory.
...
- **This is not isolated** like the dedicated smart-agent-neo-managed profile.
