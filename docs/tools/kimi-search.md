...
Mullusi supports Kimi as a `web_search` provider, using Moonshot web search
...
When you choose **Kimi** during `mullusi onboard` or
`mullusi configure --section web`, Mullusi can also ask for:
...
`https://api.moonshot.cn/v1`), Mullusi reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.mullusi/.env`.

If you omit `baseUrl`, Mullusi defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Mullusi defaults to `kimi-k2.5`.
