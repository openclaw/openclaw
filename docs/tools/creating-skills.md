...
<Steps>
  ...
  <Step title="Load the skill">
    Start a new session so Mullusi picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    mullusi agent --message "give me a greeting"
    ```
...
| `metadata.mullusi.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.mullusi.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.mullusi.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `mullusi agent --message "..."` to test before sharing
...
| `~/.mullusi/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Mullusi) | Low        | Global                |
