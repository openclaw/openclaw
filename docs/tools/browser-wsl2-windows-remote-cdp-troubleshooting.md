...
read_when:
  - Running EVOX.sh Gateway in WSL2 while Chrome lives on Windows
...
- EVOX.sh Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/sonpiaz/evox-sh/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- EVOX.sh and Chrome are on the same machine
...
- EVOX.sh points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, EVOX.sh is not the problem yet.
...
Fix that before touching EVOX.sh config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point EVOX.sh at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting EVOX.sh to succeed
...
```bash
evox browser open https://example.com --browser-profile remote
evox browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `evox browser tabs` returns the target
...
3. EVOX.sh config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug EVOX.sh config or Control UI auth
