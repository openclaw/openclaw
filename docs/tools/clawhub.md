---
summary: "ClawHub guide: public registry, native Mullusi install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Mullusi skills and plugins**.

- Use native `mullusi` commands to search/install/update skills and install
...
```bash
mullusi plugins install clawhub:<package>
...
Native `mullusi` commands install into your active workspace and persist source
...
`mullusi plugins install clawhub:...` only accepts installable plugin families.
If a ClawHub package is actually a skill, Mullusi stops and points you at
`mullusi skills install <slug>` instead.

## What ClawHub is

- A public registry for Mullusi skills and plugins.
...
4. Users browse, download, and install skills in Mullusi.
...
If you want to add new capabilities to your Mullusi agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `mullusi skills install <skill-slug>`
3. Start a new Mullusi session so it picks up the new skill.
...
## How it fits into Mullusi

Native `mullusi skills install` installs into the active workspace `skills/`
directory. `mullusi plugins install clawhub:...` records a normal managed
...
Community or other non-official channels can still install, but Mullusi warns
...
current working directory. If an Mullusi workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Mullusi loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.mullusi/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Mullusi how to perform a
...
Interested in becoming a moderator? Ask in the Mullusi Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Mullusi workspace).
...
Code plugins must include the required Mullusi metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/mullusi/skills` and `~/.mullusi/skills`). This is designed to find older skill installs without extra flags.
