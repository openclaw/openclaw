---
summary: "ClawHub guide: public registry, native EVOX.sh install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **EVOX.sh skills and plugins**.

- Use native `evox` commands to search/install/update skills and install
...
```bash
evox plugins install clawhub:<package>
...
Native `evox` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for EVOX.sh skills.
...
4. Users browse, download, and install skills in EVOX.sh.
...
If you want to add new capabilities to your EVOX.sh agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `evox skills install <skill-slug>`
3. Start a new EVOX.sh session so it picks up the new skill.
...
## How it fits into EVOX.sh

Native `evox skills install` installs into the active workspace `skills/`
directory. `evox plugins install clawhub:...` records a normal managed
...
current working directory. If an EVOX.sh workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). EVOX.sh loads workspace skills from `<workspace>/skills`
...
A skill is a versioned bundle of files that teaches EVOX.sh how to perform a
...
Interested in becoming a moderator? Ask in the EVOX.sh Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to EVOX.sh workspace).
