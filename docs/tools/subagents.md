...
- Sub-agent context only injects `AGENTS.md`, `TOOLS.md`, and optional `TOOLS_SHARED.md` (no `SOUL.md`, `IDENTITY.md`, `USER.md`, `MEMORY.md`, `HEARTBEAT.md`, or `BOOTSTRAP.md`). Codex-native subagents follow the same boundary: `TOOLS.md` and `TOOLS_SHARED.md` stay in inherited Codex thread instructions, while parent-only persona, identity, and user files are injected as turn-scoped collaboration instructions so children do not clone them. `TOOLS_SHARED.md` can be a workspace-local file or the exact supported sibling symlink `<agent-workspace>/TOOLS_SHARED.md -> ../shared/TOOLS.md`.
