...
- Hidden native `sessions_spawn` (default runtime, `visible` omitted/false) is always non-blocking: it does not wait for child completion. After context preparation and Gateway dispatch, an ordinary startup acceptance returns `{ status: "accepted", runId, childSessionKey, sessionId }`. `sessionId` is the durable child UUID for those ordinary direct subagent accepts (prefer it over parsing `childSessionKey`). Collector launches retain their existing receipt with `sessionKey` and omit `sessionId`. `visible=true` accepts `{ status, runId, childSessionKey }` via the dashboard path and omit `sessionId`. The supported `runtime="acp"` accept shape remains `{ status, runId, childSessionKey }` and omits `sessionId`. Cloud-worker spawns can wait for provisioning before returning this receipt.
