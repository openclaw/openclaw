...
- Hidden native `sessions_spawn` (default runtime, `visible` omitted/false) is always non-blocking: it does not wait for child completion. After context preparation and Gateway dispatch, startup acceptance returns `{ status: "accepted", runId, childSessionKey, sessionId }`. `sessionId` is the durable child UUID for those direct subagent accepts (prefer it over parsing `childSessionKey`). `visible=true` accepts `{ status, runId, childSessionKey }` via the dashboard path and does not include `sessionId`. `sessionKey` is returned only for collector launches. The supported `runtime="acp"` accept shape remains `{ status, runId, childSessionKey }` and does not include `sessionId`. Cloud-worker spawns can wait for provisioning before returning this receipt.
