...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `activi sandbox explain`.
...
Legacy `agent.*` configs are migrated by `activi doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=activi-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ACTIVI_STATE_DIR:-$HOME/.activi}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
