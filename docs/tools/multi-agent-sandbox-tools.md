...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `EasyHub sandbox explain`.
...
Legacy `agent.*` configs are migrated by `EasyHub doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=EasyHub-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${EASYHUB_STATE_DIR:-$HOME/.EasyHub}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
