---
title: Lobster
summary: "Typed workflow runtime for EVOX.sh with resumable approval gates."
read_when:
...
Lobster is a workflow shell that lets EVOX.sh run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: EVOX.sh runs one Lobster tool call and gets a structured result.
...
EVOX.sh launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In EVOX.sh tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the EVOX.sh Gateway (see the [Lobster repo](https://github.com/openclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), EVOX.sh keeps core tools enabled. To restrict core
...
→ evox calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ evox calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls EVOX.sh tools that do.
