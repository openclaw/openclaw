...
- **gateway host** → `EasyHub` process on the gateway machine
...
`~/.easyhub/exec-approvals.json` directly.

CLI: `EasyHub approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
