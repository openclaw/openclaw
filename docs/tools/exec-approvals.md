...
- **gateway host** → `smart-agent-neo` process on the gateway machine
...
`~/.smart-agent-neo/exec-approvals.json` directly.

CLI: `smart-agent-neo approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
