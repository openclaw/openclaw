...
- **gateway host** → `dna` process on the gateway machine
...
`~/.dna/exec-approvals.json` directly.

CLI: `dna approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
