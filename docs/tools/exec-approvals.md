...
`ask: "always"` in `~/.mullusi/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `mullusi approvals get`, `mullusi approvals get --gateway`, or
`mullusi approvals get --node <id|name|ip>` to inspect the requested policy,
...
- **gateway host** → `mullusi` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Mullusi also tries to bind
...
- requested exec policy in Mullusi config (`tools.exec.*`)
- host-local approvals policy in `~/.mullusi/exec-approvals.json`
...
When `tools.exec.strictInlineEval=true`, Mullusi treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.mullusi/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `mullusi approvals allowlist ...`).
- `mullusi security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `mullusi doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Mullusi still rejects the `env` builtin in safe-bin
...
`~/.mullusi/exec-approvals.json` directly.

CLI: `mullusi approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, Mullusi sends a followup `agent` turn to the same session.
...
topics, Mullusi preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Mullusi prevents the agent from reusing
