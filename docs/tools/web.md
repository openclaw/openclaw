...
Mullusi also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Mullusi plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of Mullusi's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, Mullusi keeps the normal managed `web_search` behavior.
...
If no `provider` is set, Mullusi checks providers in this order and uses the
...
<Note>
  ...
  Mullusi resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
- or omit that field and let Mullusi auto-detect the first ready web-fetch
...
Legacy `tools.web.x_search.*` config is auto-migrated by `mullusi doctor --fix`.
When you choose Grok during `mullusi onboard` or `mullusi configure --section web`,
Mullusi can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, Mullusi does not
...
<Tabs>
  <Tab title="Config file">
    Run `mullusi configure --section web` or set the key directly:
...
optional structured filters. Mullusi only enables the built-in xAI `x_search`
