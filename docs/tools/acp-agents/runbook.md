...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    ...
    - Task maintenance closes terminal or orphaned parent-owned one-shot ACP sessions unless they have a stable session identity, advertised resume/load support, and a completed turn confirming resumable state. Confirmed-resumable one-shots retain metadata after runtime cleanup so their parent can send a follow-up. Persistent ACP sessions are preserved while an active conversation binding remains; stale persistent sessions without an active binding are closed so they cannot be silently resumed after the owning task is done or its task record is gone.
