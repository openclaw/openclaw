...
- `/exec host=<sandbox|gateway|node|remote-ssh|remote-container|remote-k8s-pod> security=<deny|allowlist|full> ask=<off|on-miss|always> node=<id>` (send `/exec` to show current)
