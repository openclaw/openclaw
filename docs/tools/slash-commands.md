...
<AccordionGroup>
  <Accordion title="Sessions and runs">
    ...
    | `/stop` | Abort the current run. On Telegram, replying with `/stop` stops work for the replied-to message |
...
<AccordionGroup>
  <Accordion title="Session scoping per surface">
    ...
    - **`/stop`** targets the active chat session to abort the current run. On Telegram, reply with `/stop` or `/cancel` to stop work for that message; bare Telegram `/stop` keeps targeting the active chat session.
