...
- OpenClaw sets `OPENCLAW_AGENT_ID` and `OPENCLAW_SESSION_KEY` in the spawned command environment so scripts can obtain the agent id and session key without relying on model parameter passing.
