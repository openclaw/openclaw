...
- `auto` is a routing strategy, not a wildcard: it resolves to `sandbox` when a sandbox runtime is active, otherwise `gateway`. Per-call `host=node` and per-call `host=gateway` overrides are rejected when `tools.exec.host=auto` — set `tools.exec.host=node` (or pass `host=node` on a call that uses an explicit non-auto session default) when you need node exec.
