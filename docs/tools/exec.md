...
After binding succeeds, commands with globs or chains that cannot be rebuilt with pinned executable paths still receive review. If allowed, they run as written, like a human `allow-once`, with executable-identity, mutable-file, and working-directory revalidation. Commands that can be rebuilt keep executable path pinning. Node-host auto-review still requires a bound command. Node executable-identity revalidation covers local policy evaluation through dispatch; it does not preserve every inner shell executable's identity across a remote human approval wait. See [Interpreter/runtime commands](/tools/exec-approvals-advanced#interpreter%2Fruntime-commands) for that boundary.

Dispatch through `xcrun`, BusyBox/Toybox applets, or shell `builtin`/`command`/`exec` carriers skips auto-review with `Exec auto-review skipped: dispatch wrapper identity cannot be bound`. Those forms require human approval when existing binding checks succeed; existing binding rejections still apply. Their toolchain, embedded applet, or builtin dispatch identities are outside the ordinary external-file binding contract.

POSIX login or interactive shell wrappers never receive auto-review. When binding succeeds, as with `bash -lc 'printf ok'`, they require human approval because their implicit startup files are outside operand binding. Existing binding rejections still take precedence; interactive forms rejected as code-loading options remain denied. This applies to wrappers in the requested command; the gateway's ordinary shell startup snapshot is unchanged.
