...
- Script preflight checks (for common Python/Node shell-syntax mistakes) only inspect files inside the effective `workdir` boundary. If a script path resolves outside `workdir`, preflight is skipped for that file. At `security=full`, the fail-closed interpreter-invocation heuristic is bypassed regardless of `ask` mode — `ask` controls approval prompts, not trust level. File content validation (shell variable injection checks) still runs whenever a concrete script path can be extracted from the command.
...
| `tools.exec.security`                | `deny` for sandbox, `full` for gateway/node when unset | At `security=full`, the fail-closed interpreter-invocation heuristic in script preflight is bypassed regardless of `ask` mode. File content validation (shell variable injection checks) still runs whenever a concrete script path can be extracted. |
| `tools.exec.ask`                     | `off`                                                  |                                                                                                                                                                                                                                                       |
