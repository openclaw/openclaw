...
- `tools.exec.deniedPaths`: paths exec commands may not reference, including recursive patterns such as `/run/secrets/**`. Use this as defense-in-depth for mounted secrets and tenant-controlled sensitive paths. With `host=node`, home-relative `~` patterns fail closed until the node target provides a trusted `HOME`.
