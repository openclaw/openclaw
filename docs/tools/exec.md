...
- Script preflight checks (for common Python/Node shell-syntax mistakes) inspect files that resolve within or outside the effective `workdir` boundary. If a script path resolves outside `workdir`, preflight now reads and scans that file for shell-syntax variable injection (`$VAR` patterns). Previously such paths were skipped; they are now validated and can be rejected if they contain shell-bleed patterns. Preflight skips entirely when `host=gateway` and the effective policy is `security=full` with `ask=off`.
