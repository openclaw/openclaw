...
- When `tools.exec.applyPatch.workspaceOnly` is unset in that same no-approval full-exec mode, `apply_patch` also inherits host-write authority instead of requiring a separate `tools.exec.applyPatch.workspaceOnly: false` setting. Set `tools.exec.applyPatch.workspaceOnly: true`, or `tools.fs.workspaceOnly: true`, to keep it workspace-contained even when exec is full.
