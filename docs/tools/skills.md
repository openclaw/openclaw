...
`skills.install({ source: "upload", uploadId, slug, force?, sha256? })`. Upload
mode still installs into the default agent workspace `skills/<slug>` directory;
the archive's internal folder name is ignored for the final install target.
This private archive path is disabled unless
`skills.install.allowUploadedArchives: true` is set in `openclaw.json`.
...
- Gateway private archive installs require a committed zip upload containing
  `SKILL.md` and reuse the same archive extraction, path traversal, symlink,
  force, and rollback protections as ClawHub skill installs. They are also
  gated by `skills.install.allowUploadedArchives`.
