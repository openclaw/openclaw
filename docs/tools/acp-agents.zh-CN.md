---
summary: "使用 ACP 运行时会话来运行 Codex、Claude Code、Cursor、Gemini CLI、OpenClaw ACP 和其他 harness 代理"
read_when:
  - 通过 ACP 运行编码 harness
...
如果你用通俗语言要求 OpenClaw "在 Codex 中运行这个"或"在线程中启动 Claude Code"，OpenClaw 应该将该请求路由到 ACP 运行时（而不是原生子代理运行时）。每个 ACP 会话的生成都被跟踪为 [后台任务](/automation/tasks)。

如果你希望 Codex 或 Claude Code 作为外部 MCP 客户端直接连接到现有的 OpenClaw 通道对话，请使用 [`openclaw mcp serve`](/cli/mcp) 而不是 ACP。
...
| 你想要...                                                                     | 使用这个                              | 说明                                                                                                       |
| ---------------------------------------------------------------------------------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| 通过 OpenClaw 运行 Codex、Claude Code、Gemini CLI 或其他外部 harness | 本页：ACP 代理                 | 聊天绑定会话、`/acp spawn`、`sessions_spawn({ runtime: "acp" })`、后台任务、运行时控制 |
| 将 OpenClaw Gateway 会话暴露为 ACP 服务器以供编辑器或客户端使用      | [`openclaw acp`](/cli/acp)            | 桥接模式。IDE/客户端通过 stdio/WebSocket 与 OpenClaw 对话 ACP                                          |
| 重用本地 AI CLI 作为纯文本回退模型                                 | [CLI Backends](/gateway/cli-backends) | 不是 ACP。没有 OpenClaw 工具，没有 ACP 控制，没有 harness 运行时                                             |
...
- 首次使用目标 harness 适配器时，可能会通过 `npx` 按需获取。
- 该 harness 的供应商身份验证仍然必须存在于主机上。
- 如果主机没有 npm/网络访问权限，首次运行的适配器获取可能会失败，直到缓存预热或以其他方式安装适配器。

例子：

- `/acp spawn codex`：OpenClaw 应该准备好引导 `acpx`，但 Codex ACP 适配器可能仍需要首次运行获取。
- `/acp spawn claude`：Claude ACP 适配器的情况相同，加上该主机上的 Claude 端身份验证。
...
- "使用 Gemini CLI 在此线程中执行此任务，然后在同一线程中保持后续操作。"

OpenClaw 应该做什么：

1. 选择 `runtime: "acp"`。
2. 解析请求的 harness 目标（`agentId`，例如 `codex`）。
3. 如果请求当前对话绑定且活动通道支持，则将 ACP 会话绑定到该对话。
4. 否则，如果请求线程绑定且当前通道支持，则将 ACP 会话绑定到线程。
5. 将后续绑定消息路由到同一个 ACP 会话，直到取消聚焦/关闭/过期。

## ACP 与子代理

当你需要外部 harness 运行时时使用 ACP。当你需要 OpenClaw 原生委托运行时使用子代理。
...
- ACP Claude 是具有 ACP 控制、会话恢复、后台任务跟踪和可选对话/线程绑定的 harness 会话。
- CLI 后端是单独的纯文本本地回退运行时。请参阅 [CLI Backends](/gateway/cli-backends)。
...
- `--bind here` 如果你正在生成新工作，仍然可以创建新的 ACP 会话。绑定将该会话附加到当前对话。
- `--bind here` 本身不会创建子 Discord 线程或 Telegram 主题。
- ACP 运行时仍然可以有自己的工作目录（`cwd`）或磁盘上的后端管理工作区。该运行时工作区与聊天表面分离，并不意味着新的消息线程。
- 如果你生成到不同的 ACP 代理且不传递 `--cwd`，OpenClaw 默认继承**目标代理**的工作区，而不是请求者的。
- 如果该继承的工作区路径缺失（`ENOENT`/`ENOTDIR`），OpenClaw 会回退到后端默认 cwd，而不是静默重用错误的树。
- 如果继承的工作区存在但无法访问（例如 `EACCES`），生成会返回实际的访问错误，而不是丢弃 `cwd`。

心智模型：

- 聊天表面：人们保持交谈的地方（`Discord 频道`、`Telegram 主题`、`iMessage 聊天`）
...
- 广告当前对话绑定支持的聊天/消息通道可以通过共享的对话绑定路径使用 `--bind here`。
- 具有自定义线程/主题语义的通道仍然可以在相同的共享接口后面提供通道特定的规范化。
- `--bind here` 始终意味着"就地绑定当前对话"。
- 通用当前对话绑定使用共享的 OpenClaw 绑定存储，并且在正常的网关重启后仍然存在。

注意：

- `--bind here` 和 `--thread ...` 在 `/acp spawn` 上互斥。
- 在 Discord 上，`--bind here` 就地绑定当前频道或线程。只有当 OpenClaw 需要为 `--thread auto|here` 创建子线程时，才需要 `spawnAcpSessions`。
- 如果活动通道不暴露当前对话 ACP 绑定，OpenClaw 会返回明确的不支持消息。
- "resume" 和"新会话"问题是 ACP 会话问题，不是通道问题。你可以重用或替换运行时状态而不更改当前聊天表面。

### 线程绑定会话

当通道适配器启用线程绑定时，ACP 会话可以绑定到线程：
...
- 在绑定的对话中，`/new` 和 `/reset` 就地重置同一个 ACP 会话密钥。
- 临时运行时绑定（例如由线程聚焦流程创建的）在存在时仍然适用。
- 对于没有明确 `cwd` 的跨代理 ACP 生成，OpenClaw 从代理配置继承目标代理工作区。
- 缺失的继承工作区路径回退到后端默认 cwd；真实的访问失败表现为生成错误。
...
- `thread`（可选，默认为 `false`）：在支持的地方请求线程绑定流程。
- `mode`（可选）：`run`（一次性）或 `session`（持久）。
  - 默认是 `run`
  - 如果 `thread: true` 且省略 mode，OpenClaw 可能根据运行时路径默认为持久行为
  - `mode: "session"` 需要 `thread: true`
- `cwd`（可选）：请求的运行时工作目录（由后端/运行时策略验证）。如果省略，ACP 生成在配置时继承目标代理工作区；缺失的继承路径回退到后端默认值，而真实的访问错误被返回。
- `label`（可选）：在会话/横幅文本中使用的操作员面向标签。
- `resumeSessionId`（可选）：恢复现有 ACP 会话而不是创建新会话。代理通过 `session/load` 重放其对话历史。需要 `runtime: "acp"`。
- `streamTo`（可选）：`"parent"` 将初始 ACP 运行进度摘要作为系统事件流式传输回请求者会话。
  - 可用时，接受的响应包括 `streamLogPath`，指向会话范围的 JSONL 日志（`<sessionId>.acp-stream.jsonl`），你可以尾随查看完整的中继历史。

### 恢复现有会话

使用 `resumeSessionId` 继续之前的 ACP 会话，而不是重新开始。代理通过 `session/load` 重放其对话历史，因此它会带着之前内容的完整上下文继续。
...
- 继续你在 CLI 中交互式开始的编码会话，现在通过你的代理无头进行
- 恢复因网关重启或空闲超时而中断的工作
...
- 如果会话 ID 未找到，生成会失败并返回明确的错误 — 不会静默回退到新会话。

### 操作员冒烟测试

当你希望在网关部署后快速实时检查 ACP 生成是否真正端到端工作，而不仅仅是通过单元测试时使用。

推荐的关卡：

1. 验证目标主机上部署的网关版本/提交。
2. 确认部署的源代码在 `src/gateway/sessions-patch.ts` 中包含 ACP 谱系接受（`subagent:* or acp:* sessions`）。
3. 打开临时 ACPX 桥接会话到实时代理（例如 `jpclawhq` 上的 `razor(main)`）。
...
- 保持此冒烟测试在 `mode: "run"`，除非你有意测试线程绑定的持久 ACP 会话。
- 不要为基本关卡要求 `streamTo: "parent"`。该路径取决于请求者/会话能力，是单独的集成检查。
- 将线程绑定的 `mode: "session"` 测试视为来自真实 Discord 线程或 Telegram 主题的第二个更丰富的集成传递。

## 沙盒兼容性

ACP 会话当前在主机运行时上运行，而不是在 OpenClaw 沙盒内部。

当前限制：

- 如果请求者会话被沙盒化，`sessions_spawn({ runtime: "acp" })` 和 `/acp spawn` 的 ACP 生成都会被阻止。
...
当你需要沙盒强制执行时，使用 `runtime: "subagent"`。
...
- 线程绑定生成需要通道策略支持：
  - Discord: `channels.discord.threadBindings.spawnAcpSessions=true`
  - Telegram: `channels.telegram.threadBindings.spawnAcpSessions=true`
- 当你希望固定当前对话而不创建子线程时，使用 `--bind here`。
...
`/acp status` 显示有效的运行时选项，以及在可用时显示运行时级别和后端级别的会话标识符。

一些控件取决于后端能力。如果后端不支持控件，OpenClaw 会返回明确的不支持控件错误。
...
`/acp sessions` 读取当前绑定或请求者会话的存储。接受 `session-key`、`session-id` 或 `session-label` 令牌的命令通过网关会话发现解析目标，包括自定义的每代理 `session.store` 根目录。
...
当 OpenClaw 使用 acpx 后端时，除非你的 acpx 配置定义了自定义代理别名，否则优先使用这些值作为 `agentId`。
如果你的本地 Cursor 安装仍然将 ACP 暴露为 `agent acp`，请在你的 acpx 配置中覆盖 `cursor` 代理命令，而不是更改内置默认值。

直接 acpx CLI 使用也可以通过 `--agent <command>` 目标任意适配器，但那个原始的逃生舱口是 acpx CLI 功能（不是正常的 OpenClaw `agentId` 路径）。
...
```json5
{
  acp: {
    enabled: true,
    // 可选。默认为 true；设置为 false 以暂停 ACP 调度，同时保持 /acp 控制。
...
当前对话绑定不需要子线程创建。它们需要活动的对话上下文和暴露 ACP 对话绑定的通道适配器。

请参阅 [配置参考](/gateway/configuration-reference)。

## acpx 后端的插件设置

全新安装默认启用捆绑的 `acpx` 运行时插件，因此 ACP 通常无需手动插件安装步骤即可工作。
...
如果你禁用了 `acpx`，通过 `plugins.allow` / `plugins.deny` 拒绝了它，或者想要切换到本地开发检出，请使用显式插件路径：
...
当你使用 `npm install -g openclaw` 全局安装 OpenClaw 时，acpx 运行时依赖项（平台特定的二进制文件）通过 postinstall 钩子自动安装。如果自动安装失败，网关仍然正常启动，并通过 `openclaw acp doctor` 报告缺少的依赖项。

### 插件工具 MCP 桥接

默认情况下，ACPX 会话**不**向 ACP harness 公开 OpenClaw 插件注册的工具。

如果你希望 ACP 代理（如 Codex 或 Claude Code）调用已安装的 OpenClaw 插件工具（如内存召回/存储），请启用专用桥接：
...
- 公开已由已安装和启用的 OpenClaw 插件注册的插件工具。
- 保持该功能明确且默认关闭。
...
自定义 `mcpServers` 仍然像以前一样工作。内置插件工具桥接是一个额外的可选便利，不是通用 MCP 服务器配置的替代品。

### 运行时超时配置

捆绑的 `acpx` 插件默认将嵌入式运行时轮次设置为 120 秒超时。这给较慢的 harness（如 Gemini CLI）足够的时间完成 ACP 启动和初始化。如果你的主机需要不同的运行时限制，请覆盖它：
...
这些 ACPX harness 权限与 OpenClaw 执行审批分开，与 CLI 后端供应商绕过标志（如 Claude CLI `--permission-mode bypassPermissions`）分开。ACPX `approve-all` 是 ACP 会话的 harness 级别的紧急开关。
...
控制当权限提示会显示但没有交互式 TTY 可用时会发生什么（这对于 ACP 会话始终是这种情况）。
...
> **重要：** OpenClaw 当前默认为 `permissionMode=approve-reads` 和 `nonInteractivePermissions=fail`。在非交互式 ACP 会话中，任何触发权限提示的写入或执行都可能失败，出现 `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`。
> 
...
| `ACP dispatch is disabled by policy (acp.dispatch.enabled=false)`           | 从正常线程消息的调度禁用。                                  | 设置 `acp.dispatch.enabled=true`。                                                                                                                                  |
...
| `--bind here requires running /acp spawn inside an active ... conversation` | `--bind here` 在没有活动的可绑定对话的情况下使用。                     | 移动到目标聊天/频道并重试，或使用未绑定的生成。                                                                                                  |
| `Conversation bindings are unavailable for <channel>.`                      | 适配器缺乏当前对话 ACP 绑定能力。                      | 在支持的地方使用 `/acp spawn ... --thread ...`，配置顶级 `bindings[]`，或移动到支持的频道。                                              |
...
| `Sandboxed sessions cannot spawn ACP sessions ...`                          | ACP 运行时在主机侧；请求者会话被沙盒化。                       | 从沙盒化会话使用 `runtime="subagent"`，或从非沙盒化会话运行 ACP 生成。                                                                  |
| `sessions_spawn sandbox="require" is unsupported for runtime="acp" ...`     | 为 ACP 运行时请求 `sandbox="require"`。                                  | 对于需要的沙盒使用 `runtime="subagent"`，或从非沙盒化会话使用带有 `sandbox="inherit"` 的 ACP。                                               |
| 绑定会话缺少 ACP 元数据                                      | 过时/已删除的 ACP 会话元数据。                                             | 使用 `/acp spawn` 重新创建，然后重新绑定/聚焦线程。                                                                                                             |
| `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`    | `permissionMode` 在非交互式 ACP 会话中阻止写入/执行。             | 将 `plugins.entries.acpx.config.permissionMode` 设置为 `approve-all` 并重新启动网关。请参阅 [权限配置](#permission-configuration)。                 |
| ACP 会话早期失败，输出很少                                  | 权限提示被 `permissionMode`/`nonInteractivePermissions` 阻止。 | 检查网关日志中的 `AcpRuntimeError`。对于完全权限，设置 `permissionMode=approve-all`；对于优雅降级，设置 `nonInteractivePermissions=deny`。 |
| ACP 会话在完成工作后无限期停滞                       | Harness 进程完成但 ACP 会话未报告完成。             | 使用 `ps aux  grep acpx` 监控；手动终止过时进程。                                                                                                |