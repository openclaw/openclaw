...
- **Wait / upload / dialog** - `wait --url` supports exact, substring, and glob patterns. `wait --load networkidle` is supported on managed `openclaw` and raw/remote CDP profiles, but not on `existing-session` / `user` profiles. Upload hooks require `ref` or `inputRef`, one file at a time, no CSS `element`. Dialog hooks do not support timeout overrides or `dialogId`.
