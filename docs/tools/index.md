---
summary: "Agent tool surface for EasyHub (browser, canvas, nodes, message, cron) replacing legacy `EasyHub-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `EasyHub-*` skills
title: "Tools"
---

# Tools (EasyHub)

EasyHub exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `EasyHub-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `easyhub.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, EasyHub logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`EasyHub node run`).
...
- Requires a Brave API key (recommended: `EasyHub configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated EasyHub-managed browser.
...
- Quick smoke: `EasyHub nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `EasyHub gateway` restart in-place)
