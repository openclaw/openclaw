...
| Sessions and agents     | Manage session settings and groups, delegate work, steer another run, or report status | `sessions`, `sessions_*`, `subagents`, `agents_list`, `session_status`, `goal`   | [Goal](/tools/goal), [Sub-agents](/tools/subagents), [Session tool](/concepts/session-tool) |
