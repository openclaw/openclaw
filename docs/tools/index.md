---
summary: "Agent tool surface for SmartAgentNeo (browser, canvas, nodes, message, cron) replacing legacy `smart-agent-neo-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `smart-agent-neo-*` skills
title: "Tools"
---

# Tools (SmartAgentNeo)

SmartAgentNeo exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `smart-agent-neo-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `smart-agent-neo.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, SmartAgentNeo logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`smart-agent-neo node run`).
...
- Requires a Brave API key (recommended: `smart-agent-neo configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated SmartAgentNeo-managed browser.
...
- Quick smoke: `smart-agent-neo nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `smart-agent-neo gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, SmartAgentNeo clamps `tools.sessions.visibility` to `tree`.
