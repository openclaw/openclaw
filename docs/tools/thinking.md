...
- `stream`: streams reasoning while the reply is generating. Channels with a streaming surface (Telegram, Mattermost) update an in-place preview/draft bubble, then send the final answer without reasoning.
