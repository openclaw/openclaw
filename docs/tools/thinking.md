...
- Levels (aliases): `off | minimal | low | medium | high | xhigh | adaptive | max`
  ...
  - adaptive → provider-managed adaptive thinking (supported for Claude 4.6 on Anthropic/Bedrock, Anthropic Claude Opus 4.7+, Google Gemini dynamic thinking, and MiniMax M3)
...
- Provider notes:
  ...
  - Existing stored unsupported levels are remapped by provider profile rank. `adaptive` falls back to `medium` on non-adaptive models, while `xhigh` and `max` fall back to the largest supported non-off level for the selected model. Non-adaptive budget requests do not fall forward to `adaptive`; providers that support budget levels should advertise them directly.
  ...
  - MiniMax M2.x (`minimax/MiniMax-M2*`) on the Anthropic-compatible streaming path defaults to `thinking: { type: "disabled" }` unless you explicitly set thinking in model params or request params. This avoids leaked `reasoning_content` deltas from M2.x's non-native Anthropic stream format. MiniMax M3 and M3.x expose `off`, the fixed budget levels, and `adaptive`: fixed levels keep Anthropic budget thinking, while `adaptive` uses MiniMax provider-managed thinking. M3 returns empty content when thinking is disabled, so OpenClaw only removes implicit disabled payloads and still preserves explicit `/think off`.
