...
- Provider notes:
  ...
  - Ollama thinking-capable models expose `/think low|medium|high|max`; local Ollama maps `max` to native `think: "high"` while Ollama Cloud (`:cloud` models) keeps native `think: "max"`.
