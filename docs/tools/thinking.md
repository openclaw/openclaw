...
- Provider notes:
  ...
  - Z.AI (`zai/*`) is binary (`on`/`off`) for most GLM models. GLM-5.2 is the exception: it exposes `/think off|low|high|max`, sends `thinking: { type: "enabled" }` for non-off levels, maps `low` and `high` to Z.AI `reasoning_effort: "high"`, maps `max` to `reasoning_effort: "max"`, and sends `thinking: { type: "disabled" }` when off.
