...
- Provider notes:
  ...
  - Direct DeepSeek V4 models expose DeepSeek-native `reasoning_effort` values `low|high|max`; OpenClaw maps `/think minimal|low` to `low`, `/think medium|high|xhigh` to `high`, and `/think max` to `max`.
