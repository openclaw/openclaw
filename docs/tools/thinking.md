...
- Provider notes:
  ...
  - MiniMax M2.x (`minimax/MiniMax-M2*`) on the Anthropic-compatible streaming path defaults to `thinking: { type: "disabled" }` unless you explicitly set thinking in model params or request params. This is an OpenClaw request-level setting, not a provider guarantee: OpenClaw sends the disabled payload, but per MiniMax's Anthropic-compatible API contract M2.x thinking cannot actually be disabled — the `disabled` value is accepted while upstream thinking stays on, so `off` does not ensure upstream reasoning or `reasoning_content` output is suppressed. MiniMax-M3 (and M3.x) is exempt: M3 emits proper Anthropic thinking blocks and returns empty content when thinking is disabled, so OpenClaw keeps M3 on the provider's omitted/adaptive thinking path.
