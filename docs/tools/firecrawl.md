...
EVOX.sh can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `evox configure --section web` enables the bundled Firecrawl plugin automatically.
...
EVOX.sh always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
