...
Mullusi can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `mullusi configure --section web` enables the bundled Firecrawl plugin automatically.
...
Mullusi always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Mullusi
