...
| Codex ACP flips from ChatGPT auth to API-key auth                        | Conflicting `OPENAI_API_KEY` / `CODEX_API_KEY` env vars override Codex session auth even though `~/.codex/auth.json` is already `chatgpt`. | Update to a build that preserves Codex ChatGPT auth, or ensure the ACP spawn path scrubs those API-key env vars only for Codex sessions already using ChatGPT auth. |
