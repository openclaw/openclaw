...
These ACPX harness permissions are separate from OpenClaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--dangerously-skip-permissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
