...
Provider-specific Bocha search settings now live under `plugins.entries.bocha.config.webSearch.*`.
Legacy `tools.web.search.apiKey` still loads through the compatibility shim, but it is no longer the canonical config path.

`webSearch.summary` controls whether Bocha returns the original web content:

- `true` (default): returns original web content for richer LLM context
- `false`: returns snippets and metadata only
