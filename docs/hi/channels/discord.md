...
5. डायरेक्ट चैट्स: डिलीवरी के समय `user:<id>` (या `<@id>` mention) का उपयोग करें; सभी टर्न साझा `main` सत्र में जाते हैं। केवल संख्यात्मक IDs अस्पष्ट होती हैं और अस्वीकार की जाती हैं।
6. Guild चैनल: डिलीवरी के लिए `channel:<channelId>` का उपयोग करें। डिफ़ॉल्ट रूप से mentions आवश्यक हैं और इन्हें प्रति guild या प्रति चैनल सेट किया जा सकता है।
7. डायरेक्ट चैट्स: `channels.discord.dm.policy` के माध्यम से डिफ़ॉल्ट रूप से सुरक्षित (डिफ़ॉल्ट: `"pairing"`)। अज्ञात प्रेषकों को pairing कोड मिलता है (1 घंटे बाद समाप्त); `openclaw pairing approve discord <code>` के माध्यम से अनुमोदित करें।
...
10. वैकल्पिक native commands: `commands.native` डिफ़ॉल्ट रूप से `"auto"` है (Discord/Telegram के लिए चालू, Slack के लिए बंद)। `channels.discord.commands.native: true|false|"auto"` के साथ ओवरराइड करें; `false` पहले से पंजीकृत commands साफ़ करता है। टेक्स्ट commands `commands.text` द्वारा नियंत्रित होते हैं और इन्हें standalone `/...` संदेशों के रूप में भेजना आवश्यक है। commands के लिए access-group जाँच को बायपास करने हेतु `commands.useAccessGroups: false` का उपयोग करें।
    - पूर्ण command सूची + config: [Slash commands](/tools/slash-commands)
11. वैकल्पिक guild संदर्भ इतिहास: mention के उत्तर देते समय अंतिम N guild संदेशों को संदर्भ के रूप में शामिल करने के लिए `channels.discord.historyLimit` (डिफ़ॉल्ट 20, `messages.groupChat.historyLimit` पर fallback) सेट करें। अक्षम करने के लिए `0` सेट करें।
...
टिप्पणी: नाम → id समाधान guild member search का उपयोग करता है और इसके लिए Server Members Intent आवश्यक है; यदि बॉट सदस्यों को खोज नहीं सकता, तो ids या `<@id>` mentions का उपयोग करें।
टिप्पणी: Slugs लोअरकेस होते हैं और स्पेस को `-` से बदला जाता है। चैनल नामों को अग्रणी `#` के बिना slug किया जाता है।
टिप्पणी: Guild संदर्भ `[from:]` पंक्तियों में `author.tag` + `id` शामिल होते हैं ताकि ping-ready उत्तर आसान हों।
...
आमतौर पर आपको **Presence Intent** की आवश्यकता **नहीं** होती। बॉट की अपनी उपस्थिति सेट करना (`setPresence` action) gateway OP3 का उपयोग करता है और इसके लिए इस intent की आवश्यकता नहीं होती; यह केवल तब आवश्यक है जब आप अन्य guild सदस्यों की presence अपडेट प्राप्त करना चाहते हों।
...
मल्टी-अकाउंट समर्थन: प्रति-अकाउंट टोकन और वैकल्पिक `name` के साथ `channels.discord.accounts` का उपयोग करें। साझा पैटर्न के लिए [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) देखें।
...
- Threads parent चैनल config (allowlist, `requireMention`, skills, prompts, आदि) को विरासत में लेते हैं, जब तक आप thread चैनल id को स्पष्ट रूप से न जोड़ें।
- Owner संकेत: जब प्रति-guild या प्रति-चैनल `users` allowlist प्रेषक से मेल खाती है, तो OpenClaw उस प्रेषक को system prompt में owner के रूप में मानता है। चैनलों में एक वैश्विक owner के लिए `commands.ownerAllowFrom` सेट करें।
...
- चेतावनी: यदि आप अन्य बॉट्स को उत्तर देने की अनुमति देते हैं (`channels.discord.allowBots=true`), तो `requireMention`, `channels.discord.guilds.*.channels.<id>.users` allowlists और/या `AGENTS.md` और `SOUL.md` में guardrails साफ़ करके बॉट-टू-बॉट लूप्स से बचें।
...
- `channels.discord.groupPolicy` डिफ़ॉल्ट रूप से **allowlist** है; इसे `"open"` पर सेट करें या `channels.discord.guilds` के अंतर्गत एक guild एंट्री जोड़ें (वैकल्पिक रूप से `channels.discord.guilds.<id>.channels` के अंतर्गत चैनलों की सूची देकर सीमित करें)।
  - यदि आप केवल `DISCORD_BOT_TOKEN` सेट करते हैं और कभी `channels.discord` अनुभाग नहीं बनाते, तो runtime
    `groupPolicy` को `open` पर डिफ़ॉल्ट कर देता है। इसे लॉक डाउन करने के लिए `channels.discord.groupPolicy`,
    `channels.defaults.groupPolicy`, या guild/channel allowlist जोड़ें।
- `requireMention` को `channels.discord.guilds` (या किसी विशिष्ट चैनल) के अंतर्गत होना चाहिए। शीर्ष स्तर पर `channels.discord.requireMention` को अनदेखा किया जाता है।
- **Permission audits** (`channels status --probe`) केवल संख्यात्मक चैनल IDs जाँचते हैं। यदि आप slugs/नामों को `channels.discord.guilds.*.channels` कुंजियों के रूप में उपयोग करते हैं, तो ऑडिट अनुमतियों को सत्यापित नहीं कर सकता।
...
- **Discord में Exec approvals**: Discord DMs में exec approvals के लिए **button UI** का समर्थन करता है (Allow once / Always allow / Deny)। `/approve <id> ...` केवल forwarded approvals के लिए है और Discord के button prompts को हल नहीं करेगा। यदि आपको `❌ Failed to submit approval: Error: unknown approval id` दिखता है या UI कभी दिखाई नहीं देता, तो जाँचें:
...
Outbound Discord API कॉल्स rate limits (429) पर Discord `retry_after` का उपयोग करते हुए, उपलब्ध होने पर, exponential backoff और jitter के साथ पुनः प्रयास करती हैं। `channels.discord.retry` के माध्यम से कॉन्फ़िगर करें। [Retry policy](/concepts/retry) देखें।
...
Ack reactions वैश्विक रूप से `messages.ackReaction` +
`messages.ackReactionScope` द्वारा नियंत्रित होते हैं। बॉट के उत्तर देने के बाद
ack reaction हटाने के लिए `messages.removeAckAfterReply` का उपयोग करें।

- `dm.enabled`: सभी DMs को अनदेखा करने के लिए `false` सेट करें (डिफ़ॉल्ट `true`)।
- `dm.policy`: DM access control (`pairing` अनुशंसित)। `"open"` के लिए `dm.allowFrom=["*"]` आवश्यक है।
- `dm.allowFrom`: DM allowlist (user ids या नाम)। `dm.policy="allowlist"` द्वारा उपयोग किया जाता है और `dm.policy="open"` सत्यापन के लिए। wizard usernames स्वीकार करता है और जब बॉट सदस्यों को खोज सकता है तो उन्हें ids में resolve करता है।
...
- `guilds.<id>.slug`: डिस्प्ले नामों के लिए वैकल्पिक friendly slug।
- `guilds.<id>.users`: वैकल्पिक प्रति-guild user allowlist (ids या नाम)।
- `guilds.<id>.tools`: वैकल्पिक प्रति-guild tool policy overrides (`allow`/`deny`/`alsoAllow`), जब चैनल ओवरराइड अनुपस्थित हो।
- `guilds.<id>.toolsBySender`: guild स्तर पर प्रति-प्रेषक tool policy overrides (चैनल ओवरराइड अनुपस्थित होने पर लागू; `"*"` wildcard समर्थित)।
- `guilds.<id>.channels.<channel>.allow`: `groupPolicy="allowlist"` होने पर चैनल को अनुमति/अस्वीकृति।
- `guilds.<id>.channels.<channel>.requireMention`: चैनल के लिए mention gating।
- `guilds.<id>.channels.<channel>.tools`: वैकल्पिक प्रति-चैनल tool policy overrides (`allow`/`deny`/`alsoAllow`)।
- `guilds.<id>.channels.<channel>.toolsBySender`: चैनल के भीतर प्रति-प्रेषक tool policy overrides (`"*"` wildcard समर्थित)।
- `guilds.<id>.channels.<channel>.users`: वैकल्पिक प्रति-चैनल user allowlist।
- `guilds.<id>.channels.<channel>.skills`: skill फ़िल्टर (omit = सभी skills, empty = कोई नहीं)।
- `guilds.<id>.channels.<channel>.systemPrompt`: चैनल के लिए अतिरिक्त system prompt। Discord चैनल topics **untrusted** संदर्भ के रूप में इंजेक्ट किए जाते हैं (system prompt नहीं)।
- `guilds.<id>.channels.<channel>.enabled`: चैनल अक्षम करने के लिए `false` सेट करें।
- `guilds.<id>.channels`: चैनल नियम (कुंजियाँ चैनल slugs या ids हैं)।
- `guilds.<id>.requireMention`: प्रति-guild mention आवश्यकता (प्रति चैनल ओवरराइड योग्य)।
- `guilds.<id>.reactionNotifications`: reaction system event मोड (`off`, `own`, `all`, `allowlist`)।
- `textChunkLimit`: outbound टेक्स्ट chunk आकार (chars)। डिफ़ॉल्ट: 2000।
...
- `maxLinesPerMessage`: प्रति संदेश सॉफ्ट अधिकतम पंक्ति संख्या। डिफ़ॉल्ट: 17।
...
- `dmHistoryLimit`: user turns में DM इतिहास सीमा। प्रति-उपयोगकर्ता ओवरराइड: `dms["<user_id>"].historyLimit`।
...
- `execApprovals`: Discord-केवल exec approval DMs (button UI)। `enabled`, `approvers`, `agentFilter`, `sessionFilter` का समर्थन करता है।

Reaction सूचनाएँ `guilds.<id>.reactionNotifications` का उपयोग करती हैं:

- `off`: कोई reaction events नहीं।
- `own`: बॉट के अपने संदेशों पर reactions (डिफ़ॉल्ट)।
- `all`: सभी संदेशों पर सभी reactions।
- `allowlist`: `guilds.<id>.users` से आने वाली reactions सभी संदेशों पर (खाली सूची अक्षम करती है)।

### PluralKit (PK) समर्थन

PK lookups सक्षम करें ताकि proxied संदेश underlying system + member में resolve हों।
सक्षम होने पर, OpenClaw allowlists के लिए member identity का उपयोग करता है और
आकस्मिक Discord pings से बचने के लिए प्रेषक को `Member (PK:System)` के रूप में लेबल करता है।
...
- `dm.allowFrom`, `guilds.<id>.users`, या प्रति-चैनल `users` में `pk:<memberId>` का उपयोग करें।
...
- `replyToMode`: `off` (डिफ़ॉल्ट), `first`, या `all`। केवल तब लागू होता है जब मॉडल में reply tag शामिल हो।
...
- `[[reply_to:<id>]]` — संदर्भ/इतिहास से किसी विशिष्ट संदेश id का उत्तर दें।
  वर्तमान संदेश ids prompts में `[message_id: …]` के रूप में जोड़े जाते हैं; इतिहास प्रविष्टियों में पहले से ids शामिल होते हैं।
...
- जब `guilds.<id>.channels` मौजूद हो, तो सूचीबद्ध न किए गए चैनल डिफ़ॉल्ट रूप से अस्वीकृत होते हैं।
- जब `guilds.<id>.channels` छोड़ा जाता है, तो allowlisted guild के सभी चैनल अनुमत होते हैं।
...
Discord संदेश ids injected संदर्भ (`[discord message id: …]` और history पंक्तियाँ) में प्रदर्शित होते हैं ताकि एजेंट उन्हें लक्षित कर सके।
Emoji unicode (जैसे, `✅`) या कस्टम emoji सिंटैक्स जैसे `<:party_blob:1234567890>` हो सकते हैं।
