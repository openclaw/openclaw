...
2. **Socket Mode** → टॉगल ऑन करें। फिर **Basic Information** → **App-Level Tokens** → **Generate Token and Scopes** पर जाएँ, स्कोप `connections:write` के साथ। **App Token** (`xapp-...`) कॉपी करें।
3. **OAuth & Permissions** → bot token scopes जोड़ें (नीचे दिए गए manifest का उपयोग करें)। **Install to Workspace** पर क्लिक करें। **Bot User OAuth Token** (`xoxb-...`) कॉपी करें।
4. वैकल्पिक: **OAuth & Permissions** → **User Token Scopes** जोड़ें (नीचे दी गई read-only सूची देखें)। ऐप को पुनः इंस्टॉल करें और **User OAuth Token** (`xoxp-...`) कॉपी करें।
...
7. Slash Commands → यदि आप `channels.slack.slashCommand` का उपयोग करते हैं तो `/openclaw` बनाएँ। यदि आप native commands सक्षम करते हैं, तो प्रत्येक built-in कमांड के लिए एक slash command जोड़ें (नाम `/help` के समान)। Slack के लिए native डिफ़ॉल्ट रूप से बंद रहता है जब तक आप `channels.slack.commands.native: true` सेट न करें (वैश्विक `commands.native` का डिफ़ॉल्ट `"auto"` है, जो Slack को बंद छोड़ता है)।
...
मल्टी-अकाउंट समर्थन: प्रति-अकाउंट टोकन और वैकल्पिक `name` के साथ `channels.slack.accounts` का उपयोग करें। साझा पैटर्न के लिए [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) देखें।
...
OpenClaw पढ़ने के कार्यों (history,
pins, reactions, emoji, member info) के लिए Slack user token (`xoxp-...`) का उपयोग कर सकता है। डिफ़ॉल्ट रूप से यह read-only रहता है: उपलब्ध होने पर पढ़ने के लिए user token को प्राथमिकता दी जाती है, और लिखने के लिए बॉट टोकन का उपयोग होता है जब तक आप स्पष्ट रूप से अनुमति न दें। `userTokenReadOnly: false` के साथ भी, जब बॉट टोकन उपलब्ध हो तो लिखने के लिए वही प्राथमिक रहता है।

User token को config फ़ाइल में सेट किया जाता है (env var समर्थन नहीं)। मल्टी-अकाउंट के लिए `channels.slack.accounts.<id>.userToken` सेट करें।
...
- लिखने के कार्य (संदेश भेजना/संपादित करना/हटाना, reactions जोड़ना/हटाना, pin/unpin,
  फ़ाइल अपलोड) डिफ़ॉल्ट रूप से bot token का उपयोग करते हैं। यदि `userTokenReadOnly: false` सेट हो और
  bot token उपलब्ध न हो, तो OpenClaw user token पर फ़ॉलबैक करता है।
...
- `messages.groupChat.historyLimit` पर फ़ॉलबैक करता है। निष्क्रिय करने के लिए `0` सेट करें (डिफ़ॉल्ट 50)।

## HTTP मोड (Events API)

जब आपका Gateway HTTPS के माध्यम से Slack द्वारा पहुँचा जा सकता हो (सर्वर डिप्लॉयमेंट में सामान्य), तब HTTP webhook मोड का उपयोग करें।
HTTP मोड एक साझा request URL के साथ Events API + Interactivity + Slash Commands का उपयोग करता है।
...
मल्टी-अकाउंट HTTP मोड: `channels.slack.accounts.<id>.mode = "http"` सेट करें और प्रत्येक अकाउंट के लिए एक अद्वितीय
`webhookPath` प्रदान करें ताकि प्रत्येक Slack ऐप अपने URL की ओर इशारा कर सके।

### Manifest (वैकल्पिक)

ऐप को जल्दी बनाने के लिए इस Slack app manifest का उपयोग करें (यदि चाहें तो नाम/कमांड समायोजित करें)। यदि आप user token कॉन्फ़िगर करने की योजना बनाते हैं, तो user scopes शामिल करें।
...
यदि आप native commands सक्षम करते हैं, तो जिस प्रत्येक कमांड को एक्सपोज़ करना चाहते हैं उसके लिए एक `slash_commands` एंट्री जोड़ें (जो `/help` सूची से मेल खाती हो)। `channels.slack.commands.native` के साथ ओवरराइड करें।

## Scopes (वर्तमान बनाम वैकल्पिक)

Slack का Conversations API प्रकार-आधारित स्कोपिंग का उपयोग करता है: आपको केवल उन्हीं स्कोप्स की आवश्यकता होती है जिन conversation प्रकारों को आप वास्तव में उपयोग करते हैं (channels, groups, im, mpim)। अवलोकन के लिए देखें:
...
Slack केवल Socket Mode का उपयोग करता है (कोई HTTP webhook सर्वर नहीं)। दोनों टोकन प्रदान करें:
...
Ack reactions को वैश्विक रूप से `messages.ackReaction` +
`messages.ackReactionScope` के माध्यम से नियंत्रित किया जाता है। बॉट के उत्तर देने के बाद
...
डिफ़ॉल्ट रूप से, OpenClaw मुख्य चैनल में उत्तर देता है। स्वचालित threading को नियंत्रित करने के लिए `channels.slack.replyToMode` का उपयोग करें:

| Mode    | Behavior                                                                                                                                                   |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `off`   | **डिफ़ॉल्ट।** मुख्य चैनल में उत्तर दें। केवल तब thread करें जब ट्रिगर करने वाला संदेश पहले से thread में हो।                                               |
| `first` | पहला उत्तर thread में जाता है (ट्रिगर संदेश के नीचे), बाद के उत्तर मुख्य चैनल में जाते हैं। संदर्भ को दृश्यमान रखते हुए thread clutter से बचने में उपयोगी। |
| `all`   | सभी उत्तर thread में जाते हैं। बातचीत को सीमित रखता है लेकिन दृश्यता कम हो सकती है।                                                                        |
...
- Native command registration `commands.native` का उपयोग करता है (वैश्विक डिफ़ॉल्ट `"auto"` → Slack बंद) और प्रति-workspace `channels.slack.commands.native` के साथ ओवरराइड किया जा सकता है। Text commands के लिए standalone `/...` संदेश आवश्यक हैं और इन्हें `commands.text: false` के साथ अक्षम किया जा सकता है। Slack slash commands Slack ऐप में प्रबंधित होते हैं और स्वचालित रूप से हटाए नहीं जाते। कमांड्स के लिए access-group जाँच को बायपास करने हेतु `commands.useAccessGroups: false` का उपयोग करें।
...
- `channels.slack.dm.allowFrom` user IDs, @handles, या emails स्वीकार करता है (जब टोकन अनुमति दें तो startup पर resolve किया जाता है)। विज़ार्ड usernames स्वीकार करता है और सेटअप के दौरान उन्हें ids में resolve करता है जब टोकन अनुमति दें।
...
- यदि आप केवल `SLACK_BOT_TOKEN`/`SLACK_APP_TOKEN` सेट करते हैं और कभी `channels.slack` सेक्शन नहीं बनाते,
  तो runtime डिफ़ॉल्ट रूप से `groupPolicy` को `open` पर सेट करता है। इसे लॉक करने के लिए `channels.slack.groupPolicy`,
  `channels.defaults.groupPolicy`, या channel allowlist जोड़ें।
...
Channel विकल्प (`channels.slack.channels.<id>` या `channels.slack.channels.<name>`):
...
- `userTokenReadOnly: false` सेट करने से, जब bot token उपलब्ध न हो, user token को लिखने के
  कार्यों के लिए उपयोग करने की अनुमति मिलती है, जिसका अर्थ है कि क्रियाएँ इंस्टॉल करने वाले
  उपयोगकर्ता की पहुँच के साथ चलती हैं। user token को अत्यधिक विशेषाधिकारयुक्त मानें और
  action gates व allowlists को कड़ा रखें।
...
- बॉट-लिखित संदेश डिफ़ॉल्ट रूप से अनदेखे किए जाते हैं; `channels.slack.allowBots` या `channels.slack.channels.<id>.allowBots` के माध्यम से सक्षम करें।
- चेतावनी: यदि आप अन्य बॉट्स को उत्तर देने की अनुमति देते हैं (`channels.slack.allowBots=true` या `channels.slack.channels.<id>.allowBots=true`), तो `requireMention`, `channels.slack.channels.<id>.users` allowlists, और/या `AGENTS.md` तथा `SOUL.md` में स्पष्ट guardrails के साथ bot-to-bot reply loops को रोकें।
