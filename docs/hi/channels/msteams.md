...
स्थिति: टेक्स्ट + DM अटैचमेंट समर्थित हैं; चैनल/समूह फ़ाइल भेजने के लिए `sharePointSiteId` + Graph अनुमतियाँ आवश्यक हैं (देखें [समूह चैट में फ़ाइल भेजना](#sending-files-in-group-chats))। पोल Adaptive Cards के माध्यम से भेजे जाते हैं।

## प्लगइन आवश्यक

Microsoft Teams एक प्लगइन के रूप में उपलब्ध है और कोर इंस्टॉल में शामिल नहीं है।

**ब्रेकिंग परिवर्तन (2026.1.15):** MS Teams को कोर से बाहर कर दिया गया है। यदि आप इसका उपयोग करते हैं, तो आपको प्लगइन इंस्टॉल करना होगा।
...
टिप्पणी: समूह चैट डिफ़ॉल्ट रूप से ब्लॉक रहती हैं (`channels.msteams.groupPolicy: "allowlist"`)। समूह में उत्तर देने की अनुमति देने के लिए `channels.msteams.groupAllowFrom` सेट करें (या किसी भी सदस्य को अनुमति देने के लिए `groupPolicy: "open"` का उपयोग करें, mention‑gated)।
...
- `channels.msteams.allowFrom` AAD object IDs, UPNs या display names स्वीकार करता है। क्रेडेंशियल्स अनुमति देने पर विज़ार्ड Microsoft Graph के माध्यम से नामों को IDs में बदल देता है।

**समूह प्रवेश**

- डिफ़ॉल्ट: `channels.msteams.groupPolicy = "allowlist"` (जब तक आप `groupAllowFrom` नहीं जोड़ते, तब तक ब्लॉक)। unset होने पर डिफ़ॉल्ट को ओवरराइड करने के लिए `channels.defaults.groupPolicy` का उपयोग करें।
...
> **Deprecation सूचना:** नए multi-tenant बॉट्स का निर्माण 2025-07-31 के बाद deprecated है। नए बॉट्स के लिए **Single Tenant** का उपयोग करें।
...
- fallback: `messages.groupChat.historyLimit`। अक्षम करने के लिए `0` सेट करें (डिफ़ॉल्ट 50)।
- DM history को `channels.msteams.dmHistoryLimit` (user turns) से सीमित किया जा सकता है। प्रति-उपयोगकर्ता overrides: `channels.msteams.dms["<user_id>"].historyLimit`।

## वर्तमान Teams RSC अनुमतियाँ (Manifest)

ये हमारे Teams ऐप मैनिफ़ेस्ट में **मौजूदा resourceSpecific permissions** हैं। ये केवल उसी टीम/चैट के भीतर लागू होती हैं जहाँ ऐप इंस्टॉल है।
...
**निष्कर्ष:** RSC रियल-टाइम सुनने के लिए है; Graph API ऐतिहासिक एक्सेस के लिए। ऑफ़लाइन रहते हुए छूटे संदेशों को पकड़ने के लिए Graph API के साथ `ChannelMessage.Read.All` आवश्यक है (admin consent आवश्यक)।
...
Teams HTTP webhook के माध्यम से संदेश भेजता है। यदि प्रोसेसिंग में अधिक समय लगता है (जैसे धीमे LLM responses), तो आप देख सकते हैं:
...
- `channels.msteams.allowFrom`: DMs के लिए allowlist (AAD object IDs, UPNs या display names)। Graph उपलब्ध होने पर सेटअप के दौरान विज़ार्ड नामों को IDs में resolve करता है।
...
- `channels.msteams.teams.<teamId>.replyStyle`: प्रति-टीम override।
- `channels.msteams.teams.<teamId>.requireMention`: प्रति-टीम override।
- `channels.msteams.teams.<teamId>.tools`: डिफ़ॉल्ट प्रति-टीम टूल नीति overrides (`allow`/`deny`/`alsoAllow`) जो चैनल override न होने पर उपयोग होती हैं।
- `channels.msteams.teams.<teamId>.toolsBySender`: डिफ़ॉल्ट प्रति-टीम प्रति-प्रेषक टूल नीति overrides (`"*"` wildcard समर्थित)।
- `channels.msteams.teams.<teamId>.channels.<conversationId>.replyStyle`: प्रति-चैनल override।
- `channels.msteams.teams.<teamId>.channels.<conversationId>.requireMention`: प्रति-चैनल override।
- `channels.msteams.teams.<teamId>.channels.<conversationId>.tools`: प्रति-चैनल टूल नीति overrides (`allow`/`deny`/`alsoAllow`)।
- `channels.msteams.teams.<teamId>.channels.<conversationId>.toolsBySender`: प्रति-चैनल प्रति-प्रेषक टूल नीति overrides (`"*"` wildcard समर्थित)।
...
**समस्या:** Teams API यह नहीं बताता कि कोई चैनल कौन सा UI स्टाइल उपयोग करता है। यदि आप गलत `replyStyle` उपयोग करते हैं:
...
- **चैनल/समूह:** अटैचमेंट्स M365 स्टोरेज (SharePoint/OneDrive) में रहते हैं। webhook payload में केवल HTML stub होता है, वास्तविक फ़ाइल bytes नहीं। **Graph API अनुमतियाँ आवश्यक हैं** ताकि चैनल अटैचमेंट डाउनलोड किए जा सकें।

Graph अनुमतियों के बिना, इमेज वाले चैनल संदेश केवल टेक्स्ट के रूप में प्राप्त होंगे (इमेज सामग्री बॉट के लिए उपलब्ध नहीं होगी)।
डिफ़ॉल्ट रूप से, OpenClaw केवल Microsoft/Teams होस्टनेम्स से मीडिया डाउनलोड करता है। `channels.msteams.mediaAllowHosts` के साथ ओवरराइड करें (किसी भी होस्ट को अनुमति देने के लिए `["*"]` उपयोग करें)।
Authorization headers केवल `channels.msteams.mediaAuthAllowHosts` में सूचीबद्ध होस्ट्स के लिए जोड़े जाते हैं (डिफ़ॉल्ट Graph + Bot Framework होस्ट्स)। इस सूची को सख्त रखें (multi-tenant suffixes से बचें)।

## समूह चैट में फ़ाइल भेजना

बॉट्स DMs में FileConsentCard flow (built-in) का उपयोग करके फ़ाइल भेज सकते हैं। हालांकि, **समूह चैट/चैनलों में फ़ाइल भेजने** के लिए अतिरिक्त सेटअप आवश्यक है:
...
बॉट्स के पास व्यक्तिगत OneDrive ड्राइव नहीं होता ( `/me/drive` Graph API endpoint application identities के लिए काम नहीं करता)। समूह चैट/चैनलों में फ़ाइल भेजने के लिए, बॉट **SharePoint साइट** पर अपलोड करता है और एक sharing link बनाता है।
...
प्रति-उपयोगकर्ता sharing अधिक सुरक्षित है क्योंकि केवल चैट प्रतिभागी ही फ़ाइल तक पहुँच सकते हैं। यदि `Chat.Read.All` अनुमति अनुपस्थित है, तो बॉट संगठन-व्यापी sharing पर fallback करता है।
...
`card` पैरामीटर एक Adaptive Card JSON ऑब्जेक्ट स्वीकार करता है। जब `card` प्रदान किया जाता है, तो संदेश टेक्स्ट वैकल्पिक होता है।
...
कार्ड schema और उदाहरणों के लिए [Adaptive Cards documentation](https://adaptivecards.io/) देखें। target format विवरण के लिए नीचे [Target formats](#target-formats) देखें।
...
टिप्पणी: `user:` prefix के बिना, नाम डिफ़ॉल्ट रूप से group/team resolution पर जाते हैं। display name द्वारा लोगों को target करते समय हमेशा `user:` का उपयोग करें।
...
Teams URLs में `groupId` query parameter वह team ID **नहीं** है जिसका उपयोग कॉन्फ़िगरेशन में किया जाता है। IDs को URL path से निकालें:
...
- **चैनलों में इमेज नहीं दिख रही:** Graph अनुमतियाँ या admin consent अनुपस्थित। Teams ऐप पुनः इंस्टॉल करें और Teams को पूरी तरह बंद/खोलें।
...
- **Webhook से 401 Unauthorized:** Azure JWT के बिना मैन्युअल परीक्षण पर अपेक्षित — इसका अर्थ endpoint पहुँचा जा सकता है लेकिन auth विफल हुआ। सही परीक्षण के लिए Azure Web Chat उपयोग करें।

### Manifest अपलोड त्रुटियाँ

- **"Icon file cannot be empty":** मैनिफ़ेस्ट 0 bytes वाले icon files को संदर्भित करता है। वैध PNG icons बनाएँ ( `outline.png` के लिए 32x32, `color.png` के लिए 192x192)।
- **"webApplicationInfo.Id already in use":** ऐप अभी किसी अन्य टीम/चैट में इंस्टॉल है। पहले उसे अनइंस्टॉल करें, या propagation के लिए 5-10 मिनट प्रतीक्षा करें।
