...
- लीगेसी `gateway` कमांड को प्रतिस्थापित करती है। CLI एंट्री पॉइंट: `openclaw gateway`।
...
- डिफ़ॉल्ट रूप से `canvasHost.port` (डिफ़ॉल्ट `18793`) पर Canvas फ़ाइल सर्वर शुरू करता है, जो `~/.openclaw/workspace/canvas` से `http://<gateway-host>:18793/__openclaw__/canvas/` सर्व करता है। `canvasHost.enabled=false` या `OPENCLAW_SKIP_CANVAS_HOST=1` से अक्षम करें।
...
- Gateway प्रमाणीकरण डिफ़ॉल्ट रूप से आवश्यक है: `gateway.auth.token` (या `OPENCLAW_GATEWAY_TOKEN`) या `gateway.auth.password` सेट करें। क्लाइंट्स को `connect.params.auth.token/password` भेजना होगा, जब तक कि Tailscale Serve पहचान का उपयोग न हो।
...
आमतौर पर अनावश्यक: एक Gateway कई मैसेजिंग चैनल और एजेंट्स को सेवा दे सकता है। कई Gateway केवल रिडंडेंसी या सख्त आइसोलेशन (उदा: रेस्क्यू बॉट) के लिए उपयोग करें।

समर्थित है यदि आप स्टेट + कॉन्फ़िग को अलग रखें और यूनिक पोर्ट्स का उपयोग करें। पूर्ण मार्गदर्शिका: [Multiple gateways](/gateway/multiple-gateways)।

सेवा नाम प्रोफ़ाइल-अवेयर होते हैं:

- macOS: `bot.molt.<profile>` (लीगेसी `com.openclaw.*` अभी भी मौजूद हो सकता है)
...
रेस्क्यू-बॉट पैटर्न: दूसरे Gateway को उसके अपने प्रोफ़ाइल, स्टेट डायरेक्टरी, वर्कस्पेस और बेस पोर्ट स्पेसिंग के साथ अलग रखें। पूर्ण मार्गदर्शिका: [Rescue-bot guide](/gateway/multiple-gateways#rescue-bot-guide)।
...
- ब्राउज़र प्रोफ़ाइल CDP पोर्ट्स `browser.controlPort + 9 .. + 108` से ऑटो-एलोकेट होते हैं (प्रोफ़ाइल के अनुसार पर्सिस्टेड)।
...
- क्लाइंट से अनिवार्य पहला फ़्रेम: `req {type:"req", id, method:"connect", params:{minProtocol,maxProtocol,client:{id,displayName?,version,platform,deviceFamily?,modelIdentifier?,mode,instanceId?}, caps, auth?, locale?, userAgent? } }`।
...
- संरचित presence एंट्रीज़: `{host, ip, version, platform?, deviceFamily?, modelIdentifier?, mode, lastInputSeconds?, ts, reason?, tags?[], instanceId? }` (WS क्लाइंट्स के लिए, `instanceId` `connect.client.instanceId` से आता है)।
...
- `node.invoke` — किसी नोड पर कमांड इनवोक करें (उदा. `canvas.*`, `camera.*`)।
...
- `shutdown` — Gateway बाहर निकल रहा है; पेलोड में `reason` और वैकल्पिक `restartExpectedMs` शामिल हैं। क्लाइंट्स को पुनः कनेक्ट करना चाहिए।
...
- त्रुटियाँ `{ code, message, details?, retryable?, retryAfterMs? }` का उपयोग करती हैं।
- मानक कोड:
...
- इवेंट्स रीप्ले नहीं किए जाते। क्लाइंट्स seq गैप्स का पता लगाते हैं और जारी रखने से पहले रिफ़्रेश (`health` + `system-presence`) करना चाहिए। WebChat और macOS क्लाइंट्स अब गैप पर ऑटो-रिफ़्रेश करते हैं।
...
- LaunchAgents प्रति-यूज़र होते हैं और लॉग-इन सत्र की आवश्यकता होती है; हेडलेस सेटअप्स के लिए कस्टम LaunchDaemon का उपयोग करें (शिप नहीं किया गया)।
  - `openclaw gateway install` `~/Library/LaunchAgents/bot.molt.gateway.plist` लिखता है
    (या `bot.molt.<profile>.plist`; लीगेसी `com.openclaw.*` साफ़ कर दिया जाता है)।
...
- यदि अन्य gateway-जैसी सेवाएँ मिलती हैं, तो CLI चेतावनी देता है जब तक वे OpenClaw प्रोफ़ाइल सेवाएँ न हों।
  हम अधिकांश सेटअप्स के लिए **प्रति मशीन एक gateway** की ही अनुशंसा करते हैं; रिडंडेंसी या रेस्क्यू बॉट के लिए आइसोलेटेड प्रोफ़ाइल/पोर्ट्स का उपयोग करें। देखें [Multiple gateways](/gateway/multiple-gateways)।
...
- OpenClaw.app Node-आधारित gateway रिले को बंडल कर सकता है और प्रति-यूज़र LaunchAgent लेबल
  `bot.molt.gateway` (या `bot.molt.<profile>`; लीगेसी `com.openclaw.*` लेबल्स भी साफ़-सुथरे ढंग से अनलोड हो जाते हैं) इंस्टॉल करता है।
...
OpenClaw Linux/WSL2 पर डिफ़ॉल्ट रूप से **systemd user service** इंस्टॉल करता है। हम
सिंगल-यूज़र मशीनों के लिए यूज़र सेवाओं की अनुशंसा करते हैं (सरल env, प्रति-यूज़र कॉन्फ़िग)।
मल्टी-यूज़र या हमेशा-ऑन सर्वरों के लिए **system service** का उपयोग करें (lingering आवश्यक नहीं,
शेयर्ड सुपरविज़न)।

`openclaw gateway install` यूज़र यूनिट लिखता है। `openclaw doctor` यूनिट का ऑडिट करता है और
इसे वर्तमान अनुशंसित डिफ़ॉल्ट्स से मेल खाने के लिए अपडेट कर सकता है।
...
Onboarding Linux/WSL2 पर यह चलाता है (sudo के लिए प्रॉम्प्ट आ सकता है; `/var/lib/systemd/linger` लिखता है)।
फिर सेवा सक्षम करें:
...
**वैकल्पिक (system service)** — हमेशा-ऑन या मल्टी-यूज़र सर्वरों के लिए, आप यूज़र यूनिट के बजाय
systemd **system** यूनिट इंस्टॉल कर सकते हैं (lingering आवश्यक नहीं)।
`/etc/systemd/system/openclaw-gateway[-<profile>].service` बनाएँ (ऊपर की यूनिट कॉपी करें,
`WantedBy=multi-user.target` स्विच करें, `User=` + `WorkingDirectory=` सेट करें), फिर:
