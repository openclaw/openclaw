...
- अंतराल: `30m` (या जब Anthropic OAuth/setup-token पहचाना गया प्रमाणीकरण मोड हो तो `1h`)। `agents.defaults.heartbeat.every` या प्रति-एजेंट `agents.list[].heartbeat.every` सेट करें; अक्षम करने के लिए `0m` का उपयोग करें।
- प्रॉम्प्ट बॉडी (`agents.defaults.heartbeat.prompt` के माध्यम से विन्यस्त):
  `Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.`
- Heartbeat प्रॉम्प्ट **जैसा है वैसा** उपयोगकर्ता संदेश के रूप में भेजा जाता है। सिस्टम
  प्रॉम्प्ट में “Heartbeat” अनुभाग शामिल होता है और रन को आंतरिक रूप से फ़्लैग किया जाता है।
- सक्रिय घंटे (`heartbeat.activeHours`) कॉन्फ़िगर किए गए टाइमज़ोन में जाँचे जाते हैं।
  विंडो के बाहर, Heartbeat को विंडो के भीतर अगले टिक तक छोड़ा जाता है।
...
- **मानव चेक-इन**: “Checkup sometimes on your human during day time” दिन के समय कभी-कभार हल्का “क्या आपको कुछ चाहिए?” संदेश भेजने के लिए प्रेरित करता है, लेकिन आपके कॉन्फ़िगर किए गए स्थानीय टाइमज़ोन का उपयोग करके रात में स्पैम से बचता है (देखें [/concepts/timezone](/concepts/timezone))।
...
- Heartbeat रन के दौरान, OpenClaw `HEARTBEAT_OK` को एक ack मानता है जब यह
  उत्तर के **आरंभ या अंत** में दिखाई देता है। टोकन हटा दिया जाता है और शेष सामग्री
  **≤ `ackMaxChars`** (डिफ़ॉल्ट: 300) होने पर उत्तर गिरा दिया जाता है।
...
- `channels.<channel>.accounts.<id>.heartbeat` (मल्टी-अकाउंट चैनल) प्रति-चैनल सेटिंग्स को ओवरराइड करता है।

### प्रति-एजेंट Heartbeat

यदि किसी भी `agents.list[]` प्रविष्टि में `heartbeat` ब्लॉक शामिल है, तो **केवल वही एजेंट**
Heartbeat चलाते हैं। प्रति-एजेंट ब्लॉक `agents.defaults.heartbeat` के ऊपर मर्ज होता है
(ताकि आप साझा डिफ़ॉल्ट एक बार सेट कर सकें और प्रति एजेंट ओवरराइड कर सकें)।
...
इस विंडो के बाहर (पूर्वी समय में सुबह 9 बजे से पहले या रात 10 बजे के बाद), Heartbeat छोड़े जाते हैं। विंडो के भीतर अगला निर्धारित टिक सामान्य रूप से चलेगा।
...
- `accountId`: मल्टी-अकाउंट चैनलों के लिए वैकल्पिक अकाउंट आईडी। जब `target: "last"`, तो अकाउंट आईडी उस हल किए गए अंतिम चैनल पर लागू होती है यदि वह अकाउंट का समर्थन करता है; अन्यथा इसे अनदेखा किया जाता है। यदि अकाउंट आईडी हल किए गए चैनल के लिए कॉन्फ़िगर किए गए अकाउंट से मेल नहीं खाती, तो डिलीवरी छोड़ी जाती है।
...
- `activeHours`: Heartbeat रन को एक समय विंडो तक सीमित करता है। `start` (HH:MM, समावेशी), `end` (HH:MM बहिष्कृत; दिन के अंत के लिए `24:00` अनुमत), और वैकल्पिक `timezone` वाला ऑब्जेक्ट।
...
- Heartbeat डिफ़ॉल्ट रूप से एजेंट के मुख्य सत्र में चलते हैं (`agent:<id>:<mainKey>`),
  या `global` जब `session.scope = "global"`। किसी विशिष्ट चैनल सत्र (Discord/WhatsApp आदि) पर ओवरराइड करने के लिए `session` सेट करें।
- `session` केवल रन संदर्भ को प्रभावित करता है; डिलीवरी `target` और `to` द्वारा नियंत्रित होती है।
- किसी विशिष्ट चैनल/प्राप्तकर्ता पर डिलीवर करने के लिए `target` + `to` सेट करें। `target: "last"` के साथ, डिलीवरी उस सत्र के लिए अंतिम बाहरी चैनल का उपयोग करती है।
...
डिफ़ॉल्ट रूप से, `HEARTBEAT_OK` acknowledgments दबा दिए जाते हैं जबकि अलर्ट सामग्री
डिलीवर की जाती है। आप इसे प्रति चैनल या प्रति अकाउंट समायोजित कर सकते हैं:
...
यदि वर्कस्पेस में `HEARTBEAT.md` फ़ाइल मौजूद है, तो डिफ़ॉल्ट प्रॉम्प्ट एजेंट से उसे पढ़ने के लिए कहता है। इसे अपनी “Heartbeat चेकलिस्ट” की तरह समझें: छोटी, स्थिर, और हर 30 मिनट में शामिल करने के लिए सुरक्षित।

यदि `HEARTBEAT.md` मौजूद है लेकिन प्रभावी रूप से खाली है (केवल खाली पंक्तियाँ और Markdown हेडर जैसे `# Heading`), तो OpenClaw API कॉल बचाने के लिए Heartbeat रन छोड़ देता है। यदि फ़ाइल गायब है, तो Heartbeat फिर भी चलता है और मॉडल तय करता है कि क्या करना है।
...
सक्षम होने पर, Heartbeat एक अलग संदेश भी डिलीवर करेंगे, जिसके पहले
`Reasoning:` होगा (आकार `/reasoning on` जैसा ही)। यह तब उपयोगी हो सकता है जब एजेंट कई सत्रों/कोडेक्स का प्रबंधन कर रहा हो और आप देखना चाहते हों कि उसने आपको पिंग करने का निर्णय क्यों लिया — लेकिन इससे आपकी अपेक्षा से अधिक आंतरिक विवरण भी लीक हो सकता है। समूह चैट में इसे बंद रखना बेहतर है।

## लागत जागरूकता

Heartbeat पूर्ण एजेंट टर्न चलाते हैं। छोटे अंतराल अधिक टोकन खर्च करते हैं। `HEARTBEAT.md` को छोटा रखें और यदि आपको केवल आंतरिक स्थिति अपडेट चाहिए, तो सस्ता `model` या `target: "none"` पर विचार करें।
