...
प्रमाणीकरण प्रोफ़ाइल रोटेशन, कूलडाउन, और उनका फॉलबैक के साथ अंतःक्रिया कैसे होती है, इसके लिए [/concepts/model-failover](/concepts/model-failover) देखें।
त्वरित प्रदाता अवलोकन + उदाहरण: [/concepts/model-providers](/concepts/model-providers)।
...
मॉडल रेफ़्स को lowercase में सामान्यीकृत किया जाता है। प्रदाता उपनाम जैसे `z.ai/*` सामान्यीकृत होकर
`zai/*` बनते हैं।
...
यदि `agents.defaults.models` सेट है, तो यह `/model` और
सत्र ओवरराइड्स के लिए **allowlist** बन जाता है। जब कोई उपयोगकर्ता ऐसा मॉडल चुनता है
जो उस allowlist में नहीं है, तो OpenClaw लौटाता है:

```
Model "provider/model" is not allowed. Use /model to list available models.
```

यह सामान्य उत्तर बनने से **पहले** होता है, इसलिए संदेश ऐसा लग सकता है कि उसने
“उत्तर नहीं दिया।” समाधान के लिए:
...
- मॉडल रेफ़्स को **पहले** `/` पर विभाजित करके पार्स किया जाता है। `/model <ref>` टाइप करते समय `provider/model` का उपयोग करें।
...
डिफ़ॉल्ट रूप से कॉन्फ़िगर किए गए मॉडल दिखाता है। उपयोगी फ़्लैग्स:
...
रिज़ॉल्व किया गया प्राथमिक मॉडल, फॉलबैक, इमेज मॉडल, और कॉन्फ़िगर किए गए प्रदाताओं का
प्रमाणीकरण अवलोकन दिखाता है। यह ऑथ स्टोर में पाए गए प्रोफ़ाइल्स के लिए OAuth
समाप्ति स्थिति भी दिखाता है (डिफ़ॉल्ट रूप से 24 घंटे के भीतर चेतावनी देता है)। `--plain` केवल
रिज़ॉल्व किया गया प्राथमिक मॉडल प्रिंट करता है।
OAuth स्थिति हमेशा दिखाई जाती है (और `--json` आउटपुट में शामिल होती है)। यदि किसी
कॉन्फ़िगर किए गए प्रदाता के पास क्रेडेंशियल्स नहीं हैं, तो `models status` **Missing auth**
अनुभाग प्रिंट करता है।
JSON में `auth.oauth` (चेतावनी विंडो + प्रोफ़ाइल्स) और `auth.providers`
(प्रति प्रदाता प्रभावी प्रमाणीकरण) शामिल होते हैं।
ऑटोमेशन के लिए `--check` का उपयोग करें (exit `1` जब missing/expired हो, `2` जब expiring हो)।
...
प्रोबिंग के लिए OpenRouter API कुंजी आवश्यक है (ऑथ प्रोफ़ाइल्स से या
`OPENROUTER_API_KEY`)। कुंजी के बिना, केवल उम्मीदवारों की सूची के लिए `--no-probe` का उपयोग करें।
...
TTY में चलाने पर, आप इंटरैक्टिव रूप से फॉलबैक चुन सकते हैं। नॉन‑इंटरैक्टिव
मोड में, डिफ़ॉल्ट स्वीकार करने के लिए `--yes` पास करें।

## मॉडल्स रजिस्ट्री (`models.json`)

`models.providers` में कस्टम प्रदाताओं को एजेंट निर्देशिका के अंतर्गत
`models.json` में लिखा जाता है (डिफ़ॉल्ट `~/.openclaw/agents/<agentId>/models.json`)। यह फ़ाइल
डिफ़ॉल्ट रूप से मर्ज की जाती है, जब तक कि `models.mode` को `replace` पर सेट न किया गया हो।
