...
OpenClaw मेमोरी **एजेंट वर्कस्पेस में सादा Markdown** है। फ़ाइलें ही
सत्य का स्रोत हैं; मॉडल केवल वही “याद” रखता है जो डिस्क पर लिखा जाता है।

मेमोरी खोज टूल सक्रिय मेमोरी प्लगइन द्वारा प्रदान किए जाते हैं (डिफ़ॉल्ट:
`memory-core`)। मेमोरी प्लगइन्स को `plugins.slots.memory = "none"` के साथ अक्षम करें।
...
- यह क्षेत्र अभी विकसित हो रहा है। मॉडल को मेमोरी सहेजने की याद दिलाना सहायक है; वह जानता होगा क्या करना है।
- यदि आप चाहते हैं कि कुछ टिके, **बॉट से उसे मेमोरी में लिखने को कहें**।

## Automatic memory flush (pre-compaction ping)

जब कोई सत्र **ऑटो-कम्पैक्शन के क़रीब** होता है, OpenClaw एक **मौन,
एजेंटिक टर्न** ट्रिगर करता है जो मॉडल को **कॉन्टेक्स्ट कम्पैक्ट होने से पहले**
स्थायी मेमोरी लिखने की याद दिलाता है। डिफ़ॉल्ट प्रॉम्प्ट स्पष्ट रूप से कहते हैं कि मॉडल _उत्तर दे सकता है_,
लेकिन सामान्यतः `NO_REPLY` सही प्रतिक्रिया होती है ताकि उपयोगकर्ता को यह टर्न दिखाई न दे।
...
- डिफ़ॉल्ट रूप से रिमोट एम्बेडिंग्स का उपयोग करता है। यदि `memorySearch.provider` सेट नहीं है, तो OpenClaw स्वचालित रूप से चुनता है:
...
रिमोट एम्बेडिंग्स के लिए एम्बेडिंग प्रदाता की API कुंजी **आवश्यक** है। OpenClaw
कुंजियों को auth प्रोफ़ाइल्स, `models.providers.*.apiKey`, या पर्यावरण
चर से सुलझाता है। Codex OAuth केवल चैट/कम्प्लीशन्स को कवर करता है और
मेमोरी खोज के लिए एम्बेडिंग्स को **पूरा नहीं** करता। Gemini के लिए, `GEMINI_API_KEY` या
`models.providers.google.apiKey` का उपयोग करें। Voyage के लिए, `VOYAGE_API_KEY` या
`models.providers.voyage.apiKey` का उपयोग करें। कस्टम OpenAI-संगत एंडपॉइंट का उपयोग करते समय,
`memorySearch.remote.apiKey` सेट करें (और वैकल्पिक `memorySearch.remote.headers`)।

### QMD backend (experimental)

बिल्ट-इन SQLite इंडेक्सर को
[QMD](https://github.com/tobi/qmd) से बदलने के लिए `memory.backend = "qmd"` सेट करें: एक local-first सर्च साइडकार जो
BM25 + वेक्टर + री-रैंकिंग को जोड़ता है। Markdown सत्य का स्रोत बना रहता है; OpenClaw
रीट्रीवल के लिए QMD को शेल-आउट करता है। मुख्य बिंदु:
...
- OS समर्थन: macOS और Linux Bun + SQLite इंस्टॉल होने पर आउट-ऑफ़-द-बॉक्स काम करते हैं। Windows WSL2 के माध्यम से सबसे अच्छा समर्थित है।
...
- खोजें `qmd query --json` के माध्यम से चलती हैं। यदि QMD विफल हो जाता है या बाइनरी गायब है,
  OpenClaw स्वतः बिल्ट-इन SQLite मैनेजर पर फ़ॉलबैक कर देता है ताकि मेमोरी टूल्स
  काम करते रहें।
...
- **पहली खोज धीमी हो सकती है**: QMD पहली `qmd query` रन पर लोकल GGUF मॉडल (री-रैंकर/क्वेरी
  ...
  - यदि आप मॉडल्स को मैन्युअली प्री-डाउनलोड करना चाहते हैं (और वही इंडेक्स वार्म करना चाहते हैं जिसे OpenClaw
    ...
    OpenClaw का QMD स्टेट आपके **state dir** के तहत रहता है (डिफ़ॉल्ट `~/.openclaw`)।
    आप वही XDG vars एक्सपोर्ट करके `qmd` को बिल्कुल उसी इंडेक्स पर पॉइंट कर सकते हैं
    जिन्हें OpenClaw उपयोग करता है:
...
- `scope`: [`session.sendPolicy`](/gateway/configuration#session) जैसा ही स्कीमा।
  डिफ़ॉल्ट DM-only है (`deny` सभी, `allow` डायरेक्ट चैट्स); समूहों/चैनलों में QMD
  हिट्स दिखाने के लिए इसे ढीला करें।
...
- जब `qmd` चलता है, हम `status().backend = "qmd"` टैग करते हैं ताकि डायग्नॉस्टिक्स दिखा सकें कि
  किस इंजन ने परिणाम दिए। यदि QMD सबप्रोसेस बाहर निकलता है या JSON आउटपुट पार्स नहीं हो पाता,
  तो सर्च मैनेजर चेतावनी लॉग करता है और QMD के ठीक होने तक बिल्ट-इन प्रदाता
  (मौजूदा Markdown एम्बेडिंग्स) पर लौट आता है।
...
- `memory_search` `MEMORY.md` + `memory/**/*.md` से Markdown चंक्स (~400 टोकन लक्ष्य, 80-टोकन ओवरलैप) को सेमांटिक रूप से खोजता है। यह स्निपेट टेक्स्ट (~700 अक्षरों तक सीमित), फ़ाइल पाथ, लाइन रेंज, स्कोर, प्रदाता/मॉडल, और क्या हमने लोकल → रिमोट एम्बेडिंग्स पर फ़ॉलबैक किया, लौटाता है। पूर्ण फ़ाइल पेलोड नहीं लौटाया जाता।
- `memory_get` किसी विशिष्ट मेमोरी Markdown फ़ाइल (वर्कस्पेस-रिलेटिव) को पढ़ता है, वैकल्पिक रूप से किसी प्रारंभिक लाइन से और N लाइनों तक। `MEMORY.md` / `memory/` के बाहर के पाथ अस्वीकार किए जाते हैं।
...
- ताज़गी: `MEMORY.md` + `memory/` पर वॉचर इंडेक्स को डर्टी मार्क करता है (debounce 1.5s)। सिंक सत्र प्रारंभ पर, खोज पर, या किसी अंतराल पर शेड्यूल होता है और असिंक्रोनस रूप से चलता है। सत्र ट्रांसक्रिप्ट्स बैकग्राउंड सिंक ट्रिगर करने के लिए डेल्टा थ्रेशहोल्ड्स का उपयोग करते हैं।
- Reindex ट्रिगर्स: इंडेक्स एम्बेडिंग **प्रदाता/मॉडल + एंडपॉइंट फ़िंगरप्रिंट + चंकिंग पैरामीटर्स** संग्रहीत करता है। इनमें से किसी के बदलने पर, OpenClaw स्वतः पूरी स्टोर को रीसेट और रीइंडेक्स करता है।
...
BM25 (फुल-टेक्स्ट) इसके विपरीत है: सटीक टोकन में मजबूत, पैराफ़्रेज़ में कमजोर।
हाइब्रिड खोज व्यावहारिक मध्य मार्ग है: **दोनों रिट्रीवल सिग्नल का उपयोग करें** ताकि
“नेचुरल लैंग्वेज” क्वेरीज़ और “घास के ढेर में सुई” क्वेरीज़—दोनों के लिए अच्छे परिणाम मिलें।
...
यह “IR-थ्योरी परफ़ेक्ट” नहीं है, लेकिन सरल, तेज़ है, और वास्तविक नोट्स पर रिकॉल/प्रिसिजन सुधारने की प्रवृत्ति रखता है।
यदि बाद में और उन्नत होना चाहें, तो सामान्य अगले कदम Reciprocal Rank Fusion (RRF) या स्कोर नॉर्मलाइज़ेशन
(min/max या z-score) हैं।
...
आप वैकल्पिक रूप से **सत्र ट्रांसक्रिप्ट्स** को इंडेक्स कर सकते हैं और उन्हें `memory_search` के माध्यम से दिखा सकते हैं।
यह एक experimental फ़्लैग के पीछे गेटेड है।
...
- सत्र लॉग्स डिस्क पर रहते हैं (`~/.openclaw/agents/<agentId>/sessions/*.jsonl`)। फ़ाइलसिस्टम एक्सेस वाला कोई भी प्रोसेस/यूज़र उन्हें पढ़ सकता है, इसलिए डिस्क एक्सेस को ट्रस्ट बाउंडरी मानें। कड़े आइसोलेशन के लिए, एजेंट्स को अलग OS यूज़र्स या होस्ट्स के तहत चलाएँ।
...
जब sqlite-vec एक्सटेंशन उपलब्ध होता है, OpenClaw एम्बेडिंग्स को
SQLite वर्चुअल टेबल (`vec0`) में स्टोर करता है और वेक्टर डिस्टेंस क्वेरीज़
डेटाबेस में ही करता है। इससे हर एम्बेडिंग को JS में लोड किए बिना खोज तेज़ रहती है।
...
- जब `memorySearch.provider = "local"`, `node-llama-cpp` `modelPath` सुलझाता है; यदि GGUF गायब है तो यह **ऑटो-डाउनलोड** होकर कैश (या `local.modelCacheDir` यदि सेट हो) में जाता है, फिर लोड होता है। डाउनलोड्स रीट्राई पर रिज़्यूम होते हैं।
...
- `remote.headers` OpenAI हेडर्स के साथ मर्ज होते हैं; कुंजी टकराव पर रिमोट जीतता है। OpenAI डिफ़ॉल्ट्स का उपयोग करने के लिए `remote.headers` छोड़ दें।
