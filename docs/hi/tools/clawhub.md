...
ClawHub **OpenClaw के लिए सार्वजनिक Skill रजिस्ट्री** है। यह एक निःशुल्क सेवा है: सभी Skills सार्वजनिक, खुले और साझा करने व पुनः उपयोग के लिए सभी को दिखाई देती हैं। एक Skill मूल रूप से एक फ़ोल्डर होता है जिसमें एक `SKILL.md` फ़ाइल (और सहायक टेक्स्ट फ़ाइलें) होती हैं। आप वेब ऐप में Skills ब्राउज़ कर सकते हैं या CLI का उपयोग करके Skills खोज, इंस्टॉल, अपडेट और प्रकाशित कर सकते हैं।
...
यदि आप अपने OpenClaw एजेंट में नई क्षमताएँ जोड़ना चाहते हैं, तो ClawHub Skills खोजने और इंस्टॉल करने का सबसे आसान तरीका है। आपको यह जानने की आवश्यकता नहीं है कि बैकएंड कैसे काम करता है। आप यह कर सकते हैं:
...
डिफ़ॉल्ट रूप से, CLI Skills को आपके वर्तमान कार्य निर्देशिका के अंतर्गत `./skills` में इंस्टॉल करता है। यदि OpenClaw वर्कस्पेस कॉन्फ़िगर है, तो `clawhub` उस वर्कस्पेस पर फ़ॉलबैक करता है जब तक कि आप `--workdir` (या `CLAWHUB_WORKDIR`) को ओवरराइड न करें। OpenClaw वर्कस्पेस Skills को `<workspace>/skills` से लोड करता है और उन्हें **अगले** सत्र में उपलब्ध कराता है। यदि आप पहले से `~/.openclaw/skills` या बंडल की गई Skills का उपयोग करते हैं, तो वर्कस्पेस Skills को प्राथमिकता मिलती है।
...
एक Skill फ़ाइलों का एक संस्करणित बंडल होता है जो OpenClaw को किसी विशिष्ट कार्य को करने का तरीका सिखाता है। प्रत्येक प्रकाशन एक नया संस्करण बनाता है, और रजिस्ट्री संस्करणों का इतिहास रखती है ताकि उपयोगकर्ता परिवर्तनों का ऑडिट कर सकें।
...
ClawHub डिस्कवरी को सक्षम करने और Skill क्षमताओं को सुरक्षित रूप से उजागर करने के लिए मेटाडेटा का उपयोग करता है। रजिस्ट्री रैंकिंग और दृश्यता में सुधार के लिए उपयोग संकेतों (जैसे स्टार्स और डाउनलोड्स) को भी ट्रैक करती है।
...
ClawHub डिफ़ॉल्ट रूप से खुला है। कोई भी Skills अपलोड कर सकता है, लेकिन प्रकाशित करने के लिए GitHub खाता कम से कम एक सप्ताह पुराना होना चाहिए। यह वैध योगदानकर्ताओं को अवरुद्ध किए बिना दुरुपयोग को धीमा करने में मदद करता है।
...
मॉडरेटर बनना चाहते हैं? OpenClaw Discord में पूछें और किसी मॉडरेटर या मेंटेनर से संपर्क करें।
...
अपडेट्स स्थानीय Skill सामग्री की तुलना रजिस्ट्री संस्करणों से एक कंटेंट हैश का उपयोग करके करते हैं। यदि स्थानीय फ़ाइलें किसी प्रकाशित संस्करण से मेल नहीं खातीं, तो CLI ओवरराइट करने से पहले पूछता है (या नॉन‑इंटरैक्टिव रन में `--force` की आवश्यकता होती है)।

### सिंक स्कैनिंग और फ़ॉलबैक रूट्स

`clawhub sync` पहले आपकी वर्तमान workdir को स्कैन करता है। यदि कोई Skills नहीं मिलतीं, तो यह ज्ञात लेगेसी स्थानों पर फ़ॉलबैक करता है (उदाहरण के लिए `~/openclaw/skills` और `~/.openclaw/skills`)। यह अतिरिक्त फ़्लैग्स के बिना पुराने Skill इंस्टॉल खोजने के लिए डिज़ाइन किया गया है।
...
जब आप लॉग‑इन रहते हुए `clawhub sync` चलाते हैं, तो CLI इंस्टॉल काउंट की गणना के लिए एक न्यूनतम स्नैपशॉट भेजता है। आप इसे पूरी तरह अक्षम कर सकते हैं:
