...
यह आपके व्यक्तिगत ब्राउज़र से अलग-थलग होती है और Gateway के अंदर एक छोटे स्थानीय
नियंत्रण सेवा के माध्यम से प्रबंधित की जाती है (केवल loopback)।
...
यह ब्राउज़र आपका दैनिक ड्राइवर **नहीं** है। यह एजेंट ऑटोमेशन और सत्यापन के लिए
एक सुरक्षित, अलग-थलग सतह है।
...
- ब्राउज़र नियंत्रण सेवा loopback पर `gateway.port` से व्युत्पन्न पोर्ट से बाइंड होती है
  (डिफ़ॉल्ट: `18791`, जो gateway + 2 है)। रिले अगला पोर्ट उपयोग करता है (`18792`)।
...
- डिफ़ॉल्ट प्रोफ़ाइल `chrome` (एक्सटेंशन रिले) है। प्रबंधित ब्राउज़र के लिए `defaultProfile: "openclaw"` का उपयोग करें।
...
तो OpenClaw इसे स्वचालित रूप से उपयोग करता है। ऑटो-डिटेक्शन ओवरराइड करने के लिए
`browser.executablePath` सेट करें:
...
- **रिमोट CDP:** रिमोट Chromium-आधारित ब्राउज़र से अटैच करने के लिए `browser.profiles.<name>.cdpUrl` (या `browser.cdpUrl`) सेट करें। इस स्थिति में OpenClaw लोकल ब्राउज़र लॉन्च नहीं करेगा।
...
OpenClaw `/json/*` एंडपॉइंट्स को कॉल करते समय और CDP WebSocket से कनेक्ट करते समय
auth को संरक्षित रखता है। config फ़ाइलों में कमिट करने के बजाय
पर्यावरण चर या सीक्रेट्स मैनेजर का उपयोग करना बेहतर है।

## Node browser proxy (ज़ीरो-कॉन्फ़िग डिफ़ॉल्ट)

यदि आप उस मशीन पर **node होस्ट** चलाते हैं जहाँ आपका ब्राउज़र है, तो OpenClaw
किसी अतिरिक्त ब्राउज़र config के बिना ब्राउज़र टूल कॉल्स को उस node पर स्वतः रूट कर सकता है।
यह रिमोट gateways के लिए डिफ़ॉल्ट पथ है।
...
[Browserless](https://browserless.io) एक होस्टेड Chromium सेवा है जो
HTTPS पर CDP एंडपॉइंट्स एक्सपोज़ करती है। आप OpenClaw ब्राउज़र प्रोफ़ाइल को
Browserless क्षेत्रीय एंडपॉइंट की ओर इंगित कर सकते हैं और अपनी API कुंजी से प्रमाणीकरण कर सकते हैं।
...
OpenClaw कई नामित प्रोफ़ाइल्स (routing configs) का समर्थन करता है। प्रोफ़ाइल्स हो सकती हैं:
...
यदि एजेंट सत्र sandboxed है, तो `browser` टूल डिफ़ॉल्ट रूप से `target="sandbox"` (sandbox ब्राउज़र) पर जा सकता है।
Chrome एक्सटेंशन रिले takeover के लिए होस्ट ब्राउज़र नियंत्रण आवश्यक है, इसलिए या तो:
...
कुछ विशेषताएँ (navigate/act/AI snapshot/role snapshot, element screenshots, PDF)
Playwright की आवश्यकता होती है। यदि Playwright इंस्टॉल नहीं है, तो वे एंडपॉइंट्स
स्पष्ट 501 त्रुटि लौटाते हैं। openclaw-managed Chrome के लिए ARIA स्नैपशॉट्स और
बुनियादी स्क्रीनशॉट्स अभी भी काम करते हैं।
Chrome एक्सटेंशन रिले ड्राइवर के लिए, ARIA स्नैपशॉट्स और स्क्रीनशॉट्स को Playwright चाहिए।
...
यदि आपका Gateway Docker में चलता है, तो `npx playwright` से बचें (npm override conflicts)।
इसके बजाय बंडल्ड CLI का उपयोग करें:
...
ब्राउज़र डाउनलोड्स को स्थायी रखने के लिए `PLAYWRIGHT_BROWSERS_PATH` सेट करें (उदाहरण के लिए,
`/home/node/.cache/ms-playwright`) और सुनिश्चित करें कि `/home/node` को
`OPENCLAW_HOME_VOLUME` या bind mount के माध्यम से persist किया गया है।
देखें [Docker](/install/docker)।
...
सभी कमांड्स किसी विशिष्ट प्रोफ़ाइल को लक्षित करने के लिए `--browser-profile <name>` स्वीकार करते हैं।
सभी कमांड्स मशीन-पठनीय आउटपुट (स्थिर payloads) के लिए `--json` भी स्वीकार करते हैं।
...
- `click`/`type`/आदि के लिए `snapshot` से एक `ref` आवश्यक है (या तो संख्यात्मक `12` या role ref `e12`)।
  क्रियाओं के लिए CSS selectors जानबूझकर समर्थित नहीं हैं।
...
जब कोई क्रिया विफल होती है (उदा. “not visible”, “strict mode violation”, “covered”):
...
- `browser act kind=evaluate` / `openclaw browser evaluate` और `wait --fn`
  पेज कॉन्टेक्स्ट में मनमाना JavaScript निष्पादित करते हैं। प्रॉम्प्ट इंजेक्शन इसे निर्देशित कर सकता है।
  यदि आवश्यकता न हो तो `browser.evaluateEnabled=false` के साथ इसे अक्षम करें।
