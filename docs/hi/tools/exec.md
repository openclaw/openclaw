...
वर्कस्पेस में शेल कमांड चलाएँ। `process` के माध्यम से foreground + background निष्पादन का समर्थन करता है।
यदि `process` की अनुमति नहीं है, तो `exec` सिंक्रोनस रूप से चलता है और `yieldMs`/`background` को अनदेखा करता है।
बैकग्राउंड सत्र प्रति एजेंट सीमित होते हैं; `process` केवल उसी एजेंट के सत्र देखता है।
...
- महत्वपूर्ण: sandboxing **डिफ़ॉल्ट रूप से बंद** है। यदि sandboxing बंद है, तो `host=sandbox` सीधे Gateway होस्ट पर चलता है (कोई कंटेनर नहीं) और **अनुमोदन की आवश्यकता नहीं होती**। अनुमोदन आवश्यक करने के लिए, `host=gateway` के साथ चलाएँ और exec अनुमोदन कॉन्फ़िगर करें (या sandboxing सक्षम करें)।
...
- `host=gateway`: आपके login-shell के `PATH` को exec वातावरण में मर्ज करता है। होस्ट निष्पादन के लिए `env.PATH` ओवरराइड्स अस्वीकार किए जाते हैं। डेमन स्वयं न्यूनतम `PATH` के साथ चलता है:
  - macOS: `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin`, `/bin`
  - Linux: `/usr/local/bin`, `/usr/bin`, `/bin`
- `host=sandbox`: कंटेनर के भीतर `sh -lc` (login shell) चलाता है, इसलिए `/etc/profile` `PATH` को रीसेट कर सकता है।
  OpenClaw प्रोफ़ाइल sourcing के बाद एक आंतरिक env var के माध्यम से `env.PATH` prepend करता है (कोई shell interpolation नहीं);
  `tools.exec.pathPrepend` यहाँ भी लागू होता है।
- `host=node`: केवल वे non-blocked env ओवरराइड्स जिन्हें आप पास करते हैं, node को भेजे जाते हैं। होस्ट निष्पादन के लिए `env.PATH` ओवरराइड्स अस्वीकार किए जाते हैं। Headless node hosts `PATH` को केवल तब स्वीकार करते हैं जब वह node host PATH को prepend करता है (कोई replacement नहीं)। macOS nodes `PATH` ओवरराइड्स को पूरी तरह हटा देते हैं।
...
`/exec` का उपयोग करके `host`, `security`, `ask`, और `node` के लिए **प्रति-सत्र** डिफ़ॉल्ट सेट करें।
वर्तमान मान दिखाने के लिए बिना आर्ग्युमेंट के `/exec` भेजें।
...
`/exec` केवल **अधिकृत प्रेषकों** (चैनल allowlists/pairing तथा `commands.useAccessGroups`) के लिए मान्य होता है।
यह केवल **session state** को अपडेट करता है और config नहीं लिखता। exec को हार्ड-डिसेबल करने के लिए, टूल
policy (`tools.deny: ["exec"]` या प्रति-एजेंट) के माध्यम से इसे अस्वीकार करें। होस्ट अनुमोदन तब तक लागू रहते हैं जब तक आप स्पष्ट रूप से
`security=full` और `ask=off` सेट नहीं करते।

## Exec approvals (companion app / node host)

Sandboxed एजेंट Gateway या node host पर `exec` चलने से पहले प्रति-रिक्वेस्ट अनुमोदन की आवश्यकता रख सकते हैं।
नीति, allowlist, और UI फ़्लो के लिए [Exec approvals](/tools/exec-approvals) देखें।

जब अनुमोदन आवश्यक होते हैं, तो exec टूल तुरंत
`status: "approval-pending"` और एक approval id के साथ लौटता है। अनुमोदित (या अस्वीकृत / समय-सीमा समाप्त) होने पर,
Gateway सिस्टम इवेंट्स (`Exec finished` / `Exec denied`) उत्सर्जित करता है। यदि कमांड
`tools.exec.approvalRunningNoticeMs` के बाद भी चल रही हो, तो एकल `Exec running` सूचना उत्सर्जित की जाती है।

## Allowlist + safe bins

Allowlist प्रवर्तन **केवल resolved binary paths** से मेल करता है (basename मैच नहीं)। जब
`security=allowlist`, शेल कमांड तभी स्वतः allow होती हैं जब हर pipeline सेगमेंट
allowlisted या safe bin हो। Chaining (`;`, `&&`, `||`) और redirections
allowlist मोड में अस्वीकार किए जाते हैं।
...
`apply_patch` संरचित multi-file edits के लिए `exec` का एक subtool है।
