...
The onboarding wizard is the **recommended** way to set up DNA on macOS,
...
`dna dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
...
(`web_fetch` works without a key). Easiest path: `dna configure --section web`
...
1) **Existing config detection**
   - If `~/.dna/dna.json` exists, choose **Keep / Modify / Reset**.
   - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
     (or pass `--reset`).
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `dna doctor` before continuing.
...
2) **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.dna/.env` so launchd can read it.
  ...
  - OAuth credentials live in `~/.dna/credentials/oauth.json`; auth profiles live in `~/.dna/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `dna pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `dna health`.
   - Tip: `dna status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `dna agents add <name>` to create a separate agent with its own workspace,
...
- Stores it under `~/.dna/tools/signal-cli/<version>/`.
...
Typical fields in `~/.dna/dna.json`:
...
`dna agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.dna/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.dna/agents/<agentId>/sessions/`.
