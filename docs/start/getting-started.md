...
Fastest chat: open the Control UI (no channel setup needed). Run `dna dashboard`
and chat in the browser, or open `http://127.0.0.1:18789/` on the gateway host.
Docs: [Dashboard](/web/dashboard) and [Control UI](/web/control-ui).

Recommended path: use the **CLI onboarding wizard** (`dna onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `dna configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.dna/credentials/oauth.json`
...
If you’re hacking on DNA itself, run from source:
...
```bash
node dna.mjs gateway --port 18789 --verbose
```
...
If `dna health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `dna status --all` is the best pasteable, read-only debug report.
Health probes: `dna health` (or `dna status --deep`) asks the running gateway for a health snapshot.
