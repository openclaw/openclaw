...
When onboarding finishes, we auto-open the dashboard and print a clean (non-tokenized) link. If it prompts for auth, paste the token from `gateway.auth.token` into Navigation > Overview > Gateway Access. To reopen later: `openclaw dashboard`.
