...
- Host-local sends still only allow media and safe document types (images, audio, video, PDF, Office documents, common archives, APKs, CSV, Markdown, and validated `.txt/.json/.yaml/.yml` files). HTML, `.log`, and binary lookalikes under text extensions stay blocked.
