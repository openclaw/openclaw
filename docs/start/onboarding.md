---
summary: "First-run onboarding flow for DNA (macOS app)"
read_when:
...
- Writes credentials to `~/.dna/credentials/oauth.json`
...
The app can install the global `dna` CLI via npm/pnpm so terminal
...
On the first agent run, DNA bootstraps a workspace (default `~/clawd`):
...
```bash
dna webhooks gmail setup --account you@gmail.com
```
