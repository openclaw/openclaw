...
Gemini CLI, Antigravity, Pi, and OpenCode remain available for normal use after
setup. Their installed CLIs are shown for context when they cannot be selected
as the reusable guided-setup inference route. Gemini and Antigravity cannot
enforce the tool-free inference probe. Pi and OpenCode are whole-agent
harnesses; Pi sessions and OpenCode ACP sessions stay available after setup.
