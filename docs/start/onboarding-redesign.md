...
| 3   | Browser-first handoff (GUI-session detection, wait-for-dashboard-connect, TUI as fallback)                                                                                | CLI → web            | implemented on [`claude/onboard-browser-handoff`](https://github.com/openclaw/openclaw/compare/main...claude/onboard-browser-handoff); PR pending operator creation |
