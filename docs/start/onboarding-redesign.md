...
| 3   | Browser-first handoff (GUI-session detection, wait-for-dashboard-connect, TUI as fallback)                                                                                | CLI → web            | draft on [`claude/onboard-browser-handoff`](https://github.com/openclaw/openclaw/compare/main...claude/onboard-browser-handoff) — **held**: presence-read auth works on SecretRef local gateways but is rejected with a token mismatch on fresh quickstart gateways (falls back to the terminal hatch); needs gateway token-resolution follow-up before enabling by default |
