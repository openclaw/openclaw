...
고정되지 않으면 라운드 로빈으로 인해 메시지 간에 전환될 수 있습니다.
단일 프로필을 강제하려면 다음 중 하나를 사용하십시오:
...
페일오버 대상이지만, 보통 일시적이지 않습니다.
짧은 쿨다운 대신 OpenClaw 는 프로필을 **비활성화**(더 긴 백오프 적용)로 표시하고
다음 프로필/프로바이더로 회전합니다.
...
`agents.defaults.model.fallbacks` 에서 다음 모델로 이동합니다.
이는 인증 실패, 속도 제한, 그리고 프로필 회전을 소진한 타임아웃에 적용됩니다
