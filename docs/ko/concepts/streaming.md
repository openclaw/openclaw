...
블록 스트리밍이 활성화되면, OpenClaw 는 전송 전에 **연속된 블록 청크를 병합**할 수 있습니다.
이는 점진적 출력을 유지하면서도 '단일 줄 스팸'을 줄입니다.
...
블록 스트리밍이 활성화되면, 블록 응답 사이(첫 번째 블록 이후)에 **무작위 지연**을 추가할 수 있습니다.
이는 여러 버블로 구성된 응답이 더 자연스럽게 느껴지도록 합니다.
