...
- `openclaw status` → 구성된 채널이 표시되고 명백한 인증 오류가 표시되지 않습니다.
- `openclaw status --all` → 전체 보고서가 존재하며 공유 가능합니다.
- `openclaw gateway probe` → 예상 게이트웨이 대상에 도달할 수 있습니다.
- `openclaw gateway status` → `Runtime: running` 그리고 `RPC probe: ok`.
- `openclaw doctor` → 차단 구성/서비스 오류가 없습니다.
- `openclaw channels status --probe` → 채널 보고 `connected` 또는 `ready`.
- `openclaw logs --follow` → 꾸준한 활동, 반복되는 치명적인 오류 없음.
