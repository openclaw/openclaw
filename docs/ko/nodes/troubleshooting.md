...
- 노드가 연결되어 역할에 대해 페어링되었습니다. `node`.
- `nodes describe` 호출하는 기능이 포함되어 있습니다.
- Exec 승인에는 예상 모드/허용 목록이 표시됩니다.

## 전경 요구 사항

`canvas.*`, `camera.*`, 그리고 `screen.*` iOS/Android 노드에서만 포그라운드입니다.
...
당신이 본다면 `NODE_BACKGROUND_UNAVAILABLE`, 노드 앱을 포그라운드로 가져오고 다시 시도하세요.
...
1. **장치 페어링**: 이 노드가 게이트웨이에 연결될 수 있나요?
2. **임원 승인**: 이 노드가 특정 쉘 명령을 실행할 수 있습니까?
...
페어링이 잘 되었지만 `system.run` 실패하면 실행 승인/허용 목록을 수정하세요.

## 공통 노드 오류 코드

- `NODE_BACKGROUND_UNAVAILABLE` → 앱이 백그라운드에서 실행됩니다. 그것을 전경으로 가져 오십시오.
- `CAMERA_DISABLED` → 노드 설정에서 카메라 토글이 비활성화되었습니다.
- `*_PERMISSION_REQUIRED` → OS 권한이 누락/거부되었습니다.
- `LOCATION_DISABLED` → 위치 모드가 꺼져 있습니다.
- `LOCATION_PERMISSION_REQUIRED` → 요청한 위치 모드가 승인되지 않았습니다.
...
- [/노드/색인](/nodes/index)
- [/노드/카메라](/nodes/camera)
- [/노드/위치-명령](/nodes/location-command)
