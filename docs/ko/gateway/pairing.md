...
Gateway 소유 페어링에서는 **Gateway**가 어떤 노드의 참여를 허용할지에 대한 단일 진실 공급원입니다.
UI (macOS 앱, 향후 클라이언트)는 보류 중인 요청을 승인하거나 거부하는 프런트엔드 역할만 합니다.
...
- **전송**: Gateway WS 엔드포인트는 요청을 전달하지만 멤버십을 결정하지 않습니다.
  (레거시 TCP 브리지 지원은 사용 중단/제거되었습니다.)
