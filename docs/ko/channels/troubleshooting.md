...
| 다이렉트 메시지 응답 누락        | `openclaw pairing list discord`    | 다이렉트 메시지 페어링을 승인하거나 다이렉트 메시지 정책을 조정하십시오. |
...
| 증상                               | 가장 빠른 확인                     | 해결 방법                                                |
| ---------------------------------- | ---------------------------------- | -------------------------------------------------------- |
| 데몬은 도달 가능하지만 봇이 침묵함 | `openclaw channels status --probe` | `signal-cli` 데몬 URL/계정 및 수신 모드를 확인하십시오.  |
...
| 다이렉트 메시지가 처리되지 않음   | `openclaw pairing list matrix`     | 발신자를 승인하거나 다이렉트 메시지 정책을 조정하십시오. |
