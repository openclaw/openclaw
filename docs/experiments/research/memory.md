...
read_when:
  - Designing workspace memory (~/.activi/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Activi integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.activi/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.activi/workspace` as canonical human-readable memory.
...
```
~/.activi/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Activi**, but keep a separable core library.

### Why integrate into Activi?

- Activi already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `activi memory recall "…" --k 25 --since 30d`
  - `activi memory reflect --since 7d`
...
Pragmatic take for `~/.activi/workspace`:
