---
summary: "Research notes: offline memory system for Neo workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.smart-agent-neo/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SmartAgentNeo integration
...
Target: Neo-style workspace (`agents.defaults.workspace`, default `~/.smart-agent-neo/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.smart-agent-neo/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Neo to see every session.
...
```
~/.smart-agent-neo/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SmartAgentNeo**, but keep a separable core library.

### Why integrate into SmartAgentNeo?

- SmartAgentNeo already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `smart-agent-neo memory recall "…" --k 25 --since 30d`
  - `smart-agent-neo memory reflect --since 7d`
...
Pragmatic take for `~/.smart-agent-neo/workspace`:
