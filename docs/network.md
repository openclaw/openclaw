...
This hub links the core docs for how Mullusi connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`mullusi gateway`), a single long-running process that owns channel connections and the WebSocket control plane.

- **Loopback first**: the Gateway WS defaults to `ws://127.0.0.1:18790`.
...
- **Canvas host** is served on the same port as the Gateway (`/__mullusi__/canvas/`, `/__mullusi__/a2ui/`), protected by Gateway auth when bound beyond loopback.
