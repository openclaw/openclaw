...
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `preflight`                      | Detect changed scopes, build the CI manifest, and run selected PR baseline ratchets before fanout; dependency setup retains the existing cache trust policy                                                                                                                                              | Always on non-draft pushes and PRs                    |
...
| `checks-baseline-ratchets`       | Standalone baseline ratchets for non-PR runs; PRs execute the same checks in preflight, including environment-variable counts, max-lines suppression, line-cap growth, assertion safety, config docs, and plugin inventory                                                                               | Selected non-PR Node work                             |
...
4. Baseline ratchets run in `checks-baseline-ratchets` after preflight. Selected `checks-node-*` test shards wait for those ratchets to pass; trusted same-repository hybrid PR first attempts, automatic main runs, and admitted qualification dispatches use the existing Blacksmith 4-class to shorten this serial hop. Hybrid retries, ordinary manual targets, and the GitHub override retain hosted routing. Frozen targets skip the dedicated ratchets and keep their existing Node test admission. See the [ratchet admission measurements](/ci/routing-costs#ratchet-admission-before-node-tests).
...
6. PR Node matrices use native fail-fast. For same-repository PRs selecting Node rows, `pr-fail-fast` watches the first attempt and cancels remaining job families after a failure. Only that job has `actions: write`. It starts after preflight and observes failures while the installed check planner queues or runs. Clean completion combines preflight's other job counts with the planner's exact admitted check count, published by its successful `CI check job count v1: N` step. It observes every 30 seconds until the exact final inventory is present with at most three jobs unfinished, then every five seconds for up to one minute so successful completion does not wait for another broad polling interval. The faster window adds at most ten inventory scans, each using the existing four-page bound; longer-running final work returns to the normal cadence. It rechecks the current PR head and newer runs before cancellation; fork PRs retain native matrix fail-fast because their tokens cannot cancel base-repository workflows. The monitor adds one 4-vCPU Blacksmith registration per eligible PR, or uses hosted Ubuntu under the outage override. Main, manual runs, and retries do not start it. Observation ends before the monitor's job limit; ordinary lane verification still owns the result when no failure was observed. Partial reruns retain native fail-fast and ordinary lane verification, ignoring monitor causes and results retained from earlier attempts.
