...
Precise PR compiler plans use those same five core stripes rather than suppressing them and putting every selected graph before the central tail. A broad public input can select all 25 graphs: intersection bounds each stripe to five graphs at the unchanged two-child concurrency, with the central extensions/scripts/root row independent. Relative to the formerly collapsed precise plan, this admits five additional jobs (Blacksmith on eligible hybrid first attempts, hosted on GitHub and hybrid retries). It does not widen the existing full-plan inventory or its 157/217 registration ceilings: `4 × 157 + 21 × 217 = 5,185` still leaves 815 below the historical 6,000 reference. Hosted admission counts all five stripes even when changed paths are present; the 40/45 limits, routing, and 15/20-minute stripe/central deadlines remain unchanged. Each precise stripe repeats boundary discovery, so setup and inventory costs increase for narrow plans. Native complete-job timing and current organization pressure still require qualification; these coverage and row bounds are not a measured speedup.
