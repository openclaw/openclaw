...
Current macOS Swift targets use three phases: App Intents release qualification,
the complete debug/coverage suite, and native Gateway proof with its own debug
build. Compared with the original phase selection, this adds two GitHub-hosted
`macos-26` jobs to each selected ordinary PR/main or PR `release_gate` run, one job
to full validation, and zero Blacksmith registrations. The concurrency cap stays
at two, with the same 30-minute per-worker timeout; historical targets keep their
