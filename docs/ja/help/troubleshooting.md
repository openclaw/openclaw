...
read_when:
  ...
  - 詳細なランブックに入る前にトリアージ フローを確認したい場合
title: "トラブルシューティング"
x-i18n:
...
<AccordionGroup>
  ...
  <Accordion title="チャンネルは接続されるが、メッセージが流れない">
    ```bash
    openclaw status
    openclaw gateway status
    openclaw logs --follow
  ...
  <Accordion title="ノードはペアリング済みだが、ツールの camera/canvas/screen/exec が失敗する">
    ```bash
    openclaw status
    openclaw gateway status
