...
良い PR はレビューしやすいものです。レビュアーが意図をすぐに理解し、振る舞いを検証し、安全に変更をマージできる必要があります。このガイドでは、人間および LLM によるレビューに向けた、簡潔で高シグナルな提出方法を説明します。

## 良い PR とは

- [ ] 問題、その重要性、変更内容を説明する。
- [ ] 変更範囲を絞る。広範なリファクタリングは避ける。
...
簡潔さを重視してください。文法よりも簡潔なレビューが重要です。該当しないセクションは省略してください。
...
- [ ] インフラ／ビルド: 環境／検証内容。
- [ ] セキュリティ: リスク、再現、検証を要約し、機密データは含めない。根拠のある主張のみ。
