...
- `steer`: 現在の実行に即時注入します（次のツール境界以降の保留中ツール呼び出しをキャンセル）。ストリーミングでない場合はフォローアップにフォールバックします。
- `followup`: 現在の実行が終了した後の次のエージェントターン用にエンキューします。
- `collect`: キューに入ったすべてのメッセージを **単一** のフォローアップターンに統合します（デフォルト）。異なるチャンネル／スレッドを対象とするメッセージは、ルーティングを保持するため個別に処理されます。
...
Steer-backlog を使用すると、制御された実行の後にフォローアップ応答が返る可能性があるため、  
ストリーミング対応の画面では重複して見えることがあります。  
受信メッセージごとに 1 回の応答にしたい場合は、`collect`/`steer` を推奨します。  
`/queue collect` はスタンドアロンコマンド（セッション単位）として送信するか、`messages.queue.byChannel.discord: "collect"` を設定してください。
...
Summarize は、破棄されたメッセージの短い箇条書きリストを保持し、それを合成フォローアッププロンプトとして注入します。  
