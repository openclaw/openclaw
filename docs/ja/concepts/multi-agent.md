...
目的：1 つの稼働中の Gateway で、複数の _分離された_ エージェント（個別のワークスペース + `agentDir` + セッション）と、複数のチャンネル アカウント（例：2 つの WhatsApp）を同時に扱います。インバウンドは、バインディングによってエージェントへルーティングされます。 インバウンドはバインディングを介してエージェントにルーティングされます。
...
認証プロファイルは **エージェント単位** です。各エージェントは、次から読み取ります。 各エージェントは以下のように読み込みます。

```
~/.openclaw/agents/<agentId>/agent/auth-profiles.json
```

メインエージェントの資格情報は **共有されません** 。 エージェント間で `agentDir`
を再利用しないでください (認証/セッションの衝突を引き起こします)。 メイン エージェントの認証情報は自動的に共有 **されません**。エージェント間で `agentDir` を再利用しないでください（認証／セッションの衝突を引き起こします）。認証情報を共有したい場合は、`auth-profiles.json` を別のエージェントの `agentDir` にコピーしてください。

Skills は、各ワークスペースの `skills/` フォルダーを通じて **エージェント単位** で提供され、共有 Skills は `~/.openclaw/skills` から利用できます。詳細は [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) を参照してください。 [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills)を参照してください。

Gateway は **1 つのエージェント**（デフォルト）または **複数のエージェント** を並列にホストできます。

**ワークスペースに関する注意：** 各エージェントのワークスペースは **デフォルトの cwd** であり、厳密なサンドボックスではありません。相対パスはワークスペース内に解決されますが、サンドボックス化が有効でない限り、絶対パスはホスト上の他の場所へ到達できます。詳細は [Sandboxing](/gateway/sandboxing) を参照してください。 ワークスペース内では相対パスが解決されますが、サンドボックス化が有効になっていない限り、絶対パスは
他のホストロケーションに到達することができます。
...
**複数アカウント** をサポートするチャンネル（例：WhatsApp）では、各ログインを識別するために `accountId` を使用します。各 `accountId` は異なるエージェントへルーティングできるため、1 台のサーバーで複数の電話番号を、セッションを混在させずにホストできます。 それぞれの`accountId`は異なるエージェントにルーティングすることができるので、1つのサーバーはセッションを混合せずに
複数の電話番号をホストできます。
...
- ツールの許可/拒否リストは**ツール**であり、スキルではありません。 スキルが
  バイナリを実行する必要がある場合は、`exec`が許可され、バイナリがサンドボックスに存在することを確認してください。
...
注意: `setupCommand` は `sandbox.docker` の下にあり、コンテナ作成時に一度実行されます。
...
注意: `tools.elevated` は **global** で送信者ベースです。エージェントごとに設定することはできません。
エージェント毎の境界線が必要な場合は、 `exec` を拒否するには `agents.list[].tools` を使用します。
