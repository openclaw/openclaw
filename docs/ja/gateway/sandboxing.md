...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T09:22:12Z
---

# サンドボックス化

OpenClaw は **Docker コンテナ内でツールを実行** することで、影響範囲（ブラス半径）を低減できます。
これは **オプション** であり、設定（`agents.defaults.sandbox` または
`agents.list[].sandbox`）によって制御されます。サンドボックス化がオフの場合、ツールはホスト上で実行されます。
Gateway（ゲートウェイ）はホスト上に常駐し、有効化されている場合のみ、ツール実行が分離されたサンドボックス内で行われます。
...
- 明示的にホスト上での実行が許可されたツール（例：`tools.elevated`）。
  ...
  - サンドボックス化がオフの場合、`tools.elevated` は実行に影響しません（既にホスト上で実行）。[Elevated Mode](/tools/elevated) を参照してください。
...
`/workspace/skills` から読み取り可能になります。
...
`scope: "shared"` 配下では、エージェント単位の bind は無視されます。
...
ツールの許可／拒否ポリシーは、サンドボックスルールの前に引き続き適用されます。
