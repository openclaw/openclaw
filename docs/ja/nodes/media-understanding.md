...
OpenClaw は、返信パイプラインが実行される前に **受信メディア**（画像／音声／動画）を要約できます。ローカルツールやプロバイダーのキーが利用可能かを自動検出し、無効化やカスタマイズも可能です。理解がオフの場合でも、モデルには通常どおり元のファイル／URL が渡されます。
...
`capabilities` を設定すると、そのエントリーは指定されたメディアタイプでのみ実行されます。共有リストでは、OpenClaw がデフォルトを推論できます。
...
CLI エントリーでは、想定外の一致を避けるため **`capabilities` を明示的に設定**してください。`capabilities` を省略した場合、そのエントリーは配置されているリストに対して適格になります。
...
| Audio      | OpenAI、Groq、Deepgram、Google                     | プロバイダーの文字起こし（Whisper／Deepgram／Gemini）。 |
