...
OS の権限は多段階です。アプリ内ではセレクターを公開できますが、実際の付与は OS が決定します。

- iOS/macOS：ユーザーはシステムのプロンプトや設定で **While Using** または **Always** を選択できます。アプリは昇格を要求できますが、OS により設定画面が必要になる場合があります。
...
- iOS：Always 権限 + バックグラウンド位置情報モードが必要です。サイレントプッシュはスロットリングされる可能性があり、断続的な失敗が想定されます。
...
- Always：「バックグラウンドでの位置情報を許可します。システム権限が必要です。」
- Precise：「正確な GPS 位置情報を使用します。オフにすると概算位置を共有します。」
