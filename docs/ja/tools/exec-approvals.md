...
許可リストは **エージェント単位** です。複数のエージェントが存在する場合は、
macOS アプリで編集対象のエージェントを切り替えてください。パターンは **大文字小文字を区別しない glob マッチ** です。
...
厳密な手動許可リストを使用したい場合は、これを無効にしてください。
...
UI には各パターンの **last used** メタデータが表示されるため、一覧を整理できます。
...
承認が必要な場合、exec ツールは承認 ID を返して即座に終了します。
この ID を使用して、後続のシステムイベント（`Exec finished` / `Exec denied`）と関連付けてください。
タイムアウトまでに決定が届かない場合、リクエストは承認タイムアウトとして扱われ、拒否理由として表示されます。
...
ゲートウェイホストの exec 承認も、コマンド完了時（および必要に応じて、しきい値を超えて実行中の場合）に
同じライフサイクルイベントを発行します。
承認ゲート付き exec では、これらのメッセージ内で相関を容易にするため、
承認 ID が `runId` として再利用されます。
