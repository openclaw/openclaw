...
read_when:
  ...
  - スタンドアロン CLI と OpenClaw への深い統合の判断時
...
- 意見・嗜好の安定性（および変更時の根拠）
- 時間制約（「2025 年 11 月時点で何が真だったか？」）と競合解決
...
- **意見**: 「Peter は何を好むか？」（信頼度 + 根拠付き）

返却形式はエージェント フレンドリーで、出典を明示します。
...
省察はスケジュール実行（毎日、またはハートビート `ultrathink`）のジョブで、次を行います。
