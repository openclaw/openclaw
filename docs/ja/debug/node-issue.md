...
これは、開発用スクリプトを Bun から `tsx` に切り替えた後（コミット `2871657e`、2026-01-06）に発生し始めました。同じランタイムパスは Bun では動作していました。
...
- `tsx` は TS/ESM を変換するために esbuild を使用します。esbuild の `keepNames` は `__name` ヘルパーを出力し、関数定義を `__name(...)` でラップします。
