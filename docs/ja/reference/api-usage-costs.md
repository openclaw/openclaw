...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T09:23:08Z
---

# API 使用量 & コスト

このドキュメントでは、**API キーを呼び出す可能性のある機能**と、そのコストがどこに表示されるかを一覧します。  
プロバイダーの使用量や有料 API 呼び出しを生成し得る OpenClaw の機能に焦点を当てています。
...
受信したメディアは、返信の実行前に要約や文字起こしが行われる場合があります。これはモデル / プロバイダーの API を使用します。
...
一部のステータス コマンドは、クォータ ウィンドウや認証状態を表示するために **プロバイダーの使用量エンドポイント** を呼び出します。  
通常は低頻度の呼び出しですが、プロバイダーの API にはアクセスします。
...
Skills は `apiKey` を `skills.entries.<name>.apiKey` に保存できます。スキルがそのキーを外部 API に使用する場合、  
スキルのプロバイダーに応じてコストが発生する可能性があります。
