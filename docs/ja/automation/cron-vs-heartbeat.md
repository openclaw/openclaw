...
Heartbeat と cron ジョブはいずれも、スケジュールに従ってタスクを実行できます。本ガイドでは、ユースケースに適した仕組みを選ぶための指針を示します。
...
Heartbeat は **メインセッション** で一定間隔（デフォルト：30 分）ごとに実行されます。エージェントが状況を確認し、重要な点を浮き彫りにすることを目的としています。
