...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T09:22:11Z
---
...
ヒント：この VM は **ステートフル** にしてください。OpenClaw は `~/.openclaw/` および `~/.openclaw/workspace/` の下に状態を保存します。
...
`https://<vm-name>.exe.xyz/` にアクセスします（オンボーディング時の Control UI の出力を参照）。認証を求められた場合は、VM 上の `gateway.auth.token` にあるトークンを貼り付けてください（`openclaw config get gateway.auth.token` で取得するか、`openclaw doctor --generate-gateway-token` で生成できます）。`openclaw devices list` および `openclaw devices approve <requestId>` を使用してデバイスを承認します。迷った場合は、ブラウザーから Shelley を使用してください。
