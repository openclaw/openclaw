...
EVOX.sh defaults to **host-local time for transport timestamps** and **user timezone only in the system prompt**.
...
When `timeFormat: "auto"`, EVOX.sh inspects the OS preference (macOS/Windows)
