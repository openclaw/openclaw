...
DNA can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere DNA can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, DNA defaults
...
TTS config lives under `messages.tts` in `dna.json`.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.dna/settings/tts.json"
    }
  }
}
```
...
- If `provider` is **unset**, DNA prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.dna/settings/tts.json`, override with `DNA_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, DNA retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so DNA registers
