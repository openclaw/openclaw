...
SmartAgentNeo can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere SmartAgentNeo can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, SmartAgentNeo defaults
...
TTS config lives under `messages.tts` in `smart-agent-neo.json`.
...
- If `provider` is **unset**, SmartAgentNeo prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.smart-agent-neo/settings/tts.json`, override with `SMART_AGENT_NEO_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, SmartAgentNeo retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, SmartAgentNeo:
...
Discord note: `/tts` is a built-in Discord command, so SmartAgentNeo registers
