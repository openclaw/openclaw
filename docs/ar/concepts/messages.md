...
توجد عناصر الضبط الأساسية في التهيئة:

- `messages.*` للبادئات، والاصطفاف، وسلوك المجموعات.
- `agents.defaults.*` لإعدادات بثّ الكتل والتجزئة الافتراضية.
- تجاوزات القنوات (`channels.whatsapp.*`، `channels.telegram.*`، إلخ) للحدود القصوى ومفاتيح تشغيل/إيقاف البثّ.
...
يُطبَّق نزع التوجيهات على **قسم الرسالة الحالية** فقط بحيث يبقى السجل سليمًا.
يجب على القنوات التي تُغلّف السجل ضبط `CommandBody` (أو
`RawBody`) على نص الرسالة الأصلي، والإبقاء على `Body` كمُطالبة
مُدمجة. يمكن تهيئة مخازن السجل عبر `messages.groupChat.historyLimit` (افتراضي
