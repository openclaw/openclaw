...
إذا تم تعيين `agents.defaults.models`، فسيصبح **قائمة السماح** لـ `/model` ولتجاوزات الجلسة.
عندما يختار المستخدم نموذجًا غير موجود في تلك القائمة، يُرجع OpenClaw:

```
Model "provider/model" is not allowed. Use /model to list available models.
```

يحدث هذا **قبل** إنشاء رد طبيعي، لذا قد يبدو أن الرسالة «لم تستجب».
الحل هو أحد الخيارات التالية:
