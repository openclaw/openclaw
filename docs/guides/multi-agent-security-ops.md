---
summary: "Operational security patterns for multi-agent OpenClaw deployments — credential lifecycle, circuit breakers, file integrity, incident response"
read_when:
  - Running multiple agents in production
  - Hardening a long-running multi-agent deployment
...
The patterns below are drawn from a production 9-agent deployment running on a Mac mini. They're not theoretical — each one exists because something broke or was found exposed during a live audit.
...
```bash
# Create wrapper for SecretRef exec provider (JSON protocol)
cat > ~/.openclaw/scripts/sops-secret-provider.sh << 'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
# OpenClaw sends requested secret IDs via stdin as JSON:
#   {"ids": ["KEY_1", "KEY_2"]}
# Must respond with JSON:
#   {"values": {"KEY_1": "val1", "KEY_2": "val2"}}

DECRYPTED=$(sops --input-type dotenv --output-type dotenv -d ~/.openclaw/.env.enc)
IDS=$(python3 -c "import sys,json; print(' '.join(json.load(sys.stdin)['ids']))")

VALUES="{"
FIRST=true
for ID in $IDS; do
  VAL=$(echo "$DECRYPTED" | grep "^${ID}=" | head -1 | cut -d= -f2-)
  [ -n "$VAL" ] || continue
  $FIRST || VALUES+=","
  VALUES+="$(python3 -c "import json; print(json.dumps({"$ID": "$VAL"}))[1:-1]")"
  FIRST=false
done
VALUES+="}"
...
*Real example: An expired MS Graph token caused 40+ failed cron runs over 21 hours, consuming ~640K tokens before anyone noticed.*
...
*Real example: An audit flagged 3 unsandboxed agents as a critical finding. Investigation showed one (main) had exec/process denied via tool policy, making sandboxing redundant. The others were read-only agents with no dangerous tool access. Without a baseline, we'd have wasted time "fixing" an accepted risk.*
...
### Pattern: Define explicit authority tiers
...
### Pattern: Convention-based file routing
...
*This guide is a living document. Update it as your deployment evolves and new patterns emerge.*
