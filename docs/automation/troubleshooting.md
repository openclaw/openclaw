...
EasyHub system heartbeat last
EasyHub logs --follow
EasyHub config get agents.defaults.heartbeat
EasyHub channels status --probe
```
...
```bash
EasyHub config get agents.defaults.heartbeat.activeHours
EasyHub config get agents.defaults.heartbeat.activeHours.timezone
EasyHub config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
EasyHub cron list
EasyHub logs --follow
```
