...
An authenticated sender listed in `commands.ownerAllowFrom` can ask the agent to list, inspect, update, run, or remove any existing automation on that Gateway from any admitted channel conversation, regardless of the job's creator or channel. For example, an owner can disable a reminder created in a Slack thread from a DM. Channel `allowFrom` lists alone do not grant this authority. In the authenticated Control UI, an administrator with `operator.admin` has the same management access as on the **Automations** page. Create command payloads through the operator CLI or Gateway API.

The Gateway grants this authority from the authenticated turn's admission facts. Each operation uses a one-use grant that expires after 60 seconds and remains bound to that exact active run. Owner membership is rechecked against the current configuration before each operation and before a suspended operation commits. Ordinary channel turns and Control UI turns without `operator.admin` remain caller-scoped; matching account IDs or session routes never establish management authority. Heartbeats, scheduled runs, and relayed continuations do not inherit an owner's chat management access. If access is denied or a grant expires, retry from a fresh configured owner chat turn or authenticated Control UI administrator turn, or use the **Automations** page.
