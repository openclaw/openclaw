...
- `agent:turn:save` carries `context.sessionKey`, `context.success`, `context.saveOutcome`, `context.turnSuccess`, `context.durationMs`, optional `context.turnErrorCode`, optional `context.saveError`, and optional `context.saveSkipReason`. Its `saveOutcome` field is `saved`, `skipped`, or `failed`; `success` is true only for `saved`. `turnSuccess` preserves the underlying runtime turn outcome. It is scheduled after the ACP turn save phase reaches an outcome, including save failure, no registered save callback, a save callback returning no durable-write evidence, or a save layer declining the durable-readiness boundary.
