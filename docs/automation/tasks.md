...
The shared database replaced the `tasks/runs.sqlite` and `flows/registry.sqlite` sidecar stores in `v2026.5.30-beta.1`, stable from `v2026.6.1`. Current versions no longer import or archive these pre-June files. To preserve their records when upgrading an older installation, back up the state root and run `openclaw doctor --fix` on `v2026.6.1` before updating further. Installs from `v2026.6.1` onward use the shared database.
