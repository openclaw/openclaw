...
The shared database replaced the `flows/registry.sqlite` sidecar in `v2026.5.30-beta.1`, stable from `v2026.6.1`. Current versions leave this pre-June file untouched. Older installations that need its records must first [migrate through `v2026.6.1`](/automation/tasks#where-tasks-live).
