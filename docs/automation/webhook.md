...
read_when:
  ...
  - Wiring external systems into SmartAgentNeo
title: "Webhooks"
---
...
```json5
{
  hooks: {
    enabled: true,
    token: "${SMART_AGENT_NEO_HOOKS_TOKEN}",
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your SmartAgentNeo config directory (typically `~/.smart-agent-neo/hooks/transforms`).
...
- `smart-agent-neo webhooks gmail setup` writes `hooks.gmail` config for `smart-agent-neo webhooks gmail run`.
