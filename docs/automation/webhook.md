...
read_when:
  ...
  - Wiring external systems into Activi
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Activi config directory (typically `~/.activi/hooks/transforms`).
...
- `activi webhooks gmail setup` writes `hooks.gmail` config for `activi webhooks gmail run`.
