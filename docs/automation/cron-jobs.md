...
- Jobs persist under `~/.dna/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.dna/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.dna/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
dna cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
dna cron edit <jobId> --agent ops
dna cron edit <jobId> --clear-agent
```
```
...
```bash
dna system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`dna system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `DNA_SKIP_CRON`.
