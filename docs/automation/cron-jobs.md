...
- Jobs persist under `~/.activi/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.activi/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `activi cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Activi applies a
...
If `delivery` is omitted for isolated jobs, Activi defaults to `announce`.
...
- Job store: `~/.activi/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.activi/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```bash
activi cron run <jobId>
activi cron run <jobId> --due
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`activi system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ACTIVI_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Activi applies exponential retry backoff for recurring jobs after consecutive errors:
