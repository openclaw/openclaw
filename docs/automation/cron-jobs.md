...
- Jobs persist under `~/.smart-agent-neo/cron/` so restarts don’t lose schedules.
...
smart-agent-neo cron run <job-id>
...
Cron jobs are persisted on the Gateway host at `~/.smart-agent-neo/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `smart-agent-neo cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, SmartAgentNeo defaults to `announce`.
...
- Job store: `~/.smart-agent-neo/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.smart-agent-neo/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
- `SMART_AGENT_NEO_SKIP_CRON=1` (env)
...
```bash
smart-agent-neo cron run <jobId>
smart-agent-neo cron run <jobId> --due
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`smart-agent-neo system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SMART_AGENT_NEO_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- SmartAgentNeo applies exponential retry backoff for recurring jobs after consecutive errors:
