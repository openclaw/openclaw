...
- If an isolated agent-turn stalls before the runner starts or before the first model call, cron records a phase-specific timeout such as `setup timed out before runner start` or `stalled before first model call (last phase: context-engine)`. These watchdogs cover embedded providers and CLI-backed providers before their external CLI process is actually started. They default to `3m`, and `cron.isolatedAgentPreExecutionWatchdog` is capped to half of `timeoutSeconds`. Tune `cron.isolatedAgentSetupWatchdog` / `cron.isolatedAgentPreExecutionWatchdog` for slow provider/runtime startup while keeping the overall job timeout as the final bound after execution starts.
