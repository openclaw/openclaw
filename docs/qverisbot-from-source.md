...
| 工具              | 用途                                                                                                                                                                     |
| :---------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qveris_discover` | 按**能力类型**发现工具（实时数据 API、外部服务能力、地理位置 API）。查询词应描述"工具能力"而非"任务目标"，必须用英文。**不适用于**本地文件读写、软件配置查询或文档检索。 |
| `qveris_invoke`   | 调用 `qveris_discover` 返回的工具，或调用 `qveris_inspect` 返回且带有 `discovery_id` 的工具，以 JSON 传入参数。                                                          |
| `qveris_inspect`  | 通过已知 `tool_id` 直接验证工具可用性并获取最新参数结构，无需重新发现——适用于本次会话内已成功使用过的工具；当会话内已知时也会返回可复用的 `discovery_id`。               |

**工具路由决策树（Agent 使用前先判断）：**

1. **本地操作**（读文件、查配置、session 状态）→ 用本地工具，不调 QVeris
2. **需要访问网页内容** → 直接用 `web_search` / `web_fetch`
3. **需要结构化实时数据**（价格、天气、汇率、财务数据）→ `qveris_discover("weather forecast API")`，描述能力类型
4. **需要外部服务能力**（图片生成、OCR、TTS、翻译）→ `qveris_discover("text to image generation API")`
5. **本次会话已用过某工具** → 先用 `qveris_inspect` 恢复 `discovery_id`；若返回了 `discovery_id` 再 `qveris_invoke`，否则先重新 `qveris_discover`
6. **以上均不符合** → 不调 QVeris，改用 `web_search` 或如实告知限制

Agent 内置**会话级工具速查表（Tool Rolodex）**：每次成功调用后自动记录 `tool_id`，后续发现结果会标注 `previously_used` 和 `session_known_tools`；当会话内已知时，也会保留可复用的 `discovery_id`，避免重复发现。
