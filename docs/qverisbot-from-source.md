...
| 工具                | 用途                                                                                                                                                                     |
| :------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qveris_search`     | 按**能力类型**发现工具（实时数据 API、外部服务能力、地理位置 API）。搜索词应描述"工具能力"而非"任务目标"，必须用英文。**不适用于**本地文件读写、软件配置查询或文档检索。 |
| `qveris_execute`    | 执行 `qveris_search` 或 `qveris_get_by_ids` 返回的工具，以 JSON 传入参数。                                                                                               |
| `qveris_get_by_ids` | 通过已知 `tool_id` 直接验证工具可用性并获取最新参数结构，无需重新搜索——适用于本次会话内已成功使用过的工具。                                                              |

**工具路由决策树（Agent 使用前先判断）：**

1. **本地操作**（读文件、查配置、session 状态）→ 用本地工具，不调 QVeris
2. **需要访问网页内容** → 直接用 `web_search` / `web_fetch`
3. **需要结构化实时数据**（价格、天气、汇率、财务数据）→ `qveris_search("weather forecast API")`，描述能力类型
4. **需要外部服务能力**（图片生成、OCR、TTS、翻译）→ `qveris_search("text to image generation API")`
5. **本次会话已用过某工具** → 用 `qveris_get_by_ids` + `qveris_execute`，跳过重复搜索
6. **以上均不符合** → 不调 QVeris，改用 `web_search` 或如实告知限制

Agent 内置**会话级工具速查表（Tool Rolodex）**：每次成功执行后自动记录 `tool_id`，后续搜索结果会标注 `previously_used` 和 `session_known_tools`，避免重复发现。
