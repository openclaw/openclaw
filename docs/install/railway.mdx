...
Deploy EasyHub on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/EasyHub` — Control UI
...
- `EASYHUB_STATE_DIR=/data/.easyhub` (recommended)
- `EASYHUB_WORKSPACE_DIR=/data/workspace` (recommended)
- `EASYHUB_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your EasyHub state + workspace so you can migrate to another host without losing config or memory.
