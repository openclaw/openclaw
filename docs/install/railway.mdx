---
summary: "Deploy EVOX.sh on Railway with one-click template"
read_when:
  - Deploying EVOX.sh to Railway
  - You want a one-click cloud deploy with browser-based Control UI
title: "Railway"
---

Deploy EVOX.sh on Railway with a one-click template and access it through the web Control UI.
...
Use the Control UI at `/openclaw` or run `evox onboard` via Railway's shell for channel setup instructions:
