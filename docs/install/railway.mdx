...
- `PORT=8080` (required — must match the port in Public Networking; also tells the gateway to bind to `0.0.0.0` so Railway's health probe can reach it)
- `OPENCLAW_GATEWAY_TOKEN` (required when `PORT` is set — non-loopback binds require auth; Railway's template generates this automatically)
