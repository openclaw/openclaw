...
- `extended-stable` pins npm-publishable official plugins to the exact installed
  core version after the core package succeeds. Official plugin
  `@extended-stable` selectors are not queried. This overrides official npm
  pins/tags and converts trusted official ClawHub installs to the exact npm
  version; third-party selections are preserved.
