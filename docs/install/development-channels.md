...
Mullusi ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/mullusi`, override with
  `MULLUSI_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
mullusi update --tag 2026.4.1-beta.1
...
mullusi update --tag mullusi@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `mullusi update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Mullusi prompts for confirmation (skip with `--yes`).
...
Preview what `mullusi update` would do without making changes:

```bash
mullusi update --dry-run
mullusi update --channel beta --dry-run
mullusi update --tag 2026.4.1-beta.1 --dry-run
mullusi update --dry-run --json
```
...
When you switch channels with `mullusi update`, Mullusi also syncs plugin
sources:
