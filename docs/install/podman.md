...
Then open `http://127.0.0.1:3000/` and use the token from `~openclaw/.openclaw/.env` (or the value printed by setup).
...
- **Host ports:** By default the script maps `3000` (gateway) and `18790` (bridge). Override the **host** port mapping with `OPENCLAW_PODMAN_GATEWAY_HOST_PORT` and `OPENCLAW_PODMAN_BRIDGE_HOST_PORT` when launching.
