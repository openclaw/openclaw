---
summary: "Run EVOX.sh in a rootless Podman container"
read_when:
...
Run the EVOX.sh Gateway in a **rootless** Podman container. Uses the same image as Docker (built from the repo [Dockerfile](https://github.com/sonpiaz/evox-sh/blob/main/Dockerfile)).
...
<Steps>
  <Step title="One-time setup">
    From the repo root, run the setup script. It creates a dedicated `evox` user, builds the container image, and installs the launch script:
...
If you ran `./scripts/podman/setup.sh --quadlet` (or `OPENCLAW_PODMAN_QUADLET=1`), a [Podman Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html) unit is installed so the gateway runs as a systemd user service for the evox user. The service is enabled and started at the end of setup.

- **Start:** `sudo systemctl --machine evox@ --user start openclaw.service`
- **Stop:** `sudo systemctl --machine evox@ --user stop openclaw.service`
- **Status:** `sudo systemctl --machine evox@ --user status openclaw.service`
- **Logs:** `sudo journalctl --machine evox@ --user -u openclaw.service -f`

The quadlet file lives at `~openclaw/.config/containers/systemd/openclaw.container`. To change ports or env, edit that file (or the `.env` it sources), then `sudo systemctl --machine evox@ --user daemon-reload` and restart the service. On boot, the service starts automatically if lingering is enabled for evox (setup does this when loginctl is available).

To add quadlet **after** an initial setup that did not use it, re-run: `./scripts/podman/setup.sh --quadlet`.

## The evox user (non-login)

`scripts/podman/setup.sh` creates a dedicated system user `evox`:
...
- **Rootless Podman:** The user must have a **subuid** and **subgid** range. Many distros assign these automatically when the user is created. If setup prints a warning, add lines to `/etc/subuid` and `/etc/subgid`:
  ...
  ```bash
  sudo -u evox /home/openclaw/run-openclaw-podman.sh
  sudo -u evox /home/openclaw/run-openclaw-podman.sh setup
  ```

- **Config:** Only `evox` and root can access `/home/openclaw/.openclaw`. To edit config: use the Control UI once the gateway is running, or `sudo -u evox $EDITOR /home/openclaw/.openclaw/openclaw.json`.
...
- **Optional:** In that `.env` you can set provider keys (e.g. `GROQ_API_KEY`, `OLLAMA_API_KEY`) and other EVOX.sh env vars.
...
- **Logs:** With quadlet: `sudo journalctl --machine evox@ --user -u openclaw.service -f`. With script: `sudo -u evox podman logs -f evox`
- **Stop:** With quadlet: `sudo systemctl --machine evox@ --user stop openclaw.service`. With script: `sudo -u evox podman stop evox`
- **Start again:** With quadlet: `sudo systemctl --machine evox@ --user start openclaw.service`. With script: re-run the launch script or `podman start evox`
- **Remove container:** `sudo -u evox podman rm -f evox` — config and workspace on the host are kept
...
- **Rootless Podman fails for user evox:** Check `/etc/subuid` and `/etc/subgid` contain a line for `evox` (e.g. `evox:100000:65536`). Add it if missing and restart.
- **Container name in use:** The launch script uses `podman run --replace`, so the existing container is replaced when you start again. To clean up manually: `podman rm -f evox`.
- **Script not found when running as evox:** Ensure `scripts/podman/setup.sh` was run so that `run-openclaw-podman.sh` is copied to evox’s home (e.g. `/home/openclaw/run-openclaw-podman.sh`).
- **Quadlet service not found or fails to start:** Run `sudo systemctl --machine evox@ --user daemon-reload` after editing the `.container` file. Quadlet requires cgroups v2: `podman info --format '{{.Host.CgroupsVersion}}'` should show `2`.

## Optional: run as your own user

To run the gateway as your normal user (no dedicated evox user): build the image, create `~/.openclaw/.env` with `OPENCLAW_GATEWAY_TOKEN`, and run the container with `--userns=keep-id` and mounts to your `~/.openclaw`. The launch script is designed for the openclaw-user flow; for a single-user setup you can instead run the `podman run` command from the script manually, pointing config and workspace to your home. Recommended for most users: use `scripts/podman/setup.sh` and run as the evox user so config and process are isolated.
