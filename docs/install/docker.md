...
1. Set **OPENCLAW_GITHUB_REPO** to the target repo in **owner/repo** form only (e.g. `myorg/my-repo`). This is what `gh` uses to know which repo to interact with. Do not use a URL (no `https://` or `git@...`).
