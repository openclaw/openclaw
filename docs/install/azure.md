---
summary: "Run Mullusi Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Mullusi running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Mullusi Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Mullusi on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Mullusi.
...
- Install Mullusi with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-mullusi"
    LOCATION="westus2"
    VNET_NAME="vnet-mullusi"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-mullusi-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-mullusi-vm"
    VM_NAME="vm-mullusi"
    ADMIN_USERNAME="mullusi"
    BASTION_NAME="bas-mullusi"
    BASTION_PIP_NAME="pip-mullusi-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Mullusi (in the VM shell)">
    ```bash
    curl -fsSL https://mullusi.com/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Mullusi, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Mullusi onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Mullusi Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Mullusi Azure deployment with the GitHub Copilot model provider: [Mullusi on Azure with GitHub Copilot](https://github.com/johnsonshi/mullusi-azure-github-copilot)
