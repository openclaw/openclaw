...
After the core swap, normal plugin integrity and convergence checks still run.
Npm-publishable official plugins use the exact installed core version; OpenClaw
does not query plugin `@extended-stable` selectors. This compatibility rule
overrides an official plugin's explicit npm pin/tag and converts a trusted
official ClawHub install to the exact npm package version. Third-party plugin
pins/tags and other non-npm sources remain unchanged.
