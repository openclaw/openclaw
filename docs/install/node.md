...
read_when:
  - "You installed DNA but `dna` is “command not found”"
...
DNA’s runtime baseline is **Node 22+**.

If you can run `npm install -g dna@latest` but later see `dna: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
