...
Deploy SmartAgentNeo on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/betterbrand/smart-agent-neo)
...
repository configures everything needed to run SmartAgentNeo:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SMART_AGENT_NEO_STATE_DIR
        value: /data/.smart-agent-neo
      - key: SMART_AGENT_NEO_WORKSPACE_DIR
        value: /data/workspace
      - key: SMART_AGENT_NEO_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: smart-agent-neo-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/smart-agent-neo`.
...
If you use the original SmartAgentNeo repository, Render will not auto-deploy your SmartAgentNeo. To update it, run a manual Blueprint sync from the dashboard.
...
For SmartAgentNeo, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any SmartAgentNeo host.
