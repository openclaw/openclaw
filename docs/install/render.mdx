...
Deploy Activi on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/activi/activi)
...
repository configures everything needed to run Activi:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ACTIVI_STATE_DIR
        value: /data/.activi
      - key: ACTIVI_WORKSPACE_DIR
        value: /data/workspace
      - key: ACTIVI_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/activi`.
...
If you use the original Activi repository, Render will not auto-deploy your Activi. To update it, run a manual Blueprint sync from the dashboard.
...
For Activi, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Activi host.
