---
summary: "Deploy EVOX.sh on Render with Infrastructure-as-Code"
read_when:
  - Deploying EVOX.sh to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy EVOX.sh on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/sonpiaz/evox-sh)
...
repository configures everything needed to run EVOX.sh:
...
If you use the original EVOX.sh repository, Render will not auto-deploy your EVOX.sh. To update it, run a manual Blueprint sync from the dashboard.
...
For EVOX.sh, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive you can restore on any EVOX.sh host. See [Backup](/cli/backup) for details.
...
regularly export your config via `evox backup create` in the Render shell.
