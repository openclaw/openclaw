---
summary: "Deploy Mullusi on Render with Infrastructure-as-Code"
read_when:
  - Deploying Mullusi to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Mullusi on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/mullusi/mullusi)
...
repository configures everything needed to run Mullusi:
...
services:
  - type: web
    ...
    envVars:
      - key: MULLUSI_GATEWAY_PORT
        value: "8080"
      - key: MULLUSI_STATE_DIR
        value: /data/.mullusi
      - key: MULLUSI_WORKSPACE_DIR
        value: /data/workspace
      - key: MULLUSI_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Mullusi state
...
`MULLUSI_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Mullusi repository, Render will not auto-deploy your Mullusi. To update it, run a manual Blueprint sync from the dashboard.
...
For Mullusi, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Mullusi state plus any configured
...
- Missing `MULLUSI_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `MULLUSI_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `mullusi backup create` in the Render shell.
