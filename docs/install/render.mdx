...
Deploy EasyHub on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/EasyHub/EasyHub)
...
repository configures everything needed to run EasyHub:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: EASYHUB_STATE_DIR
        value: /data/.EasyHub
      - key: EASYHUB_WORKSPACE_DIR
        value: /data/workspace
      - key: EASYHUB_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/EasyHub`.
...
If you use the original EasyHub repository, Render will not auto-deploy your EasyHub. To update it, run a manual Blueprint sync from the dashboard.
...
For EasyHub, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any EasyHub host.
