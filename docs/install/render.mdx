...
services:
  - type: web
    ...
    runtime: docker
    plan: starter
    dockerCommand: sh -lc 'node openclaw.mjs gateway --allow-unconfigured --bind lan --port "${PORT:-8080}"'
...
| Feature               | Purpose                                                    |
| --------------------- | ---------------------------------------------------------- |
| `dockerCommand`       | Binds gateway to `0.0.0.0` and Render's `$PORT`            |
...
- Port binding mismatch — ensure the process binds `0.0.0.0:$PORT` (the Blueprint `dockerCommand` does this)
