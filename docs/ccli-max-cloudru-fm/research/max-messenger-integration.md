            ...
            [
              {
                ...
                url: "https://openclaw.max.ru/dashboard",
              },
            ],
...
class OpenClawService {
  ...
  async getHistory(userId: string, limit: number = 10) {
    const response = await this.client.get(`/api/v1/history?user_id=${userId}&limit=${limit}`);
...
class SessionManager {
  ...
  async createSession(userId: string, chatId: string): Promise<SessionData> {
    ...
    await this.redis.setex(`${this.prefix}${userId}`, this.ttl, JSON.stringify(session));
...
export const sessionManager = new SessionManager(process.env.REDIS_URL || "redis://localhost:6379");
```
