---
title: "Memory"
summary: "How EVOX.sh memory works (workspace files + automatic memory flush)"
read_when:
...
EVOX.sh memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, EVOX.sh loads both (deduplicated by realpath so symlinks pointing to the same file are not injected twice).
...
EVOX.sh exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, EVOX.sh triggers a **silent,
...
EVOX.sh can build a small vector index over `MEMORY.md` and `memory/*.md` so
