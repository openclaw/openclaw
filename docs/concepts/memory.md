...
- Uses remote embeddings by default. If `memorySearch.provider` is not set, OpenClaw auto-selects:
  ...
  6. `jina` if a Jina key can be resolved.
...
`models.providers.mistral.apiKey`. For Jina, use `JINA_API_KEY` or
`models.providers.jina.apiKey`. Ollama typically does not require a real API
key (a placeholder like `OLLAMA_API_KEY=ollama-local` is enough when needed by
local policy). When using a custom OpenAI-compatible endpoint,
