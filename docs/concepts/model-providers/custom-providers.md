...
<Note>
On MiniMax's Anthropic-compatible streaming path, OpenClaw's M2.x logical default is `off` and it may send `thinking: { type: "disabled" }` as an outbound compatibility shim unless you explicitly set thinking; MiniMax M2.x ignores that setting and continues thinking. MiniMax-M3 (and M3.x) stays on the provider's omitted/adaptive thinking path by default. `/fast on` rewrites `MiniMax-M2.7` to `MiniMax-M2.7-highspeed`.
</Note>
