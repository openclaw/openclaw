...
read_when:
  - You want to understand how EVOX.sh assembles model context
...
A **context engine** controls how EVOX.sh builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

EVOX.sh ships with a built-in `legacy` engine. Plugins can register
...
Context engine plugins are installed like any other EVOX.sh plugin. Install
...
evox plugins install -l ./my-context-engine
```
...
Every time EVOX.sh runs a model prompt, the context engine participates at
...
EVOX.sh currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. EVOX.sh
...
The built-in `legacy` engine preserves EVOX.sh's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. EVOX.sh uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. EVOX.sh disables Pi's built-in
...
`ownsCompaction: false` does **not** mean EVOX.sh automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  ...
  EVOX.sh's built-in compaction behavior.
...
EVOX.sh resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to EVOX.sh's built-in summarization. Plugin engines can implement
...
- Use `evox doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, EVOX.sh
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `evox plugins install -l ./my-engine` to link a
