...
- CLI helpers: `evox onboard`, `evox models list`, `evox models set <provider/model>`.
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  EVOX.sh merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while EVOX.sh keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `evox onboard`, `evox models auth`, and headless setup
...
That covers providers that still fit EVOX.sh's normal transports. A provider
...
EVOX.sh ships with the pi‑ai catalog. These providers require **no**
...
- CLI: `evox onboard --auth-choice openai-api-key`
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in EVOX.sh because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- CLI: `evox onboard --auth-choice token` (paste setup-token) or `evox models auth paste-token --provider anthropic`
- Direct API-key models support the shared `/fast` toggle and `params.fastMode`; EVOX.sh maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- CLI: `evox onboard --auth-choice openai-codex` or `evox models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like EVOX.sh.
...
- CLI: `evox onboard --auth-choice opencode-zen` or `evox onboard --auth-choice opencode-go`
...
- Compatibility: legacy EVOX.sh config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `evox onboard --auth-choice gemini-api-key`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in EVOX.sh is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Enable: `evox plugins enable google`
  - Login: `evox models auth login --provider google-gemini-cli --set-default`
...
- CLI: `evox onboard --auth-choice zai-api-key`
...
- CLI: `evox onboard --auth-choice ai-gateway-api-key`
...
- CLI: `evox onboard --kilocode-api-key <key>`
...
- CLI: `evox onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `evox onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
```bash
evox models auth login --provider qwen-portal --set-default
```
...
- CLI: `evox onboard --auth-choice volcengine-api-key`
...
- CLI: `evox onboard --auth-choice byteplus-api-key`
...
- CLI: `evox onboard --auth-choice synthetic-api-key`
...
`evox onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, EVOX.sh defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), EVOX.sh forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- If `baseUrl` is empty/omitted, EVOX.sh keeps the default OpenAI behavior (which resolves to `api.openai.com`).
- For safety, an explicit `compat.supportsDeveloperRole: true` is still overridden on non-native `openai-completions` endpoints.

## CLI examples

```bash
evox onboard --auth-choice opencode-zen
evox models set opencode/claude-opus-4-6
evox models list
```
