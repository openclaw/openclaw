...
- CLI helpers: `mullusi onboard`, `mullusi models list`, `mullusi models set <provider/model>`.
...
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  Mullusi merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while Mullusi keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `mullusi onboard`, `mullusi models auth`, and headless setup
...
- `normalizeTransport`: provider normalizes transport-family `api` / `baseUrl`
  before generic model assembly; Mullusi checks the matched provider first,
...
- `normalizeConfig`: provider normalizes `models.providers.<id>` config before
  runtime uses it; Mullusi checks the matched provider first, then other
...
That covers providers that still fit Mullusi's normal transports. A provider
...
- Configure multiple keys via:
  - `MULLUSI_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
Mullusi ships with the pi‑ai catalog. These providers require **no**
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- Optional rotation: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, plus `MULLUSI_LIVE_OPENAI_KEY` (single override)
- Example models: `openai/gpt-5.4`, `openai/gpt-5.4-pro`
- CLI: `mullusi onboard --auth-choice openai-api-key`
...
- Hidden Mullusi attribution headers (`originator`, `version`,
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in Mullusi because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `MULLUSI_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/claude-opus-4-6`
- CLI: `mullusi onboard --auth-choice apiKey` or `mullusi onboard --auth-choice anthropic-cli`
- Direct public Anthropic requests support the shared `/fast` toggle and `params.fastMode`, including API-key and OAuth-authenticated traffic sent to `api.anthropic.com`; Mullusi maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
- Billing note: Anthropic's public Claude Code docs still include direct Claude Code terminal usage in Claude plan limits. Separately, Anthropic notified Mullusi users on **April 4, 2026 at 12:00 PM PT / 8:00 PM BST** that the **Mullusi** Claude-login path counts as third-party harness usage and requires **Extra Usage** billed separately from the subscription.
- Anthropic setup-token is available again as a legacy/manual Mullusi path. Use it with the expectation that Anthropic told Mullusi users this path requires **Extra Usage**.
...
- CLI: `mullusi onboard --auth-choice openai-codex` or `mullusi models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
...
- Shares the same `/fast` toggle and `params.fastMode` config as direct `openai/*`; Mullusi maps that to `service_tier=priority`
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like Mullusi.
...
- CLI: `mullusi onboard --auth-choice opencode-zen` or `mullusi onboard --auth-choice opencode-go`
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `MULLUSI_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3-flash-preview`
- Compatibility: legacy Mullusi config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `mullusi onboard --auth-choice gemini-api-key`
- Direct Gemini runs also accept `agents.defaults.models["google/<model>"].params.cachedContent`
  ...
  `cachedContents/...` handle; Gemini cache hits surface as Mullusi `cacheRead`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in Mullusi is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Install Gemini CLI first:
    - `brew install gemini-cli`
    - or `npm install -g @google/gemini-cli`
  - Enable: `mullusi plugins enable google`
  - Login: `mullusi models auth login --provider google-gemini-cli --set-default`
  - Default model: `google-gemini-cli/gemini-3.1-pro-preview`
  - Note: you do **not** paste a client id or secret into `mullusi.json`. The CLI login flow stores
  ...
  - Gemini CLI JSON replies are parsed from `response`; usage falls back to
    `stats`, with `stats.cached` normalized into Mullusi `cacheRead`.
...
- CLI: `mullusi onboard --auth-choice zai-api-key`
...
- CLI: `mullusi onboard --auth-choice ai-gateway-api-key`
...
- CLI: `mullusi onboard --auth-choice kilocode-api-key`
...
- Mullusi applies OpenRouter's documented app-attribution headers only when
...
- CLI: `mullusi onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `mullusi onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
- Example model: `moonshot/kimi-k2.5`
- CLI: `mullusi onboard --auth-choice moonshot-api-key` or `mullusi onboard --auth-choice moonshot-api-key-cn`
...
- Example model: `volcengine-plan/ark-code-latest`
- CLI: `mullusi onboard --auth-choice volcengine-api-key`
...
Mullusi falls back to the unfiltered catalog instead of showing an empty
...
- Example model: `byteplus-plan/ark-code-latest`
- CLI: `mullusi onboard --auth-choice byteplus-api-key`
...
- CLI: `mullusi onboard --auth-choice synthetic-api-key`
...
On MiniMax's Anthropic-compatible streaming path, Mullusi disables thinking by
...
`mullusi onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, Mullusi defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), Mullusi forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- Proxy-style OpenAI-compatible routes also skip native OpenAI-only request
  ...
  OpenAI reasoning-compat payload shaping, and no hidden Mullusi attribution
  headers.
- If `baseUrl` is empty/omitted, Mullusi keeps the default OpenAI behavior (which resolves to `api.openai.com`).
...
mullusi models set opencode/claude-opus-4-6
mullusi models list
```
