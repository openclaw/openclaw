...
- Idle-timeout cost-runaway breaker: if a model request idle-times out repeatedly without producing completed text or tool-call progress, the outer run loop trips a circuit breaker after 5 consecutive such attempts (`MAX_CONSECUTIVE_IDLE_TIMEOUTS_BEFORE_OUTPUT`) and halts further retries. The breaker applies across auth-profile failover attempts inside the same embedded run, so a wedged provider cannot fan paid model calls across every fallback profile in sequence. The counter resets when an attempt produces completed text or tool-call progress; partial output tokens alone do not reset it. See [#76293](https://github.com/openclaw/openclaw/issues/76293) for the original report (761–1,384 paid API calls generated in 60 seconds from a single stalled heartbeat).
