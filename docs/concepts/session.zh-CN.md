...
OpenClaw 将对话组织成**会话**。每条消息根据其来源（DM、群聊、cron 作业等）路由到相应的会话。
...
默认情况下，所有 DM 共享一个会话以保持连续性。这对于单用户设置是没问题的。

<Warning>
如果多个人可以向你的代理发送消息，请启用 DM 隔离。否则，所有用户将共享相同的对话上下文 — Alice 的私人消息会对 Bob 可见。
</Warning>
...
- **每日重置**（默认）— 在网关主机的本地时间凌晨 4:00 开始新会话。
- **空闲重置**（可选）— 一段时间不活动后开始新会话。设置 `session.reset.idleMinutes`。
- **手动重置** — 在聊天中输入 `/new` 或 `/reset`。`/new <model>` 也会切换模型。

当同时配置了每日和空闲重置时，以先过期的为准。
...
OpenClaw 会随着时间自动限制会话存储。默认情况下，它以 `warn` 模式运行（报告将要清理的内容）。将 `session.maintenance.mode` 设置为 `"enforce"` 以进行自动清理：
...
- [会话工具](/concepts/session-tool) — 跨会话工作的代理工具
- [会话管理深度探索](/reference/session-management-compaction) — 存储模式、记录、发送策略、来源元数据和高级配置
