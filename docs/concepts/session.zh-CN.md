...
OpenClaw 将对话组织成**会话**。每条消息根据其来源（DM、群聊、cron 作业等）路由到一个会话。
...
<Warning>
如果多人可以向你的代理发送消息，请启用 DM 隔离。没有它，所有用户共享相同的对话上下文 — Alice 的私人消息会对 Bob 可见。
</Warning>
...
<Tip>
如果同一个人从多个频道联系你，请使用 `session.identityLinks` 链接他们的身份，以便他们共享一个会话。
</Tip>
...
- **每日重置**（默认）— 在网关主机本地时间的凌晨 4:00 开始新会话。
...
OpenClaw 会随时间自动限制会话存储。默认情况下，它以 `warn` 模式运行（报告将被清理的内容）。设置 `session.maintenance.mode` 为 `"enforce"` 以进行自动清理：
