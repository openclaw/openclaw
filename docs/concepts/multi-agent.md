...
- **Session store** (chat history + routing state) under `~/.dna/agents/<agentId>/sessions`.
...
available from `~/.dna/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.dna` (or `DNA_STATE_DIR`)
- Workspace: `~/clawd` (or `~/clawd-<agentId>`)
- Agent dir: `~/.dna/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.dna/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, DNA runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/clawd` (or `~/clawd-<profile>` when `DNA_PROFILE` is set).
- State defaults to `~/.dna/agents/main/agent`.
...
```js
{
    ...
    list: [
      {
        ...
        agentDir: "~/.dna/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/clawd-work",
        agentDir: "~/.dna/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.dna/credentials/whatsapp/personal
          // authDir: "~/.dna/credentials/whatsapp/personal",
        },
        biz: {
