...
- **Session store** (chat history + routing state) under `~/.mullusi/agents/<agentId>/sessions`.
...
`~/.mullusi/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.mullusi` (or `MULLUSI_STATE_DIR`)
- Workspace: `~/.mullusi/workspace` (or `~/.mullusi/workspace-<agentId>`)
- Agent dir: `~/.mullusi/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.mullusi/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Mullusi runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.mullusi/workspace` (or `~/.mullusi/workspace-<profile>` when `MULLUSI_PROFILE` is set).
- State defaults to `~/.mullusi/agents/main/agent`.
...
```bash
mullusi agents add coding
mullusi agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.mullusi/agents/<agentId>`.
...
- If you later add the same binding for the same agent with an explicit account id, Mullusi upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, Mullusi falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.mullusi/workspace-main" },
      { id: "coding", workspace: "~/.mullusi/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.mullusi/workspace-main" },
      { id: "alerts", workspace: "~/.mullusi/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
mullusi channels login --channel whatsapp --account personal
mullusi channels login --channel whatsapp --account biz
```
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.mullusi/credentials/whatsapp/personal
          // authDir: "~/.mullusi/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.mullusi/credentials/whatsapp/biz
