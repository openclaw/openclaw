...
代理循环是代理的完整“真实”运行：输入 → 上下文组装 → 模型推理 → 工具执行 → 流式回复 → 持久化。这是将消息转化为操作和最终回复的权威路径，同时保持会话状态一致。
...
- 消息通道可以选择队列模式（收集/引导/跟进），为这个通道系统提供输入。
...
- **`agent:bootstrap`**：在系统提示最终确定之前构建引导文件时运行。
  使用此钩子添加/删除引导上下文文件。
- **命令钩子**：`/new`、`/reset`、`/stop` 和其他命令事件（见钩子文档）。
...
- **`before_prompt_build`**：在会话加载后运行（带 `messages`），在提示提交之前注入 `prependContext`、`systemPrompt`、`prependSystemContext` 或 `appendSystemContext`。使用 `prependContext` 用于每轮动态文本，使用系统上下文字段用于应位于系统提示空间中的稳定指导。
- **`before_agent_start`**：可能在任一阶段运行的遗留兼容性钩子；更喜欢上面的显式钩子。
- **`before_agent_reply`**：在内联操作后和 LLM 调用前运行，让插件声明轮次并返回合成回复或完全静音轮次。
...
请参阅 [插件钩子](/plugins/architecture#provider-runtime-hooks) 了解钩子 API 和注册详情。
...
- 工具结果在日志记录/发出之前会针对大小和图像有效负载进行清理。
...
- 最终有效负载由以下部分组装：
  ...
  - 内联工具摘要（当详细且允许时）
  - 模型出错时的助手错误文本
- 确切的静默令牌 `NO_REPLY` / `no_reply` 从出站有效负载中过滤。
- 消息传递工具重复项从最终有效负载列表中删除。
...
- LLM 空闲超时：`agents.defaults.llm.idleTimeoutSeconds` 在空闲窗口之前没有响应块到达时中止模型请求。为慢速本地模型或推理/工具调用提供商明确设置它；将其设置为 0 以禁用。如果未设置，OpenClaw 使用配置的 `agents.defaults.timeoutSeconds`，否则为 120 秒。没有明确 LLM 或代理超时的 cron 触发运行会禁用空闲监视器并依赖于 cron 外部超时。
