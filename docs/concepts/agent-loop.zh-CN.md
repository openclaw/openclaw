...
代理循环是代理的完整“真实”运行：输入 → 上下文组装 → 模型推理 → 工具执行 → 流式回复 → 持久化。这是将消息转化为行动和最终回复的权威路径，同时保持会话状态一致。

在 OpenClaw 中，循环是每个会话的单个序列化运行，在模型思考、调用工具和流式输出时发出生命周期和流事件。本文档解释了这个真实循环如何端到端连接。
...
- 运行通过会话键（会话通道）和可选的全局通道序列化。
- 这可以防止工具/会话竞争，并保持会话历史一致。
- 消息通道可以选择队列模式（collect/steer/followup）来为这个通道系统提供输入。
...
- 系统提示由 OpenClaw 的基础提示、技能提示、引导上下文和每次运行的覆盖构建而成。
- 强制执行特定于模型的限制和压缩保留令牌。
- 请参阅 [系统提示](/concepts/system-prompt) 了解模型看到的内容。
...
- **`agent:bootstrap`**：在系统提示最终确定之前构建引导文件时运行。
  使用此钩子添加/删除引导上下文文件。
- **命令钩子**：`/new`、`/reset`、`/stop` 和其他命令事件（请参阅钩子文档）。

请参阅 [钩子](/automation/hooks) 了解设置和示例。

### 插件钩子（代理 + 网关生命周期）

这些在代理循环或网关管道内运行：

- **`before_model_resolve`**：在会话前运行（无 `messages`），在模型解析之前确定性地覆盖提供者/模型。
- **`before_prompt_build`**：在会话加载后运行（带 `messages`），在提示提交之前注入 `prependContext`、`systemPrompt`、`prependSystemContext` 或 `appendSystemContext`。对于每轮动态文本使用 `prependContext`，对于应该位于系统提示空间中的稳定指导使用系统上下文字段。
- **`before_agent_start`**：可能在任一阶段运行的遗留兼容性钩子；优先使用上面的显式钩子。
- **`before_agent_reply`**：在内联操作后和 LLM 调用前运行，让插件声明回合并返回合成回复或完全沉默回合。
...
- `before_tool_call`：`{ block: false }` 是无操作，不会清除先前的阻塞。
- `before_install`：`{ block: true }` 是终端的，会停止低优先级处理程序。
- `before_install`：`{ block: false }` 是无操作，不会清除先前的阻塞。
- `message_sending`：`{ cancel: true }` 是终端的，会停止低优先级处理程序。
- `message_sending`：`{ cancel: false }` 是无操作，不会清除先前的取消。

请参阅 [插件钩子](/plugins/architecture#provider-runtime-hooks) 了解钩子 API 和注册详细信息。
...
- 请参阅 [流式传输](/concepts/streaming) 了解分块和块回复行为。

## 工具执行 + 消息工具

- 工具开始/更新/结束事件在 `tool` 流上发出。
- 工具结果在记录/发出之前会根据大小和图像有效载荷进行清理。
- 消息工具发送被跟踪以抑制重复的助手确认。
...
- 最终有效载荷由以下内容组装：
  ...
  - 内联工具摘要（当详细且允许时）
  - 模型错误时的助手错误文本
- 确切的静默令牌 `NO_REPLY` / `no_reply` 会从出站有效载荷中过滤掉。
- 消息工具重复项从最终有效载荷列表中移除。
...
- 请参阅 [压缩](/concepts/compaction) 了解压缩管道。

## 事件流（当前）

- `lifecycle`：由 `subscribeEmbeddedPiSession` 发出（作为 `agentCommand` 的回退）
...
- LLM 空闲超时：`agents.defaults.llm.idleTimeoutSeconds` 当没有响应块在空闲窗口之前到达时中止模型请求。为慢速本地模型或推理/工具调用提供者明确设置；设置为 0 以禁用。如果未设置，OpenClaw 在配置时使用 `agents.defaults.timeoutSeconds`，否则使用 120s。没有明确 LLM 或代理超时的 cron 触发运行会禁用空闲看门狗并依赖于 cron 外部超时。
