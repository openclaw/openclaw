...
- 在 `connect` 中提供设备身份；配对是**基于设备**的（角色 `node`），批准存储在设备配对存储中。
...
- `hello-ok.features.methods` / `events` 是发现元数据，不是每个可调用帮助路由的生成转储。
- 共享密钥认证使用 `connect.params.auth.token` 或 `connect.params.auth.password`，取决于配置的网关认证模式。
- 身份承载模式，如 Tailscale Serve（`gateway.auth.allowTailscale: true`）或非环回 `gateway.auth.mode: "trusted-proxy"` 从请求头而非 `connect.params.auth.*` 满足认证。
- 私有入口 `gateway.auth.mode: "none"` 完全禁用共享密钥认证；在公共/不可信入口上保持该模式关闭。
...
- OpenClaw 还为受信任的共享密钥帮助流程提供了狭窄的后端/容器本地自连接路径。
- Tailnet 和 LAN 连接，包括同一主机的 tailnet 绑定，仍然需要明确的配对批准。
- 所有连接必须对 `connect.challenge` 随机数进行签名。
- 签名有效负载 `v3` 还绑定 `platform` + `deviceFamily`；网关在重新连接时固定配对元数据，并且元数据更改需要修复配对。
- **非本地**连接仍然需要明确批准。
- 网关认证（`gateway.auth.*`）仍然适用于**所有**连接，本地或远程。
