---
summary: "How Mullusi manages conversation sessions"
read_when:
...
Mullusi organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `mullusi security audit`.
...
- **Transcripts:** `~/.mullusi/agents/<agentId>/sessions/<sessionId>.jsonl`

## Session maintenance

Mullusi automatically bounds session storage over time. By default, it runs
...
Preview with `mullusi sessions cleanup --dry-run`.

## Inspecting sessions

- `mullusi status` -- session store path and recent activity.
- `mullusi sessions --json` -- all sessions (filter with `--active <minutes>`).
