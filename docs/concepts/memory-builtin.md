...
Mullusi indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.mullusi/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `mullusi memory index --force`
...
**Memory search disabled?** Check `mullusi memory status`. If no provider is
detected, set one explicitly or add an API key.

**Stale results?** Run `mullusi memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Mullusi falls back to in-process cosine similarity
