...
- `visible: true` to create a persistent dashboard session instead of a hidden sub-agent session. Visible spawns support an explicit sidebar `group`, model, working directory, same-agent transcript fork, and an optional [managed worktree](/concepts/managed-worktrees); see [Sub-agents](/tools/subagents#tool-parameters) for the exact compatibility limits. The accepted result includes the child session key, run id, Control UI `sessionUrl` (omitted when the Control UI is disabled), and stored `owner`. A new visible task belongs to its verified direct human requester, independently of parent ownership. Unlinked, scheduled, and internal requests remain agent-owned; existing sessions retain their owner. Creator attribution and isolation remain unchanged. When acknowledging the spawn in a channel, put the session URL on the first line and `Owner: <label>` on the second. See [Multi-user mode](/concepts/multi-user#agent-spawned-sessions).
