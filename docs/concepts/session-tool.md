...
Each delegated turn can use only the OpenClaw tools available to the sender when it called `sessions_send`, further restricted by the target's own policy. This includes deny-only policies and chained sends. The restriction applies to that turn and its reply/announce steps; it does not change the target session's settings. Automatic delivery to the established announce destination remains separate from permission to use the `message` tool.

An active target accepts a handoff only when its current tool authority is compatible with the sender's and its runtime supports guarded injection. Otherwise, wait for it to finish and retry with its durable session key. CLI targets use the mediated OpenClaw tool surface; a backend or transport that cannot enforce the restriction returns an error before starting the turn.
