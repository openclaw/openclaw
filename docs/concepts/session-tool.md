...
- `taskDeliveryMode: "system_and_transcript"` for native sub-agents when the
  child transcript must visibly contain the delegated task for auditability.
  The default `"system"` keeps the task in the hidden system prompt only to
  avoid duplicating first-request tokens.
