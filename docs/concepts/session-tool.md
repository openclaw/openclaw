...
The `sessions` tool exposes bounded self-service surfaces. Agent turns without affirmative owner authority receive only `assign_owner`, including senderless children; settings, lifecycle, cloud-profile discovery, and global group operations remain owner-gated. Explicit tool denies still remove the tool. Standalone HTTP/RPC tool invocation and session-bound MCP attach grants retain their owner gate and do not gain agent identity. Non-owner assignment requires a live admitted agent turn; its authority is rechecked at the owner write.
