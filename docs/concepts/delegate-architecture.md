---
summary: "Delegate architecture: running Mullusi as a named agent on behalf of an organization"
...
Goal: run Mullusi as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Mullusi agent that:
...
Mullusi's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Mullusi's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.mullusi/workspace-delegate",
  tools: {
...
- Cron run history: `~/.mullusi/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.mullusi/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Mullusi's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
mullusi agents add delegate
```

This creates:

- Workspace: `~/.mullusi/workspace-delegate`
- State: `~/.mullusi/agents/delegate/agent`
- Sessions: `~/.mullusi/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.mullusi/workspace" },
      {
        id: "delegate",
        workspace: "~/.mullusi/workspace-delegate",
        tools: {
...
~/.mullusi/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.mullusi/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.mullusi/workspace-org",
        agentDir: "~/.mullusi/agents/org-assistant/agent",
...
recall view. Mullusi redacts credential/token-like text, truncates long
