...
- `/usage cost` in chats: local cost summary aggregated from EasyHub session logs.
- CLI: `EasyHub status --usage` prints a full per-provider breakdown.
- CLI: `EasyHub channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
