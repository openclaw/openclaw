...
- `/usage cost` in chats: local cost summary aggregated from Activi session logs.
- CLI: `activi status --usage` prints a full per-provider breakdown.
- CLI: `activi channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
