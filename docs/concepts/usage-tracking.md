...
- `/usage cost` in chats: local cost summary aggregated from EVOX.sh session logs.
- CLI: `evox status --usage` prints a full per-provider breakdown.
- CLI: `evox channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
