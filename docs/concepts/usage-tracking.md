...
- `/usage cost` in chats: local cost summary aggregated from Mullusi session logs.
- CLI: `mullusi status --usage` prints a full per-provider breakdown.
- CLI: `mullusi channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Mullusi treats
  ...
  quota, so Mullusi inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Mullusi prefers the
...
can supply plugin-specific usage auth logic; otherwise Mullusi falls back to
