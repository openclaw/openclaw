...
- `/usage cost` in chats: local cost summary aggregated from DNA session logs.
- CLI: `dna status --usage` prints a full per-provider breakdown.
- CLI: `dna channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
