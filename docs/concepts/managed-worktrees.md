...
When a sandboxed agent with read-write workspace access starts another worktree session, the Gateway coordinates creation but runs repository Git in a dedicated Docker or Podman provisioning sandbox. The source checkout, allocation directory, and worktree snapshot temporary directory are mounted at stable paths so Git linked-worktree metadata remains valid. Repository transports, credential helpers, filters, and other Git-configured commands therefore run with sandbox authority rather than Gateway-host authority. The provisioning command receives the agent's explicitly configured sandbox environment, but not ambient Gateway environment variables, host credential helpers, SSH agent sockets, or arbitrary sandbox bind mounts.

This path fails closed if its local container sandbox cannot be created; it never retries repository Git on the Gateway host. The worktree registry retains that provenance, so snapshot, restore, and cleanup operations remain contained even if the session's sandbox policy changes later. Install and configure Docker or Podman for agents that combine read-write sandbox workspaces with managed-worktree spawning. Trusted operator-created worktrees and repositories that were not writable by a sandbox retain the normal host Git path. After preparation, the child starts in its own sandbox. OpenClaw mounts the registered repository's shared Git metadata at its canonical path so ordinary Git reads—and writes when the child's workspace policy is read-write—continue to work in the linked checkout without sharing the provisioning runtime.
