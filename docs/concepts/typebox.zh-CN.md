...
TypeBox 是一个 TypeScript 优先的模式库。我们使用它来定义 **网关
WebSocket 协议**（握手、请求/响应、服务器事件）。这些模式驱动 **运行时验证**、**JSON Schema 导出**和 **Swift 代码生成** 用于
macOS 应用。一个事实来源；其他一切都是生成的。
...
第一个帧 **必须** 是 `connect` 请求。之后，客户端可以调用
...
- 广告特性/发现注册表：`src/gateway/server-methods-list.ts`
...
- **服务器端**：每个入站帧都使用 AJV 进行验证。握手只接受参数匹配 `ConnectParams` 的 `connect` 请求。
- **客户端**：JS 客户端在使用事件和响应帧之前验证它们。
- **特性发现**：网关在 `hello-ok` 中发送保守的 `features.methods` 和 `features.events` 列表，来自 `listGatewayMethods()` 和 `GATEWAY_EVENTS`。
- 该发现列表不是 `coreGatewayHandlers` 中每个可调用助手的生成转储；一些助手 RPC 在 `src/gateway/server-methods/*.ts` 中实现，而未在广告特性列表中枚举。
...
如果该方法可由操作员或节点客户端调用，还需在 `src/gateway/method-scopes.ts` 中对其进行分类，以便范围强制和 `hello-ok` 特性广告保持一致。
...
- `GatewayFrame` 枚举，包含 `req`、`res`、`event` 和 `unknown` 案例
- 强类型有效负载结构体/枚举
- `ErrorCode` 值和 `GATEWAY_PROTOCOL_VERSION`

未知帧类型保留为原始有效负载，以实现向前兼容性。
...
- 大多数对象使用 `additionalProperties: false` 用于严格有效负载。
- `NonEmptyString` 是 ID 和方法/事件名称的默认值。
- 顶级 `GatewayFrame` 在 `type` 上使用 **鉴别器**。
- 具有副作用的方法通常需要参数中的 `idempotencyKey`（示例：`send`、`poll`、`agent`、`chat.send`）。
