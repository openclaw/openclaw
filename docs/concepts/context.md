...
read_when:
  - You want to understand what “context” means in Hanzo Bot
...
“Context” is **everything Hanzo Bot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Hanzo Bot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Hanzo Bot builds the system prompt

The system prompt is **Hanzo Bot-owned** and rebuilt each run. It includes:
...
By default, Hanzo Bot injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). Bot also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `150000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
