...
read_when:
  - You want to understand what “context” means in EasyHub
...
“Context” is **everything EasyHub sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (EasyHub-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How EasyHub builds the system prompt

The system prompt is **EasyHub-owned** and rebuilt each run. It includes:
...
By default, EasyHub injects a fixed set of workspace files (if present):
