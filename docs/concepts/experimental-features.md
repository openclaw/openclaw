...
This Labs entry permits automatic Decision assistance for separately enabled
consumer modes and agents with an effective Decision model. Turning it on does
not enable consumer modes, select a provider, provision credentials, or download
models. Turn-context curation additionally requires its explicit shadow or apply
mode; shadow observes without altering model input, while apply may omit
owner-attested discretionary completed tool exchanges from the temporary input.
Compaction shadow observation also requires its separate
`agents.defaults.compaction.semanticCuration.mode: "shadow"` setting and
preserves summarizer input and accepted summaries. Its cooperative cancellation
budget can delay completion until a provider ignoring cancellation settles.
These consumers send bounded conversation evidence to the selected provider. The canonical
transcript is preserved; hosted evaluations may incur charges and add latency.
