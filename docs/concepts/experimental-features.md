...
| Skill Search     | `skills.experimental.search`                                            | You want optional local search across eligible skills in Code Mode without enlarging the prompt; disabled by default              | [Skill Search](/tools/skills#skill-search)                                             |
...
Labs includes Code Mode, Skill Search, Tool Search for all models, Custom plugin UI,
...
Code Mode, Skill Search, and Tool Search changes take effect for future agent runs.
...
Skill Search is off by default and requires Code Mode. Its Labs switch sets
`skills.experimental.search: true`; only the boolean `true` enables it. With
the setting off or absent, `skills.list()` and `skills.read(name)` retain the
prompt-admitted catalog, and no `skills.search` API or search hint is exposed.
Opting in makes the broader eligible catalog available to list, read, and local
search without enlarging the prompt. Existing skill exclusions still apply.
