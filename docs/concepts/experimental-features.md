...
| Structured planning tool | `tools.experimental.planTool`                                                              | Kill-switch for the structured `update_plan` tool. Default-on for all models (Codex-parity plan mode); set `false` to remove it    | [Gateway configuration reference](/gateway/config-tools#toolsexperimental)                    |
| Autonomous goal driver   | `tools.experimental.goalDriver`                                                            | You want a session carrying an active goal pursued unattended — the driver wakes a fresh turn while idle until done/blocked/paused | [Gateway configuration reference](/gateway/config-tools#toolsexperimental)                    |
