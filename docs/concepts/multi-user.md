...
New visible sessions created with `sessions_spawn` belong to the verified person who directly requested the work, including requests in a shared channel or another person's session. The Gateway records the owner before the first task starts. Agents do not need a separate assignment command. An unlinked requester, scheduled job, or internal agent turn keeps the requesting agent as owner. Resuming an existing session preserves its owner.

The requesting agent normally remains the immutable creator; a required sandbox preserves the parent's creator provenance as an isolation policy. Assignment does not change sharing, visibility, sandbox requirements, permission mode, or model credentials. Eligible later human turns use the assigned person's personal instructions, as described in [User model](/concepts/user-model).
