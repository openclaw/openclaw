...
| `config.circuitBreakerMaxTimeouts` | `number` | Skip recall after this many consecutive timeouts for the same agent/model. After the cooldown a single probe recall is allowed through: it clears the breaker on success or re-opens it immediately if the probe also times out, so a persistently failing agent/model stops paying the recall timeout on every turn (range 1-20; default 3). |
| `config.circuitBreakerCooldownMs`  | `number` | How long to skip recall after the circuit breaker trips before allowing one probe recall through, in ms. Turns spaced further apart than this still probe on each turn, so raise it to widen protection for low-cadence chats (range 5000-600000; default 60000).                                                                             |
