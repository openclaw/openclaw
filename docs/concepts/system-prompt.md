...
bootstrap files. `SOUL.md` is forwarded as Codex developer instructions so the
active agent's persona and operating style remain high priority without being
repeated in every user turn. `TOOLS.md`, `IDENTITY.md`, `USER.md`,
`HEARTBEAT.md`, `BOOTSTRAP.md`, and `MEMORY.md` keep the normal
workspace-context role without duplicating `AGENTS.md`.
