...
EVOX.sh selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models EVOX.sh can use (plus aliases).
...
- If you omit the provider, EVOX.sh treats the input as an alias or a model for the **default provider** (only works when there is no `/` in the model ID).
...
evox models list
evox models status
evox models set <provider/model>
evox models set-image <provider/model>

evox models aliases list
evox models aliases add <alias> <provider/model>
evox models aliases remove <alias>

evox models fallbacks list
evox models fallbacks add <provider/model>
evox models fallbacks remove <provider/model>
evox models fallbacks clear

evox models image-fallbacks list
evox models image-fallbacks add <provider/model>
evox models image-fallbacks remove <provider/model>
evox models image-fallbacks clear
```

`evox models` (no subcommand) is a shortcut for `models status`.
...
`evox models scan` inspects OpenRouter’s **free model catalog** and can
...
Marker persistence is source-authoritative: EVOX.sh writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever EVOX.sh regenerates `models.json`, including command-driven paths like `evox agent`.
