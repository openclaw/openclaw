...
An existing session whose pin is now blocked keeps that pin and runs on its effective default. The reply starts with a notice that the pin is outside the allow list, the default was used, and `/model` changes the selection. The notice appears once per session until the pin changes. OpenClaw stores the delivered notice in that reply's existing transcript row, so other clients sharing the session can read the same explanation. It records the notice receipt only after delivery and transcript publication succeed. `/compact` follows the same rule for OpenClaw-owned history. If a native runtime owns the session history, change the model with `/model` before compacting; OpenClaw does not compact a different runtime's history. These turns do not use automatic model fallback. If the default cannot run, the turn stays blocked with repair guidance. `/model` and `/models` remain available for recovery.

If an authored primary is absent from the prepared model catalog, OpenClaw uses an allowed model from the configured provider order, then the available catalog. The reply explains which default answered. The picker labels that same model **Default**. A fallback that resolves to the same confirmed-missing primary is skipped, including an alias for that model. Authored configuration and existing session pins stay unchanged. A valid custom model or provider-resolved model remains usable even when it is absent from the browse list. Missing credentials do not mean that a model is absent. If no allowed default exists, the reply gives repair guidance without sending the missing primary or a blocked session pin.
