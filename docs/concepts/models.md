...
| Auto fallback                                                           | Turn-local: the fallback candidate applies to the current turn and the next turn starts from the selected primary again. Nothing writes a new `modelOverrideSource: "auto"` session entry; OpenClaw only still recognizes entries stored by older releases, reprobes their origin, and clears them on recovery. See [Model failover](/concepts/model-failover). |
