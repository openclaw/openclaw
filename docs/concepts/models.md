...
- Chat, New Session, and Models settings put the selected model first. Other rows keep the Gateway's catalog order, including provider-curated recommendations where supplied. Text `/models <provider>` pages also put the current model first instead of alphabetizing the catalog. Picker search still checks the full list, not just the visible rows.
