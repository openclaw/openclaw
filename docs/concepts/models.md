...
- User session selections are a preference, not a hard lock. `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` store `modelOverrideSource: "user"`; the pinned model is always tried first, but if it hits a long-term quota limit or subscription exhaustion, OpenClaw falls back to the configured fallback chain instead of failing indefinitely.
...
<AccordionGroup>
  ...
  <Accordion title="Persistence and live switching">
    ...
    - A user-selected `/model` ref is preferred for that session: the pinned model is always tried first. On transient errors the reply fails visibly; on long-term quota exhaustion or subscription limits, OpenClaw falls back to the configured fallback chain rather than failing indefinitely.
