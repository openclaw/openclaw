...
- Signing in to a provider keeps existing choices visible in open Control UI and terminal model pickers while discovery refreshes in the background. Changes to model restrictions or operator roles still retire the old choices until the replacement catalog is ready.
