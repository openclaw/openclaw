...
Mullusi selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Mullusi can use (plus aliases).
...
- If a run is already active, Mullusi marks a live switch as pending and only restarts into the new model at a clean retry point.
...
- If you omit the provider, Mullusi resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, Mullusi
...
mullusi models set <provider/model>
mullusi models set-image <provider/model>

mullusi models aliases list
mullusi models aliases add <alias> <provider/model>
mullusi models aliases remove <alias>

mullusi models fallbacks list
mullusi models fallbacks add <provider/model>
mullusi models fallbacks remove <provider/model>
mullusi models fallbacks clear

mullusi models image-fallbacks list
mullusi models image-fallbacks add <provider/model>
mullusi models image-fallbacks remove <provider/model>
mullusi models image-fallbacks clear
```

`mullusi models` (no subcommand) is a shortcut for `models status`.
...
`mullusi models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.mullusi/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Mullusi writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Mullusi regenerates `models.json`, including command-driven paths like `mullusi agent`.
