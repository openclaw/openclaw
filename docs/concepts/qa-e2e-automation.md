...
For the Docker observability smoke, run:

```bash
pnpm test:docker:observability
```

That builds or reuses the shared Docker E2E image, runs the OTEL trace smoke
inside the container, then runs the `docker-prometheus-smoke` QA scenario with
the `diagnostics-prometheus` plugin enabled. Set
