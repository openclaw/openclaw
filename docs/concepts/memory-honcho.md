...
[Honcho](https://honcho.dev) adds AI-native memory to Mullusi. It persists
...
```bash
mullusi plugins install @honcho-ai/mullusi-honcho
mullusi honcho setup
...
Settings live under `plugins.entries["mullusi-honcho"].config`:
    ...
    entries: {
      "mullusi-honcho": {
        config: {
          ...
          workspaceId: "mullusi", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `mullusi honcho setup` detects and
...
```bash
mullusi honcho setup                        # Configure API key and migrate files
mullusi honcho status                       # Check connection status
mullusi honcho ask <question>               # Query Honcho about the user
mullusi honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/mullusi-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Mullusi integration guide](https://docs.honcho.dev/v3/guides/integrations/mullusi)
- [Memory](/concepts/memory) -- Mullusi memory overview
