...
<AccordionGroup>
  <Accordion title="What lands in the rate-limit / timeout bucket">
    ...
    HTTP status mapping follows that same rule. An untyped `5xx` response classifies as **`server_error`**, so a `502` or `503` reports a provider failure instead of claiming the request timed out, and it stays eligible for retry-limit model fallback. Only the timing statuses classify as **`timeout`**: the request timeout `408`, the gateway timeouts `504`, `522`, and `524`, and the nginx client-abort `499`. A provider-typed body still wins: a `5xx` whose payload declares `overloaded_error` or `server_error` keeps that classification.
