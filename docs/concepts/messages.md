...
default. `messages.queue` selects fallback behavior for runtimes that cannot
accept same-turn steering.

- Configure via `messages.queue` (and `messages.queue.byChannel`).
- Default fallback mode is `followup`, with a 500ms debounce.
- Fallback modes: `followup`, `collect`, and `interrupt`.
