...
read_when:
  ...
  - Vous avez besoin de configurer les identifiants et la région AWS pour les appels de modèle
...
- `enabled` prend par défaut la valeur `true` lorsque des identifiants AWS sont présents.
...
Lorsque OpenClaw s’exécute sur une instance EC2 avec un rôle IAM attaché, le SDK AWS utilisera automatiquement le service de métadonnées d’instance (IMDS) pour l’authentification. Cependant, la détection des identifiants par OpenClaw vérifie actuellement uniquement les variables d’environnement, et non les identifiants IMDS.

**Contournement :** Définissez `AWS_PROFILE=default` pour indiquer que des identifiants AWS sont disponibles. L’authentification réelle utilise toujours le rôle d’instance via IMDS.
...
- Si vous préférez un flux avec clé gérée, vous pouvez également placer un proxy compatible OpenAI devant Bedrock et le configurer comme fournisseur OpenAI à la place.
