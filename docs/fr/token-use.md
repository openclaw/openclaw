---
summary: "Comment OpenClaw construit le contexte de prompt et signale l’utilisation des tokens et les coûts"
read_when:
...
OpenClaw suit les **tokens**, pas les caractères. Les tokens dépendent du modèle, mais la plupart des
modèles de type OpenAI font en moyenne ~4 caractères par token pour le texte anglais.
...
- Fichiers d’espace de travail + de bootstrap (`AGENTS.md`, `SOUL.md`, `TOOLS.md`, `IDENTITY.md`, `USER.md`, `HEARTBEAT.md`, `BOOTSTRAP.md` lorsqu’ils sont nouveaux). Les fichiers volumineux sont tronqués par `agents.defaults.bootstrapMaxChars` (par défaut : 20000).
...
- Enveloppes fournisseur ou en‑têtes de sécurité (non visibles, mais comptabilisés)
...
- `/usage off|tokens|full` → ajoute un **pied de page d’utilisation par réponse** à chaque réplique.
  - Persiste par session (stocké comme `responseUsage`).
  - L’authentification OAuth **masque le coût** (tokens uniquement).
- `/usage cost` → affiche un récapitulatif de coûts local à partir des journaux de session OpenClaw.
...
Il s’agit de **USD par 1 M de tokens** pour `input`, `output`, `cacheRead` et
...
La mise en cache des prompts côté fournisseur ne s’applique que pendant la fenêtre de TTL du cache. OpenClaw peut
exécuter en option un **élagage cache‑ttl** : il élague la session une fois le TTL du cache
expiré, puis réinitialise la fenêtre de cache afin que les requêtes suivantes puissent réutiliser le
contexte fraîchement mis en cache au lieu de remettre en cache tout l’historique. Cela maintient
des coûts d’écriture de cache plus bas lorsqu’une session reste inactive au‑delà du TTL.

Configurez‑le dans la [configuration du Gateway (passerelle)](/gateway/configuration) et consultez les
détails de comportement dans [Session pruning](/concepts/session-pruning).

Le heartbeat peut maintenir le cache **chaud** pendant les périodes d’inactivité. Si le TTL de cache de votre modèle
est `1h`, définir l’intervalle de heartbeat juste en dessous (par ex. `55m`) peut éviter
la remise en cache du prompt complet, réduisant les coûts d’écriture du cache.
...
la plus récente du prompt caching d’Anthropic pour les taux et multiplicateurs de TTL :
https://docs.anthropic.com/docs/build-with-claude/prompt-caching

### Exemple : maintenir un cache d’1 h chaud avec le heartbeat
