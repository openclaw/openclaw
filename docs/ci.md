...
- Node shard and build-artifact jobs also restore Node's portable on-disk compile cache. Protected Blacksmith snapshots are read-only in PRs and have one writer on trusted non-PR runs; fallback archives remain PR-scoped. This reuses V8 bytecode for Node-loaded orchestration, build tooling, and external dependencies across different checkout paths. Vitest child processes disable an inherited compile cache because coverage can be enabled inside dynamic configs and V8 coverage can lose source-position precision when scripts are deserialized from bytecode.
