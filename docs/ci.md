...
Eligible PR source and test changes use compiler-consumer type selection only when every selected path exists in the checkout. GitHub and hybrid retain their five core stripes even when selection reaches all graphs; each stripe checks only its selected consumers. Deleting a core test keeps the full striped plan. See [static checks](/ci/runners#runner-backend-modes).
