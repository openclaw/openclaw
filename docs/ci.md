...
The `check-dependencies` shard runs Knip twice over `config/knip.config.ts`: a `--production` pass and a full-tree pass. The unused-file guard fails when a PR adds an unused file, while preserving intentional dynamic plugin, generated, build, live-test, and package bridge surfaces modeled in that config. The production pass fails on exports no production module consumes; the full-tree pass makes tests entrypoints and fails on dead test support, so neither hides the other's dead code. Config hints are left enabled, but the compact reporter does not render them; rerun a scan with `--reporter symbols` to audit ignore entries that no longer suppress anything. Historical targets run the export guard when they provide it and retain their older dead-code fallback otherwise.
