...
Manual runs use a unique concurrency group so a release-candidate full suite is not cancelled by another push or PR run on the same ref. The optional `target_ref` input lets a trusted caller run that graph against a branch, tag, or full commit SHA while using the workflow file from the selected dispatch ref. The optional `loc_base_ref` supplies an exact comparison SHA for standalone manual runs. The `release_gate` input is an exact-SHA maintainer fallback for capacity-stalled PR CI: current workflow revisions require `target_ref` to be a full commit SHA that matches the dispatched branch head and `pr_number` to identify the open pull request. The workflow authenticates that PR's head and derives its merge base from the PR's actual target branch for the changed-file TypeScript LOC ratchet. Because manual dispatch uses the target-owned workflow revision, callers must check its declared inputs: current revisions receive `pr_number`; transitional revisions that declare `loc_base_ref` are valid for default-branch PRs only and receive that PR's merge-base SHA; oldest revisions declaring neither input keep the legacy exact-SHA command without either input. A non-default-base PR on the transitional schema must first update its head to the current `pr_number` workflow and restart proof on the new SHA.
