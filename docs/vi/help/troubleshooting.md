...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T09:39:21Z
---
...
<AccordionGroup>
  ...
  <Accordion title="Bảng điều khiển hoặc Control UI không kết nối được">
    ```bash
    openclaw status
    openclaw gateway status
    openclaw logs --follow
  ...
  <Accordion title="Cron hoặc heartbeat không kích hoạt hoặc không gửi được">
    ```bash
    openclaw status
    openclaw gateway status
  ...
  <Accordion title="Node đã ghép cặp nhưng công cụ thất bại với camera/canvas/screen/exec">
    ```bash
    openclaw status
    openclaw gateway status
