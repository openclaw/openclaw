---
summary: "CLI reference for `evox memory` (status/index/search)"
read_when:
...
```bash
evox memory status
evox memory status --deep
evox memory index --force
evox memory search "meeting notes"
evox memory search --query "deployment" --max-results 20
evox memory status --json
evox memory status --deep --index
evox memory status --deep --index --verbose
evox memory status --agent main
evox memory index --agent main --verbose
```
