...
- When the CLI times out waiting for a Gateway-backed agent reply, the CLI returns `status: "accepted_timeout"` with the Gateway-assigned `runId` only if the Gateway already accepted the turn. No embedded fallback is launched for an accepted run — the Gateway accepted the turn before transport was lost and it may still be running — which prevents duplicate execution and preserves the configured provider/model routing. The stderr hint says to check `openclaw gateway status` and the session transcript before retrying or rerunning with `--local`, so the turn does not execute twice.
