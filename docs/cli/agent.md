...
- A transport close or timeout is ambiguous even if the CLI never received the Gateway's `accepted` response. The CLI reconnects, waits for the original run id, and requests a cache-only idempotent replay before considering fallback. Each invocation uses a separate random recovery capability, so knowing another run id is not enough to read its cached result.
- During a mixed-version upgrade, if an older Gateway rejects the recovery capability field, the CLI retries that initial request once with the legacy parameter set. Cached-result recovery is unavailable for that request, so a later ambiguous transport failure follows the existing isolated embedded-fallback behavior.
- If recovery cannot determine whether the original run owns the work, the CLI preserves the existing automatic fallback behavior. It starts a fresh `gateway-fallback-*` session/run id and reports `meta.transport: "embedded"`, `meta.fallbackFrom: "gateway"`, `meta.fallbackReason`, and the fallback session fields. The original Gateway run may still have completed, so tools or `--deliver` can produce duplicate external side effects in this rare path. A cache replay that reports `status: "in_flight"` positively confirms that the original run still owns the work; the CLI keeps waiting until its deadline, then reports `in_flight` without starting an embedded duplicate.
