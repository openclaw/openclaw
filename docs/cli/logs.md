...
- When the Gateway reports a different log file path, `--follow` rereads the new file without the old byte cursor before printing its lines. The new tail still respects `--limit` and `--max-bytes`.
