---
summary: "CLI reference for `evox message` (send + channel actions)"
read_when:
...
- On cache miss, EVOX.sh will attempt a live directory lookup when the provider supports it.
...
- `evox message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, EVOX.sh does not force a `default` account SecretRef scope
...
```
evox message send --channel discord \
...
```
evox message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
evox message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
evox message send --channel msteams \
...
```
evox message poll --channel msteams \
...
```
evox message react --channel slack \
...
```
evox message react --channel signal \
...
```
evox message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
evox message send --channel telegram --target @mychat \
