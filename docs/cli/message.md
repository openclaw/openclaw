...
| `edit`          | Discord, Google Chat, Matrix, Microsoft Teams, Slack, Telegram                                                  | `--message-id`, `--message`, `--target`                        | Google Chat requires a full `spaces/<spaceId>/messages/<messageId>` resource name from the same target space and can edit only the selected app's messages. Telegram forum threads use `--thread-id`.                                                                                                  |
