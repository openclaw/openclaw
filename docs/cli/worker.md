...
On current Linux and macOS node hosts, the launch journal identifies the worker's process
owner; the application `worker.mjs` runs as its child. The owner survives an
application crash and retains nested command cleanup before releasing capacity.
...
Existing launch records with a live raw worker retain their TERM/KILL recovery.
If that root has already disappeared while its group remains, or its live role
cannot be read, recovery keeps capacity occupied and reports that the cleanup
owner could not be verified. Inspect the remaining worker processes before
retrying recovery; missing role evidence never authorizes killing a possible
cleanup observer.
