...
| `clientCert` / `clientKey`  | mTLS client certificate and key paths (64 KiB maximum per file)  |
...
For mutual TLS, `clientCert` and `clientKey` must point to regular PEM files of at
most 64 KiB each, including any certificate chain. Symlinks and hardlinks are
supported. Run `openclaw mcp doctor` before connecting to report missing,
unreadable, or oversized files. These checks also apply to Streamable HTTP.
OpenClaw rejects oversized files before opening an MCP connection; remove
unrelated certificates or extra content from an oversized PEM file.
