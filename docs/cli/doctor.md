...
- Doctor repairs stale plugin config by removing missing plugin ids from `plugins.allow`/`plugins.entries`, resetting stale `plugins.slots.*` selections to their defaults, plus removing matching dangling channel config, heartbeat targets, and channel model overrides when plugin discovery is healthy.
