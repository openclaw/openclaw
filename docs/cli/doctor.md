...
- Doctor preserves explicit `openai-codex/*` model refs instead of silently moving them across billing/auth provider domains. `--fix` repairs generic `openai/*` refs only when they are explicitly paired with the Codex runtime and should stay on the Codex provider route, covers primary and fallback model slots, preserves explicit provider/model `agentRuntime` policy, and removes stale whole-agent/session runtime pins only when they came from older fallback notices.
