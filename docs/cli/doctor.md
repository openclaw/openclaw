...
- Doctor rewrites legacy `openai-codex/*` primary model refs to canonical `openai/*` refs with `agentRuntime.id: "auto"`. That lets the Codex harness claim turns when it is installed and enabled, while PI remains the fallback when it is not.
