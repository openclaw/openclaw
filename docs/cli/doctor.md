...
- Doctor rewrites legacy `openai-codex/*` model refs to canonical `openai/*` refs across primary models, fallbacks, heartbeat/subagent/compaction overrides, hooks, channel model overrides, and stale session route pins. `--fix` preserves explicit `agentRuntime` policy instead of pinning the whole agent to Codex; canonical OpenAI agent refs already select the Codex harness automatically when the official OpenAI provider is in use.
