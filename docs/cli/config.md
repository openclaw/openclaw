---
summary: "CLI reference for `mullusi config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `mullusi.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `mullusi configure`).

Root options:

- `--section <section>`: repeatable guided-setup section filter when you run `mullusi config` without a subcommand
...
```bash
mullusi config file
mullusi config --section model
mullusi config --section gateway --section daemon
mullusi config schema
...
mullusi config set secrets.providers.vaultfile --provider-source file --provider-path /etc/mullusi/secrets.json --provider-mode json
...
mullusi config validate
mullusi config validate --json
```

### `config schema`

Print the generated JSON schema for `mullusi.json` to stdout as JSON.
...
```bash
mullusi config schema > mullusi.schema.json
```
...
mullusi config set channels.whatsapp.groups '["*"]' --strict-json
```
...
`mullusi config set` supports four assignment styles:

1. Value mode: `mullusi config set <path> <value>`
2. SecretRef builder mode:

```bash
mullusi config set channels.discord.token \
...
```bash
mullusi config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/mullusi-vault \
  --provider-arg read \
...
mullusi config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/mullusi/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `mullusi.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.mullusi/mullusi.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
...
- `config file`: Print the active config file path (resolved from `MULLUSI_CONFIG_PATH` or default location).
