...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Mullusi Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`mullusi acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Mullusi channel
conversations instead of hosting an ACP harness session, use
[`mullusi mcp serve`](/cli/mcp) instead.
...
- Mullusi acts as an ACP server
- an IDE or ACP client connects to Mullusi
- Mullusi forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Mullusi runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Mullusi: use `mullusi acp`
- Mullusi should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Mullusi gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
mullusi acp --url wss://gateway-host:18790 --token <token>

# Remote Gateway (token from file)
mullusi acp --url wss://gateway-host:18790 --token-file ~/.mullusi/gateway.token

# Attach to an existing session key
mullusi acp --session agent:main:main

# Attach by label (must already exist)
mullusi acp --session-label "support inbox"

# Reset the session key before the first prompt
mullusi acp --session agent:main:main --reset-session
```
...
mullusi acp client --server-args --url wss://gateway-host:18790 --token-file ~/.mullusi/gateway.token

# Override the server command (default: mullusi)
mullusi acp client --server "node" --server-args mullusi.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Mullusi through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Mullusi Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `mullusi acp` over stdio.

Example config (persisted):

```bash
mullusi config set gateway.remote.url wss://gateway-host:18790
mullusi config set gateway.remote.token <token>
```
...
```bash
mullusi acp --session agent:main:main
mullusi acp --session agent:design:main
mullusi acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Mullusi plugin tools, enable the
...
Mullusi bot over ACP, use `acpx` with its built-in `mullusi` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx mullusi` at `mullusi acp`.
3. Target the Mullusi session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Mullusi ACP session
acpx mullusi exec "Summarize the active Mullusi session state."

# Persistent named session for follow-up turns
acpx mullusi sessions ensure --name codex-bridge
acpx mullusi -s codex-bridge --cwd /path/to/repo \
  "Ask my Mullusi work agent for recent context relevant to this repo."
```

If you want `acpx mullusi` to target a specific Gateway and session key every
time, override the `mullusi` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "mullusi": {
      "command": "env MULLUSI_HIDE_BANNER=1 MULLUSI_SUPPRESS_NOTES=1 mullusi acp --url ws://127.0.0.1:18790 --token-file ~/.mullusi/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Mullusi checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env MULLUSI_HIDE_BANNER=1 MULLUSI_SUPPRESS_NOTES=1 node mullusi.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Mullusi agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Mullusi ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`MULLUSI_GATEWAY_TOKEN`, `MULLUSI_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`MULLUSI_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `MULLUSI_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `mullusi acp client` sets `MULLUSI_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `mullusi`).
