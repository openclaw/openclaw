...
cron resources. It is blocked (blocker `adopted_agent_referenced`) while any
binding, allow-list entry, owner reference, broadcast entry, or hook still
references an adopted agent. Adoption never creates or records those
references, so pruning them would delete operator-owned routing or
authorization. Reassign or remove the named references, then rerun
`claws remove --dry-run`. Removal never deletes an adopted agent's pre-existing
agent directory or database, its durable database registration, session index,
