...
During archive creation, OpenClaw skips known live-mutation files with no restoration value: active agent session transcripts, cron run logs, rolling logs, delivery queues, agent runtime caches, browser cache/resource-cache paths, retired archives, socket/pid/temp files under the state directory, related durable-queue temp files, and lock/partial files only under cache, queue, and tmp scratch roots. The JSON result's `skippedVolatileCount` reports how many files were intentionally omitted. SQLite databases under the state directory are snapshotted safely (`VACUUM INTO`) rather than copied live, so open WAL/SHM files do not corrupt the backup.
