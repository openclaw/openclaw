...
During archive creation, OpenClaw skips known live-mutation and disposable cache paths that do not have restoration value, including active agent session transcripts, agent runtime tmp/cache/shell snapshot directories, browser user-data cache/resource directories, retired archive paths, cron run logs, rolling logs, delivery queues, socket/pid/lock/partial/temp paths under the state directory, and related durable-queue temp files. The JSON result includes `skippedVolatileCount` so automation can see how many paths were intentionally omitted.
