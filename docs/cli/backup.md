...
During archive creation, OpenClaw skips known live-mutation files with no restoration value: active agent session transcripts, cron run logs, rolling logs, delivery queues, socket/pid/temp files under the state directory, and related durable-queue temp files. The JSON result's `skippedVolatileCount` reports how many files were intentionally omitted. Canonical OpenClaw SQLite databases are compacted with `VACUUM INTO`; dedicated plugin databases use SQLite's online backup API so owner-defined functions, collations, and virtual tables are not required by the backup process. Live WAL/SHM files are not copied into the archive.
