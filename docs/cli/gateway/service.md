...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - If no native service is installed, `gateway stop` can also stop a foreground Gateway using its isolated state directory and configuration. This requires a verified live lock and a matching current foreground ownership record for that state; a configured port alone is insufficient. Missing or changed ownership leaves the process running. Native service mutations still require the owning install identity; supervised Gateways must be stopped through their supervisor.
