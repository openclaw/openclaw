---
summary: "CLI reference for `mullusi daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `mullusi daemon ...` in scripts
...
`mullusi daemon ...` maps to the same service control surface as `mullusi gateway ...` service commands.

## Usage

```bash
mullusi daemon status
mullusi daemon install
mullusi daemon start
mullusi daemon stop
mullusi daemon restart
mullusi daemon uninstall
```
...
Use [`mullusi gateway`](/cli/gateway) for current docs and examples.
