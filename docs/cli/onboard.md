---
summary: "CLI reference for `mullusi onboard` (interactive onboarding)"
read_when:
...
```bash
mullusi onboard
mullusi onboard --flow quickstart
mullusi onboard --flow manual
mullusi onboard --mode remote --remote-url wss://gateway-host:18790
```

For plaintext private-network `ws://` targets (trusted networks only), set
`MULLUSI_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `mullusi gateway run`.
...
- Fastest first chat: `mullusi dashboard` (Control UI, no channel setup).
