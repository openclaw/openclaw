...
| `update --force`                 | Replaces a tracked skill even when its installed files no longer match the recorded install digests. Without it, `update` skips skills with local changes so they are not destroyed. Skills installed before OpenClaw recorded install digests cannot be verified, so their first update also asks for one `--force`; that run records digests and later updates verify normally. |
