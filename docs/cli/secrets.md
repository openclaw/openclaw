---
summary: "CLI reference for `mullusi secrets` (reload, audit, configure, apply)"
read_when:
...
Use `mullusi secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
mullusi secrets audit --check
mullusi secrets configure
mullusi secrets apply --from /tmp/mullusi-secrets-plan.json --dry-run
mullusi secrets apply --from /tmp/mullusi-secrets-plan.json
mullusi secrets audit --check
...
```bash
mullusi secrets reload
mullusi secrets reload --json
mullusi secrets reload --url ws://127.0.0.1:18790 --token <token>
```
...
Scan Mullusi state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `mullusi.json` refs)
...
```bash
mullusi secrets audit
mullusi secrets audit --check
mullusi secrets audit --json
mullusi secrets audit --allow-exec
```
...
```bash
mullusi secrets configure
mullusi secrets configure --plan-out /tmp/mullusi-secrets-plan.json
mullusi secrets configure --apply --yes
mullusi secrets configure --providers-only
mullusi secrets configure --skip-provider-setup
mullusi secrets configure --agent ops
mullusi secrets configure --json
```
...
- `configure` targets secret-bearing fields in `mullusi.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Mullusi fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
```bash
mullusi secrets apply --from /tmp/mullusi-secrets-plan.json
mullusi secrets apply --from /tmp/mullusi-secrets-plan.json --allow-exec
mullusi secrets apply --from /tmp/mullusi-secrets-plan.json --dry-run
mullusi secrets apply --from /tmp/mullusi-secrets-plan.json --dry-run --allow-exec
mullusi secrets apply --from /tmp/mullusi-secrets-plan.json --json
```
...
- `mullusi.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.mullusi/.env` known secret keys whose values were migrated
