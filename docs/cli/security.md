...
It also warns when small models (`<=300B`) are used without sandboxing and with web/browser tools enabled. A safe local-first pattern is to set `agents.defaults.sandbox.mode="all"`, keep `tools.fs.workspaceOnly=true`, deny `["group:web", "browser"]` in `tools.sandbox.tools.deny`, and re-grant web/browser only on explicit sandboxed specialist agents that use a stronger non-small model.
