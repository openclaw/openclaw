...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    - `gateway install` installs and starts the service. `--force` rewrites an existing install and may restart a running Gateway; on Linux it reloads systemd, enables the unit, and restarts it. Verify the intended configuration and runtime before using forced installation to repair a stopped service.
    ...
    - On Linux, `gateway start` and `gateway restart` also refuse ineffective repairs when an operator-owned systemd drop-in overrides the command, working directory, or Gateway target environment (such as `OPENCLAW_STATE_DIR` or `OPENCLAW_CONFIG_PATH`). Inspect the effective unit with `systemctl --user cat <unit>.service`, then update or remove that drop-in. `gateway install --force` rewrites only the managed base unit, preserves drop-ins, and warns if a launcher override remains. Other `Environment=` drop-ins remain supported.
