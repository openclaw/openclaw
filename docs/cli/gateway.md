...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - A restart that completes native activation or accepted recovery but then fails its health check emits `action: "restart"`, `ok: false`, and `result: "restart-health-failed"`, retaining its error, hints, warnings, and exit code 1. This diagnostic does not claim a healthy Gateway or authorize another activation. Refusals, unexpected exceptions, and definition repair without confirmed activation do not emit this result. A scheduled restart remains an acceptance result, not proof of successor health.
