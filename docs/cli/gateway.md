...
- Set `OPENCLAW_GATEWAY_FAST_CONFIG=1` (opt-in, off by default) to defer pre-ready channel startup maintenance and startup session migration into background tasks instead of blocking plugin bootstrap on them. This lowers time-to-ready, but means channel maintenance and session migration may still be settling when the Gateway reports ready; failures in the deferred tasks are surfaced as warnings rather than swallowed. Leave it unset to keep the default behavior, where startup waits for both tasks before plugin loading continues.
