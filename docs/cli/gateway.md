...
- `OPENCLAW_GATEWAY_FAST_CONFIG=1` (opt-in, off by default) defers pre-ready channel startup maintenance and startup session migration into background tasks instead of blocking plugin bootstrap on them, lowering time-to-ready. Channel maintenance and session migration may still be settling when the Gateway reports ready; deferred-task failures surface as warnings rather than being swallowed. Unset keeps the default blocking behavior, where startup waits for both tasks before plugin loading continues.
