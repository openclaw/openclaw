...
- Set `OPENCLAW_STARTUP_WATCHDOG_MS=<ms>` to override the gateway startup watchdog threshold. Default is `60000` (60s); set `0` to disable. The watchdog arms once at the top of the `gateway run` startup loop and is cancelled after `startupTrace.mark("http.bound")` fires — i.e. only after every loopback bind host (typically `127.0.0.1` and `::1` on dual-stack Linux) has completed listen. If the threshold elapses before that, the watchdog writes one line directly to stderr with the shape `[startup-watchdog] stuck step="<phase>" elapsed=<ms> threshold=<ms>ms pending=["<outer>"@<ms>ms ...]` and exits without re-arming. The stderr write bypasses `OPENCLAW_GATEWAY_STARTUP_TRACE` and the logger pipeline so the diagnostic still surfaces when normal logging is starved or filtered. Malformed values (negative, `NaN`, non-numeric, `Infinity`) disable the watchdog rather than silently falling back to the default — so the misconfiguration becomes visible (no spurious lines) rather than re-arming an unrequested default.
