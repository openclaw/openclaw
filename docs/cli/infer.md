...
| Generate an image             | `openclaw infer image generate --prompt "..." --json`                                         | Repeat `--file` for provider-supported reference inputs |
...
- `image generate` accepts optional repeatable `--file` reference inputs. `image edit` and `image describe-many` require at least one `--file`; `embedding create` requires at least one `--text`. Repeat the flag for multiple inputs. Omitting a required input is a usage error, not an empty successful result, and no inference request is sent.
...
openclaw infer image generate --file ./reference.png --prompt "create an image using this style" --json
...
- `image generate --file` and `image edit --file` send files through the same provider-specific reference/edit path. Providers may treat them as edit inputs or style references; use `image edit` when the command should explicitly describe an edit.
- `--size`, `--aspect-ratio`, or `--resolution` add geometry hints on providers/models that support them.
