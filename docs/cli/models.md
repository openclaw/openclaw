...
`models auth logout <profileId>` removes one saved auth profile from its credential owner and removes matching authentication from generated model catalogs. It preserves model inventory, other profiles for that provider, and independent credentials owned by other agents. Use the profile id shown by `models auth list`. It also drops that profile from `auth.profiles` and from every `auth.order` list in your config, so no stale reference is left behind, and it deletes an `auth.order.<provider>` entry that would otherwise be emptied (an authored empty order means "select no profiles" and would disable the provider). It prompts for confirmation on a TTY; pass `--yes` for scripts and agents. Logout refuses when the profile is not in the store, or when a `models.providers.<id>.apiKey` entry names it — change that config value first.

Logout invalidates catalog work and sign-in attempts already in progress. If an auth command reports that authentication changed while it was running, start a fresh sign-in or explicit credential import. Doctor does not recreate missing credentials from generated caches; configured credentials and user-authored root catalogs retain their explicit repair path.

Logout removes local credential state; it does not revoke a token at the provider. Secret-redacted backups omit agent caches. Restoring an older full backup that includes credentials can restore those credentials; use the provider's revocation controls when that must also be prevented.
