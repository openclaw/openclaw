...
- The Control UI resolves the descriptor through an authenticated Gateway resource endpoint and renders the document inline under the tool summary, including while the tool disclosure itself is collapsed. The outer frame has an opaque origin, the nested app frame receives the extension-required `allow-scripts allow-same-origin` sandbox, and the app-declared CSP (`_meta.ui.csp`) is enforced by the proxy response header.
