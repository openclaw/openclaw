---
summary: "CLI reference for `mullusi qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
mullusi qr
mullusi qr --setup-code-only
mullusi qr --json
mullusi qr --remote
mullusi qr --url wss://gateway.example/ws
```
...
- With `--remote`, Mullusi requires either `gateway.remote.url` or
