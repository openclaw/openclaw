---
summary: "CLI reference for `mullusi node` (headless node host)"
read_when:
...
- `--port <port>`: Gateway WebSocket port (default: `18790`)
...
`mullusi node run` and `mullusi node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `MULLUSI_GATEWAY_TOKEN` / `MULLUSI_GATEWAY_PASSWORD` are checked first.
...
Use `mullusi node run` for a foreground node host (no service).
...
- `mullusi approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Mullusi prepares a canonical `systemRunPlan`
