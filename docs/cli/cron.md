---
summary: "CLI reference for `mullusi cron` (schedule and run background jobs)"
read_when:
...
Tip: run `mullusi cron --help` for the full command surface.
...
Note: `mullusi cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `mullusi cron runs --id <job-id>` to follow the eventual outcome.

Note: `mullusi cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.mullusi/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`mullusi doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
mullusi cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
mullusi cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
mullusi cron run <job-id>
mullusi cron run <job-id> --due
mullusi cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
mullusi cron edit <job-id> --agent ops
mullusi cron edit <job-id> --clear-agent
mullusi cron edit <job-id> --session current
mullusi cron edit <job-id> --session "session:daily-brief"
```
...
```bash
mullusi cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
mullusi cron edit <job-id> --best-effort-deliver
mullusi cron edit <job-id> --no-best-effort-deliver
mullusi cron edit <job-id> --no-deliver
```
