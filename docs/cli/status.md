...
- When persisted session cost is available, `/status` uses it instead of estimating from token counts and pricing tables; the bounded recent transcript fallback can also provide cost when present. `openclaw status` shows each recent session's persisted cost in the Sessions table when available.
