...
- When a transcript usage log includes `usage.cost.total`, `/status` uses that transcript-backed session cost instead of estimating from token counts and pricing tables. `openclaw status` also shows each recent session's persisted cost in the Sessions table when available.
