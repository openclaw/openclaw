...
- When transcript usage includes `usage.cost.total`, `/status` and `openclaw status` can show cumulative transcript-backed session cost instead of estimating from token counts and pricing tables.
