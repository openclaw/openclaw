...
| Externally managed repair | `openclaw doctor --fix --externally-managed --non-interactive` | Repairs offline runtime state while leaving deployment-owned config and service lifecycle unchanged. |
...
### Externally managed repair

Use the externally managed posture when a deployment controller owns both
`openclaw.json` and the Gateway process lifecycle:

```bash
openclaw doctor --fix --externally-managed --non-interactive --json
```
...
authorize work outside the state-only deployment handoff.
...
| `--externally-managed`          | With `--fix --non-interactive`, repair offline runtime state without changing deployment-owned config or Gateway service lifecycle. Add `--json` for the structured convergence report.                                                               |
