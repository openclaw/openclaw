---
summary: "CLI reference for `mullusi channels` (accounts, status, login/logout, logs)"
read_when:
...
Tip: `mullusi channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
...
When you run `mullusi channels add` without flags, the interactive wizard can prompt:
...
- Run `mullusi status --deep` for a broad probe.
- Use `mullusi doctor` for guided fixes.
- `mullusi channels list` prints `Claude: HTTP 403 ... user:profile` → usage snapshot needs the `user:profile` scope. Use `--no-usage`, or provide a claude.ai session key (`CLAUDE_WEB_SESSION_KEY` / `CLAUDE_WEB_COOKIE`), or re-auth via Claude CLI.
- `mullusi channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
