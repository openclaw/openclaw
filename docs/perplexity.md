...
Mullusi supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Mullusi also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `mullusi configure --section web`. It stores the key in
`~/.mullusi/mullusi.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.mullusi/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
