...
<AccordionGroup>
  ...
  <Accordion title="Panel lub Control UI nie łączy się">
    ```bash
    openclaw status
    openclaw gateway status
    openclaw logs --follow
  ...
  <Accordion title="Cron lub heartbeat nie uruchomił się lub nie dostarczył">
    ```bash
    openclaw status
    openclaw gateway status
  ...
  <Accordion title="Węzeł jest sparowany, ale narzędzie nie wykonuje camera/canvas/screen/exec">
    ```bash
    openclaw status
    openclaw gateway status
