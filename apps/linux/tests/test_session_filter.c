...
static void test_system_noise_filters(void) {
    ...
    g_assert_false(session_filter_is_system_noise("agent:main:main"));
}
...
static void test_build_choices_adds_default(void) {
    ...
    /*
     * Canonical main-session key is `agent:<agent>:main`; the label is
     * the short suffix `main`. This must stay in sync with the server's
     * `resolveMainSessionKey()` — a drift here bricks the chat surface
     * because chat.send targets a session the gateway has never heard of.
     */
    g_assert_cmpstr(c->key, ==, "agent:alice:main");
    g_assert_cmpstr(c->label, ==, "main");
}
...
static void test_build_choices_filters_and_labels(void) {
    ...
    g_assert_cmpstr(c2->key, ==, "agent:alice:main");
...
static void test_no_duplicate_default(void) {
    GatewaySession sessions[2] = {
        make_session("agent:alice:main", NULL, NULL),
    ...
    for (guint i = 0; i < choices->len; i++) {
        ...
        if (g_strcmp0(c->key, "agent:alice:main") == 0) {
            default_count++;
        }
    }
