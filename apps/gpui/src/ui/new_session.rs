...
use crate::{
    ...
    model::{attachments::Attachment, new_session::DraftSession, sessions::SessionRow},
};
use gpui_kit::{
...
impl NewSessionUi {
    ...
    pub fn sync_toolbar(&mut self, default_model: Option<String>) {
        self.toolbar_row = SessionRow {
            agent_id: Some(self.draft.agent_id.clone()),
            display_name: Some("New chat".into()),
            model: self.draft.model.clone().or(default_model),
            thinking_level: self.draft.thinking_level.clone(),
