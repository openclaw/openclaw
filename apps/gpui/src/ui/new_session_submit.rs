...
use crate::{
    gateway::{
        new_session_rpc::{
            CreateDraftParams, CreateDraftResult, DescribedSessionResult, DispatchResult, Project,
...
impl AppView {
    ...
    pub(super) fn submit_new_session(&mut self, cx: &mut Context<Self>) {
        ...
        if let Some(pending) = &self.new_session.submitted {
            ...
            let mut create = match draft.create_params(&message, attachments) {
                Ok(params) => params,
                Err(error) => {
            ...
            let prepare_project = !draft.destination.is_remote()
                && draft.project_id.trim().is_empty()
                && !draft.project_git_url.trim().is_empty()
            ...
            self.new_session.submitted = Some(PendingDraftSubmission {
                ...
                phase: if prepare_project {
                    SubmissionPhase::PreparingProject
                } else {
                    SubmissionPhase::Creating
                },
                project_url: draft.project_git_url.clone(),
                gateway,
                recovery_scope,
                boot_id,
    ...
    fn advance_new_session(&mut self, cx: &mut Context<Self>) {
        ...
        match pending.phase {
            SubmissionPhase::PreparingProject => {
                let request = json!({"gitUrl":pending.project_url});
                self.request("projects.add", request, cx, move |this, result, cx| {
                    if !this.pending_draft_matches(&id) {
                        return;
                    }
                    match result.and_then(|value| {
                        serde_json::from_value::<Project>(value).map_err(|error| error.to_string())
                    }) {
                        Ok(project) => {
                            let pending = this.new_session.submitted.as_mut().unwrap();
                            pending.create.project_id = Some(project.id);
                            pending.create.cwd = None;
                            pending.phase = SubmissionPhase::Creating;
                            this.advance_new_session(cx);
                        }
                        Err(error) => {
                            this.new_session.submitted = None;
                            this.fail_new_session(format!(
                                "Project could not be prepared: {error}"
                            ));
                        }
                    }
                });
