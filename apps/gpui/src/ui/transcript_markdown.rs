...
pub(in crate::ui) fn code_markdown_extensions() -> MarkdownExtensions {
    extensions(false)
}
fn extensions(user: bool) -> MarkdownExtensions {
    static ASSISTANT: OnceLock<MarkdownExtensions> = OnceLock::new();
    static USER: OnceLock<MarkdownExtensions> = OnceLock::new();
    (if user { &USER } else { &ASSISTANT })
        .get_or_init(|| build_extensions(user, None))
        .clone()
}
    ...
    match node {
        ...
        Node::Paragraph(paragraph) if paragraph.children.len() == 1 => match &paragraph.children[0]
        {
            Node::Link(link) => image_destination(&link.url).map(|url| {
                ...
                MarkdownNode::new("transcript-image", image_block(&label, &url)).text(label)
            }),
            _ => None,
        },
        Node::Blockquote(_) => {
...
fn image_block(label: &str, url: &str) -> ImageBlock {
    let image = url
        .strip_prefix("data:")
        .and_then(|data| data.split_once(","))
        .and_then(|(metadata, data)| {
            let mime = metadata.strip_suffix(";base64")?;
            if !matches!(
                mime,
                "image/png" | "image/jpeg" | "image/gif" | "image/webp"
            ) || data.len() > 5 * 1024 * 1024 * 4 / 3 + 4
            {
                return None;
            }
