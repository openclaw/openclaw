...
impl AppView {
    ...
    pub(super) fn sync_draft_toolbar(&mut self) {
        ...
        self.new_session.sync_toolbar(model);
    }
    ...
    pub(super) fn patch_draft_settings(&mut self, fields: &Value, cx: &mut Context<Self>) {
        ...
        for (name, target) in [
            ...
            ("thinkingLevel", &mut draft.thinking_level),
            ("permissionMode", &mut draft.permission_mode),
        ] {
        ...
        self.sync_draft_toolbar();
        if fields.get("model").is_some() {
            self.load_draft_destinations(cx);
        }
        cx.notify();
    }

    pub(super) fn sync_draft_inputs(&self, window: &mut Window, cx: &mut Context<Self>) {
        for (input, value) in [
            (
    ...
    pub(super) fn load_draft_catalogs(&mut self, cx: &mut Context<Self>) {
        ...
        self.request(
            ...
            move |this, result, cx| {
                ...
                this.load_draft_destinations(cx);
            },
        );
        self.request(
            "commands.list",
