...
impl DraftSession {
    ...
    pub fn create_params(
        ...
        let params = CreateDraftParams {
            ...
            agent_runtime: model
                .as_ref()
                .and_then(|_| option_text(&self.agent_runtime)),
            model,
            context_window: catalog_id
                .is_none()
                .then(|| option_text(&self.context_window))
                .flatten(),
            thinking_level: catalog_id
                .is_none()
                .then(|| option_text(&self.thinking_level))
                .flatten(),
            fast_mode: catalog_id
                .is_none()
                .then(|| self.fast_mode.clone())
                .flatten(),
...
mod tests {
    ...
    fn local_create_preserves_initial_turn_and_selected_controls() {
        ...
        let params = serde_json::to_value(
            draft
                .create_params("  Keep bytes  ", vec![attachment()])
                .unwrap(),
        )
        .unwrap();
        assert_eq!(
            ...
            json!({
                ...
                "model":"openai/gpt-5.5","agentRuntime":"codex","thinkingLevel":"high",
    ...
    fn all_remote_destinations_defer_initial_turn_and_flatten_dispatch() {
        ...
        for (destination, fields) in destinations {
            ...
            let params = serde_json::to_value(
                draft
                    .create_params("  first turn  ", vec![attachment()])
                    .unwrap(),
            )
            .unwrap();
            assert_eq!(
    ...
    fn worktree_wire_uses_base_ref_and_unprefixed_validated_name() {
        ...
        assert_eq!(
            serde_json::to_value(draft.create_params("Fix", vec![]).unwrap()).unwrap(),
            json!({
        ...
        for invalid in [
            ...
            assert!(draft.create_params("Fix", vec![]).is_err(), "{invalid}");
        }
        for valid in ["", "   ", "a", "0-", &"a".repeat(64)] {
            draft.worktree_name = valid.into();
            assert!(draft.create_params("Fix", vec![]).is_ok(), "{valid}");
        }
    }
    ...
    fn remote_repository_and_fresh_workspace_retire_previous_checkout_fields() {
        ...
        assert_eq!(
            serde_json::to_value(draft.create_params(&message, vec![]).unwrap()).unwrap(),
            json!({
        ...
        assert_eq!(
            serde_json::to_value(draft.create_params("Private topic", vec![]).unwrap()).unwrap(),
            json!({
    ...
    fn catalog_owns_model_controls_and_local_remote_projects_need_an_initial_turn() {
        let mut draft = DraftSession {
            ...
            thinking_level: Some("high".into()),
            fast_mode: Some(json!(true)),
            project_git_url: "https://example.com/repo.git".into(),
            ..Default::default()
        };
        assert_eq!(
            serde_json::to_value(draft.create_params("", vec![]).unwrap()).unwrap(),
            json!({"agentId":"main","message":"","catalogId":"terminal"})
        );
        draft.visibility = Visibility::Draft;
        let params = serde_json::to_value(draft.create_params("Clone", vec![]).unwrap()).unwrap();
        assert_eq!(
            params,
