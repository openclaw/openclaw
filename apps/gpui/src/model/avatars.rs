...
pub fn agent_face_svg(id: &str) -> String {
    let seed = fnv1a_utf16(id);
    let hue = [8, 32, 48, 82, 142, 174, 202, 232, 272, 322][seed as usize % 10];
    let eye_seed = seed / 10;
    let mouth_seed = eye_seed / 4;
    let pastel = mouth_seed / 4 % 2 == 1;
    let color = format!("hsl({hue},58%,62%)");
    let pale = format!("hsl({hue},65%,90%)");
    let ink = format!("hsl({hue},55%,18%)");
    let eyes = [
