...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      MullusiCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      MullusiCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      MullusiCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      MullusiCanvasA2UICommand.Push.rawValue, MullusiCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      MullusiCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      MullusiCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      MullusiCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      MullusiLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      MullusiDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      MullusiDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      MullusiDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      MullusiDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      MullusiNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      MullusiNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      MullusiSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.mullusi.app.protocol.MullusiPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      MullusiContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      MullusiContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      MullusiCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      MullusiCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      MullusiMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      MullusiMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      MullusiSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      MullusiSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      MullusiCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
