...
private const val NOTIFICATION_CHANNEL_BASE_ID = "mullusi.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Mullusi Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Mullusi Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Mullusi Active")
      }
