...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Mullusi Android gateway connection failure.
    ...
    - tell me whether `mullusi devices list` should show a pending pairing request
    - if more signal is needed, ask for `mullusi qr --json`, `mullusi devices list`, and `mullusi nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Mullusi gateway diagnostics", report))
