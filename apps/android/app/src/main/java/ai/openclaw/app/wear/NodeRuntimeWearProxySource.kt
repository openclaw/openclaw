...
internal class NodeRuntimeWearProxySource(
  private val app: NodeApp,
  private val clock: () -> Long = System::currentTimeMillis,
) : PhoneWearProxySource {
  override suspend fun snapshot(): PhoneWearProxyResult =
    app
      .peekRuntime()
      ?.let { runtime -> PhoneWearProxyResult(snapshot = runtime.toWearSnapshot()) }
      ?: phoneNotReady()
  ...
  override suspend fun sendMessage(message: String): PhoneWearProxyResult {
    ...
    return if (accepted) {
      PhoneWearProxyResult(snapshot = runtime.toWearSnapshot())
    } else {
  ...
  override suspend fun selectSession(sessionId: String): PhoneWearProxyResult {
    ...
    return PhoneWearProxyResult(snapshot = runtime.toWearSnapshot())
  }

  override suspend fun abort(): PhoneWearProxyResult {
    val runtime = app.peekRuntime() ?: return phoneNotReady()
    if (!runtime.gatewayConnectionDisplay.value.isConnected) return gatewayOffline()
    runtime.abortChat()
    return PhoneWearProxyResult(snapshot = runtime.toWearSnapshot())
  ...
  private fun phoneNotReady(): PhoneWearProxyResult =
    PhoneWearProxyResult(
      errorCode = WearConversationErrorCode.PHONE_NOT_READY,
    )

  private fun gatewayOffline(): PhoneWearProxyResult =
    PhoneWearProxyResult(
      errorCode = WearConversationErrorCode.GATEWAY_OFFLINE,
    )

  private fun notFound(): PhoneWearProxyResult =
    PhoneWearProxyResult(
