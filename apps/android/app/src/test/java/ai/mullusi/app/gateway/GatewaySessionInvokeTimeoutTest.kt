...
class GatewaySessionInvokeTimeoutTest {
  ...
  fun formatGatewayAuthority_bracketsIpv6Hosts() {
    assertEquals("[::1]:18790", formatGatewayAuthority("::1", 18_789))
  }

  @Test
  fun buildGatewayWebSocketUrl_bracketsIpv6Hosts() {
    assertEquals("ws://[::1]:18790", buildGatewayWebSocketUrl("::1", 18_789, useTls = false))
  ...
  fun replaceCanvasCapabilityInScopedHostUrl_rewritesTerminalCapabilitySegment() {
    assertEquals(
      "http://127.0.0.1:18790/__mullusi__/cap/new-token",
      replaceCanvasCapabilityInScopedHostUrl(
        "http://127.0.0.1:18790/__mullusi__/cap/old-token",
        "new-token",
      ),
    )
  }
  ...
  fun replaceCanvasCapabilityInScopedHostUrl_rewritesWhenQueryAndFragmentPresent() {
    assertEquals(
      "http://127.0.0.1:18790/__mullusi__/cap/new-token?a=1#frag",
      replaceCanvasCapabilityInScopedHostUrl(
        "http://127.0.0.1:18790/__mullusi__/cap/old-token?a=1#frag",
        "new-token",
      ),
    )
  }
