...
Supported devices show Android's embedded photo picker without whole-library
permission. Other devices, or a failed embedded picker, offer the system picker.
File also provides video selection. Location requests foreground permission and
adds a map link to the draft for review before sending. Closing a picker preserves
the draft; switching conversations retires the opening. Embedded gallery selections
enter the draft only after Done. Android grants access when an item is selected;
dismissing the sheet does not undo that platform grant.
