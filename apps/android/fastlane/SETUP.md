...
- `pnpm android:release:archive` builds the signed Play AAB, third-party APK, and Wear OS AAB into `apps/android/build/release-artifacts/`.
- `pnpm android:release:upload` uploads the Play AAB to the configured Google Play track. The default track is `internal`.
- The Wear OS AAB uses the same signing identity, a distinct Wear `versionCode`, and release-APK manifest validation.
  It is not uploaded automatically until the Android owner approves the Wear product and track rollout.
