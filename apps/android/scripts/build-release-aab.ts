...
function parseVersionMatches(buildGradleText: string): ParsedVersionMatches {
	...
	if (!versionCodeMatch || !versionNameMatch) {
		throw new Error(
			`Couldn't parse versionName/versionCode from ${buildGradlePath}`,
		);
	}
	return { versionCodeMatch, versionNameMatch };
}

function resolveNextVersionCode(
	...
	return Number.parseInt(
		`${todayPrefix}${nextSuffix.toString().padStart(2, "0")}`,
		10,
	);
}
...
function resolveNextVersion(buildGradleText: string, date: Date): VersionState {
	...
	const versionCode = resolveNextVersionCode(
		currentVersionCode,
		formatVersionCodePrefix(date),
	);
	return { versionName, versionCode };
}

function updateBuildGradleVersions(
