...
echo "Starting Mission Control for chat e2e smoke on ${BASE_URL}..."
MISSION_CONTROL_API_KEY="${TEST_API_KEY}" \
MISSION_CONTROL_CSRF_PROTECTION="false" \
PORT="${PORT}" npm run start >"${LOG_DIR}/next-start-chat.log" 2>&1 &
server_pid="$!"
...
if [[ "${ready}" != "true" ]]; then
  ...
  tail -n 80 "${LOG_DIR}/next-start-chat.log" || true
  exit 1
fi

MC_TEST_BASE_URL="${BASE_URL}" MC_TEST_API_KEY="${TEST_API_KEY}" npm run test:chat-e2e
