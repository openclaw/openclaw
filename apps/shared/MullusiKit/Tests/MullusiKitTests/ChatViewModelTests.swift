...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> MullusiChatHistoryPayload
{
    MullusiChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> MullusiChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> MullusiChatSessionEntry
{
    MullusiChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> MullusiChatModelChoice {
    MullusiChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [MullusiChatHistoryPayload],
    sessionsResponses: [MullusiChatSessionsListResponse] = [],
    modelResponses: [[MullusiChatModelChoice]] = [],
    ...
    -> (TestChatTransport, MullusiChatViewModel)
{
...
private func sendUserMessage(_ vm: MullusiChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, MullusiChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MullusiChatHistoryPayload]
    private let sessionsResponses: [MullusiChatSessionsListResponse]
    private let modelResponses: [[MullusiChatModelChoice]]
    ...
    private let stream: AsyncStream<MullusiChatTransportEvent>
    private let continuation: AsyncStream<MullusiChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<MullusiChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MullusiChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MullusiChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [MullusiChatAttachmentPayload]) async throws -> MullusiChatSendResponse
    {
        ...
        return MullusiChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MullusiChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MullusiChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MullusiChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = MullusiChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = MullusiChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: MullusiChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(MullusiChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == MullusiChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = MullusiChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: MullusiChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = MullusiChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = MullusiChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = MullusiChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { MullusiChatViewModel(sessionKey: "main", transport: transport) }
