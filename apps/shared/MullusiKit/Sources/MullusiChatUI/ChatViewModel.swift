...
private let chatUILogger = Logger(subsystem: "ai.mullusi", category: "MullusiChatUI")

@MainActor
@Observable
public final class MullusiChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [MullusiChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [MullusiChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [MullusiPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MullusiChatPendingToolCall] = []
    public private(set) var sessions: [MullusiChatSessionEntry] = []
    private let transport: any MullusiChatTransport
    private var sessionDefaults: MullusiChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: MullusiChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any MullusiChatTransport,
    ...
    public var sessionChoices: [MullusiChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [MullusiChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MullusiPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MullusiChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MullusiChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: MullusiChatMessage) -> MullusiChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> MullusiChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return MullusiChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: MullusiChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: MullusiChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: MullusiChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [MullusiChatMessage],
        incoming: [MullusiChatMessage]) -> [MullusiChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MullusiChatMessage]) -> [MullusiChatMessage] {
    ...
    private static func dedupeKey(for message: MullusiChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MullusiChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> MullusiChatAttachmentPayload in
            MullusiChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MullusiChatSessionEntry {
        MullusiChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = MullusiChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: MullusiChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MullusiChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MullusiAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MullusiChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MullusiPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
