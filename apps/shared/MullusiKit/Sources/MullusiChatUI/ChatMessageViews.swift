...
struct ChatMessageBubble: View {
    let message: MullusiChatMessage
    let style: MullusiChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? MullusiChatTheme.userText : MullusiChatTheme.assistantText
    ...
    private var inlineAttachments: [MullusiChatMessageContent] {
    ...
    private var toolCalls: [MullusiChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [MullusiChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? MullusiChatTheme.userBubble
        }
        if self.style == .onboarding {
            return MullusiChatTheme.onboardingAssistantBubble
        }
        return MullusiChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return MullusiChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: MullusiChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? MullusiChatTheme.userText : MullusiChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: MullusiChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MullusiChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(MullusiChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [MullusiChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: MullusiChatTheme.assistantText)
            }
        }
    }
}
