...
private final class TestChatTransport: @unchecked Sendable, EasyHubChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [EasyHubChatHistoryPayload]
    private let sessionsResponses: [EasyHubChatSessionsListResponse]

    private let stream: AsyncStream<EasyHubChatTransportEvent>
    private let continuation: AsyncStream<EasyHubChatTransportEvent>.Continuation

    init(
        historyResponses: [EasyHubChatHistoryPayload],
        sessionsResponses: [EasyHubChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<EasyHubChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> EasyHubChatHistoryPayload {
        ...
        return self.historyResponses.last ?? EasyHubChatHistoryPayload(
        ...
        attachments _: [EasyHubChatAttachmentPayload]) async throws -> EasyHubChatSendResponse
    {
        ...
        return EasyHubChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> EasyHubChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? EasyHubChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: EasyHubChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = EasyHubChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = EasyHubChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { EasyHubChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = EasyHubChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = EasyHubChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = EasyHubChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { EasyHubChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
