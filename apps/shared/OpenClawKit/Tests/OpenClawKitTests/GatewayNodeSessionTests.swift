...
struct GatewayNodeSessionTests {
    ...
    @Test
    func storedDeviceTokenIsScopedToGatewayStableID() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        let previousStateDir = ProcessInfo.processInfo.environment["OPENCLAW_STATE_DIR"]
        ...
        let blockedSession = FakeGatewayWebSocketSession()
        let blockedGateway = GatewayNodeSession()
        let blockedOptions = GatewayConnectOptions(
            role: "operator",
            scopes: ["operator.read"],
            caps: [],
            commands: [],
        ...
        try await blockedGateway.connect(
            url: URL(string: "ws://example.invalid")!,
            token: nil,
            bootstrapToken: nil,
            password: nil,
            connectOptions: blockedOptions,
            sessionBox: WebSocketSessionBox(session: blockedSession),
            onConnected: {},
            onDisconnected: { _ in },
            onInvoke: { req in
                BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: nil, error: nil)
        ...
        let blockedAuth = try #require(blockedSession.latestTask()?.latestConnectAuth())
        #expect(blockedAuth["token"] == nil)
        #expect(blockedAuth["bootstrapToken"] == nil)
        await blockedGateway.disconnect()

        let allowedSession = FakeGatewayWebSocketSession()
        let allowedGateway = GatewayNodeSession()
        let allowedOptions = GatewayConnectOptions(
            role: "operator",
            scopes: ["operator.read"],
            caps: [],
            commands: [],
        ...
        try await allowedGateway.connect(
            url: URL(string: "ws://example.invalid")!,
            token: nil,
            bootstrapToken: nil,
            password: nil,
            connectOptions: allowedOptions,
            sessionBox: WebSocketSessionBox(session: allowedSession),
            onConnected: {},
            onDisconnected: { _ in },
            onInvoke: { req in
                BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: nil, error: nil)
        ...
        let allowedAuth = try #require(allowedSession.latestTask()?.latestConnectAuth())
        #expect(allowedAuth["token"] as? String == "stored-device-token")
        await allowedGateway.disconnect()
    }

    @Test
    func legacyUnscopedTokenCanMigrateIntoScopedGatewaySlot() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        let previousStateDir = ProcessInfo.processInfo.environment["OPENCLAW_STATE_DIR"]
        ...
        let options = GatewayConnectOptions(
            ...
            clientDisplayName: "iOS Test",
            gatewayStableID: "gateway-a",
            allowLegacyUnscopedDeviceTokenFallback: true,
            includeDeviceIdentity: true)
        ...
        let auth = try #require(session.latestTask()?.latestConnectAuth())
        #expect(auth["token"] as? String == "legacy-device-token")
        await gateway.disconnect()
    }

    @Test
    func preservesStoredTokensForMultipleGateways() {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try? FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        let previousStateDir = ProcessInfo.processInfo.environment["OPENCLAW_STATE_DIR"]
        ...
        #expect(
            DeviceAuthStore.loadToken(
                deviceId: identity.deviceId,
                role: "operator",
                gatewayStableID: "gateway-a")?.token == "token-a")
        #expect(
            DeviceAuthStore.loadToken(
                deviceId: identity.deviceId,
                role: "operator",
                gatewayStableID: "gateway-b")?.token == "token-b")
    }
