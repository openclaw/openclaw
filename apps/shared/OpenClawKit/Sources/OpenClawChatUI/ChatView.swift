...
@MainActor
public struct EasyHubChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: EasyHubChatViewModel
    ...
    public init(
        viewModel: EasyHubChatViewModel,
    ...
    private var visibleMessages: [EasyHubChatMessage] {
        let base: [EasyHubChatMessage]
    ...
    private func mergeToolResults(in messages: [EasyHubChatMessage]) -> [EasyHubChatMessage] {
    ...
    private func isToolResultMessage(_ message: EasyHubChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: EasyHubChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: EasyHubChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(EasyHubChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
