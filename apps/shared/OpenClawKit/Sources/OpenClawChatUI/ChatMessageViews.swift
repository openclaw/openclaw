...
struct ChatMessageBubble: View {
    let message: EasyHubChatMessage
    let style: EasyHubChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? EasyHubChatTheme.userText : EasyHubChatTheme.assistantText
    ...
    private var inlineAttachments: [EasyHubChatMessageContent] {
    ...
    private var toolCalls: [EasyHubChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [EasyHubChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? EasyHubChatTheme.userBubble
        }
        if self.style == .onboarding {
            return EasyHubChatTheme.onboardingAssistantBubble
        }
        return EasyHubChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return EasyHubChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: EasyHubChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? EasyHubChatTheme.userText : EasyHubChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: EasyHubChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(EasyHubChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [EasyHubChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: EasyHubChatTheme.assistantText)
            }
        }
    }
}
