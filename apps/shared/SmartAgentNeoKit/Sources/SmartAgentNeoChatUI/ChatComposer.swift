...
@MainActor
struct SmartAgentNeoChatComposer: View {
    @Bindable var viewModel: SmartAgentNeoChatViewModel
    let style: SmartAgentNeoChatView.Style
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SmartAgentNeoChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SmartAgentNeoChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SmartAgentNeoPendingAttachment.id)
                { (att: SmartAgentNeoPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SmartAgentNeoPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(SmartAgentNeoChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SmartAgentNeoChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SmartAgentNeoChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SmartAgentNeoChatTheme.subtleCard)
        .clipShape(Capsule())
    }
