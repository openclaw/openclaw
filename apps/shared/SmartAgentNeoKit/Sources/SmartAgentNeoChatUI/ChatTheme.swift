...
enum SmartAgentNeoChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("SmartAgentNeoChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("SmartAgentNeoChatTheme.onboardingAssistantBubble"),
...
enum SmartAgentNeoPlatformImageFactory {
    static func image(_ image: SmartAgentNeoPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
