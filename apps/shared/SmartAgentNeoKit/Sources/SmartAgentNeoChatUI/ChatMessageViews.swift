...
struct ChatMessageBubble: View {
    let message: SmartAgentNeoChatMessage
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SmartAgentNeoChatTheme.userText : SmartAgentNeoChatTheme.assistantText
    ...
    private var inlineAttachments: [SmartAgentNeoChatMessageContent] {
    ...
    private var toolCalls: [SmartAgentNeoChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SmartAgentNeoChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SmartAgentNeoChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SmartAgentNeoChatTheme.onboardingAssistantBubble
        }
        return SmartAgentNeoChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SmartAgentNeoChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SmartAgentNeoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SmartAgentNeoChatTheme.userText : SmartAgentNeoChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SmartAgentNeoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SmartAgentNeoChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            if self.style == .standard {
                Text("SmartAgentNeo is thinking…")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                Spacer()
            }
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SmartAgentNeoChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SmartAgentNeoChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SmartAgentNeoChatTheme.assistantText)
            }
        }
    }
}
