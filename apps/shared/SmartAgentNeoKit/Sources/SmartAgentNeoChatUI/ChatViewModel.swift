...
private let chatUILogger = Logger(subsystem: "ai.smart-agent-neo", category: "SmartAgentNeoChatUI")

@MainActor
@Observable
public final class SmartAgentNeoChatViewModel {
    public private(set) var messages: [SmartAgentNeoChatMessage] = []
    ...
    public var attachments: [SmartAgentNeoPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SmartAgentNeoChatPendingToolCall] = []
    public private(set) var sessions: [SmartAgentNeoChatSessionEntry] = []
    private let transport: any SmartAgentNeoChatTransport
    ...
    private var pendingToolCallsById: [String: SmartAgentNeoChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SmartAgentNeoChatTransport) {
    ...
    public var sessionChoices: [SmartAgentNeoChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [SmartAgentNeoChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SmartAgentNeoPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SmartAgentNeoChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SmartAgentNeoChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func messageIdentityKey(for message: SmartAgentNeoChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [SmartAgentNeoChatMessage],
        incoming: [SmartAgentNeoChatMessage]) -> [SmartAgentNeoChatMessage]
    {
        ...
        return incoming.map { message in
            ...
            return SmartAgentNeoChatMessage(
                id: reusedId,
                role: message.role,
    ...
    private static func dedupeMessages(_ messages: [SmartAgentNeoChatMessage]) -> [SmartAgentNeoChatMessage] {
    ...
    private static func dedupeKey(for message: SmartAgentNeoChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SmartAgentNeoChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> SmartAgentNeoChatAttachmentPayload in
            SmartAgentNeoChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SmartAgentNeoChatSessionEntry {
        SmartAgentNeoChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SmartAgentNeoChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SmartAgentNeoChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SmartAgentNeoAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SmartAgentNeoChatPendingToolCall(
    ...
    private func addImageAttachment(url: URL?, data: Data, fileName: String, mimeType: String) async {
        ...
        self.attachments.append(
            SmartAgentNeoPendingAttachment(
                url: url,
                data: data,
                fileName: fileName,
                mimeType: mimeType,
    ...
    private static func previewImage(data: Data) -> SmartAgentNeoPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
