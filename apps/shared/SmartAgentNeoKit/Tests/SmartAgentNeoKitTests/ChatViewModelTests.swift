...
private final class TestChatTransport: @unchecked Sendable, SmartAgentNeoChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SmartAgentNeoChatHistoryPayload]
    private let sessionsResponses: [SmartAgentNeoChatSessionsListResponse]

    private let stream: AsyncStream<SmartAgentNeoChatTransportEvent>
    private let continuation: AsyncStream<SmartAgentNeoChatTransportEvent>.Continuation

    init(
        historyResponses: [SmartAgentNeoChatHistoryPayload],
        sessionsResponses: [SmartAgentNeoChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SmartAgentNeoChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SmartAgentNeoChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SmartAgentNeoChatHistoryPayload(
        ...
        attachments _: [SmartAgentNeoChatAttachmentPayload]) async throws -> SmartAgentNeoChatSendResponse
    {
        ...
        return SmartAgentNeoChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SmartAgentNeoChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SmartAgentNeoChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SmartAgentNeoChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SmartAgentNeoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SmartAgentNeoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SmartAgentNeoChatViewModel(sessionKey: "main", transport: transport) }
        ...
        transport.emit(
            .agent(
                SmartAgentNeoAgentEventPayload(
                    runId: sessionId,
                    seq: 1,
                    stream: "assistant",
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SmartAgentNeoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SmartAgentNeoChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SmartAgentNeoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SmartAgentNeoChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
