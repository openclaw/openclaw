...
/**
 * API endpoint to start a bot gateway
 */
export const startBot: PayloadHandler = async (req, res) => {
  ...
  try {
    ...
    if (req.user?.role !== 'admin') {
      ...
      if (!userBots.includes(botId)) {
        return res.status(403).json({
          error: 'You do not have permission to start this bot'
        })
      }
    }
    ...
    req.payload.logger.error(`Failed to start bot ${botId}: ${error}`)
