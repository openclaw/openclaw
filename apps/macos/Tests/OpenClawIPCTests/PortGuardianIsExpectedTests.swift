...
struct PortGuardianIsExpectedTests {
    @Test @MainActor
    func `direct remote diagnostics do not invent a local tunnel listener`() async throws {
    ...
    @Test(arguments: [AppState.RemoteTransport.direct, .ssh]) @MainActor
        ...
        transport: AppState.RemoteTransport) async throws
    {
        ...
        try await TestIsolation.withIsolatedState(env: [
            ...
            let reports = await PortGuardian.shared.diagnose(mode: .remote, hostsLocalGateway: true)
            #expect(reports.map(\.port) == (transport == .ssh ? [49218, 49217] : [49217]))
