...
@Suite struct VoiceWakeRuntimeTests {
    @Test func trimsAfterTriggerKeepsPostSpeech() {
        let triggers = ["claude", "smart-agent-neo"]
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["smart-agent-neo"]
        let text = "hey smart-agent-neo"
    ...
    @Test func trimsAfterChineseTriggerKeepsPostSpeech() {
        let triggers = ["小爪", "smart-agent-neo"]
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey smart-agent-neo do thing"
        let segments = makeSegments(
            transcript: transcript,
            words: [
                ("hey", 0.0, 0.1),
                ("smart-agent-neo", 0.2, 0.1),
                ("do", 0.35, 0.1),
                ("thing", 0.5, 0.1),
            ])
        let config = WakeWordGateConfig(triggers: ["smart-agent-neo"], minPostTriggerGap: 0.3)
