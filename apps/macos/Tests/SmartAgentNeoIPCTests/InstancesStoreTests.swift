...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SmartAgentNeoProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SmartAgentNeoProtocol.AnyCodable] = [
            "presence": .init([SmartAgentNeoProtocol.AnyCodable(entry)]),
        ]
        let payload = SmartAgentNeoProtocol.AnyCodable(payloadMap)
