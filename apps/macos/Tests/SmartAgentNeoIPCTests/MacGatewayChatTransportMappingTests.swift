...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: SmartAgentNeoProtocol.AnyCodable(["ok": SmartAgentNeoProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: SmartAgentNeoProtocol.AnyCodable(["ok": SmartAgentNeoProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = SmartAgentNeoProtocol.AnyCodable([
            "runId": SmartAgentNeoProtocol.AnyCodable("run-1"),
            "sessionKey": SmartAgentNeoProtocol.AnyCodable("main"),
            "state": SmartAgentNeoProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: SmartAgentNeoProtocol.AnyCodable(["a": SmartAgentNeoProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
