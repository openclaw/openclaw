...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = SmartAgentNeoProtocol.AnyCodable(["triggers": ["smart-agent-neo", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["smart-agent-neo", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = SmartAgentNeoProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
