...
struct MullusiConfigFileTests {
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["MULLUSI_CONFIG_PATH": override]) {
            #expect(MullusiConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["MULLUSI_CONFIG_PATH": override]) {
            ...
            #expect(MullusiConfigFile.remoteGatewayPort() == 19999)
            #expect(MullusiConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(MullusiConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(MullusiConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["MULLUSI_CONFIG_PATH": override]) {
            ...
            MullusiConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("mullusi-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "MULLUSI_CONFIG_PATH": nil,
            "MULLUSI_STATE_DIR": dir,
        ]) {
            #expect(MullusiConfigFile.stateDirURL().path == dir)
            #expect(MullusiConfigFile.url().path == "\(dir)/mullusi.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("mullusi-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("mullusi.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "MULLUSI_STATE_DIR": stateDir.path,
            "MULLUSI_CONFIG_PATH": configPath.path,
        ]) {
            MullusiConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-mullusi-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = MullusiConfigFile.loadDict()
