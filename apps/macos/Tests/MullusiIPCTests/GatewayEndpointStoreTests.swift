...
struct GatewayEndpointStoreTests {
    ...
    @Test func `resolve gateway token prefers env and falls back to launchd`() {
        let snapshot = self.makeLaunchAgentSnapshot(
            env: ["MULLUSI_GATEWAY_TOKEN": "launchd-token"],
            token: "launchd-token",
            password: nil)
        ...
        let envToken = GatewayEndpointStore._testResolveGatewayToken(
            isRemote: false,
            root: [:],
            env: ["MULLUSI_GATEWAY_TOKEN": "env-token"],
    ...
    @Test func resolveGatewayPasswordFallsBackToLaunchd() {
        let snapshot = self.makeLaunchAgentSnapshot(
            env: ["MULLUSI_GATEWAY_PASSWORD": "launchd-pass"],
            token: nil,
    ...
    @Test func `local config uses local gateway auth and host resolution`() {
        ...
        #expect(config.url.absoluteString == "wss://100.64.1.8:18790")
    ...
    @Test func `dashboard URL uses local base path in local mode`() throws {
        let config: GatewayConnection.Config = try (
            url: #require(URL(string: "ws://127.0.0.1:18790")),
            token: nil,
            password: nil)
        ...
        #expect(url.absoluteString == "http://127.0.0.1:18790/control/")
    }

    @Test func `dashboard URL skips local base path in remote mode`() throws {
        let config: GatewayConnection.Config = try (
            url: #require(URL(string: "ws://gateway.example:18790")),
            token: nil,
            password: nil)
        ...
        #expect(url.absoluteString == "http://gateway.example:18790/")
    }
    ...
    @Test func `dashboard URL uses fragment token and omits password`() throws {
        ...
        let url = try GatewayEndpointStore.dashboardURL(
            ...
            mode: .local,
            localBasePath: "/control")
        #expect(url.absoluteString == "http://127.0.0.1:18790/control/#token=abc123")
    ...
    @Test func `normalize gateway url adds default port for loopback ws`() {
        ...
        #expect(url?.port == 18790)
        #expect(url?.absoluteString == "ws://127.0.0.1:18790")
    }

    @Test func `normalize gateway url rejects non loopback ws`() {
        let url = GatewayRemoteConfig.normalizeGatewayUrl("ws://gateway.example:18790")
        #expect(url == nil)
    }
