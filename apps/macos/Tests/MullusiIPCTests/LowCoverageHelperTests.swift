...
struct LowCoverageHelperTests {
    ...
    @Test func `port guardian parses listeners and builds reports`() {
        ...
        let emptyReport = PortGuardian._testBuildReport(port: 18790, mode: .local, listeners: [])
    ...
    @Test func `port guardian remote mode does not kill docker`() {
        #expect(PortGuardian._testIsExpected(
            ...
            port: 18790, mode: .remote) == true)

        #expect(PortGuardian._testIsExpected(
            command: "ssh",
            fullCommand: "ssh -L 18790:localhost:18790 user@host",
            port: 18790, mode: .remote) == true)
    ...
    @Test func `port guardian local mode still rejects unexpected`() {
        ...
        #expect(PortGuardian._testIsExpected(
            command: "node",
            fullCommand: "mullusi-gateway",
            port: 18790, mode: .local) == true)
