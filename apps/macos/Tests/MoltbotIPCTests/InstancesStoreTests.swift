...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: DNAProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: DNAProtocol.AnyCodable] = [
            "presence": .init([DNAProtocol.AnyCodable(entry)]),
        ]
        let payload = DNAProtocol.AnyCodable(payloadMap)
