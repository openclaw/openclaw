...
final class QuickChatModel {
    ...
    var canSelectRecentSession: Bool {
        // Choosing a canonical recent establishes ownership; sending still needs an owned target.
        self.connectionGate == .available &&
            !self.isGrantingPermissions &&
            !self.isCapturingTextContext &&
            !self.isStartingDictation &&
    ...
    private func settleControlPatch(request: ControlPatchRequest) async -> Bool {
        ...
        do {
            ...
            if request.settings.fastMode != nil,
               self.retryIdentity?.routingIdentity.target.sessionKey == request.target.sessionKey,
               self.retryIdentity?.routingIdentity.target.agentID == request.target.agentID
            {
                self.retryIdentity = nil
            }
