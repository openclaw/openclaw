        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 12) {
                    SettingsToggleRow(
                        title: "\(Branding.appName) active",
                        subtitle: "Pause to stop the \(Branding.appName) gateway; no messages will be processed.",
                    ...
                    SettingsToggleRow(
                        ...
                        subtitle: "Automatically start \(Branding.appName) after you sign in.",
                        binding: self.$state.launchAtLogin)

                    SettingsToggleRow(
                        title: "Show Dock icon",
                        subtitle: "Keep \(Branding.appName) visible in the Dock instead of menu-bar-only mode.",
                ...
                DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        LabeledContent("CLI path") {
                            TextField("/Applications/\(Branding.appBundleName)/.../openclaw", text: self.$state.remoteCliPath)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: 280)
                        }
                    }
    ...
    private var remoteTokenRow: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if self.state.remoteTokenUnsupported {
                Text(
                    "The current gateway.remote.token value is not plain text. \(Branding.appName) for macOS cannot use it directly; enter a plaintext token here to replace it.")
                    .font(.caption)
