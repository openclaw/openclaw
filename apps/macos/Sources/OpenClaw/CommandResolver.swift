...
enum CommandResolver {
    private static let projectRootDefaultsKey = "EasyHub.gatewayProjectRootPath"
    private static let helperName = "EasyHub"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let EasyHubEntry = root.appendingPathComponent("easyhub.mjs").path
        if FileManager().isReadableFile(atPath: EasyHubEntry) { return EasyHubEntry }
        let binEntry = root.appendingPathComponent("bin/EasyHub.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/EasyHub")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let EasyHubPaths = self.EasyHubManagedPaths(home: home)
        if !EasyHubPaths.isEmpty {
            extras.insert(contentsOf: EasyHubPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + EasyHubPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func EasyHubManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".easyhub"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func EasyHubExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectEasyHubExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("easyhub.mjs").path,
            root.appendingPathComponent("bin/EasyHub.js").path,
        ]
    ...
    static func hasAnyEasyHubInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.EasyHubExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func EasyHubNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let EasyHubPath = self.projectEasyHubExecutable(projectRoot: root) {
                return [EasyHubPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "EasyHub", subcommand] + extraArgs
            }
            if let EasyHubPath = self.EasyHubExecutable(searchPaths: searchPaths) {
                return [EasyHubPath, subcommand] + extraArgs
            }

            let missingEntry = """
            EasyHub entrypoint missing (looked for dist/index.js or easyhub.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real EasyHub CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/EasyHub"
        ...
        \(cliSection)
        \(projectSection)
        if command -v EasyHub >/dev/null 2>&1; then
          CLI="$(command -v EasyHub)"
          EasyHub \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/easyhub.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/easyhub.mjs"
            node "$PRJ/easyhub.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/EasyHub.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/EasyHub.js"
            node "$PRJ/bin/EasyHub.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent EasyHub"
          pnpm --silent EasyHub \(quotedArgs);
        else
          echo "EasyHub CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? EasyHubConfigFile.loadDict()
