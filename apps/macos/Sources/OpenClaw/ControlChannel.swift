...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: EasyHubProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.easyhub", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: EasyHubKit.AnyCodable]()) {
                $0[$1.key] = EasyHubKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: EasyHubProtocol.AnyCodable?) -> [String: EasyHubProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: EasyHubProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: EasyHubKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: EasyHubProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("EasyHub.control.heartbeat")
    static let controlAgentEvent = Notification.Name("EasyHub.control.agent")
}
