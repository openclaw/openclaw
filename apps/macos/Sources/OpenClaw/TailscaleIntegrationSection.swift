...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.easyhub/easyhub.json. Prefer EASYHUB_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.easyhub/easyhub.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.easyhub/easyhub.json. Restart the gateway to apply."
        }
