...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.easyhub", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> EasyHubSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: EasyHubSessionPreviewEntry,
    ...
    private static func cache(payload: EasyHubSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: EasyHubChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [EasyHubKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [EasyHubKit.AnyCodable]) -> [EasyHubChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(EasyHubChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: EasyHubChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: EasyHubChatMessage) -> Bool {
    ...
    private static func toolNames(for message: EasyHubChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: EasyHubChatMessage) -> String? {
