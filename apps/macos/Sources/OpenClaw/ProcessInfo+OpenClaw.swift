...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.EasyHub.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["EASYHUB_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "EasyHub.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "EasyHub.nixMode") { return true }
