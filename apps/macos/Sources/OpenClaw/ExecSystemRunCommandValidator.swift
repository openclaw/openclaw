...
enum ExecSystemRunCommandValidator {
    ...
    static func resolve(command: [String], rawCommand: String?) -> ValidationResult {
        ...
        let canonicalArgv = ExecCommandFormatter.displayString(for: command)
        ...
        let previewShellPayload: String? =
            if let shellCommand, !envManipulationBeforeShellWrapper, !shellWrapperPositionalArgv {
                shellCommand
            } else {
                nil
            }

        if let raw = normalizedRaw, raw != canonicalArgv, raw != previewShellPayload {
            return .invalid(message: "INVALID_REQUEST: rawCommand does not match command")
        }

        return .ok(ResolvedCommand(displayCommand: canonicalArgv))
    }
