...
private let deepLinkLogger = Logger(subsystem: "ai.mullusi", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Mullusi is paused", message: "Unpause Mullusi to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Mullusi agent?", message: body) else { return }
        }
