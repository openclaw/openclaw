...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.mullusi", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["MULLUSI_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = MullusiConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
        let stored = UserDefaults.standard.integer(forKey: "gatewayPort")
        return stored > 0 ? stored : 18790
    }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.mullusiExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "mullusi CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["MULLUSI_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "mullusi@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "mullusi@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "mullusi@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "mullusi@\(target)"])
            }

        statusHandler("Installing mullusi@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed mullusi@\(target)")
        } else {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("mullusi ") {
            normalized = String(normalized.dropFirst("mullusi ".count))
        }
        return normalized
    }
