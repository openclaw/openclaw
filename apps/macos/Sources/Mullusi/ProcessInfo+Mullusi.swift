...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.mullusi.mac`) even if the shipped
    ...
    {
        if environment["MULLUSI_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "mullusi.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "mullusi.nixMode") { return true }
