...
private let webChatSwiftLogger = Logger(subsystem: "ai.mullusi", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "mullusi.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: MullusiChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> MullusiChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(MullusiChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            MullusiChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return MullusiChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MullusiChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MullusiGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: MullusiProtocol.ModelChoice) -> MullusiChatModelChoice {
        MullusiChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MullusiChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MullusiChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MullusiChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: MullusiChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MullusiChatView>) -> NSViewController
    {
