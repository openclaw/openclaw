...
enum DebugActions {
    private static let verboseDefaultsKey = "mullusi.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "mullusi.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Mullusi", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MullusiPaths.configURL
        guard
