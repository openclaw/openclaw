...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.mullusi", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MullusiSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MullusiSessionPreviewEntry,
    ...
    private static func cache(payload: MullusiSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MullusiChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MullusiKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MullusiKit.AnyCodable]) -> [MullusiChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MullusiChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MullusiChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MullusiChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MullusiChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MullusiChatMessage) -> String? {
