...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MullusiProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.mullusi", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MullusiKit.AnyCodable]()) {
                $0[$1.key] = MullusiKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MullusiProtocol.AnyCodable?) -> [String: MullusiProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MullusiProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MullusiKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MullusiProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("mullusi.control.heartbeat")
    static let controlAgentEvent = Notification.Name("mullusi.control.agent")
}
