...
enum CommandResolver {
    private static let projectRootDefaultsKey = "mullusi.gatewayProjectRootPath"
    private static let helperName = "mullusi"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let mullusiEntry = root.appendingPathComponent("mullusi.mjs").path
        if FileManager().isReadableFile(atPath: mullusiEntry) { return mullusiEntry }
        let binEntry = root.appendingPathComponent("bin/mullusi.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/mullusi")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let mullusiPaths = self.mullusiManagedPaths(home: home)
        if !mullusiPaths.isEmpty {
            extras.insert(contentsOf: mullusiPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + mullusiPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func mullusiManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".mullusi"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func mullusiExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMullusiExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("mullusi.mjs").path,
            root.appendingPathComponent("bin/mullusi.js").path,
        ]
    ...
    static func hasAnyMullusiInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.mullusiExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func mullusiNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let mullusiPath = self.projectMullusiExecutable(projectRoot: root) {
            return [mullusiPath, subcommand] + extraArgs
        }
        if let mullusiPath = self.mullusiExecutable(searchPaths: searchPaths) {
            return [mullusiPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "mullusi", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            mullusi entrypoint missing (looked for dist/index.js or mullusi.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real mullusi CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/mullusi"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/mullusi.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/mullusi.mjs"
            node "$PRJ/mullusi.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/mullusi.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/mullusi.js"
            node "$PRJ/bin/mullusi.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent mullusi"
          pnpm --silent mullusi \(quotedArgs);
        else
          echo "mullusi CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MullusiConfigFile.loadDict()
