...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MULLUSI_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/mullusi")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("mullusi-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("mullusi-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/mullusi/ (rolling or stdout), or nil if none exist.
