...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.mullusi/mullusi.json. Prefer MULLUSI_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.mullusi/mullusi.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.mullusi/mullusi.json. Restart the gateway to apply."
        }
