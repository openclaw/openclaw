...
actor CameraCaptureService {
    ...
    private let logger = Logger(subsystem: "ai.mullusi", category: "camera")
        ...
        outPath: String?) async throws -> (path: String, durationMs: Int, hasAudio: Bool)
    {
        ...
        let tmpMovURL = FileManager().temporaryDirectory
            .appendingPathComponent("mullusi-camera-\(UUID().uuidString).mov")
        ...
        let outputURL: URL = {
            ...
            return FileManager().temporaryDirectory
                .appendingPathComponent("mullusi-camera-\(UUID().uuidString).mp4")
        }()
