...
struct MullusiMacCLI {
    static func main() async {
        ...
        default:
            fputs("mullusi-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      ...
      mullusi-mac discover [--timeout <ms>] [--json] [--include-local]
      mullusi-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      mullusi-mac connect
      mullusi-mac connect --url ws://127.0.0.1:18790 --json
      mullusi-mac discover --timeout 3000 --json
      mullusi-mac wizard --mode local
    """)
}
