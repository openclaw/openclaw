...
struct ConnectOptions {
    ...
    var clientId: String = "mullusi-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Usage:
          mullusi-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: mullusi-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Mullusi macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Mullusi macOS Gateway Connect")
