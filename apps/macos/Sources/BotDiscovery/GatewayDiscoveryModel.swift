...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.bot", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in BotBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: BotBonjour.gatewayServiceType, domain: domain),
                using: params)
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = BotBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = BotBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Bot)", with: "")
