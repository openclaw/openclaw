...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: DNAOAuthStore.AnthropicOAuthStatus = DNAOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: DNAOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = DNAOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = DNAOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. DNA can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: DNAOAuthStore.AnthropicOAuthStatus,
