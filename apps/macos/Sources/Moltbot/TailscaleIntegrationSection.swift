...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.dna/dna.json. Prefer DNA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.dna/dna.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.dna/dna.json. Restart the gateway to apply."
        }
