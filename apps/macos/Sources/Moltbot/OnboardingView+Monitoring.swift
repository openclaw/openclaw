...
extension OnboardingView {
    ...
    func refreshLocalGatewayProbe() async {
        ...
        await MainActor.run {
            ...
            let expectedTokens = ["node", "dna", "tsx", "pnpm", "bun"]
    ...
    func refreshAnthropicOAuthStatus() {
        _ = DNAOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        let previous = self.anthropicAuthDetectedStatus
        let status = DNAOAuthStore.anthropicOAuthStatus()
    ...
    func verifyAnthropicOAuthIfNeeded(force: Bool = false) async {
        ...
        guard let refresh = DNAOAuthStore.loadAnthropicOAuthRefreshToken(), !refresh.isEmpty else {
        ...
        do {
            ...
            try DNAOAuthStore.saveAnthropicOAuth(updated)
            self.refreshAnthropicOAuthStatus()
