...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["DNA_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/dna")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("dna-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("dna-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/dna/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("dna") && $0.pathExtension == "log" }
            .max { lhs, rhs in
