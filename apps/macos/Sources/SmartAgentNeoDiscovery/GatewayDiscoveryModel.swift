...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.smart-agent-neo", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in SmartAgentNeoBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: SmartAgentNeoBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "ai.smartagentneo.macos.gateway-discovery.\(domain)"))
        }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = SmartAgentNeoBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = SmartAgentNeoBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (SmartAgentNeo)", with: "")
