...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.smart-agent-neo", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["SMART_AGENT_NEO_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = SmartAgentNeoConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.smartAgentNeoExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "smart-agent-neo CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["SMART_AGENT_NEO_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "smart-agent-neo@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "smart-agent-neo@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "smart-agent-neo@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "smart-agent-neo@\(target)"])
            }

        statusHandler("Installing smart-agent-neo@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed smart-agent-neo@\(target)")
        } else {
