...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SmartAgentNeoProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.smart-agent-neo", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SmartAgentNeoKit.AnyCodable]()) {
                $0[$1.key] = SmartAgentNeoKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SmartAgentNeoProtocol.AnyCodable?) -> [String: SmartAgentNeoProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SmartAgentNeoProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SmartAgentNeoKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SmartAgentNeoProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("smart-agent-neo.control.heartbeat")
    static let controlAgentEvent = Notification.Name("smart-agent-neo.control.agent")
}
