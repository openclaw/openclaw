...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.smart-agent-neo", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SmartAgentNeoPeekabooBridgeServices?
    private static var smartAgentNeoSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("SmartAgentNeo", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = SmartAgentNeoPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.smartAgentNeoSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.smartAgentNeoSocketPath, privacy: .public)")
    }
...
@MainActor
private final class SmartAgentNeoPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.smartagentneo.peekaboo")
