...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (SmartAgentNeo token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SmartAgentNeoOAuthStore.AnthropicOAuthStatus = SmartAgentNeoOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.smart-agent-neo", category: "anthropic-oauth")
...
enum SmartAgentNeoOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let smartAgentNeoOAuthDirEnv = "SMART_AGENT_NEO_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "SmartAgentNeo OAuth token file not found"
            case .unreadableFile: "SmartAgentNeo OAuth token file not readable"
            case .invalidJSON: "SmartAgentNeo OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in SmartAgentNeo OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "SmartAgentNeo OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.smartAgentNeoOAuthDirEnv]?
