...
enum DebugActions {
    private static let verboseDefaultsKey = "smart-agent-neo.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "smart-agent-neo.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SmartAgentNeoPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SmartAgentNeo", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SmartAgentNeoPaths.configURL
        guard
