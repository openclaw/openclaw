...
enum CommandResolver {
    private static let projectRootDefaultsKey = "smart-agent-neo.gatewayProjectRootPath"
    private static let helperName = "smart-agent-neo"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let smartAgentNeoEntry = root.appendingPathComponent("smart-agent-neo.mjs").path
        if FileManager().isReadableFile(atPath: smartAgentNeoEntry) { return smartAgentNeoEntry }
        let binEntry = root.appendingPathComponent("bin/smart-agent-neo.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__SMART_AGENT_NEO_ERR__' >&2
        \(message)
        __SMART_AGENT_NEO_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/smart-agent-neo")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let smartAgentNeoPaths = self.smartAgentNeoManagedPaths(home: home)
        if !smartAgentNeoPaths.isEmpty {
            extras.insert(contentsOf: smartAgentNeoPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + smartAgentNeoPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func smartAgentNeoManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".smart-agent-neo"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func smartAgentNeoExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSmartAgentNeoExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("smart-agent-neo.mjs").path,
            root.appendingPathComponent("bin/smart-agent-neo.js").path,
        ]
    ...
    static func hasAnySmartAgentNeoInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.smartAgentNeoExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func smartAgentNeoNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let smartAgentNeoPath = self.projectSmartAgentNeoExecutable(projectRoot: root) {
                return [smartAgentNeoPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "smart-agent-neo", subcommand] + extraArgs
            }
            if let smartAgentNeoPath = self.smartAgentNeoExecutable(searchPaths: searchPaths) {
                return [smartAgentNeoPath, subcommand] + extraArgs
            }

            let missingEntry = """
            smart-agent-neo entrypoint missing (looked for dist/index.js or smart-agent-neo.mjs); run pnpm build.
            """
    ...
    static func smartAgentNeoCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real smart-agent-neo CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/smart-agent-neo"
        ...
        \(cliSection)
        \(projectSection)
        if command -v smart-agent-neo >/dev/null 2>&1; then
          CLI="$(command -v smart-agent-neo)"
          smart-agent-neo \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/smart-agent-neo.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/smart-agent-neo.mjs"
            node "$PRJ/smart-agent-neo.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/smart-agent-neo.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/smart-agent-neo.js"
            node "$PRJ/bin/smart-agent-neo.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent smart-agent-neo"
          pnpm --silent smart-agent-neo \(quotedArgs);
        else
          echo "smart-agent-neo CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SmartAgentNeoConfigFile.loadDict()
