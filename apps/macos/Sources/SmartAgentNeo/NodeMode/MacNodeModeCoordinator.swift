...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.smart-agent-neo", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: SmartAgentNeoNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [SmartAgentNeoCapability.canvas.rawValue, SmartAgentNeoCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(SmartAgentNeoCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if SmartAgentNeoLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(SmartAgentNeoCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            SmartAgentNeoSystemCommand.which.rawValue,
            SmartAgentNeoSystemCommand.run.rawValue,
            SmartAgentNeoSystemCommand.execApprovalsGet.rawValue,
            SmartAgentNeoSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(SmartAgentNeoCapability.camera.rawValue) {
        ...
        if capsSet.contains(SmartAgentNeoCapability.location.rawValue) {
