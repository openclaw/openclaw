...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.smart-agent-neo", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous SmartAgentNeo run already has an SSH listener on the expected port (common after restarts),
