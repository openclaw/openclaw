...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.smart-agent-neo/smart-agent-neo.json. Prefer SMART_AGENT_NEO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.smart-agent-neo/smart-agent-neo.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.smart-agent-neo/smart-agent-neo.json. Restart the gateway to apply."
        }
