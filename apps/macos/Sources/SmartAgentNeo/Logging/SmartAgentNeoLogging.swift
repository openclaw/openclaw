...
enum SmartAgentNeoLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SmartAgentNeoOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SmartAgentNeoFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: labelSeparator) else {
            return ("ai.smart-agent-neo", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SmartAgentNeoLogging.bootstrapIfNeeded()
        let label = SmartAgentNeoLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SmartAgentNeoOSLogHandler: LogHandler {
...
struct SmartAgentNeoFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SmartAgentNeoLogging.parseLabel(self.label)
