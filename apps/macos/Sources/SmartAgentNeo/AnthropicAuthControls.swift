...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SmartAgentNeoOAuthStore.AnthropicOAuthStatus = SmartAgentNeoOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                Spacer()
                Button("Reveal") {
                    NSWorkspace.shared.activateFileViewerSelecting([SmartAgentNeoOAuthStore.oauthURL()])
                }
                .buttonStyle(.bordered)
                .disabled(!FileManager().fileExists(atPath: SmartAgentNeoOAuthStore.oauthURL().path))
            ...
            Text(SmartAgentNeoOAuthStore.oauthURL().path)
    ...
    private func refresh() {
        let imported = SmartAgentNeoOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SmartAgentNeoOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. SmartAgentNeo can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SmartAgentNeoOAuthStore.AnthropicOAuthStatus,
