...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.smartagentneo.mac`) even if the shipped
    ...
    {
        if environment["SMART_AGENT_NEO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "smart-agent-neo.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "smart-agent-neo.nixMode") { return true }
