...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("smart-agent-neo").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing smart-agent-neo CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed smart-agent-neo \($0)." } ?? "Installed smart-agent-neo."
    ...
    private static func installPrefix() -> String {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".smart-agent-neo")
            .path
    }
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://smart-agent-neo.bot/install-cli.sh | \
