...
private let deepLinkLogger = Logger(subsystem: "ai.smart-agent-neo", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "SmartAgentNeo is paused", message: "Unpause SmartAgentNeo to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run SmartAgentNeo agent?", message: body) else { return }
        }
