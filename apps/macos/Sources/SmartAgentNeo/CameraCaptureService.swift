...
actor CameraCaptureService {
    ...
    private let logger = Logger(subsystem: "ai.smart-agent-neo", category: "camera")
        ...
        outPath: String?) async throws -> (path: String, durationMs: Int, hasAudio: Bool)
    {
        ...
        let tmpMovURL = FileManager().temporaryDirectory
            .appendingPathComponent("smart-agent-neo-camera-\(UUID().uuidString).mov")
        ...
        let outputURL: URL = {
            ...
            return FileManager().temporaryDirectory
                .appendingPathComponent("smart-agent-neo-camera-\(UUID().uuidString).mp4")
        }()
