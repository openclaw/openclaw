...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so SmartAgentNeo can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = SmartAgentNeoLocationMode.off.rawValue
    ...
    private var locationMode: SmartAgentNeoLocationMode {
        SmartAgentNeoLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: SmartAgentNeoLocationMode) async -> Bool {
