...
private let webChatSwiftLogger = Logger(subsystem: "ai.smart-agent-neo", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SmartAgentNeoChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SmartAgentNeoChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SmartAgentNeoChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SmartAgentNeoChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SmartAgentNeoGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    SmartAgentNeoChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SmartAgentNeoAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SmartAgentNeoChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SmartAgentNeoChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SmartAgentNeoChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SmartAgentNeoChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "SmartAgentNeo Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SmartAgentNeoChatView>) -> NSViewController
    {
