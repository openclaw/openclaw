...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.smart-agent-neo", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SmartAgentNeoSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SmartAgentNeoSessionPreviewEntry,
    ...
    private static func cache(payload: SmartAgentNeoSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SmartAgentNeoChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SmartAgentNeoKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SmartAgentNeoKit.AnyCodable]) -> [SmartAgentNeoChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SmartAgentNeoChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SmartAgentNeoChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SmartAgentNeoChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SmartAgentNeoChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SmartAgentNeoChatMessage) -> String? {
