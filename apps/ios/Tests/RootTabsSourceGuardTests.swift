...
struct RootTabsSourceGuardTests {
    ...
    @Test func `professional layout avoids nested pills and card stacks`() throws {
        ...
        let appearanceMenu = try Self.extract(
            settingsSource,
            from: "var appearanceMenu: some View",
            to: "var gatewaySection: some View")
        ...
        #expect(!appearanceMenu.contains(".pickerStyle(.segmented)"))
    ...
    @Test func `settings privacy owns notification controls`() throws {
        ...
        #expect(notificationsSection.contains("Text(\"Notifications\")"))
        #expect(notificationsSection.matches(of: /ProCard\(/).count == 1)
        #expect(notificationsSection.contains("ProCard(padding: 0"))
        #expect(notificationsSection.contains("ProIconBadge(systemName: \"bell\", color: .secondary)"))
        #expect(notificationsSection.contains("settings-privacy-notifications-card"))
        #expect(!notificationsSection.contains("settings-notifications-status"))
        #expect(!notificationsSection.contains("notificationEffectiveStatusColor"))
        #expect(notificationsSection.contains("Toggle(\"Notifications\", isOn: self.notificationToggleBinding)"))
        #expect(notificationsSection.contains("settings-notifications-toggle"))
        #expect(notificationsSection.contains(".accessibilityValue(self.notificationsServingEnabled ? \"On\" : \"Off\")"))
        #expect(notificationsSection.contains("Divider().padding(.leading, 54)"))
        #expect(!notificationsSection.contains("self.handleNotificationAction()"))
        #expect(!notificationsSection.contains("buttonStyle(.borderedProminent)"))
        #expect(notificationsSection.contains(
            "OpenClaw can show approval prompts and event alerts when the app is not active."))
        #expect(!notificationsSection.contains("notificationStatusDetail"))
        #expect(notificationsSection.contains("settings-notifications-detail"))
        #expect(notificationsSection.contains("settings-notifications-relay-detail"))
        #expect(notificationsSection.contains("self.notificationRelayDetail"))
        let detailToRelay = try Self.extract(
            notificationsSection,
            from: "settings-notifications-detail",
            to: "settings-notifications-relay-detail")
        #expect(!detailToRelay.contains("Divider()"))
        #expect(!notificationsSection.contains("detail: \"\""))
        #expect(!notificationsSection.contains("systemName: \"network\""))
        #expect(actionsSource.contains("--openclaw-notification-status"))
    ...
    @Test func `push enrollment stays behind notification disclosure flow`() throws {
        ...
        #expect(actionsSource.contains("self.appModel.disableAPNsDeliveryRegistration()"))
        #expect(modelSource.contains("PushEnrollmentConsent.disclosureAccepted"))
        #expect(modelSource.contains("PushBuildConfig.current.usesOpenClawHostedRelay"))
        #expect(modelSource.contains("NotificationServingPreference.isServingEnabled(status: status)"))
        #expect(modelSource.contains("PushRelayRegistrationStore.clearRegistrationState()"))
        #expect(modelSource.contains("try await self.publishGatewayNodeEvent("))
        #expect(modelSource.contains("method: \"node.event\""))
        #expect(modelSource.contains("event: \"push.apns.unregister\""))
        #expect(modelSource.contains("clientRegistrationId"))
        #expect(modelSource.contains("makeAPNsUnregisterPayloadJSON"))
        #expect(modelSource.contains("result.event == \"push.apns.unregister\""))
        #expect(modelSource.contains("result.handled == true"))
        #expect(modelSource.contains("result.reason == \"missing\""))
        #expect(modelSource.contains("self.apnsClientRegistrationId == clientRegistrationId"))
        #expect(modelSource.contains("guard await self.canPublishAPNsRegistration(usesRelayTransport: usesRelayTransport) else"))
    ...
    @Test func `native chat uses gateway transport`() throws {
        ...
        #expect(appModelSource.contains("return IOSGatewayChatTransport(gateway: self.operatorSession)"))
        #expect(channelsSource.contains("Installed channel clients, account state, and message-routing readiness."))
