...
struct RootTabsPresentationTests {
    ...
    @Test func `legacy initial tabs map to matching sidebar destinations`() {
        ...
        #expect(RootTabs.shouldPresentTalkOnLaunch(arguments: ["OpenClaw", "--openclaw-initial-tab", "voice"]))
        #expect(RootTabs.shouldPresentTalkOnLaunch(arguments: ["OpenClaw", "--openclaw-initial-destination", "talk"]))
        #expect(RootTabs.normalizedSidebarDestination(.talk) == .chat)
