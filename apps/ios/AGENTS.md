...
- Token role balance matters after a view is on branded fonts. Reserve display/semibold tokens (`title*`, `headline*`, `subheadSemiBold`, `caption*Bold`) for screen titles, selected/active states, destructive/high-risk calls, and the one primary action in a region. Use body-family regular/medium tokens (`body`, `subhead`, `subheadMedium`, `caption`, `captionMedium`, `caption2Medium`) for repeated rows, secondary actions, neutral menu items, inactive segmented options, metadata, helper text, and dense operational surfaces.
- Shared controls should encode that hierarchy by default: prominent/primary controls may use `subheadSemiBold`; non-prominent controls should prefer `subheadMedium` or `captionMedium` unless the caller is explicitly marking an active/selected/destructive state.
