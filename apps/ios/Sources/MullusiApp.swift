...
@MainActor
final class MullusiAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.mullusi.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.mullusi.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.mullusi.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "mullusi.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "mullusi.watch.promptId"
    static let sessionKeyKey = "mullusi.watch.sessionKey"
    static let actionPrimaryIDKey = "mullusi.watch.action.primary.id"
    static let actionPrimaryLabelKey = "mullusi.watch.action.primary.label"
    static let actionSecondaryIDKey = "mullusi.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "mullusi.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "mullusi.watch.action.primary"
    static let actionSecondaryIdentifier = "mullusi.watch.action.secondary"
    static let actionIdentifierPrefix = "mullusi.watch.action."
    static let actionIDKeyPrefix = "mullusi.watch.action.id."
    static let actionLabelKeyPrefix = "mullusi.watch.action.label."
    static let categoryPrefix = "mullusi.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: MullusiWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> MullusiWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return MullusiWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Mullusi" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [MullusiWatchAction]) -> [UNNotificationAction] {
...
@main
struct MullusiApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(MullusiAppDelegate.self) private var appDelegate
...
extension MullusiApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Mullusi: installing uncaught exception handler")
