...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__MULLUSI_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<MullusiTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.mullusi", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = MullusiTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> MullusiTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return MullusiTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> MullusiTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> MullusiTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return MullusiTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> MullusiTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = MullusiTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: MullusiTalkPTTStopPayload) {
