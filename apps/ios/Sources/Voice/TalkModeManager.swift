...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SMART_AGENT_NEO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<SmartAgentNeoTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = SmartAgentNeoTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> SmartAgentNeoTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return SmartAgentNeoTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> SmartAgentNeoTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> SmartAgentNeoTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return SmartAgentNeoTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> SmartAgentNeoTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = SmartAgentNeoTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: SmartAgentNeoTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: SmartAgentNeoAgentEventPayload.self) else {
                continue
            }
