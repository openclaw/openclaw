...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SmartAgentNeoContactsSearchParams) async throws -> SmartAgentNeoContactsSearchPayload {
        ...
        return SmartAgentNeoContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SmartAgentNeoContactsAddParams) async throws -> SmartAgentNeoContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SmartAgentNeoContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SmartAgentNeoContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SmartAgentNeoContactPayload {
        SmartAgentNeoContactPayload(
