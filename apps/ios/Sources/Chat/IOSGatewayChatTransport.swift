...
struct IOSGatewayChatTransport: SmartAgentNeoChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.smart-agent-neo", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> SmartAgentNeoChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SmartAgentNeoChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SmartAgentNeoChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SmartAgentNeoChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SmartAgentNeoChatAttachmentPayload]) async throws -> SmartAgentNeoChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SmartAgentNeoChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(SmartAgentNeoChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(SmartAgentNeoGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SmartAgentNeoGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SmartAgentNeoChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SmartAgentNeoAgentEventPayload.self)
                        {
