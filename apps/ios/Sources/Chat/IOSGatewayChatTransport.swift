...
struct IOSGatewayChatTransport: OpenClawChatGatewayTransport {
    ...
    func scoped(toAgentID agentID: String) -> (any OpenClawChatTransport)? {
        // Scoping changes the fallback agent, never the captured account or route.
        // Reinitializing would either drop the native binding or restore its original agent.
        var scoped = self
        ...
        overrideAgentID: String? = nil) -> SessionTarget
    {
        if let nativeBinding, sessionKey.utf8.elementsEqual(nativeBinding.session.sessionKey.utf8) {
            return SessionTarget(sessionKey: sessionKey, agentID: overrideAgentID ?? self.globalAgentId)
        }
        ...
        replacing failedResource: OpenClawChatWidgetResource?) async -> OpenClawChatWidgetResource?
    {
        ...
        let refreshOperatorSurface: @Sendable (GatewayCanvasHostRoute?) async -> GatewayCanvasHostRoute? = { observed in
            await gateway.refreshCanvasHostRoute(
                ...
                profileObservationID: nativeBinding?.profileObservationID,
                onProfileObservation: { nativeBinding?.observe($0) })
        }
        ...
        playback: OpenClawChatPlaybackMode?) async throws -> OpenClawChatLoadedMedia?
    {
        ...
        guard await self.currentSessionMutationRoute() == route else { throw CancellationError() }
