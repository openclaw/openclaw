...
struct IOSGatewayChatTransport: DNAChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> DNAChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(DNAChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> DNAChatHistoryPayload {
        ...
        return try JSONDecoder().decode(DNAChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [DNAChatAttachmentPayload]) async throws -> DNAChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [DNAChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(DNAChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(DNAGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DNAGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DNAChatEventPayload.self)
                        {
