...
final class ScreenController {
    ...
    /// Callback invoked when an EasyHub:// deep link is tapped in the canvas
    ...
    fileprivate func applyDebugStatusIfNeeded() {
        ...
        (() => {
          try {
            const api = globalThis.__EasyHub;
            if (!api) return;
    ...
    func waitForA2UIReady(timeoutMs: Int) async -> Bool {
        ...
        while clock.now < deadline {
            do {
                ...
                (() => {
                  try {
                    const host = globalThis.EasyHubA2UI;
    ...
    func snapshotBase64(
        ...
        format: EasyHubCanvasSnapshotFormat,
...
/// Handles navigation policy to intercept EasyHub:// deep links from canvas
@MainActor
private final class ScreenNavigationDelegate: NSObject, WKNavigationDelegate {
        ...
        decisionHandler: @escaping @MainActor @Sendable (WKNavigationActionPolicy) -> Void)
    {
        ...
        // Intercept EasyHub:// deep links.
        if url.scheme?.lowercased() == "EasyHub" {
...
private final class CanvasA2UIActionMessageHandler: NSObject, WKScriptMessageHandler {
    static let messageName = "EasyHubCanvasA2UIAction"
