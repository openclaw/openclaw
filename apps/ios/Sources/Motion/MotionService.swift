...
final class MotionService: MotionServicing {
    func activities(params: MullusiMotionActivityParams) async throws -> MullusiMotionActivityPayload {
        ...
        let mapped: [MullusiMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return MullusiMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: MullusiPedometerParams) async throws -> MullusiPedometerPayload {
        ...
        let payload: MullusiPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = MullusiPedometerPayload(
