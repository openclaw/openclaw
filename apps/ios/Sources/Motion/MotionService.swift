...
final class MotionService: MotionServicing {
    func activities(params: SmartAgentNeoMotionActivityParams) async throws -> SmartAgentNeoMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SmartAgentNeoMotionActivityEntry], Error>) in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        SmartAgentNeoMotionActivityEntry(
        ...
        return SmartAgentNeoMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SmartAgentNeoPedometerParams) async throws -> SmartAgentNeoPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<SmartAgentNeoPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SmartAgentNeoPedometerPayload(
