...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        let message = DNACanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = DNACanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__dna__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: DNALocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for dna:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case DNACameraCommand.list.rawValue,
                 DNACameraCommand.snap.rawValue,
                 DNACameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case DNAScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: DNANodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DNANodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case DNACanvasA2UICommand.reset.rawValue:
            ...
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DNACanvasA2UICommand.push.rawValue, DNACanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(DNAScreenRecordParams.self, from: req.paramsJSON)) ??
            DNAScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> DNALocationMode {
