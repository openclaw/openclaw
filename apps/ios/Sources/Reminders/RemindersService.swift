...
final class RemindersService: RemindersServicing {
    func list(params: MullusiRemindersListParams) async throws -> MullusiRemindersListPayload {
        ...
        let payload: [MullusiReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return MullusiRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: MullusiRemindersAddParams) async throws -> MullusiRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = MullusiReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return MullusiRemindersAddPayload(reminder: payload)
    }
