...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> MullusiDeviceStatusPayload {
        ...
        return MullusiDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> MullusiDeviceInfoPayload {
        ...
        return MullusiDeviceInfoPayload(
    ...
    private func batteryStatus() -> MullusiBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: MullusiBatteryState = switch device.batteryState {
        ...
        return MullusiBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> MullusiThermalStatusPayload {
        let state: MullusiThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return MullusiThermalStatusPayload(state: state)
    }

    private func storageStatus() -> MullusiStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return MullusiStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
