...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> EasyHubNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> EasyHubNetworkStatusPayload {
        let status: EasyHubNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> EasyHubNetworkStatusPayload {
        EasyHubNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
