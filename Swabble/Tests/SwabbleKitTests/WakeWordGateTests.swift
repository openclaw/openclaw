...
@Suite struct WakeWordGateTests {
    @Test func matchRequiresGapAfterTrigger() {
        let transcript = "hey mullusi do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["mullusi"], minPostTriggerGap: 0.3)
    ...
    @Test func matchHandlesMultiWordTriggers() {
        let transcript = "hey mullusi do it"
        let segments = makeSegments(
            transcript: transcript,
            words: [
                ("hey", 0.0, 0.1),
        ...
        let config = WakeWordGateConfig(triggers: ["hey mullusi"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
    ...
    @Test func commandTextHandlesForeignRangeIndices() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "mullusi", start: 0.2, duration: 0.1, range: transcript.range(of: "mullusi")),
